        ...
        if (dist < pdebris->radius + pship->radius) {
                ...
                if (hit) {
                        ...
                        if (Ship_info[Ships[pship->instance].ship_info_index]
                                ...
                                ship_damage = MIN(
                                        ship_damage,
                                        cap_percent_damage * Ships[pship->instance].ship_max_hull_strength);
                        }
...
int collide_asteroid_ship(obj_pair *pair)
{
        ...
        if (dist < pasteroid->radius + pship->radius) {
                ...
                if (hit) {
                        ...
                        // 2*m*v_max_relative
                        float max_ship_impulse = (2.0f * pship->phys_info.max_vel.xyz.z + vm_vec_mag_quick(&asteroid_vel)) * (pship->phys_info.mass * pasteroid->phys_info.mass) / (pship->phys_info.mass + pasteroid->phys_info.mass);
