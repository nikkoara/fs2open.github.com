...
// const std::list<waypoint>::iterator INVALID_WAYPOINT_POSITION =
// dummy_waypoint.end();
...
vec3d* waypoint::get_pos () { return &m_position; }

int waypoint::get_objnum () { return objnum; }
...
char* waypoint_list::get_name () { return m_name; }
...
//********************FUNCTIONS********************
void waypoint_parse_init () { Waypoint_lists.clear (); }
...
void waypoint_create_game_object (
    ...
    wpt->objnum = obj_create (
        OBJ_WAYPOINT, -1, calc_waypoint_instance (list_index, wpt_index), NULL,
        wpt->get_pos (), 0.0f, default_flags);
}
...
void waypoint_create_game_objects () {
    ...
    for (ii = Waypoint_lists.begin (); ii != Waypoint_lists.end (); ++ii) {
        int wpt = 0;
        for (jj = ii->get_waypoints ().begin ();
             jj != ii->get_waypoints ().end (); ++jj) {
...
waypoint* find_waypoint_with_objnum (int objnum) {
    if (objnum < 0 || Objects[objnum].type != OBJ_WAYPOINT) return NULL;
    ...
    for (ii = Waypoint_lists.begin (); ii != Waypoint_lists.end (); ++ii) {
        for (jj = ii->get_waypoints ().begin ();
             jj != ii->get_waypoints ().end (); ++jj) {
            if (jj->get_objnum () == objnum) return &(*jj);
        }
    }
...
waypoint_list*
find_waypoint_list_with_instance (int waypoint_instance, int* waypoint_index) {
    if (waypoint_instance < 0) {
        if (waypoint_index != NULL) *waypoint_index = -1;
        return NULL;
    }

    waypoint_list* wp_list = find_waypoint_list_at_index (
        calc_waypoint_list_index (waypoint_instance));
    if (wp_list == NULL) {
        if (waypoint_index != NULL) *waypoint_index = -1;
        return NULL;
    }
...
waypoint* find_waypoint_with_instance (int waypoint_instance) {
    if (waypoint_instance < 0) return NULL;

    waypoint_list* wp_list = find_waypoint_list_at_index (
        calc_waypoint_list_index (waypoint_instance));
    if (wp_list == NULL) return NULL;

    return find_waypoint_at_index (
        wp_list, calc_waypoint_index (waypoint_instance));
}
...
waypoint_list* find_waypoint_list_at_index (int index) {
    ...
    for (ii = Waypoint_lists.begin (); ii != Waypoint_lists.end ();
         ++i, ++ii) {
        if (i == index) return &(*ii);
    }
...
waypoint* find_waypoint_at_index (waypoint_list* list, int index) {
    ...
    for (ii = list->get_waypoints ().begin ();
         ii != list->get_waypoints ().end (); ++i, ++ii) {
        if (i == index) return &(*ii);
    }
...
int find_index_of_waypoint (waypoint_list* wp_list, waypoint* wpt) {
    ...
    int index = 0;
    for (ii = wp_list->get_waypoints ().begin ();
         ii != wp_list->get_waypoints ().end (); ++ii) {
        if (&(*ii) == wpt) return index;
        index++;
    }
...
int waypoint_add (vec3d* pos, int waypoint_instance) {
    ...
    // create the waypoint on the same list as, and immediately after,
    // waypoint_instance
    else {
        calc_waypoint_indexes (waypoint_instance, wp_list_index, wp_index);
        wp_list = find_waypoint_list_at_index (wp_list_index);

        // theoretically waypoint_instance points to a current waypoint, so
        // advance past it
        wp_index++;
        ...
        for (object* objp = GET_FIRST (&obj_used_list);
            ...
            if ((objp->type == OBJ_WAYPOINT) &&
                (calc_waypoint_list_index (objp->instance) == wp_list_index) &&
                (calc_waypoint_index (objp->instance) >= wp_index))
                objp->instance++;
        }
    }
    ...
    for (i = 0; i < wp_index; i++) ++ii;
    wp_list->get_waypoints ().insert (ii, new_waypoint);
    wpt = find_waypoint_at_index (wp_list, wp_index);

    // apparently we create it in-game too; this is called by both scripting
    // and FRED
...
void waypoint_remove (waypoint* wpt) {
    ...
    if (wp_list->get_waypoints ().size () == 1) {
        ...
        if (Waypoint_lists.size () == 1) { Waypoint_lists.clear (); }
        // shift the other waypoint lists down
        else {
            // remove this particular waypoint list
            std::list< waypoint_list >::iterator ii;
            int i;
            for (i = 0, ii = Waypoint_lists.begin ();
                 ii != Waypoint_lists.end (); ++i, ++ii) {
                if (i == this_list) {
            ...
            for (object* objp = GET_FIRST (&obj_used_list);
                 ...
                 objp = GET_NEXT (objp)) {
                if ((objp->type == OBJ_WAYPOINT) &&
                    (calc_waypoint_list_index (objp->instance) > this_list))
    ...
    // shift the other waypoints down
        ...
        std::vector< waypoint >::iterator ii;
        int i;
        for (i = 0, ii = wp_list->get_waypoints ().begin ();
             ii != wp_list->get_waypoints ().end (); ++i, ++ii) {
            if (i == this_index) {
        ...
        for (object* objp = GET_FIRST (&obj_used_list);
            ...
            if ((objp->type == OBJ_WAYPOINT) &&
                (calc_waypoint_list_index (objp->instance) == this_list) &&
                (calc_waypoint_index (objp->instance) > this_index))
                objp->instance--;
        }
    }
