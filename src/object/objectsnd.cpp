...
typedef struct _obj_snd {
    ...
    sound_handle instance; // handle of currently playing sound (a ds3d handle
                           // if USES_DS3D flag set)
...
int Obj_snd_last_update; // timer used to run object sound updates at fixed
                         // time intervals
...
DCF (objsnd, "Persistent sound stuff") {
    ...
    if (dc_optional_string ("-list")) {
        for (osp = GET_FIRST (&obj_snd_list);
             osp != END_OF_LIST (&obj_snd_list); osp = GET_NEXT (osp)) {
            vec3d source_pos;
            float distance;
            ...
            dc_printf (
                "Object %d => name: %s vol: %.2f pan: %.2f dist: %.2f status: "
                "%s\n",
...
int obj_snd_get_slot () {
    ...
    for (i = 0; i < MAX_OBJ_SNDS; i++) {
        if (!(Objsnds[i].flags & OS_USED)) return i;
    }
...
void obj_snd_level_init () {
    ...
    for (i = 0; i < MAX_OBJ_SNDS; i++) { Objsnds[i].flags = 0; }
...
void obj_snd_stop (object* objp, int index) {
    ...
    if (index == -1) {
        ...
        for (std::vector< int >::iterator iter = objp->objsnd_num.begin ();
             iter != objp->objsnd_num.end (); ++iter) {
            if (*iter == -1) { continue; }
        ...
        if (objp->objsnd_num[index] == -1) { return; }
...
int obj_snd_stop_lowest_vol (float new_vol) {
    ...
    if (lowest_vol_osp != NULL) objp = &Objects[lowest_vol_osp->objnum];

    if ((lowest_vol < new_vol) && (objp != NULL)) {
        int idx = 0;
        // determine what index in this guy the sound is
        for (std::vector< int >::iterator iter = objp->objsnd_num.begin ();
             iter != objp->objsnd_num.end (); ++iter, ++idx) {
...
void maybe_play_flyby_snd (
    ...
    if (closest_dist < FLYBY_MIN_DISTANCE) {
        ...
        vec3d diff;
        vm_vec_sub (
            &diff, &listener_objp->phys_info.vel,
            &closest_objp->phys_info.vel);
        ...
        if (relative_speed > FLYBY_MIN_RELATIVE_SPEED) {
            if (timestamp_elapsed (Flyby_next_sound)) {
                ...
                ship_info* sip =
                    &Ship_info[Ships[closest_objp->instance].ship_info_index];
                game_snd* snd;
                ...
                if (snd->sound_entries.empty ())
                    return; // This species does not define any relevant flyby
                            // sounds
...
void obj_snd_do_frame () {
    float closest_dist, distance, speed_vol_multiplier, rot_vol_mult,
        percent_max, alive_vol_mult;
    obj_snd* osp;
    ...
    if (Obj_snd_enabled == FALSE) return;

    int now = timer_get_milliseconds ();
    if ((now - Obj_snd_last_update) > 100) { Obj_snd_last_update = now; }
    else {
        return;
    }
    ...
    if (Viewer_obj != NULL) { observer_obj = Viewer_obj; }
    else if (Viewer_mode & VM_OTHER_SHIP && Player_ai->target_objnum != -1) {
        // apparently Viewer_obj is still null when Viewing Other Ship
        // externally
    ...
    for (osp = GET_FIRST (&obj_snd_list); osp != END_OF_LIST (&obj_snd_list);
        ...
        add_distance = 0.0f;
        if (osp->flags & OS_MAIN) { add_distance = objp->radius; }
        ...
        // If the object is a ship, we don't want to start the engine sound
        // unless the ship is moving (unless flag SIF_BIG_SHIP is set)
        ...
        if (objp->type == OBJ_SHIP) {
            ...
            if (!(sip->is_big_or_huge ())) {
                if (objp->phys_info.max_vel.xyz.z <= 0.0f) {
                    ...
                    percent_max =
                        objp->phys_info.fspeed / objp->phys_info.max_vel.xyz.z;

                if (sip->min_engine_vol == -1.0f) {
                    // Retail behavior: volume ramps from 0.5 (when stationary)
                    // to 1.0 (when at half speed)
                    if (percent_max >= 0.5f) { speed_vol_multiplier = 1.0f; }
                    else {
                        speed_vol_multiplier =
                            0.5f + (percent_max); // linear interp: 0.5->1.0
                                                  // when 0.0->0.5
                    }
                }
                else {
                    // Volume ramps from min_engine_vol (when stationary)
                    // to 1.0 (when at full speed)
                    speed_vol_multiplier =
                        sip->min_engine_vol +
                        ((1.0f - sip->min_engine_vol) * percent_max);
                }
            }
            if (osp->ss != NULL) {
                if (osp->flags & OS_TURRET_BASE_ROTATION) {
                    if (osp->ss->base_rotation_rate_pct > 0.0f)
                        rot_vol_mult =
                            ((0.25f +
                              (0.75f * osp->ss->base_rotation_rate_pct)) *
                             osp->ss->system_info
                                 ->turret_base_rotation_snd_mult);
                    else
                        rot_vol_mult = 0.0f;
                }
                if (osp->flags & OS_TURRET_GUN_ROTATION) {
                    if (osp->ss->gun_rotation_rate_pct > 0.0f)
                        rot_vol_mult =
                            ((0.25f +
                              (0.75f * osp->ss->gun_rotation_rate_pct)) *
                             osp->ss->system_info
                                 ->turret_gun_rotation_snd_mult);
                    else
                        rot_vol_mult = 0.0f;
                }
        ...
        if (!osp->instance.isValid ()) {
            if (distance < gs->max) {
                ...
                if (distance <= gs->min) { new_vol = max_vol; }
                else {
                    new_vol = max_vol - (distance - gs->min) * max_vol /
                                            (gs->max - gs->min);
                }

                if (new_vol < 0.1f) { continue; }
                ...
                if (go_ahead_flag) {
                    osp->instance = snd_play_3d (
                        gs, &source_pos, &View_position, add_distance,
                        &objp->phys_info.vel, 1, 1.0f,
        ...
        if (!osp->instance.isValid ()) continue;

        sp = NULL;
        if (objp->type == OBJ_SHIP) sp = &Ships[objp->instance];
        ...
        if (sp == NULL ||
            ...
            snd_set_volume (
                osp->instance, gs->volume_range.next () *
                                   speed_vol_multiplier * rot_vol_mult *
                                   alive_vol_mult);
        }
        else {
...
int obj_snd_assign (
    ...
    if (objnum < 0 || objnum > MAX_OBJECTS) return -1;

    if (!sndnum.isValid ()) return -1;

    if (Obj_snd_enabled == FALSE) return -1;

    // Initialize object sound list, if not initialized
    if (!Obj_snd_level_inited) obj_snd_level_init ();
    ...
    if (main) { snd->flags |= OS_MAIN; }

    if (flags > 0) { snd->flags |= flags; }
...
void obj_snd_delete_type (int objnum, gamesnd_id sndnum, ship_subsys* ss) {
    ...
    if (objnum < 0 || objnum >= MAX_OBJECTS) return;
    ...
    for (std::vector< int >::iterator iter = objp->objsnd_num.begin ();
        ...
        // Also check if this is assigned to the right subsystem, if one has
        // been given.
...
// obj_snd_close()
//
// Called once at game close to de-initialize the persistent object sound
// system
//
void obj_snd_level_close () {
    if (!Obj_snd_level_inited) { return; }
    obj_snd_delete_all ();
...
int obj_snd_is_playing (int object, int index) {
    if (!obj_snd_return_instance (object, index).isValid ()) return 0;
...
sound_handle obj_snd_return_instance (int objnum, int index) {
    if (objnum < 0 || objnum >= MAX_OBJECTS) return sound_handle::invalid ();

    if (index < 0 || index >= MAX_OBJ_SNDS) return sound_handle::invalid ();
...
int obj_snd_update_offset (int objnum, int index, vec3d* new_offset) {
    ...
    if (index < 0 || index >= MAX_OBJ_SNDS) return 0;
