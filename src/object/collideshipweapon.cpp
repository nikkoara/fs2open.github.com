...
static int check_inside_radius_for_big_ships (
    object* ship, object* weapon_obj, obj_pair* pair);
extern float flFrametime;

/**
 * If weapon_obj is likely to hit ship_obj sooner than current
 * aip->danger_weapon_objnum, then update danger_weapon_objnum.
 */
static void update_danger_weapon (object* pship_obj, object* weapon_obj) {
    ...
    if (aip->danger_weapon_objnum == -1) {
        ...
        Objects[aip->danger_weapon_objnum].signature) {
        float danger_old_time, danger_new_time;

        danger_old_time = ai_endangered_time (
            pship_obj, &Objects[aip->danger_weapon_objnum]);
 ...
 */
static void ship_weapon_do_hit_stuff (
    object* pship_obj, object* weapon_obj, vec3d* world_hitpos, vec3d* hitpos,
    int quadrant_num, int submodel_num,
    vec3d /*not a pointer intentionaly*/ hit_dir) {
    ...
    model_instance_find_world_dir (
        &worldNormal, &hit_dir, shipp->model_instance_num, submodel_num,
        &pship_obj->orient);

    // Apply hit & damage & stuff to weapon
    weapon_hit (
        weapon_obj, pship_obj, world_hitpos, quadrant_num, &worldNormal);

    if (wip->damage_time >= 0.0f && wp->lifeleft <= wip->damage_time) {
        if (wip->atten_damage >= 0.0f) {
            damage =
                (((wip->damage - wip->atten_damage) *
                  (wp->lifeleft / wip->damage_time)) +
                 wip->atten_damage);
        }
        else {
    ...
    if ((Ships[pship_obj->instance].wing_status_wing_index >= 0) &&
        ((Ships[pship_obj->instance].wing_status_wing_pos >= 0))) {
        hud_wingman_status_start_flash (
    ...
    if (quadrant_num == -1 && wip->impact_decal.definition_handle >= 0) {
        ...
        vm_vector_2_matrix_norm (
            &decal_orient, &hit_dir); // hit_dir is already normalized so we
                                      // can use the more efficient function

        decals::addDecal (
            wip->impact_decal, pship_obj, submodel_num, *hitpos, decal_orient);
    }
}

extern int Framecount;

static int ship_weapon_check_collision (
    object* ship_objp, object* weapon_objp, float time_limit = 0.0f,
    int* next_hit = NULL) {
    ...
    // Goober5000 - I tried to make collision code here much saner... here
    // begin the (major) changes
    mc_info_init (&mc);
    ...
    // for putting real shields on the Lucifer.  This also fixes the strange
    // bug where shots will occasionally go through the shield mesh when they
    // shouldn't.  I don't know what causes this, but this fixes that --
    // shields will absorb it when it hits the hull instead.  This has no fancy
    // graphical effect, though. Someone should make one.
    ...
    if (!(ship_objp->flags[Object::Object_Flags::No_shields])) {
        if (sip->flags[Ship::Info_Flags::Auto_spread_shields]) {
            // The weapon is not allowed to impact the shield before it reaches
            // this point
            vec3d shield_ignored_until = weapon_objp->last_pos;

            float weapon_flown_for =
                vm_vec_dist (&wp->start_pos, &weapon_objp->last_pos);
            ...
            // If weapon hasn't yet flown a distance greater than the maximum
            // ignore range, then some part of the currently checked range
            // needs to be ignored
            if (weapon_flown_for < min_weapon_span) {
                vm_vec_sub (
                    &shield_ignored_until, &weapon_end_pos, &wp->start_pos);
            ...
            float this_range =
                vm_vec_dist (&weapon_objp->last_pos, &weapon_end_pos);

            // The range during which the weapon is not allowed to collide with
            // the shield, except if it actually hits the hull
            float ignored_range;

            // If the weapon has not yet surpassed the ignore range, calculate
            // the remaining ignore range
            if (vm_vec_dist (&wp->start_pos, &shield_ignored_until) >
                weapon_flown_for)
                ignored_range = vm_vec_dist (
                    &weapon_objp->last_pos, &shield_ignored_until);
            else
                ignored_range = 0.0f;
            ...
            if (ignored_range > 0.0f) {
                ...
                mc_shield.hit_dist =
                    mc_shield.hit_dist * (ignored_range / this_range);
            }

            // If no collision with the model found in the ignore range, only
            // then do we check for sphereline collisions with the model during
            // the non-ignored range
            if (!shield_collision &&
                weapon_flown_for + this_range > min_weapon_span) {
                ...
                mc_shield.p1 = &weapon_end_pos;
                mc_shield.hit_dist =
                    (ignored_range + (active_range * mc_shield.hit_dist)) /
                    this_range;
            }

            if (shield_collision) {
                // If we used a sphereline check, then the collision point will
                // lie somewhere on the ship's hull; this re-positions it to
                // lie on the correct point along the weapon's path
                ...
                // Re-calculate hit_point because it's likely pointing to the
                // wrong place
                vec3d tempv;
                vm_vec_sub (
                    &tempv, &mc_shield.hit_point_world, &ship_objp->pos);
                vm_vec_rotate (
                    &mc_shield.hit_point, &tempv, &ship_objp->orient);
            }
        }
            ...
            if (!shield_collision) {
                // But if no shield mesh or it was missed, check for a hull
                // collision
                mc_shield.flags = MC_CHECK_MODEL;
                shield_collision = model_collide (&mc_shield);

                // Because we used MC_CHECK_MODEL, the returned hit position
                // might be in a submodel's frame of reference, so we need to
                // ensure we end up in the ship's frame of reference
                vec3d local_pos;
                vm_vec_sub (
                    &local_pos, &mc_shield.hit_point_world, &ship_objp->pos);
                vm_vec_rotate (
                    &mc_shield.hit_point, &local_pos, &ship_objp->orient);
            }
        }
        else {
    ...
    if (shield_collision && mc_shield.flags == MC_CHECK_MODEL) {
        ...
        // The weapon has impacted on the hull, so if it should therefore
        // bypass the shields altogether, we do it here
        if (sip->auto_shield_spread_bypass) { shield_collision = 0; }
    }
    else {
    ...
    if (shield_collision) {
        ...
        if (quadrant_num >= 0) {
            ...
            if (mc_shield.shield_hit_tri != -1 &&
                (mc_shield.hit_dist * (flFrametime + time_limit) -
                 flFrametime) < 0.0f) {
                add_shield_point (
    ...
    if ((shipp->flags[Ship::Ship_Flags::Depart_warp]) &&
        (shipp->warpout_effect) && (valid_hit_occurred)) {
    ...
    // deal with predictive collisions.  Find their actual hit time and see if
    // they occured in current frame
    if (next_hit && valid_hit_occurred) {
        // find hit time
        *next_hit =
            (int)(1000.0f * (mc.hit_dist * (flFrametime + time_limit) - flFrametime));
        if (*next_hit > 0)
    ...
    if (valid_hit_occurred) {
        ...
        (wp->homing_object == ship_objp)) {
        if (dist < wip->shockwave.inner_rad) {
            vec3d vec_to_ship;

            vm_vec_normalized_dir (
                &vec_to_ship, &ship_objp->pos, &weapon_objp->pos);

            if (vm_vec_dot (&vec_to_ship, &weapon_objp->orient.vec.fvec) <
                0.0f) {
 ...
 * @param pair obj_pair pointer to the two objects. pair->a is ship and pair->b
 * is weapon.
...
int collide_ship_weapon (obj_pair* pair) {
    ...
    if (Player->control_mode > PCM_WARPOUT_STAGE1) {
        if (ship == Player_obj) return 0;
    }

    if (reject_due_collision_groups (ship, weapon_obj)) return 0;

    // Cull lasers within big ship spheres by casting a vector forward for (1)
    // exit sphere or (2) lifetime of laser If it does hit, don't check the
    // pair until about 200 ms before collision. If it does not hit and is
    // within error tolerance, cull the pair.

    if ((sip->is_big_or_huge ()) &&
        (Weapon_info[Weapons[weapon_obj->instance].weapon_info_index]
             .subtype == WP_LASER)) {
        // Check when within ~1.1 radii.
        // This allows good transition between sphere checking (leaving the
        // laser about 200 ms from radius) and checking within the sphere with
        // little time between.  There may be some time for "small" big ships
        // Note: culling ships with auto spread shields seems to waste more
        // performance than it saves, so we're not doing that here
        if (!(sip->flags[Ship::Info_Flags::Auto_spread_shields]) &&
            vm_vec_dist_squared (&ship->pos, &weapon_obj->pos) <
                (1.2f * ship->radius * ship->radius)) {
    ...
    if (!did_hit) {
        // Since we didn't hit, check to see if we can disable all future
...
static float estimate_ship_speed_upper_limit (object* ship, float time) {
    ...
    delta_v = Ship_info[Ships[ship->instance].ship_info_index].max_vel.xyz.z -
              ship->phys_info.speed;
...
// maximum error allowed in detecting collisions between laser and big ship
// inside the radius this is set for a 150 m radius ship.  For ships with
// larger radius, the error scales according to the ration of radii, but is
// never less than 2 m
#define ERROR_STD 2
...
static int check_inside_radius_for_big_ships (
    ...
    if (max_error < 2) max_error = 2.0f;

    time_to_exit_sphere =
        (ship->radius + vm_vec_dist (&ship->pos, &weapon_obj->pos)) /
        (weapon_obj->phys_info.max_vel.xyz.z - ship->phys_info.max_vel.xyz.z);
    ship_speed_at_exit_sphere =
        estimate_ship_speed_upper_limit (ship, time_to_exit_sphere);
    // update estimated time to exit sphere
    time_to_exit_sphere =
        (ship->radius + vm_vec_dist (&ship->pos, &weapon_obj->pos)) /
        (weapon_obj->phys_info.max_vel.xyz.z - ship_speed_at_exit_sphere);
    vm_vec_scale_add (
        &error_vel, &ship->phys_info.vel, &weapon_obj->orient.vec.fvec,
        -vm_vec_dot (&ship->phys_info.vel, &weapon_obj->orient.vec.fvec));
    error_vel_mag = vm_vec_mag_quick (&error_vel);
    error_vel_mag +=
        0.5f * (ship->phys_info.max_vel.xyz.z - error_vel_mag) *
        (time_to_exit_sphere / ship->phys_info.forward_accel_time_const);
    ...
    // if ship_weapon_check_collision comes back with a hit_time > error limit,
    // ok if ship_weapon_check_collision comes finds no collision, next check
    // time based on error time
    float
        limit_time; // furthest time to check (either lifetime or exit sphere)
    ...
    // modify ship_weapon_check_collision to do damage if hit_time is negative
    // (ie, hit occurs in this frame)
    if (ship_weapon_check_collision (
            ship, weapon_obj, limit_time, &hit_time)) {
        ...
        if (limit_time > time_to_max_error) {
            ...
            if (1000 * time_to_max_error > 200) {
                pair->next_check_time =
                    timestamp ((int)(1000 * time_to_max_error) - 200);
            }
            else {
                pair->next_check_time = 1;
            }
