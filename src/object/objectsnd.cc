        ...
        for (osp = GET_FIRST(&obj_snd_list); osp != END_OF_LIST(&obj_snd_list);
             osp = GET_NEXT(osp)) {
                        ...
                        if (!(sip->is_big_or_huge())) {
                                ...
                                if (sip->min_engine_vol == -1.0f) {
                                        ...
                                        if (percent_max >= 0.5f) {
                                                ...
                                                speed_vol_multiplier = 0.5f + (percent_max); // linear interp: 0.5->1.0
                                                                                             // when 0.0->0.5
                                        }
                                } else {
                ...
                if (!osp->instance.isValid()) {
                        if (distance < gs->max) {
                                ...
                                if (distance <= gs->min) {
                                        ...
                                        new_vol = max_vol - (distance - gs->min) * max_vol / (gs->max - gs->min);
                                }
                ...
                if (sp == NULL || ((sp != NULL) && (sp->flags[Ship::Ship_Flags::Engines_on]))) {
                        snd_set_volume(
                                osp->instance, gs->volume_range.next() * speed_vol_multiplier * rot_vol_mult * alive_vol_mult);
                } else {
...
void obj_snd_delete_type(int objnum, gamesnd_id sndnum, ship_subsys *ss)
{
        ...
        for (std::vector< int >::iterator iter = objp->objsnd_num.begin();
             iter != objp->objsnd_num.end(); ++iter, ++idx) {
                ...
                // been given.
                if (((sndnum.isValid()) && (osp->id != sndnum)) || ((ss != NULL) && (osp->ss != ss))) {
                        continue;
                }
