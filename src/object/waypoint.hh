...
private:
    ...
    friend void
    waypoint_create_game_object (waypoint* wpt, int list_index, int wpt_index);
};
...
waypoint_list* find_waypoint_list_with_instance (
    int waypoint_instance, int* waypoint_index = NULL);
...
// Attempts to create a waypoint with the specified instance (used to calculate
// list and index). Returns the object number, or -1 on failure.  Used by
// scripting and FRED.
int waypoint_add (vec3d* pos, int waypoint_instance);

// Removes a waypoint, including its entire list if it's the last waypoint
// remaining.
void waypoint_remove (waypoint* wpt);

#endif // FREESPACE2_OBJECT_WAYPOINT_HH
