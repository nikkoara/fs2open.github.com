...
static void update_danger_weapon(object *pship_obj, object *weapon_obj)
{
        ...
        if (aip->danger_weapon_objnum == -1) {
                ...
                aip->danger_weapon_signature == Objects[aip->danger_weapon_objnum].signature) {
        ...
        int *next_hit = NULL)
{
        ...
        if ((shipp->flags[Ship::Ship_Flags::Depart_warp]) && (shipp->warpout_effect) && (valid_hit_occurred)) {
        ...
        if (valid_hit_occurred) {
                ...
                (Missiontime - wp->creation_time > F1_0 / 2) && (wip->is_homing()) && (wp->homing_object == ship_objp)) {
