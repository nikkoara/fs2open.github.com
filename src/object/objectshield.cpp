...
// Private variables
static const float shield_scale_factor = static_cast< float > (
    1.0 / (log (50.0) - log (1.0))); // Factor used in Goober5000's scale_quad

// Private Function Declarations
/**
 * @brief Logarithmically scales a quadrant's strength according to the shield
 * generator's health %
 *
...
float scale_quad (float generator_fraction, float quad_strength) {
    ...
    float effective_strength =
        quad_strength * (static_cast< float > (log (generator_fraction)) *
                         shield_scale_factor);
...
void shield_add_quad (object* objp, int quadrant_num, float delta) {
    ...
    if (quadrant_num < 0 || quadrant_num >= objp->n_quadrants) return;
    ...
    if (strength > max_quad) strength = max_quad;
...
void shield_add_strength (object* objp, float delta) {
    ...
    if ((delta > 0.0f) && (shield_str >= shield_recharge_limit)) return;

    if (!(Ai_info[Ships[objp->instance].ai_index]
              .ai_profile_flags[AI::Profile_Flags::Smart_shield_management]) ||
        delta <= 0.0f) // SUSHI: We don't want smart shield management for
                       // negative delta
    {
        ...
        while (delta > 0.0f) {
            ...
            if (weakest >= section_max) break;

            // set the limit for the shield recharge
            if ((delta > 0.0f) &&
                ((shield_str + delta) > shield_recharge_limit))
                delta = shield_recharge_limit - shield_str;

            // throw all possible shield power at this quadrant
            // if there's any left over then apply it to the next weakest on
            // the next pass
            float xfer_amount;
...
void shield_balance (object* objp, float rate, float penalty) {
    ...
    for (int i = 0; i < objp->n_quadrants; ++i) {
        if (fabsf (objp->shield_quadrant[i] - shield_hp_avg) < 0.01f) {
            ...
            objp->shield_quadrant[i] +=
                rate * (shield_hp_avg - objp->shield_quadrant[i]);
        }
    }
}
...
float shield_get_max_strength (object* objp, bool no_msr) {
    ...
    if (no_msr == true)
        ...
        return Ships[objp->instance].ship_max_shield_strength *
               Ships[objp->instance].max_shield_recharge;
}
...
float shield_get_quad (object* objp, int quadrant_num) {
    ...
    if (objp->flags[Object::Object_Flags::No_shields]) return 0.0f;

    // check array bounds
    ASSERT (quadrant_num >= 0 && quadrant_num < objp->n_quadrants);
    if (quadrant_num < 0 || quadrant_num >= objp->n_quadrants) return 0.0f;
...
int shield_is_up (object* objp, int quadrant_num) {
    ...
    if ((quadrant_num >= 0) && (quadrant_num < objp->n_quadrants)) {
        ...
        if (quad > MAX (
                       2.0f, 0.1f * shield_get_max_quad (
                                        objp))) // [z64555] Where the heck does
                                                // this 2HP come from?
            return 1;
    }
    else {
        ...
        if (strength > MAX (
                           2.0f * objp->n_quadrants,
                           0.1f * shield_get_max_strength (
                                      objp))) // [z64555] Where the heck does
                                              // this 2HP come from?
            return 1;
    }
...
void shield_transfer (object* objp, int quadrant, float rate) {
    ...
    float energy_avail =
        0.0f; // Energy available from the other quadrants that we can transfer
    ...
    if (percent_to_take > 1.0f) { percent_to_take = 1.0f; }
    ...
    for (int i = 0; i < objp->n_quadrants; i++) {
        ...
        if (objp->shield_quadrant[quadrant] > max_quadrant_val) {
            // Already reached the max quadrant value. Clamp and bail before
            // losing more any energy.
