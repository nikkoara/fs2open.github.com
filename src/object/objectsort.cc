...
{
        return obj->type == OBJ_SHIP || (obj->type == OBJ_WEAPON && Weapon_info[Weapons[obj->instance].weapon_info_index].render_type == WRT_POF) || obj->type == OBJ_ASTEROID || obj->type == OBJ_DEBRIS || obj->type == OBJ_JUMP_NODE;
}
        ...
        bool *draw_viewer_last)
{
        ...
        for (os = Sorted_objects.begin(); os != Sorted_objects.end(); ++os) {
                ...
                // you see fit
                if ((obj == Viewer_obj) && (obj->type == OBJ_SHIP) && c_viewer && (Ship_info[Ships[obj->instance].ship_info_index].flags[Ship::Info_Flags::Show_ship_model])) {
