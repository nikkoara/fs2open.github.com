...
void obj_add_pair(object *A, object *B, int check_time, int add_to_end)
{
        ...
        if ((A->flags[Object::Object_Flags::Immobile]) && (B->flags[Object::Object_Flags::Immobile]))
        ...
        */
        ...
        if (check_collision == collide_ship_weapon) {
                // weapon is B
                if ((B->parent >= 0) && (Objects[B->parent].signature == B->parent_sig) && !(Objects[B->parent].flags[Object::Object_Flags::Player_ship]) && (Ships[Objects[B->parent].instance].team == Ships[A->instance].team) && (Ship_info[Ships[A->instance].ship_info_index].is_small_ship()) && (Weapon_info[Weapons[B->instance].weapon_info_index].subtype == WP_LASER)) {
                        pairs_not_created++;
                        return;
                }
        }
        ...
        object *obj_weapon, object *other, obj_pair *current_pair)
{
        ...
        if (current_pair) {
                ...
                if (!(wip->wi_flags[Weapon::Info_Flags::Turns])) {
                        ...
                        // max_vel_weapon );
                        delta_t = (other->radius + 10.0f) / max_vel_other; // time to get from center to radius of
                                                                           // other obj
                        ...
                        if (earliest_time > 100) {
                                current_pair->next_check_time = timestamp(int(earliest_time));
                                return 0;
                        } else {
                                current_pair->next_check_time = timestamp(0); // check next time
                                return 0;
