...
public:
        ...
        // The following were originally unions, but it became necessary to use
        // structs for certain functions that need to maintain two or more values.
        struct {
                double double_value;
                object *objp_value;
                vec3d *vecp_value;
        ...
        // (This used to memset the entire class (which at the time was a struct)
        // to 0, but that hosed the function address table.  Fortunately, by the
        // time the constructor was called, the address table wasn't needed any
        // more!  It's now revised because it's best to keep the code robust in
        // case of future unanticipated changes.)
        dock_function_info() : early_return_condition(false)
        {
...
// currently the game assumes the center of mass is the center of an object;
// this will need to be fixed eventually (though this function does weight the
// object masses properly)
...
// The semilatus rectum is analagous to the radius, but restricted to one
// dimension (whereas the radius covers two dimensions).  It is half of the
// latus rectum.  More information can be found in Wikipedia or in any math
// textbook. :p
...
// Uberfunction for evaluating all objects that could possibly be docked to
// objp.  This will call "function" for each docked object.  The function
// should store its intermediate and return values in the dock_function_info
// class.
...
// remove objp1 and objp2 from each others' dock lists; currently only called
// by ai_do_objects_undocked_stuff
void dock_undock_objects(object *objp1, object *objp2);

/**
 * @brief Undocks everything from the given object
 * @note This is a slow method. use dock_free_dock_list() when doing object
 * cleanup.
 * @note Currently, this function cannot be called from within ship_cleanup()
 * [Github Issue
 * #1177:https://github.com/scp-fs2open/fs2open.github.com/issues/1177]
 */
...
#endif // FREESPACE2_OBJECT_OBJECTDOCK_HH
