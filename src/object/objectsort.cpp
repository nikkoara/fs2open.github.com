...
public:
    ...
    sorted_obj () : obj (NULL), z (0.0f), min_z (0.0f), max_z (1.0f) {}
...
inline bool sorted_obj::operator< (const sorted_obj& other) const {
    ...
    if (obj->type == OBJ_SHIP) {
        ...
        if (wip->render_type == WRT_POF) { model_num_a = wip->model_num; }
    }
    ...
    if (other.obj->type == OBJ_SHIP) {
        ship_info* sip =
            &Ship_info[Ships[other.obj->instance].ship_info_index];
        ...
        if (wip->render_type == WRT_POF) { model_num_b = wip->model_num; }
    }
    ...
    if (model_num_a == model_num_b) { return (max_z > other.max_z); }
...
int obj_in_view_cone (object* objp) {
    ...
    // mprintf(( "All points inside, so render it, but doesn't need
    // clipping.\n" ));
    return 1;
}
...
inline bool obj_render_is_model (object* obj) {
    return obj->type == OBJ_SHIP ||
           (obj->type == OBJ_WEAPON &&
            Weapon_info[Weapons[obj->instance].weapon_info_index]
                    .render_type == WRT_POF) ||
           obj->type == OBJ_ASTEROID || obj->type == OBJ_DEBRIS ||
...
void obj_render_all (
    ...
    for (i = 0; i <= Highest_object_index; i++, objp++) {
        if ((objp->type != OBJ_NONE) &&
            (objp->flags[Object::Object_Flags::Renders])) {
...
#ifdef DYN_CLIP_DIST
                if (objp != Viewer_obj) {
                    if (osp->min_z < closest_obj) closest_obj = osp->min_z;
                    if (osp->max_z > farthest_obj) farthest_obj = osp->max_z;
                }
#endif
            }
        }
    ...
    if (Sorted_objects.empty ()) return;

    std::sort (Sorted_objects.begin (), Sorted_objects.end ());

#ifdef DYN_CLIP_DIST
    if (closest_obj < Min_draw_distance) closest_obj = Min_draw_distance;
    if (farthest_obj > Max_draw_distance) farthest_obj = Max_draw_distance;

    gr_set_proj_matrix (
        Proj_fov, gr_screen.clip_aspect, closest_obj, farthest_obj);
    ...
    for (os = Sorted_objects.begin (); os != Sorted_objects.end (); ++os) {
        ...
        // This is for ship cockpits. Bobb, feel free to optimize this any way
        // you see fit
        if ((obj == Viewer_obj) && (obj->type == OBJ_SHIP) && c_viewer &&
            (Ship_info[Ships[obj->instance].ship_info_index]
        ...
        if (full_neb) {
            ...
            // maybe skip rendering an object because its obscured by the
            // nebula
            if (neb2_skip_render (obj, os->z)) { continue; }
        }

        if (obj_render_is_model (obj)) {
            if (((obj->type == OBJ_SHIP) &&
                 Ships[obj->instance].shader_effect_active) ||
    ...
    for (os = Sorted_objects.begin (); os != Sorted_objects.end (); ++os) {
        ...
        if (obj_render_is_model (obj)) continue;
    ...
    for (i = 0; i <= Highest_object_index; i++, objp++) {
        if ((objp->type != OBJ_NONE) &&
            ...
            if ((The_mission.flags[Mission::Mission_Flags::Fullneb]) &&
                ...
                if (neb2_skip_render (objp, z)) { continue; }
            }

            if (obj_render_is_model (objp)) {
                if ((objp->type == OBJ_SHIP) &&
                    Ships[objp->instance].shader_effect_active) {
