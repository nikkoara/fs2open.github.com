...
void collect_ship_ship_physics_info (
    object* heavier_obj, object* lighter_obj, mc_info* mc_info_obj,
    collision_info_struct* ship_ship_hit_info);
...
static int ships_are_docking (object* objp1, object* objp2) {
    ...
    if (dock_check_find_direct_docked_object (objp1, objp2)) { return 1; }

    if (aip1->mode == AIM_DOCK) {
        if (aip1->goal_objnum == OBJ_INDEX (objp2)) { return 1; }
    }

    if (aip2->mode == AIM_DOCK) {
        if (aip2->goal_objnum == OBJ_INDEX (objp1)) { return 1; }
    }
...
static int bay_emerge_or_depart (object* heavy_objp, object* light_objp) {
    if (light_objp->type != OBJ_SHIP) return 0;

    ai_info* aip = &Ai_info[Ships[light_objp->instance].ai_index];

    // the player shouldn't be allowed to fly through the ship just cause the
    // rest of their wing can
    if ((Player_obj == light_objp) && !Player_use_ai) { return 0; }

    if ((aip->mode == AIM_BAY_EMERGE) || (aip->mode == AIM_BAY_DEPART)) {
        if (aip->goal_objnum == OBJ_INDEX (heavy_objp)) return 1;
    }

    return 0;
}

int ship_ship_check_collision (
    collision_info_struct* ship_ship_hit_info, vec3d* hitpos) {
    ...
    if (heavy_shipp->flags[Ship::Ship_Flags::Arriving_stage_1]) { return 0; }

    // Don't do collision detection for docking ships, since they will always
    ...
    vec3d zero, p0, p1;
    vm_vec_zero (&zero); // we need the physical vector and can not set its
                         // value to zero
    vm_vec_sub (&p0, &light_obj->last_pos, &heavy_obj->last_pos);
    vm_vec_sub (&p1, &light_obj->pos, &heavy_obj->pos);

    // find the light object's position in the heavy object's reference frame
    // at last frame and also in this frame.
    vec3d p0_temp, p0_rotated;

    // Collision detection from rotation enabled if at max rotaional velocity
    // and 5fps, rotation is less than PI/2 This should account for all ships
    if ((vm_vec_mag_squared (&heavy_obj->phys_info.max_rotvel) * .04) <
        (PI * PI / 4)) {
        // collide_rotate calculate (1) start position and (2) relative
        // velocity
    ...
    // should be no ships that can rotate this fast
        ...
        vm_vec_sub (
            &ship_ship_hit_info->light_rel_vel, &light_obj->phys_info.vel,
    ...
    if (!(heavy_shipp->flags[Ship::Ship_Flags::Dont_collide_invis])) {
        if ((heavy_obj->flags[Object::Object_Flags::Player_ship]) ||
            ...
            (heavy_shipp->team != light_shipp->team)) {
            mc.flags |= MC_CHECK_INVISIBLE_FACES;
        }
    }

    // copy important data
    int copy_flags = mc.flags; // make a copy of start end positions of sphere
                               // in  big ship RF
    vec3d copy_p0, copy_p1;
    copy_p0 = *mc.p0;
    copy_p1 = *mc.p1;

    // first test against the sphere - if this fails then don't do any submodel
    // tests
    ...
    if (pm_light->submodel[pm_light->detail[0]].no_collisions) { return 0; }
    ...
    if (model_collide (&mc)) {
        ...
        if (ship_ship_hit_info->collide_rotate) {
            ...
            for (smv = submodel_vector.begin (); smv != submodel_vector.end ();
                 ++smv) {
                pmi->submodel[*smv].collision_checked = true;
            }

            // reset flags to check MC_CHECK_MODEL | MC_CHECK_SPHERELINE and
            // maybe MC_CHECK_INVISIBLE_FACES and MC_SUBMODEL_INSTANCE
            ...
            for (smv = submodel_vector.begin (); smv != submodel_vector.end ();
                ...
                world_find_model_instance_point (
                    &p0, &light_obj->last_pos, pmi, *smv,
                    &heavy_obj->last_orient, &heavy_obj->last_pos);

                pmi->submodel[*smv].angs = copy_angles;
                world_find_model_instance_point (
                    &p1, &light_obj->pos, pmi, *smv, &heavy_obj->orient,
                    &heavy_obj->pos);
                ...
                if (model_collide (&mc)) {
                    if (mc.hit_dist < ship_ship_hit_info->hit_time) {
                        ...
                        set_hit_struct_info (
                            ship_ship_hit_info, &mc, SUBMODEL_ROT_HIT);
                        model_instance_find_world_point (
                            &ship_ship_hit_info->hit_pos, &mc.hit_point,
                            mc.model_instance_num, mc.hit_submodel,
                            &heavy_obj->orient, &zero);
                        ...
                        if (ship_ship_hit_info->edge_hit == 0) {
                            model_instance_find_obj_dir (
                                ...
                                &mc.hit_normal, mc.model_instance_num,
                                mc.hit_submodel, &heavy_obj->orient);
                        }

                        // find position in submodel RF of light object at
                        // collison
                        vec3d int_light_pos, diff;
                        vm_vec_sub (&diff, mc.p1, mc.p0);
                        vm_vec_scale_add (
                            &int_light_pos, mc.p0, &diff, mc.hit_dist);
                        model_instance_find_world_point (
                            &ship_ship_hit_info->light_collision_cm_pos,
                            &int_light_pos, mc.model_instance_num,
        ...
        // Recover and do usual ship_ship collision, but without rotating
        // submodels
        mc.flags = copy_flags;
        *mc.p0 = copy_p0;
        *mc.p1 = copy_p1;
        ...
        if (model_collide (&mc)) {
            ...
            if (mc.hit_dist < ship_ship_hit_info->hit_time) {
                ...
                set_hit_struct_info (
                    ship_ship_hit_info, &mc, SUBMODEL_NO_ROT_HIT);

                // get collision normal if not edge hit
                if (ship_ship_hit_info->edge_hit == 0) {
                    model_instance_find_obj_dir (
                        &ship_ship_hit_info->collision_normal, &mc.hit_normal,
                        mc.model_instance_num, mc.hit_submodel,
                        &heavy_obj->orient);
                }
                ...
                vm_vec_sub (&diff, mc.p1, mc.p0);
                vm_vec_scale_add (
                    &ship_ship_hit_info->light_collision_cm_pos, mc.p0, &diff,
                    mc.hit_dist);
            }
        }
    }
    ...
    if (valid_hit_occured) {
        ...
        if (heavy_obj == Player_obj) { collide_obj = light_obj; }
        else if (light_obj == Player_obj) {
            collide_obj = heavy_obj;
        }
        if ((collide_obj != NULL) &&
            (Ship_info[Ships[collide_obj->instance].ship_info_index]
                 .is_fighter_bomber ())) {
        ...
        if (heavy_obj - Objects ==
            Ai_info[light_shipp->ai_index].target_objnum) {
            Ai_info[light_shipp->ai_index].ai_flags.set (
                AI::AI_Flags::Target_collision);
        }
        if (light_obj - Objects ==
            Ai_info[heavy_shipp->ai_index].target_objnum) {
            Ai_info[heavy_shipp->ai_index].ai_flags.set (
                AI::AI_Flags::Target_collision);
        }
        ...
        vm_vec_sub (
            &ship_ship_hit_info->r_light, &ship_ship_hit_info->hit_pos,
            &ship_ship_hit_info->light_collision_cm_pos);
        ...
        if (ship_ship_hit_info->edge_hit) {
            vm_vec_copy_normalize (
                &ship_ship_hit_info->collision_normal,
                &ship_ship_hit_info->r_light);
            vm_vec_negate (&ship_ship_hit_info->collision_normal);
        }

        // get world hitpos
        vm_vec_add (
            hitpos, &ship_ship_hit_info->heavy->pos,
            &ship_ship_hit_info->r_heavy);
        ...
        if (heavy_shipp->team == light_shipp->team) {
            ...
            if ((heavy_sip->is_small_ship ()) &&
                ...
                if ((heavy_obj->flags[Object::Object_Flags::Player_ship]) ||
                    ...
                    vm_vec_sub (
                        &rel_vel_h, &heavy_obj->phys_info.vel,
                        &light_obj->phys_info.vel);
                    float mass_sum =
                        light_obj->phys_info.mass + heavy_obj->phys_info.mass;

                    // get comp of rel_vel perp to h_to_l_vec;
                    float mag = vm_vec_dot (&h_to_l_vec, &rel_vel_h) /
                                vm_vec_mag_squared (&h_to_l_vec);
                    vm_vec_scale_add (
                        &perp_rel_vel, &rel_vel_h, &h_to_l_vec, -mag);
                    vm_vec_normalize (&perp_rel_vel);

                    vm_vec_scale_add2 (
                        &heavy_obj->phys_info.vel, &perp_rel_vel,
                        heavy_sip->collision_physics.both_small_bounce *
                            light_obj->phys_info.mass / mass_sum);
                    vm_vec_scale_add2 (
                        &light_obj->phys_info.vel, &perp_rel_vel,
                        -(light_sip->collision_physics.both_small_bounce) *
                            heavy_obj->phys_info.mass / mass_sum);

                    vm_vec_rotate (
                        &heavy_obj->phys_info.prev_ramp_vel,
                        &heavy_obj->phys_info.vel, &heavy_obj->orient);
                    vm_vec_rotate (
                        &light_obj->phys_info.prev_ramp_vel,
                        &light_obj->phys_info.vel, &light_obj->orient);
                }
            }
            else {
                // add extra velocity to separate the two objects, backing up
                // the direction we came in.
                // TODO: add effect of velocity from rotating submodel
                float rel_vel =
                    vm_vec_mag_quick (&ship_ship_hit_info->light_rel_vel);
                if (rel_vel < 1) { rel_vel = 1.0f; }
                float mass_sum =
                    heavy_obj->phys_info.mass + light_obj->phys_info.mass;
                vm_vec_scale_add2 (
                    &heavy_obj->phys_info.vel,
                    &ship_ship_hit_info->light_rel_vel,
                    heavy_sip->collision_physics.bounce *
                        light_obj->phys_info.mass / (mass_sum * rel_vel));
                vm_vec_rotate (
                    &heavy_obj->phys_info.prev_ramp_vel,
                    &heavy_obj->phys_info.vel, &heavy_obj->orient);
                vm_vec_scale_add2 (
...
/**
 * Gets modified mass of cruiser in cruiser/asteroid collision so cruisers
 * don't get bumped so hard. modified mass is 10x, 4x, or 2x larger than
 * asteroid mass
 * @return 1 if modified mass is larger than given mass, 0 otherwise
 */
static int check_special_cruiser_asteroid_collision (
    object* heavy, object* lighter, float* cruiser_mass, int* cruiser_light) {
    int asteroid_type;
 ...
 */
static bool check_subsystem_landing_allowed (
    ship_info* heavy_sip, collision_info_struct* ship_ship_hit_info) {
    if (!(heavy_sip->flags[Ship::Info_Flags::Allow_landings])) return false;

    for (int i = 0; i < heavy_sip->n_subsystems; i++) {
        if (heavy_sip->subsystems[i]
                .flags[Model::Subsystem_Flags::Allow_landing] &&
            heavy_sip->subsystems[i].subobj_num ==
...
void calculate_ship_ship_collision_physics (
    ...
    ship_info* light_sip =
        (lighter->type == OBJ_SHIP)
            ? &Ship_info[Ships[lighter->instance].ship_info_index]
            : NULL;
    ship_info* heavy_sip =
        (heavy->type == OBJ_SHIP)
            ? &Ship_info[Ships[heavy->instance].ship_info_index]
            : NULL;
    ...
    special_cruiser_asteroid_collision =
        check_special_cruiser_asteroid_collision (
            heavy, lighter, &cruiser_mass, &cruiser_light);
    ...
    float v_rel_normal_m;    // relative collision velocity in the direction of
                             // the collision normal
    vec3d v_rel_parallel_m;  // normalized v_rel (Va-Vb) projected onto
                             // collision surface
    vec3d world_rotvel_heavy_m, world_rotvel_light_m, vel_from_rotvel_heavy_m,
        vel_from_rotvel_light_m, v_rel_m, vel_heavy_m, vel_light_m;

    coeff_restitution = 0.1f; // relative velocity wrt normal is zero after the
                              // collision ( range 0-1 )
    ...
    if (ship_ship_hit_info->collide_rotate) {
        // if we have collisions from rotation, the effective velocity from
        // rotation of the large body is alreay taken account
        vm_vec_zero (&vel_heavy_m);
    }
    else {
        // take account the effective velocity from rotation
        vm_vec_unrotate (
            &world_rotvel_heavy_m, &heavy->phys_info.rotvel,
            &heavy->orient); // heavy's world rotvel before collision
        vm_vec_cross (
            &vel_from_rotvel_heavy_m, &world_rotvel_heavy_m,
            &ship_ship_hit_info
                 ->r_heavy); // heavy's velocity from rotvel before collision
        vel_heavy_m = vel_from_rotvel_heavy_m;
    }

    // if collision from rotating submodel of heavy obj, add in vel from rotvel
    // of submodel
    ...
    if (ship_ship_hit_info->submodel_rot_hit == 1) {
        ...
        if (heavy->type == OBJ_SHIP) {
            ...
            pm = Asteroid_info[Asteroids[heavy->instance].asteroid_type]
                     .modelp[Asteroids[heavy->instance].asteroid_subtype];
        ...
        if (pmi != NULL &&
            pmi->submodel[ship_ship_hit_info->submodel_num].sii != NULL) {
            // set point on axis of rotating submodel if not already set.
            if (!pmi->submodel[ship_ship_hit_info->submodel_num]
                     .sii->axis_set) {
                model_init_submodel_axis_pt (
                    pmi->submodel[ship_ship_hit_info->submodel_num].sii,
                    pm->id, ship_ship_hit_info->submodel_num);
            }

            vec3d omega, axis, r_rot;
            if (pm->submodel[ship_ship_hit_info->submodel_num].movement_axis ==
                MOVEMENT_AXIS_X) {
                axis = vmd_x_vector;
            }
            else if (
                pm->submodel[ship_ship_hit_info->submodel_num].movement_axis ==
                MOVEMENT_AXIS_Y) {
                axis = vmd_y_vector;
            }
            else if (
            ...
            model_instance_find_obj_dir (
                &omega, &axis, model_instance_num,
                ship_ship_hit_info->submodel_num, &heavy->orient);

            vm_vec_scale (
                &omega, pmi->submodel[ship_ship_hit_info->submodel_num]
                            .sii->cur_turn_rate);
            ...
            vm_vec_unrotate (
                &temp,
                &pmi->submodel[ship_ship_hit_info->submodel_num]
                     .sii->pt_on_axis,
                &heavy->orient);
    ...
    vm_vec_unrotate (
        &world_rotvel_light_m, &lighter->phys_info.rotvel,
        &lighter->orient); // light's world rotvel before collision
    vm_vec_cross (
        &vel_from_rotvel_light_m, &world_rotvel_light_m,
        &ship_ship_hit_info
             ->r_light); // light's velocity from rotvel before collision
    vm_vec_add (
        &vel_light_m, &vel_from_rotvel_light_m,
        &ship_ship_hit_info->light_rel_vel);
    ...
    v_rel_normal_m = vm_vec_dot (
        &v_rel_m,
        &ship_ship_hit_info->collision_normal); // if less than zero, colliding
                                                // contact taking place (v_slow
                                                // - v_fast) dot (n_fast)
    ...
    bool subsys_landing_allowed =
        lighter->type == OBJ_SHIP && heavy->type == OBJ_SHIP &&
        check_subsystem_landing_allowed (heavy_sip, ship_ship_hit_info);
    if (subsys_landing_allowed) {
        vm_vec_rotate (
            &light_local_vel, &ship_ship_hit_info->light_rel_vel,
            &lighter->orient);
        light_uvec_dot_norm = vm_vec_dot (
            &ship_ship_hit_info->collision_normal, &lighter->orient.vec.uvec);
        light_fvec_dot_norm = vm_vec_dot (
            &ship_ship_hit_info->collision_normal, &lighter->orient.vec.fvec);
        light_rvec_dot_norm = vm_vec_dot (
            &ship_ship_hit_info->collision_normal, &lighter->orient.vec.rvec);
    }
    if (subsys_landing_allowed &&
    ...
    vec3d rotational_impulse_heavy, rotational_impulse_light,
        delta_rotvel_heavy, delta_rotvel_light;
    vec3d delta_vel_from_delta_rotvel_heavy, delta_vel_from_delta_rotvel_light,
        impulse;
    ...
    // F = I * sin (collision_normal, normalized v_rel_m)  [sin is ratio of
    // v_rel_parallel_m to v_rel_m] note:  (-) sign is needed to account for
    // the direction of the v_rel_parallel_m
    if (IS_VEC_NULL (&v_rel_m)) {
        // If the relative velocity is zero then the compuatation below would
        // cause NaN errors
        vm_vec_zero (&impulse);
    }
    else {
        ...
        vm_vec_projection_onto_plane (
            &v_rel_parallel_m, &v_rel_m,
            &ship_ship_hit_info->collision_normal);
        collision_speed_parallel = vm_vec_normalize_safe (&v_rel_parallel_m);
        float friction = (lighter->type == OBJ_SHIP)
                             ? light_sip->collision_physics.friction
                             : COLLISION_FRICTION_FACTOR;
        parallel_mag = float(-friction) * collision_speed_parallel /
                       vm_vec_mag (&v_rel_m);
        vm_vec_scale_add2 (&impulse, &v_rel_parallel_m, parallel_mag);
    }

    // calculate the effect on the velocity of the collison point per unit
    // impulse first find the effect thru change in rotvel then find the change
    // in the cm vel
    if (heavy == Player_obj) {
        ...
        vm_vec_cross (
            &rotational_impulse_heavy, &ship_ship_hit_info->r_heavy, &impulse);
        get_I_inv (&heavy_I_inv, &heavy->phys_info.I_body_inv, &heavy->orient);
        vm_vec_rotate (
            &delta_rotvel_heavy, &rotational_impulse_heavy, &heavy_I_inv);
        float rotation_factor =
            (heavy->type == OBJ_SHIP)
                ? heavy_sip->collision_physics.rotation_factor
                : COLLISION_ROTATION_FACTOR;
        vm_vec_scale (
            &delta_rotvel_heavy,
            rotation_factor); // hack decrease rotation (delta_rotvel)
        vm_vec_cross (
            &delta_vel_from_delta_rotvel_heavy, &delta_rotvel_heavy,
            &ship_ship_hit_info->r_heavy);
        heavy_denom = vm_vec_dot (
            &delta_vel_from_delta_rotvel_heavy,
            &ship_ship_hit_info->collision_normal);
        if (heavy_denom < 0) {
            // sanity check
            heavy_denom = 0.0f;
    ...
    // calculate the effect on the velocity of the collison point per unit
    // impulse first find the effect thru change in rotvel then find the change
    // in the cm vel SUSHI: If on a landing surface, use the same shortcut the
    // player gets This is a bit of a hack, but gets around some nasty
    // unpredictable collision behavior when trying to do AI landings for
    // certain ships
    if (lighter == Player_obj || subsys_landing_allowed) {
        ...
        vm_vec_cross (
            &rotational_impulse_light, &ship_ship_hit_info->r_light, &impulse);
        get_I_inv (
            &light_I_inv, &lighter->phys_info.I_body_inv, &lighter->orient);
        vm_vec_rotate (
            &delta_rotvel_light, &rotational_impulse_light, &light_I_inv);
        float rotation_factor =
            (lighter->type == OBJ_SHIP)
                ? light_sip->collision_physics.rotation_factor
                : COLLISION_ROTATION_FACTOR;
        vm_vec_scale (
            &delta_rotvel_light,
            rotation_factor); // hack decrease rotation (delta_rotvel)
        vm_vec_cross (
            &delta_vel_from_delta_rotvel_light, &delta_rotvel_light,
            &ship_ship_hit_info->r_light);
        light_denom = vm_vec_dot (
            &delta_vel_from_delta_rotvel_light,
            &ship_ship_hit_info->collision_normal);
        if (light_denom < 0) {
            // sanity check
            light_denom = 0.0f;
    ...
    // calculate the necessary impulse to achieved the desired relative
    // velocity after the collision update damage info in mc
    impulse_mag = -(1.0f + coeff_restitution) * v_rel_normal_m /
    ...
    // since we have already calculated delta rotvel for heavy and light in
    // world coords physics should not have to recalculate this, just change
    // into body coords (done in collide_whack)
    vm_vec_scale (&impulse, impulse_mag);
    vm_vec_scale (&delta_rotvel_light, impulse_mag);
    physics_collide_whack (
        &impulse, &delta_rotvel_light, &lighter->phys_info, &lighter->orient,
        ship_ship_hit_info->is_landing);
    vm_vec_negate (&impulse);
    vm_vec_scale (&delta_rotvel_heavy, -impulse_mag);
    physics_collide_whack (
        &impulse, &delta_rotvel_heavy, &heavy->phys_info, &heavy->orient,
        true);

    // If within certain bounds, we want to add some more rotation towards the
    // "resting orientation" of the ship These bounds are defined separately
    // from normal "landing" bounds so that they can be more generous: we can
    // have crash landings that still re-orient the ship.
    if (subsys_landing_allowed &&
        ...
        landing_delta_rotvel.xyz.x =
            (light_fvec_dot_norm *
             light_sip->collision_physics.reorient_mult) -
            light_sip->collision_physics.landing_rest_angle;
        // For yaw, use the dot product between vel vector (normalized) and
        // orientation on the xz plane This reduces to the following math We
        // also clamp to reduce huge nose swings at low speeds
        float xzVelMag = sqrt (
            light_local_vel.xyz.x * light_local_vel.xyz.x +
            light_local_vel.xyz.z * light_local_vel.xyz.z);
        float xzVelDotOrient = MIN (
            MAX (
                (xzVelMag > 0 ? (light_local_vel.xyz.x / xzVelMag) : 0),
                -0.5f),
            0.5f);
        landing_delta_rotvel.xyz.y =
            (xzVelMag > 2)
                ? (xzVelDotOrient * light_sip->collision_physics.reorient_mult)
                : 0;
        landing_delta_rotvel.xyz.z =
            light_rvec_dot_norm * light_sip->collision_physics.reorient_mult *
            -1;
    ...
    vm_vec_add (
        &lighter->pos, &heavy->pos,
        &ship_ship_hit_info->light_collision_cm_pos);

    // Try to move each body back to its position just before collision occured
    // to prevent interpenetration Move away in direction of light and away in
    // direction of normal
    ...
    vm_vec_scale_add2 (
        ...
        0.2f * lighter->phys_info.mass /
            (heavy->phys_info.mass + lighter->phys_info.mass));
    vm_vec_scale_add2 (
        &heavy->pos, &ship_ship_hit_info->collision_normal,
        -0.1f * lighter->phys_info.mass /
            (heavy->phys_info.mass + lighter->phys_info.mass));
    // For landings, we want minimal movement on the light ship (just enough to
    // keep the collision detection honest)
    if (ship_ship_hit_info->is_landing) {
        vm_vec_scale_add2 (
            &lighter->pos, &ship_ship_hit_info->collision_normal,
            LANDING_POS_OFFSET);
    }
    else {
        vm_vec_scale_add2 (
            &lighter->pos, &direction_light,
            -0.2f * heavy->phys_info.mass /
                (heavy->phys_info.mass + lighter->phys_info.mass));
        vm_vec_scale_add2 (
            &lighter->pos, &ship_ship_hit_info->collision_normal,
            0.1f * heavy->phys_info.mass /
                (heavy->phys_info.mass + lighter->phys_info.mass));
    }

    // restore mass in case of special cruiser / asteroid collision
    if (special_cruiser_asteroid_collision) {
        if (cruiser_light) { lighter->phys_info.mass = copy_mass; }
        else {
...
static void get_I_inv (matrix* I_inv, matrix* I_inv_body, matrix* orient) {
    ...
    // This is opposite to what is commonly seen in books since we are rotating
    // coordianates axes which is equivalent to rotating in the opposite
    // direction (or transpose)
...
static void mcp_1 (object* player_objp, object* planet_objp) {
    ...
    if (dist > planet_radius * PLANET_DAMAGE_RANGE) return;

    ship_apply_global_damage (
        player_objp, planet_objp, NULL,
        PLANET_DAMAGE_SCALE * flFrametime *
            (float)pow ((planet_radius * PLANET_DAMAGE_RANGE) / dist, 3.0f));

    if ((Missiontime - Last_planet_damage_time > F1_0) ||
        (Missiontime < Last_planet_damage_time)) {
        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN, "%s",
            XSTR ("Too close to planet.  Taking damage!", 465));
        Last_planet_damage_time = Missiontime;
        snd_play_3d (
            gamesnd_get_game_sound (
                ship_get_sound (player_objp, GameSounds::ABURN_ENGAGE)),
            &player_objp->pos, &View_position);
    }
}
...
/**
 * If exactly one of these is a planet and the other is a player ship, do
 * something special.
 * @return true if this was a ship:planet (or planet_ship) collision and we
 * processed it. Else return false.
 */
...
/**
 * Given a global point and an object, get the quadrant number the point
 * belongs to.
 */
...
#define MIN_REL_SPEED_FOR_LOUD_COLLISION                                      \
    50 // relative speed of two colliding objects at which we play the "loud"
        // collide sound
 ...
 */
void collide_ship_ship_do_sound (
    vec3d* world_hit_pos, object* A, object* B, int player_involved) {
    vec3d rel_vel;
    float rel_speed;

    vm_vec_sub (
        &rel_vel, &A->phys_info.desired_vel, &B->phys_info.desired_vel);
    rel_speed = vm_vec_mag_quick (&rel_vel);

    if (rel_speed > MIN_REL_SPEED_FOR_LOUD_COLLISION) {
        snd_play_3d (
            gamesnd_get_game_sound (GameSounds::SHIP_SHIP_HEAVY),
        ...
        if (player_involved) {
            if (!snd_is_playing (Player_collide_sound)) {
                Player_collide_sound = snd_play_3d (
                    gamesnd_get_game_sound (GameSounds::SHIP_SHIP_LIGHT),
                    world_hit_pos, &View_position);
            }
        }
        else {
    ...
    if ((shield_get_strength (A) > 5) || (shield_get_strength (B) > 5)) {
        if (player_involved) {
            if (!snd_is_playing (Player_collide_sound)) {
                Player_collide_shield_sound = snd_play_3d (
                    gamesnd_get_game_sound (GameSounds::SHIP_SHIP_SHIELD),
                    world_hit_pos, &View_position);
            }
        }
        else {
            if (!snd_is_playing (Player_collide_sound)) {
                AI_collide_shield_sound = snd_play_3d (
                    gamesnd_get_game_sound (GameSounds::SHIP_SHIP_SHIELD),
                    world_hit_pos, &View_position);
            }
        }
 ...
 */
static void maybe_push_little_ship_from_fast_big_ship (
    object* big_obj, object* small_obj, float impulse, vec3d* normal) {
    // Move player out of the way of a BIG|HUGE ship warping in or out
    int big_class =
        Ship_info[Ships[big_obj->instance].ship_info_index].class_type;
    int small_class =
        Ship_info[Ships[small_obj->instance].ship_info_index].class_type;
    if (big_class > -1 &&
        Ship_types[big_class].flags[Ship::Type_Info_Flags::Warp_pushes]) {
        if (small_class > -1 &&
            Ship_types[small_class]
                .flags[Ship::Type_Info_Flags::Warp_pushable]) {
            ...
            if (big_speed > 3 * big_obj->phys_info.max_vel.xyz.z) {
                ...
                vm_vec_scale_add (
                    &perp, &temp, &big_obj->orient.vec.fvec,
                    -vm_vec_dot (&temp, &big_obj->orient.vec.fvec));
                vm_vec_normalize_quick (&perp);

                // don't drive into sfc we just collided with
                if (vm_vec_dot (&perp, normal) < 0) { vm_vec_negate (&perp); }
                ...
                vm_vec_scale_add2 (
                    &small_obj->phys_info.vel, &perp, added_perp_vel_mag);
                vm_vec_rotate (
                    &small_obj->phys_info.prev_ramp_vel,
                    &small_obj->phys_info.vel, &small_obj->orient);
            }
        }
    }
}
 ...
 * @return 1 if all future collisions between these can be ignored because
 * pair->a or pair->b aren't ships
 * @return Otherwise always returns 0, since two ships can always collide
 * unless one (1) dies or (2) warps out.
 */
int collide_ship_ship (obj_pair* pair) {
    ...
    // If the player is one of the two colliding ships, flag this... it is used
    // in several places this function.
    if (A == Player_obj || B == Player_obj) { player_involved = 1; }
    else {
        player_involved = 0;
    }
    ...
    if (dist < A->radius + B->radius) {
        ...
        ship_info* light_sip =
            &Ship_info[Ships[LightOne->instance].ship_info_index];

        collision_info_struct ship_ship_hit_info;
        init_collision_info_struct (&ship_ship_hit_info);

        ship_ship_hit_info.heavy =
            HeavyOne; // heavy object, generally slower moving
        ship_ship_hit_info.light =
            LightOne; // light object, generally faster moving
        ...
        if (hit) {
            ...
            if (!Collide_friendly) {
                if (Ships[A->instance].team == Ships[B->instance].team) {
                    ...
                    vm_vec_normalized_dir (
                        &collision_vec, &ship_ship_hit_info.hit_pos, &A->pos);
            ...
            if (!ship_ship_hit_info.is_landing) {
                ...
                ship_apply_local_damage (
                    ...
                    &world_hit_pos, 100.0f * damage / HeavyOne->phys_info.mass,
        ...
        // if ship is warping in, in stage 1, its velocity is 0, so make ship
        // try to collide next frame

        // if ship is huge and warping in or out
        if (((Ships[A->instance].flags[Ship::Ship_Flags::Arriving_stage_1]) &&
             (Ship_info[Ships[A->instance].ship_info_index]
                  .is_huge_ship ())) ||
            ((Ships[B->instance].flags[Ship::Ship_Flags::Arriving_stage_1]) &&
             (Ship_info[Ships[B->instance].ship_info_index]
                  .is_huge_ship ()))) {
            pair->next_check_time = timestamp (0); // check next time
            return 0;
        }

        // get max of (1) max_vel.z, (2) 10, (3) afterburner_max_vel.z, (4)
        // vel.z (for warping in ships exceeding expected max vel)
        float shipA_max_speed, shipB_max_speed, time;

        // get shipA max speed
        if (ship_is_beginning_warpout_speedup (A)) {
            shipA_max_speed = MAX (
                ship_get_max_speed (&Ships[A->instance]),
                ship_get_warpout_speed (A));
        }
        else {
        ...
        shipA_max_speed =
            MAX (shipA_max_speed, vm_vec_mag (&A->phys_info.vel));
        shipA_max_speed = MAX (shipA_max_speed, 10.0f);

        // get shipB max speed
        if (ship_is_beginning_warpout_speedup (B)) {
            shipB_max_speed = MAX (
                ship_get_max_speed (&Ships[B->instance]),
                ship_get_warpout_speed (B));
        }
        else {
        ...
        // Maybe warping in or finished warping in with excessive speed
        shipB_max_speed =
            MAX (shipB_max_speed, vm_vec_mag (&B->phys_info.vel));
        shipB_max_speed = MAX (shipB_max_speed, 10.0f);

        time = 1000.0f * (dist - A->radius - B->radius) /
...
void collect_ship_ship_physics_info (
    object* heavier_obj, object* lighter_obj, mc_info* mc_info_obj,
    collision_info_struct* ship_ship_hit_info) {
    // slower moving object [A] is checked at its final position (polygon and
    // position is found on obj) faster moving object [B] is reduced to a point
    // and a ray is drawn from its last_pos to pos collision code returns hit
    // position and normal on [A]

    // estimate location on B that contacts A
    // first find orientation of B relative to the normal it collides against.
    // then find an approx hit location using the position hit on the bounding
    // box
    ...
    float core_rad = model_get_core_radius (
    ...
    if (!ship_ship_hit_info->submodel_rot_hit) {
        ...
        *light_collide_cm_pos = *mc_info_obj->p0;
        vm_vec_scale_add2 (
            light_collide_cm_pos, &displacement, ship_ship_hit_info->hit_time);
    }
    ...
    if (ship_ship_hit_info->edge_hit) {
        // For an edge hit, just take the closest valid plane normal as the
        // collision normal
    ...
    float q = vm_vec_dist (heavy_collide_cm_pos, light_collide_cm_pos) /
              (heavier_obj->radius + core_rad);
    if (q > 1.0f) {
        WARNINGF (LOCATION, "Warning: q = %f.  Supposed to be <= 1.0.", q);
    }
