...
void dock_count_total_docked_objects_helper (
    object* objp, dock_function_info* infop);
void dock_check_find_docked_object_helper (
    object* objp, dock_function_info* infop);
void dock_calc_docked_center_helper (object* objp, dock_function_info* infop);
void dock_calc_docked_center_of_mass_helper (
    object* objp, dock_function_info* infop);
void dock_calc_total_docked_mass_helper (
    object* objp, dock_function_info* infop);
void dock_calc_max_cross_sectional_radius_squared_perpendicular_to_line_helper (
    object* objp, dock_function_info* infop);
void dock_calc_max_semilatus_rectum_squared_parallel_to_directrix_helper (
    object* objp, dock_function_info* infop);
...
int dock_count_total_docked_objects (object* objp) {
    ...
    dock_evaluate_all_docked_objects (
        objp, &dfi, dock_count_total_docked_objects_helper);
...
bool dock_check_find_docked_object (object* objp, object* other_objp) {
    ...
    dock_evaluate_all_docked_objects (
        objp, &dfi, dock_check_find_docked_object_helper);
...
void dock_calc_docked_center (vec3d* dest, object* objp) {
    ...
    dock_evaluate_all_docked_objects (
        objp, &dfi, dock_calc_docked_center_helper);
...
void dock_calc_docked_center_of_mass (vec3d* dest, object* objp) {
    ...
    dock_evaluate_all_docked_objects (
        objp, &dfi, dock_calc_docked_center_of_mass_helper);

    // overall center of mass = weighted sum of centers of mass divided by
    // total mass
...
float dock_calc_total_docked_mass (object* objp) {
    ...
    dock_evaluate_all_docked_objects (
        objp, &dfi, dock_calc_total_docked_mass_helper);

    return dfi.maintained_variables.float_value;
}

float dock_calc_max_cross_sectional_radius_perpendicular_to_axis (
    object* objp, axis_type axis) {
    ASSERT (objp != NULL);
    ...
    // to calculate the cross-sectional radius, we need a line that will be
    ...
    switch (axis) {
    case X_AXIS: local_line_end.xyz.x = 1.0f; break;

    case Y_AXIS: local_line_end.xyz.y = 1.0f; break;
    ...
    // now we have a unit vector starting at the object's position and pointing
    // along the chosen axis (although the length doesn't matter, as it's
    // calculated as an endless line)
...
float dock_calc_max_semilatus_rectum_parallel_to_axis (
    ...
    // to calculate the semilatus rectum, we need a directrix that will be
...
float dock_calc_docked_fspeed (object* objp) {
    ...
    // *sigh*... the docked fspeed is simply the max fspeed of all docked
    // objects
    dock_function_info dfi;
...
void dock_evaluate_all_docked_objects (
    ...
    // we have multiple objects docked and we're treating them as a hub
        ...
        for (dock_instance* ptr = hub_objp->dock_list; ptr != NULL;
             ptr = ptr->next) {
...
void dock_move_docked_objects (object* objp) {
    ...
    if ((objp->type != OBJ_SHIP) && (objp->type != OBJ_START)) return;

    if (!object_is_docked (objp)) return;

    // has this object (by extension, this group of docked objects) been
    // handled already?
    if (objp->flags[Object::Object_Flags::Docked_already_handled]) return;
    ...
    if (fastest_objp) fastest_objp = objp;
    ...
    // we don't need a bit array because OF_DOCKED_ALREADY_HANDLED takes care
    // of it and must persist for the entire game frame

    // start evaluating the tree, starting with the fastest object having no
    // parent
...
void dock_calc_docked_center_of_mass_helper (
    ...
    vm_vec_scale_add2 (
        infop->maintained_variables.vecp_value, &objp->pos,
        objp->phys_info.mass);
...
// What we're doing here is finding the distances between each extent of the
// object and the line, and then taking the maximum distance as the
// cross-sectional radius.  We're actually maintaining the square of the
// distance rather than the actual distance, as it's faster to calculate and it
// gives the same result in a greater-than or less-than comparison.  When we're
// done calculating everything for all objects (i.e. when we return to the
// parent function) we take the square root of the final value.
void dock_calc_max_cross_sectional_radius_squared_perpendicular_to_line_helper (
    object* objp, dock_function_info* infop) {
    ...
    // We must find world coordinates for each of the six endpoints on the
    // three axes of the object.  I looked up which axis is front/back,
    // left/right, and up/down, as well as which endpoint is which.  It doesn't
    // really matter, though, as all we need are the distances.
    ...
    for (i = 0; i < 6; i++) {
        ...
        vm_vec_dist_squared_to_line (
            &world_point, line_start, line_end, &nearest, &dist_squared);
...
// What we're doing here is projecting each object extent onto the directrix,
// calculating the distance between the projected point and the origin, and
// then taking the maximum distance as the semilatus rectum.  We're actually
// maintaining the square of the distance rather than the actual distance, as
// it's faster to calculate and it gives the same result in a greater-than or
// less-than comparison.  When we're done calculating everything for all
// objects (i.e. when we return to the parent function) we take the square root
// of the final value.
void dock_calc_max_semilatus_rectum_squared_parallel_to_directrix_helper (
    object* objp, dock_function_info* infop) {
    vec3d world_point, local_point[6], nearest;
    ...
    for (i = 0; i < 6; i++) {
        ...
        vm_vec_dist_squared_to_line (
            &world_point, line_start, line_end, &nearest, &temp);

        // find the distance squared between the origin of the line and the
        // point on the line
...
// -------------------------------------------------------------------------------------------
bool dock_check_assume_hub () {
    // There are several ways of handling ships docking to other ships.  Level
    // 1, the simplest, is the one-docker, one-dockee model used in retail FS2.
    // Level 2 is the hub model, where we stipulate that any given set of
    // docked ships includes one ship to which all other ships are docked.  No
    // ship except for the hub ship can be docked to more than one ship.  Level
    // 3 is the daisy-chain model, where you can string ships along and make a
    // rooted tree.
    //
    // The new code can handle level 3 ship formations, but it requires more
    // overhead than level 2 or level 1.  (Whether the additional overhead is
    // significant or not has not been determined.)  In the vast majority of
    // cases, level 3 is not needed.  So this function is provided to allow the
    // code to optimize itself for level 2, should level 1 evaluation fail.
...
void dock_remove_instance (object* objp, object* other_objp) {
    ...
    if (found) {
        ...
        if (prev_ptr == NULL) { objp->dock_list = ptr->next; }
        // normal case
        else {
