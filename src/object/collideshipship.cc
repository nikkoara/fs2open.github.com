        ...
        collision_info_struct *ship_ship_hit_info)
{
        ...
        ASSERT(
                heavy->type == OBJ_SHIP || heavy->type == OBJ_ASTEROID || heavy->type == OBJ_DEBRIS);
        ASSERT(
                lighter->type == OBJ_SHIP || lighter->type == OBJ_ASTEROID || lighter->type == OBJ_DEBRIS);

        ship_info *light_sip = (lighter->type == OBJ_SHIP)
                                       ? &Ship_info[Ships[lighter->instance].ship_info_index]
                                       : NULL;
        ship_info *heavy_sip = (heavy->type == OBJ_SHIP)
        ...
        special_cruiser_asteroid_collision = check_special_cruiser_asteroid_collision(
        ...
        if (ship_ship_hit_info->submodel_rot_hit == 1) {
                ...
                if (pmi != NULL && pmi->submodel[ship_ship_hit_info->submodel_num].sii != NULL) {
                        ...
                        if (pm->submodel[ship_ship_hit_info->submodel_num].movement_axis == MOVEMENT_AXIS_X) {
                                ...
                                pm->submodel[ship_ship_hit_info->submodel_num].movement_axis == MOVEMENT_AXIS_Y) {
                                axis = vmd_y_vector;
                        } else if (
                                pm->submodel[ship_ship_hit_info->submodel_num].movement_axis == MOVEMENT_AXIS_Z) {
                                axis = vmd_z_vector;
                        } else {
        ...
        if (subsys_landing_allowed && light_local_vel.xyz.z < light_sip->collision_physics.landing_max_z && light_local_vel.xyz.z > light_sip->collision_physics.landing_min_z && light_local_vel.xyz.y > light_sip->collision_physics.landing_min_y && fabsf(light_local_vel.xyz.x) < light_sip->collision_physics.landing_max_x && light_uvec_dot_norm > 0 && light_fvec_dot_norm < light_sip->collision_physics.landing_max_angle && light_fvec_dot_norm > light_sip->collision_physics.landing_min_angle && fabsf(light_rvec_dot_norm) < light_sip->collision_physics.landing_max_rot_angle) {
        ...
        if (heavy == Player_obj) {
                ...
                float rotation_factor = (heavy->type == OBJ_SHIP)
        ...
        if (lighter == Player_obj || subsys_landing_allowed) {
                ...
                float rotation_factor = (lighter->type == OBJ_SHIP)
        ...
        if (subsys_landing_allowed && light_local_vel.xyz.z < light_sip->collision_physics.reorient_max_z && light_local_vel.xyz.z > light_sip->collision_physics.reorient_min_z && light_local_vel.xyz.y > light_sip->collision_physics.reorient_min_y && fabsf(light_local_vel.xyz.x) < light_sip->collision_physics.reorient_max_x && light_uvec_dot_norm > 0 && light_fvec_dot_norm < light_sip->collision_physics.reorient_max_angle && light_fvec_dot_norm > light_sip->collision_physics.reorient_min_angle && fabsf(light_rvec_dot_norm) < light_sip->collision_physics.reorient_max_rot_angle) {
                vec3d landing_delta_rotvel;
                landing_delta_rotvel.xyz.x = (light_fvec_dot_norm * light_sip->collision_physics.reorient_mult) - light_sip->collision_physics.landing_rest_angle;
                ...
                float xzVelMag = sqrt(
                        light_local_vel.xyz.x * light_local_vel.xyz.x + light_local_vel.xyz.z * light_local_vel.xyz.z);
                float xzVelDotOrient = MIN(
                        MAX(
                                (xzVelMag > 0 ? (light_local_vel.xyz.x / xzVelMag) : 0),
                                -0.5f),
                        0.5f);
                landing_delta_rotvel.xyz.y = (xzVelMag > 2)
        ...
        vm_vec_scale_add2(
                ...
                0.2f * lighter->phys_info.mass / (heavy->phys_info.mass + lighter->phys_info.mass));
        vm_vec_scale_add2(
                &heavy->pos, &ship_ship_hit_info->collision_normal,
                -0.1f * lighter->phys_info.mass / (heavy->phys_info.mass + lighter->phys_info.mass));
        ...
        if (ship_ship_hit_info->is_landing) {
                ...
                vm_vec_scale_add2(
                        ...
                        -0.2f * heavy->phys_info.mass / (heavy->phys_info.mass + lighter->phys_info.mass));
                vm_vec_scale_add2(
                        &lighter->pos, &ship_ship_hit_info->collision_normal,
                        0.1f * heavy->phys_info.mass / (heavy->phys_info.mass + lighter->phys_info.mass));
        }
...
int collide_ship_ship(obj_pair *pair)
{
        ...
        if (dist < A->radius + B->radius) {
                ...
                if (hit) {
                        ...
                        if (!ship_ship_hit_info.is_landing) {
                                ...
                                if ((LightOne->flags[Object::Object_Flags::Player_ship]) || (HeavyOne->flags[Object::Object_Flags::Player_ship])) {
                                        ...
                                        Ships[LightOne->instance].team == Ships[HeavyOne->instance].team) {
                ...
                if (((Ships[A->instance].flags[Ship::Ship_Flags::Arriving_stage_1]) && (Ship_info[Ships[A->instance].ship_info_index].is_huge_ship())) || ((Ships[B->instance].flags[Ship::Ship_Flags::Arriving_stage_1]) && (Ship_info[Ships[B->instance].ship_info_index].is_huge_ship()))) {
                ...
                if (time > 0) {
                        pair->next_check_time = timestamp(int(time));
                } else {
                        pair->next_check_time = timestamp(0); // check next time
                }
        }
