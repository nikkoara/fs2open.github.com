...
public:
    ...
    collider_pair ()
        : a (NULL), b (NULL), signature_a (-1), signature_b (-1),
          next_check_time (-1), initialized (false) {}
};
...
void obj_reset_pairs () {
    ...
    for (i = 0; i < MIN_PAIRS; i++) { Obj_pairs[i].next = &Obj_pairs[i + 1]; }
...
int reject_obj_pair_on_parent (object* A, object* B) {
    if (A->type == OBJ_SHIP) {
        if (B->type == OBJ_DEBRIS) {
            if (B->parent_sig == A->signature) { return 0; }
        }
    }

    if (B->type == OBJ_SHIP) {
        if (A->type == OBJ_DEBRIS) {
            if (A->parent_sig == B->signature) { return 0; }
        }
    }

    if (A->parent_sig == B->signature) { return 1; }

    if (B->parent_sig == A->signature) { return 1; }

    return 0;
}

int reject_due_collision_groups (object* A, object* B) {
    if (A->collision_group_id == 0 || B->collision_group_id == 0) return 0;
...
void obj_add_pair (object* A, object* B, int check_time, int add_to_end) {
    ...
    if (!(A->flags[Object::Object_Flags::Collides]))
        return; // This object doesn't collide with anything
    if (!(B->flags[Object::Object_Flags::Collides]))
        return; // This object doesn't collide with anything

    if ((A->flags[Object::Object_Flags::Immobile]) &&
        (B->flags[Object::Object_Flags::Immobile]))
        return; // Two immobile objects will never collide with each other

    // Make sure you're not checking a parent with it's kid or vicy-versy
    // if ( A->parent_sig == B->signature && !(A->type == OBJ_SHIP && B->type
    //== OBJ_DEBRIS) ) return;  if ( B->parent_sig == A->signature && !(A->type
    //== OBJ_DEBRIS && B->type == OBJ_SHIP) ) return;
    if (reject_obj_pair_on_parent (A, B)) { return; }
    ...
    case COLLISION_OF (OBJ_BEAM, OBJ_SHIP):
        if (beam_collide_early_out (A, B)) { return; }
    ...
    case COLLISION_OF (OBJ_WEAPON, OBJ_WEAPON): {
        ...
        /*
           ...
           (bwip->wi_flags&WIF_BOMB) ) { check_collision =
        ...
        */
        /*
                ...
                atype =
           Weapon_info[Weapons[A->instance].weapon_info_index].subtype; btype =
    ...
    // MWA -- 4/1/98 -- I'd do it, but I don't want to bust anything, so I'm
    // doing my stuff here instead :-)
    // if ( MULTIPLAYER_CLIENT && !(Netgame.debug_flags &
    // NETD_FLAG_CLIENT_NODAMAGE)){
    // multiplayer clients will only do ship/ship collisions, and their own
    // ship to boot
    ...
    if (check_collision == collide_debris_weapon) {
        ...
        if (!(Weapon_info[Weapons[B->instance].weapon_info_index]
                  .wi_flags[Weapon::Info_Flags::Turns])) {
            ...
            if (Weapon_info[Weapons[B->instance].weapon_info_index].subtype ==
                ...
                vm_vec_sub (
                    &velocity_rel_weapon, &B->phys_info.vel,
                    &A->phys_info.vel);
            ...
            if (vm_vec_dist_squared (&A->pos, &B->pos) >
                (vm_vec_mag_squared (&delta_v) *
            ...
            if ((B->parent >= 0) &&
                ...
                (vm_vec_dist (&B->pos, &A->pos) < (4.0f * A->radius + 200.0f)))
                return;
        }
    }
    ...
    if (check_collision == collide_ship_weapon) {
        ...
        if ((B->parent >= 0) &&
            (Objects[B->parent].signature == B->parent_sig) &&
            !(Objects[B->parent].flags[Object::Object_Flags::Player_ship]) &&
            (Ships[Objects[B->parent].instance].team ==
             Ships[A->instance].team) &&
    ...
    */
    ...
    if (check_time == -1) {
        new_pair->next_check_time =
            timestamp (0); // 0 means instantly time out
    }
    else {
...
void obj_check_all_collisions () {
    ...
    while (tmp != NULL) {
        ...
        if (!timestamp_elapsed (tmp->next_check_time)) goto NextPair;
...
// We never need to check this pair again.
    ...
    NextPair:
        if (!removed) {
            ...
            if (tmp) {
                ...
                if (add_time > 0) avg_time_to_next_check += (float)add_time;
            }
#endif
        }
    }
    ...
    // mprintf(( "[pairs checked: %d, start_pairs: %d, num obj: %d, avg next
    // time: %f]\n", n, org_pairs, Num_objects, avg_time_to_next_check ));
    // mprintf(( "[Num_hull_pieces: %3d, Num_weapons_created: %3d,
    // pairs_not_created: %3d, pairs_created: %3d, percent new saved:
    ...
    // FYI: (n*(n-1))/2 is the total number of checks required for comparing n
    // objects.
...
int collide_subdivide (
    ...
    if (ab_dist < a_dist + b_dist + prad + qrad) {
        if (ab_dist < prad + qrad)
            ...
            if (!r1) r2 = collide_subdivide (&pa, p1, prad, &qa, q1, qrad);
    ...
    if (radius_scale == 0.0f) {
        ...
        if (A->phys_info.speed != 0.0f) {
            r = find_nearest_point_on_line (
                &nearest_point, &prev_pos, &A->pos, &B->pos);
            if (r < 0) { nearest_point = prev_pos; }
            else if (r > 1) {
                nearest_point = A->pos;
            }
...
int vector_object_collision (
    ...
    r = find_nearest_point_on_line (
        &nearest_point, start_pos, end_pos, &objp->pos);
    if ((r >= 0.0f) && (r <= 1.0f)) {
...
int weapon_will_never_hit (
    object* obj_weapon, object* other, obj_pair* current_pair) {
    ...
    if (!(wip->wi_flags[Weapon::Info_Flags::Turns])) {
        ...
        // are heading in opposite directions.   If so, we don't need to ever
        // check them again.   This is only valid for weapons that don't turn.

        float vdot;
        if (wip->subtype == WP_LASER) {
            vec3d velocity_rel_weapon;
            vm_vec_sub (
                &velocity_rel_weapon, &obj_weapon->phys_info.vel,
                &other->phys_info.vel);
            vdot = -vm_vec_dot (
                &velocity_rel_weapon, &obj_weapon->orient.vec.fvec);
        }
        else {
            vdot =
                vm_vec_dot (&other->phys_info.vel, &obj_weapon->phys_info.vel);
        }
        if (vdot <= 0.0f) {
            // They're heading in opposite directions...
            // check their positions
            ...
            float pdot =
                vm_vec_dot (&obj_weapon->orient.vec.fvec, &weapon2other);
    ...
    if (current_pair) {
        // Find the time it will take before these get within each others
        // distances. tmp->next_check_time = timestamp(500);
        ...
        // If the PF_CONST_VEL flag is set, we can safely assume it doesn't
        // change speed.
        ...
        // check weapon that does not turn against sphere expanding at ship
        // maxvel compare (weeapon) ray with expanding sphere (ship) to find
        // earliest possible collision time look for two time solutions to Xw =
        // Xs, where Xw = Xw0 + Vwt*t  Xs = Xs + Vs*(t+dt), where Vs*dt =
        // radius of ship Since direction of Vs is unknown, solve for (Vs*t)
        // and find norm of both sides
        if (!(wip->wi_flags[Weapon::Info_Flags::Turns])) {
            ...
            if (max_vel_weapon == max_vel_other) {
                // this will give us NAN using the below formula, so check
                // every frame
            ...
            // vm_vec_copy_scale( &laser_vel, &weapon->orient.vec.fvec,
            // max_vel_weapon );
            delta_t = (other->radius + 10.0f) /
                      max_vel_other; // time to get from center to radius of
                                     // other obj
            delta_x_dot_vl = vm_vec_dot (&delta_x, &laser_vel);

            a = max_vel_weapon * max_vel_weapon -
                max_vel_other * max_vel_other;
            b = 2.0f *
                (delta_x_dot_vl - max_vel_other * max_vel_other * delta_t);
            c = vm_vec_mag_squared (&delta_x) -
                max_vel_other * max_vel_other * delta_t * delta_t;
            ...
            if (max_vel_weapon > max_vel_other) {
                ...
                if (root1 > 0) { earliest_time = root1; }
                else if (root2 > 0) {
                    // root1 < 0 and root2 > 0, so we're inside sphere and next
                    // check should be next frame
                    current_pair->next_check_time =
                        timestamp (0); // check next time
                    return 0;
                }
                else {
            ...
            // need to modify it for weapons that are slower than ships
                ...
                if (root2 > 0) { earliest_time = root2; }
                else {
                    current_pair->next_check_time = timestamp (0);
                    return 0;
                }
            ...
            if (earliest_time > 1000 * wp->lifeleft) return 1;
            ...
            // suggest that fudge factor for other radius be changed to
            // other_radius + const (~10)
            dist = vm_vec_dist (&other->pos, &obj_weapon->pos) -
                   (other->radius + 10.0f);
            if (dist > 0.0f) {
                ...
                time_ms -= 200; // Allow at least one worst case frametime to
                                // elapse (~5 fps)

                if (time_ms > 100) { // If it takes longer than 1/10th of a
                                     // second, then delay it
...
int pp_collide (
    ...
    mc.model_num = Ship_info[Ships[goalobjp->instance].ship_info_index]
...
int collide_predict_large_ship (object* objp, float distance) {
    ...
    for (objp2 = GET_FIRST (&obj_used_list);
         objp2 != END_OF_LIST (&obj_used_list); objp2 = GET_NEXT (objp2)) {
        if ((objp != objp2) && (objp2->type == OBJ_SHIP)) {
            if (Ship_info[Ships[objp2->instance].ship_info_index]
                    .is_big_or_huge ()) {
                if (dock_check_find_docked_object (objp, objp2)) continue;

                if (cpls_aux (&goal_pos, objp2, objp)) return 1;
            }
        }
        else if (!(sip->is_big_or_huge ()) && (objp2->type == OBJ_ASTEROID)) {
            if (vm_vec_dist_quick (&objp2->pos, &objp->pos) <
                (distance + objp2->radius) * 2.5f) {
                vec3d pos, delvec;
                int count;
                float d1;
                ...
                for (; count > 0; count--) {
                    if (vm_vec_dist_quick (&pos, &objp2->pos) <
                        objp->radius + objp2->radius)
                        return 1;
...
// which could be deleted since they are not going to hit anything.  Passed
// into this function is a 'time' parameter used as watermark for which weapons
// to check.
...
void crw_check_weapon (int weapon_num, int collide_next_check) {
    ...
    // if this weapons life left > time before next collision, then we cannot
    // remove it
    crw_status[WEAPON_INDEX (wp)] = CRW_IN_PAIR;
    next_check_time =
        ((float)(timestamp_until (collide_next_check)) / 1000.0f);
...
int collide_remove_weapons () {
    ...
    if (Cmdline_old_collision_sys) {
        ...
        opp = opp->next;
        while (opp != NULL) {
            // for each collide pair, if the two objects can still collide,
            // then set the remove_weapon parameter for the weapon to 0.  need
            // to check both parameters
        ...
        for (it = Collision_cached_pairs.begin ();
             it != Collision_cached_pairs.end (); ++it) {
            pair_obj = &it->second;

            if (!pair_obj->initialized) { continue; }

            if (pair_obj->a->type == OBJ_WEAPON &&
                pair_obj->signature_a == pair_obj->a->signature) {
                crw_check_weapon (
                    pair_obj->a->instance, pair_obj->next_check_time);
            ...
            if (pair_obj->b->type == OBJ_WEAPON &&
                pair_obj->signature_b == pair_obj->b->signature) {
                crw_check_weapon (
                    pair_obj->b->instance, pair_obj->next_check_time);
    ...
    if (num_deleted) return num_deleted;

    // if we didn't remove any weapons, try to the N oldest weapons.  first
    // checking for pairs, then checking for oldest weapons in general.  We
    // will go through the loop a max of 2 times.  first time through, we check
    // oldest weapons with pairs, next time through, for oldest weapons.
    loop_count = 0;
    do {
        for (j = 0; j < CRW_MAX_TO_DELETE; j++) {
            ...
            for (i = 0; i < MAX_WEAPONS; i++) {
                if (Weapons[i].objnum ==
                    -1) // shouldn't happen, but this is the safe thing to do.
                    continue;
                if (((loop_count || crw_status[i] == CRW_NO_PAIR)) &&
                    (Weapons[i].lifeleft < oldest_time)) {
...
void set_hit_struct_info (
    collision_info_struct* hit, mc_info* mc, int submodel_rot_hit) {
...
void obj_add_collider (int obj_index) {
    ...
    CheckObjects[obj_index].flags =
        objp->flags - Object::Object_Flags::Not_in_coll;
    CheckObjects[obj_index].parent_sig = objp->parent_sig;
    CheckObjects[obj_index].parent_type = objp->parent_type;
#endif

    if (!(objp->flags[Object::Object_Flags::Not_in_coll])) { return; }
...
void obj_sort_and_collide () {
    ...
    std::vector< int > sort_list_y;
    std::vector< int > sort_list_z;

    sort_list_y.clear ();
    {
        TRACE_SCOPE (tracing::SortColliders);
        obj_quicksort_colliders (
            &Collision_sort_list, 0, (int)(Collision_sort_list.size () - 1),
            0);
    }
    ...
    sort_list_z.clear ();
    {
        TRACE_SCOPE (tracing::SortColliders);
        obj_quicksort_colliders (
            &sort_list_y, 0, (int)(sort_list_y.size () - 1), 1);
    }
    ...
    sort_list_y.clear ();
    {
        TRACE_SCOPE (tracing::SortColliders);
        obj_quicksort_colliders (
            &sort_list_z, 0, (int)(sort_list_z.size () - 1), 2);
    }
    obj_find_overlap_colliders (&sort_list_y, &sort_list_z, 2, true);
}

void obj_find_overlap_colliders (
    std::vector< int >* overlap_list_out, std::vector< int >* list, int axis,
    bool collide) {
    ...
    for (i = 0; i < (*list).size (); ++i) {
        ...
        for (j = 0; j < overlappers.size ();) {
            overlap_max =
                obj_get_collider_endpoint (overlappers[j], axis, false);
            if (min <= overlap_max) {
                overlapped = true;
                ...
                if (collide) {
                    obj_collide_pair (
                        &Objects[(*list)[i]], &Objects[overlappers[j]]);
                }
            }
            else {
        ...
        if (overlappers.empty ()) { first_not_added = true; }

        if (overlapped) { overlap_list_out->push_back ((*list)[i]); }
...
float obj_get_collider_endpoint (int obj_num, int axis, bool min) {
    if (Objects[obj_num].type == OBJ_BEAM) {
        ...
        if (Objects[obj_num].pos.a1d[axis] >
            Objects[obj_num].last_pos.a1d[axis]) {
        ...
        if (min) { return min_end - Objects[obj_num].radius; }
        else {
        ...
        if (min) { return pos->a1d[axis] - Objects[obj_num].radius; }
        else {
...
void obj_quicksort_colliders (
    std::vector< int >* list, int left, int right, int axis) {
    ASSERT (axis >= 0);
    ASSERT (axis <= 2);
    ...
    if (right > left) {
        ...
        float pivot_value =
            obj_get_collider_endpoint ((*list)[pivot_index], axis, true);
        ...
        for (i = left; i < right; ++i) {
            if (obj_get_collider_endpoint ((*list)[i], axis, true) <=
                pivot_value) {
                temp = (*list)[i];
...
void obj_collide_pair (object* A, object* B) {
    ...
    case COLLISION_OF (OBJ_SHIP, OBJ_BEAM):
        if (beam_collide_early_out (B, A)) { return; }
        swapped = 1;
        check_collision = beam_collide_ship;
        break;
    ...
    if (collision_info->initialized) {
        // make sure we're referring to the correct objects in case the
        // original pair was deleted
    ...
    if (valid && A->type != OBJ_BEAM) {
        // if this signature is valid, make the necessary checks to see if we
        // need to collide check
        if (collision_info->next_check_time == -1) { return; }
        else {
            ...
            if (!(Weapon_info[Weapons[B->instance].weapon_info_index]
                ...
                if (Weapon_info[Weapons[B->instance].weapon_info_index]
                    ...
                    vm_vec_sub (
                        ...
                        &A->phys_info.vel);
                    vdot = -vm_vec_dot (
                        &velocity_rel_weapon, &B->orient.vec.fvec);
                }
                else {
                    vdot = vm_vec_dot (&A->phys_info.vel, &B->phys_info.vel);
                }
                if (vdot <= 0.0f) {
                    ...
                    vm_vec_sub (&weapon2other, &A->pos, &B->pos);
                    float pdot =
                        vm_vec_dot (&B->orient.vec.fvec, &weapon2other);
                    if (pdot <= -A->radius) {
                        // The other object is behind the weapon by more than
                        // its radius, so it will never hit...
                        collision_info->next_check_time = -1;
                ...
                // for nonplayer ships, only create collision pair if close
                // enough
                if ((B->parent >= 0) &&
                    !((Objects[B->parent].signature == B->parent_sig) &&
                      (Objects[B->parent]
                           .flags[Object::Object_Flags::Player_ship])) &&
                    (vm_vec_dist (&B->pos, &A->pos) <
                     (4.0f * A->radius + 200.0f))) {
                    collision_info->next_check_time = -1;
                    return;
                }
            }
        ...
        // don't check same team laser:ship collisions on small ships if not
        // player
        if (check_collision == collide_ship_weapon) {
            // weapon is B
            if ((B->parent >= 0) &&
