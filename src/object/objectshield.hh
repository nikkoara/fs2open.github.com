 ...
 * @param[in] penalty      Percent of the shield energy to lose during the
 * balance (0 < x <= 1)
 *
 * @author z64555
 *
 * @details Each quadrant is observed to exponentially approach the average
 * shield HP, weaker quadrants quickly fill up while stronger quadrants fille
 * up slower. We don't have to worry about the shield gaining energy from
 *   somewhere, since the sum of each (shield_hp_avg - shield_quadrant[i]) is
 * 0. Applying a balance rate is legal since it's equivalent to applying the
 * rate to the entire sum, which is still 0.
 *
 ...
 *
 * @details The transfer sips a percentage of each quadrant, taking more energy
 * from stronger quadrants and less from weaker ones.
 */
 ...
 *
 * @param[in] quadrant_num Index of the quadrant/sector to set.
 * @param[in] strength HP to add (or subtract if negative) to the
 * quadrant/sector
 *
 * @author Goober5000
 */
 ...
 *
 * @note $Max Shield Recharge is not intended to affect max strength of
 * individual shield segments
 *
 * @author Goober5000
 */
...
/**
 * @brief Sets the max shield HP of the given object. Use this to init or
 * override a ship's default shield HP
 */
...
/**
 * @brief Applies damage to the given shield quandrant/sector of the given
 * object.
 *
 ...
 *
 * @param[in] quadrant_num Quadrant index to check. If -1, then check if the
 * entire shield is stronger than 10%
 *
...
#endif // FREESPACE2_OBJECT_OBJECTSHIELD_HH
