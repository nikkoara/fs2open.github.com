...
const char* Object_type_names[MAX_OBJECT_TYPES] = {
    // XSTR:OFF
    "None",     "Ship",     "Weapon",    "Fireball",
    "Start",    "Waypoint", "Debris",    "Countermeasure",
    "Ghost",    "Point",    "Shockwave", "Wing",
    "Observer", "Asteroid", "Jump Node", "Beam",
    // XSTR:ON
};
...
// all we need to set are the pointers, but type, parent, and instance are
// useful to set as well
object::object ()
...
// DO NOT set next and prev to NULL because they keep the object on the free
// and used lists
void object::clear () {
...
int free_object_slots (int num_used) {
    ...
    num_already_free = 0;
    for (objp = GET_FIRST (&obj_free_list);
         objp != END_OF_LIST (&obj_free_list); objp = GET_NEXT (objp))
        num_already_free++;

    if (MAX_OBJECTS - num_already_free < num_used) return 0;
    ...
    if (!num_to_free) return original_num_to_free;
...
float get_hull_pct (object* objp) {
    ...
    if (objp->hull_strength <
        0.0f) // this sometimes happens when a ship is being destroyed
        return 0.0f;
...
void obj_init () {
    ...
    Object_inited = 1;
    for (i = 0; i < MAX_OBJECTS; ++i) Objects[i].clear ();
    Viewer_obj = NULL;
    ...
    if (Cmdline_old_collision_sys) { obj_reset_pairs (); }
    else {
        obj_reset_colliders ();
    }
}
...
void obj_shutdown () {
    for (auto& obj : Objects) { obj.clear (); }
}
...
int obj_allocate (void) {
    ...
    if (Num_objects > num_objects_hwm) { num_objects_hwm = Num_objects; }
 ...
 * This function deallocates the object entry after the object has been
 * unlinked
 */
 ...
 *
 * The object will be a non-rendering, non-physics object.   Pass -1 if no
 * parent.
...
int obj_create (
    ...
    obj->n_quadrants =
        DEFAULT_SHIELD_SECTIONS; // Might be changed by the ship creation code
...
void obj_delete (int objnum) {
    ...
    if (Cmdline_old_collision_sys) { obj_remove_pairs (objp); }
    else {
    ...
    case OBJ_SHIP:
        if ((objp == Player_obj) && !Fred_running) {
            ...
            // we have to traverse the ship_obj list and remove this guy from
            // it as well
    ...
    case OBJ_SHOCKWAVE: shockwave_delete (objp); break;
    case OBJ_START:
    case OBJ_WAYPOINT:
    case OBJ_POINT: break; // requires no action, handled by the Fred code.
    case OBJ_JUMP_NODE:
        break; // requires no further action, handled by jumpnode
               // deconstructor.
    case OBJ_DEBRIS: debris_delete (objp); break;
    case OBJ_ASTEROID:
    ...
    while (objp != END_OF_LIST (&obj_used_list)) {
        ...
        if (objp->flags[Object::Object_Flags::Should_be_dead])
            obj_delete (OBJ_INDEX (
                objp)); // MWA says that john says that let obj_delete handle
                        // everything because of the editor
        objp = temp;
    }
}
...
void obj_merge_created_list (void) {
    ...
    while (objp != END_OF_LIST (&obj_create_list)) {
        ...
        if (Cmdline_old_collision_sys) { obj_add_pairs (OBJ_INDEX (objp)); }
        else {
...
void obj_move_one_docked_object (object* objp, object* parent_objp) {
    ...
    if ((aip->mode == AIM_DOCK) && (aip->submode >= AIS_UNDOCK_1)) {
        if (aip->goal_objnum == OBJ_INDEX (parent_objp)) { return; }
    }

    // check the guy that I'm docked with and don't move if he's undocking from
    // me
    ai_info* other_aip = &Ai_info[Ships[parent_objp->instance].ai_index];
    if ((other_aip->mode == AIM_DOCK) &&
        (other_aip->submode >= AIS_UNDOCK_1)) {
        if (other_aip->goal_objnum == OBJ_INDEX (objp)) { return; }
    }
...
void obj_player_fire_stuff (object* objp, control_info ci) {
    ...
    // single player pilots, and all players in multiplayer take care of firing
    // their own primaries
        ...
        if (ci.fire_primary_count) {
            ...
            if (shipp != NULL) {
                ...
                ship_stop_fire_primary (
                    objp); // if it hasn't fired do the "has just stoped
                           // fireing" stuff
            }
        }
    ...
    if (ci.afterburner_stop) { afterburners_stop (objp, 1); }
}
...
void obj_move_call_physics (object* objp, float frametime) {
    ...
    if (objp->flags[Object::Object_Flags::Physics]) {
        ...
        if ((objp->type == OBJ_SHIP) &&
            !(Ships[objp->instance].flags[Ship::Ship_Flags::Dying])) {
            ship* shipp = &Ships[objp->instance];
            float engine_strength;

            engine_strength =
                ship_get_subsystem_strength (shipp, SUBSYSTEM_ENGINE);
            ...
            if (engine_strength ==
                ...
                objp->phys_info.side_slip_time_const =
                    Ship_info[shipp->ship_info_index].damp * 4.0f;
            }
            ...
            if (shipp->weapons.num_secondary_banks > 0) {
                ...
                for (int i = 0; i < shipp->weapons.num_secondary_banks; i++) {
                    ...
                    float fire_wait =
                        Weapon_info[shipp->weapons.secondary_bank_weapons[i]]
                            .fire_wait;
                    float reload_time =
                        (fire_wait == 0.0f) ? 1.0f : 1.0f / fire_wait;

                    // ok so...we want to move up missles but only if there is
                    // a missle there to be moved up there is a missle behind
                    // next_point, and how ever many missles there are left
                    // after that

                    if (points > missles_left) {
                        // there are more slots than missles left, so not all
                        // of the slots will have missles drawn on them
                        for (int k = next_point; k < next_point + missles_left;
                             k++) {
                            float& s_pct =
                                shipp->secondary_point_reload_pct[i]
                                                                 [k % points];
                            if (s_pct < 1.0) s_pct += reload_time * frametime;
                        ...
                        for (int k = 0; k < points; k++) {
                            float& s_pct =
                                shipp->secondary_point_reload_pct[i][k];
                            if (s_pct < 1.0) s_pct += reload_time * frametime;
                            if (s_pct > 1.0) s_pct = 1.0f;
                        }
                    }
        ...
        if ((objp->type == OBJ_WEAPON) &&
            (Weapons[objp->instance]
                 .weapon_flags[Weapon::Weapon_Flags::Dead_in_water])) {
            vm_vec_zero (&objp->phys_info.desired_vel);
            vm_vec_zero (&objp->phys_info.desired_rotvel);
            objp->phys_info.flags |= (PF_REDUCED_DAMP | PF_DEAD_DAMP);
            objp->phys_info.side_slip_time_const =
                1.0f; // FIXME?  originally indexed into Ship_info[], which was
                      // a bug...
        }
        ...
        if (physics_paused) {
            ...
            // if the object is the player object, do things that need to be
            ...
            // fire streaming weapons for ships in here - ALL PLAYERS,
            // regardless of client, single player, server, whatever. do stream
            // weapon firing for all ships themselves.
            if (objp->type == OBJ_SHIP) {
    ...
    if (has_fired == -1) {
        ship_stop_fire_primary (
            objp); // if it hasn't fired do the "has just stoped fireing" stuff
    }
...
void obj_check_object (object* obj) {
    ...
    if (CheckObjects[objnum].type != obj->type) {
        if ((obj->type == OBJ_WAYPOINT) &&
            ...
            // We know about player changing into a ghost after dying and that
            // is ok.
            CheckObjects[objnum].type = OBJ_GHOST;
        }
        else if (
...
void obj_set_flags (
    ...
    if ((obj->flags[Object::Object_Flags::Collides]) &&
        (!(new_flags[Object::Object_Flags::Collides]))) {
        // Remove all object pairs
        if (Cmdline_old_collision_sys) { obj_remove_pairs (obj); }
        else {
            obj_remove_collider (objnum);
        }
    ...
    if ((!(obj->flags[Object::Object_Flags::Collides])) &&
        ...
        if (Cmdline_old_collision_sys) { obj_add_pairs (objnum); }
        else {
...
void obj_move_all_pre (object* objp, float frametime) {
    ...
    switch (objp->type) {
    case OBJ_WEAPON:
        if (!physics_paused) { weapon_process_pre (objp, frametime); }
        break;
    case OBJ_SHIP:
    ...
    case OBJ_ASTEROID:
        ...
        /*      case OBJ_CMEASURE:
                ...
                break;*/
    case OBJ_WAYPOINT: break; // waypoints don't move..
...
void obj_move_all_post (object* objp, float frametime) {
    ...
    case OBJ_WEAPON: {
        ...
        if (!physics_paused) weapon_process_post (objp, frametime);
        ...
        if (Detail.lighting > 3) {
            ...
            if (cast_light) {
                ...
                if (wi->render_type == WRT_LASER) {
                    ...
                    // b, objp->parent );
                    light_add_point (
                        &objp->pos, 10.0f, 100.0f, 1.0f, r, g, b,
                        objp->parent);
                }
                else {
                    light_add_point (
                        &objp->pos, 10.0f, 20.0f, 1.0f, 1.0f, 1.0f, 1.0f,
                        objp->parent);
                }
            }
        }
    ...
    case OBJ_SHIP: {
            ...
            if ((Detail.lighting > 3) && (objp != Viewer_obj)) {
                ...
                for (i = 0; i < MAX_SHIP_ARCS; i++) {
                    if (timestamp_valid (shipp->arc_timestamp[i])) {
                        ...
                        vec3d tmp1, tmp2;
                        vm_vec_unrotate (
                            &tmp1, &shipp->arc_pts[i][0], &objp->orient);
                        vm_vec_add2 (&tmp1, &objp->pos);

                        vm_vec_unrotate (
                            &tmp2, &shipp->arc_pts[i][1], &objp->orient);
        ...
        if (Ship_info[shipp->ship_info_index].uses_team_colors &&
            strcasecmp (shipp->secondary_team_name.c_str (), "none") != 0) {
            if (f2fl (Missiontime) * 1000 >
                f2fl (shipp->team_change_timestamp) * 1000 +
                    shipp->team_change_time) {
    ...
    case OBJ_FIREBALL: {
        ...
        if (!physics_paused) fireball_process_post (objp, frametime);

        if (Detail.lighting > 2) {
            float r = 0.0f, g = 0.0f, b = 0.0f;

            fireball_get_color (
                Fireballs[objp->instance].fireball_info_index, &r, &g, &b);

            // we don't cast black light, so just bail in that case
            if ((r == 0.0f) && (g == 0.0f) && (b == 0.0f)) break;
            ...
            if (p > 0.0f) {
                ...
                if (fireball_is_warp (objp)) {
                    intensity = fireball_wormhole_intensity (
                        objp); // Valathil: Get wormhole radius for lighting
                    rad = objp->radius;
                }
                ...
                if (rad > 0.0001f)
                    light_add_point (
                        &objp->pos, rad * 2.0f, rad * 5.0f, intensity, r, g, b,
                        -1);
            }
        }
    ...
    case OBJ_DEBRIS: {
        ...
        if (!physics_paused) debris_process_post (objp, frametime);
                ...
                if (db->arc_frequency > 0) {
                    for (i = 0; i < MAX_DEBRIS_ARCS; i++) {
                        if (timestamp_valid (db->arc_timestamp[i])) {
                            ...
                            vec3d tmp1, tmp2;
                            vm_vec_unrotate (
                                &tmp1, &db->arc_pts[i][0], &objp->orient);
                            vm_vec_add2 (&tmp1, &objp->pos);

                            vm_vec_unrotate (
                                &tmp2, &db->arc_pts[i][1], &objp->orient);
                            vm_vec_add2 (&tmp2, &objp->pos);
    ...
    case OBJ_ASTEROID: {
        ...
        if (!physics_paused) asteroid_process_post (objp);
...
void obj_move_all (float frametime) {
    ...
    if (global_cmeasure_timer) Cmeasures_homing_check--;

    // Goober5000 - HACK HACK HACK
    // this function also resets the OF_DOCKED_ALREADY_HANDLED flag, to save
    // trips through the used object list
    ...
    for (objp = GET_FIRST (&obj_used_list);
        ...
        if (objp->flags[Object::Object_Flags::Should_be_dead]) { continue; }

        // if this is an observer object, skip it
        if (objp->type == OBJ_OBSERVER) { continue; }

        // Compile a list of active countermeasures during an existing
        // traversal of obj_used_list
        if (objp->type == OBJ_WEAPON) {
            ...
            if (wip->wi_flags[Weapon::Info_Flags::Cmeasure]) {
                if ((wip->cmeasure_timer_interval > 0 &&
                     timestamp_elapsed (
                         wp->cmeasure_timer)) // If it's timer-based and ready
                                              // to pulse...
                    ||
                    (wip->cmeasure_timer_interval <= 0 &&
                     global_cmeasure_timer)) { // ...or it's not and the global
                                               // counter is active...
                    // ...then it's actively pulsing and we need to add objp to
                    // cmeasure_list.
                    cmeasure_list.push_back (objp);
                    if (wip->cmeasure_timer_interval > 0) {
                        // Reset the timer
                        wp->cmeasure_timer =
                            timestamp (wip->cmeasure_timer_interval);
                    }
                }
            }
        }
        ...
        // Goober5000 - skip objects which don't move, but only until they're
        // destroyed
    ...
    // intrinsic rotations.  We do that here because we already handled the
    // ship models in obj_move_all_post, and this is more or less conceptually
    // close enough to move the rest.  (Originally all models were
    // intrinsic-rotated here, but for sequencing reasons, intrinsic ship
    ...
    while (objp != END_OF_LIST (&obj_used_list)) {
        ...
        // Valathil - Move the screen rotation calculation for billboards here
        // to get the updated orientation matrices caused by docking
        // interpolation
        vec3d tangles;
        ...
        if (&objp->phys_info == Viewer_physics_info) {
            ...
            if (objp->dock_list &&
                objp->dock_list->docked_objp->type == OBJ_SHIP &&
                Ai_info[Ships[objp->dock_list->docked_objp->instance].ai_index]
                        .submode == AIS_DOCK_4) {
...
void obj_queue_render (object* obj, model_draw_list* scene) {
    ...
    case OBJ_WEAPON:
        ...
        break;
    case OBJ_SHIP: ship_render (obj, scene); break;
    case OBJ_FIREBALL: fireball_render (obj, scene); break;
    case OBJ_SHOCKWAVE: shockwave_render (obj, scene); break;
    case OBJ_DEBRIS: debris_render (obj, scene); break;
    case OBJ_ASTEROID: asteroid_render (obj, scene); break;
    case OBJ_JUMP_NODE:
        for (std::list< CJumpNode >::iterator jnp = Jump_nodes.begin ();
             jnp != Jump_nodes.end (); ++jnp) {
            if (jnp->GetSCPObject () != obj) { continue; }
...
void obj_get_average_ship_pos (vec3d* pos) {
    ...
    if (count) vm_vec_scale (pos, 1.0f / (float)count);
}
...
int obj_team (object* objp) {
    ...
    case OBJ_JUMP_NODE: team = Player_ship->team; break;
...
void obj_remove_pairs (object* a) {
    ...
    while (tmp != NULL) {
        if ((tmp->a == a) || (tmp->b == a)) {
            // Hmmm... a potenial compiler optimization problem here... either
            // tmp->a or tmp->b is equal to 'a' and we modify 'num_pairs' in
...
void obj_reset_all_collisions () {
    // clear checkobjects
#ifndef NDEBUG
    for (int i = 0; i < MAX_OBJECTS; ++i) { CheckObjects[i] = checkobject (); }
#endif
    ...
    while (moveup != END_OF_LIST (&obj_used_list)) {
        ...
        if (Cmdline_old_collision_sys) { obj_add_pairs (OBJ_INDEX (moveup)); }
        else {
...
// Goober5000
int object_is_docked (object* objp) { return (objp->dock_list != NULL); }
...
void object_set_gliding (object* objp, bool enable, bool force) {
    ...
    if (enable) {
        ...
        vm_vec_rotate (
            &objp->phys_info.prev_ramp_vel, &objp->phys_info.vel,
            &objp->orient); // Backslash
    }
}
...
bool object_get_gliding (object* objp) {
    ...
    return (
        ...
        ((objp->phys_info.flags & PF_FORCE_GLIDE) != 0));
}
...
int obj_get_by_signature (int sig) {
    ...
    while (objp != END_OF_LIST (&obj_used_list)) {
        if (objp->signature == sig) return OBJ_INDEX (objp);
...
int object_get_model (object* objp) {
    ...
    case OBJ_ASTEROID: {
        ...
        return Asteroid_info[asp->asteroid_type]
