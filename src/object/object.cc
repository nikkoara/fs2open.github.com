...
void obj_move_call_physics(object *objp, float frametime)
{
        ...
        if (objp->flags[Object::Object_Flags::Physics]) {
                ...
                if ((objp->type == OBJ_SHIP) && !(Ships[objp->instance].flags[Ship::Ship_Flags::Dying])) {
                        ...
                        if (engine_strength == 0.0f) { // All this is necessary to make ship gradually come
                        ...
                        if (shipp->weapons.num_secondary_banks > 0) {
                                ...
                                for (int i = 0; i < shipp->weapons.num_secondary_banks; i++) {
                                        ...
                                        float fire_wait = Weapon_info[shipp->weapons.secondary_bank_weapons[i]]
                                                                  .fire_wait;
                                        ...
                                        if (points > missles_left) {
                                                ...
                                                for (int k = next_point; k < next_point + missles_left;
                                                     k++) {
                                                        float &s_pct = shipp->secondary_point_reload_pct[i]
                                                                                                        [k % points];
                                                        if (s_pct < 1.0)
                ...
                if ((objp->type == OBJ_WEAPON) && (Weapons[objp->instance].weapon_flags[Weapon::Weapon_Flags::Dead_in_water])) {
                        ...
                        vm_vec_zero(&objp->phys_info.desired_rotvel);
                        objp->phys_info.flags |= (PF_REDUCED_DAMP | PF_DEAD_DAMP);
                        objp->phys_info.side_slip_time_const = 1.0f; // FIXME?  originally indexed into Ship_info[], which was
                                                                     // a bug...
                }
                ...
                if (physics_paused) {
                        ...
                        if (objp->type == OBJ_SHIP) {
                                ...
                                if (/* (objnum_I_am_docked_or_docking_with != -1) || */
                                    ((aip->mode == AIM_DOCK) && ((aip->submode == AIS_DOCK_2) || (aip->submode == AIS_DOCK_3) || (aip->submode == AIS_UNDOCK_0))) || ((aip->mode == AIM_WARP_OUT) && (aip->submode >= AIS_WARP_3))) {
                                        if (ship_get_subsystem_strength(
                                                    &Ships[objp->instance], SUBSYSTEM_ENGINE)
                                            > 0.0f) {
                                                objp->phys_info.flags |= PF_USE_VEL;
                                        } else {
                                                objp->phys_info.flags &= ~PF_USE_VEL; // If engine blown, don't PF_USE_VEL,
...
void obj_check_object(object *obj)
{
        ...
        if (CheckObjects[objnum].type != obj->type) {
                if ((obj->type == OBJ_WAYPOINT) && (CheckObjects[objnum].type == OBJ_SHIP)) {
                        ...
                        (obj->type == OBJ_SHIP) && (CheckObjects[objnum].type == OBJ_GHOST)) {
                        // We know about player changing into a ghost after dying and that
                        // is ok.
                        CheckObjects[objnum].type = OBJ_GHOST;
                } else if (
                        (obj->type == OBJ_GHOST) && (CheckObjects[objnum].type == OBJ_SHIP)) {
                        // We know about player changing into a ghost after dying and that
                        // is ok.
...
void obj_move_all_post(object *objp, float frametime)
{
        ...
        case OBJ_SHIP: {
                ...
                ship *shipp = &Ships[objp->instance];
                if (Ship_info[shipp->ship_info_index].uses_team_colors && strcasecmp(shipp->secondary_team_name.c_str(), "none") != 0) {
             ...
             objp != END_OF_LIST(&obj_used_list); objp = GET_NEXT(objp)) {
                ...
                if (objp->type == OBJ_WEAPON) {
                        ...
                        if (wip->wi_flags[Weapon::Info_Flags::Cmeasure]) {
                                if ((wip->cmeasure_timer_interval > 0 && timestamp_elapsed(wp->cmeasure_timer)) // If it's timer-based and ready
                                                                                                                // to pulse...
                                    || (wip->cmeasure_timer_interval <= 0 && global_cmeasure_timer)) {          // ...or it's not and the global
                                                                                                                // counter is active...
...
bool object_get_gliding(object *objp)
{
        ...
        return (
                ((objp->phys_info.flags & PF_GLIDING) != 0) || ((objp->phys_info.flags & PF_FORCE_GLIDE) != 0));
}
