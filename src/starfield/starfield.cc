...
void stars_post_level_init()
{
        ...
        stars_load_debris(
                ((The_mission.flags[Mission::Mission_Flags::Fullneb]) || Nebula_sexp_used));
...
void stars_draw_lens_flare(vertex *sun_vex, int sun_n)
{
    ...
    opposite position on the screen. */
        dx = 2.0f * (float(gr_screen.clip_right - gr_screen.clip_left) * 0.5f - sun_vex->screen.xyw.x);
        dy = 2.0f * (float(gr_screen.clip_bottom - gr_screen.clip_top) * 0.5f - sun_vex->screen.xyw.y);
...
void subspace_render()
{
        ...
        if (!Rendering_to_env) {
                ...
                framenum = int((Subspace_glow_frame * NOISE_NUM_FRAMES) / total_time);
        ...
        int env, bool in_mission)
{
        ...
        if (!Rendering_to_env && (Game_detail_flags & DETAIL_FLAG_MOTION) && (supernova_active() < 3) && (!Cmdline_nomotiondebris) && in_mission) {
                stars_draw_debris();
        }
...
int stars_find_sun(char *name)
{
        ...
        // lookup
        for (idx = 0; idx < (int)Sun_bitmaps.size(); idx++) {
                if (!strcasecmp(name, Sun_bitmaps[idx].filename)) {
                        return idx;
                }
        }
...
starfield_bitmap *stars_get_bitmap_entry(int index, bool is_a_sun)
{
        ...
        if (is_a_sun && (Suns[index].star_bitmap_index >= 0)) {
                ...
                !is_a_sun && (Starfield_bitmap_instances[index].star_bitmap_index >= 0)) {
