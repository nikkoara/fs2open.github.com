...
typedef struct starfield_bitmap {
    ...
    flare_info flare_infos[MAX_FLARE_COUNT]; // each flare can use a texture in
                                             // flare_bmp, with different scale
    flare_bitmap flare_bitmaps[MAX_FLARE_BMP]; // bitmaps for different lens
                                               // flares (can be re-used)
...
typedef struct starfield_bitmap_instance {
    ...
    starfield_bitmap_instance ()
        : scale_x (1.0f), scale_y (1.0f), div_x (1), div_y (1),
          star_bitmap_index (0), n_verts (0), verts (NULL) {
        ang.p = 0.0f;
        ang.b = 0.0f;
        ang.h = 0.0f;
    }
...
int stars_debris_loaded =
    0; // 0 = not loaded, 1 = normal vclips, 2 = nebula vclips
...
void stars_load_debris_vclips (debris_vclip* vclips) {
    ...
    for (i = 0; i < MAX_DEBRIS_VCLIPS; i++) {
        vclips[i].bm = bm_load_animation (
            vclips[i].name, &vclips[i].nframes, nullptr, nullptr, nullptr,
            true);
...
void stars_load_debris (int fullneb) {
    if (Cmdline_nomotiondebris) { return; }
...
extern void
stars_project_2d_onto_sphere (vec3d* pnt, float rho, float phi, float theta);

static void starfield_create_bitmap_buffer (const int si_idx) {
    vec3d s_points[MAX_PERSPECTIVE_DIVISIONS + 1]
                  [MAX_PERSPECTIVE_DIVISIONS + 1];
    vec3d t_points[MAX_PERSPECTIVE_DIVISIONS + 1]
                  [MAX_PERSPECTIVE_DIVISIONS + 1];
    ...
    if (sbi->verts != NULL) { delete[] sbi->verts; }
    ...
    // scale_x *= (gr_screen.clip_aspect + (0.7333333f/gr_screen.clip_aspect));
    // // fudge factor
    ...
    for (idx = 0; idx <= div_x; idx++) {
        for (s_idx = 0; s_idx <= div_y; s_idx++) {
            ...
            stars_project_2d_onto_sphere (
                &s_points[idx][s_idx], 1000.0f, s_phi + ((float)idx * d_phi),
                s_theta + ((float)s_idx * d_theta));

            // bank the bitmap first
            vm_vec_rotate (
                &t_points[idx][s_idx], &s_points[idx][s_idx], &m_bank);
...
static void starfield_bitmap_entry_init (starfield_bitmap* sbm) {
    ...
    for (i = 0; i < MAX_FLARE_COUNT; i++) { sbm->flare_infos[i].tex_num = -1; }
}
...
void parse_startbl (const char* filename) {
    ...
    try {
        ...
        // freaky! ;)
        while (!check_for_eof ()) {
            while ((rc = optional_string_either ("$Bitmap:", "$BitmapX:")) !=
                   -1) {
                in_check = true;
                ...
                sbm.xparent = rc; // 0 == intensity alpha bitmap,  1 == green
                                  // xparency bitmap
            ...
            while (optional_string ("$Sun:")) {
                ...
                if (optional_string ("$Flare:")) {
                    ...
                    stuff_string (
                        sbm.flare_bitmaps[0].filename, F_NAME,
                        MAX_FILENAME_LEN);

                    sbm.n_flare_bitmaps = 1;

                    for (idx = 1; idx < MAX_FLARE_BMP; idx++) {
                        // allow 9999 textures (theoretically speaking, that
                        // is)
            ...
            // since it's possible for some idiot to have a tbl screwed up
            // enough that this ends up in an endless loop, give an opportunity
            // to advance through the file no matter what, because even the
            // retail tbl has an extra "#end" line in it.
...
void stars_load_all_bitmaps () {
    ...
    for (idx = 0; idx < (int)Starfield_bitmaps.size (); idx++) {
        ...
        if (sb->bitmap_id < 0) {
            ...
            if (sb->bitmap_id < 0) {
                sb->bitmap_id = bm_load_animation (
                    sb->filename, &sb->n_frames, &sb->fps, nullptr, nullptr,
                    true);
    ...
    for (idx = 0; idx < (int)Sun_bitmaps.size (); idx++) {
        ...
        if (sb->glow_bitmap < 0) {
            ...
            // maybe didn't load a static image so try for an animated one
            if (sb->glow_bitmap < 0) {
                sb->glow_bitmap = bm_load_animation (
                    sb->glow_filename, &sb->glow_n_frames, &sb->glow_fps,
        ...
        if (sb->flare) {
            for (i = 0; i < MAX_FLARE_BMP; i++) {
                if (!strlen (sb->flare_bitmaps[i].filename)) continue;

                if (sb->flare_bitmaps[i].bitmap_id < 0) {
                    sb->flare_bitmaps[i].bitmap_id =
...
void stars_init () {
    ...
    if (Cmdline_env) { ENVMAP = Default_env_map = bm_load ("cubemap"); }
}
...
void stars_pre_level_init (bool clear_backgrounds) {
    ...
    if (clear_backgrounds) {
        // Goober5000 - clear entire array, including backgrounds that will be
        // unused
...
static void environment_map_gen () {
    const int size = 512;
    int gen_flags =
        (BMP_FLAG_RENDER_TARGET_STATIC | BMP_FLAG_CUBEMAP |
         BMP_FLAG_RENDER_TARGET_MIPMAP);

    if (!Cmdline_env) { return; }
    ...
    if (Dynamic_environment ||
        (The_mission.flags[Mission::Mission_Flags::Subspace])) {
    ...
    gr_screen.envmap_render_target =
        bm_make_render_target (size, size, gen_flags);
}

// call this in game_post_level_init() so we know whether we're running in full
// nebula mode or not
void stars_post_level_init () {
    ...
    stars_set_background_model (
        The_mission.skybox_model, NULL, The_mission.skybox_flags);
    ...
    // following code randomly distributes star points within a sphere volume,
    // which avoids there being denser areas along the edges and in corners
    // that we had in the old rectangular distribution scheme.
    ...
    for (i = 0; i < MAX_STARS; i++) {
...

        {
            ...
            gr_init_alphacolor (
                &Stars[i].col, red, green, blue, alpha, AC_TYPE_BLEND);
        }
    }
    ...
    for (i = 0; i < 8; i++) {
        ...
        gr_init_alphacolor (
            &star_aacolors[i], 255, 255, 255, intensity, AC_TYPE_BLEND);
...
DCF (stars, "Set parameters for starfield") {
    ...
    if (dc_optional_string_either ("help", "--help")) {
        dc_printf (
            "Usage: stars keyword\nWhere keyword can be in the following "
            "forms:\n");
        dc_printf ("stars default   Resets stars to all default values\n");
        dc_printf (
            "stars num X     Sets number of stars to X.  Between 0 and %d.\n",
            MAX_STARS);
        dc_printf (
            "stars tail X    Where X is the percent of 'tail' between 0 and "
            "1.0\n");
        dc_printf (
            "stars dim X     Where X is the amount stars dim between 0 and "
            "255.0\n");
        dc_printf (
            "stars cap X     Where X is the cap of dimming between 0 and "
            "255.\n");
        dc_printf ("stars len X     Where X is the cap of length.\n");
        dc_printf (
            "stars m0        Macro0. Old 'pixel type' crappy stars. "
            "flags=none\n");
        dc_printf (
            "stars m1        Macro1. (default) tail=.75, dim=20.0, cap=75.0, "
            "flags=dim,tail\n");
        dc_printf (
            "stars m2        Macro2. tail=.75, dim=20.0, cap=75.0, "
            "flags=dim,tail,aa\n");
        dc_printf (
            "stars flag X    Toggles flag X, where X is tail or dim or aa "
            "(aa=antialias)\n");
        dc_printf (
    ...
    if (arg == "num") {
        ...
        dc_printf (
            "Starfield set: (default) tail=.75, dim=20.0, cap=75.0, "
            "flags=dim,tail\n");
    }
    else if (arg == "m2") {
        Star_amount = 0.75f;
        ...
        dc_printf (
            "Starfield set: tail=.75, dim=20.0, cap=75.0, "
            "flags=dim,tail,aa\n");
    }
    else if (arg == "flag") {
        dc_stuff_string_white (arg);
        if (arg == "tail") { Star_flags ^= STAR_FLAG_TAIL; }
        else if (arg == "dim") {
...
int reload_old_debris =
    1; // If set to one, then reload all the last_pos of the debris
...
void stars_draw_sun (int show_sun) {
    ...
    for (idx = 0; idx < num_suns; idx++) {
        // get the instance
        if (Suns[idx].star_bitmap_index < 0) return;

        bm = &Sun_bitmaps[Suns[idx].star_bitmap_index];

        // if no bitmap then bail...
        if (bm->bitmap_id < 0) continue;
        ...
        // add the light source corresponding to the sun, except when rendering
        // to an envmap
        if (!Rendering_to_env)
            light_add_directional (
                &sun_dir, bm->i, bm->r, bm->g, bm->b, bm->spec_r, bm->spec_g,
                bm->spec_b, true);

        // if supernova
        if (supernova_active () && (idx == 0))
            local_scale =
                1.0f + (SUPERNOVA_SUN_SCALE * supernova_pct_complete ());
        else
            local_scale = 1.0f;
        ...
        int bitmap_id = -1;
        if (bm->fps) {
            // gr_set_bitmap(bm->bitmap_id + ((timestamp() / (int)(bm->fps)) %
            // bm->n_frames), GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL,
            // 0.999f);
            bitmap_id = bm->bitmap_id +
                        ((timestamp () / (int)(bm->fps)) % bm->n_frames);
        }
        else {
            // gr_set_bitmap(bm->bitmap_id, GR_ALPHABLEND_FILTER,
        ...
        if (sun_vex.codes & (CC_BEHIND | CC_OFF_USER)) { continue; }

        if (!(sun_vex.flags & PF_PROJECTED)) { g3_project_vertex (&sun_vex); }

        if (sun_vex.flags & PF_OVERFLOW) { continue; }

        material mat_params;
        material_set_unlit (&mat_params, bitmap_id, 0.999f, true, false);
        g3_render_rect_screen_aligned_2d (
...
void stars_draw_lens_flare (vertex* sun_vex, int sun_n) {
    ...
    // get the instance
    if (Suns[sun_n].star_bitmap_index < 0) { return; }
    else {
    ...
    for (j = 0; j < bm->n_flare_bitmaps; j++) {
        // if no bitmap then bail...
        if (bm->flare_bitmaps[j].bitmap_id < 0) continue;

        // gr_set_bitmap(bm->flare_bitmaps[j].bitmap_id, GR_ALPHABLEND_FILTER,
        // GR_BITBLT_MODE_NORMAL, 0.999f);

        for (i = 0; i < bm->n_flares; i++) {
            // draw sorted by texture, to minimize texture changes. not the
            // most efficient way, but better than non-sorted
            if (bm->flare_infos[i].tex_num == j) {
                flare_vex.screen.xyw.x =
                    sun_vex->screen.xyw.x + dx * bm->flare_infos[i].pos;
                flare_vex.screen.xyw.y =
                    sun_vex->screen.xyw.y + dy * bm->flare_infos[i].pos;
                // g3_draw_bitmap(&flare_vex, 0, 0.05f *
                // bm->flare_infos[i].scale, TMAP_FLAG_TEXTURED);
                material mat_params;
                material_set_unlit (
                    &mat_params, bm->flare_bitmaps[j].bitmap_id, 0.999f, true,
                    false);
                g3_render_rect_screen_aligned_2d (
                    &mat_params, &flare_vex, 0,
                    0.05f * bm->flare_infos[i].scale);
            }
        }
    }
}
...
void stars_draw_sun_glow (int sun_n) {
    ...
    int bitmap_id = -1;
    if (bm->glow_fps) {
        // gr_set_bitmap(bm->glow_bitmap + ((timestamp() / (int)(bm->glow_fps))
        // % bm->glow_n_frames), GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL,
        // 0.5f);
        bitmap_id = bm->glow_bitmap +
                    ((timestamp () / (int)(bm->glow_fps)) % bm->glow_n_frames);
    }
    else {
        // gr_set_bitmap(bm->glow_bitmap, GR_ALPHABLEND_FILTER,
        // GR_BITBLT_MODE_NORMAL, 0.5f);
        bitmap_id = bm->glow_bitmap;
    }

    g3_rotate_faraway_vertex (&sun_vex, &sun_pos);
    // int zbuff = gr_zbuffer_set(GR_ZBUFF_NONE);
    // g3_draw_bitmap(&sun_vex, 0, 0.10f * Suns[sun_n].scale_x * local_scale,
    // TMAP_FLAG_TEXTURED);
    material mat_params;
    material_set_unlit (&mat_params, bitmap_id, 0.5f, true, false);
    g3_render_rect_screen_aligned_2d (
        &mat_params, &sun_vex, 0, 0.10f * Suns[sun_n].scale_x * local_scale);
    ...
    if (bm->flare) {
        ...
        if (dot > 0.7f) // Only render the flares if the sun is reasonably near
...
void stars_draw_bitmaps (int show_bitmaps) {
    ...
    for (idx = 0; idx < sb_instances; idx++) {
        ...
        // if no bitmap then bail...
        if (Starfield_bitmaps[star_index].bitmap_id < 0) { continue; }
        ...
        if (Starfield_bitmaps[star_index].xparent) {
            if (Starfield_bitmaps[star_index].fps) {
                bitmap_id = Starfield_bitmaps[star_index].bitmap_id +
                            ((timestamp () /
                              (int)(Starfield_bitmaps[star_index].fps)) %
        ...
        material_set_unlit (
            &material_params, bitmap_id, alpha, blending, false);
        g3_render_primitives_textured (
            &material_params, Starfield_bitmap_instances[idx].verts,
            Starfield_bitmap_instances[idx].n_verts, PRIM_TYPE_TRIS, false);
    }
...
DCF (subspace_set, "Set parameters for subspace effect") {
    ...
    if (dc_optional_string_either ("help", "--help")) {
        ...
        dc_printf (
            "<axis>  -- May be either 'u' or 'v', and corresponds to the "
            "texture axis\n");
        dc_printf (
            "<speed> -- is the speed along the axis that the texture is "
            "moved\n");
        return;
    }
...
void subspace_render () {
    ...
    gr_set_texture_panning (
        Interp_subspace_offset_v, Interp_subspace_offset_u, true);

    model_render_immediate (
        &render_info, Subspace_model_outer, &tmp,
        &Eye_position); // MR_NO_CORRECT|MR_SHOW_OUTLINE
    ...
    model_render_immediate (
        &render_info, Subspace_model_inner, &tmp,
        &Eye_position); // MR_NO_CORRECT|MR_SHOW_OUTLINE
    ...
    // gr_set_bitmap(Subspace_glow_bitmap, GR_ALPHABLEND_FILTER,
    ...
    // g3_draw_bitmap(&glow_vex, 0, 17.0f + 0.5f * Noise[framenum],
    // TMAP_FLAG_TEXTURED);
    g3_render_rect_screen_aligned_2d (
        &mat_params, &glow_vex, 0, 17.0f + 0.5f * Noise[framenum]);
...
void stars_draw_stars () {
    ...
    for (i = 0; i < tmp_num_stars; i++) {
        ...
        if (p2.codes) { can_draw = 0; }
        else {
            g3_project_vertex (&p2);
            if (p2.flags & PF_OVERFLOW) { can_draw = 0; }
        }
        ...
        if (can_draw && (Star_flags & (STAR_FLAG_TAIL | STAR_FLAG_DIM))) {
            ...
            if (p1.codes) { can_draw = 0; }
            else {
                g3_project_vertex (&p1);
                if (p1.flags & PF_OVERFLOW) { can_draw = 0; }
            }
        }
...
void stars_draw_debris () {
    ...
    if (Motion_debris_override) return;
    ...
    for (i = 0; i < MAX_DEBRIS; i++, d++) {
        ...
        if (p.codes == 0) {
            int frame =
                Missiontime / (DEBRIS_ROT_MIN + (i % DEBRIS_ROT_RANGE) *
                                                    DEBRIS_ROT_RANGE_SCALER);
            ...
            // g3_draw_laser( &d->pos,d->size,&tmp,d->size,
            // TMAP_FLAG_TEXTURED|TMAP_FLAG_XPARENT, 25.0f );
            material mat_params;
            material_set_unlit (
                &mat_params, Debris_vclips[d->vclip].bm + frame, alpha, true,
                true);
            g3_render_laser_2d (
                &mat_params, &d->pos, d->size, &tmp, d->size, 25.0f);
        }
...
void stars_draw (
    int show_stars, int show_suns, int /*show_nebulas*/, int show_subspace,
    int env, bool in_mission) {
    ...
    if (show_subspace) subspace_render ();
    ...
    else if (!show_subspace) // don't render the background pof when rendering
                             // subspace
    {
        stars_draw_background ();
    }

    if (!env && show_stars && (Nmodel_num < 0) &&
        (Game_detail_flags & DETAIL_FLAG_STARS) &&
        !(The_mission.flags[Mission::Mission_Flags::Fullneb]) &&
        (supernova_active () < 3)) {
        stars_draw_stars ();
    }
...
void stars_page_in () {
    ...
    // extra SEXP related checks to preload anything that might get used from
    // there
    for (idx = 0; idx < (int)Starfield_bitmaps.size (); idx++) {
        sb = &Starfield_bitmaps[idx];

        if (sb->used_this_level) continue;
        ...
        if (sb->preload) {
            if (sb->bitmap_id < 0) {
                ...
                if (sb->bitmap_id < 0) {
                    sb->bitmap_id = bm_load_animation (
                        sb->filename, &sb->n_frames, &sb->fps, nullptr,
                        nullptr, true);
            ...
            // this happens whether it loaded properly or not, no harm should
            // come from it
            if (sb->xparent) { bm_page_in_xparent_texture (sb->bitmap_id); }
            else {
    ...
    for (idx = 0; idx < (int)Starfield_bitmap_instances.size (); idx++) {
        ...
        if (sbi->star_bitmap_index < 0) continue;
        ...
        // this happens whether it loaded properly or not, no harm should come
        // from it
        if (sb->xparent) { bm_page_in_xparent_texture (sb->bitmap_id); }
        else {
            bm_page_in_texture (sb->bitmap_id);
    ...
    for (idx = 0; idx < MAX_DEBRIS_VCLIPS; idx++) {
        bm_page_in_xparent_texture (
            Debris_vclips[idx].bm, Debris_vclips[idx].nframes);
    }
}
...
void stars_draw_background () {
    ...
    if (Nmodel_num < 0) return;

    model_render_params render_info;

    if (Nmodel_bitmap >= 0) { render_info.set_forced_bitmap (Nmodel_bitmap); }
    ...
    model_render_immediate (
        &render_info, Nmodel_num, &Nmodel_orient, &Eye_position,
        MODEL_RENDER_ALL, false);
}

// call this to set a specific model as the background model
void stars_set_background_model (
    const char* model_name, const char* texture_name, int flags) {
    if (Nmodel_bitmap >= 0) {
    ...
    if ((model_name == NULL) || (*model_name == '\0')) return;
...
void stars_set_background_orientation (matrix* orient) {
    if (orient == NULL) { vm_set_identity (&Nmodel_orient); }
    else {
...
int stars_add_sun_entry (starfield_list_entry* sun_ptr) {
    ...
    if (Sun_bitmaps[idx].bitmap_id < 0) {
        ...
        // maybe didn't load a static image so try for an animated one
        if (Sun_bitmaps[idx].bitmap_id < 0) {
            Sun_bitmaps[idx].bitmap_id = bm_load_animation (
                Sun_bitmaps[idx].filename, &Sun_bitmaps[idx].n_frames,
                &Sun_bitmaps[idx].fps, nullptr, nullptr, true);
        ...
        if (Sun_bitmaps[idx].glow_bitmap < 0) {
            Sun_bitmaps[idx].glow_bitmap =
                bm_load (Sun_bitmaps[idx].glow_filename);

            // maybe didn't load a static image so try for an animated one
            if (Sun_bitmaps[idx].glow_bitmap < 0) {
                Sun_bitmaps[idx].glow_bitmap = bm_load_animation (
                    Sun_bitmaps[idx].glow_filename,
                    &Sun_bitmaps[idx].glow_n_frames,
        ...
        if (Sun_bitmaps[idx].flare) {
            for (i = 0; i < MAX_FLARE_BMP; i++) {
                ...
                if (!strlen (fbp->filename)) continue;
...
// add an instance for a starfield bitmap (something actually used in a
// mission) NOTE that we assume a duplicate is ok here
int stars_add_bitmap_entry (starfield_list_entry* sle) {
    ...
    if (Starfield_bitmaps[idx].bitmap_id < 0) {
        Starfield_bitmaps[idx].bitmap_id =
            bm_load (Starfield_bitmaps[idx].filename);

        // maybe didn't load a static image so try for an animated one
        if (Starfield_bitmaps[idx].bitmap_id < 0) {
            Starfield_bitmaps[idx].bitmap_id = bm_load_animation (
                Starfield_bitmaps[idx].filename,
                &Starfield_bitmaps[idx].n_frames, &Starfield_bitmaps[idx].fps,
                nullptr, nullptr, true);
    ...
    starfield_create_bitmap_buffer (
        (int)(Starfield_bitmap_instances.size () - 1));

    return (int)(Starfield_bitmap_instances.size () - 1);
}

// get the number of entries that each vector contains
// "is_a_sun" will get sun instance counts, otherwise it gets normal starfield
// bitmap instance counts "bitmap_count" will get number of starfield_bitmap
// entries rather than starfield_bitmap_instance entries
int stars_get_num_entries (bool is_a_sun, bool bitmap_count) {
    // try for instance counts first
    if (!bitmap_count) {
        if (is_a_sun) { return (int)Suns.size (); }
        else {
    ...
    // looks like we want bitmap counts (probably only FRED uses this)
        ...
        if (is_a_sun) { return (int)Sun_bitmaps.size (); }
        else {
...
starfield_bitmap* stars_get_bitmap_entry (int index, bool is_a_sun) {
    ...
    if (is_a_sun && (Suns[index].star_bitmap_index >= 0)) {
        ...
        !is_a_sun &&
        (Starfield_bitmap_instances[index].star_bitmap_index >= 0)) {
        return &Starfield_bitmaps[Starfield_bitmap_instances[index]
                                      .star_bitmap_index];
    }
...
void stars_mark_instance_unused (int index, bool is_a_sun) {
    ...
    if (is_a_sun) { Suns[index].star_bitmap_index = -1; }
    else {
...
int stars_get_first_valid_background () {
    ...
    if (Num_backgrounds == 0) return -1;

    // scan every background except the last and return the first one that has
    // all its suns and bitmaps present
...
bool stars_background_empty (int idx) {
    return (
        Backgrounds[idx].suns.empty () && Backgrounds[idx].bitmaps.empty ());
}
...
static void render_environment (
    int i, vec3d* eye_pos, matrix* new_orient, float new_zoom) {
    ...
    gr_set_proj_matrix (
        PI_2 * new_zoom, 1.0f, Min_draw_distance, Max_draw_distance);
    gr_set_view_matrix (&Eye_position, &Eye_matrix);

    if (Game_subspace_effect) { stars_draw (0, 0, 0, 1, 1); }
    else {
...
void stars_setup_environment_mapping (camid cid) {
    ...
    // prefer the mission specified envmap over the static-generated envmap,
    // but the dynamic envmap should always get preference if in a subspace
    // mission
    ...
    if (gr_screen.envmap_render_target < 0) {
        ...
        if (Mission_env_map >= 0) { ENVMAP = Mission_env_map; }
        else {
            ENVMAP = Default_env_map;
        }
    ...
    if (!Dynamic_environment) { Env_cubemap_drawn = true; }
}
