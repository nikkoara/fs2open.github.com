...
typedef struct tagSNDPATTERN {
    ...
    int next_pattern;         // Next pattern to play at loop time (can be same
                              // pattern)
    ...
    int force_pattern; // flag to indicate that we want to not continue loop,
                       // but go to next_pattern
...
int Num_soundtracks;
int Current_soundtrack_num; // Active soundtrack for the current mission..
                            // index into Soundtracks[]
...
// stores the number of measures for the different patterns (data from
// music.tbl)
...
// Because of how inflexible music.tbl is, to make it flexible, we must keep
// the stuff in the old order. So to make it reasonable, we use this. This is a
// list of the old pattern indexes. If you add a song you should add it in the
// right spot in here, with a value of largest+1.
...
int Pattern_default_next[MAX_PATTERNS] =
{
    ...
    SONG_NRML_1,	// NRML_2 progresses to NRML_1 by default (but we intercept
this) SONG_NRML_1		// NRML_3 progresses to NRML_1 by default (but we
intercept this)
};


// Certain patterns can be interrupted (such as the long-playing NRML and BTTL
tracks).
...
// Holds file names of spooled music that is played at menus, briefings,
// credits etc. Indexed into by a #define enumeration of the different kinds of
// spooled music
menu_music Spooled_music[MAX_SPOOLED_MUSIC];
int Num_music_files; // Number of spooled music files

// Array that holds indicies into Spooled_music[], these specify which music is
// played in briefing/debriefing
...
void event_music_init () {
    ...
    // Goober5000
    // set all the filenames to "none" so we're compatible with the extra NRMLs
    // in FS1 music
...
void event_music_close () {
    if (Event_music_inited == FALSE) return;
...
int event_music_cycle_pattern () {
    ...
    if (!(Soundtracks[Current_soundtrack_num].flags & EMF_CYCLE_FS1)) {
        if (Current_pattern == SONG_BTTL_3 && new_pattern == SONG_BTTL_1) {
            ...
            if (Player_obj != NULL && Player_ship != NULL) {
                ...
                float integrity = Player_obj->hull_strength /
                                  Player_ship->ship_max_hull_strength;
    ...
    if ((new_pattern == SONG_BTTL_2) && (Patterns[SONG_BTTL_2].handle < 0)) {
        ...
        (new_pattern == SONG_BTTL_3) && (Patterns[SONG_BTTL_3].handle < 0)) {
        if ((Current_pattern == SONG_BTTL_2) ||
            (Patterns[SONG_BTTL_2].handle < 0))
            new_pattern = SONG_BTTL_1;
        else
            new_pattern = SONG_BTTL_2;
    }
    ...
    if ((new_pattern == SONG_NRML_2) && (Patterns[SONG_NRML_2].handle < 0)) {
        ...
        (new_pattern == SONG_NRML_3) && (Patterns[SONG_NRML_3].handle < 0)) {
        new_pattern = SONG_NRML_1;
    }
...
//
// Performs a switch between patterns.  Sets the cutoff limit for the pattern
// that is being switch to.
//
void event_music_force_switch () {
    if (Event_music_enabled == FALSE) return;

    if (Event_music_level_inited == FALSE) return;

    Patterns[Current_pattern].loop_for =
        Patterns[Current_pattern].default_loop_for;

    int new_pattern = event_music_cycle_pattern ();

    if (new_pattern == -1) { return; }

    if (Patterns[new_pattern].num_measures == 0) return; // invalid pattern
...
//
// Called once per game frame, to check for transitions of patterns (and to
// start the music off at the beginning).
//
void event_music_do_frame () {
    ...
    if (timestamp_elapsed (Pattern_timer_id)) {
        ...
        if (Current_pattern != -1 && Patterns[Current_pattern].handle >= 0) {
            ...
            audiostream_play (
                ...
                (Master_event_music_volume * aav_music_volume),
                0); // no looping
            audiostream_set_sample_cutoff (
                Patterns[Current_pattern].handle,
                fl2i (
    ...
    if (Event_music_begun == FALSE) { return; }

    if (Current_pattern != -1) {
        SNDPATTERN* pat;
        pat = &Patterns[Current_pattern];

        // First case: switching to a different track since first track is
        // almost at end
        ...
        // Second case: looping back to start of same track since at the end
            ...
            !audiostream_is_playing (pat->handle) &&
            !audiostream_is_paused (pat->handle)) {
        ...
        if (Event_Music_battle_started == 0) {
            if (Current_pattern == SONG_NRML_1 ||
                Current_pattern == SONG_NRML_2 ||
                Current_pattern == SONG_NRML_3) {
                ...
                if (timestamp_elapsed (Battle_over_timestamp)) {
                    ...
                    if (hostile_ships_present () == FALSE) {
                        ...
                        if (Soundtracks[Current_soundtrack_num].flags &
                            EMF_CYCLE_FS1) {
                            Patterns[Current_pattern].next_pattern =
                                SONG_NRML_3;
                        }
            ...
            // We want to go back to NRML track music if all the hostiles have
            // been destroyed, and we are still playing the battle music
            if (Current_pattern == SONG_BTTL_1 ||
                Current_pattern == SONG_BTTL_2 ||
                Current_pattern == SONG_BTTL_3) {
                if (timestamp_elapsed (Battle_over_timestamp)) {
                    if (hostile_ships_present () == FALSE) {
                        if (Patterns[Current_pattern].next_pattern !=
                            SONG_VICT_2) {
                            // Goober5000
                            if (Soundtracks[Current_soundtrack_num].flags &
                                EMF_CYCLE_FS1) {
...
//
// Called at the start of a mission (level).  Sets up the pattern data, and
...
void event_music_level_init (int force_soundtrack) {
    ...
    if (!audiostream_is_inited ()) return;

    if (Event_music_level_inited == TRUE) return;
    ...
    if (force_soundtrack != -1) Current_soundtrack_num = force_soundtrack;
    ...
    Assert (
        Current_soundtrack_num >= 0 &&
        Current_soundtrack_num < Num_soundtracks);
    ...
    // Goober5000 - changed from strack->num_patterns to MAX_PATTERNS so that
    // *all* patterns would be checked; the behavior is the same because they
    // were previously set to none.wav in event_music_parse_musictbl, but this
    // change was needed because the NRML_2 and NRML_3 at the end of the
    ...
    for (i = 0; i < MAX_PATTERNS; i++) {
        ...
        Patterns[i].handle =
            audiostream_open (strack->pattern_fnames[i], ASF_EVENTMUSIC);
        ...
        Patterns[i].samples_per_measure =
            Pattern_samples_per_measure[Current_soundtrack_num][i];
        Patterns[i].num_measures =
            Pattern_num_measures[Current_soundtrack_num][i];
    }
    ...
    if (Event_music_level_inited) {
        if (force_soundtrack >= 0) event_music_first_pattern ();
    }
}
...
void event_music_first_pattern () {
    ...
    if (Event_music_level_inited == FALSE) { event_music_level_init (); }

    if (Event_music_level_inited == FALSE) { //-V581
        return;
    }

    if (Event_music_begun == TRUE) { return; }
    ...
    Event_music_begun = FALSE;
    if (Event_Music_battle_started == TRUE) { Current_pattern = SONG_BTTL_1; }
    else {
...
//
// Called at the end of each mission (level).  Stops any playing patterns by
// fading them out.
//
void event_music_level_close () {
    ...
    if (Current_soundtrack_num >= 0 &&
        Current_soundtrack_num < MAX_SOUNDTRACKS) {
        SOUNDTRACK_INFO* strack;
...
int event_music_battle_start () {
    if (!hostile_ships_present ()) { return 0; }
    ...
    if (Event_Music_battle_started == 1) { return 0; }

    if (Event_music_enabled == FALSE) return -1;

    if (Event_music_level_inited == FALSE) return -1;

    if (Current_pattern == SONG_BTTL_1) return 0; // already playing
    ...
    Event_Music_battle_started =
        1; // keep track of this state though, need on restore
    Battle_over_timestamp = timestamp (BATTLE_CHECK_INTERVAL);
...
int event_music_enemy_arrival () {
    ...
    int next_pattern;
    if (Event_Music_battle_started == TRUE) { next_pattern = SONG_EARV_2; }
    else {
        next_pattern = SONG_EARV_1;
    }

    if (Current_pattern < 0) return 0;

    if (Current_pattern == next_pattern) return 0; // already playing
    ...
    if (Soundtracks[Current_soundtrack_num].flags & EMF_CYCLE_FS1) {
        ...
        // AL 7-25-99: If hull is less than 70% then switch to battle 2 or 3,
        // otherwise switch to 1 or 2
...
int event_music_friendly_arrival () {
    ...
    if (timestamp_elapsed (Next_arrival_timestamp) == false) { return 0; }

    int next_pattern;
    if (Event_Music_battle_started == TRUE) { next_pattern = SONG_AARV_2; }
    else {
    ...
    if (Current_pattern < 0 || Current_pattern >= MAX_PATTERNS) return 0;
    ...
    if (Current_pattern != -1) {
        ...
        // Goober5000 - do this based on a flag (set in music.tbl for FS1
        // soundtracks)

        // overlay
        if (Soundtracks[Current_soundtrack_num].flags &
            EMF_ALLIED_ARRIVAL_OVERLAY) {
            // Goober5000 - I didn't touch this part... for some reason, FS2
            // only has one arrival music pattern, and this is it
...
int event_music_primary_goal_failed () {
    ...
    if (Patterns[SONG_FAIL_1].handle <
        0) // can't play if music file doesn't exist
        return 0;

    if (hostile_ships_present ()) { next_pattern = SONG_BTTL_1; }
    else {
...
int event_music_primary_goals_met () {
    ...
    if (hostile_ships_present ()) {
        ...
        // If the mission goals aren't met (or there are no goals), or if
        // victory 2 music has already played, then go to the next default
        // track
        if (!mission_goals_met () || Victory2_music_played ||
            (Num_goals == 0)) {
            Patterns[next_pattern].next_pattern =
                Patterns[next_pattern].default_next_pattern;
        }
        else {
            Victory2_music_played = 1;
        }
...
int event_music_player_death () {
    ...
    if (Current_pattern == SONG_DEAD_1) return 0; // already playing
...
bool parse_soundtrack_line (int strack_idx, int pattern_idx) {
    ...
    while (token != NULL) {
        ...
        if (count == 0) {
            Pattern_num_measures[strack_idx][pattern_idx] =
                (float)atof (token); // Num_measures
        }
        else if (count == 1) {
            Pattern_samples_per_measure[strack_idx][pattern_idx] =
                atoi (token); // Samples per measure
        }
...
void parse_soundtrack () {
    ...
    if (strack_idx < 0 && (nocreate || Num_soundtracks >= MAX_SOUNDTRACKS)) {
        ...
        if (!skip_to_start_of_string_either (
                "#SoundTrack Start", "#Menu Music Start") &&
            !skip_to_string ("#SoundTrack End")) {
            error_display (
                1,
                "Couldn't find #Soundtrack Start, #Menu Music Start or "
                "#Soundtrack End.");
        }
    ...
    if (check_for_string ("$Name:")) {
        ...
        for (i = 0; i < Num_pattern_types; i++) {
            ...
            if (optional_string (tagbuf)) {
                // Parse it
                if (parse_soundtrack_line (strack_idx, i)) {
                    // If the new pattern is higher than the old one, change
                    // num_patterns
                    if (i + 1 > Soundtracks[strack_idx].num_patterns) {
                        Soundtracks[strack_idx].num_patterns = i + 1;
                    }
    ...
    // Goober5000 - set the valid flag according to whether we can load all our
    // patterns (since someone may be running an enhanced music.tbl without
    // warble_fs1 installed)
    for (i = 0; i < Soundtracks[strack_idx].num_patterns; i++) {
        ...
        // check for file
        if (!cf_exists_full (
                Soundtracks[strack_idx].pattern_fnames[i], CF_TYPE_MUSIC))
            return;
    }
...
void parse_menumusic () {
    ...
    if (cf_exists_full_ext (
            Spooled_music[idx].filename, CF_TYPE_MUSIC, NUM_AUDIO_EXT,
            audio_ext_list))
        Spooled_music[idx].flags |= SMF_VALID;

    if (!nocreate) Num_music_files++;
}

// -------------------------------------------------------------------------------------------------
// event_music_parse_musictbl() will parse the music.tbl file, and set up the
// Mission_songs[] array
//
void event_music_parse_musictbl (const char* filename) {
    try {
        ...
        while (check_for_string ("#Soundtrack Start") ||
               check_for_string ("#Menu Music Start")) {
            if (optional_string ("#Soundtrack Start")) { parse_soundtrack (); }
            if (optional_string ("#Menu Music Start")) {
                while (required_string_either ("$Name", "#Menu Music End") ==
                       0) {
                    parse_menumusic ();
                }
            }
...
void event_music_change_pattern (int new_pattern) {
    ...
    if (Current_pattern == new_pattern) return; // already playing
...
//
// Simply return what the current pattern being played is.  Don't want to make
// global.
//
int event_music_return_current_pattern () { return Current_pattern; }

// -------------------------------------------------------------------------------------------------
// event_music_disable()
//
// Stop any patterns that are playing, and prevent any further patterns from
// playing (ie set Event_music_enabled = FALSE).  We don't uninit event music,
// since it might be toggled back on in this level.
//
void event_music_disable () {
    ...
    if (audiostream_is_playing (Patterns[Current_pattern].handle)) {
        audiostream_stop (
            Patterns[Current_pattern].handle); // stop current and rewind
    }
...
// event_music_enable()
//
// Init the event music (ie load the patterns) if required.  Set up the first
// song to play, and set Event_music_enabled = TRUE to allow patterns to play.
//
void event_music_enable () {
    if (Event_music_enabled == TRUE) return;
    ...
    if (Event_music_level_inited == FALSE) {
        ...
        if (Game_mode & GM_IN_MISSION) { event_music_first_pattern (); }
    }
    else {
        // start a new pattern
        Event_music_begun = FALSE;
...
void event_music_start_default () {
    ...
    // switch now
    if (Current_pattern == -1) { Current_pattern = next_pattern; }
    // switch later
    else {
        Patterns[Current_pattern].next_pattern = next_pattern;
        Patterns[Current_pattern].force_pattern = TRUE;
...
void event_music_pause () {
    ...
    if (audiostream_is_playing (Patterns[Current_pattern].handle)) {
        audiostream_stop (
            Patterns[Current_pattern].handle,
            0); // stop current and don't rewind
    }
}
...
int hostile_ships_present () {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        // skip ourselves
        if (shipp == Player_ship) continue;

        // check if ship is enemy ship (we attack it)
        if (!(iff_x_attacks_y (Player_ship->team, shipp->team))) continue;
        ...
        if (!Ship_info[shipp->ship_info_index].is_flyable ()) { continue; }

        // check if ship is visible by player's team
        if (hud_target_invalid_awacs (&Objects[so->objnum]) == 1) { continue; }
...
int hostile_ships_to_arrive () {
    ...
    for (p_objp = GET_FIRST (&Ship_arrival_list);
        ...
        // check if ship is enemy ship (we attack it)
        if (!(iff_x_attacks_y (Player_ship->team, p_objp->team))) continue;
...
void event_music_get_info (char* outbuf) {
    if (Event_music_enabled == FALSE || Event_music_level_inited == FALSE ||
        ...
        sprintf (
            outbuf, XSTR ("soundtrack: %s [%s]", 214),
            Soundtracks[Current_soundtrack_num].name,
...
int event_music_next_soundtrack (int delta) {
    ...
    if (new_soundtrack >= Num_soundtracks) new_soundtrack = 0;
...
void event_sexp_change_soundtrack (char* name) {
    ...
    if (new_soundtrack == Current_soundtrack_num) { return; }
