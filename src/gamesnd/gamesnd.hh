 ...
 * Add interface (ie non-gameplay) sounds to the end of the interface list of
 * sounds. Add gameplay sounds to the correct portion of the gameplay sounds
 * section (ie Misc, Weapons, or Ship).
 *
...
enum class GameSounds {
    ...
    OUT_OF_WEAPON_ENERGY =
        27,               //!< tried to fire lasers when not enough energy left
    TARGET_FAIL = 28,     //!< target fail sound (i.e. press targeting key, but
                          //!< nothing happens)
    ...
    WARP_OUT =
        46, //!< warp hole opening up for departing (Same as warp in for now)
    ...
    SUBSPACE_LEFT_CHANNEL =
        72, //!< subspace ambient sound (left channel) (looped)
    SUBSPACE_RIGHT_CHANNEL =
        73, //!< subspace ambient sound (right channel) (looped)
...
enum InterfaceSounds {
    ...
    SCREEN_MODE_PRESSED =
        4, //!< press briefing, ship selection or weapons bar (top-left)
    ...
    SHIP_ICON_CHANGE =
        11, //!< ship animation starts (ie text and ship first appear)
    MAIN_HALL_AMBIENT =
        12,         //!< ambient sound for the Terran main hall (looping)
...
// These two id types are defined as sublcasses so that type safe implicit
// conversions are possible from the predefined sound enum values
...
public:
    /*implicit*/ gamesnd_id (GameSounds snd)
        : util::ID< gamesnd_id_tag, int, -1 > (static_cast< int > (snd)) {}
...
public:
    /*implicit*/ interface_snd_id (InterfaceSounds snd)
        : util::ID< interface_snd_tag, int, -1 > (static_cast< int > (snd)) {}
    interface_snd_id () {}
    explicit interface_snd_id (int val) : ID (val) {}
};

void gamesnd_parse_soundstbl (); // Loads in general game sounds from
                                 // sounds.tbl
...
// This should handle NO_SOUND just fine since it doesn't directly access
// lowlevel code Does all parsing for a sound
...
void parse_iface_sound_list (
    ...
    const char* object_name, bool scp_list = false);
 ...
 *
 * @details Use this instead of snd_get_duration since a game sound may have
 * multiple sounds with different durations. This function also accounts for
 * pitch changes which also changes the length of a sound.
 *
...
/**
 * @brief Chooses a sound entry for the specified game sound according to the
 * rules specified by the game data
 *
 * @warning The returned pointer is temporary and may not be valid forever. Do
 * no store this pointer anywhere!
 *
...
#endif // FREESPACE2_GAMESND_GAMESND_HH
