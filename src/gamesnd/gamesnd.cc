             ...
             gs != Snds.end(); ++gs) {
                if (gs->preload) {
                        for (auto &entry : gs->sound_entries) {
                                if (entry.filename[0] != 0 && strncasecmp(entry.filename, NOX("none.wav"), 4) != 0) {
                                        game_busy(NOX(
...
void parse_gamesnd_old(game_snd *gs)
{
        ...
        if (is_3d) {
                ...
                if (!strcasecmp(entry.filename, "l_hit.wav") || !strcasecmp(entry.filename, "m_hit.wav")) {
...
game_snd_entry *gamesnd_choose_entry(game_snd *gs)
{
        ...
        case GameSoundCycleType::Random:
                if (gs->sound_entries.size() == 1) {
                        ...
                        index = util::UniformRange< size_t >(0, gs->sound_entries.size() - 1)
                                        .next();
                }
                break;
