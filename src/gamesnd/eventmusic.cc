...
void event_music_force_switch()
{
        ...
        audiostream_set_sample_cutoff(
                Patterns[new_pattern].handle,
                int(
                        Patterns[new_pattern].num_measures * Patterns[new_pattern].samples_per_measure));
...
void event_music_do_frame()
{
        ...
        if (timestamp_elapsed(Pattern_timer_id)) {
                ...
                if (Current_pattern != -1 && Patterns[Current_pattern].handle >= 0) {
                        ...
                        audiostream_set_sample_cutoff(
                                Patterns[Current_pattern].handle,
                                int(
                                        Patterns[Current_pattern].num_measures * Patterns[Current_pattern].samples_per_measure));
                }
        }
        ...
        if (Current_pattern != -1) {
                ...
                // Second case: looping back to start of same track since at the end
                        ...
                        !audiostream_is_playing(pat->handle) && !audiostream_is_paused(pat->handle)) {
                ...
                if (Event_Music_battle_started == 0) {
                        if (Current_pattern == SONG_NRML_1 || Current_pattern == SONG_NRML_2 || Current_pattern == SONG_NRML_3) {
                                ...
                                Current_pattern == SONG_BTTL_1 || Current_pattern == SONG_BTTL_2 || Current_pattern == SONG_BTTL_3) {
...
void event_music_level_init(int force_soundtrack)
{
        ...
        ASSERT(
                Current_soundtrack_num >= 0 && Current_soundtrack_num < Num_soundtracks);
...
void event_music_level_close()
{
        ...
        if (Current_soundtrack_num >= 0 && Current_soundtrack_num < MAX_SOUNDTRACKS) {
                ...
                ASSERT(
                        Current_soundtrack_num >= 0 && Current_soundtrack_num < MAX_SOUNDTRACKS);
                strack = &Soundtracks[Current_soundtrack_num];
        ...
        if (Current_pattern != -1) {
                ...
                if (Soundtracks[Current_soundtrack_num].flags & EMF_ALLIED_ARRIVAL_OVERLAY) {
                        ...
                        audiostream_set_sample_cutoff(
                                Patterns[SONG_AARV_1].handle,
                                int(
                                        Patterns[SONG_AARV_1].num_measures * Patterns[SONG_AARV_1].samples_per_measure));
                }
                // don't overlay
                else {
                        Patterns[Current_pattern].next_pattern = next_pattern;
...
void parse_soundtrack()
{
        ...
        for (i = 0; i < Soundtracks[strack_idx].num_patterns; i++) {
                // check for "none"
                if (!strlen(Soundtracks[strack_idx].pattern_fnames[i]) || !strncasecmp(Soundtracks[strack_idx].pattern_fnames[i], "none", 4))
                        continue;
...
void event_sexp_change_soundtrack(char *name)
{
        ...
        for (i = 0; i < Num_soundtracks; i++) {
                if (!strcasecmp(name, Soundtracks[i].name)) {
                        new_soundtrack = i;
                }
        }
...
int event_music_get_spooled_music_index(const char *name)
{
        ...
        for (int i = 0; i < Num_music_files; i++) {
                if (!strcasecmp(name, Spooled_music[i].name)) {
                        return i;
                }
        }
