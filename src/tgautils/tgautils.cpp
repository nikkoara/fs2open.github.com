...
#define TARGA_FOOTER_SIZE \
    26 // using sizeof(targa_footer) doesn't work unless we
...
typedef struct targa_footer {
    ...
    char sig_string[18]; // check to see if this is new TGA format, string
                         // should be value of Xfile_ID
} targa_footer;
...
//
int targa_compress (
    char* out, char* in, int outsize, int pixsize, int bytecount) {
    ...
    do {
        ...
        if (pixcount == 129) {
            *flagbyte = 127;
            ...
            ++flagbyte;
            flagbyte += targa_copy_data (
                flagbyte, copyloc, pixcount - 1, pixsize, outsize);
            pixcount = 1;
        ...
        if (targa_pixels_equal (inputpixel, matchpixel, outsize)) {
            ...
            if (++rlcount == rlthresh) {
                ...
                if (pixcount > (rlcount + 1)) {
                    ...
                    *flagbyte++ = (char)(pixcount - 2 - rlthresh);
                    ...
                    flagbyte += targa_copy_data (
                        flagbyte, copyloc, (pixcount - 1 - rlcount), pixsize,
                        outsize);
            ...
            if (rlcount >= rlthresh) {
                *flagbyte++ = (char)(0x80 | rlcount);
                flagbyte +=
                    targa_copy_data (flagbyte, copyloc, 1, pixsize, outsize);
                pixcount = 1;
                continue;
            }
            else {
    ...
    if (--pixcount >= 1) {
        *flagbyte = (char)(pixcount - 1);
        ...
        ++flagbyte;
        flagbyte +=
            targa_copy_data (flagbyte, copyloc, pixcount, pixsize, outsize);
    }
...
// NOTE : for FreeSpace2, this also swizzles data into the proper screen (NOT
// texture) format - just like
// the pcxutils do.
//
...
//
static void targa_read_pixel (
    int num_pixels, ubyte** dst, ubyte** src, int bytes_per_pixel,
    int dest_size) {
    int idx;
    ushort pixel;
    int pixel32;
...
int targa_read_header (
    ...
    if ((header.pixel_depth != 16) && (header.pixel_depth != 24) &&
        (header.pixel_depth != 32)) {
        ASSERTX (
            (header.pixel_depth != 16) && (header.pixel_depth != 24) &&
                (header.pixel_depth != 32),
            "Invalid colour depth (%d) in header of tga file %s\n",
...
int targa_uncompress (
    ...
    while (pixel_count < bitmap_width) {
        ...
        if (run_count & 0x80) {
            ...
            targa_read_pixel (
                1, &tmp, &src_pixels, bytes_per_pixel, dest_size);
            ...
            targa_read_pixel (
                run_count + 1, &pixdata, &src_pixels, bytes_per_pixel,
                dest_size);
        }
    }
...
int targa_read_bitmap (
    ...
    cfseek (
        targa_file, cfilelength (targa_file) - TARGA_FOOTER_SIZE, CF_SEEK_SET);
    ...
    if (!strcmp (footer.sig_string, Xfile_ID)) {
        // it's an extended file to lets be sure to skip the extra crap which
        // comes after the image data section, dev section comes first in the
        // file and we only need one offset
        if (footer.dev_offset || footer.ext_offset) {
            xfile_offset =
                cfilelength (targa_file) -
                ((footer.dev_offset) ? footer.dev_offset : footer.ext_offset);
        }
    }
    ...
    // we're only allowing 2 bytes per pixel (16 bit compressed), unless
    // Cmdline_jpgtga is used
    ASSERT (
    ...
    int bytes_remaining =
        cfilelength (targa_file) - cftell (targa_file) - xfile_offset;
    ...
    if ((header.image_type == 1) || (header.image_type == 2) ||
        ...
        for (int i = 0; i < header.height; i++) {
            ...
            if (ULORIGIN) { pixptr = image_data + i * rowsize; }
            else {
                pixptr = image_data + ((header.height - i - 1) * rowsize);
            }

            targa_read_pixel (
                header.width, &pixptr, &src_pixels, bytes_per_pixel,
                dest_size);
        }
    }
    else if (
        ...
        for (int i = 0; i < header.height; i++) {
            ...
            if (ULORIGIN) { pixdata = image_data + i * rowsize; }
            else {
                pixdata = image_data + ((header.height - i - 1) * rowsize);
            }

            src_pixels += targa_uncompress (
                pixdata, src_pixels, header.width, bytes_per_pixel, dest_size);
        }
    }
...
int targa_write_bitmap (
    char* real_filename, ubyte* data, ubyte* /*palette*/, int w, int h,
    int bpp) {
    ASSERT (bpp == 24);
    char filename[MAX_FILENAME_LEN];
    CFILE* f;
    ...
    compressed_data_len = targa_compress (
