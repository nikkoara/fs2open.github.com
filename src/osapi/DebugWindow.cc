...
uint32_t get_debug_display () {
    // If there are two or more monitors then we would like to display the
    // window on the second monitor but it shouldn't be shown on the same
    // monitor that the game is running on If there is only then then we'll
    // just display it on that one
...
DebugWindow::DebugWindow () {
    ...
    // Make the window a bit smaller so that it doesn't take up the whole
    // screen
    ...
    attrs.flags.set (
        os::ViewPortFlags::Resizeable); // Make this window resizeable
    ...
    if (debugView) {
        ...
        if (debug_sdl_window != nullptr) {
            os::events::addEventListener (
                SDL_KEYUP, os::events::DEFAULT_LISTENER_WEIGHT - 5,
                [this](const SDL_Event& e) {
                    return this->debug_key_handler (e);
                });
        }
    }
    if (debug_view->toSDLWindow () != nullptr &&
...
void DebugWindow::doFrame (float) {
    ...
    // If the current view offset is at the end of the list then we also need
    // to display the line that is currently being constructed
    ...
    // Backwards iteration of the vector comes from here:
    // http://stackoverflow.com/a/4206815
...
float DebugWindow::print_line (float bottom_y, const LineInfo& line) {
    ...
    // Make sure that the width doesn't go too low or else split_str will not
    // be able to fit enough characters in one line
    auto max_w = std::max (40, gr_screen.max_w - max_category_width - 40);

    split_str (line.text.c_str (), max_w, line_lengths, split_lines);

    auto text_height =
        split_lines.size () * font::get_current_font ()->getHeight ();

    float y_pos = bottom_y - text_height;

    float cat_x_pos = max_category_width - category_width + 10.f;
    // Give each category a unique color. We do this by hashing the string and
    // using that to construct the RGB values
    auto hash = std::hash< std::string > () (line.category);
    gr_set_color (
        (int)(hash & 0xFF), (int)((hash & 0xFF00) >> 8),
        (int)((hash & 0xFF0000) >> 16));
    ...
    for (size_t i = 0; i < split_lines.size (); ++i) {
        gr_string (
            max_category_width + 18.f, y_pos, split_lines[i], GR_RESIZE_NONE,
            line_lengths[i]);
...
bool DebugWindow::debug_key_handler (const SDL_Event& evt) {
    ...
    case SDLK_DOWN: diff = 1; break;
    case SDLK_UP: diff = -1; break;
    case SDLK_PAGEDOWN:
        diff = 60; // It's not actually a page but this should still work
        break;
    case SDLK_PAGEUP: diff = -60; break;
    case SDLK_END: log_view_offset = lines.size () - 1; return true;
    case SDLK_HOME: log_view_offset = 0; return true;
    default: return true;
    }
...
void DebugWindow::addDebugMessage (const char* category, const char* text) {
    ...
    if (category != current_category) {
        // There is a new category so we need to write the old string to our
        // log
...
void DebugWindow::split_current_and_add_to_log (const std::string& category) {
    ...
    while ((pos = current_line.find ('\n')) != std::string::npos) {
        ...
        // Since pos refers to a valid position in the string, the expression
        // pos + 1 is always valid since substr allows values up to str.size()
...
void DebugWindow::addToLog (LineInfo&& line) {
    ...
    if (log_view_offset == lines.size () - 1) {
        // We are currently showing the last line so we need to follow new
        // entries
        ++log_view_offset;
    }
