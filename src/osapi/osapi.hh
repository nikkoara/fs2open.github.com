 ...
 * Can be deleted which will properly free the resources of the underlying
 * OpenGL context
 * @ingroup os_graphics_api
 */
class OpenGLContext {
 ...
 *
 * A viewport is something that supports rendering operations. Typically this
 * is a window but here it's more abstract.
 *
 * @ingroup os_graphics_api
 */
class Viewport {
public:
     ...
     * renderable area might be bigger if the window is created with support
     * for that.
     *
 ...
 *
 * This is used for providing platform specific functionality for various
 * graphics operations.
 *
 * @ingroup os_graphics_api
 */
...
public:
     ...
     * and height values may be used for creating the main window.
     *
     ...
     * certain number of viewports are created. E.g. FRED may not want to
     * create more than one viewport.
     *
 ...
 *
 * The osapi system will automatically free these viewports when it is shut
 * down.
 *
 * @param viewport The viewport to add. The osapi system assumes ownership over
 * the pointer
 ...
 * @return The SDL handle or @c nullptr if there is no main window or if the
 * main window can't be represented as an SDL window
 *
 * @ingroup os_graphics_api
 */
 ...
 * The default event handler use this weight so other event handlers should
 * have a weight that is less than this.
 */
 ...
 * @param weigth The weight of this handler, used for determining in which
 * order handlers are called.
 ...
 * events that belong to a specific window.
 *
 ...
 * The OS-API exposed a generic event handler interface to allow user code to
 * consume SDL events. The handler will be passed the SDL_Event structure and
 * should return @c true if it handled the event or @c false if not. If it
 * handled the event then the processing of this event will be stopped and it
 * will not be delivered to subsequent listeners.
 */

/**
 * @page graphics_api_page OS Graphics API
 * Since multiple different applications need to use the FSO rendering engine
 * it's necessary to provide an abstraction of how an OpenGL context is
 * created. Furthermore, since FSO may want to use multiple viewports it's also
 * necessary to provide an abstraction for that. This is done via the @ref
 * GraphicsOperations class which must be implemented by users of the FSO
 * rendering engine. This class allows to create viewports and OpenGL contexts.
 * First, a viewport will be created which can be a standalone window or a part
 * of a GUI. Then an OpenGL context will be created based on that viewport
 * which will be used for further rendering.
 *
 * @see @ref os_graphics_api
 */

#endif // FREESPACE2_OSAPI_OSAPI_HH
