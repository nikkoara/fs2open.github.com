...
#ifndef FREESPACE2_OSAPI_DEBUGWINDOW_HH
#define FREESPACE2_OSAPI_DEBUGWINDOW_HH
 ...
 *
 * A debug window is a separate os::Viewport which displays additional debug
 * information at runtime.
 */
class DebugWindow {
    /**
     * A single line in the debug log can have multiple properties which are
    ...
    os::Viewport* debug_view =
        nullptr; //!< The viewport in which the debug output is shown
    SDL_Window* debug_sdl_window =
        nullptr; //!< The SDL window handle of the debug view

    std::vector< LineInfo > lines; //!< Finished lines

    std::string current_category; //!< The category of the current line
    std::string current_line; //!< Line that is currently being constructed,
                              //!< may be empty

    int max_category_width =
        0; //!< The maximum width of any category string encountered thus far

    size_t log_view_offset = 0; //!< The offset into the @c lines array of the
    ...
    /**
     * @brief Splits current_line into individual strings and adds them to the
     * log
     *
     * Since debug output can contain multiple newlines in one string we need
     * to split them before storing them in the vector
     *
...
#endif // FREESPACE2_OSAPI_DEBUGWINDOW_HH
