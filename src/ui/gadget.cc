...
int UI_GADGET::get_hotspot () {
    if (!linked_to_hotspot) return -1;

    return hotspot_num;
}

void UI_GADGET::reset () { m_flags = 0; }

int UI_GADGET::is_hidden () { return hidden; }
...
// --------------------------------------------------------------------
// Extract MAX_BMAPS_PER_CONTROL bitmaps for the different states of the
// control.
//
// The bitmap ids are stored in the bmap_ids[] array.  If you don't want to
// store from the zero index, fill in the offset parameter.  offset is a
...
int UI_GADGET::set_bmaps (
    const char* ani_fname, int nframes, int start_frame) {
    int first_frame, i;
    ...
    for (idx = 0; idx < MAX_BMAPS_PER_GADGET; idx++) { bmap_ids[idx] = -1; }
    ...
    for (idx = start_frame; idx < nframes; idx++) {
        ...
        if (bmap_ids[idx] != -1) { its_all_good = 1; }
    }
    ...
    if ((first_frame >= 0) && (m_num_frames <= MAX_BMAPS_PER_GADGET)) {
        // seems pretty stupid that we didn't just use a variable for the first
        // frame and access all other frames offset from it instead of
        // accessing this bmap_ids[] array, but probably too much trouble to go
        // through and change this anymore.  How sad..
        for (i = 0; i < m_num_frames; i++) { bmap_ids[i] = first_frame + i; }
    }
...
// Handle drawing of all children of the gadget.  Since this the base of all
// other gadgets, it doesn't have any look to it (kind of like a soul.  Can you
// see someone's soul?) and thus doesn't actually render itself.
//
void UI_GADGET::draw () {
    UI_GADGET* cur;
...
// Use this if you need to change the size and position of a gadget after you
// have created it.
//
...
void UI_GADGET::hide () { hidden = 1; }

void UI_GADGET::unhide () { hidden = 0; }

// Capture mouse with this gadget, which basically means only this gadget will
// get process() called on it until the mouse button 1 is released.
//
...
//
void UI_GADGET::base_create (
    UI_WINDOW* wnd, int _kind, int _x, int _y, int _w, int _h) {
    int i;
    ...
    // this actually links the gadget into the UI window's top level family (as
    // the youngest gadget)
    set_parent (NULL);
    ...
    uses_bmaps = 0;
    m_num_frames = 0;
    for (i = 0; i < MAX_BMAPS_PER_GADGET; i++) { bmap_ids[i] = -1; }
}

void UI_GADGET::set_hotkey (int key) { hotkey = key; }

// Far as I can tell, the callback function is handled differently for each
// gadget type, if handled by a given gadget type at all.
//
...
UI_GADGET* UI_GADGET::get_next () {
    ...
    tmp = next;
    while ((tmp != this) && tmp->disabled_flag) tmp = tmp->next;
...
UI_GADGET* UI_GADGET::get_prev () {
    ...
    tmp = prev;
    while ((tmp != this) && tmp->disabled_flag) tmp = tmp->prev;
...
//
void UI_GADGET::set_focus () { my_wnd->selected_gadget = this; }

// Make no gadget have focus in the UI window.
//
void UI_GADGET::clear_focus () { my_wnd->selected_gadget = NULL; }

// Return true or false if this gadget currently has the focus
int UI_GADGET::has_focus () { return my_wnd->selected_gadget == this ? 1 : 0; }

// get mouse pointer position relative to gadget's origin (UL corner)
//
void UI_GADGET::get_mouse_pos (int* xx, int* yy) {
    if (xx) *xx = ui_mouse.x - x;
    if (yy) *yy = ui_mouse.y - y;
}

// Call process() for all children of gadget.  As a base gadget for all other
// gadget types, it doesn't actually do anything for itself.
//
...
int UI_GADGET::is_mouse_on () {
    ...
    if (linked_to_hotspot) {
        ...
        // NOTE : this happens when using smaller mask bitmaps than the screen
        // resolution (during development)
        if ((ui_mouse.x < 0) || (ui_mouse.x >= mask_w) || (ui_mouse.y < 0) ||
            (ui_mouse.y >= mask_h)) {
            return 0;
        }
        ...
        if (pixel_val == hotspot_num) { return 1; }
        else {
            return 0;
        }
        ...
        if ((ui_mouse.x >= x) && (ui_mouse.x < x + w) && (ui_mouse.y >= y) &&
            (ui_mouse.y < y + h)) {
            return 1;
        }
        else {
...
int UI_GADGET::is_mouse_on_children () {
    ...
    if (tmp) {
        do {
            if (tmp->is_mouse_on ()) return 1;
            if (tmp->is_mouse_on_children ()) return 1;
...
void UI_GADGET::disable () { disabled_flag = 1; }

// Enables (or possibly disables) the gadget.  n is an optional argument.  If
// not supplied, enables the garget.  If supplied, enables garget if n is true,
// disables it if n is false.
//
void UI_GADGET::enable (int n) { disabled_flag = n ? 0 : 1; }

// Check if gadget is disabled
//
int UI_GADGET::disabled () { return disabled_flag; }

// Check if gadget is enabled
//
int UI_GADGET::enabled () { return !disabled_flag; }
...
// Take gadget out of family.  Children of this gadget stay with gadget,
// though.
//
// A family is basically the whole parent/sibling/children hierarchy for
// gadgets.  Any gadget that is linked to another gadget by one of these
// pointers is said to be in the same family as that gadget.  This function,
// therefore, caused all references to a gadget by it's family's gadgets'
// sibling or children pointers to be broken, and all references to any of them
// by this gadget's parent or sibling pointers to also be broken.  This
// isolates the gadget and it's children into a new family.
//
void UI_GADGET::remove_from_family () {
    if (parent) {
        ...
        if (my_wnd->first_gadget == this) {
            if (next == this) // an only child?
                my_wnd->first_gadget =
                    NULL; // if so, parent now has no children
            else
...
void UI_GADGET::set_parent (UI_GADGET* daddy) {
    ...
    if (!daddy) {
        if (!my_wnd->first_gadget) { my_wnd->first_gadget = this; }
        else {
    ...
    if (!daddy->children) { daddy->children = this; }
    else {
        UI_GADGET *eldest_sibling, *youngest_sibling;
