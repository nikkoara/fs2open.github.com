...
void UI_CHECKBOX::create (
    UI_WINDOW* wnd, const char* _text, int _x, int _y, int _state) {
    int _w, _h;
...
void UI_CHECKBOX::process (int focus) {
    ...
    if (focus && ((my_wnd->keypress == KEY_SPACEBAR) ||
                  (my_wnd->keypress == KEY_ENTER)))
        position = 2;

    if (focus)
        if ((oldposition == 2) &&
            (keyd_pressed[KEY_SPACEBAR] || keyd_pressed[KEY_ENTER]))
            position = 2;

    pressed_down = 0;

    if (position == 0) {
        if ((oldposition == 1) && OnMe) { pressed_down = 1; }
        if ((oldposition == 2) && focus) { pressed_down = 1; }
    }

    if (pressed_down && user_function) { user_function (); }

    if (pressed_down) flag = !flag;
}

int UI_CHECKBOX::changed () { return pressed_down; }

int UI_CHECKBOX::checked () { return flag; }

void UI_CHECKBOX::set_state (int _state) { flag = _state; }
