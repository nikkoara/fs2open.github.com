...
void UI_DOT_SLIDER_NEW::create (
    ...
    int slider_mask, const char* bm_left, int left_mask, int left_x,
    int left_y, const char* bm_right, int right_mask, int right_x, int right_y,
    int _dot_width) {
    // no end buttons yet
    has_end_buttons = 0;

    // if any of the left/right arrow information is specified, make sure its
    // _all_ specified
...
void UI_DOT_SLIDER_NEW::process (int focus) {
    if (disabled_flag) {
        if (!hidden && !my_wnd->use_hack_to_get_around_stupid_problem_flag) {
            if (button.is_mouse_on () && B1_JUST_PRESSED) {
                ...
                has_end_buttons &&
                (up_button.is_mouse_on () || down_button.is_mouse_on ())) {
                gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
            }
    ...
    if (button.button_down () || button.pressed () || mouse_captured ()) {
        capture_mouse (); // while we are changing level, ignore all other
                          // buttons
...
//
// OLD DOT SLIDER - TO BE PHASED OUT. IF YOU NEED TO USE A UI_DOT_SLIDER, use a
// UI_DOT_SLIDER_NEW -------------------
//

/// DOT_SLIDER class down here
void UI_DOT_SLIDER::create (
    UI_WINDOW* wnd, int _x, int _y, char* bm, int id, int end_buttons,
    int _num_pos) {
...
void UI_DOT_SLIDER::process (int focus) {
    ...
    if (button.button_down () || button.pressed () || mouse_captured ()) {
        ...
        if (has_end_buttons) { pos = (ui_mouse.x - x + 17) / 19; }
        else {
            pos = (ui_mouse.x - x) / 19;
        }
