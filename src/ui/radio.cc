...
void UI_RADIO::draw () {
    ...
    if (uses_bmaps) {
        ...
        if (flag) {
            gr_circle (
                Middle (w) + offset, Middle (h) + offset, 8, GR_RESIZE_MENU);
        }
        else {
            gr_circle (
                Middle (w) + offset, Middle (h) + offset, 8, GR_RESIZE_MENU);
            ...
            gr_circle (
                Middle (w) + offset, Middle (h) + offset, 4, GR_RESIZE_MENU);
        }
...
int UI_RADIO::changed () { return pressed_down; }

int UI_RADIO::checked () { return flag; }
