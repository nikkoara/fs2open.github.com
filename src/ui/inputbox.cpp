...
int keypad_to_ascii (int c) {
    ...
    case KEY_PAD8:
    case KEY_PAD9:
    case KEY_PADPERIOD: return key_to_ascii (c); break;
...
void UI_INPUTBOX::create (
    UI_WINDOW* wnd, int _x, int _y, int _w, int _text_len, const char* _text,
    ...
    // check to see if the user passed in a text color otherwise use the
    // default green color
    if (clr) { text_color = clr; }
    else {
        text_color = &CBRIGHT;
    }
    ...
    if (_text_len > 0) { strncpy (text, _text, _text_len); }
    text[_text_len] = 0;
    ...
    if (!disabled_flag) {
        if (my_wnd->selected_gadget == this) {
            if (cursor_first_frame == -1) {
                ...
                // new method is simpler and should give the same results
                // unless the target device is super slow i.e. can't render an
                // interface frame in less than the anim frame delays (retail
                // 15 fps == 66.67 msec)
...
int UI_INPUTBOX::validate_input (int chr) {
    ...
    if ((flags & UI_INPUTBOX_FLAG_NO_LETTERS) && isalpha (chr)) { return 0; }

    // if we're disallowing numbers altogether
    if ((flags & UI_INPUTBOX_FLAG_NO_NUMERALS) && isdigit (chr)) { return 0; }

    // otherwise allow numbers and alpha chars by
    if (isdigit (chr) || isalpha (chr)) { return chr; }

    // if we have specified no valid or invalid chars, accept everything
    if (!valid_chars && !invalid_chars) { return chr; }

    // otherwise compare against the valid chars list
    if ((valid_chars) && strchr (valid_chars, chr)) { return chr; }

    // otherwise compare against the invalid chars list0
    if ((invalid_chars) && !strchr (invalid_chars, chr)) { return chr; }
...
void UI_INPUTBOX::process (int focus) {
    ...
    if (B1_PRESSED && is_mouse_on ()) { set_focus (); }
    ...
    if (focus) {
        ...
        case KEY_BACKSP:
            ...
            text[position] = 0;
            if (flags & UI_INPUTBOX_FLAG_PASSWD) { passwd_text[position] = 0; }
        ...
        case KEY_ESC:
            ...
            if (flags & UI_INPUTBOX_FLAG_ESC_FOC) { clear_focus (); }

            break;

        default:
            if (!locked) {
                // MWA -- determine if alt or ctrl held down on this key and
                // don't process if it is.  We need to be able to pass these
                // keys back to the top level.  (And anyway -- ctrl-a shouldn't
                ...
                if (key_check == -1) { key_check = key_to_ascii (key); }

                ascii = validate_input (key_check);
                if ((ascii > 0) && (ascii < 255)) {
                    if (flags & UI_INPUTBOX_FLAG_LETTER_FIRST) {
                        if ((position == 0) && !is_letter ((char)ascii)) break;
                    }
...
int UI_INPUTBOX::changed () { return changed_flag; }

int UI_INPUTBOX::pressed () { return pressed_down; }
