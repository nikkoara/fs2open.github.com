...
#define UI_INPUTBOX_FLAG_KEYTHRU \
    (1 << 1) // pass all keypresses through to parent
#define UI_INPUTBOX_FLAG_ESC_CLR \
    (1 << 2) // allow escape key to clear input box
#define UI_INPUTBOX_FLAG_ESC_FOC \
    (1 << 3) // escape loses focus for the input box
...
protected:
    ...
    // Data for supporting bitmaps associated with different states of the
    // control
    int uses_bmaps;
    int m_num_frames;
...
public:
    ...
    void
    base_create (UI_WINDOW* wnd, int _kind, int _x, int _y, int _w, int _h);
    virtual void draw ();
    void set_focus ();
    void clear_focus ();
    int has_focus ();
    ...
    // this keeps the loading code from trying to load bitmaps which don't
    // exist and taking an unnecessary disk hit.
    int set_bmaps (
...
class UI_BUTTON : public UI_GADGET {
    ...
    char* text;
    int position;    // indicates position of button (0 - up, 1 - down by mouse
                     // click 2 - down by keypress
    ...
    void (*m_just_highlighted_function) (
        void); // call-back that gets called when button gets highlighted
    void (*m_disabled_function) (
        void); // callback that gets called when disabled button gets pressed
               // (sound, popup, etc)
    ...
    io::mouse::Cursor*
        custom_cursor; // bmap handle of special cursor used on mouseovers
...
public:
    ...
    void create (
        UI_WINDOW* wnd, const char* _text, int _x, int _y, int _w, int _h,
    ...
    void set_custom_cursor (io::mouse::Cursor* cursor) {
...
class UI_INPUTBOX : public UI_GADGET {
    ...
    int changed_flag;
    int flags;
    int pixel_limit; // base max characters on how wide the string is (-1 to
                     // ignore) in pixels
    int locked;
    color* text_color;
    char* valid_chars;
...
public:
    void create (
        UI_WINDOW* wnd, int _x, int _y, int _w, int _textlen,
        const char* _text, int _flags = 0, int pixel_lim = -1,
        color* clr = NULL);
...
public:
    ...
    int checked ();
    void
    create (UI_WINDOW* wnd, const char* _text, int _x, int _y, int _state);
...
class UI_RADIO : public UI_GADGET {
    ...
    // Used to index into bmap_ids[] array to locate right bitmap for radio
    // button
...
public:
    void create (
        UI_WINDOW* wnd, int _x, int _y, int _h, int _start, int _stop,
...
private:
    ...
    int slider_w, slider_h,
        slider_half_h; // this is the width and height and half height of the
                       // bitmap used for the slider
    void draw () override;
    void process (int focus = 0) override;
...
public:
    // create the slider
    void create (
        UI_WINDOW* wnd, int _x, int _y, int _w, int _h, int _numberItems,
    ...
    void set_numberItems (
        int _numberItems,
        int _reset = 1); // change range. maybe reset back to position 0
    void set_currentItem (
        int _currentItem); // force slider to new position manually
    void force_currentItem (
        int _currentItem); // force slider to new position manually, _not_
...
public:
    ...
    void create (
        UI_WINDOW* wnd, int _x, int _y, char* bm, int id, int end_buttons = 1,
        int _num_pos = 10);
    void draw () override;
    void process (int focus = 0) override;
    void destroy () override;
...
public:
    ...
    void create (
        UI_WINDOW* wnd, int _x, int _y, int _num_pos, const char* bm_slider,
...
public:
    void create (
        UI_WINDOW* wnd, int _x, int _y, int _w, int _h, int _numitem,
        char** _list, char* _check_list = NULL, int _max_items = -1);
    int selected (); // selected: Returns >= 0 if an item was selected
    int current ();  // current:  Returns which item listbox bar is currently
                     // on. This could be -1, if none selected!
    int toggled (); // toggled:  Returns which item was toggled with spacebr or
                    // mouse click of check_list is not NULL
    ...
    void clear_all_items (); // deletes all the items in the list (makes them
                             // empty strings)
    ...
    int set_bmaps (
        char* lbox_fname, char* b_up_fname, char* b_down_fname,
        char* sb_fname);
    void link_hotspot (int up_button_num, int down_button_num);
};
...
protected:
    ...
    UI_XSTR* xstrs[MAX_UI_XSTRS]; // strings for drawing in code instead of in
                                  // artwork
...
public:
    ...
    void add_XSTR (
        ...
        int _color_type, int _font_id = -1);
