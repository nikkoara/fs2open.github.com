...
// 0 ==> success
//
int UI_LISTBOX::set_bmaps (
    char* lbox_fname, char* b_up_fname, char* b_down_fname, char* sb_fname) {
    if (has_scrollbar) {
...
void UI_LISTBOX::create (
    UI_WINDOW* wnd, int _x, int _y, int _w, int _h, int _numitems,
    char** _list, char* _check_list, int _max_items) {
    int tw, th, nrows;
    int real_h;
    ...
    if (_numitems > nrows) {
        scrollbar.create (
            wnd, _x + _w + 3, _y, real_h, 0, _numitems - nrows, 0, nrows);
...
void UI_LISTBOX::set_drawframe (int mode) { draw_frame = mode; }
...
void UI_LISTBOX::draw () {
    ...
    if (uses_bmaps) {
        ...
        if (draw_frame) {
            if (has_scrollbar) {
                ui_draw_sunken_border (
                    x - 2, y - 2, x + w + scrollbar.w + 4, y + h + 1);
            }
            else {
    ...
    for (i = first_item; i < stop; i++) {
        ...
        if (i != current_item) {
            /*
                        if ((current_item == -1) && (my_wnd->selected_gadget ==
               this ) && (i == first_item)  )   { if ( !uses_bmaps ) {
                                gr_set_color_fast( &CBLACK );
                                gr_rect( x1, y1, w1+2, h1,
               GR_RESIZE_MENU_NO_OFFSET );
                            }
        ...
        if (check_list) {
            if (check_list[i]) { gr_string (x1 + 2, y1, "X", GR_RESIZE_MENU); }
...
void UI_LISTBOX::process (int focus) {
    ...
    if (has_scrollbar) {
        if (scrollbar.moved) {
            ...
            if (current_item < first_item) current_item = first_item;
    ...
    if (key_buffer_count &&
        (timer_get_milliseconds () > last_typed + KEY_BUFFER_TIMEOUT))
        key_buffer_count = 0;
    ...
    if (focus) {
        if (my_wnd->keypress) {
            ...
            case KEY_ENTER: selected_item = current_item; break;

            case KEY_SPACEBAR: toggled_item = current_item; break;
        ...
        if (kf == 1) {
            ...
            if (current_item >= num_items) current_item = num_items - 1;
            ...
            if (num_items <= num_items_displayed) { first_item = 0; }
            else {
                if (has_scrollbar) {
                    scrollbar.position = first_item;

                    scrollbar.bar_position =
                    ...
                    if (scrollbar.bar_position >
                        scrollbar.h - scrollbar.bar_size) {
                        scrollbar.bar_position =
                            scrollbar.h - scrollbar.bar_size;
                    }
                }
    ...
    if (focus) {
        if (B1_PRESSED && dragging) {
            ...
            if ((mitem < 0) &&
                (timer_get_milliseconds () > last_scrolled + 1000 / 18)) {
                current_item--;
            ...
            if (num_items <= num_items_displayed) { first_item = 0; }
                ...
                if (scrollbar.bar_position < 0) { scrollbar.bar_position = 0; }
        ...
        if (check_list) {
            if (B1_JUST_RELEASED) toggled_item = current_item;
        }

        if (B1_DOUBLE_CLICKED) { selected_item = current_item; }
    }
}

int UI_LISTBOX::toggled () {
    if (check_list) { return toggled_item; }
    else {
        return -1;
    }
}
...
int UI_LISTBOX::selected () {
    if (check_list) { return -1; }
    else {
        return selected_item;
    }
}

int UI_LISTBOX::current () { return current_item; }

void UI_LISTBOX::set_current (int _index) { current_item = _index; }
...
char* UI_LISTBOX::get_string (int _index) { return list[_index]; }

void UI_LISTBOX::clear_all_items () {
    int idx;

    for (idx = 0; idx < num_items; idx++) list[idx][0] = 0;
...
void UI_LISTBOX::RemoveFirstItem () {
    for (int i = 0; i < num_items - 1; i++) list[i] = list[i + 1];
    num_items--;
}

int UI_LISTBOX::MaxSize () { return max_items; }

int UI_LISTBOX::CurSize () { return num_items; }

int UI_LISTBOX::add_string (char* str) {
    if (max_items <
        0) // only if we created an "empty" listbox can we add items
        return 0;

    else {
        if ((num_items == max_items - 1) ||
            (strlen (str) > DEFAULT_LISTBOX_ITEM_LENGTH))
