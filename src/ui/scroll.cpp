...
void UI_SCROLLBAR::process (int focus) {
    ...
    if ((ui_mouse.y >= bar_position + y) &&
        (ui_mouse.y < bar_position + y + bar_size) && OnMe)
        OnSlider = 1;
    ...
    if (B1_PRESSED && OnMe && !OnSlider &&
        (timer_get_milliseconds () > last_scrolled + 1000 / (18 * 4))) {
    ...
    if (B1_PRESSED && dragging) {
        ...
        if (bar_position > h - bar_size) { bar_position = h - bar_size; }
...
int UI_SCROLLBAR::getpos () { return position; }

int UI_SCROLLBAR::changed () { return moved; }
