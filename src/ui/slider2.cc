...
// when the scrollbar has been released
void UI_SLIDER2::create (
    ...
    bm_get_info (
        bmap_ids[S2_NORMAL], &buttonWidth, &buttonHeight, NULL, NULL, NULL);
    ...
    if (numberItems <= 0) { disabled_flag = 1; }
...
void UI_SLIDER2::draw () {
    ...
    if (uses_bmaps && !disabled_flag) {
        ...
        case S2M_ON_ME:
            ...
            break;
        case S2M_MOVING: gr_set_bitmap (bmap_ids[S2_PRESSED]); break;
        case S2M_DEFAULT:
        default:
...
void UI_SLIDER2::process (int /*focus*/) {
    ...
    if (OnMe) {
        ...
        if ((ui_mouse.y >= (y + currentPosition)) &&
            (ui_mouse.y <= (y + currentPosition + slider_h))) {
            slider_mode = S2M_ON_ME;
            if (B1_PRESSED) { mouse_locked = 1; }
        }
    }
    else
        slider_mode = S2M_DEFAULT;
    ...
    if (!OnMe && !mouse_locked) return;

    // could we possibly be moving up?
    if ((OnMe && B1_PRESSED &&
         ui_mouse.y < (currentPosition + y + slider_half_h - 1)) ||
        (mouse_locked &&
         (ui_mouse.y < (currentPosition + y + slider_half_h - 1)))) {
        // make sure we wait at least 50 ms between events unless mouse locked
        if ((timer_get_milliseconds () > last_scrolled + 50) ||
            B1_JUST_PRESSED || mouse_locked) {
    ...
    // if we are centerd on the bitmap and still in mouse lock mode, we need to
    // make sure the MOVING bitmap is still shown or if mouse is on us and we
    // are pressing the mouse button
    if (mouse_locked || (OnMe && B1_PRESSED)) { slider_mode = S2M_MOVING; }
}

// return number of itmes
int UI_SLIDER2::get_numberItems () { return numberItems; }

// return current position
int UI_SLIDER2::get_currentPosition () { return currentPosition; }

// return current item
int UI_SLIDER2::get_currentItem () { return currentItem; }
...
void UI_SLIDER2::set_currentItem (int _currentItem) {
    if (_currentItem > numberItems) goto cpSafety;

    if (_currentItem == currentItem) goto cpSafety;

    if (_currentItem < 0) goto cpSafety;
...
cpSafety: // helps fix math problem on x86_64
    if (currentPosition > numberItems) currentPosition = numberItems;
