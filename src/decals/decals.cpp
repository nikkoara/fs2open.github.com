...
public:
    ...
    ~DecalDefinition () {
        if (_diffuseBitmap >= 0) { bm_release (_diffuseBitmap); }
        if (_glowBitmap >= 0) { bm_release (_glowBitmap); }
        if (_normalBitmap >= 0) { bm_release (_normalBitmap); }
    }
    ...
    void parse () {
        if (optional_string ("+Diffuse:")) {
            ...
            if (!bm_validate_filename (_diffuseFilename, true, true)) {
                error_display (
                    0, "Animation '%s' is not valid!",
                    _diffuseFilename.c_str ());
                _diffuseFilename = "";
            }

            if (optional_string ("+Loop:")) { stuff_boolean (&_loopDiffuse); }
        }
        if (optional_string ("+Glow:")) {
            ...
            if (!bm_validate_filename (_glowFilename, true, true)) {
                error_display (
                    0, "Animation '%s' is not valid!", _glowFilename.c_str ());
                _glowFilename = "";
            }

            if (optional_string ("+Loop:")) { stuff_boolean (&_loopGlow); }
        }
        if (optional_string ("+Normal:")) {
            ...
            if (optional_string ("+Loop:")) { stuff_boolean (&_loopNormal); }
        }
    }
    ...
    void pageIn () {
        if (_diffuseBitmap >= 0) { bm_page_in_texture (_diffuseBitmap); }
        if (_glowBitmap >= 0) { bm_page_in_texture (_glowBitmap); }
        if (_normalBitmap >= 0) { bm_page_in_texture (_normalBitmap); }
    }

    bool bitmapsLoaded () {
        // Since both bitmap types are optional we need to check if either is
        // loaded to determine if any bitmap is loaded
        return _diffuseBitmap >= 0 || _glowBitmap >= 0 || _normalBitmap >= 0;
    }

    const std::string& getName () const { return _name; }
    int getDiffuseBitmap () const { return _diffuseBitmap; }
    int getGlowBitmap () const { return _glowBitmap; }
    int getNormalBitmap () const { return _normalBitmap; }
    bool isDiffuseLooping () const { return _loopDiffuse; }
    bool isGlowLooping () const { return _loopGlow; }
    bool isNormalLooping () const { return _loopNormal; }
};

std::vector< DecalDefinition > decalDefinitions;

// Variable to indicate if the system is able to work correctly on the current
// system
...
struct Decal {
    ...
    float creation_time =
        -1.0f; //!< The mission time at which this decal was created
    float lifetime = -1.0f; //!< The time this decal is active. When negative
                            //!< it never expires
    ...
    Decal () { vm_vec_make (&scale, 1.f, 1.f, 1.f); }
...
bool required_string_if_new (const char* token, bool new_entry) {
    if (!new_entry) { return optional_string (token) == 1; }
...
void parseDecalReference (creation_info& dest_info, bool is_new_entry) {
    ...
    if (required_string_if_new ("+Radius:", is_new_entry)) {
        ...
        if (dest_info.radius <= 0.0f) {
            error_display (
                0, "Invalid radius of %f! Must be a positive number.",
                dest_info.radius);
    ...
    if (required_string_if_new ("+Lifetime:", is_new_entry)) {
        if (optional_string ("Eternal")) {
            ...
            // Require at least a small lifetime so that the calculations don't
            // have to deal with div-by-zero
            dest_info.lifetime = util::parseUniformRange (0.0001f);
        }
    }
}
void loadBitmaps (const creation_info& info) {
    if (!decal_system_active) { return; }
    // Silently ignore invalid definition handle since weapons use the default
...
matrix4 getDecalTransform (Decal& decal) {
    ...
    model_instance_find_world_point (
        &worldPos, &decal.position, ship->model_instance_num, decal.submodel,
        &objp->orient, &objp->pos);

    vec3d worldDir;
    model_instance_find_world_dir (
        &worldDir, &decal.orientation.vec.fvec, ship->model_instance_num,
        decal.submodel, &objp->orient);

    vec3d worldUp;
    model_instance_find_world_dir (
        &worldUp, &decal.orientation.vec.fvec, ship->model_instance_num,
        decal.submodel, &objp->orient);
...
void renderAll () {
    ...
    for (auto iter = active_decals.begin (); iter != active_decals.end ();
         ++iter) {
        if (!iter->isValid ()) {
            // if we're sitting on the very last element, popping-back will
    ...
    for (auto& decal : active_decals) {
        ...
        if (decalDef.getDiffuseBitmap () >= 0) {
            diffuse_bm = decalDef.getDiffuseBitmap () +
                         bm_get_anim_frame (
                             decalDef.getDiffuseBitmap (), decal_time, 0.0f,
                             decalDef.isDiffuseLooping ());
        }

        if (decalDef.getGlowBitmap () >= 0) {
            glow_bm = decalDef.getGlowBitmap () +
                      bm_get_anim_frame (
                          decalDef.getGlowBitmap (), decal_time, 0.0f,
                          decalDef.isGlowLooping ());
        }

        if (decalDef.getNormalBitmap () >= 0) {
            normal_bm = decalDef.getNormalBitmap () +
                        bm_get_anim_frame (
                            decalDef.getNormalBitmap (), decal_time, 0.0f,
                            decalDef.isNormalLooping ());
        }

        draw_list.add_decal (
            diffuse_bm, glow_bm, normal_bm, decal_time,
            getDecalTransform (decal), alpha);
    }

    draw_list.render ();
}
void addDecal (
    creation_info& info, object* host, int submodel, const vec3d& local_pos,
    ...
    if (!def.bitmapsLoaded ()) {
        // If this decal was never used before then the bitmaps are not loaded
