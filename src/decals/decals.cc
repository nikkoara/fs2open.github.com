...
struct Decal {
        ...
        bool isValid()
        {
                ...
                if (objp->type == OBJ_SHIP) {
                        ...
                        ASSERTX(
                                submodel >= 0 && submodel < model_get(object_get_model(objp))->n_models,
...
void loadBitmaps(const creation_info &info)
{
        ...
        ASSERTX(
                info.definition_handle >= 0 && info.definition_handle < (int)decalDefinitions.size(),
...
void renderAll()
{
        ...
        for (auto &decal : active_decals) {
                ...
                ASSERTX(
                        decal.definition_handle >= 0 && decal.definition_handle < (int)decalDefinitions.size(),
                        "Invalid decal handle detected!");
                ...
                if (decalDef.getDiffuseBitmap() >= 0) {
                        diffuse_bm = decalDef.getDiffuseBitmap() + bm_get_anim_frame(decalDef.getDiffuseBitmap(), decal_time, 0.0f, decalDef.isDiffuseLooping());
                }

                if (decalDef.getGlowBitmap() >= 0) {
                        glow_bm = decalDef.getGlowBitmap() + bm_get_anim_frame(decalDef.getGlowBitmap(), decal_time, 0.0f, decalDef.isGlowLooping());
                }

                if (decalDef.getNormalBitmap() >= 0) {
                        normal_bm = decalDef.getNormalBitmap() + bm_get_anim_frame(decalDef.getNormalBitmap(), decal_time, 0.0f, decalDef.isNormalLooping());
                }
