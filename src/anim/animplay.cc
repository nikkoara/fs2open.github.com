...
int Anim_ignore_frametime = 0; // flag used to ignore frametime... useful when
                               // need to avoid saturated frametimes

/**
 * @brief Initialise animation
 * @details Queue all the ::anim_render_instance[] elements onto the
 * ::anim_free_list
 */
void anim_init () {
    int i;

    if (Anim_inited == TRUE) return;
 ...
 * @details It will ignore animations which do not have the same id as the
 * passed screen_id
 */
void anim_render_one (int screen_id, anim_instance* ani, float frametime) {
    ...
    if (screen_id != ani->screen_id) { return; }
 ...
 * @details Will fill in default values, which you can then change before
 * calling ::anim_play().
 */
void anim_play_init (
    anim_play_struct* aps, anim* a_info, int x, int y, int base_w,
    int base_h) {
 ...
 * This will cause the anim to be played at the x,y position specified in the
 * parameter list.
 *
 * @param aps Compressed animation that we should make an instance from
 * @return If success pointer to instance, NULL if anim anim could not be
 * played
 */
anim_instance* anim_play (anim_play_struct* aps) {
    ...
    if (aps->color == NULL) { instance->xlate_pal = 1; }
    else {
    ...
    // key-frame. Futhermore, need to subtract 1 from key-frame number, since
    // frame number is always incremented the first time anim_show_next_frame()
    // is called
...
/**
 * @brief This function is called to blit the next frame of an anim instance to
...
int anim_show_next_frame (anim_instance* instance, float frametime) {
    ...
    if (instance->direction == ANIM_DIRECT_FORWARD)
        new_frame_num =
            instance->start_at - 1 + (int)lround (percent_through * n_frames);
    else
        new_frame_num =
            instance->start_at - 1 - (int)lround (percent_through * n_frames);
    ...
    // If framerate independent, use the new_frame_num... unless
    // instance->skip_frames is FALSE, then only advance a maximum of one frame
    // (this is needed since some big animations should just play slower rather
    // than taking the hit of decompressing multiple frames and creating an
    ...
    if (instance->stop_now == TRUE) { return -1; }

    // If past the last frame, clamp to the last frame and then set the
    // stop_now flag in the anim instance.  The next iteration, the animation
    // will stop.
    if (instance->direction == ANIM_DIRECT_FORWARD) {
    ...
    // if the anim is paused, ignore all the above changes and still display
    // this frame
    ...
    if (instance->parent->flags & ANF_XPARENT) { bitmap_flags = 0; }
    bpp = 16;
    ...
    if (frame_diff > 0) {
        ...
        for (i = 0; i < frame_diff; i++) {
            ...
            // if we're playing backwards, every frame must be a keyframe and
            // we set the data ptr here
            if (instance->direction == ANIM_DIRECT_REVERSE) {
                if (anim_instance_is_streamed (instance)) {
                    instance->file_offset =
                        instance->parent->file_offset +
                        instance->parent->keys[instance->frame_num - 1].offset;
                }
                else {
                    instance->data =
            ...
            if (anim_instance_is_streamed (instance)) {
                if (instance->xlate_pal) {
                    temp_file_offset = unpack_frame_from_file (
                        instance, instance->frame,
                        instance->parent->width * instance->parent->height,
                ...
                if (instance->xlate_pal) {
                    temp = unpack_frame (
                        instance, instance->data, instance->frame,
                        instance->parent->width * instance->parent->height,
                        instance->parent->palette_translation, aabitmap, bpp);
                }
                else {
    ...
    // this only happens when the anim is being looped, we need to reset the
    // last_frame_num
    ...
    if (frame_diff == 0 && instance->last_bitmap != -1) {
        ...
        bitmap_id = bm_create (
            bpp, instance->parent->width, instance->parent->height,
            instance->frame, bitmap_flags);
    }
    ...
    if (bitmap_id == -1) {
        ...
        if (instance->world_pos == NULL) {
            ...
            gr_set_clip (
                0, 0, instance->base_w, instance->base_h, GR_RESIZE_MENU);
            if (instance->aa_color == NULL) {
                ...
                gr_aabitmap (
                    instance->x, instance->y, GR_RESIZE_MENU_NO_OFFSET);
            }
            gr_set_screen_scale (
                old_max_w_unscaled, old_max_h_unscaled,
                old_max_w_unscaled_zoomed, old_max_h_unscaled_zoomed);
            gr_reset_clip ();
        }
        else {
            ...
            g3_render_rect_screen_aligned_2d (
                &mat_params, &image_vertex, 0, instance->radius * 1.5f);
        }
...
void anim_read_header (anim* ptr, CFILE* fp) {
    ...
    // If first 2 bytes are zero, this means we are using a new format, which
    // includes a version, and fps values. This is only done since a version
    // number was not included in the original header.
    ...
    float waste =
        100.0f * float((floor_size - diff)) / (2.0f * (float)floor_size);
...
/**
 * @brief Load an animation.  This stores the compressed data, which instances
 ...
 *
 * @details Memory-mapped files will page in the animation from disk as it is
 * needed, but performance is not as good.
...
anim* anim_load (const char* real_filename, int cf_dir_type, int file_mapped) {
    ...
    if (!ptr) {
        ...
        if (ptr->width < 0 || ptr->height < 0) {
            ASSERTX (0, "Ani file %s has a faulty header and cannot be loaded.", name);
        }
        ...
        if (file_mapped == PAGE_FROM_MEM) {
            ...
            ptr->cfile_ptr =
                cfopen (name, "rb", CFILE_MEMORY_MAPPED, cf_dir_type);
        }

        // couldn't memory-map file... must be in a packfile, so stream
        // manually
        ...
        if (ptr->cfile_ptr != NULL) {
            // VERY IMPORTANT STEP
            // Set the data pointer to the compressed data (which is not at the
            // start of the file).  Use ftell() to find out how far we've
            // already parsed into the file
            //
            int offset;
            offset = cftell (fp);
 ...
 * @details All instances referencing this animation must be free'ed or get an
 * assert.
 */
...
/**
 * @brief Called at the beginning of a mission to initialize any mission
 * dependent anim data.
 * @todo Redundant?
 */
...
/**
 * @brief Called after the end of a mission to clean up any mission dependent
 * anim data.
 */
void anim_level_close () { anim_release_all_instances (); }
...
int anim_write_frames_out (char* filename) {
    ...
    for (i = 0; i < source_anim->total_frames; i++) {
        ...
        pcx_write_bitmap (
            pcxname, source_anim->width, source_anim->height, row_data,
            source_anim->palette);
...
void anim_display_info (char* real_filename) {
    ...
    if (A.width < 0 || A.height < 0) {
        ASSERTX (0, "Ani file %s has a faulty header and cannot be loaded.",filename);
    }
...
#ifndef NDEBUG
    printf (
        "Transparent RGB:           (%u,%u,%u)\n", A.xparent_r, A.xparent_g,
        A.xparent_b);
#endif
...
void anim_reverse_direction (anim_instance* ai) {
    ...
    if (ai->direction == ANIM_DIRECT_FORWARD) {
        ai->time_elapsed =
            ((float)ai->frame_num - (float)ai->start_at - 1.0f) /
            (float)ai->parent->fps;
    }
    else if (ai->direction == ANIM_DIRECT_REVERSE) {
        ai->time_elapsed =
            ((float)ai->start_at - (float)ai->frame_num - 1.0f) /
            (float)ai->parent->fps;
    }
}

void anim_pause (anim_instance* ai) { ai->paused = 1; }

void anim_unpause (anim_instance* ai) { ai->paused = 0; }
