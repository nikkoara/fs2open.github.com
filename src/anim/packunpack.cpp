...
int anim_get_next_frame (anim_instance* inst) {
    ...
    if (anim_instance_is_streamed (inst)) {
        if (inst->file_offset <= 0) { return -1; }
    }
    else {
    ...
    if (anim_instance_is_streamed (inst)) {
        inst->file_offset = unpack_frame_from_file (
            inst, inst->frame, inst->parent->width * inst->parent->height,
            inst->parent->palette_translation, aabitmap, bpp);
    }
    else {
        inst->data = unpack_frame (
            inst, inst->data, inst->frame,
            inst->parent->width * inst->parent->height,
            inst->parent->palette_translation, aabitmap, bpp);
    }

    bm = bm_create (
        bpp, inst->parent->width, inst->parent->height, inst->frame,
        bitmap_flags);
    bm_unload (bm);
    return bm;
}

ubyte* anim_get_next_raw_buffer (
    anim_instance* inst, int xlate_pal, int aabitmap, int bpp) {
    if (anim_instance_is_streamed (inst)) {
        if (inst->file_offset < 0) { return NULL; }
    }
    else {
    ...
    if (anim_instance_is_streamed (inst)) {
        ...
        if (xlate_pal) {
            ...
            inst->data = unpack_frame (
                inst, inst->data, inst->frame,
                inst->parent->width * inst->parent->height, NULL, aabitmap,
                bpp);
        }
    }
 ...
 */
int pack_frame (
    ubyte* frame, ubyte* frame2, ubyte* save, long size, long max,
    int compress_type) {
...
void convert_24_to_16 (int bit_24, ushort* bit_16) {
    ...
    bm_set_components (
        (ubyte*)bit_16, (ubyte*)&pixel[0], (ubyte*)&pixel[1],
        (ubyte*)&pixel[2], &alpha);
}
 ...
 */
int unpack_pixel (
    anim_instance* ai, ubyte* data, ubyte pix, int aabitmap, int bpp) {
    int bit_24;
    ushort bit_16 = 0;
    ubyte bit_8 = 0;
    ubyte al = 0;
    ...
    if (aabitmap) {
        switch (bpp) {
        case 16: bit_16 = (ushort)pix; break;
        case 8:
            // 8 bit-per-pixel aa bitmaps are a bit special since they only use
            // value in the range [0, 15] where 15 wraps around back to 0.
            // Since the rest of the code expects the value to be in the range
            // [0, 255] the pixel value needs to be adjusted here. By
            // muliplying the value with 17 the original range [0, 15] is
            // mapped to [0, 255]
            if (pix >= 15) { bit_8 = 0; }
            else {
                bit_8 = (ubyte) (pix * 17);
        ...
        if (((a->palette[pix * 3] == a->xparent_r) &&
             (a->palette[pix * 3 + 1] == a->xparent_g) &&
             (a->palette[pix * 3 + 2] == a->xparent_b))) {
            if (pixel_size > 2) { bit_24 = 0; }
            else {
                r = b = 0;
                g = 255;
    ...
    switch (bpp) {
    case 32:
    case 24: memcpy (data, &bit_24, pixel_size); break;

    case 16: memcpy (data, &bit_16, pixel_size); break;
...
int unpack_pixel_count (
    anim_instance* ai, ubyte* data, ubyte pix, int count = 0, int aabitmap = 0,
    int bpp = 8) {
    int bit_24;
    int idx;
    ubyte al = 0;
 ...
 * @param pal_translate Color translation lookup table (NULL if no palette
 * translation desired)
 * @param aabitmap
 * @param bpp
 */
ubyte* unpack_frame (
    anim_instance* ai, ubyte* ptr, ubyte* frame, int size,
    ubyte* pal_translate, int aabitmap, int bpp) {
    int xlate_pal, value, count = 0;
    int stuffed;
    int pixel_size = (bpp / 8);

    if (pal_translate == NULL) { xlate_pal = 0; }
    else {
        xlate_pal = 1;
    }
    ...
    if (*ptr == PACKING_METHOD_RLE_KEY) { // key frame, Hoffoss's RLE format
        ...
        while (size > 0) {
            ...
            if (value != packer_code) {
                if (xlate_pal) {
                    stuffed = unpack_pixel (
                        ai, frame, pal_translate[value], aabitmap, bpp);
                }
                else {
                    stuffed =
                        unpack_pixel (ai, frame, (ubyte)value, aabitmap, bpp);
                }
                frame += stuffed;
                size--;
            }
                ...
                if (xlate_pal) {
                    stuffed = unpack_pixel_count (
                        ai, frame, pal_translate[value], count, aabitmap, bpp);
                }
                else {
                    stuffed = unpack_pixel_count (
                        ai, frame, (ubyte)value, count, aabitmap, bpp);
                }
    ...
    else if (*ptr == PACKING_METHOD_STD_RLE_KEY) { // key frame, with high bit
                                                   // as count
        ptr++;
        while (size > 0) {
            value = *ptr++;
    ...
    else if (*ptr == PACKING_METHOD_RLE) { // normal frame, Hoffoss's RLE
                                           // format
            ...
            if (value != packer_code) {
                if (value != transparent_code) {
                    if (xlate_pal) {
                        ...
                        stuffed = unpack_pixel (
                            ai, frame, (ubyte)value, aabitmap, bpp);
                    }
                }
                else {
                    // temporary pixel
                ...
                if (value != transparent_code) {
                    if (xlate_pal) {
                        stuffed = unpack_pixel_count (
                            ai, frame, pal_translate[value], count, aabitmap,
                            bpp);
                    }
                    else {
                        stuffed = unpack_pixel_count (
    ...
    else if (*ptr == PACKING_METHOD_STD_RLE) { // normal frame, with high bit
                                               // as count
        ptr++;
        while (size > 0) {
            value = *ptr++;
 ...
 * @param bpp
 */
int unpack_frame_from_file (
    anim_instance* ai, ubyte* frame, int size, ubyte* pal_translate,
    int aabitmap, int bpp) {
    int xlate_pal, value, count = 0;
    int offset = 0;
    int stuffed;
    int pixel_size = (bpp / 8);
    ...
    if (anim_instance_get_byte (ai, offset) ==
        PACKING_METHOD_RLE_KEY) { // key frame, Hoffoss's RLE format
        offset++;
        while (size > 0) {
        ...
        anim_instance_get_byte (ai, offset) ==
        ...
        anim_instance_get_byte (ai, offset) ==
        PACKING_METHOD_RLE) { // normal frame, Hoffoss's RLE format
    ...
    else if (anim_instance_get_byte (
                 ai, offset)) { // normal frame, with high bit as count
        offset++;
        while (size > 0) {
            value = anim_instance_get_byte (ai, offset);
            offset++;
...
void anim_set_palette (anim* ptr) {
    ...
    for (i = 0; i < 256; i++) { ptr->palette_translation[i] = (ubyte)i; }

    if (xparent_found) { ptr->flags |= ANF_XPARENT; }
    else {
