...
struct anim  {
    ...
    ubyte
        xparent_g; // green component for the transparent color in source image
    ubyte
        xparent_b; // blue component for the transparent color in source image
    int flags;
    uint screen_sig;
...
struct anim_instance  {
    ...
    int frame_num; // current frame, or last frame if between frames (first
                   // frame is 0)
    ...
    int framerate_independent; // animation should play back in same amount of
                               // time, regardless
    int skip_frames; // should anim skip frames during framerate independent
                     // playback
    ...
    int screen_id;   // 0 means all screens should render, otherwise screen
                     // specific
...
#endif // FREESPACE2_ANIM_PACKUNPACK_HH
