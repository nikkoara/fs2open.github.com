                ...
                if (Player_ai->target_objnum != -1) {
                        ...
                        if (objp->type == OBJ_SHIP) {
                                ship_apply_local_damage(
                                        ...
                                        Ships[objp->instance].ship_max_hull_strength * 0.1f + 10.0f,
...
int button_function(int n)
{
     ...
     */
        ...
        case END_MISSION:
                ...
                if (collide_predict_large_ship(Player_obj, 200.0f) || (Ship_info[Ships[Player_obj->instance].ship_info_index].warpout_type == WT_HYPERSPACE && collide_predict_large_ship(Player_obj, 100000.0f))) {
                        gamesnd_play_iface(InterfaceSounds::GENERAL_FAIL);
                        HUD_printf(
                                "%s", XSTR(
        ...
        case AUTO_PILOT_TOGGLE:
                if (!(The_mission.flags[Mission::Mission_Flags::Deactivate_ap])) {
                        if (AutoPilotEngaged) {
                                if (Cmdline_autopilot_interruptable == 1) // allow WCS to disable autopilot interrupt via
                                                                          // commandline
                                        EndAutoPilot();
                        } else {
                                if (!StartAutopilot())
