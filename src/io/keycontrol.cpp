...
public:
    ...
    /**
     * Returns the next natural number factor for the given number and current
     * factor.
     *
...
private:
    ...
    /**
     * Returns true if the given factor is a natural number factor for the
     * given value.
     *
...
factor_table::factor_table (size_t size) { resize (size); }
...
void factor_table::resize (size_t size) {
    ...
    for (size_t i = oldSize; i < size; ++i) {
        for (size_t j = 1; j <= i + 1; ++j) {
            if (isNaturalNumberFactor (j, i + 1)) { _lookup[i].push_back (j); }
        }
    }
}
...
extern float do_subobj_hit_stuff (
    object* ship_obj, object* other_obj, vec3d* hitpos, int submodel_num,
    float damage, bool* hull_should_apply_armor);

extern void mission_goal_mark_all_true (int type);

int Normal_key_set[] = { TARGET_NEXT,
                         TARGET_PREV,
...
int Critical_key_set[] = {
    CYCLE_NEXT_PRIMARY, CYCLE_PREV_PRIMARY, CYCLE_SECONDARY,
    CYCLE_NUM_MISSLES,  INCREASE_WEAPON,    DECREASE_WEAPON,
    INCREASE_SHIELD,    DECREASE_SHIELD,    INCREASE_ENGINE,
    DECREASE_ENGINE,    ETS_EQUALIZE,       SHIELD_EQUALIZE,
    SHIELD_XFER_TOP,    SHIELD_XFER_BOTTOM, SHIELD_XFER_LEFT,
    SHIELD_XFER_RIGHT,  XFER_SHIELD,        XFER_LASER,
};

int Non_critical_key_set[] = { MATCH_TARGET_SPEED,
                               TOGGLE_AUTO_MATCH_TARGET_SPEED,
                               TARGET_NEXT,
                               TARGET_PREV,
                               TARGET_NEXT_CLOSEST_HOSTILE,
...
void debug_cycle_player_ship (int delta) {
    ...
    while (TRUE) {
        ...
        if (sip->flags[Ship::Info_Flags::Player_ship]) { break; }

        // just in case
        sanity++;
        if (sanity >= static_cast< int > (Ship_info.size ())) { break; }
    }

    change_ship_type (Player_obj->instance, si_index);
    HUD_sourced_printf (
        HUD_SOURCE_HIDDEN, XSTR ("Player ship changed to %s", 0),
        Ship_info[si_index].name);
}
...
void debug_cycle_targeted_ship (int delta) {
    ...
    while (TRUE) {
        si_index += delta;
        if (si_index >= static_cast< int > (Ship_info.size ())) si_index = 0;
        if (si_index < 0)
            si_index = static_cast< int > (Ship_info.size () - 1);
    ...
    HUD_sourced_printf (
        HUD_SOURCE_HIDDEN, XSTR ("Changed player target to %s", 1),
        Ship_info[si_index].name);
}
...
void debug_max_secondary_weapons (object* objp) {
    ...
    for (index = 0; index < MAX_SHIP_SECONDARY_BANKS; index++) {
        swp->secondary_bank_ammo[index] =
            sip->secondary_bank_ammo_capacity[index];
    }
}
...
void debug_change_song (int delta) {
    ...
    if (event_music_next_soundtrack (delta) != -1) {
        ...
        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN, XSTR ("Soundtrack changed to: %s", 2), buf);
    }
    else {
        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN, "%s", XSTR ("Event music is not playing", 3));
    }
}
...
extern void g3_set_view_matrix (
...
int get_prev_weapon_looped (int current_weapon, int subtype) {
    ...
    for (i = 1; i < Num_weapon_types; i++) {
        ...
        if (Weapon_info[new_index].subtype == subtype) { return new_index; }
    }
...
void process_debug_keys (int k) {
    ...
    case KEY_DEBUGGED1 + KEY_Q: Snapshot_all_events = true; break;

    case KEY_DEBUGGED + KEY_H: hud_target_toggle_hidden_from_sensors (); break;

    case KEY_DEBUGGED + KEY_F:
        extern int wacky_scheme;
        if (wacky_scheme == 3) { wacky_scheme = 0; }
        else {
            wacky_scheme++;
        }
        break;
    ...
    case KEY_DEBUGGED + KEY_ALTED + KEY_F:
        ...
        HUD_printf (
            XSTR (
                "Framerate delay increased to %i milliseconds per frame.", 4),
            Framerate_delay);
        break;

    case KEY_DEBUGGED + KEY_ALTED + KEY_SHIFTED + KEY_F:
        Framerate_delay -= 10;
        if (Framerate_delay < 0) Framerate_delay = 0;

        HUD_printf (
            XSTR (
                "Framerate delay decreased to %i milliseconds per frame.", 5),
            Framerate_delay);
        break;
    ...
    case KEY_DEBUGGED1 + KEY_X:
        HUD_printf (
            "Cloaking has been disabled, thank you for playing fs2_open, %s",
            Player->callsign);
        break;
    ...
    case KEY_DEBUGGED1 + KEY_C:
        ...
        if (Player_obj->flags[Object::Object_Flags::Collides]) {
            obj_set_flags (
                Player_obj,
                Player_obj->flags - Object::Object_Flags::Collides);
            HUD_sourced_printf (
                HUD_SOURCE_HIDDEN, "Player no longer collides");
        }
        else {
            obj_set_flags (
                Player_obj,
    ...
    case KEY_DEBUGGED1 + KEY_SHIFTED + KEY_C:
        ...
        HUD_printf (
            XSTR ("Countermeasure firing: %s", 6), Countermeasures_enabled
                                                       ? XSTR ("ENABLED", 7)
                                                       : XSTR ("DISABLED", 8));
        break;

// Goober5000: this should only be compiled in debug builds, since it crashes
// release
#ifndef NDEBUG
        ...
        if (Player_ai->target_objnum != -1) {
            ...
            case OBJ_SHIP:
                ...
                ship_apply_local_damage (
                    objp, Player_obj, &objp->pos, 100000.0f, MISS_SHIELDS,
                    CREATE_SPARKS);
                ship_apply_local_damage (
                    objp, Player_obj, &objp->pos, 1.0f, MISS_SHIELDS,
                    CREATE_SPARKS);
                break;
            case OBJ_WEAPON: Weapons[objp->instance].lifeleft = 0.01f; break;
            }
        }
    ...
    case KEY_DEBUGGED + KEY_V:
        ...
        if (Num_messages_playing) { message_kill_all (1); }
        ...
        message_send_unique_to_player (
            ...
            MESSAGE_SOURCE_SPECIAL, MESSAGE_PRIORITY_HIGH, 0, 0);
    ...
    case KEY_DEBUGGED1 + KEY_ALTED + KEY_SHIFTED + KEY_K:
        if (Player_ai->target_objnum != -1) {
            ...
            if (objp->type == OBJ_SHIP) {
                ship_apply_local_damage (
                    objp, Player_obj, &objp->pos,
                    Ships[objp->instance].ship_max_hull_strength * 0.1f +
                        10.0f,
                    MISS_SHIELDS, CREATE_SPARKS);
            }
        }
        break;
        ...
        if ((Player_ai->target_objnum != -1) &&
            ...
            if (objp->type == OBJ_SHIP) {
                ...
                get_subsystem_world_pos (
                    objp, Player_ai->targeted_subsys, &g_subobj_pos);

                do_subobj_hit_stuff (
                    objp, Player_obj, &g_subobj_pos,
                    Player_ai->targeted_subsys->system_info->subobj_num,
                    (float)-Player_ai->targeted_subsys->system_info->type,
                    NULL); // 100.0f);

                if (sp->subsys_info[SUBSYSTEM_ENGINE].aggregate_current_hits <=
                    0.0f) {
                    mission_log_add_entry (
                        LOG_SHIP_DISABLED, sp->ship_name, NULL);
                    sp->flags.set (
                        Ship::Ship_Flags::Disabled); // add the disabled flag
                }

                if (sp->subsys_info[SUBSYSTEM_TURRET].aggregate_current_hits <=
                    0.0f) {
                    mission_log_add_entry (
                        LOG_SHIP_DISARMED, sp->ship_name, NULL);
                }
            }
        }
        break;
    ...
    case KEY_DEBUGGED1 + KEY_ALTED + KEY_K: {
        ...
        vm_vec_scale_add (
            &pos, &Player_obj->pos, &randvec, Player_obj->radius);
        ship_apply_local_damage (
            Player_obj, Player_obj, &pos, 25.0f, MISS_SHIELDS, CREATE_SPARKS);
        hud_get_target_strength (Player_obj, &shield, &integrity);
        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN,
            XSTR ("You knocked yourself down to %7.3f percent hull.\n", 9),
            100.0f * integrity);
        break;
    }
    ...
    case KEY_DEBUGGED1 + KEY_I:
        ...
        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN, XSTR ("You are %s", 10),
            Player_obj->flags[Object::Object_Flags::Invulnerable]
                ? XSTR ("now INVULNERABLE!", 11)
                : XSTR ("no longer invulnerable...", 12));
        break;
    ...
    case KEY_DEBUGGED1 + KEY_SHIFTED + KEY_I:
        if (Player_ai->target_objnum != -1) {
            ...
            HUD_sourced_printf (
                HUD_SOURCE_HIDDEN, XSTR ("Player's target [%s] is %s", 13),
                Ships[objp->instance].get_display_string (),
    ...
    case KEY_DEBUGGED + KEY_N:
        ...
        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN,
            XSTR ("Spewing debug info about object #%d", 14), AI_watch_object);
        break;

    case KEY_DEBUGGED + KEY_O:
    case KEY_DEBUGGED1 + KEY_O: toggle_player_object (); break;

    case KEY_DEBUGGED + KEY_SHIFTED + KEY_O:
        extern int Debug_octant;
        if (Debug_octant == 7) { Debug_octant = -1; }
        else {
            Debug_octant++;
        }
        ...
        if (Weapon_energy_cheat) {
            if (k & KEY_SHIFTED)
                HUD_sourced_printf (
                    HUD_SOURCE_HIDDEN, "%s",
                    XSTR (
                        "Weapon energy and missile count will always be at "
                        "full ALL SHIPS!",
                        15));
            else
                HUD_sourced_printf (
    ...
    case KEY_DEBUGGED1 + KEY_9:
        ...
        int* weap = &shipp->weapons.secondary_bank_weapons
                         [shipp->weapons.current_secondary_bank];
        *weap = get_next_weapon_looped (*weap, WP_MISSILE);

        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN, XSTR ("Secondary Weapon forced to %s", 18),
            Weapon_info[*weap].get_display_string ());
        break;
    }
    ...
    case KEY_DEBUGGED1 + KEY_U:
        // launch asteroid
        object* asteroid_create (
            asteroid_field * asfieldp, int asteroid_type, int subtype);
        ...
        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN, "%s", XSTR ("Asteroid launched", 1595));
        break;
    }
    ...
    case KEY_DEBUGGED1 + KEY_0:
        ...
        *weap = get_next_weapon_looped (*weap, WP_LASER);
        ...
        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN, XSTR ("Primary Weapon forced to %s", 19),
            Weapon_info[*weap].get_display_string ());
        break;
    }
    ...
    case KEY_DEBUGGED + KEY_J: {
        ...
        new_pattern++;
        if (new_pattern >= MAX_PATTERNS) new_pattern = 0;
    ...
    case KEY_DEBUGGED + KEY_M: {
        if (Event_music_enabled) {
            event_music_disable ();
            HUD_sourced_printf (
                HUD_SOURCE_HIDDEN, "%s", XSTR ("Event music disabled", 20));
        }
        else {
            event_music_enable ();
            HUD_sourced_printf (
                HUD_SOURCE_HIDDEN, "%s", XSTR ("Event music enabled", 21));
        }
    ...
    case KEY_DEBUGGED1 + KEY_R: {
        ...
        object* obj_to_rearm = (Player_ai->target_objnum >= 0)
                                   ? &Objects[Player_ai->target_objnum]
                                   : Player_obj;

        if (is_support_allowed (obj_to_rearm)) {
            HUD_sourced_printf (
                HUD_SOURCE_HIDDEN, XSTR ("Issuing rearm request for %s", 1610),
                Ships[obj_to_rearm->instance].get_display_string ());
            ai_issue_rearm_request (obj_to_rearm);
        }
        else {
            HUD_sourced_printf (
                HUD_SOURCE_HIDDEN,
                XSTR ("Cannot issue rearm request for %s", 1611),
                Ships[obj_to_rearm->instance].get_display_string ());
        }
    ...
    case KEY_DEBUGGED + KEY_SHIFTED + KEY_T: {
        ...
        if (Test_begin == 1) break;

        Test_begin = 1;
        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN, "%s", XSTR ("Frame Rate test started", 23));
    ...
    case KEY_DEBUGGED + KEY_D:
        ...
        if (MULTIPLAYER_MASTER) {
            do {
                OO_update_index++;
            } while ((OO_update_index < (MAX_PLAYERS - 1)) &&
                     !MULTI_CONNECTED (Net_players[OO_update_index]));
            if (OO_update_index >= MAX_PLAYERS - 1) { OO_update_index = -1; }
        }
        else {
            if (OO_update_index < 0) { OO_update_index = MY_NET_PLAYER_NUM; }
            else {
                OO_update_index = -1;
            }
        }
    ...
    case KEY_DEBUGGED + KEY_RIGHT: debug_cycle_player_ship (1); break;

    // change player ship to previous flyable ship
    case KEY_DEBUGGED + KEY_LEFT: debug_cycle_player_ship (-1); break;
    ...
    case KEY_DEBUGGED + KEY_S: {
        if (Player_ai->target_objnum < 0) break;
        ...
        if (sip->species >= (int)Species_info.size ()) sip->species = 0;

        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN, XSTR ("Species of target changed to: %s", 24),
    ...
    case KEY_DEBUGGED + KEY_SHIFTED + KEY_S:
        ...
        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN, XSTR ("Skill level set to %s.", 25),
    ...
    case KEY_DEBUGGED1 + KEY_UP: debug_change_song (1); break;

    case KEY_DEBUGGED + KEY_DOWN:
    case KEY_DEBUGGED1 + KEY_DOWN: debug_change_song (-1); break;
    ...
    case KEY_DEBUGGED1 + KEY_ALTED + KEY_EQUAL: {
        ...
        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN, "Camera fov raised to %0.2f", cam->get_fov ());
    } break;
    ...
    case KEY_DEBUGGED1 + KEY_ALTED + KEY_MINUS: {
        ...
        cam->set_fov (cam->get_fov () - 0.1f);
        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN, "Camera fov lowered to %0.2f", cam->get_fov ());
    } break;
...
void ppsk_hotkeys (int k) {
    ...
    case KEY_F12 + KEY_SHIFTED:
        ...
        if (Player_ai->target_objnum == -1)
            HUD_sourced_printf (
                HUD_SOURCE_HIDDEN,
                XSTR ("No target to add/remove from set %d.", 26),
                hotkey_set + 1);
        else {
            hud_target_hotkey_add_remove (
                hotkey_set, &Objects[Player_ai->target_objnum],
                HOTKEY_USER_ADDED);
            HUD_sourced_printf (
                HUD_SOURCE_HIDDEN, XSTR ("%s added to set %d. (F%d)", 27),
                Ships[Objects[Player_ai->target_objnum].instance]
                    .get_display_string (),
                hotkey_set, 4 + hotkey_set + 1);
        }
    ...
    case KEY_F12 + KEY_SHIFTED + KEY_ALTED:
        hotkey_set =
            mission_hotkey_get_set_num (k & (~(KEY_SHIFTED + KEY_ALTED)));
...
/**
 * Check keypress 'key' against a set of valid controls and mark the match in
 * the player's button info bitfield.  Also checks joystick controls in the
 * set.
 *
...
void process_player_ship_keys (int k) {
    ...
    if (keyd_pressed[KEY_DEBUG_KEY]) { return; }

    // if we're in supernova mode. do nothing
    if (Player->control_mode == PCM_SUPERNOVA) { return; }

    // pass the key to the squadmate messaging code.  If the messaging code
    // took the key, then return from here immediately since we don't want to
    // do further key processing.
...
void game_do_end_mission_popup () {
    ...
    if (Game_mode & GM_MULTIPLAYER) { multi_quit_game (PROMPT_ALL); }
    else {
        ...
        case 2: gameseq_post_event (GS_EVENT_ENTER_GAME); break;
...
void game_process_pause_key () {
    ...
    if (Game_mode & GM_MULTIPLAYER) {
        if (Multi_pause_status) { multi_pause_request (0); }
        else {
...
void game_process_cheats (int k) {
    ...
    if (detectedCheatCode == CHEAT_CODE_SKIP &&
        (gameseq_get_state () == GS_STATE_MAIN_MENU)) {
        extern void main_hall_campaign_cheat ();
        main_hall_campaign_cheat ();
    }
    if (detectedCheatCode == CHEAT_CODE_TOOLED &&
        (Game_mode & GM_IN_MISSION)) {
        Tool_enabled = 1;
        HUD_printf ("Prepare to be taken to school");
    }
    if (detectedCheatCode == CHEAT_CODE_PIRATE &&
        (Game_mode & GM_IN_MISSION) && (Player_obj != NULL)) {
        ...
        // create a name for the ship.  use "Volition Bravos #".  look for
        // collisions until one isn't found anymore
        ship_idx = 1;
        do {
        ...
        for (ptr = GET_FIRST (&shipp->subsys_list);
             ptr != END_OF_LIST (&shipp->subsys_list); ptr = GET_NEXT (ptr)) {
...
void game_process_keys () {
    ...
    button_info_clear (
        &Player->bi); // clear out the button info struct for the player
    do {
        k = game_poll ();

        if (Game_mode & GM_DEAD_BLEW_UP) { continue; }
        ...
        case KEY_ESC:
            if (Player->control_mode != PCM_NORMAL) {
                ...
                if ((Viewer_mode & VM_TOPDOWN) &&
                    !(The_mission.flags[Mission::Mission_Flags::Mission_2d]) &&
                    !(Perspective_locked)) {
                ...
                if (!(Game_mode & GM_DEAD_DIED)) game_do_end_mission_popup ();
            }
            break;
        ...
        case KEY_ALTED + KEY_PAUSE:
            if (Game_mode & GM_DEAD_BLEW_UP || Game_mode & GM_DEAD_DIED) {
                break;
            }
    ...
    button_info_do (&Player->bi); // call functions based on status of
                                  // button_info bit vectors
}
...
int button_function_critical (int n, net_player* p = NULL) {
    ...
    if (MULTIPLAYER_CLIENT) {
        // if this flag is set, we should apply the button itself (came from
        // the server)
        if (!Multi_button_info_ok) { return 0; }
    }

    // in single player mode make sure we're using the player object and the
    // player himself, otherwise use the object and player pertaining to the
    // passed net_player
    npl = NULL;
    if (p == NULL) {
        objp = Player_obj;
        ...
        if (Game_mode & GM_MULTIPLAYER) {
            ...
            // if we're the server in multiplayer and we're an observer, don't
            // process our own critical button functions
        ...
        if (NETPLAYER_IS_DEAD (npl) || (Ships[Objects[pl->objnum].instance]
    ...
    case CYCLE_PRIMARY_WEAPON_SEQUENCE: {
        ...
        if (sip->flags[Ship::Info_Flags::Dyn_primary_linking]) {
            ...
            count = (int)ftables.getNext (
                pm->gun_banks[swp->current_primary_bank].num_slots,
                swp->primary_bank_slot_count[swp->current_primary_bank]);
            swp->primary_bank_slot_count[swp->current_primary_bank] = count;
            shipp->last_fired_point[swp->current_primary_bank] +=
                count -
                (shipp->last_fired_point[swp->current_primary_bank] % count);
    ...
    case CYCLE_NEXT_PRIMARY:
        if (at_self) { control_used (CYCLE_NEXT_PRIMARY); }
    ...
    case CYCLE_PREV_PRIMARY:
        if (at_self) { control_used (CYCLE_PREV_PRIMARY); }
    ...
    case CYCLE_SECONDARY:
        if (at_self) control_used (CYCLE_SECONDARY);
    ...
    case CYCLE_NUM_MISSLES: {
        if (at_self) control_used (CYCLE_NUM_MISSLES);

        if (objp == Player_obj) {
            if (Player_ship->weapons.num_secondary_banks <= 0) {
                HUD_sourced_printf (
                    HUD_SOURCE_HIDDEN, "%s",
                    XSTR ("This ship has no secondary weapons", 33));
        ...
        int firepoints = pm->missile_banks[Ships[objp->instance]
                                               .weapons.current_secondary_bank]
                             .num_slots;

        if (Ships[objp->instance]
                .flags[Ship::Ship_Flags::Secondary_dual_fire] ||
            firepoints < 2) {
            Ships[objp->instance].flags.remove (
                Ship::Ship_Flags::Secondary_dual_fire);
            if (at_self) {
                HUD_sourced_printf (
                    HUD_SOURCE_HIDDEN, "%s",
                    XSTR ("Secondary weapon set to normal fire mode", 34));
            ...
            if (at_self) {
                HUD_sourced_printf (
                    HUD_SOURCE_HIDDEN, "%s",
                    XSTR ("Secondary weapon set to dual fire mode", 35));
                snd_play (gamesnd_get_game_sound (
                    ship_get_sound (Player_obj, GameSounds::SECONDARY_CYCLE)));
                hud_gauge_popup_start (HUD_WEAPONS_GAUGE);
            }
    ...
    case INCREASE_WEAPON:
        if (at_self) control_used (INCREASE_WEAPON);
    ...
    case DECREASE_WEAPON:
        if (at_self) control_used (DECREASE_WEAPON);
    ...
    case INCREASE_SHIELD:
        if (at_self) control_used (INCREASE_SHIELD);
    ...
    case DECREASE_SHIELD:
        if (at_self) control_used (DECREASE_SHIELD);
    ...
    case INCREASE_ENGINE:
        if (at_self) control_used (INCREASE_ENGINE);
    ...
    case DECREASE_ENGINE:
        if (at_self) control_used (DECREASE_ENGINE);
    ...
    case ETS_EQUALIZE:
        if (at_self) { control_used (ETS_EQUALIZE); }
...
int button_function_demo_valid (int n) {
    // by default, we'll return "not processed". ret will get set to 1, if this
    ...
    case VIEW_CHASE:
        ...
        if (!Perspective_locked) { Viewer_mode ^= VM_CHASE; }
        else {
    ...
    case VIEW_EXTERNAL:
        ...
        if (!Perspective_locked) {
            ...
            Viewer_mode &= ~VM_CAMERA_LOCKED; // reset camera lock when
                                              // leaving/entering external view
        }
        else {
            snd_play (gamesnd_get_game_sound (GameSounds::TARGET_FAIL));
        }
    ...
    case VIEW_TOPDOWN:
        ...
        if (!Perspective_locked) { Viewer_mode ^= VM_TOPDOWN; }
        else {
            snd_play (gamesnd_get_game_sound (GameSounds::TARGET_FAIL));
        }
        ret = 1;
        ...
        if (Viewer_mode & VM_EXTERNAL) {
            ...
            if (Viewer_mode & VM_CAMERA_LOCKED) {
                ...
                HUD_sourced_printf (
                    HUD_SOURCE_HIDDEN, "%s",
                    XSTR (
                        "External camera is free, controls will move the "
                        "camera, not the ship",
                        37));
            }
        }
    ...
    case TIME_SLOW_DOWN:
        if (Game_mode & GM_NORMAL) {
            // Goober5000 - time dilation only available in cheat mode (see
            // above); now you can do it with or without pressing the tilde,
            // per Kazan's request
            if (((Game_time_compression > F1_0) ||
                 (Cheats_enabled &&
                  (Game_time_compression > (F1_0 / MAX_TIME_DIVIDER)))) &&
                !Time_compression_locked) {
...
bool key_is_targeting (int n) {
    ...
    case TARGET_NEXT_SUBOBJECT: return true;
...
int button_function (int n) {
    ...
    if (Ignored_keys[n]) {
        if (Ignored_keys[n] > 0) { Ignored_keys[n]--; }
        return 0;
    }
    ...
    // work: so test and exit early
    if (Player_ship->flags[Ship::Ship_Flags::No_subspace_drive]) {
        switch (n) {
        case END_MISSION:
            control_used (n); // set the timestamp for when we used the
                              // control, in case we need it
            return 1; // pretend we took the action: if we return 0, strange
                      // stuff may happen
        }
    }

    // Goober5000 - if we have primitive sensors, some keys don't work: so test
    // and exit early
    ...
    case MAX_THROTTLE:
    case TOGGLE_GLIDING:
    case GLIDE_WHEN_PRESSED: return 0;
    }
    ...
    case CYCLE_NUM_MISSLES:  // cycle number of missiles fired from secondary
                             // bank
    ...
    /**
     * Assume the switches below will catch the key, if not, set to FALSE in
     * default
     *
    ...
    case MULTI_MESSAGE_ALL: multi_msg_key_down (MULTI_MSG_ALL); break;
    ...
    case MULTI_MESSAGE_HOSTILE: multi_msg_key_down (MULTI_MSG_HOSTILE); break;

    // message targeted ship (if player)
    case MULTI_MESSAGE_TARGET: multi_msg_key_down (MULTI_MSG_TARGET); break;
    ...
    case TOGGLE_AUTO_MATCH_TARGET_SPEED:
        ...
        if (Players[Player_num].flags & PLAYER_FLAGS_AUTO_MATCH_SPEED) {
            snd_play (
                gamesnd_get_game_sound (GameSounds::SHIELD_XFER_OK), 1.0f);
    ...
    case TARGET_NEWEST_SHIP: hud_target_newest_ship (); break;

    case TARGET_NEXT_LIVE_TURRET: hud_target_live_turret (1); break;

    case TARGET_PREV_LIVE_TURRET: hud_target_live_turret (0); break;

    // end the mission
    case END_MISSION:
        // in multiplayer, all end mission requests should go through the
        // server
        if (Game_mode & GM_MULTIPLAYER) {
        ...
        if (collide_predict_large_ship (Player_obj, 200.0f) ||
            (Ship_info[Ships[Player_obj->instance].ship_info_index]
                     .warpout_type == WT_HYPERSPACE &&
             collide_predict_large_ship (Player_obj, 100000.0f))) {
            gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
            HUD_printf (
                "%s", XSTR (
                          "** WARNING ** Collision danger.  Subspace drive "
                          "not activated.",
                          39));
        }
            ...
            HUD_printf (
                "%s",
                XSTR ("Engine failure.  Cannot engage subspace drive.", 40));
        }
        else if (!ship_navigation_ok_to_warp (Player_ship)) {
            gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
            HUD_printf (
                "%s", XSTR (
                          "Navigation failure.  Cannot engage subspace drive.",
                          1572));
        }
            ...
            HUD_printf (
                "%s",
                XSTR ("Cannot engage subspace drive while gliding.", 1573));
        }
        else {
    ...
    case MULTI_OBSERVER_ZOOM_TO: multi_obs_zoom_to_target (); break;
    ...
    case MULTI_SELF_DESTRUCT:
        if (!(Game_mode & GM_MULTIPLAYER)) { break; }

        // bogus netplayer
        if ((Net_player == NULL) || (Net_player->m_player == NULL)) { break; }
    ...
    case AUTO_PILOT_TOGGLE:
        if (!(The_mission.flags[Mission::Mission_Flags::Deactivate_ap])) {
            if (AutoPilotEngaged) {
                if (Cmdline_autopilot_interruptable ==
                    1) // allow WCS to disable autopilot interrupt via
                       // commandline
                    EndAutoPilot ();
            }
            else {
    ...
    case NAV_CYCLE:
        if (!Sel_NextNav ())
            gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
        break;
    default: keyHasBeenUsed = FALSE; break;
    }
    ...
    if (hud_sensors_ok (Player_ship)) {
        ...
        switch (n) {
        // target next
        case TARGET_NEXT: hud_target_next (); break;

        // target previous
        case TARGET_PREV: hud_target_prev (); break;

        // target the next hostile target
        case TARGET_NEXT_CLOSEST_HOSTILE: hud_target_next_list (); break;

        // target the previous closest hostile
        case TARGET_PREV_CLOSEST_HOSTILE: hud_target_next_list (1, 0); break;

        // target the next friendly ship
        case TARGET_NEXT_CLOSEST_FRIENDLY: hud_target_next_list (0); break;

        // target the closest friendly ship
        case TARGET_PREV_CLOSEST_FRIENDLY: hud_target_next_list (0, 0); break;

        // target ship closest to center of reticle
        case TARGET_SHIP_IN_RETICLE: hud_target_in_reticle_new (); break;
        ...
        case TARGET_CLOSEST_SHIP_ATTACKING_TARGET:
            ...
            hud_target_closest (
                iff_get_attacker_mask (
                    obj_team (&Objects[Player_ai->target_objnum])),
        ...
        case TARGET_CLOSEST_SHIP_ATTACKING_SELF:
            hud_target_next_list (
                1, 0, iff_get_attacker_mask (Player_ship->team),
                OBJ_INDEX (Player_obj), TRUE, 0, 1);
            break;

        // target your target's target
        case TARGET_TARGETS_TARGET: hud_target_targets_target (); break;
        ...
        case TARGET_PREV_SUBOBJECT: hud_target_prev_subobject (); break;

        // target next subsystem on current target
        case TARGET_NEXT_SUBOBJECT: hud_target_next_subobject (); break;
    ...
    case TOGGLE_AUTO_TARGETING:
        ...
        if (Players[Player_num].flags & PLAYER_FLAGS_AUTO_TARGETING) {
            if (hud_sensors_ok (Player_ship)) {
                hud_target_closest (
                    iff_get_attackee_mask (Player_ship->team), -1, FALSE,
                    TRUE);
                snd_play (
                    gamesnd_get_game_sound (GameSounds::SHIELD_XFER_OK), 1.0f);
                // HUD_sourced_printf(HUD_SOURCE_HIDDEN, XSTR( "Auto targeting
                // activated", -1));
            }
            else {
                Players[Player_num].flags ^= PLAYER_FLAGS_AUTO_TARGETING;
    ...
    // stop targeting ship
    case STOP_TARGETING_SHIP: hud_cease_targeting (); break;

    // stop targeting subsystems on ship
    case STOP_TARGETING_SUBSYSTEM: hud_cease_subsystem_targeting (); break;

    case TARGET_NEXT_BOMB: hud_target_missile (Player_obj, 1); break;

    case TARGET_PREV_BOMB: hud_target_missile (Player_obj, 0); break;

    // wingman message: attack current target
    case ATTACK_MESSAGE: hud_squadmsg_shortcut (ATTACK_TARGET_ITEM); break;

    // wingman message: disarm current target
    case DISARM_MESSAGE: hud_squadmsg_shortcut (DISARM_TARGET_ITEM); break;

    // wingman message: disable current target
    case DISABLE_MESSAGE: hud_squadmsg_shortcut (DISABLE_TARGET_ITEM); break;
    ...
    case CAPTURE_MESSAGE: hud_squadmsg_shortcut (CAPTURE_TARGET_ITEM); break;

    // wingman message: engage enemy
    case ENGAGE_MESSAGE: hud_squadmsg_shortcut (ENGAGE_ENEMY_ITEM); break;

    // wingman message: form on my wing
    case FORM_MESSAGE: hud_squadmsg_shortcut (FORMATION_ITEM); break;

    // wingman message: protect current target
    case PROTECT_MESSAGE: hud_squadmsg_shortcut (PROTECT_TARGET_ITEM); break;

    // wingman message: cover me
    case COVER_MESSAGE: hud_squadmsg_shortcut (COVER_ME_ITEM); break;

    // wingman message: warp out
    case WARP_MESSAGE: hud_squadmsg_shortcut (DEPART_ITEM); break;

    case IGNORE_MESSAGE: hud_squadmsg_shortcut (IGNORE_TARGET_ITEM); break;

    // rearm message
    case REARM_MESSAGE: hud_squadmsg_rearm_shortcut (); break;

    // cycle to next radar range
    case RADAR_RANGE_CYCLE:
        HUD_config.rp_dist++;
        if (HUD_config.rp_dist >= RR_MAX_RANGES) HUD_config.rp_dist = 0;

        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN, XSTR ("Radar range set to %s", 38),
    ...
    case SHOW_GOALS: gameseq_post_event (GS_EVENT_SHOW_GOALS); break;
    ...
    case ESCORT_CLEAR: hud_escort_clear_all (true); break;

    case TARGET_NEXT_ESCORT_SHIP: hud_escort_target_next (); break;
...
void button_info_do (button_info* bi) {
    for (int i = 0; i < CCFG_MAX; i++) {
        if (button_info_query (bi, i)) {
            ...
            if (!keyHasBeenUsed) { keyHasBeenUsed = button_function (i); }

            if (keyHasBeenUsed) { button_info_unset (bi, i); }
        }
    }
}
...
void button_info_clear (button_info* bi) {
    ...
    for (i = 0; i < NUM_BUTTON_FIELDS; i++) { bi->status[i] = 0; }
}
