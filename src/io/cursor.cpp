...
void changeMouseStatus (bool show, bool grab) {
    if (show) {
        ...
        if (grab) { setRelativeMouseMode (true); }
        else {
            setRelativeMouseMode (false);
        }
...
Cursor::~Cursor () {
    ...
    for (iter = mAnimationFrames.begin (); iter != mAnimationFrames.end ();
         ++iter) {
        SDL_FreeCursor (*iter);
    }
...
Cursor* CursorManager::loadCursor (const char* fileName, bool animated) {
    ...
    if (animated) { handle = bm_load_animation (fileName, nullptr, nullptr); }
    else {
...
Cursor* CursorManager::loadFromBitmap (int bitmapHandle) {
    ...
    for (int i = 0; i < nframes; ++i) {
        ...
        if (sdlCursor != nullptr) { cursor->addFrame (sdlCursor); }
        else {
...
void CursorManager::shutdown () { delete mSingleton; }
} // namespace mouse
} // namespace io
