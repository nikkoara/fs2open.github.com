...
bool mouse_wheel_event_handler (const SDL_Event& e) {
    ...
    mousewheel_motion (
        e.wheel.x, e.wheel.y, e.wheel.direction == SDL_MOUSEWHEEL_FLIPPED);
#else
...
/**
 * @brief Decays the mousewheel position back to 0 and clears the appropriate
 * flags when nuetral
...
void mouse_init () {
    ...
    // we do want to make sure that button presses go through event polling
    // though (should be on by default already, just here as a reminder)
...
void mouse_mark_button (uint flags, int set) {
    ...
    if (!(mouse_flags & MOUSE_LEFT_BUTTON)) {
        ...
        if ((flags & MOUSE_LEFT_BUTTON) && (set == 0)) { mouse_left_up++; }
    }
    ...
    if (!(mouse_flags & MOUSE_RIGHT_BUTTON)) {
        ...
        if ((flags & MOUSE_RIGHT_BUTTON) && (set == 0)) { mouse_right_up++; }
    }
    ...
    if (!(mouse_flags & MOUSE_MIDDLE_BUTTON)) {
        ...
        if ((flags & MOUSE_MIDDLE_BUTTON) && (set == 0)) { mouse_middle_up++; }
    }

    if (!(mouse_flags & MOUSE_X1_BUTTON)) {
        if ((flags & MOUSE_X1_BUTTON) && (set == 1)) { mouse_x1_pressed++; }
    }
    else {
        if ((flags & MOUSE_X1_BUTTON) && (set == 0)) { mouse_x1_up++; }
    }

    if (!(mouse_flags & MOUSE_X2_BUTTON)) {
        if ((flags & MOUSE_X2_BUTTON) && (set == 1)) { mouse_x2_pressed++; }
    }
    else {
        if ((flags & MOUSE_X2_BUTTON) && (set == 0)) { mouse_x2_up++; }
    }

    if (set) { mouse_flags |= flags; }
    else {
        mouse_flags &= ~flags;
    }
...
int mouse_down_count (int n, int reset_count) {
    ...
    case MOUSE_LEFT_BUTTON:
        ...
        if (reset_count) { mouse_left_pressed = 0; }
        break;

    case MOUSE_RIGHT_BUTTON:
        tmp = mouse_right_pressed;
        if (reset_count) { mouse_right_pressed = 0; }
        break;

    case MOUSE_MIDDLE_BUTTON:
        tmp = mouse_middle_pressed;
        if (reset_count) { mouse_middle_pressed = 0; }
        break;

    case MOUSE_X1_BUTTON:
        tmp = mouse_x1_pressed;
        if (reset_count) { mouse_x1_pressed = 0; }
        break;

    case MOUSE_X2_BUTTON:
        tmp = mouse_x2_pressed;
        if (reset_count) { mouse_x2_pressed = 0; }
        break;
    } // end switch
...
// mouse_up_count() returns the number of times button n has gone from down to
...
float mouse_down_time (int btn) {
    ...
    // Bail if not a button or wheel direction
    if ((btn < LOWEST_MOUSE_BUTTON) || (btn > HIGHEST_MOUSE_WHEEL))
        return 0.0f;
...
int mouse_get_pos (int* xpos, int* ypos) {
    ...
    if (Mouse_x >= gr_screen.max_w) { Mouse_x = gr_screen.max_w - 1; }

    if (Mouse_y >= gr_screen.max_h) { Mouse_y = gr_screen.max_h - 1; }
...
int mouse_get_pos_unscaled (int* xpos, int* ypos) {
    ...
    gr_unsize_screen_pos (
        (xpos) ? xpos : NULL, (ypos) ? ypos : NULL, NULL, NULL,
        GR_RESIZE_MENU);

    return flags;
}

void mouse_get_real_pos (int* mx, int* my) { SDL_GetMouseState (mx, my); }

void mouse_set_pos (int xpos, int ypos) { mouse_force_pos (xpos, ypos); }
...
void mousewheel_motion (int x, int y, bool reversed) {
    ...
    // Since x and y's magnitudes can be larger than 1, it is possible to
    // ignore the idle state
    if (Mouse_wheel_y > 0) {
        // UP
...
void mousewheel_decay (int btn) {
    switch (btn) {
    case MOUSE_WHEEL_UP: Mouse_wheel_y -= 1; break;
    case MOUSE_WHEEL_DOWN: Mouse_wheel_y += 1; break;
    case MOUSE_WHEEL_LEFT: Mouse_wheel_x -= 1; break;
    case MOUSE_WHEEL_RIGHT: Mouse_wheel_x += 1; break;
    }
