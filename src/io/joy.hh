 ...
 *
 * @details The implementation is event based so repeatedly polling the values
 * will not cause a performance penalty as the joystick values will be cached.
 */
class Joystick {
public:
    /**
     ...
     * freed when this object is deleted.
     *
     ...
     * @param[in] ext If @c false, uses the four-position hat values. If @c
     * ture, uses the eight-position hat values
     *
     ...
     * @param[in] pos The position to query, must be in [0, JOY_NUM_HAT_POS)
     * @param[in] ext If @c false, uses the four-position hat values. If @c
     * true, uses the eight-position hat values
     * @param reset @c true to reset the count
     * @return The number of times the button has been pressed
     *
     * @warning This function may be removed in the future, it's only here for
 ...
 * The returned pointer stays valid even if a device is disconnected in case it
 * is reconnected at a later time.
