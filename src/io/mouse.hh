...
// same as above but gets an unscaled reading (ie. the 1024x768/640x480
// equivalent)
...
#define MOUSE_WHEEL_UP \
    (1 << 5) // Wheel is treated like a pair of buttons, but they don't have a
             // down nor up count
...
// These mouse_button defines are a quick check to verify the input button is a
// mouse button Likewise, the mouse_wheel defines quickly check against the
// mousewheel directions
...
/**
 * @brief Returns the number of times button n went from up to down since last
 * call
 */
int mouse_down_count (int n, int reset_count = 1);

/**
 * @brief Returns the number of times button n went from down to up since last
 * call
 */
int mouse_up_count (int n);

/**
 * @brief Flushes the mouse's states clean. Called whenever it gains/loses
 * focus
 */
void mouse_flush ();
...
float mouse_down_time (
    int btn); // returns the fraction of time btn has been down since last call
