...
int key_checkch () {
    ...
    if (key_data.keytail != key_data.keyhead) { is_one_waiting = 1; }
...
void key_outkey (int key) {
    ...
    if (bufp >= KEY_BUFFER_SIZE) { bufp = 0; }
...
#ifndef NDEBUG
    if (keyd_pressed[KEY_DEBUG_KEY]) shift_status |= KEY_DEBUGGED;
#else
    if (keyd_pressed[KEY_DEBUG_KEY]) {
        WARNINGF (LOCATION, "Cheats_enabled = %i, Key_normal_game = %i",Cheats_enabled, Key_normal_game);
        if ((Cheats_enabled) && Key_normal_game) {
            WARNINGF (LOCATION, "Debug key");
...
void key_mark (uint code, int state, uint latency) {
    ...
    // If running in the UK, need to translate their wacky slash scancode to
    // ours
...
#ifndef NDEBUG
            if (keyd_pressed[KEY_DEBUG_KEY]) { keycode |= KEY_DEBUGGED; }
#else
            if (keyd_pressed[KEY_DEBUG_KEY]) {
                WARNINGF (LOCATION, "Cheats_enabled = %i, Key_normal_game = %i",Cheats_enabled, Key_normal_game);
                if (Cheats_enabled && Key_normal_game) {
...
void key_level_init () {
    ...
    for (i = 0; i < NUM_KEYS; i++) key_data.down_check[i] = 0;
}
