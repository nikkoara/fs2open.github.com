...
inline HatPosition hatBtnToEnum (int in) {
    ...
    switch (in) {
    case 0: return HAT_DOWN;

    case 1: return HAT_UP;

    case 2: return HAT_LEFT;

    case 3: return HAT_RIGHT;

    case 4: return HAT_LEFTDOWN;

    case 5: return HAT_LEFTUP;

    case 6: return HAT_RIGHTDOWN;

    case 7: return HAT_RIGHTUP;
...
std::string getJoystickGUID (SDL_Joystick* stick) {
    ...
    SDL_JoystickGetGUIDString (
        guid, &joystickGUID[0], static_cast< int > (joystickGUID.size ()));
    ...
    std::transform (
        begin (joystickGUID), end (joystickGUID), begin (joystickGUID),
        toupper);
...
void setCurrentJoystick (Joystick* stick) {
    ...
    if (currentJoystick != nullptr) {
        WARNINGF (LOCATION, "  Using '%s' as the primary joystick",currentJoystick->getName ().c_str ());
        WARNINGF (LOCATION, "  Number of axes: %d", currentJoystick->numAxes ());
        WARNINGF (LOCATION, "  Number of buttons: %d",currentJoystick->numButtons ());
        WARNINGF (LOCATION, "  Number of hats: %d", currentJoystick->numHats ());
        WARNINGF (LOCATION, "  Number of trackballs: %d",currentJoystick->numBalls ());
    }
    else {
...
Joystick* getJoystickForID (SDL_JoystickID id) {
    for (auto iter = joysticks.begin (); iter != joysticks.end (); ++iter) {
        if ((*iter)->getID () == id) { return (*iter).get (); }
    }
...
bool axis_event_handler (const SDL_Event& evt) {
    ...
    if (stick != nullptr) { stick->handleJoyEvent (evt); }
...
bool device_event_handler (const SDL_Event& evt) {
    ...
    if (evtType == SDL_JOYDEVICEADDED) {
        // A new joystick has been added, add it to our list and check if it's
        // the configured joystick
        Joystick* addedStick;
        {
            ...
            for (auto iter = joysticks.begin (); iter != joysticks.end ();
                 ++iter) {
        ...
        if (currentJoystick != nullptr) {
            // We already have a valid joystick device, ignore any further
            // events
            return true;
        }
        ...
        if (currentJoystick != nullptr &&
            joyDeviceEvent.which == currentJoystick->getID ()) {
...
int Joystick::getButtonDownCount (int index, bool reset) {
    ...
    if (reset) { _button[index].DownCount = 0; }
...
int Joystick::getHatDownCount (
    ...
    if (pos == HAT_CENTERED) { return 0; }

    if (ext) {
        // Use 8-position
        val = _hat[index].DownCount8[pos];

        if (reset) { _hat[index].DownCount8[pos] = 0; }
    }
    else {
        // Use 4-position
        val = _hat[index].DownCount4[pos];

        if (reset) { _hat[index].DownCount4[pos] = 0; }
    }
...
int Joystick::numHats () const { return static_cast< int > (_hat.size ()); }

std::string Joystick::getGUID () const { return _guidStr; }

std::string Joystick::getName () const { return _name; }

SDL_JoystickID Joystick::getID () const { return _id; }
...
void Joystick::fillValues () {
    ...
    for (auto i = 0; i < hatNum; ++i) {
        ...
        // Reset timestampts
        for (auto j = 0; j < 4; ++j) { _hat[i].DownTimestamp4[j] = -1; }
        for (auto j = 0; j < 8; ++j) { _hat[i].DownTimestamp8[j] = -1; }
...
SDL_Joystick* Joystick::getDevice () { return _joystick; }

void Joystick::handleJoyEvent (const SDL_Event& evt) {
    switch (evt.type) {
    case SDL_JOYAXISMOTION: handleAxisEvent (evt.jaxis); break;
    case SDL_JOYBALLMOTION: handleBallEvent (evt.jball); break;
    case SDL_JOYBUTTONDOWN:
    case SDL_JOYBUTTONUP: handleButtonEvent (evt.jbutton); break;
    case SDL_JOYHATMOTION: handleHatEvent (evt.jhat); break;
    default: break;
    }
}
...
void Joystick::handleButtonEvent (const SDL_JoyButtonEvent& evt) {
    ...
    if (down) { ++_button[button].DownCount; }
}
...
void Joystick::handleHatEvent (const SDL_JoyHatEvent& evt) {
    ...
    for (auto i = 0; i < 4; ++i) {
        if (!hatset[i]) { _hat[hat].DownTimestamp4[i] = -1; }
    }
    for (auto i = 0; i < 8; ++i) {
        if (i != hatpos) { _hat[hat].DownTimestamp8[i] = -1; }
    }
    ...
    if (hatpos != HAT_CENTERED) {
        ...
        if ((hatset[HAT_RIGHT]) &&
            (_hat[hat].DownTimestamp4[HAT_RIGHT] == -1)) {
...
void Joystick::printInfo () {
    WARNINGF (LOCATION, "  Joystick name: %s", getName ().c_str ());
    WARNINGF (LOCATION, "  Joystick GUID: %s", getGUID ().c_str ());
    WARNINGF (LOCATION, "  Joystick ID: %d", getID ());
    WARNINGF (LOCATION, "  Joystick device ID: %d", _device_id);
}
...
bool init () {
    ...
    addEventListener (
        SDL_JOYAXISMOTION, DEFAULT_LISTENER_WEIGHT, axis_event_handler);
    addEventListener (
        SDL_JOYBALLMOTION, DEFAULT_LISTENER_WEIGHT, ball_event_handler);

    addEventListener (
        SDL_JOYBUTTONDOWN, DEFAULT_LISTENER_WEIGHT, button_event_handler);
    addEventListener (
        SDL_JOYBUTTONUP, DEFAULT_LISTENER_WEIGHT, button_event_handler);

    addEventListener (
        SDL_JOYHATMOTION, DEFAULT_LISTENER_WEIGHT, hat_event_handler);

    addEventListener (
        SDL_JOYDEVICEADDED, DEFAULT_LISTENER_WEIGHT, device_event_handler);
    addEventListener (
        SDL_JOYDEVICEREMOVED, DEFAULT_LISTENER_WEIGHT, device_event_handler);
...
Joystick* getCurrentJoystick () { return currentJoystick; }
...
std::vector< JoystickInformation > getJoystickInformations () {
    ...
    for (auto i = 0; i < num_joysticks; ++i) {
        ...
        info.num_axes =
            static_cast< uint32_t > (SDL_JoystickNumAxes (joystick));
        info.num_balls =
            static_cast< uint32_t > (SDL_JoystickNumBalls (joystick));
        info.num_buttons =
            static_cast< uint32_t > (SDL_JoystickNumButtons (joystick));
        info.num_hats =
            static_cast< uint32_t > (SDL_JoystickNumHats (joystick));
...
int joystick_read_raw_axis (int num_axes, int* axis) {
    ...
    if (!initialized || current == nullptr) {
        // fake a return value so that controlconfig doesn't get freaky with no
        // joystick
        for (i = 0; i < num_axes; i++) { axis[i] = 32768; }
        return 0;
    }
...
float joy_down_time (int btn) {
    ...
    if (btn >= JOY_TOTAL_BUTTONS ||
        (btn >= current->numButtons () && btn < JOY_NUM_BUTTONS)) {
        // Not a valid button
        return 0.f;
    }
...
int joy_down_count (int btn, int reset_count) {
    ...
    if (btn >= JOY_TOTAL_BUTTONS ||
        ...
        // Is hat
        if (current->numHats () > 0) {
            return current->getHatDownCount (
                0, hatBtnToEnum (btn), false, reset_count != 0);
        }
        else {
            // Don't have any hats to query
            return 0;
...
int joy_down (int btn) { return joy_down_time (btn) > 0.0f; }
