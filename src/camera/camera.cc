...
void camera::set_fov (
    float in_fov, float in_fov_time, float in_fov_acceleration_time,
    float in_fov_deceleration_time) {
    if (in_fov_time == 0.0f && in_fov_acceleration_time == 0.0f &&
        in_fov_deceleration_time == 0.0f) {
    ...
    fov.setAVD (
        in_fov, in_fov_time, in_fov_acceleration_time,
        in_fov_deceleration_time, 0.0f);
}

void camera::set_object_host (object* objp, int n_object_host_submodel) {
    if (objp == NULL) object_host = object_h ();
            ...
            while (ssp != END_OF_LIST (&Ships[objp->instance].subsys_list)) {
                if (ssp->system_info->subobj_num == n_object_host_submodel) {
                    if (ssp->system_info->type == SUBSYSTEM_TURRET) {
                        ...
                        set_custom_orientation_function (
                            get_turret_cam_orient);
                    }
                }
                ssp = GET_NEXT (ssp);
            }
...
void camera::set_object_target (object* objp, int n_object_target_submodel) {
    if (objp == NULL) object_target = object_h ();
 ...
 */
void camera::set_custom_position_function (
    void (*n_func_custom_position) (camera* cam, vec3d* camera_pos)) {
 ...
 * It should be replaced or added to as the custom function modifier sees fit.
 */
void camera::set_custom_orientation_function (
    void (*n_func_custom_orientation) (camera* cam, matrix* camera_ori)) {
    func_custom_orientation = n_func_custom_orientation;
}

void camera::set_position (
    vec3d* in_position, float in_translation_time,
    float in_translation_acceleration_time,
    float in_translation_deceleration_time, float in_end_velocity) {
    if (in_position != NULL && in_translation_time == 0.0f &&
    ...
    if (in_position == NULL) {
        pos_x.setVD (
            in_translation_time, in_translation_acceleration_time,
            in_end_velocity);
        pos_y.setVD (
            in_translation_time, in_translation_acceleration_time,
            in_end_velocity);
    ...
    pos_x.setAVD (
        in_position->xyz.x, in_translation_time,
        in_translation_acceleration_time, in_translation_deceleration_time,
        in_end_velocity);
    pos_y.setAVD (
        in_position->xyz.y, in_translation_time,
        in_translation_acceleration_time, in_translation_deceleration_time,
        in_end_velocity);
    pos_z.setAVD (
        in_position->xyz.z, in_translation_time,
        in_translation_acceleration_time, in_translation_deceleration_time,
        in_end_velocity);
}

void camera::set_translation_velocity (
    vec3d* in_velocity, float in_acceleration_time) {
    pos_x.setVD (
        in_acceleration_time, in_acceleration_time, in_velocity->xyz.x);
    pos_y.setVD (
        in_acceleration_time, in_acceleration_time, in_velocity->xyz.y);
    pos_z.setVD (
        in_acceleration_time, in_acceleration_time, in_velocity->xyz.z);
}

void camera::set_rotation (
    matrix* in_orientation, float in_rotation_time,
    float in_rotation_acceleration_time, float in_rotation_deceleration_time) {
    if (in_orientation != NULL && in_rotation_time == 0.0f &&
        in_rotation_acceleration_time == 0.0f &&
        in_rotation_deceleration_time == 0.0f) {
        c_ori = *in_orientation;
        for (int i = 0; i < 9; i++) ori[i].set (in_orientation->a1d[i]);
    ...
    if (in_orientation == NULL) {
        for (int i = 0; i < 9; i++)
            ori[i].setVD (
                in_rotation_time, in_rotation_acceleration_time, 0.0f);
        return;
    }

    for (int i = 0; i < 9; i++)
        ori[i].setAVD (
            in_orientation->a1d[i], in_rotation_time,
            in_rotation_acceleration_time, in_rotation_deceleration_time,
            0.0f);
}
...
void camera::set_rotation (
    ...
    this->set_rotation (
        &mtx, in_rotation_time, in_rotation_acceleration_time,
        in_rotation_deceleration_time);
}

void camera::set_rotation_facing (
    vec3d* in_target, float in_rotation_time,
    float in_rotation_acceleration_time, float in_rotation_deceleration_time) {
    ...
    set_rotation (
        &temp_matrix, in_rotation_time, in_rotation_acceleration_time,
        in_rotation_deceleration_time);
}
...
int camera::get_object_host_submodel () { return object_host_submodel; }
...
int camera::get_object_target_submodel () { return object_target_submodel; }

float camera::get_fov () {
    if (!(flags & CAM_STATIONARY_FOV)) { fov.get (&c_fov, NULL); }
...
void camera::get_info (vec3d* position, matrix* orientation) {
    ...
    if (!(flags & CAM_STATIONARY_POS) || object_host.IsValid ()) {
        ...
        if (object_host.IsValid ()) {
            ...
            if (model_num > -1) { pm = model_get (model_num); }
            ...
            if (object_host_submodel < 0 || pm == NULL) {
                ...
                if (eyep) {
                    ...
                    find_submodel_instance_point_normal (
                        &c_pos_in, &host_normal,
                        Ships[objp->instance].model_instance_num, eyep->parent,
                        &eyep->pnt, &eyep->norm);
                    ...
                    model_find_world_point (
                        &c_pos, &pt, pm->id, object_host_submodel,
    ...
    if (position != NULL) *position = c_pos;

    // ORIENTATION
    if (orientation != NULL) {
        bool target_set = false;
        if (!(flags & CAM_STATIONARY_ORI) || object_target.IsValid () ||
            object_host.IsValid ()) {
            if (object_target.IsValid ()) {
                ...
                // If we don't have a submodel or don't have the model use
                // object pos Otherwise, find the submodel pos as it is rotated
                if (object_target_submodel < 0 || pm == NULL) {
                    target_pos = objp->pos;
                }
                else {
                    model_find_world_point (
                        &target_pos, &vmd_zero_vector, pm->id,
                ...
                if (eyep) {
                    vm_vector_2_matrix (
                        &c_ori, &host_normal,
                        vm_vec_same (
                            &host_normal, &object_host.objp->orient.vec.uvec)
                            ? NULL
                            : &object_host.objp->orient.vec.uvec,
                        NULL);
                    target_set = true;
                }
                else {
...
//*************************warp_camera*************************
warp_camera::warp_camera () { this->reset (); }
...
void warp_camera::set_position (vec3d* in_pos) { c_pos = *in_pos; }

void warp_camera::set_rotation (matrix* in_ori) { c_ori = *in_ori; }
...
void warp_camera::set_velocity (vec3d* in_vel, bool instantaneous) {
    ...
    if (instantaneous) { c_vel = c_desired_vel; }
}
...
void warp_camera::do_frame (float in_frametime) {
    ...
    apply_physics (
        c_damping, c_desired_vel.xyz.x, c_vel.xyz.x, in_frametime,
        &new_vel.xyz.x, &delta_pos.xyz.x);
    apply_physics (
        c_damping, c_desired_vel.xyz.y, c_vel.xyz.y, in_frametime,
        &new_vel.xyz.y, &delta_pos.xyz.y);
    apply_physics (
        c_damping, c_desired_vel.xyz.z, c_vel.xyz.z, in_frametime,
        &new_vel.xyz.z, &delta_pos.xyz.z);
...
void warp_camera::get_info (vec3d* position, matrix* orientation) {
    ...
    if (orientation != NULL) *orientation = c_ori;
}
...
subtitle::subtitle (
    int in_x_pos, int in_y_pos, const char* in_text, const char* in_imageanim,
    float in_display_time, float in_fade_time, const color* in_text_color,
    int in_text_fontnum, bool center_x, bool center_y, int in_width,
    int in_height, bool in_post_shaded)
    : display_time (-1.0f), fade_time (-1.0f), text_fontnum (-1),
      time_displayed (-1.0f), time_displayed_end (-1.0f), post_shaded (false) {
    // Initialize color
    ...
    if (((in_text != NULL) && (strlen (in_text) <= 0)) &&
        ((in_imageanim != NULL) && (strlen (in_imageanim) <= 0)))
        return;
    ...
    if ((in_text != NULL) && (in_text[0] != '\0')) {
        ...
        num_text_lines = split_str (
            in_text, split_width, text_line_lens, text_line_ptrs,
            MAX_SUBTITLE_LINES);
    ...
    if (center_x || center_y) {
        ...
        // restore old font
        if (old_fontnum >= 0) { font::set_font (old_fontnum); }

        // Get image size
        if (image_id != -1) {
            bm_get_info (image_id, &w, &h);
            if (in_width > 0) w = in_width;
            if (in_height > 0) h = in_height;
        ...
        if (center_x)
            image_pos.x =
                (gr_screen.center_w - tw) / 2 + gr_screen.center_offset_x;
        if (center_y)
            image_pos.y =
                (gr_screen.center_h - th) / 2 + gr_screen.center_offset_y;
    }

    if (in_x_pos < 0 && !center_x)
        image_pos.x +=
            gr_screen.center_offset_x + gr_screen.center_w + in_x_pos;
    else if (!center_x)
        image_pos.x += gr_screen.center_offset_x + in_x_pos;

    if (in_y_pos < 0 && !center_y)
        image_pos.y +=
            gr_screen.center_offset_y + gr_screen.center_h + in_y_pos;
    else if (!center_y)
...
void subtitle::do_frame (float frametime) {
    ...
    for (std::vector< std::string >::iterator line = text_lines.begin ();
         line != text_lines.end (); ++line) {
    ...
    if (image_id >= 0) {
        gr_set_bitmap (
            image_id, GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL,
            text_color.alpha / 255.0f);
        ...
        if (image_pos.w > 0 || image_pos.h > 0) {
            ...
            scale.xyz.x =
                (image_pos.w > 0) ? (image_pos.w / (float)orig_w) : 1.0f;
            scale.xyz.y =
                (image_pos.h > 0) ? (image_pos.h / (float)orig_h) : 1.0f;
            scale.xyz.z = 1.0f;
...
subtitle::~subtitle () {
    if (image_id != -1) {
        if (bm_release (image_id)) { image_id = -1; }
    }
}
...
size_t camid::getIndex () { return idx; }

int camid::getSignature () { return sig; }

bool camid::isValid () {
    if (idx >= Cameras.size ()) return false;

    if (Cameras[idx] == NULL) return false;

    if (Cameras[idx]->get_signature () != this->sig) return false;
...
camid cam_create (
    const char* n_name, vec3d* n_pos, vec3d* n_norm, object* n_object,
...
camid cam_create (
    const char* n_name, vec3d* n_pos, matrix* n_ori, object* n_object,
    int n_object_host_submodel) {
    camera* cam = NULL;
    camid cid;
    ...
    // Set attributes
    if (n_pos != NULL) cam->set_position (n_pos);
    if (n_ori != NULL) cam->set_rotation (n_ori);
    if (n_object != NULL)
...
void cam_delete (camid cid) {
    if (cid.isValid ()) { cid.getCamera ()->clear (); }
}

void cam_do_frame (float frametime) {
    size_t i, size = Cameras.size ();
    for (i = 0; i < size; i++) {
        if (Cameras[i] != NULL) Cameras[i]->do_frame (frametime);
    }
}

camid cam_get_camera (uint idx) {
    if (idx >= Cameras.size ()) return camid ();

    return camid (idx, Cameras[idx]->get_signature ());
}

camid cam_get_current () { return Current_camera; }

size_t cam_get_num () { return Cameras.size (); }
    ...
    while (ssp != END_OF_LIST (&shipp->subsys_list)) {
        if (ssp->system_info->subobj_num == cam->get_object_host_submodel ()) {
            ship_get_global_turret_gun_info (
                cam->get_object_host (), ssp, pos, &normal_cache, 1, NULL);
            vec3d offset = vmd_zero_vector;
            offset.xyz.x = 0.0001f;
            vm_vec_add2 (
                pos,
                &offset); // prevent beam turrets from crashing with a nullvec
            break;
        }
        ssp = GET_NEXT (ssp);
    }
...
void get_turret_cam_orient (camera* cam, matrix* ori) {
    ...
    vm_vector_2_matrix (
        ori, &normal_cache,
        vm_vec_same (&normal_cache, &cam->get_object_host ()->orient.vec.uvec)
            ? NULL
            : &cam->get_object_host ()->orient.vec.uvec,
        NULL);
}
...
eye* get_submodel_eye (polymodel* pm, int submodel_num) {
    if (pm->n_view_positions > 0) {
        for (int i = 0; i < pm->n_view_positions; ++i) {
            ...
            while (sm > 0) // look for eyepoints attached to children,
                           // important for turret arms
            {
