...
std::string dc_command_str; //!< The entered command line, arguments and all.
                            //!< Is progressively culled from the left as
                            //!< commands, arguments are parsed in DCF's

// Misc
bool debug_inited = FALSE;
uint lastline = 0; // Number of lines written to the console since the last
...
size_t lastwhite = 0; // Last whitespace character encountered, used by putc
                      // for 'true' word wrapping
ubyte DTABS = 4;      //!< Tab size in spaces

/**
 * Human readable versions of the dc_token's. Primarily used in error diagnosis
 */
static const char* token_str[DCT_MAX_ITEMS] = {
    "nothing",          "string", "float",         "integer",
    "unsigned integer", "byte",   "unsigned byte", "boolean"
};
...
std::deque< std::string >::iterator
    last_oldcommand; // Iterator to the last old command. Is reset to the start
                     // every new command push.

const char dc_prompt[] = "> "; // The prompt c_str
std::string dc_command_buf;    // The command line as shown in the console.
                            // Essentially an input buffer for dc_command_str
...
void dc_do_command (std::string* cmd_str) {
     ...
     *  Process the command...
     *      Call the function to process the command (the rest of the command
     * line is in the parser) Function takes care of long_help and status
     * depending on the mode.
     */
    int i;
    std::string command;
    extern debug_command* dc_commands[]; // z64: I don't like this extern here,
    ...
    try {
        ...
        dc_printf (
            "Invalid argument. Expected %s, found '%s'\n",
            token_str[err.expected_type], err.found_token.c_str ());
    }

    // dc_maybe_stuff_string is vulnerable to overflow. Once the
    // errParseOverflow throw class (or w/e) gets implemented, this last
    // command should be put into its own try{} catch{} block.
...
void dc_draw (bool show_prompt = FALSE) {
    ...
    gr_string (
        (gr_screen.clip_width - w) / 2, 3, dc_title.c_str (), GR_RESIZE_NONE);
...
void dc_draw_window (bool show_prompt) {
    ...
    if (show_prompt) { buffer_lines = DROWS - cmd_lines; }
    else {
        buffer_lines = DROWS;
    }
    ...
    if (dc_buffer.size () >= buffer_lines) {
        dc_scroll_y =
            MIN (dc_scroll_y, (uint32_t) (dc_buffer.size () - buffer_lines));
    }
    else {
        dc_scroll_y =
            0; // Disallow vscroll until the buffer is larger than the window
    }
        ...
        if ((i + dc_scroll_y) < dc_buffer.size ()) {
            gr_string (
                ...
                gr_screen.center_offset_y +
                    (((int)i * row_height) + row_height),
                dc_buffer[i + dc_scroll_y].substr (dc_scroll_x).c_str (),
                GR_RESIZE_NONE);
        }
    }
    ...
    if (show_prompt) {
        ...
        for (str_it = out_str.begin (); str_it < out_str.end (); ++str_it) {
            if (j == (DCOLS - 1)) {
                // Insert a newline char at every place the string needs to
                // return the 'carriage'
        ...
        gr_string (
            gr_screen.center_offset_x,
            gr_screen.center_offset_y + (((int)i * row_height) + row_height),
            out_str.c_str (), GR_RESIZE_NONE);
...
void dc_init (void) {
    ...
    gr_get_string_size (
        &col_width, nullptr, " "); // Col/Character width, in pixels

    dc_scroll_x = 0;
    dc_scroll_y = 0;
    DCOLS = (gr_screen.center_w / col_width) -
            1; // Subtract as needed. Windowed mode has some quirks with the
               // resolution
...
bool dc_pause_output (void) {
    ...
    do {
        ...
        key = key_inkey ();
        switch (key) {
        case KEY_ESC: return true; break;

        case KEY_PAGEUP:
            if (dc_scroll_y > 1) { dc_scroll_y--; }
            break;

        case KEY_PAGEDOWN:
            if (dc_scroll_y < DBROWS) { dc_scroll_y++; }
            else {
                dc_scroll_y = DBROWS;
            }
            break;
...
void dc_putc (char c) {
    ...
    if (c == ' ') {
         ...
         * subsequent block handle it, Else, push the space onto the line and
         * bail
         */
        temp_str = *line_str;
    ...
    if (c == '\t') {
         ...
         * If we run out of room on the line, or
         * If we run out of room on the screen, change c to a '\n' and let
         * subsequent block handle it, Else, copy temp_str onto the line,
         * update the lastwhite index, and bail
         */
    ...
    if (c == '\n') {
         ...
         * bail
         */
        if (line_str->size () > DBCOLS) { line_str->resize (DBCOLS); }
        dc_buffer.push_back ("");
        ...
        while (dc_buffer.size () > DBROWS) { dc_buffer.pop_front (); }
    ...
    if ((temp_str.size () >= DBCOLS) || (w > gr_screen.center_w)) {
        /**
         * Word wrapping
         * Save the word, clear the line of the word, push new line with the
         * word on it Update scroll_y, if needed, Pop off old lines, and
         * finally Push new character onto the new line
         */
...
void debug_console (void (*_func) (void)) {
    ...
    while (!done) {
        ...
        case KEY_DOWN:
            if (last_oldcommand > dc_history.begin ()) { --last_oldcommand; }
        ...
        case KEY_PAGEDOWN:
            if (dc_scroll_y < (DBROWS - DROWS)) { dc_scroll_y++; }
            else {
        ...
        case KEY_ENTER:
            dc_scroll_y =
                (DBROWS - DROWS); // Set the scroll to look at the bottom
            ...
            // z64: Thread Note: Maybe lock a mutex here to allow a previous
            // DCF to finish/abort before starting a new one z64: We'll just
            // assume we won't be here unless a command has finished...
            dc_history.push_front (
                dc_command_buf); // Push the command onto the history queue
            last_oldcommand = dc_history.begin (); // Reset oldcommand

            while (dc_history.size () > DCMDS) {
                dc_history.pop_back (); // Keep the commands less than or equal
                                        // to DCMDS
            }

            dc_command_str =
                dc_command_buf; // Xfer to the command string for processing
            dc_command_buf.resize (0); // Nullify the buffer
            dc_printf (
                "%s%s\n", dc_prompt,
                dc_command_str.c_str ()); // Print the command w/ prompt.
            dc_draw (FALSE); // Redraw the console without the command line.
            dc_do_command (&dc_command_str); // Try to do the command
            break;

        default:
            // Not any of the control key codes, so it's probably a letter or
            // number.
