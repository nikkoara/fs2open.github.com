...
// 1999. You may not sell or otherwise commercially exploit the source or
// things you created based on the source.
//
// This file contains documentation on locally referenced functions. For
// documentation on globally referenced functions, see consoleparse.h
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

/////////////////
// @todo Make a fast version of the parse_long, parse_ulong, etc. That just
// checks the first 1-3 characters. The fast version will be used in retail
// builds while the slow/safe version will be in debug's
 ...
 *
 * @details Similar in operation to dc_stuff_string_white, but won't throw any
 * error messages and will only grab the first word. (dc_stuff_string_white may
 * grab quoted strings)
 */
...
/**
 * @brief Parses a double-precision floating point type. Supports, Whole,
 * Fractional, and Mixed numbers, and supports scientific notation (exponent
 * prefixed by 'e' or 'E').
 *
 * @param[in] ch   Points to the start of the string to parse
 * @param[in] type The expected type. is thrown along with ch when an
 * unexpected/malformed float is found
 *
 * @return The value of the parsed token
 * @details
 *   The returned double may be cast to a single-precision float, but be sure
 * to check it before doing so!
 */
double dc_parse_double (const char* ch, dc_token type);

/**
 * @brief Parses a long integral type. Supports decimal, binary, octal, and
 * hexidecimal strings.
 *
 * @param[in] ch    Points to the start of the string to parse.
 * @param[in] type  The expected type. Is thrown along with ch when an
 * unexpected/malformed integral is found
 *
 * @details
 * ! Non-decimal values must be prefixed by their corresponding sequence.
 * Binary: "0b", Octal: "0o", Hex: "0x"
 *
 *   The returned long may be cast to a smaller integral, but be sure to check
 * it before doing so!
 *
 *   The only thing left making this function specific to the DC is the
 * expected type. So, if you want to use this for parsing something other than
 * the debug CL, you'll have to make a set of errParse classes that take a
 *   different expected type.
 */
long dc_parse_long (const char* ch, dc_token type);

/**
 * @brief Parses an unsigned long integral type. Supports decimal, binary,
 * octal, and hexidecimal strings.
 *
 * @param[in] ch    Points to the start of the string to parse.
 * @param[in] type  The expected type. Is thrown along with ch when an
 * unexpected/malformed integral is found
 *
 * @details
 * ! Non-decimal values must be delimited by their corresponding sequence.
 * Binary: "0b", Octal: "0o", Hex: "0x" The returned long may be cast to a
 * smaller integral, but be sure to check it before doing so! The only thing
 * left making this function specific to the DC is the expected type. So, if
 * you want to use this for parsing something other than the debug CL, you'll
 * have to make a set of errParse classes that take a different expected type.
 */
ulong dc_parse_ulong (const char* ch, dc_token type);

// State processes for dc_parse_double
state_float
dc_parse_double_sign (const char*& ch_ptr, std::string& buffer_str);
state_float
dc_parse_double_whole (const char*& ch_ptr, std::string& buffer_str);
state_float
dc_parse_double_decimal (const char*& ch_ptr, std::string& buffer_str);
state_float
dc_parse_double_fraction (const char*& ch_ptr, std::string& buffer_str);
state_float
dc_parse_double_expprefix (const char*& ch_ptr, std::string& buffer_str);
state_float
dc_parse_double_expsign (const char*& ch_ptr, std::string& buffer_str);
state_float
dc_parse_double_exponent (const char*& ch_ptr, std::string& buffer_str);

// State processes for dc_parse_long and dc_parse_ulong.
state_int
dc_parse_long_prefix (const char*& ch_ptr, std::string& buffer_str, int& base);
...
state_int
dc_parse_long_numeral_bin (const char*& ch_ptr, std::string& buffer_str);
state_int
dc_parse_long_numeral_hex (const char*& ch_ptr, std::string& buffer_str);
state_int
dc_parse_long_numeral_octal (const char*& ch_ptr, std::string& buffer_str);

// ============================== IMPLEMENTATIONS =============================
inline bool ch_is_sign (char ch) { return ((ch == '-') || (ch == '+')); }

inline bool ch_is_numeral (char ch) { return ((ch >= '0') && (ch <= '9')); }

inline bool ch_is_decimal (char ch) { return (ch == '.'); }

inline bool ch_is_binary (char ch) { return ((ch == '0') || (ch == '1')); }

inline bool ch_is_octal (char ch) { return ((ch >= '0') && (ch <= '7')); }

inline bool ch_is_hex (char ch) {
    return (
        ((ch >= '0') && (ch <= '9')) || ((ch >= 'a') && (ch <= 'f')) ||
        ((ch >= 'A') && (ch <= 'F')));
};

inline bool ch_is_prefix (char ch) { return (ch == '0'); }

inline bool ch_is_binary_prefix (char ch) {
    return ((ch == 'b') || (ch == 'B'));
}

inline bool ch_is_octal_prefix (char ch) {
    return ((ch == 'o') || (ch == 'O'));
}

inline bool ch_is_hex_prefix (char ch) { return ((ch == 'x') || (ch == 'X')); }

inline bool ch_is_exp_prefix (char ch) { return ((ch == 'e') || (ch == 'E')); }
...
void dc_get_token (std::string& out_str) {
    ...
    // Scan the string, stopping at grayspace, null terminator, or before we go
    // over MAX_TOKEN_LENGTH
    c_ptr = Cp;
...
void dc_ignore_white_space (void) {
    while (is_white_space (*Cp) && (*Cp != '\0')) { Cp++; }
}

void dc_ignore_gray_space (void) {
    while (is_gray_space (*Cp) && (*Cp != '\0')) { Cp++; }
}
...
bool dc_maybe_stuff_string (char* out_str, size_t maxlen) {
    ...
    // Bail if overflow
    if (count == maxlen) { throw errParse ("", DCT_STRING); }
...
bool dc_maybe_stuff_string (std::string& out_str) {
    ...
    // Bail if overflow
    if (count == out_str.max_size ()) { return false; }
...
void dc_required_string (char* pstr) {
    ...
    if (strncmp (pstr, Cp, strlen (pstr)) == 0) { str_found = pstr; }
...
uint dc_required_string_any (const uint n, ...) {
    ...
    // Populate the vector.
    va_start (vl, n);
    for (i = 0; i < n; ++i) { strings.push_back (va_arg (vl, char*)); }
    va_end (vl);
...
bool dc_optional_string_either (const char* str1, const char* str2) {
    ...
    if (strncmp (str1, Cp, strlen (str1)) == 0) { str_found = str1; }
    else if (strncmp (str2, Cp, strlen (str2)) == 0) {
        str_found = str2;
    }
    else {
...
double dc_parse_double (const char* ch, dc_token type) {
    ...
    while ((*ch_ptr != '\0') && is_white_space (*ch_ptr)) { ch_ptr++; }
    ...
    do {
        ...
        case sf_start:
            if (ch_is_sign (*ch_ptr)) {
                ...
                if (Dc_debug_on) {
                    dc_printf (
                        "<debug> [parse_double] Invalid character '%c' found "
                        "in sf_start\n",
                        *ch_ptr);
                }
                state = sf_invalid;
        ...
        case sf_end:
            ...
            break;
        case sf_sign: state = dc_parse_double_sign (ch_ptr, buffer_str); break;
        ...
        case sf_invalid:
        default: throw errParse (ch, type);
        }
    } while (state != sf_end);
...
dc_parse_double_sign (const char*& ch_ptr, std::string& buffer_str) {
    ...
    if (ch_is_numeral (*ch_ptr)) {
        ...
        if (Dc_debug_on) {
            dc_printf (
                "<debug> [parse_double] Invalid character '%c' found in "
                "sf_sign\n",
                *ch_ptr);
        }
    }
    ...
    do {
        ...
        case si_start:
            if (ch_is_sign (*ch_ptr)) {
                ...
                if (*ch_ptr == '-') {
                    buffer_str.push_back (*ch_ptr);
                } // Else, it's positive. Positive sign isn't needed for
                  // conversion
                ch_ptr++;
            }
            else if (ch_is_prefix (*ch_ptr)) {
                // prefixes must be checked before numeral, because they all
                // start with a numeral zero
                state = si_prefix;
                ch_ptr++;
            }
            else if (ch_is_numeral (*ch_ptr)) {
                ...
                if (Dc_debug_on) {
                    dc_printf (
                        "<debug> [parse_long] Invalid character '%c' found in "
                        "si_start\n",
                        *ch_ptr);
                }
                state = si_invalid;
        ...
        case si_sign: state = dc_parse_long_sign (ch_ptr, buffer_str); break;
        ...
        case si_invalid:
        default: throw errParse (ch, type); break;
        }
    } while (state != si_end);

    ret = strtol (buffer_str.c_str (), &end_ptr, base);

    // This last check can be omitted once I can verify the operation of strtol
    // in this sense
    if (*end_ptr != '\0') {
        dc_printf (
...
void dc_stuff_int (int* i) {
    ...
    if ((value_l < INT_MAX) && (value_l > INT_MIN)) { *i = value_l; }
    else {
...
void dc_stuff_ubyte (ubyte* i) {
    ...
    // Since some system's chars may be greater than 1 byte, we can't use
    // UCHAR_MAX for a UBYTE
    if (value_ul <= 255) { *i = static_cast< ubyte > (value_ul); }
    else {
...
void dc_stuff_string (char* out_str, size_t maxlen = MAX_TOKEN_LENGTH) {
    ...
    // Bail if we're at the terminator
    if (*Cp == '\0') { throw errParse ("Nothing!", DCT_STRING); }
...
void dc_stuff_string_white (char* out_str, size_t maxlen) {
    ...
    // Scan the string, stopping at grayspace, null terminator, or before we
    // overflow
    c_ptr = Cp;
...
void dc_stuff_string_white (std::string& out_str) {
    ...
    c_ptr = Cp;
    while (!is_gray_space (*c_ptr) && (*c_ptr != '\0') &&
           (count < out_str.max_size ())) {
        count++;
        c_ptr++;
    }

    // Bail if overflow
    if (count == out_str.max_size ()) { throw errParse ("", DCT_STRING); }
...
inline state_int dc_parse_long_prefix (
    const char*& ch_ptr, std::string& buffer_str, int& base) {
    ...
    if (ch_is_binary_prefix (*ch_ptr)) {
        ...
        // Just ignore the first 0, if the user passed something like 00001
        // then state si_numeral can handle it
        state = si_numeral;
        ch_ptr++;
    }
    else if ((*ch_ptr == '\0') || is_white_space (*ch_ptr)) {
...
inline state_int
dc_parse_long_sign (const char*& ch_ptr, std::string& buffer_str) {
    state_int state;
    if (ch_is_numeral (*ch_ptr)) {
        state = si_numeral;
        buffer_str.push_back (*ch_ptr);
...
inline state_int
dc_parse_long_numeral (const char*& ch_ptr, std::string& buffer_str) {
