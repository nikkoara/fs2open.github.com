...
/**
 * @file console.h
 * @brief An overhauled/updated debug console to allow monitoring, testing, and
 * general debugging of new features.
 *
 * @details
 * Of key interest is Volition's DCF macro, which adds the function argument to
 * the available command list in the debug console. These functions may be
 * defined in the .cpp file that they are related to, but it is recommended
 * that they be in their own .cpp if they have multiple sub-arguments (ex: Git
 ...
 * @def DCF
 *
 * @brief The potent DCF macro, used to define new debug commands for the
 * console.
 *
 * @param function_name[in] The name of the function, as shown in the debug
 * console
 * @param help_txt[in] The short-help text, as shown as listed from the 'help'
 * command
 *
 * @details
 * Each DCF is responsible for getting data from the debug console's command
 * line. The parsing functions for the command line have been set up to have
 * similar syntax and usage as the parselo commands. (see consoleparse.h) Most,
 * if not all, argument and subcommand strings should be parsed by the
 * dc_optional_string functions. Usage example: DCF(toggle_it, "description")
 * {
 *     if (dc_optional_string_either("help", "--help")) {
 *         dc_printf("Usage: sample. Toggles This_var on/off\n");
 *
 *     } else if (dc_optional_string_either("status", "--status") ||
 * dc_optional_string_either("?", "--?")) { dc_printf("This_var is %s\n",
 * (This_var ? "ON" : "OFF"));
 *
 *     } else {
 *         This_var = !This_var;
 ...
 * }
 *
 *  In the console, the command would be listed as 'toggle_it', and dcf_help
 ...
 *
 *  @param [in] function_name Name of the function, as shown in the debug
 * console
...
/**
 *  @class debug_command
 *  @brief Class to aggregate a debug command with its name (as shown in the
 * console) and short help.
 *
 *  @details
 *  Note: Long help, as evoked by '<command> help', should be handled by the
 * function itself. It is recommended that arguments that have sub-arguments be
 * in their own function, so as to aide in organization and to keep the size of
 * the command function down.
 */
class debug_command {
public:
    const char*
...
extern std::string dc_command_str; // The rest of the command line, from the
                                   // end of the last processed arg on.

/**
 * @brief   Pauses the output of a command and allows user to scroll through
 * the output history.
 *
 ...
 */
void dc_printf (SCP_FORMAT_STRING const char* format, ...)
