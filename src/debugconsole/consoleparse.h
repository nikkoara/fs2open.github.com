...
/**
 * @file consoleparse.h
 * @brief Parsing functions for the command line. Previously known as the
 ...
 *
 *  @details This is a basic parser error, it contains the token (a single
 * word) that was found, and the expected token type. Some DCT's, such as the
 * DCT_STRING's, have their own specific requirements for error handling, and
 * as such get their own class derived from errParse. The catching routines
 * should be able to catch the derived error objects, but if not, they can be
 * caught by a routine looking for the base class and then be casted to their
 *      proper type.
 */
class errParse {
public:
 ...
 *
 * @brief Class thrown when an expected string was not found. Can/should
 * contain all of the expected strings.
 */
 ...
 *
 * @brief Class thrown when the parsed string or token could not be stuffed
 * into the smaller destination container
 *
...
/**
 * @brief Searches for a specified required string, throwing an errParse if not
 * found.
 *
 ...
 *
 * @returns The index of the specified required strings (as if they were an
 * array)
 *
 * @throws errParseString with the required strings
 */
 ...
 *
 * @throws errParse if an unexpected or otherwise malformed float string is
 * found.
 ...
 * hexadecimal (0x), and octal (0o) formats
 *
 * @param[in] i  The int variable to stuff to
 *
 * @details The binary, hexadecimal, and octal formats must be prefixed by
 * their associated string. Ex: "0xDEADBEEF" would be parsed properly while
 * "DEADBEEF" would throw an error
 *
 * @throws errParse if an unexpected or otherwise malformed float string is
 * found.
 * @throws errParse if nothing was found
 ...
 *
 * @details The binary, hexadecimal, and octal formats must be prefixed by
 * their associated string. Ex: "0x0F" would be parsed properly while "0F"
 ...
 *
 * @details Supports a number of literal strings as true and false, including
 * "true", "false", "yes", "no" and the yes/no equivalents in other languages
 * supported in localization.
 ...
 * @param[in]  maxlen   Maximum length to copy. (maxlen <= sizeof(str)) &&
 ...
 * line, stopping at the end of the command line
 *
 * @param[out] str  Destination string
 * @param[in]  len   Maximum length to copy. (len <= sizeof(str)) && (len <=
 * MAX_TOKEN_LENGTH)
 *
 * @throws errParse when nothing left was found on the command line
 * @throws errParseOverflow when parser cannot stuff the entirety of the found
 ...
 *
 *   If there's something on the command line, and we're able to convert it, *f
 * = the converted value, true is returned, and the parser is advanced past the
 * token
 *
 ...
 *
 *   If there's something on the command line, and we're able to convert it, *i
 * = the converted value, true is returned, and the parser is advanced past the
 * token
 *
 *   If there's something on command line, but we can't convert it, an errParse
 ...
 *
 *   If there's something on the command line, and we're able to convert it, *b
 * = the converted value, true is returned, and the parser is advanced past the
 * token
 *
 *   If there's something on command line, but we can't convert it, an errParse
 ...
 * command line, stopping at the end of the command line
 *
 * @param[out] str  Destination string. If nothing was found, str = ""
 * @param[in]  len   Maximum length to copy. (maxlen <= sizeof(str)) && (maxlen
 * <= MAX_TOKEN_LENGTH)
 *
 * @retval true if the operation was successful,
 * @retval false otherwise
