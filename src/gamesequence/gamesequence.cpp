...
static int state_processing_event_post =
    0; // set if we are already processing an event to switch states
...
int gameseq_get_event () {
    ...
    if (gs[gs_current_stack].queue_head == gs[gs_current_stack].queue_tail)
        return -1;
    event =
        gs[gs_current_stack].event_queue[gs[gs_current_stack].queue_head++];
...
bool GameState_Stack_Valid () { return (gs_current_stack != -1); }
...
int gameseq_get_depth () { return gs_current_stack; }

void gameseq_set_state (int new_state, int override) {
    int event, old_state;

    if ((new_state == gs[gs_current_stack].current_state) && !override) return;
...
void gameseq_push_state (int new_state) {
    if (new_state == gs[gs_current_stack].current_state) return;
...
void gameseq_pop_state () {
    ...
    if (gs_current_stack >= 1) {
        ...
        // set the old state to be the state which is about to be popped off
        // the queue
        old_state = gs[gs_current_stack].current_state;
        ...
        // swap all remaining events from the state which just got popped to
        // this new state
        while (gs[gs_current_stack + 1].queue_head !=
               gs[gs_current_stack + 1].queue_tail) {
            gameseq_post_event (
                gs[gs_current_stack + 1]
                    .event_queue[gs[gs_current_stack + 1].queue_head++]);
        }
...
// gameseq_pop_and_discard_state() is used to remove a state that was pushed
// onto the stack, but will never need to be popped.  An example of this is
// entering a state that may require returning to the previous state (then you
// would call gameseq_pop_state).  Or you may simply continue to another part
// of the game, to avoid filling up the stack with states that may never be
// popped, you call this function to discard the top of the gs.
//

void gameseq_pop_and_discard_state () {
    if (gs_current_stack > 0) { gs_current_stack--; }
}
...
int gameseq_process_events () {
    ...
    while ((event = gameseq_get_event ()) != -1) {
        ...
        if (old_state != gs[gs_current_stack].current_state) break;
    }
...
int gameseq_get_state_idx (int state) {
    for (int i = 0; i <= gs_current_stack; i++) {
        if (gs[i].current_state == state) { return i; }
    }
