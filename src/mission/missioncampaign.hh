...
// types of campaigns -- these defines match the string literals listed below
// which are found in the campaign files.  I don't think that we need campaigns
// for furball missions.
...
// Goober5000 - Bastion flag is not needed anymore; there can now be more than
// two main halls; but the flag is kept in order to maintain compatibility with
// older campaigns
...
#define CMISSION_FLAG_HAS_FORK                                                 \
        (1 << 3) // campaign fork, e.g. Scroll or BWO (mutually exclusive with
                 // loop)
...
// structure for a campaign definition.  It contains the mission names and
...
public:
        ...
        char briefing_cutscene[NAME_LENGTH]; // name of the cutscene to be played
                                             // before this mission
        ...
        mgoal *goals;                        // malloced array of mgoals (of num_goals size) which has the
                                             // goal completion status
        int num_events;                      // number of events this mission had
        mevent *events;                      // malloced array of mevents (of num_events size) which has
                                             // event completion status
        int num_variables;                   // number of variables this mission had - Goober5000
        sexp_variable *
                variables;         // malloced array of sexp_variables (of num_variables size)
                                   // containing mission-persistent variables - Goober5000
        int mission_loop_formula;  // formula to determine whether to allow a side
                                   // loop
        ...
        int flags;
        std::string main_hall;       // which main hall the player is in - converted to
                                     // std::string by CommanderDJ
        ubyte debrief_persona_index; // which persona is used for ranks/badges -
                                     // Goober5000
...
public:
        ...
        int num_missions_completed;            // number of missions in the campaign that have
                                               // been flown
        int current_mission;                   // the current mission that the player is playing.
                                               // Only valid during the mission
        int next_mission;                      // number of the next mission to fly when comtinuing the
                                               // campaign.  Always valid
        int prev_mission;                      // mission that we just came from.  Always valid
        int loop_enabled;                      // whether mission loop is chosen - true during a loop,
                                               // false otherwise
        ...
        int num_players;                       // valid in multiplayer campaigns -- number of players
                                               // campaign supports.
        ubyte ships_allowed[MAX_SHIP_CLASSES]; // which ships the player can use
        ubyte
                weapons_allowed[MAX_WEAPON_TYPES]; // which weapons the player can use
        cmission missions[MAX_CAMPAIGN_MISSIONS];  // decription of the missions
        std::vector< sexp_variable >
                persistent_variables; // These variables will be saved at the end of a
                                      // mission
        std::vector< sexp_variable >
                red_alert_variables; // state of the variables in the previous mission
                                     // of a Red Alert scenario.

        campaign() : desc(nullptr), num_missions(0)
        {
                name[0] = 0;
                filename[0] = 0;
        }
...
int mission_campaign_load(
        char *filename, player *pl = NULL, int load_savefile = 1,
        bool reset_stats = true);

bool campaign_is_ignored(const char *filename);

// function to save the state of the campaign between missions or to load a
// campaign save file
...
// function which sets up internal variable for player to play next mission in
// the campaign
...
void mission_campaign_build_list(
        bool desc = false, bool sort = true, bool multiplayer = false);
...
void mission_campaign_store_variables(
        int persistence_type, bool store_red_alert = true);
...
// End CSFE stuff
#endif // FREESPACE2_MISSION_MISSIONCAMPAIGN_HH
