...
#define TRAINING_TIMING_BASE \
    1000 // Minimum milliseconds to display any message
...
#define TRAINING_OBJ_DISPLAY_LINES \
    5 // only display this many lines on screen max
...
const char* Training_lines[MAX_TRAINING_MESSAGE_LINES]; // Training message
                                                        // split into lines
...
int Training_message_window_coords[GR_NUM_RESOLUTIONS][2] = { { 174, 40 },
                                                              { 379, 125 } };

// coordinates for "directives" window on hud
int Training_obj_window_coords[GR_NUM_RESOLUTIONS][2] = { { 0, 187 },
                                                          { 0, 287 } };
...
HudGaugeDirectives::HudGaugeDirectives ()
    : HudGauge (
          HUD_OBJECT_DIRECTIVES, HUD_DIRECTIVES_VIEW, false, true,
...
void HudGaugeDirectives::initTextHeight (int h) { text_h = h; }

void HudGaugeDirectives::initMaxLineWidth (int w) { max_line_width = w; }

void HudGaugeDirectives::initBottomBgOffset (int offset) {
    bottom_bg_offset = offset;
}

void HudGaugeDirectives::initBitmaps (
...
bool HudGaugeDirectives::canRender () {
    ...
    if (hud_disabled_except_messages ()) { return false; }

    if (hud_disabled () && !hud_disabled_except_messages ()) { return false; }

    // force the directives list to display in training missions even if this
    // gauge isn't active.
...
void HudGaugeDirectives::pageIn () {
    bm_page_in_aabitmap (
        directives_top.first_frame, directives_top.num_frames);
    bm_page_in_aabitmap (
        directives_middle.first_frame, directives_middle.num_frames);
    bm_page_in_aabitmap (
        directives_bottom.first_frame, directives_bottom.num_frames);
}
...
void HudGaugeDirectives::render (float /*frametime*/) {
    ...
    if (!Training_obj_num_lines) { return; }
    ...
    renderPrintf (
        ...
        EG_OBJ_TITLE, "%s", XSTR ("directives", 422));
...
void training_mission_init () {
    ...
    for (i = 0; i < TRAINING_OBJ_LINES; i++) Training_obj_lines[i] = -1;
    ...
    if (The_mission.game_type & MISSION_TYPE_TRAINING) {
        Player->flags &=
            ~(PLAYER_FLAGS_MATCH_TARGET | PLAYER_FLAGS_MSG_MODE |
              PLAYER_FLAGS_AUTO_TARGETING | PLAYER_FLAGS_AUTO_MATCH_SPEED |
              PLAYER_FLAGS_LINK_PRIMARY | PLAYER_FLAGS_LINK_SECONDARY);
    }
}
...
void training_mission_page_in () {
    ...
    for (i = 0; i < NUM_DIRECTIVE_GAUGES; i++) {
        bm_page_in_aabitmap (
            Directive_gauge[i].first_frame, Directive_gauge[i].num_frames);
    }
}
...
int comp_training_lines_by_born_on_date (const void* m1, const void* m2) {
    ...
    return (
        Mission_events[*e1 & 0xffff].born_on_date -
        Mission_events[*e2 & 0xffff].born_on_date);
}

/**
 * Sort list of training events
 *
 * Sort on EVENT_CURRENT and born on date, for other events (EVENT_SATISFIED,
 * EVENT_FAILED) sort on born on date
 */
...
void sort_training_objectives () {
    ...
    insertion_sort (
        Training_obj_lines, Training_obj_num_lines, sizeof (int),
        comp_training_lines_by_born_on_date);
    ...
    // go through lines 0 to offset-1 and check if there are any CURRENT or
    // RECENTLY_KNOWN events that should be shown
    ...
    for (i = 0; i < offset; i++) {
        ...
        if (event_status == EVENT_CURRENT) {
            ...
            if (f2i (
                    Missiontime - Mission_events[TRAINING_OBJ_LINES_MASK (i)]
                                      .satisfied_time) < MIN_SATISFIED_TIME) {
                Training_obj_lines[i] |= TRAINING_OBJ_STATUS_UNKNOWN;
                num_offset_events++;
            }
            else {
            ...
            if (f2i (
                    Missiontime - Mission_events[TRAINING_OBJ_LINES_MASK (i)]
                                      .satisfied_time) < MIN_FAILED_TIME) {
                Training_obj_lines[i] |= TRAINING_OBJ_STATUS_UNKNOWN;
                num_offset_events++;
            }
            else {
    ...
    if (num_offset_events == 0) { return; }

    // go through lines offset to Training_obj_num_lines to check which should
    ...
    for (i = 0; i < num_offset_events; i++) {
        ...
        for (int j = slot_idx; j > 0; j--) {
            Training_obj_lines[j + offset - 1] =
                Training_obj_lines[j + offset - 2];
        }
...
void training_check_objectives () {
    ...
    for (event_idx = 0; event_idx < Num_mission_events; event_idx++) {
        ...
        if ((event_status != EVENT_UNBORN) &&
            ...
            if (!Training_failure || !strncasecmp (
                ...
                for (i = 0; i < Training_obj_num_lines; i++) {
                    if (Training_obj_lines[i] == event_idx) { break; }
                }
                ...
                for (i = 0; i < Training_obj_num_lines; i++) {
                    if (Training_obj_lines[i] ==
                        (event_idx | TRAINING_OBJ_LINES_KEY)) {
                        break;
                    }
                }

                // if there is a keypress message with directive, process that
                // too.
                if (Mission_events[event_idx].objective_key_text) {
                    if (event_status == EVENT_CURRENT) {
                        ...
                        if (i == Training_obj_num_lines) {
                            if (Training_obj_num_lines == TRAINING_OBJ_LINES) {
                                // objective list full, remove topmost
                                // objective
                                for (i = 1; i < Training_obj_num_lines; i++) {
                                    Training_obj_lines[i - 1] =
                                        Training_obj_lines[i];
                                }
                                Training_obj_num_lines--;
                            }
                            // mark objective as having key text
                            Training_obj_lines[Training_obj_num_lines++] =
                                event_idx | TRAINING_OBJ_LINES_KEY;
                        }
                    }
                    else {
                        // message with key press text is no longer valid, so
                        // remove it
                        if (i < Training_obj_num_lines) {
    ...
    // sort on EVENT_CURRENT and born on date, for other events
    // (EVENT_SATISFIED, EVENT_FAILED) sort on born on date
...
/**
 * Translates all special tokens in a message, producing the new finalized
    ...
    *buf = 0;
    ...
    while (toke1 || toke2) { // is either token types present?
        if (!toke2 || (toke1 && (toke1 < toke2))) { // found $ before #
            ...
            toke2 = strchr (text, '$');
            if (!toke2 ||
                ((toke2 - text) == 0)) // No second one, or possibly a double?
                break;

            // make sure we aren't going to have any type of out-of-bounds
            // issues
            if (((toke2 - text) < 0) ||
                ...
                strncpy (
                    temp, text,
                    toke2 - text);      // isolate token into seperate buffer
                ...
                if (ptr) {                  // was key translated properly?
                    if (!strcasecmp (ptr, NOX ("none")) &&
                        ...
                        if (The_mission.game_type & MISSION_TYPE_TRAINING) {
                            r = popup (
                                PF_TITLE_BIG | PF_TITLE_RED, 2,
                                XSTR ("&Bind Control", 424),
                                XSTR ("&Abort mission", 425),
                                XSTR (
                                    "Warning\nYou have no control bound to "
                                    "the action \"%s\".  You must do so "
                                    "before you can continue with your "
                                    "training.",
                                    426),
                                XSTR (
                                    Control_config[Failed_key_index].text,
                            ...
                            gameseq_post_event (
                                GS_EVENT_CONTROL_CONFIG); // goto control
                                                          // config screen to
                                                          // bind the control
                        }
                    }
            ...
            toke1 = strchr (text, '#');
            if (!toke1 ||
                ((toke1 - text) == 0)) // No second one, or possibly a double?
                break;
            ...
            if (((toke1 - text) < 0) ||
                ...
                strncpy (
                    temp, text,
                    toke1 - text);      // isolate token into seperate buffer
 ...
 *
 * Automatically streams the file from disk if it's over 100k, otherwise plays
 * it as a normal file in memory.  Returns -1 if it didn't play, otherwise
 * index of voice
 */
int message_play_training_voice (int index) {
    ...
    if (!Message_waves[index].num.isValid ()) {
        ...
        cfclose (fp);
        if (len > 100000) {
            if ((Training_voice < 0) || !Training_voice_type ||
                (Training_voice != index)) {
                if (Training_voice >= 0) {
                    if (Training_voice_type) {
                        if (Training_voice == index)
                            audiostream_stop (
                                Training_voice_soundstream, 1, 0);
                        else
                            audiostream_close_file (
                                Training_voice_soundstream, false);
                    }
                    else {
                        snd_stop (Training_voice_snd_handle);
                    }
            ...
            if (Training_voice_soundstream >= 0)
                audiostream_play (
                    Training_voice_soundstream,
                    (Master_voice_volume * aav_voice_volume), 0);
...
/**
 * One time initializations done when we want to display a new training
 * mission.
 *
 * This does all the processing and setup required to actually display it,
 * including starting the voice file playing
 */
void message_training_setup (int m, int length, char* special_message) {
    if ((m < 0) ||
        !Messages[m].message[0]) { // remove current message from the screen
    ...
    // moved from message_training_display() because we got rid of an extra
    // buffer and we have to determine the number of lines earlier to avoid
    // inadvertant modification of Training_buf.  - taylor
    training_process_message (Training_buf);
    Training_num_lines = split_str (
    ...
    if (message_play_training_voice (Messages[m].wave_info.index) < 0) {
        if (length > 0)
            ...
            Training_message_timestamp = timestamp (
                TRAINING_TIMING_BASE +
                (int)strlen (Messages[m].message) *
...
void message_training_queue (const char* text, int timestamp, int length) {
    ...
    if (Training_message_queue_count < TRAINING_MESSAGE_QUEUE_MAX) {
        ...
        Training_message_queue[Training_message_queue_count].timestamp =
            timestamp;
...
void message_training_remove_from_queue (int idx) {
    ...
    Training_message_queue[Training_message_queue_count].special_message =
        NULL; // not a memory leak because we copied the pointer
}

/**
 * Check the training message queue to see if we should play a new message yet
 * or not.
 */
void message_training_queue_check () {
    ...
    if (iship_num != -1) // added by Goober5000
        if (Ships[iship_num].is_dying_or_departing ()) return;

    if (Training_failure) return;

    for (i = 0; i < Training_message_queue_count; i++) {
        if (timestamp_elapsed (Training_message_queue[i].timestamp)) {
            message_training_setup (
                Training_message_queue[i].num,
                Training_message_queue[i].length,
...
void message_training_update_frame () {
    ...
    if (hud_disabled () && !hud_disabled_except_messages ()) { return; }
    ...
    if (timestamp_elapsed (Training_message_timestamp) ||
        !strlen (Training_buf)) {
        return;
    }
...
HudGaugeTrainingMessages::HudGaugeTrainingMessages ()
    : HudGauge (
          HUD_OBJECT_TRAINING_MESSAGES, HUD_DIRECTIVES_VIEW, false, true,
          VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
              VM_OTHER_SHIP,
          255, 255, 255) {}
...
void HudGaugeTrainingMessages::render (float /*frametime*/) {
    ...
    gr_shade (
        position[0], position[1], TRAINING_MESSAGE_WINDOW_WIDTH,
        Training_num_lines * height + height);
    gr_reset_screen_scale ();

    gr_set_color_fast (&Color_bright_blue);
    mode = count = 0;
    for (i = 0; i < Training_num_lines;
         i++) { // loop through all lines of message
        str = Training_lines[i];
        z = 0;
        x = position[0] +
            (TRAINING_MESSAGE_WINDOW_WIDTH - TRAINING_LINE_WIDTH) / 2;
        y = position[1] + i * height + height / 2 + 1;

        while ((str - Training_lines[i]) <
               Training_line_lengths[i]) { // loop through each character of
                                           // each line
            if ((count < MAX_TRAINING_MESSAGE_MODS) &&
                (str == Training_message_mods[count].pos)) {
                buf[z] = 0;
                ...
                case TMMOD_BOLD: gr_set_color_fast (&Color_white); break;
                }
            }
...
/**
 * Processes a new training message to get hilighting information and store it
