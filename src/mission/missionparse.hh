...
// mission parse flags used for parse_mission() to tell what kind of
// information to get from the mission file
...
struct support_ship_info {
        ...
        int max_concurrent_ships;          // max number of concurrent support ships in
                                           // mission per team
...
struct mission {
        ...
        int num_players;                       // valid in multiplayer missions -- number of players
                                               // supported
        uint num_respawns;                     // valid in multiplayer missions -- number of respawns
                                               // allowed
        int max_respawn_delay;                 // valid in multiplayer missions -- number of
                                               // respawns allowed
        support_ship_info support_ships;       // Goober5000
        char squad_filename[MAX_FILENAME_LEN]; // if the player has been reassigned
                                               // to a squadron, this is the
                                               // filename of the logo, otherwise
                                               // empty string
        char squad_name[NAME_LENGTH];          // if the player has been reassigned to a
                                               // squadron, this is the name of the
                                               // squadron, otherwise empty string
        ...
        void Reset()
        {
                ...
                squad_filename[0] = '\0';
                squad_name[0] = '\0';
                for (i = 0; i < GR_NUM_RESOLUTIONS; i++) loading_screen[i][0] = '\0';
                skybox_model[0] = '\0';
...
// FURTHER NOTE (Goober5000): If a new flag is added here, the code
// (particularly in sexp.cpp) must be reworked so that all the flags are
// maintained from function to function
...
extern char
        Mission_filename[80]; // filename of mission in The_mission (Fred only)

#define MAX_FORMATION_NAMES 3
#define MAX_STATUS_NAMES    3

// defines for arrival locations.  These defines should match their
// counterparts in the arrival location array
...
// defines for departure locations.  These defines should match their
// counterparts in the departure location array
...
public:
        ...
        int arrival_distance;  // used when arrival location is near or in front of
                               // some ship
        ...
        int pos_in_wing; // Goober5000 - needed for FRED with the new way things
                         // work
        ...
        int score;
        float assist_score_pct; // percentage of the score which players who gain
                                // an assist will get when this ship is killed
        int orders_accepted;    // which orders this ship will accept from the player
        p_dock_instance *
                dock_list;      // Goober5000 - parse objects this parse object is docked to
        ...
        uint respawn_count;   // number of respawns for this object.  Applies only to
                              // player wing ships in multiplayer
        int respawn_priority; // priority this ship has for controlling respawn
                              // points
...
#define POBJ_INDEX(pobjp) \
        (int)(pobjp - &Parse_objects[0]) // yes, this arithmetic is valid :D
...
int parse_wing_create_ships(
        wing *wingp, int num_to_create, int force = 0, int specific_instance = -1);
...
int get_mission_info(
        const char *filename, mission *missionp = NULL, bool basic = true);
...
#endif // FREESPACE2_MISSION_MISSIONPARSE_HH
