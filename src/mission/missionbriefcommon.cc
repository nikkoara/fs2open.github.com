...
void brief_preload_icon_anim(brief_icon *bi)
{
        ...
        if (!strcasecmp(NOX("none"), ga->filename))
                return;
...
void brief_preload_fade_anim(brief_icon *bi)
{
        ...
        if (!strcasecmp(NOX("none"), ha->filename))
                return;
...
{
        ASSERT(
                0 <= instance && instance < (int)(sizeof(Colored_stream) / sizeof(*Colored_stream)));
        ...
        size_t truncate_len = int(
                Brief_text_wipe_time_elapsed / BRIEF_TEXT_WIPE_TIME * Max_briefing_line_len);
        ...
        int max_lines, const bool append)
{
        ...
        if (default_color == '\0' || !brief_verify_color_tag(default_color)) { // call brief_verify_color_tag() to make sure our
        ...
        grid *gridp, vec3d *pos, matrix *orient, int force)
{
        ...
        dist_to_plane = fabsf(vm_dist_to_plane(pos, &gridp->gmatrix.vec.uvec, &c));
        square_size = 1.0f;
        ...
        if (!ux)
                gpos.xyz.x = mroundf(gpos.xyz.x, roundoff);
        if (!uy)
                gpos.xyz.y = mroundf(gpos.xyz.y, roundoff);
        if (!uz)
                gpos.xyz.z = mroundf(gpos.xyz.z, roundoff);

        if ((square_size != gridp->square_size) || (gpos.xyz.x != gridp->center.xyz.x) || (gpos.xyz.y != gridp->center.xyz.y) || (gpos.xyz.z != gridp->center.xyz.z) || force) {
...
{
        for (std::vector< briefing_icon_info >::iterator ii = Briefing_icon_info.begin();
