...
// defines for types of goals.  We will use part of the int field of the
// mission_goal struct as a bit field for goal flags
...
// defines for goal status.  These status are also used in campaign file for
// marking goal status in campaign save file
...
// defined for event states.  We will also use the satisfied/failed for saving
// event information in campaign save file
...
#define MEF_DIRECTIVE_SPECIAL                                                                      \
        (1 << 1)                         // used to mark a directive as true even though not fully
                                         // satisfied
#define MEF_DIRECTIVE_TEMP_TRUE (1 << 2) // this directive is temporarily true.
#define MEF_USING_TRIGGER_COUNT                                              \
        (1 << 3) // Karajorma - use trigger count as well as repeat count to
                 // determine how many repeats this event has

#define MAX_MISSION_EVENT_LOG_FLAGS \
        9 // this must be changed if a mission log flag is added below
...
#define MLF_ALL_REPETITION_FLAGS \
        (MLF_FIRST_REPEAT_ONLY | MLF_LAST_REPEAT_ONLY | MLF_FIRST_TRIGGER_ONLY | MLF_LAST_TRIGGER_ONLY)
...
struct mission_event {
        ...
        int interval;           // interval (in seconds) at which an evaulation is repeated
                                // once true.
        ...
        int satisfied_time; // this is used to temporarily mark the directive as
                            // satisfied when the event isn't (e.g. for a destroyed
                            // wave when there are more waves later)
        int born_on_date;   // timestamp at which event was born
        int team;           // for multiplayer games

        // event log stuff
        int mission_log_flags; // flags that are used to determing which events are
                               // written to the log
...
int mission_ai_goal_achievable(
        ai_goal *aigp); // determines if an AI goal is achievable
void mission_add_ai_goal(
        int sexp, ai_info *aip); // adds a goal onto the given ai_info structure
int mission_evaluate_primary_goals(
        void); // determine if the primary goals for the mission are complete --
               // returns one of the above defines
...
void mission_goal_fetch_num_resolved(
        int desired_type, int *num_resolved, int *total, int team = -1);
...
void ML_render_objectives_key(); // renders objectives key on ingame
                                 // objectives screen

#endif // FREESPACE2_MISSION_MISSIONGOALS_HH
