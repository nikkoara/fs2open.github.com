...
static int Hotkey_bits[MAX_SHIPS]; // bitfield indicating which hotkeys are
                                   // used by each ship

static int Hotkey_sets_saved; // have we saved the sets for this mission

static int Mission_hotkey_save_timestamp; // timestamp used to tell us when we
                                          // can save
...
static UI_XSTR Hotkey_text[GR_NUM_RESOLUTIONS][HOTKEY_NUM_TEXT] = {
    { // GR_640
      { "Cancel", 1516, 7, 392, UI_XSTR_COLOR_GREEN, -1,
        &Buttons[GR_640][CANCEL_BUTTON].button },
      { "Clear", 1517, 85, 392, UI_XSTR_COLOR_GREEN, -1,
        &Buttons[GR_640][CLEAR_BUTTON].button },
      { "Reset", 1518, 159, 392, UI_XSTR_COLOR_GREEN, -1,
        &Buttons[GR_640][RESET_BUTTON].button },
      { "Help", 1519, 500, 440, UI_XSTR_COLOR_GREEN, -1,
        &Buttons[GR_640][HELP_BUTTON].button },
      { "Options", 1520, 479, 464, UI_XSTR_COLOR_GREEN, -1,
        &Buttons[GR_640][OPTIONS_BUTTON].button },
      { "Accept", 1521, 573, 413, UI_XSTR_COLOR_GREEN, -1,
        &Buttons[GR_640][ACCEPT_BUTTON].button } },
    { // GR_1024
      { "Cancel", 1516, 30, 629, UI_XSTR_COLOR_GREEN, -1,
        &Buttons[GR_1024][CANCEL_BUTTON].button },
      { "Clear", 1517, 151, 629, UI_XSTR_COLOR_GREEN, -1,
        &Buttons[GR_1024][CLEAR_BUTTON].button },
      { "Reset", 1518, 269, 629, UI_XSTR_COLOR_GREEN, -1,
        &Buttons[GR_1024][RESET_BUTTON].button },
      { "Help", 1519, 800, 704, UI_XSTR_COLOR_GREEN, -1,
        &Buttons[GR_1024][HELP_BUTTON].button },
      { "Options", 1520, 797, 743, UI_XSTR_COLOR_GREEN, -1,
        &Buttons[GR_1024][OPTIONS_BUTTON].button },
      { "Accept", 1521, 902, 661, UI_XSTR_COLOR_GREEN, -1,
...
// function used in a couple of places to get the actual hotkey set number from
// a key value. not trivial since our current keysets (F5 - F12) do not have
// sequential keycodes
...
// function to maybe restore some hotkeys during the first N seconds of the
// mission
...
void mission_hotkey_set_defaults () {
    ...
    for (i = 0; i < MAX_KEYED_TARGETS; i++) { hud_target_hotkey_clear (i); }
    ...
    // if we have hotkeys saved from the previous run of this mission, then
    // simply keep the cleared sets, and let the restore code take care of it!
    // This works because this function is currently only called from one place
    // -- after the mission loads.
    ...
    for (A = GET_FIRST (&obj_used_list); A != END_OF_LIST (&obj_used_list);
        ...
        // if the hotkey is the last hotkey in the list, then don't add it
        // either since this hotkey is a special marker to indicate that this
        // ship should remain invisible in the hotkey screen until after
        // mission starts
    ...
    for (i = 0; i < Num_wings; i++) {
        ...
        if (wp->hotkey == -1) continue;

        // like ships, skip this wing if the hotkey is the last hotkey item
        if (wp->hotkey == MAX_KEYED_TARGETS) continue;

        for (j = 0; j < wp->current_count; j++) {
            if (wp->ship_index[j] == -1) continue;

            sp = &Ships[wp->ship_index[j]];
            hud_target_hotkey_add_remove (
                wp->hotkey, &Objects[sp->objnum], HOTKEY_MISSION_FILE_ADDED);
        }
    }
}

// function to reset the saved hotkeys -- called when a new mission is loaded
void mission_hotkey_reset_saved () { Num_hotkeys_saved = 0; }

// next function called when we might want to save the hotkey sets for the
// player.  We will save the hotkey sets N seconds into the mission
...
// function which gets called from MissionParse to maybe add a ship or wing to
// a hotkey set. this intermediate function is needed so that we don't blast
// over possibly saved hotkey sets
void mission_hotkey_mf_add (int set, int objnum, int how_to_add) {
    // if we are restoring hotkeys, and the timer hasn't elapsed, then return
    // and let the hotkey restoration code deal with it
    if (Num_hotkeys_saved &&
        !timestamp_elapsed (Mission_hotkey_save_timestamp))
        return;
...
void mission_hotkey_validate () {
    ...
    for (i = 0; i < MAX_KEYED_TARGETS; i++) {
        ...
        while (hitem != END_OF_LIST (plist)) {
            ...
            obj_valid = FALSE;
            for (A = GET_FIRST (&obj_used_list);
                 A != END_OF_LIST (&obj_used_list); A = GET_NEXT (A)) {
...
// get the Hotkey_bits of a whole wing (bits must be set in all ships of wing
// for a hotkey bit to be set)
int get_wing_hotkeys (int n) {
    ...
    for (i = 0; i < Wings[n].current_count; i++) {
        ...
        // don't count the player ship for the total -- you cannot assign the
        // player since bad things can happen on the hud.
        ship_index = Wings[n].ship_index[i];
        if (&Ships[ship_index] == Player_ship) continue;
...
// insert a line of hotkey smuck somewhere between 'start' and end of list such
// that it is sorted by name
...
int hotkey_build_team_listing (int enemy_team_mask, int y, bool list_enemies) {
    ...
    for (i = 0; i < MAX_SHIPS; i++)
        if (iff_matches_mask (Ships[i].team, team_mask)) break;

    if (i >= MAX_SHIPS) return y;
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (Ship_info[Ships[shipnum].ship_info_index].class_type < 0 ||
            !(Ship_types[Ship_info[Ships[shipnum].ship_info_index].class_type]
                  .flags[Ship::Type_Info_Flags::Hotkey_on_list]))
            continue;

        // don't process ships invisible to sensors, dying or departing
        if (Ships[shipnum].flags[Ship::Ship_Flags::Hidden_from_sensors] ||
            Ships[shipnum].is_dying_or_departing ())
            continue;

        // if a ship's hotkey is the last hotkey on the list, then maybe make
        // the hotkey -1 if we are now in mission.  Otherwise, skip this ship
        if (Ships[shipnum].hotkey == MAX_KEYED_TARGETS) {
            if (!(Game_mode & GM_IN_MISSION)) continue; // skip to next ship
            Ships[shipnum].hotkey = -1;
        }

        // be sure this ship isn't in a wing, and that the teams match
        if (iff_matches_mask (Ships[shipnum].team, team_mask) &&
            (Ships[shipnum].wingnum < 0)) {
            hotkey_line_add_sorted (
                Ships[shipnum].get_display_string (), HOTKEY_LINE_SHIP,
                shipnum, start);
        }
    }

    for (i = 0; i < Num_wings; i++) {
        if (Wings[i].current_count &&
            (Wings[i].ship_index[Wings[i].special_ship] >= 0) &&
            iff_matches_mask (
                Ships[Wings[i].ship_index[Wings[i].special_ship]].team,
                team_mask)) {
            // special check for the player's wing.  If he's in a wing, and the
            // only guy left, don't do anything
            if ((Player_ship->wingnum == i) && (Wings[i].current_count == 1))
                continue;

            // if a ship's hotkey is the last hotkey on the list, then maybe
            // make the hotkey -1 if we are now in mission.  Otherwise, skip
            // this ship
            ...
            for (j = 0; j < Wings[i].current_count; j++) {
                if (Ships[Wings[i].ship_index[j]]
                        .flags[Ship::Ship_Flags::Hidden_from_sensors])
                    break;
            }
            // if we didn't reach the end of the list, don't display the wing
            if (j < Wings[i].current_count) continue;

            z = hotkey_line_add_sorted (
                Wings[i].name, HOTKEY_LINE_WING, i, start);
            if (Wings[i].flags[Ship::Wing_Flags::Expanded]) {
                for (j = 0; j < Wings[i].current_count; j++) {
                    s = Wings[i].ship_index[j];
                    z = hotkey_line_insert (
                        z + 1, Ships[s].get_display_string (),
                        HOTKEY_LINE_SUBSHIP, s);
                }
            }
        }
    }
...
int hotkey_line_query_visible (int n) {
    ...
    if ((y < 0) ||
        (y + gr_get_font_height () > Hotkey_list_coords[gr_screen.res][3]))
        return 0;
...
void hotkey_scroll_screen_down () {
    if (Hotkey_lines[Num_lines - 1].y + gr_get_font_height () >
        Hotkey_lines[Scroll_offset].y + Hotkey_list_coords[gr_screen.res][3]) {
        Scroll_offset++;
        while (!hotkey_line_query_visible (Selected_line) ||
               (Hotkey_lines[Selected_line].type == HOTKEY_LINE_HEADING)) {
            Selected_line++;
...
void expand_wing () {
    ...
    if (Hotkey_lines[Selected_line].type == HOTKEY_LINE_WING) {
        ...
        for (z = 0; z < Num_lines; z++)
            if ((Hotkey_lines[z].type == HOTKEY_LINE_WING) &&
                (Hotkey_lines[z].index == i)) {
                Selected_line = z;
                break;
            }
    }
...
void reset_hotkeys () {
    ...
    for (i = 0; i < MAX_SHIPS; i++) Hotkey_bits[i] = 0;
    ...
    for (i = 0; i < MAX_KEYED_TARGETS; i++) {
        ...
        for (so = GET_FIRST (&Ship_obj_list);
            ...
            if (Hotkey_bits[Objects[so->objnum].instance] & (1 << i)) {
                hud_target_hotkey_add_remove (
                    i, &Objects[so->objnum], HOTKEY_USER_ADDED);
            }
        }
    }
}
...
void hotkey_button_pressed (int n) {
    switch (n) {
    case SCROLL_UP_BUTTON: hotkey_scroll_screen_up (); break;

    case SCROLL_DOWN_BUTTON: hotkey_scroll_screen_down (); break;
...
// mission_hotkey_init()
//
// Initialize the hotkey assignment screen system.  Called when
// GS_STATE_HOTKEY_SCREEN is entered.
//
void mission_hotkey_init () {
    ...
    for (i = 0; i < NUM_BUTTONS; i++) {
        ...
        b->button.create (
            &Ui_window, "", b->x, b->y, 60, 30, i < 2 ? 1 : 0, 1);
...
//
// Cleanup the hotkey assignment screen system.  Called when
// GS_STATE_HOTKEY_SCREEN is left.
//
void mission_hotkey_close () {
    if (Background_bitmap) bm_release (Background_bitmap);
    if (Wing_bmp >= 0) bm_release (Wing_bmp);
...
void mission_hotkey_do_frame (float /*frametime*/) {
    ...
    case KEY_TAB:
    case KEY_ENTER:
    case KEY_PADENTER: expand_wing (); break;

    case KEY_EQUAL:
    case KEY_PADPLUS: add_hotkey (Cur_hotkey); break;

    case KEY_MINUS:
    case KEY_PADMINUS: remove_hotkey (); break;

    case KEY_F2: gameseq_post_event (GS_EVENT_OPTIONS_MENU); break;

    case KEY_CTRLED | KEY_R: reset_hotkeys (); break;

    case KEY_CTRLED | KEY_C: clear_hotkeys (); break;
    } // end switch

    // ?
    for (i = 0; i < MAX_KEYED_TARGETS; i++) {
        if (k == Key_sets[i]) Cur_hotkey = i;

        if (k == (Key_sets[i] | KEY_SHIFTED)) add_hotkey (i);
    }
    ...
    for (i = 0; i < LIST_BUTTONS_MAX; i++) {
        ...
        if (List_buttons[i].pressed ()) {
            ...
            z += Hotkey_list_coords[gr_screen.res]
                                   [0]; // adjust to full screen space
            if ((z >= Hotkey_wing_icon_x[gr_screen.res]) &&
                (z < (Hotkey_wing_icon_x[gr_screen.res]) +
                         Hotkey_function_field_width[gr_screen.res])) {
                expand_wing ();
            }
        }
    ...
    gr_printf_menu (
        Hotkey_function_name_coords[gr_screen.res][0] +
            (Hotkey_function_name_coords[gr_screen.res][2] - w) / 2,
        Hotkey_function_name_coords[gr_screen.res][1], "%s", buf);

    font::set_font (font::FONT1);
    line = Scroll_offset;
    while (hotkey_line_query_visible (line)) {
        z = Hotkey_lines[line].index;
        y = Hotkey_list_coords[gr_screen.res][1] + Hotkey_lines[line].y -
            Hotkey_lines[Scroll_offset].y;
        hotkeys = 0;
        ...
        case HOTKEY_LINE_HEADING:
            ...
            i = y + h / 2 - 1;
            gr_line (
                Hotkey_list_coords[gr_screen.res][0], i,
                Hotkey_ship_x[gr_screen.res] - 2, i, GR_RESIZE_MENU);
            gr_line (
                Hotkey_ship_x[gr_screen.res] + w + 1, i,
        ...
        if (hotkeys) {
            for (i = 0; i < MAX_KEYED_TARGETS; i++) {
                if (hotkeys & (1 << i)) {
                    gr_printf_menu (
                        Hotkey_list_coords[gr_screen.res][0] +
                            Hotkey_function_field_width[gr_screen.res] * i,
                        y, "%s", Scan_code_text[Key_sets[i]]);
                }
            }
            /*
                        *buf = 0;
        ...
        // draw ship/wing name
        strcpy (buf, Hotkey_lines[line].label.c_str ());
        ...
        if (Hotkey_lines[line].type == HOTKEY_LINE_SUBSHIP) {
            ...
            font::force_fit_string (
                ...
                Hotkey_list_coords[gr_screen.res][0] +
                    Hotkey_list_coords[gr_screen.res][2] -
                    (Hotkey_ship_x[gr_screen.res] + 20));
...
void mission_hotkey_exit () { gameseq_post_event (GS_EVENT_PREVIOUS_STATE); }
