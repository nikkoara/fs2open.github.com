...
int hotkey_line_add_sorted(const char *text, int type, int index, int start)
{
        ...
        z = Num_lines - 1;
        while ((z >= start) && ((Hotkey_lines[z].type == HOTKEY_LINE_SUBSHIP) || (strcasecmp(text, Hotkey_lines[z].label.c_str()) < 0)))
                z--;
...
void mission_hotkey_do_frame(float /*frametime*/)
{
        ...
        gr_printf_menu(
                Hotkey_function_name_coords[gr_screen.res][0] + (Hotkey_function_name_coords[gr_screen.res][2] - w) / 2,
        ...
        while (hotkey_line_query_visible(line)) {
                ...
                if (Hotkey_lines[line].type != HOTKEY_LINE_HEADING) {
                        ...
                        List_buttons[line - Scroll_offset].update_dimensions(
                                ...
                                Hotkey_list_coords[gr_screen.res][0] + Hotkey_list_coords[gr_screen.res][2] - Hotkey_list_coords[gr_screen.res][0],
                                font_height);
                ...
                if (hotkeys) {
                        for (i = 0; i < MAX_KEYED_TARGETS; i++) {
                                if (hotkeys & (1 << i)) {
                                        gr_printf_menu(
                                                Hotkey_list_coords[gr_screen.res][0] + Hotkey_function_field_width[gr_screen.res] * i,
                ...
                if (Hotkey_lines[line].type == HOTKEY_LINE_SUBSHIP) {
                        ...
                        font::force_fit_string(
                                buf, 255,
                                Hotkey_list_coords[gr_screen.res][0] + Hotkey_list_coords[gr_screen.res][2] - (Hotkey_ship_x[gr_screen.res] + 20));
                        gr_printf_menu(Hotkey_ship_x[gr_screen.res] + 20, y, "%s", buf);
                } else {
                        font::force_fit_string(
                                buf, 255,
                                Hotkey_list_coords[gr_screen.res][0] + Hotkey_list_coords[gr_screen.res][2] - Hotkey_ship_x[gr_screen.res]);
