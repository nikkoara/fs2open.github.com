...
int comp_training_lines_by_born_on_date(const void *m1, const void *m2)
{
        ...
        return (
                Mission_events[*e1 & 0xffff].born_on_date - Mission_events[*e2 & 0xffff].born_on_date);
}
...
void sort_training_objectives()
{
        ...
        for (i = 0; i < offset; i++) {
                ...
                if (event_status == EVENT_CURRENT) {
                        ...
                        if (f2i(
                                    Missiontime - Mission_events[TRAINING_OBJ_LINES_MASK(i)].satisfied_time)
                            < MIN_SATISFIED_TIME) {
                                Training_obj_lines[i] |= TRAINING_OBJ_STATUS_UNKNOWN;
                                num_offset_events++;
                        } else {
...
void training_check_objectives()
{
        ...
        for (event_idx = 0; event_idx < Num_mission_events; event_idx++) {
                ...
                if ((event_status != EVENT_UNBORN) && Mission_events[event_idx].objective_text && (timestamp() > Mission_events[event_idx].born_on_date + Directive_wait_time)) {
                        if (!Training_failure || !strncasecmp(Mission_events[event_idx].name, XSTR("Training failed", 423), 15)) {
                ...
                if (!toke2 || (toke1 && (toke1 < toke2))) {   // found $ before #
                        ...
                        if (((toke2 - text) < 0) || ((toke2 - text) >= (ptr_s)sizeof(temp))) {
                                ...
                                if (ptr) {                 // was key translated properly?
                                        if (!strcasecmp(ptr, NOX("none")) && (Training_bind_warning != Missiontime)) {
                                                if (The_mission.game_type & MISSION_TYPE_TRAINING) {
                                                        r = popup(
        ...
        if (!Message_waves[index].num.isValid()) {
                ...
                if (len > 100000) {
                        if ((Training_voice < 0) || !Training_voice_type || (Training_voice != index)) {
                                ...
                                if (strncasecmp(
                                            Message_waves[index].name, NOX("none.wav"), 4)
                                    != 0) {
        ...
        if (message_play_training_voice(Messages[m].wave_info.index) < 0) {
                if (length > 0)
                        ...
                        Training_message_timestamp = timestamp(
                                TRAINING_TIMING_BASE + (int)strlen(Messages[m].message) * TRAINING_TIMING); // no voice file playing
        } else
...
void message_training_queue(const char *text, int timestamp, int length)
{
        ...
        if (Training_message_queue_count < TRAINING_MESSAGE_QUEUE_MAX) {
                if (!strcasecmp(text, NOX("none"))) {
                        m = -1;
                } else {
                        for (m = 0; m < Num_messages; m++)
                                if (!strcasecmp(text, Messages[m].name))
                                        break;
...
void HudGaugeTrainingMessages::render(float /*frametime*/)
{
        ...
        for (i = 0; i < Training_num_lines;
             i++) { // loop through all lines of message
                ...
                while ((str - Training_lines[i]) < Training_line_lengths[i]) { // loop through each character of
                                                                               // each line
