...
typedef struct log_text_seg {
    ...
    int flags; // used to possibly print special characters when displaying the
               // log
} log_text_seg;
...
std::array< log_entry, MAX_LOG_ENTRIES >
    log_entries; // static array because John says....
int last_entry;

void mission_log_init () {
    last_entry = 0;

    // zero out all the memory so we don't get bogus information when playing
    // across missions!
    log_entries.fill ({});
}

// returns the number of entries in the mission log
int mission_log_query_scrollback_size () { return last_entry; }

// function to clean up the mission log removing obsolete entries.  Entries
// might get marked obsolete in several ways -- having to recycle entries, a
// ship's subsystem destroyed entries when a ship is fully destroyed, etc.
void mission_log_cull_obsolete_entries () {
    ...
    do {
        // get to the next non-obsolete entry.  The obsolete entry must not be
        // essential either!
...
// function to mark entries as obsolete.  Passed is the type of entry that is
// getting added to the log.  Some entries might get marked obsolete as a
// result of this type
void mission_log_obsolete_entries (LogType type, const char* pname) {
    int i;
    log_entry* entry = NULL;

    // before adding this entry, check to see if the entry type is a ship
    // destroyed or destructed entry. If so, we can remove any subsystem
    // destroyed entries from the log for this ship.
    if (type == LOG_SHIP_DESTROYED || type == LOG_SELF_DESTRUCTED) {
        for (i = 0; i < last_entry; i++) {
            entry = &log_entries[i];

            // check to see if the type is a subsystem destroyed entry, and
    ...
    // check to see if we are getting to about 80% of our log capacity.  If so,
    // cull the log.
    if (last_entry > LOG_CULL_MARK) {
        mission_log_cull_obsolete_entries ();

        // if we culled the entries, and we are still low on space, we need to
        // take more drastic measures. these include removing all non-essential
        ...
        if (last_entry > LOG_CULL_MARK) {
            ...
            // if we get to this point, and there are no entries left -- we are
            // in big trouble.  We will simply mark the first 20% of the log as
...
// that this event is for.  Don't add entries with this function for
// multiplayer
void mission_log_add_entry (
    LogType type, const char* pname, const char* sname, int info_index) {
    int last_entry_save;
    log_entry* entry;
    ...
    // determine the contents of the flags member based on the type of entry we
    // added.  We need to store things like team for the primary and (possibly)
    // secondary object for this entry.
    switch (type) {
        int index;
    ...
    case LOG_SELF_DESTRUCTED:
        ...
        // some of the entries have a secondary component.  Figure out what is
        // up with them.
        if ((type == LOG_SHIP_DOCKED) || (type == LOG_SHIP_UNDOCKED)) {
            ...
            (type == LOG_SHIP_SUBSYS_DESTROYED) &&
            (Ship_info[Ships[index].ship_info_index].is_small_ship ())) {
    ...
    case LOG_WING_ARRIVED:
        ...
        // get the team value from the first ship in the list because the
        // info_index contains the wave count
        if (type == LOG_WING_ARRIVED) {
            int i, si = -1;

            // Goober5000 - get the team value from any ship in the list,
            // because ships that arrive initially docked could be created in
            // random order
...
#ifndef NDEBUG
        // MWA 2/25/98.  debug code to try to find any ships in this wing that
        // have departed. scan through all log entries and find at least one
        // ship_depart entry for a ship that was in this wing.
        if (type == LOG_WING_DEPARTED) {
            int i;

            // if all were destroyed, then don't do this debug code.
            if ((Wings[index].total_destroyed + Wings[index].total_vanished) ==
                Wings[index].total_arrived_count) {
                break;
            }

            for (i = 0; i < last_entry; i++) {
                if (log_entries[i].type != LOG_SHIP_DEPARTED) { continue; }
    ...
    case LOG_WAYPOINTS_DONE: entry->flags |= MLF_HIDDEN; break;
...
int mission_log_get_time_indexed (
    ...
    for (i = 0; i < last_entry; i++, entry++) {
        ...
        if (entry->type == type) {
            // if we are looking for a dock/undock entry, then we don't care
            // about the order in which the names were passed into this
            // function.  Count the entry as found if either name matches both
            // in the other set.
            if ((type == LOG_SHIP_DOCKED) || (type == LOG_SHIP_UNDOCKED)) {
                if (sname == NULL) {
                    Int3 ();
            ...
            if (found) {
                ...
                if (!count) {
                    entry->flags |=
                        MLF_ESSENTIAL; // since the goal code asked for this
                                       // entry, mark it as essential

                    if (time) { *time = entry->timestamp; }
...
int mission_log_get_time (
    LogType type, const char* pname, const char* sname, fix* time) {
...
int mission_log_get_count (
    LogType type, const char* pname, const char* sname) {
    int i;
    log_entry* entry;
    int count = 0;
...
void message_log_add_seg (
    int n, int x, int msg_color, const char* text, int flags = 0) {
    log_text_seg *seg, **parent;

    if ((n < 0) || (n >= MAX_LOG_LINES)) return;
...
int message_log_team_get_color (int team) { return NUM_LOG_COLORS + team; }
...
void message_log_init_scrollback (int pw) {
    ...
    P_width = pw;
    mission_log_cull_obsolete_entries (); // compact array so we don't have
                                          // gaps
    ...
    for (i = 0; i < last_entry; i++) {
        ...
        if (entry->flags & MLF_HIDDEN) continue;
        ...
        if ((Lcl_gr) && ((entry->type == LOG_GOAL_FAILED) ||
            ...
            // in german goal events, just say "objective" instead of objective
            // name this is cuz we can't translate objective names
            message_log_add_seg (Num_log_lines, OBJECT_X, c, "Einsatzziel");
        }
        else if (
            (Lcl_pl) && ((entry->type == LOG_GOAL_FAILED) ||
                         (entry->type == LOG_GOAL_SATISFIED))) {
            ...
            message_log_add_seg (
                Num_log_lines, OBJECT_X, c, entry->pname_display.c_str ());
        }
        ...
        case LOG_SHIP_DESTROYED:
            ...
            if (!entry->sname_display.empty ()) {
                message_log_add_segs (
                    XSTR ("  Kill: ", 405), LOG_COLOR_NORMAL);
        ...
        case LOG_SELF_DESTRUCTED:
            message_log_add_segs (
                XSTR ("Self Destructed", 1476), LOG_COLOR_NORMAL);
            break;
        ...
        case LOG_SHIP_DOCKED:
            message_log_add_segs (
                XSTR ("Docked with ", 409), LOG_COLOR_NORMAL);
            message_log_add_segs (entry->sname_display.c_str (), c);
            break;
        ...
        case LOG_SHIP_UNDOCKED:
            message_log_add_segs (
                XSTR ("Undocked with ", 412), LOG_COLOR_NORMAL);
            message_log_add_segs (entry->sname_display.c_str (), c);
            break;
        ...
        case LOG_PLAYER_CALLED_FOR_REARM:
            message_log_add_segs (
                XSTR (" called for rearm", 415), LOG_COLOR_NORMAL);
            break;

        case LOG_PLAYER_ABORTED_REARM:
            message_log_add_segs (
                XSTR (" aborted rearm", 416), LOG_COLOR_NORMAL);
            break;

        case LOG_PLAYER_CALLED_FOR_REINFORCEMENT:
            message_log_add_segs (
                XSTR ("Called in as reinforcement", 417), LOG_COLOR_NORMAL);
            break;
        ...
        case LOG_CAP_SUBSYS_CARGO_REVEALED:
            ...
            message_log_add_segs (
                XSTR (" subsystem cargo revealed: ", 1488), LOG_COLOR_NORMAL);
            strncpy (text, Cargo_names[entry->index], sizeof (text) - 1);
            message_log_add_segs (text, LOG_COLOR_BRIGHT);
            break;
        ...
        case LOG_GOAL_FAILED: {
            ...
            message_log_add_segs (
                ...
                (entry->type == LOG_GOAL_SATISFIED ? LOG_FLAG_GOAL_TRUE
        ...
        if (kill) { message_log_remove_segs (Num_log_lines); }
            ...
            if (Num_log_lines < MAX_LOG_LINES - 1) Num_log_lines++;
        }
    }
}
...
void message_log_shutdown_scrollback () {
    ...
    for (i = 0; i < MAX_LOG_LINES; i++) message_log_remove_segs (i);

    Num_log_lines = 0;
}

// message_log_scrollback displays the contents of the mesasge log currently
// much like the HUD message scrollback system.  I'm sure this system will be
// overhauled.
void mission_log_scrollback (
    int line, int list_x, int list_y, int list_w, int list_h) {
    char buf[256];
    int y;
    ...
    while (y + font_h <= list_h) {
        if (line >= Num_log_lines) break;

        if (Log_line_timestamps[line]) {
            gr_set_color_fast (&Color_text_normal);
            gr_print_timestamp (
                list_x + TIME_X, list_y + y, Log_line_timestamps[line],
                GR_RESIZE_MENU);
        }
        ...
        while (seg) {
            switch (seg->color) {
            case LOG_COLOR_BRIGHT: gr_set_color_fast (&Color_bright); break;

            case LOG_COLOR_OTHER: gr_set_color_fast (&Color_normal); break;
            ...
            if ((seg->flags & LOG_FLAG_GOAL_TRUE) ||
                ...
                gr_line (
                    list_x + TIME_X - 10, i, list_x + TIME_X - 8, i,
                    GR_RESIZE_MENU);
                gr_line (
                    list_x + TIME_X - 6, i - 4, list_x + TIME_X - 6, i - 2,
                    GR_RESIZE_MENU);
                gr_line (
                    list_x + TIME_X - 4, i, list_x + TIME_X - 2, i,
                    GR_RESIZE_MENU);
                gr_line (
                    list_x + TIME_X - 6, i + 2, list_x + TIME_X - 6, i + 4,
                    GR_RESIZE_MENU);
            }
