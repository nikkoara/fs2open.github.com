...
// ------------------------------------------------------------------------
// names for the icons that can appear in the briefing.  If you modify this
// list, update the Icons_names[] string array located in MissionParse.cpp
// ------------------------------------------------------------------------
...
// Moving out of missionbriefcommon.cpp so it can be referenced elsewhere
// -MageKing17
...
#define MAX_BRIEF_LINE_W_1024 \
        600 // max width of line in pixels in 1024x768 mode
...
#define BI_MIRROR_ICON \
        (1 << 3)                   // mirror the briefing icon so it points the other way - phreak
...
public:
        ...
        brief_stage()
                : text(), camera_time(0), flags(0), formula(-1), num_icons(0),
                  icons(NULL), num_lines(0), lines(NULL)
        {
                voice[0] = 0;
...
public:
        ...
        debrief_stage() : formula(-1), text(), recommendation_text()
        {
                voice[0] = 0;
        }
};
...
public:
        ...
        briefing() : num_stages(0)
        {
                memset(
                        background, 0,
                        GR_NUM_RESOLUTIONS * MAX_FILENAME_LEN * sizeof(char));
        }
};
...
void brief_render_icon(
        int stage_num, int icon_num, float frametime, int selected = 0,
        float w_scale_factor = 1.0f, float h_scale_factor = 1.0f);
...
int brief_color_text_init(
        const char *src, int w, const char default_color = '\0', int instance = 0,
        int max_lines = MAX_BRIEF_LINES, const bool append = false);
int brief_render_text(
        int line_offset, int x, int y, int h, float frametime, int instance = 0,
        int line_spacing = 0);
...
#endif // FREESPACE2_MISSION_MISSIONBRIEFCOMMON_HH
