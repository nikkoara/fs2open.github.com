...
// keep seperate lists of AVI's and wav files.  I suspect that many messages
...
// this number in this define should match the number of elements in the next
// array
...
#define MESSAGE_ALL_ALONE \
    31 // message sent when player is last ship left and primary objectives
       // still exist
...
struct MissionMessage  {
    ...
    // unions for avi/wave information.  Because of issues with Fred, we are
    // using the union to specify either the index into the avi or wave arrays
    // above, or refernce the name directly.  The currently plan is to only
    // have Fred reference the name field!!!
    union {
...
extern int
    Num_builtin_messages; // from messages.tbl -- index of message location to
                          // load mission specific messages into
...
// flags for personas.  the type flags must be sequential starting from 0, and
// must match the persona_type_names defined in missionmessage.cpp
...
// be sure that MAX_PERSONA_TYPES is always 1 greater than the last type
// bitfield above!!!
...
// function to parse a message from either messages.tbl or the mission file.
// Both files have the exact same format, so this function just gets reused in
// both instances.
...
void message_queue_message (
    ...
    int source, int group, int delay, int builtin_type = -1);

// functions which send messages to player -- called externally
void message_send_unique_to_player (
    char* id, void* data, int source, int priority, int group, int delay);
...
// preload mission messages (this is called by the level paging code when
...
// Kazan
// Use these functions with caution as everything else uses indexes... so make
// sure if you're going to use these there will be no remove_messages called
// before your message is displayed.

// these two are probably safe
// if change_message fails to find the message it'll fall through to
// add_message
bool add_message (
...
#endif // FREESPACE2_MISSION_MISSIONMESSAGE_HH
