...
// stuff for selecting campaigns.  We need to keep both arrays around since we
// display the list of campaigns by name, but must load campaigns by filename
...
// modules local variables to deal with getting new ships/weapons available to
// the player
int Num_granted_ships,
    Num_granted_weapons; // per mission counts of new ships and weapons
...
/**
 * Returns a string (which is malloced in this routine) of the name of the
 * given freespace campaign file. In the type field, we return if the campaign
 * is a single player or multiplayer campaign. The type field will only be
 * valid if the name returned is non-NULL
 */
int mission_campaign_get_info (
    const char* filename, char* name, int* type, int* max_players,
    char** desc) {
    int i, success = 0;
 ...
 * @return Number of missions added to the 'list', and up to 'max' missions may
 * be added to 'list'.
 * @return Negative on error.
 */
int mission_campaign_get_mission_list (
    const char* filename, char** list, int max) {
    int i, num = 0;
...
void mission_campaign_free_list () {
    ...
    if (!Campaign_names_inited) return;
...
int mission_campaign_maybe_add (const char* filename) {
    ...
    if (campaign_is_ignored (filename)) { return 0; }
    ...
    if (mission_campaign_get_info (
        ...
        if (!MC_multiplayer && (type == CAMPAIGN_TYPE_SINGLE)) {
            ...
            if (MC_desc) Campaign_descs[Num_campaigns] = desc;
 ...
 * It uses the multiplayer flag to tell if we should display a list of single
 * or multiplayer campaigns. This routine sets the Num_campaigns and
 * Campaign_names global variables
 */
void mission_campaign_build_list (bool desc, bool sort, bool multiplayer) {
    ...
    if (Num_campaigns != 0) mission_campaign_free_list ();
    ...
    // NOTE: we don't do sorting here, but we assume CF_SORT_NAME, and do it
    // manually below
    rc = cf_get_file_list (
...
void mission_campaign_get_sw_info () {
    ...
    for (auto it = Ship_info.cbegin (); it != Ship_info.cend (); ++it) {
        if (it->flags[Ship::Info_Flags::Player_ship])
            Campaign.ships_allowed[std::distance (Ship_info.cbegin (), it)] =
                1;
    }

    for (i = 0; i < MAX_WEAPON_TYPES; i++) Campaign.weapons_allowed[i] = 1;
    ...
    if (optional_string ("+Starting Weapons:")) {
        for (i = 0; i < MAX_WEAPON_TYPES; i++) Campaign.weapons_allowed[i] = 0;

        count =
            stuff_int_list (weapon_list, MAX_WEAPON_TYPES, WEAPON_POOL_TYPE);
...
/**
 * Starts a new campaign.  It reads in the mission information in the campaign
 * file It also sets up all variables needed inside of the game to deal with
 * starting mission numbers, etc
 *
 * Note: Due to difficulties in generalizing this function, parts of it are
 * duplicated throughout this file.  If you change the format of the campaign
 * file, you should be sure these related functions work properly and update
 * them if it breaks them.
 */
int mission_campaign_load (
    char* filename, player* pl, int load_savefile, bool reset_stats) {
    int i;
    ...
    try {
        ...
        // this also frees sexpressions so the next call to init_sexp will be
        // able to reclaim nodes previously used by another campaign.
        ...
        if (optional_string ("$Flags:")) { stuff_int (&(Campaign.flags)); }
        ...
        while (required_string_either ("#End", "$Mission:")) {
            ...
            cm->flags = 0;
            if (optional_string ("+Flags:")) stuff_int (&cm->flags);

            cm->main_hall = "0";
            // deal with previous campaign versions
            if (cm->flags & CMISSION_FLAG_BASTION) { cm->main_hall = "1"; }
            ...
            if (optional_string ("+Mission Loop Text:") ||
                optional_string ("+Mission Fork Text:")) {
                cm->mission_branch_desc =
                    stuff_and_malloc_string (F_MULTITEXT, NULL);
            }

            cm->mission_branch_brief_anim = NULL;
            if (optional_string ("+Mission Loop Brief Anim:") ||
                optional_string ("+Mission Fork Brief Anim:")) {
                cm->mission_branch_brief_anim =
                    stuff_and_malloc_string (F_MULTITEXT, NULL);
            }

            cm->mission_branch_brief_sound = NULL;
            if (optional_string ("+Mission Loop Brief Sound:") ||
                optional_string ("+Mission Fork Brief Sound:")) {
                cm->mission_branch_brief_sound =
                    stuff_and_malloc_string (F_MULTITEXT, NULL);
            }
    ...
    // we must try and load the campaign save file for this player.  Since all
    // campaign loads go through this routine, I think this place should be the
    // only necessary place to load the campaign save stuff.  The campaign save
    // file will get written when a mission has ended by player choice.
 ...
 * Generally, the filename will probably be the freespace campaign file, but
 * not necessarily.
 */
...
void player_loadout_init () {
    ...
    memset (
        Player_loadout.last_modified, 0,
        sizeof (Player_loadout.last_modified));

    for (i = 0; i < MAX_SHIP_CLASSES; i++) { Player_loadout.ship_pool[i] = 0; }
...
/**
 * Initializes some variables then loads the default FreeSpace single player
 * campaign.
 */
...
// the below two functions is internal to this module.  It is here so that I
// can group the save/load functions together.
//
...
void mission_campaign_delete_all_savefiles (const char* pilot_name) {
    ...
    // HACK HACK HACK HACK!!!!  cf_get_file_list is not reentrant.  Pretty dumb
    // because it should be.  I have to save any file filters
    filter_save = Get_file_list_filter;
    Get_file_list_filter = NULL;
    num_files = cf_get_file_list (
        names, dir_type, file_spec, CF_SORT_NONE, nullptr,
        CF_LOCATION_ROOT_USER | CF_LOCATION_ROOT_GAME | CF_LOCATION_TYPE_ROOT);
    Get_file_list_filter = filter_save;

    for (i = 0; i < num_files; i++) {
        strcpy (filename, names[i].c_str ());
        strcat (filename, ext);
        cf_delete (
            filename, dir_type,
            CF_LOCATION_ROOT_USER | CF_LOCATION_ROOT_GAME |
                CF_LOCATION_TYPE_ROOT);
    }
}
...
/**
 * Sets up the internal veriables of the campaign structure so the player can
 * play the next mission. If there are no more missions available in the
 * campaign, this function returns -1, else 0 if the mission was set
 * successfully
 */
int mission_campaign_next_mission () {
    if ((Campaign.next_mission == -1) ||
        (Campaign.name[0] ==
         '\0')) // will be set to -1 when there is no next mission
        return -1;

    if (Campaign.num_missions < 1) return -2;
    ...
    // reset the number of persistent ships and weapons for the next campaign
    // mission
    Num_granted_ships = 0;
...
/**
 * Called to go to the previous mission in the campaign.  Used only for Red
 * Alert missions
 */
int mission_campaign_previous_mission () {
    if (!(Game_mode & GM_CAMPAIGN_MODE)) return 0;

    if (Campaign.prev_mission == -1) return 0;
...
/**
 * Evaluate next campaign mission - set as Campaign.next_mission.  Also set
...
void mission_campaign_eval_next_mission () {
    ...
    if (Campaign.missions[cur].formula != -1) {
        flush_sexp_tree (Campaign.missions[cur]
                             .formula); // force formula to be re-evaluated
        eval_sexp (Campaign.missions[cur]
                       .formula); // this should reset Campaign.next_mission to
                                  // proper value
    }
    ...
    if (Campaign.missions[cur].flags & CMISSION_FLAG_HAS_LOOP) {
        ...
        if (Campaign.missions[cur].mission_loop_formula != -1) {
            flush_sexp_tree (
                Campaign.missions[cur]
                    .mission_loop_formula); // force formula to be re-evaluated
            eval_sexp (Campaign.missions[cur]
                           .mission_loop_formula); // this should reset
                                                   // Campaign.next_mission to
                                                   // proper value
        }
...
void mission_campaign_store_goals_and_events () {
    ...
    // first we must save the status of the current missions goals in the
    // campaign mission structure. After that, we can determine which mission
    // is tagged as the next mission.  Finally, we can save the campaign save
    // file we might have goal and event status if the player replayed a
    // mission
    if (mission_obj->goals != NULL) {
        free (mission_obj->goals);
        mission_obj->goals = NULL;
    }

    mission_obj->num_goals = Num_goals;
    if (mission_obj->num_goals > 0) {
        mission_obj->goals = (mgoal*)malloc (sizeof (mgoal) * Num_goals);
    ...
    for (i = 0; i < Num_goals; i++) {
        if (Mission_goals[i].name[0] == '\0') {
            ...
            strcpy (mission_obj->goals[i].name, goal_name);
        }
        else
            strcpy (mission_obj->goals[i].name, Mission_goals[i].name);
        ASSERT (
    ...
    if (mission_obj->num_events > 0) {
        mission_obj->events =
            (mevent*)malloc (sizeof (mevent) * Num_mission_events);
    ...
    for (i = 0; i < Num_mission_events; i++) {
        if (Mission_events[i].name[0] == '\0') {
            ...
            strcpy (mission_obj->events[i].name, Mission_events[i].name);

        // getting status for the events is a little different.  If the formula
        // value for the event entry is -1, then we know the value of the
        // result field will never change.  If the formula is not -1 (i.e.
        // still being evaluated at mission end time), we will write
        // "incomplete" for the event evaluation
...
void mission_campaign_store_variables (
    int persistence_type, bool store_red_alert) {
    int cur, i, j;
    cmission* mission_obj;
        ...
        for (i = 0; i < sexp_variable_count (); i++) {
            if (!(Sexp_variables[i].type &
                ...
                if (Sexp_variables[i].type & persistence_type) {
                    ...
                    for (j = 0; j < (int)Campaign.persistent_variables.size ();
                        ...
                        if (!(strcasecmp (
                                ...
                                Campaign.persistent_variables[j]
                                    .variable_name))) {
                            add_it = false;
                            Campaign.persistent_variables[j].type =
                                Sexp_variables[i].type;
                            strcpy (
                                Campaign.persistent_variables[j].text,
                    ...
                    // new variable
                    if (add_it) {
                        Campaign.persistent_variables.push_back (
                            Sexp_variables[i]);
                    }
                }
            }
            // we might need to save some eternal variables
                ...
                (persistence_type & SEXP_VARIABLE_SAVE_ON_MISSION_PROGRESS) &&
                (Sexp_variables[i].type & persistence_type) &&
...
/**
 * Called when the player's mission is over.  It updates the internal store of
 * goals and their status then saves the state of the campaign in the campaign
 * file. This gets called after player accepts mission results in debriefing.
 */
void mission_campaign_mission_over (bool do_next_mission) {
    int mission_num, i;
    cmission* mission_obj;

    // I don't think that we should have a record for these -- maybe we
    // might??????  If we do, then we should free them
    ...
    // if we are moving to a new mission, then change our data.  If we are
    // staying on the same mission, then we don't want to do anything.  Remove
    // information about goals/events
    if (Campaign.next_mission != mission_num) {
        ...
        // save the scoring values from the previous mission at the start of
        // this mission -- for red alert

        // save the state of the campaign in the campaign save file and move to
        // the end_game state
        if (Campaign.type == CAMPAIGN_TYPE_SINGLE) { Pilot.save_savefile (); }
    }
    else {
        // free up the goals and events which were just malloced.  It's kind of
        // like erasing any fact that the player played this mission in the
        // campaign.
        if (mission_obj->goals != NULL) {
            free (mission_obj->goals);
            mission_obj->goals = NULL;
    ...
    if (do_next_mission)
        mission_campaign_next_mission (); // sets up whatever needs to be set
                                          // to actually play next mission
}

/**
 * Called when the game closes -- to get rid of memory errors for Bounds
 * checker also called at campaign init and campaign load
 */
void mission_campaign_clear () {
    ...
    for (i = 0; i < Campaign.num_missions; i++) {
        ...
        if (Campaign.missions[i].variables != NULL) {
            free (Campaign.missions[i].variables);
 ...
 * dest[MAX_CAMPAIGN_MISSIONS][NAME_LENGTH]
 */
int mission_campaign_get_filenames (
...
/**
 * Read the goals and events from a mission in a campaign file and store that
 * information in the campaign structure for use in the campaign editor, error
 * checking, etc
 */
void read_mission_goal_list (int num) {
    ...
    try {
        ...
        // first, read the mission notes for this mission.  Used in campaign
        // editor
        if (skip_to_string ("#Mission Info")) {
            if (skip_to_string ("$Notes:")) {
                ...
                Campaign.missions[num].notes =
                    (char*)malloc (strlen (notes) + 1);
                strcpy (Campaign.missions[num].notes, notes);
            }
        }

        event_count = 0;
        // skip to events section in the mission file.  Events come before
        // goals, so we process them first
        if (skip_to_string ("#Events")) {
            while (1) {
                if (skip_to_string ("$Formula:", "#Goals") != 1) { break; }
                ...
                if (z == 1) {
                    ...
                    sprintf (
                        events[event_count], NOX ("Event #%d"),
                        event_count + 1);
                }
        ...
        if (skip_to_string ("#Goals")) {
            while (1) {
                if (skip_to_string ("$Type:", "#End") != 1) { break; }
        ...
        if (count) {
            Campaign.missions[num].goals =
                (mgoal*)malloc (count * sizeof (mgoal));
            ASSERT (
        ...
        if (event_count) {
            Campaign.missions[num].events =
                (mevent*)malloc (event_count * sizeof (mevent));
 ...
 * extension since filenames are stored with the extension in the campaign
 * file.
 *
 * @return index of mission in campaign structure.  -1 if mission name not
 * found.
 */
int mission_campaign_find_mission (char* name) {
    ...
    // look for an extension on the file.  If no extension, add default ".fsm"
...
/**
 * Save persistent information during a mission -- which then might or might
 * not get saved out when the mission is over depending on whether player
 * replays mission or commits.
 */
...
int mission_load_up_campaign (player* pl) {
    ...
    if (rc < 0) {
        ...
        for (idx = 0; (idx < Num_campaigns) && (rc < 0); idx++) {
            if ((Campaign_file_names[idx] == NULL) ||
                !strlen (Campaign_file_names[idx])) {
                continue;
            }
    ...
    if (rc == 0) { strcpy (pl->current_campaign, Campaign.filename); }

    return rc;
}

/**
 * For end of campaign in the single player game.  Called when the end of
 * campaign state is entered, which is triggered when the end-campaign
 * sexpression is hit
 */
...
void mission_campaign_end_do () {
    ...
    // play the movies
    // eventually we'll want to play one of two options (good ending or bad
    // ending)
...
void mission_campaign_skip_to_next (int start_game) {
    ...
    // these do not really matter, since is-previous-event-* and
    // is-previous-goal-* sexps check to see if the mission was skipped, and
    // use defaults accordingly.
    ...
    if (start_game) {
        ...
        if ((Campaign.missions[Campaign.current_mission].flags &
             CMISSION_FLAG_HAS_LOOP) &&
            ...
            if (Campaign.next_mission == -1 ||
                (The_mission.flags[Mission::Mission_Flags::End_to_mainhall])) {
                // go to main hall, either the campaign is over or the FREDer
                // requested it.
                gameseq_post_event (GS_EVENT_MAIN_MENU);
            }
            else {
...
void mission_campaign_jump_to_mission (char* name, bool no_skip) {
    ...
    if (mission_num < 0) {
        ...
        for (std::vector< ship_info >::iterator it = Ship_info.begin ();
             it != Ship_info.end (); it++) {
...
void mission_campaign_save_on_close_variables () {
    ...
    if (!(Campaign.type == CAMPAIGN_TYPE_SINGLE)) return;

    // now save variables
    for (i = 0; i < sexp_variable_count (); i++) {
        // we only want the on mission close type. On campaign progress type
        ...
        if ((Sexp_variables[i].type & SEXP_VARIABLE_SAVE_TO_PLAYER_FILE)) {
            ...
            // if not found then add new entry
            if (!found) { Player->variables.push_back (Sexp_variables[i]); }
        }
    }

    // store any non-eternal on mission close variables
    mission_campaign_store_variables (
        SEXP_VARIABLE_SAVE_ON_MISSION_CLOSE, false);
}

void mission_campaign_load_failure_popup () {
    if (Campaign_load_failure == 0) { return; }
    ...
    if (Campaign_load_failure == CAMPAIGN_ERROR_CORRUPT) {
        popup (
            ...
            1, POPUP_OK,
            XSTR (
                "Error!\n\nRequested campaign is corrupt and cannot be "
                "loaded.\n\n"
                "Please select a different campaign in the Campaign Room.",
                1614));
    }
        ...
        popup (
            ...
            1, POPUP_OK,
            XSTR (
                "Error!\n\nRequested campaign requires too many SEXPs and "
                "cannot be loaded.\n\n"
                "Please select a different campaign in the Campaign Room.",
                1615));
    }
    else if (Campaign_load_failure == CAMPAIGN_ERROR_MISSING) {
        // if it's just the campaign missing, there's another popup to deal
        // with that
        ;
    }
        ...
        popup (
            ...
            XSTR (
                ...
                "select another campaign or switch to the correct "
                "mod in order to use this campaign.",
                1617));
    }
