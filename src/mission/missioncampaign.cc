        ...
        char **desc)
{
        ...
        if ((fname_len < 4) || strcasecmp(fname + fname_len - 4, FS_CAMPAIGN_FILE_EXT) != 0) {
                ...
                while (incr > 0) {
                        for (i = incr; i < Num_campaigns; i++) {
                                ...
                                while (j >= 0) {
                                        ...
                                        if (!strncasecmp(name1, "the ", 4))
                                                name1 += 4;

                                        if (!strncasecmp(name2, "the ", 4))
                                                name2 += 4;
...
void mission_campaign_store_goals_and_events()
{
        ...
        for (i = 0; i < Num_goals; i++) {
                ...
                ASSERT(
                        Mission_goals[i].satisfied != GOAL_INCOMPLETE); // should be true or false at this point!!!
        ...
        if (mission_obj->num_events > 0) {
                mission_obj->events = (mevent *)malloc(sizeof(mevent) * Num_mission_events);
        ...
        if (num_mission_variables > 0) {
                ...
                for (i = 0; i < sexp_variable_count(); i++) {
                        ...
                        // we might need to save some eternal variables
                                ...
                                (persistence_type & SEXP_VARIABLE_SAVE_ON_MISSION_PROGRESS) && (Sexp_variables[i].type & persistence_type) && (Sexp_variables[i].type & SEXP_VARIABLE_SAVE_TO_PLAYER_FILE)) {
                                bool add_it = true;
...
int mission_campaign_find_mission(char *name)
{
        ...
        for (i = 0; i < Campaign.num_missions; i++) {
                if (!strcasecmp(realname, Campaign.missions[i].name)) {
                        return i;
                }
        }
...
int mission_campaign_parse_is_multi(char *filename, char *name)
{
        ...
        try {
                ...
                for (i = 0; i < MAX_CAMPAIGN_TYPES; i++) {
                        if (!strcasecmp(temp, campaign_types[i])) {
                                return i;
                        }
                }
...
int mission_load_up_campaign(player *pl)
{
        ...
        if (rc == 0) {
                strcpy(pl->current_campaign, Campaign.filename);
        }
...
void mission_campaign_jump_to_mission(char *name, bool no_skip)
{
        ...
        for (i = 0; i < Campaign.num_missions; i++) {
                if ((Campaign.missions[i].name != NULL) && !strcasecmp(Campaign.missions[i].name, dest_name)) {
                        mission_num = i;
                        break;
                } else if (!no_skip) {
