...
int goal_text::add(const char *text)
{
        ...
        count = split_str(
                text,
                Goal_screen_text_w - Goal_screen_text_coords[gr_screen.res][GOAL_SCREEN_X_COORD] + Goal_screen_icon_xcoord[gr_screen.res],
...
void mission_process_event(int event)
{
        ...
        if (sindex >= 0) {
                ...
                if ((Directive_count == DIRECTIVE_WING_ZERO) && !(Mission_events[event].flags & MEF_DIRECTIVE_SPECIAL)) {
                        ...
                        (Mission_events[event].flags & MEF_DIRECTIVE_SPECIAL) && Directive_count > 1) {
                        // make it non special
        ...
        if (result || timestamp_valid(Mission_events[event].timestamp)) {
                ...
                // trigger this frame.
                        ...
                        bump_timestamp || (!((Mission_events[event].repeat_count == -1) && (Mission_events[event].flags & MEF_USING_TRIGGER_COUNT) && (Mission_events[event].trigger_count != 0)))) {
