...
const char* Brief_static_name[GR_NUM_RESOLUTIONS] = { "BriefMap",
                                                      "2_BriefMap" };
...
int Brief_bmap_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                   0, 115 },
                                                 { // GR_1024
                                                   0, 184 } };

int Brief_grid_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                   19, 147, 555, 232 },
                                                 { // GR_1024
                                                   30, 235, 888, 371 } };

int Brief_text_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                   28, 399, 395, 74 },
                                                 { // GR_1024
                                                   46, 637, 630, 120 } };

int Brief_stage_text_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                         138, 117 },
                                                       { // GR_1024
                                                         227, 194 } };

int Brief_stage_text_coords_multi[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                               479, 385 },
                                                             { // GR_1024
                                                               821, 616 } };

int Brief_text_max_lines[GR_NUM_RESOLUTIONS] = { 8, 13 };

#define LOOKAT_DIST 500.0f
#define STAGE_ADVANCE_DELAY \
    1000 // time in ms to wait after voice stops before advancing stage

extern const float BRIEF_TEXT_WIPE_TIME =
    1.5f; // time in seconds for wipe to occur

// --------------------------------------------------------------------------------------
// Game-wide global data
// --------------------------------------------------------------------------------------
briefing Briefings[MAX_TVT_TEAMS]; // there is exactly one briefing per mission
debriefing Debriefings[MAX_TVT_TEAMS]; // there can be multiple debriefings per
                                       // mission
...
int Briefing_voice_enabled =
    1; // flag which turn on/off voice playback of briefings/debriefings
...
// flag to indicate that the sound for a spinning highlight animation has
// played
static sound_handle Brief_stage_highlight_sound_handle =
...
// Used to support drawing colored text for the briefing.  Gets complicated
// since we need to be able to draw one character at a time as well when the
// briefing text first appears.
...
static sound_handle
    Brief_text_wipe_snd; // sound handle of sound effect for text wipe
...
void brief_maybe_create_new_grid (
...
void brief_parse_icon_tbl () {
    ...
    try {
        ...
        const size_t num_species_covered =
            Briefing_icon_info.size () / MIN_BRIEF_ICONS;
        size_t bii_index = 0;
        ...
        if (num_species_covered < Species_info.size ()) {
            ...
            for (species = num_species_covered; species < Species_info.size ();
                 species++) {
...
void brief_move_icon_reset () {
    ...
    for (i = 0; i < MAX_MOVING_ICONS; i++) Icon_movers[i].used = 0;
}

/**
 * Does one time initialization of the briefing and debriefing structures.
 * Namely setting all malloc'ble pointers to NULL.  Called once at game
 * startup.
 */
void mission_brief_common_init () {
    ...
    gr_init_alphacolor (
        &Brief_color_legacy_neutral, 255, 0, 0, iff_get_alpha_value (false));
...
/**
 * Frees all the memory allocated in the briefing and debriefing structures and
...
/**
 * Set up the screen regions.  A mulitplayer briefing will look different than
 * a single player briefing.
 */
void brief_init_screen (int /*multiplayer_flag*/) {
    bscreen.map_x1 = Brief_grid_coords[gr_screen.res][0];
    bscreen.map_x2 = Brief_grid_coords[gr_screen.res][0] +
                     Brief_grid_coords[gr_screen.res][2];
...
briefing_icon_info* brief_get_icon_info (brief_icon* bi) {
    if (bi->ship_class < 0) return NULL;
    ...
    if (sip->bii_index_ship >= 0) {
        ...
        // this will be reached if we just want the plain ship icon, or if we
        // specified icon modifiers which didn't exist
        return &Briefing_icon_info[sip->bii_index_ship];
    }

    if (sip->species < 0) return NULL;

    int bii_index = Species_info[sip->species].bii_index[bi->type];
    if (bii_index < 0) return NULL;
...
void brief_render_fade_outs (float frametime) {
    ...
    for (i = 0; i < Num_fade_icons; i++) {
        ...
        if (!(tv.flags & PF_PROJECTED)) g3_project_vertex (&tv);

        if (!(tv.flags &
              PF_OVERFLOW)) { // make sure point projected before drawing text

            brief_set_icon_color (fi->team);

            if (fi->fade_anim.first_frame < 0) { continue; }
            ...
            gr_unsize_screen_posf (
                &screenX, &screenY, NULL, NULL, GR_RESIZE_MENU_NO_OFFSET);
            ...
            if (fi->fade_anim.first_frame >= 0) {
                ...
                hud_anim_render (
                    &fi->fade_anim, frametime, 1, 0, 0, 0, GR_RESIZE_MENU);
            }
        }
    }
}
...
float brief_icon_get_dist_moved (icon_move_info* mi, float elapsed_time) {
    ...
    if (elapsed_time < mi->total_move_time / 2.0f) {
        dist_moved =
            0.5f * mi->accel * elapsed_time * elapsed_time; // d = 1/2at^2
        return dist_moved;
    }

    // second half of movement
    time = elapsed_time - mi->total_move_time / 2.0f;
    dist_moved = (mi->total_dist / 2.0f) + (mi->peak_speed * time) -
                 0.5f * mi->accel * time * time;
    return dist_moved;
}
...
void brief_render_icon_line (int stage_num, int line_num) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (!(icon_vertex[i].flags &
              PF_OVERFLOW)) { // make sure point projected before drawing text
            icon_status[i] = 1;
        }
    }

    if (!icon_status[0] || !icon_status[1]) { return; }
 ...
 */
void brief_render_icon (
    int stage_num, int icon_num, float frametime, int selected,
    ...
    if (!interp_pos_found) pos = &bi->pos;
    ...
    if (!(tv.flags &
        ...
        if ((bi->flags & BI_SHOWHIGHLIGHT) && (bi->flags & BI_HIGHLIGHT)) {
            ...
            if (ha->first_frame >= 0) {
                ...
                if (bi->label[0] != '\0') {
                    ha->sy = bi->hold_y -
                             (int)std::lround (gr_get_font_height () / 2.0f) -
                             2;
                }
                else {
                    ha->sy = bi->hold_y;
        ...
        if (bi->flags & BI_FADEIN) {
            ...
            if (ha->first_frame >= 0) {
                ...
                if (hud_anim_render (
                        ha, frametime, 1, 0, 0, 1, GR_RESIZE_MENU,
                        mirror_icon) == 0) {
                    bi->flags &= ~BI_FADEIN;
                }
            }
...
void brief_start_highlight_anims (int stage_num) {
    ...
    for (i = 0; i < bs->num_icons; i++) {
        ...
        if (bi->flags & BI_HIGHLIGHT) {
            ...
            if (bi->highlight_anim.first_frame < 0) { continue; }
...
void brief_render_map (int stage_num, float frametime) {
    gr_set_clip (
        bscreen.map_x1 + 1, bscreen.map_y1 + 1,
        bscreen.map_x2 - bscreen.map_x1 - 1,
        bscreen.map_y2 - bscreen.map_y1 - 2, GR_RESIZE_MENU);
    ...
    g3_start_frame (0);
    g3_set_view_matrix (
        &Current_cam_pos, &Current_cam_orient, Briefing_window_FOV);

    brief_maybe_create_new_grid (
        The_grid, &Current_cam_pos, &Current_cam_orient);
...
void brief_blit_stage_num (int stage_num, int stage_max) {
    ...
    if (Game_mode & GM_MULTIPLAYER) {
        gr_printf_menu (
            Brief_stage_text_coords_multi[gr_screen.res][0],
            Brief_stage_text_coords_multi[gr_screen.res][1], "%s", buf);
    }
    else {
        gr_printf_menu (
            Brief_stage_text_coords[gr_screen.res][0],
            Brief_stage_text_coords[gr_screen.res][1], "%s", buf);
    }
}

/**
 * Render a line of text for the briefings.  Lines are drawn in as a wipe, with
 * leading bright white characters.  Have to jump through some hoops since we
 * support colored words.  This means that we need to process the line one
 * character at a time.
 *
...
void brief_render_line (int line_num, int x, int y, int instance) {
    ...
    // truncate_len is going to be the number of characters displayed with
    // normal intensity bright_len is the additional characters displayed with
    // high intensity bright_len+truncate_len<len chars are displayed
    size_t bright_len = 0;
    ...
    size_t char_seq_pos =
        0; // Cursor position into the following character sequence
    char char_seq[MAX_BRIEF_LINE_LEN];
    int offset = 0; // offset is the horizontal position of the screen where
                    // strings are drawn
    gr_set_color_fast (&Color_white);

    // PART1: Draw the the briefing line part with normal intensity and word
    // colors. The following algorithm builds a character sequence of color
    // 'last_color' into 'line' buffer. When the color changes, the buffer is
    // drawn and reset.
    {
        char last_color = src->at (0).color;
        for (size_t current_pos = 0; current_pos < truncate_len;
             current_pos++) {
            colored_char& current_char = src->at (current_pos);
            // when the current color changes, the accumulated character
            // sequence is drawn.
...

    {
        ...
        char_seq_pos = 0;
        for (size_t current_pos = truncate_len;
 ...
 * @return
 */
int brief_render_text (
    int line_offset, int x, int y, int h, float frametime, int instance,
    int line_spacing) {
    int fh, line, yy;
    ...
    yy = 0;
    while (yy + fh <= h) {
        if (line >= Num_brief_text_lines[instance]) break;
...
void brief_render_elements (vec3d* pos, grid* gridp) {
    ...
    if (pos->xyz.y < 1 && pos->xyz.y > -1) return;
    ...
    if (gv->xyz.x * pos->xyz.x + gv->xyz.y * pos->xyz.y +
            gv->xyz.z * pos->xyz.z <
        -gridp->planeD)
...
void brief_set_camera_target (vec3d* pos, matrix* orient, int time) {
    ...
    Start_cam_pos = Current_cam_pos; // we need this when checking if camera
                                     // movement complete
    Start_cam_move =
        timer_get_milliseconds () * 1000.0f; // start time, convert to seconds
    Total_move_time = time_in_seconds;
    Elapsed_time = 0.0f;

    vm_vec_scale_add (
        &Start_lookat_pos, &Start_cam_pos, &Start_cam_orient.vec.fvec,
        LOOKAT_DIST);
    vm_vec_scale_add (
        &Target_lookat_pos, &Target_cam_pos, &Target_cam_orient.vec.fvec,
        LOOKAT_DIST);
    ...
    anim_release_all_instances (
        ON_BRIEFING_SELECT); // stop any briefing-specific anims

    // calculate camera velocity
    vm_vec_sub (&Cam_vel, pos, &Current_cam_pos);

    if (!IS_VEC_NULL_SQ_SAFE (&Cam_vel)) { vm_vec_normalize (&Cam_vel); }
    ...
    get_camera_limits (
        &Start_cam_orient, &Target_cam_orient, Total_move_time, &Acc_limit,
        &Vel_limit);
}

bool brief_verify_color_tag (unicode::codepoint_t color_tag) {
    if (color_tag > 127) {
        // The tag will always be out of range. This is done to be sure that
        // the case below does not accidentally produce a valid tag character.
        std::string tag_str;
 ...
 * @return true when the given character is a word separator, and false when
 * the character is part of a word.
 */
bool is_a_word_separator (unicode::codepoint_t character) {
    return character <=
           32; // all control characters including space, newline, and tab
}
 ...
 *   '$' + a char standing for a color + contigous multiple spaces (chars t n
 * and ' ') The markup is completely removed from the resulting character
 * sequence.
 *
 ...
 * @param[in,out] default_color_stack pointer to an array containing a stack of
 * default colors (for color spans)
 * @param[in,out] color_stack_index pointer to the current index in the above
 * stack
...
int brief_text_colorize (
    ...
    for (auto iter = std::begin (range); iter != end_iter; ++iter) {
        ...
        // text markup consists of a '$' plus a character plus an optional
        // space
        if (iter + 1 != end_iter && *iter == BRIEF_META_CHAR) {
            ...
            // normal $c or $c{
                ...
                ++iter; // Consume the color identifier and focus on the white
                        // character (if any)
            ...
            while ((iter != end_iter) && is_white_space (*iter)) ++iter;

            // The next character is not a whitespace, let's process it as
            // usual (subtract 1 because the for loop will add it again)
            --iter;
            continue;
        }
 ...
 * @param[in] append add on to the existing lines instead of replacing them
 * (defaults to false)
 */
int brief_color_text_init (
    const char* src, int w, const char default_color, int instance,
    int max_lines, const bool append) {
    int i, n_lines, len;
    ...
    if (default_color == '\0' ||
        !brief_verify_color_tag (
            default_color)) { // call brief_verify_color_tag() to make sure our
                              // default is actually a defined color tag
    ...
    // fiction viewer
    if ((max_lines > 0) && (n_lines > max_lines)) { n_lines = max_lines; }
    ...
    for (i = 0; i < n_lines; i++) {
        ...
        len = brief_text_colorize (
            &tmp_brief_line[0], instance, default_color_stack,
            color_stack_index);
        if (len > Max_briefing_line_len) Max_briefing_line_len = len;
    }

    Brief_text_wipe_time_elapsed = 0.0f;
    Play_brief_voice = 0;

    if (append) { Num_brief_text_lines[instance] += n_lines; }
    else {
...
int brief_get_free_move_icon () {
    ...
    for (i = 0; i < MAX_MOVING_ICONS; i++) {
        if (Icon_movers[i].used == 0) break;
    }

    if (i == MAX_MOVING_ICONS) return -1;
        ...
        for (j = 0; j < newb->num_icons; j++) {
            if ((cb->icons[i].id != 0) &&
                ...
                if (vm_vec_cmp (&cb->icons[i].pos, &newb->icons[j].pos)) {
                    ...
                    imi->peak_speed =
                        imi->total_dist / imi->total_move_time * 2.0f;
...
void brief_clear_fade_out_icons () { Num_fade_icons = 0; }
...
void brief_set_new_stage (
    ...
    if (stage_num == Last_new_stage) { return; }

    num_movers = 0;
    brief_move_icon_reset ();
    brief_clear_fade_out_icons ();
    if ((Last_new_stage != -1) && not_objv) {
        num_movers = brief_set_move_list (
            stage_num, Last_new_stage, new_time / 1000.0f);
    }

    if ((Last_new_stage != -1) && (num_movers == 0) && not_objv) {
        if (!vm_vec_cmp (
                &Briefing->stages[stage_num].camera_pos,
                &Briefing->stages[Last_new_stage].camera_pos)) {
            if (!vm_vec_cmp (
                    &Briefing->stages[stage_num].camera_orient.vec.fvec,
                    &Briefing->stages[Last_new_stage]
    ...
    if (not_objv) { msg = Briefing->stages[stage_num].text.c_str (); }
    else {
    ...
    if (gr_screen.res == GR_640) {
        // GR_640
        Num_brief_text_lines[0] = brief_color_text_init (
            msg, MAX_BRIEF_LINE_W_640, default_briefing_color);
    }
    else {
        // GR_1024
        Num_brief_text_lines[0] = brief_color_text_init (
            msg, MAX_BRIEF_LINE_W_1024, default_briefing_color);
    }

    Top_brief_text_line = 0;

    if (not_objv) { brief_set_camera_target (pos, orient, new_time); }
 ...
 * elapsed_time/total_time gives percentage of interpolation between cur and
 * goal.
 */
void interpolate_matrix (
    matrix* result, matrix* goal, matrix* start, float elapsed_time,
    float total_time) {
    vec3d fvec, rvec;
    float time0, time1;

    if (!vm_matrix_cmp (goal, start)) { return; }
...
float brief_camera_get_dist_moved (float elapsed_time) {
    ...
    if (elapsed_time < Total_move_time / 2.0f) {
        dist_moved =
            0.5f * Cam_accel * elapsed_time * elapsed_time; // d = 1/2at^2
        return dist_moved;
    }

    // second half of movement
    time = elapsed_time - Total_move_time / 2.0f;
    dist_moved = (Total_dist / 2.0f) + (Peak_speed * time) -
                 0.5f * Cam_accel * time * time;
    return dist_moved;
}
...
void brief_camera_move (float frametime, int /*stage_num*/) {
    ...
    if (Cam_target_reached) return;

    // Update orientation
    if ((Elapsed_time < Total_move_time)) {
        vm_matrix_interpolate (
            &Target_cam_orient, &Current_cam_orient, &W_init, frametime,
            &result, &w_out, &Vel_limit, &Acc_limit);
...
void brief_maybe_create_new_grid (
    ...
    if (fvi_ray_plane (
            &gpos, &gridp->center, &gridp->gmatrix.vec.uvec, pos,
            &orient->vec.fvec, 0.0f) < 0.0f) {
        vec3d p;
...
grid* brief_create_default_grid (void) {
    ...
    rgrid =
        brief_create_grid (&Global_grid, &fvec, &rvec, &cvec, 100, 100, 5.0f);
...
void brief_modify_grid (grid* gridp) {
    brief_create_grid (
        gridp, &gridp->gmatrix.vec.fvec, &gridp->gmatrix.vec.rvec,
        &gridp->center, gridp->nrows, gridp->ncols, gridp->square_size);
}

void brief_unload_anims () {
    for (std::vector< briefing_icon_info >::iterator ii =
             Briefing_icon_info.begin ();
         ii != Briefing_icon_info.end (); ++ii) {
...
void brief_common_close () { brief_unload_anims (); }
...
void brief_voice_init () {
    int i;
    for (i = 0; i < MAX_BRIEF_STAGES; i++) { Brief_voices[i] = -1; }
}
...
void brief_load_voice_file (int voice_num, char* name) {
    ...
    while (1) {
        ...
        if (Brief_voices[voice_num] >= 0) { break; }
...
void cmd_brief_reset () {
    ...
    inited = 1;
    for (i = 0; i < MAX_TVT_TEAMS; i++) Cmd_briefs[i].num_stages = 0;
}
...
int brief_time_to_advance (int stage_num) {
    if (brief_get_closeup_icon () != NULL) return 0;

    if (Briefing_paused) return 0;

    if (!Player->auto_advance) return 0;

    if (!brief_text_wipe_finished ()) return 0;

    if (Voice_ended_time &&
        (timer_get_milliseconds () - Voice_ended_time >= STAGE_ADVANCE_DELAY))
        return 1;
    ...
    // if we get here, there is no voice, so we simulate the time it would take
    // instead
    if (!Voice_ended_time)
        Voice_ended_time =
            Voice_started_time + MAX (5000, Num_brief_text_lines[0] * 3500);
