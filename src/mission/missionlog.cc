        ...
        LogType type, const char *pname, const char *sname, int info_index)
{
        ...
        case LOG_SELF_DESTRUCTED:
                ...
                if ((type == LOG_SHIP_DOCKED) || (type == LOG_SHIP_UNDOCKED)) {
                        ...
                        (type == LOG_SHIP_SUBSYS_DESTROYED) && (Ship_info[Ships[index].ship_info_index].is_small_ship())) {
        ...
        for (i = 0; i < last_entry; i++, entry++) {
                ...
                if (entry->type == type) {
                        ...
                        if ((type == LOG_SHIP_DOCKED) || (type == LOG_SHIP_UNDOCKED)) {
                                ...
                                if ((!strcasecmp(entry->pname, pname) && !strcasecmp(entry->sname, sname)) || (!strcasecmp(entry->pname, sname) && !strcasecmp(entry->sname, pname))) {
                                        found = 1;
                                }
                        } else {
                                ...
                                if (strcasecmp(entry->pname, pname) != 0) {
                                        continue;
                                }
                                ...
                                if ((type == LOG_SHIP_SUBSYS_DESTROYED || type == LOG_CAP_SUBSYS_CARGO_REVEALED)) {
                                        if ((sname == NULL) || !subsystem_strcasecmp(sname, entry->sname)) {
                                                found = 1;
                                        }
                                } else {
                        ...
                        if (found) {
                                ...
                                if (!count) {
                                        entry->flags |= MLF_ESSENTIAL; // since the goal code asked for this
...
void message_log_init_scrollback(int pw)
{
        ...
        for (i = 0; i < last_entry; i++) {
                ...
                if ((Lcl_gr) && ((entry->type == LOG_GOAL_FAILED) || (entry->type == LOG_GOAL_SATISFIED))) {
                        ...
                        (Lcl_pl) && ((entry->type == LOG_GOAL_FAILED) || (entry->type == LOG_GOAL_SATISFIED))) {
