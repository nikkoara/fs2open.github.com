...
int add_avi(char *avi_name)
{
        ...
        for (i = 0; i < (int)Message_avis.size(); i++) {
                if (!strcasecmp(Message_avis[i].name, avi_name))
                        return i;
        }
...
int add_wave(const char *wave_name)
{
        ...
        for (i = 0; i < (int)Message_waves.size(); i++) {
                if (!strcasecmp(Message_waves[i].name, wave_name))
                        return i;
        }
...
void message_parse(bool importing_from_fsm)
{
        ...
        if (optional_string("$Mood:")) {
                ...
                for (std::vector< std::string >::iterator iter = Builtin_moods.begin();
...
int message_get_priority(int which)
{
        ...
        for (i = 0; i < Num_messages_playing; i++) {
                if ((which == MESSAGE_GET_HIGHEST) && (Playing_messages[i].priority > priority)) {
                        ...
                        (which == MESSAGE_GET_LOWEST) && (Playing_messages[i].priority < priority)) {
                        priority = Playing_messages[i].priority;
                }
        }
...
bool message_filename_is_generic(char *filename)
{
        ...
        if ((ptr == NULL) || (strcasecmp(ptr, ".ogg") != 0 && strcasecmp(ptr, ".wav") != 0))
                return false;
...
void message_play_anim(message_q *q)
{
        ...
        if ((!anim_info->exists) && // if the base animation doesn't exist, then a,
            ...
            ((q->message_num < Num_builtin_messages) || !(strncasecmp(HEAD_PREFIX_STRING, ani_name, strlen(HEAD_PREFIX_STRING) - 1)))) {
                ...
                if (persona_index >= 0) {
                        if (Personas[persona_index].flags & (PERSONA_FLAG_WINGMAN | PERSONA_FLAG_SUPPORT)) {
                                ...
                                if (q->builtin_type == MESSAGE_WINGMAN_SCREAM) {
                                        rand_index = MAX_WINGMAN_HEADS; // [0,MAX) are regular heads; MAX is
                                                                        // always death head
                                        is_death_scream = 1;
                                } else {
                                ...
                                Personas[persona_index].flags & (PERSONA_FLAG_COMMAND | PERSONA_FLAG_LARGE)) {
                                // get a random head
                                // Goober5000 - *sigh*... if mission designers assign a command
                                // persona to a wingman head, they risk having the death ani
                                // play
                                if (!strncasecmp(ani_name, "Head-TP", 7) || !strncasecmp(ani_name, "Head-VP", 7)) {
...
void message_queue_process()
{
                    ...
                    anim_stop_playing( Playing_messages[i].anim );
                        ...
                        if (Playing_messages[i].shipnum != -1) {
                                if ((Ships[Playing_messages[i].shipnum]
                                    ...
                                    && (Playing_messages[i].builtin_type != MESSAGE_WINGMAN_SCREAM)) {
                                        int shipnum;
                        ...
                        // brackets on hud
                        if (wave_done && ani_done && (timestamp_elapsed(Message_expire) || (Playing_messages[i].wave.isValid()) || (Playing_messages[i].shipnum == -1))) {
        ...
        while (MessageQ_num > 0) {
                ...
                if (timestamp_valid(q->window_timestamp) && timestamp_elapsed(q->window_timestamp) && !q->group) {
                        ...
                        if (q->message_num < Num_builtin_messages) { // we should only ever remove builtin
                                                                     // messages this way
        ...
        if (Num_messages_playing) {
                ...
                if (q->builtin_type == MESSAGE_HAMMER_SWINE) {
                        ...
                        message_playing_builtin() && (q->message_num >= Num_builtin_messages) && (q->priority > MESSAGE_PRIORITY_LOW)) {
                        ...
                        message_playing_builtin() && (q->message_num < Num_builtin_messages)) {
                        ...
                        message_playing_unique() && (q->message_num < Num_builtin_messages)) {
        ...
        if (Message_shipnum >= 0) {
                ...
                if (shipp->callsign_index >= 0) {
                        ...
                        ((Iff_info[shipp->team].flags & IFFF_WING_NAME_HIDDEN) && (shipp->wingnum != -1)) || (shipp->flags[Ship::Ship_Flags::Hide_ship_name])) {
        ...
        int source, int group, int delay, int builtin_type)
{
        ...
        MessageQ[i].flags = 0;
        if (!strcasecmp(who_from, The_mission.command_sender) && (m_persona != -1) && (Personas[m_persona].flags & PERSONA_FLAG_WINGMAN)) {
        ...
        int /* multi_target */, int multi_team_filter)
{
        ...
        for (i = 0; i < Num_builtin_messages; i++) {
                ...
                if (!strcasecmp(Messages[i].name, name)) {
                        ...
                        if (!Messages[i].excluded_moods.empty() && (current_builtin.type_of_match == BUILTIN_MATCHES_PERSONA_CHECK_MOOD)) {
                                for (std::vector< int >::iterator iter = Messages[i].excluded_moods.begin();
        ...
        if (!strcasecmp(who_from, The_mission.command_sender) && (type == MESSAGE_REARM_ON_WAY)) {
...
int message_persona_name_lookup(char *name)
{
        ...
        for (i = 0; i < Num_personas; i++) {
                if (!strcasecmp(Personas[i].name, name))
                        return i;
        }
        ...
        for (i = 0; i < Num_messages_playing; i++) {
                ...
                if (comm_between_player_and_ship(Playing_messages[i].shipnum) != COMM_OK) {
                        ...
                        if (timestamp_elapsed(Next_mute_time)) {
                                Next_mute_time = int(
                                        Distort_patterns[Distort_num][Distort_next++] * Message_wave_duration);
    ...
    */
        {
                if (player_comm_state == COMM_OK && other_comm_state == COMM_OK)
                        ...
                        player_comm_state == COMM_SCRAMBLED || other_comm_state == COMM_SCRAMBLED)
