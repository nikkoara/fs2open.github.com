...
// here is the list of the builtin message names and the settings which control
// how frequently they are heard.  These names are used to match against names
// read in for builtin message radio bits to see what message to play. These
// are generic names, meaning that there will be the same message type for a
...
int Message_shipnum; // ship number of who is sending message to player -- used
                     // outside this module
int Message_expire;  // timestamp to extend the duration of message brackets
                     // when not using voice files

// variables to control message queuing.  All new messages to the player are
// queued.  The array will be ordered by priority, then time submitted.
...
typedef struct message_q {
    ...
    int window_timestamp;  // timestamp which will tell us how long we have to
                           // play the message
    int priority;          // priority of the message
    int message_num;       // index into the Messages[] array
    char* special_message; // Goober5000 - message to play if we've replaced
                           // stuff (like variables)
    ...
    int flags; // should this message entry be converted to Terran Command
               // head/wave file
    int min_delay_stamp; // minimum delay before this message will start
                         // playing
...
#define MESSAGE_SOON_TIMESTAMP \
    5000 // "soon" messages must play within 5 seconds
...
int Head_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                             7, 45 },
                                           { // GR_1024
                                             7, 66 } };
...
// following functions to parse messages.tbl -- code pretty much ripped from
// weapon/ship table parsing code
...
// functions to deal with parsing personas.  Personas are just a list of names
// that give someone sending a message an identity which spans the life of the
// mission
void persona_parse () {
    int i;
...
int add_avi (char* avi_name) {
    ...
    extra.exists =
        (generic_anim_load (&extra.anim_data) ==
         0);                                // load only to validate the anim
...
void message_parse (bool importing_from_fsm) {
    ...
    if (optional_string ("$Exclude Mood:")) {
        ...
        for (std::vector< std::string >::iterator parsed_moods = buff.begin ();
             parsed_moods != buff.end (); ++parsed_moods) {
            for (std::vector< std::string >::iterator iter =
                     Builtin_moods.begin ();
                 iter != Builtin_moods.end (); ++iter) {
...
void message_frequency_parse () {
    ...
    if (optional_string ("+Occurrence Chance:")) {
        ...
        if ((occurrence_chance >= 0) && (occurrence_chance <= 100)) {
            Builtin_messages[builtin_type].occurrence_chance =
                occurrence_chance;
        }
    }
...
void parse_msgtbl () {
    ...
    // speed things up a little by setting the capacities for the message
    // vectors to roughly the FS2 amounts
    ...
    try {
        ...
        if (pVawacs25) {
            ...
            if (pAwacs75) {
                // move the 'V' from the first filename to the second, and
                // adjust the 'A' case
                *pVawacs25 = 'A';
        ...
        if (optional_string ("#Message Frequencies")) {
            while (
                !required_string_one_of (3, "$Name:", "#Personas", "#Moods")) {
                message_frequency_parse ();
            }
        }

        Builtin_moods.push_back ("Default");
        if (optional_string ("#Moods")) { message_moods_parse (); }
        ...
        while (required_string_either ("#End", "$Name:")) { message_parse (); }

        required_string ("#End");

        // save the number of builtin message things -- make initing between
        // missions easier
        ...
        memset (
            Valid_builtin_message_types, 0,
            sizeof (int) * MAX_BUILTIN_MESSAGE_TYPES);
        // now cycle through the messages to determine which type of builtins
...
void messages_init () {
    ...
    // this forces a reload of the AVI's and waves for builtin messages. Needed
    // because the flic and sound system also get reset between missions!
    ...
    Messages.erase (
        (Messages.begin () + Num_builtin_messages), Messages.end ());
    Message_avis.erase (
        (Message_avis.begin () + Num_builtin_avis), Message_avis.end ());
    Message_waves.erase (
        (Message_waves.begin () + Num_builtin_waves), Message_waves.end ());
}

// free a loaded avi
void message_mission_free_avi (int m_index) {
    // check for bogus index
    if ((m_index < 0) || (m_index >= Num_message_avis)) return;
...
// function to kill all currently playing messages.  kill_all parameter tells
// us to kill only the animations that are playing, or wave files too
void message_kill_all (int kill_all) {
    ...
    for (i = 0; i < Num_messages_playing; i++) {
        /*if ( (Playing_messages[i].anim != NULL) &&
        anim_playing(Playing_messages[i].anim) ) { anim_stop_playing(
        Playing_messages[i].anim ); Playing_messages[i].anim=NULL;
        }*/
...
void message_kill_playing (int message_num) {
    ...
    Playing_messages[message_num].anim );
    ...
    if ((Playing_messages[message_num].wave.isValid ()) &&
...
int message_playing_builtin () {
    ...
    for (i = 0; i < Num_messages_playing; i++) {
        if (Playing_messages[i].id >= Num_builtin_messages) { break; }
    }

    // if we got through the list without breaking, all playing messages are
    // builtin messages
    if (i == Num_messages_playing) { return 1; }
    else {
        return 0;
    }
}
...
int message_playing_specific_builtin (int builtin_type) {
    ...
    for (i = 0; i < Num_messages_playing; i++) {
        if ((Playing_messages[i].id < Num_builtin_messages) &&
            (Playing_messages[i].builtin_type == builtin_type)) {
            return 1;
        }
    }
...
int message_get_priority (int which) {
    ...
    if (which == MESSAGE_GET_HIGHEST) { priority = MESSAGE_PRIORITY_LOW; }
    else {
    ...
    for (i = 0; i < Num_messages_playing; i++) {
        if ((which == MESSAGE_GET_HIGHEST) &&
            (Playing_messages[i].priority > priority)) {
            priority = Playing_messages[i].priority;
        }
        else if (
            (which == MESSAGE_GET_LOWEST) &&
            (Playing_messages[i].priority < priority)) {
            priority = Playing_messages[i].priority;
        }
    }
...
void message_remove_from_queue (message_q* q) {
    ...
    if (MessageQ_num > 0) {
        insertion_sort (
            MessageQ, MAX_MESSAGE_Q, sizeof (message_q),
            message_queue_priority_compare);
    }
}
...
bool message_play_wave (message_q* q) {
    ...
    if (m->wave_info.index >= 0) {
        ...
        if (q->flags & MQF_CONVERT_TO_COMMAND) {
            ...
            Message_waves[index].num =
                sound_load_id::invalid (); // forces us to reload the message

            // bash the filename here. Look for "[1-6]_" at the front of the
        ...
        if (!Message_waves[index].num.isValid ()) { m->wave_info.index = -1; }

        if (m->wave_info.index >= 0) {
            // this call relies on the fact that snd_play returns -1 if the
            // sound cannot be played
            Message_wave_duration =
                snd_get_duration (Message_waves[index].num);
            Playing_messages[Num_messages_playing].wave =
                snd_play_raw (Message_waves[index].num, 0.0f);
...
void message_calc_anim_start_frame (int time, generic_anim* ani, int reverse) {
    ...
    if (wave_time >= (anim_time)) { return; }

    float fps = ani->num_frames / ani->total_time;
    if (reverse) {
        start_frame =
            (ani->num_frames - 1) - (int)std::lround (fps * wave_time);
    }
    else {
        int num_frames_extra;
        num_frames_extra = (int)std::lround (fps * (anim_time - wave_time));
        if (num_frames_extra > 0) { start_frame = rand () % num_frames_extra; }
    }
...
void message_play_anim (message_q* q) {
    ...
    if (!hud_gauge_active (HUD_TALKING_HEAD)) { return; }

    m = &Messages[q->message_num];

    // check to see if the avi_index is valid -- try and load/play the avi if
    // so.
    if (m->avi_info.index < 0) { return; }
    ...
    // builtin messages are given a base ani which we should add a suffix on
    // before trying to load the animation.  See if this message is a builtin
    // message which has a persona attached to it.  Deal with munging the name
    ...
    if ((!anim_info->exists) && // if the base animation doesn't exist, then a,
        ...
        // if this ani should be converted to a terran command, set the persona
        ...
        // persona was assigned, the logic will drop down below like it's
        // supposed to
        if (persona_index >= 0) {
            if (Personas[persona_index].flags &
                (PERSONA_FLAG_WINGMAN | PERSONA_FLAG_SUPPORT)) {
                // get a random head
                if (q->builtin_type == MESSAGE_WINGMAN_SCREAM) {
                    rand_index =
                        MAX_WINGMAN_HEADS; // [0,MAX) are regular heads; MAX is
                                           // always death head
                    is_death_scream = 1;
                }
                else {
    ...
    // an error loading the avi, set the top level index to -1 to avoid
    // multiple tries at loading the flick.
    ...
    if (m->avi_info.index >= 0) {
        // This call relies on the fact that AVI_play will return -1 if the AVI
        // cannot be played if any messages are already playing, kill off any
        ...
        Playing_messages[Num_messages_playing].anim_data =
            &anim_info->anim_data;
        message_calc_anim_start_frame (
            Message_wave_duration, &anim_info->anim_data, is_death_scream);
...
void message_queue_process () {
    ...
    // determine if all playing messages (if any) are done playing.  If any are
    // done, remove their entries collapsing the Playing_messages array if
    // necessary
    if (Num_messages_playing > 0) {
        ...
        while (i < Num_messages_playing) {
            ...
            ani_done = 1;
            if (Playing_messages[i].play_anim) ani_done = 0;
            ...
            // if player is a traitor remove all messages that aren't traitor
            // related
            ...
            // see if the ship sending this message is dying.  If do, kill wave
            // and anim
            if (Playing_messages[i].shipnum != -1) {
                if ((Ships[Playing_messages[i].shipnum]
                         .flags[Ship::Ship_Flags::Dying]) &&
                    (Playing_messages[i].builtin_type !=
                    ...
                    if (!(Ships[shipnum]
                        ...
                        !(Ships[shipnum]
                              .flags[Ship::Ship_Flags::No_death_scream])) {
                        ship_scream (&Ships[shipnum]);
                    }
                    continue; // this should keep us in the while() loop with
                              // same value of i.
                } // we should enter the next 'if' statement during next pass
            }

            // if both ani and wave are done, mark internal variable so we can
            // do next message on queue, and global variable to clear voice
            // brackets on hud
    ...
    // preprocess message queue and remove anything on the queue that is too
    // old.  If next message on the queue can be played, then break out of the
    // loop.  Otherwise, loop until nothing on the queue
    ...
    while ((found == -1) && (idx < MessageQ_num)) {
        // if this guy has no min delay timestamp, or it has expired, select
        // him
        if ((MessageQ[idx].min_delay_stamp == -1) ||
    ...
    if (Num_messages_playing) {
        ...
        // interrupt, or overlap a currently playing message.  Mission specific
        ...
        if (q->builtin_type == MESSAGE_HAMMER_SWINE) { message_kill_all (1); }
            ...
            // builtin is playing and we have a unique message to play.  Kill
            // currently playing message so unique can play uninterrupted. Only
            ...
            message_playing_builtin () &&
            (q->message_num < Num_builtin_messages)) {
            // when a builtin message is queued, we might either overlap or
            // interrupt the currently playing message.
            //
            // we have to check for num_messages_playing (again), since code
            // for death scream might kill all messages.
            if (Num_messages_playing) {
            ...
            message_playing_unique () &&
            (q->message_num < Num_builtin_messages)) {
            // code messages can kill any low priority mission specific
            // messages
            if (Num_messages_playing) {
    ...
    // if we are playing the maximum number of voices, then return.  Make the
    // check here since the above code might kill off currently playing
    // messages
    if (Num_messages_playing == MAX_PLAYING_MESSAGES) return;

    // Goober5000 - argh, don't conflate special sources with ships!
    // NOTA BENE: don't check for != MESSAGE_SOURCE_COMMAND, because with the
    // new command persona code, Command could be a ship
    ...
    // if this is a ship, then don't play anything if this ship is already
    // talking
    if (Message_shipnum != -1) {
        for (i = 0; i < Num_messages_playing; i++) {
            if ((Playing_messages[i].shipnum != -1) &&
                (Playing_messages[i].shipnum == Message_shipnum)) {
                return;
            }
        }
    }
    ...
    if (gameseq_get_state () != GS_STATE_GAME_PLAY) goto all_done;
    ...
    if (q->builtin_type == MESSAGE_WINGMAN_SCREAM) {
        ...
        if (!(Ship_info[Ships[Message_shipnum].ship_info_index]
              ...
              (Ships[Message_shipnum]
                   .flags[Ship::Ship_Flags::Always_death_scream]))) {
            goto all_done;
        }
    }
...
#ifndef NDEBUG
    // debug only -- if the message is a builtin message, put in parens whether
    ...
    if (Message_shipnum >= 0) {
        ...
        if (shipp->callsign_index >= 0) {
            ...
            ((Iff_info[shipp->team].flags & IFFF_WING_NAME_HIDDEN) &&
             (shipp->wingnum != -1)) ||
...
void message_queue_message (
    int message_num, int priority, int timing, const char* who_from,
    ...
    // some messages can get queued quickly.  Try to filter out certain types
    // of messages before they get queued if there are other messages of the
    // same type already queued
    if ((builtin_type == MESSAGE_REARM_ON_WAY) ||
        (builtin_type == MESSAGE_OOPS)) {
        // if it is already playing, then don't play it
        if (message_playing_specific_builtin (builtin_type)) return;

        for (i = 0; i < MessageQ_num; i++) {
            // if one of these messages is already queued, then don't play
            if ((MessageQ[i].message_num == message_num) &&
    ...
    // set the timestamp of when to play this message based on the 'timing'
    // value
    if (timing == MESSAGE_TIME_IMMEDIATE)
        ...
        MessageQ[i].window_timestamp =
            timestamp (MESSAGE_ANYTIME_TIMESTAMP); // make invalid
...
// function to return the persona index of the given ship.  If it isn't
// assigned, it will be in this function.  persona_type could be a wingman,
// Terran Command, or other generic ship type personas.  ship is the ship we
// should assign a persona to
int message_get_persona (ship* shipp) {
    ...
    if (shipp != NULL) {
        ...
        for (i = 0; i < Num_personas; i++) {
            ...
            if (Personas[i].species !=
                Ship_info[shipp->ship_info_index].species)
                continue;

            // check the ship types, and don't try to assign those which don't
            // type match
            if (Personas[i].flags & persona_needed) {
                if (!(Personas[i].flags & PERSONA_FLAG_USED)) {
                    ...
                    // otherwise add it to our list of valid options to
...
// builtin messages
void message_send_unique_to_player (
    char* id, void* data, int m_source, int priority, int group, int delay) {
    int i, source;
    const char* who_from;
    ...
    for (i = 0; i < Num_messages; i++) {
        ...
        if (!strcasecmp (id, Messages[i].name)) {
            ...
            if (m_source == MESSAGE_SOURCE_COMMAND) {
                ...
                // find a wingman with the same persona as this message.  If
                // the message's persona doesn't exist, we will use Terran
                // command
                ...
                // messange and therefore the mission designer should have
                // taken into account that the ship may have been silenced.
                ship_index = ship_get_random_player_wing_ship (
                    SHIP_GET_NO_PLAYERS, 0.0f, m_persona, 1,
                    Messages[i].multi_team);

                // if the ship_index is -1, then make the message come from
                // Terran command
                if (ship_index == -1) {
                    who_from = The_mission.command_sender;
                    source = HUD_SOURCE_TERRAN_CMD;
...
// send builtin_to_player sends a message (from messages.tbl) to the player.
// These messages are the generic informational type messages.  The have
// priorities like misison specific messages, and use a timing to tell how long
...
void message_send_builtin_to_player (
    ...
    if (The_mission.flags[Mission::Mission_Flags::No_builtin_msgs]) return;

    // Karajorma - If we aren't showing builtin msgs from command and this is
    // not a ship, bail
    if ((shipp == NULL) &&
    ...
    if (!Valid_builtin_message_types[type]) {
        // downgrade certain message types to more generic ones more likely to
        // be supported
        if (type == MESSAGE_HIGH_PRAISE) { type = MESSAGE_PRAISE; }
        ...
        if (!Valid_builtin_message_types[type]) { return; }
    }

    // see if there is a persona assigned to this ship.  If not, then try to
    // assign one!!!
    if (shipp) {
        ...
        persona_index =
            The_mission.command_persona; // use the terran command persona
    }
    ...
    // we may try to play a message in the wrong persona if we can't find the
    ...
    for (i = 0; i < Num_builtin_messages; i++) {
        ...
        if (!strcasecmp (Messages[i].name, name)) {
            ...
            if ((persona_species >= 0) &&
                (Personas[Messages[i].persona_index].species ==
                 persona_species)) {
            ...
            // NOTE: doesn't need to be nested under the species condition
            // above
            if ((persona_index >= 0) &&
                (Messages[i].persona_index == persona_index)) {
                // condition 3: type + species + persona index match
                current_builtin.type_of_match =
                    BUILTIN_MATCHES_PERSONA_CHECK_MOOD;
            }

            // check if the personas mood suits this particular message, first
            // check if it is excluded
            if (!Messages[i].excluded_moods.empty () &&
                (current_builtin.type_of_match ==
                 BUILTIN_MATCHES_PERSONA_CHECK_MOOD)) {
                for (std::vector< int >::iterator iter =
                         Messages[i].excluded_moods.begin ();
                     iter != Messages[i].excluded_moods.end (); ++iter) {
                    if (*iter == Current_mission_mood) {
                        current_builtin.type_of_match =
                            BUILTIN_MATCHES_PERSONA_EXCLUDED;
                        break;
                    }
                }
            }

            if (current_builtin.type_of_match ==
                BUILTIN_MATCHES_PERSONA_CHECK_MOOD) {
                if (Current_mission_mood == Messages[i].mood) {
                    current_builtin.type_of_match =
                        BUILTIN_MATCHES_PERSONA_MOOD;
                }
                else {
            ...
            // otherwise check to see if the this is the best kind of match
            // we've found so far
    ...
    // loop through the vector until we have found enough elements of the
    ...
    // get who this message is from -- kind of a hack since we assume Terran
    // Command in the absence of a ship.  This will be fixed later
    if (shipp) {
        who_from = shipp->ship_name;
        source = HUD_team_get_source (shipp->team);
    }
        ...
        // Goober5000 - if Command is a ship that is present, change the source
        // accordingly
    ...
    // determine what we should actually do with this dang message.  In
    // multiplayer, we must deal with the fact that this message might not get
...
// message_is_playing()
//
// Return the Message_playing flag.  Message_playing is local to
// MissionMessage.cpp, but this info is needed by code in HUDsquadmsg.cpp
//
int message_is_playing () { return Num_messages_playing ? 1 : 0; }
...
// Blank out portions of the audio playback for the sound identified by
// Message_wave This works by using the same Distort_pattern[][] that was used
// to distort the associated text
void message_maybe_distort () {
    int i;
    int was_muted;

    if (Num_messages_playing == 0) return;

    for (i = 0; i < Num_messages_playing; i++) {
        if (!snd_is_playing (Playing_messages[i].wave)) return;
    }
    ...
    for (i = 0; i < Num_messages_playing; i++) {
        ...
        if (comm_between_player_and_ship (Playing_messages[i].shipnum) !=
            COMM_OK) {
            ...
            if (Message_wave_muted) {
                ...
                if (was_muted)
                    snd_set_volume (
                        Playing_messages[i].wave,
                        (Master_sound_volume * aav_voice_volume));
            }
        }
    }
}

// if the player communications systems are heavily damaged, distort incoming
// messages.
//
...
void message_maybe_distort_text (char* text, int shipnum) {
    ...
    if (comm_between_player_and_ship (shipnum) == COMM_OK) { return; }
...
int message_anim_is_playing () {
    ...
    for (i = 0; i < Num_messages_playing; i++) {
        // if ( (Playing_messages[i].anim != NULL) &&
        // anim_playing(Playing_messages[i].anim) )
        if (Playing_messages[i].play_anim) return 1;
    }

    return 0;
}

// Load mission messages (this is called by the level paging code when running
// with low memory)
...
/**
 * Ideally, this would return the minimum of the comm state between the player
 * and the other ship.  In practice, retail has no checks whatsoever on a
 * ship's ability to send messages unless that ship is the player, so such a
 * change would require an AI profiles option and we must default to the
 * player's state.  However, we have a bit of wiggle room with COMM_SCRAMBLED,
 * because EMP effects are either transient or set by the newly enhanced
 * scramble-messages SEXP.  Thus any comm dropout does not cause an
 * unanticipated deviation in the mission design.
 */
int comm_between_player_and_ship (int other_shipnum) {
    int player_comm_state = hud_communications_state (Player_ship);

    if (other_shipnum < 0) return player_comm_state;
    ...
    */
    {
        if (player_comm_state == COMM_OK && other_comm_state == COMM_OK)
            ...
            player_comm_state == COMM_SCRAMBLED ||
            other_comm_state == COMM_SCRAMBLED)
