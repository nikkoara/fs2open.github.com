...
void parse_music(mission *pm, int flags)
{
        ...
        if (flags & MPF_IMPORT_FSM) {
                // no specified music?
                if (!strcasecmp(pm->event_music_name, "none"))
                ...
                if (!strcasecmp(pm->event_music_name, "7: Marauder") && event_music_get_soundtrack_index("5: Deuteronomy") >= 0) {
...
done_event_music:
                ...
                if (!strcasecmp(pm->briefing_music_name, "Choco Mousse") && event_music_get_spooled_music_index("Aquitaine") >= 0) {
...
int parse_create_object_sub(p_object *p_objp)
{
        ...
        if (!p_objp->replacement_textures.empty()) {
                shipp->ship_replacement_textures = (int *)malloc(MAX_REPLACEMENT_TEXTURES * sizeof(int));
        ...
        while (i--) {
                ...
                if (!strcasecmp(sssp->name, NOX("Pilot"))) {
                        ...
                        for (j = 0; j < wp->num_primary_banks; j++) {
                                if (wp->primary_bank_weapons[j] >= 0 && Weapon_info[wp->primary_bank_weapons[j]].wi_flags[Weapon::Info_Flags::Ballistic]) {
                                        ASSERTX(
                                                Weapon_info[wp->primary_bank_weapons[j]].cargo_size > 0.0f,
                                        ...
                                        int capacity = (int)std::lround(
                                                sssp->primary_ammo[j] / 100.0f * sip->primary_bank_ammo_capacity[j]);
                                        wp->primary_bank_ammo[j] = (int)std::lround(
                                                capacity / Weapon_info[wp->primary_bank_weapons[j]].cargo_size);
                                }
                        }
                        ...
                        for (j = 0; j < wp->num_secondary_banks; j++) {
                                if (wp->secondary_bank_weapons[j] >= 0) {
                                        ASSERTX(
                                                Weapon_info[wp->secondary_bank_weapons[j]].cargo_size > 0.0f,
                                        ...
                                        int capacity = (int)std::lround(
                                                sssp->secondary_ammo[j] / 100.0f * sip->secondary_bank_ammo_capacity[j]);
                                        wp->secondary_bank_ammo[j] = (int)std::lround(
                                                capacity / Weapon_info[wp->secondary_bank_weapons[j]].cargo_size);
                                }
                        }
                ...
                while (ptr != END_OF_LIST(&shipp->subsys_list)) {
                        ...
                        if (The_mission
                                ...
                                if (ptr->system_info->type == SUBSYSTEM_TURRET) {
                                        ...
                                        ptr->turret_next_fire_stamp = timestamp((int)fs2::prng::randf(0, 50.0f, 4000.0f));
                                }
                        }
                        ...
                        if (!subsystem_strcasecmp(
                                ...
                                for (j = 0; j < ptr->weapons.num_primary_banks; j++) {
                                        if (ptr->weapons.primary_bank_weapons[j] >= 0 && Weapon_info[ptr->weapons.primary_bank_weapons[j]].wi_flags[Weapon::Info_Flags::Ballistic]) {
                                                ASSERTX(
                                                ...
                                                int capacity = (int)std::lround(
                                                        sssp->primary_ammo[j] / 100.0f * ptr->weapons.primary_bank_capacity[j]);
                                                ptr->weapons.primary_bank_ammo[j] = (int)std::lround(
                                                        capacity / Weapon_info[ptr->weapons.primary_bank_weapons[j]].cargo_size);
                                        }
                                }
                                ...
                                for (j = 0; j < ptr->weapons.num_secondary_banks; j++) {
                                        if (ptr->weapons.secondary_bank_weapons[j] >= 0) {
                                                ...
                                                int capacity = (int)std::lround(
                                                        sssp->secondary_ammo[j] / 100.0f * ptr->weapons.secondary_bank_capacity[j]);
                                                ptr->weapons
                                                        .secondary_bank_ammo[j]
                                                        = (int)std::lround(
                                                                capacity / Weapon_info[ptr->weapons.secondary_bank_weapons[j]].cargo_size);
                                        }
                                }
...
int parse_object(mission *pm, int /*flag*/, p_object *p_objp)
{
        ...
        if (optional_string("+Flags2:")) {
                ...
                if (!unparsed.empty()) {
                        for (size_t k = 0; k < unparsed.size(); ++k) {
                                ...
                                if (!strcasecmp(unparsed[k].c_str(), "no-collide") || !strcasecmp(unparsed[k].c_str(), "no_collide")) {
                                        p_objp->flags.set(
        ...
        ASSERT(
                p_objp->ship_max_hull_strength > 0.0f); // Goober5000: div-0 check (not shield because we might not have
                                                        // one)
        ...
        p_objp->replacement_textures = Ship_info[p_objp->ship_class]
...
void mission_parse_maybe_create_parse_object(p_object *pobjp)
{
        ...
        if ((object_is_docked(pobjp) && !(pobjp->flags[Mission::Parse_Object_Flags::SF_Dock_leader])) || ((!eval_sexp(pobjp->arrival_cue) || !timestamp_elapsed(pobjp->arrival_delay) || (pobjp->flags[Mission::Parse_Object_Flags::SF_Reinforcement])))) {
        ...
        // the ship is present at the beginning of the mission, so we create it
                ...
                int real_objnum = parse_create_object(pobjp); // this object may later get destroyed
...
void swap_parse_object(p_object *p_obj, int new_ship_class)
{
        ...
        if ((p_obj->ship_max_shield_strength != old_ship_info->max_shield_strength) && (p_obj->ship_max_shield_strength > 0) && (new_ship_info->max_shield_strength > 0)) {
                // This ship is using special hitpoints to alter the shield strength
                float shield_multiplier = p_obj->ship_max_shield_strength / float(old_ship_info->max_shield_strength);
...
int find_wing_name(char *name)
{
        ...
        for (i = 0; i < Num_wings; i++) {
                if (!strcasecmp(name, Wings[i].name))
                        return i;
        }
        ...
        wing *wingp, int num_to_create, int force, int specific_instance)
{
        ...
        if (!force) {
                ...
                if (!timestamp_valid(wingp->wave_delay_timestamp)) {
                        ...
                        if ((wingp->wave_delay_min > 0) || (wingp->wave_delay_max > 0)) {
                                ...
                                time_to_arrive = wingp->wave_delay_min + (int)(fs2::prng::randf(0) * (wingp->wave_delay_max - wingp->wave_delay_min));
        ...
        for (std::vector< p_object >::iterator ii = Parse_objects.begin();
             ii != Parse_objects.end(); ++ii) {
                ...
                ASSERT(
                        (p_objp->pos_in_wing >= 0) && (p_objp->pos_in_wing < MAX_SHIPS_PER_WING));
...
int mission_parse_get_multi_mission_info(const char *filename)
{
        ...
        ASSERT(
                The_mission.game_type & MISSION_TYPE_MULTI); // assume multiplayer only for now?
        ...
        vec3d *new_pos, matrix *new_orient)
{
        ...
        if (shipnum < 0) {
                ASSERT(
                        location != ARRIVE_FROM_DOCK_BAY); // bogus data somewhere!!!  get mwa
...
int mission_do_departure(object *objp, bool goal_is_to_warp)
{
        ...
        if (location == DEPART_AT_DOCK_BAY) {
                ...
                if (ai_acquire_depart_path(
                            objp, Ships[anchor_shipnum].objnum, path_mask)
                    >= 0) {
...
parse_get_subsys_status(p_object *pobjp, const char *subsys_name)
{
        ...
        for (i = 0; i < pobjp->subsys_count; i++) {
                ...
                if (!subsystem_strcasecmp(sssp->name, subsys_name))
                        return sssp;
        }
...
int get_parse_name_index(const char *name)
{
        ...
        for (i = 0; i < Num_parse_names; i++)
                if (!strcasecmp(name, Parse_names[i]))
                        return i;
...
int get_special_anchor(char *name)
{
        ...
        if (stristr(name + 5, "player") != NULL)
                return (
                        iff_index | SPECIAL_ARRIVAL_ANCHOR_FLAG | SPECIAL_ARRIVAL_ANCHOR_PLAYER_FLAG);
        else
...
void mission_bring_in_support_ship(object *requester_objp)
{
        ...
        requester_shipp = &Ships[requester_objp->instance]; // MK, 10/23/97, used to be
                                                            //->type, bogus, no?
        ...
        for (i = 0; i < Num_cargo; i++)
                if (!strcasecmp(Cargo_names[i], NOX("nothing")))
                        break;
