...
#ifndef FREESPACE2_CONTROLCONFIG_CONTROLSCONFIG_HH
#define FREESPACE2_CONTROLCONFIG_CONTROLSCONFIG_HH
...
/*!
 * These are used to index a corresponding (analog) action, namely controlling
 * the orientation angles and throttle.
 */
 ...
 */
enum CC_type {
    CC_TYPE_TRIGGER = 0, //!< A normal, one-shot type control that is activated
                         //!< when a key is or button is pressed
    CC_TYPE_CONTINUOUS   //!< A continous control that is activated as long as
                         //!< the key or button is held down
};
...
struct config_item  {
    ...
    int used; //!< has control been used yet in mission?  If so, this is the
              //!< timestamp
...
enum IoActionId {
    ...
    TARGET_CLOSEST_SHIP_ATTACKING_TARGET =
        8, //!< target the closest ship attacking current target
    ...
    CYCLE_NUM_MISSLES =
        22, //!< cycle number of missiles fired from secondary bank
    ...
    TARGET_CLOSEST_SHIP_ATTACKING_SELF =
        50, //!< target closest ship that is attacking player
    ...
    VIEW_EXTERNAL_TOGGLE_CAMERA_LOCK =
        53,                  //!< VIEW_EXTERNAL_TOGGLE_CAMERA_LOCK
    ...
    //!< -----------------
    GLIDE_WHEN_PRESSED =
        83, //!< GLIDE_WHEN_PRESSED
            //!< @remark Backslash -- this was a convenient place for Glide
            //!< When Pressed, since Show Damage Popup isn't used
    ...
    MULTI_OBSERVER_ZOOM_TO =
        101, //!< if i'm an observer, zoom to my targeted object
    ...
    CYCLE_PRIMARY_WEAPON_SEQUENCE =
        118, //!< cycle num primaries to fire at once

    /*!
     * This must always be below the last defined item
     */
    CCFG_MAX //!<  The total number of defined control actions (or last define
             //!<  + 1)
};
...
extern std::vector< config_item* >
    Control_config_presets; // tabled control presets; pointers to config_item
                            // arrays
extern std::vector< std::string >
    Control_config_preset_names; // names for Control_config_presets (identical
                                 // order of items)
extern const char** Scan_code_text;
extern const char** Joy_button_text;

void control_config_common_init (); //!< initialize common control config stuff
                                    //!< - call at game startup after
                                    //!< localization has been initialized
void control_config_common_close (); //!< close common control config stuff -
 ...
 * @return The text representation of the code. The returned value is stored in
 * a temporary location, copy it to your own buffer if you want to continue
 * using it.
 */
const char* textify_scancode (int code);

/**
 * @note Same as textify_scancode but always returns the same value regardless
 * of current language
 * @param code The key code to convert
...
#endif // FREESPACE2_CONTROLCONFIG_CONTROLSCONFIG_HH
