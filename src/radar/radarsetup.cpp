...
int Radar_calc_bright_dist_timer; // timestamp at which we recalc
                                  // Radar_bright_range
...
void radar_stuff_blip_info (
    object* objp, int is_bright, color** blip_color, int* blip_type) {
    ship* shipp = NULL;
    ...
    case OBJ_SHIP:
        ...
        if (shipp->flags[Ship::Ship_Flags::Arriving_stage_1]) {
            ...
            *blip_type = BLIP_TYPE_TAGGED_SHIP;
            ...
            Ship_info[shipp->ship_info_index].flags[Ship::Info_Flags::Cargo]) {
            *blip_color = &Radar_colors[RCOL_NAVBUOY_CARGO][is_bright];
            *blip_type = BLIP_TYPE_NAVBUOY_CARGO;
        }
        else {
            *blip_color = iff_get_color_by_team_and_object (
                shipp->team, Player_ship->team, is_bright, objp);
    ...
    case OBJ_WEAPON:
        if ((Weapons[objp->instance].lssm_stage == 2) ||
            (Weapons[objp->instance].lssm_stage == 4)) {
            *blip_color = &Radar_colors[RCOL_WARPING_SHIP][is_bright];
            *blip_type = BLIP_TYPE_WARPING_SHIP;
        }
        else {
...
void radar_plot_object (object* objp) {
    ...
    if (awacs_level < 0.0f && !See_all) { return; }
    ...
    case OBJ_JUMP_NODE: {
        for (jnp = Jump_nodes.begin (); jnp != Jump_nodes.end (); ++jnp) {
            if (jnp->GetSCPObject () == objp) break;
        }
    ...
    case OBJ_WEAPON: {
        ...
        if (!(Weapon_info[Weapons[objp->instance].weapon_info_index]
                  .wi_flags[Weapon::Info_Flags::Shown_on_radar]))
            if (!(Weapon_info[Weapons[objp->instance].weapon_info_index]
        ...
        if (Weapon_info[Weapons[objp->instance].weapon_info_index]
                .wi_flags[Weapon::Info_Flags::Dont_show_on_radar])
            return;

        // if we don't attack the bomb, return
        if ((!(Weapon_info[Weapons[objp->instance].weapon_info_index]
                   .wi_flags[Weapon::Info_Flags::Show_friendly])) &&
            (!iff_x_attacks_y (Player_ship->team, obj_team (objp))))
            return;

        // if a local ssm is in subspace, return
        if (Weapons[objp->instance].lssm_stage == 3) return;
    ...
    // JAS -- new way of getting the rotated point that doesn't require this to
    // be in a g3_start_frame/end_frame block.
    ...
    if (timestamp_elapsed (Radar_calc_bright_dist_timer)) {
        ...
        if (Radar_bright_range <= 0) Radar_bright_range = 1500.0f;
    }
    ...
    if (objp->type == OBJ_SHIP) {
        ...
        if (awacs_level < 1.0f) b->flags |= BLIP_DRAW_DISTORTED;

        ship_info* Iff_ship_info =
            &Ship_info[Ships[objp->instance].ship_info_index];

        if (Iff_ship_info->radar_image_2d_idx >= 0 ||
            Iff_ship_info->radar_color_image_2d_idx >= 0) {
            ...
            b->radar_projection_size =
                Iff_ship_info->radar_projection_size_mult;
        }
    }

    // don't distort the sensor blips if the player has primitive sensors and
    // the nebula effect is not active
    ...
    for (int i = 0; i < MAX_RADAR_COLORS; i++) {
        for (int j = 0; j < MAX_RADAR_LEVELS; j++) {
            if (radar_iff_color[i][j][0] >= 0) {
                gr_init_alphacolor (
                    &Radar_colors[i][j], radar_iff_color[i][j][0],
                    radar_iff_color[i][j][1], radar_iff_color[i][j][2],
                    radar_iff_color[i][j][3]);
            }
            else {
                gr_init_alphacolor (
                    &Radar_colors[i][j], Radar_color_rgb[i][j].r,
                    Radar_color_rgb[i][j].g, Radar_color_rgb[i][j].b, 255);
            }
        }
    }
...
void radar_frame_init () { radar_null_nblips (); }

HudGaugeRadar::HudGaugeRadar ()
    : HudGauge (
          HUD_OBJECT_RADAR_STD, HUD_RADAR, false, false,
...
HudGaugeRadar::HudGaugeRadar (int _gauge_object, int r, int g, int b)
    : HudGauge (
          _gauge_object, HUD_RADAR, false, false,
          (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
           VM_OTHER_SHIP),
          r, g, b) {}
...
void HudGaugeRadar::initInfinityIcon () {
    ...
    if (sc == 0) { Radar_infinity_icon = ubyte ('*'); }
    else {
...
void HudGaugeRadar::drawRange () {
    ...
    case RR_SHORT:
        renderPrintf (
            position[0] + Radar_dist_offsets[RR_SHORT][0],
            position[1] + Radar_dist_offsets[RR_SHORT][1], "%s",
            XSTR ("2k", 467));
        break;

    case RR_LONG:
        renderPrintf (
            position[0] + Radar_dist_offsets[RR_LONG][0],
            position[1] + Radar_dist_offsets[RR_LONG][1], "%s",
            XSTR ("10k", 468));
        break;

    case RR_INFINITY:
        if (Unicode_text_mode) {
            // This escape sequence is the UTF-8 encoding of the infinity
            // symbol. We can't use u8 yet since VS2013 doesn't support it
            renderPrintf (
                position[0] + Radar_dist_offsets[RR_INFINITY][0],
                position[1] + Radar_dist_offsets[RR_INFINITY][1],
                "\xE2\x88\x9E");
        }
        else {
            renderPrintf (
                position[0] + Radar_dist_offsets[RR_INFINITY][0],
                position[1] + Radar_dist_offsets[RR_INFINITY][1], "%c",
                Radar_infinity_icon);
        }
        break;
 ...
 * @return A RadarVisibility enum specifying the visibility of the specified
 * object
 */
RadarVisibility radar_is_visible (object* objp) {
    ...
    // if the awacs level is unviewable - bail
    if (awacs_level < 0.0f && !See_all) { return NOT_VISIBLE; }
    ...
    case OBJ_JUMP_NODE: {
        ...
        // don't plot hidden jump nodes
        if (jnp->IsHidden ()) return NOT_VISIBLE;
    ...
    case OBJ_WEAPON: {
        ...
        // if a local ssm is in subspace, return
        if (Weapons[objp->instance].lssm_stage == 3) return NOT_VISIBLE;

        break;
    }

    // if any other kind of object, don't show it on radar
    default: return NOT_VISIBLE;
    }
    ...
    dist = vm_vec_dist (&world_pos, &Player_obj->pos);
    max_radar_dist = Radar_ranges[HUD_config.rp_dist];
    if (dist > max_radar_dist) { return NOT_VISIBLE; }
    ...
    if (objp->type == OBJ_SHIP) {
        ...
        // determine if its AWACS distorted
        if (awacs_level < 1.0f) return DISTORTED;
    }
