...
#define BLIP_DRAW_DIM \
    (1 << 1) // object is farther than Radar_bright_range units away
#define BLIP_DRAW_DISTORTED \
    (1 << 2) // object is resistant to sensors, so draw distorted

extern float
    Radar_bright_range; // range within which the radar blips are bright
extern int Radar_calc_bright_dist_timer; // timestamp at which we recalc
                                         // Radar_bright_range
...
protected:
    ...
    int Radar_flicker_timer[NUM_FLICKER_TIMERS]; // timestamp used to flicker
                                                 // blips on and off
...
#endif // FREESPACE2_RADAR_RADARSETUP_HH
