...
HudGaugeRadarDradis::HudGaugeRadarDradis ()
    : HudGaugeRadar (HUD_OBJECT_RADAR_BSG, 255, 255, 255), xy_plane (-1),
      xz_yz_plane (-1), sweep_plane (-1), target_brackets (-1),
      unknown_contact_icon (-1), sweep_duration (6.0), sweep_percent (0.0),
      scale (1.20f), sub_y_clip (false) {
...
void HudGaugeRadarDradis::initBitmaps (
    char* fname_xy, char* fname_xz_yz, char* fname_sweep,
    char* fname_target_brackets, char* fname_unknown) {
    xy_plane = bm_load (fname_xy); // Base
    if (xy_plane < 0) {
        WARNINGF (LOCATION, "Cannot load hud bitmap: %s", fname_xy);
    }

    xz_yz_plane = bm_load (fname_xz_yz); // Two vertical cross rings
    if (xz_yz_plane < 0) {
        WARNINGF (LOCATION, "Cannot load hud bitmap: %s", fname_xz_yz);
    }

    sweep_plane = bm_load (fname_sweep); // Sweep lines
    if (sweep_plane < 0) {
        WARNINGF (LOCATION, "Cannot load hud bitmap: %s", fname_sweep);
    }

    target_brackets = bm_load (fname_target_brackets);
    if (target_brackets < 0) {
        WARNINGF (LOCATION, "Cannot load hud bitmap: %s", fname_target_brackets);
    }

    unknown_contact_icon = bm_load (fname_unknown);
    if (unknown_contact_icon < 0) {
        WARNINGF (LOCATION, "Cannot load hud bitmap: %s", fname_unknown);
    }
}
...
void HudGaugeRadarDradis::plotBlip (blip* b, vec3d* pos, float* alpha) {
    ...
    *alpha = ((sweep_duration - b->time_since_update) / sweep_duration) *
             fade_multi / 2.0f;

    if (*alpha < 0.0f) { *alpha = 0.0f; }
}

void HudGaugeRadarDradis::drawContact (
    vec3d* pnt, int idx, int clr_idx, float /*dist*/, float alpha,
    float scale_factor) {
    vec3d p;
    int h, w;
    vertex vert;
    ...
    if ((sub_y_clip && (pnt->xyz.y > 0)) ||
        ((!sub_y_clip) && (pnt->xyz.y <= 0)))
        return;
    ...
    if (clr_idx >= 0) {
        ...
        // gr_set_bitmap(clr_idx, GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL,
        // alpha); g3_draw_polygon(&p, &vmd_identity_matrix, sizef/35.0f,
        // aspect_mp*sizef/35.0f, TMAP_FLAG_TEXTURED | TMAP_HTL_3D_UNLIT);
        material mat_params;
        material_set_unlit_color (
            &mat_params, clr_idx, &Color_bright_white, alpha, true, false);
        g3_render_rect_oriented (
            &mat_params, &p, &vmd_identity_matrix, sizef / 35.0f,
    ...
    if (idx >= 0) {
        ...
        // gr_set_bitmap(idx, GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL,
        // alpha); g3_draw_polygon(&p, &vmd_identity_matrix, sizef/35.0f,
        // aspect_mp*sizef/35.0f, TMAP_FLAG_TEXTURED | TMAP_FLAG_BW_TEXTURE |
        // TMAP_HTL_3D_UNLIT);
        material mat_params;
        material_set_unlit_color (
            &mat_params, idx, &gr_screen.current_color, alpha, true, false);
        g3_render_rect_oriented (
            &mat_params, &p, &vmd_identity_matrix, sizef / 35.0f,
            aspect_mp * sizef / 35.0f);
    }
...
void HudGaugeRadarDradis::blipDrawDistorted (
    blip* b, vec3d* pos, float alpha) {
    float temp_scale;
...
void HudGaugeRadarDradis::drawBlips (int blip_type, int bright, int distort) {
    ...
    if (bright) { blip_head = &Blip_bright_list[blip_type]; }
    else {
    ...
    for (b = GET_FIRST (blip_head); b != END_OF_LIST (blip_head);
        ...
        if (b->flags & BLIP_CURRENT_TARGET) {
            ...
            if (radar_target_id_flags & RTIF_BLINK) {
                if (Missiontime & 8192) continue;
            }
            if (radar_target_id_flags & RTIF_ENLARGE) { scale_factor *= 1.3f; }

            alpha = 1.0;
            b->rad = Radar_blip_radius_target;
            drawContact (
                &pos, -1, target_brackets, b->dist, alpha, scale_factor);
        }
        else {
        ...
        // maybe distort blip
        if (distort) { blipDrawDistorted (b, &pos, alpha); }
        else {
            if (b->flags & BLIP_DRAW_DISTORTED) {
                ...
                drawContact (
                    &pos, b->radar_image_2d, b->radar_color_image_2d, b->dist,
                    alpha, scale_factor);
            }
            else {
                drawContact (
                    &pos, -1, unknown_contact_icon, b->dist, alpha,
                    scale_factor);
            }
        }
    }
...
void HudGaugeRadarDradis::drawOutlinesHtl () {
    ...
    if ((xy_plane == -1) || (xz_yz_plane == -1)) return;
    ...
    // gr_set_bitmap(xy_plane, GR_ALPHABLEND_FILTER,
    // GR_BITBLT_MODE_NORMAL, 1.0f); // base g3_draw_polygon(&vmd_zero_vector,
    // &base_tilt_norm, scale, scale, TMAP_FLAG_TEXTURED | TMAP_HTL_3D_UNLIT);
    material mat_params;
    material_set_unlit (&mat_params, xy_plane, 1.0f, true, false);
    g3_render_rect_oriented (
        &mat_params, &vmd_zero_vector, &base_tilt_norm, scale, scale);

    // gr_set_bitmap(xz_yz_plane, GR_ALPHABLEND_FILTER,
    // GR_BITBLT_MODE_NORMAL, 1.0f);

    // g3_draw_polygon(&vmd_zero_vector, &vmd_x_vector, scale, scale,
    // TMAP_FLAG_TEXTURED | TMAP_HTL_3D_UNLIT); // forward facing ring
    // g3_draw_polygon(&vmd_zero_vector, &vmd_z_vector, scale, scale,
    // TMAP_FLAG_TEXTURED | TMAP_HTL_3D_UNLIT); // side facing ring

    material_set_unlit (&mat_params, xz_yz_plane, 1.0f, true, false);
    g3_render_rect_oriented (
        &mat_params, &vmd_zero_vector, &vmd_x_vector, scale,
        scale); // forward facing ring
    g3_render_rect_oriented (
        &mat_params, &vmd_zero_vector, &vmd_z_vector, scale,
        scale); // side facing ring
    g3_done_instance (true);
}

void HudGaugeRadarDradis::drawSweeps () {
    if (sweep_plane == -1) return;

    sweep_percent = (fmod (
                         ((float)game_get_overall_frametime () / (float)65536),
                         sweep_duration) /
                     sweep_duration) *
                    PI * 2; // convert to radians from 0 <-> 1
    ...
    vm_rot_point_around_line (
        &sweep_a, &vmd_y_vector, sweep_percent, &vmd_zero_vector,
        &vmd_z_vector); // Sweep line: XZ
    vm_rot_point_around_line (
        &sweep_b, &vmd_y_vector, sweep_percent, &vmd_zero_vector,
        &vmd_x_vector); // Sweep line: YZ
    vm_rot_point_around_line (
        &sweep_c, &vmd_x_vector, sweep_perc_z, &vmd_zero_vector,
        &vmd_y_vector); // Sweep line: XY
    ...
    g3_render_rect_oriented (
        &mat_params, &vmd_zero_vector, &sweep_a, scale, scale);
    g3_render_rect_oriented (
        &mat_params, &vmd_zero_vector, &sweep_b, scale, scale);
    g3_render_rect_oriented (
        &mat_params, &vmd_zero_vector, &sweep_c, scale, scale);

    float rotation = sweep_percent;

    vm_rot_point_around_line (
        &sweep_a, &vmd_y_vector, rotation, &vmd_zero_vector,
        &vmd_z_vector); // Sweep line: XZ
    vm_rot_point_around_line (
        &sweep_b, &vmd_y_vector, rotation, &vmd_zero_vector,
        &vmd_x_vector); // Sweep line: YZ
    vm_rot_point_around_line (
        &sweep_c, &vmd_x_vector, sweep_perc_z, &vmd_zero_vector,
        &vmd_y_vector); // Sweep line: YZ
    ...
    g3_render_rect_oriented (
        &mat_params, &vmd_zero_vector, &sweep_a, scale,
        scale); // Sweep line: XZ
    g3_render_rect_oriented (
        &mat_params, &vmd_zero_vector, &sweep_b, scale,
        scale); // Sweep line: YZ
    g3_render_rect_oriented (
        &mat_params, &vmd_zero_vector, &sweep_c, scale, scale);
    ...
    alpha);
        ...
        vm_rot_point_around_line(&sweep_a, &vmd_y_vector, rotation,
    &vmd_zero_vector, &vmd_z_vector); // Sweep line: XZ
        vm_rot_point_around_line(&sweep_b, &vmd_y_vector, rotation,
    &vmd_zero_vector, &vmd_x_vector); // Sweep line: YZ
    ...
    TMAP_FLAG_TEXTURED | TMAP_HTL_3D_UNLIT); // Sweep line: YZ
        ...
        if (i < (dist * 0.5f))
        {
            vm_rot_point_around_line(&sweep_c, &vmd_x_vector,sweep_perc_z + (i
    * RADIANS_PER_DEGREE), &vmd_zero_vector, &vmd_y_vector); // Sweep line: YZ
            g3_draw_polygon(&vmd_zero_vector, &sweep_c, scale, scale,
    TMAP_FLAG_TEXTURED | TMAP_HTL_3D_UNLIT);
        }
    }*/
...
void HudGaugeRadarDradis::drawBlipsSorted (int distort) {
    ...
    for (int is_bright = 0; is_bright < 2; is_bright++) {
        sub_y_clip = true;
        g3_start_instance_matrix (
            &vmd_zero_vector, /*&Player_obj->orient*/ &base_tilt, true);
...
void HudGaugeRadarDradis::doLoopSnd () {
    if (!this->m_loop_snd.isValid ()) { return; }

    if (!this->shouldDoSounds ()) {
        if (loop_sound_handle.isValid () &&
            snd_is_playing (loop_sound_handle)) {
            snd_stop (loop_sound_handle);
            loop_sound_handle = sound_handle::invalid ();
        }
    }
    else if (
        !this->loop_sound_handle.isValid () ||
        !snd_is_playing (this->loop_sound_handle)) {
        loop_sound_handle = snd_play (
            gamesnd_get_game_sound (m_loop_snd), 0.0f, loop_sound_volume);
    }
}

void HudGaugeRadarDradis::doBeeps () {
    if (!this->shouldDoSounds ()) { return; }
    ...
    if (!arrival_beep_snd.isValid () && !departure_beep_snd.isValid () &&
    ...
    if (timestamp_elapsed (arrival_beep_next_check)) {
        if (arrival_beep_snd.isValid () && arrival_happened) {
            ...
            m_stealth_arrival_snd.isValid () && stealth_arrival_happened) {
    ...
    if (timestamp_elapsed (departure_beep_next_check)) {
        if (departure_beep_snd.isValid () && departure_happened) {
            ...
            stealth_departure_snd.isValid () && stealth_departure_happened) {
...
void HudGaugeRadarDradis::initSound (
    gamesnd_id loop_snd, float _loop_snd_volume, gamesnd_id arrival_snd,
    gamesnd_id departure_snd, gamesnd_id stealth_arrival_snd,
    gamesnd_id stealth_departue_snd, float arrival_delay,
