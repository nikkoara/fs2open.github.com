        ...
        float scale_factor)
{
        ...
        float sizef = sqrtf(vm_vec_dist(&Orb_eye_position, pnt) * 8.0f) * scale_factor;
        ...
        blip *b, vec3d *pos, float alpha)
{
        ...
        if (emp_active_local()) {
                ...
                dist *= fs2::prng::randf(0, MAX(0.75f, 0.75f * temp_scale), MIN(1.25f, 1.25f * temp_scale));
                distortion_angle *= fs2::prng::randf(0, -3.0f, 3.0f) * fs2::prng::randf(0, 0.0f, temp_scale);
...
void HudGaugeRadarDradis::doLoopSnd()
{
        ...
        if (!this->shouldDoSounds()) {
                ...
                !this->loop_sound_handle.isValid() || !snd_is_playing(this->loop_sound_handle)) {
...
void HudGaugeRadarDradis::doBeeps()
{
        ...
        if (!arrival_beep_snd.isValid() && !departure_beep_snd.isValid() && !m_stealth_arrival_snd.isValid() && !stealth_departure_snd.isValid()) {
                return;
        }
                ...
                if (shipp->objnum >= 0) {
                        if (shipp->radar_visible_since >= 0 || shipp->radar_last_contact >= 0) {
                                if (shipp->radar_visible_since == Missiontime) {
                                        ...
                                        shipp->radar_visible_since < 0 && shipp->radar_last_contact == Missiontime) {
