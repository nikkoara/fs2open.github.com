...
void HudGaugeRadarStd::drawBlips (int blip_type, int bright, int distort) {
    ...
    for (b = GET_FIRST (blip_head); b != END_OF_LIST (blip_head);
        ...
        // maybe distort blip
        if (distort) { blipDrawDistorted (b, x, y); }
            ...
            if (b->radar_image_2d == -1 && b->radar_color_image_2d == -1)
                ...
                drawContactImage (
                    x, y, b->rad, b->radar_image_2d, b->radar_color_image_2d,
                    b->radar_image_size);
        }
    }
}
...
void HudGaugeRadarStd::drawContactImage (
    int x, int y, int rad, int idx, int clr_idx, int size) {
    ...
    if (rad == Radar_blip_radius_target) {
        ...
        if (radar_target_id_flags & RTIF_ENLARGE) { scalef *= 1.3f; }
    }
...
void HudGaugeRadarStd::plotBlip (blip* b, int* x, int* y) {
    ...
    if (b->dist < pos->xyz.z) { rscale = 0.0f; }
    else {
