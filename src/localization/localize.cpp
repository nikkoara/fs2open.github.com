...
// XSTR_SIZE is the total count of unique XSTR index numbers.  An index is used
// to reference an entry in strings.tbl.  This is used for translation of
// strings from the english version (in the code) to a foreign version (in the
// table).  Thus, if you add a new string to the code, you must assign it a new
// index.  Use the number below for that index and increase the number below by
// one.
