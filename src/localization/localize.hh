...
struct lang_info {
        ...
        ubyte special_char_indexes
                [LCL_MAX_FONTS]; // where in the font do we have the special characters
...
// string
void lcl_ext_localize(
        const char *in, char *out, size_t max_len, int *id = NULL);
void lcl_ext_localize(
        const std::string &in, std::string &out, int *id = NULL);
...
#endif // FREESPACE2_LOCALIZATION_LOCALIZE_HH
