...
typedef struct fhash_node {
    ...
    int id;                  // can be -1
    fhash_node *next, *prev; // for chaining in an individual hash table entry
...
// hash a string. return an index into the hash table where it should be
// inserted
int fhash_get_hash_index (const char* str);

// insert a string into hash table index N, will take care of
// allocating/chaining everything
...
void fhash_activate () { Fhash_active = 1; }

// set the hash table to be inactive for parsing
void fhash_deactivate () { Fhash_active = 0; }

// if the hash table is active
int fhash_active () { return Fhash_active; }
...
int fhash_string_exists (const char* str) {
    ...
    if (Hash_table_fred[hash_index] == NULL) { return -2; }
...
int fhash_get_hash_index (const char* str) {
    ...
    // add up the string
    str_len = strlen (str);
    for (idx = 0; idx < str_len; idx++) { accum += str[idx]; }
...
void fhash_insert (const char* str, int id, int n) {
    ...
    if (Hash_table_fred[n] == NULL) { Hash_table_fred[n] = new_node; }
    else {
        moveup = Hash_table_fred[n];
        while (moveup->next != NULL) { moveup = moveup->next; }
