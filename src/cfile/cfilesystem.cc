...
void cf_build_root_list(const char *cdrom_dir)
{
        ...
        // install
        if ((strlen(root->path) < (CF_MAX_PATHNAME_LENGTH - 1)) && (root->path[strlen(root->path) - 1] != '/')) {
                strcat(
        ...
        if ((path_len < (CF_MAX_PATHNAME_LENGTH - 1)) && (root->path[path_len - 1] != '/')) {
...
void cf_search_root_pack(int root_index)
{
        ...
        if (cfilelength(fileno(fp)) < (int)(sizeof(VP_FILE_HEADER) + (sizeof(int) * 3))) {
                ...
                if (find.size == 0) {
                        ...
                        if (!strcasecmp(find.filename, "..")) {
                                ...
                                *p = 0;
                                ...
                                if (search_path_len && (search_path[search_path_len - 1] != '/')) {
                                        strcat(search_path, "/");
                                }
        ...
        int pathtype, bool localize)
{
        ...
        ASSERT(
                (ext_list != NULL) && (ext_num > 1)); // if we are searching for just one ext
        ...
        for (ui = 0; ui < Num_files; ui++) {
                ...
                if (strncasecmp(f->name_ext, filespec, filespec_len) != 0)
                        continue;
...
int cf_matches_spec(const char *filespec, const char *filename)
{
        ...
        if (src_ext == filespec) {
                return !strcasecmp(dst_ext, src_ext);
        } else {
                return (
                        !strcasecmp(dst_ext, src_ext) && !strncasecmp(dst_ext, src_ext, src_ext - filespec));
        }
}
        ...
        int sort, std::vector< file_list_info > *info, uint32_t location_flags)
{
        ...
        if (dirp) {
                while ((dir = readdir(dirp)) != NULL) {
                        ...
                        char fn[PATH_MAX];
                        if (snprintf(fn, PATH_MAX, "%s/%s", filespec, dir->d_name) >= PATH_MAX) {
