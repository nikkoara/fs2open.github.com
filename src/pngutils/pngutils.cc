...
void apng_ani::_process_chunk()
{
        ...
        if (_id == id_acTL && !_got_IDAT && !_got_acTL) {
                ...
                if (_reading && (_framew > cMaxPNGSize || _frameh > cMaxPNGSize || _x_offset > cMaxPNGSize || _y_offset > cMaxPNGSize || _x_offset + _framew > w || _y_offset + _frameh > h || _dispose_op > 2 || _blend_op > 1)) {
                ...
                if (_delay_den == 0)
                        _delay_den = 100; // APNG spec
                if (_delay_num == 0)
                        _delay_num = 1; // arbitrary lower bound
                ...
                not_chunk(_chunk.data[4]) || not_chunk(_chunk.data[5]) || not_chunk(_chunk.data[6]) || not_chunk(_chunk.data[7])) {
