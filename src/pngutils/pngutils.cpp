 ...
 */
static void
png_scp_read_data (png_structp png_ptr, png_bytep data, png_size_t length) {
    png_size_t check;

    if (png_ptr == NULL) return;

    png_status* status =
        reinterpret_cast< png_status* > (png_get_io_ptr (png_ptr));
...
static void
png_scp_write_data (png_structp png_ptr, png_bytep data, png_size_t length) {
    auto status = static_cast< png_status* > (png_get_io_ptr (png_ptr));

    ASSERTX (
        status != nullptr, "Invalid file pointer in PNG writing function.");

    auto check =
        (png_size_t)cfwrite (data, (png_size_t)1, (int)length, status->cfp);
 ...
 */
int png_read_header (
    const char* real_filename, CFILE* img_cfp, int* w, int* h, int* bpp,
    ubyte* /*palette*/) {
    ...
    if (img_cfp == NULL) {
        ...
        if (!status.cfp) { return PNG_ERROR_READING; }
    }
    else {
        status.cfp = img_cfp;
    }
    ...
    if (status.cfp == NULL) return PNG_ERROR_READING;
    ...
    png_ptr = png_create_read_struct_2 (
        PNG_LIBPNG_VER_STRING, &status, png_error_fn, png_warning_fn, &status,
    ...
    if (bpp) {
        // bit depth can also be 16 bit we tell libpng to reduce that to 8 bits
        // so we also need to tell our caller about that
...
int png_read_bitmap (
     ...
     */
    png_ptr = png_create_read_struct_2 (
        PNG_LIBPNG_VER_STRING, &status, png_error_fn, png_warning_fn, NULL,
        NULL, NULL);
    ...
    png_read_png (
        png_ptr, info_ptr,
        PNG_TRANSFORM_BGR | PNG_TRANSFORM_EXPAND | PNG_TRANSFORM_STRIP_16,
        NULL);
...
bool png_write_bitmap (
    ...
    if (!status.cfp) { return false; }

    auto png_ptr = png_create_write_struct_2 (
        PNG_LIBPNG_VER_STRING, &status, png_error_fn, png_warning_fn, &status,
        png_malloc_fn, png_free_fn);

    auto info_ptr = png_create_info_struct (png_ptr);
    png_set_IHDR (
        png_ptr, info_ptr, (png_uint_32)width, (png_uint_32)height, 8,
    ...
    // According to the documentation level 6 should perform reasonably well
    // for us
...
info_callback (png_structp png_ptr, png_infop /*info_ptr*/) {
    static_cast< apng_ani* > (png_get_progressive_ptr (png_ptr))
        ->info_callback ();
}
 ...
 */
static inline void row_callback (
    png_structp png_ptr, png_bytep new_row, png_uint_32 row_num,
    int /*pass*/) {
    static_cast< apng_ani* > (png_get_progressive_ptr (png_ptr))
        ->row_callback (new_row, row_num);
}

/*
 * @brief compose new frame from previous frame & currently-processing frames
 * raw data
...
void apng_ani::_compose_frame () {
    ...
    ASSERTX (
        bpp == 32,
        "apng frame composition assumes 4 bytes of data per pixel, get a "
        "coder!");
    for (uint j = 0; j < _frameh; j++) {
        ...
        if (_blend_op == 0) {
            // blend operation 0 (APNG_BLEND_OP_SOURCE) means ignore
            // destination
                ...
                if (sp[3] == 255) {
                    ...
                    if (sp[3] != 0) {
                        if (dp[3] != 0) {
                            ...
                            v = (255 - sp[3]) * dp[3];
                            al = u + v;
                            dp[0] = static_cast< ubyte > (
                                (sp[0] * u + dp[0] * v) / al);
                            dp[1] = static_cast< ubyte > (
                                (sp[1] * u + dp[1] * v) / al);
                            dp[2] = static_cast< ubyte > (
                                (sp[2] * u + dp[2] * v) / al);
...
int apng_ani::_processing_start () {
    ...
    _pngp = png_create_read_struct (
        PNG_LIBPNG_VER_STRING, nullptr, nullptr, nullptr);
    _infop = png_create_info_struct (_pngp);
    if (_pngp == nullptr || _infop == nullptr) { return 1; }
    ...
    png_set_progressive_read_fn (
        _pngp, this, apng::info_callback, apng::row_callback, nullptr);

    png_process_data (_pngp, _infop, png_sig, 8);
    memcpy (
        &_chunk_IHDR.data[8], &_chunk.data[12],
        8); // use frame width & height from fcTL
    ...
    for (size_t i = 0; i < _info_chunks.size (); ++i) {
        ...
        png_process_data (
            _pngp, _infop, &_info_chunks.at (i).data[0],
            _info_chunks.at (i).size);
    }
...
void apng_ani::info_callback () {
    ...
    png_set_add_alpha (
        _pngp, 0xff, PNG_FILLER_AFTER); // doesn't affect images that already
                                        // have alpha channel
...
void apng_ani::_process_chunk () {
    ...
    if (_id == id_acTL && !_got_IDAT && !_got_acTL) {
        ...
        if (_reading && (_framew > cMaxPNGSize || _frameh > cMaxPNGSize ||
        ...
        // according to spec...
        if (current_frame == 0 && _dispose_op == 2) { _dispose_op = 1; }

        if (_delay_den == 0) _delay_den = 100; // APNG spec
        if (_delay_num == 0) _delay_num = 1;   // arbitrary lower bound
        float frame_delay = static_cast< float > (_delay_num) /
                            static_cast< float > (_delay_den);
        ...
        not_chunk (_chunk.data[4]) || not_chunk (_chunk.data[5]) ||
 ...
 * @note useful for preloading without having to load the apng into bmpman
 * slots
 */
void apng_ani::preload () {
    _reading = false;
    _cache = true; // implied, otherwise preload is almost pointless
    while (current_frame < nframes) { next_frame (); }
    goto_start ();
}

/*
 * @brief get previous apng frame
 * @note due to apng format, this can only play backwards from the furthest
 * forward frame reached
 */
...
void apng_ani::next_frame () {
    ...
    if (_cache == false || (_frames.size () <= current_frame)) {
        if (current_frame > 0) {
            if (_dispose_op == 1) {
                // clear previous frame region of output buffer to fully
                // transparent black
                for (uint row = 0; row < _frameh; ++row) {
                    memset (
                        &frame.data.at (
                            (_y_offset + row) * _row_len + _x_offset * 4),
                        0, _framew * 4);
                }
            }
            else if (_dispose_op == 2) {
                // revert previous frames region of output buffer to previous
                // contents
                for (uint row = 0; row < _frameh; ++row) {
                    uint pos = (_y_offset + row) * _row_len + _x_offset * 4;
                    memcpy (
                        &frame.data.at (pos), &_frame_next.data.at (pos),
                        _framew * 4);
                }
            }
        ...
        if (_dispose_op == 2) {
            ...
            for (uint row = 0; row < _frameh; ++row) {
                uint pos = (_y_offset + row) * _row_len + _x_offset * 4;
                memcpy (
                    &_frame_next.data.at (pos), &frame.data.at (pos),
                    _framew * 4);
            }
        }
 ...
 */
size_t apng_ani::imgsize () { return _image_size; }
...
int apng_ani::load_header () {
    ...
    if (_cfp == nullptr) { _apng_failed ("couldn't open filename"); }
    ...
    _chunk.data.resize (
        25); // match the other sizes, maybe waste up to 13 bytes (ooooh)
    ...
    // read all data
    while (!cfeof (_cfp)) { _process_chunk (); }

    // should be at EOF; attach to _frame_offsets to make next_frame code
    // simpler
    ASSERTX (cfeof (_cfp) != 0, "apng not at EOF, get a coder!");
    ...
    if (nframes < 1) {
        _apng_failed (
            "animation didn't have any frames, is this a static png?");
    }

    // back to start, including reset of _cfp so it can be used for the 1st
    // frame
    _reading = false;
    if (cfseek (_cfp, _frame_offsets.at (0), CF_SEEK_SET) != 0) {
        _apng_failed ("couldn't seek to 1st fcTL offset");
...
apng_ani::apng_ani (const char* filename, bool cache)
    : w (0), h (0), bpp (32) // force all apngs to use this
      ,
      nframes (0), current_frame (0), plays (0), anim_time (0.0f),
      _filename (filename), _pngp (nullptr), _infop (nullptr), _cfp (nullptr),
      _offset (0), _sequence_num (0), _id (0), _row_len (0), _image_size (0),
      _framew (0), _frameh (0), _x_offset (0), _y_offset (0),
      _max_chunk_size (25) // IHDR must be 1st and it must be this big
      ,
      _delay_num (1), _delay_den (100), _dispose_op (0), _blend_op (0),
      _reading (true), _got_acTL (false), _got_IDAT (false), _cache (cache) {
    load_header ();
}

apng_ani::~apng_ani () { _cleanup_resources (); }
