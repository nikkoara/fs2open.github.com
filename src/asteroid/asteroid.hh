...
// these should always be equal for the benefit of generic asteroids (c.f.
// asteroid_page_in)
#define MAX_ACTIVE_DEBRIS_TYPES NUM_DEBRIS_SIZES

// Goober5000 - currently same as MAX_SHIP_DETAIL_LEVELS (put here to avoid an
// #include)
...
public:
        ...
        char pof_files[NUM_DEBRIS_POFS]
                      [MAX_FILENAME_LEN];                // POF files to load/associate with ship
        int num_detail_levels;                           // number of detail levels for this ship
        int detail_distance[MAX_ASTEROID_DETAIL_LEVELS]; // distance to change
                                                         // detail levels at
        ...
        int damage_type_idx_sav;                         // stored value from table used to reset
                                                         // damage_type_idx
        float
                inner_rad;               // radius within which maximum area effect damage is applied
        ...
        float fireball_radius_multiplier; // the model radius is multiplied by this
                                          // to determine the fireball size

        asteroid_info()
                : num_detail_levels(0), max_speed(0), damage_type_idx(0),
                  damage_type_idx_sav(-1), inner_rad(0), outer_rad(0), damage(0),
                  blast(0), initial_asteroid_strength(0),
...
struct asteroid {
        ...
        int check_for_wrap;    // timestamp to check for asteroid wrapping around
                               // field
        int check_for_collide; // timestamp to check for asteroid colliding with
                               // escort ships
        int final_death_time;  // timestamp to swap in new models after explosion
                               // starts
...
typedef enum { DG_ASTEROID,
               DG_SHIP } debris_genre_t;

// TYPEDEF FOR FIELD TYPE
typedef enum { FT_ACTIVE,
               FT_PASSIVE } field_type_t;
...
extern char
        Asteroid_icon_closeup_model[NAME_LENGTH]; // model for asteroid field
                                                  // briefing icon rendering
extern vec3d Asteroid_icon_closeup_position;      // closeup position for asteroid
                                                  // field briefing icon rendering
extern float Asteroid_icon_closeup_zoom;          // zoom position for asteroid field
                                                  // briefing icon rendering
...
int asteroid_check_collision(
        object *asteroid_objp, object *other_obj, vec3d *hitpos,
        collision_info_struct *asteroid_hit_info = NULL, vec3d *hitnormal = NULL);
void asteroid_hit(
        object *pasteroid_objp, object *other_objp, vec3d *hitpos, float damage);
int asteroid_count();
...
#endif // FREESPACE2_ASTEROID_ASTEROID_HH
