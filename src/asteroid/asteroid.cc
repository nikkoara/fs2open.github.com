...
asteroid_obj Asteroid_objs[MAX_ASTEROID_OBJS]; // array used to store asteroid
                                               // object indexes
...
#define ASTEROID_UPDATE_COLLIDE_TIMESTAMP \
    2000 // how often asteroid is checked for impending collisions with escort
         // ships
...
static void asteroid_obj_list_init () {
    ...
    for (i = 0; i < MAX_ASTEROID_OBJS; i++) { Asteroid_objs[i].flags = 0; }
}
...
static float asteroid_cap_speed (int asteroid_info_index, float speed) {
    ...
    while (speed > double_max) { speed *= 0.5f; }

    if (speed > max) { speed *= 0.75f; }
 ...
 */
static int asteroid_in_inner_bound_with_axes (
    asteroid_field* asfieldp, vec3d* pos, float delta) {
    ASSERT (asfieldp->has_inner_bound);

    int rval = 0;
    if ((pos->xyz.x > asfieldp->inner_min_bound.xyz.x - delta) &&
        (pos->xyz.x < asfieldp->inner_max_bound.xyz.x + delta)) {
        rval += 1;
    }

    if ((pos->xyz.y > asfieldp->inner_min_bound.xyz.y - delta) &&
        (pos->xyz.y < asfieldp->inner_max_bound.xyz.y + delta)) {
        rval += 2;
    }

    if ((pos->xyz.z > asfieldp->inner_min_bound.xyz.z - delta) &&
        (pos->xyz.z < asfieldp->inner_max_bound.xyz.z + delta)) {
        rval += 4;
    }
 ...
 */
static int
asteroid_in_inner_bound (asteroid_field* asfieldp, vec3d* pos, float delta) {
    if (!asfieldp->has_inner_bound) { return 0; }
...
static void inner_bound_pos_fixup (asteroid_field* asfieldp, vec3d* pos) {
    if (!asteroid_in_inner_bound (asfieldp, pos, 0)) { return; }
...
object* asteroid_create (
    asteroid_field* asfieldp, int asteroid_type, int asteroid_subtype) {
    int n, objnum;
    matrix orient;
    object* objp;
    asteroid* asp;
    ...
    for (n = 0; n < MAX_ASTEROIDS; n++) {
        if (!(Asteroids[n].flags & AF_USED)) { break; }
    }
    ...
    if ((asteroid_type < 0) ||
        (asteroid_type >=
         (((int)Species_info.size () + 1) * NUM_DEBRIS_SIZES))) {
        return NULL;
    }
    ...
    if (Game_mode & GM_MULTIPLAYER) { asteroid_subtype = 0; }

    asip = &Asteroid_info[asteroid_type];

    // bogus
    if (asip->modelp[asteroid_subtype] == NULL) { return NULL; }
    ...
    asp->check_for_collide =
        timestamp_rand (0, ASTEROID_UPDATE_COLLIDE_TIMESTAMP);
    ...
    if (model_get (asip->model_num[asteroid_subtype])->flags &
        PM_FLAG_HAS_INTRINSIC_ROTATE) {
        asp->model_instance_num =
            model_create_instance (false, asip->model_num[asteroid_subtype]);
    }
    ...
    objp->phys_info.flags |=
        (PF_REDUCED_DAMP |
         PF_DEAD_DAMP); // set damping equal for all axis and not changable
    ...
    objp->phys_info.I_body_inv.vec.rvec.xyz.x =
        1.0f / (objp->phys_info.mass * asip->modelp[asteroid_subtype]->rad);
    objp->phys_info.I_body_inv.vec.uvec.xyz.y =
        objp->phys_info.I_body_inv.vec.rvec.xyz.x;
    objp->phys_info.I_body_inv.vec.fvec.xyz.z =
        objp->phys_info.I_body_inv.vec.rvec.xyz.x;
    objp->hull_strength = asip->initial_asteroid_strength *
                          (0.8f + (float)Game_skill_level / NUM_SKILL_LEVELS) /
                          2.0f;
 ...
 */
void asteroid_sub_create (
    object* parent_objp, int asteroid_type, vec3d* relvec) {
    object* new_objp;
    float speed;
...
static void asteroid_load (int asteroid_info_index, int asteroid_subtype) {
    ...
    if ((asteroid_info_index >= (int)Asteroid_info.size ()) ||
        (asteroid_subtype >= NUM_DEBRIS_POFS)) {
        return;
    }

    asip = &Asteroid_info[asteroid_info_index];

    if (!VALID_FNAME (asip->pof_files[asteroid_subtype])) return;

    asip->model_num[asteroid_subtype] =
        model_load (asip->pof_files[asteroid_subtype], 0, NULL);

    if (asip->model_num[asteroid_subtype] >= 0) {
        polymodel* pm = asip->modelp[asteroid_subtype] =
 ...
 *
 * The weight is then used to determine the frequencty of different sizes of
 * ship debris
 */
...
void asteroid_create_all () {
    ...
    // ship_debris_odds_table keeps track of debris type of the next debris
    // piece each different type (size) of debris piece has a diffenent weight,
    // smaller weighted more heavily than larger. choose next type from table
    // ship_debris_odds_table by rand()%max_weighted_range, the threshold
    // *below* which the debris type is selected.
    struct {
    ...
    if (Asteroid_field.debris_genre == DG_SHIP) {
        ...
        for (idx = 0; idx < num_debris_types; idx++) {
            int debris_weight =
                get_debris_weight (Asteroid_field.field_debris_type[idx]);
            ship_debris_odds_table[idx].random_threshold =
                max_weighted_range + debris_weight;
            ship_debris_odds_table[idx].debris_type =
                Asteroid_field.field_debris_type[idx];
            max_weighted_range += debris_weight;
        }
    }
        ...
        if (Asteroid_field.debris_genre == DG_ASTEROID) {
            ...
            for (idx = 0; idx < MAX_ACTIVE_DEBRIS_TYPES; idx++) {
                ...
                if (rand_choice <
                    ...
                    asteroid_create (
                        &Asteroid_field,
                        ship_debris_odds_table[idx].debris_type, 0);
                    break;
                }
            }
        }
 ...
 * Multiplayer clients will always return 0 from this function.  We will force
 * a wrap on the clients when server tells us
 *
...
static int asteroid_should_wrap (object* objp, asteroid_field* asfieldp) {
    ...
    if (asfieldp->has_inner_bound) {
        if ((objp->pos.xyz.x > asfieldp->inner_min_bound.xyz.x) &&
            (objp->pos.xyz.x < asfieldp->inner_max_bound.xyz.x) &&
            (objp->pos.xyz.y > asfieldp->inner_min_bound.xyz.y) &&
            (objp->pos.xyz.y < asfieldp->inner_max_bound.xyz.y) &&
            (objp->pos.xyz.z > asfieldp->inner_min_bound.xyz.z) &&
            (objp->pos.xyz.z < asfieldp->inner_max_bound.xyz.z)) {
            return 1;
        }
    }
...
static void asteroid_wrap_pos (object* objp, asteroid_field* asfieldp) {
    if (objp->pos.xyz.x < asfieldp->min_bound.xyz.x) {
        objp->pos.xyz.x = asfieldp->max_bound.xyz.x +
                          (objp->pos.xyz.x - asfieldp->min_bound.xyz.x);
    }
    ...
    // wrap on inner bound, check all 3 axes as needed, use of rand ok for
    // multiplayer with send_asteroid_throw()
...
static int asteroid_is_targeted (object* objp) {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (Ai_info[Ships[ship_objp->instance].ai_index].target_objnum ==
            asteroid_obj_index) {
            return 1;
        }
    }
 ...
 */
static void asteroid_aim_at_target (
    object* objp, object* asteroid_objp, float delta_time) {
    vec3d predicted_center_pos;
    vec3d rand_vec;
    float speed;

    vm_vec_scale_add (
        &predicted_center_pos, &objp->pos, &objp->phys_info.vel, delta_time);
    ...
    vm_vec_scale_add (
        &asteroid_objp->pos, &predicted_center_pos,
        &asteroid_objp->phys_info.vel, -delta_time);
    vm_vec_scale_add (
        &asteroid_objp->last_pos, &asteroid_objp->pos,
        &asteroid_objp->phys_info.vel, -flFrametime);
}
...
static void maybe_throw_asteroid (int count) {
    if (!timestamp_elapsed (Next_asteroid_throw)) { return; }
    ...
    Next_asteroid_throw =
        timestamp (1000 + 1200 * count / (Game_skill_level + 1));
 ...
 * Only reposition if oustide the bounding volume and the player isn't looking
 * towards the asteroid.
 */
static void
asteroid_maybe_reposition (object* objp, asteroid_field* asfieldp) {
    // passive field does not wrap
    if (asfieldp->field_type == FT_PASSIVE) { return; }
    ...
    if (asteroid_should_wrap (objp, asfieldp)) {
        ...
        if (!asteroid_is_targeted (objp)) {
            // only wrap if player won't see asteroid disappear/reverse
            // direction
            dist = vm_vec_normalized_dir (
                &vec_to_asteroid, &objp->pos, &Eye_position);
            ...
            if ((dot < 0.7f) || (dist > asfieldp->bound_rad)) {
                if (Num_asteroids > MAX_ASTEROIDS - 10) {
                    ...
                    if ((dot > 0.7f) &&
                        (dist < (asfieldp->bound_rad * 1.3f))) {
                        // player would see asteroid pop out other side, so
                        // reverse velocity instead of wrapping
                        objp->pos = old_asteroid_pos;
                        vm_vec_copy_scale (
                            &objp->phys_info.vel, &old_vel, -1.0f);
                        objp->phys_info.desired_vel = objp->phys_info.vel;
                        Asteroids[objp->instance].target_objnum = -1;
                    }
                    ...
                    vm_vec_scale_add (
                        &objp->last_pos, &objp->pos, &objp->phys_info.vel,
                        -flFrametime);
...
void asteroid_process_pre (object* objp) {
    if (Asteroids_enabled) {
        ...
        lerp (
            &objp->phys_info.vel.xyz.x, objp->phys_info.vel.xyz.x,
            objp->phys_info.desired_vel.xyz.x, flFrametime);
        lerp (
            &objp->phys_info.vel.xyz.y, objp->phys_info.vel.xyz.y,
            objp->phys_info.desired_vel.xyz.y, flFrametime);
        lerp (
            &objp->phys_info.vel.xyz.z, objp->phys_info.vel.xyz.z,
            objp->phys_info.desired_vel.xyz.z, flFrametime);
    }
}

int asteroid_check_collision (
    object* pasteroid, object* other_obj, vec3d* hitpos,
    collision_info_struct* asteroid_hit_info, vec3d* hitnormal) {
    ...
    if (asteroid_hit_info == NULL) {
        ...
        mc.model_num =
            Asteroid_info[Asteroids[num].asteroid_type]
                .model_num[asteroid_subtype]; // Fill in the model to check
            ...
            *hitpos = mc.hit_point_world;
            ...
            if (hitnormal) {
                ...
                model_find_world_dir (
                    &normal, &mc.hit_normal, mc.model_num, mc.hit_submodel,
                    mc.orient);
    ...
    // Collision detection from rotation enabled if at rotaion is less than 30
    // degree in frame This should account for all ships
    if ((vm_vec_mag_squared (&heavy->phys_info.rotvel) * flFrametime *
         flFrametime) < (PI * PI / 36)) {
        ...
        // HACK - this applies to big ships warping in/out of asteroid fields -
        // not sure what it does
        if (vm_vec_mag (&asteroid_hit_info->light_rel_vel) > 300) {
            asteroid_hit_info->collide_rotate = 0;
            vm_vec_sub (
                &asteroid_hit_info->light_rel_vel, &lighter->phys_info.vel,
                &heavy->phys_info.vel);
        }
    }
    else {
    ...
    if (asteroid_hit_info->heavy ==
        pship_obj) { // ship is heavier, so asteroid is sphere. Check sphere
                     // collision against ship poly model
        mc.model_instance_num = Ships[pship_obj->instance].model_instance_num;
        mc.model_num = Ship_info[Ships[pship_obj->instance].ship_info_index]
        ...
        // copy important data
        int copy_flags = mc.flags; // make a copy of start end positions of
                                   // sphere in  big ship RF
        vec3d copy_p0, copy_p1;
        copy_p0 = *mc.p0;
        copy_p1 = *mc.p1;

        // first test against the sphere - if this fails then don't do any
        // submodel tests
        ...
        if (model_collide (&mc)) {
            ...
            if (asteroid_hit_info->collide_rotate) {
                // We collide with the sphere, find the list of rotating
                // submodels and test one at a time
                model_get_rotating_submodel_list (&submodel_vector, heavy_obj);

                // Get polymodel and turn off all rotating submodels, collide
                // against only 1 at a time.
                pmi = model_get_instance (
                    Ships[heavy_obj->instance].model_instance_num);
                ...
                // reset flags to check MC_CHECK_MODEL | MC_CHECK_SPHERELINE
                // and maybe MC_CHECK_INVISIBLE_FACES and MC_SUBMODEL_INSTANCE
                ...
                for (size_t i = 0; i < submodel_vector.size (); i++) {
                    ...
                    pmi->submodel[submodel_vector[i]].collision_checked =
                        false;
                    ...
                    // submodel RF
                    pmi->submodel[submodel_vector[i]].angs =
                        pmi->submodel[submodel_vector[i]].prev_angs;
                    world_find_model_instance_point (
                        &p0, &light_obj->last_pos, pmi, submodel_vector[i],
                        &heavy_obj->last_orient, &heavy_obj->last_pos);

                    pmi->submodel[submodel_vector[i]].angs = copy_angles;
                    world_find_model_instance_point (
                        &p1, &light_obj->pos, pmi, submodel_vector[i],
                        &heavy_obj->orient, &heavy_obj->pos);
                    ...
                    if (model_collide (&mc)) {
                        if (mc.hit_dist < asteroid_hit_info->hit_time) {
                            ...
                            model_instance_find_world_point (
                                &asteroid_hit_info->light_collision_cm_pos,
            ...
            *mc.p1 = copy_p1;
            ...
            if (model_collide (&mc)) {
                ...
                if (mc.hit_dist < asteroid_hit_info->hit_time) {
                    ...
                    set_hit_struct_info (
                        asteroid_hit_info, &mc, SUBMODEL_NO_ROT_HIT);
                    ...
                    vm_vec_sub (&diff, mc.p1, mc.p0);
                    vm_vec_scale_add (
                        &asteroid_hit_info->light_collision_cm_pos, mc.p0,
                        &diff, mc.hit_dist);
                }
            }
        }
        ...
        model_clear_instance (mc.model_num);
        mc.orient = &pasteroid->orient; // The object's orient
        mc.radius = model_get_core_radius (
            Ship_info[Ships[pship_obj->instance].ship_info_index].model_num);
        ...
        if (mc_ret_val) {
            ...
            if (!asteroid_hit_info->edge_hit) {
                vm_vec_unrotate (
                    &asteroid_hit_info->collision_normal, &mc.hit_normal,
                    &heavy->orient);
            }
            ...
            vm_vec_sub (&diff, mc.p1, mc.p0);
            vm_vec_scale_add (
                &asteroid_hit_info->light_collision_cm_pos, mc.p0, &diff,
                mc.hit_dist);
        }
    }
    ...
    if (mc_ret_val) {
        ...
        vm_vec_sub (
            &asteroid_hit_info->r_light, &asteroid_hit_info->hit_pos,
            &asteroid_hit_info->light_collision_cm_pos);
        ...
        if (asteroid_hit_info->edge_hit) {
            vm_vec_copy_normalize (
                &asteroid_hit_info->collision_normal,
                &asteroid_hit_info->r_light);
            vm_vec_negate (&asteroid_hit_info->collision_normal);
        }

        // get world hitpos
        vm_vec_add (
            hitpos, &asteroid_hit_info->heavy->pos,
            &asteroid_hit_info->r_heavy);
...
void asteroid_render (object* obj, model_draw_list* scene) {
    if (Asteroids_enabled) {
        ...
        model_clear_instance (Asteroid_info[asp->asteroid_type]
                                  .model_num[asp->asteroid_subtype]);
        ...
        model_render_queue (
            &render_info, scene,
            Asteroid_info[asp->asteroid_type].model_num[asp->asteroid_subtype],
            &obj->orient,
            &obj->pos); // Replace MR_NORMAL with 0x07 for big yellow blobs
    }
}

/**
 * Create a normalized vector generally in the direction from *hitpos to
 * other_obj->pos
 */
...
static float asteroid_get_fireball_scale_multiplier (int num) {
    if (Asteroids[num].flags & AF_USED) {
        ...
        if (asip->fireball_radius_multiplier >= 0) {
            ...
            case ASTEROID_TYPE_LARGE: return 1.5f; break;
...
static float asteroid_create_explosion (object* objp) {
    ...
    asteroid_info* asip =
        &Asteroid_info[Asteroids[objp->instance].asteroid_type];
    ...
    fireball_scale_multiplier =
        asteroid_get_fireball_scale_multiplier (objp->instance);

    fireball_objnum = fireball_create (
        &objp->pos, fireball_type, FIREBALL_LARGE_EXPLOSION, OBJ_INDEX (objp),
...
static void asteroid_explode_sound (object* objp, int type, int play_loud) {
    ...
    float range_factor =
        1.0f; // how many times sound should traver farther than normal
    ...
    if (!play_loud) { range_factor = 1.0f; }

    snd_play_3d (
        gamesnd_get_game_sound (sound_index), &objp->pos, &Eye_position,
        objp->radius, NULL, 0, 1.0f, SND_PRIORITY_MUST_PLAY, NULL,
        range_factor);
}
...
static void asteroid_do_area_effect (object* asteroid_objp) {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (weapon_area_calc_damage (
                ship_objp, &asteroid_objp->pos, asip->inner_rad,
                asip->outer_rad, asip->blast, asip->damage, &blast, &damage,
                asip->outer_rad) == -1)
            continue;

        ship_apply_global_damage (
            ship_objp, asteroid_objp, &asteroid_objp->pos, damage);
        weapon_area_apply_blast (
            NULL, ship_objp, &asteroid_objp->pos, blast, 0);
    } // end for
}

/**
 * Upon asteroid asteroid_obj being hit. Apply damage and maybe make it break
 ...
 */
void asteroid_hit (
    object* pasteroid_obj, object* other_obj, vec3d* hitpos, float damage) {
    ...
    if (pasteroid_obj->hull_strength < 0.0f) {
        if (asp->final_death_time <= 0) {
            ...
            asteroid_explode_sound (
                pasteroid_obj, asp->asteroid_type, play_loud_collision);
            ...
            if (hitpos) { asp->death_hit_pos = *hitpos; }
                ...
                // randomize hit pos a bit, otherwise we will get a NULL vector
                // when trying to find direction to toss child asteroids
                vec3d rand_vec;
                vm_vec_rand_vec_quick (&rand_vec);
        ...
        if (other_obj->type == OBJ_WEAPON) {
            ...
            // If the weapon didn't play any impact animation, play custom
            // asteroid impact animation
            if (!wip->impact_weapon_expl_effect.isValid ()) {
                particle::create (
                    hitpos, &vmd_zero_vector, 0.0f,
                    Asteroid_impact_explosion_radius,
                    particle::PARTICLE_BITMAP, Asteroid_impact_explosion_ani);
            }
        }
    }
...
void asteroid_level_close () {
    ...
    for (i = 0; i < MAX_ASTEROIDS; i++) {
        if (Asteroids[i].flags & AF_USED) {
            ...
            ASSERT (
                Asteroids[i].objnum >= 0 && Asteroids[i].objnum < MAX_OBJECTS);
            Objects[Asteroids[i].objnum].flags.set (
...
DCF_BOOL2 (
    asteroids, Asteroids_enabled, "enables or disables asteroids",
    "Usage: asteroids [bool]\nTurns asteroid system on/off.  If nothing "
    "passed, then toggles it.\n");

/**
 * Return the number of active asteroids
 */
int asteroid_count () { return Num_asteroids; }
...
static void asteroid_test_collide (
    object* pasteroid_obj, object* pship_obj, mc_info* mc, bool lazy = false) {
    float asteroid_ray_dist;
    vec3d asteroid_fvec, terminus;

    // See if ray from asteroid intersects bounding box of escort ship
    asteroid_ray_dist =
        vm_vec_mag_quick (&pasteroid_obj->phys_info.desired_vel) *
    ...
    if (IS_VEC_NULL_SQ_SAFE (&asteroid_fvec)) {
        ...
        vm_vec_scale_add (
            &terminus, &pasteroid_obj->pos, &asteroid_fvec, asteroid_ray_dist);
    }
    ...
    if (lazy) { mc->flags = MC_CHECK_MODEL | MC_ONLY_BOUND_BOX; }
    else {
        mc->flags = MC_CHECK_MODEL | MC_CHECK_SPHERELINE;
    }
    ...
    // PVS-Studio says that mc->p1 will become invalid... on the one hand, it
    // will; on the other hand, we won't use it again; on the *other* other
    // hand, we should keep things proper in case of future changes
    mc->p1 = NULL;
}

/**
 * Test if asteroid will collide with escort ship within
...
static int asteroid_valid_ship_to_warn_collide (ship* shipp) {
    if (!(Ship_info[shipp->ship_info_index].is_big_or_huge ())) { return 0; }
    ...
    // Goober5000 used to be if teams were unequal and player was not traitor,
    // but this works for allies not on your team
    if (iff_x_attacks_y (Player_ship->team, shipp->team)) { return 0; }

    return 1;
}

/**
 * See if asteroid will collide with a large ship on the escort list in the
 * next ASTEROID_MIN_COLLIDE_TIME seconds.
 */
    ...
    for (i = 0; i < num_escorts; i++) {
        ...
        if (escort_objnum >= 0) {
            ...
            if (asteroid_valid_ship_to_warn_collide (escort_shipp)) {
                will_collide = asteroid_will_collide (
                    asteroid_objp, &Objects[escort_objnum]);
                if (will_collide) {
    ...
    if (Asteroids_enabled) {
        ...
        if (Asteroid_field.field_type == FT_ACTIVE) {
            if (timestamp_elapsed (asp->check_for_wrap)) {
                ...
                asp->check_for_wrap =
                    timestamp (ASTEROID_CHECK_WRAP_TIMESTAMP);
            }
        }
        ...
        if (timestamp_elapsed (asp->check_for_collide)) {
            ...
            asp->check_for_collide =
                timestamp (ASTEROID_UPDATE_COLLIDE_TIMESTAMP);
        }
...
float asteroid_time_to_impact (object* asteroid_objp) {
    ...
    if (asp->collide_objnum < 0) { return time; }

    asteroid_test_collide (
        asteroid_objp, &Objects[asp->collide_objnum], &mc, true);

    if (mc.num_hits) {
        total_dist = vm_vec_dist (&mc.hit_point_world, &asteroid_objp->pos) -
                     asteroid_objp->radius;
        if (total_dist < 0) { total_dist = 0.0f; }
...
static void asteroid_parse_section (asteroid_info* asip) {
    ...
    asip->num_detail_levels = stuff_int_list (
        asip->detail_distance, MAX_ASTEROID_DETAIL_LEVELS, RAW_INTEGER_TYPE);
    ...
    if (optional_string ("$Explosion Animations:")) {
        ...
        asip->explosion_bitmap_anims.insert (
            asip->explosion_bitmap_anims.begin (), temp, temp + parsed_ints);
    }
    ...
    try {
        ...
        while (required_string_either ("#End", "$Name:")) {
            ...
            if (tally >= max_asteroids) {
                ...
                WARNINGF (LOCATION, "Ignoring extra asteroid/debris");
#else
                std::string msg ("Ignoring extra asteroid/debris '");
                msg.append (new_asteroid.name);
                msg.append ("'\n");
                WARNINGF (LOCATION, "%s", msg.c_str ());
                ...
                case ASTEROID_TYPE_SMALL: msg.append (" small\n"); break;
        ...
        if (VALID_FNAME (impact_ani_file)) {
            int num_frames;
            Asteroid_impact_explosion_ani = bm_load_animation (
                impact_ani_file, &num_frames, nullptr, nullptr, nullptr, true);
        }
        ...
        if (optional_string ("$Briefing Icon Closeup Position:")) {
            ...
            vm_vec_make (
                &Asteroid_icon_closeup_position, 0.0f, 0.0f,
                -334.0f); // magic numbers from retail
        }
...
static int set_asteroid_throw_objnum () {
    if (Asteroid_field.num_initial_asteroids < 1) return -1;
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (Ship_info[Ships[ship_objp->instance].ship_info_index]
            ...
            if (ship_objp->pos.xyz.x + radius > Asteroid_field.min_bound.xyz.x)
                if (ship_objp->pos.xyz.y + radius >
                    Asteroid_field.min_bound.xyz.y)
                    if (ship_objp->pos.xyz.z + radius >
                        Asteroid_field.min_bound.xyz.z)
                        if (ship_objp->pos.xyz.x - radius <
                            Asteroid_field.max_bound.xyz.x)
                            if (ship_objp->pos.xyz.y - radius <
                                Asteroid_field.max_bound.xyz.y)
                                if (ship_objp->pos.xyz.z - radius <
                                    Asteroid_field.max_bound.xyz.z)
                                    if (!asteroid_in_inner_bound (
                                            &Asteroid_field, &ship_objp->pos,
                                            radius))
                                        return so->objnum;
        }
    }
...
void asteroid_frame () {
    if (Num_asteroids < 1) return;

    // Only throw if active field
    if (Asteroid_field.field_type == FT_PASSIVE) { return; }
 ...
 */
void asteroid_init () { asteroid_parse_tbl (); }

extern int Cmdline_targetinfo;

/**
 * Draw brackets around on-screen asteroids that are about to collide,
 * otherwise draw an offscreen indicator
 */
void asteroid_show_brackets () {
    ...
    // get pointer to player target, so we don't need to take OBJ_INDEX() of
    // asteroid_objp to compare to target_objnum
    ...
    for (asteroid_objp = GET_FIRST (&obj_used_list);
         asteroid_objp != END_OF_LIST (&obj_used_list);
         asteroid_objp = GET_NEXT (asteroid_objp)) {
        if (asteroid_objp->type != OBJ_ASTEROID) { continue; }

        asp = &Asteroids[asteroid_objp->instance];

        if (asp->collide_objnum < 0) { continue; }

        if (asteroid_objp == player_target) { continue; }
        ...
        if (Cmdline_targetinfo) {
            hud_target_add_display_list (
                asteroid_objp, &asteroid_vertex, &asteroid_objp->pos, 0, NULL,
                NULL, TARGET_DISPLAY_DIST | TARGET_DISPLAY_LEAD);
        }
        else {
            hud_target_add_display_list (
                asteroid_objp, &asteroid_vertex, &asteroid_objp->pos, 0, NULL,
                NULL, TARGET_DISPLAY_DIST);
        }
    }
}
        ...
        for (i = 0; i < MAX_ACTIVE_DEBRIS_TYPES; i++) {
            ...
            for (k = 0; k < NUM_DEBRIS_POFS; k++) {
                ...
                if (Asteroid_field.debris_genre == DG_SHIP) {
                    ...
                    // ASTEROID DEBRIS - use subtype
                    // (Asteroid_field.field_debris_type[] != -1)
                    if (Asteroid_field.field_debris_type[k] == -1) {
                        continue;
                    }
                }

                if (asip->model_num[k] < 0) continue;
