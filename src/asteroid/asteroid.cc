        ...
        asteroid_field *asfieldp, int asteroid_type, int asteroid_subtype)
{
        ...
        speed = asteroid_cap_speed(
                asteroid_type,
                asfieldp->speed * fs2::prng::randf(0, 0.5f + (float)Game_skill_level / NUM_SKILL_LEVELS, 2.0f + (float)(2 * Game_skill_level) / NUM_SKILL_LEVELS));
...
{
        if (objp->pos.xyz.x < asfieldp->min_bound.xyz.x || objp->pos.xyz.y < asfieldp->min_bound.xyz.y || objp->pos.xyz.z < asfieldp->min_bound.xyz.z || objp->pos.xyz.x > asfieldp->max_bound.xyz.x || objp->pos.xyz.y > asfieldp->max_bound.xyz.y || objp->pos.xyz.z > asfieldp->max_bound.xyz.z)
                return 1;

        // check against inner bound
        if (asfieldp->has_inner_bound) {
                if ((objp->pos.xyz.x > asfieldp->inner_min_bound.xyz.x) && (objp->pos.xyz.x < asfieldp->inner_max_bound.xyz.x) && (objp->pos.xyz.y > asfieldp->inner_min_bound.xyz.y) && (objp->pos.xyz.y < asfieldp->inner_max_bound.xyz.y) && (objp->pos.xyz.z > asfieldp->inner_min_bound.xyz.z) && (objp->pos.xyz.z < asfieldp->inner_max_bound.xyz.z)) {
                        return 1;
                }
        }
        ...
        collision_info_struct *asteroid_hit_info, vec3d *hitnormal)
{
        ...
        if (asteroid_hit_info == NULL) {
                ...
                mc.model_num = Asteroid_info[Asteroids[num].asteroid_type]
        ...
        if (asteroid_hit_info->heavy == pship_obj) { // ship is heavier, so asteroid is sphere. Check sphere
                ...
                if (model_collide(&mc)) {
                        ...
                        if (asteroid_hit_info->collide_rotate) {
                                ...
                                for (size_t i = 0; i < submodel_vector.size(); i++) {
                                        ...
                                        // p0)
                                        angles_t copy_angles = pmi->submodel[submodel_vector[i]].angs;
        ...
        if (pasteroid_obj->hull_strength < 0.0f) {
                if (asp->final_death_time <= 0) {
                        ...
                        asp->final_death_time = timestamp(
                                int(explosion_life * 1000.0f) / 5); // Wait till 30% of vclip time before breaking the asteroid
                        // up.
                        if (hitpos) {
...
static void asteroid_parse_tbl()
{
        ...
        try {
                ...
                size_t tally = 0;
                size_t max_asteroids = NUM_DEBRIS_SIZES + Species_info.size() * NUM_DEBRIS_SIZES;
