...
class Monitor : public MonitorBase {
        ...
        static_assert(
                ...
                "Monitor values must be convertible to float!");

public:
        Monitor(const char *name, const T &defaultVal)
                : MonitorBase(name), _value(defaultVal) { }
...
#define MONITOR(function_name) \
        static ::tracing::Monitor< int > mon_##function_name(#function_name, 0);

// Increments a monitor variable
#define MONITOR_INC(function_name, inc)       \
        do {                                  \
                mon_##function_name += (inc); \
        } while (0)

#endif // FREESPACE2_TRACING_MONITOR_HH
