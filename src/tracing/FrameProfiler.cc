...
void process_begin (
    std::vector< profile_sample >& samples, const trace_event& evt) {
    int parent = -1;
    for (int i = 0; i < (int)samples.size (); i++) {
        if (!samples[i].open_profiles) { continue; }
    ...
    for (int i = 0; i < (int)samples.size (); i++) {
        if (!strcmp (samples[i].name.c_str (), evt.category->getName ()) &&
            samples[i].parent == parent) {
...
void process_end (
    ...
    for (int i = 0; i < (int)samples.size (); i++) {
        if (samples[i].open_profiles) {
            if (samples[i].num_children == 1) { child_of = i; }
        }
    }

    for (int i = 0; i < (int)samples.size (); i++) {
        if (!strcmp (samples[i].name.c_str (), evt.category->getName ()) &&
            samples[i].parent == child_of) {
            int inner = 0;
            int parent = -1;
            ...
            while (inner < (int)samples.size ()) {
                if (samples[inner].open_profiles > 0) {
                    ...
                    if (parent < 0) {
                        ...
                        samples[inner].start_time >=
                        samples[parent].start_time) {
            ...
            if (parent >= 0) {
                ...
                samples[parent].children_sample_time +=
                    end_time - samples[i].start_time;
            }
...
void FrameProfiler::processEvent (const trace_event* event) {
    ...
    if (_mainThreadID == -1) {
        // We need the ID of the main thread to filter out multi threaded
        // events. We just assume that the first event of the main process we
        // see is from the main thread. That should be a safe assumption.
        _mainThreadID = event->tid;
    }

    if (event->tid != _mainThreadID) {
        // Multithreaded events don't have a deterministic sequence and that
        // confuses the old profiling system
        return;
    }
    ...
    if (event->category == &MainFrame) {
        // The main frame category doesn't work right since the output is
        // generated while we are still in that category
        return;
    }
...
void FrameProfiler::get_profile_from_history (
    std::string& name, uint64_t* avg_micro_sec, uint64_t* min_micro_sec,
...
void FrameProfiler::store_profile_in_history (
    ...
    for (int i = 0; i < (int)history.size (); i++) {
        if (history[i].valid && history[i].name == name) {
            // found the sample
            history[i].avg_micro_sec = (uint64_t) (
                (history[i].avg_micro_sec * old_ratio) + (time * new_ratio));

            if (time < history[i].min_micro_sec) {
                history[i].min_micro_sec = time;
            }
            else {
                history[i].min_micro_sec = (uint64_t) (
                    (history[i].min_micro_sec * old_ratio) +
                    (time * new_ratio));
            }
            ...
            if (time > history[i].max_micro_sec) {
                ...
                history[i].max_micro_sec = (uint64_t) (
                    (history[i].max_micro_sec * old_ratio) +
                    (time * new_ratio));
            }
    ...
    new_history.avg_micro_sec = new_history.min_micro_sec =
        new_history.max_micro_sec = time;

    history.push_back (new_history);
}
void FrameProfiler::dump_output (
    std::stringstream& out, uint64_t /*start_profile_time*/,
    uint64_t /*end_profile_time*/, std::vector< profile_sample >& samples) {
    ...
    for (int i = 0; i < (int)samples.size (); i++) {
        ...
        avg_micro_seconds = min_micro_seconds = max_micro_seconds =
            sample_time;
        ...
        get_profile_from_history (
            samples[i].name, &avg_micro_seconds, &min_micro_seconds,
            &max_micro_seconds);
...
std::string FrameProfiler::getContent () { return content; }
void FrameProfiler::processFrame () {
    ...
    for (auto& event : _bufferedEvents) {
        ...
        switch (event.type) {
        case EventType::Begin: process_begin (samples, event); break;
        case EventType::End: process_end (samples, event); break;
        default: break;
        }
    }
