...
struct neb2_detail {
        ...
        float break_alpha;      // break alpha (below which, poofs don't draw). this
                                // affects the speed and visual quality a lot
        float break_x, break_y; // x and y alpha fade/break values. adjust alpha on
                                // the polys as they move offscreen
...
// NOTE : in most cases you will want to pass -1.0f for outer_radius. Trust me
// on this
int neb2_create(
        vec3d *offset, int num_poofs, float inner_radius, float outer_radius,
...
// get adjusted near and far fog values (allows mission-specific fog
// adjustments)
void neb2_get_adjusted_fog_values(
        float *fnear, float *ffar, object *obj = NULL);
...
#endif // FREESPACE2_NEBULA_NEB_HH
