...
neb2_detail Neb2_detail[MAX_DETAIL_LEVEL] = {
    {
        ...
        0.13f, // break alpha (below which, poofs don't draw). this affects the
               // speed and visual quality a lot
        150.0f, 150.0f / 1.3333f, // x and y alpha fade/break values. adjust
                                  // alpha on the polys as they move offscreen
    ...
    },
    {
        ...
        0.575f, // max alpha for this detail level in Glide
        0.71f,  // max alpha for this detail level in D3d
        0.125f, // break alpha (below which, poofs don't draw). this affects
                // the speed and visual quality a lot
        300.0f, 300.0f / 1.3333f, // x and y alpha fade/break values. adjust
                                  // alpha on the polys as they move offscreen
    ...
    },
    {
        ...
        0.575f, // max alpha for this detail level in Glide
        0.71f,  // max alpha for this detail level in D3d
        0.1f, // break alpha (below which, poofs don't draw). this affects the
              // speed and visual quality a lot
        300.0f, 300.0f / 1.3333f, // x and y alpha fade/break values. adjust
                                  // alpha on the polys as they move offscreen
        550.0f,                   // total dimension of player poof cube
    ...
    },
    {
        ...
        0.05f, // break alpha (below which, poofs don't draw). this affects the
               // speed and visual quality a lot
        200.0f, 200.0f / 1.3333f, // x and y alpha fade/break values. adjust
                                  // alpha on the polys as they move offscreen
...
// return the alpha the passed poof should be rendered with, for a 2 shell
// nebula
float neb2_get_alpha_2shell (
    float inner_radius, float outer_radius, float magic_num, vec3d* v);
...
void neb2_level_init () { Nebula_sexp_used = false; }

// initialize nebula stuff - call from game_post_level_init(), so the mission
// has been loaded
void neb2_post_level_init () {
    ...
    if (strlen (Neb2_texture_name)) {
        ...
        if ((Neb2_htl_fog_data != NULL) &&
            (pcx_read_header (
                 Neb2_texture_name, NULL, NULL, NULL, NULL,
                 Neb2_htl_fog_data) == PCX_ERROR_NONE)) {
            // based on the palette, get an average color value (this doesn't
            // really account for actual pixel usage though)
            ushort r = 0, g = 0, b = 0, pcount = 0;
            for (idx = 0; idx < 768; idx += 3) {
                if (Neb2_htl_fog_data[idx] || Neb2_htl_fog_data[idx + 1] ||
                    Neb2_htl_fog_data[idx + 2]) {
    ...
    // if we are going to use fullneb, but aren't fullneb yet, then be sure to
    // reset our mode
    if (!(The_mission.flags[Mission::Mission_Flags::Fullneb])) {
        Neb2_render_mode = NEB2_RENDER_NONE;
        Neb2_awacs = -1.0f;
...
void neb2_level_close () {
    ...
    // unflag the mission as being fullneb so stuff doesn't fog in the techdata
    // room :D
...
void neb2_render_setup (camid cid) {
    ...
    if (Neb2_render_mode == NEB2_RENDER_HTL) {
        // RT The background needs to be the same colour as the fog and this
        // seems to be the ideal place to do it
...
DCF (neb_skip, "Toggles culling of objects obscured by nebula") {
    ...
    if (neb_skip_opt) { dc_printf ("Using neb object skipping!\n"); }
    else {
...
int neb2_skip_render (object* objp, float z_depth) {
    ...
    case OBJ_GHOST:
    case OBJ_BEAM:
    case OBJ_WAYPOINT: return 0;

    // any weapon over 500 meters away
    // Use the "far" distance multiplier here
    case OBJ_WEAPON:
        if (z_depth >= 500.0f * Neb2_fog_far_mult) { return 1; }
        break;

    // any small ship over the fog limit, or any cruiser 50% further than the
    // fog limit
    case OBJ_SHIP:
        ship_info* sip;
        if ((objp->instance >= 0) &&
...
float neb2_get_lod_scale (int objnum) {
    ...
    // small ship?
    if (sip->is_small_ship ()) { return 1.8f; }
...
float neb2_get_alpha_2shell (
    ...
    // if the point is outside the inner radius, it starts out as completely
    // transparent at max outer radius, and becomes more opaque as it moves
    // towards inner radius
...
float g3_draw_rotated_bitmap_area (
    vertex* pnt, float angle, float rad, uint tmap_flags, float area);
int neb_mode = 1;
int frames_total = 0;
int frame_count = 0;
float frame_avg;
void neb2_render_player () {
    ...
    if (Neb2_render_mode == NEB2_RENDER_NONE) { return; }
    ...
    for (idx1 = 0; idx1 < Neb2_slices; idx1++) {
        for (idx2 = 0; idx2 < Neb2_slices; idx2++) {
            for (idx3 = 0; idx3 < Neb2_slices; idx3++) {
                ...
                if (Neb2_cubes[idx1][idx2][idx3].bmap == -1) { continue; }

                pneb_tried++;

                // rotate the poof
                Neb2_cubes[idx1][idx2][idx3].rot +=
                    (Neb2_cubes[idx1][idx2][idx3].rot_speed * flFrametime);
                ...
                // useless
                if (vm_vec_dot_to_point (
                        &eye_orient.vec.fvec, &eye_pos,
                        &Neb2_cubes[idx1][idx2][idx3].pt) <= 0.0f) {
                    pneb_tossed_dot++;
                    continue;
                }
                ...
                alpha = neb2_get_alpha_2shell (
                    Nd->cube_inner, Nd->cube_outer, Nd->prad / 4.0f,
                    &Neb2_cubes[idx1][idx2][idx3].pt);
                ...
                if ((ptemp.screen.xyw.x < 0.0f) ||
                    ...
                    alpha = neb2_get_alpha_offscreen (
                        ptemp.screen.xyw.x, ptemp.screen.xyw.y, alpha);
                }
                ...
                // gr_set_bitmap(Neb2_cubes[idx1][idx2][idx3].bmap,
                // GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL, (alpha +
                // Neb2_cubes[idx1][idx2][idx3].flash));
                ...
                material mat_params;
                material_set_unlit (
                    &mat_params, Neb2_cubes[idx1][idx2][idx3].bmap,
                    alpha + Neb2_cubes[idx1][idx2][idx3].flash, true, true);
...
// call this when the player's viewpoint has changed, this will cause the code
// to properly reset the eye's local poofs
void neb2_eye_changed () { Neb2_regen = 1; }
...
void neb2_get_adjusted_fog_values (float* fnear, float* ffar, object* objp) {
    ...
    *ffar *= Neb2_fog_far_mult;
    ...
    if ((*fnear - *ffar) == 0) *ffar = *fnear + 1.0f;
}

float nNf_near, nNf_far;
// given a position in space, return a value from 0.0 to 1.0 representing the
// fog level
...
// this only gets called after the one above has been called as it assumes you
// have set the near and far planes properly doun't use this outside of ship
// rendering
...
// If you want to save some rendering time, I would suggest maybe kill this
// off. It doesn't use much, but it APPEARS to be fairly useless unless someone
...
void neb2_pre_render (camid cid) {
    ...
    if (Neb2_render_mode != NEB2_RENDER_POF) { return; }
...
#define PIXEL_INDEX_SMALL(xv, yv)                    \
    ((this_esize * (yv)*gr_screen.bytes_per_pixel) + \
...
void neb2_get_pixel (int x, int y, int* r, int* g, int* b) {
    ...
    // pixel plus all immediate neighbors (on the small screen - should be more
    // effective than 2 or 1)
    xs = (int)(ex_scale * x);
    ys = (int)(ey_scale * y);

    // sometimes goes over by 1 in direct3d
    if (ys >= (this_esize - 1)) { ys--; }

    avg_count = 0;
    bm_get_components (
        &tpixels[PIXEL_INDEX_SMALL (xs, ys)], &rv, &gv, &bv, NULL);
    ra = rv;
    ga = gv;
    ba = bv;
    avg_count++;
    if (xs > 0) {
        bm_get_components (
            &tpixels[PIXEL_INDEX_SMALL (xs - 1, ys)], &rv, &gv, &bv,
            NULL); // left
        ra += rv;
        ba += bv;
        ga += gv;
    ...
    if (ys > 0) {
        bm_get_components (
            &tpixels[PIXEL_INDEX_SMALL (xs, ys - 1)], &rv, &gv, &bv,
            NULL); // top
        ra += rv;
        ba += bv;
        ga += gv;
    ...
    if ((xs > 0) && (ys > 0)) {
        bm_get_components (
            &tpixels[PIXEL_INDEX_SMALL (xs - 1, ys - 1)], &rv, &gv, &bv,
            NULL); // upper left
        ra += rv;
        ba += bv;
        ga += gv;
...
// fill in the position of the eye for this frame
void neb2_get_eye_pos (vec3d* eye_vector) { *eye_vector = Eye_position; }

// fill in the eye orient for this frame
void neb2_get_eye_orient (matrix* eye_matrix) { *eye_matrix = Eye_matrix; }
...
int neb2_get_bitmap () {
    ...
    while (count > 0) {
        ...
        huh = 0;
        do {
            if (neb2_choose == MAX_NEB2_POOFS - 1) { neb2_choose = 0; }
            else {
                neb2_choose++;
            }
    ...
    // bitmap 0
    if (Neb2_poofs[neb2_choose] < 0) { return Neb2_poofs[0]; }
...
DCF (neb2, "list nebula console commands") {
    ...
    dc_printf (
        "neb2_break_alpha : alpha value (0.0 to 1.0) at which faded polygons "
        "are not drawn. higher values generally equals higher framerate, with "
        "more visual cloud popping\n");
    dc_printf (
        "neb2_break_off   : how many pixels offscreen (left, right, top, "
        "bottom) when a cloud poof becomes fully transparent. Lower values "
        "cause quicker fading\n");
    dc_printf ("neb2_smooth      : magic fog smoothing modes (0 - 3)\n");
    dc_printf (
        "neb2_select      : <int> <int>  where the first # is the bitmap to "
        "be adjusting (0 through 5), and the second int is a 0 or 1, to turn "
        "off and on\n");
    ...
    dc_printf (
        "neb2_mode        : switch between no nebula, polygon background, pof "
...
DCF (neb2_cdim, "poof cube dimension") { dc_stuff_float (&Nd->cube_dim); }
...
DCF (
    neb2_break_off,
    "how many pixels offscreen (left, right, top, bottom) when a cloud poof "
...
DCF (neb2_smooth, "magic fog smoothing modes (0 - 3)") {
    int index;
    dc_stuff_int (&index);
    if ((index >= 0) && (index <= 3)) { wacky_scheme = index; }
    else {
...
DCF (neb2_ff, "flash fade/sec") { dc_stuff_float (&neb2_flash_fade); }
...
DCF (neb2_mode, "Switches nebula render modes") {
    ...
    switch (mode) {
    case NEB2_RENDER_NONE: Neb2_render_mode = NEB2_RENDER_NONE; break;
    ...
    case NEB2_RENDER_HTL: Neb2_render_mode = NEB2_RENDER_HTL; break;
    }
}
