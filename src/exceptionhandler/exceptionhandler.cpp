...
static void PrintTime (char* output, FILETIME TimeToPrint) {
    ...
    if (FileTimeToLocalFileTime (&TimeToPrint, &TimeToPrint) &&
        ...
        wsprintf (
            output, "%d/%d/%d %02d:%02d:%02d", (Date / 32) & 15, Date & 31,
            (Date / 512) + 1980, (Time / 2048), (Time / 32) & 63,
            (Time & 31) * 2);
    }
    else {
        output[0] = 0;
    ...
    __try {
        ...
        if (GetModuleFileName (ModuleHandle, ModName, sizeof (ModName)) > 0) {
            ...
            HANDLE ModuleFile = CreateFile (
                ModName, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING,
            ...
            if (ModuleFile != INVALID_HANDLE_VALUE) {
                ...
                if (GetFileTime (ModuleFile, 0, 0, &LastWriteTime)) {
                    ...
                    PrintTime (
                        TimeBuffer + lstrlen (TimeBuffer), LastWriteTime);
                }
                CloseHandle (ModuleFile);
            }
            hprintf (
                LogFile, "%s, loaded at 0x%08x - %d bytes - %08x%s\r\n",
...
static void RecordModuleList (HANDLE LogFile) {
    ...
    while (pageNum < NumPages) {
        ...
        if (VirtualQuery (
                (void*)(pageNum * PageSize), &MemInfo, sizeof (MemInfo))) {
...
static void RecordSystemInformation (HANDLE LogFile) {
    ...
    hprintf (
        LogFile, "%d MBytes physical memory.\r\n",
...
static const char* GetExceptionDescription (DWORD ExceptionCode) {
    ...
    for (int i = 0; i < sizeof (ExceptionMap) / sizeof (ExceptionMap[0]);
         i++) {
...
// into an except() statment at the beginning of a thread and is called anytime
// that there is a program exception The data is stored in a file called
// ErrorLog.txt in the data directory.
//
...
int __cdecl RecordExceptionInfo (
    ...
    if (BeenHere) { return EXCEPTION_CONTINUE_SEARCH; }
    ...
    HANDLE LogFile = CreateFile (
        ModuleName, GENERIC_WRITE, 0, 0, OPEN_ALWAYS,
    ...
    // Print out some blank lines to separate this error log from any previous
    // ones.
    hprintf (LogFile, "\r\n\r\n\r\n\r\n");
    ...
    // compatible format.
    hprintf (
        LogFile, "%s caused %s in module %s at %04x:%08x.\r\n", FileName,
    ...
    if (Exception->ExceptionCode == STATUS_ACCESS_VIOLATION &&
        ...
        if (Exception->ExceptionInformation[0]) { readwrite = "Write to"; }

        wsprintf (
            DebugMessage, "%s location %08x caused an access violation.\r\n",
    ...
    hprintf (
        LogFile, "EAX=%08x CS=%04x EIP=%08x EFLGS=%08x\r\n", Context->Eax,
        Context->SegCs, Context->Eip, Context->EFlags);
    hprintf (
        LogFile, "EBX=%08x SS=%04x ESP=%08x EBP=%08x\r\n", Context->Ebx,
        Context->SegSs, Context->Esp, Context->Ebp);
    hprintf (
        LogFile, "ECX=%08x DS=%04x ESI=%08x FS=%04x\r\n", Context->Ecx,
        Context->SegDs, Context->Esi, Context->SegFs);
    hprintf (
        LogFile, "EDX=%08x ES=%04x EDI=%08x GS=%04x\r\n", Context->Edx,
        Context->SegEs, Context->Edi, Context->SegGs);
