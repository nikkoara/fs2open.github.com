...
// ***** NOTE:  bitmap.flags is an 8-bit value, no more BMP_TEX_* flags can be
// added unless the type is changed!! ******
...
#define BMP_MASK_BITMAP \
    (1 << 7) //!< a bitmap that will be used for masking mouse interaction.
             //!< Typically not used in render operations

// Combined flags
#define BMP_TEX_COMP \
    (BMP_TEX_DXT1 | BMP_TEX_DXT3 | BMP_TEX_DXT5) //!< Compressed textures
#define BMP_TEX_NONCOMP \
    (BMP_TEX_XPARENT | BMP_TEX_OTHER) //!< Non-compressed textures
...
#define BMP_FLAG_RENDER_TARGET_DYNAMIC \
    (1 << 1)                      //!< Texture is a dynamic type (animation)
...
enum BM_TYPE {
    ...
    BM_TYPE_EFF, //!< specifies any type of animated image, the EFF itself is
                 //!< just text

    // special types
    BM_TYPE_RENDER_TARGET_STATIC,  //!< 24/32 bit setup internally as a static
                                   //!< render target
    BM_TYPE_RENDER_TARGET_DYNAMIC, //!< 24/32 bit setup internally as a dynamic
                                   //!< render target
    ...
    BM_TYPE_CUBEMAP_DDS,  //!< generic DDS cubemap (uncompressed cubemap
                          //!< surface)
    BM_TYPE_CUBEMAP_DXT1, //!< 24-bit cubemap        (compressed cubemap
                          //!< surface)
    BM_TYPE_CUBEMAP_DXT3, //!< 32-bit cubemap        (compressed cubemap
                          //!< surface)
    BM_TYPE_CUBEMAP_DXT5  //!< 32-bit cubemap        (compressed cubemap
                          //!< surface)
};
...
struct bitmap {
    ...
    ubyte* palette; /**< @brief   Pointer to this bitmap's palette (if it has
                     * one).
                     *   @details If BMP_NO_PALETTE_MAP flag is cleared, this
                     * palette just points to the screen palette. (gr_palette)
                     */
};
...
extern int
    Bm_paging; //!< Bool type that indicates if BMPMAN is currently paging.
...
extern const BM_TYPE
    bm_ani_type_list[];               //!< List of valid bitmap animation types
extern const char* bm_ani_ext_list[]; //!< List of extensions for those types
extern const int
    BM_ANI_NUM_TYPES; //!< Calculated number of bitmap animation types

extern int GLOWMAP; //!< References a map that is a fully lit version of its
                    //!< index -Bobboau
extern int SPECMAP; //!< References a map that is for specular mapping -Bobboau
extern int
    ENVMAP; //!< References a map that is for environment mapping -Bobboau
extern int NORMMAP;   //!< Normal mapping
extern int HEIGHTMAP; //!< Height map for normal mapping
extern int MISCMAP; //!< Utility map, to be utilized for various things shader
...
/**
 * @brief Closes the bitmap manager, freeing any allocated memory used by
 * bitmaps. Is called at program close.
 */
void bm_close ();
 ...
 *
 * @note z64 - This function looks fishy. Not only is handle not used in
 * release builds, but bm_bitmaps[handle].size and bm_texture_size aren't
 * modified unless this is a debug build
 */
void* bm_malloc (int handle, size_t size);

/**
 * @brief (DEBUG) Similar to bm_malloc, but only updates how much memory is
 * used
 *
 ...
 *
 * @details This function basically allows you to load a bitmap which already
 * exists (by filename). This is useful because in some cases we need to have a
 * bitmap which is locked in screen format _and_ texture format, such as the
 * pilot pics and squad logos
 *
 * @param filename
 *
 * @returns The bitmap number if successful, else
 ...
 * @param flags
 *
 * @note The used RAM cannot be freed until bm_release is called on the created
 * bitmap
 */
 ...
 *
 * @details The bm number (n) may be reused once this function has been called
 on it. However, it will have to be paged
 ...
 *
 * @details Similar to bm_unload(), except that it can be safely used to free
 * data without worrying about load_count. It's safe to use in relation to
 * bm_release() and in gr_*_texture functions
 *
 ...
 *
 * @details Once called, the bm number 'n' cannot be used until bm_load or
 * bm_create is re-inits the slot.
 *
 ...
 * @returns 0 otherwise
 *
 * @note If the passed handle is that of an ANI, it frees EVERY frame. Be sure
 * to only pass the handle of the first frame!
 *
 ...
 *
 * @param[in] filename
 * @param[out] nframes          If non-null, set to the number of frames the
 * animation has
 * @param[out] fps              If non-null, set to the fps of this animation
 * @param[out] keyframe         if non null, set to the keyframe index of this
 * animation
...
int bm_load_animation (
    const char* filename, int* nframes = nullptr, int* fps = nullptr,
    int* keyframe = nullptr, float* total_time = nullptr,
    bool can_drop_frames = 0, int dir_type = CF_TYPE_ANY);

/**
 * @brief Loads either animation (bm_load_animation) or still image (bm_load)
 *
 * @param[in] filename
 * @param[out] nframes  If non-null and loading was successful, set to the
 * number of frames the animation has
 * @param[out] fps      If non-null and loading was successful, set to the fps
 * of this animation
 * @param[out] keyframe if non null and loading was successful, set to the
 * keyframe index of this animation
 ...
 */
int bm_load_either (
    const char* filename, int* nframes = NULL, int* fps = NULL,
    int* keyframe = NULL, bool can_drop_frames = false,
    int dir_type = CF_TYPE_ANY);

/**
 * @brief Locks down the bitmap indexed by bitmapnum.
 *
 * @details Also converts the bitmap to the appropriate format specified by bpp
 and flags. Only lock a bitmap when you
 *   need it!
 *
 ...
 * @returns A pointer to the bitmap that's valid until bm_unlock is called if
 successful, or
 * @returns NULL if unsuccessful
 */
 ...
 *
 * @details Decrements the ref_count member of the bitmap_entry struct, A
 * bitmap can only be unloaded when the ref_count is 0
 */
 ...
 *
 * @details Some early known false or out of range handles (such as negative)
 * are checked within an initial routine pass, followed by a more thorough
 * check routine to ensure the series of bitmaps within the bm_bitmaps[]
 * structure have not become invalid.
 *
 * @returns Nonzero if valid, else
 * @returns Zero otherwise
 *
 * @todo z64 - Returned value is essentially a bool type, need to check all
 * caller functions to see if it can safely be updated to reflect this
 */
 ...
 */
int bm_get_info (
    int handle, int* w = NULL, int* h = NULL, ubyte* flags = NULL,
    int* nframes = NULL, int* fps = NULL);

/**
 * @brief Gets the filename of the bitmap indexed by handle
 *
 * @param[out] filename The filename of the bitmap, if successful. Is set to an
 * empty string if unseccessful
 *
 ...
 *
 * @details If the bitmap does not exist (i.e. the handle is invalid), then an
 * Assertion halts program execution.
 *
 * @returns A const char* to the filename of the bitmap
 *
 * @todo z64 - maybe deprecate this in favor of an std::string version.
 * @todo z64 - maybe combine this with the other bm_get_filename function like
 * so: void bm_get_filename(int handle, std::string *filename, bool must_exist
 * = false);
 */
const char* bm_get_filename (int handle);

/**
 * @brief Unloads all used bitmaps, should only ever be called by
 * game_shutdown()
 *
 * @todo Maybe move this declaration into bmpman.cpp and then extern this
 * function within game_shutdown() to ensure it only ever gets called there.
 */
void bm_unload_all ();

/**
 * @brief Gets the palette for a given bitmap indexed by handle, and optionally
 * the filename
 *
 * @param[out] pal Is set to reference the bitmap's palette
 * @param[out] name (optional) Is set to the bitmap's filename
 *
 * @todo Maybe get rid of the optional filename and have the callers call
 * bm_get_filename. Less efficient, however.
 */
 ...
 *
 * @details Both params are non-optional, and must be non-null. There's
 * currently no safety checks on either.
 *
 * @param ntotal[out] Memory size, in bytes, of the locked bitmaps
 * @param nnew[out]   Memory size, in bytes, of the bitmaps that were locked
 ...
 *
 * @note This should only be used if you are certain the new picture is the
 * same type, has same dimensions, etc.
 */
 ...
 *
 * @todo This should use an enum, or instead allow an arbitrary number to drop
 * frames (like 1/2, 1/3, etc.)
 */
void bm_set_low_mem (int mode);

/**
 * @brief Sets bm_set_components and bm_get_components to reference screen
 * format functions
 */
void BM_SELECT_SCREEN_FORMAT ();

/**
 * @brief Sets bm_set_components and bm_get_components to reference texture
 * format functions
 */
...
/**
 * @brief Sets bm_set_components and bm_get_components to reference texture
 * format functions (with alpha)
 */
void BM_SELECT_ALPHA_TEX_FORMAT ();

/**
 * @brief Functional pointer that references any of the bm_set_components
 * functions.
 *
 * @details The bm_set_components functions packs the RGBA values into the
 * ubyte array referenced by pixel, whose format differs according to its bpp
 * value and presence of an alpha channel. The RGBA values are scaled
 * accordingly.
 *
 ...
 *
 * @note z64 - These functions were made predating the introduction of C++
 * classes, and are basically the equivalent of Pixel::set(ubyte *r, ubyte *b,
 * ubyte *g). The original comment mentions that any of the rgba params may be
 * NULL, but this is by far _NOT_ the case, as a NULL value will cause
 * undefined behavior (really really bad chroma corruption)
 *
 * @note These functions assume the incoming bitdepth will always be >= to the
 * outgoing bitdepth of the pixel. Should the incoming bitdepth be lower, the
 * outgoing values will appear darker than they should be
 */
extern void (*bm_set_components) (
    ubyte* pixel, ubyte* r, ubyte* g, ubyte* b, ubyte* a);

/**
 * @brief Functional pointer that references any of the bm_set_components_32
 * functions.
 *
 * @details The bm_set_components functions packs the RGBA values into the
 * ubyte array referenced by pixel, whose format differs according to its bpp
 ...
 */
extern void (*bm_set_components_32) (
    ubyte* pixel, ubyte* r, ubyte* g, ubyte* b, ubyte* a);
 ...
 *
 * @see BM_SELECT_SCREEN_FORMAT() BM_SELECT_TEX_FORMAT()
...
/**
 * @brief Gets the correct TCACHE_TYPE for compressed graphics (uncompressed
 * are assumed TCACHE_TYPE_NORMAL)
 */
...
/**
 * @brief Creates a render target as close to the desired resolution as
 * possible.
 *
 * @returns the handle of an avilable render target if successful, or
 * @returns -1 if not successful
 *
 * @note BM_FLAG_RENDER_TARGET_STATIC are drawn once/infrequently, while
 * BM_FLAG_RENDER_TARGET_DYNAMIC are drawn roughly once every frame
 */
int bm_make_render_target (int width, int height, int flags);

/**
 * @brief Checks to see if the given bitmap indexed by handle is a render
 * target
 *
...
/**
 * @brief (GR function) Calls gr_bm_set_render target for the given bitmap
 * indexed by handle
 *
 ...
 * @param[in]  dir_type
 * @param[out] nframes (optional) If given, is set to the number of frames this
 * .EFF has
 * @param[out] nfps    (optional) If given, is set to the fps of this .EFF
 * @param[out] key     (optional) If given, is set to the keyframe index of
 * this .EFF
 ...
 * @param[in] elapsed_time   Time in seconds since the animation started
 * playing
 * @param[in] loop           (optional) specifies if the animation should loop,
 * default is false which causes animation to hold on the last frame
 * @param[in] divisor        (optional) if greater than zero, elapsed time will
 * be divided by this value i.e. allows "percentage complete" to be used to
 * determine the frame instead of the animations total time derived from fps *
 * frames
 *
 * @returns current frame of the animation (range is zero to the number of
 * frames minus one)
 */
int bm_get_anim_frame (
    const int frame1_handle, float elapsed_time, const float divisor = 0.0f,
 ...
 * @return @c true if all frames have the same size, @c false otherwise. Always
 * returns @c true for single images
 */
 ...
 *
 * @details The base frame is the frame of the animation that should be used to
 * determine if two animation frames can be used in the same draw call. Use
 * this if you want to optimize your draw calls to make use of texture arrays.
 *
 * @param handle The handle of the animation to check. This may also be a
 * single frame bitmap
 ...
 *
 * This should be used when passing the array index to the GPU (e.g. when
 * building uniform structs or streaming particle data)
 *
 ...
 *
 * This needs to iterate through all the slots in order to determine whether or
 * not they're used, so shouldn't be used frivolously.
 *
 ...
 *
 * Since the number of slots is dynamic now, this should be used for
 * determining the total amount of available slots at the moment.
 *
 * @warning This is entirely for debugging and logging purposes. It should not
 * be used in actual engine code.
 *
 ...
 *
 * @note At least one of @c single_frame or @c animation must be true since the
 * result would make no sense otherwise. It is also valid to set both parmeters
 * to @c true.
 *
 * @todo This currently just tries to load the file but could be improved by
 * not actually loading the file in the future.
 *
...
#endif // FREESPACE2_BMPMAN_BMPMAN_HH
