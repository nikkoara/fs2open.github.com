...
void (*bm_set_components)(
        ubyte *pixel, ubyte *r, ubyte *g, ubyte *b, ubyte *a)
        = NULL;
...
{
        return (
                (entry->type == BM_TYPE_ANI) || (entry->type == BM_TYPE_EFF) || (entry->type == BM_TYPE_PNG && entry->info.ani.apng.is_apng));
}
...
bitmap_lookup::bitmap_lookup(int bitmap_num) : Bitmap_data(NULL)
{
        ...
        Bitmap_data = (ubyte *)malloc(Width * Height * Num_channels * sizeof(ubyte));
...
void bm_get_components(ubyte *pixel, ubyte *r, ubyte *g, ubyte *b, ubyte *a)
{
        ...
        if (r != NULL) {
                if (bit_32) {
                        *r = ubyte(
                                ((*((unsigned int *)pixel) & Gr_current_red->mask) >> Gr_current_red->shift) * Gr_current_red->scale);
                } else {
                        *r = ubyte(
                                ((((unsigned short *)pixel)[0] & Gr_current_red->mask) >> Gr_current_red->shift) * Gr_current_red->scale);
                }
        }
        ...
        if (g != NULL) {
                if (bit_32) {
                        *g = ubyte(
                                ((*((unsigned int *)pixel) & Gr_current_green->mask) >> Gr_current_green->shift) * Gr_current_green->scale);
                } else {
                        *g = ubyte(
                                ((((unsigned short *)pixel)[0] & Gr_current_green->mask) >> Gr_current_green->shift) * Gr_current_green->scale);
                }
        }
        ...
        if (b != NULL) {
                if (bit_32) {
                        *b = ubyte(
                                ((*((unsigned int *)pixel) & Gr_current_blue->mask) >> Gr_current_blue->shift) * Gr_current_blue->scale);
                } else {
                        *b = ubyte(
                                ((((unsigned short *)pixel)[0] & Gr_current_blue->mask) >> Gr_current_blue->shift) * Gr_current_blue->scale);
                }
        }
        ...
        float *total_time, bool can_drop_frames, int dir_type)
{
        ...
        // regular ani file
                ...
                if (the_anim.num_keys == 2) {
                        the_anim.keys = (key_frame *)malloc(sizeof(key_frame) * the_anim.num_keys);
                ...
                try {
                        ...
                        anim_fps = int(
                                float(anim_frames) / the_apng.anim_time); // note; apng bails on loading if
                                                                          // anim_time is <= 0.0f
...
bitmap *bm_lock(int handle, int bpp, ubyte flags, bool nodebug)
{
        ...
        if (flags & BMP_AABITMAP) {
                ...
                (flags & BMP_TEX_DXT1) || (flags & BMP_TEX_DXT3) || (flags & BMP_TEX_DXT5)) {
                ASSERT(bpp >= 16); // cheating but bpp passed isn't what we
                                   // normally end up with
        } else if (flags & BMP_TEX_CUBEMAP) {
                ASSERT(
                        (be->type == BM_TYPE_CUBEMAP_DDS) || (be->type == BM_TYPE_CUBEMAP_DXT1) || (be->type == BM_TYPE_CUBEMAP_DXT3) || (be->type == BM_TYPE_CUBEMAP_DXT5));
                ASSERT(bpp >= 16);
        } else {
                ASSERT(0); //?
        }
        ...
        int /*handle*/, bitmap_slot *bs, bitmap * /*bmp*/, int bpp, ubyte flags)
{
        ...
        if ((the_anim = anim_load(
                     first_entry->filename, first_entry->dir_type))
            == nullptr) {
        ...
        int handle, bitmap_slot *bs, bitmap *bmp, int bpp, ubyte flags)
{
        ...
        ASSERT(
                !((flags & BMP_AABITMAP) && (flags & BMP_TEX_ANY))); // no aabitmap textures
...
void bm_page_in_stop()
{
        ...
        for (auto &block : bm_blocks) {
                for (auto &slot : block) {
                        ...
                        if ((entry.type != BM_TYPE_NONE) && (entry.type != BM_TYPE_RENDER_TARGET_DYNAMIC) && (entry.type != BM_TYPE_RENDER_TARGET_STATIC)) {
                                if (entry.preloaded) {
...
int bm_release(int handle, int clear_render_targets)
{
        ...
        if (bm_is_anim(be)) {
                ...
                for (i = 0; i < total; i++) {
                        ...
                        entry->palette_checksum = 0xDEADBEEF; // checksum used to be sure bitmap is in current
                                                              // palette
...
void bm_unlock(int handle)
{
        ...
        be->ref_count--;
        ASSERT(
                be->ref_count >= 0); // Trying to unlock data more times than lock was called!!!
}
