...
/**
 * @todo upgrade this to an inline funciton, taking bitmap_entry and const
 * char* as arguments
 */
...
// However there's lots of assumptions throughout the codebase that file
// extensions are only three chars long. If this is ever changed the the .apng
// extension could be used
const char* bm_ani_ext_list[] = { ".eff", ".ani", ".png" };
const int BM_ANI_NUM_TYPES =
    sizeof (bm_ani_type_list) / sizeof (bm_ani_type_list[0]);

void (*bm_set_components) (
    ubyte* pixel, ubyte* r, ubyte* g, ubyte* b, ubyte* a) = NULL;
void (*bm_set_components_32) (
    ubyte* pixel, ubyte* r, ubyte* g, ubyte* b, ubyte* a) = NULL;
 ...
 *
 * @note was initialized to 16*1024*1024 at some point to "use only 16MB for
 * textures"
 */
...
static bool bm_is_anim (bitmap_entry* entry) {
    return (
        (entry->type == BM_TYPE_ANI) || (entry->type == BM_TYPE_EFF) ||
...
bitmap_slot* bm_get_slot (int handle, bool separate_ani_frames) {
    ...
    // Get the entry of the first frame by calling ourself again but this time
    // we enforce separate ani frames (since we already know that this handle
    // is the correct one)
    return bm_get_slot (entry->info.ani.first_frame, true);
}

// --------------------------------------------------------------------------------------------------------------------
// Declaration of private functions and templates(declared as static type
// func(type param);)
...
bitmap_lookup::bitmap_lookup (int bitmap_num) : Bitmap_data (NULL) {
    ...
    if (bm_has_alpha_channel (bitmap_num)) { Num_channels = 4; }
 ...
 * @returns index into ext_list[] if it was found as a file, fills img_cfg if
 * available
 */
static int bm_load_sub_slow (
    const char* real_filename, const int num_ext, const char** ext_list,
    CFILE** img_cfp = NULL, int dir_type = CF_TYPE_ANY);
 ...
 */
static int bm_load_sub_fast (
    const char* real_filename, int* handle, int dir_type = CF_TYPE_ANY,
 ...
 * @param start_block At which block the search should start. Do not use. This
...
DCF (bm_frag, "Shows BmpMan fragmentation") {
    if (dc_optional_string_either ("help", "--help")) {
        dc_printf (
            "Displays a graphic showing the BmpMan fragmentation. Color "
            "key:\n");
        dc_printf ("\tGray  : NONE\n");
        dc_printf ("\tRed   : PCXn");
        ...
        dc_printf (
            "Once done reviewing the graphic, press any key to return to the "
            "console\n");
        return;
    }
    ...
    for (auto& block : bm_blocks) {
        for (size_t i = 0; i < BM_BLOCK_SIZE; ++i) {
            ...
            case BM_TYPE_NONE: gr_set_color (128, 128, 128); break;
            case BM_TYPE_PCX: gr_set_color (255, 0, 0); break;
            case BM_TYPE_USER:
            case BM_TYPE_TGA:
            case BM_TYPE_PNG:
            case BM_TYPE_DDS: gr_set_color (0, 255, 0); break;
            case BM_TYPE_ANI:
            case BM_TYPE_EFF: gr_set_color (0, 0, 255); break;
            default: gr_set_color (0, 255, 0); break;
            }
...
DCF (bm_used, "Shows BmpMan Slot Usage") {
    if (dc_optional_string_either ("help", "--help")) {
        dc_printf (
            "Displays used bmpman slots usage with a breakdown per "
            "filetype\n\n");
        return;
    }
    ...
    int eff = 0, eff_dds = 0, eff_tga = 0, eff_png = 0, eff_jpg = 0,
        eff_pcx = 0;
    ...
    for (auto& block : bm_blocks) {
        for (size_t i = 0; i < BM_BLOCK_SIZE; ++i) {
            switch (block[i].entry.type) {
            case BM_TYPE_NONE: none++; break;
            case BM_TYPE_PCX: pcx++; break;
            case BM_TYPE_USER: user++; break;
            case BM_TYPE_TGA: tga++; break;
            case BM_TYPE_PNG:
                // TODO distinguish png(static) from apng
                png++;
                break;
            case BM_TYPE_JPG: jpg++; break;
            case BM_TYPE_DDS: dds++; break;
            case BM_TYPE_ANI: ani++; break;
            case BM_TYPE_EFF:
                eff++;
                switch (block[i].entry.info.ani.eff.type) {
                case BM_TYPE_DDS: eff_dds++; break;
                case BM_TYPE_TGA: eff_tga++; break;
                case BM_TYPE_PNG: eff_png++; break;
    ...
    text << "  " << std::dec << std::setw (4) << std::setfill ('0') << pcx
         << ", PCX\n";
    text << "  " << std::dec << std::setw (4) << std::setfill ('0') << user
         << ", User\n";
    text << "  " << std::dec << std::setw (4) << std::setfill ('0') << tga
         << ", TGA\n";
    text << "  " << std::dec << std::setw (4) << std::setfill ('0') << png
         << ", PNG\n";
    text << "  " << std::dec << std::setw (4) << std::setfill ('0') << jpg
         << ", JPG\n";
    text << "  " << std::dec << std::setw (4) << std::setfill ('0') << dds
         << ", DDS\n";
    text << "  " << std::dec << std::setw (4) << std::setfill ('0') << ani
         << ", ANI\n";
    text << "  " << std::dec << std::setw (4) << std::setfill ('0') << eff
         << ", EFF\n";
    text << "  " << std::dec << std::setw (4) << std::setfill ('0') << eff_dds
         << ", EFF/DDS\n";
    text << "  " << std::dec << std::setw (4) << std::setfill ('0') << eff_tga
         << ", EFF/TGA\n";
    text << "  " << std::dec << std::setw (4) << std::setfill ('0') << eff_png
         << ", EFF/PNG\n";
    text << "  " << std::dec << std::setw (4) << std::setfill ('0') << eff_jpg
         << ", EFF/JPG\n";
    text << "  " << std::dec << std::setw (4) << std::setfill ('0') << eff_pcx
         << ", EFF/PCX\n";
    text << "  " << std::dec << std::setw (4) << std::setfill ('0')
         << render_target_static << ", Render/Static\n";
    text << "  " << std::dec << std::setw (4) << std::setfill ('0')
         << render_target_dynamic << ", Render/Dynamic\n";
    text << "  " << std::dec << std::setw (4) << std::setfill ('0')
...
DCF (bmpman, "Shows/changes bitmap caching parameters and usage") {
    if (dc_optional_string_either ("help", "--help")) {
        ...
        dc_printf (
            "\tram [x]  Sets max mem usage to x MB. (Set to 0 to have no "
            "limit.)\n");
...
void bm_free_data (bitmap_slot* bs, bool release) {
    ...
    if (be->type == BM_TYPE_NONE) goto SkipFree;
...
#ifdef BMPMAN_NDEBUG
        if (be->data_size != 0) bm_texture_ram -= be->data_size;
#endif
        goto SkipFree;
    }
...
void bm_free_data_fast (int handle) {
    ...
    // If there isn't a bitmap in this structure, don't
    // do anything but clear out the bitmap info
    if (be->type == BM_TYPE_NONE) return;
...
int bm_get_anim_frame (
    const int frame1_handle, float elapsed_time, const float divisor,
    const bool loop) {
    ...
    if (be->info.ani.apng.is_apng == true) {
        if (divisor > 0.0f) {
            ...
            elapsed_time = elapsed_time / divisor *
                           last_framep->info.ani.apng.frame_delay;
        }

        if (loop == true) {
            elapsed_time =
                fmod (elapsed_time, last_framep->info.ani.apng.frame_delay);
        }

        int i = frame1_handle;
        for (; i < (frame1_handle + be->info.ani.num_frames); ++i) {
            // see bm_lock_apng for precalculated incremental delay for each
            // frame
...
void bm_get_components (ubyte* pixel, ubyte* r, ubyte* g, ubyte* b, ubyte* a) {
    ...
    if (r != NULL) {
        if (bit_32) {
            *r = ubyte (
                ((*((unsigned int*)pixel) & Gr_current_red->mask) >>
                 Gr_current_red->shift) *
                Gr_current_red->scale);
        }
        else {
            *r = ubyte (
                ((((unsigned short*)pixel)[0] & Gr_current_red->mask) >>
                 Gr_current_red->shift) *
                Gr_current_red->scale);
        }
    }
    ...
    if (g != NULL) {
        if (bit_32) {
            *g = ubyte (
                ((*((unsigned int*)pixel) & Gr_current_green->mask) >>
                 Gr_current_green->shift) *
                Gr_current_green->scale);
        }
        else {
            *g = ubyte (
                ((((unsigned short*)pixel)[0] & Gr_current_green->mask) >>
                 Gr_current_green->shift) *
                Gr_current_green->scale);
        }
    }
    ...
    if (b != NULL) {
        if (bit_32) {
            *b = ubyte (
                ((*((unsigned int*)pixel) & Gr_current_blue->mask) >>
                 Gr_current_blue->shift) *
                Gr_current_blue->scale);
        }
        else {
            *b = ubyte (
                ((((unsigned short*)pixel)[0] & Gr_current_blue->mask) >>
                 Gr_current_blue->shift) *
                Gr_current_blue->scale);
        }
    }
    ...
    for (auto& block : bm_blocks) {
        for (auto& slot : block) {
            ...
            if ((entry.type != BM_TYPE_NONE) && (entry.used_this_frame)) {
                if (!entry.used_last_frame) { *nnew += (int)entry.mem_taken; }
...
int bm_get_info (
    int handle, int* w, int* h, ubyte* flags, int* nframes, int* fps) {
    bitmap* bmp;
    ...
    if (bm_is_anim (entry)) {
        if (nframes) { *nframes = entry->info.ani.num_frames; }
        if (fps) { *fps = entry->info.ani.fps; }
...
int bm_get_num_mipmaps (int num) {
    ...
    if (entry->num_mipmaps == 0) return 1;
...
int bm_get_tcache_type (int num) {
    if (bm_is_compressed (num)) return TCACHE_TYPE_COMPRESSED;
...
bool bm_has_alpha_channel (int handle) {
    ...
    // assume that PCX never has a real alpha channel (it may be 32-bit, but
    // without any alpha)
    if (entry->type == BM_TYPE_PCX) return 0;
...
int bm_is_compressed (int num) {
    ...
    // duh
    if (!Use_compressed_textures) return 0;

    type = entry->comp_type;

    switch (type) {
    case BM_TYPE_NONE:
    case BM_TYPE_DDS: return 0;

    case BM_TYPE_DXT1: return DDS_DXT1;

    case BM_TYPE_DXT3: return DDS_DXT3;

    case BM_TYPE_DXT5: return DDS_DXT5;

    case BM_TYPE_CUBEMAP_DXT1: return DDS_CUBEMAP_DXT1;

    case BM_TYPE_CUBEMAP_DXT3: return DDS_CUBEMAP_DXT3;

    case BM_TYPE_CUBEMAP_DXT5: return DDS_CUBEMAP_DXT5;
...
int bm_is_render_target (int bitmap_id) {
    ...
    if (!((entry->type == BM_TYPE_RENDER_TARGET_STATIC) ||
...
int bm_is_valid (int handle) {
    // Ensure that certain known false or out of range handles are quickly
    // returned as invalid, prior to utilising the handle in a way which leads
    // to memory access outside bm_bitmaps[]
...
static int bm_load_info (
    ...
    if (type == BM_TYPE_DDS) {
        ...
        switch (dds_ct) {
        case DDS_DXT1: *c_type = BM_TYPE_DXT1; break;

        case DDS_DXT3: *c_type = BM_TYPE_DXT3; break;

        case DDS_DXT5: *c_type = BM_TYPE_DXT5; break;

        case DDS_UNCOMPRESSED: *c_type = BM_TYPE_DDS; break;

        case DDS_CUBEMAP_DXT1: *c_type = BM_TYPE_CUBEMAP_DXT1; break;

        case DDS_CUBEMAP_DXT3: *c_type = BM_TYPE_CUBEMAP_DXT3; break;

        case DDS_CUBEMAP_DXT5: *c_type = BM_TYPE_CUBEMAP_DXT5; break;
...
int bm_load (const char* real_filename) {
    ...
    if ((real_filename == NULL) || (strlen (real_filename) <= 0)) return -1;
    ...
    // Lets find out what type it is
    {
        ...
        if (bm_load_sub_fast (filename, &handle)) return handle;

        // if we are still here then we need to fall back to a file-based
        // search
        int rval =
            bm_load_sub_slow (filename, BM_NUM_TYPES, bm_ext_list, &img_cfp);
    ...
    rc = bm_load_info (
        type, filename, img_cfp, &w, &h, &bpp, &c_type, &mm_lvl, &bm_size);
    ...
    if ((bm_size <= 0) && (w) && (h) && (bpp)) bm_size = (w * h * (bpp >> 3));
...
bool bm_load_and_parse_eff (
    const char* filename, int dir_type, int* nframes, int* nfps, int* key,
    BM_TYPE* type) {
    ...
    try {
        ...
        if (optional_string ("$FPS:")) stuff_int (&fps);
 ...
 */
static int
bm_load_image_data (int handle, int bpp, ubyte flags, bool nodebug) {
    BM_TYPE c_type = BM_TYPE_NONE;
    int true_bpp;
    ...
    if (bmp->data == 0) {
        ...
        if (be->type == BM_TYPE_EFF) { c_type = be->info.ani.eff.type; }
        else {
            c_type = be->type;
        }
        ...
        default:
            WARNINGF (LOCATION, "Unsupported type in bm_lock -- %d", c_type);
            return -1;
        }
...
int bm_load_animation (
    const char* real_filename, int* nframes, int* fps, int* keyframe,
    float* total_time, bool can_drop_frames, int dir_type) {
    int i, n;
    anim the_anim;
    ...
    BM_TYPE type = BM_TYPE_NONE, eff_type = BM_TYPE_NONE,
            c_type = BM_TYPE_NONE;
    int bpp = 0, mm_lvl = 0;
    size_t img_size = 0;
    ...
    if (nframes != nullptr) *nframes = 0;

    if (fps != nullptr) *fps = 0;

    if (keyframe != nullptr) *keyframe = 0;

    if (total_time != nullptr) *total_time = 0.0f;

    memset (filename, 0, MAX_FILENAME_LEN);
    strncpy (filename, real_filename, MAX_FILENAME_LEN - 1);
    char* p = strchr (filename, '.');
    if (p) {
        WARNINGF (LOCATION,"Someone passed an extension to bm_load_animation for file '%s'",real_filename);
        *p = 0;
    }
    ...
    // MAX_FILENAME_LEN-5 == '.' plus 3 letter ext plus NULL terminator
    if (strlen (filename) > MAX_FILENAME_LEN - 5) {
        WARNINGF (LOCATION,"Passed filename, '%s', is too long to support an extension!!\n\nMaximum length, minus the extension, is %i characters.",filename, MAX_FILENAME_LEN - 5);
        return -1;
    }
    ...
    // Lets find out what type it is
    {
        ...
        if (bm_load_sub_fast (filename, &handle, dir_type, true)) {
            ...
            if (keyframe) *keyframe = entry->info.ani.keyframe;
        ...
        // if we are still here then we need to fall back to a file-based
        // search
        int rval = bm_load_sub_slow (
            filename, BM_ANI_NUM_TYPES, bm_ani_ext_list, &img_cfp, dir_type);
    ...
    // If we found an animation then there is an extra 5 char size limit to
    // adhere to. We don't do this check earlier since it's only needed if we
    // found an anim an ANI needs about 5 extra characters to have the "[###]"
    // frame designator EFF/APNG need 5 extra characters for each frame
    // filename too, which just happens to be the same length as the frame
    // designator needed otherwise MAX_FILENAME_LEN-10 == 5 character frame
    // designator plus '.' plus 3 letter ext plus NULL terminator we only check
    // for -5 here since the filename should already have the extension on it,
    // and it must have passed the previous check
    if (strlen (filename) > MAX_FILENAME_LEN - 5) {
        WARNINGF (LOCATION,"Passed filename, '%s', is too long to support an extension and frames!!\n\nMaximum length for an ANI/EFF/APNG, minus the extension, is %i characters.",filename, MAX_FILENAME_LEN - 10);
        if (img_cfp != nullptr) cfclose (img_cfp);
        return -1;
    }
    ...
    if (type == BM_TYPE_EFF) {
        if (!bm_load_and_parse_eff (
            ...
            WARNINGF (LOCATION, "BMPMAN: Found EFF (%s) with %d frames at %d fps.",filename, anim_frames, anim_fps);
        }
        if (anim_fps == 0) {
    ...
    // regular ani file
        ...
        // we only care if there are 2 keyframes - first frame, other frame to
        // jump to for ship/weapons mainhall door anis hav every frame as
    ...
    // if all images of the animation have the same size then we can use a
    // texture array
    bool is_array = true;
    ...
    for (i = 0; i < anim_frames; i++) {
        ...
        if (type == BM_TYPE_EFF) {
            ...
            if (bm_load_info (
                    eff_type, entry->info.ani.eff.filename, nullptr,
                    &anim_width, &anim_height, &bpp, &c_type, &mm_lvl,
                    &img_size)) {
                // if we didn't get anything then bail out now
                if (i == 0) {
                    WARNINGF (LOCATION,"EFF: No frame images were found.  EFF, %s, is invalid.",filename);
                ...
                WARNINGF (LOCATION,"EFF: Unable to load all frames for '%s', stopping at #%d",filename, i);
        ...
        if (type == BM_TYPE_PNG) { entry->info.ani.apng.is_apng = true; }
        else {
            entry->info.ani.apng.is_apng = false;
        }

        if (first_entry->bm.w != entry->bm.w ||
            first_entry->bm.h != entry->bm.h) {
            // We found a frame with a different size than the first frame ->
            // this can't be used as a texture array
            is_array = false;

            WARNINGF (LOCATION,"Animation '%s' has images that are of different sizes (currently at frame %d).Performance could be improved by making all images the same size.",filename, i + 1);
        }
        if (first_entry->comp_type != entry->comp_type) {
            // Different compression type
            is_array = false;

            WARNINGF (LOCATION,"Animation '%s' has images that are of different compression formats (currently at frame %d).Performance could be improved by making all images the same compression format.",filename, i + 1);
        }
        if (first_entry->bm.true_bpp != entry->bm.true_bpp) {
            // We found a frame with an incompatible pixel format
            is_array = false;

            WARNINGF (LOCATION,"Animation '%s' has images that are of different pixel formats (currently at frame %d).Performance could be improved by making all images the same pixel format.",filename, i + 1);
        }
        if (first_entry->num_mipmaps != entry->num_mipmaps) {
            // We found a frame with a different number of mipmaps
            is_array = false;

            WARNINGF (LOCATION,"Animation '%s' has images that have a different number of mipmaps (currently at frame %d).Performance could be improved by giving all frames the same number of mipmaps.",filename, i + 1);
        }
    }
    ...
    if (nframes != nullptr) *nframes = anim_frames;

    if (fps != nullptr) *fps = anim_fps;

    if (img_cfp != nullptr) cfclose (img_cfp);

    if (keyframe != nullptr) *keyframe = key;

    if (total_time != nullptr) *total_time = anim_total_time;
...
int bm_load_either (
    const char* filename, int* nframes, int* fps, int* keyframe,
    bool can_drop_frames, int dir_type) {
    if (nframes != NULL) *nframes = 0;
    if (fps != NULL) *fps = 0;
    int tidx = bm_load_animation (
        filename, nframes, fps, keyframe, nullptr, can_drop_frames, dir_type);
    if (tidx == -1) {
        tidx = bm_load (filename);
        if (tidx != -1 && nframes != NULL) *nframes = 1;
    }

    return tidx;
}

int bm_load_sub_fast (
    const char* real_filename, int* handle, int dir_type, bool animated_type) {
    if (Bm_ignore_duplicates) return 0;

    for (auto& block : bm_blocks) {
        for (auto& slot : block) {
            auto& entry = slot.entry;
            if (entry.type == BM_TYPE_NONE) continue;

            if (entry.dir_type != dir_type) continue;
...
int bm_load_sub_slow (
    const char* real_filename, const int num_ext, const char** ext_list,
    CFILE** img_cfp, int dir_type) {
    auto res = cf_find_file_location_ext (
        real_filename, num_ext, ext_list, dir_type, false);

    // could not be found, or is invalid for some reason
    if (!res.found) return -1;

    CFILE* test = cfopen_special (
        res.full_name.c_str (), "rb", res.size, res.offset, res.data_ptr,
        dir_type);

    if (test != NULL) {
        if (img_cfp != NULL) *img_cfp = test;
...
bitmap* bm_lock (int handle, int bpp, ubyte flags, bool nodebug) {
...
#ifdef BMPMAN_NDEBUG
    if (be->used_this_frame < 255) { be->used_this_frame++; }
#endif
    ...
    if (bm_is_anim (be) == true) {
        ...
        for (i = 0; i < frames; i++) {
            // Mark all the bitmaps in this bitmap or animation as recently
            // used
...
#ifdef BMPMAN_NDEBUG
            // Mark all the bitmaps in this bitmap or animation as used for the
            // usage tracker.
...
void bm_lock_ani (
    int /*handle*/, bitmap_slot* bs, bitmap* /*bmp*/, int bpp, ubyte flags) {
    anim* the_anim;
        ...
        if ((bm->w != the_anim->width) || (bm->h != the_anim->height)) {
            ...
            // 16 bpp
                ...
                for (h = 0; h < bm->h; h++) {
                    ...
                    unsigned short* srow =
                        &((unsigned short*)sptr)[f2i (v) * the_anim->width];
...
void bm_lock_apng (
    ...
    for (int i = 0; i < nframes; i++) {
        ...
        WARNINGF (LOCATION, "locking apng frame: %s (%i|%i|%i) (%f) %zu",be->filename, bpp, bmp->bpp, bm->true_bpp,be->info.ani.apng.frame_delay, be->mem_taken);
    }
}
...
void bm_lock_pcx (
    ...
    pcx_error = pcx_read_bitmap (
        filename, data, NULL, (bpp >> 3), (flags & BMP_AABITMAP),
        (flags & BMP_MASK_BITMAP) != 0, be->dir_type);

    if (pcx_error != PCX_ERROR_NONE) {
        WARNINGF (LOCATION, "Couldn't load PCX!!! (%s)", filename);
        return;
    }
...
void bm_lock_png (
    ...
    bmp->bpp = 32;
    d_size = bmp->bpp >> 3;
    // we waste memory if it turns out to be 24-bit, but the way this whole
    // thing works is dodgy anyway
    ...
    png_error =
        png_read_bitmap (filename, data, &bmp->bpp, d_size, be->dir_type);
...
void bm_lock_tga (
    ...
    if (data) { memset (data, 0, be->mem_taken); }
    else {
        return;
    }
    ...
    tga_error =
        targa_read_bitmap (filename, data, nullptr, byte_size, be->dir_type);
...
int bm_make_render_target (int width, int height, int flags) {
    ...
    if (flags & BMP_FLAG_RENDER_TARGET_STATIC) {
        ...
        if (flags & BMP_FLAG_CUBEMAP) { size *= 6; }
    }
    ...
    entry->type = (flags & BMP_FLAG_RENDER_TARGET_STATIC)
                ...
                if (entry.preloaded) {
                    ...
                    if (bm_preloading) {
                        if (!gr_preload (
                            ...
                            WARNINGF (LOCATION, "Out of VRAM.  Done preloading.");
                            bm_preloading = 0;
                        }
                    }
                    else {
                        bm_lock (
                            entry.handle,
                            (entry.used_flags == BMP_AABITMAP) ? 8 : 16,
                            entry.used_flags);
...
void bm_page_in_xparent_texture (int bitmapnum, int nframes) {
    ...
    for (i = 0; i < nframes; i++) {
        ...
        case BM_TYPE_DXT1: entry->used_flags = BMP_TEX_DXT1; continue;

        case BM_TYPE_DXT3: entry->used_flags = BMP_TEX_DXT3; continue;

        case BM_TYPE_DXT5: entry->used_flags = BMP_TEX_DXT5; continue;
...
bool bm_page_out (int handle) {
    ...
    if (entry->type == BM_TYPE_NONE) return 0;
    ...
    if (entry->preload_count > 0) {
        WARNINGF (LOCATION, "PAGE-OUT: %s - preload_count remaining: %d",entry->filename, entry->preload_count);
...
void bm_print_bitmaps () {
    ...
    for (auto& block : bm_blocks) {
        for (auto& slot : block) {
            ...
            if (entry.type != BM_TYPE_NONE) {
                if (entry.data_size) {
                    WARNINGF (LOCATION, "BMPMAN = num: %d, name: %s, handle: %d - (%s) size: %.3fM", entry.handle, entry.filename, entry.handle, entry.data_size ? "*LOCKED*" : "", ((float)entry.data_size / 1024.0f) / 1024.0f);
                }
                else {
                    WARNINGF (LOCATION, "BMPMAN = num: %d, name: %s, handle: %d",entry.handle, entry.filename, entry.handle);
                }
            }
        }
    }

    WARNINGF (LOCATION, "BMPMAN = LOCKED memory usage: %.3fM",((float)bm_texture_ram / 1024.0f) / 1024.0f);
#endif
}
...
int bm_release (int handle, int clear_render_targets) {
    ...
    if (be->ref_count != 0) {
        WARNINGF (LOCATION,"Tried to release %s that has a lock count of %d.. not releasing",be->filename, be->ref_count);
        return 0;
    }

    // kind of like ref_count except it gets around the lock/unlock usage
    // problem this gets set for each bm_load() call so we can make sure and
    // not unload it from memory, even if we *can*, until it's really not
    // needed anymore
    if (be->load_count > 0) be->load_count--;

    if (be->load_count != 0) {
        WARNINGF (LOCATION,"Tried to release %s that has a load count of %d.. not releasing",be->filename, be->load_count + 1);
        return 0;
    }

    if (be->type != BM_TYPE_USER) {
        WARNINGF (LOCATION, "Releasing bitmap %s with handle %i", be->filename,handle);
    }
    ...
    if (bm_is_anim (be)) {
        ...
        // The graphics system requires that the bitmap information for the
        // whole animation is still valid when the data is freed so we first
        // free all the data and then clear the bitmaps data
        for (i = 0; i < total; i++) {
        ...
        for (i = 0; i < total; i++) {
            ...
            entry->palette_checksum =
                0xDEADBEEF; // checksum used to be sure bitmap is in current
                            // palette

            // bookeeping
            entry->ref_count = -1; // Number of locks on bitmap.  Can't unload
                                   // unless ref_count is 0.
...
int bm_reload (int bitmap_handle, const char* filename) {
    ...
    if (entry->ref_count) {
        WARNINGF (LOCATION,"Trying to reload a bitmap that is still locked. Filename: %s, ref_count: %d",entry->filename, entry->ref_count);
        return -1;
    }
...
void bm_set_components_argb_16_screen (
    ubyte* pixel, ubyte* rv, ubyte* gv, ubyte* bv, ubyte* av) {
    if (*av == 0) {
    ...
    *((unsigned short*)pixel) =
        (unsigned short)(((int)*rv / Gr_current_red->scale) << Gr_current_red->shift);
    *((unsigned short*)pixel) |= (unsigned short)(((int)*gv / Gr_current_green->scale) << Gr_current_green->shift);
    *((unsigned short*)pixel) |=
        (unsigned short)(((int)*bv / Gr_current_blue->scale) << Gr_current_blue->shift);
}

void bm_set_components_argb_16_tex (
    ubyte* pixel, ubyte* rv, ubyte* gv, ubyte* bv, ubyte* av) {
    if (*av == 0) {
...
void bm_set_components_argb_32_screen (
    ubyte* pixel, ubyte* rv, ubyte* gv, ubyte* bv, ubyte* av) {
    if (*av == 0) {
    ...
    *((unsigned int*)pixel) =
        (unsigned int)(((int)*rv / Gr_current_red->scale) << Gr_current_red->shift);
    *((unsigned int*)pixel) |=
        (unsigned int)(((int)*gv / Gr_current_green->scale) << Gr_current_green->shift);
    *((unsigned int*)pixel) |=
        (unsigned int)(((int)*bv / Gr_current_blue->scale) << Gr_current_blue->shift);
}

void bm_set_components_argb_32_tex (
    ubyte* pixel, ubyte* rv, ubyte* gv, ubyte* bv, ubyte* av) {
    if (*av == 0) {
...
bool bm_set_render_target (int handle, int face) {
    ...
    if (handle >= 0) {
        if ((entry->type != BM_TYPE_RENDER_TARGET_STATIC) &&
            ...
            // bm_load()
            WARNINGF (LOCATION,"Trying to set invalid bitmap (handle: %i) as render target!",handle);
            return false;
        }
    }
    ...
    if (gr_bm_set_render_target (handle, face)) {
        if (gr_screen.rendering_to_texture == -1) {
            // if we are moving from the back buffer to a texture save whatever
            ...
            gr_screen.save_max_w_unscaled_zoomed =
                gr_screen.max_w_unscaled_zoomed;
            gr_screen.save_max_h_unscaled_zoomed =
...
int bm_unload (int handle, int clear_render_targets, bool nodebug) {
    ...
    if (be->ref_count != 0 && !nodebug) {
        WARNINGF (LOCATION,"Tried to unload %s that has a lock count of %d.. not unloading",be->filename, be->ref_count);
        return 0;
    }
    ...
    if (!Bm_ignore_load_count) {
        ...
        if (be->load_count != 0 && !nodebug) {
            WARNINGF (LOCATION,"Tried to unload %s that has a load count of %d.. not unloading",be->filename, be->load_count + 1);
            return 0;
        }
    }
    ...
    if (bm_is_anim (be) == true) {
        ...
        // for the unload all case, don't try to unload every frame of every
        // frame all additional frames automatically get unloaded with the
        // first one
        if ((handle != be->info.ani.first_frame) &&
            (first_entry->bm.data == 0))
            return 1;
...
void bm_unload_all () {
    // since bm_unload_all() should only be called from game_shutdown() it
    // should be safe to ignore load_count's and unload anyway
    Bm_ignore_load_count = 1;
    for (auto& block : bm_blocks) {
        for (auto& slot : block) {
            auto& entry = slot.entry;

            if (entry.type != BM_TYPE_NONE) { bm_unload (entry.handle, 1); }
        }
    }
...
int bm_unload_fast (int handle, int clear_render_targets) {
    ...
    if (be->type == BM_TYPE_USER) { return -1; }
    ...
    // If it is locked, cannot free it.
    if (be->ref_count != 0) {
        WARNINGF (LOCATION,"Tried to unload_fast %s that has a lock count of %d.. not unloading",be->filename, be->ref_count);
        return 0;
    }
    ...
    WARNINGF (LOCATION, "Fast-unloading %s.  %dx%dx%d", be->filename, bmp->w,bmp->h, bmp->bpp);
...
static int find_block_of (int n, int start_block) {
    ...
    int block_idx;
    for (block_idx = start_block; block_idx < (int)bm_blocks.size ();
         ++block_idx) {
        ...
        for (auto& slot : block) {
            ...
            if (entry.type == BM_TYPE_NONE) {
                if (cnt == 0) { nstart = get_handle (block_idx, entry_idx); }
    ...
    // If we are here it means that we could not find a block to store the
    // bitmap blocks in. In that case we just allocate a new block and try to
    // allocate the block in that
    allocate_new_block ();

    // This call is sure to succeed since we now have a contiguous block of
    // size BM_BLOCK_SIZE and n must be less than that
...
bool bm_is_texture_array (const int handle) {
    ...
    if (!bm_is_anim (entry)) { return true; }
...
int bm_get_base_frame (const int handle, int* num_frames) {
    if (handle == -1) {
        if (num_frames != nullptr) { *num_frames = 0; }
        return -1;
    }

    // If the bitmap is no animation then num_frames will still be at least 1
    auto animation_begin =
        bm_get_info (handle, nullptr, nullptr, nullptr, num_frames);
    ...
    if (!bm_is_texture_array (animation_begin)) {
        // If this is not a texture array then we just treat it like a single
        // frame animation
        animation_begin = handle;
        if (num_frames != nullptr) { *num_frames = 1; }
    }
...
bool bm_validate_filename (
    ...
    if (!VALID_FNAME (file.c_str ())) { return false; }
...
SDL_Surface* bm_to_sdl_surface (int handle) {
    ...
    if (SDL_LockSurface (bitmapSurface) < 0) { return nullptr; }
    bitmap* bmp = bm_lock (handle, 32, BMP_TEX_XPARENT);

    memcpy (
        bitmapSurface->pixels, reinterpret_cast< void* > (bmp->data),
