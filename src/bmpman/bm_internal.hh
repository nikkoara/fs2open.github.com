 ...
 *
 * @details It is a "protected" header that requires BMPMAN_INTERNAL to be
 * defined before it can be included. This is to provide a small measure of
 * safety since this functions can cause problems if not used correctly
 */
...
union bm_extra_info {
        struct {
                ...
                int first_frame;  //!< used for animations -- points to index of first
                                  //!< frame
                int num_frames;   //!< used for animations -- number of frames in the
                                  //!< animation
                int keyframe;     //!< used for animations -- keyframe info
                float total_time; //!< used for animations -- total animation time (not
                                  //!< always derived from num_frames/fps)
                ubyte fps;        //!< used for animations -- frames per second
                bool is_array;    //!< Flag for if all frames of an animation have the
                                  //!< same size which means that it can be put into a
                                  //!< texture array
                ...
                struct {
                        ...
                        char
                                filename[MAX_FILENAME_LEN]; //!< filename for individual images
                } eff;
                struct {
...
struct bitmap_entry {
        ...
        BM_TYPE comp_type;     //!< What sort of compressed type, BM_TYPE_NONE if not
                               //!< compressed
        signed char ref_count; //!< Number of locks on bitmap.  Can't unload unless
                               //!< ref_count is 0.

        int dir_type; //!< which directory this was loaded from (to skip other
                      //!< locations with same name)

        // compressed bitmap stuff (.dds) - RT please take a look at this and tell
        // me if we really need it
        size_t
                mem_taken; //!< How much memory does this bitmap use? - UnknownPlayer
        ...
        int preload_count; //!< how many times this gets used in game, for
                           //!< unlocking
...
inline bitmap_entry *
bm_get_entry(int handle, bool separate_ani_frames = true)
{
...
#endif // FREESPACE2_BMPMAN_BM_INTERNAL_HH
