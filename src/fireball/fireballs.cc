...
void fireball_play_warphole_open_sound (int ship_class, fireball* fb) {
    ...
    snd_play_3d (
        gamesnd_get_game_sound (sound_index), &fireball_objp->pos,
        &Eye_position, fireball_objp->radius, NULL, 0, 1.0f,
...
void fireball_play_warphole_close_sound (fireball* fb) {
    ...
    snd_play_3d (
        gamesnd_get_game_sound (sound_index), &fireball_objp->pos,
        &Eye_position, fireball_objp->radius); // play warp sound effect
}
 ...
 * Parse fireball tbl
 *
 * NOTE: we can't be too trusting here so a tbm will only modify the LOD count,
 * not add an entry
 */
void parse_fireball_tbl (const char* filename) {
    ...
    try {
        ...
        while (required_string_either ("#End", "$Name:")) {
            ...
            // these entries should only be in TBMs, and it has to include at
            // least one
            ...
            if (optional_string ("$Light color:")) {
                ...
                // to keep things simple, we just use 0 alpha to indicate that
                // a default value should be used
                memset (&fb_color, 0, sizeof (color));
            }

            // we may use one filename for multiple entries so we'll have to
            // handle dupes post parse
...
void fireball_parse_tbl () {
    ...
    for (lod = LOD_checker.begin (); lod != LOD_checker.end (); ++lod) {
        if ((i < MAX_FIREBALL_TYPES) && (lod->override < 0)) {
            strcpy (Fireball_info[i].lod[0].filename, lod->filename);
    ...
    for (lod = LOD_checker.begin (); lod != LOD_checker.end (); ++lod) {
        ...
        if ((lod->override >= 0) && (lod->override < Num_fireball_types)) {
            strcpy (
                Fireball_info[lod->override].lod[0].filename, lod->filename);
            Fireball_info[lod->override].lod_count = lod->num_lods;

            if (LOD_color[i].alpha == 255) {
                Fireball_info[lod->override].exp_color[0] =
                    (LOD_color[i].red / 255.0f);
                Fireball_info[lod->override].exp_color[1] =
                    (LOD_color[i].green / 255.0f);
                Fireball_info[lod->override].exp_color[2] =
                    (LOD_color[i].blue / 255.0f);
            }
            else {
    ...
    for (i = 0; i < Num_fireball_types; i++) {
        for (j = 1; j < Fireball_info[i].lod_count; j++) {
            sprintf (
                Fireball_info[i].lod[j].filename, "%s_%d",
                Fireball_info[i].lod[0].filename, j);
        }
    }
...
void fireball_load_data () {
    ...
    for (i = 0; i < Num_fireball_types; i++) {
        ...
        for (idx = 0; idx < fd->lod_count; idx++) {
            // we won't use a warp effect lod greater than MAX_WARP_LOD so
            // don't load it either
            if ((i == FIREBALL_WARP) && (idx > MAX_WARP_LOD)) continue;

            fd->lod[idx].bitmap_id = bm_load_animation (
                fd->lod[idx].filename, &fd->lod[idx].num_frames,
                &fd->lod[idx].fps, nullptr, nullptr, true);
            if (fd->lod[idx].bitmap_id < 0) {
                ASSERTX (0, "Could not load %s anim file\n",fd->lod[idx].filename);
            }
        }
    }
...
void fireball_init () {
    ...
    Num_fireballs = 0;
    for (i = 0; i < MAX_FIREBALLS; i++) { Fireballs[i].objnum = -1; }
...
void fireball_delete_all () {
    ...
    for (i = 0; i < MAX_FIREBALLS; i++) {
        fb = &Fireballs[i];
        if (fb->objnum != -1) { obj_delete (fb->objnum); }
    }
}
...
void fireball_set_framenum (int num) {
    ...
    if (fb->fireball_render_type == FIREBALL_WARP_EFFECT) {
        framenum =
            bm_get_anim_frame (fl->bitmap_id, fb->time_elapsed, 0.0f, true);
        ...
        framenum = bm_get_anim_frame (
            fl->bitmap_id, fb->time_elapsed, fb->total_time);
        fb->current_bitmap = fl->bitmap_id + framenum;
    }
}
...
int fireball_is_perishable (object* obj) {
    ...
    if (fb->fireball_render_type == FIREBALL_MEDIUM_EXPLOSION) return 1;

    if (!(fb->fireball_render_type == FIREBALL_WARP_EFFECT)) {
        if (!(obj->flags[Object::Object_Flags::Was_rendered])) { return 1; }
    }
...
int fireball_free_one () {
    ...
    for (i = 0; i < MAX_FIREBALLS; i++) {
        ...
        if ((fb->objnum >= 0) &&
            fireball_is_perishable (&Objects[fb->objnum])) {
    ...
    if (oldest_objnum > -1) { obj_delete (oldest_objnum); }
...
int fireball_is_warp (object* obj) {
    ...
    if (fb->fireball_render_type == FIREBALL_WARP_EFFECT) return 1;
...
void fireball_maybe_play_warp_close_sound (fireball* fb) {
    ...
    if (!(fb->fireball_render_type == FIREBALL_WARP_EFFECT)) { return; }

    // If the warhole close sound has been played, don't play it again!
    if (fb->flags & FBF_WARP_CLOSE_SOUND_PLAYED) { return; }
...
int fireball_get_lod (vec3d* pos, fireball_info* fd, float size) {
    ...
    if (!g3_get_bitmap_dims (
            fd->lod[0].bitmap_id, &v, size, &x, &y, &w, &h, &bm_size)) {
        if (Detail.hardware_textures == 4) {
            // straight LOD
            if (w <= bm_size / 8) { ret_lod = 3; }
            else if (w <= bm_size / 2) {
                ret_lod = 2;
            }
 ...
 */
int fireball_create (
    vec3d* pos, int fireball_type, int render_type, int parent_obj, float size,
    int reverse, vec3d* velocity, float warp_lifetime, int ship_class,
    matrix* orient_override, int low_res, int extra_flags,
    gamesnd_id warp_open_sound, gamesnd_id warp_close_sound) {
    int n, objnum, fb_lod;
    object* obj;
    fireball* fb;
    fireball_info* fd;
    ...
    if (fireball_type == FIREBALL_WARP) { fb_lod = MAX_WARP_LOD; }
    fl = &fd->lod[fb_lod];
    ...
    matrix orient;
    if (orient_override != NULL) { orient = *orient_override; }
    else {
        if (parent_obj < 0) { orient = vmd_identity_matrix; }
        else {
    ...
    case FIREBALL_WARP_EFFECT:
        ...
        if (reverse) {
            ...
            vm_vec_scale (
                &obj->orient.vec.fvec, -1.0f); // Reverse the forward vector
            vm_vec_scale (
                &obj->orient.vec.rvec, -1.0f); // Reverse the right vector
        }
        else {
            fb->orient = 0;
        }
...
void fireballs_page_in () {
    ...
    for (i = 0; i < Num_fireball_types; i++) {
        if ((i < NUM_DEFAULT_FIREBALLS) || fireball_used[i]) {
            ...
            // if this is a Knossos ani, only load if Knossos_warp_ani_used is
            // true
            if ((i == FIREBALL_KNOSSOS) && !Knossos_warp_ani_used) continue;
            ...
            for (idx = 0; idx < fd->lod_count; idx++) {
                ...
                bm_page_in_texture (
                    fd->lod[idx].bitmap_id, fd->lod[idx].num_frames);
            }
        }
    }
...
int fireball_ship_explosion_type (ship_info* sip) {
    ...
    if (sip->class_type >= 0) {
        objecttype_fireballs =
            (int)Ship_types[sip->class_type].explosion_bitmap_anims.size ();
    }
    ...
    if (ship_fireballs > 0) {
        ...
        index = Ship_types[sip->class_type]
                    .explosion_bitmap_anims[rand () % objecttype_fireballs];
    }
...
void fireball_render (object* obj, model_draw_list* scene) {
    ...
    if (Fireballs[num].current_bitmap < 0) return;
    ...
    case FIREBALL_MEDIUM_EXPLOSION: {
        batching_add_volume_bitmap (
            Fireballs[num].current_bitmap, &p, fb->orient, obj->radius);
    } break;
    ...
    case FIREBALL_WARP_EFFECT: {
        ...
        if (t < WARPHOLE_GROW_TIME) {
            ...
            rad =
                (float)pow ((fb->total_time - t) / WARPHOLE_GROW_TIME, 0.4f) *
                obj->radius;
        }

        warpin_queue_render (
            scene, obj, &obj->orient, &obj->pos, Fireballs[num].current_bitmap,
            rad, percent_life, obj->radius,
