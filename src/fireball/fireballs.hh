...
#define MAX_FIREBALL_TYPES \
        32 // The maximum number of fireballs that can be defined
...
struct fireball {
        ...
        int current_bitmap;
        int orient;                        // For fireballs, which orientation.  For warps, 0 is warpin, 1
                                           // is warpout
...
// warp_lifetime: If warp_lifetime > 0.0f then makes the explosion loop so it
// lasts this long.  Only works for warp effect
int fireball_create(
        vec3d *pos, int fireball_type, int render_type, int parent_obj, float size,
        int reversed = 0, vec3d *velocity = NULL, float warp_lifetime = 0.0f,
        int ship_class = -1, matrix *orient = NULL, int low_res = 0,
        int extra_flags = 0, gamesnd_id warp_open_sound = gamesnd_id(),
        gamesnd_id warp_close_sound = gamesnd_id());
...
// returns the index of the fireball bitmap for this asteroid. -1 if there is
// none.
...
extern void warpin_render(
        object *obj, matrix *orient, vec3d *pos, int texture_bitmap_num,
        float radius, float life_percent, float max_radius, int warp_3d = 0);

extern void warpin_queue_render(
...
#endif // FREESPACE2_FIREBALL_FIREBALLS_HH
