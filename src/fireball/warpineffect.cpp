...
void warpin_batch_draw_face (int texture, vertex* v1, vertex* v2, vertex* v3) {
    ...
    // batch_add_tri(texture, TMAP_FLAG_TEXTURED | TMAP_HTL_3D_UNLIT |
    // TMAP_FLAG_EMISSIVE, vertlist, 1.0f);
    batching_add_tri (texture, vertlist); // TODO render as emissive
}

void warpin_queue_render (
    model_draw_list* scene, object* obj, matrix* orient, vec3d* pos,
    int texture_bitmap_num, float radius, float life_percent, float max_radius,
    int warp_3d) {
    vec3d center;
    vec3d vecs[5];
    vertex verts[5];
    ...
    vm_vec_scale_add (
        &center, pos, &orient->vec.fvec, -(max_radius / 2.5f) / 3.0f);
...
#define IN_PERCENT2 0.20f
        ...
        if (Cmdline_warp_flash) {
            if ((life_percent >= IN_PERCENT1) &&
                (life_percent < IN_PERCENT2)) {
                r *=
                    (life_percent - IN_PERCENT1) / (IN_PERCENT2 - IN_PERCENT1);
                // render_it = true;
            }
            else if (
                (life_percent >= OUT_PERCENT1) &&
                (life_percent < OUT_PERCENT2)) {
                r *= (OUT_PERCENT2 - life_percent) /
                     (OUT_PERCENT2 - OUT_PERCENT1);
                // render_it = true;
            }
        }
        ...
        if (render_it) {
            ...
            if (Cmdline_warp_flash) {
                r += powf ((2.0f * life_percent) - 1.0f, 24.0f) * max_radius *
                     1.5f;
            }
            ...
            float alpha = (The_mission.flags[Mission::Mission_Flags::Fullneb])
                              ? (1.0f - neb2_get_fog_intensity (obj))
                              : 1.0f;

            // batch_add_bitmap(Warp_glow_bitmap, TMAP_FLAG_TEXTURED |
            // TMAP_HTL_3D_UNLIT, &verts[4], 0, r, alpha);
    ...
    if ((Warp_model >= 0) && (warp_3d || Cmdline_3dwarp)) {
        ...
        render_info.set_warp_params (
            texture_bitmap_num, radius / max_radius, warp_scale);
        render_info.set_detail_level_lock ((int)(dist / (radius * 10.0f)));
        render_info.set_flags (
            MR_NO_LIGHTING | MR_NORMAL | MR_NO_FOGGING | MR_NO_CULL |
            MR_NO_BATCH);
        ...
        // gr_set_bitmap( texture_bitmap_num, GR_ALPHABLEND_FILTER,
        ...
        warpin_batch_draw_face (
            texture_bitmap_num, &verts[0], &verts[4], &verts[1]);
        warpin_batch_draw_face (
            texture_bitmap_num, &verts[1], &verts[4], &verts[2]);
        warpin_batch_draw_face (
            texture_bitmap_num, &verts[4], &verts[3], &verts[2]);
        warpin_batch_draw_face (
            texture_bitmap_num, &verts[0], &verts[3], &verts[4]);
    }

    if (Warp_ball_bitmap > -1 && Cmdline_warp_flash == 1) {
        flash_ball warp_ball (
            20, .1f, .25f, &orient->vec.fvec, pos, 4.0f, 0.5f);
        ...
        if (pct > 0.00001f) {
            warp_ball.render (
                Warp_ball_bitmap, max_radius * pct * 0.5f, adg * adg,
                adg * adg * 6.0f);
        }
    }
}
