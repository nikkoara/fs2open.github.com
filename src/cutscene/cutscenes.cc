    ...
    try {
        ...
        while (required_string_either ("#End", "$Filename:")) {
            ...
            if (isFirstCutscene) {
                ...
                // The original code assumes the first movie is the intro, so
                // always viewable
...
UI_XSTR Cutscene_text[GR_NUM_RESOLUTIONS][NUM_CUTSCENE_TEXT] = {
    {
        // GR_640
        { "Technical Database", 1055, 37, 7, UI_XSTR_COLOR_GREEN, -1,
          &Buttons[0][TECH_DATABASE_BUTTON].button },
        { "Mission Simulator", 1056, 37, 23, UI_XSTR_COLOR_GREEN, -1,
          &Buttons[0][SIMULATOR_BUTTON].button },
        { "Cutscenes", 1057, 37, 38, UI_XSTR_COLOR_GREEN, -1,
          &Buttons[0][CUTSCENES_BUTTON].button },
        { "Credits", 1058, 37, 54, UI_XSTR_COLOR_GREEN, -1,
          &Buttons[0][CREDITS_BUTTON].button },

        { "Play", 1335, 587, 366, UI_XSTR_COLOR_GREEN, -1,
          &Buttons[0][PLAY_BUTTON].button },
        { "Exit", 1419, 587, 413, UI_XSTR_COLOR_PINK, -1,
          &Buttons[0][EXIT_BUTTON].button },
    },
    {
        // GR_1024
        { "Technical Database", 1055, 59, 12, UI_XSTR_COLOR_GREEN, -1,
          &Buttons[1][TECH_DATABASE_BUTTON].button },
        { "Mission Simulator", 1056, 59, 37, UI_XSTR_COLOR_GREEN, -1,
          &Buttons[1][SIMULATOR_BUTTON].button },
        { "Cutscenes", 1057, 59, 62, UI_XSTR_COLOR_GREEN, -1,
          &Buttons[1][CUTSCENES_BUTTON].button },
        { "Credits", 1058, 59, 88, UI_XSTR_COLOR_GREEN, -1,
          &Buttons[1][CREDITS_BUTTON].button },

        { "Play", 1335, 940, 586, UI_XSTR_COLOR_GREEN, -1,
          &Buttons[1][PLAY_BUTTON].button },
        { "Exit", 1419, 940, 661, UI_XSTR_COLOR_PINK, -1,
          &Buttons[1][EXIT_BUTTON].button },
    }
};

int Cutscene_list_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                      9, 117, 621, 198 },
                                                    { // GR_1024
                                                      14, 188, 994, 316 } };

int Cutscene_desc_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                      9, 378, 484, 73 },
                                                    { // GR_1024
                                                      14, 605, 775, 117 } };

#define MAX_TEXT_LINES 20
int Cutscene_max_text_lines[GR_NUM_RESOLUTIONS] = { 10, MAX_TEXT_LINES };
...
void cutscenes_screen_scroll_line_down () {
    ...
    if (Selected_line >= Scroll_offset + h) { Scroll_offset++; }
}
...
void cutscenes_screen_scroll_screen_down () {
    ...
    if (Scroll_offset + h < (int)Cutscene_list.size ()) {
        Scroll_offset++;
        if (Selected_line < Scroll_offset) { Selected_line = Scroll_offset; }
...
int cutscenes_screen_button_pressed (int n) {
    ...
    case SCROLL_UP_BUTTON: cutscenes_screen_scroll_screen_up (); break;

    case SCROLL_DOWN_BUTTON: cutscenes_screen_scroll_screen_down (); break;

    case PLAY_BUTTON: cutscenes_screen_play (); break;
...
void cutscenes_screen_init () {
    ...
    List_region.create (
        &Ui_window, "", Cutscene_list_coords[gr_screen.res][0],
        Cutscene_list_coords[gr_screen.res][1],
        Cutscene_list_coords[gr_screen.res][2],
        Cutscene_list_coords[gr_screen.res][3], 0, 1);
    List_region.hide ();
    ...
    int u = 0;
    for (std::vector< cutscene_info >::iterator cut = Cutscenes.begin ();
         cut != Cutscenes.end (); ++cut, u++) {
        if ((*cut).viewable) { Cutscene_list.push_back (u); }
    }
}
...
void cutscenes_screen_do_frame () {
    ...
    case KEY_ENTER: cutscenes_screen_play (); break;
    ...
    case KEY_CTRLED | KEY_SHIFTED | KEY_S: {
        ...
        for (size_t t = 0; t < size; t++) { Cutscene_list.push_back ((int)t); }
    ...
    for (i = 0; i < NUM_BUTTONS; i++) {
        if (Buttons[gr_screen.res][i].button.pressed ()) {
            if (cutscenes_screen_button_pressed (i)) { return; }
        }
    }
    ...
    if (List_region.pressed ()) {
        List_region.get_mouse_pos (NULL, &y);
        z = Scroll_offset + y / font_height;
        if ((z >= 0) && (z < (int)Cutscene_list.size ())) Selected_line = z;
    }
    ...
    while (y + font_height <= Cutscene_list_coords[gr_screen.res][3]) {
        if (z >= (int)Cutscene_list.size ()) { break; }

        if (z == Selected_line) { gr_set_color_fast (&Color_text_selected); }
        ...
        gr_printf_menu (
            Cutscene_list_coords[gr_screen.res][0],
            Cutscene_list_coords[gr_screen.res][1] + y, "%s",
    ...
    if (Description_index >= 0) {
        ...
        while (y + font_height <= Cutscene_desc_coords[gr_screen.res][3]) {
            if (z >= Text_size || z >= MAX_TEXT_LINES - 1) break;
            ...
            line[len] = 0;
            gr_string (
                Cutscene_desc_coords[gr_screen.res][0],
                Cutscene_desc_coords[gr_screen.res][1] + y, line,
                GR_RESIZE_MENU);
