...
void processEvents(PlaybackState *state)
{
        ...
        case KEY_ESC:
        case KEY_ENTER:
        case KEY_SPACEBAR: state->playing = false; break;
        default: break;
        }
}
...
void showVideoInfo(const PlayerState &state)
{
        ...
        y = print_string(
                x, y, "Size: %dx%d", state.props.size.width, state.props.size.height);
...
void displayVideo(Player *player, PlaybackState *state)
{
        ...
        gr_clear();
        player->draw(
                state->posTopLeft.x, state->posTopLeft.y, state->posBottomRight.x,
                state->posBottomRight.y);
        ...
        if (!subtitle.empty() && state->subtitle_font >= 0) {
                ...
                auto y_offset = static_cast< int >(
                        (state->posBottomRight.y - state->posTopLeft.y) * 0.1);
...
void determine_display_positions(Player *player, PlaybackState *state)
{
        ...
        if (!Cmdline_noscalevid && (scale_by != 1.0f)) {
                ...
                screenX = float(
                        ((gr_screen.center_w - static_cast< int >(props.size.width)) / 2) + gr_screen.center_offset_x);
                screenY = float(
                        ((gr_screen.center_h - static_cast< int >(props.size.height)) / 2) + gr_screen.center_offset_y);
        }
...
void initialize_player_state(Player *player, PlaybackState *state)
{
        ...
        if (state->subtitle_font < 0) {
                WARNINGF(LOCATION, "Failed to load subtitle font '%s'! Subtitles will be disabled.", Movie_subtitle_font.c_str());
        }
}

void movie_display_loop(Player *player, PlaybackState *state)
{
        // Compute the maximum time we will sleep to make sure we can still
        // maintain the movie FPS and not waste too much CPU time We will sleep at
        // most half the time a frame would be displayed
        auto sleepTime = static_cast< std::uint64_t >(
                (1. / (4. * player->getMovieProperties().fps)) * 1000.);
