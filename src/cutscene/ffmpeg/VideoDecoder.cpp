...
SwsContext* getSWSContext (
    int width, int height, AVPixelFormat fmt, AVPixelFormat destination_fmt) {
    return sws_getContext (
        width, height, fmt, width, height, destination_fmt, SWS_BILINEAR,
...
public:
    FFMPEGVideoFrame (size_t width, size_t height, AVFrame* frame)
        : _width (width), _height (height), _frame (frame) {}
...
VideoDecoder::VideoDecoder (
    ...
    : FFMPEGStreamDecoder (status), m_frameId (0),
      m_destinationFormat (destination_fmt) {
    m_swsCtx = getSWSContext (
        m_status->videoCodecPars.width, m_status->videoCodecPars.height,
        m_status->videoCodecPars.pixel_format, destination_fmt);
}

VideoDecoder::~VideoDecoder () { sws_freeContext (m_swsCtx); }
...
void VideoDecoder::convertAndPushPicture (const AVFrame* frame) {
    ...
    av_image_alloc (
        yuvFrame->data, yuvFrame->linesize, m_status->videoCodecPars.width,
        m_status->videoCodecPars.height, m_destinationFormat, 1);

    if (m_status->videoCodecPars.pixel_format == m_destinationFormat) {
        av_image_copy (
            yuvFrame->data, yuvFrame->linesize, (const uint8_t**)(frame->data),
            frame->linesize, m_destinationFormat,
            m_status->videoCodecPars.width, m_status->videoCodecPars.height);
    }
    else {
        // Convert frame to destination format
        sws_scale (
            m_swsCtx, (uint8_t const* const*)frame->data, frame->linesize, 0,
            m_status->videoCodecPars.height, yuvFrame->data,
            yuvFrame->linesize);
    }

    std::unique_ptr< FFMPEGVideoFrame > videoFramePtr (new FFMPEGVideoFrame (
        static_cast< size_t > (m_status->videoCodecPars.width),
    ...
    videoFramePtr->frameTime = getFrameTime (
        frame->best_effort_timestamp, m_status->videoStream->time_base);
#else
    videoFramePtr->frameTime = getFrameTime (
        av_frame_get_best_effort_timestamp (frame),
        m_status->videoStream->time_base);
#endif
...
void VideoDecoder::decodePacket (AVPacket* packet) {
    ...
    do {
        ...
        while (avcodec_receive_frame (
                   m_status->videoCodecCtx, m_decodeFrame) == 0) {
            convertAndPushPicture (m_decodeFrame);
        }
    } while (send_result == AVERROR (EAGAIN));
#else
    int finishedFrame = 0;
    auto result = avcodec_decode_video2 (
        m_status->videoCodecCtx, m_decodeFrame, &finishedFrame, packet);
...
void VideoDecoder::finishDecoding () {
    ...
    while (true) {
        auto ret =
            avcodec_receive_frame (m_status->videoCodecCtx, m_decodeFrame);

        if (ret == 0) { convertAndPushPicture (m_decodeFrame); }
        else {
    ...
    while (true) {
        int finishedFrame = 1;
        auto err = avcodec_decode_video2 (
            m_status->videoCodecCtx, m_decodeFrame, &finishedFrame,
            &nullPacket);
