...
int alloc_array_and_samples (
    uint8_t*** outData, int* linesize, int channels, int samples,
    AVSampleFormat format) {
    auto planes = av_sample_fmt_is_planar (format) ? channels : 1;

    *outData = reinterpret_cast< uint8_t** > (
        av_mallocz (planes * sizeof (**outData)));

    auto ret =
        av_samples_alloc (*outData, linesize, channels, samples, format, 0);

    if (ret < 0) { av_freep (outData); }
...
int resample_convert (
    SwrContext* ctx, uint8_t** output, int out_plane_size, int out_samples,
    uint8_t** input, int in_plane_size, int in_samples) {
#ifdef WITH_LIBAV
    return avresample_convert (
        ctx, output, out_plane_size, out_samples, input, in_plane_size,
        in_samples);
#else
    (void)out_plane_size;
    (void)in_plane_size;
    ...
    return swr_convert (
        ctx, output, out_samples, (const uint8_t**)input, in_samples);
#endif
}
} // namespace
...
AudioDecoder::AudioDecoder (DecoderStatus* status)
    ...
    m_audioBuffer.reserve (
        static_cast< size_t > (OUT_SAMPLE_RATE * OUT_NUM_CHANNELS / 2));

    m_resampleCtx = getSWRContext (
        m_status->audioCodecPars.channel_layout,
     ...
     */
    m_maxOutNumSamples = m_outNumSamples = static_cast< int > (av_rescale_rnd (
        DEFAULT_SRC_NUM_SAMPLES, OUT_SAMPLE_RATE,
        m_status->audioCodecPars.sample_rate, AV_ROUND_UP));

    auto ret = alloc_array_and_samples (
        &m_outData, &m_outLinesize, OUT_NUM_CHANNELS, m_outNumSamples,
        OUT_SAMPLE_FORMAT);
...
AudioDecoder::~AudioDecoder () {
    if (m_outData) { av_freep (&m_outData[0]); }
    av_freep (&m_outData);
...
void AudioDecoder::flushAudioBuffer () {
    ...
    audioFrame->audioData.assign (
        m_audioBuffer.begin (), m_audioBuffer.end ());
...
void AudioDecoder::handleDecodedFrame (AVFrame* frame) {
    ...
    m_outNumSamples = static_cast< int > (av_rescale_rnd (
        getDelay (m_resampleCtx, frame->sample_rate) + frame->nb_samples,
    ...
    if (m_outNumSamples > m_maxOutNumSamples) {
        ...
        auto ret = av_samples_alloc (
            ...
            OUT_SAMPLE_FORMAT, 1);
        if (ret < 0) {
            WARNINGF (LOCATION, "FFMPEG: Failed to allocate samples!!!");
            return;
        }
    ...
    auto ret = resample_convert (
        m_resampleCtx, m_outData, 0, m_outNumSamples, (uint8_t**)frame->data,
        0, frame->nb_samples);
    if (ret < 0) {
        ERRORF (LOCATION, "FFMPEG: Error while converting audio!\n");
        return;
    }
...
void AudioDecoder::decodePacket (AVPacket* packet) {
    ...
    do {
        ...
        while (avcodec_receive_frame (
                   m_status->audioCodecCtx, m_decodeFrame) == 0) {
            handleDecodedFrame (m_decodeFrame);
        }
    } while (send_result == AVERROR (EAGAIN));
#else
    int finishedFrame = 0;
    auto err = avcodec_decode_audio4 (
        m_status->audioCodecCtx, m_decodeFrame, &finishedFrame, packet);

    if (err >= 0 && finishedFrame) { handleDecodedFrame (m_decodeFrame); }
#endif
}
...
void AudioDecoder::finishDecoding () {
    ...
    while (true) {
        auto ret =
            avcodec_receive_frame (m_status->audioCodecCtx, m_decodeFrame);

        if (ret == 0) { handleDecodedFrame (m_decodeFrame); }
        else {
    ...
    while (true) {
        int finishedFrame = 1;
        auto err = avcodec_decode_audio4 (
            m_status->audioCodecCtx, m_decodeFrame, &finishedFrame,
            &nullPacket);

        if (err < 0 || !finishedFrame) { break; }
