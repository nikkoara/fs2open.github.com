...
public:
    explicit AVPacketScope (AVPacket* av_packet) : _packet (av_packet) {}

    AVPacketScope (const AVPacketScope&) = delete;
    AVPacketScope& operator= (const AVPacketScope&) = delete;

    ~AVPacketScope () { av_packet_unref (_packet); }
};
...
double getFrameRate (AVStream* stream, AVCodecContext* codecCtx) {
    ...
    if (fps < 0.000001) { fps = av_q2d (stream->avg_frame_rate); }

    if (fps < 0.000001) { fps = 1.0 / av_q2d (codecCtx->time_base); }
...
/**
 * @brief Given a FFmpeg pixel format returns the format it should be converted
 * to
...
AVPixelFormat getConversionFormat (AVPixelFormat in_fmt) {
    ...
    case AV_PIX_FMT_BGR0: return AV_PIX_FMT_BGR24;
...
FramePixelFormat getPixelFormat (AVPixelFormat fmt) {
    switch (fmt) {
    case AV_PIX_FMT_BGR24: return FramePixelFormat::BGR;
    case AV_PIX_FMT_BGRA: return FramePixelFormat::BGRA;
    default: return FramePixelFormat::YUV420;
    }
}
} // namespace
...
std::unique_ptr< DecoderStatus > initializeStatus (
    ...
    if (subt) {
        ...
        auto subtStream = av_find_best_stream (
    ...
    if (status->subtitleStream == nullptr) {
        // We don't have an external subtitle stream or loading from that file
        // has failed
        ...
        for (uint32_t i = 0; i < ctx->nb_streams; ++i) {
            ...
            if (!(pars->codec_type == AVMEDIA_TYPE_SUBTITLE)) { continue; }
#endif
            AVDictionaryEntry* tag = nullptr;
            if ((tag = av_dict_get (
                     test_stream->metadata, "language", nullptr, 0)) ==
                nullptr) {
                continue;
            }
    ...
    err = avcodec_parameters_to_context (
        status->videoCodecCtx, status->videoStream->codecpar);
    if (err < 0) {
        char errorStr[512];
    ...
    if (audioStream >= 0) {
        ...
        err = avcodec_parameters_to_context (
            status->audioCodecCtx, status->audioStream->codecpar);
        if (err < 0) {
            char errorStr[512];
            av_strerror (err, errorStr, sizeof (errorStr));
            mprintf (
        ...
        err =
            avcodec_open2 (status->audioCodecCtx, status->audioCodec, nullptr);
        if (err < 0) {
            char errorStr[512];
            av_strerror (err, errorStr, sizeof (errorStr));
            mprintf (
    ...
    if (status->subtitleStream != nullptr) {
        // Not really sure if this makes sense for subtitles but it shouldn't
        // break anything...
        status->subtitleCodecPars =
            getCodecParameters (status->subtitleStream);

        status->subtitleCodec =
            avcodec_find_decoder (status->subtitleCodecPars.codec_id);

#if LIBAVCODEC_VERSION_INT > AV_VERSION_INT(57, 24, 255)
        status->subtitleCodecCtx =
            avcodec_alloc_context3 (status->subtitleCodec);

        err = avcodec_parameters_to_context (
            status->subtitleCodecCtx, status->subtitleStream->codecpar);
        if (err < 0) {
            char errorStr[512];
            av_strerror (err, errorStr, sizeof (errorStr));
            mprintf (
        ...
        err = avcodec_open2 (
            status->subtitleCodecCtx, status->subtitleCodec, nullptr);
        if (err < 0) {
            char errorStr[512];
            av_strerror (err, errorStr, sizeof (errorStr));
            mprintf (
...
bool FFMPEGDecoder::initialize (
    const std::string& fileName, const PlaybackProperties& properties) {
    std::string movieName = fileName;
    // First make the file name lower case
    std::transform (
        movieName.begin (), movieName.end (), movieName.begin (), ::tolower);

    // Then remove the extension
    size_t dotPos = movieName.find ('.');
    if (dotPos != std::string::npos) { movieName.resize (dotPos); }
    ...
    initializeQueues (
        static_cast< size_t > (
            ceil (getFrameRate (status->videoStream, status->videoCodecCtx))) *
        2);
...
MovieProperties FFMPEGDecoder::getProperties () const {
    ...
    props.size.height =
        static_cast< size_t > (m_status->videoCodecPars.height);

    props.fps = static_cast< float > (
        getFrameRate (m_status->videoStream, m_status->videoCodecCtx));

    props.pixelFormat = getPixelFormat (
        getConversionFormat (m_status->videoCodecPars.pixel_format));

    return props;
}

void FFMPEGDecoder::startDecoding () {
    std::unique_ptr< VideoDecoder > videoDecoder (new VideoDecoder (
        m_status.get (),
        getConversionFormat (m_status->videoCodecPars.pixel_format)));
    ...
    if (hasExternalSubtitle ()) {
        // Start a new thread for the subtitles since otherwise we would need
        // to make sure that neither of the input streames starves while the
        // other waits for more space in the queues
        subtitle_thread.reset (new std::thread ([this, &subtitleDecoder]() {
            runSubtitleDecoder (subtitleDecoder.get ());
        }));
    }
    ...
    do {
        if (m_properties.looping) {
            // Only seek if we are looping the movie. The MVE code doesn't
            // support seeking to this function may not be called there
        ...
        if (audioDecoder) { audioDecoder->flushBuffers (); }
        if (subtitleDecoder) { subtitleDecoder->flushBuffers (); }
        ...
        while (isDecoding ()) {
            ...
            if (packet.stream_index == m_status->videoStreamIndex) {
                ...
                audioDecoder &&
                packet.stream_index == m_status->audioStreamIndex) {
                ...
                subtitleDecoder &&
                packet.stream_index == m_status->subtitleStreamIndex) {
        ...
        if (isDecoding ()) {
            // If we are still alive then read the last frames from the
            // decoders
    ...
    // Stop the decoder first to let the subtitle thread know that it shouldn't
    // continue
    stopDecoder ();

    if (subtitle_thread) { subtitle_thread->join (); }
}
...
bool FFMPEGDecoder::hasSubtitles () const {
    return m_status->subtitleStream != nullptr;
}

bool FFMPEGDecoder::hasExternalSubtitle () const {
    return m_status->externalSubtitles;
}

void FFMPEGDecoder::close () {
    if (m_status) { m_status = nullptr; }
    ...
    if (m_subtitleInput) { m_subtitleInput = nullptr; }
}
