...
SubtitleDecoder::SubtitleDecoder (cutscene::ffmpeg::DecoderStatus* status)
...
void SubtitleDecoder::decodePacket (AVPacket* packet) {
    ...
    auto result = avcodec_decode_subtitle2 (
        m_status->subtitleCodecCtx, &subtitle, &finishedFrame, packet);
...
void SubtitleDecoder::finishDecoding () {
    ...
    while (true) {
        int finishedFrame = 1;
        auto err = avcodec_decode_subtitle2 (
            m_status->subtitleCodecCtx, &subtitle, &finishedFrame,
            &nullPacket);
...
void SubtitleDecoder::pushSubtitleFrame (
    AVPacket* packet, AVSubtitle* subtitle) {
    if (subtitle->format != 1) {
        // Non-text subtitles are not supported yet.
        WARNINGF (LOCATION,"FFmpeg: Detected a non-text subtitle! This is not supported yet!");
        return;
    }
    if (subtitle->num_rects < 1) { return; }

    int64_t pts = subtitle->pts;
    if (pts == AV_NOPTS_VALUE) { pts = packet->pts; }
    ...
    if (subtitle->end_display_time == 0 &&
        m_status->subtitleStream->time_base.num != 0) {
        end_time = packet_time +
                   getFrameTime (
                       packet->duration, m_status->subtitleStream->time_base);
    }
    ...
    if (subtitle_rect->type == SUBTITLE_BITMAP) {
        ...
        // We are not interested in any of the other information contained in
        // the ASS line but we still need to figure out where our text starts
        ...
        // This + 1 is safe since comma_pos points to a valid character so the
        // next character is at worst the end of the string
        processed_text = ass_text.substr (comma_pos + 1);

        // ASS has a special sequence for new lines that needs to be converted
        // to an actual new line
        ...
        WARNINGF (LOCATION, "FFmpeg: Detected unknown subtitle name in movie!");
        return;
    }

    // Remove \r from string. Some subtitles use \r\n for line breaks and we
    // only support \n
