...
findDecoder (const std::string& name, const PlaybackProperties& properties) {
    {
        std::unique_ptr< Decoder > ffmpeg (
            new ::cutscene::ffmpeg::FFMPEGDecoder ());
        if (ffmpeg->initialize (name, properties)) { return ffmpeg; }
    }
...
void audioPlaybackInit (PlayerState* state) {
    ...
    OpenAL_ErrorCheck (
        alGenBuffers (
            static_cast< ALsizei > (state->audioBuffers.size ()),
            state->audioBuffers.data ()),
        return );
    ...
    while (currentTime >= state->nextFrame->frameTime) {
        if (state->currentFrame) {
            if (state->nextFrame->frameTime < state->currentFrame->frameTime) {
                // We reached the end of the movie and just looped to the
                // beginning. Now we need to reset the playback time
...
void processSubtitleData (PlayerState* state) {
    ...
    if (state->currentSubtitle &&
        playbackGetTime (state) <= state->currentSubtitle->displayEndTime) {
        // We currently have a frame and it it still being displayed so there
    ...
    while (!state->queued_subtitles.empty ()) {
        // Take a look at the first entry and check if it's time to display it
        // yet
        ...
        if (playbackGetTime (state) >= nextSubtitle->displayStartTime) {
            ...
            // The next subtitle should not be displayed yet so we are done
            // here
            break;
        }
    }
...
bool processAudioData (PlayerState* state) {
    ...
    if (!state->hasAudio) {
        if (state->decoder->hasAudio ()) {
            // Even if we don't play the sound we still need to remove it from
            // the queue
    ...
    ALint processed = 0;
    OpenAL_ErrorCheck (
        alGetSourcei (state->audioSid, AL_BUFFERS_PROCESSED, &processed),
        return false);
    ...
    for (int i = 0; i < processed; ++i) {
        ALuint buffer;
        OpenAL_ErrorPrint (
            alSourceUnqueueBuffers (state->audioSid, 1, &buffer));
        state->unqueuedAudioBuffers.push (buffer);
    }

    AudioFramePtr audioData;

    while (!state->unqueuedAudioBuffers.empty () &&
           state->decoder->tryPopAudioData (audioData)) {
        auto buffer = state->unqueuedAudioBuffers.front ();
        state->unqueuedAudioBuffers.pop ();

        ALenum format =
            (audioData->channels == 2) ? AL_FORMAT_STEREO16 : AL_FORMAT_MONO16;

        OpenAL_ErrorCheck (
            alBufferData (
                buffer, format, audioData->audioData.data (),
                static_cast< ALsizei > (
                    audioData->audioData.size () * sizeof (short)),
                static_cast< ALsizei > (audioData->rate)),
            return false);

        OpenAL_ErrorCheck (
            alSourceQueueBuffers (state->audioSid, 1, &buffer), return false);
    }

    ALint status = 0, queued = 0;
    OpenAL_ErrorCheck (
        alGetSourcei (state->audioSid, AL_SOURCE_STATE, &status),
        return false);

    OpenAL_ErrorCheck (
        alGetSourcei (state->audioSid, AL_BUFFERS_QUEUED, &queued),
        return false);
...
void audioPlaybackClose (PlayerState* state) {
    ...
    OpenAL_ErrorPrint (
        alGetSourcei (state->audioSid, AL_BUFFERS_PROCESSED, &p));
    OpenAL_ErrorPrint (alSourceUnqueueBuffers (
        state->audioSid, p, state->audioBuffers.data ()));
    OpenAL_ErrorPrint (alDeleteSources (1, &state->audioSid));

    for (auto buffer : state->audioBuffers) {
        // make sure that the buffer is real before trying to delete, it could
        // crash for some otherwise
...
bool shouldBeginPlayback (Decoder* decoder) {
    ...
    // Also only wait while the decoder is still working. Otherwise we will
    // wait indefinitely if the video is very short.
...
Player::Player (
    ...
    m_decoderThread.reset (
        new std::thread (std::bind (&Player::decoderThread, this)));
    ...
    if (properties.with_audio) { audioPlaybackInit (&m_state); }
}

Player::~Player () {
    // If video is initialized it means that stopPlayback has not been called
    // yet
    if (m_state.videoInited) { stopPlayback (); }
...
bool Player::processDecoderData () {
    if (!m_state.playbackHasBegun) {
        ...
        if (!shouldBeginPlayback (m_state.decoder)) { return true; }
    ...
    // Set the playing flag if the decoder is still active or there is still
    // data available
    auto decoding = m_decoder->isDecoding ();

    // Audio is pending if there is data left in the queue or if OpenAL is
    // still playing audio
    auto pendingAudio =
...
bool Player::update (uint64_t diff_time_micro) {
    if (m_state.playbackHasBegun) { m_state.playback_time += diff_time_micro; }
...
std::unique_ptr< Player > Player::newPlayer (
    ...
    return std::unique_ptr< Player > (
        new Player (std::move (decoder), properties));
}
const PlayerState& Player::getInternalState () const { return m_state; }
...
void Player::draw (float x1, float y1, float x2, float y2) {
    if (!m_state.currentFrame) { return; }
...
std::string Player::getCurrentSubtitle () {
    if (!m_state.currentSubtitle) { return ""; }
    else {
        return m_state.currentSubtitle->text;
    }
}
bool Player::isPlaybackReady () const {
    return m_state.playbackHasBegun || shouldBeginPlayback (m_decoder.get ());
}
} // namespace cutscene
