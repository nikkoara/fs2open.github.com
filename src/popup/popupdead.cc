...
int Popupdead_region_coords[GR_NUM_RESOLUTIONS][POPUPDEAD_NUM_CHOICES_MAX][4] =
    {
        {
            // GR_640
            { 464, 389, 497, 403 }, // upper right pixel of text, lower right
                                    // pixel of button (for tiny popup)
            { 464, 413, 497, 427 },
            { 464, 435, 497, 446 },
            { 464, 457, 497, 466 },
        },
        {
            // GR_1024
            { 745, 627, 809, 664 },
            { 745, 663, 809, 700 }, // upper right pixel of text, lower right
                                    // pixel of button (for tiny popup)
            { 745, 699, 809, 736 },
            { 745, 735, 809, 772 },
        },
...
int Popupdead_background_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                             131, 363 },
                                                           { // GR_1024
                                                             205, 581 } };

const char* Popupdead_button_filenames[GR_NUM_RESOLUTIONS]
                                      [POPUPDEAD_NUM_CHOICES_MAX] = {
                                          {
                                              // GR_640
...
int Popupdead_skip_message_y[GR_NUM_RESOLUTIONS] = { 96, // GR_640
                                                     160 };
...
int Popupdead_default_choice;  // What the default choice is (ie activated when
                               // Enter pressed)
...
void popupdead_start () {
    ...
    if (Popupdead_active) { return; }

    // increment number of deaths
    Player->failures_this_session++;

    // create base window
    Popupdead_window.create (
        Popupdead_background_coords[gr_screen.res][0],
        Popupdead_background_coords[gr_screen.res][1], 1, 1, 0);
    Popupdead_window.set_foreground_bmap (
        Popupdead_background_filename[gr_screen.res]);
    ...
    for (i = 0; i < Popupdead_num_choices; i++) {
        ...
        b->create (
            &Popupdead_window, "",
            Popupdead_button_coords[gr_screen.res][i][0],
        ...
        b->create (
            &Popupdead_window, "", lx,
            Popupdead_region_coords[gr_screen.res][i][1],
            Popupdead_region_coords[gr_screen.res][i][2] - lx,
            Popupdead_region_coords[gr_screen.res][i][3] -
                Popupdead_region_coords[gr_screen.res][i][1],
            0, 1);
        b->hide ();
    }
...
int popupdead_process_keys (int k) {
    ...
    case KEY_ESC:
        if (Popupdead_skip_active) {
            ...
            return 1; // do nothing here for now - 1 mimics a "return to flight
                      // deck"
        }
        break;
    ...
    case KEY_PAUSE: game_process_pause_key (); break;
...
void popupdead_force_draw_buttons () {
    ...
    if ((!mouse_is_highlighting) && (Popupdead_num_choices > 1)) {
        for (i = 0; i < Popupdead_num_choices; i++) {
            ...
            if (i == Popupdead_default_choice) { b->draw_forced (1); }
        }
    }
}
...
int popupdead_do_frame (float /*frametime*/) {
    ...
    if (!Popupdead_active) { return -1; }

    // maybe show skip mission popup
    if ((!Popupdead_skip_already_shown) && (Player->show_skip_popup) &&
    ...
    if (choice >= 0) {
        ...
        if (Game_mode & GM_NORMAL) { Popupdead_choice = choice; }
            ...
            case POPUPDEAD_OBS_QUIT:
                ...
                if ((Popupdead_multi_type == POPUPDEAD_OBS_QUIT) &&
                    (choice == 1))
            ...
            case POPUPDEAD_RESPAWN_QUIT:
                ...
                if ((Popupdead_multi_type == POPUPDEAD_RESPAWN_QUIT) &&
                    (choice == 1))
                    Popupdead_choice = POPUPDEAD_DO_MAIN_HALL;
                break;
...
int popupdead_is_active () { return Popupdead_active; }
