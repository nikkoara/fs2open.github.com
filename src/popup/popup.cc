...
const char *popup_get_button_filename(popup_info *pi, int i, int flags)
{
        ...
        // them.
        if ((!strcasecmp(pi->button_text[i], POPUP_OK + 1) || !strcasecmp(pi->button_text[i], POPUP_YES + 1)) && !(flags & PF_NO_SPECIAL_BUTTONS)) {
                return Popup_button_filenames[gr_screen.res][is_tiny][BUTTON_POSITIVE];
        }

        if ((!strcasecmp(pi->button_text[i], POPUP_CANCEL + 1) || !strcasecmp(pi->button_text[i], POPUP_NO + 1)) && !(flags & PF_NO_SPECIAL_BUTTONS)) {
...
int popup_init(popup_info *pi, int flags)
{
        ...
        for (i = 0; i < pi->nchoices; i++) {
                ...
                if ((pi->nchoices == 1) && (flags & PF_USE_AFFIRMATIVE_ICON)) {
                        b->create(
                                ...
                                Button_regions[gr_screen.res][i + 1][3] - Button_regions[gr_screen.res][i + 1][1],
                                0, 1);
                } else {
                        b->create(
                                &Popup_window, "", lx, Button_regions[gr_screen.res][i][1],
                                Button_regions[gr_screen.res][i][2] - lx,
                                Button_regions[gr_screen.res][i][3] - Button_regions[gr_screen.res][i][1],
                                0, 1);
                }
        ...
        if (flags & PF_INPUT) {
                Popup_input.create(
                        ...
                        UI_INPUTBOX_FLAG_INVIS | UI_INPUTBOX_FLAG_ESC_CLR | UI_INPUTBOX_FLAG_ESC_FOC | UI_INPUTBOX_FLAG_KEYTHRU | UI_INPUTBOX_FLAG_TEXT_CEN);
...
int popup_calc_starting_y(popup_info *pi, int flags)
{
        ...
        sy = (int)std::lround(
                (Popup_text_coords[gr_screen.res][1] + Popup_text_coords[gr_screen.res][3] / 2.0f) - total_h / 2.0f);
