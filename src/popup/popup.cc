...
typedef struct popup_info {
    ...
    int shortcut_index[POPUP_MAX_CHOICES]; // what char should be underlines
                                           // for shortcut
    ...
    int nlines;
    char msg_lines[POPUP_MAX_LINES]
                  [POPUP_MAX_LINE_CHARS];   // lines of text in popup
    char input_text[POPUP_INPUT_MAX_CHARS]; // input box text (if this is an
                                            // inputbox popup)
    int max_input_text_len;
    int web_cursor_flag[POPUP_MAX_CHOICES]; // flag for using web cursor over
                                            // button
} popup_info;
...
UI_BUTTON Popup_button_regions[POPUP_MAX_CHOICES]; // fake buttons used for
                                                   // mouse detection over text
UI_INPUTBOX Popup_input;                           // input box for the popup
UI_SLIDER2 Popup_slider; // if we have more text in the popup than can be
                         // displayed at once

// extents for message portion of popup
int Popup_text_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                   137, 106, 343, 113 },
                                                 { // GR_1024
                                                   219, 169, 558, 182 } };
...
int Popup_input_y_offset[GR_NUM_RESOLUTIONS] = { 40, 40 };

// offset from the first y text line value to start drawing text
int Popup_input_text_y_offset[GR_NUM_RESOLUTIONS] = { 30, 30 };
...
static int Popup_should_die =
    0; // popup should quit during the next iteration of its loop
...
static int Button_regions[GR_NUM_RESOLUTIONS][3][4] = {
    { // GR_640
      { 464, 232, 510,
        250 }, // upper right pixel of text, lower right pixel of button
      { 464, 262, 510, 279 },
      { 464, 292, 510, 308 } },
    { // GR_1024
      { 752, 373, 806,
        406 }, // upper right pixel of text, lower right pixel of button
      { 752, 421, 806, 461 },
      { 752, 468, 806, 506 } }
};
...
int Popup_default_choice; // which choice is highlighted (ie what gets choosen
...
void popup_play_default_change_sound (popup_info* pi) {
    if (pi->nchoices > 1) {
        ...
        for (i = 0; i < pi->nchoices; i++) {
            ...
            if (b->button_hilighted ()) { mouse_over = 1; }
        }

        if (!mouse_over) { gamesnd_play_iface (InterfaceSounds::USER_SELECT); }
    }
}
...
int popup_process_keys (popup_info* pi, int k, int flags) {
    ...
    case KEY_ESC:
        ...
        if (!(flags & PF_IGNORE_ESC)) { return POPUP_ABORT; }
        break;
    ...
    case KEY_TAB:
        ...
        if (Popup_default_choice >= pi->nchoices) { Popup_default_choice = 0; }
        break;
...
void popup_split_lines (popup_info* pi, int flags) {
    ...
    if (flags & PF_BODY_BIG) { font::set_font (font::FONT2); }
...
const char* popup_get_button_filename (popup_info* pi, int i, int flags) {
    ...
    case 1:
        if ((flags & PF_USE_AFFIRMATIVE_ICON) &&
            !(flags & PF_NO_SPECIAL_BUTTONS)) {
            fname = Popup_button_filenames[gr_screen.res][is_tiny]
                                          [BUTTON_POSITIVE];
        }
        else if (
            flags & PF_USE_NEGATIVE_ICON && !(flags & PF_NO_SPECIAL_BUTTONS)) {
            fname = Popup_button_filenames[gr_screen.res][is_tiny]
                                          [BUTTON_NEGATIVE];
        }
        else {
            fname = Popup_button_filenames[gr_screen.res][is_tiny]
                                          [BUTTON_GENERIC_FIRST];
        }
        break;
...
int popup_init (popup_info* pi, int flags) {
    ...
    if (pi->nchoices == 0) { pbg = &Popup_background[gr_screen.res][0]; }
    else {
        pbg = &Popup_background[gr_screen.res][pi->nchoices - 1];
    }

    // anytime in single player, and multiplayer, not in mission, go ahead and
    // stop time
    ...
    // create base window
    Popup_window.create (
        pbg->coords[0], pbg->coords[1],
        Popup_text_coords[gr_screen.res][2] + 100,
    ...
    for (i = 0; i < pi->nchoices; i++) {
        ...
        if ((pi->nchoices == 1) && (flags & PF_USE_AFFIRMATIVE_ICON)) {
            b->create (
                &Popup_window, "", Button_coords[gr_screen.res][i + 1][0],
                Button_coords[gr_screen.res][i + 1][1], 30, 25, 0, 1);
        }
        else {
            b->create (
                &Popup_window, "", Button_coords[gr_screen.res][i][0],
                Button_coords[gr_screen.res][i][1], 30, 25, 0, 1);
        }

        fname = popup_get_button_filename (pi, i, flags);
        b->set_bmaps (fname, 3, 0);
        b->set_highlight_action (common_play_highlight_sound);
        if (pi->keypress[i] >= 0) { b->set_hotkey (pi->keypress[i]); }

        // create invisible buttons to detect mouse presses... can't use mask
        // since button region is dynamically sized
        int lx, w, h;
        ...
        if ((pi->nchoices == 1) && (flags & PF_USE_AFFIRMATIVE_ICON)) {
            b->create (
                &Popup_window, "", lx, Button_regions[gr_screen.res][i + 1][1],
                Button_regions[gr_screen.res][i + 1][2] - lx,
                Button_regions[gr_screen.res][i + 1][3] -
                    Button_regions[gr_screen.res][i + 1][1],
                0, 1);
        }
        else {
            b->create (
                &Popup_window, "", lx, Button_regions[gr_screen.res][i][1],
                Button_regions[gr_screen.res][i][2] - lx,
                Button_regions[gr_screen.res][i][3] -
                    Button_regions[gr_screen.res][i][1],
                0, 1);
        }
    ...
    if (flags & PF_INPUT) {
        Popup_input.create (
            &Popup_window, Popup_text_coords[gr_screen.res][0],
            pbg->coords[1] + Popup_input_y_offset[gr_screen.res],
            Popup_text_coords[gr_screen.res][2], pi->max_input_text_len, "",
            UI_INPUTBOX_FLAG_INVIS | UI_INPUTBOX_FLAG_ESC_CLR |
                UI_INPUTBOX_FLAG_ESC_FOC | UI_INPUTBOX_FLAG_KEYTHRU |
                UI_INPUTBOX_FLAG_TEXT_CEN);
    ...
    if (flags & PF_RUN_STATE) { Popup_running_state = 1; }
    else {
    ...
    Popup_slider.create (
        &Popup_window, Popup_slider_coords[gr_screen.res][0],
        Popup_slider_coords[gr_screen.res][1],
        Popup_slider_coords[gr_screen.res][2],
        Popup_slider_coords[gr_screen.res][3],
        pi->nlines > Popup_max_display[gr_screen.res]
            ? pi->nlines - Popup_max_display[gr_screen.res]
            : 0,
        Popup_slider_name[gr_screen.res], popup_slider_bogus,
        popup_slider_bogus, NULL);
...
void popup_close (popup_info* pi, int screen_id) {
    ...
    gamesnd_play_iface (
        InterfaceSounds::POPUP_DISAPPEAR); // play sound when popup disappears
    ...
    if (screen_id >= 0) { gr_free_screen (screen_id); }
    Popup_window.destroy ();
    anim_ignore_next_frametime (); // to avoid skips in animation since next
                                   // frametime is saturated
    game_flush ();
...
void popup_draw_title (int sy, char* line, int flags) {
    ...
    if (flags & PF_TITLE_BIG) { font::set_font (font::FONT2); }
    else {
        font::set_font (font::FONT1);
    }
...
int popup_calc_starting_index (popup_info* pi) {
    ...
    if (pi->nlines <= Popup_max_display[gr_screen.res]) { return 0; }
...
int popup_calc_starting_y (popup_info* pi, int flags) {
    int sy, total_h = 0;
    int num_lines = pi->nlines > Popup_max_display[gr_screen.res]
                        ? Popup_max_display[gr_screen.res]
                        : pi->nlines;
    ...
    sy = (int)std::lround (
        (Popup_text_coords[gr_screen.res][1] +
         Popup_text_coords[gr_screen.res][3] / 2.0f) -
        total_h / 2.0f);

    // if this is an input style box, add in some y
    if (flags & PF_INPUT) { sy += Popup_input_text_y_offset[gr_screen.res]; }
...
void popup_draw_msg_text (popup_info* pi, int flags) {
    ...
    for (i = line_index; i < pi->nlines; i++, line_count++) {
        ...
        if (line_count >= Popup_max_display[gr_screen.res]) { break; }
    ...
    if (i < pi->nlines) {
        ...
        gr_string (
            Title_coords[gr_screen.res][4],
            sy + (Popup_max_display[gr_screen.res]) * h, XSTR ("More", 459),
            GR_RESIZE_MENU);
    }
...
void popup_force_draw_buttons (popup_info* pi) {
    ...
    for (i = 0; i < pi->nchoices; i++) {
        ...
        if ((b->button_hilighted ()) ||
            (br->button_hilighted () && !b->button_down ())) {
            Popup_default_choice = i;
            mouse_is_highlighting = 1;
            b->draw_forced (1);
        }
    ...
    // Only if mouse is not highlighting an option, let the default choice be
    // drawn highlighted
    if ((!mouse_is_highlighting) && (pi->nchoices > 1)) {
        for (i = 0; i < pi->nchoices; i++) {
            b = &Popup_buttons[i];
            // highlight the default choice
            if (i == Popup_default_choice) { b->draw_forced (1); }
        }
    }
}
...
int popup_do (popup_info* pi, int flags) {
    ...
    if (popup_init (pi, flags) == -1) { return -1; }
    ...
    while (!done) {
        ...
        // if we're flagged as should be running the state underneath, then do
        // so
        if (flags & PF_RUN_STATE) { game_do_state (gameseq_get_state ()); }
        ...
        if (choice != POPUP_NOCHANGE) { done = 1; }
        ...
        if (flags & PF_INPUT) { Popup_input.get_text (pi->input_text); }
    ...
    gr_set_screen_scale (
        old_max_w_unscaled, old_max_h_unscaled, old_max_w_unscaled_zoomed,
        old_max_h_unscaled_zoomed);
...
int popup (int flags, int nchoices, ...) {
    ...
    vsnprintf (
        Popup_info.raw_text, sizeof (Popup_info.raw_text) - 1, format, args);
    va_end (args);
    Popup_info.raw_text[sizeof (Popup_info.raw_text) - 1] = '\0';

    gamesnd_play_iface (
        InterfaceSounds::POPUP_APPEAR); // play sound when popup appears
    ...
    switch (choice) {
    case POPUP_ABORT: return -1;
    default: return choice;
    } // end switch
}

// determine if a popup is being drawn
int popup_active () { return Popup_is_active; }

// This function displays a popup message box and every frame it checks the
// condition() function which is passed in as an argument. If the condition()
// returns TRUE, the message box ends itself.  This function returns whatever
// the condition() did if the condition() occurred, and FALSE if the cancel
// button was pressed.
...
char* popup_input (int flags, const char* caption, int max_output_len) {
    ...
    if (popup_do (&Popup_info, Popup_flags) == POPUP_ABORT) { return NULL; }

    // otherwise return the
    return Popup_info.input_text;
}

int popup_running_state () { return Popup_running_state; }

// kill any active popup, forcing it to return -1 (similar to ESC)
void popup_kill_any_active () {
    if (Popup_is_active) { Popup_should_die = 1; }
}
...
// If we're running on demo data, certain menus can't be displayed; this
// message will get shown instead.
