...
bool generic_bitmap_exists(const char *filename)
{
        return cf_exists_full_ext(
                       filename, CF_TYPE_ANY, BM_NUM_TYPES, bm_ext_list)
               != 0;
}
...
{
        return cf_exists_full_ext(
                       filename, CF_TYPE_ANY, BM_ANI_NUM_TYPES, bm_ani_ext_list)
               != 0;
}
        ...
        generic_anim *ga, float frametime)
{
                ...
                if (ga->direction & GENERIC_ANIM_DIRECTION_BACKWARDS) {
                        ...
                        if (ga->keyframe && (ga->anim_time >= keytime)) {
                                ...
                                if ((ga->direction & GENERIC_ANIM_DIRECTION_NOLOOP) && ga->anim_time <= 0.0) {
                                        ...
                                        while (ga->anim_time <= 0.0)
                                                ga->anim_time += ga->total_time; // make sure we're always positive,
                        ...
                        if (ga->anim_time >= ga->total_time) {
                                if (ga->direction & GENERIC_ANIM_DIRECTION_NOLOOP) {
                                        ga->anim_time = ga->total_time - 0.001f; // stop on last frame when playing - if it's
        ...
        if (ga->num_frames > 0) {
                ...
                ga->current_frame += int(ga->anim_time * ga->num_frames / ga->total_time);
                // sanity check
        ...
        generic_anim *ga, float frametime, float alpha)
{
        ...
        if (ga->num_frames > 0) {
                ...
                if (ga->direction & GENERIC_ANIM_DIRECTION_BACKWARDS) {
                        if (ga->anim_time <= ga->png.previous_frame_time - ga->png.anim->frame.delay && ga->png.anim->current_frame > 0) {
                                ga->png.previous_frame_time -= ga->png.anim->frame.delay;
                                ga->current_frame--;
                        }
                } else {
                        if (ga->anim_time >= ga->png.previous_frame_time + ga->png.anim->frame.delay && ga->png.anim->current_frame < ga->png.anim->nframes - 1) {
                                ga->png.previous_frame_time += ga->png.anim->frame.delay;
                                ga->current_frame++;
                        }
                }
