...
void geometry_batcher::allocate (int quad, int n_tri) {
    ...
    if (quad > 0) { to_alloc += (quad * 6); }

    // a single triangle has a mere 3 verts
    if (n_tri > 0) { to_alloc += (n_tri * 3); }
...
3----2
*/
void geometry_batcher::draw_bitmap (
    vertex* pnt, int orient, float rad, float depth) {
    float radius = rad;
...
void geometry_batcher::draw_bitmap (
    vertex* pnt, float rad, float angle, float depth) {
    float radius = rad;
    rad *= 1.41421356f; // 1/0.707, becase these are the points of a square or
                        // width and height rad
...
void geometry_batcher::draw_beam (
    vec3d* start, vec3d* end, float width, float intensity, float offset) {
    vec3d p[4];
    vertex* P = &vert[n_to_render * 3];
    float* R = &radius_list[n_to_render * 3];
...
float geometry_batcher::draw_laser (
    vec3d* p0, float width1, vec3d* p1, float width2, int r, int g, int b) {
    width1 *= 0.5f;
    width2 *= 0.5f;
...
void geometry_shader_batcher::render_buffer (
    int /*buffer_handle*/, int /*flags*/) {}

void geometry_shader_batcher::load_buffer (
    particle_pnt* buffer, size_t* n_verts) {
...
void geometry_shader_batcher::draw_bitmap (
    vertex* position, int orient, float rad, float depth) {
    rad *= 1.41421356f; // 1/0.707, becase these are the points of a square or
                        // width and height rad
...
struct batch_item {
    batch_item ()
        : texture (-1), tmap_flags (0), alpha (1.0f), laser (false) {}
...
int batch_add_bitmap (
    ...
    if (tmap_flags & TMAP_FLAG_SOFT_QUAD && !Cmdline_softparticles) {
        // don't render this as a soft particle if we don't support soft
        // particles
        tmap_flags &= ~(TMAP_FLAG_SOFT_QUAD);
    }

    if (Cmdline_softparticles && !Cmdline_no_geo_sdr_effects &&
        (tmap_flags & TMAP_FLAG_VERTEX_GEN)) {
        geometry_batch_add_bitmap (
            texture, tmap_flags, pnt, orient, rad, alpha, depth);
        return 0;
    }
...
int geometry_batch_add_bitmap (
    ...
    std::map< int, g_sdr_batch_item >::iterator it =
        geometry_shader_map.find (texture);
...
int batch_add_polygon (
    int texture, int tmap_flags, vec3d* pos, matrix* orient, float width,
...
void batch_render_lasers (int buffer_handle) {
    for (std::map< int, batch_item >::iterator bi = geometry_map.begin ();
         bi != geometry_map.end (); ++bi) {
        if (!bi->second.laser) continue;
        ...
        gr_set_bitmap (
            ...
            0.99999f);
        if (buffer_handle >= 0) {
            bi->second.batch.render_buffer (
                buffer_handle, TMAP_FLAG_TEXTURED | TMAP_FLAG_XPARENT |
                                   TMAP_HTL_3D_UNLIT | TMAP_FLAG_RGB |
                                   TMAP_FLAG_GOURAUD | TMAP_FLAG_CORRECT |
                                   TMAP_FLAG_EMISSIVE);
        }
        else {
            bi->second.batch.render (
...
void batch_render_geometry_map_bitmaps (int buffer_handle) {
    for (std::map< int, batch_item >::iterator bi = geometry_map.begin ();
        ...
        if (buffer_handle >= 0) {
            bi->second.batch.render_buffer (
                buffer_handle, bi->second.tmap_flags);
        }
        else {
...
void batch_render_geometry_shader_map_bitmaps (int buffer_handle) {
    for (std::map< int, g_sdr_batch_item >::iterator bi =
             geometry_shader_map.begin ();
         bi != geometry_shader_map.end (); ++bi) {
        if (bi->second.laser) continue;
...
void batch_load_buffer_geometry_map_bitmaps (
    effect_vertex* buffer, int* n_verts) {
    for (std::map< int, batch_item >::iterator bi = geometry_map.begin ();
         bi != geometry_map.end (); ++bi) {
        if (bi->second.laser) continue;
...
void geometry_batch_render (int stream_buffer) {
    ...
    batch_load_buffer_geometry_shader_map_bitmaps (
        (particle_pnt*)Batch_geometry_buffer, &n_verts);

    gr_update_buffer_data (
        stream_buffer, Batch_geometry_buffer_size, Batch_geometry_buffer);
...
void batch_render_all (int stream_buffer) {
    if (stream_buffer >= 0) {
        ...
        batch_load_buffer_geometry_map_bitmaps (
            (effect_vertex*)Batch_buffer, &n_verts);
        batch_load_buffer_distortion_map_bitmaps (
            (effect_vertex*)Batch_buffer, &n_verts);
...
int distortion_add_bitmap_rotated (
    int texture, int tmap_flags, vertex* pnt, float angle, float rad,
    float alpha, float depth) {
    if (texture < 0) {
        ASSERT (0);
...
void batch_render_distortion_map_bitmaps (int buffer_handle) {
    for (std::map< int, batch_item >::iterator bi = distortion_map.begin ();
         bi != distortion_map.end (); ++bi) {
        if (bi->second.laser) continue;
...
void batch_load_buffer_distortion_map_bitmaps (
    effect_vertex* buffer, int* n_verts) {
    for (std::map< int, batch_item >::iterator bi = distortion_map.begin ();
         bi != distortion_map.end (); ++bi) {
        if (bi->second.laser) continue;
...
size_t geometry_batch_get_size () {
    ...
    for (bi = geometry_shader_map.begin (); bi != geometry_shader_map.end ();
         ++bi) {
        n_to_render += bi->second.batch.need_to_render ();
    }
