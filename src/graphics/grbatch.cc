             ...
             bi != geometry_map.end(); ++bi) {
                ...
                if (buffer_handle >= 0) {
                        bi->second.batch.render_buffer(
                                buffer_handle, TMAP_FLAG_TEXTURED | TMAP_FLAG_XPARENT | TMAP_HTL_3D_UNLIT | TMAP_FLAG_RGB | TMAP_FLAG_GOURAUD | TMAP_FLAG_CORRECT | TMAP_FLAG_EMISSIVE);
                } else {
                        bi->second.batch.render(
                                TMAP_FLAG_TEXTURED | TMAP_FLAG_XPARENT | TMAP_HTL_3D_UNLIT | TMAP_FLAG_RGB | TMAP_FLAG_GOURAUD | TMAP_FLAG_CORRECT | TMAP_FLAG_EMISSIVE);
                }
        }
}
...
{
        for (std::map< int, g_sdr_batch_item >::iterator bi = geometry_shader_map.begin();
