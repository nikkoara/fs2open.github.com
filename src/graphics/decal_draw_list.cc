...
uint32_t BOX_FACES[] = {
    7, 3, 4, 3, 0, 4, 2, 6, 1, 6, 5, 1, 7, 6, 3, 6, 2, 3,
    0, 1, 4, 1, 5, 4, 6, 7, 4, 5, 6, 4, 3, 2, 0, 2, 1, 0,
};
...
void init_buffers () {
    ...
    box_index_buffer =
        gr_create_buffer (BufferType::Index, BufferUsageHint::Static);
...
bool decal_draw_list::sort_draws (
    ...
    auto left_diffuse_base =
        bm_get_base_frame (left.draw_mat.get_texture_map (TM_BASE_TYPE));
    auto right_diffuse_base =
        bm_get_base_frame (right.draw_mat.get_texture_map (TM_BASE_TYPE));

    if (left_diffuse_base != right_diffuse_base) {
        return left_diffuse_base < right_diffuse_base;
    }
    auto left_glow_base =
        bm_get_base_frame (left.draw_mat.get_texture_map (TM_GLOW_TYPE));
    auto right_glow_base =
        bm_get_base_frame (right.draw_mat.get_texture_map (TM_GLOW_TYPE));

    if (left_glow_base != right_glow_base) {
        return left_glow_base < right_glow_base;
    }

    auto left_normal_base =
        bm_get_base_frame (left.draw_mat.get_texture_map (TM_NORMAL_TYPE));
    auto right_normal_base =
        bm_get_base_frame (left.draw_mat.get_texture_map (TM_NORMAL_TYPE));
...
void decal_draw_list::add_decal (
    ...
    info->diffuse_index =
        diffuse_bitmap < 0 ? -1 : bm_get_array_index (diffuse_bitmap);
    info->glow_index = glow_bitmap < 0 ? -1 : bm_get_array_index (glow_bitmap);
    info->normal_index =
        normal_bitmap < 0 ? -1 : bm_get_array_index (normal_bitmap);

    decal_draw_info current_draw;
    current_draw.uniform_offset = aligner.getCurrentOffset ();

    material_set_decal (
        &current_draw.draw_mat, bm_get_base_frame (diffuse_bitmap),
        bm_get_base_frame (glow_bitmap), bm_get_base_frame (normal_bitmap));
    info->diffuse_blend_mode =
        current_draw.draw_mat.get_blend_mode (0) == ALPHA_BLEND_ADDITIVE ? 1
                                                                         : 0;
    info->glow_blend_mode =
        current_draw.draw_mat.get_blend_mode (2) == ALPHA_BLEND_ADDITIVE ? 1
                                                                         : 0;
