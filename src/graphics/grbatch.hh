...
private:
    ...
    // makes sure we have enough space in the memory buffer for the geometry we
    // are about to put into it you need to figure out how many verts are going
    // to be required
...
public:
    geometry_batcher ()
        : n_to_render (0), n_allocated (0), vert (NULL), use_radius (true),
          radius_list (NULL), buffer_offset (-1) {}
    ~geometry_batcher ();
    ...
    // allocate an additional block of memory to what we already have (this is
    // a resize function)
    ...
    // draw a beam
    void draw_beam (
        vec3d* start, vec3d* end, float width, float intensity = 1.0f,
        float offset = 0.0f);
    ...
    // draw all of the batched geometry to the back buffer and flushes the
    // cache accepts tmap flags so you can use anything you want really
    ...
    // determine if we even need to try and render this (helpful for particle
    // system)
