...
static void create_perspective_projection_matrix (
...
void gr_start_instance_matrix (const vec3d* offset, const matrix* rotation) {
    ...
    if (rotation == NULL) { rotation = &vmd_identity_matrix; }
...
void gr_set_proj_matrix (float fov, float aspect, float z_near, float z_far) {
    if (gr_screen.rendering_to_texture != -1) {
        gr_set_viewport (
            gr_screen.offset_x, gr_screen.offset_y, gr_screen.clip_width,
    ...
    if (gr_screen.rendering_to_texture != -1) {
        create_perspective_projection_matrix (
            &gr_projection_matrix, -clip_width, clip_width, clip_height,
            -clip_height, z_near, z_far);
    }
    else {
        create_perspective_projection_matrix (
            &gr_projection_matrix, -clip_width, clip_width, -clip_height,
...
void gr_end_proj_matrix () {
    ...
    // the top and bottom positions are reversed on purpose, but RTT needs them
    // the other way
    if (gr_screen.rendering_to_texture != -1) {
...
void gr_set_2d_matrix (/*int x, int y, int w, int h*/) {
    ...
    vm_matrix4_x_matrix4 (
        &gr_model_view_matrix, &gr_view_matrix, &identity_mat);
...
void gr_push_scale_matrix (const vec3d* scale_factor) {
    if ((scale_factor->xyz.x == 1) && (scale_factor->xyz.y == 1) &&
        (scale_factor->xyz.z == 1))
        return;
...
void gr_setup_viewport () {
    if (Gr_inited) {
        // This may be called by FRED before the gr system is actually
        // initialized so we need to make sure that this function call is
        // actually valid at this point.
        gr_set_viewport (0, 0, gr_screen.max_w, gr_screen.max_h);
    }
