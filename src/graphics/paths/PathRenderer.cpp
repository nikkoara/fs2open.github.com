...
NVGcolor fsColorToNVG (color* c) {
    return nvgRGBA (
        c->red, c->green, c->blue, c->is_alphacolor ? c->alpha : 255);
}
...
void PathRenderer::shutdown () { s_instance = nullptr; }
...
void PathRenderer::resetScissor () { nvgResetScissor (m_context); }

void PathRenderer::resetTransform () { nvgResetTransform (m_context); }

void PathRenderer::translate (float x, float y) {
    nvgTranslate (m_context, x, y);
}

void PathRenderer::rotate (float rad) { nvgRotate (m_context, rad); }

void PathRenderer::skewX (float rad) { nvgSkewX (m_context, rad); }

void PathRenderer::skewY (float rad) { nvgSkewY (m_context, rad); }

void PathRenderer::scale (float x, float y) { nvgScale (m_context, x, y); }

DrawPaint PathRenderer::createLinearGradient (
    float sx, float sy, float ex, float ey, color* icol, color* ocol) {
    DrawPaint p;

    p.nvg = nvgLinearGradient (
        m_context, sx, sy, ex, ey, fsColorToNVG (icol), fsColorToNVG (ocol));
...
void PathRenderer::beginPath () { nvgBeginPath (m_context); }

void PathRenderer::moveTo (float x, float y) { nvgMoveTo (m_context, x, y); }

void PathRenderer::setSolidity (Solidity solid) {
    int nvgSolid;
    switch (solid) {
    case SOLIDITY_HOLE: nvgSolid = NVG_HOLE; break;
    case SOLIDITY_SOLID: nvgSolid = NVG_SOLID; break;
    default: nvgSolid = NVG_SOLID; break;
    }

    nvgPathWinding (m_context, nvgSolid);
}

void PathRenderer::lineTo (float x, float y) { nvgLineTo (m_context, x, y); }

void PathRenderer::rectangle (float x, float y, float w, float h) {
    nvgRect (m_context, x, y, w, h);
}

void PathRenderer::roundedRectangle (
...
void PathRenderer::arc (
    float cx, float cy, float r, float a0, float a1, Direction dir) {
    int nvgDir;
    switch (dir) {
    case DIR_CCW: nvgDir = NVG_CCW; break;
    case DIR_CW: nvgDir = NVG_CW; break;
    default: nvgDir = NVG_CW; break;
    }

    nvgArc (m_context, cx, cy, r, a0, a1, nvgDir);
}

void PathRenderer::closePath () { nvgClosePath (m_context); }

int PathRenderer::createFontMem (
    const char* name, unsigned char* data, int ndata, int freeData) {
    return nvgCreateFontMem (m_context, name, data, ndata, freeData);
}

void PathRenderer::fontSize (float size) { nvgFontSize (m_context, size); }

void PathRenderer::textLetterSpacing (float spacing) {
    nvgTextLetterSpacing (m_context, spacing);
}

void PathRenderer::fontFaceId (int font) { nvgFontFaceId (m_context, font); }

float PathRenderer::text (
    float x, float y, const char* string, const char* end) {
    return nvgText (m_context, x, y, string, end);
}

float PathRenderer::textBounds (
    float x, float y, const char* string, const char* end, float* bounds) {
    return nvgTextBounds (m_context, x, y, string, end, bounds);
}

void PathRenderer::textMetrics (
...
void PathRenderer::textAlign (TextAlign align) {
    int nvgAlign = 0;
    if (align & ALIGN_LEFT) { nvgAlign |= NVG_ALIGN_LEFT; }
    if (align & ALIGN_CENTER) { nvgAlign |= NVG_ALIGN_CENTER; }
    if (align & ALIGN_RIGHT) { nvgAlign |= NVG_ALIGN_RIGHT; }

    if (align & ALIGN_TOP) { nvgAlign |= NVG_ALIGN_TOP; }
    if (align & ALIGN_MIDDLE) { nvgAlign |= NVG_ALIGN_MIDDLE; }
    if (align & ALIGN_BOTTOM) { nvgAlign |= NVG_ALIGN_BOTTOM; }
    if (align & ALIGN_BASELINE) { nvgAlign |= NVG_ALIGN_BASELINE; }

    nvgTextAlign (m_context, nvgAlign);
}

void PathRenderer::fill () { nvgFill (m_context); }

void PathRenderer::stroke () { nvgStroke (m_context); }

void PathRenderer::saveState () { nvgSave (m_context); }

void PathRenderer::resetState () { nvgReset (m_context); }

void PathRenderer::restoreState () { nvgRestore (m_context); }
} // namespace paths
} // namespace graphics
