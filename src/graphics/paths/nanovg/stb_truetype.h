...
STBTT_DEF int stbtt_PackFontRange (
    stbtt_pack_context* spc, unsigned char* fontdata, int font_index,
...
STBTT_DEF int stbtt_PackFontRangesGatherRects (
    stbtt_pack_context* spc, stbtt_fontinfo* info, stbtt_pack_range* ranges,
...
STBTT_DEF int stbtt_InitFont (
    stbtt_fontinfo* info, const unsigned char* data2, int fontstart) {
        ...
        for (i = 0; i < n; ++i) {
            ...
            if (next_move == i) {
                ...
                if (start_off) {
                    ...
                    // need to find a point on the curve where we can start,
                    // and we need to save some state for when we wraparound.
                    scx = x;
                    scy = y;
                ...
                if (!(flags & 1)) { // if it's a curve
                    ...
                    if (was_off)
                        ...
                        stbtt_setvertex (
                            &vertices[num_vertices++], STBTT_vline, x, y, 0,
                            0);
                    was_off = 0;
                }
            }
...
stbtt_GetCodepointKernAdvance (const stbtt_fontinfo* info, int ch1, int ch2) {
    ...
    return stbtt_GetGlyphKernAdvance (
        info, stbtt_FindGlyphIndex (info, ch1),
...
static void stbtt__rasterize_sorted_edges (
    ...
    while (j < result->h) {
        ...
        STBTT_memcpy (
            result->pixels + j * result->stride, scanline, result->w);
        ++j;
    }
        ...
        if (e->fdx == 0) {
            ...
            if (x_top >= 0 && x_bottom >= 0 && x_top < len && x_bottom < len) {
                ...
                if ((int)x_top == (int)x_bottom) {
                    ...
                    scanline[x2] +=
                        ...
                        sign * (1 - ((x2 - x2) + (x_bottom - x2)) / 2) *
                            (sy1 - y_crossing); // NOLINT (not a false positive
                                                // but external code)
                ...
                for (x = 0; x < len; ++x) {
                    ...
                    // splitting into two (or three) regions. intersections
                    // with top & bottom do not necessitate case-wise logic.
                    //
                    ...
                    // was epsilon across the x border, then the corresponding
                    // y position might not be distinct from the other y
                    // segment, and it might ignored as an empty segment. to
                    // avoid that, we need to explicitly produce segments based
                    // on x positions.
...
STBTT_DEF void stbtt_GetBakedQuad (
    stbtt_bakedchar* chardata, int pw, int ph, int char_index, float* xpos,
...
STBTT_DEF void stbtt_GetPackedQuad (
    stbtt_packedchar* chardata, int pw, int ph, int char_index, float* xpos,
...
STBTT_DEF int stbtt_FindMatchingFont (
    const unsigned char* font_collection, const char* name_utf8,
    stbtt_int32 flags) {
    stbtt_int32 i;
    for (i = 0;; ++i) {
