...
void nvgBeginFrame (
    NVGcontext* ctx, int windowWidth, int windowHeight,
...
// Linearly interpoaltes from color c0 to c1, and returns resulting color
// value.
...
// Skews the current coordinate system along X axis. Angle is specifid in
// radians.
void nvgSkewX (NVGcontext* ctx, float angle);

// Skews the current coordinate system along Y axis. Angle is specifid in
// radians.
...
// transformation matrices. A 2x3 matrix is representated as float[6].
...
void nvgTransformPoint (
    float* dstx, float* dsty, const float* xform, float srcx, float srcy);
...
// Returns handle to the image.
int nvgCreateImageMem (
    NVGcontext* ctx, int imageFlags, unsigned char* data, int ndata);

// Creates image from specified image data.
// Returns handle to the image.
int nvgCreateImageRGBA (
    NVGcontext* ctx, int w, int h, int imageFlags, const unsigned char* data);
...
NVGpaint nvgLinearGradient (
    NVGcontext* ctx, float sx, float sy, float ex, float ey, NVGcolor icol,
    NVGcolor ocol);

// Creates and returns a box gradient. Box gradient is a feathered rounded
// rectangle, it is useful for rendering drop shadows or hilights for boxes.
...
NVGpaint nvgBoxGradient (
    NVGcontext* ctx, float x, float y, float w, float h, float r, float f,
...
NVGpaint nvgRadialGradient (
    NVGcontext* ctx, float cx, float cy, float inr, float outr, NVGcolor icol,
    NVGcolor ocol);
...
NVGpaint nvgImagePattern (
    NVGcontext* ctx, float ox, float oy, float ex, float ey, float angle,
...
// and circles, and lower level step-by-step functions, which allow to define a
// path curve by curve.
//
...
void nvgBezierTo (
    NVGcontext* ctx, float c1x, float c1y, float c2x, float c2y, float x,
    float y);
...
// specified points.
void nvgArcTo (
    NVGcontext* ctx, float x1, float y1, float x2, float y2, float radius);
...
void nvgArc (
    NVGcontext* ctx, float cx, float cy, float r, float a0, float a1, int dir);
...
void nvgRoundedRect (
    NVGcontext* ctx, float x, float y, float w, float h, float r);
...
// Returns handle to the font.
int nvgCreateFontMem (
    NVGcontext* ctx, const char* name, unsigned char* data, int ndata,
    int freeData);
...
float nvgText (
    NVGcontext* ctx, float x, float y, const char* string, const char* end);
...
// boundaries or when new-line characters are encountered. Words longer than
// the max width are slit at nearest character (i.e. no hyphenation).
void nvgTextBox (
    NVGcontext* ctx, float x, float y, float breakRowWidth, const char* string,
    const char* end);
...
// bounds value are [xmin,ymin, xmax,ymax] Measured values are returned in
...
int nvgTextBreakLines (
    NVGcontext* ctx, const char* string, const char* end, float breakRowWidth,
...
struct NVGparams {
    ...
    int (*renderCreateTexture) (
        void* uptr, int type, int w, int h, int imageFlags,
        const unsigned char* data);
    int (*renderDeleteTexture) (void* uptr, int image);
    int (*renderUpdateTexture) (
        void* uptr, int image, int x, int y, int w, int h,
        const unsigned char* data);
    ...
    void (*renderFill) (
        void* uptr, NVGpaint* paint, NVGscissor* scissor, float fringe,
        const float* bounds, const NVGpath* paths, int npaths);
    void (*renderStroke) (
        void* uptr, NVGpaint* paint, NVGscissor* scissor, float fringe,
        float strokeWidth, const NVGpath* paths, int npaths);
    void (*renderTriangles) (
        void* uptr, NVGpaint* paint, NVGscissor* scissor,
