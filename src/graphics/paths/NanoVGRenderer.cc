...
// This code was adapted from the original NanoVG OpenGL renderer to work with
...
void nvgRenderTriangles (
    void* userptr, NVGpaint* paint, NVGscissor* scissor,
    const NVGvertex* verts, int nverts) {
    auto renderer = static_cast< NanoVGRenderer* > (userptr);
    renderer->renderTriangles (paint, scissor, verts, nverts);
}
void nvgRenderFill (
    void* userptr, NVGpaint* paint, NVGscissor* scissor, float fringe,
...
void nvgRenderStroke (
    ...
    renderer->renderStroke (
        paint, scissor, fringe, strokeWidth, paths, npaths);
}
...
int nvgCreateTexture (
    void* userptr, int type, int w, int h, int imageFlags,
    const unsigned char* data) {
    auto renderer = static_cast< NanoVGRenderer* > (userptr);
    return renderer->createTexture (type, w, h, imageFlags, data);
}
int nvgUpdateTexture (
    void* userptr, int image, int x, int y, int w, int h,
    const unsigned char* data) {
    auto renderer = static_cast< NanoVGRenderer* > (userptr);
...
void NanoVGRenderer::initialize () {
    ...
    _vertexLayout.add_vertex_component (
        vertex_format_data::POSITION2, sizeof (NVGvertex),
        (int)offsetof (NVGvertex, x));
    _vertexLayout.add_vertex_component (
        vertex_format_data::TEX_COORD2, sizeof (NVGvertex),
        (int)offsetof (NVGvertex, u));

    // Now create all the materials that we are going to need
    {
        auto material = getDefaultMaterial ();

        // Material for simple triangle draws. These use different primitive
        // types so if that is moved into the material struct at some point
...
void NanoVGRenderer::renderFill (
    NVGpaint* paint, NVGscissor* scissor, float fringe, const float* bounds,
    ...
    if (npaths == 1 && paths[0].convex) { call->type = CallType::ConvexFill; }
    ...
    for (int i = 0; i < npaths; i++) {
        ...
        if (path->nstroke > 0) {
            ...
            copy->strokeCount =
                static_cast< uint32_t > ((size_t)path->nstroke);
            memcpy (
                &_vertices[offset], path->stroke,
    ...
    if (call->type == CallType::Fill) {
        ...
        // Fill shader
        convertPaint (
            &_uniformData[call->uniformIndex + 1], paint, scissor, fringe,
            fringe, -1.0f);
    }
    else {
        ...
        // Fill shader
        convertPaint (
            &_uniformData[call->uniformIndex], paint, scissor, fringe, fringe,
            -1.0f);
    }
}
void NanoVGRenderer::renderTriangles (
    ...
    call->triangleOffset = static_cast< uint32_t > (
        addVertices (verts, static_cast< size_t > (nverts)));
    call->image = paint->image;
...
void NanoVGRenderer::renderStroke (
    NVGpaint* paint, NVGscissor* scissor, float fringe, float strokeWidth,
    const NVGpath* paths, int npaths) {
    if (npaths <= 0) {
        // Ignore irrelevant render calls
    ...
    convertPaint (
        &_uniformData[call->uniformIndex], paint, scissor, strokeWidth, fringe,
        -1.0f);
    convertPaint (
        &_uniformData[call->uniformIndex + 1], paint, scissor, strokeWidth,
...
void NanoVGRenderer::renderFlush () {
    ...
    // This copies the uniform data from our vector into the uniform buffer
    // aligner
    for (auto& uniform : _uniformData) {
        memcpy (
            _uniformBuffer->aligner ().addTypedElement< nanovg_draw_data > (),
            &uniform, sizeof (uniform));
    }

    _uniformBuffer->submitData ();
    gr_update_buffer_data (
        _vertexBuffer, sizeof (NVGvertex) * _vertices.size (),
        _vertices.data ());

    for (auto& drawCall : _drawCalls) {
        switch (drawCall.type) {
        case CallType::Fill: drawFill (drawCall); break;
        case CallType::ConvexFill: drawConvexFill (drawCall); break;
        case CallType::Stroke: drawStroke (drawCall); break;
        case CallType::Triangles: drawTriangles (drawCall); break;
        }
    }
...
int NanoVGRenderer::createTexture (
    int type, int w, int h, int imageFlags, const unsigned char* data) {
    int bpp;
    if (type == NVG_TEXTURE_RGBA) { bpp = 32; }
    else {
        bpp = 8;
    }
    ...
    img.bitmap =
        bm_create (bpp, w, h, img.data.get (), bpp == 8 ? BMP_AABITMAP : 0);
    img.type = type;
...
int NanoVGRenderer::updateTexture (
    int image, int /*x*/, int /*y*/, int /*w*/, int /*h*/,
    const unsigned char* data) {
    auto texture = getTexture (image);
    if (texture == nullptr) { return 0; }

    int bpp;
    if (texture->type == NVG_TEXTURE_RGBA) { bpp = 32; }
    else {
        bpp = 8;
    }
    bm_get_info (texture->bitmap);
    // TODO: This could probably be done better by only uploading the changed
    // area
    gr_update_texture (
        texture->bitmap, bpp, data, texture->width, texture->height);

    return 1;
}
int NanoVGRenderer::deleteTexture (int image) {
    auto iter = _textureMap.find (image);
    if (iter == _textureMap.end ()) { return 0; }
...
bool NanoVGRenderer::convertPaint (
    graphics::nanovg_draw_data* frag, NVGpaint* paint, NVGscissor* scissor,
    ...
    if (scissor->extent[0] < -0.5f || scissor->extent[1] < -0.5f) {
        ...
        frag->scissorScale.x = sqrtf (
                                   scissor->xform[0] * scissor->xform[0] +
                                   scissor->xform[2] * scissor->xform[2]) /
                               fringe;
        frag->scissorScale.y = sqrtf (
                                   scissor->xform[1] * scissor->xform[1] +
    ...
    auto tex = getTexture (paint->image);
    if (tex != nullptr) {
        // Since textures may be stored in texture arrays we need to pass the
        // index of the image into the shader
...
NanoVGRenderer::Image* NanoVGRenderer::getTexture (int id) {
    ...
    if (iter == _textureMap.end ()) { return nullptr; }
    return &iter->second;
}
...
void NanoVGRenderer::drawTriangles (const DrawCall& call) {
    ...
    gr_render_nanovg (
        &mat, PRIM_TYPE_TRIS, &_vertexLayout, call.triangleOffset,
        call.triangleCount, _vertexBuffer);
}
void NanoVGRenderer::drawFill (const DrawCall& call) {
    ...
    for (size_t i = 0; i < call.pathCount; ++i) {
        gr_render_nanovg (
            &mat, PRIM_TYPE_TRISTRIP, &_vertexLayout,
            _paths[pathOffset + i].fillOffset,
            _paths[pathOffset + i].fillCount, _vertexBuffer);
    }
    ...
    for (size_t i = 0; i < call.pathCount; ++i) {
        gr_render_nanovg (
            ...
            _paths[pathOffset + i].strokeCount, _vertexBuffer);
    }
...
void NanoVGRenderer::drawConvexFill (const DrawCall& call) {
    ...
    for (size_t i = 0; i < call.pathCount; ++i) {
        gr_render_nanovg (
            &mat, PRIM_TYPE_TRIFAN, &_vertexLayout,
            _paths[pathOffset + i].fillOffset,
            _paths[pathOffset + i].fillCount, _vertexBuffer);
    }
