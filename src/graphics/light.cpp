...
void FSLight2GLLight (light* FSLight, gr_light* GLLight) {
    ...
    case Light_Type::Point: {
        ...
        GLLight->ConstantAtten = 1.0f;
        GLLight->LinearAtten =
            (1.0f / MAX (FSLight->rada, FSLight->radb)) * 1.25f;
    ...
    case Light_Type::Tube: {
        ...
        GLLight->LinearAtten =
            (1.0f / MAX (FSLight->rada, FSLight->radb)) * 1.25f;
        GLLight->QuadraticAtten =
            (1.0f / MAX (FSLight->rada_squared, FSLight->radb_squared)) *
            1.25f;
        ...
        GLLight->Position.xyzw.x =
            FSLight->vec2.xyz
                .x; // Valathil: Use endpoint of tube as light position
        GLLight->Position.xyzw.y = FSLight->vec2.xyz.y;
        GLLight->Position.xyzw.z = FSLight->vec2.xyz.z;
        GLLight->Position.xyzw.w = 1.0f;

        // Valathil: When using shaders pass the beam direction (not normalized
        // IMPORTANT for calculation of tube)
        vec3d a;
    ...
    case Light_Type::Directional: {
        ...
        GLLight->Position.xyzw.w =
            0.0f; // This is a direction so the w part must be 0
...
static void set_light (int light_num, gr_light* ltp) {
    ...
    vm_vec_transform (
        ...
        &gr_view_matrix);
    vm_vec_transform (
        &gr_light_uniforms[light_num].direction, &ltp->SpotDir,
        &gr_view_matrix, false);

    gr_light_uniforms[light_num].diffuse_color =
        vm_vec4_to_vec3 (ltp->Diffuse);
...
static bool sort_active_lights (const gr_light& la, const gr_light& lb) {
    ...
    if ((la.type != LT_TUBE) && (lb.type == LT_TUBE)) { return false; }
    ...
    if (la.LinearAtten > lb.LinearAtten) { return false; }
    else if (la.LinearAtten < lb.LinearAtten) {
        return true;
    }

    // as one extra check, if we're still here, go with overall brightness of
    // light
    ...
    if (la_value < lb_value) { return false; }
...
void gr_light_shutdown () { gr_lights.clear (); }
...
void gr_set_lighting (bool /*set*/, bool state) {
    ...
    if (!Deferred_lighting) { pre_render_init_lights (); }

    int i = 0;

    for (i = 0; i < (int)graphics::MAX_UNIFORM_LIGHTS; i++) {
        if (i >= Num_active_gr_lights) { break; }

        if (gr_lights[i].type != LT_DIRECTIONAL) continue;
