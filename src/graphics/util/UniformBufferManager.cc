...
UniformBufferManager::UniformBufferManager (uniform_block_type uniform_type)
    : _type (uniform_type) {}
UniformBuffer* UniformBufferManager::getBuffer () {
    ...
    std::unique_ptr< UniformBuffer > buffer (
        new UniformBuffer (getElementSize (_type), getHeaderSize (_type)));
    auto ptr = buffer.get ();
...
void UniformBufferManager::retireBuffers () {
    ...
    for (auto& buffer : _usedBuffers) {
        if (!buffer->isInUse ()) {
            // This buffer will be retired soon so add it to the list before
    ...
    _usedBuffers.erase (
        std::remove_if (
            _usedBuffers.begin (), _usedBuffers.end (),
            [](UniformBuffer* buffer) { return !buffer->isInUse (); }),
        _usedBuffers.end ());
}
