...
void UniformAligner::setAlignment (size_t align) {
    ...
    // Clear the buffer since changing the alignment invalidates the stored
    // data
    clear ();
}
void UniformAligner::resize (size_t num_elements) {
    size_t final_size =
        alignSize (_headerSize, _requiredAlignment) +
        alignSize (_dataSize, _requiredAlignment) * num_elements;
...
void* UniformAligner::addElement () {
    _buffer.insert (
        _buffer.end (), alignSize (_dataSize, _requiredAlignment), 0);
    ++_numElements;

    return getElement (_numElements - 1);
}
void* UniformAligner::getElement (size_t index) {
    size_t offset = alignSize (_headerSize, _requiredAlignment) +
                    alignSize (_dataSize, _requiredAlignment) * index;

    ASSERTX (offset < _buffer.size (), "Invalid index specified!");
...
size_t UniformAligner::getNumElements () { return _numElements; }
size_t UniformAligner::getSize () { return _buffer.size (); }
void* UniformAligner::getData () { return _buffer.data (); }
void UniformAligner::clear () { resize (0); }
