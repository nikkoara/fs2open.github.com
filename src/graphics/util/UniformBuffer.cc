...
UniformBuffer::UniformBuffer (size_t element_size, size_t header_size)
    : _aligner (element_size, header_size) {
    int offsetAlignment;
    ...
    // This uses Dynamic since that matches our usage pattern the closest
    // (update once and then use multiple times)
    _buffer_obj =
...
UniformBuffer::~UniformBuffer () {
    ...
    if (_sync_obj != nullptr) { gr_sync_delete (_sync_obj); }
}
...
void UniformBuffer::finished () {
    ...
    // We use fences to determine if the GPU is still using this buffer. That
    // allows us to use multiple buffers and avoid implicit synchronization
    // which might be performed by the driver otherwise.
    _sync_obj = gr_sync_fence ();

    // Uniform data is single-use only so we can discard the old data of the
    // aligner
    _aligner.clear ();
}
bool UniformBuffer::isInUse () {
    if (_sync_obj == nullptr) {
        // Probably was signaled before, just assume that this means that the
        // buffer is not in use anymore
        return false;
    }
    ...
    // Fence still exists and has not been signaled yet. The buffer is still in
    // use
    return true;
}
