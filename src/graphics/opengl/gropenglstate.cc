...
{
        ASSERT(
                (num >= 0) && (num < (int)(sizeof(clipdistance_Status) / sizeof(GLboolean))));
...
bvec4 opengl_state::ColorMask(bool red, bool green, bool blue, bool alpha)
{
        ...
        if (colormask_Status.x != red || colormask_Status.y != green || colormask_Status.z != blue || colormask_Status.w != alpha) {
                glColorMask(
        ...
        GLsizei stride, GLvoid *pointer)
{
        ...
        if (!va_unit->reset_ptr && va_unit->ptr_init && va_unit->normalized == normalized && va_unit->pointer == pointer && va_unit->size == size && va_unit->stride == stride && va_unit->type == type && va_unit->buffer == array_buffer) {
                return;
        }
        ...
        GLuint bindingindex, GLuint buffer, GLintptr offset, GLsizei stride)
{
        ...
        if (bindingInfo.valid_data && bindingInfo.buffer == buffer && bindingInfo.offset == offset && bindingInfo.stride == stride) {
                return;
        }
