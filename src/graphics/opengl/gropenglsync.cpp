...
bool gr_opengl_sync_wait (gr_sync sync, uint64_t timeoutns) {
    ...
    // We only need GL_SYNC_FLUSH_COMMANDS_BIT if we actually wait for the sync
    // object. Otherwise we only check if the object is signaled which does not
    // require a flush.
    auto res = glClientWaitSync (
        (GLsync)sync, timeoutns != 0 ? GL_SYNC_FLUSH_COMMANDS_BIT : 0,
        timeoutns);
