...
void gr_opengl_deferred_lighting_begin () {
    if (Cmdline_no_deferred_lighting) return;
    ...
    // Copy the existing color data into the emissive part of the G-buffer
    // since everything that already existed is treated as emissive
...
void gr_opengl_deferred_lighting_finish () {
    ...
    // We need to use stable sorting here to make sure that the relative
    // ordering of the same light types is the same as the rest of the code.
    // Otherwise the shadow mapping would be applied while rendering the wrong
    // light which would lead to flickering lights in some circumstances
...

    {
        ...
        // Only the first directional light uses shaders so we need to know
        // when we already saw that light
        bool first_directional = true;
        for (auto& l : Lights) {
            auto light_data =
                uniformAligner.addTypedElement< deferred_light_data > ();
            ...
            case Light_Type::Directional:
                ...
                vm_vec_scale (
                    &light_data->specLightColor, static_light_factor);
            ...
            case Light_Type::Point:
                ...
                vm_vec_scale (
                    &light_data->specLightColor, static_point_factor);
            ...
            case Light_Type::Tube: {
                ...
                // Tube lights consist of two different types of lights with
        ...
        // Uniform data has been assembled, upload it to the GPU and issue the
        // draw calls
        buffer->submitData ();
    }
    {
        ...
        gr_bind_uniform_buffer (
            uniform_block_type::DeferredGlobals, 0,
    ...
    if (The_mission.flags[Mission::Mission_Flags::Fullneb] &&
        ...
        gr_zbuffer_set (GR_ZBUFF_NONE);
        opengl_shader_set_current (
            gr_opengl_maybe_create_shader (SDR_TYPE_SCENE_FOG, 0));
        ...
        Current_shader->program->Uniforms.setUniformf (
            "fog_scale", 1.0f / (fog_far - fog_near));
        Current_shader->program->Uniforms.setUniform3f (
            "fog_color", r / 255.f, g / 255.f, b / 255.f);
        Current_shader->program->Uniforms.setUniformf (
            "zNear", Min_draw_distance);
        Current_shader->program->Uniforms.setUniformf (
            "zFar", Max_draw_distance);
        ...
        // TODO: Maybe this could be improved so that it doesn't require the
        // copy back operation?
...
void gr_opengl_deferred_light_cylinder_init (
    int segments) // Generate a VBO of a cylinder of radius and height 1.0f,
                  // based on code at
                  // http://www.ogre3d.org/tikiwiki/ManualSphereMeshes
{
    ...
    *pVertex++ = 1.0f;
    ...
    if (deferred_light_cylinder_vbo) {
        ...
        glBufferData (
            GL_ARRAY_BUFFER, nVertex * sizeof (float), Vertices,
            GL_STATIC_DRAW);
    ...
    if (deferred_light_cylinder_ibo) {
        ...
        glBufferData (
            GL_ELEMENT_ARRAY_BUFFER, nIndex * sizeof (ushort), Indices,
            GL_STATIC_DRAW);
...
void gr_opengl_deferred_light_sphere_init (
    int rings,
    int segments) // Generate a VBO of a sphere of radius 1.0f, based on code
                  // at http://www.ogre3d.org/tikiwiki/ManualSphereMeshes
{
...
void opengl_draw_sphere () {
    ...
    vertex_declare.add_vertex_component (
        vertex_format_data::POSITION3, sizeof (float) * 3, 0);

    opengl_bind_vertex_layout (
        vertex_declare, deferred_light_sphere_vbo, deferred_light_sphere_ibo);

    glDrawRangeElements (
        GL_TRIANGLES, 0, deferred_light_sphere_vcount,
        deferred_light_sphere_icount, GL_UNSIGNED_SHORT, 0);
}

void gr_opengl_draw_deferred_light_cylinder (
    const vec3d* position, const matrix* orient) {
    ...
    opengl_bind_vertex_layout (
        vertex_declare, deferred_light_cylinder_vbo,
        deferred_light_cylinder_ibo);

    glDrawRangeElements (
        GL_TRIANGLES, 0, deferred_light_cylinder_vcount,
        deferred_light_cylinder_icount, GL_UNSIGNED_SHORT, 0);
