...
int opengl_free_texture(tcache_slot_opengl *t)
{
        ...
        if (t->bitmap_handle >= 0) {
                ...
                // animation
                auto end_slot_handle = animation_begin + num_frames - 1; // Slots are contiguous for the same animation. Since num_frames
                ...
                for (int slot_handle = animation_begin; slot_handle <= end_slot_handle;
                     ++slot_handle) {
                        ...
                        something_in_use = something_in_use || current_slot->used; // We use or here since only one slot needs
                                                                                   // to be in use
                                ...
                                if ((i < bmap_h) && (j < bmap_w)) {
                                        if (byte_mult > 1) {
                                                ...
                                                if (byte_mult > 3) {
                                                        ...
                                                        *texmemp++ = (ubyte)(
                                                                (luminance / 3) * (bmp_data[(i * bmap_w + j) * byte_mult + 3] / 255.0f));
                                                } else {
        ...
        int bitmap_handle, int bitmap_type, tcache_slot_opengl *tslot)
{
        ...
        auto base_level = 0;
        auto resize = false;
        if ((Detail.hardware_textures < 4) && (bitmap_type != TCACHE_TYPE_AABITMAP) && (bitmap_type != TCACHE_TYPE_INTERFACE) && (bitmap_type != TCACHE_TYPE_CUBEMAP) && ((bitmap_type != TCACHE_TYPE_COMPRESSED) || ((bitmap_type == TCACHE_TYPE_COMPRESSED) && (max_levels > 1)))) {
                if (max_levels == 1) {
        ...
        uint32_t *array_index, int tex_unit = 0)
{
        ...
        if (ret_val && t->texture_id) {
                ...
                *array_index = t->array_index;
                ...
                if ((t->wrap_mode != GL_texture_addressing) && (bitmap_type != TCACHE_TYPE_AABITMAP) && (bitmap_type != TCACHE_TYPE_INTERFACE) && (bitmap_type != TCACHE_TYPE_CUBEMAP)) {
                                ...
                                if ((i < height) && (j < width)) {
                                        if (true_byte_mult > 1) {
                                                ...
                                                if (true_byte_mult > 3) {
                                                        ...
                                                        *texmemp++ = (ubyte)(
                                                                (luminance / 3) * (data[(i * width + j) * true_byte_mult + 3] / 255.0f));
                                                } else {
