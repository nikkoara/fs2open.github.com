...
int opengl_create_texture (
    int bitmap_handle, int bitmap_type, tcache_slot_opengl* tslot = NULL);
...
GLfloat opengl_get_max_anisotropy () {
    if (!GLAD_GL_EXT_texture_filter_anisotropic) { return 0.0f; }
...
void opengl_set_texture_target (GLenum target) { GL_texture_target = target; }
...
void opengl_tcache_init () {
    ...
    // if we are not using sections then make sure we have the min texture size
    ...
    // max size (width and/or height) that we can use for
...
void opengl_tcache_flush () {
    for (auto& block : bm_blocks) {
        for (auto& slot : block) {
            opengl_free_texture (
                static_cast< tcache_slot_opengl* > (slot.gr_info));
        }
    }
}
...
void opengl_tcache_frame () { GL_textures_in_frame = 0; }
...
bool opengl_texture_slot_valid (int handle) {
    ...
    if (t->bitmap_handle < 0) { return false; }

    if (t->bitmap_handle != handle) { return false; }

    if (!glIsTexture (t->texture_id)) { return false; }
...
int opengl_free_texture (tcache_slot_opengl* t) {
    ...
    // that's the case we can free the texture array
    t->used = false;
    ...
    if (t->bitmap_handle >= 0) {
        ...
        auto animation_begin =
            bm_get_base_frame (t->bitmap_handle, &num_frames);
        ...
        // Get the index of the first slot and then check every slot of the
        // animation
        auto end_slot_handle =
            animation_begin + num_frames -
            1; // Slots are contiguous for the same animation. Since num_frames
               // is 1-base we need to substract 1

        bool something_in_use = false; // Will be used to check if there is
                                       // still a frame left in use
        for (int slot_handle = animation_begin; slot_handle <= end_slot_handle;
             ++slot_handle) {
            auto current_slot =
                bm_get_gr_info< tcache_slot_opengl > (slot_handle);
            something_in_use =
                something_in_use ||
                current_slot->used; // We use or here since only one slot needs
                                    // to be in use

            if (something_in_use) {
                // If this is true then we can break early since we already
                // determined that one frame is in use
                break;
            }
        }

        if (something_in_use) {
            // There are still used frames left in the animation so we can't
            // free the texture object here

            // We still need to reset this slot though since it has been
            // "freed"
            t->reset ();
...
int opengl_texture_set_level (
    int bitmap_handle, int bitmap_type, int bmap_w, int bmap_h, int tex_w,
    int tex_h, ubyte* data, tcache_slot_opengl* tSlot, int /*base_level*/,
    ...
    // bogus
    if ((tSlot == NULL) || (bmp_data == NULL)) { return 0; }
    ...
    if (byte_mult == 4) {
        ...
        ASSERTX (
            ...
            "compression on a non-power-of-2 texture.\n",
    ...
    case TCACHE_TYPE_COMPRESSED: {
        if (block_size > 0) {
            for (auto i = 0; i < mipmap_levels; i++) {
                ...
                dsize =
                    ((mipmap_h + 3) / 4) * ((mipmap_w + 3) / 4) * block_size;

                glCompressedTexSubImage3D (
                    tSlot->texture_target, i, 0, 0, tSlot->array_index,
                    mipmap_w, mipmap_h, 1, intFormat, dsize,
                    bmp_data + doffset);
                ...
                if ((i < bmap_h) && (j < bmap_w)) {
                    if (byte_mult > 1) {
                        ...
                        if (byte_mult > 3) {
                            for (auto k = 0; k < 3; k++) {
                                luminance +=
                                    bmp_data[(i * bmap_w + j) * byte_mult + k];
                            }

                            *texmemp++ = (ubyte) (
                                (luminance / 3) *
                                (bmp_data[(i * bmap_w + j) * byte_mult + 3] /
                                 255.0f));
                        }
                        else {
        ...
        for (auto i = 0; i < 6; i++) {
            ...
            if (block_size > 0) {
                ...
                glCompressedTexSubImage2D (
                    GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, 0, 0, mipmap_w,
                    mipmap_h, intFormat, dsize, bmp_data + doffset);
                ...
                for (auto j = 1; j < mipmap_levels; j++) {
                    ...
                    // size of data block (4x4)
                    dsize = ((mipmap_h + 3) / 4) * ((mipmap_w + 3) / 4) *
                            block_size;

                    glCompressedTexSubImage2D (
                        GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, j, 0, 0, mipmap_w,
                        mipmap_h, intFormat, dsize, bmp_data + doffset);
                }
            }
    ...
    default: {
        ...
        if (resize) {
            ...
            for (auto j = 0; j < tex_h; j++) {
                ...
                for (auto i = 0; i < tex_w; i++) {
                    for (auto k = 0; k < byte_mult; k++) {
                        *texmemp++ = bmp_data
                            [(f2i (v) * bmap_w + f2i (utmp)) * byte_mult + k];
                    }
                    utmp += du;
                }
                v += dv;
        ...
        for (auto i = 0; i < mipmap_levels; i++) {
            ...
            glTexSubImage3D (
                tSlot->texture_target, i, 0, 0, tSlot->array_index, mipmap_w,
                mipmap_h, 1, glFormat, texFormat,
                (texmem != NULL) ? texmem : bmp_data + doffset);
    ...
    tSlot->size = (dsize) ? ((doffset + dsize) - skip_size)
                          : (tex_w * tex_h * byte_mult);
...
static GLenum
opengl_get_internal_format (int handle, int bitmap_type, int bpp) {
    // set the byte per pixel multiplier
    ...
    case DDS_DXT1:
    case DDS_CUBEMAP_DXT1: return GL_COMPRESSED_RGB_S3TC_DXT1_EXT;

    case DDS_DXT3:
    case DDS_CUBEMAP_DXT3: return GL_COMPRESSED_RGBA_S3TC_DXT3_EXT; break;

    case DDS_DXT5:
    case DDS_CUBEMAP_DXT5: return GL_COMPRESSED_RGBA_S3TC_DXT5_EXT;
    default:
        // Not compressed
        break;
    }
...
void opengl_determine_bpp_and_flags (
    int bitmap_handle, int bitmap_type, ubyte& flags, int& bpp) {
    flags = 0;
    bpp = 16;
    switch (bitmap_type) {
    case TCACHE_TYPE_AABITMAP:
    ...
    case TCACHE_TYPE_NORMAL:
        ...
        if (bm_get_type (bitmap_handle) == BM_TYPE_PCX) {
            // PCX is special since the locking code only works with bpp = 16
            // for some reason
            bpp = 16;
        }
        else {
            if (bm_has_alpha_channel (bitmap_handle)) {
                bpp = 32; // Since this bitmap has an alpha channel we need to
                          // respect that
            }
            else {
    ...
    if (target == GL_TEXTURE_CUBE_MAP) {
        ...
        if (GLAD_GL_ARB_texture_storage) {
            ...
            if (levels == 1) {
                ...
                // allocate additional storage for the mip maps we're going to
                // later generate using glGenerateMipMap
            ...
            for (auto i = 0; i < 6; ++i) {
                ...
                for (auto j = 0; j < levels; j++) {
                    glTexImage2D (
                        GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, j, format,
                        mip_width, mip_height, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                        nullptr);
        ...
        if (GLAD_GL_ARB_texture_storage) {
            ...
            for (auto i = 0; i < levels; i++) {
                glTexImage3D (
                    target, i, format, width, height, frames, 0, GL_RGBA,
...
int opengl_create_texture (
    int bitmap_handle, int bitmap_type, tcache_slot_opengl* tslot) {
    GR_DEBUG_SCOPE ("Create Texture");

    if (tslot == NULL) { return 0; }

    int num_frames = 0;
    // This takes care of checking if the animation can be put into a texture
    // array
    ...
    // if we ended up locking a texture that wasn't originally compressed then
    // this should catch it
    if (bitmap_type != TCACHE_TYPE_CUBEMAP &&
        bm_is_compressed (bitmap_handle)) {
        bitmap_type = TCACHE_TYPE_COMPRESSED;
    }
    ...
    // This is very important! All bitmaps are bound as texture arrays even if
    // they only contain a single image. This simplifies shader handling since
    // they only ever work with texture arrays instead of having to introduce
    // another compile time flag for every single sampler
    GL_state.Texture.SetTarget (tslot->texture_target);
    GL_state.Texture.Enable (tslot->texture_id);

    opengl_set_object_label (
        GL_TEXTURE, tslot->texture_id, bm_get_filename (bitmap_handle));

    GLenum min_filter = GL_LINEAR;

    auto mipmap_levels = (ubyte) (max_levels - base_level);
    if (mipmap_levels > 1) {
        min_filter = (GL_mipmap_filter) ? GL_LINEAR_MIPMAP_LINEAR
                                        : GL_LINEAR_MIPMAP_NEAREST;

        if (GLAD_GL_EXT_texture_filter_anisotropic) {
            glTexParameterf (
                tslot->texture_target, GL_TEXTURE_MAX_ANISOTROPY_EXT,
                GL_anisotropy);
        }
    }
    ...
    if (base_level != 0) {
        glTexParameteri (
            tslot->texture_target, GL_TEXTURE_BASE_LEVEL, base_level);
    }
    glTexParameteri (
        tslot->texture_target, GL_TEXTURE_MAX_LEVEL, mipmap_levels);
    ...
    glTexParameteri (
        tslot->texture_target, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri (
        tslot->texture_target, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glTexParameteri (
        tslot->texture_target, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
    ...
    opengl_determine_bpp_and_flags (
        animation_begin, bitmap_type, bitmap_flags, bits_per_pixel);

    auto intFormat = opengl_get_internal_format (
        bitmap_handle, bitmap_type, bits_per_pixel);
    opengl_tex_array_storage (
        tslot->texture_target, max_levels, intFormat, width, height,
        num_frames);

    bool frames_loaded = true;
    for (int frame = animation_begin; frame < animation_begin + num_frames;
         ++frame) {
#ifndef NDEBUG
        // I'm not sure if these values are consistent across the whole
        // animation but they really should be. This should catch any instances
        // where this assumption isn't right
        ubyte debug_flags = 0;
        int debug_bpp;
        opengl_determine_bpp_and_flags (
        ...
        *frame_slot =
            *tslot; // Copy the existing properties to the slot of this texture
        frame_slot->array_index = (uint32_t)index;

        // set the bits per pixel
        frame_slot->bpp = bmp->bpp;

        // max number of mipmap levels (NOTE: this is the max number used by
        // the API, not how many true mipmap levels there are in the image!!)
        ...
        // call the helper
        int ret_val = opengl_texture_set_level (
            frame, bitmap_type, bmp->w, bmp->h, width, height,
            (ubyte*)bmp->data, frame_slot, base_level, resize, intFormat);
    ...
    if (tslot->mipmap_levels == 1 && bitmap_type == TCACHE_TYPE_CUBEMAP) {
        ...
        glTexParameteri (
            tslot->texture_target, GL_TEXTURE_MAX_LEVEL,
            tslot->mipmap_levels - 1);
        glTexParameteri (
            tslot->texture_target, GL_TEXTURE_MIN_FILTER,
    ...
    // NOTE: this doesn't do anything for user bitmaps (like screen grabs or
    // streamed animations)
    if (frames_loaded) { bm_unload_fast (bitmap_handle); }
...
int gr_opengl_tcache_set_internal (
    int bitmap_handle, int bitmap_type, float* u_scale, float* v_scale,
    uint32_t* array_index, int tex_unit = 0) {
    int ret_val = 1;
    ...
    if (ret_val && t->texture_id) {
        ...
        *array_index = t->array_index;
        ...
        if ((t->wrap_mode != GL_texture_addressing) &&
            (bitmap_type != TCACHE_TYPE_AABITMAP) &&
            (bitmap_type != TCACHE_TYPE_INTERFACE) &&
            (bitmap_type != TCACHE_TYPE_CUBEMAP)) {
            // In this case we need to make sure that the texture unit is
            // actually active
            GL_state.Texture.SetActiveUnit (tex_unit);
            glTexParameteri (
                t->texture_target, GL_TEXTURE_WRAP_S, GL_texture_addressing);
            glTexParameteri (
                t->texture_target, GL_TEXTURE_WRAP_T, GL_texture_addressing);
            glTexParameteri (
                t->texture_target, GL_TEXTURE_WRAP_R, GL_texture_addressing);
...
int gr_opengl_tcache_set (
    ...
    if (bitmap_handle < 0) { return 0; }

    GL_CHECK_FOR_ERRORS ("start of tcache_set()");

    // set special type if it's so, needed to be right later, but cubemaps are
    // special
    if (!(bitmap_type == TCACHE_TYPE_CUBEMAP)) {
        int type = bm_get_tcache_type (bitmap_handle);

        if (type != TCACHE_TYPE_NORMAL) { bitmap_type = type; }
    }

    rc = gr_opengl_tcache_set_internal (
        bitmap_handle, bitmap_type, u_scale, v_scale, array_index, stage);
...
void gr_opengl_set_texture_addressing (int mode) {
    ...
    switch (mode) {
    case TMAP_ADDRESS_CLAMP: GL_texture_addressing = GL_CLAMP_TO_EDGE; break;

    case TMAP_ADDRESS_WRAP: GL_texture_addressing = GL_REPEAT; break;
...
int opengl_compress_image (
    ...
    // a quick proxy test.  this will tell us if it's possible without wasting
    // a lot of time and resources in the attempt
    glTexImage2D (
        GL_PROXY_TEXTURE_2D, 0, intFormat, width, height, 0, glFormat,
        texFormat, in_data);

    glGetTexLevelParameteriv (
        GL_PROXY_TEXTURE_2D, 0, GL_TEXTURE_COMPRESSED, &compressed);
    glGetTexLevelParameteriv (
        GL_PROXY_TEXTURE_2D, 0, GL_TEXTURE_WIDTH, &testing);

    if (compressed == GL_TRUE) {
        glGetTexLevelParameteriv (
            GL_PROXY_TEXTURE_2D, 0, GL_TEXTURE_INTERNAL_FORMAT, &compressed);
    }

    if ((compressed == GL_FALSE) || (compressed != intFormat) ||
        (testing == 0)) {
    ...
    glTexImage2D (
        GL_TEXTURE_2D, 0, intFormat, width, height, 0, glFormat, texFormat,
        in_data);
    ...
    for (i = 0; i < num_mipmaps; i++) {
        glGetTexLevelParameteriv (
            GL_TEXTURE_2D, i, GL_TEXTURE_COMPRESSED_IMAGE_SIZE, &testing);
    ...
    // reset compressed_size and go back through each mipmap level to get both
    // size and the image data itself
    compressed_size = 0;
...
int opengl_get_texture (
    ...
    for (int i = 0; i < num_mipmaps; i++) {
        glGetTexImage (
            target, i, pixel_format, data_format,
            (ubyte*)image_data + m_offset);
...
void gr_opengl_get_bitmap_from_texture (void* data_out, int bitmap_num) {
    ...
    gr_opengl_tcache_set (
        bitmap_num, TCACHE_TYPE_NORMAL, &u, &v, &array_index);
    ...
    // We can't read a specific layer of the texture so we need to read the
    // entire texture and then memcpy the right part from that...
    int num_frames = 0;
    bm_get_info (bitmap_num, nullptr, nullptr, nullptr, &num_frames);
    if (!bm_is_texture_array (bitmap_num)) { num_frames = 1; }
    ...
    glGetTexImage (
        ts->texture_target, 0, pixel_format, data_format, buffer.get ());
 ...
 * be used for uncompressed 24-bit or 32-bit (distiguished by "alpha" var)
 * images
 *
 * @return 0 on failure
 * @return size of data on success
 */
size_t opengl_export_render_target (
    int slot, int width, int height, int alpha, int num_mipmaps,
    ubyte* image_data) {
    ...
    int m_bpp = (alpha) ? 4 : 3;
    int m_offset = 0;
    GLenum target = (ts->texture_target == GL_TEXTURE_CUBE_MAP)
                        ? GL_TEXTURE_CUBE_MAP_POSITIVE_X
                        : GL_TEXTURE_2D;

    if (ts->texture_target == GL_TEXTURE_CUBE_MAP) faces = 6;
    ...
    for (int i = 0; i < faces; i++) {
        m_offset = opengl_get_texture (
            target + i, (alpha) ? GL_BGRA : GL_BGR,
            (alpha) ? GL_UNSIGNED_INT_8_8_8_8_REV : GL_UNSIGNED_BYTE,
            ts->mipmap_levels, m_width, m_height, m_bpp, image_data, m_offset);
    }
...
void gr_opengl_update_texture (
    int bitmap_handle, int bpp, const ubyte* data, int width, int height) {
    GLenum texFormat, glFormat;
    auto t = bm_get_gr_info< tcache_slot_opengl > (bitmap_handle);
    if (!t->texture_id) return;
    int byte_mult = (bpp >> 3);
                ...
                if ((i < height) && (j < width)) {
                    if (true_byte_mult > 1) {
                        ...
                        if (true_byte_mult > 3) {
                            for (int k = 0; k < 3; k++) {
                                luminance +=
                                    data[(i * width + j) * true_byte_mult + k];
                            }

                            *texmemp++ = (ubyte) (
                                (luminance / 3) *
                                (data[(i * width + j) * true_byte_mult + 3] /
                                 255.0f));
                        }
                        else {
...
static fbo_t* opengl_get_fbo (int id) {
    ...
    for (auto& target : RenderTarget) {
        if (target.fbo_id == id) { return &target; }
    }
...
void opengl_kill_render_target (bitmap_slot* slot) {
    // this will happen when opengl_kill_all_render_targets() gets called first
    // on exit
    ...
    // this may happen when textures are flushed, the w and h will get reset to
    // 0
...
int opengl_set_render_target (int slot, int face, int is_static) {
    ...
    if (slot < 0) {
        if ((render_target != nullptr) &&
            (render_target->working_handle >= 0)) {
            if (bm_get_gr_info< tcache_slot_opengl > (
                    render_target->working_handle)
                    ->mipmap_levels > 1) {
            ...
            if (render_target->is_static && Cmdline_save_render_targets) {
                ...
                gr_opengl_bm_save_render_target (
                    render_target->working_handle);
            }
        }
...
int opengl_make_render_target (
    ...
    // size check
    if (*w > GL_max_renderbuffer_size) { *w = GL_max_renderbuffer_size; }

    if (*h > GL_max_renderbuffer_size) { *h = GL_max_renderbuffer_size; }
    ...
    // Initialize the texture storage of the framebuffer. Framebuffers always
    // have only a single layer in the texture array This automatically handles
    // cubemaps correctly
    opengl_tex_array_storage (
        GL_state.Texture.GetTarget (), ts->mipmap_levels, GL_RGBA8, *w, *h, 1);
    ...
    glTexParameteri (
        GL_texture_target, GL_TEXTURE_MAX_LEVEL, ts->mipmap_levels - 1);
    ...
    if (flags & BMP_FLAG_CUBEMAP) {
        glFramebufferTexture2D (
            GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
            GL_TEXTURE_CUBE_MAP_POSITIVE_X, ts->texture_id, 0);
    }
    else {
        // Since we use texture arrays for all bitmaps we use a single image
        // array here
    ...
    if (mm_lvl) { *mm_lvl = ts->mipmap_levels; }
