...
public:
    opengl_texture_state ()
        : active_texture_unit (0), shader_mode (GL_FALSE), units (NULL) {}
    ~opengl_texture_state ();
     ...
     *
     * This is a more efficient version of the standard activeTexture -> Enable
     * since it does not cause an OpenGL call if the texture is already set
     * correctly
     *
     * @warning This may not actually set the active texture unit to the
     * specified value! If you require that then you need to call
     * SetActiveUnit!
     *
...
private:
    ...
    std::vector< vertex_buffer_binding >
        vertex_buffer_bindings; //!< Use for keeping track of
                                //!< glBindVertexBuffer calls

    /**
     * @brief Signals that the VAO binding changed which invalidates the state
     * of our array and element buffer state
     */
    void VertexArrayChanged ();

public:
    opengl_array_state ()
        : active_client_texture_unit (0), client_texture_units (NULL) {
...
private:
    ...
    std::array< std::pair< GLenum, GLenum >, material::NUM_BUFFER_BLENDS >
        buffer_blendfunc_Value;
    ...
    // The framebuffer state actually consists of draw and read buffers but we
    // only use both at the same time
...
void opengl_setup_render_states (
    int& r, int& g, int& b, int& alpha, int& tmap_type, int flags,
    int is_scaler = 0);

#endif // FREESPACE2_GRAPHICS_OPENGL_GROPENGLSTATE_HH
