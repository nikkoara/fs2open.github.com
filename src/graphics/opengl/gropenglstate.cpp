...
void opengl_texture_state::SetTarget (GLenum tex_target) {
    if (units[active_texture_unit].texture_target != tex_target) {
        ...
        // reset modes, since those were only valid for the previous texture
        // target
...
void opengl_texture_state::Enable (GLuint tex_id) {
    if (units[active_texture_unit].texture_id == tex_id) { return; }
...
void opengl_texture_state::Enable (
    ...
    if (units[unit].texture_target == tex_target &&
        ...
        // The texture unit already uses this texture. There is no need to
        // change it
        return;
    }
...
void opengl_state::init () {
    ...
    for (i = 0; i < (int)(sizeof (clipplane_Status) / sizeof (GLboolean));
         i++) {
        // glDisable(GL_CLIP_PLANE0+i);
        clipplane_Status[i] = GL_FALSE;
    }

    for (i = 0; i < (int)(sizeof (clipdistance_Status) / sizeof (GLboolean));
         i++) {
...
GLboolean opengl_state::ClipDistance (GLint num, bool state) {
    ...
    if (state != clipdistance_Status[num]) {
        if (state) { glEnable (GL_CLIP_DISTANCE0 + num); }
        else {
...
bvec4 opengl_state::ColorMask (bool red, bool green, bool blue, bool alpha) {
    ...
    if (colormask_Status.x != red || colormask_Status.y != green ||
        colormask_Status.z != blue || colormask_Status.w != alpha) {
        glColorMask (
...
void opengl_state::BlendFunci (int buffer, GLenum s_val, GLenum d_val) {
    ...
    state.first = s_val;
    state.second = d_val;
    // Set the non-buffer values to an invalid value to make sure that the next
    // call to BlendFunc resets the state.
...
void opengl_state::SetAlphaBlendMode (gr_alpha_blend ab) {
    ...
    case ALPHA_BLEND_NONE: GL_state.BlendFunc (GL_ONE, GL_ZERO); break;

    case ALPHA_BLEND_ADDITIVE: GL_state.BlendFunc (GL_ONE, GL_ONE); break;
    ...
    case ALPHA_BLEND_ALPHA_BLEND_SRC_COLOR:
        GL_state.BlendFunc (
            /*GL_SRC_COLOR*/ GL_SRC_ALPHA, GL_ONE_MINUS_SRC_COLOR);
        break;
...
void opengl_state::SetAlphaBlendModei (int buffer, gr_alpha_blend ab) {
    ...
    case ALPHA_BLEND_ALPHA_BLEND_SRC_COLOR:
        GL_state.BlendFunci (
            buffer, /*GL_SRC_COLOR*/ GL_SRC_ALPHA, GL_ONE_MINUS_SRC_COLOR);
        break;
...
void opengl_state::SetLineWidth (GLfloat width) {
    if (width == line_width_Value) { return; }
...
void opengl_state::StencilOpSeparate (
    ...
    if (face == GL_FRONT) {
        if (stencilOpFrontStencilFail == sfail &&
        ...
        if (stencilOpBackStencilFail == sfail &&
            stencilOpBackDepthFail == dpfail && stencilOpBackPass == dppass) {
            return;
        }
...
void opengl_array_state::EnableVertexAttrib (GLuint index) {
    ...
    if (va_unit->status_init && va_unit->status == GL_TRUE) { return; }
...
void opengl_array_state::DisableVertexAttrib (GLuint index) {
    ...
    if (va_unit->status_init && va_unit->status == GL_FALSE) { return; }
...
void opengl_array_state::VertexAttribPointer (
    GLuint index, GLint size, GLenum type, GLboolean normalized,
    GLsizei stride, GLvoid* pointer) {
    opengl_vertex_attrib_unit* va_unit = &vertex_attrib_units[index];

    if (!va_unit->reset_ptr && va_unit->ptr_init &&
        va_unit->normalized == normalized && va_unit->pointer == pointer &&
        va_unit->size == size && va_unit->stride == stride &&
        va_unit->type == type && va_unit->buffer == array_buffer) {
        return;
    }
...
void opengl_array_state::ResetVertexAttribs () {
    ...
    for (it = vertex_attrib_units.begin (); it != vertex_attrib_units.end ();
         ++it) {
...
void opengl_array_state::BindArrayBuffer (GLuint id) {
    if (array_buffer_valid && array_buffer == id) { return; }
...
void opengl_array_state::BindElementBuffer (GLuint id) {
    if (element_array_buffer_valid && element_array_buffer == id) { return; }
...
void opengl_array_state::BindTextureBuffer (GLuint id) {
    if (texture_array_buffer == id) { return; }
...
void opengl_array_state::BindUniformBufferBindingIndex (
    GLuint id, GLuint index) {
    if (uniform_buffer_index_bindings[index] == id) { return; }
...
void opengl_array_state::BindVertexBuffer (
    ...
    if (bindingInfo.valid_data && bindingInfo.buffer == buffer &&
        bindingInfo.offset == offset && bindingInfo.stride == stride) {
        return;
    }
...
void opengl_constant_state::init () {
    glGetIntegerv (
        GL_UNIFORM_BUFFER_OFFSET_ALIGNMENT, &_uniform_buffer_offset_alignment);
    glGetIntegerv (GL_MAX_UNIFORM_BLOCK_SIZE, &_max_uniform_block_size);
    glGetIntegerv (
        GL_MAX_UNIFORM_BUFFER_BINDINGS, &_max_uniform_block_bindings);
}
