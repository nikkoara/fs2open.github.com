...
public:
    ...
    matrix4& get_transform () { return Current_transform; }
    ...
    void
    push (const vec3d* pos, const matrix* orient, const vec3d* scale = NULL) {
        ...
        if (pos != NULL) { new_pos = *pos; }

        if (orient != NULL) { new_orient = *orient; }

        if (scale != NULL) { new_scale = *scale; }
        ...
        vm_matrix4_x_matrix4 (
            &Current_transform, &current_transform_copy, &new_transform);
        Stack.push_back (Current_transform);
    }
...
enum shader_type {
    ...
    SDR_TYPE_PASSTHROUGH_RENDER, //!< Shader for doing the old style
                                 //!< fixed-function rendering. Only used
                                 //!< internally, use
...
#define SDR_FLAG_MODEL_THICK_OUTLINES \
    (1 << 21) // Renders the model geometry as an outline with configurable
              // line width
...
struct vertex_format_data {
    ...
    vertex_format_data (
        vertex_format i_format_type, size_t i_stride, size_t i_offset)
    ...
    bool operator== (const vertex_format_data& other) const {
        return format_type == other.format_type && stride == other.stride &&
               offset == other.offset;
    }
};
class vertex_layout {
...
public:
    ...
    size_t get_num_vertex_components () const {
        return Vertex_components.size ();
    }

    const vertex_format_data* get_vertex_component (size_t index) const {
        return &Vertex_components[index];
    }

    bool resident_vertex_format (
        vertex_format_data::vertex_format format_type) const;

    void add_vertex_component (
...
class poly_list {
    ...
    struct finder {
        ...
        finder (poly_list* _search_list)
            : search_list (_search_list), compare_indices (true),
              vert_to_find (NULL), norm_to_find (NULL) {}
        finder (poly_list* _search_list, vertex* _vert, vec3d* _norm)
            : search_list (_search_list), compare_indices (false),
              vert_to_find (_vert), norm_to_find (_norm) {}
...
public:
    poly_list ()
        : n_verts (0), vert (NULL), norm (NULL), tsb (NULL), submodels (NULL),
          sorted_indices (NULL), currently_allocated (0) {}
    ~poly_list ();
...
public:
    ...
    const uint* get_index () const { return index; }
...
public:
    ...
    vertex_buffer ()
        : flags (0), stride (0), vertex_offset (0), vertex_num_offset (0),
          model_list (NULL) {}
    ...
    void release () {
        ...
        for (std::vector< buffer_data >::iterator tbi = tex_buf.begin ();
             tbi != tex_buf.end (); ++tbi) {
            tbi->release ();
        }
    }
...
enum class BufferUsageHint { Static, Dynamic, Streaming };
...
struct screen  {
    ...
    float aspect,
        clip_aspect;        // Aspect ratio, aspect of clip_width/clip_height
    int rowsize;            // What you need to add to go to next row (includes
                            // bytes_per_pixel)
    ...
    int offset_x_unscaled,
        offset_y_unscaled; // Offsets into the screen, in unscaled dimensions
    int clip_width, clip_height;
    int clip_width_unscaled,
        clip_height_unscaled; // Height and width of clip aread, in unscaled
                              // dimensions
    // center of clip area
    ...
    int clip_left_unscaled, clip_right_unscaled, clip_top_unscaled,
        clip_bottom_unscaled;
    ...
    bool custom_size;
    int rendering_to_texture; // wich texture we are rendering to, -1 if the
                              // back buffer
    int rendering_to_face; // wich face of the texture we are rendering to, -1
                           // if the back buffer
    ...
    // Clears the zbuffer.  If use_zbuffer is FALSE, then zbuffering mode is
    // ignored and zbuffer is always off.
    ...
    void (*gf_update_buffer_data_offset) (
    ...
    void (*gf_shadow_map_start) (
        matrix4* shadow_view_matrix, const matrix* light_matrix);
    ...
    void (*gf_render_model) (
        model_material* material_info, indexed_vertex_source* vert_source,
        vertex_buffer* bufferp, size_t texi);
    void (*gf_render_shield_impact) (
        shield_material* material_info, primitive_type prim_type,
        vertex_layout* layout, int buffer_handle, int n_verts);
    void (*gf_render_primitives) (
        material* material_info, primitive_type prim_type,
        vertex_layout* layout, int offset, int n_verts, int buffer_handle,
        size_t buffer_offset);
    void (*gf_render_primitives_particle) (
        particle_material* material_info, primitive_type prim_type,
        vertex_layout* layout, int offset, int n_verts, int buffer_handle);
    void (*gf_render_primitives_distortion) (
        distortion_material* material_info, primitive_type prim_type,
        vertex_layout* layout, int offset, int n_verts, int buffer_handle);
    void (*gf_render_movie) (
        movie_material* material_info, primitive_type prim_type,
        vertex_layout* layout, int n_verts, int buffer, size_t buffer_offset);
    void (*gf_render_primitives_batched) (
        batched_bitmap_material* material_info, primitive_type prim_type,
        vertex_layout* layout, int offset, int n_verts, int buffer_handle);
    void (*gf_render_nanovg) (
        nanovg_material* material_info, primitive_type prim_type,
        vertex_layout* layout, int offset, int n_verts, int buffer_handle);
    void (*gf_render_decals) (
        decal_material* material_info, primitive_type prim_type,
        vertex_layout* layout, int num_elements,
...
#define GR_MAYBE_CLEAR_RES(bmap)                                        \
    do {                                                                \
        ...
        int bmh = -1;                                                   \
        if (bmap != -1) {                                               \
            bm_get_info (bmap, &bmw, &bmh, NULL, NULL, NULL);           \
...
// # Software Re-added by Kazan --- THIS HAS TO STAY -- It is used by
// standalone!
#define GR_DEFAULT (-1) // set to use default settings
#define GR_STUB (100)
#define GR_OPENGL (104) // Use OpenGl hardware renderer

// resolution constants   - always keep resolutions in ascending order and
// starting from 0
...
void gr_set_screen_scale (
    int x, int y, int zoom_x = -1, int zoom_y = -1,
    int max_x = gr_screen.max_w, int max_y = gr_screen.max_h,
    int center_x = gr_screen.center_w, int center_y = gr_screen.center_h,
    bool force_stretch = false);
void gr_reset_screen_scale ();
bool gr_unsize_screen_pos (
    int* x, int* y, int* w = NULL, int* h = NULL,
    int resize_mode = GR_RESIZE_FULL);
bool gr_resize_screen_pos (
    int* x, int* y, int* w = NULL, int* h = NULL,
    int resize_mode = GR_RESIZE_FULL);
bool gr_unsize_screen_posf (
    float* x, float* y, float* w = NULL, float* h = NULL,
    int resize_mode = GR_RESIZE_FULL);
...
// directly.
extern void gr_printf (int x, int y, const char* format, SCP_FORMAT_STRING...)
    SCP_FORMAT_STRING_ARGS (3, 4);
// same as gr_printf but positions text correctly in menus
extern void
gr_printf_menu (int x, int y, const char* format, SCP_FORMAT_STRING...)
    SCP_FORMAT_STRING_ARGS (3, 4);
// same as gr_printf_menu but accounts for menu zooming
extern void
gr_printf_menu_zoomed (int x, int y, const char* format, SCP_FORMAT_STRING...)
    SCP_FORMAT_STRING_ARGS (3, 4);
// same as gr_printf but doesn't resize for non-standard resolutions
extern void
gr_printf_no_resize (int x, int y, const char* format, SCP_FORMAT_STRING...)
    SCP_FORMAT_STRING_ARGS (3, 4);

// Returns the size of the string in pixels in w and h
extern void
gr_get_string_size (int* w, int* h, const char* text, int len = 9999);

// Returns the height of the current font
extern int gr_get_font_height ();

extern void
gr_set_palette (const char* name, ubyte* palette, int restrict_to_128 = 0);
...
#define gr_translate_texture_matrix \
    GR_CALL (gr_screen.gf_translate_texture_matrix)
...
#define gr_update_buffer_data_offset \
    GR_CALL (gr_screen.gf_update_buffer_data_offset)
#define gr_update_transform_buffer \
    GR_CALL (gr_screen.gf_update_transform_buffer)
...
#define gr_post_process_set_effect \
    GR_CALL (gr_screen.gf_post_process_set_effect)
#define gr_post_process_set_defaults \
    GR_CALL (gr_screen.gf_post_process_set_defaults)
...
#define gr_get_bitmap_from_texture \
    GR_CALL (gr_screen.gf_get_bitmap_from_texture)
...
__inline void gr_render_primitives (
    material* material_info, primitive_type prim_type, vertex_layout* layout,
    int vert_offset, int n_verts, int buffer_handle = -1,
    size_t buffer_offset = 0) {
    (*gr_screen.gf_render_primitives) (
        material_info, prim_type, layout, vert_offset, n_verts, buffer_handle,
        buffer_offset);
}

__inline void gr_render_primitives_particle (
    particle_material* material_info, primitive_type prim_type,
    vertex_layout* layout, int offset, int n_verts, int buffer_handle = -1) {
    (*gr_screen.gf_render_primitives_particle) (
        material_info, prim_type, layout, offset, n_verts, buffer_handle);
}

__inline void gr_render_primitives_batched (
    batched_bitmap_material* material_info, primitive_type prim_type,
    vertex_layout* layout, int offset, int n_verts, int buffer_handle = -1) {
    (*gr_screen.gf_render_primitives_batched) (
        material_info, prim_type, layout, offset, n_verts, buffer_handle);
}

__inline void gr_render_nanovg (
    nanovg_material* material_info, primitive_type prim_type,
    vertex_layout* layout, int offset, int n_verts, int buffer_handle) {
    (*gr_screen.gf_render_nanovg) (
        material_info, prim_type, layout, offset, n_verts, buffer_handle);
}

__inline void gr_render_primitives_distortion (
    distortion_material* material_info, primitive_type prim_type,
    vertex_layout* layout, int offset, int n_verts, int buffer_handle = -1) {
...
inline void gr_render_movie (
    movie_material* material_info, primitive_type prim_type,
    vertex_layout* layout, int n_verts, int buffer, size_t buffer_offset = 0) {
    (*gr_screen.gf_render_movie) (
        material_info, prim_type, layout, n_verts, buffer, buffer_offset);
}

__inline void gr_render_model (
    model_material* material_info, indexed_vertex_source* vert_source,
    vertex_buffer* bufferp, size_t texi) {
...
inline void gr_pop_debug_group () { (*gr_screen.gf_pop_debug_group) (); }
...
inline void gr_bind_uniform_buffer (
    uniform_block_type bind_point, size_t offset, size_t size, int buffer) {
    (*gr_screen.gf_bind_uniform_buffer) (bind_point, offset, size, buffer);
}

inline gr_sync gr_sync_fence () { return (*gr_screen.gf_sync_fence) (); }
...
void gr_init_alphacolor (
    color* clr, int r, int g, int b, int alpha, int type = AC_TYPE_HUD);
...
uint gr_determine_model_shader_flags (
    bool lighting, bool fog, bool textured, bool in_shadow_map,
    bool thruster_scale, bool transform, bool team_color_set, int tmap_flags,
    int spec_map, int glow_map, int normal_map, int height_map,
    int ambient_map, int env_map, int misc_map);
...
// special function for drawing polylines. this function is specifically
// intended for polylines where each section is no more than 90 degrees away
// from a previous section. Moreover, it is _really_ intended for use with 45
// degree angles.
void gr_pline_special (
    std::vector< vec3d >* pts, int thickness,
    int resize_mode = GR_RESIZE_FULL);
 ...
 *
 * Draws the timestamp of the current #Missiontime in the format @c h:mm:ss at
...
class DebugScope {
public:
    explicit DebugScope (const char* name) { gr_push_debug_group (name); }
    ~DebugScope () { gr_pop_debug_group (); }
};
} // namespace graphics
...
/**
 * @brief Allocates storage on the specified GPU heap and stores data in that
 * storage
 ...
 * @param[out] handle_out The handle of the buffer object this data is stored
 * in
 */
