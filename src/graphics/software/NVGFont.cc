...
size_t NVGFont::getTokenLength (const char* string, size_t maxLength) {
    ...
    if (!Unicode_text_mode) {
        // If we are not in unicode mode then we need to handle special
        // characters Otherwise we just assume that the text is UTF-8 encoded
        if (*string >= Lcl_special_chars || *string < 0) return 1;
    }

    const char* nullPtr = strchr (const_cast< char* > (string), '\0');
    const char* nextToken =
        strpbrk (const_cast< char* > (string), TOKEN_SEPARATORS);
...
NVGFont::NVGFont ()
    : m_handle (-1), m_letterSpacing (0.0f), m_size (12.0f),
      m_tabWidth (20.0f) {}
...
float NVGFont::getTextHeight () const { return m_lineHeight; }

extern int
get_char_width_old (font* fnt, ubyte c1, ubyte c2, int* width, int* spacing);
void NVGFont::getStringSize (
    ...
    while ((tokenLength = getTokenLength (s, textLen)) > 0) {
        if (tokenLength > textLen) { tokenLength = textLen; }
        ...
        if (tokenLength == 1) {
            ...
            default:
                if (!Unicode_text_mode) {
                    // Same as before, this code is only needed in non-unicode
                    // mode
        ...
        if (!specialChar) {
            lineWidth +=
                path->textBounds (0.f, 0.f, s, s + tokenLength, nullptr);
        }
        ...
        // see above, tokenLength is at most equal to textLen so this == 0
        // comparision should be safe
        if (textLen == 0) { break; }
    }
