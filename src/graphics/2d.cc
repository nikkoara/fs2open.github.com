...
color_gun *Gr_current_red, *Gr_current_green, *Gr_current_blue,
    *Gr_current_alpha;
...
void gr_set_screen_scale (
    int w, int h, int zoom_w, int zoom_h, int max_w, int max_h, int center_w,
    int center_h, bool force_stretch) {
    ...
    if (do_zoom) {
        float aspect_quotient = ((float)center_w / (float)center_h) /
                                ((float)zoom_w / (float)zoom_h);

        Gr_resize_X = (float)center_w / (float)zoom_w /
                      ((aspect_quotient > 1.0f) ? aspect_quotient : 1.0f);
        Gr_resize_Y = (float)center_h / (float)zoom_h *
                      ((aspect_quotient < 1.0f) ? aspect_quotient : 1.0f);

        Gr_menu_offset_X =
            ((center_w - w * Gr_resize_X) / 2.0f) + gr_screen.center_offset_x;
        Gr_menu_offset_Y =
            ((center_h - h * Gr_resize_Y) / 2.0f) + gr_screen.center_offset_y;

        Gr_menu_zoomed_offset_X = (Gr_menu_offset_X >= 0.0f)
                                      ? Gr_menu_offset_X
                                      : gr_screen.center_offset_x;
        Gr_menu_zoomed_offset_Y = (Gr_menu_offset_Y >= 0.0f)
                                      ? Gr_menu_offset_Y
        ...
        if (force_stretch || Cmdline_stretch_menu) {
            if (Gr_menu_offset_X > (float)gr_screen.center_offset_x) {
                ...
                Gr_menu_offset_X = Gr_menu_zoomed_offset_X =
                    (float)gr_screen.center_offset_x;
            }
            if (Gr_menu_offset_Y > (float)gr_screen.center_offset_y) {
                Gr_resize_Y = Gr_full_center_resize_Y;
                Gr_menu_offset_Y = Gr_menu_zoomed_offset_Y =
                    (float)gr_screen.center_offset_y;
            }
        }
    }
    else {
        if (force_stretch || Cmdline_stretch_menu) {
            ...
            float aspect_quotient =
                ((float)center_w / (float)center_h) / ((float)w / (float)h);
            ...
            Gr_menu_offset_X = Gr_menu_zoomed_offset_X =
                ((aspect_quotient > 1.0f)
                     ? ((center_w - w * Gr_resize_X) / 2.0f)
                     : 0.0f) +
                gr_screen.center_offset_x;
            Gr_menu_offset_Y = Gr_menu_zoomed_offset_Y =
                ((aspect_quotient < 1.0f)
                     ? ((center_h - h * Gr_resize_Y) / 2.0f)
                     : 0.0f) +
                gr_screen.center_offset_y;
        }
    }
    ...
    gr_screen.custom_size =
        (w != max_w || w != center_w || h != max_h || h != center_h);
...
void gr_reset_screen_scale () {
    ...
    if (gr_screen.rendering_to_texture == -1) {
        gr_screen.max_w_unscaled = gr_screen.max_w_unscaled_zoomed =
            (gr_screen.res == GR_1024) ? 1024 : 640;
        gr_screen.max_h_unscaled = gr_screen.max_h_unscaled_zoomed =
            (gr_screen.res == GR_1024) ? 768 : 480;
    }
}

/**
 * This function is to be called if you wish to scale GR_1024 or GR_640 x and y
 * positions or lengths in order to keep the correctly scaled to nonstandard
 * resolutions
 *
...
bool gr_resize_screen_pos (int* x, int* y, int* w, int* h, int resize_mode) {
    ...
    if (x) {
        switch (resize_mode) {
        case GR_RESIZE_FULL: xy_tmp = (*x) * Gr_full_resize_X; break;

        case GR_RESIZE_FULL_CENTER:
            xy_tmp = (*x) * Gr_full_center_resize_X +
                     (float)gr_screen.center_offset_x;
            break;
        ...
        case GR_RESIZE_MENU_NO_OFFSET: xy_tmp = (*x) * Gr_resize_X; break;
        }
        (*x) = fl2ir (xy_tmp);
    }
    ...
    if (y) {
        switch (resize_mode) {
        case GR_RESIZE_FULL: xy_tmp = (*y) * Gr_full_resize_Y; break;

        case GR_RESIZE_FULL_CENTER:
            xy_tmp = (*y) * Gr_full_center_resize_Y +
                     (float)gr_screen.center_offset_y;
            break;
        ...
        case GR_RESIZE_MENU_NO_OFFSET: xy_tmp = (*y) * Gr_resize_Y; break;
        }
        (*y) = fl2ir (xy_tmp);
    }
    ...
    if (w) {
        switch (resize_mode) {
        case GR_RESIZE_FULL: xy_tmp = (*w) * Gr_full_resize_X; break;
        ...
        case GR_RESIZE_MENU:
        case GR_RESIZE_MENU_ZOOMED:
        case GR_RESIZE_MENU_NO_OFFSET: xy_tmp = (*w) * Gr_resize_X; break;
        }
        (*w) = fl2ir (xy_tmp);
    }
    ...
    if (h) {
        switch (resize_mode) {
        case GR_RESIZE_FULL: xy_tmp = (*h) * Gr_full_resize_Y; break;
        ...
        case GR_RESIZE_MENU:
        case GR_RESIZE_MENU_ZOOMED:
        case GR_RESIZE_MENU_NO_OFFSET: xy_tmp = (*h) * Gr_resize_Y; break;
        }
        (*h) = fl2ir (xy_tmp);
    }
...
bool gr_unsize_screen_pos (int* x, int* y, int* w, int* h, int resize_mode) {
    ...
    if (x) {
        ...
        case GR_RESIZE_FULL_CENTER:
            xy_tmp = ((*x) - (float)gr_screen.center_offset_x) /
                     Gr_full_center_resize_X;
            break;
    ...
    if (y) {
        ...
        case GR_RESIZE_FULL_CENTER:
            xy_tmp = ((*y) - (float)gr_screen.center_offset_y) /
                     Gr_full_center_resize_Y;
            break;
 ...
 * @return always true unless error
 */
bool gr_resize_screen_posf (
    float* x, float* y, float* w, float* h, int resize_mode) {
    if (resize_mode == GR_RESIZE_NONE ||
        (!gr_screen.custom_size && (gr_screen.rendering_to_texture == -1))) {
        return false;
    }
...
void gr_close () {
    ...
    case GR_OPENGL: gr_opengl_cleanup (true); break;
...
void gr_set_palette_internal (
    ...
    if (Gr_inited) {
        ...
        if (palette) { memmove (palette, Gr_current_palette, 768); }
    }
}
...
void gr_screen_resize (int width, int height) {
    gr_screen.save_center_w = gr_screen.center_w = gr_screen.save_max_w =
        gr_screen.max_w = gr_screen.max_w_unscaled =
            gr_screen.max_w_unscaled_zoomed = width;
    gr_screen.save_center_h = gr_screen.center_h = gr_screen.save_max_h =
        gr_screen.max_h = gr_screen.max_h_unscaled =
            gr_screen.max_h_unscaled_zoomed = height;
    ...
    gr_screen.clip_bottom = gr_screen.clip_bottom_unscaled =
        gr_screen.max_h - 1;
    ...
    if (gr_screen.custom_size) {
        gr_unsize_screen_pos (
            &gr_screen.max_w_unscaled, &gr_screen.max_h_unscaled);
        gr_unsize_screen_pos (
            &gr_screen.max_w_unscaled_zoomed,
            &gr_screen.max_h_unscaled_zoomed);
        gr_unsize_screen_pos (
...
int gr_get_resolution_class (int width, int height) {
    if ((width >= GR_1024_THRESHOLD_WIDTH) &&
        (height >= GR_1024_THRESHOLD_HEIGHT)) {
        return GR_1024;
    }
    else {
        return GR_640;
...
static bool gr_init_sub (
    ...
    if ((((width == 640) && (height == 480)) ||
        ...
        (aspect_ratio == center_aspect_ratio)) {
    ...
    gr_screen.save_max_w = gr_screen.max_w = gr_screen.max_w_unscaled =
        gr_screen.max_w_unscaled_zoomed = width;
    gr_screen.save_max_h = gr_screen.max_h = gr_screen.max_h_unscaled =
        gr_screen.max_h_unscaled_zoomed = height;
    if (aspect_ratio > center_aspect_ratio) {
        gr_screen.save_center_w = gr_screen.center_w =
            fl2ir (height * center_aspect_ratio);
        ...
        gr_screen.save_center_h = gr_screen.center_h =
            fl2ir (width / center_aspect_ratio);
    }
    else {
        gr_screen.save_center_w = gr_screen.center_w = width;
        gr_screen.save_center_h = gr_screen.center_h = height;
    ...
    Gr_save_full_resize_X = Gr_full_resize_X =
        (float)width / ((res == GR_1024) ? 1024.0f : 640.0f);
    Gr_save_full_resize_Y = Gr_full_resize_Y =
        (float)height / ((res == GR_1024) ? 768.0f : 480.0f);

    Gr_save_full_center_resize_X = Gr_full_center_resize_X =
        (float)gr_screen.center_w / ((res == GR_1024) ? 1024.0f : 640.0f);
    Gr_save_full_center_resize_Y = Gr_full_center_resize_Y =
        (float)gr_screen.center_h / ((res == GR_1024) ? 768.0f : 480.0f);

    if (gr_screen.custom_size && !Cmdline_stretch_menu) {
        float aspect_quotient = center_aspect_ratio / (4.0f / 3.0f);

        Gr_save_resize_X = Gr_resize_X =
            Gr_full_center_resize_X /
            ((aspect_quotient > 1.0f) ? aspect_quotient : 1.0f);
        Gr_save_resize_Y = Gr_resize_Y =
            Gr_full_center_resize_Y *
            ((aspect_quotient < 1.0f) ? aspect_quotient : 1.0f);

        Gr_save_menu_offset_X = Gr_menu_offset_X =
            ((aspect_quotient > 1.0f)
                 ? ((gr_screen.center_w -
                     gr_screen.center_w / aspect_quotient) /
                    2.0f)
                 : 0.0f) +
            gr_screen.center_offset_x;
        Gr_save_menu_offset_Y = Gr_menu_offset_Y =
            ((aspect_quotient < 1.0f)
                 ? ((gr_screen.center_h -
                     gr_screen.center_h * aspect_quotient) /
                    2.0f)
                 : 0.0f) +
            gr_screen.center_offset_y;
    }
    ...
    gr_screen.clip_center_y =
        (gr_screen.clip_top + gr_screen.clip_bottom) * 0.5f;
...
bool gr_init (
    ...
    if (Gr_inited) {
        switch (gr_screen.mode) {
        case GR_OPENGL: gr_opengl_cleanup (false); break;
    ...
    if (Cmdline_center_res != NULL) {
        ...
        if ((sscanf (
                 Cmdline_center_res, "%dx%d", &tmp_center_width,
                 &tmp_center_height) == 2) &&
            (tmp_center_width > 0) && (tmp_center_height > 0)) {
            center_aspect_ratio =
                (float)tmp_center_width / (float)tmp_center_height;
        }
    }
    ...
    // see if we passed good values, and use those instead of the config
    // settings
    ...
    if (d_depth != GR_DEFAULT) { depth = d_depth; }

    if (gr_get_resolution_class (width, height) != GR_640) {
        // check for hi-res interface files so that we can verify our
        // width/height is correct if we don't have it then fall back to
        // 640x480 mode instead
...
// These compiler macros will force windowed mode at the specified resolution
// if built in debug mode.  This helps if you run with the debugger active as
// the game won't be switching from fullscreen to minimized every time you hit
// a breakpoint or warning message.
#ifdef _DEBUG
    ...
    if (center_aspect_ratio <= 0.0f) {
        float aspect_ratio = (float)width / (float)height;
        if (aspect_ratio > 3.5f) { center_aspect_ratio = aspect_ratio * 0.3f; }
        else {
    ...
    if (gr_init_sub (
            std::move (graphicsOps), mode, width, height, depth,
            center_aspect_ratio) == false) {
        return false;
    }
    ...
    if (!running_unittests && Web_cursor == nullptr) {
        if (Is_standalone) {
            // Cursors don't work in standalone mode, just check if the
            // animation exists.
            auto handle = bm_load_animation ("cursorweb");
            if (handle < 0) { missing_installation = true; }
            else {
                bm_release (handle);
            }
        }
        else {
            Web_cursor = io::mouse::CursorManager::get ()->loadCursor (
                "cursorweb", true);
...
void gr_init_alphacolor (
    color* clr, int r, int g, int b, int alpha, int type) {
    CAP (r, 0, 255);
    CAP (g, 0, 255);
    CAP (b, 0, 255);
    CAP (alpha, 0, 255);
...
void gr_set_color_fast (color* dst) {
    if (dst->screen_sig != gr_screen.signature) {
        if (dst->is_alphacolor) {
            gr_init_alphacolor (
                dst, dst->red, dst->green, dst->blue, dst->alpha,
                dst->ac_type);
        }
        else {
...
void gr_bitmap (int _x, int _y, int resize_mode) {
    ...
    if (gr_screen.mode == GR_STUB) { return; }
    ...
    material_set_interface (
        &mat_params, gr_screen.current_bitmap,
        gr_screen.current_alphablend_mode == GR_ALPHABLEND_FILTER ? true
                                                                  : false,
        gr_screen.current_alpha);

    g3_render_primitives_textured (
        &mat_params, verts, 4, PRIM_TYPE_TRIFAN, true);

    // gr_zbuffer_set(saved_zbuffer_mode);
}

void gr_bitmap_uv (
    int _x, int _y, int _w, int _h, float _u0, float _v0, float _u1, float _v1,
    int resize_mode) {
    ...
    material_set_interface (
        &material_params, gr_screen.current_bitmap,
        gr_screen.current_alphablend_mode == GR_ALPHABLEND_FILTER ? true
                                                                  : false,
        gr_screen.current_alpha);
    g3_render_primitives_textured (
        &material_params, verts, 4, PRIM_TYPE_TRIFAN, true);
}
...
void gr_bitmap_list (bitmap_rect_list* list, int n_bm, int resize_mode) {
    ...
    for (int i = 0; i < n_bm; i++) {
        ...
        // if no valid hight or width values were given get some from the
        // bitmap
        if ((l->w <= 0) || (l->h <= 0)) {
            bm_get_info (
                gr_screen.current_bitmap, &l->w, &l->h, NULL, NULL, NULL);
        }
    ...
    material_set_interface (
        ...
        gr_screen.current_alpha);
    g3_render_primitives_textured (
        &mat_params, vert_list, 6 * n_bm, PRIM_TYPE_TRIS, true);
...
void gr_pline_helper (vec3d* out, vec3d* in1, vec3d* in2, int thickness) {
    ...
    // slope of the line
    if (vm_vec_same (in1, in2)) { slope = vmd_zero_vector; }
    else {
 ...
 */
void gr_pline_special (
    std::vector< vec3d >* pts, int thickness, int resize_mode) {
    ...
    for (size_t idx = 0; idx < num_pts - 1; idx++) {
        ...
        if (resize_mode != GR_RESIZE_NONE) {
            for (j = 0; j < 4; j++) {
                gr_resize_screen_posf (
                    &v[j].screen.xyw.x, &v[j].screen.xyw.y, NULL, NULL,
                    resize_mode);
            }
        }

        // draw the polys
        // g3_draw_poly_constant_sw(4, verts, TMAP_FLAG_GOURAUD |
        // TMAP_FLAG_RGB, 0.1f);
        g3_render_primitives_colored (
            &material_def, v, 4, PRIM_TYPE_TRIFAN, true);

        // if we're past the first section, draw a "patch" triangle to fill any
        // gaps
        if (idx > 0) {
            // stuff coords
            v[0].screen.xyw.x = (float)ceil (s1.xyz.x);
            ...
            g3_render_primitives_colored (
                &material_def, v, 3, PRIM_TYPE_TRIFAN, true);
        }
...
int poly_list::find_first_vertex_fast (int idx) {
    uint* first_idx = std::lower_bound (
        sorted_indices, sorted_indices + n_verts, idx,
        finder (this, NULL, NULL));
...
/**
 * Given a list (plist) find the index within the indexed list that the vert at
 * position idx within list is at
 */
...
int poly_list::find_index_fast (poly_list* plist, int idx) {
    // searching for an out of bounds index using the finder means we're trying
    // to find the vert and norm we're passing into the finder instance
    uint* first_idx = std::lower_bound (
        sorted_indices, sorted_indices + n_verts, n_verts,
        finder (this, &plist->vert[idx], &plist->norm[idx]));

    if (first_idx == sorted_indices + n_verts) { return -1; }
...
void poly_list::make_index_buffer (std::vector< int >& vertex_list) {
    ...
    for (j = 0; j < n_verts; j++) {
        if (!nverts_good[j]) { continue; }

        buffer_list_internal.vert[z] = vert[j];
        buffer_list_internal.norm[z] = norm[j];

        if (Cmdline_normal) { buffer_list_internal.tsb[z] = tsb[j]; }
...
poly_list& poly_list::operator= (const poly_list& other_list) {
    ...
    memcpy (
        submodels, other_list.submodels, sizeof (int) * other_list.n_verts);

    memcpy (
        sorted_indices, other_list.sorted_indices,
        sizeof (uint) * other_list.n_verts);
...
void poly_list::generate_sorted_index_list () {
    for (int j = 0; j < n_verts; ++j) { sorted_indices[j] = j; }
...
void gr_set_bitmap (
    int bitmap_num, int alphablend_mode, int bitblt_mode, float alpha) {
...
void gr_print_timestamp (int x, int y, fix timestamp, int resize_mode) {
    ...
    std::string time;
    sprintf (
        time, "%.1d:%.2d:%.2d", (seconds / 3600) % 10, (seconds / 60) % 60,
        seconds % 60);
...
static std::unique_ptr< graphics::util::UniformBufferManager >
    uniform_buffer_managers[static_cast< size_t > (
        uniform_block_type::NUM_BLOCK_TYPES)];

static void uniform_buffer_managers_init () {
    for (size_t i = 0;
         i < static_cast< size_t > (uniform_block_type::NUM_BLOCK_TYPES);
         ++i) {
        auto enumVal = static_cast< uniform_block_type > (i);

        uniform_buffer_managers[i].reset (
            new graphics::util::UniformBufferManager (enumVal));
    }
}
static void uniform_buffer_managers_deinit () {
    for (auto& manager : uniform_buffer_managers) { manager.reset (); }
}
static void uniform_buffer_managers_retire_buffers () {
...
graphics::util::UniformBuffer*
gr_get_uniform_buffer (uniform_block_type type) {
...
std::vector< DisplayData > gr_enumerate_displays () {
    ...
    for (auto i = 0; i < num_displays; ++i) {
        ...
        if (name != nullptr) { display.name = name; }

        auto num_mods = SDL_GetNumDisplayModes (i);
        for (auto j = 0; j < num_mods; ++j) {
            SDL_DisplayMode mode;
            if (SDL_GetDisplayMode (i, j, &mode) != 0) { continue; }
            ...
            if (SDL_ISPIXELFORMAT_ALPHA (mode.format)) {
                ...
                // Fix a few values
                if (sdlBits == 24) { videoMode.bit_depth = 32; }
                else if (sdlBits == 15) {
...
void vertex_layout::add_vertex_component (
    vertex_format_data::vertex_format format_type, size_t stride,
    size_t offset) {
    ...
    if (Vertex_mask == 0) {
        // This is the first element so we need to initialize the global stride
        // here
...
static void gpu_heap_deinit () {
    for (auto& heap : gpu_heaps) { heap.reset (); }
}
...
void gr_heap_allocate (
    GpuHeap heap_type, size_t size, void* data, size_t& offset_out,
    int& handle_out) {
