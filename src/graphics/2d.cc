        ...
        int center_h, bool force_stretch)
{
        ...
        if (do_zoom) {
                ...
                if (force_stretch || Cmdline_stretch_menu) {
                        ...
                        Gr_menu_offset_X = Gr_menu_zoomed_offset_X = ((aspect_quotient > 1.0f)
                                                                              ? ((center_w - w * Gr_resize_X) / 2.0f)
                                                                              : 0.0f)
                                                                     + gr_screen.center_offset_x;
                        Gr_menu_offset_Y = Gr_menu_zoomed_offset_Y = ((aspect_quotient < 1.0f)
        ...
        if (gr_screen.rendering_to_texture == -1) {
                ...
                if (do_zoom) {
                        gr_screen.max_w_unscaled_zoomed = gr_screen.max_w_unscaled + int(Gr_menu_offset_X * 2.0f / Gr_resize_X);
                        gr_screen.max_h_unscaled_zoomed = gr_screen.max_h_unscaled + int(Gr_menu_offset_Y * 2.0f / Gr_resize_Y);
        ...
        int width, int height, int depth, float center_aspect_ratio)
{
        ...
        if (gr_screen.custom_size && !Cmdline_stretch_menu) {
                ...
                Gr_save_menu_offset_X = Gr_menu_offset_X = ((aspect_quotient > 1.0f)
                                                                    ? ((gr_screen.center_w - gr_screen.center_w / aspect_quotient) / 2.0f)
                                                                    : 0.0f)
                                                           + gr_screen.center_offset_x;
                Gr_save_menu_offset_Y = Gr_menu_offset_Y = ((aspect_quotient < 1.0f)
                                                                    ? ((gr_screen.center_h - gr_screen.center_h * aspect_quotient) / 2.0f)
                                                                    : 0.0f)
                                                           + gr_screen.center_offset_y;
        } else {
...
int poly_list::find_first_vertex(int idx)
{
        ...
        for (int i = 0; i < idx; i++) {
                if ((*p_norm == *o_norm) && (p_vert->world == o_vert->world) && (p_vert->texture_position == o_vert->texture_position)) {
                        return i;
                }
