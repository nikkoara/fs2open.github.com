...
static void gr_flash_internal (int r, int g, int b, int a, bool alpha_flash) {
    ...
    vert_def.add_vertex_component (
        vertex_format_data::SCREEN_POS, sizeof (int) * 2, 0);

    gr_render_primitives_2d_immediate (
        &render_material, PRIM_TYPE_TRISTRIP, &vert_def, 4, glVertices,
        sizeof (int) * 8);
}
...
static void draw_textured_quad (
    material* mat, float x1, float y1, float u1, float v1, float x2, float y2,
    float u2, float v2) {
    ...
    vert_def.add_vertex_component (
        vertex_format_data::POSITION2, sizeof (float) * 4, 0);
    vert_def.add_vertex_component (
        vertex_format_data::TEX_COORD2, sizeof (float) * 4,
        sizeof (float) * 2);

    gr_render_primitives_immediate (
        mat, PRIM_TYPE_TRISTRIP, &vert_def, 4, glVertices,
        sizeof (float) * 4 * 4);
}

static void bitmap_ex_internal (
    int x, int y, int w, int h, int sx, int sy, int resize_mode, bool aabitmap,
    bool mirror, color* clr) {
    if ((w < 1) || (h < 1)) { return; }

    if (aabitmap && !clr->is_alphacolor) { return; }
...
void gr_aabitmap (int x, int y, int resize_mode, bool mirror) {
    ...
    int clip_left =
        ((do_resize) ? gr_screen.clip_left_unscaled : gr_screen.clip_left);
    int clip_right =
        ((do_resize) ? gr_screen.clip_right_unscaled : gr_screen.clip_right);
    int clip_top =
        ((do_resize) ? gr_screen.clip_top_unscaled : gr_screen.clip_top);
    int clip_bottom =
        ((do_resize) ? gr_screen.clip_bottom_unscaled : gr_screen.clip_bottom);
    ...
    bitmap_ex_internal (
        dx1, dy1, (dx2 - dx1 + 1), (dy2 - dy1 + 1), sx, sy, resize_mode, true,
        mirror, &gr_screen.current_color);
}
void gr_aabitmap_ex (
    int x, int y, int w, int h, int sx, int sy, int resize_mode, bool mirror) {
    int reclip;
#ifndef NDEBUG
    int count = 0;
#endif
...
void gr_bitmap_ex (
    int x, int y, int w, int h, int sx, int sy, int resize_mode) {
    int reclip;
#ifndef NDEBUG
    int count = 0;
#endif
    ...
    // regions.
    bitmap_ex_internal (
        dx1, dy1, (dx2 - dx1 + 1), (dy2 - dy1 + 1), sx, sy, resize_mode, false,
        false, &clr);
}
...
namespace font {
extern int
get_char_width_old (font* fnt, ubyte c1, ubyte c2, int* width, int* spacing);
}

static void gr_string_old (
    float sx, float sy, const char* s, const char* end, font::font* fontData,
    float height, int resize_mode) {
    ...
    render_mat.set_color (
        gr_screen.current_color.red, gr_screen.current_color.green,
    ...
    vert_def.add_vertex_component (
        vertex_format_data::POSITION2, sizeof (v4), (int)offsetof (v4, x));
    vert_def.add_vertex_component (
        vertex_format_data::TEX_COORD2, sizeof (v4), (int)offsetof (v4, u));
    ...
    while (s < end) {
        ...
        letter = font::get_char_width_old (
            fontData, (ubyte)s[0], (ubyte)s[1], &width, &spacing);
        s++;
        ...
        if ((wc < 1) || (hc < 1)) { continue; }

        int u = fontData->bm_u[letter];
        int v = fontData->bm_v[letter];

        x1 = xc +
             ((do_resize) ? gr_screen.offset_x_unscaled : gr_screen.offset_x);
        y1 = yc +
        ...
        if (buffer_offset == MAX_VERTS_PER_DRAW) {
            gr_render_primitives_immediate (
                &render_mat, PRIM_TYPE_TRIS, &vert_def, buffer_offset,
...
void setupTransforms (graphics::paths::PathRenderer* path, int resize_mode) {
    ...
    int clip_width =
        ((do_resize) ? gr_screen.clip_width_unscaled : gr_screen.clip_width);
    int clip_height =
        ((do_resize) ? gr_screen.clip_height_unscaled : gr_screen.clip_height);
...
void gr_string (
    float sx, float sy, const char* s, int resize_mode, int in_length) {
    ...
    size_t length;
    if (in_length < 0) { length = strlen (s); }
    else {
    ...
    if (currentFont->getType () == VFNT_FONT) {
        ...
        gr_string_old (
            sx, sy, s, s + length, fontData, fnt->getHeight (), resize_mode);
    }
        ...
        gr_resize_screen_posf (
            nullptr, nullptr, &scaleX, &scaleY, resize_mode);
        ...
        // Do a two pass algorithm, first render text using NanoVG, then render
        // old characters
        for (int pass = 0; pass < 2; ++pass) {
            ...
            size_t tokenLength;
            while ((tokenLength = NVGFont::getTokenLength (text, textLen)) >
                   0) {
                textLen -= tokenLength;
                ...
                if (tokenLength == 1) {
                    ...
                    case '\r':
                        ...
                        break;
                    default:
                        // Only do special character handling if we are not in
                        // unicode mode. Otherwise this is just a normal
                        // character for us
                ...
                if (specialChar) {
                    if (pass == 1) {
                        // We compute the top offset of the special character
                        // by aligning it to the base line of the string This
                        // is done by moving to the base line of the string by
                        // adding the ascender value and then accounting for
                        // the height of the text with the height of the
                        // special font
                        auto yOffset =
                            nvgFont->getTopOffset () +
                            (nvgFont->getAscender () -
                             nvgFont->getSpecialCharacterFont ()->h);

                        gr_string_old (
                            sx + x * scaleX, sy + (y + yOffset) * scaleY, text,
                            text + 1, nvgFont->getSpecialCharacterFont (),
                            nvgFont->getHeight (), resize_mode);
                    }
                    ...
                    if (doRender && tokenLength > 0) {
                        ...
                        if (pass == 0) {
                            path->text (
                                currentX, currentY, text, text + tokenLength);
                        }

                        advance = path->textBounds (
                            0.f, 0.f, text, text + tokenLength, nullptr);
                        x += advance * invscaleX;
                    }
                }
            ...
            if (pass == 0) { endDrawing (path); }

            if (!twoPassRequired) { break; }
        }
    }
    else {
...
size_t gr_add_to_immediate_buffer (size_t size, void* data) {
    ...
    if (immediate_buffer_offset + size > immediate_buffer_size) {
        ...
        immediate_buffer_size +=
            MAX (IMMEDIATE_BUFFER_RESIZE_BLOCK_SIZE, size);

        gr_update_buffer_data (
            gr_immediate_buffer_handle, immediate_buffer_size, NULL);
    }

    // only update a section of the immediate vertex buffer
    gr_update_buffer_data_offset (
        gr_immediate_buffer_handle, immediate_buffer_offset, size, data);
...
void gr_render_primitives_immediate (
    ...
    gr_render_primitives (
        material_info, prim_type, layout, 0, n_verts,
        gr_immediate_buffer_handle, offset);
}
...
void gr_render_primitives_2d_immediate (
    ...
    gr_render_primitives_immediate (
        material_info, prim_type, layout, n_verts, data, size);
