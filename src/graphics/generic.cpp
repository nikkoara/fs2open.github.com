...
// These two functions find if a bitmap or animation exists by filename, no
// extension needed.
bool generic_bitmap_exists (const char* filename) {
    return cf_exists_full_ext (
               filename, CF_TYPE_ANY, BM_NUM_TYPES, bm_ext_list) != 0;
}

bool generic_anim_exists (const char* filename) {
    return cf_exists_full_ext (
               filename, CF_TYPE_ANY, BM_ANI_NUM_TYPES, bm_ani_ext_list) != 0;
}

// Goober5000
int generic_anim_init_and_stream (
    generic_anim* ga, const char* anim_filename, BM_TYPE bg_type,
    bool attempt_hi_res) {
    int stream_result = -1;
...
// Goober5000
void generic_anim_init (generic_anim* ga) { generic_anim_init (ga, NULL); }
...
void generic_bitmap_init (generic_bitmap* gb, const char* filename) {
    if (filename == NULL) { gb->filename[0] = '\0'; }
    else {
...
int generic_anim_load (generic_anim* ga) {
    ...
    ga->first_frame = bm_load_animation (
        ga->filename, &ga->num_frames, &fps, &ga->keyframe, &ga->total_time);
    // mprintf(("generic_anim_load: %s - keyframe = %d\n", ga->filename,
    // ga->keyframe));

    if (ga->first_frame < 0) return -1;
...
int generic_anim_stream (generic_anim* ga, const bool cache) {
    ...
    if (ga->type == BM_TYPE_ANI) {
        ...
        ga->bitmap_id = bm_create (
            bpp, ga->width, ga->height, ga->buffer,
            (bpp == 8) ? BMP_AABITMAP : 0);
        ...
        if (ga->png.anim == nullptr) {
            ...
            nprintf (
                ("apng", "apng read OK (%ix%i@%i) duration (%f)\n",
        ...
        ga->bitmap_id = bm_create (
            ga->png.anim->bpp, ga->width, ga->height, ga->buffer, 0);
    }
    else {
        bpp = 32;
        if (ga->use_hud_color) bpp = 8;
        bm_load_and_parse_eff (
            ga->filename, CF_TYPE_ANY, &ga->num_frames, &anim_fps,
            &ga->keyframe, 0);
        char* p = strrchr (ga->filename, '.');
        if (p) *p = 0;
        char frame_name[MAX_FILENAME_LEN];
        if (snprintf (frame_name, MAX_FILENAME_LEN, "%s_0000", ga->filename) >=
            MAX_FILENAME_LEN) {
...
int generic_bitmap_load (generic_bitmap* gb) {
    if (!VALID_FNAME (gb->filename)) return -1;

    gb->bitmap_id = bm_load (gb->filename);

    if (gb->bitmap_id < 0) return -1;
    ...
    if (ga->num_frames > 0) {
        if (ga->streaming) {
            ...
            if (ga->type == BM_TYPE_EFF) {
                if (ga->eff.next_frame >= 0) bm_release (ga->eff.next_frame);
                if (ga->bitmap_id >= 0) bm_release (ga->bitmap_id);
            }
        ...
        if (ga->buffer) { bm_release (ga->bitmap_id); }
    }
...
void generic_render_eff_stream (generic_anim* ga) {
    ...
    char frame_name[MAX_FILENAME_LEN];
    if (snprintf (
            frame_name, MAX_FILENAME_LEN, "%s_%.4d", ga->filename,
            ga->current_frame) >= MAX_FILENAME_LEN) {
        // Make sure the string is null terminated
        frame_name[MAX_FILENAME_LEN - 1] = '\0';
    }
    if (bm_reload (ga->eff.next_frame, frame_name) == ga->eff.next_frame) {
        bitmap* next_frame_bmp = bm_lock (
            ga->eff.next_frame, bpp,
            (bpp == 8) ? BMP_AABITMAP : BMP_TEX_NONCOMP, true);
        if (next_frame_bmp->data)
            gr_update_texture (
                ga->bitmap_id, bpp, (ubyte*)next_frame_bmp->data, ga->width,
                ga->height);
        bm_unlock (ga->eff.next_frame);
        bm_unload (ga->eff.next_frame, 0, true);
        if (ga->current_frame == ga->num_frames - 1) {
            if (snprintf (
                    frame_name, MAX_FILENAME_LEN, "%s_0001", ga->filename) >=
                MAX_FILENAME_LEN) {
                // Make sure the string is null terminated
                frame_name[MAX_FILENAME_LEN - 1] = '\0';
            }
...
void generic_render_ani_stream (generic_anim* ga) {
    ...
    if (ga->direction & GENERIC_ANIM_DIRECTION_BACKWARDS) {
        ...
        if (ga->ani.animation->flags & ANF_STREAMED) {
            ga->ani.instance->file_offset =
                ga->ani.animation->file_offset +
                ga->ani.animation->keys[ga->current_frame].offset;
        }
        else {
            ga->ani.instance->data =
                ga->ani.animation->data +
                ga->ani.animation->keys[ga->current_frame].offset;
        }
        if (ga->ani.animation->flags & ANF_STREAMED) {
            ga->ani.instance->file_offset = unpack_frame_from_file (
                ga->ani.instance, ga->buffer, ga->width * ga->height,
            ...
            ga->ani.instance->data = unpack_frame (
                ga->ani.instance, ga->ani.instance->data, ga->buffer,
                ga->width * ga->height,
                (ga->ani.instance->xlate_pal)
                    ? ga->ani.animation->palette_translation
                    : NULL,
        ...
        // looping back
        if ((ga->current_frame == 0) ||
            (ga->current_frame < ga->previous_frame)) {
            // go back to keyframe if there is one
            if (ga->keyframe && (ga->current_frame > 0)) {
                if (ga->ani.animation->flags & ANF_STREAMED) {
                    ga->ani.instance->file_offset =
                        ga->ani.animation->file_offset + ga->keyoffset;
                }
                else {
                    ga->ani.instance->data =
                        ga->ani.animation->data + ga->keyoffset;
                }
    ...
    // we need to use this because performance is worse if we flush the gfx
    // card buffer
    ...
    // mprintf(("frame crc = %08X\n", cf_add_chksum_long(0, ga->buffer,
    // ga->width * ga->height * (bpp >> 3))));
...
void generic_render_png_stream (generic_anim* ga) {
    ...
    try {
        if ((ga->direction & GENERIC_ANIM_DIRECTION_BACKWARDS) &&
            ...
            // in that case (i.e. previous_frame is -1) we actually want to
            // call next_frame, in order to retrieve the 1st frame of the
            // animation
        ...
        nprintf (
            ("apng", "Unable to get next/prev apng frame: %s\n", e.what ()));
        return;
    }

    bm_lock (
        ga->bitmap_id, ga->png.anim->bpp, BMP_TEX_NONCOMP,
        true); // lock in 32 bpp for png
    int bpp = ga->png.anim->bpp;
    if (ga->use_hud_color) { bpp = 8; }
    gr_update_texture (
        ga->bitmap_id, bpp, ga->buffer, ga->width,
        ga->height); // this will convert to 8 bpp if required
    bm_unlock (ga->bitmap_id);
}

/*
 * @brief calculate current frame for fixed frame delay animation formats (ani
 * & eff)
 *
 * @param [in] *ga  animation data
 * @param [in] frametime  how long this frame took
 */
void generic_anim_render_fixed_frame_delay (
    generic_anim* ga, float frametime) {
    float keytime = 0.0;

    if (ga->keyframe)
        keytime =
            (ga->total_time * ((float)ga->keyframe / (float)ga->num_frames));
        ...
        if (ga->direction & GENERIC_ANIM_DIRECTION_BACKWARDS) {
            ...
            if (ga->keyframe && (ga->anim_time >= keytime)) {
                ...
                if ((ga->direction & GENERIC_ANIM_DIRECTION_NOLOOP) &&
                    ga->anim_time <= 0.0) {
                    ga->anim_time =
                        0; // stop on first frame when playing in reverse
                }
                else {
                    while (ga->anim_time <= 0.0)
                        ga->anim_time +=
                            ga->total_time; // make sure we're always positive,
                                            // so we can go back to the end
                }
            }
        }
        else {
            ...
            if (ga->anim_time >= ga->total_time) {
                if (ga->direction & GENERIC_ANIM_DIRECTION_NOLOOP) {
                    ga->anim_time =
                        ga->total_time -
                        0.001f; // stop on last frame when playing - if it's
                                // equal we jump to the first frame
                }
                if (!ga->done_playing) {
    ...
    if (ga->num_frames > 0) {
        ...
        if (ga->done_playing && ga->keyframe) {
            ga->anim_time =
                fmod (ga->anim_time - keytime, ga->total_time - keytime) +
                keytime;
        }
        else {
        ...
        if (ga->streaming) {
            ...
            if (ga->type == BM_TYPE_ANI) { generic_render_ani_stream (ga); }
            else {
...
/*
 * @brief calculate current frame for variable frame delay animation formats
 * (e.g. apng)
 *
 * @param [in] *ga  animation data
 * @param [in] frametime  how long this frame took
 ...
 *
 * @note
 * uses both time & frame counts to determine end state; so that if the anims
 * playing can't be processed fast enough, all frames will still play, rather
 * than the end frames being skipped
 */
void generic_anim_render_variable_frame_delay (
    ...
    if ((ga->direction & GENERIC_ANIM_DIRECTION_PAUSED) == 0) {
        if (ga->direction & GENERIC_ANIM_DIRECTION_BACKWARDS) {
            ...
            if (ga->anim_time >= ga->total_time &&
                ...
                if (ga->direction & GENERIC_ANIM_DIRECTION_NOLOOP) {
                    ga->anim_time =
                        ga->total_time; // stop on last frame when playing
                }
                else {
                    // loop back to start
                    ga->anim_time = 0.0f;
    ...
    if (ga->num_frames > 0) {
        ...
        // jumping forwards multiple frames will just exacerbate slowdowns as
        // multiple frames would need to be composed
        if (ga->direction & GENERIC_ANIM_DIRECTION_BACKWARDS) {
            if (ga->anim_time <=
        ...
        // non-streaming generic anims
        generic_render_png_stream (ga);
        gr_set_bitmap (
            ga->bitmap_id, GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL, alpha);
    }
}
    ...
    if (ga->num_frames > 0) {
        ...
        if (ga->use_hud_color) {
            ...
            if (ge == nullptr) {
                ...
                gr_bitmap_uv (
                    x, y, ge->width, ge->height, ge->u0, ge->v0, ge->u1,
                    ge->v1, GR_RESIZE_NONE);
            }
        }
    }
}
