...
bool shadows_obj_in_frustum (
    object* objp, matrix* light_orient, vec3d* min, vec3d* max) {
    vec3d pos, pos_rot;
...
void shadows_construct_light_proj (light_frustum_info* shadow_data) {
    ...
    shadow_data->proj_matrix.a1d[0] =
        2.0f / (shadow_data->max.xyz.x - shadow_data->min.xyz.x);
    shadow_data->proj_matrix.a1d[5] =
        2.0f / (shadow_data->max.xyz.y - shadow_data->min.xyz.y);
    shadow_data->proj_matrix.a1d[10] =
        -2.0f / (shadow_data->max.xyz.z - shadow_data->min.xyz.z);
    shadow_data->proj_matrix.a1d[12] =
        -(shadow_data->max.xyz.x + shadow_data->min.xyz.x) /
        (shadow_data->max.xyz.x - shadow_data->min.xyz.x);
    shadow_data->proj_matrix.a1d[13] =
        -(shadow_data->max.xyz.y + shadow_data->min.xyz.y) /
        (shadow_data->max.xyz.y - shadow_data->min.xyz.y);
    shadow_data->proj_matrix.a1d[14] =
        -(shadow_data->max.xyz.z + shadow_data->min.xyz.z) /
        (shadow_data->max.xyz.z - shadow_data->min.xyz.z);
    shadow_data->proj_matrix.a1d[15] = 1.0f;
}

void shadows_debug_show_frustum (
    matrix* orient, vec3d* pos, float fov, float aspect, float z_near,
    float z_far) {
    // find the widths and heights of the near plane and far plane to determine
    // the points of this frustum
...
void shadows_construct_light_frustum (
    light_frustum_info* shadow_data, matrix* light_matrix, matrix* orient,
...
matrix shadows_start_render (
    ...
    if (Static_light.empty ()) return vmd_identity_matrix;
    ...
    vm_vector_2_matrix (
        &light_matrix, &light_dir, &eye_orient->vec.uvec, NULL);

    shadows_construct_light_frustum (
        &Shadow_frustums[0], &light_matrix, eye_orient, eye_pos, fov, aspect,
        0.0f, veryneardist);
    shadows_construct_light_frustum (
        &Shadow_frustums[1], &light_matrix, eye_orient, eye_pos, fov, aspect,
        veryneardist - (veryneardist - 0.0f) * 0.2f, neardist);
    shadows_construct_light_frustum (
        &Shadow_frustums[2], &light_matrix, eye_orient, eye_pos, fov, aspect,
        neardist - (neardist - veryneardist) * 0.2f, middist);
    shadows_construct_light_frustum (
        &Shadow_frustums[3], &light_matrix, eye_orient, eye_pos, fov, aspect,
        middist - (middist - neardist) * 0.2f, fardist);
...
void shadows_end_render () { gr_shadow_map_end (); }
...
void shadows_render_all (float fov, matrix* eye_orient, vec3d* eye_pos) {
    ...
    if (Static_light.empty ()) { return; }

    if (!Cmdline_shadow_quality) { return; }

    // shadows_debug_show_frustum(&Player_obj->orient, &Player_obj->pos, fov,
    // gr_screen.clip_aspect, Min_draw_distance, 3000.0f);

    gr_end_proj_matrix ();
    gr_end_view_matrix ();

    // these cascade distances are a result of some arbitrary tuning to give a
    // good balance of quality and banding. maybe we could use a more
    // programmatic algorithim?
    matrix light_matrix = shadows_start_render (
        eye_orient, eye_pos, fov, gr_screen.clip_aspect, 200.0f, 600.0f,
        2500.0f, 8000.0f);
    ...
    for (int i = 0; i <= Highest_object_index; i++, objp++) {
        ...
        for (int j = 0; j < MAX_SHADOW_CASCADES; ++j) {
            if (shadows_obj_in_frustum (
                    objp, &light_matrix, &Shadow_frustums[j].min,
                    &Shadow_frustums[j].max)) {
                cull = false;
                break;
            }
        }
        ...
        case OBJ_ASTEROID: {
            ...
            render_info.set_flags (
                MR_IS_ASTEROID | MR_NO_TEXTURING | MR_NO_LIGHTING);
        ...
        case OBJ_DEBRIS: {
            ...
            if (!(db->flags & DEBRIS_USED)) { continue; }
            ...
            submodel_render_queue (
                &render_info, &scene, db->model_num, db->submodel_num,
                &objp->orient, &objp->pos);
        } break;
        }
    }
