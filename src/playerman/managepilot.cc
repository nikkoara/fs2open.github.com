...
{
        return popup(
                PF_USE_AFFIRMATIVE_ICON | PF_USE_NEGATIVE_ICON | PF_TITLE_RED | PF_TITLE_BIG,
...
void player_set_squad(player *p, char *squad_name)
{
        ...
        if (Game_mode & GM_MULTIPLAYER) {
                strcpy(p->m_squad_name, squad_name);
        } else {
...
void player::assign(const player *other)
{
        ...
        for (std::vector< sexp_variable >::const_iterator ii = other->variables.begin();
