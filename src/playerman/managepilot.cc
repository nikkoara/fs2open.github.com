...
// internal function to delete a player file.  Called after a pilot is
...
void init_new_pilot (player* p, int reset) {
    ...
    if (reset) {
        ...
        // set the default detail level based on tabling rather than the above
        // method
...
#error Code in ManagePilot assumes NUM_DEFAULT_DETAIL_LEVELS = 4
        ...
        WARNINGF (LOCATION, "Setting detail level to %d because of new pilot",cur_speed);
        Use_mouse_to_fly = 1;
        Mouse_sensitivity = 4;
        Joy_sensitivity = 9;
...
// Generates a list of available campaigns. Sets active campaign to
// "freespace2.fc2", or if that is unavailable, the first campaign found
void pilot_set_start_campaign (player* p) {
    ...
    // manually below
    rc = cf_get_file_list (
        MAX_CAMPAIGNS, campaign_file_list, CF_TYPE_MISSIONS, wild_card,
        CF_SORT_NONE);
...
void pilot_set_random_squad_pic (player* p) {
    ...
    if (Num_pilot_squad_images <= 0) {
        ...
        player_set_squad_bitmap (
            p, Pilot_squad_images_arr[random_index], false);
    }
}

// format a pilot's callsign into a "personal" form - ie, adding a 's or just
// an ' as appropriate
void pilot_format_callsign_personal (char* in_callsign, char* out_callsign) {
    // don't do anything if we've got invalid strings
    if ((in_callsign == NULL) || (out_callsign == NULL)) { return; }
...
// throw up a popup asking the user to verify the overwrite of an existing
// pilot name 1 == ok to overwrite, 0 == not ok
int pilot_verify_overwrite () {
    return popup (
        PF_USE_AFFIRMATIVE_ICON | PF_USE_NEGATIVE_ICON | PF_TITLE_RED |
            PF_TITLE_BIG,
        2, XSTR ("&No", 47), XSTR ("&Yes", 48),
        XSTR ("Warning\nA duplicate pilot exists\nOverwrite?", 49));
}

// load up the list of pilot image filenames (do this at game startup as well
// as barracks startup)
void pilot_load_pic_list () {
    Num_pilot_images = 0;

    // load pilot images from the player images directory
    Num_pilot_images = cf_get_file_list_preallocated (
        MAX_PILOT_IMAGES, Pilot_images_arr, Pilot_image_names,
...
void pilot_load_squad_pic_list () {
    ...
    // load pilot images from the player images directory
    Num_pilot_squad_images = cf_get_file_list_preallocated (
        MAX_PILOT_IMAGES, Pilot_squad_images_arr, Pilot_squad_image_names,
        CF_TYPE_SQUAD_IMAGES, NOX ("*.pcx"));

    // sort all filenames
    cf_sort_filenames (
        Num_pilot_squad_images, Pilot_squad_image_names, CF_SORT_NAME);
}
...
void player_set_squad_bitmap (player* p, const char* fname, bool ismulti) {
    ...
    char* squad_pic_p;
    if (ismulti) { squad_pic_p = p->m_squad_filename; }
    else {
        squad_pic_p = p->s_squad_filename;
    }

    // if he has another bitmap already - unload it
    if (p->insignia_texture > 0) { bm_release (p->insignia_texture); }
...
void player::reset () {
    ...
    if (Player == this) { Pilot.reset_stats (); }
...
void player::assign (const player* other) {
    ...
    for (i = 0; i < MAX_KEYED_TARGETS; i++) {
        if (other->keyed_targets[i].next == NULL)
            ...
            size_t index =
                (other->keyed_targets[i].next - &other->keyed_targets[0]);
        ...
        if (other->keyed_targets[i].prev == NULL)
            ...
            size_t index =
                (other->keyed_targets[i].prev - &other->keyed_targets[0]);
    ...
    for (std::vector< sexp_variable >::const_iterator ii =
             other->variables.begin ();
         ii != other->variables.end (); ++ii) {
        sexp_variable temp;
