...
#define MAX_KEYED_TARGETS \
    8 // number of hot keys available to assign targets to
...
#define PLAYER_FLAGS_DIST_TO_BE_KILLED \
    (1 << 14) // the pilot has been warned about distance and will be killed
              // after message finishes playing
#define PLAYER_FLAGS_KILLED_BY_ENGINE_WASH \
    (1 << 15) // player was killed by engine wash
#define PLAYER_FLAGS_KILLED_SELF_UNKNOWN \
    (1 << 16) // player died by his own hand
...
#define LGC_B_OVERRIDE \
    (1 << 4) // allow lua to override current button commands
#define LGC_B_ADDITIVE \
    (1 << 5) // allow lua add current lua button commands to current commands

#define LGC_B_POLL_ALL \
    (1 << 6) // tell game code to pass all button commands to lua.
...
public:
    ...
    char short_callsign[CALLSIGN_LEN + 1]; // callsign truncated to
                                           // SHORT_CALLSIGN_PIXEL_W pixels
    int short_callsign_width; // useful for mutliplayer chat boxes.

    char image_filename[MAX_FILENAME_LEN + 1];   // filename of the image for
                                                 // this pilot
    char s_squad_filename[MAX_FILENAME_LEN + 1]; // filename of the squad image
                                                 // for this pilot
    char s_squad_name[NAME_LENGTH + 1];          // pilot's squadron name
    char
        m_squad_filename[MAX_FILENAME_LEN + 1]; // filename of the squad image
                                                // for this pilot (multiplayer)
    char m_squad_name[NAME_LENGTH + 1]; // pilot's squadron name (multiplayer)

    char current_campaign[MAX_FILENAME_LEN + 1]; // Name of the currently
                                                 // active campaign, or
                                                 // zero-length string if none
    ...
    int lead_target_cheat;     // whether cheat for firing at lead indicator is
                               // active
    int lead_indicator_active; // flag that indicates the lead indicator is
                               // enabled
    ...
    int lock_indicator_start_x; // 2D screen x-coordinate of where lock
                                // indicator originated
    int lock_indicator_start_y; // 2D screen y-coordinate of where lock
                                // indicator originated
    int lock_indicator_visible; // flag indicating if the lock indicator is on
                                // screen or not
    ...
    int allow_warn_timestamp; // Timestamp used to regulate how often a player
                              // might receive warning messages about ships
                              // attacking.
    int warn_count; // number of attack warnings player has received this
                    // mission
    float damage_this_burst; // amount of damage done this frame to friendly
                             // craft

    sound_handle
        repair_sound_loop; // Sound id for ship repair looping sound, this is
                           // in the player file since the repair sound only
                           // plays when Player ship is getting repaired
    ...
    int allow_praise_timestamp; // timestamp marking time until next praise is
                                // allowed
    int praise_delay_timestamp; // timestamp used to delay a praise by a second
                                // or two

    int ask_help_count; // number of times wingmen have asked for help this
                        // mission
    int allow_ask_help_timestamp; // timestamp marking time until next 'ask
                                  // help' is allowed

    int scream_count; // number of wingman screams received this mission
    int allow_scream_timestamp; // timestamp marking time until next wingman
                                // scream is allowed

    int low_ammo_complaint_count; // number of complaints about low ammo
                                  // received in this mission
    int allow_ammo_timestamp; // timestamp marking time until next 'low ammo'
                              // complaint is allowed

    int praise_self_count;     // number of boasts about kills received in this
                               // mission
    int praise_self_timestamp; // timestamp marking time until next boast is
                               // allowed

    int subsys_in_view; // set to -1 when this information needs to be
                        // re-evaluated
    int request_repair_timestamp; // timestamp marking time until next time we
                                  // can be informed of a repair ship getting
                                  // called in
    ...
    int current_target_sx;   // Screen x-pos of current target (or subsystem if
                             // applicable)
    int current_target_sy;   // Screen y-pos of current target (or subsystem if
                             // applicable)
    int target_in_lock_cone; // Is the current target in secondary weapon lock
                             // cone?
    ship_subsys* locking_subsys; // Subsystem pointer that missile lock is
                                 // trying to seek
    int locking_subsys_parent;   // objnum of the parent of locking_subsystem
    int locking_on_center;       // boolean, whether missile lock is trying for
                                 // center of ship or not
    ...
    char killer_parent_name[NAME_LENGTH]; // name of parent object that killed
                                          // the player

    int check_for_all_alone_msg; // timestamp to check for playing of 'all
                                 // alone' msg
    ...
    int failures_this_session; // number of times dude has failed the mission
                               // he is on this session
    ubyte show_skip_popup; // false if dude clicked "don't show this again" --
                           // persists for current mission only
    ...
    control_info lua_ci; // copy of control info for scripting purposes (not to
                         // disturb real controls).
    button_info lua_bi;  // copy of button info for scripting purposes (not to
                         // disturb real controls).
    button_info lua_bi_full; // gets all the button controls, not just the ones
                             // usually allowed

    int player_was_multi; // 1 if the player file was last used in Multiplayer

    char
        language[LCL_LANG_NAME_LEN + 1]; // language the pilot was created with
};
...
extern void player_controls_init (); // initialize Descent style controls for
                                     // use in various places
extern void player_match_target_speed (
    char* no_target_text = NULL, char* match_off_text = NULL,
    char* match_on_text =
        NULL); // call to continually match speed with selected target
extern void player_clear_speed_matching ();

extern int
    lua_game_control; // defines the level of control set to lua scripting
...
extern void
toggle_player_object (); // toggles between descent-style ship and player ship
...
extern int Warpout_forced; // If non-zero, bash the player to speed and go
                           // through effect
//=============================================================

#endif // FREESPACE2_PLAYERMAN_PLAYER_HH
