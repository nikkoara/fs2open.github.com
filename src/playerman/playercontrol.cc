...
void do_view_slew(float frame_time)
{
        ...
        if (!(Viewer_mode & VM_PADLOCK_ANY)) {
                if (check_control_timef(VIEW_SLEW)) {
                        ...
                        check_control_timef(VIEW_CENTER) || !(Viewer_mode & VM_CAMERA_LOCKED)) {
        ...
        control_info *ci, float frame_time, physics_info *pi)
{
        ...
        int centering_speed = 7;                              // the scale speed in which the camera will smoothly center when the
                ...
                if (Player_ship != nullptr && Player_ship->ship_info_index >= 0) {
                        ...
                        if (z_min > 0.0f && pi->speed <= z_min) {
                                ...
                                ci->forward_cruise_percent = MAX(
                                        ...
                                        z_min / Ship_info[Player_ship->ship_info_index].max_vel.xyz.z * 100.0f);
                                override_analog_throttle = 1;
                        }
                }
                ...
                if ((objp->type == OBJ_SHIP) && (!(Game_mode & GM_DEAD))) {
                        ...
                        if (!(Warpout_forced) && (Ships[objp->instance].flags[Ship::Ship_Flags::Warp_broken] || Ships[objp->instance].flags[Ship::Ship_Flags::Warp_never])) {
                                ...
                                if (Warpout_forced) {
                                        ...
                                        Ships[objp->instance].current_max_speed < target_warpout_speed) {
                        ...
                        if (Player->control_mode == PCM_WARPOUT_STAGE1) {
                                ...
                                if (Warpout_time > warpout_delay) {
                                        ...
                                        if (target_warpout_speed != 0.0f) {
                                                diffSpeed = fabsf(
                                                                    objp->phys_info.fspeed - target_warpout_speed)
...
void player_level_init()
{
        ...
        Player->allow_warn_timestamp = 1; // init timestamp that is used for managing attack warnings sent to
                                          // player
        ...
        if (timestamp_elapsed(Player->praise_delay_timestamp)) {
                ...
                if (ship_index >= 0) {
                        ...
                        if (get_hull_pct(&Objects[Ships[ship_index].objnum]) > 0.5f) {
                                if (Player->stats.m_kill_count_ok > 10) { // this number should probably be in the AI profile
                                ...
                                Player->allow_praise_timestamp = timestamp(
                                        Builtin_messages[MESSAGE_PRAISE].min_delay * (Game_skill_level + 1));
...
int player_inspect_cargo(float frametime, char *outstr)
{
        ...
        if (cargo_sp->flags[Ship::Ship_Flags::Cargo_revealed]) {
                if (!(cargo_sp->flags[Ship::Ship_Flags::Scannable])) {
                        ...
                        ASSERT(
                                cargo_sip->flags[Ship::Info_Flags::Cargo] || cargo_sip->flags[Ship::Info_Flags::Transport]);
        ...
        if (Player_ai->current_target_distance < MAX(
                    player_sip->scan_range_normal,
                    (cargo_objp->radius + player_sip->scan_range_normal - CARGO_RADIUS_REAL_DELTA))) {
...
int player_inspect_cap_subsys_cargo(float frametime, char *outstr)
{
        ...
        if (cargo_sip->is_huge_ship()) {
                scan_dist = MAX(
                        ...
                        (subsys_rad + player_sip->scan_range_capital - CARGO_RADIUS_REAL_DELTA));
        } else {
                scan_dist = MAX(
                        player_sip->scan_range_normal,
                        (subsys_rad + player_sip->scan_range_normal - CARGO_RADIUS_REAL_DELTA));
        }
