...
int jpeg_read_header (
    ...
    if (img_cfp == NULL) {
        ...
        if (!jpeg_file) { return JPEG_ERROR_READING; }
    }
    else {
        jpeg_file = img_cfp;
    }
...
int jpeg_read_bitmap (
    ...
    if (img_cfp == NULL) return JPEG_ERROR_READING;
    ...
    // SPECIAL NOTE: we've already allocated memory on the basis of original
    // height, width and bpp of the image from the header.  DO NOT change any
    // settings that affect output variables here or we risk needed more memory
    // than we have available in "image_data".  The only exception is bpp since
    // 'dest_size' should already indicate what we want to end up with.
    ...
    if (setjmp (FSJpegError) == 0) {
        ...
        // set the output components to be 'dest_size' (so we can support
        // 16/24/32-bit images with this one function) NOTE: only 24-bit is
        // actually supported right now, we don't currently up/down sample at
        // all
        jpeg_info.output_components = dest_size;
        jpeg_info.out_color_components =
        ...
        // multiplying by rec_outbuf_height isn't required but is more
        // efficient
        ...
        while (jpeg_info.output_scanline < jpeg_info.output_height) {
            ...
            // niffiwan: swap some bytes (FSO uses BGR, not RGB) so that
            // libjpeg code is used in its original state NOTE: assumes only
            // one scanline is being read at a time. If multiple lines are read
            // this needs updating also note that this doesn't deal with jpegs
            // that are greyscale
            JSAMPLE tmp;
...
void jpeg_cf_skip_input_data (j_decompress_ptr cinfo, long num_bytes) {
    ...
    if (num_bytes > 0) {
        while (num_bytes > (long)src->pub.bytes_in_buffer) {
            ...
            if (!jpeg_cf_fill_input_buffer (cinfo)) return;
        }
...
void jpeg_cfile_src (j_decompress_ptr cinfo, CFILE* cfp) {
    ...
    if (cinfo->src == NULL) { // first time for this JPEG object?
        cinfo->src = (struct jpeg_source_mgr*)(*cinfo->mem->alloc_small) (
            (j_common_ptr)cinfo, JPOOL_PERMANENT, sizeof (cfile_source_mgr));
