...
void extract_all_files (char* file) {
    ...
    for (size_t i = 0; i < VP_FileInfo.size (); i++) {
        ...
        if (have_outdir) {
            ...
            sprintf (
                path, "%s%s%s%s", out_dir, "/",
                VP_FileInfo[i].file_path, "/");
        }
        else {
            sprintf (
                path, "%s%s", VP_FileInfo[i].file_path, "/");
        }
        ...
        sprintf (
            path, "%s%s%s", VP_FileInfo[i].file_path, "/",
        ...
        if (have_outdir) {
            ...
            sprintf (path2, "%s%s%s", out_dir, "/", path);
        }
        else {
            strcpy (path2, path);
        }
