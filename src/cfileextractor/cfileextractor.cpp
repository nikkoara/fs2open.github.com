...
typedef struct vp_header {
    char id[4]; // 'VPVP'
    int version;
    int index_offset; // offset where the file index starts, also a total file
                      // data size count
    int num_files;    // number of files, including each step in the directory
                      // structure
} vp_header;
...
typedef struct vp_fileinfo {
    ...
    _fs_time_t write_time; // date/time in _fs_time_t type (a 32-bit version of
                           // time_t)

    // not in the VP
    char file_path[CF_MAX_PATHNAME_LENGTH]; // file path, generated here and
                                            // not actually in the VP on a per
                                            // file basis
...
void read_header () {
    ...
    if ((VP_Header.id[0] != 'V') && (VP_Header.id[1] != 'P') &&
        (VP_Header.id[2] != 'V') && (VP_Header.id[3] != 'P'))
        print_error (ERR_INVALID_VP);

    // the index_offset needs to be greater than the size of vp_header at the
    // very least and there should be at least one file
    if (!(VP_Header.index_offset > (int)sizeof (vp_header)) ||
        !(VP_Header.num_files > 1))
        print_error (ERR_INVALID_VP);
...
void read_index (int lc = 0) {
    ...
    for (i = 0; i < VP_Header.num_files; i++) {
        ...
        // check if it's a directory and if so then create a path to use for
        // files
        if (vpinfo.file_size == 0) {
            ...
            // don't add the very first directory separator character, easier
            // to clean later
            if (path[0] != 0) strcat (path, DIR_SEPARATOR_STR);
...
void extract_all_files (char* file) {
    ...
    char path
        [CF_MAX_PATHNAME_LENGTH + CF_MAX_FILENAME_LENGTH +
         1]; // path length + filename length + extra NULL
    char path2
        [CF_MAX_PATHNAME_LENGTH + CF_MAX_FILENAME_LENGTH +
         1]; // path length + filename length + extra NULL
    char* c;
    ...
    if (out_len > 0) have_outdir = 1;

    printf ("VP file extractor - version 0.6\n");
    printf ("\n");

    if (have_outdir) { printf ("Output directory: \"%s\"\n", out_dir); }
    ...
    for (size_t i = 0; i < VP_FileInfo.size (); i++) {
        ...
        if (have_outdir) {
            ...
            sprintf (
                path, "%s%s%s%s", out_dir, DIR_SEPARATOR_STR,
                VP_FileInfo[i].file_path, DIR_SEPARATOR_STR);
        }
        else {
            sprintf (
                path, "%s%s", VP_FileInfo[i].file_path, DIR_SEPARATOR_STR);
        }
        ...
        sprintf (
            path, "%s%s%s", VP_FileInfo[i].file_path, DIR_SEPARATOR_STR,
            VP_FileInfo[i].file_name);
        ...
        while (nbytes_remaining > 0) {
            nbytes =
                fread (tmp_data, 1, MIN (BLOCK_SIZE, nbytes_remaining), fp_in);
...
// TODO: the formatting of this is a bit loco but I don't care enough to make
// it better...
void list_all_files (char* file) {
    ...
    if (!have_index) print_error (ERR_NO_INDEX);
    ...
    for (size_t i = 0; i < VP_FileInfo.size (); i++) {
        plat_time =
            VP_FileInfo[i]
                .write_time; // gets rid of some platform strangeness this way
        strftime (
            out_time, 32, "%F %H:%M",
            localtime (&plat_time)); // YYYY-mm-dd HH:mm  (ISO 8601 date
                                     // format, 24-hr time)
        ...
        printf (
            "%s  %*.1f%c %10i %18s %3s%s\n", VP_FileInfo[i].file_name,
            33 - len, (float)VP_FileInfo[i].file_size / div_by, m,
            VP_FileInfo[i].offset, out_time, DIR_SEPARATOR_STR,
            VP_FileInfo[i].file_path);
    }

    printf ("\n");

    // we use the vector size here since VP_Header.num_files would include each
    // entry in the directory tree as well as individual files and that
    // artificially inflates the number of files that we show or would extract
...
void help () {
    ...
    printf (
        "  -o <dir>              Extract files to <dir> instead of current "
        "directory.\n");
    printf (
        "                        NOTE: No spaces allowed, enclose path in \" "
        "\" if needed.\n");
    printf ("\n");
    printf (
...
int main (int argc, char* argv[]) {
    ...
    if (fp_in == NULL) print_error (ERR_NO_FP_IN);
    ...
    if (extract) {
        ...
        // if only -L is specified or no command is specified then just list
        // files
        list_all_files (argv[argc - 1]);
        return 0;
    }
