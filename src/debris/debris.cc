        ...
        vec3d *exp_center, int hull_flag, float exp_force)
{
        ...
        if (hull_flag && sip->debris_min_lifetime >= 0.0f && sip->debris_max_lifetime >= 0.0f) {
                db->lifeleft = ((sip->debris_max_lifetime - sip->debris_min_lifetime) * fs2::prng::randf(0)) + sip->debris_min_lifetime;
        } else {
        ...
        if (hull_flag) {
                if (sip->debris_min_lifetime >= 0.0f && sip->debris_max_lifetime >= 0.0f) {
                        db->must_survive_until = timestamp(int(sip->debris_min_lifetime * 1000.0f));
                        db->lifeleft = ((sip->debris_max_lifetime - sip->debris_min_lifetime) * fs2::prng::randf(0)) + sip->debris_min_lifetime;
        ...
        if (db->is_hull) {
                ...
                // 50%)
                if (fs2::prng::randf(0) < sip->debris_arc_percent) {
        ...
        if (hull_flag) {
                if (sip->debris_min_hitpoints >= 0.0f && sip->debris_max_hitpoints >= 0.0f) {
                        obj->hull_strength = ((sip->debris_max_hitpoints - sip->debris_min_hitpoints) * fs2::prng::randf(0)) + sip->debris_min_hitpoints;
        ...
        if (hull_flag) {
                float t;
                scale = exp_force * float((myrand() % 20) + 10); // for radial_vel away from location of blast center
                ...
                t = 1000 * Objects[db->source_objnum].radius / 3 + myrand() % (int(1000 * 3 * Objects[db->source_objnum].radius));
                db->fire_timeout = timestamp(int(t)); // fireballs last from 5 - 30 seconds
                ...
                scale = exp_force * float((myrand() % 20) + 10); // for radial_vel away from blast center (non-hull)
        }
        ...
        collision_info_struct *debris_hit_info, vec3d *hitNormal)
{
        ...
        if (debris_hit_info->heavy == pship_obj) { // ship is heavier, so debris is sphere. Check sphere
                ...
                if (model_collide(&mc)) {
                        ...
                        if (debris_hit_info->collide_rotate) {
                                ...
                                if (Ship_info[Ships[pship_obj->instance].ship_info_index]
                                            .collision_lod
                                    > -1) {
                                        mc.lod = Ship_info[Ships[pship_obj->instance].ship_info_index]
                                                         .collision_lod;
                                }
