...
#define MAX_HULL_PIECES \
    MAX_DEBRIS_PIECES // limit the number of hull debris chunks that can exist.

int Num_hull_pieces;     // number of hull pieces in existance
debris Hull_debris_list; // head of linked list for hull debris chunks, for
                         // quick search
...
#define MAX_SPEED_CAPITAL_DEBRIS \
    100 // maximum velocity of capital debris piece
...
static void debris_start_death_roll (object* debris_obj, debris* debris_p) {
    if (debris_p->is_hull) {
        ...
        fireball_create (
            &debris_obj->pos, fireball_type, FIREBALL_LARGE_EXPLOSION,
            OBJ_INDEX (debris_obj), debris_obj->radius * 1.75f);

        // only play debris destroy sound if hull piece and it has been around
        // for at least 2 seconds
        if (Missiontime > debris_p->time_started + 2 * F1_0) {
            snd_play_3d (
                gamesnd_get_game_sound (
                    gamesnd_id (GameSounds::MISSILE_IMPACT1)),
                &debris_obj->pos, &View_position, debris_obj->radius);
        }
    }
...
void debris_init () {
    ...
    if (!Debris_inited) { Debris_inited = 1; }
    ...
    if (timestamp_elapsed (db->next_distance_check) &&
        ...
        if (!(Game_mode & GM_MULTIPLAYER)) { //	In single player game, just
            ...
            if (vm_vec_dist_quick (
                ...
                db->next_distance_check =
                    timestamp (DEBRIS_DISTANCE_CHECK_TIME);
        }
        else {
            for (objp = GET_FIRST (&obj_used_list);
                ...
                if (objp->flags[Object::Object_Flags::Player_ship]) {
                    if (vm_vec_dist_quick (
                            &objp->pos, &Objects[db->objnum].pos) <
                        MAX_DEBRIS_DIST) {
                        db->next_distance_check =
                            timestamp (DEBRIS_DISTANCE_CHECK_TIME);
                        return;
...
void debris_process_post (object* obj, float frame_time) {
    ...
    if (db->lifeleft >= 0.0f) {
        ...
        if (db->lifeleft < 0.0f) { debris_start_death_roll (obj, db); }
    }
    ...
    if (!timestamp_elapsed (db->fire_timeout) &&
        timestamp_elapsed (db->next_fireball)) {
        db->next_fireball =
            timestamp_rand (db->arc_frequency, db->arc_frequency * 2);
        ...
        if (db->is_hull) {
            ...
            if (Cmdline_old_collision_sys) {
                submodel_get_two_random_points (
                    db->model_num, db->submodel_num, &v1, &v2);
                submodel_get_two_random_points (
                    db->model_num, db->submodel_num, &v3, &v4);
            }
            else {
...
/**
 * Locate the oldest hull debris chunk.  Search through the ::Hull_debris_list,
 * which is a list of all the hull debris chunks.
 */
int debris_find_oldest () {
    ...
    for (db = GET_FIRST (&Hull_debris_list);
         db != END_OF_LIST (&Hull_debris_list); db = GET_NEXT (db)) {
        if ((db->time_started < oldest_time) &&
            !(Objects[db->objnum]
...
object* debris_create (
    ...
    vec3d* exp_center, int hull_flag, float exp_force) {
    ...
    if (!hull_flag) {
        ...
        if (vaporize) { dist /= 2.0f; }
        if (dist > 200.0f) { return NULL; }
    }
    ...
    for (n = 0; n < MAX_DEBRIS_PIECES; n++) {
        if (!(Debris[n].flags & DEBRIS_USED)) break;
    }
    ...
    if (hull_flag && sip->debris_min_lifetime >= 0.0f &&
        ...
        if (hull_flag) {
            if (rand () <
                RAND_MAX /
                    6) // Make some pieces blow up shortly after explosion.
    ...
    db->team = shipp->team;
    db->fire_timeout =
        0; // if not changed, timestamp_elapsed() will return false
    db->time_started = Missiontime;
    db->species = Ship_info[shipp->ship_info_index].species;
    db->next_distance_check =
        (myrand () % 2000) + 4 * DEBRIS_DISTANCE_CHECK_TIME;
    ...
    if (model_num < 0) {
        if (vaporize) { db->model_num = Debris_vaporize_model; }
        else {
    ...
    db->next_fireball =
        timestamp_rand (500, 2000); // start one 1/2 - 2 secs later

    if (pos == NULL) pos = &source_obj->pos;
    ...
    // DA: here we need to vel_from_rot = w x to_center, where w is world is
    // unrotated to world coords and offset is the displacement fromt the
    // center of the parent object to the center of the debris piece
    vec3d world_rotvel, vel_from_rotvel;
    vm_vec_unrotate (
        &world_rotvel, &source_obj->phys_info.rotvel, &source_obj->orient);
    ...
    if (hull_flag) { shipfx_debris_limit_speed (db, shipp); }
    ...
    vm_vec_zero (
        &pi->max_vel); // make so he can't turn on his own VOLITION anymore.
    vm_vec_zero (&pi->max_rotvel); // make so he can't change speed on his own
                                   // VOLITION anymore.
...
/**
 * Alas, poor debris_obj got whacked.  Fortunately, we know who did it, where
 * and how hard, so we can do something about it.
 */
void debris_hit (
    object* debris_obj, object* /*other_obj*/, vec3d* hitpos, float damage) {
    ...
    // Do a little particle spark shower to show we hit
    {
        ...
        pe.vel =
            debris_obj->phys_info.vel; // Initial velocity of all the particles
...
/**
 * See if poor debris object *obj got whacked by evil *other_obj at point
 * *hitpos. NOTE: debris_hit_info pointer NULL for debris:weapon collision,
 * otherwise debris:ship collision.
 * @return true if hit, else return false.
 */
int debris_check_collision (
    object* pdebris, object* other_obj, vec3d* hitpos,
    collision_info_struct* debris_hit_info, vec3d* hitNormal) {
    mc_info mc;
    mc_info_init (&mc);
    int num;
    ...
    if ((vm_vec_mag_squared (&heavy->phys_info.rotvel) * flFrametime *
        ...
        vm_vec_sub (
            &debris_hit_info->light_rel_vel, &lighter->phys_info.vel,
            &heavy->phys_info.vel);
    }
    ...
    if (debris_hit_info->heavy ==
        pship_obj) { // ship is heavier, so debris is sphere. Check sphere
        ...
        if (model_collide (&mc)) {
            ...
            if (debris_hit_info->collide_rotate) {
                ...
                for (smv = submodel_vector.begin ();
                     smv != submodel_vector.end (); ++smv) {
                ...
                if (Ship_info[Ships[pship_obj->instance].ship_info_index]
                        .collision_lod > -1) {
                    mc.lod =
                        Ship_info[Ships[pship_obj->instance].ship_info_index]
                            .collision_lod;
                }
                ...
                for (smv = submodel_vector.begin ();
                    ...
                    if (model_collide (&mc)) {
                        if (mc.hit_dist < debris_hit_info->hit_time) {
                            ...
                            set_hit_struct_info (
                                debris_hit_info, &mc, SUBMODEL_ROT_HIT);
                            model_instance_find_world_point (
                                &debris_hit_info->hit_pos, &mc.hit_point,
                            ...
                            if (debris_hit_info->edge_hit == 0) {
                                model_instance_find_obj_dir (
                                    &debris_hit_info->collision_normal,
            ...
            *mc.p1 = copy_p1;
            ...
            if (model_collide (&mc)) {
                ...
                if (mc.hit_dist < debris_hit_info->hit_time) {
                    ...
                    set_hit_struct_info (
                        debris_hit_info, &mc, SUBMODEL_NO_ROT_HIT);

                    // get collision normal if not edge hit
                    if (debris_hit_info->edge_hit == 0) {
                        model_instance_find_obj_dir (
                            &debris_hit_info->collision_normal, &mc.hit_normal,
                            Ships[heavy_obj->instance].model_instance_num,
                            mc.hit_submodel, &heavy_obj->orient);
                    }
                    ...
                    vm_vec_sub (&diff, mc.p1, mc.p0);
                    vm_vec_scale_add (
                        &debris_hit_info->light_collision_cm_pos, mc.p0, &diff,
                        mc.hit_dist);
                }
            }
        }
    ...
    if (mc_ret_val) {
        ...
        vm_vec_sub (
            &debris_hit_info->r_light, &debris_hit_info->hit_pos,
            &debris_hit_info->light_collision_cm_pos);

        // set normal for edge hit
        if (debris_hit_info->edge_hit) {
            vm_vec_copy_normalize (
                &debris_hit_info->collision_normal, &debris_hit_info->r_light);
            vm_vec_negate (&debris_hit_info->collision_normal);
        }

        // get world hitpos
        vm_vec_add (
            hitpos, &debris_hit_info->heavy->pos, &debris_hit_info->r_heavy);
...
/**
 * Fills in debris physics properties when created, specifically mass and
 * moment of inertia
 */
void calc_debris_physics_properties (
    physics_info* pi, vec3d* mins, vec3d* maxs) {
    float dx, dy, dz, mass;
...
void debris_render (object* obj, model_draw_list* scene) {
    ...
    if (db->species >= 0) {
        ...
        if (pm != NULL && (pm->n_textures == 1)) {
            ...
            tbase->SetTexture (
                Species_info[db->species].debris_texture.bitmap_id);
        }
    }
    ...
    if (vm_vec_dist_quick (&obj->pos, &Eye_position) < obj->radius * 50.0f) {
        for (i = 0; i < MAX_DEBRIS_ARCS; i++) {
            if (timestamp_valid (db->arc_timestamp[i])) {
                model_add_arc (
                    db->model_num, db->submodel_num, &db->arc_pts[i][0],
                    &db->arc_pts[i][1], MARC_TYPE_NORMAL);
            }
        }
    }
    ...
    if (db->is_hull) { MONITOR_INC (NumHullDebrisRend, 1); }
    else {
    ...
    submodel_render_queue (
        &render_info, scene, db->model_num, db->submodel_num, &obj->orient,
        &obj->pos);
