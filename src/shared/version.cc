...
version parse_version () {
    ...
    if (optional_string ("+Revision:")) { stuff_int (&v.revision); }
...
bool version::operator< (const version& v) const {
    ...
    if (revision == 0 || v.revision == 0) {
        // Special case, if there is no revision info, then the versions are
        // equal which means they are not less than each other
        return false;
    }
