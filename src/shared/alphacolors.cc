...
color Color_text_error, Color_text_error_hi, Color_text_active,
    Color_text_active_hi;
color Color_text_heading, Color_more_indicator, Color_more_bright,
...
color Color_dim_red, Color_red, Color_bright_red, Color_yellow,
    Color_bright_yellow;
color Color_orange;
...
color* COLOR_LIST[TOTAL_COLORS] = {
    &Color_blue,          &Color_bright_blue,    &Color_green,
    &Color_bright_green,  &Color_black,          &Color_grey,
    &Color_silver,        &Color_white,          &Color_bright_white,
    &Color_violet_gray,   &Color_violet,         &Color_dim_red,
    &Color_red,           &Color_bright_red,     &Color_pink,
    &Color_light_pink,    &Color_yellow,         &Color_bright_yellow,
    &Color_orange,        &Color_ui_light_green, &Color_ui_green,
    &Color_ui_light_pink, &Color_ui_pink,
};

const char* COLOR_NAMES[TOTAL_COLORS] = {
    "Blue",         "Bright Blue",   "Green",  "Bright Green",
    "Black",        "Grey",          "Silver", "White",
    "Bright White", "Violet Gray",   "Violet", "Dim Red",
    "Red",          "Bright Red",    "Pink",   "Light Pink",
    "Yellow",       "Bright Yellow", "Orange", "UI Light Green",
    "UI Green",     "UI Light Pink", "UI Pink"
};
...
color* interface_colors[INTERFACE_COLORS] = {
    &Color_text_normal,    &Color_text_subselected, &Color_text_selected,
    &Color_text_error,     &Color_text_error_hi,    &Color_text_active,
    &Color_text_active_hi, &Color_text_heading,     &Color_more_indicator,
    &Color_more_bright,    &Color_bright,           &Color_normal,
    &Color_briefing_grid,
};
...
const char DEFAULT_TAG_LIST[DEFAULT_TAGS] = {
    'w', 'W', 'r', 'o', 'g', 'y', 'b', 'f', 'h', 'n', 'B',
    'G', 'R', 'Y', 'k', 'e', 'E', 'v', 'V', 'p', 'P',
};

color* DEFAULT_TAG_COLORS[DEFAULT_TAGS] = {
    &Color_white,       &Color_bright_white,
    &Color_red,         &Color_orange,
    &Color_green,       &Color_yellow,
    &Color_blue,        &Brief_color_green,
    &Brief_color_red,   &Brief_color_legacy_neutral,
    &Color_bright_blue, &Color_bright_green,
    &Color_bright_red,  &Color_bright_yellow,
    &Color_black,       &Color_grey,
    &Color_silver,      &Color_violet_gray,
...
color* Color_netplayer[NETPLAYER_COLORS] = {
    ...
    &Color_blue,          &Color_bright_blue,    &Color_green,
    &Color_bright_green,  &Color_pink,           &Color_grey,
    &Color_silver,        &Color_white,          &Color_bright_white,
    &Color_violet_gray,   &Color_violet,         &Color_dim_red,
    &Color_red,           &Color_bright_red,     &Color_yellow,
    &Color_bright_yellow, &Color_ui_light_green, &Color_ui_green,
    &Color_ui_light_pink, &Color_ui_pink,
};
...
void alpha_colors_init () {
    ...
    for (i = 0; i < TOTAL_COLORS; i++) {
        gr_init_alphacolor (
            COLOR_LIST[i], rgba_defaults[i][0], rgba_defaults[i][1],
            rgba_defaults[i][2], rgba_defaults[i][3]);
    }
    ...
    // Set defaults for interface colors and color tags (must be done after the
    // above because they're generally just copies of above-defined colors).
    for (i = 0; i < INTERFACE_COLORS; i++) {
        memcpy (
            interface_colors[i], COLOR_LIST[interface_defaults[i]],
            sizeof (color));
    }
    ...
    try {
        ...
        if (optional_string ("#Start Colors")) {
            ...
            const char* color_names[TOTAL_COLORS] = {
                ...
                "$Bright Green:",  "$Black:",          "$Grey:",
                "$Silver:",        "$White:",          "$Bright White:",
                "$Violet Gray:",   "$Violet:",         "$Dim Red:",
                "$Red:",           "$Bright Red:",     "$Pink:",
                "$Light Pink:",    "$Yellow:",         "$Bright Yellow:",
                "$Orange:",        "$UI Light Green:", "$UI Green:",
                "$UI Light Pink:", "$UI Pink:"
            };
    ...
    try {
        ...
        if (optional_string ("#Team Colors")) {
            while (required_string_either ("#End", "$Team Name:")) {
                required_string ("$Team Name:"); // required to move the parse
                                                 // pointer forward
                ...
                if (required_string ("$Team Stripe Color:")) {
                    ...
                    for (i = 0; i < 3; i++) { CLAMP (rgb[i], 0, 255); }
                ...
                if (Team_Colors.find (temp) ==
                    Team_Colors.end ()) { // Only push to the vector if the
                                          // team isn't already defined.
                    ...
                    if (check_for_string ("(")) {
                        ...
                        if (j == TOTAL_COLORS) {
                            ...
                            ASSERTX (
                                ...
                                j, TOTAL_COLORS - 1);
                            memcpy (
                                interface_colors[i], COLOR_LIST[j],
                                sizeof (color));
                        }
                    }
                }
        ...
        if (optional_string ("#Color Tags")) {
            while (required_string_either ("$Tag:", "#End") < 1) {
                ...
                if (Tagged_Colors.find (tag) ==
                    Tagged_Colors.end ()) { // Only push the tag to our list of
                                            // tags if it's actually new, not
                                            // just a redefinition.
                    Color_Tags.push_back (tag);
                }

                switch (required_string_one_of (
                    4, "+Color:", "+Friendly", "+Hostile", "+Neutral")) {
                case 0: // +Color
                    required_string ("+Color:");

                    rgba[0] = rgba[1] = rgba[2] = 0;
                    rgba[3] = 255; // Odds are pretty high you want it to have
                                   // full alpha...
                    ...
                    if (check_for_string ("(")) {
                        ...
                        gr_init_alphacolor (
                            &temp_color, rgba[0], rgba[1], rgba[2], rgba[3]);
                ...
                case -1:
                    ...
                    if (Tagged_Colors.find (tag) ==
                        Tagged_Colors
                            .end ()) { // It was a new color, but since we
        ...
        if (optional_string ("#Default Text Colors")) {
            const char* color_names[MAX_DEFAULT_TEXT_COLORS] = {
                "$Fiction Viewer:", "$Command Briefing:",
                "$Briefing:",       "$Redalert Briefing:",
            ...
            for (i = 0; i < MAX_DEFAULT_TEXT_COLORS; i++) {
                if (optional_string (color_names[i])) {
                    ...
                    if (Tagged_Colors.find (*color_value[i]) ==
                        Tagged_Colors.end ()) {
                        // Just mprintf() this information instead of
