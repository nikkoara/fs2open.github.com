...
const size_t HEAP_SIZE_INCREASE =
    1 * 1024 * 1024; // Always increment in 1MB steps
const size_t HEAP_MAX_INCREASE =
    20 * 1024 * 1024; // never increase heap size by more than 20MB
...
TIter get_element_before (TIter first, TIter last, const T& value) {
    ...
    // We found the position (which may be last) so now we can decrement it
    // once to get the element before this found position
...
namespace util {
HeapAllocator::HeapAllocator (const HeapAllocator::HeapResizer& creator)
    : _heapResizer (creator) {
...
size_t HeapAllocator::allocate (size_t size) {
    // Do a simple first-fit algorithm. Not very efficient but it should do the
    // job well for our purposes
    for (auto iter = _freeRanges.begin (); iter != _freeRanges.end ();
         ++iter) {
        if (iter->size >= size) {
            // Found a match!
            ...
            if (iter->size == 0) {
                // This range is now empty and must be removed. Since we won't
                // be using this iterator anymore it can be safely erased
    ...
    // We increase the heap size every time we run out of memory in order to
    // reduce reallocation times
    _lastSizeIncraese = std::min (HEAP_MAX_INCREASE, 2 * _lastSizeIncraese);

    // Make sure that our allocation can actually fit into the new block if its
    // too large for a single increase
...
void HeapAllocator::addFreeRange (const HeapAllocator::MemoryRange& range) {
    ...
    auto left =
        get_element_before (_freeRanges.begin (), _freeRanges.end (), range);
    auto right =
        std::upper_bound (_freeRanges.begin (), _freeRanges.end (), range);
    ...
    if (left != _freeRanges.end ()) {
        ...
        if (left->offset + left->size == range.offset) {
            ...
            // After this operation the two ranges may be connected. If that
            // happens then they need to be merged as well
            if (right != _freeRanges.end () &&
                check_connected_range (*left, *right)) {
...
void HeapAllocator::addAllocatedRange (
    ...
    // We need to keep the vector sorted for better search performance so we
    // insert this new range at the sorted position into the vector
    auto it = std::upper_bound (
        _allocatedRanges.begin (), _allocatedRanges.end (), range);

    // This works even if it is .end() since .insert will just add it to the
    // vector in that case
...
void HeapAllocator::free (size_t offset) {
    ...
    // Copy the range out of the vector before erasing it so we can add it to
    // the free ranges
    auto freedRange = *it;
...
bool HeapAllocator::check_connected_range (
...
bool HeapAllocator::MemoryRange::
operator< (const HeapAllocator::MemoryRange& other) const {
