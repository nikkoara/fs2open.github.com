 ...
 * bytes
 * @param assume_utf8 Set the @true true if no BOM should be treated as UTF-8
 * instead of ASCII
 ...
 * be skipped to get to the real text
 ...
 * @return @c true if the algorithm determined with high probability that the
 * text is Latin1 encoded
 *
 ...
 * the BOM if present
 *
 * Use this function if you directly read a text file which may be UTF-8
 * encoded. This will respect the unicode mode of the current mod so it will
 * also make sure that the file looks like it's ASCII encoded if Unicode mode
 * is disabled.
 *
 * @note If there is a BOM at the start of the file then this function will
 * adjust the read offset of the file pointer to point to the first valid text
 * byte. You can retrieve that offset by using @c start_offset.
 *
 * @param file The file pointer to check
 * @param filename The name of the file. Only used for possible error messages.
 * @param[out] start_offset A pointer to an int variable. If this is a valid
 * pointer then this variable will contain the offset of the first text byte
