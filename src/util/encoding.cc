...
Encoding guess_encoding (const std::string& content, bool assume_utf8) {
    if (content.size () >= 3 &&
        !strncmp (content.c_str (), "\xEF\xBB\xBF", 3)) { // UTF-8
        return Encoding::UTF8;
    }

    if (content.size () >= 4 &&
        !strncmp (
            content.c_str (), "\x00\x00\xFE\xFF", 4)) { // UTF-32 big-endian
        return Encoding::UTF32BE;
    }

    if (content.size () >= 4 &&
        !strncmp (
            content.c_str (), "\xFF\xFE\x00\x00", 4)) { // UTF-32 little-endian
        return Encoding::UTF32LE;
    }

    if (content.size () >= 2 &&
        !strncmp (content.c_str (), "\xFE\xFF", 2)) { // UTF-16 big-endian
        return Encoding::UTF16BE;
    }

    if (content.size () >= 2 &&
        !strncmp (content.c_str (), "\xFF\xFE", 2)) { // UTF-16 little-endian
...
int check_encoding_and_skip_bom (
    CFILE* file, const char* filename, int* start_offset) {
    ...
    if (Unicode_text_mode) {
        ...
        if (util::has_bom (probe)) {
            ...
            // Bytes long This makes sure that the first byte we read will be
            // the first actual text byte.
            cfseek (file, 3, SEEK_SET);
            filelength -= 3;

            if (start_offset) { *start_offset = 3; }
        }
    }
    else {
...
// The following code is adapted from the uchardet library, the original
// licence of the file has been kept
