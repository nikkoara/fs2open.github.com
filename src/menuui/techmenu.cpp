...
const char* Tech_background_filename[GR_NUM_RESOLUTIONS] = {
    "TechShipData", "2_TechShipData"
};
const char* Tech_mask_filename[GR_NUM_RESOLUTIONS] = { "TechShipData-M",
                                                       "2_TechShipData-M" };
const char* Tech_slider_filename[GR_NUM_RESOLUTIONS] = { "slider",
                                                         "2_slider" };
...
int Tech_ship_display_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                          239, 98, 393, 222 },
                                                        { // GR_1024
                                                          382, 158, 629,
                                                          355 } };

int Tech_desc_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                  29, 347, 365, 125 },
                                                { // GR_1024
                                                  47, 555, 584, 200 } };

int Tech_ani_centre_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                        416, 215 },
                                                      { // GR_1024
                                                        669, 345 } };
...
struct techroom_buttons {
    ...
    techroom_buttons (
        const char* name, int x1, int y1, int xt1, int yt1, int h, int t,
        int f = 0)
...
static techroom_buttons Buttons[GR_NUM_RESOLUTIONS][NUM_BUTTONS] = {
    {
        // GR_640
        techroom_buttons (
            "TDB_04", 406, 384, 447, 393, 4, -1), // ship data tab
        techroom_buttons (
            "TDB_05", 404, 418, 447, 429, 5, -1), // weapons data tab
        techroom_buttons (
            "TDB_06", 404, 447, 447, 461, 6, -1), // species data tab
        techroom_buttons (
            "TDB_00", 7, 3, 37, 7, 0, -1), // technical database tab
        techroom_buttons (
            "TDB_01", 7, 18, 37, 23, 1, -1), // mission simulator tab
        techroom_buttons ("TDB_02", 7, 34, 37, 38, 2, -1), // cutscenes tab
        techroom_buttons ("TDB_03", 7, 49, 37, 54, 3, -1), // credits tab
        techroom_buttons (
            "TDB_07", 1, 86, -1, -1, 7, SHIPS_DATA_MODE,
            REPEAT), // prev data entry
        techroom_buttons (
            "TDB_08", 1, 317, -1, -1, 8, SHIPS_DATA_MODE,
            REPEAT), // next data entry
        techroom_buttons (
            "TDB_09", 1, 406, -1, -1, 9, SHIPS_DATA_MODE,
            REPEAT), // prev data entry
        techroom_buttons (
            "TDB_10", 1, 447, -1, -1, 10, SHIPS_DATA_MODE,
            REPEAT), // next data entry
        techroom_buttons (
            "TDB_11a", 559, 323, -1, -1, 11, SHIPS_DATA_MODE,
            REPEAT), // prev data entry
        techroom_buttons (
            "TDB_12a", 609, 323, -1, -1, 12, SHIPS_DATA_MODE,
            REPEAT), // next data entry
    ...
    },
    {
        // GR_1024
        techroom_buttons (
            "2_TDB_04", 649, 614, 717, 630, 4, -1), // ship data tab
        techroom_buttons (
            "2_TDB_05", 646, 669, 717, 687, 5, -1), // weapons data tab
        techroom_buttons (
            "2_TDB_06", 646, 716, 717, 739, 6, -1), // species data tab
        techroom_buttons (
            "2_TDB_00", 12, 5, 59, 12, 0, -1), // technical database tab
        techroom_buttons (
            "2_TDB_01", 12, 31, 59, 37, 1, -1), // mission simulator tab
        techroom_buttons ("2_TDB_02", 12, 56, 59, 62, 2, -1), // cutscenes tab
        techroom_buttons ("2_TDB_03", 12, 81, 59, 88, 3, -1), // credits tab
        techroom_buttons (
            "2_TDB_07", 1, 138, -1, -1, 7, SHIPS_DATA_MODE,
            REPEAT), // prev data entry
        techroom_buttons (
            "2_TDB_08", 1, 507, -1, -1, 8, SHIPS_DATA_MODE,
            REPEAT), // next data entry
        techroom_buttons (
            "2_TDB_09", 1, 649, -1, -1, 9, SHIPS_DATA_MODE,
            REPEAT), // prev data entry
        techroom_buttons (
            "2_TDB_10", 1, 716, -1, -1, 10, SHIPS_DATA_MODE,
            REPEAT), // next data entry
        techroom_buttons (
            "2_TDB_11a", 895, 518, -1, -1, 11, SHIPS_DATA_MODE,
            REPEAT), // prev data entry
        techroom_buttons (
            "2_TDB_12a", 974, 518, -1, -1, 12, SHIPS_DATA_MODE,
            REPEAT), // next data entry
...
static int Cur_entry =
    -1; // this is the current entry selected, using entry indexing
static int Cur_entry_index =
    -1; // this is the current entry selected, using master list indexing
...
typedef struct {
    ...
    int has_anim;  // flag to indicate the presence of an animation for this
                   // item
    int model_num; // model reference handle
    int textures_loaded; // if the model has textures loaded for it or not
                         // (hacky mem management)
} tech_list_entry;
...
// Intelligence master data structs (these get inited @ game startup from
// species.tbl)
...
void techroom_select_new_entry () {
    ...
    if (Tab == SHIPS_DATA_TAB) {
        ...
        // little memory management, kinda hacky but it should keep the
        // techroom at around 100meg rather than the 700+ it can get to with
        // all ships loaded - taylor
        for (int i = 0; i < Current_list_size; i++) {
            if ((Current_list[i].model_num > -1) &&
                (Current_list[i].textures_loaded)) {
        ...
        int stream_result = generic_anim_init_and_stream (
            &Current_list[Cur_entry].animation,
            Current_list[Cur_entry].tech_anim_filename,
            bm_get_type (Tech_background_bitmap), true);

        if (stream_result >= 0) { Current_list[Cur_entry].has_anim = 1; }
        else {
            // we've failed to load any animation
            // load an image and treat it like a 1 frame animation
            Current_list[Cur_entry].bitmap =
...
void techroom_render_desc (int xo, int yo, int ho) {
    ...
    while (y + font_height <= ho) {
        if (z >= Text_size) { break; }

        len = Text_line_size[z];
        if (len > MAX_TEXT_LINE_LEN) { len = MAX_TEXT_LINE_LEN; }
    ...
    if (z < Text_size) {
        ...
        int more_txt_x =
            Tech_desc_coords[gr_screen.res][0] +
            (Tech_desc_coords[gr_screen.res][2] / 2) -
            10; // FIXME should move these to constants since they don't move
        int more_txt_y =
            Tech_desc_coords[gr_screen.res][1] +
            Tech_desc_coords[gr_screen.res]
                            [3]; // located below brief text, centered
        int w, h;
        gr_get_string_size (
            &w, &h, XSTR ("more", 1469),
            static_cast< int > (strlen (XSTR ("more", 1469))));
        gr_set_color_fast (&Color_black);
        gr_rect (more_txt_x - 2, more_txt_y, w + 3, h, GR_RESIZE_MENU);
        gr_set_color_fast (&Color_more_indicator);
        gr_string (
            more_txt_x, more_txt_y, XSTR ("more", 1469),
            GR_RESIZE_MENU); // base location on the input x and y?
    }
}
...
void tech_common_render () {
    ...
    techroom_render_desc (
        Tech_desc_coords[gr_screen.res][SHIP_X_COORD],
        Tech_desc_coords[gr_screen.res][SHIP_Y_COORD],
    ...
    while (y + font_height <= Tech_list_coords[gr_screen.res][SHIP_H_COORD]) {
        if (z >= Current_list_size) { break; }

        if (z == Cur_entry) { gr_set_color_fast (&Color_text_selected); }
        ...
        font::force_fit_string (
            buf, 255, Tech_list_coords[gr_screen.res][SHIP_W_COORD]);
        gr_string (
            Tech_list_coords[gr_screen.res][SHIP_X_COORD],
            Tech_list_coords[gr_screen.res][SHIP_Y_COORD] + y, buf,
            GR_RESIZE_MENU);

        List_buttons[z - List_offset].update_dimensions (
            Tech_list_coords[gr_screen.res][SHIP_X_COORD],
            Tech_list_coords[gr_screen.res][SHIP_Y_COORD] + y,
            Tech_list_coords[gr_screen.res][SHIP_W_COORD], font_height);
    ...
    z -= List_offset;
    while (z < LIST_BUTTONS_MAX) { List_buttons[z++].disable (); }
}
...
void techroom_ships_render (float frametime) {
    ...
    gr_set_clip (
        Tech_ship_display_coords[gr_screen.res][SHIP_X_COORD],
        Tech_ship_display_coords[gr_screen.res][SHIP_Y_COORD],
        Tech_ship_display_coords[gr_screen.res][SHIP_W_COORD],
        Tech_ship_display_coords[gr_screen.res][SHIP_H_COORD], GR_RESIZE_MENU);

    // render the ship
    g3_start_frame (1);
    g3_set_view_matrix (
        &sip->closeup_pos, &vmd_identity_matrix, sip->closeup_zoom * 1.3f);
    ...
    for (i = 0; i < sip->n_subsystems; i++) {
        ...
        if (msp->type == SUBSYSTEM_TURRET) {
            ...
            if (msp->subobj_num >= 0) {
                model_set_instance_techroom (
                    Techroom_ship_modelnum, msp->subobj_num, 0.0f, h);
            }
            if ((msp->subobj_num != msp->turret_gun_sobj) &&
                (msp->turret_gun_sobj >= 0)) {
                model_set_instance_techroom (
                    Techroom_ship_modelnum, msp->turret_gun_sobj, p, 0.0f);
            }
        }
    }
    ...
    if (sip->replacement_textures.size () > 0) {
        render_info.set_replacement_textures (
            Techroom_ship_modelnum, sip->replacement_textures);
    }
    ...
    if (Cmdline_shadow_quality) {
        ...
        shadows_start_render (
            &Eye_matrix, &Eye_position, Proj_fov, gr_screen.clip_aspect,
...
void tech_prev_entry () {
    // unload the current animation, we load another one for the new current
    // entry
    ...
    if (Cur_entry < 0) {
        ...
        List_offset = Cur_entry -
                      Tech_list_coords[gr_screen.res][SHIP_H_COORD] /
                          gr_get_font_height () +
                      1;
        if (List_offset < 0) {
...
void tech_next_entry () {
    ...
    if (Cur_entry >= Current_list_size) {
        ...
        // maybe adjust list position by 1
        if (List_offset + Tech_list_coords[gr_screen.res][SHIP_H_COORD] /
                              gr_get_font_height () <=
            Cur_entry) {
...
void techroom_anim_render (float frametime) {
    ...
    if (Current_list_size == 0 || Cur_entry < 0) return;

    // render the animation
    if (Current_list[Cur_entry].animation.num_frames > 0) {
        // grab dimensions
        bm_get_info (
            (Current_list[Cur_entry].animation.streaming)
                ? Current_list[Cur_entry].animation.bitmap_id
                : Current_list[Cur_entry].animation.first_frame,
            &x, &y, NULL, NULL, NULL);
        // get the centre point - adjust
        x = Tech_ani_centre_coords[gr_screen.res][0] - x / 2;
        y = Tech_ani_centre_coords[gr_screen.res][1] - y / 2;
        generic_anim_render (
            &Current_list[Cur_entry].animation, frametime, x, y, true);
    }
...
void techroom_change_tab (int num) {
    ...
    // unload the current animation, we load another one for the new current
    // entry
    if (Tab != SHIPS_DATA_TAB) techroom_unload_animation ();
    ...
    for (int i = 0; i < LIST_BUTTONS_MAX; i++) { List_buttons[i].disable (); }
    ...
    case SHIPS_DATA_TAB:
        si_mask.set (
            multi ? Ship::Info_Flags::In_tech_database_m
                  : Ship::Info_Flags::In_tech_database);
        si_mask.set (
            multi ? Ship::Info_Flags::Default_in_tech_database_m
                  : Ship::Info_Flags::Default_in_tech_database);
        ...
        if (Ships_loaded == 0) {
            ...
            for (auto it = Ship_info.begin (); it != Ship_info.end (); ++it) {
                if (Techroom_show_all || (it->flags & si_mask).any_set ()) {
                    ...
                    Ship_list[Ship_list_size].index =
                        (int)std::distance (Ship_info.begin (), it);
                    Ship_list[Ship_list_size].animation.num_frames =
                        0; // no anim for ships
                    Ship_list[Ship_list_size].has_anim =
                        0; // no anim for ships
                    Ship_list[Ship_list_size].name =
                        *it->tech_title
                            ? it->tech_title
                            : (*it->alt_name ? it->alt_name : it->name);
            ...
            // make sure that at least the default entry is cleared out if we
        ...
        font_height = gr_get_font_height ();
        max_num_entries_viewable =
            Tech_list_coords[gr_screen.res][SHIP_H_COORD] / font_height;
        Tech_slider.set_numberItems (
    ...
    case WEAPONS_DATA_TAB:
        ...
        if (Weapons_loaded == 0) {
            ...
            Weapon_list_size = 0;
            wi_mask.set (
                multi ? Weapon::Info_Flags::Player_allowed
                      : Weapon::Info_Flags::In_tech_database);
            wi_mask.set (Weapon::Info_Flags::Default_in_tech_database);

            for (int i = 0; i < Num_weapon_types; i++) {
                if (Techroom_show_all ||
                    (Weapon_info[i].wi_flags & wi_mask).any_set ()) {
                    // we have a weapon that should be in the tech db, so fill
                    // out the entry struct
                    Weapon_list[Weapon_list_size].index = i;
                    Weapon_list[Weapon_list_size].desc =
                        Weapon_info[i].tech_desc;
                    Weapon_list[Weapon_list_size].has_anim = 1;
                    Weapon_list[Weapon_list_size].name =
                        *Weapon_info[i].tech_title
                            ? Weapon_info[i].tech_title
                            : Weapon_info[i].get_display_string ();
                    ...
                    strncpy (
                        Weapon_list[Weapon_list_size].tech_anim_filename,
                        Weapon_info[i].tech_anim_filename,
                        MAX_FILENAME_LEN - 1);
        ...
        if (Intel_loaded == 0) {
            ...
            for (int i = 0; i < Intel_info_size; i++) {
                if (Techroom_show_all ||
                    ...
                    (Intel_info[i].flags & IIF_DEFAULT_IN_TECH_DATABASE)) {
                    ...
                    if (!strcmp (Intel_info[i].anim_filename, "none")) {
                        ...
                        strncpy (
                            Intel_list[Intel_list_size].tech_anim_filename,
                            Intel_info[i].anim_filename, NAME_LENGTH - 1);
                    }
...
int techroom_button_pressed (int num) {
    ...
    case PREV_ENTRY_BUTTON: tech_prev_entry (); break;

    case NEXT_ENTRY_BUTTON: tech_next_entry (); break;
    ...
    case SCROLL_INFO_UP: tech_scroll_info_up (); break;

    case SCROLL_INFO_DOWN: tech_scroll_info_down (); break;
...
int techroom_load_ani (anim** /*animpp*/, char* name) {
    ...
    if (gr_screen.res == GR_1024) { strcat_s (anim_filename, name); }
    else {
        strcpy_s (anim_filename, name);
    }

    while (1) {
        if (load_attempts++ > 5) { return 0; }
...
void techroom_intel_init () {
    ...
    if (intel_info_init_done) return;
    ...
    try {
        ...
        while (optional_string ("$Entry:")) {
            ...
            stuff_string (
                Intel_info[Intel_info_size].name, F_NAME, NAME_LENGTH);

            required_string ("$Anim:");
            stuff_string (
                Intel_info[Intel_info_size].anim_filename, F_NAME,
                NAME_LENGTH);
            ...
            if (temp) {
                ...
                Intel_info[Intel_info_size].flags |=
                    IIF_DEFAULT_IN_TECH_DATABASE;
            }

            required_string ("$Description:");
            stuff_string (
                Intel_info[Intel_info_size].desc, F_MULTITEXT,
                TECH_INTEL_DESC_LEN);
...
void techroom_init () {
    ...
    Ui_window.add_XSTR (
        ...
        Buttons[gr_screen.res][TECH_DATABASE_TAB].xt,
        Buttons[gr_screen.res][TECH_DATABASE_TAB].yt,
        &Buttons[gr_screen.res][TECH_DATABASE_TAB].button,
        UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR (
        "Mission Simulator", 1056, Buttons[gr_screen.res][SIMULATOR_TAB].xt,
        Buttons[gr_screen.res][SIMULATOR_TAB].yt,
        &Buttons[gr_screen.res][SIMULATOR_TAB].button, UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR (
        "Cutscenes", 1057, Buttons[gr_screen.res][CUTSCENES_TAB].xt,
        Buttons[gr_screen.res][CUTSCENES_TAB].yt,
        &Buttons[gr_screen.res][CUTSCENES_TAB].button, UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR (
        "Credits", 1058, Buttons[gr_screen.res][CREDITS_TAB].xt,
        Buttons[gr_screen.res][CREDITS_TAB].yt,
        &Buttons[gr_screen.res][CREDITS_TAB].button, UI_XSTR_COLOR_GREEN);

    // common ship/weapon/intel text
    Ui_window.add_XSTR (
        "Ships", 293, Buttons[gr_screen.res][SHIPS_DATA_TAB].xt,
        Buttons[gr_screen.res][SHIPS_DATA_TAB].yt,
        &Buttons[gr_screen.res][SHIPS_DATA_TAB].button, UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR (
        "Weapons", 1553, Buttons[gr_screen.res][WEAPONS_DATA_TAB].xt,
        Buttons[gr_screen.res][WEAPONS_DATA_TAB].yt,
        &Buttons[gr_screen.res][WEAPONS_DATA_TAB].button, UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR (
        "Intelligence", 1066, Buttons[gr_screen.res][INTEL_DATA_TAB].xt,
        Buttons[gr_screen.res][INTEL_DATA_TAB].yt,
        &Buttons[gr_screen.res][INTEL_DATA_TAB].button, UI_XSTR_COLOR_GREEN);

    // common help/options/commit text
    Ui_window.add_XSTR (
        "Exit", 1418, Buttons[gr_screen.res][EXIT_BUTTON].xt,
        Buttons[gr_screen.res][EXIT_BUTTON].yt,
        &Buttons[gr_screen.res][EXIT_BUTTON].button, UI_XSTR_COLOR_PINK);
    ...
    View_window.create (
        &Ui_window, "", Tech_ship_display_coords[gr_screen.res][SHIP_X_COORD],
        Tech_ship_display_coords[gr_screen.res][SHIP_Y_COORD],
        Tech_ship_display_coords[gr_screen.res][SHIP_W_COORD],
    ...
    Buttons[gr_screen.res][EXIT_BUTTON].button.set_hotkey (
    ...
    // setup slider
    Tech_slider.create (
        &Ui_window, Tech_slider_coords[gr_screen.res][SHIP_X_COORD],
        Tech_slider_coords[gr_screen.res][SHIP_Y_COORD],
        Tech_slider_coords[gr_screen.res][SHIP_W_COORD],
        Tech_slider_coords[gr_screen.res][SHIP_H_COORD],
        (int)Ship_info.size (), Tech_slider_filename[gr_screen.res],
        &tech_scroll_list_up, &tech_scroll_list_down,
...
void techroom_close () {
    ...
    if (Tech_background_bitmap != -1) { bm_release (Tech_background_bitmap); }
...
void techroom_do_frame (float frametime) {
    ...
    if (Trackball_active) { Ui_window.set_ignore_gadgets (1); }
    else {
    ...
    case KEY_CTRLED | KEY_UP: techroom_button_pressed (CREDITS_TAB); break;
    ...
    if (B1_RELEASED) { Trackball_active = 0; }

    // check all da buttons
    for (i = 0; i < NUM_BUTTONS; i++) {
        if (Buttons[gr_screen.res][i].button.pressed ()) {
            if (techroom_button_pressed (i)) { return; }
        }
    }
    ...
    // render
    switch (Tab) {
    case SHIPS_DATA_TAB: techroom_ships_render (frametime); break;

    case WEAPONS_DATA_TAB:
    case INTEL_DATA_TAB: techroom_anim_render (frametime); break;
    }
...
// note: the name has to be pre-translated before being passed into this
// function
