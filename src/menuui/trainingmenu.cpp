...
void training_menu_init () {
    ...
    read_menu_tbl (
        NOX ("TRAINING MENU"), background_img_filename,
    ...
    if (trainingMenuBitmap < 0) {
        fs2::dialog::error (LOCATION, "Could not load in %s!", background_img_filename);
    }
    ...
    if (trainingMenuMask < 0) {
        fs2::dialog::error (LOCATION, "Could not load in %s!", background_mask_filename);
    }
    else {
        // get a pointer to bitmap by using bm_lock()
        trainingMenuMaskPtr =
            bm_lock (trainingMenuMask, 8, BMP_AABITMAP | BMP_MASK_BITMAP);
...
void training_menu_do_frame (float /*frametime*/) {
    ...
    int snazzy_action = -1;
    training_menu_choice = snazzy_menu_do (
        mask_data, Training_mask_w, Training_mask_h, num_training, region,
        &snazzy_action);
    if (snazzy_action != SNAZZY_CLICKED) { training_menu_choice = -1; }

    switch (training_menu_choice) {
    case TRAINING_MENU_TRAINING_MISSIONS_MASK: break;
    case TRAINING_MENU_REPLAY_MISSIONS_MASK:
        // TODO: load the mission and start the briefing
        break;
    case TRAINING_MENU_RETURN_MASK:
    case ESC_PRESSED: gameseq_post_event (GS_EVENT_MAIN_MENU); break;
    case -1:
        // nothing selected
        break;
    default:
