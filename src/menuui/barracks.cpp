...
static int Barracks_list_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                             42, 34, 400, 90 },
                                                           { // GR_1024
                                                             45, 51, 646,
                                                             144 } };
...
int Barracks_squad_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                       495, 177, 128, 128 },
                                                     { // GR_1024
                                                       829, 323, 128, 128 } };

// pilot squad # of # location
int Barracks_squad_number_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                              492, 307 },
                                                            { // GR_1024
                                                              794, 500 } };
...
struct barracks_buttons {
    ...
    barracks_buttons (
        const char* name, int x1, int y1, int x2, int y2, int h, int r = 0)
        : filename (name), x (x1), y (y1), text_x (x2), text_y (y2),
          hotspot (h), repeat (r) {}
};
...
void barracks_init_stats (scoring_struct* stats) {
    ...
    // Set up variables
    if (Stat_labels != NULL) { delete[] Stat_labels; }
    if (Stats != NULL) { delete[] Stats; }

    Stat_labels =
        (char(*)[STAT_COLUMN1_W]) new char[Max_stat_lines * STAT_COLUMN1_W];
    Stats =
        (char(*)[STAT_COLUMN2_W]) new char[Max_stat_lines * STAT_COLUMN2_W];
    ...
    Assert (Num_stat_lines < Max_stat_lines);
    STRCPY1 (
        Stat_labels[Num_stat_lines], XSTR ("Primary friendly hit %:", 56));
    if (stats->p_shots_fired > 0) {
        f = (float)stats->p_bonehead_hits * 100.0f /
            (float)stats->p_shots_fired;
    }
    else {
        f = 0.0f;
    }
    ...
    Assert (Num_stat_lines < Max_stat_lines);
    STRCPY1 (
        Stat_labels[Num_stat_lines], XSTR ("Secondary weapon shots:", 57));
    ...
    Assert (Num_stat_lines < Max_stat_lines);
    STRCPY1 (
        Stat_labels[Num_stat_lines], XSTR ("Secondary friendly hits:", 59));
    ...
    Assert (Num_stat_lines < Max_stat_lines);
    STRCPY1 (
        Stat_labels[Num_stat_lines], XSTR ("Secondary friendly hit %:", 61));
    if (stats->s_shots_fired > 0) {
        f = (float)stats->s_bonehead_hits * 100.0f /
            (float)stats->s_shots_fired;
    }
    else {
        f = 0.0f;
    }
    ...
    for (auto it = Ship_info.cbegin (); it != Ship_info.cend (); i++, ++it) {
        if (stats->kills[i]) {
            ...
            // Goober5000 - in case above Assert isn't triggered (such as in
            // non-debug builds)
            if (Num_stat_lines >= Max_stat_lines) { break; }
...
void barracks_set_hotkeys (bool pilot_text_enter_mode) {
    Buttons[gr_screen.res][B_PILOT_SCROLL_UP_BUTTON].button.set_hotkey (
        pilot_text_enter_mode ? KEY_UP : -1);
    Buttons[gr_screen.res][B_PILOT_SCROLL_DOWN_BUTTON].button.set_hotkey (
        pilot_text_enter_mode ? KEY_DOWN : -1);

    Buttons[gr_screen.res][B_PILOT_CREATE_BUTTON].button.set_hotkey (
        pilot_text_enter_mode ? KEY_C : -1);
    Buttons[gr_screen.res][B_PILOT_SET_ACTIVE_BUTTON].button.set_hotkey (
        pilot_text_enter_mode ? KEY_ENTER : -1);
    Buttons[gr_screen.res][B_PILOT_DELETE_BUTTON].button.set_hotkey (
        pilot_text_enter_mode ? KEY_DELETE : -1);

    Buttons[gr_screen.res][B_PIC_PREV_PILOT_BUTTON].button.set_hotkey (
        pilot_text_enter_mode ? KEY_LEFT : -1);
    Buttons[gr_screen.res][B_PIC_NEXT_PILOT_BUTTON].button.set_hotkey (
        pilot_text_enter_mode ? KEY_RIGHT : -1);

    Buttons[gr_screen.res][B_ACCEPT_BUTTON].button.set_hotkey (
        pilot_text_enter_mode ? KEY_CTRLED | KEY_ENTER : -1);
}
...
int barracks_new_pilot_selected () {
    ...
    if (strlen (Cur_pilot->callsign)) { Pilot.save_player (Cur_pilot); }

    // check if we have a valid pilot hilighted.  If so, attempt to active it
    if ((Num_pilots < 1) || (Selected_line < 0) ||
...
void barracks_set_callsign_enter_mode (bool set_callsign_enter_mode) {
    ...
    for (int idx = 0; idx < BARRACKS_NUM_BUTTONS; idx++) {
        ...
        if ((idx != B_SQUAD_PREV_BUTTON) && (idx != B_SQUAD_NEXT_BUTTON)) {
            Buttons[gr_screen.res][idx].button.enable (
                !set_callsign_enter_mode);
        }
    }
...
void barracks_create_new_pilot () {
    ...
    if (strlen (Player->callsign)) { Pilot.save_player (Player); }
    ...
    Inputbox.update_dimensions (
        Barracks_list_coords[gr_screen.res][BARRACKS_X_COORD],
...
int barracks_pilot_accepted () {
    ...
    if (!Cur_pilot->callsign[0]) { return -1; }
    ...
    if (Game_mode & GM_MULTIPLAYER) {
        ...
        player_set_squad_bitmap (
            Cur_pilot, Cur_pilot->s_squad_filename, false);
    }
...
void barracks_scroll_callsign_down () {
    ...
    // num_pilots_to_fill_height is the number of pilots that can fit in given
    // height
...
void barracks_scroll_stats_down () {
    ...
    if (Stats_scroll_offset +
            Barracks_stats_coords[gr_screen.res][BARRACKS_H_COORD] /
                font_height <
        static_cast< int > (Num_stat_lines)) {
        Stats_scroll_offset++;
        gamesnd_play_iface (InterfaceSounds::SCROLL);
    }
    else {
...
void barracks_prev_pic () {
    ...
    Pic_number--;
    if (Pic_number < 0) { Pic_number = Num_pilot_images - 1; }
...
void barracks_next_pic () {
    ...
    // reset pilot pic number
    Pic_number++;
    if (Pic_number >= Num_pilot_images) { Pic_number = 0; }
...
void barracks_next_squad_pic () {
    ...
    // reset squad pic number
    Pic_squad_number++;
    if (Pic_squad_number >= Num_pilot_squad_images) { Pic_squad_number = 0; }
...
void barracks_delete_pilot () {
    ...
    if (Player_sel_mode == PLAYER_SELECT_MODE_MULTI) {
        ...
        popup (
            ...
            XSTR (
                "Disabled!\n\nMulti and single player pilots are now "
                "identical. "
                "Deleting a multi-player pilot will also delete all "
                "single-player data for that pilot.\n\nAs a safety "
                "precaution, pilots can only be "
                "deleted from the single-player menu.",
                1598));
        return;
    }
    ...
    if (!del_rval) {
        popup (
            PF_USE_AFFIRMATIVE_ICON | PF_TITLE_BIG | PF_TITLE_RED, 1, POPUP_OK,
            XSTR (
        ...
        if (active) { Cur_pilot->callsign[0] = 0; }
    }
    ...
    Num_pilots--;
    if (Selected_line >= Num_pilots) { Selected_line = Num_pilots - 1; }

    if (active) {
        if (Selected_line >= 0) { barracks_new_pilot_selected (); }
        else {
...
// Filter out pilots of wrong type (which shouldn't be in the directory we are
// checking, but just to be safe..)
int barracks_pilot_filter (const char* filename) {
    ...
    if (rank >= Rank_pips_count) rank = Rank_pips_count - 1;

    if (r) { Pilot_ranks[Num_pilots++] = rank; }

    return (int)r;
}

// callback handler for the squadon selection buttons when they are disabled
// (in single player)
void barracks_squad_change_popup () {
    // show a popup
    popup (
...
void barracks_init_player_stuff (int mode) {
    ...
    // determine if we should be looking for single or multiplayers at the
    // outset
    ...
    if (mode == PLAYER_SELECT_MODE_SINGLE) {
        ...
        Buttons[gr_screen.res][B_SQUAD_PREV_BUTTON]
            .button.set_disabled_action (barracks_squad_change_popup);
        Buttons[gr_screen.res][B_SQUAD_NEXT_BUTTON].button.hide ();
        Buttons[gr_screen.res][B_SQUAD_NEXT_BUTTON].button.disable ();
        Buttons[gr_screen.res][B_SQUAD_NEXT_BUTTON]
            .button.set_disabled_action (barracks_squad_change_popup);
    }
    ...
    // get the list of pilots based upon whether we're in single or multiplayer
    // mode moved down here so pilotfile::verify knows which Game_mode to get
    // ranks for
    Num_pilots = 0;
    Get_file_list_filter = barracks_pilot_filter;

    Num_pilots = cf_get_file_list_preallocated (
        MAX_PILOTS, Pilots_arr, Pilots, CF_TYPE_PLAYERS, NOX ("*.plr"),
        CF_SORT_TIME, nullptr,
    ...
    for (i = 0; i < Num_pilots; i++) { Pilot_ranks[i] = ranks[i]; }
...
void barracks_button_pressed (int n) {
    switch (n) {
    case B_PILOT_SCROLL_UP_BUTTON: barracks_scroll_callsign_up (); break;

    case B_PILOT_SCROLL_DOWN_BUTTON: barracks_scroll_callsign_down (); break;

    case B_STATS_SCROLL_UP_BUTTON: barracks_scroll_stats_up (); break;

    case B_STATS_SCROLL_DOWN_BUTTON: barracks_scroll_stats_down (); break;

    case B_PIC_PREV_PILOT_BUTTON: barracks_prev_pic (); break;

    case B_PIC_NEXT_PILOT_BUTTON: barracks_next_pic (); break;

    case B_SQUAD_PREV_BUTTON: barracks_prev_squad_pic (); break;

    case B_SQUAD_NEXT_BUTTON: barracks_next_squad_pic (); break;

    case B_PILOT_SET_ACTIVE_BUTTON:
        if (barracks_new_pilot_selected ()) {
            gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);

            // if it's just the missing campaign file that failed for us then
            // don't give the second popup
            if (Campaign_file_missing) { break; }
        }
        else {
            gamesnd_play_iface (InterfaceSounds::SCROLL);
    ...
    case B_PILOT_CONVERT_BUTTON: {
        ...
        popup (
            PF_TITLE_BIG | PF_TITLE_BLUE | PF_USE_AFFIRMATIVE_ICON, 1,
            POPUP_OK,
            XSTR (
                "Disabled!\n\n\nMulti and Single Player Pilot files are now "
                "identical.\n\n"
                "Conversion between the two is no longer necessary.",
                1601));
    ...
    case B_PILOT_DELETE_BUTTON: barracks_delete_pilot (); break;
...
void barracks_display_pilot_callsigns (int prospective_pilot) {
    ...
    int multi = 0;
    if (Player_sel_mode == PLAYER_SELECT_MODE_MULTI) { multi = 1; }
    ...
    while (y + font_height <=
        ...
        gr_printf_menu (
            Barracks_list_coords[gr_screen.res][BARRACKS_X_COORD],
            Barracks_list_coords[gr_screen.res][BARRACKS_Y_COORD] + y, "%s",
            Pilots[cur_pilot_idx]);
        gr_set_bitmap (Rank_pips_bitmaps + Pilot_ranks[cur_pilot_idx]);
        gr_bitmap (
            Barracks_list_coords[gr_screen.res][BARRACKS_X_COORD] - 34,
            Barracks_list_coords[gr_screen.res][BARRACKS_Y_COORD] + y,
            GR_RESIZE_MENU);
...
void barracks_display_pilot_stats () {
    ...
    while (y + font_height <=
        ...
        if (*str == '*') {
            ...
            i = Barracks_stats_coords[gr_screen.res][BARRACKS_Y_COORD] + y +
                h / 2 - 1;
            gr_line (
                Barracks_stats_coords[gr_screen.res][BARRACKS_X_COORD], i,
                Barracks_stats_coords[gr_screen.res][BARRACKS_X_COORD] +
                    Barracks_stats_coords[gr_screen.res][BARRACKS_W_COORD] -
                    w - 2,
                i, GR_RESIZE_MENU);
            gr_line (
                Barracks_stats_coords[gr_screen.res][BARRACKS_X_COORD] +
                    Barracks_stats_coords[gr_screen.res][BARRACKS_W_COORD] + 1,
                i,
                Barracks_stats2_coords[gr_screen.res][BARRACKS_X_COORD] +
                    Barracks_stats2_coords[gr_screen.res][BARRACKS_W_COORD],
                i, GR_RESIZE_MENU);
        }
        else {
        ...
        gr_printf_menu (
            Barracks_stats_coords[gr_screen.res][BARRACKS_X_COORD] +
                Barracks_stats_coords[gr_screen.res][BARRACKS_W_COORD] - w,
            Barracks_stats_coords[gr_screen.res][BARRACKS_Y_COORD] + y, "%s",
            str);
        str = Stats[z];
        if (*str) {
            gr_printf_menu (
                Barracks_stats2_coords[gr_screen.res][BARRACKS_X_COORD],
                Barracks_stats_coords[gr_screen.res][BARRACKS_Y_COORD] + y,
                "%s", str);
        }
...
void barracks_accept_new_pilot_callsign () {
    ...
    if (!isalpha (*buf)) { z = 1; }
        ...
        for (i = 1; buf[i]; i++) {
            if (!isalpha (buf[i]) && !isdigit (buf[i]) &&
                !strchr (VALID_PILOT_CHARS, buf[i])) {
                z = 1;
                return;
            }
        }
    ...
    // to the proper directory and gets displayed correctly
    if (Player_sel_mode == PLAYER_SELECT_MODE_SINGLE) {
        Cur_pilot->flags &= ~(PLAYER_FLAGS_IS_MULTI);
    }
    else {
    ...
    if (!(Game_mode & GM_STANDALONE_SERVER)) { Pilot.save_player (Cur_pilot); }
...
void barracks_draw_pilot_pic () {
    // draw pilot pic
    if (Cur_pilot->callsign[0] && (Pic_number >= 0) &&
        (Pic_number < Num_pilot_images)) {
        if (Pilot_images[Pic_number] >= 0) {
            // JAS: This code is hacked to allow the animation to use all 256
            // colors
            gr_set_bitmap (Pilot_images[Pic_number]);
            gr_bitmap (
                Barracks_image_coords[gr_screen.res][BARRACKS_X_COORD],
            ...
            char buf[40];
            sprintf (
                buf, XSTR ("%d of %d", 71), Pic_number + 1, Num_pilot_images);
            gr_printf_menu (
                Barracks_image_number_coords[gr_screen.res][BARRACKS_X_COORD],
                Barracks_image_number_coords[gr_screen.res][BARRACKS_Y_COORD],
                "%s", buf);
        }
    }
    else {
...
void barracks_draw_squad_pic () {
    ...
    // draw pilot pic
    if (Cur_pilot->callsign[0] && (Pic_squad_number >= 0) &&
        (Pic_squad_number < Num_pilot_squad_images)) {
        if (Pilot_squad_images[Pic_squad_number] >= 0) {
            ...
            gr_bitmap (
                Barracks_squad_coords[gr_screen.res][BARRACKS_X_COORD],
            ...
            if (Player_sel_mode != PLAYER_SELECT_MODE_SINGLE) {
                sprintf (
                    buf, XSTR ("%d of %d", 71), Pic_squad_number + 1,
                    Num_pilot_squad_images);
                gr_printf_menu (
                    Barracks_squad_number_coords[gr_screen.res]
                                                [BARRACKS_X_COORD],
                    Barracks_squad_number_coords[gr_screen.res]
                                                [BARRACKS_Y_COORD],
                    "%s", buf);
...
void barracks_init () {
    ...
    for (i = 0; i < BARRACKS_NUM_BUTTONS; i++) {
        // create the object
        Buttons[gr_screen.res][i].button.create (
            &Ui_window, "", Buttons[gr_screen.res][i].x,
            Buttons[gr_screen.res][i].y, 60, 30,
            Buttons[gr_screen.res][i].repeat, 1);

        // set the sound to play when highlighted
        Buttons[gr_screen.res][i].button.set_highlight_action (
            common_play_highlight_sound);

        // set the ani for the button
        Buttons[gr_screen.res][i].button.set_bmaps (
            Buttons[gr_screen.res][i].filename);

        // set the hotspot
        Buttons[gr_screen.res][i].button.link_hotspot (
            Buttons[gr_screen.res][i].hotspot);
    }

    // add all strings
    w->add_XSTR (
        "Create", 1034, Buttons[gr_screen.res][0].text_x,
        Buttons[gr_screen.res][0].text_y, &Buttons[gr_screen.res][0].button,
        UI_XSTR_COLOR_GREEN);
    w->add_XSTR (
        "Accept", 1035, Buttons[gr_screen.res][5].text_x,
        Buttons[gr_screen.res][5].text_y, &Buttons[gr_screen.res][5].button,
        UI_XSTR_COLOR_PINK);
    w->add_XSTR (
        "Help", 928, Buttons[gr_screen.res][6].text_x,
        Buttons[gr_screen.res][6].text_y, &Buttons[gr_screen.res][6].button,
        UI_XSTR_COLOR_GREEN);
    w->add_XSTR (
        "Options", 1036, Buttons[gr_screen.res][7].text_x,
        Buttons[gr_screen.res][7].text_y, &Buttons[gr_screen.res][7].button,
        UI_XSTR_COLOR_GREEN);
    w->add_XSTR (
        "Medals", 1037, Buttons[gr_screen.res][8].text_x,
        Buttons[gr_screen.res][8].text_y, &Buttons[gr_screen.res][8].button,
        UI_XSTR_COLOR_GREEN);
    w->add_XSTR (
        "Remove", 1038, Buttons[gr_screen.res][11].text_x,
        Buttons[gr_screen.res][11].text_y, &Buttons[gr_screen.res][11].button,
        UI_XSTR_COLOR_GREEN);
    w->add_XSTR (
        "Select", 1552, Buttons[gr_screen.res][12].text_x,
        Buttons[gr_screen.res][12].text_y, &Buttons[gr_screen.res][12].button,
        UI_XSTR_COLOR_GREEN);
    w->add_XSTR (
        "Clone", 1040, Buttons[gr_screen.res][13].text_x,
        Buttons[gr_screen.res][13].text_y, &Buttons[gr_screen.res][13].button,
        UI_XSTR_COLOR_GREEN);
    w->add_XSTR (
        "Single", 1041, Buttons[gr_screen.res][14].text_x,
        Buttons[gr_screen.res][14].text_y, &Buttons[gr_screen.res][14].button,
        UI_XSTR_COLOR_GREEN);
    w->add_XSTR (
        "Multi", 1042, Buttons[gr_screen.res][15].text_x,
    ...
    List_region.create (
        &Ui_window, "", Barracks_list_coords[gr_screen.res][BARRACKS_X_COORD],
        Barracks_list_coords[gr_screen.res][BARRACKS_Y_COORD],
        Barracks_list_coords[gr_screen.res][BARRACKS_W_COORD],
    ...
    List_scroll_offset = Stats_scroll_offset = Pic_number = Pic_squad_number =
        Selected_line = 0;
    ...
    // load ramp pips
    Rank_pips_bitmaps =
        bm_load_animation ("IconRankMini.ani", &Rank_pips_count);
    ...
    // base the mode we're in (single or multi) on the status of the currently
    // selected pilot
    barracks_init_player_stuff (
        (Game_mode & GM_MULTIPLAYER) == GM_MULTIPLAYER);
}
...
void barracks_do_frame (float /*frametime*/) {
    ...
    if (Barracks_callsign_enter_mode) {
        ...
        case KEY_ENTER: barracks_accept_new_pilot_callsign (); break;
        }
    }
    else {
    ...
    // do pilot pic stuff
    if ((Pic_number >= 0) && (Pic_number < Num_pilot_images)) {
        if (Pilot_images[Pic_number] ==
            BARRACKS_IMAGE_NOT_LOADED) { // haven't tried loading it yet
    ...
    if ((Pic_squad_number >= 0) &&
        (Pic_squad_number < Num_pilot_squad_images)) {
        if (Pilot_squad_images[Pic_squad_number] ==
            BARRACKS_IMAGE_NOT_LOADED) { // haven't tried loading it yet
            Pilot_squad_images[Pic_squad_number] =
                bm_load_duplicate (Pilot_squad_image_names[Pic_squad_number]);
            if (Pilot_squad_images[Pic_squad_number] >= 0) {
                int w, h;

                bm_get_info (
                    Pilot_squad_images[Pic_squad_number], &w, &h, NULL);
                // check for invalid pilot pic file
    ...
    if (Player_sel_mode == PLAYER_SELECT_MODE_SINGLE) {
        Buttons[gr_screen.res][B_PILOT_SINGLE_MODE_BUTTON].button.draw_forced (
            2);
    }
    else {
        Buttons[gr_screen.res][B_PILOT_MULTI_MODE_BUTTON].button.draw_forced (
            2);
    }
...
void barracks_close () {
    ...
    for (int i = 0; i < Num_pilot_images; i++) {
        if (Pilot_images[i] >= 0) { bm_release (Pilot_images[i]); }
    }
