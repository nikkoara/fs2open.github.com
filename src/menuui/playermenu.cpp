...
int Max_lines; // Max number of pilots displayed in Window. Gets set in
...
UI_BUTTON Player_select_list_region; // button for detecting mouse clicks on
                                     // this screen
...
int Player_select_mode; // single or multiplayer - never set directly. use
                        // player_select_init_player_stuff()
int Player_select_num_pilots; // # of pilots on the list
int Player_select_list_start; // index of first list item to start displaying
                              // in the box
int Player_select_pilot; // index into the Pilot array of which is selected as
                         // the active pilot
int Player_select_input_mode; // 0 if the player _isn't_ typing a callsign, 1
                              // if he is
...
char Player_select_last_pilot[CALLSIGN_LEN + 10]; // callsign of the last used
                                                  // pilot, or none if there
                                                  // wasn't one
int Player_select_last_is_multi;

std::string Player_select_force_main_hall = "";

static int Player_select_no_save_pilot =
    0; // to skip save of pilot in pilot_select_close()
...
void player_select_init_player_stuff (
    int mode); // switch between single and multiplayer modes
...
/*
 * validate that a pilot/player was created with the same language FSO is
 * currently using
 *
 * @param pilots callsign
 * @note not longer needed if intel entry "primary keys" change to a
 * non-translated value
 */
bool valid_pilot_lang (char* callsign) {
    char pilot_lang[LCL_LANG_NAME_LEN + 1],
        current_lang[LCL_LANG_NAME_LEN + 1];
    ...
    if (Pilot.verify (filename.c_str (), NULL, pilot_lang)) {
        if (!strcmp (current_lang, pilot_lang)) { return true; }
    }
    return false;
}

// basically, gray out all controls (gray == 1), or ungray the controls (gray
// == 0)
void player_select_set_controls (int gray) {
    int idx;
...
void player_select_init () {
    ...
    for (i = 0; i < NUM_PLAYER_SELECT_BUTTONS; i++) {
        ...
        // create the button
        b->button.create (
            &Player_select_window, NULL, b->x, b->y, 60, 30, 1, 1);
    ...
    Player_select_list_region.create (
    ...
    // key is pressed
    Player_select_buttons[gr_screen.res][SCROLL_LIST_UP_BUTTON]
        .button.set_hotkey (KEY_UP);
    ...
    Player_select_buttons[gr_screen.res][ACCEPT_BUTTON].button.set_hotkey (
        KEY_ENTER);
    Player_select_buttons[gr_screen.res][CREATE_PILOT_BUTTON]
        .button.set_hotkey (KEY_C);
    ...
    if (Cmdline_benchmark_mode ||
        ((Player_select_num_pilots == 1) && Player_select_input_mode)) {
...
void player_select_do () {
    ...
    // enter a callsign
    if (Player_select_input_mode) { Player_select_input_box.set_focus (); }
    ...
    case KEY_ESC: {
        ...
        if (!Player_select_input_mode) { Player_select_no_save_pilot = 1; }
    ...
    if (Player_select_mode == PLAYER_SELECT_MODE_SINGLE) {
        ...
        Player_select_buttons[gr_screen.res][MULTI_BUTTON].button.draw_forced (
            2);
    }
    ...
    if (!Player_select_input_mode) { player_select_process_noninput (k); }
    else {
...
void player_select_close () {
    ...
    if (Player_select_input_mode) { player_select_cancel_create (); }
    ...
    // New pilot file makes no distinction between multi pilots and regular
    // ones, so let's do this here.
    if (Player_select_mode == PLAYER_SELECT_MODE_MULTI) {
        Player->flags |= PLAYER_FLAGS_IS_MULTI;
    }

    // WMC - Set appropriate game mode
    if (Player->flags & PLAYER_FLAGS_IS_MULTI) { Game_mode = GM_MULTIPLAYER; }
    else {
        Game_mode = GM_NORMAL;
    }
...
void player_select_button_pressed (int n) {
        ...
        if (Player_select_pilot < 0) {
            ...
            if (valid_pilot_lang (Pilots[Player_select_pilot])) {
                ...
                popup (
                    PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                    XSTR (
                        "Selected pilot was created with a different "
                        "language\n"
                        "to the currently active language.\n\n"
                        "Please select a different pilot or change the "
                        "language",
                        1637));
            }
        }
    ...
    case CLONE_BUTTON:
        ...
        if (Player_select_pilot >= 0) {
            // first we have to make sure this guy is actually loaded for when
            // we create the clone
            if (Player == NULL) {
                Player = &Players[0];
                Player->flags |= PLAYER_FLAGS_STRUCTURE_IN_USE;
            ...
            // create the new pilot (will be cloned with
            // Player_select_clone_flag_set)
    ...
    case DELETE_BUTTON:
        ...
        if (Player_select_pilot >= 0) {
            if (Player_select_mode == PLAYER_SELECT_MODE_MULTI) {
                popup (
                    PF_TITLE_BIG | PF_TITLE_RED | PF_USE_AFFIRMATIVE_ICON, 1,
                    POPUP_OK,
                    XSTR (
                ...
                // delete the pilot
                if (ret == 1) { player_select_delete_pilot (); }
            }
        }
        break;
...
int player_select_create_new_pilot () {
    ...
    if (play_scroll_sound) { gamesnd_play_iface (InterfaceSounds::SCROLL); }
...
void player_select_scroll_list_up () {
    if (Player_select_pilot == -1) { return; }
...
int player_select_get_last_pilot_info () {
    ...
    if (Player_select_last_pilot[strlen (Player_select_last_pilot) - 1] ==
            'M' ||
        Player_select_last_pilot[strlen (Player_select_last_pilot) - 1] ==
            'S') {
        Player_select_last_pilot[strlen (Player_select_last_pilot) - 1] =
            '\0'; // chop off last char, M|P
    }
...
int player_select_get_last_pilot () {
    // if the player has the Cmdline_use_last_pilot command line option set,
    // try and drop out quickly
    if (Cmdline_use_last_pilot) {
        int idx;

        if (!player_select_get_last_pilot_info ()) { return 0; }
...
void player_select_init_player_stuff (int mode) {
    ...
    // load up the list of players based upon the Player_select_mode (single or
    // multiplayer)
    Get_file_list_filter = player_select_pilot_file_filter;
    ...
    // if we have a "last_player", and they're in the list, bash them to the
    // top of the list
...
void player_select_draw_list () {
    ...
    if (gr_screen.res == 1) {
        Max_lines =
            145 / gr_get_font_height (); // Make the max number of lines
                                         // dependent on the font height. 145
                                         // and 85 are magic numbers, based on
                                         // the window size in retail.
    }
    else {
    ...
    for (idx = 0; idx < Max_lines; idx++) {
        ...
        if ((idx + Player_select_list_start) == Player_select_pilot) {
            // if he's the active pilot and is also the current selection,
            // super-highlight him
        ...
        gr_printf_menu (
            ...
            "%s", Pilots[idx + Player_select_list_start]);
    }
}
...
void player_select_process_noninput (int k) {
    ...
    case KEY_ESC: gameseq_post_event (GS_EVENT_QUIT_GAME); break;
    ...
    case KEY_DELETE: player_select_button_pressed (DELETE_BUTTON); break;
    }
    ...
    // if the player has double clicked on a valid pilot, choose it and hit the
    // accept button
...
void player_select_process_input (int k) {
    ...
    switch (k) {
    // cancel create pilot
    case KEY_ESC: player_select_cancel_create (); break;
    ...
    case KEY_ENTER:
        ...
        if (!*buf || (idx < Player_select_num_pilots)) { z = 1; }
    ...
    default: player_select_set_middle_text (""); break;
    }
}
...
void player_select_display_copyright () {
    ...
    if (Unicode_text_mode) {
        ...
        sprintf (
            Copyright_msg2,
            XSTR (
                "Copyright %c 1999, Volition, Inc.  All rights reserved.",
                385),
            lcl_get_font_index (font::get_current_fontnum ()) + 4);
    }
...
void player_select_display_all_text () {
    ...
    if (strlen (Player_select_bottom_text)) {
        ...
        gr_printf_menu (
            w, Player_select_bottom_text_y[gr_screen.res], "%s",
    ...
    if (strlen (Player_select_middle_text)) {
        ...
        gr_set_color_fast (&Color_bright_white);
        gr_printf_menu (
            w, Player_select_middle_text_y[gr_screen.res], "%s",
...
void player_select_set_bottom_text (const char* txt) {
    if (txt) { strncpy (Player_select_bottom_text, txt, 149); }
}

void player_select_set_middle_text (const char* txt) {
    if (txt) { strncpy (Player_select_middle_text, txt, 149); }
}
...
void player_select_cancel_create () {
    ...
    // make sure we correct the Selected_pilot index to account for the
    // cancelled action
    if (Player_select_num_pilots == 0) { Player_select_pilot = -1; }
...
DCF (
    bastion, "Sets the player to be on the bastion (or any other main hall)") {
    int idx;

    if (gameseq_get_state () != GS_STATE_INITIAL_PLAYER_SELECT) {
        dc_printf (
            "This command can only be run while in the initial player select "
            "screen.\n");
        return;
    }
    ...
    if (dc_optional_string_either ("help", "--help")) {
        ...
        dc_printf (
            "    [index] -- optional main hall index; if not supplied, "
            "defaults to 1\n");
        return;
    }
...
void player_tips_init () {
    ...
    try {
        ...
        while (!optional_string ("#end")) {
            ...
            if (Num_player_tips >= MAX_PLAYER_TIPS) { break; }
            Player_tips[Num_player_tips++] =
                stuff_and_malloc_string (F_NAME, NULL);
        }
    }
...
void player_tips_popup () {
    ...
    if ((Player != NULL) && !Player->tips) { return; }
    // only show tips once per instance of FreeSpace
    if (Player_tips_shown == 1) { return; }
    Player_tips_shown = 1;
    ...
    do {
        ...
        ret = popup (
            PF_NO_SPECIAL_BUTTONS | PF_TITLE | PF_TITLE_WHITE, 3,
        ...
        // next
        case 1:
            if (tip >= Num_player_tips - 1) { tip = 0; }
            else {
                tip++;
            }
            break;
