...
void main_hall_init(const std::string &main_hall_name)
{
        ...
        if (Main_hall->tooltip_padding == -1) {
                Main_hall->tooltip_padding = Main_hall_default_tooltip_padding[gr_get_resolution_class(
                        ...
                        if (Main_hall->misc_anim_paused.at(idx)) {
                                ...
                                if (Main_hall->misc_anim_delay.at(idx).at(0) == -1) {
                                                        ...
                                                        for (jdx = 0; jdx < Main_hall->num_misc_animations;
                                                             jdx++) {
                                                                if (Main_hall->misc_anim_group.at(jdx) == Main_hall->misc_anim_group.at(idx)) {
                                                                        ASSERT(
                                                                                group_anims_weve_checked.size() < INT_MAX);
                                        ...
                                        if (regen_idx >= 0) {
                                                ...
                                                Main_hall->misc_anim_delay.at(regen_idx).at(0) = timestamp(min + (int)(fs2::prng::randf(0) * (max - min)));
                                        }
                                        ...
                                        ASSERT(
                                                Main_hall->misc_anim_sound_flag.at(idx).size() < INT_MAX);
                                        for (s_idx = 0;
                                             s_idx < (int)Main_hall->misc_anim_sound_flag.at(idx).size();
                                             s_idx++) {
                                ...
                                ASSERT(
                                        Main_hall->misc_anim_special_sounds.at(idx).size() < INT_MAX);
                                for (s_idx = 0;
                                     s_idx < (int)Main_hall->misc_anim_special_sounds.at(idx).size();
                                     s_idx++) {
                                        // if we've passed the trigger point, then play the sound
                                        // and break out of the loop
                                        if ((Main_hall_misc_anim.at(idx).current_frame >= Main_hall->misc_anim_special_trigger.at(idx).at(
                                                     s_idx))
                                            && !Main_hall->misc_anim_sound_flag.at(idx).at(s_idx)) {
                                                Main_hall->misc_anim_sound_flag.at(idx).at(s_idx) = 1;

                                                auto sound = Main_hall->misc_anim_special_sounds.at(idx).at(
                                                        s_idx);
                                ...
                                if (Main_hall_misc_anim.at(idx).current_frame == Main_hall_misc_anim.at(idx).num_frames - 1) {
                                        ...
                                        if (Main_hall->misc_anim_modes.at(idx) != MISC_ANIM_MODE_HOLD) {
                                                ...
                                                ASSERT(
                                                        Main_hall->misc_anim_sound_flag.at(idx).size() < INT_MAX);
                                                for (s_idx = 0;
                                                     s_idx < (int)Main_hall->misc_anim_sound_flag.at(idx)
                                                                     .size();
                                                     s_idx++) {
...
void main_hall_mouse_release_region(int region)
{
        ...
        if (Main_hall_door_anim.at(region).num_frames > 0) {
                ...
                snd_set_pos(Main_hall_door_sound_handles.at(region), (float)((Main_hall_door_anim.at(region).keyframe) ? Main_hall_door_anim.at(region).keyframe : Main_hall_door_anim.at(region).num_frames - Main_hall_door_anim.at(region).current_frame) / (float)Main_hall_door_anim.at(region).num_frames, 1);
        }
}
...
void main_hall_mouse_grab_region(int region)
{
        ...
        if ((Main_hall_door_anim.at(region).num_frames > 0) && (Main_hall_door_anim.at(region).current_frame != -1)) {
                snd_set_pos(
                        ...
                        (float)Main_hall_door_anim.at(region).current_frame / (float)Main_hall_door_anim.at(region).num_frames,
                        1);
        }
}
...
void main_hall_handle_random_intercom_sounds()
{
        ...
        if ((Main_hall_next_intercom_sound_stamp == -1) && (!Main_hall_intercom_sound_handle.isValid())) {
                Main_hall_next_intercom_sound_stamp = timestamp((int)((rand() * RAND_MAX_1f) * (float)(Main_hall->intercom_delay.at(Main_hall_next_intercom_sound).at(1) - Main_hall->intercom_delay.at(Main_hall_next_intercom_sound).at(0))));
        }
                ...
                while (!optional_string("#end")) {
                         ...
                         */
                                ...
                                for (idx = 0; idx < m->num_misc_animations; idx++) {
                                        ...
                                        ASSERT(
                                                m->misc_anim_special_sounds.at(idx).size() < INT_MAX);
                                        for (s_idx = 0;
                                             s_idx < (int)m->misc_anim_special_sounds.at(idx).size();
                                             s_idx++) {
...
int main_hall_is_vasudan()
{
        ...
        return (
                !strcasecmp(Main_hall->music_name.c_str(), "Psampik") || !strcasecmp(Main_hall->music_name.c_str(), "Psamtik"));
}
