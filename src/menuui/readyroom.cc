...
int Campaign_list_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                      491, 108, 115, 279 },
                                                    { // GR_1024
                                                      491, 175, 115, 459 } };
...
struct sim_room_buttons {
    ...
    sim_room_buttons (
        const char* name, int x1, int y1, int xt1, int yt1, int h)
...
const char* Sim_filename[GR_NUM_RESOLUTIONS] = { "LoadMission",
                                                 "2_LoadMission" };
const char* Sim_mask_filename[GR_NUM_RESOLUTIONS] = { "LoadMission-m",
                                                      "2_LoadMission-m" };

const char* Campaign_filename[GR_NUM_RESOLUTIONS] = { "Campaign",
                                                      "2_Campaign" };
const char* Campaign_mask_filename[GR_NUM_RESOLUTIONS] = { "Campaign-m",
                                                           "2_Campaign-m" };

const char* Sim_room_slider_filename[GR_NUM_RESOLUTIONS] = { "slider",
                                                             "2_slider" };
...
int Sim_room_slider_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                        4, 131, 20, 245 },
                                                      { // GR_1024
                                                        5, 209, 32, 392 } };
...
// icon offsets
static int Sim_volition_icon_x[GR_NUM_RESOLUTIONS] = { 38, 49 };
...
void sim_room_blit_icons (
    int line_index, int y_start, fs_builtin_mission* fb = NULL, int is_md = 0);
...
int sim_room_line_add (
    int type, char* name, char* filename, int x, int y, int flags) {
...
int sim_room_campaign_mission_filter (const char* filename) {
    ...
    num = mission_campaign_get_mission_list (
        filename, &Campaign_missions[Num_campaign_missions],
...
int sim_room_standalone_mission_filter (const char* filename) {
    ...
    if (campaign_mission_hash_collision (filename)) { return 0; }
...
// builds up list of already played missions in a campaign and adds them to
// missions simulator processes one mission per frame
//
// returns 1 if finished with all missions, 0 otherwise
//
int build_campaign_mission_list_do_frame () {
    int font_height = gr_get_font_height ();
    char str[256];
    static int valid_missions_with_info =
        0; // we use this to avoid blank entries in the mission list
    ...
    if (Campaign.missions[Num_campaign_missions_with_info].completed ||
        ...
        if (!get_mission_info (
            ...
            // determine some extra information
            int flags = 0;
            fs_builtin_mission* fb = game_find_builtin_mission (
                Campaign.missions[Num_campaign_missions_with_info].name);
            if ((fb != NULL) && (fb->flags & FSB_FROM_VOLITION)) {
                flags |= READYROOM_FLAG_FROM_VOLITION;
            }
...
// Builds list of either (1) standalone missions or (2) already played missions
// in current campaign First time through, it builds list, next time it uses
// precompiled list
void sim_room_build_listing () {
    ...
    if (Player->readyroom_listing_mode == MODE_MISSIONS) {
        if (Hash_table_inited) {
            if (!Standalone_mission_names_inited) { // Is this the first time
                                                    // through
                // build_list_do_frame builds list and adds sim room line and
                // sets Standalone_mission_names_inited
                popup_till_condition (
                    build_standalone_mission_list_do_frame, POPUP_CANCEL,
                ...
                for (i = 0; i < Num_standalone_missions_with_info; i++) {
                    if (Standalone_mission_names[i]) {
                        ...
                        fs_builtin_mission* fb =
                            game_find_builtin_mission (full_filename);
                        if ((fb != NULL) && (fb->flags & FSB_FROM_VOLITION)) {
                            flags |= READYROOM_FLAG_FROM_VOLITION;
                        }
                        ...
                        sim_room_line_add (
                            READYROOM_LINE_MISSION,
                            Standalone_mission_names[i], Mission_filenames[i],
                            list_x1 + M_TEXT_X, y, flags);
                        y += font_height + 2;
                    }
                }
        ...
        if (!Campaign_mission_names_inited) { // Is this the first time through
            // builds list, adds sim room line and sets
            // Campaign_mission_names_inited
            popup_till_condition (
                build_campaign_mission_list_do_frame, POPUP_CANCEL,
            ...
            for (i = 0; i < Num_campaign_missions_with_info; i++) {
                if (Campaign_mission_names[i]) {
                    ...
                    sim_room_line_add (
                        READYROOM_LINE_CMISSION, Campaign_mission_names[i],
                        Campaign.missions[i].name, list_x1 + C_SUBTEXT_X, y,
                        flags);
                    y += font_height +
                         2; // Goober5000 - added +2 to conform with above
                }
            }
        }
    }
    ...
    Sim_room_slider.set_numberItems (
        (Num_lines > max_num_entries_viewable)
            ? (Num_lines - max_num_entries_viewable)
            : 0);
}
...
void sim_room_reset_campaign_listing () {
    ...
    if (!Campaign_mission_names_inited) return;
...
int sim_room_line_query_visible (int n) {
    ...
    if ((n < 0) || (n >= Num_lines)) return 0;

    y = sim_room_lines[n].y - sim_room_lines[Scroll_offset].y;
    if ((y < 0) || (y + gr_get_font_height () > list_h)) return 0;
...
void sim_room_scroll_screen_down () {
    if (sim_room_lines[Num_lines - 1].y + gr_get_font_height () >
        sim_room_lines[Scroll_offset].y + list_h) {
        Scroll_offset++;
...
int readyroom_continue_campaign () {
    // check for possible mission loop, need to do this before calling
    // mission_campaign_next_mission()!
    if (!(Game_mode & GM_MULTIPLAYER) && !Campaign.loop_enabled &&
        (Campaign.current_mission == -1) && (Campaign.prev_mission != -1) &&
        (Campaign.next_mission != -1)) {
        if (Campaign.missions[Campaign.prev_mission].flags &
            CMISSION_FLAG_HAS_LOOP) {
            // NOTE: the order of these calls is *very* important

            // we must manually set the current mission to the *previous*
            // mission and the mission name. this isn't the same thing that is
    ...
    if (mc_rval == -1) { // is campaign and next mission valid?
        ...
        popup (
            ...
            XSTR (
                "The campaign is over.  To replay the campaign, either create "
                "a new pilot or restart the campaign in the campaign room.",
                112));
        return -1;
    }
    else if (mc_rval == -2) {
        ...
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            NOX ("The current campaign has no missions"));
        return -1;
    }
...
void sim_room_commit () {
    if ((Selected_line >= Num_lines) ||
        !sim_room_lines[Selected_line].filename) {
        gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
        return;
    }
...
int sim_room_button_pressed (int n) {
    ...
    case CAMPAIGN_TAB:
        if (!strlen (Campaign.filename)) {
            popup (
                ...
                XSTR (
                    "The currently active campaign cannot be found, unable to "
    ...
    case COMMIT_BUTTON: sim_room_commit (); break;
...
//
// Initialize the sim_room assignment screen system.  Called when
// GS_STATE_sim_room_SCREEN is entered.
//
...
void sim_room_init () {
    ...
    // force mode to valid range.  I once had a bogus value here.  Don't know
    ...
    *Game_current_mission_filename = 0;
    ...
    Ui_window.add_XSTR (
        "Single Missions", 1060, Buttons[gr_screen.res][MISSION_TAB].xt,
        Buttons[gr_screen.res][MISSION_TAB].yt,
        &Buttons[gr_screen.res][MISSION_TAB].button, UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR (
        "Campaign Missions", 1061, Buttons[gr_screen.res][CAMPAIGN_TAB].xt,
        Buttons[gr_screen.res][CAMPAIGN_TAB].yt,
        &Buttons[gr_screen.res][CAMPAIGN_TAB].button, UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR (
        "Help", 928, Buttons[gr_screen.res][HELP_BUTTON].xt,
        Buttons[gr_screen.res][HELP_BUTTON].yt,
        &Buttons[gr_screen.res][HELP_BUTTON].button, UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR (
        "Commit", 1062, Buttons[gr_screen.res][COMMIT_BUTTON].xt,
        Buttons[gr_screen.res][COMMIT_BUTTON].yt,
        &Buttons[gr_screen.res][COMMIT_BUTTON].button, UI_XSTR_COLOR_PINK);
    Ui_window.add_XSTR (
        "Options", 1036, Buttons[gr_screen.res][OPTIONS_BUTTON].xt,
        Buttons[gr_screen.res][OPTIONS_BUTTON].yt,
        &Buttons[gr_screen.res][OPTIONS_BUTTON].button, UI_XSTR_COLOR_GREEN);

    // common tab button text
    Ui_window.add_XSTR (
        "Technical Database", 1055,
        Buttons[gr_screen.res][TECH_DATABASE_BUTTON].xt,
        Buttons[gr_screen.res][TECH_DATABASE_BUTTON].yt,
        &Buttons[gr_screen.res][TECH_DATABASE_BUTTON].button,
        UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR (
        "Mission Simulator", 1056, Buttons[gr_screen.res][SIMULATOR_BUTTON].xt,
        Buttons[gr_screen.res][SIMULATOR_BUTTON].yt,
        &Buttons[gr_screen.res][SIMULATOR_BUTTON].button, UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR (
        "Cutscenes", 1057, Buttons[gr_screen.res][CUTSCENES_BUTTON].xt,
        Buttons[gr_screen.res][CUTSCENES_BUTTON].yt,
        &Buttons[gr_screen.res][CUTSCENES_BUTTON].button, UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR (
        "Credits", 1058, Buttons[gr_screen.res][CREDITS_BUTTON].xt,
        Buttons[gr_screen.res][CREDITS_BUTTON].yt,
        &Buttons[gr_screen.res][CREDITS_BUTTON].button, UI_XSTR_COLOR_GREEN);

    // misc text - not associated with any buttons
    Ui_window.add_XSTR (
        "Mission", 1063,
        Sim_misc_text_coords[gr_screen.res][SIM_MISC_TEXT_MISSION][0],
        Sim_misc_text_coords[gr_screen.res][SIM_MISC_TEXT_MISSION][1], NULL,
        UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR (
        "Filename", 1064,
        Sim_misc_text_coords[gr_screen.res][SIM_MISC_TEXT_FILENAME][0],
        Sim_misc_text_coords[gr_screen.res][SIM_MISC_TEXT_FILENAME][1], NULL,
        UI_XSTR_COLOR_GREEN);
    ...
    // set up sim_rooms for buttons so we draw the correct animation frame when
    // a key is pressed
    Buttons[gr_screen.res][SCROLL_UP_BUTTON].button.set_hotkey (KEY_PAGEUP);
    Buttons[gr_screen.res][SCROLL_DOWN_BUTTON].button.set_hotkey (
        KEY_PAGEDOWN);
    Buttons[gr_screen.res][COMMIT_BUTTON].button.set_hotkey (
    ...
    // if there is no campaign available then force the use of mode_missions
    // only
    if (!strlen (Campaign.filename))
        Player->readyroom_listing_mode = MODE_MISSIONS;
    ...
    Sim_room_slider.create (
        &Ui_window, Sim_room_slider_coords[gr_screen.res][X_COORD],
        ...
        &sim_room_scroll_screen_down, &sim_room_scroll_capture);

    Num_campaign_missions_with_info = Num_standalone_missions_with_info =
        Standalone_mission_names_inited = Campaign_mission_names_inited = 0;
    Simroom_show_all = 0;
    sim_room_build_listing ();
...
// sim_room_close()
//
// Cleanup the sim_room assignment screen system.  Called when
// GS_STATE_sim_room_SCREEN is left.
//
void sim_room_close () {
    int i;
...
// sim_room_do_frame()
//
// Called once per frame to process user input for the sim_room Assignment
// Screen
//
void sim_room_do_frame (float /*frametime*/) {
    ...
    for (i = 0; i < NUM_BUTTONS; i++) {
        if (Buttons[gr_screen.res][i].button.pressed ()) {
            if (sim_room_button_pressed (i)) { return; }
        }
    }
    ...
    if (!Buttons[gr_screen.res][CAMPAIGN_TAB].button.button_down () &&
        !Buttons[gr_screen.res][MISSION_TAB].button.button_down ()) {
        if (Player->readyroom_listing_mode == MODE_CAMPAIGNS) {
    ...
    if (Player->readyroom_listing_mode == MODE_CAMPAIGNS) {
        ...
        if (Campaign.filename[0] != '\0') {
            sprintf (
                buf, NOX ("%s%s"), Campaign.filename, FS_CAMPAIGN_FILE_EXT);
            font::force_fit_string (buf, 255, list_w2);
            gr_printf_menu (
                list_x2, Mission_list_coords[gr_screen.res][1], "%s", buf);
            ...
            if (fb != NULL) {
                // sim_room_blit_icons(0,
                // Mission_list_coords[gr_screen.res][1], fb, 0);
            }
        }
    }
    ...
    while (sim_room_line_query_visible (line)) {
        ...
        if (sim_room_lines[line].type != READYROOM_LINE_CAMPAIGN) {
            List_buttons[line - Scroll_offset].update_dimensions (
                list_x1, y, list_x2 + list_w2 - list_x1, font_height);
...
void sim_room_blit_icons (
    int line_index, int y_start, fs_builtin_mission* fb, int /*is_md*/) {
    int is_from_volition = 0;

    // determine icon status
    if (fb == NULL) {
        is_from_volition =
            (sim_room_lines[line_index].flags & READYROOM_FLAG_FROM_VOLITION)
                ? 1
                : 0;
    }
    else {
    ...
    if (is_from_volition &&
        (Mission_icon_bitmaps[MISSION_ICON_VOLITION] >= 0)) {
        gr_set_bitmap (Mission_icon_bitmaps[MISSION_ICON_VOLITION]);
        gr_bitmap (
            Sim_volition_icon_x[gr_screen.res],
            y_start + MISSION_ICON_VOLITION_Y_OFFSET, GR_RESIZE_MENU);
    }
}
...
UI_XSTR Cr_text[GR_NUM_RESOLUTIONS][CR_NUM_TEXT] = {
    {
        // GR_640
        { "Restart", 1403, 569, 326, UI_XSTR_COLOR_GREEN, -1,
          &Cr_buttons[0][CR_RESET_BUTTON].button },
        { "Campaign", 1404, 569, 337, UI_XSTR_COLOR_GREEN, -1,
          &Cr_buttons[0][CR_RESET_BUTTON].button },
        { "Select", 1409, 568, 413, UI_XSTR_COLOR_PINK, -1,
          &Cr_buttons[0][CR_COMMIT_BUTTON].button },
    },
    {
        // GR_1024
        { "Restart", 1403, 922, 523, UI_XSTR_COLOR_GREEN, -1,
          &Cr_buttons[1][CR_RESET_BUTTON].button },
        { "Campaign", 1404, 922, 538, UI_XSTR_COLOR_GREEN, -1,
          &Cr_buttons[1][CR_RESET_BUTTON].button },
        { "Select", 1409, 921, 665, UI_XSTR_COLOR_PINK, -1,
...
void campaign_room_build_listing () {
    ...
    for (i = 0; i < Num_campaigns; i++) {
        if (Campaign_names[i] != NULL) {
            ...
            int flags = 0;
            fs_builtin_mission* fb =
                game_find_builtin_mission (Campaign_file_names[i]);
            ...
            sim_room_line_add (
                READYROOM_LINE_CAMPAIGN, Campaign_names[i],
                Campaign_file_names[i], Cr_list_coords[gr_screen.res][0], y,
                flags);
            y += font_height + 2;
        }
    }
...
void campaign_room_scroll_info_down () {
    if ((Num_info_lines - Desc_scroll_offset) * gr_get_font_height () >
        Cr_info_coords[gr_screen.res][3]) {
        Desc_scroll_offset++;
        gamesnd_play_iface (InterfaceSounds::SCROLL);
    }
    else
...
int campaign_room_reset_campaign (int n) {
    ...
    // "Warning\nThis will cause all progress in your\ncurrent campaign to be
    // lost", 110), Campaign_names[n]);
    z = popup (
        PF_TITLE_BIG | PF_TITLE_RED, 2, POPUP_CANCEL, POPUP_OK,
        XSTR (
            "Warning\nThis will cause all progress in your\ncurrent campaign "
            "to be lost",
            110));
...
void campaign_room_commit () {
    ...
    if (strcasecmp (
        ...
        int load_status = mission_campaign_load (
        ...
        if (load_status == 0) {
            ...
            // sanity check: if we just loaded a savefile, but we have no next
            // mission, then we are switching back to an old campaign that we
            // previously completed, and we want to clear it to start afresh
        ...
        // TODO: other return values were never checked previously; do we need
        // to check them???
    }

    if (mission_campaign_next_mission ()) { // is campaign and next mission
                                            // valid?
        gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
        return;
    }
...
int campaign_room_button_pressed (int n) {
    switch (n) {
    case CR_SCROLL_UP_BUTTON: sim_room_scroll_screen_up (); break;

    case CR_SCROLL_DOWN_BUTTON: sim_room_scroll_screen_down (); break;

    case CR_SCROLL_INFO_UP_BUTTON: campaign_room_scroll_info_up (); break;

    case CR_SCROLL_INFO_DOWN_BUTTON: campaign_room_scroll_info_down (); break;
    ...
    case CR_RESET_BUTTON:
        if ((Active_campaign_index < 0) ||
            (Active_campaign_index >= Num_campaigns))
            gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
...
void campaign_room_init () {
    ...
    if (Campaign_background_bitmap_mask < 0) {
        WARNINGF (LOCATION, "Error loading campaign room mask %s",Campaign_mask_filename[gr_screen.res]);
    }
    else {
    ...
    // set up sim_rooms for buttons so we draw the correct animation frame when
    // a key is pressed
    Cr_buttons[gr_screen.res][CR_SCROLL_UP_BUTTON].button.set_hotkey (
        KEY_PAGEUP);
    Cr_buttons[gr_screen.res][CR_SCROLL_DOWN_BUTTON].button.set_hotkey (
        KEY_PAGEDOWN);
    Cr_buttons[gr_screen.res][CR_RESET_BUTTON].button.set_hotkey (KEY_DELETE);
    Cr_buttons[gr_screen.res][CR_COMMIT_BUTTON].button.set_hotkey (
        KEY_CTRLED | KEY_ENTER);
    ...
    if (Background_bitmap < 0) {
        WARNINGF (LOCATION, "Error loading campaign room background %s",Campaign_filename[gr_screen.res]);
    }
    ...
    if (!load_failed) { mission_campaign_next_mission (); }
    else {
        Campaign.filename[0] = 0;
        Campaign.num_missions = 0;
...
void campaign_room_close () {
    ...
    // Reset info text pointers and size since they may contain pointers into
    // the campaign description which will be freed soon.
...
void campaign_room_do_frame (float /*frametime*/) {
    ...
    if (Campaign_background_bitmap_mask == -1) {
        {
            ...
            int reset_campaign = popup (
                ...
                "Restart Campaign",
                "Campaign Room only available in full version. However, you "
                "may restart the campaign.");
            if (reset_campaign == 1) {
    ...
    if ((k > 0) || B1_JUST_RELEASED) {
        if (help_overlay_active (Campaign_room_overlay_id)) {
            help_overlay_set_state (
                Campaign_room_overlay_id, gr_screen.res, 0);
            Ui_window.set_ignore_gadgets (0);
            k = 0;
        }
    }
    ...
    case KEY_UP: // scroll list up
        if (Selected_campaign_index < 0) Selected_campaign_index = 1;
    ...
    for (i = 0; i < CR_NUM_BUTTONS; i++) {
        if (Cr_buttons[gr_screen.res][i].button.pressed ()) {
            if (campaign_room_button_pressed (i)) { return; }
        }
    }
    ...
    line = Scroll_offset;
    while (sim_room_line_query_visible (line)) {
        y = Cr_list_coords[gr_screen.res][1] + sim_room_lines[line].y -
            sim_room_lines[Scroll_offset].y;

        List_buttons[line - Scroll_offset].update_dimensions (
            Cr_list_coords[gr_screen.res][0], y,
        ...
        if (!strcasecmp (sim_room_lines[line].filename, Campaign.filename)) {
            ...
            i = y + font_height / 2 - 1;
            gr_circle (
                Cr_list_coords[gr_screen.res][0] - 6, i, 5, GR_RESIZE_MENU);

            gr_set_color_fast (&Color_bright_white);
            gr_line (
                Cr_list_coords[gr_screen.res][0] - 10, i,
                Cr_list_coords[gr_screen.res][0] - 8, i, GR_RESIZE_MENU);
            gr_line (
                Cr_list_coords[gr_screen.res][0] - 6, i - 4,
                Cr_list_coords[gr_screen.res][0] - 6, i - 2, GR_RESIZE_MENU);
            gr_line (
                Cr_list_coords[gr_screen.res][0] - 4, i,
                Cr_list_coords[gr_screen.res][0] - 2, i, GR_RESIZE_MENU);
            gr_line (
                Cr_list_coords[gr_screen.res][0] - 6, i + 2,
                Cr_list_coords[gr_screen.res][0] - 6, i + 4, GR_RESIZE_MENU);
        }
        ...
        font::force_fit_string (
            ...
            Cr_list_coords[gr_screen.res][0] +
                Cr_list_coords[gr_screen.res][2] - sim_room_lines[line].x);
        gr_printf_menu (sim_room_lines[line].x, y, "%s", buf);
        line++;
    }
    ...
    while (y + font_height <= Cr_info_coords[gr_screen.res][3]) {
        ...
        gr_string (
            Cr_info_coords[gr_screen.res][0],
            Cr_info_coords[gr_screen.res][1] + y, line_text, GR_RESIZE_MENU);
        y += font_height;
        i++;
    }
    ...
    if (Num_campaigns < 1) {
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            XSTR ("No campaigns are available!", 1613));
...
void sim_room_load_mission_icons () {
    ...
    for (idx = 0; idx < NUM_MISSION_ICONS; idx++) {
        ...
        Mission_icon_bitmaps[idx] =
            bm_load (Mission_icon_bitmap_filenames[idx]);
    }
}
