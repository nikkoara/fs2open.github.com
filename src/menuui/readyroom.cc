...
int campaign_mission_hash_collision(const char *filename)
{
        ...
        do {
                if (!strcasecmp(filename, cur_node->filename)) {
                        return 1;
                }
...
int readyroom_continue_campaign()
{
        ...
        if (!(Game_mode & GM_MULTIPLAYER) && !Campaign.loop_enabled && (Campaign.current_mission == -1) && (Campaign.prev_mission != -1) && (Campaign.next_mission != -1)) {
        ...
        int line_index, int y_start, fs_builtin_mission *fb, int /*is_md*/)
{
        ...
        if (fb == NULL) {
                is_from_volition = (sim_room_lines[line_index].flags & READYROOM_FLAG_FROM_VOLITION)
                                           ? 1
                                           : 0;
        } else {
...
void campaign_room_commit()
{
        ...
        if (strcasecmp(
            ...
            != 0) {
                ...
                if ((Campaign.flags & CF_CUSTOM_TECH_DATABASE) || !strcasecmp(Campaign.filename, "freespace2")) {
...
void campaign_room_do_frame(float /*frametime*/)
{
        ...
        while (sim_room_line_query_visible(line)) {
                ...
                font::force_fit_string(
                        buf, 255,
                        Cr_list_coords[gr_screen.res][0] + Cr_list_coords[gr_screen.res][2] - sim_room_lines[line].x);
                gr_printf_menu(sim_room_lines[line].x, y, "%s", buf);
                line++;
        }
