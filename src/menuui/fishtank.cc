...
void fish_generate()
{
        ...
        f->y = fs2::prng::randf(0, -40.0f, (float)gr_screen.max_h_unscaled_zoomed + 40.0f);
...
void fishtank_process()
{
        ...
        for (idx = 0; idx < MAX_FISH; idx++) {
                ...
                onscreen = 0;
                if ((f->x < (float)gr_screen.max_w_unscaled_zoomed) && ((f->x + FISH_ANIM_WIDTH) >= 0.0f) && (f->y < (float)gr_screen.max_h_unscaled_zoomed) && ((f->y + FISH_ANIM_HEIGHT) >= 0.0f)) {
                        onscreen = 1;
                }
