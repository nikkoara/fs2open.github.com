...
void credits_load_music (char* fname) {
    if (Credits_music_handle != -1) { return; }
...
void credits_start_music () {
    if (Credits_music_handle != -1) {
        if (!audiostream_is_playing (Credits_music_handle)) {
            audiostream_play (
                Credits_music_handle, Master_event_music_volume, 1);
        }
    }
    else {
...
void credits_parse_table (const char* filename) {
    try {
        ...
        if (optional_string ("$Number of Images:")) {
            int temp;
            stuff_int (&temp);
            if (temp > 0) Credits_num_images = temp;
        }
        if (optional_string ("$Start Image Index:")) {
            stuff_int (&Credits_artwork_index);

            // bounds check
            if (Credits_artwork_index < 0) { Credits_artwork_index = 0; }
        ...
        if (optional_string ("$Text scroll rate:")) {
            ...
            if (Credits_scroll_rate < 0.01f) Credits_scroll_rate = 0.01f;
        }
        ...
        while (!check_for_eof_raw () && !check_for_string_raw ("#end")) {
            ...
            // This is a bit odd but it means if a total conversion uses
            // different credits the Volition credit won't happen Also don't
            // append the default credits anymore when there was already a
            // parsed table
            ...
            if (line.empty ()) {
                // If the line is empty then just append a newline, don't
                // bother with splitting it first
                credits_text.append ("\n");
            }
            else {
                ...
                numLines = split_str (
                    line.c_str (), Credits_text_coords[gr_screen.res][2],
                    charNum, lines);
                ...
                // Now add all splitted lines to the credit text and append a
                // newline to the end
...
void credits_init () {
    ...
    int credits_spooled_music_index =
        event_music_get_spooled_music_index (Credits_music_name);
    if (credits_spooled_music_index != -1) {
        char* credits_wavfile_name =
            Spooled_music[credits_spooled_music_index].filename;
    ...
    if (!Credits_parsed) {
        ...
        case START:
            Credit_text_parts.insert (
                Credit_text_parts.begin (), fs2_open_credit_text);
            break;
    ...
    for (iter = Credit_text_parts.begin (); iter != Credit_text_parts.end ();
         ++iter) {
        for (std::string::iterator ii = iter->begin (); ii != iter->end ();
             ++ii) {
            ch = *ii;
            switch (ch) {
            case -4: ch = 129; break;
    ...
    for (iter = Credit_text_parts.begin (); iter != Credit_text_parts.end ();
         ++iter) {
        gr_get_string_size (
            NULL, &temp_h, iter->c_str (), (int)iter->length ());
    ...
    Ui_window.add_XSTR (
        ...
        Buttons[TECH_DATABASE_BUTTON][gr_screen.res].xt,
        Buttons[TECH_DATABASE_BUTTON][gr_screen.res].yt,
        &Buttons[TECH_DATABASE_BUTTON][gr_screen.res].button,
        UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR (
        "Mission Simulator", 1056, Buttons[SIMULATOR_BUTTON][gr_screen.res].xt,
        Buttons[SIMULATOR_BUTTON][gr_screen.res].yt,
        &Buttons[SIMULATOR_BUTTON][gr_screen.res].button, UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR (
        "Cutscenes", 1057, Buttons[CUTSCENES_BUTTON][gr_screen.res].xt,
        Buttons[CUTSCENES_BUTTON][gr_screen.res].yt,
        &Buttons[CUTSCENES_BUTTON][gr_screen.res].button, UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR (
        "Credits", 1058, Buttons[CREDITS_BUTTON][gr_screen.res].xt,
        Buttons[CREDITS_BUTTON][gr_screen.res].yt,
        &Buttons[CREDITS_BUTTON][gr_screen.res].button, UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR (
        "Exit", 1420, Buttons[EXIT_BUTTON][gr_screen.res].xt,
        Buttons[EXIT_BUTTON][gr_screen.res].yt,
        &Buttons[EXIT_BUTTON][gr_screen.res].button, UI_XSTR_COLOR_PINK);
...
void credits_close () {
    ...
    for (i = 0; i < Credits_num_images; i++) {
        if (Credits_bmps[i] >= 0) { bm_release (Credits_bmps[i]); }
    }
    Credits_bmps.clear ();
...
void credits_do_frame (float /*frametime*/) {
    ...
    for (i = 0; i < NUM_BUTTONS; i++) {
        if (Buttons[i][gr_screen.res].button.pressed ()) {
            if (credits_screen_button_pressed (i)) { return; }
        }
    }
    ...
    percent =
        (int)(100.0f - (Credits_artwork_display_time - Credits_counter) * 100.0f / Credits_artwork_fade_time);
    if (percent < 0) { percent = 0; }

    next = Credits_artwork_index + 1;
    if (next >= Credits_num_images) { next = 0; }
    ...
    if ((bm1 != -1) && (bm2 != -1)) {
        ...
        bx1 =
            Credits_image_coords[gr_screen.res][CREDITS_X_COORD] +
            ((Credits_image_coords[gr_screen.res][CREDITS_W_COORD] - bw1) / 2);
        by1 =
            Credits_image_coords[gr_screen.res][CREDITS_Y_COORD] +
            ((Credits_image_coords[gr_screen.res][CREDITS_H_COORD] - bh1) / 2);
        bx2 =
            Credits_image_coords[gr_screen.res][CREDITS_X_COORD] +
            ((Credits_image_coords[gr_screen.res][CREDITS_W_COORD] - bw2) / 2);
        by2 =
            Credits_image_coords[gr_screen.res][CREDITS_Y_COORD] +
            ((Credits_image_coords[gr_screen.res][CREDITS_H_COORD] - bh2) / 2);

        auto alpha = (float)percent / 100.0f;

        gr_set_bitmap (
            bm1, GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL, 1.0f - alpha);
        gr_bitmap (bx1, by1, GR_RESIZE_MENU);

        gr_set_bitmap (
            bm2, GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL, alpha);
    ...
    for (i = TECH_DATABASE_BUTTON; i <= CREDITS_BUTTON; i++) {
        if (Buttons[i][gr_screen.res].button.button_down ()) { break; }
    }
    ...
    gr_set_clip (
        Credits_text_coords[gr_screen.res][CREDITS_X_COORD],
        Credits_text_coords[gr_screen.res][CREDITS_Y_COORD],
        Credits_text_coords[gr_screen.res][CREDITS_W_COORD],
        Credits_text_coords[gr_screen.res][CREDITS_H_COORD], GR_RESIZE_MENU);
    font::set_font (font::FONT1);
    gr_set_color_fast (&Color_normal);

    int y_offset = 0;
    for (std::vector< std::string >::iterator iter =
             Credit_text_parts.begin ();
         iter != Credit_text_parts.end (); ++iter) {
        size_t currentPos = 0;
        size_t lineEnd;
        do {
            int height;
            ...
            gr_get_string_size (
                &width, &height, iter->c_str () + currentPos,
                static_cast< int > (length));
            // Check if the text part is actually visible
            if (Credit_position + y_offset + height > 0.0f) {
                float x = static_cast< float > (
                    (gr_screen.clip_width_unscaled - width) / 2);
                gr_string (
                    x, Credit_position + y_offset, iter->c_str () + currentPos,
                    GR_RESIZE_MENU, static_cast< int > (length));
            }
