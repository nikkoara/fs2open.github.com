...
int pcx_read_header (
    ...
    if ((header.Manufacturer != 10) || (header.Encoding != 1) ||
        (header.Nplanes != 1) || (header.BitsPerPixel != 8) ||
        (header.Version != 5)) {
        if (img_cfp == NULL) cfclose (PCXfile);
...
int pcx_read_bitmap (
    const char* real_filename, ubyte* org_data, ubyte* /*pal*/, int byte_size,
    int aabitmap, bool mask_bitmap, int cf_type) {
    PCXHeader header;
    CFILE* PCXfile;
        ...
        for (col = 0; col < header.BytesPerLine; col++) {
            ...
            if (col < xsize) {
                ...
                if (byte_size == 1) {
                    auto pixel_val = data;
                    if (!mask_bitmap) {
                        // 8 bit-per-pixel aa bitmaps are a bit special since
                        // they only use values in the range [0, 15] where 15
                        // wraps around back to 0. Since the rest of the code
                        // expects the value to be in the range [0, 255] the
                        // pixel value needs to be adjusted here. By
                        // multiplying the value with 17 the original range [0,
                        // 15] is mapped to [0, 255] This only applies to
                        // bitmaps that are not used as masks since mask
                        // bitmaps use higher values to indicate their mask
                        // area
                        if (data > 15) { pixel_val = 0; }
                        else if (data == 15) {
                            pixel_val = 17;
                        }
                        else {
                    ...
                    *pixdata++ = pixel_val;
                    ...
                    if ((byte_size == 2) && aabitmap) {
                        ...
                        *((ushort*)pixdata) = (ushort)data;
                        ...
                        // if the color matches the transparent color, make it
                        // so
                        al = 255;

                        if ((0 == (int)palette[data * 3]) &&
                            (255 == (int)palette[data * 3 + 1]) &&
...
int pcx_encode_byte (ubyte byt, ubyte cnt, FILE* fid) {
    if (cnt) {
        if ((cnt == 1) && (0xc0 != (0xc0 & byt))) {
            ...
            if (EOF == putc ((int)byt, fid)) return 0; // disk write error
            return 2;
        }
    }
    return 0;
...
int pcx_write_bitmap (
    const char* real_filename, int w, int h, ubyte** row_ptrs,
    ubyte* palette) {
    int i;
    ubyte data;
    PCXHeader header;
