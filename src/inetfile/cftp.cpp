...
CFtpGet::CFtpGet (char* URL, char* localfile, char* Username, char* Password) {
    ...
    if (Username) { strcpy_s (m_szUserName, Username); }
    else {
        strcpy_s (m_szUserName, "anonymous");
    }
    if (Password) { strcpy_s (m_szPassword, Password); }
    else {
    ...
    if (INVALID_SOCKET == m_ListenSock) {
        ...
        if (bind (
                m_ListenSock, (SOCKADDR*)&listensockaddr, sizeof (SOCKADDR))) {
...
CFtpGet::~CFtpGet () {
    if (thread_id) SDL_WaitThread (thread_id, NULL);
...
// Returns a value to specify the status (ie.
// connecting/connected/transferring/done)
int CFtpGet::GetStatus () { return m_State; }

uint CFtpGet::GetBytesIn () { return m_iBytesIn; }

uint CFtpGet::GetTotalBytes () { return m_iBytesTotal; }
...
void CFtpGet::WorkerThread () {
    ...
    if (m_State != FTP_STATE_LOGGING_IN) { return; }
    LoginHost ();
    if (m_State != FTP_STATE_LOGGED_IN) { return; }
    GetFile ();
...
uint CFtpGet::GetFile () {
    ...
    if (m_szDir[0]) {
        if (snprintf (
                szCommandString, sizeof (szCommandString), "CWD %s\r\n",
...
uint CFtpGet::IssuePort () {
    ...
    if (getsockname (m_ListenSock, (LPSOCKADDR)&listenaddr, &iLength) ==
        SOCKET_ERROR) {
        m_State = FTP_STATE_SOCKET_ERROR;
        return 0;
    }
    ...
    if (getsockname (m_ControlSock, (LPSOCKADDR)&listenaddr, &iLength) ==
        SOCKET_ERROR) {
        m_State = FTP_STATE_SOCKET_ERROR;
        return 0;
    }
...
int CFtpGet::ConnectControlSocket () {
    ...
    if (se == NULL) { hostaddr.sin_port = htons (21); }
    else {
...
uint CFtpGet::SendFTPCommand (char* command) {
    ...
    if (SOCKET_ERROR == (send (
                            m_ControlSock, command,
                            static_cast< int > (strlen (command)), 0))) {
...
uint CFtpGet::ReadDataChannel () {
    ...
    do {
        if (m_Aborting) return 0;
        nBytesRecv =
            (int)recv (m_DataSock, sDataBuffer, sizeof (sDataBuffer), 0);

        m_iBytesIn += nBytesRecv;
        if (nBytesRecv > 0) { fwrite (sDataBuffer, nBytesRecv, 1, LOCALFILE); }
