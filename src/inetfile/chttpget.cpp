...
ChttpGet::ChttpGet (
    char* URL, char* localfile, char* proxyip, unsigned short proxyport) {
    m_ProxyEnabled = true;
    m_ProxyIP = proxyip;
...
int ChttpGet::ConnectSocket () {
    ...
    if (m_Aborting) return 0;
    if (se == NULL) { hostaddr.sin_port = htons (80); }
    else {
    ...
    if (m_ProxyEnabled) {
        // This is on a proxy, so we need to make sure to connect to the proxy
        // machine
        ...
        if (ip == INADDR_NONE) {
            ...
            do {
                ...
                rcode =
                    http_Asyncgethostbyname (&ip, NW_AGHBN_READ, m_ProxyIP);
    ...
    if (serr) {
        while ((cerr == WSAEALREADY) || (cerr == WSAEINVAL) ||
               (cerr == WSAEWOULDBLOCK)) {
            FD_ZERO (&wfds);
            ...
            serr =
                connect (m_DataSock, (SOCKADDR*)&hostaddr, sizeof (SOCKADDR));
...
uint ChttpGet::ReadDataChannel () {
    ...
    do {
        ...
        nBytesRecv =
            recv (m_DataSock, (char*)&sDataBuffer, sizeof (sDataBuffer), 0);
...
int http_Asyncgethostbyname (unsigned int* ip, int command, char* hostname) {
    if (command == NW_AGHBN_LOOKUP) {
        if (http_lastaslu) http_lastaslu->abort = true;
