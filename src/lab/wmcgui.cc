...
ObjectClassInfoEntry *GUIScreen::GetObjectClassInfo(GUIObject *cgp)
{
        ...
        if (cgp->Parent != NULL && cgp->Parent->InfoEntry != NULL) {
                ...
                for (i = 0; i < len; i++) {
                        if (cgp->Parent->InfoEntry->Subentries[i].Name.empty() && cgp->Parent->InfoEntry->Subentries[i].Object == cgp->Type) {
                                return &cgp->Parent->InfoEntry->Subentries[i];
                        }
                }
                ...
                for (i = 0; i < len; i++) {
                        if (ScreenClassInfo->Entries[i].Name.empty() && ScreenClassInfo->Entries[i].Object == cgp->Type) {
                                return &ScreenClassInfo->Entries[i];
                        }
                }
        }
        ...
        if (OwnerSystem != NULL) {
                ...
                for (i = 0; i < len; i++) {
                        if (OwnerSystem->GetClassInfo()->Entries[i].Name.empty() && OwnerSystem->GetClassInfo()->Entries[i].Object == cgp->Type) {
                                return &OwnerSystem->GetClassInfo()->Entries[i];
                        }
                }
        }
                ...
                if (OwnerSystem->GetFocusObject() != NULL) {
                        ...
                        for (; cgp != END_OF_LIST(&Guiobjects); cgp = cgp_prev) {
                                ...
                                if (OwnerSystem->GetGraspedObject() == NULL) {
                                        if (OwnerSystem->GetMouseX() >= cgp->Coords[0] && OwnerSystem->GetMouseX() <= cgp->Coords[2] && OwnerSystem->GetMouseY() >= cgp->Coords[1] && OwnerSystem->GetMouseY() <= cgp->Coords[3]) {
...
{
        if (OwnerSystem->GetMouseX() >= HideCoords[0] && OwnerSystem->GetMouseX() <= HideCoords[2] && OwnerSystem->GetMouseY() >= HideCoords[1] && OwnerSystem->GetMouseY() <= HideCoords[3]) {
                HideHighlight = true;
        } else {
                HideHighlight = false;
        }

        if (OwnerSystem->GetMouseX() >= CloseCoords[0] && OwnerSystem->GetMouseX() <= CloseCoords[2] && OwnerSystem->GetMouseY() >= CloseCoords[1] && OwnerSystem->GetMouseY() <= CloseCoords[3]) {
                CloseHighlight = true;
        } else {
...
int Window::DoMouseDown(float /*frametime*/)
{
        ...
        if ((OwnerSystem->GetGraspedObject() == NULL) && (Status & GST_MOUSE_LEFT_BUTTON) && OwnerSystem->GetMouseX() >= CaptionCoords[0] && ((OwnerSystem->GetMouseX() < CaptionCoords[2]) || ((Style & WS_NOTITLEBAR) && OwnerSystem->GetMouseX() <= Coords[2])) && OwnerSystem->GetMouseY() >= CaptionCoords[1] && ((OwnerSystem->GetMouseY() <= CaptionCoords[3]) || ((Style & WS_NOTITLEBAR) && OwnerSystem->GetMouseY() <= Coords[2]))) {
...
void Button::DoDraw(float /*frametime*/)
{
        if (IMG_HANDLE_IS_VALID(GetCIEImageHandle(BCI_BUTTON))) {
                ...
                if ((Status & GST_MOUSE_OVER) && IMG_HANDLE_IS_VALID(GetCIEImageHandle(BCI_BUTTON, CIE_HANDLE_M))) {
                        ...
                        (Status & GST_MOUSE_LEFT_BUTTON) && IMG_HANDLE_IS_VALID(GetCIEImageHandle(BCI_BUTTON, CIE_HANDLE_C))) {
                        IMG_SET(GetCIEImageHandle(BCI_BUTTON, CIE_HANDLE_C));
                } else if (
                        (Style & BS_STICKY) && IsDown && IMG_HANDLE_IS_VALID(GetCIEImageHandle(BCI_BUTTON, CIE_HANDLE_S))) {
...
TreeItem *Tree::HitTest(TreeItem *items)
{
        ...
        for (TreeItem *tip = (TreeItem *)GET_FIRST(items);
             tip != END_OF_LIST(items); tip = (TreeItem *)GET_NEXT(tip)) {
                if (OwnerSystem->GetMouseX() >= tip->Coords[0] && OwnerSystem->GetMouseX() <= tip->Coords[2] && OwnerSystem->GetMouseY() >= tip->Coords[1] && OwnerSystem->GetMouseY() <= tip->Coords[3]) {
                        hti = tip;
                }
...
void Checkbox::DoDraw(float /*frametime*/)
{
        ...
        if ((IsChecked && ((FlagPtr == NULL) && (BoolFlagPtr == NULL))) || ((FlagPtr != NULL) && ((*FlagPtr) & Flag)) || ((BoolFlagPtr != NULL) && (*BoolFlagPtr))) {
...
{
        if ((OwnerSystem->GetMouseX() >= CheckCoords[0]) && (OwnerSystem->GetMouseX() <= CheckCoords[2]) && (OwnerSystem->GetMouseY() >= CheckCoords[1]) && (OwnerSystem->GetMouseY() <= CheckCoords[3])) {
                HighlightStatus = 1;
        }
...
void ImageAnim::DoDraw(float frametime)
{
        ...
        gr_set_bitmap(
                ...
                GR_BITBLT_MODE_NORMAL,
                ((Progress - (float)(CurrentFrame / TotalFrames)) / (float)(TotalFrames / CurrentFrame)));
        IMG_DRAW(Coords[0], Coords[1]);
}
...
{
        return int(
                SliderScale * (BarCoords[2] - BarCoords[0] - SliderWidth) + BarCoords[0]);
}

float Slider::GetSliderPos(int x)
{
        return float(x - BarCoords[0]) / float(BarCoords[2] - BarCoords[0] - SliderWidth);
}
...
int Slider::DoMouseDown(float /*frametime*/)
{
        ...
        if (SliderGrabbed) {
                ...
                if (x >= sliderX && x <= sliderX + SliderWidth && y >= BarCoords[1] && y <= BarCoords[3]) {
                        // start grabbing the slider
                        SliderGrabbed = true;
                } else if (
                        x >= BarCoords[0] && x <= BarCoords[2] && y >= BarCoords[1] && y <= BarCoords[3]) {
