...
ClassInfoEntry::~ClassInfoEntry () {
    ...
    if (CIEType == CIE_IMAGE_NMCSD || CIEType == CIE_IMAGE ||
        CIEType == CIE_IMAGE_BORDER) {
        int i = 0;
...
void GUISystem::ParseClassInfo (const char* filename) {
    ...
    try {
        ...
        do {
            ...
            flag = sciep->Parse ();
            if (flag) { list_append (&ScreenClassInfo, sciep); }
            else {
                delete sciep;
            }
        } while (flag);
...
void ClassInfoEntry::Parse (const char* tag, int in_type) {
    ...
    if (optional_string (buf)) {
        ...
        if (in_type == CIE_IMAGE || in_type == CIE_IMAGE_NMCSD) {
            int num_frames;
            stuff_string (buf, F_NAME, sizeof (buf));
            Handles[CIE_HANDLE_N].Image =
                IMG_LOAD_ANIM (buf, &num_frames, NULL);
            if (IMG_HANDLE_IS_VALID (Handles[CIE_HANDLE_N].Image) &&
                num_frames) {
                if (num_frames > 1) {
                    IMG_HANDLE_SET_FRAME (
                        Handles[CIE_HANDLE_N].Image,
                        Handles[CIE_HANDLE_M].Image, 1);
                }
                if (num_frames > 2) {
                    IMG_HANDLE_SET_FRAME (
                        Handles[CIE_HANDLE_N].Image,
                        Handles[CIE_HANDLE_C].Image, 2);
                }
                if (num_frames > 3) {
                    IMG_HANDLE_SET_FRAME (
                        Handles[CIE_HANDLE_N].Image,
                        Handles[CIE_HANDLE_S].Image, 3);
                }
                if (num_frames > 4) {
                    IMG_HANDLE_SET_FRAME (
                        Handles[CIE_HANDLE_N].Image,
                        Handles[CIE_HANDLE_D].Image, 4);
                }
                IMG_HANDLE_SET_FRAME (
                    Handles[CIE_HANDLE_N].Image, Handles[CIE_HANDLE_N].Image,
                    0);
            }
            else {
...
GUIScreen::GUIScreen (const std::string& in_Name) {
    ...
    // this may allow for saving of screens in the future. Incredibly useful
    // for HUDs.
    OwnerSystem = NULL;
...
ObjectClassInfoEntry* GUIScreen::GetObjectClassInfo (GUIObject* cgp) {
    ...
    // Generic templates - ie if you want every single OK button to look the
    // same, or every single window to look the same.
    if (OwnerSystem != NULL) {
...
GUIObject* GUIScreen::Add (GUIObject* new_gauge) {
    ...
    for (GUIObject* tgp = (GUIObject*)GET_FIRST (&Guiobjects);
         tgp != END_OF_LIST (&Guiobjects); tgp = (GUIObject*)GET_NEXT (tgp)) {
    ...
    new_gauge->GetOIECoords (
        &new_gauge->Coords[0], &new_gauge->Coords[1], &new_gauge->Coords[2],
        &new_gauge->Coords[3]);
...
int GUIScreen::OnFrame (float frametime, bool doevents) {
    ...
    if (NOT_EMPTY (&Guiobjects) && doevents) {
        ...
        if (OwnerSystem->GetFocusObject () != NULL) {
            ...
            for (; cgp != END_OF_LIST (&Guiobjects); cgp = cgp_prev) {
                ...
                // If we are moving something, nothing else can get click
                // events
    ...
    // save zbuffer so that we can reset it after drawing (FIXME: this could
    // probably be done better)
    int saved_zbuf = gr_zbuffer_get ();
    gr_zbuffer_set (GR_ZBUFF_NONE);

    // Draw now. This prevents problems from an object deleting itself or
    // moving around in the list
    cgp = (GUIObject*)GET_FIRST (&Guiobjects);

    while (cgp != END_OF_LIST (&Guiobjects)) {
        if (!doevents) { cgp->Status = 0; }
    ...
    if (SomethingPressed) { return GSOF_SOMETHINGPRESSED; }
    else {
...
void GUISystem::PullScreen (GUIScreen* in_screen) {
    GUIScreen* csp_next;
    for (GUIScreen* csp = (GUIScreen*)GET_FIRST (&Screens);
         csp != END_OF_LIST (&Screens); csp = csp_next) {
        csp_next = (GUIScreen*)GET_NEXT (csp);
        if (csp == in_screen) {
...
ScreenClassInfoEntry*
GUISystem::GetScreenClassInfo (const std::string& screen_name) {
    ScreenClassInfoEntry* sciep;
    for (sciep = (ScreenClassInfoEntry*)GET_FIRST (&ScreenClassInfo);
         sciep != END_OF_LIST (&ScreenClassInfo);
         sciep = (ScreenClassInfoEntry*)GET_NEXT (sciep)) {
        if (sciep->GetName () == screen_name) { return sciep; }
    }
...
int GUISystem::OnFrame (float frametime, bool doevents, bool clearandflip) {
    ...
    if (NOT_EMPTY (&Screens)) {
        if (doevents) {
            ...
            if (KeyPressed) {
                ...
                if (KeyPressed & KEY_CTRLED) { Status |= GST_KEYBOARD_CTRL; }
                if (KeyPressed & KEY_ALTED) { Status |= GST_KEYBOARD_ALT; }
                if (KeyPressed & KEY_SHIFTED) { Status |= GST_KEYBOARD_SHIFT; }
            }
            ...
            if (GraspedGuiobject != NULL) {
                if (Status & GraspingButton) {
                    ...
                    GraspedGuiobject->SetPosition (
                        MouseX - GraspedDiff[0], MouseY - GraspedDiff[1]);
            ...
            if (FocusGuiObject != NULL) {
                if (!(Status & GST_MOUSE_PRESS)) { FocusGuiObject = NULL; }
            }
        }

        GUIScreen* csp_prev; // so screens can delete themselves
        for (GUIScreen* csp = (GUIScreen*)GET_LAST (&Screens);
             csp != END_OF_LIST (&Screens); csp = csp_prev) {
    ...
    if (clearandflip) { gr_flip (); }

    if (something_pressed) { return GSOF_SOMETHINGPRESSED; }
    else {
        return GSOF_NOTHINGPRESSED;
    }
}
...
void GUISystem::SetFocusObject (GUIObject* cgp) { FocusGuiObject = cgp; }

void GUISystem::DestroyClassInfo () {
    ScreenClassInfoEntry *sciep, *next_sciep;
    for (sciep = (ScreenClassInfoEntry*)GET_FIRST (&ScreenClassInfo);
         sciep != END_OF_LIST (&ScreenClassInfo); sciep = next_sciep) {
...
//*****************************GUIObject*******************************
GUIObject::GUIObject (
    const std::string& in_Name, int x_coord, int y_coord, int x_width,
    ...
    if (x_width > 0) { Style |= GS_NOAUTORESIZEX; }
    if (y_height > 0) { Style |= GS_NOAUTORESIZEY; }
...
GUIObject::~GUIObject () {
    if (CloseFunction != NULL) { CloseFunction (this); }
...
GUIObject* GUIObject::AddChildInternal (GUIObject* cgp) {
    ...
    // Update coordinates (Should be relative x/y and width/height) to absolute
    // coordinates
    ...
    // Check position
    cgp->GetOIECoords (
        &cgp->Coords[0], &cgp->Coords[1], &cgp->Coords[2], &cgp->Coords[3]);
    // In case we need to resize
    cgp->OnRefreshSize ();
...
GUIObject* GUIObject::AddChild (GUIObject* cgp) {
    ...
    if (cgp->Style & GS_INTERNALCHILD) { return NULL; }
...
void GUIObject::Delete () {
    if (OwnerScreen != NULL) { OwnerScreen->DeleteObject (this); }
    else {
        delete this;
    }
}
...
void GUIObject::OnDraw (float frametime) {
    ...
    if (!(Style & GS_HIDDEN)) {
        GUIObject* cgp_prev;
        for (GUIObject* cgp = (GUIObject*)GET_LAST (&Children);
             cgp != END_OF_LIST (&Children); cgp = cgp_prev) {
            cgp_prev = (GUIObject*)GET_PREV (cgp);
            cgp->OnDraw (frametime);
        }
    }
...
int GUIObject::OnFrame (float frametime, int* unused_queue) {
    ...
    GUIObject* cgp_prev; // Elements will move themselves to the end of the
                         // list if they become active
    ...
    if (Status) {
        ...
        if (Status & GST_MOUSE_OVER) {
            ...
            if (rval == OF_TRUE) { (*unused_queue) &= ~GST_MOUSE_OVER; }
        }
        ...
        if (Status & GST_MOUSE_PRESS) {
            ...
            if (rval == OF_TRUE) {
                (*unused_queue) &=
                    ~(GST_MOUSE_LEFT_BUTTON | GST_MOUSE_RIGHT_BUTTON |
                      GST_MOUSE_MIDDLE_BUTTON);
            }
        }

        // MOUSE UP
        if ((LastStatus & GST_MOUSE_PRESS) &&
            !(OwnerSystem->GetStatus () & GST_MOUSE_PRESS)) {
        ...
        // KEY STATE
        if ((Status & GST_KEYBOARD_CTRL) || (Status & GST_KEYBOARD_ALT) ||
            (Status & GST_KEYBOARD_SHIFT)) {
            rval = DoKeyState (frametime);

            if (rval == OF_TRUE) {
                (*unused_queue) &= ~(
                    GST_KEYBOARD_CTRL | GST_KEYBOARD_ALT | GST_KEYBOARD_SHIFT);
            }
        }
        ...
        if (Status & GST_KEYBOARD_KEYPRESS) {
            ...
            if (rval == OF_TRUE) { (*unused_queue) &= ~GST_KEYBOARD_KEYPRESS; }
        }
    }
...
void GUIObject::OnMove (int dx, int dy) {
    ...
    for (GUIObject* cgp = (GUIObject*)GET_FIRST (&Children);
         cgp != END_OF_LIST (&Children); cgp = (GUIObject*)GET_NEXT (cgp)) {
        cgp->OnMove (dx, dy);
    }
...
int GUIObject::GetOIECoords (int* x1, int* y1, int* x2, int* y2) {
    if (InfoEntry != NULL) {
        ...
        if (Parent != NULL) {
            if (rv & CIE_GC_X_SET) {
                if (*x1 < 0) { *x1 += Parent->ChildCoords[2]; }
                else {
                    *x1 += Parent->ChildCoords[0];
                }
            }
            if (rv & CIE_GC_Y_SET) {
                if (*y1 < 0) { *y1 += Parent->ChildCoords[3]; }
                else {
            ...
            if (rv & CIE_GC_X_SET) {
                if (*x1 < 0) { *x1 += gr_screen.clip_right; }
                else {
                    *x1 += gr_screen.clip_left;
                }
            }
            if (rv & CIE_GC_Y_SET) {
                if (*y1 < 0) { *y1 += gr_screen.clip_bottom; }
                else {
...
// Call this after an object's type is set to enable the GUIObject CIE
// functions
void GUIObject::SetCIPointer () {
    if (OwnerScreen) { InfoEntry = OwnerScreen->GetObjectClassInfo (this); }
}
...
int Window::DoRefreshSize () {
    ...
    if (IMG_HANDLE_IS_VALID (GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_ML))) {
        IMG_INFO (
            GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_ML), &BorderSizes[0],
            NULL);
    }
    else {
    ...
    if (IMG_HANDLE_IS_VALID (GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_MR))) {
        IMG_INFO (
            GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_MR), &BorderSizes[2],
            NULL);
    }
    else {
    ...
    if (IMG_HANDLE_IS_VALID (GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_TL))) {
        IMG_INFO (
            GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_TL), NULL,
            &BorderSizes[1]);
    }
    else if (IMG_HANDLE_IS_VALID (
                 GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_TR))) {
        IMG_INFO (
            GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_TR), NULL,
            &BorderSizes[1]);
    }
    else if (IMG_HANDLE_IS_VALID (
                 GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_TM))) {
        IMG_INFO (
            GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_TM), NULL,
            &BorderSizes[1]);
    }
    else {
        custom_top = false;
    ...
    if (IMG_HANDLE_IS_VALID (GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_BL))) {
        IMG_INFO (
            GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_BL), NULL,
            &BorderSizes[3]);
    }
    else if (IMG_HANDLE_IS_VALID (
                 GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_BR))) {
        IMG_INFO (
            GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_BR), NULL,
            &BorderSizes[3]);
    }
    else if (IMG_HANDLE_IS_VALID (
                 GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_BM))) {
        IMG_INFO (
            GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_BM), NULL,
            &BorderSizes[3]);
    }
    else {
    ...
    if (IMG_HANDLE_IS_VALID (GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_TL))) {
        IMG_INFO (
            GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_BL), &CornerWidths[0],
            NULL);
    }
    else {
    ...
    if (IMG_HANDLE_IS_VALID (GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_TR))) {
        IMG_INFO (
            GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_TR), &CornerWidths[1],
            NULL);
    }
    else {
    ...
    if (IMG_HANDLE_IS_VALID (GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_BL))) {
        IMG_INFO (
            GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_BL), &CornerWidths[2],
            NULL);
    }
    else {
    ...
    if (IMG_HANDLE_IS_VALID (GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_BR))) {
        IMG_INFO (
            GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_BL), &CornerWidths[3],
            NULL);
    }
    else {
    ...
    if (custom_top || (Style & WS_NOTITLEBAR)) {
        ...
        ChildCoords[1] =
            Coords[1] + gr_get_font_height () + 5 + 3 * W_BORDERHEIGHT;
    }
    ChildCoords[2] = Coords[2] - BorderSizes[2];
    ChildCoords[3] = Coords[3] - BorderSizes[3];

    if (!(Style & GS_NOAUTORESIZEX)) { ChildCoords[2] = Coords[2] = 0; }
    if (!(Style & GS_NOAUTORESIZEY)) { ChildCoords[3] = Coords[3] = 0; }

    for (GUIObject* cgp = (GUIObject*)GET_FIRST (&Children);
         cgp != END_OF_LIST (&Children); cgp = (GUIObject*)GET_NEXT (cgp)) {
        // SetPosition children around so they're within the upper-left corner
        // of dialog
    ...
    if (!(Style & WS_NOTITLEBAR)) {
        ...
        if ((Coords[2] - Coords[0]) < (caption_min_size)) {
            Coords[2] =
                Coords[0] + caption_min_size + BorderSizes[0] + BorderSizes[2];
        }
        ...
        if (!Caption.empty ()) {
            if (IMG_HANDLE_IS_VALID (GetCIEImageHandle (WCI_CAPTION))) {
                ...
                GetCIECoords (
                    WCI_CAPTION, &CaptionCoords[0], &CaptionCoords[1]);

                // do the image
                IMG_INFO (GetCIEImageHandle (WCI_CAPTION), &cw, &ch);
                num = (float)w / cw;
                CaptionRectList = bitmap_rect_list (
                    Coords[0] + CornerWidths[2], Coords[1], w, ch, 0, 0, 1.0f,
                    1.0f);
                h = ch;
            }
            else {
                ...
                CaptionCoords[0] =
                    Coords[0] +
                    (((Coords[2] - Coords[0]) - w - (close_w + hide_w)) / 2);
                CaptionCoords[1] = Coords[1] + BorderSizes[1];
            }
...
int Window::DoMouseDown (float /*frametime*/) {
    ...
    if (Style & WS_NONMOVEABLE) { return OF_TRUE; }

    if ((OwnerSystem->GetGraspedObject () == NULL) &&
        (Status & GST_MOUSE_LEFT_BUTTON) &&
        OwnerSystem->GetMouseX () >= CaptionCoords[0] &&
        ((OwnerSystem->GetMouseX () < CaptionCoords[2]) ||
         ((Style & WS_NOTITLEBAR) &&
          OwnerSystem->GetMouseX () <= Coords[2])) &&
        OwnerSystem->GetMouseY () >= CaptionCoords[1] &&
        ((OwnerSystem->GetMouseY () <= CaptionCoords[3]) ||
         ((Style & WS_NOTITLEBAR) &&
...
int Window::DoMouseUp (float /*frametime*/) {
    ...
    if (HideHighlight) {
        if (Style & GS_HIDDEN) {
            ...
            Coords[3] = Coords[1] + (CaptionCoords[3] - CaptionCoords[1]) +
                        BorderSizes[1] + BorderSizes[3];
            Style |= GS_HIDDEN;
        }

        if (IMG_HANDLE_IS_VALID (
                GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_BM))) {
            int h;
...
void Window::DoMove (int dx, int dy) {
    ...
    if (Style & GS_HIDDEN) { UnhiddenHeight += dy; }
...
void draw_open_rect (
    int x1, int y1, int x2, int y2, int resize_mode = GR_RESIZE_NONE) {
...
void Window::DoDraw (float /*frametime*/) {
    ...
    // if the window has no title bar, and no content, then don't draw any of
    // it
    if ((Style & WS_NOTITLEBAR) && !HasChildren ()) { return; }

    // shade the background of the window so that it's just slightly
    // transparent
    color clr;
    gr_init_alphacolor (
        &clr, WindowShade.r, WindowShade.g, WindowShade.b, WindowShade.c);
    ...
    if (IMG_HANDLE_IS_VALID (GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_TM))) {
        ...
        gr_line (
            Coords[0] + BorderSizes[0], Coords[1], Coords[2] - BorderSizes[2],
            Coords[1], GR_RESIZE_NONE);
    }
    ...
    if (IMG_HANDLE_IS_VALID (GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_BM))) {
        ...
        gr_line (
            Coords[0] + BorderSizes[0], Coords[3], Coords[2] - BorderSizes[2],
            Coords[3], GR_RESIZE_NONE);
    }

    if (!(Style & GS_HIDDEN)) {
        if (IMG_HANDLE_IS_VALID (
                GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_ML))) {
            IMG_SET (GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_ML));
            gr_bitmap_list (
                &BorderRectLists[CIE_HANDLE_ML], 1, GR_RESIZE_NONE);
        }
        else {
            gr_line (
                Coords[0], Coords[1] + BorderSizes[1], Coords[0],
                Coords[3] - BorderSizes[3], GR_RESIZE_NONE);
        }

        if (IMG_HANDLE_IS_VALID (
                GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_MR))) {
            IMG_SET (GetCIEImageHandle (WCI_BORDER, CIE_HANDLE_MR));
            gr_bitmap_list (
                &BorderRectLists[CIE_HANDLE_MR], 1, GR_RESIZE_NONE);
        }
        else {
            gr_line (
                Coords[2], Coords[1] + BorderSizes[1], Coords[2],
                Coords[3] - BorderSizes[3], GR_RESIZE_NONE);
        }
    }
    ...
    if (!(Style & WS_NOTITLEBAR)) {
        ...
        // Close button
        if (IMG_HANDLE_IS_VALID (GetCIEImageHandle (WCI_CLOSE))) {
            if (CloseHighlight && IMG_HANDLE_IS_VALID (GetCIEImageHandle (
                                      WCI_CLOSE, CIE_HANDLE_M))) {
            ...
            if (CloseHighlight) { gr_set_color_fast (&Color_text_active); }
            else {
                gr_set_color_fast (&Color_text_normal);
            }
        ...
        // Hide button
        if (IMG_HANDLE_IS_VALID (GetCIEImageHandle (WCI_HIDE))) {
            if (HideHighlight && IMG_HANDLE_IS_VALID (GetCIEImageHandle (
                                     WCI_HIDE, CIE_HANDLE_M))) {
            ...
            if (HideHighlight) { gr_set_color_fast (&Color_text_active); }
            else {
                gr_set_color_fast (&Color_text_normal);
            }
        ...
        gr_string (
            CaptionCoords[0], CaptionCoords[1], Caption.c_str (),
            GR_RESIZE_NONE);
    }
}
...
Window::Window (
    const std::string& in_caption, int x_coord, int y_coord, int x_width,
    int y_width, int in_style)
...
Button::Button (
    const std::string& in_caption, int x_coord, int y_coord,
    void (*in_function) (Button* caller), int x_width, int y_height,
    int in_style)
...
int Button::DoRefreshSize () {
    ...
    if (IMG_HANDLE_IS_VALID (GetCIEImageHandle (BCI_BUTTON))) {
        ...
        if (!(Style & GS_NOAUTORESIZEX)) { Coords[2] = Coords[0] + w; }
        if (!(Style & GS_NOAUTORESIZEY)) { Coords[3] = Coords[1] + h; }
    }
    else {
        gr_get_string_size (&w, &h, Caption.c_str ());
        if (!(Style & GS_NOAUTORESIZEX)) { Coords[2] = Coords[0] + w; }
...
void Button::DoDraw (float /*frametime*/) {
    if (IMG_HANDLE_IS_VALID (GetCIEImageHandle (BCI_BUTTON))) {
        ...
        if ((Status & GST_MOUSE_OVER) &&
            IMG_HANDLE_IS_VALID (
                GetCIEImageHandle (BCI_BUTTON, CIE_HANDLE_M))) {
            IMG_SET (GetCIEImageHandle (BCI_BUTTON, CIE_HANDLE_M));
        }
        else if (
            (Status & GST_MOUSE_LEFT_BUTTON) &&
            IMG_HANDLE_IS_VALID (
                GetCIEImageHandle (BCI_BUTTON, CIE_HANDLE_C))) {
            IMG_SET (GetCIEImageHandle (BCI_BUTTON, CIE_HANDLE_C));
        }
        else if (
            (Style & BS_STICKY) && IsDown &&
            IMG_HANDLE_IS_VALID (
                GetCIEImageHandle (BCI_BUTTON, CIE_HANDLE_S))) {
            IMG_SET (GetCIEImageHandle (BCI_BUTTON, CIE_HANDLE_S));
        }
        IMG_DRAW (Coords[0], Coords[1]);
    }
    else {
        if (Status == 0) { gr_set_color_fast (&Color_text_normal); }
        ...
        draw_open_rect (
            Coords[0], Coords[1], Coords[2], Coords[3], GR_RESIZE_NONE);
...
int Button::DoMouseDown (float /*frametime*/) {
    ...
    if (!(Style & BS_STICKY)) { IsDown = true; }
    return OF_TRUE;
}
...
int Button::DoMouseUp (float /*frametime*/) {
    ...
    if (!(Style & BS_STICKY)) { IsDown = false; }
    else {
        IsDown = !IsDown;
    }
...
TreeItem::~TreeItem () { ClearAllItems (); }
...
void Tree::CalcItemsSize (TreeItem* items, int DrawData[4]) {
    ...
    for (TreeItem* tip = (TreeItem*)GET_FIRST (items);
         tip != END_OF_LIST (items); tip = (TreeItem*)GET_NEXT (tip)) {
        if (!DrawData[3]) {
            ...
            // This should really be moved to where we handle the scrolling
            // code I'm not doing that right now. :) Or at least this should be
            // called from that
            if (((DrawData[2] + h) > Coords[3] &&
                 (Style & GS_NOAUTORESIZEY)) ||
        ...
        if (DrawData[3] || !tip->ShowThis) {
            ...
            tip->Coords[0] = tip->Coords[1] = tip->Coords[2] = tip->Coords[3] =
                -1;
        }
    }
}
...
Tree::Tree (
    const std::string& in_name, int x_coord, int y_coord,
    void* in_associateditem, int x_width, int y_height, int in_style)
...
void Tree::DrawItems (TreeItem* items) {
    for (TreeItem* tip = (TreeItem*)GET_FIRST (items);
        ...
        if (tip->ShowThis) {
            ...
            gr_string (
                tip->Coords[0], tip->Coords[1], tip->Name.c_str (),
                GR_RESIZE_NONE);
...
int Tree::DoMouseUp (float /*frametime*/) {
    ...
    if (HighlightedItem != NULL) {
        ...
        if (SelectedItem->Function != NULL) { SelectedItem->Function (this); }
    }
...
void Tree::DoMove (int dx, int dy) { MoveTreeItems (dx, dy, &Items); }

TreeItem* Tree::AddItem (
    TreeItem* parent, const std::string& in_name, int in_data,
    bool in_delete_data, void (*in_function) (Tree* caller)) {
    ...
    if (parent != NULL) { list_append (&parent->Children, ni); }
    else {
...
int Text::DoRefreshSize () {
    int width;
    if (Style & GS_NOAUTORESIZEX) { width = Coords[2] - Coords[0]; }
    ...
    NumLines = split_str (
        Content.c_str (), width, LineLengths, LineStartPoints, MAX_TEXT_LINES);
    ...
    for (int i = 0; i < NumLines; i++) {
        gr_get_string_size (
            &width_test, NULL, LineStartPoints[i], LineLengths[i]);
        if (width_test > longest_width) { longest_width = width_test; }
    }
    if (longest_width < width) { width = longest_width; }

    // Resize along the Y axis
    if (Style & GS_NOAUTORESIZEY) {
        int new_height = (Coords[3] - Coords[1]) / gr_get_font_height ();

        if (new_height < NumLines) { NumLines = new_height; }
    }
    else {
        Coords[3] = Coords[1] + gr_get_font_height () * NumLines;
        if (Parent != NULL) {
            if (Coords[3] >= Parent->ChildCoords[3] &&
                (Parent->Style & GS_NOAUTORESIZEY)) {
    ...
    if (!(Style & GS_NOAUTORESIZEX)) { Coords[2] = Coords[0] + width; }
...
void Text::DoDraw (float /*frametime*/) {
    ...
    for (int i = 0; i < NumLines; i++) {
        gr_string (
            ChildCoords[0], ChildCoords[1] + (i * font_height),
            Content
                .substr (LineStartPoints[i] - Content.c_str (), LineLengths[i])
                .c_str (),
            GR_RESIZE_NONE);
    }
}
...
int Text::DoKeyPress (float /*frametime*/) {
    if (!(Style & T_EDITTABLE)) { return OF_FALSE; }
    ...
    case KEY_ENTER:
        if (SaveType & T_ST_ONENTER) {
            if (Save ()) { OwnerSystem->SetActiveObject (Parent); }
            else {
                Load ();
            }
        }
    ...
    default: {
        ...
        if (iscntrl (symbol)) {
            ...
            if (!isdigit (symbol) && symbol != '-') { return OF_FALSE; }
        }
            ...
            if (!isdigit (symbol)) { return OF_FALSE; }
        }
        ...
        Content += char(symbol);
        if (SaveType & T_ST_REALTIME) { Save (); }
    }
    }
    OnRefreshSize ();
    return OF_TRUE;
...
Text::Text (
    const std::string& in_name, const std::string& in_content, int x_coord,
    int y_coord, int x_width, int y_height, int in_style)
    : GUIObject (in_name, x_coord, y_coord, x_width, y_height, in_style) {
    Content = in_content;
    NumLines = 0;
    SaveType = T_ST_NONE;
...
bool Text::Save () {
    if (SaveType == T_ST_NONE) { return false; }
    ...
    if (SaveType & T_ST_INT) {
        ...
        if (SaveType & T_ST_NEW && len < uSaveMax) {
            if (*chpSavePointer != NULL) { delete[] * chpSavePointer; }
...
void Text::Load () {
    if (SaveType == T_ST_NONE) { return; }

    if (SaveType & T_ST_INT) { SetText (*iSavePointer); }
    else if (SaveType & T_ST_FLOAT) {
...
Checkbox::Checkbox (
    const std::string& in_label, int x_coord, int y_coord,
    void (*in_function) (Checkbox* caller), int x_width, int y_height,
    int in_style)
...
int Checkbox::DoRefreshSize () {
    ...
    if (!(Style & GS_NOAUTORESIZEX)) { Coords[2] = Coords[0] + tw; }

    if (!(Style & GS_NOAUTORESIZEY)) { Coords[3] = Coords[1] + th; }
...
void Checkbox::DoDraw (float /*frametime*/) {
    if (HighlightStatus == 1) { gr_set_color_fast (&Color_text_active); }
    ...
    draw_open_rect (
        CheckCoords[0], CheckCoords[1], CheckCoords[2], CheckCoords[3],
        GR_RESIZE_NONE);
    ...
    gr_set_color_fast (&Color_text_normal);
    gr_string (
        CheckCoords[2] + CB_TEXTCHECKDIST, CheckCoords[1], Label.c_str (),
        GR_RESIZE_NONE);
}
    ...
    if ((OwnerSystem->GetMouseX () >= CheckCoords[0]) &&
        ...
        if (FlagPtr != NULL) {
            ...
            *BoolFlagPtr = !(*BoolFlagPtr);
            ...
            IsChecked =
                Wip->wi_flags[static_cast< Weapon::Info_Flags > (Flag)];
        }
        else {
            IsChecked = !IsChecked;
        }
...
//*****************************ImageAnim*******************************
ImageAnim::ImageAnim (
    const std::string& in_name, const std::string& /*in_imagename*/,
    int x_coord, int y_coord, int x_width, int y_width, int in_style)
...
void ImageAnim::DoDraw (float frametime) {
    ...
    if (PlayType != PT_STOPPED) {
        if (PlayType == PT_PLAYING) { ElapsedTime += frametime; }
    ...
    gr_set_bitmap (
        ImageHandle + CurrentFrame, GR_ALPHABLEND_FILTER,
        GR_BITBLT_MODE_NORMAL,
        1 - ((Progress - (float)(CurrentFrame / TotalFrames)) /
             (float)(TotalFrames / CurrentFrame)));
    IMG_DRAW (Coords[0], Coords[1]);
    gr_set_bitmap (
        ImageHandle + CurrentFrame + 1, GR_ALPHABLEND_FILTER,
        GR_BITBLT_MODE_NORMAL,
...
void ImageAnim::SetImage (const std::string& in_imagename) {
    if (in_imagename.size ()) {
        ...
        if (IMG_HANDLE_IS_INVALID (ImageHandle)) {
            ...
            if (IMG_HANDLE_IS_VALID (ImageHandle)) { TotalFrames = 1; }
            else {
                TotalFrames = 0;
            }
            FPS = 0;
...
void ImageAnim::Play (bool /*in_isreversed*/) {
    if (ImageFlags & IF_REVERSED) { PlayType = PT_PLAYING; }
}

void ImageAnim::Pause () {
    if (PlayType == PT_PLAYING) { PlayType = PT_STOPPED; }
    else {
...
void ImageAnim::Stop () {
    if (ImageFlags & IF_REVERSED) { PlayType = PT_STOPPED_REVERSE; }
    else {
        PlayType = PT_STOPPED;
    }
...
Slider::Slider (
    const std::string& in_label, float min, float max, int x_coord,
    int y_coord, void (*in_function) (Slider* caller), int x_width,
    int y_height, int in_style)
    : GUIObject (in_label, x_coord, y_coord, x_width, y_height, in_style) {
    Label = in_label;
...
float Slider::GetSliderValue () { return (SliderScale * (Max - Min) + Min); }
...
void Slider::DoDraw (float /*frametime*/) {
    ...
    draw_open_rect (
        BarCoords[0], BarCoords[1], BarCoords[2], BarCoords[3], false);

    auto sliderX = GetSliderOffset ();

    color clr;
    gr_init_alphacolor (
        &clr, SliderShade.r, SliderShade.g, SliderShade.b, SliderShade.c);
    gr_set_color_fast (&clr);
    gr_rect (
        sliderX, BarCoords[1], SliderWidth, BarCoords[3] - BarCoords[1],
        false);
}
...
int Slider::DoMouseDown (float /*frametime*/) {
    ...
    if (SliderGrabbed) { UpdateSlider (x); }
    else {
        // first check if the mouse is within the slider bounds
        if (x >= sliderX && x <= sliderX + SliderWidth && y >= BarCoords[1] &&
            y <= BarCoords[3]) {
            // start grabbing the slider
            SliderGrabbed = true;
        }
        else if (
            x >= BarCoords[0] && x <= BarCoords[2] && y >= BarCoords[1] &&
            y <= BarCoords[3]) {
