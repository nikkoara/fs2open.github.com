...
#define LAB_FLAG_SUBMODEL_ROTATE \
    (1 << 3) // do rotation for any rotating ship subobjects
#define LAB_FLAG_LIGHTNING_ARCS (1 << 4) // show damage lightning
#define LAB_FLAG_UNUSED \
    (1 << 5) // feel free to change that to something you need - Valathil
#define LAB_FLAG_SHOW_WEAPONS \
    (1 << 6) // determines if external weapons models are displayed
#define LAB_FLAG_INITIAL_ROTATION (1 << 7) // initial rotation setting
#define LAB_FLAG_DESTROYED_SUBSYSTEMS \
    (1 << 8) // render model as if all subsystems are destroyed
...
void labviewer_change_model (
    char* model_fname, int lod = 0, int sel_index = -1) {
    ...
    if (change_model) {
        ...
        // only load a new model if we are supposed to (so that we can use this
        // function to reset states)
        if (model_fname != NULL) {
            model_subsystem* subsystems = NULL;

            Lab_model_num = model_load (
                model_fname,
                (valid_ship) ? Ship_info[sel_index].n_subsystems : 0,
                subsystems, 0);
            ...
            if (Lab_mode == LAB_MODE_SHIP) {
                ...
                for (j = 0; j < sip->num_primary_banks; j++) {
                    weapon_info* wip =
                        &Weapon_info[sip->primary_bank_weapons[j]];
                    if (!sip->draw_primary_models[j]) continue;
                    Lab_weaponmodel_num[l] = -1;
                    if (strlen (wip->external_model_name)) {
                        Lab_weaponmodel_num[l] =
                ...
                for (j = 0; j < sip->num_secondary_banks; j++) {
                    weapon_info* wip =
                        &Weapon_info[sip->secondary_bank_weapons[j]];
                    if (!sip->draw_secondary_models[j]) continue;
                    Lab_weaponmodel_num[l] = -1;
                    if (strlen (wip->external_model_name)) {
                        Lab_weaponmodel_num[l] =
                            model_load (wip->external_model_name, 0, NULL);
    ...
    if (model_fname == NULL) { Lab_last_selected_ship = -1; }
...
void labviewer_render_model (float frametime) {
    ...
    if (Lab_selected_object != -1 &&
        (Objects[Lab_selected_object].type == OBJ_SHIP) &&
        (Lab_selected_index >= 0)) {
        sip = &Ship_info[Lab_selected_index];
    }
    ...
    if (Trackball_active) {
        ...
        if (dx || dy) {
            ...
            // zoom in/out
                ...
                if (lab_cam_distance < 1.0f) lab_cam_distance = 1.0f;
            }
            // rotate background
    ...
    if (Lab_selected_object != -1) {
        ...
        if (obj->type == OBJ_SHIP) {
            Ships[obj->instance].flags.set (
                Ship::Ship_Flags::Rotators_locked, !Lab_rotate_subobjects);
            Ships[obj->instance].flags.set (
                Ship::Ship_Flags::Draw_as_wireframe, Lab_render_wireframe);
            Ships[obj->instance].flags.set (
                Ship::Ship_Flags::Render_full_detail, Lab_render_show_detail);
            Ships[obj->instance].flags.set (
                Ship::Ship_Flags::Render_without_light,
                Lab_render_without_light);
            Ships[obj->instance].flags.set (
                Ship::Ship_Flags::Render_without_diffuse,
                Lab_Basemap_override);
            Ships[obj->instance].flags.set (
                Ship::Ship_Flags::Render_without_glowmap,
                Lab_Glowmap_override);
            Ships[obj->instance].flags.set (
                Ship::Ship_Flags::Render_without_normalmap,
                Lab_Normalmap_override);
            Ships[obj->instance].flags.set (
                Ship::Ship_Flags::Render_without_specmap,
                Lab_Specmap_override);
            Ships[obj->instance].flags.set (
                Ship::Ship_Flags::Render_without_heightmap,
                Lab_Heightmap_override);
            Ships[obj->instance].flags.set (
                Ship::Ship_Flags::Render_without_miscmap,
                Lab_Miscmap_override);
        ...
        if (Cmdline_frame_profile) {
            ...
            gr_string (
                ...
                gr_screen.center_offset_y + 100 + gr_get_font_height () + 1,
...
void labviewer_do_render (float frametime) {
    ...
    if (Lab_selected_object >= 0) {
        ...
        if (strlen (Lab_model_filename)) {
            ...
            gr_string (
                gr_screen.center_offset_x + gr_screen.center_w - w,
                gr_screen.center_offset_y + gr_screen.center_h - h,
                Lab_model_filename, GR_RESIZE_NONE);
        }
    }
    ...
    if (frametotal != 0.0f) {
        gr_printf_no_resize (
            gr_screen.center_offset_x + 2,
            gr_screen.center_offset_y + gr_screen.center_h -
                gr_get_font_height (),
            "FPS: %3i Camera Distance: %4f", (int)std::lround (Framerate),
            lab_cam_distance);
    }
    else {
        gr_string (
    ...
    if (Lab_team_color != "<none>")
        gr_printf_no_resize (
            gr_screen.center_offset_x + 2,
            gr_screen.center_offset_y + gr_screen.center_h -
                (gr_get_font_height () * 3) - 3,
            "Use T and Y to cycle through available Team Color settings. "
            "Current: %s",
    ...
    gr_printf_no_resize (
        gr_screen.center_offset_x + 2,
        gr_screen.center_offset_y + gr_screen.center_h -
            (gr_get_font_height () * 4) - 3,
        "Hold LMB to rotate the ship or weapon. Hold RMB to rotate the "
        "Camera. Hold Shift + LMB to zoom in or out. Use number keys to "
        "switch between FXAA presets.");
}
...
void labviewer_close_class_window (GUIObject* /*caller*/) {
    if (Lab_class_toolbar) { Lab_class_toolbar->DeleteChildren (); }
...
void labviewer_set_class_window (int mode) {
    if (Lab_class_window == NULL) {
        Lab_class_window = (Window*)Lab_screen->Add (new Window (
            "Class Window", gr_screen.center_offset_x + 50,
            gr_screen.center_offset_y + 50));
        Lab_class_window->SetCloseFunction (labviewer_close_class_window);
    }

    if (Lab_class_toolbar == NULL) {
        Lab_class_toolbar = (Window*)Lab_screen->Add (new Window (
            "Class Toolbar", gr_screen.center_offset_x + 0,
            gr_screen.center_offset_y + Lab_toolbar->GetHeight (), -1, -1,
...
void labviewer_flags_clear () {
    if (Lab_flags_window != NULL) { Lab_flags_window->DeleteChildren (); }
...
template< class T >
void labviewer_flags_add (
    int* X, int* Y, const char* flag_name, T flag,
    ...
    new_flag.cb =
        (Checkbox*)Lab_flags_window->AddChild (new Checkbox (flag_name, x, y));
    new_flag.flag = flag;
    flag_list.push_back (new_flag);

    if (X) { *X += new_flag.cb->GetWidth () + 2; }

    if (Y) { *Y += new_flag.cb->GetHeight () + 1; }
}

void labviewer_populate_flags_window () {
    if (Lab_mode == LAB_MODE_NONE) { return; }

    if (Lab_flags_window == NULL) { return; }
    ...
    if (Lab_mode == LAB_MODE_SHIP) {
        for (size_t i = 0; i < Num_ship_flags; ++i) {
            labviewer_flags_add< Ship::Info_Flags > (
                nullptr, &y, Ship_flags[i].name, Ship_flags[i].def,
                Ship_Class_Flags);
        }
    }
    // weapon flags ...
        ...
        for (size_t i = 0; i < num_weapon_info_flags; ++i) {
            labviewer_flags_add< Weapon::Info_Flags > (
                nullptr, &y, Weapon_Info_Flags[i].name,
                Weapon_Info_Flags[i].def, Weapon_Class_Flags);
        }
    }
}

void labviewer_update_flags_window () {
    if ((Lab_selected_index < 0) || (Lab_mode == LAB_MODE_NONE)) { return; }
...
void labviewer_make_flags_window (Button* /*caller*/) {
    if (Lab_flags_window == NULL) {
        Lab_flags_window = (Window*)Lab_screen->Add (new Window (
            "Flags Window",
            gr_screen.center_offset_x + gr_screen.center_w - 205,
    ...
    // populate the window with our flags (controls both ships and weapons
    // flags)
...
#define VAR_ADD_HEADER(name)                                                 \
    {                                                                        \
        ntp = (Text*)Lab_variables_window->AddChild (new Text (              \
            (name), (name), VAR_POS_RIGHTX / 2, y + 8, VAR_POS_RIGHTWIDTH)); \
        y += ntp->GetHeight () + 10;                                         \
    }
...
void labviewer_variables_add (int* Y, const char* var_name) {
    ...
    // variable
    Lab_variables_window->AddChild (
        new Text ((var_name), (var_name), 0, y, VAR_POS_LEFTWIDTH));
    // edit box
    new_text = (Text*)Lab_variables_window->AddChild (new Text (
        std::string ((var_name)) + std::string ("Editbox"), "", VAR_POS_RIGHTX,
        y, VAR_POS_RIGHTWIDTH, 12, T_EDITTABLE));

    if (Y) { *Y += new_text->GetHeight () + 2; }
...
void labviewer_populate_variables_window () {
    ...
    if (Lab_variables_window == NULL) { return; }
    ...
    // labviewer_update_variables_window() as well! ship vFEWfe<ariables ...
    if (Lab_mode == LAB_MODE_SHIP) {
...
#define VAR_SET_VALUE_SAVE(value, max_size)                        \
    {                                                              \
        ...
        if ((max_size) < 1) {                                      \
            ...
            Lab_variables[i]->SetSaveLoc (                         \
                &(value), T_ST_ONENTER, (max_size), 0);            \
        }                                                          \
        i++;                                                       \
    }
...
void labviewer_update_variables_window () {
    ...
    if (Lab_selected_index < 0) { return; }
    ...
    if (Lab_mode == LAB_MODE_SHIP) {
        ...
        VAR_SET_VALUE_SAVE (
            sip->shield_icon_index, (int)(Hud_shield_filenames.size () - 1));
...
void labviewer_make_variables_window (Button* /*caller*/) {
    ...
    Lab_variables_window = (Window*)Lab_screen->Add (new Window (
        "Class Variables",
        gr_screen.center_offset_x + gr_screen.center_w -
            (VAR_POS_RIGHTX + VAR_POS_RIGHTWIDTH + 25),
        gr_screen.center_offset_y + 200));
...
#define ADD_RENDER_FLAG(text, flag, var)                       \
    {                                                          \
        cbp = (Checkbox*)Lab_render_options_window->AddChild ( \
            new Checkbox ((text), 2, y));                      \
...
void labviewer_make_render_options_window (Button* /*caller*/) {
    ...
    if (Lab_render_options_window != NULL) { return; }
    ...
    // viewer flags
    ADD_RENDER_FLAG (
        "Disable Model Rotation", Lab_viewer_flags, LAB_FLAG_NO_ROTATION);
    ADD_RENDER_FLAG (
        "Show Insignia", Lab_viewer_flags, LAB_FLAG_SHOW_INSIGNIA);
    ADD_RENDER_FLAG (
        "Show Damage Lightning", Lab_viewer_flags, LAB_FLAG_LIGHTNING_ARCS);
    ...
    ADD_RENDER_FLAG (
        "Show Ship Weapons", Lab_viewer_flags, LAB_FLAG_SHOW_WEAPONS);
    ADD_RENDER_FLAG (
        "Initial Rotation", Lab_viewer_flags, LAB_FLAG_INITIAL_ROTATION);
    ADD_RENDER_FLAG (
        "Show Destroyed Subsystems", Lab_viewer_flags,
        LAB_FLAG_DESTROYED_SUBSYSTEMS);

    ADD_RENDER_BOOL ("Emissive Lighting", Lab_emissive_light_override);
    Slider* sldr = (Slider*)Lab_render_options_window->AddChild (new Slider (
        "Ambient Factor", 0, 128, 0, y + 2,
    ...
    sldr = (Slider*)Lab_render_options_window->AddChild (new Slider (
        "Direct. Lights", 0.0f, 2.0f, 0, y + 2,
        labviewer_render_options_set_static_light_factor,
        Lab_render_options_window->GetWidth ()));
    y += sldr->GetHeight () + 1;
    sldr = (Slider*)Lab_render_options_window->AddChild (new Slider (
        "Bloom", 0, 200, 0, y + 2, labviewer_render_options_set_bloom,
        Lab_render_options_window->GetWidth ()));
    y += sldr->GetHeight () + 1;

    // start tree
    cmp = (Tree*)Lab_render_options_window->AddChild (new Tree (
        "Detail Options Tree", 0, y + 2, NULL,
        Lab_render_options_window->GetWidth ()));
    ...
    // set close function
    Lab_render_options_window->SetCloseFunction (
        labviewer_close_render_options_window);
}
...
void labviewer_make_material_override_window (Button* /*caller*/) {
    ...
    if (Lab_material_override_window != NULL) { return; }
    ...
    cbp = (Checkbox*)Lab_material_override_window->AddChild (
        new Checkbox ("Override Diffuse Color", 2, y));
    cbp->SetBool (&Basemap_color_override_set);
    y += cbp->GetHeight () + 1;

    sldr = (Slider*)Lab_material_override_window->AddChild (new Slider (
        "Red", 0.0f, 255.0f, 0, y + 2,
        labviewer_set_material_override_diffuse_red, 275));
    y += sldr->GetHeight () + 1;

    sldr = (Slider*)Lab_material_override_window->AddChild (new Slider (
        "Green", 0.0f, 255.0f, 0, y + 2,
        labviewer_set_material_override_diffuse_green, 275));
    y += sldr->GetHeight () + 1;

    sldr = (Slider*)Lab_material_override_window->AddChild (new Slider (
        "Blue", 0.0f, 255.0f, 0, y + 2,
        labviewer_set_material_override_diffuse_blue, 275));
    y += sldr->GetHeight () + 1;

    if (Cmdline_glow) {
        cbp = (Checkbox*)Lab_material_override_window->AddChild (
            new Checkbox ("Override Glow Color", 2, y));
        cbp->SetBool (&Glowmap_color_override_set);
        y += cbp->GetHeight () + 1;

        sldr = (Slider*)Lab_material_override_window->AddChild (new Slider (
            "Red", 0.0f, 255.0f, 0, y + 2,
            labviewer_set_material_override_glow_red, 275));
        y += sldr->GetHeight () + 1;

        sldr = (Slider*)Lab_material_override_window->AddChild (new Slider (
            "Green", 0.0f, 255.0f, 0, y + 2,
            labviewer_set_material_override_glow_green, 275));
        y += sldr->GetHeight () + 1;

        sldr = (Slider*)Lab_material_override_window->AddChild (new Slider (
            "Blue", 0.0f, 255.0f, 0, y + 2,
            labviewer_set_material_override_glow_blue, 275));
        y += sldr->GetHeight () + 1;
    }
    if (Cmdline_spec) {
        cbp = (Checkbox*)Lab_material_override_window->AddChild (
            new Checkbox ("Override Spec Color", 2, y));
        cbp->SetBool (&Specmap_color_override_set);
        y += cbp->GetHeight () + 1;

        sldr = (Slider*)Lab_material_override_window->AddChild (new Slider (
            "Red", 0.0f, 255.0f, 0, y + 2,
            labviewer_set_material_override_specular_red, 275));
        y += sldr->GetHeight () + 1;

        sldr = (Slider*)Lab_material_override_window->AddChild (new Slider (
            "Green", 0.0f, 255.0f, 0, y + 2,
            labviewer_set_material_override_specular_green, 275));
        y += sldr->GetHeight () + 1;

        sldr = (Slider*)Lab_material_override_window->AddChild (new Slider (
            "Blue", 0.0f, 255.0f, 0, y + 2,
            labviewer_set_material_override_specular_blue, 275));
        y += sldr->GetHeight () + 1;

        cbp = (Checkbox*)Lab_material_override_window->AddChild (
            new Checkbox ("Override Gloss", 2, y));
        cbp->SetBool (&Gloss_override_set);
        y += cbp->GetHeight () + 1;

        sldr = (Slider*)Lab_material_override_window->AddChild (new Slider (
            "Gloss", 0.0f, 255.0f, 0, y + 2,
            labviewer_set_material_override_specular_gloss, 275));
        y += sldr->GetHeight () + 1;
    }

    // set close function
    Lab_material_override_window->SetCloseFunction (
        labviewer_close_material_override_window);
}
...
void labviewer_update_desc_window () {
    ...
    if (Lab_selected_index != -1) {
        if (Lab_mode == LAB_MODE_SHIP) {
            Lab_description_window->SetCaption (
                Ship_info[Lab_selected_index].name);

            if (Ship_info[Lab_selected_index].tech_desc != NULL) {
                Lab_description_text->SetText (
                    Ship_info[Lab_selected_index].tech_desc);
            }
            else {
            ...
            Lab_description_window->SetCaption (
                Weapon_info[Lab_selected_index].get_display_string ());

            if (Weapon_info[Lab_selected_index].tech_desc != NULL) {
                Lab_description_text->SetText (
                    Weapon_info[Lab_selected_index].tech_desc);
            }
            else {
                Lab_description_text->SetText ("No description available.");
            }
...
void labviewer_make_desc_window (Button* /*caller*/) {
    if (Lab_description_window != NULL) { return; }

    Lab_description_window = (Window*)Lab_screen->Add (new Window (
        "Description",
        gr_screen.center_offset_x + gr_screen.center_w -
            gr_screen.center_w / 3 - 50,
        gr_screen.center_offset_y + gr_screen.center_h -
            gr_screen.center_h / 6 - 50,
        gr_screen.center_w / 3, gr_screen.center_h / 6));
    Lab_description_text = (Text*)Lab_description_window->AddChild (
        new Text ("Description Text", "No ship selected.", 0, 0));
...
void labviewer_make_ship_window (Button* /*caller*/) {
    ...
    if (Lab_mode == LAB_MODE_SHIP) { return; }
    ...
    x = 0;
    cbp = Lab_class_toolbar->AddChild (
        new Button ("Class Description", x, 0, labviewer_make_desc_window));

    x += cbp->GetWidth () + 10;
    cbp = Lab_class_toolbar->AddChild (
        new Button ("Class Options", x, 0, labviewer_make_flags_window));

    x += cbp->GetWidth () + 10;
    cbp = Lab_class_toolbar->AddChild (
        new Button ("Class Variables", x, 0, labviewer_make_variables_window));

    // populate ship class window
    Tree* cmp =
        (Tree*)Lab_class_window->AddChild (new Tree ("Ship Tree", 0, 0));
    ...
    // Add species nodes
    for (idx = 0; idx < (int)Species_info.size (); idx++) {
        Lab_species_nodes[idx] =
            cmp->AddItem (NULL, Species_info[idx].species_name, 0, false);
    }

    // Just in case. I don't actually think this is possible though.
    Lab_species_nodes[Species_info.size ()] =
        cmp->AddItem (NULL, "Other", 0, false);
    ...
    for (auto it = Ship_info.cbegin (); it != Ship_info.cend (); ++it) {
        ...
        ctip = cmp->AddItem (
            stip, it->name, (int)std::distance (Ship_info.cbegin (), it),
            false, labviewer_change_ship);
        cmp->AddItem (ctip, "Model", 0, false, labviewer_change_ship_lod);
        // cmp->AddItem(ctip, "Debris", 99, false, labviewer_change_ship_lod);
    }

    // if any nodes are empty, just add a single "<none>" entry so we know that
    // species doesn't have anything yet No the <= is not a mistake :)
    for (idx = 0; idx < (int)Species_info.size (); idx++) {
    ...
    if (Lab_description_window != NULL) { labviewer_update_desc_window (); }
...
void labviewer_change_ship_lod (Tree* caller) {
    ...
    if (Lab_selected_object == -1) {
        ...
        // vector, but normally subsystems are loaded into the Ship_info entry.
        // Note also that models are only loaded once each.  If a lab model was
        // previously loaded using the "lightweight" method, ship_create will
        // not find any of the subsystems it is looking for.  So we have to
        // make sure the model is only loaded for the purposes of
        // error-checking and then immediately cleared so that it can be
        // subsequently loaded the lab way.
    ...
    Lab_selected_object =
        ship_create (&vmd_identity_matrix, &Lab_model_pos, ship_index);
    ...
    labviewer_change_model (
        Ship_info[ship_index].pof_file, caller->GetSelectedItem ()->GetData (),
        ship_index);
...
void labviewer_show_tech_model (Tree* caller) {
    ...
    labviewer_change_model (
        ...
        caller->GetSelectedItem ()->GetData (), weap_index);
}
...
void labviewer_change_weapon (Tree* caller) {
    ...
    if (!(Weapon_info[weap_index].wi_flags[Weapon::Info_Flags::Beam])) {
        Lab_selected_object = weapon_create (
            &vmd_zero_vector, &vmd_identity_matrix, weap_index, -1);
    }
...
void labviewer_make_weap_window (Button* /*caller*/) {
    ...
    if (Lab_mode == LAB_MODE_WEAPON) { return; }
    ...
    Tree* cmp =
        (Tree*)Lab_class_window->AddChild (new Tree ("Weapon Tree", 0, 0));
    ...
    for (i = 0; i < Num_weapon_types; i++) {
        if (Weapon_info[i].subtype == WP_UNUSED) { continue; }
            ...
            WARNINGF (LOCATION, "Invalid weapon subtype found on weapon %s",Weapon_info[i].name);
            continue;
        }
        ...
        cmp->AddItem (
            stip, Weapon_info[i].get_display_string (), i, false,
    ...
    for (i = 0; i < Num_weapon_subtypes; i++) {
        if (!type_nodes[i]->HasChildren ()) { delete type_nodes[i]; }
    }
    delete[] type_nodes;
...
std::string get_directory_or_vp (const char* path) {
    ...
    size_t found = result.find ("data/missions");
    ...
    auto directory_name_pos = result.rfind (
        '/', found - strlen ("/") - 1);
...
void labviewer_change_background_actual () {
    ...
    if (Lab_selected_mission != "None") {
        read_file_text (
            (Lab_selected_mission + ".fs2").c_str (), CF_TYPE_MISSIONS);
        reset_parse ();

        flagset< Mission::Mission_Flags > flags;
        skip_to_start_of_string ("+Flags");
        if (optional_string ("+Flags:")) stuff_flagset (&flags);
        ...
        if (optional_string ("#Background bitmaps")) {
            ...
            if (flags[Mission::Mission_Flags::Fullneb]) {
                ...
                if (Nebula_index >= 0) {
                    nebula_init (
                        Nebula_filenames[Nebula_index], Nebula_pitch,
                        Nebula_bank, Nebula_heading);
                }
                else {
                    nebula_close ();
                }
            ...
            while (optional_string ("$Bitmap List:") ||
                   check_for_string ("$Sun:") ||
        ...
        // (DahBlount) - This spot should be used to disable rendering features
        // that only apply to missions.
...
void labviewer_make_background_window (Button* /*caller*/) {
    ...
    Lab_background_window = (Window*)Lab_screen->Add (new Window (
        "Mission Backgrounds", gr_screen.center_offset_x + 250,
        gr_screen.center_offset_y + 50));
    ...
    for (const auto& filename : missions) {
        auto res = cf_find_file_location (
            (filename + ".fs2").c_str (), CF_TYPE_MISSIONS);
    ...
    Tree* missiontree =
        (Tree*)Lab_background_window->AddChild (new Tree ("Missions", 0, 0));
    ...
    for (auto directory : directories) {
        ...
        for (const auto& Lab_mission : directory.second) {
            missiontree->AddItem (
                directoryItem, Lab_mission, 0, true,
                labviewer_change_background);
        }
    }
}
...
void lab_init () {
    ...
    Lab_toolbar = (Window*)Lab_screen->Add (new Window (
        "Toolbar", gr_screen.center_offset_x, gr_screen.center_offset_y, -1,
        -1, WS_NOTITLEBAR | WS_NONMOVEABLE));

    // start filling the main toolbar
    x = 0;
    cbp = Lab_toolbar->AddChild (
        new Button ("Ships", x, 0, labviewer_make_ship_window));

    x += cbp->GetWidth () + 10;
    cbp = Lab_toolbar->AddChild (
        new Button ("Weapons", x, 0, labviewer_make_weap_window));

    x += cbp->GetWidth () + 10;
    cbp = Lab_toolbar->AddChild (new Button (
        "Render Options", x, 0, labviewer_make_render_options_window));

    x += cbp->GetWidth () + 10;
    cbp = Lab_toolbar->AddChild (new Button (
        "Material Overrides", x, 0, labviewer_make_material_override_window));

    x += cbp->GetWidth () + 10;
    cbp = Lab_toolbar->AddChild (
        new Button ("Backgrounds", x, 0, labviewer_make_background_window));
    ...
    for (i = 0; i < MAX_SHIP_WEAPONS; i++) { Lab_weaponmodel_num[i] = -1; }
    ...
    auto obj_index = obj_create (
        OBJ_OBSERVER, -1, 0, &vmd_identity_matrix, &vmd_zero_vector, 0,
        obs_flags);
    ...
    if (!Lab_cam.isValid ()) Lab_cam = cam_create ("Lab camera");
...
void lab_do_frame (float frametime) {
    ...
    bool test1 =
        (GUI_system.OnFrame (
             frametime, !(Trackball_active) ? true : false, false) ==
         GSOF_NOTHINGPRESSED);
    ...
    if (test1) {
        ...
        std::map< std::string, team_color >::iterator color_itr =
            Team_Colors.find (Lab_team_color);
        // handle any key presses
        switch (key) {
            // switch between the current insignia bitmap to render with
        case KEY_DIVIDE: {
            if (!(Lab_viewer_flags & LAB_FLAG_SHOW_INSIGNIA)) { break; }
            ...
            Lab_insignia_bitmap = bm_load_duplicate (
                Pilot_squad_image_names[Lab_insignia_index]);
        ...
        case KEY_L:
            ...
            // Adjust FXAA presets
        case KEY_0:
            if (!PostProcessing_override) Cmdline_fxaa_preset = 0;
            break;
        case KEY_1:
            if (!PostProcessing_override) Cmdline_fxaa_preset = 1;
            break;
        case KEY_2:
            if (!PostProcessing_override) Cmdline_fxaa_preset = 2;
            break;
        case KEY_3:
            if (!PostProcessing_override) Cmdline_fxaa_preset = 3;
            break;
        case KEY_4:
            if (!PostProcessing_override) Cmdline_fxaa_preset = 4;
            break;
        case KEY_5:
            if (!PostProcessing_override) Cmdline_fxaa_preset = 5;
            break;
        case KEY_6:
            if (!PostProcessing_override) Cmdline_fxaa_preset = 6;
            break;
        case KEY_7:
            if (!PostProcessing_override) Cmdline_fxaa_preset = 7;
            break;
        case KEY_8:
            if (!PostProcessing_override) Cmdline_fxaa_preset = 8;
            break;
        case KEY_9:
            if (!PostProcessing_override) Cmdline_fxaa_preset = 9;
            break;
        ...
        case KEY_Y:
            ...
            // bail...
        case KEY_ESC: labviewer_exit (NULL); break;
