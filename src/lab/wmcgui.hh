...
protected:
    ...
    // CALCULATESIZE
    // Sort of an on and do function; if you define your own, the following
...
public:
    ...
    GUIObject (
        const std::string& in_Name = "", int x_coord = 0, int y_coord = 0,
...
//*****************************GUISystem*******************************
class GUISystem {
    friend class GUIScreen; // I didn't want to do this, but it's the easiest
                            // way to keep it from being confusing about how to
                            // remove GUIScreens
private:
    GUIScreen Screens;
    ...
    int GraspedDiff[2]; // Diff between initial mouse position and object
                        // corner
...
#define WS_NOTITLEBAR \
    (1 << 31) // doesn't have a title bar (ie, no title or min/close buttons)
...
public:
    Button (
        const std::string& in_caption, int x_coord, int y_coord,
        void (*in_function) (Button* caller) = NULL, int x_width = -1,
        int y_height = -1, int in_style = 0);
...
public:
    Tree (
        ...
        void* in_associateditem = NULL, int x_width = -1, int y_width = -1,
        int in_style = 0);

    // void LoadItemList(TreeItem *in_list, unsigned int count);
    TreeItem* AddItem (
        TreeItem* parent, const std::string& in_name, int in_data = 0,
        bool in_delete_data = true, void (*in_function) (Tree* caller) = NULL);
    void ClearItems ();
...
public:
    Text (
        ...
        int y_coord, int x_width = -1, int y_width = -1, int in_style = 0);
    ...
    void SetSaveLoc (
        int* ptr, int save_method, int max_value = INT_MAX,
        int min_value = INT_MIN);
    void SetSaveLoc (
        short int* sint_ptr, int save_method, short int max_value = SHRT_MAX,
        short int min_value = SHRT_MIN);
    void SetSaveLoc (
        float* ptr, int save_method, float max_value = INT_MAX,
        float min_value = INT_MIN);
    void SetSaveLoc (
        char* ptr, int save_method, uint max_len = UINT_MAX, uint min_len = 0);
    void SetSaveLoc (
        ubyte* ptr, int save_method, int max_value = UCHAR_MAX,
        int min_value = 0);
    void SetSaveStringAlloc (
        char** ptr, int save_method, int mem_flags, uint max_len = UINT_MAX,
        uint min_len = 0);
...
public:
    Checkbox (
        ...
        void (*in_function) (Checkbox* caller) = NULL, int x_width = -1,
        int y_height = DEFAULT_BUTTON_HEIGHT, int in_style = 0);
    ...
    void SetFlag (int* in_flag_ptr, int in_flag) {
        ...
        if ((FlagPtr != NULL) && (*FlagPtr & Flag)) { IsChecked = true; }
    }
    ...
    void SetFlag (
        flagset< Ship::Info_Flags >& in_flag_set, Ship::Info_Flags flag_value,
        ship_info* si) {
        FlagPtr = nullptr;
        Sip = si;
    ...
    void SetFlag (
        flagset< Weapon::Info_Flags >& in_flag_set,
        Weapon::Info_Flags flag_value, weapon_info* wi) {
        FlagPtr = nullptr;
        Wip = wi;
        Flag = static_cast< size_t > (flag_value);
        IsChecked = in_flag_set[flag_value];
    ...
    void SetBool (bool* in_bool_ptr) { BoolFlagPtr = in_bool_ptr; }
};
...
public:
    ImageAnim (
        ...
        int x_coord, int y_coord, int x_width = -1, int y_width = -1,
        int in_style = 0);
...
public:
    Slider (
        ...
        int y_coord, void (*in_function) (Slider* caller) = NULL,
        int x_width = -1, int y_height = DEFAULT_BUTTON_HEIGHT,
        int in_style = 0);
...
#endif // FREESPACE2_LAB_WMCGUI_HH
