...
#define HUD_WINGMAN_STATUS_NOT_HERE \
    3 // wingman hasn't arrived, or has departed

typedef struct Wingman_status {
    int ignore; // set to 1 when we should ignore this item -- used in team v.
                // team
    int used;
...
void hud_set_wingman_status_none (int wing_index, int wing_pos) {
    ...
    for (i = 0; i < MAX_SHIPS_PER_WING; i++) {
        if (HUD_wingman_status[wing_index].status[i] !=
            HUD_WINGMAN_STATUS_NONE) {
            used = 1;
            break;
        }
    }
        ...
        for ( i = 0; i < Num_wings; i++ ) {
            ...
            if ( (wing_index >= 0) && (Wings[i].total_arrived_count == 0) ) {
                ...
                for (j = 0; j < Wings[i].wave_count; j++) {
                    HUD_wingman_status[wing_index].status[j] =
       HUD_WINGMAN_STATUS_NOT_HERE;
                }
            }
        }
...
void hud_wingman_status_update () {
    if (timestamp_elapsed (HUD_wingman_update_timer)) {
        ...
        HUD_wingman_update_timer =
            timestamp (HUD_WINGMAN_UPDATE_STATUS_INTERVAL);
        ...
        for (so = GET_FIRST (&Ship_obj_list);
            ...
            if ((wing_index >= 0) && (wing_pos >= 0) &&
                !(ship_objp->flags[Object::Object_Flags::Should_be_dead])) {
                HUD_wingman_status[wing_index].used = 1;
                if (!(shipp->is_departing ())) {
                    HUD_wingman_status[wing_index].status[wing_pos] =
                        HUD_WINGMAN_STATUS_ALIVE;
                }
                HUD_wingman_status[wing_index].hull[wing_pos] =
                    get_hull_pct (ship_objp);
                if (HUD_wingman_status[wing_index].hull[wing_pos] <= 0) {
                    HUD_wingman_status[wing_index].status[wing_pos] =
                        HUD_WINGMAN_STATUS_DEAD;
                }
            }
        }
    }
...
HudGaugeWingmanStatus::HudGaugeWingmanStatus ()
    : HudGauge (
          HUD_OBJECT_WINGMAN_STATUS, HUD_WINGMEN_STATUS, false, false,
...
void HudGaugeWingmanStatus::initBitmaps (
...
void HudGaugeWingmanStatus::renderBackground (int num_wings_to_draw) {
    ...
    if (bitmap > -1) { renderBitmap (bitmap, sx, sy); }
    ...
    // bring us to the end of the left portion so we can draw the last or
    // middle bits depending on how many wings we have to draw
    if (grow_mode == GROW_DOWN) { sy += left_frame_end_x; }
    else {
        sx += left_frame_end_x;
    }
...
void HudGaugeWingmanStatus::renderDots (
    int wing_index, int screen_index, int num_wings_to_draw) {
    int i, sx, sy, is_bright, bitmap = -1;

    if (Wingman_status_dots.first_frame < 0) { return; }
    ...
    if (num_wings_to_draw == 1) {
        ...
        sy = actual_origin[1] + multiple_wing_offsets[1] +
             screen_index * wing_width;
    }
    else {
        sx = actual_origin[0] + multiple_wing_offsets[0] +
             (screen_index - 1) * wing_width; // wing_width = 35
        sy = actual_origin[1] + multiple_wing_offsets[1];
    }

    // draw wingman dots
    for (i = 0; i < MAX_SHIPS_PER_WING; i++) {
        if (maybeFlashStatus (wing_index, i)) { is_bright = 1; }
        else {
            is_bright = 0;
        }
        ...
        default: bitmap = -1; break;

        } // end swtich

        if (bitmap > -1) {
            renderBitmap (
                bitmap, sx + wingmate_offsets[i][0],
                sy + wingmate_offsets[i][1]);
        }
    }
    ...
    renderString (
        sx - (int)std::lround ((float)abbrev_width / 2.0f), sy, abbrev);
}
    ...
    case 1:
        for (i = 0; i < MAX_SQUADRON_WINGS; i++) {
            if (HUD_wingman_status[i].used > 0) {
                for (j = 0; j < MAX_SHIPS_PER_WING; j++) {
                    if (HUD_wingman_status[i].status[j] !=
                        HUD_WINGMAN_STATUS_NONE) {
                        count++;
                    }
                }
...
void HudGaugeWingmanStatus::render (float /*frametime*/) {
    ...
    if (!hud_wingman_status_wingmen_exist (num_wings_to_draw)) { return; }
...
void hud_wingman_status_start_flash (int wing_index, int wing_pos) {
    HUD_wingman_flash_duration[wing_index][wing_pos] =
        timestamp (TBOX_FLASH_DURATION);
}
...
int hud_wingman_status_maybe_flash (int wing_index, int wing_pos) {
    ...
    if (!timestamp_elapsed (
        ...
        if (timestamp_elapsed (HUD_wingman_flash_next[wing_index][wing_pos])) {
            HUD_wingman_flash_next[wing_index][wing_pos] =
                timestamp (TBOX_FLASH_INTERVAL);
            HUD_wingman_flash_is_bright ^=
                (1 << index); // toggle between default and bright frames
        }

        if (HUD_wingman_flash_is_bright & (1 << index)) { draw_bright = 1; }
    }
...
bool HudGaugeWingmanStatus::maybeFlashStatus (int wing_index, int wing_pos) {
    ...
    if (!timestamp_elapsed (
        ...
        if (flash_status & (1 << index)) { draw_bright = true; }
    }
...
void HudGaugeWingmanStatus::pageIn () {
    bm_page_in_aabitmap (
        Wingman_status_left.first_frame, Wingman_status_left.num_frames);
    bm_page_in_aabitmap (
        Wingman_status_middle.first_frame, Wingman_status_middle.num_frames);
    bm_page_in_aabitmap (
        Wingman_status_right.first_frame, Wingman_status_right.num_frames);
    bm_page_in_aabitmap (
        Wingman_status_dots.first_frame, Wingman_status_dots.num_frames);
}
