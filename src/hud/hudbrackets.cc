...
void draw_brackets_square (
    ...
    if ((x1 + bracket_width > 0) && (x1 < gr_screen.clip_width)) {
        draw_list->add_gradient (
            x1, y1, x1 + bracket_width - 1, y1, GR_RESIZE_NONE); // top left
        draw_list->add_gradient (
            x1, y2, x1 + bracket_width - 1, y2, GR_RESIZE_NONE); // bottom left
    }

    if ((x2 - bracket_width < gr_screen.clip_width) && (x2 > 0)) {
        draw_list->add_gradient (
            x2, y1, x2 - bracket_width + 1, y1, GR_RESIZE_NONE); // top right
        draw_list->add_gradient (
            x2, y2, x2 - bracket_width + 1, y2,
            GR_RESIZE_NONE); // bottom right
    }

    // vertical lines
    if ((y1 + bracket_height > 0) && (y1 < gr_screen.clip_height)) {
        draw_list->add_gradient (
            x1, y1, x1, y1 + bracket_height - 1, GR_RESIZE_NONE); // top left
        draw_list->add_gradient (
            x2, y1, x2, y1 + bracket_height - 1, GR_RESIZE_NONE); // top right
    }

    if ((y2 - bracket_height < gr_screen.clip_height) && (y2 > 0)) {
        draw_list->add_gradient (
            x1, y2, x1, y2 - bracket_height + 1,
            GR_RESIZE_NONE); // bottom left
        draw_list->add_gradient (
            x2, y2, x2, y2 - bracket_height + 1,
            GR_RESIZE_NONE); // bottom right
    }
}

// NOTE: all values should be in unscaled range
void draw_brackets_square_quick (
    graphics::line_draw_list* draw_list, int x1, int y1, int x2, int y2,
    int thick) {
    int width, height;
...
#define NUM_DASHES 2
void draw_brackets_dashed_square_quick (
    graphics::line_draw_list* draw_list, int x1, int y1, int x2, int y2) {
    int width, height, i;
...
void draw_brackets_diamond_quick (
    graphics::line_draw_list* draw_list, int x1, int y1, int x2, int y2) {
    int width, height, half_width, half_height;
    int center_x, center_y;
    int x_delta, y_delta;
    ...
    draw_list->add_line (
        center_x - x_delta, center_y - y_delta, center_x + x_delta,
        center_y + y_delta);
    draw_list->add_line (
        center_x - x_delta, center_y + y_delta, center_x + x_delta,
        center_y - y_delta);
...
void hud_target_show_dist_on_bracket (
    int x, int y, float distance, int font_num) {
    char text_dist[64];
    int w, h;
    float displayed_distance;

    if (y < 0 || y > gr_screen.clip_height) { return; }

    if (x < 0 || x > gr_screen.clip_width) { return; }
    ...
    int y_delta = 4;
    if (HUD_drew_selection_bracket_on_target) { y += 4; }
...
// draw_bounding_brackets() will draw the faded brackets that surround the
// current target NOTE: x1, y1, x2 & y2 are assumed to be scaled sizes,
// w_correction & h_correction are assumed to be unscaled!! unused function,
...
int draw_subsys_brackets (
    ...
    bound_rc = subobj_find_2d_bound (
        subsys->system_info->radius, &targetp->orient, &subobj_pos, &x1, &y1,
        &x2, &y2);
    ...
    // determine if subsystem is on far or near side of the ship
    int in_sight = ship_subsystem_in_sight (
        targetp, subsys, &View_position, &subobj_pos, 0);

    if (draw) {
        if (set_color) {
            // AL 29-3-98: If subsystem is destroyed, draw gray brackets
            // Goober5000: this will now execute for fighterbays if the bay has
            // been given a percentage subsystem strength in ships.tbl
            // Goober5000: this will now execute for any subsys that takes
            // damage and will not execute for any subsys that doesn't take
            // damage
            if ((subsys->current_hits <= 0) &&
                (ship_subsys_takes_damage (subsys))) {
                gr_set_color_fast (iff_get_color (IFF_COLOR_MESSAGE, 1));
            }
            else {
                hud_set_iff_color (targetp, 1);
    ...
    if (draw_coords != nullptr) {
        // Positions are unsized, we need to resize them to get the actual
        // screen coordinates
...
HudGaugeBrackets::HudGaugeBrackets ()
    : HudGauge (
          HUD_OBJECT_BRACKETS, HUD_OFFSCREEN_INDICATOR, false, true,
          VM_DEAD_VIEW, 255, 255, 255) {}
    ...
    for (size_t i = 0; i < target_display_list.size (); i++) {
        ...
        if (!(target_display_list[i].target_point.flags & PF_OVERFLOW)) {
            if (target_display_list[i].objp) {
                renderObjectBrackets (
                    target_display_list[i].objp,
                    &target_display_list[i].bracket_clr,
                    target_display_list[i].correction,
                    target_display_list[i].correction,
                    target_display_list[i].flags);
            }
            else {
...
void HudGaugeBrackets::renderObjectBrackets (
    object* targetp, color* clr, int w_correction, int h_correction,
    int flags) {
    ...
    if (Player->target_is_dying <= 0) {
        ...
        case OBJ_SHIP:
            ...
            if (bound_rc != 0) { draw_box = false; }
            break;

        case OBJ_DEBRIS:
            modelnum = Debris[targetp->instance].model_num;
            bound_rc = submodel_find_2d_bound_min (
                modelnum, Debris[targetp->instance].submodel_num,
                &targetp->orient, &targetp->pos, &x1, &y1, &x2, &y2);
            if (bound_rc != 0) { draw_box = false; }
            break;
        ...
        case OBJ_WEAPON:
            ...
            if (modelnum != -1)
                bound_rc = model_find_2d_bound_min (
                    modelnum, &targetp->orient, &targetp->pos, &x1, &y1, &x2,
                    &y2);
            else {
                vertex vtx;
        ...
        case OBJ_ASTEROID: {
            ...
            modelnum =
                Asteroid_info[Asteroids[targetp->instance].asteroid_type]
                    .model_num[pof];
            bound_rc = model_find_2d_bound_min (
                modelnum, &targetp->orient, &targetp->pos, &x1, &y1, &x2, &y2);
        } break;
        ...
        if (clr->red && clr->green && clr->blue) { gr_set_color_fast (clr); }
        else {
        ...
        if (draw_box) {
            float distance =
                0.0f; // init to 0 if we don't have to display distance
            int target_objnum = -1;
            ...
            renderBoundingBrackets (
                x1 - 5, y1 - 5, x2 + 5, y2 + 5, w_correction, h_correction,
                distance, target_objnum, flags);
        }
...
void HudGaugeBrackets::renderNavBrackets (
    vec3d* nav_pos, vertex* nav_point, color* clr, char* string) {
    ...
    if (dist < 1000) box_scale = int(float(dist) / 1000.0f * 15);
    if (box_scale < 4) box_scale = 4;

    gr_set_color_fast (clr);
    draw_brackets_square (
        &line_draw_list, x - box_scale, y - box_scale, x + box_scale,
        y + box_scale);
    ...
    // draw distance to target in lower right corner of box
    hud_target_show_dist_on_bracket (
        x + (box_scale + 10), y + (box_scale + 10), dist, font_num);

    // bring the scale back to normal
    gr_reset_screen_scale ();
}

void HudGaugeBrackets::renderBoundingBrackets (
    int x1, int y1, int x2, int y2, int w_correction, int h_correction,
    float distance, int target_objnum, int flags) {
    int width, height;
    ...
    draw_brackets_square (
        &line_draw_list, x1 - w_correction, y1 - h_correction,
        x2 + w_correction, y2 + h_correction);
    ...
    if ((target_objnum != -1) &&
        (flags & (TARGET_DISPLAY_NAME | TARGET_DISPLAY_CLASS))) {
        object* t_objp = &Objects[target_objnum];
        const char* tinfo_name = NULL;
        const char* tinfo_class = NULL;
        char temp_name[NAME_LENGTH * 2 + 3];
        ...
        case OBJ_SHIP:
            ...
            if (*temp_name) {
                ...
                hud_stuff_ship_callsign (
                    temp_callsign, &Ships[t_objp->instance]);
                if (*temp_callsign)
                    sprintf (
                        &temp_name[strlen (temp_name)], " (%s)",
                        temp_callsign);
            }
        ...
        case OBJ_DEBRIS: tinfo_name = XSTR ("Debris", 348); break;
        case OBJ_WEAPON:
            strcpy (
                temp_name,
                Weapon_info[Weapons[t_objp->instance].weapon_info_index]
                    .get_display_string ());
            end_string_at_first_hash_symbol (temp_name);
            tinfo_name = temp_name;
            break;
        case OBJ_ASTEROID:
            ...
            case ASTEROID_TYPE_SMALL:
            case ASTEROID_TYPE_MEDIUM:
            case ASTEROID_TYPE_LARGE: tinfo_name = NOX ("Asteroid"); break;
            default: tinfo_name = XSTR ("Debris", 348);
            }
            break;
        case OBJ_JUMP_NODE:
            for (jnp = Jump_nodes.begin (); jnp != Jump_nodes.end (); ++jnp) {
                if (jnp->GetSCPObject () == t_objp) break;
            }
...
void HudGaugeBrackets::renderBoundingBracketsSubobject () {
    if (Player_ai->targeted_subsys_parent == Player_ai->target_objnum)
        if (Player_ai->targeted_subsys != NULL) {
            ...
            bound_rc = subobj_find_2d_bound (
                subsys->system_info->radius, &targetp->orient, &subobj_pos,
                &x1, &y1, &x2, &y2);
            if (bound_rc != 0) return;
