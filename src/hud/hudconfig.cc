...
int HUD_observer_default_flags = {
        (1 << HUD_CENTER_RETICLE) | (1 << HUD_OFFSCREEN_INDICATOR) | (1 << HUD_MESSAGE_LINES) | (1 << HUD_HOSTILE_TRIANGLE) | (1 << HUD_TARGET_TRIANGLE) | (1 << HUD_TARGET_MINI_ICON) | (1 << HUD_TARGET_MONITOR)
};
...
int HUD_config_default_flags = {
        (1 << HUD_LEAD_INDICATOR) | (1 << HUD_ORIENTATION_TEE) | (1 << HUD_HOSTILE_TRIANGLE) | (1 << HUD_TARGET_TRIANGLE) | (1 << HUD_MISSION_TIME) | (1 << HUD_RETICLE_CIRCLE) | (1 << HUD_THROTTLE_GAUGE) | (1 << HUD_RADAR) | (1 << HUD_TARGET_MONITOR) | (1 << HUD_CENTER_RETICLE) | (1 << HUD_TARGET_MONITOR_EXTRA_DATA) | (1 << HUD_TARGET_SHIELD_ICON) | (1 << HUD_PLAYER_SHIELD_ICON) | (1 << HUD_ETS_GAUGE) | (1 << HUD_AUTO_TARGET) | (1 << HUD_AUTO_SPEED) | (1 << HUD_WEAPONS_GAUGE) | (1 << HUD_ESCORT_VIEW) | (1 << HUD_DIRECTIVES_VIEW) | (1 << HUD_THREAT_GAUGE) | (1 << HUD_AFTERBURNER_ENERGY) | (1 << HUD_WEAPONS_ENERGY) | (1 << HUD_WEAPON_LINKING_GAUGE) | (1 << HUD_TARGET_MINI_ICON) | (1 << HUD_OFFSCREEN_INDICATOR) | (1 << HUD_TALKING_HEAD) | (1 << HUD_DAMAGE_GAUGE) | (1 << HUD_MESSAGE_LINES) | (1 << HUD_MISSILE_WARNING_ARROW) | (1 << HUD_CMEASURE_GAUGE) | (1 << HUD_OBJECTIVES_NOTIFY_GAUGE) | (1 << HUD_WINGMEN_STATUS)
};

int HUD_config_default_flags2 = {
        (1 << (HUD_OFFSCREEN_RANGE - 32)) | (1 << (HUD_KILLS_GAUGE - 32)) | (1 << (HUD_ATTACKING_TARGET_COUNT - 32)) | (1 << (HUD_SUPPORT_GAUGE - 32)) | (1 << (HUD_LAG_GAUGE - 32)) | (1 << (HUD_TEXT_FLASH - 32)) | (1 << (HUD_MESSAGE_BOX - 32))
};
...
void hud_config_init_ui()
{
        ...
        for (i = 0; i < NUM_HUD_GAUGES; i++) {
                ...
                if (!strcasecmp(hg->filename, NOX("none"))) {
                        continue;
                }
        ...
        case HCB_I_UP:
                if (HCS_CONV(HC_color_sliders[HCS_ALPHA].get_currentItem()) >= 255) {
                        ...
                        HC_color_sliders[HCS_ALPHA].force_currentItem(HCS_CONV(
                                HCS_CONV(HC_color_sliders[HCS_ALPHA].get_currentItem()) + 1));
        ...
        case HCB_SAVE_HCF:
                ...
                if (name[0] != '\0') {
                        ...
                        for (idx = 0; idx < HC_num_files; idx++) {
                                if (!strcasecmp(HC_filenames[idx], name)) {
                                        exists = 1;
                                }
                        }
                }
...
void hud_config_render_description()
{
        ...
        if (HC_gauge_selected >= 0) {
                ...
                sx = int(
                        HC_gauge_description_coords[gr_screen.res][0] + (HC_gauge_description_coords[gr_screen.res][2] - w) / 2.0f);
