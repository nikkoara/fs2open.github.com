...
// Flash timers that don't actually affect the targetbox have been moved to
// respective gauge classes (Swifty)
...
class HudGaugeTargetBox : public HudGauge // HUD_TARGET_MONITOR
{
        ...
        // remember, these coords describe the rightmost position of this element,
        // not the leftmost like it usually does.
        int Hull_offsets[2];
        ...
        // first element is time flashing expires, second element is time of next
        // flash
...
public:
        ...
        void
        renderTargetSetup(vec3d *camera_eye, matrix *camera_orient, float zoom);
...
class HudGaugeExtraTargetData
        : public HudGauge // HUD_TARGET_MONITOR_EXTRA_DATA
{
        hud_frames bracket;
...
// functions to manage the targetbox static that appears when sensors are
// severely damaged
...
#endif // FREESPACE2_HUD_HUDTARGETBOX_HH
