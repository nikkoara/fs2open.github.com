...
struct shield_hit_info  {
    ...
    int hull_hit_index; // used to access the members in shield_hit_info that
                        // pertain to the hull
    std::vector< int >
        shield_hit_timers; // timestamps that get set for SHIELD_FLASH_TIME
                           // when a quadrant is hit
    std::vector< int > shield_hit_next_flash;
    int shield_hit_status;  // bitfield, if offset for shield quadrant is set,
...
void hud_show_mini_ship_integrity (
    object* objp, int force_x = -1, int force_y = -1);
void hud_shield_show_mini (
    object* objp, int x_force = -1, int y_force = -1, int x_hull_offset = 0,
    int y_hull_offset = 0);
...
#endif // FREESPACE2_HUD_HUDSHIELD_HH
