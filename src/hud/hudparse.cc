...
void load_missing_retail_gauges()
{
        ...
        if (Hud_retail) {
                ...
                retail_gauge_loaded = false;
                for (int j = 0; j < num_loaded_gauges; j++) {
                        if (HUD_OBJECT_RADAR_STD == default_hud_gauges[j]->getObjectType() || HUD_OBJECT_RADAR_ORB == default_hud_gauges[j]->getObjectType() || HUD_OBJECT_RADAR_BSG == default_hud_gauges[j]->getObjectType()) {
                                retail_gauge_loaded = true;
                        }
                }
        ...
        for (auto it = Ship_info.cbegin(); it != Ship_info.cend(); k++, ++it) {
                ...
                if (it->hud_enabled && it->hud_retail) {
                        ...
                        retail_gauge_loaded = false;
                        for (int j = 0; j < num_loaded_gauges; j++) {
                                if (HUD_OBJECT_RADAR_ORB == it->hud_gauges[j]->getObjectType() || HUD_OBJECT_RADAR_STD == it->hud_gauges[j]->getObjectType() || HUD_OBJECT_RADAR_BSG == it->hud_gauges[j]->getObjectType()) {
                                        retail_gauge_loaded = true;
                                }
                        }
        ...
        int *base_res, int *force_scaling_above_res, bool scaling = true)
{
        ...
        if ((!scaling && gr_screen.center_w >= base_res[0] && gr_screen.center_h >= base_res[1] && (force_scaling_above_res[0] <= 0 || force_scaling_above_res[1] <= 0 || gr_screen.center_w <= force_scaling_above_res[0] || gr_screen.center_h <= force_scaling_above_res[1])) || (gr_screen.center_w >= base_res[0] && gr_screen.center_h == base_res[1]) || (gr_screen.center_w == base_res[0] && gr_screen.center_h >= base_res[1])) {
                base_res[0] = gr_screen.center_w;
                base_res[1] = gr_screen.center_h;
                return;
        }

        if (!scaling && force_scaling_above_res[0] > base_res[0] && force_scaling_above_res[1] > base_res[1] && gr_screen.center_w > force_scaling_above_res[0] && gr_screen.center_h > force_scaling_above_res[1]) {
...
void load_gauge_weapon_energy(gauge_settings *settings)
{
        ...
        if (Hud_reticle_style == HUD_RETICLE_STYLE_FS1) {
                if (gr_screen.res == GR_640) {
                        strcpy(fname, "energy2_fs1");
                } else {
                        strcpy(fname, "2_energy2_fs1");
                }
        } else {
                if (gr_screen.res == GR_640) {
                        strcpy(fname, "energy2");
                } else {
