...
char Lock_fname[NUM_HUD_RETICLE_STYLES][GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = { { "lock1_fs1", "2_lock1_fs1" }, { "lock1", "2_lock1" } };
...
void HudGaugeLock::render(float frametime)
{
        ...
        if (Player_ai->current_target_is_locked) {
                ...
                sx = int(lock_point.screen.xyw.x) - int(float(Player->current_target_sx - Players[Player_num].lock_indicator_x) * scaling_factor);
                sy = int(lock_point.screen.xyw.y) - int(float(Player->current_target_sy - Players[Player_num].lock_indicator_y) * scaling_factor);
                gr_unsize_screen_pos(&sx, &sy);
...
void hud_lock_reset(float lock_time_scale)
{
        ...
        if ((swp->current_secondary_bank >= 0) && (swp->secondary_bank_weapons[swp->current_secondary_bank] >= 0)) {
                ...
                ASSERT(
                        swp->secondary_bank_weapons[swp->current_secondary_bank] < MAX_WEAPON_TYPES);
                wip = &Weapon_info
                              [swp->secondary_bank_weapons[swp->current_secondary_bank]];
                Player->lock_time_to_target = float(wip->min_lock_time * lock_time_scale);
        } else {
...
void hud_do_lock_indicator(float frametime)
{
        ...
        // sight
        if (wip->wi_flags[Weapon::Info_Flags::Homing_javelin] && tobjp->type == OBJ_SHIP && Player->locking_subsys != NULL) {
                vec3d subobj_pos;
                vm_vec_unrotate(
...
void hud_calculate_lock_position(float frametime)
{
        ...
        if (Player->target_in_lock_cone) {
                if (!Players[Player_num].lock_indicator_visible) {
                        ...
                        Players[Player_num].lock_time_to_target = float(wip->min_lock_time);
                        catching_up = 0;
                }
                ...
                if (fabsf((float)accumulated_x_pixels) > 1.0f) {
                        ...
                        if (fabsf(
                                    (float)Players[Player_num].lock_indicator_x - (float)Player->current_target_sx)
                ...
                if (fabsf((float)accumulated_y_pixels) > 1.0f) {
                        ...
                        if (fabsf(
                                    (float)Players[Player_num].lock_indicator_y - (float)Player->current_target_sy)
                            < fabsf((float)int_portion))
                                Players[Player_num].lock_indicator_y = Player->current_target_sy;
                ...
                if (fabsf((float)accumulated_x_pixels) > 1.0f) {
                        ...
                        if (fabsf(
                                    (float)Players[Player_num].lock_indicator_x - (float)Players[Player_num].lock_indicator_start_x)
                            < fabsf((float)int_portion))
                                Players[Player_num].lock_indicator_x = Players[Player_num].lock_indicator_start_x;
                ...
                if (fabsf((float)accumulated_y_pixels) > 1.0f) {
                        ...
                        if (fabsf(
                                    (float)Players[Player_num].lock_indicator_y - (float)Players[Player_num].lock_indicator_start_y)
                            < fabsf((float)int_portion))
                                Players[Player_num].lock_indicator_y = Players[Player_num].lock_indicator_start_y;
...
void hud_calculate_lock_start_pos()
{
        ...
        if ((delta_x == 0.0) && (delta_y == 0.0)) {
                Players[Player_num].lock_indicator_start_x = int(gr_screen.clip_center_x + Lock_start_dist);
                Players[Player_num].lock_indicator_start_y = int(gr_screen.clip_center_y);
                return;
        }
        ...
        if (hypotenuse >= Lock_start_dist) {
                Players[Player_num].lock_indicator_start_x = int(gr_screen.clip_center_x);
                Players[Player_num].lock_indicator_start_y = int(gr_screen.clip_center_y);
                return;
        }
        ...
        Players[Player_num].lock_indicator_start_x = int(gr_screen.clip_center_x - target_x);
        Players[Player_num].lock_indicator_start_y = int(gr_screen.clip_center_y - target_y);
...
void hud_lock_get_new_lock_pos(object *target_objp)
{
        ...
        if ((target_shipp) && (Ship_info[target_shipp->ship_info_index].is_big_or_huge()) && !(wip->wi_flags[Weapon::Info_Flags::Homing_javelin])) {
                ...
                (target_shipp) && (wip->wi_flags[Weapon::Info_Flags::Homing_javelin])) {
                Player->locking_subsys = ship_get_closest_subsys_in_sight(
...
void hud_lock_determine_lock_point(vec3d *lock_world_pos_out)
{
        ...
        if (Player_ai->targeted_subsys && !(wip->wi_flags[Weapon::Info_Flags::Homing_javelin])) {
                ...
                (wip->wi_flags[Weapon::Info_Flags::Homing_javelin]) && (target_objp->type == OBJ_SHIP)) {
                if (!Player->locking_subsys || Player->locking_subsys->system_info->type != SUBSYSTEM_ENGINE) {
                        Player->locking_subsys = ship_get_closest_subsys_in_sight(
