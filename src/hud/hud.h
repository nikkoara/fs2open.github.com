...
#define HUD_COLOR_ALPHA_USER_MAX \
    13 // max user-settable alpha, absolute max is 15
#define HUD_COLOR_ALPHA_USER_MIN \
    3 // min user-settable alpha, absolute min is 0
...
#define RTIF_ENLARGE \
    (1 << 3) // make targeted blip appear larger than the other blips
...
void hud_update_frame (
    float frametime); // updates hud systems not dependant on rendering
...
// returns the offset between the player's view vector and the forward vector
// of the ship in pixels (Swifty)
...
int hud_anim_render (
    hud_anim* ha, float frametime, int draw_alpha = 0, int loop = 1,
    int hold_last = 0, int reverse = 0, int resize_mode = GR_RESIZE_FULL,
    bool mirror = false);
...
public:
    ...
    HudGauge (
        ...
        char* _custom_text, char* frame_fname, int txtoffset_x,
        int txtoffset_y);
    virtual ~HudGauge ();
    ...
    void renderBitmapUv (
        int frame, int x, int y, int w, int h, float u0, float v0, float u1,
        float v1);
    void
    renderBitmapEx (int frame, int x, int y, int w, int h, int sx, int sy);
    ...
    void renderPrintf (int x, int y, SCP_FORMAT_STRING const char* format, ...)
        SCP_FORMAT_STRING_ARGS (4, 5);
    void renderPrintf (
        int x, int y, int gauge_id, SCP_FORMAT_STRING const char* format, ...)
        SCP_FORMAT_STRING_ARGS (5, 6);
