...
struct scrollback_buttons {
    ...
    scrollback_buttons (
        const char* name, int x1, int y1, int x2, int y2, int h)
        : filename (name), x (x1), y (y1), xt (x2), yt (y2), hotspot (h) {}
};
...
// There is a maximum number of lines that will be stored in the message
// scrollback.  Oldest messages are deleted to make way for newest messages.
...
void hud_init_msg_window () {
    ...
    if (HUD_msg_inited == TRUE) { hud_free_scrollback_list (); }
...
HudGaugeMessages::HudGaugeMessages ()
    : HudGauge (
          HUD_OBJECT_MESSAGES, HUD_MESSAGE_LINES, false, true, (VM_WARP_CHASE),
          255, 255, 255) {}

void HudGaugeMessages::initMaxLines (int lines) {
    Max_lines =
        lines +
        1; // One additional line that's not displayed to scroll offscreen.
}
...
void HudGaugeMessages::initScrollTime (int ms) { Scroll_time = ms; }

void HudGaugeMessages::initStepSize (int h) { Step_size = h; }

void HudGaugeMessages::initTotalLife (int ms) { Total_life = ms; }

void HudGaugeMessages::initLineHeight (int h) { Line_h = h; }
...
void HudGaugeMessages::initialize () {
    // calculate the window height based on the number of lines, and line
    // height
    Window_height = Max_lines * Line_h;

    active_messages.clear ();
    pending_messages =
        std::queue< HUD_message_data > (); // there's no clear() method for
                                           // queues :/
...
void HudGaugeMessages::processMessageBuffer () {
    ...
    for (i = 0; i < HUD_msg_buffer.size (); i++) {
        ...
        while ((ptr = split_str_once (split_str, Max_width - x - 7)) !=
               NULL) { // the 7 is a fudge hack
            // make sure that split went ok, if not then bail
            if (ptr == split_str) { break; }
...
void HudGaugeMessages::scrollMessages () {
    // check if there is a message to display on the HUD, and if there is room
    // to display it
    if (!pending_messages.empty () && !Scroll_needed) {
        ...
        for (std::vector< Hud_display_info >::iterator m =
                 active_messages.begin ();
             m != active_messages.end (); ++m) {
            // determine if there are any existing messages, if so need to
            // scroll them up
        ...
        if (!timestamp_elapsed (m->total_life)) {
            if (m->y > m->target_y) {
                ...
                if (timestamp_elapsed (Scroll_time_id)) {
                    ...
                    if (m->y < m->target_y) { m->y = m->target_y; }
    ...
    if (Scroll_in_progress) { Scroll_time_id = timestamp (Scroll_time); }
}

void HudGaugeMessages::clearMessages () { active_messages.clear (); }
...
/**
 * HudGaugeMessages::render() will display the active HUD messages on the HUD.
 * It will scroll the messages up when a new message arrives.
 */
    ...
    for (std::vector< Hud_display_info >::iterator m =
        ...
        if (!timestamp_elapsed (m->total_life)) {
            if (!(Player->flags & PLAYER_FLAGS_MSG_MODE) ||
                !Hidden_by_comms_menu) {
                // set the appropriate color
                if (m->msg.source) { setGaugeColor (HUD_C_BRIGHT); }
                else {
                    setGaugeColor ();
                }
...
int HUD_team_get_source (int team) { return team + HUD_SOURCE_TEAM_OFFSET; }
...
void HUD_ship_sent_printf (int sh, const char* format, ...) {
    ...
    snprintf (
        tmp, sizeof (tmp) - 1, NOX ("%s: "), Ships[sh].get_display_string ());
    len = strlen (tmp);
...
// HUD_sourced_printf()
//
// HUD_sourced_printf() has the same parameters as printf(), but displays the
// text as a scrolling message on the HUD.  Text is split into multiple lines
// if width exceeds msg display area width.  'source' is used to indicate who
// send the message, and is used to color code text.
//
...
int hud_query_scrollback_size () {
    ...
    if (EMPTY (&Msg_scrollback_used_list) || !HUD_msg_inited) return 0;
...
// hud_add_msg_to_scrollback() adds the new_msg to the scroll-back message
// list.  If there are no more free slots, the first slot is released to make
...
// hud_free_scrollback_list() will free the memory that was allocated to store
// the messages for the scroll-back list
//
void hud_free_scrollback_list () {
    line_node* A;

    // check if the list has been inited yet.  If not, return with doing
    // nothing.
    if (Msg_scrollback_used_list.next == NULL ||
        Msg_scrollback_used_list.prev == NULL)
        return;
...
int hud_get_scroll_max_pos () {
    ...
    if (Scrollback_mode == SCROLLBACK_MODE_MSGS_LOG) {
        ...
        int excess =
            Scroll_max - Hud_mission_log_list_coords[gr_screen.res][3];

        if (EMPTY (&Msg_scrollback_used_list) || !HUD_msg_inited) { max = 0; }
        else {
            ptr = GET_FIRST (&Msg_scrollback_used_list);
            while (ptr != END_OF_LIST (&Msg_scrollback_used_list)) {
                if (ptr->source != HUD_SOURCE_HIDDEN) {
        ...
        max = (Scroll_max - Hud_mission_log_list_coords[gr_screen.res][3]) /
              font_height;
    }
...
void hud_scroll_reset () {
    if (Scrollback_mode == SCROLLBACK_MODE_OBJECTIVES) { Scroll_offset = 0; }
    else {
...
void hud_goto_pos (int delta) {
    ...
    if (Scrollback_mode == SCROLLBACK_MODE_MSGS_LOG) {
        ...
        if (EMPTY (&Msg_scrollback_used_list) || !HUD_msg_inited) return;
        ...
        while (ptr != END_OF_LIST (&Msg_scrollback_used_list)) {
            if (ptr->source != HUD_SOURCE_HIDDEN) {
                if (y <= pos + delta) Scroll_offset = count;
...
void hud_scrollback_button_pressed (int n) {
    switch (n) {
    case SCROLL_UP_BUTTON: hud_scroll_list (1); break;

    case SCROLL_DOWN_BUTTON: hud_scroll_list (0); break;
    ...
    case ACCEPT_BUTTON: hud_scrollback_exit (); break;
    }
}
...
void hud_scrollback_init () {
    ...
    common_set_interface_palette (
        "BriefingPalette"); // set the interface palette
    Ui_window.create (
    ...
    // add all strings
    Ui_window.add_XSTR (
        "Continue", 1069, Buttons[gr_screen.res][ACCEPT_BUTTON].xt,
        Buttons[gr_screen.res][ACCEPT_BUTTON].yt,
        &Buttons[gr_screen.res][ACCEPT_BUTTON].button, UI_XSTR_COLOR_PINK);
    Ui_window.add_XSTR (
        "Events", 1070, Buttons[gr_screen.res][SHOW_EVENTS_BUTTON].xt,
        Buttons[gr_screen.res][SHOW_EVENTS_BUTTON].yt,
        &Buttons[gr_screen.res][SHOW_EVENTS_BUTTON].button,
        UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR (
        "Objectives", 1071, Buttons[gr_screen.res][SHOW_OBJS_BUTTON].xt,
        Buttons[gr_screen.res][SHOW_OBJS_BUTTON].yt,
        &Buttons[gr_screen.res][SHOW_OBJS_BUTTON].button, UI_XSTR_COLOR_GREEN);
    Ui_window.add_XSTR (
        "Messages", 1072, Buttons[gr_screen.res][SHOW_MSGS_BUTTON].xt,
        Buttons[gr_screen.res][SHOW_MSGS_BUTTON].yt,
        &Buttons[gr_screen.res][SHOW_MSGS_BUTTON].button, UI_XSTR_COLOR_GREEN);
    ...
    Num_obj_lines = ML_objectives_init (
        Hud_mission_log_list_coords[gr_screen.res][0],
...
void hud_scrollback_do_frame (float /*frametime*/) {
    ...
    case KEY_PAGEUP: hud_page_scroll_list (1); break;

    case KEY_PAGEDOWN: hud_page_scroll_list (0); break;

    case KEY_ENTER:
    case KEY_CTRLED | KEY_ENTER:
    case KEY_ESC: hud_scrollback_exit (); break;
    ...
    /*
    if ((Scrollback_mode == SCROLLBACK_MODE_OBJECTIVES) && (Status_bitmap >=
    0)) { gr_set_bitmap(Status_bitmap);
        gr_bitmap(Hud_mission_log_status_coords[gr_screen.res][0],
    Hud_mission_log_status_coords[gr_screen.res][1], GR_RESIZE_MENU);
    }
    */
    ...
    if (Scrollback_mode == SCROLLBACK_MODE_EVENT_LOG) {
        ...
        mission_log_scrollback (
            Scroll_offset, Hud_mission_log_list_coords[gr_screen.res][0],
            Hud_mission_log_list_coords[gr_screen.res][1],
            Hud_mission_log_list_coords[gr_screen.res][2],
        ...
        if (!EMPTY (&Msg_scrollback_used_list) && HUD_msg_inited) {
            ...
            while (node_ptr != END_OF_LIST (&Msg_scrollback_used_list)) {
                if ((node_ptr->source == HUD_SOURCE_HIDDEN) ||
                    (i++ < Scroll_offset)) {
                    ...
                    if (team >= 0) {
                        gr_set_color_fast (iff_get_color_by_team (
                            team, Player_ship->team, 0));
                    }
                    else {
                        ...
                        default: gr_set_color_fast (&Color_text_normal); break;
                        }
                    }
                    ...
                    if (node_ptr->time)
                        gr_print_timestamp (
                            Hud_mission_log_list_coords[gr_screen.res][0],
                            Hud_mission_log_list_coords[gr_screen.res][1] + y,
                            node_ptr->time, GR_RESIZE_MENU);

                    x = Hud_mission_log_list2_coords[gr_screen.res][0] +
                        node_ptr->x;
                    gr_printf_menu (
                        x, Hud_mission_log_list_coords[gr_screen.res][1] + y,
                        "%s", node_ptr->text);
                    if (node_ptr->underline_width)
                        gr_line (
                            ...
                            Hud_mission_log_list_coords[gr_screen.res][1] + y +
                                font_height - 1,
                            x + node_ptr->underline_width,
                            Hud_mission_log_list_coords[gr_screen.res][1] + y +
                                font_height - 1,
                            GR_RESIZE_MENU);

                    if ((node_ptr->source == HUD_SOURCE_FAILED) ||
                        (node_ptr->source == HUD_SOURCE_SATISFIED)) {
                        // draw goal icon
                        if (node_ptr->source == HUD_SOURCE_FAILED)
                        ...
                        i = Hud_mission_log_list_coords[gr_screen.res][1] + y +
                            font_height / 2 - 1;
                        gr_circle (
                            Hud_mission_log_list2_coords[gr_screen.res][0] - 6,
                            i, 5, GR_RESIZE_MENU);

                        gr_set_color_fast (&Color_bright);
                        gr_line (
                            Hud_mission_log_list2_coords[gr_screen.res][0] -
                                10,
                            i,
                            Hud_mission_log_list2_coords[gr_screen.res][0] - 8,
                            i, GR_RESIZE_MENU);
                        gr_line (
                            Hud_mission_log_list2_coords[gr_screen.res][0] - 6,
                            i - 4,
                            ...
                            i - 2, GR_RESIZE_MENU);
                        gr_line (
                            Hud_mission_log_list2_coords[gr_screen.res][0] - 4,
                            i,
                            Hud_mission_log_list2_coords[gr_screen.res][0] - 2,
                            i, GR_RESIZE_MENU);
                        gr_line (
                            Hud_mission_log_list2_coords[gr_screen.res][0] - 6,
                            i + 2,
    ...
    gr_print_timestamp (
        Hud_mission_log_time_coords[gr_screen.res][0],
        Hud_mission_log_time_coords[gr_screen.res][1] - font_height,
        Missiontime, GR_RESIZE_MENU);
    gr_string (
        Hud_mission_log_time2_coords[gr_screen.res][0],
        Hud_mission_log_time_coords[gr_screen.res][1] - font_height,
        XSTR ("Current time", 289), GR_RESIZE_MENU);
    gr_flip ();
}

void hud_scrollback_exit () { gameseq_post_event (GS_EVENT_PREVIOUS_STATE); }

HudGaugeTalkingHead::HudGaugeTalkingHead ()
    : HudGauge (
          HUD_OBJECT_TALKING_HEAD, HUD_TALKING_HEAD, false, true,
          (VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY), 255, 255, 255) {}
...
HudGaugeTalkingHead::createAnim (int anim_start_frame, anim* anim_data) {
    ...
    anim_play_init (
        ...
        fl2ir ((position[1] + Anim_offsets[1] + HUD_offset_y) / scale_y),
        base_w, base_h);
    aps.start_at = anim_start_frame;

    // aps.color = &HUD_color_defaults[HUD_color_alpha];
    aps.color = &HUD_config.clr[HUD_TALKING_HEAD];
    // I'd much rather use gr_init_color and retrieve the colors from this
    // object but no, aps.color just happens to be a pointer. So, just give it
    // the address from the player's HUD configuration. You win, aps.color.
    // I'll take care of you next time. (Swifty)
...
void HudGaugeTalkingHead::render (float frametime) {
    if (Head_frame.first_frame == -1) { return; }
    ...
    if (msg_id != -1 && head_anim != NULL) {
        if (!head_anim->done_playing) {
            ...
            // clear
            setClip (
                position[0] + Anim_offsets[0], position[1] + Anim_offsets[1],
                Anim_size[0], Anim_size[1]);
            gr_clear ();
            resetClip ();
            ...
            generic_anim_render (
                head_anim, frametime,
                fl2ir (
                    (position[0] + Anim_offsets[0] + HUD_offset_x) / scale_x),
                fl2ir (
                    (position[1] + Anim_offsets[1] + HUD_offset_y) / scale_y));
            // draw title
            gr_set_screen_scale (base_w, base_h);
            renderString (
                position[0] + Header_offsets[0],
                position[1] + Header_offsets[1], XSTR ("message", 217));
        }
        else {
    ...
    // check playing messages to see if we have any messages with talking
    // animations that need to be created.
    for (int i = 0; i < Num_messages_playing; i++) {
        if (Playing_messages[i].play_anim &&
            Playing_messages[i].id != msg_id) {
...
HudGaugeFixedMessages::HudGaugeFixedMessages ()
    : HudGauge (
          HUD_OBJECT_FIXED_MESSAGES, HUD_MESSAGE_LINES, false, true,
          (VM_WARP_CHASE), 255, 255, 255),
      center_text (true) {}
...
void HudGaugeFixedMessages::render (float /*frametime*/) {
    ...
    if (!timestamp_elapsed (hp->end_time)) {
        gr_set_color (
            (hp->color >> 16) & 0xff, (hp->color >> 8) & 0xff,
            hp->color & 0xff);
        ...
        // renderString(0x8000, MSG_WINDOW_Y_START + MSG_WINDOW_HEIGHT + 8,
        // hp->text);
    }
}
