...
int Target_window_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                      8, 362, 131, 112 },
                                                    { // GR_1024
                                                      8, 629, 131, 112 } };
...
int Cargo_scan_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                   7, 364, 130, 109 },
                                                 { // GR_1024
                                                   7, 635, 130, 109 } };
...
// Different target states.  This drives the text display right below the hull
// integrity on the targetbox.
#define TS_DIS 0
#define TS_OK 1
#define TS_DMG 2
#define TS_CRT 3
...
void hud_targetbox_truncate_subsys_name (char* outstr) {
    if (Lcl_gr) {
        if (strstr (outstr, "communication")) { strcpy (outstr, "Komm"); }
            ...
            strstr (outstr, "fighterbay") || strstr (outstr, "Fighterbay")) {
            ...
            strstr (outstr, "Command Tower") || strstr (outstr, "Bridge")) {
            strcpy (
                outstr,
                "Br\x81"
                "cke");
        ...
        if (strstr (outstr, "communication")) { strcpy (outstr, "comm"); }
            ...
            strstr (outstr, "laser") || strstr (outstr, "turret") ||
            strstr (outstr, "missile")) {
...
HudGaugeTargetBox::HudGaugeTargetBox ()
    : HudGauge (
          HUD_OBJECT_TARGET_MONITOR, HUD_TARGET_MONITOR, false, false,
          (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY), 255,
          255, 255),
      Monitor_mask (-1), Use_subsys_name_offsets (false),
...
void HudGaugeTargetBox::initIntegrityHeight (int h) { integrity_bar_h = h; }
...
void HudGaugeTargetBox::initSubsysIntegrityOffsets (
...
void HudGaugeTargetBox::initDisabledStatusOffsets (
    int x, int y, bool activate) {
...
void HudGaugeTargetBox::initBitmaps (
    char* fname_monitor, char* fname_monitor_mask, char* fname_integrity,
    char* fname_static) {
...
void HudGaugeTargetBox::initialize () {
    hud_anim_init (
        &Monitor_static, position[0] + Viewport_offsets[0],
        position[1] + Viewport_offsets[1], NOX (static_fname));

    for (int i = 0; i < NUM_TBOX_FLASH_TIMERS; i++) { initFlashTimer (i); }
...
void HudGaugeTargetBox::render (float frametime) {
    ...
    if (Target_static_playing) return;
    ...
    case OBJ_SHIP: renderTargetShip (target_objp); break;

    case OBJ_DEBRIS: renderTargetDebris (target_objp); break;

    case OBJ_WEAPON: renderTargetWeapon (target_objp); break;

    case OBJ_ASTEROID: renderTargetAsteroid (target_objp); break;

    case OBJ_JUMP_NODE: renderTargetJumpNode (target_objp); break;

    default: hud_cease_targeting (); break;
    } // end switch
    ...
    if (Target_display_cargo) {
        ...
        if (maybeFlashSexp () == 1) { setGaugeColor (HUD_C_BRIGHT); }
        else {
            maybeFlashElement (TBOX_FLASH_CARGO);
        }

        renderString (
            position[0] + Cargo_string_offsets[0],
            position[1] + Cargo_string_offsets[1], EG_TBOX_CARGO,
            Cargo_string);
    }
}
 ...
 */
void HudGaugeTargetBox::renderTargetIntegrity (
    int disabled, int force_obj_num) {
    int clip_h, w, h;
    char buf[16];

    if (Integrity_bar.first_frame == -1) return;

    if (disabled) {
        renderBitmap (
            Integrity_bar.first_frame, position[0] + Integrity_bar_offsets[0],
            position[1] + Integrity_bar_offsets[1]);
        return;
    }
    ...
    renderString (
        position[0] + Status_offsets[0], position[1] + Status_offsets[1],
        EG_TBOX_INTEG, buf);
    ...
    if (clip_h > 0) {
        ...
        renderBitmapEx (
            Integrity_bar.first_frame, position[0] + Integrity_bar_offsets[0],
            position[1] + Integrity_bar_offsets[1], w, clip_h, 0, 0);
    }

    if (clip_h <= integrity_bar_h) {
        // draw the bright portion
        renderBitmapEx (
            Integrity_bar.first_frame + 1,
            position[0] + Integrity_bar_offsets[0],
            position[1] + Integrity_bar_offsets[1] + clip_h, w, h - clip_h, 0,
            clip_h);
    }
}

void HudGaugeTargetBox::renderTargetSetup (
    vec3d* camera_eye, matrix* camera_orient, float zoom) {
    ...
    setClip (
        position[0] + Viewport_offsets[0], position[1] + Viewport_offsets[1],
...
void HudGaugeTargetBox::renderTargetShip (object* target_objp) {
    ...
    if (Detail.targetview_model) {
        // take the forward orientation to be the vector from the player to the
        // current target
        ...
        // use the player's up vector, and construct the viewers orientation
        // matrix
        ...
        // normalize the vector from the player to the current target, and
        // scale by a factor to calculate the objects position
        vm_vec_copy_scale (&obj_pos, &orient_vec, factor);

        // RT, changed scaling here
        renderTargetSetup (
            &camera_eye, &camera_orient, target_sip->closeup_zoom);
        ...
        case 1:
            if (ship_is_tagged (target_objp))
                ...
                render_info.set_color (*iff_get_color_by_team_and_object (
                    target_shipp->team, Player_ship->team, 1, target_objp));

            if (target_sip->uses_team_colors) {
                render_info.set_team_color (
                    target_shipp->team_name, target_shipp->secondary_team_name,
                    target_shipp->team_change_timestamp,
                    target_shipp->team_change_time);
            }

            flags = MR_SHOW_OUTLINE_HTL | MR_NO_POLYS | MR_NO_LIGHTING |
                    MR_NO_TEXTURING;
        ...
        if (Monitor_mask >= 0) { gr_stencil_set (GR_STENCIL_READ); }
        ...
        if (!Glowpoint_override) Glowpoint_override = true;

        // set glowmap flag here since model_render (etc) require an objnum to
        // handle glowmaps if we did pass the objnum, we'd also have thrusters
        // drawn in the targetbox
        ...
        if (target_sip->model_num_hud >= 0) {
            model_render_immediate (
                &render_info, target_sip->model_num_hud, &target_objp->orient,
                &obj_pos);
        }
        else {
            ...
            model_render_immediate (
                &render_info, target_sip->model_num, &target_objp->orient,
                &obj_pos);
        }

        Glowpoint_override = false;

        if (Monitor_mask >= 0) { gr_stencil_set (GR_STENCIL_NONE); }
        ...
        if (Player_ai->targeted_subsys == Player_ai->last_subsys_target) {
            ...
            subsys_in_view =
                hud_targetbox_subsystem_in_view (target_objp, &sx, &sy);
            target_objp->pos = save_pos;

            if (subsys_in_view != -1) {
                // AL 29-3-98: If subsystem is destroyed, draw gray brackets
                // Goober5000 - hm, caught a tricky bug for destroyable
                // fighterbays
                ...
                if (subsys_in_view) {
                    draw_brackets_square_quick (
                        &line_draw_list, sx - 10, sy - 10, sx + 10, sy + 10);
                }
                else {
                    draw_brackets_diamond_quick (
                        &line_draw_list, sx - 10, sy - 10, sx + 10, sy + 10);
                }
...
void HudGaugeTargetBox::renderTargetDebris (object* target_objp) {
    ...
    if (Detail.targetview_model) {
        ...
        submodel_render_immediate (
            &render_info, debrisp->model_num, debrisp->submodel_num,
            &target_objp->orient, &obj_pos);
    ...
    if (debrisp->parent_alt_name >= 0)
        ...
        strcpy_s (
            ...
            (Ship_info[debrisp->ship_info_index].alt_name[0])
                ...
                : Ship_info[debrisp->ship_info_index].name);

    end_string_at_first_hash_symbol (printable_ship_class);

    renderString (
        position[0] + Class_offsets[0], position[1] + Class_offsets[1],
...
void HudGaugeTargetBox::renderTargetWeapon (object* target_objp) {
    ...
    int target_team, is_homing, is_player_missile, missile_view,
        viewed_model_num, hud_target_lod, w, h;
    float factor;
    ...
    if (target_wip->model_num == -1) return;
    ...
    if (Detail.targetview_model) {
        ...
        // adding a check here to make sure the homing object is a ship,
        // technically it could be some other object just as well
        ...
        // use the viewer's up vector, and construct the viewers orientation
        // matrix
        ...
        if (missile_view == FALSE)
            ...
            vm_vector_2_matrix (
                &camera_orient, &projection_vec, &up_vector, NULL);

        // normalize the vector from the viewer to the viwed target, and scale
        // by a factor to calculate the objects position
        if (missile_view == FALSE) {
            vm_vec_copy_scale (&obj_pos, &orient_vec, factor);
        }
        else {
        ...
        if (missile_view == FALSE) {
            ...
            case 1:
                render_info.set_color (*iff_get_color_by_team_and_object (
                    target_team, Player_ship->team, 0, target_objp));
            ...
            case 1:
                if (ship_is_tagged (viewed_obj))
                    render_info.set_color (
                        *iff_get_color (IFF_COLOR_TAGGED, 1));
                else
                    render_info.set_color (*iff_get_color_by_team_and_object (
                        homing_shipp->team, Player_ship->team, 1, viewed_obj));

                if (homing_sip->uses_team_colors) {
                    render_info.set_team_color (
                        homing_shipp->team_name,
                        homing_shipp->secondary_team_name,
                        homing_shipp->team_change_timestamp,
                        homing_shipp->team_change_time);
                }
        ...
        if (missile_view == TRUE) {
            ...
            // set glowmap flag here since model_render (etc) require an objnum
            // to handle glowmaps if we did pass the objnum, we'd also have
            // thrusters drawn in the targetbox
        ...
        if (missile_view == FALSE) {
            render_info.set_flags (
                flags | MR_AUTOCENTER | MR_IS_MISSILE | MR_NO_FOGGING);
            render_info.set_replacement_textures (replacement_textures);

            model_render_immediate (
                &render_info, viewed_model_num, &viewed_obj->orient, &obj_pos);
        }
        else {
            // maybe render a special hud-target-only model
            ...
            if (homing_sip->model_num_hud >= 0) {
                ...
                model_render_immediate (
                    &render_info, homing_sip->model_num_hud,
                    &viewed_obj->orient, &obj_pos);
            }
            else {
                render_info.set_flags (flags | MR_NO_FOGGING);
                render_info.set_replacement_textures (
                    homing_shipp->ship_replacement_textures);

                model_render_immediate (
                    &render_info, homing_sip->model_num, &viewed_obj->orient,
                    &obj_pos);
            }
        }

        if (missile_view == TRUE) { Glowpoint_override = false; }
    ...
    if (is_homing) {
        ...
        if (!(IS_VEC_NULL_SQ_SAFE (&target_objp->phys_info.vel))) {
            ...
            // in other words substract the magnitude of the target's velocity
            // vectors parallel component from the speed of the weapon
            vm_vec_copy_normalize (&unit_vec, &target_objp->phys_info.vel);
            speed -= vm_vec_projection_parallel (
                &component_vec, &wp->homing_object->phys_info.vel, &unit_vec);
        }
...
void HudGaugeTargetBox::renderTargetAsteroid (object* target_objp) {
    ...
    if (Detail.targetview_model) {
        ...
        model_clear_instance (
            Asteroid_info[asteroidp->asteroid_type].model_num[pof]);
    ...
    default:
        sprintf (
            hud_name, NOX ("%s debris"),
            Species_info[(asteroidp->asteroid_type / NUM_DEBRIS_SIZES) - 1]
                .species_name);
        break;
    }
...
void HudGaugeTargetBox::renderTargetJumpNode (object* target_objp) {
    ...
    for (jnp = Jump_nodes.begin (); jnp != Jump_nodes.end (); ++jnp) {
        if (jnp->GetSCPObject () != target_objp) continue;
        ...
        if (Detail.targetview_model) {
            // take the forward orientation to be the vector from the player to
            // the current target
            vm_vec_sub (&orient_vec, &target_objp->pos, &Player_obj->pos);
            vm_vec_normalize (&orient_vec);
            ...
            // use the player's up vector, and construct the viewers
            // orientation matrix
            if (Player_obj->type == OBJ_SHIP) {
                vec3d tempv;
                ship_get_eye (
                    &tempv, &camera_orient, Player_obj, false, false);
            }
            else {
                camera_orient = Player_obj->orient;
            }
        ...
        if (Hud_unit_multiplier >
            0.0f) { // use a different displayed distance scale
        ...
        renderPrintf (
            position[0] + Dist_offsets[0] + hx,
            position[1] + Dist_offsets[1] + hy, EG_TBOX_DIST, "%s", outstr);
    }
}
...
void hud_targetbox_switch_wireframe_mode () {
    Targetbox_wire++;
    if (Targetbox_wire == 3) Targetbox_wire = 0;
}
 ...
 * @note Need to call initFlashTimers for any TargetBox gauges and call
 * initDockFlashTimer() for Extra Target Info gauges (Switfty)
 */
...
int HudGaugeTargetBox::maybeFlashElement (int index, int flash_fast) {
    ...
    if (!timestamp_elapsed (Targetbox_flash_timers[index])) {
        if (timestamp_elapsed (Next_flash_timers[index])) {
            ...
            flash_flags ^=
                (1 << index); // toggle between default and bright frames
        }
...
HudGaugeExtraTargetData::HudGaugeExtraTargetData ()
    : HudGauge (
          HUD_OBJECT_EXTRA_TARGET_DATA, HUD_TARGET_MONITOR_EXTRA_DATA, false,
          false, (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY),
          255, 255, 255) {
    initDockFlashTimer ();
}
...
/**
 * @note Formerly hud_targetbox_show_extra_ship_info(target_shipp, target_objp)
 * (Swifty)
 */
void HudGaugeExtraTargetData::render (float /*frametime*/) {
    ...
    if (target_objp->type != OBJ_SHIP) return;
    ...
    if (not_training) {
        ...
        // Backslash: actually let's consult the IFF table.  Maybe we want to
        // show orders for certain teams, or hide orders for friendlies
        if (((Player_ship->team == target_shipp->team) ||
            ...
            renderString (
                position[0] + order_offsets[0], position[1] + order_offsets[1],
                EG_TBOX_EXTRA1, orders.c_str ());
        }
        ...
        if (has_orders) {
            ...
            if (ship_return_time_to_goal (tmpbuf, target_shipp)) {
                ...
                renderString (
                    ...
                    position[1] + time_offsets[1], EG_TBOX_EXTRA2, outstr);
            }
        }
    }
    ...
    if (object_is_docked (target_objp)) {
        ...
        if (dock_count == 1) {
            sprintf (
                outstr, XSTR ("Docked: %s", 339),
                Ships[dock_get_first_docked_object (target_objp)->instance]
                    .get_display_string ());
            end_string_at_first_hash_symbol (outstr);
        }
        ...
        renderString (
            position[0] + dock_offsets[0], position[1] + dock_offsets[1],
            EG_TBOX_EXTRA3, outstr);
        extra_data_shown = 1;
    }

    if (extra_data_shown) {
        renderBitmap (
            bracket.first_frame, position[0] + bracket_offsets[0],
            position[1] + bracket_offsets[1]);
    }
}
...
extern flagset< Weapon::Info_Flags >
...
void HudGaugeTargetBox::renderTargetShipInfo (object* target_objp) {
    ...
    if (*outstr_name) {
        ...
        if (*outstr_callsign)
            sprintf (
                &outstr_name[strlen (outstr_name)], " (%s)", outstr_callsign);
    }
    ...
    if (screen_integrity == 0) {
        if (ship_integrity > 0) { screen_integrity = 1; }
    }
    // Print out right-justified integrity
    sprintf (outstr, XSTR ("%d%%", 341), screen_integrity);
    gr_get_string_size (&w, &h, outstr);

    if (HudGauge::maybeFlashSexp () == 1) { setGaugeColor (HUD_C_BRIGHT); }
    else {
        maybeFlashElement (TBOX_FLASH_HULL);
    }

    renderPrintf (
        position[0] + Hull_offsets[0] - w, position[1] + Hull_offsets[1],
        EG_TBOX_HULL, "%s", outstr);
    setGaugeColor ();

    // print out the targeted sub-system and % integrity
    if (Player_ai->targeted_subsys != NULL) {
        shield_strength = Player_ai->targeted_subsys->current_hits /
        ...
        if (Player_ai->targeted_subsys->system_info->type ==
            ...
            get_turret_subsys_name (
                &Player_ai->targeted_subsys->weapons, outstr);
        }
        else {
            sprintf (
                outstr, "%s",
                ship_subsys_get_name (Player_ai->targeted_subsys));
        }
        ...
        if (n_linebreaks) {
            ...
            while (p_line != NULL) {
                renderPrintf (
                    subsys_name_pos_x,
                    subsys_name_pos_y - h - ((h + 1) * n_linebreaks), "%s",
                    p_line);
        ...
        if (Use_subsys_integrity_offsets) {
            ...
            subsys_integrity_pos_x =
                position[0] + Viewport_offsets[0] + Viewport_w - 1;
            subsys_integrity_pos_y =
        ...
        if (ship_subsys_takes_damage (Player_ai->targeted_subsys)) {
            ...
            gr_get_string_size (&w, &h, outstr);
            renderPrintf (
                subsys_integrity_pos_x - w, subsys_integrity_pos_y - h, "%s",
                outstr);
        }
    ...
    if ((target_shipp->flags[Ship::Ship_Flags::Disabled]) ||
        ...
        if (Use_disabled_status_offsets) {
            ...
            disabled_status_pos_y =
                position[1] + Viewport_offsets[1] + Viewport_h - 2 * h;
        }

        renderPrintf (
            disabled_status_pos_x, disabled_status_pos_y, "%s", outstr);
    }
}

/**
 * Determine if the subsystem is in line-of sight, without taking into account
 * whether the player ship is facing the subsystem
 */
int hud_targetbox_subsystem_in_view (object* target_objp, int* sx, int* sy) {
    ...
    if (subsys != NULL) {
        ...
        if (Player->subsys_in_view == -1) {
            rval = ship_subsystem_in_sight (
                target_objp, subsys, &View_position, &subobj_pos, 0);
        }
        else {
...
/**
 * If the player is scanning for cargo, draw some cool scanning lines on the
 * target monitor
 */
void HudGaugeTargetBox::maybeRenderCargoScan (ship_info* target_sip) {
    ...
    if (Player->cargo_inspect_time <= 0) { return; }
...
void HudGaugeTargetBox::showTargetData (float /*frametime*/) {
    ...
    ship_info* sip = NULL;
    int is_ship = 0;
    float displayed_target_distance, displayed_target_speed,
        current_target_distance, current_target_speed;
    ...
    if (Hud_unit_multiplier >
        0.0f) { // use a different displayed distance scale
        displayed_target_distance =
            current_target_distance * Hud_unit_multiplier;
    }
    else {
    ...
    case OBJ_WEAPON: sip = NULL; break;

    case OBJ_ASTEROID: sip = NULL; break;
    ...
    sprintf (
        outstr, XSTR ("d: %.0f%s", 350), displayed_target_distance,
    ...
    renderString (
        position[0] + Dist_offsets[0] + hx, position[1] + Dist_offsets[1] + hy,
        EG_TBOX_DIST, outstr);
    ...
    if (current_target_speed < 0.1f) { current_target_speed = 0.0f; }
    // if the speed is 0, determine if we are docked with something -- if so,
    // get the docked velocity
    if ((current_target_speed == 0.0f) && is_ship) {
        current_target_speed =
            dock_calc_docked_fspeed (&Objects[shipp->objnum]);
    ...
    sprintf (
        outstr, XSTR ("s: %.0f%s", 351), displayed_target_speed,
        (displayed_target_speed > 1)
            ? modifiers[Player_ai->current_target_speed_trend]
            : "");
    hud_num_make_mono (outstr, font_num);

    renderString (
        position[0] + Speed_offsets[0] + hx,
        position[1] + Speed_offsets[1] + hy, EG_TBOX_SPEED, outstr);
    ...
    if (Show_target_debug_info && (is_ship == 1)) {
        ...
        if (shipp->ai_index >= 0) {
            ...
            gr_printf_no_resize (
                sx, sy, "Max speed = %d, (%d%%)",
                (int)shipp->current_max_speed,
                (int)(100.0f * vm_vec_mag (&target_objp->phys_info.vel) / shipp->current_max_speed));
            sy += dy;
            ...
            if (aip->target_objnum != -1) {
                ...
                if (aip->target_objnum == OBJ_INDEX (Player_obj))
                    ...
                    sprintf (
                        target_str, "%s",
                        Ships[Objects[aip->target_objnum].instance]
                            .get_display_string ());
                ...
                dist = vm_vec_dist_quick (
                    ...
                    &Objects[aip->target_objnum].pos);
                vm_vec_normalized_dir (
                    &v2t, &Objects[aip->target_objnum].pos,
                    &Objects[Player_ai->target_objnum].pos);

                dot = vm_vec_dot (
                    &v2t, &Objects[Player_ai->target_objnum].orient.vec.fvec);
                ...
                if (aip->targeted_subsys != NULL) {
                    sprintf (
                        outstr, "Subsys: %s",
                        aip->targeted_subsys->system_info->subobj_name);
                    gr_printf_no_resize (sx, sy, "%s", outstr);
                }
                sy += dy;
            }
            ...
            sprintf (
                outstr, "MAX G/E: %.0f/%.0f", shipp->weapon_energy,
                shipp->current_max_speed);
            gr_printf_no_resize (sx, sy, "%s", outstr);
            sy += dy;

            sprintf (
                outstr, "G/S/E: %.2f/%.2f/%.2f",
                Energy_levels[shipp->weapon_recharge_index],
                Energy_levels[shipp->shield_recharge_index],
                Energy_levels[shipp->engine_recharge_index]);
            gr_printf_no_resize (sx, sy, "%s", outstr);
            sy += dy;
                ...
                if (Enemy_attacker != NULL)
                    if (Enemy_attacker->type == OBJ_SHIP) {
                        ...
                        if (eaip->target_objnum == OBJ_INDEX (Player_obj)) {
                            found = 1;
                            dist = vm_vec_dist_quick (
                                &Enemy_attacker->pos, &Player_obj->pos);
                            vm_vec_normalized_dir (
                                &v2t, &Objects[eaip->target_objnum].pos,
                                &Enemy_attacker->pos);

                            dot = vm_vec_dot (
                                &v2t, &Enemy_attacker->orient.vec.fvec);

                            gr_printf_no_resize (
                                sx, sy, "#%i: %s", OBJ_INDEX (Enemy_attacker),
                                Ships[Enemy_attacker->instance]
                                    .get_display_string ());
                            sy += dy;
                            gr_printf_no_resize (
                                sx, sy, "Targ dist: %5.1f", dist);
                            sy += dy;
                            gr_printf_no_resize (
                                sx, sy, "Targ dot: %3.2f", dot);
                            sy += dy;
                        }
                    }
                    ...
                    for (i = 0; i < MAX_OBJECTS; i++)
                        if (Objects[i].type == OBJ_SHIP) {
                            ...
                            if (i != Player_ai->target_objnum) {
                                enemy = Ai_info[Ships[Objects[i].instance]
                                                    .ai_index]
                                            .target_objnum;
            ...
            // hud_target_w
            gr_printf_no_resize (
                sx, sy, "Targ size: %dx%d", Hud_target_w, Hud_target_h);
            sy += dy;
...
void hud_update_target_static () {
    ...
    if (sensors_str > SENSOR_STR_TARGET_NO_EFFECTS) {
        ...
        if (Target_static_next == 0) Target_static_next = 1;
    }
    ...
    if (Target_static_playing) {
        if (!Target_static_looping.isValid ()) {
            Target_static_looping =
                snd_play_looping (gamesnd_get_game_sound (GameSounds::STATIC));
        }
    }
    else {
        if (Target_static_looping.isValid ()) {
 ...
 *
 * Checks for disabled or ships with disrupted engines, as well as damage
 * levels of the target ship. If status has changed, then the HUD will flash
 *
 * @param targetp Instance of the ship target -- note the targetp->instance
 * cannot be negative
 *
 */
void hud_update_ship_status (object* targetp) {
    ...
    if ((targetp->instance >= 0) && (targetp->instance < MAX_SHIPS)) {
        ...
        if ((Ships[targetp->instance].flags[Ship::Ship_Flags::Disabled]) ||
            (ship_subsys_disrupted (
                &Ships[targetp->instance], SUBSYSTEM_ENGINE))) {
            Current_ts = TS_DIS;
        }
        else {
            if (Pl_target_integrity > 0.9) { Current_ts = TS_OK; }
