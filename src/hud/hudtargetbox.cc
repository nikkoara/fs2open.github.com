...
void hud_targetbox_truncate_subsys_name(char *outstr)
{
        if (Lcl_gr) {
                ...
                if (strstr(outstr, "communication")) {
                        ...
                        strstr(outstr, "laser") || strstr(outstr, "turret") || strstr(outstr, "missile")) {
...
int HudGaugeTargetBox::maybeFlashElement(int index, int flash_fast)
{
        ...
        if (!timestamp_elapsed(Targetbox_flash_timers[index])) {
                if (timestamp_elapsed(Next_flash_timers[index])) {
                        if (flash_fast) {
                                Next_flash_timers[index] = timestamp(int(TBOX_FLASH_INTERVAL / 2.0f));
                        } else {
...
void HudGaugeTargetBox::maybeRenderCargoScan(ship_info *target_sip)
{
        ...
        y1 = int(
                0.5f + position[1] + Cargo_scan_start_offsets[1] + ((float(Player->cargo_inspect_time) / scan_time) * Cargo_scan_h));
        x2 = x1 + Cargo_scan_w;
        ...
        if (Cmdline_dualscanlines) {
                ...
                y1 = int(
                        position[1] + Cargo_scan_start_offsets[1] + Cargo_scan_h - ((float(Player->cargo_inspect_time) / scan_time) * Cargo_scan_h));
                renderLine(x1, y1, x2, y1);
        }

        // draw vertical scan line
        x1 = int(
                0.5f + position[0] + Cargo_scan_start_offsets[0] + ((float(Player->cargo_inspect_time) / scan_time) * Cargo_scan_w));
        ...
        if (Cmdline_dualscanlines) {
                ...
                x1 = int(
                        0.5f + Cargo_scan_w + position[0] + Cargo_scan_start_offsets[0] - ((float(Player->cargo_inspect_time) / scan_time) * Cargo_scan_w));
                renderLine(x1, y1 - 3, x1, y2 - 1);
        }
}
...
void HudGaugeTargetBox::showTargetData(float /*frametime*/)
{
        ...
        if (Show_target_debug_info && (is_ship == 1)) {
                ...
                if (shipp->ai_index >= 0) {
                        ...
                        case AIM_STRAFE:
                                ASSERT(
                                        aip->submode <= AIS_STRAFE_POSITION); // Must be <= largest chase
                                                                              // submode value.
                                sprintf(
                                        outstr2, " / %s",
