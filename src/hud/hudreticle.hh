...
#ifndef FREESPACE2_HUD_HUDRETICLE_HH
#define FREESPACE2_HUD_HUDRETICLE_HH
...
public:
    ...
    void
    initFirepointDisplay (bool firepoint, int scaleX, int scaleY, int size);
...
protected:
    ...
    int Max_speed_offsets[2]; // Max_speed_coords[gr_screen.res][0]
                              // Max_speed_coords[gr_screen.res][1]
    bool Show_max_speed;

    int Zero_speed_offsets[2]; // Zero_speed_coords[gr_screen.res][0]
                               // Zero_speed_coords[gr_screen.res][1]
    bool Show_min_speed;

    int Orbit_center_offsets[2]; // Hud_reticle_center[gr_screen.res][0]
    ...
    bool Match_speed_draw_background; // When true draw the match icon onto a
                                      // rectangle
...
public:
    ...
    void initThrottleSizes (
        int w,
        int h); // throttle_w will be implicitly figured out using bm_get_info
...
protected:
    hud_frames threat_arc; // The right arc if FS2 style reticle. Top arc if
                           // FS1 style reticle
...
public:
    ...
    void initBitmaps (
        char* fname_arc, char* fname_primary_link_1,
        char* fname_primary_link_2, char* fname_secondary_link_1,
        char* fname_secondary_link_2, char* fname_secondary_link_3);
    void render (float frametime) override;
    void pageIn () override;
};
...
#endif // FREESPACE2_HUD_HUDRETICLE_HH
