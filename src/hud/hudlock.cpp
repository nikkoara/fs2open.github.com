...
float Lock_triangle_base[GR_NUM_RESOLUTIONS] = { 4.0f, 6.5f };
float Lock_triangle_height[GR_NUM_RESOLUTIONS] = { 4.0f, 6.5f };

int Lock_gauge_half_w[NUM_HUD_RETICLE_STYLES][GR_NUM_RESOLUTIONS] = {
    { 15, 24 }, { 17, 28 }
};
int Lock_gauge_half_h[GR_NUM_RESOLUTIONS] = { 15, 25 };
...
char Lock_fname[NUM_HUD_RETICLE_STYLES][GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] =
    { { "lock1_fs1", "2_lock1_fs1" }, { "lock1", "2_lock1" } };

char Lockspin_fname[NUM_HUD_RETICLE_STYLES][GR_NUM_RESOLUTIONS]
                   [MAX_FILENAME_LEN] = { { "lockspin_fs1", "2_lockspin_fs1" },
                                          { "lockspin", "2_lockspin" } };
...
HudGaugeLock::HudGaugeLock ()
    : HudGauge (
          HUD_OBJECT_LOCK, HUD_LEAD_INDICATOR, false, false, VM_DEAD_VIEW, 255,
          255, 255) {}
...
void HudGaugeLock::initTriHeight (float h) { Lock_triangle_height = h; }

void HudGaugeLock::initTriBase (float length) { Lock_triangle_base = length; }
...
void HudGaugeLock::initLoopLockedAnim (bool loop) { loop_locked_anim = loop; }

void HudGaugeLock::initBitmaps (
    char* lock_gauge_fname, char* lock_anim_fname) {
...
// hud_show_lock_indicator() will display the lock indicator for homing
// missiles. lock_point_pos should be the world coordinates of the target being
// locked. Assuming all the necessary locking calculations are done for this
// frame, this function will compute where the indicator should be relative to
// the player's viewpoint and will render accordingly.
void HudGaugeLock::render (float frametime) {
    ...
    // the lock indicator if there are missiles to fire.
    if (!ship_secondary_bank_has_ammo (Player_obj->instance)) { return; }
    ...
    if (Player_ai->current_target_is_locked) {
        ...
        if (reset_timers) { Lock_gauge.time_elapsed = 0.0f; }
    }
    else {
...
int hud_lock_has_homing_point () {
    if (Player_ai->targeted_subsys || Player->locking_subsys ||
...
int hud_lock_world_pos_in_range (
    vec3d* target_world_pos, vec3d* vec_to_target) {
    ...
    if (wip->wi_flags[Weapon::Info_Flags::Local_ssm])
        ...
        // indicator when if the weapon can actually hit the target
        weapon_range =
    ...
    if (dist_to_target > weapon_range) { target_in_range = 0; }
...
int hud_lock_target_in_range () {
    ...
    if (Player_ai->targeted_subsys != NULL) {
        vm_vec_unrotate (
            &target_world_pos, &Player_ai->targeted_subsys->system_info->pnt,
            &targetp->orient);
        ...
        if (Player->locking_subsys) {
            vm_vec_unrotate (
                &target_world_pos, &Player->locking_subsys->system_info->pnt,
                &targetp->orient);
            vm_vec_add2 (&target_world_pos, &targetp->pos);
        }
        else {
...
int hud_abort_lock () {
    ...
    if (Player_ship->weapons.num_secondary_banks <= 0) { return 1; }

    if (Player_ship->weapons.current_secondary_bank < 0) { return 1; }

    // check to see if there are any missile to fire.. we don't want to show
    // the lock indicator if there are no missiles to fire.
    if (!ship_secondary_bank_has_ammo (Player_obj->instance)) { return 1; }

    if (Player_ship->flags[Ship::Ship_Flags::No_secondary_lockon]) {
        return 1;
    }

    // if we're on the same team and the team doesn't attack itself, then don't
    // lock!
...
int hud_lock_on_subsys_ok () {
    ...
    vm_vec_unrotate (
        &subobj_pos, &subsys->system_info->pnt, &target_objp->orient);
    vm_vec_add2 (&subobj_pos, &target_objp->pos);

    if (Player->subsys_in_view < 0) {
        in_sight = ship_subsystem_in_sight (
            target_objp, subsys, &View_position, &subobj_pos);
    }
    else {
...
void hud_lock_check_if_target_in_lock_cone () {
    ...
    if (dot > 0.85) { Player->target_in_lock_cone = 1; }
    else {
        Player->target_in_lock_cone = 0;
    }
}

// return 1 if current secondary weapon is different than previous secondary
// weapon
    ...
    /*
       ...
       swp->secondary_bank_weapons[swp->current_secondary_bank];
        ...
        if ( Player_ai->last_secondary_index >= 0 ) {
            last_wi_index =
       swp->secondary_bank_weapons[Player_ai->last_secondary_index];
        }
...
// hud_do_lock_indicator() manages missle locking, both the non-rendering
// calculations and the 2D HUD rendering
void hud_do_lock_indicator (float frametime) {
    ...
    if (hud_lock_secondary_weapon_changed (swp)) { hud_lock_reset (); }
    ...
    // Allow locking on ships and bombs (only targeted weapon allowed is a
    // bomb, so don't bother checking flags)
    ...
    // Javelins must lock on engines if locking on a ship and those must be in
    // sight
    if (wip->wi_flags[Weapon::Info_Flags::Homing_javelin] &&
        tobjp->type == OBJ_SHIP && Player->locking_subsys != NULL) {
        vec3d subobj_pos;
        vm_vec_unrotate (
            &subobj_pos, &Player->locking_subsys->system_info->pnt,
            &tobjp->orient);
        vm_vec_add2 (&subobj_pos, &tobjp->pos);
        int target_subsys_in_sight = ship_subsystem_in_sight (
            tobjp, Player->locking_subsys, &Player_obj->pos, &subobj_pos);

        if (!target_subsys_in_sight ||
            Player->locking_subsys->system_info->type != SUBSYSTEM_ENGINE) {
            Player->locking_subsys = ship_get_closest_subsys_in_sight (
                &Ships[tobjp->instance], SUBSYSTEM_ENGINE, &Player_obj->pos);
        }
    }
    ...
    if (!hud_lock_has_homing_point ()) { Player->target_in_lock_cone = 0; }

    hud_lock_check_if_target_in_lock_cone ();

    // check if the target is within range of the current secondary weapon.  If
    // it is not, a lock will not be detected
    ...
    if (Player_ai->current_target_is_locked) {
        if (Missile_track_loop.isValid ()) {
            ...
            if (wip->hud_locked_snd.isValid ()) {
                Missile_lock_loop =
                    snd_play (gamesnd_get_game_sound (wip->hud_locked_snd));
            }
            else {
                Missile_lock_loop = snd_play (gamesnd_get_game_sound (
                    ship_get_sound (Player_obj, GameSounds::MISSILE_LOCK)));
            }
        }
    }
    else {
        Player_ai->ai_flags.set (
            AI::AI_Flags::Seek_lock); // set this flag so multiplayer's
                                      // properly track lock on other ships
        if (Missile_lock_loop.isValid () &&
            snd_is_playing (Missile_lock_loop)) {
...
// hud_draw_lock_triangles() will draw the 4 rotating triangles around a lock
// indicator (This is done when a lock has been acquired)
#define ROTATE_DELAY 40
void HudGaugeLock::renderLockTrianglesOld (
...
void HudGaugeLock::renderLockTriangles (
    int center_x, int center_y, float frametime) {
    if (Lock_anim.first_frame == -1) {
        ...
        if (Lock_anim.time_elapsed < Lock_anim.total_time) {
            ...
            if ((Lock_gauge_draw_stamp < 0) ||
                timestamp_elapsed (Lock_gauge_draw_stamp)) {
                // reset timestamp
                Lock_gauge_draw_stamp =
                    timestamp (1000 / (2 * LOCK_GAUGE_BLINK_RATE));
...
// hud_calculate_lock_position()  will determine where on the screen to draw
// the lock indicator, and will determine when a lock has occurred.  If the
// lock indicator is not on the screen yet, hud_calculate_lock_start_pos() is
// called to pick a starting location
    ...
    if (Player->target_in_lock_cone) {
        ...
        if (catching_up) {
            ...
            if ((Players[Player_num].lock_dist_to_target -
                ...
                catching_up = 1;
                catch_up_distance =
                    last_dist_to_target + wip->catchup_pixel_penalty;
            }
        }
        ...
        if (Players[Player_num].lock_time_to_target > 0) {
            lock_pixels_per_sec = Players[Player_num].lock_dist_to_target /
                                  Players[Player_num].lock_time_to_target;
        }
        else {
        ...
        if (catching_up) {
            pixels_moved_while_locking =
                wip->catchup_pixels_per_sec * frametime;
        }
        else {
        ...
        if ((delta_x != 0) && (hypotenuse != 0)) {
            accumulated_x_pixels +=
                pixels_moved_while_locking * delta_x / hypotenuse;
        }
        ...
        if (!Missile_track_loop.isValid ()) {
            if (wip->hud_tracking_snd.isValid ()) {
                Missile_track_loop = snd_play_looping (
                    gamesnd_get_game_sound (wip->hud_tracking_snd), 0.0f, -1,
                    -1);
            }
            else {
                Missile_track_loop = snd_play_looping (
                    gamesnd_get_game_sound (ship_get_sound (
                        Player_obj, GameSounds::MISSILE_TRACKING)),
                    0.0f, -1, -1);
            }
        }

        if (!Players[Player_num].lock_time_to_target) {
            if ((Players[Player_num].lock_indicator_x ==
                 Player->current_target_sx) &&
                (Players[Player_num].lock_indicator_y ==
                 Player->current_target_sy)) {
        ...
        delta_x = Players[Player_num].lock_indicator_x -
                  Players[Player_num].lock_indicator_start_x;
        delta_y = Players[Player_num].lock_indicator_y -
                  Players[Player_num].lock_indicator_start_y;
        ...
        if ((delta_x != 0) && (hypotenuse != 0))
            accumulated_x_pixels +=
                pixels_moved_while_degrading * delta_x / hypotenuse;
...
// hud_calculate_lock_start_pos() will determine where to draw the starting
// location of the lock indicator.  It does this by picking a location that is
// Lock_start_dist pixels away from the current target (in 2D).  This is
// accomplished by finding the endpoint of a line that passes through the
// origin, and connects the target and lock indicator postion (and has a
// magnitude of Lock_start_dist)
void hud_calculate_lock_start_pos () {
    ...
    CLAMP (
        ...
        gr_screen.clip_right);
    CLAMP (
        Players[Player_num].lock_indicator_start_y, gr_screen.clip_top,
        gr_screen.clip_bottom);
}

// hud_stop_looped_locking_sounds() will terminate any hud related looping
...
void hud_lock_update_lock_pos (object* target_objp) {
    ...
    if (Player_ai->targeted_subsys &&
        ...
        get_subsystem_world_pos (
            target_objp, Player_ai->targeted_subsys, &lock_world_pos);
        return;
    }
...
void hud_lock_get_new_lock_pos (object* target_objp) {
    ...
    if ((target_shipp) &&
        (Ship_info[target_shipp->ship_info_index].is_big_or_huge ()) &&
        !(wip->wi_flags[Weapon::Info_Flags::Homing_javelin])) {
        ...
        Player->locking_subsys_parent = -1;
        lock_in_range =
            hud_lock_world_pos_in_range (&lock_world_pos, &vec_to_lock);
        vm_vec_normalize (&vec_to_lock);
        if (lock_in_range) {
            best_lock_dot =
                vm_vec_dot (&Player_obj->orient.vec.fvec, &vec_to_lock);
        }
        // take center if reasonable dot
        if (best_lock_dot > 0.95) { return; }
        ...
        while (ss != END_OF_LIST (&target_shipp->subsys_list)) {
            ...
            if (hud_lock_world_pos_in_range (
                    &subsys_world_pos, &vec_to_lock)) {
                vm_vec_normalize (&vec_to_lock);
                lock_dot =
                    vm_vec_dot (&Player_obj->orient.vec.fvec, &vec_to_lock);
        ...
        (wip->wi_flags[Weapon::Info_Flags::Homing_javelin])) {
        Player->locking_subsys = ship_get_closest_subsys_in_sight (
            target_shipp, SUBSYSTEM_ENGINE, &Player_obj->pos);
        if (Player->locking_subsys != NULL) {
            get_subsystem_world_pos (
                target_objp, Player->locking_subsys, &lock_world_pos);
            Player->locking_on_center = 0;
...
void hud_lock_determine_lock_point (vec3d* lock_world_pos_out) {
    ...
    if (Player_ai->targeted_subsys &&
        ...
        (wip->wi_flags[Weapon::Info_Flags::Homing_javelin]) &&
        (target_objp->type == OBJ_SHIP)) {
        if (!Player->locking_subsys ||
            Player->locking_subsys->system_info->type != SUBSYSTEM_ENGINE) {
            Player->locking_subsys = ship_get_closest_subsys_in_sight (
                &Ships[target_objp->instance], SUBSYSTEM_ENGINE,
                &Player_obj->pos);
        }
        if (Player->locking_subsys != NULL) {
            get_subsystem_world_pos (
    ...
    *lock_world_pos_out = lock_world_pos;
    ...
    if (lock_local_pos.xyz.z > 0.0f) {
        // Get the location of our target in the "virtual frame" where the
        // locking computation will be done
        float w = 1.0f / lock_local_pos.xyz.z;
        // Let's force our "virtual frame" to be 640x480. -MageKing17
        float sx = gr_screen.clip_center_x +
                   (lock_local_pos.xyz.x * VIRTUAL_FRAME_HALF_WIDTH * w);
        float sy = gr_screen.clip_center_y -
                   (lock_local_pos.xyz.y * VIRTUAL_FRAME_HALF_HEIGHT * w);
