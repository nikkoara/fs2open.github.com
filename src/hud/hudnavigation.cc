...
void hud_draw_navigation () {
    if (CurrentNav != -1 && Navs[CurrentNav].flags & NP_VALIDTYPE &&
        !(Navs[CurrentNav].flags & NP_NOSELECT)) {
        int in_cockpit;
        if (!(Viewer_mode &
        ...
        vertex target_point; // temp vertex used to find screen position for
                             // 3-D object;
        ...
        if (in_cockpit)
            hud_target_add_display_list (
                NULL, &target_point, target_pos, 0, &NavColor,
                Navs[CurrentNav].m_NavName, TARGET_DISPLAY_DIST);
    }
}
