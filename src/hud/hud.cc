...
#include "hud/hudnavigation.hh" //kazan
...
color HUD_color_defaults[HUD_NUM_COLOR_LEVELS]; // array of colors with
...
// the offset of the player's view vector and the ship forward vector in pixels
// (Swifty)
int HUD_nose_x;
int HUD_nose_y;
...
static int
    Hud_last_can_target; // whether Player is able to target in the last frame
static int
    Hud_can_target_timer; // timestamp to allow target gauge to draw static
                          // once targeting functions are not allowed
...
int Multi_msg_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                  5, 150 },
                                                { // GR_1024
                                                  8, 240 } };

// multiplayer voice stuff
int Voice_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                              5, 165 },
                                            { // GR_1024
                                              8, 255 } };

// ping text coords
int Ping_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                             560, 3 },
                                           { // GR_1024
                                             896, 5 } };

// supernova coords
int Supernova_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                  100, 100 },
                                                { // GR_1024
                                                  170, 170 } };
...
static int
    Hud_support_view_abort; // active when we need to display abort message
...
void HUD_init_hud_color_array () {
    ...
    for (i = 0; i < HUD_NUM_COLOR_LEVELS; i++) {
        gr_init_alphacolor (
            &HUD_color_defaults[i], HUD_color_red, HUD_color_green,
            HUD_color_blue, (i + 1) * 16);
    }
}

// HUD_init will call all the various HUD gauge init functions.  This function
// is called at the start of each mission (level)
...
// The following global data is used to determine if we should change the
// engine sound. We only check if the throttle has changed every
// THROTTLE_SOUND_CHECK_INTERVAL ms, and then we make sure that the throttle
// has actually changed.  If it has changed, we start a new sound and/or adjust
// the volume.  This occurs in update_throttle_sound()
//
...
HudGauge::HudGauge ()
    : base_w (0), base_h (0), gauge_config (-1), font_num (font::FONT1),
      lock_color (false), sexp_lock_color (false), reticle_follow (false),
      active (false), off_by_default (false), sexp_override (false),
      pop_up (false), disabled_views (0), custom_gauge (false),
    ...
    gr_init_alphacolor (
        &gauge_color, 255, 255, 255, (HUD_color_alpha + 1) * 16);
...
HudGauge::HudGauge (
    int _gauge_object, int _gauge_config, bool _slew, bool _message,
    int _disabled_views, int r, int g, int b)
    : base_w (0), base_h (0), gauge_config (_gauge_config),
      gauge_object (_gauge_object), font_num (font::FONT1), lock_color (false),
      sexp_lock_color (false), reticle_follow (_slew), active (false),
      off_by_default (false), sexp_override (false), pop_up (false),
      message_gauge (_message), disabled_views (_disabled_views),
...
HudGauge::HudGauge (
    int _gauge_config, bool _slew, int r, int g, int b, char* _custom_name,
    char* _custom_text, char* frame_fname, int txtoffset_x, int txtoffset_y)
    : base_w (0), base_h (0), gauge_config (_gauge_config),
      gauge_object (HUD_OBJECT_CUSTOM), font_num (font::FONT1),
      lock_color (false), sexp_lock_color (false), reticle_follow (_slew),
      active (false), off_by_default (false), sexp_override (false),
      pop_up (false), message_gauge (false),
      disabled_views (
          VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY),
      custom_gauge (true), textoffset_x (txtoffset_x),
      textoffset_y (txtoffset_y), texture_target (-1), canvas_w (-1),
      canvas_h (-1), target_w (-1), target_h (-1) {
    position[0] = 0;
    position[1] = 0;
    ...
    if (frame_fname) {
        ...
        if (custom_frame.first_frame < 0) {
            WARNINGF (LOCATION, "Cannot load hud ani: %s", frame_fname);
        }
    }
}
...
void HudGauge::initSlew (bool slew) { reticle_follow = slew; }

void HudGauge::initFont (int input_font_num) {
    if (input_font_num >= 0 &&
        input_font_num < font::FontManager::numberOfFonts ()) {
        font_num = input_font_num;
    }
}

const char* HudGauge::getCustomGaugeName () { return custom_name; }

const char* HudGauge::getCustomGaugeText () { return custom_text.c_str (); }

void HudGauge::updateCustomGaugeCoords (int _x, int _y) {
    if (!custom_gauge) { return; }
...
void HudGauge::updateCustomGaugeFrame (int frame_offset) {
    ...
    if (frame_offset < 0 || frame_offset > custom_frame.num_frames) { return; }
...
void HudGauge::setFont () { font::set_font (font_num); }
...
void HudGauge::setGaugeColor (int bright_index) {
    ...
    if (bright_index != HUD_C_NONE) {
        ...
        case HUD_C_DIM:
            ...
            gr_init_alphacolor (
                &gauge_color, gauge_color.red, gauge_color.green,
                gauge_color.blue, alpha);
            break;

        case HUD_C_NORMAL:
            alpha =
                HUD_contrast ? HUD_NEW_ALPHA_NORMAL_HI : HUD_NEW_ALPHA_NORMAL;
            gr_init_alphacolor (
                &gauge_color, gauge_color.red, gauge_color.green,
                gauge_color.blue, alpha);
            break;
        ...
        case HUD_C_BRIGHT:
            alpha =
                HUD_contrast ? HUD_NEW_ALPHA_BRIGHT_HI : HUD_NEW_ALPHA_BRIGHT;
            gr_init_alphacolor (
                &gauge_color, gauge_color.red, gauge_color.green,
                gauge_color.blue, alpha);
            break;
...
int HudGauge::getObjectType () { return gauge_object; }

void HudGauge::lockConfigColor (bool lock) { lock_color = lock; }

void HudGauge::sexpLockConfigColor (bool lock) { sexp_lock_color = lock; }

void HudGauge::updateColor (int r, int g, int b, int a) {
    if (sexp_lock_color || lock_color) return;

    gr_init_alphacolor (&gauge_color, r, g, b, a);
}

void HudGauge::updateActive (bool show) { active = show; }

void HudGauge::initRenderStatus (bool do_render) {
    off_by_default = !do_render;
}

bool HudGauge::isOffbyDefault () { return off_by_default; }

bool HudGauge::isActive () { return active && !sexp_override; }

void HudGauge::updateSexpOverride (bool sexp) { sexp_override = sexp; }

void HudGauge::updatePopUp (bool pop_up_flag) { pop_up = pop_up_flag; }
...
int HudGauge::popUpActive () {
    ...
    if (!timestamp_elapsed (popup_timer)) { return 1; }
    else {
        return 0;
    }
}
...
bool HudGauge::flashExpiredSexp () {
    if (timestamp_elapsed (flash_duration)) { return true; }
...
void HudGauge::render (float /*frametime*/) {
    ...
    if (!custom_text.empty ()) {
        ...
        renderString (
            position[0] + textoffset_x, position[1] + textoffset_y, text);
    ...
    if (custom_frame.first_frame > -1) {
        renderBitmap (
            custom_frame.first_frame + custom_frame_offset, position[0],
            position[1]);
    }
}
...
void HudGauge::renderString (int x, int y, const char* str) {
    ...
    if (gr_screen.rendering_to_texture != -1) {
        gr_set_screen_scale (
            canvas_w, canvas_h, -1, -1, target_w, target_h, target_w, target_h,
            true);
    }
    else {
        if (reticle_follow) {
...
void HudGauge::renderStringAlignCenter (
    int x, int y, int area_width, const char* s) {
    int w, h;
...
void HudGauge::renderBitmapEx (
    int frame, int x, int y, int w, int h, int sx, int sy) {
    int nx = 0, ny = 0;
...
void HudGauge::pageIn () {
    if (custom_gauge) {
        if (custom_frame.first_frame > -1 && custom_frame.num_frames > 0) {
            bm_page_in_aabitmap (
                custom_frame.first_frame, custom_frame.num_frames);
        }
    }
}
...
bool HudGauge::canRender () {
    if (sexp_override) { return false; }

    if (hud_disabled_except_messages () && !message_gauge) { return false; }

    if (hud_disabled () && !hud_disabled_except_messages ()) { return false; }
...
void HudGauge::initCockpitTarget (
    const char* display_name, int _target_x, int _target_y, int _target_w,
    int _target_h, int _canvas_w, int _canvas_h) {
    if (strlen (display_name) <= 0) { return; }
...
bool HudGauge::setupRenderCanvas (int render_target) {
    if (texture_target_fname[0] != '\0') {
        ...
        if (render_target < 0) { return true; }
    }
...
void HudGauge::setCockpitTarget (const cockpit_display* display) {
    ...
    if (strcmp (texture_target_fname, display->name) != 0) { return; }
...
void HudGauge::resetCockpitTarget () { texture_target = -1; }
...
void hud_toggle_draw () { HUD_draw = !HUD_draw; }

// Goober5000
void hud_set_draw (int draw) { HUD_draw = draw; }

// WMC
int hud_get_draw () { return HUD_draw; }
 ...
 */
int hud_disabled_except_messages () { return HUD_disable_except_messages; }

/**
 * @brief Checks if HUD disabled
 * @return !0 if HUD is disabled (ie no gauges are shown/usable), otherwise
 * return 0
 */
int hud_disabled () { return !HUD_draw; }
    ...
    if (hud_gauge_is_popup (HUD_WEAPONS_GAUGE)) {
        ...
        for (i = 0; i < swp->num_secondary_banks; i++) {
            if (swp->secondary_bank_ammo[i] > 0) {
                int ms_till_fire =
                    timestamp_until (swp->next_secondary_fire_stamp[i]);
 ...
 *
 * @details This function updates those parts of the HUD that are not dependant
 * on the rendering of the HUD.
 */
void hud_update_frame (float /*frametime*/) {
    ...
    // we will intercept player targeting requests in hud_sensors_ok() when
    // checking key commands DB - 8/24/98
    can_target = hud_sensors_ok (Player_ship, 0);
    if (emp_active_local ()) { can_target = 1; }
    ...
    // if there is no target, check if auto-targeting is enabled, and select
    // new target
    int retarget = 0;
    int retarget_turret = 0;

    if (Player_ai->target_objnum == -1) { retarget = 1; }
    ...
    // check if big ship and currently selected subsys is turret and turret is
    // dead only do this is not retargeting
    if ((!retarget) && (Player_ai->target_objnum != -1)) {
        if (Objects[Player_ai->target_objnum].type == OBJ_SHIP) {
            if (!(Ships[Objects[Player_ai->target_objnum].instance]
                      .flags[Ship::Ship_Flags::Dying])) {
                if (Ship_info[Ships[Objects[Player_ai->target_objnum].instance]
                                  .ship_info_index]
                        .is_big_or_huge ()) {
    ...
    // check to see if we are in messaging mode.  If so, send the key to the
    // code to deal with the message.  hud_sqaudmsg_do_frame will return 0 if
    // the key wasn't used in messaging mode, otherwise 1.  In the event the
    // key was used, return immediately out of this function.
    ...
    if (Player_ai->targeted_subsys != NULL) {
        ...
        Player_ai->current_target_distance =
            vm_vec_dist_quick (&target_pos, &Player_obj->pos);

        float shield_strength = Player_ai->targeted_subsys->current_hits /
                                Player_ai->targeted_subsys->max_hits * 100.0f;
        int screen_integrity = (int)std::lround (shield_strength);

        if (screen_integrity < 0) { screen_integrity = 0; }

        if (screen_integrity == 0) {
            if (shield_strength > 0) { screen_integrity = 1; }
        }
        ...
        if (ship_subsys_takes_damage (Player_ai->targeted_subsys)) {
            if (screen_integrity <= 0) {
                hud_targetbox_start_flash (
                    TBOX_FLASH_SUBSYS); // need to flash 0% continuously
            }
        }
    }
    else {
        ...
        Player_ai->current_target_distance =
            hud_find_target_distance (targetp, Player_obj);
    }
    ...
    if (targetp->type == OBJ_SHIP) {
        ...
        if (target_shipp->flags[Ship::Ship_Flags::Depart_warp] ||
            target_shipp->flags[Ship::Ship_Flags::Exploded]) {
            stop_targetting_this_thing = 1;
        }
    }

    // Check if can still be seen in Nebula
    if (hud_target_invalid_awacs (targetp)) { stop_targetting_this_thing = 1; }
    ...
    if (targetp->type == OBJ_SHIP && Event_Music_battle_started == 0) {
        ...
        if (iff_x_attacks_y (target_shipp->team, Player_ship->team)) {
            ...
            dist_to_target =
                vm_vec_dist_quick (&targetp->pos, &Player_obj->pos);
    ...
    if (Player_ai->target_objnum >= 0 &&
        ...
        if (Weapons[Objects[Player_ai->target_objnum].instance].lssm_stage ==
            3) {
    ...
    // Since we need to reference the player's target integrity in several
    // places this upcoming frame, only calculate once here
    if (target_shipp) { Pl_target_integrity = get_hull_pct (targetp); }
...
/**
 * @brief Draw white brackets around asteroids which has the AF_DRAW_BRACKETS
 * flag set
 */
void hud_show_asteroid_brackets () {
    if (hud_sensors_ok (Player_ship, 0)) { asteroid_show_brackets (); }
}

/**
 * @brief Render gauges that need to be between a ::g3_start_frame() and a
 * ::g3_end_frame()
 */
void hud_render_preprocess (float frametime) {
    ...
    if (Viewer_mode & (VM_DEAD_VIEW)) { return; }
    ...
    if (hud_disabled_except_messages ()) { return; }
...
HudGaugeMissionTime::HudGaugeMissionTime ()
    : HudGauge (
          HUD_OBJECT_MISSION_TIME, HUD_MISSION_TIME, false, false,
          (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
           VM_OTHER_SHIP),
          255, 255, 255) {}
...
void HudGaugeMissionTime::render (float /*frametime*/) {
    ...
    renderPrintf (
        position[0] + time_text_offsets[0], position[1] + time_text_offsets[1],
    ...
    if (time_comp < 1) {
        renderPrintf (
            position[0] + time_val_offsets[0],
            position[1] + time_val_offsets[1],
            /*XSTR( "x%.1f", 215), time_comp)*/ NOX ("%.2f"), time_comp);
    }
    else {
        renderPrintf (
            position[0] + time_val_offsets[0],
            position[1] + time_val_offsets[1], XSTR ("x%.2f", 216), time_comp);
    }
}
...
void hud_maybe_display_supernova () {
    ...
    if (Lcl_pl) {
        gr_printf (
            Supernova_coords[gr_screen.res][0],
            Supernova_coords[gr_screen.res][1], "Wybuch supernowej: %.2f s",
            time_left);
    }
    else {
        gr_printf (
            Supernova_coords[gr_screen.res][0],
            Supernova_coords[gr_screen.res][1], "Supernova Warning: %.2f s",
            time_left);
    }
}
...
void hud_render_gauges (int cockpit_display_num) {
    ...
    if (cockpit_display_num >= 0) {
        ...
        render_target =
            ship_start_render_cockpit_display (cockpit_display_num);

        if (render_target <= 0) { return; }
    }
    else {
        if (supernova_active () >= 3) { return; }
    }
    ...
    if (sip->hud_enabled) {
        ...
        for (j = 0; j < num_gauges; j++) {
            ...
            if (cockpit_display_num < 0) { sip->hud_gauges[j]->preprocess (); }
            ...
            if (!sip->hud_gauges[j]->canRender ()) { continue; }
        ...
        for (j = 0; j < num_gauges; j++) {
            ...
            if (!default_hud_gauges[j]->canRender ()) { continue; }
 ...
 *
 * @details This function will set the loop id's for the engine noises to -1,
 * this will force any looping engine sounds to stop.
 */
 ...
 *
 * @details Determine what engine sound to play, based upon the percentage
 * throttle set. If we're a multiplayer observer, stop any engine sounds from
 * playing and return.
 */
void update_throttle_sound () {
    ...
    if (timestamp_elapsed (throttle_sound_check_id)) {
        ...
        if (object_get_gliding (Player_obj)) { // Backslash
            ...
            percent_throttle =
                ...
                Ship_info[Ships[Player_obj->instance].ship_info_index]
                    .max_speed;
        }
        else {
            percent_throttle = Player_obj->phys_info.fspeed /
                               Ships[Player_obj->instance].current_max_speed;
        }

        if (percent_throttle != last_percent_throttle ||
            !Player_engine_snd_loop.isValid ()) {
            if (percent_throttle < ZERO_PERCENT) {
                if (Player_engine_snd_loop.isValid ()) {
                    snd_stop (
                        Player_engine_snd_loop); // Backslash - otherwise, long
                                                 // engine loops keep playing
                    Player_engine_snd_loop = sound_handle::invalid ();
                }
            }
            else {
                if (!Player_engine_snd_loop.isValid ()) {
                    Player_engine_snd_loop = snd_play_looping (
                        gamesnd_get_game_sound (
                            ship_get_sound (Player_obj, GameSounds::ENGINE)),
                        0.0f, -1, -1, percent_throttle * ENGINE_MAX_VOL,
                        FALSE);
                }
                else {
                    // The sound may have been trashed at the low-level if
                    // sound channel overflow.
                    // TODO: implement system where certain sounds cannot be
                    // interrupted (priority?)
                    if (snd_is_playing (Player_engine_snd_loop)) {
                        snd_set_volume (
                            Player_engine_snd_loop,
                            percent_throttle * ENGINE_MAX_VOL);
                    }
                    else {
                        Player_engine_snd_loop = sound_handle::invalid ();
                    }
...
/**
 * @brief Called at the beginning of each level. Loads frame data in once, and
 * initializes any damage gauge specific data.
 */
void hud_damage_popup_init () {
    int i;
...
HudGaugeDamage::HudGaugeDamage ()
    : HudGauge (
          HUD_OBJECT_DAMAGE, HUD_DAMAGE_GAUGE, false, false,
          (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
           VM_OTHER_SHIP),
          255, 255, 255) {}
...
void HudGaugeDamage::initLineHeight (int h) { line_h = h; }

void HudGaugeDamage::initBitmaps (
    const char* fname_top, const char* fname_middle,
...
void HudGaugeDamage::render (float /*frametime*/) {
    ...
    if (damage_top.first_frame == -1) { return; }

    if ((The_mission.game_type & MISSION_TYPE_TRAINING) &&
        Training_message_visible) {
        return;
    }
    ...
    num = 0;
    for (pss = GET_FIRST (&Player_ship->subsys_list);
         pss != END_OF_LIST (&Player_ship->subsys_list);
         pss = GET_NEXT (pss)) {
        ...
        if (strength < 1) {
            ...
            if (strength < Pl_hud_subsys_info[psub->type].last_str) {
                Pl_hud_subsys_info[psub->type].flash_duration_timestamp =
                    timestamp (SUBSYS_DAMAGE_FLASH_DURATION);
            }
            Pl_hud_subsys_info[psub->type].last_str = strength;

            // Don't display more than the max number of damaged subsystems.
            if (num >= SUBSYSTEM_MAX) { break; }
        }
    }

    // Build a list of damage values to display and then actually display them
    // in a second pass This allows to hide the gauge when there is no damage
    ...
    for (int i = 0; i < num; i++) {
        ...
        if (!timestamp_elapsed (
                Pl_hud_subsys_info[type].flash_duration_timestamp)) {
            ...
            if (flash_status) {
                int alpha_color = MIN (
                    HUD_COLOR_ALPHA_MAX, HUD_color_alpha + HUD_BRIGHT_DELTA);
    ...
    // The third case is there to make the gauge appear only if needed if the
    // right option is set
    if (screen_integrity < 100 || !info_lines.empty ()) {
        ...
        if (screen_integrity == 0) { screen_integrity = 1; }
        ...
        if (screen_integrity < 30) { info.color_override = &Color_red; }
    ...
    // These variables keep track of where the background was drawn last so we
    // can draw the bottom correctly
    int last_bx = position[0];
    int last_by = position[1] + middle_frame_start_offset_y;
    for (auto& line : info_lines) {
        if (line.draw_background) {
            renderBitmap (
                damage_middle.first_frame, line.background_x,
                line.background_y);
            last_bx = line.background_x;
            last_by =
                line.background_y + line_h; // Add line_h here so that the
    ...
    setGaugeColor ();
    renderBitmap (
        damage_bottom.first_frame, last_bx, last_by + bottom_bg_offset);
}
...
int hud_anim_load (hud_anim* ha) {
    ...
    if (ha->first_frame == -1) {
        WARNINGF (LOCATION, "Couldn't load hud animation for file '%s'",ha->filename);
        return -1;
    }
...
/**
 * @brief Render out a frame of a hud or briefing animation, based on how much
 * time has elapsed
 ...
 */
int hud_anim_render (
    hud_anim* ha, float frametime, int draw_alpha, int loop, int hold_last,
    int reverse, int resize_mode, bool mirror) {
    int framenum;

    if (ha->num_frames <= 0) {
        if (hud_anim_load (ha) == -1) return 0;
    }
    ...
    // Note; total_time for hud_anim is derived only from the fps, no need to
    // pass it in
    framenum =
        bm_get_anim_frame (ha->first_frame, ha->time_elapsed, 0.0f, loop != 0);
    if (reverse) { framenum = (ha->num_frames - 1) - framenum; }

    // Blit the bitmap for this frame
    if (emp_should_blit_gauge ()) {
        gr_set_bitmap (ha->first_frame + framenum);
        if (draw_alpha) { gr_aabitmap (ha->sx, ha->sy, resize_mode, mirror); }
        else {
...
void hud_num_make_mono (char* num_str, int font_num) {
    ...
    for (size_t i = 0; i < len; i++) {
        if (num_str[i] == '1') { num_str[i] = (char)(sc + 1); }
    }
}
...
HudGaugeTextWarnings::HudGaugeTextWarnings ()
    : HudGauge (
          HUD_OBJECT_TEXT_WARNINGS, HUD_TEXT_FLASH, true, false,
          (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
           VM_OTHER_SHIP),
          255, 255, 255) {}
...
void HudGaugeTextWarnings::render (float /*frametime*/) {
    ...
    if (maybeTextFlash ()) {
        ...
        // draw the box
        renderRect (
            (int)((float)position[0] - (float)w / 2.0f - 1.0f),
            (int)((float)position[1] - 1.0f), w + 2, h + 1);
    }
...
HudGaugeKills::HudGaugeKills ()
    : HudGauge (
          HUD_OBJECT_KILLS, HUD_KILLS_GAUGE, false, false,
          (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
           VM_OTHER_SHIP),
          255, 255, 255) {}
...
void HudGaugeKills::initBitmaps (const char* fname) {
    ...
    if (Kills_gauge.first_frame == -1) {
        WARNINGF (LOCATION, "Could not load in the ani: %s", fname);
    }
}
...
void HudGaugeKills::render (float /*frametime*/) {
    if (Kills_gauge.first_frame < 0) { return; }
    ...
    renderString (
        position[0] + text_offsets[0], position[1] + text_offsets[1],
        XSTR ("kills:", 223));
    ...
    renderString (
        position[0] + text_value_offsets[0] - w,
        position[1] + text_value_offsets[1], num_kills_string);
}

HudGaugeLag::HudGaugeLag ()
    : HudGauge (
          HUD_OBJECT_LAG, HUD_LAG_GAUGE, false, false,
          (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
           VM_OTHER_SHIP),
          255, 255, 255) {}

void HudGaugeLag::initBitmaps (const char* fname) {
    Netlag_icon.first_frame =
        bm_load_animation (fname, &Netlag_icon.num_frames);

    if (Netlag_icon.first_frame == -1) {
        WARNINGF (LOCATION, "Could not load in the netlag ani: %s", fname);
    }
}
...
/**
 * @brief Called at mission start to init data, and load support view bitmap if
 * required
 */
void hud_support_view_init () {
...
/**
 * @brief Start displaying the support view pop-up. This will remain up until
 * ::hud_support_view_stop() is called.
 */
...
/**
 * @brief Get the number of seconds until repair ship will dock with ther
 * player
 * @details mwa made this function more general purpose. Goober5000 made
 * clearer
 *
 ...
 *
 * @todo This function is pretty stupid now.  It just assumes the dockee is
 * sitting still, and the docker is moving directly to the dockee.
 */
int hud_get_dock_time (object* docker_objp) {
    ...
    if (aip->goal_objnum == -1) {
        ...
        WARNINGF (LOCATION,"'aip->goal_objnum == -1' in hud_get_dock_time(), line %i",__LINE__);
        return 0;
    }
    ...
    vm_vec_sub (
        &rel_vel, &docker_objp->phys_info.vel, &dockee_objp->phys_info.vel);
    ...
    if (rel_speed <= docker_speed / 2.0f) { // This means the player is moving
                                            // away fast from the support ship.
        ...
        if (rel_speed < 20.0f) rel_speed = 20.0f;

        // When far away, use max speed, not current speed.  Might not have
        // sped up yet.
        if (d > 100.0f) {
 ...
 * @return Number of support ship, -1 if there is no support ship currently
 * trying to dock
 */
int hud_support_find_closest (int objnum) {
    ...
    while (sop != END_OF_LIST (&Ship_obj_list)) {
        if (Ship_info[Ships[Objects[sop->objnum].instance].ship_info_index]
            ...
            if (!(shipp->flags[Ship::Ship_Flags::Dying] ||
                ...
                // we must check all goals for this support ship -- not just
                // the first one
...
HudGaugeSupport::HudGaugeSupport ()
    : HudGauge (
          HUD_OBJECT_SUPPORT, HUD_SUPPORT_GAUGE, false, false,
          (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
           VM_OTHER_SHIP),
          255, 255, 255) {}
...
void HudGaugeSupport::initTextValueOffsetY (int y) { text_val_offset_y = y; }

void HudGaugeSupport::initTextDockOffsetX (int x) { text_dock_offset_x = x; }
...
void HudGaugeSupport::render (float /*frametime*/) {
        ...
        if (!timestamp_elapsed (Hud_support_view_fade)) {
            if (Hud_support_view_abort) {
                renderStringAlignCenter (
                    position[0], position[1] + text_val_offset_y, w,
                    XSTR ("aborted", 225));
            }
            else {
                renderStringAlignCenter (
    ...
    if (Player_ai->ai_flags[AI::AI_Flags::Being_repaired]) {
        ...
        if (Hud_support_objnum == -1) {
            ...
            renderString (
                position[0] + text_dock_offset_x,
                position[1] + text_val_offset_y, outstr);
        }
    }
    ...
    if (show_time) {
        ...
        renderPrintf (
            position[0] + text_dock_val_offset_x,
            position[1] + text_val_offset_y, NOX ("%02d:%02d"), minutes,
            seconds);
    }
}
...
void hud_set_bright_color () {
    int alpha_color;
    alpha_color =
        MIN (HUD_COLOR_ALPHA_MAX, HUD_color_alpha + HUD_BRIGHT_DELTA);
...
void hud_set_dim_color () {
    if (HUD_color_alpha > 2) { gr_set_color_fast (&HUD_color_defaults[2]); }
}
    ...
    if (ship_is_tagged (objp)) {
        ...
        if (objp->type == OBJ_ASTEROID) {
            if (OBJ_INDEX (objp) == Player_ai->target_objnum) {
                use_color = iff_get_color_by_team (
                    Iff_traitor, Player_ship->team, is_bright);
            }
            else {
            ...
            use_color = iff_get_color_by_team_and_object (
                obj_team (objp), Player_ship->team, is_bright, objp);
        }
    }
...
static int Vm_other_ship_gauges[NUM_VM_OTHER_SHIP_GAUGES] = {
    HUD_CENTER_RETICLE, HUD_TARGET_MONITOR, HUD_TARGET_MONITOR_EXTRA_DATA,
    HUD_MESSAGE_LINES, HUD_TALKING_HEAD
};
...
int hud_gauge_active (int gauge_index) {
    ...
    if (Viewer_mode & VM_OTHER_SHIP) {
        for (int i = 0; i < NUM_VM_OTHER_SHIP_GAUGES; i++) {
            if (gauge_index == Vm_other_ship_gauges[i]) { return 1; }
        }
        return 0;
    }
 ...
 * @details Load gauge properties defined in the HUD config if gauge is not
 * customized.
 */
...
void hud_gauge_start_flash (int gauge_index) {
    ...
    HUD_gauge_flash_duration[gauge_index] =
        timestamp (HUD_GAUGE_FLASH_DURATION);
...
/**
 * @brief Set the HUD color for the gauge, based on whether it is flashing or
 * not
 */
void hud_set_gauge_color (int gauge_index, int bright_index) {
    ...
    if (bright_index != HUD_C_NONE) {
        ...
        case HUD_C_DIM:
            alpha = HUD_contrast ? HUD_NEW_ALPHA_DIM_HI : HUD_NEW_ALPHA_DIM;
            gr_init_alphacolor (
                use_color, use_color->red, use_color->green, use_color->blue,
                alpha);
            break;
 ...
 * @return Gauge is not flashing -1; gauge is flashing, draw dim 0; gauge is
    ...
    if (!timestamp_elapsed (HUD_gauge_flash_duration[gauge_index])) {
        if (timestamp_elapsed (HUD_gauge_flash_next[gauge_index])) {
            HUD_gauge_flash_next[gauge_index] =
                timestamp (HUD_GAUGE_FLASH_INTERVAL);
            HUD_gauge_bright ^=
                (1 << gauge_index); // toggle between default and bright frames
        }

        if (HUD_gauge_bright & (1 << gauge_index)) { flash_status = 1; }
        else {
            flash_status = 0;
        }
    }
...
void hud_update_objective_message () {
    ...
    if (timestamp_elapsed (Objective_display.display_timer)) {
        ...
        !hud_objective_notify_active () && !hud_subspace_notify_active ()) {
...
HudGaugeObjectiveNotify::HudGaugeObjectiveNotify ()
    : HudGauge (
          HUD_OBJECT_OBJ_NOTIFY, HUD_OBJECTIVES_NOTIFY_GAUGE, false, false,
          VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
              VM_OTHER_SHIP,
          255, 255, 255) {}
...
void HudGaugeObjectiveNotify::pageIn () {
    bm_page_in_aabitmap (
        Objective_display_gauge.first_frame,
        Objective_display_gauge.num_frames);
}
...
void HudGaugeObjectiveNotify::renderSubspace () {
    ...
    if (!hud_subspace_notify_active ()) { return; }

    if (Objective_display_gauge.first_frame < 0) { return; }

    // Blit the background
    setGaugeColor ();
    renderBitmap (
        Objective_display_gauge.first_frame, position[0], position[1]);
    ...
    renderStringAlignCenter (
        position[0], position[1] + Subspace_text_offset_y, w,
        XSTR ("subspace drive", 233));
    if (warp_aborted) {
        renderStringAlignCenter (
            position[0], position[1] + Subspace_text_val_offset_y, w,
            XSTR ("aborted", 225));
    }
    else {
        renderStringAlignCenter (
...
void HudGaugeObjectiveNotify::renderRedAlert () {
    ...
    if (!red_alert_in_progress ()) { return; }

    if (Objective_display_gauge.first_frame < 0) { return; }

    if (hud_subspace_notify_active ()) { return; }

    if (hud_objective_notify_active ()) { return; }
    ...
    startFlashNotify ();
    if (maybeFlashNotify ()) { gr_set_color_fast (&Color_red); }
    else {
        gr_set_color_fast (&Color_bright_red);
    }
    ...
    renderStringAlignCenter (
        position[0], position[1] + Red_text_offset_y, w,
        XSTR ("downloading new", 235));
    renderStringAlignCenter (
        position[0], position[1] + Red_text_val_offset_y, w,
        XSTR ("orders...", 236));
...
void HudGaugeObjectiveNotify::renderObjective () {
    ...
    if (timestamp_elapsed (Objective_display.display_timer)) { return; }
    ...
    startFlashNotify ();
    if (maybeFlashNotify ()) { setGaugeColor (HUD_C_BRIGHT); }
    else {
        setGaugeColor ();
    }
    ...
    case PRIMARY_GOAL:
        renderStringAlignCenter (
            position[0], position[1] + Objective_text_offset_y, w,
            XSTR ("primary objective", 237));
        break;
    case SECONDARY_GOAL:
        renderStringAlignCenter (
            position[0], position[1] + Objective_text_offset_y, w,
            XSTR ("secondary objective", 238));
        break;
    case BONUS_GOAL:
        renderStringAlignCenter (
            position[0], position[1] + Objective_text_offset_y, w,
            XSTR ("bonus objective", 239));
        break;
    }
    ...
    case SECONDARY_GOAL:
        ...
        case GOAL_FAILED:
            sprintf (
                buf, XSTR ("failed (%d/%d)", 240),
                Objective_display.goal_nresolved,
                Objective_display.goal_ntotal);
            renderStringAlignCenter (
                position[0], position[1] + Objective_text_val_offset_y, w,
                buf);
            break;
        default:
            sprintf (
                buf, XSTR ("complete (%d/%d)", 241),
                Objective_display.goal_nresolved,
                Objective_display.goal_ntotal);
            renderStringAlignCenter (
                position[0], position[1] + Objective_text_val_offset_y, w,
...
void hud_stop_subspace_notify () { Subspace_notify_active = 0; }

void hud_start_subspace_notify () { Subspace_notify_active = 1; }

int hud_subspace_notify_active () { return Subspace_notify_active; }

void hud_stop_objective_notify () { Objective_notify_active = 0; }
...
int hud_objective_notify_active () { return Objective_notify_active; }

/**
 * @brief Set the offset values for this render frame
 * @details Since the player's view vector may be different from the ship's
 * forward vector, we calculate the offset of those two in pixels and store the
 * x and y offsets in variables HUD_nose_x and HUD_nose_y (Swifty)
 *
...
void HUD_set_offsets (
    object* viewer_obj, int wiggedy_wack, matrix* eye_orient) {
    if ((viewer_obj == Player_obj) && wiggedy_wack) {
        ...
        if (HUD_offset_y > 100.0f) { HUD_offset_y = 100.0f; }
...
/**
 * @brief Returns the offset between the player's view vector and the forward
 * vector of the ship in pixels (Swifty)
 */
void HUD_get_nose_coordinates (int* x, int* y) {
    ...
    *y = 0;
    ...
    vm_vec_scale_add (
        &p0, &Player_obj->pos, &Player_obj->orient.vec.fvec, 10000.0f);
    ...
    if (v0.codes == 0) {
        ...
        if (!(v0.codes & PF_OVERFLOW)) {
            ...
            // Means that the ship forward vector is not going through the
            // frame buffer. We're assigning a high negative value so that the
            // the bitmaps will be drawn offscreen so that we can give the
            // illusion that the player is looking away from the slewable HUD
            // reticle.
            *x = -100000;
            *y = -100000;
            return;
        ...
        // Means that the ship forward vector is not going through the frame
        // buffer. We're assigning a high negative value so that the the
        // bitmaps will be drawn offscreen so that we can give the illusion
        // that the player is looking away from the slewable HUD reticle.
        *x = -100000;
        *y = -100000;
        return;
    }
...
void hud_toggle_contrast () { HUD_contrast = !HUD_contrast; }

void hud_set_contrast (int high) { HUD_contrast = high; }
...
void hud_page_in () {
    ...
    for (j = 0; j < num_gauges; j++) { default_hud_gauges[j]->pageIn (); }
}
...
HudGauge* hud_get_gauge (const char* name) {
    ...
    if (!Ship_info[Player_ship->ship_info_index].hud_gauges.empty ()) {
        for (j = 0;
             j < Ship_info[Player_ship->ship_info_index].hud_gauges.size ();
             j++) {
            gauge_name = Ship_info[Player_ship->ship_info_index]
                             .hud_gauges[j]
                             ->getCustomGaugeName ();
            if (!strcmp (name, gauge_name)) {
                return Ship_info[Player_ship->ship_info_index]
                    .hud_gauges[j]
                    .get ();
            }
        }
...
HudGaugeSupernova::HudGaugeSupernova ()
    : HudGauge (
          HUD_OBJECT_SUPERNOVA, HUD_DIRECTIVES_VIEW, false, false, 0, 255, 255,
          255) {}
...
void HudGaugeSupernova::render (float /*frametime*/) {
    ...
    if (Lcl_pl) {
        renderPrintf (
            position[0], position[1], "Wybuch supernowej: %.2f s", time_left);
    }
    else {
        renderPrintf (
            position[0], position[1], "Supernova Warning: %.2f s", time_left);
    }
}

HudGaugeFlightPath::HudGaugeFlightPath ()
    : HudGauge (
          HUD_OBJECT_FLIGHT_PATH, HUD_CENTER_RETICLE, false, false,
    ...
    if (v0.codes == 0) { // on screen
        ...
        if (!(v0.flags & PF_OVERFLOW)) {
            if (Marker.first_frame >= 0) {
                ...
                unsize (&sx, &sy);
                renderBitmap (
                    Marker.first_frame, sx - Marker_half[0],
                    sy - Marker_half[1]);
            }
        }
    }
