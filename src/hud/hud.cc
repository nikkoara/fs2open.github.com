          ...
          canvas_h(-1), target_w(-1), target_h(-1)
{
        ...
        if (_custom_name) {
                strcpy(custom_name, _custom_name);
        } else {
                custom_name[0] = '\0';
        }
...
void HudGaugeDamage::render(float /*frametime*/)
{
        ...
        for (auto &line : info_lines) {
                if (line.draw_background) {
                        ...
                        last_by = line.background_y + line_h; // Add line_h here so that the
...
int hud_anim_load(hud_anim *ha)
{
        ...
        if ((ha->first_frame == -1) && !strcasecmp(ha->filename, "FadeIconS-FreighterCW")) {
...
void hud_start_text_flash(const char *txt, int t, int interval)
{
        ...
        if (!timestamp_elapsed(Hud_text_flash_timer))
                if (!strcasecmp(Hud_text_flash, NOX("Emp")) || !strcasecmp(txt, NOX("Launch")))
                        return;
...
void hud_update_objective_message()
{
        ...
        if ((Player->control_mode == PCM_WARPOUT_STAGE1) || (Player->control_mode == PCM_WARPOUT_STAGE2) || (Player->control_mode == PCM_WARPOUT_STAGE3) || (Sexp_hud_display_warpout > 0)) {
        ...
        object *viewer_obj, int wiggedy_wack, matrix *eye_orient)
{
        if ((viewer_obj == Player_obj) && wiggedy_wack) {
                ...
                HUD_offset_x -= 0.45f * (float(gr_screen.clip_width_unscaled) * 0.5f - pt.screen.xyw.x);
                HUD_offset_y -= 0.45f * (float(gr_screen.clip_height_unscaled) * 0.5f - pt.screen.xyw.y);
