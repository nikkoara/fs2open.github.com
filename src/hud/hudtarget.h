...
#ifndef FREESPACE2_HUD_HUDTARGET_H
#define FREESPACE2_HUD_HUDTARGET_H
...
const int CARGO_RADIUS_REAL_DELTA =
    50; // difference in distances used for cargo scanning of subsystems (which
        // also uses object radius), vs whole-ship scanning
#define CARGO_REVEAL_MIN_DIST \
    150 // minimum distance for reveal cargo (used if radius+CARGO_RADIUS_DELTA
        // < CARGO_REVEAL_MIN_DIST)
#define CAP_CARGO_REVEAL_MIN_DIST \
    300 // minimum distance for reveal cargo (used if radius+CARGO_RADIUS_DELTA
        // < CARGO_REVEAL_MIN_DIST)
...
#define MAX_HOTKEY_TARGET_ITEMS \
    150 // maximum number of ships that can be targeted on *all* keys
#define SELECTION_SET \
    0x5000 // variable used for drawing brackets.  The bracketing code uses
...
int hud_target_closest (
    int team_mask = -1, int attacked_objnum = -1, int play_fail_sound = TRUE,
    int filter = 0, int turret_attacking_target = 0);
...
void hud_target_next_list (
    int hostile = 1, int next_flag = 1, int team_mask = -1,
    int attacked_objnum = -1, int play_fail_sound = TRUE, int filter = 0,
...
void hud_target_live_turret (
    int next_flag, int auto_advance = 0, int turret_attacking_target = 0);
...
int subsystem_in_sight (
    object* objp, ship_subsys* subsys, vec3d* eye, vec3d* subsystem);
...
extern void polish_predicted_target_pos (
...
void hud_calculate_lead_pos (
    ...
    weapon_info* wip, float dist_to_target, vec3d* rel_pos = NULL);
...
protected:
    hud_frames
        primary_top[NUM_HUD_SETTINGS]; // Weapon_gauges[ballistic_hud_index][0]
    int top_offset_x
        [NUM_HUD_SETTINGS]; // Weapon_gauge_primary_coords[ballistic_hud_index][gr_screen.res][0]

    int Weapon_header_offsets[NUM_HUD_SETTINGS][2];

    hud_frames primary_middle
        [NUM_HUD_SETTINGS]; // Weapon_gauges[ballistic_hud_index][1]
    hud_frames primary_last[NUM_HUD_SETTINGS]; // New_weapon

    // for the rest of the gauge
    int frame_offset_x
        [NUM_HUD_SETTINGS]; // Weapon_gauge_primary_coords[ballistic_hud_index][gr_screen.res][1][0]
...
public:
    ...
    void initSecondaryWeaponOffsets (
...
public:
    ...
    void renderTriangleMissileTail (
...
public:
    ...
    void render (float frametime) override;
    void renderSight (
        int indicator_frame, vec3d* target_pos, vec3d* lead_target_pos);
    void pageIn () override;
};
