...
// defines for messages that can be sent from the player.  Defined at bitfields
// so that we can enable and disable messages on a message by message basis
...
// next item for all ships again -- to try to preserve relative order within
// the message menu
...
// following defines are the set of possible commands that can be given to a
// ship.  A mission designer might not allow some messages

// WMC - Formerly FIGHTER_MESSAGES
#define DEFAULT_MESSAGES \
        (ATTACK_TARGET_ITEM | DISABLE_TARGET_ITEM | DISARM_TARGET_ITEM | PROTECT_TARGET_ITEM | IGNORE_TARGET_ITEM | FORMATION_ITEM | COVER_ME_ITEM | ENGAGE_ENEMY_ITEM | DEPART_ITEM | DISABLE_SUBSYSTEM_ITEM)
/*
...
// which cannot be given to a ship when the target is on the same team, or the
// target is not a ship.
#define ENEMY_TARGET_MESSAGES \
        (ATTACK_TARGET_ITEM | DISABLE_TARGET_ITEM | DISARM_TARGET_ITEM | IGNORE_TARGET_ITEM | STAY_NEAR_TARGET_ITEM | CAPTURE_TARGET_ITEM | DISABLE_SUBSYSTEM_ITEM)
...
struct squadmsg_history {
        ...
        int special_index; // any extra data the order might need (subsystem names
                           // for instance)
        fix order_time;    // when this order was sent (or received by the server in
                           // multiplayer)
        squadmsg_history()
...
extern int hud_squadmsg_hotkey_select(
        int k); // a hotkey was hit -- maybe send a message to those ship(s)
extern void hud_squadmsg_save_keys(
        int do_scroll = 0); // saves into local area keys which need to be
                            // saved/restored when in messaging mode
extern int hud_squadmsg_do_frame();
extern int hud_query_order_issued(
        char *to, char *order_name, char *target = NULL, int timestamp = 0,
        char *from = NULL, char *special_index = NULL);
extern int
...
#endif // FREESPACE2_HUD_HUDSQUADMSG_HH
