...
void hud_observer_init (ship* shipp, ai_info* aip) {
    ...
    Hud_obs_ai = *aip;
    // (we used to do a memcpy here, but that doesn't work any longer, so let's
    // just assign the values we need)
    Hud_obs_ship.clear ();
    ...
    memcpy (
        &Hud_obs_ship.np_updates, shipp->np_updates,
        MAX_PLAYERS * sizeof (np_update));
