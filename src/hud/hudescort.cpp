...
int HudGaugeEscort::setGaugeColorEscort (int index, int team) {
    ...
    if (!timestamp_elapsed (Escort_ships[index].escort_hit_timer)) {
        is_flashing = 1;
        if (Escort_ships[index].escort_show_bright) { is_bright = 1; }
    }
...
void HudGaugeEscort::render (float /*frametime*/) {
    ...
    if (!Show_escort_view) { return; }

    if (!Num_escort_ships) { return; }
...
void HudGaugeEscort::renderIcon (int x, int y, int index) {
    ...
    */
    ...
    if ((sp->flags[Ship::Ship_Flags::Disabled]) ||
        (ship_subsys_disrupted (sp, SUBSYSTEM_ENGINE))) {
        renderString (
    ...
    if (screen_integrity < 100) {
        ...
        if (screen_integrity == 0) {
            if (integrity > 0) { screen_integrity = 1; }
        }
    }
    renderPrintf (
        ...
        for (int i = 0; i < Num_escort_ships; i++) {
            if (!timestamp_elapsed (Escort_ships[i].escort_hit_timer)) {
                if (timestamp_elapsed (
                    ...
                    Escort_ships[i].escort_hit_next_flash =
                        timestamp (SHIELD_FLASH_INTERVAL);
                    Escort_ships[i].escort_show_bright =
                        !Escort_ships[i]
...
void hud_escort_clear_all (bool clear_flags) {
    ...
    for (i = 0; i < Max_escort_ships; i++) {
        if (clear_flags && (Escort_ships[i].objnum >= 0) &&
            (Objects[Escort_ships[i].objnum].type == OBJ_SHIP) &&
...
void hud_setup_escort_list (int level) {
    ...
    if (num_escorts > Max_escort_ships) { num_escorts = Max_escort_ships; }

    // add ships to escort list
    for (Num_escort_ships = 0; Num_escort_ships < num_escorts;
         Num_escort_ships++) {
        Escort_ships[Num_escort_ships].obj_signature =
            complete_escorts[Num_escort_ships].obj_signature;
        Escort_ships[Num_escort_ships].priority =
            complete_escorts[Num_escort_ships].priority;
        Escort_ships[Num_escort_ships].objnum =
            complete_escorts[Num_escort_ships].objnum;
        Escort_ships[Num_escort_ships].np_id =
            complete_escorts[Num_escort_ships].np_id;
...
void hud_remove_ship_from_escort_index (int dead_index, int objnum) {
    ...
    for (i = 0; i < Num_escort_ships; i++) {
        if (i != dead_index) { bakup_arr[count++] = Escort_ships[i]; }
    }
...
void hud_add_ship_to_escort (int objnum, int supress_feedback) {
    ...
    if (num_complete_escorts == MAX_COMPLETE_ESCORT_LIST) { return; }
    ...
    if (!found) {
        ...
        complete_escorts[num_complete_escorts].obj_signature =
            Objects[objnum].signature;
        complete_escorts[num_complete_escorts].priority =
            Ships[Objects[objnum].instance].escort_priority;
...
void hud_escort_target_next () {
    ...
    Last_target_index++;
    if (Last_target_index >= Num_escort_ships) { Last_target_index = 0; }
