...
void parse_hud_gauges_tbl (const char* filename) {
    ...
    try {
        ...
        if (optional_string ("$Color:")) {
            ...
            check_color (colors);
            gr_init_alphacolor (
                &hud_color, colors[0], colors[1], colors[2], 255);
            hud_clr_p = &hud_color;
        }

        if (optional_string ("$Font:")) { Hud_font = font::parse_font (); }
        ...
        if (optional_string ("$Targetbox Shader Effect:")) {
            ...
            if (Targetbox_shader_effect < 0) { Targetbox_shader_effect = 0; }
        }
        ...
        if (optional_string ("$Reticle Style:")) {
            ...
            // using require_string_either won't advance the Mp pointer to the
            // next token so force it instead
        ...
        while (optional_string ("#Gauge Config")) {
            ...
            case 0:
                ...
                if (ship_idx >= 0) {
                    ...
                    if (optional_string ("$Color:")) {
                        ...
                        check_color (colors);
                        gr_init_alphacolor (
                            &ship_color, colors[0], colors[1], colors[2], 255);
            ...
            if (ship_clr_p != NULL) { use_clr_p = ship_clr_p; }
            else {
                use_clr_p = hud_clr_p;
            }

            if (ship_font >= 0) { use_font = ship_font; }
            else {
                use_font = Hud_font;
            }
            ...
            if (optional_string ("$Force Scaling Above:")) {
                ...
                memcpy (
                    force_scaling_above_res, Force_scaling_above_res_global,
                    sizeof (force_scaling_above_res));
            }

            // Pruning time. Let's see if the current resolution defined by the
            // user matches the conditions set by this entry
            if (optional_string ("$Required Aspect:")) {
                // filter aspect ratio.
                if (optional_string ("Full Screen")) {
                    if ((float)gr_screen.center_w / (float)gr_screen.center_h >
                        1.5) {
                        skip_to_start_of_string ("#Gauge Config");
                        // skip_to_start_of_string_either("#Gauge Config",
                        // "#End");
                        continue;
                    }
                }
                else if (optional_string ("Wide Screen")) {
                    if ((float)gr_screen.center_w /
                            (float)gr_screen.center_h <=
                        1.5) {
                        skip_to_start_of_string ("#Gauge Config");
                        // skip_to_start_of_string_either("#Gauge Config",
                        // "#End");
            ...
            while ((gauge_type = parse_gauge_type ()) >= 0) {
                // change some of the default gauge settings to the appropriate
                // values.
                gauge_settings settings;
                settings.font_num = use_font;
                settings.scale_gauge = scale_gauge;
                memcpy (
                    settings.force_scaling_above_res, force_scaling_above_res,
                    sizeof (settings.force_scaling_above_res));
                settings.ship_idx = &ship_classes;
                settings.use_clr = use_clr_p;

                // if "default" is specified, then the base resolution is {-1,
                // -1}, indicating GR_640 or GR_1024 to the handlers.
                // otherwise, change it to the given base resolution.
                if (!optional_string ("default")) {
                    memcpy (
                        settings.base_res, base_res,
                        sizeof (settings.base_res));
                }

                // then call the specific gauge load handler function for this
                // gauge type.
                load_gauge (gauge_type, &settings);

                if (saved_Mp && (saved_Mp == Mp)) { Mp++; }

                // HACK: The previous code simply skipped invalid entries but
                // now we try to generate a warning for that. If we don't see
                // either $ or + then it means that there was an invalid token
                // somewhere in between
                if (!check_for_string ("$") && !check_for_string ("+")) {
                    error_display (
                        0,
                        "Detected invalid tokens while parsing HUD gauges: "
                        "[%.32s]",
                        next_tokens ());
                ...
                // if we've been through once already and are at the same
                // place, force a move
                saved_Mp = Mp;
            }
    ...
    if (Hud_retail) {
        ...
        for (int i = 0; i < num_default_gauges; i++) {
            ...
            for (int j = 0; j < num_loaded_gauges; j++) {
                if (retail_gauges[i] ==
                    default_hud_gauges[j]->getObjectType ()) {
        ...
        retail_gauge_loaded = false;
        for (int j = 0; j < num_loaded_gauges; j++) {
            if (HUD_OBJECT_RADAR_STD ==
                    default_hud_gauges[j]->getObjectType () ||
                HUD_OBJECT_RADAR_ORB ==
                    default_hud_gauges[j]->getObjectType () ||
                HUD_OBJECT_RADAR_BSG ==
                    default_hud_gauges[j]->getObjectType ()) {
                retail_gauge_loaded = true;
            }
        }
        ...
        if (!retail_gauge_loaded) {
            gauge_settings settings;
            load_gauge (
                (Cmdline_orb_radar ? HUD_OBJECT_RADAR_ORB
                                   : HUD_OBJECT_RADAR_STD),
                &settings);
        }
...
// Called once after mission load is complete. Sets initial gauge activity
// states.
void init_hud () {
    int config_type;
    size_t i, num_gauges;
    ...
    if (!Ship_info[Player_ship->ship_info_index].hud_gauges.empty ()) {
        num_gauges =
            Ship_info[Player_ship->ship_info_index].hud_gauges.size ();

        for (i = 0; i < num_gauges; i++) {
            config_type = Ship_info[Player_ship->ship_info_index]
                              .hud_gauges[i]
                              ->getConfigType ();
            ...
            if (!Ship_info[Player_ship->ship_info_index]
                     .hud_gauges[i]
                     ->isOffbyDefault () &&
                hud_config_show_flag_is_set (config_type))
                Ship_info[Player_ship->ship_info_index]
                    .hud_gauges[i]
                    ->updateActive (true);
            else
                Ship_info[Player_ship->ship_info_index]
            ...
            Ship_info[Player_ship->ship_info_index]
                .hud_gauges[i]
                ->updatePopUp (
                    hud_config_popup_flag_is_set (config_type) ? true : false);
            Ship_info[Player_ship->ship_info_index]
                .hud_gauges[i]
                ->updateColor (
...
void set_current_hud () {
    ...
    // before we load any hud gauges, see whether we're carring a ballistic
    // weapon (Mantis #2962)
    hud_init_ballistic_index ();

    // go through all HUD gauges. Load gauge properties defined in the HUD
    // config if gauge is not customized.
    if (!Ship_info[Player_ship->ship_info_index].hud_gauges.empty ()) {
        num_gauges =
            Ship_info[Player_ship->ship_info_index].hud_gauges.size ();
        ...
        for (i = 0; i < num_gauges; i++) {
            HudGauge* hgp =
                Ship_info[Player_ship->ship_info_index].hud_gauges[i].get ();
            config_type = hgp->getConfigType ();

            if (((!hgp->isOffbyDefault () || hgp->isActive ()) &&
                 hud_config_show_flag_is_set (config_type)))
                hgp->updateActive (true);
            else
                hgp->updateActive (false);

            // hgp->updateActive(hud_config_show_flag_is_set(config_type) ?
            // true : false);
            hgp->updatePopUp (
                hud_config_popup_flag_is_set (config_type) ? true : false);
            hgp->updateColor (
        ...
        for (i = 0; i < num_gauges; i++) {
            ...
            if (((!default_hud_gauges[i]->isOffbyDefault () ||
                  default_hud_gauges[i]->isActive ()) &&
                 hud_config_show_flag_is_set (config_type)))
                default_hud_gauges[i]->updateActive (true);
            else
                default_hud_gauges[i]->updateActive (false);

            // default_hud_gauges[i]->updateActive(hud_config_show_flag_is_set(config_type)
            // ? true : false);
            default_hud_gauges[i]->updatePopUp (
                hud_config_popup_flag_is_set (config_type) ? true : false);
            default_hud_gauges[i]->updateColor (
...
int parse_gauge_type () {
    ...
    // Likely involving a for loop, an array of strings and only one if
    // statement with a strcmp.
    if (optional_string ("+Custom:")) return HUD_OBJECT_CUSTOM;

    if (optional_string ("+Messages:")) return HUD_OBJECT_MESSAGES;

    if (optional_string ("+Training Messages:"))
        return HUD_OBJECT_TRAINING_MESSAGES;

    if (optional_string ("+Support:")) return HUD_OBJECT_SUPPORT;

    if (optional_string ("+Damage:")) return HUD_OBJECT_DAMAGE;

    if (optional_string ("+Wingman Status:")) return HUD_OBJECT_WINGMAN_STATUS;

    if (optional_string ("+Auto Speed:")) return HUD_OBJECT_AUTO_SPEED;

    if (optional_string ("+Auto Target:")) return HUD_OBJECT_AUTO_TARGET;

    if (optional_string ("+Countermeasures:")) return HUD_OBJECT_CMEASURES;

    if (optional_string ("+Talking Head:")) return HUD_OBJECT_TALKING_HEAD;

    if (optional_string ("+Directives:")) return HUD_OBJECT_DIRECTIVES;

    if (optional_string ("+Weapons:")) return HUD_OBJECT_WEAPONS;

    if (optional_string ("+Objective Notify:")) return HUD_OBJECT_OBJ_NOTIFY;
    ...
    if (optional_string ("+Player Shields:")) return HUD_OBJECT_PLAYER_SHIELD;

    if (optional_string ("+Target Shields:")) return HUD_OBJECT_TARGET_SHIELD;

    if (optional_string ("+Escort View:")) return HUD_OBJECT_ESCORT;

    if (optional_string ("+Mission Time:")) return HUD_OBJECT_MISSION_TIME;

    if (optional_string ("+ETS Weapons:")) return HUD_OBJECT_ETS_WEAPONS;

    if (optional_string ("+ETS Shields:")) return HUD_OBJECT_ETS_SHIELDS;

    if (optional_string ("+ETS Engines:")) return HUD_OBJECT_ETS_ENGINES;

    if (optional_string ("+ETS Retail:")) return HUD_OBJECT_ETS_RETAIL;

    if (optional_string ("+Target Monitor:")) return HUD_OBJECT_TARGET_MONITOR;

    if (optional_string ("+Extra Target Data:"))
        return HUD_OBJECT_EXTRA_TARGET_DATA;

    if (optional_string ("+Radar:")) {
        if (Cmdline_orb_radar) { return HUD_OBJECT_RADAR_ORB; }
        else {
            return HUD_OBJECT_RADAR_STD;
        }
    }

    if (optional_string ("+Radar Orb:")) return HUD_OBJECT_RADAR_ORB;

    if (optional_string ("+Radar BSG:")) return HUD_OBJECT_RADAR_BSG;

    if (optional_string ("+Afterburner Energy:"))
        return HUD_OBJECT_AFTERBURNER;

    if (optional_string ("+Weapon Energy:")) return HUD_OBJECT_WEAPON_ENERGY;

    if (optional_string ("+Text Warnings:")) return HUD_OBJECT_TEXT_WARNINGS;

    if (optional_string ("+Center Reticle:")) return HUD_OBJECT_CENTER_RETICLE;

    if (optional_string ("+Throttle:")) return HUD_OBJECT_THROTTLE;

    if (optional_string ("+Threat Indicator:")) return HUD_OBJECT_THREAT;

    if (optional_string ("+Lead Indicator:")) return HUD_OBJECT_LEAD;

    if (optional_string ("+Lead Sight:")) return HUD_OBJECT_LEAD_SIGHT;

    if (optional_string ("+Lock Indicator:")) return HUD_OBJECT_LOCK;
    ...
    if (optional_string ("+Offscreen Indicator:")) return HUD_OBJECT_OFFSCREEN;

    if (optional_string ("+Target Brackets:")) return HUD_OBJECT_BRACKETS;

    if (optional_string ("+Hostile Triangle:")) return HUD_OBJECT_HOSTILE_TRI;

    if (optional_string ("+Target Triangle:")) return HUD_OBJECT_TARGET_TRI;

    if (optional_string ("+Missile Triangles:")) return HUD_OBJECT_MISSILE_TRI;

    if (optional_string ("+Kills:")) return HUD_OBJECT_KILLS;

    if (optional_string ("+Fixed Messages:")) return HUD_OBJECT_FIXED_MESSAGES;

    if (optional_string ("+Flight Path Marker:"))
        return HUD_OBJECT_FLIGHT_PATH;

    if (optional_string ("+Warhead Count:")) return HUD_OBJECT_WARHEAD_COUNT;

    if (optional_string ("+Hardpoints:")) return HUD_OBJECT_HARDPOINTS;
...
void load_gauge (int gauge, gauge_settings* settings) {
    ...
    case HUD_OBJECT_TRAINING_MESSAGES:
        ...
        break;
    case HUD_OBJECT_SUPPORT: load_gauge_support (settings); break;
    case HUD_OBJECT_DAMAGE: load_gauge_damage (settings); break;
    case HUD_OBJECT_WINGMAN_STATUS:
        load_gauge_wingman_status (settings);
        break;
    case HUD_OBJECT_AUTO_SPEED: load_gauge_auto_speed (settings); break;
    case HUD_OBJECT_AUTO_TARGET: load_gauge_auto_target (settings); break;
    case HUD_OBJECT_CMEASURES: load_gauge_countermeasures (settings); break;
    case HUD_OBJECT_TALKING_HEAD: load_gauge_talking_head (settings); break;
    case HUD_OBJECT_DIRECTIVES: load_gauge_directives (settings); break;
    ...
    case HUD_OBJECT_ESCORT: load_gauge_escort_view (settings); break;
    case HUD_OBJECT_MISSION_TIME: load_gauge_mission_time (settings); break;
    case HUD_OBJECT_ETS_WEAPONS: load_gauge_ets_weapons (settings); break;
    case HUD_OBJECT_ETS_SHIELDS: load_gauge_ets_shields (settings); break;
    case HUD_OBJECT_ETS_ENGINES: load_gauge_ets_engines (settings); break;
    case HUD_OBJECT_ETS_RETAIL: load_gauge_ets_retail (settings); break;
    ...
    case HUD_OBJECT_EXTRA_TARGET_DATA:
        ...
        break;
    case HUD_OBJECT_RADAR_STD: load_gauge_radar_std (settings); break;
    case HUD_OBJECT_RADAR_ORB: load_gauge_radar_orb (settings); break;
    case HUD_OBJECT_RADAR_BSG: load_gauge_radar_dradis (settings); break;
    case HUD_OBJECT_AFTERBURNER: load_gauge_afterburner (settings); break;
    case HUD_OBJECT_WEAPON_ENERGY: load_gauge_weapon_energy (settings); break;
    case HUD_OBJECT_TEXT_WARNINGS: load_gauge_text_warnings (settings); break;
    case HUD_OBJECT_CENTER_RETICLE:
        load_gauge_center_reticle (settings);
        break;
    case HUD_OBJECT_THROTTLE: load_gauge_throttle (settings); break;
    case HUD_OBJECT_THREAT: load_gauge_threat_indicator (settings); break;
    case HUD_OBJECT_LEAD: load_gauge_lead (settings); break;
    case HUD_OBJECT_LEAD_SIGHT: load_gauge_lead_sight (settings); break;
    case HUD_OBJECT_LOCK: load_gauge_lock (settings); break;
    ...
    case HUD_OBJECT_ORIENTATION_TEE:
        ...
        break;
    case HUD_OBJECT_HOSTILE_TRI: load_gauge_hostile_tri (settings); break;
    case HUD_OBJECT_TARGET_TRI: load_gauge_target_tri (settings); break;
    case HUD_OBJECT_MISSILE_TRI: load_gauge_missile_tri (settings); break;
    case HUD_OBJECT_KILLS: load_gauge_kills (settings); break;
    case HUD_OBJECT_FIXED_MESSAGES:
        load_gauge_fixed_messages (settings);
        break;
    case HUD_OBJECT_FLIGHT_PATH: load_gauge_flight_path (settings); break;
    case HUD_OBJECT_WARHEAD_COUNT: load_gauge_warhead_count (settings); break;
    case HUD_OBJECT_HARDPOINTS: load_gauge_hardpoints (settings); break;
...
void adjust_base_res (
    int* base_res, int* force_scaling_above_res, bool scaling = true) {
    // Don't scale gauge if:
    // no scaling is set and current res is between base res and "force scaling
    // above res" Avoid HUD blurring caused solely by rounding errors
    if ((!scaling && gr_screen.center_w >= base_res[0] &&
         gr_screen.center_h >= base_res[1] &&
         (force_scaling_above_res[0] <= 0 || force_scaling_above_res[1] <= 0 ||
          gr_screen.center_w <= force_scaling_above_res[0] ||
          gr_screen.center_h <= force_scaling_above_res[1])) ||
        (gr_screen.center_w >= base_res[0] &&
         gr_screen.center_h == base_res[1]) ||
        (gr_screen.center_w == base_res[0] &&
         gr_screen.center_h >= base_res[1])) {
    ...
    if (!scaling && force_scaling_above_res[0] > base_res[0] &&
        force_scaling_above_res[1] > base_res[1] &&
        gr_screen.center_w > force_scaling_above_res[0] &&
        gr_screen.center_h > force_scaling_above_res[1]) {
    ...
    float aspect_quotient =
        ((float)gr_screen.center_w / (float)gr_screen.center_h) /
        ((float)base_res[0] / (float)base_res[1]);
...
void adjust_for_multimonitor (int* base_res, bool set_position, int* coords) {
    ...
    base_res[0] = fl2ir (
        base_res[0] * ((float)gr_screen.max_w / (float)gr_screen.center_w));
    base_res[1] = fl2ir (
        base_res[1] * ((float)gr_screen.max_h / (float)gr_screen.center_h));
...
template< class T >
std::unique_ptr< T >
gauge_load_common (gauge_settings* settings, T* preAllocated = NULL) {
    int colors[3] = { 255, 255, 255 };
    ...
    if (check_base_res (settings->base_res)) {
        if (settings->set_position) {
            if (optional_string ("Position:")) {
                ...
                adjust_base_res (
                    settings->base_res, settings->force_scaling_above_res,
                    settings->scale_gauge);

                // If no positioning information is specified, use the default
                // position
                ...
                if (!(settings->use_coords && use_default_pos)) {
                    settings->coords[0] =
                        (int)(settings->base_res[0] * settings->origin[0]) +
                        settings->offset[0];
                    settings->coords[1] =
                        (int)(settings->base_res[1] * settings->origin[1]) +
                        settings->offset[1];
                    settings->use_coords = true;
                }
            }
    ...
    if (settings->set_position) {
        if (optional_string ("Cockpit Target:") &&
            settings->ship_idx->at (0) >= 0) {
            ...
            // adjust for multimonitor setups ONLY if not rendering gauge to a
            // texture
            adjust_for_multimonitor (
                settings->base_res, true, settings->coords);
        }
    }
    else {
    ...
    if (optional_string ("Font:")) {
        ...
        if (settings->font_num < 0) { settings->font_num = font::FONT1; }
    }

    if (settings->set_position) {
        if (optional_string ("Slew:")) { stuff_boolean (&settings->slew); }
    }

    std::unique_ptr< T > instance (preAllocated);

    if (instance == NULL) { instance.reset (new T ()); }

    instance->initBaseResolution (
        settings->base_res[0], settings->base_res[1]);
    ...
    if (settings->set_position) {
        ...
        instance->initCockpitTarget (
            display_name, display_offset[0], display_offset[1],
            display_size[0], display_size[1], canvas_size[0], canvas_size[1]);
    }
    if (settings->set_colour) {
...
template< typename T >
void gauge_assign_common (
    const gauge_settings* settings, std::unique_ptr< T >&& hud_gauge) {
    if (settings->ship_idx->at (0) >= 0) {
        for (auto ship_index = settings->ship_idx->begin ();
             ship_index != settings->ship_idx->end (); ++ship_index) {
...
void load_gauge_custom (gauge_settings* settings) {
    ...
    if (check_base_res (settings->base_res)) {
        ...
        if (optional_string ("X Offset:")) { stuff_int (&txtoffset_x); }

        if (optional_string ("Y Offset:")) { stuff_int (&txtoffset_y); }
    ...
    std::unique_ptr< HudGauge > hud_gauge (new HudGauge (
        gauge_type, settings->slew, r, g, b, name, text, filename, txtoffset_x,
        txtoffset_y));
    hud_gauge->initBaseResolution (
        settings->base_res[0], settings->base_res[1]);
    ...
    hud_gauge->initCockpitTarget (
        display_name, display_offset[0], display_offset[1], display_size[0],
        display_size[1], canvas_size[0], canvas_size[1]);
...
void load_gauge_mini_shields (gauge_settings* settings) {
    ...
    hud_gauge->init1DigitOffsets (
        Mini_1digit_offsets[0], Mini_1digit_offsets[1]);
    hud_gauge->init2DigitOffsets (
        Mini_2digit_offsets[0], Mini_2digit_offsets[1]);
    hud_gauge->init3DigitOffsets (
        Mini_3digit_offsets[0], Mini_3digit_offsets[1]);
    hud_gauge->initBitmaps (fname);
...
void load_gauge_weapon_energy (gauge_settings* settings) {
    ...
    if (optional_string ("Text Offsets:")) {
        ...
        if (optional_string ("Text Alignment:")) {
            if (required_string ("Right")) { text_alignment = 1; }
        }
    }
    if (optional_string ("Always Show Text:")) {
        stuff_boolean (&always_show_text);
    }
    if (optional_string ("Text Follows:")) { stuff_boolean (&moving_text); }
    ...
    if (optional_string ("Armed Guns List Offsets:")) {
        ...
        if (optional_string ("Armed Guns List Alignment:")) {
            if (required_string ("Right")) { weapon_alignment = 1; }
        }
    ...
    hud_gauge->initTextOffsets (
        Wenergy_text_offsets[0], Wenergy_text_offsets[1]);
    ...
    hud_gauge->initArmedOffsets (
        armed_weapon_offsets[0], armed_weapon_offsets[1], armed_weapon_h,
        show_weapons);
...
void load_gauge_escort_view (gauge_settings* settings) {
    ...
    if (optional_string ("Entry Height:")) { stuff_int (&entry_h); }
    ...
    hud_gauge->initHeaderTextOffsets (
        header_text_offsets[0], header_text_offsets[1]);
    hud_gauge->initListStartOffsets (
        list_start_offsets[0], list_start_offsets[1]);
    hud_gauge->initShipIntegrityOffsets (
        ship_integrity_offsets[0], ship_integrity_offsets[1]);
    hud_gauge->initShipNameOffsets (
        ship_name_offsets[0], ship_name_offsets[1]);
    hud_gauge->initShipStatusOffsets (
        ship_status_offsets[0], ship_status_offsets[1]);
...
void load_gauge_afterburner (gauge_settings* settings) {
    ...
    if (optional_string ("Foreground Clip Height:")) { stuff_int (&energy_h); }
...
void load_gauge_threat_indicator (gauge_settings* settings) {
    ...
    hud_gauge->initLaserWarnOffsets (
        Laser_warn_offsets[0], Laser_warn_offsets[1]);
    hud_gauge->initLockWarnOffsets (
        Lock_warn_offsets[0], Lock_warn_offsets[1]);
...
void load_gauge_center_reticle (gauge_settings* settings) {
    ...
    if (optional_string ("Firepoint display:")) stuff_boolean (&firepoints);

    if (optional_string ("Firepoint size:")) stuff_int (&size);
    ...
    if (optional_string ("Autoaim Frame:")) stuff_int (&autoaim_frame);
...
void load_gauge_throttle (gauge_settings* settings) {
    ...
    if (optional_string ("Orbit Radius:")) { stuff_int (&orbit_radius); }
    ...
    hud_gauge->initMaxSpeedOffsets (
        max_speed_offset[0], max_speed_offset[1], show_max_speed);
    hud_gauge->initZeroSpeedOffsets (
        zero_speed_offset[0], zero_speed_offset[1], show_min_speed);
    hud_gauge->initOrbitCenterOffsets (
        orbit_center_offset[0], orbit_center_offset[1], orbit);
    hud_gauge->initOrbitRadius (orbit_radius);
    hud_gauge->initTargetSpeedOffsets (
        target_speed_offset[0], target_speed_offset[1], show_target_speed,
        show_target_speed_percent);
    hud_gauge->initGlideOffsets (
        glide_offset[0], glide_offset[1], custom_glide);
    hud_gauge->initMatchSpeedOffsets (
        match_speed_offset[0], match_speed_offset[1], custom_match);
    hud_gauge->initBitmaps (fname);
...
void load_gauge_ets_retail (gauge_settings* settings) {
    ...
    if (Lcl_gr) {
        ets_letters[0] = 'G';
        ets_letters[1] = 'S';
        ets_letters[2] = 'A'; // German
    }
    else if (Lcl_fr) {
        ets_letters[0] = 'C';
        ets_letters[1] = 'B';
        ets_letters[2] = 'M'; // French
    }
    else {
        ets_letters[0] = 'G';
        ets_letters[1] = 'S';
        ets_letters[2] = 'E'; // English
    }
    ...
    if (optional_string ("Foreground Clip Height:")) { stuff_int (&bar_h); }
    if (optional_string ("Letter Offsets:")) {
        stuff_int_list (letter_offsets, 2);
    }
    if (optional_string ("Top Offsets:")) { stuff_int_list (top_offsets, 2); }
    if (optional_string ("Bottom Offsets:")) {
        stuff_int_list (bottom_offsets, 2);
    }
    if (optional_string ("Gauge Offset:")) { stuff_int (&gauge_offset); }

    // calculate offsets for the three gauges from settings->coords[0], which
    // was set by gauge_load_common
...
void load_gauge_extra_target_data (gauge_settings* settings) {
    ...
    if (optional_string ("Dock Max Width:")) { stuff_int (&dock_max_w); }
    if (optional_string ("Order Offsets:")) {
        stuff_int_list (order_offsets, 2);
    }
    if (optional_string ("Order Max Width:")) { stuff_int (&order_max_w); }
...
void load_gauge_radar_std (gauge_settings* settings) {
    ...
    if (optional_string ("Radar Size:")) { stuff_int_list (Radar_radius, 2); }
    ...
    if (Cmdline_orb_radar == 0) {
        hud_gauge->initBitmaps (fname);
        hud_gauge->initBlipRadius (
            Radar_blip_radius_normal, Radar_blip_radius_target);
        hud_gauge->initCenterOffsets (
            Radar_center_offsets[0], Radar_center_offsets[1]);
        hud_gauge->initDistanceInfinityOffsets (
            Radar_dist_offsets[2][0], Radar_dist_offsets[2][1]);
        hud_gauge->initDistanceLongOffsets (
            Radar_dist_offsets[1][0], Radar_dist_offsets[1][1]);
        hud_gauge->initDistanceShortOffsets (
            Radar_dist_offsets[0][0], Radar_dist_offsets[0][1]);
 ...
 * Unfortunately, I can't see how to make this fit the gauge_load_common
 * function
 */
void load_gauge_radar_dradis (gauge_settings* settings) {
    ...
    if (optional_string ("Size:")) { stuff_int_list (Radar_radius, 2); }
    ...
    std::unique_ptr< HudGaugeRadarDradis > hud_gauge (
        new HudGaugeRadarDradis ());
    hud_gauge->initBaseResolution (
        settings->base_res[0], settings->base_res[1]);
    hud_gauge->initPosition (settings->coords[0], settings->coords[1]);
    hud_gauge->initRadius (Radar_radius[0], Radar_radius[1]);
    hud_gauge->initBitmaps (
        xy_fname, xz_yz_fname, sweep_fname, target_fname, unknown_fname);
    hud_gauge->initCockpitTarget (
        display_name, display_offset[0], display_offset[1], display_size[0],
        display_size[1], canvas_size[0], canvas_size[1]);
    hud_gauge->initFont (settings->font_num);
    hud_gauge->initSound (
        loop_snd, loop_snd_volume, arrival_beep_snd, departure_beep_snd,
        stealth_arrival_snd, stealth_departure_snd, arrival_beep_delay,
        departure_beep_delay);
...
void load_gauge_target_monitor (gauge_settings* settings) {
    ...
    // remember, below coords describe the rightmost position of their
    // respective sub-element, not leftmost like it usually does.
    Hull_offsets[0] = 134;
    Hull_offsets[1] = 42;
    ...
    if (optional_string ("Desaturate:")) { stuff_boolean (&desaturate); }
    ...
    hud_gauge->initIntegrityOffsets (
        Integrity_bar_offsets[0], Integrity_bar_offsets[1]);
    ...
    hud_gauge->initCargoStringOffsets (
        Cargo_string_offsets[0], Cargo_string_offsets[1]);
    hud_gauge->initHullOffsets (Hull_offsets[0], Hull_offsets[1]);
    hud_gauge->initCargoScanStartOffsets (
        Cargo_scan_start_offsets[0], Cargo_scan_start_offsets[1]);
    hud_gauge->initCargoScanSize (Cargo_scan_size[0], Cargo_scan_size[1]);
    hud_gauge->initSubsysNameOffsets (
        Subsys_name_offsets[0], Subsys_name_offsets[1],
        Use_subsys_name_offsets);
    hud_gauge->initSubsysIntegrityOffsets (
        Subsys_integrity_offsets[0], Subsys_integrity_offsets[1],
        Use_subsys_integrity_offsets);
    hud_gauge->initDisabledStatusOffsets (
        Disabled_status_offsets[0], Disabled_status_offsets[1],
        Use_disabled_status_offsets);
    hud_gauge->initDesaturate (desaturate);
    hud_gauge->initBitmaps (
        fname_monitor, fname_monitor_mask, fname_integrity, fname_static);
...
void load_gauge_squad_message (gauge_settings* settings) {
    ...
    if (optional_string ("Entry Height:")) { stuff_int (&Item_h); }
    if (optional_string ("Bottom Background Offset:")) {
        stuff_int (&bottom_bg_offset);
    }
    if (optional_string ("Command X-offset:")) { stuff_int (&Item_offset_x); }
    ...
    hud_gauge->initItemStartOffsets (
        Item_start_offsets[0], Item_start_offsets[1]);
...
void load_gauge_weapons (gauge_settings* settings) {
    ...
    // The bottom portion of the primary weapons listing for the ballistic ammo
    // version of this gauge can simply use the middle portion.
    ...
    hud_gauge->initHeaderOffsets (
        Weapon_header_offsets[0][0], Weapon_header_offsets[0][1],
        Weapon_header_offsets[1][0], Weapon_header_offsets[1][1]);
    hud_gauge->initFrameOffsetX (frame_offset_x[0], frame_offset_x[1]);
    hud_gauge->initStartNameOffsetsY (
        pname_start_offset_y, sname_start_offset_y);
    hud_gauge->initPrimaryWeaponOffsets (
        Weapon_plink_offset_x, Weapon_pname_offset_x, Weapon_pammo_offset_x);
    hud_gauge->initSecondaryWeaponOffsets (
        Weapon_sammo_offset_x, Weapon_sname_offset_x, Weapon_sreload_offset_x,
        Weapon_slinked_offset_x, Weapon_sunlinked_offset_x);
...
void load_gauge_directives (gauge_settings* settings) {
    ...
    if (optional_string ("Entry Height:")) { stuff_int (&text_h); }
    if (optional_string ("Bottom Background Offset:")) {
        stuff_int (&bottom_bg_offset);
    }
    if (optional_string ("Max Line Width:")) { stuff_int (&max_line_width); }

    hud_gauge->initBitmaps (fname_top, fname_middle, fname_bottom);
    hud_gauge->initMiddleFrameOffsetY (middle_frame_offset_y);
    hud_gauge->initTextHeight (text_h);
    hud_gauge->initBottomBgOffset (bottom_bg_offset);
    hud_gauge->initTextStartOffsets (
        text_start_offsets[0], text_start_offsets[1]);
    hud_gauge->initHeaderOffsets (header_offsets[0], header_offsets[1]);
...
void load_gauge_countermeasures (gauge_settings* settings) {
    ...
    hud_gauge->initCountValueOffsets (
        cm_text_val_offset[0], cm_text_val_offset[1]);
...
void load_gauge_auto_target (gauge_settings* settings) {
    ...
    if (optional_string ("On Text Color:")) { stuff_int_list (on_color, 4); }

    if (optional_string ("Off Text Color:")) { stuff_int_list (off_color, 4); }

    hud_gauge->initAutoTextOffsets (auto_text_offset[0], auto_text_offset[1]);
    hud_gauge->initBitmaps (fname);
    hud_gauge->initTargetTextOffsets (
        target_text_offset[0], target_text_offset[1]);
    hud_gauge->initOnColor (
        on_color[0], on_color[1], on_color[2], on_color[3]);
    hud_gauge->initOffColor (
        off_color[0], off_color[1], off_color[2], off_color[3]);
...
void load_gauge_auto_speed (gauge_settings* settings) {
    ...
    hud_gauge->initAutoTextOffsets (auto_text_offset[0], auto_text_offset[1]);
    hud_gauge->initBitmaps (fname);
    hud_gauge->initSpeedTextOffsets (
        speed_text_offset[0], speed_text_offset[1]);
    hud_gauge->initOnColor (
        on_color[0], on_color[1], on_color[2], on_color[3]);
    hud_gauge->initOffColor (
        off_color[0], off_color[1], off_color[2], off_color[3]);
...
void load_gauge_wingman_status (gauge_settings* settings) {
    ...
    // "wingman5" isn't used anymore since Goober implemented string based wing
    // names
    ...
    if (optional_string ("Right Background Filename:") ||
        optional_string ("Last Background Filename:")) {
    ...
    if (optional_string ("Entry Width:")) { stuff_int (&wing_width); }
    if (optional_string ("Entry Size:")) { stuff_int (&wing_width); }
    if (optional_string ("Right Background Offset:") ||
        optional_string ("Last Background Offset:")) {
    ...
    int grow_mode =
        0; // By default, expand the gauge to the left (in -x direction)
    ...
    hud_gauge->initMultipleWingOffsets (
        multiple_wing_offsets[0], multiple_wing_offsets[1]);
    hud_gauge->initSingleWingOffsets (
        single_wing_offsets[0], single_wing_offsets[1]);
    hud_gauge->initWingmate1Offsets (
        wingmate_offsets[0][0], wingmate_offsets[0][1]);
    hud_gauge->initWingmate2Offsets (
        wingmate_offsets[1][0], wingmate_offsets[1][1]);
    hud_gauge->initWingmate3Offsets (
        wingmate_offsets[2][0], wingmate_offsets[2][1]);
    hud_gauge->initWingmate4Offsets (
        wingmate_offsets[3][0], wingmate_offsets[3][1]);
    hud_gauge->initWingmate5Offsets (
        wingmate_offsets[4][0], wingmate_offsets[4][1]);
    hud_gauge->initWingmate6Offsets (
        wingmate_offsets[5][0], wingmate_offsets[5][1]);
    hud_gauge->initWingNameOffsets (
        wing_name_offsets[0], wing_name_offsets[1]);
...
void load_gauge_damage (gauge_settings* settings) {
    ...
    if (optional_string ("Subsystem Entry Height:")) { stuff_int (&line_h); }
    ...
    hud_gauge->initBitmaps (fname_top, fname_middle, fname_bottom);
    hud_gauge->initHullIntegOffsets (
        hull_integ_offsets[0], hull_integ_offsets[1]);
    hud_gauge->initHullIntegValueOffsetX (hull_integ_val_offset_x);
    hud_gauge->initLineHeight (line_h);
    hud_gauge->initMiddleFrameStartOffsetY (middle_frame_start_offset_y);
    hud_gauge->initSubsysIntegStartOffsets (
        subsys_integ_start_offsets[0], subsys_integ_start_offsets[1]);
...
void load_gauge_support (gauge_settings* settings) {
    ...
    if (optional_string ("Text Y-offset:")) { stuff_int (&text_val_offset_y); }
...
void load_gauge_messages (gauge_settings* settings) {
    ...
    if (gr_screen.res == GR_640) { max_width = 620; }
    else {
        max_width = 1004;
    }
    ...
    if (optional_string ("Max Lines:")) { stuff_int (&max_lines); }
    if (optional_string ("Max Width:")) { stuff_int (&max_width); }
    if (optional_string ("Line Height:")) { stuff_int (&line_height); }
    if (optional_string ("Total Lifetime:")) { stuff_int (&total_life); }
    if (optional_string ("Scroll Time:")) { stuff_int (&scroll_time); }
    if (optional_string ("Step Size:")) { stuff_int (&step_size); }
...
void load_gauge_fixed_messages (gauge_settings* settings) {
    ...
    settings->use_coords = true;
    // set both coords to INT_MIN so that it's possible to check whether a HUD
    // table has overridden them (this is necessary to determine the default
    // value for center_text)
    ...
    if (settings->coords[0] == INT_MIN && settings->coords[1] == INT_MIN) {
        // coords have almost certainly not been overridden; set them to their
        // true defaults and set center_text to true
...
void load_gauge_weapon_linking (gauge_settings* settings) {
    ...
    hud_gauge->init1PrimaryOffsets (
        Weapon_link_offsets[LINK_ONE_PRIMARY][0],
        Weapon_link_offsets[LINK_ONE_PRIMARY][1]);
    hud_gauge->init2PrimaryOffsets (
        Weapon_link_offsets[LINK_TWO_PRIMARY][0],
        Weapon_link_offsets[LINK_TWO_PRIMARY][1]);
    hud_gauge->init1SecondaryOffsets (
        Weapon_link_offsets[LINK_ONE_SECONDARY][0],
        Weapon_link_offsets[LINK_ONE_SECONDARY][1]);
    hud_gauge->init2SecondaryOffsets (
        Weapon_link_offsets[LINK_TWO_SECONDARY][0],
        Weapon_link_offsets[LINK_TWO_SECONDARY][1]);
    hud_gauge->init3SecondaryOffsets (
        Weapon_link_offsets[LINK_THREE_SECONDARY][0],
        Weapon_link_offsets[LINK_THREE_SECONDARY][1]);
    hud_gauge->initBitmaps (
...
void load_gauge_lock (gauge_settings* settings) {
    ...
    hud_gauge->initTargetBoxSize (
        Lock_target_box_width, Lock_target_box_height);
...
void load_gauge_brackets (gauge_settings* settings) {
    ...
    hud_gauge->initBitmaps (fname);
    hud_gauge->initMinSubTargetBoxSizes (
        min_subtarget_box[0], min_subtarget_box[1]);
...
void load_gauge_hostile_tri (gauge_settings* settings) {
    ...
    if (optional_string ("Radius:")) { stuff_int (&Radius); }
...
void load_gauge_kills (gauge_settings* settings) {
    ...
    hud_gauge->initTextValueOffsets (
        text_value_offsets[0], text_value_offsets[1]);
...
void load_gauge_warhead_count (gauge_settings* settings) {
    ...
    if (optional_string ("Icon Width:")) { stuff_int (&icon_width); }

    if (optional_string ("Icon Height:")) { stuff_int (&icon_height); }

    if (optional_string ("Max Icons:")) { stuff_int (&max_icons); }

    if (optional_string ("Max Columns:")) { stuff_int (&max_columns); }

    if (optional_string ("Name Alignment:")) {
        if (optional_string ("Right")) { alignment = 1; }
        else {
            alignment = 0;
        }
    }
    ...
    hud_gauge->initBitmap (fname);
    hud_gauge->initNameOffsets (
        warhead_name_offsets[0], warhead_name_offsets[1]);
    hud_gauge->initCountOffsets (
        warhead_count_offsets[0], warhead_count_offsets[1]);
...
void load_gauge_hardpoints (gauge_settings* settings) {
    ...
    if (optional_string ("Size:")) { stuff_int_list (sizes, 2); }

    if (optional_string ("Line Width:")) { stuff_float (&line_width); }

    if (optional_string ("View Direction:")) {
        if (optional_string ("Top")) { view_dir = HudGaugeHardpoints::TOP; }
...
void load_gauge_primary_weapons (gauge_settings* settings) {
    ...
    if (optional_string ("Ammo X-offset:")) { stuff_int (&ammo_x); }

    if (optional_string ("Link X-offset:")) { stuff_int (&link_x); }

    if (optional_string ("Name X-offset:")) { stuff_int (&name_x); }
...
void load_gauge_secondary_weapons (gauge_settings* settings) {
    ...
    if (optional_string ("Unlink X-offset:")) { stuff_int (&unlink_x); }

    if (optional_string ("Reload X-offset:")) { stuff_int (&reload_x); }
