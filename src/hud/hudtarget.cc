...
// The following variables are global to this file, and do not need to be
// persistent from frame-to-frame This means the variables are not
// player-specific
...
int Cm_text_val_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                  ...
                                                  { // GR_1024
                                                    889, 606 } };
char Cm_fname[GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = { "countermeasure1",
                                                        "countermeasure1" };
...
int Weapon_sunlinked_x[GR_NUM_RESOLUTIONS] = {
    530, // where to draw the first thingie if this weapon is selected at all
         // (fire-linked or not)
    913
};
...
const char* Weapon_gauge_fnames
    [NUM_HUD_SETTINGS][GR_NUM_RESOLUTIONS][NUM_WEAPON_GAUGES] = {
        // XSTR:OFF
        {   // normal HUD
          { // GR_640
        ...
        {   // ballistic HUD - slightly different alignment
          { // GR_640
            "weapons1_b", "weapons2_b", "weapons3_b", "weapons4_b",
            "weapons5_b" },
          { // GR_1024
            "weapons1_b", "weapons2_b", "weapons3_b", "weapons4_b",
            "weapons5_b" } }
...
// Flash the line for a weapon.  This normally occurs when the player tries to
// fire that weapon, but the firing fails (due to lack of energy or damaged
// weapons subsystem).
...
typedef struct homing_beep_info {
    ...
    float precalced_interp;  // a precalculated value used in a linear
                             // interpretation
} homing_beep_info;

homing_beep_info Homing_beep = {
    sound_handle::invalid (), 0, 150, 1000, 30.0f, 1500.0f, 1.729412f
};

// Set at the start of a mission, used to decide how to draw the separation for
// the warning missile indicators
...
int hud_target_invalid_awacs (object* objp) {
    ...
    if (objp->type == OBJ_SHIP) {
        if (Player_ship != NULL) {
            if (ship_is_visible_by_team (objp, Player_ship)) { return 0; }
        }
    }

    // check for invalid status
    if ((Player_ship != NULL) &&
        (awacs_get_level (objp, Player_ship) < 1.0f)) {
        return 1;
    }
...
ship_subsys* advance_subsys (ship_subsys* cur, int next_flag) {
    if (next_flag) { return GET_NEXT (cur); }
    else {
        return GET_LAST (cur);
    }
}

// select a sorted turret subsystem on a ship if no other subsys has been
// selected
void hud_maybe_set_sorted_turret_subsys (ship* shipp) {
    ...
    if (Objects[Player_ai->target_objnum].type != OBJ_SHIP) { return; }
...
void hud_reticle_clear_list (reticle_list* rlist) {
    reticle_list* cur;
    for (cur = GET_FIRST (rlist); cur != END_OF_LIST (rlist);
         cur = GET_NEXT (cur)) {
        cur->flags = 0;
    }
    list_init (rlist);
...
void hud_reticle_list_init () {
    ...
    for (i = 0; i < MAX_RETICLE_TARGETS; i++) { Reticle_list[i].flags = 0; }
...
int hud_reticle_list_find_free () {
    ...
    for (i = 0; i < MAX_RETICLE_TARGETS; i++) {
        if (!(Reticle_list[i].flags & RL_USED)) { break; }
    }
...
void hud_stuff_reticle_list (
    reticle_list* rl, object* objp, float measure, int dot_flag) {
    if (dot_flag) {
        rl->dot = measure;
...
void hud_reticle_list_update (object* objp, float measure, int dot_flag) {
    ...
    if (objp->type == OBJ_JUMP_NODE) {
        for (jnp = Jump_nodes.begin (); jnp != Jump_nodes.end (); ++jnp) {
            if (jnp->GetSCPObject () != objp) continue;

            if (jnp->IsHidden ()) return;
        }
    }

    for (rl = GET_FIRST (&Reticle_cur_list);
         rl != END_OF_LIST (&Reticle_cur_list); rl = GET_NEXT (rl)) {
        if (rl->objp == objp) return;
    }
    ...
    if (!was_inserted) { list_append (&Reticle_cur_list, new_rl); }
}
...
object* hud_reticle_pick_target () {
    ...
    // As a first step, see if both ships and debris are in the list.  If so,
    // cull the debris.
    int debris_in_list = 0;
    int ship_in_list = 0;
    for (cur_rl = GET_FIRST (&Reticle_cur_list);
         cur_rl != END_OF_LIST (&Reticle_cur_list);
         cur_rl = GET_NEXT (cur_rl)) {
        if ((cur_rl->objp->type == OBJ_SHIP) ||
            (cur_rl->objp->type == OBJ_JUMP_NODE)) {
            ship_in_list = 1;
            continue;
        }

        if (cur_rl->objp->type == OBJ_WEAPON) {
            if (Weapon_info[Weapons[cur_rl->objp->instance].weapon_info_index]
                    .subtype == WP_MISSILE) {
                ship_in_list = 1;
                continue;
            }
        ...
        if ((cur_rl->objp->type == OBJ_DEBRIS) ||
            (cur_rl->objp->type == OBJ_ASTEROID)) {
            debris_in_list = 1;
            continue;
        }
    }
    ...
    for (cur_rl = GET_FIRST (&Reticle_cur_list);
         ...
         cur_rl = GET_NEXT (cur_rl)) {
        in_save_list = 0;
        for (save_rl = GET_FIRST (&Reticle_save_list);
             save_rl != END_OF_LIST (&Reticle_save_list);
...
// hud_target_hotkey_add_remove takes as its parameter which hotkey (1-0) to
// add/remove the current target from.  This function behaves like the
// Shift-<selection> does in Windows -- using shift # will toggle the current
...
void hud_target_hotkey_add_remove (int k, object* ctarget, int how_to_add) {
    ...
    if ((k < 0) || (k >= MAX_KEYED_TARGETS)) {
        WARNINGF (LOCATION, "Bogus hotkey %d sent to hud_target_hotkey_add_remove",k);
        return;
    }
    ...
    if (ctarget->type != OBJ_SHIP) return;

    // don't allow player into hotkey set
    if (ctarget == Player_obj) return;

    // don't put dying or departing
    if (Ships[ctarget->instance].is_dying_or_departing ()) return;

    // don't add mission file added hotkey assignments if there are player
    // added assignments already in the list
    if ((how_to_add == HOTKEY_MISSION_FILE_ADDED) && NOT_EMPTY (plist)) {
        for (hitem = GET_FIRST (plist); hitem != END_OF_LIST (plist);
            ...
            if (hitem->how_added == HOTKEY_USER_ADDED) return;
        }
    }
    ...
    for (hitem = GET_FIRST (plist); hitem != END_OF_LIST (plist);
         hitem = GET_NEXT (hitem)) {
        if (hitem->objp == ctarget) break;
    }

    // if hitem == end of the list, then the target should be added, else it
    // should be removed
    if (hitem == END_OF_LIST (plist)) {
        ...
        WARNINGF (LOCATION, "Hotkey: Adding %s",Ships[ctarget->instance].ship_name);
        ...
        WARNINGF (LOCATION, "Hotkey: Removing %s",Ships[ctarget->instance].ship_name);
...
void hud_target_hotkey_clear (int k) {
    ...
    if (Players[Player_num].current_hotkey_set ==
        k) // clear this variable if we removed the bindings
        Players[Player_num].current_hotkey_set = -1;
}

// the next function sets the current selected set to be N.  If there is just
// one ship in the selection set, this ship will become the new target.  If
// there is more than one ship in the selection set, then the current_target
...
void hud_target_hotkey_select (int k) {
    ...
    // set the current target to be the "next" ship in the list.  Scan the list
    // to see if our current target is in the set.  If so, target the next ship
    ...
    for (hitem = GET_FIRST (plist); hitem != END_OF_LIST (plist);
        ...
        if (awacs_get_level (hitem->objp, Player_ship, 1) > 1) {
            ...
            if (first_target == NULL) { first_target = hitem; }

            // get the next target in the list following the player currently
            // selected target
            if (target != NULL) {
                next_target = hitem;
                break;
        ...
        if (OBJ_INDEX (hitem->objp) == target_objnum) { target = hitem; }
    }

    // if current target is not in list, then target and next_target will be
    // NULL so we use the first found target
    if (target == NULL) {
        ...
        // if current target is in the list but this is not our current
        // selection set, then we don't want to change target.
    }
...
// hud_init_targeting_colors() will initialize the shader and gradient objects
// used on the HUD
//

color HUD_color_homing_indicator;

void hud_make_shader (
...
void hud_weapons_init () {
    ...
    // The E: There used to be a number of checks here. They are no longer
    // needed, as the new HUD code handles it on its own.
...
void hud_init_homing_beep () {
    ...
    Homing_beep.precalced_interp =
        (Homing_beep.max_cycle_dist - Homing_beep.min_cycle_dist) /
...
void hud_init_ballistic_index () {
    ...
    for (i = 0; i < Player_ship->weapons.num_primary_banks; i++) {
        if (Weapon_info[Player_ship->weapons.primary_bank_weapons[i]]
...
void hud_init_targeting () {
    ...
    // Init the lists that hold targets in reticle (to allow cycling of targets
    // in reticle)
    hud_reticle_list_init ();
...
void hud_target_subobject_common (int next_flag) {
    if (Player_ai->target_objnum == -1) {
        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN, "%s", XSTR ("No target selected.", 322));
    ...
    for (A = start2; A != start; A = advance_subsys (A, next_flag)) {
        if (A == &target_shipp->subsys_list) { continue; }

        // ignore turrets
        if (A->system_info->type == SUBSYSTEM_TURRET) { continue; }

        if (A->flags[Ship::Subsystem_Flags::Untargetable]) { continue; }
    ...
    if (subsys_to_target == NULL) {
        ...
        set_targeted_subsys (
            Player_ai, subsys_to_target, Player_ai->target_objnum);
        target_shipp->last_targeted_subobject[Player_num] =
...
// hud_target_next() will set the Players[Player_num].current_target to the
// next target in the object used list whose team matches the team parameter.
// The player is NOT included in the target list.
//
...
void hud_target_common (int team_mask, int next_flag) {
    ...
    for (A = start2; A != start; A = advance_fb (A, next_flag)) {
        ...
        if (A == &obj_used_list) continue;

        if ((A == Player_obj) ||
            ((A->type != OBJ_SHIP) && (A->type != OBJ_WEAPON) &&
             (A->type != OBJ_JUMP_NODE)))
            continue;

        if (hud_target_invalid_awacs (A)) continue;

        if (A->type == OBJ_WEAPON) {
            if (!(Weapon_info[Weapons[A->instance].weapon_info_index]
                      .wi_flags[Weapon::Info_Flags::Can_be_targeted]))
                if (!(Weapon_info[Weapons[A->instance].weapon_info_index]
                          .wi_flags[Weapon::Info_Flags::Bomb]))
                    continue;

            if (Weapons[A->instance].lssm_stage == 3) continue;
        }

        if (A->type == OBJ_SHIP) {
            if (should_be_ignored (&Ships[A->instance])) continue;
        ...
        if (A->type == OBJ_JUMP_NODE) {
            for (jnp = Jump_nodes.begin (); jnp != Jump_nodes.end (); ++jnp) {
                if (jnp->GetSCPObject () == A) break;
            }

            ASSERTX (
                jnp != Jump_nodes.end (),
                "Failed to find jump node with object index %d; trace out and "
                "fix!\n",
                OBJ_INDEX (A));

            if (jnp->IsHidden ()) continue;
        }

        if (vm_vec_same (&A->pos, &Eye_position)) continue;

        if (is_ship) {
            shipp =
                &Ships[A->instance]; // get a pointer to the ship information

            if (!iff_matches_mask (shipp->team, team_mask)) { continue; }

            if (A == Player_obj) { continue; }
...
missile_obj* advance_missile_obj (missile_obj* mo, int next_flag) {
    if (next_flag) { return GET_NEXT (mo); }

    return GET_LAST (mo);
}

ship_obj* advance_ship (ship_obj* so, int next_flag) {
    if (next_flag) { return GET_NEXT (so); }
...
static object* select_next_target_by_distance (
    const bool targeting_from_closest_to_farthest, const int valid_team_mask,
    ...
    float current_distance = hud_find_target_distance (
        &Objects[Player_ai->target_objnum], Player_obj);
    float minimum_distance = 1e20f;
    float maximum_distance = 0.0f;
    int player_object_index = OBJ_INDEX (Player_obj);
    flagset< Ship::Info_Flags > filter;

    if (target_filters != NULL) { filter = *target_filters; }
    else {
    ...
    if (targeting_from_closest_to_farthest) { nearest_distance = 1e20f; }
    else {
    ...
    for (ship_object_ptr = GET_FIRST (&Ship_obj_list);
         ship_object_ptr != END_OF_LIST (&Ship_obj_list);
        ...
        float new_distance;
        if ((prospective_victim_ptr == Player_obj) ||
            (should_be_ignored (prospective_victim_ship_ptr))) {
            continue;
        }

        // choose from the correct team
        if (!iff_matches_mask (
                prospective_victim_ship_ptr->team, valid_team_mask)) {
            continue;
        }
        ...
        if (attacked_object_number == -1) {
            ...
            if ((Ship_info[prospective_victim_ship_ptr->ship_info_index]
                     .flags &
                 filter)
                    .any_set ()) {
                continue;
            ...
            new_distance =
                hud_find_target_distance (prospective_victim_ptr, Player_obj);
        }
        else {
            // Filter out any target that is not targeting the player
            // --Mastadon
            if ((attacked_object_number == player_object_index) &&
                (Ai_info[prospective_victim_ship_ptr->ai_index]
                     .target_objnum != player_object_index)) {
                continue;
            }
            esct eval_ship_as_closest_target_args;
            eval_ship_as_closest_target_args.attacked_objnum =
                attacked_object_number;
            eval_ship_as_closest_target_args.check_all_turrets =
                (attacked_object_number == player_object_index);
            eval_ship_as_closest_target_args.check_nearest_turret = FALSE;
            // We don't ever filter our target selection to just bombers or
            // fighters because the select next attacker logic doesn't.
            // --Mastadon
            eval_ship_as_closest_target_args.filter = 0;
            eval_ship_as_closest_target_args.team_mask = valid_team_mask;
            // We always get the turret attacking, since that's how the select
            // next attacker logic does it.  --Mastadon
            eval_ship_as_closest_target_args.turret_attacking_target = 1;
            eval_ship_as_closest_target_args.shipp =
                prospective_victim_ship_ptr;
            evaluate_ship_as_closest_target (
...
void hud_target_missile (object* source_obj, int next_flag) {
    ...
    if (source_obj->type != OBJ_SHIP) return;
    ...
    if (aip->target_objnum != -1) {
        ...
        if (target_objp->type == OBJ_WEAPON &&
            Weapon_info[Weapons[target_objp->instance].weapon_info_index]
                    .subtype == WP_MISSILE) { // must be a missile
            end = missile_obj_return_address (
                Weapons[target_objp->instance].missile_list_index);
        }
    }
    ...
    for (mo = start; mo != end; mo = advance_missile_obj (mo, next_flag)) {
        if (mo == &Missile_obj_list) { continue; }
        ...
        if (!(wip->wi_flags[Weapon::Info_Flags::Can_be_targeted])) {
            if (!(wip->wi_flags[Weapon::Info_Flags::Bomb])) { continue; }
        }

        if (wp->lssm_stage == 3) { continue; }

        // only allow targeting of hostile bombs
        if (!iff_x_attacks_y (Player_ship->team, obj_team (A))) { continue; }
    ...
    if (!target_found) {
        ...
        if ((aip->target_objnum != -1) &&
            ...
            ((Ship_info[Ships[Objects[aip->target_objnum].instance]
                            .ship_info_index]
                  .flags[Ship::Info_Flags::Bomber]) ||
             (Objects[aip->target_objnum]
                  .flags[Object::Object_Flags::Targetable_as_bomb]))) {
            int index =
                Ships[Objects[aip->target_objnum].instance].ship_list_index;
        ...
        for (so = advance_ship (startShip, next_flag); so != startShip;
             so = advance_ship (so, next_flag)) {
            A = &Objects[so->objnum];

            // don't look at header
            if (so == &Ship_obj_list) { continue; }
            ...
            if (!(Ship_info[Ships[A->instance].ship_info_index]
                      .flags[Ship::Info_Flags::Bomber]) &&
...
int hud_target_ship_can_be_scanned (ship* shipp) {
    ...
    if (shipp->flags[Ship::Ship_Flags::Cargo_revealed]) return 0;

    // allow ships with scannable flag set
    if (shipp->flags[Ship::Ship_Flags::Scannable]) return 1;

    // ignore ships that don't carry cargo
    if ((sip->class_type < 0) ||
        !(Ship_types[sip->class_type].flags[Ship::Type_Info_Flags::Scannable]))
        return 0;
...
void hud_target_uninspected_cargo (int next_flag) {
    ...
    if (Player_ai->target_objnum == -1) { start = &obj_used_list; }
    else {
        start = &Objects[Player_ai->target_objnum];
    }
    ...
    for (A = start2; A != start; A = advance_fb (A, next_flag)) {
        ...
        if (A == Player_obj || (A->type != OBJ_SHIP)) { continue; }

        shipp = &Ships[A->instance]; // get a pointer to the ship information

        if (should_be_ignored (shipp)) { continue; }

        // ignore all non-cargo carrying craft
        if (!hud_target_ship_can_be_scanned (shipp)) { continue; }
...
void hud_target_newest_ship () {
    ...
    if (Player_ai->target_objnum >= 0) {
        ...
        if (player_target_objp->type == OBJ_SHIP) {
            current_target_arrived_time =
                Ships[player_target_objp->instance].create_time;
        }
    }
    else {
    ...
    // If no target is selected, then simply target the closest uninspected
    // cargo
    if (Player_ai->target_objnum == -1 ||
        timestamp_elapsed (Target_newest_ship_timestamp)) {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if ((A == Player_obj) || (should_be_ignored (shipp))) continue;

        // ignore navbuoys
        if (Ship_info[shipp->ship_info_index]
                .flags[Ship::Info_Flags::Navbuoy]) {
            continue;
        }

        if (A == player_target_objp) { continue; }

        if (hud_target_invalid_awacs (A)) { continue; }

        if ((shipp->create_time >= newest_time) &&
            (shipp->create_time <= current_target_arrived_time)) {
...
void hud_target_live_turret (
    int next_flag, int auto_advance, int only_player_target) {
    ship_subsys* A;
    ...
    // If no target is selected, then simply target the closest (or facing)
    // turret
    int last_subsys_turret = FALSE;
    if (Player_ai->targeted_subsys != NULL) {
        if (Player_ai->targeted_subsys->system_info->type ==
            SUBSYSTEM_TURRET) {
    ...
    int get_closest_turret =
        (auto_advance || !last_subsys_turret ||
    ...
    for (A = GET_FIRST (&target_shipp->subsys_list);
         A != END_OF_LIST (&target_shipp->subsys_list); A = GET_NEXT (A)) {
        // get a turret
        if (A->system_info->type == SUBSYSTEM_TURRET) {
            ...
            if ((A->current_hits > 0) &&
                (A->weapons.num_primary_banks > 0 ||
                 A->weapons.num_secondary_banks > 0)) {
                if (!only_player_target ||
                    (A->turret_enemy_objnum == OBJ_INDEX (Player_obj))) {
                    vec3d gsubpos, vec_to_subsys;
                    float distance, dot;
                    // get world pos of subsystem and its distance
                    get_subsystem_world_pos (objp, A, &gsubpos);
                    distance = vm_vec_normalized_dir (
                        &vec_to_subsys, &gsubpos, &View_position);

                    // check if facing and in view
                    int facing = ship_subsystem_in_sight (
                        objp, A, &View_position, &gsubpos, 0);

                    if (!auto_advance && get_closest_turret &&
                        !only_player_target) {
                        // if within 3 degrees and not previous subsys, use
                        // subsys in front
                        dot = vm_vec_dot (
                            &vec_to_subsys, &Player_obj->orient.vec.fvec);
                    ...
                    if (turret_weapon_has_flags (
                            &A->weapons, Weapon::Info_Flags::Beam)) {
                        ent[num_live_turrets].type = TYPE_FACING_BEAM;
                    }
                    else if (turret_weapon_has_flags (
                                 &A->weapons, Weapon::Info_Flags::Flak)) {
                        ent[num_live_turrets].type = TYPE_FACING_FLAK;
                    }
                    else {
                        if (turret_weapon_has_subtype (
                                &A->weapons, WP_MISSILE)) {
                            ent[num_live_turrets].type = TYPE_FACING_MISSILE;
                        }
                        else if (turret_weapon_has_subtype (
                                     &A->weapons, WP_LASER)) {
    ...
    if (!use_straight_ahead_turret) {
        insertion_sort (
            ent, num_live_turrets, sizeof (eval_next_turret),
            turret_compare_func);
    }
    ...
    if (use_straight_ahead_turret) {
        ...
        // check if we have a currently targeted turret and find its position
        // after the sort
        int i, start_index, next_index;
        if (get_closest_turret) { start_index = 0; }
        else {
            start_index = -1;
        ...
        if (num_live_turrets == 0) {
            ...
            if (next_flag) {
                ...
                if (next_index == num_live_turrets) { next_index = 0; }
            }
            else {
                // go to next farther turret
                next_index = start_index - 1;
                if (next_index == -1) { next_index = num_live_turrets - 1; }
            }
    ...
    for (mo = GET_NEXT (&Missile_obj_list);
        ...
        if (wp->homing_object == locked_obj) {
            dist = vm_vec_dist_quick (
                &A->pos, &locked_obj->pos); // Find distance!
...
void hud_target_auto_target_next () {
    ...
    if (Game_mode & (GM_DEAD | GM_DEAD_BLEW_UP)) return;
    ...
    // um, ok.  Try targeting asteroids that are on a collision course for an
    // escort ship
    if (Player_ai->target_objnum == -1) { //-V581
...
float hud_find_target_distance (object* targetee, object* targeter) {
    ...
    case OBJ_SHIP:
        model_num =
            Ship_info[Ships[targetee->instance].ship_info_index].model_num;
        break;
    case OBJ_DEBRIS:
    ...
    case OBJ_WEAPON:
        ...
        // model_num =
        // Weapon_info[Weapons[targetee->instance].weapon_info_index].model_num;
        break;
    case OBJ_ASTEROID:
        // Don't find model_num since circles would work better
        // model_num =
        // Asteroid_info[Asteroids[targetee->instance].type].model_num;
        break;
    case OBJ_JUMP_NODE:
        // Don't find model_num since circles would work better
    ...
    if (model_num > -1) {
        dist = model_find_closest_point (
            &tmp_pnt, model_num, -1, &targetee->orient, &targetee->pos,
            &targeter->pos);
    }
    else {
        // Old way, that uses radius.
        dist = vm_vec_dist_quick (&targetee->pos, &targeter->pos) -
               targetee->radius;
...
bool evaluate_ship_as_closest_target (esct* esct_p) {
    ...
    if (objp->type != OBJ_SHIP) { return false; }
    ...
    if ((esct_p->shipp->objnum == OBJ_INDEX (Player_obj)) ||
        (should_be_ignored (esct_p->shipp))) {
        return false;
    }
    ...
    if (esct_p->filter) {
        if (!(Ship_info[esct_p->shipp->ship_info_index]
                  .is_fighter_bomber ())) {
            return false;
        }
    }
    ...
    if (Ship_info[esct_p->shipp->ship_info_index].is_big_or_huge ()) {
        for (ss = GET_FIRST (&esct_p->shipp->subsys_list);
             ss != END_OF_LIST (&esct_p->shipp->subsys_list);
             ss = GET_NEXT (ss)) {
            if (ss->flags[Ship::Subsystem_Flags::Untargetable]) continue;

            if ((ss->system_info->type == SUBSYSTEM_TURRET) &&
                (ss->current_hits > 0)) {
                if (esct_p->check_all_turrets ||
                    (ss->turret_enemy_objnum == esct_p->attacked_objnum)) {
                    turret_is_attacking = 1;
                    esct_p->check_nearest_turret = TRUE;

                    if (!esct_p->turret_attacking_target ||
                        (esct_p->turret_attacking_target &&
                         (ss->turret_enemy_objnum ==
                          esct_p->attacked_objnum))) {
                        vec3d gsubpos;
                        // get world pos of subsystem
                        vm_vec_unrotate (
                            &gsubpos, &ss->system_info->pnt, &objp->orient);
                        vm_vec_add2 (&gsubpos, &objp->pos);
                        new_distance =
                            vm_vec_dist_quick (&gsubpos, &Player_obj->pos);

                        /*
                        // GET TURRET TYPE, FAVOR BEAM, FLAK, OTHER
                        int turret_type = ss->system_info->turret_weapon_type;
                        if
                        (Weapon_info[turret_type].wi_flags[Weapon::Info_Flags::Beam])
                        { new_distance *= 0.3f; } else if
                        (Weapon_info[turret_type].wi_flags[Weapon::Info_Flags::Flak])
                        { new_distance *= 0.6f;
                        } */
    ...
    // If no turret is attacking, check if objp is actually targeting
    // attacked_objnum don't bail if targeting is for player
    if (!targeting_player && !turret_is_attacking) {
        ai_info* aip = &Ai_info[esct_p->shipp->ai_index];

        if (aip->target_objnum != esct_p->attacked_objnum) { return false; }

        if ((Game_mode & GM_NORMAL) && (aip->mode != AIM_CHASE) &&
            (aip->mode != AIM_STRAFE) && (aip->mode != AIM_EVADE) &&
            (aip->mode != AIM_EVADE_WEAPON) && (aip->mode != AIM_AVOID)) {
            return false;
        }
    }
...
int hud_target_closest (
    int team_mask, int attacked_objnum, int play_fail_snd, int filter,
    ...
    if ((attacked_objnum >= 0) && (attacked_objnum != player_obj_index)) {
        ...
        if (Player_ai->target_objnum == -1) { goto Target_closest_done; }
    ...
    if (attacked_objnum == -1) { attacked_objnum = player_obj_index; }
    ...
    eval_ship_as_closest_target_args.turret_attacking_target =
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (eval_ship_as_closest_target_args.min_distance < min_distance) {
            ...
            nearest_obj = A;
            check_nearest_turret =
                eval_ship_as_closest_target_args.check_nearest_turret;
        }
    }
...
Target_closest_done:
    ...
    if (target_found) {
        ...
        float dist = vm_vec_dist_quick(&Objects[attacked_objnum].pos,
    &nearest_obj->pos);

        // no distance limit for player obj
        if ((attacked_objnum != player_obj_index) && (dist >
    MIN_DISTANCE_TO_CONSIDER_THREAT)) { target_found = FALSE;
        }
    } */
    ...
    if (target_found) {
        ...
        if (check_nearest_turret) {
            ...
            ss = Ships[nearest_obj->instance]
                     .last_targeted_subobject[Player_num];
            if (ss == NULL || get_closest_turret_attacking_player) {
                // update nearest turret with later func
                hud_target_live_turret (
                    1, 1, get_closest_turret_attacking_player);
                Ships[nearest_obj->instance]
                    .last_targeted_subobject[Player_num] =
                    Player_ai->targeted_subsys;
            }
        }
...
void hud_update_closest_turret () {
    ...
    /*
        ...
        Assert(Ship_info[shipp->ship_info_index].flags &
       (SIF_BIG_SHIP|SIF_HUGE_SHIP));

        for (ss=GET_FIRST(&shipp->subsys_list);
       ss!=END_OF_LIST(&shipp->subsys_list); ss=GET_NEXT(ss)) { if (
       (ss->system_info->type == SUBSYSTEM_TURRET) && (ss->current_hits > 0) )
       {
                // make sure turret is not "unused"
                if (ss->system_info->turret_weapon_type >= 0) {
                    vec3d gsubpos;
                    // get world pos of subsystem
                    vm_vec_unrotate(&gsubpos, &ss->system_info->pnt,
       &objp->orient); vm_vec_add2(&gsubpos, &objp->pos); new_distance =
       vm_vec_dist_quick(&gsubpos, &Player_obj->pos);
       ...
       (Weapon_info[turret_type].wi_flags[Weapon::Info_Flags::Flak]) {
                    ...
                    // check if facing and in view
                    int facing = ship_subsystem_in_sight(objp, ss,
       &View_position, &gsubpos, 0);
        ...
        if (Player_ai->targeted_subsys != NULL) {
            set_targeted_subsys(Player_ai, closest_subsys,
       Player_ai->target_objnum); shipp->last_targeted_subobject[Player_num] =
       Player_ai->targeted_subsys;
        }
        */
}
...
void hud_target_targets_target () {
    ...
    objp = &Objects[Player_ai->target_objnum];
    if (objp->type != OBJ_SHIP) { goto ttt_fail; }

    tt_objnum = Ai_info[Ships[objp->instance].ai_index].target_objnum;
    if (tt_objnum < 0 || tt_objnum >= MAX_OBJECTS) { goto ttt_fail; }

    if (tt_objnum == OBJ_INDEX (Player_obj)) { goto ttt_fail; }

    tt_objp = &Objects[tt_objnum];

    if (hud_target_invalid_awacs (tt_objp)) { goto ttt_fail; }

    if (tt_objp->type != OBJ_SHIP) { goto ttt_fail; }

    if (should_be_ignored (&Ships[tt_objp->instance])) { goto ttt_fail; }
    ...
    if (Objects[tt_objnum].type == OBJ_SHIP) {
        hud_maybe_set_sorted_turret_subsys (
            &Ships[Objects[tt_objnum].instance]);
    }
...
// Return !0 if target_objp is a valid object type for targeting in reticle,
// otherwise return 0
int object_targetable_in_reticle (object* target_objp) {
    int obj_type;
    std::list< CJumpNode >::iterator jnp;

    if (target_objp == Player_obj) { return 0; }

    obj_type = target_objp->type;

    if ((obj_type == OBJ_SHIP) || (obj_type == OBJ_DEBRIS) ||
        (obj_type == OBJ_WEAPON) || (obj_type == OBJ_ASTEROID)) {
        return 1;
    }
    else if (obj_type == OBJ_JUMP_NODE) {
        for (jnp = Jump_nodes.begin (); jnp != Jump_nodes.end (); ++jnp) {
            if (jnp->GetSCPObject () == target_objp) break;
        }

        if (!jnp->IsHidden ()) return 1;
    }

    return 0;
}

// hud_target_in_reticle_new() will target the object that is closest to the
// player, and who is intersected by a ray passed from the center of the
// reticle out along the forward vector of the player.
//
// targeting of objects of type OBJ_SHIP and OBJ_DEBRIS are supported
//
// Method: A ray is cast from the center of the reticle, and we keep track of
// any eligible object
// the ray intersects.  We take the ship closest to us that intersects
// an object.
//
// Since this method may work poorly with objects that are far away,
// hud_target_in_reticle_old() is called if no intersections are found.
//
//
...
void hud_target_in_reticle_new () {
    ...
    vm_vec_scale_add (
        &terminus, &Eye_position, &Player_obj->orient.vec.fvec,
    ...
    for (A = GET_FIRST (&obj_used_list); A != END_OF_LIST (&obj_used_list);
         A = GET_NEXT (A)) {
        if (!object_targetable_in_reticle (A)) { continue; }
        ...
        case OBJ_SHIP:
            mc.model_num =
                Ship_info[Ships[A->instance].ship_info_index].model_num;
            break;
        case OBJ_DEBRIS: mc.model_num = Debris[A->instance].model_num; break;
        case OBJ_WEAPON:
            mc.model_num =
        ...
        case OBJ_ASTEROID: {
            ...
            mc.model_num = Asteroid_info[Asteroids[A->instance].asteroid_type]
                               .model_num[pof];
        } break;
        case OBJ_JUMP_NODE:
            for (jnp = Jump_nodes.begin (); jnp != Jump_nodes.end (); ++jnp) {
        ...
        if (mc.model_num == -1) {
            ...
            if (mc.num_hits) {
                dist =
                    vm_vec_dist_squared (&mc.hit_point_world, &Eye_position);
                hud_reticle_list_update (A, dist, 0);
            }
        }
    } // end for (go to next object)

    hud_target_in_reticle_old (); // try the old method (works well with ships
                                  // far away)
}

// hud_target_in_reticle_old() will target the object that is closest to the
// reticle center and inside the reticle
//
// targeting of objects of type OBJ_SHIP and OBJ_DEBRIS are supported
//
//
// Method:  take the dot product of the foward vector and the vector to target.
// Take
...
void hud_target_in_reticle_old () {
    ...
    for (A = GET_FIRST (&obj_used_list); A != END_OF_LIST (&obj_used_list);
        ...
        if (dot > MIN_DOT_FOR_TARGET) { hud_reticle_list_update (A, dot, 1); }
    }
...
// hud_target_subsystem_in_reticle() will target the subsystem that is within
// the reticle and is closest to the reticle center.  The current target is the
// only object that is searched for subsystems
//
// Method:  take the dot product of the foward vector and the vector to target.
// Take
// the one that is closest to 1 and at least MIN_DOT_FOR_TARGET
...
void hud_target_subsystem_in_reticle () {
    ...
    if (Player_ai->target_objnum == -1) { hud_target_in_reticle_old (); }
    ...
    if (targetp->type == OBJ_SHIP) {
        if (should_be_ignored (&Ships[shipnum])) { return; }
    }

    for (subsys = GET_FIRST (&Ships[shipnum].subsys_list);
         subsys != END_OF_LIST (&Ships[shipnum].subsys_list);
         subsys = GET_NEXT (subsys)) {
        // if the subsystem isn't targetable, skip it
        if (subsys->flags[Ship::Subsystem_Flags::Untargetable]) continue;
        ...
        if (dot > best_dot) {
            best_dot = dot;
            if (best_dot > MIN_DOT_FOR_TARGET) nearest_subsys = subsys;
        }
    } // end for
    ...
    if (nearest_subsys != NULL) {
        ...
        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN, XSTR ("Targeting subsystem %s.", 323), r_name);
        Ships[shipnum].last_targeted_subobject[Player_num] =
            Player_ai->targeted_subsys;
    }
    else {
        snd_play (gamesnd_get_game_sound (GameSounds::TARGET_FAIL));
...
// On entry:
// color set
void HudGaugeOrientationTee::renderOrientation (
    object* from_objp, object* to_objp, matrix* from_orientp) {
    float dot_product;
    vec3d target_to_obj;
    ...
    // calculate the dot product between the target_to_player vector and the
    // targets forward vector
    //
    // 0 - vectors are perpendicular
    // 1 - vectors are collinear and in the same direction (target is facing
    // player) -1 - vectors are collinear and in the opposite direction (target
    // is facing away from player)
    dot_product = vm_vec_dot (&from_orientp->vec.fvec, &target_to_obj);

    if (vm_vec_dot (&from_orientp->vec.rvec, &target_to_obj) >= 0) {
        if (dot_product >= 0) { dot_product = -PI_2 * dot_product + PI; }
        else {
...
void hud_tri (float x1, float y1, float x2, float y2, float x3, float y3) {
    ...
    g3_render_primitives_colored (
        &material_def, verts, 3, PRIM_TYPE_TRIFAN, true);

    // g3_draw_poly_constant_sw(3, vertlist, TMAP_FLAG_GOURAUD | TMAP_FLAG_RGB
    // | TMAP_FLAG_ALPHA, 0.1f);
...
HudGaugeReticleTriangle::HudGaugeReticleTriangle ()
    : HudGauge (
          HUD_OBJECT_HOSTILE_TRI, HUD_HOSTILE_TRIANGLE, false, false,
          VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
              VM_OTHER_SHIP,
          255, 255, 255) {}

HudGaugeReticleTriangle::HudGaugeReticleTriangle (
    int _gauge_object, int _gauge_config)
    : HudGauge (
          _gauge_object, _gauge_config, true, false,
          VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
              VM_OTHER_SHIP,
          255, 255, 255) {}
...
void HudGaugeReticleTriangle::renderTriangleMissileTail (
    float ang, float xpos, float ypos, float cur_dist, int draw_solid,
    int draw_inside) {
    float x1 = 0.0f;
    float x2 = 0.0f;
    float y1 = 0.0f;
    ...
    if (cur_dist < Min_warning_missile_dist) { tail_len = 0.0f; }
    ...
    if (draw_solid) { hud_tri (xpos, ypos, x1, y1, x2, y2); }
    else {
...
void HudGaugeReticleTriangle::renderTriangle (
    vec3d* hostile_pos, int aspect_flag, int show_interior, int split_tri) {
    ...
    if (hostile_vertex.codes == 0) { // on screen
        ...
        if (!(hostile_vertex.flags &
            ...
            mag_squared =
                (projected_x - position[0]) * (projected_x - position[0]) +
                (projected_y - position[1]) * (projected_y - position[1]);

            if (mag_squared < Radius * Radius) {
                if (show_interior) { draw_inside = 1; }
                else {
                    return;
                }
            }
    ...
    if (split_tri) {
        // renderTriangleMissileSplit(ang, xpos, ypos, cur_dist, aspect_flag,
        // draw_inside);
        renderTriangleMissileTail (
            ang, xpos, ypos, cur_dist, aspect_flag, draw_inside);
    }
    else {
        float x1 = 0.0f;
        float x2 = 0.0f;
        ...
        // renderPrintf(position[0], position[1], "%d",
        // fl2i((360*ang)/(2*PI)));
        gr_set_screen_scale (base_w, base_h);
        if (aspect_flag) { hud_tri (xpos, ypos, x1, y1, x2, y2); }
        else {
            hud_tri_empty (xpos, ypos, x1, y1, x2, y2);
        }
        gr_reset_screen_scale ();
...
void hud_process_homing_missiles () {
    ...
    if (nearest_dist < Homing_beep.max_cycle_dist) {
        ...
        cycle_time = Homing_beep.min_cycle_time +
                     (nearest_dist - Homing_beep.min_cycle_dist) *
                         Homing_beep.precalced_interp;
            ...
            if (closest_is_aspect) {
                Homing_beep.snd_handle =
                    snd_play (gamesnd_get_game_sound (ship_get_sound (
                        Player_obj, GameSounds::PROXIMITY_ASPECT_WARNING)));
            }
            else {
                Homing_beep.snd_handle =
                    snd_play (gamesnd_get_game_sound (ship_get_sound (
                        Player_obj, GameSounds::PROXIMITY_WARNING)));
            }
        }
    }
}
...
HudGaugeMissileTriangles::HudGaugeMissileTriangles ()
    : HudGaugeReticleTriangle (
          HUD_OBJECT_MISSILE_TRI, HUD_MISSILE_WARNING_ARROW) {}
...
void HudGaugeMissileTriangles::render (float /*frametime*/) {
    ...
    for (mo = GET_NEXT (&Missile_obj_list);
        ...
        if (wp->homing_object == Player_obj) {
            renderTriangle (
                &A->pos,
                Weapon_info[wp->weapon_info_index].is_locked_homing (), 1, 1);
        }
    }

    gr_reset_screen_scale ();
    if (!in_frame) g3_end_frame ();
}

HudGaugeOrientationTee::HudGaugeOrientationTee ()
    : HudGauge (
          HUD_OBJECT_ORIENTATION_TEE, HUD_ORIENTATION_TEE, true, false,
          VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
              VM_OTHER_SHIP,
          255, 255, 255) {}
...
// hud_show_orientation_tee() will draw the orientation gauge that orbits the
// inside of the outer reticle ring.  If the T is at 12 o'clock, the target is
// facing the player, if the T is at 6 o'clock the target is facing away from
// the player.  If the T is at 3 or 9 o'clock the target is facing 90 away from
// the player.
void HudGaugeOrientationTee::render (float /*frametime*/) {
    object* targetp;

    if (Player_ai->target_objnum == -1 || Player->target_is_dying) return;

    targetp = &Objects[Player_ai->target_objnum];

    if (maybeFlashSexp () == 1) { hud_set_iff_color (targetp); }
    else {
...
// routine to draw a bounding box around a remote detonate missile and distance
// to
void hud_process_remote_detonate_missile () {
    ...
    for (mo = GET_FIRST (&Missile_obj_list);
        ...
        if ((Player_obj != NULL) &&
            (mobjp->parent_sig == Player_obj->parent_sig)) {
            if (Weapon_info[Weapons[mobjp->instance].weapon_info_index]
                ...
                if (!(target_point.flags &
                      PF_OVERFLOW)) { // make sure point projected
                    switch (mobjp->type) {
                    case OBJ_WEAPON:
                        hud_target_add_display_list (
                            mobjp, &target_point, &mobjp->pos, 0,
                            iff_get_color (IFF_COLOR_MESSAGE, 1), NULL,
                            TARGET_DISPLAY_DIST);
                        break;
...
// routine to possibly draw a bounding box around a ship sending a message to
// the player
void hud_show_message_sender () {
    ...
    if (Message_shipnum == -1) return;

    targetp = &Objects[Ships[Message_shipnum].objnum];
    ASSERT (targetp != NULL);

    if (targetp->type != OBJ_SHIP) {
        // if it's not a ship (maybe it got ship-vanished in the middle of
        // talking) then clear Message_shipnum
        Message_shipnum = -1;
        return;
    }

    // Don't do this for the ship you're flying!
    if (targetp == Player_obj) { return; }

    // Goober5000 - don't draw if primitive sensors
    if (Ships[Player_obj->instance]
            .flags[Ship::Ship_Flags::Primitive_sensors]) {
        return;
    }

    // Karajorma - If we've gone to all the trouble to make our friendly ships
    // stealthed they shouldn't then give away their position cause they're
    // feeling chatty MageKing17 - Make the check see if they're actually
    // stealthed at the time, and may as well include a check for being hidden
    // from sensors, too; logic copied from a similar check in hudescort.cpp
    if ((Ships[Message_shipnum]
    ...
    // check the object flags to see if this ship is gone.  If so, then don't
    ...
    if (!(target_point.flags & PF_OVERFLOW)) { // make sure point projected
        hud_target_add_display_list (
            targetp, &target_point, &targetp->pos, 10,
            iff_get_color (IFF_COLOR_MESSAGE, 1), NULL, 0);
    }
    else if (target_point.codes != 0) { // target center is not on screen
        // draw the offscreen indicator at the edge of the screen where the
        // target is closest to AL 11-19-97: only show offscreen indicator if
        // player sensors are functioning
        if ((OBJ_INDEX (targetp) != Player_ai->target_objnum) ||
            ...
            if (hud_sensors_ok (Player_ship, 0)) {
                hud_target_add_display_list (
                    targetp, &target_point, &targetp->pos, 0,
                    iff_get_color (IFF_COLOR_MESSAGE, 1), NULL,
                    TARGET_DISPLAY_DIST);
            }
        }
...
// hud_prune_hotkeys()
//
// Check for ships that are dying, departed or dead.  These should be removed
// from the player's hotkey lists.
void hud_prune_hotkeys () {
    ...
    for (i = 0; i < MAX_KEYED_TARGETS; i++) {
        ...
        while (hitem != END_OF_LIST (plist)) {
            ...
            if (objp->type == OBJ_SHIP) {
                ...
                // if the object isn't a ship, it shouldn't be on the list, so
                // remove it without question
                remove_item = 1;
                sp = NULL;
            }

            // check to see if the object is dying -- if so, remove it from the
            // list check to see if the ship is departing -- if so, remove it
            // from the list
            if (remove_item ||
                (objp->flags[Object::Object_Flags::Should_be_dead]) ||
                (sp->is_dying_or_departing ())) {
                if (sp != NULL) {
                    WARNINGF (LOCATION, "Hotkey: Pruning %s", sp->ship_name);
                }
    ...
    // save the hotkey sets with mission time reaches a certain point.  Code
    // was put here because this function always called for both
    // single/multiplayer.  Maybe not the best location, but whatever.
    mission_hotkey_maybe_save_sets ();
}

int HUD_drew_selection_bracket_on_target;

// hud_show_selection_set draws some indicator around all the ships in the
// current selection set.  No indicators will be drawn if there is only 1 ship
// in the set.
void hud_show_selection_set () {
    ...
    for (hitem = GET_FIRST (plist); hitem != END_OF_LIST (plist);
        ...
        // Goober5000 - don't draw indicators for non-visible ships, per Mantis
        // #1972 (the only way we could get here is if the hotkey set contained
        // a mix of visible and invisible ships)
        if (awacs_get_level (targetp, Player_ship, 1) < 1) { continue; }
        ...
        if (!(target_point.flags & PF_OVERFLOW)) { // make sure point projected
            ...
            if (OBJ_INDEX (targetp) == Player_ai->target_objnum) {
                hud_target_add_display_list (
                    targetp, &target_point, &targetp->pos, 5,
                    iff_get_color (IFF_COLOR_SELECTION, 1), NULL, 0);
                HUD_drew_selection_bracket_on_target = 1;
            }
            else if (Cmdline_targetinfo) { // Backslash -- show the distance
                                           // and a lead indicator
                hud_target_add_display_list (
                    targetp, &target_point, &targetp->pos, 5,
        ...
        if (target_point.codes != 0) { // target center is not on screen
            // draw the offscreen indicator at the edge of the screen where the
            // target is closest to AL 11-19-97: only show offscreen indicator
            // if player sensors are functioning
...
// hud_show_targeting_gauges() will display the targeting information on the
// HUD.  Called once per frame.
//
// Must be inside a g3_start_frame()
...
void hud_show_targeting_gauges (float frametime) {
    ...
    if (targetp == &obj_used_list) { return; }

    // AL 1/20/97: Point to targted subsystem if one exists
    if (Player_ai->targeted_subsys != NULL) {
        get_subsystem_world_pos (
            targetp, Player_ai->targeted_subsys, &target_pos);
    }
    else {
    ...
    // find the current target vertex
    //
    // The 2D screen pos depends on the current viewer position and
    // orientation.
    ...
    if (!(target_point.flags & PF_OVERFLOW)) { // make sure point projected
        if (target_point.codes == 0) { // target center is not on screen
            ...
            if (Cmdline_targetinfo) {
                target_display_flags =
                    TARGET_DISPLAY_DIST | TARGET_DISPLAY_DOTS |
                    TARGET_DISPLAY_SUBSYS | TARGET_DISPLAY_NAME |
                    TARGET_DISPLAY_CLASS;
            }
            else {
                target_display_flags = TARGET_DISPLAY_DIST |
                                       TARGET_DISPLAY_DOTS |
                                       TARGET_DISPLAY_SUBSYS;
            }

            hud_target_add_display_list (
                targetp, &target_point, &targetp->pos, 0, NULL, NULL,
                target_display_flags);
        }
    }
    else {
    ...
    if (target_point.codes != 0) {
        ...
        // AL 11-11-97: don't draw the indicator if the ship is messaging, the
        // indicator is drawn in the message sending color in
        // hud_show_message_sender()
        if (Message_shipnum != Objects[Player_ai->target_objnum].instance) {
            hud_target_add_display_list (
                targetp, &target_point, &targetp->pos, 0, NULL, NULL, 0);
        }
    }
}

// hud_show_hostile_triangle() will draw an empty triangle that oribits around
// the outer circle of the reticle.  It will point to the closest enemy that is
// firing on the player. Currently, it points to the closest enemy that has the
// player as its target_objnum and has SM_ATTACK or SM_SUPER_ATTACK as its ai
// submode.
void hud_show_hostile_triangle () {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if ((A == Player_obj) ||
            !(iff_x_attacks_y (Ships[A->instance].team, Player_ship->team))) {
            continue;
        }
        ...
        if (should_be_ignored (sp)) { continue; }
        ...
        // check if any turrets on ship are firing at the player (only on non
        // fighter-bombers)
            ...
            for (ss = GET_FIRST (&sp->subsys_list);
                ...
                if ((ss->system_info->type == SUBSYSTEM_TURRET) &&
                    ...
                    if (ss->turret_enemy_objnum == player_obj_index) {
                        ...
                        // get world pos of subsystem
                        vm_vec_unrotate (
                            &gsubpos, &ss->system_info->pnt, &A->orient);
        ...
        if (!turret_is_attacking) {
            ...
            if (aip->target_objnum != Player_ship->objnum) { continue; }
    ...
    if (nearest_obj == &obj_used_list) { return; }

    if (min_distance > MIN_DISTANCE_TO_CONSIDER_THREAT) { return; }
...
void hud_calculate_lead_pos (
    vec3d* lead_target_pos, vec3d* target_pos, object* targetp,
    weapon_info* wip, float dist_to_target, vec3d* rel_pos) {
    ...
    if (The_mission.ai_profile
        ...
        vm_vec_scale_sub2 (
            &target_moving_direction, &Player_obj->phys_info.vel,
    ...
    if (vm_vec_mag_quick (&target_moving_direction) < 0.1f) { // Find distance!
        *lead_target_pos = *target_pos;
        ...
        polish_predicted_target_pos (
            wip, targetp, target_pos, lead_target_pos, dist_to_target,
            &last_delta_vector, 1); // Not used:, float time_to_enemy)

        if (rel_pos) { // needed for quick lead indicators, not needed for
                       // normal lead indicators.
            vm_vec_add2 (lead_target_pos, rel_pos);
        }
    }
}

// Return the bank number for the primary weapon that can fire the farthest,
// from the number of active primary weapons input: range       =>      output
// parameter... it is the range of the selected bank
int hud_get_best_primary_bank (float* range) {
    ...
    for (i = 0; i < num_to_test; i++) {
        ...
        // calculate the range of the weapon, and only display the lead target
        // indicator if the weapon can actually hit the target
        ASSERT (bank_to_fire >= 0 && bank_to_fire < swp->num_primary_banks);
        ASSERT (swp->primary_bank_weapons[bank_to_fire] < MAX_WEAPON_TYPES);

        if (swp->primary_bank_weapons[bank_to_fire] < 0) continue;

        wip = &Weapon_info[swp->primary_bank_weapons[bank_to_fire]];
        weapon_range =
            MIN ((wip->max_speed * wip->lifetime), wip->weapon_range);

        // don't consider this primary if it's a ballistic that's out of ammo -
        // Goober5000
        if (wip->wi_flags[Weapon::Info_Flags::Ballistic]) {
            if (swp->primary_bank_ammo[bank_to_fire] <= 0) { continue; }
        }
...
// polish_predicted_target_pos()
//
// Called by the draw lead indicator code to predict where the enemy is going
// to be
//
void polish_predicted_target_pos (
    weapon_info* wip, object* targetp, vec3d* enemy_pos,
    ...
    if (The_mission.ai_profile
            ->flags[AI::Profile_Flags::Use_additive_weapon_velocity]) {
        vm_vec_scale_sub2 (
            &enemy_vel, &Player_obj->phys_info.vel, wip->vel_inherit_amount);
    }
    ...
    for (iteration = 0; iteration < num_polish_steps; iteration++) {
        ...
        vm_vec_scale_add (
            predicted_enemy_pos, enemy_pos, &enemy_vel, time_to_enemy);
        vm_vec_sub (
            last_delta_vec, predicted_enemy_pos, &last_predicted_enemy_pos);
...
HudGaugeLeadIndicator::HudGaugeLeadIndicator ()
    : HudGauge (
          HUD_OBJECT_LEAD, HUD_LEAD_INDICATOR, false, false,
          VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
              VM_OTHER_SHIP,
          255, 255, 255) {}
...
void HudGaugeLeadIndicator::initBitmaps (char* fname) {
    Lead_indicator_gauge.first_frame =
        bm_load_animation (fname, &Lead_indicator_gauge.num_frames);
...
void HudGaugeLeadIndicator::pageIn () {
    bm_page_in_aabitmap (
        Lead_indicator_gauge.first_frame, Lead_indicator_gauge.num_frames);
}
...
int HudGaugeLeadIndicator::pickFrame (
    float prange, float srange, float dist_to_target) {
    int frame_offset = -1;
    int in_prange = 0, in_srange = 0;

    if (dist_to_target < prange) { in_prange = 1; }

    if (dist_to_target < srange) { in_srange = 1; }

    if (in_prange && in_srange) { frame_offset = 1; }
...
void HudGaugeLeadIndicator::render (float /*frametime*/) {
    if (Player->target_is_dying) { return; }
    ...
    // if extra targeting info is enabled, render lead indicators for objects
    // in the target display list.
    for (size_t i = 0; i < target_display_list.size (); i++) {
        if ((target_display_list[i].flags & TARGET_DISPLAY_LEAD) &&
            target_display_list[i].objp) {
            // set the color
            if (target_display_list[i].bracket_clr.red &&
                target_display_list[i].bracket_clr.green &&
            ...
            renderLeadQuick (
                &target_display_list[i].target_pos,
                target_display_list[i].objp);
        }
    }
...
void HudGaugeLeadIndicator::renderIndicator (
    int frame_offset, object* targetp, vec3d* lead_target_pos) {
    ...
    if (lead_target_vertex.codes == 0) { // on screen
        ...
        if (!(lead_target_vertex.flags & PF_OVERFLOW)) {
            if (maybeFlashSexp () == 1) { hud_set_iff_color (targetp, 0); }
            else {
                hud_set_iff_color (targetp, 1);
            }
...
// HudGaugeLeadIndicator::renderTargetLead() determine where to draw the lead
// target box and display it
void HudGaugeLeadIndicator::renderLeadCurrentTarget () {
    ...
    targetp = &Objects[Player_ai->target_objnum];
    if ((targetp->type != OBJ_SHIP) && (targetp->type != OBJ_WEAPON) &&
        (targetp->type != OBJ_ASTEROID)) {
        return;
    }
    ...
    // Added to take care of situation where there are no primary banks on the
    // player ship (this may not be possible, depending on what we decide for
    // the weapons loadout rules)
    if (swp->num_primary_banks == 0) return;

    bank_to_fire = hud_get_best_primary_bank (&prange);

    if (bank_to_fire < 0) return;
    ...
    // gun turret of the currently selected primary weapon.
    ...
    // (playing it safe, will usually be in range at slightly further away
    // due to subsys radius)
    ...
    if ((swp->current_secondary_bank >= 0) &&
        ...
        if (!(tmp->is_homing ()) &&
            !(tmp->is_locked_homing () && Player->target_in_lock_cone)) {
            // The secondary lead indicator is handled farther below if it is a
            // non-locking type
            srange = -1.0f;
        }
    }
    ...
    if (frame_offset < 0) { return; }

    hud_calculate_lead_pos (
        &lead_target_pos, &target_pos, targetp, wip, dist_to_target);
    renderIndicator (frame_offset, targetp, &lead_target_pos);

    // do dumbfire lead indicator - color is orange (255,128,0) - bright,
    // (192,96,0) - dim phreak changed 9/01/02
    if ((swp->current_secondary_bank >= 0) &&
        (swp->secondary_bank_weapons[swp->current_secondary_bank] >= 0)) {
        int bank = swp->current_secondary_bank;
        wip = &Weapon_info[swp->secondary_bank_weapons[bank]];

        // get out of here if the secondary weapon is a homer or if its out of
        // range
        if (wip->is_homing ()) return;

        double max_dist =
            MIN ((wip->lifetime * wip->max_speed), wip->weapon_range);

        if (dist_to_target > max_dist) return;
    }
...
// Ideally I'd like to later turn this into something (or make a new function)
// that would actually WORK with gun convergence/normals instead of the
// existing code (copied from above) that does some calculations and then is
// ignored ;-) (Go look, what's it actually DO with source_pos?) And also,
// something that could be called for multiple weapons, ITTS style.
void HudGaugeLeadIndicator::renderLeadQuick (
    vec3d* target_world_pos, object* targetp) {
    vec3d source_pos;
    vec3d* rel_pos;
    vec3d lead_target_pos;
    polymodel* pm;
    ...
    bank_to_fire = hud_get_best_primary_bank (&prange); // Backslash note:
                                                        // this!
    if (bank_to_fire < 0) return;
    wip = &Weapon_info[swp->primary_bank_weapons[bank_to_fire]];
    ...
    // Determine "accurate" distance to target.  This is the distance from the
    // player ship to the closest point on the bounding box of the target
    dist_to_target = hud_find_target_distance (targetp, Player_obj);
    ...
    hud_calculate_lead_pos (
        &lead_target_pos, target_world_pos, targetp, wip, dist_to_target,
        rel_pos);
    renderIndicator (frame_offset, targetp, &lead_target_pos);
}
...
HudGaugeLeadSight::HudGaugeLeadSight ()
    : HudGauge (
          HUD_OBJECT_LEAD_SIGHT, HUD_LEAD_INDICATOR, true, false,
          VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
              VM_OTHER_SHIP,
          255, 255, 255) {}
...
void HudGaugeLeadSight::renderSight (
    int frame_offset, vec3d* target_pos, vec3d* lead_target_pos) {
    ...
    if (lead_target_vertex.codes != 0) return;

    g3_project_vertex (&lead_target_vertex);

    if (lead_target_vertex.flags & PF_OVERFLOW) return;
    ...
    if (target_vertex.codes != 0) return;

    g3_project_vertex (&target_vertex);

    if (target_vertex.flags & PF_OVERFLOW) return;
    ...
    if (Lead_sight.first_frame >= 0) {
        ...
        float reticle_target_sx =
            target_sx - Lead_sight_half[0] - target_lead_sx;
        float reticle_target_sy =
            target_sy - Lead_sight_half[1] - target_lead_sy;
...
void HudGaugeLeadSight::render (float /*frametime*/) {
    ...
    if (dist_to_target < prange) {
        ...
        renderSight (
            1, &target_pos,
            &lead_target_pos); // render the primary weapon lead sight
    ...
    renderSight (
        0, &target_pos,
        &lead_target_pos); // now render the secondary weapon lead sight

    if (!in_frame) { g3_end_frame (); }
}

// hud_cease_subsystem_targeting() will cease targeting the current targets
// subsystems
//
void hud_cease_subsystem_targeting (int print_message) {
    ...
    if (print_message) {
        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN, "%s",
            XSTR ("Deactivating sub-system targeting", 324));
    }
...
void hud_cease_targeting () {
    ...
    HUD_sourced_printf (
        HUD_SOURCE_HIDDEN, "%s", XSTR ("Deactivating targeting system", 325));
    hud_lock_reset ();
}
...
void hud_restore_subsystem_target (ship* shipp) {
    ...
    if (shipp->last_targeted_subobject[Player_num] != NULL) {
        Player_ai->targeted_subsys =
            shipp->last_targeted_subobject[Player_num];
...
// --------------------------------------------------------------------------------
// get_subsystem_world_pos() returns the world position for a given subsystem
// on a ship
//
vec3d* get_subsystem_world_pos (
...
void hud_target_change_check () {
    ...
    // Check if player subsystem target has changed, and reset necessary player
    // flag
    ...
    if (Player_ai->last_target != Player_ai->target_objnum) {
        if (Player_ai->target_objnum != -1) {
            snd_play (
                gamesnd_get_game_sound (
                    ship_get_sound (Player_obj, GameSounds::TARGET_ACQUIRE)),
                0.0f);
        }

        // if we have a hotkey set active, see if new target is in set.  If not
        // in set, deselect the current hotkey set.
        if (Player->current_hotkey_set != -1) {
            htarget_list *hitem, *plist;
        ...
        if (Players[Player_num].flags & PLAYER_FLAGS_AUTO_MATCH_SPEED) {
            ...
            if (Players[Player_num].flags & PLAYER_FLAGS_MATCH_TARGET)
                Players[Player_num].flags &=
                    ~PLAYER_FLAGS_MATCH_TARGET; // no more target matching.
        }
        ...
        if (Player_ai->current_target_distance < Player_ai->last_dist - 0.01) {
            ...
            Player_ai->current_target_distance > Player_ai->last_dist + 0.01) {
...
void HudGaugeTargetTriangle::render (float /*frametime*/) {
    ...
    // draw the targeting triangle that orbits the outside of the outer circle
    // of the reticle
...
void hud_maybe_flash_weapon (int index) {
    ...
    if (!timestamp_elapsed (Weapon_flash_info.flash_duration[index])) {
        if (timestamp_elapsed (Weapon_flash_info.flash_next[index])) {
            Weapon_flash_info.flash_next[index] =
                timestamp (TBOX_FLASH_INTERVAL);
            Weapon_flash_info.is_bright ^= (1 << index);
        }
...
int hud_sensors_ok (ship* sp, int show_msg) {
    ...
    if (emp_active_local ()) { return 0; }

    // ensure targeting functions are not disabled through damage
    sensors_str = ship_get_subsystem_strength (sp, SUBSYSTEM_SENSORS);
    if ((sensors_str < MIN_SENSOR_STR_TO_TARGET) ||
        (ship_subsys_disrupted (sp, SUBSYSTEM_SENSORS))) {
        if (show_msg) {
            HUD_sourced_printf (
                HUD_SOURCE_HIDDEN, "%s",
                XSTR ("Targeting is disabled due to sensors damage", 330));
            snd_play (gamesnd_get_game_sound (GameSounds::TARGET_FAIL));
        }
        return 0;
    }
...
int hud_communications_state (ship* sp) {
    ...
    if (Game_skill_level == 0 || (Game_mode & GM_DEAD)) { return COMM_OK; }

    // Goober5000 - if the ship is the player, and he's dying, return OK (so
    // laments can be played)
...
void hud_target_next_list (
    int hostile, int next_flag, int team_mask, int attacked_objnum,
    int play_fail_snd, int filter, int get_closest_turret_attacking_player) {
    ...
    if (hostile) {
        ...
        // everyone hates a traitor including other traitors so the friendly
        // target option shouldn't work for them
    ...
    if (Player_ai->target_objnum == -1 || timestamp_elapsed (timestamp_val)) {
        hud_target_closest (
            valid_team_mask, attacked_objnum, play_fail_snd, filter,
            get_closest_turret_attacking_player);
        return;
    }

    object* nearest_object = select_next_target_by_distance (
        (next_flag != 0), valid_team_mask, attacked_objnum);
...
HudGaugeAutoTarget::HudGaugeAutoTarget ()
    : HudGauge (
          HUD_OBJECT_AUTO_TARGET, HUD_AUTO_TARGET, false, false,
          (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
           VM_OTHER_SHIP),
          255, 255, 255) {}
...
void HudGaugeAutoTarget::initBitmaps (char* fname) {
    Toggle_frame.first_frame =
        bm_load_animation (fname, &Toggle_frame.num_frames);
...
void HudGaugeAutoTarget::render (float /*frametime*/) {
    if (Player_ship->flags[Ship::Ship_Flags::Primitive_sensors]) return;
    ...
    setGaugeColor ();
    renderBitmap (
        Toggle_frame.first_frame + frame_offset, position[0], position[1]);

    // draw the text on top
    if (frame_offset == 1) {
        // static color text_color;
        // gr_init_alphacolor(&text_color, 0, 0, 0, Toggle_text_alpha);
        if (Use_on_color) { gr_set_color_fast (&On_color); }
    }
    ...
    renderString (
        position[0] + Auto_text_offsets[0], position[1] + Auto_text_offsets[1],
        XSTR ("auto", 1463));
    renderString (
        position[0] + Target_text_offsets[0],
        position[1] + Target_text_offsets[1], XSTR ("target", 1465));
}
...
HudGaugeAutoSpeed::HudGaugeAutoSpeed ()
    : HudGauge (
          HUD_OBJECT_AUTO_SPEED, HUD_AUTO_SPEED, false, false,
          (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
           VM_OTHER_SHIP),
          255, 255, 255) {}
...
void HudGaugeAutoSpeed::render (float /*frametime*/) {
    ...
    renderString (
        ...
        XSTR ("auto", 1463));
    renderString (
        position[0] + Speed_text_offsets[0],
        position[1] + Speed_text_offsets[1], XSTR ("speed", 1464));
}
...
int hud_target_closest_repair_ship (int goal_objnum) {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (!(Player_ship->team == shipp->team)) { continue; }
...
void hud_target_toggle_hidden_from_sensors () {
    if (Ignore_List[Ship::Ship_Flags::Hidden_from_sensors]) {
        ...
        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN, NOX ("Target hiding from sensors disabled"));
    }
    else {
        Ignore_List.set (Ship::Ship_Flags::Hidden_from_sensors);
        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN, NOX ("Target hiding from sensors enabled"));
    }
}
...
void hud_target_uninspected_object (int next_flag) {
    ...
    // cargo
    if (Player_ai->target_objnum == -1 ||
        timestamp_elapsed (Target_next_uninspected_object_timestamp)) {
    ...
    cur_dist = hud_find_target_distance (
        &Objects[Player_ai->target_objnum], Player_obj);

    min_obj = max_obj = nearest_obj = NULL;
    min_dist = 1e20f;
    max_dist = 0.0f;
    if (next_flag) { nearest_dist = 1e20f; }
    else {
        nearest_dist = 0.0f;
    }
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        // don't use object if it is already a target
        if (OBJ_INDEX (A) == Player_ai->target_objnum) { continue; }
    ...
    if (nearest_obj == NULL) {
        if (next_flag) {
            if (min_obj != NULL) { nearest_obj = min_obj; }
        }
        else {
            if (max_obj != NULL) { nearest_obj = max_obj; }
        }
    }
...
// internal function only.. used to find index for last recorded ship
// transmission
int hud_target_last_transmit_newest () {
    int latest_slot;

    latest_slot = Transmit_target_next_slot - 1;
    if (latest_slot < 0) { latest_slot = MAX_TRANSMIT_TARGETS - 1; }

    return latest_slot;
}

// called externally to set the player target to the last ship which sent a
// transmission to the player
void hud_target_last_transmit () {
    ...
    for (i = 0; i < MAX_TRANSMIT_TARGETS; i++) {
        if (Transmit_target_list[transmit_index].objnum >= 0) {
            ...
            if (Player_ai->target_objnum == transmit_objnum) {
                ...
                if (Transmit_target_list[transmit_index].objsig ==
                    Objects[Transmit_target_list[transmit_index].objnum]
                        .signature) {
                    if (!(should_be_ignored (
                            &Ships[Objects[transmit_objnum].instance]))) {
                        Transmit_target_current_slot = transmit_index - 1;
                        if (Transmit_target_current_slot < 0) {
                            Transmit_target_current_slot =
                                MAX_TRANSMIT_TARGETS - 1;
                        }
                        break;
                    }
                }
        ...
        transmit_index--;
        if (transmit_index < 0) { transmit_index = MAX_TRANSMIT_TARGETS - 1; }
    }
    ...
    // hud_restore_subsystem_target(). -- MK, 9/15/99, 1:59 pm.
    int targeted_objnum;
    ...
    if ((targeted_objnum >= 0) && (targeted_objnum < MAX_OBJECTS)) {
        set_target_objnum (
            Player_ai, Transmit_target_list[transmit_index].objnum);
        hud_shield_hit_reset (
            &Objects[Transmit_target_list[transmit_index].objnum]);
        hud_restore_subsystem_target (
            &Ships[Objects[Transmit_target_list[transmit_index].objnum]
                       .instance]);
    }
}
...
void hud_target_last_transmit_add (int ship_num) {
    ...
    if (Player_obj == ship_objp) { return; }

    Transmit_target_list[Transmit_target_next_slot].objnum = ship_objnum;
    Transmit_target_list[Transmit_target_next_slot].objsig =
        ship_objp->signature;
...
void hud_target_random_ship () {
    ...
    if ((shipnum < 0) || (Ships[shipnum].objnum < 0)) { return; }
    objnum = Ships[shipnum].objnum;

    if ((objnum >= 0) && (Player_ai != NULL) &&
        !hud_target_invalid_awacs (&Objects[objnum])) {
...
void hudtarget_page_in () {
    ...
    for (i = 0; i < NUM_WEAPON_GAUGES; i++) {
        bm_page_in_aabitmap (
            Weapon_gauges[ballistic_hud_index][i].first_frame,
...
void hud_stuff_ship_name (char* ship_name_text, ship* shipp) {
    // print ship name
    if (((Iff_info[shipp->team].flags & IFFF_WING_NAME_HIDDEN) &&
         (shipp->wingnum != -1)) ||
        ...
        *ship_name_text = 0;
        ...
        if (!Disable_built_in_translations) {
            // handle translation
            if (Lcl_gr) { lcl_translate_targetbox_name_gr (ship_name_text); }
            else if (Lcl_pl) {
...
void hud_stuff_ship_callsign (char* ship_callsign_text, ship* shipp) {
    *ship_callsign_text = 0;
    ...
    if (!Disable_built_in_translations) {
        // handle translation
        if (Lcl_gr) { lcl_translate_targetbox_name_gr (ship_callsign_text); }
        else if (Lcl_pl) {
...
void hud_stuff_ship_class (char* ship_class_text, ship* shipp) {
    *ship_class_text = 0;
    ...
    if (!*ship_class_text) {
        strcpy (
            ship_class_text, (Ship_info[shipp->ship_info_index].alt_name[0])
    ...
    if (!Disable_built_in_translations) {
        // handle translation
        if (Lcl_gr) { lcl_translate_targetbox_name_gr (ship_class_text); }
        else if (Lcl_pl) {
...
HudGaugeCmeasures::HudGaugeCmeasures ()
    : HudGauge (
          HUD_OBJECT_CMEASURES, HUD_CMEASURE_GAUGE, false, false,
          VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
              VM_OTHER_SHIP,
          255, 255, 255) {}
...
void HudGaugeCmeasures::initBitmaps (char* fname) {
    Cmeasure_gauge.first_frame =
        bm_load_animation (fname, &Cmeasure_gauge.num_frames);
...
void HudGaugeCmeasures::render (float /*frametime*/) {
    ...
    if (sip->cmeasure_max < 0 || sip->cmeasure_type < 0) { return; }
    ...
    renderPrintf (
        position[0] + Cm_text_val_offsets[0],
        position[1] + Cm_text_val_offsets[1], NOX ("%02d"),
        Player_ship->cmeasure_count);
}

HudGaugeAfterburner::HudGaugeAfterburner ()
    : HudGauge (
          HUD_OBJECT_AFTERBURNER, HUD_AFTERBURNER_ENERGY, true, false,
          (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
           VM_OTHER_SHIP),
          255, 255, 255) {}
...
void HudGaugeAfterburner::render (float /*frametime*/) {
    ...
    if (Energy_bar.first_frame == -1) { return; }

    ASSERT (Player_ship);
    if (!(Ship_info[Player_ship->ship_info_index]
              .flags[Ship::Info_Flags::Afterburner])) {
        // Goober5000 - instead of drawing an empty burner gauge, don't draw
        // the gauge at all
        return;
    }
    else {
        percent_left =
            ...
            Ship_info[Player_ship->ship_info_index].afterburner_fuel_capacity;
    }

    if (percent_left > 1) { percent_left = 1.0f; }
    ...
    if (clip_h > 0) {
        renderBitmapEx (
            Energy_bar.first_frame, position[0], position[1], w, clip_h, 0, 0);
    }

    if (clip_h <= Energy_h) {
        renderBitmapEx (
            Energy_bar.first_frame + 1, position[0], position[1] + clip_h, w,
            h - clip_h, 0, clip_h);
    }
}
...
HudGaugeWeaponEnergy::HudGaugeWeaponEnergy ()
    : HudGauge (
          HUD_OBJECT_WEAPON_ENERGY, HUD_WEAPONS_ENERGY, true, false,
          (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
           VM_OTHER_SHIP),
          255, 255, 255) {}
...
void HudGaugeWeaponEnergy::initBitmaps (char* fname) {
    ...
    if (Energy_bar.first_frame < 0) {
        ...
        if (Energy_bar.num_frames != 4) {
            error_display (
                0,
                "The animation '%s' has %d frames but the energy bar requires "
                "4 frames in the animation! Ignoring animation...",
                fname, Energy_bar.num_frames);
            // Free the bitmap slot and set the frame handle to -1 so that it's
            // ignored when rendering this gauge
...
void HudGaugeWeaponEnergy::render (float /*frametime*/) {
    ...
    // Goober5000 - only check for the new gauge in case of command line + a
    // ballistic-capable ship
    if (Cmdline_ballistic_gauge) {
        for (x = 0; x < Player_ship->weapons.num_primary_banks; ++x) {
            if (Weapon_info[Player_ship->weapons.primary_bank_weapons[x]]
                    .wi_flags[Weapon::Info_Flags::Ballistic]) {
                use_new_gauge = true;
                break;
            }
    ...
    if (use_new_gauge) {
        ...
        if (gr_screen.max_w_unscaled == 640) { max_w = 60; }

        //*****ENERGY GAUGE
        if (Weapon_info[Player_ship->weapons.current_primary_bank]
                .energy_consumed > 0.0f)
        ...
        remaining = max_w * ((float)Player_ship->weapon_energy /
                             (float)Ship_info[Player_ship->ship_info_index]
                                 .max_weapon_reserve);
        if (remaining > 0) {
            setGaugeColor (HUD_C_BRIGHT);
            for (y = 0; y < 10; y++) {
        ...
        for (x = 0; x < Player_ship->weapons.num_primary_banks; x++) {
            ...
            // Draw the weapon bright or normal depending if it's active or
            // not.
        ...
        if (Player_ship->weapons.num_primary_banks <= 0) { return; }
        ...
        if (Show_ballistic) {
            if (Player_ship->flags[Ship::Ship_Flags::Primary_linked]) {
                // go through all ballistic primaries and add up their
                // ammunition totals and max capacities
                for (i = 0; i < sw->num_primary_banks; i++) {
                    // skip all pure-energy weapons
                    if (!(Weapon_info[sw->primary_bank_weapons[i]]
                              .wi_flags[Weapon::Info_Flags::Ballistic])) {
                        continue;
                    }
                ...
                ballistic_ammo =
                    sw->primary_bank_ammo[sw->current_primary_bank];
                max_ballistic_ammo =
                    sw->primary_bank_start_ammo[sw->current_primary_bank];
            }
            ...
            if (!ship_has_energy_weapons (Player_ship)) return;

            percent_left =
                Player_ship->weapon_energy /
                Ship_info[Player_ship->ship_info_index].max_weapon_reserve;
            if (percent_left > 1) { percent_left = 1.0f; }
        }
        ...
        if (percent_left <= 0.3 || Show_ballistic || Always_show_text) {
            ...
            if (percent_left < 0.1) { gr_set_color_fast (&Color_bright_red); }
            else {
                setGaugeColor ();
            }

            if (Show_ballistic) { sprintf (buf, "%d", ballistic_ammo); }
            else {
                sprintf (
                    buf, XSTR ("%d%%", 326),
                    (int)std::lround (percent_left * 100));
            }

            if (Moving_text) { delta_y = clip_h; }
            ...
            renderString (
                position[0] + Wenergy_text_offsets[0] + delta_x,
        ...
        if (Show_armed) {
            if (Player_ship->flags[Ship::Ship_Flags::Primary_linked]) {
                ...
                for (i = 0; i < Player_ship->weapons.num_primary_banks; i++) {
                    ...
                    renderString (
                        position[0] + Armed_name_offsets[0] - w,
                        position[1] + Armed_name_offsets[1] + Armed_name_h * i,
                        buf);
                }
            }
            else {
                ...
                renderString (
                    position[0] + Armed_name_offsets[0] - w,
                    position[1] + Armed_name_offsets[1], buf);
            }
        }
        ...
        if (clip_h > 0) {
            renderBitmapEx (
                Energy_bar.first_frame + 2, position[0], position[1], w,
                clip_h, 0, 0);
        }

        if (clip_h <= Wenergy_h) {
            renderBitmapEx (
                Energy_bar.first_frame + 3, position[0], position[1] + clip_h,
                w, h - clip_h, 0, clip_h);
        }
...
HudGaugeWeapons::HudGaugeWeapons ()
    : HudGauge (
          HUD_OBJECT_WEAPONS, HUD_WEAPONS_GAUGE, false, false,
          VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
              VM_OTHER_SHIP,
          255, 255, 255) {}
...
void HudGaugeWeapons::initLinkIcon () {
    ...
    if (sc == 0) { Weapon_link_icon = ubyte ('>'); }
    else {
...
void HudGaugeWeapons::initPrimaryWeaponOffsets (
    int link_x, int name_x, int ammo_x) {
    Weapon_plink_offset_x = link_x;
    Weapon_pname_offset_x = name_x;
    Weapon_pammo_offset_x = ammo_x;
}

void HudGaugeWeapons::initSecondaryWeaponOffsets (
    int ammo_x, int name_x, int reload_x, int linked_x, int unlinked_x) {
...
void HudGaugeWeapons::initBitmapsPrimaryTop (
    char* fname, char* fname_ballistic) {
    // load the graphics for the top portion of the weapons gauge
    primary_top[0].first_frame =
        bm_load_animation (fname, &primary_top[0].num_frames);
    if (primary_top[0].first_frame < 0) {
        WARNINGF (LOCATION, "Cannot load hud ani: %s", fname);
    }

    primary_top[1].first_frame =
        bm_load_animation (fname_ballistic, &primary_top[1].num_frames);
...
void HudGaugeWeapons::initBitmapsPrimaryMiddle (
    char* fname, char* fname_ballistic) {
    // load the graphics for the middle portion of the primary weapons listing
    primary_middle[0].first_frame =
        bm_load_animation (fname, &primary_middle[0].num_frames);
    if (primary_middle[0].first_frame < 0) {
        WARNINGF (LOCATION, "Cannot load hud ani: %s", fname);
    }

    primary_middle[1].first_frame =
        bm_load_animation (fname_ballistic, &primary_middle[1].num_frames);
...
void HudGaugeWeapons::initBitmapsPrimaryLast (
    char* fname, char* fname_ballistic) {
    // load the graphics for the bottom portion of the primary weapons listing
    // if there is one. Don't bother the user if there isn't one since retail
    // doesn't use this.
    primary_last[0].first_frame =
        bm_load_animation (fname, &primary_last[0].num_frames);

    primary_last[1].first_frame =
        bm_load_animation (fname_ballistic, &primary_last[1].num_frames);
...
void HudGaugeWeapons::initBitmapsSecondaryTop (
    char* fname, char* fname_ballistic) {
    // top portion of the secondary weapons gauge
    secondary_top[0].first_frame =
        bm_load_animation (fname, &secondary_top[0].num_frames);
    if (secondary_top[0].first_frame < 0) {
        WARNINGF (LOCATION, "Cannot load hud ani: %s", fname);
    }

    secondary_top[1].first_frame =
        bm_load_animation (fname_ballistic, &secondary_top[1].num_frames);
...
void HudGaugeWeapons::initBitmapsSecondaryMiddle (
    char* fname, char* fname_ballistic) {
    // middle portion of the secondary weapons gauge
    secondary_middle[0].first_frame =
        bm_load_animation (fname, &secondary_middle[0].num_frames);
    if (secondary_middle[0].first_frame < 0) {
        WARNINGF (LOCATION, "Cannot load hud ani: %s", fname);
    }

    secondary_middle[1].first_frame =
        bm_load_animation (fname_ballistic, &secondary_middle[1].num_frames);
...
void HudGaugeWeapons::initBitmapsSecondaryBottom (
    char* fname, char* fname_ballistic) {
    // bottom portion of the entire weapons gauge
    secondary_bottom[0].first_frame =
        bm_load_animation (fname, &secondary_bottom[0].num_frames);
    if (secondary_bottom[0].first_frame < 0) {
        WARNINGF (LOCATION, "Cannot load hud ani: %s", fname);
    }

    secondary_bottom[1].first_frame =
        bm_load_animation (fname_ballistic, &secondary_bottom[1].num_frames);
...
void HudGaugeWeapons::pageIn () {
    if (primary_top[0].first_frame > -1) {
        bm_page_in_aabitmap (
            primary_top[0].first_frame, primary_top[0].num_frames);
    }

    if (primary_top[1].first_frame > -1) {
        bm_page_in_aabitmap (
            primary_top[1].first_frame, primary_top[1].num_frames);
    }

    if (primary_middle[0].first_frame > -1) {
        bm_page_in_aabitmap (
            primary_middle[0].first_frame, primary_middle[0].num_frames);
    }

    if (primary_middle[1].first_frame > -1) {
        bm_page_in_aabitmap (
            primary_middle[1].first_frame, primary_middle[1].num_frames);
    }

    if (primary_last[1].first_frame > -1) {
        bm_page_in_aabitmap (
            primary_last[1].first_frame, primary_last[1].num_frames);
    }

    if (secondary_top[0].first_frame > -1) {
        bm_page_in_aabitmap (
            secondary_top[0].first_frame, secondary_top[0].num_frames);
    }

    if (secondary_top[1].first_frame > -1) {
        bm_page_in_aabitmap (
            secondary_top[1].first_frame, secondary_top[1].num_frames);
    }

    if (secondary_middle[0].first_frame > -1) {
        bm_page_in_aabitmap (
            secondary_middle[0].first_frame, secondary_middle[0].num_frames);
    }
    ...
    if (secondary_bottom[0].first_frame > -1) {
        bm_page_in_aabitmap (
            secondary_bottom[0].first_frame, secondary_bottom[0].num_frames);
    }
...
void HudGaugeWeapons::render (float /*frametime*/) {
    ...
    if (Player_obj->type == OBJ_OBSERVER) return;
    ...
    renderBitmap (
        primary_top[ballistic_hud_index].first_frame,
        position[0] + top_offset_x[ballistic_hud_index], position[1]);

    // render the header of this gauge
    renderString (
        position[0] + Weapon_header_offsets[ballistic_hud_index][0],
        position[1] + Weapon_header_offsets[ballistic_hud_index][1],
    ...
    for (i = 0; i < np; i++) {
        ...
        // It is assumed that the top primary wep frame already has this
        // rendered.
        if (i == 1) {
            // used to draw the second primary weapon background
            renderBitmap (
                primary_middle[ballistic_hud_index].first_frame,
            ...
            if (primary_last[ballistic_hud_index].first_frame != -1)
                renderBitmap (
                    primary_last[ballistic_hud_index].first_frame,
                    position[0] + frame_offset_x[ballistic_hud_index], y);
        }
        ...
        if (HudGauge::maybeFlashSexp () == i) { setGaugeColor (HUD_C_BRIGHT); }
        else {
            maybeFlashWeapon (i);
        }

        // indicate if this is linked or currently armed
        if (((sw->current_primary_bank == i) &&
             !(Player_ship->flags[Ship::Ship_Flags::Primary_linked])) ||
            ((Player_ship->flags[Ship::Ship_Flags::Primary_linked]) &&
             !(Weapon_info[sw->primary_bank_weapons[i]]
                   .wi_flags[Weapon::Info_Flags::Nolink]))) {
            renderPrintf (
                position[0] + Weapon_plink_offset_x, name_y, EG_NULL, "%c",
                Weapon_link_icon);
        }
        ...
        if (Weapon_info[sw->primary_bank_weapons[0]].hud_image_index != -1) {
            renderBitmap (
                Weapon_info[sw->primary_bank_weapons[i]].hud_image_index,
                position[0] + Weapon_pname_offset_x, name_y);
        }
        else {
            renderPrintf (
                position[0] + Weapon_pname_offset_x, name_y, EG_WEAPON_P2,
                "%s", name);
        }
        ...
        if (Weapon_info[sw->primary_bank_weapons[i]]
            ...
            renderString (
                position[0] + Weapon_pammo_offset_x - w, name_y, EG_NULL,
                ammo_str);
        }

        if (i != 0) { y += primary_text_h; }
    ...
    renderBitmap (
        secondary_top[ballistic_hud_index].first_frame,
        position[0] + frame_offset_x[ballistic_hud_index], y);
    ...
    for (i = 0; i < ns; i++) {
        ...
        if (i != 0) {
            renderBitmap (
                secondary_middle[ballistic_hud_index].first_frame,
                position[0] + frame_offset_x[ballistic_hud_index], y);
        }
        ...
        if (sw->current_secondary_bank == i) {
            ...
            renderPrintf (
                position[0] + Weapon_sunlinked_offset_x, name_y, EG_NULL, "%c",
                Weapon_link_icon);

            // indicate if this is linked
            if (Player_ship->flags[Ship::Ship_Flags::Secondary_dual_fire]) {
                renderPrintf (
                    position[0] + Weapon_slinked_offset_x, name_y, EG_NULL,
                    "%c", Weapon_link_icon);
            }
            ...
            if (wip->hud_image_index != -1) {
                renderBitmap (
                    wip->hud_image_index, position[0] + Weapon_sname_offset_x,
                    name_y);
            }
            else {
                renderString (
                    position[0] + Weapon_sname_offset_x, name_y,
                    i ? EG_WEAPON_S1 : EG_WEAPON_S2, weapon_name);
            }

            // show the cooldown time
            if ((sw->secondary_bank_ammo[i] > 0) &&
                (sw->current_secondary_bank >= 0)) {
                int ms_till_fire = timestamp_until (
                    sw->next_secondary_fire_stamp[sw->current_secondary_bank]);
                if ((ms_till_fire >= 500) &&
                    ((wip->fire_wait >= 1) ||
                     (ms_till_fire > wip->fire_wait * 1000))) {
                    renderPrintf (
                        position[0] + Weapon_sreload_offset_x, name_y, EG_NULL,
                        "%d", (int)std::lround (ms_till_fire / 1000.0f));
                }
            }
        }
        else {
        ...
        renderString (
            position[0] + Weapon_sammo_offset_x - w, name_y, EG_NULL,
            ammo_str);

        if (i != 0) { y += secondary_text_h; }
    ...
    if (ns == 0) {
        renderString (
            position[0] + Weapon_pname_offset_x, name_y, EG_WEAPON_S1,
            XSTR ("<none>", 329));
    }
    ...
    renderBitmap (
        secondary_bottom[ballistic_hud_index].first_frame,
        position[0] + frame_offset_x[ballistic_hud_index], y);
}
    ...
    for (int i = 0; i < num_weapons; i++) {
        if (!timestamp_elapsed (Weapon_flash_info.flash_duration[i])) {
            if (timestamp_elapsed (Weapon_flash_info.flash_next[i])) {
                Weapon_flash_info.flash_next[i] =
                    timestamp (TBOX_FLASH_INTERVAL);
...
void hud_target_add_display_list (
    object* objp, vertex* target_point, vec3d* target_pos, int correction,
    ...
    if (bracket_clr) { element.bracket_clr = *bracket_clr; }
    else {
        // no color given, so this will tell the target display gauges to use
        // IFF colors.
...
HudGaugeOffscreen::HudGaugeOffscreen ()
    : HudGauge (
          HUD_OBJECT_OFFSCREEN, HUD_OFFSCREEN_INDICATOR, false, true,
          VM_DEAD_VIEW | VM_OTHER_SHIP, 255, 255, 255) {}
    ...
    for (size_t i = 0; i < target_display_list.size (); i++) {
        if (target_display_list[i].target_point.codes != 0) {
            ...
            if (target_display_list[i].objp) {
                dist = hud_find_target_distance (
                    target_display_list[i].objp, Player_obj);
            }
            else {
                // if we don't have a corresponding object, use given position
                // to figure out distance
                dist = vm_vec_dist_quick (
                    &Player_obj->pos, &target_display_list[i].target_pos);
            }
            ...
            if (target_display_list[i].bracket_clr.red &&
                ...
                if (target_display_list[i].objp) {
                    ...
                    // no object so this must mean it's a nav point. but for
                    // some odd reason someone forgot to include a color.
                    gr_set_color_fast (&target_display_list[i].bracket_clr);
                }
            }

            calculatePosition (
                &target_display_list[i].target_point,
                &target_display_list[i].target_pos, &coords, &dir,
                &half_triangle_sep);
            renderOffscreenIndicator (
                &coords, dir, dist, half_triangle_sep, true);
        }
    }
}

void HudGaugeOffscreen::calculatePosition (
    vertex* target_point, vec3d* tpos, vec2d* outcoords, int* dir,
    float* half_triangle_sep) {
    float xpos, ypos;
    ...
    // calculate the dot product between the players forward vector and the
    // vector connecting the player to the target. Normalize targ_to_player
    // since we want the dot product to range between 0 -> 1.
    ...
    if (dist_behind < 0) { // still in front of player, but not in view
        ...
        triangle_sep = dist_behind * Max_offscreen_tri_seperation +
                       Max_offscreen_tri_seperation;
    }
    ...
    // calculate these values only once, since it will be used in several
    // places
    *half_triangle_sep = 0.5f * triangle_sep;
    half_gauge_length = *half_triangle_sep + Offscreen_tri_base;

    // We need to find the screen (x,y) for where to draw the offscreen
    // indicator
    //
    // The best way I've found is to draw a line from the eye_pos to the
    // target, and then use clip_line() to find the screen (x,y) for where the
    // line hits the edge of the screen.
    //
    // The weird thing about clip_line() is that is flips around the two
    // verticies, so I use eye_vertex->sx and eye_vertex->sy for the off-screen
    // indicator (x,y)
    //
    vertex* eye_vertex = NULL;
    vertex real_eye_vertex;
    eye_vertex =
        &real_eye_vertex; // this is needed since clip line takes a **vertex
    vec3d eye_pos;
    ...
    if (target_point->flags & PF_TEMP_POINT) free_temp_point (target_point);

    if (eye_vertex->flags & PF_TEMP_POINT) free_temp_point (eye_vertex);

    xpos = eye_vertex->screen.xyw.x;
    ypos = eye_vertex->screen.xyw.y;

    // we need it unsized here and it will be fixed when things are acutally
    // drawn
...
void HudGaugeOffscreen::renderOffscreenIndicator (
    vec2d* coords, int dir, float distance, float half_triangle_sep,
    bool draw_solid) {
    float xpos, ypos;
...
HudGaugeWarheadCount::HudGaugeWarheadCount ()
    : HudGauge (
          HUD_OBJECT_WARHEAD_COUNT, HUD_WEAPONS_GAUGE, false, false,
...
void HudGaugeWarheadCount::render (float /*frametime*/) {
    ...
    if (sw->num_secondary_banks <= 0) { return; }
    ...
    if (Text_align) {
        ...
        renderString (
            position[0] + Warhead_name_offsets[0] - w,
            position[1] + Warhead_name_offsets[1], weapon_name);
    }
    else {
        renderString (
            position[0] + Warhead_name_offsets[0],
            position[1] + Warhead_name_offsets[1], weapon_name);
    }
    ...
    if (ammo > Max_symbols) {
        ...
        if (Text_align) {
            ...
            gr_get_string_size (&w, &h, ammo_str);
            renderString (
                position[0] + Warhead_count_offsets[0] - w,
                position[1] + Warhead_count_offsets[1], ammo_str);
        }
        else {
            renderString (
                position[0] + Warhead_count_offsets[0],
                position[1] + Warhead_count_offsets[1], ammo_str);
        }
    ...
    if (Text_align) { delta_x = -Warhead_count_size[0]; }
    else {
    ...
    for (i = 0; i < ammo; i++) {
        ...
        renderBitmap (
            Warhead.first_frame,
            position[0] + Warhead_count_offsets[0] + column * delta_x,
...
HudGaugeWeaponList::HudGaugeWeaponList (int _gauge_object)
    : HudGauge (
          _gauge_object, HUD_WEAPONS_GAUGE, false, false,
          VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY, 255,
          255, 255) {}
...
void HudGaugeWeaponList::initBitmaps (
    char* fname_first, char* fname_entry, char* fname_last) {
    _background_first.first_frame =
        bm_load_animation (fname_first, &_background_first.num_frames);
    if (_background_first.first_frame < 0) {
        WARNINGF (LOCATION, "Cannot load hud ani: %s", fname_first);
    }

    _background_entry.first_frame =
        bm_load_animation (fname_entry, &_background_entry.num_frames);
    if (_background_entry.first_frame < 0) {
        WARNINGF (LOCATION, "Cannot load hud ani: %s", fname_entry);
    }

    _background_last.first_frame =
        bm_load_animation (fname_last, &_background_last.num_frames);
    if (_background_last.first_frame < 0) {
        WARNINGF (LOCATION, "Cannot load hud ani: %s", fname_last);
    }
}
...
void HudGaugeWeaponList::pageIn () {
    if (_background_first.first_frame >= 0) {
        bm_page_in_aabitmap (
            _background_first.first_frame, _background_first.num_frames);
    }

    if (_background_entry.first_frame >= 0) {
        bm_page_in_aabitmap (
            _background_entry.first_frame, _background_entry.num_frames);
    }

    if (_background_last.first_frame >= 0) {
        bm_page_in_aabitmap (
            _background_last.first_frame, _background_last.num_frames);
    }
}
...
void HudGaugePrimaryWeapons::render (float /*frametime*/) {
    ...
    // render the header of this gauge
    renderString (
        position[0] + _header_offsets[0], position[1] + _header_offsets[1],
        EG_WEAPON_TITLE, header_text);
    ...
    for (i = 0; i < num_primaries; ++i) {
        ...
        if ((sw->current_primary_bank == i) ||
            (Player_ship->flags[Ship::Ship_Flags::Primary_linked])) {
            renderPrintf (
                position[0] + _plink_offset_x, position[1] + text_y_offset,
                EG_NULL, "%c", Weapon_link_icon);
        }
        ...
        if (Weapon_info[sw->primary_bank_weapons[0]].hud_image_index != -1) {
            renderBitmap (
                Weapon_info[sw->primary_bank_weapons[i]].hud_image_index,
                position[0] + _pname_offset_x, text_y_offset);
        }
        else {
            renderPrintf (
                position[0] + _pname_offset_x, position[1] + text_y_offset,
                EG_WEAPON_P2, "%s", name);
        }
        ...
        if (Weapon_info[sw->primary_bank_weapons[i]]
            ...
            renderString (
                position[0] + _pammo_offset_x - w, position[1] + text_y_offset,
                EG_NULL, ammo_str);
        }
    ...
    if (num_primaries == 0) {
        ...
        renderString (
            position[0] + _pname_offset_x, position[1] + text_y_offset,
            EG_WEAPON_P1, XSTR ("<none>", 329));

        bg_y_offset += _background_entry_h;
    }

    renderBitmap (
        _background_last.first_frame, position[0],
        position[1] + bg_y_offset + _bg_last_offset_y);
}
...
void HudGaugeSecondaryWeapons::render (float /*frametime*/) {
    ...
    for (i = 0; i < num_secondaries; ++i) {
        ...
        if (sw->current_secondary_bank == i) {
            // show that this is the current secondary armed
            renderPrintf (
                position[0] + _sunlinked_offset_x, position[1] + text_y_offset,
                EG_NULL, "%c", Weapon_link_icon);

            // indicate if this is linked
            if (Player_ship->flags[Ship::Ship_Flags::Secondary_dual_fire]) {
                renderPrintf (
                    position[0] + _slinked_offset_x,
                    position[1] + text_y_offset, EG_NULL, "%c",
                    Weapon_link_icon);
            }
            ...
            if (wip->hud_image_index != -1) {
                renderBitmap (
                    wip->hud_image_index, position[0] + _sname_offset_x,
                    position[1] + text_y_offset);
            }
            else {
                renderString (
                    position[0] + _sname_offset_x, position[1] + text_y_offset,
                    i ? EG_WEAPON_S1 : EG_WEAPON_S2, weapon_name);
            }
            ...
            if ((sw->secondary_bank_ammo[i] > 0) &&
                ...
                if ((ms_till_fire >= 500) &&
                    ((wip->fire_wait >= 1) ||
                     (ms_till_fire > wip->fire_wait * 1000))) {
                    renderPrintf (
                        position[0] + _sreload_offset_x,
                        position[1] + text_y_offset, EG_NULL, "%d",
        ...
        renderString (
            position[0] + _sammo_offset_x - w, position[1] + text_y_offset,
            EG_NULL, ammo_str);
...
HudGaugeHardpoints::HudGaugeHardpoints ()
    : HudGauge (
          HUD_OBJECT_HARDPOINTS, HUD_WEAPONS_GAUGE, false, false,
          VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY, 255,
          255, 255) {}
...
void HudGaugeHardpoints::initDrawOptions (
    bool primary_models, bool secondary_models) {
...
void HudGaugeHardpoints::render (float /*frametime*/) {
    ...
    case TOP: vm_angles_2_matrix (&object_orient, &top_view); break;
    case FRONT: vm_angles_2_matrix (&object_orient, &front_view); break;
    }
    ...
    g3_set_view_matrix (
        &sip->closeup_pos, &vmd_identity_matrix, sip->closeup_zoom * 1.5f);
    ...
    render_info.set_flags (
        MR_NO_LIGHTING | MR_AUTOCENTER | MR_NO_FOGGING | MR_NO_TEXTURING |
        MR_NO_CULL | MR_NO_ZBUFFER | MR_STENCIL_WRITE | MR_NO_COLOR_WRITES);
    render_info.set_object_number (OBJ_INDEX (objp));

    model_render_immediate (
        &render_info, sip->model_num, &object_orient, &vmd_zero_vector);

    render_info.set_color (gauge_color);
    render_info.set_flags (
        MR_NO_LIGHTING | MR_AUTOCENTER | MR_NO_FOGGING | MR_NO_TEXTURING |
        MR_NO_ZBUFFER | MR_NO_CULL | MR_STENCIL_READ);
    // Factor was chosen pretty arbitrarily to make outline feature work with
    // existing data
    ...
    int render_flags = MR_NO_LIGHTING | MR_AUTOCENTER | MR_NO_FOGGING |
                       MR_NO_TEXTURING | MR_NO_ZBUFFER;
        ...
        for (i = 0; i < swp->num_secondary_banks; i++) {
            ...
            if (Weapon_info[swp->secondary_bank_weapons[i]]
                ...
                for (k = 0; k < bank->num_slots; k++) {
                    ...
                    weapon_render_info.set_detail_level_lock (
                        detail_level_lock);
                    weapon_render_info.set_flags (render_flags);

                    // We need to transform the position local to the model to
                    // be in "world" space relative to the rendered outline
                    vec3d world_position;
                    vm_vec_unrotate (
                        &world_position, &bank->pnt[k], &object_orient);
                ...
                for (k = 0; k < bank->num_slots; k++) {
                    ...
                    if (num_secondaries_rendered >=
                        sp->weapons.secondary_bank_ammo[i])
                        break;

                    if (sp->secondary_point_reload_pct[i][k] <= 0.0) continue;

                    model_render_params weapon_render_info;

                    if (swp->current_secondary_bank == i &&
                        (swp->secondary_next_slot[i] == k ||
                         (swp->secondary_next_slot[i] + 1 == k &&
                          sp->flags[Ship::Ship_Flags::Secondary_dual_fire]))) {
                    ...
                    vec3d world_position;
                    vm_vec_unrotate (
                        &world_position, &secondary_weapon_pos,
                        &object_orient);
        ...
        for (i = 0; i < swp->num_primary_banks; i++) {
            ...
            for (k = 0; k < bank->num_slots; k++) {
                if ((Weapon_info[swp->primary_bank_weapons[i]]
                    ...
                    vm_vec_unrotate (
                        &subobj_pos, &bank->pnt[k], &object_orient);
                    ...
                    // unsize(&xc, &yc);
                    // unsize(&draw_point.screen.xyw.x,
                    // &draw_point.screen.xyw.y);

                    renderCircle (
                        (int)draw_point.screen.xyw.x + position[0],
                        (int)draw_point.screen.xyw.y + position[1], 10);
                    // renderCircle(xc, yc, 25);
                }
                else {
                    polymodel* pm =
                        model_get (Weapon_info[swp->primary_bank_weapons[i]]
                                       .external_model_num);
