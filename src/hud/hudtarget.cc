...
{
        ASSERT(
                (Player_ai->target_objnum >= 0) && (Player_ai->target_objnum < MAX_OBJECTS));
...
void hud_target_missile(object *source_obj, int next_flag)
{
        ...
        if (!target_found) {
                ...
                if ((aip->target_objnum != -1) && (Objects[aip->target_objnum].type == OBJ_SHIP) && ((Ship_info[Ships[Objects[aip->target_objnum].instance].ship_info_index].flags[Ship::Info_Flags::Bomber]) || (Objects[aip->target_objnum].flags[Object::Object_Flags::Targetable_as_bomb]))) {
        ...
        vec3d *hostile_pos, int aspect_flag, int show_interior, int split_tri)
{
        ...
        if (hostile_vertex.codes == 0) { // on screen
                ...
                if (!(hostile_vertex.flags & PF_OVERFLOW)) { // make sure point projected
                        ...
                        mag_squared = (projected_x - position[0]) * (projected_x - position[0]) + (projected_y - position[1]) * (projected_y - position[1]);
        ...
        if (nearest_dist < Homing_beep.max_cycle_dist) {
                ...
                if ((delta_time * 1000) > cycle_time) {
                        ...
                        if (closest_is_aspect) {
                                Homing_beep.snd_handle = snd_play(gamesnd_get_game_sound(ship_get_sound(
...
void HudGaugeLeadIndicator::render(float /*frametime*/)
{
        ...
        for (size_t i = 0; i < target_display_list.size(); i++) {
                if ((target_display_list[i].flags & TARGET_DISPLAY_LEAD) && target_display_list[i].objp) {
                        // set the color
                        if (target_display_list[i].bracket_clr.red && target_display_list[i].bracket_clr.green && target_display_list[i].bracket_clr.blue) {
        ...
        if (use_new_gauge) {
                ...
                if (Show_ballistic) {
                        ...
                        if (max_ballistic_ammo > 0) {
                                percent_left = float(ballistic_ammo) / float(max_ballistic_ammo);
                        } else {
                                percent_left = 1.0f;
                        }
                } else {
...
void HudGaugeHardpoints::render(float /*frametime*/)
{
        ...
        render_info.set_flags(
                MR_NO_LIGHTING | MR_AUTOCENTER | MR_NO_FOGGING | MR_NO_TEXTURING | MR_NO_CULL | MR_NO_ZBUFFER | MR_STENCIL_WRITE | MR_NO_COLOR_WRITES);
        ...
        render_info.set_flags(
                MR_NO_LIGHTING | MR_AUTOCENTER | MR_NO_FOGGING | MR_NO_TEXTURING | MR_NO_ZBUFFER | MR_NO_CULL | MR_STENCIL_READ);
                        ...
                        if (Weapon_info[swp->secondary_bank_weapons[i]]
                                ...
                                for (k = 0; k < bank->num_slots; k++) {
                                        ...
                                        vm_vec_scale_add2(
                                                ...
                                                -(1.0f - sp->secondary_point_reload_pct[i][k]) * model_get(Weapon_info[swp->secondary_bank_weapons[i]].external_model_num)->rad);
                        ...
                        for (k = 0; k < bank->num_slots; k++) {
                                if ((Weapon_info[swp->primary_bank_weapons[i]]
                                     ...
                                     || !sip->draw_primary_models[i])) {
                                        ...
                                        polymodel *pm = model_get(Weapon_info[swp->primary_bank_weapons[i]]
                                                                          .external_model_num);
