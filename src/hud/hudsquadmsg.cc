...
static int
    Msg_key_used;  // local variable which tells if the key being processed
...
int Msg_instance; // variable which holds ship/wing instance to send the
                  // message to
...
static int Msg_enemies;   // tells us whether or not to message enemy ships or
                         // friendlies

static int Msg_eat_key_timestamp; // used to temporarily "eat" keys

// defined to position the messaging box
int Mbox_item_h[GR_NUM_RESOLUTIONS] = { 10, 10 };
int Mbox_item_xoffset[GR_NUM_RESOLUTIONS] = { 17, 17 };

// top of the message box gauge
int Mbox_top_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                 445, 5 },
                                               { // GR_1024
                                                 827, 5 } };

int Mbox_bmap_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                  445, 17 },
                                                { // GR_1024
                                                  827, 17 } };

// squadmsg menu pgup and pgdn
int Menu_pgup_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                  590, 9 },
                                                { // GR_1024
                                                  937, 9 } };
int Menu_pgdn_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                  590, 120 },
                                                { // GR_1024
                                                  937, 120 } };

// -----------
// following defines/vars are used to build menus that are used in messaging
// mode
...
// -----------
// following set of vars/defines are used to store/restore key bindings for
// keys that are used in messaging mode

// array to temporarily store key bindings that will be in use for the
// messaging system
...
#define MAX_KEYS_USED \
    12 // maximum number of keys used for the messaging system

key_store key_save[MAX_KEYS_USED]; // array to save the key information during
                                   // messaging mode
int num_keys_saved = 0;            // number of keys that are saved.

// next array is the array of MAX_KEYS_USED size which are the keys to use for
// messaging mode

int keys_used[] = { KEY_1, KEY_2, KEY_3, KEY_4, KEY_5,      KEY_6,
                    KEY_7, KEY_8, KEY_9, KEY_0, KEY_PAGEUP, KEY_PAGEDOWN };

#define ID1 1
#define ID2 2

// following are defines and character strings that are used as part of
// messaging mode
...
// Karajorma
// moving the defines to a non-temporary array is no less stupid. But at least
// now the SEXP system can get at them.
...
char* comm_order_get_text (int item) {
    ...
    for (i = 0; i < NUM_COMM_ORDER_ITEMS; i++) {
        if (Comm_orders[i].item == item) return Comm_orders[i].name;
    }
...
int hud_squadmsg_is_target_order_valid (
    int order, int find_order, ai_info* aip = NULL);
...
void hud_squadmsg_start () {
    ...
    // if ( num_keys_saved < 0 )  // save the keys if they haven't been saved
    // yet
    ...
    /*
        for (i=0; i<num_keys_saved; i++)
            clear_key_binding ( (short) key_save[i].key_value );
       // removes all mention of this key from Control_config
    */
    ...
    Msg_mode_timestamp =
        timestamp (DEFAULT_MSG_TIMEOUT); // initialize our timer to bogus value
    Msg_shortcut_command = -1;           // assume no shortcut key being used
    Msg_target_objnum =
        Player_ai->target_objnum; // save the players target object number
    Msg_targeted_subsys =
        Player_ai
            ->targeted_subsys; // save the players currently targted subsystem
#ifndef NDEBUG
...
// functions which will restore all of the key binding stuff when messaging
// mode is done
...
// function which returns true if there are fighters/bombers on the players
// team in the mission In debug versions, we will allow messaging to enemies
bool hud_squadmsg_exist_fighters () {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (shipp->orders_accepted == 0) continue;

            // be sure ship is on correct team
#ifdef NDEBUG
        if (shipp->team != Player_ship->team) continue;
#else
        if (!Msg_enemies && (shipp->team != Player_ship->team)) continue;
#endif

        // cannot be my ship or an instructor
        if ((objp == Player_obj) || is_instructor (objp)) continue;
...
// Routine to return true if a ship should be put onto the messaging menu.
// Analagous to hud_squadmsg_wing_valid() below. -MageKing17
bool hud_squadmsg_ship_valid (ship* shipp, object* objp) {
    ASSERTX (
        shipp != nullptr,
        "hud_squadmsg_ship_valid() was passed a null shipp pointer; get a "
        "coder!\n");
    if (!objp) // For simplicity's sake, allow it to be called with just the
               // ship pointer
        objp = &Objects[shipp->objnum];

    // ships must be able to receive a message
    if (Ship_info[shipp->ship_info_index].class_type < 0 ||
        !(Ship_types[Ship_info[shipp->ship_info_index].class_type]
              .flags[Ship::Type_Info_Flags::AI_accept_player_orders]))
        return false;

        // be sure ship is on correct team
#ifdef NDEBUG
    if (shipp->team != Player_ship->team) return false;
#else
    if (!Msg_enemies && (shipp->team != Player_ship->team)) return false;
#endif

    // departing or dying ships cannot be on list
    if (shipp->is_dying_or_departing ()) return false;

    // cannot be my ship or an instructor
    if ((objp == Player_obj) || is_instructor (objp)) return false;
    ...
    if (Msg_shortcut_command != -1) {
        if (!(shipp->orders_accepted & Msg_shortcut_command))
            return false;
        else if (!hud_squadmsg_ship_order_valid (
...
// function which counts the number of ships available for messaging.  Used to
// determine if we should grey out a menu or allow a shortcut command to apply.
// parameter "flag" is used to tell us whether or not we should add the ship to
// a menu item or not.  We include the flag so that we don't have to have
// conditions for messaging ships/wings in two places.
int hud_squadmsg_count_ships (int add_to_menu) {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        ASSERTX (
            shipp->objnum != -1,
            "hud_squadmsg_count_ships() discovered that ship #%d ('%s') has "
            "an objnum of -1. Since the ship was retrieved from its object "
            "number (%d), this should be impossible; get a coder!\n",
            objp->instance, shipp->ship_name, so->objnum);

        if (!hud_squadmsg_ship_valid (shipp, objp)) continue;
        ...
        if (add_to_menu) {
            ...
            end_string_at_first_hash_symbol (
                MsgItems[Num_menu_items]
                    .text); // truncate the name if it has a # in it
    ...
    // if adding to the menu and we have > 10 items, then don't allow page up
    // and page down to be used.
...
int hud_squadmsg_wing_valid (wing* wingp) {
    ...
    // a couple of special cases to account for before adding to count (or to
    // menu).  Don't count wings that are leaving or left.
    if (wingp->flags[Ship::Wing_Flags::Gone, Ship::Wing_Flags::Departing])
        return 0;

    // Goober5000 - instead of checking wing leader, let's check all ships in
    // wing; there are several significant cases when e.g. wings contain ships
    // of different IFFs
    for (idx = 0; idx < wingp->current_count; idx++) {
        ...
        // don't check player
        if (ship_num == SHIP_INDEX (Player_ship)) continue;

            // be sure ship is on same team
#ifdef NDEBUG
        if (Ships[ship_num].team != Player_ship->team) continue;
#else
        ...
        if (Ships[ship_num].orders_accepted == 0) continue;

        // if doing a message shortcut is being used, be sure the ship can
        // "accept" the command.
        if (Msg_shortcut_command >= 0 &&
            !(Ships[ship_num].orders_accepted & Msg_shortcut_command))
            return 0;
...
int hud_squadmsg_count_wings (int add_to_menu) {
    ...
    for (i = 0; i < Num_wings; i++) {
        // if this wing is a player starting wing, skip it since we added it
        // above
        for (j = 0; j < MAX_STARTING_WINGS; j++) {
            if (i == Starting_wings[j]) break;
        }
        if (j < MAX_STARTING_WINGS) continue;
...
// function to set the current submode in message mode -- also resets variables
// that should be reset inbetween submodes
...
int hud_squadmsg_get_total_keys () {
    ...
    if (Num_menu_items > MAX_MENU_DISPLAY) num_keys_used = MAX_KEYS_USED;

    return num_keys_used;
}

// function called from high level keyboard read code to give the squadmsg code
// a key. return 1 is the key was used by the messaging code, 0 otherwise
int hud_squadmsg_read_key (int k) {
    ...
    if (!(Player->flags & PLAYER_FLAGS_MSG_MODE)) {
        // check to see if any messaging keys are still down for some length of
        // time after messaging is over.  Return true for a while.
        if (!timestamp_elapsed (Msg_eat_key_timestamp)) {
            for (i = 0; i < num_keys_used; i++) {
                if (keyd_pressed[keys_used[i]]) return 1;
            }
        }
    ...
    if (key_found) return 1;

    return 0;
}

// function which reads the keyboard array and determines if a menu key has
// been hit
int hud_squadmsg_get_key () {
    int k, i, num_keys_used;

    if (Msg_key == -1) return -1;
    ...
    for (i = 0; i < num_keys_used; i++) {
        if (k == keys_used[i]) {
            ...
            // use a timestamp to prevent top level key code from possibly
            // reprocessing this key
            Msg_eat_key_timestamp = timestamp (MSG_KEY_EAT_TIME);
            if (k == KEY_PAGEDOWN) { // pageup and pagedown scroll the menu --
                                     // deal with these seperately!!
                ...
                (i < Num_menu_items) &&
                (Squad_msg_mode ==
                 SM_MODE_REINFORCEMENTS)) // return any key if selecting
                                          // reinforcement
                return i;
            ...
            // play general fail sound if inactive item hit.
                ...
                (i < Num_menu_items) &&
                (MsgItems[i].active)) // only return keys that are associated
                                      // with menu items
                return i;

            else {
                Msg_key_used = 0; // if no #-key pressed for visible item,
                                  // break and allow timer to
...
int hud_squadmsg_can_rearm (ship* shipp) {
    ...
    if ((shipp == Player_ship) && (Player_ship->team == Iff_traitor)) return 0;

    // 5/6/98 -- MWA  Decided to always be able to call in support.
    return 1;
}

// calls for repair/rearm of the player ship.  Checks for the presence of the
// support ship and does the appropriate action if found
    ...
    if (is_support_allowed (tobj)) {
        if (mission_is_repair_scheduled (tobj)) {
            ...
            if (robjnum != -1) {
                ...
                // warping in, objnum of repair ship otherwise
                ...
                if (robjnum != -1) {
                    ...
                    // ship has been warped in to service us.  Issue
                    // appropriate message
                    message_send_builtin_to_player (
                ...
                mission_log_add_entry (
                    LOG_PLAYER_CALLED_FOR_REARM,
                    Ships[tobj->instance].ship_name, NULL);
            }
        }
    }
...
// function which gets called from keyboard code to issues a shortcut command
// for rearming.
...
void hud_squadmsg_repair_rearm_abort (int toggle_state, object* obj) {
    ...
    // try to abort the request.  We shoudln't be in this function unless we
    // are actually queued for repair.  Send a message from support ship if the
    // support ship is in the mission
    ai_abort_rearm_request (tobj);

    // move the next statements outside of the above if-statement.  Seems like
    // this place is the right place, since we want to change state of the
    // messaging system regardless of what happened above.
    if (toggle_state) hud_squadmsg_toggle (); // take us out of message mode
}

// Goober5000 - redone and added a bit
// returns 1 if an order is valid for a ship.  Applies to things like departure
// when engines are blown, etc.
int hud_squadmsg_ship_order_valid (int shipnum, int order) {
    // Goober5000
    ...
    case DEPART_ITEM:
        ...
        if (shipp->flags[Ship::Ship_Flags::Disabled]) return 0;

        // Goober5000: also can't depart if no subspace drives and no valid
        // mothership
...
// find_order is true when we need to search the comm_orders array for the
// order entry.  We have to do this action in some cases since all we know is
// the actual "value" of the order
int hud_squadmsg_is_target_order_valid (
    int order, int find_order, ai_info* aip) {
    int target_objnum, i;
    ship *shipp, *ordering_shipp;
    object* objp;

    if (aip == NULL) aip = Player_ai;

    // find the comm_menu item for this command
    if (find_order) {
        for (i = 0; i < NUM_COMM_ORDER_ITEMS; i++) {
            if (Comm_orders[i].item == order) break;
        }
    ...
    if (!(Comm_orders[order].item & TARGET_MESSAGES)) return 1;

    target_objnum = aip->target_objnum;

    // order isn't valid if there is no player target
    if (target_objnum == -1) { return 0; }
    ...
    // if it's a weapon, then it needs to be a WIF_BOMB weapon.  Only attack
    // order valid, and only valid on bombs not on the player's team
    // MageKing17: Now also works on WIF3_FIGHTER_INTERCEPTABLE weapons.
    if (objp->type == OBJ_WEAPON) {
        if (Weapons[objp->instance].lssm_stage == 3) { return 0; }
        ...
        if ((Comm_orders[order].item == ATTACK_TARGET_ITEM) &&
            ((Weapon_info[Weapons[objp->instance].weapon_info_index]
             ...
             (Weapon_info[Weapons[objp->instance].weapon_info_index]
                  .wi_flags[Weapon::Info_Flags::Fighter_Interceptable])) &&
    ...
    // if we are messaging a ship, and that ship is our target, no target type
    // orders are ever active
    if ((Squad_msg_mode == SM_MODE_SHIP_COMMAND) &&
        (Objects[target_objnum].instance == Msg_instance)) {
        return 0;
    }

    // if the order is a disable order or depart, and the ship is disabled,
    // order isn't active
    if ((Comm_orders[order].item == DISABLE_TARGET_ITEM) &&
        (shipp->flags[Ship::Ship_Flags::Disabled])) {
        return 0;
    }

    // same as above except for disarmed.
    if ((Comm_orders[order].item == DISARM_TARGET_ITEM) &&
        ((shipp->subsys_info[SUBSYSTEM_TURRET].type_count == 0) ||
         (shipp->subsys_info[SUBSYSTEM_TURRET].aggregate_current_hits <=
          0.0f))) {
        return 0;
    }

    // if order is disable subsystem, and no subsystem targeted or no hits,
    // then order not valid
    if ((Comm_orders[order].item == DISABLE_SUBSYSTEM_ITEM) &&
    ...
    if ((shipp->team == ordering_shipp->team) &&
        (FRIENDLY_TARGET_MESSAGES & Comm_orders[order].item)) {
        return 1;
    }
    else if (
        (shipp->team != ordering_shipp->team) &&
        (ENEMY_TARGET_MESSAGES & Comm_orders[order].item)) {
        return 1;
    }
    else {
        return 0;
...
void hud_squadmsg_send_to_all_fighters (int command) {
    ...
    for (i = 0; i < Num_wings; i++) {
        ...
        if ((Wings[i].flags[Ship::Wing_Flags::Gone]) ||
            (Wings[i].current_count == 0))
            continue;

        if (Wings[i].flags[Ship::Wing_Flags::Departing]) continue;

        // get the first ship on the wing list and look at its team and then
        // its type
        ...
        if (shipp->team != ordering_shipp->team) continue;

        // can't message if ship not fighter/bomber if the command isn't to
        // everyone.
        if (!(Ship_info[shipp->ship_info_index].is_fighter_bomber ()))
            continue;

        // don't send the command if the "wing" won't accept the command.  We
        // do this by looking at the set of orders accepted for the wing leader
        if (!(command & shipp->orders_accepted)) continue;
    ...
    for (objp = GET_FIRST (&obj_used_list);
        ...
        // don't send messge to ships not on player's team, or that are in a
        // wing.
        shipp = &Ships[objp->instance];
...
int hud_squadmsg_enemies_present () {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (shipp->team != Player_ship->team) return 1;
    }
...
inline bool override_protect_ship_type (ship_info* sip) {
    return sip->is_fighter_bomber () ||
           sip->flags[Ship::Info_Flags::Freighter] ||
           sip->flags[Ship::Info_Flags::Transport];
}
// function which sends a message to a specific ship.  This routine can be
// called from one of two places.  Either after selecting a ship when using a
// hotkey, or after selecting a command when using the entire messaging menu
// system
//
...
int hud_squadmsg_send_ship_command (
    ...
    int message;
    int target_team, ship_team; // team id's for the ship getting message and
                                // any target the player has
    ship* ordering_shipp;
    ...
    // a shortcut to save on repetitive coding.  If the order is a 'target'
    // order, make the default mesage be "no target"
    ...
    if (hud_squadmsg_is_target_order_valid (command, 1, ainfo)) {
        ...
        if (ainfo->target_objnum != -1) {
            if (Objects[ainfo->target_objnum].type == OBJ_SHIP) {
                if (ainfo->target_objnum != Ships[shipnum].objnum) {
                    target_shipname =
                        Ships[Objects[ainfo->target_objnum].instance]
                            .ship_name; // I think this is right
                    target_team =
                        Ships[Objects[ainfo->target_objnum].instance].team;
                }
            }
        }
        ...
        ASSERT (ainfo->shipnum != -1);
        ship_team =
            Ships[ainfo->shipnum].team; // team of the ship issuing the message
        ...
        case ATTACK_TARGET_ITEM:
            if (Objects[ainfo->target_objnum].type == OBJ_SHIP) {
                ...
                if (override_protect_ship_type (
                        &Ship_info
                            [Ships[Objects[ainfo->target_objnum].instance]
                                 .ship_info_index])) {
                    Objects[ainfo->target_objnum].flags.remove (
                ...
                ai_submode =
                    Objects[ainfo->target_objnum]
                        .instance; // store the instance of the weapon -- ai
        ...
        case DISABLE_TARGET_ITEM:
            ...
            // Orders to override protect
            if (override_protect_ship_type (
                    &Ship_info[Ships[Objects[ainfo->target_objnum].instance]
                                   .ship_info_index])) {
                Objects[ainfo->target_objnum].flags.remove (
                    Object::Object_Flags::Protected);
            }
        ...
        case DISABLE_SUBSYSTEM_ITEM:
            ...
            ai_submode = ship_get_subsys_index (
                &Ships[Objects[ainfo->target_objnum].instance],
        ...
        case CAPTURE_TARGET_ITEM:
            ...
            Objects[ainfo->target_objnum].flags.set (
                Object::Object_Flags::Protected);
        ...
        case PROTECT_TARGET_ITEM:
            ...
            // AL 31-3-98: Can't protect self... this can happen if all
            // fighters
            // are told to protect another friendly ship
            if (ainfo->target_objnum == Ships[shipnum].objnum) { return 0; }
        ...
        case ENGAGE_ENEMY_ITEM:
            ...
            ai_submode = SM_ATTACK;
            // if no enemies present, use the affirmative, instead of engaging
            // enemies message
        ...
        case REARM_REPAIR_ME_ITEM:
            ...
            hud_update_last_order (
                ordering_shipp->ship_name, player_num, special_index);
            return 0;
        ...
        case STAY_NEAR_TARGET_ITEM: // can't attack anything on same team
            ...
            if ((command == STAY_NEAR_TARGET_ITEM) &&
                (ship_team != target_team))
                break;
        ...
        // handle case of messaging one ship.  Deal with messaging all fighters
        // next.
        if (ai_mode != AI_GOAL_NONE) {
            ASSERT (ai_submode != -1234567);
            ai_add_ship_goal_player (
                AIG_TYPE_PLAYER_SHIP, ai_mode, ai_submode, target_shipname,
                &Ai_info[Ships[shipnum].ai_index]);
            if (update_history == SQUADMSG_HISTORY_ADD_ENTRY) {
                hud_add_issued_order (Ships[shipnum].ship_name, command);
                hud_update_last_order (
                    target_shipname, player_num, special_index);
            }
    ...
    if (send_message &&
        ...
        message_send_builtin_to_player (
            message, &Ships[shipnum], MESSAGE_PRIORITY_NORMAL,
            MESSAGE_TIME_ANYTIME, 0, 0, player_num, message_team_filter);
    }

    return send_message;
}

// function to send a command to a wing.  Like above function, called from one
// of two places
//
// if local and addr are non-null, it means the function is being called by the
// (multiplayer) server in response to a PLAYER_COMMAND_PACKET
...
int hud_squadmsg_send_wing_command (
    ...
    // get the shipnum of the ship the player has targeted.  Used in enough
    // places to do this just once.  If the ship targeted is part of the wing
    // that was messages -- bail out!!!
    ...
    if (hud_squadmsg_is_target_order_valid (command, 1, ainfo)) {
        ...
        if (ainfo->target_objnum != -1) {
            if (Objects[ainfo->target_objnum].type == OBJ_SHIP) {
                target_shipname = Ships[Objects[ainfo->target_objnum].instance]
                                      .ship_name; // I think this is right
                target_team =
                    Ships[Objects[ainfo->target_objnum].instance].team;
            }
        ...
        case ATTACK_TARGET_ITEM:
            if (Objects[ainfo->target_objnum].type == OBJ_SHIP) {
                ...
                if ((Ships[Objects[ainfo->target_objnum].instance].wingnum !=
                     -1) &&
                    (Ships[Objects[ainfo->target_objnum].instance].wingnum ==
                     wingnum)) {
                    message = MESSAGE_NOSIR;
        ...
        if (ai_mode != AI_GOAL_NONE) {
            ...
            ai_add_wing_goal_player (
                AIG_TYPE_PLAYER_WING, ai_mode, ai_submode, target_shipname,
                wingnum);
    ...
    if (send_message) {
        ...
        // valid in the message menu, but the get_random_ship* functions won't
        // return dying ships. Karajorma - No valid ships will be found if all
        // the remaining ships have been silence either.
        if (ship_num != -1) {
            message_send_builtin_to_player (
                message, &Ships[ship_num], MESSAGE_PRIORITY_NORMAL,
                MESSAGE_TIME_ANYTIME, 0, 0, player_num, message_team_filter);
            message_sent = 1;
        }
    }
...
int hud_squadmsg_reinforcements_available (int team) {
    ...
    for (i = 0; i < Num_reinforcements; i++) {
        ...
        // no more left
        if (Reinforcements[i].num_uses >= Reinforcements[i].uses) { continue; }

        // incorrect team
        if (team != ship_get_reinforcement_team (i)) { continue; }

        // check the arrival cue sexpression of the ship/wing of this
        // reinforcement.
        // Goober5000 - if it can't arrive, it doesn't count.  This should
        // check for SEXP_FALSE as well as SEXP_KNOWN_FALSE, otherwise you end
        // up with a reinforcement menu containing no valid selections.
        if ((wingnum = wing_name_lookup (Reinforcements[i].name, 1)) != -1) {
            ...
            if (Sexp_nodes[Wings[wingnum].arrival_cue].value == SEXP_FALSE ||
                Sexp_nodes[Wings[wingnum].arrival_cue].value ==
                    SEXP_KNOWN_FALSE) {
                continue;
            }
        }
            ...
            if (p_objp != NULL) {
                if (Sexp_nodes[p_objp->arrival_cue].value == SEXP_FALSE ||
                    Sexp_nodes[p_objp->arrival_cue].value ==
                        SEXP_KNOWN_FALSE) {
                    continue;
                }
            }
...
// function to put up window in upper right to allow for player to select the
// type of entity to select for a message (i.e. a wing or a ship)
void hud_squadmsg_type_select () {
    ...
    // based on ship counts, wing counts, shortcut active, grey out possible
    // menu choices
    if (!hud_squadmsg_count_ships (0)) MsgItems[TYPE_SHIP_ITEM].active = 0;

    if (!hud_squadmsg_count_wings (0)) MsgItems[TYPE_WING_ITEM].active = 0;

    // check to be sure that we have some fighters/bombers on the players team
    // that we can message
    ...
    if ((Player_ship != NULL) &&
        !hud_squadmsg_reinforcements_available (Player_ship->team)) {
        MsgItems[TYPE_REINFORCEMENT_ITEM].active = 0;
    }

    MsgItems[TYPE_REPAIR_REARM_ITEM].active =
        1; // this item will always be available (I think)
    ...
    // check to see if the player is awaiting repair or being repaired.  Active
    // the abort and inactive the repair items check to see if the player is
    // scheduled to be repaired by incoming ship
    if (Ai_info[Ships[Player_obj->instance].ai_index].ai_flags
    ...
    // de-activate the rearm/repair item if the player has a full load of
    // missiles and all subsystems at full strength.  We will only check if
    // this item hasn't been marked inactive because of some other reason
...
void hud_squadmsg_ship_select () {
    ...
    if (k != -1) { // if true, we have selected a ship.
        if (Msg_shortcut_command == -1) {
            Msg_instance = MsgItems[First_menu_item + k]
                               .instance; // store the instance id in a global
            hud_squadmsg_do_mode (
                SM_MODE_SHIP_COMMAND); // and move to a new mode
        }
        else {
            // we must convert the Msg_shortcut_command value to a value that
            // the message system normally uses to select a command.  Since the
            // menu
            // ASSERT (Msg_shortcut_command != IGNORE_TARGET_ITEM );
            hud_squadmsg_send_ship_command (
                MsgItems[First_menu_item + k].instance, Msg_shortcut_command,
                1, SQUADMSG_HISTORY_ADD_ENTRY);
            hud_squadmsg_toggle ();
        }
    }
}
    ...
    if (k != -1) { // if true, we have selected a ship.
        if (Msg_shortcut_command ==
            ...
            Msg_instance = MsgItems[First_menu_item + k]
                               .instance; // store the instance id in a global
            hud_squadmsg_do_mode (
                SM_MODE_WING_COMMAND); // and move to a new mode
        }
        else {
            // ASSERT (Msg_shortcut_command != IGNORE_TARGET_ITEM );
            hud_squadmsg_send_wing_command (
...
// code which gives an order to all fighters/bombers.  If there is a message
// shortcut active, then make that order apply to all fighters/bombers.
// Otherwise, move to the ship_command menu
...
void hud_squadmsg_call_reinforcement (int reinforcement_num) {
    ...
    // if we found no wing name that matched the reinforcement name, then look
    // for a ship of the same name
    if (i == Num_wings) {
        p_objp = mission_parse_get_arrival_ship (rp->name);
        if (p_objp) {
            // by resetting the reinforcement flag, we will allow code which
            // normally handles arrivals to make this reinforcement arrive.
            // Doing so keeps the data structures clean.
            p_objp->flags.remove (
                Mission::Parse_Object_Flags::SF_Reinforcement);
    ...
    // increment the number of times this is used.  Incremented here on single
    // player and multiplayer server side only.  Clients keep track of own
    // count when they actually call something in.
    rp->num_uses++;

    // commented out on 9/9/98 because these messages simply are not used
    /*
    // now play a message (if there is one to play) for this reinforcement
    arrival.  The first for loop
    // determine how many messages there are to play, since the array is
    packet.  Then, if >= 1 message
    // to play, play one
    ...
    //if ( i > 0 )
    // message_send_to_player( rp->yes_messages[myrand() % i], rp->name,
    MESSAGE_PRIORITY_NORMAL, HUD_SOURCE_FRIENDLY );
    */

    mission_log_add_entry (
        LOG_PLAYER_CALLED_FOR_REINFORCEMENT, rp->name, NULL);
}
...
void hud_squadmsg_reinforcement_select () {
    ...
    if (Num_menu_items == -1) {
        ...
        for (i = 0; i < Num_reinforcements; i++) {
            ...
            // don't put reinforcements onto the list that have already been
            // used up.
            if (rp->num_uses == rp->uses) { continue; }

            // don't put items which are not on my team
            if ((Player_ship != NULL) &&
                (ship_get_reinforcement_team (i) != Player_ship->team)) {
                continue;
            }
            ...
            // reinforcement.
            // Goober5000 - if it can't arrive, it doesn't count.  This should
            // check for SEXP_FALSE as well as SEXP_KNOWN_FALSE, otherwise you
            // end up with a reinforcement menu containing no valid selections.
            if ((wingnum = wing_name_lookup (rp->name, 1)) != -1) {
                ASSERT (Wings[wingnum].arrival_cue >= 0);
                if (Sexp_nodes[Wings[wingnum].arrival_cue].value ==
                        SEXP_FALSE ||
                    Sexp_nodes[Wings[wingnum].arrival_cue].value ==
                        SEXP_KNOWN_FALSE) {
                    continue;
...
void hud_squadmsg_ship_command () {
    ...
    // when adding ship commands, we must look at the type of ship, and what
    // messages that ship allows.  First, place all messages that are possible
    // onto the menu, then

    // see if messaging all ships or just one.  Messaging all ships will mean
    // all default orders show on comm menu.
    if (Msg_instance != MESSAGE_ALL_FIGHTERS) {
        ...
        default_orders = ship_get_default_orders_accepted (
            &Ship_info[Ships[Msg_instance].ship_info_index]);
    }
    else {
    ...
    Num_menu_items = 0;
    for (i = 0; i < NUM_COMM_ORDER_ITEMS; i++) {
        // check to see if the comm order should even be added to the menu --
        // if so, then add it the order will be activated if the bit is set for
        // the ship.
        if (default_orders & Comm_orders[i].item) {
            ...
            // if the order cannot be carried out by the ship, then item should
            // be inactive
            if ((Msg_instance != MESSAGE_ALL_FIGHTERS) &&
                !hud_squadmsg_ship_order_valid (
                    Msg_instance, Comm_orders[i].item))
                MsgItems[Num_menu_items].active = 0;

            // do some other checks to possibly gray out other items.
            // if no target, remove any items which are associated with the
            // players target
            if (!hud_squadmsg_is_target_order_valid (i, 0))
                MsgItems[Num_menu_items].active = 0;

            // if messaging all fighters, see if we should gray out the order
            // if no one will accept it, or modify the text if only some of the
            // ships will accept it
            if (Msg_instance == MESSAGE_ALL_FIGHTERS) {
                ship_obj* so;
                ship* shipp;
                int partial_accept, all_accept; // value which tells us what to
                                                // do with menu item
                ...
                for (so = GET_FIRST (&Ship_obj_list);
                    ...
                    // don't send messge to ships not on player's team, or that
                    // are in a wing.
                    shipp = &Ships[Objects[so->objnum].instance];
                    if (shipp->team != Player_ship->team) continue;
                    ...
                    all_accept &=
                        shipp->orders_accepted; // 'and'ing will either keep
                                                // this bit set or zero it
                                                // properly
                    partial_accept |=
                        (shipp->orders_accepted &
                         Comm_orders[i].item); // 'or'ing will tell us if at
                                               // least one accepts
                }
    ...
    if (k != -1) {
        ASSERT (k < Num_menu_items);
        // when messaging all fighters or ignoring target, call the
        // send_to_all_fighters routine Goober5000 - ignore no longer sends to
        // all fighters
        if (Msg_instance == MESSAGE_ALL_FIGHTERS)
            hud_squadmsg_send_to_all_fighters (MsgItems[k].instance);
        else
            hud_squadmsg_send_ship_command (
                Msg_instance, MsgItems[k].instance, 1,
...
void hud_squadmsg_wing_command () {
    ...
    // when adding commands for wings, we will look at all of the ships in the
    // wing, and create the order list from that set of ships.  In the future,
    ...
    for (i = 0; i < wingp->current_count; i++) {
        orders = ship_get_default_orders_accepted (
            &Ship_info[Ships[wingp->ship_index[i]].ship_info_index]);
        default_orders |= orders;
    }
    default_orders &=
        ~CAPTURE_TARGET_ITEM; // we cannot capture any target with a wing.

    Num_menu_items = 0;
    shipnum = wingp->ship_index[wingp->special_ship];
    ASSERTX (
        shipnum >= 0,
        "Special ship (%d) for wing '%s' has a negative ship_index (%d). This "
        "should not happen; get a coder!\n",
        wingp->special_ship, wingp->name, shipnum);
    orders = Ships[shipnum].orders_accepted; // get the orders that the wing
                                             // leader will accept
    for (i = 0; i < NUM_COMM_ORDER_ITEMS; i++) {
        // add the set of default orders to the comm menu.  We will currently
        ...
        if (default_orders & Comm_orders[i].item) {
            ...
            // possibly grey out the menu item depending on whether or not the
            // "wing" will accept this order the "wing" won't accept the order
            // if the first ship in the wing doesn't accept it.
            if (orders & Comm_orders[i].item)
                MsgItems[Num_menu_items].active = 1;
...
// this code below will get called only the key config changes (from
// ControlsConfig.cpp) or if the bindings haven't been saved yet.  This code
// doesn't remove the bindings but just sets up the array so that the bindings
// can be removed when messaging mode is entered.
//
...
void hud_squadmsg_save_keys (int /*do_scroll*/) {
    ...
    /*
       ...
       match page up and page down
                    // then skip them.
                    if ( !do_scroll && ((keys_used[j] == KEY_PAGEDOWN) ||
...
// previous player flag value.  I thought it better to isolate all system
// changes in this code.
void hud_squadmsg_toggle () {
    // if the emp effect is active, always ignore this
    if (emp_active_local ()) { return; }

    // if entering this mode, must setup messaging system.  Don't start
    // squadmessging if the player is dead.
    if (!(Player->flags & PLAYER_FLAGS_MSG_MODE)) {
...
void hud_enemymsg_toggle () {
    hud_squadmsg_toggle ();
    // if we just entered message mode, turn on var that says to message
    // enemies
    if (Player->flags & PLAYER_FLAGS_MSG_MODE) Msg_enemies = 1;
}

// external entry point into code when a keyboard shortcut is used for a
// command we are passed in an ID for the command to set internal variables.
// This command will be used in place of the last menu in the messaging code
void hud_squadmsg_shortcut (int command) {
    ...
    if ((hud_communications_state (Player_ship) != COMM_OK) &&
        ...
        HUD_sourced_printf (
            HUD_SOURCE_HIDDEN, "%s",
            XSTR (
                "Messaging is restricted due to communications damage", 331));
        return;
    }
    ...
    if (Player->flags &
        PLAYER_FLAGS_MSG_MODE) // we are already in messaging mode -- maybe do
...
// (1-0) while in messaging mode.  If we are in messaging mode, send the
// shortcut command to the ships that are part of the passed in selection set.
// If there is no shortcut command, do nothing.  Returns 1 if the key was used,
// else 0.  This return value is used to tell the key control system that it
// should call the normal targeting selection stuff.
int hud_squadmsg_hotkey_select (int k) {
    ...
    if (Msg_shortcut_command == -1) return 0;
    ...
    send_message = 1;
    // for each ship in the selection set list, send the shortcut command that
    // the player previously entered.  Be sure to check that we are not trying
    // to send a command to an enemy ship.
    for (hitem = GET_FIRST (plist); hitem != END_OF_LIST (plist);
         hitem = GET_NEXT (hitem)) {
        objp = hitem->objp;
        ASSERT (objp->type == OBJ_SHIP);
        if (Ships[objp->instance].team != Player_ship->team) continue;
        ...
        hud_squadmsg_send_ship_command (
            objp->instance, Msg_shortcut_command, send_message,
            SQUADMSG_HISTORY_ADD_ENTRY);
        send_message = 0;
    }
...
// the next function is called once a frame when the player is messaging
// someone in his squad.  After a period of 5 seconds of inactivity (i.e. no
// keypress to select something in the menu), the menu will disappear.  This
// function will only get called if the player flag PLAYER_FLAG_MSG_MODE is
// set.  Parameter is the key that was hit this frame
...
int hud_squadmsg_do_frame () {
    ...
    ASSERT (
        Player->flags &
        PLAYER_FLAGS_MSG_MODE); // be sure that messaging mode is set!!!

    // if the player is now dead, or the timestamp elapsed, then get out of
    // messaging mode.
...
void hud_add_issued_order (const char* name, int order) {
    ...
    if (!strcmp (name, "All Fighters")) { latest_order->order_to = -1; }
    else {
                ...
                if (target == -1 || Squadmsg_history[i].target == target) {
                    if ((!timestamp) ||
                        ...
                        if (order & DISABLE_SUBSYSTEM_ITEM) {
                            if (special_argument == NULL) { continue; }

                            int target_ship = ship_name_lookup (target_name);

                            if (target_ship < 0) { continue; }

                            int subsys_index = ship_get_subsys_index (
                                &Ships[target_ship], special_argument, 1);
                            // if the order is for s different subsystem
                            if (Squadmsg_history[i].special_index !=
                                subsys_index) {
                                continue;
                            }
                        }
...
HudGaugeSquadMessage::HudGaugeSquadMessage ()
    : HudGauge (
          HUD_OBJECT_SQUAD_MSG, HUD_MESSAGE_BOX, false, false,
...
void HudGaugeSquadMessage::initItemHeight (int h) { Item_h = h; }

void HudGaugeSquadMessage::initItemOffsetX (int x) { Item_offset_x = x; }
...
void HudGaugeSquadMessage::initBitmaps (
...
bool HudGaugeSquadMessage::maybeFlashPageScroll (bool flash_fast) {
    ...
    if (!timestamp_elapsed (flash_timer[0])) {
        ...
        if (flash_flag) { draw_bright = true; }
    }
...
bool HudGaugeSquadMessage::canRender () {
    if (hud_disabled_except_messages () && !message_gauge) { return false; }

    if (hud_disabled ()) { return false; }

    if (!(Game_detail_flags & DETAIL_FLAG_HUD)) { return false; }

    if ((Viewer_mode & disabled_views)) { return false; }

    if (!(Player->flags & PLAYER_FLAGS_MSG_MODE)) { return false; }
...
void HudGaugeSquadMessage::render (float /*frametime*/) {
    ...
    if (title) {
        renderString (
            position[0] + Header_offsets[0], position[1] + Header_offsets[1],
            title);
    }
    ...
    if (Num_menu_items < MAX_MENU_DISPLAY)
        ...
        if (First_menu_item ==
            0) // First_menu_item == 0 means first page of items
            nitems = MAX_MENU_DISPLAY;
        else if (
            (Num_menu_items - First_menu_item) <=
            MAX_MENU_DISPLAY) // check if remaining items fit on one page
    ...
    by =
        position[1] +
        Middle_frame_start_offset_y; // global y-offset where bitmap gets drawn

    none_valid = 1; // variable to tell us whether all items in the menu are
                    // valid or not
    ...
    for (i = 0; i < nitems; i++) {
        ...
        // if we have at least one item active, then set the variable so we
        // don't display any message about no active items
        if (MsgItems[First_menu_item + i].active) none_valid = 0;
    }

    // maybe draw an extra line in to make room for [pgdn], or for the 'no
    // active items' display
    if (!messaging_allowed || none_valid ||
        ((First_menu_item + nitems) < Num_menu_items) ||
    ...
    // determine if we should put the text "[more]" at top or bottom to
    // indicate you can page up or down
    startFlashPageScroll ();
    maybeFlashPageScroll ();
    if (First_menu_item > 0) {
        renderPrintf (
            position[0] + Pgup_offsets[0], position[1] + Pgup_offsets[1], "%s",
            XSTR ("[pgup]", 312));
    }

    if ((First_menu_item + nitems) < Num_menu_items) {
        renderPrintf (
            position[0] + Pgdn_offsets[0], position[1] + Pgdn_offsets[1], "%s",
            XSTR ("[pgdn]", 313));
    }
    ...
    if (messaging_allowed) {
        if (none_valid) {
            renderPrintf (
                sx, by - Item_h + 2, "%s", XSTR ("No valid items", 314));
        }
        else if (Msg_shortcut_command != -1) {
            renderPrintf (
                sx, by - Item_h + 2, "%s",
                comm_order_get_text (Msg_shortcut_command));
        }
    }
    else {
        // if this player is not allowed to message, then display message
        // saying so
        renderPrintf (
            sx, by - Item_h + 2, "%s", XSTR ("Not allowed to message", 315));
    }
}
...
void HudGaugeSquadMessage::pageIn () {
    for (int i = 0; i < NUM_MBOX_FRAMES; i++) {
        bm_page_in_aabitmap (
            Mbox_gauge[i].first_frame, Mbox_gauge[i].num_frames);
    }
}
