        ...
        for (i = 0; i < num_keys_used; i++) {
                if (k == keys_used[i]) {
                        ...
                        if (k == KEY_PAGEDOWN) { // pageup and pagedown scroll the menu --
                                ...
                                (i < Num_menu_items) && (Squad_msg_mode == SM_MODE_REINFORCEMENTS)) // return any key if selecting
                                                                                                    // reinforcement
                                return i;
                        ...
                        // play general fail sound if inactive item hit.
                                ...
                                (i < Num_menu_items) && (MsgItems[i].active)) // only return keys that are associated
                                                                              // with menu items
                                return i;
        ...
        int order, int find_order, ai_info *aip)
{
        ...
        if (objp->type == OBJ_WEAPON) {
                ...
                if ((Comm_orders[order].item == ATTACK_TARGET_ITEM) && ((Weapon_info[Weapons[objp->instance].weapon_info_index].wi_flags[Weapon::Info_Flags::Bomb]) || (Weapon_info[Weapons[objp->instance].weapon_info_index].wi_flags[Weapon::Info_Flags::Fighter_Interceptable])) && (Weapons[objp->instance].team != ordering_shipp->team))
        ...
        if ((shipp->team == ordering_shipp->team) && (FRIENDLY_TARGET_MESSAGES & Comm_orders[order].item)) {
                ...
                (shipp->team != ordering_shipp->team) && (ENEMY_TARGET_MESSAGES & Comm_orders[order].item)) {
                return 1;
        } else {
                return 0;
        }
        ...
        if (hud_squadmsg_is_target_order_valid(command, 1, ainfo)) {
                ...
                case ATTACK_TARGET_ITEM:
                        if (Objects[ainfo->target_objnum].type == OBJ_SHIP) {
                                ...
                                ai_submode = Objects[ainfo->target_objnum]
...
int hud_squadmsg_reinforcements_available(int team)
{
        ...
        for (i = 0; i < Num_reinforcements; i++) {
                ...
                if ((wingnum = wing_name_lookup(Reinforcements[i].name, 1)) != -1) {
                        ...
                        if (Sexp_nodes[Wings[wingnum].arrival_cue].value == SEXP_FALSE || Sexp_nodes[Wings[wingnum].arrival_cue].value == SEXP_KNOWN_FALSE) {
                                continue;
                        }
                } else {
                        p_object *p_objp;
                        ...
                        if (p_objp != NULL) {
                                if (Sexp_nodes[p_objp->arrival_cue].value == SEXP_FALSE || Sexp_nodes[p_objp->arrival_cue].value == SEXP_KNOWN_FALSE) {
                                        continue;
                                }
                        } else {
                                ASSERT(0);
...
void hud_squadmsg_call_reinforcement(int reinforcement_num)
{
        ...
        for (i = 0; i < Num_wings; i++) {
                if (!strcasecmp(rp->name, Wings[i].name)) {
                        ...
                        if (delay == 0)
                                delay = (int)(fs2::prng::randf(0) * 3.0) + 3;
        ...
        if (i == Num_wings) {
                ...
                if (p_objp) {
                        ...
                        delay = rp->arrival_delay;
                        if (delay == 0)
                                delay = (int)(fs2::prng::randf(0) * 3.0) + 3; // between 3 and 6 seconds to arrive
                        ...
                        if (Msg_instance == MESSAGE_ALL_FIGHTERS) {
                                ...
                                for (so = GET_FIRST(&Ship_obj_list);
                                     so != END_OF_LIST(&Ship_obj_list); so = GET_NEXT(so)) {
                                        ...
                                        all_accept &= shipp->orders_accepted;                             // 'and'ing will either keep
                                                                                                          // this bit set or zero it
                                                                                                          // properly
                                        partial_accept |= (shipp->orders_accepted & Comm_orders[i].item); // 'or'ing will tell us if at
                                                                                                          // least one accepts
                                }
...
void hud_squadmsg_shortcut(int command)
{
        ...
        if (Player->flags & PLAYER_FLAGS_MSG_MODE) // we are already in messaging mode -- maybe do
...
int hud_squadmsg_do_frame()
{
        ...
        ASSERT(
                Player->flags & PLAYER_FLAGS_MSG_MODE); // be sure that messaging mode is set!!!
...
void HudGaugeSquadMessage::render(float /*frametime*/)
{
        ...
        if (Num_menu_items < MAX_MENU_DISPLAY)
                ...
                if (First_menu_item == 0) // First_menu_item == 0 means first page of items
                        ...
                        (Num_menu_items - First_menu_item) <= MAX_MENU_DISPLAY) // check if remaining items fit on one page
