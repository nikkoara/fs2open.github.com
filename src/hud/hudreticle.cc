                        ...
                        if (pm->n_guns > 0) {
                                ...
                                for (int i = 0; i < pm->n_guns; i++) {
                                        ...
                                        if (!timestamp_elapsed(
                                                ...
                                                i == shipp->weapons.current_primary_bank || shipp->flags[Ship::Ship_Flags::Primary_linked])
                                                bankactive = 2;
...
void HudGaugeThreatIndicator::renderLaserThreat()
{
        ...
        if (Player->threat_flags & THREAT_DUMBFIRE) {
                if (timestamp_elapsed(laser_warn_timer)) {
                        ...
                        laser_warn_frame++;
                        if (laser_warn_frame > (num_frames - 1)) { // The first frame being the default "off" setting, we
...
void HudGaugeThreatIndicator::renderLockThreat()
{
        ...
        if (Player->threat_flags & (THREAT_LOCK | THREAT_ATTEMPT_LOCK)) {
                if (timestamp_elapsed(lock_warn_timer)) {
                        ...
                        lock_warn_frame++;
                        if (lock_warn_frame > (num_frames - 1)) { // The first frame being the default "off" setting, we
                                                                  // need to cycle through all the other frames
                                lock_warn_frame = 1;
                        }
                }
        ...
        char *fname_secondary_link_3)
{
        ...
        weapon_linking_modes[LINK_THREE_SECONDARY].first_frame = bm_load_animation(
