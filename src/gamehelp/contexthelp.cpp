...
static int current_helpid = -1; // the currently active overlay_id, only really
                                // used for the debug console funxions
...
void help_overlay_set_state (int overlay_id, int resolution_index, int state) {
    ...
    if ((overlay_id >= 0) && (overlay_id < num_help_overlays) &&
        (resolution_index >= 0) &&
        (resolution_index < help_overlaylist[overlay_id].num_resolutions)) {
        if (state > 0) {
...
void help_overlay_reset_all () { Help_overlay_flags = 0; }

// Set up Grey_shader, which is used game-wide to grey out background when
// using help overlays
...
void launch_context_help () {
    ...
    case GS_STATE_BRIEFING: overlay_id = Briefing_overlay_id; break;

    case GS_STATE_SHIP_SELECT: overlay_id = Ship_select_overlay_id; break;

    case GS_STATE_WEAPON_SELECT: overlay_id = Weapon_select_overlay_id; break;

    case GS_STATE_BARRACKS_MENU: overlay_id = Barracks_overlay_id; break;
    ...
    case GS_STATE_CAMPAIGN_ROOM: overlay_id = Campaign_room_overlay_id; break;

    case GS_STATE_SIMULATOR_ROOM: overlay_id = Sim_room_overlay_id; break;

    case GS_STATE_TECH_MENU: overlay_id = Techroom_overlay_id; break;
...
void close_help () {
    for (int overlay_id = 0; overlay_id < MAX_HELP_OVERLAYS; overlay_id++) {
        if (!help_overlaylist[overlay_id].textlist.empty ()) {
            for (std::vector< help_text >::iterator ii =
    ...
    try {
        ...
        while (optional_string ("$")) {
            ...
            if (optional_string ("+font")) {
                int font_index;
                for (i = 0; i < help_overlaylist[overlay_id].num_resolutions;
                     i++) {
                    stuff_int (&font_index);
                    help_overlaylist[overlay_id].fontlist.push_back (
                        font_index);
                }
            }
            else {
            ...
            for (i = 0; i < help_overlaylist[overlay_id].num_resolutions;
                ...
                help_overlaylist[overlay_id].rbracketlist.push_back (
                    rbracket_temp);
                help_overlaylist[overlay_id].lbracketlist.push_back (
                    lbracket_temp);
            }

            int type;
            // read in all elements for this overlay
            while ((type = required_string_one_of (
                        5, "+pline", "+text", "+right_bracket",
                        "+left_bracket", "$end")) !=
                   4) { // Doing it this way means an error lists "$end" at the
                        // end, which seems appropriate. -MageKing17
                ...
                case 0: // +pline
                    ...
                    for (i = 0;
                         i < help_overlaylist[overlay_id].num_resolutions;
                         i++) {
                        help_overlaylist[overlay_id]
                            .plinelist.at (i)
                            .push_back (pline_temp2);
                        for (j = 0; j < vtxcount; j++) {
                            help_overlaylist[overlay_id]
                                .plinelist.at (i)
                                .at (currcount)
                                .vtx.push_back (vec3d_temp);
                            help_overlaylist[overlay_id]
                                .plinelist.at (i)
                                .at (currcount)
                                .vtxcount = vtxcount;
                ...
                case -1:
                    ...
                    break;
                case 4: // $end
                default: UNREACHABLE ("This should never happen.\n"); break;
                }
            } // end while
            required_string ("$end");
        } // end while
...
void help_overlay_blit (int overlay_id, int resolution_index) {
    ...
    // this draws each line of help text with white on black text (use the
    // first resolution index for the string)
    font::set_font (
        help_overlaylist[overlay_id].fontlist.at (resolution_index));
    for (idx = 0; idx < textcount; idx++) {
        gr_set_color_fast (&Color_black);
        gr_get_string_size (
            &width, &height,
            help_overlaylist[overlay_id].textlist.at (0).at (idx).string,
            (int)strlen (
                help_overlaylist[overlay_id].textlist.at (0).at (idx).string));
        gr_rect (
            help_overlaylist[overlay_id]
                    .textlist.at (resolution_index)
                    .at (idx)
                    .x_coord -
                2 * HELP_PADDING,
            help_overlaylist[overlay_id]
    ...
    for (idx = 0; idx < rbracketcount; idx++) {
        ...
        gr_bitmap (
            help_overlaylist[overlay_id]
                .rbracketlist.at (resolution_index)
                .at (idx)
                .x_coord,
            help_overlaylist[overlay_id]
                .rbracketlist.at (resolution_index)
    ...
    for (idx = 0; idx < lbracketcount; idx++) {
        ...
        gr_bitmap (
            help_overlaylist[overlay_id]
                .lbracketlist.at (resolution_index)
                .at (idx)
                .x_coord,
            help_overlaylist[overlay_id]
                .lbracketlist.at (resolution_index)
    ...
    for (idx = 0; idx < plinecount; idx++) {
        gr_pline_special (
            &help_overlaylist[overlay_id]
                 .plinelist.at (resolution_index)
                 .at (idx)
                 .vtx,
            HELP_PLINE_THICKNESS, GR_RESIZE_MENU);
    }
}
...
void nudgetext_x (int textnum, int amount) {
    help_overlaylist[current_helpid]
        .textlist.at (current_resolution)
        .at (textnum)
        .x_coord += amount;
}
...
void nudgepline_x (int plinenum, int plinevert, int amount) {
    help_overlaylist[current_helpid]
        .plinelist.at (current_resolution)
        .at (plinenum)
        .vtx[plinevert]
        .xyz.x += amount;
}
...
void nudgerbracket_x (int num, int amount) {
    help_overlaylist[current_helpid]
        .rbracketlist.at (current_resolution)
        .at (num)
        .x_coord += amount;
}
...
void nudgelbracket_x (int num, int amount) {
    help_overlaylist[current_helpid]
        .lbracketlist.at (current_resolution)
        .at (num)
        .x_coord += amount;
}
...
void showtextpos (int textnum) {
    dc_printf (
        "text %d is now located at (%d, %d)", textnum,
        help_overlaylist[current_helpid]
            .textlist.at (current_resolution)
            .at (textnum)
            .x_coord,
...
void showrbracketpos (int num) {
    dc_printf (
        "rbracket %d is now located at (%d, %d)", num,
        help_overlaylist[current_helpid]
            .rbracketlist.at (current_resolution)
            .at (num)
            .x_coord,
...
void showlbracketpos (int num) {
    dc_printf (
        "lbracket %d on overlay %d is now located at (%d, %d)", num,
        current_helpid,
        help_overlaylist[current_helpid]
            .lbracketlist.at (current_resolution)
            .at (num)
...
void showplinepos (int plinenum) {
    int i;
    dc_printf (
        "pline %d on overlay %d vertices are now ", plinenum, current_helpid);
    for (i = 0; i < help_overlaylist[current_helpid]
                        .plinelist.at (gr_screen.res)
                        .at (plinenum)
                        .vtxcount;
         i++) {
        dc_printf (
...
DCF (help_nudgepline_x, "Use to visually position overlay polylines.") {
    if (dc_optional_string_either ("help", "--help")) {
        dc_printf (
            "Usage: help_nudgepline [pline_number] [vertex_number] "
            "[distance]\n");
        return;
    }
