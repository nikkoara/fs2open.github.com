...
// set this to GP_LAST_SCREEN_SINGLE or GP_LAST_SCREEN_MULTI based upon what
// game mode we're in
int Gp_last_screen;
...
static UI_XSTR Game_help_text[GR_NUM_RESOLUTIONS][GAME_HELP_NUM_TEXT] = {
    { // GR_640
      { "Press ESC to return to the game", 1441, 263, 389, UI_XSTR_COLOR_GREEN,
        -1, NULL },
      { "Continue", 1069, 571, 413, UI_XSTR_COLOR_GREEN, -1,
        &Buttons[gr_screen.res][CONTINUE_BUTTON].button } },
    { // GR_1024
      { "Press ESC to return to the game", 1441, 421, 622, UI_XSTR_COLOR_GREEN,
        -1, NULL },
      { "Continue", 1069, 928, 663, UI_XSTR_COLOR_GREEN, -1,
        &Buttons[gr_screen.res][CONTINUE_BUTTON].button } }
};

static const char* Game_help_filename[GR_NUM_RESOLUTIONS] = { "F1", "2_F1" };

static const char* Game_help_mask_filename[GR_NUM_RESOLUTIONS] = { "F1-m",
                                                                   "2_F1-m" };
...
void gameplay_help_blit_control_line_raw (
    int x, int y, const char* control_text, const char* control_description) {
    gr_string (x, y, control_text, GR_RESIZE_MENU);
    gr_string (
        x + KEY_DESCRIPTION_OFFSET, y, control_description, GR_RESIZE_MENU);
}
...
void gameplay_help_set_title (const char* title) {
    ...
    sprintf (
        buf, XSTR ("Page %d of %d", 132), Current_help_page + 1,
        Gp_last_screen + 1);
    gr_get_string_size (&w, NULL, buf);
    gr_printf_menu (
        (gr_screen.clip_width_unscaled - w) / 2,
        sy + gr_get_font_height () + 2, "%s", buf);
...
void gameplay_help_init () {
    ...
    if (Gameplay_help_inited) { return; }
    ...
    if (Game_mode & GM_MULTIPLAYER) { Gp_last_screen = GP_LAST_SCREEN_MULTI; }
    else {
        Gp_last_screen = GP_LAST_SCREEN_SINGLE;
    }

    // setup hotkeys so lights flash when keys are pressed
    Buttons[gr_screen.res][CONTINUE_BUTTON].button.set_hotkey (
...
void gameplay_help_process_key (int k) {
    switch (k) {
    case KEY_ESC: gameplay_help_leave (); break;
...
void gameplay_help_button_pressed (int n) {
    switch (n) {
    case PREVIOUS_PAGE_BUTTON: gameplay_help_goto_prev_screen (); break;

    case NEXT_PAGE_BUTTON: gameplay_help_goto_next_screen (); break;
...
void gameplay_help_draw_text () {
    ...
    case GP_HELP_BASIC_KEYS: {
        ...
        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TOGGLE_AUTO_TARGETING);
        ...
        gr_printf_menu (
            (gr_screen.clip_width - w) / 2, y_offset, "%s",
            XSTR ("Function Keys", 134));
        gr_set_color_fast (&Color_normal);
        ...
        y_offset += separation;
        gameplay_help_blit_control_line_raw (
            x_offset, y_offset, XSTR ("F3", 139),
            XSTR ("hotkey assignment", 140));

        y_offset += separation;
        gameplay_help_blit_control_line_raw (
            x_offset, y_offset, XSTR ("F4", 141),
        ...
        y_offset += separation;
        gameplay_help_blit_control_line_raw (
            x_offset, y_offset, XSTR ("Shift-Esc", 145),
    ...
    case GP_HELP_MOVEMENT_KEYS:
        ...
        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, ONE_THIRD_THROTTLE);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TWO_THIRDS_THROTTLE);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, MINUS_5_PERCENT_THROTTLE);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, PLUS_5_PERCENT_THROTTLE);
        ...
        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, BANK_WHEN_PRESSED);
        break;
    ...
    case GP_HELP_COMMON_TARGET_KEYS:
        ...
        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TARGET_SHIP_IN_RETICLE);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TARGET_NEXT_CLOSEST_HOSTILE);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TARGET_PREV_CLOSEST_HOSTILE);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TARGET_NEXT_CLOSEST_FRIENDLY);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TARGET_PREV_CLOSEST_FRIENDLY);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TARGET_NEXT_SUBOBJECT);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TARGET_PREV_SUBOBJECT);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TARGET_SUBOBJECT_IN_RETICLE);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, MATCH_TARGET_SPEED);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TOGGLE_AUTO_MATCH_TARGET_SPEED);
        break;
    ...
    case GP_HELP_ADVANCED_TARGET_KEYS:
        ...
        gameplay_help_blit_control_line (
            x_offset, y_offset, TARGET_CLOSEST_SHIP_ATTACKING_SELF);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TARGET_CLOSEST_SHIP_ATTACKING_TARGET);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TARGET_LAST_TRANMISSION_SENDER);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TARGET_TARGETS_TARGET);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TARGET_NEXT_ESCORT_SHIP);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TARGET_CLOSEST_REPAIR_SHIP);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TARGET_NEXT_UNINSPECTED_CARGO);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TARGET_PREV_UNINSPECTED_CARGO);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TARGET_NEWEST_SHIP);
        ...
        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, STOP_TARGETING_SHIP);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TARGET_NEXT_LIVE_TURRET);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, TARGET_PREV_LIVE_TURRET);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, STOP_TARGETING_SUBSYSTEM);
        ...
        y_offset += separation;
        gameplay_help_blit_control_line_raw (
            x_offset, y_offset, XSTR ("Shift-F5...F12", 151),
            XSTR ("Add/remove target from that hotkey", 152));

        y_offset += separation;
        gameplay_help_blit_control_line_raw (
            x_offset, y_offset, XSTR ("Alt-Shift-F5...F12", 153),
    ...
    case GP_HELP_WEAPON_KEYS:
        ...
        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, CYCLE_NEXT_PRIMARY);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, CYCLE_PREV_PRIMARY);
        ...
        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, CYCLE_NUM_MISSLES);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, LAUNCH_COUNTERMEASURE);
        break;
    ...
    case GP_HELP_THROTTLE_AND_ETS_KEYS:
        ...
        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, SHIELD_XFER_BOTTOM);
        ...
        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, SHIELD_XFER_RIGHT);
        ...
        /*
                    y_offset += separation;
                    gameplay_help_blit_control_line(x_offset,
           y_offset,XFER_LASER);

                    y_offset += separation;
                    gameplay_help_blit_control_line(x_offset,
           y_offset,XFER_SHIELD);
        */
        break;
    ...
    case GP_HELP_MISC_KEYS:
        ...
        y_offset += separation;
        gameplay_help_blit_control_line_raw (
            x_offset, y_offset, XSTR ("ESC", 160),
            XSTR ("invoke abort mission popup", 161));
        ...
        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, RADAR_RANGE_CYCLE);

        // escort view keys
        //
        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, ADD_REMOVE_ESCORT);
        ...
        y_offset += separation;
        gameplay_help_blit_control_line_raw (
            x_offset, y_offset, XSTR ("Pause", 162), XSTR ("pause game", 163));

        y_offset += separation;
        gameplay_help_blit_control_line_raw (
            x_offset, y_offset, XSTR ("Print Scrn", 164),
            XSTR ("take screen shot", 165));
    ...
    case GP_HELP_VIEW_KEYS:
        ...
        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, VIEW_EXTERNAL_TOGGLE_CAMERA_LOCK);
        ...
        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, VIEW_DIST_INCREASE);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, VIEW_DIST_DECREASE);
    ...
    case GP_HELP_MULTI_KEYS: {
        ...
        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, MULTI_MESSAGE_ALL);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, MULTI_MESSAGE_FRIENDLY);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, MULTI_MESSAGE_HOSTILE);

        y_offset += separation;
        gameplay_help_blit_control_line (
            x_offset, y_offset, MULTI_MESSAGE_TARGET);
