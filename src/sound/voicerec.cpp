/*
 * Code created by Thomas Whittaker (Random Tiger) for a FreeSpace 2 source
 * code project
 *
...
// Visual Studio complains that some functions are deprecated so this fixes
// that
#include <cstring>
#include <cwchar>
#include <cstdio>
...
CComPtr< ISpRecognizer >
    p_recogEngine;           // Pointer to our recognition engine instance
...
void doVid_Action (int action) {
    ...
    if (Player->flags & PLAYER_FLAGS_MSG_MODE) {
        switch (action) {
        case VID_DestroyTarget:
            Msg_shortcut_command = ATTACK_TARGET_ITEM;
            break;
        case VID_DisableTarget:
            Msg_shortcut_command = DISABLE_TARGET_ITEM;
            break;
        case VID_DisarmTarget:
            Msg_shortcut_command = DISARM_TARGET_ITEM;
            break;
        case VID_DestroySubsys:
            Msg_shortcut_command = DISABLE_SUBSYSTEM_ITEM;
            break;
        case VID_ProtectTarget:
            Msg_shortcut_command = PROTECT_TARGET_ITEM;
            break;
        case VID_IgnoreTarget:
            Msg_shortcut_command = IGNORE_TARGET_ITEM;
            break;
        ...
        if (Msg_instance == MESSAGE_ALL_FIGHTERS ||
            ...
            if (hud_squadmsg_ship_valid (&Ships[Msg_instance]))
                hud_squadmsg_send_ship_command (
                    Msg_instance, Msg_shortcut_command, 1);
        }
...
bool VOICEREC_init (
    HWND hWnd, int event_id, int grammar_id, int command_resource) {
    HRESULT hr = S_OK;
    ...
    while (true) {
        ...
        // Let SR know that window we want it to send event information to, and
        // using what message
        hr = p_recogContext->SetNotifyWindowMessage (hWnd, event_id, 0, 0);
        if (FAILED (hr)) {
            os::dialogs::Message (
                os::dialogs::MESSAGEBOX_ERROR,
                "Failed to SetNotifyWindowMessage\n", "Error");
            break;
        }

        // Tell SR what types of events interest us.  Here we only care about
        // command recognition.
        hr = p_recogContext->SetInterest (
            SPFEI (SPEI_RECOGNITION), SPFEI (SPEI_RECOGNITION));
        if (FAILED (hr)) {
            os::dialogs::Message (
                os::dialogs::MESSAGEBOX_ERROR, "Failed to set events\n",
                "Error");
            break;
        }
        ...
        if (FAILED (hr)) {
            os::dialogs::Message (
                os::dialogs::MESSAGEBOX_ERROR, "Failed to create grammar\n",
                "Error");
            break;
        }

        // Load our grammar from data\phrases.xml, or if that doesn't exist,
        // from the compiled in user defined ("SRGRAMMAR") resource type.
        hr = p_grammarObject->LoadCmdFromFile (
            L"data\\phrases.xml", SPLO_STATIC);
        if (FAILED (hr)) {
            hr = p_grammarObject->LoadCmdFromResource (
                NULL, MAKEINTRESOURCEW (command_resource), L"SRGRAMMAR",
                MAKELANGID (LANG_NEUTRAL, SUBLANG_NEUTRAL), SPLO_STATIC);
            if (FAILED (hr)) {
                os::dialogs::Message (
                    os::dialogs::MESSAGEBOX_ERROR,
                    "Failed to load resource SRGRAMMAR\n", "Error");
                break;
            }
        }
        ...
        if (FAILED (hr)) {
            os::dialogs::Message (
                os::dialogs::MESSAGEBOX_ERROR, "Failed to set audio input\n",
                "Error");
        }
    ...
    if (FAILED (hr)) { VOICEREC_deinit (); }

    os::events::addEventListener (
        SDL_SYSWMEVENT, os::events::DEFAULT_LISTENER_WEIGHT,
...
void VOICEREC_deinit () {
    if (cpAudio) { cpAudio.Release (); }
    // Release grammar, if loaded
    if (p_grammarObject) { p_grammarObject.Release (); }
    ...
    if (p_recogEngine) { p_recogEngine.Release (); }
}
 ...
 *----------------*
 *   Description:
 *       Called to Execute commands that have been identified by the speech
 *engine.
 *
 ******************************************************************************/
    ...
    if (SUCCEEDED (pPhrase->GetPhrase (&pElements))) {
        ...
        if (DEBUG_ON) {
            ...
            pPhrase->GetText (
                ...
                static_cast< ULONG > (SP_GETWHOLEPHRASE), TRUE, &pwszText,
                NULL);

            memset (szText, 0, 255);
            for (i = 0; i < 254; i++) {
                if (*(pwszText + i) == 0) { break; }
        ...
        case VID_WingName: {
            ...
            if (pElements->pProperties->pNextSibling) {
                ...
                if (pElements->pProperties->pNextSibling->pNextSibling) {
                    part3 = pElements->pProperties->pNextSibling->pNextSibling
                                ->vValue.ulVal;
                }
            }
            if (part2 == -1) break; // no ship number or wing
            ...
            for (size_t i = 0; i < wcslen (valstr); i++) {
                ...
                if (c != EOF) { wing_name[j++] = static_cast< char > (c); }
            }
            wing_name[j] = '\0';
        ...
        case VID_Action: {
            int action;
            if (part3 == -1) { action = pElements->pProperties->vValue.ulVal; }
            else {
                action = part3;
            }
        ...
        case VID_TopMenu: {
            ...
            if (Player->flags & PLAYER_FLAGS_MSG_MODE) { msgWindow = true; }

            // If the command window is not up, or it is and its a cancel
            // request toggle
            if ((msgWindow && action == VID_Cancel) ||
                (!msgWindow && action != VID_Cancel)) {
                hud_squadmsg_toggle ();
            }

            switch (action) {
            case VID_Ships: hud_squadmsg_do_mode (SM_MODE_SHIP_SELECT); break;

            case VID_Wings: hud_squadmsg_do_mode (SM_MODE_WING_SELECT); break;
            ...
            case VID_AllWings:
                ...
                if (pElements->pProperties->pFirstChild) {
                    doVid_Action (
                        pElements->pProperties->pFirstChild->vValue.ulVal);
                }
                break;
        ...
        // basic cheat, phrase as a value equivalent to the defines in
        // ControlConfig/ControlsConfig.h it just calls the button_function
        // with this value
        case VID_speed:
        case VID_targeting:
        case VID_other: {
            ...
            if (action > -1) { button_function (action); }
            break;
        }
        ...
        case VID_power: {
            ...
            if (action >= INCREASE_WEAPON && action <= ETS_EQUALIZE) {
                ...
                for (int i = 1; i < 7; i++) { button_function (action - 132); }
            }
            break;
        }
        }
