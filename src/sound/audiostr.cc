...
ubyte* Compressed_buffer =
    NULL; // Used to load in compressed data during a cueing interval
ubyte* Compressed_service_buffer =
    NULL; // Used to read in compressed data during a service interval

// Globalize the list of audio extensions for use in several sound related
// files
...
public:
    ...
    bool
    Create (uint nPeriod, uint nRes, ptr_u dwUser, TIMERCALLBACK pfnCallback);
...
protected:
    ...
    bool m_bReadingDone;  // no more bytes to be read from disk, still have
                          // remaining buffer to play
    uint m_fade_timer_id; // timestamp so we know when to start fade
    uint m_finished_id;   // timestamp so we know when we've played #bytes
                          // required
    bool m_bPastLimit;    // flag to show we've played past the number of bytes
                          // requred
...
// constructor
void Timer::constructor (void) { m_nIDTimer = 0; }
...
uint Timer::TimeProc (uint interval, void* dwUser) {
    ...
    if (ptimer->m_nPeriod) { return interval; }
    else {
...
const ushort DefBufferServiceInterval =
    250; // default buffer service interval in msec

// Constructor
AudioStream::AudioStream (void)
    : m_total_uncompressed_bytes_read (0),
      m_max_uncompressed_bytes_to_read (0) {
    write_lock = SDL_CreateMutex ();
}

// Destructor
AudioStream::~AudioStream (void) { SDL_DestroyMutex (write_lock); }
...
bool AudioStream::Create (char* pszFilename) {
    ...
    if (pszFilename) {
        ...
        if (!strlen (pszFilename)) return false;
        ...
        if (m_pwavefile) {
            ...
            if (m_pwavefile->Open (pszFilename, (type == ASF_EVENTMUSIC))) {
                m_cbBufSize = (m_pwavefile->getSampleRate () *
                               m_pwavefile->getSampleByteSize ()) >>
                              2;
                // make sure that we are a multiple of the frame size
                m_cbBufSize -=
                    (m_cbBufSize % m_pwavefile->getSampleByteSize ());
                ...
                OpenAL_ErrorPrint (
                    alSourcef (m_source_id, AL_ROLLOFF_FACTOR, 1.0f));
                OpenAL_ErrorPrint (
                ...
                if ((type == ASF_SOUNDFX) && ds_eax_is_inited ()) {
                    ...
                    OpenAL_ErrorPrint (alSource3i (
                        m_source_id, AL_AUXILIARY_SEND_FILTER, AL_EFX_aux_id,
                        0, AL_FILTER_NULL));
                }
...
bool AudioStream::WriteWaveData (
    uint size, uint* num_bytes_written, int service) {
    bool fRtn = true;
    ubyte* uncompressed_wave_data;

    *num_bytes_written = 0;

    if (size == 0 || m_bReadingDone) { return fRtn; }

    if ((m_buffer_ids[0] == 0) || !m_pwavefile) { return fRtn; }

    if (service) { SDL_LockMutex (Global_service_lock); }

    if (service) { uncompressed_wave_data = Wavedata_service_buffer; }
    else {
    ...
    if (!service) {
        for (int ib = 0; ib < MAX_STREAM_BUFFERS; ib++) {
            num_bytes_read =
                m_pwavefile->Read (uncompressed_wave_data, m_cbBufSize);

            if (num_bytes_read < 0) { m_bReadingDone = 1; }
            else if (num_bytes_read > 0) {
                OpenAL_ErrorCheck (
                    alBufferData (
                        m_buffer_ids[ib], m_pwavefile->getALFormat (),
                        uncompressed_wave_data, num_bytes_read,
                        m_pwavefile->getSampleRate ()),
                    {
                        fRtn = false;
                        goto ErrorExit;
                    });
        ...
        OpenAL_ErrorPrint (alGetSourcei (
            m_source_id, AL_BUFFERS_PROCESSED, &buffers_processed));
        ...
        while (buffers_processed) {
            ...
            if (num_bytes_read < 0) { m_bReadingDone = 1; }
                ...
                OpenAL_ErrorPrint (alBufferData (
                    buffer_id, m_pwavefile->getALFormat (),
                    uncompressed_wave_data, num_bytes_read,
                    m_pwavefile->getSampleRate ()));
                OpenAL_ErrorPrint (
...
ErrorExit:
    ...
    if (service) { SDL_UnlockMutex (Global_service_lock); }
...
// GetMaxWriteSize
//
// Helper function to calculate max size of sound buffer write operation, i.e.
// how much free space there is in buffer.
uint AudioStream::GetMaxWriteSize (void) {
    uint dwMaxSize = m_cbBufSize;
    ALint n, q;

    OpenAL_ErrorCheck (
        alGetSourcei (m_source_id, AL_BUFFERS_PROCESSED, &n), return 0);

    OpenAL_ErrorCheck (
        alGetSourcei (m_source_id, AL_BUFFERS_QUEUED, &q), return 0);
...
bool AudioStream::ServiceBuffer (void) {
    ...
    if (status != ASF_USED) return false;
    ...
    if (dwFreeSpace) {
        ...
        if (WriteWaveData (dwFreeSpace, &num_bytes_written) == true) {
            ...
            if (m_total_uncompressed_bytes_read >=
                ...
                m_fade_timer_id = timer_get_milliseconds () +
                                  1700; // start fading 1.7 seconds from now
                m_finished_id = timer_get_milliseconds () +
                                2000; // 2 seconds left to play out buffer
                m_max_uncompressed_bytes_to_read =
                    std::numeric_limits< uint >::max ();
            }

            if ((m_fade_timer_id > 0) &&
                ((uint)timer_get_milliseconds () > m_fade_timer_id)) {
                m_fade_timer_id = 0;
                Fade_and_Stop ();
            }

            if ((m_finished_id > 0) &&
                ((uint)timer_get_milliseconds () > m_finished_id)) {
                m_finished_id = 0;
                m_bPastLimit = true;
            }
            ...
            if (PlaybackDone ()) {
                ...
                if (m_bLooping && !m_bFade) { Play (m_lVolume, m_bLooping); }
                else {
                    Stop_and_Rewind ();
                }
            }
...
void AudioStream::Play (float volume, int looping) {
    if (m_buffer_ids[0] != 0) {
        // If playing, stop
        if (m_fPlaying) {
            if (m_bIsPaused == false) Stop_and_Rewind ();
        }
        ...
        m_timer.Create (
            m_nBufService, m_nBufService, ptr_u (this), TimerCallback);
...
void AudioStream::Set_Sample_Cutoff (unsigned int sample_cutoff) {
    if (m_pwavefile == NULL) return;

    m_max_uncompressed_bytes_to_read =
        (sample_cutoff *
         (m_pwavefile->getSampleByteSize () / m_pwavefile->getNumChannels ()));
}

uint AudioStream::Get_Samples_Committed (void) {
    if (m_pwavefile == NULL) return 0;

    return (uint) (
        m_total_uncompressed_bytes_read /
        (m_pwavefile->getSampleByteSize () / m_pwavefile->getNumChannels ()));
}
...
void AudioStream::Fade_and_Destroy (void) {
    if (!m_fPlaying || PlaybackDone ()) { Destroy (); }
    else {
        m_bFade = true;
...
void AudioStream::Stop (int paused) {
    if (m_fPlaying) {
        if (paused) { OpenAL_ErrorPrint (alSourcePause (m_source_id)); }
        else {
...
// Set_Volume
float AudioStream::Get_Volume () { return m_lVolume; }
...
void audiostream_init () {
    ...
    if (Audiostream_inited == 1) return;
...
// Close down the audiostream system.  Must call audiostream_init() before any
// audiostream functions can be used.
void audiostream_close () {
    if (Audiostream_inited == 0) return;
...
int audiostream_open (const char* filename, int type) {
    ...
    if (!Audiostream_inited || !snd_is_inited ()) return -1;
...
void audiostream_close_all (bool fade) {
    ...
    for (i = 0; i < MAX_AUDIO_STREAMS; i++) {
        if (Audio_streams[i].status == ASF_FREE) continue;
...
void audiostream_set_volume_all (float volume, int type) {
    ...
    for (i = 0; i < MAX_AUDIO_STREAMS; i++) {
        ...
        if ((Audio_streams[i].type == type) ||
            ((Audio_streams[i].type == ASF_MENUMUSIC) &&
             (type == ASF_EVENTMUSIC))) {
...
int audiostream_is_inited () { return Audiostream_inited; }
...
void audiostream_pause (int i, bool via_sexp_or_script) {
    ...
    if (audiostream_is_playing (i) == (int)true) audiostream_stop (i, 0, 1);

    if (via_sexp_or_script) Audio_streams[i].paused_via_sexp_or_script = true;
}
...
void audiostream_unpause (int i, bool via_sexp_or_script) {
    ...
    if (audiostream_is_paused (i) == (int)true) {
        audiostream_play (
            i, Audio_streams[i].Get_Volume (), Audio_streams[i].Is_looping ());
    }

    if (via_sexp_or_script) Audio_streams[i].paused_via_sexp_or_script = false;
}
...
void audiostream_unpause_all (bool via_sexp_or_script) {
    ...
    for (i = 0; i < MAX_AUDIO_STREAMS; i++) {
        ...
        // if we explicitly paused this and we are not explicitly unpausing,
        // skip this stream
