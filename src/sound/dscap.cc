...
int dscap_create_buffer (
    ...
    if (nchannels == 1) {
        ...
        if (bits_per_sample == 8) { al_format = AL_FORMAT_STEREO8; }
        else if (bits_per_sample == 16) {
    ...
    ds_capture_device =
        alcCaptureOpenDevice (dev_name, freq, al_format, buf_size);

    if (ds_capture_device == NULL) { return -1; }

    if (alcGetError (ds_capture_device) != ALC_NO_ERROR) { return -1; }
...
int dscap_start_record () {
    if (!dscap_inited) { return -1; }

    if (dscap_recording) { return -1; }
...
int dscap_max_buffersize () {
    ...
    OpenAL_C_ErrorCheck (
        alcGetIntegerv (
            ds_capture_device, ALC_CAPTURE_SAMPLES, sizeof (ALCsizei),
            &num_samples),
        return -1);
...
int dscap_get_raw_data (unsigned char* outbuf, unsigned int max_size) {
    ...
    OpenAL_C_ErrorPrint (alcGetIntegerv (
        ds_capture_device, ALC_CAPTURE_SAMPLES, sizeof (ALCsizei),
        &num_samples));

    if (num_samples <= 0) { return 0; }

    ALCsizei max_buf_size =
        MIN (num_samples, ALsizei (max_size / ALCaptureInfo.block_align));

    OpenAL_C_ErrorCheck (
        alcCaptureSamples (ds_capture_device, outbuf, max_buf_size), return 0);
