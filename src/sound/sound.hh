...
// Priorities that can be passed to snd_play() functions to limit how many
// concurrent sounds of a given type are played.
...
struct EnhancedSoundData {
    ...
    unsigned int limit =
        0; // limit on how many instances of the sound can play concurrently
...
struct game_snd_entry {
    ...
    sound_load_id id =
        sound_load_id::invalid (); //!< index into Sounds[], where sound data
                                   //!< is stored
...
enum class GameSoundCycleType { Random, Sequential };
...
struct game_snd {
    ...
    std::vector< game_snd_entry >
        sound_entries; //!< A game sound consists of one or more distinct
                       //!< entries
    ...
    util::UniformFloatRange
        pitch_range; //!< The range of possible pitch values used randomly for
                     //!< this sound
    util::UniformFloatRange
        volume_range; //!< The possible range of the default volume (range is
                      //!< (0, 1]).

    bool preload =
        false; //!< preload sound (ie read from disk before mission starts)
...
// Pan goes from -1.0 all the way left to 0.0 in center to 1.0 all the way
// right.
sound_handle snd_play (
    game_snd* gs, float pan = 0.0f, float vol_scale = 1.0f,
...
sound_handle snd_play_raw (
    sound_load_id soundnum, float pan, float vol_scale = 1.0f,
...
sound_handle snd_play_3d (
    game_snd* gs, vec3d* source_pos, vec3d* listen_pos, float radius = 0.0f,
    vec3d* vel = nullptr, int looping = 0, float vol_scale = 1.0f,
    int priority = SND_PRIORITY_SINGLE_INSTANCE, vec3d* sound_fvec = nullptr,
    float range_factor = 1.0f, int force = 0, bool is_ambient = false);

// update the given 3d sound with a new position
void snd_update_3d_pos (
    sound_handle soudnnum, game_snd* gs, vec3d* new_pos, float radius = 0.0f,
...
sound_handle snd_play_looping (
    game_snd* gs, float pan = 0.0f, int start_loop = -1, int stop_loop = -1,
...
int snd_get_3d_vol_and_pan (
    game_snd* gs, vec3d* pos, float* vol, float* pan, float radius = 0.0f,
    float range_factor = 1.0f);
...
void snd_set_pos (
    ...
    int as_pct); // set the position val as either a percentage (if as_pct) or
                 // as a # of seconds into the sound
...
/**
 * @brief Get the sound id that was used to start the sound with the specified
 * handle
...
#endif // FREESPACE2_SOUND_SOUND_HH
