...
float Default_sound_volume =
    1.0f; // range is 0 -> 1, used for non-music sound fx
float Default_voice_volume =
    0.7f; // range is 0 -> 1, used for all voice playback
...
struct LoopingSoundInfo {
    ...
    float
        m_defaultVolume; //!< The default volume of this sound (from game_snd)
    float m_dynamicVolume; //!< The dynamic volume before scripted volume
                           //!< adjustment is applied (is updated via
                           //!< snd_set_volume)

    LoopingSoundInfo (
        sound_handle dsHandle, float defaultVolume, float dynamicVolume)
        : m_dsHandle (dsHandle), m_defaultVolume (defaultVolume),
          m_dynamicVolume (dynamicVolume) {}

    LoopingSoundInfo ()
        : m_dsHandle (-1), m_defaultVolume (0.0f), m_dynamicVolume (0.0f) {}
};
...
// convert the game level sound priorities to the DirectSound priority
// descriptions
int ds_priority (int priority) {
    switch (priority) {
    case SND_PRIORITY_MUST_PLAY: return DS_MUST_PLAY;
...
//
// Initialize the game sound system.  Depending on what sound library is being
// used, call the appropriate low-level initiailizations
//
...
void snd_spew_info () {
    ...
    for (idx = 0; idx < Sounds.size (); idx++) {
        if (!(Sounds[idx].flags & SND_F_USED)) { continue; }

        sprintf (
            txt, "%s (%ds)\n", Sounds[idx].filename,
            Sounds[idx].info.duration);
...
// snd_load()
//
// Load a sound into memory and prepare it for playback.  The sound will reside
// in memory as a single instance, and can be played multiple times
// simultaneously.  Through the magic of DirectSound, only 1 copy of the sound
// is used.
//
...
sound_load_id
snd_load (game_snd_entry* entry, int flags, int /*allow_hardware_load*/) {
    int type;
    sound_info* si;
    sound* snd;
    size_t n;

    if (!ds_initialized) return sound_load_id::invalid ();

    if (!VALID_FNAME (entry->filename)) return sound_load_id::invalid ();
    ...
    for (n = 0; n < Sounds.size (); n++) {
        if (!(Sounds[n].flags & SND_F_USED)) { break; }
            ...
            //
            // NOTE: this will allow a duplicate 3D entry if 2D stereo entry
            // exists,
            // but will not load a duplicate 2D entry to get stereo if 3D
            // version already loaded
            if ((Sounds[n].info.n_channels == 1) ||
                !(flags & GAME_SND_USE_DS3D)) {
        ...
        if (audio_file->getNumChannels () > 1) {
...
#ifndef NDEBUG
            // Retail has a few sounds that triggers this warning so we need to
            // ignore those
            ...
            if (show_warning) {
                if (mod_supports_version (3, 8, 0)) {
                    // This warning was introduced in 3.8.0 and caused a few
    ...
    // Load was a success
    si->n_channels =
        audio_file->getNumChannels (); // 16-bit channel count (nChannels)
    si->sample_rate =
        audio_file->getSampleRate (); // 32-bit sample rate (nSamplesPerSec)
    si->avg_bytes_per_sec =
        audio_file->getSampleRate () *
        audio_file->getSampleByteSize (); // 32-bit average bytes per second
                                          // (nAvgBytesPerSec)
    si->bits = audio_file->getSampleByteSize () /
               audio_file->getNumChannels () *
               8; // Read 16-bit bits per sample
    si->size =
        audio_file->getTotalSamples () * audio_file->getSampleByteSize ();
...
// snd_unload()
//
// Unload a sound from memory.  This will release the storage, and the sound
// must be re-loaded via sound_load() before it can be played again.
//
int snd_unload (sound_load_id n) {
    ...
    if ((size_t)n.value () >= Sounds.size ()) { return 0; }

    auto& snd = Sounds[n.value ()];

    ds_unload_buffer (snd.sid);

    if (snd.sid != -1) { Snd_sram -= snd.uncompressed_size; }

    // If this sound is at the end of the array, we might as well get rid of it
    if ((size_t)n.value () == Sounds.size () - 1) { Sounds.pop_back (); }
    else {
        snd.sid = -1;
        snd.flags &= ~SND_F_USED;
    }
...
// snd_unload_all()
//
// Unload all sounds from memory. If there's a problem unloading a file the
// array may not be fully cleared but future files will still use unused spots,
// so the array size shouldn't grow out of control.
//
void snd_unload_all () {
    while (!Sounds.empty ()) {
...
// snd_close()
//
// This is the companion function to snd_init()... it closes down the game
// sound system.
//
void snd_close (void) {
    snd_stop_all ();
    if (!ds_initialized) return;
    snd_unload_all (); // free the sound data stored in DirectSound secondary
                       // buffers
...
// snd_play_raw()
//
// Allow a sound to be played directly from the index in Sounds[].  This
// bypasses the normal game sound management system.
//
// returns:             -1              =>              sound could not be played
// n               =>              handle for instance of sound
//
sound_handle snd_play_raw (
    sound_load_id soundnum, float pan, float vol_scale, int priority) {
    game_snd gs;

    if (!soundnum.isValid ()) { return sound_handle::invalid (); }
...
// n               =>              handle for instance of sound
//
sound_handle snd_play (
    game_snd* gs, float pan, float vol_scale, int priority,
    bool is_voice_msg) {
    float volume;
    sound* snd;
    ...
    if (!entry->id.isValid ()) return sound_handle::invalid ();
    ...
    if (!(snd->flags & SND_F_USED)) return sound_handle::invalid ();
    ...
    if (volume > MIN_SOUND_VOLUME) {
        handle = ds_play (
            snd->sid, entry->id_sig, ds_priority (priority),
            &gs->enhanced_sound_data, volume, pan, 0, is_voice_msg);
...
// n               =>              handle for instance of sound
//
sound_handle snd_play_3d (
    game_snd* gs, vec3d* source_pos, vec3d* listen_pos, float radius,
    vec3d* source_vel, int looping, float vol_scale, int priority,
    vec3d* /*sound_fvec*/, float range_factor, int force,
    bool /*is_ambient*/) {
    ...
    if (snd->sid < 0) { return sound_handle::invalid (); }
    ...
    // DirectSound3D will not cut off sounds, no matter how quite they become..
    // so manually prevent sounds from playing past the max distance.
    // IMPORTANT THIS IS NOT WORKING RIGHT OMG WTF
    distance =
        vm_vec_normalized_dir_quick (&vector_to_sound, source_pos, listen_pos);

    if ((distance > max_range) && !force) { return sound_handle::invalid (); }

    float default_volume = gs->volume_range.next ();
    max_volume = default_volume * vol_scale;

    if (distance <= min_range) { volume = max_volume; }
    else {
    ...
    if (priority == SND_PRIORITY_MUST_PLAY) {
        if (volume < 0.3) { priority = SND_PRIORITY_DOUBLE_INSTANCE; }
    }
    ...
    ASSERTX (
        snd->info.n_channels == 1, "Sound should be mono! Sound file: %s",
        snd->filename);

    sound_handle handle;
    if (Cmdline_no_3d_sound) {
        if (distance <= 0.0f) { pan = 0.0f; }
        else {
        ...
        handle = ds_play (
            snd->sid, entry->id_sig, ds_priority (priority),
            &gs->enhanced_sound_data, volume / default_volume, pan, looping);
    }
    else {
        handle = ds3d_play (
            snd->sid, entry->id_sig, source_pos, source_vel, min_range,
            max_range, looping,
            (max_volume * Master_sound_volume * aav_effect_volume), volume,
            &gs->enhanced_sound_data, ds_priority (priority));
    }

    if (handle.isValid ()) { snd_set_pitch (handle, gs->pitch_range.next ()); }

    return handle;
}

// update the given 3d sound with a new position
void snd_update_3d_pos (
    sound_handle soundnum, game_snd* gs, vec3d* new_pos, float radius,
    float range_factor) {
    if (Cmdline_no_3d_sound) {
        float vol, pan;
        ...
        // MageKing17 - It's a 3D sound effect, we should use the function for
        // setting the position of a 3D sound effect.
        int channel;

        if (!ds_initialized) return;

        ASSERTX (
            gs != NULL, "*gs was NULL in snd_update_3d_pos(); get a coder!\n");
...
int snd_get_3d_vol_and_pan (
    ...
    *pan = 0.0f;
    ...
    distance =
        vm_vec_normalized_dir_quick (&vector_to_sound, pos, &View_position);
    distance -= radius;

    max_volume = gs->volume_range.max ();
    if (distance <= min_range) { *vol = max_volume; }
    else {
        *vol = max_volume -
               (distance - min_range) * max_volume / (max_range - min_range);
    }
 ...
 * @param vol_scale factor to scale the static volume by (applied before
 * attenuation)
 * @param scriptingUpdateVolume if true the looping sound value is updated
 * default is TRUE
 * @return -1 on error, else the handle for this playing sound
 */
sound_handle snd_play_looping (
    game_snd* gs, float pan, int /*start_loop*/, int /*stop_loop*/,
    float vol_scale, int scriptingUpdateVolume) {
    float volume;
    sound* snd;
    ...
    if (!entry->id.isValid ()) { entry->id = snd_load (entry, gs->flags); }
    else if (entry->id_sig != Sounds[entry->id.value ()].sig) {
        entry->id = snd_load (entry, gs->flags);
    }
    ...
    if (volume > MIN_SOUND_VOLUME) {
        handle = ds_play (
            snd->sid, entry->id_sig, DS_MUST_PLAY, &gs->enhanced_sound_data,
            volume, pan, 1);

        if (handle.isValid ()) {
            if (scriptingUpdateVolume) {
                currentlyLoopingSoundInfos.push_back (
                    LoopingSoundInfo (handle, default_volume, vol_scale));
            }
...
void snd_stop (sound_handle sig) {
    ...
    std::list< LoopingSoundInfo >::iterator iter =
        currentlyLoopingSoundInfos.begin ();
...
void snd_set_volume (sound_handle sig, float volume) {
    ...
    std::list< LoopingSoundInfo >::iterator iter;
    for (iter = currentlyLoopingSoundInfos.begin ();
         iter != currentlyLoopingSoundInfos.end (); ++iter) {
        if (iter->m_dsHandle == sig) {
...
//
// parameters:          sig             => handle to sound, what is returned from
// snd_play()
...
int snd_get_duration (sound_load_id snd_id) {
    if (!snd_id.isValid ()) return 0;

    ASSERTX (
        !Sounds.empty (),
        "Sounds vector is empty. Why are we trying to look up an index?\n");

    if (Sounds.empty ()) return 0;

    ASSERTX (
        Sounds[snd_id.value ()].duration > 0,
...
const char* snd_get_filename (sound_load_id snd_id) {
    ASSERTX (snd_id.isValid (), "Invalid sound id %d!", snd_id.value ());
...
void snd_rewind (sound_handle snd_handle, float seconds) {
    ...
    if (!snd_is_playing (snd_handle)) return;
    ...
    current_offset =
        ds_get_play_position (channel); // current offset into the sound
    bps = (float)snd->sample_rate * (float)snd->bits; // data rate
    current_time =
        (float)current_offset / bps; // how many seconds we're into the sound

    // don't rewind if it'll put us before the beginning of the sound
    if (current_time - seconds < 0.0f) return;
...
void snd_ffwd (sound_handle snd_handle, float seconds) {
    ...
    current_offset = ds_get_play_position (
        ds_get_channel (snd_handle)); // current offset into the sound
    bps = (float)snd->sample_rate * (float)snd->bits; // data rate
    current_time =
        (float)current_offset / bps; // how many seconds we're into the sound

    // don't rewind if it'll put us past the end of the sound
    if (current_time + seconds > (float)snd->duration) return;
...
void snd_set_pos (sound_handle snd_handle, float val, int as_pct) {
    ...
    if (as_pct) {
        ...
        ds_set_position (
            ds_get_channel (snd_handle), (uint32_t) ((float)snd->size * val));
    }
    // set the position as an absolute # of seconds from the beginning of the
    // sound
    else {
        float bps;
...
int snd_num_playing () { return ds_get_number_channels (); }
...
int snd_get_data (sound_load_id handle, char* data) {
    ...
    if (ds_get_data (Sounds[handle.value ()].sid, data)) { return -1; }
...
int snd_size (sound_load_id handle, int* size) {
    ...
    if (ds_get_size (Sounds[handle.value ()].sid, size)) { return -1; }
...
void snd_get_format (
    sound_load_id handle, int* bits_per_sample, int* frequency) {
    ASSERT ((handle.isValid ()) && ((size_t)handle.value () < Sounds.size ()));

    if (bits_per_sample) *bits_per_sample = Sounds[handle.value ()].info.bits;

    if (frequency) *frequency = Sounds[handle.value ()].info.sample_rate;
}
...
int snd_time_remaining (sound_handle handle) {
    ...
    if ((bits_per_sample <= 0) || (frequency <= 0)) return 0;

    // handle ADPCM properly.  It's always 16bit for OpenAL but should be 8 or
    // 16 for Windows.  We can't leave it as 4 here (the ADPCM rate) because
    // that is the compressed bps and the math is against the uncompressed bps.
    ...
    if (current_offset < max_offset) {
        ...
        time_remaining =
            (int)std::lround (1000.0f * samples_remaining / frequency);
    }
...
sound_load_id snd_get_sound_id (sound_handle snd_handle) {
    ...
    if (channel < 0) { return sound_load_id::invalid (); }
...
//
int sound_env_supported () { return ds_eax_is_inited (); }
...
void snd_do_frame () {
    ...
    for (iter = currentlyLoopingSoundInfos.begin ();
         iter != currentlyLoopingSoundInfos.end (); ++iter) {
        float new_volume = iter->m_defaultVolume * iter->m_dynamicVolume *
                           (Master_sound_volume * aav_effect_volume);
...
void adjust_volume_on_frame (float* volume_now, aav* data) {
    ...
    if (*volume_now == (data->start_volume + data->delta)) return;
    ...
    *volume_now = data->start_volume + (data->delta * done);
    ...
    // done here in order to avoid setting music volume in every frame
    // regardless if it changed or not
    if (&aav_music_volume == volume_now) {
        audiostream_set_volume_all (
            Master_event_music_volume * aav_music_volume, ASF_EVENTMUSIC);
    }
}
...
game_snd::game_snd ()
    : last_entry_index (std::numeric_limits< size_t >::max ()) {}

EnhancedSoundData::EnhancedSoundData () {}
EnhancedSoundData::EnhancedSoundData (
    const int new_priority, const unsigned int new_limit)
    : priority (new_priority), limit (new_limit) {
    ASSERTX (
        priority >= SND_ENHANCED_PRIORITY_MUST_PLAY &&
            priority <= SND_ENHANCED_PRIORITY_LOW,
        "EnhancedSoundData ctor given invalid priority %d", priority);
    ASSERTX (
        limit > 0, "EnhancedSoundData ctor given invalid limit %d", limit);
}
game_snd_entry::game_snd_entry () { filename[0] = 0; }
