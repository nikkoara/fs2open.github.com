...
snd_load(game_snd_entry *entry, int flags, int /*allow_hardware_load*/)
{
        ...
        si->avg_bytes_per_sec = audio_file->getSampleRate() * audio_file->getSampleByteSize(); // 32-bit average bytes per second
                                                                                               // (nAvgBytesPerSec)
...
{
        ASSERTX(
                priority >= SND_ENHANCED_PRIORITY_MUST_PLAY && priority <= SND_ENHANCED_PRIORITY_LOW,
