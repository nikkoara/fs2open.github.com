 ...
 *
 * Contrary to its name it doesn't only support Wave files but can also play
 * Ogg files.
 *
...
public:
     ...
     * @param keep_ext @c true if the extension of the specified file name
     * should be kept
     ...
     * data. Future calls to Read start reading from the beginning of the file
     * again
     *
     ...
     * multiple of the size of one sample
     *
     * @param pbDest The buffer to write data to
     * @param cbSize The size of the buffer
     * @return The number of bytes written into the buffer. Can be 0 if no data
     ...
     * used to reduce the number of channels the audio has if it's not
     * supported.
     *
...
#endif // FREESPACE2_SOUND_FFMPEG_WAVEFILE_HH
