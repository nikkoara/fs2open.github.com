...
AudioProperties getAdjustedAudioProps (const AudioProperties& baseProps) {
    ...
    case DS_SQ_HIGH: max_bytes_per_sample = Ds_float_supported ? 4 : 2; break;

    case DS_SQ_MEDIUM: max_bytes_per_sample = 2; break;

    default: max_bytes_per_sample = 1; break;
    }

    // Determine the right format. Downsample if sound quality is low but don't
    // increase size of samples if the source doesn't support it
    auto bytes_per_sample = std::min (
        av_get_bytes_per_sample (baseProps.format), max_bytes_per_sample);

    switch (bytes_per_sample) {
    case 1: adjusted.format = AV_SAMPLE_FMT_U8; break;
...
SwrContext*
getSWRContext (const AudioProperties& base, const AudioProperties& adjusted) {
    SwrContext* swr = nullptr;
    swr = swr_alloc_set_opts (
        swr, adjusted.channel_layout, adjusted.format, adjusted.sample_rate,
        base.channel_layout, base.format, base.sample_rate, 0, nullptr);

    if (swr_init (swr) < 0) { return nullptr; }
...
WaveFile::~WaveFile () {
    ...
    if (m_resampleCtx) { swr_free (&m_resampleCtx); }
...
bool WaveFile::Open (const char* pszFilename, bool keep_ext) {
    ...
    try {
        ...
        if (keep_ext) {
            ...
            // not a supported extension format ... somebody screwed up their
            // tbls :)
            if (!known_format) {
            ...
            if (!res.found) { throw FFmpegException ("File not found."); }

            cfp = cfopen_special (
                res.full_name.c_str (), "rb", res.size, res.offset,
                res.data_ptr, CF_TYPE_ANY);
        }
        else {
            // ... otherwise we just find the best match
            auto res = cf_find_file_location_ext (
                filename, NUM_AUDIO_EXT, audio_ext_list, CF_TYPE_ANY, false);

            if (!res.found) {
                throw FFmpegException (
                    "File not found with any known extension.");
            }

            // cf_find_file_location_ext already handles file names with
            // extensions so we also need to handle that here
        ...
        if (cfp == NULL) { throw FFmpegException ("Failed to open file."); }
        ...
        err = avcodec_parameters_to_context (
            m_audioCodecCtx, m_audioStream->codecpar);
        if (err < 0) {
            char errorStr[512];
        ...
        WARNINGF (LOCATION, "SOUND => %s => Using codec %s (%s)", filename,audio_codec->long_name, audio_codec->name);
    }
    catch (const FFmpegException& e) {
        WARNINGF (LOCATION,"SOUND ==> Could not open wave file %s for streaming. Reason: %s",filename, e.what ());
        return false;
    }
...
bool WaveFile::Cue () {
    auto err =
        av_seek_frame (m_ctx->ctx (), m_audioStreamIndex, 0, AVSEEK_FLAG_BYTE);
...
size_t WaveFile::handleDecodedFrame (
    AVFrame* av_frame, uint8_t* out_buffer, size_t buffer_size) {
    const auto sample_size =
        (av_get_bytes_per_sample (m_audioProps.format) * getNumChannels ());

    int dest_num_samples = static_cast< int > (buffer_size / sample_size);
    auto written = swr_convert (
        m_resampleCtx, &out_buffer, dest_num_samples,
...
size_t WaveFile::getBufferedData (uint8_t* buffer, size_t buffer_size) {
    ...
    if (buffered > 0) {
        const auto sample_size =
            (av_get_bytes_per_sample (m_audioProps.format) *
             getNumChannels ());
        int dest_num_samples = static_cast< int > (buffer_size / sample_size);
        auto written =
...
int WaveFile::Read (uint8_t* pbDest, size_t cbSize) {
    ...
    if (buffer_pos == cbSize) { return static_cast< int > (buffer_pos); }

    while (m_frameReader->readFrame (m_decodeFrame)) {
        // Got a new frame
        auto advance = handleDecodedFrame (
    ...
    // If we are here then the audio stream is finished, write any buffered
    // data
...
int WaveFile::getSampleRate () const { return m_audioProps.sample_rate; }

double WaveFile::getDuration () const {
    return av_q2d (av_mul_q (
        av_make_q (static_cast< int > (m_audioStream->duration), 1),
        m_audioStream->time_base));
}

int WaveFile::getTotalSamples () const {
    auto duration = av_mul_q (
        av_make_q (static_cast< int > (m_audioStream->duration), 1),
        m_audioStream->time_base);
    auto samples_r =
        av_mul_q (av_make_q (m_audioProps.sample_rate, 1), duration);
...
ALenum WaveFile::getALFormat () const {
    return openal_get_format (
        av_get_bytes_per_sample (m_audioProps.format) * 8, getNumChannels ());
    ;
}
} // namespace ffmpeg
