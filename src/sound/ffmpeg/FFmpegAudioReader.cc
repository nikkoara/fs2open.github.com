...
FFmpegAudioReader::FFmpegAudioReader (
    ...
    int stream_idx)
    : _stream_idx (stream_idx), _format_ctx (av_format_context),
      _codec_ctx (codec_ctx) {}

bool FFmpegAudioReader::readFrame (AVFrame* decode_frame) {
    // FFmpeg 3.1 and onwards has a new, better packet handling API but that
    // version is relatively new so we still need to support the earlier API.
    // The compatibility code can be removed once the new version is supported
    // by most platforms
    ...
    if (_currentPacket != nullptr) {
        // Got some data left
        int finishedFrame = 0;
        auto bytes_read = avcodec_decode_audio4 (
            _codec_ctx, decode_frame, &finishedFrame, _currentPacket);
    ...
    while (av_read_frame (_format_ctx, &packet) >= 0) {
        ...
        if (packet.stream_index == _stream_idx) {
            ...
            int finishedFrame = 0;
            auto bytes_read = avcodec_decode_audio4 (
                _codec_ctx, decode_frame, &finishedFrame, &packet);
...
#if LIBAVCODEC_VERSION_INT > AV_VERSION_INT(57, 24, 255)
    // Flush decoder
    if (avcodec_send_packet (_codec_ctx, nullptr) != 0) { return false; }
    ...
    auto err = avcodec_decode_audio4 (
        _codec_ctx, decode_frame, &finishedFrame, &nullPacket);

    if (finishedFrame && err >= 0) { return true; }
#endif
