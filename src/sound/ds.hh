...
// limits placed on how many concurrent sounds of the same type can play
// simultaneously
...
ds_sound_handle ds_play (
    ...
    int looping, bool is_voice_msg = false);
...
ds_sound_handle ds3d_play (
    ...
    const EnhancedSoundData* enhanced_sound_data, int priority = DS_MUST_PLAY,
...
int ds_eax_get_prop (
    EFXREVERBPROPERTIES** props, const char* name,
...
int ds_eax_set_all (
    unsigned long id, float volume, float damping, float decay);
int ds_eax_get_all (EAX_REVERBPROPERTIES* er, int id = -1);
int ds_eax_is_inited ();

#endif // FREESPACE2_SOUND_DS_HH
