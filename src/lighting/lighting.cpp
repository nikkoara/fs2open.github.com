...
#define MIN_LIGHT \
    0.03f // When light drops below this level, ignore it.  Must be non-zero!
          // (1/32)
...
DCF (light, "Changes lighting parameters") {
    ...
    if (dc_optional_string_either ("help", "--help")) {
        ...
        dc_printf (
            "light ambient X       Where X is the ambient light between 0 and "
            "1.0\n");
        dc_printf (
            "light reflect X       Where X is the material reflectiveness "
            "between 0 and 1.0\n");
...
void light_add_directional (
    const vec3d* dir, float intensity, float r, float g, float b, float spec_r,
    float spec_g, float spec_b, bool specular) {
...
void light_rotate_all () {
    ...
    for (auto& l : Lights) light_rotate (&l);
}

/**
 * Return the # of global light sources
 */
int light_get_global_count () { return (int)Static_light.size (); }
...
int light_get_global_dir (vec3d* pos, int n) {
    if ((n < 0) || (n >= (int)Static_light.size ())) { return 0; }
...
void light_apply_rgb (
    ubyte* param_r, ubyte* param_g, ubyte* param_b, const vec3d* pos,
    const vec3d* norm, float static_light_level) {
    int idx;
    ...
    if (!Light_in_shadow) {
        ...
        for (idx = 0; idx < (int)Static_light.size (); idx++) {
            ...
            ltmp = -vm_vec_dot (&Static_light[idx].local_vec, norm) *
                   Static_light[idx].intensity *
    ...
    // At this point, l must be between 0 and 0.75 (0.75-1.0 is for dynamic
    // light only)
    ...
    for (auto l : Lights) {
        ...
        // tube lights
        case Light_Type::Tube:
            if (vm_vec_dist_to_line (
                    pos, &l.local_vec, &l.local_vec2, &temp, &dist) != 0) {
                continue;
            }
        ...
        case Light_Type::Directional: continue;
        ...
        if (dot > 0.0f) {
            ...
            if (dist < 0.0f) { dist = vm_vec_mag_squared (&to_light); }
...
void light_add_cone (
    const vec3d* pos, const vec3d* dir, float angle, float inner_angle,
...
void scene_lights::setLightFilter (int objnum, const vec3d* pos, float rad) {
    ...
    for (auto& l : AllLights) {
        switch (l.type) {
        case Light_Type::Directional: ++i; continue;
        case Light_Type::Point: {
        ...
        case Light_Type::Tube: {
            if (l.light_ignore_objnum != objnum) {
                ...
                vm_vec_dist_squared_to_line (
                    pos, &l.vec, &l.vec2, &nearest, &dist_squared);
...
light_indexing_info scene_lights::bufferLights () {
    ...
    if (FilteredLights.empty ()) { return light_info; }
...
bool scene_lights::setLights (const light_indexing_info* info) {
    if (info->index_start == current_light_index &&
        info->num_lights == current_num_lights) {
        // don't need to set new lights since the ones requested to be set are
        // currently set
        return false;
    }
