...
void pilotfile_convert::csg_import_ships_weapons () {
    ...
    if (csg->last_ship_flown_index < 0) {
        std::ostringstream error_msg;
        error_msg << "Data mismatch (player ship: "
                  << csg->last_ship_flown_index << ")!";
        throw std::runtime_error (error_msg.str ().c_str ());
    }
    ...
    for (idx = 0; idx < num_missions; idx++) {
        ...
        if (count > 0) {
            ...
            for (j = 0; j < count; j++) {
                ...
                cfread_string_len (
                    n_variable.text, sizeof (n_variable.text), cfp);
                cfread_string_len (
                    n_variable.variable_name,
                    sizeof (n_variable.variable_name), cfp);
...
void pilotfile_convert::csg_import (bool inferno) {
    ...
    if (fver != 15) { throw std::runtime_error ("Unsupported file version!"); }
...
void pilotfile_convert::csg_export_missions () {
    ...
    for (size_t idx = 0; idx < list_size; idx++) {
        ...
        for (size_t j = 0; j < count; j++) {
            ...
            cfwrite_string_len (
                csg->missions[idx].variables[j].variable_name, cfp);
        }
