...
public:
    ...
    bool
    verify (const char* fname, int* rank = NULL, char* valid_language = NULL);

    // whether current campaign savefile has valid data to work with
    bool is_invalid () { return m_data_invalid; }
...
private:
    ...
    // set in case data appears wrong, so we can avoid loading/saving campaign
    // savefile
    bool m_data_invalid;
    ...
    typedef struct scoring_special_t {
        ...
        scoring_special_t ()
            : score (0), rank (RANK_ENSIGN), assists (0), kill_count (0),
              kill_count_ok (0), bonehead_kills (0), p_shots_fired (0),
...
#endif // FREESPACE2_PILOTFILE_PILOTFILE_HH
