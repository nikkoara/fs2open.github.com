...
void convert_pilot_files () {
    ...
    cf_get_file_list (
        existing, CF_TYPE_PLAYERS, "*.plr", CF_SORT_NONE, nullptr,
        CF_LOCATION_ROOT_USER | CF_LOCATION_ROOT_GAME | CF_LOCATION_TYPE_ROOT);

    // get list of old pilots which may need converting, starting with inferno
    // pilots
    Get_file_list_child = "inferno";
    cf_get_file_list (
        old_files, CF_TYPE_SINGLE_PLAYERS, "*.pl2", CF_SORT_NONE, nullptr,
        CF_LOCATION_ROOT_USER | CF_LOCATION_ROOT_GAME | CF_LOCATION_TYPE_ROOT);
    ...
    for (idx = 0; idx < count; idx++) {
        if (old_files[idx].empty ()) { continue; }
        ...
        if (pcon->plr_convert (old_files[idx].c_str (), inferno)) {
            ...
            if (inferno) { Get_file_list_child = "inferno"; }

            cf_get_file_list (
                savefiles, CF_TYPE_SINGLE_PLAYERS, wildcard.c_str (),
                CF_SORT_NONE, nullptr,
