...
void pilotfile::csg_read_missions()
{
        ...
        for (i = 0; i < Campaign.num_missions_completed; i++) {
                ...
                if (missionp->num_goals > 0) {
                        missionp->goals = (mgoal *)malloc(missionp->num_goals * sizeof(mgoal));
                ...
                if (missionp->num_events > 0) {
                        missionp->events = (mevent *)malloc(missionp->num_events * sizeof(mevent));
...
void pilotfile::csg_read_loadout()
{
        ...
        for (uint i = 0; i < list_size; i++) {
                ...
                if ((ship_idx >= (int)ship_list.size()) || (ship_idx < -1)) { // on the casts, assume that ship & weapon lists
...
void pilotfile::csg_read_redalert()
{
        ...
        for (idx = 0; idx < list_size; idx++) {
                ...
                if ((i >= (int)ship_list.size()) || (i < RED_ALERT_LOWEST_VALID_SHIP_CLASS)) {
                        ...
                        (i < 0) && (i >= RED_ALERT_LOWEST_VALID_SHIP_CLASS)) { // ship destroyed/exited
                        ras.ship_class = i;
                } else {
...
bool pilotfile::save_savefile()
{
        ...
        const auto filename = ((fs::path(p->callsign) += ".") += basename) += ".csg";
