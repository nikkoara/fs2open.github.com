...
SourceOrigin::SourceOrigin ()
    : m_originType (SourceOriginType::NONE),
...
void SourceOrigin::getHostOrientation (matrix* matOut) const {
    ...
    case SourceOriginType::PARTICLE:
        vm_vector_2_matrix (
            matOut, &m_origin.m_particle.lock ()->velocity, nullptr, nullptr);
        break;
    case SourceOriginType::VECTOR: // Intentional fall-through, plain vectors
                                   // have no orientation
    default: *matOut = vmd_identity_matrix; break;
    }
}
...
void SourceOrigin::applyToParticleInfo (
    ...
    if (allowRelative) {
        switch (m_originType) {
        case SourceOriginType::OBJECT: {
            info.attached_objnum =
                static_cast< int > (OBJ_INDEX (m_origin.m_object.objp));
...
vec3d SourceOrigin::getVelocity () const {
    ...
    default: return vmd_zero_vector;
    }
}
...
bool SourceOrigin::isValid () const {
    switch (m_originType) {
    case SourceOriginType::NONE: return false;
    case SourceOriginType::OBJECT: {
        if (!m_origin.m_object.IsValid ()) { return false; }
    ...
    case SourceOriginType::PARTICLE: return !m_origin.m_particle.expired ();
    case SourceOriginType::VECTOR: return true;
    }
...
SourceOrientation::SourceOrientation ()
    : m_orientation (vmd_identity_matrix) {}
...
vec3d SourceOrientation::getDirectionVector (
    const SourceOrigin* origin) const {
    if (!m_isRelative) { return m_orientation.vec.fvec; }
...
SourceTiming::SourceTiming ()
    : m_creationTimestamp (timestamp (-1)), m_beginTimestamp (timestamp (-1)),
...
bool SourceTiming::isActive () const {
    if (!timestamp_valid (m_beginTimestamp) &&
        !timestamp_valid (m_endTimestamp)) {
    ...
    return timestamp_elapsed (m_beginTimestamp) &&
           !timestamp_elapsed (m_endTimestamp);
}

bool SourceTiming::isFinished () const {
    // If end isn't valid the timing is never finished
    if (!timestamp_valid (m_endTimestamp)) { return false; }
...
ParticleSource::ParticleSource ()
    : m_effect (nullptr), m_processingCount (0) {}

bool ParticleSource::isValid () const {
    if (m_timing.isFinished ()) { return false; }

    if (m_effect == nullptr) { return false; }

    if (!m_origin.isValid ()) { return false; }

    return true;
}

void ParticleSource::initializeThrusterOffset (
    weapon* /*wp*/, weapon_info* wip) {
    polymodel* pm = model_get (wip->model_num);

    if (pm->n_thrusters < 1) { return; }

    // Only use the first thruster, for multiple thrusters we need more sources
    auto thruster = &pm->thrusters[0];

    if (thruster->num_points < 1) { return; }

    // Only use the first point in the bank
    auto point = &thruster->points[0];

    model_find_world_point (
        &this->m_origin.m_offset, &point->pnt, wip->model_num,
        thruster->submodel_num, &vmd_identity_matrix, &vmd_zero_vector);
}
        ...
        if (IS_VEC_NULL (&m_origin.m_offset)) {
            ...
            if (obj->type == OBJ_WEAPON) {
                ...
                if (wip->subtype == WP_MISSILE && wip->model_num >= 0) {
                    // Now that we are here we know that this is a missile
                    // which has no offset set The particles of a missile
                    // should be created at its thruster
