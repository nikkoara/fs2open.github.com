...
bool BeamPiercingEffect::processSource (const ParticleSource* source) {
    ...
    if (m_velocity != 0.0f) { base_v = m_velocity; }
    else {
        base_v = m_radius;
    }

    if (m_backVelocity != 0.0f) { back_v = m_backVelocity; }
    else {
        back_v = base_v * (-0.2f);
    }
...
void BeamPiercingEffect::pageIn () { bm_page_in_texture (m_effectBitmap); }

void BeamPiercingEffect::setValues (
