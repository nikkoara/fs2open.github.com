    ...
    vec3d getNewDirection (const ParticleSource* source) const {
        ...
        case ConeDirection::Normal: {
            ...
            if (!source->getOrientation ()->getNormal (&normal)) {
                WARNINGF (LOCATION,"Effect '%s' tried to use normal direction for source without a normal!",m_name.c_str ());
                return source->getOrientation ()->getDirectionVector (
                    source->getOrigin ());
            }
...
public:
    explicit GenericShapeEffect (const std::string& name)
        : ParticleEffect (name) {}
    ...
    bool processSource (const ParticleSource* source) override {
        ...
        for (uint i = 0; i < num; ++i) {
            ...
            if (TShape::scale_velocity_deviation ()) {
                // Scale the vector with a random velocity sample and also
                // multiply that with cos(angle between info.vel and sourceDir)
                // That should produce good looking directions where the
                // maximum velocity is only achieved when the particle travels
                // directly on the normal/reflect vector
            ...
            if (m_particleTrail.isValid ()) {
                auto part =
                    m_particleProperties.createPersistentParticle (info);

                auto trailSource =
                    ParticleManager::get ()->createSource (m_particleTrail);
    ...
    void parseValues (bool nocreate) override {
        ...
        if (optional_string ("+Direction:")) {
            ...
            if (!strcasecmp (dirStr.c_str (), "Incoming")) {
                ...
                error_display (
                    0, "Unknown direction name '%s'!", dirStr.c_str ());
            }
        }
...
#endif // FREESPACE2_PARTICLE_EFFECTS_GENERICSHAPEEFFECT_HH
