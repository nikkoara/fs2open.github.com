 ...
 *
 * This class is responsible for managing the loaded effects and processing the
 * created particle sources. The whole effect subsystem is designed to not
 * allocate memory at runtime (except if the vector resizes but that should
...
private:
     ...
     *
     * @warning The pointer is temporary and only valid until the #m_sources is
     * modified!
...
public:
     ...
     * lookup is being done by a sequential search which means it's pretty
     * slow.
     *
     * @param name The name of the effect that is being searchd, may not be
     * empty
 ...
 *
 * This can either be the name of an existing effect or a new effect which is
 * created in-place. If forcedType is specified then the effect will have the
 * specified type or an error will be generated.
 *
 * @param forcedType The type the effect should have, EffectType::Invalid can
 * be specified for any effect type
 * @param name The name of the created effect, an empty string means no special
 * name
 * @return The index of the added effect
 */
 ...
 *
 * If no_create is @c false then this will act like #required_string, otherwise
 * it will be an #optional_string
 *
 ...
 *
 * Parses and animation and returns the handle. If critical is @c true then a
 * failure to load the animation will cause an error. Otherwise it will be
 * cause a warning.
 *
 ...
 *
 * This can be used to parse an effect from another part of the engine. This
 * parses an effect name and returns the index. If the name is not valid an
 * error is displayed.
 *
 * @param objectName Can be optionally specified so the error message is a bit
 * more specific
 * @return The index
 */
...
#endif // FREESPACE2_PARTICLE_PARTICLEMANAGER_HH
