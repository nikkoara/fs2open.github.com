 ...
 *
 * The SourceOrigin class encapsualtes the information about where a source is
 * located. It allows a source to be relative to an object or a particle and
 * keeps track of whether the host objects are still valid.
 */
class SourceOrigin {
private:
...
public:
     ...
     *
     * This checks if the hosting object is valid or if the hosting particle is
     * still valid.
     *
     ...
     *
     * This can be used to initialize the position of a created particle from a
     * source.
     *
     * @param info The particle_info this should be applied to
     * @param allowRelative If @c true then the location of the particle can be
     * relative to the host
     */
        void applyToParticleInfo(
                particle_info &info, bool allowRelative = false) const;
 ...
 *
 * Currently only the forward direction vector is useful because the other
 * vectors of the matrix are chosen pretty arbitrarily. This also contains a
 * normal vector if it was specified when creating the source.
 *
 * An orientation can be either relative or global. In relative mode all
 * transforms should be relative to the host object while in global mode all
 * directions are in world-space. Normals are always in world-space.
 */
...
public:
     ...
     *
     * The vector doesn't have to be normalized before passing it to this
     * function.
     *
 ...
 *
 * This can be used to control when a source is active and when it will be
 * deleted. Since a source can be active at a later time the timing goes
 * through three stages. In the first state the source is created but not
 * active. #isActive() returns false but #isFinished() also return false. When
 * it activates #isActive() will return true while #isFinished() stays @c
 * false. Then #isActive() will be @c false and #isFinished() will be @c true.
 * At this point the source is considered to have expired and it will be
 * removed the next time it is processed.
 */
class SourceTiming {
private:
...
public:
     ...
     *
     * If the source is not active or if the timestamps are not valid then -1
     * is returned
     *
 ...
 *
 * A particle source contains the information about where and for how long
 * particles are created. A particle effect uses this information to create new
 * particles. A particle source has not effect-specific information which means
 * that an effect can only use the information contained in this object.
 *
...
private:
        ...
        SourceOrientation
                m_effectOrientation; //!< The orientation of the particle source
        ...
        size_t m_processingCount; //!< The number of times this effect has been
                                  //!< processed
...
public:
        ...
        inline const SourceOrientation *getOrientation() const
        {
                return &m_effectOrientation;
        }

        inline SourceOrientation *getOrientation()
        {
                return &m_effectOrientation;
        }
     ...
     *
     * This is used to initialize some status that is only available after
...
#endif // FREESPACE2_PARTICLE_PARTICLESOURCE_HH
