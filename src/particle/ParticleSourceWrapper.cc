...
void ParticleSourceWrapper::finish () {
    // This function is currently only used for debugging but may be extended
    ...
    for (auto& source : m_sources) {
        ASSERTX (
            ...
            "initialized a particle source)");
    }
#endif
    for (auto& source : m_sources) { source->finishCreation (); }
...
void ParticleSourceWrapper::moveTo (vec3d* pos) {
    for (auto& source : m_sources) { source->getOrigin ()->moveTo (pos); }
}

void ParticleSourceWrapper::setOrientationFromNormalizedVec (
    vec3d* normalizedDir) {
    for (auto& source : m_sources) {
