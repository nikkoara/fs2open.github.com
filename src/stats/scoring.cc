...
void scoring_eval_badges(scoring_struct *sc)
{
        ...
        for (auto i = 0; i < Num_medals; i++) {
                if (total_kills >= Medals[i].kills_needed && Medals[i].kills_needed > last_badge_kills && Medals[i].kills_needed > 0) {
                        ...
                        if (Player->stats.m_badge_earned.size()) {
                                for (size_t medal = 0;
                                     medal < Player->stats.m_badge_earned.size(); medal++) {
                                        Player->stats
                                                .medal_counts[Player->stats.m_badge_earned[medal]]
                                                = 0;
                                }
...
int scoring_eval_kill_on_weapon(object *weapon_obj, object *other_obj)
{
        ...
        if ((weapon_obj->type != OBJ_WEAPON) || (weapon_obj->instance < 0) || (weapon_obj->instance >= MAX_WEAPONS) || (other_obj == nullptr) || (other_obj->type != OBJ_WEAPON) || (other_obj->instance < 0) || (other_obj->instance >= MAX_WEAPONS) || (other_obj->parent == -1) || (Objects[other_obj->parent].type != OBJ_SHIP)) {
                return -1;
        }

        weapon *dead_wp = &Weapons[weapon_obj->instance]; // the weapon that was killed

        weapon_info *dead_wip = &Weapon_info[dead_wp->weapon_info_index]; // info on the weapon that
                                                                          // was killed
...
void scoring_eval_hit(object *hit_obj, object *other_obj, int from_blast)
{
        ...
        if ((other_obj->type == OBJ_WEAPON) && !(Weapons[other_obj->instance].weapon_flags[Weapon::Weapon_Flags::Already_applied_stats])) {
                ...
                if (hit_obj->type == OBJ_WEAPON) {
                        ...
                        hit_obj_is_bomb = (Weapon_info[Weapons[hit_obj->instance].weapon_info_index]
                ...
                int is_bonehead = 0;
                int sub_type = Weapon_info[Weapons[other_obj->instance].weapon_info_index]
                                       .subtype;

                // determine if this was a bonehead hit or not
                if (hit_obj->type == OBJ_SHIP) {
                        is_bonehead = Ships[hit_obj->instance].team == Ships[Objects[other_obj->parent].instance].team
                                              ? 1
                                              : 0;
                } else if (hit_obj_is_bomb) {
                        is_bonehead = Weapons[hit_obj->instance].team == Ships[Objects[other_obj->parent].instance].team
                                              ? 1
                                              : 0;
                }
