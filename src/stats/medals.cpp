...
static const char* Default_debriefing_bitmaps[NUM_MEDALS_FS2] = {
    "DebriefMedal00", "DebriefMedal01", "DebriefMedal02", "DebriefMedal03",
    "DebriefMedal04", "DebriefMedal05", "DebriefMedal06", "DebriefMedal07",
    "DebriefMedal08", "DebriefMedal09", "DebriefMedal10", "DebriefMedal11",
    "DebriefRank##",  "DebriefWings##", "DebriefBadge01", "DebriefBadge02",
    "DebriefBadge03", "DebriefCrest"
};
...
UI_XSTR Medals_text[GR_NUM_RESOLUTIONS][MEDALS_NUM_TEXT] = {
    {
        // GR_640
        { "Exit", 1466, 587, 416, UI_XSTR_COLOR_PINK, -1,
          &Medals_buttons[GR_640][MEDALS_EXIT].button },
    },
    {
        // GR_1024
        { "Exit", 1466, 943, 673, UI_XSTR_COLOR_PINK, -1,
...
typedef struct medal_display_info {
    ...
    coord2d coords[GR_NUM_RESOLUTIONS]; // screen position (can now be defined
                                        // by the table)
} medal_display_info;
...
medal_stuff::medal_stuff ()
    : num_versions (1), version_starts_at_1 (false),
      available_from_start (false), kills_needed (0) {
    name[0] = '\0';
    bitmap[0] = '\0';
    debrief_bitmap[0] = '\0';
    voice_base[0] = '\0';
...
const char* medal_stuff::get_display_string () {
    if (!alt_name.empty ()) { return alt_name.c_str (); }
    else {
        return name;
    }
}
...
void parse_medal_tbl () {
    ...
    try {
        ...
        if (optional_string ("+Callsign Position 640:")) {
            ...
            Medals_callsign_coords[GR_640].x =
                Default_medals_callsign_coords[GR_640][0];
            Medals_callsign_coords[GR_640].y =
                Default_medals_callsign_coords[GR_640][1];
        }
        if (optional_string ("+Callsign Position 1024:")) {
            ...
            Medals_callsign_coords[GR_1024].x =
                Default_medals_callsign_coords[GR_1024][0];
            Medals_callsign_coords[GR_1024].y =
                Default_medals_callsign_coords[GR_1024][1];
        }
        ...
        if (optional_string ("+Label Position 640:")) {
            ...
            Medals_label_coords[GR_640].x =
                Default_medals_label_coords[GR_640][0];
            Medals_label_coords[GR_640].y =
                Default_medals_label_coords[GR_640][1];
            Medals_label_coords[GR_640].w =
                Default_medals_label_coords[GR_640][2];
        }
        if (optional_string ("+Label Position 1024:")) {
            ...
            Medals_label_coords[GR_1024].x =
                Default_medals_label_coords[GR_1024][0];
            Medals_label_coords[GR_1024].y =
                Default_medals_label_coords[GR_1024][1];
            Medals_label_coords[GR_1024].w =
                Default_medals_label_coords[GR_1024][2];
        }
        ...
        while (required_string_either ("#End", "$Name:")) {
            ...
            if (optional_string ("+Position 640:")) {
                ...
                temp_display.coords[GR_640].x =
                    Default_medal_coords[GR_640][Num_medals][0];
                temp_display.coords[GR_640].y =
            ...
            if (optional_string ("+Position 1024:")) {
                ...
                temp_display.coords[GR_1024].x =
                    Default_medal_coords[GR_1024][Num_medals][0];
                temp_display.coords[GR_1024].y =
            ...
            // some medals are based on kill counts.  When string +Num Kills:
            // is present, we know that this medal is a badge and should be
            // treated specially
            if (optional_string ("+Num Kills:")) {
                ...
                if (optional_string ("$Wavefile 1:"))
                    stuff_string (
                        temp_medal.voice_base, F_NAME, MAX_FILENAME_LEN);
        ...
        // WMC - I don't think this is needed anymore due to my changes to
        // post-mission functions but I'm keeping it here to be sure.
...
DCF (medals, "Grant or revoke medals") {
    ...
    if (dc_optional_string ("all")) {
        for (i = 0; i < Num_medals; i++) { Player->stats.medal_counts[i]++; }
        dc_printf ("Granted all medals\n");
        return;
    }
    else if (dc_optional_string ("clear")) {
        for (i = 0; i < Num_medals; i++) { Player->stats.medal_counts[i] = 0; }
        dc_printf ("Cleared all medals\n");
        return;
    }
    else if (dc_optional_string ("promote")) {
        if (Player->stats.rank < MAX_FREESPACE2_RANK) { Player->stats.rank++; }
        dc_printf ("Promoted to %s\n", Ranks[Player->stats.rank].name);
        return;
    }
    else if (dc_optional_string ("demote")) {
        if (Player->stats.rank > 0) { Player->stats.rank--; }
...
void medal_main_init (player* pl, int mode) {
    ...
    for (idx = 0; idx < Num_medals; idx++) {
        if ((Medals[idx].available_from_start) &&
            (Medals_player->stats.medal_counts[idx] < 1)) {
            Medals_player->stats.medal_counts[idx] = 1;
        }
    }
    ...
    if (Exit_button_hotspot_override >= 0) {
        for (idx = 0; idx < GR_NUM_RESOLUTIONS; idx++) {
            Medals_buttons[idx][MEDALS_EXIT].hotspot =
                Exit_button_hotspot_override;
        }
    }
    ...
    for (idx = 0; idx < MEDALS_NUM_BUTTONS; idx++) {
        // create the object
        Medals_buttons[gr_screen.res][idx].button.create (
            &Medals_window, "", Medals_buttons[gr_screen.res][idx].x,
            Medals_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);

        // set the sound to play when highlighted
        Medals_buttons[gr_screen.res][idx].button.set_highlight_action (
            common_play_highlight_sound);

        // set the ani for the button
        Medals_buttons[gr_screen.res][idx].button.set_bmaps (
            Medals_buttons[gr_screen.res][idx].filename);

        // set the hotspot
        Medals_buttons[gr_screen.res][idx].button.link_hotspot (
            Medals_buttons[gr_screen.res][idx].hotspot);
    }
    ...
    if (Medals_bitmap_mask < 0) {
        ...
        Medals_mask =
            bm_lock (Medals_bitmap_mask, 8, BMP_AABITMAP | BMP_MASK_BITMAP);
    ...
    if (Medals_bitmap_mask >= 0) Medals_window.set_mask_bmap (bitmap_buf);
}
...
void blit_label (const char* label, int num) {
    ...
    // Medals now have alternate display names so this code can be disabled in
    // the mod table
    ...
    x = Medals_label_coords[gr_screen.res].x +
        (Medals_label_coords[gr_screen.res].w - sw) / 2;
...
void blit_callsign () {
    ...
    // Goober5000 - from previous code revisions, I assume 0x8000 means center
    // it on-screen... m!m - 0x8000 was removed so we need to calculate it
    // ourself
    if (x < 0) {
        int w;
...
int medal_main_do () {
    ...
    if ((k == (KEY_CTRLED | KEY_ENTER)) ||
        (Medals_buttons[gr_screen.res][MEDALS_EXIT].button.pressed ())) {
    ...
    region = snazzy_menu_do (
        (ubyte*)Medals_mask->data, Medals_mask_w, Medals_mask_h, Num_medals,
    ...
    if (region == Rank_medal_index) {
        ...
        case ESC_PRESSED:
            if (Medals_mode == MM_NORMAL) {
                ...
                // any calling popup function will know to close the screen
                // down
                return 0;
            }
            break;
        ...
        default:
            if (Player_score->medal_counts[region] > 0) {
                blit_label (
                    Medals[region].get_display_string (),
                    Player_score->medal_counts[region]);
            }
            break;
        } // end switch
...
void medal_main_close () {
    if (Init_flags & MEDAL_BITMAP_INIT) bm_release (Medals_bitmap);

    if (Init_flags & MASK_BITMAP_INIT) { bm_unlock (Medals_bitmap_mask); }

    for (std::vector< medal_display_info >::iterator idx =
             Medal_display_info.begin ();
         idx != Medal_display_info.end (); ++idx) {
        if (idx->bitmap >= 0) { bm_release (idx->bitmap); }
        idx->bitmap = -1;
    }
    ...
    if (Init_flags & MASK_BITMAP_INIT) { bm_release (Medals_bitmap_mask); }

    snazzy_menu_close ();
}

// function to load in the medals for this player.  It loads medals that the
// player has (known by whether or not a non-zero number is present in the
...
void init_medal_bitmaps () {
    ...
    if (gr_screen.res == GR_1024) {
        ...
        if (snprintf (
                filename, NAME_LENGTH, "2_%s",
                Ranks[Player_score->rank].bitmap) >= NAME_LENGTH) {
...
void blit_medals () {
    ...
    for (idx = 0; idx < Num_medals; idx++) {
        if (idx != Rank_medal_index && Player_score->medal_counts[idx] > 0) {
            ...
            if (Medal_display_info[idx].bitmap < 0) { continue; }
#endif
            gr_set_bitmap (Medal_display_info[idx].bitmap);
            gr_bitmap (
                Medal_display_info[idx].coords[gr_screen.res].x,
                Medal_display_info[idx].coords[gr_screen.res].y,
                GR_RESIZE_MENU);
        }
    }

    // now blit rank, since that "medal" doesn't get loaded (or drawn) the
    // normal way
    gr_set_bitmap (Rank_bm);
    gr_bitmap (
        Medal_display_info[Rank_medal_index].coords[gr_screen.res].x,
        Medal_display_info[Rank_medal_index].coords[gr_screen.res].y,
        GR_RESIZE_MENU);
}
