...
/*
    The ins and outs of when/where stats are stored and retreived - BE SURE TO
   FOLLOW THESE GUIDELINES

   SINGLE PLAYER :
        scoring_level_init() is called from game_level_init(). This zeroes out
   mission specific stats scoring_level_close() is called from the debriefing
   screen when the player hits accept. This saves the mission stats to alltime
...
public:
        ...
        std::vector< int > m_badge_earned; // which badge(s) was(/were) earned.
                                           // Calculated after mission is over
        int m_promotion_earned;            // was a promotion earned.  Calculated after
                                           // mission is over

        int m_score;
        int m_kills[MAX_SHIP_CLASSES];   // this will represent all kills in the
                                         // mission (bonehead or not)
        int m_okKills[MAX_SHIP_CLASSES]; // this will be only the "valid" kills the
                                         // player made
        ...
        int m_player_deaths;             // player deaths for the mission (really only useful
                                         // for multiplayer)

        int m_dogfight_kills[MAX_PLAYERS]; // kills by player for multiplayer
                                           // dogfight
...
#endif // FREESPACE2_STATS_SCORING_HH
