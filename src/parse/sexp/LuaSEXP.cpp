...
std::unordered_map< std::string, int > parameter_type_mapping{
    { "boolean", OPF_BOOL },
    { "number", OPF_NUMBER },
    { "ship", OPF_SHIP },
    { "string", OPF_STRING },
...
int get_parameter_type (const std::string& name) {
    ...
    if (iter == parameter_type_mapping.end ()) { return -1; }
    else {
        return iter->second;
    }
}

std::unordered_map< std::string, int > return_type_mapping{
    { "number", OPR_NUMBER },
    { "boolean", OPR_BOOL },
    { "nothing", OPR_NULL },
};
int get_return_type (const std::string& name) {
    ...
    if (iter == return_type_mapping.end ()) { return -1; }
    else {
        return iter->second;
    }
}
...
int get_category (const std::string& name) {
    for (auto& subcat : op_menu) {
        if (subcat.name == name) { return subcat.id; }
    }
...
int LuaSEXP::getMinimumArguments () { return _min_args; }
int LuaSEXP::getMaximumArguments () { return _max_args; }
int LuaSEXP::getArgumentType (int argnum) const {
    if (argnum < 0) { return OPF_NONE; }
    ...
    // First, adjust the argnum base index so that our variable index starts at
    // 0
...
luacpp::LuaValue LuaSEXP::sexpToLua (int node, int argnum) const {
    ...
    case OPF_VARIABLE_NAME: {
        ...
        // Add the variable to the parameter list as a SEXPVariable object
        // handle
        return LuaValue::createValue (
            _action.getLuaState (),
            l_SEXPVariable.Set (sexpvar_h (sexp_variable_index)));
    }
    case OPF_IFF: {
        auto team_idx = iff_lookup (CTEXT (node));

        return LuaValue::createValue (
            _action.getLuaState (), l_Team.Set (team_idx));
    }
    case OPF_WAYPOINT_PATH: {
        ...
        return LuaValue::createValue (
            _action.getLuaState (),
            l_WaypointList.Set (waypointlist_h (wp_list)));
    }
    ...
    case OPF_SHIP: {
        ...
        if (ship_id < 0) {
            // Name is invalid
            return LuaValue::createValue (
                _action.getLuaState (), l_Ship.Set (object_h ()));
        }

        auto objp = &Objects[Ships[ship_id].objnum];

        // The other SEXP code does not validate the object type so this should
        // be safe
        Assertion (
            objp->type == OBJ_SHIP,
            "Ship '%s' was found in the Ships array but has a different "
            "object type in the Objects array. Get a coder!",
            CTEXT (node));

        return LuaValue::createValue (
            _action.getLuaState (), l_Ship.Set (object_h (objp)));
    }
    case OPF_MESSAGE: {
        ...
        return LuaValue::createValue (
            _action.getLuaState (), l_Message.Set (idx));
    }
    case OPF_WING: {
        auto name = CTEXT (node);

        return LuaValue::createValue (
            _action.getLuaState (), l_Wing.Set (wing_name_lookup (name)));
    }
    case OPF_SHIP_CLASS_NAME: {
        auto name = CTEXT (node);
        return LuaValue::createValue (
            _action.getLuaState (), l_Shipclass.Set (ship_info_lookup (name)));
    }
    case OPF_WEAPON_NAME: {
        auto name = CTEXT (node);
        return LuaValue::createValue (
            _action.getLuaState (),
            l_Weaponclass.Set (weapon_info_lookup (name)));
    }
    case OPF_STRING: {
    ...
    default:
        UNREACHABLE (
            "Unhandled argument type! Someone added an argument type but "
            "didn't add handling code to execute().");
...
int LuaSEXP::getSexpReturnValue (const LuaValueList& retVals) const {
    ...
    case OPR_NUMBER:
        if (retVals.size () != 1) {
            Warning (
                LOCATION,
                "Wrong number of return values for Lua SEXP '%s'! Expected 1, "
                "got " SIZE_T_ARG ".",
                _name.c_str (), retVals.size ());
            return 0;
        }
        else if (retVals[0].getValueType () != ValueType::NUMBER) {
            Warning (
                LOCATION,
                "Wrong return type detected for Lua SEXP '%s', expected a "
                "number.",
                _name.c_str ());
            return 0;
        }
    ...
    case OPR_NULL:
        if (retVals.size () != 0) {
            Warning (
                LOCATION,
                "Wrong number of return values for Lua SEXP '%s'! Expected 0, "
                "got " SIZE_T_ARG ".",
                _name.c_str (), retVals.size ());
        }
        return SEXP_TRUE;
    default: return SEXP_TRUE;
    }
}
int LuaSEXP::execute (int node) {
    if (!_action.isValid ()) {
        Error (
            LOCATION,
            "Lua SEXP called without a valid action function! A script "
            "probably failed to set the action for some reason.");
        return SEXP_CANT_EVAL;
    }

    LuaValueList luaParameters;

    // We need to adapt how we handle parameters based on their type. We use
    // this variable to keep track of which parameter we are currently looking
    // at
    int argnum = 0;
    while (node != -1) {
        if (argnum < (int)_argument_types.size ()) {
            // This is a parameter in the normal list so we add it to the
            ...
            // The varargs part is handled in chunks so that scripts can use
            // the data more easily Every repeat pattern instance is put into
            // its own table
            LuaTable varargs_part = LuaTable::create (_action.getLuaState ());

            // Iterate over all parameters in this varargs pattern and put them
            // all in the table If we reach the end of the parameter list
            // inside this for loop we just exit since all parameters are
            // optional
            for (auto i = 0;
                 i < (int)_varargs_type_pattern.size () && node != -1; ++i) {
    ...
    try {
        ...
        // This is only caused by an error in the function but those are
        // already handled by our error function These values are close to what
        // the existing SEXP code does if errors occur while processing a SEXP
        switch (_return_type) {
        case OPR_NUMBER:
        case OPR_BOOL: return SEXP_NAN;
        case OPR_NULL: return SEXP_TRUE;
        default: return SEXP_NAN;
        }
    }
}
int LuaSEXP::getReturnType () { return _return_type; }

int LuaSEXP::getSubcategory () { return _subcategory; }
int LuaSEXP::getCategory () {
    if (_category == OP_CATEGORY_CHANGE) {
        // "Change" is a special case since we can't add new SEXPs to the
        // primary category
...
void LuaSEXP::parseTable () {
    ...
    if (_category < 0) {
        error_display (
            0,
            "Invalid category '%s' found. New main categories can't be added!",
            category.c_str ());
        _category = OP_CATEGORY_CHANGE; // Default to change2 so we have a
                                        // valid value later on
    }
    ...
    if (_subcategory < 0) {
        ...
        if (_subcategory < 0) {
            ...
            error_display (
                0, "No more space for subcategory '%s' free in category '%s'!",
                subcategory.c_str (), category.c_str ());

            // Default to the first subcategory in our category, hopefully it
            // will exist...
        ...
        error_display (
            0,
            "Subcategory '%s' is in a different category than the specified "
            "category! Subcategory names must be unique.",
            subcategory.c_str ());

        // Default to the first subcategory in our category, hopefully it will
        // exist...
        _subcategory = 0x0000 | _category;
    }
    ...
    if (_min_args < 0) {
        error_display (
            0, "Minimum argument number must be at least 0! Got %d.",
            _min_args);
        _min_args = 0;
    }
    ...
    if (optional_string ("$Maximum Arguments:")) {
        ...
        if (_max_args < 0) {
            error_display (
                0, "Maximum argument number must be at least 0! Got %d.",
                _max_args);
            _max_args = 0;
        }
    }
    ...
    if (_max_args < _min_args) {
        error_display (
            0,
            "Maximum argument number must be greater or equal to minimum "
            "number of arguments!");
    ...
    while (optional_string ("$Parameter:")) {
        ...
        if (type < 0) {
            error_display (
                0, "Unknown parameter type '%s'!", type_str.c_str ());
            type = OPF_STRING;
        }

        if (variable_arg_part) { _varargs_type_pattern.push_back (type); }
        else {
        ...
        if (optional_string ("$Repeat")) {
            if (!variable_arg_part) {
                ...
                error_display (
                    0,
                    "A second $Repeat has been encountered! Only one may "
...
luacpp::LuaFunction LuaSEXP::getAction () const { return _action; }
} // namespace sexp
