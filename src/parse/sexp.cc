...
void sexp_nodes_init()
{
        ...
        // if there's enough of a difference to make it worthwhile, free some nodes
                ...
                Num_sexp_nodes - (last_persistent_node + 1) > 2 * SEXP_NODE_INCREMENT) {
...
int alloc_sexp(const char *text, int type, int subtype, int first, int rest)
{
        ...
        if ((sexp_const == OP_TRUE) && (type == SEXP_ATOM) && (subtype == SEXP_ATOM_OPERATOR))
                ...
                (sexp_const == OP_FALSE) && (type == SEXP_ATOM) && (subtype == SEXP_ATOM_OPERATOR))
...
int free_sexp(int num)
{
        ...
        ASSERT(
                Sexp_nodes[num].type != SEXP_NOT_USED); // make sure it is actually used
        ...
        int node, int return_type, int recursive, int *bad_node, int)
{
        ...
        if (Sexp_nodes[node].subtype == SEXP_ATOM_NUMBER && return_type == OPR_BOOL) {
                ...
                ASSERT(
                        Sexp_nodes[node].first == -1); // only lists should have a first pointer
        ...
        while (node != -1) {
                ...
                case OPF_SUBSYSTEM: {
                        ...
                        if ((Operators[op].value == OP_SABOTAGE_SUBSYSTEM) || (Operators[op].value == OP_REPAIR_SUBSYSTEM) || (Operators[op].value == OP_SET_SUBSYSTEM_STRNGTH) || (Operators[op].value == OP_BEAM_FIRE)) {
                                if (!strcasecmp(CTEXT(node), SEXP_HULL_STRING) || !strcasecmp(CTEXT(node), SEXP_SIM_HULL_STRING)) {
                                        break;
                                }
                        }
                ...
                case OPF_AI_GOAL:
                        ...
                        if ((z = check_sexp_syntax(
                                     z, OPR_AI_GOAL, recursive, bad_node))
                            != 0) {
                                return z;
                        }
                ...
                case OPF_MEDAL_NAME:
                        ...
                        for (i = 0; i < Num_medals; i++) {
                                if (!strcasecmp(CTEXT(node), Medals[i].name))
                                        break;
                        }
                ...
                case OPF_WEAPON_NAME:
                        ...
                        for (i = 0; i < Num_weapon_types; i++) {
                                if (!strcasecmp(CTEXT(node), Weapon_info[i].name))
                                        break;
                        }
                ...
                case OPF_HUD_GAUGE_NAME:
                        ...
                        for (i = 0; i < NUM_HUD_GAUGES; i++) {
                                if (!strcasecmp(CTEXT(node), HUD_gauge_text[i]))
                                        break;
                        }
...
int stuff_sexp_variable_list()
{
        ...
        while (*Mp != ')') {
                ...
                if (check_for_string("\"player-persistent\"") || check_for_string("\"save-on-mission-close\"")) {
                        ...
                        check_for_string("\"campaign-persistent\"") || check_for_string("\"save-on-mission-progress\"")) {
                        // eat it
                        get_string(persistent);
                        ignore_white_space();
...
void stuff_sexp_text_string(std::string &dest, int node, int mode)
{
        ...
        if (Sexp_nodes[node].type & SEXP_FLAG_VARIABLE) {
                ...
                ASSERT(
                        (Sexp_variables[sexp_variables_index].type & SEXP_VARIABLE_NUMBER) || (Sexp_variables[sexp_variables_index].type & SEXP_VARIABLE_STRING));

                // number
                if (Sexp_nodes[node].subtype == SEXP_ATOM_NUMBER) {
                        ASSERT(
                                Sexp_variables[sexp_variables_index].type & SEXP_VARIABLE_NUMBER);
        ...
        if (n != -1) {
                if (CAR(n) != -1) {
                        ...
                        if (Sexp_nodes[CAR(n)].value == SEXP_KNOWN_FALSE || Sexp_nodes[CAR(n)].value == SEXP_NAN_FOREVER)
                                ...
                                Sexp_nodes[CAR(n)].value == SEXP_KNOWN_TRUE) // not KNOWN_TRUE == KNOWN_FALSE
                                return SEXP_KNOWN_FALSE;
...
int sexp_has_departed(int n, fix *latest_time)
{
        ...
        while (n != -1) {
                ...
                if (mission_log_get_time(LOG_SHIP_DESTROYED, name, NULL, NULL) || mission_log_get_time(LOG_WING_DESTROYED, name, NULL, NULL) || mission_log_get_time(LOG_SELF_DESTRUCTED, name, NULL, NULL))
                        ...
                        mission_log_get_time(LOG_SHIP_DEPARTED, name, NULL, &time) || mission_log_get_time(LOG_WING_DEPARTED, name, NULL, &time)) {
                        num_departed++;
                        if (latest_time && (time > *latest_time))
                                *latest_time = time;
                }
...
{
        ASSERT(
                op_num == OP_HAS_DOCKED || op_num == OP_HAS_UNDOCKED || op_num == OP_HAS_DOCKED_DELAY || op_num == OP_HAS_UNDOCKED_DELAY);
...
int sexp_special_warp_dist(int n)
{
        ...
        ASSERT(
                (Ships[shipnum].special_warpout_objnum >= 0) && (Ships[shipnum].special_warpout_objnum < MAX_OBJECTS));
...
int sexp_was_medal_granted(int n)
{
        ...
        for (i = 0; i < Num_medals; i++) {
                if (!strcasecmp(medal_name, Medals[i].name))
                        break;
        }
...
int sexp_special_warpout_name(int node)
{
        ...
        // check to see if either ship was destroyed or departed.  If so, then make
        // this node known false
        if (mission_log_get_time(LOG_SHIP_DESTROYED, ship_name, NULL, NULL) || mission_log_get_time(LOG_SHIP_DEPARTED, ship_name, NULL, NULL) || mission_log_get_time(LOG_SELF_DESTRUCTED, ship_name, NULL, NULL) || mission_log_get_time(LOG_SHIP_DESTROYED, knossos, NULL, NULL) || mission_log_get_time(LOG_SHIP_DEPARTED, knossos, NULL, NULL) || mission_log_get_time(LOG_SELF_DESTRUCTED, knossos, NULL, NULL))
                return SEXP_NAN_FOREVER;
        ...
        char *name, char *who_from, char *priority, int group, int delay)
{
        ...
        if (who_from[0] == '#') {
                ...
                mission_log_get_time(LOG_SHIP_DESTROYED, who_from, NULL, NULL) || mission_log_get_time(LOG_SHIP_DEPARTED, who_from, NULL, NULL) || mission_log_get_time(LOG_SELF_DESTRUCTED, who_from, NULL, NULL) || mission_log_get_time(LOG_WING_DESTROYED, who_from, NULL, NULL) || mission_log_get_time(LOG_WING_DEPARTED, who_from, NULL, NULL)) {
                ...
                !strcasecmp(who_from, "<any wingman>") || (wing_name_lookup(who_from) != -1)) {
...
void sexp_transfer_cargo(int n)
{
        ...
        Ships[shipnum2].cargo1 = char(
                (Ships[shipnum1].cargo1 & CARGO_INDEX_MASK) | (Ships[shipnum2].cargo1 & CARGO_NO_DEPLETE));
...
int sexp_targeted(int node)
{
        ...
        if (CDR(node) >= 0) {
                ...
                if (CDR(CDR(node)) >= 0) {
                        ...
                        if (!ptr || subsystem_strcasecmp(ptr->system_info->subobj_name, CTEXT(CDR(CDR(node))))) {
                                return SEXP_FALSE;
                        }
                }
        }
...
int sexp_is_in_turret_fov(int node)
{
        ...
        return object_in_turret_fov(
                       target_objp, turret_subsys, &tvec, &tpos, dist)
                               != 0
                       ? SEXP_TRUE
                       : SEXP_FALSE;
}
...
int process_special_sexps(int index)
{
        ...
        case 5: // Player's shield is quick repaired
                if (!(Ship_info[Player_ship->ship_info_index]
                        ...
                        if (Player_obj->shield_quadrant[FRONT_QUAD] > Player_obj->shield_quadrant
...
char *CTEXT(int n)
{
        ...
        if (Sexp_nodes[n].type & SEXP_FLAG_VARIABLE) {
                ...
                ASSERT(
                        !(Sexp_variables[sexp_variable_index].type & SEXP_VARIABLE_NOT_USED));
                ...
                for (y = 0; y < (int)op_submenu.size(); y++) {
                        if (((op_submenu[y].id & OP_CATEGORY_MASK) == op_menu[x].id)) {
                                ...
                                for (z = 0; z < (int)Operators.size(); z++) {
                                        if ((get_category(Operators[z].value) == op_menu[x].id) && (get_subcategory(Operators[z].value) != -1) && (get_subcategory(Operators[z].value) == op_submenu[y].id)) {
