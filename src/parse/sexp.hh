...
#define OPR_AI_GOAL 4           // is an ai operator (doesn't really return a value, but used for type
                                // matching)
...
#define OPR_FLEXIBLE_ARGUMENT 8 // Goober5000 - is an argument operator (doesn't really return a value,
...
#define OP_END_MISSION     (0x0041 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)            //-Sesquipedalian. replaces end-mission-delay, which
                                                                                          // did nothing
...
#define SEXP_NAN         -32764 //-4 // not a number -- used when ships/wing part of boolean and
                                // haven't arrived yet
#define SEXP_NAN_FOREVER -32763 //-5 // not a number and will never change -- used to falsify
                                // boolean sexpressions
#define SEXP_CANT_EVAL -32762   //-6 // can't evaluate yet for whatever reason (acts like false)
#define SEXP_NUM_EVAL  -32761   //-7 // already completed an arithmetic operation and result is
                                // stored
...
extern int alloc_sexp(const char *, int, int, int, int);
extern int find_free_sexp();
extern int free_one_sexp(int);
extern int free_sexp(int);
extern int free_sexp2(int);
extern int dup_sexp_chain(int);
extern int cmp_sexp_chains(int, int);
extern int find_sexp_list(int);
extern int find_parent_operator(int);
extern int is_sexp_top_level(int);

// Goober5000 - renamed these to be more clear, to prevent bugs :p
extern int get_operator_index(const char *);
extern int get_operator_const(const char *);

extern int check_sexp_syntax(int, int return_type = OPR_BOOL, int recursive = 0, int *bad_node = 0 /*NULL*/, int mode = 0);

extern int get_sexp_main(void);    // Returns start node
extern int run_sexp(const char *); // debug and lua sexps
extern int stuff_sexp_variable_list();
extern int eval_sexp(int, int referenced_node = -1);
extern int eval_num(int);
...
extern int validate_float(char **);
extern int build_sexp_string(std::string &, int, int, int);
extern int sexp_query_type_match(int, int);
extern const char *sexp_error_message(int);
extern int count_free_sexp_nodes();

// Goober5000
void do_action_for_each_special_argument(int);
int special_argument_appears_in_sexp_tree(int);
int special_argument_appears_in_sexp_list(int);
...
// sexp_variable
void sexp_modify_variable(const char *, int, bool sexp_callback = true);
int get_index_sexp_variable_from_node(int);
int get_index_sexp_variable_name(const char *);
int get_index_sexp_variable_name(std::string &);                 // Goober5000
int get_index_sexp_variable_name_special(const char *);          // Goober5000
int get_index_sexp_variable_name_special(std::string &, size_t); // Goober5000
bool sexp_replace_variable_names_with_values(char *, int);       // Goober5000
bool sexp_replace_variable_names_with_values(std::string &);     // Goober5000
int sexp_variable_count();
int sexp_campaign_file_variable_count(); // Goober5000
void sexp_variable_delete(int);
void sexp_variable_sort();
void sexp_fred_modify_variable(const char *, const char *, int, int);
int sexp_add_variable(const char *, const char *, int, int index = -1);
bool generate_special_explosion_block_variables();
int num_block_variables();
bool has_special_explosion_block_index(ship *, int *);

// menu and category stuff
extern int get_sexp_id(char *);
extern int get_category(int);
extern int category_of_subcategory(int);
extern int get_subcategory(int);
...
extern std::vector< op_menu_struct > op_menu, op_submenu;

// WMC
// Outputs sexp.html file
bool output_sexps(const char *);
...
int get_effect_from_name(char *);

void maybe_write_to_event_log(int);
