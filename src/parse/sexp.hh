...
#define OPERATOR_LENGTH \
    30 // if this ever exceeds TOKEN_LENGTH, let JasonH know!
...
#define OPF_SHIP_WING_WHOLETEAM \
    22 // Karajorma - Ship, wing or an entire team's worth of ships
#define OPF_SHIP_WING_SHIPONTEAM_POINT \
    23 // name of a ship, wing, any ship on a team, or a point
...
#define OPF_CARGO \
    41 // Goober5000 - a cargo string (currently used for set-cargo and
       // is-cargo)
...
#define OPF_ARRIVAL_ANCHOR_ALL \
    45 // Goober5000 - all of a ship's possible arrival anchors
...
#define OPF_TURRET_TARGET_ORDER \
    62 // WMC - name of a turret target type (see aiturret.cpp)
#define OPF_SUBSYSTEM_OR_NONE \
    63                 // Goober5000 - an "optional" subsystem argument
#define OPF_PERSONA 64 // Karajorma - name of a persona
#define OPF_SUBSYS_OR_GENERIC \
    65 // Karajorma - a subsystem or a generic name (like engine) which covers
       // all subsystems of that type
#define OPF_ORDER_RECIPIENT \
    66 // Karajorma - since orders can go to All Fighters as well as a ship or
       // wing
#define OPF_SUBSYSTEM_TYPE \
    67 // Goober5000 - a generic subsystem type (navigation, engines, etc.)
       // rather than a specific subsystem
...
#define OPF_WEAPON_BANK_NUMBER \
    77 // Karajorma - The number of a primary/secondary/tertiary weapon bank or
       // all of them
#define OPF_MESSAGE_OR_STRING \
    78 // Goober5000 - provides a list of messages like OPF_MESSAGE, but also
       // allows entering arbitrary strings
#define OPF_HUD_GAUGE 79   // The E
#define OPF_DAMAGE_TYPE 80 // FUBAR - Damage type or <none>
#define OPF_SHIP_EFFECT \
    81 // The E - per-ship effects, as defined in post-processing.tbl
#define OPF_ANIMATION_TYPE 82 // Goober5000 - as defined in modelanim.h
#define OPF_MISSION_MOOD \
    83 // Karajorma - Moods determine which builtin messages will be sent
...
#define OPF_NEBULA_PATTERN \
    86 // Axem - Full Nebula Background Patterns, as defined in nebula.tbl
...
#define OPR_AI_GOAL \
    4 // is an ai operator (doesn't really return a value, but used for type
      // matching)
...
#define OPR_AMBIGUOUS \
    7 // not really a return type, but used for type matching.
#define OPR_FLEXIBLE_ARGUMENT \
    8 // Goober5000 - is an argument operator (doesn't really return a value,
      // but used for type matching)
...
#define OP_CATEGORY_GOAL_EVENT \
    0x0f00 // final category can't be higher than 0x0f00,
...
// Adding more subcategories is possible with the new code.  All that needs to
// be done is to add a #define here (a number from 0x0000 to 0x00ff ORred with
// the category that it goes under), some appropriate case statements in
// get_subcategory() (in sexp.cpp) that will return the subcategory for each
// sexp that uses it, and the submenu name in the op_submenu[] array in
// sexp.cpp.
...
#define OP_HAS_TIME_ELAPSED \
    (0x0008 | OP_CATEGORY_LOGICAL | OP_NONCAMPAIGN_FLAG)
...
#define OP_GOAL_INCOMPLETE \
    (0x0000 | OP_CATEGORY_GOAL_EVENT | OP_NONCAMPAIGN_FLAG)
#define OP_GOAL_TRUE_DELAY \
    (0x0001 | OP_CATEGORY_GOAL_EVENT | OP_NONCAMPAIGN_FLAG)
#define OP_GOAL_FALSE_DELAY \
    (0x0002 | OP_CATEGORY_GOAL_EVENT | OP_NONCAMPAIGN_FLAG)
#define OP_EVENT_INCOMPLETE \
    (0x0003 | OP_CATEGORY_GOAL_EVENT | OP_NONCAMPAIGN_FLAG)
#define OP_EVENT_TRUE_DELAY \
    (0x0004 | OP_CATEGORY_GOAL_EVENT | OP_NONCAMPAIGN_FLAG)
#define OP_EVENT_FALSE_DELAY \
    (0x0005 | OP_CATEGORY_GOAL_EVENT | OP_NONCAMPAIGN_FLAG)
...
#define OP_EVENT_TRUE_MSECS_DELAY \
    (0x000b | OP_CATEGORY_GOAL_EVENT | OP_NONCAMPAIGN_FLAG)
#define OP_EVENT_FALSE_MSECS_DELAY \
    (0x000c | OP_CATEGORY_GOAL_EVENT | OP_NONCAMPAIGN_FLAG)

#define OP_IS_DESTROYED_DELAY \
    (0x0000 | OP_CATEGORY_OBJECTIVE | OP_NONCAMPAIGN_FLAG)
#define OP_IS_SUBSYSTEM_DESTROYED_DELAY \
    (0x0001 | OP_CATEGORY_OBJECTIVE | OP_NONCAMPAIGN_FLAG)
#define OP_IS_DISABLED_DELAY \
    (0x0002 | OP_CATEGORY_OBJECTIVE | OP_NONCAMPAIGN_FLAG)
#define OP_IS_DISARMED_DELAY \
    (0x0003 | OP_CATEGORY_OBJECTIVE | OP_NONCAMPAIGN_FLAG)
#define OP_HAS_DOCKED_DELAY \
    (0x0004 | OP_CATEGORY_OBJECTIVE | OP_NONCAMPAIGN_FLAG)
#define OP_HAS_UNDOCKED_DELAY \
    (0x0005 | OP_CATEGORY_OBJECTIVE | OP_NONCAMPAIGN_FLAG)
#define OP_HAS_ARRIVED_DELAY \
    (0x0006 | OP_CATEGORY_OBJECTIVE | OP_NONCAMPAIGN_FLAG)
#define OP_HAS_DEPARTED_DELAY \
    (0x0007 | OP_CATEGORY_OBJECTIVE | OP_NONCAMPAIGN_FLAG)
#define OP_WAYPOINTS_DONE_DELAY \
    (0x0008 | OP_CATEGORY_OBJECTIVE | OP_NONCAMPAIGN_FLAG)
#define OP_SHIP_TYPE_DESTROYED \
    (0x0009 | OP_CATEGORY_OBJECTIVE | OP_NONCAMPAIGN_FLAG)
#define OP_PERCENT_SHIPS_DEPARTED \
    (0x000a | OP_CATEGORY_OBJECTIVE | OP_NONCAMPAIGN_FLAG)
#define OP_PERCENT_SHIPS_DESTROYED \
    (0x000b | OP_CATEGORY_OBJECTIVE | OP_NONCAMPAIGN_FLAG)
#define OP_DEPART_NODE_DELAY \
    (0x000c | OP_CATEGORY_OBJECTIVE | OP_NONCAMPAIGN_FLAG)
#define OP_DESTROYED_DEPARTED_DELAY \
    (0x000d | OP_CATEGORY_OBJECTIVE | OP_NONCAMPAIGN_FLAG)
#define OP_PERCENT_SHIPS_DISARMED \
    (0x000e | OP_CATEGORY_OBJECTIVE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_PERCENT_SHIPS_DISABLED \
    (0x000f | OP_CATEGORY_OBJECTIVE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_PERCENT_SHIPS_ARRIVED \
    (0x0010 | OP_CATEGORY_OBJECTIVE | OP_NONCAMPAIGN_FLAG) // FUBAR-BDHR
#define OP_NAV_IS_VISITED \
    (0x0011 | OP_CATEGORY_OBJECTIVE | OP_NONCAMPAIGN_FLAG) // Kazan
#define OP_WAS_DESTROYED_BY_DELAY \
    (0x0012 | OP_CATEGORY_OBJECTIVE | OP_NONCAMPAIGN_FLAG) // WCS

#define OP_TIME_SHIP_DESTROYED \
    (0x0000 | OP_CATEGORY_TIME | OP_NONCAMPAIGN_FLAG)
...
#define OP_TIME_WING_DESTROYED \
    (0x0003 | OP_CATEGORY_TIME | OP_NONCAMPAIGN_FLAG)
...
#define OP_MISSION_TIME_MSECS \
    (0x0007 | OP_CATEGORY_TIME | OP_NONCAMPAIGN_FLAG) // Goober5000
...
#define OP_HITS_LEFT_SUBSYSTEM \
    (0x0002 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // deprecated
...
#define OP_DISTANCE_SUBSYSTEM \
    (0x0005 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000
...
#define OP_SKILL_LEVEL_AT_LEAST \
    (0x0008 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG)
#define OP_WAS_PROMOTION_GRANTED \
    (0x0009 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG)
#define OP_WAS_MEDAL_GRANTED \
    (0x000a | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG)
#define OP_CARGO_KNOWN_DELAY \
    (0x000b | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG)
#define OP_CAP_SUBSYS_CARGO_KNOWN_DELAY \
    (0x000c | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG)
#define OP_HAS_BEEN_TAGGED_DELAY \
    (0x000d | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG)
...
#define OP_SHIELD_RECHARGE_PCT \
    (0x0012 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG)
#define OP_ENGINE_RECHARGE_PCT \
    (0x0013 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG)
#define OP_WEAPON_RECHARGE_PCT \
    (0x0014 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG)
#define OP_SHIELD_QUAD_LOW (0x0015 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG)
#define OP_SECONDARY_AMMO_PCT \
    (0x0016 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG)
#define OP_IS_SECONDARY_SELECTED \
    (0x0017 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG)
#define OP_IS_PRIMARY_SELECTED \
    (0x0018 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG)
#define OP_SPECIAL_WARP_DISTANCE \
    (0x0019 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG)
...
#define OP_PRIMARY_AMMO_PCT \
    (0x001c | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_IS_SHIP_STEALTHY \
    (0x001d | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_IS_CARGO \
    (0x001e | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_IS_FRIENDLY_STEALTH_VISIBLE \
    (0x001f | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000

#define OP_GET_OBJECT_X \
    (0x0020 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_GET_OBJECT_Y \
    (0x0021 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_GET_OBJECT_Z \
    (0x0022 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_IS_AI_CLASS \
    (0x0023 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_IS_SHIP_TYPE \
    (0x0024 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_IS_SHIP_CLASS \
    (0x0025 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_NUM_SHIPS_IN_BATTLE \
    (0x0026 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // phreak
#define OP_CURRENT_SPEED \
    (0x0027 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // WMCoolmon
#define OP_IS_IFF \
    (0x0028 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_NUM_WITHIN_BOX \
    (0x0029 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // WMCoolmon
#define OP_SCRIPT_EVAL_NUM \
    (0x002a | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // WMCoolmon
#define OP_SCRIPT_EVAL_STRING \
    (0x002b | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // WMCoolmon
#define OP_NUM_SHIPS_IN_WING \
    (0x002c | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_GET_PRIMARY_AMMO \
    (0x002d | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_GET_SECONDARY_AMMO \
    (0x002e | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_NUM_ASSISTS \
    (0x002f | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma

#define OP_SHIP_SCORE \
    (0x0030 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_SHIP_DEATHS \
    (0x0031 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_RESPAWNS_LEFT \
    (0x0032 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_IS_PLAYER \
    (0x0033 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_GET_DAMAGE_CAUSED \
    (0x0034 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_AFTERBURNER_LEFT \
    (0x0035 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_WEAPON_ENERGY_LEFT \
    (0x0036 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_PRIMARY_FIRED_SINCE \
    (0x0037 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_SECONDARY_FIRED_SINCE \
    (0x0038 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_CUTSCENES_GET_FOV \
    (0x0039 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Echelon9
#define OP_GET_THROTTLE_SPEED \
    (0x003a | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_HITS_LEFT_SUBSYSTEM_GENERIC \
    (0x003b | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_HITS_LEFT_SUBSYSTEM_SPECIFIC \
    (0x003c | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_GET_OBJECT_PITCH \
    (0x003d | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_GET_OBJECT_BANK \
    (0x003e | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_GET_OBJECT_HEADING \
    (0x003f | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000

#define OP_HAS_PRIMARY_WEAPON \
    (0x0040 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_HAS_SECONDARY_WEAPON \
    (0x0041 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_STRING_TO_INT \
    (0x0042 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_STRING_GET_LENGTH \
    (0x0043 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_GET_OBJECT_SPEED_X \
    (0x0044 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG)
#define OP_GET_OBJECT_SPEED_Y \
    (0x0045 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG)
#define OP_GET_OBJECT_SPEED_Z \
    (0x0046 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG)
#define OP_NAV_DISTANCE \
    (0x0047 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Kazan
#define OP_NAV_ISLINKED \
    (0x0048 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Kazan
#define OP_IS_FACING \
    (0x0049 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // The E
#define OP_DIRECTIVE_VALUE \
    (0x004a | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_GET_NUM_COUNTERMEASURES \
    (0x004b | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_IS_IN_BOX \
    (0x004c | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Sushi
#define OP_IS_IN_MISSION \
    (0x004d | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_ARE_SHIP_FLAGS_SET \
    (0x004e | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_TURRET_GET_PRIMARY_AMMO \
    (0x004f | OP_CATEGORY_STATUS | \
     OP_NONCAMPAIGN_FLAG) // DahBlount, part of the turret ammo code

#define OP_TURRET_GET_SECONDARY_AMMO \
    (0x0050 | OP_CATEGORY_STATUS |   \
     OP_NONCAMPAIGN_FLAG) // DahBlount, part of the turret ammo code
#define OP_IS_DOCKED \
    (0x0051 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_IS_IN_TURRET_FOV \
    (0x0052 | OP_CATEGORY_STATUS | OP_NONCAMPAIGN_FLAG) // Goober5000
...
#define OP_DO_FOR_VALID_ARGUMENTS \
    (0x000c | OP_CATEGORY_CONDITIONAL) // Karajorma
#define OP_INVALIDATE_ALL_ARGUMENTS \
    (0x000d | OP_CATEGORY_CONDITIONAL) // Karajorma
#define OP_VALIDATE_ALL_ARGUMENTS \
    (0x000e | OP_CATEGORY_CONDITIONAL)                            // Karajorma
...
#define OP_SABOTAGE_SUBSYSTEM \
    (0x0002 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_SET_SUBSYSTEM_STRNGTH \
    (0x0003 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
...
#define OP_REMOVE_GOAL \
    (0x0009 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
...
#define OP_SEND_RANDOM_MESSAGE \
    (0x000c | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
...
#define OP_GOOD_SECONDARY_TIME \
    (0x0016 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
...
#define OP_SHIP_INVULNERABLE \
    (0x001d | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
...
#define OP_JETTISON_CARGO_DELAY \
    (0x0023 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
...
#define OP_BEAM_PROTECT_SHIP \
    (0x002b | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_BEAM_UNPROTECT_SHIP \
    (0x002c | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
...
#define OP_ADD_REMOVE_ESCORT \
    (0x0031 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_AWACS_SET_RADIUS (0x0032 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_SEND_MESSAGE_LIST \
    (0x0033 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_CAP_WAYPOINT_SPEED \
    (0x0034 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
...
#define OP_TURRET_TAGGED_ONLY_ALL \
    (0x0037 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_TURRET_TAGGED_CLEAR_ALL \
    (0x0038 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_SUBSYS_SET_RANDOM \
    (0x0039 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
...
#define OP_SET_SPECIAL_WARPOUT_NAME \
    (0X003c | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_SHIP_VANISH (0X003d | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_SHIELDS_ON \
    (0x003e | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) //-Sesquipedalian
#define OP_SHIELDS_OFF \
    (0x003f | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) //-Sesquipedalian

#define OP_CHANGE_AI_LEVEL \
    (0x0040 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) //-Sesquipedalian
...
#define OP_SET_UNSCANNED \
    (0x0043 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SHIP_STEALTHY \
    (0x0044 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SHIP_UNSTEALTHY \
    (0x0045 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SET_CARGO \
    (0x0046 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_CHANGE_AI_CLASS \
    (0x0047 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_FRIENDLY_STEALTH_INVISIBLE \
    (0x0048 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_FRIENDLY_STEALTH_VISIBLE \
    (0x0049 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_DAMAGED_ESCORT_LIST \
    (0x004a | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // phreak
#define OP_DAMAGED_ESCORT_LIST_ALL \
    (0x004b | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SHIP_VAPORIZE \
    (0x004c | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SHIP_NO_VAPORIZE \
    (0x004d | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_COLLIDE_INVISIBLE \
    (0x004e | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_DONT_COLLIDE_INVISIBLE \
    (0x004f | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000

#define OP_PRIMITIVE_SENSORS_SET_RANGE \
    (0x0050 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_CHANGE_SHIP_CLASS \
    (0x0051 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SCRIPT_EVAL \
    (0x0052 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_SET_SUPPORT_SHIP \
    (0x0053 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_DEACTIVATE_GLOW_POINTS \
    (0x0054 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) //-Bobboau
#define OP_ACTIVATE_GLOW_POINTS \
    (0x0055 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) //-Bobboau
#define OP_DEACTIVATE_GLOW_MAPS \
    (0x0056 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) //-Bobboau
#define OP_ACTIVATE_GLOW_MAPS \
    (0x0057 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) //-Bobboau
#define OP_DEACTIVATE_GLOW_POINT_BANK \
    (0x0058 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) //-Bobboau
#define OP_ACTIVATE_GLOW_POINT_BANK \
    (0x0059 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) //-Bobboau
#define OP_CHANGE_SOUNDTRACK \
    (0x005a | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_TECH_ADD_INTEL \
    (0x005b | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_TECH_RESET_TO_DEFAULT \
    (0x005c | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_EXPLOSION_EFFECT \
    (0x005d | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_WARP_EFFECT \
    (0x005e | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SET_OBJECT_FACING \
    (0x005f | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000

#define OP_SET_OBJECT_FACING_OBJECT \
    (0x0060 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SET_OBJECT_POSITION \
    (0x0061 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_PLAY_SOUND_FROM_TABLE \
    (0x0062 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_PLAY_SOUND_FROM_FILE \
    (0x0063 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_CLOSE_SOUND_FROM_FILE \
    (0x0064 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_HUD_DISABLE \
    (0x0065 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_KAMIKAZE \
    (0x0066 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) //-Sesquipedalian
#define OP_MISSION_SET_SUBSPACE \
    (0x0067 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_TURRET_TAGGED_SPECIFIC \
    (0x0068 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // phreak
#define OP_TURRET_TAGGED_CLEAR_SPECIFIC \
    (0x0069 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // phreak
#define OP_LOCK_ROTATING_SUBSYSTEM \
    (0x006a | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_FREE_ROTATING_SUBSYSTEM \
    (0x006b | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_REVERSE_ROTATING_SUBSYSTEM \
    (0x006c | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_ROTATING_SUBSYS_SET_TURN_TIME \
    (0x006d | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_PLAYER_USE_AI \
    (0x006e | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_PLAYER_NOT_USE_AI \
    (0x006f | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000

#define OP_HUD_DISABLE_EXCEPT_MESSAGES \
    (0x0070 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_FORCE_JUMP \
    (0x0071 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_HUD_SET_TEXT \
    (0x0072 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_HUD_SET_TEXT_NUM \
    (0x0073 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_HUD_SET_COORDS \
    (0x0074 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_HUD_SET_FRAME \
    (0x0075 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_HUD_SET_COLOR \
    (0x0076 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_HUD_SET_MAX_TARGETING_RANGE \
    (0x0077 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SHIP_TAG \
    (0x0078 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SHIP_UNTAG \
    (0x0079 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SHIP_CHANGE_ALT_NAME \
    (0x007a | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SCRAMBLE_MESSAGES \
    (0x007b | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // phreak
#define OP_UNSCRAMBLE_MESSAGES \
    (0x007c | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // phreak
#define OP_CUTSCENES_SET_CUTSCENE_BARS \
    (0x007d | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_CUTSCENES_UNSET_CUTSCENE_BARS \
    (0x007e | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_CUTSCENES_FADE_IN \
    (0x007f | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC

#define OP_CUTSCENES_FADE_OUT \
    (0x0080 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_CUTSCENES_SET_CAMERA_POSITION \
    (0x0081 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_CUTSCENES_SET_CAMERA_FACING \
    (0x0082 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_CUTSCENES_SET_CAMERA_FACING_OBJECT \
    (0x0083 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_CUTSCENES_SET_CAMERA_ROTATION \
    (0x0084 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_CUTSCENES_SET_FOV \
    (0x0085 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_CUTSCENES_RESET_FOV \
    (0x0086 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_CUTSCENES_RESET_CAMERA \
    (0x0087 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_CUTSCENES_SHOW_SUBTITLE \
    (0x0088 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC / deprecated
#define OP_CUTSCENES_SET_TIME_COMPRESSION \
    (0x0089 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_CUTSCENES_RESET_TIME_COMPRESSION \
    (0x008a | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_CUTSCENES_FORCE_PERSPECTIVE \
    (0x008b | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_JUMP_NODE_SET_JUMPNODE_NAME \
    (0x008c | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // CommanderDJ
#define OP_JUMP_NODE_SET_JUMPNODE_COLOR \
    (0x008d | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_JUMP_NODE_SET_JUMPNODE_MODEL \
    (0x008e | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_JUMP_NODE_SHOW_JUMPNODE \
    (0x008f | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC

#define OP_JUMP_NODE_HIDE_JUMPNODE \
    (0x0090 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_SHIP_GUARDIAN_THRESHOLD \
    (0x0091 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SHIP_SUBSYS_GUARDIAN_THRESHOLD \
    (0x0092 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SET_SKYBOX_MODEL \
    (0x0093 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // taylor
#define OP_SHIP_CREATE (0X0094 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_WEAPON_CREATE \
    (0x0095 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SET_OBJECT_SPEED_X \
    (0X0096 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_SET_OBJECT_SPEED_Y \
    (0X0097 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_SET_OBJECT_SPEED_Z \
    (0X0098 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_MISSION_SET_NEBULA \
    (0x0099 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_ADD_BACKGROUND_BITMAP \
    (0x009a | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_REMOVE_BACKGROUND_BITMAP \
    (0x009b | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_ADD_SUN_BITMAP (0x009c | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_REMOVE_SUN_BITMAP \
    (0x009d | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_NEBULA_CHANGE_STORM \
    (0x009e | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_NEBULA_TOGGLE_POOF \
    (0x009f | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)

#define OP_TURRET_CHANGE_WEAPON \
    (0x00a0 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_TURRET_SET_TARGET_ORDER \
    (0x00a1 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_SHIP_TURRET_TARGET_ORDER \
    (0x00a2 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_SET_PRIMARY_AMMO \
    (0x00a3 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_SET_SECONDARY_AMMO \
    (0x00a4 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_SHIP_BOMB_TARGETABLE \
    (0x00a5 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_SHIP_BOMB_UNTARGETABLE \
    (0x00a6 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_SHIP_SUBSYS_TARGETABLE \
    (0x00a7 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SHIP_SUBSYS_UNTARGETABLE \
    (0x00a8 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SET_DEATH_MESSAGE \
    (0x00a9 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SET_PRIMARY_WEAPON \
    (0x00aa | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_SET_SECONDARY_WEAPON \
    (0x00ab | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_DISABLE_BUILTIN_MESSAGES \
    (0x00ac | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_ENABLE_BUILTIN_MESSAGES \
    (0x00ad | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_LOCK_PRIMARY_WEAPON \
    (0x00ae | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_UNLOCK_PRIMARY_WEAPON \
    (0x00af | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma

#define OP_LOCK_SECONDARY_WEAPON \
    (0x00b0 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_UNLOCK_SECONDARY_WEAPON \
    (0x00b1 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_SET_CAMERA_SHUDDER \
    (0x00b2 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_ALLOW_TREASON \
    (0x00b3 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_SHIP_COPY_DAMAGE \
    (0x00b4 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_CHANGE_SUBSYSTEM_NAME \
    (0x00b5 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_SET_PERSONA \
    (0x00b6 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_CHANGE_PLAYER_SCORE \
    (0x00b7 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_CHANGE_TEAM_SCORE \
    (0x00b8 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_CUTSCENES_SET_CAMERA_FOV \
    (0x00b9 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_CUTSCENES_SET_CAMERA \
    (0x00ba | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_CUTSCENES_SET_CAMERA_HOST \
    (0x00bb | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_CUTSCENES_SET_CAMERA_TARGET \
    (0x00bc | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // WMC
#define OP_LOCK_AFTERBURNER \
    (0x00bd | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // KeldorKatarn
#define OP_UNLOCK_AFTERBURNER \
    (0x00be | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // KeldorKatarn
#define OP_SHIP_CHANGE_CALLSIGN \
    (0x00bf | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // FUBAR

#define OP_SET_RESPAWNS \
    (0x00c0 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_SET_AFTERBURNER_ENERGY \
    (0x00c1 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_SET_WEAPON_ENERGY \
    (0x00c2 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_SET_SHIELD_ENERGY \
    (0x00c3 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_SET_AMBIENT_LIGHT \
    (0x00c4 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_CHANGE_IFF_COLOR \
    (0x00c5 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Wanderer
#define OP_TURRET_SUBSYS_TARGET_DISABLE \
    (0x00c6 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Wanderer
#define OP_TURRET_SUBSYS_TARGET_ENABLE \
    (0x00c7 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Wanderer
#define OP_REMOVE_WEAPONS \
    (0x00c8 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Wanderer
#define OP_SHIP_MANEUVER \
    (0x00c9 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Wanderer
#define OP_SHIP_ROT_MANEUVER \
    (0x00ca | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Wanderer
#define OP_SHIP_LAT_MANEUVER \
    (0x00cb | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Wanderer
#define OP_GET_VARIABLE_BY_INDEX \
    (0x00cc | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SET_VARIABLE_BY_INDEX \
    (0x00cd | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SET_POST_EFFECT \
    (0x00ce | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Hery
#define OP_TURRET_SET_OPTIMUM_RANGE \
    (0x00cf | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // FUBAR

#define OP_TURRET_SET_DIRECTION_PREFERENCE \
    (0x00d0 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // FUBAR
#define OP_TURRET_SET_TARGET_PRIORITIES \
    (0x00d1 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // FUBAR
#define OP_SET_ARMOR_TYPE \
    (0x00d2 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // FUBAR
#define OP_CUTSCENES_SHOW_SUBTITLE_TEXT \
    (0x00d3 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_CUTSCENES_SHOW_SUBTITLE_IMAGE \
    (0x00d4 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_HUD_DISPLAY_GAUGE \
    (0x00d5 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)
#define OP_SET_SOUND_ENVIRONMENT \
    (0x00d6 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Taylor
#define OP_UPDATE_SOUND_ENVIRONMENT \
    (0x00d7 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Taylor
#define OP_SET_EXPLOSION_OPTION \
    (0x00d8 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_ADJUST_AUDIO_VOLUME \
    (0x00d9 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // The E
#define OP_FORCE_GLIDE \
    (0x00da | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // The E
#define OP_TURRET_SET_RATE_OF_FIRE \
    (0x00db | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // FUBAR
#define OP_HUD_SET_MESSAGE \
    (0x00dc | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // The E
#define OP_SHIP_SUBSYS_NO_REPLACE \
    (0x00dd | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // FUBAR
#define OP_SET_IMMOBILE \
    (0x00de | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SET_MOBILE \
    (0x00df | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000

#define OP_SHIP_SUBSYS_NO_LIVE_DEBRIS \
    (0x00e0 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // FUBAR
#define OP_SHIP_SUBSYS_VANISHED \
    (0x00e1 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // FUBAR
#define OP_SHIP_SUBSYS_IGNORE_IF_DEAD \
    (0x00e2 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // FUBAR
#define OP_HUD_SET_DIRECTIVE \
    (0x00e3 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // The E
#define OP_HUD_GAUGE_SET_ACTIVE    \
    (0x00e4 | OP_CATEGORY_CHANGE | \
     OP_NONCAMPAIGN_FLAG) // The E - slightly deprecated
#define OP_HUD_ACTIVATE_GAUGE_TYPE \
    (0x00e5 | OP_CATEGORY_CHANGE | \
     OP_NONCAMPAIGN_FLAG) // The E - slightly deprecated
#define OP_SET_OBJECT_ORIENTATION \
    (0x00e6 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_STRING_CONCATENATE \
    (0x00e7 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_INT_TO_STRING \
    (0x00e8 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_WEAPON_SET_DAMAGE_TYPE \
    (0x00e9 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // FUBAR
#define OP_SHIP_SET_DAMAGE_TYPE \
    (0x00ea | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // FUBAR
#define OP_SHIP_SHOCKWAVE_SET_DAMAGE_TYPE \
    (0x00eb | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // FUBAR
#define OP_FIELD_SET_DAMAGE_TYPE \
    (0x00ec | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // FUBAR
#define OP_TURRET_PROTECT_SHIP \
    (0x00ed | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_TURRET_UNPROTECT_SHIP \
    (0x00ee | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_DISABLE_ETS \
    (0x00ef | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // The E

#define OP_ENABLE_ETS \
    (0x00f0 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // The E
#define OP_NAV_ADD_WAYPOINT \
    (0x00f1 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Kazan
#define OP_NAV_ADD_SHIP \
    (0x00f2 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG)                // Kazan
#define OP_NAV_DEL (0x00f3 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Kazan
#define OP_NAV_HIDE \
    (0x00f4 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Kazan
#define OP_NAV_RESTRICT \
    (0x00f5 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Kazan
#define OP_NAV_UNHIDE \
    (0x00f6 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Kazan
#define OP_NAV_UNRESTRICT \
    (0x00f7 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Kazan
#define OP_NAV_SET_VISITED \
    (0x00f8 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Kazan
#define OP_NAV_SET_CARRY \
    (0x00f9 | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Kazan
#define OP_NAV_UNSET_CARRY \
    (0x00fa | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Kazan
#define OP_NAV_UNSET_VISITED \
    (0x00fb | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Kazan
#define OP_NAV_SET_NEEDSLINK \
    (0x00fc | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Kazan
#define OP_NAV_UNSET_NEEDSLINK \
    (0x00fd | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Kazan
#define OP_NAV_USECINEMATICS \
    (0x00fe | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Kazan
#define OP_NAV_USEAP \
    (0x00ff | OP_CATEGORY_CHANGE | OP_NONCAMPAIGN_FLAG) // Kazan

// 0x00ff is the last remaining sexp in the CHANGE category!  Future change
// sexps should go under CHANGE2

#define OP_STRING_GET_SUBSTRING \
    (0x0000 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_STRING_SET_SUBSTRING \
    (0x0001 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SET_NUM_COUNTERMEASURES \
    (0x0002 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_ADD_TO_COLGROUP \
    (0x0003 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // The E
#define OP_REMOVE_FROM_COLGROUP \
    (0x0004 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // The E
#define OP_GET_COLGROUP_ID \
    (0x0005 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // The E
#define OP_SHIP_EFFECT \
    (0x0006 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Valathil
#define OP_CLEAR_SUBTITLES \
    (0x0007 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // The E
#define OP_BEAM_FIRE_COORDS \
    (0x0008 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SET_DOCKED \
    (0x0009 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Sushi
#define OP_SET_THRUSTERS \
    (0x000a | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // The E
#define OP_TRIGGER_SUBMODEL_ANIMATION \
    (0x000b | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_HUD_CLEAR_MESSAGES \
    (0x000c | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Swifty
#define OP_SET_PLAYER_ORDERS \
    (0x000d | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_SUPERNOVA_STOP \
    (0x000e | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // CommanderDJ
...
#define OP_SET_DEBRIEFING_TOGGLED \
    (0x0010 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SET_SUBSPACE_DRIVE \
    (0x0011 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SET_ARRIVAL_INFO \
    (0x0012 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SET_DEPARTURE_INFO \
    (0x0013 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SET_SKYBOX_ORIENT \
    (0x0014 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_DESTROY_INSTANTLY \
    (0x0015 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Admiral MS
#define OP_DESTROY_SUBSYS_INSTANTLY \
    (0x0016 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Admiral MS
#define OP_DEBUG \
    (0x0017 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_SET_MISSION_MOOD \
    (0x0018 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_NAV_SELECT \
    (0x0019 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Talon1024
#define OP_NAV_UNSELECT \
    (0x001a | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Talon1024
#define OP_ALTER_SHIP_FLAG \
    (0x001b | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_CHANGE_TEAM_COLOR \
    (0x001c | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // The E
#define OP_NEBULA_CHANGE_PATTERN \
    (0x001d | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Axem
#define OP_PLAYER_IS_CHEATING_BASTARD \
    (0x001e | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // The E
#define OP_TECH_ADD_INTEL_XSTR \
    (0x001f | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Goober5000

#define OP_COPY_VARIABLE_FROM_INDEX \
    (0x0020 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_COPY_VARIABLE_BETWEEN_INDEXES \
    (0x0021 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_GET_ETS_VALUE \
    (0x0022 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // niffiwan
#define OP_SET_ETS_VALUES \
    (0x0023 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // niffiwan
#define OP_CALL_SSM_STRIKE \
    (0x0024 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // X3N0-Life-Form
#define OP_SET_MOTION_DEBRIS \
    (0x0025 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // The E
#define OP_HUD_SET_CUSTOM_GAUGE_ACTIVE \
    (0x0026 | OP_CATEGORY_CHANGE2 |    \
     OP_NONCAMPAIGN_FLAG) // The E, just revamped a bit by Axem
#define OP_HUD_SET_RETAIL_GAUGE_ACTIVE \
    (0x0027 | OP_CATEGORY_CHANGE2 |    \
     OP_NONCAMPAIGN_FLAG) // The E, just revamped a bit by Axem
#define OP_SCRIPT_EVAL_MULTI \
    (0x0028 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Karajorma
#define OP_PAUSE_SOUND_FROM_FILE \
    (0x0029 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_SCRIPT_EVAL_BLOCK \
    (0x002a | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // niffiwan
#define OP_BEAM_FLOATING_FIRE \
    (0x002b | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // MageKing17
#define OP_TURRET_SET_PRIMARY_AMMO  \
    (0x002c | OP_CATEGORY_CHANGE2 | \
     OP_NONCAMPAIGN_FLAG) // DahBlount, part of the turret ammo changes
#define OP_TURRET_SET_SECONDARY_AMMO \
    (0x002d | OP_CATEGORY_CHANGE2 |  \
     OP_NONCAMPAIGN_FLAG) // DahBlount, part of the turret ammo changes
#define OP_JETTISON_CARGO_NEW \
    (0x002e | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_STRING_CONCATENATE_BLOCK \
    (0x002f | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_MODIFY_VARIABLE_XSTR \
    (0X0030 | OP_CATEGORY_CHANGE2 | OP_NONCAMPAIGN_FLAG) // m!m
...
#define OP_AI_KEEP_SAFE_DISTANCE \
    (0x000d | OP_CATEGORY_AI | OP_NONCAMPAIGN_FLAG)
...
#define OP_AI_IGNORE_NEW \
    (0x0011 | OP_CATEGORY_AI | OP_NONCAMPAIGN_FLAG) // Goober5000
#define OP_AI_FORM_ON_WING \
    (0x0012 | OP_CATEGORY_AI | OP_NONCAMPAIGN_FLAG) // The E
#define OP_AI_CHASE_SHIP_CLASS \
    (0x0013 | OP_CATEGORY_AI | OP_NONCAMPAIGN_FLAG) // Goober5000

#define OP_GOALS_ID (0x0000 | OP_CATEGORY_UNLISTED)
#define OP_NEXT_MISSION \
    (0x0001 | OP_CATEGORY_UNLISTED) // used in campaign files for branching
 ...
 *
 * The name CAR originates from the original LISP language where it was a
 * function which retrieved the first element of a list.
 *
 * @see https://en.wikipedia.org/wiki/CAR_and_CDR
 */
#define CAR(n) ((n < 0) ? -1 : Sexp_nodes[n].first)
/**
 * @brief Returns the rest of a SEXP list. The rest is everything starting from
 * the second element.
 *
 * The name CDR originates from the original LISP language where it was a
 * function which retrieved the "rest" of a list.
 *
 * @see https://en.wikipedia.org/wiki/CAR_and_CDR
 */
...
// defines for type field of sexp nodes.  The actual type of the node will be
// stored in the lower two bytes of the field.  The upper two bytes will be
// used for flags (bleah...) Be sure not to conflict with type field of
// sexp_variable
...
#define SEXP_VARIABLE_IS_PERSISTENT           \
...
// defines to short circuit evaluation when possible. Also used when goals
// can't be satisfied yet because ship (or wing) hasn't been created yet.

#define SEXP_TRUE 1
#define SEXP_FALSE 0
// Goober5000: changed these to unlikely values, because now we have sexps
...
// next define used to eventually mark a directive as satisfied even though
// there may be more waves for a wing.  bascially a hack for the directives
// display.
...
extern int Sexp_useful_number; // a variable to pass useful info in from
                               // external modules
...
extern void
update_sexp_references (const char* old_name, const char* new_name);
extern void update_sexp_references (
...
// functions to change the attributes of an sexpression tree to persistent or
// not persistent
...
void sexp_modify_variable (
    const char* text, int index, bool sexp_callback = true);
...
int get_index_sexp_variable_name_special (
    std::string& text, size_t startpos); // Goober5000
bool sexp_replace_variable_names_with_values (
    char* text, int max_len);                                     // Goober5000
...
int sexp_variable_typed_count (
    int sexp_variables_index, int variable_type); // Karajorma
...
int sexp_add_variable (
    const char* text, const char* var_name, int type, int index = -1);
...
#endif // FREESPACE2_PARSE_SEXP_HH
