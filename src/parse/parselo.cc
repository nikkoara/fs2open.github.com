...
int skip_to_string(const char *pstr, const char *end)
{
        ...
        while ((*Mp != '\0') && strncasecmp(pstr, Mp, len) != 0) {
                ...
                if (end && !strncasecmp(end, Mp, len2))
                        return -1;
...
int skip_to_start_of_string(const char *pstr, const char *end)
{
        ...
        while ((*Mp != '\0') && strncasecmp(pstr, Mp, len) != 0) {
                ...
                if (end && !strncasecmp(end, Mp, endlen))
                        return 0;
        ...
        const char *pstr1, const char *pstr2, const char *end)
{
        ...
        while ((*Mp != '\0') && strncasecmp(pstr1, Mp, len1) != 0 && strncasecmp(pstr2, Mp, len2) != 0) {
                if (end && *Mp == '#')
                        return 0;
        ...
        const char *pstr, const char *end, const char *end2)
{
        ...
        ignore_white_space();
        while ((*Mp != '\0') && strncasecmp(pstr, Mp, strlen(pstr)) != 0) {
                if ((*Mp == '#') || (end && !strncasecmp(end, Mp, strlen(end))) || (end2 && !strncasecmp(end2, Mp, strlen(end2)))) {
                        Mp = NULL;
                        break;
                }
...
void copy_to_next_white(char *outstr, char *instr, int max)
{
        ...
        while (((ch = *instr++) > 0) && (ch != EOLN) && (ch != '\0') && (count < max)) {
                ...
                if (!in_quotes && is_parenthesis(ch)) // not in quotes, parentheses are important
...
char *alloc_block(const char *startstr, const char *endstr, int extra_chars)
{
        ...
        int level = 1;
        while (*pos != '\0') {
                if (!strncasecmp(pos, startstr, slen)) {
                        level++;
        ...
        const char *line_start, bool &compatible_version, int &tag_len)
{
        ...
        // -MageKing17
        if (strncasecmp(line_start, ";;FSO ", 6) != 0)
                return false;
        ...
        // hack for releases
        if (s_num == 4 && FS_VERSION_REVISION < 1000) {
                s_num = 3;
        }
...
void read_raw_file_text(const char *filename, int mode, char *raw_text)
{
        ...
        if (Unicode_text_mode) {
                ...
                auto invalid = unicode::detail::find_invalid(raw_text, raw_text + file_len);
...
void stuff_boolean(bool *b, bool a_to_eol)
{
        ...
        if (isdigit(token[0])) {
                ...
                if (!strcasecmp(token, "yes") || !strcasecmp(token, "true") || !strcasecmp(token, "ja") // German
                    || !strcasecmp(token, "Oui")                                                        // French
                    || !strcasecmp(token, "si")                                                         // Spanish
                    || !strcasecmp(token, "ita vero")                                                   // Latin
                    || !strcasecmp(token, "HIja'") || !strcasecmp(token, "HISlaH"))                     // Klingon
                {
                        *b = true;
                } else if (
                        !strcasecmp(token, "no") || !strcasecmp(token, "false") || !strcasecmp(token, "nein") // German
...
int stuff_loadout_list(int *ilp, int max_ints, int lookup_type)
{
        ...
        while (*Mp != ')') {
                ...
                if ((lookup_type == MISSION_LOADOUT_SHIP_LIST) && (!(Ship_info[index].flags[Ship::Info_Flags::Player_ship]))) {
                        ...
                        (lookup_type == MISSION_LOADOUT_WEAPON_LIST) && (!(Weapon_info[index].wi_flags[Weapon::Info_Flags::Player_allowed]))) {
                        clean_loadout_list_entry();
