 ...
 */
int object_in_turret_fov (
    object* objp, ship_subsys* ss, vec3d* tvec, vec3d* tpos, float dist) {
    vec3d v2e;
    float size_mod;
    bool in_fov;
...
bool is_object_radius_in_turret_fov (
    object* objp, ship_subsys* ss, vec3d* tvec, vec3d* tpos, vec3d* v2e,
    vec3d* predicted_pos, float distance) {
    float target_dist = distance;
    if (distance == 0.0f) target_dist = vm_vec_dist (predicted_pos, tpos);

    if (object_in_turret_fov (
            objp, ss, tvec, tpos, target_dist + objp->radius)) {
        ...
        if (IS_VEC_NULL_SQ_SAFE (&temp_vec)) {
            // return false, target is perfectly aligned over or below the
            // turret safe bet is to allow turret to reacquire better target
            return false;
        }
            ...
            if (!((temp_vec2.xyz.x == 0) && (temp_vec2.xyz.y == 0))) {
                ...
                if (!IS_VEC_NULL_SQ_SAFE (&temp_vec2)) {
                    ...
                    if (-temp_vec2.xyz.y < tp->turret_y_fov) {
                        ...
                        if (temp_vec2.xyz.x < 0) check_pos = -1;
                        temp_vec2.xyz.x =
                            check_pos *
                            sqrtf (1 - (temp_vec2.xyz.y * temp_vec2.xyz.y));
...
int bomb_headed_towards_ship (object* bomb_objp, object* ship_objp) {
    ...
    vm_vec_normalized_dir (
        &bomb_to_ship_vector, &ship_objp->pos, &bomb_objp->pos);
 ...
 *
 * @note All non-negative return values are expressed in what I like to call
 * "widx"s.
 ...
 */
bool all_turret_weapons_have_flags (
    ship_weapon* swp, flagset< Weapon::Info_Flags > flags) {
    int i;
...
bool all_turret_weapons_have_flags (
    ship_weapon* swp, Weapon::Info_Flags flags) {
    int i;
    for (i = 0; i < swp->num_primary_banks; i++) {
        if (!(Weapon_info[swp->primary_bank_weapons[i]].wi_flags[flags]))
            return false;
...
/**
 * Just gloms all the flags from all the weapons into one variable.  More
 * efficient if all you need to do is test for the existence of a flag.
 */
 ...
 *
 * @note It might be a little faster to optimize based on WP_LASER should only
 * appear in primaries and WP_MISSILE in secondaries. but in the interest of
...
/**
 * Use for getting a Weapon_info pointer, given a turret and a turret weapon
 * indice
 *
...
weapon_info* get_turret_weapon_wip (ship_weapon* swp, int weapon_num) {
    ...
    if (weapon_num >= MAX_SHIP_PRIMARY_BANKS)
        return &Weapon_info
            [swp->secondary_bank_weapons[weapon_num - MAX_SHIP_PRIMARY_BANKS]];
    else
...
int get_turret_weapon_next_fire_stamp (ship_weapon* swp, int weapon_num) {
    ...
    if (weapon_num >= MAX_SHIP_PRIMARY_BANKS)
        return swp
            ->next_secondary_fire_stamp[weapon_num - MAX_SHIP_PRIMARY_BANKS];
    else
...
int valid_turret_enemy (object* objp, object* turret_parent) {
    if (objp == turret_parent) { return 0; }
    ...
    if (objp->type == OBJ_SHIP) {
        ...
        if (objp->flags[Object::Object_Flags::Protected]) { return 0; }

        // don't shoot at ships without collision check
        if (!(objp->flags[Object::Object_Flags::Collides])) { return 0; }

        // don't shoot at arriving ships
        if (shipp->is_arriving ()) { return 0; }

        // Goober5000 - don't fire at cargo containers (now specified in
        // ship_types)
        if ((sip->class_type >= 0) &&
            !(Ship_types[sip->class_type]
                  .flags[Ship::Type_Info_Flags::AI_turrets_attack])) {
            return 0;
        }
    ...
    if (objp->type == OBJ_WEAPON) {
        ...
        if (wip->subtype == WP_LASER &&
            !(wip->wi_flags[Weapon::Info_Flags::
                                Turret_Interceptable])) { // If the thing can't
                                                          // be shot down,
                                                          // don't try.
                                                          // -MageKing17
            return 0;
        ...
        if ((!((wip->wi_flags[Weapon::Info_Flags::Bomb]) ||
               (wip->wi_flags[Weapon::Info_Flags::Turret_Interceptable])) &&
             !(Ai_info[Ships[turret_parent->instance].ai_index]
                   .ai_profile_flags
                       [AI::Profile_Flags::
...
void evaluate_obj_as_target (object* objp, eval_enemy_obj_struct* eeo) {
    ...
    if (objp->type == OBJ_WEAPON && !eeo->weapon_system_ok) { return; }

    if (!valid_turret_enemy (objp, turret_parent_obj)) { return; }

#ifndef NDEBUG
    if (!Player_attacking_enabled && (objp == Player_obj)) { return; }
#endif
    ...
    if (objp->type == OBJ_SHIP) {
        ...
        // check on enemy team
        if (!iff_matches_mask (shipp->team, eeo->enemy_team_mask)) { return; }

        // check if protected
        if (objp->flags[Object::Object_Flags::Protected]) { return; }

        // check if beam protected
        if (eeo->eeo_flags & EEOF_BEAM) {
            if (objp->flags[Object::Object_Flags::Beam_protected]) { return; }
        }

        // check if flak protected
        if (eeo->eeo_flags & EEOF_FLAK) {
            if (objp->flags[Object::Object_Flags::Flak_protected]) { return; }
        }

        // check if laser protected
        if (eeo->eeo_flags & EEOF_LASER) {
            if (objp->flags[Object::Object_Flags::Laser_protected]) { return; }
        }
        ...
        // Note: retail behaviour was turrets with tagged-only could fire at
        // bombs and could fire their spawn weapons this check is almost
        // redundant; see the almost identical check in ai_fire_from_turret
        // however if this is removed turrets still track targets but don't
        // fire at them (which looks silly)
        ...
        if (!object_is_targetable (
                objp, &Ships[Objects[eeo->turret_parent_objnum].instance])) {
    ...
    if (optimum_range > 0.0f) {
        if (dist < optimum_range) { dist_comp = (2 * optimum_range) - dist; }
    }

    // if turret has been told to prefer targets from the current direction
    // then do so
    float favor_one_side = ss->favor_current_facing;
    if (favor_one_side >= 1.0f) {
        vm_vec_normalize (&vec_to_target);
        float dot_to_target =
            vm_vec_dot (&ss->turret_last_fire_direction, &vec_to_target);
    ...
    if ((objp->type == OBJ_WEAPON) && check_weapon) {
        ...
        if (Weapons[objp->instance].homing_object ==
            &Objects[eeo->turret_parent_objnum]) {
            if (dist_comp < eeo->nearest_homing_bomb_dist) {
                if (!(ss->flags[Ship::Subsystem_Flags::FOV_Required]) &&
                    (eeo->current_enemy == -1)) {
        ...
        else if (bomb_headed_towards_ship (
                     objp, &Objects[eeo->turret_parent_objnum])) {
    ...
    if ((shipp) && (dist < eeo->weapon_travel_dist)) {
        ...
        if (objp->flags[Object::Object_Flags::Player_ship]) {
            max_turrets = The_mission.ai_profile
                              ->max_turret_ownage_player[Game_skill_level];
        }
        // Apply the per-turret limit for small targets, if there is one and
        // this is a small target
        if (ss->turret_max_target_ownage != -1 &&
            (Ship_info[shipp->ship_info_index].is_small_ship ())) {
            max_turrets =
                MIN (max_turrets, ss->system_info->turret_max_target_ownage);
        }
        if (num_att_turrets > max_turrets) { return; }
        ...
        if (shipp->level2_tag_left > 0.0f) { dist_comp *= 0.3f; }

        // check if objp is targeting the turret's ship, or if objp has just
        // hit the turret's ship
        if (aip->target_objnum == eeo->turret_parent_objnum ||
            aip->last_objsig_hit ==
                Objects[eeo->turret_parent_objnum].signature) {
            // A turret will always target a ship that is attacking itself...
            // self-preservation!
...
int is_target_beam_valid (ship_weapon* swp, object* objp) {
    ...
    if (all_turret_weapons_have_flags (swp, Weapon::Info_Flags::Beam)) {
        if (objp->flags[Object::Object_Flags::Beam_protected]) { return 0; }
 ...
 * @param missile_flag
 */
int get_nearest_turret_objnum (
    int turret_parent_objnum, ship_subsys* turret_subsys, int enemy_team_mask,
    ...
    // weapon_travel_dist = MIN(wip->lifetime * wip->max_speed,
    // wip->weapon_range);
    ...
    if (ship_get_subsystem_strength (
            &Ships[Objects[turret_parent_objnum].instance],
            SUBSYSTEM_WEAPONS) > 0) {
        weapon_system_ok = 1;
    }
    ...
    // set flags
    eeo.eeo_flags = 0;
    if (big_only_flag) eeo.eeo_flags |= EEOF_BIG_ONLY;
    if (small_only_flag) eeo.eeo_flags |= EEOF_SMALL_ONLY;
    if (tagged_only_flag) eeo.eeo_flags |= EEOF_TAGGED_ONLY;
    ...
    if (n_tgt_priorities <= 0) {
        ...
        if (priority_weapon_idx > -1)
            n_tgt_priorities =
                Weapon_info[priority_weapon_idx].num_targeting_priorities;
    }
    ...
    if (n_tgt_priorities > 0) {
        for (int i = 0; i < n_tgt_priorities; i++) {
            ...
            if (priority_weapon_idx == -1)
                ...
                tt = &Ai_tp_list[Weapon_info[priority_weapon_idx]
                                     .targeting_priorities[i]];
            ...
            while (ptr != END_OF_LIST (&obj_used_list)) {
                ...
                if ((n_types > 0) && (ptr->type == OBJ_SHIP)) {
                    for (int j = 0; j < n_types; j++) {
                        if (Ship_info[Ships[ptr->instance].ship_info_index]
                                .class_type == tt->ship_type[j]) {
                            found_something = true;
                        }
                    }
                }
                ...
                if ((n_s_classes > 0) && (ptr->type == OBJ_SHIP)) {
                    for (int j = 0; j < n_s_classes; j++) {
                        if (Ships[ptr->instance].ship_info_index ==
                            tt->ship_class[j]) {
                            found_something = true;
                        }
                    }
                ...
                if ((n_w_classes > 0) && (ptr->type == OBJ_WEAPON)) {
                    for (int j = 0; j < n_w_classes; j++) {
                        if (Weapons[ptr->instance].weapon_info_index ==
                            tt->weapon_class[j]) {
                            found_something = true;
                        }
                    }
                ...
                if ((tt->wif_flags.any_set ()) && (ptr->type == OBJ_WEAPON)) {
                    if (((Weapon_info[Weapons[ptr->instance].weapon_info_index]
                              .wi_flags &
                ...
                if ((tt->sif_flags.any_set () && (ptr->type == OBJ_SHIP))) {
                    if ((Ship_info[Ships[ptr->instance].ship_info_index]
                             .flags &
                         tt->sif_flags) == tt->sif_flags) {
                        found_something = true;
                    }
                }

                if ((tt->obj_flags.any_set ()) &&
                    !((ptr->flags & tt->obj_flags) == tt->obj_flags)) {
                    found_something = true;
                }
            ...
            highest priority is an incoming homing bomb return
            eeo.nearest_homing_bomb_objnum;
                //weapon entry...
            } else if ( eeo.nearest_bomb_objnum != -1 ) {               // next
            highest priority is an incoming dumbfire bomb return
            eeo.nearest_bomb_objnum;
                //ship entry...
            } else if ( eeo.nearest_attacker_objnum != -1 ) {        // next
            highest priority is an attacking ship return
            eeo.nearest_attacker_objnum;
        ...
        const Weapon::Info_Flags weapon_flags[] = {
            Weapon::Info_Flags::Huge,
            Weapon::Info_Flags::Flak,
            Weapon::Info_Flags::Homing_aspect,
            Weapon::Info_Flags::Homing_heat,
            Weapon::Info_Flags::Homing_javelin,
            Weapon::Info_Flags::Spawn
        };
        tmp_flagset.set_multiple (
            std::begin (weapon_flags), std::end (weapon_flags));

        for (int i = 0; i < NUM_TURRET_ORDER_TYPES; i++) {
            ai_info* aip =
                &Ai_info[Ships[Objects[eeo.turret_parent_objnum].instance]
                             .ai_index];
            ...
            case 0:
                ...
                if (!((aip->ai_profile_flags
                    ...
                    else if (eeo.nearest_bomb_objnum != -1) { // next highest
                                                              // priority is an
                                                              // incoming
                                                              // dumbfire bomb
            ...
            case 1:
                ...
                if (eeo.nearest_attacker_objnum !=
                    -1) { // next highest priority is an attacking ship
            ...
            case 2:
                ...
                if (!all_turret_weapons_have_flags (swp, tmp_flagset)) {
                    ...
                    for (ao = GET_FIRST (&Asteroid_obj_list);
                         ao != END_OF_LIST (&Asteroid_obj_list);
                         ao = GET_NEXT (ao)) {
...
int find_turret_enemy (
    ship_subsys* turret_subsys, int objnum, vec3d* tpos, vec3d* tvec,
    int current_enemy) {
    ...
    bool big_only_flag = all_turret_weapons_have_flags (
        &turret_subsys->weapons, Weapon::Info_Flags::Huge);
    bool small_only_flag = all_turret_weapons_have_flags (
        &turret_subsys->weapons, Weapon::Info_Flags::Small_only);
    bool tagged_only_flag =
        all_turret_weapons_have_flags (
            &turret_subsys->weapons, Weapon::Info_Flags::Tagged_only) ||
        (turret_subsys->weapons.flags[Ship::Weapon_Flags::Tagged_Only]);

    bool beam_flag = turret_weapon_has_flags (
        &turret_subsys->weapons, Weapon::Info_Flags::Beam);
    bool flak_flag = turret_weapon_has_flags (
        &turret_subsys->weapons, Weapon::Info_Flags::Flak);
    bool laser_flag =
        turret_weapon_has_subtype (&turret_subsys->weapons, WP_LASER);
    bool missile_flag =
    ...
    if ((Ship_types[sip->class_type]
             .flags[Ship::Type_Info_Flags::Turret_tgt_ship_tgt]) &&
        (aip->target_objnum != -1)) {
        ...
        if ((frand () < 0.8f) && (aip->target_objnum != -1) &&
            ...
            bool skip = false;
            if (target_flags[Object::Object_Flags::Beam_protected] &&
                beam_flag)
                skip = true;
            else if (
                target_flags[Object::Object_Flags::Flak_protected] &&
                flak_flag)
                skip = true;
            else if (
                target_flags[Object::Object_Flags::Laser_protected] &&
                laser_flag)
                skip = true;
            else if (
                target_flags[Object::Object_Flags::Missile_protected] &&
                missile_flag)
                skip = true;
                ...
                if (Objects[aip->target_objnum].type == OBJ_SHIP) {
                    ...
                    if (!big_only_flag ||
                        ...
                        if (tagged_only_flag &&
                            ...
                            // select new target if aip->target_objnum is out
                            // of field of view
                            vec3d v2e;
                            float dist;
                            bool in_fov;
                            ...
                            in_fov =
                                turret_fov_test (turret_subsys, tvec, &v2e);

                            if (turret_subsys->flags
                                    [Ship::Subsystem_Flags::FOV_edge_check]) {
                                if (in_fov == false)
                                    if (object_in_turret_fov (
                                            &Objects[aip->target_objnum],
                                            turret_subsys, tvec, tpos,
                                            dist + Objects[aip->target_objnum]
                                                       .radius))
                                        in_fov = true;
                            }
                            // MODIFY FOR ATTACKING BIG SHIP
                            // dot += (0.5f *
                            // Objects[aip->target_objnum].radius / dist);
                            if (in_fov) { return aip->target_objnum; }
                        }
                    }
                }
            }
    ...
    enemy_objnum = get_nearest_turret_objnum (
        objnum, turret_subsys, enemy_team_mask, tpos, tvec, current_enemy,
 ...
 *
 * @note The gun normal is the unrotated gun normal, (the center of the FOV
 * cone), not the actual gun normal given using the current turret heading. But
 * it _is_ rotated into the model's orientation in global space.
 */
...
/**
 * Given an object and a turret on that object, return the actual firing point
 * of the gun and its normal.
 *
 * @note This uses the current turret angles.  We are keeping track of which
 * gun to fire next in the ship specific info for this turret subobject.  Use
 * this info to determine which position to fire from next.
 *
 * @param objp          Pointer to object
 ...
 */
void ship_get_global_turret_gun_info (
    ...
    gun_pos = &tp->turret_firing_point
                   [ssp->turret_next_fire_pos % tp->turret_num_firing_points];

    // model_find_world_point(gpos, gun_pos, tp->model_num,
    // tp->turret_gun_sobj, &objp->orient, &objp->pos );
    model_instance_find_world_point (
        gpos, gun_pos, Ships[objp->instance].model_instance_num,
        tp->turret_gun_sobj, &objp->orient, &objp->pos);

    if (use_angles) {
        model_instance_find_world_dir (
            gvec, &tp->turret_norm, Ships[objp->instance].model_instance_num,
            tp->turret_gun_sobj, &objp->orient);
    }
    else if (tp->flags[Model::Subsystem_Flags::Share_fire_direction]) {
        vec3d avg, tmp_pos, tmp_target, enemy_point, turret_norm;
        vm_vec_avg_n (
            &avg, tp->turret_num_firing_points, tp->turret_firing_point);

        model_instance_find_world_point (
            &tmp_pos, &avg, Ships[objp->instance].model_instance_num,
            tp->turret_gun_sobj, &objp->orient, &objp->pos);
        ...
        if (targetp == nullptr) {
            Assertion (
                ...
                "is invalid.",
                ssp->turret_enemy_objnum, ssp->sub_name, ssp->parent_objnum);
            ...
            if (best_weapon_tidx < 0) return;

            weapon_info* wip =
                get_turret_weapon_wip (&ssp->weapons, best_weapon_tidx);

            float weapon_system_strength = ship_get_subsystem_strength (
                &Ships[ssp->parent_objnum], SUBSYSTEM_WEAPONS);

            turret_ai_update_aim (
                &Ai_info[Ships[ssp->parent_objnum].ai_index],
                &Objects[ssp->turret_enemy_objnum], ssp);

            if ((ssp->targeted_subsys != nullptr) &&
                !(ssp->flags[Ship::Subsystem_Flags::No_SS_targeting])) {
                vm_vec_unrotate (
                    &enemy_point, &ssp->targeted_subsys->system_info->pnt,
                    &Objects[ssp->turret_enemy_objnum].orient);
                vm_vec_add2 (&enemy_point, &ssp->last_aim_enemy_pos);
            }
            else {
                if ((lep->type == OBJ_SHIP) &&
                    (Ship_info[Ships[lep->instance].ship_info_index]
                         .is_big_or_huge ())) {
                    vm_vec_unrotate (
                        &turret_norm, &tp->turret_norm, &objp->orient);
                    ai_big_pick_attack_point_turret (
                        lep, ssp, &tmp_pos, &turret_norm, &enemy_point,
                        MIN (
                            wip->max_speed * wip->lifetime, wip->weapon_range),
                        tp->turret_fov);
                }
                else {
            ...
            // Try to guess where the enemy will be, and store that spot in
            // predicted_enemy_pos
            if (The_mission.ai_profile
                    ->flags[AI::Profile_Flags::Use_additive_weapon_velocity]) {
                vm_vec_scale_sub2 (
                    &target_moving_direction, &objp->phys_info.vel,
                    wip->vel_inherit_amount);
            }

            set_predicted_enemy_pos_turret (
                &tmp_target, &tmp_pos, objp, &enemy_point,
                &target_moving_direction, wip->max_speed,
                ssp->turret_time_enemy_in_range *
                    (weapon_system_strength + 1.0f) / 2.0f);
        }
        else {
            tmp_target = *targetp;
        }
...
void turret_ai_update_aim (ai_info* aip, object* En_Objp, ship_subsys* ss) {
    if (Missiontime >= ss->next_aim_pos_time) {
        ...
        // change)
        vm_vec_scale_add2 (
            &ss->last_aim_enemy_pos, &ss->last_aim_enemy_vel, flFrametime);
    }
}
...
int aifft_rotate_turret (
    ...
    int ret_val __UNUSED =
        0; // to be used in future, see comment @ end of function
    ...
    if (ss->turret_enemy_objnum != -1) {
        ...
        // This turret doesn't have any good weapons
        if (best_weapon_tidx < 0) return 0;
        ...
        ship_get_global_turret_info (
            &Objects[parent_objnum], tp, &gun_pos, &gun_vec);

        // Update "known" position and velocity of target. Only matters if
        // max_aim_update_delay is set.
        turret_ai_update_aim (
            &Ai_info[shipp->ai_index], &Objects[ss->turret_enemy_objnum], ss);

        // Figure out what point on the ship we want to point the gun at, and
        // store the global location in enemy_point.
        vec3d enemy_point;
        if ((ss->targeted_subsys != NULL) &&
            !(ss->flags[Ship::Subsystem_Flags::No_SS_targeting])) {
            if (ss->turret_enemy_objnum != -1) {
                vm_vec_unrotate (
                    &enemy_point, &ss->targeted_subsys->system_info->pnt,
                    &Objects[ss->turret_enemy_objnum].orient);
            ...
            if ((lep->type == OBJ_SHIP) &&
                (Ship_info[Ships[lep->instance].ship_info_index]
                     .is_big_or_huge ())) {
                ai_big_pick_attack_point_turret (
                    lep, ss, &gun_pos, &gun_vec, &enemy_point, tp->turret_fov,
        ...
        set_predicted_enemy_pos_turret (
            predicted_enemy_pos, &gun_pos, objp, &enemy_point,
            &target_moving_direction, wip->max_speed,
            ss->turret_time_enemy_in_range * (weapon_system_strength + 1.0f) /
                2.0f);
        ...
        if (ss->flags[Ship::Subsystem_Flags::FOV_edge_check]) {
            if (in_fov == false)
                in_fov = is_object_radius_in_turret_fov (
                    &Objects[ss->turret_enemy_objnum], ss, gvec, &gun_pos,
                    &v2e, predicted_enemy_pos, 0.0f);
        }
        ...
        if (in_fov) {
            ret_val = model_rotate_gun (
                ...
                predicted_enemy_pos, shipp->objnum);
        }
        else if (
            ...
            ret_val = model_rotate_gun (
                ...
                &Objects[parent_objnum].orient, &ss->submodel_info_1.angs,
                &ss->submodel_info_2.angs, &Objects[parent_objnum].pos,
                predicted_enemy_pos, shipp->objnum, true);
        }
    }
    else if (
    ...
    // by default "ret_val" should be set to 1 for multi-part turrets, and 0
    // for single-part turrets but we need to keep retail behavior by default,
    // which means always returning 0 unless a special flag is used.  the
    // "ret_val" stuff is here is needed/wanted at a later date however.
    // return ret_val;

    // return 0 by default (to preserve retail behavior) but allow for a
    // per-subsystem option for using the turret normals for firing
...
float aifft_compute_turret_dot (
    object* objp, object* enemy_objp, vec3d* abs_gunposp,
    ship_subsys* turret_subsysp, ship_subsys* enemy_subsysp) {
    float dot_out;
    vec3d subobj_pos, vector_out;

    vm_vec_unrotate (
        &subobj_pos, &enemy_subsysp->system_info->pnt, &enemy_objp->orient);
    vm_vec_add2 (&subobj_pos, &enemy_objp->pos);

    if (ship_subsystem_in_sight (
            enemy_objp, enemy_subsysp, abs_gunposp, &subobj_pos, 1, &dot_out,
            &vector_out)) {
        vec3d turret_norm;

        vm_vec_unrotate (
            &turret_norm, &turret_subsysp->system_info->turret_norm,
            &objp->orient);
        float dot_return = vm_vec_dot (&turret_norm, &vector_out);

        if (Ai_info[Ships[objp->instance].ai_index].ai_profile_flags
            ...
            // target is in sight and we don't care if its in turret's fov or
            // not
            return dot_return;
        }
    }
...
DCF (
    mf,
    "Adjusts the maximum number of tries an AI may do when trying to pick a "
    "subsystem to attack (Default is 5)") {
 ...
 */
ship_subsys* aifft_find_turret_subsys (
    object* objp, ship_subsys* ssp, object* enemy_objp, float* dot_out) {
    ship *eshipp, *shipp;
    ship_info* esip;
    ...
    *dot_out = best_dot;
    ...
    if (esip->n_subsystems == 0) { return best_subsysp; }

    // first build up a list subsystems to traverse
    ship_subsys* pss;
    aifft_list_size = 0;
    for (pss = GET_FIRST (&eshipp->subsys_list);
         pss != END_OF_LIST (&eshipp->subsys_list); pss = GET_NEXT (pss)) {
        model_subsystem* psub = pss->system_info;

        // if we've reached max turrets bail
        if (aifft_list_size >= MAX_AIFFT_TURRETS) { break; }

        // Don't process destroyed objects
        if (pss->current_hits <= 0.0f) { continue; }
    ...
    if (aifft_list_size == 0) { return best_subsysp; }

    // determine a stride value so we're not checking too many turrets
    int stride = aifft_list_size > aifft_max_checks
    ...
    for (idx = offset; idx < aifft_list_size; idx += stride) {
        dot = aifft_compute_turret_dot (
            objp, enemy_objp, &abs_gun_pos, ssp, aifft_list[idx]);
...
/**
 * @return !0 if the specified target should scan for a new target, otherwise
 * return 0
 */
...
void turret_set_next_fire_timestamp (
    ...
    if (wip->burst_shots > turret->weapons.burst_counter[weapon_num]) {
        ...
        if ((wip->burst_shots > 0) &&
            (wip->burst_flags[Weapon::Burst_Flags::Random_length])) {
            turret->weapons.burst_counter[weapon_num] =
                (myrand () % wip->burst_shots);
        }
        else {
    ...
    if (weapon_num < MAX_SHIP_PRIMARY_BANKS)
        ...
        fs_dest = &turret->weapons.next_secondary_fire_stamp
                       [weapon_num - MAX_SHIP_PRIMARY_BANKS];
 ...
 */
int turret_should_fire_aspect (
    ship_subsys* turret, weapon_info* wip, bool in_sight) {
    if (in_sight &&
        (turret->turret_time_enemy_in_range >=
         MIN (wip->min_lock_time, AICODE_TURRET_MAX_TIME_IN_RANGE))) {
        return 1;
    }
 ...
 */
bool turret_fire_weapon (
    int weapon_num, ship_subsys* turret, int parent_objnum, vec3d* turret_pos,
    vec3d* turret_fvec, vec3d* predicted_pos = NULL,
    ...
    if (turret->system_info->flags[Model::Subsystem_Flags::Turret_salvo]) {
        if ((turret->turret_next_fire_pos + 1) ==
            (turret->system_info->turret_num_firing_points)) {
    ...
    if ((!timestamp_elapsed (get_turret_weapon_next_fire_stamp (
            &turret->weapons, weapon_num))) &&
        last_shot_in_salvo)
        return false;
    ...
    if (turret->turret_enemy_objnum >= 0) {
        ...
        // should not get this far. check if ship is protected from beam and
        // weapon is type beam
    ...
    if (check_ok_to_fire (parent_objnum, turret->turret_enemy_objnum, wip)) {
        ...
        if (last_shot_in_salvo)
            turret_set_next_fire_timestamp (
                weapon_num, wip, turret, parent_aip);
        ...
        if (wip->wi_flags[Weapon::Info_Flags::Beam]) {
            ...
            turret->flags.set (
                Ship::Subsystem_Flags::Has_fired); // set fired flag for
                                                   // scripting -nike
            return true;
        }
        // don't fire swam, but set up swarm info instead
        else if (
            (wip->wi_flags[Weapon::Info_Flags::Swarm]) ||
            ...
            if ((turret->system_info
                     ->flags[Model::Subsystem_Flags::Turret_use_ammo]) &&
                (swp->secondary_bank_ammo[bank_to_fire] < 0)) {
                if (!(turret->system_info->flags
                          [Model::Subsystem_Flags::Use_multiple_guns])) {
                    swp->current_secondary_bank++;
                    if (swp->current_secondary_bank >=
                ...
                turret_swarm_set_up_info (
                    parent_objnum, turret, wip, turret->turret_next_fire_pos);
        ...
        // now do anything else
            ...
            for (int i = 0; i < wip->shots; i++) {
                if (turret->system_info
                    ...
                    if (wip->subtype == WP_LASER) {
                        int points;
                        if (swp->num_primary_banks <= 0) { return false; }
                        ...
                        if (swp->num_secondary_banks <= 0) { return false; }
                        ...
                        for (int j = start_slot; j <= end_slot; j++) {
                            ...
                            if (swp->secondary_next_slot[bank_to_fire] >
                                (num_slots - 1)) {
                ...
                ship_get_global_turret_gun_info (
                    &Objects[parent_objnum], turret, turret_pos, turret_fvec,
                    1, NULL);

                weapon_objnum = weapon_create (
                    turret_pos, &turret_orient, turret_weapon_class,
                    parent_objnum, -1, 1, 0, 0.0f, turret);
                ...
                if (weapon_objnum != -1) {
                    ...
                    parent_ship->last_fired_turret = turret;
                    turret->last_fired_weapon_info_index =
                    ...
                    if (wip->wi_flags[Weapon::Info_Flags::Flak]) {
                        ...
                        if (predicted_pos != NULL) {
                            // pick a firing range so that it detonates
                            // properly
                            flak_pick_range (
                    ...
                    if (wip->launch_snd.isValid ()) {
                        // Don't play turret firing sound if turret sits on
                        // player ship... it gets annoying.
                        if (parent_objnum != OBJ_INDEX (Player_obj) ||
                            (turret->flags[Ship::Subsystem_Flags::
                                               Play_sound_for_player])) {
                            snd_play_3d (
                                gamesnd_get_game_sound (wip->launch_snd),
            ...
            // (I'm not sure how accurate this timestamp would be in practice -
            // taylor)
    ...
    model_anim_start_type (
        turret, TRIGGER_TYPE_TURRET_FIRED, ANIMATION_SUBTYPE_ALL, 1);
    return true;
}

// void turret_swarm_fire_from_turret(ship_subsys *turret, int parent_objnum,
// int target_objnum, ship_subsys *target_subsys)
void turret_swarm_fire_from_turret (turret_swarm_info* tsi) {
    ...
    // make turret_orient from turret_fvec --
    // turret->turret_last_fire_direction
    vm_vector_2_matrix (&turret_orient, &turret_fvec, NULL, NULL);

    // create weapon and homing info
    weapon_objnum = weapon_create (
        &turret_pos, &turret_orient, tsi->weapon_class, tsi->parent_objnum, -1,
        1, 0, 0.0f, tsi->turret);
    weapon_set_tracking_info (
        weapon_objnum, tsi->parent_objnum, tsi->target_objnum, 1,
        tsi->target_subsys);

    // do other cool stuff if weapon is created.
    if (weapon_objnum > -1) {
        Weapons[Objects[weapon_objnum].instance].turret_subsys = tsi->turret;
        Weapons[Objects[weapon_objnum].instance].target_num =
            tsi->turret->turret_enemy_objnum;
        ...
        if (Weapon_info[tsi->weapon_class].muzzle_flash >= 0) {
            mflash_create (
                &turret_pos, &turret_fvec,
                &Objects[tsi->parent_objnum].phys_info,
                Weapon_info[tsi->weapon_class].muzzle_flash);
        }

        // maybe sound
        if (Weapon_info[tsi->weapon_class].launch_snd.isValid ()) {
            // Don't play turret firing sound if turret sits on player ship...
            // it gets annoying.
...
/**
 * Given a turret tp and its parent parent_objnum, fire from the turret at its
 * enemy.
 */
...
void ai_fire_from_turret (ship* shipp, ship_subsys* ss, int parent_objnum) {
    ...
    if (!Ai_firing_enabled) { return; }

    if (ss->current_hits <= 0.0f) { return; }

    if (ship_subsys_disrupted (ss)) { // AL 1/19/98: Make sure turret isn't
                                      // suffering disruption effects
        return;
    }

    // Check turret free
    if (ss->weapons.flags[Ship::Weapon_Flags::Turret_Lock]) { return; }

    // check if there is any available weapon to fire
    int weap_check;
    bool valid_weap = false;
    for (weap_check = 0; (weap_check < swp->num_primary_banks) && !valid_weap;
         weap_check++) {
        if (swp->primary_bank_weapons[weap_check] >= 0) valid_weap = true;
    }

    if (!valid_weap) {
        for (weap_check = 0;
             (weap_check < ss->weapons.num_secondary_banks) && !valid_weap;
             weap_check++) {
            if (ss->weapons.secondary_bank_weapons[weap_check] >= 0)
                valid_weap = true;
        }

        if (!valid_weap) return;
    }

    // If beam weapon, check beam free
    if (all_turret_weapons_have_flags (swp, Weapon::Info_Flags::Beam) &&
        !(swp->flags[Ship::Weapon_Flags::Beam_Free])) {
        return;
    }
    ...
    if (ss->turret_animation_position == MA_POS_SET) {
        if (timestamp_elapsed (ss->turret_animation_done_time)) {
            ...
            // (NOTE: this will probably get changed by other parts of the code
            // (swarming, beams, etc) to be a more accurate time, but we need
            // to give it a long enough time for the other parts of the code to
            // change the timestamp before it gets acted upon - taylor)
    ...
    if ((ss->turret_enemy_objnum < 0 ||
         ss->turret_enemy_objnum >= MAX_OBJECTS) ||
        (ss->turret_enemy_sig != Objects[ss->turret_enemy_objnum].signature)) {
    ...
    if (tp->flags[Model::Subsystem_Flags::Turret_alt_math]) {
        vm_matrix_x_matrix (
            &ss->world_to_turret_matrix, &Objects[parent_objnum].orient,
            &tp->turret_matrix);
    }

    // Rotate the turret even if time hasn't elapsed, since it needs to turn to
    // face its target.
    int use_angles = aifft_rotate_turret (
        shipp, parent_objnum, ss, objp, lep, &predicted_enemy_pos, &gvec);

    if ((tp->flags[Model::Subsystem_Flags::Fire_on_target]) &&
        (ss->points_to_target >= 0.0f)) {
        // value probably needs tweaking... could perhaps be made into table
        // option?
        if (ss->points_to_target > 0.010f) return;
    }

    if (!timestamp_elapsed (ss->turret_next_fire_stamp)) { return; }

    // Don't try to fire beyond weapon_limit_range
    // WMC - OTC
    // weapon_firing_range = MIN(Weapon_info[tp->turret_weapon_type].lifetime *
    // Weapon_info[tp->turret_weapon_type].max_speed,
    // Weapon_info[tp->turret_weapon_type].weapon_range);

    // WMC - build a list of valid weapons. Fire spawns if there are any.
    float dist_to_enemy = 0.0f;
    if (lep != NULL) {
        if (The_mission.ai_profile
                ->flags[AI::Profile_Flags::Turrets_ignore_target_radius])
            dist_to_enemy = MAX (
                0, vm_vec_normalized_dir (&v2e, &predicted_enemy_pos, &gpos));
        else
            dist_to_enemy = MAX (
                0, vm_vec_normalized_dir (&v2e, &predicted_enemy_pos, &gpos) -
                       lep->radius);
    ...
    weapon_info* wip;
    int num_ships_nearby = num_nearby_fighters (
        iff_get_attackee_mask (obj_team (&Objects[parent_objnum])), &gpos,
        1500.0f);
    int i;
    int secnum = 0;
    for (i = 0; i < (MAX_SHIP_WEAPONS); i++) {
        ...
        if (num_valid > 0 &&
            !(tp->flags[Model::Subsystem_Flags::Use_multiple_guns]))
            break;

        if (i < MAX_SHIP_PRIMARY_BANKS) {
            if (i >= swp->num_primary_banks) {
                // set to MAX_SHIP_PRIMARY_BANKS - 1 since i is incremented at
                // the top of the loop otherwise, we would pass over the first
                // secondary weapon in the bank
            ...
            if (secnum >= swp->num_secondary_banks) break; // we are done.
        ...
        if (lep != NULL) {
            if ((dist_to_enemy <= weapon_firing_range) &&
                (dist_to_enemy >= WeaponMinRange)) {
                if (wip->wi_flags[Weapon::Info_Flags::Huge]) {
                    if (lep->type != OBJ_SHIP) { continue; }
                    if (!(Ship_info[Ships[lep->instance].ship_info_index]
                              .is_big_or_huge ())) {
                        continue;
                    }
                }
                ...
                bool tagged_only =
                    ((wip->wi_flags[Weapon::Info_Flags::Tagged_only]) ||
                     (ss->weapons.flags[Ship::Weapon_Flags::Tagged_Only]));
                ...
                if (lep->type == OBJ_SHIP) {
                    ...
                    // Check if we're targeting a beam protected ship with a
                    // beam weapon
                    if ((lep->flags[Object::Object_Flags::Beam_protected]) &&
                        (wip->wi_flags[Weapon::Info_Flags::Beam])) {
                        ss->turret_enemy_objnum = -1;
                        ss->turret_time_enemy_in_range = 0.0f;
                        continue;
                    }
                    // Check if we're targeting a flak protected ship with a
                    // flak weapon
                    else if (
                        (lep->flags[Object::Object_Flags::Flak_protected]) &&
                        (wip->wi_flags[Weapon::Info_Flags::Flak])) {
                        ss->turret_enemy_objnum = -1;
                        ss->turret_time_enemy_in_range = 0.0f;
                        continue;
                    }
                    // Check if we're targeting a laser protected ship with a
                    // laser weapon
                    else if (
                        (lep->flags[Object::Object_Flags::Laser_protected]) &&
                        (wip->subtype == WP_LASER)) {
                        ss->turret_enemy_objnum = -1;
                        ss->turret_time_enemy_in_range = 0.0f;
                        continue;
                    }
                    // Check if we're targeting a missile protected ship with a
                    // missile weapon
                    else if (
                        (lep->flags
                    ...
                    // must check here in case turret has multiple weapons and
                    // not all are tagged-only
    ...
    if (turret_should_pick_new_target (ss) && !ss->scripting_target_override) {
        ...
        int objnum = find_turret_enemy (
            ss, parent_objnum, &gpos, &gvec, ss->turret_enemy_objnum);
        // Assert(objnum < 0 || is_target_beam_valid(tp, objnum));

        if (objnum >= 0 &&
            is_target_beam_valid (&ss->weapons, &Objects[objnum])) {
            if (ss->turret_enemy_objnum == -1) {
                ss->turret_enemy_objnum = objnum;
                ss->targeted_subsys =
                    NULL; // Turret has retargeted; reset subsystem - Valathil
                          // for Mantis #2652
        ...
        if (ss->turret_enemy_objnum != -1) {
            ...
            if ((lep->type == OBJ_SHIP) &&
                !(ss->flags[Ship::Subsystem_Flags::No_SS_targeting])) {
                ss->targeted_subsys =
                    aifft_find_turret_subsys (objp, ss, lep, &dot);
            }
            ss->turret_next_enemy_check_stamp =
                timestamp ((int)(MAX (dot, 0.5f) * 2000.0f) + 1000);
        }
        else {
    ...
    if (ss->turret_enemy_objnum != -1) {
        ...
        if (dock_check_find_docked_object (
                objp, &Objects[ss->turret_enemy_objnum])) {
            ss->turret_enemy_objnum = -1;
            return;
        }

        // Goober5000 - Also, don't shoot at a ship we're docking with.
        // Volition had this in the code originally but messed it up so that it
        // didn't work.
        ai_info* aip = &Ai_info[shipp->ai_index];
        if ((aip->mode == AIM_DOCK) &&
            (aip->goal_objnum == ss->turret_enemy_objnum)) {
            ss->turret_enemy_objnum = -1;
            return;
        }
    ...
    if (in_fov) {
        ...
        for (i = 0; i < number_of_firings; i++) {
            if (tp->flags[Model::Subsystem_Flags::Turret_fixed_fp]) {
                ...
                if (ffp_bank < MAX_SHIP_PRIMARY_BANKS) { ffp_pos = ffp_bank; }
                else {
                    ffp_pos = ffp_bank - MAX_SHIP_PRIMARY_BANKS +
                              ss->weapons.num_primary_banks;
                }
            ...
            ship_get_global_turret_gun_info (
                &Objects[parent_objnum], ss, &gpos, &gvec, use_angles,
                &predicted_enemy_pos);

            // Fire in the direction the turret is facing, not right at the
            // target regardless of turret dir.
            ...
            // We're ready to fire... now get down to specifics, like where is
            // the actual gun point and normal, not just the one for whole
            // turret. moved here as if there are two weapons with indentical
            // fire stamps they would have shared the fire point.
            tv2e = gvec;

            // make sure to reset this for current weapon
            ok_to_fire = false;

            // if the weapon is a flak gun, add some jitter to its aim so it
            // fires in a "cone" to make a cool visual effect and make them
            // less lethal
            if (wip->wi_flags[Weapon::Info_Flags::Flak]) {
                flak_jitter_aim (
                    &tv2e, dist_to_enemy,
            ...
            if (The_mission.ai_profile->flags
                    [AI::Profile_Flags::Use_only_single_fov_for_turrets]) {
            ...
            if (!(wip->is_homing ())) {
            ...
            else if (wip->wi_flags[Weapon::Info_Flags::
                                       Homing_heat]) { // if heat seekers
                if ((dist_to_enemy < 50.0f) || in_sight) {
                    ...
                    ok_to_fire = true;
                                       ...
                                       Homing_aspect]) { // if aspect seeker
                if ((dist_to_enemy < 50.0f) || in_sight) {
                    turret_update_enemy_in_range (ss, 2 * wip->fire_wait);
                }
            ...
            else if (wip->wi_flags
                         [Weapon::Info_Flags::Homing_javelin]) { // if javelin
                                                                 // heat seeker
                if ((dist_to_enemy < 50.0f) || in_sight) {
                    turret_update_enemy_in_range (ss, 2 * wip->fire_wait);
                }
                ...
                if (turret_should_fire_aspect (ss, wip, in_sight) &&
                    ship_get_closest_subsys_in_sight (
                        &Ships[lep->signature], SUBSYSTEM_ENGINE, &gpos)) {
                    ok_to_fire = true;
                }
            }

            if (ok_to_fire &&
                (tp->flags[Model::Subsystem_Flags::Turret_hull_check])) {
                int model_num = Ship_info[shipp->ship_info_index].model_num;
                vec3d end;
                vm_vec_scale_add (
                    &end, &gpos, &gvec, model_get_radius (model_num));
                ...
                if (model_collide (&hull_check)) { ok_to_fire = false; }
            }
            ...
            if (ok_to_fire) {
                ...
                if (ss->turret_animation_position == MA_POS_NOT_SET) {
                    if (model_anim_start_type (
                            ...
                            ss->system_info->subobj_num, 1)) {
                        ss->turret_animation_done_time =
                            model_anim_get_time_type (
                                shipp, TRIGGER_TYPE_TURRET_FIRING,
                                ss->system_info->subobj_num);
                        ss->turret_animation_position = MA_POS_SET;
                    }
                }

                // Wait until the animation is done to actually fire
                if (tp->flags[Model::Subsystem_Flags::Turret_anim_wait] &&
                    (ss->turret_animation_position != MA_POS_READY)) {
                    ok_to_fire = false;
                }
            }
            ...
            if (ok_to_fire) {
                ...
                if (tp->flags[Model::Subsystem_Flags::Turret_salvo])
                    turret_fire_weapon (
                        valid_weapons[0], ss, parent_objnum, &gpos, &tv2e,
                        &predicted_enemy_pos);
                else
                    turret_fire_weapon (
                        valid_weapons[i], ss, parent_objnum, &gpos, &tv2e,
                        &predicted_enemy_pos);
            }
            else {
                // make sure salvo fire mode does not turn into autofire
                if ((tp->flags[Model::Subsystem_Flags::Turret_salvo]) &&
                    ((i + 1) == number_of_firings)) {
                    ai_info* parent_aip =
                        &Ai_info[Ships[Objects[parent_objnum].instance]
                                     .ai_index];
                    turret_set_next_fire_timestamp (
                        valid_weapons[0], wip, ss, parent_aip);
                }
            }
        ...
        if (!something_was_ok_to_fire) {
            ...
            if (ss->turret_best_weapon >= 0) {
                ...
                // goal, or just not being able to fire.
                turret_update_enemy_in_range (
                    ss, -4 * Weapon_info[ss->turret_best_weapon].fire_wait);
            }
            ss->turret_next_fire_stamp = timestamp (500);

            // If nothing is OK to fire (lost track of the target?)
            // reset the target (so we don't continue to track what we can't
            // hit)
            if (tp->flags
            ...
            for (i = 0; i < MAX_SHIP_WEAPONS; i++) {
                if (i < MAX_SHIP_PRIMARY_BANKS) {
                    ...
                    if (swp->next_primary_fire_stamp[i] <
                        ss->turret_next_fire_stamp) {
                        ss->turret_next_fire_stamp =
                            swp->next_primary_fire_stamp[i];
                    }
                }
                else {
                    // timestamp range is 2 to INT_MAX/2
                    if (swp->next_secondary_fire_stamp
                            [i - MAX_SHIP_PRIMARY_BANKS] < 2)
                        continue;

                    if (swp->next_secondary_fire_stamp
                            [i - MAX_SHIP_PRIMARY_BANKS] <
                        ss->turret_next_fire_stamp) {
                        ss->turret_next_fire_stamp =
            ...
            // if timestamp is still at int_max reset the wait to 100 ms
            // instead of locking the turret for the rest of the game
...
bool turret_std_fov_test (
    ship_subsys* ss, vec3d* gvec, vec3d* v2e, float size_mod) {
    model_subsystem* tp = ss->system_info;
    float dot = vm_vec_dot (v2e, gvec);
    if (((dot + size_mod) >= tp->turret_fov) &&
        ((dot - size_mod) <= tp->turret_max_fov))
        return true;

    return false;
}

bool turret_adv_fov_test (
    ship_subsys* ss, vec3d* gvec, vec3d* v2e, float size_mod) {
    model_subsystem* tp = ss->system_info;
    float dot = vm_vec_dot (v2e, gvec);
    if (((dot + size_mod) >= tp->turret_fov) &&
        ...
        if ((of_dst.xyz.x == 0) && (of_dst.xyz.y == 0)) { return true; }
        else {
            of_dst.xyz.z = 0;
            if (!IS_VEC_NULL_SQ_SAFE (&of_dst)) {
                vm_vec_normalize (&of_dst);
                // now we have 2d vector with lenght of 1 that points at the
                // targets direction after being rotated to turrets FOR
