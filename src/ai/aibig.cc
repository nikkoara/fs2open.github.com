        ...
        float weapon_travel_dist, vec3d *surface_normal, ship_subsys *ss)
{
        ...
        if (pm->octants[octs[0]].verts) {
                *local_attack_point = *pm->octants[octs[0]]
                                ...
                                if (in_fov) {
                                        if (dist < nearest_dist) {
                                                ...
                                                *local_attack_point = rel_point;
                                                ...
                                                if (dot > (1.0f + fov) / 2.0f) // If this point is quite good,
        ...
        vec3d *attack_point, float fov, float weapon_travel_dist)
{
        if (!timestamp_elapsed(ssp->turret_pick_big_attack_point_timestamp)) {
                ...
                ssp->turret_pick_big_attack_point_timestamp = timestamp(2000 + (int)(fs2::prng::randf(0) * 500.0f));
                ai_bpap(
        ...
        object *objp, object *attacker_objp, vec3d *attack_point, float fov)
{
        ...
        case OBJ_SHIP: {
                ...
                attacker_aip->pick_big_attack_point_timestamp = timestamp(2000 + (int)(fs2::prng::randf(0) * 500.0f));
                break;
        }
        case OBJ_WEAPON: {
                ...
                wp->pick_big_attack_point_timestamp = timestamp(2000 + (int)(fs2::prng::randf(0) * 500.0f));
...
void ai_big_evade_ship()
{
        ...
        if (box_dist > EVADE_BOX_BASE_DISTANCE) {
                ...
                (box_dist > EVADE_BOX_MIN_DISTANCE) && (Missiontime - aip->submode_start_time > i2f(5))) {
                aip->submode = SM_ATTACK;
                aip->submode_start_time = Missiontime;
        }
        ...
        if (((time_to_hit = ai_endangered_by_weapon(aip)) < 4.0f) && (time_to_hit >= 0.0f)) {
                ...
                if (Pl_objp->phys_info.speed < 3.0f) {
                        ...
                        if (ai_big_maybe_start_strafe(aip, sip)) {
                                aip->previous_mode = aip->mode;
                                aip->mode = AIM_STRAFE;
                                aip->submode_parm0 = Missiontime; // use parm0 as time strafe mode entered (i.e.
                                                                  // MODE start time)
                ...
                if ((sip->can_glide == true) && !(aip->ai_flags[AI::AI_Flags::Kamikaze]) && fs2::prng::randf(0) < aip->ai_glide_strafe_percent) {
                        aip->previous_mode = aip->mode;
                        aip->mode = AIM_STRAFE;
                ...
                if (aip->targeted_subsys != NULL) {
                        if (dist_to_enemy > (weapon_travel_dist - 20)) {
                                ...
                                if ((aip->ai_flags[AI::AI_Flags::Free_afterburner_use]) && !(shipp->flags[Ship::Ship_Flags::Afterburner_locked]) && (dot_to_enemy > 0.75f)) {
                                        ...
                                        (Pl_objp->phys_info.flags & PF_AFTERBURNER_ON) && !(aip->ai_flags[AI::AI_Flags::Kamikaze])) {
                                        afterburners_stop(Pl_objp);
                                }
                        } else {
                                ...
                                accelerate_ship(
                                        aip, MAX(
                                                     1.0f - dot_to_enemy,
                                                     Objects[aip->target_objnum].phys_info.fspeed / sip->max_speed));
        ...
        if (dot_to_enemy > 0.95f - 0.5f * En_objp->radius / MAX(1.0f, En_objp->radius + dist_to_enemy)) {
                ...
                if (dot_to_enemy > MAX(
                        ...
                        if (tswp->num_primary_banks > 0) {
                                ...
                                weapon_info *wip = &Weapon_info[tswp->primary_bank_weapons
                                                                        [tswp->current_primary_bank]];

                                if (dist_to_enemy < MIN(
                        ...
                        if (tswp->num_secondary_banks > 0) {
                                if (!(En_objp->flags[Object::Object_Flags::Protected]) || (aip->goals[0].ai_mode & (AI_GOAL_DISABLE_SHIP | AI_GOAL_DISARM_SHIP))) {
                                        ...
                                        if (current_bank > -1) {
                                                weapon_info *swip = &Weapon_info
                                                                            [tswp->secondary_bank_weapons[current_bank]];

                                                if (!(En_objp
                                                              ->flags[Object::Object_Flags::Protected])
                                                    || ((aip->goals[0].ai_mode & (AI_GOAL_DISABLE_SHIP | AI_GOAL_DISARM_SHIP)) && swip->wi_flags[Weapon::Info_Flags::Puncture])) { // override lockdown on
        ...
        case SM_SUPER_ATTACK:
                ...
                if (aip->target_time < 2.0f)
                        if ((dot_to_enemy < 0.9f) || (dist_to_enemy > 300.0f)) {
                                ...
                                aip->submode_start_time = Missiontime - fl2f(2.75f); // This backdated start time
        ...
        if (aip->submode != SM_AVOID && aip->submode != SM_EVADE) {
                ...
                if (vm_vec_mag_squared(&aip->big_attack_surface_normal) > 0.9) {
                        dist_normal_to_enemy = fabsf(
                                (dist_to_enemy * vm_vec_dot(&vec_to_enemy, &aip->big_attack_surface_normal)));
                } else {
                ...
                if (dist_to_enemy < ATTACK_COLLIDE_BASE_DIST) {
                        ...
                        if (vm_vec_mag_squared(&aip->big_attack_surface_normal) > 0.9) {
                                if (Pl_objp->phys_info.speed > 0.1) {
                                        time_to_enemy = dist_normal_to_enemy / fabsf(vm_vec_dot(&Pl_objp->phys_info.vel, &aip->big_attack_surface_normal));
                                } else {
                                        // a big time
                        ...
                        if ((dist_normal_to_enemy < ATTACK_COLLIDE_AVOID_DIST + speed_dist) || (time_to_enemy < ATTACK_COLLIDE_AVOID_TIME)) {
                                ...
                                (dist_normal_to_enemy < ATTACK_COLLIDE_SLOW_DIST) || (time_to_enemy < ATTACK_COLLIDE_SLOW_TIME)) {
                                // slow down
                                accelerate_ship(aip, -1.0f);
                        }
                }
...
void ai_big_strafe_glide_attack()
{
        ...
        if (aip->submode_parm1 == 2) {
                ...
                if (target_ship_dist > (STRAFE_RETREAT_BOX_DIST + target_objp->radius) && Missiontime - aip->submode_start_time > i2f(GLIDE_STRAFE_MIN_TIME)) {
...
void ai_big_strafe()
{
    ...
    */
        ...
        if (aip->submode == AIS_STRAFE_ATTACK || aip->submode == AIS_STRAFE_AVOID || aip->submode == AIS_STRAFE_RETREAT1 || aip->submode == AIS_STRAFE_RETREAT2 || aip->submode == AIS_STRAFE_POSITION) {
        ...
        const object *ship_objp, const object *weapon_objp)
{
        ...
        // ship.
        if ((fs2::prng::randf(0) * 100) > (aip->ai_courage - 15))
                return;
