...
void ai_mission_wing_goal_complete(int wingnum, ai_goal *remove_goalp)
{
        ...
        for (i = 0; i < wingp->current_count; i++) {
                ...
                for (j = 0; j < MAX_AI_GOALS; j++) {
                        ...
                        if ((aigp->ai_mode == mode) && (aigp->ai_submode == submode) && (aigp->priority == priority) && !strcasecmp(name, aigp->target_name)) {
        ...
        for (i = 0; i < MAX_AI_GOALS; purge_goal++, i++) {
                ...
                if (purge_ai_mode == AI_GOAL_CHASE_SHIP_CLASS) {
                        ...
                        if (strcasecmp(
                                    ...
                                    Ship_info[Ships[ship_index].ship_info_index].name)
                            != 0)
                                continue;
                }
                // standard goals operating on either wings or ships
                        ...
                        if (purge_wing == -1) {
                                if (strcasecmp(purge_goal->target_name, name) != 0)
                                        continue;
                ...
                case AI_GOAL_DISABLE_SHIP:
                        if (purge_ai_mode & (AI_GOAL_CHASE | AI_GOAL_CHASE_WING | AI_GOAL_CHASE_SHIP_CLASS)) {
                                ...
                                if (ai_wingnum >= 0) {
                                        ai_ship_type = Ship_info[Ships[Wings[ai_wingnum].special_ship]
                                                                         .ship_info_index]
                                                               .class_type;
                                }
                                // otherwise we simply grab it from the ship itself
                                        ...
                                        ASSERT(aip != NULL);
                                        ai_ship_type = Ship_info[Ships[aip->shipnum].ship_info_index]
                                                               .class_type;
                                }

                                // grab the ship type of the ship that is being
                                // disarmed/disabled
                                ship_type_info *crippled_ships_type = &Ship_types[Ship_info[Ships[ship_index].ship_info_index]
                                                                                          .class_type];
...
int ai_find_goal_index(ai_goal *aigp, int mode, int submode, int priority)
{
        ...
        for (int i = 0; i < MAX_AI_GOALS; i++) {
                if (aigp[i].ai_mode == mode && (submode == -1 || aigp[i].ai_submode == submode) && (priority == -1 || aigp[i].priority == priority)) {
                        return i;
                }
        }
...
int ai_mission_goal_achievable(int objnum, ai_goal *aigp)
{
        ...
        if ((aigp->ai_mode == AI_GOAL_KEEP_SAFE_DISTANCE) || (aigp->ai_mode == AI_GOAL_CHASE_ANY) || (aigp->ai_mode == AI_GOAL_STAY_STILL) || (aigp->ai_mode == AI_GOAL_PLAY_DEAD))
        ...
        if ((aigp->ai_mode == AI_GOAL_DOCK) && (status == SHIP_STATUS_ARRIVED)) {
                ...
                if (!(aigp->flags[AI::Goal_Flags::Dockee_index_valid])) {
                        ...
                        if (sindex != -1) {
                                ...
                                aigp->dockee.index = -1; // this will force code into if statement below making
                                                         // goal not achievable.
                }
                ...
                (aigp->ai_mode == AI_GOAL_DESTROY_SUBSYSTEM) && (status == SHIP_STATUS_ARRIVED)) {
                ...
                ((aigp->ai_mode == AI_GOAL_IGNORE) || (aigp->ai_mode == AI_GOAL_IGNORE_NEW)) && (status == SHIP_STATUS_ARRIVED)) {
                object *ignored;
...
int ai_goal_priority_compare(const void *a, const void *b)
{
        ...
        if ((ga->flags[AI::Goal_Flags::Goal_on_hold]) && !(gb->flags[AI::Goal_Flags::Goal_on_hold]))
                ...
                !(ga->flags[AI::Goal_Flags::Goal_on_hold]) && (gb->flags[AI::Goal_Flags::Goal_on_hold]))
                return -1;
        ...
        // push this goal higher in the list
                ...
                (ga->flags[AI::Goal_Flags::Goal_override]) && !(gb->flags[AI::Goal_Flags::Goal_override]))
                return -1;
        else if (
                !(ga->flags[AI::Goal_Flags::Goal_override]) && (gb->flags[AI::Goal_Flags::Goal_override]))
                return 1;
...
void ai_process_mission_orders(int objnum, ai_info *aip)
{
        ...
        case AI_GOAL_DOCK: {
                ...
                ASSERT(
                        current_goal->flags[AI::Goal_Flags::Dockee_index_valid] && current_goal->flags[AI::Goal_Flags::Docker_index_valid]);
                ai_dock_with_object(
...
int query_referenced_in_ai_goals(ai_goal *goals, int type, const char *name)
{
             ...
             i++) // loop through all the goals in the Ai_info entry
        {
                ...
                if (flag) // is this a valid goal to parse for this conversion?
                {
                        if (!strcasecmp(goals[i].target_name, name))
                                return 1;
                }
        }
...
char *ai_add_dock_name(const char *str)
{
        ...
        for (i = 0; i < Num_ai_dock_names; i++)
                if (!strcasecmp(Ai_dock_names[i], str))
