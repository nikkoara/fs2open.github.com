...
// Goober5000 - okay, this seems really stupid.  If any ship in the mission is
// assigned a goal in PURGE_GOALS_ALL_SHIPS, *every* other ship will have
// certain goals purged.  So I added PURGE_GOALS_ONE_SHIP for goals which
// should only purge other goals in the one ship.
#define PURGE_GOALS_ALL_SHIPS \
    (AI_GOAL_IGNORE | AI_GOAL_DISABLE_SHIP | AI_GOAL_DISARM_SHIP)
#define PURGE_GOALS_ONE_SHIP (AI_GOAL_IGNORE_NEW)

// goals given from the player to other ships in the game are also handled in
// this code
...
// AL 11-17-97: A text description of the AI goals.  This is used for printing
// out on the HUD what a ship's current orders are.  If the AI goal doesn't
// correspond to something that ought to be printable, then NULL is used. JAS:
// Converted to a function in order to externalize the strings
const char* Ai_goal_text (int goal) {
    switch (goal) {
    case AI_GOAL_CHASE:
    case AI_GOAL_CHASE_WING:
    case AI_GOAL_CHASE_SHIP_CLASS: return XSTR ("attack ", 474);
    case AI_GOAL_DOCK: return XSTR ("dock ", 475);
    case AI_GOAL_WAYPOINTS:
    case AI_GOAL_WAYPOINTS_ONCE: return XSTR ("waypoints", 476);
    case AI_GOAL_DESTROY_SUBSYSTEM: return XSTR ("destroy ", 477);
    case AI_GOAL_FORM_ON_WING: return XSTR ("form on ", 478);
    case AI_GOAL_UNDOCK: return XSTR ("undock ", 479);
    case AI_GOAL_GUARD:
    case AI_GOAL_GUARD_WING: return XSTR ("guard ", 480);
    case AI_GOAL_DISABLE_SHIP: return XSTR ("disable ", 481);
    case AI_GOAL_DISARM_SHIP: return XSTR ("disarm ", 482);
    case AI_GOAL_EVADE_SHIP: return XSTR ("evade ", 483);
    case AI_GOAL_REARM_REPAIR: return XSTR ("rearm ", 484);
    case AI_GOAL_FLY_TO_SHIP: return XSTR ("rendezvous with ", 1597);
    }
...
// function to maybe add the form on my wing goal for a player's starting wing.
// Called when a player wing arrives.
void ai_maybe_add_form_goal (wing* wingp) {
    int j;

    // iterate through the ship_index list of this wing and check for orders.
    // We will do this for all ships in the wing instead of on a wing only
    // basis in case some ships in the wing actually have different orders than
    // others
    for (j = 0; j < wingp->current_count; j++) {
        ...
        if (aip == Player_ai) continue;

        // it is sufficient enough to check the first goal entry to see if it
        // has a valid goal
        if (aip->goals[0].ai_mode == AI_GOAL_NONE) {
            // need to add a form on my wing goal here.  Ships are always
            // forming on the player's wing.
            ai_add_ship_goal_player (
                AIG_TYPE_PLAYER_SHIP, AI_GOAL_FORM_ON_WING, -1,
                Player_ship->ship_name, aip);
        }
    }
}
...
void ai_post_process_mission () {
    ...
    // Check ships in player starting wings.  Those ships should follow these
    // rules: (1) if they have no orders, they should get a form on my wing
    ...
    // for every valid ship object, call process_mission_orders to be sure that
    // ships start the mission following the orders in the mission file right
    // away instead of waiting N seconds before following them.  Do both the
    // created list and the object list for safety
...
int ai_query_goal_valid (int ship, int ai_goal_type) {
    ...
    if (ai_goal_type == AI_GOAL_NONE) return 1; // anything can have no orders.
...
// remove an ai goal from it's list.  Uses the active_goal member as the goal
// to remove
void ai_remove_ship_goal (ai_info* aip, int index) {
    ...
    aip->goals[index].flags.reset (); // must reset the flags since not doing
                                      // so will screw up goal sorting.

    if (index == aip->active_goal) aip->active_goal = AI_GOAL_NONE;

    // mwa -- removed this line 8/5/97.  Just because we remove a goal doesn't
    // mean to do the default behavior.  We will make the call commented out
    // below in a more reasonable location
...
void ai_clear_ship_goals (ai_info* aip) {
    ...
    for (i = 0; i < MAX_AI_GOALS; i++)
        ai_remove_ship_goal (
            aip, i); // resets active_goal and default behavior
...
void ai_clear_wing_goals (int wingnum) {
    ...
    for (i = 0; i < wingp->current_count; i++) {
        ...
        if (num > -1) ai_clear_ship_goals (&Ai_info[Ships[num].ai_index]);
    }
...
// routine which marks a wing goal as being complete.  We get the wingnum and a
// pointer to the goal structure of the goal to be removed.  This process is
// slightly tricky since some member of the wing might be pursuing a different
// goal.  We will have to compare based on mode, submode, priority, and name..
// This routine is only currently called from waypoint code!!!
void ai_mission_wing_goal_complete (int wingnum, ai_goal* remove_goalp) {
    ...
    for (i = 0; i < wingp->current_count; i++) {
        ...
        for (j = 0; j < MAX_AI_GOALS; j++) {
            ...
            // don't need to worry about these types of goals since they can't
            // possibly be a goal we are looking for.
    ...
    for (i = 0; i < MAX_AI_GOALS; i++) {
        ...
        if ((aigp->ai_mode == AI_GOAL_NONE) || !aigp->target_name) continue;
...
// routine which is called with an ai object complete it's goal.  Do some
// action based on the goal what was just completed

void ai_mission_goal_complete (ai_info* aip) {
    // if the active goal is dynamic or none, just return.  (AI_GOAL_NONE is
    // probably an error, but I don't think that this is a problem)
    ...
    ai_do_default_behavior (
        &Objects[Ships[aip->shipnum].objnum]); // do the default behavior
}

// function to prune out goals which are no longer valid, based on a goal
// pointer passed in. for instance, if we get passed a goal of "disable X",
// then any goals in the given goal array which are destroy, etc, should get
// removed.  goal list is the list of goals to purge.  It is always
// MAX_AI_GOALS in length.  This function will only get called when the goal
// which causes purging becomes valid.
void ai_goal_purge_invalid_goals (
    ai_goal* aigp, ai_goal* goal_list, ai_info* aip, int ai_wingnum) {
    int i, j;
    ai_goal* purge_goal;
    char* name;
    ...
    // these goals cannot be associated to wings, but can to a ship in a wing.
    // So, we should find out if the ship is in a wing so we can purge goals
    ...
    for (i = 0; i < MAX_AI_GOALS; purge_goal++, i++) {
        ...
        if (purge_ai_mode == AI_GOAL_NONE) continue;

        // goals must operate on something to be purged.
        if (purge_goal->target_name == NULL) continue;
        ...
        // standard goals operating on either wings or ships
            ...
            // determine if the purge goal is acting either on the ship or the
            // ship's wing.
            purge_wing = wing_name_lookup (purge_goal->target_name, 1);

            // if the target of the purge goal is a ship (purge_wing will be
        ...
        case AI_GOAL_IGNORE_NEW:
            if (purge_ai_mode &
                (AI_GOAL_DISABLE_SHIP | AI_GOAL_DISARM_SHIP | AI_GOAL_CHASE |
                 AI_GOAL_CHASE_WING | AI_GOAL_CHASE_SHIP_CLASS |
        ...
        case AI_GOAL_DISABLE_SHIP:
            if (purge_ai_mode & (AI_GOAL_CHASE | AI_GOAL_CHASE_WING |
                ...
                if (ai_wingnum >= 0) {
                    ai_ship_type =
                        Ship_info[Ships[Wings[ai_wingnum].special_ship]
                                      .ship_info_index]
                            .class_type;
                }
                ...
                // grab the ship type of the ship that is being
                // disarmed/disabled
                ship_type_info* crippled_ships_type =
                    &Ship_types[Ship_info[Ships[ship_index].ship_info_index]
                                    .class_type];

                // work through all the ship types which to see if the class
                // matching our ai ship must ignore the ship being
                // disarmed/disabled
                for (j = 0;
                     j < (int)crippled_ships_type->ai_cripple_ignores.size ();
                     j++) {
                    if (crippled_ships_type->ai_cripple_ignores[j] ==
                        ai_ship_type) {
                        purge_goal->flags.set (AI::Goal_Flags::Purge);
                    }
                }
...
// function to purge the goals of *all* ships in the game based on the incoming
// goal structure
void ai_goal_purge_all_invalid_goals (ai_goal* aigp) {
    ...
    if (!(aigp->ai_mode & PURGE_GOALS_ALL_SHIPS)) return;

    for (sop = GET_FIRST (&Ship_obj_list); sop != END_OF_LIST (&Ship_obj_list);
         sop = GET_NEXT (sop)) {
        ship* shipp = &Ships[Objects[sop->objnum].instance];
        ai_goal_purge_invalid_goals (
            aigp, Ai_info[shipp->ai_index].goals, &Ai_info[shipp->ai_index],
            -1);
    }
...
int ai_goal_find_dockpoint (int shipnum, int dock_type) {
    ...
    while (loop_count < 100) {
        dock_index = model_find_dock_index (
            Ship_info[shipp->ship_info_index].model_num, dock_type,
            dock_index + 1);

        // not found?
        if (dock_index == -1) {
            if (loop_count == 0) {
                // first time around... there are no slots fitting this
                // description
                return -1;
            }
            else {
...
// passed are the pointer to goal we are working with.  aip is the ai_info
// pointer of the ship with the order.  aigp is a pointer to the goal (of aip)
// of which we are fixing up the docking points
void ai_goal_fixup_dockpoints (ai_info* aip, ai_goal* aigp) {
    ...
    // For this, the common flags for aip's active point (ie point it wants to
    // dock with) and aigp's passive point (point it wants to be docked with)
    // are combined and the common ones are used, in order of precedence. Yes,
    // it does sound vaguely like a double-entree.

    int aip_type_dock =
        Ship_info[Ships[aip->shipnum].ship_info_index].class_type;
    ...
    for (int i = 0; i < Num_dock_type_names; i++) {
        if (common_docks & Dock_type_names[i].def) {
            docker_index =
                ai_goal_find_dockpoint (aip->shipnum, Dock_type_names[i].def);
            dockee_index =
                ai_goal_find_dockpoint (shipnum, Dock_type_names[i].def);
            break;
        }
    }

    // look for docking points of the appriopriate type.  Use cargo docks for
    // cargo ships.
    /*
    if
    (Ship_info[Ships[shipnum].ship_info_index].flags[Ship::Info_Flags::Cargo])
    ...
    (Ship_info[Ships[aip->shipnum].ship_info_index].flags[Ship::Info_Flags::Support])
    ...
    */
    ...
    // if we didn't find dockpoints above, then we should just look for generic
    // docking points
    if (docker_index == -1)
        docker_index =
            ai_goal_find_dockpoint (aip->shipnum, DOCK_TYPE_GENERIC);
    if (dockee_index == -1)
...
// these functions deal with adding goals sent from the player.  They are
// slightly different from the mission goals (i.e. those goals which come from
// events) in that we don't use sexpressions for goals from the player...so we
// enumerate all the parameters

void ai_add_goal_sub_player (
    int type, int mode, int submode, char* target_name, ai_goal* aigp) {
    ...
    aigp->ai_submode =
        submode; // could mean different things depending on mode
    ...
    if (mode == AI_GOAL_CHASE_WEAPON) {
        aigp->target_instance =
            submode; // submode contains the instance of the weapon
    ...
    if (target_name != NULL)
        aigp->target_name =
            ai_get_goal_target_name (target_name, &aigp->target_name_index);
    else
        aigp->target_name = NULL;

    // special case certain orders from player so that ships continue to do the
    // right thing

    // make priority for these two support ship orders low so that they will
    // prefer repairing a ship over staying near a ship.
    if ((mode == AI_GOAL_STAY_NEAR_SHIP) ||
        (mode == AI_GOAL_KEEP_SAFE_DISTANCE))
    ...
    // Goober5000 - same with form-on-wing, since it's a type of staying near
        ...
        aigp->priority = PLAYER_PRIORITY_WING; // player wing goals not as high
                                               // as ship goals
    else
        aigp->priority = PLAYER_PRIORITY_SHIP;
}

// Goober5000 - Modified this function for clarity and to avoid returning the
// active goal's index as the empty slot.  This avoids overwriting the active
// goal while it's being executed.  So far the only time I've noticed it being
// a problem is during a rare situation where more than five friendlies want to
// rearm at the same time.  The support ship forgets what it's doing and flies
// off to repair somebody while still docked.  I reproduced this with retail,
// so it's not a bug in my new docking code. :)
int ai_goal_find_empty_slot (ai_goal* goals, int active_goal) {
    ...
    for (gindex = 0; gindex < MAX_AI_GOALS; gindex++) {
        ...
        if (goals[gindex].ai_mode == AI_GOAL_NONE) return gindex;

        // if this is the active goal, don't consider it for pre-emption!!
        if (gindex == active_goal) continue;
...
int ai_goal_num (ai_goal* goals) {
    ...
    int num_goals = 0;
    for (gindex = 0; gindex < MAX_AI_GOALS; gindex++) {
        if (goals[gindex].ai_mode != AI_GOAL_NONE) num_goals++;
    }

    return num_goals;
}

void ai_add_goal_sub_scripting (
    int type, int mode, int submode, int priority, char* target_name,
    ai_goal* aigp) {
    ASSERT ((type == AIG_TYPE_PLAYER_WING) || (type == AIG_TYPE_PLAYER_SHIP));
    ...
    if (mode == AI_GOAL_WARP) aigp->wp_list = NULL;
...
void ai_add_ship_goal_scripting (
    ...
    ai_add_goal_sub_scripting (
        AIG_TYPE_PLAYER_SHIP, mode, submode, priority, shipname, aigp);

    if ((mode == AI_GOAL_REARM_REPAIR) ||
        ((mode == AI_GOAL_DOCK) && (submode == AIS_DOCK_0))) {
...
// adds a goal from a player to the given ship's ai_info structure.  'type'
// tells us if goal is issued to ship or wing (from player),  mode is
// AI_GOAL_*. submode is the submode the ship should go into.  shipname is the
// object of the action.  aip is the ai_info pointer of the ship receiving the
// order
void ai_add_ship_goal_player (
    ...
    // if the goal is to dock, then we must determine which dock points on the
    // two ships to use. If the target of the dock is a cargo type container,
    // then we should use DOCK_TYPE_CARGO on both ships.  Code is here instead
    // of in ai_add_goal_sub_player() since a dock goal should only occur to a
    // specific ship.
...
// adds a goal from the player to the given wing (which in turn will add it to
// the proper ships in the wing
void ai_add_wing_goal_player (
    ...
    // there are more waves to come.  We use the same method here as when
    // adding a goal to a ship -- find the first empty entry.  If none exists,
    // take the oldest entry and replace it.
    empty_index = ai_goal_find_empty_slot (wingp->ai_goals, -1);
    ai_add_goal_sub_player (
        type, mode, submode, shipname, &wingp->ai_goals[empty_index]);
}

// common routine to add a sexpression mission goal to the appropriate goal
// structure.
void ai_add_goal_sub_sexp (
    int sexp, int type, ai_goal* aigp, char* actor_name) {
    int node, dummy, op;
    char* text;
    ...
    case OP_AI_WAYPOINTS: {
        ...
        if (ref_type == SEXP_ATOM_STRING) { // referenced by name
            // save the waypoint path name -- the list will get resolved when
            // the goal is checked for achievability.
            aigp->target_name = ai_get_goal_target_name (
                CTEXT (CDR (node)),
                &aigp->target_name_index); // waypoint path name;
            aigp->wp_list = NULL;
        }
        else
            Int3 ();
        ...
        if (op == OP_AI_WAYPOINTS_ONCE) aigp->ai_mode = AI_GOAL_WAYPOINTS_ONCE;
        break;
    }
    ...
    case OP_AI_DESTROY_SUBSYS:
        ...
        // store the name of the subsystem in the docker.name field for now --
        // this field must get fixed up when the goal is valid since we need to
        // locate the subsystem on the ship's model
        aigp->docker.name =
            ai_get_goal_target_name (CTEXT (CDR (CDR (node))), &dummy);
    ...
    case OP_AI_UNDOCK:
        ...
        if (CDR (CDR (node)) != -1)
            aigp->target_name = ai_get_goal_target_name (
                CTEXT (CDR (CDR (node))), &aigp->target_name_index);
        else
            aigp->target_name = NULL;
    ...
    case OP_AI_IGNORE_NEW:
        ...
        if (op == OP_AI_CHASE) {
            ...
            // in the case of ai_chase (and ai_guard) we must do a
            // wing_name_lookup on the name passed here to see if we could be
            // chasing a wing.  Hoffoss and I have consolidated sexpression
            // operators which makes this step necessary
...
int ai_remove_goal_sexp_sub (int sexp, ai_goal* aigp) {
    ...
    case OP_AI_DESTROY_SUBSYS:
        ...
        priority = (CDR (CDR (CDR (node))) >= 0)
                       ? atoi (CTEXT (CDR (CDR (CDR (node)))))
                       : -1;
        break;
    case OP_AI_DISABLE_SHIP:
    ...
    case OP_AI_DOCK:
        ...
        priority = (CDR (CDR (CDR (CDR (node)))) >= 0)
                       ? atoi (CTEXT (CDR (CDR (CDR (CDR (node))))))
                       : -1;
        break;
    case OP_AI_CHASE_ANY:
    ...
    if (goalindex == -1) return -1; /* no more to do; */

    /* Clear out the contents of the goal. We can't use ai_remove_ship_goal
     * since it needs ai_info and we've only got ai_goals */
    aigp[goalindex].ai_mode = AI_GOAL_NONE;
    aigp[goalindex].signature = -1;
    aigp[goalindex].priority = -1;
    aigp[goalindex].flags.reset (); // must reset the flags since not doing so
                                    // will screw up goal sorting.
...
// type determines who has issues this ship a goal (i.e. the player/mission
// event/etc)
void ai_add_ship_goal_sexp (int sexp, int type, ai_info* aip) {
    ...
    ai_add_goal_sub_sexp (
        sexp, type, &aip->goals[gindex], Ships[aip->shipnum].ship_name);
}
...
void ai_add_wing_goal_sexp (int sexp, int type, int wingnum) {
    ...
    if ((wingp->num_waves - wingp->current_wave > 0) || Fred_running) {
        ...
        ai_add_goal_sub_sexp (
            sexp, type, &wingp->ai_goals[gindex], wingp->name);
    }
}

// function for internal code to add a goal to a ship.  Needed when the AI
// finds itself in a situation that it must get out of by issuing itself an
// order.
//
// objp is the object getting the goal
// goal_type is one of AI_GOAL_*
// other_name is a character string objp might act on (for docking, this is a
// shipname, for guarding this name can be a shipname or a wingname)
// docker_point and dockee_point are used for the AI_GOAL_DOCK command to tell
// two ships where to dock immediate means to process this order right away
void ai_add_goal_ship_internal (
    ai_info* aip, int goal_type, char* name, int /*docker_point*/,
    int /*dockee_point*/, int immediate) {
    int gindex;
    ai_goal* aigp;

#ifndef NDEBUG
    // Goober5000 - none of the goals act on the actor, as in
    // ai_add_goal_sub_sexp
    ...
    switch (goal_type) {
        /* Goober5000 - this seems to not be used
            case AI_GOAL_DOCK:
                ...
                aigp->ai_mode = AI_GOAL_DOCK;
                aigp->ai_submode = AIS_DOCK_0;          // be sure to set the
           submode break;
        */
        ...
        /* Goober5000 - this seems to not be used
            case AI_GOAL_GUARD:
                if ( wing_name_lookup(name, 1) != -1 )
                    ...
                    aigp->ai_mode = AI_GOAL_GUARD;
                aigp->priority = PLAYER_PRIORITY_MIN-1;         // make the
           priority always less than what the player's is break;
        */
    ...
    case AI_GOAL_REARM_REPAIR:
        ...
        aigp->priority =
            PLAYER_PRIORITY_MIN -
            1; // make the priority always less than what the player's is
        aigp->flags.remove (
            AI::Goal_Flags::Goal_override); // don't override this goal.  rearm
                                            // repair requests should happen in
                                            // order
        ai_goal_fixup_dockpoints (aip, aigp);
        break;
    ...
    // process the orders immediately so that these goals take effect right
    // away
    if (immediate) ai_process_mission_orders (Ships[aip->shipnum].objnum, aip);
}

// function to add an internal goal to a wing.  Mike K says that the goal
// doesn't need to persist across waves of the wing so we merely need to add
// the goal to each ship in the wing.  Certain goal are simply not valid for
// wings (like dock, undock).  Immediate parameter gets passed to add_ship_goal
// to say whether or not we should process this goal right away
void ai_add_goal_wing_internal (
    wing* wingp, int goal_type, char* name, int immediate) {
    int i;
    ...
    for (i = 0; i < wingp->current_count; i++) {
        ...
        ai_add_goal_ship_internal (
            &Ai_info[Ships[num].ai_index], goal_type, name, -1, -1, immediate);
    }
}
...
int ai_mission_goal_achievable (int objnum, ai_goal* aigp) {
    ...
    if (aigp->ai_mode == AI_GOAL_WARP) {
        ...
        if (shipp->departure_location == DEPART_AT_DOCK_BAY) {
            int anchor_shipnum =
                ship_name_lookup (Parse_names[shipp->departure_anchor]);
            if (anchor_shipnum >= 0 &&
    ...
    // form on wing is always achievable if we are forming on Player, but it's
    // up for grabs otherwise if the wing target is valid then be sure to set
    // the override bit so that it always gets executed next
    if (aigp->ai_mode == AI_GOAL_FORM_ON_WING) {
        sindex = ship_name_lookup (aigp->target_name);

        if (sindex < 0) return AI_GOAL_NOT_ACHIEVABLE;

        aigp->flags.set (AI::Goal_Flags::Goal_override);
        return AI_GOAL_ACHIEVABLE;
    }

    // check to see if we have a valid list.  If not, then try to set one up.
    // If that fails, then we must pitch this order
    if ((aigp->ai_mode == AI_GOAL_WAYPOINTS_ONCE) ||
        (aigp->ai_mode == AI_GOAL_WAYPOINTS)) {
    ...
    // chasing all ships of a certain ship class is achievable if there are
    // ships of that class present; if not, the status is not known because
    // more ships of that class could arrive in the future (c.f.
    // AI_GOAL_CHASE_WING subsequent to the next switch statement)
    ...
    case AI_GOAL_DOCK:
    case AI_GOAL_UNDOCK:
        // MWA 3/20/97 -- cannot short circuit a dock or undock goal already
        // succeeded -- we must
        // rely on the goal removal code to just remove this goal.  This is
        // because docking/undock can happen > 1 time per mission per pair of
        // ships.  The above checks will find only if the ships docked or
        // undocked at all, which is not what we want.
        status = 0;
        break;
    ...
    case AI_GOAL_DESTROY_SUBSYSTEM: {
        ...
        // shipnum could be -1 depending on if the ship hasn't arrived or died.
        // only look for subsystem destroyed when shipnum is valid
        sindex = ship_name_lookup (aigp->target_name);

        // can't determine the status of this goal if ship not valid
        // or we haven't found a valid subsystem index yet
        if ((sindex == -1) ||
            (aigp->flags[AI::Goal_Flags::Subsys_needs_fixup])) {
            status = 0;
            break;
        }

        // if the ship is not in the mission or the subsystem name is still
        // being stored, mark the status as 0 so we can continue.  (The
        // subsystem name must be turned into an index into the ship's
        // subsystems for this goal to be valid).
        ASSERT (aigp->ai_submode >= 0);
        ssp = ship_get_indexed_subsys (&Ships[sindex], aigp->ai_submode);
        if (ssp != NULL) {
            status = mission_log_get_time (
                LOG_SHIP_SUBSYS_DESTROYED, aigp->target_name,
                ssp->system_info->subobj_name, NULL);
        }
        else {
    ...
    case AI_GOAL_DISABLE_SHIP:
        status = mission_log_get_time (
            LOG_SHIP_DISABLED, aigp->target_name, NULL, NULL);
        break;

    case AI_GOAL_DISARM_SHIP:
        status = mission_log_get_time (
            LOG_SHIP_DISARMED, aigp->target_name, NULL, NULL);
        break;

    // to guard or ignore a ship, the goal cannot continue if the ship being
    // guarded is either destroyed or has departed.
    case AI_GOAL_CHASE:
    case AI_GOAL_GUARD:
    case AI_GOAL_IGNORE:
    case AI_GOAL_IGNORE_NEW:
    ...
    case AI_GOAL_REARM_REPAIR: {
        // MWA -- 4/22/98.  Check for the ship actually being in the mission
        // before checking departure and destroyed.  In multiplayer, since
        // ships can respawn, they get log entries for being destroyed even
        // though they have respawned.
        sindex = ship_name_lookup (aigp->target_name);
        if (sindex < 0) {
            status = mission_log_get_time (
                LOG_SHIP_DEPARTED, aigp->target_name, NULL, NULL);
            if (!status) {
                status = mission_log_get_time (
                    LOG_SHIP_DESTROYED, aigp->target_name, NULL, NULL);
                if (!status) {
                    status = mission_log_get_time (
                        LOG_SELF_DESTRUCTED, aigp->target_name, NULL, NULL);
                }
            }

            if (status) return_val = AI_GOAL_NOT_ACHIEVABLE;
        }
        else {
            status = 0;
        }
    ...
    case AI_GOAL_GUARD_WING: {
        status = mission_log_get_time (
            LOG_WING_DEPARTED, aigp->target_name, NULL, NULL);
        if (!status) {
            status = mission_log_get_time (
                LOG_WING_DESTROYED, aigp->target_name, NULL, NULL);
            if (status) return_val = AI_GOAL_NOT_ACHIEVABLE;
        }
    ...
    case AI_GOAL_CHASE_WEAPON: {
        // for chase weapon, we simply need to look at the weapon instance that
        // we are trying to attack and see if the object still exists, and has
        // the same signature that we expect.
        ...
        if (Objects[Weapons[aigp->target_instance].objnum].signature !=
            aigp->target_signature)
            return AI_GOAL_NOT_ACHIEVABLE;
    ...
    // if status is true, then the mission log event was found and the goal was
    // satisfied.  return AI_GOAL_SATISFIED which should allow this ai object
    // to move onto the next order
    if (status) return return_val;

    // determine the status of the shipname that this object is acting on.
    // There are a couple of special cases to deal with.  Both the chase wing
    // and undock commands will return from within the if statement.
    if ((aigp->ai_mode == AI_GOAL_CHASE_WING) ||
        (aigp->ai_mode == AI_GOAL_GUARD_WING)) {
        sindex = wing_name_lookup (aigp->target_name);

        if (sindex < 0) return AI_GOAL_NOT_KNOWN;
    ...
    // undocking from a specified object is handled below
        ...
        (aigp->ai_mode == AI_GOAL_UNDOCK) && (aigp->target_name == NULL)) {
        return AI_GOAL_ACHIEVABLE;
    }
    else {
    ...
    // Goober5000 - before doing anything else, check if this is a disarm goal
    // for an arrived ship...
    if ((status == SHIP_STATUS_ARRIVED) &&
        (aigp->ai_mode == AI_GOAL_DISARM_SHIP)) {
        // if the ship has no turrets, we can't disarm it!
        if (Ships[ship_name_lookup (aigp->target_name)]
                .subsys_info[SUBSYSTEM_TURRET]
                .type_count == 0)
            return AI_GOAL_NOT_ACHIEVABLE;
    }
    ...
    if ((status == SHIP_STATUS_ARRIVED) &&
        !(aigp->flags[AI::Goal_Flags::Goals_purged])) {
    ...
    // if we are docking, validate the docking indices on both ships.  We might
    // have to change names to indices. only enter this calculation if the ship
    // we are docking with has arrived.  If the ship is gone, then this goal
    // will get removed.
    if ((aigp->ai_mode == AI_GOAL_DOCK) && (status == SHIP_STATUS_ARRIVED)) {
        ...
        if (!(aigp->flags[AI::Goal_Flags::Dockee_index_valid])) {
            ...
            if (sindex != -1) {
                ...
                index =
                    model_find_dock_name_index (modelnum, aigp->dockee.name);
                aigp->dockee.index = index;
                aigp->flags.set (AI::Goal_Flags::Dockee_index_valid);
            }
            else
                aigp->dockee.index =
                    -1; // this will force code into if statement below making
                        // goal not achievable.
        }
        ...
        // we must also determine if we're prevented from docking for any
        // reason
        sindex = ship_name_lookup (aigp->target_name);
        ASSERT (sindex >= 0);
        ...
        if (object_is_docked (goal_objp)) {
            ...
            // than me
            object* obstacle_objp =
                dock_find_object_at_dockpoint (goal_objp, aigp->dockee.index);
            if (obstacle_objp == NULL) {
                // nobody in the way... we're good
            }
            else if (obstacle_objp != objp) {
                // return NOT_KNOWN which will place the goal on hold until the
                // dockpoint is clear
                return AI_GOAL_NOT_KNOWN;
            }
        }

        // if this ship is docked and needs to get docked with something else,
        // then undock this ship
        if (object_is_docked (objp)) {
            // if the dockpoint I need to dock with is occupied by someone
            // other than the guy I need to dock to
            object* obstacle_objp =
                dock_find_object_at_dockpoint (objp, aigp->docker.index);
            if (obstacle_objp == NULL) {
                // nobody in the way... we're good
            }
                ...
                if (!(aigp->flags[AI::Goal_Flags::Goal_on_hold]))
                    ai_add_goal_ship_internal (
                        aip, AI_GOAL_UNDOCK,
                        Ships[obstacle_objp->instance].ship_name, -1, -1, 0);
        ...
        (aigp->ai_mode == AI_GOAL_UNDOCK) && (status == SHIP_STATUS_ARRIVED)) {
        // Put this goal on hold if we're already undocking.  Otherwise the new
        // goal will pre-empt the current goal and strange things might happen.
        // One is that the object movement code forgets the previous undocking
        // and "re-docks" the previous goal's ship.  Other problems might
        // happen too, so err on the safe side.  (Yay for emergent paragraph
        // justification!)
        if ((aip->mode == AIM_DOCK) && (aip->submode >= AIS_UNDOCK_0)) {
            // only put it on hold if it's someone other than the guy we're
            // undocking from right now!!
            if (aip->goal_objnum !=
                Ships[ship_name_lookup (aigp->target_name)].objnum)
                return AI_GOAL_NOT_KNOWN;
        }
    }
    else if (
        (aigp->ai_mode == AI_GOAL_DESTROY_SUBSYSTEM) &&
        (status == SHIP_STATUS_ARRIVED)) {
        // if the ship has arrived, and the goal is destroy subsystem, then
        // check to see that we have fixed up the subsystem name (of the
        // subsystem to destroy) into an index into the ship's subsystem list
        if (aigp->flags[AI::Goal_Flags::Subsys_needs_fixup]) {
            sindex = ship_name_lookup (aigp->target_name);

            if (sindex != -1) {
                aigp->ai_submode =
                    ship_get_subsys_index (&Ships[sindex], aigp->docker.name);
        ...
        // for ignoring a ship, call the ai_ignore object function, then
        // declare the goal satisfied
        sindex = ship_name_lookup (aigp->target_name);
        ASSERT (sindex != -1); // should be true because of above status
        ignored = &Objects[Ships[sindex].objnum];

        ai_ignore_object (
            objp, ignored, (aigp->ai_mode == AI_GOAL_IGNORE_NEW));
    ...
    // for rearm repair ships, a goal is only achievable if the support ship
    // isn't repairing anything else at the time, or is set to repair the ship
    // for this goal.  All other goals should be placed on hold by returning
    // GOAL_NOT_KNOWN.
    case AI_GOAL_REARM_REPAIR: {
        ...
        if (status == SHIP_STATUS_GONE) return AI_GOAL_NOT_ACHIEVABLE;
        ...
        if (Ai_info[Ships[sindex].ai_index]
                .ai_flags[AI::AI_Flags::Being_repaired])
            return AI_GOAL_ACHIEVABLE;

        // if the destination ship is dying or departing (but not completed
        // yet), the mark goal as not achievable.
        if (Ships[sindex].is_dying_or_departing ())
            return AI_GOAL_NOT_ACHIEVABLE;

        // if the destination object is no longer awaiting repair, then remove
        // the item
        if (!(Ai_info[Ships[sindex].ai_index]
                  .ai_flags[AI::AI_Flags::Awaiting_repair]))
            return AI_GOAL_NOT_ACHIEVABLE;
        ...
        // test code!!!
        if (aip->goal_objnum == -1) { return AI_GOAL_ACHIEVABLE; }

        // if he is repairing something, he can satisfy his repair goal (his
        // goal_objnum) return GOAL_NOT_KNOWN which is kind of a hack which
        // puts the goal on hold until it can be satisfied.
        if (aip->goal_objnum != Ships[sindex].objnum) return AI_GOAL_NOT_KNOWN;
...
int ai_goal_priority_compare (const void* a, const void* b) {
    ...
    // first, sort based on whether or not the ON_HOLD flag is set for the
    // goal. If the flag is set, don't push the goal higher in the list even if
    // priority is higher since goal cannot currently be achieved.

    if ((ga->flags[AI::Goal_Flags::Goal_on_hold]) &&
        !(gb->flags[AI::Goal_Flags::Goal_on_hold]))
        return 1;
    else if (
        !(ga->flags[AI::Goal_Flags::Goal_on_hold]) &&
        (gb->flags[AI::Goal_Flags::Goal_on_hold]))
        return -1;

    // check whether or not the goal override flag is set.  If it is set, then
    // push this goal higher in the list

    else if (
        (ga->flags[AI::Goal_Flags::Goal_override]) &&
        !(gb->flags[AI::Goal_Flags::Goal_override]))
        return -1;
    else if (
        !(ga->flags[AI::Goal_Flags::Goal_override]) &&
        (gb->flags[AI::Goal_Flags::Goal_override]))
    ...
    if (ga->time > gb->time) return -1;
    // V had this check commented out and would always return 1 here, that
    // messes up where multiple goals get assigned at the same time though,
    // when the priorities are also the same (Enif station bug) - taylor
...
void prioritize_goals (ai_info* aip) {
    ...
    insertion_sort (
        aip->goals, MAX_AI_GOALS, sizeof (ai_goal), ai_goal_priority_compare);
}
...
void validate_mission_goals (int objnum, ai_info* aip) {
    ...
    // loop through all of the goals to determine which goal should be
    // followed. This determination will be based on priority, and the time at
    // which it was issued.
    for (i = 0; i < MAX_AI_GOALS; i++) {
        int state;
        ai_goal* aigp;
        ...
        // quick check to see if this goal is valid or not, or if we are trying
        // to process the current goal
        if (aigp->ai_mode == AI_GOAL_NONE) continue;
        ...
        if ((state == AI_GOAL_NOT_ACHIEVABLE) ||
            (state == AI_GOAL_SATISFIED)) {
            ai_remove_ship_goal (aip, i);
            continue;
        }

        // if the status is achievable, and the on_hold flag is set, clear the
        // flagb
        if ((state == AI_GOAL_ACHIEVABLE) &&
            (aigp->flags[AI::Goal_Flags::Goal_on_hold]))
            aigp->flags.remove (AI::Goal_Flags::Goal_on_hold);

        // if the goal is not known, then set the ON_HOLD flag so that it
        // doesn't get counted as a goal to be pursued
        if (state == AI_GOAL_NOT_KNOWN)
            aigp->flags.set (
                AI::Goal_Flags::Goal_on_hold); // put this goal on hold until
                                               // it becomes true
    }

    // if we had an active goal, and that goal is now in hold, make the mode
    // AIM_NONE.  If a new valid goal is produced after prioritizing, then the
    // mode will get reset immediately.  Otherwise, setting the mode to none
    // will force ship to do default behavior.
    if ((aip->goals[0].ai_mode != AI_GOAL_NONE) &&
        (aip->goals[0].flags[AI::Goal_Flags::Goal_on_hold]))
        aip->mode = AIM_NONE;

    // if the active goal is a rearm/repair goal, the put all other valid goals
    // (which are not repair goals) on hold
    if ((aip->goals[0].ai_mode == AI_GOAL_REARM_REPAIR) &&
        object_is_docked (&Objects[objnum])) {
        for (i = 1; i < MAX_AI_GOALS; i++) {
            if ((aip->goals[i].ai_mode == AI_GOAL_NONE) ||
                (aip->goals[i].ai_mode == AI_GOAL_REARM_REPAIR))
                continue;
...
// code to process ai "orders".  Orders include those determined from the
// mission file and those given by the player to a ship that is under his
// control.  This function gets called for every AI object every N seconds
// through the ai loop.
void ai_process_mission_orders (int objnum, ai_info* aip) {
    ...
    if (aip->goals[0].signature == -1) {
        if (aip->mode == AIM_NONE) ai_do_default_behavior (objp);
        return;
    }
    ...
    // if the first goal in the list has the ON_HOLD flag, set, there is no
    // current valid goal to pursue.
    ...
    // from the goal list as soon as it is called, so we are safe removing this
    // bit here.
    aip->ai_flags.remove (AI::AI_Flags::Formation_object);

    // Goober5000 - we may want to use AI for the player
    // AL 3-7-98: If this is a player ship, and the goal is not a formation
    // goal, then do a quick out
    if (!(Player_use_ai) && (objp->flags[Object::Object_Flags::Player_ship]) &&
        (current_goal->ai_mode != AI_GOAL_FORM_ON_WING)) {
        return;
    }
    ...
    case AI_GOAL_GUARD:
        ...
        ASSERT (shipnum != -1); // shouldn't get here if this is false!!!!
        other_obj = &Objects[Ships[shipnum].objnum];
        // shipnum and other_obj are the shipnumber and object pointer of the
        // object that you should guard.
        if (objp != other_obj) {
    ...
    case AI_GOAL_WAYPOINTS_ONCE: {
        ...
        if (current_goal->ai_mode == AI_GOAL_WAYPOINTS) flags |= WPF_REPEAT;
    ...
    case AI_GOAL_DOCK: {
        ...
        // be sure that we have indices for docking points here!  If we ever
        // had names, they should get fixed up in goal_achievable so that the
        // points can be checked there for validity
        ASSERT (
            current_goal->flags[AI::Goal_Flags::Dockee_index_valid] &&
            current_goal->flags[AI::Goal_Flags::Docker_index_valid]);
        ai_dock_with_object (
            objp, current_goal->docker.index, other_obj,
            current_goal->dockee.index, AIDO_DOCK);
        break;
    }

    case AI_GOAL_UNDOCK:
        // try to find the object which which this object is docked with.  Use
        // that object as the "other object" for the undocking proceedure.  If
        // "other object" isn't found, then the undock goal cannot continue.
        // Spit out a warning and remove the goal.
        ...
        // no specific ship
            ...
            if (object_is_docked (objp)) {
                ...
                current_goal->target_name =
                    Ships[other_obj->instance].ship_name;
            }
        ...
        if (other_obj == NULL) {
            // assume that the guy he was docked with doesn't exist anymore.
            // (i.e. a cargo containuer can get destroyed while docked with a
            // freighter.)  We should just remove this goal and let this ship
            // pick up it's next goal.
            ai_mission_goal_complete (
                aip); // mark as complete, so we can remove it and move on!!!
            break;
        }

        // Goober5000 - Sometimes a ship will be assigned a new goal before it
        // can finish undocking.  Later, when the ship returns to this goal, it
        // will try to resume undocking from a ship it's not attached to.  If
        // this happens, remove the goal as above.
        if (!dock_check_find_direct_docked_object (objp, other_obj)) {
            ai_mission_goal_complete (aip);
            break;
        }

        // passing 0, 0 is okay because the undock code will figure out where
        // to undock from
        ai_dock_with_object (objp, 0, other_obj, 0, AIDO_UNDOCK);
        break;

        // when destroying a subsystem, we can destroy a specific instance of a
        // subsystem or all instances of a type of subsystem (i.e. a specific
        // engine or all engines). the ai_submode value is > 0 for a specific
        // instance of subsystem and < 0 for all instances of a specific type
    case AI_GOAL_DESTROY_SUBSYSTEM:
    case AI_GOAL_DISABLE_SHIP:
    case AI_GOAL_DISARM_SHIP: {
        shipnum = ship_name_lookup (current_goal->target_name);
        ...
        ai_set_attack_subsystem (
            objp,
            current_goal->ai_submode); // submode stored the subsystem type
        if (current_goal->ai_mode != AI_GOAL_DESTROY_SUBSYSTEM) {
            if (aip->target_objnum != -1) {
                ...
                if (Ship_types[Ship_info[Ships[shipnum].ship_info_index]
                    ...
                    Objects[aip->target_objnum].flags.set (
    ...
    case AI_GOAL_CHASE_WING:
        ...
        ASSERTX (
            wingnum >= 0,
            "The target of AI_GOAL_CHASE_WING must refer to a valid wing!");
        ai_attack_wing (objp, wingnum);
        break;

    case AI_GOAL_CHASE_ANY: ai_attack_object (objp, NULL, NULL); break;
    ...
    case AI_GOAL_CHASE_SHIP_CLASS:
        ...
        ASSERTX (
            shipnum >= 0,
            "The target of AI_GOAL_CHASE_SHIP_CLASS must refer to a valid "
            "ship class!");
    ...
    case AI_GOAL_STAY_STILL:
        ...
        // clear out the object's goals.  Seems to me that if a ship is staying
        // still for a purpose then we need to clear everything out since there
        // is not a real way to get rid of this goal clearing out goals is okay
        // here since we are now what mode to set this AI object to.
        ai_clear_ship_goals (aip);
        ai_stay_still (objp, NULL);
        break;

    case AI_GOAL_PLAY_DEAD:
        // if a ship is playing dead, MWA says that it shouldn't try to do
        // anything else. clearing out goals is okay here since we are now what
        // mode to set this AI object to.
        ai_clear_ship_goals (aip);
    ...
    case AI_GOAL_FORM_ON_WING:
        // for form on wing, we need to clear out all goals for this ship, and
        // then call the form on wing AI code clearing out goals is okay here
        // since we are now what mode to set this AI object to.
        ai_clear_ship_goals (aip);
        shipnum = ship_name_lookup (current_goal->target_name);
        ASSERT (shipnum >= 0);
        other_obj = &Objects[Ships[shipnum].objnum];
    ...
    case AI_GOAL_STAY_NEAR_SHIP: {
        ...
        ASSERT (shipnum >= 0);
        other_obj = &Objects[Ships[shipnum].objnum];
        // todo MK:  hook to keep support ship near other_obj -- other_obj
        // could be the player object!!!
        float dist =
            300.0f; // How far away to stay from ship.  Should be set in SEXP?
...
void ai_update_goal_references (
    ai_goal* goals, int type, const char* old_name, const char* new_name) {
    int i, mode, flag, dummy;

    for (i = 0; i < MAX_AI_GOALS;
         i++) // loop through all the goals in the Ai_info entry
    {
        ...
        case REF_TYPE_PLAYER:
            ...
            case AI_GOAL_IGNORE_NEW:
            case AI_GOAL_EVADE_SHIP:
            case AI_GOAL_STAY_NEAR_SHIP: flag = 1;
            }
            break;
        ...
        case REF_TYPE_WING:
            switch (mode) {
            case AI_GOAL_CHASE_WING:
            case AI_GOAL_GUARD_WING: flag = 1;
            }
            break;
        ...
        case REF_TYPE_WAYPOINT:
            switch (mode) {
            case AI_GOAL_WAYPOINTS:
            case AI_GOAL_WAYPOINTS_ONCE: flag = 1;
            }
            break;
        ...
        if (flag) // is this a valid goal to parse for this conversion?
        {
            if (!strcasecmp (goals[i].target_name, old_name)) {
                if (*new_name == '<') // target was just deleted..
                    ...
                    goals[i].target_name =
                        ai_get_goal_target_name (new_name, &dummy);
            }
        }
    }
}
