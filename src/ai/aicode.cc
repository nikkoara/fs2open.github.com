...
void parse_aitbl()
{
        try {
                ...
                Ai_classes = (ai_class *)malloc(Num_alloced_ai_classes * sizeof(ai_class));
                Ai_class_names = (char **)malloc(Num_alloced_ai_classes * sizeof(char *));
        ...
        int sexp_flags)
{
        ...
        if ((objp->type == OBJ_SHIP) && !(sexp_flags & AITTV_VIA_SEXP)) {
                if (ship_get_subsystem_strength(
                            &Ships[objp->instance], SUBSYSTEM_ENGINE)
                    <= 0.0f)
                        return;
        }
        ...
        if ((objp->type == OBJ_WEAPON) || (sexp_flags & AITTV_IGNORE_BANK))
                ...
                (bank_override) && (iff_x_attacks_y(Ships[objp->instance].team,
...
int set_target_objnum(ai_info *aip, int objnum)
{
        ...
        if (aip->target_objnum == objnum) {
                ...
                if ((Game_mode & GM_MULTIPLAYER) && (aip == Player_ai) && (Player_obj->type == OBJ_OBSERVER)) {
                        ...
                        ASSERT(
                                objnum != Ships[aip->shipnum].objnum); // make sure not targeting self
                }
                ...
                aip->last_hit_target_time = Missiontime; // Also, the "last hit target" time should be reset
                ...
                if (OBJ_INDEX(eno->trial_objp) != eno->objnum) {
                        ...
                        if (iff_matches_mask(shipp->team, eno->enemy_team_mask)) {
                                ...
                                if (!sip->is_big_or_huge() && num_attacking < eno->max_attackers) {
                                        dist *= (float)(num_attacking + 2) / 2.0f; // prevents lots of ships from attacking
                                                                                   // same target
                                }
                                ...
                                if ((sip->is_big_or_huge()) || (num_attacking < eno->max_attackers)) {
                                        if (eno->trial_objp
                                                ...
                                                dist *= 1.0f + (NUM_SKILL_LEVELS - Game_skill_level - 1) / NUM_SKILL_LEVELS; // Favor attacking
                                                                                                                             // non-players based
                                                                                                                             // on skill level.
                                        }
        ...
        int randomize_pnt)
{
        ...
        if (dir == 1) {
                ...
                ASSERT(
                        dir == -1); // direction must be up by 1 or down by 1 and it's neither!
...
void ai_update_aim(ai_info *aip)
{
        if (Missiontime >= aip->next_aim_pos_time) {
                ...
                aip->next_aim_pos_time = Missiontime + fl2f(fs2::prng::randf(0, 0.0f, aip->ai_max_aim_update_delay));
        } else {
...
void modify_model_path_points(object *objp)
{
        ...
        ASSERT(
                pnp->path_index != -1); // If this is -1, that means we never found the model path points
        ...
        if (force_recreate_flag || timestamp_elapsed(aip->path_next_create_time)) {
                ...
                if (override_hash || (hashval != aip->path_goal_obj_hash)) {
                        ...
                        if (force_recreate_flag || (dist > 2.0f)) {
                                aip->path_next_create_time = timestamp(1000); // Update again in as little as 1000
...
float ai_path_0()
{
        ...
        if (aip->path_start == -1) {
                ...
                ASSERT(
                        aigp->flags[AI::Goal_Flags::Dockee_index_valid] && aigp->flags[AI::Goal_Flags::Docker_index_valid]);
        ...
        if ((aip->path_cur + aip->path_dir - aip->path_start < num_points) || (aip->path_cur + aip->path_dir < aip->path_start))
                ...
                // direction!
                ASSERT(
                        (aip->path_cur - aip->path_dir >= aip->path_start) && (aip->path_cur - aip->path_dir - aip->path_start < num_points));
        ...
        if ((dist_to_goal < MIN_DIST_TO_WAYPOINT_GOAL) || (vm_vec_dist_quick(&Pl_objp->last_pos, &Pl_objp->pos) > 0.1f)) {
                ...
                if ((vm_vec_dist_quick(&Pl_objp->pos, &gcvp) < min_dist_to_goal) || (((r >= 0.0f) && (r <= 1.0f)) && (vm_vec_dist_quick(&nearest_point, &gcvp) < (MIN_DIST_TO_WAYPOINT_GOAL + Pl_objp->radius)))) {
                        aip->path_cur += aip->path_dir;
...
float ai_path_1()
{
        ...
        if ((dist_to_goal < MIN_DIST_TO_WAYPOINT_GOAL) || (vm_vec_dist_quick(&Pl_objp->last_pos, &Pl_objp->pos) > 0.1f)) {
                ...
                // inertia
                        ...
                        aip->path_cur - aip->path_start < aip->path_length - (aip->mode == AIM_DOCK ? 0 : 1)) {
                        vec3d cp_rel;
        ...
        bool *pl_treat_as_ship_p = NULL)
{
  ...
  */
        ...
        if (AutoPilotEngaged && timestamp_elapsed(LockAPConv) && carry_flag && ((The_mission.flags[Mission::Mission_Flags::Use_ap_cinematics]) || (Pl_objp != Autopilot_flight_leader))) {
                ASSERTX(
        ...
        if (Pl_objp->phys_info.speed < 0.0f) {
                ...
                if (dist_to_goal < 250.0f) {
                        dot1 = dot * dot * dot; // Very important to be pointing towards goal
        ...
        if ((self_ab_speed < 5.0f) || (max_allowed_ab_speed < 5.0f)) { // || ((shipp->wingnum != -1) && (dist_to_goal /
        ...
        if ((dist_to_goal < MIN_DIST_TO_WAYPOINT_GOAL) || (vm_vec_dist_quick(&Pl_objp->last_pos, &Pl_objp->pos) > 0.1f)) {
                ...
                if ((dist_to_goal < (MIN_DIST_TO_WAYPOINT_GOAL + sqrtf(Pl_objp->radius) + vm_vec_dist_quick(&Pl_objp->pos, &Pl_objp->last_pos))) || (((r >= 0.0f) && (r <= 1.0f)) && (vm_vec_dist_quick(&nearest_point, target_pos) < (MIN_DIST_TO_WAYPOINT_GOAL + sqrtf(Pl_objp->radius))))) {
                        int treat_as_ship;
...
void ai_fly_to_ship()
{
        ...
        for (int j = 0; j < MAX_SHIPS; j++) {
                if (Ships[j].objnum != -1 && !strcasecmp(aip->goals[aip->active_goal].target_name, Ships[j].ship_name)) {
...
int static_rand_timed(int num, int modulus)
{
        if (modulus < 2)
                ...
                t = Missiontime >> 18; // Get time in quarters of a second (SUSHI: this comment is
...
void evade_ship()
{
                ...
                if (percent_left > 30.0f + ((OBJ_INDEX(Pl_objp)) & 0x0f)) {
                        ...
                        if (aip->ai_profile_flags
                                ...
                                aip->afterburner_stop_time = (fix)(
                                        Missiontime + F1_0 + fs2::prng::randf(0) * F1_0 / 4);
                        } else {
                                aip->afterburner_stop_time = Missiontime + F1_0 + fs2::prng::randi(0) / 4;
                        }
                }
        }
        ...
        if (dist > 250.0f) {
                ...
                if (dot_to_enemy > 0.75f) {
                        ...
                        bank_override = Pl_objp->phys_info.speed; // In enemy's sights, not pointing
                                                                  // at him, twirl away.
                        goto evade_ship_l1;
                }
        } else {
evade_ship_l1:;
                if (aip->ai_evasion > myrand() * 100.0f / 32767.0f) {
                        ...
                        psrandval = (float)(((Missiontime >> 14) & 0x0f) - 8); // Value between
                                                                               //-8 and 7
...
ai_select_primary_weapon_OLD(const object *objp, Weapon::Info_Flags flags)
{
        ...
        ASSERT(
                shipp->ship_info_index >= 0 && shipp->ship_info_index < static_cast< int >(Ship_info.size()));
...
int ai_fire_primary_weapon(object *objp)
{
        ...
        if (aip->target_objnum != -1) {
                ...
                enemy_sip = (enemy_objp->type == OBJ_SHIP)
        ...
        if (The_mission.ai_profile->primary_ammo_burst_mult[Game_skill_level] > 0 &&         // Make sure we are using burst fire
            ...
            (enemy_sip->is_small_ship() || enemy_sip->flags[Ship::Info_Flags::Transport]) && // Only burst for
                                                                                             // small ships
            // (transports count)
                                                              ...
                                                              // ballistic
        {
                ...
                float weaponRange = Weapon_info[swp->primary_bank_weapons[swp->current_primary_bank]]
                                            .weapon_range;
        ...
        if ((enemy_objp != NULL) && (enemy_objp->flags[Object::Object_Flags::Protected])) {
                ...
                if ((enemy_objp->hull_strength < 750.0f) && ((aip->targeted_subsys == NULL) || (enemy_objp->hull_strength < aip->targeted_subsys->current_hits + 50.0f)) && (swp->current_primary_bank >= 0)) {
                        if (!(Weapon_info
                                      [swp->primary_bank_weapons[swp->current_primary_bank]]
                                              .wi_flags[Weapon::Info_Flags::Puncture])) {
        ...
        flagset< Weapon::Info_Flags > *priority2 = NULL)
{
        ...
        for (i = 0; i < num_weapon_types; i++) {
                auto wi_flags = Weapon_info[swp->secondary_bank_weapons[weapon_bank_list[i]]]
                                        .wi_flags;
                auto ignore_mask_to_use = ((aip->ai_profile_flags
                                                    [AI::Profile_Flags::Smart_secondary_weapon_selection])
                ...
                if (!(wi_flags & ignore_mask_to_use)
                        ...
                        if ((wi_flags & prio1).any_set()) {
                                ...
                                priority2_index = weapon_bank_list[i]; // Found second priority, but might
...
int ai_fire_secondary_weapon(object *objp)
{
        ...
        if ((wip->is_locked_homing()) && (!Ai_info[shipp->ai_index].current_target_is_locked)) {
                ...
                (wip->wi_flags[Weapon::Info_Flags::Bomb]) || (vm_vec_dist_quick(&objp->pos, &En_objp->pos) > 50.0f)) {
...
float ai_endangered_time(object *ship_objp, object *weapon_objp)
{
        ...
        if (weapon_objp->phys_info.speed < 1.0f)
                ...
                (from_dot > 0.1f) && (dist / (from_dot * from_dot) < 48 * ship_objp->radius)) //: don't require them to see it, they have
        ...
        float dot_from_enemy)
{
        ...
        if (dist_to_enemy > 200.0f + vm_vec_mag_quick(&En_objp->phys_info.vel) * dot_from_enemy + Pl_objp->phys_info.speed * speed_ratio) {
                                        ...
                                        if (sip_local->afterburner_fuel_capacity > 0.0f) {
                                                ...
                                                if (percent_left > 30.0f + ((OBJ_INDEX(Pl_objp)) & 0x0f)) {
                                                        ...
                                                        if (aip->ai_profile_flags
                                                                ...
                                                                // devide by 0 later
                                                                max_ab_vel = sip_local->afterburner_max_vel.xyz.z > 0.0f
                ...
                (Missiontime - aip->last_hit_time > F1_0 * 7) && (En_objp->phys_info.speed < 10.0f) && (dist_to_enemy > 25.0f) && (dot_to_enemy > 0.8f) && (dot_from_enemy < 0.8f)) {
                accelerate_ship(
        ...
        object *objp, object *big_objp, vec3d *collision_point, vec3d *avoid_pos)
{
        ...
        for (s = 0.5f; s < 1.3f; s += 0.25f) {
                ...
                for (i = 0; i < 4; i++) {
                        ...
                        float ku = big_objp->radius * s + objp->radius * (OBJ_INDEX(objp) % 4) / 4; // This objp->radius stuff to prevent ships
...
void ai_set_evade_object(object *objp, object *other_objp)
{
        ...
        ASSERT(
                other_objnum != Ships[aip->shipnum].objnum); // make sure not targeting self
...
void ai_set_guard_object(object *objp, object *other_objp)
{
        ...
        if ((other_aip->wing != -1) && (other_aip->wing != aip->wing) && !(The_mission.ai_profile->flags[AI::Profile_Flags::Ai_guards_specific_ship_in_wing])) {
                ...
                ASSERT(
                        other_objnum >= 0); // Hmm, bogus object and we need its position for guard_vec.
        ...
        float enemy_radius)
{
        ...
        if (num_weapon_types && (wip->is_locked_homing()) && !(shipp->flags[Ship::Ship_Flags::No_secondary_lockon])) {
                ...
                if (dot_to_enemy > needed_dot && (wip->wi_flags[Weapon::Info_Flags::Homing_aspect] || (wip->wi_flags[Weapon::Info_Flags::Homing_javelin] && (tshpp == NULL || ship_get_closest_subsys_in_sight(tshpp, SUBSYSTEM_ENGINE, &aiobjp->pos))))) {
                ...
                if (swp->secondary_bank_capacity[i] > 0) {
                        if (swp->secondary_bank_ammo[i] > 0) {
                                if (is_preferred_weapon(
                                            swp->secondary_bank_weapons[i], objp, en_objp)
                                    != -1) {
                                        return i;
                                }
                        }
...
int maybe_hack_cruiser_chase_abort()
{
        ...
        if (strcasecmp(Mission_filename, "sm3-08.fs2") == 0) {
                if ((strcasecmp(eshipp->ship_name, "colossus") == 0) || (strcasecmp(shipp->ship_name, "colossus") == 0)) {
...
void ai_cruiser_chase()
{
        ...
        // really track down and chase
                ...
                // check valid submode
                ASSERT(
                        (aip->submode == SM_ATTACK) || (aip->submode == SM_BIG_APPROACH) || (aip->submode == SM_BIG_CIRCLE) || (aip->submode == SM_BIG_PARALLEL));
...
void ai_chase()
{
        ...
        if (aip->submode == SM_ATTACK || aip->submode == SM_SUPER_ATTACK || aip->submode == SM_EVADE_SQUIGGLE || aip->submode == SM_EVADE || aip->submode == SM_GET_AWAY || aip->submode == AIS_CHASE_GLIDEATTACK) {
                ...
                float current_weapon_range = Weapon_info[swp->primary_bank_weapons[swp->current_primary_bank]]
                                                     .weapon_range;
                if (fs2::prng::randf(0) < aip->ai_random_sidethrust_percent && ((Missiontime - aip->last_hit_time < i2f(5) && aip->hitter_objnum >= 0) || dist_to_enemy < current_weapon_range)) {
                        do_random_sidethrust(aip, sip);
                }
        }
        ...
        if ((aip->submode != SM_AVOID) && (aip->submode != SM_ATTACK_FOREVER)) {
                ...
                if (aip->ai_stalemate_time_thresh > 0.0f && Missiontime - aip->last_hit_target_time > fl2f(aip->ai_stalemate_time_thresh) && Missiontime - aip->last_hit_time > fl2f(aip->ai_stalemate_time_thresh) && aip->time_enemy_near > aip->ai_stalemate_time_thresh && (dot_to_enemy < 0.95f - 0.5f * En_objp->radius / MAX(1.0f, En_objp->radius + dist_to_enemy))) {
                        ...
                        if (fs2::prng::randf(0) > (aip->ai_patience * .01)) {
                                if ((sip->can_glide == true) && (fs2::prng::randf(0) < aip->ai_glide_attack_percent)) {
        ...
        case SM_ATTACK:
                ...
                if ((aip->ai_flags[AI::AI_Flags::Stealth_pursuit]) && (ai_is_stealth_visible(Pl_objp, En_objp) == STEALTH_NOT_IN_FRUSTUM)) {
                        ...
                        dist_to_enemy < CIRCLE_STRAFE_MAX_DIST + En_objp->radius && (En_objp->phys_info.speed < MAX(sip->max_vel.xyz.x, sip->max_vel.xyz.y) * 1.5f) && (fs2::prng::randf(0) < aip->ai_circle_strafe_percent)) {
                        aip->submode = AIS_CHASE_CIRCLESTRAFE;
                        aip->submode_start_time = Missiontime;
                        aip->last_attack_time = Missiontime;
                } else if (
                        ai_near_full_strength(Pl_objp) && (Missiontime - aip->last_hit_target_time > i2f(3)) && (dist_to_enemy < 500.0f) && (dot_to_enemy < 0.5f)) {
                        aip->submode = SM_SUPER_ATTACK;
                        aip->submode_start_time = Missiontime;
                        aip->last_attack_time = Missiontime;
                } else if (
                        (Missiontime - aip->last_hit_target_time > i2f(6)) && (dist_to_enemy < 500.0f) && (dot_to_enemy < 0.2f) && (fs2::prng::randf(0) < (float)Game_skill_level / NUM_SKILL_LEVELS)) {
                        aip->submode = SM_GET_AWAY;
                        aip->submode_start_time = Missiontime;
                        aip->last_hit_target_time = Missiontime;
                } else if (
                        enemy_sip != NULL && (enemy_sip->is_small_ship()) && (dot_to_enemy < dot_from_enemy) && (En_objp->phys_info.speed > 15.0f) && (dist_to_enemy < 200.0f) && (dist_to_enemy > 50.0f) && (dot_to_enemy < 0.1f) && (Missiontime - aip->submode_start_time > i2f(2))) {
                        aip->submode = SM_EVADE_BRAKE;
                        aip->submode_start_time = Missiontime;
                } else if (
                        (dot_to_enemy > 0.2f) && (dot_from_enemy > -0.2f) && (dot_from_enemy < 0.1f)) {
                        aip->submode = SM_GET_BEHIND;
                        aip->submode_start_time = Missiontime;
                } else if (
                        enemy_sip != NULL && (enemy_sip->is_small_ship()) && (dist_to_enemy < 150.0f) && (dot_from_enemy > dot_to_enemy + 0.5f + aip->ai_courage * .002)) {
                        ...
                        if ((Missiontime - aip->last_hit_target_time > i2f(5)) && (fs2::prng::randf(0) < get_away_chance)) {
                                aip->submode = SM_GET_AWAY;
                                aip->submode_start_time = Missiontime;
                                aip->last_hit_target_time = Missiontime;
                        } else {
                        ...
                        enemy_sip != NULL && (enemy_sip->is_small_ship()) && (Missiontime - aip->submode_start_time > F1_0 * 2)) {
        ...
        case SM_EVADE_SQUIGGLE:
                if ((Missiontime - aip->submode_start_time > i2f(5)) || (dist_to_enemy > 300.0f)) {
                        if ((dist_to_enemy < 100.0f) && (dot_to_enemy < 0.0f) && (dot_from_enemy > 0.5f)) {
                                ...
                                (Pl_objp->phys_info.speed >= Pl_objp->phys_info.max_vel.xyz.z / 2.0f) && (sip->can_glide == true) && (fs2::prng::randf(0) < aip->ai_glide_attack_percent)) {
                                aip->last_attack_time = Missiontime;
                                aip->submode = AIS_CHASE_GLIDEATTACK;
                        } else {
                                aip->last_attack_time = Missiontime;
        ...
        case SM_EVADE_BRAKE:
                if ((dist_to_enemy < 15.0f) || (En_objp->phys_info.speed < 10.0f)) {
                        ...
                        (dot_to_enemy > 0.9f) || ((dot_from_enemy > 0.9f) && (Missiontime - aip->submode_start_time > i2f(1)))) {
                        aip->last_attack_time = Missiontime;
                        aip->submode = SM_ATTACK;
                        aip->submode_start_time = Missiontime;
        ...
        case SM_EVADE:
                ...
                if ((dot_to_enemy < 0.2f) && (dot_from_enemy < 0.8f) && (dist_to_enemy < 100.0f) && (En_objp->phys_info.speed > 15.0f)) {
                        ...
                        ((dot_to_enemy > dot_from_enemy - 0.1f) && (Missiontime > aip->submode_start_time + i2f(1))) || (dist_to_enemy > 150.0f + 2 * (Pl_objp->radius + En_objp->radius))) {
                        aip->last_attack_time = Missiontime;
                        aip->submode = SM_ATTACK;
                        aip->submode_start_time = Missiontime;
        ...
        case SM_SUPER_ATTACK:
                ...
                if ((aip->ai_flags[AI::AI_Flags::Stealth_pursuit]) && (ai_is_stealth_visible(Pl_objp, En_objp) == STEALTH_NOT_IN_FRUSTUM)) {
                        ...
                        (dist_to_enemy < 100.0f) && (dot_to_enemy < 0.8f) && (enemy_sip != NULL) && (enemy_sip->is_small_ship()) && (Missiontime - aip->submode_start_time > i2f(5))) {
                        aip->ai_flags.remove(
                        ...
                        case 3:
                                ...
                                break;
                        case 4:
                                if (dot_from_enemy + (NUM_SKILL_LEVELS - Game_skill_level) * 0.1f > dot_to_enemy) { // Less likely to GET_AWAY at lower skill
                                                                                                                    // levels.
                                        aip->submode = SM_EVADE;
                                        aip->submode_start_time = Missiontime;
                                } else {
        ...
        case SM_GET_AWAY:
                if (Missiontime - aip->submode_start_time > i2f(2)) {
                        ...
                        if ((Missiontime - aip->submode_start_time > i2f(5)) || (dist_to_enemy > rand_dist) || (dot_from_enemy < 0.4f)) {
                                ...
                                aip->submode_start_time = Missiontime;
                                aip->time_enemy_in_range = 2.0f; // Cheat.  Presumably if they were running away
        ...
        case SM_STEALTH_FIND:
                ...
                if (!(aip->ai_flags[AI::AI_Flags::Stealth_pursuit]) || (ai_is_stealth_visible(Pl_objp, En_objp) == STEALTH_IN_FRUSTUM)) {
                        ...
                        ((Missiontime - aip->submode_start_time) > i2f(5)) || (aip->submode_parm0 == SM_SF_BAIL)) {
                        // begin sweep mode
        ...
        case AIS_CHASE_GLIDEATTACK:
                ...
                if (Missiontime - aip->submode_start_time > i2f((int)(sip->rotation_time.xyz.x) + 4 + fs2::prng::randi(0, 0, 3))) {
                        aip->submode = SM_ATTACK;
                        aip->submode_start_time = Missiontime;
                        aip->last_attack_time = Missiontime;
                }
        ...
        case AIS_CHASE_CIRCLESTRAFE:
                ...
                if ((dist_to_enemy > CIRCLE_STRAFE_MAX_DIST + En_objp->radius) || (Missiontime - aip->submode_start_time > i2f(8)) || (En_objp->phys_info.speed > MAX(sip->max_vel.xyz.x, sip->max_vel.xyz.y) * 1.5)) {
                        aip->submode = SM_ATTACK;
                        aip->submode_start_time = Missiontime;
                        aip->last_attack_time = Missiontime;
                }
        ...
        if (aip->ai_stalemate_dist_thresh > 0.0f && dist_to_enemy < aip->ai_stalemate_dist_thresh && aip->submode != SM_GET_AWAY && aip->submode != AIS_CHASE_GLIDEATTACK && aip->submode != SM_FLY_AWAY && (!(aip->ai_flags[AI::AI_Flags::Stealth_pursuit]) || (ai_is_stealth_visible(Pl_objp, En_objp) == STEALTH_IN_FRUSTUM))) {
        ...
        if (aip->mode != AIM_EVADE) {
                if (dot_to_enemy > 0.95f - 0.5f * En_objp->radius / MAX(1.0f, En_objp->radius + dist_to_enemy)) {
                        ...
                        if (dot_to_enemy > MAX(
                                    0.5f, 0.90f + aip->ai_accuracy / 10.0f - En_objp->radius / MAX(1.0f, dist_to_enemy))) {
                                ship *temp_shipp;
                                ship_weapon *tswp;
                                ...
                                if (tswp->num_primary_banks > 0) {
                                        ...
                                        weapon_info *pwip = &Weapon_info[tswp->primary_bank_weapons
                                ...
                                if (tswp->num_secondary_banks > 0) {
                                        ...
                                        if (!(En_objp->flags[Object::Object_Flags::Protected])) {
                                                ...
                                                if (current_bank > -1) {
                                                        weapon_info *swip = &Weapon_info[tswp->secondary_bank_weapons
                                                                                                 [current_bank]];
                                                        if (aip->ai_flags
                                                        ...
                                                        if (timestamp_elapsed(
                                                                ...
                                                                if (current_bank >= 0) {
                                                                        ...
                                                                        if (swip->wi_flags
                                                                                ...
                                                                                firing_range = swip->lssm_lock_range; // that
                                                                                                                      // should be
                                                                                                                      // enough
                                                                        else if (swip->wi_flags
                                                                                         [Weapon::Info_Flags::Bomb])
                                                                                firing_range = MIN(
                                                                                        (swip->max_speed * swip->lifetime * 0.75f),
                                                                                        swip->weapon_range);
                                                                        else {
                                                                                ...
                                                                                firing_range = MIN(
                                                                                        (swip->max_speed * swip->lifetime * secondary_range_mult),
                                                                                        swip->weapon_range);
                                                                        }
                                                                        ...
                                                                        if (swip->wi_flags
                                                                                ...
                                                                                if (count > 3)
                                                                                        ...
                                                                                        if (fs2::prng::randf(0) < 0.25f / (30.0f * hull_percent) * count) // With timestamp
                                                                                                                                                          // below, this means
                                                                                                                                                          // could fire in 30
                                                                                                                                                          // seconds if one
                                                                                                                                                          // enemy.
                                                                                ...
                                                                                if (ai_fire_secondary_weapon(
                                                                                        ...
                                                                                        if ((aip->ai_flags
                                                                                            ...
                                                                                            || (swip->burst_flags
                                                                                                        [Weapon::Burst_Flags::
                                                                                                                 Fast_firing])) {
                                                                                                if (swip->burst_shots > swp->burst_counter
...
{
        ASSERTX(
                global_p0 != nullptr && global_p1 != nullptr && global_p0_norm != nullptr && objp != nullptr && pm != nullptr,
                "arguments cannot be null!");
        ...
        int dockee_index, int dock_mode, rotating_dockpoint_info *rdinfo)
{
        ...
        if (dock_mode != DOA_DOCK_STAY)
                if (ship_get_subsystem_strength(
                            &Ships[docker_objp->instance], SUBSYSTEM_ENGINE)
                    <= 0.0f)
                        return 9999.9f;
        ...
        case DOA_UNDOCK_3: {
                ...
                if (vm_vec_dist_quick(&goal_point, &dockee_objp->pos) < vm_vec_dist_quick(&docker_objp->pos, &dockee_objp->pos))
                        ...
                        float dot, accel;
                        float turn_time = Ship_info[Ships[docker_objp->instance].ship_info_index]
                                                  .srotation_time;
                        ai_turn_towards_vector(
             ...
             so = GET_NEXT(so)) {
                ...
                if (iff_x_attacks_y(
                        ...
                        if (attacking_aip->target_objnum == target_objnum) {
                                if (((Game_mode & GM_MULTIPLAYER) && (attacking_objp->flags[Object::Object_Flags::Player_ship])) || (attacking_aip->mode == AIM_CHASE)) {
                                        count++;
                                }
                        }
                }
...
void remove_farthest_attacker(int objnum)
{
        ...
        if (farthest_objp != NULL) {
                ...
                ASSERT(
                        (farthest_objp->instance > -1) && (farthest_objp->instance < MAX_SHIPS));
...
int ai_maybe_limit_attackers(int attacked_objnum)
{
        ...
        if (Objects[attacked_objnum].flags[Object::Object_Flags::Player_ship]) {
                ...
                if (num_attacking == The_mission.ai_profile->max_attackers[Game_skill_level]) {
                        ...
                        num_attacking > The_mission.ai_profile->max_attackers[Game_skill_level]) {
                        rval = 1;
                }
        }
...
void guard_object_was_hit(object *guard_objp, object *hitter_objp)
{
        ...
        ASSERT(
                (hitter_objp->type == OBJ_SHIP) || (hitter_objp->type == OBJ_ASTEROID) || (hitter_objp->type == OBJ_WEAPON));
                ...
                if (objp->instance != -1) {
                        ...
                        if ((aip->mode == AIM_GUARD) || (aip->active_goal == AI_ACTIVE_GOAL_DYNAMIC)) {
                                if (aip->guard_objnum == OBJ_INDEX(hit_objp)) {
                                        ...
                                        (aip->guard_wingnum != -1) && (aip->guard_wingnum == Ai_info[Ships[hit_objp->instance].ai_index].wing)) {
                                        guard_object_was_hit(objp, hitter_objp);
                                }
                        }
                }
                ...
                if (iff_x_attacks_y(guarding_shipp->team, eshipp->team)) {
                        ...
                        if (Ship_info[eshipp->ship_info_index].class_type >= 0 && (Ship_types[Ship_info[eshipp->ship_info_index].class_type].flags[Ship::Type_Info_Flags::AI_guards_attack])) {
                                ...
                                if (dist < (MAX_GUARD_DIST + guarded_objp->radius) * 3) {
                                        ...
                                        (dist < 3000.0f) && (Ai_info[eshipp->ai_index].target_objnum == guarding_aip->guard_objnum)) {
                                        guard_object_was_hit(guarding_objp, enemy_objp);
                                }
                        }
                }
                        ...
                        if (speed > 10.0f) {
                                ...
                                if (vm_vec_dist_quick(&goal_point, &Pl_objp->pos) > 40.0f) {
                                        if (vm_vec_dot(&Pl_objp->orient.vec.fvec, &v2g) < 0.0f) {
                                                ...
                                                set_accel_for_target_speed(
                                                        Pl_objp, guard_objp->phys_info.speed - dist_to_goal_point / 10.0f);
                                        } else {
                                                ...
                                                set_accel_for_target_speed(
                                                        Pl_objp, guard_objp->phys_info.speed + (dist_to_goal_point - 40.0f) / 20.0f);
                                        }
                                } else {
                                        if (dot_to_goal_point > 0.8f) {
                                                turn_towards_point(Pl_objp, &goal_point, NULL, 0.0f);
                                                set_accel_for_target_speed(
                                                        Pl_objp, guard_objp->phys_info.speed + dist_to_goal_point * 0.1f);
                                        } else {
                                                set_accel_for_target_speed(
                                                        Pl_objp, guard_objp->phys_info.speed - dist_to_goal_point * 0.1f - 1.0f);
                                        }
                                }
                                ...
                                dist_to_guardobj > MAX_GUARD_DIST + Pl_objp->radius + guard_objp->radius) {
                                ...
                                if ((dist_to_guardobj > guard_objp->radius + Pl_objp->radius + 50.0f) && (guard_objp->phys_info.speed > Pl_objp->phys_info.speed - 1.0f))
                                        set_accel_for_target_speed(
                                                Pl_objp, (0.5f * (1.0f + dot)) * (guard_objp->phys_info.speed + (dist_to_guardobj - guard_objp->radius - Pl_objp->radius) / 10.0f));
                                else
                                        accelerate_ship(
        ...
        object *repaired_objp, object *repair_objp, int how)
{
        ...
        case REPAIR_INFO_BEGIN:
                ...
                if (repaired_objp->flags[Object::Object_Flags::Player_ship]) {
                        ...
                        Player->stats.m_score -= The_mission.ai_profile->repair_penalty[Game_skill_level];
                }
                break;
...
void ai_dock()
{
        ...
        // exists.
        if (((aip->active_goal < 0) && (aip->submode != AIS_DOCK_4A)) || (aip->goal_objnum < 0) || (Objects[aip->goal_objnum].signature != aip->goal_signature)) {
        ...
        case AIS_DOCK_1: {
                ...
                if ((r = maybe_dock_obstructed(Pl_objp, goal_objp, 1)) != -1) {
                        ...
                        if ((r1 = maybe_avoid_big_ship(
                                     Pl_objp, goal_objp, aip, &goal_objp->pos, 7.0f))
                            != 0) {
        ...
        case AIS_DOCK_3: {
                ...
                if ((r = maybe_dock_obstructed(Pl_objp, goal_objp, 0)) != -1) {
                        ...
                        float tolerance = 2 * flFrametime * (1.0f + sqrtf(goal_objp->phys_info.speed));
                                ...
                                if (aip->mode != AIM_BAY_DEPART) {
                                        ai_attack_object(
                                                objp, NULL, NULL);        // Regardless of current mode,
                                                                          // enter attack mode.
                                        aip->submode = SM_ATTACK_FOREVER; // Never leave attack submode,
...
#define DEFAULT_WING_MAG \
        (sqrtf(          \
                DEFAULT_WING_X_DELTA * DEFAULT_WING_X_DELTA + DEFAULT_WING_Y_DELTA * DEFAULT_WING_Y_DELTA + DEFAULT_WING_Z_DELTA * DEFAULT_WING_Z_DELTA))
        ...
        int formation_object_flag)
{
        ...
        wing_spread_size = MAX(
                50.0f,
                3.0f * get_wing_largest_radius(leader_objp, formation_object_flag) + 15.0f);
        ...
        if (objp->type == OBJ_SHIP) {
                ...
                if (Ai_info[Ships[objp->instance].ai_index].wing == infop->parameter_variables.int_value) {
                        ...
                        // signature - if so, he is now the most massive object
                                ...
                                objp->phys_info.mass == infop->maintained_variables.objp_value->phys_info.mass) {
...
int ai_formation()
{
        ...
        ASSERT(!(
                aip->ai_flags[AI::AI_Flags::Formation_wing] && aip->ai_flags[AI::AI_Flags::Formation_object])); // Make sure not both
                                                                                                                // types of formation
                                                                                                                // flying in effect.
        ...
        if (dist_to_goal > 500.0f) {
                ...
                if (dot_to_goal > -0.5f) {
                        ...
                        if (range_speed > 0.0f) {
                                set_accel_for_target_speed(
                                        Pl_objp, leader_speed + range_speed * (dist_to_goal + 100.0f) / 500.0f);
                                if (Pl_objp->phys_info.flags & PF_AFTERBURNER_ON)
                                        afterburners_stop(Pl_objp);
                        } else {
                ...
                if ((dist = vm_vec_dist_quick(&objp->pos, &weapon_objp->pos)) < weapon_objp->phys_info.speed * 2.0f) {
                        ...
                        if (fire_chance < r) {
                                shipp->cmeasure_fire_stamp = timestamp(
                                        CMEASURE_WAIT + (int)(fire_chance * 2000)); // Wait 1/2 second
...
void ai_chase_circle(object *objp)
{
        ...
        ASSERT(
                vm_vec_mag(&aip->goal_point) >= 0.0f); // Supposedly detects bogus vector
                ...
                if ((missile_objp->type == OBJ_WEAPON) && (Weapon_info[Weapons[missile_objp->instance].weapon_info_index].is_homing())) {
                        ...
                        if (dist < dist2) {
                                ...
                                case AIM_CHASE:
                                        ...
                                        if (((((Missiontime >> 18) ^ OBJ_INDEX(objp)) & 3) == 0) || (objp->phys_info.speed < 40.0f) || (fs2::prng::randf(0) < 1.0f - (float)shipp->cmeasure_count / 8.0f)) {
                                                if (aip->submode != SM_ATTACK_FOREVER) { // SM_ATTACK_FOREVER means
                                                                                         // engines blown.
                        ...
                        if ((aip->support_ship_objnum == -1) && (get_hull_pct(objp) < 0.25f)) {
                                if (!shipp->is_departing()) {
                                        if (!mission_do_departure(objp)) {
                                                ...
                                                aip->warp_out_timestamp = timestamp((int)((30 + 10 * fs2::prng::randf(0)) * 1000));
                                        }
                                }
                        }
                }
...
void ai_warp_out(object *objp)
{
        ...
        case AIS_WARP_2: // Make sure won't collide with any object.
                if (timestamp_elapsed(aip->force_warp_time) || (!collide_predict_large_ship(objp, objp->radius * 2.0f + 100.0f) || (Ship_info[shipp->ship_info_index].warpout_type == WT_HYPERSPACE && !collide_predict_large_ship(objp, 100000.0f)))) {
        ...
        case AIS_WARP_3:
                ...
                if (timestamp_elapsed(aip->force_warp_time) || (fabsf(objp->phys_info.speed - goal_speed) < 2.0f)) {
                        aip->submode = AIS_WARP_4;
                        aip->submode_start_time = Missiontime;
                }
                break;
        ...
        for (so = GET_NEXT(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
             so = GET_NEXT(so)) {
                ...
                if (shipp->flags[Ship::Ship_Flags::Dying]) {
                        ...
                        if (damage >= EVADE_SHOCKWAVE_DAMAGE_THRESHOLD) { // Only evade quite large
                                                                          // blasts
                                float dist;
...
void ai_maybe_self_destruct(object *objp, ai_info *aip)
{
        ...
        if ((Ship_info[shipp->ship_info_index].is_small_ship()) && (shipp->wingnum >= 0) && !(shipp->flags[Ship::Ship_Flags::No_disabled_self_destruct])) {
                if ((ship_get_subsystem_strength(shipp, SUBSYSTEM_ENGINE) <= 0.0f) || (ship_get_subsystem_strength(shipp, SUBSYSTEM_WEAPONS) <= 0.0f)) {
...
void ai_frame(int objnum)
{
        ...
        ASSERT(
                (aip->mode != AIM_WAYPOINTS) || (aip->active_goal != AI_ACTIVE_GOAL_DYNAMIC));
...
void init_ai_object(int objnum)
{
        ...
        aip->rearm_first_ballistic_primary = TRUE; // flag to indicate that next ballistic to load is the first
                                                   // ballistic
        ...
        aip->choose_enemy_timestamp = timestamp(
                3 * (NUM_SKILL_LEVELS - Game_skill_level) * ((fs2::prng::randi(0) % 500) + 500));
        ...
        object *objp_hitter, object *objp_hit, object *objp_weapon)
{
        ...
        if ((obj_team(objp_hitter) == obj_team(objp_hit)) && (objp_hitter == Player_obj)) {
                ...
                ASSERT(
                        (objp_weapon->type == OBJ_WEAPON) || (objp_weapon->type == OBJ_BEAM)); // beam added for traitor detection - FUBAR
                ...
                if (objp_weapon->type == OBJ_BEAM) // added beam for traitor detection -FUBAR
                        ...
                        damage = Weapon_info[Weapons[objp_weapon->instance].weapon_info_index]
                                         .damage;
                ...
                if (is_instructor(objp_hit)) {
                        ...
                        pp->friendly_damage > FRIENDLY_DAMAGE_THRESHOLD * (1.0f + (float)(NUM_SKILL_LEVELS + 1 - Game_skill_level) / 3.0f)) {
                        ...
                        (damage > fs2::prng::randf(0)) && (Missiontime - pp->last_warning_message_time > F1_0 * 4) && (pp->friendly_damage > FRIENDLY_DAMAGE_THRESHOLD)) {
...
void ai_ship_hit(object *objp_ship, object *hit_objp, vec3d *hit_normal)
{
        ...
        if ((hit_objp->type == OBJ_WEAPON) || (hit_objp->type == OBJ_BEAM)) {
                ...
                if (shipp->team == Ships[hit_objp->instance].team) // Don't have AI react to
...
void ai_ship_destroy(int shipnum)
{
        ...
        ASSERT(
                (Ships[shipnum].ai_index >= 0) && (Ships[shipnum].ai_index < MAX_AI_INFO));
...
int ai_abort_rearm_request(object *requester_objp)
{
        ...
        ASSERT(
                (requester_objp->instance >= 0) && (requester_objp->instance < MAX_SHIPS));
        if ((requester_objp->instance < 0) || (requester_objp->instance >= MAX_SHIPS)) {
                return 0;
        }
        requester_shipp = &Ships[requester_objp->instance];
        ASSERT(
                (requester_shipp->ai_index >= 0) && (requester_shipp->ai_index < MAX_AI_INFO));
