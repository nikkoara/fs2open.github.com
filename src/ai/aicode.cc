 ...
 *
 * The code in Ai.cpp is just for bookkeeping, allocating AI slots and linking
 * them to ships.
 */
...
#define CIRCLE_STRAFE_MAX_DIST \
    300.0f // Maximum distance for circle strafe behavior.
...
const char* Mode_text[MAX_AI_BEHAVIORS] = {
    ...
    "STRAFE",   "PLAY_DEAD", "BAY_EMERGE", "BAY_DEPART", "SENTRYGUN",
    "WARP_OUT",
};

// Submode text is only valid for CHASE mode.
const char* Submode_text[] = { "undefined", "CONT_TURN", "ATTACK   ",
                               "E_SQUIG  ", "E_BRAKE  ", "EVADE    ",
                               "SUP_ATTAK", "AVOID    ", "BEHIND   ",
                               "GET_AWAY ", "E_WEAPON ", "FLY_AWAY ",
                               "ATK_4EVER", "STLTH_FND", "STLTH_SWP",
                               "BIG_APPR",  "BIG_CIRC",  "BIG_PARL" };

const char* Strafe_submode_text[5] = { "ATTACK", "AVOID", "RETREAT1",
                                       "RETREAT2", "POSITION" };
// XSTR:ON
...
#define STEALTH_MAX_VIEW_DIST \
    400 // dist at which 1) stealth no longer visible 2) firing inaccuracy is
        // greatest
...
ai_flag_name Ai_flag_names[] = {
    {
        AI::AI_Flags::No_dynamic,
        "no-dynamic",
    },
    {
        AI::AI_Flags::Free_afterburner_use,
...
#error Number of skill levels is wrong!
    ...
    if (translate) {
        switch (level) {
        case 0: str = XSTR ("Very Easy", 469); break;
        case 1: str = XSTR ("Easy", 470); break;
        ...
        switch (level) {
        case 0: str = NOX ("Very Easy"); break;
        case 1: str = NOX ("Easy"); break;
...
#define DELAY_TARGET_TIME \
    (12 * 1000) // time in milliseconds until a ship can target a new enemy
                // after an order.
...
int Ai_last_arrive_path; // index of ship_bay path used by last arrival from a
                         // fighter bay

// forward declarations
int ai_return_path_num_from_dockbay (object* dockee_objp, int dockbay_index);
void create_model_exit_path (
    object* pl_objp, object* mobjp, int path_num, int count = 1);
void copy_xlate_model_path_points (
    object* objp, model_path* mp, int dir, int count, int path_num, pnode* pnp,
    int randomize_pnt = -1);
...
/**
 * "safe" is currently defined by the mission designer using the good/bad time
 * to rearm sexpressions.  This status is currently team based.
 *
 * @return true(1) or false(0) if it is "safe" for the given object to rearm.
 * @todo This function could be easily expended to further the definition of
 * "safe"
 */
 ...
 */
void ai_good_secondary_time (
    int team, int weapon_index, int max_fire_count, char* shipname) {
...
/**
 * Called internally to the AI code to tell whether or not weapon_num can be
 * fired from firer_objp at target_objp.  This function will resolve the team
 * for the firer.
 *
 * @return -1 when conditions don't allow firer to fire weapon_num on
 * target_objp
 * @return >=0 when conditions allow firer to fire.  Return value is max number
 * of weapon_nums which can be fired on target_objp
 */
int is_preferred_weapon (
    int weapon_num, object* firer_objp, object* target_objp) {
    ...
    for (hfi = Ai_huge_fire_info.begin (); hfi != Ai_huge_fire_info.end ();
         ++hfi) {
        int ship_index, signature;

        if (hfi->weapon_index == -1) continue;

        ship_index = ship_name_lookup (hfi->shipname);
        if (ship_index == -1) continue;

        signature = Objects[Ships[ship_index].objnum].signature;

        // sigatures, weapon_index, and team must match
        if ((signature == target_signature) &&
            (hfi->weapon_index == weapon_num) && (hfi->team == firer_team))
            break;
    }

    // return -1 if not found
    if (hfi == Ai_huge_fire_info.end ()) return -1;

    // otherwise, we can return the max number of weapons we can fire against
    // target_objps
 ...
 */
void ai_init_secondary_info () { Ai_huge_fire_info.clear (); }

/**
 * Garbage collect the ::Path_points buffer.
 *
 * Scans all objects, looking for used Path_points records.
 *      Compresses Path_points buffer, updating aip->path_start and aip->path_cur
 *indices. Updates Ppfp to point to first free record. This function is fairly
 *fast.  Its worst-case running time is proportional to 3*MAX_PATH_POINTS +
 *MAX_OBJECTS
 *
 * @todo Things to do to optimize this function:
 *              1. if (t != 0) xlt++; can be replaced by xlt += t; assuming t can only
 *be 0 or 1.
 *              2. When pp_xlate is getting stuffed the first time, note highest index
 *and use that instead of MAX_PATH_POINTS in following two for loops.
 */
void garbage_collect_path_points () {
    ...
    for (i = 0; i < MAX_PATH_POINTS; i++) pp_xlate[i] = 0;
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (shipp->ai_index != -1) {
            ...
            if ((aip->path_length > 0) && (aip->path_start > -1)) {
                for (i = aip->path_start;
                     i < aip->path_start + aip->path_length; i++) {
                    ASSERT (pp_xlate[i] == 0); // If this is not 0, then two
                                               // paths use this point!
                    pp_xlate[i] = 1;
                }
            }
        }
    ...
    // Now, stuff xlate index in pp_xlate.  This is the number to translate
    // any path_start   or path_cur index to.
    int xlt = 0;
    ...
    // Now, using pp_xlate, fixup all aip->path_cur and aip->path_start
    // indices
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
         so = GET_NEXT (so)) {
        A = &Objects[so->objnum];
        ...
        if (shipp->ai_index != -1) {
            ...
            if ((aip->path_length > 0) && (aip->path_start > -1)) {
                ...
                ASSERT (
                    (aip->path_cur >= 0) && (aip->path_cur < MAX_PATH_POINTS));
    ...
    for (i = 0; i < MAX_PATH_POINTS; i++)
        if (i != pp_xlate[i]) Path_points[pp_xlate[i]] = Path_points[i];
}
 ...
 * Boolean overrides are unset, others initialized to FLT_MIN or INT_MIN (used
 * as the "not set" state)
 */
void init_ai_class (ai_class* aicp) {
    ...
    aicp->ai_class_autoscale =
        true; // Retail behavior is to do the stupid autoscaling
}
...
void parse_ai_class () {
    ...
    if (optional_string ("$AI Always Links Energy Weapons:"))
        parse_float_list (
            aicp->ai_link_energy_levels_always, NUM_SKILL_LEVELS);
    ...
    if (optional_string ("$Glide Attack Percent:")) {
        ...
        for (int i = 0; i < NUM_SKILL_LEVELS; i++) {
            if (aicp->ai_glide_attack_percent[i] < 0.0f ||
                ...
                WARNINGF (LOCATION,"$Glide Attack Percent should be between 0 and 100.0 (read %f). Setting to 0.",aicp->ai_glide_attack_percent[i]);
            }
    ...
    if (optional_string ("$Circle Strafe Percent:")) {
        ...
        for (int i = 0; i < NUM_SKILL_LEVELS; i++) {
            if (aicp->ai_circle_strafe_percent[i] < 0.0f ||
                ...
                WARNINGF (LOCATION,"$Circle Strafe Percent should be between 0 and 100.0 (read %f). Setting to 0.",aicp->ai_circle_strafe_percent[i]);
            }
    ...
    if (optional_string ("$Glide Strafe Percent:")) {
        ...
        for (int i = 0; i < NUM_SKILL_LEVELS; i++) {
            if (aicp->ai_glide_strafe_percent[i] < 0.0f ||
                ...
                WARNINGF (LOCATION,"$Glide Strafe Percent should be between 0 and 100.0 (read %f). Setting to 0.",aicp->ai_glide_strafe_percent[i]);
            }
    ...
    if (optional_string ("$Random Sidethrust Percent:")) {
        parse_float_list (
            aicp->ai_random_sidethrust_percent, NUM_SKILL_LEVELS);
        for (int i = 0; i < NUM_SKILL_LEVELS; i++) {
            if (aicp->ai_random_sidethrust_percent[i] < 0.0f ||
                ...
                WARNINGF (LOCATION,"$Random Sidethrust Percent should be between 0 and 100.0 (read %f). Setting to 0.",aicp->ai_random_sidethrust_percent[i]);
            }
    ...
    if (optional_string ("$Chance AI Has to Fire Missiles at Player:"))
        parse_int_list (
            aicp->ai_chance_to_use_missiles_on_plr, NUM_SKILL_LEVELS);
    ...
    if (optional_string ("$Turret Max Aim Update Delay:"))
        parse_float_list (
            aicp->ai_turret_max_aim_update_delay, NUM_SKILL_LEVELS);

    set_aic_flag (
        aicp, "$big ships can attack beam turrets on untargeted ships:",
        AI::Profile_Flags::
            Big_ships_can_attack_beam_turrets_on_untargeted_ships);

    set_aic_flag (
        aicp, "$smart primary weapon selection:",
        AI::Profile_Flags::Smart_primary_weapon_selection);

    set_aic_flag (
        aicp, "$smart secondary weapon selection:",
        AI::Profile_Flags::Smart_secondary_weapon_selection);

    set_aic_flag (
        aicp, "$smart shield management:",
        AI::Profile_Flags::Smart_shield_management);

    set_aic_flag (
        aicp, "$smart afterburner management:",
        AI::Profile_Flags::Smart_afterburner_management);

    set_aic_flag (
        aicp, "$allow rapid secondary dumbfire:",
        AI::Profile_Flags::Allow_rapid_secondary_dumbfire);

    set_aic_flag (
        aicp, "$huge turret weapons ignore bombs:",
        AI::Profile_Flags::Huge_turret_weapons_ignore_bombs);

    set_aic_flag (
        aicp, "$don't insert random turret fire delay:",
        AI::Profile_Flags::Dont_insert_random_turret_fire_delay);

    set_aic_flag (
        aicp, "$prevent turrets targeting too distant bombs:",
        AI::Profile_Flags::Prevent_targeting_bombs_beyond_range);

    set_aic_flag (
        aicp, "$smart subsystem targeting for turrets:",
        AI::Profile_Flags::Smart_subsystem_targeting_for_turrets);

    set_aic_flag (
        aicp, "$allow turrets target weapons freely:",
        AI::Profile_Flags::Allow_turrets_target_weapons_freely);

    set_aic_flag (
        aicp,
        "$allow vertical dodge:", AI::Profile_Flags::Allow_vertical_dodge);

    set_aic_flag (
        aicp, "$no extra collision avoidance vs player:",
        AI::Profile_Flags::No_special_player_avoid);

    set_aic_flag (
        aicp, "$all ships manage shields:",
        AI::Profile_Flags::All_ships_manage_shields);

    set_aic_flag (
        aicp, "$ai can slow down when attacking big ships:",
        AI::Profile_Flags::Ai_can_slow_down_attacking_big_ships);

    set_aic_flag (
        aicp, "$use actual primary range:",
        AI::Profile_Flags::Use_actual_primary_range);
}
...
void ai_level_init () {
    ...
    for (i = 0; i < MAX_AI_INFO; i++) { Ai_info[i].shipnum = -1; }
...
int ai_is_stealth_visible (object* viewer_objp, object* stealth_objp) {
    ...
    if (!ship_is_visible_by_team (
            stealth_objp, &Ships[viewer_objp->instance])) {
        vm_vec_sub (&vec_to_stealth, &stealth_objp->pos, &viewer_objp->pos);
        dist_to_stealth = vm_vec_mag_quick (&vec_to_stealth);
        dot_to_stealth =
            vm_vec_dot (&viewer_objp->orient.vec.fvec, &vec_to_stealth) /
            dist_to_stealth;

        // get max dist at which stealth is visible
        max_stealth_dist =
            get_skill_stealth_dist_scaler () * STEALTH_MAX_VIEW_DIST;

        // now check if within view frustum
        float needed_dot_to_stealth;
        if (dist_to_stealth < 100) { needed_dot_to_stealth = 0.0f; }
        else {
            needed_dot_to_stealth = get_skill_stealth_dot_scaler () *
                                    float(STEALTH_VIEW_CONE_DOT) *
                                    (dist_to_stealth / max_stealth_dist);
        }
 ...
 */
float compute_dots (
    object* objp, object* other_objp, float* to_dot, float* from_dot) {
    vec3d v2o;
    float dist;
 ...
 *
 * This is a "cheat" update. Error increases with time not seen, true distance
 * away, dot to enemy this is done only if we can not see the stealth target
...
/**
 * Update danger_weapon_objnum and signature in ai_info to say this weapon is
 * to be avoided.
 */
void ai_update_danger_weapon (int attacked_objnum, int weapon_objnum) {
    ...
    if ((attacked_objnum == -1) || (weapon_objnum == -1)) { return; }

    objp = &Objects[attacked_objnum];

    // AL 2-24-98: If this isn't a ship, we don't need to worry about updating
    // weapon_objnum (ie it would be
    // an asteroid or bomb).
    ...
    if (aip->target_objnum >= 0) {
        if (is_object_stealth_ship (&Objects[aip->target_objnum])) {
            if (ai_is_stealth_visible (objp, &Objects[aip->target_objnum]) ==
                STEALTH_NOT_IN_FRUSTUM) {
    ...
    if (aip->danger_weapon_objnum != -1) {
        ...
        if ((old_weapon_objp->type == OBJ_WEAPON) &&
            (old_weapon_objp->signature == aip->danger_weapon_signature)) {
            ;
        }
        else {
...
void ai_turn_towards_vector (
    vec3d* dest, object* objp, float turn_time, vec3d* slide_vec,
    vec3d* rel_pos, float bank_override, int flags, vec3d* rvec,
    int sexp_flags) {
    matrix curr_orient;
    ...
    if ((objp->type == OBJ_SHIP) && !(sexp_flags & AITTV_VIA_SEXP)) {
        if (ship_get_subsystem_strength (
                &Ships[objp->instance], SUBSYSTEM_ENGINE) <= 0.0f)
            return;
    }
    ...
    if (turn_time > 0.0f) {
        ...
        if (!(flags & AITTV_FAST) && !(sexp_flags & AITTV_VIA_SEXP)) {
            if (objp->type == OBJ_SHIP) {
                if (iff_x_attacks_y (
                        Player_ship->team, Ships[objp->instance].team)) {
                    turn_time *= Ai_info[Ships[objp->instance].ai_index]
                                     .ai_turn_time_scale;
                }
            }
        }
    ...
    acc_limit = vel_limit;
    if (objp->type == OBJ_WEAPON) vm_vec_scale (&acc_limit, 8.0f);
    ...
    // Since ship isn't necessarily moving in the direction it's pointing,
    // sometimes it's better    to be moving towards goal rather than just
    // pointing.  So, if slide_vec is !NULL, try to     make ship move towards
    // goal, not point at goal.
    if (slide_vec != NULL) {
        vm_vec_add2 (&desired_fvec, slide_vec);
        vm_vec_normalize (&desired_fvec);
    }

    // Should be more general case here.  Currently, anything that is not a
    // weapon will bank when it turns.
    ...
    if ((objp->type == OBJ_WEAPON) || (sexp_flags & AITTV_IGNORE_BANK))
        ...
        (bank_override) &&
        (iff_x_attacks_y (
            Ships[objp->instance].team,
            Player_ship->team))) { // Theoretically, this will only happen
                                   // for Shivans.
        delta_bank = bank_override;
    }
    else {
        delta_bank =
            vm_vec_dot (&curr_orient.vec.rvec, &objp->last_orient.vec.rvec);
        delta_bank = 100.0f * (1.0f - delta_bank);
        if (vm_vec_dot (&objp->last_orient.vec.fvec, &objp->orient.vec.rvec) <
            0.0f)
            delta_bank = -delta_bank;
    }

    // Dave Andsager: The non-indented lines here are debug code to help you
    // track down the problem in the physics    that is causing ships to
    // inexplicably rotate very far.  If you see the message below in the log,
    // set the next statement to be     the one marked "HERE".  (Do this
    // clicking the cursor there, then right clicking.  Choose the right
    // option.)         This will allow you to rerun vm_forward_interpolate() with
    // the values that caused the error.
#ifndef NDEBUG
    ...
    if (rvec != NULL) {
        ...
        vm_matrix_interpolate (
            &goal_orient, &curr_orient, &vel_in, delta_time, &out_orient,
            &vel_out, &vel_limit, &acc_limit);
        objp->orient = out_orient;
    }
    else {
        vm_forward_interpolate (
            &desired_fvec, &curr_orient, &vel_in, delta_time, delta_bank,
            &objp->orient, &vel_out, &vel_limit, &acc_limit);
    }

#ifndef NDEBUG
    if (!((objp->type == OBJ_WEAPON) &&
          (Weapon_info[Weapons[objp->instance].weapon_info_index].subtype ==
           WP_MISSILE))) {
        if (delta_time < 0.25f &&
            vm_vec_dot (&objp->orient.vec.fvec, &tvec) < 0.1f)
            WARNINGF (LOCATION,"A ship rotated too far. Offending vessel is %s, please investigate.",Ships[objp->instance].ship_name);
    }
#endif
    pip->rotvel = vel_out;
}
...
int set_target_objnum (ai_info* aip, int objnum) {
    if ((aip != Player_ai) &&
        (!timestamp_elapsed (aip->ok_to_target_timestamp))) {
    ...
    if (aip->target_objnum == objnum) {
        ...
        if ((Game_mode & GM_MULTIPLAYER) && (aip == Player_ai) &&
            (Player_obj->type == OBJ_OBSERVER)) {}
        else {
            ASSERT (
                objnum !=
                Ships[aip->shipnum].objnum); // make sure not targeting self
        }
        ...
        aip->time_enemy_near = 0.0f; // SUSHI: Reset the "time near" counter
                                     // whenever the target is changed
        aip->last_hit_target_time =
            Missiontime; // Also, the "last hit target" time should be reset
...
int ai_select_primary_weapon (
    object* objp, object* other_objp, Weapon::Info_Flags flags);

/**
 * Make new_subsys the targeted subsystem of ship *aip.
 */
ship_subsys* set_targeted_subsys (
    ai_info* aip, ship_subsys* new_subsys, int parent_objnum) {
    ASSERT (aip != NULL);
        ...
        if (new_subsys->system_info->type == SUBSYSTEM_ENGINE) {
            if (aip != Player_ai) {
                ...
                ai_select_primary_weapon (
                    &Objects[Ships[aip->shipnum].objnum],
                    &Objects[parent_objnum], Weapon::Info_Flags::Puncture);
                ship_primary_changed (
                    &Ships[aip->shipnum]); // AL: maybe send multiplayer
                                           // information when AI ship changes
                                           // primaries
            }
        }

        if (aip == Player_ai) { hud_lock_reset (0.5f); }
    }
    else {
...
/**
 * If *aip is docked, set max acceleration to A->mass/(A->mass + B->mass) where
 * A is *aip and B is dock object
 */
void adjust_accel_for_docking (ai_info* aip) {
    ...
    if (object_is_docked (objp)) {
        float ratio =
            objp->phys_info.mass / dock_calc_total_docked_mass (objp);

        // put cap on how much ship can slow down
        if ((ratio < 0.8f) &&
            !(The_mission.ai_profile
                  ->flags[AI::Profile_Flags::No_min_dock_speed_cap])) {
            ratio = 0.8f;
        }
        ...
        if (AI_ci.forward > ratio) { AI_ci.forward = ratio; }
    }
}
...
void change_acceleration (ai_info* aip, float delta_accel) {
    ...
    if (delta_accel < 0.0f) {
        if (aip->prev_accel > 0.0f) aip->prev_accel = 0.0f;
    }
...
void set_accel_for_target_speed (object* objp, float tspeed) {
    ...
    if (max_speed > 0.0f) { AI_ci.forward = tspeed / max_speed; }
    else {
        AI_ci.forward = 0.0f;
    }
...
/**
 * Stuff perim_point with a point on the perimeter of the sphere defined by
 * object *objp on the vector from the center of *objp through the point *vp.
 */
void project_point_to_perimeter (
    vec3d* perim_point, vec3d* pos, float radius, vec3d* vp) {
    vec3d v1;
    float mag;
    ...
    if (mag == 0.0f) {
        WARNINGF (LOCATION, "projectable point is at center of sphere.");
 ...
 * @param *p1           is another point in space to define the plane in which
 * tan1, tan2 reside.
 * @param radius        is the radius of the sphere.
 *
 * Note, this is a very approximate function just for AI.
 * Note also: On 12/26/96, p1 is used to define the plane perpendicular to that
 * which contains the tangent point.
 */
void get_tangent_point (
    vec3d* tan1, vec3d* p0, vec3d* centerp, vec3d* p1, float radius) {
    ...
    if (vm_vec_dist (p0, centerp) < radius)
        ...
        if (ratio < 2.0f)
            ...
            vm_vec_scale_add (
                &dest_vec, &v2c, &perp_vec, (1.0f + 1.0f / ratio));
 ...
 * approach behavior.
 */
void turn_towards_point (
    object* objp, vec3d* point, vec3d* slide_vec, float bank_override) {
    ai_info* aip;
    aip = &Ai_info[Ships[Pl_objp->instance].ai_index];

    // check if in formation and if not leader, don't change rotvel.z (bank to
    // match leader elsewhere)
    if (aip->ai_flags
            [AI::AI_Flags::Formation_object, AI::AI_Flags::Formation_wing]) {
        if (&Objects[aip->goal_objnum] != objp) {
            ...
            ai_turn_towards_vector (
                ...
                Ship_info[Ships[objp->instance].ship_info_index]
                    .srotation_time,
                slide_vec, nullptr, bank_override, 0);
        ...
        ai_turn_towards_vector (
            point, objp,
            Ship_info[Ships[objp->instance].ship_info_index].srotation_time,
            slide_vec, nullptr, bank_override, 0);
    }
}

/**
 * Given an object and a point, turn away from the point, resulting in
 * avoidance behavior. Note: Turn away at full speed, not scaled down by skill
 * level.
 */
void turn_away_from_point (object* objp, vec3d* point, float bank_override) {
    ...
    ai_turn_towards_vector (
        &opposite_point, objp,
        Ship_info[Ships[objp->instance].ship_info_index].srotation_time,
        nullptr, nullptr, bank_override, AITTV_FAST);
}
...
// Note that if you want to circle an object without hitting it, you should
// use  about twice that object's radius for radius, else you'll certainly bump
// into it.     Return dot product to goal point.
float turn_towards_tangent (object* objp, vec3d* point, float radius) {
    vec3d vec_to_point;
    vec3d goal_point;
    vec3d perp_point; // point radius away from *point on vector to
                      // objp->pos
...
float turn_toward_tangent_with_axis (
    object* objp, object* center_objp, float radius) {
    ...
    vm_vec_scale_add (
        &center_vec, &center_objp->pos, &center_objp->orient.vec.fvec,
        center_obj_z);
...
/**
 * Returns a point radius units away from *point that *objp should turn towards
 * to orbit *point
 */
void get_tangent_point (
    vec3d* goal_point, object* objp, vec3d* point, float radius) {
    vec3d vec_to_point;
    vec3d perp_point; // point radius away from *point on vector to
                      // objp->pos
    vec3d up_vec, perp_vec;
...
int object_is_targetable (object* target, ship* viewer) {
    ...
    if (target->type == OBJ_SHIP) {
        if (ship_is_visible_by_team (target, viewer)) { return 1; }

        // for AI partially targetable works as fully targetable, except for
        // stealth ship
        if (Ships[target->instance].flags[Ship::Ship_Flags::Stealth]) {
            // if not team targetable, check if within frustum
            if (ai_is_stealth_visible (&Objects[viewer->objnum], target) ==
                STEALTH_IN_FRUSTUM) {
                return 1;
            }
            else {
...
int num_enemies_attacking (int objnum) {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (Ai_info[sp->ai_index].target_objnum == objnum) count++;

        // consider turrets that may be attacking objnum (but only turrets on
        // SIF_BIG_SHIP ships)
        if (Ship_info[sp->ship_info_index].is_big_ship ()) {
            // loop through all the subsystems, check if turret has objnum as a
            // target
            ssp = GET_FIRST (&sp->subsys_list);
            while (ssp != END_OF_LIST (&sp->subsys_list)) {
                if (ssp->system_info->type == SUBSYSTEM_TURRET) {
                    if ((ssp->turret_enemy_objnum == objnum) &&
                        (ssp->current_hits > 0)) {
                        count++;
                    }
                }
...
/**
 * Scan all the ships in *objp's wing. Return the lowest maximum speed of a
 * ship in the wing.
 *
...
float get_wing_lowest_max_speed (object* objp) {
    ...
    if (wingnum == -1) return lowest_max_speed;
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if ((oaip->mode == AIM_WAYPOINTS) && (oaip->wing == wingnum)) {
            // Note: If a ship in the wing has a super low max speed, probably
            // its engines are disabled.  So, fly along and     ignore the poor
            // guy.
            ...
            if ((oshipp->current_max_speed > 5.0f) &&
                (cur_max < lowest_max_speed)) {
...
/**
 * Scan all the ships in *objp's wing. Return the lowest average speed with
 * afterburner of a ship in the wing.
 */
float get_wing_lowest_av_ab_speed (object* objp) {
    ...
    if (((shipp->flags[Ship::Ship_Flags::Afterburner_locked]) ||
        ...
        (shipp->current_max_speed < 5.0f) ||
        (objp->phys_info.afterburner_max_vel.xyz.z <=
         shipp->current_max_speed) ||
        ...
        recharge_scale = sip->afterburner_recover_rate * recharge_scale /
                         (sip->afterburner_burn_rate +
                          sip->afterburner_recover_rate * recharge_scale);
        lowest_max_av_ab_speed =
            recharge_scale * (objp->phys_info.afterburner_max_vel.xyz.z -
                              shipp->current_max_speed) +
            shipp->current_max_speed;
    }

    if (wingnum == -1) return lowest_max_av_ab_speed;
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if ((oaip->mode == AIM_WAYPOINTS) && (oaip->wing == wingnum) &&
            (oaip->ai_flags
                 [AI::AI_Flags::Formation_object,
                  AI::AI_Flags::Formation_wing])) {
            float cur_max;
            if ((oshipp->flags[Ship::Ship_Flags::Afterburner_locked]) ||
                !(osip->flags[Ship::Info_Flags::Afterburner]) ||
                (o->phys_info.afterburner_max_vel.xyz.z <=
                 oshipp->current_max_speed) ||
                !(oaip->ai_flags[AI::AI_Flags::Free_afterburner_use])) {
                ...
                recharge_scale =
                    ...
                    The_mission.ai_profile
                        ->afterburner_recharge_scale[Game_skill_level];
                recharge_scale =
                    osip->afterburner_recover_rate * recharge_scale /
                    (osip->afterburner_burn_rate +
                     osip->afterburner_recover_rate * recharge_scale);
                cur_max =
                    recharge_scale * (o->phys_info.afterburner_max_vel.xyz.z -
                                      oshipp->current_max_speed) +
                    oshipp->current_max_speed;
            }
            ...
            // guy.
            if ((oshipp->current_max_speed > 5.0f) &&
                (cur_max < lowest_max_av_ab_speed)) {
...
/**
 * Determine if object objnum is supposed to be ignored by object with ai_info
 * *aip.
 * @return TRUE         if objnum is aip->ignore_objnum (and signatures match) or
 * objnum is in ignore wing
 * @return FALSE        otherwise
 */
int is_ignore_object_sub (
    int* ignore_objnum, int* ignore_signature, int objnum) {
    // Not ignoring anything.
    if (*ignore_objnum == UNUSED_OBJNUM) { return 0; }
...
int find_ignore_new_object_index (ai_info* aip, int objnum) {
    ...
    for (i = 0; i < MAX_IGNORE_NEW_OBJECTS; i++) {
        if (is_ignore_object_sub (
                &aip->ignore_new_objnums[i], &aip->ignore_new_signatures[i],
                objnum))
            return i;
    }
...
int is_ignore_object (ai_info* aip, int objnum, int just_the_original = 0) {
    ...
    if (is_ignore_object_sub (
            &aip->ignore_objnum, &aip->ignore_signature, objnum))
        return 1;

    // check new ignore
    if (!just_the_original) {
        if (find_ignore_new_object_index (aip, objnum) >= 0) return 1;
    }
...
void evaluate_object_as_nearest_objnum (eval_nearest_objnum* eno) {
    ...
    if ((attacking_subsystem != NULL) ||
        !(eno->trial_objp->flags[Object::Object_Flags::Protected])) {
        if (OBJ_INDEX (eno->trial_objp) != eno->objnum) {
#ifndef NDEBUG
            if (!Player_attacking_enabled && (eno->trial_objp == Player_obj))
                return;
#endif
            // If only supposed to attack ship in a specific wing, don't
            // attack other ships.
            if ((eno->enemy_wing != -1) && (shipp->wingnum != eno->enemy_wing))
                return;

            // If only supposed to attack ships of a certain ship class, don't
            // attack other ships.
            if ((eno->enemy_ship_info_index >= 0) &&
                (shipp->ship_info_index != eno->enemy_ship_info_index))
                return;

            // Don't keep firing at a ship that is in its death throes.
            if (shipp->flags[Ship::Ship_Flags::Dying]) return;

            if (is_ignore_object (aip, OBJ_INDEX (eno->trial_objp))) return;

            if (eno->trial_objp->flags[Object::Object_Flags::Protected])
                return;

            if (shipp->is_arriving ()) return;

            ship_info* sip = &Ship_info[shipp->ship_info_index];

            if (sip->flags[Ship::Info_Flags::No_ship_type] ||
                sip->flags[Ship::Info_Flags::Navbuoy])
                return;
            ...
            if (iff_matches_mask (shipp->team, eno->enemy_team_mask)) {
                ...
                if (!eno->check_danger_weapon_objnum) {
                    ...
                    if (!object_is_targetable (
                            eno->trial_objp,
                            &Ships[Objects[eno->objnum].instance])) {
                        // check if stealth ship is visible, but not
                        // "targetable"
                        if (!((shipp->flags[Ship::Ship_Flags::Stealth]) &&
                              ai_is_stealth_visible (
                                  &Objects[eno->objnum], eno->trial_objp))) {
                            return;
                        }
                    }
                }
                ...
                if (sip->is_big_or_huge ()) {
                    ...
                    int inside = get_nearest_bbox_point (
                        eno->trial_objp, &Objects[eno->objnum].pos, &box_pt);
                    if (inside) {
                        dist = 10.0f;
                        // on the box
                    }
                        ...
                        dist = vm_vec_dist_quick (
                            &Objects[eno->objnum].pos, &box_pt);
                    }
                }
                else {
                    dist = vm_vec_dist_quick (
                        &Objects[eno->objnum].pos, &eno->trial_objp->pos);
                }

                // Make it more likely that fighters (or bombers) will be
                // picked as an enemy by scaling up distance for other types.
                if (Ship_info[shipp->ship_info_index].is_fighter_bomber ()) {
                    dist = dist * 0.5f;
                }

                num_attacking =
                    num_enemies_attacking (OBJ_INDEX (eno->trial_objp));

                if (!sip->is_big_or_huge () &&
                    num_attacking < eno->max_attackers) {
                    dist *= (float)(num_attacking + 2) /
                            2.0f; // prevents lots of ships from attacking
                                  // same target
                }
                ...
                if ((sip->is_big_or_huge ()) ||
                    ...
                    if (eno->trial_objp
                        ...
                        dist *=
                            1.0f + (NUM_SKILL_LEVELS - Game_skill_level - 1) /
                                       NUM_SKILL_LEVELS; // Favor attacking
                                                         // non-players based
                                                         // on skill level.
                    }
 ...
 * object. Unless aip->targeted_subsys != NULL, don't allow to attack objects
 ...
 * @param max_attackers         Don't attack a ship that already has at least
 * max_attackers attacking it.
 * @param ship_info_index       If >=0, the enemy object must be of the specified
 * ship class
 */
int get_nearest_objnum (
    int objnum, int enemy_team_mask, int enemy_wing, float range,
    int max_attackers, int ship_info_index) {
    ...
    if (aip->danger_weapon_objnum >= 0) {
        ...
        if (danger_weapon_objp->signature == aip->danger_weapon_signature) {
            ...
            if (danger_weapon_objp->parent >= 0) {
                if (is_object_stealth_ship (
                        &Objects[danger_weapon_objp->parent])) {
                    // check if stealthy
                    if (ai_is_stealth_visible (
                            &Objects[objnum],
                            &Objects[danger_weapon_objp->parent]) !=
                        STEALTH_FULLY_TARGETABLE) {
                        // check if weapon is laser
                        if (Weapon_info[Weapons[danger_weapon_objp->instance]
                                            .weapon_info_index]
                                .subtype == WP_LASER) {
    ...
    // If only looking for target in certain wing and couldn't find anything
    // in       that wing, look for any object.
    if ((eno.nearest_objnum == -1) && (enemy_wing != -1)) {
        return get_nearest_objnum (
            objnum, enemy_team_mask, -1, range, max_attackers,
            ship_info_index);
    }
 ...
 * object.
 *
 * Unlike find_enemy or find_nearest_objnum, this doesn't care about things
 * like the protected flag or number of enemies attacking. It is used to find
 * the nearest enemy to determine things like whether to rearm.
 */
int find_nearby_threat (
    int objnum, int enemy_team_mask, float range, int* count) {
    int nearest_objnum;
    float nearest_dist;
    object* objp;
    ship_obj* so;
    ...
    *count = 0;
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (OBJ_INDEX (objp) != objnum) {
            if (Ships[objp->instance].flags[Ship::Ship_Flags::Dying]) continue;

            if (Ship_info[Ships[objp->instance].ship_info_index]
                    .flags[Ship::Info_Flags::No_ship_type] ||
                Ship_info[Ships[objp->instance].ship_info_index]
                    .flags[Ship::Info_Flags::Navbuoy])
                continue;

            if (iff_matches_mask (
                    Ships[objp->instance].team, enemy_team_mask)) {
                float dist;

                dist = vm_vec_dist_quick (&Objects[objnum].pos, &objp->pos) -
                       objp->radius * 0.75f;
...
int num_turrets_attacking (object* turret_parent, int target_objnum) {
    ...
    for (ss = GET_FIRST (&shipp->subsys_list);
         ss != END_OF_LIST (&shipp->subsys_list); ss = GET_NEXT (ss)) {
        // check if subsys is alive
        if (ss->current_hits <= 0.0f) { continue; }

        // check if it's a turret
        if (ss->system_info->type != SUBSYSTEM_TURRET) { continue; }

        // if the turret is locked
        if (ss->weapons.flags[Ship::Weapon_Flags::Turret_Lock]) { continue; }

        // check if turret is targeting target_objnum
        if (ss->turret_enemy_objnum == target_objnum) { count++; }
    }
 ...
 */
int find_enemy (
    int objnum, float range, int max_attackers, int ship_info_index) {
    int enemy_team_mask;
    ...
    if (timestamp_elapsed (aip->choose_enemy_timestamp)) {
        ...
        if (aip->target_objnum != -1) {
            ...
            // This could cause attack on ship on fringe on nebula to stop if
            // attackee moves our of nebula range.  (BAD)
            if (Objects[target_objnum].signature == aip->target_signature) {
                if (iff_matches_mask (
                        Ships[Objects[target_objnum].instance].team,
                        enemy_team_mask)) {
                    if (ship_info_index < 0 ||
                        ship_info_index ==
                            Ships[Objects[target_objnum].instance]
                                .ship_info_index) {
                        if (!(Objects[target_objnum]
        ...
        return get_nearest_objnum (
            objnum, enemy_team_mask, aip->enemy_wing, range, max_attackers,
            ship_info_index);
    }
    else {
        aip->target_objnum = -1;
 ...
 * However, do not abort process for an object that is currently being repaired
 * -- let it finish.
 */
void ai_set_goal_maybe_abort_dock (object* objp, ai_info* aip) {
    if (aip->ai_flags[AI::AI_Flags::Awaiting_repair]) {
        ...
        if (aip->support_ship_objnum == -1) { repair_obj = NULL; }
        else {
    ...
    aip->next_rearm_request_timestamp = timestamp (
        NEXT_REARM_TIMESTAMP); // Might request again after 30 seconds.
}

void force_avoid_player_check (object* objp, ai_info* aip) {
    if (Ships[objp->instance].team == Player_ship->team) {
        aip->avoid_check_timestamp =
            timestamp (0); // Force a check for collision next frame.
    }
}
 ...
 * Attack point *rel_pos on object.  This is for supporting attacking
 * subsystems.
 */
void ai_attack_object (
    object* attacker, object* attacked, ship_subsys* ssp,
    int ship_info_index) {
    int temp;
    ai_info* aip;
    ...
    if (!(Ship_info[Ships[attacker->instance].ship_info_index]
              .is_small_ship ()) &&
        (attacked != NULL)) {
        WARNINGF (LOCATION, "AI ship %s is large ship ordered to attack %s",Ships[attacker->instance].ship_name,Ships[attacked->instance].ship_name);
    }
    ...
    if (attacked == NULL) {
        ...
        set_target_objnum (
            aip,
            find_enemy (OBJ_INDEX (attacker), 99999.9f, 4, ship_info_index));
    }
    else {
    ...
    aip->ok_to_target_timestamp =
        timestamp (DELAY_TARGET_TIME); // No dynamic targeting for 7 seconds.

    // Goober5000
    temp = find_ignore_new_object_index (aip, aip->target_objnum);
    if (temp >= 0) { aip->ignore_new_objnums[temp] = UNUSED_OBJNUM; }
    else if (is_ignore_object (aip, aip->target_objnum, 1)) {
        aip->ignore_objnum = UNUSED_OBJNUM;
    }

    aip->mode = AIM_CHASE;
    aip->submode = SM_ATTACK; // AL 12-15-97: need to set submode?  I got an
                              // assert() where submode was bogus
    aip->submode_start_time = Missiontime; // for AIM_CHASE... it may have been
                                           // not set correctly here

    if (ssp == NULL) {
        set_targeted_subsys (aip, NULL, -1);
        if (aip->target_objnum != -1) {
            Objects[aip->target_objnum].flags.remove (
                Object::Object_Flags::Protected); // If ship had been
...
void ai_attack_wing (object* attacker, int wingnum) {
    ...
    if (count > 0) {
        ...
        set_target_objnum (
            aip, Ships[Wings[wingnum].ship_index[index]].objnum);

        ai_set_goal_maybe_abort_dock (attacker, aip);
        aip->ok_to_target_timestamp = timestamp (
            DELAY_TARGET_TIME); // No dynamic targeting for 7 seconds.
    }
}
...
int compact_ignore_new_objects (ai_info* aip, int force = 0) {
    if (force) aip->ignore_new_objnums[0] = UNUSED_OBJNUM;

    for (int current_index = 0; current_index < MAX_IGNORE_NEW_OBJECTS;
         current_index++) {
        ...
        if (aip->ignore_new_objnums[current_index] != UNUSED_OBJNUM) {
            ...
            if (Objects[aip->ignore_new_objnums[current_index]].signature !=
                aip->ignore_new_signatures[current_index])
        ...
        for (int i = current_index + 1; i < MAX_IGNORE_NEW_OBJECTS; i++) {
            // skip empty slots
            if (aip->ignore_new_objnums[i] == UNUSED_OBJNUM) continue;
        ...
        if (next_occupied_index < 0) return current_index;

        // move the occupied slot down to this one
        aip->ignore_new_objnums[current_index] =
            aip->ignore_new_objnums[next_occupied_index];
        aip->ignore_new_signatures[current_index] =
            aip->ignore_new_signatures[next_occupied_index];
...
void ai_ignore_object (object* ignorer, object* ignored, int ignore_new) {
    ...
    // retail behavior
        ...
        ignored->flags.set (
            Object::Object_Flags::Protected); // set protected bit of ignored
                                              // ship.
    }
}
 ...
 */
void add_path_point (
    vec3d* pos, int path_num, int path_index, int modify_index) {
    pnode* pnp;
...
/**
 * Given two points on a sphere, the center of the sphere and the radius,
 * return a point on the vector through the midpoint of the chord on the
 * sphere.
 */
 ...
 *
 * The current position is in the current object and the goal position is in
 * the goal object. It is ok to intersect the current object, but not the goal
 * object. This function is useful for creating a path to an initial point near
 * a large object.
 *
 ...
 * @param subsys_path   Optional param (default 0), indicates this is a path to
 * a subsystem
 */
void create_path_to_point (
    vec3d* curpos, vec3d* goalpos, object* curobjp, object* goalobjp,
    int subsys_path) {
    ...
    if (pp_collide (curpos, goalpos, goalobjp, curobjp->radius)) {
        ...
        // If this is a path to a subsystem, use SUBSYS_PATH_DIST as the radius
        // for the object you are trying to avoid.  This is needed since
        // subsystem paths extend out to SUBSYS_PATH_DIST, and we want ships to
        // reach their path destination without flying to points that sit on
        // the radius of a small ship
        ...
        // tangent to *goalobjp sphere through point *curpos in plane
        // defined by *curpos, *goalpos, goalobjp->pos  Note, there are two
        // tangents through *curpos, unless *curpos is on the   sphere.  The
        // tangent that causes the nearer intersection (to *goalpos) is chosen.
...
/**
 * Given an object and a model path, globalize the points on the model and copy
 * into the global path list. If pnp != NULL, then modify, in place, the path
 * points.  This is used to create new globalized points when the base object
 * has moved.
 *
 ...
 * @param randomize_pnt Optional parameter (default value -1), add random
 * vector in sphere to this path point
 */
void copy_xlate_model_path_points (
    object* objp, model_path* mp, int dir, int count, int path_num, pnode* pnp,
    int randomize_pnt) {
    ...
    vec3d v1;
    int pp_index; // index in Path_points at which to store point, if this
                  // is a modify-in-place (pnp ! NULL)
    int start_index, finish_index;
    vec3d submodel_offset, local_vert;
    bool rotating_submodel;

    // Initialize pp_index.
    // If pnp == NULL, that means we're creating new points.  If not NULL,
    // then modify in place.
    if (pnp == NULL)
        pp_index = -1; // This tells add_path_point to create a new point.
    else
        pp_index = 0; // pp_index will get assigned to index in Path_points
                      // to reuse.
    ...
    if (dir == 1) {
        ...
        ASSERT (
            dir ==
            -1); // direction must be up by 1 or down by 1 and it's neither!
    ...
    if ((mp->parent_submodel >= 0) &&
        (pm->submodel[mp->parent_submodel].movement_type >= 0)) {
        rotating_submodel = true;

        model_find_submodel_offset (
            &submodel_offset, modelnum, mp->parent_submodel);
    }
    else {
    ...
    for (i = start_index; i != finish_index; i += dir) {
        ...
        if (rotating_submodel) {
            // movement... find location of point like with docking code and
            // spark generation
            vm_vec_sub (&local_vert, &mp->verts[i].pos, &submodel_offset);
            model_instance_find_world_point (
                &v1, &local_vert, model_instance_num, mp->parent_submodel,
        ...
        if (pp_index != -1) pp_index = (int)(pnp - Path_points) + offset;
 ...
 * The tricky part of this problem is creating the entry to the first point on
 * the predefined path. The points on this entry path are based on the location
 * of Pl_objp relative to the start of the path.
 *
 * @param pl_objp               Player object
 * @param mobjp                 Model object
 * @param path_num              Number of path
 * @param subsys_path   Optional param (default 0), indicating this is a path
 * to a subsystem
 */
void create_model_path (
    object* pl_objp, object* mobjp, int path_num, int subsys_path) {
    ship* shipp = &Ships[pl_objp->instance];
    ai_info* aip = &Ai_info[shipp->ai_index];

    ship_info* osip = &Ship_info[Ships[mobjp->instance].ship_info_index];
    polymodel* pm = model_get (
        Ship_info[Ships[mobjp->instance].ship_info_index].model_num);
    int num_points;
    model_path* mp;
    ...
    if (pp_collide (&pl_objp->pos, &gp0, mobjp, pl_objp->radius)) {
        ...
        // If object that wants to dock is inside bounding sphere of object it
        // wants to dock with, make it fly out.         Assume it can fly
        // "straight" out to the bounding sphere.
        if (vm_vec_dist_quick (&pl_objp->pos, &mobjp->pos) < mobjp->radius) {
            project_point_to_perimeter (
                &perim_point2, &mobjp->pos, mobjp->radius, &pl_objp->pos);
            add_path_point (&perim_point2, path_num, -1, -1);
        }

        // If last point on pre-defined path is inside bounding sphere, create
        // a new point on the surface of the sphere.
        if (vm_vec_dist_quick (&mobjp->pos, &gp0) < mobjp->radius) {
            project_point_to_perimeter (
                &perim_point1, &mobjp->pos, mobjp->radius, &gp0);
            create_path_to_point (
                &perim_point2, &perim_point1, pl_objp, mobjp, subsys_path);
            add_path_point (&perim_point1, path_num, -1, -1);
        }
        else { // The predefined path extends outside the sphere.  Create
               // path to that point.
            create_path_to_point (
                &perim_point2, &gp0, pl_objp, mobjp, subsys_path);
        }
    }

    // AL 12-31-97: If following a subsystem path, add random vector to second
    // last path point
    if (subsys_path) {
        copy_xlate_model_path_points (
            mobjp, mp, 1, mp->nverts, path_num, NULL, mp->nverts - 2);
    }
    else {
        copy_xlate_model_path_points (
            mobjp, mp, 1, mp->nverts, path_num, NULL);
    }
    ...
    aip->path_next_create_time =
        timestamp (1000); // OK to try to create one second later
    aip->path_create_pos = pl_objp->pos;
    aip->path_create_orient = pl_objp->orient;

    // Get path departure orientation from ships.tbl if it exists, otherwise
    // zero it
    std::string pathName (mp->name);
    if (osip->pathMetadata.find (pathName) != osip->pathMetadata.end () &&
        !IS_VEC_NULL (&osip->pathMetadata[pathName].departure_rvec)) {
        vm_vec_copy_normalize (
            &aip->path_depart_orient,
            &osip->pathMetadata[pathName].departure_rvec);
    }
    else {
    ...
    aip->ai_flags.remove (
        AI::AI_Flags::Use_exit_path); // ensure this flag is cleared
}
 ...
 */
void create_model_exit_path (
    object* pl_objp, object* mobjp, int path_num, int count) {
    ship* shipp = &Ships[pl_objp->instance];
    ai_info* aip = &Ai_info[shipp->ai_index];
    ...
    aip->ai_flags.set (
        AI::AI_Flags::Use_exit_path); // mark as exit path, referenced in maybe
}

/**
 * Return true if the vector from curpos to goalpos intersects with any ship
 * other than the ignore objects. Calls pp_collide()
 */
int pp_collide_any (
    vec3d* curpos, vec3d* goalpos, float radius, object* ignore_objp1,
    object* ignore_objp2, int big_only_flag) {
    ship_obj* so;
...
// subsys_path: optional param (default 0) that indicates this is a path to a
// subsystem
// Exit:
// ai_info struct in Pl_objp gets stuffed with information to enable Pl_objp
// to fly the path.
void ai_find_path (
    object* pl_objp, int objnum, int path_num, int exit_flag,
    int subsys_path) {
...
extern int vector_object_collision (
...
// If player collision would occur, set AIF_AVOIDING_SMALL_SHIP bit in
// ai_flags.    Set aip->avoid_goal_point
int maybe_avoid_player (object* objp, vec3d* goal_pos) {
    ...
    if (!timestamp_elapsed (aip->avoid_check_timestamp)) return 0;
    ...
    if (obj_obj_dist > speed_time * 2.0f) return 0;
    ...
    if (vector_object_collision (
            &objp->pos, &new_goal_pos, player_objp, 1.5f)) {
        ...
        // Now, avoid_vec is a vector perpendicular to the vector to the
        // player and the direction *objp       should fly in to avoid the player
        // while still approaching its goal.
        vm_vec_scale_add (
            &aip->avoid_goal_point, &player_objp->pos, &avoid_vec, 400.0f);
        ...
        aip->avoid_check_timestamp =
            timestamp ((int)(obj_obj_dist / 200.0f) + 500);
...
void ai_stay_still (object* still_objp, vec3d* view_pos) {
    ...
    ASSERT (
        (still_objp->instance >= 0) && (still_objp->instance < MAX_OBJECTS));
    ...
    // If view_pos not NULL, point at that point.  Else, point at a point
    // directly in front of ship.  Ie, don't turn.
    if (view_pos != NULL)
        aip->goal_point = *view_pos;
    else
        vm_vec_scale_add (
            &aip->goal_point, &still_objp->pos, &still_objp->orient.vec.fvec,
            100.0f);
}

// code which is called from ai_dock_with_object and ai_dock to set flags and
// apprioriate variable when two objects have completed docking.  used because
// we can dock object initially at misison load time (meaning that ai_dock()
...
void ai_do_objects_docked_stuff (
    ...
    if (dock_check_find_direct_docked_object (docker, dockee)) {
        WARNINGF (LOCATION,"Call to ai_do_objects_docked_stuff when objects are already docked!  Trace out and fix!");
        return;
    }
    ...
    if (docker->type == OBJ_SHIP && dockee->type == OBJ_SHIP) {
        ...
        if ((Ship_info[Ships[docker->instance].ship_info_index]
                 .flags[Ship::Info_Flags::Support]) ||
            (Ship_info[Ships[dockee->instance].ship_info_index]
                 .flags[Ship::Info_Flags::Support])) {
...
// function. Goober5000 - dockee must always be non-NULL
void ai_do_objects_undocked_stuff (object* docker, object* dockee) {
    ...
    if (!dock_check_find_direct_docked_object (docker, dockee)) {
        WARNINGF (LOCATION,"Call to ai_do_objects_undocked_stuff when objects are already undocked!  Trace out and fix!");
        return;
    }
...
// AIDO_DOCK_NOW   immediately dock, used for ships that need to be docked
// at mission start             AIDO_UNDOCK             set goal of undocking
void ai_dock_with_object (
    object* docker, int docker_index, object* dockee, int dockee_index,
    int dock_type) {
    ASSERT (docker != NULL);
    ASSERT (dockee != NULL);
    ...
    // easier to grab the path from the dockpoint a debug check would be a good
    // thing here, though
#ifndef NDEBUG
    if (dock_type == AIDO_UNDOCK) {
        polymodel* pm = model_get (
            Ship_info[Ships[dockee->instance].ship_info_index].model_num);
    ...
    if (dock_type == AIDO_DOCK_NOW) {
        ...
        // (fortunately, this function is called AFTER
        // model_anim_set_initial_states in the sea of ship creation functions,
        // which is necessary for model animations to start from t=0 at the
        // correct positions)
        ship* shipp = &Ships[docker->instance];
        ship* goal_shipp = &Ships[dockee->instance];
        model_anim_start_type (
            shipp, TRIGGER_TYPE_DOCKING_STAGE_1, docker_index, 1, true);
        model_anim_start_type (
            goal_shipp, TRIGGER_TYPE_DOCKING_STAGE_1, dockee_index, 1, true);
        model_anim_start_type (
            shipp, TRIGGER_TYPE_DOCKING_STAGE_2, docker_index, 1, true);
        model_anim_start_type (
            goal_shipp, TRIGGER_TYPE_DOCKING_STAGE_2, dockee_index, 1, true);
        model_anim_start_type (
            shipp, TRIGGER_TYPE_DOCKING_STAGE_3, docker_index, 1, true);
        model_anim_start_type (
            goal_shipp, TRIGGER_TYPE_DOCKING_STAGE_3, dockee_index, 1, true);
        model_anim_start_type (
            shipp, TRIGGER_TYPE_DOCKED, docker_index, 1, true);
        model_anim_start_type (
            goal_shipp, TRIGGER_TYPE_DOCKED, dockee_index, 1, true);

        dock_orient_and_approach (
            docker, docker_index, dockee, dockee_index, DOA_DOCK_STAY);
        ai_do_objects_docked_stuff (
            docker, docker_index, dockee, dockee_index, false);
    }
...
void ai_start_waypoints (object* objp, waypoint_list* wp_list, int wp_flags) {
    ...
    if ((aip->mode == AIM_WAYPOINTS) && (aip->wp_list == wp_list)) {
        if (aip->wp_index == INVALID_WAYPOINT_POSITION) {
            WARNINGF (LOCATION, "aip->wp_index should have been assigned already!");
            aip->wp_index = 0;
        }
        return;
    }
...
// Goober5000 - enter safety mode (used by support ships, now possibly by
// fighters too)
...
void ai_form_on_wing (object* objp, object* goal_objp) {
    ...
    // objp == goal_objp sometimes in multiplayer when someone leaves a game --
    // make a simple out for this case.
    if (Game_mode & GM_MULTIPLAYER) {
        if (objp == goal_objp) { return; }
    }
    ...
    if (!(sip->is_fighter_bomber ())) {
        WARNINGF (LOCATION,"Ship %s tried to form on player's wing, but not fighter or bomber.",shipp->ship_name);
        return;
    }
    ...
    aip->ok_to_target_timestamp = timestamp (
        DELAY_TARGET_TIME *
        4); // Super extra long time until can target another ship.
}

/**
 * Given an object and an object on whose wing to form, return slot to use.
 *
 * @todo This function is called per object in formation per frame.  Should
 * store slot in ::ai_info struct.
 */
int ai_formation_object_get_slotnum (int objnum, object* objp) {
    int slotnum = 1; // Note: Slot #0 means leader, which isn't someone who was
                     // told to form-on-wing.

    object* o;
    for (o = GET_FIRST (&obj_used_list); o != END_OF_LIST (&obj_used_list);
         o = GET_NEXT (o)) {
        if (objp == o)
            break;
        else if (o->type == OBJ_SHIP)
            if (Ai_info[Ships[o->instance].ai_index]
                    .ai_flags[AI::AI_Flags::Formation_object])
                if (Ai_info[Ships[o->instance].ai_index].goal_objnum == objnum)
                    slotnum++;
    }

    ASSERT (o != END_OF_LIST (&obj_used_list)); // Didn't find objp in list of
                                                // used ships.  Impossible!
...
/**
 * Given an attacker's position and a target's position and velocity, compute
 * the time of intersection of a weapon fired by the attacker with speed
 * weapon_speed.
 *
 * Return this value.  Return value of 0.0f means no collision is possible.
 */
float compute_collision_time (
    vec3d* targpos, vec3d* targvel, vec3d* attackpos, float weapon_speed) {
    vec3d vec_to_target;
    float pos_dot_vel;
    float vel_sqr;
    float discrim;
    ...
    discrim = pos_dot_vel * pos_dot_vel -
              vel_sqr * vm_vec_dot (&vec_to_target, &vec_to_target);
    ...
    if (discrim > 0.0f) {
        ...
        if (t1 > 0.0f) t_solve = t1;
        if ((t2 > 0.0f) && (t2 < t_solve)) t_solve = t2;
...
// (or his laser) gets to the current enemy position, the enemy will have
// moved. This is dealt with in polish_predicted_enemy_pos.
float compute_time_to_enemy (float dist_to_enemy, object* pobjp) {
    ...
    max_laser_distance = MIN (
        (max_laser_speed * Weapon_info[weapon_num].lifetime),
    ...
    if (dist_to_enemy > 1.5 * max_laser_distance) {
        ...
        if (pl_speed > 0.1f) {
            ...
            time_to_enemy =
                (float)(dist_to_enemy / (pl_speed * scale + max_laser_speed * (1.0f - scale)));
        }
        else
            time_to_enemy = 2.0f;
    }
...
// --------------------------------------------------------------------------
void ai_set_positions (
    object* pl_objp, object* en_objp, ai_info* aip, vec3d* player_pos,
    vec3d* enemy_pos) {
    *player_pos = pl_objp->pos;
    ...
    if (aip->next_predict_pos_time > Missiontime) {
        ...
        *enemy_pos = en_objp->pos;
        ...
        aip->next_predict_pos_time =
            Missiontime + aip->ai_predict_position_delay;
...
void ai_update_aim (ai_info* aip) {
    if (Missiontime >= aip->next_aim_pos_time) {
        ...
        // change)
        vm_vec_scale_add2 (
            &aip->last_aim_enemy_pos, &aip->last_aim_enemy_vel, flFrametime);
    }
}
...
int get_base_path_info (
    int path_cur, int goal_objnum, model_path** pmp, mp_vert** pmpv) {
...
// Only modify those points that are defined in the model path.  Don't modify
// the  leadin points, such as those that are necessary to get the model on the
// path.
void modify_model_path_points (object* objp) {
    ...
    while ((pnp->path_index == -1) &&
           (pnp - Path_points - aip->path_start < aip->path_length))
        pnp++;
    ...
    dir = 1;
    if (aip->ai_flags[AI::AI_Flags::Use_exit_path]) { dir = -1; }

    copy_xlate_model_path_points (
        mobjp, &pm->paths[path_num], dir, pm->paths[path_num].nverts, path_num,
        pnp);
}
...
// Paths are created in absolute space, so a moving object needs to have model
// paths within it recreated.   This uses the hash functions which means the
// slightest movement will cause a recreate, though the timestamp       prevents
// this from happening too often.       force_recreate_flag TRUE means to recreate
// regardless of timestamp.     Returns TRUE if path recreated.
float maybe_recreate_path (
    object* objp, ai_info* aip, int force_recreate_flag,
    int override_hash = 0) {
    int hashval;
    ...
    if (aip->path_start == -1) return 0.0f;

    // AL 11-12-97: If AIF_USE_STATIC_PATH is set, don't try to recreate.  This
    // is needed when ships
    // emerge from fighter bays.  We don't need to recreate
    // the path.. and in case the
    // parent ship dies, we still want to be able to continue on
    // the path
    if (aip->ai_flags[AI::AI_Flags::Use_static_path]) return 0.0f;

    if (force_recreate_flag ||
        timestamp_elapsed (aip->path_next_create_time)) {
        object* path_objp;
        ...
        if (override_hash || (hashval != aip->path_goal_obj_hash)) {
            ...
            dist +=
                ai_matrix_dist (&path_objp->orient, &aip->path_create_orient) *
                25.0f;

            if (force_recreate_flag || (dist > 2.0f)) {
                aip->path_next_create_time =
                    timestamp (1000); // Update again in as little as 1000
                                      // milliseconds, ie 1 second.
                ...
                // Get path departure orientation from ships.tbl if it exists,
                // otherwise zero it
                ship_info* osip =
                    &Ship_info[Ships[path_objp->instance].ship_info_index];
                model_path* mp =
                    &model_get (osip->model_num)->paths[aip->mp_index];
                std::string pathName (mp->name);
                if (osip->pathMetadata.find (pathName) !=
                        osip->pathMetadata.end () &&
                    !IS_VEC_NULL (
                        &osip->pathMetadata[pathName].departure_rvec)) {
                    vm_vec_copy_normalize (
                        &aip->path_depart_orient,
 ...
 */
void set_accel_for_docking (
    object* objp, ai_info* aip, float dot, float dot_to_next,
    float dist_to_next, float dist_to_goal, ship_info* sip,
    float max_allowed_speed, object* gobjp) {
    float prev_dot_to_goal = aip->prev_dot_to_goal;

    aip->prev_dot_to_goal = dot;

    if (objp->phys_info.speed < 0.0f) { accelerate_ship (aip, 1.0f / 32.0f); }
    else if ((prev_dot_to_goal - dot) > 0.01) {
        if (prev_dot_to_goal > dot + 0.05f) { accelerate_ship (aip, 0.0f); }
        else {
            change_acceleration (
                aip, -1.0f); // -1.0f means subtract off flFrametime from
                             // acceleration value in 0.0..1.0
        }
    }
    else {
        float max_bay_speed = sip->max_speed;

        // Maybe gradually ramp up/down the speed of a ship flying a fighterbay
        // path
        if (aip->mode == AIM_BAY_EMERGE ||
            (aip->mode == AIM_BAY_DEPART &&
             aip->path_cur != aip->path_start)) {
            ship_info* gsip =
                &Ship_info[Ships[gobjp->instance].ship_info_index];
            polymodel* pm = model_get (gsip->model_num);
            std::string pathName (
                pm->paths[Path_points[aip->path_start].path_num].name);
            float speed_mult = FLT_MIN;

            if (aip->mode == AIM_BAY_EMERGE) { // Arriving
                if (gsip->pathMetadata.find (pathName) !=
                    gsip->pathMetadata.end ()) {
                    speed_mult =
                        gsip->pathMetadata[pathName].arrive_speed_mult;
                }
                ...
                if (gsip->pathMetadata.find (pathName) !=
                    gsip->pathMetadata.end ()) {
                    speed_mult =
                        gsip->pathMetadata[pathName].depart_speed_mult;
                }
            ...
            if (speed_mult != FLT_MIN && speed_mult != 1.0f) {
                // We use the distance between the first and last point on the
                // path here; it's not accurate if the path is not straight,
                // but should be good enough usually; can be changed if
                // necessary.
                float total_path_length = vm_vec_dist_quick (
                    &Path_points[aip->path_start].pos,
                    &Path_points[aip->path_start + aip->path_length - 1].pos);
                float dist_to_end;

                if (aip->mode == AIM_BAY_EMERGE) { // Arriving
                    dist_to_end = vm_vec_dist_quick (
                        &Pl_objp->pos, &Path_points[aip->path_start].pos);
                }
                else { // Departing
                    dist_to_end = vm_vec_dist_quick (
                        &Pl_objp->pos,
                        &Path_points[aip->path_start + aip->path_length - 1]
                             .pos);
                }

                // Calculate max speed, but respect the waypoint speed cap if
                // it's lower
                max_bay_speed =
                    sip->max_speed *
                    (speed_mult +
                     (1.0f - speed_mult) * (dist_to_end / total_path_length));
            }
        }
        ...
        if ((aip->mode == AIM_DOCK) && (dist_to_next < 150.0f) &&
            (aip->path_start + aip->path_length - 2 == aip->path_cur)) {
            set_accel_for_target_speed (
                objp, max_allowed_speed * MAX (dist_to_next / 500.0f, 1.0f));
        }
        else if ((dot_to_next >= dot * .9) || (dist_to_next > 100.0f)) {
            if (dist_to_goal > 200.0f) {
                set_accel_for_target_speed (
                    objp, max_allowed_speed * (dot + 1.0f) / 2.0f);
            }
            else {
                float xdot;

                xdot = (dot_to_next + dot) / 2.0f;
                if (xdot < 0.0f) xdot = 0.0f;

                // AL: if following a path not in dock mode, move full speed
                if ((aip->mode != AIM_DOCK) && (dot > 0.9f)) {
                    set_accel_for_target_speed (
                        objp, max_allowed_speed * dot * dot * dot);
                }
                else {
                    if ((aip->path_cur - aip->path_start <
                         aip->path_length - 2) &&
                        (dist_to_goal < 2 * objp->radius)) {
                        set_accel_for_target_speed (
                            objp, dist_to_goal / 8.0f + 2.0f);
                    }
                    else {
                        set_accel_for_target_speed (
                            objp,
                            max_allowed_speed * (2 * xdot + 0.25f) / 4.0f);
                    }
                }
            }
        }
            ...
            if (aip->mode != AIM_DOCK) {
                ...
                float speed;
                if ((aip->path_cur - aip->path_start < aip->path_length - 2) &&
                    (dist_to_goal < 2 * objp->radius)) {
...
float ai_path_0 () {
    ...
    object* gobjp;
    vec3d *cvp, *nvp, next_vec, gcvp,
        gnvp; // current and next vertices in global coordinates.
    ...
    if (aip->path_start == -1) {
        ...
        ASSERT (
            aigp->flags[AI::Goal_Flags::Dockee_index_valid] &&
            aigp->flags[AI::Goal_Flags::Docker_index_valid]);

        path_num = ai_return_path_num_from_dockbay (
            &Objects[aip->goal_objnum], aigp->dockee.index);
    ...
    // Set cvp and nvp as pointers to current and next vertices of interest on
    // path.
    cvp = &Path_points[aip->path_cur].pos;
    if ((aip->path_cur + aip->path_dir - aip->path_start < num_points) ||
        (aip->path_cur + aip->path_dir < aip->path_start))
        nvp = &Path_points[aip->path_cur + aip->path_dir].pos;
    else {
        // If this is 0, then path length must be 1 which means we have no
        // direction!
        ASSERT (
            (aip->path_cur - aip->path_dir >= aip->path_start) &&
            (aip->path_cur - aip->path_dir - aip->path_start < num_points));
        // Cleanup for above Assert() which we hit too near release. -- MK,
        // 5/24/98.
        if (aip->path_cur - aip->path_dir - aip->path_start >= num_points) {
            if (aip->path_dir == 1)
                aip->path_cur = aip->path_start;
        ...
        vec3d delvec;
        vm_vec_sub (
            &delvec, cvp, &Path_points[aip->path_cur - aip->path_dir].pos);
    ...
    // Can't use fvec, need to use velocity vector because we aren't
    // necessarily      moving in the direction we're facing.
    ...
    // If moving not-very-slowly and sliding, then try to slide at goal,
    // rather than      point at goal.
    slop_vec = NULL;
    if (mag < 1.0f)
    ...
    if (dist_to_goal > 0.1f)
        ai_turn_towards_vector (
            &gcvp, Pl_objp, sip->srotation_time, slop_vec, nullptr, 0.0f, 0);

    // Code to control speed is MUCH less forgiving in path following than in
    // waypoint         following.  Must be very close to path or might hit
    // objects.
    ...
    set_accel_for_docking (
        Pl_objp, aip, dot, dot_to_next, dist_to_next, dist_to_goal, sip, 0,
        gobjp);
    aip->prev_dot_to_goal = dot;

    // If moving at a non-tiny velocity, detect attaining path point by its
    // being close to   line between previous and current object location.
    if ((dist_to_goal < MIN_DIST_TO_WAYPOINT_GOAL) ||
        (vm_vec_dist_quick (&Pl_objp->last_pos, &Pl_objp->pos) > 0.1f)) {
        vec3d nearest_point;
        float r, min_dist_to_goal;

        r = find_nearest_point_on_line (
            &nearest_point, &Pl_objp->last_pos, &Pl_objp->pos, &gcvp);

        // Set min_dist_to_goal = how close must be to waypoint to pick next
        // one.         If docking and this is the second last waypoint, must be
        // very close.
        ...
        if ((vm_vec_dist_quick (&Pl_objp->pos, &gcvp) < min_dist_to_goal) ||
            (((r >= 0.0f) && (r <= 1.0f)) &&
             (vm_vec_dist_quick (&nearest_point, &gcvp) <
              (MIN_DIST_TO_WAYPOINT_GOAL + Pl_objp->radius)))) {
            aip->path_cur += aip->path_dir;
...
float ai_path_1 () {
    ...
    float mag;
    object* gobjp;
    vec3d *pvp, *cvp, *nvp, next_vec, gpvp, gcvp,
        gnvp; // previous, current and next vertices in global coordinates.
    ...
    if (prev_point >= aip->path_start &&
        prev_point <= aip->path_start + num_points)
    ...
    if (mag < 1.0f) nvel_vec = Pl_objp->orient.vec.fvec;

    // For departures, optionally rotate so the ship is pointing the correct
    // direction (so you can always have "wheels-down" landings)
    vec3d rvec;
    vec3d* prvec = NULL;
    if (aip->mode == AIM_BAY_DEPART &&
        !IS_VEC_NULL (&aip->path_depart_orient)) {
        vm_vec_unrotate (
            &rvec, &aip->path_depart_orient,
            &Objects[aip->path_objnum].orient);
        prvec = &rvec;
    }

    // Turn toward a point between where we are on the path and the goal. This
    // helps keep the ship flying "down the line"
    if (dist_to_goal > 0.1f) {
        // Get nearest point on line between current and previous path points
        vec3d closest_point;
        float r = find_nearest_point_on_line (
            &closest_point, &gpvp, &gcvp, &Pl_objp->pos);

        // Turn towards the 1/3 point between the closest point on the line and
        // the current goal point (unless we are already past the current goal)
        if (r <= 1.0f) {
            vec3d midpoint;
            vm_vec_avg3 (&midpoint, &gcvp, &closest_point, &closest_point);
            ai_turn_towards_vector (
                &midpoint, Pl_objp, sip->srotation_time, nullptr, nullptr,
                0.0f, 0, prvec);
        }
        else {
            ai_turn_towards_vector (
                &gcvp, Pl_objp, sip->srotation_time, nullptr, nullptr, 0.0f, 0,
                prvec);
        }
    }
    ...
    set_accel_for_docking (
        Pl_objp, aip, dot, dot_to_next, dist_to_next, dist_to_goal, sip,
        max_allowed_speed, gobjp);
    aip->prev_dot_to_goal = dot;
    ...
    if ((dist_to_goal < MIN_DIST_TO_WAYPOINT_GOAL) ||
        ...
        // very close.
        if ((aip->mode == AIM_DOCK) &&
            (aip->path_cur - aip->path_start >= aip->path_length - 1)) {
            min_dist_to_goal = MIN_DIST_TO_WAYPOINT_GOAL;
        }
        // If this is not the last waypoint, include a term to compensate for
        // inertia
        else if (
            aip->path_cur - aip->path_start <
            aip->path_length - (aip->mode == AIM_DOCK ? 0 : 1)) {
            vec3d cp_rel;
...
// Stuff bounding box of all enemy objects within "range" units of object
//*my_objp.     Stuff ni min_vec and max_vec.   Return value: Number of enemy
// objects in bounding box.
int get_enemy_team_range (
    object* my_objp, float range, int enemy_team_mask, vec3d* min_vec,
    vec3d* max_vec) {
    object* objp;
    ship_obj* so;
    int count = 0;
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (iff_matches_mask (Ships[objp->instance].team, enemy_team_mask)) {
            ...
            if (sip->is_fighter_bomber () ||
                ...
                sip->flags[Ship::Info_Flags::Capital] ||
                sip->flags[Ship::Info_Flags::Supercap] ||
                sip->flags[Ship::Info_Flags::Drydock] ||
                sip->flags[Ship::Info_Flags::Corvette] ||
                sip->flags[Ship::Info_Flags::Awacs] ||
                sip->flags[Ship::Info_Flags::Gas_miner])
                if (vm_vec_dist_quick (&my_objp->pos, &objp->pos) < range) {
                    if (count == 0) {
                        ...
                        *max_vec = objp->pos;
                        ...
                        update_min_max (
                            objp->pos.xyz.x, &min_vec->xyz.x, &max_vec->xyz.x);
                        update_min_max (
                            objp->pos.xyz.y, &min_vec->xyz.y, &max_vec->xyz.y);
                        update_min_max (
                            objp->pos.xyz.z, &min_vec->xyz.z, &max_vec->xyz.z);
                    }
                }
        }
    }
...
void ai_safety_pick_spot (object* objp) {
    ...
    if (get_enemy_team_range (
            objp, 1000.0f, enemy_team_mask, &min_vec, &max_vec)) {
        ...
        vm_vec_scale_add (
            &goal_pos, &objp->pos, &objp->orient.vec.fvec, 100.0f);
...
float ai_safety_goto_spot (object* objp) {
    ...
    if (dist > 200.0f) {
        ...
        set_accel_for_target_speed (
            objp, sip->max_speed * dot_val * (dist / 200.0f + 0.2f));
...
void ai_safety_circle_spot (object* objp) {
    ...
    dot = turn_towards_tangent (
        objp, &goal_point, 250.0f); // Increased from 50 to 250 to make
                                    // circling not look so wacky.

    set_accel_for_target_speed (
        objp, 0.5f * (1.0f + dot) * sip->max_speed / 4.0f);
}
...
// optionally returns true if Pl_objp (pl_done_p) has reached the target
// position (can be NULL) optionally returns true if Pl_objp's
// (pl_treat_as_ship_p) fly to order was issued to him directly, or if the
// order was issued to his wing (returns false) (can be NULL, and result is
// only valid if Pl_done_p was not NULL and was set true by function.
void ai_fly_to_target_position (
    vec3d* target_pos, bool* pl_done_p = NULL,
  ...
  */
    if ((aip->active_goal == AI_GOAL_NONE) ||
    ...
    // Can't use fvec, need to use velocity vector because we aren't
    // necessarily      moving in the direction we're facing.
    // AL 23-3-98: Account for very small velocities by checking result of
    // vm_vec_mag().
    // If we don't vm_vec_copy_normalize() will think it is
    // normalizing a null vector.
    if (vm_vec_mag_quick (&Pl_objp->phys_info.vel) < AICODE_SMALL_MAGNITUDE) {
        mag = 0.0f;
        vm_vec_zero (&nvel_vec);
    }
    ...
    bool carry_flag =
        ((shipp->flags[Ship::Ship_Flags::Navpoint_carry]) ||
         ((shipp->wingnum >= 0) &&
          (Wings[shipp->wingnum].flags[Ship::Wing_Flags::Nav_carry])));

    if (AutoPilotEngaged) ab_allowed = false;

    if (AutoPilotEngaged && timestamp_elapsed (LockAPConv) && carry_flag &&
        ((The_mission.flags[Mission::Mission_Flags::Use_ap_cinematics]) ||
         (Pl_objp != Autopilot_flight_leader))) {
        ASSERTX (
        ...
        if (The_mission.flags[Mission::Mission_Flags::Use_ap_cinematics]) {
            if (aip->wing != -1) {
                ...
                if (wing_leader != Pl_objp) {
                    // not wing leader.. get my position relative to wing
                    // leader
                    get_absolute_wing_pos_autopilot (
                        &goal_point, wing_leader, wing_index,
                        aip->ai_flags[AI::AI_Flags::Formation_object]);
                }
                else {
                    // Am wing leader.. get the wings position relative to the
                    // flight leader
                    j = 1 + int((float)floor (
                                double(autopilot_wings[aip->wing] - 1) / 2.0));
                    ...
                    case 1: // back-left
                        ...
                        vm_vec_scale (
                            &perp,
                            -166.0f * j); // 166m is supposedly the optimal
                                          // range according to tolwyn
                        vm_vec_add (
                            &goal_point, &Autopilot_flight_leader->pos, &perp);
                        break;
            ...
            vm_vec_sub (
                &perp, Navs[CurrentNav].GetPosition (),
                &Autopilot_flight_leader->pos);
            vm_vector_2_matrix (&Pl_objp->orient, &perp, NULL, NULL);
        }
        else {
            vm_vec_scale_add (
                &perp, &Pl_objp->pos, &Autopilot_flight_leader->phys_info.vel,
                1000.0f);
            ai_turn_towards_vector (
                &perp, Pl_objp, sip->srotation_time * 3.0f * scale, slop_vec,
                nullptr, 0.0f, 0);
        }
    }
    else {
        if (dist_to_goal > 0.1f) {
            ai_turn_towards_vector (
                target_pos, Pl_objp, sip->srotation_time * 3.0f * scale,
                slop_vec, nullptr, 0.0f, 0);
        }
    }
    ...
    if (Pl_objp->phys_info.speed < 0.0f) {
        ...
        // We are further from pointing at our goal this frame than last
        // frame, so slow down.
        ...
        if (dist_to_goal < 250.0f) {
            dot1 =
                dot * dot * dot; // Very important to be pointing towards goal
                                 // when nearby.  Note, cubing preserves sign.
        }
        else {
            if (dot > 0.0f) { dot1 = dot * dot; }
            else {
                dot1 = dot;
            }
        }
        ...
        if (dot1 < 0.8f) ab_allowed = false;
    }

    if (!(sip->flags[Ship::Info_Flags::Afterburner]) ||
        (shipp->flags[Ship::Ship_Flags::Afterburner_locked]) ||
        !(aip->ai_flags[AI::AI_Flags::Free_afterburner_use])) {
        ab_allowed = false;
    }
    ...
    if (shipp->wingnum != -1) {
        ...
        if (ab_allowed) {
            float self_ab_scale =
                ...
                The_mission.ai_profile
                    ->afterburner_recharge_scale[Game_skill_level];
            self_ab_scale = sip->afterburner_recover_rate * self_ab_scale /
                            (sip->afterburner_burn_rate +
                             sip->afterburner_recover_rate * self_ab_scale);
            self_ab_speed =
                0.95f * (self_ab_scale *
                             (Pl_objp->phys_info.afterburner_max_vel.xyz.z -
                              shipp->current_max_speed) +
                         shipp->current_max_speed);
        }
    }
    ...
    if ((self_ab_speed < 5.0f) ||
        (max_allowed_ab_speed <
         5.0f)) { // || ((shipp->wingnum != -1) && (dist_to_goal /
                  // max_allowed_ab_speed < 5.0f))){
        ab_allowed = false;
    }
    ...
    if (ab_allowed) {
        if (self_ab_speed <= (1.001 * max_allowed_ab_speed)) {
            if (!(Pl_objp->phys_info.flags & PF_AFTERBURNER_ON)) {
                float percent_left = 100.0f * shipp->afterburner_fuel /
                                     sip->afterburner_fuel_capacity;
            ...
            float switch_value;
            if ((self_ab_speed - max_allowed_ab_speed) >
                (0.2f * max_allowed_ab_speed)) {
            ...
            if (!(Pl_objp->phys_info.flags & PF_AFTERBURNER_ON)) {
                float percent_left = 100.0f * shipp->afterburner_fuel /
                                     sip->afterburner_fuel_capacity;
                if ((Pl_objp->phys_info.speed <
                     (max_allowed_ab_speed - switch_value)) &&
                    (percent_left > 30.0f)) {
                    afterburners_start (Pl_objp);
                    aip->afterburner_stop_time = Missiontime + 5 * F1_0;
                }
                ...
                if (Pl_objp->phys_info.speed >
                    (max_allowed_ab_speed + 0.9 * switch_value)) {
        ...
        if (!(Pl_objp->phys_info.flags & PF_AFTERBURNER_ON)) {
            if (aip->prev_accel * shipp->current_max_speed >
                max_allowed_ab_speed) {
                accelerate_ship (
                    aip, max_allowed_ab_speed / shipp->current_max_speed);
            }
        }
    }
    else {
        ...
        if (aip->prev_accel * shipp->current_max_speed > max_allowed_speed) {
            accelerate_ship (
                aip, max_allowed_speed / shipp->current_max_speed);
        }
    }
    ...
    if ((dist_to_goal < MIN_DIST_TO_WAYPOINT_GOAL) ||
        ...
        if ((dist_to_goal <
            ...
            // might be completing a waypoint for for the entire wing, or it
            // might be completing a goal for itself.  If for itself and in a
            // wing, treat the completion as we would a ship
            treat_as_ship = 1;
            if (Ships[Pl_objp->instance].wingnum != -1) {
                ...
                if ((aip->active_goal >= 0) &&
                    (aip->active_goal < MAX_AI_GOALS)) {
                    type = aip->goals[aip->active_goal].type;
                    if ((type == AIG_TYPE_EVENT_WING) ||
                        (type == AIG_TYPE_PLAYER_WING)) {
                        treat_as_ship = 0;
                    }
                    else {
                        treat_as_ship = 1;
            ...
            if (pl_done_p != NULL) {
                *pl_done_p = true;
                if (pl_treat_as_ship_p != NULL) {
                    if (treat_as_ship) { *pl_treat_as_ship_p = true; }
                    else {
                ...
                ASSERTX (
                    pl_treat_as_ship_p != NULL,
                    "pl_done_p cannot be NULL while pl_treat_as_ship_p is not "
                    "NULL");
            }
        }
    }
...
void ai_waypoints () {
    ...
    if (aip->wp_index == INVALID_WAYPOINT_POSITION) {
        if (aip->wp_list == NULL) {
            ...
            WARNINGF (LOCATION, "Waypoints should have been started already!");
            ai_start_waypoints (Pl_objp, aip->wp_list, WPF_REPEAT);
        }
    }

    ASSERT (
        !aip->wp_list->get_waypoints ()
             .empty ()); // What? Is this zero? Probably never got initialized!

    bool done, treat_as_ship;
    ai_fly_to_target_position (
        aip->wp_list->get_waypoints ()[aip->wp_index].get_pos (), &done,
        &treat_as_ship);
    ...
    if (done) {
        ...
        if (aip->wp_index == aip->wp_list->get_waypoints ().size ()) {
            ...
            if (aip->wp_flags & WPF_REPEAT) {
                ...
                // remove the goal from the AI goals of the ship pr wing,
                // respectively. Whether we should treat this as a ship or a
                // wing is determined by ai_fly_to_target_position when it
                // marks the AI directive as complete
                if (treat_as_ship) {
                    ai_mission_goal_complete (
                        aip); // this call should reset the AI mode
                    mission_log_add_entry (
                        LOG_WAYPOINTS_DONE, Ships[Pl_objp->instance].ship_name,
                        aip->wp_list->get_name (), -1);
                }
                else {
                    ai_mission_wing_goal_complete (
                        Ships[Pl_objp->instance].wingnum,
                        &(aip->goals[aip->active_goal]));
                    mission_log_add_entry (
                        LOG_WAYPOINTS_DONE,
                        Wings[Ships[Pl_objp->instance].wingnum].name,
                        aip->wp_list->get_name (), -1);
                }
            }
        }
...
void ai_fly_to_ship () {
    ...
    if (aip->mode != AIM_FLY_TO_SHIP) {
        WARNINGF (LOCATION,"ai_fly_to_ship called for '%s' when ai_info.mode not equal to AIM_FLY_TO_SHIP. Is actually '%d'",Ships[Pl_objp->instance].ship_name, aip->mode);
        aip->mode = AIM_NONE;
        return;
    }
    if (aip->active_goal < 0 || aip->active_goal >= MAX_AI_GOALS) {
        WARNINGF (LOCATION,"'%s' is trying to fly-to a ship without an active AI_GOAL\n\nActive ai mode is '%d'",Ships[Pl_objp->instance].ship_name, aip->active_goal);
        aip->mode = AIM_NONE;
        return;
    }
    ASSERT (aip->goals[aip->active_goal].target_name != NULL);
    if (aip->goals[aip->active_goal].target_name[0] == '\0') {
        WARNINGF (LOCATION,"'%s' is trying to fly-to-ship without a name for the ship",Ships[Pl_objp->instance].ship_name);
        aip->mode = AIM_NONE;
        ai_remove_ship_goal (
            aip,
            aip->active_goal); // function sets aip->active_goal to NONE for me
        return;
    }
    ...
    if (target_p == NULL) {
        ...
        for (int i = 0; i < MAX_AI_GOALS; i++) {
            if (aip->mode == AIM_FLY_TO_SHIP ||
                ...
                WARNINGF (LOCATION, "Ship '%s' told to fly to target ship '%s'",Ships[Pl_objp->instance].ship_name,aip->goals[i].target_name);
            }
        }
#endif
        WARNINGF (LOCATION,"Ship '%s' told to fly to a ship but none of the ships it was told to fly to exist.\nSee log before this message for list of ships set as fly-to tagets",Ships[Pl_objp->instance].ship_name);
        aip->mode = AIM_NONE;
        ai_remove_ship_goal (
            aip,
            aip->active_goal); // function sets aip->active_goal to NONE for me
        ...
        if (done) {
            // remove the goal from the AI goals of the ship pr wing,
            // respectively. Wether or not we should treat this as a ship or a
            // wing is determined by ai_fly_to_target when it marks the AI
            // directive as complete
            if (treat_as_ship) {
                ai_mission_goal_complete (
                    aip); // this call should reset the AI mode
            }
...
void avoid_ship () {
    // To avoid an object, turn towards right or left vector until facing away
    // from object.     To choose right vs. left, pick one that is further from
    // center of avoid object.  Keep turning away from until pointing away from
    // ship.    Stay in avoid mode until at least 3 enemy ship radii away.
    ...
    // if we're avoiding a stealth ship, then we know where he is, update with
    // no error
    ...
    if (away_dot < 0.0f) {
        ...
        vm_vec_scale_add (
            &goal_pos, &En_objp->pos, &En_objp->orient.vec.fvec, -100.0f);
        turn_towards_point (
            Pl_objp, &goal_pos, NULL, Pl_objp->phys_info.speed);
    }
...
int maybe_resume_previous_mode (object* objp, ai_info* aip) {
    ...
    if (aip->active_goal != AI_ACTIVE_GOAL_DYNAMIC) return 0;
    ...
    if (aip->mode == AIM_EVADE_WEAPON) {
        ...
        if ((aip->guard_objnum != -1) &&
            ...
            // If guarding ship is far away from guardee and enemy is far away
            // from guardee,    then stop chasing and resume guarding.
...
// Call this function if you want something to happen on average every N
// quarters of a second.        The truth value returned by this function will be
// the same for any given quarter second interval.      The value "num" is only
// passed in to get asynchronous behavior for different objects.        modulus ==
// 1 will always return true.   modulus == 2 will return true half the time.
// modulus == 16 will return true for one quarter second interval every four
// seconds.
int static_rand_timed (int num, int modulus) {
    if (modulus < 2)
        ...
        t = Missiontime >>
            18; // Get time in quarters of a second (SUSHI: this comment is
                // wrong! It's actually every 4 seconds!)
        t += num;
...
int ai_maybe_fire_afterburner (object* objp, ai_info* aip) {
    ...
    if (aip->ai_aburn_use_factor == INT_MIN && aip->ai_class == 0) {
        return 0; // Lowest level never aburners away (unless
                  // ai_aburn_use_factor is specified)
    }
    else {
        // Maybe don't afterburner because of a potential collision with the
        // player.      If not multiplayer, near player and player in front,
        // probably don't afterburner.
        if (!(Game_mode & GM_MULTIPLAYER)) {
            if (Ships[objp->instance].team == Player_ship->team) {
                float dist;

                dist = vm_vec_dist_quick (&objp->pos, &Player_obj->pos) -
                       Player_obj->radius - objp->radius;
                if (dist < 150.0f) {
                    vec3d v2p;
                    float dot;
        ...
        if (aip->ai_aburn_use_factor == INT_MIN &&
            aip->ai_class >= Num_ai_classes - 2)
            return 1; // Highest two levels always aburner away (unless
                      // ai_aburn_use_factor is specified).
        else {
            // If ai_aburn_use_factor is not specified, calculate a number
            // based on the AI class. Otherwise, use that value.
            if (aip->ai_aburn_use_factor == INT_MIN)
                return static_rand_timed (
                    OBJ_INDEX (objp), Num_ai_classes - aip->ai_class);
            else
                return static_rand_timed (
                    OBJ_INDEX (objp), aip->ai_aburn_use_factor);
        }
    }
}
 ...
 */
void maybe_afterburner_after_ship_hit (
    object* objp, ai_info* aip, object* en_objp) {
    // Only do if facing a little away.
    if (en_objp != NULL) {
        vec3d v2e;

        vm_vec_normalized_dir (&v2e, &en_objp->pos, &objp->pos);
        if (vm_vec_dot (&v2e, &objp->orient.vec.fvec) > -0.5f) return;
    }
...
void evade_weapon () {
    ...
    if (is_instructor (Pl_objp)) return;
    ...
    if (aip->danger_weapon_objnum != -1) {
        if (Objects[aip->danger_weapon_objnum].signature ==
            aip->danger_weapon_signature)
            unlocked_weapon_objp = &Objects[aip->danger_weapon_objnum];
        else
            aip->danger_weapon_objnum =
                -1; // Signatures don't match, so no longer endangered.
    }

    if (locked_weapon_objp != NULL) {
        if (unlocked_weapon_objp != NULL) {
            if (vm_vec_dist_quick (&locked_weapon_objp->pos, &Pl_objp->pos) <
                1.5f * vm_vec_dist_quick (
                           &unlocked_weapon_objp->pos, &Pl_objp->pos))
    ...
    dot_from_enemy =
        vm_vec_dot (&weapon_objp->orient.vec.fvec, &vec_from_enemy);
    ...
    if (dot_from_enemy < 0.3f) {
        ...
        if ((dot_to_enemy < -0.5f) || (dot_to_enemy > 0.5f)) {
            ...
            // Turn hard left or right, depending on which gets out of way
            // quicker. SUSHI: Also possibly turn up or down.
            ...
            if (aip->ai_profile_flags
                ...
                if ((rdot < -0.5f) || (rdot > 0.5f))
                    vm_vec_scale_add (
                        &goal_point, &Pl_objp->pos, &Pl_objp->orient.vec.rvec,
                        -200.0f);
                else
                    vm_vec_scale_add (
                        &goal_point, &Pl_objp->pos, &Pl_objp->orient.vec.rvec,
...
// (Coded 2/20/98.  Works fine, but it's hard to see how to integrate it into
// the AI system.        Typically ships are moving so fast that a little sliding
// isn't enough to gain an advantage.    It's currently used to avoid
// collisions and could be used to evade weapon fire, but the latter     would
// be frustrating, I think.      This function is currently not called.)
void slide_face_ship () {
    ...
    if ((sip->max_vel.xyz.x == 0.0f) && (sip->max_vel.xyz.y == 0.0f)) return;
    ...
    ai_turn_towards_vector (
        &En_objp->pos, Pl_objp, sip->srotation_time, nullptr, nullptr, 0.0f,
        0);
    ...
    vm_vec_scale_add (
        &goal_pos, &En_objp->pos, &En_objp->orient.vec.rvec, right * 200.0f);
    vm_vec_scale_add (
        &goal_pos, &En_objp->pos, &En_objp->orient.vec.uvec, up * 200.0f);
    ...
    if (dist < 200.0f) {
        ...
        if (dot_from_enemy < 0.7f) { accelerate_ship (aip, 0.2f); }
        else {
            accelerate_ship (aip, 1.0f);
        }
    }
...
void evade_ship () {
    ...
    if (Game_skill_level == NUM_SKILL_LEVELS - 1) {
        ...
        accelerate_ship (
            aip, (float)(Game_skill_level + 2) / (NUM_SKILL_LEVELS + 1));

    if ((Missiontime - aip->submode_start_time > F1_0 / 2) &&
        (sip->afterburner_fuel_capacity > 0.0f)) {
        float percent_left =
            100.0f * shipp->afterburner_fuel / sip->afterburner_fuel_capacity;
    ...
    if (dist > 250.0f) {
        vec3d gp1, gp2;
        // If far away from enemy, circle, going to nearer of point far off
        // left or right wing
        vm_vec_scale_add (&gp1, &enemy_pos, &En_objp->orient.vec.rvec, 250.0f);
        vm_vec_scale_add (
            &gp2, &enemy_pos, &En_objp->orient.vec.rvec, -250.0f);
        if (vm_vec_dist_quick (&gp1, &Pl_objp->pos) <
            vm_vec_dist_quick (&gp2, &Pl_objp->pos))
            goal_point = gp1;
        else
            goal_point = gp2;
    }
        ...
        vm_vec_scale_add (
            &goal_point, &enemy_pos, &En_objp->orient.vec.fvec, -1000.0f);
    }
    else if (dot_from_enemy > 0.9f) {
        // If enemy pointing almost right at self, and self pointing close to
        // enemy, turn away from
        vec3d vec_to_enemy;
        float dot_to_enemy;
        ...
        if (dot_to_enemy > 0.75f) {
            // Used to go to En_objp's right vector, but due to banking while
            // turning, that    caused flying in an odd spiral.
            vm_vec_scale_add (
                &goal_point, &enemy_pos, &Pl_objp->orient.vec.rvec, 1000.0f);
            if (dist < 100.0f) bank_override = Pl_objp->phys_info.speed;
        }
        else {
            bank_override =
                Pl_objp->phys_info.speed; // In enemy's sights, not pointing
                                          // at him, twirl away.
            goto evade_ship_l1;
        }
    }
    ...
    evade_ship_l1:;
        if (aip->ai_evasion > myrand () * 100.0f / 32767.0f) {
            int temp;
            float scale;
            float psrandval; // some value close to zero to choose whether to
                             // turn right or left.

            psrandval =
                (float)(((Missiontime >> 14) & 0x0f) - 8); // Value between
                                                           //-8 and 7
            psrandval =
                psrandval / 16.0f; // Value between -1/2 and 1/2 (approx)

            // If not close to behind, turn towards his right or left vector,
            // whichever won't cross his path.
            if (vm_vec_dot (&vec_from_enemy, &En_objp->orient.vec.rvec) >
                psrandval) {
                scale = 1000.0f;
            }
            else {
            ...
            vm_vec_scale_add (
                &goal_point, &enemy_pos, &En_objp->orient.vec.rvec, scale);

            temp = ((Missiontime >> 16) & 0x07);
            temp = ((temp * (temp + 1)) % 16) / 2 - 4;
            if ((psrandval == 0) && (temp == 0)) temp = 3;
            ...
            // Reason: If you don't, you lose rotational momentum.  Turning
            // every other frame,       and not in between results in a very slow
            // turn because of loss of momentum.
            if ((aip->prev_goal_point.xyz.x != 0.0f) ||
                (aip->prev_goal_point.xyz.y != 0.0f) ||
                (aip->prev_goal_point.xyz.z != 0.0f))
                goal_point = aip->prev_goal_point;
            else
                vm_vec_scale_add (
                    &goal_point, &enemy_pos, &En_objp->orient.vec.rvec,
                    100.0f);
        }
    }
...
void ai_evade () { evade_ship (); }

float G_collision_time;
vec3d G_predicted_pos, G_fire_pos;

// old version of this fuction, this will be useful for playing old missions
// and not having the new primary selection code throw off the balance of the
// mission.     If:             flags & Weapon::Info_Flags::Puncture    Then Select
// a Puncture
// weapon.      Else            Select Any ol' weapon.  Returns primary_bank index.
static int
ai_select_primary_weapon_OLD (const object* objp, Weapon::Info_Flags flags) {
    ship* shipp = &Ships[objp->instance];
    ...
    if (flags == Weapon::Info_Flags::Puncture) {
        if (swp->current_primary_bank >= 0) {
            ...
            if (Weapon_info[swp->primary_bank_weapons[bank_index]]
        ...
        // AL 26-3-98: If we couldn't find a puncture weapon, pick first
        // available weapon if one isn't active
        if (swp->current_primary_bank < 0) {
            if (swp->num_primary_banks > 0) { swp->current_primary_bank = 0; }
        }
    }
    else { // Don't need to be using a puncture weapon.
        if (swp->current_primary_bank >= 0) {
            if (!(Weapon_info
                      [swp->primary_bank_weapons[swp->current_primary_bank]]
                          .wi_flags[Weapon::Info_Flags::Puncture])) {
                return swp->current_primary_bank;
            }
        }
        for (int i = 0; i < swp->num_primary_banks; i++) {
            if (swp->primary_bank_weapons[i] > -1) {
                if (!(Weapon_info[swp->primary_bank_weapons[i]]
                          .wi_flags[Weapon::Info_Flags::Puncture])) {
                    swp->current_primary_bank = i;
                    WARNINGF (LOCATION, "%i: Ship %s selecting weapon %s",Framecount, Ships[objp->instance].ship_name,Weapon_info[swp->primary_bank_weapons[i]].name);
                    return i;
                }
            }
        }
        // Wasn't able to find a non-puncture weapon.  Stick with what we
        // have.
    }
...
/**
 * Etc. Etc. This is like the 4th rewrite of the code here. Special thanks to
 * Bobboau for finding the get_shield_strength function.
 *
 * The AI will now intelligently choose the best weapon to use based on the
 * overall shield status of the target.
 */
int ai_select_primary_weapon (
    object* objp, object* other_objp, Weapon::Info_Flags flags) {
    // Pointer Set Up
    ...
    if (!(Ai_info[shipp->ai_index].ai_profile_flags
              [AI::Profile_Flags::Smart_primary_weapon_selection])) {
    ...
    // made it so it only selects puncture weapons if the active goal is to
    // disable something -Bobboau
    if ((flags == Weapon::Info_Flags::Puncture) &&
        (Ai_info[shipp->ai_index].goals[0].ai_mode &
         (AI_GOAL_DISARM_SHIP | AI_GOAL_DISABLE_SHIP))) {
        if (swp->current_primary_bank >= 0) {
            int bank_index;
    ...
    if ((other_objp->type == OBJ_SHIP) &&
        (Ship_info[Ships[other_objp->instance].ship_info_index]
             .is_big_or_huge ())) {
        // Check if we have a capital+ weapon on board
        for (int i = 0; i < swp->num_primary_banks; i++) {
            if (swp->primary_bank_weapons[i] >
                -1) // Make sure there is a weapon in the bank
            {
                if (Weapon_info[swp->primary_bank_weapons[i]]
    ...
    if (enemy_remaining_shield <= 0.05f) {
        // Then it is safe to start using a heavy hull damage weapon such as
        // the maxim
        int i;
        float i_hullfactor_prev = 0; // Previous weapon bank hull factor (this
                ...
                -1) // Make sure there is a weapon in the bank
            {
                if ((((Weapon_info[swp->primary_bank_weapons[i]]
                    ...
                    if (!(Weapon_info[swp->primary_bank_weapons[i]]
                        ...
                        i_hullfactor_prev =
                            (((Weapon_info[swp->primary_bank_weapons[i]]
                                   .armor_factor) *
                              (Weapon_info[swp->primary_bank_weapons[i]]
                                   .damage)) /
                             Weapon_info[swp->primary_bank_weapons[i]]
        ...
        if (i_hullfactor_prev_bank == -1) // In the unlikely instance we don't
                                          // find at least 1 candidate weapon
        {
            i_hullfactor_prev_bank = 0; // Just switch to the first one
        }
        swp->current_primary_bank =
            i_hullfactor_prev_bank;    // Select the best weapon
        return i_hullfactor_prev_bank; // Return
    }

    // if the shields are above lets say 10% definitely use a pierceing weapon
    // if there are any-Bobboau
    if (enemy_remaining_shield >= 0.10f) {
        if (swp->current_primary_bank >= 0) {
            ...
            if ((Weapon_info[swp->primary_bank_weapons[bank_index]]
                     .wi_flags[Weapon::Info_Flags::Pierce_shields]) &&
                !(Weapon_info[swp->primary_bank_weapons[bank_index]]
                      .wi_flags[Weapon::Info_Flags::Capital_plus])) {
                return swp->current_primary_bank;
            }
        }
    ...
    if (enemy_remaining_shield <= 0.50f) {
        ...
        float i_hullfactor_prev = 0; // Previous weapon bank hull factor (this
                                     // is the safe way to do it)
        int i_hullfactor_prev_bank = -1; // Bank which gave us this hull factor
        // Find the weapon with the highest average hull and shield * damage /
        // fire delay factor
        for (i = 0; i < swp->num_primary_banks; i++) {
            if (swp->primary_bank_weapons[i] >
                -1) // Make sure there is a weapon in the bank
...
void set_primary_weapon_linkage (object* objp) {
    ...
    if ((aip->target_objnum != -1) &&
        ...
        // If trying to destroy a big ship (i.e., not disable/disarm), always
        // unleash all weapons
        if (Ship_info[Ships[Objects[aip->target_objnum].instance]
                          .ship_info_index]
                .is_big_ship ()) {
    ...
    if (Num_weapons > (int)(MAX_WEAPONS * 0.75f) ||
        ...
        if (shipp->flags[Ship::Ship_Flags::Primary_linked])
            WARNINGF (LOCATION, "Frame %i, ship %s: Unlinking primaries.",Framecount, shipp->ship_name);
        shipp->flags.remove (Ship::Ship_Flags::Primary_linked);
        return; // If low on slots or primary linking disallowed, don't link.
    }

    // AL 2-11-98: If ship has a disarm or disable goal, don't link unless both
    // weapons are
    // puncture weapons
    if ((aip->active_goal != AI_GOAL_NONE) &&
        (aip->active_goal != AI_ACTIVE_GOAL_DYNAMIC)) {
        if (aip->goals[aip->active_goal].ai_mode &
            (AI_GOAL_DISABLE_SHIP | AI_GOAL_DISARM_SHIP)) {
            // only continue if both primaries are puncture weapons
            if (swp->num_primary_banks == 2) {
                if (!(Weapon_info[swp->primary_bank_weapons[0]]
                          .wi_flags[Weapon::Info_Flags::Puncture]))
                    return;
                if (!(Weapon_info[swp->primary_bank_weapons[1]]
                          .wi_flags[Weapon::Info_Flags::Puncture]))
                    return;
            }
        }
    ...
    if (!(The_mission.ai_profile
              ->flags[AI::Profile_Flags::Allow_primary_link_at_start])) {
    ...
    float energy;
    if (The_mission.ai_profile
            ->flags[AI::Profile_Flags::Fix_linked_primary_bug]) {
...
int ai_fire_primary_weapon (object* objp) {
    ...
    if (!Ai_firing_enabled) { return 0; }

    if (aip->target_objnum != -1) {
        enemy_objp = &Objects[aip->target_objnum];
        enemy_sip =
            (enemy_objp->type == OBJ_SHIP)
                ? &Ship_info[Ships[enemy_objp->instance].ship_info_index]
                : NULL;
    }
    else {
        enemy_objp = NULL;
    ...
    if ((swp->current_primary_bank < 0) ||
        ...
        ship_primary_changed (shipp); // AL: maybe send multiplayer information
                                      // when AI ship changes primaries
        aip->primary_select_timestamp = timestamp (
            5 * 1000); // Maybe change primary weapon five seconds from now.
    }

    // If pointing nearly at predicted collision point of target, bash
    // orientation to be perfectly pointing.
    float dot;
    vec3d v2t;
    ...
    if (!(vm_vec_mag_quick (&G_predicted_pos) < AICODE_SMALL_MAGNITUDE) &&
        ...
        if (vm_vec_cmp (&G_predicted_pos, &G_fire_pos)) {
            ...
            if (dot > .998629534f) { // if within 3.0 degrees of desired
                                     // heading, bash
                vm_vector_2_matrix (
                    &objp->orient, &v2t, &objp->orient.vec.uvec, NULL);
            }
        }
    }

    // SUSHI: Burst-fire for ballistic primaries.
    if (The_mission.ai_profile->primary_ammo_burst_mult[Game_skill_level] >
            0 && // Make sure we are using burst fire
        enemy_objp != NULL &&
        ...
        Weapon_info[swp->primary_bank_weapons[swp->current_primary_bank]]
            .wi_flags[Weapon::Info_Flags::Ballistic]) // Current weapon must be
                                                      // ballistic
    {
        float percentAmmoLeft =
            ((float)swp->primary_bank_ammo[swp->current_primary_bank] /
             (float)swp->primary_bank_start_ammo[swp->current_primary_bank]);
        ...
        dotToTarget = pow (
            dotToTarget, 4); // This makes the dot a tiny bit more impactful
                             // (otherwise nearly always over 0.98 or so)
        float fof_spread_cooldown_factor =
            1.0f - swp->primary_bank_fof_cooldown[swp->current_primary_bank];

        // Combine factors
        float burstFireProb =
            ((0.6f * percentAmmoLeft) + (0.4f * distanceFactor)) *
            dotToTarget * aip->ai_primary_ammo_burst_mult *
    ...
    // Make sure not firing at a protected ship unless firing at a live
    // subsystem.       Note: This happens every time the ship tries to fire,
    // perhaps every frame.     Should be wrapped in a timestamp, same one that
    // enables it to fire, but that is complicated      by multiple banks it can
    // fire from.
    if (aip->target_objnum != -1) {
        ...
        if (tobjp->flags[Object::Object_Flags::Protected]) {
            if (aip->targeted_subsys != NULL) {
                ...
                if (ship_get_subsystem_strength (
                        &Ships[tobjp->instance], type) == 0.0f) {
                    aip->target_objnum = -1;
                    return 0;
                }
            }
    ...
    // If enemy is protected, not firing a puncture weapon and enemy's hull is
    // low, don't fire.
    if ((enemy_objp != NULL) &&
        (enemy_objp->flags[Object::Object_Flags::Protected])) {
        // AL: 3-6-98: Check if current_primary_bank is valid
        if ((enemy_objp->hull_strength < 750.0f) &&
            ((aip->targeted_subsys == NULL) ||
             (enemy_objp->hull_strength <
              aip->targeted_subsys->current_hits + 50.0f)) &&
            (swp->current_primary_bank >= 0)) {
            if (!(Weapon_info
                      [swp->primary_bank_weapons[swp->current_primary_bank]]
                          .wi_flags[Weapon::Info_Flags::Puncture])) {
                swp->next_primary_fire_stamp[swp->current_primary_bank] =
                    timestamp (1000);
                return 0;
            }
        }
...
// pos                                     =>      world position to measure ship
// distances from                               threshold                       =>      max distance from pos
...
int num_nearby_fighters (int enemy_team_mask, vec3d* pos, float threshold) {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (iff_matches_mask (
                Ships[ship_objp->instance].team, enemy_team_mask)) {
            if (Ship_info[Ships[ship_objp->instance].ship_info_index]
                    .is_fighter_bomber ()) {
...
// If 2 or more, if the current weapon is one of them, stick with it,
// otherwise choose a random one.       priority1 and priority2 are Weapon_info[]
// bitmasks such as WIF_HOMING_ASPECT.  If any weapon has any bit in priority1
...
void ai_select_secondary_weapon (
    ...
    flagset< Weapon::Info_Flags >* priority2 = NULL) {
    ...
    num_weapon_types = get_available_secondary_weapons (
        objp, weapon_id_list, weapon_bank_list);
    ...
    if ((aip->ai_profile_flags
        ...
        ignore_mask.set (Weapon::Info_Flags::Homing_aspect)
            .set (Weapon::Info_Flags::Homing_heat)
            .set (Weapon::Info_Flags::Homing_javelin);
    }
    ...
    for (i = 0; i < num_weapon_types; i++) {
        auto wi_flags =
            Weapon_info[swp->secondary_bank_weapons[weapon_bank_list[i]]]
                .wi_flags;
        auto ignore_mask_to_use =
            ((aip->ai_profile_flags
                  [AI::Profile_Flags::Smart_secondary_weapon_selection]) &&
             (wi_flags[Weapon::Info_Flags::Bomber_plus]))
                ? (ignore_mask - Weapon::Info_Flags::Huge)
                : ignore_mask;

        if (!(wi_flags & ignore_mask_to_use)
                 .any_set ()) { // Maybe bombs are illegal.
            if ((wi_flags & prio1).any_set ()) {
                swp->current_secondary_bank =
                    weapon_bank_list[i]; // Found first priority, return it.
                break;
            }
            else if ((wi_flags & prio2).any_set ())
                priority2_index =
                    weapon_bank_list[i]; // Found second priority, but might
                                         // still find first priority.
        }
    }
    ...
    if (i == num_weapon_types) {
        swp->current_secondary_bank =
            priority2_index; // Assume we won't find anything.
        if (priority2_index == -1) {
            for (i = 0; i < num_weapon_types; i++) {
                auto wi_flags =
                    Weapon_info
                        [swp->secondary_bank_weapons[weapon_bank_list[i]]]
                            .wi_flags;
                auto ignore_mask_to_use =
                    ((aip->ai_profile_flags
                          [AI::Profile_Flags::
    ...
    if (swp->current_secondary_bank >= 0 &&
        ...
        if ((aip->ai_profile_flags
                 [AI::Profile_Flags::Allow_rapid_secondary_dumbfire]) &&
            !(wip->is_homing ()) && (wip->fire_wait < .5f)) {
            aip->ai_flags.set (AI::AI_Flags::Unload_secondaries);
        }
    }

    ship_secondary_changed (
        &Ships[objp->instance]); // AL: let multiplayer know if secondary bank
                                 // has changed
}
...
int compute_num_homing_objects (object* target_objp) {
    ...
    for (objp = GET_FIRST (&obj_used_list);
        ...
        if (objp->type == OBJ_WEAPON) {
            if (Weapon_info[Weapons[objp->instance].weapon_info_index]
                    .is_homing ()) {
...
void ai_maybe_announce_shockwave_weapon (
    object* firing_objp, int weapon_index) {
    if ((firing_objp->type == OBJ_SHIP) &&
        (Weapon_info[weapon_index].shockwave.speed > 0.0f)) {
        ship_obj* so;
        ...
        for (so = GET_FIRST (&Ship_obj_list);
            ...
            if (Ships[A->instance].team == firing_ship_team) {
                ...
                if (!object_is_docked (A) &&
                    !(aip->ai_flags
                          [AI::AI_Flags::Repairing,
                           AI::AI_Flags::Being_repaired])) {
...
float compute_incoming_payload (object* target_objp) {
    ...
    for (mo = GET_NEXT (&Missile_obj_list);
        ...
        if (Weapons[objp->instance].homing_object == target_objp) {
            payload +=
                Weapon_info[Weapons[objp->instance].weapon_info_index].damage;
        }
    }
...
// a large ship, but that large ship is not defending itself with missiles.
int check_ok_to_fire (int objnum, int target_objnum, weapon_info* wip) {
    ...
    if (target_objnum >= 0) {
        ...
        if (tobjp->type == OBJ_SHIP) {
            if (Ship_info[Ships[tobjp->instance].ship_info_index]
                    .is_small_ship ()) {
                num_homers =
                    compute_num_homing_objects (&Objects[target_objnum]);
            }
        }

        // If player, maybe fire based on Skill_level and number of incoming
        // weapons.     If non-player, maybe fire based on payload of incoming
        // weapons.
        if (wip->is_homing ()) {
            if ((target_objnum > -1) &&
                (tobjp->flags[Object::Object_Flags::Player_ship])) {
                if (Ai_info[Ships[tobjp->instance].ai_index].target_objnum !=
                    objnum) {
                    // Don't allow AI ships to fire at player for fixed
                    // periods of time based on skill level.    With 5 skill
                    // levels, at Very Easy, they fire in 1/7 of every 10
                    // second interval.         At Easy, 2/7...at Expert, 5/7
                    int t =
                        ((Missiontime / (65536 * 10)) ^ target_objnum ^ 0x01) %
                        (NUM_SKILL_LEVELS + 2);
                    if (t > Ai_info[Ships[tobjp->instance].ai_index]
                                .ai_chance_to_use_missiles_on_plr) {
                        return 0;
                    }
                }
                int swarmers = 0;
                if (wip->wi_flags[Weapon::Info_Flags::Swarm])
                    swarmers = 2; // Note, always want to be able to fire
                                  // swarmers if no currently incident homers.
                if (The_mission.ai_profile
                        ->max_allowed_player_homers[Game_skill_level] <
                    num_homers + swarmers) {
                    return 0;
                }
            }
                ...
                incoming_payload =
                    compute_incoming_payload (&Objects[target_objnum]);

                if (incoming_payload > tobjp->hull_strength) { return 0; }
            }
        }
    }
    else {
...
int ai_fire_secondary_weapon (object* objp) {
...
#ifndef NDEBUG
    if (!Ai_firing_enabled) return rval;
#endif
    ...
    if (current_bank == -1) { return rval; }

    ASSERT (current_bank < shipp->weapons.num_secondary_banks);

    weapon_info* wip =
        &Weapon_info[shipp->weapons.secondary_bank_weapons[current_bank]];

    if ((wip->is_locked_homing ()) &&
        (!Ai_info[shipp->ai_index].current_target_is_locked)) {
        swp->next_secondary_fire_stamp[current_bank] = timestamp (250);
    }
    else if (
        (wip->wi_flags[Weapon::Info_Flags::Bomb]) ||
        (vm_vec_dist_quick (&objp->pos, &En_objp->pos) > 50.0f)) {
        // This might look dumb, firing a bomb even if closer than 50 meters,
        // but the reason is, if you're carrying        bombs, delivering them is
        // probably more important than surviving.
        ai_info* aip;

        aip = &Ai_info[shipp->ai_index];

        // Note, maybe don't fire if firing at player and any homers yet
        // fired.       Decreasing chance to fire the more homers are incoming on
        // player.
...
/**
 * Return true if it looks like obj1, if continuing to move along current
 * vector, will collide with obj2.
 */
int might_collide_with_ship (
    object* obj1, object* obj2, float dot_to_enemy, float dist_to_enemy,
    float duration) {
    if (obj1->phys_info.speed * duration + 2 * (obj1->radius + obj2->radius) >
        dist_to_enemy)
        if (dot_to_enemy >
            0.8f - 2 * (obj1->radius + obj2->radius) / dist_to_enemy)
...
int might_hit_teammate (object* firing_objp) {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (Ships[objp->instance].team == team) {
            ...
            dot = vm_vec_dot (&firing_objp->orient.vec.fvec, &vec_to_objp) /
                  dist;
...
void render_all_ship_bay_paths (object* objp) {
    ...
    pm =
        model_get (Ship_info[Ships[objp->instance].ship_info_index].model_num);
    vec3d global_path_point;
    vertex v, prev_vertex;

    if (pm->ship_bay == NULL) return;
    ...
    for (i = 0; i < pm->ship_bay->num_paths; i++) {
        ...
        for (j = 0; j < mp->nverts; j++) {
            vm_vec_unrotate (
                &global_path_point, &mp->verts[j].pos, &objp->orient);
            ...
            if (j == mp->nverts - 1) { gr_set_color (255, 0, 0); }

            g3_draw_sphere (&v, 1.5f);

            if (j > 0) g3_draw_line (&v, &prev_vertex);
...
void render_path_points (object* objp) {
    ...
    if (aip->goal_objnum < 0) return;

    dobjp = &Objects[aip->goal_objnum];
    pm = model_get (
        Ship_info[Ships[dobjp->instance].ship_info_index].model_num);
    ...
    if (pm->n_docks) {
        ...
        model_instance_find_world_point (
            &global_dock_point, &dock_point, shipp->model_instance_num, 0,
            &dobjp->orient, &dobjp->pos);
    ...
    if (aip->path_start != -1) {
        ...
        for (i = 0; i < num_points; i++) {
            ...
            if (i != 0) g3_draw_line (&v0, &prev_vertex);

            if (pp - Path_points == aip->path_cur) gr_set_color (255, 255, 0);
...
float ai_get_weapon_dist (ship_weapon* swp) {
    ...
    if (weapon_num == -1) { return 1000.0f; }

    return MIN (
        (Weapon_info[weapon_num].max_speed * Weapon_info[weapon_num].lifetime),
        Weapon_info[weapon_num].weapon_range);
}
...
float ai_get_weapon_speed (ship_weapon* swp) {
    ...
    bank_num = swp->current_primary_bank;
    if (bank_num < 0) return 100.0f;

    weapon_num = swp->primary_bank_weapons[bank_num];

    if (weapon_num == -1) { return 100.0f; }
...
weapon_info* ai_get_weapon (ship_weapon* swp) {
    ...
    bank_num = swp->current_primary_bank;
    if (bank_num < 0) return NULL;

    weapon_num = swp->primary_bank_weapons[bank_num];

    if (weapon_num == -1) { return NULL; }

    return &Weapon_info[weapon_num];
}

// Compute the predicted position of a ship to be fired upon from a turret.
// This is based on position of firing gun, enemy object, weapon speed and
// skill level constraints.     Return value in *predicted_enemy_pos.   Also,
// stuff globals G_predicted_pos, G_collision_time and G_fire_pos.      *pobjp
// object firing the weapon     *eobjp          object being fired upon
void set_predicted_enemy_pos_turret (
    vec3d* predicted_enemy_pos, vec3d* gun_pos, object* pobjp,
    vec3d* enemy_pos, vec3d* enemy_vel, float weapon_speed,
    float time_enemy_in_range) {
    ship* shipp = &Ships[pobjp->instance];
    float range_time;

    if (weapon_speed < 1.0f) weapon_speed = 1.0f;
    ...
    if (time_enemy_in_range < range_time) {
        ...
        vm_vec_scale_add (
            predicted_enemy_pos, enemy_pos, enemy_vel,
            time_enemy_in_range * dist / weapon_speed);
    }
    else {
        ...
        collision_time = compute_collision_time (
            enemy_pos, enemy_vel, gun_pos, weapon_speed);

        if (collision_time == 0.0f) { collision_time = 100.0f; }

        vm_vec_scale_add (
            predicted_enemy_pos, enemy_pos, enemy_vel, collision_time);
...
// This is based on current position of firing object, enemy object, relative
// position of gun on firing object,    weapon speed and skill level
// constraints.         Return value in *predicted_enemy_pos.   Also, stuff globals
// G_predicted_pos, G_collision_time and G_fire_pos.
// SUSHI: Modified to take in a position and accel value instead of reading it
// directly from the enemy object
void set_predicted_enemy_pos (
    vec3d* predicted_enemy_pos, object* pobjp, vec3d* enemy_pos,
    vec3d* enemy_vel, ai_info* aip) {
    ...
    if (wip != NULL &&
        The_mission.ai_profile
            ->flags[AI::Profile_Flags::Use_additive_weapon_velocity])
        vm_vec_scale_sub2 (
            &target_moving_direction, &pobjp->phys_info.vel,
            wip->vel_inherit_amount);
    ...
    if (aip->time_enemy_in_range < range_time) {
        ...
        dist = vm_vec_dist_quick (&pobjp->pos, enemy_pos);
        vm_vec_scale_add (
            predicted_enemy_pos, enemy_pos, &target_moving_direction,
            aip->time_enemy_in_range * dist / weapon_speed);
    }
    else {
        ...
        // Compute position of gun in absolute space and use that as fire
        // position
        // ...unless we want to just use the ship center
        if (pm->gun_banks != NULL &&
            !(The_mission.ai_profile
                  ->flags[AI::Profile_Flags::Ai_aims_from_ship_center])) {
            pnt = pm->gun_banks[0].pnt[0];
        }
        else {
            // Use the convergence offset, if there is one
            vm_vec_copy_scale (
                &pnt, &Ship_info[shipp->ship_info_index].convergence_offset,
                1.0f);
        }
        ...
        collision_time = compute_collision_time (
            enemy_pos, &target_moving_direction, &gun_pos, weapon_speed);
    ...
    if (aip->time_enemy_in_range > 2 * range_time) {
        ...
        scale = (1.0f - aip->ai_accuracy) * 4.0f *
                (1.0f +
                 4.0f * (1.0f - aip->time_enemy_in_range / (2 * range_time)));
    }

    // if this ship is under the effect of an EMP blast, throw his aim off a
    // bit
    if (shipp->emp_intensity > 0.0f) {
        // never go lower than 1/2 of the EMP effect max, otherwise things
        // aren't noticeable
        scale +=
            (MAX_EMP_INACCURACY *
             (shipp->emp_intensity < 0.5f ? 0.5f : shipp->emp_intensity));
        WARNINGF (LOCATION, "AI miss scale factor (EMP) %f", scale);
    }
    ...
    if (aip->ai_flags[AI::AI_Flags::Stealth_pursuit]) {
        ...
        float st_err = 3.0f * (1.4f - dot) *
                       (1.0f + dist / (get_skill_stealth_dist_scaler () *
                                       STEALTH_MAX_VIEW_DIST)) *
                       (1 - aip->ai_accuracy);
        scale += st_err;
    }
...
void ai_chase_ct () {
    ...
    if (vm_vec_dist_quick (&tvec, &Pl_objp->pos) < 0.1f) {
        ...
        if (aip->submode_parm0 == 0) aip->submode_parm0 = 1;
        vm_vec_add2 (&tvec, &Pl_objp->orient.vec.rvec);
    }

    ai_turn_towards_vector (
        &tvec, Pl_objp, sip->srotation_time, nullptr, nullptr, 0.0f, 0);
    accelerate_ship (aip, 1.0f);
}
 ...
 */
static void
ai_chase_eb (ai_info* aip, ship_info* sip, vec3d* predicted_enemy_pos) {
    vec3d _pep;
    float dot_to_enemy, dot_from_enemy;

    compute_dots (Pl_objp, En_objp, &dot_to_enemy, &dot_from_enemy);

    // If we're trying to slow down to get behind, then point to turn towards
    // is different.
    _pep = *predicted_enemy_pos;
    if ((dot_to_enemy > dot_from_enemy + 0.1f) || (dot_to_enemy > 0.9f))
        vm_vec_scale_add (
            &_pep, &Pl_objp->pos, &En_objp->orient.vec.fvec, 100.0f);

    ai_turn_towards_vector (
        &_pep, Pl_objp, sip->srotation_time, nullptr, nullptr, 0.0f, 0);
...
float ai_endangered_time (object* ship_objp, object* weapon_objp) {
    ...
    // Only proceed if weapon sort of pointing at object and object pointing
    // towards or away from weapon      (Ie, if object moving at right angle to
    // weapon, just continue for now...)
...
float ai_endangered_by_weapon (ai_info* aip) {
    ...
    if (aip->danger_weapon_objnum == -1) { return -1.0f; }
    ...
    return ai_endangered_time (
        &Objects[Ships[aip->shipnum].objnum], weapon_objp);
}
 ...
 */
void attack_set_accel (
    ai_info* aip, ship_info* sip, float dist_to_enemy, float dot_to_enemy,
    float dot_from_enemy) {
    float speed_ratio;
    ...
    if (aip->ai_flags[AI::AI_Flags::Attack_slowly]) {
        if ((dist_to_enemy > 200.0f) && (dist_to_enemy < 800.0f)) {
            if ((dot_from_enemy < 0.9f) || ai_near_full_strength (Pl_objp)) {
                accelerate_ship (
                    aip, MAX (1.0f - (dist_to_enemy - 200.0f) / 600.0f, 0.1f));
                return;
            }
        }
        else
    ...
    // Glide attack: we turn on glide to maintain current vector while aiming
    // at the enemy The aiming part should already be taken care of.
    ...
    // Circle Strafe: We try to maintain a constant distance from the target
    // while using sidethrust to move in a circle around the target
    if (aip->submode == AIS_CHASE_CIRCLESTRAFE) {
        // If glide is available, use it (smooths things out a bit)
        if (sip->can_glide == true) Pl_objp->phys_info.flags |= PF_GLIDING;

        // Try to maintain a distance between 50% and 75% of maximum circle
        // strafe distance
    ...
    if (dist_to_enemy >
        200.0f + vm_vec_mag_quick (&En_objp->phys_info.vel) * dot_from_enemy +
            Pl_objp->phys_info.speed * speed_ratio) {
        if (ai_maybe_fire_afterburner (Pl_objp, aip)) {
            if (dist_to_enemy > 800.0f) {
                if (!(Pl_objp->phys_info.flags & PF_AFTERBURNER_ON)) {
                    ...
                    if (sip_local->afterburner_fuel_capacity > 0.0f) {
                        percent_left = 100.0f * shipp->afterburner_fuel /
                                       sip_local->afterburner_fuel_capacity;
                        if (percent_left >
                            30.0f + ((OBJ_INDEX (Pl_objp)) & 0x0f)) {
                            afterburners_start (Pl_objp);
                            if (aip->ai_profile_flags
                                    [AI::Profile_Flags::
                                ...
                                // Max afterburner speed - make sure we don't
                                // devide by 0 later
                                max_ab_vel =
                                    sip_local->afterburner_max_vel.xyz.z > 0.0f
                                        ? sip_local->afterburner_max_vel.xyz.z
                                        : sip_local->max_vel.xyz.z;
                                max_ab_vel =
                                    max_ab_vel > 0.0f ? max_ab_vel : 0.0001f;

                                // Time to exhaust 25% of the remaining fuel
                                time_to_exhaust_25pct_fuel =
                                    shipp->afterburner_fuel * 0.25f /
                                    sip_local->afterburner_burn_rate;

                                // Time to fly 75% of the distance to the
                                // target
                                time_to_fly_75pct_of_distance =
                                    dist_to_enemy * 0.75f / max_ab_vel;
        ...
        (Missiontime - aip->last_hit_time > F1_0 * 7) &&
        (En_objp->phys_info.speed < 10.0f) && (dist_to_enemy > 25.0f) &&
        (dot_to_enemy > 0.8f) && (dot_from_enemy < 0.8f)) {
        accelerate_ship (
            aip, 0.0f); // No one attacking us, so don't need to move.
    }
        ...
        if (dot_from_enemy > 0.75f)
            ...
            set_accel_for_target_speed (
                Pl_objp, En_objp->phys_info.speed * 0.75f + 3.0f);
    }
    else {
...
// New on 2/21/98: If this ship can move backwards and slide, maybe do that to
// get behind.
static void get_behind_ship (ai_info* aip, ship_info* sip) {
    ...
    vm_vec_scale_add (
        &new_pos, &En_objp->pos, &En_objp->orient.vec.fvec,
        -100.0f); // Pick point 100 units behind.
    ai_turn_towards_vector (
        &new_pos, Pl_objp, sip->srotation_time, nullptr, nullptr, 0.0f, 0);

    dot = vm_vec_dot (&vec_from_enemy, &En_objp->orient.vec.fvec);

    if (dot > 0.25f) { accelerate_ship (aip, 1.0f); }
    else {
...
int avoid_player (object* objp, vec3d* goal_pos) {
    ...
    if (aip->ai_flags[AI::AI_Flags::Avoiding_small_ship]) {
        ...
        ai_turn_towards_vector (
            &aip->avoid_goal_point, objp, sip->srotation_time, nullptr,
            nullptr, 0.0f, 0);
...
// Determine if a cylinder of width radius from p0 to p1 will collide with
// big_objp.    If so, stuff *collision_point.
int will_collide_pp (
    vec3d* p0, vec3d* p1, float radius, object* big_objp,
    vec3d* collision_point) {
    mc_info mc;
    mc_info_init (&mc);

    polymodel* pm = model_get (
        Ship_info[Ships[big_objp->instance].ship_info_index].model_num);
    ...
    if (mc.num_hits) *collision_point = mc.hit_point_world;

    return mc.num_hits;
}

// Return true/false if *objp will collide with *big_objp
// Stuff distance in *distance to collision point if *objp will collide with
//*big_objp within delta_time seconds.  Global collision point stuffed in
//*collision_point
int will_collide_with_big_ship (
    object* objp, vec3d* goal_point, object* big_objp, vec3d* collision_point,
    float delta_time) {
    float radius;
    vec3d end_pos;
    ...
    if (vm_vec_dist_quick (&big_objp->pos, &objp->pos) > radius) { return 0; }

    if (goal_point == NULL) {
        vm_vec_scale_add (
            &end_pos, &objp->pos, &objp->phys_info.vel,
    ...
    return will_collide_pp (
        &objp->pos, &end_pos, objp->radius, big_objp, collision_point);
}

// Return true if *objp is expected to collide with a large ship.
// Stuff global collision point in *collision_point.
// If *goal_point is not NULL, use that as the point towards which *objp will
// be flying.  Don't use *objp velocity         *ignore_objp will typically be the
// target this ship is pursuing, either to attack or guard.  We don't want to
// avoid it.
int will_collide_with_big_ship_all (
    object* objp, object* ignore_objp, vec3d* goal_point,
    vec3d* collision_point, float* distance, float delta_time) {
    ship_obj* so;
    object* big_objp;
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (big_objp == ignore_objp) continue;

        if (Ship_info[Ships[big_objp->instance].ship_info_index]
                .is_big_or_huge ()) {
            vec3d cur_collision_point;
            float cur_dist;

            if (will_collide_with_big_ship (
                    objp, goal_point, big_objp, &cur_collision_point,
                    delta_time)) {
...
// Pick a point for *objp to fly towards to avoid a collision with *big_objp
// at *collision_point  Return result in *avoid_pos
void mabs_pick_goal_point (
    object* objp, object* big_objp, vec3d* collision_point, vec3d* avoid_pos) {
    matrix mat1;
    sgoal goals[4];
    vec3d v2b;
    ...
    // First try 0.5f to see if we can find a point that near the center of
    // the target ship, which presumably        means less of a turn.   Try going
    // as far as 1.25f * radius.
    float s;
    for (s = 0.5f; s < 1.3f; s += 0.25f) {
        int i;
        for (i = 0; i < 4; i++) {
            vec3d p = big_objp->pos;
            float ku = big_objp->radius * s +
                       objp->radius * (OBJ_INDEX (objp) % 4) /
                           4; // This objp->radius stuff to prevent ships
                              // from glomming together at one point
            float kr = big_objp->radius * s +
                       objp->radius * ((OBJ_INDEX (objp) % 4) ^ 2) / 4;
            if (i & 1) ku = -ku;
            if (i & 2) kr = -kr;
            ...
            goals[i].collide = will_collide_pp (
                &objp->pos, &p, objp->radius, big_objp, collision_point);
            if (!goals[i].collide) found = 1;
        }

        // If we found a point that doesn't collide, find the nearest one and
        // make that the *avoid_pos.
        if (found) {
    ...
    // Drat.  We tried and tried and could not find a point that did not cause
    // a collision.     Get this dump pilot far away from the problem ship.
    vec3d away_vec;
    vm_vec_normalized_dir (&away_vec, &objp->pos, collision_point);
    vm_vec_scale_add (
        avoid_pos, &objp->pos, &away_vec, big_objp->radius * 1.5f);
}

/**
 * Return true if a large ship is being ignored.
 */
int maybe_avoid_big_ship (
    object* objp, object* ignore_objp, ai_info* aip, vec3d* goal_point,
    float delta_time) {
    if (timestamp_elapsed (aip->avoid_check_timestamp)) {
        float distance;
        vec3d collision_point;
        int ship_num;
        if ((ship_num = will_collide_with_big_ship_all (
                 Pl_objp, ignore_objp, goal_point, &collision_point, &distance,
                 delta_time)) != -1) {
            aip->ai_flags.set (AI::AI_Flags::Avoiding_big_ship);
            mabs_pick_goal_point (
                objp, &Objects[ship_num], &collision_point,
                &aip->avoid_goal_point);
            float dist =
    ...
    if (aip->ai_flags[AI::AI_Flags::Avoiding_big_ship]) {
        ...
        ai_turn_towards_vector (
            &aip->avoid_goal_point, Pl_objp, sip->srotation_time, nullptr,
            nullptr, 0.0f, 0);
 ...
 * Since this is governed only by vector to target, it causes ships to align
 * bank and look less chaotic.
 */
void compute_desired_rvec (vec3d* rvec, vec3d* goal_pos, vec3d* cur_pos) {
    ...
    if (vm_vec_mag_squared (rvec) < 0.001f) rvec->xyz.y = 1.0f;
}
...
void ai_stealth_find () {
    ...
    if (delta_time > 10000) { aip->submode_parm0 = SM_SF_BAIL; }

    // guestimate new position
    vm_vec_scale_add (
        &new_pos, &aip->stealth_last_pos, &aip->stealth_velocity,
        (delta_time * 0.001f));

    // if I think he's behind me, go to the goal point
    if (aip->submode_parm0 == SM_SF_BEHIND) { new_pos = aip->goal_point; }
    ...
    // if i think i should see him ahead and i don't, set goal pos and turn
    // around, but only if I haven't seen him for a while
    if ((delta_time > 800) && (aip->submode_parm0 == SM_SF_AHEAD) &&
        (dot_to_enemy > .94) &&
        (dist_to_enemy <
         get_skill_stealth_dist_scaler () * STEALTH_MAX_VIEW_DIST + 50)) {
        // do turn around)
        vm_vec_scale_add (
            &aip->goal_point, &Pl_objp->pos, &Pl_objp->orient.vec.fvec,
            -300.0f);
    ...
    // If enemy more than 500 meters away, all ships flying there will tend to
    // match bank.      They do this by using their vector to their target to
    // compute their right vector and causing ai_turn_towards_vector    to
    // interpolate a matrix rather than just a vector.
...
void ai_stealth_sweep () {
    ...
    if (aip->submode_parm0 == SM_SS_SET_GOAL) {
        ...
        vm_vec_scale_add (
            &goal_pt, &aip->stealth_last_pos, &aip->stealth_velocity,
            (0.001f * lost_time));

        // make box size based on speed of stealth and expected time to
        // intercept (keep box in range 200-500)
        float box_size =
            vm_vec_mag_quick (&aip->stealth_velocity) * (0.001f * lost_time);
    ...
    // if velocity changes in stealth mode, then ship is *seen*, and falls out
    // of sweep mode if stealth has no velocity make a velocity
    ...
    case SM_SS_BOX0: goal_pt = aip->goal_point; break;

    // pt1 -U +R
    case SM_SS_LR:
        vm_vec_scale_add (
            &goal_pt, &aip->goal_point, &up, -aip->stealth_sweep_box_size);
        vm_vec_scale_add2 (&goal_pt, &right, aip->stealth_sweep_box_size);
        vm_vec_scale_add2 (
            &goal_pt, &forward, 0.5f * aip->stealth_sweep_box_size);
        break;
    ...
    // pt3 back
    case SM_SS_BOX1: goal_pt = aip->goal_point; break;
    ...
    if (dist_to_goal < 15) { aip->submode_parm0++; }
    ...
    ai_turn_towards_vector (
        &goal_pt, Pl_objp, sip->srotation_time, nullptr, nullptr, 0.0f, 0);
 ...
 * ATTACK submode handler for chase mode.
 */
void ai_chase_attack (
    ai_info* aip, ship_info* sip, vec3d* predicted_enemy_pos,
    float dist_to_enemy, int modelnum) {
    int start_bank;
    float dot_to_enemy, dot_from_enemy;
    float bank_override = 0.0f;

    if (!(aip->ai_profile_flags[AI::Profile_Flags::No_special_player_avoid]) &&
        avoid_player (Pl_objp, predicted_enemy_pos))
        return;
    ...
    // If ship moving slowly relative to its size, then don't attack its
    ...
    if (En_objp->radius > En_objp->phys_info.speed) {
        ...
        scale *= 0.5f * En_objp->radius /
                 (En_objp->phys_info.speed +
                  En_objp->radius); // scale downward by 1/2 to 1/4
    ...
    if (dist_to_enemy < 250.0f && dot_from_enemy > 0.7f &&
    ...
    if (dist_to_enemy > 500.0f) {
        ...
        ai_turn_towards_vector (
            &new_pos, Pl_objp, sip->srotation_time, nullptr, rel_pos,
            bank_override, 0, &rvec);
    }
    else {
        ai_turn_towards_vector (
            &new_pos, Pl_objp, sip->srotation_time, nullptr, rel_pos,
...
void ai_chase_es (ai_info* aip, ship_info* sip) {
    ...
    if (timeslice & 0x01)
        vm_vec_scale_add2 (
            &tvec, &Pl_objp->orient.vec.rvec, f2fl (scale ^ 0x10000));
    if (timeslice & 0x02)
        vm_vec_scale_sub2 (&tvec, &Pl_objp->orient.vec.rvec, f2fl (scale));
    if (timeslice & 0x04)
        vm_vec_scale_add2 (
            &tvec, &Pl_objp->orient.vec.uvec, f2fl (scale ^ 0x10000));
    if (timeslice & 0x08)
    ...
    ai_turn_towards_vector (
        &tvec, Pl_objp, sip->srotation_time, nullptr, nullptr, bank_override,
        0);
    accelerate_ship (aip, 1.0f);
}
...
void ai_chase_ga (ai_info* aip, ship_info* sip) {
    // If not near end of this submode, evade squiggly.  If near end, just fly
    // straight for a bit
    vec3d tvec;
    float bank_override;
    vec3d vec_from_enemy;
    ...
    if (ai_maybe_fire_afterburner (Pl_objp, aip)) {
        if (!(Pl_objp->phys_info.flags & PF_AFTERBURNER_ON)) {
            float percent_left = 100.0f *
                                 Ships[Pl_objp->instance].afterburner_fuel /
                                 sip->afterburner_fuel_capacity;
            if (percent_left > 30.0f + ((OBJ_INDEX (Pl_objp)) & 0x0f)) {
                afterburners_start (Pl_objp);
                aip->afterburner_stop_time = Missiontime + 3 * F1_0 / 2;
...
int ai_set_attack_subsystem (object* objp, int subnum) {
    ...
    // MWA -- 2/27/98.  Due to AL's changes, target_objnum is now not always
    // valid (at least sometimes in terms of goals).  So, bail if we don't have
    // a valid target.
    if (aip->target_objnum == -1) return 0;

    attacked_objp = &Objects[aip->target_objnum];
    shipp = &Ships[attacked_objp->instance]; // need to get our target's ship
                                             // pointer!!!
    ...
    // -- Done at caller in ai_process_mission_orders -- attacked_objp->flags
    // .add(Object::Object_Flags::Protected);
...
void ai_set_guard_vec (object* objp, object* guard_objp) {
    ...
    if (radius > 300.0f) { radius = guard_objp->radius * 1.25f; }

    vm_vec_sub (&aip->guard_vec, &objp->pos, &guard_objp->pos);

    if (vm_vec_mag (&aip->guard_vec) > 3.0f * radius) {
        // Far away, don't just use vector to object, causes clustering of
        // guard ships.
        vec3d tvec, rvec;
        float mag;
...
void ai_set_guard_wing (object* objp, int wingnum) {
    ...
    // This function is called whenever a guarded ship is destroyed, so this
    // code     prevents a ship from trying to guard a non-existent wing.
    if (Wings[wingnum].current_count < 1) {
        ...
        if (leader_objnum == OBJ_INDEX (objp)) { return; }
...
void ai_set_guard_object (object* objp, object* other_objp) {
    ...
    // If ship to guard is in a wing, guard that whole wing, unless the
    ...
    if ((other_aip->wing != -1) && (other_aip->wing != aip->wing) &&
        !(The_mission.ai_profile
              ->flags[AI::Profile_Flags::Ai_guards_specific_ship_in_wing])) {
        ai_set_guard_wing (
            objp, Ai_info[Ships[other_objp->instance].ai_index].wing);
    }
    else {
        other_objnum = OBJ_INDEX (other_objp);
        ...
        ASSERT (
            other_objnum >=
            0); // Hmm, bogus object and we need its position for guard_vec.
...
void update_aspect_lock_information (
    ai_info* aip, vec3d* vec_to_enemy, float dist_to_enemy,
    float enemy_radius) {
    float dot_to_enemy;
    int num_weapon_types;
    int weapon_id_list[MAX_WEAPON_TYPES], weapon_bank_list[MAX_WEAPON_TYPES];
    ...
    // AL 3-7-98: This probably should never happen, but check to ensure that
    // current_secondary_bank is valid
    if ((swp->current_secondary_bank < 0) ||
        (swp->current_secondary_bank > swp->num_secondary_banks)) {
        return;
    }

    if (tobjp->type == OBJ_SHIP) { tshpp = &Ships[tobjp->instance]; }

    num_weapon_types = get_available_secondary_weapons (
        Pl_objp, weapon_id_list, weapon_bank_list);

    wip =
        &Weapon_info[swp->secondary_bank_weapons[swp->current_secondary_bank]];

    if (num_weapon_types && (wip->is_locked_homing ()) &&
        !(shipp->flags[Ship::Ship_Flags::No_secondary_lockon])) {
        aip->ai_flags.set (
            AI::AI_Flags::Seek_lock,
            dist_to_enemy > 300.0f - MIN (enemy_radius, 100.0f));
        ...
        if (dot_to_enemy > needed_dot &&
            (wip->wi_flags[Weapon::Info_Flags::Homing_aspect] ||
             (wip->wi_flags[Weapon::Info_Flags::Homing_javelin] &&
              (tshpp == NULL ||
               ship_get_closest_subsys_in_sight (
                   tshpp, SUBSYSTEM_ENGINE, &aiobjp->pos))))) {
            ...
            if (aip->aspect_locked_time < 0.0f) aip->aspect_locked_time = 0.0f;
        }
    }
    else {
        aip->current_target_is_locked = 0;
        aip->aspect_locked_time =
            0.0f; // Used to be this, why?: wip->min_lock_time;
...
void ai_chase_fly_away (object* objp, ai_info* aip) {
    ...
    if (aip->ai_flags[AI::AI_Flags::Target_collision]) {
        aip->ai_flags.remove (
            AI::AI_Flags::Target_collision); // Don't process this hit again
                                             // next frame.
    ...
    if ((aip->target_objnum == -1) ||
        (Objects[aip->target_objnum].signature != aip->target_signature)) {
        abort_flag = 1;
    }
    ...
    if (abort_flag || (Missiontime > aip->submode_start_time + F1_0)) {
        ...
        vm_vec_normalized_dir (
            &v2e, &Objects[aip->target_objnum].pos, &objp->pos);
...
// "favored" means SEXPs have specified the weapon as being good to fire at
// en_objp.
    ...
    for (i = 0; i < swp->num_secondary_banks; i++) {
        if (swp->secondary_bank_capacity[i] > 0) {
            if (swp->secondary_bank_ammo[i] > 0) {
                if (is_preferred_weapon (
                        swp->secondary_bank_weapons[i], objp, en_objp) != -1) {
                    return i;
                }
            }
        }
...
// Note, this is not like ai_select_secondary_weapon().  "choose" means make a
// choice.      "select" means execute an order.  Get it?       This function calls
// ai_select_secondary_weapon() with the characteristics it should search for.
void ai_choose_secondary_weapon (object* objp, ai_info* aip, object* en_objp) {
    ...
    if (preferred_secondary != -1) {
        ...
        if (esip) { is_big_ship = esip->is_big_or_huge (); }
        else {
            is_big_ship = 0;
        }
        ...
        if (is_big_ship) {
            ...
            (aip->ai_profile_flags
                 [AI::Profile_Flags::Smart_secondary_weapon_selection]) &&
            (en_objp->type ==
             OBJ_ASTEROID)) // prefer dumbfires if its an asteroid
        {
            wif_priority1.reset ();
            wif_priority2.reset ();
        }
...
/**
 * Return time, in seconds, at which this ship can next fire its current
 * secondary weapon.
 */
float set_secondary_fire_delay (
    ai_info* aip, ship* shipp, weapon_info* swip, bool burst) {
    float t;
    if (burst) { t = swip->burst_delay; }
    else {
...
void ai_chase_big_approach_set_goal (
    vec3d* goal_pos, object* attack_objp, object* target_objp, float* accel) {
    float dist_to_goal;
    ...
    // set accel
    if (dist_to_goal > 400.0f) { *accel = 1.0f; }
    else {
        *accel = dist_to_goal / 400.0f;
    }
}

void ai_chase_big_circle_set_goal (
    vec3d* goal_pos, object* attack_objp, object* target_objp, float* accel) {
    get_tangent_point (
        goal_pos, attack_objp, &target_objp->pos,
        attack_objp->radius + target_objp->radius + 100.0f);
 ...
 */
void ai_chase_big_get_separations (
    object* attack_objp, object* target_objp, vec3d* horz_vec_to_target,
    ...
    polymodel* pm = model_get (
        Ship_info[Ships[attack_objp->instance].ship_info_index].model_num);
    ...
    // subtract off perp component to get "horizontal" separation vector
    // between cylinders [ASSUMING parallel]
    vm_vec_scale_add (
        horz_vec_to_target, &vec_to_target, &target_objp->orient.vec.fvec,
        -perp_dist);
...
void ai_chase_big_parallel_set_goal (
    vec3d* goal_pos, object* attack_objp, object* target_objp, float* accel) {
    int opposing;
    float temp, r_target, r_attacker;
    ...
    opposing =
        (vm_vec_dot (
             &attack_objp->orient.vec.fvec, &target_objp->orient.vec.fvec) <
         0);

    ai_chase_big_get_separations (
        attack_objp, target_objp, &horz_vec_to_target, &optimal_separation,
        &separation);

    // choose dist (2000) so that we don't bash
    float dist = 2000;
    if (opposing) { dist = -dist; }

    // set the goal pos as dist forward from target along target forward
    vm_vec_scale_add (
        goal_pos, &target_objp->pos, &target_objp->orient.vec.fvec, dist);
    // then add horizontal separation
    vm_vec_scale_add2 (
        goal_pos, &horz_vec_to_target, optimal_separation / separation);

    // find the distance between centers along forward direction of ships
    vec3d vec_to_target;
    vm_vec_sub (&vec_to_target, &target_objp->pos, &attack_objp->pos);
    float perp_dist =
        vm_vec_dot (&vec_to_target, &target_objp->orient.vec.fvec);

    float match_accel = 0.0f;
    float length_scale = attack_objp->radius;

    if (Ship_info[Ships[attack_objp->instance].ship_info_index]
            .max_vel.xyz.z != 0.0f) {
        match_accel = target_objp->phys_info.vel.xyz.z /
                      Ship_info[Ships[attack_objp->instance].ship_info_index]
                          .max_vel.xyz.z;
    }

    // if we're heading toward enemy ship, we want to keep going if we're ahead
    if (opposing) { perp_dist = -perp_dist; }

    if (perp_dist > 0) {
        // falling behind, so speed up
        *accel =
            match_accel + (1.0f - match_accel) / length_scale * (perp_dist);
    }
    else {
...
void ai_cruiser_chase_set_goal_pos (
    vec3d* goal_pos, object* pl_objp, object* en_objp) {
    ai_info* aip;
...
// (Note, called "ai_cruiser_chase" because we already have ai_chase_big()
// which means fighter chases big ship.
void ai_cruiser_chase () {
    ...
    vec3d goal_pos;
    float turn_time =
        Ship_info[Ships[Pl_objp->instance].ship_info_index].srotation_time;

    // kamikaze - ram and explode
    if (aip->ai_flags[AI::AI_Flags::Kamikaze]) {
        ai_turn_towards_vector (
            &En_objp->pos, Pl_objp, turn_time, nullptr, nullptr, 0.0f, 0);
        accelerate_ship (aip, 1.0f);
    }
    ...
    // really track down and chase
        ...
        // check valid submode
        ASSERT (
            (aip->submode == SM_ATTACK) || (aip->submode == SM_BIG_APPROACH) ||
        ...
        case SM_BIG_APPROACH:
            ...
            ai_chase_big_approach_set_goal (
                &goal_pos, Pl_objp, En_objp, &accel);
            // maybe set rvec
            break;
        ...
        // now move as desired
        ai_turn_towards_vector (
            &goal_pos, Pl_objp, turn_time, nullptr, nullptr, 0.0f, 0, rvecp);
        ...
        case SM_BIG_APPROACH:
            if (dist_to_enemy <
                (Pl_objp->radius + En_objp->radius) * 1.25f + 200.0f) {
                // moving
                if (moving) {
                    // if within 90 degrees of en forward, go into parallel,
                    // otherwise circle
                    if (vm_vec_dot (
                            &En_objp->orient.vec.fvec,
                            &Pl_objp->orient.vec.fvec) > 0) {
                        aip->submode = SM_BIG_PARALLEL;
                        aip->submode_start_time = Missiontime;
                    }
                }
        ...
        case SM_BIG_CIRCLE:
            ...
            if (moving) {
                ...
                if (vm_vec_dot (&vec_to_enemy, &En_objp->orient.vec.fvec) >
                    0) {
                    ...
                    if (vm_vec_dot (
                        ...
                        // get separation
                        ai_chase_big_get_separations (
                            Pl_objp, En_objp, &temp, &desired_sep, &cur_sep);
        ...
        case SM_BIG_PARALLEL:
            // we're opposing
            if (vm_vec_dot (
                    &Pl_objp->orient.vec.fvec, &En_objp->orient.vec.fvec) <
                0) {
                // and the other ship is moving
                if (moving) {
                    // and we no longer overlap
                    if (dist_to_enemy >
                        (0.75 * (En_objp->radius + Pl_objp->radius))) {
                        aip->submode = SM_BIG_APPROACH;
                        aip->submode_start_time = Missiontime;
                    }
                }
...
void ai_chase () {
    ...
    vec3d player_pos, enemy_pos, predicted_enemy_pos, real_vec_to_enemy,
    ...
    if ((sip->class_type > -1) && (En_objp->type == OBJ_SHIP)) {
        ...
        if (esip->class_type > -1) {
            ...
            WARNINGF (LOCATION,"AI-WARNING: No class_type specified for '%s', assuming that it's ok to chase!",esip->name);
        }
    }
    ...
    if (sip->class_type > -1 &&
        (Ship_types[sip->class_type]
             .flags[Ship::Type_Info_Flags::AI_attempt_broadside])) {
        ai_cruiser_chase ();
        return;
    }
    ...
    if (En_objp->type == OBJ_SHIP) {
        enemy_sip_flags =
            Ship_info[Ships[En_objp->instance].ship_info_index].flags;
    ...
    if (enemy_sip_flags.any_set ()) {
        if (Ship_info[Ships[En_objp->instance].ship_info_index]
                .is_big_or_huge ()) {
            ai_big_chase ();
            return;
        }
    ...
    // This should prevent the embarrassing behavior of ships getting stuck on
    // each other       as if they were magnetically attracted. -- MK, 11/13/97.
    if ((aip->ai_flags[AI::AI_Flags::Target_collision]) ||
        (aip->submode == SM_FLY_AWAY)) {
    ...
    // Enemy position for the purpose of aiming is already calculated
    // differently, do it explicitly here
    ai_update_aim (aip);

    vm_vec_normalize (&real_vec_to_enemy);

    real_dot_to_enemy =
        vm_vec_dot (&real_vec_to_enemy, &Pl_objp->orient.vec.fvec);

    int is_stealthy_ship = 0;
    if ((enemy_sip_flags.any_set ()) &&
        (enemy_shipp_flags[Ship::Ship_Flags::Stealth])) {
        if (ai_is_stealth_visible (Pl_objp, En_objp) !=
            STEALTH_FULLY_TARGETABLE) {
            is_stealthy_ship = 1;
        }
    }
    ...
    if (En_objp->type == OBJ_SHIP &&
        !(Ships[En_objp->instance]
              .flags[Ship::Ship_Flags::Hidden_from_sensors]) &&
        !is_stealthy_ship) {
        update_aspect_lock_information (
            aip, &real_vec_to_enemy, dist_to_enemy, En_objp->radius);
    }
    else {
        aip->current_target_is_locked = 0;
        aip->ai_flags.remove (AI::AI_Flags::Seek_lock);
    ...
    // If seeking lock, try to point directly at ship, else predict position
    // so lasers can hit it.    If just acquired target, or target is not in
    // reasonable cone, don't refine believed enemy position.
    if ((real_dot_to_enemy < 0.25f) || (aip->target_time < 1.0f)) {
        ...
        set_predicted_enemy_pos (
            &predicted_enemy_pos, Pl_objp, &aip->last_aim_enemy_pos,
            &aip->last_aim_enemy_vel, aip); // Set G_fire_pos
        predicted_enemy_pos = enemy_pos;
        G_predicted_pos = predicted_enemy_pos;
    }
    else {
        ...
        if (aip->targeted_subsys != NULL) {
            ...
            if (get_shield_pct (En_objp) < HULL_DAMAGE_THRESHOLD_PERCENT) {
                if (aip->targeted_subsys != NULL) {
                    get_subsystem_pos (
                        &enemy_pos, En_objp, aip->targeted_subsys);
                    predicted_enemy_pos = enemy_pos;
                    predicted_vec_to_enemy = real_vec_to_enemy;
                }
                else {
                    set_predicted_enemy_pos (
                        &predicted_enemy_pos, Pl_objp,
                        &aip->last_aim_enemy_pos, &aip->last_aim_enemy_vel,
                        aip);
    ...
    dot_to_enemy =
        vm_vec_dot (&Pl_objp->orient.vec.fvec, &predicted_vec_to_enemy);
    dot_from_enemy =
        -vm_vec_dot (&En_objp->orient.vec.fvec, &real_vec_to_enemy);

    // Set turn and acceleration based on submode.
    switch (aip->submode) {
    case SM_CONTINUOUS_TURN: ai_chase_ct (); break;

    case SM_STEALTH_FIND: ai_stealth_find (); break;

    case SM_STEALTH_SWEEP: ai_stealth_sweep (); break;
    ...
    case AIS_CHASE_CIRCLESTRAFE:
        if (vm_vec_dist_quick (&Pl_objp->pos, &predicted_enemy_pos) >
            100.0f + En_objp->radius * 2.0f) {
            if (maybe_avoid_big_ship (
                    Pl_objp, En_objp, aip, &predicted_enemy_pos, 10.0f))
                return;
        }

        ai_chase_attack (
            aip, sip, &predicted_enemy_pos, dist_to_enemy, sip->model_num);
        break;

    case SM_EVADE_SQUIGGLE: ai_chase_es (aip, sip); break;

    case SM_EVADE_BRAKE: ai_chase_eb (aip, sip, &predicted_enemy_pos); break;

    case SM_EVADE: evade_ship (); break;

    case SM_AVOID: avoid_ship (); break;

    case SM_GET_BEHIND: get_behind_ship (aip, sip); break;

    case SM_GET_AWAY: // Used to get away from opponent to prevent endless
                      // circling.
        ai_chase_ga (aip, sip);
        break;

    case SM_EVADE_WEAPON: evade_weapon (); break;
    ...
    // The following are valid targets for random sidethrust (circle strafe
    // uses it too, but that is handled separately)
    if (aip->submode == SM_ATTACK || aip->submode == SM_SUPER_ATTACK ||
        aip->submode == SM_EVADE_SQUIGGLE || aip->submode == SM_EVADE ||
        aip->submode == SM_GET_AWAY || aip->submode == AIS_CHASE_GLIDEATTACK) {
        // Re-roll for random sidethrust every 2 seconds
        // Also, only do this if we've recently been hit or are in current
        // primary weapon range of target
    ...
    if ((aip->submode != SM_AVOID) && (aip->submode != SM_ATTACK_FOREVER)) {
        ...
        if (Missiontime - aip->last_attack_time > i2f (6)) {
            if ((aip->submode != SM_SUPER_ATTACK) &&
                ...
                !(aip->ai_flags[AI::AI_Flags::Stealth_pursuit])) {
        ...
        // SUSHI: Alternate stalemate dection method: if nobody has hit each
        // other for a while (and we've been near that whole time), shake
        // things
        // up somehow Only do this if stalemate time threshold > 0
        if (aip->ai_stalemate_time_thresh > 0.0f &&
            Missiontime - aip->last_hit_target_time >
                fl2f (aip->ai_stalemate_time_thresh) &&
            Missiontime - aip->last_hit_time >
                fl2f (aip->ai_stalemate_time_thresh) &&
            aip->time_enemy_near > aip->ai_stalemate_time_thresh &&
            (dot_to_enemy <
             0.95f - 0.5f * En_objp->radius /
        ...
        // If enemy is pointing away and moving a bit, don't worry about
        // collision detection.
        if ((dot_from_enemy > 0.5f) || (En_objp->phys_info.speed < 10.0f)) {
            // If we're in circle strafe mode, don't worry about colliding with
            // the target
            if (aip->submode != AIS_CHASE_CIRCLESTRAFE &&
                might_collide_with_ship (
                    Pl_objp, En_objp, dot_to_enemy, dist_to_enemy, 4.0f)) {
                if ((Missiontime - aip->last_hit_time > F1_0 * 4) &&
                    (dist_to_enemy <
                     Pl_objp->radius * 2 + En_objp->radius * 2)) {
                    accelerate_ship (aip, -1.0f);
                }
                else {
                    aip->submode = SM_AVOID;
    ...
    case SM_ATTACK:
        // if target is stealth and stealth not visible, then enter stealth
        // find mode
        if ((aip->ai_flags[AI::AI_Flags::Stealth_pursuit]) &&
            (ai_is_stealth_visible (Pl_objp, En_objp) ==
             STEALTH_NOT_IN_FRUSTUM)) {
            ...
            ai_near_full_strength (Pl_objp) &&
            (Missiontime - aip->last_hit_target_time > i2f (3)) &&
            ...
            (En_objp->phys_info.speed > 15.0f) && (dist_to_enemy < 200.0f) &&
            (dist_to_enemy > 50.0f) && (dot_to_enemy < 0.1f) &&
            (Missiontime - aip->submode_start_time > i2f (2))) {
            aip->submode = SM_EVADE_BRAKE;
            aip->submode_start_time = Missiontime;
        }
        else if (
            (dot_to_enemy > 0.2f) && (dot_from_enemy > -0.2f) &&
            (dot_from_enemy < 0.1f)) {
            ...
            enemy_sip != NULL && (enemy_sip->is_small_ship ()) &&
            (dist_to_enemy < 150.0f) &&
            (dot_from_enemy > dot_to_enemy + 0.5f + aip->ai_courage * .002)) {
            float get_away_chance =
                (aip->ai_get_away_chance == FLT_MIN)
    ...
    case SM_EVADE_SQUIGGLE:
        if ((Missiontime - aip->submode_start_time > i2f (5)) ||
            (dist_to_enemy > 300.0f)) {
            if ((dist_to_enemy < 100.0f) && (dot_to_enemy < 0.0f) &&
                (dot_from_enemy > 0.5f)) {
                aip->submode = SM_EVADE_BRAKE;
            }
            else if (
    ...
    case SM_EVADE:
        // Modified by MK on 5/5/97 to keep trying to regain attack mode. It's
        // what a human would do.
        if ((dot_to_enemy < 0.2f) && (dot_from_enemy < 0.8f) &&
            (dist_to_enemy < 100.0f) && (En_objp->phys_info.speed > 15.0f)) {
            aip->last_attack_time = Missiontime;
            aip->submode = SM_EVADE_BRAKE;
            aip->submode_start_time = Missiontime;
        }
        else if (
            ((dot_to_enemy > dot_from_enemy - 0.1f) &&
             (Missiontime > aip->submode_start_time + i2f (1))) ||
            (dist_to_enemy >
             150.0f + 2 * (Pl_objp->radius + En_objp->radius))) {
            aip->last_attack_time = Missiontime;
            aip->submode = SM_ATTACK;
            aip->submode_start_time = Missiontime;
        }
    ...
    case SM_SUPER_ATTACK:
        ...
        if ((aip->ai_flags[AI::AI_Flags::Stealth_pursuit]) &&
            ...
            (dist_to_enemy < 100.0f) && (dot_to_enemy < 0.8f) &&
            (enemy_sip != NULL) && (enemy_sip->is_small_ship ()) &&
            (Missiontime - aip->submode_start_time > i2f (5))) {
            aip->ai_flags.remove (
                AI::AI_Flags::Attack_slowly); // Just in case, clear here.
            ...
            case 4:
                if (dot_from_enemy +
                        (NUM_SKILL_LEVELS - Game_skill_level) * 0.1f >
                    dot_to_enemy) { // Less likely to GET_AWAY at lower skill
                                    // levels.
                    aip->submode = SM_EVADE;
                    aip->submode_start_time = Missiontime;
                }
    ...
    case SM_AVOID:
        if ((dot_to_enemy > -0.2f) &&
            (dist_to_enemy / (dot_to_enemy + 0.3f) < 100.0f)) {
            aip->submode_start_time = Missiontime;
        }
        else if (Missiontime - aip->submode_start_time > i2f (1) / 2) {
            if (might_collide_with_ship (
                    Pl_objp, En_objp, dot_to_enemy, dist_to_enemy, 3.0f)) {
                aip->submode_start_time = Missiontime;
            }
            else {
                aip->submode = SM_GET_BEHIND;
    ...
    case SM_GET_AWAY:
        if (Missiontime - aip->submode_start_time > i2f (2)) {
            ...
            rand_dist = ((Missiontime >> 17) & 0x03) * 100.0f +
                        200.0f; // Some value in 200..500
            if ((Missiontime - aip->submode_start_time > i2f (5)) ||
                ...
                aip->time_enemy_in_range =
                    2.0f; // Cheat.  Presumably if they were running away
                          // from you, they were monitoring you!
                aip->last_attack_time = Missiontime;
            }
        }
        break;
    ...
    case SM_STEALTH_FIND:
        ...
        if (!(aip->ai_flags[AI::AI_Flags::Stealth_pursuit]) ||
            (ai_is_stealth_visible (Pl_objp, En_objp) == STEALTH_IN_FRUSTUM)) {
            aip->submode = SM_ATTACK;
            aip->submode_start_time = Missiontime;
            aip->last_attack_time = Missiontime;
    ...
    case AIS_CHASE_CIRCLESTRAFE:
        // Break out of circle strafe if the target is too far, moving too
        // fast, or we've been doing this for a while
        if ((dist_to_enemy > CIRCLE_STRAFE_MAX_DIST + En_objp->radius) ||
            (Missiontime - aip->submode_start_time > i2f (8)) ||
            (En_objp->phys_info.speed >
             MAX (sip->max_vel.xyz.x, sip->max_vel.xyz.y) * 1.5)) {
            aip->submode = SM_ATTACK;
            aip->submode_start_time = Missiontime;
            aip->last_attack_time = Missiontime;
        }
    ...
    if (aip->ai_stalemate_dist_thresh > 0.0f &&
        ...
        if (aip->time_enemy_near < 0.0f) aip->time_enemy_near = 0.0f;
    }
    ...
    if (aip->mode != AIM_EVADE) {
        if (dot_to_enemy >
            ...
            // Chance of hitting ship is based on dot product of firing ship's
            // forward vector with vector to ship       and also the size of the
            // target relative to distance to target.
            if (dot_to_enemy >
                MAX (
                    0.5f, 0.90f + aip->ai_accuracy / 10.0f -
                              En_objp->radius / MAX (1.0f, dist_to_enemy))) {
                ship* temp_shipp;
                ship_weapon* tswp;
                ...
                if (tswp->num_primary_banks > 0) {
                    float scale;
                    ASSERT (
                        tswp->current_primary_bank < tswp->num_primary_banks);
                    weapon_info* pwip =
                        &Weapon_info[tswp->primary_bank_weapons
                                         [tswp->current_primary_bank]];

                    // Less likely to fire if far away and moving.
                    scale = pwip->max_speed /
                            (En_objp->phys_info.speed + pwip->max_speed);
                    if (scale > 0.6f)
                ...
                if (tswp->num_secondary_banks > 0) {
                    ...
                    if (!(En_objp->flags[Object::Object_Flags::Protected])) {
                        ...
                        if (current_bank > -1) {
                            weapon_info* swip =
                                &Weapon_info[tswp->secondary_bank_weapons
                                                 [current_bank]];
                            if (aip->ai_flags
                                    [AI::AI_Flags::Unload_secondaries]) {
                                if (timestamp_until (
                                        swp->next_secondary_fire_stamp
                                            [current_bank]) >
                                    swip->fire_wait * 1000.0f) {
                                    swp->next_secondary_fire_stamp
                                        [current_bank] = timestamp (
                                        (int)(swip->fire_wait * 1000.0f));
                                }
                            }
                            ...
                            if (timestamp_elapsed (
                                ...
                                if (current_bank >= 0) {
                                    ...
                                    if (swip->wi_flags
                                            [Weapon::Info_Flags::Local_ssm])
                                        firing_range =
                                            swip->lssm_lock_range; // that
                                                                   // should be
                                                                   // enough
                                    else if (swip->wi_flags
                                                 [Weapon::Info_Flags::Bomb])
                                        firing_range = MIN (
                                            (swip->max_speed * swip->lifetime *
                                             0.75f),
                                            swip->weapon_range);
                                    else {
                                        float secondary_range_mult =
                                            (aip->ai_secondary_range_mult ==
                                             FLT_MIN)
                                                ? (float)(Game_skill_level + 1 + (3 * aip->ai_class / (Num_ai_classes - 1))) /
                                                      NUM_SKILL_LEVELS
                                                : aip->ai_secondary_range_mult;

                                        firing_range = MIN (
                                            (swip->max_speed * swip->lifetime *
                                             secondary_range_mult),
                                            swip->weapon_range);
                                    }
                                    ...
                                    // If firing a spawn weapon, distance
                                    // doesn't matter.
                                    int spawn_fire = 0;

                                    if (swip->wi_flags
                                            [Weapon::Info_Flags::Spawn]) {
                                        int count;

                                        count = num_nearby_fighters (
                                            iff_get_attackee_mask (
                                                obj_team (Pl_objp)),
                                            &Pl_objp->pos, 1000.0f);
                                        ...
                                        if (count > 3)
                                            ...
                                            float hull_percent =
                                                Pl_objp->hull_strength /
                                                temp_shipp
                                                    ->ship_max_hull_strength;
                                    ...
                                    if (spawn_fire ||
                                        (dist_to_enemy < firing_range)) {
                                        if (ai_fire_secondary_weapon (
                                                Pl_objp)) {
                                            // Only if weapon was fired do we
                                            // specify time until next fire. If
                                            // not fired, done in
                                            // ai_fire_secondary...
                                            float t;
                                            int current_bank_adjusted =
                                            ...
                                            swp->next_secondary_fire_stamp
                                                [current_bank] =
                                                timestamp ((int)(t * 1000.0f));
                                        }
                                    }
                                    else {
                                        swp->next_secondary_fire_stamp
                                            [current_bank] = timestamp (250);
                                    }
                                }
                            }
                        }
    ...
    if (has_fired == -1) { ship_stop_fire_primary (Pl_objp); }
}

/**
 * Make the object *objp move so that the point *start moves towards the point
 * *finish.
 * @return distance.
 */
float dock_move_towards_point (
    object* objp, vec3d* start, vec3d* finish, float speed_scale,
    float other_obj_speed = 0.0f, rotating_dockpoint_info* rdinfo = NULL) {
    ...
    if (dist > 0.0f) {
        ...
        if (rdinfo && (rdinfo->submodel >= 0)) {
            ...
            case DOA_UNDOCK_1:
                ...
                speed +=
                    (vm_vec_dist (&rdinfo->submodel_pos, &objp->pos) *
                     rdinfo->submodel_w);
                break;
            }
        }
...
void set_goal_dock_orient (
    matrix* dom, vec3d* docker_p0, vec3d* docker_p1, vec3d* docker_p0_norm,
    matrix* docker_orient, vec3d* dockee_p0, vec3d* dockee_p1,
    vec3d* dockee_p0_norm, matrix* dockee_orient) {
    ...
    // Pre-multiply the orientation of the source object (docker_orient) by
    // the transpose    of the docking bay's orientation, ie unrotate the
    // source object's matrix.
    vm_transpose (&m2);
    vm_matrix_x_matrix (dom, &m3, &m2);
}

// Goober5000
// Return the rotating submodel on which is mounted the specified dockpoint, or
// -1 for none.
int find_parent_rotating_submodel (polymodel* pm, int dock_index) {
    ASSERTX (pm != nullptr, "pm cannot be null!");
    ASSERTX (
        dock_index >= 0 && dock_index < pm->n_docks,
        "for model %s, dock_index %d must be >= 0 and < %d!", pm->filename,
        dock_index, pm->n_docks);
    int path_num, submodel;

    // make sure we have a spline path to check against before going any
    // further
    if (pm->docking_bays[dock_index].num_spline_paths <= 0) { return -1; }
    ...
    if ((path_num >= 0) && (path_num < pm->n_paths)) {
        ...
        if ((submodel >= 0) && (submodel < pm->n_models) &&
            (pm->submodel[submodel].movement_type >= 0)) {
            return submodel;
        }
    }

    // if path doesn't exist or the submodel doesn't exist or the submodel
    // doesn't move
    return -1;
}
...
void find_adjusted_dockpoint_normal (
    vec3d* global_p0_norm, object* objp, polymodel* pm, int submodel,
    int dock_index) {
    ASSERTX (
        global_p0_norm != nullptr && objp != nullptr && pm != nullptr,
    ...
    if (submodel >= 0) {
        ...
        model_instance_find_world_dir (
            global_p0_norm, &pm->docking_bays[dock_index].norm[0],
            Ships[objp->instance].model_instance_num, submodel, &objp->orient);
    }
...
// Goober5000
void find_adjusted_dockpoint_info (
    vec3d* global_p0, vec3d* global_p1, vec3d* global_p0_norm, object* objp,
    polymodel* pm, int modelnum, int submodel, int dock_index) {
    ASSERTX (
        global_p0 != nullptr && global_p1 != nullptr &&
            global_p0_norm != nullptr && objp != nullptr && pm != nullptr,
        "arguments cannot be null!");
    ASSERTX (pm->id == modelnum, "inconsistent polymodel and modelnum!");
    ASSERTX (
        dock_index >= 0 && dock_index < pm->n_docks,
        "for model %s, dock_index %d must be >= 0 and < %d!", pm->filename,
        dock_index, pm->n_docks);
    ...
    if (submodel >= 0) {
        ...
        vm_vec_sub (
            &local_p0, &pm->docking_bays[dock_index].pnt[0], &submodel_offset);
        vm_vec_sub (
            &local_p1, &pm->docking_bays[dock_index].pnt[1], &submodel_offset);

        // find the dynamic positions of the dockpoints
        model_instance_find_world_point (
            global_p0, &local_p0, shipp->model_instance_num, submodel,
            &objp->orient, &objp->pos);
        model_instance_find_world_point (
            global_p1, &local_p1, shipp->model_instance_num, submodel,
            &objp->orient, &objp->pos);

        // find the normal of the first dockpoint
        model_instance_find_world_dir (
            global_p0_norm, &pm->docking_bays[dock_index].norm[0],
            shipp->model_instance_num, submodel, &objp->orient);
    }
    // use the static dockpoints
    else {
        vm_vec_unrotate (
            global_p0, &pm->docking_bays[dock_index].pnt[0], &objp->orient);
        vm_vec_add2 (global_p0, &objp->pos);

        vm_vec_unrotate (
            global_p1, &pm->docking_bays[dock_index].pnt[1], &objp->orient);
...
// Returns distance to goal, defined as distance between corresponding dock
// points, plus 10.0f * rotational velocity vector (DOA_DOCK only) DOA_APPROACH
// means        approach point aip->path_cur    DOA_DOCK                        means dock
// DOA_UNDOCK_1 means undock, moving to point nearest dock bay  DOA_UNDOCK_2
// means undock, moving to point nearest dock bay and facing away from ship
// DOA_UNDOCK_3 means undock, moving directly away from ship    DOA_DOCK_STAY
// means rigidly maintain position in dock bay.
float dock_orient_and_approach (
    object* docker_objp, int docker_index, object* dockee_objp,
    int dockee_index, int dock_mode, rotating_dockpoint_info* rdinfo) {
    ship_info *sip0, *sip1;
    polymodel *pm0, *pm1;
    ai_info* aip;
    matrix dom, nm;
    ...
    docker_objp->phys_info.forward_thrust =
        0.0f; // Kill thrust so we don't have a sputtering thruster.

    // Goober5000 - moved out here to save calculations
    if (dock_mode != DOA_DOCK_STAY)
        if (ship_get_subsystem_strength (
                &Ships[docker_objp->instance], SUBSYSTEM_ENGINE) <= 0.0f)
            return 9999.9f;
    ...
    int dockee_rotating_submodel =
        find_parent_rotating_submodel (pm1, dockee_index);

    // Goober5000 - move docking points with submodels if necessary, for both
    // docker and dockee
    find_adjusted_dockpoint_info (
        &docker_p0, &docker_p1, &docker_p0_norm, docker_objp, pm0,
        sip0->model_num, -1, docker_index);
    find_adjusted_dockpoint_info (
        &dockee_p0, &dockee_p1, &dockee_p0_norm, dockee_objp, pm1,
        sip1->model_num, dockee_rotating_submodel, dockee_index);
    ...
    if ((dockee_rotating_submodel >= 0) && (dock_mode != DOA_DOCK_STAY)) {
        ...
        // get submodel center
        model_find_submodel_offset (
            &submodel_offset, sip1->model_num, dockee_rotating_submodel);
        vm_vec_add (&submodel_pos, &dockee_objp->pos, &submodel_offset);

        polymodel_instance* pmi1 = model_get_instance (
            Ships[dockee_objp->instance].model_instance_num);
        ...
        if (pmi1->submodel[dockee_rotating_submodel].sii != NULL) {
            submodel_omega =
                pmi1->submodel[dockee_rotating_submodel].sii->cur_turn_rate;
        }
    ...
    if (rdinfo == NULL) rdinfo = &rdinfo_buf;
    ...
    if (sip0->flags[Ship::Info_Flags::Support]) { speed_scale = 3.0f; }

    switch (dock_mode) {
    case DOA_APPROACH: {
        vec3d* goal_point;

        // Compute the desired global orientation matrix for the docker's
        // station.     That is, the normal vector of the docking station must
        // be the same as the   forward vector and the vector between its two
        // points must be the uvec.
        set_goal_dock_orient (
            &dom, &docker_p0, &docker_p1, &docker_p0_norm,
            &docker_objp->orient, &dockee_p0, &dockee_p1, &dockee_p0_norm,
            &dockee_objp->orient);
        ...
        vm_matrix_interpolate (
            &dom, &docker_objp->orient, &omega_in, flFrametime, &nm,
            &omega_out, &vel_limit, &acc_limit, 1);
        ...
        if (mdist > 0.1f) {
            ...
            if (ss1 > 2.0f) ss1 = 2.0f;
        }
        else
            ss1 = 2.0f;

        // if we're docking to a rotating submodel, speed up translation
        if (dockee_rotating_submodel >= 0) ss1 = 2.0f;

        speed_scale *= 1.0f + ss1;

        fdist = dock_move_towards_point (
            docker_objp, &docker_objp->pos, goal_point, speed_scale,
            dockee_objp->phys_info.speed, rdinfo);

        // Note, we're interested in distance from goal, so if we're still
        // turning, bash that into return value.
        fdist += 2.0f * mdist;
    ...
    case DOA_DOCK_STAY: {
        ...
        if (dock_mode == DOA_DOCK) {
            ...
            vm_matrix_interpolate (
                &dom, &docker_objp->orient, &omega_in, flFrametime, &nm,
                &omega_out, &vel_limit, &acc_limit);
            docker_objp->phys_info.rotvel = omega_out;
            docker_objp->orient = nm;

            fdist = dock_move_towards_point (
                docker_objp, &docker_point, &dockee_point, speed_scale,
                dockee_objp->phys_info.speed, rdinfo);
            ...
            // find out the rotation matrix that will get us from the old to
            // the new rotation
            vm_copy_transpose (&temp, &docker_objp->orient);
            vm_matrix_x_matrix (&m_offset, &temp, &dom);

            // now find out the new docker point after being adjusted for the
            // new orientation
            vm_vec_sub (
                &origin_docker_point, &docker_point, &docker_objp->pos);
            vm_vec_rotate (
                &adjusted_docker_point, &origin_docker_point, &m_offset);
            vm_vec_add2 (&adjusted_docker_point, &docker_objp->pos);

            // find the vector that will get us from the old to the new
            // position
    ...
    case DOA_UNDOCK_1: {
        ...
        fdist = dock_move_towards_point (
            docker_objp, &docker_objp->pos,
            &Path_points[aip->path_start + aip->path_length - 2].pos,
    ...
    case DOA_UNDOCK_2: {
        // Undocking.
        // Move to point on dock path nearest to dock station and orient away
        // from big ship.
        int desired_index;
        ...
        fdist = dock_move_towards_point (
            docker_objp, &docker_objp->pos,
            &Path_points[aip->path_start + desired_index].pos, speed_scale,
            0.0f, rdinfo);
    ...
    case DOA_UNDOCK_3: {
        ...
        dist = vm_vec_normalized_dir (
            &away_vec, &docker_objp->pos, &dockee_objp->pos);
        vm_vec_scale_add (
            &goal_point, &dockee_objp->pos, &away_vec, goal_dist);
        if (vm_vec_dist_quick (&goal_point, &dockee_objp->pos) <
            vm_vec_dist_quick (&docker_objp->pos, &dockee_objp->pos))
            fdist = 0.0f;
        else {
            float dot, accel;
            float turn_time =
                Ship_info[Ships[docker_objp->instance].ship_info_index]
                    .srotation_time;
            ai_turn_towards_vector (
                &goal_point, docker_objp, turn_time, nullptr, nullptr, 0.0f,
                0);

            dot = vm_vec_dot (&docker_objp->orient.vec.fvec, &away_vec);
            accel = 0.1f;
            if (dot > accel) accel = dot;
            if (dist > goal_dist / 2) accel *= 1.2f - 0.5f * goal_dist / dist;
...
int num_ships_attacking (int target_objnum) {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (attacking_objp->instance < 0) continue;
        ...
        if (iff_x_attacks_y (
                Ships[attacking_objp->instance].team, target_team)) {
...
// For all objects attacking object #objnum, remove the one that is farthest
// away.        Do this by resuming previous behavior, if any.  If not, set
// target_objnum to -1.
void remove_farthest_attacker (int objnum) {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (!(objp->flags[Object::Object_Flags::Player_ship])) {
            if (objp->instance != -1) {
                ...
                if ((aip2->mode == AIM_CHASE) &&
                    (aip2->target_objnum == objnum)) {
                    if (iff_x_attacks_y (
                            Ships[objp->instance].team,
                            Ships[objp2->instance].team)) {
                        float dist;
    ...
    if (farthest_objp != NULL) {
        ...
        ASSERT (
            (farthest_objp->instance > -1) &&
            (farthest_objp->instance < MAX_SHIPS));
        ...
        if (!maybe_resume_previous_mode (Pl_objp, aip)) {
            // If already ignoring something under player's orders, don't
            // ignore current target.
            if ((aip->ignore_objnum == UNUSED_OBJNUM) ||
                (aip->ai_flags[AI::AI_Flags::Temporary_ignore])) {
                ...
                aip->ignore_expire_timestamp = timestamp (
                    ((myrand () % 10) + 20) *
                    1000); // OK to attack again in 20 to 24 seconds.
            }
            aip->target_objnum = -1;
...
// Maybe limit the number of attackers on attack_objnum.  For now, only limit
// attackers in attacked_objnum is the player input:    attacked_objnum =>
// object index for ship we want to limit attacks on
//
...
void guard_object_was_hit (object* guard_objp, object* hitter_objp) {
    ...
    if (guard_objp == hitter_objp) { return; }

    if (guard_objp->type == OBJ_GHOST || hitter_objp->type == OBJ_GHOST)
        return;

    if (aip->ai_flags[AI::AI_Flags::No_dynamic]) // Not allowed to pursue
                                                 // dynamic goals.  So, why are
                                                 // we guarding?
        return;

    ASSERT (
        (hitter_objp->type == OBJ_SHIP) ||
        (hitter_objp->type == OBJ_ASTEROID) ||
    ...
    if (hitter_objp->type == OBJ_SHIP) {
        ...
        if (is_ignore_object (aip, OBJ_INDEX (hitter_objp))) return;

        // If hitter is on same team as me, don't attack him.
        if (Ships[guard_objp->instance].team ==
            Ships[hitter_objp->instance].team)
            return;

        // limit the number of ships attacking hitter_objnum (for now, only if
        // hitter_objnum is player)
        if (ai_maybe_limit_attackers (hitter_objnum) == 1) { return; }
        ...
        if (awacs_get_level (hitter_objp, &Ships[aip->shipnum], 1) < 1) {
            ...
            if (is_object_stealth_ship (hitter_objp)) {
                if (ai_is_stealth_visible (guard_objp, hitter_objp) !=
                    STEALTH_IN_FRUSTUM) {
                    return;
                }
            }
    ...
    if ((aip->submode == AIS_GUARD_PATROL) ||
        (aip->submode == AIS_GUARD_STATIC)) {
        if (hitter_objp->type == OBJ_SHIP) {
            if (!(Ship_info[Ships[guard_objp->instance].ship_info_index]
                      .is_small_ship ())) {
                return;
            }

            // limit the number of ships attacking hitter_objnum (for now, only
            // if hitter_objnum is player)
            if (ai_maybe_limit_attackers (hitter_objnum) == 1) { return; }
        }
        ...
        if (aip->target_objnum == -1) {
            if (hitter_objp->type == OBJ_SHIP) {
                // limit the number of ships attacking hitter_objnum (for now,
                // only if hitter_objnum is player)
                if (ai_maybe_limit_attackers (hitter_objnum) == 1) { return; }
            }
            ...
            if (num_attacking_cur > 1) {
                ...
                if (num_attacking_new < num_attacking_cur) {
                    if (hitter_objp->type == OBJ_SHIP) {
                        // limit the number of ships attacking hitter_objnum
                        // (for now, only if hitter_objnum is player)
        ...
        if (objp->instance != -1) {
            ...
            if ((aip->mode == AIM_GUARD) ||
                ...
                if (aip->guard_objnum == OBJ_INDEX (hit_objp)) {
                    ...
                    (aip->guard_wingnum != -1) &&
                    (aip->guard_wingnum ==
                     Ai_info[Ships[hit_objp->instance].ai_index].wing)) {
                    guard_object_was_hit (objp, hitter_objp);
                }
            }
        }
...
// return 1 if bomb is found (and targeted by guarding_objp), otherwise return
// 0
int ai_guard_find_nearby_bomb (object* guarding_objp, object* guarded_objp) {
    ...
    for (mo = GET_NEXT (&Missile_obj_list);
        ...
        if (!((wip->wi_flags[Weapon::Info_Flags::Bomb]) ||
              (wip->wi_flags[Weapon::Info_Flags::Fighter_Interceptable]))) {
            continue;
        }

        if (wp->homing_object != guarded_objp) { continue; }

        if (wp->lssm_stage == 3) { continue; }

        dist = vm_vec_dist_quick (&bomb_objp->pos, &guarded_objp->pos);

        if (dist < (MAX_GUARD_DIST + guarded_objp->radius) * 3) {
            dist_to_guarding_obj =
                vm_vec_dist_quick (&bomb_objp->pos, &guarding_objp->pos);
...
/**
 * Scan enemy ships and see if one is near enough to guard object to be
 * pursued.
 */
void ai_guard_find_nearby_ship (object* guarding_objp, object* guarded_objp) {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (enemy_objp->instance < 0) { continue; }
        ...
        if (iff_x_attacks_y (guarding_shipp->team, eshipp->team)) {
            ...
            if (Ship_info[eshipp->ship_info_index].class_type >= 0 &&
                (Ship_types[Ship_info[eshipp->ship_info_index].class_type]
                     .flags[Ship::Type_Info_Flags::AI_guards_attack])) {
                dist =
                    vm_vec_dist_quick (&enemy_objp->pos, &guarded_objp->pos);
                if (dist < (MAX_GUARD_DIST + guarded_objp->radius) * 3) {
                    guard_object_was_hit (guarding_objp, enemy_objp);
                }
                else if (
                    (dist < 3000.0f) &&
                    (Ai_info[eshipp->ai_index].target_objnum ==
                     guarding_aip->guard_objnum)) {
                    guard_object_was_hit (guarding_objp, enemy_objp);
                }
            }
        }
...
// Scan for nearby asteroids.  Favor asteroids which have their collide_objnum
// set to that of the guarded ship.  Also, favor asteroids that are closer to
// the guarding ship, since it looks cooler when a ship blows up an asteroid
// then goes after the pieces that break off.
void ai_guard_find_nearby_asteroid (
    object* guarding_objp, object* guarded_objp) {
    float dist;

    object *closest_asteroid_objp = NULL, *danger_asteroid_objp = NULL,
           *asteroid_objp;
    float dist_to_self, closest_danger_asteroid_dist = 999999.0f,
                        closest_asteroid_dist = 999999.0f;

    for (asteroid_objp = GET_FIRST (&obj_used_list);
         asteroid_objp != END_OF_LIST (&obj_used_list);
        ...
        if (asteroid_objp->type == OBJ_ASTEROID) {
            ...
            if (dist < (MAX_GUARD_DIST + guarded_objp->radius) * 2) {
                dist_to_self = vm_vec_dist_quick (
                    &asteroid_objp->pos, &guarding_objp->pos);
                if (OBJ_INDEX (guarded_objp) ==
                    asteroid_collide_objnum (asteroid_objp)) {
                ...
                if (dist_to_self < closest_asteroid_dist) {
                    ...
                    if (vm_vec_mag_quick (&asteroid_objp->phys_info.vel) <
                        guarding_objp->phys_info.max_vel.xyz.z) {
...
/**
 * Scan potential harmful objects and see if one is near enough to guard object
 * to be pursued.
 */
...
/**
 * Gets closest point on extended axis of cylinder, r_vec, and radius of
 * cylinder
 * @return z of axis_point in cyl_objp reference frame
 */
float get_cylinder_points (
    object* other_objp, object* cyl_objp, vec3d* axis_pt, vec3d* r_vec,
    float* radius) {
    ...
    polymodel* pm = model_get (
        Ship_info[Ships[cyl_objp->instance].ship_info_index].model_num);
    float tempx, tempy;
    ...
    *radius = MAX (tempx, tempy);
    ...
    float extended_cylinder_z =
        vm_vec_dot (&r_sph, &cyl_objp->orient.vec.fvec);

    // get pt on axis of extended cylinder
    vm_vec_scale_add (
        axis_pt, &cyl_objp->pos, &cyl_objp->orient.vec.fvec,
        extended_cylinder_z);
...
void ai_big_guard () {
    ...
    case AIS_GUARD_PATROL: {
        ...
        extended_z = get_cylinder_points (
            Pl_objp, guard_objp, &axis_pt, &r_vec, &radius);
        vm_vec_cross (&theta_vec, &guard_objp->orient.vec.fvec, &r_vec);

        // half ships circle each way
        if (objval > 0.5f) { vm_vec_negate (&theta_vec); }

        float min_guard_dist = radius + Pl_objp->radius + 50.0f;
        float desired_guard_dist =
            min_guard_dist + 0.5f * ((1.0f + objval) * MAX_GUARD_DIST);
        float max_guard_dist =
            min_guard_dist + 1.0f * ((1.0f + objval) * MAX_GUARD_DIST);

        // get z extents
        float min_z, max_z, length;
        polymodel* pm = model_get (
            Ship_info[Ships[guard_objp->instance].ship_info_index].model_num);
        min_z = pm->mins.xyz.z;
        max_z = pm->maxs.xyz.z;
        length = max_z - min_z;
        ...
        if (cur_guard_rad < max_guard_dist) {
            if (cur_guard_rad > min_guard_dist) {
                if (inside) {
                    ...
                    vm_vec_scale_add2 (
                        &goal_pt, &theta_vec, desired_guard_dist);
                }
                else {
                    // move to where I can orbit
                    if (extended_z < min_z) {
                        vm_vec_scale_add (
                            &goal_pt, &guard_objp->pos,
                            &guard_objp->orient.vec.fvec, min_z);
                    }
                    else {
                        vm_vec_scale_add (
                            &goal_pt, &guard_objp->pos,
                            &guard_objp->orient.vec.fvec, max_z);
                    }
                ...
                if (inside) {
                    ...
                    vm_vec_scale_add (
                        &goal_pt, &axis_pt, &r_vec, max_guard_dist);
                }
                else {
            ...
            // make sure we have a forward velocity worth calculating (values
            // too low can produce NaN in goal_pt) - taylor
            if (Pl_objp->phys_info.fspeed > 0.0001f) {
                ...
                vm_vec_scale_add2 (
                    &goal_pt, &guard_objp->orient.vec.fvec, v_z * time);
            }
        }
        else {
            ...
            float delta_z = desired_z - extended_z;
            vm_vec_scale_add (
                &goal_pt, &guard_objp->pos, &guard_objp->orient.vec.fvec,
                delta_z);
        }

        // try not to bump into things along the way
        if ((cur_guard_rad > max_guard_dist) || (extended_z < min_z) ||
            (extended_z > max_z)) {
            if (maybe_avoid_big_ship (
                    Pl_objp, guard_objp, aip, &goal_pt, 5.0f)) {
                if (Pl_objp->phys_info.flags & PF_AFTERBURNER_ON) {
                    afterburners_stop (Pl_objp);
                }
                return;
...
void ai_guard () {
    ...
    if (guard_objp->instance == -1) { return; }

    // Not sure whether this should be impossible, or a reasonable cleanup
    // condition.       For now (3/31/97), it's getting trapped by an Assert, so
    // clean it up.
    ...
    case AIS_GUARD_PATROL:
        // Stay near ship
        dist_to_guardobj = vm_vec_normalized_dir (
            &vec_to_guardobj, &guard_objp->pos, &Pl_objp->pos);
        ...
        if (dist_to_goal_point >
            MAX_GUARD_DIST + 1.5 * (Pl_objp->radius + guard_objp->radius)) {
            if (maybe_avoid_big_ship (
                    Pl_objp, guard_objp, aip, &goal_point, 5.0f)) {
                if (Pl_objp->phys_info.flags & PF_AFTERBURNER_ON) {
                    afterburners_stop (Pl_objp);
                }
                return;
            ...
            ai_turn_towards_vector (
                &goal_point, Pl_objp,
                Ship_info[shipp->ship_info_index].srotation_time, nullptr,
                nullptr, 0.0f, 0, &rvec);
            ...
            if ((aip->ai_flags[AI::AI_Flags::Free_afterburner_use]) &&
                ...
                accelerate_ship (
                    aip, accel_scale * (0.25f + dist_to_goal_point / 700.0f));
            }
        }
        else {
            if (maybe_avoid_big_ship (
                    Pl_objp, guard_objp, aip, &goal_point, 2.0f)) {
                if (Pl_objp->phys_info.flags & PF_AFTERBURNER_ON) {
                    afterburners_stop (Pl_objp);
                }
                return;
            ...
            // If going to guard_vec might cause a collision with guarded
            // object, pick a new guard point.
            if (vm_vec_dot (&v2g, &vec_to_guardobj) > 0.8f) {
                if (dist_to_guardobj < dist_to_goal_point) {
                    ai_set_guard_vec (
                        Pl_objp, guard_objp); // OK to return here.
                    return;
                }
            }

            if (speed > 10.0f) {
                // If goal ship is moving more than a tiny bit, don't orbit
                // it, get near it.
                if (vm_vec_dist_quick (&goal_point, &Pl_objp->pos) > 40.0f) {
                    if (vm_vec_dot (&Pl_objp->orient.vec.fvec, &v2g) < 0.0f) {
                        // Just slow down, don't turn.
                        set_accel_for_target_speed (
                            Pl_objp, guard_objp->phys_info.speed -
                                         dist_to_goal_point / 10.0f);
                    }
                    else {
                        // Goal point is in front.
                        // If close to goal point, don't change direction,
                        // just change speed.
                        if (dist_to_goal_point > Pl_objp->radius + 10.0f) {
                            turn_towards_point (
                                Pl_objp, &goal_point, NULL, 0.0f);
                        }
                    ...
                    if (dot_to_goal_point > 0.8f) {
                        ...
                        set_accel_for_target_speed (
                            Pl_objp, guard_objp->phys_info.speed +
                                         dist_to_goal_point * 0.1f);
                    }
                    else {
                        set_accel_for_target_speed (
                            Pl_objp, guard_objp->phys_info.speed -
                                         dist_to_goal_point * 0.1f - 1.0f);
                    }
                }
                ...
                if (dist_to_goal_point > 15.0f) {
                    turn_towards_point (Pl_objp, &goal_point, NULL, 0.0f);
                    set_accel_for_target_speed (
                        Pl_objp, (dist_to_goal_point - 10.0f) / 2.0f);
                }
                ...
                dist_to_guardobj >
                MAX_GUARD_DIST + Pl_objp->radius + guard_objp->radius) {
                // Orbiting ship, too far away
                float dot = turn_towards_tangent (
                    Pl_objp, &guard_objp->pos,
                ...
                turn_away_from_point (Pl_objp, &guard_objp->pos, 0.0f);
                if ((dist_to_guardobj >
                     guard_objp->radius + Pl_objp->radius + 50.0f) &&
                    (guard_objp->phys_info.speed >
                     Pl_objp->phys_info.speed - 1.0f))
                ...
                float dot = turn_towards_tangent (
                    Pl_objp, &guard_objp->pos,
                    (1.5f + objval / 2.0f) * guard_objp->radius);
                if ((dist_to_guardobj >
                     guard_objp->radius + Pl_objp->radius + 50.0f) &&
                    (guard_objp->phys_info.speed >
                     Pl_objp->phys_info.speed - 1.0f))
                    set_accel_for_target_speed (
                        Pl_objp, (0.5f * (1.0f + dot)) *
                                     (guard_objp->phys_info.speed +
                                      (dist_to_guardobj - guard_objp->radius -
                                       Pl_objp->radius) /
                                          10.0f));
                else
                    accelerate_ship (
                        aip, 0.5f * (1.0f + dot) * (0.3f + objval / 3.0f));
            }
            if (Pl_objp->phys_info.flags & PF_AFTERBURNER_ON) {
                afterburners_stop (Pl_objp);
            }
...
// for a ship that was getting repaired.  The how parameter is useful for
// multiplayer only in that it tells us why the repaired ship is being cleaned
// up.
void ai_do_objects_repairing_stuff (
    ...
    case REPAIR_INFO_END:
        // when only awaiting repair, and the repair is ended, then set support
        // to -1.
        if (aip->ai_flags[AI::AI_Flags::Awaiting_repair]) {
    ...
    case REPAIR_INFO_QUEUE:
        aip->ai_flags.set (AI::AI_Flags::Awaiting_repair);
        if (aip == Player_ai) { hud_support_view_start (); }
        stamp = timestamp (-1);
        break;

    case REPAIR_INFO_ABORT:
    case REPAIR_INFO_KILLED:
        // undock if necessary (we may be just waiting for a repair in which
        // case we aren't docked)
        if ((repair_objp != NULL) && dock_check_find_direct_docked_object (
                                         repair_objp, repaired_objp)) {
            ai_do_objects_undocked_stuff (repair_objp, repaired_objp);
        }

        // 5/4/98 -- MWA -- Need to set support objnum to -1 to let code know
        // this guy who was getting repaired (or queued for repair), isn't
        // really going to be docked with anyone anymore.
        aip->support_ship_objnum = -1;
        aip->ai_flags.remove (AI::AI_Flags::Being_repaired);
        aip->ai_flags.remove (AI::AI_Flags::Awaiting_repair);
    ...
    case REPAIR_INFO_ONWAY:
        // need to set the signature so that clients in multiplayer games rearm
        // correctly
        ASSERT (repair_objp);
    ...
    // support ship getting killed.
    if (repair_objp) {
        Ai_info[Ships[repair_objp->instance].ai_index].warp_out_timestamp =
            stamp;
        ...
        case REPAIR_INFO_BROKEN: break;

        case REPAIR_INFO_END:
        case REPAIR_INFO_ABORT:
        case REPAIR_INFO_KILLED:
            if (how == REPAIR_INFO_ABORT) aip->goal_objnum = -1;
...
void ai_get_dock_goal_indexes (
    object* objp, ai_info* aip, ai_goal* aigp, object* goal_objp,
    int& docker_index, int& dockee_index) {
    // get the indexes
    switch (aip->submode) {
    case AIS_DOCK_1:
    case AIS_DOCK_2:
    case AIS_DOCK_3:
        WARNINGF (LOCATION,"Normally dock indexes should be calculated for only AIS_DOCK_0 and AIS_UNDOCK_0.  Trace out and debug.");
    case AIS_DOCK_0: {
    ...
    case AIS_UNDOCK_3:
    case AIS_UNDOCK_4: {
        WARNINGF (LOCATION,"Normally dock indexes should be calculated for only AIS_DOCK_0 and AIS_UNDOCK_0.  Additionally, dock indexes can't always be determined for AIS_UNDOCK_3 or AIS_UNDOCK_4.  Trace out and debug.");
        docker_index = -1;
        dockee_index = -1;
        break;
    }
...
void ai_cleanup_dock_mode_subjective (object* objp) {
    ...
    if ((aip->mode == AIM_DOCK) && (aip->submode < AIS_UNDOCK_3)) {
        ...
        case AIS_UNDOCK_0:
            model_anim_start_type (
                shipp, TRIGGER_TYPE_DOCKED, docker_index, -1);
            model_anim_start_type (
                goal_shipp, TRIGGER_TYPE_DOCKED, dockee_index, -1);
        case AIS_UNDOCK_1:
            model_anim_start_type (
                shipp, TRIGGER_TYPE_DOCKING_STAGE_3, docker_index, -1);
            model_anim_start_type (
                goal_shipp, TRIGGER_TYPE_DOCKING_STAGE_3, dockee_index, -1);
        case AIS_UNDOCK_2:
            model_anim_start_type (
                shipp, TRIGGER_TYPE_DOCKING_STAGE_2, docker_index, -1);
            model_anim_start_type (
                goal_shipp, TRIGGER_TYPE_DOCKING_STAGE_2, dockee_index, -1);
            break;
...
// It ensures that any ship docking or undocking with it will finish
// gracefully.)
void ai_cleanup_dock_mode_objective (object* objp) {
...
void ai_cleanup_rearm_mode (object* objp) {
    ...
    if (aip->ai_flags[AI::AI_Flags::Repairing]) {
        ...
        ai_do_objects_repairing_stuff (
            &Objects[aip->goal_objnum], objp, REPAIR_INFO_KILLED);
    }
    else if (aip->ai_flags[AI::AI_Flags::Being_repaired]) {
        // MWA/Goober5000 -- note that we have to use support object here
        // instead of goal_objnum.
        ASSERT (aip->support_ship_objnum != -1);
        ai_do_objects_repairing_stuff (
            objp, &Objects[aip->support_ship_objnum], REPAIR_INFO_ABORT);
    }
    else if (aip->ai_flags[AI::AI_Flags::Awaiting_repair]) {
        // MWA/Goober5000 -- note that we have to use support object here
        // instead of goal_objnum. MWA -- 3/38/98  Check to see if this guy is
        // queued for a support ship, or there is already one in the mission
        if (mission_is_repair_scheduled (objp)) {
            mission_remove_scheduled_repair (
                objp); // this function will notify multiplayer clients.
        }
        else {
            if (aip->support_ship_objnum != -1)
                ai_do_objects_repairing_stuff (
                    objp, &Objects[aip->support_ship_objnum],
                    REPAIR_INFO_ABORT);
            else
...
void ai_stay_near () {
    ...
    if ((goal_objnum < 0) || (Objects[goal_objnum].type != OBJ_SHIP) ||
        (Objects[goal_objnum].signature != aip->goal_signature)) {
        aip->mode = AIM_NONE;
    }
    else {
        ...
        // Scale the random vector by an amount proportional to the distance
        // from Pl_objp to the true goal.
...
int maybe_dock_obstructed (
    object* cur_objp, object* goal_objp, int big_only_flag) {
    ...
    Ai_info[Ships[goal_objp->instance].ai_index].ai_flags.remove (
        AI::AI_Flags::Repair_obstructed);

    if (goal_objp != Player_obj) return -1;

    curpos = &cur_objp->pos;
    radius = cur_objp->radius;
    goalpos = &Path_points[aip->path_cur].pos;
    collide_objnum = pp_collide_any (
        curpos, goalpos, radius, cur_objp, goal_objp, big_only_flag);

    if (collide_objnum != -1)
        Ai_info[Ships[goal_objp->instance].ai_index].ai_flags.set (
            AI::AI_Flags::Repair_obstructed);
...
// Docking behavior.
// Approach a ship, follow path to docking platform, approach platform; after
// awhile,      undock.
void ai_dock () {
    ship* shipp = &Ships[Pl_objp->instance];
    ai_info* aip = &Ai_info[shipp->ai_index];
    ...
    // Make sure the object we're supposed to dock with or undock from still
    // exists.
    ...
    // we need to keep the dock indexes stored in the submode because the goal
    // may become invalid at any point (when we first dock or first undock,
    // we'll calculate and overwrite these for the first time)
    int docker_index = aip->submode_parm0;
    int dockee_index = aip->submode_parm1;
    ...
    // For docking submodes (ie, not undocking), follow path.  Once at second
    // last point on path (point just before point on dock platform), orient
    // into position.
    //
    // For undocking, first mode pushes docked ship straight back from docking
    // point second mode turns ship and moves to point on docking radius
    switch (aip->submode) {
    // This mode means to find the path to the docking point.
    case AIS_DOCK_0: {
        // save the dock indexes we're currently using
        ai_get_dock_goal_indexes (
            Pl_objp, aip, aigp, goal_objp, docker_index, dockee_index);
        ...
        if (aip->path_length < 4) {
            ...
            WARNINGF (LOCATION,"Ship class %s has only %i points on dock path \"%s\".  Recommended minimum number of points is 4.  Docking along that path will look strange.  You may wish to edit the model.",goal_ship_class_name, aip->path_length, goal_dock_path_name);
        }

        aip->submode = AIS_DOCK_1;
        aip->submode_start_time = Missiontime;
        model_anim_start_type (
            shipp, TRIGGER_TYPE_DOCKING_STAGE_1, docker_index, 1);
        model_anim_start_type (
            goal_shipp, TRIGGER_TYPE_DOCKING_STAGE_1, dockee_index, 1);
    ...
    case AIS_DOCK_1: {
        ...
        if ((r = maybe_dock_obstructed (Pl_objp, goal_objp, 1)) != -1) {
            ...
            if ((r1 = maybe_avoid_big_ship (
                ...
                WARNINGF (LOCATION, "Support ship %s avoiding large ship %s",Ships[Pl_objp->instance].ship_name,Ships[Objects[r1].instance].ship_name);
                break;
            }
        } // else {
        {
            ...
            if (aip->path_cur - aip->path_start >=
                aip->path_length -
                    1) { // If got this far, advance no matter what.
                ...
                if (Pl_objp->phys_info.speed >
                    goal_objp->phys_info.speed + 1.5f) {
                    set_accel_for_target_speed (
                        Pl_objp, goal_objp->phys_info.speed);
                }
                else {
                    aip->submode = AIS_DOCK_2;
                    aip->submode_start_time = Missiontime;
                    model_anim_start_type (
                        shipp, TRIGGER_TYPE_DOCKING_STAGE_2, docker_index, 1);
                    model_anim_start_type (
                        goal_shipp, TRIGGER_TYPE_DOCKING_STAGE_2, dockee_index,
                        1);
                }
            }
        }
    ...
    // This mode means to drag oneself right to the second last point on the
    // path.    Path code allows it to overshoot.
    case AIS_DOCK_2: {
        float dist;
        int r;

        if ((r = maybe_dock_obstructed (Pl_objp, goal_objp, 0)) != -1) {
            WARNINGF (LOCATION, "Dock 2: Obstructed by %s",Ships[Objects[r].instance].ship_name);
            accelerate_ship (aip, 0.0f);
            ...
            dist = dock_orient_and_approach (
                Pl_objp, docker_index, goal_objp, dockee_index, DOA_APPROACH);
    ...
    case AIS_DOCK_3: {
        ...
        if ((r = maybe_dock_obstructed (Pl_objp, goal_objp, 0)) != -1) {
            WARNINGF (LOCATION, "Dock 1: Obstructed by %s",Ships[Objects[r].instance].ship_name);
            accelerate_ship (aip, 0.0f);
            ...
            float dist = dock_orient_and_approach (
                Pl_objp, docker_index, goal_objp, dockee_index, DOA_DOCK,
                &rdinfo);
            ASSERT (dist != UNINITIALIZED_VALUE);
            ...
            if (rdinfo.submodel >= 0) {
                tolerance +=
                    4 * flFrametime * (rdinfo.submodel_r * rdinfo.submodel_w);
            }

            if (dist < tolerance) {
                // - Removed by MK on 11/7/97, causes errors for ships docked
                // at mission start: maybe_recreate_path(Pl_objp, aip, 1);
                dist = dock_orient_and_approach (
                    Pl_objp, docker_index, goal_objp, dockee_index, DOA_DOCK);
                ASSERT (dist != UNINITIALIZED_VALUE);
                ...
                ai_do_objects_docked_stuff (
                    Pl_objp, docker_index, goal_objp, dockee_index);

                if (aip->submode == AIS_DOCK_3) {
                    // Play a ship docking attach sound
                    snd_play_3d (
                        gamesnd_get_game_sound (GameSounds::DOCK_ATTACH),
                        &Pl_objp->pos, &View_position);
                ...
                if (aip->ai_flags[AI::AI_Flags::Repairing]) {
                    aip->submode =
                        AIS_DOCK_4; // Special rearming only dock mode.
                    aip->submode_start_time = Missiontime;
                }
                else {
                    aip->submode = AIS_DOCK_4A;
    ...
    // Yes, we just sit here.  We wait for further orders.  No, it's not a
    // bug.
    case AIS_DOCK_4A: {
        if (aigp == NULL) { // Can happen for initially docked ships.
            ...
            mission_log_add_entry (
                LOG_SHIP_DOCKED, shipp->ship_name, goal_shipp->ship_name);

            if (aigp->ai_mode == AI_GOAL_DOCK) {
                ai_mission_goal_complete (
                    aip); // Note, this calls ai_do_default_behavior().
            }
        }
    ...
    case AIS_DOCK_4: {
        ...
        // The ship that is performing the rearm enters this mode after it
        // docks.
        ...
        if (goal_objp->phys_info.speed > MAX_REPAIR_SPEED) {
            ...
            mission_log_add_entry (
                LOG_SHIP_UNDOCKED, shipp->ship_name, goal_shipp->ship_name);
        }
        ...
        else if (dist > 5.0f) // Oops, too far away!
        {
            if (goal_aip->ai_flags[AI::AI_Flags::Being_repaired]) {
                ai_do_objects_repairing_stuff (
                    goal_objp, Pl_objp, REPAIR_INFO_BROKEN);
                // Goober5000 - add missionlog for support ships, per Mantis
                // #2999
                mission_log_add_entry (
                    LOG_SHIP_UNDOCKED, shipp->ship_name,
                    goal_shipp->ship_name);
            }

            if (dist > Pl_objp->radius * 2 + goal_objp->radius * 2) {
                // Got real far away from goal, so move back a couple modes
                // and try again.
                aip->submode = AIS_DOCK_2;
                aip->submode_start_time = Missiontime;
                model_anim_start_type (
                ...
                model_anim_start_type (
                    shipp, TRIGGER_TYPE_DOCKING_STAGE_3, docker_index, -1);
                model_anim_start_type (
                    goal_shipp, TRIGGER_TYPE_DOCKING_STAGE_3, dockee_index,
                    -1);
            }
        }
        else {
            if (goal_aip->ai_flags[AI::AI_Flags::Awaiting_repair]) {
                ai_do_objects_repairing_stuff (
                    goal_objp, Pl_objp, REPAIR_INFO_BEGIN);
                // Goober5000 - add missionlog for support ships, per Mantis
                // #2999
                mission_log_add_entry (
                    LOG_SHIP_DOCKED, shipp->ship_name, goal_shipp->ship_name);
    ...
    case AIS_UNDOCK_0: {
        ...
        // If this is the first frame for this submode, play the animation and
        // set the timestamp
        if (aip->mode_time < 0) {
            // save the dock indexes we're currently using
            ai_get_dock_goal_indexes (
            ...
            int time1 = model_anim_get_time_type (
                shipp, TRIGGER_TYPE_DOCKED, docker_index);
            int time2 = model_anim_get_time_type (
                goal_shipp, TRIGGER_TYPE_DOCKED, dockee_index);
            aip->mode_time = MAX (time1, time2);
        }

        // if not enough time has passed, just wait
        if (!timestamp_elapsed (aip->mode_time)) break;
        ...
        snd_play_3d (
            gamesnd_get_game_sound (GameSounds::DOCK_DETACH), &Pl_objp->pos,
            &View_position);
    ...
    case AIS_UNDOCK_1: {
        // Using thrusters, exit from dock station to nearest next dock path
        // point.
        float dist;
        rotating_dockpoint_info rdinfo;

        // Waiting for one second to elapse to let detach sound effect play
        // out.
        if (Missiontime - aip->submode_start_time < REARM_BREAKOFF_DELAY)
            break;

        // play the depart sound, but only once, since this mode is called
        // multiple times per frame
        if (!(aigp->flags[AI::Goal_Flags::Depart_sound_played])) {
            snd_play_3d (
                gamesnd_get_game_sound (GameSounds::DOCK_DEPART),
                &Pl_objp->pos, &View_position);
            aigp->flags.set (AI::Goal_Flags::Depart_sound_played);
        }

        dist = dock_orient_and_approach (
            Pl_objp, docker_index, goal_objp, dockee_index, DOA_UNDOCK_1,
            &rdinfo);
        ASSERT (dist != UNINITIALIZED_VALUE);
        ...
        if (rdinfo.submodel >= 0)
            dist_to_dock =
                vm_vec_dist_quick (&Pl_objp->pos, &rdinfo.dockee_point);
        else
            dist_to_dock = vm_vec_dist_quick (&Pl_objp->pos, &goal_objp->pos);

        // Move to within 0.1 units of second last point on path before
        // orienting, or just plain far away from docked-to ship.       This allows
        // undock to complete if first ship flies away.
    ...
    case AIS_UNDOCK_2: {
        ...
        dist = dock_orient_and_approach (
            Pl_objp, docker_index, goal_objp, dockee_index, DOA_UNDOCK_2);
        ASSERT (dist != UNINITIALIZED_VALUE);

        // If at goal point, or quite far away from dock object
        // NOTE: the speed check has an etra 5 thousandths added on to account
        // for some floating point error
        if ((dist < 2.0f) ||
            (vm_vec_dist_quick (&Pl_objp->pos, &goal_objp->pos) >
             (Pl_objp->radius + goal_objp->radius) * 2) ||
            ((goal_objp->phys_info.speed + 0.005f) > MAX_UNDOCK_ABORT_SPEED)) {
            // reset the dock flags.  If rearm/repair, reset rearm repair flags
            // for those ships as well.
            if (sip->flags[Ship::Info_Flags::Support]) {
                ai_do_objects_repairing_stuff (
                    &Objects[aip->support_ship_objnum], Pl_objp,
                    REPAIR_INFO_END);
            }
            ...
            // Goober5000 - deliberately contravening Volition's above comment
            // - add missionlog for support ships, per Mantis #2999
            mission_log_add_entry (
                LOG_SHIP_UNDOCKED, shipp->ship_name, goal_shipp->ship_name);
        }
        break;
    ...
    case AIS_UNDOCK_3: {
        if (goal_objp == NULL) {
            // this might happen when a goal is cancelled before docking has
            // finished
            ...
            float dist = dock_orient_and_approach (
                Pl_objp, docker_index, goal_objp, dockee_index, DOA_UNDOCK_3);
            ASSERT (dist != UNINITIALIZED_VALUE);
            ...
            if (dist < Pl_objp->radius / 2 + 5.0f) {
                ...
                model_anim_start_type (
                    shipp, TRIGGER_TYPE_DOCKING_STAGE_1, docker_index, -1);
                model_anim_start_type (
                    goal_shipp, TRIGGER_TYPE_DOCKING_STAGE_1, dockee_index,
                    -1);
            }

            // possible that this flag hasn't been cleared yet.  When aborting
            // a rearm, this submode might be entered directly.
            if ((sip->flags[Ship::Info_Flags::Support]) &&
                (aip->ai_flags[AI::AI_Flags::Repairing])) {
                ai_do_objects_repairing_stuff (
                    goal_objp, Pl_objp, REPAIR_INFO_ABORT);
            }
        }
...
void ai_debug_render_stuff () {
    ...
    for (i = 0; i < Num_AI_debug_render_stuff; i++) {
        ...
        ship_get_global_turret_info (
            &Objects[parent_objnum], tp, &gpos, &gvec);
...
void process_subobjects (int objnum) {
    ...
    for (pss = GET_FIRST (&shipp->subsys_list);
        ...
        // Don't process destroyed objects (but allow subobjects with hitpoints
        // disabled -nuke) (but also process subobjects that are allowed to
        // rotate)
        if (pss->max_hits > 0 && pss->current_hits <= 0.0f &&
            !(psub->flags[Model::Subsystem_Flags::Destroyed_rotation]))
            continue;
        ...
        case SUBSYSTEM_TURRET:
            ...
            if ((pss->turret_animation_position == MA_POS_READY) &&
                ...
                if (model_anim_start_type (
                        shipp, TRIGGER_TYPE_TURRET_FIRING,
                        pss->system_info->subobj_num, -1)) {
            ...
            if (psub->turret_num_firing_points > 0) {
                    ...
                    for (unsigned int i = 0; i < MAX_OBJECTS; i++) {
                        ...
                        case OBJ_WEAPON:
                            if (obj_team (objp) != shipp->team)
                                enemies_present = 1;
                            break;
                        case OBJ_ASTEROID: enemies_present = 1; break;
                        }

                        if (enemies_present == 1) break;
                    }
                    // Reset objp
                    objp = &Objects[objnum];
                }
                ...
                WARNINGF (LOCATION,"Turret %s on ship %s has no firing points assigned to it.\nThis needs to be fixed in the model.",psub->name, shipp->ship_name);
            }
            break;
    ...
    if (ship_get_subsystem_strength (shipp, SUBSYSTEM_ENGINE) == 0.0f) {
        // Karajorma - if Player_use_ai is ever fixed to work on multiplayer it
        // should be checked that any player ships aren't under AI control here
        if ((!(objp->flags[Object::Object_Flags::Player_ship])) &&
            (sip->is_fighter_bomber ()) &&
            !(shipp->flags[Ship::Ship_Flags::Dying])) {
            // Goober5000 - don't do anything if docked
            if (!object_is_docked (objp)) {
                // AL: Only attack forever if not trying to depart to a docking
                // bay.  Need to have this in, since
                // a ship may get repaired... and it should still try to
                // depart.  Since docking bay departures are not handled as
                // goals, we don't want to leave the AIM_BAY_DEPART mode.
                if (aip->mode != AIM_BAY_DEPART) {
                    ai_attack_object (
                        objp, NULL, NULL); // Regardless of current mode,
                                           // enter attack mode.
                    aip->submode =
                        SM_ATTACK_FOREVER; // Never leave attack submode,
                                           // don't avoid, evade, etc.
                    aip->submode_start_time = Missiontime;
                }
            }
        }
...
int get_wing_index (object* objp, int wingnum) {
    ...
    for (i = wingp->current_count - 1; i >= 0; i--)
        if (objp->instance == wingp->ship_index[i]) break;
...
object* get_wing_leader (int wingnum) {
    ...
    int n = 0;
    while (ship_get_subsystem_strength (&Ships[ship_num], SUBSYSTEM_ENGINE) ==
           0.0f) {
        n++;

        if (n >= wingp->current_count) { break; }
...
// next constant is higher that MAX_SHIPS_IN_WINGS to deal with forming on
// player's wing
#define MAX_FORMATION_ROWS 4

// Given a position in a wing, return the desired location of the ship
...
void get_wing_delta (vec3d* _delta_vec, int wing_index) {
    ...
    int bank =
        wing_index / (MAX_FORMATION_ROWS * (MAX_FORMATION_ROWS + 1) / 2);
    ...
    _delta_vec->xyz.x = ((float)column - (float)row / 2.0f) *
                        DEFAULT_WING_X_DELTA / DEFAULT_WING_MAG;
    _delta_vec->xyz.y = ((float)row + (float)bank * 2.25f) *
                        DEFAULT_WING_Y_DELTA / DEFAULT_WING_MAG;
    _delta_vec->xyz.z = -((float)row + 0.5f * (float)bank) *
                        DEFAULT_WING_Z_DELTA / DEFAULT_WING_MAG;
}
...
float gwlr_1 (object* objp, ai_info* aip) {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (Ai_info[Ships[o->instance].ai_index].wing == wingnum)
            if (o->radius > max_radius) max_radius = o->radius;
    }
...
static float gwlr_object_1 (object* objp) {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
         so = GET_NEXT (so)) {
        o = &Objects[so->objnum];
        if (Ai_info[Ships[o->instance].ai_index].goal_objnum ==
            OBJ_INDEX (objp))
            if (o->radius > max_radius) max_radius = o->radius;
    }
...
/**
 * For the wing that *objp is part of, return the largest ship radius in that
 * wing.
 */
float get_wing_largest_radius (object* objp, int formation_object_flag) {
    ...
    if (formation_object_flag) { return gwlr_object_1 (objp); }
    else {
...
/**
 * Given a wing leader and a position in the wing formation, return the desired
 * absolute location to fly to.
 * @return result in *result_pos.
 */
void get_absolute_wing_pos (
    vec3d* result_pos, object* leader_objp, int wing_index,
    ...
    get_wing_delta (
        &wing_delta,
        wing_index); // Desired location in leader's reference frame

    wing_spread_size = MAX (
        50.0f,
        3.0f * get_wing_largest_radius (leader_objp, formation_object_flag) +
            15.0f);
    ...
    vm_vec_scale (
        &wing_delta,
        wing_spread_size * (1.0f + leader_objp->phys_info.speed / 70.0f));

    vm_vec_unrotate (
        &rotated_wing_delta, &wing_delta,
        &leader_objp->orient); // Rotate into leader's reference.

    vm_vec_add (
        result_pos, &leader_objp->pos,
        &rotated_wing_delta); // goal_point is absolute 3-space point.
}

// autopilot variant.. removes some scaling crap
void get_absolute_wing_pos_autopilot (
    vec3d* result_pos, object* leader_objp, int wing_index,
    int formation_object_flag) {
    vec3d wing_delta, rotated_wing_delta;
    float wing_spread_size;
...
void render_wing_phantoms_all () {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        ai_info* aip;
        int wingnum;
        int wing_index; // Index in wing struct, defines 3-space location in
                        // wing.
        ...
        if (wingnum == -1) continue;

        wing_index = get_wing_index (objp, wingnum);

        // If this ship is NOT the leader, abort.
        if (wing_index != 0) continue;
...
void ai_fly_in_formation (int wingnum) {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (Ai_info[shipp->ai_index].wing == wingnum) {
            Ai_info[shipp->ai_index].ai_flags.set (
                AI::AI_Flags::Formation_wing);
            Ai_info[shipp->ai_index].ai_flags.remove (
                AI::AI_Flags::Formation_object);
        }
    }
}
...
int formation_is_leader_chaotic (object* objp) {
    ...
    if (Framecount != Chaos_frame) {
        ...
        fdot =
            5.0f *
            (1.0f - vm_vec_dot (
                        &objp->orient.vec.fvec, &objp->last_orient.vec.fvec)) *
            flFrametime;
        udot =
            8.0f *
            (1.0f - vm_vec_dot (
                        &objp->orient.vec.uvec, &objp->last_orient.vec.uvec)) *
            flFrametime;
...
void ai_most_massive_object_of_its_wing_of_all_docked_objects_helper (
    object* objp, dock_function_info* infop) {
    // check that I am a ship
    if (objp->type == OBJ_SHIP) {
        // check that wings match
        if (Ai_info[Ships[objp->instance].ai_index].wing ==
            infop->parameter_variables.int_value) {
            // if this guy has a higher mass, he is now the most massive object
            if (objp->phys_info.mass >
                infop->maintained_variables.objp_value->phys_info.mass) {
                infop->maintained_variables.objp_value = objp;
            }
            // if masses are equal, then check if this guy has a higher
            // signature - if so, he is now the most massive object
            else if (
                objp->phys_info.mass ==
                infop->maintained_variables.objp_value->phys_info.mass) {
                if (objp->signature >
                    infop->maintained_variables.objp_value->signature) {
                    infop->maintained_variables.objp_value = objp;
                }
            }
        }
...
// If the leader of the wing is doing something stupid, like fighting a
// battle,      then the poor sap wingmates will be in for a "world of hurt"
...
int ai_formation () {
    ...
    int wingnum;
    int wing_index; // Index in wing struct, defines 3-space location in wing.
    vec3d goal_point, future_goal_point_5, future_goal_point_2,
        future_goal_point_x, future_goal_point_1000x, vec_to_goal, dir_to_goal;
    ...
    ASSERT (!(
        aip->ai_flags[AI::AI_Flags::Formation_wing] &&
        aip->ai_flags[AI::AI_Flags::Formation_object])); // Make sure not both
                                                         // types of formation
                                                         // flying in effect.
    ...
    if (aip->ai_flags[AI::AI_Flags::Formation_object]) {
        ...
        wing_index =
            ai_formation_object_get_slotnum (aip->goal_objnum, Pl_objp);
    ...
    // if Pl_objp is docked with a ship in his own wing, only the most massive
    // one in the whole assembly actually flies in formation Goober5000 - this
    // is really stupid code
    if (object_is_docked (Pl_objp)) {
        ...
        if (dfi.maintained_variables.objp_value != Pl_objp) { return 0; }
    }
    ...
    if (wing_index == -1) { return 1; }

    // skip ourselves, if we happen to be the leader
    if (leader_objp == Pl_objp) { return 1; }

    if (aip->mode == AIM_WAYPOINTS) {
        if (The_mission.ai_profile
                ->flags[AI::Profile_Flags::Fix_ai_path_order_bug]) {
            // skip if wing leader has no waypoint order or a different
            // waypoint list
    ...
    get_absolute_wing_pos (
        &goal_point, leader_objp, wing_index,
        aip->ai_flags[AI::AI_Flags::Formation_object]);
    ...
    vm_vec_scale_add (
        &future_goal_point_x, &goal_point, &leader_objp->orient.vec.fvec,
        10.0f); // used when very close to destination
    vm_vec_scale_add (
        &future_goal_point_1000x, &goal_point, &leader_objp->orient.vec.fvec,
        1000.0f); // used when very close to destination

    // Now, get information telling this object how to turn and accelerate to
    // get to its       desired location.
    ...
    float dist_to_goal_2 =
        vm_vec_dist_quick (&Pl_objp->pos, &future_goal_point_2);

    int chaotic_leader = 0;

    chaotic_leader = formation_is_leader_chaotic (
        leader_objp); // Set to 1 if leader is player and flying
                      // erratically.  Causes ships to not aggressively pursue
                      // formation location.
    ...
    if (ab_allowed) {
        if ((Pl_objp->phys_info.afterburner_max_vel.xyz.z - leader_speed) >
            (0.2f * Pl_objp->phys_info.afterburner_max_vel.xyz.z)) {
            switch_value = 0.2f * Pl_objp->phys_info.afterburner_max_vel.xyz.z;
        }
        else {
            switch_value =
                Pl_objp->phys_info.afterburner_max_vel.xyz.z - leader_speed;
        }
    }

    if (dist_to_goal > 500.0f) {
        turn_towards_point (Pl_objp, &goal_point, NULL, 0.0f);
        if (ab_allowed && !(Pl_objp->phys_info.flags & PF_AFTERBURNER_ON) &&
            (dot_to_goal > 0.2f)) {
            float percent_left = 100.0f * shipp->afterburner_fuel /
                                 sip->afterburner_fuel_capacity;
            if (percent_left > 30.0f) {
        ...
        if (dot_to_goal > -0.5f) {
            ...
            if (range_speed > 0.0f) {
                set_accel_for_target_speed (
                    Pl_objp, leader_speed + range_speed *
                                                (dist_to_goal + 100.0f) /
                                                500.0f);
                if (Pl_objp->phys_info.flags & PF_AFTERBURNER_ON)
                    afterburners_stop (Pl_objp);
                ...
                if (ab_allowed) {
                    if (!(Pl_objp->phys_info.flags & PF_AFTERBURNER_ON) &&
                        ...
                        float percent_left = 100.0f * shipp->afterburner_fuel /
                                             sip->afterburner_fuel_capacity;
                        if ((percent_left > 30.0f) &&
                            (Pl_objp->phys_info.speed < leader_speed)) {
                            afterburners_start (Pl_objp);
                            aip->afterburner_stop_time =
                                Missiontime + 5 * F1_0;
                        }
            ...
            if (leader_speed > 10.0f)
                set_accel_for_target_speed (
                    Pl_objp, leader_speed * (1.0f + dot_to_goal));
            else
        ...
        if (chaotic_leader) {
            ...
            if (range_speed > 0.0f) {
                ...
                if (ab_allowed) {
                    if (!(Pl_objp->phys_info.flags & PF_AFTERBURNER_ON) &&
                        ...
                        if ((Pl_objp->phys_info.speed >
                             leader_speed + 0.95 * switch_value) ||
                            (dot_to_goal < 0.4f))
                            afterburners_stop (Pl_objp);
                    }
                }
            ...
            if (leader_speed < 5.0f)
                if (delta_speed < 5.0f) delta_speed = 5.0f;
            ...
            set_accel_for_target_speed (
                Pl_objp, scale * (leader_speed + delta_speed));
        }
        else {
            if (leader_speed < 5.0f) {
                // Leader very slow.  If not close to goal point, get very
                // close.  Note, keep trying to get close unless        moving very
                // slowly, else momentum can carry far away from goal.

                if ((dist_to_goal > 10.0f) ||
                    ((Pl_objp->phys_info.speed > leader_speed + 2.5f) &&
                     (dot_to_goal > 0.5f))) {
                    turn_towards_point (Pl_objp, &goal_point, NULL, 0.0f);
                    set_accel_for_target_speed (
                        Pl_objp, leader_speed + dist_to_goal / 10.0f);
                }
                else {
                    if (Pl_objp->phys_info.speed < 0.5f) {
                        turn_towards_point (
                            Pl_objp, &future_goal_point_1000x, NULL, 0.0f);
                    }
                ...
                if (dist_to_goal > 25.0f) {
                    ...
                    set_accel_for_target_speed (
                        Pl_objp, leader_speed + dist_to_goal / 5.0f * dv);
                }
                else {
                    set_accel_for_target_speed (
                        Pl_objp, leader_speed + 1.5f * dot_to_goal - 1.0f);
                }
                ...
                if (range_speed > 0.0f) {
                    ...
                    if (ab_allowed) {
                        if (!(Pl_objp->phys_info.flags & PF_AFTERBURNER_ON) &&
                            (dot_to_goal > 0.9f)) {
                            float percent_left =
                                100.0f * shipp->afterburner_fuel /
                                sip->afterburner_fuel_capacity;
                            if ((percent_left > 30.0f) &&
                                (Pl_objp->phys_info.speed <
                                 leader_speed - 0.75 * switch_value)) {
                                afterburners_start (Pl_objp);
                                aip->afterburner_stop_time =
                                    Missiontime + 3 * F1_0;
                            }
                ...
                if (Pl_objp->phys_info.speed < 0.1f)
                    ...
                    turn_towards_point (
                        Pl_objp, &future_goal_point_x, NULL, 0.0f);
    ...
    float up_dot =
        vm_vec_dot (&leader_objp->orient.vec.uvec, &Pl_objp->orient.vec.uvec);
    if (up_dot < 0.996f) {
        vec3d w_out;
        matrix new_orient;
        vec3d angular_accel;
        ...
        vm_vec_copy_scale (
            &angular_accel, &Pl_objp->phys_info.max_rotvel, 0.2f);
        vm_matrix_interpolate (
            &leader_objp->orient, &Pl_objp->orient, &Pl_objp->phys_info.rotvel,
            flFrametime, &new_orient, &w_out, &Pl_objp->phys_info.max_rotvel,
            &angular_accel, 1);
...
void ai_do_repair_frame (object* objp, ai_info* aip, float frametime) {
    ...
    if (aip->ai_flags
            [AI::AI_Flags::Being_repaired, AI::AI_Flags::Awaiting_repair]) {
        int support_objnum;
        ...
        if (support_objnum == -1) return;

        // Curious -- object numbers match, but signatures do not.
        // Must mean original repair ship died and was replaced by current
        // ship.
        ASSERT (
            Objects[support_objnum].signature == aip->support_ship_signature);

        repair_aip =
            &Ai_info[Ships[Objects[support_objnum].instance].ai_index];

        if (aip->ai_flags[AI::AI_Flags::Being_repaired]) {
            // Wait awhile into the mode to synchronize with sound effect.
            if (Missiontime - repair_aip->submode_start_time >
                REARM_SOUND_DELAY) {
                int repaired;

                if (!rearm_eta_found && objp == Player_obj &&
                    Player_rearm_eta <= 0) {
                    rearm_eta_found = true;
                    Player_rearm_eta = ship_calculate_rearm_duration (objp);
                }

                repaired = ship_do_rearm_frame (
                    objp, frametime); // hook to do missile rearming
        ...
        // AL 11-24-97: If this is the player ship, ensure the repair sound
        // isn't playing.  We need to
        // do this check, since this is a looping sound, and may
        // continue on if rearm/repair finishes abnormally once
        // sound begins looping.
        if (objp == Player_obj) {
            Player_rearm_eta = 0;
...
// Shell around dock_orient_and_approach to detect whether dock process should
// be aborted.  Goober5000 - The child should always keep up with the parent.
void call_doa (object* child, object* parent) {
    ...
    dock_orient_and_approach (
        child, child_index, parent, parent_index, DOA_DOCK_STAY);
}
...
void ai_maybe_launch_cmeasure (object* objp, ai_info* aip) {
    ...
    if (!shipp->cmeasure_count) return;

    if (!timestamp_elapsed (shipp->cmeasure_fire_stamp)) return;

    // If not on player's team and Skill_level + ai_class is low, never fire a
    // countermeasure.  The ship is too dumb.
    ...
    if ((aip->nearest_locked_object != -1) &&
        ...
        if ((dist = vm_vec_dist_quick (&objp->pos, &weapon_objp->pos)) <
            ...
            // For ships on player's team, have constant, average chance to
            // fire.    For enemies, increasing chance with higher skill level.
            if (shipp->team == Player_ship->team)
                fire_chance = The_mission.ai_profile
                                  ->cmeasure_fire_chance[NUM_SKILL_LEVELS / 2];
            else
                fire_chance = aip->ai_cmeasure_fire_chance;

            // Decrease chance to fire at lower ai class (SUSHI: Only if
            // autoscale is on)
            if (aip->ai_class_autoscale)
            ...
            if (fire_chance < r) {
                shipp->cmeasure_fire_stamp = timestamp (
                    CMEASURE_WAIT +
                    (int)(fire_chance * 2000)); // Wait 1/2 second
                                                //(CMEASURE_WAIT) + additional
                                                // delay to decrease chance of
                                                // firing very soon.
                return;
            }
...
static void ai_preprocess_ignore_objnum (ai_info* aip) {
    ...
    if (is_ignore_object (aip, aip->goal_objnum)) {
        ...
        // AL 12-11-97: If in STRAFE mode, we need to ensure that target_objnum
        // is also
        // set to -1
        if (aip->mode == AIM_STRAFE) aip->target_objnum = -1;
    }

    if (is_ignore_object (aip, aip->target_objnum)) aip->target_objnum = -1;
}
...
void ai_chase_circle (object* objp) {
    ...
    ASSERT (
        vm_vec_mag (&aip->goal_point) >=
        0.0f); // Supposedly detects bogus vector
    ...
    if (aip->ignore_objnum == UNUSED_OBJNUM) {
        ...
        if (dist_to_goal > 2 * CHASE_CIRCLE_DIST) {
            ...
            vm_vec_scale_add (
                &aip->goal_point, &objp->pos, &vec_to_goal, CHASE_CIRCLE_DIST);
        }
        ...
        dist =
            vm_vec_normalized_dir (&tvec1, &Pl_objp->pos, &ignore_objp->pos);

        if (dist < ignore_objp->radius * 2 + 1500.0f) {
            vm_vec_scale_add (
                &goal_point, &Pl_objp->pos, &tvec1,
                ignore_objp->radius * 2 + 1400.0f);
            if (dist < ignore_objp->radius * 2 + 1300.0f)
                target_speed =
                    sip->max_speed *
                    (1.25f - dist / (ignore_objp->radius * 2 + 1500.0f));
        }
    }
...
#define SHIELD_BALANCE_RATE \
    0.2f // 0.1f -> takes 10 seconds to equalize shield.
...
void ai_manage_shield (object* objp, ai_info* aip) {
    ...
    if (timestamp_elapsed (aip->shield_manage_timestamp)) {
        ...
        if (iff_x_attacks_y (Player_ship->team, Ships[objp->instance].team)) {
            ...
            delay = The_mission.ai_profile
                        ->shield_manage_delay[NUM_SKILL_LEVELS / 2];
        }

        // Scale between 1x and 3x based on ai_class (SUSHI: only if autoscale
        // is on)
        if (aip->ai_class_autoscale)
            delay =
                delay +
                delay *
                    (float)(3 * (Num_ai_classes - aip->ai_class - 1) / (Num_ai_classes - 1));
...
void ai_maybe_evade_locked_missile (object* objp, ai_info* aip) {
    ...
    // Only small ships evade an incoming missile.  Why would a capital ship
    // try to swerve?
    ...
    if (aip->ai_flags
            [AI::AI_Flags::No_dynamic,
             AI::AI_Flags::Kamikaze]) { // If not allowed to pursue dynamic
                                        // objectives, don't evade.  Dumb?
    ...
    if (aip->nearest_locked_object != -1) {
        ...
        if ((missile_objp->type == OBJ_WEAPON) &&
            (Weapon_info[Weapons[missile_objp->instance].weapon_info_index]
                 .is_homing ())) {
            float dist = vm_vec_dist_quick (&missile_objp->pos, &objp->pos);
            float dist2 =
                4.0f * vm_vec_mag_quick (&missile_objp->phys_info.vel);
            if (dist < dist2) {
                switch (aip->mode) {
                // If in AIM_STRAFE mode, don't evade if parent of weapon is
                // targeted ship.
                case AIM_STRAFE:
                    if ((missile_objp->parent != -1) &&
                        (missile_objp->parent == aip->target_objnum)) {
                        ;
                    }
                    else {
                    ...
                    break;
                case AIM_CHASE:
                    // Don't always go into evade weapon mode.  Usually, a
                    // countermeasure gets launched.
                    // If low on countermeasures, more likely to try to evade.
                    // If 8+, never evade due to low cmeasures.
                    ...
                    break;
                case AIM_DOCK: // Ships in dock mode can evade iif they are
                               // not currently repairing or docked.
                    if (object_is_docked (objp) ||
                        (aip->ai_flags
                             [AI::AI_Flags::Repairing,
                              AI::AI_Flags::Being_repaired]))
                        break;
                case AIM_GUARD:
                    // If in guard mode and far away from guard object, don't
                    // pursue guy that hit me.
                    if ((aip->guard_objnum != -1) &&
                        (aip->guard_signature ==
                         Objects[aip->guard_objnum].signature)) {
                        if (vm_vec_dist_quick (
                                &objp->pos, &Objects[aip->guard_objnum].pos) >
                            500.0f) {
                            return;
                        }
                    }
                ...
                case AIM_FLY_TO_SHIP:
                    ...
                    aip->submode_start_time = Missiontime;
                    aip->mode_time =
                        timestamp (MAX_EVADE_TIME); // Max time to evade.
                    break;
                case AIM_EVADE_WEAPON: // Note: We don't want to change mode
                                       // on another evasion, or previous_mode
                                       // will get bashed.
                case AIM_PLAY_DEAD:
                case AIM_BAY_DEPART:
...
void maybe_evade_dumbfire_weapon (ai_info* aip) {
    // Only small ships evade an incoming missile.  Why would a capital ship
    // try to swerve?
    if (!Ship_info[Ships[Pl_objp->instance].ship_info_index]
             .is_small_ship ()) {
        return;
    }

    // don't evade missiles if you're docked
    if (object_is_docked (Pl_objp)) { return; }

    // Make sure in a mode in which we evade dumbfire weapons.
    switch (aip->mode) {
    case AIM_CHASE:
        if (aip->submode == SM_ATTACK_FOREVER) { return; }
        break;
    case AIM_GUARD:
        // If in guard mode and far away from guard object, don't pursue guy
        // that hit me.
        if ((aip->guard_objnum != -1) &&
            (aip->guard_signature == Objects[aip->guard_objnum].signature)) {
            if (vm_vec_dist_quick (
                    &Objects[Ships[aip->shipnum].objnum].pos,
                    &Objects[aip->guard_objnum].pos) > 500.0f) {
                return;
            }
        }
    case AIM_STILL:
    ...
    if ((t > 0.0f) && (t < 1.0f)) {
        // Check if this weapon is from a large ship Pl_objp is attacking... if
        // so, enter strafe mode
        if (!(aip->ai_flags[AI::AI_Flags::No_dynamic])) {
            if (ai_big_maybe_enter_strafe_mode (
                    Pl_objp, aip->danger_weapon_objnum)) {
                return;
            }
        }
        ...
        case AIM_SAFETY:
            if (!(aip->ai_flags
                      [AI::AI_Flags::No_dynamic, AI::AI_Flags::Kamikaze]) &&
                ...
                aip->submode = -1;
                aip->submode_start_time = Missiontime;
                aip->mode_time = timestamp (
                    MAX_EVADE_TIME); // Evade for up to five seconds.
            }
            break;
        case AIM_STRAFE:
        case AIM_BIGSHIP:
...
// input:       pl_objp  =>   the object which is/has entered/exited the fighter
// bay
// aip      =>   ai info for said object
// done     =>   true if the object has is finished with the path, or
// is dead
void ai_manage_bay_doors (object* pl_objp, ai_info* aip, bool done) {
    ...
    if (done && !shipp->bay_doors_need_open) return;

    if (done) shipp->bay_doors_need_open = false;

    // if this happens then the parent was probably destroyed/departed, so just
    // skip the rest
    if (shipp->bay_doors_parent_shipnum < 0) return;

    ship* parent_ship = &Ships[shipp->bay_doors_parent_shipnum];

    if (done) parent_ship->bay_doors_wanting_open--;

    // trigger an open if we need it
    if ((parent_ship->bay_doors_status == MA_POS_NOT_SET) &&
        (parent_ship->bay_doors_wanting_open > 0)) {
        if (model_anim_start_type (
                parent_ship, TRIGGER_TYPE_DOCK_BAY_DOOR,
                shipp->bay_doors_launched_from, 1)) {
            parent_ship->bay_doors_status = MA_POS_SET;
            parent_ship->bay_doors_anim_done_time = model_anim_get_time_type (
                parent_ship, TRIGGER_TYPE_DOCK_BAY_DOOR,
    ...
    if ((parent_ship->bay_doors_status == MA_POS_READY) &&
        (parent_ship->bay_doors_wanting_open <= 0)) {
        model_anim_start_type (
            parent_ship, TRIGGER_TYPE_DOCK_BAY_DOOR,
            shipp->bay_doors_launched_from, -1);
    ...
    if ((parent_ship->bay_doors_status == MA_POS_SET) &&
        timestamp_elapsed (parent_ship->bay_doors_anim_done_time)) {
        parent_ship->bay_doors_status = MA_POS_READY;
    }
}
...
// 0              => success
int ai_acquire_emerge_path (
    object* pl_objp, int parent_objnum, int allowed_path_mask, vec3d* pos,
    vec3d* fvec) {
    ...
    if (bay == NULL) {
        WARNINGF (LOCATION,"Ship %s was set to arrive from fighter bay on object %s, but no fighter bay exists on that ships' model (%s).",shipp->ship_name, parent_shipp->ship_name, pm->filename);
        return -1;
    }

    if (bay->num_paths <= 0) {
        WARNINGF (LOCATION,"Ship %s was set to arrive from fighter bay on object %s, but no fighter bay paths exist on that ships' model (%s).",shipp->ship_name, parent_shipp->ship_name, pm->filename);
        return -1;
    }
    ...
    create_model_exit_path (
        pl_objp, parent_objp, path_index, pm->paths[path_index].nverts);

    // now return to the caller what the starting world pos and starting fvec
    ...
    *pos = pnp->pos;
    ...
    // record the parent objnum, since we'll need it once we're done with
    // following the path
    ...
    // due to the door animations the ship has to remain still until the doors
    // are open
    ...
    pl_objp->phys_info.forward_thrust =
...
void ai_bay_emerge () {
    ...
    if (aip->goal_objnum < 0) { parent_died = 1; }
    ...
    if (!parent_died) {
        ASSERT (Objects[aip->goal_objnum].type == OBJ_SHIP);
        if (Ships[Objects[aip->goal_objnum].instance]
                .flags[Ship::Ship_Flags::Dying]) {
            parent_died = 1;
        }
    }
    ...
    if (Ships[Objects[aip->goal_objnum].instance].bay_doors_status !=
        MA_POS_READY)
        return;
    ...
    // New test: must have been in AI_EMERGE mode for at least 10 seconds, and
    // be a minimum distance from the start point
    if (((Missiontime - aip->submode_start_time) > 10 * F1_0) &&
        (vm_vec_dist_quick (&Pl_objp->pos, &Objects[aip->goal_objnum].pos) >
         0.75f * Objects[aip->goal_objnum].radius)) {
        // erase path
        ai_emerge_bay_path_cleanup (aip);
    ...
    // 2-25-99: Need this check to fix an assert for supercap ships... maybe
    // we'll only do this check for supercaps
...
// pm              =>              pointer to polymodel for the ship contining the
// ship bay/depart paths
//
// exit:                >=0     =>              ship bay path index for depart path (ie index into
// sb->paths[])
...
int ai_find_closest_depart_path (
    ...
    if (best_free_path >= 0) { return best_free_path; }
...
// 0       => found depart path
int ai_acquire_depart_path (
    object* pl_objp, int parent_objnum, int allowed_path_mask) {
    int path_index = -1;
    int ship_bay_path = -1;
    ...
    if (parent_objnum < 0) {
        WARNINGF (LOCATION,"In ai_acquire_depart_path(), specified a negative object number for the parent ship!  (Departing ship is %s.)  Looking for another ship...",shipp->ship_name);

        // try to locate a capital ship on the same team:
        int shipnum = ship_get_ship_for_departure (shipp->team);

        if (shipnum >= 0) parent_objnum = Ships[shipnum].objnum;
    }

    aip->path_start = -1;

    if (parent_objnum < 0) return -1;
    ...
    // Set this flag, so we don't bother recreating the path... we won't need
    // to update the path that has just been created.
...
void ai_bay_depart () {
    ...
    anchor_shipnum = ship_name_lookup (
        Parse_names[Ships[Pl_objp->instance].departure_anchor]);
    if (anchor_shipnum < 0 ||
    ...
    if (Ships[anchor_shipnum].bay_doors_status != MA_POS_READY) return;

    // follow the path to the final point
    ai_path ();

    // if we are on the last segment of the path and the bay doors are not
    // open, open them if we are on any other segment of the path, close them
    ...
    if (aip->path_cur >= (aip->path_start + aip->path_length)) {
        ...
        pm = model_get (Ship_info[Ships[Objects[aip->goal_objnum].instance]
                            ...
                            .model_num);
        bay = pm->ship_bay;
        if (bay != NULL) { bay->depart_flags &= ~(1 << aip->submode_parm0); }
...
/**
 * Handler for AIM_SENTRYGUN.  This AI mode is for sentry guns only (ie
 * floating turrets).
 */
void ai_sentrygun () {
    // Nothing to do here.  Turret firing is handled via process_subobjects().
    // If you want the sentry guns to do anything beyond firing their turrets
    // at enemies, add it here!
}
...
void ai_execute_behavior (ai_info* aip) {
    ...
    case AIM_EVADE:
        if (En_objp) { ai_evade (); }
            ...
            vec3d tvec;
            vm_vec_scale_add (
                &tvec, &Pl_objp->pos, &Pl_objp->orient.vec.rvec, 100.0f);
            turn_towards_point (Pl_objp, &tvec, NULL, 0.0f);
            accelerate_ship (aip, 0.5f);
        }
        break;
    case AIM_STILL: ai_still (); break;
    case AIM_STAY_NEAR: ai_stay_near (); break;
    case AIM_GUARD: ai_guard (); break;
    case AIM_WAYPOINTS: ai_waypoints (); break;
    case AIM_FLY_TO_SHIP: ai_fly_to_ship (); break;
    case AIM_DOCK: ai_dock (); break;
    case AIM_NONE: break;
    case AIM_BIGSHIP: break;
    case AIM_PATH: {
        int path_num;
        path_num =
            ai_return_path_num_from_dockbay (&Objects[aip->goal_objnum], 0);
        ai_find_path (Pl_objp, aip->goal_objnum, path_num, 0);
        ai_path ();
        break;
    }
    case AIM_SAFETY: ai_safety (); break;
    case AIM_EVADE_WEAPON: evade_weapon (); break;
    case AIM_STRAFE:
        if (En_objp) {
            ASSERT (En_objp->type == OBJ_SHIP);
        ...
        break;
    case AIM_BAY_EMERGE: ai_bay_emerge (); break;
    case AIM_BAY_DEPART: ai_bay_depart (); break;
...
// Since subsystems cannot be repaired if they are at 0 strength, don't return
// 1 if subsystem is dead.
int mrs_subsystem (ship* shipp, int type) {
    float t;

    t = ship_get_subsystem_strength (shipp, type);

    if (t > 0.0f) { return (int)((1.0f - t) * 3); }
    else {
        return 3;
    }
}
...
int num_allies_rearming (object* objp) {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (Ships[A->instance].team == team) {
            if (Ai_info[Ships[A->instance].ai_index].ai_flags
                    [AI::AI_Flags::Repairing, AI::AI_Flags::Awaiting_repair]) {
                count++;
            }
        }
    }
...
int maybe_request_support (object* objp) {
    ...
    if (!(sip->is_fighter_bomber ())) return 0;
    ...
    if (!timestamp_elapsed (aip->next_rearm_request_timestamp)) return 0;

    // just do a simple check first, and the more expensive check later
    if (!is_support_allowed (objp, true)) return 0;

    // Goober5000 - a ship that is currently docked shouldn't request repair.
    // This is to prevent support ships being summoned for a ship that is
    // getting towed away by other means.
    if (object_is_docked (objp)) return 0;

    // Compute a desire value.
    // Desire of 0 means no reason to request support.
    // 1 is slight, 2 more, etc.  Maximum is around 20.  Anything larger than
    // 3 is pretty strong.
    desire = 0;

    // Set desire based on hull strength.
    // Note: We no longer repair hull, so this would cause repeated repair
    // requests.
    ...
    for (i = 0; i < swp->num_secondary_banks; ++i) {
        if (swp->secondary_bank_start_ammo[i] > 0) {
            r = (float)swp->secondary_bank_ammo[i] /
                swp->secondary_bank_start_ammo[i];
    ...
    for (i = 0; i < swp->num_primary_banks; ++i) {
        ...
        if (wip->wi_flags[Weapon::Info_Flags::Ballistic] &&
            swp->primary_bank_start_ammo[i] > 0) {
            r = (float)swp->primary_bank_ammo[i] /
                swp->primary_bank_start_ammo[i];

            // cube ammo level for better behavior, and adjust for number of
            // banks
            desire +=
                (int)((1.0f - r) * (1.0f - r) * (1.0f - r) * (5.0f / swp->num_primary_banks));
        }
    }
    ...
    if (ai_good_time_to_rearm (objp)) { try_to_rearm = true; }
    else if (num_allies_rearming (objp) < 2) {
        if (desire >= 8) { // guarantees disabled will cause repair request
            try_to_rearm = true;
        }
        else if (desire >= 3) { // >= 3 means having a single subsystem fully
                                // blown will cause repair.
            int count;
            int objnum = find_nearby_threat (
                OBJ_INDEX (objp), iff_get_attacker_mask (obj_team (objp)),
                2000.0f, &count);

            if ((objnum == -1) || (count < 2) ||
                (vm_vec_dist_quick (&objp->pos, &Objects[objnum].pos) >
                 3000.0f * count / desire)) {
                try_to_rearm = true;
            }
        }
    }
...
// Shivan and HoL fighter/bomber warp out if their weapons subsystems have
// been destroyed.
void ai_maybe_depart (object* objp) {
    ...
    if (aip->mode == AIM_WARP_OUT || aip->mode == AIM_BAY_DEPART) return;

    // If a support ship with no goals and low hull, depart.  Be sure that
    ...
    if (Iff_info[shipp->team].flags & IFFF_SUPPORT_ALLOWED) return;

    if (!shipp->is_departing ()) {
        if (sip->is_fighter_bomber ()) {
            if (aip->warp_out_timestamp == 0) {
                // if (ship_get_subsystem_strength(shipp, SUBSYSTEM_WEAPONS) ==
                // 0.0f) {      aip->warp_out_timestamp = timestamp(((myrand() %
                // 10)
                //+ 10) * 1000);
                //}
            }
...
void ai_warp_out (object* objp) {
    ...
    if (!ship_engine_ok_to_warp (shipp) ||
        !ship_navigation_ok_to_warp (shipp)) {
        // you shouldn't hit this... if you do, then I need to add a check for
        // it in whatever function initiates a warpout
        ASSERT (!(shipp->flags[Ship::Ship_Flags::No_subspace_drive]));

        // flag us as trying to warp so that this function keeps getting called
        // (in other words, if we can't warp just yet, we want to warp at the
        // first opportunity)
        if (aip->mode != AIM_WARP_OUT) aip->mode = AIM_WARP_OUT;
        aip->submode = AIS_WARP_1;
    ...
    switch (aip->submode) {
    case AIS_WARP_1:
        aip->force_warp_time = timestamp (
            10 * 1000); // Try to avoid a collision for up to ten seconds.
    ...
    case AIS_WARP_2: // Make sure won't collide with any object.
        if (timestamp_elapsed (aip->force_warp_time) ||
            (!collide_predict_large_ship (
                 objp, objp->radius * 2.0f + 100.0f) ||
             (Ship_info[shipp->ship_info_index].warpout_type ==
                  WT_HYPERSPACE &&
            ...
            if ((objp->flags[Object::Object_Flags::Collides]) &&
                (ship_get_warpout_speed (objp) > ship_get_max_speed (shipp))) {
                // recalculate collision pairs
                OBJ_RECALC_PAIRS (objp);
            }

            aip->force_warp_time = timestamp (
                4 * 1000); // Try to attain target speed for up to 4 seconds.
        }
        else {
            vec3d goal_point;
            vm_vec_scale_add (
                &goal_point, &objp->pos, &objp->orient.vec.uvec, 100.0f);
            turn_towards_point (objp, &goal_point, NULL, 0.0f);
            accelerate_ship (aip, 0.0f);
        }
        break;
    case AIS_WARP_3:
        // Rampup desired_vel in here from current to desired velocity and set
        // PF_USE_VEL. (not sure this is the right flag)        See shipfx#572 for
        // sample code.
        ...
        speed = goal_speed * flFrametime +
                objp->phys_info.speed * (1.0f - flFrametime);
        vm_vec_copy_scale (
            &objp->phys_info.vel, &objp->orient.vec.fvec, speed);
    ...
    case AIS_WARP_4: {
        ...
        if ((Missiontime / 100) >=
            (aip->submode_start_time / 100 +
             Ship_info[shipp->ship_info_index].warpout_engage_time)) {
...
// Return object index of weapon that could produce a shockwave that should be
// known about to *objp.        Return nearest one.
...
// OK to tell everyone to avoid.  If they're too far away, that gets cleaned
// up in the frame interval.
void ai_announce_ship_dying (object* dying_objp) {
    ...
    if (damage >= EVADE_SHOCKWAVE_DAMAGE_THRESHOLD) {
        ...
        for (so = GET_FIRST (&Ship_obj_list);
            ...
            // Goober5000 - the disallow is now handled uniformly in the
            // ai_avoid_shockwave function
...
static int ai_find_shockwave_ship (object* objp) {
    ...
    for (so = GET_NEXT (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (shipp->flags[Ship::Ship_Flags::Dying]) {
            ...
            if (damage >=
                EVADE_SHOCKWAVE_DAMAGE_THRESHOLD) { // Only evade quite large
                                                    // blasts
                float dist;
...
int aas_1 (object* objp, ai_info* aip, vec3d* safe_pos) {
    ...
    if (aip->ai_flags[AI::AI_Flags::Avoid_shockwave_weapon]) {
        ...
        // If we can't find one, then clear the bit so we don't keep coming
        // here.
        ...
        // OK, we have reason to believe we should avoid
        // aip->shockwave_object.
        ...
        float danger_dist;
        vec3d expected_pos; // Position at which we expect the weapon to
                            // detonate.
        int pos_set = 0;

        danger_dist = wip->shockwave.outer_rad;
        // Set predicted position of detonation.
        // If an aspect locked missile, assume it will detonate at the homing
        // position.    If not, which is not possible in a default FreeSpace
        // weapon, then predict it will detonate at some        time in the future,
        // this time based on max lifetime and life left.
        if (wip->is_locked_homing ()) {
        ...
        if (!pos_set) {
            ...
            vm_vec_scale_add (
                &expected_pos, &weapon_objp->pos,
                &weapon_objp->orient.vec.fvec, time_scale);
        }

        // See if too far away to care about shockwave.
        if (vm_vec_dist_quick (&objp->pos, &expected_pos) >
            danger_dist * 2.0f) {
            ...
            float dot =
                vm_vec_dot (&vec_from_exp, &weapon_objp->orient.vec.fvec);
            if (dot > -30) {
                // if we're already on the other side of the explosion, don't
                // try to fly behind it
                dir = -1.0f;
            }

            // Fly towards a point behind the weapon.
            vm_vec_scale_add (
                safe_pos, &weapon_objp->pos, &weapon_objp->orient.vec.fvec,
                -50000.0f * dir);
        ...
        // Optimize note! Don't really have to normalize.  We only need a
        // point away from the blowing-up ship.
        vec3d safe_vec;

        vm_vec_normalized_dir (&safe_vec, &objp->pos, &ship_objp->pos);
        vm_vec_scale_add (
            safe_pos, &ship_objp->pos, &safe_vec,
            50000.0f); // Fly away from the ship.

        float outer_rad = ship_get_exp_outer_rad (ship_objp);

        if (vm_vec_dist_quick (&objp->pos, &ship_objp->pos) >
            outer_rad * 1.5f) {
            aip->ai_flags.remove (AI::AI_Flags::Avoid_shockwave_ship);
            return 0;
        }
...
// If it looks like there is no valid shockwave-producing weapon then clear
// the AIF_AVOID_SHOCKWAVE_WEAPON bit in ai_flags and return.   Return 1 if
// avoiding a shockwave, else return 0.
int ai_avoid_shockwave (object* objp, ai_info* aip) {
    vec3d safe_pos;

    // BIG|HUGE do not respond to shockwaves
    // Goober5000 - let's treat shockwave response the same way whether from
    // weapon or ship
    if (!Ship_info[Ships[objp->instance].ship_info_index]
             .avoids_shockwaves ()) {
...
int ai_await_repair_frame (object* objp, ai_info* aip) {
    ...
    if (aip->support_ship_objnum == -1) return 0;
    ...
    aip->ai_flags.remove (
        AI::AI_Flags::Formation_object); // Prevents endless rotation.

    if (!(sip->flags[Ship::Info_Flags::Support])) return 0;
    ...
    vm_vec_scale_add (
        &goal_point, &repair_objp->pos, &repair_objp->orient.vec.uvec,
...
// Currently, any small ship (SIF_SMALL_SHIP) that has been disabled will
// self-destruct after awhile.  Maybe should only do this if they are
// preventing their wing from re-entering.
void ai_maybe_self_destruct (object* objp, ai_info* aip) {
    ASSERTX (
        objp->type == OBJ_SHIP,
        "ai_maybe_self_destruct() can only be called with objects that are "
        "ships!");
    ship* shipp = &Ships[objp->instance];

    // Some IFFs can be repaired, so no self-destruct.
    // In multiplayer, just don't self-destruct.  I figured there would be a
    // problem. -- MK, 3/19/98.
    if ((Iff_info[shipp->team].flags & IFFF_SUPPORT_ALLOWED) ||
        (Game_mode & GM_MULTIPLAYER))
        return;

    // Small ships in a wing blow themselves up after awhile if engine or
    // weapons system has been destroyed.       Reason: Don't want them to prevent
    // a re-emergence of the wing.      Note: Don't blow up if not in a wing for
    // two reasons: One, won't affect re-emergence of waves and (1) disable the
    // Dragon   mission would be broken.        Also, don't blow up the ship if it
    // has a ship flag preventing this - Goober5000
    if ((Ship_info[shipp->ship_info_index].is_small_ship ()) &&
        (shipp->wingnum >= 0) &&
        !(shipp->flags[Ship::Ship_Flags::No_disabled_self_destruct])) {
        if ((ship_get_subsystem_strength (shipp, SUBSYSTEM_ENGINE) <= 0.0f) ||
            (ship_get_subsystem_strength (shipp, SUBSYSTEM_WEAPONS) <= 0.0f)) {
            if (aip->self_destruct_timestamp < 0)
                aip->self_destruct_timestamp =
                    timestamp (90 * 1000); // seconds until self-destruct
        }
        else {
            aip->self_destruct_timestamp = -1;
        }

        if (aip->self_destruct_timestamp < 0) { return; }

        if (timestamp_elapsed (aip->self_destruct_timestamp)) {
            ship_apply_local_damage (
                objp, objp, &objp->pos,
                objp->hull_strength * flFrametime + 1.0f, MISS_SHIELDS);
        }
    }
}
...
int ai_need_new_target (object* pl_objp, int target_objnum) {
    ...
    if (objp->type == OBJ_SHIP) {
        if (Ships[objp->instance].flags[Ship::Ship_Flags::Dying]) { return 1; }
        else if (Ships[objp->instance].team == Ships[pl_objp->instance].team) {
            // Goober5000 - targeting the same team is allowed if pl_objp is
            // going bonkers
            ai_info* pl_aip = &Ai_info[Ships[pl_objp->instance].ai_index];
            if (pl_aip->active_goal != AI_GOAL_NONE &&
                pl_aip->active_goal != AI_ACTIVE_GOAL_DYNAMIC) {
                if (pl_aip->goals[pl_aip->active_goal]
                        .flags[AI::Goal_Flags::Target_own_team]) {
                    return 0;
                }
            }
...
int maybe_big_ship_collide_recover_frame (object* objp, ai_info* aip) {
    ...
    if (aip->ai_flags[AI::AI_Flags::Big_ship_collide_recover_1]) {
        ai_turn_towards_vector (
            &aip->big_recover_pos_1, objp,
            Ship_info[Ships[objp->instance].ship_info_index].srotation_time,
            nullptr, nullptr, 0.0f, 0, nullptr);
        dist =
            vm_vec_normalized_dir (&v2g, &aip->big_recover_pos_1, &objp->pos);
        dot = vm_vec_dot (&objp->orient.vec.fvec, &v2g);
        accelerate_ship (aip, dot);

        // If close to desired point, or 15+ seconds since entered this mode,
        // continue to next mode.
        if ((timestamp_until (aip->big_recover_timestamp) < -15 * 1000) ||
            (dist < (0.5f + flFrametime) * objp->phys_info.speed)) {
        ...
        ai_turn_towards_vector (
            &aip->big_recover_pos_2, objp,
            Ship_info[Ships[objp->instance].ship_info_index].srotation_time,
            nullptr, nullptr, 0.0f, 0, nullptr);
        dist =
            vm_vec_normalized_dir (&v2g, &aip->big_recover_pos_2, &objp->pos);
        dot = vm_vec_dot (&objp->orient.vec.fvec, &v2g);
        accelerate_ship (aip, dot);

        // If close to desired point, or 30+ seconds since started avoiding
        // collision, done avoiding.
        if ((timestamp_until (aip->big_recover_timestamp) < -30 * 1000) ||
            (dist < (0.5f + flFrametime) * objp->phys_info.speed)) {
...
void ai_frame (int objnum) {
    ...
    // warping out?
    if ((aip->mode == AIM_WARP_OUT) ||
        (aip->ai_flags[AI::AI_Flags::Trying_unsuccessfully_to_warp])) {
        ai_warp_out (Pl_objp);

        // Goober5000 - either we were never trying unsuccessfully, or we were
        // but now we're successful... in either case, since we're actually
        // warping we simply return
    ...
    // Avoid a shockwave, if necessary.  If a shockwave and rearming, stop
    // rearming.
    if (aip->ai_flags
            [AI::AI_Flags::Avoid_shockwave_ship,
             AI::AI_Flags::Avoid_shockwave_weapon]) {
        if (ai_avoid_shockwave (Pl_objp, aip)) {
            aip->ai_flags.remove (AI::AI_Flags::Big_ship_collide_recover_1);
            aip->ai_flags.remove (AI::AI_Flags::Big_ship_collide_recover_2);
            if (aip->ai_flags
                    [AI::AI_Flags::Being_repaired,
                     AI::AI_Flags::Awaiting_repair])
                ai_abort_rearm_request (Pl_objp);
            return;
        }
    }
    ...
    // moved call to ai_do_repair frame here from below because of the
    // subsequent if statment returning if the ship is getting repaired
    ...
    if ((aip->ai_flags[AI::AI_Flags::Awaiting_repair]) ||
        ...
        if (ai_await_repair_frame (Pl_objp, aip)) return;
    }

    if (aip->mode == AIM_PLAY_DEAD) return;

    // If recovering from a collision with a big ship, don't continue.
    if (maybe_big_ship_collide_recover_frame (Pl_objp, aip)) return;
    ...
    if (maybe_request_support (Pl_objp)) {
        if (Ships[Pl_objp->instance]
                .flags[Ship::Ship_Flags::From_player_wing]) {
    ...
    if (ai_need_new_target (Pl_objp, target_objnum)) {
        if ((aip->mode != AIM_EVADE_WEAPON) &&
            ...
            // AL 12-9-97: Don't allow cargo and navbuoys to set their
            // aip->target_objnum
            if (Ship_info[shipp->ship_info_index].class_type > -1 &&
                (Ship_types[Ship_info[shipp->ship_info_index].class_type]
                     .flags[Ship::Type_Info_Flags::AI_auto_attacks])) {
                target_objnum = find_enemy (
                    objnum, MAX_ENEMY_DISTANCE,
                    The_mission.ai_profile
                        ->max_attackers[Game_skill_level]); // Attack up to
                                                            // 25K units away.
                if (target_objnum != -1) {
    ...
    if (En_objp && En_objp->type == OBJ_SHIP) {
        if (Ships[En_objp->instance].flags[Ship::Ship_Flags::Stealth]) {
            ...
            if ((stealth_state == STEALTH_FULLY_TARGETABLE) ||
                (stealth_state == STEALTH_IN_FRUSTUM)) {
                aip->stealth_last_visible_stamp = timestamp ();
                aip->stealth_last_cheat_visible_stamp =
                    aip->stealth_last_visible_stamp;
                ...
                // set approximate pos and vel, with increasing error as time
                // from last_visible_stamp increases
                update_ai_stealth_info_with_error (aip /*, 0*/);
            }
        }
    }

    // AL 12-10-97: ensure that cargo and navbuoys aip->target_objnum is always
    // -1.
    if (Ship_info[shipp->ship_info_index].class_type > -1 &&
        !(Ship_types[Ship_info[shipp->ship_info_index].class_type]
              .flags[Ship::Type_Info_Flags::AI_auto_attacks])) {
    ...
    if ((En_objp != NULL) && (En_objp->pos.xyz.x == Pl_objp->pos.xyz.x) &&
        ...
        WARNINGF (LOCATION, "Object and its enemy have same position.  Object #%d",OBJ_INDEX (Pl_objp));
        En_objp = NULL;
    }

    if (aip->mode == AIM_CHASE) {
        if (En_objp == NULL) { aip->active_goal = -1; }
    }

    // If there is a goal to resume and enough time has elapsed, resume the
    // goal.
    ...
    // check if targeted subsystem has been destroyed, if so, move onto another
    // subsystem if trying to disable or disarm the target
    if ((En_objp != NULL) && (aip->targeted_subsys != NULL)) {
        ...
        if (aip->targeted_subsys->current_hits <= 0.0f) {
            ...
            if (subsys_type != -1) {
                ...
                new_subsys = ship_return_next_subsys (
                    &Ships[En_objp->instance], subsys_type, &Pl_objp->pos);
                if (new_subsys != NULL) {
                    set_targeted_subsys (aip, new_subsys, aip->target_objnum);
                }
                else {
                    // AL 12-16-97: no more subsystems to attack... reset
                    // targeting info
                    aip->target_objnum = -1;
                    set_targeted_subsys (aip, NULL, -1);
                }
    ...
    if (!in_formation) { ai_execute_behavior (aip); }
...
static void ai_control_info_check (ai_info* aip) {
    if (aip->ai_override_flags.none_set ()) return;
    ...
    if (timestamp_elapsed (aip->ai_override_timestamp)) {
        ...
        if (aip->ai_override_flags[AI::Maneuver_Override_Flags::Full_lat]) {
            ...
            if (aip->ai_override_flags
                    [AI::Maneuver_Override_Flags::Sideways]) {
                AI_ci.sideways = aip->ai_override_ci.sideways;
            }
...
void ai_process (object* obj, int ai_index, float frametime) {
    ...
    // return if ship is dead, unless it's a big ship...then its turrets still
    // fire, like I was quoted in a magazine.  -- MK, 5/15/98.
    if ((Ships[obj->instance].flags[Ship::Ship_Flags::Dying]) &&
        !(Ship_info[Ships[obj->instance].ship_info_index].is_big_or_huge ())) {
        return;
    }
    ...
    if (OBJ_INDEX (obj) <= Last_ai_obj) { AI_FrameCount++; }
    ...
    case AIM_DOCK:
        ...
        break;
    case AIM_WARP_OUT:
        if (aip->submode >= AIS_WARP_3) rfc = 0;
        break;
    default: break;
    }

    if (rfc == 1) {
        // Wanderer - sexp based override goes here - only if rfc is valid
        // though
        ai_control_info_check (aip);
        vec3d copy_desired_rotvel = obj->phys_info.rotvel;
        physics_read_flying_controls (
            &obj->orient, &obj->phys_info, &AI_ci, frametime);
...
void init_ai_object (int objnum) {
    ...
    // Things that shouldn't have to get initialized, but initialize them just
    // in case!
    aip->ai_flags.reset ();
    ...
    init_aip_from_class_and_profile (
        aip, &Ai_classes[Ship_info[ship_type].ai_class],
        The_mission.ai_profile);
    ...
    aip->last_predicted_enemy_pos.xyz.x =
        0.0f; // Says this value needs to be recomputed!
    aip->time_enemy_in_range = 0.0f;
    aip->time_enemy_near = 0.0f;
    ...
    // The next two fields are used to time the rearming to allow useful sound
    // effects for missile rearming
    aip->rearm_first_missile = TRUE; // flag to indicate that next missile to
                                     // load is the first missile
    aip->rearm_first_ballistic_primary =
        TRUE; // flag to indicate that next ballistic to load is the first
              // ballistic
    aip->rearm_release_delay =
        0; // timestamp to delay the separation of docked ships after rearm
    ...
    aip->last_objsig_hit =
        -1; // object signature of the ship most recently hit by aip
    ...
    aip->self_destruct_timestamp =
        -1; // This is a flag that we have not yet set this.
...
void init_ai_system () { Ppfp = Path_points; }

// Sets the ai_info stuff based on what is in the ai class and the current ai
// profile Stuff in the ai class will override what is in the ai profile, but
// only if it is set. Unset per-difficulty-level values are marked with FLT_MIN
// or INT_MIN Which flags are set is handled by using two flag ints: one with
// the flag values (TRUE/FALSE), one that just says which flags are set.
void init_aip_from_class_and_profile (
    ai_info* aip, ai_class* aicp, ai_profile_t* profile) {
    ...
    if (Game_skill_level < 0 || Game_skill_level >= NUM_SKILL_LEVELS) {
        WARNINGF (LOCATION,"Invalid skill level %i! Valid range 0 to %i. Resetting to default.",Game_skill_level, NUM_SKILL_LEVELS);
    ...
    aip->ai_shockwave_evade_chance =
        aicp->ai_shockwave_evade_chance[Game_skill_level];
    aip->ai_get_away_chance = aicp->ai_get_away_chance[Game_skill_level];
    aip->ai_secondary_range_mult =
        aicp->ai_secondary_range_mult[Game_skill_level];
    ...
    aip->ai_cmeasure_fire_chance =
        (aicp->ai_cmeasure_fire_chance[Game_skill_level] == FLT_MIN)
            ? profile->cmeasure_fire_chance[Game_skill_level]
            : aicp->ai_cmeasure_fire_chance[Game_skill_level];
    aip->ai_in_range_time =
        (aicp->ai_in_range_time[Game_skill_level] == FLT_MIN)
            ? profile->in_range_time[Game_skill_level]
            : aicp->ai_in_range_time[Game_skill_level];
    aip->ai_link_ammo_levels_maybe =
        (aicp->ai_link_ammo_levels_maybe[Game_skill_level] == FLT_MIN)
            ? profile->link_ammo_levels_maybe[Game_skill_level]
            : aicp->ai_link_ammo_levels_maybe[Game_skill_level];
    aip->ai_link_ammo_levels_always =
        (aicp->ai_link_ammo_levels_always[Game_skill_level] == FLT_MIN)
    ...
    aip->ai_primary_ammo_burst_mult =
        (aicp->ai_primary_ammo_burst_mult[Game_skill_level] == FLT_MIN)
            ? profile->primary_ammo_burst_mult[Game_skill_level]
            : aicp->ai_primary_ammo_burst_mult[Game_skill_level];
    aip->ai_link_energy_levels_maybe =
        (aicp->ai_link_energy_levels_maybe[Game_skill_level] == FLT_MIN)
            ? profile->link_energy_levels_maybe[Game_skill_level]
            : aicp->ai_link_energy_levels_maybe[Game_skill_level];
    aip->ai_link_energy_levels_always =
        (aicp->ai_link_energy_levels_always[Game_skill_level] == FLT_MIN)
            ? profile->link_energy_levels_always[Game_skill_level]
            : aicp->ai_link_energy_levels_always[Game_skill_level];
    aip->ai_predict_position_delay =
        (aicp->ai_predict_position_delay[Game_skill_level] == INT_MIN)
            ? profile->predict_position_delay[Game_skill_level]
            : aicp->ai_predict_position_delay[Game_skill_level];
    aip->ai_shield_manage_delay =
        (aicp->ai_shield_manage_delay[Game_skill_level] == FLT_MIN)
            ? profile->shield_manage_delay[Game_skill_level]
            : aicp->ai_shield_manage_delay[Game_skill_level];
    aip->ai_ship_fire_delay_scale_friendly =
        (aicp->ai_ship_fire_delay_scale_friendly[Game_skill_level] == FLT_MIN)
            ? profile->ship_fire_delay_scale_friendly[Game_skill_level]
            : aicp->ai_ship_fire_delay_scale_friendly[Game_skill_level];
    aip->ai_ship_fire_delay_scale_hostile =
        (aicp->ai_ship_fire_delay_scale_hostile[Game_skill_level] == FLT_MIN)
            ? profile->ship_fire_delay_scale_hostile[Game_skill_level]
            : aicp->ai_ship_fire_delay_scale_hostile[Game_skill_level];
    aip->ai_ship_fire_secondary_delay_scale_friendly =
        (aicp->ai_ship_fire_secondary_delay_scale_friendly[Game_skill_level] ==
         FLT_MIN)
            ? profile
                  ->ship_fire_secondary_delay_scale_friendly[Game_skill_level]
            : aicp->ai_ship_fire_secondary_delay_scale_friendly
                  [Game_skill_level];
    aip->ai_ship_fire_secondary_delay_scale_hostile =
        (aicp->ai_ship_fire_secondary_delay_scale_hostile[Game_skill_level] ==
         FLT_MIN)
            ? profile
                  ->ship_fire_secondary_delay_scale_hostile[Game_skill_level]
            : aicp->ai_ship_fire_secondary_delay_scale_hostile
                  [Game_skill_level];
    aip->ai_turn_time_scale =
        (aicp->ai_turn_time_scale[Game_skill_level] == FLT_MIN)
            ? profile->turn_time_scale[Game_skill_level]
            : aicp->ai_turn_time_scale[Game_skill_level];
    aip->ai_glide_attack_percent =
        (aicp->ai_glide_attack_percent[Game_skill_level] == FLT_MIN)
            ? profile->glide_attack_percent[Game_skill_level]
            : aicp->ai_glide_attack_percent[Game_skill_level];
    aip->ai_circle_strafe_percent =
        (aicp->ai_circle_strafe_percent[Game_skill_level] == FLT_MIN)
            ? profile->circle_strafe_percent[Game_skill_level]
            : aicp->ai_circle_strafe_percent[Game_skill_level];
    aip->ai_glide_strafe_percent =
        (aicp->ai_glide_strafe_percent[Game_skill_level] == FLT_MIN)
    ...
    aip->ai_random_sidethrust_percent =
        (aicp->ai_random_sidethrust_percent[Game_skill_level] == FLT_MIN)
            ? profile->random_sidethrust_percent[Game_skill_level]
            : aicp->ai_random_sidethrust_percent[Game_skill_level];
    aip->ai_stalemate_time_thresh =
        (aicp->ai_stalemate_time_thresh[Game_skill_level] == FLT_MIN)
            ? profile->stalemate_time_thresh[Game_skill_level]
            : aicp->ai_stalemate_time_thresh[Game_skill_level];
    aip->ai_stalemate_dist_thresh =
        (aicp->ai_stalemate_dist_thresh[Game_skill_level] == FLT_MIN)
            ? profile->stalemate_dist_thresh[Game_skill_level]
            : aicp->ai_stalemate_dist_thresh[Game_skill_level];
    aip->ai_chance_to_use_missiles_on_plr =
        (aicp->ai_chance_to_use_missiles_on_plr[Game_skill_level] == INT_MIN)
            ? profile->chance_to_use_missiles_on_plr[Game_skill_level]
            : aicp->ai_chance_to_use_missiles_on_plr[Game_skill_level];
    aip->ai_max_aim_update_delay =
        (aicp->ai_max_aim_update_delay[Game_skill_level] == FLT_MIN)
            ? profile->max_aim_update_delay[Game_skill_level]
            : aicp->ai_max_aim_update_delay[Game_skill_level];
    aip->ai_turret_max_aim_update_delay =
        (aicp->ai_turret_max_aim_update_delay[Game_skill_level] == FLT_MIN)
            ? profile->turret_max_aim_update_delay[Game_skill_level]
            : aicp->ai_turret_max_aim_update_delay[Game_skill_level];

    // Combine AI profile and AI class flags
    aip->ai_profile_flags =
        profile->flags | (aicp->ai_profile_flags & aicp->ai_profile_flags_set);
}
...
void ai_do_default_behavior (object* obj) {
    ...
    // default behavior in most cases (especially if we're docked) is to just
    // stay put
    aip->mode = AIM_NONE;
    aip->submode_start_time = Missiontime;
    aip->active_goal = AI_GOAL_NONE;
    ...
    if (!object_is_docked (obj)) {
        ...
        if (!is_instructor (obj) && (sip->is_fighter_bomber ())) {
            int enemy_objnum = find_enemy (
                OBJ_INDEX (obj), 1000.0f,
                The_mission.ai_profile->max_attackers[Game_skill_level]);
...
#define FRIENDLY_DAMAGE_THRESHOLD \
    50.0f // Display a message at this threshold.  Note, this gets scaled by
          // Skill_level

// send the given message from objp.  called from the
// maybe_process_friendly_hit code below when a message must get send to the
// player when he fires on friendlies
void process_friendly_hit_message (int message, object* objp) {
    ...
    // check if objp is a fighter/bomber -- if not, then find a new ship to
    // send the message
    index = objp->instance;
    if (!Ship_info[Ships[objp->instance].ship_info_index]
             .is_fighter_bomber ()) {
    ...
    // Karajorma - pick a random ship to send Command messages if command is
    // silenced.
    if (index < 0 &&
        (The_mission.flags[Mission::Mission_Flags::No_builtin_command])) {
        index = ship_get_random_player_wing_ship (SHIP_GET_UNSILENCED);
    }

    if (index >= 0) {
        message_send_builtin_to_player (
            message, &Ships[index], MESSAGE_PRIORITY_HIGH,
            MESSAGE_TIME_ANYTIME, 0, 0, -1, -1);
    }
    else {
        message_send_builtin_to_player (
            message, NULL, MESSAGE_PRIORITY_HIGH, MESSAGE_TIME_ANYTIME, 0, 0,
            -1, -1);
    }
}

extern void
ship_set_subsystem_strength (ship* shipp, int type, float strength);

/**
 * Object *objp_weapon, fired by *objp_hitter, hit object *objp_ship.
 */
void maybe_process_friendly_hit (
    object* objp_hitter, object* objp_hit, object* objp_weapon) {
    // no turning traitor in multiplayer
    if (Game_mode & GM_MULTIPLAYER) { return; }

    // ditto if mission says no traitors allowed
    if (The_mission.flags[Mission::Mission_Flags::No_traitor]) { return; }

    if ((obj_team (objp_hitter) == obj_team (objp_hit)) &&
        (objp_hitter == Player_obj)) {
        // AL 12-4-97: It is possible the Player is a OBJ_GHOST at this point.
        // If so, bail out.
        if (objp_hitter->type != OBJ_SHIP) { return; }

        ASSERT (objp_hitter->type == OBJ_SHIP);
        ASSERT (objp_hit->type == OBJ_SHIP);
        ASSERT (
            (objp_weapon->type == OBJ_WEAPON) ||
            (objp_weapon->type ==
             OBJ_BEAM)); // beam added for traitor detection - FUBAR

        ship* shipp_hitter = &Ships[objp_hitter->instance];
        ship* shipp_hit = &Ships[objp_hit->instance];

        if (shipp_hitter->team != shipp_hit->team) { return; }
        ...
        if (pp->friendly_hits != 0) {
            float time_since_last_hit =
                f2fl (Missiontime - pp->friendly_last_hit_time);
            if ((time_since_last_hit >= 0.0f) &&
                (time_since_last_hit < 10000.0f)) {
                ...
                if (pp->friendly_damage < 0.0f) { pp->friendly_damage = 0.0f; }

                if (pp->friendly_hits < 0) { pp->friendly_hits = 0; }
            }
        }

        float damage; // Damage done by weapon.  Gets scaled down based on
                      // size of ship.

        if (objp_weapon->type ==
            OBJ_BEAM) // added beam for traitor detection -FUBAR
            damage =
                beam_get_ship_damage (&Beams[objp_weapon->instance], objp_hit);
        else
            damage =
                Weapon_info[Weapons[objp_weapon->instance].weapon_info_index]
                    .damage;
        ...
        // Hit ship, but not targeting it, so it's not so heinous, maybe an
        // accident.
        if (Ai_info[shipp_hitter->ai_index].target_objnum !=
            OBJ_INDEX (objp_hit)) {
            damage /= 5.0f;
        }
        ...
        float accredited_damage =
            MIN (MAX_BURST_DAMAGE, pp->damage_this_burst + damage) -
        ...
        // friendly_damage
        WARNINGF (LOCATION,"Friendly damage: %.1f, threshold: %.1f, inc damage: %.1f, max burst: %d",pp->friendly_damage,FRIENDLY_DAMAGE_THRESHOLD *(1.0f +(float)(NUM_SKILL_LEVELS + 1 - Game_skill_level) / 3.0f),pp->damage_this_burst, MAX_BURST_DAMAGE);

        if (is_instructor (objp_hit)) {
            // it's not nice to hit your instructor
            if (pp->friendly_damage > FRIENDLY_DAMAGE_THRESHOLD) {
                message_send_builtin_to_player (
                    MESSAGE_INSTRUCTOR_ATTACK, NULL, MESSAGE_PRIORITY_HIGH,
                    MESSAGE_TIME_IMMEDIATE, 0, 0, -1, -1);
                pp->last_warning_message_time = Missiontime;
                ship_set_subsystem_strength (
                    Player_ship, SUBSYSTEM_WEAPONS, 0.0f);
                ...
                gameseq_post_event (
                    GS_EVENT_PLAYER_WARPOUT_START_FORCED); // Force player to
                                                           // warp out.
            }
            else if (Missiontime - pp->last_warning_message_time > F1_0 * 4) {
                // warning every 4 sec
                // use NULL as the message sender here since it is the Terran
                // Command persona
                message_send_builtin_to_player (
                    MESSAGE_INSTRUCTOR_HIT, NULL, MESSAGE_PRIORITY_HIGH,
                    MESSAGE_TIME_IMMEDIATE, 0, 0, -1, -1);
                pp->last_warning_message_time = Missiontime;
            }
...
void maybe_set_dynamic_chase (ai_info* aip, int hitter_objnum) {
    ASSERT (
        Ship_info[Ships[aip->shipnum].ship_info_index].is_fighter_bomber ());
    ...
    if (awacs_get_level (&Objects[hitter_objnum], &Ships[aip->shipnum], 1) <
        1) {
        return;
    }

    if (aip->target_objnum != hitter_objnum) aip->aspect_locked_time = 0.0f;
    set_target_objnum (aip, hitter_objnum);
    aip->resume_goal_time =
        Missiontime + i2f (20); // Only chase up to 20 seconds.
    aip->active_goal = AI_ACTIVE_GOAL_DYNAMIC;

    set_targeted_subsys (
        aip, NULL, -1); // Say not attacking any particular subsystem.
...
// This function written so a ship with an important bomb to fire will
// willingly take hits in the face to fire its bomb.
    ...
    if (bank_index != -1) {
        if (swp->secondary_bank_weapons[bank_index] > 0) {
            if (swp->secondary_bank_ammo[bank_index] > 0) {
                if (Weapon_info[swp->secondary_bank_weapons[bank_index]]
...
// collision_normal is NULL, when a collision is imminent and we just want to
// bug out.
void big_ship_collide_recover_start (
    object* objp, object* big_objp, vec3d* collision_normal) {
    ai_info* aip;
    ...
    if (!timestamp_elapsed (aip->big_recover_timestamp) &&
        (aip->ai_flags[AI::AI_Flags::Big_ship_collide_recover_1]))
        return;
    ...
    vec3d direction;
    if (collision_normal) { direction = *collision_normal; }
    else {
    ...
    get_world_closest_box_point_with_delta (
        &aip->big_recover_pos_2, big_objp, &aip->big_recover_pos_1, NULL,
        300.0f);
...
void ai_update_lethality (object* pship_obj, object* other_obj, float damage) {
    ...
    if (Objects[parent].type == OBJ_SHIP) {
        if (Objects[parent].signature == other_obj->parent_sig) {
            ...
            if (iff_x_attacks_y (
                    Ships[pship_obj->instance].team,
                    Ships[Objects[parent].instance].team)) {
                // other is weapon
                if (other_obj->type == OBJ_WEAPON) {
                    ...
                    if (Ship_info[Ships[Objects[parent].instance]
                                      .ship_info_index]
                            .is_big_or_huge ()) {
                ...
                if (!dont_count) {
                    float lethality =
                        0.025f *
                        damage; // 2 cyclops (@2000) put you at 100 lethality

                    // increase lethality weapon's parent ship
                    ai_info* aip =
                        &Ai_info[Ships[Objects[parent].instance].ai_index];
...
/**
 * Object *objp_ship was hit by either weapon *objp_weapon or collided into by
 * ship hit_objp
 */
void ai_ship_hit (object* objp_ship, object* hit_objp, vec3d* hit_normal) {
    ...
    if (objp_ship->flags[Object::Object_Flags::Player_ship]) {
        // SUSHI: So that hitting a player ship actually resets the
        // last_hit_target_time counter for whoever hit the player. This is all
        // copypasted from code below
    ...
    if ((aip->mode == AIM_WARP_OUT) || (aip->mode == AIM_PLAY_DEAD)) return;

    if (hit_objp->type == OBJ_SHIP) {
        // If the object that this ship collided with is a big ship
        if (Ship_info[Ships[hit_objp->instance].ship_info_index]
                .is_big_or_huge ()) {
            // And the current object is _not_ a big ship
            if (!Ship_info[Ships[objp_ship->instance].ship_info_index]
                     .is_big_or_huge ()) {
                // Recover from hitting a big ship.  Note, if two big ships
                // collide, they just pound away at each other.  Oh well.
                // Recovery looks dumb and it's very late.
                big_ship_collide_recover_start (
                    objp_ship, hit_objp, hit_normal);
            }
        }
    }
    ...
    if ((hit_objp->type == OBJ_WEAPON) || (hit_objp->type == OBJ_BEAM)) {
        ...
        if (objp_hitter->type == OBJ_GHOST) return;
        ...
        maybe_process_friendly_hit (
            objp_hitter, objp_ship,
            hit_objp); // Deal with player's friendly fire.

        ship_maybe_ask_for_help (shipp);
    }
    else if (hit_objp->type == OBJ_SHIP) {
        if (shipp->team ==
            Ships[hit_objp->instance].team) // Don't have AI react to
    ...
    if ((hit_objp->type == OBJ_BEAM) &&
        (!(objp_hitter->flags[Object::Object_Flags::Player_ship])))
        return;

    // Collided into a protected ship, don't attack it.
    if (hit_objp->flags[Object::Object_Flags::Protected]) return;

    ASSERT (objp_hitter != NULL);
    hitter_aip = &Ai_info[Ships[objp_hitter->instance].ai_index];
    hitter_aip->last_hit_target_time = Missiontime;

    // store the object signature of objp_ship into ai_info, since we want to
    // track the last ship hit by 'hitter_objnum'
    ...
    if (aip->ai_flags
            [AI::AI_Flags::No_dynamic,
             AI::AI_Flags::Kamikaze]) // If not allowed to pursue dynamic
                                      // objectives, don't evade.  Dumb?  Maybe
                                      // change. -- MK, 3/15/98
        return;
    ...
    if (aip->ai_flags
        ...
        if (get_hull_pct (objp_ship) < 0.3f) {
            ...
            aip->next_rearm_request_timestamp = timestamp (
                NEXT_REARM_TIMESTAMP /
                2); // Might request again after 15 seconds.
            if (!(objp_ship->flags[Object::Object_Flags::
                                       Player_ship])) // mwa -- don't abort
                                                      // rearm for a player
    ...
    // Only ignore fire if aspect_locked_time > 0.5f, as this means we're in
    // range.
    if (firing_aspect_seeking_bomb (objp_ship)) {
        if ((aip->ai_flags[AI::AI_Flags::Seek_lock]) &&
            (aip->aspect_locked_time > 0.1f))
            return;
    }

    // If in AIM_STRAFE mode and got hit by target, maybe attack turret if
    // appropriate
    ...
    if ((objp_ship == Player_obj) &&
        !Player_use_ai) // Goober5000 - allow for exception
        return;         // We don't do AI for the player.

    maybe_update_guard_object (objp_ship, objp_hitter);

    // Big ships don't go any further.
    if (!(Ship_info[shipp->ship_info_index].is_small_ship ())) return;

    // If the hitter object is the ignore object, don't attack it.
    ship_info* sip = &Ship_info[shipp->ship_info_index];
    if (is_ignore_object (aip, OBJ_INDEX (objp_hitter)) &&
        (sip->is_fighter_bomber ())) {
        if (!object_is_docked (objp_ship)) {
            if (aip->mode == AIM_NONE) {
                aip->mode = AIM_CHASE; // This will cause the ship to move,
                                       // if not attack.
                aip->submode = SM_EVADE;
                aip->submode_start_time = Missiontime;
            }
    ...
    case AIM_CHASE:
        ...
        if ((hit_objp->type == OBJ_WEAPON) &&
            !(aip->ai_flags[AI::AI_Flags::No_dynamic])) {
            if (ai_big_maybe_enter_strafe_mode (
                    objp_ship, OBJ_INDEX (hit_objp)))
                return;
        }
        break;
    case AIM_GUARD:
        ...
        if ((aip->guard_objnum != -1) &&
            (aip->guard_signature == Objects[aip->guard_objnum].signature)) {
            if (vm_vec_dist_quick (
                    &objp_ship->pos, &Objects[aip->guard_objnum].pos) >
                500.0f) {
                return;
            }
        }
    ...
    case AIM_NONE:
    case AIM_BAY_DEPART:
    case AIM_SENTRYGUN: return;
    case AIM_BAY_EMERGE:
        // If just leaving the docking bay, don't react to enemy fire... just
        // keep flying away from docking bay
        if ((Missiontime - aip->submode_start_time) < 5 * F1_0) { return; }
        break;
    case AIM_WAYPOINTS:
        if (sip->is_fighter_bomber ())
            break;
    ...
    if (timestamp_elapsed (aip->ok_to_target_timestamp)) {
        aip->ai_flags.remove (
            AI::AI_Flags::Formation_object); // If flying in formation, bug
                                             // out!
        aip->ai_flags.remove (AI::AI_Flags::Formation_wing);
    }
    ...
    if (iff_x_attacks_y (shipp->team, Ships[objp_hitter->instance].team)) {
        if ((hitter_objnum != aip->target_objnum) &&
            (sip->is_fighter_bomber ())) {
            maybe_set_dynamic_chase (aip, hitter_objnum);
            maybe_afterburner_after_ship_hit (
                objp_ship, aip, &Objects[hitter_objnum]);
        }
        else {
            if ((aip->mode == AIM_CHASE) &&
                ai_near_full_strength (objp_ship)) {
                switch (aip->submode) {
                case SM_ATTACK:
                case SM_SUPER_ATTACK:
                case SM_GET_AWAY: break;
                default:
                    if (sip->is_fighter_bomber ()) {
                        maybe_set_dynamic_chase (aip, hitter_objnum);
                    }
...
void ai_ship_destroy (int shipnum) {
    ...
    ASSERT (
        (Ships[shipnum].ai_index >= 0) &&
        (Ships[shipnum].ai_index < MAX_AI_INFO));
    objnum = Ships[shipnum].objnum;
    dead_aip = &Ai_info[Ships[shipnum].ai_index];

    // if I was getting repaired, or awaiting repair, then cleanup the repair
    // mode.  When awaiting repair, the support objnum is -1.  When the support
    // ship is on the way, the suppoort objnum >= 0 (points to support ship).
    if (dead_aip->ai_flags
            [AI::AI_Flags::Being_repaired, AI::AI_Flags::Awaiting_repair]) {
        if (dead_aip->support_ship_objnum >= 0)
            ai_do_objects_repairing_stuff (
                &Objects[objnum], &Objects[dead_aip->support_ship_objnum],
                REPAIR_INFO_END);
        else
            ai_do_objects_repairing_stuff (
                &Objects[objnum], NULL, REPAIR_INFO_END);
    }

    // clear bay door animations
    ai_manage_bay_doors (&Objects[objnum], dead_aip, true);

    // For all objects that had this ship as a target, wipe it out, forcing
    // find of a new enemy.
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
         so = GET_NEXT (so)) {
        ...
        if (aip->target_objnum == objnum) {
            set_target_objnum (aip, -1);
            // If this ship had a dynamic goal of chasing the dead ship, clear
            // the dynamic goal.
        ...
        if ((aip->guard_wingnum != -1) &&
            (aip->guard_wingnum ==
             Ai_info[Ships[Objects[objnum].instance].ai_index].wing)) {
            if (aip->guard_wingnum != aip->wing)
                ai_set_guard_wing (other_objp, aip->guard_wingnum);
        }

        if (aip->hitter_objnum == objnum) aip->hitter_objnum = -1;
    }
}
...
void ai_deathroll_start (object* dying_objp) {
    ...
    // mark objects we are docked with so we can do damage and separate during
    // death roll
    for (dock_instance* ptr = dying_objp->dock_list; ptr != NULL;
         ptr = ptr->next) {
        object* docked_objp = ptr->docked_objp;
        int docker_index = ptr->dockpoint_used;
        int dockee_index =
            dock_find_dockpoint_used_by_object (docked_objp, dying_objp);

        dock_dead_dock_objects (
            dying_objp, docker_index, docked_objp, dockee_index);
    }
    ...
    // Undock from every object directly docked to dying_objp.  We can't just
    // iterate through the list because we're undocking the objects while we
    // iterate over them and the pointers get seriously messed up. So we just
    // repeatedly remove the first object until the dying object is no longer
    // docked to anything.
...
int ai_abort_rearm_request (object* requester_objp) {
    ...
    if (requester_objp->type != OBJ_SHIP) { return 0; }
    ASSERT (
        (requester_objp->instance >= 0) &&
        (requester_objp->instance < MAX_SHIPS));
    if ((requester_objp->instance < 0) ||
        (requester_objp->instance >= MAX_SHIPS)) {
        return 0;
    }
    requester_shipp = &Ships[requester_objp->instance];
    ASSERT (
        (requester_shipp->ai_index >= 0) &&
        (requester_shipp->ai_index < MAX_AI_INFO));
    if ((requester_shipp->ai_index < 0) ||
        (requester_shipp->ai_index >= MAX_AI_INFO)) {
        return 0;
    }
    requester_aip = &Ai_info[requester_shipp->ai_index];

    if (requester_aip->ai_flags
            [AI::AI_Flags::Being_repaired, AI::AI_Flags::Awaiting_repair]) {
        // support objnum is always valid once a rearm repair has been
        // requested.  It points to the ship that is coming to repair me.
        if (requester_aip->support_ship_objnum != -1) {
            ...
            // Make sure signatures match.  This prevents nasty bugs in which
            // an object        that was repairing another is destroyed and is
            // replaced by another ship         before this code comes around.
            if (repair_objp->signature ==
                requester_aip->support_ship_signature) {
                ASSERT (repair_objp->type == OBJ_SHIP);

                // if support ship is in the process of undocking, don't do
                // anything.
                if (repair_aip->submode < AIS_UNDOCK_0) {
                    ai_do_objects_repairing_stuff (
                        requester_objp, repair_objp, REPAIR_INFO_ABORT);
                    ...
                    WARNINGF (LOCATION,"Not aborting rearm since already undocking");
                }
            }
        }
        else {
            // setting these flags is the safe things to do.  There may not be
            // a corresponding repair ship for this guys since a repair ship
            // may be currently repairing someone else.
            ai_do_objects_repairing_stuff (
                requester_objp, NULL, REPAIR_INFO_ABORT);
        ...
        // a support ship can request to abort when he is told to do something
        // else (like warp out). see if this support ships goal_objnum is
        // valid.  If so, then issue this ai_abort comment for the ship that he
        // is enroute to repair
        if (requester_aip->goal_objnum != -1) {
            int val;

            val =
                ai_abort_rearm_request (&Objects[requester_aip->goal_objnum]);
            return val;
        }
    }
...
// function which gets called from ai-issue_rearm_request and from code in
// missionparse.cpp to actually issue the rearm goal (support_obj to rearm
// requester_obj);
void ai_add_rearm_goal (object* requester_objp, object* support_objp) {
    ...
    // if the requester is a player object, issue the order as the squadmate
    // messaging code does.  Doing so ensures that the player get a higher
    // priority!
    requester_aip->ai_flags.set (
        AI::AI_Flags::Awaiting_repair); // Tell that I'm awaiting repair.
    if (requester_objp->flags[Object::Object_Flags::Player_ship])
        ai_add_ship_goal_player (
            AIG_TYPE_PLAYER_SHIP, AI_GOAL_REARM_REPAIR, -1,
            requester_shipp->ship_name, support_aip);
    else
        ai_add_goal_ship_internal (
            support_aip, AI_GOAL_REARM_REPAIR, requester_shipp->ship_name, -1,
            -1);
}

// Object *requester_objp requests rearming.
// Returns objnum of ship coming to repair requester on success
// Success means you found someone to rearm you and you weren't previously
// rearming.
int ai_issue_rearm_request (object* requester_objp) {
    ...
    // call ship_find_repair_ship to get a support ship.  If none is found,
    // then we will warp one in.  This function will return the next available
    // ship which can repair requester
    int result = ship_find_repair_ship (requester_objp, &objp);

    // we are able to call in a ship, or a ship is warping in
    // (Arriving_support_ship may be non-NULL in either of these cases, but
    // mission_bring_in_support_ship has a check for that)
    if (result == 0 || result == 2) {
        ai_do_objects_repairing_stuff (
            requester_objp, NULL, REPAIR_INFO_QUEUE);
    ...
    // we are able to service a request
        ...
        ASSERT (objp != NULL);
        ai_do_objects_repairing_stuff (
            requester_objp, objp, REPAIR_INFO_QUEUE);
        return OBJ_INDEX (objp);
    }
 ...
 */
void ai_rearm_repair (
    object* objp, int docker_index, object* goal_objp, int dockee_index) {
    ...
    ai_dock_with_object (
        objp, docker_index, goal_objp, dockee_index, AIDO_DOCK);
    aip->ai_flags.set (
        AI::AI_Flags::Repairing); // Tell that repair guy is busy trying to
                                  // repair someone.

    goal_aip = &Ai_info[Ships[goal_objp->instance].ai_index];

    goal_aip->support_ship_objnum =
        OBJ_INDEX (objp); // Tell which object is coming to repair.
...
// Given a dockee object and the index of the dockbay for that object (ie the
// dockbay index into polymodel->dockbays[] for the model associated with the
// object), return the index of a path_num associated with than dockbay (this
...
int ai_return_path_num_from_dockbay (object* dockee_objp, int dockbay_index) {
    ...
    if (dockee_objp->type == OBJ_SHIP) {
        ...
        pm = model_get (
            Ship_info[Ships[dockee_objp->instance].ship_info_index].model_num);
        ...
        if (pm->n_docks <= dockbay_index) { return -1; }
        if (pm->docking_bays[dockbay_index].num_spline_paths <= 0) {
            return -1;
        }
        if (pm->docking_bays[dockbay_index].splines == NULL) { return -1; }
...
static void cheat_fire_synaptic (object* objp, ship* shipp) {
    ...
    if (timestamp_elapsed (swp->next_secondary_fire_stamp[current_bank])) {
        if (ship_fire_secondary (objp)) {
            WARNINGF (LOCATION, "ship %s cheat fired synaptic!", shipp->ship_name);
        ...
        if (!(strncasecmp (shipp->ship_name, NOX ("delta"), 5))) {
            ...
            if ((wing_index >= 0) && (wing_index < MAX_SHIPS_PER_WING)) {
                ...
                if (time > 0) {
                    ...
                    if ((time % modulus) < 2) {
                        int count = num_nearby_fighters (
                            iff_get_attackee_mask (obj_team (objp)),
                            &objp->pos, 1500.0f);

                        if (count > 0) cheat_fire_synaptic (objp, shipp);
                    }
                }
            }
        }
