...
#define DOA_APPROACH \
        1               // Approach the current point on the path (aip->path_cur)
...
// types of ai goals -- tyese types will help us to determination on which
// goals should have priority over others (i.e. when a player issues a goal to
// a wing, then a seperate goal to a ship in that wing).  We would probably use
// this type in conjunction with goal priority to establish which goal to
// follow
...
struct ai_goal {
        ...
        int target_name_index;  // index of goal_target_name in
                                // Goal_target_names[][]
        waypoint_list *wp_list; // waypoints that this ship might fly.
        int target_instance;    // instance of thing this ship might be chasing
                                // (currently only used for weapons; note, not the
                                // same as objnum!)
        int target_signature;   // signature of object this ship might be chasing
                                // (currently only used for weapons; paired with
                                // above value to confirm target)

        // unions for docking stuff.
        // (AIGF_DOCKER_INDEX_VALID and AIGF_DOCKEE_INDEX_VALID tell us to use
        // indexes; otherwise we use names) these are the dockpoints used on the
        // docker and dockee ships, not the ships themselves
        union {
                char *name;
                int index;
        } docker;
...
#define AIM_STILL \
        4                   // Sit still.  Don't move.  Hold your breath.  Don't blink.
...
#define AIM_BAY_EMERGE \
        17 // Emerging from a fighter bay, following path to do so
#define AIM_BAY_DEPART \
        18               // Departing to a fighter bay, following path to do so
...
struct ai_class {
        ...
        // SUSHI: These were originally in AI_Profiles, adding the option to
        // override in AI.tbl INT_MIN and FLT_MIN represent the "not set" state for
        // which defaults are used instead.
        ...
        flagset< AI::Profile_Flags >
                ai_profile_flags; // Holds the state of flags that are set
        flagset< AI::Profile_Flags >
                ai_profile_flags_set; // Holds which flags are set and which are just
                                      // left alone

        // SUSHI: These are optional overrides to an AI class to prevent the
        // automatic scaling based on AI class index
...
#define SM_BIG_CIRCLE                                                                             \
        16                 // Big ship flies circle around other big ship to get good angle to go
                           // parallel
...
#define AIS_CHASE_GLIDEATTACK                                                                           \
        18                        // Ship uses glide to move in a constant direction while pointing and
                                  // shooting at target
...
// Goober5000 (based on the "you can only remember 7 things in short-term
// memory" assumption)
...
struct ai_info {
        ...
        int stealth_last_cheat_visible_stamp; // when within 100m, always update
                                              // pos and velocity, with error
                                              // increasing for increasing time
                                              // from last legal visible
        ...
        int waypoint_speed_cap; // -1 no cap, otherwise cap - changed to int by
                                // Goober5000
        ...
        int path_subsystem_next_check; // timestamp to next check if subsystem is
                                       // still visible
        vec3d path_depart_orient;      // Rotational orientation associated with the
                                       // path
        ...
        // SUSHI: like last_predicted_enemy_pos, but for aiming (which currently
        // ignores predicted position) Unlike the predicted position stuff, also
        // takes into account velocity Only used against small ships
        ...
        flagset< AI::Profile_Flags >
                ai_profile_flags; // Holds AI_Profiles flags (possibly overriden by AI
                                  // class) that actually apply to AI
        ...
        // Goober5000
        int support_ship_objnum;    // objnum of support ship docking with us, or (if
                                    // we're a support ship) object we're docking to
        ...
        int current_target_is_locked;   // Flag to indicate whether the current
                                        // target is locked for missile fire
        int current_target_dist_trend;  // Tracks whether distance to target is
                                        // increasing or decreasing
        int current_target_speed_trend; // Tracks whether speed of target is
                                        // increasing or decreasing
        ...
        int rearm_first_missile;           // flag to show that reloading of missilies hasn't
                                           // begun yet
        int rearm_first_ballistic_primary; // flag to show that reloading of
                                           // ballistic primaries hasn't begun yet
        int rearm_release_delay;           // timestamp used to delay separation of ships
                                           // after rearm complete

        fix afterburner_stop_time;        // Missiontime to turn off afterburner
        int last_objsig_hit;              // The object number signature of the ship last hit by
                                          // this ship
        int ignore_expire_timestamp;      // Timestamp at which temporary ignore
        ...
        int scan_for_enemy_timestamp; // When to next look for enemy fighters if
        ...
        vec3d artillery_lock_pos;  // base position of the lock point on (in model's
                                   // frame of reference)
        float lethality;           // measure of how dangerous ship is to enemy BIG|HUGE
                                   // ships (likelyhood of targeting)

        flagset< AI::Maneuver_Override_Flags >
                ai_override_flags;   // flags for marking ai overrides from sexp or lua
                                     // systems
...
// SUBSYS_PATH_DIST is used as the distance that a subsystem path should
// terminate from the actual subsystem.  We don't want to rely on the model
// path points for this, since we may need to be changed.
...
#define BURST_DURATION                                                        \
        500     // decay time over which Player->damage_this_burst falls from
                // MAX_BURST_DAMAGE to 0
...
extern void
ai_ignore_object(object *ignorer, object *ignored, int ignore_new);
extern void ai_ignore_wing(object *ignorer, int wingnum);
extern void ai_dock_with_object(
...
extern void
ai_start_waypoints(object *objp, waypoint_list *wp_list, int wp_flags);
extern void
ai_ship_hit(object *objp_ship, object *hit_objp, vec3d *hit_normal);
extern void ai_ship_destroy(int shipnum);
extern void ai_turn_towards_vector(
        vec3d *dest, object *objp, float turn_time, vec3d *slide_vec,
        vec3d *rel_pos, float bank_override, int flags, vec3d *rvec = nullptr,
        int sexp_flags = 0);
...
extern int ai_issue_rearm_request(
        object *requester_objp); // Object requests rearm/repair.
extern int
ai_abort_rearm_request(object *requester_objp); // Object aborts rearm/repair.
extern void ai_do_repair_frame(
        object *objp, ai_info *aip,
        float frametime); // Repair a ship object, player or AI.
extern void ai_update_danger_weapon(int objnum, int weapon_objnum);

// called externally from MissionParse.cpp to position ships in wings upon
// arrival into the mission.
extern void get_absolute_wing_pos(
        vec3d *result_pos, object *leader_objp, int wing_index,
        int formation_object_flag);
extern void get_absolute_wing_pos_autopilot(
        vec3d *result_pos, object *leader_objp, int wing_index,
        int formation_object_flag);

// Interface from goals code to AI.  Set ship to guard.  *objp guards
//*other_objp
...
extern void
ai_fly_in_formation(int wing_num); // Force wing to fly in formation.
extern void
...
extern ship_subsys *
set_targeted_subsys(ai_info *aip, ship_subsys *new_subsys, int parent_objnum);
extern void ai_rearm_repair(
...
extern void create_model_path(
        object *pl_objp, object *mobjp, int path_num, int subsys_path = 0);
extern int ai_find_goal_index(
        ai_goal *aigp, int mode, int submode = -1, int priority = -1);
...
extern void ai_good_secondary_time(
        int team, int weapon_index, int num_weapons, char *shipname);

extern void ai_do_objects_docked_stuff(
...
extern void ai_do_objects_repairing_stuff(
        object *repaired_obj, object *repair_obj, int how);
...
extern float dock_orient_and_approach(
        ...
        int dockee_index, int dock_mode, rotating_dockpoint_info *rdinfo = NULL);

extern int find_danger_weapon(
        object *sobjp, float dtime, float *atime, float dot_threshhold);
