...
int valid_turret_enemy(object *objp, object *turret_parent)
{
        ...
        if (objp->type == OBJ_WEAPON) {
                ...
                if (wip->subtype == WP_LASER && !(wip->wi_flags[Weapon::Info_Flags::Turret_Interceptable])) { // If the thing can't
                                                                                                              // be shot down,
                                                                                                              // don't try.
                                                                                                              // -MageKing17
                        return 0;
...
void evaluate_obj_as_target(object *objp, eval_enemy_obj_struct *eeo)
{
        ...
        if (objp->type == OBJ_SHIP) {
                ...
                if (eeo->eeo_flags & EEOF_TAGGED_ONLY) {
                        if (!ship_is_tagged(objp) && ((The_mission.ai_profile->flags[AI::Profile_Flags::Strict_turred_tagged_only_targeting]) || (!(objp->type == OBJ_WEAPON) && !(turret_weapon_has_flags(&eeo->turret_subsys->weapons, Weapon::Info_Flags::Spawn))))) {
                                return;
                        }
                }
        ...
        bool flak_flag, bool laser_flag, bool missile_flag)
{
        ...
        if (n_tgt_priorities > 0) {
            ...
            */
                        // if we got something...
                        if ((eeo.nearest_homing_bomb_objnum != -1) || (eeo.nearest_bomb_objnum != -1) || (eeo.nearest_attacker_objnum != -1) || (eeo.nearest_objnum != -1)) {
                ...
                for (int i = 0; i < NUM_TURRET_ORDER_TYPES; i++) {
                        ai_info *aip = &Ai_info[Ships[Objects[eeo.turret_parent_objnum].instance]
                                                        .ai_index];
                        ...
                        case 1:
                                ...
                                ASSERT(
                                        eeo.nearest_attacker_objnum < 0 || is_target_beam_valid(swp, &Objects[eeo.nearest_attacker_objnum]));
        ...
        int current_enemy)
{
        ...
        bool tagged_only_flag = all_turret_weapons_have_flags(
        ...
        if ((Ship_types[sip->class_type]
            ...
            && (aip->target_objnum != -1)) {
                ...
                if ((fs2::prng::randf(0) < 0.8f) && (aip->target_objnum != -1) && Use_parent_target) {
                        ...
                        if (target_flags[Object::Object_Flags::Beam_protected] && beam_flag)
                                ...
                                target_flags[Object::Object_Flags::Flak_protected] && flak_flag)
                                skip = true;
                        else if (
                                target_flags[Object::Object_Flags::Laser_protected] && laser_flag)
                                skip = true;
                        else if (
                                target_flags[Object::Object_Flags::Missile_protected] && missile_flag)
                                skip = true;
        ...
        if (use_angles) {
                ...
                if (targetp == nullptr) {
                        ...
                        set_predicted_enemy_pos_turret(
                                ...
                                ssp->turret_time_enemy_in_range * (weapon_system_strength + 1.0f) / 2.0f);
                } else {
                        tmp_target = *targetp;
                }
...
void turret_ai_update_aim(ai_info *aip, object *En_Objp, ship_subsys *ss)
{
        if (Missiontime >= ss->next_aim_pos_time) {
                ...
                ss->next_aim_pos_time = Missiontime + fl2f(fs2::prng::randf(0, 0.0f, aip->ai_turret_max_aim_update_delay));
        } else {
        ...
        vec3d *predicted_enemy_pos, vec3d *gvec)
{
        if (ss->turret_enemy_objnum != -1) {
                ...
                if (in_fov) {
                        ...
                        (tp->flags[Model::Subsystem_Flags::Turret_reset_idle]) && (timestamp_elapsed(ss->rotation_timestamp))) {
                        model_rotate_gun(
                                Ship_info[shipp->ship_info_index].model_num, tp,
                                &Objects[parent_objnum].orient, &ss->submodel_info_1.angs,
                                &ss->submodel_info_2.angs, &Objects[parent_objnum].pos,
                ...
                (ss->system_info->flags[Model::Subsystem_Flags::Turret_reset_idle]) && (timestamp_elapsed(ss->rotation_timestamp))) {
                model_rotate_gun(
        ...
        float flak_range_override = 100.0f)
{
        ...
        if (turret->turret_enemy_objnum >= 0) {
                ...
                // weapon is type flak
                        ...
                        (wip->wi_flags[Weapon::Info_Flags::Flak]) && (tobjp->flags[Object::Object_Flags::Flak_protected])) {
                        ASSERT(0);
                        return 0;
                }
                ...
                // weapon is type laser
                        ...
                        (wip->subtype == WP_LASER) && (tobjp->flags[Object::Object_Flags::Laser_protected])) {
                        ASSERT(0);
                        return 0;
                }
                ...
                // weapon is type missile
                        ...
                        (wip->subtype == WP_MISSILE) && (tobjp->flags[Object::Object_Flags::Missile_protected])) {
                        ASSERT(0);
                        return 0;
                }
        }
        ...
        if (check_ok_to_fire(parent_objnum, turret->turret_enemy_objnum, wip)) {
                ...
                // don't fire swam, but set up swarm info instead
                        ...
                        (wip->wi_flags[Weapon::Info_Flags::Swarm]) || (wip->wi_flags[Weapon::Info_Flags::Corkscrew])) {
                ...
                // now do anything else
                        ...
                        for (int i = 0; i < wip->shots; i++) {
                                if (turret->system_info
                                        ...
                                        swp = &turret->weapons;
                                        int bank_to_fire,
                                                num_slots = turret->system_info->turret_num_firing_points;
...
void turret_swarm_fire_from_turret(turret_swarm_info *tsi)
{
        ...
        ASSERT(
                (Weapon_info[tsi->weapon_class].wi_flags[Weapon::Info_Flags::Swarm]) || (Weapon_info[tsi->weapon_class].wi_flags[Weapon::Info_Flags::Corkscrew]));
        ...
        if ((Weapon_info[tsi->weapon_class].subtype == WP_LASER) || ((The_mission.ai_profile->flags[AI::Profile_Flags::Hack_improve_non_homing_swarm_turret_fire_accuracy]) && !(Weapon_info[tsi->weapon_class].is_homing()))) {
...
void ai_fire_from_turret(ship *shipp, ship_subsys *ss, int parent_objnum)
{
        ...
        if (lep != NULL) {
                if (The_mission.ai_profile
                        ...
                        dist_to_enemy = MAX(
                                0, vm_vec_normalized_dir(&v2e, &predicted_enemy_pos, &gpos) - lep->radius);
        }
        ...
        for (i = 0; i < (MAX_SHIP_WEAPONS); i++) {
                ...
                if ((wip->wi_flags[Weapon::Info_Flags::Spawn]) && !(wip->wi_flags[Weapon::Info_Flags::Smart_spawn])) {
                        if ((num_ships_nearby >= 3) || ((num_ships_nearby >= 2) && (fs2::prng::randf(0) < 0.1f))) {
                                turret_fire_weapon(
                                        i, ss, parent_objnum, &gpos,
                                        &ss->turret_last_fire_direction);
                        } else {
                                ss->turret_next_fire_stamp = timestamp(1000); // Regardless of firing rate, don't
                        ...
                        if ((dist_to_enemy <= weapon_firing_range) && (dist_to_enemy >= WeaponMinRange)) {
                                ...
                                if (lep->type == OBJ_SHIP) {
                                        ...
                                        // flak weapon
                                                ...
                                                (lep->flags[Object::Object_Flags::Flak_protected]) && (wip->wi_flags[Weapon::Info_Flags::Flak])) {
                                                ss->turret_enemy_objnum = -1;
                                                ss->turret_time_enemy_in_range = 0.0f;
                                                continue;
                                        }
                                        ...
                                        // laser weapon
                                                ...
                                                (lep->flags[Object::Object_Flags::Laser_protected]) && (wip->subtype == WP_LASER)) {
                                                ss->turret_enemy_objnum = -1;
                                                ss->turret_time_enemy_in_range = 0.0f;
                                                continue;
                                        }
        ...
        if (turret_should_pick_new_target(ss) && !ss->scripting_target_override) {
                ...
                if (objnum >= 0 && is_target_beam_valid(&ss->weapons, &Objects[objnum])) {
                        if (ss->turret_enemy_objnum == -1) {
                                ...
                                ss->targeted_subsys = NULL; // Turret has retargeted; reset subsystem - Valathil
                                                            // for Mantis #2652
        ...
        if (in_fov) {
                ...
                for (i = 0; i < number_of_firings; i++) {
                        ...
                        if (!(wip->is_homing())) {
                                ...
                                // in line of sight
                                if (turret_should_fire_aspect(ss, wip, in_sight) && ship_get_closest_subsys_in_sight(&Ships[lep->signature], SUBSYSTEM_ENGINE, &gpos)) {
                                        ok_to_fire = true;
                                }
                        }
                        ...
                        if (ok_to_fire) {
                                ...
                                if (ss->turret_animation_position == MA_POS_NOT_SET) {
                                        if (model_anim_start_type(
                                                ...
                                                ss->turret_animation_done_time = model_anim_get_time_type(
                                                        shipp, TRIGGER_TYPE_TURRET_FIRING,
                        ...
                        if (ok_to_fire) {
                                ...
                                if ((tp->flags[Model::Subsystem_Flags::Turret_salvo]) && ((i + 1) == number_of_firings)) {
                                        ai_info *parent_aip = &Ai_info[Ships[Objects[parent_objnum].instance]
                                                                               .ai_index];
                                        turret_set_next_fire_timestamp(
                ...
                if (!something_was_ok_to_fire) {
                        ...
                        for (i = 0; i < MAX_SHIP_WEAPONS; i++) {
                                if (i < MAX_SHIP_PRIMARY_BANKS) {
                                        ...
                                        if (swp->next_primary_fire_stamp[i] < 2)
                                                continue;
                                        ...
                                        if (swp->next_secondary_fire_stamp
                                            ...
                                            < ss->turret_next_fire_stamp) {
                                                ss->turret_next_fire_stamp = swp->next_secondary_fire_stamp
                                                                                     [i - MAX_SHIP_PRIMARY_BANKS];
                                        }
                                }
                        }
