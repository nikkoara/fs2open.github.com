...
public:
    ...
    int max_incoming_asteroids[NUM_SKILL_LEVELS];    // max number of asteroids
                                                     // thrown at friendlies
    int max_allowed_player_homers[NUM_SKILL_LEVELS]; // max number of
                                                     // simultaneous homing
                                                     // weapons on player
    int max_attackers[NUM_SKILL_LEVELS]; // max number of ships that can attack
                                         // a ship
    fix predict_position_delay[NUM_SKILL_LEVELS]; // how long until AI predicts
                                                  // a ship position
    float in_range_time[NUM_SKILL_LEVELS]; // seconds to add to time it takes
                                           // to get enemy in range (only for
                                           // player's enemies)
    float shield_manage_delay[NUM_SKILL_LEVELS]; // how long before AI manages
                                                 // shields (note that the
                                                 // player's team always uses
                                                 // the average skill's delay)
    ...
    float link_energy_levels_maybe[NUM_SKILL_LEVELS];  // link if hull strength
                                                       // low
    ...
    float
        link_ammo_levels_maybe[NUM_SKILL_LEVELS]; // link if hull strength low
    float primary_ammo_burst_mult
        [NUM_SKILL_LEVELS]; // SUSHI: Multiplier adjusting burst frequency for
                            // ballistic primary weapons

    // difficulty-related scales
    float cmeasure_life_scale[NUM_SKILL_LEVELS]; // life of countermeasures
    float cmeasure_fire_chance
        [NUM_SKILL_LEVELS]; // chance a countermeasure will be fired based on
                            // skill level (also scaled by ai_class)
    ...
    float afterburner_recharge_scale[NUM_SKILL_LEVELS]; // speed of afterburner
                                                        // recharge
    float
        player_damage_scale[NUM_SKILL_LEVELS]; // damage applied to the player

    float subsys_damage_scale[NUM_SKILL_LEVELS]; // damage applied to a player
                                                 // subsystem
    float beam_friendly_damage_cap[NUM_SKILL_LEVELS]; // damage cap values for
                                                      // friendly beam fire
    float turn_time_scale[NUM_SKILL_LEVELS]; // speed at which enemy ships turn
    float glide_attack_percent[NUM_SKILL_LEVELS]; // SUSHI: The likelihood
                                                  // (0.0-1.0) of the AI to use
                                                  // the "glide attack" move
    float
        circle_strafe_percent[NUM_SKILL_LEVELS]; // SUSHI: The likelihood
                                                 // (0.0-1.0) of the AI to use
                                                 // the "circle strafe" move
    float
        glide_strafe_percent[NUM_SKILL_LEVELS]; // SUSHI: The likelihood
                                                // (0.0-1.0) of the AI to use
                                                // glide when strafing capships
    float random_sidethrust_percent
        [NUM_SKILL_LEVELS]; // SUSHI: The likelihood (0.0-1.0) of the AI to
                            // randomly use sidethrust while dogfihthing
    float stalemate_time_thresh
        [NUM_SKILL_LEVELS]; // SUSHI: The amount of time required for the AI to
                            // detect (and try to break) dogfight stalemate
    float
        stalemate_dist_thresh[NUM_SKILL_LEVELS]; // SUSHI: The maximum distance
                                                 // the AI and target must be
                                                 // within for a stalemate
    float
        max_aim_update_delay[NUM_SKILL_LEVELS]; // SUSHI: The maximum delay
                                                // before the AI updates their
                                                // aim against small ships
    float turret_max_aim_update_delay[NUM_SKILL_LEVELS]; // SUSHI: As above,
                                                         // but for turrets
                                                         // updating their aim
    ...
    // the number of points subtracted from score for a rearm started on a
    // player
    ...
    float detail_distance_mult[MAX_DETAIL_LEVEL + 1]; // MAX_DETAIL_LEVEL
                                                      // really needs to be 4
...
#endif // FREESPACE2_AI_AI_PROFILES_HH
