...
bool Sel_NextNav () {
    ...
    if (CurrentNav == -1) {
        for (i = 0; i < MAX_NAVPOINTS; i++) {
            if (Navs[i].flags & NP_VALIDTYPE &&
                !(Navs[i].flags & NP_NOSELECT)) {
                CurrentNav = i;
                return true;
            }
        }
        ...
        for (i = CurrentNav + 1; i < MAX_NAVPOINTS + CurrentNav; i++) {
            if (Navs[i % MAX_NAVPOINTS].flags & NP_VALIDTYPE &&
                !(Navs[i % MAX_NAVPOINTS].flags & NP_NOSELECT)) {
                if (i != CurrentNav) {
...
char* NavPoint::GetInternalName () {
    ...
    if (flags & NP_WAYPOINT) {
        NavName =
            new char[strlen (((waypoint_list*)target_obj)->get_name ()) + 5];
        memset (
            NavName, 0,
            strlen (((waypoint_list*)target_obj)->get_name ()) + 5);
...
bool CanAutopilot (vec3d targetPos, bool send_msg) {
    if (CurrentNav == -1) {
        if (send_msg) send_autopilot_msgID (NP_MSG_FAIL_NOSEL);
        return false;
    }

    if (object_get_gliding (Player_obj)) {
        if (send_msg) send_autopilot_msgID (NP_MSG_FAIL_GLIDING);
        return false;
    }

    // You cannot autopilot if you're within 1000 meters of your destination
    // nav point
    if (vm_vec_dist_quick (&targetPos, Navs[CurrentNav].GetPosition ()) <
        1000) {
        if (send_msg) send_autopilot_msgID (NP_MSG_FAIL_TOCLOSE);
        return false;
    }
    ...
    if (AutopilotMinEnemyDistance > 0) {
        ...
        for (ship_obj* so = GET_FIRST (&Ship_obj_list);
            ...
            // attacks player?
            if (iff_x_attacks_y (
                    obj_team (other_objp), obj_team (Player_obj)) &&
                !(Ship_info[Ships[other_objp->instance].ship_info_index]
                      .flags[Ship::Info_Flags::Cargo])) // ignore cargo
            {
                // Cannot autopilot if enemy within AutopilotMinEnemyDistance
                // meters
                if (vm_vec_dist_quick (&targetPos, &other_objp->pos) <
                    AutopilotMinEnemyDistance) {
                    if (send_msg) send_autopilot_msgID (NP_MSG_FAIL_HOSTILES);
                    return false;
                }
            }
        }
    ...
    if (AutopilotMinAsteroidDistance > 0) {
        ...
        for (int n = 0; n < MAX_ASTEROIDS; n++) {
            ...
            if (Asteroids[n].flags & AF_USED) {
                // Cannot autopilot if asteroid within
                // AutopilotMinAsteroidDistance meters
                if (vm_vec_dist_quick (
                        &targetPos, &Objects[Asteroids[n].objnum].pos) <
                    AutopilotMinAsteroidDistance) {
                    if (send_msg) send_autopilot_msgID (NP_MSG_FAIL_HAZARD);
                    return false;
                }
            }
        }
    ...
    if (ship_find_repair_ship (Player_obj) != 0) {
        if (send_msg) send_autopilot_msgID (NP_MSG_FAIL_SUPPORT_PRESENT);
        return false;
    }
...
bool StartAutopilot () {
    ...
    for (object* objp = GET_FIRST (&obj_used_list);
        ...
        if ((objp->type == OBJ_SHIP) &&
            ...
            if (ai_find_goal_index (
                    support_ship_aip->goals, AI_GOAL_REARM_REPAIR) == -1) {
                // no, so tell it to depart
                ai_add_ship_goal_player (
                    AIG_TYPE_PLAYER_SHIP, AI_GOAL_WARP, -1, NULL,
                    support_ship_aip);
            }
            else {
                // yes
    ...
    for (i = 0; i < MAX_SHIPS; i++) {
        if (Ships[i].objnum != -1 &&
            (Ships[i].flags[Ship::Ship_Flags::Navpoint_carry] ||
             (Ships[i].wingnum != -1 &&
              Wings[Ships[i].wingnum].flags[Ship::Wing_Flags::Nav_carry]))) {
            if (speed_cap >
                vm_vec_mag (&Ship_info[Ships[i].ship_info_index].max_vel))
                speed_cap =
                    vm_vec_mag (&Ship_info[Ships[i].ship_info_index].max_vel);
        }
    }
    ...
    // assign ship goals
    // when assigning goals to individual ships only do so if
    // Ships[shipnum].wingnum != -1 we will assign wing goals below
    ...
    for (i = 0; i < MAX_SHIPS; i++) {
        if (Ships[i].objnum != -1 &&
            ...
            if (Ship_info[Ships[i].ship_info_index].is_big_ship () ||
                Ship_info[Ships[i].ship_info_index]
                    .flags[Ship::Info_Flags::Capital] ||
                Ship_info[Ships[i].ship_info_index]
                ...
                else if (Ship_info[Ships[i].ship_info_index]
            ...
            /*if (!vm_vec_cmp(&rpos, &Player_obj->pos))
                // want to make sure rpos isn't player pos - we can worry about
            it being largest object's later
            {
            ...
            if (The_mission
                    .flags[Mission::Mission_Flags::
                               Use_ap_cinematics]) { // instantly turn the ship
                                                     // to match the direction
                                                     // player is looking
                // vm_vec_sub(&norm1, Navs[CurrentNav].GetPosition(),
                // &Player_obj->pos);
                vm_vector_2_matrix (
                    &Objects[Ships[i].objnum].orient, &norm1, NULL, NULL);
            }

            // snap wings into formation
            if (The_mission.flags[Mission::Mission_Flags::
                                      Use_ap_cinematics] && // only if using
                                                            // cinematics
                (Ships[i].wingnum != -1 &&
                 Wings[Ships[i].wingnum]
                     .flags[Ship::Wing_Flags::Nav_carry]) // only if in a wing
                && Autopilot_flight_leader !=
                       &Objects[Ships[i].objnum]) // only if not flight
                                                  // leader's object
            {
                ai_info* aip = &Ai_info[Ships[i].ai_index];
                int wingnum = aip->wing,
                    wing_index =
                        get_wing_index (&Objects[Ships[i].objnum], wingnum);
                vec3d goal_point;
                ...
                if (leader_objp != &Objects[Ships[i].objnum]) {
                    ...
                    case 1: // back-left
                        vm_vec_add (
                            &perp, &zero,
                            &Autopilot_flight_leader->orient.vec.rvec);
                        // vm_vec_sub(&perp, &perp,
                        // &Player_obj->orient.vec.fvec);
                ...
                if (vm_vec_dist_quick (
                        ...
                        &Objects[Ships[i].objnum].pos) > distance) {
                    distance = vm_vec_dist_quick (
                        &Autopilot_flight_leader->pos,
                        &Objects[Ships[i].objnum].pos);
                }
            }
            ...
            if (Ships[i].wingnum == -1 ||
                ...
                if (Navs[CurrentNav].flags & NP_WAYPOINT) {
                    ai_add_ship_goal_player (
                        AIG_TYPE_PLAYER_SHIP, AI_GOAL_WAYPOINTS_ONCE, 0,
                        ((waypoint_list*)Navs[CurrentNav].target_obj)
                            ->get_name (),
                    ...
                    ai_add_ship_goal_player (
                        AIG_TYPE_PLAYER_SHIP, AI_GOAL_FLY_TO_SHIP, 0,
                        ((ship*)Navs[CurrentNav].target_obj)->ship_name,
                        &Ai_info[Ships[i].ai_index]);
                }
            }
        }
    ...
    if (!(The_mission.flags[Mission::Mission_Flags::Use_ap_cinematics])) {
        for (i = 0; i < MAX_WINGS; i++) {
            if (Wings[i].flags[Ship::Wing_Flags::Nav_carry]) {
                // ai_add_ship_goal_player( int type, int mode, int submode,
                // char *shipname, ai_info *aip );

                // ai_add_wing_goal_player( AIG_TYPE_PLAYER_WING,
                // AI_GOAL_STAY_NEAR_SHIP, 0, target_shipname, wingnum );
                // ai_add_wing_goal_player( AIG_TYPE_PLAYER_WING,
                // AI_GOAL_WAYPOINTS_ONCE, 0, target_shipname, wingnum );
                // ai_clear_ship_goals( &(Ai_info[Ships[num].ai_index]) );

                ai_clear_wing_goals (i);
                if (Navs[CurrentNav].flags & NP_WAYPOINT) {
                    ai_add_wing_goal_player (
                        AIG_TYPE_PLAYER_WING, AI_GOAL_WAYPOINTS_ONCE, 0,
                        ((waypoint_list*)Navs[CurrentNav].target_obj)
                            ->get_name (),
                        i);
                    ...
                    ai_add_wing_goal_player (
                        AIG_TYPE_PLAYER_WING, AI_GOAL_FLY_TO_SHIP, 0,
                        ((ship*)Navs[CurrentNav].target_obj)->ship_name, i);
                }
            }
        }
    }

    // fixup has to go down here because ships are assigned goals during wing
    // goals as well
    for (i = 0; i < MAX_SHIPS; i++) {
        if (Ships[i].objnum != -1) {
            if (Ships[i].flags[Ship::Ship_Flags::Navpoint_carry] ||
                (Ships[i].wingnum != -1 &&
                 Wings[Ships[i].wingnum].flags[Ship::Wing_Flags::Nav_carry]))
                for (j = 0; j < MAX_AI_GOALS; j++) {
                    if (Ai_info[Ships[i].ai_index].goals[j].ai_mode ==
                            AI_GOAL_WAYPOINTS_ONCE ||
                        Ai_info[Ships[i].ai_index].goals[j].ai_mode ==
                            AIM_WAYPOINTS) {
                        autopilot_ai_waypoint_goal_fixup (
                            &(Ai_info[Ships[i].ai_index].goals[j]));

                        // formation fixup
                        // ai_form_on_wing(&Objects[Ships[i].objnum],
                        // &Objects[Player_ship->objnum]);
                    }
                }
        }
    }
    ...
    if (The_mission.flags[Mission::Mission_Flags::Use_ap_cinematics]) {
        if (capshipPresent) {
            ...
            for (std::vector< int >::iterator idx = capIndexes.begin ();
                 idx != capIndexes.end (); ++idx) {
                vm_vec_add (
                    &right, &Autopilot_flight_leader->orient.vec.rvec, &zero);
                vm_vec_add (
                    &front, &Autopilot_flight_leader->orient.vec.fvec, &zero);
                vm_vec_add (
                    &up, &Autopilot_flight_leader->orient.vec.uvec, &zero);
                vm_vec_add (&offset, &zero, &zero);
                if (Ship_info[Ships[*idx].ship_info_index]
                        .flags[Ship::Info_Flags::Capital] ||
                    Ship_info[Ships[*idx].ship_info_index]
                        .flags[Ship::Info_Flags::Supercap]) {
                    ...
                    // position below
                    vm_vec_scale (
                        &up, capship_spreads[0]); // scale the up vector by the
                                                  // radius of the largest ship
                                                  // in this formation part
                    ...
                    vm_vec_scale (
                        &right,
                        (1 + ((float)floor ((float)capship_placed[0] / 3))));
                    vm_vec_scale (
                        &up,
                        -(1 + ((float)floor ((float)capship_placed[0] / 3))));
                ...
                else if (Ship_info[Ships[*idx].ship_info_index]
                             .flags[Ship::Info_Flags::Corvette]) {
                    ...
                    // position below
                    vm_vec_scale (
                        &up, capship_spreads[1]); // scale the up vector by the
                                                  // radius of the largest ship
                                                  // in this formation part
                    ...
                    vm_vec_scale (
                        &right,
                        (1 + ((float)floor ((float)capship_placed[1] / 3))));
                    vm_vec_scale (
                        &up,
                        (1 + ((float)floor ((float)capship_placed[1] / 3))));

                    // move ourselves up and out of the way of the smaller
                    // ships
                    vm_vec_add (
                        &perp, &Autopilot_flight_leader->orient.vec.uvec,
                        &zero);
                    ...
                    // 2 either side - 6 lines of position (right (dir, front,
                    // back), left (dir, front, back) )
                    // placing pattern: right, left, front right, front left,
                    ...
                    vm_vec_scale (
                        &right, 2 * (1 + ((float)floor (
                                             (float)capship_placed[2] / 3))));
                    vm_vec_scale (
                        &front, 2 * (1 + ((float)floor (
                                             (float)capship_placed[2] / 3))));

                    // move "out" by 166*(wcount-1) so we don't bump into
                    // fighters
                    vm_vec_add (
                ...
                vm_vec_add (
                    &Objects[Ships[*idx].objnum].pos,
                    &Autopilot_flight_leader->pos, &offset);

                if (vm_vec_dist_quick (
                        &Autopilot_flight_leader->pos,
                        &Objects[Ships[*idx].objnum].pos) > distance) {
                    distance = vm_vec_dist_quick (
                        &Autopilot_flight_leader->pos,
                        &Objects[Ships[*idx].objnum].pos);
                }
        ...
        ftemp = floor (
            Autopilot_flight_leader->phys_info.max_vel.xyz.z / speed_cap);
        ...
        vm_vec_normalize (&pos); // pos is now a unit vector in the direction
                                 // we will be moving the camera
        // norm1 = pos;
        vm_vec_scale (
            &pos, 5 * speed_cap * tc_factor); // pos is now scaled by 5 times
                                              // the speed (5 seconds ahead)
        vm_vec_add (
            &pos, &pos,
            &Autopilot_flight_leader
                 ->pos); // pos is now 5*speed cap in front of player ship

        switch (myrand () % 24)
        // 8 different ways of getting perp points
        // 4 of which will not be used when capships are present (anything
        // below, or straight above)
        {
        case 1: // down
        case 9:
        case 16:
            if (capship_placed[0] == 0)
                vm_vec_sub (
                    &perp, &zero, &Autopilot_flight_leader->orient.vec.uvec);
            else { // become up-left
                vm_vec_add (
                    &perp, &zero, &Autopilot_flight_leader->orient.vec.uvec);
                vm_vec_sub (
                    &perp, &perp, &Autopilot_flight_leader->orient.vec.rvec);
            }
            break;
        ...
        case 23:
            vm_vec_add (
                &perp, &perp, &Autopilot_flight_leader->orient.vec.uvec);
        ...
        case 22:
            vm_vec_sub (
                &perp, &zero, &Autopilot_flight_leader->orient.vec.rvec);
            break;
        ...
        case 0: // right
        case 8:
        case 17: perp = Autopilot_flight_leader->orient.vec.rvec; break;
        }
        vm_vec_normalize (&perp);
        ...
        if (capshipPresent) {
            ...
            // place it behind
            // vm_vec_copy_scale(&norm1, &Player_obj->orient.vec.fvec,
            // -2*(Player_obj->radius+radius*(1.0f+(float(j)/100.0f))));
            // vm_vec_add(&cameraTarget, &cameraTarget, &norm1);

            vm_vec_copy_scale (&cameraTarget, &perp, radius / 5.0f);

            // vm_vec_scale(&cameraTarget,
            // Player_obj->radius+radius*(1.0f+(float(j)/100.0f)));
        ...
        EndAPCinematic = timestamp (
            (10000 * tc_factor) +
            NPS_TICKRATE); // 10 objective seconds before end of cinematic
...
void EndAutoPilot () {
    ...
    {
        if (UseCutsceneBars) { Cutscene_bar_flags &= ~CUB_CUTSCENE; }
        cam_reset_camera ();
...

    {
        if (Ships[i].objnum != -1 &&
            ...
            Ai_info[Ships[i].ai_index].waypoint_speed_cap =
                -1; // uncap their speed
            Ai_info[Ships[i].ai_index].mode =
                AIM_NONE; // make AI re-evaluate current ship mode
...
camera* nav_get_set_camera () {
    ...
    if (!nav_camera.isValid ()) { nav_camera = cam_create ("Nav camera"); }
...
void nav_warp (bool prewarp = false) {
    ...
    vm_vec_sub (
        &pos, Navs[CurrentNav].GetPosition (), &Autopilot_flight_leader->pos);
    ...
    while (CanAutopilot (tpos)) { vm_vec_add (&tpos, &tpos, &pos); }
    ...
    vm_vec_scale (
        &velocity,
        (float)Ai_info[Ships[Autopilot_flight_leader->instance].ai_index]
            .waypoint_speed_cap);

    // Find all ships that are supposed to autopilot with the player and move
    // them to the cinimatic location or the final destination
        ...
        if (Ships[i].objnum != -1 &&
            (Ships[i].flags[Ship::Ship_Flags::Navpoint_carry] ||
             (Ships[i].wingnum != -1 &&
              Wings[Ships[i].wingnum].flags[Ship::Wing_Flags::Nav_carry]))) {
            vm_vec_add (
                &Objects[Ships[i].objnum].pos, &Objects[Ships[i].objnum].pos,
                &targetPos);
    ...
    if (Cmdline_old_collision_sys) { obj_all_collisions_retime (); }
    else {
...
void NavSystem_Do () {
        ...
        if (AutoPilotEngaged) {
            if (The_mission.flags[Mission::Mission_Flags::Use_ap_cinematics]) {
                ...
                if (CinematicStarted) {
                    ...
                    if (timestamp () >= MoveCamera && !CameraMoving &&
                        vm_vec_mag (&cameraTarget) > 0.0f) {
                        // Free_camera->set_position(&cameraTarget,
                        // float(camMovingTime), float(camMovingTime)/2.0f);
    ...
    if (AutoPilotEngaged &&
        ...
        if (dstfrm_start < (3500 * ramp_bias)) {
            if (dstfrm_start >= (3000 * ramp_bias) &&
                DistanceTo (CurrentNav) > 30000)
    ...
    are where they're supposed to be at all times is like herding cats" -Woolie
    Wool
    */
    for (int i = 0; i < MAX_SHIPS; i++) {
        if (Ships[i].objnum != -1 &&
            Ships[i].flags[Ship::Ship_Flags::Navpoint_needslink]) {
            object* other_objp = &Objects[Ships[i].objnum];

            if (vm_vec_dist_quick (&Player_obj->pos, &other_objp->pos) <
                (NavLinkDistance + other_objp->radius)) {
...
void send_autopilot_msgID (int msgid) {
    if (msgid < 0 || msgid >= NP_NUM_MESSAGES) return;
...
void send_autopilot_msg (char* msg, char* snd) {
    ...
    if (audio_handle != -1) {
        audiostream_play (
            audio_handle, (Master_event_music_volume * aav_music_volume), 0);
    }

    if (msg[0] != '\0' && strcmp (msg, "none") != 0)
        message_training_queue (
            "autopilot builtin message", timestamp (0),
            5); // display message for five seconds
}

// ********************************************************************************************
// Inits the Nav System
void NavSystem_Init () {
    for (int i = 0; i < MAX_NAVPOINTS; i++) Navs[i].clear ();
...
void parse_autopilot_table (const char* filename) {
    ...
    try {
        ...
        if (optional_string ("+No_Cutscene_Bars")) UseCutsceneBars = false;
        // optional no cutscene bars
...
bool Nav_Alt_Flags (char* Nav, int flags) {
    flags &= ~NP_VALIDTYPE; // clear the NP_SHIP and NP_WAYPOINT bits, make
                            // sure they haven't been set

    int nav = FindNav (Nav);

    if (nav != -1) {
        Navs[nav].flags &=
            NP_VALIDTYPE;         // Clear all bits BUT NP_SHIP or NO_WAYPOINT
...
//+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
// Named
bool Nav_Set_Hidden (char* Nav) { return Nav_Set_Flag (Nav, NP_HIDDEN); }

//+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+

bool Nav_Set_NoAccess (char* Nav) { return Nav_Set_Flag (Nav, NP_NOACCESS); }

//+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+

bool Nav_Set_Visited (char* Nav) { return Nav_Set_Flag (Nav, NP_VISITED); }

//+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+

bool Nav_UnSet_Hidden (char* Nav) { return Nav_UnSet_Flag (Nav, NP_HIDDEN); }
...
bool Nav_UnSet_Visited (char* Nav) { return Nav_UnSet_Flag (Nav, NP_VISITED); }
...
unsigned int DistanceTo (int nav) {
    if (nav >= MAX_NAVPOINTS || nav < 0) return 0xFFFFFFFF;

    return (uint)vm_vec_dist_quick (
        &Player_obj->pos, Navs[nav].GetPosition ());
}
...
bool IsVisited (int nav) {
    if (nav >= MAX_NAVPOINTS || nav < 0) return 0;

    if (Navs[nav].flags & NP_VISITED) return 1;
    return 0;
}
