...
bool StartAutopilot()
{
             ...
             objp != END_OF_LIST(&obj_used_list); objp = GET_NEXT(objp)) {
                if ((objp->type == OBJ_SHIP) && !(objp->flags[Object::Object_Flags::Should_be_dead])) {
                        ...
                        if (ai_find_goal_index(
                                    support_ship_aip->goals, AI_GOAL_REARM_REPAIR)
                            == -1) {
        ...
        for (i = 0; i < MAX_SHIPS; i++) {
                if (Ships[i].objnum != -1 && (Ships[i].flags[Ship::Ship_Flags::Navpoint_carry] || (Ships[i].wingnum != -1 && Wings[Ships[i].wingnum].flags[Ship::Wing_Flags::Nav_carry]))) {
            ...
            it being largest object's later
            {
                rpos = Objects[Ships[i].objnum].pos;
                        ...
                        if (The_mission.flags[Mission::Mission_Flags::
                            ...
                            && Autopilot_flight_leader != &Objects[Ships[i].objnum])                               // only if not flight
                                                                                                                   // leader's object
                        {
                                ai_info *aip = &Ai_info[Ships[i].ai_index];
                                int wingnum = aip->wing,
                                    wing_index = get_wing_index(&Objects[Ships[i].objnum], wingnum);
                                vec3d goal_point;
                        ...
                        if (Wings[i].flags[Ship::Wing_Flags::Nav_carry]) {
                                ...
                                if (Navs[CurrentNav].flags & NP_WAYPOINT) {
                                        ...
                                        for (j = 0; j < MAX_AI_GOALS; j++) {
                                                if (Wings[i].ai_goals[j].ai_mode == AI_GOAL_WAYPOINTS_ONCE || Wings[i].ai_goals[j].ai_mode == AIM_WAYPOINTS) {
        ...
        for (i = 0; i < MAX_SHIPS; i++) {
                if (Ships[i].objnum != -1) {
                        if (Ships[i].flags[Ship::Ship_Flags::Navpoint_carry] || (Ships[i].wingnum != -1 && Wings[Ships[i].wingnum].flags[Ship::Wing_Flags::Nav_carry]))
                                for (j = 0; j < MAX_AI_GOALS; j++) {
                                        if (Ai_info[Ships[i].ai_index].goals[j].ai_mode == AI_GOAL_WAYPOINTS_ONCE || Ai_info[Ships[i].ai_index].goals[j].ai_mode == AIM_WAYPOINTS) {
                                                autopilot_ai_waypoint_goal_fixup(
        ...
        if (The_mission.flags[Mission::Mission_Flags::Use_ap_cinematics]) {
                             ...
                             idx != capIndexes.end(); ++idx) {
                                ...
                                if (Ship_info[Ships[*idx].ship_info_index]
                                    ...
                                    || Ship_info[Ships[*idx].ship_info_index]
                                        ...
                                        vm_vec_scale(
                                                &right, 2 * (1 + ((float)floor((float)capship_placed[2] / 3))));
                                        vm_vec_scale(
                                                &front, 2 * (1 + ((float)floor((float)capship_placed[2] / 3))));
                ...
                EndAPCinematic = timestamp(
                        (10000 * tc_factor) + NPS_TICKRATE); // 10 objective seconds before end of cinematic
...

        {
                if (Ships[i].objnum != -1 && (Ships[i].flags[Ship::Ship_Flags::Navpoint_carry] || (Ships[i].wingnum != -1 && Wings[Ships[i].wingnum].flags[Ship::Wing_Flags::Nav_carry]))) {
                        ...
                        for (j = 0; j < MAX_AI_GOALS; j++) {
                                ...
                                if (((aigp->target_name != NULL) && !strcasecmp(aigp->target_name, goal_name)) && (aigp->ai_mode == goal)) {
...
bool AddNav_Ship(char *Nav, char *TargetName, int flags)
{
        ...
        for (i = 0; i < MAX_SHIPS; i++) {
                if (Ships[i].objnum != -1 && !strcasecmp(TargetName, Ships[i].ship_name)) {
