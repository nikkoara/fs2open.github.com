...
int dds_read_header (
    const char* filename, CFILE* img_cfp, int* width, int* height, int* bpp,
    int* compression_type, int* levels, size_t* size, ubyte* palette) {
    ...
    if (img_cfp == NULL) {
        ...
        // file not found
        if (ddsfile == NULL) return DDS_ERROR_INVALID_FILENAME;
    }
    else {
        ddsfile = img_cfp;
    }
    ...
    for (i = 0; i < 11; i++) cfread_uint (ddsfile);
    ...
    // sanity
    if (dds_header.dwDepth == 0) dds_header.dwDepth = 1;

    if (dds_header.dwMipMapCount < 1) dds_header.dwMipMapCount = 1;
    ...
    if (dds_header.ddpfPixelFormat.dwFlags & DDPF_FOURCC) {
        ...
        // this is here to fix the situation where Microsoft doesn't follow
        // their own docs
        if (dds_header.dwPitchOrLinearSize <= 0) {
            // calculate the block size, mult by 8 for DXT1, 16 for DXT3 & 5
            d_size =
                ((dds_header.dwWidth + 3) / 4) *
                ((dds_header.dwHeight + 3) / 4) *
                ((dds_header.dwDepth + 3) / 4) *
                ((dds_header.ddpfPixelFormat.dwFourCC == FOURCC_DXT1) ? 8
                                                                      : 16);
        }
        else {
        ...
        for (i = 1; i < (int)dds_header.dwMipMapCount; i++) {
            ...
            if (d_height <= 0) d_height = 1;

            // size of data block (4x4)
            d_size +=
                ((d_width + 3) / 4) * ((d_height + 3) / 4) *
                ((dds_header.ddpfPixelFormat.dwFourCC == FOURCC_DXT1) ? 8
                                                                      : 16);
        }
        ...
        if (dds_header.ddsCaps.dwCaps2 & DDSCAPS2_CUBEMAP) {
            ...
            // the previously computed size is just per face, mult by 6 for
            // full size
            d_size *= 6;
        }
        ...
        *size = d_size;
        ...
        dds_header.ddpfPixelFormat.dwFlags &
        (DDPF_RGB | DDPF_PALETTEINDEXED8)) {
        d_size += d_width * d_height * d_depth *
    ...
    if ((ct != DDS_UNCOMPRESSED) &&
        (!is_power_of_two (dds_header.dwWidth, dds_header.dwHeight))) {
        retval = DDS_ERROR_NON_POWER_OF_2;
        goto Done;
    }

    // make sure that the video card can handle compressed textures before
    // using them
    if (!Use_compressed_textures && (ct != DDS_UNCOMPRESSED) &&
        (ct != DDS_DXT_INVALID)) {
    ...
    if (width) *width = dds_header.dwWidth;

    if (height) *height = dds_header.dwHeight;

    if (levels) *levels = dds_header.dwMipMapCount;
...
int dds_read_bitmap (
    const char* filename, ubyte* data, ubyte* bpp, int cf_type) {
    int retval;
    int w, h, ct, lvl;
    size_t size = 0;
    int bits = 0;
    ...
    // just in case
    if (cfp == NULL) return DDS_ERROR_INVALID_FILENAME;
...
// NOTE: we only support, uncompressed, 24-bit RGB and 32-bit RGBA images
// here!!
void dds_save_image (
    int width, int height, int bpp, int num_mipmaps, ubyte* data, int cubemap,
    const char* filename) {
    DDSURFACEDESC2 dds_header;
    ...
    // not that that's out of the way we can get down to business, constructing
    // a sane filename
    ...
    if (filename == NULL) {
        if (cubemap) { sprintf (real_filename, "cubemap%.3d.dds", count++); }
        else {
    ...
    dds_header.ddpfPixelFormat.dwRGBAlphaBitMask =
...
const char* dds_error_string (int code) {
    switch (code) {
    case DDS_ERROR_NONE: return "No error";

    case DDS_ERROR_INVALID_FILENAME: return "File not found";

    case DDS_ERROR_BAD_HEADER: return "Filecode did not equal \"DDS \"";
    ...
    case DDS_ERROR_NON_POWER_OF_2: return "Cannot load DDS if not power-of-2";

    case DDS_ERROR_CUBEMAP_FACES: return "Cubemaps must have all 6 faces";

    default: return "Abort, retry, fail?";
    }
}
