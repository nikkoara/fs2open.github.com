...
#define MAKEFOURCC(ch0, ch1, ch2, ch3) \
        ((uint)(ubyte)(ch0) | ((uint)(ubyte)(ch1) << 8) | ((uint)(ubyte)(ch2) << 16) | ((uint)(ubyte)(ch3) << 24))
...
#define DDS_OFFSET \
        4 + sizeof(DDSURFACEDESC2) // place where the data starts -- should be 128

// reads a dds header
// returns one of the error values
//'compression_type' comes back as one of the DDS_DXTC* defines
int dds_read_header(
        const char *filename, CFILE *img_cfp = NULL, int *width = 0,
        int *height = 0, int *bpp = 0, int *compression_type = 0, int *levels = 0,
        size_t *size = 0, ubyte *palette = NULL);

// reads bitmap
// size of the data it stored in size
int dds_read_bitmap(
        const char *filename, ubyte *data, ubyte *bpp = NULL,
        int cf_type = CF_TYPE_ANY);

// writes a DDS file using given data
void dds_save_image(
        int width, int height, int bpp, int num_mipmaps, ubyte *data = NULL,
        int cubemap = 0, const char *filename = NULL);
...
#endif // FREESPACE2_DDSUTILS_DDSUTILS_HH
