...
void vm_project_point_onto_plane (
    ...
    float dist;
    ASSERT (
        vm_vec_mag (plane_normal) > 0.999f &&
...
vec3d* vm_vec_avg4 (
    ...
    const vec3d* src3) {
    dest->xyz.x =
        (src0->xyz.x + src1->xyz.x + src2->xyz.x + src3->xyz.x) * 0.25f;
    dest->xyz.y =
        (src0->xyz.y + src1->xyz.y + src2->xyz.y + src3->xyz.y) * 0.25f;
    dest->xyz.z =
        (src0->xyz.z + src1->xyz.z + src2->xyz.z + src3->xyz.z) * 0.25f;
    return dest;
}
...
float vm_vec_dot (const vec3d* v0, const vec3d* v1) {
    return (v1->xyz.x * v0->xyz.x) + (v1->xyz.y * v0->xyz.y) +
           (v1->xyz.z * v0->xyz.z);
}
...
#if _M_IX86_FP < 1
    return 1.0f / sqrt (
                      (v->xyz.x * v->xyz.x) + (v->xyz.y * v->xyz.y) +
                      (v->xyz.z * v->xyz.z));
#else
    float x =
        (v->xyz.x * v->xyz.x) + (v->xyz.y * v->xyz.y) + (v->xyz.z * v->xyz.z);
...
float vm_vec_delta_ang (const vec3d* v0, const vec3d* v1, const vec3d* fvec) {
    ...
    if (NULL == fvec) { t = vm_vec_delta_ang_norm (&t0, &t1, NULL); }
    else {
...
void vm_vector_2_matrix_gen_vectors (matrix* m) {
    ...
    if ((zvec->xyz.x == 0.0f) &&
        (zvec->xyz.z == 0.0f)) { // forward vec is straight up or down
        m->vec.rvec.xyz.x = 1.0f;
        m->vec.uvec.xyz.z = (zvec->xyz.y < 0.0f) ? 1.0f : -1.0f;

        m->vec.rvec.xyz.y = m->vec.rvec.xyz.z = m->vec.uvec.xyz.x =
            m->vec.uvec.xyz.y = 0.0f;
    }
...
void vm_orthogonalize_matrix (matrix* m_src) {
    ...
    if (umag <= 0.0f) {     // no up vector to use..
        if (rmag <= 0.0f) { // no right vector either, so make something up
            if (!m->vec.fvec.xyz.x && !m->vec.fvec.xyz.z &&
                m->vec.fvec.xyz.y) // vertical vector
...
void vm_fix_matrix (matrix* m) {
    ...
    if (fmag <= 0.0f) {
        if ((umag > 0.0f) && (rmag > 0.0f) &&
            ...
            if (!m->vec.uvec.xyz.x && !m->vec.uvec.xyz.y &&
                m->vec.uvec.xyz.z) // z vector
    ...
    if ((umag <= 0.0f) ||
        vm_test_parallel (
            &m->vec.fvec, &m->vec.uvec)) { // no up vector to use..
        if ((rmag <= 0.0f) ||
            vm_test_parallel (
                &m->vec.fvec, &m->vec.rvec)) { // no right vector either, so
                                               // make something up
            if (!m->vec.fvec.xyz.x && m->vec.fvec.xyz.y &&
                !m->vec.fvec.xyz.z) // vertical vector
                vm_vec_make (&m->vec.uvec, 0.0f, 0.0f, -1.0f);
            else
                vm_vec_make (&m->vec.uvec, 0.0f, 1.0f, 0.0f);
        }
...
int vm_vec_same (const vec3d* v1, const vec3d* v2) {
    if (v1->xyz.x == v2->xyz.x && v1->xyz.y == v2->xyz.y &&
        v1->xyz.z == v2->xyz.z)
        return 1;
...
void vm_matrix_to_rot_axis_and_angle (
    ...
    if (cos_theta >
        0.999999875f) { // angle is less than 1 milirad (0.057 degrees)
        *theta = 0.0f;
        ...
        *theta = PI;
        ...
        if (m->a2d[1][1] > m->a2d[0][0]) { largest_diagonal_index = 1; }
        if (m->a2d[2][2] >
            m->a2d[largest_diagonal_index][largest_diagonal_index]) {
...
static float approach (
    ...
    if (w_in * w_in > 2.0f * 1.05f * aa * theta_goal) { // overshoot condition
        ...
        if (delta_theta > theta_goal) { // pass goal during this frame
            ...
            delta_theta =
                w_in * t_goal + 0.5f * effective_aa * t_goal * t_goal;
            delta_theta -= away (
...

{
    ...
    t_excess = delta_t - t0;
    delta_theta +=
        approach (0.0f, w_max, theta_goal, aa, t_excess, w_out, no_overshoot);
    return delta_theta;
}
...
void vm_forward_interpolate (
    ...
    if (t < SMALLER_NUM) {
        ...
        if (z_dotprod < 0.0f) theta = PI - theta;
    }
    ...
    // is 0
    no_bank =
        (delta_bank == 0.0f && vel_limit->xyz.z == 0.0f &&
    ...
    if (theta > SMALL_NUM) vm_vec_scale (&rot_axis, 1 / theta);
...
void vm_find_bounding_sphere (
    ...
    for (i = 0; i < num_pnts; i++) {
        p = &pnts[i];
        if (p->xyz.x < xmin.xyz.x) xmin = *p;
        if (p->xyz.x > xmax.xyz.x) xmax = *p;
        if (p->xyz.y < ymin.xyz.y) ymin = *p;
        if (p->xyz.y > ymax.xyz.y) ymax = *p;
        if (p->xyz.z < zmin.xyz.z) zmin = *p;
        if (p->xyz.z > zmax.xyz.z) zmax = *p;
    }
    ...
    for (i = 0; i < num_pnts; i++) {
        ...
        if (old_to_p_sq > rad_sq) {
            ...
            center->xyz.x =
                (rad * center->xyz.x + old_to_new * p->xyz.x) / old_to_p;
            center->xyz.y =
                (rad * center->xyz.y + old_to_new * p->xyz.y) / old_to_p;
            center->xyz.z =
                (rad * center->xyz.z + old_to_new * p->xyz.z) / old_to_p;
...
int is_valid_vec (const vec3d* vec) {
    return !std::isnan (vec->xyz.x) && !std::isnan (vec->xyz.y) &&
           !std::isnan (vec->xyz.z);
}

// Return true if all elements of *m are legal, that is, not a NAN.
int is_valid_matrix (const matrix* m) {
    return is_valid_vec (&m->vec.fvec) && is_valid_vec (&m->vec.uvec) &&
...
int vm_vec_dist_to_line (
    ...
    if (dist != NULL) { *dist = vm_vec_dist (nearest, p); }
...
bool vm_inverse_matrix4 (const matrix4* m, matrix4* invOut) {
    ...
    det = m->a2d[0][0] * inv.a2d[0][0] + m->a2d[0][1] * inv.a2d[1][0] +
          m->a2d[0][2] * inv.a2d[2][0] + m->a2d[0][3] * inv.a2d[3][0];
...
float vm_vec4_dot4 (float x, float y, float z, float w, const vec4* v) {
    return (x * v->xyzw.x) + (y * v->xyzw.y) + (z * v->xyzw.z) +
           (w * v->xyzw.w);
}

void vm_vec_transform (vec4* dest, const vec4* src, const matrix4* m) {
    dest->xyzw.x = (m->vec.rvec.xyzw.x * src->xyzw.x) +
                   (m->vec.uvec.xyzw.x * src->xyzw.y) +
                   (m->vec.fvec.xyzw.x * src->xyzw.z) +
                   (m->vec.pos.xyzw.x * src->xyzw.w);
    dest->xyzw.y = (m->vec.rvec.xyzw.y * src->xyzw.x) +
                   (m->vec.uvec.xyzw.y * src->xyzw.y) +
                   (m->vec.fvec.xyzw.y * src->xyzw.z) +
                   (m->vec.pos.xyzw.y * src->xyzw.w);
    dest->xyzw.z = (m->vec.rvec.xyzw.z * src->xyzw.x) +
                   (m->vec.uvec.xyzw.z * src->xyzw.y) +
                   (m->vec.fvec.xyzw.z * src->xyzw.z) +
                   (m->vec.pos.xyzw.z * src->xyzw.w);
    dest->xyzw.w = (m->vec.rvec.xyzw.w * src->xyzw.x) +
                   (m->vec.uvec.xyzw.w * src->xyzw.y) +
                   (m->vec.fvec.xyzw.w * src->xyzw.z) +
                   (m->vec.pos.xyzw.w * src->xyzw.w);
}
