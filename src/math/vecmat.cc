...
{
        return eqf(self.a1d[0], other.a1d[0]) && eqf(self.a1d[1], other.a1d[1]) && eqf(self.a1d[2], other.a1d[2]) && eqf(self.a1d[3], other.a1d[3]);
}

bool vm_vec_equal(const vec3d &self, const vec3d &other)
{
        return eqf(self.a1d[0], other.a1d[0]) && eqf(self.a1d[1], other.a1d[1]) && eqf(self.a1d[2], other.a1d[2]);
}
...
{
        return vm_vec_equal(self.vec.fvec, other.vec.fvec) && vm_vec_equal(self.vec.rvec, other.vec.rvec) && vm_vec_equal(self.vec.uvec, other.vec.uvec) && vm_vec_equal(self.vec.pos, other.vec.pos);
}
...
//
float vm_vec_projection_parallel(
        vec3d *component, const vec3d *src, const vec3d *unit_vec)
{
        float mag;
...
//
void vm_vec_projection_onto_plane(
        vec3d *projection, const vec3d *src, const vec3d *unit_normal)
{
        float mag;
        ASSERT(
                vm_vec_mag(unit_normal) > 0.999f && vm_vec_mag(unit_normal) < 1.001f);
...
// moves the point in the direction of the plane normal until it is on the
// plane
//
...
{
        float D; // plane constant in Ax+By+Cz+D = 0   or   dot(X,n) - dot(Xp,n) =
                 // 0, so D = -dot(Xp,n)
        float dist;
        ASSERT(
                vm_vec_mag(plane_normal) > 0.999f && vm_vec_mag(plane_normal) < 1.001f);
...
vec3d *vm_vec_avg3(
        vec3d *dest, const vec3d *src0, const vec3d *src1, const vec3d *src2)
{
...
// dest = src1 + k * src2
void vm_vec_scale_add(
        vec3d *dest, const vec3d *src1, const vec3d *src2, float k)
{
...
{
        return (
                (v->xyz.x * v->xyz.x) + (v->xyz.y * v->xyz.y) + (v->xyz.z * v->xyz.z));
}
...
float vm_vec_copy_normalize(vec3d *dest, const vec3d *src)
{
        ...
        if (m <= 0.0f) {
                WARNINGF(LOCATION, "Null vec3d in vec3d normalize.\nTrace out of vecmat.cpp and find offending code.");
...
float vm_vec_normalized_dir(
        vec3d *dest, const vec3d *end, const vec3d *start)
{
        float t;
...
// dest = normalized(end - start).  Returns mag of direction vector
// NOTE: the order of the parameters matches the vector subtraction
float vm_vec_normalized_dir_quick(
        vec3d *dest, const vec3d *end, const vec3d *start)
{
        vm_vec_sub(dest, end, start);
...
vec3d *vm_vec_normal(
        vec3d *dest, const vec3d *p0, const vec3d *p1, const vec3d *p2)
{
        ASSERT(dest != p0);
        ASSERT(dest != p1);
        ASSERT(dest != p2);
...
// dest CANNOT equal either source
vec3d *
vm_vec_perp(vec3d *dest, const vec3d *p0, const vec3d *p1, const vec3d *p2)
{
        ASSERT(dest != p0);
        ASSERT(dest != p1);
        ASSERT(dest != p2);
...
static matrix *sincos_2_matrix(
        matrix *m, float sinp, float cosp, float sinb, float cosb, float sinh,
        float cosh)
{
...
matrix *vm_vector_2_matrix(
        matrix *m, const vec3d *fvec, const vec3d *uvec, const vec3d *rvec)
{
        vec3d fvec_norm;
...
matrix *vm_vector_2_matrix_norm(
        matrix *m, const vec3d *fvec, const vec3d *uvec, const vec3d *rvec)
{
        matrix temp = *m;
        ...
        if (uvec == NULL) {
                if (rvec == NULL) { // just forward vec
                        ...
                        *xvec = *rvec;
                        ...
                        // now recompute right vector, in case it wasn't entirely
                        // perpendiclar
...
vm_matrix_x_matrix(matrix *dest, const matrix *src0, const matrix *src1)
{
        ...
        dest->vec.rvec.xyz.x = vm_vec_dot3(
                src0->vec.rvec.xyz.x, src0->vec.uvec.xyz.x, src0->vec.fvec.xyz.x,
                &src1->vec.rvec);
        dest->vec.uvec.xyz.x = vm_vec_dot3(
                src0->vec.rvec.xyz.x, src0->vec.uvec.xyz.x, src0->vec.fvec.xyz.x,
                &src1->vec.uvec);
        dest->vec.fvec.xyz.x = vm_vec_dot3(
                src0->vec.rvec.xyz.x, src0->vec.uvec.xyz.x, src0->vec.fvec.xyz.x,
                &src1->vec.fvec);

        dest->vec.rvec.xyz.y = vm_vec_dot3(
                src0->vec.rvec.xyz.y, src0->vec.uvec.xyz.y, src0->vec.fvec.xyz.y,
                &src1->vec.rvec);
        dest->vec.uvec.xyz.y = vm_vec_dot3(
                src0->vec.rvec.xyz.y, src0->vec.uvec.xyz.y, src0->vec.fvec.xyz.y,
                &src1->vec.uvec);
        dest->vec.fvec.xyz.y = vm_vec_dot3(
                src0->vec.rvec.xyz.y, src0->vec.uvec.xyz.y, src0->vec.fvec.xyz.y,
                &src1->vec.fvec);

        dest->vec.rvec.xyz.z = vm_vec_dot3(
                src0->vec.rvec.xyz.z, src0->vec.uvec.xyz.z, src0->vec.fvec.xyz.z,
                &src1->vec.rvec);
        dest->vec.uvec.xyz.z = vm_vec_dot3(
                src0->vec.rvec.xyz.z, src0->vec.uvec.xyz.z, src0->vec.fvec.xyz.z,
                &src1->vec.uvec);
        dest->vec.fvec.xyz.z = vm_vec_dot3(
                src0->vec.rvec.xyz.z, src0->vec.uvec.xyz.z, src0->vec.fvec.xyz.z,
                &src1->vec.fvec);
...
angles_t *vm_extract_angles_matrix(angles_t *a, const matrix *m)
{
        ...
        if (cosp == 0.0f) // the cosine of pitch is zero.  we're pitched straight
                          // up. say no bank
        ...
        const vec3d *int_pnt)
{
        ...
        if (mag < 0.01f) {
                *nearest_point = *int_pnt;
                return 9999.9f;
                // Warning(LOCATION, "Very small magnitude in
...
// like vm_orthogonalize_matrix(), except that zero vectors can exist within
// the matrix without causing problems.  Valid vectors will be created where
// needed.
void vm_fix_matrix(matrix *m)
{
        ...
        if ((umag <= 0.0f) || vm_test_parallel(&m->vec.fvec, &m->vec.uvec)) {              // no up vector to use..
                if ((rmag <= 0.0f) || vm_test_parallel(&m->vec.fvec, &m->vec.rvec)) {      // no right vector either, so
                                                                                           // make something up
...
float vm_vec_dot_to_point(
        const vec3d *dir, const vec3d *p1, const vec3d *p2)
{
        vec3d tvec;
...
void vm_rot_point_around_line(
        vec3d *out, const vec3d *in, float angle, const vec3d *line_point,
...
float vm_interp_angle(
        float *h, float desired_angle, float step_size, bool force_front)
{
        float delta;
        float abs_delta;
...
// void vm_matrix_to_rot_axis_and_angle(matrix *m, float *theta, vec3d
// *rot_axis)
...
void vm_matrix_to_rot_axis_and_angle(
        const matrix *m, float *theta, vec3d *rot_axis)
{
...
// velocity in trying to reach a goal given an angular velocity APPROACHing a
// goal.  It uses maximal acceleration to a point (called peak), then maximal
// deceleration to arrive at the goal with zero angular velocity.  This can
// occasionally cause overshoot. w_in                   > 0 w_max                       > 0
// theta_goal   > 0
// aa                           > 0
// returns delta_theta
static float away(
        float w_in, float w_max, float theta_goal, float aa, float delta_t,
        ...
        float *w_out, int no_overshoot)
{
        ...
        if (w_in * w_in > 2.0f * 1.05f * aa * theta_goal) { // overshoot condition
                ...
                if (delta_theta > theta_goal) { // pass goal during this frame
                        float t_goal = (-w_in + sqrtf(w_in * w_in + 2.0f * effective_aa * theta_goal)) / effective_aa;
                        ...
                        delta_theta -= away(
                                -w_in, w_max, 0.0f, aa, delta_t - t_goal, w_out, no_overshoot);
                        *w_out = -*w_out;
                        return delta_theta;
                } else {
                        if (delta_theta < 0) {
...
// This routine determines the resultant angular displacement and angular
// velocity in trying to reach a goal given an angular velocity AWAY from a
// goal.  It uses maximal acceleration to a point (called peak), then maximal
...
void vm_matrix_interpolate(
        const matrix *goal_orient, const matrix *curr_orient, const vec3d *w_in,
        float delta_t, matrix *next_orient, vec3d *w_out, const vec3d *vel_limit,
        const vec3d *acc_limit, int no_overshoot)
{
        ...
        vec3d
                theta_goal; // desired angular position at the end of the time interval
        ...
        vm_matrix_x_matrix(
                &rot_matrix, &Mtemp1, goal_orient); // R = goal * Mtemp1
        vm_orthogonalize_matrix(&rot_matrix);
        vm_matrix_to_rot_axis_and_angle(
                &rot_matrix, &theta,
        ...
        if (theta_goal.xyz.x > 0) {
                if (w_in->xyz.x >= 0) {
                        delta_theta = approach(
                                w_in->xyz.x, vel_limit->xyz.x, theta_goal.xyz.x,
                                acc_limit->xyz.x, delta_t, &w_out->xyz.x, no_overshoot);
        ...
        if (theta_goal.xyz.y > 0) {
                if (w_in->xyz.y >= 0) {
                        delta_theta = approach(
                                w_in->xyz.y, vel_limit->xyz.y, theta_goal.xyz.y,
                                acc_limit->xyz.y, delta_t, &w_out->xyz.y, no_overshoot);
        ...
        if (theta_goal.xyz.z > 0) {
                if (w_in->xyz.z >= 0) {
                        delta_theta = approach(
                                w_in->xyz.z, vel_limit->xyz.z, theta_goal.xyz.z,
                                acc_limit->xyz.z, delta_t, &w_out->xyz.z, no_overshoot);
...
void get_camera_limits(
        const matrix *start_camera, const matrix *end_camera, float time,
        vec3d *acc_max, vec3d *w_max)
{
...
void vm_forward_interpolate(
        ...
        const vec3d *vel_limit, const vec3d *acc_limit, int no_overshoot)
{
        matrix Mtemp1;        // temporary matrix
        vec3d local_rot_axis; // vector indicating direction of rotation axis
                              // (local coords)
        vec3d rot_axis;       // vector indicating direction of rotation axis (world
                              // coords)
        vec3d
                theta_goal; // desired angular position at the end of the time interval
        vec3d theta_end;    // actual angular position at the end of the time interval
        ...
        // no rotation if delta_bank and w_in both 0 or rotational acc in forward
        // is 0
        ...
        if (!no_bank) {
                ...
                if (delta_bank > 0) {
                        if (w_in->xyz.z >= 0) {
                                delta_theta = approach(
                                        w_in->xyz.z, vel_limit->xyz.z, delta_bank,
                                        acc_limit->xyz.z, delta_t, &w_out->xyz.z, no_overshoot);
                                bank = delta_theta;
                        } else { // w_in->xyz.z < 0
                                delta_theta = away(
...
void vm_find_bounding_sphere(
        const vec3d *pnts, int num_pnts, vec3d *center, float *radius)
{
        int i;
        ...
        for (i = 0; i < num_pnts; i++) {
                ...
                if (old_to_p_sq > rad_sq) {
                        ...
                        WARNINGF(LOCATION, "New sphere: cen, rad = %f %f %f  %f", center->xyz.x, center->xyz.y, center->xyz.z, rad);
                }
        }
...
//
vec3d *vm_rotate_vec_to_body(
        vec3d *body_vec, const vec3d *world_vec, const matrix *orient)
{
...
// orient          =>              orientation matrix
//
vec3d *vm_rotate_vec_to_world(
        vec3d *world_vec, const vec3d *body_vec, const matrix *orient)
{
...
void vm_vec_interp_constant(
        vec3d *out, const vec3d *v0, const vec3d *v1, float t)
{
        vec3d cross;
        float total_ang;
        ...
        vm_rot_point_around_line(
                out, v0, t * total_ang, &vmd_zero_vector, &cross);
}

// randomly perturb a vector around a given (normalized vector) or optional
// orientation matrix
void vm_vec_random_cone(
        vec3d *out, const vec3d *in, float max_angle, const matrix *orient)
{
        vec3d temp;
        const matrix *rot;
        matrix m;
        ...
        vm_vec_make(
                &temp, sqrtf(1.0f - z * z) * cosf(phi),
                sqrtf(1.0f - z * z) * sinf(phi),
                z); // Using the z-vec as the starting point

        vm_vec_unrotate(out, &temp, rot); // We find the final vector by rotating
                                          // temp to the correct orientation
}

void vm_vec_random_cone(
        vec3d *out, const vec3d *in, float min_angle, float max_angle,
        const matrix *orient)
{
        vec3d temp;
        const matrix *rot;
...
void vm_vec_random_in_circle(
        vec3d *out, const vec3d *in, const matrix *orient, float radius,
        int on_edge)
{
        vec3d temp;
...
// sphere
void vm_vec_random_in_sphere(
        vec3d *out, const vec3d *in, float radius, int on_edge)
{
        vec3d temp;
        // Uniformly distributing each coordinate of a vector then normalizing
        // results in a uniform sphere distribution
        util::UniformFloatRange coords(-1.0f, 1.0f);
        vm_vec_make(&temp, coords.next(), coords.next(), coords.next());
        vm_vec_normalize(&temp);
        // We then add the scaled result to the initial position to get the final
        // position
        vm_vec_scale_add(
                out, in, &temp,
                on_edge ? radius : util::UniformFloatRange(0.0f, radius).next());
}

// find the nearest point on the line to p. if dist is non-NULL, it is filled
// in returns 0 if the point is inside the line segment, -1 if "before" the
// line segment and 1 ir "after" the line segment
int vm_vec_dist_to_line(
        const vec3d *p, const vec3d *l0, const vec3d *l1, vec3d *nearest,
        float *dist)
{
        vec3d a, b, c;
        float b_mag, comp;
...
void vm_vec_dist_squared_to_line(
        const vec3d *p, const vec3d *l0, const vec3d *l1, vec3d *nearest,
        float *dist_squared)
{
        vec3d a, b, c;
...
{
        matrix4 inv; // create a temp matrix so we can avoid getting a determinant
                     // that is 0
        float det;
        int i, j;

        // Use a2d so it's easier for people to read
        inv.a2d[0][0] = m->a2d[1][1] * m->a2d[2][2] * m->a2d[3][3] - m->a2d[1][1] * m->a2d[2][3] * m->a2d[3][2] - m->a2d[2][1] * m->a2d[1][2] * m->a2d[3][3] + m->a2d[2][1] * m->a2d[1][3] * m->a2d[3][2] + m->a2d[3][1] * m->a2d[1][2] * m->a2d[2][3] - m->a2d[3][1] * m->a2d[1][3] * m->a2d[2][2];

        inv.a2d[1][0] = -m->a2d[1][0] * m->a2d[2][2] * m->a2d[3][3] + m->a2d[1][0] * m->a2d[2][3] * m->a2d[3][2] + m->a2d[2][0] * m->a2d[1][2] * m->a2d[3][3] - m->a2d[2][0] * m->a2d[1][3] * m->a2d[3][2] - m->a2d[3][0] * m->a2d[1][2] * m->a2d[2][3] + m->a2d[3][0] * m->a2d[1][3] * m->a2d[2][2];

        inv.a2d[2][0] = m->a2d[1][0] * m->a2d[2][1] * m->a2d[3][3] - m->a2d[1][0] * m->a2d[2][3] * m->a2d[3][1] - m->a2d[2][0] * m->a2d[1][1] * m->a2d[3][3] + m->a2d[2][0] * m->a2d[1][3] * m->a2d[3][1] + m->a2d[3][0] * m->a2d[1][1] * m->a2d[2][3] - m->a2d[3][0] * m->a2d[1][3] * m->a2d[2][1];

        inv.a2d[3][0] = -m->a2d[1][0] * m->a2d[2][1] * m->a2d[3][2] + m->a2d[1][0] * m->a2d[2][2] * m->a2d[3][1] + m->a2d[2][0] * m->a2d[1][1] * m->a2d[3][2] - m->a2d[2][0] * m->a2d[1][2] * m->a2d[3][1] - m->a2d[3][0] * m->a2d[1][1] * m->a2d[2][2] + m->a2d[3][0] * m->a2d[1][2] * m->a2d[2][1];

        inv.a2d[0][1] = -m->a2d[0][1] * m->a2d[2][2] * m->a2d[3][3] + m->a2d[0][1] * m->a2d[2][3] * m->a2d[3][2] + m->a2d[2][1] * m->a2d[0][2] * m->a2d[3][3] - m->a2d[2][1] * m->a2d[0][3] * m->a2d[3][2] - m->a2d[3][1] * m->a2d[0][2] * m->a2d[2][3] + m->a2d[3][1] * m->a2d[0][3] * m->a2d[2][2];

        inv.a2d[1][1] = m->a2d[0][0] * m->a2d[2][2] * m->a2d[3][3] - m->a2d[0][0] * m->a2d[2][3] * m->a2d[3][2] - m->a2d[2][0] * m->a2d[0][2] * m->a2d[3][3] + m->a2d[2][0] * m->a2d[0][3] * m->a2d[3][2] + m->a2d[3][0] * m->a2d[0][2] * m->a2d[2][3] - m->a2d[3][0] * m->a2d[0][3] * m->a2d[2][2];

        inv.a2d[2][1] = -m->a2d[0][0] * m->a2d[2][1] * m->a2d[3][3] + m->a2d[0][0] * m->a2d[2][3] * m->a2d[3][1] + m->a2d[2][0] * m->a2d[0][1] * m->a2d[3][3] - m->a2d[2][0] * m->a2d[0][3] * m->a2d[3][1] - m->a2d[3][0] * m->a2d[0][1] * m->a2d[2][3] + m->a2d[3][0] * m->a2d[0][3] * m->a2d[2][1];

        inv.a2d[3][1] = m->a2d[0][0] * m->a2d[2][1] * m->a2d[3][2] - m->a2d[0][0] * m->a2d[2][2] * m->a2d[3][1] - m->a2d[2][0] * m->a2d[0][1] * m->a2d[3][2] + m->a2d[2][0] * m->a2d[0][2] * m->a2d[3][1] + m->a2d[3][0] * m->a2d[0][1] * m->a2d[2][2] - m->a2d[3][0] * m->a2d[0][2] * m->a2d[2][1];

        inv.a2d[0][2] = m->a2d[0][1] * m->a2d[1][2] * m->a2d[3][3] - m->a2d[0][1] * m->a2d[1][3] * m->a2d[3][2] - m->a2d[1][1] * m->a2d[0][2] * m->a2d[3][3] + m->a2d[1][1] * m->a2d[0][3] * m->a2d[3][2] + m->a2d[3][1] * m->a2d[0][2] * m->a2d[1][3] - m->a2d[3][1] * m->a2d[0][3] * m->a2d[1][2];

        inv.a2d[1][2] = -m->a2d[0][0] * m->a2d[1][2] * m->a2d[3][3] + m->a2d[0][0] * m->a2d[1][3] * m->a2d[3][2] + m->a2d[1][0] * m->a2d[0][2] * m->a2d[3][3] - m->a2d[1][0] * m->a2d[0][3] * m->a2d[3][2] - m->a2d[3][0] * m->a2d[0][2] * m->a2d[1][3] + m->a2d[3][0] * m->a2d[0][3] * m->a2d[1][2];

        inv.a2d[2][2] = m->a2d[0][0] * m->a2d[1][1] * m->a2d[3][3] - m->a2d[0][0] * m->a2d[1][3] * m->a2d[3][1] - m->a2d[1][0] * m->a2d[0][1] * m->a2d[3][3] + m->a2d[1][0] * m->a2d[0][3] * m->a2d[3][1] + m->a2d[3][0] * m->a2d[0][1] * m->a2d[1][3] - m->a2d[3][0] * m->a2d[0][3] * m->a2d[1][1];

        inv.a2d[3][2] = -m->a2d[0][0] * m->a2d[1][1] * m->a2d[3][2] + m->a2d[0][0] * m->a2d[1][2] * m->a2d[3][1] + m->a2d[1][0] * m->a2d[0][1] * m->a2d[3][2] - m->a2d[1][0] * m->a2d[0][2] * m->a2d[3][1] - m->a2d[3][0] * m->a2d[0][1] * m->a2d[1][2] + m->a2d[3][0] * m->a2d[0][2] * m->a2d[1][1];

        inv.a2d[0][3] = -m->a2d[0][1] * m->a2d[1][2] * m->a2d[2][3] + m->a2d[0][1] * m->a2d[1][3] * m->a2d[2][2] + m->a2d[1][1] * m->a2d[0][2] * m->a2d[2][3] - m->a2d[1][1] * m->a2d[0][3] * m->a2d[2][2] - m->a2d[2][1] * m->a2d[0][2] * m->a2d[1][3] + m->a2d[2][1] * m->a2d[0][3] * m->a2d[1][2];

        inv.a2d[1][3] = m->a2d[0][0] * m->a2d[1][2] * m->a2d[2][3] - m->a2d[0][0] * m->a2d[1][3] * m->a2d[2][2] - m->a2d[1][0] * m->a2d[0][2] * m->a2d[2][3] + m->a2d[1][0] * m->a2d[0][3] * m->a2d[2][2] + m->a2d[2][0] * m->a2d[0][2] * m->a2d[1][3] - m->a2d[2][0] * m->a2d[0][3] * m->a2d[1][2];

        inv.a2d[2][3] = -m->a2d[0][0] * m->a2d[1][1] * m->a2d[2][3] + m->a2d[0][0] * m->a2d[1][3] * m->a2d[2][1] + m->a2d[1][0] * m->a2d[0][1] * m->a2d[2][3] - m->a2d[1][0] * m->a2d[0][3] * m->a2d[2][1] - m->a2d[2][0] * m->a2d[0][1] * m->a2d[1][3] + m->a2d[2][0] * m->a2d[0][3] * m->a2d[1][1];

        inv.a2d[3][3] = m->a2d[0][0] * m->a2d[1][1] * m->a2d[2][2] - m->a2d[0][0] * m->a2d[1][2] * m->a2d[2][1] - m->a2d[1][0] * m->a2d[0][1] * m->a2d[2][2] + m->a2d[1][0] * m->a2d[0][2] * m->a2d[2][1] + m->a2d[2][0] * m->a2d[0][1] * m->a2d[1][2] - m->a2d[2][0] * m->a2d[0][2] * m->a2d[1][1];
        ...
        for (i = 0; i < 4; i++) {
                for (j = 0; j < 4; j++) { invOut->a2d[i][j] = inv.a2d[i][j] * det; }
        }
...
{
        // this is basically the same function as the opengl view matrix
        // construction except we don't invert the Z-axis
        vec3d scaled_pos;
        vec3d inv_pos;
        matrix inv_orient;
...
void vm_matrix4_x_matrix4(
        matrix4 *dest, const matrix4 *src0, const matrix4 *src1)
{
        dest->vec.rvec.xyzw.x = vm_vec4_dot4(
                src0->vec.rvec.xyzw.x, src0->vec.uvec.xyzw.x, src0->vec.fvec.xyzw.x,
                src0->vec.pos.xyzw.x, &src1->vec.rvec);
        dest->vec.uvec.xyzw.x = vm_vec4_dot4(
                src0->vec.rvec.xyzw.x, src0->vec.uvec.xyzw.x, src0->vec.fvec.xyzw.x,
                src0->vec.pos.xyzw.x, &src1->vec.uvec);
        dest->vec.fvec.xyzw.x = vm_vec4_dot4(
                src0->vec.rvec.xyzw.x, src0->vec.uvec.xyzw.x, src0->vec.fvec.xyzw.x,
                src0->vec.pos.xyzw.x, &src1->vec.fvec);
        dest->vec.pos.xyzw.x = vm_vec4_dot4(
                src0->vec.rvec.xyzw.x, src0->vec.uvec.xyzw.x, src0->vec.fvec.xyzw.x,
                src0->vec.pos.xyzw.x, &src1->vec.pos);

        dest->vec.rvec.xyzw.y = vm_vec4_dot4(
                src0->vec.rvec.xyzw.y, src0->vec.uvec.xyzw.y, src0->vec.fvec.xyzw.y,
                src0->vec.pos.xyzw.y, &src1->vec.rvec);
        dest->vec.uvec.xyzw.y = vm_vec4_dot4(
                src0->vec.rvec.xyzw.y, src0->vec.uvec.xyzw.y, src0->vec.fvec.xyzw.y,
                src0->vec.pos.xyzw.y, &src1->vec.uvec);
        dest->vec.fvec.xyzw.y = vm_vec4_dot4(
                src0->vec.rvec.xyzw.y, src0->vec.uvec.xyzw.y, src0->vec.fvec.xyzw.y,
                src0->vec.pos.xyzw.y, &src1->vec.fvec);
        dest->vec.pos.xyzw.y = vm_vec4_dot4(
                src0->vec.rvec.xyzw.y, src0->vec.uvec.xyzw.y, src0->vec.fvec.xyzw.y,
                src0->vec.pos.xyzw.y, &src1->vec.pos);

        dest->vec.rvec.xyzw.z = vm_vec4_dot4(
                src0->vec.rvec.xyzw.z, src0->vec.uvec.xyzw.z, src0->vec.fvec.xyzw.z,
                src0->vec.pos.xyzw.z, &src1->vec.rvec);
        dest->vec.uvec.xyzw.z = vm_vec4_dot4(
                src0->vec.rvec.xyzw.z, src0->vec.uvec.xyzw.z, src0->vec.fvec.xyzw.z,
                src0->vec.pos.xyzw.z, &src1->vec.uvec);
        dest->vec.fvec.xyzw.z = vm_vec4_dot4(
                src0->vec.rvec.xyzw.z, src0->vec.uvec.xyzw.z, src0->vec.fvec.xyzw.z,
                src0->vec.pos.xyzw.z, &src1->vec.fvec);
        dest->vec.pos.xyzw.z = vm_vec4_dot4(
                src0->vec.rvec.xyzw.z, src0->vec.uvec.xyzw.z, src0->vec.fvec.xyzw.z,
                src0->vec.pos.xyzw.z, &src1->vec.pos);

        dest->vec.rvec.xyzw.w = vm_vec4_dot4(
                src0->vec.rvec.xyzw.w, src0->vec.uvec.xyzw.w, src0->vec.fvec.xyzw.w,
                src0->vec.pos.xyzw.w, &src1->vec.rvec);
        dest->vec.uvec.xyzw.w = vm_vec4_dot4(
                src0->vec.rvec.xyzw.w, src0->vec.uvec.xyzw.w, src0->vec.fvec.xyzw.w,
                src0->vec.pos.xyzw.w, &src1->vec.uvec);
        dest->vec.fvec.xyzw.w = vm_vec4_dot4(
                src0->vec.rvec.xyzw.w, src0->vec.uvec.xyzw.w, src0->vec.fvec.xyzw.w,
                src0->vec.pos.xyzw.w, &src1->vec.fvec);
        dest->vec.pos.xyzw.w = vm_vec4_dot4(
                src0->vec.rvec.xyzw.w, src0->vec.uvec.xyzw.w, src0->vec.fvec.xyzw.w,
...
void vm_vec_transform(
        vec3d *dest, const vec3d *src, const matrix4 *m, bool pos)
{
        ...
        // 0.0f will prevent matrix4 m's offset from being added. 1.0f will add the
        // offset.
