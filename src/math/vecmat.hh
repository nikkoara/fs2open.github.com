...
#define IS_VEC_NULL_SQ_SAFE(v) \
        (IS_NEAR_ZERO((v)->xyz.x, 1e-16f) && IS_NEAR_ZERO((v)->xyz.y, 1e-16f) && IS_NEAR_ZERO((v)->xyz.z, 1e-16f))

#define IS_VEC_NULL(v) \
        (IS_NEAR_ZERO((v)->xyz.x, 1e-36f) && IS_NEAR_ZERO((v)->xyz.y, 1e-36f) && IS_NEAR_ZERO((v)->xyz.z, 1e-36f))

#define IS_MAT_NULL(v) \
        (IS_VEC_NULL(&(v)->vec.fvec) && IS_VEC_NULL(&(v)->vec.uvec) && IS_VEC_NULL(&(v)->vec.rvec))
...
#define vm_angvec_make(v, _p, _b, _h) \
...
float vm_vec_projection_parallel(
        vec3d *component, const vec3d *src, const vec3d *unit_vector);

// finds the projection of source vector onto a surface given by surface normal
void vm_vec_projection_onto_plane(
        vec3d *projection, const vec3d *src, const vec3d *normal);

// returns magnitude of a vector
float vm_vec_mag(const vec3d *v);

// returns the square of the magnitude of a vector (useful if comparing
// distances)
...
// dest CANNOT equal either source
vec3d *
vm_vec_normal(vec3d *dest, const vec3d *p0, const vec3d *p1, const vec3d *p2);
 ...
 *
 * @param[out] matrix The generated matrix. Does not need to be an Identity
 * matrix
 ...
 * @note If uvec was bogus (either being in the same direction of fvec or
 * -fvec) then only fvec is used
 *
 * @sa vm_vector_2_matrix_norm
 */
matrix *vm_vector_2_matrix(
        matrix *m, const vec3d *fvec, const vec3d *uvec = nullptr,
        const vec3d *rvec = nullptr);
 ...
 * @param[in] rvec Normalized Vector referencing the right-hand direction
 * (Optional)
 *
 * @returns Pointer to the generated matrix
 *
...
matrix *vm_vector_2_matrix_norm(
        matrix *m, const vec3d *fvec, const vec3d *uvec = NULL,
...
float vm_interp_angle(
        float *h, float desired_angle, float step_size, bool force_front = false);
...
void vm_matrix_interpolate(
        const matrix *goal_orient, const matrix *start_orient,
        const vec3d *rotvel_in, float delta_t, matrix *next_orient,
...
void vm_forward_interpolate(
        const vec3d *goal_fvec, const matrix *orient, const vec3d *rotvel_in,
        float delta_t, float delta_bank, matrix *next_orient, vec3d *rotvel_out,
        const vec3d *vel_limit, const vec3d *acc_limit, int no_overshoot = 0);
...
void vm_vec_interp_constant(
        vec3d *out, const vec3d *v1, const vec3d *v2, float t);

// randomly perturb a vector around a given (normalized vector) or optional
// orientation matrix
void vm_vec_random_cone(
        vec3d *out, const vec3d *in, float max_angle, const matrix *orient = NULL);
void vm_vec_random_cone(
...
void vm_vec_transform(
        vec3d *dest, const vec3d *src, const matrix4 *m, bool pos = true);
...
/**
 * @brief Converts a 4 component vector to a 3 component vector by discarding
 * the w component
...
/**
 * @brief Converts a 3 component vector to a 4 component vector with the
 * specified w value
 * @param vec The first 3 components of the new vector
 * @param w The w component of the new vector. Defaults to 1.0f which is
 * correct for position vectors.
...
{
        return vm_vec_same(&left, &right) != 0;
}
inline bool operator!=(const vec3d &left, const vec3d &right)
{
        return !(left == right);
 ...
 *
 * @note This actually follows the definition of the * operator in linear
 * algebra. The standard vm_vec_rotate actually implements a multiplication
...
#endif // FREESPACE2_MATH_VECMAT_HH
