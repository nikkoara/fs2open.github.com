...
int static_rand (int num) {
    ...
    if (!Semirand_inited) init_semirand ();
...
/**
 * @brief Return a random float in 0.0f .. 1.0f- (ie, it will never
 * return 1.0f).
 *
 * @param num Seed input number
...
/**
 * @brief Return a random integer within a range. Note: min and max are
 * inclusive
 *
 * @param num Seed input number
...
/**
 * @brief Randomly perturb a vector around a given (normalized vector) or
 * optional orientation matrix.
 *
 * @param num
 * @param out
 * @param in
 ...
 * @param orient
 */
void static_rand_cone (
    int num, vec3d* out, vec3d* in, float max_angle, matrix* orient) {
    vec3d t1, t2;
    matrix* rot;
    matrix m;
    ...
    vm_rot_point_around_line (
        &t1, in, fl_radians (static_randf_range (num, -max_angle, max_angle)),
        &vmd_zero_vector, &rot->vec.fvec);

    // axis 2
    vm_rot_point_around_line (
        &t2, &t1,
        fl_radians (static_randf_range (num + 1, -max_angle, max_angle)),
        &vmd_zero_vector, &rot->vec.rvec);

    // axis 3
    vm_rot_point_around_line (
        out, &t2,
        fl_radians (static_randf_range (num + 2, -max_angle, max_angle)),
 ...
 */
void init_static_rand_alt (int seed) { Rnd_seed = seed; }
