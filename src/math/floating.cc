...
/**
 * @brief Return random value in range 0.0..1.0- (1.0- means the closest number
 * less than 1.0)
 */
float frand () {
    int i_rval;
    do { i_rval = myrand (); } while (i_rval == RAND_MAX);
 ...
 * @details If you want it to return TRUE 3 times per second, call it in the
 * frame interval like so: rand_chance(flFrametime, 3.0f);
 * @param frametime
 * @param chance
 */
int rand_chance (float frametime, float chance) {
    while (--chance > 0.0f)
        if (frand () < frametime) return 1;
