...
static float matrix_determinant_from_vectors (
...
/**
 * Finds the point on each line of closest approach (s and t) (lines need not
 * intersect to get closest point) Taken from graphic gems I, p. 304
 *
...
float fvi_ray_plane (
    ...
    const vec3d* ray_origin, const vec3d* ray_direction, float rad) {
    vec3d w;
    float num, den, t;
    ...
    if (new_pnt) vm_vec_scale_add (new_pnt, ray_origin, ray_direction, t);
...
int fvi_segment_plane (
    ...
    t = fvi_ray_plane (
        new_pnt, plane_pnt,
        plane_norm, // Plane description, a point and a normal
...
int fvi_segment_sphere (
    ...
    if (w_dist > mag_d + sphere_rad) return 0; // cannot hit
...
int fvi_ray_boundingbox (
    ...
    for (i = 0; i < 3; i++) {
        if ((middle & (1 << i)) || (pdir->a1d[i] == 0.0f)) { maxt[i] = -1.0f; }
        else {
    ...
    which_plane = 0;
    for (i = 1; i < 3; i++) {
        if (maxt[which_plane] < maxt[i]) { which_plane = i; }
    }
    ...
    for (i = 0; i < 3; i++) {
        if (which_plane == i) { hitpt->a1d[i] = candidate_plane[i]; }
        else {
            hitpt->a1d[i] = (maxt[which_plane] * pdir->a1d[i]) + p0->a1d[i];

            if ((hitpt->a1d[i] < min->a1d[i]) ||
                (hitpt->a1d[i] > max->a1d[i])) {
                return 0;
            }
        }
    }
...
static int check_sphere_point (
    ...
    float radius, float* collide_time);
...
int fvi_sphere_plane (
    ...
    *hit_time = t1;
    ...
    if (t1 > 0 && t1 < 1) {
        ...
        vm_project_point_onto_plane (
            intersect_point, &v_temp, plane_normal, plane_point);
    }
...
/**
 * Returns whether a sphere hits and edge for the case the edge is
...
int fvi_sphere_perp_edge (
    vec3d* intersect_point, const vec3d* sphere_center_start,
    const vec3d* sphere_velocity, float sphere_radius, vec3d* edge_point1,
    vec3d* edge_point2, float* collide_time) {
    // find the intersection in the plane normal to sphere velocity and edge
    // velocity choose a plane point V0 (first vertex of the edge) project
    // vectors and points into the plane find the projection of the
    // intersection and see if it lies on the edge
    ...
    return (check_sphere_point (
        intersect_point, sphere_center_start, sphere_velocity, sphere_radius,
        collide_time));
}
...
int fvi_polyedge_sphereline (
    ...
    for (i = 0; i < nv; i++) {
        ...
        v0 = *verts[i];
        if (i + 1 != nv) { v1 = *verts[i + 1]; }
        else {
            v1 = *verts[0];
        }

        // Calculate edge velocity.
        // Position along the edge is given by: P_edge = v0 + ve*t, where t is
        // in the range [0,1]
        vm_vec_sub (&ve, &v1, &v0);

        // First find the closest intersection between the edge_line and the
        // sphere_line.
        ...
        C = delta_x_dot_ve * delta_x_dot_ve + Rs2 * ve_sqr -
            delta_x_sqr * ve_sqr;
        ...
        if (discriminant > 0.0f) {
            ...
            if ((root1 < 0.0f) && (root1 >= -0.05f)) root1 = 0.000001f;

            // look only at first hit
            if ((root1 <= 1.0f) && (root1 >= 0.0f)) { t_sphere_hit = root1; }
            else {
                goto TryVertex;
            }
        }
        ...
        if (t_sphere_hit >= best_sphere_time) continue;
        ...
        C = 2.0f * delta_x_dot_ve * delta_x_dot_vs * ve_dot_vs +
            Rs2 * ve_dot_vs * ve_dot_vs - delta_x_sqr * ve_dot_vs * ve_dot_vs -
            delta_x_dot_ve * delta_x_dot_ve * vs_sqr;
        ...
        // given sphere position, find which edge time (position) allows a
        // valid solution
 ...
 */
int fvi_check_sphere_sphere (
    const vec3d* x_p0, const vec3d* x_p1, const vec3d* x_s0, const vec3d* x_s1,
    float radius_p, float radius_s, float* t1, float* t2) {
    vec3d delta_x, delta_v;
    float discriminant, separation, delta_x_dot_delta_v, delta_v_sqr,
        delta_x_sqr;
    float time1, time2;
    ...
    discriminant = delta_x_dot_delta_v * delta_x_dot_delta_v -
                   delta_v_sqr * (delta_x_sqr - separation * separation);
 ...
 *
 * Polygon face is characterized by a center and a radius.  This routine checks
 ...
 */
int fvi_cull_polyface_sphere (
    const vec3d* poly_center, float poly_r, const vec3d* sphere_start,
    const vec3d* sphere_end, float sphere_r) {
    vec3d closest_point, closest_separation;
    float max_sep;

    fvi_closest_point_on_line_segment (
        &closest_point, poly_center, sphere_start, sphere_end);
    vm_vec_sub (&closest_separation, &closest_point, poly_center);

    max_sep = vm_vec_mag (&closest_separation) + poly_r;

    if (max_sep > sphere_r) { return 0; }
    else {
        return 1;
    }
}
 ...
 */
void fvi_closest_line_line (
    const vec3d* x0, const vec3d* vx, const vec3d* y0, const vec3d* vy,
    float* x_time, float* y_time) {
