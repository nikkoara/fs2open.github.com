...
void multi_data_reset () {
    ...
    for (idx = 0; idx < MAX_MULTI_DATA; idx++) { Multi_data[idx].used = 0; }
}
...
void multi_data_do () {
    ...
    if ((time (NULL) - Multi_data_time) < 1) { return; }
    ...
    if (Net_player && (Net_player->flags & NETINFO_FLAG_AM_MASTER)) {
        // for all valid files
        for (idx = 0; idx < MAX_MULTI_DATA; idx++) {
            // a valid file that isn't currently xferring (ie, anything we've
            // got completely on our hard drive)
            if (Multi_data[idx].used &&
                (multi_xfer_lookup (Multi_data[idx].filename) < 0)) {
                // send it to all players who need it
                for (p_idx = 0; p_idx < MAX_PLAYERS; p_idx++) {
                    // if he doesn't have it
                    if ((Net_player != &Net_players[p_idx]) &&
                        MULTI_CONNECTED (Net_players[p_idx]) &&
                        (Net_players[p_idx].reliable_socket !=
                         INVALID_SOCKET) &&
                        (Multi_data[idx].status[p_idx] == 0)) {
                        // queue up the file to send to him, or at least try to
                        if (multi_xfer_send_file (
                                Net_players[p_idx].reliable_socket,
...
void multi_data_handle_incoming (int handle) {
    ...
    // if i'm a client, this is an incoming file from the server
        ...
        // set the xfer handle to autodestroy itself since we don't really want
        // to have to manage all incoming pics
        multi_xfer_xor_flags (handle, MULTI_XFER_FLAG_AUTODESTROY);
...
void multi_data_send_my_junk () {
    ...
    if (ok_to_send) {
        with_ext =
            cf_add_ext (Net_player->m_player->image_filename, NOX (".pcx"));
        if (with_ext != NULL) {
        ...
        if ((Net_player->flags & NETINFO_FLAG_AM_MASTER) &&
            ...
            (Net_player->m_player->image_filename[0] != '\0')) {
            multi_data_add_new (
                Net_player->m_player->image_filename, MY_NET_PLAYER_NUM);
        }
        // otherwise clients should just queue up a send
        else {
            // add a file extension if necessary
            multi_xfer_send_file (
                Net_player->reliable_socket,
                Net_player->m_player->image_filename, CF_TYPE_ANY,
    ...
    if (ok_to_send) {
        with_ext =
            cf_add_ext (Net_player->m_player->m_squad_filename, NOX (".pcx"));
        if (with_ext != NULL) {
        ...
        if ((Net_player->flags & NETINFO_FLAG_AM_MASTER) &&
            !(Game_mode & GM_STANDALONE_SERVER) &&
            (Net_player->m_player->m_squad_filename[0] != '\0')) {
            multi_data_add_new (
                Net_player->m_player->m_squad_filename, MY_NET_PLAYER_NUM);
        }
        // otherwise clients should just queue up a send
        else {
            // add a file extension if necessary
            multi_xfer_send_file (
                Net_player->reliable_socket,
                Net_player->m_player->m_squad_filename, CF_TYPE_ANY,
                MULTI_XFER_FLAG_AUTODESTROY | MULTI_XFER_FLAG_QUEUE);
        }
    }
}
...
int multi_data_is_data (char* filename) {
    ...
    if (strstr (filename, NOX (".pcx"))) { return 1; }
...
int multi_data_get_free () {
    ...
    // find a free slot
    for (idx = 0; idx < MAX_MULTI_DATA; idx++) {
        if (!Multi_data[idx].used) { return idx; }
    }
...
int multi_data_add_new (char* filename, int player_index) {
    ...
    Multi_data[slot].player_id =
        Net_players[player_index]
            .player_id; // player id of who's sending the file
...
void multi_data_maybe_reload () {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            strlen (Net_players[idx].m_player->m_squad_filename) &&
            (Net_players[idx].m_player->insignia_texture == -1)) {
            Net_players[idx].m_player->insignia_texture = bm_load_duplicate (
                Net_players[idx].m_player->m_squad_filename);

            // if the bitmap loaded properly, lock it as a transparent texture
            if (Net_players[idx].m_player->insignia_texture != -1) {
                bm_lock (
                    Net_players[idx].m_player->insignia_texture, 16,
                    BMP_TEX_XPARENT);
