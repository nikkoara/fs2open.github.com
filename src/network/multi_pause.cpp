...
// state of the game (paused or not) on _my_ machine. Obviously this is
// important for the server call multi_pause_reset() to reinitialize
...
const char* Multi_paused_bg_fname[GR_NUM_RESOLUTIONS] = { "MPPause",
                                                          "2_MPPause" };

const char* Multi_paused_bg_mask[GR_NUM_RESOLUTIONS] = { "MPPause-m",
                                                         "2_MPPause-m" };

// where to place the pauser's callsign
int Mp_callsign_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                    110, 132 },
                                                  { // GR_1024
                                                    171, 218 } };

ui_button_info
    Multi_paused_buttons[GR_NUM_RESOLUTIONS][MULTI_PAUSED_NUM_BUTTONS] = {
        {
            // GR_640
...
UI_XSTR Multi_paused_text[GR_NUM_RESOLUTIONS][MULTI_PAUSED_NUM_BUTTONS] = {
    {
        // GR_640
        { "Exit", 1059, 493, 297, UI_XSTR_COLOR_PINK, -1,
          &Multi_paused_buttons[0][MP_EXIT_MISSION].button },
        { "Mission", 1063, 482, 306, UI_XSTR_COLOR_PINK, -1,
          &Multi_paused_buttons[0][MP_EXIT_MISSION].button },
        { "Mission Paused", 1440, 107, 356, UI_XSTR_COLOR_PINK, -1, NULL },
    },
    {
        // GR_1024
        { "Exit", 1059, 787, 478, UI_XSTR_COLOR_PINK, -1,
          &Multi_paused_buttons[1][MP_EXIT_MISSION].button },
        { "Mission", 1063, 778, 490, UI_XSTR_COLOR_PINK, -1,
          &Multi_paused_buttons[1][MP_EXIT_MISSION].button },
...
// (server) evaluate a pause request from the given player (should call for
// himself as well)
...
void multi_pause_pause () {
    ...
    // if we're the server of the game, send a packet which will pause the
    // clients in the game now
...
void multi_pause_unpause () {
    ...
    // if we're the server of the game, send a packet which will unpause the
    // clients in the game now if we're the server of the game, send a packet
    // which will pause the clients in the game now
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        for (idx = 0; idx < MAX_PLAYERS; idx++) {
            if (MULTI_CONNECTED (Net_players[idx]) &&
                (Net_player != &Net_players[idx])) {
...
// send a request to pause or unpause a game (all players should use this
// function)
...
void multi_pause_server_eval_request (net_player* pl, int pause) {
    ...
    // if this is a pause request and we're already in the pause state, do
    // nothing
    if (pause && Multi_pause_status) { return; }
    ...
    if ((cur_state == GS_STATE_DEBRIEF) ||
        (cur_state == GS_STATE_MULTI_MISSION_SYNC) ||
        (cur_state == GS_STATE_BRIEFING) ||
        (cur_state == GS_STATE_STANDALONE_POSTGAME) ||
        (cur_state == GS_STATE_MULTI_STD_WAIT) ||
        (cur_state == GS_STATE_WEAPON_SELECT) ||
        (cur_state == GS_STATE_TEAM_SELECT) ||
        (cur_state == GS_STATE_MULTI_DOGFIGHT_DEBRIEF)) {
        return;
    }
...
int multi_pause_eat_keys () {
    ...
    if (Multi_pause_eat < 0.0f) { return 0; }
...
void multi_pause_init () {
    ...
    // everyone else should be doing UI stuff
        ...
        // create ui window
        Multi_paused_window.create (
            0, 0, gr_screen.max_w_unscaled, gr_screen.max_h_unscaled, 0);
        Multi_paused_window.set_mask_bmap (
            Multi_paused_bg_mask[gr_screen.res]);
        Multi_paused_background =
            bm_load (Multi_paused_bg_fname[gr_screen.res]);

        for (i = 0; i < MULTI_PAUSED_NUM_BUTTONS; i++) {
            // create the button
            Multi_paused_buttons[gr_screen.res][i].button.create (
                &Multi_paused_window, "",
                Multi_paused_buttons[gr_screen.res][i].x,
                Multi_paused_buttons[gr_screen.res][i].y, 1, 1, 0, 1);

            // set the highlight action
            Multi_paused_buttons[gr_screen.res][i]
        ...
        for (i = 0; i < MULTI_PAUSED_NUM_TEXT; i++) {
            Multi_paused_window.add_XSTR (
                &Multi_paused_text[gr_screen.res][i]);
        }
...
void multi_pause_do () {
    ...
    // server of the game should periodically be sending pause packets for good
    // measure
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {}
    ...
    if (!(Game_mode & GM_STANDALONE_SERVER)) {
        ...
        if (!popup_active ()) {
            ...
            switch (k) {
            case KEY_ESC:
            case KEY_PAUSE: multi_pause_request (0); break;
            }
        }
...
void multi_pause_button_pressed (int n) {
    ...
    case MP_SCROLL_UP: chatbox_scroll_up (); break;

    // the scroll down button
    case MP_SCROLL_DOWN: chatbox_scroll_down (); break;

    // the exit mission
    case MP_EXIT_MISSION: multi_quit_game (PROMPT_ALL); break;
    }
}
...
void multi_pause_render_callsign () {
    ...
    if ((Multi_pause_pauser != NULL) &&
        (Multi_pause_pauser->m_player != NULL)) {
        ...
        gr_string (
            Mp_callsign_coords[gr_screen.res][0],
            Mp_callsign_coords[gr_screen.res][1], pause_str, GR_RESIZE_MENU);
    }
}
