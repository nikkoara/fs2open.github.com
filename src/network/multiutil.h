...
// two types of signatures that we can request,  permanent signatures are all
// below 1000.  non-permanent are above 1000
...
extern void stuff_netplayer_info (
...
extern int find_player_socket (
    PSNET_SOCKET_RELIABLE sock); // note this is only valid to do on a server!
...
extern int multi_find_player_by_ship_name (
    const char* ship_name, bool inc_respawning = false);
extern int multi_create_player (
    int player_num, player* pl, const char* name, net_addr* addr,
...
extern object*
multi_get_network_object (ushort net_signature); // find a network object
...
// catchall to do any necessary client-side simulation processing or master
// side process for menu pauses, etc.
...
int lookup_ship_status (
    net_player* p, int unique_id,
    int remove = 0); // auto-remove if remove == 1
...
int multi_netplayer_state_check2 (
    int state, int state2, int ignore_standalone = 0);
int multi_netplayer_state_check3 (
    int state, int state2, int state3, int ignore_standalone = 0);
...
void multi_maybe_send_repair_info (
    object* dest_obj, object* source_objp, int code);
...
// determine whether (as a server), you should be rebroadcasting certain
...
// called to handle any special cases where a player is in some submenu when he
// needs to get pushed into some other state
...
void multi_process_valid_join_request (
    join_request* jr, net_addr* who_from, int ingame_join_team = -1);
...
// Karajorma - Performs any cleanup needed by missions which don't end with a
// warpout.
