...
// leaving, handling the server leaving, and notifying the user of all of these
// actions, here.
//
...
// set when the server/client has ended the game on some notification or error
// and is waiting for clients to leave
...
void multi_endgame_init () {
    ...
    // reset the timestamp used when server is waiting for all other clients to
    // leave.
...
void multi_endgame_process () {
    if (Multi_endgame_processing) return;

    Multi_endgame_processing = 1;

    // check to see if we need to be warping out (strange transition
    // possibilities)
        ...
        if (!multi_endgame_server_ok_to_leave ()) {
            if (Game_mode & GM_STANDALONE_SERVER) {
                ...
                popup_till_condition (
                    multi_endgame_server_ok_to_leave, XSTR ("&Cancel", 645),
...
// general quit function, with optional notification, error, and winsock error
// codes
int multi_quit_game (
    int prompt, int notify_code, int err_code, int wsa_error) {
    int ret_val, quit_already;

    // check for reentrancy
    if (Multi_quit_game) { return 0; }
    ...
    // reset my control info so that I don't continually do whacky stuff.  This
    // is ugly
    ...
    if (Game_mode & GM_IN_MISSION) {
        ...
        physics_read_flying_controls (
            &Player_obj->orient, &Player_obj->phys_info, &(Player->ci),
            flFrametime);
    }

    // CASE 1 - response to a user request
    // if there is no associated notification or error code, don't override the
    // prompt argument
    if ((err_code == -1) && (notify_code == -1)) {
        // if we're the server and we're already waiting for clients to leave,
        // don't do anything
        if ((Net_player->flags & NETINFO_FLAG_AM_MASTER) &&
            Multi_endgame_server_waiting) {
            Multi_quit_game = 0;
            return 0;
        }

        // if we're the client and we're already waiting to leave, don't do
        // anythin
        if (!(Net_player->flags & NETINFO_FLAG_AM_MASTER) &&
            Multi_endgame_client_waiting) {
            Multi_quit_game = 0;
            return 0;
        }

        // see if we should be prompting the host for confirmation
        if ((prompt == PROMPT_HOST || prompt == PROMPT_ALL) &&
            (Net_player->flags & NETINFO_FLAG_GAME_HOST)) {
            int p_flags;

            p_flags =
                PF_USE_AFFIRMATIVE_ICON | PF_USE_NEGATIVE_ICON | PF_BODY_BIG;
            if (Game_mode & GM_IN_MISSION) p_flags |= PF_RUN_STATE;

            ret_val = popup (
                p_flags, 2, POPUP_CANCEL, POPUP_OK,
                XSTR (
            ...
            // set this so that under certain circumstances, we don't call the
            // popup below us as well
            quit_already = 1;
        }

        // see if we should be prompting the client for confirmation
        if ((prompt == PROMPT_CLIENT || prompt == PROMPT_ALL) &&
            !quit_already) {
            ret_val = popup (
                PF_USE_AFFIRMATIVE_ICON | PF_USE_NEGATIVE_ICON | PF_BODY_BIG,
                2, POPUP_NO, POPUP_YES,
        ...
        // if i'm the server of the game, tell all clients that i'm leaving,
        // then wait
        if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
            send_netgame_end_error_packet (
                MULTI_END_NOTIFY_SERVER_LEFT, MULTI_END_ERROR_NONE);
    ...
    // this is the case where we're being forced to quit the game because of
    // some error or other notification
    else {
        // if i'm the server, send a packet to the clients telling them that
        // I'm leaving and why
        if ((Net_player->flags & NETINFO_FLAG_AM_MASTER) &&
            !Multi_endgame_server_waiting) {
            // if we're in the debrief state, mark down that the server has
            // left the game
            if (((gameseq_get_state () == GS_STATE_DEBRIEF) ||
                ...
                multi_display_chat_msg (
                    XSTR ("<Team captains have left>", 649), 0, 0);
            ...
            // by setting this, multi_endgame_process() will know to check and
            // see if it is ok for us to leave
            Multi_endgame_server_waiting = 1;
            Multi_endgame_server_wait_stamp = MULTI_ENDGAME_SERVER_WAIT;
        }
        // if i'm the client, set the error codes and leave the game now
            ...
            if ((gameseq_get_state () == GS_STATE_DEBRIEF) ||
                ...
                // add a message to the chatbox
                multi_display_chat_msg (
                    XSTR ("<The server has ended the game>", 650), 0, 0);
...
void multi_endgame_cleanup () {
    ...
    if (!(Game_mode & GM_STANDALONE_SERVER)) {
        Net_player->flags &=
            ~(NETINFO_FLAG_CONNECTED | NETINFO_FLAG_DO_NETWORKING);
    }

    /*this is a semi-hack so that if we're the master and we're quitting, we
    don't get an assert

    Karajorma - From the looks of things this code actually CAUSES an Int3 and
    doesn't cause an assert anymore besides if the game is over why are we
    setting flags on a Player_obj anyway?
    ...
    */
    ...
    // shut my socket down (will also let the server know i've received any
    // notifications/error from him) psnet_rel_close_socket(
    // &(Net_player->reliable_socket) );

    // 11/18/98 - DB, changed the above to kill all sockets. Its the safest
    // thing to do
    ...
    // set the game quitting flag in our local netgame info - this will
    // _insure_ that even if we miss a packet or there is some sequencing
    // error, the next time through the multi_do_frame() loop, the game will be
    // ended Netgame.flags |= (NG_FLAG_QUITTING | NG_FLAG_ENDED);
    ...
    // handle game disconnect from FS2NetD (NOTE: must be done *before*
    // standalone is reset!!)
    ...
    if (Game_mode & GM_STANDALONE_SERVER) {
        ...
        if (((Multi_endgame_notify_code != -1) ||
             (Multi_endgame_error_code != -1)) &&
            !(Game_mode & GM_STANDALONE_SERVER)) {
            multi_endgame_popup (
                Multi_endgame_notify_code, Multi_endgame_error_code,
                Multi_endgame_wsa_error);
        }
    }
    ...
    if (popup_active ()) {
        ...
        if (notify_code != MULTI_END_NOTIFY_NONE) {
            ...
            case MULTI_END_NOTIFY_INGAME_TIMEOUT:
                strcat_s (
                    err_msg,
                    XSTR ("You have waited too long to select a ship", 655));
                break;
            ...
            case MULTI_END_NOTIFY_KICKED_CANT_XFER:
                strcat_s (
                    err_msg, XSTR (
                                 "You were kicked because you do not have the "
                                 "builtin mission",
                                 999));
            ...
            case MULTI_END_NOTIFY_KICKED_INGAME_ENDED:
                strcat_s (
                    err_msg, XSTR (
                                 "You were kicked because you were ingame "
                                 "joining a game that has ended",
                                 1000));
                break;
            default: Int3 ();
            }
            ...
            case MULTI_END_ERROR_CONTACT_LOST:
                strcat_s (
                    err_msg, XSTR ("Contact with server has been lost", 656));
                break;
            ...
            case MULTI_END_ERROR_LOAD_FAIL:
                strcat_s (
                    err_msg,
                    XSTR ("Failed to load mission file properly", 658));
                break;
            case MULTI_END_ERROR_INGAME_SHIP:
                strcat_s (
                    err_msg,
                    XSTR ("Unable to create ingame join player ship", 659));
                break;
            ...
            case MULTI_END_ERROR_STRANS_FAIL:
                strcat_s (
                    err_msg, XSTR ("Server transfer failed (obsolete)", 661));
                break;
            case MULTI_END_ERROR_SHIP_ASSIGN:
                strcat_s (
                    err_msg, XSTR (
                                 "Server encountered errors trying to assign "
                                 "players to ships",
                                 662));
                break;
            case MULTI_END_ERROR_HOST_LEFT:
                strcat_s (
                    err_msg,
                    XSTR ("Host has left the game, aborting...", 663));
                break;
            ...
            case MULTI_END_ERROR_WAVE_COUNT:
                strcat_s (
                    err_msg, XSTR (
                                 "The player wings Alpha, Beta, Gamma, and "
                                 "Zeta must have only 1 wave.  One of these "
                                 "wings currently has more than 1 wave.",
                                 987));
                break;
            // Karajorma - both of these should really be replaced with new
            // strings in strings.tbl but for now this one has much the same
            // meaning
            case MULTI_END_ERROR_TEAM0_EMPTY:
                strcat_s (
                    err_msg,
                    XSTR ("All players from team 1 have left the game", 664));
                break;
            case MULTI_END_ERROR_TEAM1_EMPTY:
                strcat_s (
                    err_msg,
                    XSTR ("All players from team 2 have left the game", 664));
                break;
...
int multi_endgame_server_ok_to_leave () {
    ...
    if (Multi_endgame_server_wait_stamp > 0.0f) {
        ...
        if (Multi_endgame_server_wait_stamp <= 0.0f) { return 1; }
    }
...
void multi_endgame_check_for_warpout () {
    ...
    if ((Game_mode & GM_IN_MISSION) && // if i'm still in the mission
        ((Netgame.game_state ==
          NETGAME_STATE_ENDGAME) || // if the netgame ended
         (Netgame.game_state ==
          NETGAME_STATE_DEBRIEF)) // if the netgame is now in the debriefing
                                  // state
    ) {
        need_to_warpout = 1;
    }
    ...
    if (need_to_warpout && (popupdead_is_active () ||
