...
void output_bits (BitBuf* bitbuf, uint code, int count) {
    ...
    mask = 1L << (count - 1);
    while (mask != 0) {
        if (mask & code) bitbuf->rack |= bitbuf->mask;
        bitbuf->mask >>= 1;
        if (bitbuf->mask == 0) {
...
uint input_bits (BitBuf* bitbuf, int bit_count) {
    ...
    while (mask != 0) {
        if (bitbuf->mask == 0x80) {
            ...
            if (bitbuf->rack == EOF) return END_OF_STREAM;
        }
        if (bitbuf->rack & bitbuf->mask) return_value |= mask;
        mask >>= 1;
        bitbuf->mask >>= 1;
        if (bitbuf->mask == 0) bitbuf->mask = 0x80;
    }
    return (return_value);
}
...
static uint find_child_node (int parent_code, int child_character) {
    ...
    for (;;) {
        if (dict[index].code_value == UNUSED) return ((uint)index);
...
int lzw_compress (ubyte* outputbuf, ubyte* inputbuf, int input_size) {
    ...
    for (i = 1; i < input_size; i++) {
        ...
        if (dict[index].code_value != -1)
            ...
            output_bits (
                &output, (unsigned long)string_code, current_code_bits);
            string_code = character;
            if (next_code > MAX_CODE) {
                output_bits (
                    &output, (unsigned long)FLUSH_CODE, current_code_bits);
                InitializeDictionary ();
            }
            else if (next_code > next_bump_code) {
                output_bits (
                    &output, (unsigned long)BUMP_CODE, current_code_bits);
                current_code_bits++;
                next_bump_code <<= 1;
                next_bump_code |= 1;
            }
    ...
    if (output.mask != 0x80) *output.data++ = (ubyte)output.rack;
...
int lzw_expand (ubyte* outputbuf, ubyte* inputbuf) {
    ...
    for (;;) {
        ...
        if (old_code == END_OF_STREAM) return counter;
        character = old_code;
        outputbuf[counter++] = (ubyte)old_code;
        for (;;) {
            new_code = (uint)input_bits (&input, current_code_bits);
            if (new_code == END_OF_STREAM) return counter;
...
void multi_io_send (net_player* pl, ubyte* data, int len) {
    // invalid
    if ((pl == NULL) || (NET_PLAYER_NUM (pl) >= MAX_PLAYERS)) { return; }
    ...
    // If this packet will push the buffer over MAX_PACKET_SIZE, send the
    // current send_buffer
    ...
    memcpy (
        pl->s_info.unreliable_buffer + pl->s_info.unreliable_buffer_size, data,
        len);
...
void multi_io_send_to_all (ubyte* data, int length, net_player* ignore) {
    ...
    for (i = 0; i < MAX_PLAYERS; i++) {
        ...
        if ((ignore != NULL) && (&Net_players[i] == ignore)) { continue; }

        // ingame joiners not waiting to select a ship doesn't get any packets
        if ((Net_players[i].flags & NETINFO_FLAG_INGAME_JOIN) &&
            !(Net_players[i].flags & INGAME_JOIN_FLAG_PICK_SHIP)) {
            continue;
        }
...
void multi_io_send_force (net_player* pl) {
    ...
    if (MULTIPLAYER_MASTER) {
        psnet_send (
            &pl->p_info.addr, pl->s_info.unreliable_buffer,
            pl->s_info.unreliable_buffer_size, NET_PLAYER_NUM (pl));
        ...
        psnet_send (
            &Netgame.server_addr, pl->s_info.unreliable_buffer,
            pl->s_info.unreliable_buffer_size, NET_PLAYER_NUM (pl));
    }
    pl->s_info.unreliable_buffer_size = 0;
}
...
void multi_io_send_reliable (net_player* pl, ubyte* data, int len) {
    ...
    memcpy (
        pl->s_info.reliable_buffer + pl->s_info.reliable_buffer_size, data,
        len);
    pl->s_info.reliable_buffer_size += len;
}

void multi_io_send_to_all_reliable (
    ubyte* data, int length, net_player* ignore) {
    int i;
    Assert (MULTIPLAYER_MASTER);
...
void multi_io_send_reliable_force (net_player* pl) {
    ...
    if (MULTIPLAYER_MASTER) {
        psnet_rel_send (
            pl->reliable_socket, pl->s_info.reliable_buffer,
            pl->s_info.reliable_buffer_size, NET_PLAYER_NUM (pl));
    }
    else if (Net_player != NULL) {
        psnet_rel_send (
            Net_player->reliable_socket, pl->s_info.reliable_buffer,
            pl->s_info.reliable_buffer_size, NET_PLAYER_NUM (pl));
    }
    pl->s_info.reliable_buffer_size = 0;
}
...
void send_game_chat_packet (
    net_player* from, const char* msg, int msg_mode, net_player* to,
    const char* expr, int server_msg) {
    ...
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        ...
        case MULTI_MSG_ALL:
            for (idx = 0; idx < MAX_PLAYERS; idx++) {
                if (MULTI_CONNECTED (Net_players[idx]) &&
                    ...
                    multi_io_send_reliable (
                        &Net_players[idx], data, packet_size);
                }
            }
            break;
        ...
        case MULTI_MSG_FRIENDLY:
            for (idx = 0; idx < MAX_PLAYERS; idx++) {
                if (MULTI_CONNECTED (Net_players[idx]) &&
                    !MULTI_STANDALONE (Net_players[idx]) &&
                    (&Net_players[idx] != from) &&
                    (Net_players[idx].p_info.team == from->p_info.team)) {
                    multi_io_send_reliable (
                        &Net_players[idx], data, packet_size);
                }
            }
        ...
        case MULTI_MSG_EXPR:
            ...
            for (idx = 0; idx < MAX_PLAYERS; idx++) {
                if (MULTI_CONNECTED (Net_players[idx]) &&
                    !MULTI_STANDALONE (Net_players[idx]) &&
                    (&Net_players[idx] != from) &&
                    multi_msg_matches_expr (&Net_players[idx], expr)) {
                    multi_io_send_reliable (
                        &Net_players[idx], data, packet_size);
                    undeliverable = false;
                }
        ...
        if (undeliverable) {
            ...
            case MULTI_MSG_EXPR:
                ...
                if (from == Net_player) {
                    multi_display_chat_msg (
                        "Unable to send message, player does not exist", 0, 0);
                }
                // otherwise send a message back to the player
                else {
                    send_game_chat_packet (
                        Net_player,
                        "Unable to send message, player does not exist",
...
// process a general game chat packet, if we're the standalone we should
// rebroadcast
void process_game_chat_packet (ubyte* data, header* hinfo) {
    ...
    to = -1;
    switch (mode) {
    case MULTI_MSG_TARGET: GET_SHORT (to); break;
    case MULTI_MSG_EXPR: GET_STRING (expr); break;
    }
    ...
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        ...
        // if we couldn't find who sent the message or who should be getting
        // the message, the bail
        if (((to_player_index == -1) && (mode == MULTI_MSG_TARGET)) ||
            (player_index == -1)) {
            return;
        }

        // determine if _I_ should be seeing the text
        if (Game_mode & GM_STANDALONE_SERVER) { should_display = 1; }
        // check against myself for several specific cases
        else {
            if ((mode == MULTI_MSG_ALL) ||
                ((mode == MULTI_MSG_FRIENDLY) &&
                 (Net_player->p_info.team ==
                  Net_players[player_index].p_info.team)) ||
                ((mode == MULTI_MSG_HOSTILE) &&
                 (Net_player->p_info.team !=
                  Net_players[player_index].p_info.team)) ||
                ((mode == MULTI_MSG_TARGET) &&
                 (MY_NET_PLAYER_NUM == to_player_index)) ||
                ((mode == MULTI_MSG_EXPR) &&
                 multi_msg_matches_expr (Net_player, expr))) {
                should_display = 1;
            }
        }

        // if we're the server of a game, we need to rebroadcast to all other
        // players
        switch (mode) {
        // individual target mission
        case MULTI_MSG_TARGET:
            // if I was the inteneded target, or we couldn't find the intended
            // target, don't rebroadcast
            if (to_player_index != MY_NET_PLAYER_NUM) {
                send_game_chat_packet (
                    &Net_players[player_index], msg, (int)mode,
                    &Net_players[to_player_index], NULL, server_msg);
            }
            break;
        // expression mode
        case MULTI_MSG_EXPR:
            send_game_chat_packet (
                &Net_players[player_index], msg, (int)mode, NULL, expr,
                server_msg);
            break;
        // all other modes
        default:
            send_game_chat_packet (
                &Net_players[player_index], msg, (int)mode, NULL, NULL,
                server_msg);
            break;
        }
    }
    ...
    if (should_display) {
        if (server_msg == 2) { HUD_printf ("%s", msg); }
        else {
...
void process_hud_message (ubyte* data, header* hinfo) {
    ...
    // this is the only safe place to do this since only in the mission is the
    // HUD guaranteed to be inited
...
void process_join_packet (ubyte* data, header* hinfo) {
    ...
    // determine if we should accept this guy, or return a reason we should
    // reject him see the DENY_* codes in multi.h
    ...
    case JOIN_QUERY_RESTRICTED:
        ...
        if ((Netgame.type_flags & NG_TYPE_TEAM) &&
            Netgame.mode == NG_MODE_RESTRICTED) {
            ...
            if (team0_avail && team1_avail) {
                ...
                sprintf (
                    join_string,
                    "Player %s has tried to join. Accept on team 1 or 2 ?",
                    jr.callsign);
            }
                ...
                sprintf (
                    join_string,
                    "Player %s has tried to join team 0, accept y/n ? ?",
                    jr.callsign);
            }
            else if (!team0_avail && team1_avail) {
                ...
                sprintf (
                    join_string,
                    "Player %s has tried to join team 1, accept y/n ?",
                    jr.callsign);
            }
        }
            ...
            sprintf (
                join_string,
                XSTR ("Player %s has tried to join, accept y/n ?", 715),
                jr.callsign);
        }
        ...
        // NETLOG
        ml_printf (
            NOX ("Receive restricted join request from %s"), jr.callsign);
...
void send_new_player_packet (int new_player_num, net_player* target) {
    ...
    add_net_addr (
        data, &packet_size, &Net_players[new_player_num].p_info.addr);
    ...
    // broadcast the data
    if (target != NULL) { multi_io_send_reliable (target, data, packet_size); }
    else {
...
void process_new_player_packet (ubyte* data, header* hinfo) {
    ...
    // note that this new code does not check for duplicate IPs. It merely
    // checks to see if the slot referenced by new_player_num is already
    // occupied by a connected player
    if (MULTI_CONNECTED (Net_players[new_player_num])) { already_in_game = 1; }

    // if he's not alreayd in the game for one reason or another
    if (!already_in_game) {
        if (Game_mode & GM_IN_MISSION) {
            HUD_sourced_printf (
                HUD_SOURCE_COMPUTER, XSTR ("%s has entered the game\n", 716),
                new_player_name);
        }

        // create the player
        if (new_flags & NETINFO_FLAG_OBSERVER) {
            multi_obs_create_player (
                new_player_num, new_player_name, &new_addr,
                &Players[player_num]);
            Net_players[new_player_num].flags |= new_flags;
        }
        else {
            multi_create_player (
                new_player_num, &Players[player_num], new_player_name,
                &new_addr, -1, new_id);
            Net_players[new_player_num].flags |= new_flags;
        }
        ...
        if (new_player_image[0] != '\0') {
            strcpy_s (
                Net_players[new_player_num].m_player->image_filename,
                new_player_image);
        }
        else {
            strcpy_s (
        ...
        player_set_squad_bitmap (
            Net_players[new_player_num].m_player, new_player_squad, true);

        // copy in his pxo squad name
        strcpy_s (
            Net_players[new_player_num].p_info.pxo_squad_name,
            new_player_pxo_squad);

        // since we just created the player, set the last_heard_time here.
        Net_players[new_player_num].last_heard_time =
        ...
        if (*Net_players[new_player_num].m_player->callsign) {
            sprintf (
                notify_string, XSTR ("<%s has joined>", 717),
    ...
    // NETLOG
    ml_printf (
        NOX ("Received notification of new player %s"),
        Net_players[new_player_num].m_player->callsign);
...
void send_accept_player_data (net_player* npp, int is_ingame) {
    ...
    for (i = 0; i < MAX_PLAYERS; i++) {
        ...
        if (!MULTI_CONNECTED (Net_players[i])) { continue; }

        // skip this new player's entry
        if (npp->player_id == Net_players[i].player_id) { continue; }
        ...
        if (is_ingame) {
            ADD_USHORT (
                Objects[Net_players[i].m_player->objnum].net_signature);
        }
...
void process_accept_player_data (ubyte* data, header* hinfo) {
    ...
    while (stop == APD_NEXT) {
        ...
        if (Net_players[player_num].flags & NETINFO_FLAG_OBSERVER) {
            if (!multi_obs_create_player (
                    player_num, name, &addr, &Players[player_slot_num])) {
                Int3 ();
            }
        }
        else {
            //  the error handling here is less than stellar.  We should
            //  probably put up a popup and go
            // back to the main menu.  But then again, this should never ever
            // happen!
            if (!multi_create_player (
                    player_num, &Players[player_slot_num], name, &addr, -1,
                    player_id)) {
                Int3 ();
            }
        }
        ...
        strcpy_s (
            Net_players[player_num].m_player->image_filename, image_name);

        // copy his pilot squad filename
        Net_players[player_num].m_player->insignia_texture = -1;
        player_set_squad_bitmap (
            Net_players[player_num].m_player, squad_name, true);

        // copy his pxo squad name
        strcpy_s (
            Net_players[player_num].p_info.pxo_squad_name, pxo_squad_name);
        ...
        if (Net_players[player_num].flags & NETINFO_FLAG_AM_MASTER) {
            ...
            // also - always set the server address to be where this data came
            // from, NOT from the data in the packet
            fill_net_addr (
                &Net_players[player_num].p_info.addr, hinfo->addr,
                hinfo->port);
        }
        ...
        // read in the player's object net signature and store as his objnum
        // for now
    ...
    if (stop == APD_END_DATA) {
        // if joining a game automatically, set the connect address to NULl so
        // we don't try and do this next time we enter a game
        if (Cmdline_connect_addr != NULL) { Cmdline_connect_addr = NULL; }
        ...
        if (Net_player->flags & NETINFO_FLAG_ACCEPT_OBSERVER) {
            ...
            // since observers can join 1 of 2 ways, only do this if we're not
            // doing an ingame observer join
            if (!(Net_player->flags & NETINFO_FLAG_ACCEPT_INGAME)) {
...
void send_accept_packet (int new_player_num, int code, int ingame_join_team) {
    ...
    // if he's not an observer, inform all the other players in the game about
    // him inform the other players in the game about this new player
    for (i = 0; i < MAX_PLAYERS; i++) {
        // skip unconnected players as well as this new guy himself
        if (!MULTI_CONNECTED (Net_players[i]) ||
            psnet_same (
                &Net_players[new_player_num].p_info.addr,
                &(Net_players[i].p_info.addr)) ||
            (Net_player == &Net_players[i])) {
            continue;
        }
    ...
    if (Net_players[new_player_num].tracker_player_id >= 0) {
        ml_printf (
            NOX ("Server accepted %s (tracker id %d) as new client"),
            Net_players[new_player_num].m_player->callsign,
            Net_players[new_player_num].tracker_player_id);
    }
    else {
        ml_printf (
            NOX ("Server accepted %s as new client"),
            Net_players[new_player_num].m_player->callsign);
    }
}
...
void process_accept_packet (ubyte* data, header* hinfo) {
    ...
    // stuff_netplayer_info( Net_player, &Psnet_my_addr,
    // Ships[Objects[Player->objnum].instance].ship_info_index, Player );
    ...
    if (code & ACCEPT_HOST) { Net_player->flags |= NETINFO_FLAG_ACCEPT_HOST; }
    if (code & ACCEPT_CLIENT) {
    ...
    psnet_rel_connect_to_server (
        &Net_player->reliable_socket, &Netgame.server_addr);
    if (Net_player->reliable_socket == INVALID_SOCKET) {
        multi_quit_game (
            PROMPT_NONE, MULTI_END_NOTIFY_NONE, MULTI_END_ERROR_CONNECT_FAIL);
    }
}
...
void send_leave_game_packet (
    ...
    // this is the case where to server is tossing a player (or indicating a
...
void process_leave_game_packet (ubyte* data, header* hinfo) {
    ...
    if (kicked_reason >= 0) {
        ...
        if ((Net_player != NULL) && (Net_player->player_id == deader_id)) {
            ...
            case KICK_REASON_INGAME_ENDED:
                ...
                break;
            default: notify_code = MULTI_END_NOTIFY_KICKED; break;
            }
    ...
    // first of all, if we're the master, we should be rebroadcasting this
    // packet
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        char msg[255];

        sprintf (
            msg, XSTR ("%s has left the game", 719),
            Net_players[player_num].m_player->callsign);
...
void send_game_active_packet (net_addr* addr) {
    ...
    if ((Netgame.mode == NG_MODE_PASSWORD) ||
        ((Game_mode & GM_STANDALONE_SERVER) && (multi_num_players () == 0) &&
         (std_is_host_passwd ()))) {
        flags |= AG_FLAG_PASSWD;
    }

    // proper netgame type flags
    if (Netgame.type_flags & NG_TYPE_TEAM) { flags |= AG_FLAG_TEAMS; }
    ...
    case NETGAME_STATE_FORMING: flags |= AG_FLAG_FORMING; break;

    case NETGAME_STATE_BRIEFING:
    case NETGAME_STATE_MISSION_SYNC:
    case NETGAME_STATE_HOST_SETUP: flags |= AG_FLAG_BRIEFING; break;

    case NETGAME_STATE_IN_MISSION: flags |= AG_FLAG_IN_MISSION; break;

    case NETGAME_STATE_PAUSED: flags |= AG_FLAG_PAUSE; break;

    case NETGAME_STATE_ENDGAME:
    case NETGAME_STATE_DEBRIEF: flags |= AG_FLAG_DEBRIEF; break;
    }

    // if this is a standalone
    if (Game_mode & GM_STANDALONE_SERVER) { flags |= AG_FLAG_STANDALONE; }

    // if we're in campaign mode
    if (Netgame.campaign_mode == MP_CAMPAIGN) { flags |= AG_FLAG_CAMPAIGN; }
...
void process_game_active_packet (ubyte* data, header* hinfo) {
    ...
    // if this is a compatible version, and our modes are compatible, register
    // it
...
void send_netgame_update_packet (net_player* pl) {
    ...
    // only the server should ever send the netgame state (standalone
    // situation)
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        ADD_INT (Netgame.game_state);
    }

    // if we're the host on a standalone, send to the standalone and let him
    // rebroadcast
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        if (pl == NULL) {
            multi_io_send_to_all_reliable (data, packet_size);
        ...
        Assert (pl == NULL); // I don't think that a host in a standalone game
                             // would get here.
        multi_io_send_reliable (Net_player, data, packet_size);
    }
...
void process_netgame_update_packet (ubyte* data, header* hinfo) {
    ...
    GET_INT (Netgame.max_players); // ignore on the standalone, who keeps track
                                   // of this himself
    GET_INT (Netgame.security);
    GET_UINT (Netgame.respawn);

    // be sure not to blast the quitting flag because of the "one frame extra"
    // problem
    ...
    // now compare the passed in game state to our current known state.  If it
    // has changed, then maybe do something interesting. move from the forming
    // or debriefing state to the mission sync state
    if (ng_state == NETGAME_STATE_MISSION_SYNC) {
        ...
        if ((Netgame.game_state == NETGAME_STATE_FORMING) ||
            ((Netgame.game_state != NETGAME_STATE_FORMING) &&
             ((gameseq_get_state () == GS_STATE_MULTI_HOST_SETUP) ||
        ...
        // if coming from the debriefing state
            ...
            ((Netgame.game_state != NETGAME_STATE_DEBRIEF) &&
             ((gameseq_get_state () == GS_STATE_DEBRIEF) ||
              (gameseq_get_state () == GS_STATE_MULTI_DOGFIGHT_DEBRIEF)))) {
            // do any special processing for forced state transitions
            multi_handle_state_special ();
    ...
    // move from mission sync to team select
        ...
        if ((Netgame.game_state == NETGAME_STATE_MISSION_SYNC) ||
            ((Netgame.game_state != NETGAME_STATE_MISSION_SYNC) &&
             (gameseq_get_state () == GS_STATE_MULTI_MISSION_SYNC) &&
...
void send_netgame_descript_packet (net_addr* addr, int code) {
    ...
    Assert (addr != NULL);
    if (addr != NULL) { psnet_send (addr, data, packet_size); }
}
...
// broadcast a query for active games. TCP will either request from the MT or
...
void process_game_query (ubyte* /*data*/, header* hinfo) {
    ...
    fill_net_addr (&addr, hinfo->addr, hinfo->port);
    if (psnet_same (&addr, &Psnet_my_addr)) { return; }

    // if I am not a server of a game, don't send a reply!!!
    if (!(Net_player->flags & NETINFO_FLAG_AM_MASTER)) { return; }

    // if the game options are being selected, then ignore the request
    // also, if Netgame.max_players == -1, the host has not chosen a mission
    // yet and we should wait
    if ((Netgame.game_state == NETGAME_STATE_STD_HOST_SETUP) ||
...
// sends information about netplayers in the game. if called on the server,
// broadcasts information about _all_ players
void send_netplayer_update_packet (net_player* pl) {
    ...
    // if I'm the server of the game, I should send an update for _all_players
    // in the game
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        for (idx = 0; idx < MAX_PLAYERS; idx++) {
            ...
            if (MULTI_CONNECTED (Net_players[idx])) {
                ...
                if (Net_players[idx].flags & NETINFO_FLAG_HAS_CD) { val = 1; }
                else {
                    val = 0;
                }
                ADD_DATA (val);
        ...
        if (!(Game_mode & GM_IN_MISSION)) {
            ...
            if (pl == NULL) { multi_io_send_to_all (data, packet_size); }
            else {
        ...
        Assert (
            pl ==
            NULL); // shouldn't ever be the case that pl is non-null here.
        if (!(Game_mode & GM_IN_MISSION)) {
            multi_io_send_reliable (Net_player, data, packet_size);
        }
        else {
...
// process an incoming netplayer state update. if we're the server, we should
// rebroadcast
void process_netplayer_update_packet (ubyte* data, header* hinfo) {
    ...
    while (stop != 0xff) {
        ...
        // otherwise read in the data correctly
            ...
            if ((Net_players[player_num].state == NETPLAYER_STATE_JOINING) &&
                (new_state == NETPLAYER_STATE_JOINED) &&
    ...
    // if I'm the host or the server of the game, update everyone else so
    // things are synched up as tightly as possible
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        send_netplayer_update_packet (NULL);
    }

    // if i'm the standalone and this is an update from the host, maybe change
    // some netgame settings
    if ((Game_mode & GM_STANDALONE_SERVER) && (player_num != -1) &&
        (Net_players[player_num].flags & NETINFO_FLAG_GAME_HOST)) {
...
void send_ship_kill_packet (
    object* objp, object* other_objp, float percent_killed,
    int self_destruct) {
    ...
    extra_death_info = 0;
    if (vaporized) { extra_death_info |= EXTRA_DEATH_VAPORIZED; }

    if (Ships[objp->instance].wash_killed) {
        extra_death_info |= EXTRA_DEATH_WASHED;
    }

    // find out the next network signature that will be used for the debris
    // pieces.
    ...
    if (pm && !vaporized) {
        ...
        multi_set_network_signature (
            (ushort) (debris_signature + pm->num_debris_objects),
            MULTI_SIG_DEBRIS);
    ...
    // ships which are initially killed get the rest of the data sent.  self
    // destructed ships and
    if (other_objp == NULL) {
        ushort temp;
    ...
    // if the ship who died is a player, then send some extra info, like who
    // killed him, etc.
    was_player = 0;
    if (objp->flags[Object::Object_Flags::Player_ship]) {
        ...
        if (pnum != -1) {
            ...
            Assert (
                Net_players[pnum].m_player->killer_weapon_index < SHRT_MAX);
...
void process_ship_kill_packet (ubyte* data, header* hinfo) {
    ...
    char killer_name[NAME_LENGTH], killer_objtype = OBJ_NONE,
                                   killer_species = 0;
    ...
    // pnum is >=0 when the dying ship is a pleyer ship.  Get the info about
    // how he died
    if (was_player != 0) {
    ...
    if (was_player != 0) {
        ...
        if (pnum != -1) {
            ...
            Net_players[pnum].m_player->killer_weapon_index =
                killer_weapon_index;
            strcpy_s (
                Net_players[pnum].m_player->killer_parent_name, killer_name);
        }
    }
    ...
    // store the debris signature in the arrival distance which will never get
    // used for player ships
    Ships[sobjp->instance].debris_net_sig = debris_sig;

    // set this bit so that we don't accidentally start switching targets when
    // we die
...
void send_ship_create_packet (object* objp, int is_support) {
    ...
    ADD_USHORT (objp->net_signature);
    ADD_INT (is_support);
    if (is_support) { ADD_VECTOR (objp->pos); }
...
void process_ship_create_packet (ubyte* data, header* hinfo) {
    ...
    GET_INT (is_support);
    if (is_support) { GET_VECTOR (pos); }

    PACKET_SET_SIZE ();

    // find the name of this ship on ship ship arrival list.  if found, pass it
    // to parse_object_create
    if (!is_support) {
        ...
        if (Arriving_support_ship == NULL) { return; }
        ...
        Assert (objnum != -1);
        if (objnum >= 0) { mission_parse_support_arrived (objnum); }
    }
}

// send a packet indicating a wing of ships should be created
void send_wing_create_packet (
    wing* wingp, int num_to_create, int pre_create_count) {
    int packet_size, index, ship_instance;
    ubyte data[MAX_PACKET_SIZE];
    ushort signature;
    int val;

    // for creating wing -- we just send the index into the wing array of this
    // wing. all players load the same mission, and so their array's should all
    // match. We also need to send the signature of the first ship that was
    // created.  We can find this by looking num_to_create places back in the
    // ship_index field in the wing structure.
...
void process_wing_create_packet (ubyte* data, header* hinfo) {
    ...
    // do a sanity check on the wing to be sure that we are actually working on
    // a valid wing
    ...
    // set the network signature that was passed.  The client should create
    // ships in the same order as the server -- so all ships should get the
    // same sigs as assigned by the server.  We also need to set some
    // timestamps and cues correctly to be sure that these things get created
    // on the clients correctly
...
void send_secondary_fired_packet (
    ship* shipp, ushort starting_sig, int /*starting_count*/, int num_fired,
    int allow_swarm) {
    ...
    // get the object for this ship.  If it is an AI object, send all the info
    // to all player.  Otherwise, we might send the info to the other player
    // different than the one who fired
    objp = &Objects[shipp->objnum];
    if (!(objp->flags[Object::Object_Flags::Player_ship])) {
        if (num_fired == 0) { return; }
    }
    ...
    if (allow_swarm) { sinfo |= SFPF_ALLOW_SWARM; }

    if (shipp->flags[Ship::Ship_Flags::Secondary_dual_fire]) {
        sinfo |= SFPF_DUAL_FIRE;
    }

    if (aip->current_target_is_locked) { sinfo |= SFPF_TARGET_LOCKED; }
    ...
    if (aip->target_objnum != -1) {
        ...
        if ((Objects[aip->target_objnum].type == OBJ_SHIP) &&
            ...
            s_index = ship_get_index_from_subsys (
                aip->targeted_subsys, aip->target_objnum);
            Assert (s_index < CHAR_MAX); // better be less than this!!!!
            t_subsys = (char)s_index;
        }

        if (Objects[aip->target_objnum].type == OBJ_WEAPON) {
            Assert (Weapon_info[Weapons[Objects[aip->target_objnum].instance]
                                    .weapon_info_index]
    ...
    // getting here means a player fired.  Send the current packet to all
    // players except the player who fired.  If nothing got fired, then don't
    // send to the other players -- we will just send a packet to the player
    // who will find out that he didn't fire anything
    if (num_fired > 0) {
        multi_io_send_to_all_reliable (
            data, packet_size, &Net_players[net_player_num]);
    }

    // if I (the master) fired, then return
    if (Net_players[net_player_num].flags & NETINFO_FLAG_AM_MASTER) { return; }
    ...
    // add the targeting information so that the player's weapons will always
    // home on the correct ship
    ADD_USHORT (target_signature);
    ADD_DATA (t_subsys);
...
void process_secondary_fired_packet (
    ubyte* data, header* hinfo, int from_player) {
    ...
    // if from_player is false, it means that the secondary weapon info in this
    // packet was fired by an ai object (or another player).  from_player == 1
    // means tha me (the person receiving this packet) fired the secondary
    // weapon
    if (!from_player) {
        ...
        // find the object (based on network signatures) for the object that
        // fired
        ...
        // set up the ships current secondary bank and that bank's mode. Below,
        // we will set the timeout of the next fire time of this bank to 0 so
    ...
    allow_swarm = 0;
    if (sinfo & SFPF_ALLOW_SWARM) { allow_swarm = 1; }

    // set the dual fire properties of the ship
    shipp->flags.set (
        Ship::Ship_Flags::Secondary_dual_fire, (sinfo & SFPF_DUAL_FIRE) != 0);

    // determine whether current target is locked
    Assert (shipp->ai_index != -1);
    aip = &Ai_info[shipp->ai_index];
    if (sinfo & SFPF_TARGET_LOCKED) { aip->current_target_is_locked = 1; }
    else {
    ...
    shipp->weapons
        .next_secondary_fire_stamp[shipp->weapons.current_secondary_bank] =
        timestamp (0);
    shipp->weapons.detonate_weapon_time =
        timestamp (5000); // be sure that we don't detonate a remote weapon
                          // before it is time.

    // set this ship's target and subsystem information.  We will save and
    // restore target and targeted subsystem so that we do not accidentally
    // change targets for this player or any AI ships on his system.
    target_objnum_save = aip->target_objnum;
    targeted_subsys_save = aip->targeted_subsys;

    // reset these variables for accuracy.  They will get reassigned at the end
    // of this fuction
    aip->target_objnum = -1;
    aip->targeted_subsys = NULL;
    ...
    if (target_objp != NULL) {
        ...
        if ((t_subsys != -1) && (target_objp->type == OBJ_SHIP)) {
            aip->targeted_subsys = ship_get_indexed_subsys (
                &Ships[target_objp->instance], t_subsys);
        }
    }
    ...
    if (starting_sig != 0) {
        ...
        shipp->weapons.detonate_weapon_time =
            timestamp (0); // signature of -1 say detonate remote weapon
    }
...
void send_countermeasure_fired_packet (
    object* objp, int cmeasure_count, int rand_val) {
    ubyte data[MAX_PACKET_SIZE];
    int packet_size;
...
void send_turret_fired_packet (
    int ship_objnum, int subsys_index, int weapon_objnum) {
    int packet_size;
    ...
    // sanity
    if ((weapon_objnum < 0) || (Objects[weapon_objnum].type != OBJ_WEAPON) ||
        (Objects[weapon_objnum].instance < 0) ||
    ...
    if (Weapon_info[Weapons[objp->instance].weapon_info_index].subtype ==
        WP_MISSILE) {
        has_sig = 1;
    }
    ...
    ssp = ship_get_indexed_subsys (
        &Ships[Objects[ship_objnum].instance], subsys_index, NULL);
    if (ssp == NULL) { return; }

    // build the fire turret packet.
    BUILD_HEADER (FIRE_TURRET_WEAPON);
    packet_size += multi_pack_unpack_position (
        1, data + packet_size, &objp->orient.vec.fvec);
    ADD_DATA (has_sig);
    ADD_USHORT (pnet_signature);
    if (has_sig) { ADD_USHORT (objp->net_signature); }
    ADD_DATA (cindex);
...
void process_turret_fired_packet (ubyte* data, header* hinfo) {
    ...
    if (has_sig) { GET_USHORT (wnet_signature); }
    else {
        wnet_signature = 0;
    }
    ...
    GET_SHORT (heading);
    GET_SHORT (pitch);
    PACKET_SET_SIZE (); // move our counter forward the number of bytes we have
                        // read
    ...
    // find this turret, and set the position of the turret that just fired to
    // be where it fired.  Quite a hack, but should be suitable.
    shipp = &Ships[objp->instance];
    ...
    weapon_objnum = weapon_create (
        &pos, &orient, wid, OBJ_INDEX (objp), -1, 1, 0, 0.0f, ssp);

    if (weapon_objnum != -1) {
        wid = Weapons[Objects[weapon_objnum].instance].weapon_info_index;
        if (Weapon_info[wid].launch_snd.isValid ()) {
            snd_play_3d (
                gamesnd_get_game_sound (Weapon_info[wid].launch_snd), &pos,
                &View_position);
        }
    }
}
...
void send_mission_log_packet (log_entry* entry) {
    ...
    type =
        (ubyte)
            entry->type; // do the type casting thing to save on packet space
    sindex = entry->index;
    ...
    ADD_INT (entry->flags);
    ADD_INT (sindex);
    ADD_INT (entry->timestamp); // NOTE: this is a long so careful with
                                // swapping in 64-bit platforms - taylor
...
void process_mission_log_packet (ubyte* data, header* hinfo) {
    ...
    GET_INT (flags);
    GET_INT (sindex);
    GET_INT (timestamp); // NOTE: this is a long so careful with swapping in
                         // 64-bit platforms - taylor
    GET_STRING (pname);
    GET_STRING (sname);

    PACKET_SET_SIZE ();

    mission_log_add_entry_multi (
        static_cast< LogType > (type), pname, sname, sindex, timestamp, flags);
}

// send a mission message packet
void send_mission_message_packet (
    int id, const char* who_from, int priority, int timing, int source,
    int builtin_type, int multi_target, int multi_team_filter, int delay) {
    int packet_size;
...
void process_mission_message_packet (ubyte* data, header* hinfo) {
    ...
    if (!message_filter_multi (id)) {
        ...
        message_queue_message (
            id, priority, utiming, who_from, source, 0, delay, builtin_type);
    }
}
...
// right now it just routes the pong through to the standalone gui, which is
// the only system which uses ping and pong right now.
void process_pong_packet (ubyte* /*data*/, header* hinfo) {
    ...
    if (Net_player->flags & NETINFO_FLAG_CONNECTED) {
        ...
        // evaluate the ping
        multi_ping_eval_pong (
            &Net_players[lookup].s_info.ping, timer_get_fixed_seconds ());

        // put in calls to any functions which may want to know about the ping
        // times from this guy
        if (Game_mode & GM_STANDALONE_SERVER) { std_update_player_ping (p); }
    ...
    // otherwise, do any special processing
        ...
        // if we're in the join game state, see if this pong came from a server
        // on our list
...
#define MAX_MISSION_ITEMS_BYTES \
    (MAX_PACKET_SIZE - (sizeof (multi_create_info) + 1))
...
void send_mission_items (net_player* pl) {
    ...
    // send the list of missions and campaigns avilable on the server.  Stop
    // when reaching a certain maximum
...
void process_mission_item_packet (ubyte* data, header* hinfo) {
    ...
    // this will cause whatever list to get resorted (although they should be
    // appearing in order)
...
void process_multi_pause_packet (ubyte* data, header* hinfo) {
    ...
    // if we're the server, we should evaluate whether this guy is allowed to
    // send the packet
...
void send_game_info_packet () {
    ...
    ADD_INT (Missiontime); // NOTE: this is a long so careful with swapping in
                           // 64-bit platforms - taylor
    ADD_DATA (paused);
...
void process_game_info_packet (ubyte* data, header* hinfo) {
    ...
    // get the mission time -- we should examine our time and the time from the
    // server.  If off by some delta time, set our time to server time (should
    // take ping time into account!!!)
    GET_INT (mission_time); // NOTE: this is a long so careful with swapping in
                            // 64-bit platforms - taylor
    GET_DATA (paused);
    PACKET_SET_SIZE ();
}
...
// If the end_mission SEXP has been used tell clients to skip straight to the
// debrief screen
...
// process a packet indicating that we should jump straight to the debrief
// screen
void process_force_end_mission_packet (ubyte* /*data*/, header* hinfo) {
    ...
    // Since only the server sends out these packets it should never receive
    // one
    Assert (!(Net_player->flags & NETINFO_FLAG_AM_MASTER));
...
void process_endgame_packet (ubyte* /*data*/, header* hinfo) {
    ...
    // if I'm the server, I should evaluate whether the sender is authorized to
    // end the game
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        // determine who this came from and make sure he is allowed to end the
        // game
        player_num = find_player_id (hinfo->id);
        Assert (player_num != -1);
        ...
        if (!multi_can_end_mission (&Net_players[player_num])) { return; }
    ...
    // all clients process immediately
        ...
        // ingame joiners should quit when they receive an endgame packet since
        // the game is over
        if (Net_player->flags & NETINFO_FLAG_INGAME_JOIN) {
        ...
        // do any special processing for being in a state other than the
        // gameplay state
        multi_handle_state_special ();
...
void send_observer_update_packet () {
    ...
    // its possible for the master to be an observer if has run out of
    // respawns. In this case, he doesn't need to send any update packets to
    // anyone.
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) { return; }

    if ((Player_obj == NULL) || (Player_obj->type != OBJ_OBSERVER) ||
        (Net_player == NULL) || !(Net_player->flags & NETINFO_FLAG_OBSERVER)) {
        return;
    }
    ...
    packet_size += ret;
    ret =
        multi_pack_unpack_orient (1, data + packet_size, &Player_obj->orient);
    packet_size += ret;
...
void process_observer_update_packet (ubyte* data, header* hinfo) {
    ...
    // set targeting info
    if (target_sig == 0) { Net_players[obs_num].s_info.target_objnum = -1; }
    else {
        target_obj = multi_get_network_object (target_sig);
        Net_players[obs_num].s_info.target_objnum =
            (target_obj == NULL) ? -1 : OBJ_INDEX (target_obj);
    }
...
void send_netplayer_slot_packet () {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            ...
            ADD_SHORT (Net_players[idx].player_id);
            ADD_USHORT (
                Objects[Net_players[idx].m_player->objnum].net_signature);
            ADD_INT (Net_players[idx].p_info.ship_class);
...
void process_netplayer_slot_packet (ubyte* data, header* hinfo) {
    ...
    while (stop != 0x0) {
        ...
        if (player_num < 0) {
            ...
            // stuff being careful not to muck with the standalone object
    ...
    if (Game_mode & GM_STANDALONE_SERVER) { send_netplayer_slot_packet (); }

    Net_player->state = NETPLAYER_STATE_SLOT_ACK;
    send_netplayer_update_packet ();
}

// two functions to deal with ships changing their primary/secondary weapon
// status.  'what' indicates if this change is a primary or secondary change.
// new_bank is the new current primary/secondary bank, link_status is whether
// primaries are linked or not, or secondaries are dual fire or not
void send_ship_weapon_change (
    ...
    // Removed the above psnet_send() call - it didn't appear to do anything
    // since it was called only from the server anyway - DB
    multi_io_send_to_all_reliable (data, packet_size);
}
...
void process_ship_weapon_change (ubyte* data, header* hinfo) {
    ...
    // now, get the ship and set the new bank and link modes based on the
    // 'what' value
    shipp = &Ships[objp->instance];
    if (what == MULTI_PRIMARY_CHANGED) {
        ...
        shipp->flags.set (
            Ship::Ship_Flags::Secondary_dual_fire, link_status != 0);
    }
}

// ship status change procedure
// 1.) <client> - Client runs through the normal button_function procedure. Any
// remaining control bits are implied as being
//                server critical.
// 2.) <client> - Client puts this button_info item into his last_buttons array
// and sends a bunch of SHIP_STATUS packets
//                for added redundancy.
// 3.) <server> - Receives the packet. Checks to see if the net_player on his
// side already has this one defined. If so, it
//                ignores as a repeat packet. Otherwise it puts it in the
//                last_buttons array for the net_player
// 4.) <server> - Server applies the command on his side (with
// multi_apply_ship_status(...) and sends the ack (also a SHIP_STATUS)
//                back to the client. Also sends multiple times for redundancy
// 5.) <client> - Receives the packet back. Does a lookup into his last_buttons
// array. If he finds the match, apply the functions
//                and remove the item from the list. If no match is found it
//                means that either he has received an ack, has acted on it and
//                removed it, or that it has been "timed out" and replaced by a
//                newer button_info.
...
void send_ship_status_packet (net_player* pl, button_info* bi, int id) {
    ...
    if (MULTIPLAYER_MASTER) { multi_io_send_reliable (pl, data, packet_size); }
    else {
        multi_io_send (pl, data, packet_size);
    }
}
...
void process_ship_status_packet (ubyte* data, header* hinfo) {
    ...
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) { // SERVER SIDE
        ...
        if (!MULTI_OBSERVER (Net_players[player_num]) &&
            !lookup_ship_status (&Net_players[player_num], unique_id)) {
        ...
        // this is the return from the server, so we should now apply them
        // locally
...
// MWA 4/28/9 -- redid this function since message all fighers was really
// broken for clients.  Left all details to this function instead of higher
// level messaging code
void send_player_order_packet (int type, int index, int cmd) {
    ...
    // if we are not messaging all ships or wings, add the index, which is the
    // shipnum or wingnum
    ...
    if ((Player_ai->target_objnum != -1) &&
        ...
        s_index = ship_get_index_from_subsys (
            Player_ai->targeted_subsys, Player_ai->target_objnum);
        Assert (s_index < CHAR_MAX); // better be less than this!!!!
        t_subsys = (char)s_index;
    }
    ADD_DATA (t_subsys);
...
// brief explanation :
// in either case (wing or ship command), we need to send in a pseudo-ai
// object. Basically, both command handler functions "normally" (non
// multiplayer) use a couple of the Player_ai fields. So, we just fill in the
// ones necessary (which we can reconstruct from the packet data), and pass
// this as the default variable ai_info *local Its kind of a hack, but it
// eliminates the need to go in and screw around with quite a bit of code
void process_player_order_packet (ubyte* data, header* hinfo) {
    ...
    GET_DATA (type);
    if (type != SQUAD_MSG_ALL) { GET_INT (index); }
    ...
    // if the player has sent a rearm-repair me message, we should bail here
    // after evaluating it, since most likely the rest of the data is BOGUS.
    // All people should be able to to these things as well.
    ...
    // check to see if the type of order is a reinforcement call.  If so,
    // intercept it, and then call them in.
    ...
    // get the target objnum and targeted subsystem.  Quick out if we don't
    // have an object to act on.
    target_objp = multi_get_network_object (target_signature);
    if (target_objp == NULL) { return; }

    targeted_subsys = NULL;
    if (t_subsys != -1) {
        Assert (target_objp != NULL);
        targeted_subsys =
            ship_get_indexed_subsys (&Ships[target_objp->instance], t_subsys);
    }

    // save and restore the target objnum and targeted subsystem so that we
    // don't mess up other things here
    tobjnum_save = aip->target_objnum;
    tsubsys_save = aip->targeted_subsys;

    if (target_objp) { aip->target_objnum = OBJ_INDEX (target_objp); }
    else {
        aip->target_objnum = -1;
    }
    ...
    if (type == SQUAD_MSG_SHIP) {
        hud_squadmsg_send_ship_command (
            index, command, 1, SQUADMSG_HISTORY_ADD_ENTRY, player_num);
    }
...
// there are 2 cases for file signature sending which are handled very
// differently 1.) Pregame. In this case, the host requires that all clients
// send a filesig packet (when process_file_sig() is called, it
//     posts an ACK_FILE_ACCEPTED packet to ack_evaluate, so he thinks they
//     have acked).
// 2.) Ingame join. In this case, the client sends his filesig packet
// automatically to the server and the _client_ waits for
//     the ack, before continuing to join. It would be way too messy to have
//     the server wait on the clients ack, since he would have to keep track of
//     up to potentially 14 other ack handles  (ouch).
...
void process_subsystem_cargo_revealed_packet (ubyte* data, header* hinfo) {
    ...
    // server should rebroadcast
    if (MULTIPLAYER_MASTER) {
        send_subsystem_cargo_revealed_packet (
            &Ships[objp->instance], (int)uindex);
    }
}
...
void process_subsystem_cargo_hidden_packet (ubyte* data, header* hinfo) {
    ...
    // server should rebroadcast
    if (MULTIPLAYER_MASTER) {
        send_subsystem_cargo_hidden_packet (
            &Ships[objp->instance], (int)uindex);
    }
}
...
void send_netplayer_load_packet (net_player* pl) {
    ...
    if (pl == NULL) { multi_io_send_to_all_reliable (data, packet_size); }
    else {
        multi_io_send_reliable (pl, data, packet_size);
    }
}
...
void process_netplayer_load_packet (ubyte* data, header* hinfo) {
    ...
    if (!Multi_mission_loaded) {
        ...
        // ingame joiners need to "untag" all player ships as could_be_players.
        // The ingame joining code will remark the correct player ships
        if (Net_player->flags & NETINFO_FLAG_INGAME_JOIN) {
            multi_untag_player_ships ();
        }
...
void send_jump_into_mission_packet (net_player* pl) {
    ...
    // ingame joiners will get special data.  We need to tell them about the
    // state of the mission, like paused, and possible other things.
    if (pl != NULL) {
...
void process_jump_into_mission_packet (ubyte* data, header* hinfo) {
    ...
    // if I am ingame joining, there should be extra data.  For now, this data
    // is the netgame state. the game could be paused, so ingame joiner needs
    // to deal with it.
    if (Net_player->flags & NETINFO_FLAG_INGAME_JOIN) {
        GET_INT (state);
    ...
    // if I'm an ingame joiner, go to the ship select screen, or if I'm an
    // observer, jump right in!
    if (Net_player->flags & NETINFO_FLAG_INGAME_JOIN) {
    ...
    // NETLOG
    ml_string (
        NOX ("Client received mission start from server - entering mission"));
}
...
// the following two routines deal with updating and sending information
// regarding players rearming and repairing during the game.  The process
// function calls the routines to deal with setting flags and other interesting
// things.
void send_repair_info_packet (
    object* repaired_objp, object* repair_objp, int code) {
    int packet_size = 0;
    ushort repaired_signature, repair_signature;
    ubyte data[MAX_PACKET_SIZE];
    ubyte cd;

    // use the network signature of the destination object if there is one, -1
    // otherwise. client will piece it all together
    repaired_signature = repaired_objp->net_signature;

    // the repair ship may be NULL here since it might have been destroyed
    repair_signature = 0;
    if (repair_objp) { repair_signature = repair_objp->net_signature; }
...
void process_repair_info_packet (ubyte* data, header* hinfo) {
    ...
    if ((code != REPAIR_INFO_WARP_ADD) && (code != REPAIR_INFO_WARP_REMOVE)) {
        ...
        // set the dock flags when repair begins.  Prevents problem in lagging
        // docking packet.  Also set any other flags/modes which need to be set
        // to prevent Asserts. bleah.
        ...
        // if the repair is done (either by abort, or ending), mark the repair
        // ship's goal as being done.
        if (((code == REPAIR_INFO_ABORT) || (code == REPAIR_INFO_END)) &&
            repair_objp) {
...
// sends information updating clients on certain AI information that clients
// will need to know about to keep HUD information up to date.  objp is the
// object that we are updating, and what is the type of stuff that we are
// updating.
void send_ai_info_update_packet (object* objp, char what, object* other_objp) {
    ...
    // do an out here
    if (Ships[objp->instance].is_dying_or_departing ()) return;

    switch (what) {
    case AI_UPDATE_DOCK:
    case AI_UPDATE_UNDOCK:
        Assert (other_objp != NULL);
        if (other_objp == NULL) { return; }
        break;

    default: Assert (other_objp == NULL); break;
    }
    ...
    switch (what) {
    case AI_UPDATE_DOCK:
        // for docking ships, add the signature of the ship that we are docked
        // with.
        Assert (other_objp != NULL);
        ...
        docker_index =
            (ubyte)dock_find_dockpoint_used_by_object (objp, other_objp);
        dockee_index =
            (ubyte)dock_find_dockpoint_used_by_object (other_objp, objp);
    ...
    case AI_UPDATE_ORDERS: {
        ...
        // for orders, we only need to send a little bit of information here.
        // Be sure that the first order for this ship is active
        Assert (
        ...
        // the ship_name member of the goals structure may or may not contain a
        // real shipname.  If we don't have a valid shipnum, then don't sweat
        // it since it may not really be a ship.
        if (shipnum != -1) {
        ...
        if (aigp->ai_mode & (AI_GOAL_DOCK | AI_GOAL_REARM_REPAIR)) {
            Assert (
                ...
                aigp->flags[AI::Goal_Flags::Docker_index_valid]);
            Assert (
                (aigp->docker.index >= 0) && (aigp->docker.index < UCHAR_MAX));
            Assert (
                (aigp->dockee.index >= 0) && (aigp->dockee.index < UCHAR_MAX));
...
// process an ai_info update packet.  Docking/undocking, ai orders, etc. are
// taken care of here.  This information is mainly used to keep the clients HUD
// up to date with the appropriate information.
void process_ai_info_update_packet (ubyte* data, header* hinfo) {
    ...
    case AI_UPDATE_DOCK:
        ...
        if (!objp || !other_objp || (objp->type != OBJ_SHIP) ||
            (other_objp->type != OBJ_SHIP)) {
            break;
        }

        // don't assign the dock indexes, because they're part of the docking
        // ship's goal code... just dock them (and besides, we might be docking
        // initially docked ships and we wouldn't have an active goal)
        ai_do_objects_docked_stuff (
            objp, docker_index, other_objp, dockee_index);
        break;
    ...
    case AI_UPDATE_ORDERS:
        ...
        if (!objp || (objp->type != OBJ_SHIP)) break;

        // set up the information in the first goal element of the object in
        // question
        aip = &Ai_info[Ships[objp->instance].ai_index];
        aip->active_goal = 0;
        aigp = &aip->goals[aip->active_goal];
        ...
        if (other_objp && (other_objp->type == OBJ_SHIP)) {
            // get a pointer to the shipname in question.  Use the ship_name
            // value in the ship.  We are only using this for HUD display, so I
            // think that using this method will be fine.
            aigp->target_name = ai_get_goal_target_name (
                Ships[other_objp->instance].ship_name,
                &aigp->target_name_index);

            // special case for destroy subsystem -- get the ai_info pointer to
            // our target ship so that we can properly set up what subsystem
            // this ship is attacking.
            if ((mode == AI_GOAL_DESTROY_SUBSYSTEM) && (submode >= 0))
                aip->targeted_subsys = ship_get_indexed_subsys (
                    &Ships[other_objp->instance], submode);
        }
...
void send_mission_sync_packet (int mode, int start_campaign) {
    ...
    if (mode == MULTI_SYNC_PRE_BRIEFING) {
        if (Game_mode & GM_CAMPAIGN_MODE) {
            ...
            // add a byte indicating if we should be starting a campaign or
            // continuing it
...
void process_mission_sync_packet (ubyte* data, header* hinfo) {
    ...
    if (mode == MULTI_SYNC_PRE_BRIEFING) {
        // get the flag indicating if this is a single mission or a campaign
        // mode
        GET_DATA (campaign_flag);
        if (campaign_flag) {
            // get the flag indicating whether we should be starting a new
            // campaign
            GET_DATA (campaign_flag);
            ...
            // either start a new campaign or continue on to the next mission
        ...
        // process the initial orders now (moved from post_process_mission()in
        // missionparse)
...
void send_store_stats_packet (int accept) {
    ...
    // if I'm the server, send to everyone, else send to the standalone to be
    // rebroadcasted
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        multi_io_send_to_all_reliable (data, packet_size);
    }
...
void process_store_stats_packet (ubyte* data, header* hinfo) {
    ...
    // if I'm the standalone, rebroadcast. Otherwise, if I'm a client, merge my
    ...
    if (Game_mode & GM_STANDALONE_SERVER) {
        ...
        if (accept) {
            // all players should mark the stats as being accepted in the
            // debriefing
            multi_debrief_stats_accept ();
        }
        else {
            // all players should mark the stats as being "tossed" in the
            // debriefing
...
void process_debris_update_packet (ubyte* data, header* hinfo) {
    ...
    objp = NULL;
    objp = multi_get_network_object (net_sig);
    if (objp == NULL) { objp = &bogus_object; }
...
// ALAN begin
void send_wss_request_packet (
    short player_id, int from_slot, int from_index, int to_slot, int to_index,
    ...
    // a standard request
    if (p == NULL) { multi_io_send_reliable (Net_player, data, packet_size); }
...
void process_wss_request_packet (ubyte* data, header* hinfo) {
    ...
    // if we're the standalone, we have to route this packet to the host of the
    // game
    if (Game_mode & GM_STANDALONE_SERVER) {
        send_wss_request_packet (
            player_id, from_slot, from_index, to_slot, to_index, wl_ship_slot,
            ship_class, mode, Netgame.host);
    }
    // otherwise we're the host and should process the request
        ...
        case WSS_WEAPON_SELECT:
            wl_drop (
                from_slot, from_index, to_slot, to_index, wl_ship_slot,
                player_num);
            break;
        case WSS_SHIP_SELECT:
            multi_ts_drop (
                from_slot, from_index, to_slot, to_index, ship_class,
                player_num);
            break;
        default: Int3 ();
        }
...
void send_wss_update_packet (int team_num, ubyte* wss_data, int size) {
    ...
    // if we're only the host on the standalone, then send the packet to the
    // standalone to be routed
    else {
        multi_io_send_reliable (Net_player, data, packet_size);
    }
}
...
void process_wss_update_packet (ubyte* data, header* hinfo) {
    ...
    // if we're the standalone, then we should be routing this data to all the
    // other clients
    if (Game_mode & GM_STANDALONE_SERVER) {
        // read in the data
        offset += size;
...
// function to send firing information from the client to the server once they
// reach the final sync screen.
void send_firing_info_packet () {
    ...
    plinked = (ubyte) (
        (Player_ship->flags[Ship::Ship_Flags::Primary_linked]) ? 1 : 0);
    sdual = (ubyte) (
        (Player_ship->flags[Ship::Ship_Flags::Secondary_dual_fire]) ? 1 : 0);
    ADD_DATA (plinked);
    ADD_DATA (sdual);
...
void process_firing_info_packet (ubyte* data, header* hinfo) {
    ...
    shipp =
        &(Ships[Objects[Net_players[player_num].m_player->objnum].instance]);
    shipp->flags.set (Ship::Ship_Flags::Primary_linked, plinked != 0);
    shipp->flags.set (Ship::Ship_Flags::Secondary_dual_fire, sdual != 0);
}

// packet to deal with changing status of mission goals.  used to be sent every
// so often from server to clients, but with addition of reliable sockets, send
// when complete, invalid, etc. goal_num is the index into mission_goals.
// new_status means failed, success, etc.  -1 if not used. valid means goal is
// changing to invalid(0) or valid(1).  only applies if new_status == -1
...
void process_mission_goal_info_packet (ubyte* data, header* hinfo) {
    ...
    // if new_status != -1, then this is a change in goal status (i.e. goal
...
void send_player_settings_packet (net_player* p) {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx])) {
            ...
            // break the p_info structure by member, so we don't overwrite any
            // absolute pointers ADD_DATA(Net_players[idx].p_info);
    ...
    if (p == NULL) { multi_io_send_to_all_reliable (data, packet_size); }
    else {
        multi_io_send_reliable (p, data, packet_size);
    }
}
...
void process_deny_packet (ubyte* data, header* hinfo) {
    ...
    // if there is already a dialog active, do nothing - who cares at this
    // point.
    ...
    case JOIN_DENY_JR_STATE:
        popup (
            ...
            XSTR (
                "You have been rejected because the game is not in an "
                "appropriate state to accept",
                722));
        break;
    case JOIN_DENY_JR_TRACKER_INVAL:
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            XSTR (
                "You have been rejected because this is a Parallax Online "
                "game and you are not a registered Parallax Online pilot",
                723));
        break;
    case JOIN_DENY_JR_PASSWD:
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            XSTR (
                "You have been rejected because this is a password protected "
                "game",
                724));
        break;
    case JOIN_DENY_JR_CLOSED:
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            XSTR (
                "You have been rejected because this is a closed game and the "
    ...
    case JOIN_DENY_JR_TEMP_CLOSED:
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            XSTR (
                "You have been rejected because the netgame is forming and "
                "the host has temporarily closed it",
                726));
        break;
    case JOIN_DENY_JR_RANK_HIGH:
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            XSTR (
                "You have been rejected because this is a rank limited game "
                "and your rank is too high",
                727));
        break;
    ...
    case JOIN_DENY_JR_DUP:
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            XSTR (
                "You have been rejected because there is an identical player "
                "already in the game",
                729));
        break;
    case JOIN_DENY_JR_FULL:
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            XSTR ("You have been rejected because the game is full", 730));
        break;
    case JOIN_DENY_JR_BANNED:
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            XSTR (
                "You have been rejected because you are banned from this "
                "server",
                731));
        break;
    case JOIN_DENY_JR_NOOBS:
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            XSTR (
                "You have been rejected because this game does not allow "
                "observers",
                732));
        break;
    case JOIN_DENY_JR_INGAME_JOIN:
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            XSTR (
                "You cannot join at this time since someone else is currently "
                "joining.  Try again shortly.",
                733));
        break;
    case JOIN_DENY_JR_BAD_VERSION:
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            XSTR (
                "You cannot join this game because you are running an older "
                "version of FreeSpace than the server.  Exit FreeSpace, and "
                "choose the 'Update FreeSpace' button in the FreeSpace "
                "launcher to download the latest version of FreeSpace.",
                734));
        break;
    case JOIN_DENY_JR_TYPE:
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            XSTR (
                "You cannot join a game in progress unless it is a dogfight "
                "mission.",
                1433));
        break;
    }

    // call this so that the join request timestamp automatically expires when
...
void send_post_sync_data_packet (net_player* p, int std_request) {
    ...
    // 2.) having recevied this block of data from the host, it redistributes
    // it
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        // if this is a ship owned by a player, we should mark down his weapons
        // bank/link settings now if we're the server
        pl = NULL;
        if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
            player_index = multi_find_player_by_net_signature (
                Objects[so->objnum].net_signature);
            if (player_index == -1) { pl = NULL; }
            else {
        ...
        if (pl != NULL) { pl->s_info.cur_primary_bank = bval; }
        // Assert(bval != -1);
        ADD_DATA (bval);

        bval = (char)(shipp->weapons.current_secondary_bank);
        if (pl != NULL) { pl->s_info.cur_secondary_bank = bval; }
        // Assert(bval != -1);
        ADD_DATA (bval);
        ...
        if (shipp->flags[Ship::Ship_Flags::Primary_linked]) {
            if (pl != NULL) { pl->s_info.cur_link_status |= (1 << 0); }
            val |= (1 << 0);
        }
        if (shipp->flags[Ship::Ship_Flags::Secondary_dual_fire]) {
            if (pl != NULL) { pl->s_info.cur_link_status |= (1 << 1); }
            val |= (1 << 1);
        }
    ...
    // 2 cases, if I'm the host on a standalone, I should be sending this to
    // the standalone only or if I'm the server as well as the host, I should
    // be sending this to all players
    if (Net_player->flags & NETINFO_FLAG_GAME_HOST) {
        if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
            // broadcast
            if (p == NULL) {
...
void process_post_sync_data_packet (ubyte* data, header* hinfo) {
    ...
    // if I'm the host on a standalone, this is going to be a request to send
    // the data to the standalone to be routed
    if ((Net_player->flags & NETINFO_FLAG_GAME_HOST) &&
        !(Net_player->flags & NETINFO_FLAG_AM_MASTER) && val) {
        PACKET_SET_SIZE ();

        // at this point we want to delete all necessary ships, change all
        // necessary ship classes, and set all weapons up
    ...
    for (idx = 0; idx < Multi_ts_num_deleted; idx++) {
        ...
        if (objp != NULL) {
            ...
            ship_add_exited_ship (
                &Ships[objp->instance], Ship::Exit_Flags::Player_deleted);
            obj_delete (Multi_ts_deleted_objnums[idx]);
            ship_wing_cleanup (
                objp->instance, &Wings[Ships[objp->instance].wingnum]);
        }
        else {
    ...
    for (idx = 0; idx < ship_count; idx++) {
        ...
        Assertion (
            objp != NULL,
            "idx: %d, ship_count: %d, sinfo_index: %u, ts_index: %u, net_sig: "
            "%u",
            idx, ship_count, sinfo_index, ts_index, net_sig);
        Assertion (
            objp->type == OBJ_SHIP,
            "type: %d, idx: %d, ship_count: %d, sinfo_index: %u, ts_index: "
            "%u, net_sig: %u",
    ...
    for (idx = 0; idx < ship_count; idx++) {
        ...
        Objects[shipp->objnum].flags.remove (
            Object::Object_Flags::Player_ship);
        Objects[shipp->objnum].flags.remove (
            Object::Object_Flags::Could_be_player);
        if (val & (1 << 2)) {
            Objects[shipp->objnum].flags.set (
                Object::Object_Flags::Player_ship);
        }
        else {
            obj_set_flags (
                &Objects[shipp->objnum],
                Objects[shipp->objnum].flags +
                    Object::Object_Flags::Could_be_player);
        }
    ...
    // the standalone server will receive this packet from the host of the
    // game, to be applied locally and also to be rebroadcast.
    if (Game_mode & GM_STANDALONE_SERVER) {
...
void send_wss_slots_data_packet (
    int team_num, int final, net_player* p, int std_request) {
    ubyte data[MAX_PACKET_SIZE], val;
    short val_short;
    int idx, i;
    int packet_size = 0;
    ...
    // 2 cases, if I'm the host on a standalone, I should be sending this to
    // the standalone only
    // or if I'm the server as well as the host, I should be sending this to
    // all players
    if (Net_player->flags & NETINFO_FLAG_GAME_HOST) {
        if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
            // broadcast
...
void send_shield_explosion_packet (
    int objnum, int tri_num, vec3d /*hit_pos*/) {
    int packet_size, i;
    ...
    // for each player, determine if this object is behind the player -- if so,
    // don't send the packet.
    for (i = 0; i < MAX_PLAYERS; i++) {
        if (MULTI_CONNECTED (Net_players[i]) &&
            (&Net_players[i] != Net_player)) {
            float dot;
            ...
            if (vm_vec_mag_quick (&diff) < 0.01) { continue; }

            vm_vec_normalized_dir (
                &eye_to_obj_vec, &Objects[objnum].pos, &eye_pos);
            dot = vm_vec_dot (&eye_orient.vec.fvec, &eye_to_obj_vec);

            if (dot < OBJ_VISIBILITY_DOT) { continue; }
...
void process_shield_explosion_packet (ubyte* data, header* hinfo) {
    ...
    // find the object with this signature.  If found, then do a shield
    // explosion
    objp = multi_get_network_object (signature);
    if (objp) {
        polymodel* pm;
        ...
        // given the tri num, find the local position which is the average of
        // the three vertices of the triangle affected.  Use this average point
        // as the hit point Assert( objp->type == OBJ_SHIP );
        if (objp->type != OBJ_SHIP) { return; }
...
void send_player_stats_block_packet (
    net_player* pl, int stats_code, net_player* target, short offset) {
    scoring_struct* sc;
    ubyte data[MAX_PACKET_SIZE], val;
    int idx;
    int packet_size = 0;
    ...
    case STATS_ALLTIME:
        ...
        while (idx < MAX_SHIP_CLASSES) {
            send_player_stats_block_packet (
                pl, STATS_ALLTIME_KILLS, target, (short)idx);
    ...
    case STATS_MISSION:
        ...
        while (idx < MAX_SHIP_CLASSES) {
            send_player_stats_block_packet (
                pl, STATS_MISSION_CLASS_KILLS, target, (short)idx);
            idx += MAX_SHIPS_PER_PACKET;
        }
    ...
    case STATS_MISSION_CLASS_KILLS:
        ADD_SHORT (offset);
        for (idx = offset;
             idx < MAX_SHIP_CLASSES && idx < offset + MAX_SHIPS_PER_PACKET;
             idx++) {
...
void process_player_stats_block_packet (ubyte* data, header* hinfo) {
    ...
    case STATS_ALLTIME_KILLS:
        GET_SHORT (si_offset);
        for (idx = si_offset;
             idx < MAX_SHIP_CLASSES && idx < si_offset + MAX_SHIPS_PER_PACKET;
             idx++) {
            GET_INT (i_tmp);
            sc->kills[idx] = i_tmp;
        }
    ...
    case STATS_DOGFIGHT_KILLS:
        ...
        for (idx = 0; idx < MAX_PLAYERS; idx++) {
            ...
            if (player_num >= 0) {
                ml_printf (
                    "%d", Net_players[player_num]
                              .m_player->stats.m_dogfight_kills[idx]);
            }
        }
        GET_INT (sc->m_kill_count);
        GET_INT (sc->m_kill_count_ok);
...
void send_asteroid_create (
    object* new_objp, object* parent_objp, int asteroid_type, vec3d* relvec) {
    int packet_size;
    ubyte data[MAX_PACKET_SIZE];
...
void send_asteroid_hit (
    object* objp, object* other_objp, vec3d* hitpos, float damage) {
    int packet_size;
    ubyte data[MAX_PACKET_SIZE], packet_type;
    vec3d vec;

    vm_vec_zero (&vec);
    if (hitpos != NULL) vec = *hitpos;
...
void process_asteroid_info (ubyte* data, header* hinfo) {
    ...
    case ASTEROID_CREATE: {
        ...
        // after getting the values, set the next network signature, and call
    ...
    case ASTEROID_HIT: {
        ...
        objp = multi_get_network_object (signature);
        if (osignature == 0xffff) { other_objp = NULL; }
        else {
        ...
        // if we know the other object is a weapon, then do a weapon hit to
        // kill the weapon
...
void send_host_restr_packet (const char* callsign, int code, int mode) {
    ...
    // otherwise if I'm the host, I should be sending a reply back to the
    // standalone server
    else {
...
void process_host_restr_packet (ubyte* data, header* hinfo) {
    ...
    case 0:
        ...
        HUD_printf (
            XSTR ("Player %s has tried to join - allow (y/n) ?", 736),
            callsign);
        break;
    ...
    case 1:
        ...
        // let the player join if the timestamp has not already elapsed on the
        // server
        if (Multi_restr_query_timestamp != -1) {
            multi_process_valid_join_request (
                &Multi_restr_join_request, &Multi_restr_addr, (int)mode);
        }
        break;
...
void send_netgame_end_error_packet (int notify_code, int err_code) {
    ...
    // only the server should ever be here - although this might change if for
    // some reason the host wants to end the game
    Assert (Net_player->flags & NETINFO_FLAG_AM_MASTER);
...
void send_client_update_packet (net_player* pl) {
    ...
    if (Multi_pause_status) { val |= UPDATE_IS_PAUSED; }
    else if (
        (Game_mode & GM_IN_MISSION) &&
        !(pl->flags & NETINFO_FLAG_INGAME_JOIN) &&
        !(NETPLAYER_IS_OBSERVER (pl)) && !(NETPLAYER_IS_DEAD (pl)) &&
        (Objects[pl->m_player->objnum].type == OBJ_SHIP)) {
    ...
    // when not paused, send hull/shield/subsystem updates to all clients
    // (except for ingame joiners)
    if (val & UPDATE_HULL_INFO) {
        ...
        // hull strength and shield mesh information are floats (as a
        // percentage).  Pass the integer percentage value since that should be
        // close enough
        percent = (ubyte)std::lround (get_hull_pct (objp) * 100.0f);
        if ((percent == 0) && (get_hull_pct (objp) > 0.0f)) { percent = 1; }
        ADD_DATA (percent);
        ...
        for (i = 0; i < n_quadrants; i++) {
            percent = (ubyte) (
                objp->shield_quadrant[i] / shield_get_max_quad (objp) *
                100.0f);

            ADD_DATA (percent);
        }

        // add the data for the subsystem hits.  We can assume that the lists
        // will be the same side of both machines. Added as percent since that
        // number <= 100

        // also write out the number of subsystems.  We do this because the
        // client might not know about the object he is getting data for. (i.e.
        // he killed the object already).
        ns = (ubyte)sip->n_subsystems;
        ADD_DATA (ns);

        // now the subsystems.
        for (subsysp = GET_FIRST (&shipp->subsys_list);
             subsysp != END_OF_LIST (&shipp->subsys_list);
             subsysp = GET_NEXT (subsysp)) {
            percent =
                (ubyte) (subsysp->current_hits / subsysp->max_hits * 100.0f);
            ADD_DATA (percent);
        }

        // compute the threats for this player.  Only compute the threats if
        // this player is actually playing (i.e. he has a ship)
...
void process_client_update_packet (ubyte* data, header* hinfo) {
    ...
    if (have_hull_info) {
        ...
        ship_info* sip;
        ship* shipp;
        ubyte hull_percent, n_quadrants, n_subsystems,
            subsystem_percent[MAX_MODEL_SUBSYSTEMS], threats;
        ...
        // add his secondary bank ammo
        GET_INT (ammo_count);
        for (i = 0; i < ammo_count; i++) { GET_INT (ammo[i]); }

        // assign the above information to my ship, assuming that I can find
        // it!  Ingame joiners might get this packet because of delay between
        // reliable packet acknowledging my ingame ship and the start of these
        // UDP client update packets.  Only read this info if I have a ship.
        if (!(Net_player->flags & NETINFO_FLAG_INGAME_JOIN) &&
            (Player_ship != NULL) && (Player_obj != NULL) &&
            (Net_player != NULL)) {
            shipp = Player_ship;
            objp = Player_obj;
            sip = &Ship_info[shipp->ship_info_index];
            ...
            for (i = 0; i < n_quadrants; i++) {
                if (i < objp->n_quadrants) {
                    fl_val =
                        (shield_percent[i] * shield_get_max_quad (objp) /
                         100.0f);
                    objp->shield_quadrant[i] = fl_val;
                }
            }

            // for sanity, be sure that the number of susbystems that I read in
            // matches the player.  If not, then don't read these in.
            if (n_subsystems == sip->n_subsystems) {
                ...
                for (subsysp = GET_FIRST (&shipp->subsys_list);
                    ...
                    fl_val = subsystem_percent[n_subsystems] *
                             subsysp->max_hits / 100.0f;
                    subsysp->current_hits = fl_val;

                    // add the value just generated (it was zero'ed above) into
                    // the array of generic system types
                    subsys_type =
                        subsysp->system_info
                            ->type; // this is the generic type of subsystem
                    Assert (subsys_type < SUBSYSTEM_MAX);
                    if (!(subsysp
                              ->flags[Ship::Subsystem_Flags::No_aggregate])) {
                        shipp->subsys_info[subsys_type]
                            .aggregate_current_hits += fl_val;
                    }
                    n_subsystems++;
                }
            }
    ...
    int pl;
    GET_INT (pl);
    if (Net_player != NULL) { Net_player->cl_last_pl = pl; }

    PACKET_SET_SIZE ();
    // note, if we're already paused or unpaused, calling these will have no
    // effect, so it is safe to do so
    if (!popup_active () && !(Net_player->flags & NETINFO_FLAG_RESPAWNING) &&
        !(Net_player->flags & NETINFO_FLAG_LIMBO)) {
        if (is_paused) { multi_pause_pause (); }
        else {
            multi_pause_unpause ();
        }
    }
...
void process_countdown_packet (ubyte* data, header* hinfo) {
    ...
    if (gameseq_get_state () != GS_STATE_MULTI_MISSION_SYNC) { return; }

    // if we're the standalone, this should be a -1 telling us to start the
    // countdown
    if (Game_mode & GM_STANDALONE_SERVER) {
        Assert ((int)time == -1);
...
void process_debrief_info (ubyte* data, header* hinfo) {
    int offset, i, j;
    int stage_counts[MAX_TVT_TEAMS],
        active_stages[MAX_TVT_TEAMS][MAX_DEBRIEF_STAGES],
        *stages[MAX_TVT_TEAMS];
    int i_tmp;
    ...
    // now that we have the stage data for the debriefing stages, call debrief
    // function with the data so that clients can now see the debriefing stuff.
    // Do it only for my team.
    Assert (
        (Net_player->p_info.team >= 0) &&
        (Net_player->p_info.team < Num_teams));
    debrief_set_multi_clients (
        stage_counts[Net_player->p_info.team],
        stages[Net_player->p_info.team]);
}

// sends homing information to all clients.  We only need signature and
// num_missiles (because of hornets). sends homing_object and homing_subsystem
// to all clients.
void send_homing_weapon_info (int weapon_num) {
    ...
    if (!(Weapon_info[wp->weapon_info_index].is_homing ())) return;

    // default the subsystem
    t_subsys = -1;

    // get the homing signature.  If this weapon isn't homing on anything, then
    // sent 0 as the homing signature.
    homing_signature = 0;
    ...
    if (homing_object != &obj_used_list) {
        ...
        if ((homing_object->type == OBJ_SHIP) && (wp->homing_subsys != NULL)) {
            ...
            s_index = ship_get_index_from_subsys (
                wp->homing_subsys, OBJ_INDEX (homing_object), 1);
            Assert (s_index < CHAR_MAX); // better be less than this!!!!
            t_subsys = (char)s_index;
        }
    }
...
// process a homing weapon info change packet.  multiple_missiles parameter
// specifies is this packet contains information for multiple weapons (like
// hornets).
void process_homing_weapon_info (ubyte* data, header* hinfo) {
    ...
    if (homing_object->type == OBJ_WEAPON) {
        auto flags =
            Weapon_info[Weapons[homing_object->instance].weapon_info_index]
                .wi_flags;
    ...
    if (h_subsys != -1) {
        ...
        wp->homing_subsys = ship_get_indexed_subsys (
            &Ships[homing_object->instance], h_subsys);
    }
...
void process_emp_effect (ubyte* data, header* hinfo) {
    ...
    if ((objp != NULL) && (objp->type == OBJ_SHIP)) {
        ...
        if (!(Net_player->flags & NETINFO_FLAG_OBSERVER) &&
            (Player_obj != NULL) && (Player_obj->type == OBJ_SHIP) &&
            (Player_obj == objp)) {
...
void send_NEW_primary_fired_packet (ship* shipp, int banks_fired) {
    ...
    // if i'm a multiplayer client, I should never send primary fired packets
    // for anyone except me
    if (MULTIPLAYER_CLIENT && (Player_obj != objp)) { return; }

    // just in case nothing got fired
    if (banks_fired <= 0) { return; }
    ...
    // build up the standard weapon fired packet.  This packet will get sent to
    // all players if an AI ship fired the primary weapons.  If a player fired
    // the weapon, then this packet will get sent to every player but the guy
    // who actullly fired the weapon.  This method is used to help keep client
...
void process_NEW_primary_fired_packet (ubyte* data, header* hinfo) {
    ...
    // strip off all remaining bits and just keep which banks were actually
    // fired. banks_fired &= 0x3;

    // set the link status of the ship if not the player.  If it is the player,
    // we will do sanity checking only (for now). if ( !linked ){
    ...
    if ((Player_obj != NULL) && (Player_obj == objp)) { return; }

    ship_fire_primary (objp, 0, 1);

    // Karajorma - It's still a hack but at least this way it only affects AI
    // ships
    if (!(objp->flags[Object::Object_Flags::Player_ship])) {
        // Juke - this is the hackiest hack, but hopefully it will fix stream
        // weapon notifications generated by AI ships.
...
void send_NEW_countermeasure_fired_packet (
    object* objp, int cmeasure_count, int rand_val) {
    ubyte data[MAX_PACKET_SIZE];
    int packet_size;
    int np_index;
...
void process_NEW_countermeasure_fired_packet (ubyte* data, header* hinfo) {
    ...
    if ((rand_val >= NPERM_SIG_MIN) && (rand_val <= NPERM_SIG_MAX)) {
        multi_set_network_signature (
            (ushort)rand_val, MULTI_SIG_NON_PERMANENT);
    }
...
void send_beam_fired_packet (
    ...
    if (bfi_flags & BFIF_IS_FIGHTER_BEAM) {
        ...
        subsys_index =
            (char)ship_get_index_from_subsys (turret, OBJ_INDEX (shooter), 1);
    }
...
void process_beam_fired_packet (ubyte* data, header* hinfo) {
    ...
    // this check is a little convoluted but should cover all bases until we
    // decide to just break the protocol
    if (Ship_info[shipp->ship_info_index].is_fighter_bomber ()) {
        // make sure the beam is a primary weapon and not attached to a turret
        // or something
        for (i = 0; i < shipp->weapons.num_primary_banks; i++) {
            if (shipp->weapons.primary_bank_weapons[i] ==
    ...
    if (fire_info.bfi_flags & BFIF_IS_FIGHTER_BEAM) {
        ...
        float field_of_fire =
            Weapon_info[fire_info.beam_info_index].field_of_fire;
        ...
        shipp->beam_sys_info.turret_fov =
            cosf ((field_of_fire != 0.0f) ? field_of_fire : 180);
...
void send_sw_query_packet (ubyte code, char* txt) {
    ...
    ADD_DATA (code);
    if ((code == SW_STD_START) || (code == SW_STD_BAD)) { ADD_STRING (txt); }
...
void process_event_update_packet (ubyte* data, header* hinfo) {
    ...
    if (!(store_flags & MEF_DIRECTIVE_SPECIAL) &&
    ...
    // if we went directive special to non directive special
        ...
        (store_flags & MEF_DIRECTIVE_SPECIAL) &&
        !(Mission_events[u_event].flags & MEF_DIRECTIVE_SPECIAL)) {
        mission_event_unset_directive_special (u_event);
    }
}

// Karajorma - Sends a packet to all clients telling them that a SEXP variable
...
void process_variable_update_packet (ubyte* data, header* hinfo) {
    ...
    if (MULTIPLAYER_MASTER) {
        send_variable_update_packet (
            variable_index, Sexp_variables[variable_index].text);
    }
}
...
// flak fired packet
void send_flak_fired_packet (
    int ship_objnum, int subsys_index, int weapon_objnum, float flak_range) {
    int packet_size;
    ushort pnet_signature;
    ubyte data[MAX_PACKET_SIZE], cindex;
    object* objp;
...
void process_flak_fired_packet (ubyte* data, header* hinfo) {
    ...
    if (weapon_objnum != -1) {
        ...
        // create a muzzle flash from a flak gun based upon firing position and
        // weapon type
        flak_muzzle_flash (&pos, &dir, &objp->phys_info, wid);

        // set its range explicitly - make it long enough so that it's
        // guaranteed to still exist when the server tells us it blew up
        flak_set_range (&Objects[weapon_objnum], (float)flak_range);
    }
}

#define ADD_NORM_VEC(d)                                               \
    do {                                                              \
        Assert ((packet_size + 3) < MAX_PACKET_SIZE);                 \
        char vnorm[3] = { (char)(d.x * 127.0f), (char)(d.y * 127.0f), \
                          (char)(d.z * 127.0f) };                     \
        memcpy (data + packet_size, vnorm, 3);                        \
        packet_size += 3;                                             \
    } while (0);
#define GET_NORM_VEC(d)                   \
    do {                                  \
        char vnorm[3];                    \
        memcpy (vnorm, data + offset, 3); \
        d.x = (float)vnorm[0] / 127.0f;   \
        d.y = (float)vnorm[1] / 127.0f;   \
        d.z = (float)vnorm[2] / 127.0f;   \
    } while (0);

// player pain packet
void send_player_pain_packet (
    net_player* pl, int weapon_info_index, float damage, vec3d* force,
    vec3d* hitpos, int quadrant_num) {
    ubyte data[MAX_PACKET_SIZE];
    short windex;
    ushort udamage;
    int packet_size = 0;
...
void process_player_pain_packet (ubyte* data, header* hinfo) {
    ...
    Assert (
        (windex < Num_weapon_types) &&
        (Weapon_info[windex].subtype == WP_LASER));
    if (!((windex < Num_weapon_types) &&
          (Weapon_info[windex].subtype == WP_LASER))) {
        return;
    }
    ...
    weapon_hit_do_sound (
        Player_obj, wip, &Player_obj->pos, true, quadrant_num);

    // we need to do 3 things here. player pain (game flash), weapon hit sound,
    // ship_apply_whack()
...
// lightning packet
void send_lightning_packet (
    int bolt_type_internal, vec3d* start, vec3d* strike) {
    ubyte data[MAX_PACKET_SIZE];
    char val;
    int packet_size = 0;
...
void process_lightning_packet (ubyte* data, header* hinfo) {
    ...
    // invalid bolt?
    if (bolt_type_internal < 0) { return; }
...
void process_bytes_recvd_packet (ubyte* data, header* hinfo) {
    ...
    if (bytes < 0) { return; }
    pl->sv_last_pl =
        (int)(100.0f * (1.0f - ((float)pl->cl_bytes_recvd / (float)pl->sv_bytes_sent)));
...
void process_host_captain_change_packet (ubyte* data, header* hinfo) {
    ...
    if (captain_change) {
        ...
        for (idx = 0; idx < MAX_PLAYERS; idx++) {
            if (MULTI_CONNECTED (Net_players[idx]) &&
                (Net_players[idx].player_id == player_id)) {
                HUD_printf (
                    "%s is the new captain of team %d",
                    Net_players[idx].m_player->callsign,
        ...
        if (!found_player) {
            multi_quit_game (
                PROMPT_NONE, MULTI_END_NOTIFY_NONE, MULTI_END_ERROR_HOST_LEFT);
        }
    }
}
...
void send_self_destruct_packet () {
    ...
    if (MULTI_OBSERVER (Net_players[MY_NET_PLAYER_NUM])) { return; }

    // bogus object?
    if ((Player_ship == NULL) || (Player_obj == NULL)) { return; }
...
void process_self_destruct_packet (ubyte* data, header* hinfo) {
    ...
    if (np_index < 0) { return; }
    if (MULTI_OBSERVER (Net_players[np_index])) { return; }
    if (Net_players[np_index].m_player == NULL) { return; }
    if ((Net_players[np_index].m_player->objnum < 0) ||
        (Net_players[np_index].m_player->objnum >= MAX_OBJECTS)) {
        return;
    }
    if (Objects[Net_players[np_index].m_player->objnum].net_signature !=
        net_sig) {
        return;
    }
    ...
    if ((Objects[Net_players[np_index].m_player->objnum].instance < 0) ||
        (Objects[Net_players[np_index].m_player->objnum].instance >=
         MAX_SHIPS)) {
        return;
    }
...
void process_sexp_packet (ubyte* data, header* hinfo) {
    ...
    for (i = 0; i < num_ubytes; i++) { GET_DATA (received_packet[i]); }
