...
void multi_rate_reset (int np_index) {
    ...
    // sanity checks
    if ((np_index < 0) || (np_index >= MAX_RATE_PLAYERS)) { return; }
...
// add data of the specified type to datarate processing, returns 0 on fail (if
// we ran out of types, etc, etc)
int multi_rate_add (int np_index, const char* type, int size) {
    int idx;
    mr_info* m;
    // sanity checks
    if ((np_index < 0) || (np_index >= MAX_RATE_PLAYERS)) { return 0; }
    ...
    if (idx >= MAX_RATE_TYPES) { return 0; }
...
// process
#define R_AVG(ct, ar, avg)                            \
    do {                                              \
        int av_idx;                                   \
        float av_sum = 0.0f;                          \
        if (ct == 0) { avg = 0; }                     \
        else {                                        \
            for (av_idx = 0; av_idx < ct; av_idx++) { \
                av_sum += (float)ar[av_idx];          \
            }                                         \
            avg = av_sum / (float)ct;                 \
        }                                             \
    } while (0)
void multi_rate_process () {
    ...
    for (idx = 0; idx < MAX_RATE_PLAYERS; idx++) {
        for (s_idx = 0; s_idx < MAX_RATE_TYPES; s_idx++) {
            ...
            // invalid entries
            if (strlen (m->type) <= 0) { continue; }

            // process alltime
            if (m->stamp_second == -1) { m->stamp_second = timestamp (1000); }
            else if (timestamp_elapsed (m->stamp_second)) {
                // if we've reached max records
                if (m->records_second_count >= NUM_UPDATE_RECORDS) {
                    memmove (
                        m->records_second, m->records_second + 1,
                        sizeof (int) * (NUM_UPDATE_RECORDS - 1));
                    m->records_second[NUM_UPDATE_RECORDS - 1] =
                        m->bytes_second;
                }
                // haven't reached max records
                else {
                    m->records_second[m->records_second_count++] =
                        m->bytes_second;
                }

                // recalculate the average
                R_AVG (
                    m->records_second_count, m->records_second, m->avg_second);
            ...
            if (m->records_frame_count >= NUM_UPDATE_RECORDS) {
                memmove (
                    m->records_frame, m->records_frame + 1,
                    sizeof (int) * (NUM_UPDATE_RECORDS - 1));
...
void multi_rate_display (int np_index, int x, int y) {
    ...
    for (idx = 0; idx < MAX_RATE_TYPES; idx++) {
        ...
        if (strlen (m->type) <= 0) { break; }

        // display
        gr_set_color_fast (&Color_red);
        gr_printf_no_resize (
            x, y, "%s %d (%d/s) (%f/f)", m->type, m->total_bytes,
