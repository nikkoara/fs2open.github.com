...
const char* Multi_df_background_fname[GR_NUM_RESOLUTIONS] = { "KillMatrix",
                                                              "2_KillMatrix" };
const char* Multi_df_mask_fname[GR_NUM_RESOLUTIONS] = { "KillMatrix-m",
                                                        "2_KillMatrix-m" };
...
// display area coords
int Multi_df_display_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                         43, 133, 569, 269 },
                                                       { // GR_1024
                                                         60, 213, 919, 429 } };
...
// players when the screen started - we need to store this explicity so that
// even after players leave, we can display the full kill matrix
...
// stuff a string representing the # of kills, player X had on player Y (where
// X and Y are indices into Multi_df_score) returns the # of kills
...
void multi_df_level_pre_enter () {
    ...
    if (!(Netgame.type_flags & NG_TYPE_DOGFIGHT)) { return; }
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            ...
            Ships[Objects[Net_players[idx].m_player->objnum].instance].team =
                Iff_traitor;
        }
    }
...
void multi_df_debrief_init () {
    ...
    w = &Multi_df_window;
    w->add_XSTR (
        "Accept", 1035, Multi_df_buttons[gr_screen.res][ACCEPT_BUTTON].xt,
...
void multi_df_debrief_do () {
    ...
    // process keypresses
    switch (new_k) {
    case KEY_ESC: multi_debrief_esc_hit (); break;
    }
    ...
    font::force_fit_string (
        buf, 255, Kill_matrix_title_coords[gr_screen.res][2]);
    gr_set_color_fast (&Color_bright_white);
    gr_string (
        Kill_matrix_title_coords[gr_screen.res][0],
        Kill_matrix_title_coords[gr_screen.res][1], buf, GR_RESIZE_MENU);
...
void multi_df_button_pressed (int button) {
    switch (button) {
    case ACCEPT_BUTTON: multi_debrief_accept_hit (); break;
    }
}
...
void multi_df_setup_kill_matrix () {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            ...
            ml_printf (
                "Dogfight debrief stats for %s",
                Net_players[idx].m_player->callsign);
            for (s_idx = 0; s_idx < MAX_PLAYERS; s_idx++) {
                ml_printf (
                    "%d",
                    Net_players[idx].m_player->stats.m_dogfight_kills[s_idx]);
            }
...
void multi_df_blit_kill_matrix () {
    ...
    float max_item_width =
        ((float)Multi_df_display_coords[gr_screen.res][2] - 40.0f) /
        (float)(Multi_df_score_count + 1);
    float max_text_width = max_item_width * 0.8f;

    // start x for the top bar (one item to the right)
    int top_x_start =
        Multi_df_display_coords[gr_screen.res][0] + (int)max_item_width;
    ...
    for (idx = 0; idx < Multi_df_score_count; idx++) {
        ...
        gr_string (
            cx + (int)((max_item_width - (float)str_len) / 2.0f), cy,
            squashed_string, GR_RESIZE_MENU);
    ...
    for (idx = 0; idx < Multi_df_score_count; idx++) {
        ...
        if (!MULTI_CONNECTED (Net_players[Multi_df_score[idx].np_index]) ||
            (Net_players[Multi_df_score[idx].np_index].state ==
            ...
            if (Multi_common_icons[MICON_VALID] != -1) {
                ...
                gr_bitmap (
                    Multi_df_check_coords[gr_screen.res], cy, GR_RESIZE_MENU);
            }
        }
        ...
        for (s_idx = 0; s_idx < Multi_df_score_count; s_idx++) {
            // stuff the string to be displayed and select the proper display
            // color
            if (s_idx == idx) {
        ...
        gr_get_string_size (&str_len, NULL, squashed_string);
        gr_string (
            Multi_df_display_coords[gr_screen.res][0] +
                Multi_df_display_coords[gr_screen.res][2] -
                (MULTI_DF_TOTAL_ADJUST + str_len),
            cy, squashed_string, GR_RESIZE_MENU);
...
int multi_df_stuff_kills (char* kills, int player_x, int player_y) {
    ...
    sprintf (
        kills, "%d",
        s->stats.m_dogfight_kills[Multi_df_score[player_y].np_index]);
