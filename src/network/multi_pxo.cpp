...
ui_button_info Multi_pxo_buttons[GR_NUM_RESOLUTIONS][MULTI_PXO_NUM_BUTTONS] = {
    {
        ...
        ui_button_info (
            "PXB_01", 1, 334, -1, -1, 1), // scroll player list down
    ...
    },
    {
        // GR_1024
        ui_button_info (
            "2_PXB_00", 2, 166, -1, -1, 0), // scroll player list up
        ui_button_info (
            "2_PXB_01", 2, 534, -1, -1, 1), // scroll player list down
        ...
        ui_button_info (
            "2_PXB_07", 528, 175, -1, -1, 7), // join private channel
        ui_button_info ("2_PXB_08", 989, 112, -1, -1, 8), // scroll channels up
        ui_button_info (
            "2_PXB_09", 989, 170, -1, -1, 9), // scroll channels down
...
UI_XSTR Multi_pxo_text[GR_NUM_RESOLUTIONS][MULTI_PXO_NUM_TEXT] = {
    { // GR_640
      { "Web", 1313, 20, 415, UI_XSTR_COLOR_GREEN, -1,
        &Multi_pxo_buttons[0][MULTI_PXO_RANKINGS].button },
      { "Ranking", 1314, 6, 426, UI_XSTR_COLOR_GREEN, -1,
        &Multi_pxo_buttons[0][MULTI_PXO_RANKINGS].button },
      { "Pilot", 1310, 68, 415, UI_XSTR_COLOR_GREEN, -1,
        &Multi_pxo_buttons[0][MULTI_PXO_PINFO].button },
      { "Info", 1311, 72, 426, UI_XSTR_COLOR_GREEN, -1,
        &Multi_pxo_buttons[0][MULTI_PXO_PINFO].button },
      { "Find", 1315, 119, 415, UI_XSTR_COLOR_GREEN, -1,
        &Multi_pxo_buttons[0][MULTI_PXO_FIND].button },
      { "Motd", 1316, 36, 456, UI_XSTR_COLOR_GREEN, -1,
        &Multi_pxo_buttons[0][MULTI_PXO_MOTD].button },
      { "Join", 1505, 291, 100, UI_XSTR_COLOR_PINK, -1,
        &Multi_pxo_buttons[0][MULTI_PXO_JOIN].button },
      { "Channel", 1317, 266, 112, UI_XSTR_COLOR_PINK, -1,
        &Multi_pxo_buttons[0][MULTI_PXO_JOIN].button },
      { "Join", 1506, 291, 134, UI_XSTR_COLOR_PINK, -1,
        &Multi_pxo_buttons[0][MULTI_PXO_JOIN_PRIV].button },
      { "Private", 1318, 273, 146, UI_XSTR_COLOR_PINK, -1,
        &Multi_pxo_buttons[0][MULTI_PXO_JOIN_PRIV].button },
      { "Exit", 1416, 493, 424, UI_XSTR_COLOR_PINK, -1,
        &Multi_pxo_buttons[0][MULTI_PXO_EXIT].button },
      { "Help", 928, 535, 416, UI_XSTR_COLOR_GREEN, -1,
        &Multi_pxo_buttons[0][MULTI_PXO_HELP].button },
      { "Games", 1319, 579, 416, UI_XSTR_COLOR_PINK, -1,
    ...
    { // GR_1024
      { "Web", 1313, 32, 664, UI_XSTR_COLOR_GREEN, -1,
        &Multi_pxo_buttons[1][MULTI_PXO_RANKINGS].button },
      { "Ranking", 1314, 9, 674, UI_XSTR_COLOR_GREEN, -1,
        &Multi_pxo_buttons[1][MULTI_PXO_RANKINGS].button },
      { "Pilot", 1310, 109, 664, UI_XSTR_COLOR_GREEN, -1,
        &Multi_pxo_buttons[1][MULTI_PXO_PINFO].button },
      { "Info", 1311, 115, 674, UI_XSTR_COLOR_GREEN, -1,
        &Multi_pxo_buttons[1][MULTI_PXO_PINFO].button },
      { "Find", 1315, 190, 664, UI_XSTR_COLOR_GREEN, -1,
        &Multi_pxo_buttons[1][MULTI_PXO_FIND].button },
      { "Motd", 1316, 58, 729, UI_XSTR_COLOR_GREEN, -1,
        &Multi_pxo_buttons[1][MULTI_PXO_MOTD].button },
      { "Join", 1505, 488, 129, UI_XSTR_COLOR_PINK, -1,
        &Multi_pxo_buttons[1][MULTI_PXO_JOIN].button },
      { "Channel", 1317, 461, 139, UI_XSTR_COLOR_PINK, -1,
        &Multi_pxo_buttons[1][MULTI_PXO_JOIN].button },
      { "Join", 1506, 487, 184, UI_XSTR_COLOR_PINK, -1,
        &Multi_pxo_buttons[1][MULTI_PXO_JOIN_PRIV].button },
      { "Private", 1318, 467, 194, UI_XSTR_COLOR_PINK, -1,
        &Multi_pxo_buttons[1][MULTI_PXO_JOIN_PRIV].button },
      { "Exit", 1416, 789, 678, UI_XSTR_COLOR_PINK, -1,
        &Multi_pxo_buttons[1][MULTI_PXO_EXIT].button },
      { "Help", 928, 857, 667, UI_XSTR_COLOR_GREEN, -1,
        &Multi_pxo_buttons[1][MULTI_PXO_HELP].button },
      { "Games", 1319, 917, 667, UI_XSTR_COLOR_PINK, -1,
...
// condition function for popup_do_with_condition for connected to Parallax
// Online return 10 : on successful connect
...
// channel we're currently connected to, num_users == -1, if we're not
// connected
pxo_channel Multi_pxo_channel_current;

// channel we're currently trying to change to, num_users == -1, if we're not
...
// create a new channel with the given name and place it on the channel list,
...
// try and find a player with the given name, return a pointer to his entry (or
// NULL)
...
// get the absolute index of the displayed items which our currently selected
// one is
int multi_pxo_get_select_index ();

DCF (
    players,
    "Adds the specified number of bogus players to the PXO listing "
    "(Multiplayer)") {
    char name[512] = "";
    int i;
...
int Multi_pxo_chat_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                       196, 197, 412, 185 },
                                                     { // GR_1024
                                                       314, 271, 665, 330 } };

int Multi_pxo_input_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                        196, 386, 407, 24 },
                                                      { // GR_1024
                                                        314, 617, 660, 38 } };
...
#define CHAT_MODE_CHANNEL_SWITCH \
    4                    // "switching channels" message - draw in red
...
const char* Multi_pxo_chat_slider_name[GR_NUM_RESOLUTIONS] = { "slider",
                                                               "2_slider" };
...
void multi_pxo_chat_process_incoming (
    const char* txt, int mode = CHAT_MODE_NORMAL);
...
// if the text is a private message, return a pointer to the beginning of the
...
// common dialog stuff ------------------------------------------------
const char* Multi_pxo_com_fname[GR_NUM_RESOLUTIONS] = { "PXOPop", "2_PXOPop" };
const char* Multi_pxo_com_mask_fname[GR_NUM_RESOLUTIONS] = { "PXOPop-m",
                                                             "2_PXOPop-m" };

// popup coords
int Multi_pxo_com_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                      38, 129 },
                                                    { // GR_1024
                                                      61, 207 } };

// input box coords
int Multi_pxo_com_input_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                            53, 233, 448, 25 },
                                                          { // GR_1024
                                                            85, 372, 716,
                                                            40 } };
...
ui_button_info
    Multi_pxo_com_buttons[GR_NUM_RESOLUTIONS][MULTI_PXO_COM_NUM_BUTTONS] = {
        { // GR_640
...
UI_XSTR Multi_pxo_com_text[GR_NUM_RESOLUTIONS][MULTI_PXO_COM_NUM_TEXT] = {
    { // GR_640
      { "&Cancel", 645, 510, 204, UI_XSTR_COLOR_PINK, -1,
        &Multi_pxo_com_buttons[0][MULTI_PXO_COM_CANCEL].button },
      { "&Ok", 669, 548, 233, UI_XSTR_COLOR_GREEN, -1,
        &Multi_pxo_com_buttons[0][MULTI_PXO_COM_OK].button } },
    { // GR_1024
      { "&Cancel", 645, 847, 327, UI_XSTR_COLOR_PINK, -1,
        &Multi_pxo_com_buttons[1][MULTI_PXO_COM_CANCEL].button },
      { "&Ok", 669, 877, 372, UI_XSTR_COLOR_GREEN, -1,
...
int Multi_pxo_com_top_text_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                               58, 152 },
                                                             { // GR_1024
                                                               91, 227 } };
...
// player info stuff -----------------------------------------
const char* Multi_pxo_pinfo_fname[GR_NUM_RESOLUTIONS] = { "PilotInfo2",
                                                          "2_PilotInfo2" };
const char* Multi_pxo_pinfo_mask_fname[GR_NUM_RESOLUTIONS] = {
    "PilotInfo2-M", "2_PilotInfo2-M"
};
...
// help screen stuff -----------------------------------------
// XSTR:OFF
const char* Multi_pxo_help_fname[GR_NUM_RESOLUTIONS] = { "PXHelp",
                                                         "2_PXHelp" };
const char* Multi_pxo_help_mask_fname[GR_NUM_RESOLUTIONS] = { "PXOHelp-M",
                                                              "2_PXOHelp-M" };
...
ui_button_info
    Multi_pxo_help_buttons[GR_NUM_RESOLUTIONS][MULTI_PXO_HELP_NUM_BUTTONS] = {
        {
            // GR_640
...
UI_XSTR Multi_pxo_help_text[GR_NUM_RESOLUTIONS][MULTI_PXO_HELP_NUM_TEXT] = {
    {
        // GR_640
        { "Continue", 1069, 571, 413, UI_XSTR_COLOR_PINK, -1,
          &Multi_pxo_help_buttons[0][MULTI_PXO_HELP_CONTINUE].button },
    },
    {
        // GR_1024
        { "Continue", 1069, 928, 663, UI_XSTR_COLOR_PINK, -1,
...
int Multi_pxo_help_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                       40, 40 },
                                                     { // GR_1024
                                                       60, 40 } };
...
int Pxo_ban_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                149, 3, 475, 75 },
                                              { // GR_1024
                                                524, 3, 475, 75 } };
...
#define PXO_BAN_MODE_IMAGES_DONE \
    4 // done downloading everything - now maybe load an image
#define PXO_BAN_MODE_IDLE 5 // done with everything - doing nothing
#define PXO_BAN_MODE_CHOOSE_RANDOM \
    6 // choose a bitmap we've already downloaded at random
...
typedef struct pxo_banner {
    ...
    char ban_file_url[MULTI_OPTIONS_STRING_LEN + 1]; // full url of the file to
                                                     // get (convenient)
...
void multi_pxo_init (int use_last_channel) {
    ...
    for (idx = 0; idx < MULTI_PXO_NUM_BUTTONS; idx++) {
        // create the object
        Multi_pxo_buttons[gr_screen.res][idx].button.create (
            &Multi_pxo_window, "", Multi_pxo_buttons[gr_screen.res][idx].x,
            Multi_pxo_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);

        // set the sound to play when highlighted
        Multi_pxo_buttons[gr_screen.res][idx].button.set_highlight_action (
            common_play_highlight_sound);

        // set the ani for the button
        Multi_pxo_buttons[gr_screen.res][idx].button.set_bmaps (
            Multi_pxo_buttons[gr_screen.res][idx].filename);

        // set the hotspot
        Multi_pxo_buttons[gr_screen.res][idx].button.link_hotspot (
            Multi_pxo_buttons[gr_screen.res][idx].hotspot);
    }
    ...
    Multi_pxo_buttons[gr_screen.res][MULTI_PXO_TEXT_DOWN].button.repeatable (
        1);
    Multi_pxo_buttons[gr_screen.res][MULTI_PXO_CHAN_UP].button.repeatable (1);
    Multi_pxo_buttons[gr_screen.res][MULTI_PXO_CHAN_DOWN].button.repeatable (
        1);
    Multi_pxo_buttons[gr_screen.res][MULTI_PXO_PLIST_UP].button.repeatable (1);
    Multi_pxo_buttons[gr_screen.res][MULTI_PXO_PLIST_DOWN].button.repeatable (
        1);

    // set the mouseover cursor if it loaded ok
    if (Web_cursor != NULL) {
        Multi_pxo_buttons[gr_screen.res][MULTI_PXO_RANKINGS]
            .button.set_custom_cursor (Web_cursor);
    }

    // create the channel list select button and hide it
    Multi_pxo_channel_button.create (
        &Multi_pxo_window, "", Multi_pxo_chan_coords[gr_screen.res][0],
        Multi_pxo_chan_coords[gr_screen.res][1],
        Multi_pxo_chan_coords[gr_screen.res][2],
        Multi_pxo_chan_coords[gr_screen.res][3], 0, 1);
    Multi_pxo_channel_button.hide ();

    // create the player list select button and hide it
    Multi_pxo_player_button.create (
        &Multi_pxo_window, "", Multi_pxo_player_coords[gr_screen.res][0],
        Multi_pxo_player_coords[gr_screen.res][1],
        Multi_pxo_player_coords[gr_screen.res][2],
        Multi_pxo_player_coords[gr_screen.res][3], 0, 1);
    Multi_pxo_player_button.hide ();

    // create the chat input box
    Multi_pxo_chat_input.create (
        &Multi_pxo_window, Multi_pxo_input_coords[gr_screen.res][0],
        Multi_pxo_input_coords[gr_screen.res][1],
        Multi_pxo_input_coords[gr_screen.res][2], MAX_CHAT_LINE_LEN + 1, "",
        UI_INPUTBOX_FLAG_INVIS | UI_INPUTBOX_FLAG_ESC_CLR |
            UI_INPUTBOX_FLAG_KEYTHRU | UI_INPUTBOX_FLAG_EAT_USED);
    Multi_pxo_chat_input.set_focus ();

    // create the banner button and hide it
    Multi_pxo_ban_button.create (
        &Multi_pxo_window, "", Pxo_ban_coords[gr_screen.res][0],
    ...
    Multi_pxo_chat_slider.create (
        &Multi_pxo_window, Multi_pxo_chat_slider_coords[gr_screen.res][0],
        ...
        Multi_pxo_chat_slider_name[gr_screen.res], multi_pxo_scroll_chat_up,
        multi_pxo_scroll_chat_down, NULL);
...
void multi_pxo_do () {
    ...
    // run api stuff
    if (Multi_pxo_connected) { multi_pxo_api_process (); }
    ...
    case MULTI_PXO_MODE_FIND:
        ...
        break;
    // normal mode
    case MULTI_PXO_MODE_NORMAL: multi_pxo_do_normal (); break;
    }
}
// XSTR:ON
...
void multi_pxo_do_normal () {
    ...
    if (Multi_pxo_must_validate) {
        // validate the current player with the master tracker (will create the
        // pilot on the MT if necessary)
    ...
    if (Multi_pxo_must_connect) {
        ...
        // if we successfully connected, send a request for a list of channels
        // on the server
        if (Multi_pxo_connected) {
            ...
            // set our status
            multi_pxo_set_status_text (
                XSTR ("Retrieving Public Channels", 939));
        }
        else {
            // set our status
            multi_pxo_set_status_text (
                XSTR ("Failed to connect to Parallax Online", 940));
        }
...
void multi_pxo_blit_all () {
    ...
    do {
        ...
        if (bmap != -1) {
            ...
            if ((bmw != gr_screen.max_w_unscaled) ||
                (bmh != gr_screen.max_h_unscaled)) {
                gr_clear ();
            }
        }
        else {
    ...
    if (gameseq_get_state () == GS_STATE_PXO && Multi_pxo_anim.num_frames > 0)
        generic_anim_render (
            &Multi_pxo_anim, flFrametime, MULTI_PXO_ANIM_X, MULTI_PXO_ANIM_Y,
            true);
}
...
void multi_pxo_handle_disconnect () {
    ...
    if (popup_active ()) {
        popup_change_text (
            XSTR ("You have been disconnected from the server", 942));
    }
    else {
        popup (
...
void multi_pxo_strip_space (char* string1, char* string2) {
    ...
    tok = strtok (midway, " ");
    if (tok != NULL) { strcpy (string2, tok); }
    else {
        strcpy (string2, "");
    }
}
...
void multi_pxo_button_pressed (int n) {
    ...
    case MULTI_PXO_CHAN_UP: multi_pxo_scroll_channels_up (); break;

    case MULTI_PXO_CHAN_DOWN: multi_pxo_scroll_channels_down (); break;

    case MULTI_PXO_TEXT_UP: multi_pxo_scroll_chat_up (); break;

    case MULTI_PXO_TEXT_DOWN: multi_pxo_scroll_chat_down (); break;
    ...
    case MULTI_PXO_JOIN:
        ...
        if (!SWITCHING_CHANNELS () && (Multi_pxo_channel_select != NULL)) {
            ...
            multi_pxo_notify_add (
                XSTR ("Already trying to join a channel!", 945));
            gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
        }
        break;
    ...
    case MULTI_PXO_RANKINGS:
        ...
        if ((Multi_pxo_ranking_last < 0.0f) ||
            ((f2fl (timer_get_fixed_seconds ()) - Multi_pxo_ranking_last) >
             MULTI_PXO_RANK_TIME)) {
            gamesnd_play_iface (InterfaceSounds::USER_SELECT);
...
int multi_pxo_connect_do () {
    ...
    if (!mpxo_failed) {
        ...
        if (snprintf (
                ip_string, MAX_PXO_TEXT_LEN, "%s:%d", Multi_options_g.pxo_ip,
                PXO_CHAT_PORT) >= MAX_PXO_TEXT_LEN) {
...
int multi_pxo_autojoin_do () {
    ...
    // next value is not -1 when actually switching channels, so keep
    // processing by returning 0.
    if (SWITCHING_CHANNELS ()) return 0;

    // couldn't switch channel for some reason.  bail out with -1
    if (!ON_CHANNEL ()) return -1;
...
int multi_pxo_connect () {
    ...
    if (popup_till_condition (
            multi_pxo_connect_do, XSTR ("&Cancel", 779),
            XSTR ("Logging into Parallax Online", 949)) == 10) {
        int rval;
        ...
        if (Multi_pxo_use_last_channel && strlen (Multi_pxo_channel_last)) {
            ...
            strcpy_s (
                join_fail_str, XSTR ("Unable to join last channel", 983));
        }
        else {
            strcpy_s (join_str, XSTR ("Autojoining public channel", 984));
            strcpy_s (
                join_fail_str,
                XSTR ("Unable to autojoin public channel", 985));
        }

        // once connected, we should do an autojoin before allowing the guy to
        // continue.
        rval = popup_till_condition (
            multi_pxo_autojoin_do, XSTR ("&Cancel", 779), join_str);
    ...
    if (mpxo_failed) {
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            XSTR ("Failed to connect to Parallax Online!", 947));
    }

    // if we are coming from the mainhall then fail to the join game screen
    // rather than keeping the user constantly at the mainhall
...
void multi_pxo_api_process () {
    ...
    if (p) { multi_pxo_make_channels (p); }
    ...
    while (cmd) {
        ...
        case CC_USER_JOINING:
            ...
            if (ON_CHANNEL ()) {
                lookup = multi_pxo_find_channel (
                    Multi_pxo_channel_current.name, Multi_pxo_channels);

                if (lookup != NULL) lookup->num_users++;
            }
            break;
        ...
        case CC_DISCONNECTED: multi_pxo_handle_disconnect (); break;

        case CC_KICKED: multi_pxo_handle_kick (); break;
        ...
        case CC_YOURCHANNEL:
            ...
            if (lookup == NULL) {
                // create a new channel with the given name and place it on the
                // channel list, return a pointer or NULL on fail
                lookup = multi_pxo_add_channel (
                    Multi_pxo_channel_current.name, &Multi_pxo_channels);
            }

            // set the user count to be 0
            if (lookup != NULL) lookup->num_users = 0;
    ...
    // handle any processing details if we're currently trying to join a
    // channel
...
void multi_pxo_channel_count_update (char* name, int count) {
    ...
    if (lookup != NULL) {
        ...
        ml_printf (
            "PXO : unable to locate channel when trying to update count for "
            "%s",
            name);
    }
}
...
void multi_pxo_set_status_text (const char* txt) {
    ...
    font::force_fit_string (
        Multi_pxo_status_text, MAX_PXO_TEXT_LEN - 1,
        Multi_pxo_status_coords[gr_screen.res][2]);
}
...
void multi_pxo_blit_status_text () {
    ...
    if (strlen (Multi_pxo_status_text)) {
        ...
        gr_string (
            Multi_pxo_status_coords[gr_screen.res][0] +
                ((Multi_pxo_status_coords[gr_screen.res][2] - w) / 2),
 ...
 */
void multi_pxo_get_channels () { SendChatString (NOX ("/list")); }
...
void multi_pxo_make_channels (char* chan_str) {
    ...
    do {
        ...
        // if the # of users is > 0, or its not an autojoin, place it on the
        // display list
        if ((num_users > 0) || !multi_pxo_is_autojoin (name_tok)) {
            ...
            if (lookup != NULL) { lookup->num_users = (short)num_users; }
            // add the channel
            else {
...
void multi_pxo_process_channels () {
    ...
    if (Multi_pxo_channel_button.pressed ()) {
        ...
        if ((item_index + Multi_pxo_channel_start_index) <
            Multi_pxo_channel_count) {
    ...
    // last refresh time
    if ((Multi_pxo_channel_last_refresh > 0.0f) &&
        ((f2fl (timer_get_fixed_seconds ()) - Multi_pxo_channel_last_refresh) >
         CHANNEL_REFRESH_TIME)) {
        // refresh channels
        multi_pxo_set_status_text (
            XSTR ("Refreshing Public Channel List", 952));
    ...
    // last refresh time
    if ((Multi_pxo_channel_server_refresh > 0.0f) &&
        ((f2fl (timer_get_fixed_seconds ()) -
          Multi_pxo_channel_server_refresh) > CHANNEL_SERVER_REFRESH_TIME)) {
        // do it _NOW_ I"M RIGHT HERE KILL ME WHAT ARE YOU WAITING FOR DO IT
        // KILL ME DO IT NOW!
        multi_pxo_channel_refresh_servers ();
    }
}
...
void multi_pxo_channel_refresh_servers () {
    ...
    // traverse the list of existing channels we know about and query the game
    // tracker about them
...
void multi_pxo_blit_channels () {
    ...
    do {
        ...
        font::force_fit_string (
            ...
            Multi_pxo_chan_coords[gr_screen.res][2] -
                Multi_pxo_chan_column_offsets[gr_screen.res]
                                             [CHAN_PLAYERS_COLUMN]);

        // blit the strings
        gr_string (
            Multi_pxo_chan_coords[gr_screen.res][0], y_start, chan_name + 1,
            GR_RESIZE_MENU);
        gr_string (
            Multi_pxo_chan_coords[gr_screen.res][0] +
                Multi_pxo_chan_coords[gr_screen.res][2] -
                Multi_pxo_chan_column_offsets[gr_screen.res]
                                             [CHAN_PLAYERS_COLUMN],
            y_start, chan_users, GR_RESIZE_MENU);
        gr_set_color_fast (&Color_bright);
        gr_string (
            Multi_pxo_chan_coords[gr_screen.res][0] +
                Multi_pxo_chan_coords[gr_screen.res][2] -
                Multi_pxo_chan_column_offsets[gr_screen.res]
                                             [CHAN_GAMES_COLUMN],
            y_start, chan_servers, GR_RESIZE_MENU);
        ...
        moveup = moveup->next;
             ...
             (disp_count < Multi_pxo_max_chan_display[gr_screen.res]));
}
...
void multi_pxo_scroll_channels_down () {
    ...
    if ((Multi_pxo_channel_start == NULL) ||
        (Multi_pxo_channel_start->next == Multi_pxo_channels)) {
        gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
        return;
    }

    // if we can't scroll further without going past the end of the viewable
    // list, don't
    if ((Multi_pxo_channel_start_index +
         Multi_pxo_max_chan_display[gr_screen.res]) >=
        Multi_pxo_channel_count) {
        gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
        return;
    }
...
void multi_pxo_join_channel (pxo_channel* chan) {
    ...
    case 0:
        ...
        if (strlen (Multi_pxo_channel_switch.name) > 1) {
            sprintf (
                switch_msg, "[Switching to channel %s]",
                Multi_pxo_channel_switch.name + 1);
        }
        else {
            sprintf (
                switch_msg, "[Switching to channel %s]",
                Multi_pxo_channel_switch.name);
        }
...
void multi_pxo_handle_channel_change () {
    ...
    case -1:
        ...
        // notify of error
        multi_pxo_set_status_text (
            XSTR ("No channel (error while switching)", 954));
        break;
    ...
    case 1:
        // copy the current channel info, and unset the switching status
        memcpy (
            &Multi_pxo_channel_current, &Multi_pxo_channel_switch,
            sizeof (pxo_channel));
        Multi_pxo_channel_switch.num_users = -1;
        ...
        if (lookup != NULL) { lookup->num_users = 1; }
    ...
    do {
        ...
        if (!strcasecmp (name, lookup->name)) {
            ...
            // otherwise, just delete it
                ...
                if ((lookup == Multi_pxo_player_select) &&
                    (Multi_pxo_player_select != NULL)) {
...
void multi_pxo_blit_players () {
    ...
    do {
        ...
        font::force_fit_string (
            player_name, MAX_PXO_TEXT_LEN - 1,
            Multi_pxo_player_coords[gr_screen.res][2]);

        // blit the string
        gr_string (
            Multi_pxo_player_coords[gr_screen.res][0], y_start, player_name,
            GR_RESIZE_MENU);
        ...
        moveup = moveup->next;
             ...
             (disp_count < Multi_pxo_max_player_display[gr_screen.res]));
}
...
void multi_pxo_chat_init () {
    ...
    for (idx = 0; idx < MAX_CHAT_LINES; idx++) {
        ...
        if (Multi_pxo_chat == NULL) { Multi_pxo_chat = new_line; }
...
void multi_pxo_chat_add_line (const char* txt, int mode) {
    ...
    if (Multi_pxo_chat_count < MAX_CHAT_LINES) { Multi_pxo_chat_count++; }

    // set the count
    Multi_pxo_chat_slider.set_numberItems (
        Multi_pxo_chat_count > Multi_pxo_max_chat_display[gr_screen.res]
            ? Multi_pxo_chat_count - Multi_pxo_max_chat_display[gr_screen.res]
            : 0,
...
void multi_pxo_chat_process_incoming (const char* txt, int mode) {
    ...
    // if the text is a private message, return a pointer to the beginning of
    // the message, otherwise return NULL
    priv_ptr = multi_pxo_chat_is_private (txt);
    if (priv_ptr != NULL) { strcpy_s (msg_total, priv_ptr); }
    else {
    ...
    if (priv_ptr != NULL) { mode = CHAT_MODE_PRIVATE; }
    // all other chat
    else {
        // if this is a server message
        if (multi_pxo_is_server_text (txt)) { mode = CHAT_MODE_SERVER; }
        // if this is a MOTD
    ...
    if (n_lines == 1) { multi_pxo_chat_add_line (msg_total, mode); }
    // if the string was split into multiple lines
        ...
        for (idx = 1; idx < n_lines; idx++) {
            ...
            // unless the current mode is server or "switching channels", make
            // all these CHAT_MODE_CARRY
            if ((mode != CHAT_MODE_SERVER) &&
                (mode != CHAT_MODE_CHANNEL_SWITCH)) {
...
void multi_pxo_chat_blit () {
    ...
    if (ON_CHANNEL ()) {
        if (strlen (Multi_pxo_channel_current.name) > 1) {
            sprintf (
                title, XSTR ("%s on %s", 955), Multi_pxo_nick,
                Multi_pxo_channel_current.name +
                    1); // [[ <who> on <channel> ]]
        }
        else {
            sprintf (
    ...
    font::force_fit_string (
        title, MAX_PXO_TEXT_LEN - 1,
        Multi_pxo_chat_coords[gr_screen.res][2] - 10);
    gr_get_string_size (&token_width, NULL, title);
    gr_set_color_fast (&Color_normal);
    gr_string (
        Multi_pxo_chat_coords[gr_screen.res][0] +
            ((Multi_pxo_chat_coords[gr_screen.res][2] - token_width) / 2),
        Multi_pxo_chat_title_y[gr_screen.res], title, GR_RESIZE_MENU);
    ...
    while ((moveup != NULL) && (moveup != Multi_pxo_chat_add) &&
           (disp_count < (Multi_pxo_max_chat_display[gr_screen.res]))) {
        switch (moveup->mode) {
        // if this is text from the server, display it all "bright"
        case CHAT_MODE_SERVER:
            gr_set_color_fast (&Color_bright);
            gr_string (
                Multi_pxo_chat_coords[gr_screen.res][0], y_start, moveup->text,
                GR_RESIZE_MENU);
            break;
        ...
        case CHAT_MODE_NORMAL:
            ...
            if (tok != NULL) {
                ...
                gr_set_color_fast (&Color_bright);
                gr_string (
                    Multi_pxo_chat_coords[gr_screen.res][0], y_start, tok,
                    GR_RESIZE_MENU);
                ...
                if (tok != NULL) {
                    gr_set_color_fast (&Color_normal);
                    gr_string (
                        Multi_pxo_chat_coords[gr_screen.res][0] + token_width +
                            6,
                        y_start, tok, GR_RESIZE_MENU);
                }
            }
            break;
...
int multi_pxo_can_scroll_down () {
    ...
    if (count > Multi_pxo_max_chat_display[gr_screen.res]) { return 1; }
    else {
        return 0;
    }
}
...
void multi_pxo_chat_process () {
    ...
    // if the chat line is getting too long, fire off the message, putting the
    // last word on the next input line.
    memset (msg, 0, 512);
    Multi_pxo_chat_input.get_text (msg);
    remainder = "";

    // determine if the width of the string in pixels is > than the inputbox
    // width -- if so, then send the message
    ...
    if (msg_pixel_width >= (Multi_pxo_input_coords[gr_screen.res][2])) {
        ...
        if (ON_CHANNEL ()) {
            ...
            if (result != NULL) { multi_pxo_chat_process_incoming (result); }
        ...
        (Multi_pxo_chat_input.pressed () && (msg[0] != '\0')) ||
        (strlen (msg) >= MAX_CHAT_LINE_LEN)) {
        // tack on the null terminator in the boundary case
        size_t x = strlen (msg);
        if (x >= MAX_CHAT_LINE_LEN) { msg[MAX_CHAT_LINE_LEN - 1] = '\0'; }
...
// NOTE : DO NOT LOCALIZE THESE STRINGS!!!! THEY ARE CONSTANTS WHICH ARE
// CHECKED AGAINST PXO CHAT SERVER DATA. THEY CANNOT CHANGE!!!
...
int multi_pxo_is_server_text (const char* txt) {
    ...
    if ((strlen (txt) >= pxo_prefix_len) &&
        !strncmp (txt, MULTI_PXO_SERVER_PREFIX, pxo_prefix_len)) {
        return 1;
    }
 ...
 */
int multi_pxo_is_motd_text (const char* txt) {
    // if we're not on a channel, and this is not a channel switching message
    // assume its coming from a server
    if ((strlen (txt) >= motd_prefix_len) &&
        !strncmp (txt, PXO_CHAT_MOTD_PREFIX, motd_prefix_len)) {
        return 1;
    }
...
int multi_pxo_is_end_of_motd_text (const char* txt) {
    // if we're not on a channel, and this is not a channel switching message
    // assume its coming from a server
    if ((strlen (txt) >= end_motd_prefix_len) &&
        !strncmp (txt, PXO_CHAT_END_OF_MOTD_PREFIX, end_motd_prefix_len)) {
        return 1;
    }
...
int multi_pxo_chat_is_left_message (const char* txt) {
    ...
    if (!multi_pxo_is_server_text (txt)) { return 0; }

    // check to see if the last portion is the correct wording
    if ((strlen (txt) > strlen (MULTI_PXO_HAS_LEFT)) &&
        !strcmp (
            &txt[strlen (txt) - strlen (MULTI_PXO_HAS_LEFT)],
            MULTI_PXO_HAS_LEFT)) {
        return 1;
    }
...
void multi_pxo_motd_add_text (const char* text) {
    ...
    if (strlen (text) <= motd_prefix_len) { return; }
...
void multi_pxo_set_end_of_motd () {
    ...
    // do we have an old MOTD file laying around? If so, read it in and see if
    // its the same
    uint old_chksum;
    uint new_chksum;
    ...
    if (in != NULL) {
        ...
        if (new_chksum == old_chksum) { blink = 0; }
    }
...
void multi_pxo_motd_maybe_blit () {
    ...
    if (Pxo_motd_end && !Pxo_motd_read && (Pxo_motd_blink_stamp != -1)) {
        ...
        if (Pxo_motd_blink_on) {
            Multi_pxo_buttons[gr_screen.res][MULTI_PXO_MOTD]
...
void multi_pxo_com_init (int input_len) {
    ...
    // create the interface window
    Multi_pxo_com_window.create (
        0, 0, gr_screen.max_w_unscaled, gr_screen.max_h_unscaled, 0);
    Multi_pxo_com_window.set_mask_bmap (
        Multi_pxo_com_mask_fname[gr_screen.res]);

    // create the interface buttons
    for (idx = 0; idx < MULTI_PXO_COM_NUM_BUTTONS; idx++) {
        // create the object
        Multi_pxo_com_buttons[gr_screen.res][idx].button.create (
            &Multi_pxo_com_window, "",
            Multi_pxo_com_buttons[gr_screen.res][idx].x,
            Multi_pxo_com_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);

        // set the sound to play when highlighted
        Multi_pxo_com_buttons[gr_screen.res][idx].button.set_highlight_action (
            common_play_highlight_sound);

        // set the ani for the button
        Multi_pxo_com_buttons[gr_screen.res][idx].button.set_bmaps (
            Multi_pxo_com_buttons[gr_screen.res][idx].filename);

        // set the hotspot
        Multi_pxo_com_buttons[gr_screen.res][idx].button.link_hotspot (
            Multi_pxo_com_buttons[gr_screen.res][idx].hotspot);
    }

    // add xstrs
    for (idx = 0; idx < MULTI_PXO_COM_NUM_TEXT; idx++) {
        Multi_pxo_com_window.add_XSTR (
            &Multi_pxo_com_text[gr_screen.res][idx]);
    }

    // create the input box
    Multi_pxo_com_input.create (
        &Multi_pxo_com_window, Multi_pxo_com_input_coords[gr_screen.res][0],
...
void multi_pxo_com_blit_text () {
    ...
    if (Multi_pxo_com_top_text[0] != '\0') {
        gr_set_color_fast (&Color_bright);
        gr_string (
            Multi_pxo_com_top_text_coords[gr_screen.res][0],
            Multi_pxo_com_top_text_coords[gr_screen.res][1],
            Multi_pxo_com_top_text, GR_RESIZE_MENU);
    }
    if (Multi_pxo_com_middle_text[0] != '\0') {
        gr_set_color_fast (&Color_bright);
        gr_string (
            Multi_pxo_com_top_text_coords[gr_screen.res][0],
            Multi_pxo_com_middle_text_y[gr_screen.res],
            Multi_pxo_com_middle_text, GR_RESIZE_MENU);
    }
    if (Multi_pxo_com_bottom_text[0] != '\0') {
        gr_set_color_fast (&Color_bright);
        gr_string (
            Multi_pxo_com_top_text_coords[gr_screen.res][0],
            Multi_pxo_com_bottom_text_y[gr_screen.res],
            Multi_pxo_com_bottom_text, GR_RESIZE_MENU);
    }
}
...
void multi_pxo_com_set_top_text (const char* txt) {
    if ((txt != NULL) && strlen (txt)) {
        ...
        font::force_fit_string (
            Multi_pxo_com_top_text, MAX_PXO_TEXT_LEN - 1,
            Multi_pxo_com_input_coords[gr_screen.res][2]);
    }
}
...
void multi_pxo_com_set_middle_text (const char* txt) {
    if ((txt != NULL) && strlen (txt)) {
        ...
        font::force_fit_string (
            Multi_pxo_com_middle_text, MAX_PXO_TEXT_LEN - 1,
            Multi_pxo_com_input_coords[gr_screen.res][2]);
    }
}
...
void multi_pxo_com_set_bottom_text (const char* txt) {
    if ((txt != NULL) && strlen (txt)) {
        ...
        font::force_fit_string (
            Multi_pxo_com_bottom_text, MAX_PXO_TEXT_LEN - 1,
            Multi_pxo_com_input_coords[gr_screen.res][2]);
    }
}
...
void multi_pxo_priv_init () {
    ...
    // set some text
    multi_pxo_com_set_middle_text (
        XSTR ("Type the name of the channel to join/create", 961));
}
...
int multi_pxo_priv_popup () {
    ...
    case KEY_ESC: Multi_pxo_priv_return_code = 0; break;
    }
    ...
    gr_bitmap (
        Multi_pxo_com_coords[gr_screen.res][0],
        Multi_pxo_com_coords[gr_screen.res][1], GR_RESIZE_MENU);
    ...
    // user hit cancel
    case 0: multi_pxo_priv_close (); return -1;

    // user hit ok
    case 1: multi_pxo_priv_close (); return 1;
    }
...
void multi_pxo_priv_button_pressed (int n) {
    ...
    switch (n) {
    case MULTI_PXO_COM_CANCEL: Multi_pxo_priv_return_code = 0; break;
...
int multi_pxo_find_popup () {
    ...
    switch (k) {
    // like hitting the cancel button
    case KEY_ESC: Multi_pxo_find_return_code = 0; break;
    }
    ...
    case 1:
        ...
        if (Multi_pxo_find_channel[0] != '\0') {
            pxo_channel* lookup;
            lookup = multi_pxo_find_channel (
                Multi_pxo_find_channel, Multi_pxo_channels);

            // if we couldn't find it, don't join
            if (lookup != NULL) { multi_pxo_join_channel (lookup); }
        }
        return 1;
    }
...
void multi_pxo_find_button_pressed (int n) {
    switch (n) {
    case MULTI_PXO_COM_CANCEL: Multi_pxo_find_return_code = 0; break;

    case MULTI_PXO_COM_OK: Multi_pxo_find_return_code = 1; break;
    }
}
    ...
    if (Multi_pxo_com_input.pressed ()) {
        ...
        if (!Multi_pxo_searching) {
            ...
            if (name_lookup[0] != '\0') {
                ...
                sprintf (
                    search_text, XSTR ("Searching for %s", 963), name_lookup);
...
void multi_pxo_find_search_process () {
    ...
    if (channel) {
        ...
        if ((ptr_s)channel == -1) {
            ...
            if (channel[0] == '*') {
                multi_pxo_com_set_middle_text (
                    XSTR ("Player is logged in but is not on a channel", 965));
                strcpy_s (Multi_pxo_find_channel, "");
            }
            else {
                char p_text[512];
                ...
                if (channel[0] == '#') {
                    ...
                    if (Multi_pxo_find_channel
                            [strlen (Multi_pxo_find_channel) - 1] == ' ') {
                        Multi_pxo_find_channel
                            [strlen (Multi_pxo_find_channel) - 1] = '\0';
                    }
                }
                // if this is a private channel
                else if (channel[0] == '+') {
                    sprintf (
                        p_text, XSTR ("Found %s on a private channel", 967),
                        name_lookup);
                    multi_pxo_com_set_middle_text (p_text);
...
int multi_pxo_pinfo_cond () {
    ...
    case 0: {
        ...
        if (ret_string != NULL) {
            ...
            if (tok != NULL) {
                ...
                if (tok != NULL) { strcpy_s (Multi_pxo_retrieve_name, tok); }
                // failure
                else {
                    return 1;
                }
    ...
    case 1: {
        ...
        switch (fs2netd_get_pilot_info (
            Multi_pxo_retrieve_name, &Multi_pxo_pinfo_player, true)) {
        // there was some failure
        case -2: return 2;

        // still processing
        case -1: Multi_pxo_retrieve_mode = 2; break;
    ...
    case 2: {
        switch (fs2netd_get_pilot_info (
            Multi_pxo_retrieve_name, &Multi_pxo_pinfo_player, false)) {
        // there was some failure
        case -2: return 2;
...
int multi_pxo_pinfo_get (char* name) {
    ...
    switch (popup_till_condition (
        multi_pxo_pinfo_cond, XSTR ("&Cancel", 779),
        XSTR ("Retrieving player tracker id", 969))) {
    // success
    case 10: return 1;
...
void multi_pxo_pinfo_show () {
    ...
    // run the popup
    do { game_set_frametime (GS_STATE_PXO); } while (!multi_pxo_pinfo_do ());
...
void multi_pxo_pinfo_build_vals () {
    ...
    font::force_fit_string (
        Multi_pxo_pinfo_vals[0], 49,
        Multi_pxo_pinfo_coords[gr_screen.res][2] -
            (Multi_pxo_pinfo_val_x[gr_screen.res] -
             Multi_pxo_pinfo_coords[gr_screen.res][0]));

    // rank
    memset (Multi_pxo_pinfo_vals[1], 0, 50);
    multi_sg_rank_build_name (
        Ranks[fs->stats.rank].name, Multi_pxo_pinfo_vals[1]);
    font::force_fit_string (
        Multi_pxo_pinfo_vals[1], 49,
        Multi_pxo_pinfo_coords[gr_screen.res][2] -
            (Multi_pxo_pinfo_val_x[gr_screen.res] -
             Multi_pxo_pinfo_coords[gr_screen.res][0]));
    ...
    sprintf (
        Multi_pxo_pinfo_vals[4], "%d",
        fs->stats.kill_count - fs->stats.kill_count_ok);
    ...
    game_format_time (
        fl2f ((float)fs->stats.flight_time), Multi_pxo_pinfo_vals[6]);
    ...
    if (fs->stats.last_flown == 0) {
        ...
        time_t tmp_lf = fs->stats.last_flown; // don't cast a pointer to a type
                                              // that can be either 32 or 64bit
    ...
    if (fs->stats.p_shots_fired > 0) {
        sprintf (
            Multi_pxo_pinfo_vals[10], "%d%%",
            (int)((float)fs->stats.p_shots_hit / (float)fs->stats.p_shots_fired * 100.0f));
    }
    else {
    ...
    if (fs->stats.s_shots_fired > 0) {
        sprintf (
            Multi_pxo_pinfo_vals[13], "%d%%",
            (int)((float)fs->stats.s_shots_hit / (float)fs->stats.s_shots_fired * 100.0f));
    }
    else {
...
void multi_pxo_pinfo_init () {
    ...
    // create the interface window
    Multi_pxo_pinfo_window.create (
        0, 0, gr_screen.max_w_unscaled, gr_screen.max_h_unscaled, 0);
    Multi_pxo_pinfo_window.set_mask_bmap (
    ...
    for (idx = 0; idx < MULTI_PXO_PINFO_NUM_BUTTONS; idx++) {
        // create the object
        Multi_pxo_pinfo_buttons[gr_screen.res][idx].button.create (
            &Multi_pxo_pinfo_window, "",
            Multi_pxo_pinfo_buttons[gr_screen.res][idx].x,
            Multi_pxo_pinfo_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);

        // set the sound to play when highlighted
        Multi_pxo_pinfo_buttons[gr_screen.res][idx]
            .button.set_highlight_action (common_play_highlight_sound);

        // set the ani for the button
        Multi_pxo_pinfo_buttons[gr_screen.res][idx].button.set_bmaps (
            Multi_pxo_pinfo_buttons[gr_screen.res][idx].filename);

        // set the hotspot
        Multi_pxo_pinfo_buttons[gr_screen.res][idx].button.link_hotspot (
            Multi_pxo_pinfo_buttons[gr_screen.res][idx].hotspot);
    }

    // add xstrs
    for (idx = 0; idx < MULTI_PXO_PINFO_NUM_TEXT; idx++) {
        Multi_pxo_pinfo_window.add_XSTR (
            &Multi_pxo_pinfo_text[gr_screen.res][idx]);
    }
    ...
    Multi_pxo_pinfo_stats_labels[4] =
        vm_strdup (XSTR ("Friendly kills", 1536));
    Multi_pxo_pinfo_stats_labels[5] =
        vm_strdup (XSTR ("Missions flown", 1537));
    ...
    Multi_pxo_pinfo_stats_labels[8] =
        vm_strdup (XSTR ("Primary shots fired", 1540));
    Multi_pxo_pinfo_stats_labels[9] =
        vm_strdup (XSTR ("Primary shots hit", 1541));
    Multi_pxo_pinfo_stats_labels[10] =
        vm_strdup (XSTR ("Primary hit %", 1542));
    Multi_pxo_pinfo_stats_labels[11] =
        vm_strdup (XSTR ("Secondary shots fired", 1543));
    Multi_pxo_pinfo_stats_labels[12] =
        vm_strdup (XSTR ("Secondary shots hit", 1544));
    Multi_pxo_pinfo_stats_labels[13] =
        vm_strdup (XSTR ("Secondary hit %", 1545));
    Multi_pxo_pinfo_stats_labels[14] =
        vm_strdup (XSTR ("Primary friendly hits", 1546));
    Multi_pxo_pinfo_stats_labels[15] =
        vm_strdup (XSTR ("Primary friendly hit %", 1547));
    Multi_pxo_pinfo_stats_labels[16] =
        vm_strdup (XSTR ("Secondary friendly hits", 1548));
    Multi_pxo_pinfo_stats_labels[17] =
        vm_strdup (XSTR ("Secondary friendly hit %", 1549));
...
int multi_pxo_pinfo_do () {
    ...
    if (Multi_pxo_pinfo_buttons[gr_screen.res][MULTI_PXO_PINFO_OK]
            .button.pressed ()) {
        return 1;
    }

    // if he pressed the medals buttons, run the medals screen
    if (Multi_pxo_pinfo_buttons[gr_screen.res][MULTI_PXO_PINFO_MEDALS]
            .button.pressed ()) {
...
void multi_pxo_pinfo_close () {
    ...
    // unload the bitmap
    if (Multi_pxo_pinfo_bitmap != -1) { bm_unload (Multi_pxo_pinfo_bitmap); }
...
void multi_pxo_pinfo_blit () {
    ...
    for (idx = 0; idx < MULTI_PXO_PINFO_NUM_LABELS; idx++) {
        ...
        gr_set_color_fast (&Color_bright);
        gr_string (
            Multi_pxo_pinfo_coords[gr_screen.res][0], y_start,
            Multi_pxo_pinfo_stats_labels[idx], GR_RESIZE_MENU);

        // blit the label's value
        gr_set_color_fast (&Color_normal);
        gr_string (
            Multi_pxo_pinfo_val_x[gr_screen.res], y_start,
            Multi_pxo_pinfo_vals[idx], GR_RESIZE_MENU);
...
void multi_pxo_notify_blit () {
    ...
    if (Multi_pxo_notify_stamp == -1) { return; }
    ...
    gr_string (
        (gr_screen.max_w_unscaled - w) / 2, MULTI_PXO_NOTIFY_Y,
        Multi_pxo_notify_text, GR_RESIZE_MENU);
}
...
void multi_pxo_help_init () {
    ...
    // create the interface window
    Multi_pxo_help_window.create (
        0, 0, gr_screen.max_w_unscaled, gr_screen.max_h_unscaled, 0);
    Multi_pxo_help_window.set_mask_bmap (
        Multi_pxo_help_mask_fname[gr_screen.res]);

    // create the interface buttons
    for (idx = 0; idx < MULTI_PXO_HELP_NUM_BUTTONS; idx++) {
        // create the object
        Multi_pxo_help_buttons[gr_screen.res][idx].button.create (
            &Multi_pxo_help_window, "",
            Multi_pxo_help_buttons[gr_screen.res][idx].x,
            Multi_pxo_help_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);

        // set the sound to play when highlighted
        Multi_pxo_help_buttons[gr_screen.res][idx]
            .button.set_highlight_action (common_play_highlight_sound);

        // set the ani for the button
        Multi_pxo_help_buttons[gr_screen.res][idx].button.set_bmaps (
            Multi_pxo_help_buttons[gr_screen.res][idx].filename);

        // set the hotspot
        Multi_pxo_help_buttons[gr_screen.res][idx].button.link_hotspot (
            Multi_pxo_help_buttons[gr_screen.res][idx].hotspot);
    }

    // add xstrs
    for (idx = 0; idx < MULTI_PXO_HELP_NUM_TEXT; idx++) {
        Multi_pxo_help_window.add_XSTR (
            &Multi_pxo_help_text[gr_screen.res][idx]);
    }
...
void multi_pxo_help_load () {
    ...
    while (!cfeof (in)) {
        // malloc the line
        cp->text[cp->num_lines] =
            (char*)vm_malloc (Multi_pxo_chars_per_line[gr_screen.res]);
        if (cp->text[cp->num_lines] == NULL) { break; }

        // read in the next line
        cfgets (
            cp->text[cp->num_lines++], Multi_pxo_chars_per_line[gr_screen.res],
            in);
...
void multi_pxo_help_blit_page () {
    ...
    for (idx = 0; idx < cp->num_lines; idx++) {
        ...
        // blit the line
        gr_string (
            Multi_pxo_help_coords[gr_screen.res][0], y_start,
            cp->text[idx] + start_pos, GR_RESIZE_MENU);
...
void multi_pxo_ban_init () {
    ...
    if (os_config_read_uint (NULL, "PXOBanners", 1)) {
        // if we're already in idle mode, we're done downloading for this
        // instance of freespace. pick a random image we already have
...
void multi_pxo_ban_process () {
    ...
    case PXO_BAN_MODE_LIST_STARTUP:
        // remote file
        sprintf (
            url_string, "http://www.pxo.net/files/%s",
            PXO_BANNERS_CONFIG_FILE);

        // local file
        cf_create_default_path_string (
            local_file, sizeof (local_file) - 1, CF_TYPE_MULTI_CACHE,
            PXO_BANNERS_CONFIG_FILE);
    ...
    case PXO_BAN_MODE_LIST:
        ...
        if (Multi_pxo_ban_get->IsConnecting () ||
            Multi_pxo_ban_get->IsReceiving ())
            break;
    ...
    case PXO_BAN_MODE_IMAGES_STARTUP:
        ...
        if ((strlen (Multi_pxo_banner.ban_file) <= 0) ||
            (strlen (Multi_pxo_banner.ban_file_url) <= 0)) {
            Multi_pxo_ban_mode = PXO_BAN_MODE_IDLE;
            break;
        }
    ...
    case PXO_BAN_MODE_IDLE:
        ...
        if (Multi_pxo_ban_button.pressed ()) { multi_pxo_ban_clicked (); }
        break;
...
void multi_pxo_ban_parse_banner_file (int choose_existing) {
    ...
    int num_banners, idx;
    CFILE* in = cfopen (
        PXO_BANNERS_CONFIG_FILE, "rt", CFILE_NORMAL, CF_TYPE_MULTI_CACHE);
    ...
    while (num_banners < 10) {
        // try and get the pcx
        if (cfgets (banners[num_banners], 254, in) == NULL) { break; }
        // try and get the url
        if (cfgets (urls[num_banners], 254, in) == NULL) { break; }
    ...
    // no banners
    if (num_banners <= 0) { return; }

    // if we're only selecting files which already exist (previously
    // downloaded)
    if (choose_existing) {
        // non exist
        for (idx = 0; idx < 10; idx++) { exists[idx] = 0; }
        ...
        if (select >= exist_count) { select = exist_count - 1; }
        if (select < 0) { select = 0; }
        for (idx = 0; idx < exist_count; idx++) {
            if (select == 0) { break; }
            if (exists[idx]) { select--; }
        }
        ...
        if (idx < exist_count) {
            // base filename
            strncpy (
                Multi_pxo_banner.ban_file, banners[idx], MAX_FILENAME_LEN);

            // get the full file url
            strncpy (
                Multi_pxo_banner.ban_file_url, file_url,
                MULTI_OPTIONS_STRING_LEN);
            strncat (
                Multi_pxo_banner.ban_file_url, banners[idx],
                MULTI_OPTIONS_STRING_LEN);

            // url of where to go to when clicked
            strncpy (
                Multi_pxo_banner.ban_url, urls[idx], MULTI_OPTIONS_STRING_LEN);
        }
    }
    // randomly pick a file for download
        ...
        if (idx >= num_banners) { idx = num_banners - 1; }
        if (idx < 0) { idx = 0; }
...
void multi_pxo_ban_draw () {
    ...
    if (Multi_pxo_banner.ban_bitmap >= 0) {
        ...
        if (Multi_pxo_ban_button.is_mouse_on ()) {
            ...
            gr_rect (
                Pxo_ban_coords[gr_screen.res][0] - 1,
                Pxo_ban_coords[gr_screen.res][1] - 1,
                Pxo_ban_coords[gr_screen.res][2] + 2,
                Pxo_ban_coords[gr_screen.res][3] + 2, GR_RESIZE_MENU);
        }

        // draw the bitmap itself
        gr_set_bitmap (Multi_pxo_banner.ban_bitmap);
        gr_bitmap (
            Pxo_ban_coords[gr_screen.res][0], Pxo_ban_coords[gr_screen.res][1],
            GR_RESIZE_MENU);
    }
}
