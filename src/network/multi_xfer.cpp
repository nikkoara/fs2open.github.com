...
#define MULTI_XFER_INVALID_HANDLE(handle)                        \
    ((handle < 0) || (handle > (MAX_XFER_ENTRIES - 1)) ||        \
     !(Multi_xfer_entry[handle].flags & MULTI_XFER_FLAG_USED) || \
     (strlen (Multi_xfer_entry[handle].filename) <= 0))
...
#define MULTI_XFER_CODE_HEADER \
    2 // file xfer header information follows, requires a HEADER_RESPONSE
#define MULTI_XFER_CODE_DATA 3 // data block follows, requires an ack
#define MULTI_XFER_CODE_FINAL \
    4 // indication from sender that xfer is complete, requires an ack
...
#define MULTI_XFER_FLAG_PENDING \
    (1 << 3) // this entry is ready to send a header and start the process
...
#define MULTI_XFER_FLAG_UNKNOWN \
    (1 << 6) // xfer final has been sent, and we are waiting for a response
...
#define MULTI_XFER_MAX_DATA_SIZE \
    490 // this will keep us within the MULTI_XFER_MAX_SIZE_LIMIT
...
typedef struct xfer_entry {
    ...
    char filename[MAX_FILENAME_LEN + 1]; // filename of the currently xferring
                                         // file
    char ex_filename[MAX_FILENAME_LEN + 10]; // filename with xfer prefix
    ...
    int force_dir; // force the file to go to this directory on receive (will
                   // override Multi_xfer_force_dir)
    ushort sig;    // identifying sig - sender specifies this
} xfer_entry;
xfer_entry
    Multi_xfer_entry[MAX_XFER_ENTRIES]; // the file xfer entries themselves
...
// unique file signature - this along with a socket # is enough to identify all
// xfers
...
// process a header
void multi_xfer_process_header (
    ubyte* data, PSNET_SOCKET_RELIABLE who, ushort sig, char* filename,
    int file_size, ushort file_checksum);
...
void multi_xfer_do () {
    ...
    for (idx = 0; idx < MAX_XFER_ENTRIES; idx++) {
        // if this one is actually in use and has not finished for one reason
        // or another
        if ((Multi_xfer_entry[idx].flags & MULTI_XFER_FLAG_USED) &&
            !(Multi_xfer_entry[idx].flags &
              (MULTI_XFER_FLAG_SUCCESS | MULTI_XFER_FLAG_FAIL |
               MULTI_XFER_FLAG_TIMEOUT))) {
...
int multi_xfer_send_file (
    PSNET_SOCKET_RELIABLE who, char* filename, int cfile_flags, int flags) {
    xfer_entry temp_entry;
    int handle;

    // if the system is locked, return -1
    if (Multi_xfer_locked) { return -1; }
    ...
    // set the flags
    temp_entry.flags |=
        (MULTI_XFER_FLAG_USED | MULTI_XFER_FLAG_SEND |
         MULTI_XFER_FLAG_PENDING);
...
int multi_xfer_get_status (int handle) {
    ...
    if ((handle < 0) || (handle > (MAX_XFER_ENTRIES - 1)) ||
        !(Multi_xfer_entry[handle].flags & MULTI_XFER_FLAG_USED)) {
    ...
    if ((Multi_xfer_entry[handle].flags & MULTI_XFER_FLAG_QUEUE) &&
        !(Multi_xfer_entry[handle].flags & MULTI_XFER_FLAG_QUEUE_CURRENT)) {
...
void multi_xfer_abort (int handle) {
    ...
    if (MULTI_XFER_INVALID_HANDLE (handle)) { return; }
...
void multi_xfer_release_handle (int handle) {
    ...
    if (xe->file != NULL) {
        ...
        if (!(xe->flags & MULTI_XFER_FLAG_SUCCESS) &&
            (xe->flags & MULTI_XFER_FLAG_RECV) && (xe->filename[0] != '\0')) {
            cf_delete (xe->ex_filename, xe->force_dir);
        }
    }
...
char* multi_xfer_get_filename (int handle) {
    ...
    if (MULTI_XFER_INVALID_HANDLE (handle)) { return NULL; }

    // otherwise return the string
    return Multi_xfer_entry[handle].filename;
}

// lock the xfer system (don't accept incoming files, don't allow outgoing
// files)
void multi_xfer_lock () { Multi_xfer_locked = 1; }

// unlock the xfer system
void multi_xfer_unlock () { Multi_xfer_locked = 0; }
...
// called from the recv_callback function)
...
int multi_xfer_get_flags (int handle) {
    // if this is an invalid handle, return NULL
    if (MULTI_XFER_INVALID_HANDLE (handle)) { return -1; }

    // return the flags
    return Multi_xfer_entry[handle].flags;
}

// if the passed filename is being xferred, return the xfer handle, otherwise
// return -1
...
float multi_xfer_pct_complete (int handle) {
    ...
    if (MULTI_XFER_INVALID_HANDLE (handle)) { return -1.0f; }

    // if the file size is 0, return invalid
    if (Multi_xfer_entry[handle].file_size == 0) { return -1.0f; }

    // return the pct completion
    return (float)Multi_xfer_entry[handle].file_ptr /
...
uint multi_xfer_get_sock (int handle) {
    // if this is an invalid handle, return NULL
    if (MULTI_XFER_INVALID_HANDLE (handle)) { return INVALID_SOCKET; }
...
void multi_xfer_eval_entry (xfer_entry* xe) {
    ...
    if (xe->flags & MULTI_XFER_FLAG_SUCCESS) { return; }
    ...
    if (xe->flags & MULTI_XFER_FLAG_QUEUE) {
        ...
        if (!(xe->flags & MULTI_XFER_FLAG_QUEUE_CURRENT)) {
            // see if there are any other queued up xfers to this target. if
            // not, make me current and start sending
            found = 0;
            for (idx = 0; idx < MAX_XFER_ENTRIES; idx++) {
                xe_c = &Multi_xfer_entry[idx];

                // if this is a valid entry and is a queued entry and is going
                // to the same target
                if ((xe_c->flags & MULTI_XFER_FLAG_USED) &&
                    (xe_c->file_socket == xe->file_socket) &&
                    (xe_c->flags & MULTI_XFER_FLAG_SEND) &&
                    (xe_c->flags & MULTI_XFER_FLAG_QUEUE) &&
                    (xe_c->flags & MULTI_XFER_FLAG_QUEUE_CURRENT)) {
                    found = 1;
                    break;
                }
            }
    ...
    // if the entry is marked as pending - send out the header to get the ball
    // rolling
...
xfer_entry* multi_xfer_find_entry (
    PSNET_SOCKET_RELIABLE who, ushort sig, int sender_side) {
    int idx;
    ...
    for (idx = 0; idx < MAX_XFER_ENTRIES; idx++) {
        ...
        if (sender_side &&
            !(Multi_xfer_entry[idx].flags & MULTI_XFER_FLAG_SEND)) {
            continue;
        }
        // if we're looking for recv entries
        if (!sender_side &&
            !(Multi_xfer_entry[idx].flags & MULTI_XFER_FLAG_RECV)) {
            continue;
        }

        // if we found a match
        if ((Multi_xfer_entry[idx].file_socket == who) &&
            (Multi_xfer_entry[idx].sig == sig)) {
...
void multi_xfer_fail_entry (xfer_entry* xe) {
    ...
    xe->flags &=
        ~(MULTI_XFER_FLAG_WAIT_ACK | MULTI_XFER_FLAG_WAIT_DATA |
          MULTI_XFER_FLAG_UNKNOWN);
...
// process an incoming file xfer data packet, return bytes processed,
// guaranteed to process the entire packet regardless of error conditions
int multi_xfer_process_packet (
    unsigned char* data, PSNET_SOCKET_RELIABLE who) {
    ubyte val;
    xfer_entry* xe;
    char filename[255];
    ushort data_size = 0;
    ...
    case MULTI_XFER_CODE_NAK: break;

    // RECV side
    case MULTI_XFER_CODE_FINAL: sender_side = 0; break;
    ...
    if (val != MULTI_XFER_CODE_HEADER) {
        // if the code is not a request or a header, we need to look up the
        // existing xfer_entry
        xe = NULL;
        ...
        if (xe == NULL) {
            ...
            if (np_index < 0) { ml_string (": player == unknown"); }
            else {
                ml_printf (
                    ": player == %s",
                    Net_players[np_index].m_player->callsign);
            }
            if (sender_side) { ml_string (": sending"); }
            else {
                ml_string (": receiving");
            }
#endif
    ...
    case MULTI_XFER_CODE_HEADER:
        ...
        multi_xfer_process_header (
            xfer_data, who, sig, filename, file_size, file_checksum);
        break;
    }
    return offset;
}
...
void multi_xfer_process_ack (xfer_entry* xe) {
    ...
    if (xe->flags & MULTI_XFER_FLAG_SEND) {
        // if we are waiting on a final ack, then the transfer has completed
        // successfully
        if (xe->flags & MULTI_XFER_FLAG_UNKNOWN) {
            ...
            if (xe->flags & MULTI_XFER_FLAG_AUTODESTROY) {
                multi_xfer_release_handle (
                    (int)std::distance (Multi_xfer_entry, xe));
            }
        }
        // otherwise if we're waiting for an ack, we should send the next chunk
        // of data or a "final" packet if we're done
...
void multi_xfer_process_final (xfer_entry* xe) {
    ...
    chksum = 0;
    if (!cf_chksum_short (xe->ex_filename, &chksum, -1, xe->force_dir) ||
        (chksum != xe->file_chksum)) {
        // mark as failed
        xe->flags |= MULTI_XFER_FLAG_FAIL;
...
void multi_xfer_process_header (
    ...
    // individual xfer entries take precedence over the global multi xfer force
    // entry
    ...
    // if the notify function invalidated this xfer handle, then cancel the
    // whole thing
...
void multi_xfer_send_next (xfer_entry* xe) {
    ...
    // determine how much data we are going to send with this packet and add it
    // in
    if ((size_t) (xe->file_size - xe->file_ptr) >=
        (MULTI_XFER_MAX_DATA_SIZE - flen)) {
...
ushort multi_xfer_get_sig () {
    ...
    // new one
    if (Multi_xfer_sig == 0xffff) { Multi_xfer_sig = 0; }
    else {
        Multi_xfer_sig++;
    }
