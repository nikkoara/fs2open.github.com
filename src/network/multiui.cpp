...
// the common text info box stuff. This is lifted almost directly from Alans
// briefing code (minus the spiffy colored, scrolling text crap :)
int Multi_common_text_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                          29, 396, 393, 76 },
                                                        { // GR_1024
                                                          47, 634, 630,
                                                          122 } };
...
// Cutoff values for determining the color of the ping text in server select
// window Pings below MULTI_PING_MIN_YELLOW are in green
...
#define MULTI_COMMON_MAX_TEXT \
    (MULTI_COMMON_TEXT_MAX_LINES * MULTI_COMMON_TEXT_MAX_LINE_LENGTH)

char Multi_common_all_text[MULTI_COMMON_MAX_TEXT];
char Multi_common_text[MULTI_COMMON_TEXT_MAX_LINES]
...
void multi_common_scroll_text_down () {
    Multi_common_top_text_line++;
    if ((Multi_common_num_text_lines - Multi_common_top_text_line) <
        Multi_common_text_max_display[gr_screen.res]) {
        Multi_common_top_text_line--;
        if (!mouse_down (MOUSE_LEFT_BUTTON)) {
            gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
        }
...
void multi_common_move_to_bottom () {
    ...
    if (Multi_common_num_text_lines <=
        Multi_common_text_max_display[gr_screen.res]) {
        return;
    }

    Multi_common_top_text_line = Multi_common_num_text_lines -
                                 Multi_common_text_max_display[gr_screen.res];
}
...
void multi_common_set_text (const char* str, int auto_scroll) {
    ...
    if (strlen (str) >= MULTI_COMMON_MAX_TEXT) { return; }
    else {
    ...
    if (auto_scroll) { multi_common_move_to_bottom (); }
}

void multi_common_add_text (const char* str, int auto_scroll) {
    // make sure it fits
    // store the entire string as well
    if ((strlen (str) + strlen (Multi_common_all_text)) >=
        MULTI_COMMON_MAX_TEXT) {
        return;
    }
    else {
...
void multi_common_render_text () {
    ...
    for (i = Multi_common_top_text_line; i < Multi_common_num_text_lines;
         i++) {
        if (line_count >= Multi_common_text_max_display[gr_screen.res]) {
            break;
        }
        gr_string (
            Multi_common_text_coords[gr_screen.res][0],
            Multi_common_text_coords[gr_screen.res][1] + (line_count * fh),
            Multi_common_text[i], GR_RESIZE_MENU);
        line_count++;
    }
    ...
    if ((Multi_common_num_text_lines - Multi_common_top_text_line) >
        ...
        gr_string (
            Multi_common_text_coords[gr_screen.res][0],
            (Multi_common_text_coords[gr_screen.res][1] +
             Multi_common_text_coords[gr_screen.res][3]) -
                5,
            XSTR ("more", 755), GR_RESIZE_MENU);
    }
}
...
// add a notification string, drawing appropriately depending on the
// state/screen we're in
...
void multi_common_notify_do () {
    if (Multi_common_notify_stamp != -1) {
        if (timestamp_elapsed (Multi_common_notify_stamp)) {
            ...
            // determine where it should be placed based upon which screen
            // we're on
            y = -1;
            ...
            if (y != -1) {
                gr_string (
                    (gr_screen.max_w_unscaled - w) / 2, y,
                    Multi_common_notify_text, GR_RESIZE_MENU);
            }
        }
    }
}
...
void multi_common_voice_display_status () {
    ...
    case MULTI_VOICE_STATUS_PLAYING: break;

    // the system is currently idle
    case MULTI_VOICE_STATUS_IDLE: break;
    }
}
...
void multi_common_load_palette () {
    if (Multi_common_interface_palette != -1) { return; }
...
void multi_common_set_palette () {
    ...
    if (Multi_common_interface_palette == -1) { multi_common_load_palette (); }
}
...
void multi_common_verify_cd () {
    ...
    Multi_has_cd = 0;
    if ((find_freespace_cd (FS_CDROM_VOLUME_1) >= 0) ||
        (find_freespace_cd (FS_CDROM_VOLUME_2) >= 0) ||
        (find_freespace_cd (FS_CDROM_VOLUME_3) >= 0)) {
        Multi_has_cd = 1;
    }
#else
    Multi_has_cd = 1;
...
// slider
const char* Mj_slider_name[GR_NUM_RESOLUTIONS] = { "slider", "2_slider" };
int Mj_slider_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                  2, 91, 15, 202 },
                                                { // GR_1024
                                                  8, 147, 30, 322 } };
...
UI_XSTR Multi_join_text[GR_NUM_RESOLUTIONS][MULTI_JOIN_NUM_TEXT] = {
    { // GR_640
      { "Refresh", 1299, 65, 364, UI_XSTR_COLOR_GREEN, -1,
        &Multi_join_buttons[0][MJ_REFRESH].button },
      { "Join as", 1300, 476, 376, UI_XSTR_COLOR_GREEN, -1,
        &Multi_join_buttons[0][MJ_JOIN_OBSERVER].button },
      { "Observer", 1301, 467, 385, UI_XSTR_COLOR_GREEN, -1,
        &Multi_join_buttons[0][MJ_JOIN_OBSERVER].button },
      { "Create", 1408, 535, 376, UI_XSTR_COLOR_GREEN, -1,
        &Multi_join_buttons[0][MJ_START_GAME].button },
      { "Game", 1302, 541, 385, UI_XSTR_COLOR_GREEN, -1,
        &Multi_join_buttons[0][MJ_START_GAME].button },
      { "Cancel", 387, 588, 376, UI_XSTR_COLOR_PINK, -1,
        &Multi_join_buttons[0][MJ_CANCEL].button },
      { "Help", 928, 496, 436, UI_XSTR_COLOR_GREEN, -1,
        &Multi_join_buttons[0][MJ_HELP].button },
      { "Options", 1036, 479, 460, UI_XSTR_COLOR_GREEN, -1,
        &Multi_join_buttons[0][MJ_OPTIONS].button },
      { "Join", 1303, 589, 416, UI_XSTR_COLOR_PINK, -1,
    ...
    { // GR_1024
      { "Refresh", 1299, 104, 582, UI_XSTR_COLOR_GREEN, -1,
        &Multi_join_buttons[1][MJ_REFRESH].button },
      { "Join as", 1300, 783, 602, UI_XSTR_COLOR_GREEN, -1,
        &Multi_join_buttons[1][MJ_JOIN_OBSERVER].button },
      { "Observer", 1301, 774, 611, UI_XSTR_COLOR_GREEN, -1,
        &Multi_join_buttons[1][MJ_JOIN_OBSERVER].button },
      { "Create", 1408, 868, 602, UI_XSTR_COLOR_GREEN, -1,
        &Multi_join_buttons[1][MJ_START_GAME].button },
      { "Game", 1302, 872, 611, UI_XSTR_COLOR_GREEN, -1,
        &Multi_join_buttons[1][MJ_START_GAME].button },
      { "Cancel", 387, 941, 602, UI_XSTR_COLOR_PINK, -1,
        &Multi_join_buttons[1][MJ_CANCEL].button },
      { "Help", 928, 806, 699, UI_XSTR_COLOR_GREEN, -1,
        &Multi_join_buttons[1][MJ_HELP].button },
      { "Options", 1036, 782, 736, UI_XSTR_COLOR_GREEN, -1,
        &Multi_join_buttons[1][MJ_OPTIONS].button },
      { "Join", 1303, 937, 668, UI_XSTR_COLOR_PINK, -1,
...
#define MULTI_JOIN_SENT_WAIT \
    10000 // wait this long since a join was sent to allow another
...
int Mj_status_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                  34, 53, 55, 287 },
                                                { // GR_1024
                                                  53, 77, 57, 459 } };

int Mj_game_icon_coords[GR_NUM_RESOLUTIONS][3] = { { // GR_640
                                                     98, 53, 28 },
                                                   { // GR_1024
                                                     124, 77, 28 } };

int Mj_speed_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                 122, 53, 56, 261 },
                                               { // GR_1024
                                                 152, 77, 56, 261 } };

int Mj_game_name_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                     186, 53, 280, 261 },
                                                   { // GR_1024
                                                     206, 77, 311, 261 } };

int Mj_players_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                   473, 53, 50, 261 },
                                                 { // GR_1024
                                                   748, 77, 67, 459 } };

int Mj_ping_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                551, 53, 47, 261 },
                                              { // GR_1024
                                                880, 77, 48, 459 } };
...
const char* Multi_join_speed_labels[MJ_NUM_SPEED_LABELS] = { "< 56k", "56k",
                                                             "isdn", "cable",
                                                             "t1/adsl+" };
color* Multi_join_speed_colors[MJ_NUM_SPEED_LABELS] = {
    &Color_bright_red, &Color_bright_red, &Color_bright_green,
    &Color_bright_green, &Color_bright_green
};

int Mj_cd_coords[GR_NUM_RESOLUTIONS] = { 20, 30 };

// XSTR:OFF
#define IP_CONFIG_FNAME \
    "tcp.cfg" // name of the file which contains known TCP addresses

// extents of the entire boundable game info region

int Mj_list_area_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                     23, 53, 565, 279 },
                                                   { // GR_1024
                                                     53, 76, 887, 461 } };
...
active_game*
    Multi_join_list_start_item; // a pointer to the corresponding active_game
int Multi_join_list_selected;   // which item we have selected
active_game*
    Multi_join_selected_item; // a pointer to the corresponding active_game

// use this macro to modify the list start
#define MJ_LIST_START_INC() \
    do { Multi_join_list_start++; } while (0);
#define MJ_LIST_START_DEC() \
    do { Multi_join_list_start--; } while (0);
#define MJ_LIST_START_SET(vl) \
    do { Multi_join_list_start = vl; } while (0);
...
int Multi_join_mt_tried_verify; // already tried verifying the pilot with the
                                // tracker
...
DCF (mj_make, "Makes a multijoin game? (Multiplayer)") {
    ...
    for (idx = 0; idx < idx_max; idx++) {
        ...
        if (newitem != NULL) {
            // newitem->heard_from_timer = timestamp((int)frand_range(500.0f,
            // 10000.0f));
        }
    }
}
...
void multi_join_notify_new_game () {
    // reset the # of items
    Multi_join_slider.set_numberItems (
        Active_game_count > Mj_max_game_items[gr_screen.res]
            ? Active_game_count - Mj_max_game_items[gr_screen.res]
            : 0);
...
int multi_join_autojoin_do () {
    // if we have an active game on the list, then return a positive value so
    // that we can join the game
...
void multi_join_game_init () {
    ...
    // destroy any chatbox contents which previously existed (from another
    // game)
    chatbox_clear ();

    // create the interface window
    Multi_join_window.create (
        0, 0, gr_screen.max_w_unscaled, gr_screen.max_h_unscaled, 0);
    Multi_join_window.set_mask_bmap (
        Multi_join_bitmap_mask_fname[gr_screen.res]);
    ...
    if ((Multi_options_g.protocol == NET_TCP) && !MULTI_IS_TRACKER_GAME) {
        // if this is a TCP (non tracker) game, we'll load up our default
    ...
    for (idx = 0; idx < MULTI_JOIN_NUM_BUTTONS; idx++) {
        // create the object
        Multi_join_buttons[gr_screen.res][idx].button.create (
            &Multi_join_window, "", Multi_join_buttons[gr_screen.res][idx].x,
            Multi_join_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);

        // set the sound to play when highlighted
        Multi_join_buttons[gr_screen.res][idx].button.set_highlight_action (
            common_play_highlight_sound);

        // set the ani for the button
        Multi_join_buttons[gr_screen.res][idx].button.set_bmaps (
            Multi_join_buttons[gr_screen.res][idx].filename);

        // set the hotspot
        Multi_join_buttons[gr_screen.res][idx].button.link_hotspot (
            Multi_join_buttons[gr_screen.res][idx].hotspot);
    }
    ...
    Multi_join_select_button.create (
        &Multi_join_window, "", Mj_list_area_coords[gr_screen.res][MJ_X_COORD],
    ...
    // slider
    Multi_join_slider.create (
        &Multi_join_window, Mj_slider_coords[gr_screen.res][MJ_X_COORD],
        Mj_slider_coords[gr_screen.res][MJ_Y_COORD],
        Mj_slider_coords[gr_screen.res][MJ_W_COORD],
        Mj_slider_coords[gr_screen.res][MJ_H_COORD], 0,
        Mj_slider_name[gr_screen.res], &multi_join_list_scroll_up,
        &multi_join_list_scroll_down, NULL);

    // make sure that we turn music/sounds back on (will be disabled after
    // playing a mission)
    main_hall_start_music ();

    // if starting a network game, then go to the create game screen
    if (Cmdline_start_netgame) { multi_join_create_game (); }
    else if (Cmdline_connect_addr != NULL) {
        char* p;
        short port_num;
        int ip_addr;

        // joining a game.  Send a join request to the given IP address, and
        // wait for the return.
...
void multi_join_game_do_frame () {
    // check the status of our reliable socket.  If not valid, popup error and
    // return to main menu I put this code here to avoid nasty gameseq issues
    // with states.  Also, we will have nice background for the popup
    if (!psnet_rel_check ()) {
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            XSTR (
                "Network Error.  Try exiting and restarting FreeSpace to "
                "clear the error.  Otherwise, please reboot your machine.",
                756));
        gameseq_post_event (GS_EVENT_MAIN_MENU);
        return;
    }

    // return here since we will be moving to the next stage anyway -- I don't
    // want to see the backgrounds of all the screens for < 1 second for every
    // screen we automatically move to.
    if (Cmdline_start_netgame) { return; }

    // when joining a network game, wait for the server query to come back, and
    // then join the game
    if (Cmdline_connect_addr != NULL) {
        int rval;

        if (!Multi_did_autojoin) {
            rval = popup_till_condition (
                multi_join_autojoin_do, XSTR ("&Cancel", 779),
                XSTR ("Joining netgame", 1500));
            if (rval == 0) {
    ...
    case KEY_ESC:
        if (help_overlay_active (Multi_join_overlay_id)) {
            ...
            if (MULTI_IS_TRACKER_GAME) { gameseq_post_event (GS_EVENT_PXO); }
            else {
                gameseq_post_event (GS_EVENT_MAIN_MENU);
            }
    ...
    case KEY_T: multi_pinfo_popup (Net_player); break;
    ...
    case KEY_S: multi_join_create_game (); break;
...
void multi_join_check_buttons () {
    int idx;
    for (idx = 0; idx < MULTI_JOIN_NUM_BUTTONS; idx++) {
        // we only really need to check for one button pressed at a time, so we
        // can break after finding one.
...
void multi_join_button_pressed (int n) {
    ...
    case MJ_ACCEPT:
        if (Active_game_count <= 0) {
            ...
            (Multi_join_sent_stamp != -1) &&
            !timestamp_elapsed (Multi_join_sent_stamp)) {
            multi_common_add_notify (
                XSTR ("Still waiting on previous join request!", 759));
            gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
        }
        else {
    ...
    case MJ_SCROLL_INFO_UP: multi_common_scroll_text_up (); break;

    // scroll the info text box down
    case MJ_SCROLL_INFO_DOWN: multi_common_scroll_text_down (); break;

    // go to the options screen
    case MJ_OPTIONS: gameseq_post_event (GS_EVENT_OPTIONS_MENU); break;

    // go to the start game screen
    case MJ_START_GAME: multi_join_create_game (); break;
...
void multi_join_display_games () {
    ...
    if (moveup != NULL) {
        do {
            ...
            con_type = (moveup->flags & AG_FLAG_CONNECTION_SPEED_MASK) >>
                       AG_FLAG_CONNECTION_BIT;
            if ((con_type > 4) || (con_type < 0)) { con_type = 0; }
            ...
            gr_string (
                Mj_speed_coords[gr_screen.res][MJ_X_COORD], y_start, str,
                GR_RESIZE_MENU);
            ...
            font::force_fit_string (
                str, 200, Mj_game_name_coords[gr_screen.res][MJ_W_COORD]);
            gr_string (
                Mj_game_name_coords[gr_screen.res][MJ_X_COORD], y_start, str,
                GR_RESIZE_MENU);
            ...
            if (moveup->ping.ping_avg > 0) {
                ...
                gr_string (
                    Mj_ping_coords[gr_screen.res][MJ_X_COORD], y_start, str,
                    GR_RESIZE_MENU);
            }
            ...
            gr_string (
                Mj_players_coords[gr_screen.res][MJ_X_COORD] +
                    (Mj_players_coords[gr_screen.res][MJ_W_COORD] - w) / 2,
                y_start, str, GR_RESIZE_MENU);

            count++;
            y_start += line_height;
            moveup = moveup->next;
        } while ((moveup != Active_game_head) &&
                 (count < Mj_max_game_items[gr_screen.res]));
    }
    // if there are no items on the list, display this info
    else {
        gr_set_color_fast (&Color_bright);
        gr_string (
            Mj_game_name_coords[gr_screen.res][MJ_X_COORD] - 30, y_start,
            XSTR ("<No game servers found>", 763), GR_RESIZE_MENU);
    }
}
...
void multi_join_blit_game_status (active_game* game, int y) {
    ...
    if (draw) {
        gr_bitmap (
            Mj_game_icon_coords[gr_screen.res][MJ_X_COORD], y - 1,
            GR_RESIZE_MENU);
    }
    ...
    gr_string (
        Mj_status_coords[gr_screen.res][MJ_X_COORD] +
            ((Mj_status_coords[gr_screen.res][MJ_W_COORD] - str_w) / 2),
        y, status_text, GR_RESIZE_MENU);
}
...
void multi_join_load_tcp_addrs () {
    ...
    while (!cfeof (file)) {
        ...
        if (!psnet_is_valid_ip_string (line)) {
            ...
            if (addr.port == 0) { addr.port = DEFAULT_GAME_PORT; }
...
void multi_join_do_netstuff () {
    ...
    if (Net_player->flags &
        (NETINFO_FLAG_ACCEPT_INGAME | NETINFO_FLAG_ACCEPT_CLIENT |
         NETINFO_FLAG_ACCEPT_HOST | NETINFO_FLAG_ACCEPT_OBSERVER)) {
        multi_common_add_notify (
            XSTR ("Accepted.  Waiting for player data.", 770));
    }

    // check to see if any join packets we have sent have timed out
    if ((Multi_join_sent_stamp != -1) &&
        (timestamp_elapsed (Multi_join_sent_stamp))) {
        Multi_join_sent_stamp = -1;
    ...
    if ((Multi_join_ping_stamp == -1) ||
        (timestamp_elapsed (Multi_join_ping_stamp))) {
        multi_join_ping_all ();
        Multi_join_ping_stamp = timestamp (MULTI_JOIN_PING_TIME);
    }
...
void multi_join_process_select () {
    ...
    // if we don't have anything selected and there are items on the list -
    ...
    if ((Multi_join_list_selected == -1) && (Active_game_count > 0)) {
        ...
        send_netgame_descript_packet (
            &Multi_join_selected_item->server_addr, 0);
        multi_common_set_text ("");
...
// scroll through the game list
void multi_join_list_scroll_down () {
    if ((Active_game_count - Multi_join_list_start) >
...
void multi_join_list_page_down () {
    ...
    while ((count < Mj_max_game_items[gr_screen.res]) &&
           ((Active_game_count - Multi_join_list_start) >
            Mj_max_game_items[gr_screen.res])) {
        Multi_join_list_start_item = Multi_join_list_start_item->next;
        MJ_LIST_START_INC ();
...
void multi_join_cull_timeouts () {
    ...
    if (moveup != NULL) {
        do {
            if ((moveup->heard_from_timer != -1) &&
                (timestamp_elapsed (moveup->heard_from_timer))) {
                Active_game_count--;
...
void multi_join_handle_item_cull (active_game* item, int item_index) {
    ...
    if (item_index <= Multi_join_list_selected) {
        ...
        if (Multi_join_selected_item == Active_game_head) {
            ...
            // now we know its a previous item, so our pointer stays the same
            // but our index goes down by one, since there will be 1 less item
            // on the list
            else {
                // decrement the selected index by 1
                Multi_join_list_selected--;
    ...
    if (Active_game_count > Mj_max_game_items[gr_screen.res]) {
        while ((Active_game_count - Multi_join_list_start) <
               Mj_max_game_items[gr_screen.res]) {
            Multi_join_list_start_item = Multi_join_list_start_item->prev;
            MJ_LIST_START_DEC ();
        }
...
void multi_join_send_join_request (int as_observer) {
    ...
    if (Multi_join_selected_item == NULL) { return; }

    // 5/26/98 -- for team v team games, don't allow ingame joining :-(
    if ((Multi_join_selected_item->flags & AG_FLAG_TEAMS) &&
        (Multi_join_selected_item->flags &
         (AG_FLAG_PAUSE | AG_FLAG_IN_MISSION))) {
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            XSTR (
                "Joining ingame is currently not allowed for team vs. team "
                "games",
                772));
        return;
    }
    ...
    if (as_observer) { Multi_join_request.flags |= JOIN_FLAG_AS_OBSERVER; }

    // if the player has hacked data
    if (game_hacked_data ()) { Multi_join_request.flags |= JOIN_FLAG_HAXOR; }
    ...
    memcpy (
        &Multi_join_request.player_options, &Player->m_local_options,
        sizeof (multi_local_options));

    // set the server address for the netgame
    memcpy (
        &Netgame.server_addr, &Multi_join_selected_item->server_addr,
        sizeof (net_addr));

    // send a join request to the guy
    send_join_packet (
        &Multi_join_selected_item->server_addr, &Multi_join_request);
...
void multi_join_create_game () {
    ...
    if (!multi_join_maybe_warn ()) { return; }
...
void multi_join_reset_join_stamp () {
    // unset the timestamp here so the user can immediately send another join
    // request
    Multi_join_sent_stamp = -1;
    multi_common_add_notify ("");
}
...
void multi_join_blit_top_stuff () {
    ...
    if (Multi_has_cd && (Multi_common_icons[MICON_CD] != -1)) {
        ...
        int cd_w;
        bm_get_info (
            Multi_common_icons[MICON_CD], &cd_w, NULL, NULL, NULL, NULL);

        gr_set_bitmap (Multi_common_icons[MICON_CD]);
        gr_bitmap (
            (gr_screen.max_w_unscaled / 2) - (cd_w / 2),
            Mj_cd_coords[gr_screen.res], GR_RESIZE_MENU);
    }
}
...
#define LOW_WARN_TEXT                                                        \
    XSTR (                                                                   \
        "Warning - You have low object updates selected. A server with low " \
        "object updates will not be able to handle more than 1 client "      \
        "without performance problems",                                      \
        775)
#define LOW_INFO_TEXT                                                        \
    XSTR (                                                                   \
        "Low update level caps all bandwidth at ~2000 bytes/second. It is "  \
        "appropriate for clients with 28.8 modems, but is not reccomended "  \
        "for servers. In addition, any clients connecting to this server "   \
        "should use low object updates as well. To change your settings go " \
        "to the options menu (f2), and select the Multi button",             \
        776)

#define MED_WARN_TEXT                                                        \
    XSTR (                                                                   \
        "Warning - You have medium object updates selected. A server with "  \
        "medium object updates will not be able to handle more than 1 or 2 " \
        "clients without performance problems",                              \
        777)
#define MED_INFO_TEXT                                                        \
    XSTR (                                                                   \
        "Medium update level caps all bandwidth at ~4000 bytes/second. It "  \
        "is appropriate for clients with 56.6 modems, but is not "           \
        "reccomended for servers. In addition, any clients connecting to "   \
        "this server should use low object updates as well. To change your " \
        "settings go to the options menu (f2), and select the Multi button", \
        778)
...
int multi_join_warn_update_low (int code) {
    ...
    case CW_CODE_OK:
        return popup (
            0, 3, XSTR ("&Cancel", 779), XSTR ("&Continue", 780),
            XSTR ("&More info", 781), LOW_WARN_TEXT);

    case CW_CODE_INFO:
        return popup (
            0, 3, XSTR ("&Cancel", 779), XSTR ("&Continue", 780),
            XSTR ("&More info", 781), LOW_INFO_TEXT);
    }
...
int multi_join_warn_update_medium (int code) {
    ...
    case CW_CODE_OK:
        return popup (
            0, 3, XSTR ("&Cancel", 779), XSTR ("&Continue", 780),
            XSTR ("&More info", 781), MED_WARN_TEXT);

    case CW_CODE_INFO:
        return popup (
            0, 3, XSTR ("&Cancel", 779), XSTR ("&Continue", 780),
            XSTR ("&More info", 781), MED_INFO_TEXT);
    }
...
int multi_join_warn_pxo () {
    return popup (
               PF_USE_AFFIRMATIVE_ICON | PF_USE_NEGATIVE_ICON | PF_TITLE_BIG |
                   PF_TITLE_RED,
               2, XSTR ("&Back", 995), XSTR ("&Continue", 780),
               XSTR (
                   "Warning\n\nThis pilot has played PXO games. If you "
                   "continue and play a non-PXO game, your stats will not be "
                   "updated",
                   1006)) <= 0
               ? 0
               : 1;
...
int Msg_passwd_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                   36, 236, 408, 20 },
                                                 { // GR_1024
                                                   58, 377, 652, 32 } };

// input game title field
int Msg_title_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                  29, 49, 415, 23 },
                                                { // GR_1024
                                                  46, 78, 664, 36 } };

// rank selected field
int Msg_rank_sel_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                     242, 254, 126, 12 },
                                                   { // GR_1024
                                                     242, 254, 126, 12 } };

// rank list field
int Msg_rank_list_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                      37, 297, 131, 16 },
                                                    { // GR_1024
                                                      60, 469, 652, 32 } };
...
UI_XSTR Multi_sg_text[GR_NUM_RESOLUTIONS][MULTI_SG_NUM_TEXT] = {
    {
        ...
        { "Above", 1327, 210, 290, UI_XSTR_COLOR_GREEN, -1,
          &Multi_sg_buttons[0][MSG_RANK_ABOVE].button },
        { "Below", 1328, 210, 315, UI_XSTR_COLOR_GREEN, -1,
        ...
        { "Accept", 1035, 571, 415, UI_XSTR_COLOR_PINK, -1,
    ...
    },
    {
        ...
        { "Above", 1327, 335, 465, UI_XSTR_COLOR_GREEN, -1,
          &Multi_sg_buttons[1][MSG_RANK_ABOVE].button },
        { "Below", 1328, 335, 505, UI_XSTR_COLOR_GREEN, -1,
          &Multi_sg_buttons[1][MSG_RANK_BELOW].button },
        { "Help", 928, 817, 706, UI_XSTR_COLOR_GREEN, -1,
          &Multi_sg_buttons[1][MSG_HELP].button },
        { "Options", 1036, 797, 743, UI_XSTR_COLOR_GREEN, -1,
          &Multi_sg_buttons[1][MSG_OPTIONS].button },
        { "Accept", 1035, 921, 664, UI_XSTR_COLOR_PINK, -1,
...
// function which takes a rank name and returns the index.  Useful for
// commandline options for above and below rank.  We return the index of the
// rank in the Ranks[] array.  If the rank isn't found, we return -1
...
void multi_start_game_init () {
    ...
    for (idx = 0; idx < MULTI_SG_NUM_BUTTONS; idx++) {
        // create the object
        Multi_sg_buttons[gr_screen.res][idx].button.create (
            &Multi_sg_window, "", Multi_sg_buttons[gr_screen.res][idx].x,
            Multi_sg_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);

        // set the sound to play when highlighted
        Multi_sg_buttons[gr_screen.res][idx].button.set_highlight_action (
            common_play_highlight_sound);

        // set the ani for the button
        Multi_sg_buttons[gr_screen.res][idx].button.set_bmaps (
            Multi_sg_buttons[gr_screen.res][idx].filename);

        // set the hotspot
        Multi_sg_buttons[gr_screen.res][idx].button.link_hotspot (
            Multi_sg_buttons[gr_screen.res][idx].hotspot);
    }
    ...
    Multi_sg_rank_button.create (
        &Multi_sg_window, "", Msg_rank_list_coords[gr_screen.res][MSG_X_COORD],
    ...
    Multi_sg_game_name.create (
        &Multi_sg_window, Msg_title_coords[gr_screen.res][MSG_X_COORD],
    ...
    Multi_sg_game_passwd.create (
        &Multi_sg_window, Msg_passwd_coords[gr_screen.res][MSG_X_COORD],
        Msg_passwd_coords[gr_screen.res][MSG_Y_COORD],
        Msg_passwd_coords[gr_screen.res][MSG_W_COORD], 16, "",
        UI_INPUTBOX_FLAG_ESC_CLR | UI_INPUTBOX_FLAG_PASSWD |
            UI_INPUTBOX_FLAG_INVIS,
        -1, &Color_normal);
    ...
    // if starting a netgame, set the name of the game and any other options
    ...
    if (Cmdline_start_netgame) {
        ...
        // deal with the different game types -- only one should even be
        // active, so we will just go down the line.  Last one wins.
        if (Cmdline_closed_game) { Multi_sg_netgame->mode = NG_MODE_CLOSED; }
        ...
        if ((Cmdline_rank_above != NULL) || (Cmdline_rank_below != NULL)) {
            ...
            if (Cmdline_rank_above != NULL) { rank_str = Cmdline_rank_above; }
            else {
                rank_str = Cmdline_rank_below;
            }

            // try and get the rank index from the name -- if found, then set
            // the rank base and the game type.  apparently we only support
            // either above or below, not both together, so I make a random
            // choice
...
void multi_start_game_do () {
    ...
    // screen we automatically move to.
    if (Cmdline_start_netgame) {
        Cmdline_start_netgame = 0; // DTP no quit fix; by using -startgame Quit
                                   // was sort of out of function
        return;
    }
    ...
    // check to see if any of the input boxes have changed, and update the
    // appropriate Netgame fields if necessary
...
void multi_start_game_close () {
    // if i'm the host on a standalone server, send him my start game options
    // (passwd, mode, etc)
...
void multi_sg_button_pressed (int n) {
    switch (n) {
    // go to the options screen
    case MSG_OPTIONS: gameseq_post_event (GS_EVENT_OPTIONS_MENU); break;
    ...
    case MSG_RANK_SET_GAME:
        ...
        if ((Multi_sg_netgame->mode != NG_MODE_RANK_BELOW) &&
            (Multi_sg_netgame->mode != NG_MODE_RANK_ABOVE)) {
            gamesnd_play_iface (InterfaceSounds::USER_SELECT);
    ...
    case MSG_RANK_SCROLL_UP: multi_sg_rank_scroll_up (); break;

    // scroll the rank list down
    case MSG_RANK_SCROLL_DOWN: multi_sg_rank_scroll_down (); break;
...
// NOTE : this is where all Netgame initialization should take place on the
// host
void multi_sg_init_gamenet () {
    ...
    // if we're on the standalone, we're not the server, so we don't care about
    // setting the netgame state
    if (Net_player->state != NETPLAYER_STATE_STD_HOST_SETUP) {
        ...
        ml_printf (
            NOX ("Starting netgame as Host on Standalone server : %s"),
            (server_addr == NULL) ? NOX ("Unknown") : server_addr);
    }

    Net_player->tracker_player_id = Multi_tracker_id;

    Multi_sg_netgame->security =
        (rand () % 32766) + 1; // get some random security number
    Multi_sg_netgame->mode = NG_MODE_OPEN;
    Multi_sg_netgame->rank_base = RANK_ENSIGN;
    if (Multi_sg_netgame->security < 16) { Multi_sg_netgame->security += 16; }
    ...
    // setup the default game name, taking care of string length and player
    // callsigns
    ...
    sprintf (
        buf, XSTR ("%s game", 782),
        out_name); // [[ %s will be a pilot's name ]]
    ...
    multi_voice_maybe_update_vars (
        Netgame.options.voice_qos, Netgame.options.voice_record_time);
    ...
    if (Multi_has_cd) { Net_player->flags |= NETINFO_FLAG_HAS_CD; }

    // if I have hacked data
    if (game_hacked_data ()) { Net_player->flags |= NETINFO_FLAG_HAXOR; }
...
void multi_sg_draw_radio_buttons () {
    ...
    case NG_MODE_PASSWORD:
        Multi_sg_buttons[gr_screen.res][MSG_PASSWD_GAME].button.draw_forced (
            2);
        break;
        /*
    ...
    case NG_MODE_RANK_ABOVE:
        Multi_sg_buttons[gr_screen.res][MSG_RANK_SET_GAME].button.draw_forced (
            2);
...
void multi_sg_rank_scroll_down () {
    ...
    if ((NUM_RANKS - Multi_sg_rank_start) >
        Multi_sg_rank_max_display[gr_screen.res]) {
        Multi_sg_rank_start++;
        gamesnd_play_iface (InterfaceSounds::SCROLL);
    }
    else {
...
void multi_sg_rank_display_stuff () {
    ...
    while ((count < NUM_RANKS) &&
           (count < Multi_sg_rank_max_display[gr_screen.res]) &&
           (idx < NUM_RANKS)) {
        ...
        gr_string (
            Msg_rank_list_coords[gr_screen.res][MSG_X_COORD], y, rank_name,
            GR_RESIZE_MENU);
    ...
    gr_set_color_fast (&Color_bright);
    multi_sg_rank_build_name (
        Ranks[Multi_sg_netgame->rank_base].name, rank_name);
    gr_string (
        Msg_rank_sel_coords[gr_screen.res][MSG_X_COORD],
        Msg_rank_sel_coords[gr_screen.res][MSG_Y_COORD], rank_name,
        GR_RESIZE_MENU);
}
...
int multi_sg_rank_select_valid (int rank) {
    // rank above mode
    if (Multi_sg_netgame->mode == NG_MODE_RANK_ABOVE) {
        if (Net_player->m_player->stats.rank >= rank) { return 1; }
    }
    // rank below mode
    else {
        if (Net_player->m_player->stats.rank <= rank) { return 1; }
...
ui_button_info
    Multi_create_buttons[GR_NUM_RESOLUTIONS][MULTI_CREATE_NUM_BUTTONS] = {
        {
            // GR_640
            ui_button_info ("MC_00", 32, 129, 36, 158, 0), // show all missions
            ui_button_info (
                "MC_01", 76, 129, 71, 158, 1), // show coop missions
            ui_button_info (
                "MC_02", 121, 129, 119, 158, 2), // show team missions
            ui_button_info (
                "MC_03", 164, 129, 166, 158, 3), // show dogfight missions
            ui_button_info (
                "MC_04", 399, 129, 229, 130, 4), // pxo mission refresh
            ui_button_info ("MC_05", 567, 123, 467, 132, 5), // pilot info
            ui_button_info (
                "MC_06", 1, 161, -1, -1, 6), // scroll mission info up
            ui_button_info (
                "MC_08", 1, 304, -1, -1, 8), // scroll mission info down
            ui_button_info ("MC_09", 613, 160, -1, -1, 9), // scroll players up
            ui_button_info (
                "MC_10", 613, 202, -1, -1, 10), // scroll players down
            ui_button_info ("MC_11", 22, 346, 27, 376, 11), // mission filter
            ui_button_info (
                "MC_12", 104, 346, 110, 376, 12), // campaign filter
        ...
        },
        {
            // GR_1024
            ui_button_info (
                "2_MC_00", 51, 207, 61, 253, 0), // show all missions
            ui_button_info (
                "2_MC_01", 122, 207, 124, 253, 1), // show coop missions
            ui_button_info (
                "2_MC_02", 193, 207, 194, 253, 2), // show team missions
            ui_button_info (
                "2_MC_03", 263, 207, 261, 253, 3), // show dogfight missions
            ui_button_info (
                "2_MC_04", 639, 207, 479, 218, 4), // pxo mission refresh
            ui_button_info ("2_MC_05", 907, 197, 748, 216, 5), // pilot info
            ui_button_info (
                "2_MC_06", 1, 258, -1, -1, 6), // scroll mission info up
            ui_button_info (
                "2_MC_08", 1, 487, -1, -1, 8), // scroll mission info down
            ui_button_info (
                "2_MC_09", 981, 256, -1, -1, 9), // scroll players up
            ui_button_info (
                "2_MC_10", 981, 323, -1, -1, 10), // scroll players down
            ui_button_info ("2_MC_11", 35, 554, 46, 601, 11), // mission filter
            ui_button_info (
                "2_MC_12", 166, 554, 174, 601, 12), // campaign filter
...
UI_XSTR Multi_create_text[GR_NUM_RESOLUTIONS][MULTI_CREATE_NUM_BUTTONS] = {
    { // GR_640
      { "All", 1256, 36, 158, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[0][MC_SHOW_ALL].button },
      { "Coop", 1257, 71, 158, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[0][MC_SHOW_COOP].button },
      { "Team", 1258, 119, 158, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[0][MC_SHOW_TEAM].button },
      { "Dogfight", 1259, 166, 158, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[0][MC_SHOW_DOGFIGHT].button },
      { "Refresh Missions", 1260, 229, 130, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[0][MC_PXO_REFRESH].button },
      { "Pilot Info", 1261, 467, 132, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[0][MC_PILOT_INFO].button },
      { "Missions", 1262, 27, 376, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[0][MC_MISSION_FILTER].button },
      { "Campaigns", 1263, 110, 376, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[0][MC_CAMPAIGN_FILTER].button },
      { "Cancel", 387, 328, 364, UI_XSTR_COLOR_PINK, -1,
        &Multi_create_buttons[0][MC_CANCEL].button },
      { "1", 1264, 482, 381, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[0][MC_TEAM0].button },
      { "2", 1265, 514, 381, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[0][MC_TEAM1].button },
      { "Kick", 1266, 539, 381, UI_XSTR_COLOR_PINK, -1,
        &Multi_create_buttons[0][MC_KICK].button },
      { "Close", 1508, 582, 381, UI_XSTR_COLOR_PINK, -1,
        &Multi_create_buttons[0][MC_CLOSE].button },
      { "Host Options", 1267, 436, 423, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[0][MC_HOST_OPTIONS].button },
      { "Commit", 1062, 572, 413, UI_XSTR_COLOR_PINK, -1,
        &Multi_create_buttons[0][MC_ACCEPT].button } },
    { // GR_1024
      { "All", 1256, 61, 253, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[1][MC_SHOW_ALL].button },
      { "Coop", 1257, 124, 253, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[1][MC_SHOW_COOP].button },
      { "Team", 1258, 194, 253, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[1][MC_SHOW_TEAM].button },
      { "Dogfight", 1259, 261, 253, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[1][MC_SHOW_DOGFIGHT].button },
      { "Refresh Missions", 1260, 501, 218, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[1][MC_PXO_REFRESH].button },
      { "Pilot Info", 1261, 814, 216, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[1][MC_PILOT_INFO].button },
      { "Missions", 1262, 46, 601, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[1][MC_MISSION_FILTER].button },
      { "Campaigns", 1263, 174, 601, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[1][MC_CAMPAIGN_FILTER].button },
      { "Cancel", 387, 559, 582, UI_XSTR_COLOR_PINK, -1,
        &Multi_create_buttons[1][MC_CANCEL].button },
      { "1", 1264, 772, 610, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[1][MC_TEAM0].button },
      { "2", 1265, 826, 610, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[1][MC_TEAM1].button },
      { "Kick", 1266, 872, 610, UI_XSTR_COLOR_PINK, -1,
        &Multi_create_buttons[1][MC_KICK].button },
      { "Close", 1508, 949, 610, UI_XSTR_COLOR_PINK, -1,
        &Multi_create_buttons[1][MC_CLOSE].button },
      { "Host Options", 1267, 755, 683, UI_XSTR_COLOR_GREEN, -1,
        &Multi_create_buttons[1][MC_HOST_OPTIONS].button },
      { "Commit", 1062, 932, 667, UI_XSTR_COLOR_PINK, -1,
...
int Multi_create_sw_checkbox_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                                 6, 75 },
                                                               { // GR_1024
                                                                 18, 135 } };
...
// game information text areas
int Mc_list_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                105, 173, 311, 152 },
                                              { // GR_1024
                                                62, 275, 600, 262 } };

int Mc_players_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                   463, 164, 144, 180 },
                                                 { // GR_1024
                                                   741, 262, 144, 180 } };

int Mc_info_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                47, 405, 363, 59 },
                                              { // GR_1024
                                                75, 648, 363, 59 } };
...
const char* Mc_slider_bitmap[GR_NUM_RESOLUTIONS] = { "slider", "2_slider" };

// player list control thingie defs
#define MULTI_CREATE_PLIST_MAX_DISPLAY 20
int Multi_create_plist_select_flag; // flag indicating if we have a play
                                    // selected
short Multi_create_plist_select_id; // the net address of the currently
                                    // selected player (for lookup)
...
// since we can selectively filter out mission/campaign types we always need to
// map a selected index (which is relative to the displayed list), to an
// absolute index (which is relative to the total file list - some of which may
// filtered out)
...
bool Multi_create_sort_mode = false; // default to mission name sorting, "true"
                                     // is mission filename sorting

// sorting function to sort mission lists.. Basic sorting on mission name
bool multi_create_sort_func (
    const multi_create_info& m1, const multi_create_info& m2) {
    if (Multi_create_filter != MISSION_TYPE_MULTI) {
        if ((m1.flags & Multi_create_filter) &&
            !(m2.flags & Multi_create_filter)) {
            return false;
        }
        else if (
            (m2.flags & Multi_create_filter) &&
            !(m1.flags & Multi_create_filter)) {
            return true;
        }
    }
...
void multi_create_list_sort (int mode) {
    ...
    if (Multi_create_list_count < 0) { return; }

    switch (mode) {
    case MULTI_CREATE_SHOW_MISSIONS: sort_missions = true; break;

    case MULTI_CREATE_SHOW_CAMPAIGNS: sort_campaigns = true; break;
    ...
    // save our current selected item so that we can restore that one after
    // sorting
    multi_create_select_to_filename (Multi_create_list_select, selected_name);

    if (sort_missions) {
        std::sort (
            Multi_create_mission_list.begin (),
            Multi_create_mission_list.end (), multi_create_sort_func);

        for (size_t idx = 0; idx < Multi_create_mission_list.size (); idx++) {
            if (!strcmp (
                    selected_name, Multi_create_mission_list[idx].filename)) {
                new_index = (int)idx;
                break;
            }
        }
    ...
    if (sort_campaigns) {
        std::sort (
            Multi_create_campaign_list.begin (),
            Multi_create_campaign_list.end (), multi_create_sort_func);

        for (size_t idx = 0; idx < Multi_create_campaign_list.size (); idx++) {
            if (!strcmp (
                    selected_name, Multi_create_campaign_list[idx].filename)) {
                new_index = (int)idx;
                break;
            }
        }
...
void multi_create_setup_list_data (int mode) {
    ...
    if (Multi_create_list_mode == MULTI_CREATE_SHOW_MISSIONS) {
        ...
        default:
            ...
            for (idx = 0; idx < (int)Multi_create_mission_list.size ();
                 idx++) {
                if (Multi_create_mission_list[idx].flags &
                    Multi_create_filter) {
        ...
        default:
            ...
            // find all missions which match
            for (idx = 0; idx < (int)Multi_create_campaign_list.size ();
                 idx++) {
                if (Multi_create_campaign_list[idx].flags &
                    Multi_create_filter) {
                    Multi_create_list_count++;
                }
            }
    ...
    if (should_sort) { multi_create_list_sort (mode); }

    // reset the slider
    Multi_create_slider.set_numberItems (
        Multi_create_list_count > Multi_create_list_max_display[gr_screen.res]
            ? Multi_create_list_count -
                  Multi_create_list_max_display[gr_screen.res]
            : 0);
}
...
void multi_create_game_init () {
    ...
    // now make sure to initialze various netgame stuff based upon whether
    // we're on a standalone or not
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
    ...
    // create the interface window
    Multi_create_window.create (
        0, 0, gr_screen.max_w_unscaled, gr_screen.max_h_unscaled, 0);
    Multi_create_window.set_mask_bmap (
        Multi_create_bitmap_mask_fname[gr_screen.res]);
    ...
    // close any previous existing instances of the chatbox and create a new
    // one
    chatbox_close ();
    chatbox_create ();
    ...
    for (idx = 0; idx < MULTI_CREATE_NUM_BUTTONS; idx++) {
        ...
        // create the object
        b->button.create (
            &Multi_create_window, "", b->x, b->y, 1, 1,
            ((idx == MC_SCROLL_LIST_UP) || (idx == MC_SCROLL_LIST_DOWN)), 1);
        ...
        if (idx == MC_PXO_REFRESH) {
            // if not a PXO game, or if I'm not a server disable and hide the
            // button
    ...
    Multi_create_sw_checkbox.create (
        &Multi_create_window, "",
        Multi_create_sw_checkbox_coords[gr_screen.res][0],
        Multi_create_sw_checkbox_coords[gr_screen.res][1], 0);
    Multi_create_sw_checkbox.set_bmaps (
        Multi_create_sw_checkbox_fname[gr_screen.res], 6, 0);
    if (!MULTI_IS_TRACKER_GAME) {
    ...
    Multi_create_slider.create (
        &Multi_create_window, Mc_slider_coords[gr_screen.res][MC_X_COORD],
        Mc_slider_coords[gr_screen.res][MC_Y_COORD],
        Mc_slider_coords[gr_screen.res][MC_W_COORD],
    ...
    Multi_create_player_select_button.create (
        &Multi_create_window, "", Mc_players_coords[gr_screen.res][MC_X_COORD],
    ...
    Multi_create_list_select_button.create (
        &Multi_create_window, "", Mc_list_coords[gr_screen.res][MC_X_COORD],
    ...
    Multi_create_buttons[gr_screen.res][MC_ACCEPT].button.set_hotkey (
        KEY_CTRLED + KEY_ENTER);
...
void multi_create_game_do () {
    ...
    // SINCE WE ALREADY HAVE A MISSION SELECTED IF THIS MISSION IS A VALID
    // MULTIPLAYER MISSION IF NOT A VALID MULTIPLAYER MISSION CONTINUE LOADING,
    // MAYBE CALL POPUP.
    if ((Cmdline_almission) &&
        (Net_player->flags & NETINFO_FLAG_AM_MASTER)) { //
        multi_create_list_do (); // uhm here because off, hehe, my mind is
                                 // failing right now
        ...
        filename = cf_add_ext (
            Cmdline_almission,
            FS_MISSION_FILE_EXT); // DTP ADD EXTENSION needed next line
        flags = mission_parse_is_multi (
            filename, mission_name); // DTP flags will set if mission is multi
        ...
        if (flags) { // only continue if mission is multiplayer mission
            ...
            strncpy (
                almissionname, Cmdline_almission,
                MAX_FILENAME_LEN); // DTP; copying name from cmd_almission line

            Netgame.options.respawn =
                99; // override anything //for debugging, i often forget this.
            ng->respawn = Netgame.options.respawn;

            Netgame.campaign_mode =
                MP_SINGLE_MISSION; // multiplayer single mission. meaning
                                   // Single mission, not single player

            strcpy_s (
                Game_current_mission_filename,
                almissionname); // copying almissionname to
                                // Game_current_mission_filename
            strcpy_s (
                Netgame.mission_name,
                almissionname); // copying almission name to
                                // netgame.mission_name
            ...
            Cmdline_almission =
                NULL; // we don't want to autoload anymore do we, we will be
                      // able to quit. halleluja. Startgame has already been
                      // disabled, so no need to turn of
                      // "Cmdline_start_netgame"
            return;   // we don't need to check or set regarding ships/weapons
                      // anything as we are already progressing into mission so
                      // return
        }
        else {
            popup (
                ...
                XSTR (
                    " Not a multi player-mission",
                    9999)); // DTP startgame popup pilot error
            gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
    ...
    if (!Multi_create_files_loaded) {
        ...
        if (MULTIPLAYER_CLIENT) {
            ...
            loading_bitmap =
                bm_load (Multi_create_loading_fname[gr_screen.res]);
            ...
            chatbox_render ();
            if (loading_bitmap > -1) { gr_set_bitmap (loading_bitmap); }
            gr_bitmap (
                Please_wait_coords[gr_screen.res][MC_X_COORD],
                Please_wait_coords[gr_screen.res][MC_Y_COORD], GR_RESIZE_MENU);
            ...
            gr_string (
                (gr_screen.max_w_unscaled - str_w) / 2,
                (gr_screen.max_h_unscaled - str_h) / 2, loading_str,
                GR_RESIZE_MENU);
            ...
            // the above function doesn't sort initially, so we need this to
            // take care of it
            multi_create_list_sort (MULTI_CREATE_SHOW_MISSIONS);
            // the sort function probably changed our selection, but here we
            // always need it to zero
        ...
        if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
            ...
            if (MULTI_IS_TRACKER_GAME) { fs2netd_gameserver_start (); }
        }

        // if we're on the standalone we have to tell him that we're now in the
        // host setup screen
        Net_player->state = NETPLAYER_STATE_HOST_SETUP;
    ...
    gr_string (
        Mc_create_game_text[gr_screen.res][MC_X_COORD],
        Mc_create_game_text[gr_screen.res][MC_Y_COORD],
        XSTR ("Create Game", 1268), GR_RESIZE_MENU);

    // draw players text
    gr_string (
        Mc_players_text[gr_screen.res][MC_X_COORD],
        Mc_players_text[gr_screen.res][MC_Y_COORD], XSTR ("Players", 1269),
        GR_RESIZE_MENU);

    // draw players text
    gr_string (
        Mc_team_text[gr_screen.res][MC_X_COORD],
        Mc_team_text[gr_screen.res][MC_Y_COORD], XSTR ("Team", 1258),
        GR_RESIZE_MENU);

    // process and display the player list
    // NOTE : this must be done before the buttons are checked to insure that a
    // player hasn't dropped
    multi_create_plist_process ();
    if (Netgame.type_flags & NG_TYPE_TEAM) { multi_create_plist_blit_team (); }
    else {
    ...
    if (Multi_create_list_mode == MULTI_CREATE_SHOW_MISSIONS) {
        Multi_create_buttons[gr_screen.res][MC_MISSION_FILTER]
            .button.draw_forced (2);
    }
    else {
        Multi_create_buttons[gr_screen.res][MC_CAMPAIGN_FILTER]
            .button.draw_forced (2);
    }
    ...
    if (MULTI_IS_TRACKER_GAME) {
        ...
        gr_string (
            Multi_create_sw_checkbox_text[gr_screen.res][0],
            Multi_create_sw_checkbox_text[gr_screen.res][1], "SquadWar",
            GR_RESIZE_MENU);
    }
    ...
    if (Multi_create_should_show_popup) {
        // get the player index and address of the player item the mouse is
        // currently over
...
void multi_create_check_buttons () {
    ...
    if (Multi_create_sw_checkbox.changed ()) { multi_create_sw_clicked (); }
}
...
void multi_create_button_pressed (int n) {
    ...
    case MC_ACCEPT:
        ...
        if (!multi_create_ok_to_commit ()) { break; }
    ...
    // scroll the info text box up
    case MC_SCROLL_INFO_UP: multi_common_scroll_text_up (); break;

    // scroll the info text box down
    case MC_SCROLL_INFO_DOWN: multi_common_scroll_text_down (); break;

    // scroll the player list up
    case MC_SCROLL_PLAYERS_UP: multi_create_plist_scroll_up (); break;

    // scroll the player list down
    case MC_SCROLL_PLAYERS_DOWN: multi_create_plist_scroll_down (); break;
    ...
    case MC_SHOW_ALL:
        if (Multi_create_filter != MISSION_TYPE_MULTI) {
            gamesnd_play_iface (InterfaceSounds::USER_SELECT);
            Multi_create_filter = MISSION_TYPE_MULTI;
            multi_create_setup_list_data (
                Multi_create_list_mode); // update the file list
        }
        else {
            gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
        }
    ...
    case MC_KICK:
        ...
        if (Multi_create_plist_select_flag) {
            ...
            if (idx != -1) { multi_kick_player (idx, 0); }
        }
        break;
    ...
    // popup the pilot info dialog for the currently selected pilot (will occur
    // at the end of the frame)
    case MC_PILOT_INFO: Multi_create_should_show_popup = 1; break;
    ...
    case MC_PXO_REFRESH:
        if (!MULTI_IS_TRACKER_GAME) { break; }
...
void multi_create_plist_process () {
    ...
    // first determine if there are 0 players in the game. This should never
    // happen since the host is _always_ in the game
    test_count = 0;
    ...
    // if we had a selected item but that player has left, select myself
    // instead
    if (Multi_create_plist_select_flag) {
        player_index = find_player_id (Multi_create_plist_select_id);
        if (player_index == -1) {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        ...
        if (MULTI_CONNECTED (Net_players[idx]) &&
            ...
            if (Net_players[idx].flags & NETINFO_FLAG_GAME_HOST) {
                if (Multi_create_plist_select_id ==
            ...
            if ((Net_players[idx].flags & NETINFO_FLAG_HAS_CD) &&
                (Multi_common_icons[MICON_CD] != -1)) {
                gr_set_bitmap (Multi_common_icons[MICON_CD]);
                gr_bitmap (
                    Mc_players_coords[gr_screen.res][MC_X_COORD] +
                        total_offset,
                    y_start - 1, GR_RESIZE_MENU);
            ...
            font::force_fit_string (
                str, CALLSIGN_LEN,
                Mc_players_coords[gr_screen.res][MC_W_COORD] - total_offset);
            gr_string (
                Mc_players_coords[gr_screen.res][MC_X_COORD] + total_offset,
                y_start, str, GR_RESIZE_MENU);
        ...
        if (MULTI_CONNECTED (Net_players[idx]) &&
            ...
            if (Net_players[idx].flags & NETINFO_FLAG_GAME_HOST) {
                if (Multi_create_plist_select_id ==
                    ...
                    Multi_create_buttons[gr_screen.res][MC_TEAM0]
                        .button.draw_forced (2);
                }
                else {
                    gr_set_color_fast (&Color_bright);
            ...
            if (Net_players[idx].flags & NETINFO_FLAG_TEAM_CAPTAIN) {
                if (Multi_common_icons[MICON_TEAM0_SELECT] != -1) {
                    ...
                    gr_bitmap (
                        Mc_players_coords[gr_screen.res][MC_X_COORD] +
                            total_offset,
                        y_start - 2, GR_RESIZE_MENU);

                    total_offset +=
                        Multi_common_icon_dims[MICON_TEAM0_SELECT][0] + 1;
                }
            }
            else {
            ...
            // display him in the correct half of the list depending on his
            // team
            gr_string (
                Mc_players_coords[gr_screen.res][MC_X_COORD] + total_offset,
                y_start, str, GR_RESIZE_MENU);
        ...
        if (MULTI_CONNECTED (Net_players[idx]) &&
            ...
            if (Net_players[idx].flags & NETINFO_FLAG_GAME_HOST) {
                if (Multi_create_plist_select_id ==
                    ...
                    // be sure to blit the correct team button
                    Multi_create_buttons[gr_screen.res][MC_TEAM1]
                        .button.draw_forced (2);
                }
                else {
                    gr_set_color_fast (&Color_bright);
            ...
            if (Net_players[idx].flags & NETINFO_FLAG_TEAM_CAPTAIN) {
                if (Multi_common_icons[MICON_TEAM1_SELECT] != -1) {
                    ...
                    total_offset +=
                        Multi_common_icon_dims[MICON_TEAM1_SELECT][0] + 1;
                }
            }
            else {
...
void multi_create_list_scroll_down () {
    if ((Multi_create_list_count - Multi_create_list_start) >
        Multi_create_list_max_display[gr_screen.res]) {
        Multi_create_list_start++;
...
void multi_create_list_load_missions () {
    ...
    file_count =
        cf_get_file_list (1024, file_list, CF_TYPE_MISSIONS, wild_card);
    ...
    for (idx = 0; idx < file_count; idx++) {
        ...
        // if the mission is a multiplayer mission, and we can list it, then
        // add it to the mission list
    ...
    // maybe create a standalone dialog
    if (Game_mode & GM_STANDALONE_SERVER) { std_destroy_gen_dialog (); }
}
...
void multi_create_list_load_campaigns () {
    ...
    for (idx = 0; idx < file_count; idx++) {
        ...
        // if the campaign is a multiplayer campaign, and we can list it, and
        // we can get its info, then add the data to the campaign list items
        if (flags != CAMPAIGN_TYPE_SINGLE && !campaign_is_ignored (filename) &&
            mission_campaign_get_info (
                filename, title, &campaign_type, &max_players)) {
            multi_create_info mcip;
            ...
            // 0 respawns for campaign files (should be contained within the
            // mission files themselves)
            mcip.respawn = 0;
...
void multi_create_list_do () {
    ...
    if (Multi_create_list_select_button.pressed ()) {
        ...
        if ((item < Multi_create_list_max_display[gr_screen.res]) &&
            (item >= 0)) {
    ...
    if (Multi_create_list_start == -1) { return; }
    ...
    for (idx = start_index; idx < stop_index; idx++) {
        ...
        if (count == Multi_create_list_max_display[gr_screen.res]) { break; }

        // see if we should filter out this mission
        if (!(mcip->flags & Multi_create_filter)) { continue; }

        // highlight the selected item
        multi_create_select_to_filename (
            Multi_create_list_select, selected_name);
        ...
        font::force_fit_string (
            selected_name, 255, Mc_column1_w[gr_screen.res]);
        gr_string (
            Mc_mission_name_x[gr_screen.res], y_start, selected_name,
            GR_RESIZE_MENU);

        // draw the max players if in mission mode
        sprintf (selected_name, "%d", (int)mcip->max_players);
        gr_string (
            Mc_mission_count_x[gr_screen.res], y_start, selected_name,
            GR_RESIZE_MENU);

        // force fit the mission filename string
        strcpy_s (selected_name, mcip->filename);
        font::force_fit_string (
            selected_name, 255, Mc_column3_w[gr_screen.res]);
        gr_string (
            Mc_mission_fname_x[gr_screen.res], y_start, selected_name,
            GR_RESIZE_MENU);
...
// takes care of stuff like changing indices around and setting up the netgame
// structure
void multi_create_list_select_item (int n) {
    ...
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) { ng = &Netgame; }
    // on the standalone
    else {
    ...
    if (n != Multi_create_list_select) {
        ...
        if (abs_index != -1) {
            ...
            if (mcip->flags & MISSION_TYPE_MULTI_TEAMS) {
                ...
                if (old_type & NG_TYPE_SW) { ng->type_flags = NG_TYPE_SW; }
                else {
        ...
        // if we switched from something else to team vs. team mode, do some
        // special processing
        ...
        case MULTI_CREATE_SHOW_MISSIONS:
            ...
            if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
                ship_level_init (); // mwa -- 10/15/97.  Call this function to
                                    // reset number of ships in mission
                ng->max_players =
        ...
        case MULTI_CREATE_SHOW_CAMPAIGNS:
            ...
            if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
                ...
                if (!mission_campaign_get_info (
                        ng->campaign_name, title, &campaign_type, &max_players,
                        &campaign_desc)) {
            ...
            // netgame respawns are always 0 for campaigns (until the first
            // mission is loaded)
            ng->respawn = 0;
            break;
        }
        ...
        if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
            ...
            if (MULTI_IS_TRACKER_GAME) { fs2netd_gameserver_update (true); }
        }
        else {
            multi_options_update_mission (
                ng,
                Multi_create_list_mode == MULTI_CREATE_SHOW_CAMPAIGNS ? 1 : 0);
        }
    }
}
...
void multi_create_list_blit_icons (int list_index, int y_start) {
    ...
    if ((list_index < 0) || (list_index > max_index)) { return; }
    ...
    if (mcip->flags & MISSION_TYPE_MULTI_COOP) {
        if (Multi_common_icons[MICON_COOP] >= 0) {
            gr_set_bitmap (Multi_common_icons[MICON_COOP]);
            gr_bitmap (
                Mc_icon_type_coords[gr_screen.res][MC_X_COORD],
    ...
    if (MULTI_IS_TRACKER_GAME && (mcip->valid_status == MVALID_STATUS_VALID)) {
        if (Multi_common_icons[MICON_VALID] >= 0) {
            ...
            gr_bitmap (
                Mc_icon_valid_coords[gr_screen.res][MC_X_COORD],
    ...
    if ((fb != NULL) && (fb->flags & FSB_FROM_VOLITION)) {
        if (Multi_common_icons[MICON_VOLITION] >= 0) {
            ...
            gr_bitmap (
                Mc_icon_volition_coords[gr_screen.res][MC_X_COORD],
...
void multi_create_accept_hit () {
    ...
    case MULTI_CREATE_SHOW_MISSIONS:
        if (Multi_create_list_select != -1) {
            ...
            // NETLOG
            ml_printf (
                NOX ("Starting single mission %s, with %d players"),
                Game_current_mission_filename, multi_num_players ());
        }
        else {
    ...
    case MULTI_CREATE_SHOW_CAMPAIGNS:
        ...
        if (Multi_create_list_select != -1) {
            ...
            // NETLOG
            ml_printf (
                NOX ("Starting campaign %s, with %d players"), selected_name,
                multi_num_players ());
        }
        else {
    ...
    // if this is a team vs team situation, lock the players send a final team
    // update
    if ((Netgame.type_flags & NG_TYPE_TEAM) &&
        (Net_player->flags & NETINFO_FLAG_AM_MASTER)) {
    ...
    if ((Netgame.type_flags & NG_TYPE_COOP) &&
        ...
        if (Netgame.options.mission_time_limit != i2f (-1)) {
            popup_choice = popup (
                0, 3, POPUP_CANCEL, POPUP_YES, XSTR (" &No", 506),
                XSTR (
                    "A time limit is being used in a co-op game.\r\n"
                    "  Select \'Cancel\' to go back to the mission select "
                    "screen.\r\n"
                    "  Select \'Yes\' to continue with this time limit.\r\n"
                    "  Select \'No\' to continue without this time limit.",
                    1620));

            if (popup_choice == 0) { return; }
        ...
        if ((Netgame.options.kill_limit > 0) &&
            ...
            popup_choice = popup (
                ...
                XSTR (
                    ...
                    "  Select \'No\' to continue without this kill limit.",
                    1621));
    ...
    // if not on the standalone, move to the mission sync state which will take
    // care of everything
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
    ...
    // otherwise tell the standalone to do so
        ...
        // when the standalone receives this, he'll do the mission syncing
        // himself
...
void multi_create_draw_filter_buttons () {
    ...
    if (Multi_create_filter == MISSION_TYPE_MULTI) {
        Multi_create_buttons[gr_screen.res][MC_SHOW_ALL].button.draw_forced (
            2);
    }
    else if (Multi_create_filter == MISSION_TYPE_MULTI_COOP) {
        Multi_create_buttons[gr_screen.res][MC_SHOW_ALL + 1]
            .button.draw_forced (2);
    }
    else if (Multi_create_filter == MISSION_TYPE_MULTI_TEAMS) {
        Multi_create_buttons[gr_screen.res][MC_SHOW_ALL + 2]
            .button.draw_forced (2);
    }
    else if (Multi_create_filter == MISSION_TYPE_MULTI_DOGFIGHT) {
        Multi_create_buttons[gr_screen.res][MC_SHOW_ALL + 3]
            .button.draw_forced (2);
    }
    else {
        Int3 ();
...
// fill in net address of player the mouse is over, return player index (or -1
// if none)
short multi_create_get_mouse_id () {
    ...
    // select things a little differently if we're in team vs. team or non-team
    // vs. team mode
    ...
    if (Netgame.type_flags & NG_TYPE_TEAM) {
        ...
        if (player_index != -1) { return Net_players[player_index].player_id; }
    }
    else {
        // select the nth active player if possible, disregarding the
        // standalone server
        for (idx = 0; idx < MAX_PLAYERS; idx++) {
            if (MULTI_CONNECTED (Net_players[idx]) &&
                !MULTI_STANDALONE (Net_players[idx])) {
                ...
                // if this is the _nth_ guy
                if (nth < 0) { return Net_players[idx].player_id; }
            }
        }
    }
...
int multi_create_select_to_index (int select_index) {
    ...
    if (Multi_create_list_mode == MULTI_CREATE_SHOW_MISSIONS) {
        for (idx = 0; idx < Multi_create_mission_list.size (); idx++) {
            ...
            // if we found the item
            if (select_index < lookup_index) { return idx; }
        }
    }
    // look through the campaign list
    else if (Multi_create_list_mode == MULTI_CREATE_SHOW_CAMPAIGNS) {
...
int multi_create_ok_to_commit () {
    ...
    if (!MULTI_IS_TRACKER_GAME) { notify_of_hacked_ships_tbl = 0; }
    if (notify_of_hacked_ships_tbl) {
        if (popup (
                PF_USE_AFFIRMATIVE_ICON | PF_USE_NEGATIVE_ICON, 2,
                XSTR ("&Back", 995), XSTR ("&Continue", 780),
                XSTR (
                    "You or the server you are playing on has a hacked "
                    "ships.tbl. Your stats will not be updated on PXO",
                    1051)) <= 0) {
            return 0;
        }
    }
    ...
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        if (!Game_weapons_tbl_valid) { notify_of_hacked_weapons_tbl = 1; }
    }
    else {
    ...
    if (!MULTI_IS_TRACKER_GAME) { notify_of_hacked_weapons_tbl = 0; }
    if (notify_of_hacked_weapons_tbl) {
        if (popup (
                ...
                XSTR (
                    "You or the server you are playing on has a hacked "
                    "weapons.tbl. Your stats will not be updated on PXO",
                    1052)) <= 0) {
            return 0;
        }
    }
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        ...
        if (MULTI_CONNECTED (Net_players[idx]) &&
            (Net_players[idx].flags & NETINFO_FLAG_HAXOR)) {
            // we found a hack
            found_hack = 1;
            ...
            if (Net_players[idx].m_player != NULL) {
                sprintf (
                    err_string, "%s %s", Net_players[idx].m_player->callsign,
                    XSTR ("has hacked tables/data", 1271));
            }
            else {
                sprintf (
                    err_string, "somebody %s",
                    XSTR ("has hacked tables/data", 1271));
            }
            send_game_chat_packet (
                Net_player, err_string, MULTI_MSG_ALL, NULL, NULL, 1);
        }
    }
    ...
    if (found_hack) {
        ...
        if (MULTI_IS_TRACKER_GAME) {
            ...
            if (Netgame.type_flags & NG_TYPE_SW) {
                ...
                popup (
                    PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                    XSTR (
                        "One or more players has hacked data files. You "
                        "cannot play a SquadWar match unless all clients have "
                        "legal data",
                        1272));
            ...
            // otherwise, warn the players that stats will not saved
                ...
                if (popup (
                        ...
                        XSTR ("&Back", 995), XSTR ("&Continue", 780),
                        XSTR (
                            "One or more players has hacked data files. If "
                            "you continue, stats will not be stored at the "
                            "end of the mission",
                            1273)) <= 0) {
                    return 0;
                }
        ...
        // non-pxo, just give a notice
            ...
            if (popup (
                    ...
                    XSTR ("&Back", 995), XSTR ("&Continue", 780),
                    XSTR (
                        "One or more players has hacked data files", 1274)) <=
                0) {
                return 0;
            }
        }
    ...
    // check to see that we have a valid # of players for the the # of ships in
    // the game
    ...
    if (!multi_create_verify_cds ()) {
        ...
        popup (
            PF_BODY_BIG | PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            XSTR ("You need 1 CD for every 4 players!", 794));
    ...
    if (MULTI_IS_TRACKER_GAME) {
        ...
        if ((Multi_create_list_mode == MULTI_CREATE_SHOW_MISSIONS) &&
            (Multi_create_mission_list[abs_index].valid_status !=
             MVALID_STATUS_VALID)) {
            if (popup (
                    PF_USE_AFFIRMATIVE_ICON | PF_USE_NEGATIVE_ICON, 2,
                    XSTR ("&Back", 995), XSTR ("&Continue", 780),
                    XSTR (
                        "You have selected a mission which is either invalid "
                        "or unknown to PXO. Your stats will not be saved if "
                        "you continue",
                        996)) <= 0) {
                return 0;
            }
            ...
            if (multi_num_players () == 1) {
                if (popup (
                        PF_USE_AFFIRMATIVE_ICON | PF_USE_NEGATIVE_ICON |
                            PF_TITLE_RED | PF_TITLE_BIG,
                        2, XSTR ("&Back", 995), XSTR ("&Continue", 780),
                        XSTR (
                            "Warning\n\nIf you start a PXO mission by "
                            "yourself, your stats will not be updated",
                            997)) <= 0) {
                    return 0;
                }
            }
...
int multi_create_verify_cds () {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            !MULTI_STANDALONE (Net_players[idx]) &&
            (Net_players[idx].flags & NETINFO_FLAG_HAS_CD)) {
            multi_cd_count++;
        }
    }
...
void multi_create_sw_clicked () {
    ...
    if (MULTIPLAYER_MASTER) { ng = &Netgame; }
    else {
        ng_temp = Netgame;
        ng = &ng_temp;
    }
    ...
    // on the standalone
        ...
        multi_options_update_mission (
            ng, Multi_create_list_mode == MULTI_CREATE_SHOW_CAMPAIGNS ? 1 : 0);
    }
}
...
#define MULTI_HO_HOST_MODIFIES \
    10 // only the host or team captains can modify ships/weapons in briefing
...
UI_XSTR Multi_ho_text[GR_NUM_RESOLUTIONS][MULTI_HO_NUM_BUTTONS] = {
    {
        // GR_640
        { "Highest rank", 1280, 46, 166, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[0][MULTI_HO_MSG_RANK].button },
        { "Team / wing-leader", 1281, 46, 185, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[0][MULTI_HO_MSG_LEADER].button },
        { "Any", 1282, 46, 203, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[0][MULTI_HO_MSG_ANY].button },
        { "Host", 1283, 46, 220, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[0][MULTI_HO_MSG_HOST].button },
        { "Highest rank", 1280, 46, 265, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[0][MULTI_HO_END_RANK].button },
        { "Team / wing-leader", 1281, 46, 283, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[0][MULTI_HO_END_LEADER].button },
        { "Any", 1282, 46, 300, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[0][MULTI_HO_END_ANY].button },
        { "Host", 1283, 46, 317, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[0][MULTI_HO_END_HOST].button },
        { "On", 1285, 545, 185, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[0][MULTI_HO_VOICE_ON].button },
        { "Off", 1286, 604, 185, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[0][MULTI_HO_VOICE_OFF].button },
        { "Host modifies ships", 1287, 437, 363, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[0][MULTI_HO_HOST_MODIFIES].button },
        { "Exit", 1417, 572, 418, UI_XSTR_COLOR_PINK, -1,
          &Multi_ho_buttons[0][MULTI_HO_ACCEPT].button },
    },
    {
        // GR_1024
        { "Highest rank", 1280, 62, 269, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[1][MULTI_HO_MSG_RANK].button },
        { "Team / wing-leader", 1281, 62, 297, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[1][MULTI_HO_MSG_LEADER].button },
        { "Any", 1282, 62, 325, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[1][MULTI_HO_MSG_ANY].button },
        { "Host", 1283, 62, 352, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[1][MULTI_HO_MSG_HOST].button },
        { "Highest rank", 1280, 62, 425, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[1][MULTI_HO_END_RANK].button },
        { "Team / wing-leader", 1281, 62, 452, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[1][MULTI_HO_END_LEADER].button },
        { "Any", 1282, 62, 480, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[1][MULTI_HO_END_ANY].button },
        { "Host", 1283, 62, 508, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[1][MULTI_HO_END_HOST].button },
        { "On", 1285, 877, 294, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[1][MULTI_HO_VOICE_ON].button },
        { "Off", 1286, 967, 293, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[1][MULTI_HO_VOICE_OFF].button },
        { "Host modifies ships", 1287, 869, 589, UI_XSTR_COLOR_GREEN, -1,
          &Multi_ho_buttons[1][MULTI_HO_HOST_MODIFIES].button },
        { "Exit", 1417, 953, 672, UI_XSTR_COLOR_PINK, -1,
...
ho_sliders Multi_ho_sliders[GR_NUM_RESOLUTIONS][MULTI_HO_NUM_SLIDERS] = {
    ...
    },
    {
        ...
        ho_sliders (
            "2_MH_12", 685, 418, 837, 468, 12, 32, 10),       // voice duration
...
UI_XSTR Multi_ho_titles[GR_NUM_RESOLUTIONS][MULTI_HO_NUM_TITLES] = {
    {
        ...
        { "Voice Transmission", 1295, 437, 144, UI_XSTR_COLOR_GREEN, -1,
          NULL },
    ...
    },
    {
        ...
        { "Voice Transmission", 1295, 869, 239, UI_XSTR_COLOR_GREEN, -1,
          NULL },
...
int Ho_time_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                36, 362, 36, 17 },
                                              { // GR_1024
                                                58, 581, 57, 27 } };

// furball kill limit input box
int Ho_kill_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                36, 425, 45, 17 },
                                              { // GR_1024
                                                58, 684, 72, 27 } };

// voice recording duration text display area
int Ho_vd_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                              467, 292, 55, 15 },
                                            { // GR_1024
                                              750, 467, 85, 28 } };

// voice token wait input box
int Ho_vw_coords[GR_NUM_RESOLUTIONS][6] = { { // GR_640
                                              467, 332, 55, 15 },
                                            { // GR_1024
                                              750, 534, 85, 28 } };

// observer count input box
int Ho_obs_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                               36, 457, 45, 17 },
                                             { // GR_1024
                                               58, 733, 72, 27 } };

// skill text description area
int Ho_st_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                              249, 435, 172, 10 },
                                            { // GR_1024
                                              403, 699, 172, 10 } };
...
int Ho_max_rsp_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                   150, 378 },
                                                 { // GR_1024
                                                   190, 618 } };
...
void multi_host_options_init () {
    ...
    for (idx = 0; idx < MULTI_HO_NUM_BUTTONS; idx++) {
        // create the object
        Multi_ho_buttons[gr_screen.res][idx].button.create (
            &Multi_ho_window, "", Multi_ho_buttons[gr_screen.res][idx].x,
            Multi_ho_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);

        // set the sound to play when highlighted
        Multi_ho_buttons[gr_screen.res][idx].button.set_highlight_action (
            common_play_highlight_sound);

        // set the ani for the button
        Multi_ho_buttons[gr_screen.res][idx].button.set_bmaps (
            Multi_ho_buttons[gr_screen.res][idx].filename);

        // set the hotspot, ignoring the skill level button
        Multi_ho_buttons[gr_screen.res][idx].button.link_hotspot (
            Multi_ho_buttons[gr_screen.res][idx].hotspot);
    ...
    for (idx = 0; idx < MULTI_HO_NUM_SLIDERS; idx++) {
        // create the object
        Multi_ho_sliders[gr_screen.res][idx].slider.create (
            &Multi_ho_window, Multi_ho_sliders[gr_screen.res][idx].x,
            Multi_ho_sliders[gr_screen.res][idx].y,
            Multi_ho_sliders[gr_screen.res][idx].dots,
            Multi_ho_sliders[gr_screen.res][idx].filename,
            Multi_ho_sliders[gr_screen.res][idx].hotspot, NULL, -1, -1, -1,
            NULL, -1, -1, -1, Multi_ho_sliders[gr_screen.res][idx].dot_w);
    }

    // create the respawn count input box
    Multi_ho_respawns.create (
        &Multi_ho_window, Ho_rsp_coords[gr_screen.res][MULTI_HO_X_COORD],
        Ho_rsp_coords[gr_screen.res][MULTI_HO_Y_COORD],
        Ho_rsp_coords[gr_screen.res][MULTI_HO_W_COORD], 6, "",
        UI_INPUTBOX_FLAG_ESC_FOC | UI_INPUTBOX_FLAG_INVIS |
            UI_INPUTBOX_FLAG_NO_LETTERS,
        -1, &Color_bright);
    ...
    Multi_ho_time_limit.create (
        &Multi_ho_window, Ho_time_coords[gr_screen.res][MULTI_HO_X_COORD],
    ...
    Multi_ho_voice_wait.create (
        &Multi_ho_window, Ho_vw_coords[gr_screen.res][MULTI_HO_X_COORD],
    ...
    Multi_ho_kill_limit.create (
        &Multi_ho_window, Ho_kill_coords[gr_screen.res][MULTI_HO_X_COORD],
    ...
    Multi_ho_obs.create (
        &Multi_ho_window, Ho_obs_coords[gr_screen.res][MULTI_HO_X_COORD],
        Ho_obs_coords[gr_screen.res][MULTI_HO_Y_COORD],
        Ho_obs_coords[gr_screen.res][MULTI_HO_W_COORD], 1, "",
        UI_INPUTBOX_FLAG_ESC_FOC | UI_INPUTBOX_FLAG_INVIS |
            UI_INPUTBOX_FLAG_NO_LETTERS,
        -1, &Color_bright);
    ...
    if (Multi_create_list_select != -1) {
        int abs_index =
            multi_create_select_to_index (Multi_create_list_select);
...
void multi_ho_update_sliders () {
    ...
    if (Game_skill_level !=
        ...
        if (!(Netgame.type_flags & NG_TYPE_TEAM)) {
            Game_skill_level =
                Multi_ho_sliders[gr_screen.res][MULTI_HO_SLIDER_SKILL]
                    .slider.pos;
            gamesnd_play_iface (InterfaceSounds::USER_SELECT);
        }
        else {
    ...
    if (Netgame.options.voice_qos !=
        (ubyte) (
            Multi_ho_sliders[gr_screen.res][MULTI_HO_SLIDER_VOICE_QOS]
                .slider.pos +
            1)) {
        Netgame.options.voice_qos = (ubyte) (
            Multi_ho_sliders[gr_screen.res][MULTI_HO_SLIDER_VOICE_QOS]
                .slider.pos +
            1);
        gamesnd_play_iface (InterfaceSounds::USER_SELECT);
...
void multi_host_options_do () {
    ...
    // process any keypresses
    switch (k) {
    case KEY_ESC: gameseq_post_event (GS_EVENT_MULTI_HOST_SETUP); break;
    // same as ACCEPT
    ...
    // make sure that the chatbox inputbox and any inputbox on this screen are
    // mutually exclusive in terms of focus
    ...
    if (Multi_ho_host_modifies) {
        Multi_ho_buttons[gr_screen.res][MULTI_HO_HOST_MODIFIES]
            .button.draw_forced (2);
    }
...
void multi_ho_button_pressed (int n) {
    ...
    Multi_ho_buttons[gr_screen.res][n].button.get_mouse_pos (
        &x_pixel, &y_pixel);
    ...
    if (radio_index < MULTI_HO_NUM_RADIO_BUTTONS) {
        ...
        if (Multi_ho_radio_groups[Multi_ho_radio_info[radio_index][0]] !=
            Multi_ho_radio_info[radio_index][1]) {
            gamesnd_play_iface (InterfaceSounds::USER_SELECT);
            Multi_ho_radio_groups[Multi_ho_radio_info[radio_index][0]] =
                Multi_ho_radio_info[radio_index][1];
        }
        else {
            gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
...
void multi_ho_draw_radio_groups () {
    ...
    // go through each item and draw it if it is the selected button in its
    // respective group
    for (idx = 0; idx < MULTI_HO_NUM_RADIO_BUTTONS; idx++) {
        /// if this button is the currently selected one in its group
        if (Multi_ho_radio_info[idx][1] ==
            Multi_ho_radio_groups[Multi_ho_radio_info[idx][0]]) {
            Multi_ho_buttons[gr_screen.res][Multi_ho_radio_info[idx][2]]
                .button.draw_forced (2);
        }
    }
}
...
void multi_ho_accept_hit () {
    ...
    if (!multi_ho_check_values ()) { return; }
    ...
    case 0: Netgame.options.squad_set = MSO_SQUAD_RANK; break;
    case 1: Netgame.options.squad_set = MSO_SQUAD_LEADER; break;
    case 2: Netgame.options.squad_set = MSO_SQUAD_ANY; break;
    case 3: Netgame.options.squad_set = MSO_SQUAD_HOST; break;
    default: Int3 ();
    }

    // set the end mission flags
    switch (Multi_ho_radio_groups[MULTI_HO_END_GROUP]) {
    case 0: Netgame.options.endgame_set = MSO_END_RANK; break;
    case 1: Netgame.options.endgame_set = MSO_END_LEADER; break;
    case 2: Netgame.options.endgame_set = MSO_END_ANY; break;
    case 3: Netgame.options.endgame_set = MSO_END_HOST; break;
    default: Int3 ();
    }

    // set the voice toggle
    switch (Multi_ho_radio_groups[MULTI_HO_VOICE_GROUP]) {
    case 0: Netgame.options.flags &= ~(MSO_FLAG_NO_VOICE); break;
    case 1: Netgame.options.flags |= MSO_FLAG_NO_VOICE; break;
    default: Int3 ();
    }
    ...
    // set the skill level.  If in team vs. team mode, preserve the old setting
    // before saving the pilot file.  I'll bet that this doesn't work though
    // because the pilot file gets written in a bunch of locations....sigh.
    if (!(Netgame.type_flags & NG_TYPE_TEAM)) {
        Game_skill_level =
            Multi_ho_sliders[gr_screen.res][MULTI_HO_SLIDER_SKILL].slider.pos;
    }
    ...
    // maybe warn the user that respawns will not be used for a campaign
    // mission
    if (Netgame.campaign_mode == MP_SINGLE_MISSION) {
        Multi_ho_respawns.get_text (resp_str);
        uint temp_respawn = (uint)atoi (resp_str);
        // if he currently has no mission selected, let the user set any # of
        // respawns
    ...
    if (temp_time <= 0) { Netgame.options.mission_time_limit = fl2f (-1.0f); }
    ...
    if (temp_obs > MULTI_HO_MAX_OBS) { Int3 (); }
    ...
    if (temp_kills > MULTI_HO_MAX_KILL_LIMIT) { Int3 (); }
    ...
    if (temp_wait > MULTI_HO_MAX_TOKEN_WAIT) { Int3 (); }
    ...
    memcpy (
        &Player->m_local_options, &Net_player->p_info.options,
        sizeof (multi_local_options));
    memcpy (
        &Player->m_server_options, &Netgame.options,
        sizeof (multi_server_options));
    Pilot.save_player (Player);

    // apply any changes in settings (notify everyone of voice qos changes,
    // etc)
...
void multi_ho_get_options () {
    ...
    case MSO_SQUAD_RANK: Multi_ho_radio_groups[MULTI_HO_MSG_GROUP] = 0; break;
    case MSO_SQUAD_LEADER:
        Multi_ho_radio_groups[MULTI_HO_MSG_GROUP] = 1;
        break;
    case MSO_SQUAD_ANY: Multi_ho_radio_groups[MULTI_HO_MSG_GROUP] = 2; break;
    case MSO_SQUAD_HOST: Multi_ho_radio_groups[MULTI_HO_MSG_GROUP] = 3; break;
    default: Int3 ();
    }

    // set the mission end buttons
    switch (Netgame.options.endgame_set) {
    case MSO_END_RANK: Multi_ho_radio_groups[MULTI_HO_END_GROUP] = 0; break;
    case MSO_END_LEADER: Multi_ho_radio_groups[MULTI_HO_END_GROUP] = 1; break;
    case MSO_END_ANY: Multi_ho_radio_groups[MULTI_HO_END_GROUP] = 2; break;
    case MSO_END_HOST: Multi_ho_radio_groups[MULTI_HO_END_GROUP] = 3; break;
    default: Int3 ();
    }
...
void multi_ho_display_record_time () {
    ...
    // get the seconds
    full_seconds =
        (((Multi_ho_sliders[gr_screen.res][MULTI_HO_SLIDER_VOICE_DUR]
               .slider.pos +
           1) *
          500) /
         1000);
    ...
    gr_set_color_fast (&Color_bright);
    gr_string (
        Ho_vd_coords[gr_screen.res][MULTI_HO_X_COORD],
        Ho_vd_coords[gr_screen.res][MULTI_HO_Y_COORD], time_str,
        GR_RESIZE_MENU);
}
...
int multi_ho_check_values () {
    ...
    if (Multi_ho_mission_respawn != -1) {
        ...
        if (atoi (val_txt) > Multi_ho_mission_respawn) {
            ...
            sprintf (
                val_txt,
                XSTR (
                    "Warning\nRespawn count in greater than mission specified "
                    "max (%d)",
                    796),
                Multi_ho_mission_respawn);
            popup (
                PF_USE_AFFIRMATIVE_ICON | PF_TITLE_RED | PF_TITLE_BIG, 1,
                POPUP_OK, val_txt);
            return 0;
        }
    }
    ...
    if (atoi (val_txt) > MULTI_HO_MAX_TIME_LIMIT) {
        ...
        popup (
            PF_USE_AFFIRMATIVE_ICON | PF_TITLE_RED | PF_TITLE_BIG, 1, POPUP_OK,
            val_txt);
        return 0;
    }
...
void multi_ho_check_focus () {
    ...
    if (Multi_ho_respawns.pressed () || Multi_ho_time_limit.pressed () ||
        Multi_ho_voice_wait.pressed () || Multi_ho_kill_limit.pressed () ||
        Multi_ho_obs.pressed ()) {
        ...
        if (Multi_ho_respawns.has_focus () ||
            Multi_ho_time_limit.has_focus () ||
            Multi_ho_kill_limit.has_focus () ||
    ...
    // if we _did_ have focus last frame
        ...
        // if we no longer have focus on any of the input boxes, set the focus
        // on the chatbox
        if (!Multi_ho_respawns.has_focus () &&
            !Multi_ho_time_limit.has_focus () &&
            !Multi_ho_kill_limit.has_focus () &&
...
void multi_ho_blit_max_respawns () {
    ...
    if (Netgame.campaign_mode == MP_CAMPAIGN) { return; }

    // otherwise blit the max as specified by the current mission file
    sprintf (string, "(%d)", Multi_ho_mission_respawn);
    gr_set_color_fast (&Color_normal);
    gr_string (
        Ho_max_rsp_coords[gr_screen.res][MULTI_HO_X_COORD],
        Ho_max_rsp_coords[gr_screen.res][MULTI_HO_Y_COORD], string,
        GR_RESIZE_MENU);
}
...
void multi_ho_display_skill_level () {
    ...
    gr_set_color_fast (&Color_bright);
    gr_string (
        Ho_st_coords[gr_screen.res][0], Ho_st_coords[gr_screen.res][1],
        Skill_level_names (skill_level, 1), GR_RESIZE_MENU);
}
...
UI_XSTR Multi_jw_text[GR_NUM_RESOLUTIONS][MULTI_JW_NUM_TEXT] = {
    {
        // GR_640
        { "Team 1", 1308, 20, 272, UI_XSTR_COLOR_GREEN, -1,
          &Multi_jw_buttons[0][MJW_TEAM0].button },
        { "Team 2", 1309, 73, 272, UI_XSTR_COLOR_GREEN, -1,
          &Multi_jw_buttons[0][MJW_TEAM1].button },
        { "Pilot", 1310, 134, 272, UI_XSTR_COLOR_GREEN, -1,
          &Multi_jw_buttons[0][MJW_PILOT_INFO].button },
        { "Info", 1311, 134, 283, UI_XSTR_COLOR_GREEN, -1,
          &Multi_jw_buttons[0][MJW_PILOT_INFO].button },
        { "Cancel", 387, 570, 414, UI_XSTR_COLOR_PINK, -1,
    ...
    {
        // GR_1024
        { "Team 1", 1308, 47, 435, UI_XSTR_COLOR_GREEN, -1,
          &Multi_jw_buttons[1][MJW_TEAM0].button },
        { "Team 2", 1309, 133, 435, UI_XSTR_COLOR_GREEN, -1,
          &Multi_jw_buttons[1][MJW_TEAM1].button },
        { "Pilot", 1310, 225, 435, UI_XSTR_COLOR_GREEN, -1,
          &Multi_jw_buttons[1][MJW_PILOT_INFO].button },
        { "Info", 1311, 225, 446, UI_XSTR_COLOR_GREEN, -1,
          &Multi_jw_buttons[1][MJW_PILOT_INFO].button },
        { "Cancel", 387, 931, 667, UI_XSTR_COLOR_PINK, -1,
...
int Mjw_players_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                    29, 18, 153, 210 },
                                                  { // GR_1024
                                                    46, 29, 254, 336 } };

int Mjw_mission_name_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                         5, 380 },
                                                       { // GR_1024
                                                         47, 618 } };
...
int Multi_jw_sw_checkbox_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                             6, 285 },
                                                           { // GR_1024
                                                             18, 450 } };
...
int Multi_jw_plist_select_flag; // indicates whether we currently have a
                                // selected player
...
void multi_game_client_setup_init () {
    ...
    chatbox_close ();
    chatbox_create (
        CHATBOX_FLAG_BIG | CHATBOX_FLAG_DRAW_BOX | CHATBOX_FLAG_BUTTONS);
    ...
    for (idx = 0; idx < MULTI_JW_NUM_BUTTONS; idx++) {
        // create the object
        Multi_jw_buttons[gr_screen.res][idx].button.create (
            &Multi_jw_window, "", Multi_jw_buttons[gr_screen.res][idx].x,
            Multi_jw_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);

        // set the sound to play when highlighted
        Multi_jw_buttons[gr_screen.res][idx].button.set_highlight_action (
            common_play_highlight_sound);

        // set the ani for the button
        Multi_jw_buttons[gr_screen.res][idx].button.set_bmaps (
            Multi_jw_buttons[gr_screen.res][idx].filename);

        // set the hotspot
        Multi_jw_buttons[gr_screen.res][idx].button.link_hotspot (
            Multi_jw_buttons[gr_screen.res][idx].hotspot);
    }

    // if this is a PXO game, enable the squadwar checkbox
    Multi_jw_sw_checkbox.create (
        &Multi_jw_window, "", Multi_jw_sw_checkbox_coords[gr_screen.res][0],
        Multi_jw_sw_checkbox_coords[gr_screen.res][1], 0);
    Multi_jw_sw_checkbox.set_bmaps (
        Multi_jw_sw_checkbox_fname[gr_screen.res], 6, 0);
    Multi_jw_sw_checkbox.disable ();
    if (!MULTI_IS_TRACKER_GAME) { Multi_jw_sw_checkbox.hide (); }
    ...
    Multi_jw_plist_select_button.create (
        &Multi_jw_window, "", Mjw_players_coords[gr_screen.res][MJW_X_COORD],
...
void multi_game_client_setup_do_frame () {
    ...
    if (MULTI_IS_TRACKER_GAME) {
        ...
        gr_string (
            Multi_jw_sw_checkbox_text[gr_screen.res][0],
            Multi_jw_sw_checkbox_text[gr_screen.res][1], "SquadWar",
            GR_RESIZE_MENU);
    }
    ...
    if (Netgame.type_flags & NG_TYPE_TEAM) { multi_jw_plist_blit_team (); }
    else {
    ...
    if (Netgame.campaign_mode) {
        if (Netgame.campaign_name[0] != '\0') {
            ...
            gr_string (
                Mjw_mission_name_coords[gr_screen.res][MJW_X_COORD],
                Mjw_mission_name_coords[gr_screen.res][MJW_Y_COORD],
                mission_text, GR_RESIZE_MENU);
        }
    }
    else {
...
void multi_jw_button_pressed (int n) {
    ...
    case MJW_CANCEL:
        ...
        break;
    case MJW_SCROLL_PLAYERS_UP: multi_jw_scroll_players_up (); break;
    case MJW_SCROLL_PLAYERS_DOWN: multi_jw_scroll_players_down (); break;
    case MJW_SCROLL_INFO_UP: multi_common_scroll_text_up (); break;
    case MJW_SCROLL_INFO_DOWN: multi_common_scroll_text_down (); break;
    ...
    // pilot info popup
    case MJW_PILOT_INFO: Multi_jw_should_show_popup = 1; break;
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        ...
        if (MULTI_CONNECTED (Net_players[idx]) &&
            ...
            if ((Net_players[idx].flags & NETINFO_FLAG_HAS_CD) &&
                ...
                gr_set_bitmap (Multi_common_icons[MICON_CD]);
                gr_bitmap (
                    Mjw_players_coords[gr_screen.res][MJW_X_COORD] +
                        total_offset,
                    y_start - 1, GR_RESIZE_MENU);
            ...
            font::force_fit_string (
                str, CALLSIGN_LEN,
                Mjw_players_coords[gr_screen.res][MJW_W_COORD] - total_offset);
            gr_string (
                Mjw_players_coords[gr_screen.res][MJW_X_COORD] + total_offset,
                y_start, str, GR_RESIZE_MENU);
...
short multi_jw_get_mouse_id () {
    ...
    if (Netgame.type_flags & NG_TYPE_TEAM) {
        ...
        if (player_index != -1) { return Net_players[idx].player_id; }
    }
    else {
        // select the nth active player if possible, disregarding the
        // standalone server
...
int Multi_sync_button_count; // the # of buttons to use for this instance of
                             // mission sync
...
UI_XSTR Multi_sync_text[GR_NUM_RESOLUTIONS][MULTI_SYNC_NUM_TEXT] = {
    { // GR_640
      { "Kick", 1266, 479, 416, UI_XSTR_COLOR_PINK, -1,
        &Multi_sync_buttons[0][MS_KICK].button },
      { "Cancel", 387, 519, 416, UI_XSTR_COLOR_PINK, -1,
        &Multi_sync_buttons[0][MS_CANCEL].button },
      { "Launch", 801, 577, 416, UI_XSTR_COLOR_PINK, -1,
    ...
    { // GR_1024
      { "Kick", 1266, 766, 667, UI_XSTR_COLOR_PINK, -1,
        &Multi_sync_buttons[1][MS_KICK].button },
      { "Cancel", 387, 831, 667, UI_XSTR_COLOR_PINK, -1,
        &Multi_sync_buttons[1][MS_CANCEL].button },
      { "Launch", 801, 924, 667, UI_XSTR_COLOR_PINK, -1,
...
// player name
int Ms_status_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                  38, 150, 581, 220 },
                                                { // GR_1024
                                                  38, 228, 958, 367 } };

// player status coords
int Ms_status2_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                   228, 150, 391, 220 },
                                                 { // GR_1024
                                                   370, 228, 626, 367 } };
...
#define MS_FLAG_PUSHED_BRIEFING \
    (1 << 2)                       // pushed everyone else into the briefing
...
void multi_sync_display_name (
    const char* name, int index,
    int np_index); // display info on the left hand portion of the status
                   // window thingie
void multi_sync_display_status (
    const char* status, int index); // display info on the right hand portion
                                    // of the status window thingie
...
void multi_sync_init () {
    ...
    if (!(Game_mode & GM_STANDALONE_SERVER)) { multi_sync_common_init (); }

    switch (Multi_sync_mode) {
    case MULTI_SYNC_PRE_BRIEFING: multi_sync_pre_init (); break;
    case MULTI_SYNC_POST_BRIEFING: multi_sync_post_init (); break;
    case MULTI_SYNC_INGAME: multi_ingame_sync_init (); break;
    }
}

// perform the correct do frame functions
void multi_sync_do () {
    if (!(Game_mode & GM_STANDALONE_SERVER)) { multi_sync_common_do (); }

    // if the netgame is ending, don't do any sync processing
    if (multi_endgame_ending ()) { return; }

    // process appropriateliy
    switch (Multi_sync_mode) {
    case MULTI_SYNC_PRE_BRIEFING: multi_sync_pre_do (); break;
    case MULTI_SYNC_POST_BRIEFING: multi_sync_post_do (); break;
...
void multi_sync_close () {
    switch (Multi_sync_mode) {
    case MULTI_SYNC_PRE_BRIEFING: multi_sync_pre_close (); break;
    case MULTI_SYNC_POST_BRIEFING: multi_sync_post_close (); break;
    case MULTI_SYNC_INGAME: multi_ingame_sync_close (); break;
    }

    if (!(Game_mode & GM_STANDALONE_SERVER)) { multi_sync_common_close (); }
}
...
void multi_sync_common_init () {
    ...
    // create the interface window
    Multi_sync_window.create (
        0, 0, gr_screen.max_w_unscaled, gr_screen.max_h_unscaled, 0);
    Multi_sync_window.set_mask_bmap (
        Multi_sync_bitmap_mask_fname[gr_screen.res]);
    ...
    Multi_sync_plist_count = 1; // we can pretty safely assume that there's one
                                // player in the game - me.
    ...
    for (idx = 0; idx < Multi_sync_button_count; idx++) {
        // create the object
        Multi_sync_buttons[gr_screen.res][idx].button.create (
            &Multi_sync_window, "", Multi_sync_buttons[gr_screen.res][idx].x,
            Multi_sync_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);

        // set the sound to play when highlighted
        Multi_sync_buttons[gr_screen.res][idx].button.set_highlight_action (
            common_play_highlight_sound);
        ...
        if ((idx == MS_CANCEL) || (idx == MS_KICK) || (idx == MS_LAUNCH)) {
            ...
            Multi_sync_buttons[gr_screen.res][idx].button.set_bmaps (
                Multi_sync_buttons[gr_screen.res][idx].filename);
        }

        // set the hotspot
        Multi_sync_buttons[gr_screen.res][idx].button.link_hotspot (
            Multi_sync_buttons[gr_screen.res][idx].hotspot);
    }

    // add xstrs
    for (idx = 0; idx < MULTI_SYNC_NUM_TEXT; idx++) {
        // don't create the "launch" button text just yet
        if (idx == MST_LAUNCH) { continue; }
    ...
    Multi_sync_plist_button.create (
        &Multi_sync_window, "", Ms_status_coords[gr_screen.res][MS_X_COORD],
...
void multi_sync_blit_screen_all () {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            ...
            multi_sync_display_name (
                Net_players[idx].m_player->callsign, count, idx);

            // get the player state
            state = Net_players[idx].state;

            // if we're ingame joining, show all other players except myself as
            // "playing"
            if ((Net_player != NULL) && (&Net_players[idx] != Net_player) &&
            ...
            case NETPLAYER_STATE_MISSION_LOADING:
                multi_sync_display_status (
                    XSTR ("Mission Loading", 802), count);
                break;
            case NETPLAYER_STATE_INGAME_SHIP_SELECT: // I don't think its
                                                     // possible to see this
                                                     // state, but...
                multi_sync_display_status (
                    XSTR ("Ingame Ship Select", 803), count);
                break;
            ...
            case NETPLAYER_STATE_SETTINGS_ACK:
                multi_sync_display_status (
                    XSTR ("Ready To Enter Mission", 815), count);
                break;
            case NETPLAYER_STATE_INGAME_SHIPS:
                multi_sync_display_status (
                    XSTR ("Ingame Ships Packet Ack", 816), count);
                break;
            case NETPLAYER_STATE_INGAME_WINGS:
                multi_sync_display_status (
                    XSTR ("Ingame Wings Packet Ack", 817), count);
                break;
            case NETPLAYER_STATE_INGAME_RPTS:
                multi_sync_display_status (
                    XSTR ("Ingame Respawn Points Ack", 818), count);
                break;
            case NETPLAYER_STATE_SLOTS_ACK:
                multi_sync_display_status (
                    XSTR ("Ingame Weapon Slots Ack", 819), count);
                break;
            case NETPLAYER_STATE_POST_DATA_ACK:
                multi_sync_display_status (
                    XSTR ("Post Briefing Data Block Ack", 820), count);
                break;
            ...
            case NETPLAYER_STATE_MT_STATS:
                multi_sync_display_status (
                    XSTR ("Parallax Online Stats Updating", 822), count);
                break;
            case NETPLAYER_STATE_WSS_ACK:
                multi_sync_display_status (
                    XSTR ("Weapon Slots Ack", 823), count);
                break;
            ...
            case NETPLAYER_STATE_DEBRIEF_ACCEPT:
                multi_sync_display_status (
                    XSTR ("Debrief accept", 825), count);
                break;
            case NETPLAYER_STATE_DEBRIEF_REPLAY:
                multi_sync_display_status (
                    XSTR ("Debrief replay", 826), count);
                break;
            case NETPLAYER_STATE_CPOOL_ACK:
                multi_sync_display_status (
                    XSTR ("Campaign ship/weapon ack", 827), count);
                break;
            case NETPLAYER_STATE_MISSION_XFER:
                ...
                if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
                    if (Net_players[idx].s_info.xfer_handle != -1) {
                        pct_complete = multi_xfer_pct_complete (
                            Net_players[idx].s_info.xfer_handle);

                        // if we've got a valid xfer handle
                        if ((pct_complete >= 0.0) && (pct_complete <= 1.0)) {
                            sprintf (
                                txt, XSTR ("Mission file xfer %d%%", 828),
                                (int)(pct_complete * 100.0f));
                        }
                        // otherwise
                        else {
                ...
                // clients should display only for themselves (which is the
    ...
    if ((gameseq_get_state () == GS_STATE_MULTI_MISSION_SYNC &&
        ...
        (Multi_sync_countdown != -1))
        generic_anim_render (
            &Multi_sync_countdown_anim, flFrametime,
            Multi_sync_countdown_coords[gr_screen.res][MS_X_COORD],
            Multi_sync_countdown_coords[gr_screen.res][MS_Y_COORD], true);
...
void multi_sync_button_pressed (int n) {
    ...
    case MS_SCROLL_INFO_UP: multi_common_scroll_text_up (); break;

    // scroll the info box down
    case MS_SCROLL_INFO_DOWN: multi_common_scroll_text_down (); break;
    ...
    case MS_LAUNCH: multi_sync_start_countdown (); break;
...
void multi_sync_pre_init () {
    ...
    if((Netgame.type_flags & NG_TYPE_TEAM) && (Net_player->flags &
    NETINFO_FLAG_GAME_HOST)){ Netgame.options.skill_level = NUM_SKILL_LEVELS /
    2; Game_skill_level = NUM_SKILL_LEVELS / 2; multi_options_update_netgame();
    }
    */
    ...
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        ...
        if (Netgame.type_flags & NG_TYPE_TEAM) { multi_team_level_init (); }
        ...
        // do this here to insure we have the most up to date file checksum
        // info
        ...
        if (!(Game_mode & GM_STANDALONE_SERVER)) {
            multi_common_add_text (
                XSTR ("Sending update state packet\n", 833), 1);
        }
    }

    // if we're not in team vs. team mode - set all player teams to be 0, and
    // unset all captaincy bits
    if (!(Netgame.type_flags & NG_TYPE_TEAM)) {
        for (idx = 0; idx < MAX_PLAYERS; idx++) {
            Net_players[idx].p_info.team = 0;
...
void multi_sync_pre_do () {
    ...
    // If I'm the server, wait for everyone to arrive in this state, then begin
    // transferring data, etc. all servers (standalone or no, go through this)
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        ...
        if (multi_netplayer_state_check (NETPLAYER_STATE_MISSION_SYNC) &&
            ...
            if (!(Game_mode & GM_STANDALONE_SERVER)) {
                multi_common_add_text (
                    XSTR ("Sent filesig request\n", 834), 1);
            }
        }

        // if we're waiting for players to receive files, then check on their
        // status
        if ((Mission_sync_flags & MS_FLAG_SENT_FILESIG) &&
            !multi_netplayer_flag_check (NETINFO_FLAG_MISSION_OK) &&
            !multi_endgame_ending ()) {
            for (idx = 0; idx < MAX_PLAYERS; idx++) {
                // if this player is in the process of xferring a file
                if (MULTI_CONNECTED (Net_players[idx]) &&
                    (Net_players[idx].s_info.xfer_handle != -1)) {
                    switch (multi_xfer_get_status (
                        Net_players[idx].s_info.xfer_handle)) {
        ...
        if (multi_netplayer_flag_check (NETINFO_FLAG_MISSION_OK) &&
        ...
        // wait for everyone to ack on campaign pool data (even in non-campaign
        // situations)
        if ((Mission_sync_flags & MS_FLAG_DATA_DONE) &&
        ...
        // also make sure to load the mission myself _AFTER_ telling everyone
        // to do so. This makes the whole process move along faster
        if ((Mission_sync_flags & MS_FLAG_CAMP_DONE) &&
            ...
            if (!Multi_mission_loaded) {
                ...
                if (!(Game_mode & GM_STANDALONE_SERVER)) {
                    multi_common_add_text (
                        XSTR ("Loaded mission locally\n", 836), 1);
                }
            }
        }

        // if everyone has loaded the mission, randomly assign players to ships
        if (multi_netplayer_state_check (NETPLAYER_STATE_MISSION_LOADED) &&
            !(Mission_sync_flags & MS_FLAG_TS_SLOTS) &&
            !multi_endgame_ending ()) {
            // call the team select function to assign players to their ships,
            // wings, etc
        ...
        if (Sync_test &&
            multi_netplayer_state_check (NETPLAYER_STATE_SLOT_ACK) &&
            !(Mission_sync_flags & MS_FLAG_PUSHED_BRIEFING) &&
            !multi_endgame_ending ()) {
            Netgame.game_state = NETGAME_STATE_BRIEFING;
            send_netgame_update_packet (); // this will push everyone into the
                                           // next state

            // the standalone moves to his own wait state, whereas in the
            // normal game mode, the server/host moves in to the team select
            // state
            if (Game_mode & GM_STANDALONE_SERVER) {
            ...
            if (!(Game_mode & GM_STANDALONE_SERVER)) {
                multi_common_add_text (
                    XSTR ("Moving to team select\n", 837), 1);
            }
        }
    }
    else {
        // clients should detect here if they are doing a file xfer and do
        // error processing
        if ((Net_player->state == NETPLAYER_STATE_MISSION_XFER) &&
            (Net_player->s_info.xfer_handle != -1) &&
            !multi_endgame_ending ()) {
...
void multi_sync_post_init () {
    ...
    // if I am not the master of the game, then send the firing information for
    // my ship to the host
    if (!(Net_player->flags & NETINFO_FLAG_AM_MASTER)) {
    ...
    if (!(Game_mode & GM_STANDALONE_SERVER)) {
        generic_anim_init (
            &Multi_sync_countdown_anim,
            Multi_sync_countdown_fname[gr_screen.res]);
        Multi_sync_countdown_anim.ani.bg_type =
    ...
    if (Netgame.type_flags & NG_TYPE_TEAM) { multi_team_mark_all_ships (); }
...
void multi_sync_post_do () {
    ...
    // only if the host is also the master should he be doing this
    // (non-standalone situation)
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        // once everyone gets to this screen, send them the ship classes of all
        // ships.
        if (multi_netplayer_state_check (NETPLAYER_STATE_WAITING) &&
            !(Mission_sync_flags & MS_FLAG_POST_DATA)) {
            // only the host should ever do this
            if (Net_player->flags & NETINFO_FLAG_GAME_HOST) {
                // at this point we want to delete all necessary ships, change
                // all necessary ship classes, and set all weapons up
                ...
                for (idx = 0; idx < MAX_PLAYERS; idx++) {
                    if (MULTI_CONNECTED (Net_players[idx]) &&
                        ...
                        (Net_players[idx].m_player->objnum != -1)) {
                        multi_server_update_player_weapons (
                            &Net_players[idx],
            ...
            // note that this is done a little differently for standalones and
            // nonstandalones
            send_post_sync_data_packet ();

            multi_common_add_text (
                XSTR ("Sending post briefing block information\n", 839), 1);

            Mission_sync_flags |= MS_FLAG_POST_DATA;
        }

        // send weapon slots data
        if (multi_netplayer_state_check (NETPLAYER_STATE_POST_DATA_ACK) &&
            ...
            multi_common_add_text (
                XSTR ("Sending weapon slots information\n", 840), 1);

            Mission_sync_flags |= MS_FLAG_WSS_SLOTS;
        }

        // once weapon information is received, send player settings info
        if (multi_netplayer_state_check (NETPLAYER_STATE_WSS_ACK) &&
            ...
            multi_common_add_text (
                XSTR ("Sending player settings packets\n", 841), 1);

            Mission_sync_flags |= MS_FLAG_PSETTINGS;
        }

        // check to see if the countdown timer has started and act
        // appropriately
        ...
        if ((Multi_sync_countdown == 0) &&
            multi_netplayer_state_check (NETPLAYER_STATE_IN_MISSION)) {
            if (!((Net_player->flags & NETINFO_FLAG_GAME_HOST) &&
                  !Multi_sync_launch_pressed)) {
    ...
    if (Net_player->flags & NETINFO_FLAG_GAME_HOST) {
        ...
        if (Sync_test &&
            multi_netplayer_state_check (NETPLAYER_STATE_SETTINGS_ACK)) {
...
void multi_sync_display_name (const char* name, int index, int np_index) {
    ...
    if (Netgame.type_flags & NG_TYPE_TEAM) {
        font::force_fit_string (
            fit, CALLSIGN_LEN,
            Ms_status2_coords[gr_screen.res][MS_X_COORD] -
        ...
        // blit the string
        gr_string (
            Ms_status_coords[gr_screen.res][0] +
                Ms_cd_icon_offset[gr_screen.res] +
                Ms_team_icon_offset[gr_screen.res],
            Ms_status_coords[gr_screen.res][MS_Y_COORD] +
                (index * line_height),
        ...
        if (Net_players[np_index].p_info.team == 0) {
            ...
            if (Net_players[np_index].flags & NETINFO_FLAG_TEAM_CAPTAIN) {
                if (Multi_common_icons[MICON_TEAM0_SELECT] != -1) {
                    gr_set_bitmap (Multi_common_icons[MICON_TEAM0_SELECT]);
                    gr_bitmap (
                        Ms_status_coords[gr_screen.res][MS_X_COORD] +
                            Ms_cd_icon_offset[gr_screen.res],
                        Ms_status_coords[gr_screen.res][MS_Y_COORD] +
                            (index * line_height) - 2,
                        GR_RESIZE_MENU);
    ...
    if ((Net_players[np_index].flags & NETINFO_FLAG_HAS_CD) &&
        (Multi_common_icons[MICON_CD] != -1)) {
        gr_set_bitmap (Multi_common_icons[MICON_CD]);
        gr_bitmap (
            Ms_status_coords[gr_screen.res][MS_X_COORD],
...
void multi_sync_display_status (const char* status, int index) {
    ...
    strcpy_s (fit, status);
    font::force_fit_string (
        fit, 250, Ms_status2_coords[gr_screen.res][MS_W_COORD] - 20);
    gr_set_color_fast (&Color_bright);
    gr_string (
        Ms_status2_coords[gr_screen.res][MS_X_COORD],
        Ms_status2_coords[gr_screen.res][MS_Y_COORD] +
            (index * (gr_get_font_height () + 1)),
        fit, GR_RESIZE_MENU);
}

void multi_sync_force_start_pre () {
    int idx;
    int want_state = NETPLAYER_STATE_SLOT_ACK; // kick any players who are
                                               // still in this state
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            ...
            (Net_players[idx].state == want_state)) {
...
void multi_sync_force_start_post () {
    ...
    // determine the state we want all players in so that we can find those who
...
void multi_sync_start_countdown () {
    ...
    if (Multi_sync_launch_pressed) { return; }
...
void multi_sync_create_launch_button () {
    if (!Multi_launch_button_created) {
        // create the object
        Multi_sync_buttons[gr_screen.res][MS_LAUNCH].button.create (
            &Multi_sync_window, "",
            Multi_sync_buttons[gr_screen.res][MS_LAUNCH].x,
            Multi_sync_buttons[gr_screen.res][MS_LAUNCH].y, 1, 1, 0, 1);

        // set the sound to play when highlighted
        Multi_sync_buttons[gr_screen.res][MS_LAUNCH]
            .button.set_highlight_action (common_play_highlight_sound);

        // set the ani for the button
        Multi_sync_buttons[gr_screen.res][MS_LAUNCH].button.set_bmaps (
            Multi_sync_buttons[gr_screen.res][MS_LAUNCH].filename, 3, 0);

        // set the hotspot
        Multi_sync_buttons[gr_screen.res][MS_LAUNCH].button.link_hotspot (
            Multi_sync_buttons[gr_screen.res][MS_LAUNCH].hotspot);

        // hotkey
        Multi_sync_buttons[gr_screen.res][MS_LAUNCH].button.set_hotkey (
            KEY_CTRLED + KEY_ENTER);

        // create the text for the button
        Multi_sync_window.add_XSTR (
            &Multi_sync_text[gr_screen.res][MST_LAUNCH]);
...
void multi_sync_handle_plist () {
    ...
    if ((Multi_sync_player_select < 0) ||
        !MULTI_CONNECTED (Net_players[Multi_sync_player_select])) {
        for (idx = 0; idx < MAX_PLAYERS; idx++) {
            if (MULTI_CONNECTED (Net_players[idx]) &&
                !MULTI_STANDALONE (Net_players[idx])) {
    ...
    if (Multi_sync_plist_button.pressed ()) {
        ...
        if (select_index > (multi_num_connections () - 1)) { return; }

        // translate into an absolute Net_players[] index (get the Nth net
        // player)
        Multi_sync_player_select = -1;
        for (idx = 0; idx < MAX_PLAYERS; idx++) {
            if (MULTI_CONNECTED (Net_players[idx]) &&
        ...
        // if for some bizarre reason, this is an invalid player, unselect him
        // and wait for the next interation of the loop
        if ((Multi_sync_player_select >= 0) &&
            (!MULTI_CONNECTED (Net_players[Multi_sync_player_select]) ||
...
void multi_debrief_init () {
    ...
    // do this to notify the standalone or the normal server that we're in the
    // debrief state and ready to receive packets
    if (!(Net_player->flags & NETINFO_FLAG_AM_MASTER)) {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx])) {
            Net_players[idx].flags &=
                ~(NETINFO_FLAG_RESPAWNING | NETINFO_FLAG_LIMBO |
                  NETINFO_FLAG_WARPING_OUT);
        }
    }
...
void multi_debrief_do_frame () {
    ...
    if ((Net_player->flags & NETINFO_FLAG_AM_MASTER) &&
        ...
        multi_netplayer_state_check3 (
            NETPLAYER_STATE_DEBRIEF, NETPLAYER_STATE_DEBRIEF_ACCEPT,
...
void multi_maybe_set_mission_loop () {
    ...
    // check for (1) mission loop available, (2) don't have to repeat last
    // mission
    if ((Campaign.missions[cur].flags & CMISSION_FLAG_HAS_LOOP) &&
        (Campaign.loop_mission != -1) && !require_repeat_mission) {
        char buffer[512];
        debrief_assemble_optional_mission_popup_text (
            buffer, Campaign.missions[cur].mission_branch_desc);
...
void multi_debrief_accept_hit () {
    ...
    // but he may need to hit accept again after the server has left the game,
    // so allow this
    ...
    if (MULTI_IS_TRACKER_GAME) {
        int res = popup (
            PF_TITLE | PF_BODY_BIG | PF_USE_AFFIRMATIVE_ICON |
                PF_USE_NEGATIVE_ICON | PF_IGNORE_ESC,
            3, XSTR ("&Cancel", 779), XSTR ("&Accept", 844),
            XSTR ("&Toss", 845),
            XSTR (
        ...
        // undo the accept
        case -1:
        case 0: Multi_debrief_accept_hit = 0; return;

        // toss the stats
        case 2: break;

        // accept the stats
        case 1: fs2netd_store_stats (); break;
        }
    }
    ...
    if (Multi_debrief_server_left) {
        ...
        if (Net_player->flags & NETINFO_FLAG_GAME_HOST) {
            ...
            if (MULTI_IS_TRACKER_GAME) { multi_maybe_set_mission_loop (); }
            else {
                int res = popup (
                    PF_TITLE | PF_BODY_BIG | PF_USE_AFFIRMATIVE_ICON |
                        PF_USE_NEGATIVE_ICON | PF_IGNORE_ESC,
    ...
    if (Net_player->flags & NETINFO_FLAG_GAME_HOST) {
        ...
        if ((Multi_debrief_stats_accept_code == -1) &&
            ...
            res = popup (
                ...
                2, XSTR ("&Cancel", 779), XSTR ("&Leave", 848),
                XSTR (
                    "Are you sure you want to leave the netgame before stats "
                    "are stored?",
                    849));

            // evaluate the result
            if (res == 1) { multi_quit_game (PROMPT_NONE); }
        }
...
// call this when the server has left and we would otherwise be saying "contact
// lost with server
...
void multi_debrief_stats_accept () {
    ...
    if (Multi_debrief_stats_accept_code != -1) { return; }

    Multi_debrief_stats_accept_code = 1;

    // if we're the host, and we're on a standalone, tell the standalone to
    // begin the stats storing process
    if ((Net_player->flags & NETINFO_FLAG_GAME_HOST) ||
        (Net_player->flags & NETINFO_FLAG_AM_MASTER)) {
...
void multi_debrief_stats_toss () {
    ...
    // if we're the host, and we're on a standalone, tell everyone to "toss"
    // stats Kazan - or how about we don't - we tell them we stored them
    if ((Net_player->flags & NETINFO_FLAG_GAME_HOST) ||
        (Net_player->flags & NETINFO_FLAG_AM_MASTER)) {
        // send a packet to the players telling them to store their stats
        send_store_stats_packet (1);
...
void multi_debrief_server_process () {
    ...
    // if we're > 10 seconds into the debrief and not everyone is here, try
    // warping everyone out again
    if ((Multi_debrief_time >= Multi_debrief_resend_time) &&
        !multi_netplayer_state_check3 (
            NETPLAYER_STATE_DEBRIEF, NETPLAYER_STATE_DEBRIEF_ACCEPT,
            NETPLAYER_STATE_DEBRIEF_REPLAY, 1)) {
        // find all players who are not in the debrief state and hit them with
        // the endgame packet
        for (idx = 0; idx < MAX_PLAYERS; idx++) {
            if (MULTI_CONNECTED (Net_players[idx]) &&
                (Net_player != &Net_players[idx]) &&
    ...
    // evaluate the status of all players in the game (0 == not ready, 1 ==
    // ready to continue, 2 == ready to replay)
    other_status = 1;
    ...
    if (multi_netplayer_state_check3 (
            NETPLAYER_STATE_DEBRIEF, NETPLAYER_STATE_DEBRIEF_ACCEPT,
            NETPLAYER_STATE_DEBRIEF_REPLAY) &&
        (Netgame.type_flags & NG_TYPE_TEAM) && !Multi_debrief_reported_tvt &&
    ...
    // otherwise process as normal (looking for all players to be ready to go
    // to the next mission
    else {
        if (player_status == 1) {
...
// password area defs
int Mpwd_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                             134, 112, 367, 15 },
                                           { // GR_1024
                                             215, 190, 587, 24 } };
...
void multi_passwd_init () {
    ...
    // create the interface window
    Multi_pwd_window.create (
        0, 0, gr_screen.max_w_unscaled, gr_screen.max_h_unscaled, 0);
    Multi_pwd_window.set_mask_bmap (
        Multi_pwd_bitmap_mask_fname[gr_screen.res]);
    ...
    for (idx = 0; idx < MULTI_PWD_NUM_BUTTONS; idx++) {
        // create the object
        Multi_pwd_buttons[gr_screen.res][idx].button.create (
            &Multi_pwd_window, "", Multi_pwd_buttons[gr_screen.res][idx].x,
            Multi_pwd_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);

        // set the sound to play when highlighted
        Multi_pwd_buttons[gr_screen.res][idx].button.set_highlight_action (
            common_play_highlight_sound);

        // set the ani for the button
        Multi_pwd_buttons[gr_screen.res][idx].button.set_bmaps (
            Multi_pwd_buttons[gr_screen.res][idx].filename);

        // set the hotspot
        Multi_pwd_buttons[gr_screen.res][idx].button.link_hotspot (
            Multi_pwd_buttons[gr_screen.res][idx].hotspot);
    }
    ...
    Multi_pwd_passwd.create (
        &Multi_pwd_window, Mpwd_coords[gr_screen.res][MPWD_X_COORD],
        Mpwd_coords[gr_screen.res][MPWD_Y_COORD],
        Mpwd_coords[gr_screen.res][MPWD_W_COORD], MAX_PASSWD_LEN, "",
        UI_INPUTBOX_FLAG_ESC_CLR | UI_INPUTBOX_FLAG_INVIS, -1, &Color_normal);
    Multi_pwd_passwd.set_focus ();

    // link the enter key to ACCEPT
    Multi_pwd_buttons[gr_screen.res][MPWD_COMMIT].button.set_hotkey (
        KEY_ENTER);
...
void multi_passwd_do (char* passwd) {
    ...
    while (Multi_passwd_done == -1) {
        ...
        switch (k) {
        case KEY_ESC:
            // set this to indicate the user has cancelled for one reason or
            // another
            Multi_passwd_done = 0;
            break;
        }
...
// bring up the password string popup, fill in passwd (return 1 if accept was
// pressed, 0 if cancel was pressed)
int multi_passwd_popup (char* passwd) {
    // if the popup is already running for some reason, don't do anything
    if (Multi_passwd_running) { return 0; }
