...
// to join a game - your LOCAL.MULTI_FS_SERVER_COMPATIBLE_VERSION must be >=
// GAME_SERVER.MULTI_FS_SERVER_VERSION Version #. Please put the date down when
// you up these values NOTE: always keep SERVER_VERSION and
// SERVER_COMPATIBLE_VERSION the same
//
// version 32 - 1/29/99
// version 33 - 2/22/99
// version 34 - 4/8/99
...
// revert  46 - 9/7/2006 (the 47 bump wasn't needed, reverting to retail
// version for compatibility reasons) version 48 - 8/15/2016 Multiple changes
// to the packet format for multi sexps STANDALONE_ONLY
...
// the total max # of connections (players + observers + (possibly)standalone
// server)
...
#define DESCRIPT_LENGTH \
    512 // maximum length of a mission description (as specified by Fred)
...
#define RESPAWN_INVUL_TIMESTAMP \
    5000 // how long a player is invulnerable after he respawns
#define MAX_RESPAWN_POINTS \
    25 // the max # of respawn points we'll keep track of for any mission

// player information defines
#define BUTTON_INFO_SAVE_COUNT \
    30 // how many buttons infos we keep track of for sending critical
       // keypresses to the server
#define MAX_PINGS \
    10 // how many pings we keep track of for averaging player pings
...
#define MULTIPLAYER_STANDALONE                          \
    ((Game_mode & GM_MULTIPLAYER) &&                    \
     (Net_players[0].flags & NETINFO_FLAG_AM_MASTER) && \
     !(Net_players[0].flags & NETINFO_FLAG_GAME_HOST))
...
#define MULTI_STANDALONE(np)                \
    ((np.flags & NETINFO_FLAG_AM_MASTER) && \
     !(np.flags & NETINFO_FLAG_GAME_HOST))
#define MULTI_OBSERVER(np) (np.flags & NETINFO_FLAG_OBSERVER)
#define MULTI_TEMP_OBSERVER(np)            \
    ((np.flags & NETINFO_FLAG_OBSERVER) && \
     (np.flags & NETINFO_FLAG_OBS_PLAYER))
...
#define NETPLAYER_SLOTS_P \
    0x01 // data telling clients what player has what object number ...
#define FIRING_INFO 0x02 // firing info packet
#define INGAME_SHIP_UPDATE \
    0x03 // ship status-like update for ingame joiners choosing ships.
#define INGAME_SHIP_REQUEST \
    0x04 // used for requesting a replying (confirm or no) ships for an ingame
         // joiner
#define TEAM_SELECT_UPDATE \
    0x05 // update from host to players in team select regarding who has what
         // ship
#define FILE_SIG_INFO \
    0x06 // file signature info ( as calculated by get_bigass_file_signature()
         // )
#define RESPAWN_NOTICE 0x07 // from client to server, or server to client
#define LOAD_MISSION_NOW \
    0x08 // clients should load the mission and return an ack
#define FILE_SIG_REQUEST 0x09 // request from server to client for filesig info
#define JUMP_INTO_GAME \
    0x0A // from server to client telling him to jump into the mission
#define RESPAWN_POINTS \
    0x0B // from server to ingame joiners, giving respawn data
#define CLIENT_REPAIR_INFO \
    0x0C // passing various data to clients indicating their repair status
...
#define MISSION_SYNC_DATA \
    0x11 // this is a unique packet from the host to the server in a standalone
         // game
#define STORE_MISSION_STATS \
    0x12 // sent to client indicating the host has hit "accept" and they should
         // update their alltime stats
#define DEBRIS_UPDATE \
    0x13 // debris position, velocity, orientation, etc update
#define SHIP_WSTATE_CHANGE \
    0x14 // used to tell clients that a ship's primary/secondary state changed
#define WSS_REQUEST_PACKET \
    0x15 // from client to server, requesting to do an operation
#define WSS_UPDATE_PACKET 0x16 // from server to client, given any update
#define KICK_PLAYER \
    0x17 // kick a specific player (sent to server by those who are allowed to
         // do this)
...
#define NETPLAYER_PAIN \
    0x1A // to notify the player of hits which he may not otherwise see
...
#define POST_SYNC_DATA \
    0x20 // a very large packet containing all the data players will need
         // before going into the mission itself
#define PLAYER_SETTINGS 0x21 // player settings for each individual net player
#define WSS_SLOTS_DATA \
    0x22 // all weapon slots information for the starting wings (needs to be
         // synched)
#define PLAYER_STATS 0x23 // stats for a given player
#define SLOT_UPDATE \
    0x24 // an player slot position update in multiplayer ship/team select
#define TEAM_UPDATE \
    0x25 // used for performing misc operations on pregame interface screens
         // for team vs. team games
#define INGAME_EXTRA 0x26 // extra data for ingame joiners
#define HOST_RESTR_QUERY \
    0x27 // query the host when a player joins a restricted game
#define OPTIONS_UPDATE 0x28 // options (netgame or local) update packet
#define CLIENT_UPDATE \
    0x29 // sent from server to client periodically to update important info
         // (pause status, etc)
...
#define SHIP_STATUS_CHANGE \
    0xA4 // any of the relevant ship status buttons have been hit (need ack
         // from server)
#define PLAYER_ORDER_PACKET \
    0xA5 // ship and wing commands sent from client to server
...
#define HOMING_WEAPON_UPDATE \
    0xA9 // update homing object and subsystem for homing missile
...
#define MULTI_PAUSE_REQUEST \
    0xB8 // send a request to the server to pause or unpause the game
...
#define SW_STD_QUERY \
    0xBD // query from host to standalone server to query PXO about a squad war
         // match
...
#define MISSION_END \
    0xC8 // sent from host(master) to clients indicating they should go to the
         // debriefing
#define INGAME_NAK \
    0xC9 // opposite of INGAME_ACK. Uses the ACK_* defines as well.
#define OBSERVER_UPDATE \
    0xCA // sent from observers to server to give position and orientation
#define SQUADMSG_PLAYER \
    0xCB // a squadmate message command has been sent to a netplayer
#define OBJ_UPDATE_SYNC 0xCC // object update timebase syncing code
#define FORCE_MISSION_END \
    0xCD // Force all the players to skip straight to the debrief screen
...
#define NETGAME_END_ERROR \
    0xD9 // the netgame has been ended by the server because of an error
         // (passed error code)
#define COUNTERMEASURE_SUCCESS \
    0xDA // countermeasure was successful for this player.
...
#define OBJECT_UPDATE \
    0xE6 // an object update packet from server to all clients
...
#define SUBSYS_CARGO_HIDDEN \
    0xED // Capital ship cargo subsystem is hidden for both ships and
         // subsystems

#define VARIABLE_UPDATE \
    0xF0 // Karajorma - a variable update packet from server to all clients
#define SEXP \
    0xF1 // Karajorma - a general packet which can be used to update clients
         // with changes caused by SEXPs

#define MAX_TYPE_ID 0xFF // better not try to send > 255 in a single byte buddy

// ingame ack data codes
#define ACK_RESPAWN_POINTS \
    0x1 // from ingame joiner to server, indicating he got the respawn points
        // packet
#define ACK_FILE_ACCEPTED 0x2 // server to client saying their file is valid
#define ACK_FILE_REJECTED \
    0x3 // server to client saying their file is not valid

// join request denial codes
#define JOIN_DENY_JR_STATE \
    0 // join request is rejected because the game is not in the proper state
#define JOIN_DENY_JR_TRACKER_INVAL \
    1 // join request is rejected because the game is an MT game and the passed
      // player info is invalid
#define JOIN_DENY_JR_PASSWD \
    2 // join request is rejected because the game is password protected and
      // the password sent is incorrect
#define JOIN_DENY_JR_CLOSED \
    3 // join request is rejected because the game is closed and is currently
      // ingame
#define JOIN_DENY_JR_RANK_HIGH \
    4 // join request is rejected because the game is rank based and the passed
      // rank is too high
#define JOIN_DENY_JR_RANK_LOW \
    5 // join request is rejected because the game is rank based and the passed
      // rank is too low
#define JOIN_DENY_JR_DUP \
    6 // join request is denied because there is an exiting netplayer with
      // matching characteristics
#define JOIN_DENY_JR_FULL 7 // join request is denied because the game is full
#define JOIN_DENY_JR_TEMP_CLOSED \
    8 // join request is denied because the _forming_ netgame has been toggled
      // closed
#define JOIN_DENY_JR_BANNED \
    9 // join request is denied because the player has been banned for the
      // duration of the game
#define JOIN_DENY_JR_NOOBS \
    10 // join request is denied because observers are not allowed
#define JOIN_DENY_JR_INGAME_JOIN \
    11 // join request is denied because someone else is already ingame joining
#define JOIN_DENY_JR_BAD_VERSION 12 // incompatible version types
#define JOIN_QUERY_RESTRICTED \
    13 // poll the host of the game to see if he accepts this player
...
#define REPAIR_INFO_UPDATE \
    0x3 // server to client - here's some repair update info (not currently
        // used)
#define REPAIR_INFO_QUEUE \
    0x4 // server to client - client is queued for rearming.
#define REPAIR_INFO_ABORT \
    0x5 // server to client - client has aborted a rearm/repair
#define REPAIR_INFO_BROKEN \
    0x6 // server to client - client is breaking repair -- might be
        // reestablished
#define REPAIR_INFO_WARP_ADD \
    0x7 // server to client - add client onto list of people for arriving
        // support ship
#define REPAIR_INFO_WARP_REMOVE \
    0x8 // server to client - remove client from list of people for arriving
        // support ship
#define REPAIR_INFO_ONWAY 0x9 // server to client - repair ship on way
#define REPAIR_INFO_KILLED \
    0xa // server to client - repair ship was killed on way to rearm player
#define REPAIR_INFO_COMPLETE \
    0xb // server to client - repair of your ship is complete
...
#define ACCEPT_CLIENT \
    (1 << 3) // accept the player as a normal, non ingame join, non host player
...
#define INGAME_SR_REQUEST \
    0x1 // request for the ship with the given net signature
#define INGAME_SR_CONFIRM \
    0x2 // confirmation to the client that he can use the requested ship
#define INGAME_SR_DENY \
    0x3 // deny the request the ingame joiner made for the ship
#define INGAME_PLAYER_CHOICE \
    0x4 // sent to other players informing them of ingame joiners choice
...
#define AI_UPDATE_ORDERS \
    0x3 // server tells clients about new AI order for the ship
...
#define STATS_MISSION \
    0 // all stats for the mission (except kills), for one player
...
#define STATS_DOGFIGHT_KILLS \
    3 // same as mission kills, but also sends per-player kills
...
#define MAX_SHIPS_PER_PACKET \
    64 // Number of ships in a STATS_MISSION_KILLS or STATS_ALLTIME_KILLS
       // packet
...
struct net_player_server_info  {
    ...
    int invul_timestamp;   // invulnerability flag timestamp (for respawning
                           // after dying)
    button_info
        last_buttons[BUTTON_INFO_SAVE_COUNT];    // button info for sending
                                                 // critical control packets to
                                                 // the server
    int last_buttons_id[BUTTON_INFO_SAVE_COUNT]; //
    fix last_buttons_time[BUTTON_INFO_SAVE_COUNT]; //
    int num_last_buttons;                          //
    fix last_full_update_time; // time when server last updated this player
                               // position/orientation
    int xfer_handle; // handle to the file xfer handle (-1 if no file xfer is
                     // taking place)
    int kick_timestamp; // timestamp with which we'll disconnect a player if he
                        // hasn't reponded to a kick packet
    int kick_reason;    // reason he was kicked
    int voice_token_timestamp; // timestamp set when a player loses a token (so
                               // we can prevent him from getting it again too
                               // quickly)
    int reliable_connect_time; // after sending an accept packet, wait for this
                               // long for the guy to connect on the reliable
                               // socket

    // weapon select/linking information (maintained on the server and passed
    // on respawn to all clients)
    char cur_primary_bank;   // currently selected primary bank
    char cur_secondary_bank; // currently selected secondary bank
    ubyte cur_link_status;   // if (1<<0) - primaries linked. if (1<<1) -
                             // secondaries are linked
    ...
    // tracker information
    int tracker_security_last; // this is the value returned when getting
                               // tracker data. it must be used when "sending"
                               // tracker data
    ...
    // firing info (1<<0) for primary fire, (1<<1) for secondary fired, (1<<2)
    // for countermeasure fired, (1<<3) for afterburner on basically, we set
    // these bits if necessary between control info sends from the client. once
    // sent, these values are cleared until the next send time
    ubyte accum_buttons;

    // buffered packet info
    ubyte unreliable_buffer[MAX_PACKET_SIZE]; // buffer used to buffer
                                              // unreliable packets before
                                              // sending as a single UDP packet
    int unreliable_buffer_size; // length (in bytes) of data in unreliable
                                // send_buffer
    ubyte reliable_buffer[MAX_PACKET_SIZE]; // buffer used to buffer reliable
                                            // packets before sending as a
                                            // single UDP packet
...
struct net_player_info  {
    ...
    int ship_index; // index into the ship choices in team select/ship select
                    // (out of 12 choices)
...
struct netgame_info  {
    ...
    char title[NAME_LENGTH + 1]; // title of the mission (as appears in the
                                 // mission file)
    ...
    ushort server_update_seq;    // the current object update reference count
                                 // (server-side only)
    int server_update_frame_ref; // used to determine when we should increment
                                 // the server_update_seq value
...
#define AG_FLAG_CONNECTION_SPEED_MASK \
    ((1 << 12) | (1 << 13) | (1 << 14)) // mask for the connection speed

#define AG_FLAG_VALID_MISSION \
    (1 << 15) // the mission is a "valid" tracker mission

#define AG_FLAG_CONNECTION_BIT \
    12 // number of bits to shift right or left to get speed
...
#define NETINFO_FLAG_MT_CONNECTED \
    (1 << 2) // if everything is hunky dory with the tracker connection
#define NETINFO_FLAG_MT_FAILED (1 << 3) // all attempts to connect have failed
#define NETINFO_FLAG_MT_STARTUP \
    (1 << 4) // the initial state (ie, we haven't tried anything yet)
#define NETINFO_FLAG_GAME_HOST (1 << 5) // I'm the host
#define NETINFO_FLAG_INGAME_JOIN \
    (1 << 6) // means he is still in the process of joining ingame
#define NETINFO_FLAG_OBSERVER (1 << 7) // means he's an observer
#define NETINFO_FLAG_OBS_PLAYER \
    (1 << 8) // means he's an observer, but he was formerly a player (should
             // show his stats)
#define NETINFO_FLAG_LIMBO \
    (1 << 9) // (client side) means he has to choose whether to be an observer
             // or quit (no more respawns)
#define NETINFO_FLAG_MISSION_OK \
    (1 << 10) // this client's mission has been verified
#define NETINFO_FLAG_RESPAWNING \
    (1 << 11) // so that we wait on a keypress, or other user input before
              // respawning
#define NETINFO_FLAG_DO_NETWORKING \
    (1 << 12) // set when we can send/receive data
#define NETINFO_FLAG_TEAM_LOCKED \
    (1 << 13) // if this is set, only the host can modify the team settings for
              // this player
#define NETINFO_FLAG_TEAM_CAPTAIN \
    (1 << 14)                         // this player is the captain of his team
...
#define NETINFO_FLAG_WARPING_OUT \
    (1 << 20) // clients keep track of this for themselves to know if they
              // should be leaving
#define NETINFO_FLAG_HAS_CD (1 << 21) // the player has a CD in the drive
#define NETINFO_FLAG_RELIABLE_CONNECTED \
    (1 << 22) // reliable socket is now active
#define NETINFO_FLAG_MT_GET_FAILED \
    (1 << 23) // set during MT stats update process indicating we didn't
              // properly get his stats
#define NETINFO_FLAG_MT_SEND_FAILED \
    (1 << 24) // set during MT stats update process indicating we didn't
              // properly send his stats
#define NETINFO_FLAG_MT_DONE \
    (1 << 25) // set when a player has been processed for stats (fail, succeed,
              // or otherwise)
#define NETINFO_FLAG_HAXOR \
    (1 << 26) // the player has some form of hacked client data

#define NETPLAYER_IS_OBSERVER(player) \
    (player->flags & (NETINFO_FLAG_OBSERVER | NETINFO_FLAG_OBS_PLAYER))
#define NETPLAYER_IS_DEAD(player) \
    (player->flags & (NETINFO_FLAG_LIMBO | NETINFO_FLAG_RESPAWNING))
...
#define NG_FLAG_TEMP_CLOSED \
    (1 << 0) // a forming netgame is temporarily closed (should not be checked
             // otherwise)
#define NG_FLAG_SERVER_LOST \
    (1 << 1) // client has temporarily lost contact with the server
#define NG_FLAG_INGAME_JOINING (1 << 2) // someone is ingame joining.
#define NG_FLAG_INGAME_JOINING_CRITICAL \
    (1 << 3) // someone is ingame joining and at the critical point where we
             // cannot do certain things.
#define NG_FLAG_STORED_MT_STATS \
    (1 << 4) // stored tracker stats in the debriefing already
#define NG_FLAG_HACKED_SHIPS_TBL \
    (1 << 5) // set when the server is playing with a hacked ships.tbl (only
             // needed to notify hosts playing on a standalone)
#define NG_FLAG_HACKED_WEAPONS_TBL \
    (1 << 6) // set when the server is playing with a hacked weapons.tbl (only
             // needed to notify hosts playing on a standalone)
...
#define MULTI_COOP \
    ((Game_mode & GM_MULTIPLAYER) && (Netgame.type_flags & NG_TYPE_COOP))
#define MULTI_TEAM \
...
#define NETGAME_STATE_FORMING \
    1 // players are joining, host is selecting missions, etc
...
#define NETGAME_STATE_SERVER_TRANSFER \
    4 // server status is being transferred from one computer to another
...
#define NETGAME_STATE_MISSION_SYNC \
    7 // client/server data sync screens before and after the briefing stage
#define NETGAME_STATE_ENDGAME \
    8 // game is moving from gameplay to the debriefing state
#define NETGAME_STATE_STD_HOST_SETUP \
    9 // the host is on the setup netgame screen for the standalone server
#define NETGAME_STATE_HOST_SETUP \
    10 // the host is on the setup netgame screen _non_ standalone

// state defines for netplayer states
#define NETPLAYER_STATE_JOINING 0 // joining the netgame
#define NETPLAYER_STATE_JOINED \
    1 // has joined and opened a reliable socket connection
#define NETPLAYER_STATE_MISSION_LOADING \
    2 // in the process of loading the mission
...
#define NETPLAYER_STATE_WAITING \
    8 // waiting to do something (like enter the mission)
...
#define NETPLAYER_STATE_INGAME_SHIPS \
    11 // player is receiving ingame join ship data
#define NETPLAYER_STATE_INGAME_WINGS \
    12 // player is receiving ingame join wing data
#define NETPLAYER_STATE_INGAME_RPTS \
    13 // player is receiving ingame join respawn data
#define NETPLAYER_STATE_INGAME_SHIP_SELECT \
    14 // player is in the ship select screen for ingame join
...
#define NETPLAYER_STATE_STD_HOST_SETUP \
    17 // the host is on the setup netgame screen for the standalone server
#define NETPLAYER_STATE_HOST_SETUP \
    18 // the host is on the setup netgame screen _non_ standalone
#define NETPLAYER_STATE_SETTINGS_ACK \
    19 // the player has received the player settings data for all players
#define NETPLAYER_STATE_SLOTS_ACK \
    20 // the player has received wss slots data (ingame join only)
#define NETPLAYER_STATE_POST_DATA_ACK \
    21 // the player has received the post briefing data block
#define NETPLAYER_STATE_WSS_ACK 22 // have received weapon slot information
#define NETPLAYER_STATE_FLAG_ACK \
    23 // the player has received his flag change information
#define NETPLAYER_STATE_MT_STATS \
    24 // the server is in the process of requesting or updating stats from the
       // MT
#define NETPLAYER_STATE_MISSION_XFER \
    25 // the player is in the process of receiving a mission file
#define NETPLAYER_STATE_INGAME_STUFF \
    26 // received ingame "stuff"  happens just before ship selection
#define NETPLAYER_STATE_DEBRIEF_ACCEPT \
    27 // the player has hit the accept button in the debrief and is good to go
#define NETPLAYER_STATE_DEBRIEF_REPLAY \
    28 // set on the host instead of NETPLAYER_STATE_DEBRIEF_ACCEPT to indicate
       // he wants to replay the mission
#define NETPLAYER_STATE_CPOOL_ACK \
    29 // player has acked all campaign pool status data
#define NETPLAYER_STATE_INGAME_CINFO \
    30 // player has received campaign information (ingame join only)
...
// use this to check and see whether a netgame is anywhere in mission (paused,
// etc, etc)
...
// netplayer vars
extern net_player
    Net_players[MAX_PLAYERS];  // array of all netplayers in the game
...
extern ushort
    Next_ship_signature; // next network signature to assign to an object
...
extern int Multi_mission_loaded;     // flag, so that we don't load the mission
                                     // more than once client side
extern ushort Multi_ingame_join_sig; // signature for the player obj for use
                                     // when joining ingame
extern int Multi_button_info_ok;     // flag saying it is ok to apply critical
                                     // button info on a client machine
extern int Multi_button_info_id; // identifier of the stored button info to be
                                 // applying
...
// misc data
extern active_game*
    Active_game_head; // linked list of active games displayed on Join screen
extern int Active_game_count; // for interface screens as well
extern CFILE*
    Multi_chat_stream;   // for streaming multiplayer chat strings to a file
extern int Multi_has_cd; // if this machine has a cd or not (call
                         // multi_common_verify_cd() to set this)
extern int Multi_num_players_at_start; // the # of players present (kept track
                                       // of only on the server) at the very
                                       // start of the mission
extern short Multi_id_num;             // for assigning player id #'s

// permanent server list
extern server_item*
    Game_server_head; // list of permanent game servers to be querying
...
#define MULTI_JOIN_RESTR_MODE_2 \
    1 // mode 2 - team vs. team, only team 0 has ships
#define MULTI_JOIN_RESTR_MODE_3 \
    2 // mode 3 - team vs. team, only team 1 has ships
#define MULTI_JOIN_RESTR_MODE_4 \
    3 // mode 4 - team vs. team, both teams have ships

extern int
    Multi_restr_query_timestamp; // timestamp for querying the host to see if
                                 // he will allow a new player to join ingame
...
// reset all standalone stuff, including gui, networking, netgame, etc, and go
// into the main state
void multi_standalone_reset_all ();

// init for the wait mode of the standalone (when waiting for players to finish
// the briefing)
void multi_standalone_wait_init ();

// do frame for the standalone wait state (when waiting for players to finish
// the briefing)
void multi_standalone_wait_do ();

// close for the standalone wait state (when waiting for players to finish the
// briefing)
...
// do frame for the standalone postgame state (when players are in the
// debriefing)
