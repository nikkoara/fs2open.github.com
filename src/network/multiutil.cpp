...
ushort Next_ship_signature; // next permanent network signature to assign to an
                            // object
ushort Next_asteroid_signature; // next signature for an asteroid
ushort Next_non_perm_signature; // next non-permanent network signature to
                                // assign to an object
ushort Next_debris_signature;   // next debris signature

// if a client doesn't receive an update for an object after this many seconds,
// query server as to the objects status.
#define MULTI_CLIENT_OBJ_TIMEOUT 10
#define MAX_SHIPS_PER_QUERY 10

// this function assignes the given object with the given signature.  If the
// signature is 0, then we choose the next signature number from the correct
// pool.  I thought that it might be desireable to not always have to take the
// next signature on the list.  what_kind is used to assign either a permanent
// or non-permanent signature to an object.  permanent signatures are used for
// ships, non_permanent signatures are used for everything else.
ushort multi_assign_network_signature (int what_kind) {
    ushort sig;

    // do limit checking on the permanent and non_permanent signatures.  Ships
    // are considered "permanent" as are debris and asteroids since they don't
    // die very often.  It would be vary rare for this value (the permanent
    // signature) to wrap.  For now, this condition is an error condition
    if (what_kind == MULTI_SIG_SHIP) {
        ...
        if ((Next_non_perm_signature < NPERM_SIG_MIN) ||
            (Next_non_perm_signature == NPERM_SIG_MAX)) {
            Next_non_perm_signature = NPERM_SIG_MIN;
        }
    }
    else {
...
// this function returns the next network signature that will be used for a
// newly created object what_kind parameter tells us what kind of signature to
// get -- permanent or non-permanent
...
// this routine sets the network signature to the given value.  Should be
...
// multi_get_network_object() takes a net_signature and tries to locate the
// object in the object list with that network signature.  Returns NULL if the
// object cannot be found
object* multi_get_network_object (ushort net_signature) {
    object* objp;

    if (net_signature == 0) return NULL;

    if (GET_FIRST (&obj_used_list) == NULL) return NULL;

    for (objp = GET_FIRST (&obj_used_list);
         objp != END_OF_LIST (&obj_used_list); objp = GET_NEXT (objp))
        if (objp->net_signature == net_signature) break;
    ...
    if (objp == END_OF_LIST (&obj_used_list)) {
        ...
        if (objp == END_OF_LIST (&obj_create_list)) objp = NULL;
    }
...
const char* multi_random_death_word () {
    ...
    switch (index) {
    case 0: return XSTR ("zapped", 853);
    case 1: return XSTR ("caulked", 854);
    case 2: return XSTR ("killed", 855);
    case 3: return XSTR ("waxed", 856);
    case 4: return XSTR ("popped", 857);
    case 5: return XSTR ("murdered", 858);
    case 6: return XSTR ("bludgeoned", 859);
    case 7: return XSTR ("destroyed", 860);
    case 8: return XSTR ("iced", 861);
    case 9: return XSTR ("obliterated", 862);
    case 10: return XSTR ("toasted", 863);
    case 11: return XSTR ("roasted", 864);
    case 12: return XSTR ("turned into anti-matter", 865);
    case 13: return XSTR ("killed like a pig", 866);
    case 14: return XSTR ("taught a lesson", 867);
    case 15: return XSTR ("slaughtered with impunity", 868);
    case 16: return XSTR ("spanked like a naughty boy", 869);
    case 17: return XSTR ("skunked", 870);
    case 18: return XSTR ("beat senseless", 871);
    case 19: return XSTR ("shot up", 872);
    case 20: return XSTR ("spaced", 873);
    case 21: return XSTR ("hosed", 874);
    case 22: return XSTR ("capped", 875);
    case 23: return XSTR ("beat down", 876);
    case 24: return XSTR ("hit wit da shizzo", 877);
    case 25: return XSTR ("sk00led", 878);
    case 26: return XSTR ("whooped up", 879);
    case 27:
    ...
    case 29: return XSTR ("gibbed by Kayser Sozay's rocket", 882);
    case 30: return XSTR ("shot down", 883);
    case 31: return XSTR ("given early retirement", 884);
    case 32: return XSTR ("instructed", 885);
    case 33: return XSTR ("eviscerated", 886);
    case 34: return XSTR ("pummelled", 887);
    case 35: return XSTR ("eradicated", 888);
    case 36: return XSTR ("cleansed", 889);
    case 37: return XSTR ("perforated", 890);
    case 38: return XSTR ("canned", 891);
    case 39: return XSTR ("decompressed", 892);
    }
...
#define MAX_CHAT_PHRASE_LEN \
    25 // be careful not to exceed (or update if exceeded)
...
const char* multi_random_chat_start () {
    ...
    switch (index) {
    case 0: return XSTR ("says", 893);
    case 1: return XSTR ("bleats", 894);
    case 2: return XSTR ("opines", 895);
    case 3: return XSTR ("postulates", 896);
    case 4: return XSTR ("declares", 897);
    case 5: return XSTR ("vomits out", 898);
    case 6: return XSTR ("whines", 899);
    case 7: return XSTR ("barks", 900);
    }
...
int find_player (net_addr* addr) {
    ...
    for (i = 0; i < MAX_PLAYERS; i++) {
        ...
        if (psnet_same (addr, &(Net_players[i].p_info.addr))) { return i; }
    }
...
int find_player_no_port (net_addr* addr) {
    ...
    for (i = 0; i < MAX_PLAYERS; i++) {
        ...
        if (memcmp (
                &addr->addr, &Net_players[i].p_info.addr.addr,
                IP_ADDRESS_LENGTH) == 0) {
            return i;
        }
    }
...
int find_player_id (short player_id) {
    ...
    for (i = 0; i < MAX_PLAYERS; i++) {
        if (!MULTI_CONNECTED (Net_players[i])) { continue; }
        if (player_id == Net_players[i].player_id) { return i; }
    }
...
int find_player_socket (PSNET_SOCKET_RELIABLE sock) {
    ...
    for (i = 0; i < MAX_PLAYERS; i++) {
        if (!MULTI_CONNECTED (Net_players[i])) { continue; }
        if (sock == Net_players[i].reliable_socket) { return i; }
    }
...
// multi_find_player_by_object returns a player num (reference by
// Net_players[x]) when given a object *. used to find netplayers in game when
// only the object is known
int multi_find_player_by_object (object* objp) {
    ...
    for (i = 0; i < MAX_PLAYERS; i++) {
        if (!MULTI_CONNECTED (Net_players[i])) { continue; }
        if (objnum == Net_players[i].m_player->objnum) { return i; }
    }

    // return -1 if the object is not found -- this is a bad situation, but we
    // will handle it in higher level code
    return -1;
}
...
int multi_find_player_by_signature (int signature) {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        ...
        if (MULTI_CONNECTED (Net_players[idx]) &&
            (Objects[Net_players[idx].m_player->objnum].signature ==
             signature)) {
            // found the player
            return idx;
        }
...
// returns a player num based upon it's parse_object unlike the above functions
// it can be used on respawning players
int multi_find_player_by_parse_object (p_object* p_objp) {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        // compare against each player's object signature
        if (MULTI_CONNECTED (Net_players[idx]) &&
            (Net_players[idx].p_info.p_objp == p_objp)) {
            // found the player
            return idx;
        }
    }
...
int multi_find_player_by_ship_name (
    const char* ship_name, bool inc_respawning) {
    int idx;
    p_object* p_objp;

    // bogus
    if (ship_name == NULL) { return -1; }
    ...
    if (inc_respawning) {
        ...
        if ((idx >= 0) && (idx < MAX_PLAYERS)) { return idx; }
    }
    // didn't find the player
    return -1;
}
...
int multi_get_player_ship (int np_index) {
    // bogus
    if ((np_index < 0) || (np_index >= MAX_PLAYERS)) { return -1; }

    // cool?
    if (MULTI_CONNECTED (Net_players[np_index]) &&
        !MULTI_OBSERVER (Net_players[np_index]) &&
        !MULTI_STANDALONE (Net_players[np_index]) &&
        (Net_players[np_index].m_player != NULL) &&
        (Net_players[np_index].m_player->objnum >= 0) &&
        (Net_players[np_index].m_player->objnum < MAX_OBJECTS) &&
        (Objects[Net_players[np_index].m_player->objnum].type == OBJ_SHIP) &&
        (Objects[Net_players[np_index].m_player->objnum].instance >= 0) &&
...
// find_open_netplayer_slot() attempts to find a free slot in the Net_players
// structure for a new player. it returns -1 if there is no new slot, and the
// slot number if one is found NOTE : this attempts to preserve a player object
// as long as possible for him to come back to
...
// find_open_player_slot() attempts to find an open player array slot.  The 0th
// element of the array should always be taken by the console player's pilot.
// All other slots are up for grab though.
int multi_find_open_player_slot () {
    int i;

    for (i = 0; i < MAX_PLAYERS; i++)
        if (!(Players[i].flags & PLAYER_FLAGS_STRUCTURE_IN_USE)) break;
...
// stuff_netplayer_info stuffs information into the given Net_player structure.
// It is called when a new person is entering the game.  The state of the
// Net_player is set to it's basic starting state
void stuff_netplayer_info (
    net_player* nplayer, net_addr* addr, int ship_class, player* pplayer) {
    ...
    nplayer->p_info.options.obj_update_level =
        Cmdline_objupd; // Set the update to the specified value

    // if setting up my net flags, then set the flag to say I can do
    // networking.
...
// multi_assign_player_ship takes a Net_player index and an object * and
// assigned that player to that object
void multi_assign_player_ship (
    int net_player_num, object* objp, int ship_class) {
    ship* shipp;
    int idx;
    ...
    // check to see if we are assigning my player -- if so, then set
    // Player_ship and Player_ai
    ...
    // find the parse object for this ship.  Also, set the wingman status stuff
    ...
    // game server and this client need to initialize this information so
    // object updating works properly.
...
int multi_create_player (
    int net_player_num, player* pl, const char* name, net_addr* addr,
    int ship_class, short id) {
    ...
    if (ship_class == -1) {
        ...
        if (player_ship_class ==
            -1) // if ship_class is -1 (as the condition to enter this branch
                // at all means), then player_ship_class is initialized to -1
                // as well.
        {
            if (!Ship_info.empty ()) {
                player_ship_class = 0;
    ...
    stuff_netplayer_info (
        &Net_players[net_player_num], addr, player_ship_class, pl);
    ...
    pilot_set_short_callsign (
        pl, SHORT_CALLSIGN_PIXEL_W); // calculate the short callsign
    pl->flags |= PLAYER_FLAGS_STRUCTURE_IN_USE;
    pl->objnum = -1;
    pl->insignia_texture = -1;

    // if we're the standalone server and this is the first guy to join, mark
    // him as the host and give him all host priveleges
    ...
    if (MULTI_IN_MISSION) { hud_escort_add_player (id); }

    return 1;
}

// next function makes a player object an ai object (also decrementing the
// num_players in the netgame).  It appropiately sets the object flags and
...
void multi_make_player_ai (object* pobj) {
    ...
    obj_set_flags (
        pobj, pobj->flags + Object::Object_Flags::Could_be_player -
                  Object::Object_Flags::Invulnerable);

    // target_objnum must be -1 or else new AI ship will fire on whatever this
    // player had targeted.
    set_target_objnum (&(Ai_info[Ships[pobj->instance].ai_index]), -1);
}

// delete_player takes an index into the Net_players array to delete.  Deletion
// of a player might happen because of the player leaving the game on his own,
// or no net activity from the player after X seconds
void delete_player (int player_num, int kicked_reason) {
    char notify_string[256] = "";
    int idx;

    if (!MULTI_CONNECTED (Net_players[player_num])) { return; }

    // NETLOG
    ml_printf (
        NOX ("Deleting player %s"),
        Net_players[player_num].m_player->callsign);

    psnet_rel_close_socket (
        &(Net_players[player_num]
              .reliable_socket)); // close out the reliable socket

    // if this guy was ingame joining, the remove my netgame flag so others may
    // join
    ...
    Net_players[player_num].flags &=
        ~NETINFO_FLAG_CONNECTED; // person not connected anymore
    Net_players[player_num].m_player->flags &=
        ~(PLAYER_FLAGS_STRUCTURE_IN_USE); // free up his player structure
    ...
    if (Net_players[player_num].flags & NETINFO_FLAG_AM_MASTER) {
        // if the server leaves in the debriefing state, we should still wait
        // until the player selects accept before we quit
            ...
            if ((Game_mode & GM_STANDALONE_SERVER) && MULTI_IN_MISSION) {
                ...
                for (idx = 0; idx < MAX_PLAYERS; idx++) {
                    if (MULTI_CONNECTED (Net_players[idx]) &&
                        ...
                        // send a packet
                        send_host_captain_change_packet (
                            Net_players[idx].player_id, 0);
    ...
    // if we're the server of the game, notify everyone else that this guy has
    // left if he's marked as being kicked, then other players know about it
    // already, so don't send again
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        if (Net_players[player_num].flags & NETINFO_FLAG_KICKED) {
            ...
            multi_kick_get_text (
                &Net_players[player_num],
                Net_players[player_num].s_info.kick_reason, str);
            multi_display_chat_msg (str, player_num, 0);
        }
        else {
            send_leave_game_packet (
                Net_players[player_num].player_id, kicked_reason);
        }
    }

    // if this guy is an observer, we have to make sure we delete his observer
    // object (only if we're the server however)
    if ((Net_player->flags & NETINFO_FLAG_AM_MASTER) &&
        (Net_players[player_num].flags & NETINFO_FLAG_OBSERVER)) {
        if (Net_players[player_num].m_player->objnum != -1) {
            obj_delete (Net_players[player_num]
                            .m_player
                            ->objnum); // maybe change this to set flag instead
        }
    }
    else {
        // otherwise mark it so that he can return to it later if possible
        if ((Net_players[player_num].m_player->objnum >= 0) &&
            (Net_players[player_num].m_player->objnum < MAX_OBJECTS) &&
            (Objects[Net_players[player_num].m_player->objnum].type ==
             OBJ_SHIP) &&
            (Objects[Net_players[player_num].m_player->objnum].instance >=
             0) &&
            (Objects[Net_players[player_num].m_player->objnum].instance <
             MAX_SHIPS)) {
            multi_make_player_ai (
                &Objects[Net_players[player_num].m_player->objnum]);
        }
        else {
    ...
    // if we're in the team select, and we're the host, we have to make sure
    // all team select data is correctly updated MWA 7/28/98.  Don't do this
    // for a standalone server.  Since he doesn't go through the team selection
    // screens, his data structures are not 100% accurate.  Doing so on a
    // standalone resulted in the wrong ships getting marked as
    // COULD_BE_PLAYER.  On the standalone, these flags will get properly set
    // in the multi_make_player_ai code above.
    if ((Netgame.game_state == NETGAME_STATE_BRIEFING) &&
    ...
    if (*Net_players[player_num].m_player->callsign) {
        sprintf (
            notify_string, XSTR ("<%s has left>", 901),
            Net_players[player_num].m_player->callsign);
...
void multi_unpack_orient_matrix (ubyte* data, matrix* m) {
    ...
    m->vec.rvec.xyz.z = fl_sqrt (fl_abs (
        ...
        (m->vec.rvec.xyz.y * m->vec.rvec.xyz.y))); // X
    m->vec.uvec.xyz.z = fl_sqrt (fl_abs (
        1 - (m->vec.uvec.xyz.x * m->vec.uvec.xyz.x) -
        (m->vec.uvec.xyz.y * m->vec.uvec.xyz.y))); // Y
    m->vec.fvec.xyz.z = fl_sqrt (fl_abs (
        1 - (m->vec.rvec.xyz.z * m->vec.rvec.xyz.z) -
        (m->vec.uvec.xyz.z * m->vec.uvec.xyz.z))); // V
    m->vec.fvec.xyz.x = fl_sqrt (fl_abs (
        1 - (m->vec.rvec.xyz.x * m->vec.rvec.xyz.x) -
        (m->vec.uvec.xyz.x * m->vec.uvec.xyz.x))); // Z
    m->vec.fvec.xyz.y = fl_sqrt (fl_abs (
        1 - (m->vec.rvec.xyz.y * m->vec.rvec.xyz.y) -
        (m->vec.uvec.xyz.y * m->vec.uvec.xyz.y))); // W
...
void remove_ship_status_item (net_player* p, int id) {
    ...
    for (idx = id; idx < BUTTON_INFO_SAVE_COUNT - 1; idx++) {
        ...
        p->s_info.last_buttons_time[idx] =
            p->s_info.last_buttons_time[idx + 1];
    }
}
...
void add_net_button_info (net_player* p, button_info* bi, int unique_id) {
    ...
    // if the list is full, put it in the oldest slot since it's probably a
    // lost packet anyway
    if (p->s_info.num_last_buttons < BUTTON_INFO_SAVE_COUNT - 1) {
        ...
        p->s_info.last_buttons_time[p->s_info.num_last_buttons] =
        ...
        for (idx = 0; idx < BUTTON_INFO_SAVE_COUNT; idx++) {
            if ((p->s_info.last_buttons_time[idx] < earliest) ||
                (earliest == 0)) {
...
void multi_apply_ship_status (net_player* p, button_info* bi, int locally) {
    ...
    for (i = 0; i < NUM_BUTTON_FIELDS; i++) {
        if (bi->status[i] == 0) continue;
        // at least one bit is set in the status integer
        for (j = 0; j < 32; j++) {
            // check if the bit is set. If button_function returns 1 (implying
            // the action was taken), then unset the bit
            if (bi->status[i] & (1 << j)) {
                if (locally) {
                    if (button_function_critical (
                            32 * i + j, NULL)) // will apply to this console
                        bi->status[i] &= ~(1 << j);
                }
                else {
                    if (button_function_critical (
                            32 * i + j, p)) // will only apply to a net-player
                        bi->status[i] &= ~(1 << j);
                }
            }
        }
...
void multi_maybe_send_ship_status () {
    ...
    // check timestamp
    if ((Multi_ship_status_stamp < 0) ||
        timestamp_elapsed_safe (
            Multi_ship_status_stamp, MULTI_SHIP_STATUS_TIME * 2)) {
        int should_send = 0;
        for (idx = 0; idx < NUM_BUTTON_FIELDS; idx++) {
            // we have at least something to send
            if (Multi_ship_status_bi.status[idx] != 0) { should_send = 1; }
        }

        // do we have something to send
        if (should_send) {
            // add_net_button_info(Net_player, &Multi_ship_status_bi,
            // Multi_button_info_id);
            send_ship_status_packet (
                Net_player, &Multi_ship_status_bi, Multi_button_info_id++);
        }
...
void multi_subsys_update_all () {
    ...
    for(idx=0;idx<MAX_PLAYERS;idx++){
        if((Net_players[idx].flags & NETINFO_FLAG_CONNECTED) &&
    !psnet_same(&My_addr,&Net_players[idx].addr) && !(Net_players[idx].flags &
    NETINFO_FLAG_OBSERVER)) send_subsys_update_packet(&Net_players[idx]);
    }
    */
}
...
int multi_find_player_by_callsign (const char* callsign) {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            (strcmp (callsign, Net_players[idx].m_player->callsign) == 0)) {
            return idx;
        }
    }
...
int multi_is_builtin_mission () {
    ...
    if (game_find_builtin_mission (name) != NULL) { return 1; }
...
void server_verify_filesig (short player_id, ushort sum_sig, int length_sig) {
    ...
    if ((length_sig != Multi_current_file_length) ||
        (sum_sig != Multi_current_file_checksum)) {
        ok = 0;
    }
    else {
        ok = 1;
    ...
    if (pl->flags & NETINFO_FLAG_INGAME_JOIN) {
        if (!ok) {
            ...
            if (Netgame.options.flags & MSO_FLAG_INGAME_XFER) {
                ...
                pl->s_info.xfer_handle = multi_xfer_send_file (
                    pl->reliable_socket, Netgame.mission_name,
                    CF_TYPE_MISSIONS);
            }
    ...
    // in a normal join situation
        ...
        if (!ok) {
            pl->s_info.xfer_handle = multi_xfer_send_file (
                pl->reliable_socket, Netgame.mission_name, CF_TYPE_MISSIONS);
        }
...
void multi_untag_player_ships () {
    ...
    while (moveup != END_OF_LIST (&Ship_obj_list)) {
        if (Objects[moveup->objnum].flags[Object::Object_Flags::Player_ship]) {
            Objects[moveup->objnum].flags.remove (
                Object::Object_Flags::Player_ship);
            obj_set_flags (
                &Objects[moveup->objnum],
                Objects[moveup->objnum].flags +
...
// check to see if all players other than the local player are in a given
// NETPLAYER_ state this is _EXTREMELY_ useful when doing network sequencing
// with the reliable sockets
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            ...
            if (ignore_standalone &&
                ((Net_players[idx].flags & NETINFO_FLAG_AM_MASTER) &&
                 !(Net_players[idx].flags & NETINFO_FLAG_GAME_HOST))) {
                continue;
            }

            if (Net_players[idx].state != state) { return 0; }
        }
    }
    return 1;
}

int multi_netplayer_state_check2 (
    int state, int state2, int ignore_standalone) {
    int idx;
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            (Net_player->player_id != Net_players[idx].player_id)) {
...
int multi_netplayer_state_check3 (
    int state, int state2, int state3, int ignore_standalone) {
    int idx;
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            (Net_player->player_id != Net_players[idx].player_id)) {
...
int multi_netplayer_flag_check (int flags, int ignore_standalone) {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            ...
            if (!(Net_players[idx].flags & flags)) { return 0; }
        }
    }
    return 1;
}

// function which gets called from psnet_* code to evaluate an error received
// on a reliable socket.  In general, we only want to do drastic measures if
...
void multi_eval_socket_error (PSNET_SOCKET sock, int error) {
    ...
    // real pain. in essence, you can't do it!
    // return;
...
// send a repair info packet with code == code to a player if his object is the
// one being acted upon dest_objp is the player object (or possible player
// object).  source_objp is the object pointer of the repair ship that is the
// source of the action.  code means what is happening -- queueing up for
// repair, getting repaired, aborted, complete, etc.
void multi_maybe_send_repair_info (
    object* dest_objp, object* source_objp, int code) {
...
int multi_is_valid_unknown_packet (ubyte type) {
    return (type == GAME_QUERY) || (type == JOIN) || (type == PING) ||
                   (type == PONG) || (type == GAME_INFO) || (type == ACCEPT) ||
                   (type == GAME_ACTIVE) || (type == INGAME_NAK) ||
                   (type == DENY) || (type == UPDATE_DESCRIPT) ||
                   (type == ACCEPT_PLAYER_DATA)
               ? 1
               : 0;
}
...
void multi_create_standalone_object () {
    ...
    obj_set_flags (
        Player_obj, Player_obj->flags - Object::Object_Flags::Collides);
    ...
    // make ship hidden from sensors so that this observer cannot target it.
    // Observers really have two ships one observer, and one "Player_ship".
    // Observer needs to ignore the Player_ship.
...
int multi_message_should_broadcast (int type) {
    ...
    case MESSAGE_SUPPORT_KILLED: return 1;
    default: return 0;
    }
}
...
active_game* multi_update_active_games (active_game* ag) {
    ...
    // see if we have a game from this address already -- if not, create one.
    // In either case, get a pointer to an active_game structure
    ...
    if ((Multi_options_g.protocol == NET_TCP) &&
        (Net_player->p_info.options.flags & MLO_FLAG_LOCAL_BROADCAST)) {
...
int multi_can_end_mission (net_player* p) {
    ...
    if (p->flags & NETINFO_FLAG_GAME_HOST) { return 1; }
...
int multi_eval_join_request (join_request* jr, net_addr* addr) {
    ...
    char knock_message[256], knock_callsign[CALLSIGN_LEN + 1],
        jr_ip_string[16];
    ...
    // Grab the joiner's callsign as we may need this later if they aren't
    // accepted
    ...
    if ((Netgame.game_state != NETGAME_STATE_FORMING) &&
        ((Netgame.game_state != NETGAME_STATE_IN_MISSION) ||
         !Cmdline_ingamejoin)) {
        // If the game isn't password protected pass it on that someone wants
        // to join
        if (Netgame.mode != NG_MODE_PASSWORD) {
            ...
            case NETGAME_STATE_MISSION_SYNC:
                send_game_chat_packet (
                    &Net_players[MY_NET_PLAYER_NUM], knock_message,
                    MULTI_MSG_ALL, NULL, NULL, 1);
            ...
            case NETGAME_STATE_IN_MISSION:
                if (MULTIPLAYER_STANDALONE) {
                    send_game_chat_packet (
                        &Net_players[MY_NET_PLAYER_NUM], knock_message,
                        MULTI_MSG_TARGET, Netgame.host, NULL, 1);
                }
                else {
                    snd_play (gamesnd_get_game_sound (GameSounds::CUE_VOICE));
                    HUD_sourced_printf (
                        HUD_SOURCE_HIDDEN, "%s", knock_message);
                }
                break;
            }
        }
    ...
    // the standalone has some oddball situations which we must handle
    // seperately
    if (Game_mode & GM_STANDALONE_SERVER) {
        // if this is the first connection, he will be the host so we must
        // always accept him
        if (multi_num_players () == 0) {
            /*
               TODO:  We can use this now, but it's not compatible with older
            builds, so comment it out until the next release

            // check to see if this is a tracker game, and if so make sure this
            is a valid MT player
            // we probably eventually want to make sure he's not passing us a
            fake tracker id# if (MULTI_IS_TRACKER_GAME) { if (jr->tracker_id <
            0) { return JOIN_DENY_JR_TRACKER_INVAL;
                }
            }
            */

            // if we're password protected
            if (std_is_host_passwd () &&
                strcmp (jr->passwd, Multi_options_g.std_passwd) != 0) {
    ...
    // first off check to see if we're violating any of our max
    // players/observers/connections boundaries if we've already got the full
    // 16 (MAX_PLAYERS) connections - yow
    if ((multi_num_connections () >= MULTI_MAX_CONNECTIONS) ||
        // if we're full of observers and this guy wants to be an observer
        ((multi_num_observers () >= MAX_OBSERVERS) &&
         (jr->flags & JOIN_FLAG_AS_OBSERVER)) ||
        // if we're up to MULTI_MAX_PLAYERS-1 and we're on the standalone
        ((multi_num_players () >= (MULTI_MAX_PLAYERS - 1)) &&
         (Game_mode & GM_STANDALONE_SERVER)) ||
        ...
        ((Multi_options_g.std_max_players != -1) &&
         (multi_num_players () >= Multi_options_g.std_max_players))) {
    ...
    // check to see if this is a tracker game, and if so make sure this is a
    valid MT player
    // we probably eventually want to make sure he's not passing us a fake
    tracker id# if (MULTI_IS_TRACKER_GAME) { if (jr->tracker_id < 0){ return
    JOIN_DENY_JR_TRACKER_INVAL;
        }
    }
    */
    ...
    // check to see if the player has passed a valid password in a password
    // protected game
    if ((Netgame.mode == NG_MODE_PASSWORD) &&
        strcmp (Netgame.passwd, jr->passwd) != 0) {
        return JOIN_DENY_JR_PASSWD;
    }

    // check to see if the netgame is forming and is temporarily marked as
    // closed
    if ((Netgame.game_state == NETGAME_STATE_FORMING) &&
    ...
    if ((Netgame.mode == NG_MODE_RANK_ABOVE) &&
        (jr->player_rank < Netgame.rank_base)) {
    ...
    if ((MULTI_IS_TRACKER_GAME && fs2netd_player_banned (addr)) ||
        ...
        sprintf (
            knock_message,
            "Banned user %s with IP: %s attempted to join server",
            knock_callsign, psnet_addr_to_string (jr_ip_string, addr));
    ...
    // if my ingame joining flag is set, then deny since we only allow one
    // ingame joiner at a time
    if (Netgame.flags & NG_FLAG_INGAME_JOINING) {
        return JOIN_DENY_JR_INGAME_JOIN;
    }

    // check to make sure the game is not full (of observers, or players, as
    // appropriate)
    if ((jr->flags & JOIN_FLAG_AS_OBSERVER)) {
    ...
    if (Netgame.mode == NG_MODE_RESTRICTED) {
        ...
        if (MULTI_IN_MISSION) { return JOIN_QUERY_RESTRICTED; }
    }

    // check to make sure this player hasn't been kick/banned
    if (multi_kick_is_banned (addr)) { return JOIN_DENY_JR_BANNED; }

    // check to make sure this player doesn't already exist
    if (find_player (addr) >= 0) { return JOIN_DENY_JR_DUP; }

    return -1;
}

// Karajorma - called by any machine (client, host, server, standalone, etc) if
// the mission ends by any means other than a warpout
void multi_handle_sudden_mission_end () {
    if (!(MULTIPLAYER_STANDALONE)) { multi_msg_text_flush (); }
}

// called by any machine (client, host, server, standalone, etc), to begin
// warping out all player objects
void multi_warpout_all_players () {
    ...
    // if we're an observer, or we're respawning, or we can't warp out. so just
    // jump into the debrief state
...
int multi_get_highest_rank () {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            !MULTI_STANDALONE (Net_players[idx]) &&
            !MULTI_PERM_OBSERVER (Net_players[idx]) &&
            (Net_players[idx].m_player->stats.rank > max_rank)) {
...
void multi_handle_end_mission_request () {
    ...
    // the server of the game does some further processing
        ...
        // send the endgame packet to all clients, who will act on it
        // immediately
        send_endgame_packet ();
        ...
        if (Game_mode & GM_STANDALONE_SERVER) {
            // move to the standalone postgame (which is where we'll handle
            // stats packets, etc)
...
// called to handle any special cases where a player is in some submemu when
// the game is ended
void multi_handle_state_special () {
    ...
    // kill any popupdeads
    if (popupdead_is_active ()) { popupdead_close (); }

    // kill off the pilot info popup if its active
    if (multi_pinfo_popup_active ()) { multi_pinfo_popup_kill (); }

    // now do any special processing for being in states other then the
    // gameplay states
    stack_depth = gameseq_get_depth ();

    // if we're not pushed on top of any states, do any special state case
    // handling here
    if (stack_depth == 0) {
...
void multi_file_xfer_notify (int handle) {
    ...
    if (is_mission) {
        cf_type = Net_player->p_info.options.flags & MLO_FLAG_XFER_MULTIDATA
                      ? CF_TYPE_MULTI_CACHE
                      : CF_TYPE_MISSIONS;
    }
    else {
    ...
    if ((filename[0] != '\0') && !cf_access (filename, cf_type, 00) &&
        ...
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, XSTR ("&Ok", 713),
            XSTR (
                "An outdated copy of this file exists, but it cannot be "
                "overwritten by the server because it is set to be read-only. "
                "Change the permissions on this file next time.",
                714));
        multi_quit_game (PROMPT_NONE);
        return;
    }

    // if the incoming filename is a freespace file, set my netplayer state to
    // be "file xfer"
    if (is_mission) {
...
int multi_query_lag_status () {
    ...
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) { return -1; }

    // if we've been disconnected for some reason or another
    if (multi_client_server_dead ()) { return 1; }

    // if our ping time to the server is over a certain time
    if (Netgame.server->s_info.ping.ping_avg >= MULTI_LAG_VAL) { return 0; }
...
void multi_process_valid_join_request (
    join_request* jr, net_addr* who_from, int ingame_join_team) {
    ...
    if (jr->flags & JOIN_FLAG_AS_OBSERVER) {
        ...
        if (!multi_obs_create_player (
                net_player_num, jr->callsign, who_from,
                &Players[player_num])) {
            Int3 ();
        }
        ...
        if (jr->image_filename[0] != '\0') {
            strcpy_s (
                Net_players[net_player_num].m_player->image_filename,
                jr->image_filename);
        }
        else {
            strcpy_s (
        ...
        player_set_squad_bitmap (
            Net_players[net_player_num].m_player, jr->squad_filename, true);
        ...
        if (jr->pxo_squad_name[0] != '\0') {
            strcpy_s (
                Net_players[net_player_num].p_info.pxo_squad_name,
                jr->pxo_squad_name);
        }
        else {
        ...
        Net_players[net_player_num].s_info.reliable_connect_time =
            (int)time (NULL);
        ...
        send_accept_packet (
            net_player_num,
            (Net_players[net_player_num].flags & NETINFO_FLAG_INGAME_JOIN)
                ? ACCEPT_OBSERVER | ACCEPT_INGAME
                : ACCEPT_OBSERVER);
    }
    else {
        // create the player object
        if (!multi_create_player (
                net_player_num, &Players[player_num], jr->callsign, who_from,
                -1, id_num)) {
            Int3 ();
        }
        ...
        // if he's joining ingame
            ...
            if (Netgame.type_flags & NG_TYPE_TEAM) {
                /*	int i, j;
                    ...
                    for(i = 0; i < MULTI_TS_MAX_TVT_TEAMS; i++)
                    {
                        ...
                        {
                            if(Multi_ts_team[i].multi_ts_flag !=
                   MULTI_TS_FLAG_EMPTY && Multi_ts_team[i].multi_ts_flag !=
                   MULTI_TS_FLAG_NONE)
                            {
                                team_nums[i]++;
                            }
            ...
            send_accept_packet (
                net_player_num, ACCEPT_INGAME, ingame_join_team);

            // set his last full update time for updating him on ingame join
            // ships
            Net_players[net_player_num].s_info.last_full_update_time =
                timestamp (INGAME_SHIP_UPDATE_TIME);
        }
    ...
    // copy in his options
    memcpy (
        &Net_players[net_player_num].p_info.options, &jr->player_options,
...
// if a player is trying to join a restricted game, evaluate the keypress
// (accept or not, etc)
int multi_process_restricted_keys (int k) {
    ...
    if (Multi_restr_query_timestamp == -1) { return 0; }
    ...
    if ((k == key1) || (k == key2)) {
        ...
        // MWA -- 5/26/98.  Next line commented out.  It should be cleared when
        // the ingame joiner actually gets into the mission
        // Netgame.flags &= ~(NG_FLAG_INGAME_JOINING);

        // determine which team to put him on (if any)
        switch (Multi_join_restr_mode) {
        // normal restricted join, Y or N
        case MULTI_JOIN_RESTR_MODE_1: team_val = (k == key1) ? 0 : -1; break;

        // team vs team, team 0 only has ships
        case MULTI_JOIN_RESTR_MODE_2: team_val = (k == key1) ? 0 : -1; break;

        // team vs team, team 1 only has ships
        case MULTI_JOIN_RESTR_MODE_3: team_val = (k == key1) ? 1 : -1; break;

        // team vs team, both teams have ships
        case MULTI_JOIN_RESTR_MODE_4: team_val = (k == key1) ? 0 : 1; break;
        ...
        if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
            if (team_val >= 0) {
                multi_process_valid_join_request (
                    &Multi_restr_join_request, &Multi_restr_addr, team_val);
            }
        }
...
// determine the status of available player ships (use team_0 for non team vs.
// team situations)
void multi_player_ships_available (int* team_0, int* team_1) {
    ...
    *team_1 = 0;
    ...
    while (moveup != END_OF_LIST (&Ship_obj_list)) {
        ...
        if (Objects[moveup->objnum]
                .flags[Object::Object_Flags::Could_be_player]) {
            // get the team # for this ship
            mp_team_num = multi_ts_get_team (
                Ships[Objects[moveup->objnum].instance].ship_name);
            if (mp_team_num == 0) { (*team_0)++; }
...
// server should update the player's bank/link status with the data in the
// passed ship
void multi_server_update_player_weapons (net_player* pl, ship* shipp) {
    ...
    pl->s_info.cur_secondary_bank =
        (char)shipp->weapons.current_secondary_bank;
...
void multi_flush_mission_stuff () {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx])) {
            ...
            Net_players[idx].flags &=
                ~(NETINFO_FLAG_MISSION_OK | NETINFO_FLAG_RESPAWNING |
            ...
            // if this guy is a non-permanent observer, unset the appropriate
            // flags
            if (MULTI_TEMP_OBSERVER (Net_players[idx])) {
                Net_players[idx].flags &=
                    ~(NETINFO_FLAG_OBSERVER | NETINFO_FLAG_OBS_PLAYER);
            }
    ...
    if (Game_mode & GM_STANDALONE_SERVER) { std_multi_setup_goal_tree (); }

    // object signatures
    // this will eventually get reset to Netgame.security the next time an
    // object gets its signature assigned. We do this to resynchronize the
    // host/server and all clients
    Next_ship_signature = SHIP_SIG_MIN;
    Next_asteroid_signature = ASTEROID_SIG_MIN;
    Next_non_perm_signature = NPERM_SIG_MIN;

    // everyone will need to either reload the current mission, leave, or load
    // the next mission, so in any case
...
int multi_ignore_controls (int key) {
    // if the multiplayer text messaging system is active, don't return any
    // keys
    if ((key > 0) && multi_msg_text_process (key)) { return 1; }

    // if the host of the game is being prompted to accept or deny a player in
    // a restricted game
    if ((key > 0) && multi_process_restricted_keys (key)) { return 1; }

    // if we're in text messaging mode, ignore controls
    if (multi_msg_text_mode ()) { return 1; }

    // if the pause system wants to eat keys for a while
    if (multi_pause_eat_keys ()) { return 1; }
...
int multi_kill_limit_reached () {
    ...
    if (Netgame.options.kill_limit <= 0) { return 0; }
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            !MULTI_STANDALONE (Net_players[idx]) &&
            !MULTI_OBSERVER (Net_players[idx]) &&
            (Net_players[idx].m_player->stats.m_kill_count_ok >=
             Netgame.options.kill_limit)) {
...
// display a chat message (write to the correct spot - hud, standalone gui,
// chatbox, etc)
...
void multi_get_mission_checksum (const char* filename) {
    ...
    if (in != NULL) {
        ...
        // if the file doesn't exist, setup some special values, so the server
        // recognizes this
        else {
            Multi_current_file_checksum = 0xffff;
            Multi_current_file_length = -1;
...
int multi_string_to_status (char* valid_string) {
    if (strstr (valid_string, "invalid")) return MVALID_STATUS_INVALID;

    if (strstr (valid_string, "valid")) return MVALID_STATUS_VALID;
...
void multi_update_valid_missions () {
    ...
    if (in != NULL) {
        ...
        while (!cfeof (in)) {
            ...
            if (file_index >= 0)
                Multi_create_mission_list[file_index].valid_status =
                    (char)multi_string_to_status (status_string);
        }
...
short multi_get_new_id () {
    if (Multi_id_num > 20000) { Multi_id_num = 0; }
...
DCF (multi, "changes multiplayer settings (Multiplayer)") {
    if (dc_optional_string ("kick")) {
        ...
        if (!dc_maybe_stuff_int (&new_flags)) new_flags = -1;
...
void multi_spew_pxo_checksums (int max_files, const char* outfile) {
    ...
    if (file_names != NULL) {
        ...
        count = cf_get_file_list (
            max_files, file_names, CF_TYPE_MISSIONS, wild_card);

        if (count <= 0) goto Done;

        cf_create_default_path_string (
            full_name, sizeof (full_name) - 1, CF_TYPE_ROOT, outfile);
        ...
        // header
        fprintf (
            out, "filename,CRC32,mission type,max players,description\r\n");

        // do all the checksums
        for (idx = 0; idx < count; idx++) {
            memset (full_name, 0, sizeof (full_name));
            strcpy_s (
                full_name, cf_add_ext (file_names[idx], FS_MISSION_FILE_EXT));

            if (!cf_chksum_long (full_name, &checksum)) { continue; }

            if (get_mission_info (full_name)) { continue; }

            if (!(The_mission.game_type & MISSION_TYPE_MULTI)) { continue; }
            ...
            if (IS_MISSION_MULTI_DOGFIGHT) { strcpy_s (gametype, "dogfight"); }
            ...
            fprintf (
                out, "\"%s\",%u,\"%s\",%d,\"%s\"\r\n", filename, checksum,
                gametype, The_mission.num_players, description);
        }
...
void multi_spew_table_checksums(int max_files, char *outfile)
{
    ...
    if (file_names != NULL) {
        count = cf_get_file_list(max_files, file_names, CF_TYPE_TABLES,
NOX("*.tbl"));

        if (count <= 0)
            goto Done;

        cf_create_default_path_string(full_name, sizeof(full_name) - 1,
CF_TYPE_ROOT, outfile);
        ...
        for (idx = 0; idx < count; idx++) {
            ...
            if ( cf_chksum_long(full_name, &checksum) ) {
                if (idx == 0)
                    fprintf(out, "('%s',%u,'%s')", full_name, checksum,
modname); else fprintf(out, ",('%s',%u,'%s')", full_name, checksum, modname);
            }
        }
...
// make a bunch of fake players - don't rely on this to be very safe - its
// mostly used for interface testing
#ifndef NDEBUG
...
int bitbuffer_write_flush (bitbuffer* bitbuf) {
    // Flush to next byte
    if (bitbuf->mask != 0x80) { *bitbuf->data++ = (ubyte)bitbuf->rack; }
...
void bitbuffer_put (bitbuffer* bitbuf, uint data, int bit_count) {
    ...
    mask = 1L << (bit_count - 1);
    while (mask != 0) {
        if (mask & data) { bitbuf->rack |= bitbuf->mask; }
        bitbuf->mask >>= 1;
        if (bitbuf->mask == 0) {
...
uint bitbuffer_get_unsigned (bitbuffer* bitbuf, int bit_count) {
    ...
    while (mask != 0) {
        if (bitbuf->mask == 0x80) { bitbuf->rack = *bitbuf->data++; }
...
int multi_pack_unpack_desired_vel (
    ...
    if (write) {
        ...
        if (max_vel.xyz.x > 0.0f) { fields |= (1 << 0); }
        if (max_vel.xyz.y > 0.0f) { fields |= (1 << 1); }
        if (max_vel.xyz.z > 0.0f) { fields |= (1 << 2); }
        ...
        vm_vec_zero (&pi->vel);
        vm_vec_scale_add2 (
            &pi->desired_vel, &orient->vec.rvec, r * max_vel.xyz.x);
        vm_vec_scale_add2 (
            &pi->desired_vel, &orient->vec.uvec, u * max_vel.xyz.y);
        vm_vec_scale_add2 (
            &pi->desired_vel, &orient->vec.fvec, f * max_vel.xyz.z);
...
int multi_pack_unpack_desired_rotvel (
    ...
    if (write) {
        // use ship_info values for max_rotvel instead of taking it from
        // physics info

        // bitfields for each value
        if (sip->max_rotvel.xyz.x > 0.0f) { fields |= (1 << 0); }
        if (sip->max_rotvel.xyz.y > 0.0f) { fields |= (1 << 1); }
        if (sip->max_rotvel.xyz.z > 0.0f) { fields |= (1 << 2); }
        bitbuffer_put (&buf, (uint)fields, 8);

        // output desired rotational velocity as a percent of max
        if (sip->max_rotvel.xyz.x > 0.0f) {
            a = fl2i (
                pi->desired_rotvel.xyz.x * 128.0f / sip->max_rotvel.xyz.x);
            CAP (a, -128, 127);
            bitbuffer_put (&buf, (uint)a, 8);
        }
        ...
        if (sip->max_rotvel.xyz.y > 0.0f) {
            a = fl2i (
                pi->desired_rotvel.xyz.y * 128.0f / sip->max_rotvel.xyz.y);
            CAP (a, -128, 127);
            bitbuffer_put (&buf, (uint)a, 8);
        }
        ...
        if (sip->max_rotvel.xyz.z > 0.0f) {
            a = fl2i (
                pi->desired_rotvel.xyz.z * 128.0f / sip->max_rotvel.xyz.z);
            CAP (a, -128, 127);
            bitbuffer_put (&buf, (uint)a, 8);
        }
...
// Currently supports the dogfight kill matrix and normal debriefing stages but
// if new types are created they should be added here
void send_debrief_event () {
    ...
    if ((Game_mode & GM_MULTIPLAYER) && IS_MISSION_MULTI_DOGFIGHT &&
        !(The_mission.flags[Mission::Mission_Flags::Toggle_debriefing])) {
        gameseq_post_event (GS_EVENT_MULTI_DOGFIGHT_DEBRIEF);
    }
...
// sends out a ping if we are multi so that psnet2 doesn't kill us off for a
// long load
