...
void send_player_kick_packet (
    int player_index, int ban = 1, int reason = KICK_REASON_NORM);
...
// initialize all kicking details (ban lists, etc). it is safe to call this
...
// process all kick details (disconnecting players who have been kicked but
// haven't closed their socket)
void multi_kick_process () {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            (Net_players[idx].s_info.kick_timestamp != -1) &&
...
void multi_kick_player (int player_index, int ban, int reason) {
    ...
    if (!(Game_mode & GM_STANDALONE_SERVER) &&
        ...
        if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
            ...
            Net_players[player_index].s_info.kick_timestamp =
                timestamp (MULTI_KICK_RESPONSE_TIME);
            Net_players[player_index].s_info.kick_reason = reason;
            send_leave_game_packet (
                Net_players[player_index].player_id, reason,
            ...
            // add the string to the chatbox and the hud (always safe - if it
            // is not inited, nothing bad will happen)
            char str[512];
            memset (str, 0, 512);
            sprintf (
                str, XSTR ("<kicking %s ...>", 1501),
                Net_players[player_index].m_player->callsign);
            multi_display_chat_msg (str, player_index, 0);
        }
        // otherwise, we should send the packet indicating that this guy should
        // be kicked
        else {
...
int multi_kick_is_banned (net_addr* addr) {
    ...
    for (idx = 0; idx < Multi_kick_num_ban_slots; idx++) {
        ...
        if (psnet_same (&Multi_kick_ban_slots[idx], addr)) { return 1; }
    }
...
void multi_kick_get_text (net_player* pl, int reason, char* str) {
    ...
    case KICK_REASON_BAD_XFER:
        sprintf (
            str,
            XSTR (
                "<%s was kicked because of mission file xfer failure>", 1003),
            pl->m_player->callsign);
        break;
    case KICK_REASON_CANT_XFER:
        sprintf (
            str,
            XSTR ("<%s was kicked for not having builtin mission %s>", 1004),
            pl->m_player->callsign, Game_current_mission_filename);
        break;
    case KICK_REASON_INGAME_ENDED:
        sprintf (
            str,
            XSTR ("<%s was kicked for ingame joining an ended game>", 1005),
            pl->m_player->callsign);
        break;
    default:
...
void multi_kick_add_ban (net_addr* addr) {
    ...
    if (Multi_kick_num_ban_slots < (MAX_BAN_SLOTS - 1)) {
        memcpy (
            &Multi_kick_ban_slots[Multi_kick_num_ban_slots++], addr,
            sizeof (net_addr));
    }
}
...
int multi_kick_can_kick (net_player* player) {
    ...
    if ((player->flags & NETINFO_FLAG_AM_MASTER) ||
        (player->flags & NETINFO_FLAG_GAME_HOST)) {
        return 1;
    }
