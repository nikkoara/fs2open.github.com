...
/**
 * Called when the client recieves a SEXP packet from the server. Removes the
 * data from the packet that underlying code uses and puts into a new array
 * that will work with the rest of the client-side code.
 */
...
void sexp_network_packet::ensure_space_remains (size_t data_size) {
    ...
    if (packet_size + data_size + HEADER_LENGTH < MAX_PACKET_SIZE) { return; }

    // iterate back through the types array until we find a TERMINATOR and
    // store the corresponding data index
    ...
    if (packet_end < 9 && !packet_flagged_invalid) {
        Warning (
            ...
            Operators[Current_sexp_operator.back ()].text.c_str ());
    ...
    // if we have an existing argument count we need to update where to put it
    // too
...
bool sexp_network_packet::argument_count_is_valid () {
    if (current_argument_count != 0) {
        // we have a problem here, either the argument count is wrong or the
        // last SEXP didn't remove all its data from the packet
        ...
        for (int i = 0; i < current_argument_count; i++) {
            ...
            if (possible_terminator == CALLBACK_TERMINATOR) {
                Warning (
                    ...
                    "this!",
                    Operators[op_num].text.c_str (), current_argument_count,
                    i);
        ...
        // if we still haven't found the terminator it probably means the last
        // SEXP didn't remove all its data from the packet
        if (!terminator_found) {
            GET_DATA (possible_terminator);
            sexp_bytes_left--;
...
void sexp_network_packet::set_data (ubyte* received_packet, int num_ubytes) {
    ...
    for (int i = 0; i < MAX_PACKET_SIZE; i++) { data[i] = received_packet[i]; }
...
void sexp_network_packet::start_callback () {
    ...
    // Ensure that there is enough space in the packet. At present the function
    // writes the Operator number and the argument count into the packet. Both
    // are ints so we must ensure we have space left to write two ints. If this
    // function is changed to write other data, this line MUST be updated too!
    ...
    // store an invalid count, we'll come back and store the correct value once
    // we know what it is.
...
void sexp_network_packet::end_callback () {
    ...
    // ensure that there is enough space in the packet. If this function is
    // ever updated to write anything other than the terminator, this MUST be
...
int sexp_network_packet::get_next_operator () {
    if (!argument_count_is_valid ()) { return -1; }
...
int sexp_network_packet::get_operator () { return op_num; }
...
bool sexp_network_packet::sexp_discard_operator () {
    ...
    // the operation terminated correctly, probably a new SEXP that this
    // version doesn't support.
    if (terminator == CALLBACK_TERMINATOR) return true;
