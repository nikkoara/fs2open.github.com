...
// local (netplayer - nonserver) options - maintained on individual clients and
// on the server (no need for other clients to know this guy's settings)
...
typedef struct multi_global_options {
    ...
    char user_tracker_ip[MULTI_OPTIONS_STRING_LEN]; // ip address of user
                                                    // tracker
    char game_tracker_ip[MULTI_OPTIONS_STRING_LEN]; // ip address of game
                                                    // tracker
    char tracker_port[STD_NAME_LEN]; // ip address to use for user/game tracker
                                     // (used for FS2NetD only)
    int pxo;
    ...
    char pxo_create_url[MULTI_OPTIONS_STRING_LEN]; // URL of pxo create account
                                                   // page
    char pxo_verify_url[MULTI_OPTIONS_STRING_LEN]; // URL of pxo account
                                                   // validation page
    ...
    char std_pname[STD_NAME_LEN + 1]; // permanent name for the standalone - if
                                      // any
...
#define MLO_FLAG_ACCEPT_PIX \
    (1 << 0) // accept pix from server (pilot pics, squadron logos, etc)
#define MLO_FLAG_NO_VOICE (1 << 1) // turn off voice altogether
#define MLO_FLAG_LOCAL_BROADCAST \
    (1 << 2) // broadcast on the local subnet when looking for games
#define MLO_FLAG_FLUSH_CACHE \
    (1 << 3) // flush the multidata cache before every game
#define MLO_FLAG_XFER_MULTIDATA \
    (1 << 4) // xfer mission files to the multidata cache directory
#define MLO_FLAG_TEMP_CLOSED \
    (1 << 5) // send to standalone to tell him to toggle the temp closed status
...
#define MSO_END_RANK \
    0 // only the highest ranking players and the host can end the mission
#define MSO_END_LEADER \
    1 // only team/wing leaders and the host can end the mission
...
#define MSO_FLAG_INGAME_XFER \
    (1 << 0) // netgame allows file xfers to ingame joiners
...
#define MSO_FLAG_SS_LEADERS \
    (1 << 3) // in ship/weapon select, only host or team captains can modify
             // ships
