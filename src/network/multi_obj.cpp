...
float oo_arrive_time_next[MAX_SHIPS]; // how many seconds have gone by. should
                                      // be equal to oo_arrive_time_avg_diff[]
                                      // the next time we get an update

// interp stuff
int oo_interp_count[MAX_SHIPS];
vec3d oo_interp_points[MAX_SHIPS][2];
bez_spline oo_interp_splines[MAX_SHIPS][2];
void multi_oo_calc_interp_splines (
    int ship_index, vec3d* cur_pos, matrix* cur_orient,
    physics_info* cur_phys_info, vec3d* new_pos, matrix* new_orient,
...
#define OO_PRIMARY_BANK \
    (1 << 5) // if this is set, fighter has selected bank one
...
#define OO_VIEW_DIFF_TOL \
    (0.15f) // if the dotproducts differ this far between frames, he's coming
            // into view
...
int OO_update_index =
    -1; // index into OO_update_records for displaying update record info
...
bool multi_oo_sort_func (const short& index1, const short& index2) {
    ...
    if ((index1 < 0) || (index2 < 0) || (Ships[index1].objnum < 0) ||
...
void multi_oo_build_ship_list (net_player* pl) {
    ...
    for (idx = 0; idx < MAX_SHIPS; idx++) { OO_ship_index[idx] = -1; }

    // get the player object
    if (pl->m_player->objnum < 0) { return; }
    player_obj = &Objects[pl->m_player->objnum];

    // go through all other relevant objects
    ship_index = 0;
    for (moveup = GET_FIRST (&Ship_obj_list);
         moveup != END_OF_LIST (&Ship_obj_list); moveup = GET_NEXT (moveup)) {
        ...
        // if we're a standalone server, don't send any data regarding its
        // pseudo-ship
        if ((Game_mode & GM_STANDALONE_SERVER) &&
            ((&Objects[moveup->objnum] == Player_obj) ||
        ...
        if ((Ships[Objects[moveup->objnum].instance].ship_info_index >= 0) &&
            (Ships[Objects[moveup->objnum].instance].ship_info_index <
             static_cast< int > (Ship_info.size ())) &&
            (Ship_info[Ships[Objects[moveup->objnum].instance].ship_info_index]
                 .flags[Ship::Info_Flags::Knossos_device])) {
            continue;
        }

        // don't send him info for himself
        if (&Objects[moveup->objnum] == player_obj) { continue; }

        // don't send info for his targeted ship here, since its always done
        // first
        if ((pl->s_info.target_objnum != -1) &&
            (moveup->objnum == pl->s_info.target_objnum)) {
            continue;
        }

        // add the ship
        if (ship_index < MAX_SHIPS) {
            OO_ship_index[ship_index++] =
                (short)Objects[moveup->objnum].instance;
        }
    }
    ...
    if (OO_sort) {
        std::sort (
            OO_ship_index, OO_ship_index + ship_index, multi_oo_sort_func);
    }
}
...
int multi_oo_pack_client_data (ubyte* data) {
    ...
    if (Player->locking_on_center) { out_flags |= OOC_LOCKING_ON_CENTER; }
    if ((Player_ship != NULL) &&
        (Player_ship->flags[Ship::Ship_Flags::Trigger_down])) {
    ...
    // if nothing targeted
    if (Player_ai->target_objnum == -1) { tnet_signature = 0; }
    // if something is targeted
        ...
        if (Player_ai->targeted_subsys != NULL) {
            t_subsys = (char)ship_get_index_from_subsys (
                Player_ai->targeted_subsys, Player_ai->target_objnum);
        }

        // locked targeted subsys index
        if (Player->locking_subsys != NULL) {
            l_subsys = (char)ship_get_index_from_subsys (
                Player->locking_subsys, Player_ai->target_objnum, 1);
        }
    }
...
int multi_oo_pack_data (
    ...
    // header sizes
    if (MULTIPLAYER_MASTER) { header_bytes = 5; }
    else {
        header_bytes = 2;
    }

    // if we're a client (and therefore sending control info), pack
    // client-specific info
    if ((Net_player != NULL) &&
        !(Net_player->flags & NETINFO_FLAG_AM_MASTER)) {
        packet_size +=
            multi_oo_pack_client_data (data + packet_size + header_bytes);
    }

    // position, velocity
    if (oo_flags & OO_POS_NEW) {
        ret = (ubyte)multi_pack_unpack_position (
            1, data + packet_size + header_bytes, &objp->pos);
        packet_size += ret;

        // global records
        multi_rate_add (NET_PLAYER_NUM (pl), "pos", ret);

        ret = (ubyte)multi_pack_unpack_vel (
            1, data + packet_size + header_bytes, &objp->orient, &objp->pos,
            &objp->phys_info);
        packet_size += ret;
    ...
    if (oo_flags & OO_ORIENT_NEW) {
        ret = (ubyte)multi_pack_unpack_orient (
            1, data + packet_size + header_bytes, &objp->orient);
        // Assert(ret == OO_ORIENT_RET_SIZE);
        packet_size += ret;
        multi_rate_add (NET_PLAYER_NUM (pl), "ori", ret);

        ret = (ubyte)multi_pack_unpack_rotvel (
            1, data + packet_size + header_bytes, &objp->orient, &objp->pos,
            &objp->phys_info);
        packet_size += ret;
    ...
    if (oo_flags & OO_HULL_NEW) {
        ...
        if ((temp < 0.004f) && (temp > 0.0f)) {
            temp = 0.004f; // 0.004 is the lowest positive value we can have
                           // before we zero out when packing
        }
        PACK_PERCENT (temp);
    ...
    if (oo_flags & OO_SUBSYSTEMS_AND_AI_NEW) {
        ...
        // just in case we have some kind of invalid data (should've been taken
        // care of earlier in this function)
        ...
        if (Ai_info[shipp->ai_index].target_objnum != -1) {
            target_signature =
                Objects[Ai_info[shipp->ai_index].target_objnum].net_signature;
        }
    ...
    if (MULTIPLAYER_MASTER && (sip->flags[Ship::Info_Flags::Support]) &&
        ...
        if ((Ai_info[shipp->ai_index].support_ship_objnum < 0) ||
            (Ai_info[shipp->ai_index].support_ship_objnum >= MAX_OBJECTS)) {
            dock_sig = 0;
        }
        else {
            dock_sig = Objects[Ai_info[shipp->ai_index].support_ship_objnum]
                           .net_signature;
        }
...
int multi_oo_unpack_client_data (net_player* pl, ubyte* data) {
    ...
    if ((pl != NULL) && !(pl->flags & NETINFO_FLAG_RESPAWNING) &&
        ...
        if ((shipp != NULL) && (shipp->ai_index != -1)) {
            Ai_info[shipp->ai_index].current_target_is_locked =
                (in_flags & OOC_TARGET_LOCKED) ? 1 : 0;
            Ai_info[shipp->ai_index].ai_flags.set (
                AI::AI_Flags::Seek_lock,
                (in_flags & OOC_TARGET_SEEK_LOCK) != 0);
        }
    ...
    tobj = NULL;
    if (tnet_sig != 0) { tobj = multi_get_network_object (tnet_sig); }
    ...
    if ((tobj != NULL) && (pl != NULL) && (pl->m_player->objnum != -1)) {
        ...
        if (Objects[pl->m_player->objnum].type == OBJ_SHIP) {
            Ai_info[Ships[Objects[pl->m_player->objnum].instance].ai_index]
                .target_objnum = OBJ_INDEX (tobj);
        }
        ...
        if (Objects[pl->m_player->objnum].type == OBJ_SHIP) {
            if ((l_subsys != -1) && (tobj->type == OBJ_SHIP)) {
                pl->m_player->locking_subsys =
                    ship_get_indexed_subsys (&Ships[tobj->instance], l_subsys);
            }
        }
    }
...
#define UNPACK_PERCENT(v)                                   \
    {                                                       \
        ubyte temp_byte;                                    \
        memcpy (&temp_byte, data + offset, sizeof (ubyte)); \
        v = (float)temp_byte / 255.0f;                      \
        offset++;                                           \
    }
int multi_oo_unpack_data (net_player* pl, ubyte* data) {
    ...
    // if we can't find the object, set pointer to bogus object to continue
    // reading the data ignore out of sequence packets here as well
    if ((pobjp == NULL) || (pobjp->type != OBJ_SHIP) ||
        (pobjp->instance < 0) || (pobjp->instance >= MAX_SHIPS) ||
        (Ships[pobjp->instance].ship_info_index < 0) ||
        (Ships[pobjp->instance].ship_info_index >=
         static_cast< int > (Ship_info.size ()))) {
        offset += data_size;
        return offset;
    }
    ...
    if (oo_flags & OO_POS_NEW) {
        ...
        if (oo_arrive_time_count[shipp - Ships] == 5) {
            memmove (
                &oo_arrive_time[shipp - Ships][0],
                &oo_arrive_time[shipp - Ships][1], sizeof (float) * 4);
            oo_arrive_time[shipp - Ships][4] = f2fl (Missiontime);
        }
        else {
            oo_arrive_time[shipp - Ships]
                          [oo_arrive_time_count[shipp - Ships]++] =
                              f2fl (Missiontime);
        }
        ...
        if (oo_arrive_time_count[shipp - Ships] == 5) {
            ...
            for (idx = 0; idx < 4; idx++) {
                oo_arrive_time_avg_diff[shipp - Ships] +=
                    oo_arrive_time[shipp - Ships][idx + 1] -
        ...
        // int r3 = multi_pack_unpack_vel( 0, data + offset, &pobjp->orient,
        // &pobjp->pos, &pobjp->phys_info );
        int r3 = multi_pack_unpack_vel (
            0, data + offset, &pobjp->orient, &new_pos, &new_phys_info);
        offset += r3;
    ...
    if (oo_flags & OO_ORIENT_NEW) {
        ...
        // int r5 = multi_pack_unpack_rotvel( 0, data + offset, &pobjp->orient,
        // &pobjp->pos, &pobjp->phys_info );
        int r5 = multi_pack_unpack_rotvel (
            0, data + offset, &new_orient, &new_pos, &new_phys_info);
        offset += r5;
    ...
    if (oo_flags & OO_POS_NEW) {
        ...
        // this should cause our 2 interpolation splines to be exactly the
        // same. so we'll see a jump, but it should be nice and smooth
        ...
        if (oo_interp_count[shipp - Ships] < 2) {
            oo_interp_points[shipp - Ships][oo_interp_count[shipp - Ships]++] =
                new_pos;
        }
        else {
            oo_interp_points[shipp - Ships][0] =
                oo_interp_points[shipp - Ships][1];
            oo_interp_points[shipp - Ships][1] = new_pos;

            multi_oo_calc_interp_splines (
                SHIP_INDEX (shipp), &pobjp->pos, &pobjp->orient,
                &pobjp->phys_info, &new_pos, &new_orient, &new_phys_info);
        }
    ...
    // ---------------------------------------------------------------------------------------------------------------
    // ANYTHING BELOW HERE WORKS FINE - nothing here which causes jumpiness or
    // bandwidth problems :) WHEEEEE!
    // ---------------------------------------------------------------------------------------------------------------
    ...
    if (oo_flags & OO_HULL_NEW) {
        UNPACK_PERCENT (fpct);
        pobjp->hull_strength =
            fpct * Ships[pobjp->instance].ship_max_hull_strength;
    ...
    if (oo_flags & OO_SUBSYSTEMS_AND_AI_NEW) {
        ...
        for (subsysp = GET_FIRST (&shipp->subsys_list);
            ...
            // add the value just generated (it was zero'ed above) into the
            ...
            if (subsys_count >= n_subsystems) { break; }
        }
    ...
    if (support_extra) {
        ...
        // valid ship?
        if ((shipp != NULL) && (shipp->ai_index >= 0) &&
            (shipp->ai_index < MAX_AI_INFO)) {
            ...
            if (objp != NULL) {
                Ai_info[shipp->ai_index].support_ship_objnum =
                    OBJ_INDEX (objp);
            }
        }
    }
    ...
    if (!MULTIPLAYER_MASTER && (shipp != NULL)) {
        ...
        // trigger down or not - server doesn't care about this. he'll get it
        // from clients anyway
        shipp->flags.remove (Ship::Ship_Flags::Trigger_down);
...
void multi_oo_reset_timestamp (
    net_player* pl, object* objp, int range, int in_cone) {
    int stamp = 0;

    // if this is the guy's target,
    if ((pl->s_info.target_objnum != -1) &&
        (pl->s_info.target_objnum == OBJ_INDEX (objp))) {
        stamp =
            Multi_oo_target_update_times[pl->p_info.options.obj_update_level];
    }
    else {
        ...
        if (in_cone) {
            ...
            case OO_NEAR:
                stamp =
                    Multi_oo_front_near_update_times[pl->p_info.options
                                                         .obj_update_level];
                break;

            case OO_MIDRANGE:
                stamp =
                    Multi_oo_front_medium_update_times[pl->p_info.options
                                                           .obj_update_level];
                break;

            case OO_FAR:
                stamp = Multi_oo_front_far_update_times[pl->p_info.options
                                                            .obj_update_level];
                break;
            }
        }
            ...
            switch (range) {
            case OO_NEAR:
                stamp = Multi_oo_rear_near_update_times[pl->p_info.options
                                                            .obj_update_level];
                break;

            case OO_MIDRANGE:
                stamp =
                    Multi_oo_rear_medium_update_times[pl->p_info.options
                                                          .obj_update_level];
                break;

            case OO_FAR:
                stamp = Multi_oo_rear_far_update_times[pl->p_info.options
                                                           .obj_update_level];
                break;
            }
        }
    ...
    if (objp->type == OBJ_SHIP) {
        Ships[objp->instance].np_updates[NET_PLAYER_NUM (pl)].update_stamp =
            timestamp (stamp);
    }
}
...
void multi_oo_reset_status_timestamp (object* objp, int player_index) {
    Ships[objp->instance].np_updates[player_index].status_update_stamp =
        timestamp (OO_HULL_SHIELD_TIME);
}

// reset the timestamp appropriately for the passed in object
void multi_oo_reset_subsys_timestamp (object* objp, int player_index) {
    Ships[objp->instance].np_updates[player_index].subsys_update_stamp =
        timestamp (OO_SUBSYS_TIME);
}

// determine what needs to get sent for this player regarding the passed
// object, and when
int multi_oo_maybe_update (net_player* pl, object* obj, ubyte* data) {
    ...
    if (!(player_index >= 0) || !(player_index < MAX_PLAYERS)) { return 0; }

    // determine what the timestamp is for this object
    if (obj->type == OBJ_SHIP) {
        stamp =
            Ships[obj->instance].np_updates[NET_PLAYER_NUM (pl)].update_stamp;
    }
    else {
        return 0;
    }
    ...
    if (dist < OO_NEAR_DIST) { range = OO_NEAR; }
    ...
    if ((Ships[obj->instance].np_updates[player_index].status_update_stamp ==
         -1) ||
        timestamp_elapsed_safe (
            Ships[obj->instance].np_updates[player_index].status_update_stamp,
            OO_MAX_TIMESTAMP)) {
    ...
    // if the object's hull/shield timestamp has expired
    if ((Ships[obj->instance].np_updates[player_index].subsys_update_stamp ==
         -1) ||
        timestamp_elapsed_safe (
            Ships[obj->instance].np_updates[player_index].subsys_update_stamp,
            OO_MAX_TIMESTAMP)) {
    ...
    if ((pl->s_info.target_objnum != -1) &&
        (OBJ_INDEX (obj) == pl->s_info.target_objnum)) {
        oo_flags |= (OO_POS_NEW | OO_ORIENT_NEW | OO_HULL_NEW);
    }
    // all other cases
    else {
        // add info which is contingent upon being "in front"
        if (in_cone) { oo_flags |= OO_ORIENT_NEW; }
    }

    // get current position and orient checksums
    cur_pos_chksum = cf_add_chksum_short (
        cur_pos_chksum, (ubyte*)(&obj->pos), sizeof (vec3d));
    cur_orient_chksum = cf_add_chksum_short (
        cur_orient_chksum, (ubyte*)(&obj->orient), sizeof (matrix));

    // if position or orientation haven't changed
    if ((shipp->np_updates[player_index].pos_chksum != 0) &&
        (shipp->np_updates[player_index].pos_chksum == cur_pos_chksum)) {
        // if we otherwise would have been sending it, keep track of it (debug
        // only)
#ifndef NDEBUG
        if (oo_flags & OO_POS_NEW) {
            multi_rate_add (
                player_index, "skp_p", OO_POS_RET_SIZE + OO_VEL_RET_SIZE);
        }
#endif
        oo_flags &= ~(OO_POS_NEW);
    }
    if ((shipp->np_updates[player_index].orient_chksum != 0) &&
        ...
        if (oo_flags & OO_ORIENT_NEW) {
            multi_rate_add (
                player_index, "skp_o",
                OO_ORIENT_RET_SIZE + OO_ROTVEL_RET_SIZE);
        }
#endif
...
void multi_oo_process_all (net_player* pl) {
    ...
    if ((pl->s_info.target_objnum != -1) &&
        (Objects[pl->s_info.target_objnum].type == OBJ_SHIP)) {
        // build the header
    ...
    idx = 0;
    // rely on logical-AND shortcut evaluation to prevent array out-of-bounds
    // read of OO_ship_index[idx]
    while ((idx < MAX_SHIPS) && (OO_ship_index[idx] >= 0)) {
        // if this guy is over his datarate limit, do nothing
        if (multi_oo_rate_exceeded (pl)) {
            // WARNINGF (LOCATION, "Capping client");
            idx++;
        ...
        // if this data is too much for the packet, send off what we currently
        // have and start over
    ...
    // if we have anything more than 3 byte in the packet, send the last one
    // off
    if (packet_size > 3) {
        stop = 0x00;
        multi_rate_add (NET_PLAYER_NUM (pl), "stp", 1);
...
void multi_oo_process () {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            ...
            (Net_player !=
             &Net_players[idx]) /*&& !MULTI_OBSERVER(Net_players[idx])*/) {
            ...
            if ((Net_players[idx].m_player != NULL) &&
                ...
                !(Net_players[idx].flags & NETINFO_FLAG_LIMBO) &&
                !(Net_players[idx].flags & NETINFO_FLAG_RESPAWNING)) {
                if ((Objects[Net_players[idx].m_player->objnum]
                         .flags[Object::Object_Flags::Player_ship]) &&
                    !(Objects[Net_players[idx].m_player->objnum]
                          .flags[Object::Object_Flags::Should_be_dead])) {
                    obj_player_fire_stuff (
                        &Objects[Net_players[idx].m_player->objnum],
                        Net_players[idx].m_player->ci);
                }
            }
        }
    }
...
void multi_oo_process_update (ubyte* data, header* hinfo) {
    ...
    if (player_index != -1) { pl = &Net_players[player_index]; }
    // otherwise its a "regular" object update packet on a client from the
    // server. use "myself" as the reference player
    else {
        pl = Net_player;
    }
...
// initialize all object update timestamps (call whenever entering gameplay
// state)
void multi_oo_gameplay_init () {
    ...
    */
    ...
    // so = GET_NEXT(so) ) { if(Objects[so->objnum].type == OBJ_SHIP){
    ...
    extern int OO_gran;
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        Net_players[idx].s_info.rate_stamp =
            timestamp ((int)(1000.0f / (float)OO_gran));
    }
}

// send control info for a client (which is basically a "reverse" object
// update)
...
// Sends a packet from the server to the client, syncing the player's
// position/orientation to the Server's. Allows for use of certain SEXPs in
// multiplayer.
void multi_oo_send_changed_object (object* changedobj) {
    ...
    int idx = 0;
#ifndef NDEBUG
    // WARNINGF (LOCATION, "Attempting to affect player object.");
#endif
...
#ifndef NDEBUG
    // WARNINGF (LOCATION, "Index for changed object found: [%d].", idx);
#endif
    ...
    // pack the appropriate info into the data
    add_size =
        multi_oo_pack_data (&Net_players[idx], changedobj, oo_flags, data_add);
...
// timestamp for sending control info (movement only - we'll send button info
// all the time)
...
void multi_oo_rate_process () {
    ...
    if (!(Game_mode & GM_IN_MISSION)) { return; }
...
void multi_oo_server_process () {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            !MULTI_SERVER (Net_players[idx])) {
            // if his timestamp is -1 or has expired, reset it and zero his
            // rate byte count
            if ((Net_players[idx].s_info.rate_stamp == -1) ||
                timestamp_elapsed_safe (
                    Net_players[idx].s_info.rate_stamp, OO_MAX_TIMESTAMP) ||
                (abs (timestamp () - Net_players[idx].s_info.rate_stamp) >=
                 (int)(1000.0f / (float)OO_gran))) {
                Net_players[idx].s_info.rate_stamp =
                    timestamp ((int)(1000.0f / (float)OO_gran));
    ...
    if ((OO_server_rate_stamp == -1) ||
        timestamp_elapsed_safe (OO_server_rate_stamp, OO_MAX_TIMESTAMP)) {
        // reset the timestamp
...
void multi_oo_client_process () {
    ...
    if ((Multi_cirate_stamp == -1) ||
        timestamp_elapsed_safe (Multi_cirate_stamp, OO_CIRATE)) {
...
int multi_oo_rate_exceeded (net_player* pl) {
    ...
    // high update level - super high datarate (no capping, just intelligent
    // updating)
    case OBJ_UPDATE_HIGH: rate_compare = OO_LIMIT_HIGH; break;

    // LAN - no rate max
    case OBJ_UPDATE_LAN: return 0;
    ...
    if (OO_client_rate < rate_compare) { rate_compare = OO_client_rate; }

    // compare his bytes sent against the allowable amount
    if (pl->s_info.rate_bytes >= rate_compare) { return 1; }
...
void multi_oo_update_server_rate () {
    ...
    if (!(Game_mode & GM_STANDALONE_SERVER)) { num_connections--; }
    ...
    OO_client_rate =
        (int)(((float)OO_server_rate / (float)OO_gran) / (float)num_connections);
}
...
int multi_oo_is_interp_object (object* objp) {
    ...
    if (!(objp->flags[Object::Object_Flags::Player_ship])) { return 0; }
...
void multi_oo_interp (object* objp) {
    ...
    if ((oo_interp_count[objp->instance] < 2) ||
        (oo_arrive_time_count[objp->instance] < 5)) {
        return;
    }
    ...
    // the next packet
    float t = oo_arrive_time_next[objp->instance] /
              oo_arrive_time_avg_diff[objp->instance];
...
void multi_oo_calc_interp_splines (
    ...
    if (!IS_VEC_NULL_SQ_SAFE (&v_norm) && !IS_VEC_NULL_SQ_SAFE (&v_dir)) {
        ...
        if (vm_vec_dot (&v_dir, &v_norm) < 0.0f) { *new_pos = *cur_pos; }
    }

    // get the spline representing our "bad" movement. its better to be little
    // bit off than to overshoot altogether
    ...
    physics_sim (
        &c, &m_copy, &p_copy,
        avg_diff *
            oo_error); // next point, assuming we followed our current path
    oo_interp_splines[ship_index][0].bez_set_points (3, pts);

    // get the spline representing where this new point tells us we'd be
    // heading
    ...
    physics_sim (
        &c, &m_copy, &p_copy, avg_diff); // next point, given this new info
    oo_interp_splines[ship_index][1].bez_set_points (3, pts);

    // now we've got a spline representing our "new" path and where we would've
    // gone had we been perfect before we'll modify our velocity to move along
    // a blend of these splines.
}
...
DCF (
    bez,
    "Toggles rendering of player ship trajectory interpolation splines "
    "(Multiplayer) *disabled*") {
    if (dc_optional_string_either ("status", "--status") ||
        dc_optional_string_either ("?", "--?")) {
        dc_printf (
            "Rendering of interpolation splines is '%s'",
            display_oo_bez ? "ON" : "OFF");
        return;
    }
...
void oo_display () {
    /*  int idx;
        ...
        for(idx=0; idx<MAX_SHIPS; idx++){
            ...
            if( (oo_arrive_time_count[idx] == 5) && (idx != (Player_ship -
       Ships)) ){ gr_printf(20, 40, "avg time between updates : %f",
       oo_arrive_time_avg_diff[idx]);
            }
