...
const char* Multi_ts_bmap_names[GR_NUM_RESOLUTIONS][3] = {
    { // GR_640
      "TSB_340000", "TSB_340001", "TSB_340002" },
    { // GR_1024
      "2_TSB_340000", "2_TSB_340001", "2_TSB_340002" }
};
...
static int Multi_ts_slot_icon_coords[MULTI_TS_NUM_SHIP_SLOTS]
                                    [GR_NUM_RESOLUTIONS][2] = { // x,y
                                        {
                                            //
                                            { 128, 301 }, // GR_640
                                            { 205, 482 }  // GR_1024
...
static int Multi_ts_slot_text_coords[MULTI_TS_NUM_SHIP_SLOTS]
                                    [GR_NUM_RESOLUTIONS][3] = { // x,y,width
                                        {
                                            // alpha
...
static int Multi_ts_avail_coords[MULTI_TS_AVAIL_MAX_DISPLAY]
                                [GR_NUM_RESOLUTIONS][2] = { // x,y coords
                                    {
                                        //
                                        { 23, 331 }, // GR_640
                                        { 37, 530 }  // GR_1024
...
int Multi_ts_avail_start =
    0; // starting index of where we will display the available ships
...
#define MULTI_TS_SHIP_INFO_MAX_TEXT \
    (MULTI_TS_SHIP_INFO_MAX_LINE_LEN * MULTI_TS_SHIP_INFO_MAX_LINES)
...
char Multi_ts_ship_info_lines[MULTI_TS_SHIP_INFO_MAX_LINES]
                             [MULTI_TS_SHIP_INFO_MAX_LINE_LEN];
char Multi_ts_ship_info_text[MULTI_TS_SHIP_INFO_MAX_TEXT];
int Multi_ts_ship_info_line_count;

// status bar mode
static int Multi_ts_status_coords[GR_NUM_RESOLUTIONS][3] = { { // GR_640
                                                               95, 467, 426 },
                                                             { // GR_1024
                                                               152, 747,
                                                               688 } };
...
typedef struct ts_team_data {
    int multi_ts_objnum[MULTI_TS_NUM_SHIP_SLOTS]; // objnums for all slots in
                                                  // this team
    net_player*
        multi_ts_player[MULTI_TS_NUM_SHIP_SLOTS]; // net players corresponding
                                                  // to the same slots
    int multi_ts_flag[MULTI_TS_NUM_SHIP_SLOTS];   // flags indicating the
                                                  // "status" of a slot
...
// what type of region the index is (0 == ship avail list, 1 == ship slots, 2
// == player slot)
...
void multi_ts_get_team_and_slot (
    char* ship_name, int* team_index, int* slot_index, bool mantis2757switch);
...
int multi_ts_can_perform (
    int from_type, int from_index, int to_type, int to_index, int ship_class,
    int player_index = -1);

// determine the kind of drag and drop operation this is
int multi_ts_get_dnd_type (
    int from_type, int from_index, int to_type, int to_index,
    int player_index = -1);

// swap two player positions
int multi_ts_swap_player_player (
    int from_index, int to_index, interface_snd_id* sound,
    int player_index = -1);

// move a player
int multi_ts_move_player (
    int from_index, int to_index, interface_snd_id* sound,
    int player_index = -1);

// get the ship class of the current index in the avail list or -1 if none
// exists
...
// return the bitmap index into the ships icon array (in ship select) which
// should be displayed for the given slot
...
// set the status bar to reflect the status of wing slots (free or not free). 0
// or 1 are valid values for now
...
// initialize the team select screen (always call, even when switching between
// weapon select, etc)
void multi_ts_init () {
...
void multi_ts_common_init () {
    ...
    // reset timestamps here. they seem to get hosed by the loadinh of the
    // mission file
    ...
    if (!MULTI_PERM_OBSERVER (Net_players[MY_NET_PLAYER_NUM])) {
        ...
        Multi_ts_select_ship_class =
            Wss_slots[Multi_ts_select_index].ship_class;
    ...
    memset (
        Multi_ts_deleted_objnums, 0,
        sizeof (int) * MULTI_TS_MAX_TVT_TEAMS * MULTI_TS_NUM_SHIP_SLOTS);
    ...
    for (idx = 0; idx < MULTI_TS_NUM_LOCKED_BITMAPS; idx++) {
        ...
        Multi_ts_locked_bitmaps[idx] =
            bm_load (Multi_ts_bmap_names[gr_screen.res][idx]);
    }
...
void multi_ts_do () {
    ...
    Multi_ts_buttons[gr_screen.res][MULTI_TS_SHIP_SELECT].button.draw_forced (
        2);
    ...
    if (multi_ts_is_locked ()) {
        ...
        if (((Netgame.type_flags & NG_TYPE_TEAM) &&
            ...
            Multi_ts_buttons[gr_screen.res][MULTI_TS_LOCK].button.draw_forced (
                0);
        }
        else {
    ...
    // the end of the loop since we are a bit special and have to close out in
    // the proper order.
    if (Commit_pressed) {
        multi_ts_commit_pressed ();
        Commit_pressed = 0;
...
// close the team select screen (always call, even when switching between
// weapon select, etc)
void multi_ts_close () {
    int idx;

    if (!Multi_ts_inited) { return; }
...
int multi_ts_disabled_slot (int slot_num, int player_index) {
    ...
    if (pl->flags & NETINFO_FLAG_OBSERVER) { return 1; }

    // if the flag for this team isn't set to "free" we can't do anything
    if (!Multi_ts_team[pl->p_info.team].multi_players_locked) { return 1; }

    // if the "leaders" only flag is set
    if (Netgame.options.flags & MSO_FLAG_SS_LEADERS) {
        // in a team vs. team situation
        if (Netgame.type_flags & NG_TYPE_TEAM) {
            if (pl->flags & NETINFO_FLAG_TEAM_CAPTAIN) { return 0; }
        }
        // in a non team vs. team situation
        else {
            if (pl->flags & NETINFO_FLAG_GAME_HOST) { return 0; }
        }
    }
    else {
        // in a team vs. team situation
        if (Netgame.type_flags & NG_TYPE_TEAM) {
            // if i'm the team captain I can mess with my own ships as well as
            // those of the ai ships on my team
            if (pl->flags & NETINFO_FLAG_TEAM_CAPTAIN) {
                if ((Multi_ts_team[pl->p_info.team]
                         .multi_ts_player[slot_num] != NULL) &&
                    (Objects[Multi_ts_team[pl->p_info.team]
                                 .multi_ts_objnum[slot_num]]
                         .flags[Object::Object_Flags::Player_ship]) &&
                    (slot_num != pl->p_info.ship_index)) {
                    return 1;
                }
        ...
        if (slot_num == pl->p_info.ship_index) { return 0; }
    }

    return 1;
}

// is the given slot disabled for the specified player, _and_ it is his ship as
// well
int multi_ts_disabled_high_slot (int slot_index, int player_index) {
    ...
    if (multi_ts_disabled_slot (slot_index, player_index) &&
        !Multi_ts_team[pl->p_info.team].multi_players_locked &&
...
// resynch all display/interface elements based upon all the ship/weapon pool
// values
void multi_ts_sync_interface () {
    ...
    for (idx = 0; idx < static_cast< int > (Ship_info.size ()); idx++) {
        if (Ss_pool[idx] > 0) { Multi_ts_avail_count++; }
    }

    // item 2 - make sure our local Multi_ts_slot_flag array is up to date
    multi_ts_init_flags ();

    // item 3 - set/unset any necessary flags in underlying ship select data
    // structures
    for (idx = 0; idx < MAX_WSS_SLOTS; idx++) {
        switch (Multi_ts_team[Net_player->p_info.team].multi_ts_flag[idx]) {
        case MULTI_TS_FLAG_EMPTY: ss_make_slot_empty (idx); break;
        case MULTI_TS_FLAG_NONE: break;
        default: ss_make_slot_full (idx); break;
        }
    }

    // item 4 - reset the locked/unlocked status of all ships in the weapon
    // select screen
...
void multi_ts_assign_players_all () {
    ...
    // merge the created object list with the actual object list so we have all
    // available ships
    ...
    if (Netgame.type_flags & NG_TYPE_TEAM) {
        sprintf (
            name_lookup, "%s 1", TVT_wing_names[Netgame.host->p_info.team]);
    }
    else {
    ...
    if (shipnum == -1) {
        ...
        multi_quit_game (
            PROMPT_NONE, MULTI_END_NOTIFY_NONE, MULTI_END_ERROR_SHIP_ASSIGN);
        return;
    }

    multi_ts_get_team_and_slot (
        Ships[shipnum].ship_name, &team_index, &slot_index);
    ...
    while (objp != END_OF_LIST (&obj_used_list)) {
        ...
        if ((objp->flags[Object::Object_Flags::Player_ship]) &&
            ...
            if (found) {
                ...
                obj_set_flags (
                    objp, objp->flags + Object::Object_Flags::Could_be_player);
            }
    ...
    // go through and change any ships marked as player ships to be
    // COULD_BE_PLAYER
    if (objp != END_OF_LIST (&obj_used_list)) {
        for (objp = GET_NEXT (objp); objp != END_OF_LIST (&obj_used_list);
             objp = GET_NEXT (objp)) {
            if (objp->flags[Object::Object_Flags::Player_ship]) {
                objp->flags.remove (Object::Object_Flags::Player_ship);
                obj_set_flags (
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            !MULTI_STANDALONE (Net_players[idx]) &&
            !MULTI_OBSERVER (Net_players[idx])) {
            // if this guy never got assigned a player ship, there's a mission
            // problem
            if (Net_players[idx].p_info.ship_index == -1) {
                // Netgame.flags |= NG_FLAG_QUITTING;
                multi_quit_game (
                    PROMPT_NONE, MULTI_END_NOTIFY_NONE,
                    MULTI_END_ERROR_SHIP_ASSIGN);
                return;
            }
        }
    }
...
void multi_ts_create_wings () {
    ...
    // the standalone never went through this screen so he should never call
    ...
    for (idx = 0; idx < MULTI_TS_MAX_TVT_TEAMS; idx++) {
        for (s_idx = 0; s_idx < MULTI_TS_NUM_SHIP_SLOTS; s_idx++) {
            ...
            if (Multi_ts_team[idx].multi_ts_flag[s_idx] >= 0) {
                ...
                change_ship_type (
                    Objects[objnum].instance,
                    Wss_slots_teams[idx][s_idx].ship_class);

                // set the ship weapons correctly
                wl_update_ship_weapons (objnum, &Wss_slots_teams[idx][s_idx]);

                // assign ts_index of the ship to point to the proper Wss_slots
                // slot
                ...
                Multi_ts_deleted_objnums[Multi_ts_num_deleted] =
                    Multi_ts_team[idx].multi_ts_objnum[s_idx];
                ...
                ship_add_exited_ship (
                    &Ships[Objects
                               [Multi_ts_deleted_objnums[Multi_ts_num_deleted]]
                                   .instance],
                    Ship::Exit_Flags::Player_deleted);
                obj_delete (Multi_ts_deleted_objnums[Multi_ts_num_deleted]);
                ship_wing_cleanup (
                    Objects[Multi_ts_deleted_objnums[Multi_ts_num_deleted]]
                        .instance,
                    &Wings[Ships[Objects[Multi_ts_team[idx]
                                             .multi_ts_objnum[s_idx]]
                                     .instance]
                               .wingnum]);
...
void multi_ts_handle_player_drop () {
    ...
    for (idx = 0; idx < MULTI_TS_MAX_TVT_TEAMS; idx++) {
        for (s_idx = 0; s_idx < MULTI_TS_NUM_SHIP_SLOTS; s_idx++) {
            // if we found him, clear his player slot and set his object back
            // to being  OF_COULD_BE_PLAYER
            if ((Multi_ts_team[idx].multi_ts_player[s_idx] != NULL) &&
                ...
                obj_set_flags (
                    &Objects[Multi_ts_team[idx].multi_ts_objnum[s_idx]],
                    Objects[Multi_ts_team[idx].multi_ts_objnum[s_idx]].flags +
...
// set the status bar to reflect the status of wing slots (free or not free). 0
// or 1 are valid values for now
void multi_ts_set_status_bar_mode (int m) { Multi_ts_status_bar_mode = m; }
...
void multi_ts_lock_pressed () {
    ...
    send_pslot_update_packet (
        Net_player->p_info.team, TS_CODE_LOCK_TEAM, interface_snd_id ());
...
void multi_ts_maybe_host_only_popup () {
    /*
        // if this is because the "host modifies" option is set
                    if((Netgame.options.flags & MSO_FLAG_SS_LEADERS) &&
       !(Net_player->flags & NETINFO_FLAG_GAME_HOST) && !(Net_player->flags &
       NETINFO_FLAG_TEAM_CAPTAIN)){ multi_ts_host_only_popup();
                    }

        if(Netgame.type == NG_TYPE_TEAM){
            popup(PF_USE_AFFIRMATIVE_ICON,1,POPUP_OK,"Only team captains may
       modify ships and weapons in this game"); } else {
            popup(PF_USE_AFFIRMATIVE_ICON,1,POPUP_OK,"Only the host may modify
       ships and weapons in this game");
        }
        */
}
...
void multi_ts_button_pressed (int n) {
    ...
    case MULTI_TS_SHIPS_DOWN: multi_ts_avail_scroll_down (); break;
...
void multi_ts_init_graphics () {
    ...
    Multi_ts_bitmap = mission_ui_background_load (
    ...
    for (idx = 0; idx < MULTI_TS_NUM_BUTTONS; idx++) {
        ...
        if ((idx == MULTI_TS_SHIPS_UP) || (idx == MULTI_TS_SHIPS_DOWN)) {
            Multi_ts_buttons[gr_screen.res][idx].button.create (
                &Multi_ts_window, "", Multi_ts_buttons[gr_screen.res][idx].x,
        ...
        Multi_ts_buttons[gr_screen.res][idx].button.set_highlight_action (
            common_play_highlight_sound);

        // set the ani for the button
        Multi_ts_buttons[gr_screen.res][idx].button.set_bmaps (
            Multi_ts_buttons[gr_screen.res][idx].filename);

        // set the hotspot
        Multi_ts_buttons[gr_screen.res][idx].button.link_hotspot (
            Multi_ts_buttons[gr_screen.res][idx].hotspot);
    }

    // add some text
    Multi_ts_window.add_XSTR (
        "Briefing", 765, Multi_ts_buttons[gr_screen.res][MULTI_TS_BRIEFING].xt,
        Multi_ts_buttons[gr_screen.res][MULTI_TS_BRIEFING].yt,
        &Multi_ts_buttons[gr_screen.res][MULTI_TS_BRIEFING].button,
        UI_XSTR_COLOR_GREEN);
    Multi_ts_window.add_XSTR (
        "Ship Selection", 1067,
        Multi_ts_buttons[gr_screen.res][MULTI_TS_SHIP_SELECT].xt,
        Multi_ts_buttons[gr_screen.res][MULTI_TS_SHIP_SELECT].yt,
        &Multi_ts_buttons[gr_screen.res][MULTI_TS_SHIP_SELECT].button,
        UI_XSTR_COLOR_GREEN);
    Multi_ts_window.add_XSTR (
        "Weapon Loadout", 1068,
        Multi_ts_buttons[gr_screen.res][MULTI_TS_WEAPON_SELECT].xt,
        Multi_ts_buttons[gr_screen.res][MULTI_TS_WEAPON_SELECT].yt,
        &Multi_ts_buttons[gr_screen.res][MULTI_TS_WEAPON_SELECT].button,
        UI_XSTR_COLOR_GREEN);
    Multi_ts_window.add_XSTR (
        "Commit", 1062, Multi_ts_buttons[gr_screen.res][MULTI_TS_COMMIT].xt,
        Multi_ts_buttons[gr_screen.res][MULTI_TS_COMMIT].yt,
        &Multi_ts_buttons[gr_screen.res][MULTI_TS_COMMIT].button,
        UI_XSTR_COLOR_PINK);
    Multi_ts_window.add_XSTR (
        "Lock", 1270, Multi_ts_buttons[gr_screen.res][MULTI_TS_LOCK].xt,
    ...
    // if we're not the host of the game (or a tema captain in team vs. team
    // mode), disable the lock button
    if (Netgame.type_flags & NG_TYPE_TEAM) {
...
void multi_ts_blit_wings () {
    ...
    for (idx = 0; idx < MAX_WSS_SLOTS; idx++) {
        ...
        if (Multi_ts_team[Net_player->p_info.team].multi_ts_flag[idx] ==
            MULTI_TS_FLAG_NONE) {
            continue;
        }
        ...
        if (Multi_ts_team[Net_player->p_info.team].multi_ts_flag[idx] ==
            MULTI_TS_FLAG_EMPTY) {
            ss_blit_ship_icon (
                Multi_ts_slot_icon_coords[idx][gr_screen.res]
                                         [MULTI_TS_X_COORD],
                Multi_ts_slot_icon_coords[idx][gr_screen.res]
                                         [MULTI_TS_Y_COORD],
                -1, 0);
            ...
            ss_blit_ship_icon (
                ...
                Multi_ts_slot_icon_coords[idx][gr_screen.res]
                                         [MULTI_TS_Y_COORD],
                Wss_slots[idx].ship_class, multi_ts_slot_bmap_num (idx));

            // if this is a team vs team game, and the slot is occupised by a
            // team captain, put a c there
            if ((Netgame.type_flags & NG_TYPE_TEAM) &&
                (Multi_ts_team[Net_player->p_info.team].multi_ts_player[idx] !=
                 NULL) &&
                ...
                gr_string (
                    ...
                    Multi_ts_slot_icon_coords[idx][gr_screen.res]
                                             [MULTI_TS_Y_COORD] -
                        5,
                    XSTR ("C", 737), GR_RESIZE_MENU); // [[ Team captain ]]
            }
        }
    }
}
...
void multi_ts_blit_wing_callsigns () {
    ...
    for (idx = 0; idx < MAX_WSS_SLOTS; idx++) {
        ...
        if (Multi_ts_team[Net_player->p_info.team].multi_ts_player[idx] !=
            NULL) {
            // make sure the string fits
            strcpy_s (
                callsign, Multi_ts_team[Net_player->p_info.team]
                              .multi_ts_player[idx]
                              ->m_player->callsign);
        }
        else {
            // determine if this is a locked AI ship
            pobj = mission_parse_get_arrival_ship (
                Ships[Objects[Multi_ts_team[Net_player->p_info.team]
                                  .multi_ts_objnum[idx]]
                          .instance]
                    .ship_name);
            if ((pobj == NULL) ||
                !(pobj->flags[Mission::Parse_Object_Flags::OF_Player_start])) {
                strcpy_s (callsign, NOX ("<"));
                strcat_s (
                    callsign,
                    XSTR ("AI", 738)); // [[ Artificial Intellegence ]]
        ...
        font::force_fit_string (
            ...
            Multi_ts_slot_text_coords[idx][gr_screen.res][MULTI_TS_W_COORD]);

        // get the final length
        gr_get_string_size (&callsign_w, NULL, callsign);

        // blit the string
        if ((Multi_ts_hotspot_type == MULTI_TS_PLAYER_LIST) &&
            (Multi_ts_hotspot_index == idx) &&
            (Multi_ts_team[Net_player->p_info.team].multi_ts_player[idx] !=
             NULL)) {
        ...
        gr_string (
            Multi_ts_slot_text_coords[idx][gr_screen.res][MULTI_TS_X_COORD] +
                ((Multi_ts_slot_text_coords[idx][gr_screen.res]
                                           [MULTI_TS_W_COORD] -
                  callsign_w) /
                 2),
            Multi_ts_slot_text_coords[idx][gr_screen.res][MULTI_TS_Y_COORD],
            callsign, GR_RESIZE_MENU);
    }
}
...
void multi_ts_blit_avail_ships () {
    ...
    for (idx = 0; idx < static_cast< int > (Ship_info.size ()); idx++) {
        if (Ss_pool[idx] > 0) {
            ...
            if (ship_count < Multi_ts_avail_start) { ship_count++; }
            else {
                // blit the icon
                ss_blit_ship_icon (
                    Multi_ts_avail_coords[display_count][gr_screen.res]
                                         [MULTI_TS_X_COORD],
                    Multi_ts_avail_coords[display_count][gr_screen.res]
                                         [MULTI_TS_Y_COORD],
                    idx, multi_ts_avail_bmap_num (display_count));
        ...
        if (display_count >= MULTI_TS_AVAIL_MAX_DISPLAY) { return; }
    }
}
...
void multi_ts_init_snazzy () {
    ...
    memset (
        Multi_ts_region, 0,
        sizeof (MENU_REGION) * MULTI_TS_NUM_SNAZZY_REGIONS);

    // get a pointer to the mask bitmap data
    Multi_ts_mask_data =
        Multi_ts_window.get_mask_data (&Multi_ts_mask_w, &Multi_ts_mask_h);

    // add the wing slots information
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_0_SHIP_0, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_0_SHIP_1, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_0_SHIP_2, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_0_SHIP_3, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_1_SHIP_0, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_1_SHIP_1, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_1_SHIP_2, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_1_SHIP_3, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_2_SHIP_0, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_2_SHIP_1, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_2_SHIP_2, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_2_SHIP_3, 0);

    // add the name slots information
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_0_NAME_0, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_0_NAME_1, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_0_NAME_2, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_0_NAME_3, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_1_NAME_0, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_1_NAME_1, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_1_NAME_2, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_1_NAME_3, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_2_NAME_0, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_2_NAME_1, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_2_NAME_2, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_2_NAME_3, 0);

    // add the available ships region
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_LIST_0, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_LIST_1, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_LIST_2, 0);
    snazzy_menu_add_region (
        &Multi_ts_region[Multi_ts_snazzy_regions++], "", TSWING_LIST_3, 0);
}

// what type of region the index is (0 == ship avail list, 1 == ship slots, 2
// == player slot)
int multi_ts_region_type (int region) {
    if ((region == TSWING_0_SHIP_0) || (region == TSWING_0_SHIP_1) ||
        (region == TSWING_0_SHIP_2) || (region == TSWING_0_SHIP_3) ||
        (region == TSWING_1_SHIP_0) || (region == TSWING_1_SHIP_1) ||
        (region == TSWING_1_SHIP_2) || (region == TSWING_1_SHIP_3) ||
        (region == TSWING_2_SHIP_0) || (region == TSWING_2_SHIP_1) ||
        (region == TSWING_2_SHIP_2) || (region == TSWING_2_SHIP_3)) {
        return MULTI_TS_SLOT_LIST;
    }

    if ((region == TSWING_0_NAME_0) || (region == TSWING_0_NAME_1) ||
        (region == TSWING_0_NAME_2) || (region == TSWING_0_NAME_3) ||
        (region == TSWING_1_NAME_0) || (region == TSWING_1_NAME_1) ||
        (region == TSWING_1_NAME_2) || (region == TSWING_1_NAME_3) ||
        (region == TSWING_2_NAME_0) || (region == TSWING_2_NAME_1) ||
        (region == TSWING_2_NAME_2) || (region == TSWING_2_NAME_3)) {
        return MULTI_TS_PLAYER_LIST;
    }

    if ((region == TSWING_LIST_0) || (region == TSWING_LIST_1) ||
        (region == TSWING_LIST_2) || (region == TSWING_LIST_3)) {
...
int multi_ts_slot_index (int region) {
    switch (region) {
    case TSWING_0_SHIP_0: return 0;
    case TSWING_0_SHIP_1: return 1;
    case TSWING_0_SHIP_2: return 2;
    case TSWING_0_SHIP_3: return 3;
    case TSWING_1_SHIP_0: return 4;
    case TSWING_1_SHIP_1: return 5;
    case TSWING_1_SHIP_2: return 6;
    case TSWING_1_SHIP_3: return 7;
    case TSWING_2_SHIP_0: return 8;
    case TSWING_2_SHIP_1: return 9;
    case TSWING_2_SHIP_2: return 10;
    case TSWING_2_SHIP_3: return 11;
    }
...
int multi_ts_avail_index (int region) {
    switch (region) {
    case TSWING_LIST_0: return 0;
    case TSWING_LIST_1: return 1;
    case TSWING_LIST_2: return 2;
    case TSWING_LIST_3: return 3;
    }
...
int multi_ts_player_index (int region) {
    switch (region) {
    case TSWING_0_NAME_0: return 0;
    case TSWING_0_NAME_1: return 1;
    case TSWING_0_NAME_2: return 2;
    case TSWING_0_NAME_3: return 3;
    case TSWING_1_NAME_0: return 4;
    case TSWING_1_NAME_1: return 5;
    case TSWING_1_NAME_2: return 6;
    case TSWING_1_NAME_3: return 7;
    case TSWING_2_NAME_0: return 8;
    case TSWING_2_NAME_1: return 9;
    case TSWING_2_NAME_2: return 10;
    case TSWING_2_NAME_3: return 11;
    }
...
void multi_ts_blit_ship_info () {
    ...
    if (Multi_ts_select_ship_class == -1) { return; }
    ...
    gr_set_color_fast (&Color_normal);
    gr_string (
        Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start,
        XSTR ("Class", 739), GR_RESIZE_MENU);
    if (strlen ((sip->alt_name[0]) ? sip->alt_name : sip->name)) {
        ...
        gr_string (
            Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150,
            y_start, temp, GR_RESIZE_MENU);
    }
    y_start += line_height;
    ...
    gr_string (
        Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start,
        XSTR ("Type", 740), GR_RESIZE_MENU);
    if ((sip->type_str != NULL) && strlen (sip->type_str)) {
        gr_set_color_fast (&Color_bright);
        gr_string (
            Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150,
            y_start, sip->type_str, GR_RESIZE_MENU);
    }
    y_start += line_height;
    ...
    gr_string (
        Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start,
        XSTR ("Length", 741), GR_RESIZE_MENU);
    if ((sip->ship_length != NULL) && strlen (sip->ship_length)) {
        gr_set_color_fast (&Color_bright);
        gr_string (
            Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150,
            y_start, sip->ship_length, GR_RESIZE_MENU);
    }
    y_start += line_height;
    ...
    gr_string (
        Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start,
        XSTR ("Max Velocity", 742), GR_RESIZE_MENU);
    ...
    if ((sip->maneuverability_str != NULL) &&
        strlen (sip->maneuverability_str)) {
        gr_set_color_fast (&Color_bright);
        gr_string (
            Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150,
            y_start, sip->maneuverability_str, GR_RESIZE_MENU);
    }
    y_start += line_height;
    ...
    gr_string (
        Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start,
        XSTR ("Armor", 745), GR_RESIZE_MENU);
    if ((sip->armor_str != NULL) && strlen (sip->armor_str)) {
        gr_set_color_fast (&Color_bright);
        gr_string (
            Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150,
            y_start, sip->armor_str, GR_RESIZE_MENU);
    }
    y_start += line_height;
    ...
    gr_string (
        Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start,
        XSTR ("Gun Mounts", 746), GR_RESIZE_MENU);
    if ((sip->gun_mounts != NULL) && strlen (sip->gun_mounts)) {
        gr_set_color_fast (&Color_bright);
        gr_string (
            Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150,
            y_start, sip->gun_mounts, GR_RESIZE_MENU);
    }
    y_start += line_height;
    ...
    gr_string (
        Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start,
        XSTR ("Missile Banks", 747), GR_RESIZE_MENU);
    if ((sip->missile_banks != NULL) && strlen (sip->missile_banks)) {
        gr_set_color_fast (&Color_bright);
        gr_string (
            Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150,
            y_start, sip->missile_banks, GR_RESIZE_MENU);
    }
    y_start += line_height;
    ...
    gr_string (
        Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start,
        XSTR ("Manufacturer", 748), GR_RESIZE_MENU);
    if ((sip->manufacturer_str != NULL) && strlen (sip->manufacturer_str)) {
        gr_set_color_fast (&Color_bright);
        gr_string (
            Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150,
            y_start, sip->manufacturer_str, GR_RESIZE_MENU);
    }
    y_start += line_height;
    ...
    for (int idx = 0; idx < Multi_ts_ship_info_line_count; idx++) {
        gr_string (
            Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD],
            y_start, Multi_ts_ship_info_lines[idx], GR_RESIZE_MENU);
        y_start += line_height;
    }
}
...
void multi_ts_blit_status_bar () {
    ...
    if (blit) {
        ...
        gr_string (
            Multi_ts_status_coords[gr_screen.res][MULTI_TS_X_COORD] +
                ((Multi_ts_status_coords[gr_screen.res][MULTI_TS_W_COORD] -
                  text_w) /
                 2),
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            ...
            if (MULTI_OBSERVER (Net_players[idx])) {
                ...
                Multi_ts_team[Net_players[idx].p_info.team]
                    .multi_ts_player[Net_players[idx].p_info.ship_index] =
                    &Net_players[idx];
            }
        }
    }
    ...
    while (objp != END_OF_LIST (&obj_used_list)) {
        ...
        if (objp->type == OBJ_SHIP) {
            ...
            if ((slot_index != -1) && (team_index != -1)) {
                Multi_ts_team[team_index].multi_ts_objnum[slot_index] =
                    Ships[objp->instance].objnum;
            }
        }
...
// function to return the shipname of the ship in the slot designated by the
...
void multi_ts_init_flags () {
    ...
    if (Netgame.type_flags & NG_TYPE_TEAM) {
        for (idx = 0; idx < MULTI_TS_MAX_TVT_TEAMS; idx++) {
            for (s_idx = 0; s_idx < MULTI_TS_NUM_SHIP_SLOTS_TEAM; s_idx++) {
                // if the there is an objnum here but no ship class, we know
                // its currently empty
                if ((Multi_ts_team[idx].multi_ts_objnum[s_idx] != -1) &&
    ...
    // in a non team vs. team situation
        ...
        for (idx = 0; idx < MULTI_TS_NUM_SHIP_SLOTS; idx++) {
            // if the there is an objnum here but no ship class, we know its
            // currently empty
            if ((Multi_ts_team[0].multi_ts_objnum[idx] != -1) &&
                ...
                (Multi_ts_team[0].multi_ts_objnum[idx] != -1) &&
                (Wss_slots[idx].ship_class != -1)) {
                Multi_ts_team[0].multi_ts_flag[idx] =
...
// handle an available ship scroll down button press
void multi_ts_avail_scroll_down () {
    if ((Multi_ts_avail_count - Multi_ts_avail_start) >
        MULTI_TS_AVAIL_MAX_DISPLAY) {
        gamesnd_play_iface (InterfaceSounds::USER_SELECT);
...
void multi_ts_handle_mouse () {
    ...
    snazzy_region = snazzy_menu_do (
        Multi_ts_mask_data, Multi_ts_mask_w, Multi_ts_mask_h,
        Multi_ts_snazzy_regions, Multi_ts_region, &snazzy_action, 0);
    ...
    if (snazzy_region != -1) {
        ...
        case MULTI_TS_AVAIL_LIST:
            ...
            if (ship_class == -1) { region_empty = 1; }
            else {
        ...
        case MULTI_TS_SLOT_LIST:
            ...
            if (region_index >= 0) {
                region_empty = (Multi_ts_team[Net_player->p_info.team]
                                    .multi_ts_flag[region_index] >= 0)
                                   ? 0
                                   : 1;
                if (!region_empty) {
    ...
    if (!Multi_ts_carried_flag && Multi_ts_clicked_flag &&
        !mouse_down (MOUSE_LEFT_BUTTON)) {
        Multi_ts_clicked_flag = 0;
    }

    // if we're currently not carrying anything and the user has clicked
    if (!Multi_ts_carried_flag && !Multi_ts_clicked_flag &&
        mouse_down (MOUSE_LEFT_BUTTON) && !region_empty) {
    ...
    if (!Multi_ts_carried_flag && Multi_ts_clicked_flag &&
        mouse_down (MOUSE_LEFT_BUTTON) &&
        ((Multi_ts_clicked_x != mouse_x) || (Multi_ts_clicked_y != mouse_y))) {
        // if this player is an observer, he shouldn't be able to do jack
        if (Net_player->flags & NETINFO_FLAG_OBSERVER) { return; }

        // first we check for illegal conditions (any case where he cannot grab
        // what he is attempting to grab)
        switch (region_type) {
        case MULTI_TS_AVAIL_LIST:
        ...
        case MULTI_TS_SLOT_LIST:
            ...
            if (region_index >= 0) {
                if (Multi_ts_team[Net_player->p_info.team]
                        .multi_ts_flag[region_index] < 0) {
                    return;
                }
            }
            break;
        case MULTI_TS_PLAYER_LIST:
            if (!multi_ts_can_grab_player (region_index)) { return; }
            break;
        }
    ...
    if (Multi_ts_carried_flag && !mouse_down (MOUSE_LEFT_BUTTON)) {
        ...
        if ((region_type == MULTI_TS_SLOT_LIST) &&
            multi_ts_disabled_slot (region_index)) {
            multi_ts_maybe_host_only_popup ();
        }

        // if we're over some kind of valid region, apply
        multi_ts_drop (
            Multi_ts_carried_from_type, Multi_ts_carried_from_index,
            region_type, region_index, Multi_ts_carried_ship_class);
    }
}
...
int multi_ts_can_perform (
    ...
    op_type = multi_ts_get_dnd_type (
        from_type, from_index, to_type, to_index, player_index);
    ...
    case TS_GRAB_FROM_LIST:
        ...
        if (Ss_pool_teams[pl->p_info.team][ship_class] <= 0) { return 0; }

        // if he's not allowed to touch the wing slot
        if (multi_ts_disabled_slot (to_index, player_index)) { return 0; }

        // if the slot he's trying to drop it on is "permanently" empty
        if (Multi_ts_team[pl->p_info.team].multi_ts_flag[to_index] ==
            MULTI_TS_FLAG_NONE) {
            return 0;
        }
        break;
    ...
    case TS_SWAP_SLOT_SLOT:
        ...
        if (multi_ts_disabled_slot (from_index, player_index) ||
            multi_ts_disabled_slot (to_index, player_index)) {
            return 0;
        }
    ...
    case TS_DUMP_TO_LIST:
        ...
        if (multi_ts_disabled_slot (from_index, player_index)) { return 0; }
    ...
    case TS_SWAP_PLAYER_PLAYER:
        ...
        if (Multi_ts_team[pl->p_info.team].multi_players_locked) { return 0; }

        // if there isn't a player at one of the positions
        if ((Multi_ts_team[pl->p_info.team].multi_ts_player[from_index] ==
             NULL) ||
            (Multi_ts_team[pl->p_info.team].multi_ts_player[to_index] ==
             NULL)) {
            return 0;
        }
        ...
        if (Multi_ts_team[pl->p_info.team].multi_ts_objnum[to_index] != -1) {
            pobj = mission_parse_get_arrival_ship (
                Ships[Objects[Multi_ts_team[pl->p_info.team]
                                  .multi_ts_objnum[to_index]]
                          .instance]
                    .ship_name);
            if ((pobj == NULL) ||
                !(pobj->flags[Mission::Parse_Object_Flags::OF_Player_start])) {
...
int multi_ts_get_dnd_type (
    ...
    case MULTI_TS_AVAIL_LIST:
        // do nothing
        if (to_type == MULTI_TS_AVAIL_LIST) { return -1; }
    ...
    case MULTI_TS_SLOT_LIST:
        if (to_type == MULTI_TS_SLOT_LIST) { return TS_SWAP_SLOT_SLOT; }
        if (to_type == MULTI_TS_AVAIL_LIST) { return TS_DUMP_TO_LIST; }
        break;

    // came from the player lists
    case MULTI_TS_PLAYER_LIST:
        if (to_type == MULTI_TS_PLAYER_LIST) {
            if (Multi_ts_team[pl->p_info.team].multi_ts_player[to_index] ==
                NULL) {
...
void multi_ts_apply (
    ...
    int type = multi_ts_get_dnd_type (
        from_type, from_index, to_type, to_index, player_index);
    ...
    case TS_SWAP_SLOT_SLOT:
        WARNINGF (
            LOCATION, "Apply swap slot slot %d %d\n", from_index, to_index);
        update = ss_swap_slot_slot (from_index, to_index, &sound);
        break;
    case TS_DUMP_TO_LIST:
        WARNINGF (
            LOCATION, "Apply dump to list %d %d\n", from_index, to_index);
        update = ss_dump_to_list (from_index, ship_class, &sound);
        break;
    case TS_SWAP_LIST_SLOT:
        WARNINGF (
            LOCATION, "Apply swap list slot %d %d\n", from_index, to_index);
        update = ss_swap_list_slot (ship_class, to_index, &sound);
        break;
    case TS_GRAB_FROM_LIST:
        WARNINGF (
            LOCATION, "Apply grab from list %d %d\n", from_index, to_index);
        update = ss_grab_from_list (ship_class, to_index, &sound);
        break;
    case TS_SWAP_PLAYER_PLAYER:
        WARNINGF (
            LOCATION, "Apply swap player player %d %d\n", from_index,
            to_index);
        update = multi_ts_swap_player_player (
            from_index, to_index, &sound, player_index);
        break;
    case TS_MOVE_PLAYER:
        WARNINGF (LOCATION, "Apply move player %d %d\n", from_index, to_index);
        update =
            multi_ts_move_player (from_index, to_index, &sound, player_index);
        break;
    default: update = 0; break;
    }
    ...
    if (update) {
        ...
        if (MULTIPLAYER_HOST) {
            ...
            if (type == TS_SWAP_PLAYER_PLAYER) {}
                ...
                // send a player slot update packet as well, so ship class
                // information, etc is kept correct
                send_pslot_update_packet (
                    pl->p_info.team, TS_CODE_PLAYER_UPDATE,
                    interface_snd_id ());
            }

            // if the player index == -1, it means the action was done locally
            // - so play a sound
...
void multi_ts_drop (
    ...
    if (Net_player->flags & NETINFO_FLAG_GAME_HOST) {
        ...
        if (multi_ts_can_perform (
                from_type, from_index, to_type, to_index, ship_class,
                player_index)) {
            multi_ts_apply (
                from_type, from_index, to_type, to_index, ship_class,
                player_index);
            ...
            // WARNINGF (LOCATION, "Could not apply operation!");
        }
    }
    // otherwise send a request to the host
    else {
        send_wss_request_packet (
            Net_player->player_id, from_type, from_index, to_type, to_index,
            -1, ship_class, WSS_SHIP_SELECT);
    }
}

// swap two player positions
int multi_ts_swap_player_player (
    int from_index, int to_index, interface_snd_id* sound, int player_index) {
    net_player *pl, *temp;
    ...
    Multi_ts_team[pl->p_info.team].multi_ts_player[to_index] =
        Multi_ts_team[pl->p_info.team].multi_ts_player[from_index];
    ...
    if (Multi_ts_team[pl->p_info.team].multi_ts_player[from_index] != NULL) {
        Multi_ts_team[pl->p_info.team]
            .multi_ts_player[from_index]
            ->p_info.ship_index = from_index;
        Multi_ts_team[pl->p_info.team]
            .multi_ts_player[from_index]
            ->p_info.ship_class =
        ...
        multi_assign_player_ship (
            NET_PLAYER_INDEX (
                Multi_ts_team[pl->p_info.team].multi_ts_player[from_index]),
            &Objects[Multi_ts_team[pl->p_info.team]
                         .multi_ts_objnum[from_index]],
            Wss_slots_teams[pl->p_info.team][from_index].ship_class);
    }
    if (Multi_ts_team[pl->p_info.team].multi_ts_player[to_index] != NULL) {
        Multi_ts_team[pl->p_info.team]
            .multi_ts_player[to_index]
            ->p_info.ship_index = to_index;
        Multi_ts_team[pl->p_info.team]
            .multi_ts_player[to_index]
            ->p_info.ship_class =
        ...
        multi_assign_player_ship (
            NET_PLAYER_INDEX (
                Multi_ts_team[pl->p_info.team].multi_ts_player[to_index]),
            &Objects[Multi_ts_team[pl->p_info.team].multi_ts_objnum[to_index]],
            Wss_slots_teams[pl->p_info.team][to_index].ship_class);
    }

    // update ship flags
    Objects[Multi_ts_team[pl->p_info.team].multi_ts_objnum[to_index]]
        .flags.remove (Object::Object_Flags::Could_be_player);
    Objects[Multi_ts_team[pl->p_info.team].multi_ts_objnum[to_index]]
        .flags.remove (Object::Object_Flags::Player_ship);
    if (Multi_ts_team[pl->p_info.team].multi_ts_player[to_index] != NULL) {
        Objects[Multi_ts_team[pl->p_info.team].multi_ts_objnum[to_index]]
    ...
    if (Net_player->flags & NETINFO_FLAG_GAME_HOST) {
        send_pslot_update_packet (
            pl->p_info.team, TS_CODE_PLAYER_UPDATE,
            InterfaceSounds::ICON_DROP_ON_WING);
...
int multi_ts_get_avail_ship_class (int index) {
    ...
    while ((ship_count >= 0) &&
        ...
        if (Ss_pool[class_index] > 0) { ship_count--; }

        if (ship_count >= 0) { class_index++; }
    }

    if (ship_count < 0) { return class_index; }
...
void multi_ts_blit_carried_icon () {
    ...
    if (!Multi_ts_carried_flag) { return; }
    ...
    case MULTI_TS_SLOT_LIST:
        offset_x = Multi_ts_slot_icon_coords[Multi_ts_carried_from_index]
                                            [gr_screen.res][MULTI_TS_X_COORD] -
                   Multi_ts_clicked_x;
        offset_y = Multi_ts_slot_icon_coords[Multi_ts_carried_from_index]
                                            [gr_screen.res][MULTI_TS_Y_COORD] -
                   Multi_ts_clicked_y;

        // blit the icon
        ss_blit_ship_icon (
            x + offset_x, y + offset_y, Multi_ts_carried_ship_class, 0);
        break;
    case MULTI_TS_AVAIL_LIST:
        offset_x = Multi_ts_avail_coords[Multi_ts_carried_from_index]
                                        [gr_screen.res][MULTI_TS_X_COORD] -
                   Multi_ts_clicked_x;
        offset_y = Multi_ts_avail_coords[Multi_ts_carried_from_index]
                                        [gr_screen.res][MULTI_TS_Y_COORD] -
                   Multi_ts_clicked_y;
        ...
        break;
    case MULTI_TS_PLAYER_LIST:
        // get the final length of the string so we can calculate a valid
        // offset
        strcpy_s (
            callsign, Multi_ts_team[Net_player->p_info.team]
                          .multi_ts_player[Multi_ts_carried_from_index]
                          ->m_player->callsign);
        font::force_fit_string (
            callsign, CALLSIGN_LEN,
            Multi_ts_slot_text_coords[Multi_ts_carried_from_index]
                                     [gr_screen.res][MULTI_TS_W_COORD]);
        gr_get_string_size (&callsign_w, NULL, callsign);
        ...
        offset_x =
            ...
            ((Multi_ts_slot_text_coords[Multi_ts_carried_from_index]
              ...
              callsign_w) /
             2);
        offset_y = Multi_ts_slot_text_coords[Multi_ts_carried_from_index]
                                            [gr_screen.res][MULTI_TS_Y_COORD] -
                   Multi_ts_clicked_y;
...
int multi_ts_avail_bmap_num (int slot_index) {
    ...
    if ((Multi_ts_select_type == MULTI_TS_AVAIL_LIST) &&
        (Multi_ts_select_index == slot_index)) {
        return ICON_FRAME_SELECTED;
    }

    // if its otherwise being lit by the mouse
    if ((Multi_ts_hotspot_type == MULTI_TS_AVAIL_LIST) &&
        (Multi_ts_hotspot_index == slot_index)) {
...
// should be displayed for the given slot
int multi_ts_slot_bmap_num (int slot_index) {
    // special case - slot is disabled, its my ship and the host hasn't locked
    // the ships yet
    ...
    if (multi_ts_disabled_slot (slot_index)) { return ICON_FRAME_DISABLED; }

    // if this slot has been highlighted for informational purposes
    if ((Multi_ts_select_type == MULTI_TS_SLOT_LIST) &&
        (Multi_ts_select_index == slot_index)) {
        return ICON_FRAME_SELECTED;
    }
    ...
    // if its otherwise being lit by the mouse
    if ((Multi_ts_hotspot_type == MULTI_TS_SLOT_LIST) &&
        (Multi_ts_hotspot_index == slot_index)) {
        return ICON_FRAME_HOT;
    }
...
void multi_ts_select_ship () {
    ...
    case MULTI_TS_AVAIL_LIST:
        Multi_ts_select_ship_class =
            multi_ts_get_avail_ship_class (Multi_ts_select_index);
        // if he has selected an empty slot, don't do anything
        if (Multi_ts_select_ship_class < 0) { return; }
        break;
    default:
    ...
    if (Ship_info[Multi_ts_select_ship_class].desc != NULL) {
        ...
        if (Multi_ts_ship_info_text[0] != '\0') {
            ...
            n_lines = split_str (
                Multi_ts_ship_info_text,
                Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_W_COORD],
                n_chars, p_str, MULTI_TS_SHIP_INFO_MAX_LINES, 0);
...
// handle all details when the commit button is pressed (including possibly
// reporting errors/popups)
void multi_ts_commit_pressed () {
    // if my team's slots are still not "locked", we cannot commit unless we're
    ...
    if (!Multi_ts_team[Net_player->p_info.team].multi_players_locked) {
        if (multi_num_players () != 1) {
            popup (
                ...
                XSTR (
                    "Players have not yet been assigned to their ships", 751));
            return;
        }
        else {
            Multi_ts_team[Net_player->p_info.team].multi_players_locked = 1;
...
int multi_ts_ok_to_commit () {
    ...
    if (Net_player->flags & NETINFO_FLAG_OBSERVER) { return 0; }

    for (idx = 0; idx < MULTI_TS_NUM_SHIP_SLOTS; idx++) {
        // if this is a player slot this player can modify and it is empty,
        // then he cannot continue implies there is never an object in this
        // slot
        if ((Multi_ts_team[Net_player->p_info.team].multi_ts_objnum[idx] !=
             -1) &&
            ...
            ((Multi_ts_team[Net_player->p_info.team].multi_ts_player[idx] !=
              NULL) &&
             (Multi_ts_team[Net_player->p_info.team].multi_ts_flag[idx] ==
              MULTI_TS_FLAG_EMPTY))) {
            return 1;
        }

        // if the ship in this slot has a ship which can be a player but has 0
        // primary or secondary weapons, then he cannot continue
        if ((Multi_ts_team[Net_player->p_info.team].multi_ts_objnum[idx] !=
             -1) &&
            ((Objects[Multi_ts_team[Net_player->p_info.team]
                          .multi_ts_objnum[idx]]
            ...
            for (s_idx = 0; s_idx < MAX_SHIP_WEAPONS; s_idx++) {
                ...
                if ((Wss_slots_teams[Net_player->p_info.team][idx]
                         .wep[s_idx] >= 0) &&
                    (Wss_slots_teams[Net_player->p_info.team][idx]
                         .wep_count[s_idx] > 0)) {
                    switch (
                        Weapon_info[Wss_slots_teams[Net_player->p_info.team]
                                                   [idx]
                                                       .wep[s_idx]]
                            .subtype) {
                    case WP_LASER: primary_ok = 1; break;

                    case WP_MISSILE: secondary_ok = 1; break;
                ...
                if (primary_ok && secondary_ok) { break; }
            }

            // if the ship doesn't have primary weapons
            if (!primary_ok &&
                !(The_mission.ai_profile->flags
                      [AI::Profile_Flags::Multi_allow_empty_secondaries])) {
                return 2;
            }
...
void multi_ts_check_errors () {
    ...
    for(idx=0;idx<MULTI_TS_NUM_SHIP_SLOTS;idx++){
        ...
        shipp =
    &Ships[Objects[Multi_ts_team[0].multi_ts_objnum[idx]].instance];
        Assert((shipp->weapons.current_primary_bank != -1) &&
    (shipp->weapons.current_secondary_bank != -1));
    }
    */
}
    ...
    case TS_CODE_PLAYER_UPDATE:
        ...
        for (idx = 0; idx < MAX_WSS_SLOTS; idx++) {
            if (Multi_ts_team[team].multi_ts_flag[idx] != MULTI_TS_FLAG_NONE) {
                ...
                if (val) {
                    ADD_SHORT (
                        Multi_ts_team[team].multi_ts_player[idx]->player_id);

                    // should also update his p_info settings locally
                    Multi_ts_team[team]
                        .multi_ts_player[idx]
                        ->p_info.ship_class =
                        Wss_slots_teams[team][idx].ship_class;
                    Multi_ts_team[team]
                        .multi_ts_player[idx]
                        ->p_info.ship_index = idx;
                }

                // add a byte indicating what object flag should be set (0 ==
                // ~(OF_COULD_BE_PLAYER | OF_PLAYER_SHIP), 1 == player ship, 2
                // == could be player ship)
                if (Objects[Multi_ts_team[team].multi_ts_objnum[idx]]
                        .flags[Object::Object_Flags::Could_be_player]) {
                    val = 2;
                }
                else if (Objects[Multi_ts_team[team].multi_ts_objnum[idx]]
                             .flags[Object::Object_Flags::Player_ship]) {
                    val = 1;
                }
                else {
...
void process_pslot_update_packet (ubyte* data, header* hinfo) {
    ...
    case TS_CODE_LOCK_TEAM:
        ...
        if ((team == Net_player->p_info.team) &&
        ...
        // if this is the standalone server, we need to re-route the packet
        // here and there
        if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
    ...
    case TS_CODE_PLAYER_UPDATE:
        ...
        while (stop != 0xff) {
            ...
            if (val) {
                ...
                if (player_index == -1) {
                    WARNINGF (
                        LOCATION, "Couldn't find player for pslot update!");
                    Multi_ts_team[team].multi_ts_player[slot_num] = NULL;
                }
                // if we found him, assign him to this ship
                else {
                    Net_players[player_index].p_info.ship_class =
                        (int)ship_class;
                    Net_players[player_index].p_info.ship_index =
                        (int)slot_num;
                    multi_assign_player_ship (
                        player_index, &Objects[objnum], (int)ship_class);

                    // ui stuff
                    Multi_ts_team[team].multi_ts_player[slot_num] =
                        &Net_players[player_index];

                    // if this was me and my ship index changed, update the
        ...
        if ((sound.isValid ()) && !(Game_mode & GM_STANDALONE_SERVER) &&
            (gameseq_get_state () == GS_STATE_TEAM_SELECT)) {
            gamesnd_play_iface (sound);
        }
