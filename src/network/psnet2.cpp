...
// defintion of structures that actually leave this machine.  psnet_send give
// us only the data that we want to send.  We will add a header onto this data
// (packet sequence number, possibly a checksum).  We must include a 2 byte
// flags variable into both structure since the receiving end of this packet
// must know whether or not to checksum the packet.

#define MAX_TOP_LAYER_PACKET_SIZE 680

// use the pack pragma to pack these structures to 2 byte aligment.  Really
// only needed for the naked packet.
...
#define MAXNETBUFFERS \
    150 // Maximum network buffers (For between network and upper level
        // functions, which is required in case of out of order packets
#define NETRETRYTIME 0.75f // Time after sending before we resend
#define MIN_NET_RETRYTIME 0.2f
#define NETTIMEOUT \
    30 // Time after receiving the last packet before we drop that user
...
#define RELIABLE_CONNECT_TIME \
    7 // how long we'll wait for a response when doing a reliable connect
...
typedef struct {
    ...
    float send_time; // Time the packet was sent, if an ACK the time the packet
                     // being ACK'd was sent.
    ubyte data[NETBUFFERSIZE]; // Packet data
} reliable_header;

#define RELIABLE_PACKET_HEADER_ONLY_SIZE \
    (sizeof (reliable_header) - NETBUFFERSIZE)
...
typedef struct {
    reliable_net_sendbuffer*
        sbuffers[MAXNETBUFFERS]; // This is an array of pointers for quick
                                 // sorting
    unsigned short ssequence[MAXNETBUFFERS]; // This is the sequence number of
                                             // the given packet
    ...
    unsigned short rsequence[MAXNETBUFFERS]; // This is the sequence number of
                                             // the given packet
    float last_packet_received; // For a given connection, this is the last
                                // packet we received
    float last_packet_sent;
    SOCKADDR addr;                // SOCKADDR of our peer
    ushort status;                // Status of this connection
    unsigned short oursequence;   // This is the next sequence number the
                                  // application is expecting
    unsigned short theirsequence; // This is the next sequence number the peer
                                  // is expecting
...
#define CONNECTSEQ \
    0x142 // Magic number for starting a connection, just so it isn't 0
...
// returns the ip address of this machine. use for calling bind() with to
// associate with the proper IP address and network device.
int psnet_get_ip ();
...
void psnet_buffer_packet (
    network_packet_buffer_list* l, ubyte* data, int length, net_addr* from);

// get the index of the next packet in order!
int psnet_buffer_get_next (
    network_packet_buffer_list* l, ubyte* data, int* length, net_addr* from);
...
int RECVFROM (
    ...
    // if we have no buffer! The user should have made sure this wasn't the
    // case by calling SELECT()
...
int SELECT (
    ...
    struct timeval* timeout, int psnet_type) {
    network_packet_buffer_list* l;
 ...
 */
int SENDTO (
    SOCKET s, char* buf, int len, int flags, sockaddr* to, int tolen,
    int psnet_type) {
...
void PSNET_TOP_LAYER_PROCESS () {
    ...
    while (1) {
        // check if there is any data on the socket to be read.  The amount of
        // data that can be atomically read is stored in len.
        ...
        if (!FD_ISSET (Unreliable_socket, &rfds)) { return; }
        ...
        case NET_TCP:
            ...
            read_len = recvfrom (
                Unreliable_socket, (char*)packet_read.data,
        ...
        if ((packet_type >= 0) && (packet_type < PSNET_NUM_TYPES)) {
            // buffer the packet
            psnet_buffer_packet (
                &Psnet_top_buffers[packet_type], packet_read.data + 1,
                read_len - 1, &from_addr);
        }
    }
}
...
int psnet_use_protocol (int protocol) {
    ...
    case NET_TCP:
        if (Network_status != NETWORK_STATUS_RUNNING) {
            ml_string (
                "Network_status != NETWORK_STATUS_RUNNING in NET_TCP in "
                "psnet_use_protocol()");
            return 0;
        }
        ...
        Can_broadcast = Tcp_can_broadcast;
        if (Can_broadcast) { ml_string ("Psnet : TCP broadcast"); }

        // get the socket name for the TCP_socket, and put it into My_addr
        len = sizeof (SOCKADDR_IN);
        if (getsockname (TCP_socket, (SOCKADDR*)&ip_addr, &len) ==
            SOCKET_ERROR) {
            ml_printf (
                "Unable to get sock name for TCP unreliable socket (%d)",
                WSAGetLastError ());
            return 0;
        }
        ...
        custom_ip =
            os_config_read_string (NOX ("Network"), NOX ("CustomIP"), NULL);
        ...
        if (custom_ip != NULL) {
            ...
            if ((custom_address.sin_addr.s_addr = inet_addr (custom_ip)) !=
                ...
                ml_printf (
                    "WARNING  =>  psnet_get_ip() custom IP is invalid: %s",
                    custom_ip);
            }
        }

        memcpy (
            Psnet_my_addr.addr, &ip_addr.sin_addr, sizeof (ip_addr.sin_addr));
...
int psnet_get_network_status () {
    ...
    // network is running -- be sure that the RAS people know to connect if
    // they currently cannot.

    if (Psnet_connection == NETWORK_CONNECTION_DIALUP) {
        // if on a dialup connection, be sure that RAS is active.
        if (!Ras_connected) { return NETWORK_ERROR_CONNECT_TO_ISP; }
    }
    else if (Psnet_connection == NETWORK_CONNECTION_LAN) {
        // if on a LAN, and they have a dialup connection active, return error
        // to indicate that they need to pick the right connection type
        if (Ras_connected) { return NETWORK_ERROR_LAN_AND_RAS; }
    }
...
void psnet_string_to_addr (net_addr* address, char* text) {
    ...
    case NET_TCP:
        ...
        // if we get INADDR_NONE returns, then we need to try and resolve the
        // host name
        if (addr.s_addr == INADDR_NONE) {
            ...
            if (he) {
                addr.s_addr =
                    ((in_addr*)(he->h_addr))
                        ->s_addr; // this is the address in network byte order
            }
            else {
...
int psnet_send (net_addr* who_to, void* data, int len, int np_index) {
    ...
    if (psnet_same (who_to, &Psnet_my_addr)) { return 0; }
    ...
    if (!FD_ISSET (send_sock, &wfds)) { return 0; }
    ...
    case NET_TCP:
        ...
        ret = SENDTO (
            send_sock, (char*)send_data, send_len, 0, (SOCKADDR*)&sockaddr,
            sizeof (sockaddr), PSNET_TYPE_UNRELIABLE);
        break;
...
int psnet_get (void* data, net_addr* from_addr) {
    ...
    if (psnet_buffer_get_next (
            &Psnet_top_buffers[PSNET_TYPE_UNRELIABLE], (ubyte*)data,
            &buffer_size, from_addr)) {
        return buffer_size;
    }
...
int psnet_broadcast (net_addr* who_to, void* data, int len) {
    ...
    // broadcasting works on a local subnet which is all we really want to do
    // for now anyway. we might keep this in as an option for freespace later.
    switch (who_to->type) {
    case NET_TCP:
...
int psnet_is_valid_ip_string (char* ip_string, int /*allow_port*/) {
    ...
    if (addr.s_addr != INADDR_NONE) {
        ...
        if (psnet_is_valid_numeric_ip (ip_string)) { return 1; }
    }
...
void psnet_rel_send_ack (
    ...
    case NET_TCP:
        ...
        ret = SENDTO (
            ...
            RELIABLE_PACKET_HEADER_ONLY_SIZE + ack_header.data_len, 0, raddr,
            sizeof (SOCKADDR), PSNET_TYPE_RELIABLE);
        if (ret == -1) { ml_string ("TCP SENDTO failed in rel_send_ack()"); }
        break;
    default: ml_string ("Unknown protocol type in nw_SendReliable()"); break;
    }
}
 ...
 *
 * It takes a couple of things into consideration when closing, such as
 * possibly reiniting reliable sockets if they are closed here.
 */
void psnet_rel_close_socket (PSNET_SOCKET_RELIABLE* sockp) {
    ...
    if (*sockp >= MAXRELIABLESOCKETS) {
        if (*sockp != INVALID_SOCKET) {
            ml_printf (
                "Invalid socket id passed to nw_NewCloseSocket() -- %d",
                *sockp);
        }
    ...
    if (*sockp == Serverconn) { Serverconn = 0xffffffff; }
    switch (Reliable_sockets[*sockp].connection_type) {
    case NET_TCP:
        if (!Tcp_active) { return; }
        SENDTO (
            Unreliable_socket, (char*)&diss_conn_header,
            RELIABLE_PACKET_HEADER_ONLY_SIZE, 0,
            &Reliable_sockets[*sockp].addr, sizeof (SOCKADDR),
            PSNET_TYPE_RELIABLE);
        break;
    default: ml_string ("Unknown protocol type in nw_CloseSocket()!"); break;
    }
    memset (&Reliable_sockets[*sockp], 0, sizeof (reliable_socket));
    Reliable_sockets[*sockp].status = RNF_UNUSED;
}

// function to check the status of the reliable socket and try to re-initialize
// it if necessary. win95 seems to have trouble doing a reinit of the socket
// immediately after close, so this function exists to check the status, and
// reinitialize if we need to
int psnet_rel_check () { return 1; }

/**
 * Send data reliably
 */
int psnet_rel_send (
    PSNET_SOCKET_RELIABLE socketid, ubyte* data, int length, int np_index) {
    int i;
    int bytesout = 0;
    reliable_socket* rsocket;
    ...
    if (rsocket->status != RNF_CONNECTED) {
        ...
        ml_printf (
            "Can't send packet because of status %d in nw_SendReliable(). "
            "socket = %d",
    ...
    for (i = 0; i < MAXNETBUFFERS; i++) {
        if (NULL == rsocket->sbuffers[i]) {
            ...
            rsocket->sbuffers[i] = (reliable_net_sendbuffer*)vm_malloc (
                sizeof (reliable_net_sendbuffer));
            ...
            if (send_this_packet) {
                ...
                case NET_TCP:
                    ...
                    multi_rate_add (
                        np_index, "tcp(h)",
                        RELIABLE_PACKET_HEADER_ONLY_SIZE +
                            rsocket->send_len[i]);
                    bytesout = SENDTO (
                        Unreliable_socket, (char*)&send_header,
                        RELIABLE_PACKET_HEADER_ONLY_SIZE +
                            rsocket->send_len[i],
                        0, &rsocket->addr, sizeof (SOCKADDR),
                        PSNET_TYPE_RELIABLE);
                    break;
                default:
            ...
            if ((bytesout == SOCKET_ERROR) &&
                ...
                // This will cause it to try to send again next frame. (or
                // sooner)
...
int psnet_rel_get (PSNET_SOCKET socketid, ubyte* buffer, int /*max_len*/) {
    ...
    if (socketid >= MAXRELIABLESOCKETS) {
        ml_printf (
            "Invalid socket id passed to nw_NewReceiveReliable() -- %d",
            socketid);
        return -1;
    }
    rsocket = &Reliable_sockets[socketid];
    if ((RNF_CONNECTED != rsocket->status) && (RNF_LIMBO != rsocket->status)) {
        ml_printf (
            "Can't receive packet because it isn't connected in "
            "nw_ReceiveReliable(). socket = %d",
            socketid);
        return 0;
    }
    ...
    for (i = 0; i < MAXNETBUFFERS; i++) {
        if ((rsocket->rsequence[i] == rsocket->oursequence) &&
            rsocket->rbuffers[i]) {
            memcpy (
                buffer, rsocket->rbuffers[i]->buffer, rsocket->recv_len[i]);
...
void psnet_rel_work () {
    ...
    if (Serverconn != 0xffffffff) {
        ...
        if ((Reliable_sockets[Serverconn].status == RNF_LIMBO) &&
            ((Serverconn != 0xffffffff) &&
             fl_abs ((psnet_get_time () - Last_sent_iamhere)) >
                 NETRETRYTIME)) {
            ...
            case NET_TCP:
                if (!Tcp_active) {
                    ml_string (
                        "Unable to use this network connection type in "
                        "nw_WorkReliable()");
                    Int3 ();
                    return;
                }
                ret = SENDTO (
                    Unreliable_socket, (char*)&conn_header,
                    RELIABLE_PACKET_HEADER_ONLY_SIZE, 0,
                    &Reliable_sockets[Serverconn].addr, sizeof (SOCKADDR),
                    PSNET_TYPE_RELIABLE);
                break;
            default:
            ...
            if ((ret == SOCKET_ERROR) &&
                (WSAEWOULDBLOCK == WSAGetLastError ())) {
                Reliable_sockets[Serverconn].last_packet_sent =
                    psnet_get_time () - NETRETRYTIME;
            }
            else {
                Reliable_sockets[Serverconn].last_packet_sent =
                    psnet_get_time ();
    ...
    do {
        ...
        if (bytesin == -1) {
            ml_printf (
                "recvfrom returned an error! -- %d", WSAGetLastError ());
            return;
        }
        if (bytesin) {
            ...
            if (rcv_buff.type == RNT_REQ_CONN) {
                for (i = 1; i < MAXRELIABLESOCKETS; i++) {
                    if ((Reliable_sockets[i].status == RNF_CONNECTED) ||
                        (Reliable_sockets[i].status == RNF_LIMBO)) {
                        if (memcmp (
                                &d3_rcv_addr, &Reliable_sockets[i].m_net_addr,
                                sizeof (net_addr)) == 0) {
                            // We already have a reliable link to this user, so
                            // we will ignore it...
                            ml_printf (
                                "Received duplicate connection request. %d",
                                i);
                            psnet_rel_send_ack (
                                &Reliable_sockets[i].addr, rcv_buff.seq,
                                link_type, rcv_buff.send_time);
                            // We will change this as a hack to prevent later
                            // code from hooking us up
                            rcv_buff.type = 0xff;
                            continue;
                        }
                    }
                ...
                for (i = 1; i < MAXRELIABLESOCKETS; i++) {
                    if (Reliable_sockets[i].status == RNF_UNUSED) {
                        ...
                        memcpy (
                            &Reliable_sockets[i].m_net_addr, &d3_rcv_addr,
                            sizeof (net_addr));
                        memcpy (
                            &Reliable_sockets[i].addr, &rcv_addr,
                            sizeof (SOCKADDR));
                        Reliable_sockets[i].ping_pos = 0;
                        Reliable_sockets[i].num_ping_samples = 0;
                        Reliable_sockets[i].status = RNF_LIMBO;
                        Reliable_sockets[i].last_packet_received =
                            psnet_get_time ();
                ...
                psnet_rel_send_ack (
                    &rsocket->addr, rcv_buff.seq, link_type,
                    rcv_buff.send_time);
            }

            // Find out if this is a packet from someone we were expecting a
            // packet.
            rcvaddr = (SOCKADDR_IN*)&rcv_addr;
            for (i = 1; i < MAXRELIABLESOCKETS; i++) {
                if (memcmp (
            ...
            if (rcv_buff.type == RNT_HEARTBEAT) { continue; }
            if (rcv_buff.type == RNT_ACK) {
                // Update ping time
                rsocket->num_ping_samples++;

                rsocket->pings[rsocket->ping_pos] =
                    rsocket->last_packet_received - rcv_buff.send_time;
                if (rsocket->num_ping_samples >= MAX_PING_HISTORY) {
                    ...
                    rsocket->mean_ping =
                        ((sort_ping[MAX_PING_HISTORY / 2] +
                          sort_ping[(MAX_PING_HISTORY / 2) + 1])) /
                        2;
                }
                rsocket->ping_pos++;
                ...
                // if this is an ack for a send buffer on the socket, kill the
                // send buffer. its done
                for (i = 0; i < MAXNETBUFFERS; i++) {
            ...
            if (rcv_buff.type == RNT_DATA_COMP) {
                // More2Come
                // Decompress it. Put it back in the buffer. Process it as
                // RNT_DATA
                rcv_buff.type = RNT_DATA;
            }
            if (rcv_buff.type == RNT_DATA) {
                // If the data is out of order by >= MAXNETBUFFERS-1 ignore
                // that packet for now
                int seqdelta;
                ...
                if (seqdelta >= MAXNETBUFFERS - 1) {
                    ...
                    // It's out of order, so we won't ack it, which will mean
                    // we will get it again soon.
                    continue;
                }
                ...
                if (rsocket->oursequence < (0xffff - (MAXNETBUFFERS - 1))) {
                    ...
                    if (((unsigned short)(rcv_buff.seq + rsocket->oursequence)) >
                        (MAXNETBUFFERS - 1)) {
                        savepacket = 0;
                    }
                }
                ...
                if (savepacket) {
                    if (rcv_buff.data_len > max_len) {
                        ...
                        // don't ack it, which will mean we will get it again
                        // soon.
                        continue;
                    }
                    for (i = 0; i < MAXNETBUFFERS; i++) {
                        if (NULL == rsocket->rbuffers[i]) {
                            ...
                            rsocket->rbuffers[i] =
                                (reliable_net_rcvbuffer*)vm_malloc (
                                    sizeof (reliable_net_rcvbuffer));
                            memcpy (
                                rsocket->rbuffers[i]->buffer, rcv_buff.data,
                                rsocket->recv_len[i]);
                            rsocket->rsequence[i] = rcv_buff.seq;
                            break;
                        }
    ...
    // Go through each reliable socket that is connected and do any needed
    // work.
    for (j = 0; j < MAXRELIABLESOCKETS; j++) {
        rsocket = &Reliable_sockets[j];
        ...
        if (Serverconn == 0xffffffff) {
            if (rsocket->status == RNF_LIMBO) {
                if (fl_abs (
                        (psnet_get_time () - rsocket->last_packet_received)) >
                    Nettimeout) {
                    ml_printf (
                        "Reliable (but in limbo) socket (%d) timed out in "
                        "nw_WorkReliable().",
                        j);
                    memset (rsocket, 0, sizeof (reliable_socket));
                    rsocket->status = RNF_UNUSED; // Won't work if this is an
                                                  // outgoing connection.
                }
            }
        }
            ...
            if ((rsocket->status == RNF_LIMBO) &&
                (fl_abs ((psnet_get_time () - First_sent_iamhere)) >
                 Nettimeout)) {
        ...
        if (rsocket->status == RNF_CONNECTED) {
            ...
            if ((rsocket->mean_ping == 0) ||
                (rsocket->mean_ping > (NETRETRYTIME * 4))) {
                ...
                if (rsocket->mean_ping < MIN_NET_RETRYTIME) {
                    ...
                    retry_packet_time =
                        ((float)(float)rsocket->mean_ping * (float)1.25);
                }
            }
            ...
            for (i = 0; i < MAXNETBUFFERS; i++) {
                // send again
                if ((rsocket->sbuffers[i]) &&
                    (fl_abs ((psnet_get_time () - rsocket->timesent[i])) >=
                     retry_packet_time)) {
                    reliable_header send_header;
                    send_header.send_time = psnet_get_time ();
            ...
            if ((rsocket->status == RNF_CONNECTED) &&
                (fl_abs ((psnet_get_time () - rsocket->last_packet_sent)) >
                 NETHEARTBEATTIME)) {
                reliable_header send_header;
                send_header.send_time = psnet_get_time ();
                send_header.send_time = INTEL_FLOAT (&send_header.send_time);
                ...
                rcode = SENDTO (
                    Unreliable_socket, (char*)&send_header,
                    RELIABLE_PACKET_HEADER_ONLY_SIZE, 0, &rsocket->addr,
                    sizeof (SOCKADDR), PSNET_TYPE_RELIABLE);
            ...
            if ((rsocket->status == RNF_CONNECTED) &&
                (fl_abs ((psnet_get_time () - rsocket->last_packet_received)) >
                 Nettimeout)) {
...
int psnet_rel_get_status (PSNET_SOCKET_RELIABLE socketid) {
    if (socketid >= MAXRELIABLESOCKETS) { return -1; }
...
/**
 * Attempt to connect() to the server's tcp socket.  socket parameter is simply
 * assigned to the Reliable_socket socket created in psnet_init
 */
void psnet_rel_connect_to_server (
    PSNET_SOCKET* socket, net_addr* server_addr) {
    ...
    *socket = INVALID_SOCKET;
    ...
    if ((server_addr->type == NET_TCP) && (!Tcp_active)) { return; }
    ...
    if (Tcp_active && (Socket_type == NET_TCP)) {
        ...
        while (SELECT (
            ...
            bytesin = RECVFROM (
                Unreliable_socket, (char*)&ack_header,
                sizeof (reliable_header), 0, (SOCKADDR*)&rcv_addr, &addrlen,
                PSNET_TYPE_RELIABLE);
            if (bytesin == -1) {
                ml_printf (
    ...
    case NET_TCP:
        ...
        if (SOCKET_ERROR == SENDTO (
                                Unreliable_socket, (char*)&conn_header,
                                RELIABLE_PACKET_HEADER_ONLY_SIZE, 0, addr,
                                sizeof (SOCKADDR), PSNET_TYPE_RELIABLE)) {
            ml_printf (
    ...
    do {
        ...
        if (SELECT (
            ...
            addrlen = sizeof (SOCKADDR);
            bytesin = RECVFROM (
                typeless_sock, (char*)&ack_header, sizeof (reliable_header), 0,
                    ...
                    if (*acknum == CONNECTSEQ) {
                        for (i = 1; i < MAXRELIABLESOCKETS; i++) {
                            if (Reliable_sockets[i].status == RNF_UNUSED) {
                                // Add the new connection here.
                                memset (
                                    &Reliable_sockets[i], 0,
                                    sizeof (reliable_socket));
                                Reliable_sockets[i].connection_type =
                                    (ubyte)server_addr->type;
                                memcpy (
                                    &Reliable_sockets[i].m_net_addr,
                                    &d3_rcv_addr, sizeof (net_addr));
                                Reliable_sockets[i].last_packet_received =
                                    psnet_get_time ();
                                memcpy (
                                    &Reliable_sockets[i].addr, &rcv_addr,
                                ...
                                *socket = i;
                                ml_string (
                                    "Successfully connected to server in "
                                    "nw_ConnectToServer().");
                                // Now send I_AM_HERE packet
                                conn_header.type = RNT_I_AM_HERE;
                                conn_header.seq = (ushort) (~CONNECTSEQ);
                                ...
                                Last_sent_iamhere = psnet_get_time ();
                                int rcode = SENDTO (
                                    typeless_sock, (char*)&conn_header,
                                    RELIABLE_PACKET_HEADER_ONLY_SIZE, 0, addr,
                                    sizeof (SOCKADDR), PSNET_TYPE_RELIABLE);
                                if (rcode == SOCKET_ERROR) {
                                    *socket = INVALID_SOCKET;
                                    ...
                                    memset (
                                        ...
                                        sizeof (reliable_socket));
                                    ml_string (
                                        "Unable to send packet in "
                                        "nw_ConnectToServer()");
                                    return;
                                }
                                Reliable_sockets[i].last_packet_sent =
                                    psnet_get_time ();
                                float f;
                                f = psnet_get_time ();
                                while (
                                    (fl_abs ((psnet_get_time () - f)) < 2) &&
                                    (Reliable_sockets[i].status !=
                                     RNF_CONNECTING)) {
                                    psnet_rel_work ();
                                }
                        ...
                        ml_string (
                            "Out of reliable socket space in "
                            "nw_ConnectToServer().");
                        return;
                    }
                    else
                        ml_string (
                            "Received out of sequence ACK in "
                            "nw_ConnectToServer().");
                }
                else
                    ml_string (
                        "Received something that isn't an ACK in "
                        "nw_ConnectToServer().");
            }
            else
                ml_string (
                    "Received 0 bytes from recvfrom() in "
                    "nw_ConnectToServer().");
        }
    } while (fl_abs ((psnet_get_time () - first_sent_req)) <
             RELIABLE_CONNECT_TIME);
}
...
void psnet_buffer_init (network_packet_buffer_list* l) {
    ...
    memset (
        l->psnet_buffers, 0,
        sizeof (network_packet_buffer) * MAX_PACKET_BUFFERS);
...
void psnet_buffer_packet (
    ...
    if (!found_buf) { ml_string ("WARNING - Buffer overrun in psnet"); }
    else {
        // copy in the data
...
int psnet_is_valid_numeric_ip (char* ip) {
    ...
    if (token == NULL) { return 0; }
        ...
        // get the value of the token
        val4 = atoi (token);
        if ((val4 < 0) || (val4 > 255)) { return 0; }
    }

    // make sure he hasn't entered all 0's
    if ((val1 == 0) && (val2 == 0) && (val3 == 0) && (val4 == 0)) { return 0; }
...
void psnet_socket_options (SOCKET sock) {
    ...
    broadcast = 1;
    if (setsockopt (
            sock, SOL_SOCKET, SO_BROADCAST, (const char*)&broadcast,
            sizeof (broadcast))) {
        Can_broadcast = 0;
    }
    else {
    ...
    setsockopt (
        sock, SOL_SOCKET, SO_RCVBUF, (char*)&bufsize, sizeof (bufsize));
    getsockopt (sock, SOL_SOCKET, SO_RCVBUF, (char*)&cursize, &cursizesize);
    ...
    setsockopt (
        sock, SOL_SOCKET, SO_SNDBUF, (char*)&bufsize, sizeof (bufsize));
    getsockopt (sock, SOL_SOCKET, SO_SNDBUF, (char*)&cursize, &cursizesize);
...
int psnet_init_tcp () {
    ...
    if (bind (TCP_socket, (SOCKADDR*)&sockaddr, sizeof (sockaddr)) ==
        SOCKET_ERROR) {
        Tcp_failure_code = WSAGetLastError ();

        if (Tcp_failure_code == WSAEADDRINUSE) {
            ml_printf (
                "TCP socket already in use!  Another instance running?  (Try "
                "using the \"-port %i\" cmdline option)",
 ...
 * Get time in seconds
 */
float psnet_get_time () { return (float)timer_get_milliseconds () / 1000.0f; }
