...
#define CC_USER_JOINING \
    1 // A user had joined this channel (add him/her from the user listbox if
      // any)
#define CC_USER_LEAVING \
    2 // A user has left the channel (remove him/her from the user listbox if
      // any)
#define CC_DISCONNECTED \
    3 // We have been disconnected from the server (close the chat screen down)
#define CC_KICKED \
    4 // We were kicked out of the channel! (close the chat screen down?)
#define CC_NICKCHANGED \
    5 // Informing that your nickname has changed (data = "oldnick newnick")
#define CC_YOURCHANNEL \
    6 // data = name of the channel you are in. Only generated when you are
      // joining #autoselect
