...
// bitmaps defs
const char* Multi_pinfo_bitmap_name[GR_NUM_RESOLUTIONS] = { "PilotInfo",
                                                            "2_PilotInfo" };
const char* Multi_pinfo_bitmap_mask[GR_NUM_RESOLUTIONS] = { "PilotInfo-M",
                                                            "2_PilotInfo-M" };
...
int Multi_pinfo_pilot_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                          22, 159, 160, 120 },
                                                        { // GR_1024
                                                          35, 254, 256,
                                                          192 } };
int Multi_pinfo_squad_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                          22, 299, 128, 128 },
                                                        { // GR_1024
                                                          35, 479, 205,
                                                          205 } };
...
ui_button_info
    Multi_pinfo_buttons[GR_NUM_RESOLUTIONS][MULTI_PINFO_NUM_BUTTONS] = {
        { // GR_640
...
UI_XSTR Multi_pinfo_text[GR_NUM_RESOLUTIONS][MULTI_PINFO_NUM_TEXT] = {
    {
        // GR_640
        { "Close", 428, 217, 318, UI_XSTR_COLOR_PINK, -1,
          &Multi_pinfo_buttons[0][MPI_EXIT].button },
    },
    {
        // GR_1024
        { "Close", 428, 348, 510, UI_XSTR_COLOR_PINK, -1,
...
// if the pilot's image was currently loading when we started the popup, load
// it up now if its finished
...
void multi_pinfo_notify_drop (net_player* np) {
    ...
    if (np != Multi_pinfo_popup_player) { return; }
...
void multi_pinfo_popup_init (net_player* np) {
    ...
    for (idx = 0; idx < MULTI_PINFO_NUM_BUTTONS; idx++) {
        // create the object
        Multi_pinfo_buttons[gr_screen.res][idx].button.create (
            &Multi_pinfo_window, "", Multi_pinfo_buttons[gr_screen.res][idx].x,
            Multi_pinfo_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);

        // set the sound to play when highlighted
        Multi_pinfo_buttons[gr_screen.res][idx].button.set_highlight_action (
            common_play_highlight_sound);

        // set the ani for the button
        Multi_pinfo_buttons[gr_screen.res][idx].button.set_bmaps (
            Multi_pinfo_buttons[gr_screen.res][idx].filename);

        // set the hotspot
        Multi_pinfo_buttons[gr_screen.res][idx].button.link_hotspot (
            Multi_pinfo_buttons[gr_screen.res][idx].hotspot);
    }
    ...
    Multi_pinfo_stats_labels[5] =
        vm_strdup (XSTR ("Primary Shots Fired", 1012));
    Multi_pinfo_stats_labels[6] = vm_strdup (XSTR ("Primary Hit %", 1013));
    Multi_pinfo_stats_labels[7] =
        vm_strdup (XSTR ("Secondary Shots Fired", 1014));
...
void multi_pinfo_popup_do () {
    ...
    if (Multi_pinfo_popup_error) { return; }
    ...
    while (!Multi_pinfo_popup_done) {
        ...
        switch (k) {
        case KEY_ESC: Multi_pinfo_popup_done = 1; break;
        }
...
void multi_pinfo_popup_close () {
    ...
    // unload any bitmaps
    if (Multi_pinfo_bitmap != -1) { bm_release (Multi_pinfo_bitmap); }
    ...
    if (Mp_pilot.bitmap != -1) { bm_release (Mp_pilot.bitmap); }
    if (Mp_squad.bitmap != -1) { bm_release (Mp_squad.bitmap); }
...
void multi_pinfo_blit_pilot_image () {
    ...
    if (Mp_pilot.bitmap == -1) {
        ...
        // if we're not accepting images
            ...
            !(Net_player->p_info.options.flags & MLO_FLAG_ACCEPT_PIX) ||
            !(Netgame.options.flags & MSO_FLAG_ACCEPT_PIX)) {
        ...
        gr_string (
            Multi_pinfo_pilot_coords[gr_screen.res][0] +
                ((Multi_pinfo_pilot_coords[gr_screen.res][2] - w) / 2),
    ...
    // otherwise blit the bitmap
        ...
        gr_bitmap (
            Multi_pinfo_pilot_coords[gr_screen.res][0] +
                ((Multi_pinfo_pilot_coords[gr_screen.res][2] - bm_w) / 2),
            Multi_pinfo_pilot_coords[gr_screen.res][1] +
                ((Multi_pinfo_pilot_coords[gr_screen.res][3] - bm_h) / 2),
            GR_RESIZE_MENU);
        // g3_draw_2d_poly_bitmap(Multi_pinfo_pilot_coords[gr_screen.res][0],
        // Multi_pinfo_pilot_coords[gr_screen.res][1],
        // Multi_pinfo_pilot_coords[gr_screen.res][2],
        // Multi_pinfo_pilot_coords[gr_screen.res][3]);
    }
}
...
void multi_pinfo_blit_squadron_logo () {
    ...
    if (Mp_squad.bitmap == -1) {
        ...
        gr_get_string_size (&w, NULL, place_text);
        gr_string (
            Multi_pinfo_squad_coords[gr_screen.res][0] +
                ((Multi_pinfo_squad_coords[gr_screen.res][2] - w) / 2),
    ...
    // otherwise blit the bitmap
        ...
        gr_bitmap (
            Multi_pinfo_squad_coords[gr_screen.res][0] +
                ((Multi_pinfo_squad_coords[gr_screen.res][2] - bm_w) / 2),
            Multi_pinfo_squad_coords[gr_screen.res][1] +
                ((Multi_pinfo_squad_coords[gr_screen.res][3] - bm_h) / 2),
            GR_RESIZE_MENU);
        // g3_draw_2d_poly_bitmap(Multi_pinfo_squad_coords[gr_screen.res][0],
        // Multi_pinfo_squad_coords[gr_screen.res][1],
        // Multi_pinfo_squad_coords[gr_screen.res][2],
        // Multi_pinfo_squad_coords[gr_screen.res][3]);
    }
}
...
void multi_pinfo_blit_player_stats () {
    ...
    gr_string (
        Multi_pinfo_stats_area_coords[gr_screen.res][0],
        Multi_pinfo_stats_area_coords[gr_screen.res][1],
        Multi_pinfo_popup_player->m_player->callsign, GR_RESIZE_MENU);
    gr_string (
        Multi_pinfo_stats_x[gr_screen.res],
        Multi_pinfo_stats_area_coords[gr_screen.res][1],
        XSTR ("All Time Stats", 128), GR_RESIZE_MENU);
    ...
    for (idx = 0; idx < MULTI_PINFO_NUM_STATS_LABELS; idx++) {
        gr_string (
            Multi_pinfo_stats_area_coords[gr_screen.res][0], y_start,
            Multi_pinfo_stats_labels[idx], GR_RESIZE_MENU);
    ...
    for (idx = 0; idx < MULTI_PINFO_NUM_STATS_LABELS; idx++) {
        gr_string (
            Multi_pinfo_stats_x[gr_screen.res], y_start,
            Multi_pinfo_stats_vals[idx], GR_RESIZE_MENU);
        y_start += Multi_pinfo_stats_label_offsets[idx];
    }
}
...
void multi_pinfo_popup_button_pressed (int n) {
    ...
    switch (n) {
    case MPI_EXIT: Multi_pinfo_popup_done = 1; break;
...
void multi_pinfo_build_stats () {
    ...
    // missions flown
    sprintf (
        Multi_pinfo_stats_vals[MPI_MISSIONS_FLOWN], "%d",
        (int)sc->missions_flown);

    // flight time
    game_format_time (
        fl2f ((float)sc->flight_time),
        Multi_pinfo_stats_vals[MPI_FLIGHT_TIME]);

    // last flown
    if (sc->last_flown == 0) {
        strcpy_s (
            Multi_pinfo_stats_vals[MPI_LAST_FLOWN],
            XSTR ("No missions flown", 693));
    }
    else {
        ...
        if (tmr != NULL) {
            strftime (
                Multi_pinfo_stats_vals[MPI_LAST_FLOWN], MAX_LABEL_TEXT,
                "%m/%d/%y %H:%M", tmr);
        }
        else {
    ...
    // primary shots fired
    sprintf (
        Multi_pinfo_stats_vals[MPI_PSHOTS_FIRED], "%u", sc->p_shots_fired);

    // primary hit pct
    if (sc->p_shots_fired > 0) {
        sprintf (
            Multi_pinfo_stats_vals[MPI_PSHOTS_PCT], "%d%%",
            (int)(100.0f * ((float)sc->p_shots_hit / (float)sc->p_shots_fired)));
    }
    else {
    ...
    // primary shots fired
    sprintf (
        Multi_pinfo_stats_vals[MPI_SSHOTS_FIRED], "%u", sc->s_shots_fired);

    // primary hit pct
    if (sc->s_shots_fired > 0) {
        sprintf (
            Multi_pinfo_stats_vals[MPI_SSHOTS_PCT], "%d%%",
            (int)(100.0f * ((float)sc->s_shots_hit / (float)sc->s_shots_fired)));
    }
    else {
...
void multi_pinfo_maybe_reload_pic (np_bitmap* b) {
    ...
    // if the local player is not accepting pix or the netgame is not accepting
    // pix, bail here
...
int multi_pinfo_popup_active () { return Multi_pinfo_popup_running; }
