...
//
// Ingame joining is another delicate system. Although not as delicate as
// server transfer, it will help to take as many precautions as possible when
// handling ingame joins. Please be sure to follow all the same rules as
// explained in multi_strans.h
//
// --------------------------------------------------------------------------------------------------
...
// called on the server to process ingame joiners and move them through the
// motions of ingame joining
void multi_handle_ingame_joiners () {
    ...
    if (!(Netgame.flags & NG_FLAG_INGAME_JOINING)) { return; }

    // traverse through all the players
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        // only process ingame joiners
        if (!(Net_players[idx].flags & NETINFO_FLAG_RELIABLE_CONNECTED) ||
        ...
        // status
        if (Net_players[idx].s_info.ingame_join_flags &
            INGAME_JOIN_FLAG_FILE_XFER) {
            switch (
                multi_xfer_get_status (Net_players[idx].s_info.xfer_handle)) {
        ...
        // if the player has verified his file signature then send him the
        // packet to load the mission and mark this down
        if ((Net_players[idx].flags & NETINFO_FLAG_MISSION_OK) &&
            !(Net_players[idx].s_info.ingame_join_flags &
        ...
        // once he has finished loading the mission, start sending him ship
        // data packets and mark this down
        if ((Net_players[idx].state == NETPLAYER_STATE_MISSION_LOADED) &&
            !(Net_players[idx].s_info.ingame_join_flags &
            ...
            // mark the netgame as a critical stage in ingame joining so that I
            // don't evaluate mission event/ goals, ship arrivals, etc.
        ...
        // once he has finished receiving the ship data, start sending him wing
        // data and mark this down
        /*
        if((Net_players[idx].state == NETPLAYER_STATE_INGAME_SHIPS) &&
        !(Net_players[idx].s_info.ingame_join_flags &
        INGAME_JOIN_FLAG_SENDING_WINGS)){
        ...
        */
        ...
        // once he has received the respawn packet, send him the player
        // settings for all the players in the game and mark this down
        if ((Net_players[idx].state == NETPLAYER_STATE_INGAME_SHIPS) &&
            !(Net_players[idx].s_info.ingame_join_flags &
              INGAME_JOIN_FLAG_SENDING_POST)) {
            // reset the critical ingame joining flag so that I as server, will
            // start evaluating mission things again
            Netgame.flags &= ~NG_FLAG_INGAME_JOINING_CRITICAL;
        ...
        // once the settings have been received, send him the jump into mission
        // packet and mark this down. now the joiner moves into the ship select
        // state (ingame)
        if ((Net_players[idx].state == NETPLAYER_STATE_POST_DATA_ACK) &&
            !(Net_players[idx].s_info.ingame_join_flags &
              INGAME_JOIN_FLAG_PICK_SHIP)) {
            // if this guy is an obsever, create his observer object and be
            // done!
            if (Net_players[idx].flags & NETINFO_FLAG_OBSERVER) {
                multi_obs_create_observer (&Net_players[idx]);
                Net_players[idx].flags &= ~(NETINFO_FLAG_INGAME_JOIN);
                Netgame.flags &= ~(
                    NG_FLAG_INGAME_JOINING_CRITICAL | NG_FLAG_INGAME_JOINING);
            }
        ...
        // check to see if his timestamp for ship update (hull, shields, etc)
        // has popped. If so, send some info and reset
        if (timestamp_elapsed (
                Net_players[idx].s_info.last_full_update_time)) {
            // send the ships
        ...
        // once he has received the weapon state packet, send him the player
        // settings for all the players in the game and mark this down
        if (!(Net_players[idx].s_info.ingame_join_flags &
              INGAME_JOIN_FLAG_SENDING_SETS)) {
            // send the packet and mark it down
            // this will update _ALL_ players in the game which is important
            send_player_settings_packet ();
            send_player_settings_packet (
                &Net_players[idx]); // send directly so he gets the packet
...
// the final step for an ingame joining observer - create my observer object,
// unflag myself as joining and jump into mission
...
ui_button_info Multi_ingame_join_buttons
    [GR_NUM_RESOLUTIONS][MULTI_INGAME_JOIN_NUM_BUTTONS] = {
        {
            // GR_640
...
static int Mi_spacing[GR_NUM_RESOLUTIONS] = { 30, 48 };
...
static int Multi_ingame_timer_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                                  26, 411 },
                                                                { // GR_1024
                                                                  42, 658 } };
...
// net signature of the ship we've requested to grab as an ingame joiner
// (waiting for server response if >= 0)
ushort Multi_ingame_join_sig;

// the index into the list of the ship currently selected
int Multi_ingame_ship_selected;

// temporary stuff - used only until we come up with a more permanent interface
// for this screen
...
void multi_ingame_load_icons () {
    ...
    for (auto it = Ship_info.cbegin (); it != Ship_info.cend (); ++it) {
        ...
        if ((it->icon_filename[0] != '\0') &&
            (Multi_ingame_num_ship_icons < MULTI_INGAME_MAX_SHIP_ICONS)) {
            // set the ship class
            Multi_ingame_ship_icon[Multi_ingame_num_ship_icons].ship_class =
            ...
            for (s_idx = 0; s_idx < num_frames; s_idx++) {
                Multi_ingame_ship_icon[Multi_ingame_num_ship_icons]
                    .bmaps[s_idx] = first_frame + s_idx;
            }
...
void multi_ingame_select_init () {
    ...
    */
    ...
    Multi_ingame_window.create (
        0, 0, gr_screen.max_w_unscaled, gr_screen.max_h_unscaled, 0);
    Multi_ingame_window.set_mask_bmap (
        Multi_ingame_join_bitmap_mask_fname[gr_screen.res]);

    // load the background bitmap
    Multi_ingame_bitmap =
        bm_load (Multi_ingame_join_bitmap_fname[gr_screen.res]);
    if (Multi_ingame_bitmap < 0)
        ASSERTF (LOCATION, "Couldn't load background bitmap for ingame join");

    // create the interface buttons
    for (idx = 0; idx < MULTI_INGAME_JOIN_NUM_BUTTONS; idx++) {
        // create the object
        Multi_ingame_join_buttons[gr_screen.res][idx].button.create (
            &Multi_ingame_window, "",
            Multi_ingame_join_buttons[gr_screen.res][idx].x,
            Multi_ingame_join_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);

        // set the sound to play when highlighted
        Multi_ingame_join_buttons[gr_screen.res][idx]
            .button.set_highlight_action (common_play_highlight_sound);

        // set the ani for the button
        Multi_ingame_join_buttons[gr_screen.res][idx].button.set_bmaps (
            Multi_ingame_join_buttons[gr_screen.res][idx].filename);

        // set the hotspot
        Multi_ingame_join_buttons[gr_screen.res][idx].button.link_hotspot (
            Multi_ingame_join_buttons[gr_screen.res][idx].hotspot);
    }

    // create all xstrs
    for (idx = 0; idx < MULTI_INGAME_JOIN_NUM_TEXT; idx++) {
        Multi_ingame_window.add_XSTR (
            &Multi_ingame_join_text[gr_screen.res][idx]);
    }

    // create the list item select button
    Multi_ingame_select_button.create (
        &Multi_ingame_window, "", Mi_name_field[gr_screen.res][MI_FIELD_X],
...
void multi_ingame_ship_list_process () {
    ...
    // if we currently don't have any ships selected, but we've got items on
    // the list, select the first one
    if ((Multi_ingame_ship_selected == -1) && (Multi_ingame_num_avail > 0)) {
        gamesnd_play_iface (InterfaceSounds::USER_SELECT);
        Multi_ingame_ship_selected = 0;
    }

    // if we currently have a ship selected, but it disappears, select the next
    // ship (is possible0
    if ((Multi_ingame_ship_selected >= 0) &&
        (Multi_ingame_ship_selected >= Multi_ingame_num_avail)) {
        gamesnd_play_iface (InterfaceSounds::USER_SELECT);
        Multi_ingame_ship_selected = Multi_ingame_num_avail - 1;
    }

    // if the player clicked on the select button, see if the selection has
    // changed
...
void multi_ingame_join_button_pressed (int n) {
    switch (n) {
    case MIJ_CANCEL: multi_quit_game (PROMPT_CLIENT); break;
    case MIJ_JOIN:
        // don't do further processing if the game is paused
        if (Netgame.game_state == NETGAME_STATE_PAUSED) return;
        ...
        if (Multi_ingame_join_sig == 0) {
            ...
            if (Multi_ingame_ship_selected >= 0) {
                ...
                Multi_ingame_join_sig =
                    Multi_ingame_ship_sigs[Multi_ingame_ship_selected];

                // send a request to the
                send_ingame_ship_request_packet (
                    INGAME_SR_REQUEST, Multi_ingame_join_sig);
            }
            else {
...
void multi_ingame_select_do () {
    ...
    switch (k) {
    case KEY_ESC: multi_quit_game (PROMPT_CLIENT); break;

    case KEY_UP: multi_ingame_scroll_select_up (); break;

    case KEY_DOWN: multi_ingame_scroll_select_down (); break;
    }
...
// display an individual ships information, starting at the indicated y pixel
// value
void multi_ingame_join_display_ship (object* objp, int y_start) {
    ...
    gr_string (
        Mi_name_field[gr_screen.res][MI_FIELD_X], y_start + 10,
        Ships[objp->instance].ship_name, GR_RESIZE_MENU);

    // blit the ship class icon
    icon_num = multi_ingame_get_ship_class_icon (
        Ships[objp->instance].ship_info_index);
    if (icon_num != -1) {
        gr_set_bitmap (Multi_ingame_ship_icon[icon_num].bmaps[0]);
        gr_bitmap (
            Mi_class_field[gr_screen.res][MI_FIELD_X] + 15, y_start,
            GR_RESIZE_MENU);
    }
    ...
    y_spacing =
        (Mi_spacing[gr_screen.res] - (wp->num_primary_banks * line_height)) /
        2;
    for (idx = 0; idx < wp->num_primary_banks; idx++) {
        gr_string (
            Mi_primary_field[gr_screen.res][MI_FIELD_X],
            y_start + y_spacing + (idx * line_height),
            Weapon_info[wp->primary_bank_weapons[idx]].get_display_string (),
            GR_RESIZE_MENU);
    }

    // blit the ship's secondary weapons
    y_spacing =
        (Mi_spacing[gr_screen.res] - (wp->num_secondary_banks * line_height)) /
        2;
    for (idx = 0; idx < wp->num_secondary_banks; idx++) {
        gr_string (
            Mi_secondary_field[gr_screen.res][MI_FIELD_X],
            y_start + y_spacing + (idx * line_height),
            Weapon_info[wp->secondary_bank_weapons[idx]].get_display_string (),
            GR_RESIZE_MENU);
    }

    // blit the shield/hull integrity
    hud_shield_show_mini (
        objp, Mi_status_field[gr_screen.res][MI_FIELD_X] + 15, y_start + 3, 5,
        7);
}
    ...
    if (Multi_ingame_ship_selected != -1) {
        int y_start =
            (Mi_name_field[gr_screen.res][MI_FIELD_Y] +
             (Multi_ingame_ship_selected * Mi_spacing[gr_screen.res]));

        // draw the border
        gr_set_color_fast (&Color_bright_blue);
        gr_line (
            Mi_name_field[gr_screen.res][MI_FIELD_X] - 1, y_start - 1,
            (Mi_name_field[gr_screen.res][MI_FIELD_X] - 1) +
                (Mi_width[gr_screen.res] + 2),
        ...
        gr_line (
            ...
            (Mi_name_field[gr_screen.res][MI_FIELD_X] - 1) +
                (Mi_width[gr_screen.res] + 2),
            y_start + Mi_spacing[gr_screen.res] - 2, GR_RESIZE_MENU);
        gr_line (
            Mi_name_field[gr_screen.res][MI_FIELD_X] - 1, y_start,
            Mi_name_field[gr_screen.res][MI_FIELD_X] - 1,
            y_start + Mi_spacing[gr_screen.res] - 2, GR_RESIZE_MENU);
        gr_line (
            (Mi_name_field[gr_screen.res][MI_FIELD_X] - 1) +
...
void multi_ingame_handle_timeout () {
    ...
    gr_string(Multi_ingame_timer_coords[gr_screen.res][0],
    Multi_ingame_timer_coords[gr_screen.res][1], "Timer disabled!!",
    GR_RESIZE_MENU); return;
    */

    // if we've timed out, leave the game
    if (timestamp_elapsed (Ingame_time_left)) {
        multi_quit_game (
            PROMPT_NONE, MULTI_END_NOTIFY_INGAME_TIMEOUT,
            MULTI_END_ERROR_NONE);
        return;
    }
    ...
    gr_string (
        Multi_ingame_timer_coords[gr_screen.res][0],
        Multi_ingame_timer_coords[gr_screen.res][1], tl_string,
        GR_RESIZE_MENU);
}

// --------------------------------------------------------------------------------------------------
// PACKET HANDLER functions
// these are also defined in multimsgs.h, but the implementations are in the
...
void process_ingame_ships_packet (ubyte* data, header* hinfo) {
    ...
    while (p_type == INGAME_SHIP_NEXT) {
        ...
        if (p_objp == NULL) {
            // if this ship is part of wing not on its current wave, look for
            // its "original" by subtracting out wave #
            p_objp = mission_parse_get_arrival_ship (
                (ushort) (net_signature - (ushort)net_sig_modify));
        }
        if (p_objp == NULL) {
            Int3 ();
            nprintf (
        ...
        // go ahead and create the parse object.  Set the network signature of
        // this guy before creation
    ...
    if (p_type == INGAME_SHIP_LIST_EOL) {
        ...
        for (idx = 0; idx < MAX_SHIPS; idx++) {
            if (Ships[idx].objnum < 0) { continue; }

            // get the team and slot.  Team will be -1 when it isn't a part of
            // player wing.  So, if not -1, then be sure we have a valid slot,
            // then change the ship type, etc.
            objp = &Objects[Ships[idx].objnum];
            multi_ts_get_team_and_slot (
                Ships[idx].ship_name, &team_val, &slot_index);
            if (team_val != -1) {
                ASSERT (slot_index != -1);
                ...
                change_ship_type (
                    objp->instance,
                    Wss_slots_teams[team_val][slot_index].ship_class);
                wl_bash_ship_weapons (
                    &Ships[idx].weapons,
                    &Wss_slots_teams[team_val][slot_index]);
                ...
                // and set their objnum accordingly
                for (j = 0; j < MAX_PLAYERS; j++) {
                    if (MULTI_CONNECTED (Net_players[j]) &&
                        (Net_players[j].m_player->objnum ==
                         Objects[Ships[idx].objnum].net_signature)) {
                        multi_assign_player_ship (
                            j, objp, Ships[idx].ship_info_index);
...
void send_ingame_ships_packet (net_player* player) {
    ...
    // essentially, we are going to send a list of ship names to the joiner for
    // ships that are not in wings.  The joiner will take the list, create any
    // ships which should be created, and delete all other ships after the list
    // is sent.
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        // add the ship name and other information such as net signature, ship
        // and object(?) flags.
        ...
        ADD_SHORT (wing_data);
        if (wing_data >= 0) { ADD_INT (Wings[wing_data].current_wave); }

        // don't send anymore data if we are getting close to the maximum size
        // of this packet.  Send it off and keep going
...
void send_ingame_ship_request_packet (int code, int rdata, net_player* pl) {
    ...
    case INGAME_SR_CONFIRM:
        ...
        ADD_INT (Missiontime); // NOTE: this is a long so careful with swapping
                               // in 64-bit platforms - taylor
        ...
        // add the current ammo count for primary banks - copied from above -
        // Goober5000
    ...
    // if this is a confirm to a player -- send data to the other players in
    // the game telling them
    if ((code == INGAME_SR_CONFIRM) &&
        ...
        for (i = 0; i < MAX_PLAYERS; i++) {
            if (MULTI_CONNECTED (Net_players[i]) &&
                (&Net_players[i] != Net_player) && (i != player_num)) {
...
// function to validate all players in the game according to their team select
// index.  If discrepancies are found, this function should be able to fix them
// up.
void multi_ingame_validate_players () {
    ...
    for (i = 0; i < MAX_PLAYERS; i++) {
        if (MULTI_CONNECTED (Net_players[i]) &&
            ...
            if ((Objects[player_objnum].type != OBJ_SHIP) &&
                (Objects[player_objnum].type != OBJ_GHOST)) {
                Int3 ();
            }

            multi_ts_get_shipname (
                ship_name, Net_players[i].p_info.team,
                Net_players[i].p_info.ship_index);
            ...
            // if this guy's objnum isn't a ship, then it should proably be a
            // ghost!!
...
void process_ingame_ship_request_packet (ubyte* data, header* hinfo) {
    ...
    case INGAME_SR_REQUEST:
        ...
        // lookup the player and make sure he doesn't already have an objnum
        // (along with possible error conditions)
        ...
        if (Net_players[player_num].m_player->objnum != -1) {
            send_ingame_ship_request_packet (
                INGAME_SR_DENY, 0, &Net_players[player_num]);
            break;
        }
        ...
        if (objp == NULL ||
            !(objp->flags[Object::Object_Flags::Could_be_player])) {
            send_ingame_ship_request_packet (
                INGAME_SR_DENY, 0, &Net_players[player_num]);
            break;
        }
        ...
        Net_players[player_num].state =
            NETPLAYER_STATE_IN_MISSION; // since he'll do this anyway...
        Net_players[player_num].flags &= ~(NETINFO_FLAG_INGAME_JOIN);
        multi_assign_player_ship (
            player_num, objp, Ships[objp->instance].ship_info_index);

        // update his ets and link status stuff
        multi_server_update_player_weapons (
            &Net_players[player_num], &Ships[objp->instance]);

        objp->flags.remove (Object::Object_Flags::Could_be_player);
        objp->flags.set (Object::Object_Flags::Player_ship);

        // send a player settings packet to update all other players of this
        // guy's final choices
        send_player_settings_packet ();
        ...
        // ack him
        send_ingame_ship_request_packet (
            INGAME_SR_CONFIRM, OBJ_INDEX (objp), &Net_players[player_num]);
    ...
    case INGAME_SR_DENY:
        ...
        popup (
            ...
            XSTR ("You have been denied the requested ship", 686));
        break;
    ...
    case INGAME_SR_CONFIRM:
        ...
        // delete the ship this ingame joiner was using.  Unassign Player_obj
        // so that this object doesn't become a ghost. temp_objp = Player_obj;
        // Player_obj = NULL;
        ...
        GET_INT (Missiontime); // NOTE: this is a long so careful with swapping
                               // in 64-bit platforms - taylor
        ...
        multi_assign_player_ship (
            MY_NET_PLAYER_NUM, objp, Ships[objp->instance].ship_info_index);

        // must change the ship type and weapons.  An ingame joiner know about
        // the default class and weapons for a ship, but these could have
        // changed.
        multi_ts_get_team_and_slot (
            Player_ship->ship_name, &team, &slot_index);
        ASSERT (team != -1);
        ASSERT (slot_index != -1);
        change_ship_type (
            objp->instance, Wss_slots_teams[team][slot_index].ship_class);
        wl_bash_ship_weapons (
            &Player_ship->weapons, &Wss_slots_teams[team][slot_index]);
        ...
        ASSERT (
            num_secondary_banks == Player_ship->weapons.num_secondary_banks);
        ...
        // be sure that this ships current primary/secondary weapons are valid.
        // Easiest is to just bash the values to 0!
        /*
        ...
        */
        ...
        // NOTE : we check this flag because its possible that the player could
        // have received an endgame packet in the same
        // frame as getting this confirmation. In that case, he should
        // be quitting to the main menu. We must not make him continue
        // on into the mission
    ...
    case INGAME_PLAYER_CHOICE: {
        ...
        // get the player number of this guy, and the net signature of the ship
        // he has chosen
        GET_INT (player_num);
        GET_USHORT (net_signature);
        PACKET_SET_SIZE ();
...
void multi_ingame_send_ship_update (net_player* p) {
    ...
    // go through the list and send all ships which are mark as
    // OF_COULD_BE_PLAYER
    while (moveup != END_OF_LIST (&Ship_obj_list)) {
        // Make sure the object can be a player and is on the same team as this
        // guy
        if (Objects[moveup->objnum]
                .flags[Object::Object_Flags::Could_be_player] &&
            obj_team (&Objects[moveup->objnum]) == p->p_info.team) {
            // send the update
            send_ingame_ship_update_packet (
                p, &Ships[Objects[moveup->objnum].instance]);
        }
