...
int multi_obs_create_player (
    int player_num, char* name, net_addr* addr, player* pl) {
    ...
    // NETINFO_FLAG_OBSERVER); DOH!!! The lack of this caused many bugs.
    Net_players[player_num].flags =
        (NETINFO_FLAG_DO_NETWORKING | NETINFO_FLAG_OBSERVER);
    // memcpy(&Net_players[player_num].p_info.addr, addr, sizeof(net_addr));
    Net_players[player_num].m_player = pl;

    // 6/3/98 -- don't set observer to update high...let it be whatever player
    // set it at.
    // Net_players[player_num].p_info.options.obj_update_level =
    // OBJ_UPDATE_HIGH;
...
// create observer object locally, and additionally, setup some other
// information ( client-side equivalent of multi_obs_create_observer() )
void multi_obs_create_observer_client () {
    ...
    obj_set_flags (
        &Objects[pobj_num],
        Objects[pobj_num].flags + Object::Object_Flags::Player_ship);
...
// this implies for the local player in the case of a client or for _all_
// players in the case of a server
...
void multi_obs_zoom_to_target () {
    ...
    // get the normalized direction vector between the observer and the
    // targeted object
    vm_vec_sub (
        &direct, &Objects[Player_ai->target_objnum].pos, &Player_obj->pos);
