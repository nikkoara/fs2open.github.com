...
#define RESPAWN_BROADCAST                                                     \
    0x1 // server to clients - create this ship, and if you're the respawner,
        // set it to be your object
#define RESPAWN_REQUEST \
    0x2 // client to server requesting a respawn (observer or normal)
...
ai_respawn
    Ai_respawns[MAX_AI_RESPAWNS]; // this is way too many, but I don't care
...
// respawn the passed player with the passed ship object and weapon link
// settings
void multi_respawn_player (
    net_player* pl, char cur_primary_bank, char cur_secondary_bank,
    ubyte cur_link_status, ushort ship_ets, ushort net_sign, char* parse_name,
    vec3d* pos = NULL);
...
// send a request to the server saying I want to respawn (as an observer or
// not)
...
void multi_respawn_check (object* objp) {
    ...
    // get the parse object since we are storing all data for the respawns in
    // the parse object
    ...
    // if this ship isn't a player ship, then maybe it is an AI ship which
    // should get respawed.  Only respawn on the server, then send message to
    // respawn on client.
    if (pl == NULL) {
        // try and find the parse object with this net signature.  If we found
        // it, and it's a player start position, respawn it.
        if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
            if (!pobjp) { return; }

            // if we need to respawn this ai ship, add him to a list of ships
            // to get respawned
            if ((pobjp->flags[Mission::Parse_Object_Flags::OF_Player_start]) &&
                (pobjp->respawn_count < Netgame.respawn) &&
...
void multi_respawn_player_leave (net_player* pl) {
    // bogus
    if (pl == NULL) { return; }
    if (MULTI_OBSERVER ((*pl))) { return; }
    if ((Net_player == NULL) ||
        !(Net_player->flags & NETINFO_FLAG_AM_MASTER)) {
        return;
    }
    if (pl->p_info.p_objp == NULL) { return; }

    // dogfight mode
    if (MULTI_DOGFIGHT) { return; }

    // if we need to respawn this ai ship, add him to a list of ships to get
    // respawned
...
void multi_respawn_normal () {
    ...
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        ...
        // client sends a respawn request (multiple ones are ok if he clicks
        // over and over)
...
// server should check to see if any respawned players have run out of their
// invulnerability
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            (Objects[Net_players[idx].m_player->objnum]
                 .flags[Object::Object_Flags::Invulnerable])) {
            ...
            if (((Net_players[idx].s_info.invul_timestamp != -1) &&
                 (timestamp_elapsed (
                     Net_players[idx].s_info.invul_timestamp))) ||
                ((Net_players[idx].m_player->ci.fire_primary_count > 0) ||
                 (Net_players[idx].m_player->ci.fire_secondary_count > 0))) {
                objp = &Objects[Net_players[idx].m_player->objnum];
                obj_set_flags (
                    objp, objp->flags - Object::Object_Flags::Invulnerable);
            }
        }
    }
}
...
void multi_respawn_build_points () {
    ...
    while (moveup != END_OF_LIST (&Ship_obj_list)) {
        // stuff info
        if ((Ships[Objects[moveup->objnum].instance].respawn_priority > 0) &&
            (Multi_respawn_priority_count < MAX_PRIORITY_POINTS)) {
...
int multi_respawn_common_stuff (p_object* pobjp) {
    ...
    if (team != -1 && slot_index != -1) {
        ...
        wl_bash_ship_weapons (
            &shipp->weapons, &Wss_slots_teams[team][slot_index]);
    }
...
void multi_respawn_player (
    net_player* pl, char cur_primary_bank, char cur_secondary_bank,
    ubyte cur_link_status, ushort ship_ets, ushort net_sig, char* parse_name,
    vec3d* pos) {
    int objnum;
    object* objp;
    ship* shipp;
    ...
    if (pl == Net_player) {
        ...
        // this is a hack to ensure that old (dead) player ships are destroyed,
        // since at this point he's actually an OBJ_GHOST
    ...
    // give the current bank a half-second timestamp so that we don't fire
    // immediately unpon respawn
    shipp->weapons
        .next_secondary_fire_stamp[shipp->weapons.current_secondary_bank] =
        timestamp (500);

    // if this is a dogfight mission, make him TEAM_TRAITOR
    if (Netgame.type_flags & NG_TYPE_DOGFIGHT) { shipp->team = Iff_traitor; }

    // maybe bash ship position
    if (pos != NULL) { objp->pos = *pos; }
    ...
    if (MULTIPLAYER_MASTER) { multi_respawn_broadcast (pl); }
}
...
void multi_respawn_ai (p_object* pobjp) {
    ...
    // be sure the the OF_PLAYER_SHIP flag is unset, and the could be player
    // flag is set
...
void multi_respawn_make_observer (net_player* pl) {
    ...
    // MWA 6/3/98 -- don't set to high -- let it default to whatever player
    // chose
...
void multi_respawn_as_observer () {
    ...
    Net_player->m_player->flags &=
        ~(PLAYER_FLAGS_AUTO_MATCH_SPEED | PLAYER_FLAGS_MATCH_TARGET);
...
void multi_respawn_process_packet (ubyte* data, header* hinfo) {
    ...
    if (player_index == -1) {
        WARNINGF (
            LOCATION, "Couldn't find player for processing respawn packet!");
    }
    ...
    case RESPAWN_BROADCAST:
        ...
        if (player_index == -1) {
            // WARNINGF (LOCATION, "Couldn't find player to respawn!");
            break;
        }
        ...
        multi_respawn_player (
            &Net_players[player_index], cur_primary_bank, cur_secondary_bank,
            cur_link_status, ship_ets, net_sig, parse_name, &v);
    ...
    case RESPAWN_REQUEST:
        ...
        if (player_index == -1) {
            WARNINGF (
                LOCATION, "Received respawn request from unknown player!");
            break;
        }
        WARNINGF (
            LOCATION, "Received respawn request for player %s\n",
            Net_players[player_index].m_player->callsign);

        // make sure he's not making an invalid request
        if ((code == 0) &&
            !(Net_players[player_index].flags & NETINFO_FLAG_RESPAWNING)) {
            // WARNINGF (LOCATION, "This player shouldn't be respawning!");
            Int3 ();
            break;
        }
        else if (
            (code == 1) &&
            !(Net_players[player_index].flags & NETINFO_FLAG_LIMBO)) {
            WARNINGF (
                LOCATION,
                "This is a respawn observer request from a player who "
                "shouldn't be respawning as an observer!");
            Int3 ();
            break;
        }
        ...
        if (code) { multi_respawn_make_observer (&Net_players[player_index]); }
        // respawn him as normal
            ...
            if (Net_players[player_index].p_info.p_objp != NULL) {
                multi_respawn_player (
                    ...
                    Net_players[player_index].s_info.cur_primary_bank,
...
void multi_respawn_server () {
    ...
    // respawn me
    multi_respawn_player (
        Net_player, Net_player->s_info.cur_primary_bank,
        Net_player->s_info.cur_secondary_bank,
        ...
        if (Ai_respawns[i].pobjp != NULL) {
            if (timestamp_elapsed (Ai_respawns[i].timestamp)) {
                ...
                if (ship_name_lookup (Ai_respawns[i].pobjp->name) != -1) {
                    ...
                    multi_respawn_send_ai_respawn (
                        Ai_respawns[i].pobjp->net_signature);
                    Ai_respawns[i].pobjp = NULL;
                    Ai_respawns[i].timestamp = timestamp (-1);
                }
            }
...
// this is a completely off the cuff way of doing things. Feel free to find a
...
void multi_respawn_place (object* new_obj, int team) {
    ...
    for (idx = 0; idx < Multi_respawn_priority_count; idx++) {
        // all relevant ships
        if ((Multi_respawn_priority_ships[idx].team == team) ||
            !(Netgame.type_flags & NG_TYPE_TEAM)) {
            lookup =
                ship_name_lookup (Multi_respawn_priority_ships[idx].ship_name);
            if ((lookup >= 0) &&
                ((pri == NULL) ||
                 (Ships[lookup].respawn_priority > pri->respawn_priority)) &&
                (Ships[lookup].objnum >= 0) &&
                (Ships[lookup].objnum < MAX_OBJECTS)) {
                pri = &Ships[lookup];
    ...
    if ((pri != NULL) && (pri_obj != NULL)) {
        ...
        if (pm == NULL) {
            vm_vec_scale_add (
                &new_obj->pos, &pri_obj->pos, &pri_obj->orient.vec.rvec,
                2000.0f);
        }
        else {
            ...
            case 2:
                vm_vec_scale_add (
                    &new_obj->pos, &pri_obj->pos, &pri_obj->orient.vec.uvec,
            ...
            case 4:
                vm_vec_scale_add (
                    &new_obj->pos, &pri_obj->pos, &pri_obj->orient.vec.fvec,
                    (pm->maxs.xyz.z - pm->mins.xyz.z));
                break;
    ...
    // otherwise, resort to plain respawn points
        ...
        while (!lookup && (count < 13)) {
            if ((team == Iff_traitor) ||
                (team ==
                 Multi_respawn_points[Multi_next_respawn_point].team)) {
                lookup = 1;
            }

            // next item
            if (!lookup) {
                if (Multi_next_respawn_point >=
                    (Multi_respawn_point_count - 1)) {
                    Multi_next_respawn_point = 0;
                }
                else {
...
/*
#define MOVE_AWAY() { vec3d away;
vm_vec_sub(&away,&new_obj->pos,&hit_check->pos); \
                       vm_vec_normalize_quick(&away);
vm_vec_scale(&away,hit_check->radius+hit_check->radius); \
                             vm_vec_add2(&new_obj->pos,&away); }

#define WITHIN_RADIUS() { float dist;
dist=vm_vec_dist(&new_obj->pos,&hit_check->pos); \ if(dist <=
hit_check->radius) collided = 1; }
*/
...
#define WITHIN_BBOX()                                         \
    do {                                                      \
        if (pm != NULL) {                                     \
            ...
            if ((gpos.xyz.x >= pm->mins.xyz.x * scale) &&     \
                (gpos.xyz.y >= pm->mins.xyz.y * scale) &&     \
                (gpos.xyz.z >= pm->mins.xyz.z * scale) &&     \
                (gpos.xyz.x <= pm->maxs.xyz.x * scale) &&     \
                (gpos.xyz.y <= pm->maxs.xyz.y * scale) &&     \
                (gpos.xyz.z <= pm->maxs.xyz.z * scale)) {     \
                collided = 1;                                 \
            }                                                 \
        }                                                     \
    } while (0)
...
#define MOVE_AWAY_BBOX()                                  \
    do {                                                  \
        if (pm != NULL) {                                 \
            ...
            case 0: new_obj->pos.xyz.x += 200.0f; break;  \
            case 1: new_obj->pos.xyz.x -= 200.0f; break;  \
            case 2: new_obj->pos.xyz.y += 200.0f; break;  \
            case 3: new_obj->pos.xyz.y -= 200.0f; break;  \
            default: new_obj->pos.xyz.z -= 200.0f; break; \
            }                                             \
        }                                                 \
    } while (0)
...
void prevent_spawning_collision (object* new_obj) {
    ...
    do {
        ...
        for (moveup = GET_FIRST (&Ship_obj_list);
             moveup != END_OF_LIST (&Ship_obj_list);
             moveup = GET_NEXT (moveup)) {
            // don't check the new object itself!!
            if (moveup->objnum == OBJ_INDEX (new_obj)) continue;
            ...
            polymodel* pm =
                model_get (Ship_info[s_check->ship_info_index].model_num);
            WITHIN_BBOX ();
            if (collided) {
                MOVE_AWAY_BBOX ();
                break;
