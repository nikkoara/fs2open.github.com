...
// macros for building up packets -- to save on time and typing.  Important to
// note that local variables must be named correctly there are two flavors of
// sending orientation matrices, 16 bit and 32 bit. Just #define ORIENT_16 to
// use 16 bits, otherwise 32 bits is the default
...
#define ADD_ULONG(d)                                           \
    do {                                                       \
        ...
        Assert ((packet_size + sizeof (d)) < MAX_PACKET_SIZE); \
        std::uint64_t swap = INTEL_LONG (d);                   \
        memcpy (data + packet_size, &swap, sizeof (d));        \
        packet_size += sizeof (d);                             \
    } while (0)
#define ADD_FLOAT(d)                                           \
...
#define ADD_STRING(s)                                              \
    do {                                                           \
        ...
        int len_tmp = INTEL_INT (len);                             \
        ADD_DATA (len_tmp);                                        \
        memcpy (data + packet_size, s, len);                       \
        packet_size += len;                                        \
    } while (0)
#define ADD_ORIENT(d)                                  \
    {                                                  \
...
#define ADD_VECTOR(d)                          \
    do {                                       \
        vec3d tmpvec = ZERO_VECTOR;            \
        tmpvec.xyz.x = INTEL_FLOAT (&d.xyz.x); \
        tmpvec.xyz.y = INTEL_FLOAT (&d.xyz.y); \
        tmpvec.xyz.z = INTEL_FLOAT (&d.xyz.z); \
        ADD_DATA (tmpvec);                     \
    } while (0)
...
#define GET_SHORT(d)                               \
    do {                                           \
        short swap;                                \
        memcpy (&swap, data + offset, sizeof (d)); \
        d = INTEL_SHORT (swap);                    \
        offset += sizeof (d);                      \
    } while (0)
#define GET_USHORT(d)                              \
...
#define GET_ORIENT(d)                        \
    {                                        \
        ubyte dt[17];                        \
        memcpy (dt, data + offset, 17);      \
        offset += 17;                        \
        multi_unpack_orient_matrix (dt, &d); \
    }
#define GET_VECTOR(d)                          \
    do {                                       \
        vec3d tmpvec = ZERO_VECTOR;            \
        GET_DATA (tmpvec);                     \
        d.xyz.x = INTEL_FLOAT (&tmpvec.xyz.x); \
        d.xyz.y = INTEL_FLOAT (&tmpvec.xyz.y); \
        d.xyz.z = INTEL_FLOAT (&tmpvec.xyz.z); \
    } while (0)

#define PACKET_SET_SIZE() \
    do { hinfo->bytes_processed = offset; } while (0)
...
void multi_io_send_to_all_reliable (
    ubyte* data, int length, net_player* ignore = NULL);
...
// process an object update packet.  See send_object_update for information on
// how this packet data should be interpreted.  We send different information
// depending on on several variables (no change in orienation, etc).
...
void send_accept_packet (
    int new_player_num, int code, int ingame_join_team = -1);
...
void send_game_chat_packet (
    net_player* from, const char* msg, int msg_mode, net_player* to = NULL,
    const char* expr = NULL, int server_msg = 0);
...
void send_leave_game_packet (
    short player_id = -1, int kicked_reason = -1, net_player* target = NULL);
...
// send object update packet sends object updates for all objects in the game.
// This function will be smart about sending only certain objects to certain
// players based on the players distance from an object, whether the object is
// behind the player, etc.
void send_object_update_packet (int force_all = 0);

// send a packet indicating a ship has been killed
void send_ship_kill_packet (
    object* ship_obj, object* other_objp, float percent_killed,
    int self_destruct);
...
void send_mission_message_packet (
    ...
    int builtin_type, int multi_target, int multi_team_filter, int delay = 0);
...
void send_debris_create_packet (
    object* objp, ushort net_signature, int model_num, vec3d exp_center);
...
// send a request from the client to the host of the game (which is not
// necessarily the server in the case of the standalone) mode ==
// WSS_WEAPON_SELECT  or  WSS_SHIP_SELECT
...
void send_wss_slots_data_packet (
    int team_num, int final, net_player* p = NULL, int std_request = 1);

void send_shield_explosion_packet (int objnum, int tri_num, vec3d hit_pos);

void send_player_stats_block_packet (
    net_player* pl, int stats_type, net_player* target = NULL,
    short offset = 0);
...
void send_ai_info_update_packet (
    object* objp, char what, object* other_objp = NULL);
...
// host sends a -1 to the server to begin the countdown. server sends an int
// "seconds until start"
...
void send_turret_fired_packet (
    int objnum, int subsys_index, int weapon_objnum);
