...
#define MULTI_END_NOTIFY_FILE_REJECTED \
    3 // mission file was rejected by the server
#define MULTI_END_NOTIFY_EARLY_END \
    4 // game ended while the ingame joiner was joining
#define MULTI_END_NOTIFY_INGAME_TIMEOUT \
    5 // waited too long in the ship select screen
#define MULTI_END_NOTIFY_KICKED_BAD_XFER 6 // kicked because file xfer failed
#define MULTI_END_NOTIFY_KICKED_CANT_XFER \
    7 // kicked because can't xfer a builtin mission
#define MULTI_END_NOTIFY_KICKED_INGAME_ENDED \
    8 // kicked because was ingame joining in an ending game
...
#define MULTI_END_ERROR_INGAME_SHIP \
    4 // unable to create ingame join player ship
...
#define MULTI_END_ERROR_SHIP_ASSIGN \
    7 // server had problems assigning players to ships
...
#define MULTI_END_ERROR_WAVE_COUNT \
    10 // illegal data found in mission when parsing
...
#define MULTI_END_ERROR_CAPTAIN_LEFT \
    13 // captain of a team has left while not ingame
...
// codes return 0 if the act was cancelled, 1 if it was accepted
int multi_quit_game (
    int prompt, int notify_code = MULTI_END_NOTIFY_NONE,
    int err_code = MULTI_END_ERROR_NONE, int wsa_error = -1);
