...
void ChatInit (void) {
    ...
    memset (
        Getting_user_channel_info_for, 0,
        sizeof (Getting_user_channel_info_for));
    memset (
        Getting_user_tracker_info_for, 0,
        sizeof (Getting_user_tracker_info_for));
...
int ConnectToChatServer (char* serveraddr, char* nickname, char* trackerid) {
    ...
    if (!Socket_connecting) {
        ...
        strncpy (
            Orignial_nick_name, nickname, sizeof (Orignial_nick_name) - 1);
        ...
        if (0 == chat_port) { return -1; }

        Chatsock = socket (AF_INET, SOCK_STREAM, IPPROTO_TCP);

        if (INVALID_SOCKET == Chatsock) { return -1; }
        ...
        if (SOCKET_ERROR ==
            bind (Chatsock, (SOCKADDR*)&Chataddr, sizeof (sockaddr))) {
            return -1;
        }
        ...
        if (SOCKET_ERROR ==
            connect (Chatsock, (SOCKADDR*)&Chataddr, sizeof (SOCKADDR_IN))) {
            ...
            // This should never happen, connect should always return
            // WSAEWOULDBLOCK
            Socket_connecting = 1;
            Socket_connected = 1;
            return 1;
        ...
        if (Chat_server_connected) { return 1; }

        if (!Socket_connected) {
            // Do a few select to check for an error, or to see if we are
            // writeable (connected)
            ...
            if (select (
                    static_cast< int > (Chatsock + 1), NULL, &write_fds, NULL,
                    &timeout) > 0) {
                // make sure that we don't have any connect() errors (since
                // it's non-blocking)
                int err_val = 0;
                size_t err_val_size = sizeof (err_val);
                getsockopt (
                    Chatsock, SOL_SOCKET, SO_ERROR, (char*)&err_val,
                    (socklen_t*)&err_val_size);

                if (err_val) {
                    if (err_val != WSAEWOULDBLOCK) { return -1; }
                ...
                sprintf (
                    signon_str, NOX ("/USER %s %s %s :%s"), NOX ("user"),
                    NOX ("user"), NOX ("user"), Chat_tracker_id);
            ...
            if (select (
                    static_cast< int > (Chatsock + 1), NULL, NULL, &error_fds,
                    &timeout)) {
                return -1;
            }
...
const char* SendChatString (const char* line, int raw) {
    ...
    if (line[0] == '/') {
        ...
        if (strcasecmp (szCmd, NOX ("msg")) == 0) {
            strncpy (
                szTarget, GetWordNum (1, line + 1), sizeof (szTarget) - 1);
            sprintf (
                szCmd, NOX ("PRIVMSG %s :%s\n\r"), szTarget,
                line + strlen (NOX ("/msg ")) + strlen (szTarget) + 1);
            send (Chatsock, szCmd, (int)strlen (szCmd), 0);
            szCmd[strlen (szCmd) - 2] = '\0';
            return ParseIRCMessage (szCmd, MSG_LOCAL);
        }
        if (strcasecmp (szCmd, NOX ("me")) == 0) {
            sprintf (
                szCmd, NOX ("PRIVMSG %s :\001ACTION %s\001\n\r"),
                szChat_channel, line + strlen (NOX ("/me ")));
            send (Chatsock, szCmd, (int)strlen (szCmd), 0);
            szCmd[strlen (szCmd) - 2] = '\0';
            return ParseIRCMessage (szCmd, MSG_LOCAL);
        }
        if (strcasecmp (szCmd, NOX ("xyz")) == 0) {
            // Special command to send raw irc commands
            sprintf (szCmd, "%s\n\r", line + strlen (NOX ("/xyz ")));
            send (Chatsock, szCmd, (int)strlen (szCmd), 0);
            return NULL;
        }
        if (strcasecmp (szCmd, NOX ("list")) == 0) {
            sprintf (szCmd, "%s\n\r", line + 1);
            send (Chatsock, szCmd, (int)strlen (szCmd), 0);
            return NULL;
        }
...
int SetNewChatChannel (char* channel) {
    ...
    if (Joining_channel == 1) {
        if (Joined_channel == 1) {
            ...
            // Error -- we got a message that the channel was invite only, or
            // we were banned or something
            Joining_channel = 0;
    ...
    while (select (Chatsock + 1, &read_fds, NULL, NULL, &timeout)) {
        ...
        if (bytesread) {
            ...
            if ((ch[0] == 0x0a) || (ch[0] == 0x0d)) {
                ...
                strncpy (
                    return_string, Input_chat_buffer,
                    sizeof (return_string) - 1);
                Input_chat_buffer[0] = '\0';
            ...
            // Select said we had read data, but 0 bytes read means
            // disconnected
...
const char* GetWordNum (int num, const char* l_String) {
    ...
    for (int i = 0; i != num; i++) { token = strtok (NULL, seps); }
    if (token) { strncpy (strreturn, token, sizeof (strreturn) - 1); }
    else {
        return "";
    }
    // check for the ':' char....
    if (token[0] == ':') {
        ...
        strncpy (
            strreturn, l_String + ((token - strstart) + 1),
            sizeof (strreturn) - 1);
    }
...
int AddChatUser (const char* nickname) {
    Curruser = Firstuser;
    while (Curruser) {
        if (strcasecmp (nickname, Curruser->nick_name) == 0) return 0;
        Curruser = Curruser->next;
    }
    ...
    if (Firstuser == NULL) {
        ...
        AddChatCommandToQueue (
            CC_USER_JOINING, nickname, strlen (nickname) + 1);
        return 1;
    }
    else {
...
int RemoveChatUser (const char* nickname) {
    ...
    Curruser = Firstuser;
    while (Curruser) {
        if (strcasecmp (nickname, Curruser->nick_name) == 0) {
            if (prv_user) { prv_user->next = Curruser->next; }
            else {
                Firstuser = Curruser->next;
            }
            AddChatCommandToQueue (
                CC_USER_LEAVING, Curruser->nick_name,
                strlen (Curruser->nick_name) + 1);
            vm_free (Curruser);
            return 1;
        }
        prv_user = Curruser;
...
char* ParseIRCMessage (char* Line, int iMode) {
    ...
    if (strlen (Line) >= MAXLOCALSTRING) { return NULL; }
    // Nick included....
    if (iMode == MSG_REMOTE) {
        ...
        if (Line[0] == ':') {
            ...
            strncpy (
                szRemLine, Line + 1 + strlen (szPrefix),
                sizeof (szRemLine) - 1);
        }
        // Next, get the Nick
        pszTempStr = strtok (szHackPrefix, "!");
        if (pszTempStr) { strncpy (szNick, pszTempStr, sizeof (szNick) - 1); }
        else {
    ...
    if (pszTempStr[0]) { strncpy (szCmd, pszTempStr, sizeof (szCmd) - 1); }
    else {
        // Shouldn't ever happen, but we can't be sure of what the host will
        // send us.
        return NULL;
    }
    ...
    strncpy (
        szRemLine, Line + iPrefixLen + strlen (szCmd) + 2,
        sizeof (szRemLine) - 1);
    // Now parse the commands!
    if (strcasecmp (szCmd, NOX ("PRIVMSG")) == 0) {
        pszTempStr = GetWordNum (0, szRemLine);
        strncpy (szTarget, pszTempStr, sizeof (szTarget) - 1);
        strncpy (
            szRemLine,
            Line + iPrefixLen + strlen (szCmd) + strlen (szTarget) + 4,
            sizeof (szRemLine) - 1);
        if (szRemLine[0] == ':') {
            strncpy (
                szCTCPCmd, GetWordNum (0, szRemLine + 1),
                sizeof (szCTCPCmd) - 1);
            ...
            strncpy (
                szCTCPCmd, GetWordNum (0, szRemLine), sizeof (szCTCPCmd) - 1);
            if (szCTCPCmd[strlen (szCTCPCmd) - 1] == 0x01)
                szCTCPCmd[strlen (szCTCPCmd) - 1] = 0x00;
        }
        if (szCTCPCmd[0] == 0x01) {
            ...
            strncpy (
                szRemLine,
                Line + iPrefixLen + strlen (szCmd) + strlen (szTarget) +
                    strlen (szCTCPCmd) + 6,
                sizeof (szRemLine) - 1);
            szRemLine[strlen (szRemLine) - 1] =
                '\0'; // null out the ending 0x01
            if (strcasecmp (szCTCPCmd + 1, NOX ("ACTION")) == 0) {
                // Posture
            ...
            if (iMode == MSG_LOCAL) {
                ...
                sprintf (
                    szResponse, NOX ("** CTCP %s %s %s"), szTarget,
                    szCTCPCmd + 1, szRemLine);
                return szResponse;
            }
            if (strcasecmp (szCTCPCmd + 1, NOX ("PING")) == 0) {
                sprintf (
                    szResponse, NOX ("/NOTICE %s :\001PING %s\001"), szNick,
                    szRemLine); // Don't need the trailing \001 because
                                // szremline has it.
                SendChatString (szResponse, 1);
                return NULL;
            }
            if (strcasecmp (szCTCPCmd + 1, NOX ("VERSION")) == 0) { return NULL; }
            strncpy (
                szRemLine,
                1 + GetWordNum (
                        0, Line + iPrefixLen + strlen (szCmd) +
                               strlen (szTarget) + 4),
                sizeof (szRemLine) - 1);
            szRemLine[strlen (szRemLine) - 1] = '\0';
            sprintf (
                szResponse, NOX ("** CTCP Message from %s (%s)"), szNick,
                szRemLine);
            return szResponse;
        }
        ...
        if (szTarget[0] == '#') {
            ...
            if (iMode == MSG_LOCAL) {
                pszTempStr = GetWordNum (0, szRemLine);
                sprintf (
                    szResponse, NOX ("Private Message to <%s>: %s"), szNick,
                    pszTempStr);
            }
            else {
                pszTempStr = GetWordNum (0, szRemLine);
                sprintf (
                    szResponse, NOX ("Private Message from <%s>: %s"), szNick,
                    pszTempStr);
            }
            return szResponse;
        }
    ...
    if (iMode == MSG_LOCAL) { return NULL; }

    if (strcasecmp (szCmd, NOX ("NOTICE")) == 0) {
        pszTempStr = GetWordNum (0, szRemLine);
        strncpy (szTarget, pszTempStr, sizeof (szTarget) - 1);
        strncpy (
            szRemLine,
        ...
        if (szCTCPCmd[0] == 0x01) {
            ...
            szRemLine[strlen (szRemLine) - 1] =
                '\0'; // null out the ending 0x01
            if (strcasecmp (szCTCPCmd + 1, NOX ("PING")) == 0) { return NULL; }
            ...
            szRemLine[strlen (szRemLine) - 1] = '\0';
            sprintf (
                szResponse, XSTR ("** CTCP Message from %s (%s)", 635), szNick,
                szRemLine);
            return szResponse;
        }
    ...
    if (strcasecmp (szCmd, NOX ("JOIN")) == 0) {
        // see if it is me!
        if (strcasecmp (Nick_name, szNick) == 0) {
            Joined_channel = 1;
            if (strcasecmp (szChat_channel, NOX ("#autoselect")) == 0) {
                strncpy (
                    szChat_channel, GetWordNum (0, szRemLine),
                    sizeof (szChat_channel) - 1);
                AddChatCommandToQueue (
                    CC_YOURCHANNEL, szChat_channel,
                    strlen (szChat_channel) + 1);
            }
        }
        ...
        AddChatUser (szNick);
        sprintf (
            szResponse, XSTR ("** %s has joined %s", 636), szNick, szTarget);
        return NULL; // szResponse;
        // Add them to the userlist too!
    }
    if (strcasecmp (szCmd, NOX ("PART")) == 0) {
        pszTempStr = GetWordNum (0, szRemLine);
        strncpy (szTarget, pszTempStr, sizeof (szTarget) - 1);
        strncpy (
            szRemLine,
            Line + iPrefixLen + strlen (szCmd) + strlen (szTarget) + 3,
            sizeof (szRemLine) - 1);
        // see if it is me!
        if (strcasecmp (Nick_name, szNick) == 0) { RemoveAllChatUsers (); }

        RemoveChatUser (szNick);
        return NULL;
        // Remove them to the userlist too!
    }
    if (strcasecmp (szCmd, NOX ("KICK")) == 0) {
        pszTempStr = GetWordNum (0, szRemLine);
        strncpy (szTarget, pszTempStr, sizeof (szTarget) - 1);
        ...
        // see if it is me!
        if (strcasecmp (Nick_name, GetWordNum (1, szRemLine)) == 0) {
            // Yup, it's me!
            szChat_channel[0] = '\0';
        ...
        sprintf (
            szResponse,
            XSTR ("*** %s has kicked %s from channel %s (%s)", 637), szNick,
            szHackPrefix, szTarget, pszTempStr);
        // Remove them to the userlist too!
        RemoveChatUser (szNick);
        return szResponse;
    }
    if (strcasecmp (szCmd, NOX ("NICK")) == 0) {
        // see if it is me!
        if (strcasecmp (Nick_name, szNick) == 0) {
            // Yup, it's me!
            strncpy (
                Nick_name, GetWordNum (0, szRemLine), sizeof (Nick_name) - 1);
        }
        char nicks[70];
        ...
        sprintf (
            szResponse, XSTR ("*** %s is now known as %s", 638), szNick,
            GetWordNum (0, szRemLine));
        return szResponse;
    }
    if (strcasecmp (szCmd, NOX ("PING")) == 0) {
        // respond with pong (GetWordNum(0,szRemLine))
        sprintf (
            szResponse, NOX ("/PONG :%s"), GetWordNum (0, szRemLine));
        SendChatString (szResponse, 1);
        return NULL;
    }
    if (strcasecmp (szCmd, NOX ("MODE")) == 0) {
        // Channel Mode info
        return NULL;
    }

    if (strcasecmp (szCmd, "403") == 0) {
        // ERR_NOSUCHCHANNEL - Used to indicate the given channel name is
        // invalid.
        Joined_channel = -1;
        return NULL;
    }

    if (strcasecmp (szCmd, "401") == 0) {
        ...
        sprintf (
            szResponse, XSTR ("**Error: %s is not online!", 639), szWhoisUser);
        return szResponse;
    }
    if (strcasecmp (szCmd, "311") == 0) {
        char szWhoisUser[33];
        memset (szWhoisUser, 0, sizeof (szWhoisUser));
        strncpy (
            szWhoisUser, GetWordNum (1, szRemLine), sizeof (szWhoisUser) - 1);
        // This is whois user info, we can get their tracker info from here. -5
        strncpy (
            User_req_tracker_id, GetWordNum (5, szRemLine),
            sizeof (User_req_tracker_id) - 1);
        return NULL;
    }
    if (strcasecmp (szCmd, "319") == 0) {
        char szWhoisUser[33];
        memset (szWhoisUser, 0, sizeof (szWhoisUser));
        strncpy (
            szWhoisUser, GetWordNum (1, szRemLine), sizeof (szWhoisUser) - 1);
    ...
    if (strcasecmp (szCmd, "318") == 0) {
        if (!*User_req_channel) { User_req_channel[0] = '*'; }
    }

    if (strcasecmp (szCmd, "321") == 0) {
        // start of channel list
        FlushChannelList ();
        GettingChannelList = 1;
        return NULL;
    }
    if (strcasecmp (szCmd, "322") == 0) {
        // channel list data
        if (GettingChannelList == 1) {
            ...
            strncpy (
                channel_list_name, GetWordNum (1, szRemLine),
                sizeof (channel_list_name) - 1);
            AddChannel (
                channel_list_name, (short)atoi (GetWordNum (2, szRemLine)),
                sztopic);
        }
        return NULL;
    }
    if (strcasecmp (szCmd, "323") == 0) {
        // end of channel list
        GettingChannelList = 2;
        return NULL;
    }
    if (strcasecmp (szCmd, "324") == 0) {
        // Channel Mode info
        return NULL;
    }

    if (strcasecmp (szCmd, "332") == 0) {
        // Channel Topic, update status bar.
        if (strcasecmp (szChat_channel, szTarget) == 0) {
            // strncpy(szChanTopic,GetWordNum(2,szRemLine),70);
        }
        // sprintf(NewMsg.Message,"*** %s has changed the topic to:
        // %s",szNick,GetWordNum(2,szRemLine));

        return NULL;
    }
    if (strcasecmp (szCmd, NOX ("TOPIC")) == 0) {
        // Channel Topic, update status bar.
        if (strcasecmp (szChat_channel, szTarget) == 0) {
            // strncpy(szChanTopic,GetWordNum(1,szRemLine),70);
        }
        // sprintf(NewMsg.Message,"*** %s has changed the topic to:
        // %s",szNick,GetWordNum(1,szRemLine));
        return NULL;
    }
    if (strcasecmp (szCmd, NOX ("QUIT")) == 0) {
        // Remove the user!
        RemoveChatUser (szNick);
        return NULL;
    }
    if (strcasecmp (szCmd, "376") == 0) // end of motd, trigger autojoin...
    {
        if (!Chat_server_connected) { Chat_server_connected = 1; }

        // end of motd
        strncpy (
            szResponse, PXO_CHAT_END_OF_MOTD_PREFIX, sizeof (szResponse) - 1);
        return szResponse;
    }
    if ((strcasecmp (szCmd, "377") == 0) || (strcasecmp (szCmd, "372") == 0)) {
        // Stip the message, and display it.
        pszTempStr = GetWordNum (3, Line);
        sprintf (szResponse, "%s%s", PXO_CHAT_MOTD_PREFIX, pszTempStr);
        return szResponse;
    }
    // Ignore these messages
    if (((strcasecmp (szCmd, "366") == 0)) ||
        (strcasecmp (szCmd, "333") == 0) || // Who set the topic
        (strcasecmp (szCmd, "329") == 0))   // Time Channel created

    {
        return NULL;
    }
    if (strcasecmp (szCmd, "353") == 0) {
        ...
        while (pszTempStr) {
            if (pszTempStr[0] == '@') { AddChatUser (pszTempStr + 1); }
    ...
    // MOTD Codes
    if ((strcasecmp (szCmd, "001") == 0) || (strcasecmp (szCmd, "002") == 0) ||
        (strcasecmp (szCmd, "003") == 0) || (strcasecmp (szCmd, "004") == 0) ||
        (strcasecmp (szCmd, "251") == 0) || (strcasecmp (szCmd, "254") == 0) ||
        (strcasecmp (szCmd, "255") == 0) || (strcasecmp (szCmd, "265") == 0) ||
        (strcasecmp (szCmd, "372") == 0) || (strcasecmp (szCmd, "375") == 0)) {
        return NULL;
        // return szResponse;
    }
    if (strcasecmp (szCmd, "432") == 0) {
        // Channel Mode info
        sprintf (
            szResponse, "%s",
            XSTR ("Your nickname contains invalid characters", 640));
        AddChatCommandToQueue (CC_DISCONNECTED, NULL, 0);
        return szResponse;
    }
    if (strcasecmp (szCmd, "433") == 0) {
        // Channel Mode info
        char new_nick[33];
...
char* GetChannelList (void) {
    ...
    Currchannel = Firstchannel;
    while (Currchannel) {
        ichan_list_length += strlen (Currchannel->topic) + 1 +
                             strlen (Currchannel->channel_name) + 1 +
                             5; // 1 for the space, and 4 for the number of
                                // users 0000-9999 + space
...
void AddChannel (char* channel, unsigned short numusers, char* topic) {
    ...
    if (Firstchannel == NULL) {
        ...
        strncpy (
            ...
            sizeof (Currchannel->channel_name) - 1);
...
char* GetTrackerIdByUser (char* nickname) {
    ...
    if (GettingUserTID) {
        ...
        strncpy (
            Getting_user_tracker_info_for, nickname,
            sizeof (Getting_user_tracker_info_for) - 1);
...
char* GetChannelByUser (char* nickname) {
    ...
    if (GettingUserChannel) {
        ...
        strncpy (
            Getting_user_channel_info_for, nickname,
            sizeof (Getting_user_channel_info_for) - 1);
