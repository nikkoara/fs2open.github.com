...
#define MULTI_VOICE_STATUS_DENIED \
    1 // have been denied the token (show a red icon or something)
#define MULTI_VOICE_STATUS_RECORDING \
    2 // am currently recording (show a green icon or something)
#define MULTI_VOICE_STATUS_PLAYING \
    3 // playing back a stream (show another icon)

// max recording time for one stream
#define MULTI_VOICE_MAX_TIME 5000

// capabilities of this machine (make sure multi_voice_init() is called before
// referencing these)
...
// update the qos and/or duration of recording if the current setting is
// different from the passed in value
