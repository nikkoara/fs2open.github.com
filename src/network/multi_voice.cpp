...
#define MULTI_VOICE_LOCAL_ECHO // keep this defined for local echo of recorded
                               // network voice
...
int Multi_voice_send_mode =
    MULTI_MSG_NONE; // gotten from the multi_msg system when we start recording

// packet code defines
#define MV_CODE_GIVE_TOKEN \
    0 // received player side - he now has the token to speak
#define MV_CODE_DENY_TOKEN \
    1 // received player side - server has denied this request
#define MV_CODE_TAKE_TOKEN \
    2 // received player side - the server is forcibly taking his token
#define MV_CODE_RELEASE_TOKEN \
    3 // received server side - player is relinquishing token
#define MV_CODE_REQUEST_TOKEN \
    4 // received server side - player is requesting token
...
// sounds added to the front and end of a playing voice stream (set to -1 if
// none are wanted)
...
// size of an individual chunk (CHUNK == block of data stuck into a packet), in
// the worst case of header size (see above)
#define MULTI_VOICE_MAX_CHUNK_SIZE 488

// total max size of an incoming or an outgoing uncompressed buffer (note this
// is probably too big, but we won't worry about that for now)
...
char* Multi_voice_playback_buffer =
    NULL; // buffer for processing the accum buffer and playing the result
...
// the token index of a voice stream is set to one of these values, or the
// index of the player who has the token
#define MULTI_VOICE_TOKEN_INDEX_FREE -1 // the token (and the stream are free)
#define MULTI_VOICE_TOKEN_INDEX_RELEASED \
    0xBEAD // the token has been released but the stream is still active

typedef struct voice_stream {
    int token_status; // status of the token (player index if a player has it)
                      // or one of the above defines
    ...
    ubyte accum_buffer_flags
        [MULTI_VOICE_ACCUM_BUFFER_COUNT]; // flag indicating the existence of a
                                          // given accum (sub)buffer
    ushort accum_buffer_usize
        [MULTI_VOICE_ACCUM_BUFFER_COUNT]; // uncompressed size of the
                                          // corresponding (sub)buffer
    ushort accum_buffer_csize
        [MULTI_VOICE_ACCUM_BUFFER_COUNT]; // compressed size of the
                                          // corresponding (sub)buffer
    double accum_buffer_gain[MULTI_VOICE_ACCUM_BUFFER_COUNT]; // gain of the
                                                              // corresponding
                                                              // (sub)buffer
...
voice_stream
    Multi_voice_stream[MULTI_VOICE_MAX_STREAMS]; // voice streams themselves
...
int Multi_voice_recording =
    0; // flag indicating if we're currently recording or not
...
ubyte Multi_voice_stream_id =
    0; // stream id for the stream we're currently sending
int Multi_voice_current_stream_index =
    0; // packet index of the currently recodring stream
int Multi_voice_current_stream_sent = -1; // index of packet we've sent up to

// server-side data
ubyte Multi_voice_next_stream_id =
    0; // kept on the server - given to the next valid token requester
...
int multi_voice_process_data (
    ubyte* data, int player_index, int msg_mode, net_player* target);

// <server> increment the current stream id#
void multi_voice_inc_stream_id ();

// flush any old sound stream data because we've started to receive data for a
// new stream
void multi_voice_flush_old_stream (int stream_index);

// route sound data through the server to all appropriate players
void multi_voice_route_data (
    ubyte* data, int packet_size, int player_index, int mode,
    net_player* target);

// find the stream to apply incoming sound data to, freeing up old ones as
// necessary
...
// process incoming sound data in whatever way necessary (this function should
// take care of playing data when necessary)
void multi_voice_alg_process_data (int stream_index);

// process existing streams
void multi_voice_alg_process_streams ();

// we are going to flush the current stream because we have started to receive
// data for a new one. do something first
void multi_voice_alg_flush_old_stream (int stream_index);

// is the given sound stream playing (compares uncompressed sound size with
...
// process and play the current window of sound stream data we have. reset the
// window for the next incoming data as well
...
void multi_voice_init () {
    ...
    if (Multi_voice_can_play) {
        ...
        if (pre_sound.isValid ()) {
            ...
            if ((snd_size (pre_sound, &pre_size) != -1) &&
                (pre_size < MULTI_VOICE_MAX_BUFFER_SIZE)) {
    ...
    memset (
        Multi_voice_stream, 0,
        sizeof (voice_stream) * MULTI_VOICE_MAX_STREAMS);
...
void multi_voice_close () {
    ...
    for (idx = 0; idx < MULTI_VOICE_MAX_STREAMS; idx++) {
        if (Multi_voice_stream[idx].stream_rtvoice_handle != -1) {
            rtvoice_free_playback_buffer (
                Multi_voice_stream[idx].stream_rtvoice_handle);
            Multi_voice_stream[idx].stream_rtvoice_handle = -1;
            Multi_voice_stream[idx].stream_snd_handle =
...
void multi_voice_process () {
    ...
    for (idx = 0; idx < MULTI_VOICE_MAX_STREAMS; idx++) {
        if ((Multi_voice_stream[idx].stream_snd_handle.isValid ()) &&
            !multi_voice_stream_playing (idx)) {
            Multi_voice_stream[idx].stream_snd_handle =
                sound_handle::invalid ();
        }
    }
    ...
    // all "players" do this, except the standalone who isn't a real player by
    // definition
    if (!(Game_mode & GM_STANDALONE_SERVER)) { multi_voice_player_process (); }
...
// the status of the voice system - use this to determine what bitmaps to
// display, etc see above MULTI_VOICE_STATUS_* defines
int multi_voice_status () {
    ...
    if (Multi_voice_denied_stamp != -1) { return MULTI_VOICE_STATUS_DENIED; }

    // if we're currently recording (has precedence over playing back a sound
    // from somebody)
    if (Multi_voice_recording) { return MULTI_VOICE_STATUS_RECORDING; }
    ...
    for (idx = 0; idx < MULTI_VOICE_MAX_STREAMS; idx++) {
        ...
        if (Multi_voice_stream[idx].stream_snd_handle.isValid ()) {
            if ((earliest == -1) ||
                (Multi_voice_stream[idx].stream_start_time < earliest_time)) {
                earliest = idx;
    ...
    if (earliest != -1) { return MULTI_VOICE_STATUS_PLAYING; }
...
void multi_voice_maybe_update_vars (int new_qos, int new_duration) {
    ...
    if ((new_qos != Multi_voice_qos) ||
        (new_duration != Multi_voice_max_time)) {
...
void multi_voice_server_process () {
    ...
    for (idx = 0; idx < MULTI_VOICE_MAX_STREAMS; idx++) {
        ...
        case MULTI_VOICE_TOKEN_INDEX_FREE: break;

        // if the token has been released - check to see if the stream is
        // "done" (ie, can be marked as FREE once again)
        case MULTI_VOICE_TOKEN_INDEX_RELEASED:
            // if the stream_last_heard var is -1, it means we never got sound
            // from this guy so free the token up immediately
    ...
    // for each netplayer, if his token wait timestamp is running, see if it
    // has popped yet
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            (Net_players[idx].s_info.voice_token_timestamp != -1) &&
            timestamp_elapsed (
                Net_players[idx].s_info.voice_token_timestamp)) {
...
// process voice details as a player (may also be the server)
void multi_voice_player_process () {
    // if the voice key is down for the first time this frame, send a request
    // for the token
    ...
    if (Multi_voice_keydown && multi_voice_keydown () &&
        ...
        if (Multi_voice_token) {
            ...
            if (!Multi_voice_recording) {
                ...
                // start the recording process with the appropriate callback
                // function
    ...
    // if the "denied" timestamp is set, but has elapsed or the user has let up
    // on the key, set it to -1
    if ((Multi_voice_denied_stamp != -1) &&
        (timestamp_elapsed (Multi_voice_denied_stamp) ||
         !multi_voice_keydown ())) {
        Multi_voice_denied_stamp = -1;
    }
}
...
// determine if the voice key is down this frame
int multi_voice_keydown () {
    // if we're in the options screen, we should never allow the button to be
    // pressed
    if (gameseq_get_state () == GS_STATE_OPTIONS_MENU) { return 0; }

    // if we're pre-game, we should just be checking the keyboard bitflags
    if (!(Game_mode & GM_IN_MISSION)) {
        return (keyd_pressed[MULTI_VOICE_KEY] &&
...
void multi_voice_give_token (int stream_index, int player_index) {
    ...
    Multi_voice_stream[stream_index].token_stamp =
        timestamp (MULTI_VOICE_TOKEN_TIMEOUT);
    ...
    // set the last heard from time to -1 to indicate we've heard no sound from
    // this guy
...
void multi_voice_take_token (int stream_index) {
    ...
    // "officially" already
    if ((Multi_voice_stream[stream_index].token_status ==
         (int)MULTI_VOICE_TOKEN_INDEX_FREE) ||
        (Multi_voice_stream[stream_index].token_status ==
         (int)MULTI_VOICE_TOKEN_INDEX_RELEASED)) {
    ...
    if (Net_player ==
        &Net_players[Multi_voice_stream[stream_index].token_status]) {
        Multi_voice_token = 0;

        // timestamp this guy so that he can't get the token back immediately
        Net_player->s_info.voice_token_timestamp =
            timestamp (Netgame.options.voice_token_wait);
    }
    else {
        ...
        multi_io_send_reliable (
            &Net_players[Multi_voice_stream[stream_index].token_status], data,
            packet_size);

        // timestamp this guy so that he can't get the token back immediately
        Net_players[Multi_voice_stream[stream_index].token_status]
            .s_info.voice_token_timestamp =
            timestamp (Netgame.options.voice_token_wait);
    }

    // take the token back from the dude
    Multi_voice_stream[stream_index].token_status =
        MULTI_VOICE_TOKEN_INDEX_RELEASED;
    Multi_voice_stream[stream_index].token_stamp = -1;
}
...
// set the default voice quality and duration (if server passes -1, he just
// broadcasts the qos to all clients)
...
void multi_voice_process_token_request (int player_index) {
    ...
    if (Netgame.options.flags & MSO_FLAG_NO_VOICE) { return; }
    ...
    for (idx = 0; idx < MULTI_VOICE_MAX_STREAMS; idx++) {
        if (Multi_voice_stream[idx].token_status ==
            MULTI_VOICE_TOKEN_INDEX_FREE) {
...
void multi_voice_player_send_stream () {
    ...
    // get the data
    rtvoice_get_data (
        (unsigned char**)&Multi_voice_record_buffer, &sound_size, &d_gain);
    gain = (float)d_gain;
    ...
    if (msg_mode == MULTI_MSG_TARGET) {
        if (Player_ai->target_objnum != -1) {
            target_index = multi_find_player_by_object (
    ...
    while (size_sent < sound_size) {
        ...
        ADD_DATA (msg_mode);
        if (msg_mode == MULTI_MSG_TARGET) {
            ADD_DATA (Objects[Net_players[target_index].m_player->objnum]
                          .net_signature);
        }
        ...
        if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
            multi_voice_route_data (
                data, packet_size, MY_NET_PLAYER_NUM, (int)msg_mode,
                (target_index == -1) ? NULL : &Net_players[target_index]);
        }
        else {
...
int multi_voice_process_data (
    ...
    if ((Netgame.options.flags & MSO_FLAG_NO_VOICE) ||
        !Multi_options_g.std_voice) {
        offset += chunk_size;
        return offset;
    }

    // get a handle to a valid stream to be using, freeing old streams as
    // necessary
    ...
    if (stream_index != -1) {
        ...
        // set the gain
        Multi_voice_stream[stream_index].accum_buffer_gain[chunk_index] =
            (double)gain;

        // set the stream uncompressed size size
        Multi_voice_stream[stream_index].accum_buffer_usize[chunk_index] =
            uc_size;
        ...
        Multi_voice_stream[stream_index].stream_last_heard =
        ...
        if (Multi_voice_can_play) {
            memcpy (
                Multi_voice_stream[stream_index].accum_buffer[chunk_index],
                data + offset, (int)chunk_size);
        }

        Multi_voice_stream[stream_index].accum_buffer_flags[chunk_index] = 1;
        Multi_voice_stream[stream_index].accum_buffer_csize[chunk_index] =
            chunk_size;
...
void multi_voice_flush_old_stream (int stream_index) {
    ...
    memset (
        Multi_voice_stream[stream_index].accum_buffer_flags, 0,
        MULTI_VOICE_ACCUM_BUFFER_COUNT);
    ...
    case MULTI_MSG_ALL:
        for (idx = 0; idx < MAX_PLAYERS; idx++) {
            if (MULTI_CONNECTED (Net_players[idx]) && // player is connected
                (&Net_players[idx] !=
                 &Net_players[player_index]) &&      // not the sending player
                (Net_player != &Net_players[idx]) && // not me
                (Multi_voice_player_prefs[idx] &
                 (1 << player_index)) && // is accepting sound from this player
        ...
        for (idx = 0; idx < MAX_PLAYERS; idx++) {
            if (MULTI_CONNECTED (Net_players[idx]) && // player is connected
                (&Net_players[idx] !=
                 &Net_players[player_index]) &&      // not the sending player
                (Net_player != &Net_players[idx]) && // not me
                (Net_players[idx].p_info.team ==
                 Net_players[player_index].p_info.team) && // on the same team
                (Multi_voice_player_prefs[idx] &
                 (1 << player_index)) && // is accepting sound from the sender
                !(Net_players[idx].p_info.options.flags &
                  MLO_FLAG_NO_VOICE)) { // is accepting sound periods
            ...
            if (MULTI_CONNECTED (Net_players[idx]) && // player is connected
                ...
                (Net_players[idx].p_info.team !=
                 Net_players[player_index]
                     .p_info.team) && // on the opposite team
                (Multi_voice_player_prefs[idx] &
                 (1 << player_index)) && // is accepting sound from the sender
                !(Net_players[idx].p_info.options.flags &
                  MLO_FLAG_NO_VOICE)) { // is accepting sound periods
...
int multi_voice_get_stream (int stream_id) {
    ...
    // if we got to this point, we didn't find the matching stream, so we
    // should try and find an empty stream
    for (idx = 0; idx < MULTI_VOICE_MAX_STREAMS; idx++) {
        if (Multi_voice_stream[idx].token_stamp == -1) { return idx; }
    }
    ...
    for (idx = 0; idx < MULTI_VOICE_MAX_STREAMS; idx++) {
        if (((max_diff_index == -1) ||
             ((cur_time - Multi_voice_stream[idx].stream_last_heard) >
              max_diff)) &&
            (Multi_voice_stream[idx].token_stamp != -1)) {
            max_diff_index = idx;
...
int multi_voice_stream_playing (int /*stream_index*/) {
    ...
    // if the sound is playing and the buffer is past the uncompressed size,
    its effectively done
    if(ds_get_play_position(ds_get_channel(Multi_voice_stream[stream_index].stream_snd_handle))
    >= (DWORD)Multi_voice_stream[stream_index].stream_uc_size){ return 1;
    }
    */
...
// tack on pre and post sounds to a sound stream (pass -1 for either if no
// sound is wanted) return final buffer size
int multi_voice_mix (
    gamesnd_id post_sound, char* data, int cur_size, int max_size) {
    int post_size;

    // if the user passed -1 for both pre and post sounds, don't do a thing
    if (!post_sound.isValid ()) { return cur_size; }

    // get the sizes of the additional sounds

    // post sound
    auto gs = gamesnd_get_game_sound (post_sound);
    auto post_sound_handle =
        snd_load (gamesnd_choose_entry (gs), gs->flags, 0);
    if (post_sound_handle.isValid ()) {
        if (snd_size (post_sound_handle, &post_size) == -1) { post_size = 0; }
    }
    else {
        post_size = 0;
    }
...
int multi_voice_max_chunk_size (int msg_mode) {
    ...
    return (
        MAX_PACKET_SIZE - // max freespace packet size
        1 -               // packet type
...
// process the "next" chunk of standalone valid sound data from the rtvoice
// system
void multi_voice_process_next_chunk () {
    ...
    memcpy (
        str->accum_buffer[Multi_voice_current_stream_index],
        Multi_voice_record_buffer, sound_size);
    str->stream_from = Net_player->player_id;
    str->accum_buffer_flags[Multi_voice_current_stream_index] = 1;
    str->accum_buffer_usize[Multi_voice_current_stream_index] =
        (ushort)sound_size;
    str->accum_buffer_csize[Multi_voice_current_stream_index] =
        (ushort)sound_size;
...
void multi_voice_send_dummy_packet () {
    ...
    if (msg_mode == MULTI_MSG_TARGET) {
        ADD_USHORT (
            Objects[Net_players[target_index].m_player->objnum].net_signature);
    }
...
void multi_voice_process_packet (ubyte* data, header* hinfo) {
    ...
    case MV_CODE_TAKE_TOKEN:
        ...
        if (!Multi_voice_can_record) { Int3 (); }
    ...
    case MV_CODE_GIVE_TOKEN:
        ...
        if (!Multi_voice_keydown) { multi_voice_release_token (); }
        else {
            Multi_voice_token = 1;
        }
        break;
    ...
    case MV_CODE_RELEASE_TOKEN:
        ...
        if (stream_index >= 0) {
            ...
            // timestamp this guy so that he can't get the token back
            // immediately
            Net_players[player_index].s_info.voice_token_timestamp =
                timestamp (Netgame.options.voice_token_wait);
        }
        break;
    ...
    case MV_CODE_DATA:
        ...
        offset += multi_voice_process_data (
            data + offset, player_index, msg_mode,
            (target_index == -1) ? NULL : &Net_players[target_index]);

        // if we're the server of the game, we should also route this data to
        // all other players
        if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
            multi_voice_route_data (
                data, offset, player_index, msg_mode,
...
void multi_voice_client_send_pending () {
    ...
    if (!Multi_voice_recording || (Multi_voice_current_stream_sent < 0) ||
        (Multi_voice_current_stream_sent > Multi_voice_current_stream_index)) {
        return;
    }

    // stream all buffered up packets
    str = &Multi_voice_stream[0];
    while (Multi_voice_current_stream_sent <
           Multi_voice_current_stream_index) {
    ...
    if (Multi_voice_can_play) {
        ...
        for (idx = 0; idx < MULTI_VOICE_ACCUM_BUFFER_COUNT; idx++) {
            ...
            if (st->accum_buffer_flags[idx]) {
                ...
                rtvoice_uncompress (
                    st->accum_buffer[idx], (int)st->accum_buffer_csize[idx],
                    st->accum_buffer_gain[idx],
                    (ubyte*)Multi_voice_playback_buffer + buffer_offset,
        ...
        for (idx = 0; idx < MULTI_VOICE_ACCUM_BUFFER_COUNT; idx++) {
            ...
            if (st->accum_buffer_flags[idx]) {
                memcpy (
                    Multi_voice_unpack_buffer + buffer_offset,
                    st->accum_buffer[idx], st->accum_buffer_csize[idx]);
        ...
        rtvoice_uncompress (
            (ubyte*)Multi_voice_unpack_buffer, buffer_offset,
            st->accum_buffer_gain[0], (ubyte*)Multi_voice_playback_buffer,
            st->accum_buffer_usize[0]);
        buffer_offset = st->accum_buffer_usize[0];
#endif

        // mix in the SND_CUE_VOICE and the SND_END_VOICE game sounds
        buffer_offset = multi_voice_mix (
        ...
        rtvoice_stop_playback (
            Multi_voice_stream[stream_index].stream_rtvoice_handle);
        Multi_voice_stream[stream_index].stream_snd_handle =
            sound_handle::invalid ();
        ...
        if (Multi_voice_can_play) {
            ...
            if (player_index != -1) {
                ...
                sprintf (
                    voice_msg, XSTR ("<%s is speaking>", 712),
                    Net_players[player_index].m_player->callsign);

                // display a chat message (write to the correct spot - hud,
                // standalone gui, chatbox, etc)
                multi_display_chat_msg (voice_msg, player_index, 0);
            }
        }

        // now call the rtvoice playback functions
        Multi_voice_stream[stream_index].stream_snd_handle = rtvoice_play (
            Multi_voice_stream[stream_index].stream_rtvoice_handle,
            (unsigned char*)Multi_voice_playback_buffer, buffer_offset);
        Multi_voice_stream[stream_index].stream_start_time =
            timer_get_fixed_seconds ();
    }
...
// decision function which decides if we should play the current block of sound
// we have
...
void multi_voice_alg_process_streams () {
    ...
    for (idx = 0; idx < MULTI_VOICE_MAX_STREAMS; idx++) {
        ...
        if ((Multi_voice_stamps[idx] != -1) &&
            multi_voice_alg_should_play (idx)) {
            ...
            if ((Net_player->flags & NETINFO_FLAG_AM_MASTER) &&
                ((Net_player->p_info.options.flags & MLO_FLAG_NO_VOICE) ||
                 (player_index == -1) ||
                 !(Multi_voice_player_prefs[MY_NET_PLAYER_NUM] &
                   (1 << player_index)))) {
                // unset the stamp so that its not "free"
                Multi_voice_stamps[idx] = -1;

                // flush the stream (will also grab the token back, if the
                // server)
...
void multi_voice_test_process_next_chunk () {
    ...
    // otherwise get the compressed and uncompressed data and do something
    // interesting with it
...
void multi_voice_test_record_start () {
    ...
    if (Multi_voice_test_record_stamp != -1) { return; }
...
// call this function if multi_voice_test_recording() is true to process
// various odds and ends of the test recording
...
// return whether the last sampled chunk would have been too large to fit in a
// packet
