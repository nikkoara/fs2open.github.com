...
#define MULTI_OPTION_START_GAME \
    2 // host's start game options on the standalone server
#define MULTI_OPTION_MISSION \
    3 // host's mission selection stuff on a standalone server
...
void multi_options_read_config () {
    ...
    Multi_options_g.port =
        (Cmdline_network_port >= 0)
            ? (ushort)Cmdline_network_port
            : forced_port == 0 ? (ushort)DEFAULT_GAME_PORT : forced_port;
        ...
        while (!cfeof (in)) {
            ...
            if (Is_standalone) {
                ...
                if (SETTING ("+pxo")) {
                    ...
                    if (tok != NULL) {
                        strncpy (
                            Multi_options_g.std_pxo_login, tok,
                            MULTI_TRACKER_STRING_LEN);
                    }
                }
                else
                    ...
                    if (tok != NULL) {
                        strncpy (
                            Multi_options_g.std_pxo_password, tok,
                            MULTI_TRACKER_STRING_LEN);
                    }
                }
                else if (SETTING ("+webui_root")) {
                    NEXT_TOKEN ();
                    if (tok != NULL) {
                        Multi_options_g.webuiRootDirectory = std::string (tok);
                    }
                }
                else if (SETTING ("+webapi_username")) {
                    NEXT_TOKEN ();
                    if (tok != NULL) {
                        Multi_options_g.webapiUsername = std::string (tok);
                    }
                }
                else if (SETTING ("+webapi_password")) {
                    NEXT_TOKEN ();
                    if (tok != NULL) {
                        Multi_options_g.webapiPassword = std::string (tok);
                    }
                }
                else if (SETTING ("+webapi_server_port")) {
                    NEXT_TOKEN ();
                    if (tok != NULL) {
            ...
            if (SETTING ("+user_server")) {
                ...
                NEXT_TOKEN ();
                if (tok != NULL) { strcpy_s (Multi_options_g.pxo_ip, tok); }
            }
            else
                ...
                if (tok != NULL) {
                    ...
                    if (ip != NULL) { strcpy_s (Multi_options_proxy, ip); }
...
void multi_options_set_netgame_defaults (multi_server_options* options) {
    ...
    options->voice_token_wait =
        2000; // he must wait 2 seconds between voice gets
...
void multi_options_set_local_defaults (multi_local_options* options) {
    ...
    // set the object update level based on the type of network connection
    // specified by the user at install (or launcher) time.
...
// fill in the passed netgame options struct with the data from my player file
// data (only host/server should do this)
void multi_options_netgame_load (multi_server_options* options) {
    if (options != NULL) {
        memcpy (
            options, &Player->m_server_options, sizeof (multi_server_options));
    }
}

// fill in the passed local options struct with the data from my player file
// data (all machines except standalone should do this)
void multi_options_local_load (
    multi_local_options* options, net_player* pxo_pl) {
    if (options != NULL) {
        memcpy (
            options, &Player->m_local_options, sizeof (multi_local_options));
    }
...
// update the standalone with the settings I have picked at the "start game"
// screen
void multi_options_update_start_game (netgame_info* ng) {
    ...
    switch (ng->mode) {
    case NG_MODE_PASSWORD: ADD_STRING (ng->passwd); break;
    case NG_MODE_RANK_ABOVE:
    case NG_MODE_RANK_BELOW: ADD_INT (ng->rank_base); break;
    }
...
// update the standalone with the mission settings I have picked (mission
// filename, etc)
void multi_options_update_mission (netgame_info* ng, int campaign_mode) {
    ...
    ADD_DATA (code);
    if (campaign_mode) { ADD_STRING (ng->campaign_name); }
    else {
...
void multi_options_process_packet (unsigned char* data, header* hinfo) {
    ...
    case MULTI_OPTION_START_GAME:
        ...
        switch (Netgame.mode) {
        case NG_MODE_PASSWORD: GET_STRING (Netgame.passwd); break;
        case NG_MODE_RANK_ABOVE:
        case NG_MODE_RANK_BELOW: GET_INT (Netgame.rank_base); break;
        }
        ...
        if (code) {
            ...
            if (strcmp (Netgame.campaign_name, ng.campaign_name) != 0) {
                ...
                if (!mission_campaign_get_info (
                        ng.campaign_name, title, &campaign_type,
                        &max_players)) {
        ...
        // non-campaign mode
            ...
            if (strcmp (Netgame.mission_name, ng.mission_name) != 0) {
                if (strlen (ng.mission_name)) {
                    ...
                    // setting this to -1 will prevent us from being seen on
                    // the network
    ...
    case MULTI_OPTION_SERVER:
        ...
        // if i'm the standalone server, I should rebroadcast to all other
        // players
        if (Game_mode & GM_STANDALONE_SERVER) {
    ...
    case MULTI_OPTION_LOCAL:
        if (player_index == -1) { get_local_options (data, &offset, &bogus); }
        else {
            get_local_options (
                data, &offset, &Net_players[player_index].p_info.options);

            // If the client has sent an object update higher than that which
            // the server allows, reset it
            if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
                if (Net_players[player_index].p_info.options.obj_update_level >
                    Cmdline_objupd) {
                    Net_players[player_index].p_info.options.obj_update_level =
                        Cmdline_objupd;
                }
            }
        }
