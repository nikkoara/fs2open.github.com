...
// if a key is down less than this time, fire up the test messaging system,
// otherwise fire up the voice messaging system
...
// timestamp set when we leave messaging mode, use to keep eating keys for a
...
// command defines - all these commands must be followed by a ":" so that we
...
// process an entered line of text and maybe perform a command on it (return 1
// if an action was performed, 0 if not)
int multi_msg_check_command (char* str);

// perform the passed command (MULTI_MSG_CMD_* define) with the passed string
// argument
...
void multi_msg_key_down (int mode) {
    ...
    if ((Multi_msg_eat_stamp != -1) &&
        !timestamp_elapsed (Multi_msg_eat_stamp)) {
        return;
    }
...
// returns true when messaging system has determined that we should be
// messaging with voice
int multi_msg_voice_record () {
    return ((Multi_msg_mode != MULTI_MSG_NONE) &&
            timestamp_elapsed (Multi_msg_stamp) && Multi_msg_repeat_flag &&
            !Multi_msg_text_enter)
               ? 1
               : 0;
}

// general processing function to do things like timing keydown, etc. call from
// multi_do_frame()
void multi_msg_process () {
    ...
    if (Multi_msg_mode == MULTI_MSG_NONE) { return; }

    // if the key has been released
    if (!Multi_msg_repeat_flag && (Multi_msg_stamp != -1) &&
        !Multi_msg_text_enter) {
        // if the timestamp had not yet elapsed, fire up the text messaging
        // system this is the equivalent of a (TAP)
...
int multi_msg_mode () { return Multi_msg_mode; }

// return 0 or 1 if in text chat mode or not
int multi_msg_text_mode () { return Multi_msg_text_enter; }
...
void multi_msg_eval_text_msg () {
    ...
    // all other modes
        ...
        // send the chat packet
        send_game_chat_packet (
            Net_player, Multi_msg_text, Multi_msg_mode, NULL);
...
// maybe process a keypress in text messaging mode, return true if the key was
// processed
int multi_msg_text_process (int k) {
    ...
    if (!Multi_msg_text_enter) { return 0; }

    switch (k) {
    // cancel the message
    case KEY_ESC: multi_msg_text_flush (); break;
    ...
    case KEY_RCTRL + KEY_SHIFTED: break;
...
// return 0 or 1 if there is multi text to be rendered (filling in txt if
// necessary)
int multi_msg_message_text (char* txt) {
    ...
    case MULTI_MSG_ALL: strcpy (txt, XSTR ("ALL : ", 694)); break;

    // messaging friendly players
    case MULTI_MSG_FRIENDLY: strcpy (txt, XSTR ("FRIENDLY : ", 695)); break;

    // messaging hostile players
    case MULTI_MSG_HOSTILE: strcpy (txt, XSTR ("HOSTILE : ", 696)); break;

    // messaging targeted ship
    case MULTI_MSG_TARGET: strcpy (txt, XSTR ("TARGET : ", 697)); break;
...
void multi_msg_display_mission_text (const char* msg, int player_index) {
    ...
    if (MULTI_STANDALONE (Net_players[player_index])) {
        HUD_sourced_printf (
            HUD_SOURCE_NETPLAYER, "%s %s", XSTR ("<SERVER>", 698), msg);
    }
    else {
        HUD_sourced_printf (
            HUD_SOURCE_NETPLAYER, "%s: %s",
            Net_players[player_index].m_player->callsign, msg);
    }
}

// if the passed net_player's callsign matches the reg expression of the passed
// expr
...
void multi_msg_text_flush () {
    ...
    // keep eating keys for a short period of time and unset any used control
    // bits
...
int multi_msg_check_command (char* str) {
    ...
    if (Multi_msg_mode == MULTI_MSG_ALL) {
        ...
        // send the chat packet
        send_game_chat_packet (
            Net_player, Multi_msg_text, MULTI_MSG_EXPR, NULL, prefix);
...
// argument
void multi_msg_perform_command (int command, char* param) {
    // we may eventually want to split each of these cases into its own
    // function to make things neater
    switch (command) {
    // kick a player
    case MULTI_MSG_CMD_KICK:
        int np_index = multi_find_player_by_callsign (param);
        if (np_index != -1) { multi_kick_player (np_index); }
        break;
    }
}
...
const char* Multi_msg_subsys_name[SUBSYSTEM_MAX] = {
    "None",    "Engine",      "Turret",        "Bridge",
    "Radar",   "Navigation",  "Communication", "Weapons",
    "Sensors", "Solar Array", "Unknown"
};

// XSTR:ON

// display a squadmsg order directed towards _me_
void multi_msg_show_squadmsg (
    net_player* source, int command, ushort target_sig, int subsys_type) {
    char hud_string[255];
    ...
    sprintf (
        hud_string, XSTR ("ORDER FROM <%s> : ", 699),
        source->m_player->callsign);
    ...
    case ATTACK_TARGET_ITEM:
        if ((target_obj != NULL) && (target_obj->type == OBJ_SHIP)) {
            sprintf (
                temp_string, XSTR ("Attack %s", 700),
                Ships[target_obj->instance].get_display_string ());
    ...
    case DISABLE_TARGET_ITEM:
        if ((target_obj != NULL) && (target_obj->type == OBJ_SHIP)) {
            sprintf (
                temp_string, XSTR ("Disable %s", 701),
                Ships[target_obj->instance].get_display_string ());
            strcat_s (hud_string, temp_string);
        }
        else {
    ...
    case PROTECT_TARGET_ITEM:
        if ((target_obj != NULL) && (target_obj->type == OBJ_SHIP)) {
            sprintf (
                temp_string, XSTR ("Protect %s", 702),
                Ships[target_obj->instance].get_display_string ());
            strcat_s (hud_string, temp_string);
        }
        else {
    ...
    case DISABLE_SUBSYSTEM_ITEM:
        if ((target_obj != NULL) && (target_obj->type == OBJ_SHIP) &&
            (subsys_type != -1) && (subsys_type != 0)) {
            sprintf (
                temp_string, XSTR ("Disable subsystem %s on %s", 705),
                Multi_msg_subsys_name[subsys_type],
                Ships[target_obj->instance].get_display_string ());
            strcat_s (hud_string, temp_string);
        }
        else {
    ...
    // cover me
    case COVER_ME_ITEM: strcat_s (hud_string, XSTR ("Cover me", 707)); break;
    ...
    default: should_display = 0; break;
    }

    // print it out
    if (should_display) { HUD_printf ("%s", hud_string); }
}
...
int multi_msg_player_in_wing (int wingnum, net_player* pl) {
    ...
    for (idx = 0; idx < Wings[wingnum].current_count; idx++) {
        // if we found a match
        if (Wings[wingnum].ship_index[idx] ==
            Objects[pl->m_player->objnum].instance) {
            return 1;
        }
    }
...
void multi_msg_send_squadmsg_packet (
    net_player* target, net_player* source, int command, ushort net_sig,
    int subsys_type) {
    ubyte data[100];
    char s_val;
    int packet_size;
...
int multi_msg_eval_wing_squadmsg (
    int wingnum, int command, ai_info* aif, int player_num) {
    int idx;
    ushort net_sig;
    int subsys_type;
    int sent_count;
    ...
    if (player_num == -1) player_num = MY_NET_PLAYER_NUM;

    // get the target information
    if (aif->target_objnum == -1) { net_sig = 0; }
    else {
        net_sig = Objects[aif->target_objnum].net_signature;
    }
    subsys_type = -1;
    if ((aif->targeted_subsys == NULL) ||
        (aif->targeted_subsys->system_info == NULL)) {
        subsys_type = -1;
    }
    else {
        ...
        if (MULTI_CONNECTED (Net_players[idx]) &&
            ...
            if (multi_msg_player_in_wing (wingnum, &Net_players[idx])) {
                ...
                if (&Net_players[idx] == Net_player) {
                    multi_msg_show_squadmsg (
                        &Net_players[player_num], command, net_sig,
                        subsys_type);
                    sent_count--;
                }
                // otherwise send it to who is supposed to get it
                else {
                    multi_msg_send_squadmsg_packet (
                        &Net_players[idx], &Net_players[player_num], command,
                        net_sig, subsys_type);
                    sent_count--;
                }
            }
...
int multi_msg_eval_ship_squadmsg (
    int shipnum, int command, ai_info* aif, int player_num) {
    int idx;
    ushort net_sig;
    int subsys_type;
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            !MULTI_STANDALONE (Net_players[idx]) && (idx != player_num)) {
    ...
    // this will let the messaging system show a response to the sender of the
    // packet
    return 0;
}
...
void multi_msg_process_squadmsg_packet (unsigned char* data, header* hinfo) {
    ...
    if (source_index == -1) {
        WARNINGF (
            LOCATION, "Received squadmsg order packet from unknown player!!");
        return;
    }

    // display the squadmessage somehow
    multi_msg_show_squadmsg (
        &Net_players[source_index], command, net_sig, (int)s_val);
}
