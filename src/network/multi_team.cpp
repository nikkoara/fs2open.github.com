...
#define MT_CODE_TEAM_UPDATE \
    0 // send a full team update on a player-per-player basis
#define MT_CODE_TEAM_REQUEST \
    1 // a request sent to the host to be be on a given team
...
void multi_team_process_team_change_request (
    net_player* pl, net_player* who_from, int team);
...
void multi_team_level_init () {
    ...
    for (int i = 0; i < MAX_TVT_TEAMS; i++) Multi_team_score[i] = 0;
}
...
int multi_team_winner () {
    ...
    for (i = 0; i < Num_teams; i++) {
        if (Multi_team_score[i] == highest) num_equal++;
    }

    // tie situation?
    if (num_equal > 1) return -1;
...
void multi_team_maybe_add_score (int points, int team) {
    ...
    // if not squad war
    if (!(Netgame.type_flags & NG_TYPE_TEAM)) { return; }
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx])) {
            ...
            Net_players[idx].flags &=
                ~(NETINFO_FLAG_TEAM_LOCKED | NETINFO_FLAG_TEAM_CAPTAIN);
        }
    }

    // host should divvy up the teams, send a reset notice to all players and
    // assign captains
...
// set the team of this given player (if called by the host, the player becomes
// locked, cnad only the host can modify him from thereon)
...
void multi_team_host_assign_default_teams () {
    ...
    if (player_count < 2) { team0_count = 1; }
    else {
...
void multi_team_sync_captains () {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            ...
            case 0: team0_cap = 1; break;
            case 1: team1_cap = 1; break;
            }
        }
    }
...
int multi_team_ok_to_commit () {
    ...
    return ((team0_captains == 1) && (team1_captains == 1) && team0_count &&
            team1_count)
               ? 1
               : 0;
}
...
void multi_team_handle_drop () {
    ...
    if (Game_mode & GM_IN_MISSION) {
        ...
        if (team0_count <= 0) {
            multi_quit_game (
                ...
                MULTI_END_ERROR_TEAM0_EMPTY);
            return;
        }
        if (team1_count <= 0) {
            multi_quit_game (
                PROMPT_NONE, MULTI_END_NOTIFY_NONE,
                MULTI_END_ERROR_TEAM1_EMPTY);
            return;
        }
    }
    ...
    // if we have lost a team captain and we're not in the forming state, abort
    // the game
    if ((!team0_cap || !team1_cap) &&
        ...
        // quit the game
        multi_quit_game (
            PROMPT_NONE, MULTI_END_NOTIFY_NONE, MULTI_END_ERROR_CAPTAIN_LEFT);
    }
...
void multi_team_handle_join (net_player* pl) {
    ...
    if (pl->flags & NETINFO_FLAG_OBSERVER) { return; }
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            ...
            !MULTI_PERM_OBSERVER (Net_players[idx])) {
            // player is on team 0
            if (Net_players[idx].p_info.team == 0) { team0_count++; }
            // player is on team 1
...
// set all ships in the mission to be marked as the proper team (TEAM_HOSTILE,
// TEAM_FRIENLY)
...
void multi_team_mark_ship (ship* sp) {
    ...
    for (i = 0; i < MAX_TVT_WINGS; i++) {
        if (sp->wingnum == TVT_wings[i]) sp->team = i;
    }
...
void multi_team_get_player_counts (int* team0, int* team1) {
    ...
    if (team0 != NULL) { (*team0) = 0; }
    if (team1 != NULL) { (*team1) = 0; }
...
#define SEND_AND_DISPLAY(mesg)                               \
    do {                                                     \
        send_game_chat_packet (                              \
            Net_player, mesg, MULTI_MSG_ALL, NULL, NULL, 1); \
        multi_display_chat_msg (mesg, 0, 0);                 \
    } while (0);
void multi_team_report () {
    ...
    for (i = 0; i < Num_teams && i < MAX_TVT_TEAMS; i++) {
        ...
        if (i < 2) {
            sprintf (
                report, XSTR ("<Team %d had %d points>", (1275 + i)),
                Multi_team_score[i]);
        }
        // Karajorma - If the SCP has added more teams we won't have a XSTR to
        // handle it. So just output in english for now
        else {
            sprintf (
                report, "Team %d had %d points>", (i + 1),
                Multi_team_score[i]);
        }
    ...
    case -1: SEND_AND_DISPLAY (XSTR ("<Match was a tie>", 1277)); break;
    ...
    case MT_CODE_TEAM_REQUEST:
        ...
        if ((req_index == -1) || (player_index == -1)) {
            ...
            multi_team_process_team_change_request (
                &Net_players[req_index], &Net_players[player_index], req_team);
        }
        break;
...
void multi_team_send_team_request (net_player* pl, int team) {
    ...
    // send to the server of the game (will be routed to host if in a
    // standalone situation)
    multi_io_send_reliable (Net_player, data, packet_size);
}

// send a full update on a player-per-player basis (should call this to update
// all players after other relevant function calls)
void multi_team_send_update () {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            ...
            if (Net_players[idx].p_info.team == 1) { val |= (1 << 0); }
    ...
    // if i'm the server, I should broadcast to this to all players, otherwise
    // I should send it to the standalone
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
