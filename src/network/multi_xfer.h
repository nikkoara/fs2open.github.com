...
#define MULTI_XFER_UNKNOWN \
    2 // the xfer has finished but its unknown if it was successful - wait a
      // while longer
#define MULTI_XFER_TIMEDOUT \
    3 // the xfer has timed-out during some stage of the process
...
#define MULTI_XFER_FLAG_AUTODESTROY \
    (1 << 15) // automatically clear and free an xfer handle that is done
#define MULTI_XFER_FLAG_REJECT \
    (1 << 16) // set by the receive callback function if we want to disallow
              // xfer of this file
// if this flag is set, the system will only xfer one file at a time to a given
// destination. so, suppose you start sending 3 files to one target, all which
// have this flag set. Only the first file will send. Once it is complete, the
// second one will go. Then the third. This is extremely useful for files where
// you don't _really_ care if it arrives or not (eg - sending multiple pilot
// pics or sounds or squad logos, etc). If you _do_ care about the file (eg -
// mission files), you probably shouldn't be using this flag
...
int multi_xfer_send_file (
    PSNET_SOCKET_RELIABLE who, char* filename, int cfile_flags, int flags = 0);
