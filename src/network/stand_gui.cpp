...
#define STD_STATS_UPDATE_TIME \
    500 // ms between updating player stats on the visible controls
#define STD_NG_UPDATE_TIME \
    100 // ms between updating netgame information are controls
...
// timestamp for updating currently selected player stats on the player info
// page
...
BOOL CALLBACK std_gen_dialog_proc (
    HWND /*hwndDlg*/, UINT uMsg, WPARAM /*wParam*/, LPARAM /*lParam*/) {
    switch (uMsg) {
    case WM_INITDIALOG: return TRUE;

    // destory command
    case WM_DESTROY: Multi_gen_dialog = NULL; break;
    }
    return FALSE;
}
...
void std_create_gen_dialog (const char* title) {
    ...
    if (Multi_gen_dialog != NULL) { return; }

    // if we are minimized then don't bother
    if (Standalone_minimized) { return; }

    // otherwise create the dialog
    Multi_gen_dialog = CreateDialog (
        GetModuleHandle (NULL), MAKEINTRESOURCE (IDD_GEN), Psht,
        (DLGPROC)std_gen_dialog_proc);
    if (Multi_gen_dialog != NULL) { SetWindowText (Multi_gen_dialog, title); }
}
...
void std_gen_set_text (const char* str, int field_num) {
    ...
    if ((Multi_gen_dialog == NULL) || (str == NULL) || (field_num < 0) ||
        (field_num > 2)) {
        return;
    }
    ...
    ctrl = GetDlgItem (
        Multi_gen_dialog, (int)(uintptr_t)MAKEINTRESOURCE (IDC_FIELD1));
    switch (field_num) {
    case 0:
        ctrl = GetDlgItem (
            Multi_gen_dialog, (int)(uintptr_t)MAKEINTRESOURCE (IDC_FIELD1));
    ...
    case 1:
        ctrl = GetDlgItem (
            Multi_gen_dialog, (int)(uintptr_t)MAKEINTRESOURCE (IDC_FIELD2));
        break;
    case 2:
        ctrl = GetDlgItem (
            Multi_gen_dialog, (int)(uintptr_t)MAKEINTRESOURCE (IDC_FIELD3));
        break;
    }
...
int std_gen_is_active () { return Multi_gen_dialog != NULL; }
...
// convert the index of an item in the list box into an index into the net
// players array
int std_connect_lindex_to_npindex (int index);

// set the text box indicating how many players are connected, returning the
// determined count
int std_connect_set_connect_count () {
    ...
    // set the text itself
    ctrl = GetDlgItem (
        Page_handles[CONNECT_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_CON_COUNT));
    SetWindowText (ctrl, str);
...
void std_connect_set_host_connect_status () {
    ...
    ctrl = GetDlgItem (
        Page_handles[CONNECT_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_HOST_IS));
    if (found) { SetWindowText (ctrl, XSTR ("Host connected ? Yes", 912)); }
    else {
...
void std_connect_add_ip_string (char* string) {
    ...
    ctrl = GetDlgItem (
        Page_handles[CONNECT_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_CONPING));
...
void std_connect_remove_ip_string (char* string) {
    ...
    // since we've appended the ping to the end of the string, we can only
    // check the "prefix" which is the net_players name
    auto loc = SendMessage (
        ctrl, LB_FINDSTRING, (WPARAM)-1, (LPARAM) (LPCTSTR)string);
...
void std_connect_set_ip_string (char* lookup, char* string) {
    ...
    // check the "prefix" which is the net_players name
    auto loc = SendMessage (
        ctrl, LB_FINDSTRING, (WPARAM)-1, (LPARAM) (LPCTSTR)lookup);
...
void std_connect_kick_player () {
    ...
    if (sel != CB_ERR) {
        ...
        if (player_num != -1) { multi_kick_player (player_num, 0); }
    }
}
...
void std_connect_handle_name_change () {
    ...
    if (Multi_std_namechange_force) {
        ...
        // get the new text
        SendMessage (
            Multi_std_name, EM_GETLINE, (WPARAM)0, (LPARAM) (LPCSTR)buf);
        ...
        strncpy (
            Multi_options_g.std_pname, buf,
            sizeof (Multi_options_g.std_pname));
...
void std_connect_handle_passwd_change () {
    ...
    // get the new text
    SendMessage (
        Multi_std_host_passwd, EM_GETLINE, (WPARAM)0, (LPARAM) (LPCSTR)buf);

    // just copy it over for now. we may want to process this more later on
    strncpy (
        Multi_options_g.std_passwd, buf, sizeof (Multi_options_g.std_passwd));
}
...
int std_connect_lindex_to_npindex (int index) {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        ...
        if (MULTI_CONNECTED (Net_players[idx])) {
            ...
            if ((addr_text[0] != '\0') &&
                (strstr (list_text, addr_text) != NULL)) {
                ret = idx;
                break;
            }
        }
...
BOOL CALLBACK
connect_proc (HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
    ...
    case WM_INITDIALOG:
        ...
        Multi_std_name = GetDlgItem (
            hwndDlg, (int)(uintptr_t)MAKEINTRESOURCE (IDC_STD_NAME));
        SendMessage (
            Multi_std_name, EM_SETLIMITTEXT, (WPARAM)MAX_GAMENAME_LEN,
            (LPARAM)0);
        Multi_std_namechange_force = 1;

        // create the standalone host password input box
        Multi_std_host_passwd = GetDlgItem (
            hwndDlg, (int)(uintptr_t)MAKEINTRESOURCE (IDC_STD_HOST_PASSWD));
        SendMessage (
            Multi_std_host_passwd, EM_SETLIMITTEXT, (WPARAM)STD_PASSWD_LEN,
            (LPARAM)0);
    ...
    case WM_COMMAND:
        ...
        case BN_CLICKED:
            ...
            case IDC_KICK_BUTTON: std_connect_kick_player (); break;
    ...
    case WM_NOTIFY:
        // notification that this is the current selected page. set our own
        // internal data vars
...
static HWND Standalone_mission_name; // text control for showing the current
                                     // mission name
static HWND
    Standalone_missiontime; // text control for showing current mission time
...
void std_multi_add_goals () {
    ...
    for (idx = 0; idx < Num_goals; idx++) {
        ...
        // first select whether to insert under primary, secondary, or bonus
        // tree roots
        tree_insert.hParent =
            Goal_items[Mission_goals[idx].type & GOAL_TYPE_MASK];
    ...
    // check to see if there are any of the three types of mission goals. If
    // not, then insert "none"
...
void std_multi_update_goals () {
    ...
    for (idx = 0; idx < Num_goals; idx++) {
        ...
        update_item = NULL;
        update_item = std_multi_get_goal_item (
            Mission_goals[idx].name, Mission_goals[idx].type & GOAL_TYPE_MASK);

        // continue if we didn't get a valid item
        if (update_item == NULL) { continue; }

        // get the tree item itself (as it currently stands)
        lookup.mask = TVIF_IMAGE | TVIF_SELECTEDIMAGE;
        lookup.hItem = update_item;
        if (!TreeView_GetItem (Standalone_goals, &lookup)) { continue; }

        should_update = 0;
        // determine what image to set for each one (failed, satisfied,
        // incomplete, etc)
...
// update the netgame information area controls with the current Netgame
// settings
...
void std_multi_handle_framecap_scroll (HWND ctrl) {
    ...
    SetWindowText (
        GetDlgItem (
            Page_handles[MULTIPLAYER_PAGE],
            (int)(uintptr_t)MAKEINTRESOURCE (IDC_FRAMECAP_STATIC)),
        pos_text);
...
void std_multi_init_framecap_slider (HWND hwndDlg) {
    ...
    Framecap_trackbar = CreateWindowEx (
        0, TRACKBAR_CLASS, NULL, WS_CHILD | WS_VISIBLE, 10, 10, 300, 30,
...
void std_multi_init_multi_controls (HWND hwndDlg) {
    ...
    Standalone_FPS = GetDlgItem (
        hwndDlg, (int)(uintptr_t)MAKEINTRESOURCE (IDC_STANDALONE_FPS));

    // create the missiontime text box
    Standalone_missiontime = GetDlgItem (
        hwndDlg, (int)(uintptr_t)MAKEINTRESOURCE (IDC_STANDALONE_MTIME));

    // create the mission name text box
    Standalone_mission_name = GetDlgItem (
        hwndDlg, (int)(uintptr_t)MAKEINTRESOURCE (IDC_MISSION_NAME));

    // create the netgame max players text box
    Std_ng_max_players = GetDlgItem (
        hwndDlg, (int)(uintptr_t)MAKEINTRESOURCE (IDC_NG_MAXPLAYERS));

    // create the netgame max observers text box
    Std_ng_max_observers = GetDlgItem (
        hwndDlg, (int)(uintptr_t)MAKEINTRESOURCE (IDC_NG_MAXOBSERVERS));

    // create the netgame security text box
    Std_ng_security = GetDlgItem (
        hwndDlg, (int)(uintptr_t)MAKEINTRESOURCE (IDC_NG_SECURITY));

    // create the netgame respawns # text box
    Std_ng_respawns = GetDlgItem (
        hwndDlg, (int)(uintptr_t)MAKEINTRESOURCE (IDC_NG_RESPAWNS));
    ...
    Standalone_goals = CreateWindowEx (
        0, WC_TREEVIEW, XSTR ("Tree View", 921),
        WS_VISIBLE | WS_CHILD | WS_BORDER | TVS_HASLINES, GOALVIEW_X,
        GOALVIEW_Y, GOALVIEW_W, GOALVIEW_H, hwndDlg, NULL,
...
HTREEITEM std_multi_get_goal_item (char* goal_string, int type) {
    ...
    while (!done && moveup != NULL) {
        ...
        if (!done) {
            moveup =
                TreeView_GetNextItem (Standalone_goals, moveup, TVGN_NEXT);
        }
    }
    return ret;
}
...
BOOL CALLBACK
multi_proc (HWND hwndDlg, UINT uMsg, WPARAM /*wParam*/, LPARAM lParam) {
    switch (uMsg) {
    // initialize the page
    case WM_INITDIALOG:
        // set the page handle
...
static HWND
    Player_name_list; // the listbox control with player callsigns in it
static HWND Player_ship_type; // the current player's ship type
static HWND Player_ping_time; // the current player's ping time
static HWND Player_stats[MAX_PLAYER_STAT_FIELDS];  // text boxes for player
                                                   // alltime statistics info
static HWND Player_mstats[MAX_PLAYER_STAT_FIELDS]; // text boxes for player
                                                   // mission statistics info

// sprintf and set window text to the passed int
#define STD_ADDSTRING(hwnd, val)                          \
    {                                                     \
        snprintf (txt, sizeof (txt) - 1, "%d", (int)val); \
...
// returns true or false depending on whether the passed netplayer is the
...
// check to see if this player is the one being displayed, and if so, then
// update the display info return 1 if the player was updated
...
void std_pinfo_add_player_list_item (net_player* p) {
    // add the item
    SendMessage (
        Player_name_list, CB_ADDSTRING, (WPARAM)0,
        (LPARAM) (LPCTSTR)p->m_player->callsign);

    // if this is the first item on the list, then select it and display it
    if (SendMessage (Player_name_list, CB_GETCOUNT, (WPARAM)0, (LPARAM)0) ==
        1) {
        // select the item
...
void std_pinfo_remove_player_list_item (net_player* p) {
    // lookup thie player
    auto loc = SendMessage (
        Player_name_list, CB_FINDSTRINGEXACT, (WPARAM)-1,
        (LPARAM) (LPCTSTR)p->m_player->callsign);

    // if we found the entry, then delete it
    if (loc != CB_ERR) {
        SendMessage (
            Player_name_list, CB_DELETESTRING, (WPARAM)loc, (LPARAM)0);
    }
}
...
void std_pinfo_init_player_info_controls () {
    ...
    Player_name_list = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_PLAYER_LIST));

    // create the player ship type text box
    Player_ship_type = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_PSHIP_TYPE));

    // create the player ping time text box
    Player_ping_time = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_PING_TIME));

    // initialize the various and sundry statistics text controls (alltime)
    Player_stats[0] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_SCORE));
    Player_stats[1] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_KILL_COUNT));
    Player_stats[2] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_KILL_COUNT_BH));
    Player_stats[3] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_ASSISTS));
    Player_stats[4] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_PSHOTS));
    Player_stats[5] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_PHITS));
    Player_stats[6] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_PBHHITS));
    Player_stats[7] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_PPCT));
    Player_stats[8] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_PBHPCT));
    Player_stats[9] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_SSHOTS));
    Player_stats[10] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_SECHITS));
    Player_stats[11] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_SBHHITS));
    Player_stats[12] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_SPCT));
    Player_stats[13] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_SBHPCT));

    // initialize the various and sundry statistics text controls (this
    // mission)
    Player_mstats[0] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_MSCORE));
    Player_mstats[1] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_MKILL_COUNT));
    Player_mstats[2] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_MKILL_COUNT_BH));
    Player_mstats[3] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_MASSISTS));
    Player_mstats[4] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_MPSHOTS));
    Player_mstats[5] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_MPHITS));
    Player_mstats[6] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_MPBHHITS));
    Player_mstats[7] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_MPPCT));
    Player_mstats[8] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_MPBHPCT));
    Player_mstats[9] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_MSSHOTS));
    Player_mstats[10] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_MSECHITS));
    Player_mstats[11] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_MSBHHITS));
    Player_mstats[12] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_MSPCT));
    Player_mstats[13] = GetDlgItem (
        Page_handles[PLAYER_INFO_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_MSBHPCT));
}
...
int std_pinfo_player_is_active (net_player* p) {
    ...
    auto sel =
        SendMessage (Player_name_list, CB_GETCURSEL, (WPARAM)0, (LPARAM)0);
    ...
    return ((player[0] != '\0') &&
            (strcmp (p->m_player->callsign, player) == 0))
               ? 1
               : 0;
}
...
BOOL CALLBACK
player_info_proc (HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    int player_num;
    char callsign[40];
    ...
    case WM_COMMAND:
        ...
        case CBN_SELCHANGE: {
            ...
            auto val = SendMessage (
                Player_name_list, CB_GETCURSEL, (WPARAM)0, (LPARAM)0);
            if (val != CB_ERR) {
                // get the callsign
                if (SendMessage (
                        Player_name_list, CB_GETLBTEXT, (WPARAM)val,
...
#define GODSTUFF_MAX_ITEMS \
    19 // how many items we can fit on the chatbox at one time

static HWND God_player_list;         // the listbox of player callsigns
static HWND Godstuff_fps;            // the framerate text box
static HWND Godstuff_broadcast_text; // the text input box for sending messages
                                     // to players
static HWND Godstuff_broadcast_button; // the button to send the text messages
static HWND Godstuff_player_messages;  // handle to the list box containing
                                       // player chatter
static HDC Godstuff_player_messages_HDC =
    0; // DC handle to the list box containing player chatter

static int Godstuff_longest_message =
    0; // longest width of a string in the godstuff list box
...
void std_gs_add_god_player (net_player* p) {
    // add the item
    SendMessage (
        God_player_list, CB_ADDSTRING, (WPARAM)0,
        (LPARAM) (LPCTSTR)p->m_player->callsign);

    // if this is the first item on the list, then select it
    if (SendMessage (God_player_list, CB_GETCOUNT, (WPARAM)0, (LPARAM)0) ==
        1) {
        // select the item
...
void std_gs_remove_god_player (net_player* p) {
    // lookup the player
    auto loc = SendMessage (
        God_player_list, CB_FINDSTRINGEXACT, (WPARAM)-1,
        (LPARAM) (LPCTSTR)p->m_player->callsign);
...
void std_gs_send_godstuff_message () {
    ...
    txt[0] = 120;
    SendMessage (
        Godstuff_broadcast_text, EM_GETLINE, (WPARAM)0, (LPARAM)&txt[0]);
...
void std_gs_clear_controls () {
    ...
    SendMessage (
        Godstuff_player_messages, LB_RESETCONTENT, (WPARAM)0, (LPARAM)0);
}

// initialize all the controls in the godstuff tab
void std_gs_init_godstuff_controls () {
    // initialize the player listbox control
    God_player_list = GetDlgItem (
        Page_handles[GODSTUFF_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_PLAYER_GOD_LIST));

    // initialize the framerate text box
    Godstuff_fps = GetDlgItem (
        Page_handles[GODSTUFF_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_GODSTUFF_FPS));

    // initialize the messaging edit control
    Godstuff_broadcast_text = GetDlgItem (
        Page_handles[GODSTUFF_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_GODSTUFF_BROADCAST));
    SendMessage (
        Godstuff_broadcast_text, EM_SETLIMITTEXT, (WPARAM)CHATBOX_MAX_LEN,
        (LPARAM)0);
    SendMessage (
        Godstuff_broadcast_text, EM_FMTLINES, (WPARAM)TRUE, (LPARAM)0);

    // create the player chatter list box
    Godstuff_player_messages = GetDlgItem (
        Page_handles[GODSTUFF_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_GOD_CHAT));
    ...
    Godstuff_broadcast_button = GetDlgItem (
        Page_handles[GODSTUFF_PAGE],
        (int)(uintptr_t)MAKEINTRESOURCE (IDC_GODSTUFF_SENDMESS));
...
BOOL CALLBACK
godstuff_proc (HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
    // initialize the dialog
    case WM_INITDIALOG: {
    ...
    case WM_COMMAND: {
        ...
        case BN_CLICKED:
            ...
            case IDC_GODSTUFF_SENDMESS: std_gs_send_godstuff_message (); break;
            }
...
void std_debug_multilog_add_line (const char* str) {
    ...
    if (Standalone_multilog_string == NULL) { return; }
    ...
    while (nline != std::string::npos) {
        ...
        SendMessage (
            Standalone_multilog_string, LB_ADDSTRING, 0,
            (LPARAM)log_str.c_str ());

        // reset our width, using best guess, so that we have a working hscroll
        uint h_size = (uint)SendMessage (
            Standalone_multilog_string, LB_GETHORIZONTALEXTENT, 0, 0);

        if ((h_size / 6) < log_str.size ()) {
            SendMessage (
                Standalone_multilog_string, LB_SETHORIZONTALEXTENT,
                log_str.size () * 6, 0);
        }

        // remove excess lines, if needed
        auto l_count =
            SendMessage (Standalone_multilog_string, LB_GETCOUNT, 0, 0);
...
void std_debug_init_debug_controls (HWND hwndDlg) {
    ...
    Standalone_state_string = GetDlgItem (
        hwndDlg, (int)(uintptr_t)MAKEINTRESOURCE (IDC_STANDALONE_STATE));

    // standalone state indicator
    SetWindowText (Standalone_state_string, "");

    // do the multi-log string too
    Standalone_multilog_string =
        GetDlgItem (hwndDlg, (int)(uintptr_t)MAKEINTRESOURCE (IDC_MULTILOG));
    SendMessage (Standalone_multilog_string, LB_RESETCONTENT, 0, 0);
}

// message handler for the godstuff tab
BOOL CALLBACK
debug_proc (HWND hwndDlg, UINT uMsg, WPARAM /*wParam*/, LPARAM lParam) {
    switch (uMsg) {
    // initialize the dialog
    case WM_INITDIALOG:
        // setup the page handle
...
// do any gui related issues on the standalone (like periodically updating
// player stats, etc...) notify the user that the standalone has failed to
// login to the tracker on startup
void std_notify_tracker_login_fail () {}

void std_do_gui_frame () {
    int idx;

    // check to see if the timestamp for updating player selected stats has
    // popped
    if ((Standalone_stats_stamp == -1) ||
        timestamp_elapsed (Standalone_stats_stamp)) {
        // reset the timestamp
    ...
    // check to see if the timestamp for updating the netgame information
    // controls has popped
    if ((Standalone_ng_stamp == -1) ||
        timestamp_elapsed (Standalone_ng_stamp)) {
        // reset the timestamp
...
void std_add_chat_text (const char* text, int player_index, int add_id) {
    ...
    if (add_id) {
        if (MULTI_STANDALONE (Net_players[player_index])) {
            ...
            sprintf (
                format, "%s: %s", Net_players[player_index].m_player->callsign,
                text);
        }
    }
    else {
    ...
    // this thing isn't all that accurate, it typically produces a longer line,
    // but I don't really care :p
    if (Godstuff_player_messages_HDC)
        GetTextExtentPoint32 (
            Godstuff_player_messages_HDC, format, (int)strlen (format),
            &text_size);
    ...
    SendMessage (
        Godstuff_player_messages, LB_SETHORIZONTALEXTENT,
        (WPARAM)Godstuff_longest_message, (LPARAM)0);
    ...
    // insert the text string into the godstuff chat box and scroll it down to
    // the bottom
    SendMessage (
        Godstuff_player_messages, LB_INSERTSTRING, (WPARAM)-1, (LPARAM)format);

    auto num_items = SendMessage (
        Godstuff_player_messages, LB_GETCOUNT, (WPARAM)0, (LPARAM)0);

    if (num_items > 19) {
        SendMessage (
            Godstuff_player_messages, LB_SETTOPINDEX,
            (WPARAM)num_items - GODSTUFF_MAX_ITEMS, (LPARAM)0);
    }
}
...
void std_mutate_sheet () {
    ...
    while (child != NULL) {
        ...
        if (!stricmp (lookup, XSTR ("ok", 925))) { ok_button = child; }

        // if its the cancel button
        if (!stricmp (lookup, XSTR ("cancel", 926))) { cancel_button = child; }

        // if its the apply button
        if (!stricmp (lookup, XSTR ("&apply", 927))) { apply_button = child; }

        // if its the help button
        if (!stricmp (lookup, XSTR ("help", 928))) { help_button = child; }

        child = GetWindow (child, GW_HWNDNEXT);
    }

    // kill em all
    if (apply_button != NULL) { DestroyWindow (apply_button); }
    ...
    if (ok_button != NULL) {
        ...
        // move it
        SetWindowPos (
            ok_button, HWND_TOP, Std_shutdown_coords[gr_screen.res][0],
            Std_shutdown_coords[gr_screen.res][1], 0, 0,
            SWP_SHOWWINDOW | SWP_NOSIZE | SWP_NOZORDER);
    }

    // kill em all
    if (cancel_button != NULL) { DestroyWindow (cancel_button); }

    // kill em all
    if (help_button != NULL) { DestroyWindow (help_button); }
...
int std_player_is_banned (const char* name) {
    ...
    for (idx = 0; idx < Standalone_ban_count; idx++) {
        if (!stricmp (name, Standalone_ban_list[idx])) { return 1; }
    }
...
void std_add_ban (const char* name) {
    ...
    if (Standalone_ban_count >= STANDALONE_MAX_BAN) { return; }
...
void std_build_title_string (char* str) {
    ...
    if (FS_VERSION_BUILD == 0 && FS_VERSION_HAS_REVISION == 0) { //-V547
        snprintf (
            ver_str, sizeof (ver_str) - 1, "%i.%i", FS_VERSION_MAJOR,
            FS_VERSION_MINOR);
    }
    else if (FS_VERSION_HAS_REVISION == 0) {
        snprintf (
            ver_str, sizeof (ver_str) - 1, "%i.%i.%i", FS_VERSION_MAJOR,
            FS_VERSION_MINOR, FS_VERSION_BUILD);
    }
    else {
        snprintf (
            ver_str, sizeof (ver_str) - 1, "%i.%i.%i.%i", FS_VERSION_MAJOR,
            FS_VERSION_MINOR, FS_VERSION_BUILD, FS_VERSION_REVISION);
    }
    ...
    snprintf (
        temp, sizeof (temp) - 1, "%s %s", XSTR ("FreeSpace Standalone", 935),
        ver_str);
...
static HMENU std_create_systray_menu () {
    ...
    // Type of connection:
    snprintf (
        tstr, sizeof (tstr) - 1, "Connection Type: %s",
        MULTI_IS_TRACKER_GAME ? "FS2NetD" : "Local/IP");
    AppendMenu (stdPopup, MF_STRING | MF_GRAYED, 0, tstr);

    // Status of FS2NetD connection:
    if (MULTI_IS_TRACKER_GAME) {
        snprintf (
            tstr, sizeof (tstr) - 1, "FS2NetD Status: %s",
            fs2netd_is_online () ? "Online" : "Offline");
        AppendMenu (stdPopup, MF_STRING | MF_GRAYED, 0, tstr);
    }
    ...
    // Mission name:
    snprintf (
        tstr, sizeof (tstr) - 1, "Mission: %s",
        strlen (Netgame.mission_name) ? Netgame.mission_name : "<none>");
    AppendMenu (stdPopup, MF_STRING | MF_GRAYED, 0, tstr);

    // Number of players:
    snprintf (
        tstr, sizeof (tstr) - 1, "Num Players: %d", multi_num_players ());
    AppendMenu (stdPopup, MF_STRING | MF_GRAYED, 0, tstr);
...
LRESULT CALLBACK std_message_handler_proc (
    HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
    case WM_SYSCOMMAND: {
    ...
    case MSG_SYSTRAYICON: {
        if (lParam == WM_LBUTTONDBLCLK) {
            ...
            int choice = TrackPopupMenuEx (
                stdPopup, TPM_RETURNCMD | TPM_NONOTIFY, cur_pos.x, cur_pos.y,
            ...
            if (choice == STP_RESET_ALL) { multi_quit_game (PROMPT_NONE); }
            // reset fs2netd
    ...
    case WM_CLOSE: PostMessage (Psht, WM_DESTROY, 0, 0); return TRUE;

    default: return DefWindowProc (hwndDlg, uMsg, wParam, lParam);
    }
...
BOOL std_create_standalone_window () {
    // create a hidden window that will process basic events for us (for the
    // systray icon mainly)
    ...
    Standalone_hwnd = CreateWindowEx (
        0, "FS2StandaloneClass", "FreeSpace2 Standalone", WS_POPUP,
        CW_USEDEFAULT, CW_USEDEFAULT, 10, 10, NULL, NULL, hInst, NULL);

    // create the propery sheet dialog itself
    std_init_property_sheet (NULL);

    // this is kind of a big-ass hack. But here's what it does. Property sheets
    // only initialize their individual pages the first time (and ONLY the
    // first time) they are selected. So before any of this happens, their
    // control handles are bogus. So, by calling PropSheet_SetCurSel for all
    // the pages, WM_INITDIALOG is sent to all of them, and their controls
    // become valid. However, its kind of silly because each page will blink
    // into existence for a second. I can't see another way arount this
    // problem.
    int idx;
...
DWORD standalone_process (WORD /*lparam*/) {
    ...
    if (!std_create_standalone_window ()) return 0;
        ...
        if (WaitMessage ()) {
            while (PeekMessage (&msg, NULL, 0, 0, PM_REMOVE)) {
                ...
                if (msg.message == WM_DESTROY) {
                    standalone_do_systray (
                        ST_MODE_REMOVE); // shouldn't be needed, but make sure
                                         // it's gone
                    DestroyWindow (Psht);
                ...
                if ((msg.message == WM_NCLBUTTONDOWN) &&
                    (msg.wParam == HTMINBUTTON)) {
                    standalone_do_systray (ST_MODE_CREATE);
                    continue;
                }

                // see if the message is destined for the edit control, and
                // what the message is. intercept if a return
...
void std_init_standalone () {
    ...
    Standalone_thread = CreateThread (
        NULL, 0, (LPTHREAD_START_ROUTINE)standalone_process, NULL, 0,
