        ...
        float damage, bool *hull_should_apply_armor)
{
        ...
        if ((weapon_info_index >= 0) && ((other_obj->type == OBJ_WEAPON) || (Beams_use_damage_factors && (other_obj->type == OBJ_BEAM)))) {
        ...
        if (other_obj) {
                if (other_obj->type == OBJ_SHOCKWAVE) {
                        ...
                        dmg_type_idx = Weapon_info[Weapons[other_obj->instance].weapon_info_index]
                                               .damage_type_idx;
                        ...
                        dmg_type_idx = Asteroid_info[Asteroids[other_obj->instance].asteroid_type]
                                               .damage_type_idx;
        ...
        for (j = 0; j < count; j++) {
                ...
                if (damage_to_apply > 0.1f) {
                        ...
                        if (subsystem->armor_type_idx >= 0) {
                                ...
                                damage_to_apply = Armor_types[subsystem->armor_type_idx].GetDamage(
                        ...
                        if (subsystem->current_hits < 0.0f) {
                                ...
                                subsystem->current_hits = 0.0f; // set to 0 so repair on subsystem takes immediate
                                                                // effect
                        }
...
static void shiphit_record_player_killer(object *killer_objp, player *p)
{
        ...
        case OBJ_WEAPON:
                ...
                p->killer_species = Ship_info[Ships[Objects[killer_objp->parent].instance]
                                                      .ship_info_index]
                                            .species;
        ...
        case OBJ_BEAM:
                ...
                if (beam_obj != -1) {
                        ...
                        p->killer_species = Ship_info[Ships[Objects[beam_obj].instance].ship_info_index]
                                                    .species;
                } else {
...
static void player_died_start(object *killer_objp)
{
    ...
    */
        ...
        Player_ai->target_objnum = -1; // Clear targeting.  Otherwise, camera pulls away from player as
                // soon as he blows up.
...
void ship_generic_kill_stuff(object *objp, float percent_killed)
{
        ...
        if (object_is_dead_docked(objp)) {
                ...
                if (fabsf(sp->deathroll_rotvel.xyz.z) < 2.0f * largest_mag) {
                        sp->deathroll_rotvel.xyz.z *= (2.0f * largest_mag / fabsf(sp->deathroll_rotvel.xyz.z));
                }
                saturate_fabs(
        ...
        int self_destruct)
{
        ...
        if (killer_index >= 0) {
                ...
                if (objp != END_OF_LIST(&obj_used_list)) {
                        ASSERT(
                                (objp->type == OBJ_SHIP) || (objp->type == OBJ_GHOST)); // I suppose that this should be true
        ...
        if ((killer_damage_percent > 10) && (other_obj != NULL)) {
                if (other_obj->parent_sig == Player_obj->signature) {
                        ...
                        (other_obj->parent_type == OBJ_SHIP) || (other_obj->parent_type == OBJ_START)) {
        ...
        int quadrant, int submodel_num, int wash_damage = 0)
{
        ...
        if (other_obj_is_weapon) {
                ...
                if (ship_objp->flags[Object::Object_Flags::Player_ship]) {
                        ...
                        // where needed
                        difficulty_scale_factor *= The_mission.ai_profile
        ...
        if (other_obj && !(Ship_info[Ships[Player_obj->instance].ship_info_index].flags[Ship::Info_Flags::No_pain_flash])) {
                ...
                if (other_obj->type == OBJ_BEAM) {
                        ASSERT(
                                (beam_get_weapon_info_index(other_obj) >= 0) && (beam_get_weapon_info_index(other_obj) < Num_weapon_types));
                ...
                if (other_obj_is_weapon) {
                        ASSERT(
                                (Weapons[other_obj->instance].weapon_info_index > -1) && (Weapons[other_obj->instance].weapon_info_index < Num_weapon_types));
        ...
        if (quadrant >= 0 && !(ship_objp->flags[Object::Object_Flags::No_shields])) {
                ...
                if (damage > 0.0f) {
                        ...
                        float pre_shield = damage;              // Nuke: don't use the difficulty scaling in here,
        ...
        if ((damage > 0.0f) || (subsystem_damage > 0.0f)) {
                int weapon_info_index;
                float pre_subsys = subsystem_damage; // Nuke: should be the last time we need to do
        ...
        int quadrant, bool create_spark, int submodel_num, vec3d *hit_normal)
{
        ...
        if (((other_obj->type == OBJ_SHIP) || (other_obj->type == OBJ_WEAPON) || (other_obj->type == OBJ_BEAM))) {
