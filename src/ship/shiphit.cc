...
static bool is_subsys_destroyed (ship* shipp, int submodel) {
    ...
    if (submodel == -1) { return false; }

    for (subsys = GET_FIRST (&shipp->subsys_list);
         subsys != END_OF_LIST (&shipp->subsys_list);
         subsys = GET_NEXT (subsys)) {
        if (subsys->system_info->subobj_num == submodel) {
            if (subsys->current_hits > 0.0f) { return false; }
            else {
                return true;
            }
        }
...
// do_subobj_destroyed_stuff is called when a subobject for a ship is killed.
// Separated out to separate function on 10/15/97 by MWA for easy multiplayer
// access.  It does all of the cool things like blowing off the model (if
// applicable, writing the logs, etc)
void do_subobj_destroyed_stuff (
    ship* ship_p, ship_subsys* subsys, vec3d* hitpos, bool no_explosion) {
    ship_info* sip;
    object* ship_objp;
    ...
    if (!(subsys->flags[Ship::Subsystem_Flags::Vanished]) && !no_explosion) {
        if (objp->radius > 100.0f) {
            ...
            int num_fireballs;
            if (psub->radius < 3) { num_fireballs = 1; }
            else {
                num_fireballs = 5;
            }
            ...
            for (i = 0; i < num_fireballs; i++) {
                if (i == 0) {
                    ...
                    if (hitpos) { temp_vec = *hitpos; }
                    else {
                        temp_vec = g_subobj_pos;
                    }
                }
                else {
                    // make other fireballs at random positions, but try to
                    // keep on the surface
                    vm_vec_rand_vec_quick (&rand_vec);
                    float dot = vm_vec_dot (&center_to_subsys, &rand_vec);
                    vm_vec_scale_add2 (
                        &rand_vec, &center_to_subsys,
                        -dot / vm_vec_mag_squared (&center_to_subsys));
                    vm_vec_scale_add (
                        &temp_vec, &g_subobj_pos, &rand_vec,
                        0.5f * psub->radius);
                }

                // scale fireball size according to size of subsystem, but not
                // less than 10
                float fireball_rad = psub->radius * 0.2f;
                if (fireball_rad < 10) { fireball_rad = 10.0f; }

                vec3d fb_vel;
                vm_vec_cross (
                    &fb_vel, &objp->phys_info.rotvel, &center_to_subsys);
                ...
                fireball_create (
                    &temp_vec, fireball_type, FIREBALL_MEDIUM_EXPLOSION,
    ...
    // the generic subsystems I think that there might be rounding problems
    // with the floats.  This code keeps us safe.
    ...
    //
    // MWA -- 1/8/98  A problem was found when trying to determine (via
    // sexpression) when some subsystems were destroyed.  The bottom line is
    // that is the psub->name and psub->subobj_name are different, then direct
    // detection doesn't work.  (This scenario happens mainly with turrets and
    // probably with engines).  So, my solution is to encode the ship_info
    // index, and the subsystem index into one integer, and pass that as the
    // "index" parameter to add_entry.  We'll use that information to print out
    // the info in the mission log.
    ASSERT (ship_p->ship_info_index < 65535);

    // get the "index" of this subsystem in the ship info structure.
    for (i = 0; i < sip->n_subsystems; i++) {
        if (&(sip->subsystems[i]) == psub) break;
    }
    ...
    // FUBAR/Goober5000 - or about vanishing subsystems, per precedent with
    // ship-vanish
    int notify = (psub->type != SUBSYSTEM_ACTIVATION) &&
                 !(subsys->flags[Ship::Subsystem_Flags::Vanished]);

    if (notify) {
        mission_log_add_entry (
            LOG_SHIP_SUBSYS_DESTROYED, ship_p->ship_name, psub->subobj_name,
            log_index);
        if (ship_objp == Player_obj) {
            if (!no_explosion) {
                snd_play (
                    gamesnd_get_game_sound (GameSounds::SUBSYS_DIE_1), 0.0f);
            }
            if (strlen (psub->alt_dmg_sub_name))
                HUD_printf (
                    XSTR ("Your %s subsystem has been destroyed", 499),
    ...
    if (psub->type == SUBSYSTEM_TURRET) {
        if (ship_p->subsys_info[type].aggregate_current_hits <= 0.0f) {
            ...
            if (!(Ship_info[ship_p->ship_info_index].is_small_ship ())) {
                mission_log_add_entry (
                    LOG_SHIP_DISARMED, ship_p->ship_name, NULL);
                // ship_p->flags |= SF_DISARMED;
            }
        }
    }
    else if (psub->type == SUBSYSTEM_ENGINE) {
        // when an engine is destroyed, we must change the max velocity of the
        // ship to be some fraction of its normal maximum value

        if (ship_p->subsys_info[type].aggregate_current_hits <= 0.0f) {
            mission_log_add_entry (LOG_SHIP_DISABLED, ship_p->ship_name, NULL);
            ship_p->flags.set (
                Ship::Ship_Flags::Disabled); // add the disabled flag
        }
    }

    if (psub->subobj_num > -1) {
        shipfx_blow_off_subsystem (
            ship_objp, ship_p, subsys, &g_subobj_pos, no_explosion);
    ...
    if (notify && !no_explosion) {
        ...
        if (sound_index.isValid ()) {
            snd_play_3d (
                gamesnd_get_game_sound (sound_index), &g_subobj_pos,
                &View_position);
        }
    }
    ...
    if (subsys->subsys_snd_flags[Ship::Subsys_Sound_Flags::Alive]) {
        obj_snd_delete_type (
            ship_p->objnum, subsys->system_info->alive_snd, subsys);
    ...
    if (subsys->subsys_snd_flags[Ship::Subsys_Sound_Flags::Turret_rotation]) {
        obj_snd_delete_type (
            ship_p->objnum, subsys->system_info->turret_base_rotation_snd,
            subsys);
        obj_snd_delete_type (
            ship_p->objnum, subsys->system_info->turret_gun_rotation_snd,
            subsys);
        subsys->subsys_snd_flags.remove (
            Ship::Subsys_Sound_Flags::Turret_rotation);
    }
    if (subsys->subsys_snd_flags[Ship::Subsys_Sound_Flags::Rotate]) {
        obj_snd_delete_type (
            ship_p->objnum, subsys->system_info->rotation_snd, subsys);
        subsys->subsys_snd_flags.remove (Ship::Subsys_Sound_Flags::Rotate);
    }
    if ((subsys->system_info->dead_snd.isValid ()) &&
        !(subsys->subsys_snd_flags[Ship::Subsys_Sound_Flags::Dead])) {
        obj_snd_assign (
            ship_p->objnum, subsys->system_info->dead_snd,
            &subsys->system_info->pnt, 0, OS_SUBSYS_DEAD, subsys);
...
static int shiphit_get_damage_weapon (object* damaging_objp) {
    ...
    if (damaging_objp) {
        ...
        case OBJ_WEAPON:
            weapon_info_index =
                Weapons[damaging_objp->instance].weapon_info_index;
            break;
        case OBJ_SHOCKWAVE:
            weapon_info_index =
                shockwave_get_weapon_index (damaging_objp->instance);
            break;
        case OBJ_BEAM:
            ...
            break;
        default: weapon_info_index = -1; break;
        }
    }
...
static float subsys_get_range (object* other_obj, ship_subsys* subsys) {
    ...
    if ((other_obj) &&
        (other_obj->type ==
         OBJ_SHOCKWAVE)) { // Goober5000 - check for NULL when via sexp
        range = shockwave_get_max_radius (other_obj->instance) *
...
// Make some random debris particles.  Previous way was not very random. Create
...
static void create_subsys_debris (object* ship_objp, vec3d* hitpos) {
    ...
    if (show_debris <= 0.75f) {
        ...
        if (ndebris > MAX_DEBRIS_SHARDS) ndebris = MAX_DEBRIS_SHARDS;
...
#define MAX_SUBSYS_LIST \
    200 // DTP MAX SUBSYS LIST BUMPED FROM 32 to 200, ahmm 32???
...
// do_subobj_hit_stuff() is called when a collision is detected between a ship
// and something else.  This is where we see if any sub-objects on the ship
// should take damage.
//
...
//
//
// The weapon damage is not neccesarily distributed evently between sub-systems
// when more than one sub-system is to take damage.  Whenever damage is to be
// assigned to a sub-system, the above percentages are used.  So, if more than
// one sub-object is taking damage, the second sub-system to be assigned damage
// will take less damage.  Eg. weapon hits in the 25% damage range of two
// subsytems, and the weapon damage is 12.  First subsystem takes 3 points
// damage.  Second subsystem will take 0.25*9 = 2.25 damage.  Should be close
...
float do_subobj_hit_stuff (
    object* ship_objp, object* other_obj, vec3d* hitpos, int submodel_num,
    float damage, bool* hull_should_apply_armor) {
    vec3d g_subobj_pos;
    ...
    int subsys_hit_first = -1; // the subsys which should be hit first and take
                               // most of the damage; index into subsys_list
    vec3d hitpos2;
    float ss_dif_scale =
        1.0f; // Nuke: Set a base dificulty scale for compatibility

    // WMC - first, set this to damage if it isn't NULL, in case we want to
    // return with no damage to subsystems
    if (hull_should_apply_armor != NULL) { *hull_should_apply_armor = true; }
    ...
    if (The_mission.game_type & MISSION_TYPE_TRAINING) {
        if (ship_objp == Player_obj) { return damage; }
    }

    // Shockwave damage is applied like weapon damage.  It gets consumed.
    if ((other_obj != NULL) &&
        (other_obj->type == OBJ_SHOCKWAVE)) // Goober5000 check for NULL
    {
    ...
    weapon_info_index = shiphit_get_damage_weapon (
        other_obj); // Goober5000 - a NULL other_obj returns -1
    ...
    if (hitpos_dist > ship_objp->radius * 2.0f) {
        WARNINGF (LOCATION,"BOGUS HITPOS PASSED TO DO_SUBOBJ_HIT_STUFF (%.1f > %.1f)!\nInvestigate ship %s (%s), a hit was registered on this ship outside this ship's radius.",hitpos_dist, ship_objp->radius * 2.0f, ship_p->ship_name,Ship_info[ship_p->ship_info_index].name);
        // Get John ASAP!!!!  Someone passed a local coordinate instead of
        // world for hitpos probably.
    }
#endif

    if (!global_damage) { create_subsys_debris (ship_objp, hitpos); }
    ...
    for (subsys = GET_FIRST (&ship_p->subsys_list);
         subsys != END_OF_LIST (&ship_p->subsys_list);
         subsys = GET_NEXT (subsys)) {
        model_subsystem* mss = subsys->system_info;

        // Deal with cheat correctly. If damage is the negative of the
        // subsystem type, then we'll just kill the subsystem See
        // process_debug_keys() in keycontrol.cpp for details.
        if (damage < 0.0f) {
            ...
            if ((subsys == Player_ai->targeted_subsys) &&
                ...
                if (!(subsys->flags[Ship::Subsystem_Flags::No_aggregate])) {
                    ship_p->subsys_info[mss->type].aggregate_current_hits -=
                        subsys->current_hits;
                    if (ship_p->subsys_info[mss->type].aggregate_current_hits <
                        0.0f) {
                        ship_p->subsys_info[mss->type].aggregate_current_hits =
                            0.0f;
                    }
        ...
        if (subsys->current_hits > 0.0f) {
            ...
            if (Fixed_turret_collisions && submodel_num != -1 &&
                submodel_num == mss->turret_gun_sobj) {
                // Special case:
                // if the subsystem is a turret and the hit submodel is its
                // barrel, get the distance between the hit and the turret
                // barrel center
                find_submodel_instance_world_point (
                    &g_subobj_pos, ship_p->model_instance_num, submodel_num,
                    &ship_objp->orient, &ship_objp->pos);
                dist = vm_vec_dist_quick (&hitpos2, &g_subobj_pos);

                // Damage attenuation range of barrel radius * 2 makes full
                // damage be taken regardless of where the barrel is hit
                range = submodel_get_radius (
                            Ship_info[ship_p->ship_info_index].model_num,
                            submodel_num) *
                        2;
            }
            else {
            ...
            if (dist < range) {
                if (Damage_impacted_subsystem_first && submodel_num != -1 &&
                    (submodel_num == mss->subobj_num ||
                     submodel_num == mss->turret_gun_sobj)) {
                    // If the hit impacted this subsystem's submodel, then make
                    // sure this subsys gets dealt damage first, even if
                    // another subsystem is closer to the hit location
                ...
                if (mss->flags[Model::Subsystem_Flags::Collide_submodel]) {
                    if (submodel_num != -1 &&
                        submodel_num != mss->subobj_num &&
                        submodel_num != mss->turret_gun_sobj) {
                        // If this subsystem only wants to take damage when its
                        // submodel receives a direct hit and the current hit
                        // did not do so, skip it.
                        continue;
                    }
                }
                ...
                if (count >= MAX_SUBSYS_LIST) { break; }
            }
        }
    }
    ...
    if (other_obj) {
        if (other_obj->type == OBJ_SHOCKWAVE) {
            ...
            dmg_type_idx = Weapon_info[beam_get_weapon_info_index (other_obj)]
                               .damage_type_idx;
        }
        else if (other_obj->type == OBJ_ASTEROID) {
            dmg_type_idx =
                Asteroid_info[Asteroids[other_obj->instance].asteroid_type]
                    .damage_type_idx;
        }
            ...
            dmg_type_idx =
                Ships[other_obj->instance].collision_damage_type_idx;
        }
    }
    ...
    for (j = 0; j < count; j++) {
        ...
        if ((j == 0) && (!(parent_armor_flags & SAF_IGNORE_SS_ARMOR))) {
            if (subsystem->armor_type_idx > -1) {
                damage = Armor_types[subsystem->armor_type_idx].GetDamage (
                    damage, dmg_type_idx,
                    1.0f); // Nuke: I don't think we need to apply damage
                           // sacaling to this one, using 1.0f
        ...
        if ((weapon_info_index >= 0) && (dist < 10.0f) &&
            ...
            damage_left *=
                4.0f * Weapon_info[weapon_info_index].subsystem_factor;
            damage_if_hull *=
                4.0f * Weapon_info[weapon_info_index].armor_factor;
        }
        ...
        if (damage_to_apply > 0.1f) {
            ...
            if (ship_objp->flags[Object::Object_Flags::Player_ship]) {
                ss_dif_scale = The_mission.ai_profile
                                   ->subsys_damage_scale[Game_skill_level];
            }

            // Goober5000 - subsys guardian
            if (subsystem->subsys_guardian_threshold > 0) {
                float min_subsys_strength =
                    0.01f * subsystem->subsys_guardian_threshold *
                    subsystem->max_hits;
                if ((subsystem->current_hits -
                     (damage_to_apply * ss_dif_scale)) < min_subsys_strength) {
                    // find damage needed to take object to min subsys strength
                    damage_to_apply =
                        subsystem->current_hits - min_subsys_strength;
            ...
            // WMC - since armor aborbs damage, subtract the amount of damage
            // before we apply armor
            damage_left -= (damage_to_apply * ss_dif_scale);

            // if this subsystem doesn't carry damage then subtract it off of
            // our total return
            if (subsystem->system_info
                    ->flags[Model::Subsystem_Flags::Carry_no_damage]) {
                if ((other_obj->type != OBJ_SHOCKWAVE) ||
                    (!(subsystem->system_info->flags
                           [Model::Subsystem_Flags::Carry_shockwave]))) {
                    float subsystem_factor = 0.0f;
                    if ((weapon_info_index >= 0) &&
                        ((other_obj->type == OBJ_WEAPON) ||
                         (other_obj->type == OBJ_SHOCKWAVE))) {
                        if (subsystem->flags
                                [Ship::Subsystem_Flags::Damage_as_hull]) {
                            subsystem_factor =
                                Weapon_info[weapon_info_index].armor_factor;
                        }
                        else {
                            subsystem_factor = Weapon_info[weapon_info_index]
                                                   .subsystem_factor;
                        }
                    }
            ...
            if (subsystem->armor_type_idx >= 0) {
                // Nuke: this will finally factor it in to damage_to_apply and
                // i wont need to factor it in anywhere after this
                damage_to_apply =
                    Armor_types[subsystem->armor_type_idx].GetDamage (
                        damage_to_apply, dmg_type_idx, ss_dif_scale);
            }
            else { // Nuke: no get damage call to apply difficulty scaling, so
                   // factor it in now
                damage_to_apply *= ss_dif_scale;
            }
            ...
            if (!(subsystem->flags[Ship::Subsystem_Flags::No_aggregate])) {
                ship_p->subsys_info[subsystem->system_info->type]
                    .aggregate_current_hits -= damage_to_apply;
            }
            ...
            if (subsystem->current_hits < 0.0f) {
                ...
                if (!(subsystem->flags[Ship::Subsystem_Flags::No_aggregate])) {
                    ship_p->subsys_info[subsystem->system_info->type]
                        .aggregate_current_hits -= subsystem->current_hits;
                }
                subsystem->current_hits =
                    0.0f; // set to 0 so repair on subsystem takes immediate
                          // effect
            }

            if (ship_p->subsys_info[subsystem->system_info->type]
                    .aggregate_current_hits < 0.0f) {
                ship_p->subsys_info[subsystem->system_info->type]
                    .aggregate_current_hits = 0.0f;
            }
...
static void shiphit_record_player_killer (object* killer_objp, player* p) {
    ...
    case OBJ_WEAPON:
        ...
        p->killer_weapon_index =
            Weapons[killer_objp->instance].weapon_info_index;
        p->killer_species =
            Ship_info[Ships[Objects[killer_objp->parent].instance]
                          .ship_info_index]
                .species;
    ...
    case OBJ_SHOCKWAVE:
        ...
        p->killer_weapon_index =
            shockwave_get_weapon_index (killer_objp->instance);
        p->killer_species =
            Ship_info[Ships[Objects[killer_objp->parent].instance]
                          .ship_info_index]
                .species;
...
static void show_dead_message (object* ship_objp, object* other_obj) {
    ...
    if (!(ship_objp->flags[Object::Object_Flags::Player_ship])) { return; }
...
// NOTE: hitpos is in model coordinates on the detail[0] submodel (highest
// detail hull) WILL NOT WORK RIGHT IF ON A ROTATING SUBMODEL
void ship_hit_sparks_no_rotate (object* ship_objp, vec3d* hitpos) {
    ship* ship_p = &Ships[ship_objp->instance];

    int n = ship_p->num_hits;
    if (n >= MAX_SHIP_HITS) { n = rand () % MAX_SHIP_HITS; }
    else {
        ship_p->num_hits++;
    }
    ...
    shipfx_emit_spark (
        ship_objp->instance, n); // Create the first wave of sparks
...
int get_max_sparks (object* ship_objp) {
    ...
    if (si->flags[Ship::Info_Flags::Fighter]) {
        float hull_percent =
            ship_objp->hull_strength / ship_p->ship_max_hull_strength;

        if (hull_percent > 0.8f) { return 1; }
...
// for big ships, when all spark slots are filled, make intelligent choice of
// one to be recycled
static int choose_next_spark (object* ship_objp, vec3d* hitpos) {
    ...
    for (spark_num = 0; spark_num < num_sparks; spark_num++) {
        if (shipp->sparks[spark_num].submodel_num != -1) {
            model_instance_find_world_point (
                &world_hitpos[spark_num], &shipp->sparks[spark_num].pos,
                shipp->model_instance_num,
                shipp->sparks[spark_num].submodel_num, &ship_objp->orient,
                &ship_objp->pos);
        }
        else {
    ...
    for (i = 1; i < num_sparks; i++) {
        for (j = 0; j < i; j++) {
            ...
            spark_pairs[count++].dist =
                vm_vec_dist_squared (&world_hitpos[i], &world_hitpos[j]);
        }
    }
    ...
    // look through the first few sorted pairs, counting number of indices of
    // closest pair
    ...
    for (i = 1; i < 6; i++) {
        if (spark_pairs[i].index1 == index1) { count1++; }
        if (spark_pairs[i].index2 == index1) { count1++; }
        if (spark_pairs[i].index1 == index2) { count2++; }
        if (spark_pairs[i].index2 == index2) { count2++; }
    }

    // recycle spark which has most indices in sorted list of pairs
    if (count1 > count2) { return index1; }
    else {
        return index2;
    }
}
...
// Make sparks fly off a ship.
static void
ship_hit_create_sparks (object* ship_objp, vec3d* hitpos, int submodel_num) {
    vec3d tempv;
    ship* shipp = &Ships[ship_objp->instance];
    ...
    if (instancing) {
        ...
        model_instance_find_world_point (
            &temp_zero, &vmd_zero_vector, shipp->model_instance_num,
            submodel_num, &ship_objp->orient, &ship_objp->pos);
        model_instance_find_world_point (
            &temp_x, &vmd_x_vector, shipp->model_instance_num, submodel_num,
            &ship_objp->orient, &ship_objp->pos);
        model_instance_find_world_point (
            &temp_y, &vmd_y_vector, shipp->model_instance_num, submodel_num,
            &ship_objp->orient, &ship_objp->pos);
        model_instance_find_world_point (
            &temp_z, &vmd_z_vector, shipp->model_instance_num, submodel_num,
            &ship_objp->orient, &ship_objp->pos);
...
static void player_died_start (object* killer_objp) {
    ...
    /*  vm_vec_scale_add(&Dead_camera_pos, &Player_obj->pos,
       &Player_obj->orient.fvec, -10.0f); vm_vec_scale_add2(&Dead_camera_pos,
       &Player_obj->orient.uvec, 3.0f); vm_vec_scale_add2(&Dead_camera_pos,
       &Player_obj->orient.rvec, 5.0f);
    */
    ...
    // able to determine the killer of a ship (due to
    // bogus/mismatched/timed-out signatures on the client side).  If we don't
    // know the killer, use the Player_obj as the other_objp for camera
    // position.
    if (killer_objp) {
        switch (killer_objp->type) {
        case OBJ_WEAPON:
        case OBJ_SHOCKWAVE: other_objp = &Objects[killer_objp->parent]; break;
        case OBJ_SHIP:
        case OBJ_DEBRIS:
        case OBJ_ASTEROID:
        case OBJ_NONE: // Something that just got deleted due to also dying
                       //-- it happened to me! --MK.
        ...
        case OBJ_BEAM:
            ...
            if (beam_obj_parent == -1) { other_objp = killer_objp; }
            else {
    ...
    vm_vec_add (
        &Original_vec_to_deader, &Player_obj->orient.vec.fvec,
        &Player_obj->orient.vec.rvec);
    ...
    if (Player_obj == other_objp) {
        ...
        dist = vm_vec_normalized_dir (
            &vec_from_killer, &Player_obj->pos, &other_objp->pos);
    }
...
void ship_generic_kill_stuff (object* objp, float percent_killed) {
    ...
    ship_stop_fire_primary (
        objp); // mostly for stopping fighter beam looping sounds -Bobboau
    ...
    if (sip->flags[Ship::Info_Flags::Cargo]) { delta_time /= 4; }

    // Prevent bogus timestamps.
    if (delta_time < 2) delta_time = 2;

    if (objp->flags[Object::Object_Flags::Player_ship]) {
        // Note: Kamikaze ships have no minimum death time.
        if (!(Ai_info[Ships[objp->instance].ai_index]
    ...
    if (Ai_info[sp->ai_index].ai_flags[AI::AI_Flags::Kamikaze]) delta_time = 2;
    ...
    sp->death_time = sp->final_death_time =
        timestamp (delta_time); // Give him 3 secs to explode

    // SUSHI: What are the chances of an instant vaporization? Check the ship
    // type first (objecttypes.tbl), then the ship (ships.tbl)
    ...
    sp->death_roll_snd = snd_play_3d (
        gamesnd_get_game_sound (GameSounds::DEATH_ROLL), &objp->pos,
        &View_position, objp->radius);
    if (objp == Player_obj) joy_ff_deathroll ();
    ...
    // DEATHROLL_ROTVEL_MIN At lower speed, lower death rotvel (scaled
    // linearly) At higher mass, lower death rotvel (scaled logarithmically)
    // variable scale calculates the deathroll rotational velocity magnitude
    float logval = (float)log10 (
        objp->phys_info.mass / (0.05f * DEATHROLL_MASS_STANDARD));
    float velval =
        ((vm_vec_mag_quick (&objp->phys_info.vel) + 3.0f) /
    ...
    vm_vec_zero (&objp->phys_info.max_rotvel); // make so he can't change speed
                                               // on his own VOLITION anymore.
}
...
static void ship_vaporize (ship* shipp) {
    ...
    if ((shipp->objnum < 0) || (shipp->objnum >= MAX_OBJECTS)) { return; }
...
void ship_hit_kill (
    object* ship_objp, object* other_obj, float percent_killed,
    int self_destruct) {
    ...
    if (ship_objp == Player_obj) { player_died_start (other_obj); }

    // maybe vaporize him
    if (sp->flags[Ship::Ship_Flags::Vaporize]) { ship_vaporize (sp); }
...
void ship_apply_whack (vec3d* force, vec3d* hit_pos, object* objp) {
    ...
    if (object_is_docked (objp)) {
        ...
        // Goober5000 - this code attempts to account properly for whacking a
        // docked object as one mass. It isn't perfect, because physics doesn't
        // completely account for it (particularly because it still uses the
        // moment of inertia for the whacked object, not for all objects).
        // Commenting out the contents of the block restores the Volition
        // behavior, but it doesn't calculate the correct torque. Addendum:
        // this block is now not executed for docked fighters or bombers
        // because the whack looks like the fighter is doing evasive maneuvers
        ...
        // whack it
        physics_apply_whack (
            force, hit_pos, &objp->phys_info, &objp->orient, overall_mass);
    }
    else {
        physics_apply_whack (
            force, hit_pos, &objp->phys_info, &objp->orient,
            objp->phys_info.mass);
    }
}
...
static void shiphit_hit_after_death (object* ship_objp, float damage) {
    ...
    // Since the explosion has two phases (final_death_time and
    // really_final_death_time) we should only shorten the deathroll time if
    // that is the phase we're in. And you can tell by seeing if the timestamp
    // is valid, since it gets set to invalid after it does the first large
    // explosion.
    if (!timestamp_valid (shipp->final_death_time)) { return; }

    // Don't adjust vaporized ship
    if (shipp->flags[Ship::Ship_Flags::Vaporize]) { return; }

    // Don't shorten deathroll on very large ships.
    if (ship_objp->radius > BIG_SHIP_MIN_RADIUS) return;
    ...
    shipp->final_death_time =
        timestamp (delta_time); // Adjust time until explosion.
}

MONITOR (ShipHits)
MONITOR (ShipNumDied)

static int maybe_shockwave_damage_adjust (
    object* ship_objp, object* other_obj, float* damage) {
    ship_subsys* subsys;
    ship* shipp;
    weapon_info* wip;
    ship_info* sip;
    ...
    if (!other_obj) { return 0; }

    if (other_obj->type != OBJ_SHOCKWAVE) { return 0; }

    shipp = &Ships[ship_objp->instance];
    sip = &Ship_info[shipp->ship_info_index];

    if (!sip->is_huge_ship ()) { return 0; }
    ...
    // get max damage and adjust if needed to account for shockwave created
    // from destroyed weapon
    max_damage = shockwave_get_damage (other_obj->instance);
    if (shockwave_get_flags (other_obj->instance) & SW_WEAPON_KILL) {
        max_damage *= 4.0f;
    }

    // If the shockwave was caused by a weapon, then check if the weapon can
    // deal lethal damage to the ship. If it cannot, then neither should the
    // shockwave caused by the weapon be able to do the same. The code for this
    // is copied from part of weapon_get_damage_scale.
    ...
    if ((wp_index >= 0) && Weapon_shockwaves_respect_huge) {
        ...
        if ((sip->flags[Ship::Info_Flags::Supercap]) &&
            ...
            if (hull_pct <= 0.75f) {
                *damage = 0.0f;
                ...
                // If hull isn't below 3/4, then allow damage to be applied
                // just like in weapon_get_damage_scale. SUPERCAP_DAMAGE_SCALE
                // is defined in weapon.h.
        ...
        if (is_big_damage_ship && !(wip->hurts_big_ships ())) {
            if (hull_pct > 0.1f) { max_damage *= hull_pct; }
            else {
                *damage = 0.0f;
                return 1;
            }
    ...
    if (nearest_dist > outer_radius) { *damage = max_damage / 4.0f; }
    else if (nearest_dist < inner_radius) {
        *damage = max_damage;
    }
    else {
        *damage = max_damage * (1.0f - 0.75f * (nearest_dist - inner_radius) /
...
static void ship_do_damage (
    ...
    int quadrant, int submodel_num, int wash_damage = 0) {
    ...
    if (other_obj) {
        ...
        other_obj_is_beam =
            ((other_obj->type == OBJ_BEAM) && (other_obj->instance >= 0) &&
             (other_obj->instance < MAX_BEAMS));
        other_obj_is_shockwave =
            ((other_obj->type == OBJ_SHOCKWAVE) &&
             (other_obj->instance >= 0) &&
             (other_obj->instance < MAX_SHOCKWAVES));
        other_obj_is_asteroid =
            ((other_obj->type == OBJ_ASTEROID) && (other_obj->instance >= 0) &&
             (other_obj->instance < MAX_ASTEROIDS));
        other_obj_is_debris =
            ((other_obj->type == OBJ_DEBRIS) && (other_obj->instance >= 0) &&
             (other_obj->instance < MAX_DEBRIS_PIECES));
        other_obj_is_ship =
            ((other_obj->type == OBJ_SHIP) && (other_obj->instance >= 0) &&
    ...
    if (Player->control_mode >= PCM_WARPOUT_STAGE2) {
        if (ship_objp == Player_obj) { return; }
    }
    ...
    if (ship_objp->flags[Object::Object_Flags::Invulnerable]) { return; }

    // if ship is already dying, shorten deathroll.
    if (shipp->flags[Ship::Ship_Flags::Dying]) {
        shiphit_hit_after_death (
            ship_objp, (damage * difficulty_scale_factor));
        return;
    }
    ...
    if (quadrant >= 0 &&
        !(ship_objp->flags[Object::Object_Flags::No_shields])) {
        ...
        if (damage > 0.0f) {
            ...
            if (other_obj_is_weapon) {
                ...
                dmg_type_idx =
                    Weapon_info[beam_get_weapon_info_index (other_obj)]
                        .damage_type_idx;
            }
            else if (other_obj_is_shockwave) {
                dmg_type_idx =
                    shockwave_get_damage_type_idx (other_obj->instance);
            }
            ...
            if (shipp->shield_armor_type_idx != -1) {
                piercing_pct = Armor_types[shipp->shield_armor_type_idx]
                                   .GetShieldPiercePCT (dmg_type_idx);
            }

            float pre_shield =
                damage; // Nuke: don't use the difficulty scaling in here,
                        // since its also applied in Armor_type.GetDamage.
                        // Don't want it to apply twice
            ...
            // Nuke: apply pre_shield difficulty scaling here, since it was
            // meant to be applied through damage
            pre_shield *= difficulty_scale_factor;

            if (shipp->shield_armor_type_idx != -1) {
                // Nuke: this call will decide when to use the damage factor,
                // but it will get used, unless the modder is dumb (like
                // setting +Difficulty Scale Type: to 'manual' and not manually
                // applying it in their calculations)
                damage = Armor_types[shipp->shield_armor_type_idx].GetDamage (
                    damage, dmg_type_idx, difficulty_scale_factor,
                    other_obj_is_beam);
            }
            else { // Nuke: if that didn't get called, difficulty would not be
                   // applied to damage so apply it here
                damage *= difficulty_scale_factor;
            }

            damage = shield_apply_damage (ship_objp, quadrant, damage);

            if (damage > 0.0f) { subsystem_damage *= (damage / pre_shield); }
            else {
        ...
        // if shield damage was increased, don't carry over leftover damage at
        // scaled level
    ...
    if ((damage > 0.0f) || (subsystem_damage > 0.0f)) {
        int weapon_info_index;
        float pre_subsys =
            subsystem_damage; // Nuke: should be the last time we need to do
                              // this in this function
        bool apply_hull_armor = true;
        bool apply_diff_scale = true;

        subsystem_damage = do_subobj_hit_stuff (
            ship_objp, other_obj, hitpos, submodel_num, subsystem_damage,
            &apply_hull_armor);
        ...
        if (apply_hull_armor) {
            ...
            if (shipp->armor_type_idx != -1) {
                damage = Armor_types[shipp->armor_type_idx].GetDamage (
                    damage, dmg_type_idx, difficulty_scale_factor,
                    other_obj_is_beam);
        ...
        // damage by getDamage() above
        if (apply_diff_scale) {
            damage *= difficulty_scale_factor; // Nuke: we can finally stop
                                               // doing this now
        }
        ...
        if (damage > 0.0f) {
            ...
            if (shipp->ship_guardian_threshold > 0) {
                float min_hull_strength = 0.01f *
                                          shipp->ship_guardian_threshold *
            ...
            if (other_obj) {
                ...
                case OBJ_WEAPON:
                    if ((other_obj->parent < 0) ||
                        ...
                        scoring_add_damage (
                            ship_objp, &Objects[other_obj->parent], damage);
                    }
                    break;
                case OBJ_BEAM: // give kills for fighter beams-Bobboau
                {
                    ...
                    // Goober5000 - only count beams fired by fighters or
                    // bombers unless the ai profile says different
                    if (bobjn >= 0) {
                        if (!(The_mission.ai_profile
                                  ->flags[AI::Profile_Flags::
                                              Include_beams_in_stat_calcs]) &&
                            !(Ship_info[Ships[Objects[bobjn].instance]
                                            .ship_info_index]
                                  .is_fighter_bomber ()) &&
                            !(Objects[bobjn]
                    ...
                    if (bobjn == -1) {
                        ...
                        scoring_add_damage (
                            ship_objp, &Objects[bobjn], damage);
                    }
                    break;
                }
                default: break;
            ...
            if (ship_objp->hull_strength <= 0.0f) {
                ...
                if (wash_damage) { shipp->wash_killed = 1; }
    ...
    if (other_obj_is_weapon) {
        ...
        if (Weapons[other_obj->instance].weapon_info_index < 0) { return; }
        wip = &Weapon_info[Weapons[other_obj->instance].weapon_info_index];

        // if its a leech weapon - NOTE - unknownplayer: Perhaps we should do
        // something interesting like direct the leeched energy into the
        // attacker ?
        if (wip->wi_flags[Weapon::Info_Flags::Energy_suck]) {
            ...
            shipp->weapon_energy =
                (shipp->weapon_energy < 0.0f) ? 0.0f : shipp->weapon_energy;
        }
    }
}
...
// Goober5000
void ship_apply_tag (
    int ship_num, int tag_level, float tag_time, object* target, vec3d* start,
    int ssm_index, int ssm_team) {
...
// This gets called to apply damage when something hits a particular point on a
// ship. This assumes that whoever called this knows if the shield got hit or
// not. hitpos is in world coordinates. if quadrant is not -1, then that part
// of the shield takes damage properly.
void ship_apply_local_damage (
    object* ship_objp, object* other_obj, vec3d* hitpos, float damage,
    int quadrant, bool create_spark, int submodel_num, vec3d* hit_normal) {
    ASSERT (ship_objp); // Goober5000
    ASSERT (other_obj); // Goober5000
    ...
    if (other_obj->type == OBJ_WEAPON || other_obj->type == OBJ_BEAM) {
        ...
        if (wip->wi_flags[Weapon::Info_Flags::Tag]) {
            ...
            ship_apply_tag (
                ship_objp->instance, wip->tag_level, wip->tag_time, ship_objp,
                start, ssm_index, wp->team);
        }
    }
    ...
    global_damage = false;
    ship_do_damage (
        ship_objp, other_obj, hitpos, damage, quadrant, submodel_num);

    // DA 5/5/98: move ship_hit_create_sparks() after do_damage() since number
    // of sparks depends on hull strength doesn't hit shield and we want sparks
    if ((quadrant == MISS_SHIELDS) && create_spark) {
        ...
        if (!is_subsys_destroyed (ship_p, submodel_num)) {
            ...
            if (other_obj->type == OBJ_WEAPON || other_obj->type == OBJ_BEAM) {
                ...
                if (other_obj->type == OBJ_WEAPON)
                    wip = &Weapon_info[Weapons[other_obj->instance]
                                           .weapon_info_index];
                else if (other_obj->type == OBJ_BEAM)
                    wip = &Weapon_info[Beams[other_obj->instance]
                                           .weapon_info_index];
        ...
        // fireball_create( hitpos, FIREBALL_SHIP_EXPLODE1,
        // OBJ_INDEX(ship_objp), 0.25f );
    }
}

// This gets called to apply damage when a damaging force hits a ship, but at
// no point in particular.  Like from a shockwave.   This routine will see if
// the shield got hit and if so, apply damage to it. You can pass
// force_center==NULL if you the damage doesn't come from anywhere, like for
// debug keys to damage an object or something.  It will assume damage is
// non-directional and will apply it correctly.
void ship_apply_global_damage (
    object* ship_objp, object* other_obj, vec3d* force_center, float damage) {
    ASSERT (ship_objp); // Goober5000 (but not other_obj in case of sexp)
    ...
    if (force_center) {
        ...
        vm_vec_scale_add (
            &world_hitpos, &ship_objp->pos, &tmp, ship_objp->radius);
        ...
        ship_do_damage (
            ship_objp, other_obj, &world_hitpos, damage, shield_quad, -1);
    }
    else {
        // Since an force_center wasn't specified, this is probably just a
        // debug key to kill an object.   So pick a shield quadrant and a point
        // on the radius of the object.
        vm_vec_scale_add (
            &world_hitpos, &ship_objp->pos, &ship_objp->orient.vec.fvec,
            ship_objp->radius);

        for (int i = 0; i < ship_objp->n_quadrants; i++) {
            ship_do_damage (
                ship_objp, other_obj, &world_hitpos,
                damage / ship_objp->n_quadrants, i, -1);
        }
    }

    // AL 3-30-98: Show flashing blast icon if player ship has taken blast
    // damage
    if (ship_objp == Player_obj) {
        // only show blast icon if playing on medium skill or lower ->
        // unknownplayer: why? I think this should be changed. Goober5000 -
    ...
    // evaluate any player stats scoring conditions (specifically, blasts from
    // remotely detonated secondary weapons)
    scoring_eval_hit (ship_objp, other_obj, 1);
}

void ship_apply_wash_damage (
    object* ship_objp, object* other_obj, float damage) {
    vec3d world_hitpos, direction_vec, rand_vec;

    // Since an force_center wasn't specified, this is probably just a debug
    // key to kill an object.   So pick a shield quadrant and a point on the
    // radius of the object
    vm_vec_rand_vec_quick (&rand_vec);
    vm_vec_scale_add (
        &direction_vec, &ship_objp->orient.vec.fvec, &rand_vec, 0.5f);
    vm_vec_normalize_quick (&direction_vec);
    vm_vec_scale_add (
        &world_hitpos, &ship_objp->pos, &direction_vec, ship_objp->radius);
...
void ship_hit_pain (float damage, int quadrant) {
    ...
    if (!(Player_obj->flags[Object::Object_Flags::Invulnerable])) {
        if (Shield_pain_flash_factor != 0.0f && quadrant >= 0) {
            float effect = (Shield_pain_flash_factor *
                            Player_obj->shield_quadrant[quadrant] *
                            Player_obj->n_quadrants) /
                           shield_get_max_strength (Player_obj);

            if (Shield_pain_flash_factor < 0.0f)
                effect -= Shield_pain_flash_factor;

            game_flash (
                (sip->shield_color[0] * effect) / 255.0f,
                (sip->shield_color[1] * effect) / 255.0f,
                (sip->shield_color[2] * effect) / 255.0f);
        }
        else
            game_flash (
                damage * Generic_pain_flash_factor / 15.0f,
