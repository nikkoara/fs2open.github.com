...
void awacs_update_all_levels () {
    ...
    // zero all levels
    Awacs_level = 0.0f;
    for (idx = 0; idx < MAX_IFFS; idx++) Awacs_team[idx] = 0.0f;
    ...
    for (moveup = GET_FIRST (&Ship_obj_list);
        ...
        if ((Objects[moveup->objnum].type != OBJ_SHIP) ||
        ...
        if (!(Ship_info[shipp->ship_info_index]
                  .flags[Ship::Info_Flags::Has_awacs]))
            continue;
        ...
        for (ship_system = GET_FIRST (&shipp->subsys_list);
            ...
            if ((ship_system->system_info != NULL) &&
                (ship_system->system_info
                     ->flags[Model::Subsystem_Flags::Awacs])) {
                // add the intensity to the team total
                Awacs_team[shipp->team] +=
                    ship_system->awacs_intensity *
                    (ship_system->current_hits / ship_system->max_hits);
...
float awacs_get_level (object* target, ship* viewer, int use_awacs) {
    ...
    float test;
    int closest_index = -1;
    int idx, stealth_ship = 0, check_huge_ship = 0,
             friendly_stealth_invisible = 0;
    ship* shipp = NULL;
    ship_info* sip = NULL;

    int viewer_has_primitive_sensors =
        (viewer->flags[Ship::Ship_Flags::Primitive_sensors]);
...
#define FULLY_TARGETABLE                                \
    (viewer_has_primitive_sensors                       \
         ? ((distance < viewer->primitive_sensor_range) \
    ...
    if ((Hud_max_targeting_range > 0) &&
        (distance > Hud_max_targeting_range)) {
        return UNTARGETABLE;
    }

    if (target->type == OBJ_SHIP) {
        // if no valid target then bail as never viewable
        if (target->instance < 0) return UNTARGETABLE;
        ...
        friendly_stealth_invisible =
            (shipp->flags[Ship::Ship_Flags::Friendly_stealth_invis]);
    ...
    if ((stealth_ship || nebula_enabled) && use_awacs) {
        for (idx = 0; idx < Awacs_count; idx++) {
            ...
            if (Awacs[idx].team != viewer->team) continue;

            // if this awacs source has somehow become invalid
            if (Awacs[idx].objp->type != OBJ_SHIP) continue;

            // get the subsystem position
            if (!get_subsystem_pos (
                    &subsys_pos, Awacs[idx].objp, Awacs[idx].subsys))
                continue;
            ...
            if (check_huge_ship) {
                ...
                if (check_world_pt_in_expanded_ship_bbox (
                        &subsys_pos, target,
                        Awacs[idx].subsys->awacs_radius)) {
                    closest_index = idx;
                    break;
                }
            }
            // not a huge ship
                ...
                if (test > Awacs[idx].subsys->awacs_radius) continue;
    ...
    if (stealth_ship) {
        ...
        if (closest_index >= 0) {
            // if the nebula effect is active, stealth ships are only partially
            // targetable
            if (nebula_enabled) return MARGINALLY_TARGETABLE;
    ...
    // all other ships
        ...
        if (!nebula_enabled) return FULLY_TARGETABLE;

        // if the ship is within range of an awacs, its fully targetable
        if (closest_index >= 0) return FULLY_TARGETABLE;
        ...
        float scan_nebula_range =
            Neb2_awacs *
            Species_info[Ship_info[viewer->ship_info_index].species]
                .awacs_multiplier;

        // special case for huge ship - check inside expanded bounding boxes
        if (check_huge_ship) {
            if (check_world_pt_in_expanded_ship_bbox (
                    &Objects[viewer->objnum].pos, target, scan_nebula_range)) {
                if (check_world_pt_in_expanded_ship_bbox (
                        &Objects[viewer->objnum].pos, target,
                        0.5f * scan_nebula_range))
                    return FULLY_TARGETABLE;
...
void team_visibility_update () {
    ...
    for (moveup = GET_FIRST (&Ship_obj_list);
        ...
        // ignore dying, departing, or arriving ships
        if (shipp->is_dying_or_departing () || shipp->is_arriving ()) continue;

        // check if ship is flagged as invisible
        if (shipp->flags[Ship::Ship_Flags::Hidden_from_sensors]) continue;
    ...
    for (int cur_team = 0; cur_team < MAX_IFFS; cur_team++) {
        ...
        if (cur_count == 0)
            continue; // Goober5000 10/06/2005 changed from break; probably a
                      // bug
        ...
        for (int en_team = 0; en_team < MAX_IFFS; en_team++) {
            ...
            for (en_idx = 0; en_idx < en_count; en_idx++) {
                ...
                for (idx = 0; idx < cur_count; idx++) {
                    ...
                    if (Ship_info[Ships[cur_team_ships[idx]].ship_info_index]
                            .flags[Ship::Info_Flags::Cargo] ||
                        Ship_info[Ships[cur_team_ships[idx]].ship_info_index]
                    ...
                    if (awacs_get_level (
                            &Objects[Ships[en_team_ships[en_idx]].objnum],
                            &Ships[cur_team_ships[idx]], (idx == 0)) > 1.0f) {
                        Ship_visibility_by_team[cur_team]
                                               [en_team_ships[en_idx]] = 1;
                        break;
                    }
                }
            }
...
int ship_is_visible_by_team (object* target, ship* viewer) {
    ...
    if ((Hud_max_targeting_range > 0) &&
        (vm_vec_dist_quick (&target->pos, &Objects[viewer->objnum].pos) >
