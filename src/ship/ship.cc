...
static int lightningtype_match(char *p)
{
        ...
        for (i = 0; i < Num_lightning_types; i++) {
                if (!strcasecmp(Lightning_types[i], p))
                        return i;
        }
...
#define CHECK_THEN_COPY(attribute)                           \
        do {                                                 \
                if (other.attribute != NULL)                 \
                        attribute = strdup(other.attribute); \
        } while (false)
                        ...
                        for (int i = 0; i < n_subsystems; i++) {
                                for (int j = 0; j < subsystems[i].n_triggers; j++) {
                                        if (i < other.n_subsystems) {
                                                subsystems[i].triggers = (queued_animation *)realloc(
        ...
        const bool first_time)
{
        ...
        const char *allowed_banks_str = is_primary
        ...
        if (optional_string(allowed_banks_str)) {
                ...
                while (check_for_string("(")) {
                        ...
                        for (i = 0; i < num_allowed; i++) {
                                if (allowed_weapons[i] >= 0) // MK, Bug fix, 9/6/99.  Used to be "allowed_weapons"
...
static int parse_and_add_briefing_icon_info()
{
        ...
        for (icon = 0; icon < Briefing_icon_info.size(); icon++) {
                if (!strcasecmp(
                    ...
                    && !strcasecmp(fade_temp, Briefing_icon_info[icon].fade.filename) && !strcasecmp(highlight_temp, Briefing_icon_info[icon].highlight.filename)) {
                        bii_index = (int)icon;
                        break;
                }
        }
        ...
        const bool replace)
{
        ...
        if (optional_string("$Collision Physics:")) {
                ...
                if (optional_string("+Landing Max Angle:")) {
                        float degrees;
                        stuff_float(&degrees);
                        sip->collision_physics.landing_max_angle = cosf(to_radians(90.0f - degrees));
                }
                if (optional_string("+Landing Min Angle:")) {
                        float degrees;
                        stuff_float(&degrees);
                        sip->collision_physics.landing_min_angle = cosf(to_radians(90.0f - degrees));
                }
                if (optional_string("+Landing Max Rotate Angle:")) {
                        float degrees;
                        stuff_float(&degrees);
                        sip->collision_physics.landing_max_rot_angle = cosf(to_radians(90.0f - degrees));
                }
                ...
                if (optional_string("+Reorient Max Angle:")) {
                        float degrees;
                        stuff_float(&degrees);
                        sip->collision_physics.reorient_max_angle = cosf(to_radians(90.0f - degrees));
                }
                if (optional_string("+Reorient Min Angle:")) {
                        float degrees;
                        stuff_float(&degrees);
                        sip->collision_physics.reorient_min_angle = cosf(to_radians(90.0f - degrees));
                }
                if (optional_string("+Reorient Max Rotate Angle:")) {
                        float degrees;
                        stuff_float(&degrees);
                        sip->collision_physics.reorient_max_rot_angle = cosf(to_radians(90.0f - degrees));
                }
                ...
                if (optional_string("+Landing Rest Angle:")) {
                        float degrees;
                        stuff_float(&degrees);
                        sip->collision_physics.landing_rest_angle = cosf(to_radians(90.0f - degrees));
                }
                parse_game_sound(
        ...
        if (optional_string("$Flags:")) {
                ...
                for (auto i = 0; i < num_strings; i++) {
                        ...
                        if (!strcasecmp(ship_strings[i], "no-collide") || !strcasecmp(ship_strings[i], "no_collide")) {
                                flag_found = true;
                                ...
                                if (!strcasecmp(name_tmp, "triggered")) {
                                        ...
                                        if (current_trigger->type == TRIGGER_TYPE_INITIAL) {
                                                ...
                                                if (optional_string("+absolute_angle:")) {
                                                        ...
                                                        current_trigger->angle.xyz.x = to_radians(current_trigger->angle.xyz.x);
                                                        current_trigger->angle.xyz.y = to_radians(current_trigger->angle.xyz.y);
                                                        current_trigger->angle.xyz.z = to_radians(current_trigger->angle.xyz.z);
                                                } else {
                                                ...
                                                if (optional_string("+velocity:")) {
                                                        ...
                                                        current_trigger->vel.xyz.x = to_radians(current_trigger->vel.xyz.x);
                                                        current_trigger->vel.xyz.y = to_radians(current_trigger->vel.xyz.y);
                                                        current_trigger->vel.xyz.z = to_radians(current_trigger->vel.xyz.z);
                                                }

                                                if (optional_string("+acceleration:")) {
                                                        stuff_vec3d(&current_trigger->accel);
                                                        current_trigger->accel.xyz.x = to_radians(current_trigger->accel.xyz.x);
                                                        current_trigger->accel.xyz.y = to_radians(current_trigger->accel.xyz.y);
                                                        current_trigger->accel.xyz.z = to_radians(current_trigger->accel.xyz.z);
                                                }
...
static void parse_ship_type()
{
        ...
        if (!strcasecmp(stp->name, "sentrygun")) {
                ship_type = "sentry gun";
...
static void ship_parse_post_cleanup()
{
        ...
        for (auto sip = Ship_info.begin(); sip != Ship_info.end(); ++sip) {
                ...
                if ((sip->shield_icon_index == 255) && (sip->flags[Ship::Info_Flags::Afterburner]) && !(sip->flags[Ship::Info_Flags::Generate_hud_icon]) && (sip->flags[Ship::Info_Flags::Player_ship])) {
...
int ship_find_exited_ship_by_name(const char *name)
{
        ...
        for (i = 0; i < (int)Ships_exited.size(); i++) {
                if (!strcasecmp(name, Ships_exited[i].ship_name))
                        return i;
        }
...
void physics_ship_init(object *objp)
{
        ...
        if (IS_VEC_NULL(&pm->moment_of_inertia.vec.rvec) && IS_VEC_NULL(&pm->moment_of_inertia.vec.uvec) && IS_VEC_NULL(&pm->moment_of_inertia.vec.fvec)) {
        ...
        // overridden
        pi->use_newtonian_damp = The_mission.ai_profile
...
static int subsys_set(int objnum, int ignore_subsys_info)
{
        ...
        for (i = 0; i < sinfo->n_subsystems; i++) {
                ...
                ship_system = GET_FIRST(&ship_subsys_free_list); // get a new element from the
                                                                 // ship_subsystem array
                ASSERT(
                        ship_system != &ship_subsys_free_list); // shouldn't have the dummy element
                list_remove(
                ...
                ship_system->system_info = model_system; // set the system_info pointer to point to the data
                ...
                for (k = 0; k < MAX_SHIP_PRIMARY_BANKS; k++) {
                        if (model_system->primary_banks[k] != -1) {
                                ...
                                ship_system->weapons.primary_bank_capacity[j] = model_system
                ...
                for (k = 0; k < ship_system->weapons.num_secondary_banks; k++) {
                        float weapon_size = Weapon_info[ship_system->weapons.secondary_bank_weapons[k]]
                                                    .cargo_size;
                ...
                for (k = 0; k < ship_system->weapons.num_primary_banks; k++) {
                        float weapon_size = Weapon_info[ship_system->weapons.primary_bank_weapons[k]]
                                                    .cargo_size;
                        ...
                        if (weapon_size > 0.0f) { // Non-ballistic primaries are supposed
                                ...
                                ship_system->weapons.primary_bank_ammo[k] = (int)std::lround(
                                        ship_system->weapons.primary_bank_capacity[k] / weapon_size);
                        }
                }
...
void ship_init_cockpit_displays(ship *shipp)
{
        ...
        Player_cockpit_textures = (int *)malloc(MAX_REPLACEMENT_TEXTURES * sizeof(int));
...
void ship_cleanup(int shipnum, int cleanup_mode)
{
        ...
        ASSERT(
                cleanup_mode & (SHIP_DESTROYED | SHIP_DEPARTED | SHIP_VANISHED | SHIP_DESTROYED_REDALERT | SHIP_DEPARTED_REDALERT));
        ...
        if ((cleanup_mode == SHIP_DEPARTED_WARP) || (cleanup_mode == SHIP_DEPARTED_BAY) || (cleanup_mode == SHIP_DEPARTED)) { // see if this ship departed within the radius of a
                ...
                for (objp = GET_FIRST(&obj_used_list);
                     objp != END_OF_LIST(&obj_used_list); objp = GET_NEXT(objp)) {
                        ...
                        if (ship_explode_area_calc_damage(
                                    ...
                                    max_damage, max_blast, &damage, &blast)
                            == -1) {
                                continue;
                        }
        ...
        if (shipp->flags[Ship_Flags::Dying]) {
                ...
                if (knossos_ship) {
                        if (timestamp_elapsed(shipp->next_fireball)) {
                                ...
                                float scale = -vm_vec_dot(&objp->orient.vec.fvec, &rand_vec) * (0.9f + 0.2f * fs2::prng::randf(0));
                                ...
                                pe.normal_variance = pef.variance; // How close they stick to that normal
                ...
                if (timestamp_elapsed(shipp->final_death_time)) {
                        ...
                        if (!knossos_ship) {
                                if (sip->death_effect.isValid()) {
                                        ...
                                        auto source = particle::ParticleManager::get()->createSource(
                                                sip->death_effect);
                        ...
                        if (ship_get_exp_propagates(shipp)) {
                                ...
                                shipp->end_death_time = shipp->really_final_death_time = timestamp(
                                        int(explosion_life * 1000.0f) / 5); // Wait till 30% of vclip time before breaking the
                                                                            // ship up.
                        }
...
static void ship_auto_repair_frame(int shipnum, float frametime)
{
        ...
        for (ssp = GET_FIRST(&sp->subsys_list);
             ssp != END_OF_LIST(&sp->subsys_list); ssp = GET_NEXT(ssp)) {
                ASSERT(
                        ssp->system_info->type >= 0 && ssp->system_info->type < SUBSYSTEM_MAX);
...
void ship_process_post(object *obj, float frametime)
{
        ...
        if ((!(shipp->is_arriving()) || (Ai_info[shipp->ai_index].mode == AIM_BAY_EMERGE) || ((sip->warpin_type == WT_IN_PLACE_ANIM) && (shipp->flags[Ship_Flags::Arriving_stage_2]))) && !(shipp->flags[Ship_Flags::Depart_warp])) {
                // Do AI.
...
static void ship_set_default_weapons(ship *shipp, ship_info *sip)
{
        ...
        for (i = 0; i < swp->num_secondary_banks; i++) {
                ...
                swp->secondary_bank_ammo[i] = int(sip->secondary_bank_ammo_capacity[i] / size);
...
static void ship_init_afterburners(ship *shipp)
{
        ...
        for (int i = 0; i < pm->n_thrusters; i++) {
                ...
                for (int j = 0; j < bank->num_points; j++) {
                        ...
                        ci->w_start = bank->points[j].radius * sip->afterburner_trail_width_factor; // width * table loaded
                                                                                                    // width factor
                        ci->w_end = 0.05f;                                                          // end width

                        ci->a_start = 1.0f * sip->afterburner_trail_alpha_factor; // start alpha  * table
                                                                                  // loaded alpha factor
                        ...
                        ci->texture.bitmap_id = sip->afterburner_trail
...
int ship_create(matrix *orient, vec3d *pos, int ship_type, char *ship_name)
{
        ...
        ASSERTX(
                (ship_type >= 0) && (ship_type < static_cast< int >(Ship_info.size())),
                                ...
                                for (k = 0; k < pm->paths[i].verts[j].nturrets; k++) {
                                        ...
                                        while (ss != END_OF_LIST(&Ships[n].subsys_list)) {
                                                if (ss->system_info->subobj_num == ptindex) {        // when these are equal, fix up the ref
                                                        pm->paths[i].verts[j].turret_ids[k] = index; // in path structure to index a
                                                                                                     // ship_subsys
                                                        break;
                                                }
                                                index++;
...
static void ship_model_change(int n, int ship_type)
{
        ...
        // Allocate shield and initialize it.
        if (pm->shield.ntris) {
                sp->shield_integrity = (float *)malloc(sizeof(float) * pm->shield.ntris);
...
void change_ship_type(int n, int ship_type, int by_sexp)
{
                ...
                for (i = 0; i < MAX_AI_INFO; i++) {
                        if (Ai_info[i].shipnum > -1) {
                                ...
                                for (int j = 0; j < MAX_AI_GOALS; j++) {
                                        ...
                                        if (goals[j].ai_mode == AI_GOAL_DESTROY_SUBSYSTEM && !(goals[j].flags[AI::Goal_Flags::Subsys_needs_fixup])) { // If the subsystem
                                                                                                                                                      // name hasn't been
                                                                                                                                                      // parsed yet, we're
                                                                                                                                                      // fine.
        ...
        while (ss != END_OF_LIST(&sp->subsys_list)) {
                                                ...
                                                ivec3
                                                        ...
                                                        = { *it, -1,
                                                            num_saved_subsystems }; // -1 for the "goal" index means targeted, not
                                                                                    // actually a goal
                                                subsystem_matches.push_back(temp);
                                                aip->targeted_subsys = NULL; // Clear this so that aip->last_subsys_target
                ...
                // extra check
                ASSERT(
                        subsys_pcts[num_saved_subsystems] >= 0.0f && subsys_pcts[num_saved_subsystems] <= 1.0f);
        ...
        if ((p_objp
            ...
            && (sp->ship_max_shield_strength > 0.0f)) {
                ...
                (p_objp->flags[Mission::Parse_Object_Flags::OF_No_shields]) || (sp->ship_max_shield_strength == 0.0f)) {
                ...
                !(sip_orig->flags[Info_Flags::Intrinsic_no_shields]) && (sip->flags[Info_Flags::Intrinsic_no_shields])) {
                objp->flags.set(Object::Object_Flags::No_shields);
        } else if (
                (sip_orig->flags[Info_Flags::Intrinsic_no_shields]) && !(sip->flags[Info_Flags::Intrinsic_no_shields]) && (sp->ship_max_shield_strength > 0.0f)) {
                objp->flags.remove(Object::Object_Flags::No_shields);
        }
        ...
        sp->afterburner_fuel = MAX(
                0, sip->afterburner_fuel_capacity - (sip_orig->afterburner_fuel_capacity - sp->afterburner_fuel));
        ...
        if (sip->flags[Ship::Info_Flags::Afterburner]) {
                ...
                for (int h = 0; h < pm->n_thrusters; h++) {
                        for (int j = 0; j < pm->thrusters[h].num_points; j++) {
                                ...
                                ci->w_start = pm->thrusters[h].points[j].radius * sip->afterburner_trail_width_factor; // width * table
                                                                                                                       // loaded width factor

                                ci->w_end = 0.05f; // end width

                                ci->a_start = 1.0f * sip->afterburner_trail_alpha_factor; // start alpha  *
                                                                                          // table loaded alpha
                                                                                          // factor
                                ...
                                ci->n_fade_out_sections = sip->afterburner_trail_faded_out_sections; // table loaded
                                                                                                     // n sections to
                                                                                                     // be faded out
...
int ship_fire_primary_debug(object *objp)
{
        ...
        for (i = 0; i < MAX_WEAPON_TYPES; i++)
                if (!strcasecmp(Weapon_info[i].name, NOX("Debug Laser")))
                        break;
...
static int ship_maybe_play_secondary_fail_sound(weapon_info *wip)
{
        hud_start_flash_weapon(
                Player_ship->weapons.num_primary_banks + Player_ship->weapons.current_secondary_bank);
...
static int ship_weapon_maybe_fail(ship *sp)
{
        ...
        if (weapons_subsys_str < SUBSYS_WEAPONS_STR_FIRE_FAIL) {
                ...
                if ((fs2::prng::randf(0) - 0.2f) > weapons_subsys_str)
                        rval = 1;
        }
...
bool in_autoaim_fov(ship *shipp, int bank_to_fire, object *obj)
{
        ...
        has_converging_autoaim = (sip->aiming_flags[Ship::Aiming_Flags::Autoaim_convergence] || The_mission.ai_profile->player_autoaim_fov[Game_skill_level] > 0.0f) && aip->target_objnum != -1;
...
int ship_fire_primary(object *obj, int stream_weapons, int force)
{
        ...
        for (i = 0; i < num_primary_banks; i++) {
                ...
                if (winfo_p->burst_shots > swp->burst_counter[bank_to_fire]) {
                        ...
                        winfo_p->max_delay != 0.0f && winfo_p->min_delay != 0.0f) { // Random fire delay (DahBlount)
                        ...
                        } else // if this isn't a fighter beam, do it normally -Bobboau
                        {
                            ...
                            */
                                                        ...
                                                        if ((winfo_p->wi_flags
                                                            ...
                                                            && (obj == Player_obj) && !(Game_mode & GM_STANDALONE_SERVER)) {
                                                                // calculate some arbitrary value between 100
                                                                // (mass * velocity) / 10
                                                                game_shudder_apply(
                                                                        500, (winfo_p->mass * winfo_p->max_speed) * 0.1f);
                                                        }
                                        ...
                                        if (winfo_p->launch_snd.isValid()) {
                                                ...
                                                if ((winfo_p->pre_launch_snd
                                                             .isValid()) // If this weapon type has a
                                                                         // pre-fire sound
                                                    && ((timestamp() - swp->last_primary_fire_sound_stamp[bank_to_fire]) >= winfo_p
                                                                                                                                    ->pre_launch_snd_min_interval) // and if we're
                                                                                                                                                                   ...
                                                                                                                                                                   // the last
                                                                                                                                                                   // cease-fire
                                                    && (shipp->was_firing_last_frame[bank_to_fire] == 0)                                                           // and if we are at the beginning of a
                                                                                                                                                                   // firing stream
                                                ) {
                                                        snd_play(
                                                                gamesnd_get_game_sound(
        ...
        for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
             so = GET_NEXT(so)) {
                ...
                if ((shipp->targeting_laser_bank >= 0) && (shipp->targeting_laser_bank < 2)) {
                        ...
                        fire_info.beam_info_index = shipp->weapons
...
int ship_fire_secondary(object *obj, int allow_swarm)
{
        ...
        ASSERT(
                (swp->secondary_bank_weapons[bank] >= 0) && (swp->secondary_bank_weapons[bank] < MAX_WEAPON_TYPES));
...
int ship_select_next_primary(object *objp, int direction)
{
        ...
        if (swp->num_primary_banks == 0) {
                ...
                ASSERT(
                        (swp->current_primary_bank >= 0) && (swp->current_primary_bank < swp->num_primary_banks));
...
int ship_select_next_secondary(object *objp)
{
        ...
        if (swp->num_secondary_banks == 0) {
                ...
                ASSERT(
                        (swp->current_secondary_bank >= 0) && (swp->current_secondary_bank < swp->num_secondary_banks));
...
int wing_lookup(const char *name)
{
        ...
        for (idx = 0; idx < Num_wings; idx++)
                if (strcasecmp(Wings[idx].name, name) == 0)
                        return idx;
        ...
        for (i = 0; i < MAX_SHIPS; i++) {
                if (Ships[i].objnum >= 0) {
                        if (Objects[Ships[i].objnum].type == OBJ_SHIP || (Objects[Ships[i].objnum].type == OBJ_START && inc_players)) {
                                if (!strcasecmp(name, Ships[i].ship_name)) {
                                        return i;
                                }
                        }
                }
...
int ship_type_name_lookup(const char *name)
{
        ...
        for (size_t idx = 0; idx < max_size; idx++) {
                if (!strcasecmp(name, Ship_types[idx].name)) {
                        return (int)idx;
                }
        }
        // couldn't find it
...
float ship_calculate_rearm_duration(object *objp)
{
        ...
        for (i = 0; i < swp->num_primary_banks; i++) {
                ...
                if (wip->wi_flags[Weapon::Info_Flags::Ballistic]) {
                        ...
                        if (!found_first_empty && (swp->primary_bank_start_ammo[i] - swp->primary_bank_ammo[i])) {
                                ...
                                prim_rearm_time += gamesnd_get_max_duration(gamesnd_get_game_sound(
        ...
        for (i = 0; i < swp->num_secondary_banks; i++) {
                ...
                if (!found_first_empty && (swp->secondary_bank_start_ammo[i] - swp->secondary_bank_ammo[i])) {
                        found_first_empty = true;
                        sec_rearm_time += gamesnd_get_max_duration(
...
int ship_do_rearm_frame(object *objp, float frametime)
{
        ...
        if (!(objp->flags[Object::Object_Flags::No_shields])) {
                ...
                if (shield_str < (max_shield_str)) {
                        ...
                        shield_str += shipp->ship_max_shield_strength * frametime * sip->sup_shield_repair_rate; // repair rate is unaffected by
                                                                                                                 // $Max Shield Recharge
                        if (shield_str > max_shield_str) {
                ...
                for (i = 0; i < swp->num_primary_banks; i++) {
                        if (Weapon_info[swp->primary_bank_weapons[i]]
                                ...
                                if (aip->rearm_first_ballistic_primary == TRUE) {
                                        ...
                                        swp->primary_bank_rearm_time[i] = timestamp((int)gamesnd_get_max_duration(
...
int ship_find_repair_ship(object *requester_obj, object **ship_we_found)
{
        ...
        ASSERTX(
                (requester_obj->instance >= 0) && (requester_obj->instance < MAX_SHIPS),
        ...
        // one in)
                ...
                (num_support_ships < The_mission.support_ships.max_concurrent_ships) && ((Arriving_support_ship == NULL && The_mission.support_ships.tally < The_mission.support_ships.max_support_ships) || (Arriving_support_ship != NULL && The_mission.support_ships.tally <= The_mission.support_ships.max_support_ships))) {
        ...
        for (objp = GET_FIRST(&obj_used_list);
             objp != END_OF_LIST(&obj_used_list); objp = GET_NEXT(objp)) {
                ...
                if (!iff_matches_mask(Ships[objp->instance].team, team_mask))
                        ...
                        (flags == SHIP_GET_NO_PLAYERS) && (objp->flags[Object::Object_Flags::Player_ship]))
                        continue;
                else if (
                        (flags == SHIP_GET_ONLY_PLAYERS) && !(objp->flags[Object::Object_Flags::Player_ship]))
                        continue;
...
int ship_secondary_bank_has_ammo(int shipnum)
{
        ...
        ASSERT(
                swp->current_secondary_bank >= 0 && swp->current_secondary_bank < MAX_SHIP_SECONDARY_BANKS);
...
int ship_primary_bank_has_ammo(int shipnum)
{
        ...
        ASSERT(
                swp->current_primary_bank >= 0 && swp->current_primary_bank < MAX_SHIP_PRIMARY_BANKS);
...
void ship_maybe_praise_self(ship *deader_sp, ship *killer_sp)
{
        ...
        if ((int)(fs2::prng::randf(0) * 100) > Builtin_messages[MESSAGE_PRAISE_SELF].occurrence_chance) {
                return;
        }
...
static void awacs_maybe_ask_for_help(ship *sp, int multi_team_filter)
{
        ...
        if (strcasecmp(Campaign.filename, "freespace2") != 0 || !(Game_mode & GM_CAMPAIGN_MODE))
                return;
        ...
        if (objp->hull_strength < ((AWACS_HELP_HULL_LOW + .01f * (fs2::prng::randi(0) & 5)) * sp->ship_max_hull_strength)) {
                ...
                objp->hull_strength < ((AWACS_HELP_HULL_HI + 0.01f * (fs2::prng::randi(0) & 5)) * sp->ship_max_hull_strength)) {
...
void ship_maybe_scream(ship *sp)
{
        ...
        if (!(sp->flags[Ship_Flags::Always_death_scream])) {
                ...
                if ((int)(fs2::prng::randf(0) * 100) > Builtin_messages[MESSAGE_WINGMAN_SCREAM].occurrence_chance) {
                        return;
                }
                ...
                for (i = 0; i < MAX_SHIP_PRIMARY_BANKS; i++) {
                        if (swp->primary_animation_position[i] == MA_POS_NOT_SET) {
                                if (model_anim_start_type(
                                        ...
                                        swp->primary_animation_done_time[i] = model_anim_get_time_type(
                        ...
                        if (i == swp->current_secondary_bank) {
                                ...
                                if (swp->secondary_animation_position[i] == MA_POS_NOT_SET) {
                                        if (model_anim_start_type(
                                                ...
                                                swp->secondary_animation_done_time[i] = model_anim_get_time_type(
...
void ship_page_in()
{
        ...
        for (auto sip = Ship_info.begin(); sip != Ship_info.end(); i++, ++sip) {
                ...
                for (auto it = Ship_info.begin(); it != Ship_info.end(); ++it) {
                        if ((it->model_num > -1) && !strcasecmp(sip->pof_file, it->pof_file)) {
                                ...
                                for (j = 0; j < sip->n_subsystems; j++) {
                                        if (sip->subsystems[j].model_num != sip->model_num) {
                                                polymodel *sip_pm = (sip->model_num >= 0)
                                                                            ...
                                                                            : NULL;
                                                polymodel *subsys_pm = (sip->subsystems[j].model_num >= 0)
...
int is_support_allowed(object *objp, bool do_simple_check)
{
        ...
        if ((The_mission.support_ships.ship_class < 0) && !(The_mission.support_ships.support_available_for_species & (1 << Ship_info[shipp->ship_info_index].species))) {
                return 0;
        }
...
int wing_has_conflicting_teams(int wing_index)
{
        ...
        // sanity checks
        ASSERT(
                (wing_index >= 0) && (wing_index < Num_wings) && (Wings[wing_index].wave_count > 0));
...
void ship_update_artillery_lock()
{
        ...
        for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list);
             so = GET_NEXT(so)) {
                ...
                ASSERT(
                        (shipp->weapons.current_primary_bank >= 0) && (shipp->weapons.current_primary_bank < 2));
        ...
        vec3d *world_pt, object *objp, float delta_box)
{
        ...
        return (
                (ship_pt.xyz.x > pm->mins.xyz.x - delta_box) && (ship_pt.xyz.x < pm->maxs.xyz.x + delta_box) && (ship_pt.xyz.y > pm->mins.xyz.y - delta_box) && (ship_pt.xyz.y < pm->maxs.xyz.y + delta_box) && (ship_pt.xyz.z > pm->mins.xyz.z - delta_box) && (ship_pt.xyz.z < pm->maxs.xyz.z + delta_box));
}
...
float ship_get_warpout_speed(object *objp)
{
        ...
        if (sip->warpout_type == WT_IN_PLACE_ANIM && sip->warpout_speed <= 0.0f) {
                ...
                sip->warpout_type == WT_SWEEPER || sip->warpout_type == WT_IN_PLACE_ANIM) {
...
int ship_has_energy_weapons(ship *shipp)
{
        ...
        return (
                Ship_info[shipp->ship_info_index].max_weapon_reserve > WEAPON_RESERVE_THRESHOLD);
}
...
int ship_starting_wing_lookup(const char *wing_name)
{
        for (int i = 0; i < MAX_STARTING_WINGS; i++) {
                if (!strcasecmp(Starting_wing_names[i], wing_name))
                        return i;
        }
...
int ship_squadron_wing_lookup(const char *wing_name)
{
        for (int i = 0; i < MAX_SQUADRON_WINGS; i++) {
                if (!strcasecmp(Squadron_wing_names[i], wing_name))
                        return i;
        }
...
int ship_tvt_wing_lookup(const char *wing_name)
{
        for (int i = 0; i < MAX_TVT_WINGS; i++) {
                if (!strcasecmp(TVT_wing_names[i], wing_name))
                        return i;
        }
...
static int damage_type_get_idx(char *name)
{
        ...
        for (int i = 0; i < (int)Damage_types.size(); i++) {
                if (!strcasecmp(name, Damage_types[i].name))
                        return i;
        }
        ...
        if (adtp != NULL) {
                ...
                for (size_t i = 0; i < num; i++) {
                        ...
                        if ((storage_idx >= 0) && (storage_idx < AT_NUM_STORAGE_LOCATIONS)) {
                                ...
                                storage_idx == AT_CONSTANT_NOT_USED) { // save time checking all possible
                        ...
                        // damages
                        if (is_beam && (adtp->Calculations[i] == AT_TYPE_ADDITIVE || adtp->Calculations[i] == AT_TYPE_CUTOFF || adtp->Calculations[i] == AT_TYPE_REVERSE_CUTOFF || adtp->Calculations[i] == AT_TYPE_INSTANT_CUTOFF || adtp->Calculations[i] == AT_TYPE_INSTANT_REVERSE_CUTOFF || adtp->Calculations[i] == AT_TYPE_CAP || adtp->Calculations[i] == AT_TYPE_INSTANT_CAP || adtp->Calculations[i] == AT_TYPE_SET || adtp->Calculations[i] == AT_TYPE_RANDOM)) {
                                curr_arg = curr_arg * (flFrametime * 1000.0f) / float(BEAM_DAMAGE_TIME);
                        }
...
void ship_render_batch_thrusters(object *obj)
{
        ...
        for (int i = 0; i < sip->num_maneuvering; i++) {
                ...
                if (pi->desired_rotvel.xyz.x < 0 && (mtp->use_flags[Ship::Thruster_Flags::Pitch_up])) {
                        render_amount = fabsf(pi->desired_rotvel.xyz.x) / pi->max_rotvel.xyz.x;
                } else if (
                        pi->desired_rotvel.xyz.x > 0 && (mtp->use_flags[Ship::Thruster_Flags::Pitch_down])) {
                        render_amount = fabsf(pi->desired_rotvel.xyz.x) / pi->max_rotvel.xyz.x;
                } else if (
                        pi->desired_rotvel.xyz.y < 0 && (mtp->use_flags[Ship::Thruster_Flags::Roll_right])) {
                        render_amount = fabsf(pi->desired_rotvel.xyz.y) / pi->max_rotvel.xyz.y;
                } else if (
                        pi->desired_rotvel.xyz.y > 0 && (mtp->use_flags[Ship::Thruster_Flags::Roll_left])) {
                        render_amount = fabsf(pi->desired_rotvel.xyz.y) / pi->max_rotvel.xyz.y;
                } else if (
                        pi->desired_rotvel.xyz.z < 0 && (mtp->use_flags[Ship::Thruster_Flags::Bank_right])) {
                        render_amount = fabsf(pi->desired_rotvel.xyz.z) / pi->max_rotvel.xyz.z;
                } else if (
                        pi->desired_rotvel.xyz.z > 0 && (mtp->use_flags[Ship::Thruster_Flags::Bank_left])) {
                        render_amount = fabsf(pi->desired_rotvel.xyz.z) / pi->max_rotvel.xyz.z;
                }
                ...
                if (pi->side_thrust > 0 && (mtp->use_flags[Ship::Thruster_Flags::Slide_right])) {
                        ...
                        pi->side_thrust < 0 && (mtp->use_flags[Ship::Thruster_Flags::Slide_left])) {
                        render_amount = -pi->side_thrust;
                } else if (
                        pi->vert_thrust > 0 && (mtp->use_flags[Ship::Thruster_Flags::Slide_up])) {
                        render_amount = pi->vert_thrust;
                } else if (
                        pi->vert_thrust < 0 && (mtp->use_flags[Ship::Thruster_Flags::Slide_down])) {
                        render_amount = -pi->vert_thrust;
                } else if (
                        pi->forward_thrust > 0 && (mtp->use_flags[Ship::Thruster_Flags::Forward])) {
                        render_amount = pi->forward_thrust;
                } else if (
                        pi->forward_thrust < 0 && (mtp->use_flags[Ship::Thruster_Flags::Reverse])) {
                        render_amount = -pi->forward_thrust;
                }
                ...
                if (render_amount > 0.0f) {
                        ...
                        if (!Cmdline_freespace_no_sound) {
                                if (mtp->loop_snd.isValid() && shipp->thrusters_sounds[i] < 0 && (!mtp->start_snd.isValid() || (gamesnd_get_max_duration(gamesnd_get_game_sound(mtp->start_snd)) < timestamp() - shipp->thrusters_start[i]))) {
                        ...
                        if (mtp->tex_id >= 0) {
                                ...
                                if (mtp->tex_nframes > 0)
                                        bmap_frame += bm_get_anim_frame(
                                                mtp->tex_id,
                                                float(timestamp() - shipp->thrusters_start[i]) / 1000.0f,
                                                0.0f, true);
                ...
                if (Weapon_info[swp->secondary_bank_weapons[i]]
                        ...
                        for (k = 0; k < bank->num_slots; k++) {
                                ...
                                vm_vec_scale_add2(
                                        ...
                                        -(1.0f - shipp->secondary_point_reload_pct[i][k]) * model_get(Weapon_info[swp->secondary_bank_weapons[i]].external_model_num)->rad);
