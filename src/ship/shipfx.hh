...
extern void shipfx_blow_off_subsystem(
        object *ship_obj, ship *ship_p, ship_subsys *subsys, vec3d *exp_center,
        bool no_explosion = false);

// Creates "ndebris" pieces of debris on random verts of the the "submodel" in
// the ship's model.
extern void shipfx_blow_up_model(
...
void do_sub_expl_sound(
        float radius, vec3d *sound_pos, sound_handle *sound_handle);
...
void shipfx_start_cloak(
        ship *shipp, int warmup = 5000, int recalc_transform = 0, int device = 0);
...
private:
        ...
        int stage_time_end; // pops when ship is completely warped out or warped
                            // in.  Used for both warp in and out.
...
#endif // FREESPACE2_SHIP_SHIPFX_HH
