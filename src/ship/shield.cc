 ...
 *
 * Since the global shield triangle array needs the vertex information, we will
 * acutally store the information in this structure instead of the indices into
 * the vertex list
 */
...
void shield_hit_page_in () {
    ...
    if (!Shield_bitmaps_loaded) { load_shield_hit_bitmap (); }
 ...
 *
 * This doesn't need to do anything; the bitmap manager will release
 * everything.
 */
 ...
 *
 * @todo We should probably not bother releasing the shield hit bitmaps every
 * level.
 */
void shield_hit_close () { release_shield_hit_bitmap (); }
...
void create_low_detail_poly (
    int global_index, vec3d* tcp, vec3d* rightv, vec3d* upv) {
    float scale;
    gshield_tri* trip;
 ...
 */
void rs_compute_uvs (
    shield_tri* stp, shield_vertex* verts, vec3d* tcp, float /*radius*/,
    vec3d* rightv, vec3d* upv) {
    int i;
    shield_vertex* sv;
...
void shield_render_low_detail_bitmap (
    ...
    if (vm_vec_dot (&norm, &trip->verts[1].pos) < 0.0) {
        ...
        // vertlist[2] = &verts[1];
        // vertlist[3] = &verts[0];
        // g3_draw_poly( 4, vertlist, TMAP_FLAG_TEXTURED | TMAP_FLAG_RGB |
        // TMAP_FLAG_GOURAUD | TMAP_HTL_3D_UNLIT);
    }
    else {
        vertlist[0] = verts[0];
        vertlist[1] = verts[1];
    ...
    g3_render_primitives_colored_textured (
        &material_params, vertlist, 4, PRIM_TYPE_TRIFAN, false);
}
...
void shield_render_triangle (
    ...
    if (vm_vec_dot (&norm, &verts[1].world) >= 0.0) {
        ...
        g3_render_primitives_colored_textured (
            &material_params, vertlist, 3, PRIM_TYPE_TRIFAN, false);
    }
    else {
        g3_render_primitives_colored_textured (
            &material_params, verts, 3, PRIM_TYPE_TRIFAN, false);
    }
}

void shield_render_decal (
    polymodel* pm, matrix* orient, vec3d* pos, matrix* hit_orient,
    vec3d* hit_pos, float hit_radius, int bitmap_id, color* clr) {
    if (pm->shield.buffer_id < 0 || pm->shield.buffer_n_verts < 3) { return; }
    ...
    material_info.set_blend_mode (
        bm_has_alpha_channel (bitmap_id) ? ALPHA_BLEND_ALPHA_BLEND_ALPHA
    ...
    gr_render_shield_impact (
        &material_info, PRIM_TYPE_TRIS, &pm->shield.layout,
        pm->shield.buffer_id, pm->shield.buffer_n_verts);
...
void render_shield (int shield_num) {
    ...
    if (Poly_count > 50) {
        ...
        (Shield_hits[shield_num].start_time + SHIELD_HIT_DURATION) <
        Missiontime) {
        Shield_hits[shield_num].type = SH_UNUSED;
        free_global_tri_records (shield_num);
        return;
    ...
    if (sa->first_frame >= 0) {
        frame_num = bm_get_anim_frame (
            sa->first_frame,
            f2fl (Missiontime - Shield_hits[shield_num].start_time),
            f2fl (SHIELD_HIT_DURATION));
        ...
        if (bitmap_id <= -1) { return; }

        if ((Detail.shield_effects == 1) || (Detail.shield_effects == 2)) {
            shield_render_low_detail_bitmap (
                bitmap_id, alpha,
                &Global_tris[Shield_hits[shield_num].tri_list[0]], orient,
                centerp, Shield_hits[shield_num].rgb[0],
                Shield_hits[shield_num].rgb[1],
                Shield_hits[shield_num].rgb[2]);
        }
        else if (Detail.shield_effects < 4) {
            for (int i = 0; i < Shield_hits[shield_num].num_tris; i++) {
                shield_render_triangle (
                    bitmap_id, alpha,
                    &Global_tris[Shield_hits[shield_num].tri_list[i]], orient,
                    centerp, Shield_hits[shield_num].rgb[0],
                    Shield_hits[shield_num].rgb[1],
                    Shield_hits[shield_num].rgb[2]);
            }
            ...
            if (si->is_big_or_huge ()) { hit_radius = pm->core_radius * 0.5f; }
...
void render_shields () {
    ...
    for (i = 0; i < MAX_SHIELD_HITS; i++) {
        if (Shield_hits[i].type != SH_UNUSED) { render_shield (i); }
    }

    gr_clear_states ();
}

void create_tris_containing (
    vec3d* vp, matrix* orient, shield_info* shieldp, vec3d* tcp,
    vec3d* centerp, float radius, vec3d* rvec, vec3d* uvec) {
    int i, j;
    shield_vertex* verts;
    ...
    for (i = 0; i < Num_tris; i++) {
        if (!shieldp->tris[i].used) {
            for (j = 0; j < 3; j++) {
                ...
                if ((vp->xyz.x == v.xyz.x) && (vp->xyz.y == v.xyz.y) &&
                    (vp->xyz.z == v.xyz.z))
                    create_shield_from_triangle (
                        i, orient, shieldp, tcp, centerp, radius, rvec, uvec);
            }
        }
    }
}
...
void visit_children (
    int trinum, int vertex_index, matrix* orient, shield_info* shieldp,
    vec3d* tcp, vec3d* centerp, float radius, vec3d* rvec, vec3d* uvec) {
    shield_vertex* sv;

    sv = &(shieldp->verts[shieldp->tris[trinum].verts[vertex_index]]);

    if ((sv->u > 0.0f) && (sv->u < UV_MAX) && (sv->v > 0.0f) &&
        (sv->v < UV_MAX))
        create_tris_containing (
            &sv->pos, orient, shieldp, tcp, centerp, radius, rvec, uvec);
}
...
int get_free_global_shield_index () {
    ...
    while (
        (gi < MAX_SHIELD_TRI_BUFFER) && (Global_tris[gi].used) &&
...
void create_shield_from_triangle (
    int trinum, matrix* orient, shield_info* shieldp, vec3d* tcp,
    vec3d* centerp, float radius, vec3d* rvec, vec3d* uvec) {
    rs_compute_uvs (
        &shieldp->tris[trinum], shieldp->verts, tcp, radius, rvec, uvec);

    shieldp->tris[trinum].used = 1;

    visit_children (
        trinum, 0, orient, shieldp, tcp, centerp, radius, rvec, uvec);
    visit_children (
        trinum, 1, orient, shieldp, tcp, centerp, radius, rvec, uvec);
    visit_children (
        trinum, 2, orient, shieldp, tcp, centerp, radius, rvec, uvec);
}
 ...
 * We need to store vertex information in the global array since the vertex
 * list will not be available to us when we actually use the array.
 */
void copy_shield_to_globals (
    ...
    for (i = 0; i < shieldp->ntris; i++) {
        if (shieldp->tris[i].used) {
            while ((gi < MAX_SHIELD_TRI_BUFFER) && (Global_tris[gi].used) &&
                   (Global_tris[gi].creation_time + SHIELD_HIT_DURATION >
                    Missiontime)) {
                gi++;
            }
            ...
            // copy the pos/u/v elements of the shield_vertex structure into
            // the shield vertex structure for this global triangle.
            for (j = 0; j < 3; j++)
                Global_tris[gi].verts[j] =
                    shieldp->verts[shieldp->tris[i].verts[j]];
    ...
    if ((objnum >= 0) && (objnum < MAX_OBJECTS) &&
        ...
        (Ships[Objects[objnum].instance].ship_info_index <
...
/**
 * This function needs to be called by big ships which have shields. It should
 * be able to be modified to deal with the large polygons we use for their
 * shield meshes - unknownplayer
 *
 * At lower detail levels, shield hit effects are a single texture, applied to
 * one enlarged triangle.
 */
void create_shield_low_detail (
    int objnum, int /*model_num*/, matrix* /*orient*/, vec3d* /*centerp*/,
    vec3d* tcp, int tr0, shield_info* shieldp) {
    matrix tom;
    int gi;
    int shnum;
...
void create_shield_explosion (
    ...
    if (Objects[objnum].flags[Object::Object_Flags::No_shields]) return;
    ...
    if ((Detail.shield_effects == 1) || (Detail.shield_effects == 2)) {
        create_shield_low_detail (
            objnum, model_num, orient, centerp, tcp, tr0, shieldp);
        return;
    }
...
void create_shield_explosion_all (object* objp) {
    ...
    if (Detail.shield_effects == 0) { return; }
    ...
    for (i = 0; i < Num_shield_points; i++) {
        if (Shield_points[i].objnum == objnum) {
            create_shield_explosion (
                objnum, Ship_info[shipp->ship_info_index].model_num,
                &objp->orient, &objp->pos, &Shield_points[i].hit_point,
    ...
    for (i = 0; i < pm->shield.ntris; i++) {
        ...
        if (vm_vec_dot (&gnorm, &v2f) < 0.0f) {
            ...
            for (j = 0; j < 3; j++) {
                ...
                // Rotate point into world coordinates
                vm_vec_unrotate (
                    &pnt, &pm->shield.verts[tri->verts[j]].pos, &objp->orient);
...
/**
 * Returns true if the shield presents any opposition to something trying to
 * force through it.
 *
 * @return If quadrant is -1, looks at entire shield, otherwise just one
 * quadrant
 */
int ship_is_shield_up (object* obj, int quadrant) {
    if ((quadrant >= 0) && (quadrant < obj->n_quadrants)) {
        // Just check one quadrant
        if (obj->shield_quadrant[quadrant] >
            MAX (2.0f, 0.1f * shield_get_max_quad (obj))) {
            return 1;
        }
    }
    else {
        ...
        if (strength >
            MAX (2.0f * 4.0f, 0.1f * shield_get_max_strength (obj))) {
            return 1;
        }
    }
...
int get_quadrant (vec3d* hit_pnt, object* shipobjp) {
    if (shipobjp != NULL &&
        ...
        for (unsigned int i = 0;
             i < Ships[shipobjp->instance].shield_points.size (); i++) {
            float dist = vm_vec_dist (
                hit_pnt, &Ships[shipobjp->instance].shield_points.at (i));
        ...
        if (hit_pnt->xyz.x < hit_pnt->xyz.z) result |= 1;

        if (hit_pnt->xyz.x < -hit_pnt->xyz.z) result |= 2;
