...
#define MAX_SHIELD_TRI_BUFFER                                                               \
        (MAX_SHIELD_HITS * MAX_TRIS_PER_HIT) //(MAX_SHIELD_HITS*20) // Persistent buffer of
...
void render_shield(int shield_num)
{
        ...
        if (Poly_count > 50) {
                ...
                (Shield_hits[shield_num].start_time + SHIELD_HIT_DURATION) < Missiontime) {
                Shield_hits[shield_num].type = SH_UNUSED;
                free_global_tri_records(shield_num);
                return;
        }
...
int get_free_global_shield_index()
{
        ...
        while (
                (gi < MAX_SHIELD_TRI_BUFFER) && (Global_tris[gi].used) && (Global_tris[gi].creation_time + SHIELD_HIT_DURATION > Missiontime)) {
                gi++;
        }
