...
#ifndef FREESPACE2_SHIP_SHIP_H
#define FREESPACE2_SHIP_SHIP_H
...
#define WEAPON_RESERVE_THRESHOLD \
    0.01f // energy threshold where ship is considered to have no weapon energy
          // system
#define SUBSYS_MAX_HITS_THRESHOLD \
    0.01f // max_hits threshold where subsys is considered to take damage

#define HP_SCALE 1.2    //	1.2 means die when 20% of hits remaining
#define MAXFREESPACE2_SHIP_SHIP_HITS 8 // hits to kill a ship
...
typedef struct {
    ...
    int flags;
    char no_messages[MAX_REINFORCEMENT_MESSAGES]
                    [NAME_LENGTH]; // list of messages to possibly send when
                                   // calling for reinforcement not available
...
public:
    ...
    int primary_bank_weapons[MAX_SHIP_PRIMARY_BANKS]; // Weapon_info[] index
                                                      // for the weapon in the
                                                      // bank
    ...
    int next_primary_fire_stamp
        [MAX_SHIP_PRIMARY_BANKS]; // next time this primary bank can fire
    int last_primary_fire_stamp
        [MAX_SHIP_PRIMARY_BANKS]; // last time this primary bank fired (mostly
                                  // used by SEXPs)
    int next_secondary_fire_stamp
        [MAX_SHIP_SECONDARY_BANKS]; // next time this secondary bank can fire
    int last_secondary_fire_stamp
        [MAX_SHIP_SECONDARY_BANKS]; // last time this secondary bank fired
                                    // (mostly used by SEXPs)
    int next_tertiary_fire_stamp;
    int last_primary_fire_sound_stamp
        [MAX_SHIP_PRIMARY_BANKS]; // trailing end of the last time this primary
                                  // bank was fired, for purposes of timing the
                                  // pre-launch sound

    // ballistic primary support - by Goober5000
    int primary_bank_ammo[MAX_SHIP_PRIMARY_BANKS]; // Number of missiles left
                                                   // in primary bank
    int primary_bank_start_ammo[MAX_SHIP_PRIMARY_BANKS]; // Number of missiles
                                                         // starting in primary
                                                         // bank
    int primary_bank_capacity[MAX_SHIP_PRIMARY_BANKS];   // Max number of
                                                         // projectiles in bank
    int primary_next_slot[MAX_SHIP_PRIMARY_BANKS]; // Next slot to fire in the
                                                   // bank
    int primary_bank_rearm_time
        [MAX_SHIP_PRIMARY_BANKS]; // timestamp which indicates when bank can
                                  // get new projectile
    // end ballistic primary support

    float primary_bank_fof_cooldown
        [MAX_SHIP_PRIMARY_BANKS]; // SUSHI: Current FOF cooldown level for the
                                  // primary weapon

    // dynamic weapon linking - by RSAXVC
    int primary_bank_slot_count[MAX_SHIP_PRIMARY_BANKS]; // Fire this many
                                                         // slots at a time
    // end dynamic weapon linking

    int secondary_bank_ammo[MAX_SHIP_SECONDARY_BANKS]; // Number of missiles
                                                       // left in secondary
                                                       // bank
    int secondary_bank_start_ammo
        [MAX_SHIP_SECONDARY_BANKS]; // Number of missiles starting in secondary
                                    // bank
    int secondary_bank_capacity[MAX_SHIP_SECONDARY_BANKS]; // Max number of
                                                           // missiles in bank
    int secondary_next_slot[MAX_SHIP_SECONDARY_BANKS]; // Next slot to fire in
                                                       // the bank
    int secondary_bank_rearm_time
        [MAX_SHIP_SECONDARY_BANKS]; // timestamp which indicates when bank can
                                    // get new missile
    ...
    int tertiary_bank_rearm_time; // timestamp which indicates when bank can
                                  // get new something (used for ammopod or
                                  // boostpod)
    ...
    EModelAnimationPosition
        secondary_animation_position[MAX_SHIP_SECONDARY_BANKS];
...
#define ADT_DIFF_SCALE_MANUAL \
    2 // this is the user defined mode where the modder has to handle
      // difficulty scaling in their calculations

// Nuke: +value: replacing constants
// these are stored as altArguments, positive values mean storage idxes and -1
// means not used, anything below that is fair game
...
#define AT_CONSTANT_DIFF_FACTOR \
    -5 // difficulty factor (by default 0.2 (easy) to 1.0 (insane))
#define AT_CONSTANT_RANDOM \
    -6 // number between 0 and 1 (redundant but saves a calculation)
...
private:
    ...
    std::vector< int >
        altArguments; // Nuke: to facilitate optional importation of data in
                      // place of +value: tag -nuke
...
#define SAF_IGNORE_SS_ARMOR \
    (1 << 0) // hull armor is applied regardless of the subsystem armor for
             // hull damage
...
// obj_subsystem data is defined and read in the model code.  Other dynamic
// data (such as current_hits) should remain in this structure.
class ship_subsys {
public:
    class ship_subsys *next,
        *prev; //	Index of next and previous objects in list.
    model_subsystem* system_info; // pointer to static data for this subsystem
                                  // -- see model.h for definition
    ...
    // turret info
    // Important -WMC
    // With the new turret code, indexes run from 0 to MAX_SHIP_WEAPONS; a
    // value of MAX_SHIP_PRIMARY_WEAPONS or higher, an index into the turret
    // weapons is considered to be an index into the secondary weapons for much
    // of the code. See turret_next_weap_fire_stamp.
    ...
    // There are 2 of these because turrets need one for the turret and one for
    // the barrel. Things like radar dishes would only use one.
    submodel_instance_info
        submodel_info_1; // Instance data for main turret or main object
    submodel_instance_info
        submodel_info_2; // Instance data for turret guns, if there is one
    ...
    int triggered_rotation_index; // the actual currently running animation and
                                  // assosiated states
    ...
    ship_subsys () : next (NULL), prev (NULL) {}

    void clear ();
};

// structure for subsystems which tells us the total count of a particular type
...
#define MAX_SHIP_ARCS \
    2 // How many "arcs" can be active at once... Must be less than
      // MAX_ARC_EFFECTS in model.h.
...
public:
    ...
    ubyte pre_death_explosion_happened; // If set, it means the 4 or 5 smaller
                                        // explosions
    ubyte wash_killed;
    char cargo1;
    ...
    char targeting_laser_bank;  // -1 if not firing, index into polymodel gun
                                // points if it _is_ firing
    ...
    ship_spark sparks[MAXFREESPACE2_SHIP_SHIP_HITS];
    ...
    int ship_guardian_threshold; // Goober5000 - now also determines whether
                                 // ship is guardian'd
    ...
    int arrival_anchor;   // name of object this ship arrives near (or in front
                          // of)
    int arrival_path_mask; // Goober5000 - possible restrictions on which bay
                           // paths to use
    int arrival_cue;
    int arrival_delay;

    int departure_location;  // depart to hyperspace or someplace else (like
                             // docking bay)
    int departure_anchor;    // when docking bay -- index of ship to use
    int departure_path_mask; // Goober5000 - possible restrictions on which bay
                             // paths to use
    int departure_cue;       // sexpression to eval when departing
    int departure_delay; // time in seconds after sexp is true that we delay.

    int wingnum; // wing number this ship is in.  -1 if in no wing, Wing array
                 // index otherwise
    int orders_accepted; // set of orders this ship will accept from the
                         // player.

    // Subsystem fields.  The subsys_list is a list of all subsystems (which
    // might include multiple types of a particular subsystem, like engines).
    // The subsys_info struct is information for particular types of
    // subsystems.  (i.e. the list might contain 3 engines.  There will be one
    // subsys_info entry describing the state of all engines combined) -- MWA
    // 4/1/97
    ship_subsys subsys_list; //	linked list of subsystems for this ship.
    ship_subsys*
        last_targeted_subobject[MAX_PLAYERS]; // Last subobject that has been
                                              // targeted.  NULL if
                                              // none;(player specific)
    ship_subsys_info subsys_info[SUBSYSTEM_MAX]; // info on particular generic
                                                 // types of subsystems

    float* shield_integrity; //	Integrity at each triangle in shield mesh.

    // ETS fields
    int shield_recharge_index; // index into array holding the shield recharge
                               // rate
    int weapon_recharge_index; // index into array holding the weapon recharge
                               // rate
    int engine_recharge_index; // index into array holding the engine recharge
                               // rate
    float weapon_energy;       // Number of EUs in energy reserves
    float current_max_speed;   // Max ship speed (based on energy diverted to
                               // engines)
    int next_manage_ets; // timestamp for when ai can next modify ets ( -1
                         // means never )
    ...
    float
        afterburner_fuel; // amount of afterburner fuel remaining (capacity is
    ...
    int num_swarm_missiles_to_fire; // number of swarm missiles that need to be
                                    // launched
    ...
    int num_turret_swarm_info; // number of turrets in process of launching
                               // swarm
    int swarm_missile_bank;    // The missilebank the swarm was originally
                               // launched from

    int group; // group ship is in, or -1 if none.  Fred thing
    sound_handle
        death_roll_snd; // id of death roll sound, may need to be stopped early
    int ship_list_index; // index of ship in Ship_objs[] array

    int thruster_bitmap;  // What frame the current thruster bitmap is at for
                          // this ship
    float thruster_frame; // Used to keep track of which frame the animation
                          // should be on.

    int thruster_glow_bitmap;  // What frame the current thruster engine glow
                               // bitmap is at for this ship
    float thruster_glow_frame; // Used to keep track of which frame the engine
    ...
    int next_engine_stutter; // timestamp to time the engine stuttering when a
                             // ship dies

    fix base_texture_anim_frametime; // Goober5000 - zero mark for texture
                                     // animations

    float
        total_damage_received; // total damage received (for scoring purposes)
    float damage_ship[MAX_DAMAGE_SLOTS];  // damage applied from each player
    int damage_ship_id[MAX_DAMAGE_SLOTS]; // signature of the damager
                                          // (corresponds to each entry in
                                          // damage_ship)
    int persona_index;                    // which persona is this guy.

    int subsys_disrupted_flags; // bitflags used to check if SUBYSTEM_* is
                                // disrupted or not
    int subsys_disrupted_check_timestamp; // timer to control how oftern flags
                                          // are set/cleared in
                                          // subsys_disrupted_flags

    uint create_time; // time ship was created, set by gettime()

    // keep multiplayer specific stuff below this point
    int ts_index; // index into the team select and Wss_slots array (or -1 if
                  // not in one of those arrays)

    int large_ship_blowup_index; // -1 if not a large ship exploding, else this
                                 // is an index used by the shipfx large ship
                                 // exploding code.
    ...
    int arc_timestamp[MAX_SHIP_ARCS]; // When this times out, the spark goes
                                      // away.  -1 is not used
    ...
    float
        emp_decr; // how much to decrement EMP effect per second for this ship
    ...
    ship_subsys fighter_beam_turret_data; // a fake subsystem that pretends to
                                          // be a turret for fighter beams
    ...
    // apparently, early in FS1 development, there was a field called
    // current_eye_index that had this same functionality
    ...
    int bay_doors_anim_done_time; // ammount of time to transition from one
                                  // animation state to another
    EModelAnimationPosition bay_doors_status; // anim status of the bay doors
                                              // (closed/not-animating,
                                              // opening, open/not-animating)
    ...
    bool bay_doors_need_open; // keep track of whether I need the door open or
                              // not
    int bay_doors_parent_shipnum; // our parent ship, what we are
                                  // entering/leaving

    float secondary_point_reload_pct
        [MAX_SHIP_SECONDARY_BANKS]
        [MAX_SLOTS]; // after fireing a secondary it takes some time for that
                     // secondary weapon to reload, this is how far along in
                     // that proces it is (from 0 to 1)
    ...
    int ammo_low_complaint_count; // number of times this ship has complained
                                  // about low ammo
    int armor_type_idx;
    int shield_armor_type_idx;
    int collision_damage_type_idx;
    int debris_damage_type_idx;
    ushort debris_net_sig; // net signiture of the first piece of debris this
                           // ship has
    ...
    fix radar_visible_since; // The first time this ship was visible on the
                             // radar. Gets reset when ship is not visible
                             // anymore
    fix radar_last_contact;  // The last time this ship appeared on the radar.
                             // When it is currently visible this has the value
                             // if Missiontime
    ...
    std::string
        secondary_team_name; // If the change-team-color sexp is used, these
                             // fields control the fading behaviour
    ...
    inline bool is_dying_or_departing () {
        return is_departing () || flags[Ship::Ship_Flags::Dying];
    }
...
typedef struct exited_ship {
    ...
    char cargo1;
    float damage_ship[MAX_DAMAGE_SLOTS]; // A copy of the arrays from the ship
...
typedef struct ship_type_info {
    ...
    ship_type_info ()
        : debris_max_speed (0.f), ff_multiplier (0.f), emp_multiplier (0.f),
          fog_start_dist (0.f), fog_complete_dist (0.f), ai_valid_goals (0),
          ai_player_orders (0), ai_active_dock (0), ai_passive_dock (0),
          vaporize_chance (0.f)
...
typedef struct ship_collision_physics {
    ...
    // Note that all angles are stored as a dotproduct between normalized
    // vectors instead. This saves us from having to do a lot of dot product
    // calculations later.
    float landing_max_z{};
    float landing_min_z{};
    float landing_min_y{};
    ...
    // Speed & angle constraints for a "rough" landing (one with normal
    // collision consequences, but where the ship is still reoriented towards
    // its resting orientation)
    ...
    float reorient_mult{}; // How quickly the ship will reorient towards it's
                           // resting position
    float landing_rest_angle{}; // The vertical angle where the ship's
                                // orientation comes to rest
    gamesnd_id
...
public:
    ...
    char pof_file_hud[MAX_FILENAME_LEN]; // POF file to load for the HUD target
                                         // box
    ...
    int detail_distance[MAX_SHIP_DETAIL_LEVELS]; // distance to change detail
                                                 // levels at
    int collision_lod;     // check for collisions using a LOD
    int cockpit_model_num; // cockpit model
    int model_num;         // ship model
    int model_num_hud;     // model to use when rendering to the HUD (eg, mini
                           // supercap)
    int hud_target_lod;    // LOD to use for rendering to the HUD targetbox (if
                           // not already using special HUD model)
    ...
    vec3d rotation_time;  // time to rotate in x/y/z dimension traveling at max
                          // rotvel
    float srotation_time; //	scalar, computed at runtime as (rotation_time.x
                          //+ rotation_time.y)/2
    ...
    float max_overclocked_speed; // max speed when 100% power output sent to
                                 // engines
    float max_weapon_reserve; // maximum energy that can be stored for primary
                              // weapon usage
    float max_shield_regen_per_second; // Goober5000 - max percent/100 of
                                       // shield energy regenerated per second
    float max_weapon_regen_per_second; // Goober5000 - max percent/100 of
                                       // weapon energy regenerated per second
    ...
    int primary_bank_ammo_capacity
    ...
    int secondary_bank_weapons[MAX_SHIP_SECONDARY_BANKS]; // Weapon_info[]
                                                          // index for the
                                                          // weapon in the bank
    int secondary_bank_ammo_capacity
        [MAX_SHIP_SECONDARY_BANKS]; // Capacity of bank (not number of
                                    // missiles)
    ...
    bool auto_shield_spread_bypass;    // Whether weapons fired up close can
                                       // bypass shields
    int auto_shield_spread_from_lod;   // Which LOD to project the shield from
    float auto_shield_spread_min_span; // Minimum distance weapons must travel
                                       // until allowed to collide with the
                                       // shield

    int shield_point_augment_ctrls[4]; // Re-mapping of shield augmentation
                                       // controls for model point shields
    ...
    vec3d closeup_pos;  // position for camera when using ship in closeup view
                        // (eg briefing and hud target monitor)
    float closeup_zoom; // zoom when using ship in closeup view (eg briefing
                        // and hud target monitor)

    int allowed_weapons[MAX_WEAPON_TYPES]; // array which specifies which
                                           // weapons can be loaded out by the
    ...
    ubyte shield_icon_index; // index to locate ship-specific animation frames
    ...
    char anim_filename[MAX_FILENAME_LEN]; // filename for animation that plays
                                          // in ship selection
    char overhead_filename[MAX_FILENAME_LEN]; // filename for animation that
                                              // plays weapons loadout
    int selection_effect;

    int bii_index_ship; // if this ship has a briefing icon that overrides the
                        // normal icon set
    ...
    float scan_range_normal;  // this ship can scan other normal/small ships at
                              // this range
    float scan_range_capital; // this ship can scan other capital/large ships
                              // at this range
    ...
    gamesnd_id
        engine_snd; // handle to engine sound for ship (-1 if no engine sound)
    float min_engine_vol; // minimum volume modifier for engine sound when ship
                          // is stationary
    gamesnd_id glide_start_snd; // handle to sound to play at the beginning of
                                // a glide maneuver (default is 0 for regular
                                // throttle down sound)
    gamesnd_id
        glide_end_snd; // handle to sound to play at the end of a glide
                       // maneuver (default is 0 for regular throttle up sound)

    std::map< GameSounds, gamesnd_id >
        ship_sounds; // specifies ship-specific sound indexes
    ...
    inline bool is_small_ship () const {
        return flags
            [Ship::Info_Flags::Fighter, Ship::Info_Flags::Bomber,
             Ship::Info_Flags::Support, Ship::Info_Flags::Escapepod];
    }
    inline bool is_big_ship () const {
        return flags
            [Ship::Info_Flags::Cruiser, Ship::Info_Flags::Freighter,
             Ship::Info_Flags::Transport, Ship::Info_Flags::Corvette,
             Ship::Info_Flags::Gas_miner, Ship::Info_Flags::Awacs];
    }
    inline bool is_huge_ship () const {
        return flags
            [Ship::Info_Flags::Capital, Ship::Info_Flags::Supercap,
             Ship::Info_Flags::Drydock, Ship::Info_Flags::Knossos_device];
    }
    inline bool is_flyable () const {
        return !(flags
                     [Ship::Info_Flags::Cargo, Ship::Info_Flags::Navbuoy,
                      Ship::Info_Flags::Escapepod]);
    }
    inline bool is_harmless () const { return !is_flyable (); }
    inline bool is_fighter_bomber () const {
        return flags[Ship::Info_Flags::Fighter, Ship::Info_Flags::Bomber];
    }
    inline bool is_big_or_huge () const {
        return is_big_ship () || is_huge_ship ();
    }
...
typedef struct wing {
    ...
    int threshold; // when number of ships in the wing reaches this number --
                   // new wave

    fix time_gone; // time into the mission when this wing is officially gone.

    int wave_count; // max ships per wave (as defined by the number of ships in
                    // the ships list)
    int total_arrived_count; // count of number of ships that we have created,
                             // regardless of wave
    int red_alert_skipped_ships; // Goober5000 - if we skipped over any indexes
                                 // while creating red-alert ships
    int current_count; // count of number of ships actually in this wing --
                       // used for limit in next array
    int ship_index[MAX_SHIPS_PER_WING]; // index into ships array of all ships
                                        // currently in the wing

    int total_destroyed; // total number of ships destroyed in the wing
                         // (including all waves)
    int total_departed;  // total number of ships departed in this wing
                         // (including all waves)
    int total_vanished;  // total number of ships vanished in this wing
                         // (including all waves)

    int special_ship; // the leader of the wing.  An index into ship_index[].

    int arrival_location; // arrival and departure information for wings --
                          // similar to info for ships
    ...
    int departure_anchor; // name of object that we depart to (in case of dock
                          // bays)
    int departure_path_mask; // Goober5000 - possible restrictions on which bay
                             // paths to use
    int departure_cue;
    ...
    int wave_delay_timestamp; // timestamp used for delaying arrival of next
                              // wave

    flagset< Ship::Wing_Flags > flags;

    ai_goal ai_goals[MAX_AI_GOALS]; // goals for the wing -- converted to
                                    // ai_goal struct

    ushort net_signature; // starting net signature for ships in this wing.
                          // assiged at mission load time

    // Goober5000 - if this wing has a unique squad logo
    // it's specified for the wing rather than each individual ship to cut down
    // on the amount of stuff that needs to be sitting in memory at once - each
...
// structure definition for ship type counts.  Used to give a count of the
// number of ships of a particular type, and the number of times that a ship of
...
// Use the below macros when you want to find the index of an array element in
// the Wings[] or Ships[] arrays.
...
// returns -1 if failed
extern int ship_create (
    matrix* orient, vec3d* pos, int ship_type, char* ship_name = NULL);
...
extern int
ship_check_collision_fast (object* obj, object* other_obj, vec3d* hitpos);
 ...
 * @param[in] cleanup_mode Flags describing how this ship is to be removed. See
 * SHIP_VANISHED, SHIP_DESTROYED, etc.
 *
 * @details This is the deconstructor of a ship, it does all the necassary
 * processes to remove the ship from the Ships array, and frees the slot for
 * use by others. De-init of its Objects[] slot is handled by
...
extern void
ship_actually_depart (int shipnum, int method = SHIP_DEPARTED_WARP);
...
// Goober5000
extern int
get_available_primary_weapons (object* objp, int* outlist, int* outbanklist);

extern int
get_available_secondary_weapons (object* objp, int* outlist, int* outbanklist);
...
extern int ship_name_lookup (
    const char* name,
    int inc_players = 0); // returns the index into Ship array of name
...
// next function takes optional second parameter which says to ignore the
// current count of ships in the wing -- used to tell is the wing exists or
// not, not whether it exists and has ships currently present.
...
// correctly handles the # character
extern void
wing_bash_ship_name (char* ship_name, const char* wing_name, int index);
...
extern ship_subsys* ship_get_indexed_subsys (
    ship* sp, int index,
    vec3d* attacker_pos = NULL); // returns index'th subsystem of this ship
extern int ship_get_index_from_subsys (
    ship_subsys* ssp, int objnum, int error_bypass = 0);
extern int ship_get_subsys_index (
    ship* sp, const char* ss_name,
    int error_bypass =
        0); // returns numerical index in linked list of subsystems
...
extern ship_subsys* ship_get_closest_subsys_in_sight (
...
extern int
ship_find_repair_ship (object* requester_obj, object** ship_we_found = NULL);
...
extern int get_quadrant (
    vec3d* hit_pnt,
    object* shipobjp = NULL); //	Return quadrant num of given hit point.

extern void ship_obj_list_rebuild (); // only called by save/restore code
extern int ship_query_state (char* name);

// Goober5000
int ship_primary_bank_has_ammo (
    int shipnum); // check if current primary bank has ammo
int ship_secondary_bank_has_ammo (
    int shipnum); // check if current secondary bank has ammo

int ship_engine_ok_to_warp (
    ship* sp); // check if ship has engine power to warp
int ship_navigation_ok_to_warp (
    ship* sp); // check if ship has navigation power to warp

int ship_return_subsys_path_normal (
    ship* sp, ship_subsys* ss, vec3d* gsubpos, vec3d* norm);
int ship_subsystem_in_sight (
    object* objp, ship_subsys* subsys, vec3d* eye_pos, vec3d* subsys_pos,
    int do_facing_check = 1, float* dot_out = NULL, vec3d* vec_out = NULL);
ship_subsys*
ship_return_next_subsys (ship* shipp, int type, vec3d* attacker_pos);

// defines and definition for function to get a random ship of a particular
// team (any ship, any ship but player ships, or only players)
#define SHIP_GET_ANY_SHIP 0
#define SHIP_GET_NO_PLAYERS 1
#define SHIP_GET_ONLY_PLAYERS 2
#define SHIP_GET_UNSILENCED \
    3 // Karajorma - Returns no_players that can send builtin messages.

extern int ship_get_random_team_ship (
    int team_mask, int flags = SHIP_GET_ANY_SHIP, float max_dist = 0.0f);
extern int ship_get_random_player_wing_ship (
    int flags = SHIP_GET_ANY_SHIP, float max_dist = 0.0f,
    int persona_index = -1, int get_first = 0, int multi_team = -1);
extern int ship_get_random_ship_in_wing (
    int wingnum, int flags = SHIP_GET_ANY_SHIP, float max_dist = 0.0f,
    int get_first = 0);
...
extern int
    Ship_auto_repair; // flag to indicate auto-repair of subsystem should occur
#endif
...
extern void ship_do_cap_subsys_cargo_revealed (
    ship* shipp, ship_subsys* subsys, int from_network = 0);
extern void ship_do_cap_subsys_cargo_hidden (
    ship* shipp, ship_subsys* subsys, int from_network = 0);
...
// of the gun and its normal.   This uses the current turret angles.  We are
// keeping track of which gun to fire next in the ship specific info for this
// turret subobject.  Use this info to determine which position to fire from
// next.
//	Stuffs:
...
bool turret_std_fov_test (
    ship_subsys* ss, vec3d* gvec, vec3d* v2e, float size_mod = 0);
...
void object_jettison_cargo (
    object* objp, object* cargo_objp, float jettison_speed, bool jettison_new);

// get damage done by exploding ship, takes into account mods for individual
// ship
...
extern int ship_explode_area_calc_damage (
    vec3d* pos1, vec3d* pos2, float inner_rad, float outer_rad,
    float max_damage, float max_blast, float* damage, float* blast);

// determine turret status of a given subsystem, returns 0 for no turret, 1 for
// "fixed turret", 2 for "rotating" turret
...
extern void ship_subsystem_set_new_ai_class (
...
// Goober5000 - handles submodel rotation, incorporating conditions such as gun
// barrels when firing
extern void ship_do_submodel_rotation (
 ...
 *  @param id A sound id as defined in gamsesnd.h. If the given id is unknown
 * then the game_snd with the id as index is returned.
 *
 *  @return An index into the Snds vector, if the specified index could not be
 * found then the id itself will be returned
 */
