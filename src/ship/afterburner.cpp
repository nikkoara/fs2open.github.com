...
// ----------------------------------------------------------
static sound_handle
    Player_afterburner_loop_id; // identifies the looping afterburner sound of
                                // the player ship
static int Player_afterburner_loop_delay; // timestamp used to time the start
                                          // of the looping afterburner sound
 ...
 * This function should only be called once when afterburners first start. This
 * is to start an appropriate sound effect and do any one-time initializations.
 *
...
void afterburners_start (object* objp) {
    ...
    if (shipp->flags[Ship::Ship_Flags::Afterburner_locked]) { return; }
    ...
    if ((objp->flags[Object::Object_Flags::Player_ship]) &&
        ...
        if (objp->phys_info.flags & PF_AFTERBURNER_WAIT) { return; }
    }
    ...
    if (objp->phys_info.flags & PF_BOOSTER_ON) return;
    ...
    model_anim_start_type (
        shipp, TRIGGER_TYPE_AFTERBURNER, ANIMATION_SUBTYPE_ALL, 1);
        ...
        if (!timestamp_elapsed (Player_disengage_timer)) {
            ...
            if (remaining <= 0) { afterburner_stop_sounds (); }
                ...
                snd_set_volume (
                    ...
                    remaining * Player_afterburner_vol);
            }
        }
        else {
            if (Player_disengage_timer != 1) { afterburner_stop_sounds (); }
        }
    }
    ...
    if (objp == Player_obj) {
        if (timestamp_elapsed (Player_afterburner_loop_delay)) {
            ...
            if (!Player_afterburner_loop_id.isValid ()) {
                Player_afterburner_loop_id = snd_play_looping (
                    gamesnd_get_game_sound (
                        ship_get_sound (objp, GameSounds::ABURN_LOOP)),
                    0.0f, -1, -1);
                snd_set_volume (
                    Player_afterburner_loop_id, Player_afterburner_vol);
            }
        }
        ...
        if (timestamp_elapsed (volume_chg_timer)) {
            ...
            if (percent_afterburner_left < AFTERBURNER_PERCENT_VOL_ATTENUATE) {
                Player_afterburner_vol =
                    percent_afterburner_left *
                    (1 / AFTERBURNER_PERCENT_VOL_ATTENUATE) *
                    AFTERBURNER_DEFAULT_VOL;
                snd_set_volume (
                    Player_afterburner_loop_id, Player_afterburner_vol);
            }
...
void afterburners_stop (object* objp, int key_released) {
    ...
    if (!(sip->flags[Ship::Info_Flags::Afterburner])) {
        nprintf (
            ("Warning", "Ship type %s does not have afterburner capability\n",
             sip->name));
        return;
    }
