...
#ifndef FREESPACE2_SHIP_SHIP_FLAGS_HH
#define FREESPACE2_SHIP_SHIP_FLAGS_HH
...
FLAG_LIST(Subsystem_Flags){
        ...
        Vanished,                // allows subsystem to be made to disappear without a trace (for
                                 // swapping it for a true model for example.
        Missiles_ignore_if_dead, // forces homing missiles to target hull if
                                 // subsystem is dead before missile hits it.
        Rotates,
        Damage_as_hull,            // Applies armor damage instead of subsystem damge. - FUBAR
        No_aggregate,              // exclude this subsystem from the aggregate subsystem-info
                                   // tracking - Goober5000
        Play_sound_for_player,     // If this subsystem is a turret on a player ship,
                                   // play firing sounds - The E
        No_disappear,              // prevents submodel from disappearing when subsys destroyed
        Autorepair_if_disabled,    // Allows the subsystem to repair itself even when
                                   // disabled - MageKing17
        No_autorepair_if_disabled, // Inversion of the above; allow a specific
                                   // subsystem not to repair itself after being
...
FLAG_LIST(Ship_Flags){
        ...
        Arriving_stage_1,           // ship is arriving. In other words, doing warp in
                                    // effect, stage 1
        Arriving_stage_2,           // ship is arriving. In other words, doing warp in
                                    // effect, stage 2
        Engines_on,                 // engines sound should play if set
        Dock_leader,                // Goober5000 - this guy is in charge of everybody he's docked
                                    // to
        Cargo_revealed,             // ship's cargo is revealed to all friendly ships
        From_player_wing,           // set for ships that are members of any player starting
                                    // wing
        Primary_linked,             // ships primary weapons are linked together
        Secondary_dual_fire,        // ship is firing two missiles from the current
                                    // secondary bank
        ...
        Ammo_count_recorded,        // we've recorded the initial secondary weapon count
                                    // (which is used to limit support ship rearming)
        Hidden_from_sensors,        // ship doesn't show up on sensors, blinks in/out on
                                    // radar
        Scannable,                  // ship is "scannable".  Play scan effect and report as
                                    // "Scanned" or "not scanned".
        Warped_support,             // set when this is a support ship which was warped in
                                    // automatically
        Exploded,                   // ship has exploded (needed for kill messages)
        Ship_has_screamed,          // ship has let out a death scream
        Red_alert_store_status,     // ship status should be stored/restored if red
                                    // alert mission
        Vaporize,                   // ship is vaporized by beam - alternative death sequence
        Departure_ordered,          // departure of this ship was ordered by player -
                                    // Goober5000, similar to WF_DEPARTURE_ORDERED
        Primitive_sensors,          // Goober5000 - primitive sensor display
        Friendly_stealth_invis,     // Goober5000 - when stealth, don't appear on radar
                                    // even if friendly
        Stealth,                    // Goober5000 - is this particular ship stealth
        Dont_collide_invis,         // Goober5000 - is this particular ship
        ...
        Toggle_subsystem_scanning,  // Goober5000 - switch whether subsystems are
                                    // scanned
        ...
        Set_class_dynamically,      // Karajorma - This ship should have its class
                                    ...
                                    // file
        Lock_all_turrets_initially, // Karajorma - Lock all turrets on this ship at
                                    // mission start or on arrival
        Force_shields_on,
        ...
        Ship_locked,               // Karajorma - Prevents the player from changing the ship
                                   // class on loadout screen
        Weapons_locked,            // Karajorma - Prevents the player from changing the
                                   // weapons on the ship on the loadout screen
        Ship_selective_linking,    // RSAXVC - Allow pilot to pick firing
                                   // configuration
        Scramble_messages,         // Goober5000 - all messages sent from this ship appear
                                   // scrambled
        No_secondary_lockon,       // zookeeper - secondary lock-on disabled
        No_disabled_self_destruct, // Goober5000 - ship will not self-destruct
                                   // after 90 seconds if engines or weapons
                                   // destroyed (c.f. ai_maybe_self_destruct)
...
FLAG_LIST(Info_Flags){
        ...
        Ballistic_primaries,           // this ship can equip ballistic primaries -
                                       // Goober5000
        ...
        Ship_copy,                     // this ship is a copy of another ship in the table --
                                       // meaningful for scoring and possible other things
        In_tech_database,              // is ship type to be listed in the tech database?
        In_tech_database_m,            // is ship type to be listed in the tech database for
                                       // multiplayer?
        Stealth,                       // the ship has stealth capabilities
        Supercap,                      // the ship is a supercap
        Drydock,                       // the ship is a drydock
        Ship_class_dont_collide_invis, // Don't collide with this ship's invisible
                                       // polygons
        Big_damage,                    // this ship is classified as a big damage ship
        Has_awacs,                     // ship has an awacs subsystem
        Corvette,                      // corvette class (currently this only means anything for
                                       // briefing icons)
        ...
        Surface_shields,               // _argv[-1], 16 Jan 2005: Enable surface shields for this
                                       // ship.
        Generate_hud_icon,             // Enable generation of a HUD shield icon
        Disable_weapon_damage_scaling, // WMC - Disable weapon scaling based on
                                       // flags
        Gun_convergence,               // WMC - Gun convergence based on model weapon norms.
        No_thruster_geo_noise,         // Echelon9 - No thruster geometry noise.
        Intrinsic_no_shields,          // Chief - disables shields for this ship even
                                       // without No Shields in mission.
        No_primary_linking,            // Chief - slated for 3.7 originally, but this looks
                                       // pretty simple to implement.
        No_pain_flash,                 // The E - disable red pain flash
        Allow_landings,                // SUSHI: Automatically set if any subsystems allow
                                       // landings (as a shortcut)
        No_ets,                        // The E - No ETS on this ship class
        No_lighting,                   // Valathil - No lighting for this ship
        Dyn_primary_linking,           // RSAXVC - Dynamically generate weapon linking
                                       // options
        Auto_spread_shields,           // zookeeper - auto spread shields
        Draw_weapon_models,            // the ship draws weapon models of any sort (used to be
                                       // a boolean)
        Model_point_shields,           // zookeeper - uses model-defined shield points
...
// reinforcement, no_arrival_music, no_arrival_message, no_arrival_warp,
// no_departure_warp, no_dynamic and nav_carry_status Should that change, bump
// this variable and make sure to make the necessary changes to parse_wing (in
// missionparse)
...
FLAG_LIST(Wing_Flags){
        ...
        Reset_reinforcement, // needed when we need to reset the wing's
                             // reinforcement flag (after calling it in)
        ...
        No_arrival_warp,     // don't play warp effect for any arriving ships in this
                             // wing.
        No_departure_warp,   // don't play warp effect for any departing ships in
                             // this wing.
        ...
        Never_existed,       // this wing never existed because something prevented it
