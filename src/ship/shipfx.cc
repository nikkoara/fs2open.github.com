        ...
        for (i = 0; i < num_live_debris; i++) {
                ...
                if (live_debris_obj) {
                        ...
                        if (Ship_info[ship_p->ship_info_index]
                                ...
                                float vel_mag = vm_vec_mag_quick(&radial_vel) * 1.3f * (0.9f + 0.2f * fs2::prng::randf(0));
                                ...
                                if (rotvel_mag > 0.1) {
                                        float scale = (1.2f + 0.2f * fs2::prng::randf(0)) * vel_mag / rotvel_mag;
...
float shipfx_calculate_warp_time(object *objp, int warp_dir)
{
        if (objp->type == OBJ_SHIP) {
                ...
                if ((warp_dir == WD_WARP_IN) && (sip->warpin_time > 0.0f)) {
                        ...
                        (warp_dir == WD_WARP_OUT) && (objp == Player_obj) && (sip->warpout_player_speed != 0.0f)) {
                        ...
                        if (sip->cockpit_model_num > 0) {
                                ...
                                if (mc_result) {
                                        if (mc.t_poly) {
                                                ...
                                                if (tmap_num >= MAX_MODEL_TEXTURES) {
                                                        return 0;
                                                }
                ...
                if (sn > -1) {
                        ...
                        if (sip->flags[Ship::Info_Flags::Fighter]) {
                                if (hull_percent > 0.6f) {
                                        ...
                                        float spark_duration = (float)pow(2.0f, -5.0f * (hull_percent - 1.3f)) * (1.0f + 0.6f * (fs2::prng::randf(0) - 0.5f)); // +- 30%
                        ...
                        pe.normal_variance = pef.variance; // How close they stick to that normal 0=good,
                                                           // 1=360 degree
...
static void split_ship_init(ship *shipp, split_ship *split_shipp)
{
        ...
        float explosion_time = (3.0f + expl_length_scale + (fs2::prng::randf(0) - 0.5f)) * speed_reduction_factor;
        ...
        float rel_vel = (0.6f + 0.2f * fs2::prng::randf(0)) * expl_vel * speed_reduction_factor;
...
void shipfx_debris_limit_speed(debris *db, ship *shipp)
{
        ...
        if (sip->debris_min_speed >= 0.0f && sip->debris_max_speed >= 0.0f) {
                float debris_speed = ((sip->debris_max_speed - sip->debris_min_speed) * fs2::prng::randf(0)) + sip->debris_min_speed;
        ...
        if (sip->debris_min_rotspeed >= 0.0f && sip->debris_max_rotspeed >= 0.0f) {
                float debris_rotspeed = ((sip->debris_max_rotspeed - sip->debris_min_rotspeed) * fs2::prng::randf(0)) + sip->debris_min_rotspeed;
...
maybe_fireball_wipe(clip_ship *half_ship, sound_handle *handle_array)
{
        ...
        if (timestamp_elapsed(half_ship->next_fireball)) {
                if (half_ship->length_left > 0.2f * fabsf(half_ship->explosion_vel)) {
                        ...
                        pe.normal_variance = pef.variance; // How close they stick to that normal 0=on
...
void shipfx_do_damaged_arcs_frame(ship *shipp)
{
        ...
        if ((ship_subsys_disrupted(shipp, SUBSYSTEM_ENGINE)) || (ship_subsys_disrupted(shipp, SUBSYSTEM_WEAPONS)) || (ship_subsys_disrupted(shipp, SUBSYSTEM_SENSORS))) {
                disrupted_arc = 1;
                should_arc = 1;
        }
...
void shipfx_do_shockwave_stuff(ship *shipp, shockwave_create_info *sci)
{
        ...
        for (idx = 0; idx < sip->shockwave_count; idx++) {
                ...
                sci2.blast = (sci->blast / (float)sip->shockwave_count) * fs2::prng::randf(0, 0.75f, 1.25f);
                sci2.damage = (sci->damage / (float)sip->shockwave_count) * fs2::prng::randf(0, 0.75f, 1.25f);
                                ...
                                if (dot_to_ship > 0) {
                                        ...
                                        if (dist_sqr < max_wash_dist * max_wash_dist) {
                                                ...
                                                if (dist_sqr < ((radius_mult * radius_mult) * (bank->points[j].radius * bank->points[j].radius))) {
                                                        ...
                                                        inset_depth = (bank->points[j].radius / tanf(half_angle));
                                                        vm_vec_scale_add(
...
int WE_Default::warpStart()
{
        ...
        if ((direction == WD_WARP_IN) && shipfx_special_warp_objnum_valid(shipp->special_warpin_objnum)) {
                ...
                (direction == WD_WARP_OUT) && shipfx_special_warp_objnum_valid(shipp->special_warpout_objnum)) {
        ...
        if (direction == WD_WARP_OUT) {
                ...
                int fireball_type = ((portal_objp != NULL) || (sip->warpout_type == WT_KNOSSOS) || (sip->warpout_type == WT_DEFAULT_THEN_KNOSSOS))
                                            ? FIREBALL_KNOSSOS
                                            : FIREBALL_WARP;
                ...
                int fireball_type = ((portal_objp != NULL) || (sip->warpin_type == WT_KNOSSOS))
                                            ? FIREBALL_KNOSSOS
                                            : FIREBALL_WARP;
        ...
        if (direction == WD_WARP_IN) {
                ...
                stage_duration[2] = int(shipfx_calculate_warp_time(objp, WD_WARP_IN) * 1000.0f);
                ...
                if (objp == Player_obj) {
                        stage_duration[1] = int(fireball_lifeleft(&Objects[warp_objnum]) * 1000.0f);
...
int WE_Default::warpFrame(float frametime)
{
        if (direction == WD_WARP_IN) {
                if ((shipp->flags[Ship::Ship_Flags::Arriving_stage_1]) && timestamp_elapsed(stage_time_end)) {
                        ...
                        (shipp->flags[Ship::Ship_Flags::Arriving_stage_2]) && timestamp_elapsed(stage_time_end)) {
        ...
        : WarpEffect(n_objp, n_direction)
{
        ...
        if (strlen(tmp_name)) {
                ...
                if (anim > -1) {
                        anim_total_time = int(((float)anim_nframes / (float)anim_fps) * 1000.0f);
                }
...
int WE_BSG::warpShipRender()
{
        ...
        if (anim > -1) {
                ...
                int anim_frame = int(
                        ((float)(timestamp() - total_time_start) / 1000.0f) * (float)anim_fps);
        ...
        if (stage == 1 && shockwave > -1) {
                int shockwave_frame = int(
                        ((float)(timestamp() - stage_time_start) / 1000.0f) * (float)shockwave_fps);
...
int WE_Homeworld::warpShipRender()
{
        ...
        if (anim_fps > 0)
                frame = int(
                        (int)(((float)(timestamp() - (float)total_time_start) / 1000.0f) * (float)anim_fps) % anim_nframes);
...
int WE_Hyperspace::warpFrame(float /*frametime*/)
{
        ...
        if (timestamp_elapsed(total_time_end)) {
                ...
                if (direction == WD_WARP_IN) {
                        ...
                        // smoothly than this.
                        scale = MIN(
                                scale, (initial_velocity * (total_duration / 1000) * -(1.0f - progress)));
                } else {
