 ...
 *
 * DKA: 5/26/99 make velocity of debris scale according to size of debris
 * subobject (at least for large subobjects)
 */
static void shipfx_subsystem_maybe_create_live_debris (
    object* ship_objp, ship* ship_p, ship_subsys* subsys, vec3d* exp_center,
    float exp_mag) {
    // initializations
    ...
    if ((num_live_debris <= 0) ||
        (subsys->flags[Ship::Subsystem_Flags::No_live_debris])) {
        return;
    }
    ...
    if (pm->submodel[submodel_num].movement_type == MOVEMENT_TYPE_ROT ||
        ...
        // get the rotvel
        model_get_rotating_submodel_axis (
            &model_axis, &world_axis, shipp->model_instance_num, submodel_num,
            &ship_objp->orient);
        vm_vec_copy_scale (&rotvel, &world_axis, sii->cur_turn_rate);

        model_instance_find_world_point (
            &world_axis_pt, &sii->pt_on_axis, shipp->model_instance_num,
            submodel_num, &ship_objp->orient, &ship_objp->pos);

        vm_quaternion_rotate (
            &m_rot, vm_vec_mag ((vec3d*)&sii->angs), &model_axis);
    }
    else {
    ...
    for (i = 0; i < num_live_debris; i++) {
        ...
        model_instance_find_world_point (
            &start_world_pos, &pm->submodel[live_debris_submodel].offset,
            shipp->model_instance_num, live_debris_submodel,
            &ship_objp->orient, &ship_objp->pos);

        // convert to model coord of underlying submodel
        // set angle to zero
        pm->submodel[submodel_num].angs = zero_angs;
        world_find_model_instance_point (
            &start_model_pos, &start_world_pos, pmi, submodel_num,
            &ship_objp->orient, &ship_objp->pos);
        ...
        model_instance_find_world_point (
            &end_world_pos, &start_model_pos, shipp->model_instance_num,
            submodel_num, &ship_objp->orient, &ship_objp->pos);

        int fireball_type =
            fireball_ship_explosion_type (&Ship_info[ship_p->ship_info_index]);
        if (fireball_type < 0) { fireball_type = FIREBALL_EXPLOSION_MEDIUM; }
        // create fireball here.
        fireball_create (
            &end_world_pos, fireball_type, FIREBALL_MEDIUM_EXPLOSION,
            OBJ_INDEX (ship_objp), pm->submodel[live_debris_submodel].rad);

        // create debris
        live_debris_obj = debris_create (
            ship_objp, pm->id, live_debris_submodel, &end_world_pos,
            exp_center, 1, exp_mag);
        ...
        if (live_debris_obj) {
            ...
            if (Ship_info[ship_p->ship_info_index]
                ...
                vm_vec_scale_add2 (
                    &live_debris_obj->phys_info.vel, &vec_to_center, vel_mag);
            }
            else {
                ...
                vec3d temp_vel; // explosion velocity with ship_obj velocity
                                // removed
                vm_vec_sub (
                    &temp_vel, &live_debris_obj->phys_info.vel,
                ...
                if (rotvel_mag > 0.1) {
                    ...
                    vm_vec_scale_add2 (
                        &live_debris_obj->phys_info.vel, &radial_vel, scale);
                }

                // scale up speed of debris if ship_obj > 125, but not for
                // knossos
                if (ship_objp->radius > 250 &&
                    !(Ship_info[ship_p->ship_info_index]
                          .flags[Ship::Info_Flags::Knossos_device])) {
                    vm_vec_scale (
                        &live_debris_obj->phys_info.vel,
                        ship_objp->radius / 250.0f);
                }
            }

            shipfx_debris_limit_speed (
                &Debris[live_debris_obj->instance], ship_p);
        }
    }
}
 ...
 * when ship submodel has not already been blown off (and hence liberated live
 * debris)
 */
static void shipfx_maybe_create_live_debris_at_ship_death (object* ship_objp) {
    ...
    if (Ship_info[shipp->ship_info_index].n_subsystems == 0) { return; }
    ...
    for (int idx = 0; idx < pm->num_debris_objects; idx++) {
        if (pm->submodel[pm->debris_objects[idx]].is_live_debris) {
            ...
            if (!pmi->submodel[parent].blown_off) {
                ...
                // Go through all subsystems and look for submodel the
                // subsystems with "parent" submodel.
                ship_subsys* pss = NULL;
                for (pss = GET_FIRST (&shipp->subsys_list);
                     pss != END_OF_LIST (&shipp->subsys_list);
                     pss = GET_NEXT (pss)) {
                ...
                if (pss != NULL) {
                    if (pss->system_info != NULL) {
                        ...
                        model_instance_find_world_point (
                            &exp_center, &tmp, shipp->model_instance_num,
                            parent, &ship_objp->orient, &ship_objp->pos);

                        // if not blown off, blow it off
                        shipfx_subsystem_maybe_create_live_debris (
                            ship_objp, shipp, pss, &exp_center, 3.0f);

                        // now set subsystem as blown off, so we only get one
                        // copy
                        pm->submodel[parent].blown_off = 1;
                        set_ship_submodel_as_blown_off (
                            &Ships[ship_objp->instance],
                            pss->system_info->subobj_name);
                    }
                }
            }
        }
...
void shipfx_blow_off_subsystem (
    object* ship_objp, ship* ship_p, ship_subsys* subsys, vec3d* exp_center,
    bool no_explosion) {
    vec3d subobj_pos;
    ...
    if (!(subsys->flags[Ship::Subsystem_Flags::Vanished]) && !no_explosion) {
        shipfx_blow_up_model (
            ship_objp, model_num, psub->subobj_num, 50, &subobj_pos);

        // create live debris objects, if any
        // TODO:  some MULITPLAYER implcations here!!
        shipfx_subsystem_maybe_create_live_debris (
            ship_objp, ship_p, subsys, exp_center, 1.0f);
        ...
        fireball_create (
            &subobj_pos, fireball_type, FIREBALL_MEDIUM_EXPLOSION,
            OBJ_INDEX (ship_objp), psub->radius);
    }
}
    ...
    for (int i = 0; i < pm->num_debris_objects; i++) {
        if (!pm->submodel[pm->debris_objects[i]].is_live_debris) {
            ...
            if (try_live_debris) {
                ...
                // this creates *all* the live debris for *all* the currently
                // live subsystems.
...
/**
 * Creates "ndebris" pieces of debris on random verts of the the "submodel" in
 * the ship's model.
 */
void shipfx_blow_up_model (
    ...
    if (submodel == 0) { shipfx_blow_up_hull (obj, model, exp_center); }
    ...
    for (i = 0; i < ndebris; i++) {
        ...
        model_find_world_point (
            &outpnt, &tmp, model, submodel, &obj->orient, &obj->pos);
...
static float shipfx_calculate_effect_radius (object* objp, int warp_dir) {
    ...
    // if docked, we need to calculate the overall cross-sectional radius
    // around the z-axis (longitudinal axis)
    if (object_is_docked (objp)) {
        rad = dock_calc_max_cross_sectional_radius_perpendicular_to_axis (
            objp, Z_AXIS);
    }
    // if it's not docked, we can save a lot of work by just using width and
    // height
    else {
...
float shipfx_calculate_warp_time (object* objp, int warp_dir) {
    if (objp->type == OBJ_SHIP) {
        ...
        if ((warp_dir == WD_WARP_IN) && (sip->warpin_time > 0.0f)) {
            ...
            (warp_dir == WD_WARP_OUT) && (objp == Player_obj) &&
    ...
    float rad_percent =
        (objp->radius - SMALLEST_RAD) / (LARGEST_RAD - SMALLEST_RAD);
    CLAMP (rad_percent, 0.0f, 1.0f);

    float rad_time = rad_percent * (LARGEST_RAD_TIME - SMALLEST_RAD_TIME) +
                     SMALLEST_RAD_TIME;
...
static int shipfx_special_warp_objnum_valid (int objnum) {
    ...
    // must be a ship
    if (special_objp->type != OBJ_SHIP) return 0;

    // must be a knossos
    if (!(Ship_info[Ships[special_objp->instance].ship_info_index]
              .flags[Ship::Info_Flags::Knossos_device]))
        return 0;
...
void shipfx_warpin_start (object* objp) {
    ...
    if (object_is_docked (objp) &&
        !(shipp->flags[Ship::Ship_Flags::Dock_leader])) {
        return;
    }
...
static int compute_special_warpout_stuff (
    ...
    if ((ref_objnum >= 0) && (ref_objnum < MAX_OBJECTS)) {
        ...
        if (sp_objp->type == OBJ_SHIP) {
            if (Ship_info[Ships[sp_objp->instance].ship_info_index]
                    .flags[Ship::Info_Flags::Knossos_device]) {
    ...
    dist_to_plane = fvi_ray_plane (
        warp_pos, &sp_objp->pos, &facing_normal, &center_pos,
        &objp->orient.vec.fvec, 0.0f);
    ...
    // validate angle
    float max_warpout_angle =
        0.707f; // 45 degree half-angle cone for small ships
    if (Ship_info[Ships[objp->instance].ship_info_index].is_big_or_huge ()) {
        max_warpout_angle =
            0.866f; // 30 degree half-angle cone for BIG or HUGE
    }
    ...
    // Calculate how long to fly through the effect.  Not to get to the effect,
    // just through it.
...
static void compute_warpout_stuff (
    ...
    if (shipp->special_warpout_objnum >= 0) {
        if (compute_special_warpout_stuff (objp, speed, warp_time, warp_pos) !=
            -1) {
            return;
        }
        else {
    ...
    *speed = ship_get_warpout_speed (objp);
    ...
    if (sip->is_huge_ship ()) {
        ...
        if (object_is_docked (objp)) {
            // we need to get the longitudinal radius of our ship, so find the
            // semilatus rectum along the Z-axis
            radius =
                dock_calc_max_semilatus_rectum_parallel_to_axis (objp, Z_AXIS);
        }
        else
            radius = objp->radius;

        // Now we know our speed. Figure out how far the warp effect will be
        // from here.
        ship_move_dist =
            (*speed * SHIPFX_WARP_DELAY) +
            radius * 1.5f; // We want to get to 1.5R away from effect
        if (ship_move_dist < radius * 1.5f) { ship_move_dist = radius * 1.5f; }
    }
    ...
    *warp_time += ship_move_dist / *speed;
    ...
    vm_vec_scale_add (
        warp_pos, &center_pos, &objp->orient.vec.fvec, warp_dist);
}
...
void shipfx_warpout_start (object* objp) {
    ...
    // return if disabled
    if (shipp->flags[Ship::Ship_Flags::Disabled]) { return; }
...
void shipfx_warpout_frame (object* objp, float frametime) {
    ...
    // disabled ships should stay on the battlefield if they were disabled
    // during warpout phreak 5/22/03
 ...
 */
bool shipfx_point_in_shadow (
    vec3d* p0, matrix* src_orient, vec3d* src_pos, float /*radius*/) {
    mc_info mc;
    object* objp;
    ship_obj* so;
    int n_lights;
    ...
    for (idx = 0; idx < n_lights; idx++) {
        ...
        for (so = GET_FIRST (&Ship_obj_list);
            ...
            if (model_collide (&mc)) { return true; }
        }
    }
...
bool shipfx_in_shadow (object* src_obj) {
    ...
    for (idx = 0; idx < n_lights; idx++) {
        ...
        for (so = GET_FIRST (&Ship_obj_list);
            ...
            if (src_obj != objp) {
                vm_vec_scale_add (
                    &rp1, &rp0, &light_dir, objp->radius * 10.0f);
...
bool shipfx_eye_in_shadow (vec3d* eye_pos, object* src_obj, int sun_n) {
    ...
    // The mc_info struct only needs to be initialized once for this entire
    // function.  This is because every time the mc variable is reused, every
    // parameter that model_collide reads from is reassigned. Therefore the
    // stale fields in the rest of the struct do not matter because either a)
    // they are never read from, or b) they are overwritten by the new
    ...
    // get the light dir
    if (!light_get_global_dir (&light_dir, sun_n)) { return false; }
    ...
    for (i = 0; i < MAX_DEBRIS_PIECES; i++, db++) {
        if (!(db->flags & DEBRIS_USED) || !db->is_hull) { continue; }
    ...
    if (Viewer_obj != NULL && Viewer_mode != VM_TOPDOWN) {
        if (Viewer_obj->type == OBJ_SHIP && Viewer_obj->instance >= 0) {
            ...
            if (sip->cockpit_model_num > 0) {
                vm_vec_scale_add (
                    &rp1, &rp0, &light_dir, Viewer_obj->radius * 2.0f);
                vec3d pos, eye_posi;
                matrix eye_ori;
                ...
                if (mc_result) {
                    if (mc.t_poly) {
                        ...
                        if (!(pm->maps[tmap_num].is_transparent) &&
                            strcmp (
                                bm_get_filename (mc.hit_bitmap),
                                "glass.dds") != 0) {
                            return true;
                        }
                    }

                    if (mc.f_poly) { return true; }
            ...
            if (sip->flags[Ship::Info_Flags::Show_ship_model]) {
                vm_vec_scale_add (
                    &rp1, &rp0, &light_dir, Viewer_obj->radius * 10.0f);
    ...
    if (Asteroid_field.num_initial_asteroids <= 0) { return false; }

    for (i = 0; i < MAX_ASTEROIDS; i++, ast++) {
        if (!(ast->flags & AF_USED)) { continue; }
        ...
        mc.model_instance_num = -1;
        mc.model_num = Asteroid_info[ast->asteroid_type]
                           .model_num[ast->asteroid_subtype]; // Fill in the
                                                              // model to check
        mc.submodel_num = -1;
        model_clear_instance (mc.model_num);
        mc.orient = &objp->orient; // The object's orient
 ...
 */
void shipfx_flash_create (
    object* objp, int model_num, vec3d* gun_pos, vec3d* gun_dir,
    int is_primary, int weapon_info_index) {
    int i;
    ...
    if ((Weapon_info[weapon_info_index].muzzle_flash >= 0) &&
        !(Weapon_info[weapon_info_index].wi_flags[Weapon::Info_Flags::Flak])) {
        vec3d real_dir;
        vm_vec_rotate (&real_dir, gun_dir, &objp->orient);
        mflash_create (
            gun_pos, &real_dir, &objp->phys_info,
            Weapon_info[weapon_info_index].muzzle_flash, objp);
    }
    ...
    for (i = 0; i <= Ship_flash_highest; i++) {
        ...
        if ((Ship_flash[i].objnum == objnum) &&
            (Ship_flash[i].obj_signature == objp->signature)) {
    ...
    if (is_primary) { Ship_flash[first_slot].max_life = FLASH_LIFE_PRIMARY; }
    else {
...
/**
 * Sets the flash lights in the model used by this ship to the appropriate
 * values.
 *
 * There might not be any flashes linked to this ship in which case this
...
void shipfx_flash_light_model (object* objp, int model_num) {
    ...
    for (i = 0; i <= Ship_flash_highest; i++) {
        if ((Ship_flash[i].objnum == objnum) &&
            (Ship_flash[i].obj_signature == objp->signature)) {
            float v = (Ship_flash[i].max_life - Ship_flash[i].life) /
                      Ship_flash[i].max_life;
        ...
        if (sf->objnum > -1) {
            ...
            if (kill_it) {
                ...
                if (i == Ship_flash_highest) {
                    while ((Ship_flash_highest > 0) &&
                           (Ship_flash[Ship_flash_highest].objnum == -1)) {
                        Ship_flash_highest--;
                    }
                }
            }
...
void shipfx_emit_spark (int n, int sn) {
    ...
    ship_info* sip = &Ship_info[shipp->ship_info_index];
    if (sn > -1 && sip->impact_spew.n_high <= 0) return;

    if (sn < 0 && sip->damage_spew.n_high <= 0) return;
    ...
    // get spark_scale_factor -- how much to increase ship sparks, based on
    // radius
    if (ship_radius > 40) { spark_scale_factor = 1.0f; }
    ...
    float spark_time_scale =
        1.0f + spark_scale_factor * (Particle_life - 1.0f);
    float spark_width_scale =
        1.0f + spark_scale_factor * (Particle_width - 1.0f);
    float spark_num_scale =
        1.0f + spark_scale_factor * (Particle_number - 1.0f);

    obj = &Objects[shipp->objnum];

    float hull_percent = get_hull_pct (obj);
    if (hull_percent < 0.001) { hull_percent = 0.001f; }
    float fraction = 0.1f * obj->radius / hull_percent;
    if (fraction > 1.0f) { fraction = 1.0f; }

    int spark_num;
    if (sn == -1) { spark_num = myrand () % shipp->num_hits; }
    else {
        spark_num = sn;
    }

    // don't display sparks that have expired
    if (timestamp_elapsed (shipp->sparks[spark_num].end_time)) { return; }

    // get spark position
    if (shipp->sparks[spark_num].submodel_num != -1) {
        model_instance_find_world_point (
            &outpnt, &shipp->sparks[spark_num].pos, shipp->model_instance_num,
    ...
    if ((shipp->is_arriving () ||
         shipp->flags[Ship::Ship_Flags::Depart_warp]) &&
        ...
        if (vm_vec_dot (&tmp, &warp_orient.vec.fvec) < 0.0f) {
            if (shipp->is_arriving ()) // if in front of warp plane, don't
                                       // create.
                create_spark = 0;
        }
        else {
            if (shipp->flags[Ship::Ship_Flags::Depart_warp]) create_spark = 0;
        }
    }
    ...
    if (create_spark) {
        ...
        // model_find_world_dir(&out_dir, &in_dir, model_num, submodel_num,
        // &objorient, &objpos);
        ...
        if (sn > -1) {
            ...
            if (pef.variance > 0.0f) { pe.normal_variance = pef.variance; }
            else {
...
typedef struct clip_ship {
    ...
    vec3d model_center_disp_to_orig_center; // displacement from half ship
                                            // center to original model center
    vec3d clip_plane_norm;   // clip plane normal (local [0,0,1] or [0,0,-1])
    float cur_clip_plane_pt; // displacement from half ship clip plane to
...
static int get_split_ship () {
    ...
    for (i = 0; i < max_size; i++) {
        if (!Split_ships[i].used) return i;
    }
...
static void
maybe_fireball_wipe (clip_ship* half_ship, sound_handle* sound_handle);
static void split_ship_init (ship* shipp, split_ship* split_shipp) {
    ...
    snd_play_3d (
        gamesnd_get_game_sound (GameSounds::SHOCKWAVE_EXPLODE),
        &parent_ship_obj->pos, &View_position, 0.0f, NULL, 0, 1.0f,
    ...
    split_shipp->explosion_flash_timestamp =
        timestamp ((int)(0.00075f * parent_ship_obj->radius));
    ...
    vm_vec_copy_scale (
        &split_shipp->back_ship.clip_plane_norm, &vmd_z_vector, -1.0f);
    ...
    vm_vec_copy_scale (
        &split_shipp->front_ship.local_pivot, &orient->vec.fvec, dist);
    vm_vec_make (
        &split_shipp->front_ship.model_center_disp_to_orig_center, 0.0f, 0.0f,
        -dist);
    dist = (split_shipp->back_ship.cur_clip_plane_pt + pm->mins.xyz.z) / 2.0f;
    vm_vec_copy_scale (
        &split_shipp->back_ship.local_pivot, &orient->vec.fvec, dist);
    vm_vec_make (
        &split_shipp->back_ship.model_center_disp_to_orig_center, 0.0f, 0.0f,
        -dist);
    ...
    for (int i = 0; i < pm->num_debris_objects; i++) {
        ...
        // tmp is world position,  temp_pos is world_pivot,  tmp1 is offset
        // from world_pivot (in ship local coord)
        model_find_world_point (
            &tmp, &tmp1, pm->id, -1, &vmd_identity_matrix, &temp_pos);
    ...
    // set up explosion vel and relative velocities (assuming mass depends on
    // length)
    float front_length =
        pm->maxs.xyz.z - split_shipp->front_ship.cur_clip_plane_pt;
    float back_length =
        split_shipp->back_ship.cur_clip_plane_pt - pm->mins.xyz.z;
    ...
    vm_vec_copy_scale (
        &split_shipp->back_ship.phys_info.rotvel, &rotvel,
        -(front_length * front_length) / (back_length * back_length));
    split_shipp->front_ship.phys_info.rotvel.xyz.z =
        parent_ship_obj->phys_info.rotvel.xyz.z;
    split_shipp->back_ship.phys_info.rotvel.xyz.z =
        parent_ship_obj->phys_info.rotvel.xyz.z;
    ...
    vm_vec_cross (
        &vel_from_rotvel, &temp_rotvel, &split_shipp->front_ship.local_pivot);
    vm_vec_cross (
        &vel_from_rotvel, &temp_rotvel, &split_shipp->back_ship.local_pivot);
    ...
    maybe_fireball_wipe (
        &split_shipp->front_ship, split_shipp->sound_handles.data ());
    maybe_fireball_wipe (
        &split_shipp->back_ship, split_shipp->sound_handles.data ());
    vm_vec_scale_add2 (
        &split_shipp->front_ship.phys_info.vel, &orient->vec.fvec, front_vel);
    vm_vec_scale_add2 (
        &split_shipp->back_ship.phys_info.vel, &orient->vec.fvec, back_vel);

    // HANDLE LIVE DEBRIS - blow off if not already gone
    shipfx_maybe_create_live_debris_at_ship_death (parent_ship_obj);
}

void shipfx_queue_render_ship_halves_and_debris (
    model_draw_list* scene, clip_ship* half_ship, ship* shipp) {
    polymodel* pm = model_get (Ship_info[shipp->ship_info_index].model_num);

    // get rotated clip plane normal and world coord of original ship center
    vec3d orig_ship_world_center, clip_plane_norm, model_clip_plane_pt,
        debris_clip_plane_pt;
    vm_vec_unrotate (
        &clip_plane_norm, &half_ship->clip_plane_norm, &half_ship->orient);
    vm_vec_unrotate (
        &orig_ship_world_center, &half_ship->model_center_disp_to_orig_center,
        &half_ship->orient);
    vm_vec_add2 (&orig_ship_world_center, &half_ship->local_pivot);

    // get debris clip plane pt and draw debris
    vm_vec_unrotate (
        &debris_clip_plane_pt, &half_ship->model_center_disp_to_orig_center,
        &half_ship->orient);
    ...
    for (int i = 0; i < pm->num_debris_objects; i++) {
        ...
        if (half_ship->draw_debris[i] == DEBRIS_DRAW) {
            ...
            int is_live_debris =
                pm->submodel[pm->debris_objects[i]].is_live_debris;
            int create_debris = 0;
            // front ship
            if (half_ship->explosion_vel > 0) {
                if (half_ship->cur_clip_plane_pt >
                    tmp1.xyz.z +
                        pm->submodel[pm->debris_objects[i]].max.xyz.z -
                        0.1f * half_ship->explosion_vel) {
                    create_debris = 1;
                }
                // back ship
            }
                ...
                if (half_ship->cur_clip_plane_pt <
                    tmp1.xyz.z +
                        pm->submodel[pm->debris_objects[i]].min.xyz.z -
                        0.1f * half_ship->explosion_vel) {
                    create_debris = 1;
                }
            }
            ...
            if (!is_live_debris) {
                model_find_world_point (
                    &tmp, &tmp1, pm->id, -1, &half_ship->orient, &temp_pos);
                model_render_params render_info;

                render_info.set_clip_plane (
                    debris_clip_plane_pt, clip_plane_norm);
                render_info.set_replacement_textures (
                    shipp->ship_replacement_textures);
                render_info.set_flags (render_flags);

                submodel_render_queue (
                    &render_info, scene, pm->id, pm->debris_objects[i],
                    &half_ship->orient, &tmp);
            }

            // make free piece of debris
            if (create_debris) {
                half_ship->draw_debris[i] =
                    DEBRIS_FREE; // mark debris to not render with model
                ...
                // debris vel (1) split ship vel (2) split ship rotvel (3)
                // random
                if (!is_live_debris) {
                    object* debris_obj;
                    debris_obj = debris_create (
                        half_ship->parent_obj, pm->id, pm->debris_objects[i],
                        &tmp, &half_ship->local_pivot, 1, 1.0f);
                    ...
                    if (debris_obj) {
                        ...
                        vm_vec_sub (
                            &center_to_debris, &tmp, &half_ship->local_pivot);
                        vm_vec_cross (
                            &debris_vel, &center_to_debris,
                        ...
                        shipfx_debris_limit_speed (
                            &Debris[debris_obj->instance], shipp);
                    }
                }
            }
        }
    ...
    model_render_queue (
        &render_info, scene, pm->id, &half_ship->orient,
        &orig_ship_world_center);
}
...
void shipfx_debris_limit_speed (debris* db, ship* shipp) {
    ...
    if (sip->debris_min_speed >= 0.0f && sip->debris_max_speed >= 0.0f) {
        ...
        if (fabs (curspeed) >= 0.001f) {
            ...
            vm_vec_copy_scale (
                &pi->vel, &ship_objp->orient.vec.fvec, debris_speed);
        }
    }
        ...
        if (curspeed < sip->debris_min_speed) {
            if (fabs (curspeed) >= 0.001f) {
                ...
                vm_vec_copy_scale (
                    &pi->vel, &ship_objp->orient.vec.fvec,
                    sip->debris_min_speed);
            }
        }
    }
    ...
    if (sip->debris_min_rotspeed >= 0.0f && sip->debris_max_rotspeed >= 0.0f) {
        ...
        if (fabs (currotvel) >= 0.001f) {
            ...
            vm_vec_copy_scale (
                &pi->rotvel, &ship_objp->orient.vec.uvec, debris_rotspeed);
        }
    }
        ...
        if (curspeed < sip->debris_min_rotspeed) {
            if (fabs (currotvel) >= 0.001f) {
                ...
                vm_vec_copy_scale (
                    &pi->rotvel, &ship_objp->orient.vec.uvec,
                    sip->debris_min_rotspeed);
            }
        }
    }
    ...
    if (ship_type > -1) {
        if (vm_vec_mag (&pi->vel) > Ship_types[ship_type].debris_max_speed) {
            float scale =
                Ship_types[ship_type].debris_max_speed / vm_vec_mag (&pi->vel);
            vm_vec_scale (&pi->vel, scale);
        }
    }
...
// ----------------------------------------------------------------------------
// uses list of model z values with constant increment to find the radius of
// the cross section at the current model z value
static float get_model_cross_section_at_z (float z, polymodel* pm) {
    if (pm->num_xc < 2) { return 0.0f; }

    float index, increment;
    increment =
        (pm->xc[pm->num_xc - 1].z - pm->xc[0].z) / (float)(pm->num_xc - 1);
    index = (z - pm->xc[0].z) / increment;

    if (index < 0.5f) { return pm->xc[0].radius; }
...
static int get_sound_time_played (sound_load_id snd_id, sound_handle handle) {
    if (!handle.isValid () || !snd_id.isValid ()) { return 100000; }
 ...
 *
 * Forces playing of sub-explosion sounds.  Keeps track of active sounds, plays
 * them for >= 750 ms when sound has played >= 750, sound is stopped and new
 * instance is started
 */
void do_sub_expl_sound (
    float radius, vec3d* sound_pos, sound_handle* handle_array) {
    ...
    if (!handle.isValid ()) {
        ...
        handle_array[handle_index] = snd_play_3d (
            gamesnd_get_game_sound (sound_index), sound_pos, &View_position,
            0.0f, nullptr, 0, 0.6f, SND_PRIORITY_MUST_PLAY, nullptr,
            sound_range);
    }
        ...
        if (get_sound_time_played (snd_get_sound_id (handle), handle) > 400) {
            ...
            handle_array[handle_index] = snd_play_3d (
                gamesnd_get_game_sound (sound_index), sound_pos,
                &View_position, 0.0f, nullptr, 0, 0.6f, SND_PRIORITY_MUST_PLAY,
                nullptr, sound_range);
        }
    }
...
/**
 * Maybe create a fireball along model clip plane also maybe plays explosion
 * sound
 */
static void
maybe_fireball_wipe (clip_ship* half_ship, sound_handle* handle_array) {
    ...
    if (timestamp_elapsed (half_ship->next_fireball)) {
        if (half_ship->length_left >
            ...
            vm_vec_unrotate (
                &orig_ship_world_center,
                &half_ship->model_center_disp_to_orig_center,
                &half_ship->orient);
            vm_vec_add2 (&orig_ship_world_center, &half_ship->local_pivot);
            ...
            float rad = get_model_cross_section_at_z (
                half_ship->cur_clip_plane_pt, pm);
            if (rad < 1) {
            ...
            // defaults to 1.0 now that multiplier was applied to the static
            // values above
            ...
            fireball_create (
                &model_clip_plane_pt, fireball_type, FIREBALL_LARGE_EXPLOSION,
                OBJ_INDEX (half_ship->parent_obj), rad, 0,
                &half_ship->parent_obj->phys_info.vel, 0.0f, -1, NULL,
                low_res_fireballs);
            ...
            // do sound
            do_sub_expl_sound (
                half_ship->parent_obj->radius, &model_clip_plane_pt,
                handle_array);
            ...
            if (pef.max_vel > 0.0f) {
                ...
                pe.max_vel = half_ship->explosion_vel; // How fast the fastest
                                                       // particle can move
            }
...
int shipfx_large_blowup_do_frame (ship* shipp, float frametime) {
    ...
    if (timestamp_elapsed (the_split_ship->explosion_flash_timestamp)) {
        if (!the_split_ship->explosion_flash_started) {
            ...
            if (objp->flags[Object::Object_Flags::Was_rendered]) {
                float excess_dist =
                    vm_vec_dist (&Player_obj->pos, &objp->pos) -
                    2.0f * objp->radius - Player_obj->radius;
                float intensity = 1.0f - 0.1f * excess_dist / objp->radius;

                if (intensity > 1) { intensity = 1.0f; }

                if (intensity > 0.1f && Ship_info[shipp->ship_info_index]
                                            .flags[Ship::Info_Flags::Flash]) {
    ...
    physics_sim (
        &the_split_ship->front_ship.local_pivot,
        &the_split_ship->front_ship.orient,
        &the_split_ship->front_ship.phys_info, frametime);
    physics_sim (
        &the_split_ship->back_ship.local_pivot,
        &the_split_ship->back_ship.orient,
        &the_split_ship->back_ship.phys_info, frametime);
    the_split_ship->front_ship.length_left -=
        the_split_ship->front_ship.explosion_vel * frametime;
    the_split_ship->back_ship.length_left +=
        the_split_ship->back_ship.explosion_vel * frametime;
    the_split_ship->front_ship.cur_clip_plane_pt +=
        the_split_ship->front_ship.explosion_vel * frametime;
    the_split_ship->back_ship.cur_clip_plane_pt +=
        the_split_ship->back_ship.explosion_vel * frametime;

    float length_left = MAX (
        the_split_ship->front_ship.length_left,
        the_split_ship->back_ship.length_left);
    ...
    maybe_fireball_wipe (
        &the_split_ship->front_ship, the_split_ship->sound_handles.data ());
    maybe_fireball_wipe (
        &the_split_ship->back_ship, the_split_ship->sound_handles.data ());
    return 0;
}
...
void shipfx_large_blowup_queue_render (model_draw_list* scene, ship* shipp) {
    ...
    if (the_split_ship->front_ship.length_left > 0) {
        shipfx_queue_render_ship_halves_and_debris (
            scene, &the_split_ship->front_ship, shipp);
    }

    if (the_split_ship->back_ship.length_left > 0) {
        shipfx_queue_render_ship_halves_and_debris (
            scene, &the_split_ship->back_ship, shipp);
    }
}
...
void shipfx_do_damaged_arcs_frame (ship* shipp) {
    ...
    if (shipp->emp_intensity > 0.0f) { should_arc = 1; }
    ...
    if (timestamp_elapsed (shipp->arc_next_time)) {
        shipp->arc_next_time =
            timestamp (-1); // invalid, so it gets restarted next frame
        ...
        for (i = 0; i < MAX_SHIP_ARCS; i++) {
            if (!timestamp_valid (shipp->arc_timestamp[i])) {
                shipp->arc_timestamp[i] =
                    timestamp (lifetime); // live up to a second
                ...
                n++;
                if (n == n_arcs) break; // Don't need to create anymore
            }
            ...
            if (lifetime > 750) {
                // 1.00 second effect
                snd_play_3d (
                    gamesnd_get_game_sound (GameSounds::DEBRIS_ARC_05),
                    &snd_pos, &View_position, obj->radius);
            }
            else if (lifetime > 500) {
                // 0.75 second effect
                snd_play_3d (
                    gamesnd_get_game_sound (GameSounds::DEBRIS_ARC_04),
                    &snd_pos, &View_position, obj->radius);
            }
            else if (lifetime > 250) {
                // 0.50 second effect
                snd_play_3d (
                    gamesnd_get_game_sound (GameSounds::DEBRIS_ARC_03),
                    &snd_pos, &View_position, obj->radius);
            }
            else if (lifetime > 100) {
                // 0.25 second effect
                snd_play_3d (
                    gamesnd_get_game_sound (GameSounds::DEBRIS_ARC_02),
                    &snd_pos, &View_position, obj->radius);
            }
            else {
                // 0.10 second effect
                snd_play_3d (
                    gamesnd_get_game_sound (GameSounds::DEBRIS_ARC_01),
                    &snd_pos, &View_position, obj->radius);
            }
        }
    }
        ...
        if (timestamp_valid (shipp->arc_timestamp[i])) {
            if (!timestamp_elapsed (shipp->arc_timestamp[i])) {
                ...
                if (mr < RAND_MAX / 5) {
                    ...
                    if (mr % 2) { static_one = shipp->arc_pts[i][0]; }
                    else {
                    ...
                    if (obj->radius > 200.0f) {
                        float max_dist =
                            obj->radius * MAX_ARC_LENGTH_PERCENTAGE;
...
void shipfx_do_lightning_frame (ship* /*shipp*/) {
    ...
    if(!((sip->flags[Ship::Info_Flags::Cruiser]) || (sip->is_big_ship()) ||
    (sip->is_huge_ship()))){ shipp->lightning_stamp = -1; return;
    }

    // determine stamp and count values
    if(sip->flags[Ship::Info_Flags::Cruiser]){
        stamp = (int)((float)(Nebl_cruiser_min + ((Nebl_cruiser_max -
    Nebl_cruiser_min) * Nebl_intensity)) * frand_range(0.8f, 1.1f)); count =
    l_cruiser_count;
    }
    else {
        if(sip->is_huge_ship()){
            stamp = (int)((float)(Nebl_supercap_min + ((Nebl_supercap_max -
    Nebl_supercap_min) * Nebl_intensity)) * frand_range(0.8f, 1.1f)); count =
    l_huge_count; } else { stamp = (int)((float)(Nebl_cap_min + ((Nebl_cap_max
    - Nebl_cap_min) * Nebl_intensity)) * frand_range(0.8f, 1.1f)); count =
    l_big_count;
        }
    }
...
void shipfx_do_shockwave_stuff (ship* shipp, shockwave_create_info* sci) {
    ...
    if (sip->shockwave_count <= 0) { return; }
 ...
 *
 * Assumes length of engine wash is greater than radius of engine wash
 * hemisphere
 */
void engine_wash_ship_process (ship* shipp) {
    ...
    if (time_to_next_hit < 0) {
        ...
        shipp->wash_timestamp =
            timestamp (ENGINE_WASH_CHECK_INTERVAL + time_to_next_hit);
    ...
    // go thru Ship_used_list and check if we're in wash from CAP or SUPERCAP
    // (SIF_HUGE)
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
         so = GET_NEXT (so)) {
        ...
        if (!wash_objp || (wash_objp->instance < 0) ||
            (wash_objp->type != OBJ_SHIP)) {
            continue;
        }
        ...
        if (wash_sip->is_small_ship ()) { continue; }

        pm = model_get (wash_sip->model_num);
        float ship_intensity = 0;

        // if engines disabled, no engine wash
        if (ship_get_subsystem_strength (wash_shipp, SUBSYSTEM_ENGINE) <
            0.01) {
            continue;
        }
        ...
        for (idx = 0; idx < pm->n_thrusters; idx++) {
            ...
            vec3d submodel_static_offset; // The associated submodel's static
                                          // offset in the ship's frame of
                                          // reference
            bool submodel_rotation = false;

            // make sure this engine is functional before we try to process a
            // wash from it
            if (!model_should_render_engine_glow (
                    OBJ_INDEX (wash_objp), bank->obj_num)) {
                continue;
            }
            ...
            // If bank is attached to a submodel, prepare to account for
            // rotations
            //
            // TODO: This won't work in the ship lab, because the lab code
            // doesn't set the the necessary submodel instance info needed
            // here. The second condition is thus a hack to disable the feature
            // while in the lab, and can be removed if the lab is re-structured
            // accordingly. -zookeeper
            if (bank->submodel_num > -1 &&
                ...
                model_find_submodel_offset (
                    &submodel_static_offset, wash_sip->model_num,
                    bank->submodel_num);
            ...
            for (j = 0; j < bank->num_points; j++) {
                vec3d world_thruster_pos, world_thruster_norm, apex,
                    thruster_to_ship, apex_to_ship, temp;
                vec3d loc_pos = bank->points[j].pnt;
                vec3d loc_norm = bank->points[j].norm;

                if (submodel_rotation) {
                    vm_vec_sub (
                        &loc_pos, &bank->points[j].pnt,
                        &submodel_static_offset);

                    // Gets the final offset and normal in the ship's frame of
                    // reference
                    temp = loc_pos;
                    find_submodel_instance_point_normal (
                        &loc_pos, &loc_norm, wash_shipp->model_instance_num,
                        bank->submodel_num, &temp, &loc_norm);
                }

                // get world pos of thruster
                vm_vec_unrotate (
                    &world_thruster_pos, &loc_pos, &wash_objp->orient);
                vm_vec_add2 (&world_thruster_pos, &wash_objp->pos);

                // get world norm of thruster;
                vm_vec_unrotate (
                    &world_thruster_norm, &loc_norm, &wash_objp->orient);

                // get vector from thruster to ship
                vm_vec_sub (
                    &thruster_to_ship, &objp->pos, &world_thruster_pos);
                ...
                if (dot_to_ship > 0) {
                    ...
                    max_wash_dist = MAX (
                        ewp->length,
                        bank->points[j].radius * ewp->radius_mult);
                    ...
                    if (dist_sqr < max_wash_dist * max_wash_dist) {
                        ...
                        if (dist_sqr < ((radius_mult * radius_mult) *
                                        (bank->points[j].radius *
                                         bank->points[j].radius))) {
                            vm_vec_cross (
                                &temp, &world_thruster_norm,
                                &thruster_to_ship);
                            vm_vec_scale_add2 (
                                &shipp->wash_rot_axis, &temp,
                                dot_to_ship / dist_sqr);
                            ship_intensity +=
                                (1.0f -
                                 dist_sqr / (max_wash_dist * max_wash_dist));
                            if (!do_damage) {
                                if (dist_sqr <
                                    0.25 * max_wash_dist * max_wash_dist) {
                                    do_damage = 1;
                                }
                            }
                        }
                            ...
                            if (vm_vec_dot (
                                    &apex_to_ship, &world_thruster_norm) >
                                cosf (half_angle)) {
                                vm_vec_cross (
                                    &temp, &world_thruster_norm,
                                    &thruster_to_ship);
    ...
    if (shipp->wash_intensity > 0) {
        ...
        if (!do_damage) { damage = 0; }
            ...
            damage =
                (0.001f * 0.003f * ENGINE_WASH_CHECK_INTERVAL *
                 shipp->ship_max_hull_strength * shipp->wash_intensity);
        }
        ...
        if (started_with_no_wash) {
            if (shipp != Player_ship) {
                obj_snd_assign (
                    shipp->objnum, GameSounds::ENGINE_WASH, &vmd_zero_vector,
                    1);
            }
            else {
                Player_engine_wash_loop = snd_play_looping (
                    gamesnd_get_game_sound (GameSounds::ENGINE_WASH), 0.0f, -1,
                    -1, 1.0f);
            }
        }
    }
...
int CombinedVariable::getString (char* output, size_t output_max) {
    ...
    if (Type == TYPE_SOUND) {
        ASSERTX (0, "Sound CombinedVariables are not supported yet.");
...
bool CombinedVariable::isEmpty () { return (Type != TYPE_NONE); }

void parse_combined_variable_list (
    CombinedVariable* dest, flag_def_list* src, size_t num) {
    if (dest == NULL || src == NULL || num == 0) return;
...
//********************-----CLASS: WarpEffect-----********************//
WarpEffect::WarpEffect () { this->clear (); }
...
int WarpEffect::warpFrame (float /*frametime*/) { return 0; }

int WarpEffect::warpShipClip (model_render_params* /*render_info*/) {
    return 0;
}

int WarpEffect::warpShipRender () { return 0; }

int WarpEffect::warpShipQueueRender (model_draw_list* /*scene*/) { return 0; }
...
int WarpEffect::warpEnd () {
    ...
    if (direction == WD_WARP_OUT) ship_actually_depart (objp->instance);
...
int WE_Default::warpStart () {
    ...
    if ((direction == WD_WARP_IN) &&
        shipfx_special_warp_objnum_valid (shipp->special_warpin_objnum)) {
        portal_objp = &Objects[shipp->special_warpin_objnum];
    }
    else if (
        (direction == WD_WARP_OUT) &&
        shipfx_special_warp_objnum_valid (shipp->special_warpout_objnum)) {
    ...
    if (direction == WD_WARP_IN) {
        polymodel* pm = model_get (sip->model_num);
        vm_vec_scale_add (
            &pos, &objp->pos, &objp->orient.vec.fvec,
            (pm != NULL) ? -pm->mins.xyz.z : objp->radius);

        // Effect time is 'SHIPFX_WARP_DELAY' (1.5 secs) seconds to start,
        // 'shipfx_calculate_warp_time' for ship to go thru, and
        // 'SHIPFX_WARP_DELAY' (1.5 secs) to go away.
        warp_time = shipfx_calculate_warp_time (objp, WD_WARP_IN) +
                    SHIPFX_WARP_DELAY + SHIPFX_WARP_DELAY;
    }
    else {
    ...
    if (direction == WD_WARP_OUT) {
        ...
        int fireball_type =
            ((portal_objp != NULL) || (sip->warpout_type == WT_KNOSSOS) ||
             (sip->warpout_type == WT_DEFAULT_THEN_KNOSSOS))
                ? FIREBALL_KNOSSOS
                : FIREBALL_WARP;

        // create fireball
        warp_objnum = fireball_create (
            &pos, fireball_type, FIREBALL_WARP_EFFECT, OBJ_INDEX (objp),
            radius, 1, NULL, warp_time, shipp->ship_info_index, NULL, 0, 0,
            sip->warpout_snd_start, sip->warpout_snd_end);
    }
    else if (direction == WD_WARP_IN) {
        // maybe special warpin
        int fireball_type =
            ((portal_objp != NULL) || (sip->warpin_type == WT_KNOSSOS))
                ? FIREBALL_KNOSSOS
                : FIREBALL_WARP;
        ...
        warp_objnum = fireball_create (
            &pos, fireball_type, FIREBALL_WARP_EFFECT, OBJ_INDEX (objp),
            radius, 0, NULL, warp_time, shipp->ship_info_index, NULL, 0, 0,
    ...
    if (direction == WD_WARP_IN) {
        ...
        // it's current position to the warp_effect_pos;
        ...
        if (objp != Player_obj || Player_use_ai) {
            ...
            objp->phys_info.forward_thrust =
                1.0f; // How much the forward thruster is applied.  0-1.
        }
    }
...
int WE_Default::warpFrame (float frametime) {
    if (direction == WD_WARP_IN) {
        if ((shipp->flags[Ship::Ship_Flags::Arriving_stage_1]) &&
            timestamp_elapsed (stage_time_end)) {
            ...
            float speed =
                shipfx_calculate_warp_dist (objp) /
                warp_time; // How long it takes to move through warp effect

            // Make ship move at velocity so that it moves two radii in
            // warp_time seconds.
            vec3d vel;
            vel = objp->orient.vec.fvec;
        ...
        vec3d tempv;
        float
            warp_pos; // position of warp effect in object's frame of reference
        ...
        if (objp == Player_obj) {
            ...
            // MWA 10/21/97 -- added
            // shipp->flags[Ship::Ship_Flags::No_departure_warp] part of next
            // if statement.  For ships that don't get a wormhole effect, I
            // wanted to drop into this code immediately.
            if ((warp_pos > objp->radius) ||
                (shipp->flags[Ship::Ship_Flags::No_departure_warp]) ||
                timed_out) {
                this->warpEnd ();
            }
        }
...
int WE_Default::warpShipRender () { return 1; }
...
WE_BSG::WE_BSG (object* n_objp, int n_direction)
    ...
    if (pm == NULL) {
        ...
        if (tube_radius <= 0.0f) tube_radius = objp->radius;

        if (shockwave_radius <= 0.0f) shockwave_radius = objp->radius;
    }
    else {
        // Autogenerate everything from ship dimensions
        if (tube_radius <= 0.0f)
            tube_radius = MAX (
                              (pm->maxs.xyz.y - pm->mins.xyz.y),
                              (pm->maxs.xyz.x - pm->mins.xyz.x)) /
                          2.0f;
    ...
    if (direction == WD_WARP_IN) {
        ...
        stage_duration[1] =
            MAX (anim_total_time - sip->warpin_time, shockwave_total_time);
    }
    else {
        stage_duration[0] = sip->warpout_time;
        stage_duration[1] =
            MAX (anim_total_time - sip->warpout_time, shockwave_total_time);
    }
    stage_time_start = stage_time_end = total_time_start = total_time_end =
        timestamp ();
...
WE_BSG::~WE_BSG () {
    if (anim > -1) bm_unload (anim);
    if (shockwave > -1) bm_unload (shockwave);
}

void WE_BSG::pageIn () {
    if (anim > -1) bm_page_in_texture (anim);
    if (shockwave > -1) bm_page_in_texture (shockwave);
}

int WE_BSG::warpStart () {
    if (!WarpEffect::warpStart ()) return 0;
    ...
    if (object_is_docked (objp)) {
        ...
        if (tube_radius <= 0.0f) {
            float x_radius =
                dock_calc_max_semilatus_rectum_parallel_to_axis (objp, X_AXIS);
            float y_radius =
                dock_calc_max_semilatus_rectum_parallel_to_axis (objp, Y_AXIS);
    ...
    if (gs_start_index.isValid ()) {
        ...
        snd_start = snd_play_3d (
            snd_start_gs, &objp->pos, &View_position, 0.0f, NULL, 0, 1,
...
int WE_BSG::warpFrame (float /*frametime*/) {
    ...
    if (snd_start.isValid ())
        snd_update_3d_pos (
            snd_start, snd_start_gs, &objp->pos, 0.0f, snd_range_factor);
...
int WE_BSG::warpShipClip (model_render_params* render_info) {
    ...
    if (direction == WD_WARP_OUT && stage > 0) {
        vec3d position;
        vm_vec_scale_add (
            &position, &objp->pos, &objp->orient.vec.fvec, objp->radius);
...
int WE_BSG::warpShipRender () {
    ...
    if (anim < 0 && shockwave < 0) return 0;

    // SUSHI: Turning off Zbuffering results in the FTL effect showing up
    // through ship hulls. The effect is slightly degraded by leaving it on,
    // but ATM it's worth the tradeoff. turn off zbuffering
    ...
    if (anim > -1) {
        ...
        if (anim_frame < anim_nframes) {
            ...
            // gr_set_bitmap(anim + anim_frame, GR_ALPHABLEND_FILTER,
            // GR_BITBLT_MODE_NORMAL, 1.0f);

            // Do warpout geometry
            vec3d start, end;
            vm_vec_scale_add (
                &start, &pos, &objp->orient.vec.fvec, z_offset_min);
            vm_vec_scale_add (
                &end, &pos, &objp->orient.vec.fvec, z_offset_max);

            // Render the warpout effect
            // batch_add_beam(anim + anim_frame, TMAP_FLAG_GOURAUD |
            // TMAP_FLAG_RGB | TMAP_FLAG_TEXTURED | TMAP_FLAG_CORRECT |
            // TMAP_HTL_3D_UNLIT, &start, &end, tube_radius*2.0f, 1.0f);
            batching_add_beam (
                anim + anim_frame, &start, &end, tube_radius * 2.0f, 1.0f);
        }
    }
    ...
    if (stage == 1 && shockwave > -1) {
        ...
        if (shockwave_frame < shockwave_nframes) {
            ...
            batching_add_volume_bitmap (
                shockwave + shockwave_frame, &p, 0, shockwave_radius, 1.0f);
        }
    }
...
int WE_BSG::warpEnd () {
    if (snd_start.isValid ()) snd_stop (snd_start);
    if (snd_end_gs != NULL)
        snd_end = snd_play_3d (
            snd_end_gs, &objp->pos, &View_position, 0.0f, NULL, 0, 1.0f,
            SND_PRIORITY_SINGLE_INSTANCE, NULL, snd_range_factor);
...
WE_Homeworld::WE_Homeworld (object* n_objp, int n_direction)
    ...
    if (direction == WD_WARP_IN)
        stage_duration[3] =
            sip->warpin_time - (stage_duration[1] + stage_duration[2] +
                                stage_duration[4] + stage_duration[5]);
    else if (direction == WD_WARP_OUT)
        stage_duration[3] =
            sip->warpout_time - (stage_duration[1] + stage_duration[2] +
                                 stage_duration[4] + stage_duration[5]);
    if (stage_duration[3] <= 0) stage_duration[3] = 3000;

    // Anim
    if (direction == WD_WARP_IN)
        anim = bm_load_either (
            sip->warpin_anim, &anim_nframes, &anim_fps, NULL, true);
    else if (direction == WD_WARP_OUT)
        anim = bm_load_either (
            sip->warpout_anim, &anim_nframes, &anim_fps, NULL, true);
    else
        anim = -1;
    ...
    if (pm != NULL) {
        ...
        if (width_full <= 0.0f) { width_full = 2.0f * objp->radius; }
        height_full = width_full;
        z_offset_max = objp->radius;
        z_offset_min = -objp->radius;
    }
    // WMC - This scales up or down the sound depending on ship size, with
    // ~100m diameter ship as base REMEMBER: Radius != diameter
    snd_range_factor =
        sqrt (width_full * width_full + height_full * height_full) /
        141.421356f;

    if (width_full <= 0.0f) width_full = 1.0f;
    if (height_full <= 0.0f) height_full = 1.0f;
    width = width_full;
    height = 0.0f;
...
int WE_Homeworld::warpStart () {
    ...
    if (direction == WD_WARP_OUT) vm_vec_negate (&fvec);
    ...
    if (gs_index.isValid ()) {
        ...
        snd = snd_play_3d (
            snd_gs, &pos, &View_position, 0.0f, NULL, 0, 1,
            SND_PRIORITY_SINGLE_INSTANCE, NULL, snd_range_factor);
    }
...
int WE_Homeworld::warpFrame (float /*frametime*/) {
    ...
    // Process stage
    float progress = ((float)timestamp () - (float)stage_time_start) /
                     ((float)stage_time_end - (float)stage_time_start);
    switch (stage) {
    case 1: height = height_full * progress; break;
    case 2: break;
    case 3:
        vm_vec_scale_add (
            &pos, &objp->pos, &objp->orient.vec.fvec,
        ...
        break;
    case 4: break;
    case 5: height = height_full * (1.0f - progress); break;
    default: this->warpEnd (); return 0;
    }
...
WE_Hyperspace::WE_Hyperspace (object* n_objp, int n_direction)
    ...
    if (total_duration <= 0) total_duration = 1000;
...
int WE_Hyperspace::warpStart () {
    ...
    if (direction == WD_WARP_IN) {
        shipp->flags.set (Ship::Ship_Flags::Arriving_stage_1);
        objp->phys_info.flags |= PF_WARP_IN;
        objp->phys_info.vel.xyz.z =
            (scale_factor / sip->warpin_time) * 1000.0f;
...
int WE_Hyperspace::warpFrame (float /*frametime*/) {
    ...
    if (timestamp_elapsed (total_time_end)) {
        ...
        float progress = ((float)timestamp () - (float)total_time_start) /
                         (float)total_duration;
        float scale = 0.0f;
        if (direction == WD_WARP_IN) {
            scale =
                scale_factor * (1.0f - pow ((1.0f - progress), decel_exp)) -
                scale_factor;
            ...
            // smoothly than this.
            scale = MIN (
                scale, (initial_velocity * (total_duration / 1000) *
                        -(1.0f - progress)));
        }
        else {
        ...
        vm_vec_scale_add (
            &objp->pos, &pos_final, &objp->orient.vec.fvec, scale);
    }
    return 1;
}
