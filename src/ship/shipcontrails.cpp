...
// update active contrails - moving existing ones, adding new ones where
// necessary
...
void ct_ship_process (ship* shipp) {
    ...
    if ((objp->type != OBJ_SHIP) ||
        (Ship_info[Ships[objp->instance].ship_info_index].ct_count <= 0)) {
        return;
    }
    ...
    if (ct_has_contrails (shipp)) { ct_update_contrails (shipp); }
...
int ct_has_contrails (ship* shipp) {
    ...
    for (idx = 0; idx < MAX_SHIP_CONTRAILS; idx++) {
        if (shipp->trail_ptr[idx] != NULL) { return 1; }
    }
...
void ct_create_contrails (ship* shipp) {
    ...
    for (idx = 0; idx < sip->ct_count; idx++) {
        // if (this is a neb mision and this is a neb trail) or an ABtrail
        // -Bobboau
...
void ct_ship_process_ABtrails (ship* shipp) {
    ...
    if (!(objp->phys_info.flags &
          PF_AFTERBURNER_ON)) { // if the after burners aren't on -Bobboau
        for (idx = 0; idx < MAX_SHIP_CONTRAILS; idx++) {
            if (shipp->ABtrail_ptr[idx] != NULL) {
                trail_object_died (shipp->ABtrail_ptr[idx]);
                shipp->ABtrail_ptr[idx] = NULL;
    ...
    if (ct_has_ABtrails (shipp)) { ct_update_ABtrails (shipp); }
    // otherwise add new ones
    else {
...
int ct_has_ABtrails (ship* shipp) {
    ...
    // if the ship has no afterburner trail bitmap, don't bother with anything
    if (sip->afterburner_trail.bitmap_id < 0) return 0;

    for (idx = 0; idx < MAX_SHIP_CONTRAILS; idx++) {
        if (shipp->ABtrail_ptr[idx] != NULL) { return 1; }
    }
...
bool ct_display_contrails () {
    ...
    display =
        (The_mission.flags[Mission::Mission_Flags::Fullneb]) ? true : false;
