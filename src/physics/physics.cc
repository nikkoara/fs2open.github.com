        ...
        vec3d *position, matrix *orient, physics_info *pi, float sim_time)
{
        ...
        if (pi->flags & PF_CONST_VEL) {
                ...
                pi->speed = vm_vec_mag(&pi->vel); // Note, cannot use quick version, causes
        ...
        vec3d *wash_rot)
{
        ...
        if (pi->flags & PF_ACCELERATES) {
                ...
                if ((pi->flags & PF_GLIDING) || (pi->flags & PF_FORCE_GLIDE)) {
                        ...
                        if (curGlideCap >= 0.0f && vm_vec_mag(&pi->desired_vel) >= cap_decay_threshold * curGlideCap && vm_vec_mag(&goal_vel) > 0.0f && pi->glide_accel_mult != 0.0f) {
                                if (goal_vel.xyz.x == 0.0f)
                                        vm_vec_scale_add2(
        ...
        vec3d *impulse, vec3d *pos, physics_info *pi, matrix *orient, float mass)
{
        ...
        // Detect null vector.
        if ((fabsf(impulse->xyz.x) <= WHACK_LIMIT) && (fabsf(impulse->xyz.y) <= WHACK_LIMIT) && (fabsf(impulse->xyz.z) <= WHACK_LIMIT))
                return;
        ...
        vec3d *min, vec3d *max, float radius)
{
        ...
        if (normal.xyz.x < 0.0f) // normal < 0, hits the right face
                vm_vec_copy_scale(
                        ...
                        max->xyz.x * pressure * area.xyz.x * normal.xyz.x * sin.xyz.x / pi->mass);
        else // normal > 0, hits the left face
                vm_vec_copy_scale(
                        &impact_vec, &orient->vec.rvec,
                        min->xyz.x * pressure * area.xyz.x * -normal.xyz.x * sin.xyz.x / pi->mass);
        ...
        if (normal.xyz.y < 0.0f)
                vm_vec_copy_scale(
                        ...
                        max->xyz.y * pressure * area.xyz.y * normal.xyz.y * sin.xyz.y / pi->mass);
        else
                vm_vec_copy_scale(
                        &impact_vec, &orient->vec.uvec,
                        min->xyz.y * pressure * area.xyz.y * -normal.xyz.y * sin.xyz.y / pi->mass);
        ...
        if (normal.xyz.z < 0.0f)
                vm_vec_copy_scale(
                        ...
                        max->xyz.z * pressure * area.xyz.z * normal.xyz.z * sin.xyz.z / pi->mass);
        else
                vm_vec_copy_scale(
                        &impact_vec, &orient->vec.fvec,
                        min->xyz.z * pressure * area.xyz.z * -normal.xyz.z * sin.xyz.z / pi->mass);
...
int check_rotvel_limit(physics_info *pi)
{
        ...
        if (!(pi->flags & PF_DEAD_DAMP)) {
                ...
                if (fabsf(pi->rotvel.xyz.x) > pi->max_rotvel.xyz.x) {
                        pi->rotvel.xyz.x = (pi->rotvel.xyz.x / fabsf(pi->rotvel.xyz.x)) * (pi->max_rotvel.xyz.x - (float)ROTVEL_TOL);
                        change_made = 1;
                }
                if (fabsf(pi->rotvel.xyz.y) > pi->max_rotvel.xyz.y) {
                        pi->rotvel.xyz.y = (pi->rotvel.xyz.y / fabsf(pi->rotvel.xyz.y)) * (pi->max_rotvel.xyz.y - (float)ROTVEL_TOL);
                        change_made = 1;
                }
                if (fabsf(pi->rotvel.xyz.z) > pi->max_rotvel.xyz.z) {
                        pi->rotvel.xyz.z = (pi->rotvel.xyz.z / fabsf(pi->rotvel.xyz.z)) * (pi->max_rotvel.xyz.z - (float)ROTVEL_TOL);
                        change_made = 1;
                }
        } else {
                // case of dead ship
                if (fabsf(pi->rotvel.xyz.x) > DEAD_ROTVEL_CAP) {
                        pi->rotvel.xyz.x = (pi->rotvel.xyz.x / fabsf(pi->rotvel.xyz.x)) * (float)(DEAD_ROTVEL_CAP - ROTVEL_TOL);
                        change_made = 1;
                }
                if (fabsf(pi->rotvel.xyz.y) > DEAD_ROTVEL_CAP) {
                        pi->rotvel.xyz.y = (pi->rotvel.xyz.y / fabsf(pi->rotvel.xyz.y)) * (float)(DEAD_ROTVEL_CAP - ROTVEL_TOL);
                        change_made = 1;
                }
                if (fabsf(pi->rotvel.xyz.z) > DEAD_ROTVEL_CAP) {
                        pi->rotvel.xyz.z = (pi->rotvel.xyz.z / fabsf(pi->rotvel.xyz.z)) * (float)(DEAD_ROTVEL_CAP - ROTVEL_TOL);
                        change_made = 1;
                }
        }
        return change_made;
