...
#define PF_IN_SHOCKWAVE                                                         \
        (1 << 6) // Indicates whether object has recently been hit by shockwave
                 // (used to enable shake)
#define PF_DEAD_DAMP \
        (1 << 7) // Makes forward damping same as sideways (NOT reset in physics)
#define PF_AFTERBURNER_WAIT                                                       \
        (1 << 8) // true when afterburner cannot be used.  replaces variable used
                 // in afterburner code
#define PF_CONST_VEL \
...
struct physics_info {
        ...
        vec3d center_of_mass; // Goober5000 - this is never ever used by physics;
                              // currently physics assumes the center of an object
                              // is the center of mass
        matrix I_body_inv;    // inverse moment of inertia tensor (used to calculate
                              // rotational effects)
        ...
        vec3d afterburner_max_vel; // maximum foward velocity in x,y,z while
                                   // afterburner engaged
        ...
        int shockwave_decay;    // timestamp used to control how long ship affected
                                // after hit by shockwave
        int reduced_damp_decay; // timestamp used to control how long ship ship has
                                // reduced damp physics

        float glide_cap;     // Backslash - for 'newtonian'-style gliding, the cap on
                             // velocity (so that something can't accelerate to
...
struct control_info {
        ...
        // below is information that are used by the player controls for firing
        // information.
...
extern void physics_read_flying_controls(
        ...
        vec3d *wash_rot = NULL);
extern void physics_sim(
        vec3d *position, matrix *orient, physics_info *pi, float sim_time);
extern void physics_sim_editor(
        vec3d *position, matrix *orient, physics_info *pi, float sim_time);

extern void physics_sim_vel(
...
extern void physics_apply_whack(
        vec3d *force, vec3d *pos, physics_info *pi, matrix *orient, float mass);
extern void physics_apply_shock(
        vec3d *direction_vec, float pressure, physics_info *pi, matrix *orient,
        vec3d *min, vec3d *max, float radius);
extern void physics_collide_whack(
        vec3d *impulse, vec3d *delta_rotvel, physics_info *pi, matrix *orient,
        bool is_landing);
...
// this variable tracks the viewer's bank.  This is used for
...
// Acceleration, constant Velocity, Deceleration (to another speed, so maybe
// not deceleration)
class avd_movement {
private:
        // Current
...
#endif // FREESPACE2_PHYSICS_PHYSICS_HH
