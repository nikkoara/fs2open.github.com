...
//////////////////////////////////////////////////////////////////////////////
//
// (C) Copyright Vicente J. Botet Escriba 2013-2014. Distributed under the
...
enum queue_op_status { success = 0, empty, full, closed, busy };
...
private:
    ...
    try_pull_front (value_type& x, std::unique_lock< std::mutex >& lk);
    inline queue_op_status
    try_push_back (const value_type& x, std::unique_lock< std::mutex >& lk);
    inline queue_op_status
    try_push_back (value_type&& x, std::unique_lock< std::mutex >& lk);

    inline queue_op_status
    wait_pull_front (value_type& x, std::unique_lock< std::mutex >& lk);
    inline queue_op_status
    wait_push_back (const value_type& x, std::unique_lock< std::mutex >& lk);
    inline queue_op_status
    wait_push_back (value_type&& x, std::unique_lock< std::mutex >& lk);

    inline void wait_until_not_empty (std::unique_lock< std::mutex >& lk);
    inline void
    wait_until_not_empty (std::unique_lock< std::mutex >& lk, bool&);
    inline size_type wait_until_not_full (std::unique_lock< std::mutex >& lk);
    inline size_type
    wait_until_not_full (std::unique_lock< std::mutex >& lk, bool&);

    inline void
    notify_not_empty_if_needed (std::unique_lock< std::mutex >& lk) {
        if (waiting_empty_ > 0) {
            --waiting_empty_;
            lk.unlock ();
    ...
    inline void
    notify_not_full_if_needed (std::unique_lock< std::mutex >& lk) {
        if (waiting_full_ > 0) {
            --waiting_full_;
            lk.unlock ();
    ...
    inline void
    pull_front (value_type& elem, std::unique_lock< std::mutex >& lk) {
    ...
    inline void push_at (
        const value_type& elem, size_type in_p_1,
    ...
    inline void push_at (
        value_type&& elem, size_type in_p_1,
        std::unique_lock< std::mutex >& lk) {
...
sync_bounded_queue< ValueType >::sync_bounded_queue (
    typename sync_bounded_queue< ValueType >::size_type max_elems)
    : waiting_full_ (0), waiting_empty_ (0),
      data_ (new value_type[max_elems + 1]), in_ (0), out_ (0),
      capacity_ (max_elems + 1), closed_ (false) {}
...
template< typename ValueType >
bool sync_bounded_queue< ValueType >::closed (
...
sync_bounded_queue< ValueType >::capacity () const {
    std::lock_guard< std::mutex > lk (mtx_);
    return capacity (lk);
}
...
template< typename ValueType >
typename sync_bounded_queue< ValueType >::size_type
sync_bounded_queue< ValueType >::size () const {
    std::lock_guard< std::mutex > lk (mtx_);
    return size (lk);
}

template< typename ValueType >
queue_op_status sync_bounded_queue< ValueType >::try_pull_front (
    ValueType& elem, std::unique_lock< std::mutex >& lk) {
    if (empty (lk)) {
...
template< typename ValueType >
queue_op_status
sync_bounded_queue< ValueType >::try_pull_front (ValueType& elem) {
...
template< typename ValueType >
queue_op_status
sync_bounded_queue< ValueType >::nonblocking_pull_front (ValueType& elem) {
...
template< typename ValueType >
void sync_bounded_queue< ValueType >::throw_if_closed (
    std::unique_lock< std::mutex >&) {
    if (closed_) { throw sync_queue_is_closed (); }
}

template< typename ValueType >
void sync_bounded_queue< ValueType >::wait_until_not_empty (
    std::unique_lock< std::mutex >& lk) {
    for (;;) {
        if (out_ != in_) break;
        throw_if_closed (lk);
...
template< typename ValueType >
void sync_bounded_queue< ValueType >::wait_until_not_empty (
    std::unique_lock< std::mutex >& lk, bool& _closed) {
    for (;;) {
        if (out_ != in_) break;
        if (closed_) {
            _closed = true;
...
template< typename ValueType >
queue_op_status sync_bounded_queue< ValueType >::wait_pull_front (
    ValueType& elem, std::unique_lock< std::mutex >& lk) {
...
template< typename ValueType >
queue_op_status
sync_bounded_queue< ValueType >::wait_pull_front (ValueType& elem) {
    std::unique_lock< std::mutex > lk (mtx_);
    return wait_pull_front (elem, lk);
}

template< typename ValueType >
queue_op_status sync_bounded_queue< ValueType >::try_push_back (
    const ValueType& elem, std::unique_lock< std::mutex >& lk) {
    if (closed (lk)) return queue_op_status::closed;
...
template< typename ValueType >
queue_op_status
sync_bounded_queue< ValueType >::try_push_back (const ValueType& elem) {
    std::unique_lock< std::mutex > lk (mtx_);
    return try_push_back (elem, lk);
}

template< typename ValueType >
queue_op_status sync_bounded_queue< ValueType >::wait_push_back (
    const ValueType& elem, std::unique_lock< std::mutex >& lk) {
    if (closed (lk)) return queue_op_status::closed;
...
template< typename ValueType >
queue_op_status
sync_bounded_queue< ValueType >::wait_push_back (const ValueType& elem) {
    std::unique_lock< std::mutex > lk (mtx_);
    return wait_push_back (elem, lk);
}

template< typename ValueType >
queue_op_status sync_bounded_queue< ValueType >::nonblocking_push_back (
...
template< typename ValueType >
typename sync_bounded_queue< ValueType >::size_type
sync_bounded_queue< ValueType >::wait_until_not_full (
    std::unique_lock< std::mutex >& lk) {
    for (;;) {
        throw_if_closed (lk);
        size_type in_p_1 = inc (in_);
...
template< typename ValueType >
queue_op_status
sync_bounded_queue< ValueType >::try_push_back (ValueType&& elem) {
    std::unique_lock< std::mutex > lk (mtx_);
...
template< typename ValueType >
queue_op_status
sync_bounded_queue< ValueType >::wait_push_back (ValueType&& elem) {
    std::unique_lock< std::mutex > lk (mtx_);
    return try_push_back (std::move (elem), lk);
}
...
template< typename ValueType >
queue_op_status
sync_bounded_queue< ValueType >::nonblocking_push_back (ValueType&& elem) {
    std::unique_lock< std::mutex > lk (mtx_, std::try_lock);
    if (!lk.owns_lock ()) { return queue_op_status::busy; }
    return try_push_back (std::move (elem), lk);
}
...
operator<< (sync_bounded_queue< ValueType >& sbq, ValueType&& elem) {
