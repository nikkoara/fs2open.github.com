...
int Ship_select_open =
    0; // This game-wide global flag is set to 1 to indicate that the ship
...
int Commit_pressed; // flag to indicate that the commit button was pressed
                    // use a flag, so the ship_create() can be done at the end
                    // of the loop
...
static int Ss_delta_x, Ss_delta_y; // used to offset the carried icon to make
                                   // it smoothly leave static position
...
int Selected_ss_class; // set to ship class of selected ship, -1 if none
                       // selected
...
struct ss_buttons {
    ...
    ss_buttons (
        const char* name, int x1, int y1, int xt1, int yt1, int h, int s)
...
static bitmap*
    ShipSelectMaskPtr; // bitmap pointer to the ship select mask bitmap
...
int ss_icon_being_carried () {
    if (Carried_ss_icon.ship_class >= 0) { return 1; }
...
void ss_set_carried_icon (int from_slot, int ship_class) {
    ...
    Ship_select_buttons[gr_screen.res][SS_BUTTON_DUMMY]
        .button.capture_mouse ();
}

// clear all active list items, and reset the flags inside the
// SS_active_items[] array
void clear_active_list () {
    int i;
    for (i = 0; i < static_cast< int > (Ship_info.size ());
         i++) { // DTP singleplayer ship choice fix
...
ss_active_item* get_free_active_list_node () {
    ...
    for (i = 0; i < static_cast< int > (Ship_info.size ()); i++) {
        // for ( i = 0; i < MAX_WSS_SLOTS; i++ ) { //DTP, ONLY MAX_WSS_SLOTS
        // SHIPS ???
...
// Build up the ship selection active list, which is a list of all ships that
...
void ship_select_check_buttons () {
    ...
    for (i = 0; i < NUM_SS_BUTTONS; i++) {
        ...
        if (b->button.pressed ()) { ship_select_button_do (b->hotspot); }
    }
}
...
//
// Redraw any ship select buttons that are pressed down.  This function is
...
void ship_select_redraw_pressed_buttons () {
    ...
    for (i = 0; i < NUM_SS_BUTTONS; i++) {
        b = &Ship_select_buttons[gr_screen.res][i];
        if (b->button.pressed ()) { b->button.draw_forced (2); }
    }
}
...
void ship_select_buttons_init () {
    ...
    for (i = 0; i < NUM_SS_BUTTONS; i++) {
        b = &Ship_select_buttons[gr_screen.res][i];
        b->button.create (
            &Ship_select_ui_window, "", b->x, b->y, 60, 30, b->scrollable);
...
// -------------------------------------------------------------------------------------
// ship_select_button_do() do the button action for the specified pressed
// button
//
void ship_select_button_do (int i) {
    ...
    case SHIP_SELECT_SHIP_SCROLL_UP:
        if (Current_screen != ON_SHIP_SELECT) break;

        if (common_scroll_down_pressed (
                &SS_active_list_start, SS_active_list_size,
                MAX_ICONS_ON_SCREEN)) {
    ...
    case SHIP_SELECT_RESET: ss_reset_to_default (); break;
    } // end switch
}
...
void ship_select_init () {
    ...
    if (Ship_select_open) {
        ...
        common_buttons_maybe_reload (
            &Ship_select_ui_window); // AL 11-21-97: this is necessary since we
                                     // may returning from the hotkey screen,
    ...
    ShipSelectMaskPtr =
        bm_lock (ShipSelectMaskBitmap, 8, BMP_AABITMAP | BMP_MASK_BITMAP);
    ...
    snazzy_menu_add_region (
        &Region[Num_mask_regions++], "", COMMON_BRIEFING_REGION, 0);
    snazzy_menu_add_region (
        &Region[Num_mask_regions++], "", COMMON_SS_REGION, 0);
    snazzy_menu_add_region (
        &Region[Num_mask_regions++], "", COMMON_WEAPON_REGION, 0);
    snazzy_menu_add_region (
        &Region[Num_mask_regions++], "", COMMON_COMMIT_REGION, 0);
    snazzy_menu_add_region (
        &Region[Num_mask_regions++], "", COMMON_HELP_REGION, 0);
    snazzy_menu_add_region (
        &Region[Num_mask_regions++], "", COMMON_OPTIONS_REGION, 0);

    snazzy_menu_add_region (
        &Region[Num_mask_regions++], "", SHIP_SELECT_SHIP_SCROLL_UP, 0);
    snazzy_menu_add_region (
        &Region[Num_mask_regions++], "", SHIP_SELECT_SHIP_SCROLL_DOWN, 0);

    snazzy_menu_add_region (
        &Region[Num_mask_regions++], "", SHIP_SELECT_ICON_0, 0);
    snazzy_menu_add_region (
        &Region[Num_mask_regions++], "", SHIP_SELECT_ICON_1, 0);
    snazzy_menu_add_region (
        &Region[Num_mask_regions++], "", SHIP_SELECT_ICON_2, 0);
    snazzy_menu_add_region (
        &Region[Num_mask_regions++], "", SHIP_SELECT_ICON_3, 0);
    ...
    Ship_select_open =
        1; // This game-wide global flag is set to 1 to indicate that the ship
           // select screen has been opened and memory allocated.  This flag
           // is needed so we can know if ship_select_close() needs to called
           // if restoring a game from the Options screen invoked from ship
           // select

    // init ship selection masks and buttons
    Ship_select_ui_window.create (
        0, 0, gr_screen.max_w_unscaled, gr_screen.max_h_unscaled, 0);
    Ship_select_ui_window.set_mask_bmap (
        Ship_select_background_mask_fname[gr_screen.res]);
    ...
    Ship_select_background_bitmap = mission_ui_background_load (
        Briefing->ship_select_background[gr_screen.res],
        Ship_select_background_fname[gr_screen.res]);

    // init ship selection ship model rendering window
    start_ship_animation (Selected_ss_class);
}

// Return the ship class for the icon specified by index.  Need to iterate
// through the active list of icons to find out what ship class for this icon
//
...
int ss_get_ship_class_from_list (int index) {
    ...
    count = 0;
    list_entry = -1;
    for (sai = GET_FIRST (&SS_active_head);
         sai != END_OF_LIST (&SS_active_head); sai = GET_NEXT (sai)) {
        count++;
        if (count <= SS_active_list_start) continue;

        if (i >= MAX_ICONS_ON_SCREEN) break;
...
void maybe_pick_up_list_icon (int offset) {
    ...
    if (ship_class != -1) { pick_from_ship_list (offset, ship_class); }
}
...
void maybe_change_selected_ship (int offset) {
    ...
    if (Ss_mouse_down_on_region != (SHIP_SELECT_ICON_0 + offset)) { return; }
...
void maybe_change_selected_wing_ship (int wb_num, int ws_num) {
    ...
    if (Selected_ss_class != -1 &&
        Selected_ss_class !=
            Wss_slots[wb_num * MAX_WING_SLOTS + ws_num].ship_class) {
        Selected_ss_class =
            Wss_slots[wb_num * MAX_WING_SLOTS + ws_num].ship_class;
        start_ship_animation (Selected_ss_class, 1);
    }
...
int do_mouse_over_wing_slot (int block, int slot) {
    ...
    if (!mouse_down (MOUSE_LEFT_BUTTON)) {
        if (ss_icon_being_carried ()) {
            ...
            ss_drop (
                Carried_ss_icon.from_slot, Carried_ss_icon.ship_class,
                Hot_ss_slot, -1);
            ss_reset_carried_icon ();
        }
    }
    else {
        if (Ss_mouse_down_on_region ==
            (WING_0_SHIP_0 + block * MAX_WING_SLOTS + slot)) {
...
void do_mouse_over_list_slot (int index) {
    ...
    if (Ss_mouse_down_on_region != (SHIP_SELECT_ICON_0 + index)) { return; }

    if (mouse_down (MOUSE_LEFT_BUTTON)) maybe_pick_up_list_icon (index);
}
    ...
    if (Drop_icon_mflag) {
        if (ss_icon_being_carried ()) {
            ...
            if (Carried_ss_icon.from_slot >= 0) {
                // return to list
                ss_drop (
                    Carried_ss_icon.from_slot, -1, -1,
                    Carried_ss_icon.ship_class);
            }
            else {
                if (ss_carried_icon_moved ()) {
                    gamesnd_play_iface (InterfaceSounds::ICON_DROP);
...
void ss_maybe_flash_button () {
    if (common_flash_bright ()) {
        ...
        if (Common_buttons[Current_screen - 1][gr_screen.res][2]
                .button.button_hilighted ()) {
            common_flash_button_init ();
        }
        else {
            Common_buttons[Current_screen - 1][gr_screen.res][2]
...
void ship_select_blit_ship_info () {
    ...
    if ((sip->ship_length != NULL) && strlen (sip->ship_length)) {
        if (Lcl_gr || Lcl_pl) {
            // in german and polish, drop the s from Meters and make sure M is
            // caps
        ...
        gr_string (
            Ship_info_coords[gr_screen.res][SHIP_SELECT_X_COORD] + 4, y_start,
            sip->ship_length, GR_RESIZE_MENU);
    }
    ...
    if ((sip->gun_mounts != NULL) && strlen (sip->gun_mounts)) {
        ...
        int sum = 0;
        polymodel* pm = model_get (ShipSelectModelNum);
        for (i = 0; i < pm->n_guns; i++) { sum += pm->gun_banks[i].num_slots; }
        if (sum != 0)
            sprintf (str, "%d", sum);
        else
            strcpy_s (str, "None");
        ...
        gr_string (
            Ship_info_coords[gr_screen.res][SHIP_SELECT_X_COORD], y_start,
            XSTR ("Gun Banks", 1626), GR_RESIZE_MENU);
        y_start += line_height;
        gr_set_color_fast (text);
    ...
    if ((sip->missile_banks != NULL) && strlen (sip->missile_banks)) {
        gr_string (
            Ship_info_coords[gr_screen.res][SHIP_SELECT_X_COORD] + 4, y_start,
            sip->missile_banks, GR_RESIZE_MENU);
    }
    else {
    ...
    if (ShipSelectModelNum >= 0) {
        ...
        for (x = 0; x < sip->n_subsystems; x++) {
            if (sip->subsystems[x].type == SUBSYSTEM_TURRET) num_turrets++;
            /*
            for(y = 0; y < MAX_SHIP_PRIMARY_BANKS || y <
            MAX_SHIP_SECONDARY_BANKS; y++)
            {
        ...
        if (num_turrets) {
            ...
            gr_string (
                Ship_info_coords[gr_screen.res][SHIP_SELECT_X_COORD], y_start,
                XSTR ("Turrets", 1627), GR_RESIZE_MENU);
            y_start += line_height;
            gr_set_color_fast (text);
            sprintf (str, "%d", num_turrets);
            gr_string (
                Ship_info_coords[gr_screen.res][SHIP_SELECT_X_COORD] + 4,
    ...
    if ((sip->manufacturer_str != NULL) && strlen (sip->manufacturer_str)) {
        gr_string (
            Ship_info_coords[gr_screen.res][SHIP_SELECT_X_COORD] + 4, y_start,
            sip->manufacturer_str, GR_RESIZE_MENU);
    }
    else {
        gr_string (
            Ship_info_coords[gr_screen.res][SHIP_SELECT_X_COORD] + 4, y_start,
            Species_info[sip->species].species_name, GR_RESIZE_MENU);
    }
    y_start += line_height;
    ...
    char Ship_select_ship_info_lines[MAX_NUM_SHIP_DESC_LINES]
    ...
    if (Ship_select_ship_info_text[0] != '\0') {
        ...
        // MageKing17: Changed to use the widths determined by Yarn here:
        // http://scp.indiegames.us/mantis/view.php?id=3144#c16516
        n_lines = split_str (
            Ship_select_ship_info_text, gr_screen.res == GR_640 ? 204 : 328,
            n_chars, p_str, MAX_NUM_SHIP_DESC_LINES, 0);
    ...
    for (int idx = 0; idx < Ship_select_ship_info_line_count; idx++) {
        gr_string (
            Ship_info_coords[gr_screen.res][SHIP_SELECT_X_COORD] + 4, y_start,
            Ship_select_ship_info_lines[idx], GR_RESIZE_MENU);
        y_start += line_height;
    }
}
...
void ship_select_do (float frametime) {
    ...
    ship_select_choice = snazzy_menu_do (
        ShipSelectMaskData, Shipselect_mask_w, Shipselect_mask_h,
        Num_mask_regions, Region, &snazzy_action, 0);
    ...
    if (ship_select_choice > -1) {
        ...
        case SHIP_SELECT_ICON_0: do_mouse_over_list_slot (0); break;
        case SHIP_SELECT_ICON_1: do_mouse_over_list_slot (1); break;
        case SHIP_SELECT_ICON_2: do_mouse_over_list_slot (2); break;
        case SHIP_SELECT_ICON_3: do_mouse_over_list_slot (3); break;
        case WING_0_SHIP_0:
            if (do_mouse_over_wing_slot (0, 0)) ship_select_choice = -1;
            break;
        case WING_0_SHIP_1:
            if (do_mouse_over_wing_slot (0, 1)) ship_select_choice = -1;
            break;
        case WING_0_SHIP_2:
            if (do_mouse_over_wing_slot (0, 2)) ship_select_choice = -1;
            break;
        case WING_0_SHIP_3:
            if (do_mouse_over_wing_slot (0, 3)) ship_select_choice = -1;
            break;
        case WING_1_SHIP_0:
            if (do_mouse_over_wing_slot (1, 0)) ship_select_choice = -1;
            break;
        case WING_1_SHIP_1:
            if (do_mouse_over_wing_slot (1, 1)) ship_select_choice = -1;
            break;
        case WING_1_SHIP_2:
            if (do_mouse_over_wing_slot (1, 2)) ship_select_choice = -1;
            break;
        case WING_1_SHIP_3:
            if (do_mouse_over_wing_slot (1, 3)) ship_select_choice = -1;
            break;
        case WING_2_SHIP_0:
            if (do_mouse_over_wing_slot (2, 0)) ship_select_choice = -1;
            break;
        case WING_2_SHIP_1:
            if (do_mouse_over_wing_slot (2, 1)) ship_select_choice = -1;
            break;
        case WING_2_SHIP_2:
            if (do_mouse_over_wing_slot (2, 2)) ship_select_choice = -1;
            break;
        case WING_2_SHIP_3:
            if (do_mouse_over_wing_slot (2, 3)) ship_select_choice = -1;
            break;
    ...
    if (ship_select_choice > -1 && snazzy_action == SNAZZY_CLICKED) {
        switch (ship_select_choice) {
        case SHIP_SELECT_ICON_0: maybe_change_selected_ship (0); break;

        case SHIP_SELECT_ICON_1: maybe_change_selected_ship (1); break;

        case SHIP_SELECT_ICON_2: maybe_change_selected_ship (2); break;

        case SHIP_SELECT_ICON_3: maybe_change_selected_ship (3); break;

        case WING_0_SHIP_0: maybe_change_selected_wing_ship (0, 0); break;

        case WING_0_SHIP_1: maybe_change_selected_wing_ship (0, 1); break;

        case WING_0_SHIP_2: maybe_change_selected_wing_ship (0, 2); break;

        case WING_0_SHIP_3: maybe_change_selected_wing_ship (0, 3); break;

        case WING_1_SHIP_0: maybe_change_selected_wing_ship (1, 0); break;

        case WING_1_SHIP_1: maybe_change_selected_wing_ship (1, 1); break;

        case WING_1_SHIP_2: maybe_change_selected_wing_ship (1, 2); break;

        case WING_1_SHIP_3: maybe_change_selected_wing_ship (1, 3); break;

        case WING_2_SHIP_0: maybe_change_selected_wing_ship (2, 0); break;

        case WING_2_SHIP_1: maybe_change_selected_wing_ship (2, 1); break;

        case WING_2_SHIP_2: maybe_change_selected_wing_ship (2, 2); break;

        case WING_2_SHIP_3: maybe_change_selected_wing_ship (2, 3); break;
    ...
    if (!Cmdline_ship_choice_3d &&
        ...
        generic_anim_render (
            &Ss_icons[Selected_ss_class].ss_anim,
            (help_overlay_active (Ship_select_overlay_id)) ? 0 : frametime,
            Ship_anim_coords[gr_screen.res][0],
            Ship_anim_coords[gr_screen.res][1], true);
    }
    else {
        GR_DEBUG_SCOPE ("Render ship models");
        // The new rendering code for 3D ships courtesy your friendly
        // UnknownPlayer :)
        ...
        if ((Selected_ss_class >= 0) && (ShipSelectModelNum >= 0)) {
            ...
            if (sip->uses_team_colors) {
                render_info.set_team_color (
                    sip->default_team_name, "none", 0, 0);
            }

            if (sip->replacement_textures.size () > 0) {
                render_info.set_replacement_textures (
                    ShipSelectModelNum, sip->replacement_textures);
            }

            draw_model_rotating (
                &render_info, ShipSelectModelNum,
                Ship_anim_coords[gr_screen.res][0],
                Ship_anim_coords[gr_screen.res][1],
                Tech_ship_display_coords[gr_screen.res][2],
                Tech_ship_display_coords[gr_screen.res][3],
                &ShipSelectScreenShipRot, &sip->closeup_pos,
                sip->closeup_zoom * 1.3f, rev_rate,
                MR_AUTOCENTER | MR_NO_FOGGING, GR_RESIZE_MENU,
                sip->selection_effect);
        }
    }

    // The background transition plays once. Display ship icons after
    // Background done playing
    if (!Background_playing) {
    ...
    if (ss_icon_being_carried ()) {
        ...
        if (Ss_icons[Carried_ss_icon.ship_class]
                .icon_bmaps[ICON_FRAME_SELECTED] != -1) {
            gr_set_bitmap (Ss_icons[Carried_ss_icon.ship_class]
                               .icon_bmaps[ICON_FRAME_SELECTED]);
            ...
            if (Ss_icons[Carried_ss_icon.ship_class].model_index != -1) {
                draw_model_icon (
                    ...
                    sip->closeup_zoom / 1.25f, sx, sy, w, h, sip,
                    GR_RESIZE_MENU);
            }
        }
    }
...
void ship_select_close () {
    ...
    Ship_anim_class = -1;
    Ship_select_open =
        0; // This game-wide global flag is set to 0 to indicate that the ship
...
void draw_ship_icon_with_number (int screen_offset, int ship_class) {
        ...
        if (Hot_ss_icon == screen_offset) {
            if (ss_icon->model_index == -1)
                ss_icon->current_icon_bitmap =
                    ss_icon->icon_bmaps[ICON_FRAME_HOT];
            else
    ...
    if (Selected_ss_class > -1) {
        if (Selected_ss_class == ship_class) {
            if (ss_icon->model_index == -1)
                ss_icon->current_icon_bitmap =
                    ss_icon->icon_bmaps[ICON_FRAME_SELECTED];
            else
                color_to_draw = &Icon_colors[ICON_FRAME_SELECTED];
        }
    }

    if (Ss_pool[ship_class] <= 0) { return; }
    ...
    if (ss_icon->current_icon_bitmap != -1) {
        ...
        gr_bitmap (
            ...
            Ship_list_coords[gr_screen.res][screen_offset][1], GR_RESIZE_MENU);
    }
    else {
        ...
        graphics::line_draw_list line_draw_list;
        draw_brackets_square (
            &line_draw_list, Ship_list_coords[gr_screen.res][screen_offset][0],
...
void start_ship_animation (int ship_class, int /*play_sound*/) {
    ...
    if (Cmdline_ship_choice_3d || !strlen (sip->anim_filename)) {
        ...
        if (Ship_anim_class > 0 &&
            Ss_icons[Ship_anim_class].ss_anim.num_frames > 0) {
        ...
        Ss_icons[ship_class].ss_anim.ani.bg_type =
            bm_get_type (Ship_select_background_bitmap);
        ...
        if (Wss_slots[slot].ship_class >= 0) {
            for (int bank = 0; bank < MAX_SHIP_WEAPONS; bank++) {
                ...
                if (Wss_slots[slot].wep_count[bank] > 0) {
                    if (Wss_slots[slot].wep[bank] == weapon_index) return true;
                }
            }
        }
    }
...
// ------------------------------------------------------------------------
// commit_pressed() is called when the commit button from any of the
// briefing/ship select/ weapon select screens is pressed.  The ship selected
// is created, and the interface music is stopped.
void commit_pressed () {
    ...
    if (Wss_num_wings > 0) {
        ...
        if (wl_update_ship_weapons (
                Ships[Player_obj->instance].objnum, &Wss_slots[0]) == -1) {
            popup (
                PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                XSTR ("Player ship has no weapons", 461));
            return;
        }
    }
    ...
    for (j = 0; j < MAX_WEAPON_TYPES; j++) {
        if (Team_data[Common_team].weapon_required[j]) {
            ...
            if (num_required_weapons > 1) weapon_list.append (1, EOLN);
            weapon_list.append (Weapon_info[j].get_display_string ());

            // see if it's carried by any ship
            if (is_weapon_carried (j)) num_satisfied_weapons++;
        }
    }
    if (num_satisfied_weapons < num_required_weapons) {
        if (num_required_weapons == 1) {
            popup (
                PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                XSTR (
                    "The %s is required for this mission, but it has not been "
            ...
            popup (
                PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                XSTR (
                    "The following weapons are required for this mission, but "
                    "at least one of them has not been added to any ship "
                    "loadout:\n\n%s",
                    1625),
                weapon_list.c_str ());
            return;
    ...
    // Check to ensure that the hotkeys are still pointing to valid objects. It
    // is possible for the player to assign a ship to a hotkey, then go and
    // delete that ship in the ship selection, and then try to start the
    // mission.  This function will detect those objects, and remove them from
    // the hotkey linked lists.
    ...
    // warp the mouse cursor the the middle of the screen for those who control
    // with a mouse
...
int pick_from_ship_list (int screen_offset, int ship_class) {
    ...
    if (ss_icon_being_carried ()) return rval;
...
// and slot number (ws_num) can be picked up (for drag and drop).  It
// calculates the difference in x & y between the icon and the mouse, so we can
...
void pick_from_wing (int wb_num, int ws_num) {
    ...
    if (wb->wingnum < 0) return;

    // Take care of case where the mouse button goes from up to down in one
    // frame while carrying an icon
    ...
    if (ss_icon_being_carried ()) return;

    if (ss_disabled_slot (slot_index)) { return; }

    if (ws->status & WING_SLOT_IGNORE_SHIPS) { return; }
...
void draw_wing_block (
    ...
    if (wb->wingnum == -1) return;

    // print the wing name under the wing
    wp = &Wings[wb->wingnum];
    gr_get_string_size (&w, &h, wp->name);
    sx = Wing_icon_coords[gr_screen.res][wb_num * MAX_WING_SLOTS][0] + 16 -
         w / 2;
    sy = Wing_icon_coords[gr_screen.res][wb_num * MAX_WING_SLOTS + 3][1] + 32 +
         h;
    ...
    for (i = 0; i < MAX_WING_SLOTS; i++) {
        ...
        if (ship_selection) { mask &= ~WING_SLOT_WEAPONS_DISABLED; }
        else {
        ...
        case WING_SLOT_FILLED | WING_SLOT_IS_PLAYER:
            ...
            if (selected_slot == slot_index ||
                class_select == Wss_slots[slot_index].ship_class) {
                if (icon->model_index == -1)
...
void ss_make_slot_empty (int slot_index) {
    ...
    // set the flags
    ws->status &=
        ~(WING_SLOT_FILLED | WING_SLOT_SHIPS_DISABLED |
          WING_SLOT_WEAPONS_DISABLED);
...
void ss_make_slot_full (int slot_index) {
    ...
    // set the flags
    ws->status &=
        ~(WING_SLOT_EMPTY | WING_SLOT_SHIPS_DISABLED |
          WING_SLOT_WEAPONS_DISABLED);
    ...
    if (ship_class == -1) {
        ...
        if (icon->icon_bmaps[bmap_num] != -1) {
            ...
            if (icon->model_index != -1) {
                ...
                graphics::line_draw_list line_draw_list;
                draw_brackets_square (
                    &line_draw_list, x, y, x + 32, y + 28, GR_RESIZE_MENU);
                line_draw_list.flush ();

                draw_model_icon (
                    icon->model_index,
                    MR_AUTOCENTER | MR_NO_FOGGING | MR_NO_LIGHTING,
                    sip->closeup_zoom / 1.25f, x, y, 32, 28, sip,
                    GR_RESIZE_MENU);
            }
        }
    }
...
int create_wings () {
        ...
        for (j = 0; j < MAX_WING_SLOTS; j++) {
            ...
            if ((ws->status & WING_SLOT_FILLED) ||
                ...
                if (ws->status & WING_SLOT_IS_PLAYER) {
                    ...
                    if (wl_update_ship_weapons (
                            Ships[Player_obj->instance].objnum,
                            &Wss_slots[i * MAX_WING_SLOTS + j]) == -1) {
                        popup (
                            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                            XSTR ("Player ship has no weapons", 461));
                        return -1;
    ...
    for (i = 0; i < MAX_WING_BLOCKS; i++) {
        ...
        for (k = 0; k < MAX_WING_SLOTS; k++) { cleanup_ship_index[k] = -1; }
            ...
            case WING_SLOT_EMPTY:
                ...
                if (wb->is_late) {
                    ...
                    hud_set_wingman_status_none (
                        Ships[shipnum].wing_status_wing_index,
...
int create_default_player_ship (int use_last_flown) {
    ...
    if (use_last_flown) {
        ...
        if (player_ship_class == -1) return 1;
    }

    // if we still haven't found the last flown ship, handle the error
    // semi-gracefully
    ...
    // debug code to keep using descent style physics if the player starts a
    // new game
#ifndef NDEBUG
    if (use_descent) {
        use_descent = 0;
...
// so that is why there is an offset of 1 when getting ship indicies from the
// player wing.  The player is special cased by looking at the status of the
// wing slot
//
int ss_return_ship (
    int wing_block, int wing_slot, int* ship_index, p_object** ppobjp) {
    *ship_index = -1;
    *ppobjp = NULL;
    ...
    if (Ss_wings[wing_block].wingnum < 0) { return -1; }
...
// return the name of the ship in the specified wing position... if the ship is
// the player ship, return the player callsign
//
...
int ss_get_selected_ship () { return Selected_ss_class; }

// Set selected ship to the first occupied wing slot, or first ship in pool if
...
// There may be ships that are in wings but not in Team_data[0].  Since we
// still want to show those icons in the ship selection list, the code below
// checks for these cases.  If a ship is found in a wing, and is not in
// Team_data[0], it is appended to the end of the ship_count[] and ship_list[]
// arrays that are in Team_data[0]
//
...
int ss_fixup_team_data (team_data* tdata) {
    ...
    for (i = 0; i < MAX_STARTING_WINGS; i++) {
        wing* wp;
        if (Starting_wings[i] == -1) continue;
        ...
        for (j = 0; j < wp->current_count; j++) {
            ...
            if (!ship_in_parse_player) {
                ...
                p_team_data->ship_list[list_size] =
                    Ships[wp->ship_index[j]].ship_info_index;
...
void ss_load_all_icons () {
    ...
    for (i = 0; i < MAX_SHIP_CLASSES; i++) {
        ...
        if (Ss_pool[i] >= 0) { ss_load_icons (i); }
    }
}
...
void ss_init_wing_info (int wing_num, int starting_wing_num) {
    ...
    if (Starting_wings[starting_wing_num] < 0) { return; }
    ...
    if (wp->current_count == 0 || wp->ship_index[0] == -1) {
        p_object* p_objp;
        // Temporarily fill in the current count and initialize the ship list
        // in the wing This gets cleaned up before the mission is started
        for (p_objp = GET_FIRST (&Ship_arrival_list);
             p_objp != END_OF_LIST (&Ship_arrival_list);
             p_objp = GET_NEXT (p_objp)) {
            if (p_objp->wingnum == WING_INDEX (wp)) {
...
void ss_init_units () {
    ...
    for (i = 0; i < Wss_num_wings; i++) {
        ...
        for (j = 0; j < ss_wing->num_slots; j++) {
            ...
            // Set the type of slot.  Check if the slot is marked as locked, if
            // so then the player is not going to be able to modify that ship.
            if (ss_slot->sa_index == -1) {
                int objnum;
                if (Ships[wp->ship_index[j]]
                        .flags[Ship::Ship_Flags::Ship_locked]) {
                    ss_slot->status |= WING_SLOT_SHIPS_DISABLED;
                }
                if (Ships[wp->ship_index[j]]
                        .flags[Ship::Ship_Flags::Weapons_locked]) {
                    ss_slot->status |= WING_SLOT_WEAPONS_DISABLED;
                }

                // if neither the ship or weapon has been locked, mark the slot
                // as filled
                ...
                if (Parse_objects[ss_slot->sa_index]
                        .flags[Mission::Parse_Object_Flags::SF_Ship_locked]) {
                    ss_slot->status |= WING_SLOT_SHIPS_DISABLED;
                }
                if (Parse_objects[ss_slot->sa_index].flags
                        [Mission::Parse_Object_Flags::SF_Weapons_locked]) {
                    ss_slot->status |= WING_SLOT_WEAPONS_DISABLED;
                }
            ...
            Wss_slots[i * MAX_WING_SLOTS + j].ship_class =
                ss_slot->original_ship_class;
...
void ss_apply (
    int mode, int from_slot, int from_list, int to_slot, int to_list,
    int player_index) {
    int update = 0;
...
void ss_drop (
    int from_slot, int from_list, int to_slot, int to_list, int player_index) {
    int mode;
    common_flash_button_init ();
