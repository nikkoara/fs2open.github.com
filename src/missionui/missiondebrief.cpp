...
int Debrief_list_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                     416, 280, 195, 101 },
                                                   { // GR_1024
                                                     666, 448, 312, 162 } };
...
UI_XSTR Debrief_strings[GR_NUM_RESOLUTIONS][NUM_DEBRIEF_TEXT] = {
    {
        ...
        { "Pilot", 1310, 433, 413, UI_XSTR_COLOR_GREEN, -1,
          &Buttons[0][MULTI_PINFO_POPUP].button },
        { "Info", 1311, 433, 423, UI_XSTR_COLOR_GREEN, -1,
          &Buttons[0][MULTI_PINFO_POPUP].button },
        { "Kick", 1266, 510, 413, UI_XSTR_COLOR_GREEN, -1,
    ...
    },
    {
        ...
        { "Pilot", 1310, 700, 661, UI_XSTR_COLOR_GREEN, -1,
          &Buttons[1][MULTI_PINFO_POPUP].button },
        { "Info", 1311, 700, 679, UI_XSTR_COLOR_GREEN, -1,
          &Buttons[1][MULTI_PINFO_POPUP].button },
        { "Kick", 1266, 816, 661, UI_XSTR_COLOR_GREEN, -1,
...
// flag indicating if we should display info for the given player (in
// multiplayer)
...
v_campaign Volition_campaigns[NUM_VOLITION_CAMPAIGNS] = { {
    BUILTIN_CAMPAIGN, // the only campaign for now, but this leaves room for a
                      // mission pack
    35 // make sure this is equal to the  number of missions you gave in the
       // corresponding Debrief_promotion_voice_mapping
} };
...
void debrief_voice_init () {
    ...
    for (i = 0; i < MAX_DEBRIEF_STAGES; i++) { Debrief_voices[i] = -1; }
}

void debrief_load_voice_file (int voice_num, char* name) {
    int load_attempts = 0;
    while (1) {
        if (load_attempts++ > 5) { break; }

        Debrief_voices[voice_num] = audiostream_open (name, ASF_VOICE);
        if (Debrief_voices[voice_num] >= 0) { break; }
...
void debrief_voice_play () {
    if (!Briefing_voice_enabled || (Current_mode != DEBRIEF_TAB)) { return; }

    // no more stages?  We are done then.
    if (Stage_voice >= Num_debrief_stages) { return; }

    // if in delayed start, see if delay has elapsed and start voice if so
    if (Debrief_cue_voice) {
        if (!timestamp_elapsed (Debrief_cue_voice)) { return; }

        Stage_voice++; // move up to next voice
        if ((Stage_voice < Num_debrief_stages) &&
            (Debrief_voices[Stage_voice] >= 0)) {
            audiostream_play (
                Debrief_voices[Stage_voice], Master_voice_volume, 0);
            Debrief_cue_voice =
                0; // indicate no longer in delayed start checking
        }
...
// debriefing on the clients
void debrief_multi_fixup_stages () {
    int i;

    // possibly insert the badge stage first, them the promotion stage since
    // they are inserted at the front of the debrief stages.
    if (Badge_bitmap >= 0) {
...
// function called from multiplayer clients to set up the debriefing
// information for them (sent from the server).
void debrief_set_multi_clients (int stage_count, int active_stages[]) {
    int i;

    // set up the right briefing for this guy
    if (MULTI_TEAM) { Debriefing = &Debriefings[Net_player->p_info.team]; }
    else {
    ...
    for (i = 0; i < stage_count; i++) {
        Debrief_stages[Num_debrief_stages++] =
            &Debriefing->stages[active_stages[i]];
    }

    Debrief_multi_stages_loaded = 1;
}

// evaluate all stages for all teams.  Server of a multiplayer game will have
// to send that information to all clients after leaving this screen.
void debrief_multi_server_stuff () {
    debriefing* debriefp;

    int stage_active[MAX_TVT_TEAMS][MAX_DEBRIEF_STAGES],
...
int debrief_set_stages_and_multi_stuff () {
    ...
    if (Game_mode & GM_MULTIPLAYER) { debrief_multi_server_stuff (); }

    // check to see if player is a traitor (looking at his team).  If so, use
    // the special traitor debriefing.  Only done in single player
    debriefp = Debriefing;
    if (!(Game_mode & GM_MULTIPLAYER)) {
        // although the no traitor flag check seems redundant it allows the
        // mission designer to turn the traitor debriefing off before the
        // mission ends and supply one themselves
        if ((Player_ship->team == Iff_traitor) &&
            !(The_mission.flags[Mission::Mission_Flags::No_traitor]))
...
void debrief_buttons_init () {
    ...
    for (i = 0; i < NUM_BUTTONS; i++) {
        ...
        b->button.create (
            &Debrief_ui_window, "", b->x, b->y, 60, 30,
            0 /*b->flags & REPEAT*/, 1);
    ...
    for (i = 0; i < NUM_DEBRIEF_TEXT; i++) {
        ...
        if ((i == MP_TEXT_INDEX_1) || (i == MP_TEXT_INDEX_2) ||
            (i == MP_TEXT_INDEX_3)) {
            // only add if in multiplayer mode
            if (Game_mode & GM_MULTIPLAYER) {
                Debrief_ui_window.add_XSTR (
                    &Debrief_strings[gr_screen.res][i]);
            }
        }
        // all other text
        else {
    ...
    // set up hotkeys for buttons so we draw the correct animation frame when a
    // key is pressed
    Buttons[gr_screen.res][NEXT_STAGE].button.set_hotkey (KEY_RIGHT);
    Buttons[gr_screen.res][PREV_STAGE].button.set_hotkey (KEY_LEFT);
    Buttons[gr_screen.res][LAST_STAGE].button.set_hotkey (
        KEY_SHIFTED | KEY_RIGHT);
    Buttons[gr_screen.res][FIRST_STAGE].button.set_hotkey (
        KEY_SHIFTED | KEY_LEFT);
    ...
    Buttons[gr_screen.res][ACCEPT_BUTTON].button.set_hotkey (
...
void debrief_ui_init () {
    ...
    common_set_interface_palette (
        "DebriefPalette"); // set the interface palette
    Debrief_ui_window.create (
    ...
    if (Game_mode & GM_MULTIPLAYER) {
        ...
        chatbox_create ();
        List_region.create (
            &Debrief_ui_window, "", Debrief_list_coords[gr_screen.res][0],
            Debrief_list_coords[gr_screen.res][1],
            Debrief_list_coords[gr_screen.res][2],
            Debrief_list_coords[gr_screen.res][3], 0, 1);
        List_region.hide ();
    }

    Background_bitmap = mission_ui_background_load (
    ...
    Debrief_multi_loading_bitmap =
        bm_load (Debrief_loading_bitmap_fname[gr_screen.res]);
}
...
int debrief_find_persona_index () {
    ...
    if ((Campaign.current_mission >= 0) &&
        ...
        if (Campaign.missions[Campaign.current_mission]
                .debrief_persona_index != 0)
            return Campaign.missions[Campaign.current_mission]
                .debrief_persona_index;
                ...
                for (j = 0; j < Volition_campaigns[i].num_missions; j++) {
                    ...
                    if (!strcasecmp (
                            ...
                            Debrief_promotion_voice_mapping[i][j]
                                .mission_file)) {
                        return Debrief_promotion_voice_mapping[i][j]
                            .persona_index;
                    }
                }
            }
...
// this is an ugly, nasty, hateful way of doing this, but it saves us changing
// the missions at this point"
void debrief_choose_voice (
    char* voice_dest, char* voice_base, int persona_index,
    ...
    if (persona_index >= 0) {
        ...
        if (cf_exists_full (voice_dest, CF_TYPE_VOICE_DEBRIEFINGS)) return;
    }

    // that didn't work, so use the default

    // use the base file; don't prepend anything to it
    if (default_to_base) { strcpy (voice_dest, voice_base); }
    // default to Petrarch
    else {
...
void debrief_choose_medal_variant (
    ...
    sprintf (
        buf, NOX ("%s%s"), Resolution_prefixes[gr_screen.res],
        Medals[medal_earned].debrief_bitmap);

    // if the medal has multiple versions, we may want to choose a specific
    // bitmap
    char* p = strstr (buf, "##");
    if (p != NULL) {
        int version;
...
void debrief_award_init () {
    ...
    if (Player->stats.m_medal_earned != -1) {
        debrief_choose_medal_variant (
            buf, Player->stats.m_medal_earned,
            Player->stats.medal_counts[Player->stats.m_medal_earned] - 1);
        Medal_bitmap = bm_load (buf);

        debrief_add_award_text (
            Medals[Player->stats.m_medal_earned].get_display_string ());
    }
    ...
    if (Player->stats.m_promotion_earned != -1) {
        ...
        // use persona-specific promotion text if it exists; otherwise, use
        // default
        if (Ranks[Promoted].promotion_text.find (persona_index) !=
            Ranks[Promoted].promotion_text.end ()) {
            Promotion_stage.text =
                Ranks[Promoted].promotion_text[persona_index];
        }
        else {
        ...
        debrief_choose_voice (
            Promotion_stage.voice, Ranks[Promoted].promotion_voice_base,
            persona_index);
    ...
    // handle badge earned
    // only grant badge if earned and allowed.  (no_promotion really means no
    // promotion and no badges)
    if (Player->stats.m_badge_earned.size ()) {
        debrief_choose_medal_variant (
            buf, Player->stats.m_badge_earned.back (),
            Player->stats.medal_counts[Player->stats.m_badge_earned.back ()] -
                1);
        ...
        if (Medals[Player->stats.m_badge_earned.back ()].promotion_text.find (
                persona_index) != Medals[Player->stats.m_badge_earned.back ()]
                                      .promotion_text.end ()) {
            Badge_stage.text = Medals[Player->stats.m_badge_earned.back ()]
                                   .promotion_text[persona_index];
        }
        else {
            Badge_stage.text = Medals[Player->stats.m_badge_earned.back ()]
                                   .promotion_text[-1];
        ...
        debrief_choose_voice (
            Badge_stage.voice,
            Medals[Player->stats.m_badge_earned.back ()].voice_base,
            persona_index);

        debrief_add_award_text (Medals[Player->stats.m_badge_earned.back ()]
                                    .get_display_string ());
    }
...
// debrief_traitor_init() initializes local data which could be used if the
// player leaves the mission a traitor.  The same debriefing always gets played
void debrief_traitor_init () {
    ...
    if (!inited) {
        ...
        try {
            ...
            // Goober5000
            debrief_choose_voice (
                stagep->voice, traitor_voice_file,
                debrief_find_persona_index (), 1);
    ...
    if (!(Game_mode & GM_MULTIPLAYER) && (Game_mode & GM_CAMPAIGN_MODE)) {
        if (Player_ship->team == Iff_traitor) { Turned_traitor = 1; }
...
void debrief_multi_list_scroll_down () {
    ...
    if ((Multi_list_offset +
         Debrief_multi_list_team_max_display[gr_screen.res]) >=
        Multi_list_size) {
        gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
        return;
    }
...
void debrief_multi_list_draw () {
    ...
    if ((Multi_list_size >= 0) && (Multi_list_select == -1)) {
        ...
        for (idx = 0; idx < Multi_list_size; idx++) {
            if (Multi_list[idx].net_player_index == MY_NET_PLAYER_NUM) {
                ...
                Debrief_player =
                    Net_players[Multi_list[idx].net_player_index].m_player;
                break;
            }
        }
    }
    ...
    while (y + font_height <= Debrief_list_coords[gr_screen.res][3]) {
        ...
        if (!((np->flags & NETINFO_FLAG_OBSERVER) &&
              !(np->flags & NETINFO_FLAG_OBS_PLAYER))) {
            if (Netgame.type_flags & NG_TYPE_TEAM) {
                // team 0
                if (np->p_info.team == 0) {
                    // draw his "selected" icon
                    if (((np->state == NETPLAYER_STATE_DEBRIEF_ACCEPT) ||
                         (np->state == NETPLAYER_STATE_DEBRIEF_REPLAY)) &&
                        (Multi_common_icons[MICON_TEAM0_SELECT] != -1)) {
                        gr_set_bitmap (Multi_common_icons[MICON_TEAM0_SELECT]);
                        gr_bitmap (
                            Debrief_list_coords[gr_screen.res][0],
                            Debrief_list_coords[gr_screen.res][1] + y - 2,
                            GR_RESIZE_MENU);
                    }
        ...
        if (Net_players[Multi_list[z].net_player_index].flags &
                NETINFO_FLAG_OBSERVER &&
            !(Net_players[Multi_list[z].net_player_index].flags &
              NETINFO_FLAG_OBS_PLAYER)) {
            strcat_s (str, XSTR ("(O)", 438));
        }

        // bli
        gr_string (
            Debrief_list_coords[gr_screen.res][0] + MULTI_LIST_TEAM_OFFSET,
            Debrief_list_coords[gr_screen.res][1] + y, str, GR_RESIZE_MENU);
...
void debrief_accept (int ok_to_post_start_game_event) {
    ...
    if ((/*Cheats_enabled ||*/ Turned_traitor || Must_replay_mission) &&
        ...
        if (Player_ship->team == Iff_traitor) {
            str = XSTR (
                "Your career is over, Traitor!  You can't accept new "
                "missions!",
                439);
        } /* else if (Cheats_enabled) {
             str = XSTR( "You are a cheater.  You cannot accept this mission!",
         440);
         }*/
        else {
            str = XSTR (
                "You have failed this mission and cannot accept.  What do you "
                "you wish to do instead?",
                441);
        }

        z = popup (
            0, 3, XSTR ("Return to &Debriefing", 442),
    ...
    // save mission stats
    if (Game_mode & GM_MULTIPLAYER) {
        // note that multi_debrief_accept_hit() will handle all
        // mission_campaign_* calls on its own as well as doing stats
        // transfers, etc.
        ...
        // if we are just playing a single mission, then don't do many of the
        // things that need to be done.  Nothing much should happen when just
        // playing a single mission that isn't in a campaign.
        if (Game_mode & GM_CAMPAIGN_MODE) {
            mission_campaign_store_variables (
                SEXP_VARIABLE_SAVE_ON_MISSION_PROGRESS);

            // check for possible mission loop
            // check for (1) mission loop available, (2) don't have to repeat
            // last mission
            if (!(Game_mode & GM_MULTIPLAYER)) {
                ...
                if ((Campaign.missions[cur].flags & CMISSION_FLAG_HAS_LOOP) &&
                    ...
                    /*
                    char buffer[512];
                    debrief_assemble_optional_mission_popup_text(buffer,
                    Campaign.missions[cur].mission_loop_desc);
            ...
            // continue as normal
                ...
                // end the mission
                // if we can loop, but don't want to right now, then setup so
                // that we can later
                ...
                if (Campaign.next_mission == -1 ||
                    (The_mission
                         .flags[Mission::Mission_Flags::End_to_mainhall])) {
...
void debrief_next_tab () {
    ...
    if (New_mode >= NUM_TABS) New_mode = 0;
}

void debrief_prev_tab () {
    New_mode = Current_mode - 1;
    if (New_mode < 0) New_mode = NUM_TABS - 1;
}
...
void debrief_render_stagenum () {
    ...
    if (Num_stages < 2) return;

    sprintf (buf, XSTR ("%d of %d", 445), Current_stage + 1, Num_stages);
    gr_get_string_size (&w, NULL, buf);
    gr_set_color_fast (&Color_bright_blue);
    gr_string (
        Debrief_stage_info_coords[gr_screen.res][0] - w,
        Debrief_stage_info_coords[gr_screen.res][1], buf, GR_RESIZE_MENU);
...
void debrief_render_mission_difficulty (int y_loc) {
    ...
    gr_string (
        Debrief_text_x2[gr_screen.res], y_loc,
        Skill_level_names (Game_skill_level), GR_RESIZE_MENU);
}
...
void debrief_render_mission_time (int y_loc) {
    ...
    gr_string (
        Debrief_text_x2[gr_screen.res], y_loc, time_str, GR_RESIZE_MENU);
}
...
void debrief_stats_render () {
    ...
    gr_set_clip (
        ...
        Debrief_text_wnd_coords[gr_screen.res][3], GR_RESIZE_MENU);
    ...
    case DEBRIEF_MISSION_KILLS:
        ...
        while (y + font_height <= Debrief_text_wnd_coords[gr_screen.res][3]) {
            if (i >= Num_text_lines) break;

            if (!i) {
                if (Current_stage == DEBRIEF_MISSION_KILLS)
                    gr_printf_menu (
                        0, y, "%s", XSTR ("Mission Kills by Ship Type", 447));
                else
                    gr_printf_menu (
                        0, y, "%s", XSTR ("All-time Kills by Ship Type", 448));
            }
            else if (i > 1) {
                ...
                gr_printf_menu (
                    Debrief_text_x2[gr_screen.res], y, "%d",
        ...
        if (Num_text_lines == 2) {
            if (Current_stage == DEBRIEF_MISSION_KILLS)
                gr_printf_menu (
                    0, y, "%s", XSTR ("(No ship kills this mission)", 449));
            else
...
void debrief_replay_pressed () {
    if (!Turned_traitor && !Must_replay_mission &&
        ...
        choice = popup (
            ...
            XSTR (
                "If you choose to replay this mission, you will be required "
                "to complete it again before proceeding to future "
                "missions.\n\nIn addition, any statistics gathered during "
                "this mission will be discarded if you choose to replay.",
                452));
...
//
// Redraw any debriefing buttons that are pressed down.  This function is
...
// debrief specific button with hotspot 'i' has been pressed, so perform the
// associated action
//
void debrief_button_pressed (int num) {
    ...
    case DEBRIEF_TAB:
        ...
        break;
    case STATS_TAB:
        // Debrief_ui_window.use_hack_to_get_around_stupid_problem_flag = 1;
        // // allows failure sound to be played
    ...
    case REPLAY_MISSION:
        if (Game_mode & GM_MULTIPLAYER) { multi_debrief_replay_hit (); }
        else {
    ...
    case FIRST_STAGE: debrief_first_stage (); break;

    case PREV_STAGE: debrief_prev_stage (); break;

    case NEXT_STAGE: debrief_next_stage (); break;

    case LAST_STAGE: debrief_last_stage (); break;
    ...
    case ACCEPT_BUTTON: debrief_accept (); break;
    ...
    case PLAYER_SCROLL_UP: debrief_multi_list_scroll_up (); break;

    case PLAYER_SCROLL_DOWN: debrief_multi_list_scroll_down (); break;

    case MULTI_PINFO_POPUP: Debrief_should_show_popup = 1; break;

    case MULTI_KICK: debrief_kick_selected_player (); break;
    } // end swtich
}
...
void debrief_setup_ship_kill_stats (int /*stage_num*/) {
    ...
    for (auto it = Ship_info.begin (); it != Ship_info.end (); i++, ++it) {
        // code used to add in mission kills, but the new system assumes that
        // the player will accept, so all time stats already have mission stats
        // added in.
        if (kill_arr[i] <= 0) { continue; }
...
void debrief_check_buttons () {
    ...
    if (List_region.pressed ()) {
        ...
        if ((z >= 0) && (z < Multi_list_size)) {
            ...
            Debrief_player =
                Net_players[Multi_list[z].net_player_index].m_player;
            Multi_list_select = z;
    ...
    // if the player was double clicked on - we should popup a player info
    // popup
    /*
...
void debrief_text_init () {
    ...
    Num_text_lines = Text_offset = brief_color_text_init (
        "", Debrief_text_wnd_coords[gr_screen.res][2],
...
static void debrief_init_music () {
    ...
    if ((Game_mode & GM_CAMPAIGN_MODE) &&
        ...
        // you completed all primaries and secondaries, so you get the win
        // music
        score = SCORE_DEBRIEF_SUCCESS;
    }
    else {
        // you somehow passed the mission, so you get a little something for
        // your efforts.
...
void debrief_init () {
    ...
    // brief_color_text_init() calculates based on the same font as the
    // debriefing itself.
    font::set_font (DEBRIEFING_FONT);
    ...
    for (i = 0; i < Debriefing->num_stages; i++) {
        ...
        sexp_replace_variable_names_with_values (
            Debriefing->stages[i].recommendation_text);
    }
    ...
    if ((Game_mode & GM_CAMPAIGN_MODE) && (!MULTIPLAYER_CLIENT)) {
        // MUST store goals and events first - may be used to evaluate next
        // mission store goals and events
    ...
    // call traitor init before calling scoring_level_close.  traitor init will
    // essentially nullify any stats
    if (!(Game_mode & GM_MULTIPLAYER)) { // only do for single player
        debrief_traitor_init (); // initialize data needed if player becomes
                                 // traitor.
    }

    // call scoring level close for my stats.  Needed for award_init.  The
    // stats will be backed out if used chooses to replace them.
    scoring_level_close ();
    ...
    // Multiplayer clients will get their debriefing info directly from the
    // server.
    if (!MULTIPLAYER_CLIENT) {
        debrief_set_stages_and_multi_stuff ();

        if (Num_debrief_stages <= 0) { Num_debrief_stages = 0; }
        else {
            debrief_voice_load_all ();
        }
    }
    else {
        // multiplayer client may have already received their debriefing info.
        // If they have not, then set the num debrief stages to 0
        if (!Debrief_multi_stages_loaded) { Num_debrief_stages = 0; }
    }
    ...
    */
    ...
    Max_debrief_Lines = Debrief_text_wnd_coords[gr_screen.res][3] /
                        gr_get_font_height (); // Make the max number of lines
                                               // dependent on the font height.
...
void debrief_close () {
    ...
    // conditions for "accepting" are a little bit different
    if (Game_mode & GM_MULTIPLAYER) {
            ...
            if (MULTIPLAYER_MASTER) {
                for (i = 0; i < MAX_PLAYERS; i++) {
                    if (MULTI_CONNECTED (Net_players[i]) &&
                        !MULTI_STANDALONE (Net_players[i]) &&
                        !MULTI_PERM_OBSERVER (Net_players[i]) &&
    ...
    if (Debrief_accepted) { Player->show_skip_popup = 1; }
    ...
    if (Award_bg_bitmap >= 0) { bm_release (Award_bg_bitmap); }

    if (Rank_bitmap >= 0) { bm_release (Rank_bitmap); }

    if (Medal_bitmap >= 0) { bm_release (Medal_bitmap); }

    if (Badge_bitmap >= 0) { bm_release (Badge_bitmap); }

    Debrief_ui_window.destroy ();
    common_free_interface_palette (); // restore game palette
    show_stats_close ();

    if (Game_mode & GM_MULTIPLAYER) { multi_debrief_close (); }
...
void debrief_do_keys (int new_k) {
    switch (new_k) {
    case KEY_TAB: debrief_next_tab (); break;

    case KEY_SHIFTED | KEY_TAB: debrief_prev_tab (); break;
    ...
    case KEY_ESC: {
        ...
        if (Game_mode & GM_MULTIPLAYER) { multi_debrief_esc_hit (); }
        else {
            // display the normal debrief popup
            if (!Turned_traitor && !Must_replay_mission &&
                (Game_mode & GM_CAMPAIGN_MODE)) {
                pf_flags = PF_BODY_BIG; // | PF_USE_AFFIRMATIVE_ICON |
                                        // PF_USE_NEGATIVE_ICON;
                choice = popup (
                    pf_flags, 3, POPUP_CANCEL, XSTR ("&Yes", 454),
                ...
                (Turned_traitor || Must_replay_mission) &&
                (Game_mode & GM_CAMPAIGN_MODE)) {
                // need to popup saying that mission was a failure and must be
                // replayed
                choice = popup (
                    0, 2, POPUP_NO, POPUP_YES,
                    XSTR (
                        "Because this mission was a failure, you must replay "
                        "this mission when you continue your "
...
// clears out text array so we don't have old award text showing up on new
// awards.
void debrief_award_text_clear () {
    int i;
...
void debrief_add_award_text (const char* str) {
    ...
    if (!Disable_built_in_translations) {
        ...
        if (Lcl_gr) {
            lcl_translate_medal_name_gr (
                Debrief_award_text[Debrief_award_text_num_lines]);
        }
        else if (Lcl_pl) {
            lcl_translate_medal_name_pl (
                Debrief_award_text[Debrief_award_text_num_lines]);
        }
    }
    ...
    // assumes text isnt > 2 lines, but this is a safe assumption due to the
    // char limits of the ranks/badges/etc
    if (Debrief_award_text_num_lines < AWARD_TEXT_MAX_LINES) {
        ...
        if (line2 != NULL) {
            sprintf (
                Debrief_award_text[Debrief_award_text_num_lines], " %s",
                line2); // indent a space
        }
        Debrief_award_text_num_lines++; // leave blank line even if it all fits
                                        // into 1
    }
}
...
void debrief_do_frame (float frametime) {
    ...
    if (The_mission.flags[Mission::Mission_Flags::Toggle_debriefing]) {
        // make sure that we can actually advance - we don't want an endless
        // loop!!!
        if (!((/*Cheats_enabled ||*/ Turned_traitor || Must_replay_mission) &&
              (Game_mode & GM_CAMPAIGN_MODE))) {
            debrief_accept ();
    ...
    if (MULTIPLAYER_CLIENT && !Debrief_multi_stages_loaded) {
        ...
        if (Debrief_multi_loading_bitmap > -1) {
            ...
            gr_bitmap (
                Please_wait_coords[gr_screen.res][0],
                Please_wait_coords[gr_screen.res][1], GR_RESIZE_MENU);
        }
        ...
        gr_string (
            ...
            (gr_screen.max_h_unscaled - str_h) / 2, please_wait_str,
            GR_RESIZE_MENU);
        ...
        if (Game_mode & GM_MULTIPLAYER) { multi_debrief_do_frame (); }
    ...
    if (Game_mode & GM_NORMAL) { new_k = Debrief_ui_window.process (k); }
    else {
    ...
    if (Award_active && (Award_bg_bitmap >= 0)) {
        ...
        gr_bitmap (
            Debrief_award_wnd_coords[gr_screen.res][0],
            Debrief_award_wnd_coords[gr_screen.res][1], GR_RESIZE_MENU);
        if (Rank_bitmap >= 0) {
            gr_set_bitmap (Rank_bitmap);
            gr_bitmap (
                Debrief_award_coords[gr_screen.res][0],
    ...
    gr_string (
        ...
        Debrief_title_coords[gr_screen.res][1], buf, GR_RESIZE_MENU);
    ...
    // Set the font for the debriefing instead of relying on the implicit
    // font-setting of Debrief_ui_window.draw() -MageKing17
    font::set_font (DEBRIEFING_FONT);
    ...
    case DEBRIEF_TAB:
        if (Num_debrief_stages <= 0) {
            ...
            brief_render_text (
                Text_offset, Debrief_text_wnd_coords[gr_screen.res][0],
                Debrief_text_wnd_coords[gr_screen.res][1],
                Debrief_text_wnd_coords[gr_screen.res][3], frametime);
        }

        break;

    case STATS_TAB: debrief_stats_render (); break;
    } // end switch
    ...
    if (Must_replay_mission && (!Debrief_skip_popup_already_shown) &&
        (Player->show_skip_popup) && (Game_mode & GM_NORMAL) &&
        (Game_mode & GM_CAMPAIGN_MODE) &&
        ...
        int popup_choice = popup (
            ...
            XSTR (
                "You have failed this mission five times.  If you like, you "
                "may advance to the next mission.",
                1472));
        switch (popup_choice) {
        case 0:
...
void debrief_handle_player_drop () { debrief_rebuild_player_list (); }
