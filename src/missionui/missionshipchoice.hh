...
extern int
    Ship_select_open; // This game-wide global flag is set to 1 to indicate
                      // that the ship select screen has been opened and memory
                      // allocated.  This flag is needed so we can know if
                      // ship_select_close() needs to called if restoring a
                      // game from the Options screen invoked from ship select

extern int Commit_pressed; // flag to indicate that the commit button was
                           // pressed use a flag, so the ship_create() can be
...
void ss_apply (
    int mode, int from_slot, int from_index, int to_slot, int to_index,
    int player_index = -1);
void ss_drop (
    int from_slot, int from_index, int to_slot, int to_index,
    int player_index = -1);

#endif // FREESPACE2_MISSIONUI_MISSIONSHIPCHOICE_HH
