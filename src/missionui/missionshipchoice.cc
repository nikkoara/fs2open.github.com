...
void commit_pressed()
{
        ...
        if (Wss_num_wings > 0) {
                ...
                if (wl_update_ship_weapons(
                            Ships[Player_obj->instance].objnum, &Wss_slots[0])
                    == -1) {
                        popup(
...
int create_default_player_ship(int use_last_flown)
{
        ...
        if (player_ship_class == -1) {
                popup(
                        PF_TITLE_BIG | PF_TITLE_RED | PF_USE_AFFIRMATIVE_ICON | PF_NO_NETWORKING,
                        1, POPUP_OK,
                        XSTR(
                                "Error!\n\nCannot find "
...
int ss_disabled_slot(int slot_num, bool ship_selection)
{
        ...
        return status & (ship_selection ? WING_SLOT_IGNORE_SHIPS : WING_SLOT_IGNORE_WEAPONS);
}
