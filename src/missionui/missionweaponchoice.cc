...
static int
        ...
        = {
                  { { 27, 152 }, { 27, 182 }, { 27, 212 }, { 27, 242 }, { 36, 331 }, { 36, 361 }, { 36, 391 }, { 36, 421 } },
                  { { 59, 251 }, { 59, 299 }, { 59, 347 }, { 59, 395 }, { 59, 538 }, { 59, 586 }, { 59, 634 }, { 59, 682 } }
          };
                        ...
                        for (x = 0; x < pm->n_guns; x++) {
                                if ((Wss_slots[Selected_wl_slot].wep[x] == Selected_wl_class && Hot_weapon_bank < 0) || x == Hot_weapon_bank) {
                                        ...
                                        for (y = 0; y < pm->gun_banks[x].num_slots; y++) {
                                                ...
                                                xc = int(
                                                        draw_point.screen.xyw.x + Wl_overhead_coords[gr_screen.res][0]);
                                                yc = int(
                                                        draw_point.screen.xyw.y + Wl_overhead_coords[gr_screen.res][1]);
                                                ...
                                                if ((xc > Wl_bank_coords[gr_screen.res][x][0] + 106) && (Wl_bank_coords[gr_screen.res][x][1] + 12 < yc))
                                                        ...
                                                        (xc < Wl_bank_coords[gr_screen.res][x][0] + 106) && (Wl_bank_coords[gr_screen.res][x][1] + 12 < yc))
                                                        curve = 0;
                                                else if (
                                                        (xc > Wl_bank_coords[gr_screen.res][x][0] + 106) && (Wl_bank_coords[gr_screen.res][x][1] + 12 > yc))
                                                        curve = 3;
                                ...
                                if ((Wss_slots[Selected_wl_slot]
                                    ...
                                    || x + MAX_SHIP_PRIMARY_BANKS == Hot_weapon_bank) {
                                        ...
                                        for (y = 0; y < pm->missile_banks[x].num_slots; y++) {
                                                ...
                                                // get the curve right.
                                                int curve;
                                                if ((xc > Wl_bank_coords[gr_screen.res]
                                                                        [x + MAX_SHIP_PRIMARY_BANKS][0]
                                                                  - 50)
                                                    && (Wl_bank_coords[gr_screen.res]
                                                                      [x + MAX_SHIP_PRIMARY_BANKS][1]
                                                        ...
                                                        (xc < Wl_bank_coords[gr_screen.res]
                                                                            [x + MAX_SHIP_PRIMARY_BANKS][0]
                                                                      - 50)
                                                        && (Wl_bank_coords[gr_screen.res]
                                                                          [x + MAX_SHIP_PRIMARY_BANKS][1]
                                                ...
                                                if (curve == 1 || curve == 2) {
                                                        lineendy = Wl_bank_coords[gr_screen.res]
                                                                                 [x + MAX_SHIP_PRIMARY_BANKS][1]
                                                                   + 7;
                                                } else {
                                                        lineendy = Wl_bank_coords[gr_screen.res]
...
void wl_set_disabled_weapons(int ship_class)
{
        ...
        ASSERT(
                ship_class >= 0 && ship_class < static_cast< int >(Ship_info.size()));
        ...
        for (i = 0; i < MAX_SHIP_SECONDARY_BANKS; i++) {
                if (wep[i + MAX_SHIP_PRIMARY_BANKS] >= 0) {
                        wep_count[i + MAX_SHIP_PRIMARY_BANKS] = wl_calc_missile_fit(
                                ...
                                (int)std::lround(
                                        ss->secondary_ammo[i] / 100.0f * sip->secondary_bank_ammo_capacity[i]));
                }
        }
}
                                                ...
                                                for (wep_pool_index = 0;
                                                     ...
                                                     wep_pool_index++) {
                                                        ...
                                                        for (wep_precedence_index = 0;
                                                             wep_precedence_index < Num_player_weapon_precedence;
                ...
                if (Wl_icons[Carried_wl_icon.weapon_class].can_use > 0) {
                        ...
                        if (icon->icon_bmaps[WEAPON_ICON_FRAME_SELECTED] != -1) {
                                ...
                                if (icon->model_index != -1) {
                                        ...
                                        // crash.
                                        int half_x, half_y;
                                        std::string print_name = Weapon_info[Carried_wl_icon.weapon_class]
                                                                         .get_display_string();
...
void wl_render_icon_count(int num, int x, int y)
{
        char buf[32];
        int num_w, num_h;
        int number_to_draw = (num >= 10000) ? 9999
        ...
        int hot_bank_mask, int select_mask)
{
        ...
        if (!wl_icon_being_carried()) {
                if ((Hot_weapon_icon > -1 && Hot_weapon_icon == hot_mask) || (Hot_weapon_bank_icon > -1 && Hot_weapon_bank_icon == hot_bank_mask)) {
                        if (icon->icon_bmaps[0] != -1)
        ...
        if (Selected_wl_class > -1) {
                if (Selected_wl_class == select_mask) {
                        if (icon->icon_bmaps[0] != -1)
                                bitmap_id = icon->icon_bmaps[WEAPON_ICON_FRAME_SELECTED]; // selected
                                                                                          // icon
                        else
