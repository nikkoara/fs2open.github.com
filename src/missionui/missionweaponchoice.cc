...
#define NUM_WEAPON_BUTTONS \
    (Uses_apply_all_button ? MAX_WEAPON_BUTTONS : MIN_WEAPON_BUTTONS)
...
struct wl_buttons {
    ...
    int x, y, xt, yt;
    int hotspot;
    UI_BUTTON button; // because we have a class inside this struct, we need
...
static const char*
    Wl_loadout_select_mask[NUM_WEAPON_SETTINGS][GR_NUM_RESOLUTIONS] = {
        { "weaponloadout-m", "2_weaponloadout-m" },
        { "weaponloadout-mb", "2_weaponloadout-mb" }
    };

static const char*
    Weapon_select_background_fname[NUM_WEAPON_SETTINGS][GR_NUM_RESOLUTIONS] = {
        { "WeaponLoadout", "2_WeaponLoadout" },
        { "WeaponLoadoutb", "2_WeaponLoadoutb" }
    };

static const char* Weapon_select_multi_background_fname
    [NUM_WEAPON_SETTINGS][GR_NUM_RESOLUTIONS] = {
        { "WeaponLoadoutMulti", "2_WeaponLoadoutMulti" },
        { "WeaponLoadoutMultib", "2_WeaponLoadoutMultib" }
    };
...
static MENU_REGION
    Weapon_select_region[NUM_COMMON_REGIONS + 27]; // see initialization
static int Num_weapon_select_regions;

// Mask bitmap pointer and Mask bitmap_id
static bitmap*
    WeaponSelectMaskPtr; // bitmap pointer to the weapon select mask bitmap
static ubyte* WeaponSelectMaskData; // pointer to actual bitmap data
static int Weaponselect_mask_w, Weaponselect_mask_h;
static int
    WeaponSelectMaskBitmap; // bitmap id of the weapon select mask bitmap
...
#define WEAPON_DESC_WIPE_TIME \
    1.5f // time in seconds for wipe to occur (over WEAPON_DESC_MAX_LENGTH
         // number of chars)

static int Weapon_desc_wipe_done = 0;
static float Weapon_desc_wipe_time_elapsed = 0.0f;
static char Weapon_desc_lines[WEAPON_DESC_MAX_LINES]
                             [WEAPON_DESC_MAX_LENGTH]; // 1st 2 lines are
                                                       // title, rest are desc
...
UI_XSTR Weapon_select_text[GR_NUM_RESOLUTIONS][WEAPON_SELECT_NUM_TEXT] = {
    { // GR_640
      { "Reset", 1337, 580, 337, UI_XSTR_COLOR_GREEN, -1,
        &Buttons[0][WL_BUTTON_RESET].button },
      { "Lock", 1270, 602, 364, UI_XSTR_COLOR_GREEN, -1,
        &Buttons[0][WL_BUTTON_MULTI_LOCK].button } },
    { // GR_1024
      { "Reset", 1337, 938, 546, UI_XSTR_COLOR_GREEN, -1,
        &Buttons[1][WL_BUTTON_RESET].button },
      { "Lock", 1270, 964, 584, UI_XSTR_COLOR_GREEN, -1,
...
typedef struct carried_icon {
    int weapon_class; // index Wl_icons[] for carried icon (-1 if carried from
                      // bank)
    int num;          // number of units of weapon
    int from_bank;    // bank index that icon came from (0..2 primary, 3..6
                      // secondary).  -1 if from list
 ...
 */
int wl_icon_being_carried () {
    if (Carried_wl_icon.weapon_class >= 0) { return 1; }
...
void weapon_button_do (int i) {
    ...
    default:
        popup (
            ...
            "Button %d is not yet implemented", i);
        break;
    }
}
...
void weapon_check_buttons () {
    ...
    for (i = 0; i < NUM_WEAPON_BUTTONS; i++) {
        ...
        if (b->button.pressed ()) { weapon_button_do (i); }
    }
}

/**
 * Redraw any weapon loadout buttons that are pressed down.  This function is
 * needed since we sometimes need to draw pressed buttons last to ensure the
 * entire button gets drawn (and not overlapped by other buttons)
 */
void wl_redraw_pressed_buttons () {
    ...
    for (i = 0; i < NUM_WEAPON_BUTTONS; i++) {
        b = &Buttons[gr_screen.res][i];
        if (b->button.button_down ()) { b->button.draw_forced (2); }
    }
}
...
void weapon_buttons_init () {
    ...
    for (i = 0; i < NUM_WEAPON_BUTTONS; i++) {
        b = &Buttons[gr_screen.res][i];
        b->button.create (
            &Weapon_ui_window, "", Buttons[gr_screen.res][i].x,
            Buttons[gr_screen.res][i].y, 60, 30, Weapon_button_scrollable[i]);
...
void wl_render_overhead_view (float frametime) {
    ...
    if (new_ship) {
        ...
        if (Cmdline_ship_choice_3d || !strlen (sip->overhead_filename)) {
            if (wl_ship->model_num < 0) {
                if (sip->pof_file_tech[0] != '\0') {
                    wl_ship->model_num = model_load (
                        sip->pof_file_tech, sip->n_subsystems,
                        &sip->subsystems[0]);
                }
                else {
                    wl_ship->model_num = model_load (
            ...
            if (wl_ship->model_num > -1) {
                if (Cmdline_ship_choice_3d) { display_type = 2; }
                else {
                    display_type = 1;
                }
            }
        ...
        if (display_type < 0) {
            if (wl_ship->overhead_bitmap < 0) {
                ...
                if (gr_screen.res == GR_640) {
                    // lo-res
                    wl_ship->overhead_bitmap =
                        bm_load (sip->overhead_filename);
                }
                else {
                    // high-res
                    char filename[NAME_LENGTH + 2] = "2_";
    ...
    if (display_type == 0 && wl_ship->overhead_bitmap > -1) {
        ...
        gr_bitmap (
            ...
            Wl_overhead_coords[gr_screen.res][1], GR_RESIZE_MENU);
    }
    else {
        ...
        if (wl_ship->model_num < 0) {
            ...
            if (!Cmdline_ship_choice_3d) {
                ...
                if (sip->is_big_ship ()) { rev_rate *= 1.7f; }
                if (sip->is_huge_ship ()) { rev_rate *= 3.0f; }
            ...
            if (sip->replacement_textures.size () > 0) {
                render_info.set_replacement_textures (
                    wl_ship->model_num, sip->replacement_textures);
            }
            ...
            if (Cmdline_shadow_quality) {
                ...
                shadows_start_render (
                    ...
                    gr_screen.clip_aspect, -sip->closeup_pos.xyz.z + pm->rad,
                ...
                render_info.set_flags (
                    MR_NO_TEXTURING | MR_NO_LIGHTING | MR_AUTOCENTER);

                model_render_immediate (
                    &render_info, wl_ship->model_num, &object_orient,
                    &vmd_zero_vector);
                shadows_end_render ();
                gr_set_clip (
                    Wl_overhead_coords[gr_screen.res][0],
                    ...
                    gr_screen.res == 0 ? 226 : 362, GR_RESIZE_MENU);
            }
            ...
            for (x = 0; x < pm->n_guns; x++) {
                if ((Wss_slots[Selected_wl_slot].wep[x] == Selected_wl_class &&
                    ...
                    gr_circle (
                        Wl_bank_coords[gr_screen.res][x][0] + 106,
                        Wl_bank_coords[gr_screen.res][x][1] + 12, 5,
                        GR_RESIZE_MENU);
                    for (y = 0; y < pm->gun_banks[x].num_slots; y++) {
                        // Stuff
                        vm_vec_unrotate (
                            &subobj_pos, &pm->gun_banks[x].pnt[y],
                            &object_orient);
                        g3_rotate_vertex (&draw_point, &subobj_pos);
                        g3_project_vertex (&draw_point);
                        gr_unsize_screen_posf (
                        ...
                        // get the curve right.
                        int curve;
                        if ((xc > Wl_bank_coords[gr_screen.res][x][0] + 106) &&
                            (Wl_bank_coords[gr_screen.res][x][1] + 12 < yc))
                            curve = 1;
                        else if (
                            (xc < Wl_bank_coords[gr_screen.res][x][0] + 106) &&
                            (Wl_bank_coords[gr_screen.res][x][1] + 12 < yc))
                            curve = 0;
                        else if (
                            (xc > Wl_bank_coords[gr_screen.res][x][0] + 106) &&
                        ...
                        gr_line (
                            Wl_bank_coords[gr_screen.res][x][0] + 106,
                            Wl_bank_coords[gr_screen.res][x][1] + 12, lineendx,
                            Wl_bank_coords[gr_screen.res][x][1] + 12,
                            GR_RESIZE_MENU);

                        if (curve == 0 || curve == 2) lineendx = xc;
                        ...
                        gr_curve (
                            lineendx, lineendy, 5, curve, GR_RESIZE_MENU);

                        if (curve == 0 || curve == 1) {
                            lineendy =
                                Wl_bank_coords[gr_screen.res][x][1] + 17;
                        }
                        else {
                            lineendy = Wl_bank_coords[gr_screen.res][x][1] + 7;
                        }
            ...
            for (x = 0; x < pm->n_missiles; x++) {
                if ((Wss_slots[Selected_wl_slot]
                             .wep[x + MAX_SHIP_PRIMARY_BANKS] ==
                         Selected_wl_class &&
                     Hot_weapon_bank < 0) ||
                    ...
                    gr_set_color_fast (
                        &Icon_colors[ICON_FRAME_NORMAL + num_found]);
                    gr_circle (
                        Wl_bank_coords[gr_screen.res]
                                      [x + MAX_SHIP_PRIMARY_BANKS][0] -
                            50,
                        Wl_bank_coords[gr_screen.res]
                                      [x + MAX_SHIP_PRIMARY_BANKS][1] +
                            12,
                        5, GR_RESIZE_MENU);
                    for (y = 0; y < pm->missile_banks[x].num_slots; y++) {
                        vm_vec_unrotate (
                            &subobj_pos, &pm->missile_banks[x].pnt[y],
                            &object_orient);
                        g3_rotate_vertex (&draw_point, &subobj_pos);
                        g3_project_vertex (&draw_point);
                        gr_unsize_screen_posf (
    ...
    ss_return_name (
        Selected_wl_slot / MAX_WING_SLOTS, Selected_wl_slot % MAX_WING_SLOTS,
        name);
    gr_set_color_fast (&Color_normal);
    gr_string (
        Wl_ship_name_coords[gr_screen.res][0],
        Wl_ship_name_coords[gr_screen.res][1], name, GR_RESIZE_MENU);
}
...
/**
 * Return true if weapon_flags indicates a weapon that is legal for use in
 * current game type. Function added by MK on 9/6/99 to support separate legal
 * loadouts for dogfight missions. name changed by Goober5000 to better reflect
...
/**
 * Go through the possible weapons to choose from, and flag some as disabled
 * since that ship class cannot use that kind of weapon.  The weapon filter is
 * specified in ships.tbl, where each ship has a list of all the possible
 * weapons it can use.
 */
...
void maybe_select_new_weapon (int index) {
    ...
    if (index > 3) { region_index = ICON_SECONDARY_0 + (index - 4); }

    if (Wl_mouse_down_on_region != region_index) { return; }

    if (index < 4) { weapon_class = Plist[Plist_start + index]; }
    else {
...
void maybe_select_new_ship_weapon (int index) {
    ...
    if (wep[index] < 0 || wep_count[index] <= 0) { return; }

    if (Wl_mouse_down_on_region != (ICON_SHIP_PRIMARY_0 + index)) { return; }
...
void wl_init_pool (team_data* td) {
    ...
    for (i = 0; i < td->num_weapon_choices; i++) {
        Wl_pool[td->weaponry_pool[i]] +=
            td->weaponry_count[i]; // read from mission
    }
}
...
void wl_unload_all_anims () {
    if (Cur_Anim.num_frames > 0) generic_anim_unload (&Cur_Anim);
}
...
void wl_load_icons (int weapon_class) {
    ...
    if (!Cmdline_weapon_choice_3d ||
        (wip->render_type == WRT_LASER && !strlen (wip->tech_model))) {
        first_frame = bm_load_animation (
            Weapon_info[weapon_class].icon_filename, &num_frames, nullptr,
            nullptr, nullptr, false, CF_TYPE_INTERFACE);
...
void wl_load_all_icons () {
    ...
    for (i = 0; i < MAX_WEAPON_TYPES; i++) {
        ...
        if (Wl_pool[i] > 0) { wl_load_icons (i); }
    }
}
...
void wl_free_ship_class_data () {
    ...
    for (i = 0; i < static_cast< int > (Ship_info.size ()); i++) {
        ...
        if (wl_ship->model_num != -1) {
            // this should unload the model from memory only if it's not used
            // in the mission
...
void wl_reset_selected_slot () {
    ...
    for (i = 0; i < MAX_WSS_SLOTS; i++) {
        if (!ss_disabled_slot (i, false)) {
            if (ss_wing_slot_is_console_player (i) &&
                (Wss_slots[i].ship_class >= 0)) {
...
/**
 * Called whenever it is possible that the current selected slot has had it's
 * ship disappear
 */
void wl_maybe_reset_selected_slot () {
    int reset = 0;
    ...
    if (Wss_slots[Selected_wl_slot].ship_class < 0) { reset = 1; }

    if (reset) { wl_reset_selected_slot (); }
}

/**
 * If Selected_wl_class is -1, choose the first weapon available from the pool
 * for an animation
 *  - on second thought, choose the first weapon that is oin the ship, then go
 * to the pools
 */
void wl_maybe_reset_selected_weapon_class () {
    int i;
    ...
    // finally, if no others found yet, check for a secondary weapon in the
    // pool
...
/**
 * Determine how many ballistics of type 'wi_index' will fit into capacity -
 * Goober5000
 */
...
void wl_get_ship_class_weapons (int ship_class, int* wep, int* wep_count) {
    ...
    for (i = 0; i < MAX_SHIP_WEAPONS; i++) {
        wep[i] = -1;
        wep_count[i] = -1; // -1 means weapon bank doesn't exist.. 0 just means
                           // it is empty
    }
    ...
    for (i = 0; i < sip->num_secondary_banks; i++) {
        ...
        wep_count[i + MAX_SHIP_PRIMARY_BANKS] = wl_calc_missile_fit (
            sip->secondary_bank_weapons[i],
...
/**
 * Set wep and wep_count from a ship which sits in the ship arrivals list at
 * index sa_index
 */
void wl_get_parseobj_weapons (
    int sa_index, int ship_class, int* wep, int* wep_count) {
    int i, pilot_index;
    subsys_status* ss;
    ship_info* sip;
    p_object* pobjp;
    ...
    for (i = 0; i < MAX_SHIP_SECONDARY_BANKS; i++) {
        if (wep[i + MAX_SHIP_PRIMARY_BANKS] >= 0) {
            wep_count[i + MAX_SHIP_PRIMARY_BANKS] = wl_calc_missile_fit (
                wep[i + MAX_SHIP_PRIMARY_BANKS],
                (int)std::lround (
                    ss->secondary_ammo[i] / 100.0f *
                    sip->secondary_bank_ammo_capacity[i]));
        }
    }
}
...
void wl_cull_illegal_weapons (int ship_class, int* wep, int* wep_count) {
    ...
    for (i = 0; i < MAX_SHIP_WEAPONS; i++) {
        ...
        if (eval_weapon_flag_for_game_type (
                Ship_info[ship_class].restricted_loadout_flag[i])) {
            check_flag = Ship_info[ship_class]
                             .allowed_bank_restricted_weapons[i][wep[i]];
        }
...
void wl_add_index_to_list (int wi_index) {
    ...
    if (Weapon_info[wi_index].subtype == WP_MISSILE) {
        for (i = 0; i < Slist_size; i++) {
            if (Slist[i] == wi_index) break;
        }

        if (i == Slist_size) Slist[Slist_size++] = wi_index;
    }
    else {
        for (i = 0; i < Plist_size; i++) {
            if (Plist[i] == wi_index) break;
        }

        if (i == Plist_size) Plist[Plist_size++] = wi_index;
    }
}
            ...
            if ((wep_count[i] > 0) &&
                ...
                if (Wss_num_wings <= 0) { wl_add_index_to_list (wi_index); }
                    ...
                    if ((Wl_pool[wi_index] <= 0) || (wep_count[i] == 0)) {
                        // fresh out of this weapon, pick an alternate pool
                        // weapon if we can
                        int wep_pool_index, wep_precedence_index,
                            new_wi_index = -1;
                        for (wep_pool_index = 0;
                             wep_pool_index < MAX_WEAPON_TYPES;
                             wep_pool_index++) {
                            if (Wl_pool[wep_pool_index] <= 0) { continue; }

                            // AL 3-31-98: Only pick another primary if
                            // primary, etc
                            if (Weapon_info[wi_index].subtype !=
                                Weapon_info[wep_pool_index].subtype) {
                                continue;
                            }

                            if (!eval_weapon_flag_for_game_type (
                                    Ship_info[ship_class]
                                        .allowed_weapons[wep_pool_index])) {
                                continue;
                            }

                            for (wep_precedence_index = 0;
                                 wep_precedence_index <
                                 Num_player_weapon_precedence;
                                 wep_precedence_index++) {
                                if (wep_pool_index ==
                    ...
                    if (Weapon_info[wi_index].subtype == WP_MISSILE) {
                        ...
                        new_wep_count = wl_calc_missile_fit (
                            wi_index,
                            Ship_info[ship_class].secondary_bank_ammo_capacity
                                [secondary_bank_index]);
                    }
 ...
 * @note It is assumed that Wl_pool[] has been initialized, and
 * Wss_slots[].ship_class is correctly set
 */
void wl_fill_slots () {
    int i, j;
...
/**
 * Set up the primary and secondary icons lists that hold the weapons the
...
/**
 * Close out what weapon_select_init_team() set up but only when we are not
 * acutally in the weapon select screen - taylor
 */
...
/**
 * This init is called even before the weapons loadout screen is entered.  It
 ...
 * the weapon loadout screen.  common_select_init() is called to load the
 * animations and bitmaps which are in common with the ship select and briefing
 * screens.
 *
 * @note The Weapon_select_open flag is set to 1 when weapon_select_init()
...
void weapon_select_init () {
    ...
    if (Weapon_select_open) {
        ...
        common_buttons_maybe_reload (
            &Weapon_ui_window); // AL 11-21-97: this is necessary since we may
    ...
    Weapon_select_background_bitmap = mission_ui_background_load (
        Briefing->weapon_select_background[gr_screen.res],
        Weapon_select_background_fname[Uses_apply_all_button][gr_screen.res],
        Weapon_select_multi_background_fname[Uses_apply_all_button]
                                            [gr_screen.res]);
    if (Weapon_select_background_bitmap < 0) // failed to load
    {
        ...
        Weapon_select_background_bitmap = mission_ui_background_load (
            NULL,
            Weapon_select_background_fname[Uses_apply_all_button]
                                          [gr_screen.res],
            Weapon_select_multi_background_fname[Uses_apply_all_button]
                                                [gr_screen.res]);
    }
    ...
    // initialize the different regions of the menu that will react when the
    // mouse moves over it
    Num_weapon_select_regions = 0;

    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "",
        COMMON_BRIEFING_REGION, 0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "",
        COMMON_SS_REGION, 0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "",
        COMMON_WEAPON_REGION, 0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "",
        COMMON_COMMIT_REGION, 0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "",
        COMMON_HELP_REGION, 0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "",
        COMMON_OPTIONS_REGION, 0);

    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "", WING_0_SHIP_0,
        0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "", WING_0_SHIP_1,
        0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "", WING_0_SHIP_2,
        0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "", WING_0_SHIP_3,
        0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "", WING_1_SHIP_0,
        0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "", WING_1_SHIP_1,
        0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "", WING_1_SHIP_2,
        0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "", WING_1_SHIP_3,
        0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "", WING_2_SHIP_0,
        0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "", WING_2_SHIP_1,
        0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "", WING_2_SHIP_2,
        0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "", WING_2_SHIP_3,
        0);

    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "", ICON_PRIMARY_0,
        0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "", ICON_PRIMARY_1,
        0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "", ICON_PRIMARY_2,
        0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "", ICON_PRIMARY_3,
        0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "",
        ICON_SECONDARY_0, 0);
    ...
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "",
        ICON_SHIP_PRIMARY_0, 0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "",
        ICON_SHIP_PRIMARY_1, 0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "",
        ICON_SHIP_PRIMARY_2, 0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "",
        ICON_SHIP_SECONDARY_0, 0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "",
        ICON_SHIP_SECONDARY_1, 0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "",
        ICON_SHIP_SECONDARY_2, 0);
    snazzy_menu_add_region (
        &Weapon_select_region[Num_weapon_select_regions++], "",
        ICON_SHIP_SECONDARY_3, 0);
...
void wl_dump_carried_icon () {
    if (wl_icon_being_carried ()) {
        ...
        if (Carried_wl_icon.from_bank >= 0) {
            // return to list
            wl_drop (
                Carried_wl_icon.from_bank, -1, -1,
                Carried_wl_icon.weapon_class, Carried_wl_icon.from_slot);
        }
        else {
...
int drop_icon_on_slot (int bank_num) {
    ...
    wl_drop (
        Carried_wl_icon.from_bank, Carried_wl_icon.weapon_class, bank_num, -1,
        Selected_wl_slot);
    return 1;
}
...
void maybe_drop_icon_on_slot (int bank_num) {
    ...
    if (!mouse_down (MOUSE_LEFT_BUTTON)) {
        if (wl_icon_being_carried ()) dropped = drop_icon_on_slot (bank_num);

        if (dropped) { wl_reset_carried_icon (); }
    }
}
...
void do_mouse_over_list_weapon (int index) {
    ...
    if (mouse_down (MOUSE_LEFT_BUTTON)) wl_pick_icon_from_list (index);
}
...
void wl_maybe_flash_button () {
    if (common_flash_bright ()) {
        // commit button
        if (Common_buttons[Current_screen - 1][gr_screen.res][3]
                .button.button_hilighted ()) {
            common_flash_button_init ();
        }
        else {
            Common_buttons[Current_screen - 1][gr_screen.res][3]
                .button.draw_forced (1);
        }
    }
}
...
void wl_render_weapon_desc (float frametime) {
    ...
    if (!Weapon_desc_wipe_done) {
        ...
        int bright_char_index =
            (int)(Weapon_desc_wipe_time_elapsed * WEAPON_DESC_MAX_LENGTH / WEAPON_DESC_WIPE_TIME);
        int i, w, h, curr_len;
        ...
        for (i = 0; i < 2; i++) {
            ...
            if (bright_char_index < curr_len) {
                ...
                gr_string (
                    weapon_title_coords[0],
                    weapon_title_coords[1] + (line_height * i),
                    Weapon_desc_lines[i], GR_RESIZE_MENU);

                // draw the bright letters
                gr_set_color_fast (&Color_bright_white);
                gr_get_string_size (&w, &h, Weapon_desc_lines[i], curr_len);
                gr_printf_menu (
                    weapon_title_coords[0] + w,
                    weapon_title_coords[1] + (line_height * i), "%c",
                    bright_char[i]);
        ...
        for (i = 2; i < WEAPON_DESC_MAX_LINES; i++) {
            ...
            if (bright_char_index < curr_len) {
                ...
                gr_set_color_fast (&Color_white);
                gr_string (
                    weapon_desc_coords[0],
                    weapon_desc_coords[1] + (line_height * (i - 2)),
                    Weapon_desc_lines[i], GR_RESIZE_MENU);
                ...
                gr_get_string_size (&w, &h, Weapon_desc_lines[i], curr_len);
                gr_printf_menu (
                    weapon_desc_coords[0] + w,
                    weapon_desc_coords[1] + (line_height * (i - 2)), "%c",
                    bright_char[i]);
        ...
        gr_set_color_fast (&Color_white);
        gr_string (
            weapon_title_coords[0], weapon_title_coords[1],
            Weapon_desc_lines[0], GR_RESIZE_MENU);
        gr_string (
            weapon_title_coords[0], weapon_title_coords[1] + line_height,
            Weapon_desc_lines[1], GR_RESIZE_MENU);
        gr_string (
            weapon_desc_coords[0], weapon_desc_coords[1], Weapon_desc_lines[2],
            GR_RESIZE_MENU);
        gr_string (
            weapon_desc_coords[0], weapon_desc_coords[1] + line_height,
            Weapon_desc_lines[3], GR_RESIZE_MENU);
        gr_string (
            weapon_desc_coords[0], weapon_desc_coords[1] + line_height * 2,
            Weapon_desc_lines[4], GR_RESIZE_MENU);
        gr_string (
            weapon_desc_coords[0], weapon_desc_coords[1] + line_height * 3,
            Weapon_desc_lines[5], GR_RESIZE_MENU);
    }
}
    ...
    if (Weapon_info[Selected_wl_class].desc != NULL) {
        while (Weapon_info[Selected_wl_class].desc[currchar_src] != '\0') {
            if (Weapon_info[Selected_wl_class].desc[currchar_src] == '\n') {
                ...
                // straight copy
                Weapon_desc_lines[currline_dest][currchar_dest] =
...
/**
 * Calls to common_ functions are made for those functions which are common to
 * the ship select and briefing screens.
 */
void weapon_select_do (float frametime) {
    int k, wl_choice, snazzy_action;

    if (!Weapon_select_open) weapon_select_init ();

    wl_choice = snazzy_menu_do (
        WeaponSelectMaskData, Weaponselect_mask_w, Weaponselect_mask_h,
    ...
    if (Mouse_down_last_frame) { Wl_mouse_down_on_region = wl_choice; }

    if (wl_choice > -1) {
        switch (wl_choice) {
        case ICON_PRIMARY_0: do_mouse_over_list_weapon (0); break;
        case ICON_PRIMARY_1: do_mouse_over_list_weapon (1); break;
        case ICON_PRIMARY_2: do_mouse_over_list_weapon (2); break;
        case ICON_PRIMARY_3: do_mouse_over_list_weapon (3); break;
        case ICON_SECONDARY_0: do_mouse_over_list_weapon (4); break;
        case ICON_SECONDARY_1: do_mouse_over_list_weapon (5); break;
        case ICON_SECONDARY_2: do_mouse_over_list_weapon (6); break;
        case ICON_SECONDARY_3: do_mouse_over_list_weapon (7); break;
        case ICON_SHIP_PRIMARY_0:
            if (do_mouse_over_ship_weapon (0)) wl_choice = -1;
            break;
        case ICON_SHIP_PRIMARY_1:
            if (do_mouse_over_ship_weapon (1)) wl_choice = -1;
            break;
        case ICON_SHIP_PRIMARY_2:
            if (do_mouse_over_ship_weapon (2)) wl_choice = -1;
            break;
        case ICON_SHIP_SECONDARY_0:
            if (do_mouse_over_ship_weapon (3)) wl_choice = -1;
            break;
        case ICON_SHIP_SECONDARY_1:
            if (do_mouse_over_ship_weapon (4)) wl_choice = -1;
            break;
        case ICON_SHIP_SECONDARY_2:
            if (do_mouse_over_ship_weapon (5)) wl_choice = -1;
            break;
        case ICON_SHIP_SECONDARY_3:
            if (do_mouse_over_ship_weapon (6)) wl_choice = -1;
            break;
        case WING_0_SHIP_0: Hot_wl_slot = 0; break;
        case WING_0_SHIP_1: Hot_wl_slot = 1; break;
        case WING_0_SHIP_2: Hot_wl_slot = 2; break;
        case WING_0_SHIP_3: Hot_wl_slot = 3; break;
        case WING_1_SHIP_0: Hot_wl_slot = 4; break;
        case WING_1_SHIP_1: Hot_wl_slot = 5; break;
        case WING_1_SHIP_2: Hot_wl_slot = 6; break;
        case WING_1_SHIP_3: Hot_wl_slot = 7; break;
        case WING_2_SHIP_0: Hot_wl_slot = 8; break;
        case WING_2_SHIP_1: Hot_wl_slot = 9; break;
        case WING_2_SHIP_2: Hot_wl_slot = 10; break;
        case WING_2_SHIP_3: Hot_wl_slot = 11; break;

        default: break;
        } // end switch
    }

    if (!mouse_down (MOUSE_LEFT_BUTTON)) { wl_dump_carried_icon (); }
    ...
    if (wl_choice > -1 && snazzy_action == SNAZZY_CLICKED) {
        switch (wl_choice) {
        case ICON_PRIMARY_0: maybe_select_new_weapon (0); break;
        case ICON_PRIMARY_1: maybe_select_new_weapon (1); break;
        case ICON_PRIMARY_2: maybe_select_new_weapon (2); break;
        case ICON_PRIMARY_3: maybe_select_new_weapon (3); break;
        case ICON_SECONDARY_0: maybe_select_new_weapon (4); break;
        case ICON_SECONDARY_1: maybe_select_new_weapon (5); break;
        case ICON_SECONDARY_2: maybe_select_new_weapon (6); break;
        case ICON_SECONDARY_3: maybe_select_new_weapon (7); break;
        case ICON_SHIP_PRIMARY_0: maybe_select_new_ship_weapon (0); break;
        case ICON_SHIP_PRIMARY_1: maybe_select_new_ship_weapon (1); break;
        case ICON_SHIP_PRIMARY_2: maybe_select_new_ship_weapon (2); break;
        case ICON_SHIP_SECONDARY_0: maybe_select_new_ship_weapon (3); break;
        case ICON_SHIP_SECONDARY_1: maybe_select_new_ship_weapon (4); break;
        case ICON_SHIP_SECONDARY_2: maybe_select_new_ship_weapon (5); break;
        case ICON_SHIP_SECONDARY_3: maybe_select_new_ship_weapon (6); break;
        case WING_0_SHIP_0: maybe_select_wl_slot (0, 0); break;
        case WING_0_SHIP_1: maybe_select_wl_slot (0, 1); break;
        case WING_0_SHIP_2: maybe_select_wl_slot (0, 2); break;
        case WING_0_SHIP_3: maybe_select_wl_slot (0, 3); break;
        case WING_1_SHIP_0: maybe_select_wl_slot (1, 0); break;
        case WING_1_SHIP_1: maybe_select_wl_slot (1, 1); break;
        case WING_1_SHIP_2: maybe_select_wl_slot (1, 2); break;
        case WING_1_SHIP_3: maybe_select_wl_slot (1, 3); break;
        case WING_2_SHIP_0: maybe_select_wl_slot (2, 0); break;
        case WING_2_SHIP_1: maybe_select_wl_slot (2, 1); break;
        case WING_2_SHIP_2: maybe_select_wl_slot (2, 2); break;
        case WING_2_SHIP_3: maybe_select_wl_slot (2, 3); break;
    ...
    if (Selected_wl_class != -1 &&
        ...
        model_render_params render_info;
        draw_model_rotating (
            &render_info, sel_icon->model_index, weapon_ani_coords[0],
            weapon_ani_coords[1], gr_screen.res == 0 ? 202 : 332,
            gr_screen.res == 0 ? 185 : 260, &WeapSelectScreenWeapRot,
            &wip->closeup_pos, wip->closeup_zoom * 0.65f, REVOLUTION_RATE,
            MR_IS_MISSILE | MR_AUTOCENTER | MR_NO_FOGGING, GR_RESIZE_MENU,
        ...
        generic_anim_render (
            &Cur_Anim,
            (help_overlay_active (Weapon_select_overlay_id)) ? 0 : frametime,
            weapon_ani_coords[0], weapon_ani_coords[1], true);
    }
    ...
    if (wl_icon_being_carried ()) {
        ...
        if (Wl_icons[Carried_wl_icon.weapon_class].can_use > 0) {
            ...
            if (icon->icon_bmaps[WEAPON_ICON_FRAME_SELECTED] != -1) {
                ...
                draw_brackets_square (
                    &line_draw_list, sx, sy, sx + w, sy + h, GR_RESIZE_MENU);
                line_draw_list.flush ();

                if (icon->model_index != -1) {
                    // Draw the model
                    draw_model_icon (
                        icon->model_index, MR_NO_FOGGING | MR_NO_LIGHTING,
                        wip->closeup_zoom / 2.5f, sx, sy, w, h, NULL,
                        GR_RESIZE_MENU, &wip->closeup_pos);
                }
                    ...
                    // Draw the weapon name, crappy last-ditch effort to not
                    // crash.
                    int half_x, half_y;
                    ...
                    half_y = sy + ((28 - half_y) /
                                   2); // Was ((24 - half_y) / 2) Zacam
                    gr_string (
                        half_x, half_y, print_name.c_str (), GR_RESIZE_MENU);
                }
            }
        }
        ...
        if (Carried_wl_icon.from_bank >= MAX_SHIP_PRIMARY_BANKS) {
            if (mx == Carried_wl_icon.from_x && my == Carried_wl_icon.from_y) {
                int num_missiles = Wss_slots[Carried_wl_icon.from_slot]
                                       .wep_count[Carried_wl_icon.from_bank];

                wl_render_icon_count (
                    num_missiles,
                    Wl_bank_coords[gr_screen.res][Carried_wl_icon.from_bank]
                                  [0],
                    Wl_bank_coords[gr_screen.res][Carried_wl_icon.from_bank]
                                  [1]);
            }
        ...
        if (!Wl_icons[Carried_wl_icon.weapon_class].can_use) {
            ...
            if ((diffx > 2) || (diffy > 2)) {
                ...
                if (Lcl_gr && !Disable_built_in_translations) {
                    ...
                    popup (
                        PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                        XSTR ("A %s is unable to carry %s weaponry", 633),
                        (Ship_info[ship_class].alt_name[0] != '\0')
                            ? Ship_info[ship_class].alt_name
                            : Ship_info[ship_class].name,
                        display_name);
                }
                else {
                    popup (
...
void weapon_select_close () {
    ...
    if (Weapon_slot_bitmap != -1) { bm_release (Weapon_slot_bitmap); }
}
 ...
 * @param x x screen position OF THE ICON (NOT where you want the text, this is
 * calculated to prevent overlapping)
 * @param y y screen position OF THE ICON (NOT where you want the text, this is
 * calculated to prevent overlapping)
 */
void wl_render_icon_count (int num, int x, int y) {
    char buf[32];
 ...
 * Render icon
 *
 * @param index             index into Wl_icons[], identifying which weapon to
 * draw
 * @param x                 x screen position to draw icon at
 * @param y                 y screen position to draw icon at
 * @param num               count for weapon
 * @param draw_num_flag     0 if not to draw count for weapon, nonzero
 * otherwise
 * @param hot_mask          value that should match Hot_weapon_icon to show
 * mouse is over
 * @param hot_bank_mask     value that should match Hot_weapon_bank_icon to
 * show mouse is over
 * @param select_mask       value that should match Selected_wl_class to show
 * icon is selected
 */
void wl_render_icon (
    int index, int x, int y, int num, int draw_num_flag, int hot_mask,
    int hot_bank_mask, int select_mask) {
    int bitmap_id = -1;
    ...
    if (icon->icon_bmaps[0] == -1 && icon->model_index == -1 &&
        icon->laser_bmap == -1) {
        wl_load_icons (index);
    }
    ...
    if (!wl_icon_being_carried ()) {
        if ((Hot_weapon_icon > -1 && Hot_weapon_icon == hot_mask) ||
            (Hot_weapon_bank_icon > -1 &&
             Hot_weapon_bank_icon == hot_bank_mask)) {
            if (icon->icon_bmaps[0] != -1)
                bitmap_id =
                    icon->icon_bmaps[WEAPON_ICON_FRAME_HOT]; // normal frame
            else
    ...
    if (Selected_wl_class > -1) {
        if (Selected_wl_class == select_mask) {
            if (icon->icon_bmaps[0] != -1)
                bitmap_id =
                    icon->icon_bmaps[WEAPON_ICON_FRAME_SELECTED]; // selected
                                                                  // icon
            else
    ...
    if (!icon->can_use) {
        if (icon->icon_bmaps[0] != -1)
            bitmap_id =
                icon->icon_bmaps[WEAPON_ICON_FRAME_DISABLED]; // disabled icon
        else
    ...
    if (bitmap_id != -1) {
        ...
        graphics::line_draw_list line_draw_list;
        draw_brackets_square (
            &line_draw_list, x, y, x + 56, y + 24, GR_RESIZE_MENU);
        line_draw_list.flush ();
        ...
        if (icon->model_index != -1) {
            ...
            draw_model_icon (
                icon->model_index, MR_NO_FOGGING | MR_NO_LIGHTING,
                Weapon_info[index].closeup_zoom * 0.4f, x, y, 56, 24, NULL,
                GR_RESIZE_MENU);
        }
        else if (icon->laser_bmap != -1) {
    ...
    // now, right justified
    if (draw_num_flag != 0) { wl_render_icon_count (num, x, y); }
}
...
void wl_draw_ship_weapons (int index) {
    ...
    for (i = 0; i < MAX_SHIP_WEAPONS; i++) {
        if (i < Ship_info[Wss_slots[index].ship_class].num_primary_banks ||
            ...
            if (Weapon_slot_bitmap != -1) {
                ...
                gr_bitmap (
                    Wl_bank_coords[gr_screen.res][i][0] - 2,
                    Wl_bank_coords[gr_screen.res][i][1] - 2, GR_RESIZE_MENU);
            }
            else {
                gr_set_color_fast (&Icon_colors[WEAPON_ICON_FRAME_NORMAL]);
                graphics::line_draw_list line_draw_list;
                draw_brackets_square (
                    &line_draw_list, Wl_bank_coords[gr_screen.res][i][0],
                    Wl_bank_coords[gr_screen.res][i][1],
                    Wl_bank_coords[gr_screen.res][i][0] + 56,
                    Wl_bank_coords[gr_screen.res][i][1] + 24, GR_RESIZE_MENU);
                line_draw_list.flush ();
            }
        }

        if (Carried_wl_icon.from_bank == i &&
            Carried_wl_icon.from_slot == index) {
            continue;
        }

        if ((wep[i] != -1) && (wep_count[i] > 0)) {
            wl_render_icon (
                wep[i], Wl_bank_coords[gr_screen.res][i][0],
                Wl_bank_coords[gr_screen.res][i][1], wep_count[i],
                Wl_bank_count_draw_flags[i], -1, i, wep[i]);
        }
    }
}
...
void draw_wl_icon_with_number (int list_count, int weapon_class) {
    ...
    wl_render_icon (
        weapon_class, Wl_weapon_icon_coords[gr_screen.res][list_count][0],
...
int wl_slots_all_empty (wss_unit* slot) {
    ...
    for (i = 0; i < MAX_SHIP_WEAPONS; i++) {
        if ((slot->wep_count[i] > 0) && (slot->wep[i] >= 0)) return 0;
    }
...
void wl_update_parse_object_weapons (p_object* pobjp, wss_unit* slot) {
    ...
    for (i = 0; i < MAX_SHIP_PRIMARY_BANKS; i++) { ss->primary_banks[i] = -1; }
    ...
    for (i = 0; i < MAX_SHIP_PRIMARY_BANKS; i++) {
        if ((slot->wep_count[i] > 0) && (slot->wep[i] >= 0)) {
            ...
            if (Weapon_info[slot->wep[i]]
                    .wi_flags[Weapon::Info_Flags::Ballistic]) {
                // Important: the primary_ammo[] value is a percentage of max
                // capacity! which means that it's always 100%, since ballistic
                // primaries are completely full upon launch - Goober5000
    ...
    for (i = 0; i < MAX_SHIP_SECONDARY_BANKS; i++) {
        ...
        if ((slot->wep_count[sidx] > 0) && (slot->wep[sidx] >= 0)) {
            ...
            // Important: the secondary_ammo[] value is a percentage of max
            // capacity!
            max_count = wl_calc_missile_fit (
                slot->wep[sidx],
...
void start_weapon_animation (int weapon_class) {
    ...
    if (strcmp (Cur_Anim.filename, Weapon_info[weapon_class].anim_filename) !=
        ...
        if (generic_anim_stream (&Cur_Anim) == -1) {
            // we've failed to load an animation, load an image and treat it
            // like a 1 frame animation
            Cur_Anim.first_frame = bm_load (
                Weapon_info[weapon_class]
                    .anim_filename); // if we fail here, the value is still -1
            if (Cur_Anim.first_frame != -1) { Cur_Anim.num_frames = 1; }
        }
    }
    ...
    if (Wl_icons[weapon_class].model_index >= 0) return;
}
...
void wl_bash_ship_weapons (ship_weapon* swp, wss_unit* slot) {
    ...
    for (i = 0; i < MAX_SHIP_PRIMARY_BANKS; i++) {
        ...
        if ((slot->wep_count[i] > 0) && (slot->wep[i] >= 0)) {
            ...
            // ballistic primaries - Goober5000
            if (Weapon_info[swp->primary_bank_weapons[j]]
                    .wi_flags[Weapon::Info_Flags::Ballistic]) {
                // this is a bit tricky: we must recalculate ammo for full
                // capacity since wep_count for primaries does not store the
                // ammo; ballistic primaries always come with a full magazine
                swp->primary_bank_ammo[j] = wl_calc_ballistic_fit (
                    swp->primary_bank_weapons[j],
...
void wl_swap_weapons (int ship_slot, int from_bank, int to_bank) {
    ...
    if (from_bank == to_bank ||
        (from_bank >= MAX_SHIP_WEAPONS || from_bank < 0) ||
        (to_bank >= MAX_SHIP_WEAPONS || to_bank < 0)) {
        return;
    }
...
void wl_saturate_bank (int ship_slot, int bank) {
    ...
    if ((slot->wep[bank] < 0) || (slot->wep_count[bank] <= 0)) { return; }
...
int wl_swap_slot_slot (
    ...
    class_mismatch_flag =
        (IS_BANK_PRIMARY (from_bank) && IS_BANK_SECONDARY (to_bank)) ||
    ...
    if (!class_mismatch_flag) {
        ...
        if (eval_weapon_flag_for_game_type (
                sip->restricted_loadout_flag[to_bank])) {
            if (!eval_weapon_flag_for_game_type (
                    sip->allowed_bank_restricted_weapons
                        [to_bank][slot->wep[from_bank]])) {
                char display_name[NAME_LENGTH];
        ...
        // check the from-bank
        if (eval_weapon_flag_for_game_type (
                sip->restricted_loadout_flag[from_bank])) {
            if (!eval_weapon_flag_for_game_type (
                    sip->allowed_bank_restricted_weapons
                        [from_bank][slot->wep[to_bank]])) {
                // going from "from" to "to" is valid (from previous if), but
                // going the other way isn't... so return the "to" to the list
                // and just move the "from"

                // put to_bank back into list
                Wl_pool[slot->wep[to_bank]] +=
                    slot->wep_count[to_bank]; // return to list
    ...
    if (class_mismatch_flag) {
        ...
        Wl_pool[slot->wep[from_bank]] +=
            slot->wep_count[from_bank]; // return to list
    ...
    // case 1: primaries (easy, even with ballistics, because ammo is always
    // maximized)
    ...
    if (IS_BANK_SECONDARY (from_bank) && IS_BANK_SECONDARY (to_bank)) {
        ...
        if (slot->wep[from_bank] == slot->wep[to_bank]) {
            ...
            dest_max = wl_calc_missile_fit (
                slot->wep[to_bank],
                Ship_info[slot->ship_class]
                    .secondary_bank_ammo_capacity[to_bank - 3]);
            ...
            if (source_can_give > 0) {
                ...
                slot->wep_count[from_bank] -=
                    source_can_give; // take from source
                *sound = InterfaceSounds::ICON_DROP_ON_WING;
                return 1;
            }
            else {
...
int wl_grab_from_list (
    ...
    if ((IS_LIST_PRIMARY (from_list) && IS_BANK_SECONDARY (to_bank)) ||
        (IS_LIST_SECONDARY (from_list) && IS_BANK_PRIMARY (to_bank))) {
        // do nothing
        *sound = InterfaceSounds::ICON_DROP;
        return 0;
    }
    ...
    if (eval_weapon_flag_for_game_type (
            sip->restricted_loadout_flag[to_bank])) {
        if (!eval_weapon_flag_for_game_type (
                sip->allowed_bank_restricted_weapons[to_bank][from_list])) {
            char display_name[NAME_LENGTH];
            char txt[39 + NAME_LENGTH];
    ...
    if (to_bank < MAX_SHIP_PRIMARY_BANKS) { max_fit = 1; }
    else {
        max_fit = wl_calc_missile_fit (
            from_list, Ship_info[slot->ship_class].secondary_bank_ammo_capacity
                           [to_bank - MAX_SHIP_PRIMARY_BANKS]);
    }
...
int wl_swap_list_slot (
    ...
    // take weapon from list
    if (Wl_pool[from_list] < max_fit) { max_fit = Wl_pool[from_list]; }
    Wl_pool[from_list] -= max_fit;
...
int wl_apply (
    int mode, int from_bank, int from_list, int to_bank, int to_list,
    ...
    // only play this sound if the move was done locally (by the host in other
    // words)
...
void wl_apply_current_loadout_to_all_ships_in_current_wing () {
    ...
    if (source_wss_slot == -1) source_wss_slot = 0;
    ...
    for (cur_wing_slot = 0; cur_wing_slot < MAX_WING_SLOTS; cur_wing_slot++) {
        ...
        if (cur_wss_slot == source_wss_slot) continue;

        // must be valid for us to change
        if (!ss_valid_slot (cur_wss_slot)) continue;

        // copy weapons from source slot to this slot
        for (cur_bank = 0; cur_bank < MAX_SHIP_WEAPONS; cur_bank++) {
            // this bank must exist on both the source ship and the destination
            // ship
            if (cur_bank < MAX_SHIP_PRIMARY_BANKS) {
                if (cur_bank >= source_sip->num_primary_banks ||
                    cur_bank >= sip->num_primary_banks)
                    continue;
            }
            else {
                if (cur_bank - MAX_SHIP_PRIMARY_BANKS >=
                        source_sip->num_secondary_banks ||
                    cur_bank - MAX_SHIP_PRIMARY_BANKS >=
                        sip->num_secondary_banks)
                    continue;
            }

            // dump the destination ship's weapons
            wl_drop (
                cur_bank, -1, -1, Wss_slots[cur_wss_slot].wep[cur_bank],
                cur_wss_slot, -1, true);

            // weapons must be present on the source ship
            if (Wss_slots[source_wss_slot].wep[cur_bank] < 0) continue;
            ...
            if (!eval_weapon_flag_for_game_type (
                    sip->allowed_weapons[weapon_type_to_add])) {
                std::string temp;
                sprintf (
                    temp, XSTR ("%s is unable to carry %s weaponry", 1629),
                    ship_name, wep_display_name);
            ...
            if (eval_weapon_flag_for_game_type (
                    sip->restricted_loadout_flag[cur_bank])) {
                if (!eval_weapon_flag_for_game_type (
                        sip->allowed_bank_restricted_weapons
                            [cur_bank][weapon_type_to_add])) {
                    std::string temp;
                    if (cur_bank < MAX_SHIP_PRIMARY_BANKS)
                        sprintf (
                            temp,
                            XSTR (
                                "%s is unable to carry %s weaponry in primary "
                                "bank %d",
                                1630),
                            ship_name, wep_display_name, cur_bank + 1);
                    else
                        sprintf (
                            temp,
                            XSTR (
                                "%s is unable to carry %s weaponry in "
                                "secondary bank %d",
                                1631),
                            ship_name, wep_display_name,
            ...
            result = wl_drop (
                -1, weapon_type_to_add, cur_bank, -1, cur_wss_slot, -1, true);

            // bank left unfilled or partially filled
            if ((result == 0) || (result == 2)) {
                std::string temp;
                sprintf (
                    temp, XSTR ("Insufficient %s available to arm %s", 1632),
                    Weapon_info[weapon_type_to_add].get_display_string (),
                    ship_name);
                error_messages.push_back (temp);
    ...
    if (error_flag) {
        ...
        // display popup
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK, full_error_message.c_str ());
    }
}
