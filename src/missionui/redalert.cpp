...
static int Red_alert_new_mission_timestamp; // timestamp used to give user a
                                            // little warning for red alerts
...
const char* Red_alert_mask[GR_NUM_RESOLUTIONS] = { "RedAlert-m",
                                                   "2_RedAlert-m" };

// font to use for "incoming transmission"
int Ra_flash_font[GR_NUM_RESOLUTIONS] = { font::FONT2, font::FONT2 };
...
UI_XSTR Red_alert_text[GR_NUM_RESOLUTIONS][RED_ALERT_NUM_TEXT] = {
    {
        // GR_640
        { "Replay", 1405, 46, 451, UI_XSTR_COLOR_PINK, -1,
          &Buttons[0][RA_REPLAY_MISSION].button },
        { "Previous Mission", 1452, 46, 462, UI_XSTR_COLOR_PINK, -1,
          &Buttons[0][RA_REPLAY_MISSION].button },
        { "Continue", 1069, 564, 413, UI_XSTR_COLOR_PINK, -1,
          &Buttons[0][RA_CONTINUE].button },
    },
    {
        // GR_1024
        { "Replay", 1405, 75, 722, UI_XSTR_COLOR_PINK, -1,
          &Buttons[1][RA_REPLAY_MISSION].button },
        { "Previous Mission", 1452, 75, 733, UI_XSTR_COLOR_PINK, -1,
          &Buttons[1][RA_REPLAY_MISSION].button },
        { "Continue", 1069, 902, 661, UI_XSTR_COLOR_PINK, -1,
...
void red_alert_button_pressed (int n) {
    switch (n) {
    case RA_CONTINUE:
        // warp the mouse cursor the the middle of the screen for those who
    ...
    case RA_REPLAY_MISSION:
        if (Game_mode & GM_CAMPAIGN_MODE) {
            // TODO: make call to campaign code to set correct mission for
            // loading
...
void red_alert_blit_title () {
    ...
    if (Ra_flash_up) {
        gr_init_alphacolor (
            &flash_color, (int)(255.0f * (Ra_flash_time / RA_FLASH_CYCLE)), 0,
            0, 255);
    }
    else {
        gr_init_alphacolor (
            &flash_color,
            (int)(255.0f * (1.0f - (Ra_flash_time / RA_FLASH_CYCLE))), 0, 0,
            255);
    }
    ...
    gr_string (
        Ra_brief_text_wnd_coords[gr_screen.res][0] +
            ((Ra_brief_text_wnd_coords[gr_screen.res][2] - w) / 2),
        Ra_flash_y[gr_screen.res] - h - 5, str, GR_RESIZE_MENU);
...
void red_alert_init () {
    ...
    Buttons[gr_screen.res][RA_CONTINUE].button.set_hotkey (
        KEY_CTRLED | KEY_ENTER);

    // hud_anim_init(&Flash_anim, Ra_flash_coords[gr_screen.res][RA_X_COORD],
    // Ra_flash_coords[gr_screen.res][RA_Y_COORD], NOX("AlertFlash"));
    // hud_anim_load(&Flash_anim);

    Red_alert_voice = -1;

    if (!Briefing) { Briefing = &Briefings[0]; }

    // load in background image and flashing red alert animation
    Background_bitmap = mission_ui_background_load (
        Briefing->background[gr_screen.res], Red_alert_fname[gr_screen.res]);
    ...
    if (Briefing->num_stages > 0) {
        brief_color_text_init (
            Briefing->stages[0].text.c_str (),
            Ra_brief_text_wnd_coords[gr_screen.res][RA_W_COORD],
            default_redalert_briefing_color, 0);
    }

    red_alert_voice_load ();

    // we have to reset/setup the shipselect and weaponselect pointers before
    // moving on
...
void red_alert_do_frame (float frametime) {
    ...
    if (timestamp_elapsed (Text_delay)) {
        int finished_wipe = 0;
        if (Briefing->num_stages > 0) {
            finished_wipe = brief_render_text (
                0, Ra_brief_text_wnd_coords[gr_screen.res][RA_X_COORD],
                Ra_brief_text_wnd_coords[gr_screen.res][RA_Y_COORD],
                Ra_brief_text_wnd_coords[gr_screen.res][RA_H_COORD], frametime,
                0);
        }

        if (finished_wipe) { red_alert_voice_play (); }
    }
...
void red_alert_store_weapons (red_alert_ship_status* ras, ship_weapon* swp) {
    ...
    for (i = 0; i < swp->num_primary_banks; i++) {
        ...
        if (wip->wi_flags[Weapon::Info_Flags::Ballistic]) {
            // to avoid triggering the below condition: this way, minimum ammo
            // will be 2... since the red-alert representation of a
            // conventional primary is 0 -> not used, 1 -> used, I added the
            // representation 2 and above -> ballistic primary
...
// Take the weapons stored in a wingman_status struct, and bash them into the
// ship weapons struct
...
void red_alert_bash_weapons (red_alert_ship_status* ras, p_object* pobjp) {
    ...
    for (i = 0; i < list_size; i++) {
        ...
        if (Weapon_info[sssp->primary_banks[i]]
                .wi_flags[Weapon::Info_Flags::Ballistic]) {
            float max_count = sip->primary_bank_ammo_capacity[i] /
                              Weapon_info[sssp->primary_banks[i]].cargo_size;
            sssp->primary_ammo[i] = (int)std::lround (
                100.0f * (ras->primary_weapons[i].count - 2) / max_count);
        }
        else
    ...
    for (i = 0; i < list_size; i++) {
        ...
        float max_count = sip->secondary_bank_ammo_capacity[i] /
                          Weapon_info[sssp->secondary_banks[i]].cargo_size;
        sssp->secondary_ammo[i] = (int)std::lround (
            100.0f * ras->secondary_weapons[i].count / max_count);
    }
}
...
void red_alert_bash_subsys_status (red_alert_ship_status* ras, ship* shipp) {
    ...
    while (ss != END_OF_LIST (&shipp->subsys_list)) {
        ...
        if (ss->current_hits <= 0) { ss->submodel_info_1.blown_off = 1; }
    ...
    for (i = 0; i < list_size; i++) {
        shipp->subsys_info[i].aggregate_current_hits =
...
void red_alert_bash_subsys_status (
    red_alert_ship_status* ras, p_object* pobjp) {
    int i, j;
    ship_info* sip;
    ...
    // do this differently than the other bash_subsys_status... since the
    // p_object may not contain all the subsystems, iterate on the
    // red_alert_ship_status entries rather than the p_object's and create
    // missing subsystems where necessary
    for (i = 0; i < (int)ras->subsys_current_hits.size (); i++) {
        psub = &sip->subsystems[i];

        // in the p_object, subsystem 0 is the pilot, and afterwards the
        // subsystems go in order
        j = i + 1;
        ...
        float max_hits =
            psub->max_subsys_strength *
            (pobjp->ship_max_hull_strength / sip->max_hull_strength);
...
void red_alert_store_subsys_status (red_alert_ship_status* ras, ship* shipp) {
    ...
    for (i = 0; i < SUBSYSTEM_MAX; i++)
        ...
        ras->subsys_aggregate_current_hits.push_back (
            shipp->subsys_info[i].aggregate_current_hits);
}

/*
 * Record the current state of the players wingman & ships with the
 * "red-alert-carry" flag Wingmen without the red-alert-carry flag are only
 * stored if they survive dead wingmen must still be handled in
...
void red_alert_store_wingman_status () {
    ...
    // Probably not the best solution, but it prevents an assertion in
    // change_ship_type()
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (!(shipp->flags[Ship::Ship_Flags::From_player_wing]) &&
            !(shipp->flags[Ship::Ship_Flags::Red_alert_store_status])) {
            continue;
        }
    ...
    for (int idx = 0; idx < (int)Ships_exited.size (); idx++) {
        if (Ships_exited[idx].flags[Ship::Exit_Flags::Red_alert_carry]) {
            ...
            // if a ship has been destroyed or removed manually by the player,
            // then mark it as such ...
            if (Ships_exited[idx].flags[Ship::Exit_Flags::Destroyed]) {
                ras.ship_class = RED_ALERT_DESTROYED_SHIP_CLASS;
            }
            else if (Ships_exited[idx]
                         .flags[Ship::Exit_Flags::Player_deleted]) {
                ras.ship_class = RED_ALERT_PLAYER_DEL_SHIP_CLASS;
            }
...
void red_alert_delete_ship (p_object* pobjp, int ship_state) {
    if (ship_state == RED_ALERT_DESTROYED_SHIP_CLASS ||
        ship_state == RED_ALERT_PLAYER_DEL_SHIP_CLASS) {
...
/*
 * Take the red alert status information, and adjust the red alert ships
 * accordingly "red alert ships" are wingmen and any ship with the
 * red-alert-carry flag Wingmen without red alert data still need to be handled
 * / removed
 */
void red_alert_bash_wingman_status () {
    ...
    if (Red_alert_wingman_status.empty ()) { return; }

    // go through all ships in the game, and see if there is red alert status
    // data for any
    ...
    for (; so != END_OF_LIST (&Ship_obj_list);) {
        ...
        for (rasii = Red_alert_wingman_status.begin ();
             rasii != Red_alert_wingman_status.end (); ++rasii) {
        ...
        // remove ship if it was destroyed, or if there's no red-alert data for
        // it
        if (!ship_data_restored) {
            // we need to be a little tricky here because deletion invalidates
            // the ship_obj
    ...
    // NOTE: in retail, red alert data was not loaded for ships that arrived
    // later in the mission
    if (!Red_alert_applies_to_delayed_ships) return;

    // go through all ships yet to arrive, and see if there is red alert status
    // data for any

    for (poii = Parse_objects.begin (); poii != Parse_objects.end (); ++poii) {
        p_object* pobjp = &(*poii);

        // objects that have already arrived would have been handled in the
        // above loop
        if (pobjp->created_object != NULL) continue;
        ...
        if (!from_player_wing &&
            !(pobjp->flags
                  [Mission::Parse_Object_Flags::SF_Red_alert_store_status])) {
            continue;
        }
        ...
        for (rasii = Red_alert_wingman_status.begin ();
            ...
            if (!strcasecmp (ras->name.c_str (), pobjp->name)) {
                ...
                if ((ras->ship_class != RED_ALERT_DESTROYED_SHIP_CLASS) &&
                    ...
                    // restore hull (but not shields)
                    if (ras->hull >= 0.0f &&
                        ras->hull <= (pobjp->initial_hull *
                                      pobjp->ship_max_hull_strength / 100.0f))
                        pobjp->initial_hull =
        ...
        if (!ship_data_restored) {
            ...
            if (pobjp->wingnum >= 0) Wing_pobjects_deleted[pobjp->wingnum]++;
        }
    }

    // if all parse objects in a wing have been removed, decrement the count
    // for that wing
    for (ii = Wing_pobjects_deleted.begin ();
         ii != Wing_pobjects_deleted.end (); ++ii) {
        ...
        if (wingp->num_waves > 0 && wingp->wave_count == ii->second) {
            ...
            for (p_object* pobjp = GET_FIRST (&Ship_arrival_list);
                 pobjp != END_OF_LIST (&Ship_arrival_list);
                 pobjp = GET_NEXT (pobjp)) {
                ...
                if (pobjp->wingnum == ii->first) {
                    ...
                    // we skipped one complete wave, so clear the flag so the
                    // next wave creates all ships
                    else
                        pobjp->flags.remove (
...
void red_alert_start_mission () {
    ...
    if (Player_obj->type == OBJ_SHIP) {
        if (Player_obj->hull_strength > 0) {
            ...
            Player_ship->ship_guardian_threshold =
                SHIP_GUARDIAN_THRESHOLD_DEFAULT;
...
void red_alert_maybe_move_to_next_mission () {
    ...
    if (!timestamp_valid (Red_alert_new_mission_timestamp)) return;

    // return if the timestamp hasn't elapsed yet
    if (!timestamp_elapsed (Red_alert_new_mission_timestamp)) return;

    // basic premise here is to stop the current mission, and then set the next
    // mission in the campaign which better be a red alert mission
    if (Game_mode & GM_CAMPAIGN_MODE) {
 ...
 */
void red_alert_clear () { Red_alert_wingman_status.clear (); }
