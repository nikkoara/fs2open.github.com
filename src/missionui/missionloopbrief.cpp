...
UI_XSTR Loop_text[GR_NUM_RESOLUTIONS][NUM_LOOP_TEXT] = {
    {
        // GR_640
        { "Decline", 1467, 514, 413, UI_XSTR_COLOR_PINK, -1,
    ...
    {
        // GR_1024
        { "Decline", 1467, 855, 670, UI_XSTR_COLOR_PINK, -1,
          &Loop_buttons[1][LOOP_BRIEF_DECLINE].button },
        { "Accept", 1035, 928, 670, UI_XSTR_COLOR_PINK, -1,
...
// text window coords
int Loop_brief_text_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                        25, 107, 591, 143 },
                                                      { // GR_1024
                                                        40, 171, 945, 229 } };
...
void loop_brief_button_pressed (int i) {
    ...
    case LOOP_BRIEF_ACCEPT:
        ...
        Campaign.loop_reentry =
            Campaign
                .next_mission; // save reentry pt, so we can break out of loop
...
void loop_brief_init () {
    ...
    if (Campaign.missions[Campaign.current_mission]
            .mission_branch_brief_anim != NULL) {
        anim_name = Campaign.missions[Campaign.current_mission]
                        .mission_branch_brief_anim;
    }
    else {
    ...
    int stream_result = generic_anim_init_and_stream (
        &Loop_anim, anim_name, bm_get_type (Loop_brief_bitmap), true);
    // we've failed to load any animation
    if (stream_result < 0) {
        // load an image and treat it like a 1 frame animation
        Loop_anim.first_frame =
            bm_load (anim_name); // if we fail here, the value is still -1
        if (Loop_anim.first_frame != -1) { Loop_anim.num_frames = 1; }
    }
    ...
    if (Campaign.missions[Campaign.current_mission]
        ...
        Loop_sound = audiostream_open (
            Campaign.missions[Campaign.current_mission]
                .mission_branch_brief_sound,
            ASF_VOICE);
...
void loop_brief_do (float frametime) {
    ...
    case KEY_ESC:
        ...
        // this popup should be straight forward, and also not allow you to get
        // out of it without actually picking one of the two options
        do_loop = popup (
            PF_USE_NEGATIVE_ICON | PF_USE_AFFIRMATIVE_ICON | PF_IGNORE_ESC |
                PF_BODY_BIG,
            2, XSTR ("Decline", 1467), XSTR ("Accept", 1035),
            XSTR (
                "You must either Accept or Decline before returning to the "
                "Main Hall",
                1618));

        // if we accepted moving into loop then set it up for the next time the
        // user plays
        if (do_loop == 1) {
            // select the loop mission
            Campaign.loop_enabled = 1;
            Campaign.loop_reentry =
                Campaign.next_mission; // save reentry pt, so we can break out
                                       // of loop
            Campaign.next_mission = Campaign.loop_mission;
        }
    ...
    brief_render_text (
        0, Loop_brief_text_coords[gr_screen.res][0],
        Loop_brief_text_coords[gr_screen.res][1],
    ...
    if (Loop_anim.num_frames > 0) {
        ...
        bm_get_info (
            (Loop_anim.streaming) ? Loop_anim.bitmap_id
                                  : Loop_anim.first_frame,
            &x, &y, NULL, NULL, NULL);
...
void loop_brief_close () {
    // this makes sure that we're all cool no matter how the user decides to
    // exit
    mission_campaign_mission_over ();

    // free the bitmap
    if (Loop_brief_bitmap >= 0) { bm_release (Loop_brief_bitmap); }
    Loop_brief_bitmap = -1;

    // destroy the window
    Loop_brief_window.destroy ();

    if (Loop_anim.num_frames > 0) { generic_anim_unload (&Loop_anim); }
