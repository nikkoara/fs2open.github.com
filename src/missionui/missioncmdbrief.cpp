...
const char* Cmd_brief_fname[NUM_CMD_SETTINGS][GR_NUM_RESOLUTIONS] = {
    { "CommandBrief", "2_CommandBrief" },
    { "CommandBriefb", "2_CommandBriefb" }
};

const char* Cmd_brief_mask[NUM_CMD_SETTINGS][GR_NUM_RESOLUTIONS] = {
    { "CommandBrief-m", "2_CommandBrief-m" },
    { "CommandBrief-mb", "2_CommandBrief-mb" },
};
...
#define NUM_CMD_BRIEF_BUTTONS \
    (Uses_scroll_buttons ? MAX_CMD_BRIEF_BUTTONS : MIN_CMD_BRIEF_BUTTONS)
...
UI_XSTR Cmd_brief_text[GR_NUM_RESOLUTIONS][CMD_BRIEF_NUM_TEXT] = {
    ...
    {
        // GR_1024
        { "Help", 928, 820, 704, UI_XSTR_COLOR_GREEN, -1,
        ...
        { "Continue", 1069, 936, 661, UI_XSTR_COLOR_PINK, -1,
          &Cmd_brief_buttons[1][CMD_BRIEF_BUTTON_ACCEPT].button },
    }
};

static UI_WINDOW Ui_window;
static int Cmd_brief_background_bitmap; // bitmap for the background of the
                                        // cmd_briefing
...
int cmd_brief_check_stage_done () {
    ...
    if (Voice_ended_time &&
        (timer_get_milliseconds () - Voice_ended_time >= 1000))
        return 1;

    // check normal speech
    if (Briefing_voice_enabled && (Cmd_brief_last_voice >= 0)) {
        if (audiostream_is_playing (Cmd_brief_last_voice)) { return 0; }
 ...
 */
void cmd_brief_exit () {
    // I know, going to red alert from cmd brief is stupid, but we have stupid
    // fredders
    if (red_alert_mission ()) { gameseq_post_event (GS_EVENT_RED_ALERT); }
    else {
...
void cmd_brief_new_stage (int stage) {
    ...
    brief_color_text_init (
        Cur_cmd_brief->stage[stage].text.c_str (),
        Cmd_text_wnd_coords[Uses_scroll_buttons][gr_screen.res][CMD_W_COORD],
        default_command_briefing_color);

    // load a new animation if it's different to what's already playing
    if (strcmp (Cur_anim_filename, Cur_cmd_brief->stage[stage].ani_filename) !=
        0) {
        ...
        int stream_result = generic_anim_init_and_stream (
            &Cur_Anim, Cur_anim_filename,
            bm_get_type (Cmd_brief_background_bitmap), true);

        // we've failed to load any animation
        if (stream_result < 0) {
            // load an image and treat it like a 1 frame animation
            Cur_Anim.first_frame = bm_load (
                Cur_cmd_brief->stage[stage]
                    .ani_filename); // if we fail here, the value is still -1
...
void cmd_brief_hold () { cmd_brief_stop_anim (); }

void cmd_brief_unhold () { cmd_brief_new_stage (Cur_stage); }

extern int Briefing_music_handle;

void cmd_brief_pause () {
    if (Cmd_brief_paused) return;
...
void cmd_brief_button_pressed (int n) {
    ...
    case CMD_BRIEF_BUTTON_FIRST_STAGE:
        if (common_num_cutscenes_valid (MOVIE_PRE_CMD_BRIEF)) {
            ...
            common_maybe_play_cutscene (
                MOVIE_PRE_CMD_BRIEF, true, SCORE_BRIEFING);
    ...
    case CMD_BRIEF_BUTTON_SCROLL_DOWN:
        Top_cmd_brief_text_line++;
        if ((Num_brief_text_lines[0] - Top_cmd_brief_text_line) <
            Max_cmdbrief_Lines) {
            Top_cmd_brief_text_line--;
            gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
        }
...
void cmd_brief_init (int team) {
    ...
    if (Cur_cmd_brief->num_stages <= 0) return;
    ...
    Cmd_brief_background_bitmap = mission_ui_background_load (
        Cur_cmd_brief->background[gr_screen.res],
        Cmd_brief_fname[Uses_scroll_buttons]
                       [gr_screen.res]); // try to load extra one first
    if (Cmd_brief_background_bitmap < 0) // failed to load
    {
        Uses_scroll_buttons = 0; // nope, sorry
        Cmd_brief_background_bitmap = mission_ui_background_load (
            NULL, Cmd_brief_fname[Uses_scroll_buttons][gr_screen.res]);
    }

    Ui_window.create (
        0, 0, gr_screen.max_w_unscaled, gr_screen.max_h_unscaled, 0);
    Ui_window.set_mask_bmap (
        Cmd_brief_mask[Uses_scroll_buttons][gr_screen.res]);
    ...
    // set up readyrooms for buttons so we draw the correct animation frame
    // when a key is pressed
    Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_FIRST_STAGE]
        .button.set_hotkey (KEY_SHIFTED | KEY_LEFT);
    Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_LAST_STAGE]
        .button.set_hotkey (KEY_SHIFTED | KEY_RIGHT);
    Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_PREV_STAGE]
        .button.set_hotkey (KEY_LEFT);
    Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_NEXT_STAGE]
        .button.set_hotkey (KEY_RIGHT);
    Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_ACCEPT]
        .button.set_hotkey (KEY_CTRLED | KEY_ENTER);
    Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_HELP].button.set_hotkey (
        KEY_F1);
    Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_OPTIONS]
        .button.set_hotkey (KEY_F2);

    // extra - Goober5000
    if (Uses_scroll_buttons) {
        Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_SCROLL_UP]
            .button.set_hotkey (KEY_UP);
        Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_SCROLL_DOWN]
...
void cmd_brief_do_frame (float frametime) {
    ...
    if (help_overlay_active (Cmd_brief_overlay_id)) {
        Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_HELP]
            .button.reset_status ();
    ...
    if (Cur_Anim.num_frames > 0) {
        bm_get_info (
            (Cur_Anim.streaming) ? Cur_Anim.bitmap_id : Cur_Anim.first_frame,
            &x, &y, NULL, NULL, NULL);
        x = Cmd_image_center_coords[gr_screen.res][CMD_X_COORD] - x / 2;
        y = Cmd_image_center_coords[gr_screen.res][CMD_Y_COORD] - y / 2;
        generic_anim_render (
            &Cur_Anim, (Cmd_brief_paused) ? 0 : frametime, x, y, true);
    }

    Ui_window.draw ();

    if (!Player->auto_advance) {
        Cmd_brief_buttons[gr_screen.res][CMD_BRIEF_BUTTON_PAUSE]
    ...
    sprintf (
        buf, XSTR ("Stage %d of %d", 464), Cur_stage + 1,
        Cur_cmd_brief->num_stages);
    gr_get_string_size (&w, NULL, buf);
    gr_string (
        Cmd_text_wnd_coords[Uses_scroll_buttons][gr_screen.res][CMD_X_COORD] +
            Cmd_text_wnd_coords[Uses_scroll_buttons][gr_screen.res]
                               [CMD_W_COORD] -
            w,
        Cmd_stage_y[gr_screen.res], buf, GR_RESIZE_MENU);

    if (brief_render_text (
            Top_cmd_brief_text_line,
            Cmd_text_wnd_coords[Uses_scroll_buttons][gr_screen.res]
                               [CMD_X_COORD],
            Cmd_text_wnd_coords[Uses_scroll_buttons][gr_screen.res]
    ...
    Max_cmdbrief_Lines =
        ...
        (gr_get_font_height () +
         1); // Make the max number of lines dependent on the font height,
             // keeping in mind that we have an extra pixel between lines.

    // maybe output the "more" indicator
    if ((Max_cmdbrief_Lines + Top_cmd_brief_text_line) <
        Num_brief_text_lines[0]) {
        // can be scrolled down
        int more_txt_x = Cmd_text_wnd_coords[Uses_scroll_buttons]
                                            [gr_screen.res][CMD_X_COORD] +
                         (Cmd_text_wnd_coords[Uses_scroll_buttons]
                                             [gr_screen.res][CMD_W_COORD] /
                          2) -
                         10;
        int more_txt_y = Cmd_text_wnd_coords[Uses_scroll_buttons]
                                            [gr_screen.res][CMD_Y_COORD] +
                         Cmd_text_wnd_coords[Uses_scroll_buttons]
                                            [gr_screen.res][CMD_H_COORD] -
