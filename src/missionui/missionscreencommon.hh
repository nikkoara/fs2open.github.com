...
extern brief_common_buttons Common_buttons[3][GR_NUM_RESOLUTIONS]
                                          [NUM_COMMON_BUTTONS];
...
// common_select_init() performs initialization common to the briefing/ship
// select/weapon select screens.  This includes loading/setting the palette,
// loading the background animation, loading the screen switching animations,
// loading the button animation frames
...
void common_set_interface_palette(
        const char *filename = NULL); // set the interface palette
...
// music functions exported for multiplayer team selection screen to start
// briefing music
...
void common_maybe_play_cutscene(
        int movie_type, bool restart_music = false, int music = 0);
...
void draw_model_icon(
        int model_id, int flags, float closeup_zoom, int x1, int x2, int y1,
        int y2, ship_info *sip = NULL, int resize_mode = GR_RESIZE_FULL,
...
void draw_model_rotating(
        ...
        int y2, float *rotation_buffer, vec3d *closeup_pos = NULL,
        float closeup_zoom = .65f, float rev_rate = REVOLUTION_RATE,
        int flags = MR_AUTOCENTER | MR_NO_FOGGING,
        int resize_mode = GR_RESIZE_FULL, int effect = 2);
...
#endif // FREESPACE2_MISSIONUI_MISSIONSCREENCOMMON_HH
