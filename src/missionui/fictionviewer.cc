...
void fiction_viewer_init()
{
        ...
        if (*Fiction_viewer_stages[Fiction_viewer_active_stage]
            ...
            != '\0') {
                Fiction_viewer_bitmap = bm_load(Fiction_viewer_stages[Fiction_viewer_active_stage]
                                                        .background[gr_screen.res]);
                ...
                if (Fiction_viewer_ui < 0) {
                        ...
                        for (Fiction_viewer_ui = NUM_FVW_SETTINGS - 1;
                             Fiction_viewer_ui >= 0; Fiction_viewer_ui--) {
                                Fiction_viewer_bitmap = bm_load(Fiction_viewer_screen_filename[Fiction_viewer_ui]
                                                                                              [gr_screen.res]);
                                if (Fiction_viewer_bitmap >= 0)
                                        break;
                        }
        ...
        if ((Fiction_viewer_text_max_lines + Top_fiction_viewer_text_line) < Num_brief_text_lines[0]) {
                ...
                int more_txt_x = Fiction_viewer_text_coordinates[Fiction_viewer_ui]
                                 ...
                                 + (Fiction_viewer_text_coordinates[Fiction_viewer_ui]
                                    ...
                                    / 2)
                                 - 10;
                int more_txt_y = Fiction_viewer_text_coordinates[Fiction_viewer_ui][gr_screen.res]
                                                                [1]
...
int fiction_viewer_ui_name_to_index(const char *ui_name)
{
        ...
        for (i = 0; i < NUM_FVW_SETTINGS; i++) {
                if (!strcasecmp(ui_name, Fiction_viewer_ui_names[i])) {
                        return i;
                }
        }
...
{
        ASSERTX(
                stage >= 0 && static_cast< size_t >(stage) < Fiction_viewer_stages.size(),
        ...
        if (fp == NULL) {
                ...
                if (Unicode_text_mode) {
                        ...
                        Fiction_viewer_text_raw = nullptr; // Zero out the pointer so there are no accidental
                                                           // accesses anymore
                } else {
