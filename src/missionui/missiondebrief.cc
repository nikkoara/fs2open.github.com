...
void debrief_award_init()
{
        ...
        if (Player->stats.m_badge_earned.size()) {
                ...
                if (Medals[Player->stats.m_badge_earned.back()].promotion_text.find(
                            persona_index)
                    != Medals[Player->stats.m_badge_earned.back()]
                               .promotion_text.end()) {
...
void debrief_init()
{
        ...
        Max_debrief_Lines = Debrief_text_wnd_coords[gr_screen.res][3] / gr_get_font_height(); // Make the max number of lines
...
void debrief_do_keys(int new_k)
{
        ...
        case KEY_ESC: {
                ...
                if (!Turned_traitor && !Must_replay_mission && (Game_mode & GM_CAMPAIGN_MODE)) {
                        ...
                        (Turned_traitor || Must_replay_mission) && (Game_mode & GM_CAMPAIGN_MODE)) {
...
void debrief_draw_award_text()
{
        ...
        int field_width = (Medal_bitmap > 0)
        ...
        for (i = 0; i < Debrief_award_text_num_lines; i++) {
                ...
                x = (Medal_bitmap < 0) ? (Debrief_award_text_coords[gr_screen.res][0] + (field_width - sw) / 2)
...
void debrief_do_frame(float frametime)
{
        ...
        if ((Max_debrief_Lines + Text_offset) < Num_text_lines) {
                ...
                gr_printf_menu(
                        Debrief_text_wnd_coords[gr_screen.res][0] + Debrief_text_wnd_coords[gr_screen.res][2] / 2 - w / 2,
        ...
        if (Must_replay_mission && (!Debrief_skip_popup_already_shown) && (Player->show_skip_popup) && (Game_mode & GM_NORMAL) && (Game_mode & GM_CAMPAIGN_MODE) && (Player->failures_this_session >= PLAYER_MISSION_FAILURE_LIMIT)) {
