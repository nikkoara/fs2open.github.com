...
const char* Fiction_viewer_screen_filename[NUM_FVW_SETTINGS]
                                          [GR_NUM_RESOLUTIONS] = {
                                              {
...
ui_button_info Fiction_viewer_buttons
    [NUM_FVW_SETTINGS][GR_NUM_RESOLUTIONS][NUM_FVW_BUTTONS] = {
        // standard FS2-style interface
        { {
              // GR_640
              ui_button_info (
                  "fvw_accept_", 571, 425, 69, 55, FVW_BUTTON_ACCEPT),
              ui_button_info (
          ...
          },
          {
              // GR_1024
              ui_button_info (
                  "2_fvw_accept_", 918, 688, 99, 77, FVW_BUTTON_ACCEPT),
              ui_button_info (
                  "2_fvw_up_", 981, 16, 40, 50, FVW_BUTTON_SCROLL_UP),
              ui_button_info (
                  "2_fvw_down_", 981, 606, 40, 50, FVW_BUTTON_SCROLL_DOWN),
          } },
        // WCS-style interface
        { {
              // GR_640
              ui_button_info (
                  "fvw_accept_", 105, 444, 37, 26, FVW_BUTTON_ACCEPT),
              ui_button_info (
                  "fvw_up_", 576, 51, 37, 33, FVW_BUTTON_SCROLL_UP),
              ui_button_info (
                  "fvw_down_", 576, 364, 37, 33, FVW_BUTTON_SCROLL_DOWN),
          },
          {
              // GR_1024
              ui_button_info (
                  "2_fvw_accept_", 168, 710, 59, 41, FVW_BUTTON_ACCEPT),
              ui_button_info (
                  "2_fvw_up_", 922, 81, 59, 53, FVW_BUTTON_SCROLL_UP),
              ui_button_info (
                  "2_fvw_down_", 922, 582, 59, 53, FVW_BUTTON_SCROLL_DOWN),
          } }
    };

const char*
    Fiction_viewer_slider_filename[NUM_FVW_SETTINGS][GR_NUM_RESOLUTIONS] = {
        // standard FS2-style interface
        { "slider", "2_slider" },
        // WCS-style interface
        { "fvw_slider_", "2_fvw_slider_" }
    };

int Fiction_viewer_slider_coordinates
    [NUM_FVW_SETTINGS][GR_NUM_RESOLUTIONS][4] = {
        // standard FS2-style interface
        { {
              // GR_640
              618, 48, 18, 320 // Initial position x, initial position y,
                               // width, height of the slider column
          },
          { // GR_1024
            988, 70, 28, 532 } },
        // WCS-style interface
...
void fiction_viewer_scroll_down () {
    Top_fiction_viewer_text_line++;
    if ((Num_brief_text_lines[0] - Top_fiction_viewer_text_line) <
        Fiction_viewer_text_max_lines) {
        Top_fiction_viewer_text_line--;
        gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
    }
    else {
...
void fiction_viewer_init () {
    if (Fiction_viewer_inited) return;

    // no stage loaded?
    if (Fiction_viewer_active_stage < 0) return;

    // no fiction document?
    if (!mission_has_fiction ()) return;
    ...
    if (*Fiction_viewer_stages[Fiction_viewer_active_stage]
             .background[gr_screen.res] != '\0') {
    ...
    if (Fiction_viewer_bitmap < 0) {
        if (Fiction_viewer_ui < 0) {
            // no UI specified; use the last UI in the array that has an
            // available background bitmap
            for (Fiction_viewer_ui = NUM_FVW_SETTINGS - 1;
                 Fiction_viewer_ui >= 0; Fiction_viewer_ui--) {
                Fiction_viewer_bitmap =
                    bm_load (Fiction_viewer_screen_filename[Fiction_viewer_ui]
                                                           [gr_screen.res]);
                if (Fiction_viewer_bitmap >= 0) break;
            }
        }
        else {
    ...
    // window
    Fiction_viewer_window.create (
        0, 0, gr_screen.max_w_unscaled, gr_screen.max_h_unscaled, 0,
        Fiction_viewer_fontnum);
    Fiction_viewer_window.set_mask_bmap (
        Fiction_viewer_screen_mask[Fiction_viewer_ui][gr_screen.res]);

    // add the buttons
    for (int i = 0; i < NUM_FVW_BUTTONS; i++) {
        int repeat =
            (i == FVW_BUTTON_SCROLL_UP || i == FVW_BUTTON_SCROLL_DOWN);
        ui_button_info* b =
            &Fiction_viewer_buttons[Fiction_viewer_ui][gr_screen.res][i];

        b->button.create (
            &Fiction_viewer_window, "", b->x, b->y, b->xt, b->yt, repeat, 1);
    ...
    Fiction_viewer_buttons[Fiction_viewer_ui][gr_screen.res][FVW_BUTTON_ACCEPT]
        .button.set_hotkey (KEY_CTRLED | KEY_ENTER);
    Fiction_viewer_buttons[Fiction_viewer_ui][gr_screen.res]
                          [FVW_BUTTON_SCROLL_UP]
                              .button.set_hotkey (KEY_UP);
    Fiction_viewer_buttons[Fiction_viewer_ui][gr_screen.res]
                          [FVW_BUTTON_SCROLL_DOWN]
                              .button.set_hotkey (KEY_DOWN);

    // init brief text
    brief_color_text_init (
        Fiction_viewer_text,
        Fiction_viewer_text_coordinates[Fiction_viewer_ui][gr_screen.res][2],
        default_fiction_viewer_color, 0, 0);

    // if the story is going to overflow the screen, add a slider
    if (Num_brief_text_lines[0] > Fiction_viewer_text_max_lines) {
        Fiction_viewer_slider.create (
            &Fiction_viewer_window,
            Fiction_viewer_slider_coordinates[Fiction_viewer_ui][gr_screen.res]
                                             [0],
            Fiction_viewer_slider_coordinates[Fiction_viewer_ui][gr_screen.res]
                                             [1],
            Fiction_viewer_slider_coordinates[Fiction_viewer_ui][gr_screen.res]
            ...
            &fiction_viewer_scroll_up, &fiction_viewer_scroll_down,
...
void fiction_viewer_close () {
    ...
    // free the bitmap
    if (Fiction_viewer_bitmap >= 0) bm_release (Fiction_viewer_bitmap);
    Fiction_viewer_bitmap = -1;
...
void fiction_viewer_do_frame (float frametime) {
    ...
    for (i = 0; i < NUM_FVW_BUTTONS; i++)
        if (Fiction_viewer_buttons[Fiction_viewer_ui][gr_screen.res][i]
                .button.pressed ())
    ...
    brief_render_text (
        Top_fiction_viewer_text_line,
        Fiction_viewer_text_coordinates[Fiction_viewer_ui][gr_screen.res][0],
        Fiction_viewer_text_coordinates[Fiction_viewer_ui][gr_screen.res][1],
        Fiction_viewer_text_coordinates[Fiction_viewer_ui][gr_screen.res][3],
        frametime);

    // maybe output the "more" indicator
    if ((Fiction_viewer_text_max_lines + Top_fiction_viewer_text_line) <
        Num_brief_text_lines[0]) {
        use_std_font ();

        // can be scrolled down
        int more_txt_x = Fiction_viewer_text_coordinates[Fiction_viewer_ui]
                                                        [gr_screen.res][0] +
                         (Fiction_viewer_text_coordinates[Fiction_viewer_ui]
                                                         [gr_screen.res][2] /
                          2) -
                         10;
        int more_txt_y =
            Fiction_viewer_text_coordinates[Fiction_viewer_ui][gr_screen.res]
                                           [1] +
            Fiction_viewer_text_coordinates[Fiction_viewer_ui][gr_screen.res]
                                           [3]; // located below text, centered

        gr_get_string_size (
            &w, &h, XSTR ("more", 1469), (int)strlen (XSTR ("more", 1469)));
...
void fiction_viewer_load (int stage) {
    ...
    Fiction_viewer_fontnum =
        font::FontManager::getFontIndex (stagep->font_filename);

    Fiction_viewer_voice =
        audiostream_open (stagep->voice_filename, ASF_VOICE);
    ...
    if (fp == NULL) {
        ...
        if (Unicode_text_mode) {
            // Copy the pointer since we assume that we don't need to adjust
            // the text anymore
            Fiction_viewer_text = Fiction_viewer_text_raw;
            Fiction_viewer_text_raw =
                nullptr; // Zero out the pointer so there are no accidental
                         // accesses anymore
        }
        else {
            // allocate space for converted text, then perform the character
            // conversion
            auto length =
