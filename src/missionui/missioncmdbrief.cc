...
void cmd_brief_init_voice()
{
        ...
        for (i = 0; i < Cur_cmd_brief->num_stages; i++) {
                ...
                if (strcasecmp(Cur_cmd_brief->stage[i].wave_filename, NOX("none")) != 0 && Cur_cmd_brief->stage[i].wave_filename[0]) {
...
void cmd_brief_ani_wave_init(int index)
{
        ...
        if (!name[0] || !strcasecmp(name, NOX("<default>")) || !strcasecmp(name, NOX("none.ani"))) {
                name = NOX("CB_default");
...
void cmd_brief_do_frame(float frametime)
{
        ...
        gr_string(
                Cmd_text_wnd_coords[Uses_scroll_buttons][gr_screen.res][CMD_X_COORD] + Cmd_text_wnd_coords[Uses_scroll_buttons][gr_screen.res][CMD_W_COORD] - w,
        ...
        Max_cmdbrief_Lines = Cmd_text_wnd_coords[Uses_scroll_buttons][gr_screen.res][CMD_H_COORD] / (gr_get_font_height() + 1); // Make the max number of lines dependent on the font height,
