...
// chatbox coords
int Chatbox_small_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                      192, 0 },
                                                    {
                                                        // GR_1024
                                                        308, 0 // ?
...
// chatbox coords
int Chatbox_big_coords[GR_NUM_RESOLUTIONS][2] = { { // GR_640
                                                    192, 0 },
                                                  { // GR_1024
                                                    307, 0 } };
...
// input box coords
int Chatbox_big_input_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                          204, 352, 371, 22 },
                                                        { // GR_1024
                                                          328, 565, 591,
                                                          34 } };
...
// input box coords
int Chatbox_p_input_coords[GR_NUM_RESOLUTIONS][4] = { { // GR_640
                                                        106, 328, 379, 17 },
                                                      { // GR_1024
                                                        165, 525, 607, 27 } };
...
#define CHATBOX_INVALID_CHARS \
    NOX ("~`") // this is primarily so that we don't interfere with the voice
               // recording keys
...
#define MAX_BRIEF_CHAT_LINES \
    60 // how many lines we can store in the scrollback buffer
...
// set the chatbox mode without checking for any previous modes which may need
...
void chatbox_set_mode (int mode_flags) {
    ...
    if (Chatbox_mode_flags & CHATBOX_FLAG_SMALL) { size = 0; }
    // big pregame chatbox
    ...
    case 0:
        strcpy_s (
            Chatbox_mask, Chatbox_small_bitmap_mask_fname[gr_screen.res]);
        ...
        Chatbox_icon_x =
            Chatbox_small_display_coords[gr_screen.res][CHATBOX_X_COORD] -
            CHATBOX_TEAM_ICON_SPACE;
        Chatbox_w =
            Chatbox_small_display_coords[gr_screen.res][CHATBOX_W_COORD];
        Chatbox_h =
            Chatbox_small_display_coords[gr_screen.res][CHATBOX_H_COORD];
        Chatbox_begin_x =
            Chatbox_small_display_coords[gr_screen.res][CHATBOX_X_COORD];
        Chatbox_begin_y =
            Chatbox_small_display_coords[gr_screen.res][CHATBOX_Y_COORD];
        Chatbox_disp_w =
            Chatbox_small_display_coords[gr_screen.res][CHATBOX_W_COORD];
        Chatbox_max_lines = Chatbox_small_max_lines[gr_screen.res];
        Chatbox_inputbox_x =
            Chatbox_small_input_coords[gr_screen.res][CHATBOX_X_COORD];
        Chatbox_inputbox_w =
            Chatbox_small_input_coords[gr_screen.res][CHATBOX_W_COORD];
        Chatbox_textenter_y =
            Chatbox_small_input_coords[gr_screen.res][CHATBOX_Y_COORD];
        break;
    ...
    case 1:
        ...
        Chatbox_icon_x =
            Chatbox_big_display_coords[gr_screen.res][CHATBOX_X_COORD] -
            CHATBOX_TEAM_ICON_SPACE;
        ...
        Chatbox_begin_x =
            Chatbox_big_display_coords[gr_screen.res][CHATBOX_X_COORD];
        Chatbox_begin_y =
            Chatbox_big_display_coords[gr_screen.res][CHATBOX_Y_COORD];
        Chatbox_disp_w =
            Chatbox_big_display_coords[gr_screen.res][CHATBOX_W_COORD];
        Chatbox_max_lines = Chatbox_big_max_lines[gr_screen.res];
        Chatbox_inputbox_x =
            Chatbox_big_input_coords[gr_screen.res][CHATBOX_X_COORD];
        Chatbox_inputbox_w =
            Chatbox_big_input_coords[gr_screen.res][CHATBOX_W_COORD];
        Chatbox_textenter_y =
            Chatbox_big_input_coords[gr_screen.res][CHATBOX_Y_COORD];
        break;
    ...
    case 2:
        ...
        Chatbox_icon_x =
            Chatbox_p_display_coords[gr_screen.res][CHATBOX_X_COORD] -
            CHATBOX_TEAM_ICON_SPACE;
        ...
        Chatbox_begin_x =
            Chatbox_p_display_coords[gr_screen.res][CHATBOX_X_COORD];
        Chatbox_begin_y =
            Chatbox_p_display_coords[gr_screen.res][CHATBOX_Y_COORD];
        Chatbox_disp_w =
            Chatbox_p_display_coords[gr_screen.res][CHATBOX_W_COORD];
        Chatbox_max_lines = Chatbox_p_max_lines[gr_screen.res];
        Chatbox_inputbox_x =
            Chatbox_p_input_coords[gr_screen.res][CHATBOX_X_COORD];
        Chatbox_inputbox_w =
            Chatbox_p_input_coords[gr_screen.res][CHATBOX_W_COORD];
        Chatbox_textenter_y =
            Chatbox_p_input_coords[gr_screen.res][CHATBOX_Y_COORD];
        break;
    }
}
...
size_t chatbox_get_msg_target_length (char* msg) {
    if ((msg[0] != '/') && (strchr (msg, ':') != NULL)) { return 0; }
...
void chatbox_autosplit_line () {
    ...
    // if ( msg_pixel_width >= (Chatbox_inputbox_w -
    // Player->short_callsign_width) ) {
    if (msg_pixel_width >= (Chatbox_inputbox_w - 25)) {
        ...
        if (target != MULTI_MSG_ALL) {
            // we need to add the target the message is going to before we add
        ...
        (Chat_inputbox.pressed () && (msg[0] != '\0')) ||
        (strlen (msg) >= CHATBOX_MAX_LEN)) {
        // tack on the null terminator in the boundary case
        size_t x = strlen (msg);
        if (x >= CHATBOX_MAX_LEN) { msg[CHATBOX_MAX_LEN - 1] = '\0'; }
...
int chatbox_create (int mode_flags) {
    ...
    if (Chatbox_mode_flags & CHATBOX_FLAG_DRAW_BOX) {
        ...
        Chatbox_small_bitmap =
            bm_load (Chatbox_small_bitmap_fname[gr_screen.res]);
        ...
        if ((Chatbox_bitmap == -1) || (Chatbox_small_bitmap == -1) ||
            (Chatbox_big_bitmap == -1) || (Chatbox_mp_bitmap == -1)) {
            return -1;
        }
    }
    ...
    Chat_inputbox.create (
        &Chat_window, Chatbox_inputbox_x, Chatbox_textenter_y,
        Chatbox_inputbox_w, CHATBOX_MAX_LEN, "",
    ...
    if ((Chatbox_mode_flags & CHATBOX_FLAG_BUTTONS) &&
        (Chatbox_mode_flags & CHATBOX_FLAG_DRAW_BOX)) {
        for (idx = 0; idx < CHATBOX_NUM_BUTTONS; idx++) {
            // create the button
            Chatbox_buttons[gr_screen.res][idx].button.create (
                &Chat_window, "", Chatbox_buttons[gr_screen.res][idx].x,
                Chatbox_buttons[gr_screen.res][idx].y, 60, 30,
                (idx == CHATBOX_TOGGLE_SIZE) ? 0 : 1);

            // set the highlight action
            Chatbox_buttons[gr_screen.res][idx].button.set_highlight_action (
                common_play_highlight_sound);

            // set the bitmap
            Chatbox_buttons[gr_screen.res][idx].button.set_bmaps (
                Chatbox_buttons[gr_screen.res][idx].filename);

            // set the hotspot
            Chatbox_buttons[gr_screen.res][idx].button.link_hotspot (
                Chatbox_buttons[gr_screen.res][idx].hotspot);
        }

        // now create the toggle size button with the appropriate button
        if (Chatbox_mode_flags & CHATBOX_FLAG_SMALL) {
            Chatbox_buttons[gr_screen.res][CHATBOX_TOGGLE_SIZE]
                .button.set_bmaps (
                    Chatbox_buttons[gr_screen.res][CHATBOX_TOGGLE_SIZE]
                        .filename);
        }
            ...
            Chatbox_buttons[gr_screen.res][CHATBOX_TOGGLE_SIZE]
                .button.set_bmaps (
                    Chatbox_buttons[gr_screen.res][CHATBOX_TOGGLE_SIZE + 1]
                        .filename);
        }
    }
...
int chatbox_process (int key_in) {
    ...
    // if the chatbox hasn't explicitly been created, we can't do any
    // processing
    if (!Chatbox_created) { return key_out; }

    // process the incoming key appropriately
    if (key_in == -1) { key_out = Chat_window.process (); }
    else {
    ...
    if ((Chatbox_mode_flags & CHATBOX_FLAG_BUTTONS) &&
        (Chatbox_mode_flags & CHATBOX_FLAG_DRAW_BOX)) {
        if (Chatbox_buttons[gr_screen.res][CHATBOX_SCROLL_UP]
                .button.pressed ()) {
            chatbox_scroll_up ();
        }

        if (Chatbox_buttons[gr_screen.res][CHATBOX_SCROLL_DOWN]
                .button.pressed ()) {
            chatbox_scroll_down ();
        }

        if (Chatbox_buttons[gr_screen.res][CHATBOX_TOGGLE_SIZE]
                .button.pressed ()) {
            chatbox_toggle_size ();
        }
    }
    ...
    if (Chat_enter_text.pressed ()) { Chat_inputbox.set_focus (); }

    // check to see if the current input text needs to be split up and sent
    // automaticall
...
void chatbox_clear () {
    ...
    if (Chatbox_created) {
        ...
        memset (
            Brief_chat_lines, 0,
            (MAX_BRIEF_CHAT_LINES * (CHATBOX_MAX_LEN + 2)));
        ...
        memset (
            Chatbox_recall_lines, 0,
            CHATBOX_MAX_RECALL_LINES * (CHATBOX_MAX_LEN + 2));
    }
}
...
void chatbox_render () {
    ...
    // GR_RESIZE_MENU); gr_clear(); gr_reset_clip();
...
int chatbox_scroll_up () {
    if (Num_brief_chat_lines > Chatbox_max_lines) {
        ...
        // check to make sure we won't be going "up" above the "beginning" of
        // the text array
        if (Brief_chat_prev_index[Brief_current_add_line] != prev &&
            !(Num_brief_chat_lines < MAX_BRIEF_CHAT_LINES &&
              Brief_start_display_index == 0)) {
...
int chatbox_scroll_down () {
    ...
    if (Num_brief_chat_lines > Chatbox_max_lines) {
        ...
        // check to make sure we won't be going "down" below the "bottom" of
        // the text array
        if (Brief_chat_next_index[Brief_current_add_line] != next) {
            Brief_start_display_index =
                Brief_chat_next_index[Brief_start_display_index];
            return 1;
        }
    }
    return 0;
...
// in other words, if you increment an integer i = A[i], you get the next index
// (or the prev) with wrapping. In this way, as chat lines are entered, they
// are continuously wrapped around the Brief_chat_lines array so we can keep it
// at a fixed size. These arrays are used for both entering new chat strings as
// well as moving the Brief_start_display_index integer, which is
// self-explanatory
...
void chatbox_add_line (const char* msg, int pid, int add_id) {
    ...
    if (add_id) {
        if (MULTI_STANDALONE (Net_players[pid])) {
            ...
            sprintf (
                msg_extra, NOX ("%s: %s"),
                Net_players[pid].m_player->short_callsign, msg);
        }
    }
    else {
    ...
    // copy in the chars
    strncpy (
        &Brief_chat_lines[Brief_current_add_line][1], p_str[0],
        CHATBOX_STRING_LEN - 1);
    if (n_chars[0] >= CHATBOX_STRING_LEN) {
        Brief_chat_lines[Brief_current_add_line][CHATBOX_STRING_LEN - 1] =
            '\0';
    }
    else {
    ...
    // if we have more than 1 line, re-split everything so that the rest are
    // indented
    if (n_lines > 1) {
        ...
        for (idx = 0; idx < n_lines; idx++) {
            ...
            strncpy (
                &Brief_chat_lines[Brief_current_add_line][1], p_str[idx],
                CHATBOX_STRING_LEN - 1);
            if (n_chars[idx] >= CHATBOX_STRING_LEN) {
                Brief_chat_lines[Brief_current_add_line]
                                [CHATBOX_STRING_LEN - 1] = '\0';
            }
            else {
                Brief_chat_lines[Brief_current_add_line][n_chars[idx] + 1] =
                    '\0';
            }

            // increment the total line count if we haven't reached the max
            // already
            if (Num_brief_chat_lines < MAX_BRIEF_CHAT_LINES) {
                Num_brief_chat_lines++;
            }
            ...
            Brief_current_add_line =
                Brief_chat_next_index[Brief_current_add_line];
        }
    }

    // COMMAND LINE OPTION
    if (Cmdline_multi_stream_chat_to_file && Multi_chat_stream != NULL &&
        msg[0] != '\0') { // stream to the file if we're supposed to
        cfwrite_string (msg, Multi_chat_stream);
        cfwrite_char ('\n', Multi_chat_stream);
    }

    // if this line of text is from the player himself, automatically go to the
    ...
    if (pid == MY_NET_PLAYER_NUM) {
        if (Num_brief_chat_lines > Chatbox_max_lines) {
            ...
            for (backup = 1; backup <= Chatbox_max_lines; backup++) {
                Brief_start_display_index =
                    Brief_chat_prev_index[Brief_start_display_index];
            }
        }
    }
    // if we have enough lines of text to require scrolling, scroll down by
    // one.
    else {
        chatbox_scroll_down ();
    }
...
void chatbox_render_chat_lines () {
    ...
    while ((count < Chatbox_max_lines) && (count < Num_brief_chat_lines)) {
        // determine what player this chat line came from, and set the
        ...
        gr_string (
            Chatbox_begin_x + Brief_chat_indents[started_at], ly,
            &Brief_chat_lines[started_at][1], GR_RESIZE_MENU);
        ...
        if (Netgame.type_flags & NG_TYPE_TEAM) {
            ...
            case 0:
                ...
                if (Net_players[player_num].flags &
...
void chatbox_toggle_size_adjust_lines () {
    ...
    // if the chatbox is now small, move the starting display index down as far
    // as we need
...
// force the chatbox to go into small mode (if its in large mode) - will not
// wotk if in multi paused chatbox mode
void chatbox_force_small () {
    int new_mode_flags;

    // don't do anything unless we're currently in "big" mode
    if (!(Chatbox_mode_flags & CHATBOX_FLAG_BIG)) { return; }
    ...
    Chatbox_buttons[gr_screen.res][CHATBOX_TOGGLE_SIZE].button.set_bmaps (
        Chatbox_buttons[gr_screen.res][CHATBOX_TOGGLE_SIZE].filename);

    // call this to set everything up correctly
    chatbox_set_mode (new_mode_flags);

    // change the location of the input box
    Chat_inputbox.update_dimensions (
        Chatbox_inputbox_x, Chatbox_textenter_y, Chatbox_inputbox_w, 15);
    Chat_inputbox.set_focus ();

    // adjust what line we start displaying from based upon the new size of the
    // window
    chatbox_toggle_size_adjust_lines ();
}

// force the chatbox to go into big mode (if its in small mode) - will not work
// if in multi paused chatbox mode
void chatbox_force_big () {
    int new_mode_flags;

    // don't do anything unless we're currently in "small" mode
    if (!(Chatbox_mode_flags & CHATBOX_FLAG_SMALL)) { return; }
    ...
    // flip the up/down arrow
    Chatbox_buttons[gr_screen.res][CHATBOX_TOGGLE_SIZE].button.set_bmaps (
        Chatbox_buttons[gr_screen.res][CHATBOX_TOGGLE_SIZE + 1].filename);
...
int chatbox_has_focus () { return Chat_inputbox.has_focus (); }

// grab the focus for the chatbox inputbox
void chatbox_set_focus () { Chat_inputbox.set_focus (); }

// return if the inputbox was pressed - "clicked on"
int chatbox_pressed () { return Chat_inputbox.pressed (); }
...
void chatbox_recall_up () {
    ...
    if (Chatbox_recall_count <= 0) { return; }
    ...
    if (Chatbox_recall_index < (Chatbox_recall_count - 1)) {
        ...
        if (Chatbox_recall_last == Chatbox_recall_index) {
            Chat_inputbox.set_text (
                Chatbox_recall_lines[++Chatbox_recall_index]);
