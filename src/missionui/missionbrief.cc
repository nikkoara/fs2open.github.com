...
void brief_render_closeup(int ship_class, float frametime)
{
        ...
        if (Closeup_icon->type == ICON_JUMP_NODE) {
                ...
                render_info.set_flags(
                        MR_NO_LIGHTING | MR_AUTOCENTER | MR_NO_POLYS | MR_SHOW_OUTLINE_HTL | MR_NO_TEXTURING);
        } else {
...
void brief_render(float frametime)
{
        ...
        if (Briefing->num_stages > 0) {
                ...
                Max_brief_Lines = Brief_text_coords[gr_screen.res][3] / gr_get_font_height(); // Make the max number of lines dependent on
                                                                                              // the font height.
...
void brief_maybe_blit_scene_cut(float frametime)
{
        if (Start_fade_up_anim) {
                ...
                framenum = int(
                        (Fade_anim.time_elapsed * Fade_anim.num_frames) / Fade_anim.total_time);
                if (framenum < 0)
                        framenum = 0;
