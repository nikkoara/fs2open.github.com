...
static const char* Brief_multi_filename[GR_NUM_RESOLUTIONS] = {
    "BriefMulti", "2_BriefMulti"
};

static const char* Brief_mask_filename[GR_NUM_RESOLUTIONS] = { "Brief-m",
                                                               "2_Brief-m" };

static const char* Brief_multi_mask_filename[GR_NUM_RESOLUTIONS] = {
    "BriefMulti-m", "2_BriefMulti-m"
};
...
struct brief_buttons {
    ...
    brief_buttons (
        const char* name, int x1, int y1, int xt1, int yt1, int h, int r = 0)
...
UI_XSTR Brief_select_text[GR_NUM_RESOLUTIONS][BRIEF_SELECT_NUM_TEXT] = {
    { // GR_640
      ...
      { "Skip Training", 1442, 467, 7, UI_XSTR_COLOR_GREEN, -1,
        &Brief_buttons[0][BRIEF_BUTTON_SKIP_TRAINING].button },
      { "Exit Loop", 1477, 490, 7, UI_XSTR_COLOR_GREEN, -1,
    ...
    { // GR_1024
      ...
      { "Skip Training", 1442, 805, 12, UI_XSTR_COLOR_GREEN, -1,
        &Brief_buttons[1][BRIEF_BUTTON_SKIP_TRAINING].button },
      { "Exit Loop", 1477, 830, 12, UI_XSTR_COLOR_GREEN, -1,
        &Brief_buttons[1][BRIEF_BUTTON_EXIT_LOOP].button } }
};

// coordinates for briefing title -- the x value is for the RIGHT side of the
// text
...
// coordinates for briefing title in multiplayer briefings -- the x value is
// for the LEFT side of the text third coord is max width of area for it to fit
// into (it is force fit there)
...
//
// called when the skip training button on the briefing screen is hit.  When
// this happens, do a popup, then move to the next mission in the campaign.
void brief_skip_training_pressed () {
    int val;

    val = popup (
        PF_USE_NEGATIVE_ICON | PF_USE_AFFIRMATIVE_ICON, 2, POPUP_NO, POPUP_YES,
        XSTR (
            "Skip Training\n\n\n\nAre you sure you want to skip this training "
            "mission?",
            429));
...
void brief_do_next_pressed (int play_sound) {
    ...
    if ((now - Brief_last_auto_advance) < 500) { return; }

    Current_brief_stage++;
    if (Current_brief_stage >= Num_brief_stages) {
        Current_brief_stage = Num_brief_stages - 1;
        gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
        if (Quick_transition_stage != -1) brief_transition_reset ();
    }
    else {
        if (play_sound) {
...
// -------------------------------------------------------------------------------------
// brief_select_button_do() do the button action for the specified pressed
// button
//
void brief_button_do (int i) {
    switch (i) {
    case BRIEF_BUTTON_LAST_STAGE: brief_do_end_pressed (); break;

    case BRIEF_BUTTON_NEXT_STAGE: brief_do_next_pressed (1); break;

    case BRIEF_BUTTON_PREV_STAGE: brief_do_prev_pressed (); break;

    case BRIEF_BUTTON_FIRST_STAGE: brief_do_start_pressed (); break;

    case BRIEF_BUTTON_SCROLL_UP: brief_scroll_up_text (); break;

    case BRIEF_BUTTON_SCROLL_DOWN: brief_scroll_down_text (); break;
...
void brief_check_buttons () {
    ...
    if (Closeup_close_button.pressed ()) { brief_turn_off_closeup_icon (); }
}
...
void brief_redraw_pressed_buttons () {
    ...
    if (!Player->auto_advance) {
        Brief_buttons[gr_screen.res][BRIEF_BUTTON_PAUSE].button.draw_forced (
            2);
    }
}
...
void brief_buttons_init () {
    ...
    for (i = 0; i < NUM_BRIEF_BUTTONS; i++) {
        b = &Brief_buttons[gr_screen.res][i].button;
        b->create (
            &Brief_ui_window, "", Brief_buttons[gr_screen.res][i].x,
            Brief_buttons[gr_screen.res][i].y, 60, 30, 0, 1);
        // set up callback for when a mouse first goes over a button
        b->set_highlight_action (common_play_highlight_sound);

        if ((i == BRIEF_BUTTON_SKIP_TRAINING) ||
            (i == BRIEF_BUTTON_EXIT_LOOP)) {
    ...
    // Hide the 'skip training' button by default.  Only enable and unhide if
    // we are playing a training mission
    ...
    if ((Game_mode & GM_NORMAL) &&
        ...
        Brief_buttons[gr_screen.res][BRIEF_BUTTON_SKIP_TRAINING]
            .button.enable ();
        Brief_buttons[gr_screen.res][BRIEF_BUTTON_SKIP_TRAINING]
            .button.unhide ();
    }

    // Hide the 'exit loop' button by default.  Only enable and unhide if we
    // are playing a loop mission
    ...
    if ((Game_mode & GM_NORMAL) && (Campaign.loop_enabled) &&
    ...
    Closeup_close_button.create (
        &Brief_ui_window, "",
        Closeup_button_coords[gr_screen.res][BRIEF_X_COORD],
    ...
    // key is pressed
    Brief_buttons[gr_screen.res][BRIEF_BUTTON_LAST_STAGE].button.set_hotkey (
        KEY_SHIFTED | KEY_RIGHT);
    Brief_buttons[gr_screen.res][BRIEF_BUTTON_NEXT_STAGE].button.set_hotkey (
        KEY_RIGHT);
    Brief_buttons[gr_screen.res][BRIEF_BUTTON_PREV_STAGE].button.set_hotkey (
        KEY_LEFT);
    Brief_buttons[gr_screen.res][BRIEF_BUTTON_FIRST_STAGE].button.set_hotkey (
        KEY_SHIFTED | KEY_LEFT);
    Brief_buttons[gr_screen.res][BRIEF_BUTTON_SCROLL_UP].button.set_hotkey (
        KEY_UP);
    Brief_buttons[gr_screen.res][BRIEF_BUTTON_SCROLL_DOWN].button.set_hotkey (
        KEY_DOWN);
    ...
    if (Num_brief_stages <= 0) {
        Brief_buttons[gr_screen.res][BRIEF_BUTTON_LAST_STAGE]
            .button.disable ();
        Brief_buttons[gr_screen.res][BRIEF_BUTTON_LAST_STAGE].button.hide ();
        Brief_buttons[gr_screen.res][BRIEF_BUTTON_NEXT_STAGE]
            .button.disable ();
        Brief_buttons[gr_screen.res][BRIEF_BUTTON_NEXT_STAGE].button.hide ();
        Brief_buttons[gr_screen.res][BRIEF_BUTTON_PREV_STAGE]
            .button.disable ();
        Brief_buttons[gr_screen.res][BRIEF_BUTTON_PREV_STAGE].button.hide ();
        Brief_buttons[gr_screen.res][BRIEF_BUTTON_FIRST_STAGE]
            .button.disable ();
        ...
        Brief_buttons[gr_screen.res][BRIEF_BUTTON_SCROLL_DOWN]
            .button.disable ();
...
brief_icon* brief_get_closeup_icon () { return Closeup_icon; }
...
//
void brief_ui_init () {
    Brief_background_bitmap = mission_ui_background_load (
        Briefing->background[gr_screen.res], Brief_filename[gr_screen.res],
        Brief_multi_filename[gr_screen.res]);

    if (Num_brief_stages <= 0) { return; }
...
void brief_set_default_closeup () {
    ...
    for (i = 0; i < bs->num_icons; i++) {
        if (bs->icons[i].flags & BI_HIGHLIGHT) break;
    }

    if (i == bs->num_icons) { brief_setup_closeup (&bs->icons[0]); }
    else {
...
void brief_compact_stages () {
    ...
    // completely clear out the old entries (if any) so we don't access them by
    // mistake - taylor
...
void brief_init () {
    ...
    brief_compact_stages (); // compact the briefing array to eliminate unused
                             // stages
    ...
    if (Brief_inited == TRUE) {
        common_buttons_maybe_reload (
            &Brief_ui_window); // AL 11-21-97: this is necessary since we may
    ...
    if (The_mission.flags[Mission::Mission_Flags::Always_show_goals] ||
        !(The_mission.game_type & MISSION_TYPE_TRAINING))
    ...
    hud_anim_init (
        &Fade_anim, Brief_static_coords[gr_screen.res][0],
    ...
    snazzy_menu_add_region (
        &Briefing_select_region[Num_briefing_regions++], "",
        COMMON_BRIEFING_REGION, 0);
    snazzy_menu_add_region (
        &Briefing_select_region[Num_briefing_regions++], "", COMMON_SS_REGION,
        0);
    snazzy_menu_add_region (
        &Briefing_select_region[Num_briefing_regions++], "",
    ...
    snazzy_menu_add_region (
        &Briefing_select_region[Num_briefing_regions++], "",
        BRIEF_LAST_STAGE_MASK, 0);
    snazzy_menu_add_region (
        &Briefing_select_region[Num_briefing_regions++], "",
        BRIEF_NEXT_STAGE_MASK, 0);
    snazzy_menu_add_region (
        &Briefing_select_region[Num_briefing_regions++], "",
        BRIEF_PREV_STAGE_MASK, 0);
    snazzy_menu_add_region (
        &Briefing_select_region[Num_briefing_regions++], "",
        BRIEF_FIRST_STAGE_MASK, 0);
    snazzy_menu_add_region (
        &Briefing_select_region[Num_briefing_regions++], "",
        BRIEF_TEXT_SCROLL_UP_MASK, 0);
    snazzy_menu_add_region (
        &Briefing_select_region[Num_briefing_regions++], "",
        BRIEF_TEXT_SCROLL_DOWN_MASK, 0);

    // init common UI
    Brief_ui_window.create (
        0, 0, gr_screen.max_w_unscaled, gr_screen.max_h_unscaled, 0);

    if (Game_mode & GM_MULTIPLAYER) {
        Brief_ui_window.set_mask_bmap (
            Brief_multi_mask_filename[gr_screen.res]);
    }
    else {
    ...
    Brief_ui_mask_data =
        Brief_ui_window.get_mask_data (&Brief_ui_mask_w, &Brief_ui_mask_h);
    ...
    ML_objectives_init (
        Brief_goals_coords[gr_screen.res][BRIEF_X_COORD],
    ...
    if (Briefing->num_stages > 0) {
        brief_set_new_stage (
            &Briefing->stages[0].camera_pos,
            &Briefing->stages[0].camera_orient, 0, Current_brief_stage);
...
void brief_render_closeup (int ship_class, float frametime) {
    ...
    if (Closeup_bitmap < 0) return;

    ang = PI2 * frametime / Closeup_one_revolution_time;
    if (ang > MAX_ANG_CHG) ang = MAX_ANG_CHG;

    Closeup_angles.h += ang;
    if (Closeup_angles.h > PI2) Closeup_angles.h -= PI2;
    ...
    gr_set_clip (
        Closeup_region[gr_screen.res][0], Closeup_region[gr_screen.res][1], w,
        h, GR_RESIZE_MENU);
    ...
    // the following is copied from menuui/techmenu.cpp ... it works heehee :D
    // - delt. lighting for techroom
    light_reset ();
    ...
    if (Closeup_icon->type == ICON_JUMP_NODE) {
        ...
        render_info.set_flags (
            MR_NO_LIGHTING | MR_AUTOCENTER | MR_NO_POLYS |
    ...
    model_render_immediate (
        &render_info, Closeup_icon->modelnum, &Closeup_orient, &Closeup_pos);
...
void brief_render (float frametime) {
    ...
    if (Num_brief_stages <= 0) {
        ...
        gr_printf_menu (
            (gr_screen.clip_width_unscaled - w) / 2, 200,
            XSTR ("No Briefing exists for mission: %s", 430),
        ...
        sprintf (
            debugText, NOX ("[filename: %s, last mod: %s]"), Mission_filename,
            The_mission.modified);

        gr_get_string_size (&w, NULL, debugText.c_str ());
        gr_printf_menu (
            (gr_screen.clip_width_unscaled - w) / 2, 230, "%s",
            debugText.c_str ());
#endif
    ...
    gr_bitmap (
        Brief_bmap_coords[gr_screen.res][0],
        Brief_bmap_coords[gr_screen.res][1], GR_RESIZE_MENU);
    ...
    gr_bitmap (
        Brief_infobox_coords[gr_screen.res][0],
        Brief_infobox_coords[gr_screen.res][1], GR_RESIZE_MENU);
    brief_blit_stage_num (Current_brief_stage, Num_brief_stages);

    // only try to render text and play audio if there is really something here
    // - taylor
    if (Briefing->num_stages > 0) {
        z = brief_render_text (
            Top_brief_text_line, Brief_text_coords[gr_screen.res][0],
            Brief_text_coords[gr_screen.res][1],
            Brief_text_coords[gr_screen.res][3], frametime);
        if (z) { brief_voice_play (Current_brief_stage); }

        Max_brief_Lines =
            Brief_text_coords[gr_screen.res][3] /
            gr_get_font_height (); // Make the max number of lines dependent on
                                   // the font height.

        // maybe output the "more" indicator
        if ((Max_brief_Lines + Top_brief_text_line) <
            Num_brief_text_lines[0]) {
            // can be scrolled down
            int more_txt_x = Brief_text_coords[gr_screen.res][0] +
                             (Brief_max_line_width[gr_screen.res] / 2) - 10;
            int more_txt_y = Brief_text_coords[gr_screen.res][1] +
                             Brief_text_coords[gr_screen.res][3] -
                             2; // located below brief text, centered

            gr_get_string_size (
                &w, &h, XSTR ("more", 1469),
                (int)strlen (XSTR ("more", 1469)));
    ...
    gr_printf_menu (
        Brief_bmap_coords[gr_screen.res][0],
        Brief_bmap_coords[gr_screen.res][1] - title_y_offset,
    ...
    if (Game_mode & GM_MULTIPLAYER) {
        ...
        font::force_fit_string (
            buf, 255, Title_coords_multi[gr_screen.res][2]);
        gr_string (
            Title_coords_multi[gr_screen.res][0],
            Title_coords_multi[gr_screen.res][1], buf, GR_RESIZE_MENU);
    }
    else {
        gr_get_string_size (&w, NULL, The_mission.name);
        gr_string (
            Title_coords[gr_screen.res][0] - w, Title_coords[gr_screen.res][1],
            The_mission.name, GR_RESIZE_MENU);
    }
...
void brief_set_closeup_pos (brief_icon* /*bi*/) {
    Closeup_y1 = 10;
    Closeup_x1 = (int)std::lround (
        320 - Closeup_coords[gr_screen.res][BRIEF_W_COORD] / 2.0f);
}
...
void brief_get_closeup_ship_modelnum (brief_icon* ci) {
    ...
    for (objp = GET_FIRST (&obj_used_list);
        ...
        if (objp == &obj_used_list || objp->type != OBJ_SHIP) { continue; }
...
int brief_setup_closeup (brief_icon* bi) {
    ...
    if (Closeup_icon->modelnum == -1) {
        if (sip == NULL) {
            ...
            Closeup_icon->modelnum = model_load (
...
void brief_update_closeup_icon (int mode) {
    ...
    if (mode == 0) {
        ...
        if (Closeup_icon != NULL) { brief_turn_off_closeup_icon (); }
        return;
    }

    if (Closeup_icon == NULL) return;
...
void brief_check_for_anim () {
    ...
    for (i = 0; i <= BRIEF_BUTTON_FIRST_STAGE; i++) {
        Brief_buttons[gr_screen.res][i].button.get_dimensions (
            &x, &y, &iw, &ih);

        if (mx >= x && mx <= (x + iw) && my >= y && my <= (y + ih)) { return; }
    }

    // if mouse coords are outside the briefing screen, then go away
    my -= bscreen.map_y1;
    mx -= bscreen.map_x1;
    if (my < 0 || mx < 0 || mx > (bscreen.map_x2 - bscreen.map_x1 + 1) ||
        my > (bscreen.map_y2 - bscreen.map_y1 + 1))
        return;
...
void brief_maybe_flash_button () {
    ...
    if (common_flash_bright ()) {
        if (Current_brief_stage == (Num_brief_stages - 1)) {
            // AL 30-3-98: Don't flash ship selection button if in a training
            // mission,
            if (brief_only_allow_briefing ()) { return; }
        ...
        if (b->button_hilighted ()) { common_flash_button_init (); }
        else {
            b->draw_forced (1);
        }
    }
...
void brief_do_frame (float frametime) {
    ...
    if (red_alert_mission ()) { return; }

    if (!Brief_inited) { brief_init (); }
    ...
    int snazzy_action = -1;
    brief_choice = snazzy_menu_do (
        Brief_ui_mask_data, Brief_ui_mask_w, Brief_ui_mask_h,
        Num_briefing_regions, Briefing_select_region, &snazzy_action, 0);

    k = common_select_do (frametime);

    if (Closeup_icon) { Brief_mouse_up_flag = 0; }
    ...
    if (Briefing->num_stages > 0) {
        ...
        case KEY_CTRLED | KEY_PAGEUP: {
            if (Closeup_icon && Closeup_icon->ship_class) {
                ...
                WARNINGF (LOCATION, "Shiptype = %d (%s)", Closeup_icon->ship_class,sip->name);
                WARNINGF (LOCATION, "Modelnum = %d (%s)", sip->model_num,sip->pof_file);
    ...
    if (brief_choice != -1) { Brief_mouse_up_flag = 0; }
    ...
    if (Current_brief_stage < (Num_brief_stages - 1)) {
        if (!help_overlay_active (Briefing_overlay_id) &&
            brief_time_to_advance (Current_brief_stage)) {
    ...
    if (!Background_playing) {
        ...
        if (Current_brief_stage != Last_brief_stage) {
            ...
            if (time != 0) {
                if (Current_brief_stage > Last_brief_stage) {
                    if (Briefing->stages[Last_brief_stage].flags &
                        BS_FORWARD_CUT) {
                        Quick_transition_stage = Current_brief_stage;
                        Current_brief_stage = Last_brief_stage;
                        ASSERT (Current_brief_stage >= 0);
                        ...
                        time =
                            Briefing->stages[Current_brief_stage].camera_time;
                    }
                }
                else {
                    if (Briefing->stages[Last_brief_stage].flags &
            ...
            // set the camera target
            brief_set_new_stage (
                &Briefing->stages[Current_brief_stage].camera_pos,
                &Briefing->stages[Current_brief_stage].camera_orient, time,
                Current_brief_stage);
    ...
    Transition_done:
        ...
        if (Closeup_icon && (Closeup_bitmap >= 0)) {
            ...
            gr_bitmap (
                Closeup_coords[gr_screen.res][BRIEF_X_COORD],
                Closeup_coords[gr_screen.res][BRIEF_Y_COORD], GR_RESIZE_MENU);
        }
...
void brief_close () {
    ...
    if (Fade_anim.first_frame != -1) { bm_unload (Fade_anim.first_frame); }
...
void briefing_load_music (char* fname) {
    ...
    if (Briefing_music_handle != -1) return;

    if (fname) Briefing_music_handle = audiostream_open (fname, ASF_MENUMUSIC);
}
...
void briefing_start_music () {
    if (Briefing_music_handle != -1) {
        ...
        WARNINGF (LOCATION, "No music file exists to play music at this briefing!");
    }
}

void brief_stop_voices () { brief_voice_stop (Current_brief_stage); }

void brief_pause () {
    if (Briefing_paused) return;
...
Fade_down_anim_start:
    if (Start_fade_down_anim) {
        ...
        // Blit the bitmap for this frame
        gr_set_bitmap (
            Fade_anim.first_frame + (Fade_anim.num_frames - 1) - framenum);
        gr_bitmap (Fade_anim.sx, Fade_anim.sy, GR_RESIZE_MENU);
    }
}
...
// return 1 if this mission only allow players to use the briefing (and not
// ship or weapon loadout).  Otherwise return 0.
int brief_only_allow_briefing () {
    if (The_mission.game_type & MISSION_TYPE_TRAINING) { return 1; }

    if (The_mission.flags[Mission::Mission_Flags::Scramble] ||
        The_mission.flags[Mission::Mission_Flags::Red_alert]) {
        return 1;
    }
