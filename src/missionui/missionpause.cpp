...
// pause bitmap name
const char* Pause_bmp_name[GR_NUM_RESOLUTIONS] = { "PleaseWait",
                                                   "2_PleaseWait" };
...
int pause_get_type () { return Pause_type; }

void pause_set_type (int type) { Pause_type = type; }
...
void pause_do () {
    ...
    if (Pause_type == PAUSE_TYPE_NORMAL) {
        ...
        if (Pause_saved_screen == -1) { Pause_type = PAUSE_TYPE_VIEWER; }
    ...
    if (Pause_type == PAUSE_TYPE_NORMAL) {
        if (Pause_background_bitmap >= 0) {
            ...
            gr_string (
                ...
                (gr_screen.max_h_unscaled - str_h) / 2, pause_str,
                GR_RESIZE_MENU);
    ...
    if (Pause_type == PAUSE_TYPE_VIEWER) {
        if (previous_Viewer_mode < 0) previous_Viewer_mode = Viewer_mode;

        if (previous_hud_state < 0) previous_hud_state = hud_disabled ();
    }

    // process the ui window here
    k = Pause_win.process () & ~KEY_DEBUGGED;
    switch (k) {
    case KEY_TAB: hud_toggle_draw (); break;
    ...
    case KEY_PADMULTIPLY:
        if (Pause_type == PAUSE_TYPE_VIEWER) { button_function (TARGET_NEXT); }
        break;

    case KEY_ESC:
    case KEY_ALTED + KEY_PAUSE:
    case KEY_PAUSE:
        // reset previous view if we happened to be playing around with it
        // during pause
        if (Pause_type == PAUSE_TYPE_VIEWER) {
            ...
            // NOTE remember that hud state is reversed here (0 == on, 1 ==
            // off)
            if ((previous_hud_state >= 0) &&
                (hud_disabled () != previous_hud_state)) {
    ...
    // a very unique case where we shouldn't be doing the page flip because
    // we're inside of popup code
    if (!popup_active ()) {
        if (Pause_type == PAUSE_TYPE_NORMAL) { gr_flip (); }
    }
    else {
...
void pause_debug_init () {
    ...
    Pause_physics.create (
        &Pause_win, NOX ("Physics Pause <P>"), 200, 150, physics_paused);
    Pause_ai.create (&Pause_win, NOX ("AI Pause <A>"), 200, 175, ai_paused);
#ifndef NDEBUG
    Pause_ai_render.create (
        &Pause_win, NOX ("AI Render Stuff <R>"), 200, 200,
        Ai_render_debug_flag);
#endif
    Pause_firing.create (
        &Pause_win, NOX ("AI firing <F>"), 200, 225, Ai_firing_enabled);
    Pause_external_view_mode_check.create (
        &Pause_win, NOX ("External View <E>"), 200, 250,
        Pause_external_view_mode);
    Pause_single_step.create (
        &Pause_win, NOX ("Single Step <S>"), 200, 290, game_single_step);
...
void pause_debug_do () {
    ...
    if (Pause_external_view_mode_check.changed ()) {
        ...
        if (Pause_external_view_mode) {
            HUD_sourced_printf (
                HUD_SOURCE_HIDDEN, "%s",
                XSTR ("External view of player ship.", 182));
        }
        else {
            HUD_sourced_printf (
                HUD_SOURCE_HIDDEN, "%s",
                XSTR ("View from inside player ship.", 183));
        }
    }
