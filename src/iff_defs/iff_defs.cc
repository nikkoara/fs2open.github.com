...
void iff_init()
{
        ...
        try {
                ...
                if (optional_string("$Dimmed IFF brightness:")) {
                        ...
                        ASSERT(
                                dim_iff_brightness >= 0 && dim_iff_brightness <= HUD_COLOR_ALPHA_MAX);
                ...
                for (int cur_iff = 0; cur_iff < Num_iffs; cur_iff++) {
                        ...
                        for (int list_index = 0; list_index < MAX_IFFS; list_index++) {
                                ...
                                if (list_index < num_observed_colors[cur_iff]) {
                                        ...
                                        // valid?
                                        if (target_iff >= 0)
                                                iff->observed_color_index[target_iff] = observed_color_table[cur_iff][list_index]
                                                                                                .color_index;
                                        else
...
int iff_lookup(const char *iff_name)
{
        ...
        for (int i = 0; i < Num_iffs; i++)
                if (!strcasecmp(iff_name, Iff_info[i].iff_name))
                        return i;
