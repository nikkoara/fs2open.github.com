...
// global only to file
color Iff_colors[MAX_IFF_COLORS]
                [2]; // AL 1-2-97: Create two IFF colors, regular and bright

flag_def_list rti_flags[] = { { "crosshairs", RTIF_CROSSHAIRS, 0 },
                              { "blink", RTIF_BLINK, 0 },
...
int iff_init_color (int r, int g, int b) {
    ...
    // init it
    gr_init_alphacolor (
        &Iff_colors[idx][0], r, g, b, iff_get_alpha_value (false));
    gr_init_alphacolor (
        &Iff_colors[idx][1], r, g, b, iff_get_alpha_value (true));
...
void iff_init () {
    ...
    try {
        ...
        if ((optional_string ("$Selection Color:")) ||
            (optional_string ("$Selection Colour:"))) {
        ...
        if ((optional_string ("$Message Color:")) ||
            (optional_string ("$Message Colour:"))) {
            stuff_int_list (rgb, 3, RAW_INTEGER_TYPE);
            iff_init_color (rgb[0], rgb[1], rgb[2]);
        }
        else
        ...
        if ((optional_string ("$Tagged Color:")) ||
            (optional_string ("$Tagged Colour:"))) {
            stuff_int_list (rgb, 3, RAW_INTEGER_TYPE);
            iff_init_color (rgb[0], rgb[1], rgb[2]);
        }
        else
        ...
        for (i = 0; i < 5; i++) {
            for (j = 0; j < 2; j++) {
                for (k = 0; k < 3; k++) { radar_iff_color[i][j][k] = -1; }
            }
        }
        ...
        if (optional_string ("$Use Alternate Blip Coloring:") ||
            optional_string ("$Use Alternate Blip Colouring:")) {
            stuff_boolean (&alternate_blip_color);
        }

        // Parse blip colours, their order is hardcoded.
        if ((optional_string ("$Missile Blip Color:")) ||
        ...
        if ((optional_string ("$Navbuoy Blip Color:")) ||
        ...
        if ((optional_string ("$Warping Blip Color:")) ||
        ...
        if ((optional_string ("$Node Blip Color:")) ||
        ...
        if ((optional_string ("$Tagged Blip Color:")) ||
        ...
        if (optional_string ("$Radar Target ID Flags:")) {
            parse_string_flag_list (
                (int*)&radar_target_id_flags, rti_flags, Num_rti_flags);
            if (optional_string ("+reset")) radar_target_id_flags = 0;
        }
        ...
        while (required_string_either ("#End", "$IFF Name:")) {
            ...
            if (optional_string ("$Attacks:"))
                num_attack_names[cur_iff] =
                    stuff_string_list (attack_names[cur_iff], MAX_IFFS);
            else
            ...
            while (optional_string ("+Sees")) {
                // get iff observed
                stuff_string_until (
                    observed_color_table[cur_iff][num_observed_colors[cur_iff]]
                        .iff_name,
                    "As:", NAME_LENGTH);
                required_string ("As:");
                ...
                stuff_int_list (rgb, 3, RAW_INTEGER_TYPE);
                observed_color_table[cur_iff][num_observed_colors[cur_iff]]
                    .color_index = iff_init_color (rgb[0], rgb[1], rgb[2]);
            ...
            if (optional_string ("$Default Ship Flags:")) {
                parse_string_flag_list (
                    iff->default_parse_flags, Parse_object_flags,
                    num_parse_object_flags, NULL);
            }
            ...
            // this is cleared between each level but let's just set it here
            // for thoroughness
        ...
        for (int cur_iff = 0; cur_iff < Num_iffs; cur_iff++) {
            ...
            for (j = 0; j < MAX_IFFS; j++) iff->observed_color_index[j] = -1;
            ...
            for (int list_index = 0; list_index < MAX_IFFS; list_index++) {
                ...
                if (list_index < num_observed_colors[cur_iff]) {
                    ...
                    // valid?
                    if (target_iff >= 0)
                        iff->observed_color_index[target_iff] =
            ...
            if (iff->flags & IFFF_EXEMPT_FROM_ALL_TEAMS_AT_WAR) {
                ...
                for (int other_iff = 0; other_iff < Num_iffs; other_iff++) {
                    ...
                    if ((other_iff == cur_iff) &&
                        !iff_x_attacks_y (cur_iff, cur_iff))
                        continue;

                    // skip anyone exempt
                    if (Iff_info[other_iff].flags &
                        IFFF_EXEMPT_FROM_ALL_TEAMS_AT_WAR)
                        continue;

                    // add everyone else
                    iff->attackee_bitmask_all_teams_at_war |=
                        iff_get_mask (other_iff);
                }
            }
        }
 ...
 */
int iff_get_mask (int team) { return (1 << team); }
...
color* iff_get_color_by_team (int team, int seen_from_team, int is_bright) {
    ...
    if (color_index < 0) color_index = Iff_info[team].color_index;
...
color* iff_get_color_by_team_and_object (
    ...
    // switch incase some sort of parent iff color inheritance for example for
    // bombs is wanted...
    switch (objp->type) {
    case OBJ_SHIP:
        if (Ships[objp->instance].ship_iff_color[seen_from_team][team] >= 0) {
            alt_color_index =
                Ships[objp->instance].ship_iff_color[seen_from_team][team];
        }
        else {
            alt_color_index = Ship_info[Ships[objp->instance].ship_info_index]
                                  .ship_iff_info[seen_from_team][team];
        }
        break;
    default: break;
    }
    ...
    // temporary solution....
    if (alt_color_index >= 0) color_index = alt_color_index;
    if (color_index < 0) color_index = Iff_info[team].color_index;
