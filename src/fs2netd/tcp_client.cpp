...
int FS2NetD_CheckSingleMission (const char* m_name, uint crc32, bool do_send) {
    ...
    if (do_send) {
        ...
        if (FS2NetD_SendData (buffer, buffer_size) == -1) { return 3; }
    }
        ...
        do {
            rc = FS2NetD_GetData (
                buffer + rc_total, sizeof (buffer) - rc_total);
        ...
        if (rc < BASE_PACKET_SIZE) { return 0; }

        VRFY_PACKET2 (PCKT_MCHECK_REPLY);

        if (!my_packet) { return 0; }
...
int FS2NetD_SendPlayerData (
    const char* player_name, player* pl, bool do_send) {
    int buffer_size, buffer_offset;
    bool my_packet = false;
    char buffer[16384];
    ...
    if (do_send) {
        ...
        if (FS2NetD_SendData (buffer, buffer_size) == -1) { return -1; }
    }
    else if (FS2NetD_DataReady ()) {
        int rc;
        uint rc_total = 0;
        ...
        if (rc < BASE_PACKET_SIZE) { return -1; }

        VRFY_PACKET2 (PCKT_PILOT_UREPLY);

        if (!my_packet) { return -1; }
...
int FS2NetD_GetPlayerData (
    const char* player_name, player* pl, bool can_create, bool do_send) {
    int buffer_size, buffer_offset;
    bool my_packet = false;
    char buffer[16384];
    ...
    if (do_send) {
        ...
        if (buffer_size > (int)sizeof (buffer)) {
            ml_printf (
                "FS2NetD WARNING: Pilot update data is larger than receive "
                "buffer!  Some data will be lost!");
        }

        // make sure that we get the entire packet
        while ((rc_total < (uint)buffer_size) &&
               (rc_total < sizeof (buffer)) &&
               (timer_get_fixed_seconds () <= end_time)) {
        ...
        if (reply_type != 0) { return (int)reply_type; }
        ...
        for (idx = 0; idx < (int)num_type_kills; idx++) {
            ...
            if (si_index == -1) { PXO_GET_USHORT (bogus); }
            else {
...
int FS2NetD_GetBanList (std::vector< std::string >& mask_list, bool do_send) {
    ...
    if (do_send) {
        ...
        if (buffer_size > (int)sizeof (buffer)) {
            ml_printf (
                "FS2NetD WARNING: Banned user list data is larger than "
                "receive buffer!  Some data will be lost!");
        }
...
int FS2NetD_GetMissionsList (
    std::vector< file_record >& m_list, bool do_send) {
    int buffer_size, buffer_offset;
    bool my_packet = false;
    char buffer[16384];
    ...
    if (do_send) {
        ...
        if (buffer_size > (int)sizeof (buffer)) {
            ml_printf (
                "FS2NetD WARNING: Mission list data is larger than receive "
                "buffer!  Some data will be lost!");
        }
...
void FS2NetD_SendServerDisconnect () {
    ...
    if (FS2NetD_SendData (buffer, buffer_size) < 0) {
        ml_printf (
            "FS2NetD WARNING: Failed to send server disconnect packet!");
    }
}
...
void FS2NetD_RequestServerList () {
    int buffer_size;
    char buffer
        [BASE_PACKET_SIZE + sizeof (int) + sizeof (int) + sizeof (int) +
         MAX_PATH];
    int all = 0xFFFFFFFF;
    bool filtered = false;

    if (strlen (Multi_fs_tracker_filter)) { filtered = true; }
    ...
    if (filtered) { PXO_ADD_STRING (Multi_fs_tracker_filter); }
...
void FS2NetD_CheckDuplicateLogin () {
    int buffer_size;
    char buffer
        [BASE_PACKET_SIZE + sizeof (int) + sizeof (ubyte) +
         (MAX_PLAYERS * sizeof (int)) + 10];
    int ids_count = 0;
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            ...
            if ((Net_players[idx].tracker_player_id >= 0) &&
                (Net_players[idx].tracker_player_id != Multi_tracker_id)) {
    ...
    for (idx = 0; idx < ids_count; idx++) { PXO_ADD_INT (ids[idx]); }
