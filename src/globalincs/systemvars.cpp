...
#ifndef NDEBUG
// for debugging, used to print the currently processing filename on the
// loading screen
...
float Noise[NOISE_NUM_FRAMES] = { 0.468225f, 0.168765f, 0.318945f, 0.292866f,
                                  0.553357f, 0.468225f, 0.180456f, 0.418465f,
                                  0.489958f, 1.000000f, 0.468225f, 0.599820f,
                                  0.664718f, 0.294215f, 0.000000f };
...
int rand32 () {
    if (RAND_MAX == 0x7fff) {
        int random32;
        // this gets two random 16 bit numbers and stuffs them into a 32 bit
        // number
        random32 = (rand () << 16) | rand ();
        // since rand() returns between 0 and 0x7fff, there needs to be a thing
        // to randomly generate the 16th bit
        random32 |= ((rand () & 1) << 15);
...
#ifndef NDEBUG
    if (filename != NULL) strcpy (Processing_filename, filename);
#endif

    int t1 = timer_get_milliseconds ();

    if ((t1 > cf_timestamp) ||
        ((cb_counter > cb_last_counter + 155) && (cb_delta_step > 0))) {
...
int current_detail_level () {
    ...
    for (i = 0; i < NUM_DEFAULT_DETAIL_LEVELS; i++) {
        if (memcmp (&Detail, &Detail_defaults[i], sizeof (detail_levels)) ==
            0) {
            return i;
        }
    }
...
DCF (detail, "Turns on/off parts of the game for speed testing") {
    ...
    if (dc_optional_string_either ("help", "--help")) {
        ...
        dc_printf (
            "No argument will toggle between highest/lowest detail "
            "settings\n");
        return;
    }
    ...
    if (dc_optional_string_either ("status", "--status") ||
        ...
        dc_printf (
            "   2: draw the nebulas: %s\n",
            ((Game_detail_flags & 2) ? "on" : "off"));
        dc_printf (
            "   4: draw the motion debris: %s\n",
            ((Game_detail_flags & 4) ? "on" : "off"));
        dc_printf (
            "   8: draw planets: %s\n",
            ((Game_detail_flags & 8) ? "on" : "off"));
        dc_printf (
            "  16: draw models not as blobs: %s\n",
            ((Game_detail_flags & 16) ? "on" : "off"));
        dc_printf (
            "  32: draw lasers not as pixels: %s\n",
            ((Game_detail_flags & 32) ? "on" : "off"));
        dc_printf (
            "  64: clear screen background after each frame: %s\n",
            ((Game_detail_flags & 64) ? "on" : "off"));
        dc_printf (
            " 128: draw hud stuff: %s\n",
            ((Game_detail_flags & 128) ? "on" : "off"));
        dc_printf (
            " 256: draw fireballs: %s\n",
            ((Game_detail_flags & 256) ? "on" : "off"));
        dc_printf (
            " 512: do collision detection: %s\n",
            ((Game_detail_flags & 512) ? "on" : "off"));
        return;
    }

    if (dc_maybe_stuff_int (&value)) { Game_detail_flags ^= value; }
    else {
...
// Goober5000
// (Taylor says that for optimization purposes malloc/free should be used
// rather than vm_malloc/free here) NOTE: Because this uses memcpy, it
// should only be used to sort POD elements!
void insertion_sort (
    void* array_base, size_t array_size, size_t element_size,
    ...
    for (i = 1; (unsigned)i < array_size; i++) {
        ...
        for (j = i - 1;
             (j >= 0) &&
             (fncompare (array_byte_base + (j * element_size), current) > 0);
             j--) {
            memcpy (
                array_byte_base + ((j + 1) * element_size),
                array_byte_base + (j * element_size), element_size);
        }

        // insert the current element at the correct place
        memcpy (
            array_byte_base + ((j + 1) * element_size), current, element_size);
    }
