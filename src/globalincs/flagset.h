...
#ifndef FREESPACE2_GLOBALINCSFREESPACE2_GLOBALINCS_FLAGSET_HH
#define FREESPACE2_GLOBALINCSFREESPACE2_GLOBALINCS_FLAGSET_HH
...
public:
    ...
    flagset (const std::initializer_list< T >& init_list) {
        for (auto& val : init_list) { set (val); }
    }
    ...
    void operator|= (const T flag) { set (flag); }
    ...
    flagset< T >& remove (T idx) { return set (idx, false); }
...
struct flag_combinator {
    ...
    static_assert (
        SIZE <= static_cast< size_t > (TEnum::NUM_VALUES),
        "Size must be less than NUM_VALUES");

    flag_combinator (
        const flag_combinator< TEnum, SIZE - 1 >& left, TEnum right)
        : _otherValues (left) {
        _value = right;
    }
...
template< typename TEnum >
typename std::enable_if< flag_enum_checker< TEnum >::value, flag_combinator< TEnum, 2 > >::type operator, (
    TEnum left, TEnum right) {
    return flag_combinator< TEnum, 2 > (left, right);
};

template< typename TEnum, size_t SIZE >
typename std::enable_if< flag_enum_checker< TEnum >::value, flag_combinator< TEnum, SIZE + 1 > >::type operator, (
    const flag_combinator< TEnum, SIZE >& left, TEnum right) {
    return flag_combinator< TEnum, SIZE + 1 > (left, right);
};

#endif

#endif // FREESPACE2_GLOBALINCSFREESPACE2_GLOBALINCS_FLAGSET_HH
