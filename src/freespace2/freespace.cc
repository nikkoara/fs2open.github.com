...
#ifndef NDEBUG
        Use_fullscreen_at_startup = fs2::registry::read("Default.ForceFullscreen", 1);
#endif
...
void game_show_framerate()
{
        ...
        if (Show_cpu == 1) {
                ...
                if (Timing_total > 0.01f) {
                        ...
                        gr_printf_no_resize(
                                sx, sy, NOX("GAME: %.0f%%"),
                                (Timing_total - (Timing_render2 + Timing_render3 + Timing_flip + Timing_clear)) * 100.0f / Timing_total);
                        sy += line_height;
                }
        }
...
void game_tst_frame_pre()
{
        ...
        if (tst == 3) {
                ...
                // offscreen
                if (!((v.screen.xyw.x >= 0) && (v.screen.xyw.x <= gr_screen.max_w) && (v.screen.xyw.y >= 0) && (v.screen.xyw.y <= gr_screen.max_h))) {
                        return;
                }
...
{
        if (Player_obj && Player_obj->type == OBJ_SHIP && Player_ship && !Player_ship->flags[Ship::Ship_Flags::Dying]) {
                ai_do_repair_frame(
...
float get_shake(float intensity, int decay_time, int max_decay_time)
{
        ...
        if (decay_time >= 0) {
                ...
                shake *= (0.5f - fabsf(0.5f - (float)decay_time / (float)max_decay_time));
        }
...
camid game_render_frame_setup()
{
        ...
        if ((!(last_Viewer_mode & VM_EXTERNAL) && (Viewer_mode & VM_EXTERNAL)) ||                                                                                                                                                                                                                                                                                                                            // internal to external
            ...
            (!(last_Viewer_mode & VM_FREECAMERA) && (Viewer_mode & VM_FREECAMERA)) || ((last_Viewer_mode & VM_FREECAMERA) && !(Viewer_mode & VM_FREECAMERA)) || (!(last_Viewer_mode & VM_TOPDOWN) && (Viewer_mode & VM_TOPDOWN)) || ((last_Viewer_mode & VM_TOPDOWN) && !(Viewer_mode & VM_TOPDOWN)) || (fov_changed) || ((Viewer_mode & VM_OTHER_SHIP) && (last_Viewer_objnum != Player_ai->target_objnum)) // other ship mode, but targets changes
        ) {
                // regenerate the nebula
                neb2_eye_changed();
        }

        if ((last_Viewer_mode != Viewer_mode) || (last_Game_mode != Game_mode) || (fov_changed) || (Viewer_mode & VM_FREECAMERA)) {
                // mprintf(( "************** Camera cut! ************\n" ));
...
void game_render_frame(camid cid)
{
        ...
        // everything up.
        if (Viewer_obj && (Viewer_obj->type == OBJ_SHIP) && (Ship_info[Ships[Viewer_obj->instance].ship_info_index].flags[Ship::Info_Flags::Show_ship_model]) && (!Viewer_mode || (Viewer_mode & VM_PADLOCK_ANY) || (Viewer_mode & VM_OTHER_SHIP) || (Viewer_mode & VM_TRACK) || !(Viewer_mode & VM_EXTERNAL))) {
        ...
        if (Fade_type != FI_NONE) {
                ...
                if (timestamp() >= Fade_start_timestamp) {
                        ...
                        if (timestamp() < Fade_end_timestamp) {
                                ...
                                alpha = int(
                                        (float)startAlpha + (((float)endAlpha - (float)startAlpha) / (float)duration) * (float)elapsed);
                        } else {
                                // Fade finished
                                Fade_type = FI_NONE;
...
void bars_do_frame(float frametime)
{
        if ((Cutscene_bar_flags & CUB_GRADUAL) && Cutscene_bars_progress < 1.0f) {
                ...
                if (Cutscene_bar_flags & CUB_CUTSCENE)
                        yborder = int(
                                Cutscene_bars_progress * (gr_screen.max_h / CUTSCENE_BAR_DIVISOR));
                else
                        yborder = gr_screen.max_h / CUTSCENE_BAR_DIVISOR - int(Cutscene_bars_progress * (gr_screen.max_h / CUTSCENE_BAR_DIVISOR));
...
void game_set_frametime(int state)
{
        ...
        if (Game_time_compression != Desired_time_compression) {
                ...
                if ((ascending && Game_time_compression > Desired_time_compression) || (!ascending && Game_time_compression < Desired_time_compression))
...
int game_poll()
{
        ...
        if (fs2::os::is_foreground() && !io::mouse::CursorManager::get()->isCursorShown() && (Use_joy_mouse)) {
                // Move the mouse cursor with the joystick
                int mx, my, dx, dy;
                int jx, jy;
...
void game_process_event(int current_state, int event)
{
        ...
        case GS_EVENT_END_GAME:
                if ((current_state == GS_STATE_GAME_PLAY) || (current_state == GS_STATE_DEATH_DIED) || (current_state == GS_STATE_DEATH_BLEW_UP) || (current_state == GS_STATE_DEBRIEF)) {
...
void game_leave_state(int old_state, int new_state)
{
        ...
        case GS_STATE_BRIEFING:
                brief_stop_voices();
                if ((new_state != GS_STATE_OPTIONS_MENU) && (new_state != GS_STATE_WEAPON_SELECT) && (new_state != GS_STATE_SHIP_SELECT) && (new_state != GS_STATE_HOTKEY_SCREEN) && (new_state != GS_STATE_TEAM_SELECT)) {
                        common_select_close();
        ...
        case GS_STATE_SHIP_SELECT:
                if (new_state != GS_STATE_OPTIONS_MENU && new_state != GS_STATE_WEAPON_SELECT && new_state != GS_STATE_HOTKEY_SCREEN && new_state != GS_STATE_BRIEFING && new_state != GS_STATE_TEAM_SELECT) {
                        common_select_close();
                        if (new_state == GS_STATE_MAIN_MENU) {
                                freespace_stop_mission();
                        }
        ...
        case GS_STATE_WEAPON_SELECT:
                if (new_state != GS_STATE_OPTIONS_MENU && new_state != GS_STATE_SHIP_SELECT && new_state != GS_STATE_HOTKEY_SCREEN && new_state != GS_STATE_BRIEFING && new_state != GS_STATE_TEAM_SELECT) {
                        common_select_close();
                        if (new_state == GS_STATE_MAIN_MENU) {
                                freespace_stop_mission();
                        }
        ...
        case GS_STATE_TEAM_SELECT:
                if (new_state != GS_STATE_OPTIONS_MENU && new_state != GS_STATE_SHIP_SELECT && new_state != GS_STATE_HOTKEY_SCREEN && new_state != GS_STATE_BRIEFING && new_state != GS_STATE_WEAPON_SELECT) {
                        common_select_close();
                        if (new_state == GS_STATE_MAIN_MENU) {
                                freespace_stop_mission();
                        }
...
void game_enter_state(int old_state, int new_state)
{
        ...
        case GS_STATE_GAME_PLAY:
                ...
                if ((old_state == GS_STATE_BRIEFING) || (old_state == GS_STATE_SHIP_SELECT) || (old_state == GS_STATE_WEAPON_SELECT) || (old_state == GS_STATE_RED_ALERT)) {
        ...
        \sa sexp_hud_display_gauge*/
                if ((old_state == GS_STATE_GAME_PLAY) || (old_state == GS_STATE_BRIEFING) || (old_state == GS_STATE_DEBRIEF) || (old_state == GS_STATE_SHIP_SELECT) || (old_state == GS_STATE_WEAPON_SELECT) || (old_state == GS_STATE_RED_ALERT)) {
                        Sexp_hud_display_warpout = 0;
                }

                // Goober5000 - people may not have realized that pausing causes this
                // state to be re-entered
                if ((old_state != GS_STATE_GAME_PAUSED) && (old_state != GS_STATE_MAIN_MENU)) {
                        radar_mission_init();
                ...
                if (old_state == GS_STATE_BRIEFING || old_state == GS_STATE_SHIP_SELECT || old_state == GS_STATE_WEAPON_SELECT || old_state == GS_STATE_MAIN_MENU || old_state == GS_STATE_SIMULATOR_ROOM) {
...
// XSTR:ON
        ...
        case GS_STATE_DEATH_BLEW_UP:
                ...
                Players[Player_num].flags &= ~PLAYER_FLAGS_AUTO_TARGETING; // Prevent immediate switch to a
                                                                           // hostile ship.
                Game_mode |= GM_DEAD_BLEW_UP;
