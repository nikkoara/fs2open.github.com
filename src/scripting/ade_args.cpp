...
int ade_get_args (lua_State* L, const char* fmt, ...) {
    ...
    if (lua_getstack (L, 0, &ar)) {
        lua_getinfo (L, "nl", &ar);
        strcpy_s (funcname, "");
        if (ar.name != NULL) { strcat_s (funcname, ar.name); }
        if (ar.currentline > -1) {
            char buf[33];
    ...
    if (!strlen (funcname)) {
        ...
        if (!Ade_get_args_lfunction &&
            !lua_isnone (L, lua_upvalueindex (ADE_FUNCNAME_UPVALUE_INDEX))) {
            if (lua_type (L, lua_upvalueindex (ADE_FUNCNAME_UPVALUE_INDEX)) ==
                LUA_TSTRING)
                strcpy_s (
                    funcname,
                    lua_tostring (
                        L, lua_upvalueindex (ADE_FUNCNAME_UPVALUE_INDEX)));
        }

        // WMC - Totally unknown function
        if (!strlen (funcname)) { strcpy_s (funcname, "<UNKNOWN>"); }
    }
    if (total_args < needed_args) {
        LuaError (
            L,
            "Not enough arguments for '%s' - need %d, had %d. If you are "
            "using objects or handles, make sure that you are using \":\" to "
            "access member functions, rather than \".\"",
    ...
    while (*fmt && nargs <= total_args) {
        ...
        case 'b':
            if (lua_isboolean (L, nargs)) {
                *va_arg (vl, bool*) =
                ...
                LuaError (
                    L,
                    "%s: Argument %d is an invalid type '%s'; boolean "
                    "expected",
                    funcname, nargs, ade_get_type_string (L, nargs));
                if (!optional_args) {
                    va_end (vl);
                    return 0;
                }
        ...
        case 'd':
            if (lua_isnumber (L, nargs)) {
                *va_arg (vl, double*) = (double)lua_tonumber (L, nargs);
                ...
                LuaError (
                    L,
                    "%s: Argument %d is an invalid type '%s'; number expected",
                    funcname, nargs, ade_get_type_string (L, nargs));
                if (!optional_args) {
                    va_end (vl);
                    return 0;
        ...
        case 's':
            if (lua_isstring (L, nargs)) {
                ...
                if (Unicode_text_mode) {
                    // Validate the string when we are in unicode mode to
                    ...
                    if (invalid != end) {
                        if (invalid == value) {
                            ...
                            LuaError (
                                L,
                                "An invalid UTF-8 encoding sequence was "
                                "detected! The first invalid character was as "
                                "the start of the string.");
                        }
                        else {
                            // If the invalid sequence is inside the string
                            // then we try to give some context to make finding
                            // the bug easier
                            auto display_text_start =
                                std::max (value, invalid - 32);
                            std::string context_text (
                                display_text_start, invalid);

                            LuaError (
                                L,
                                "An invalid UTF-8 encoding sequence was "
                                "detected! The error was detected directly "
                                "after this string \"%s\".",
                                context_text.c_str ());
                        }

                        // Finally, assign a default value so that we can
                        // continue
                ...
                *va_arg (vl, const char**) = value;
                ...
                LuaError (
                    L,
                    "%s: Argument %d is an invalid type '%s'; string expected",
                    funcname, nargs, ade_get_type_string (L, nargs));
                if (!optional_args) {
                    va_end (vl);
                    return 0;
        ...
        case 'o': {
            ...
            if (lua_isuserdata (L, nargs)) {
                ...
                LuaError (
                    L,
                    "%s: Argument %d is an invalid type '%s'; type '%s' "
                    "expected",
                    funcname, nargs, ade_get_type_string (L, nargs),
                    getTableEntry (od.idx).GetName ());
                if (!optional_args) {
                    va_end (vl);
                    return 0;
                }
        ...
        case 't': {
            ...
            if (!luacpp::convert::popValue (L, *target, nargs, false)) {
                LuaError (
                    L,
                    "%s: Argument %d is an invalid type '%s'; table expected.",
                    funcname, nargs, ade_get_type_string (L, nargs));
                if (!optional_args) {
                    va_end (vl);
                    return 0;
        ...
        case 'u': {
            ...
            if (!luacpp::convert::popValue (L, *target, nargs, false)) {
                LuaError (
                    L,
                    "%s: Argument %d is an invalid type '%s'; function "
                    "expected.",
                    funcname, nargs, ade_get_type_string (L, nargs));
                if (!optional_args) {
                    va_end (vl);
            ...
            // Automatically assign our error function to function values
            // retrieved from the API
            target->setErrorFunction (
                luacpp::LuaFunction::createFromCFunction (
                    L, ade_friendly_error));
            break;
        }
        case '|':
        ...
        default:
            Error (
                LOCATION, "%s: Bad character passed to ade_get_args; (%c)",
                funcname, *(fmt - 1));
            break;
        }
        nargs++;
...
int ade_set_args (lua_State* L, const char* fmt, ...) {
    ...
    while (*fmt != '\0') {
        ...
        case 'f': lua_pushnumber (L, va_arg (vl, double)); break;
        ...
        case 'x': lua_pushnumber (L, f2fl (va_arg (vl, fix))); break;
        case 'o': {
        ...
        case 't': {
            // Set a table
            luacpp::LuaTable* table = va_arg (
                vl, luacpp::LuaTable*); // This must be a pointer since C++
                                        // classes can't be passed by vararg
            table->pushValue ();
            break;
        }
        case 'u': {
            // Set a function
            luacpp::LuaFunction* func = va_arg (
                vl, luacpp::LuaFunction*); // This must be a pointer since C++
                                           // classes can't be passed by vararg
            func->pushValue ();
            break;
        }
        ...
        default:
            Error (
                LOCATION, "Bad character passed to ade_set_args; (%c)",
                *(fmt - 1));
            setargs--;
        }
        nargs++;
