...
ADE_FUNC (
    sub, l_Utf8, "string arg, number start[, number end = -1]",
    "This function is similar to the standard library string.sub but this can "
    "operate on UTF-8 encoded unicode strings. "
    "This function will respect the unicode mode setting of the current mod "
    "so you can use it even if you don't use Unicode strings.",
    "string", "The requestd substring") {
    ...
    if (normalized_start < 1) { normalized_start = 1; }
    ...
    for (auto cp : unicode::codepoint_range (string, end_ptr)) {
        if ((ptrdiff_t)current >= normalized_start &&
            (ptrdiff_t)current <= normalized_end) {
...
ADE_FUNC (
    len, l_Utf8, "string arg",
    "Determines the number of codepoints in the given string. This respects "
    "the unicode mode setting of the mod.",
    "number", "The number of code points in the string.") {
    const char* string = nullptr;
    if (!ade_get_args (L, "s", &string)) { return ADE_RETURN_NIL; }
