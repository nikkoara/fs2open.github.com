...
// WMC: IMPORTANT
// Be very careful when modifying this library, as the Globals[] library does
// depend on the current number of items in the library. If you add _anything_,
// modify __len. Or run changes by me.

//*****LIBRARY: Scripting Variables
ADE_LIB_DERIV (l_HookVar_Globals, "Globals", nullptr, nullptr, l_HookVar);

ADE_INDEXER (
    l_HookVar_Globals, "number Index", "Array of current HookVariable names",
    "string",
    "Hookvariable name, or empty string if invalid index specified") {
    int idx;
    if (!ade_get_args (L, "*i", &idx)) return ade_set_error (L, "s", "");
...
ADE_FUNC (
    __len, l_HookVar_Globals, NULL, "Number of HookVariables", "number",
    ...
    size_t num_sub = ade_manager::getInstance ()
                         ->getEntry (l_HookVar.GetIdx ())
                         .Num_subentries;
