...
bool lua_sexp_h::isValid () { return sexp_handle != nullptr; }

ADE_OBJ (l_LuaSEXP, lua_sexp_h, "LuaSEXP", "Lua SEXP handle");

ADE_VIRTVAR (
    Action, l_LuaSEXP, "function action", "The action of this SEXP",
    "function", "The action function or nil on error") {
    lua_sexp_h lua_sexp;
    ...
    if (!lua_sexp.isValid ()) { return ADE_RETURN_NIL; }

    if (ADE_SETTING_VAR) {
        Assertion (
            action_arg.isValid (), "Action function reference must be valid!");
