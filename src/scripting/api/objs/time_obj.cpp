...
ADE_OBJ (
    l_Timestamp, uint64_t, "timestamp",
    "A real time time stamp of unspecified precision and resolution.");

ADE_FUNC (
    __sub, l_Timestamp, "timestamp other",
    "Computes the difference between two timestamps", "timespan",
    "The time difference") {
    uint64_t time = 0;
    uint64_t other = 0;
    if (!ade_get_args (
            L, "oo", l_Timestamp.Get (&time), l_Timestamp.Get (&other))) {
...
ADE_OBJ (
    l_TimeSpan, int64_t, "timespan", "A difference between two time stamps");

ADE_FUNC (
    getSeconds, l_TimeSpan, nullptr,
    "Gets the value of this timestamp in seconds", "number",
