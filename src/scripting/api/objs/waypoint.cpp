...
ADE_FUNC (
    getList, l_Waypoint, NULL, "Returns the waypoint list", "waypointlist",
    ...
    if (oh->IsValid () && oh->objp->type == OBJ_WAYPOINT) {
        ...
        if (wp_list != NULL) wpl = waypointlist_h (wp_list);
    }
...
waypointlist_h::waypointlist_h (waypoint_list* n_wlp) {
    wlp = n_wlp;
    if (n_wlp != NULL) { strcpy_s (name, wlp->get_name ()); }
    else {
...
//**********HANDLE: WaypointList
ADE_OBJ (
    l_WaypointList, waypointlist_h, "waypointlist", "waypointlist handle");

ADE_INDEXER (
    l_WaypointList, "number Index",
    "Array of waypoints that are part of the waypoint list", "waypoint",
    "Waypoint, or invalid handle if the index or waypointlist handle is "
    "invalid") {
    int idx = -1;
    ...
    // Get waypoint name
    sprintf (
        wpname, "%s:%d", wlh->wlp->get_name (), calc_waypoint_index (idx) + 1);
    waypoint* wpt = find_matching_waypoint (wpname);
    if ((idx >= 0) && ((uint)idx < wlh->wlp->get_waypoints ().size ()) &&
        (wpt != NULL) && (wpt->get_objnum () >= 0)) {
        return ade_set_args (
            L, "o",
            l_Waypoint.Set (
                object_h (&Objects[wpt->get_objnum ()]),
                Objects[wpt->get_objnum ()].signature));
    }
...
ADE_FUNC (
    __len, l_WaypointList, NULL,
    "Number of waypoints in the list. "
    "Note that the value returned cannot be relied on for more than one "
    "frame.",
    "number", "Number of waypoints in the list, or 0 if handle is invalid") {
    waypointlist_h* wlh = NULL;
...
ADE_VIRTVAR (
    Name, l_WaypointList, "string", "Name of WaypointList", "string",
...
ADE_FUNC (
    isValid, l_WaypointList, NULL,
    "Return if this waypointlist handle is valid", "boolean",
