...
ADE_FUNC (
    isValid, l_File, NULL, "Detects whether handle is valid", "boolean",
    "true if valid, false if handle is invalid, nil if a syntax/type error "
    "occurs") {
    CFILE* cfp = NULL;
    if (!ade_get_args (L, "o", l_File.Get (&cfp))) return ADE_RETURN_NIL;

    if (!cf_is_valid (cfp)) return ADE_RETURN_FALSE;
...
ADE_FUNC (
    ...
    CFILE* cfp;
    if (!ade_get_args (L, "o", l_File.Get (&cfp))) return ADE_RETURN_FALSE;
...
ADE_FUNC (
    flush, l_File, NULL, "Flushes file buffer to disk.", "boolean",
...
ADE_FUNC (
    getPath, l_File, NULL, "Determines path of the given file", "string",
    "Path string of the file handle, or an empty string if it doesn't have "
    "one, or the handle is invalid") {
    CFILE* cfp = NULL;
    if (!ade_get_args (L, "o", l_File.Get (&cfp)))
        return ade_set_error (L, "s", "");

    if (!cf_is_valid (cfp)) return ade_set_error (L, "s", "");
...
ADE_FUNC (
    read, l_File, "number or string, ...",
    "Reads part of or all of a file, depending on arguments passed. Based on "
    "basic Lua file:read function."
    ...
    "<li>(number) - Reads given number of characters, then returns them as a "
    "string.</li></ul>",
    "number or string, ...", "Requested data, or nil if the function fails") {
    CFILE* cfp = NULL;
    if (!ade_get_args (L, "o", l_File.Get (&cfp))) return ADE_RETURN_NIL;
    ...
    for (i = 2; i <= lastarg; i++) {
        ...
        if (type == LUA_TSTRING) {
            ...
            if (!stricmp (fmt, "*n")) {
                ...
                if (cfgets (buf, (int)(sizeof (buf) / sizeof (char)), cfp) ==
                    ...
                    // Note: we also strip carriage return in WMC's
                    // implementation
        ...
        if (type == LUA_TNUMBER ||
            (type == LUA_TSTRING && strpbrk (fmt, "1234567890"))) {
            int num = 0;
            if (type == LUA_TSTRING)
                num = atoi (fmt);
            else
...
ADE_FUNC (
    ...
    if (!(w == NULL || (!stricmp (w, "cur") && o != 0))) {
        ...
        if (cfseek (cfp, o, seek_type)) return ADE_RETURN_FALSE;
    }
...
ADE_FUNC (
    ...
    if (!cf_is_valid (cfp)) return ade_set_error (L, "i", 0);
