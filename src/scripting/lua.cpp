 ...
 * IMPORTANT!
 *
 * For various compiler reasons, all scripting API classes and libraries need
 * to be referenced here! The easiest way to do this is to declare the API
 * elements in the header with the DECLARE_* macros and then include them here
 */
...
void ade_debug_ret (lua_State* L, lua_Debug* ar) {
    ...
    int n;
    for (n = 0; n < 4; n++) { debug_stack[n][0] = '\0'; }

    for (n = 0; n < 4; n++) {
        if (lua_getstack (L, n + 1, ar) == 0) break;
        lua_getinfo (L, "n", ar);
        if (ar->name == NULL) break;
...
static int ade_return_hack (lua_State* L) {
    ...
    for (i = 0; i < num; i++) { lua_pushvalue (L, i + 1); }
...
int script_state::CreateLuaState () {
    ...
    for (i = 0; i < ade_manager::getInstance ()->getNumEntries (); i++) {
        ...
        if (ade_manager::getInstance ()->getEntry (i).ParentIdx ==
            UINT_MAX) // WMC - oh hey, we're done with the meaty point in < 10
                      // lines.
            ade_manager::getInstance ()->getEntry (i).SetTable (
                L, LUA_GLOBALSINDEX,
                LUA_GLOBALSINDEX); // Oh the miracles of OOP.
    }
...
void script_state::EndLuaFrame () { scripting::api::graphics_on_frame (); }
...
void ade_output_toc (FILE* fp, ade_table_entry* ate) {
    ...
    if (ate->Name == NULL) {
        ...
        if (ate->ShortName) fprintf (fp, " (%s)", ate->ShortName);
    }
    fputs ("</a>", fp);

    if (ate->Description) fprintf (fp, " - %s\n", ate->Description);

    fputs ("</dd>\n", fp);
}

static bool sort_table_entries (
    const ade_table_entry* left, const ade_table_entry* right) {
    const char* leftCmp = left->Name != nullptr ? left->Name : left->ShortName;
    const char* rightCmp =
        right->Name != nullptr ? right->Name : left->ShortName;

    if (leftCmp == nullptr) { return false; }
    if (rightCmp == nullptr) { return false; }

    std::string leftStr (leftCmp);
    std::transform (
        std::begin (leftStr), std::end (leftStr), std::begin (leftStr),
        ::tolower);

    std::string rightStr (rightCmp);
    std::transform (
        std::begin (rightStr), std::end (rightStr), std::begin (rightStr),
        ::tolower);
...
static bool
sort_doc_entries (const ade_table_entry* left, const ade_table_entry* right) {
    if (left->Instanced == right->Instanced) {
        // Same type -> compare as normal
        return sort_table_entries (left, right);
    }
    if (left->Instanced) { return true; }
    return false;
}
...
void script_state::OutputLuaMeta (FILE* fp) {
    ...
    std::sort (
        std::begin (table_entries), std::end (table_entries),
        sort_table_entries);
    for (auto entry : table_entries) { ade_output_toc (fp, entry); }
    table_entries.clear ();
    ...
    for (i = 0; i < ade_manager::getInstance ()->getNumEntries (); i++) {
        ate = &ade_manager::getInstance ()->getEntry (i);
        if (ate->ParentIdx == UINT_MAX && ate->Type == 'o' &&
            !ate->Instanced) {
            table_entries.push_back (ate);
        }
    }
    ...
    for (i = 0; i < ade_manager::getInstance ()->getNumEntries (); i++) {
        ate = &ade_manager::getInstance ()->getEntry (i);
        if (ate->ParentIdx == UINT_MAX) table_entries.push_back (ate);
    }

    std::sort (
        std::begin (table_entries), std::end (table_entries),
        sort_doc_entries);
    for (auto entry : table_entries) { entry->OutputMeta (fp); }
    table_entries.clear ();
    ...
    for (i = 0; i < Num_enumerations; i++) {
        ...
        // fprintf(fp, "<dd><dl><dt><b>%s</b></dt><dd>%s</dd></dl></dd>",
        // Enumerations[i].name, Enumerations[i].desc);
