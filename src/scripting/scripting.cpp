...
flag_def_list Script_conditions[] = {
    { "State", CHC_STATE, 0 },        { "Campaign", CHC_CAMPAIGN, 0 },
    { "Mission", CHC_MISSION, 0 },    { "Object Type", CHC_OBJECTTYPE, 0 },
    { "Ship", CHC_SHIP, 0 },          { "Ship class", CHC_SHIPCLASS, 0 },
    { "Ship type", CHC_SHIPTYPE, 0 }, { "Weapon class", CHC_WEAPONCLASS, 0 },
    { "KeyPress", CHC_KEYPRESS, 0 },  { "Action", CHC_ACTION, 0 },
    { "Version", CHC_VERSION, 0 },    { "Application", CHC_APPLICATION, 0 }
};

int Num_script_conditions =
    sizeof (Script_conditions) / sizeof (flag_def_list);
...
bool ConditionedHook::AddAction (script_action* sa) {
    if (!script_hook_valid (&sa->hook)) return false;
...
bool ConditionedHook::ConditionsValid (
    int action, object* objp, int more_data) {
    uint i;
    ...
    for (i = 0; i < MAX_HOOK_CONDITIONS; i++) {
            ...
            if (action == CHA_COLLIDEWEAPON) {
                if (strcasecmp (Weapon_info[more_data].name, scp->data.name) != 0)
                    return false;
                       ...
                       action == CHA_ONWPDESELECTED ||
                       action == CHA_ONWPEQUIPPED || action == CHA_ONWPFIRED ||
                       action == CHA_ONTURRETFIRED)) {
                if (objp == NULL ||
                ...
                // Okay, if we're still here, then objp is both valid and a
                // ship
                ship* shipp = &Ships[objp->instance];
                bool primary = false, secondary = false, prev_primary = false,
                ...
                case CHA_ONWPDESELECTED:
                    ...
                    if (!prev_secondary && !secondary && !prev_primary &&
                        !primary)
                        return false;

                    if ((!prev_secondary && !secondary) &&
        ...
        case CHC_VERSION: {
            // Goober5000: I'm going to assume scripting doesn't care about SVN
            // revision
            char buf[32];
            sprintf (
...
bool ConditionedHook::Run (
    script_state* sys, int action, char format, void* data) {
    Assert (sys != NULL);

    // Do the actions
    for (std::vector< script_action >::iterator sap = Actions.begin ();
         sap != Actions.end (); ++sap) {
...
bool ConditionedHook::IsOverride (script_state* sys, int action) {
    ...
    for (std::vector< script_action >::iterator sap = Actions.begin ();
         sap != Actions.end (); ++sap) {
        if (sap->action_type == action) {
            if (sys->IsOverride (sap->hook)) return true;
        }
    }
...
void script_state::SetHookObjects (int num, ...) {
    ...
    if (this->OpenHookVarTable ()) {
        ...
        LuaError (
            LuaState,
            "Could not get HookVariable library to add hook variables - get a "
            "coder");
    }
    va_end (vl);
}
...
bool script_state::OpenHookVarTable () {
    if (ohvt_isopen)
        Error (
            LOCATION,
            "OpenHookVarTable was called twice with no call to "
            "CloseHookVarTable - missing call ahoy!");
...
bool script_state::CloseHookVarTable () {
    if (!ohvt_isopen) {
        Error (
            LOCATION,
            "CloseHookVarTable was called with no associated call to "
            "OpenHookVarTable");
    }
    ...
    if (top_ldx >= ohvt_poststack) {
        ...
        Error (
            LOCATION,
            "CloseHookVarTable() was called with too few objects on the "
            "stack; get a coder. (Stack: %d OHVT post: %d OHVT num: %d",
            top_ldx, ohvt_poststack, ohvt_isopen);
        return false;
    }
}

void script_state::SetHookVar (
    const char* name, char format, const void* data) {
    if (format == '\0') return;

    if (LuaState != NULL) {
        char fmt[2] = { format, '\0' };
        int data_ldx = 0;
        if (data == NULL) data_ldx = lua_gettop (LuaState);

        if (data_ldx < 1 && data == NULL) return;
        ...
        if (this->OpenHookVarTable ()) {
            ...
            if (data == NULL) { lua_pushvalue (LuaState, data_ldx); }
            else {
                switch (format) {
                case 's': ade_set_args (LuaState, fmt, data); break;
                case 'i': ade_set_args (LuaState, fmt, *(int*)data); break;
                case 'b': ade_set_args (LuaState, fmt, *(bool*)data); break;
                case 'f': ade_set_args (LuaState, fmt, *(float*)data); break;
                default:
            ...
            if (data_ldx) lua_pop (LuaState, 1);
            ...
            LuaError (
                LuaState,
                "Could not get HookVariable library to set hook variable '%s'",
                name);
            if (data_ldx) lua_pop (LuaState, 1);
        }
    }
    ...
    if (LuaState != NULL) {
        ...
        if (this->OpenHookVarTable ()) {
            ...
            if (!lua_isnil (LuaState, -1)) {
                if (data != NULL) { ade_get_args (LuaState, fmt, data); }
                got_global = true;
            }
            ...
            LuaError (
                LuaState,
                "Could not get HookVariable library to get hook variable '%s'",
                name);
        }
    }
...
void script_state::RemHookVars (unsigned int num, ...) {
    if (LuaState != NULL) {
        ...
        if (this->OpenHookVarTable ()) {
            ...
            LuaError (
                LuaState,
                "Could not get HookVariable library to remove hook variables "
                "- get a coder");
        }
    }
}
...
int script_state::RunBytecodeSub (
    script_function& func, char format, void* data) {
    using namespace luacpp;

    if (!func.function.isValid ()) { return 1; }
...
int script_state::RunCondition (
    int action, char format, void* data, object* objp, int more_data) {
    int num = 0;
    for (std::vector< ConditionedHook >::iterator chp =
             ConditionalHooks.begin ();
         chp != ConditionalHooks.end (); ++chp) {
...
bool script_state::IsConditionOverride (int action, object* objp) {
    ...
    for (std::vector< ConditionedHook >::iterator chp =
        ...
        if (chp->ConditionsValid (action, objp)) {
            if (chp->IsOverride (this, action)) return true;
        }
    }
    return false;
}

void script_state::EndFrame () { EndLuaFrame (); }
...
script_state::~script_state () { Clear (); }

void script_state::SetLuaSession (lua_State* L) {
    if (LuaState != NULL) { lua_close (LuaState); }
    LuaState = L;
    if (LuaState != NULL) { Langs |= SC_LUA; }
...
int script_state::OutputMeta (const char* filename) {
    ...
    fprintf (
        fp,
        "<html>\n<head>\n\t<title>Script Output - FSO v%s "
        "(%s)</title>\n</head>\n",
        FS_VERSION_FULL, StateName);
    fputs ("<body>", fp);
    fprintf (
        fp, "\t<h1>Script Output - FSO v%s (%s)</h1>\n", FS_VERSION_FULL,
        StateName);
    ...
    if (Langs & SC_LUA) { fputs ("<dd><a href=\"#Lua\">Lua</a></dd>", fp); }
    fputs ("</dl>", fp);
...
bool script_state::EvalString (
    const char* string, const char* format, void* rtn, const char* debug_str) {
    using namespace luacpp;
    ...
    if (string[0] == '[' && lastchar != ']') { return false; }

    size_t s_bufSize = string_size + 8;
    std::string s;
    s.reserve (s_bufSize);
    if (string[0] != '[') {
        if (rtn != NULL) { s = "return "; }
        s += string;
    }
    else {
    ...
    try {
        ...
        function.setErrorFunction (
            LuaFunction::createFromCFunction (LuaState, ade_friendly_error));
...
void script_state::ParseChunkSub (
    script_function& script_func, const char* debug_str) {
    using namespace luacpp;
    ...
    if (check_for_string ("[[")) {
        ...
        // WMC - use filename instead of debug_str so that the filename gets
        // passed.
        ...
        if (cfp == NULL) {
            Warning (
                LOCATION, "Could not load lua script file '%s'",
                function_name.c_str ());
            return;
        }
        else {
        ...
        // Determine the current line in the file so that the Lua source can
        // begin at the same line as in the table This will make sure that the
        // line in the error message matches the line number in the table.
    ...
    try {
        auto function =
            LuaFunction::createFromCode (LuaState, source, function_name);
        function.setErrorFunction (
            LuaFunction::createFromCFunction (LuaState, ade_friendly_error));
...
int script_parse_condition () {
    ...
    for (int i = 0; i < Num_script_conditions; i++) {
        ...
        if (optional_string (buf)) return Script_conditions[i].def;
    }
...
flag_def_list* script_parse_action () {
    ...
    for (int i = 0; i < Num_script_actions; i++) {
        ...
        if (optional_string (buf)) return &Script_actions[i];
    }
...
bool script_state::ParseCondition (const char* filename) {
    ...
    for (condition = script_parse_condition (); condition != CHC_NONE;
         condition = script_parse_condition ()) {
        ...
        case CHC_VERSION:
        case CHC_APPLICATION:
        default: stuff_string (sct.data.name, F_NAME, CONDITION_LENGTH); break;
        }
        ...
        if (!chp->AddCondition (&sct)) {
            Warning (
                LOCATION,
                "Could not add condition to conditional hook in file '%s'; "
                "you may have more than %d",
    ...
    for (action = script_parse_action (); action != NULL;
         action = script_parse_action ()) {
        script_action sat;
        sat.action_type = action->def;

        // WMC - build error string
        char* buf =
            (char*)vm_malloc (strlen (filename) + strlen (action->name) + 4);
        ...
        // Add the action
        if (chp->AddAction (&sat)) actions_added = true;
    }

    if (!actions_added) {
        Warning (
            LOCATION, "No actions specified for conditional hook in file '%s'",
            filename);
...
bool script_state::IsOverride (script_hook& hd) {
    if (!hd.hook_function.function.isValid ()) return false;
...
void scripting_state_init () {
    // nothing to do here
    if (scripting_state_inited) return;
...
void scripting_state_do_frame (float /*frametime*/) {
    ...
    switch (k) {
    case KEY_ESC: gameseq_post_event (GS_EVENT_MAIN_MENU); return;
    }
}
