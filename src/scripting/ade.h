 ...
 *
 * These functions enable the code to communicate with external scripts and
 * expose an API for them to use
 */
...
#define ADE_SETTING_VAR \
    lua_toboolean (L, lua_upvalueindex (ADE_SETTING_UPVALUE_INDEX))

/** Used for internal object->lua_set and lua_get->object communication.
 * Must remain a struct and only contain POD datatypes because this is passed
 * via variable args.
 *
 * @ingroup ade_api
 */
...
public:
    ...
    static_assert (
        std::is_trivially_copyable< StoreType >::value,
        "ADE object types must be trivially copyable!");
#endif
    ade_obj (
        const char* in_name, const char* in_desc,
        const ade_lib_handle* in_deriv = NULL) {
        ade_table_entry ate;

        // WMC - object metadata are uninstanced library types
        ate.Name = in_name;
        if (in_deriv != NULL) { ate.DerivatorIdx = in_deriv->GetIdx (); }
        ate.Type = 'o';
    ...
    // WMC - Use this to store object data for return, or for setting as a
    // global
    ade_odata Set (
        const StoreType& obj, ODATA_SIG_TYPE n_sig = ODATA_SIG_DEFAULT) const {
        ade_odata od;
        od.idx = LibIdx;
        od.sig = &n_sig;
        od.buf = (void*)&obj;
...
public:
    ade_lib (
        const char* in_name, const ade_lib_handle* parent = NULL,
        const char* in_shortname = NULL, const char* in_desc = NULL);
...
public:
    ade_func (
        ...
        const char* args = NULL, const char* desc = NULL,
...
public:
    ade_indexer (
        lua_CFunction func, const ade_lib_handle& parent,
        const char* args = NULL, const char* desc = NULL,
        const char* ret_type = NULL, const char* ret_desc = NULL);
};
 ...
 *
 * This should be used everywhere where an object value is returned to make
 * sure that the scripter has access to all API functions.
 *
 ...
 *
 * This uses the specified file name and either retrieves it from the default
 * files or uses it as a file name if the mod option is enabled.
 *
 * @param L The lua state
