 ...
 * Each key-value pair is converted into the type given to the Container and
 * then push_back is used to add it.
 *
 * @param table The table to be used
 * @param keyValueList The container which should be used
 * @return void
 *
 * @tparam Container Must be a container type with a push_back function and
 * must have a value_type which exposes a first_type and second_type typedef
 ...
 * @return void
 *
 * @tparam Container The type of the container into which the values should be
 * inserted, must have a value_type typedef so the lua values can be converted
 * and a push_back function.
 *
 * @exception LuaException May be thrown if an element of the table is not
 * convertible to Container::value_type.
 */
template< typename Container >
