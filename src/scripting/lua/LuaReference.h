 ...
 *
 * Wraps a reference to a lua-value. This object may only be owned by one
 * owner. If you need reference sharing then LuaReference should be used.
 */
class UniqueLuaReference {
private:
    lua_State* _luaState;
...
public:
     ...
     *
     * Sets the lua_State which will be used to hold the reference and the
     ...
     *
     * @warning Do not call this directly. Use UniqueLuaReference::create
     * instead.
     */
     ...
     *
     * Copies the value at @c position and creates a reference to it. Also
     * replaces the value at that position with the copied value.
     *
     * @param state The state to create the reference in.
     * @param position The stack position of the value, defaults to the top of
     * the stack (-1).
     ...
     * There is no copy-constructor as unintentional copying could lead to
     * excessive creation and deletion of lua references
     *
