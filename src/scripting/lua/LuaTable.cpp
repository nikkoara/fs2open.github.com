...
LuaTableIterator::LuaTableIterator (const LuaTable& t)
    : _luaState (t.getLuaState ()) {
...
LuaTableIterator::~LuaTableIterator () { lua_settop (_luaState, _stackTop); }
bool LuaTableIterator::hasElement () { return _hasElement; }
...
bool convert::popValue (
    lua_State* luaState, LuaTable& target, int stackposition, bool remove) {
    if (!internal::isValidIndex (luaState, stackposition)) { return false; }

    if (!lua_istable (luaState, stackposition)) { return false; }
    else {
        target.setReference (
