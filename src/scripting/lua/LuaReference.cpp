...
LuaReference UniqueLuaReference::create (lua_State* state, int position) {
    if (state == nullptr) { throw LuaException ("Need a valid lua state!"); }

    lua_pushvalue (state, position);

    LuaReference ref = std::make_shared< UniqueLuaReference > (
...
UniqueLuaReference::UniqueLuaReference (lua_State* state, int reference)
    : _luaState (state), _reference (reference) {
    if (state == nullptr) { throw LuaException ("Need a valid lua state!"); }

    if (reference < 0) {
        throw LuaException (
            "Reference must be greater than or equal to zero!");
    }
}
...
UniqueLuaReference::~UniqueLuaReference () { this->removeReference (); }
...
int UniqueLuaReference::getReference () const { return _reference; }

bool UniqueLuaReference::isValid () const {
    if (_luaState == nullptr) { return false; }

    if (_reference == LUA_NOREF) { return false; }
