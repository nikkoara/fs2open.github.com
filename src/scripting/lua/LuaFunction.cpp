...
namespace luacpp {
LuaFunction LuaFunction::createFromCFunction (
    lua_State* L, lua_CFunction function, const LuaValueList& upvalues) {
    LuaFunction func;

    for (auto& val : upvalues) { val.pushValue (); }
...
LuaFunction LuaFunction::createFromCode (
    lua_State* L, std::string const& code, std::string const& name) {
    int load_err =
...
LuaFunction::LuaFunction (const LuaFunction& other)
    : LuaValue (other), _errorFunction (other._errorFunction) {}
...
LuaValueList LuaFunction::call (const LuaValueList& args) {
    ...
    for (LuaValueList::const_iterator iter = args.begin ();
         iter != args.end (); ++iter) {
        iter->pushValue ();
    }
    ...
    if (!err) {
        ...
        // If it pushed more than one value then this will discard all of them
        // except the last one
...
bool ::luacpp::convert::popValue (
    lua_State* luaState, LuaFunction& target, int stackposition, bool remove) {
    if (!internal::isValidIndex (luaState, stackposition)) { return false; }

    if (!lua_isfunction (luaState, stackposition)) { return false; }
    else {
        target.setReference (
            UniqueLuaReference::create (luaState, stackposition));
