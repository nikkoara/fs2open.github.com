...
void pushValue (lua_State* L, const ade_odata& od) {
    ...
    char* newod =
        (char*)lua_newuserdata (L, od.size + sizeof (ODATA_SIG_TYPE));
...
bool popValue (
    lua_State* luaState, int& target, int stackposition, bool remove) {
    double temp;

    auto ret = popValue (luaState, temp, stackposition, remove);
    if (ret) { target = (int)temp; }
    return ret;
}
bool popValue (
    lua_State* luaState, size_t& target, int stackposition, bool remove) {
    double temp;

    auto ret = popValue (luaState, temp, stackposition, remove);
    if (ret) { target = (size_t)temp; }
    return ret;
}
bool popValue (
    lua_State* luaState, float& target, int stackposition, bool remove) {
    double temp;

    auto ret = popValue (luaState, temp, stackposition, remove);
    if (ret) { target = (float)temp; }
    return ret;
}
bool popValue (
    lua_State* luaState, double& target, int stackposition, bool remove) {
    if (!internal::isValidIndex (luaState, stackposition)) { return false; }

    if (!lua_isnumber (luaState, stackposition)) { return false; }
    else {
        target = lua_tonumber (luaState, stackposition);

        if (remove) { lua_remove (luaState, stackposition); }

        return true;
    }
}
bool popValue (
    lua_State* luaState, std::string& target, int stackposition, bool remove) {
    if (!internal::isValidIndex (luaState, stackposition)) { return false; }

    if (!lua_isstring (luaState, stackposition)) { return false; }
    else {
        size_t size;
...
bool popValue (
    lua_State* luaState, bool& target, int stackposition, bool remove) {
    if (!internal::isValidIndex (luaState, stackposition)) { return false; }

    if (!lua_isboolean (luaState, stackposition)) { return false; }
    else {
...
bool popValue (
    lua_State* luaState, lua_CFunction& target, int stackposition,
    bool remove) {
    if (!internal::isValidIndex (luaState, stackposition)) { return false; }

    if (!lua_iscfunction (luaState, stackposition)) { return false; }
    else {
...
bool popValue (
    lua_State* L, scripting::ade_odata& od, int stackposition, bool remove) {
    // WMC - Get metatable
    ...
    if (lua_tonumber (L, -1) != od.idx) {
        ...
        if ((uint)lua_tonumber (L, -1) != od.idx) {
            // Issue the LuaError here since this is the only place where we
            // have all relevant information
            LuaError (
                L,
                "Argument %d is the wrong type of userdata; '%s' given, but "
                "'%s' expected",
                stackposition, getTableEntry ((uint)lua_tonumber (L, -2)).Name,
    ...
    if (remove) { lua_remove (L, stackposition); }
