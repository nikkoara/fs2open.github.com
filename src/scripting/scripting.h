...
public:
    ...
    script_condition () : condition_type (CHC_NONE) {
...
public:
    ...
    bool
    ConditionsValid (int action, class object* objp = NULL, int more_data = 0);
    bool IsOverride (class script_state* sys, int action);
    bool Run (
        class script_state* sys, int action, char format = '\0',
        void* data = NULL);
};
...
private:
    void
    ParseChunkSub (script_function& out_func, const char* debug_str = NULL);
    int RunBytecodeSub (
        script_function& func, char format = '\0', void* data = NULL);
...
public:
    ...
    bool EvalString (
        const char* string, const char* format = NULL, void* rtn = NULL,
    ...
    int RunCondition (
        int condition, char format = '\0', void* data = NULL,
        class object* objp = NULL, int more_data = 0);
