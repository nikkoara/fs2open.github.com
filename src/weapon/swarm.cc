        ...
        if (timestamp_elapsed(swarmp->change_timestamp)) {
                ...
                if (swarmp->path_num == -1) {
                        ...
                        swarmp->change_time = int(
                                SWARM_CHANGE_DIR_TIME + SWARM_TIME_VARIANCE * (fs2::prng::randf(0) - 0.5f) * 2);
                        ...
                        hobjp != &obj_used_list && f2fl(Missiontime - wp->creation_time) > 0.5f && f2fl(Missiontime - wp->creation_time) > wip->free_flight_time) {
        ...
        int parent_objnum, ship_subsys *turret, weapon_info *wip, int weapon_num)
{
    ...
    */
        ...
        ASSERT(
                (turret->turret_enemy_objnum >= 0) && (turret->turret_enemy_objnum < MAX_OBJECTS));
        ...
        // valid swarm weapon
        ASSERT(
                ((wip->wi_flags[Weapon::Info_Flags::Swarm]) && (wip->swarm_count > 0)) || ((wip->wi_flags[Weapon::Info_Flags::Corkscrew]) && (wip->cs_num_fired > 0)));
                                        ...
                                        if (timestamp_elapsed(tsi->time_to_fire)) {
                                                ...
                                                if (tsi->num_to_launch == 0) {
                                                        ...
                                                        if (subsys->turret_animation_position == MA_POS_READY)
                                                                subsys
                                                                        ->turret_animation_done_time
                                                                        = timestamp(
                                                                                Weapon_info[tsi->weapon_class].SwarmWait * 3);
