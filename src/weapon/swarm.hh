...
#ifndef FREESPACE2_WEAPON_SWARM_HH
#define FREESPACE2_WEAPON_SWARM_HH
...
struct swarm_info {
        ...
        int homing_objnum; // object number that swarm missile is homing on, -1 if
                           // not homing
        int change_time;   // when swarm missile should next update direction, based
                           // on missile speed
        float angle_offset;
...
void turret_swarm_set_up_info(
        int parent_objnum, ship_subsys *turret, struct weapon_info *wip,
        int weapon_num);
void turret_swarm_check_validity();

#endif // FREESPACE2_WEAPON_SWARM_HH
