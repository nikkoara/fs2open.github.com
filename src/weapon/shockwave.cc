...
void shockwave_move(object *shockwave_objp, float frametime)
{
        ...
        ASSERTX(
                ...
                shockwave_objp->type, OBJ_SHOCKWAVE);
        ASSERTX(
                shockwave_objp->instance >= 0 && shockwave_objp->instance < MAX_SHOCKWAVES,
        ...
        for (objp = GET_FIRST(&obj_used_list);
             objp != END_OF_LIST(&obj_used_list); objp = GET_NEXT(objp)) {
                ...
                if (weapon_area_calc_damage(
                            ...
                            sw->damage, &blast, &damage, sw->radius)
                    == -1) {
                        continue;
                }
...
void shockwave_render(object *objp, model_draw_list *scene)
{
        ...
        if (sw->model_id > -1) {
                ...
                render_info.set_flags(
                        MR_NO_LIGHTING | MR_NO_FOGGING | MR_NORMAL | MR_CENTER_ALPHA | MR_NO_CULL | MR_NO_BATCH);
                ...
                if (Cmdline_fb_explosions && Default_2D_shockwave_index > -1) {
                        ...
                        float intensity = ((sw->time_elapsed / sw->total_time) > 0.9f)
                                                  ? (1.0f - (sw->time_elapsed / sw->total_time)) * 10.0f
                                                  : 1.0f;
                        batching_add_distortion_bitmap_rotated(
                                Shockwave_info[Default_2D_shockwave_index].bitmap_id + shockwave_get_framenum(objp->instance, Shockwave_info[Default_2D_shockwave_index].bitmap_id),
...
void shockwave_level_init()
{
        ...
        ASSERTX(
                ...
                "empty!");
        ASSERTX(
                ((Shockwave_info[0].bitmap_id >= 0) || (Shockwave_info[0].model_id >= 0)),
