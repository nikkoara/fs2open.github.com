...
void trail_calc_facing_pts (
    vec3d* top, vec3d* bot, vec3d* fvec, vec3d* pos, float w) {
    vec3d uvec, rvec;

    vm_vec_sub (&rvec, &Eye_position, pos);
    if (!IS_VEC_NULL (&rvec)) vm_vec_normalize (&rvec);

    vm_vec_cross (&uvec, fvec, &rvec);
    if (!IS_VEC_NULL (&uvec)) vm_vec_normalize (&uvec);
...
int trail_is_on_ship (trail* trailp, ship* shipp) {
    if (trailp == NULL) return 0;

    for (int idx = 0; idx < MAX_SHIP_CONTRAILS; idx++) {
        if (shipp->trail_ptr[idx] == trailp) { return 1; }
    }
...
void trail_render (trail* trailp) {
    ...
    // if this trail is on the player ship, and he's in any padlock view except
    // rear view, don't draw
    ...
    for (i = 0; i < num_sections; i++) {
        ...
        if (i == 0) {
            if (num_sections > 1) {
                vm_vec_sub (
                    &tmp_fvec, &trailp->pos[n], &trailp->pos[sections[i + 1]]);
        ...
        if (i > 0) {
            ...
            if (i == num_sections - 1) {
                ...
                Trail_v_list[nv].r = Trail_v_list[nv].g = Trail_v_list[nv].b =
                    l;
                nv++;
    ...
    // GR_BITBLT_MODE_NORMAL, 1.0f ); gr_render(nv, Trail_v_list,
    // TMAP_FLAG_TEXTURED | TMAP_FLAG_ALPHA | TMAP_FLAG_GOURAUD | TMAP_FLAG_RGB
    // | TMAP_HTL_3D_UNLIT | TMAP_FLAG_TRISTRIP);

    material material_def;
    material_set_unlit (
        &material_def, ti->texture.bitmap_id, 1.0f, true, true);
    g3_render_primitives_colored_textured (
        &material_def, Trail_v_list, nv, PRIM_TYPE_TRISTRIP, false);
}

void trail_add_segment (trail* trailp, vec3d* pos) {
    int next = trailp->tail;
    trailp->tail++;
    if (trailp->tail >= NUM_TRAIL_SECTIONS) trailp->tail = 0;

    if (trailp->head == trailp->tail) {
        // wrapped!!
        trailp->head++;
        if (trailp->head >= NUM_TRAIL_SECTIONS) trailp->head = 0;
    }
...
void trail_set_segment (trail* trailp, vec3d* pos) {
    ...
    if (next < 0) { next = NUM_TRAIL_SECTIONS - 1; }
...
void trail_object_died (trail* trailp) { trailp->object_died = true; }
...
void trail_render_all () {
    ...
    // No trails at slot 0
    if (!Detail.weapon_extras) return;

    for (trail* trailp = Trails.next; trailp != &Trails;
         trailp = trailp->next) {
