...
float Emp_intensity = -1.0f; // current intensity of the EMP effect (normalized
                             // to EMP_INTENSITY_MAX)
...
void emp_apply (
    vec3d* pos, float inner_radius, float outer_radius, float emp_intensity,
    float emp_time, bool use_emp_time_for_capship_turrets) {
    ...
    // all machines check to see if the blast hit a bomb. if so, shut it down
    // (can't move anymore)
    for (mo = GET_FIRST (&Missile_obj_list);
        ...
        if ((wip_target->weapon_hitpoints > 0) &&
            !(wip_target->wi_flags[Weapon::Info_Flags::No_emp_kill])) {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        // if the ship is a cruiser or cap ship, only apply the EMP effect to
        // turrets
        if (Ship_info[Ships[target->instance].ship_info_index]
                .is_big_or_huge ()) {
            float capship_emp_time = use_emp_time_for_capship_turrets
                                         ? (emp_time * 1000.0f)
                                         : MAX_TURRET_DISRUPT_TIME;

            moveup = &Ships[target->instance].subsys_list;
            if (moveup->next != NULL) { moveup = moveup->next; }
            while (moveup != &Ships[target->instance].subsys_list) {
                ...
                if ((moveup->system_info != NULL) &&
                    ...
                    vm_vec_unrotate (
                        &actual_pos, &moveup->system_info->pnt,
                        &target->orient);
                    vm_vec_add2 (&actual_pos, &target->pos);
                    vm_vec_sub (&dist, &actual_pos, pos);
                    dist_mag = vm_vec_mag (&dist);

                    // if for some reason, the object was outside the blast,
                    // radius
        ...
        // otherwise coat the whole ship with the effect. mmmmmmmmm.
            ...
            if (dist_mag > outer_radius) { continue; }
            ...
            scale_factor -= Ship_info[Ships[target->instance].ship_info_index]
                                .emp_resistance_mod;

            if (scale_factor < 0.0f) { continue; }
...
// start the emp effect for the passed ship (setup lightning arcs, timestamp,
// etc) NOTE : if this ship is also me, I should call emp_start_local() as well
void emp_start_ship (object* ship_objp, float intensity, float time) {
    ...
    if (time >= EMP_TIME_MAX) { time = EMP_TIME_MAX - 0.1f; }
...
void emp_process_ship (ship* shipp) {
    ...
    if (!(Ship_info[shipp->ship_info_index].is_fighter_bomber ())) { return; }

    // if he's docked, or ordered to not move, bail now
    if (object_is_docked (objp) || (aip->mode == AIM_STILL) ||
        (aip->mode == AIM_PLAY_DEAD)) {
        return;
    }

    // pick targets randomly and wackily so that the ship flies crazily :)
    if (((int)f2fl (Missiontime) +
         (int)(EMP_INTENSITY_MAX * shipp->emp_intensity)) %
            mod_val ==
        0) {
        int ship_lookup =
            ship_get_random_team_ship (iff_get_attackee_mask (shipp->team));

        // if we got a valid ship object to target
        if ((ship_lookup >= 0) && (Ships[ship_lookup].objnum >= 0) &&
            !(Objects[Ships[ship_lookup].objnum]
...
// start the emp effect for MYSELF (intensity == arbitrary intensity variable,
// time == time the effect will last) NOTE : time should be in seconds
void emp_start_local (float intensity, float time) {
    ...
    if (Player_ai != NULL) { Player_ai->target_objnum = -1; }
...
int emp_active_local () { return EMP_ACTIVE_LOCAL (); }
...
void emp_process_local () {
    ...
    if ((Emp_wacky_target_timestamp == -1) ||
        timestamp_elapsed (Emp_wacky_target_timestamp)) {
        // choose a target (if not the "first" time)
        if (Emp_wacky_target_timestamp != -1) { hud_target_random_ship (); }
...
// emp hud string
void emp_hud_string (
    int x, int y, int gauge_id, const char* str, int resize_mode) {
    char tmp[256] = "";
...
float emp_current_intensity () { return Emp_intensity; }
