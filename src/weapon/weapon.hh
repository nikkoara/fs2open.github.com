...
#define WP_UNUSED -1
#define WP_LASER                                                                      \
        0       // PLEASE NOTE that this flag specifies ballistic primaries as well -
                // Goober5000
#define WP_MISSILE 1
#define WP_BEAM    2
...
#define PSPEW_SPARKLER \
        2            // random particles in every direction, can be sperical or ovoid
#define PSPEW_RING 3 // outward expanding ring
#define PSPEW_PLUME                                                                \
        4       // spewers arrayed within a radius for thruster style effects, may
                // converge or scatter

#define MAX_PARTICLE_SPEWERS \
        4 // i figure 4 spewers should be enough for now -nuke
#define MAX_WEP_DAMAGE_SLOTS                                                           \
        32      // Maximum number of ships which can be counted as killer or assits on
                // destroying this weapon

// scale factor for supercaps taking damage from weapons which are not
// "supercap" weapons
...
struct weapon {
        ...
        int missile_list_index;     // index for missiles into Missile_obj_list, -1
                                    // weapon not missile
        ...
        // laser stuff
        float laser_bitmap_frame;      // used to keep track of which frame the
        ...
        // particle spew info
        int particle_spew_time[MAX_PARTICLE_SPEWERS]; // time to spew next bunch of
                                                      // particles
        float particle_spew_rand;                     // per weapon randomness value used by some
                                                      // particle spew types -nuke
        ...
        float launch_speed;   // the initial forward speed (can vary due to additive
                              // velocity or acceleration) currently only gets set
                              // when weapon_info->acceleration_time is used

        mc_info *collisionInfo; // The last collision of this weapon or NULL if it
                                // had none

        sound_handle hud_in_flight_snd_sig; // Signature of the sound played while
...
struct beam_weapon_info {
        ...
        float beam_iff_miss_factor[MAX_IFFS]
                                  [NUM_SKILL_LEVELS]; // magic # which makes beams
                                                      // miss more. by parent iff
        ...
        float glow_length;                            // (DahBlount) determines the length the muzzle glow
                                                      // when using a directional glow
        bool directional_glow;                        // (DahBlount) makes the muzzle glow render to a
                                                      // poly that is oriented along the direction of fire
        int beam_shots;                               // # of shots the beam takes
        float beam_shrink_factor;                     // what percentage of total beam lifetime when
                                                      // the beam starts shrinking
        float beam_shrink_pct;                        // what percent/second the beam shrinks at
        beam_weapon_section_info
                sections[MAX_BEAM_SECTIONS]; // info on the visible sections of the
                                             // beam
...
enum InFlightSoundType { TARGETED,
                         UNTARGETED,
                         ALWAYS };
...
struct weapon_info {
        ...
        char external_model_name[MAX_FILENAME_LEN];   // the model rendered on the
                                                      // weapon points of a ship
        int external_model_num;                       // the model rendered on the weapon points of a
                                                      // ship
        int hud_target_lod;                           // LOD to use when rendering weapon model to the hud
                                                      // targetbox
        int num_detail_levels;                        // number of LODs defined in table (optional)
        int detail_distance[MAX_MODEL_DETAIL_LEVELS]; // LOD distances define in
                                                      // table (optional)
        ...
        char tech_model[MAX_FILENAME_LEN];            // Image to display in the techroom
                                                      // (TODO) or the weapon selection screen
                                                      // if the ANI isn't specified/missing

        vec3d closeup_pos;  // position for camera to set an offset for viewing the
                            // weapon model
        float closeup_zoom; // zoom when using weapon model in closeup view in
                            // loadout selection
        ...
        float acceleration_time;  // how many seconds to reach max speed
                                  // (secondaries only)
        float vel_inherit_amount; // how much of the parent ship's velocity is
                                  // inherited (0.0..1.0)
        ...
        float lifetime;        // How long the AI thinks this thing lives (used for
        ...
        float cargo_size;   // cargo space taken up by individual weapon (missiles
                            // only)
        float rearm_rate;   // rate per second at which secondary weapons are loaded
                            // during rearming
        float weapon_range; // max range weapon can be effectively fired.  (May be
                            // less than life * speed)
        ...
        particle::ParticleEffectHandle
                impact_weapon_expl_effect; // Impact particle effect

        particle::ParticleEffectHandle
                dinky_impact_weapon_expl_effect; // Dinky impact particle effect
        ...
        // Particle effect for the various states, WeaponState::NORMAL is the state
        // for the whole lifetime, even for missiles
        std::unordered_map<
                WeaponState, particle::ParticleEffectHandle, WeaponStateHash >
                state_effects;
        ...
        // Beam weapon effect
        beam_weapon_info b_info; // this must be valid if the weapon is a beam
                                 // weapon WIF_BEAM or WIF_BEAM_SMALL
        ...
        float elec_beam_mult;    // used instead of elec_weap_mult if turret is a beam
                                 // turret
        float elec_sensors_mult; // multiplier on sensors and awacs
        int elec_randomness;     // disruption time lasts + or - this value from whats
        ...
        bool cm_kill_single;         // should the countermeasure kill just the single
                                     // decoyed missile within CMEASURE_DETONATE_DISTANCE?
        ...
        int weapon_substitution_pattern
                [MAX_SUBSTITUTION_PATTERNS]; // weapon_indexes
        char
                weapon_substitution_pattern_names[MAX_SUBSTITUTION_PATTERNS]
                                                 [NAME_LENGTH]; // weapon names so that
                                                                // we can generate the
                                                                // indexs after sort
        ...
        gamesnd_id
                hud_tracking_snd; // Sound played when this weapon tracks a target
        gamesnd_id
                hud_locked_snd;       // Sound played when this weapon locked onto a target
        gamesnd_id hud_in_flight_snd; // Sound played while the weapon is in flight
        InFlightSoundType
                in_flight_play_type; // The status when the sound should be played
...
public:
        ...
        inline bool is_locked_homing()
        {
                return wi_flags
                        [Weapon::Info_Flags::Homing_aspect,
                         Weapon::Info_Flags::Homing_javelin];
        }
        inline bool hurts_big_ships()
        {
                return wi_flags
                        [Weapon::Info_Flags::Bomb, Weapon::Info_Flags::Beam,
                         Weapon::Info_Flags::Huge, Weapon::Info_Flags::Big_only];
        }
        inline bool is_interceptable()
        {
                return wi_flags
...
extern int Num_player_weapon_precedence; // Number of weapon types in
                                         // Player_weapon_precedence
extern int
        Player_weapon_precedence[MAX_WEAPON_TYPES]; // Array of weapon types,
...
// Passing a group_id of -1 means it isn't in a group.  See
// weapon_create_group_id for more help on weapon groups.
int weapon_create(
        vec3d *pos, matrix *orient, int weapon_type, int parent_obj,
        int group_id = -1, int is_locked = 0, int is_spawned = 0,
        float fof_cooldown = 0.0f, ship_subsys *src_turret = NULL);
void weapon_set_tracking_info(
        int weapon_objnum, int parent_objnum, int target_objnum,
        int target_is_locked = 0, ship_subsys *target_subsys = NULL);
...
void weapon_hit(
        object *weapon_obj, object *other_obj, vec3d *hitpos, int quadrant = -1,
        vec3d *hitnormal = NULL);
int cmeasure_name_lookup(char *name);
void spawn_child_weapons(object *objp);

// call to detonate a weapon. essentially calls weapon_hit() with other_obj as
// NULL, and sends a packet in multiplayer
void weapon_detonate(object *objp);

void weapon_area_apply_blast(
        vec3d *force_apply_pos, object *ship_objp, vec3d *blast_pos, float blast,
        int make_shockwave);
...
// call to get the "color" of the laser at the given moment (since glowing
// lasers can cycle colors)
...
#endif // FREESPACE2_WEAPON_WEAPON_HH
