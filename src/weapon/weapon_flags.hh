...
#ifndef FREESPACE2_WEAPON_WEAPON_FLAGS_HH
#define FREESPACE2_WEAPON_WEAPON_FLAGS_HH
...
enum class Info_Flags : size_t {
        ...
        Supercap,       // This is a weapon which does supercap class damage (meaning, it
                        // applies real damage to supercap ships)
        Cmeasure,       // Weapon acts as a countermeasure
        Homing_javelin, // WC Saga Javelin HS style heatseeker, locks only on
                        // target's engines
        Turns,          // Set this if the weapon ever changes heading. If you don't set
                        // this and the weapon turns, collision detection won't work, I
                        // promise!
        ...
        Child,          // No ship can have this weapon.  It gets created by weapon
                        // detonations.
        ...
        Bomber_plus,                      // Fire this missile only at a bomber or big ship.  But not a
                                          // fighter.
        ...
        Shudder,                          // causes the weapon to shudder. shudder is proportional to the
                                          // mass and damage of the weapon
        Mflash,                           // has muzzle flash
        Lockarm,                          // if the missile was fired without a lock, it does significanlty
                                          // less damage on impact
        Stream,                           // handled by "trigger down/trigger up" instead of "fire - wait -
                                          // fire - wait"
        ...
        Local_ssm,                        // localized ssm. ship that fires ssm is in mission.  ssms also
                                          // warp back in during mission
        Tagged_only,                      // can only fire if target is tagged
        Cycle,                            // will only fire from (shots (defalts to 1)) points at a time
        Small_only,                       // can only be used against small ships like fighters or
                                          // bombers
        ...
        Training,                         // Weapon does shield/hull damage, but doesn't hurt subsystems,
                                          // whack you, or put marks on your ship.
        Smart_spawn,                      // Spawn weapon that is fired via turrets like normal weapons
        Inherit_parent_target,            // child weapons home in on the target their parent
                                          // is homing on.
        No_emp_kill,                      // though weapon has hitpoints it can not be disabled by EMP
        Variable_lead_homing,             // allows user defined scaler to be added to lead (to
                                          // enable, lead, pure or lag pursuit for missiles)
        Untargeted_heat_seeker,           // forces heat seeker to lose target immeadiately
                                          // (and acquire a random new one)
        Hard_target_bomb,                 // removes the radius doubling effect bombs have for
                                          // collisions
        Non_subsys_homing,                // spreads fired missiles around the target ships hull
        No_life_lost_if_missed,           // prevents game from shortening the lifeleft of
                                          // the missed but still homing missiles
        Custom_seeker_str,                // sets the game to use custom seeker strengths instead
                                          // of default values
        ...
        Ciws,                             // This weapons' burst and shockwave damage can damage bombs
                                          // (Basically, a reverse for TAKES_BLAST/SHOCKWAVE_DAMAGE
        Antisubsysbeam,                   // This beam can target subsystems as per normal
        Nolink,                           // This weapon can not be linked with others
        Use_emp_time_for_capship_turrets, // override MAX_TURRET_DISRUPT_TIME in
                                          // emp.cpp - Goober5000
        No_linked_penalty,                // This weapon does not count into linked firing penalty
        No_homing_speed_ramp,             // Disables the 1s long speed ramping when firing
                                          // locked-on secondaries
        Cmeasure_aspect_home_on,          // This countermeasure flag makes aspect seekers
                                          // home on the countermeasure instead of going
                                          // into dumbfire mode
        Turret_Interceptable,             // These two flags mark a weapon as being
                                          // interceptable by the AI
        Fighter_Interceptable,            // (like WIF_BOMB), without forcing it to be
                                          // tagetable -MageKing17
        Aoe_Electronics,                  // Apply electronics effect across the weapon's entire
                                          // area of effect instead of just on the impacted ship
                                          // -MageKing17
        Apply_Recoil,                     // Apply Recoil using weapon and ship info
        Dont_spawn_if_shot,               // Prevent shot down parent weapons from spawning
                                          // children (DahBlount)
...
enum class Weapon_Flags : size_t {
        Lock_warning_played,      // set when a lock warning sound is played for the
                                  ...
                                  // lock sounds)
        Already_applied_stats,    // for use in ship_apply_local and ship_apply_global
                                  // damage functions so that we don't record multiple
                                  // hits (stats) for one impact
        ...
        Homing_update_needed,     // this is a newly spawned homing weapon which needs
