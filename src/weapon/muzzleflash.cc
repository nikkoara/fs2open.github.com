...
typedef struct mflash_blob_info {
    ...
    mflash_blob_info () : anim_id (-1), offset (0.0), radius (0.0) {
        name[0] = '\0';
    }
...
void parse_mflash_tbl (const char* filename) {
    ...
    try {
        ...
        while (optional_string ("$Mflash:")) {
            ...
            if (optional_string ("+override")) override_mflash = true;
            ...
            if (i == Mflash_info.size ()) { Mflash_info.push_back (mflash); }
...
void mflash_page_in (bool load_all) {
    ...
    for (i = 0; i < Mflash_info.size (); i++) {
        ...
        if (!load_all && !Mflash_info[i].used_this_level) continue;
        ...
        for (idx = 0; idx < Mflash_info[i].blobs.size ();) {
            ...
            mfbip->anim_id =
                bm_load_either (mfbip->name, &num_frames, &fps, NULL, true);
            if (mfbip->anim_id >= 0) {
...
void mflash_level_close () {
    ...
    for (i = 0; i < Mflash_info.size (); i++) {
        // blobs
        for (idx = 0; idx < Mflash_info[i].blobs.size (); idx++) {
            if (Mflash_info[i].blobs[idx].anim_id < 0) continue;
...
void mflash_create (
    ...
    // illegal value
    if ((mflash_type < 0) || (mflash_type >= (int)Mflash_info.size ())) return;
