...
#define BEAM_WARMUP_PCT(b) \
        (((float)Weapon_info[b->weapon_info_index].b_info.beam_warmup - (float)timestamp_until(b->warmup_stamp)) / (float)Weapon_info[b->weapon_info_index].b_info.beam_warmup)

// beam warmdown completion %
#define BEAM_WARMDOWN_PCT(b) \
        (((float)Weapon_info[b->weapon_info_index].b_info.beam_warmdown - (float)timestamp_until(b->warmdown_stamp)) / (float)Weapon_info[b->weapon_info_index].b_info.beam_warmdown)
...
int beam_fire(beam_fire_info *fire_info)
{
        ...
        if (!(fire_info->bfi_flags & BFIF_IS_FIGHTER_BEAM)) {
                if (((fire_info->target == NULL) && !(fire_info->bfi_flags & BFIF_TARGETING_COORDS)) || ((fire_info->target != NULL) && (fire_info->target->type != OBJ_SHIP) && (fire_info->target->type != OBJ_ASTEROID) && (fire_info->target->type != OBJ_DEBRIS) && (fire_info->target->type != OBJ_WEAPON))) {
                        return -1;
                }
        }
...
int beam_fire_targeting(fighter_beam_fire_info *fire_info)
{
        ...
        // make sure the beam_info_index is valid
        ASSERT(
                (fire_info->beam_info_index >= 0) && (fire_info->beam_info_index < MAX_WEAPON_TYPES) && (Weapon_info[fire_info->beam_info_index].wi_flags[Weapon::Info_Flags::Beam]));
        ...
        // make sure a ship is firing this
        ASSERT(
                (fire_info->shooter->type == OBJ_SHIP) && (fire_info->shooter->instance >= 0) && (fire_info->shooter->instance < MAX_SHIPS));
...
int beam_get_weapon_info_index(object *bm)
{
        ...
        ASSERT(
                (Beams[bm->instance].weapon_info_index > -1) && (Beams[bm->instance].weapon_info_index < Num_weapon_types));
...
#define BEAM_T(b) \
        (((b->binfo.delta_ang / b->life_total) * (b->life_total - b->life_left)) / b->binfo.delta_ang)
void beam_type_b_move(beam *b)
{
        vec3d actual_dir;
        vec3d temp, temp2;
...
void beam_move_all_post()
{
        ...
        while (moveup != END_OF_LIST(&Beam_used_list)) {
                ...
                {
                        if ((moveup->life_left <= 0.0f) && (moveup->warmdown_stamp == -1) && (moveup->framecount > 1)) {
                                beam_start_warmdown(moveup);
...
void beam_render(beam *b, float u_offset)
{
        ...
        for (s_idx = 0; s_idx < bwi->beam_num_sections; s_idx++) {
                ...
                if (bwsi->tile_type == 1)
                        u_scale = length / (bwsi->width * 0.5f) / bwsi->tile_factor; // beam tileing, might make a tileing
...
void beam_generate_muzzle_particles(beam *b)
{
        ...
        particle_count = (int)fs2::prng::randf(0, 0.0f, (float)wip->b_info.beam_particle_count);
        ...
        for (idx = 0; idx < particle_count; idx++) {
                ...
                float p_vel = (wip->b_info.beam_muzzle_radius / p_life) * fs2::prng::randf(0, 0.85f, 1.2f);
...
void beam_add_light_small(beam *bm, object *objp, vec3d *pt_override = NULL)
{
        ...
        if ((bm->warmup_stamp < 0) && (bm->warmdown_stamp < 0)) // disable noise when warming up or down
                noise = fs2::prng::randf(0, 1.0f - bwi->sections[0].flicker, 1.0f + bwi->sections[0].flicker);
        else
                noise = 1.0f;
...
void beam_delete(beam *b)
{
        ...
        if ((b->subsys) && (b->subsys->turret_animation_position == MA_POS_READY)) {
...
void beam_get_binfo(beam *b, float accuracy, int num_shots)
{
        ...
        case BEAM_TYPE_A:
                ...
                b->binfo.shot_aim[0] = fs2::prng::randf(0, 0.0f, 1.0f + miss_factor * accuracy);
...
int beam_collide_ship(obj_pair *pair)
{
        ...
        if (b->range < BEAM_FAR_LENGTH && (shield_collision || hull_enter_collision || hull_exit_collision)) {
        ...
        if (!(ship_objp->flags[Object::Object_Flags::No_shields])) {
                ...
                if (shield_collision)
                        ...
                        hull_enter_collision && (sip->flags[Ship::Info_Flags::Surface_shields]))
...
void beam_handle_collisions(beam *b)
{
        ...
        for (idx = 0; idx < b->f_collision_count; idx++) {
                ...
                if (draw_effects && ((wi->piercing_impact_effect.isValid()) || (wi->flash_impact_weapon_expl_effect.isValid()))) {
                                ...
                                if (!IS_VEC_NULL(&fvec)) {
                                        ...
                                        if (beam_will_tool_target(b, &Objects[target])) {
                                                ...
                                                if (Objects[target].type == OBJ_SHIP) {
                                                        ...
                                                        if (shipp->armor_type_idx != -1) {
                                                                piercing_type = Armor_types[shipp->armor_type_idx]
                                        ...
                                        if (ok_to_draw) {
                                                ...
                                                if (widest <= Objects[target].radius * BEAM_AREA_PERCENT) {
                                                        auto particleSource = particle::ParticleManager::get()
                                                                                      ->createSource(
...
void beam_apply_whack(beam *b, object *objp, vec3d *hit_point)
{
        ...
        wip = &Weapon_info[b->weapon_info_index];
        ASSERT(
                (objp != NULL) && (objp->type == OBJ_SHIP) && (objp->instance >= 0) && (objp->instance < MAX_SHIPS));
