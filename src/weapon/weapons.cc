...
missile_obj Missile_objs[MAX_MISSILE_OBJS]; // array used to store missile
                                            // object indexes
missile_obj Missile_obj_list; // head of linked list of missile_obj structs

// WEAPON SUBTYPE STUFF
const char* Weapon_subtype_names[] = { "Laser", "Missile", "Beam" };
int Num_weapon_subtypes =
    sizeof (Weapon_subtype_names) / sizeof (Weapon_subtype_names[0]);
...
const size_t Num_burst_fire_flags =
    sizeof (Burst_fire_flags) /
    sizeof (flag_def_list_new< Weapon::Burst_Flags >);
...
flag_def_list_new< Weapon::Info_Flags > Weapon_Info_Flags[] = {
    ...
    { "don't spawn if shot", Weapon::Info_Flags::Dont_spawn_if_shot, true,
      false },
    { "die on lost lock", Weapon::Info_Flags::Die_on_lost_lock, true,
      true }, // special case
};

const size_t num_weapon_info_flags =
    sizeof (Weapon_Info_Flags) /
...
int Num_player_weapon_precedence;               // Number of weapon types in
                                                // Player_weapon_precedence
int Player_weapon_precedence[MAX_WEAPON_TYPES]; // Array of weapon types,
                                                // precedence list for player
                                                // weapon selection

// Used to avoid playing too many impact sounds in too short a time interval.
// This will elimate the odd "stereo" effect that occurs when two weapons
// impact at nearly the same time, like from a double laser (also saves sound
// channels!)
...
// homing missiles have an extended lifetime so they don't appear to run out of
// gas before they can hit a moving target at extreme range. Check the comment
// in weapon_set_tracking_info() for more details
...
int weapon_explosions::Load (char* filename, int expected_lods) {
    ...
    if (new_wei.lod[0].bitmap_id < 0) {
        WARNINGF (LOCATION, "Weapon explosion '%s' does not have an LOD0 anim!",filename);

        // if we don't have the first then it's only safe to assume that the
        // rest are missing or not usable
        return -1;
    }
    ...
    if ((MAX_FILENAME_LEN - strlen (filename)) > 6) {
        ...
        if (new_wei.lod_count != expected_lods)
            WARNINGF (LOCATION, "For '%s', %i of %i LODs are missing!", filename,expected_lods - new_wei.lod_count, expected_lods);
    }
    else {
        WARNINGF (LOCATION, "Filename '%s' is too long to have any LODs.", filename);
    }
...
void weapon_explosions::PageIn (int idx) {
    ...
    if ((idx < 0) || (idx >= (int)ExplosionInfo.size ())) return;
    ...
    for (i = 0; i < wei->lod_count; i++) {
        if (wei->lod[i].bitmap_id >= 0) {
            bm_page_in_xparent_texture (
                wei->lod[i].bitmap_id, wei->lod[i].num_frames);
        }
    }
}

int weapon_explosions::GetAnim (
    int weapon_expl_index, vec3d* pos, float size) {
    if ((weapon_expl_index < 0) ||
        (weapon_expl_index >= (int)ExplosionInfo.size ()))
        return -1;

    // Get our weapon expl for the day
    weapon_expl_info* wei = &ExplosionInfo[weapon_expl_index];

    if (wei->lod_count == 1) return wei->lod[0].bitmap_id;
    ...
    if (v.codes & CC_BEHIND) {
        ...
        if (v.codes & CC_BEHIND) { behind = 0; }
    }

    if (!g3_get_bitmap_dims (
            wei->lod[0].bitmap_id, &v, size, &x, &y, &w, &h, &bm_size)) {
        if (Detail.hardware_textures == 4) {
            // straight LOD
            if (w <= bm_size / 8) { best_lod = 3; }
            else if (w <= bm_size / 2) {
                best_lod = 2;
            }
...
void parse_weapon_expl_tbl (const char* filename) {
    ...
    try {
        ...
        while (required_string_either ("#End", "$Name:")) {
            ...
            if (optional_string ("$LOD:")) { stuff_int (&lod_check.num_lods); }

            // only bother with this if we have 1 or more lods and less than
            // max lods, otherwise the stardard level loading will take care of
            // the different effects
            if ((lod_check.num_lods > 0) &&
...
void missile_obj_list_init () {
    ...
    for (i = 0; i < MAX_MISSILE_OBJS; i++) { Missile_objs[i].flags = 0; }
}
    ...
    for (objp = GET_FIRST (&obj_used_list);
        ...
        if (objp->type == OBJ_WEAPON &&
            Weapon_info[Weapons[objp->instance].weapon_info_index].subtype ==
                WP_MISSILE) {
            Weapons[objp->instance].missile_list_index =
                missile_obj_list_add (OBJ_INDEX (objp));
        }
    }
}
...
void parse_wi_flags (
    weapon_info* weaponp, flagset< Weapon::Info_Flags > wi_flags) {
    const char* spawn_str = NOX ("Spawn");
    const size_t spawn_str_len = strlen (spawn_str);

    // Make sure we HAVE flags :p
    if (!optional_string ("$Flags:")) return;

    // To make sure +override doesn't overwrite previously parsed values we
    // parse the flags into a separate flagset
    std::vector< std::string > unparsed_or_special;
    flagset< Weapon::Info_Flags > parsed_flags;
    parse_string_flag_list (
        parsed_flags, Weapon_Info_Flags, num_weapon_info_flags,
        &unparsed_or_special);
    ...
    for (auto flag = unparsed_or_special.begin ();
        ...
        if (!strncasecmp (spawn_str, flag_text.c_str (), 5)) {
            if (weaponp->num_spawn_weapons_defined <
                ...
                std::unique_ptr< char[] > temp_string (
                    new char[flag_text.size () + 1]);

                strcpy (temp_string.get (), flag_text.c_str ());

                weaponp->wi_flags.set (Weapon::Info_Flags::Spawn);
                weaponp->spawn_info[weaponp->num_spawn_weapons_defined]
                    .spawn_type = (short)Num_spawn_types;
                skip_length =
                    spawn_str_len +
                    strspn (&temp_string[spawn_str_len], NOX (" \t"));
                char* num_start = strchr (&temp_string[skip_length], ',');
                if (num_start == NULL) {
                    weaponp->spawn_info[weaponp->num_spawn_weapons_defined]
                        .spawn_count = DEFAULT_WEAPON_SPAWN_COUNT;
                    name_length = 999;
                }
                else {
                    weaponp->spawn_info[weaponp->num_spawn_weapons_defined]
                        .spawn_count = (short)atoi (num_start + 1);
                    name_length = num_start - temp_string.get () - skip_length;
                }

                weaponp->total_children_spawned +=
                    weaponp->spawn_info[weaponp->num_spawn_weapons_defined]
                        .spawn_count;
                ...
                WARNINGF (LOCATION,"Illegal to have more than %d spawn types for one weapon.\nIgnoring weapon %s",MAX_SPAWN_TYPES_PER_WEAPON, weaponp->name);
            }
        }
            ...
            WARNINGF (LOCATION,"The \"beam no whack\" flag has been deprecated.  Set the beam's mass to 0 instead.  This has been done for you.");
            weaponp->mass = 0.0f;
        }
            ...
            if (!(weaponp->is_locked_homing ())) {
                WARNINGF (LOCATION,"\"die on lost lock\" may only be used for Homing Type ASPECT/JAVELIN!");
                weaponp->wi_flags.remove (
                    Weapon::Info_Flags::Die_on_lost_lock);
            }
        }
        else {
            WARNINGF (LOCATION, "Unrecognized flag in flag list for weapon %s : %s", weaponp->name, (*flag).c_str ());
        }
    }
    ...
    if (weaponp->wi_flags[Weapon::Info_Flags::Hard_target_bomb] &&
        ...
        WARNINGF (LOCATION,"Weapon %s is not a bomb but has \"no radius doubling\" set. Ignoring this flag",weaponp->name);
    }
    ...
    if (weaponp->wi_flags[Weapon::Info_Flags::Flak]) {
        if (weaponp->wi_flags[Weapon::Info_Flags::Swarm] ||
            ...
            WARNINGF (LOCATION,"Swarm, Corkscrew, and Flak are mutually exclusive!  Removing Swarm and Corkscrew attributes from weapon %s.",weaponp->name);
        }
    }
    ...
    if (!weaponp->wi_flags[Weapon::Info_Flags::Cmeasure] &&
        ...
        WARNINGF (LOCATION,"Weapon %s has the \"pulls aspect seekers\" flag, but is not a countermeasure.",weaponp->name);
    }
}
...
void parse_shockwave_info (shockwave_create_info* sci, const char* pre_char) {
    ...
    if (optional_string (buf)) { stuff_float (&sci->damage); }
    ...
    if (optional_string (buf)) { stuff_float (&sci->blast); }

    sprintf (buf, "%sInner Radius:", pre_char);
    if (optional_string (buf)) { stuff_float (&sci->inner_rad); }

    sprintf (buf, "%sOuter Radius:", pre_char);
    if (optional_string (buf)) { stuff_float (&sci->outer_rad); }

    if (sci->outer_rad < sci->inner_rad) {
        WARNINGF (LOCATION,"Shockwave outer radius must be greater than or equal to the inner radius!");
        sci->outer_rad = sci->inner_rad;
    }

    sprintf (buf, "%sShockwave Speed:", pre_char);
    if (optional_string (buf)) { stuff_float (&sci->speed); }
    ...
    if (optional_string (buf)) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            while (angs[i] < 0) { angs[i] += PI2; }
            while (angs[i] > PI2) { angs[i] -= PI2; }
        }
...
int parse_weapon (int subtype, bool replace, const char* filename) {
    ...
    II << "weapon name : " << fname;

    if (optional_string ("+nocreate")) {
        if (!replace) {
            WARNINGF (LOCATION,"+nocreate flag used for weapon in non-modular table");
        }
    ...
    if (optional_string ("$Subtype:")) {
        ...
        if (!strcasecmp ("Primary", fname)) { wip->subtype = WP_LASER; }
            ...
            WARNINGF (LOCATION, "Unknown subtype on weapon '%s'", wip->name);
        }
    }
    else if (wip->subtype != WP_UNUSED && !first_time) {
        if (wip->subtype != subtype) {
            WARNINGF (LOCATION,"Type of weapon %s entry does not agree with original entry type.",wip->name);
        }
    }
    else {
        wip->subtype = subtype;
    ...
    if (optional_string ("+Description:")) {
        ...
        while (current_line != nullptr) {
            ...
            if (line_end == nullptr) { line_length = strlen (current_line); }
        ...
        if (num_lines >= WEAPON_DESC_MAX_LINES) {
            error_display (
                0, "Weapon description has too many lines. Maximum is %d.",
                WEAPON_DESC_MAX_LINES);
        }
    }
    ...
    // Weapon fadein effect, used when no ani is specified or weapon_select_3d
    // is active
    wip->selection_effect =
        Default_weapon_select_effect; // By default, use the FS2 effect
    ...
    if (optional_string ("$Model file:")) {
        ...
        II << "model pof file : " << wip->pofbitmap_name;
    }

    // a special LOD level to use when rendering the weapon in the hud
    // targetbox
    if (optional_string ("$POF target LOD:")) stuff_int (&wip->hud_target_lod);

    if (optional_string ("$Detail distance:")) {
        wip->num_detail_levels = stuff_int_list (
            wip->detail_distance, MAX_MODEL_DETAIL_LEVELS, RAW_INTEGER_TYPE);
    }
    ...
    if (optional_string ("@Laser Bitmap:")) {
        ...
        if (wip->render_type == WRT_POF) {
            WARNINGF (LOCATION," Weapon '%s' has both LASER and POF render types!  Will only use POF type!",wip->name);
    ...
    if (optional_string ("@Laser Color:")) {
        // This might be confusing at first glance. If we're a modular table
        // (!first_time), AND we're providing a new color for the laser (being
        // in this block at all), AND the RGB values for laser_color_1 and
        // laser_color_2 match... THEN we conclude that laser_color_2 wasn't
        // explicitly defined before, and the modder would probably prefer it
        // if the laser didn't suddenly start changing colors from the new to
        // the old over its lifespan. -MageKing17
        bool reset =
            (!first_time &&
        ...
        if (reset) {
            gr_init_color (
                &wip->laser_color_2, wip->laser_color_1.red,
                wip->laser_color_1.green, wip->laser_color_1.blue);
        }
    }
    ...
    if (optional_string ("$Mass:")) {
        ...
        // Goober5000 - hack in order to make the beam whack behavior of these
        // three beams match all other beams this relies on Bobboau's beam
        // whack hack in beam_apply_whack()
        if ((!strcmp (wip->name, "SAAA") && (wip->mass == 4.0f)) ||
            (!strcmp (wip->name, "MjolnirBeam") && (wip->mass == 1000.0f)) ||
            (!strcmp (wip->name, "MjolnirBeam#home") &&
             (wip->mass == 1000.0f))) {
            wip->mass = 100.0f;
        }

        II << "weapon mass : " << wip->mass;
    }

    if (optional_string ("$Velocity:")) {
        stuff_float (&(wip->max_speed));
        II << "weapon mass : " << wip->max_speed;
    }

    if (optional_string ("$Fire Wait:")) {
        stuff_float (&(wip->fire_wait));
        II << "weapon fire wait : " << wip->fire_wait;
        // Min and max delay stuff for weapon fire wait randomization
        if (optional_string ("+Max Delay:")) {
            stuff_float (&(wip->max_delay));
            II << "weapon fire max delay -- %7.3f" << wip->max_delay;
        }
        if (optional_string ("+Min Delay:")) {
            stuff_float (&(wip->min_delay));
            II << "weapon fire min delay -- %7.3f" << wip->min_delay;
        }
    }
    ...
    if (optional_string ("$Damage:")) {
        ...
        if (first_time) { wip->shockwave.damage = wip->damage; }
    }
    ...
    if (optional_string ("$Damage Time:")) {
        ...
        if (optional_string ("+Attenuation Damage:")) {
            ...
            WARNINGF (LOCATION,"+Min Damage: and +Max Damage: in %s are deprecated, please change to +Attenuation Damage:.",wip->name);
            stuff_float (&wip->atten_damage);
        }
    }
    ...
    if (optional_string ("$Arm distance:")) { stuff_float (&wip->arm_dist); }

    if (optional_string ("$Arm radius:")) { stuff_float (&wip->arm_radius); }
    ...
    if (optional_string ("$Lifetime Min:")) {
        ...
        if (wip->life_min < 0.0f) {
            wip->life_min = 0.0f;
            WARNINGF (LOCATION,"Lifetime min for weapon '%s' cannot be less than 0. Setting to 0.",wip->name);
        }
    }
    ...
    if (optional_string ("$Lifetime Max:")) {
        ...
        if (wip->life_max < 0.0f) {
            wip->life_max = 0.0f;
            WARNINGF (LOCATION,"Lifetime max for weapon '%s' cannot be less than 0. Setting to 0.",wip->name);
        }
        else if (wip->life_max < wip->life_min) {
            wip->life_max = wip->life_min + 0.1f;
            WARNINGF (LOCATION,"Lifetime max for weapon '%s' cannot be less than its Lifetime Min (%f) value. Setting to %f.",wip->name, wip->life_min, wip->life_max);
        }
        else {
    ...
    if (wip->life_min >= 0.0f && wip->life_max < 0.0f) {
        ...
        wip->life_min = -1.0f;
        WARNINGF (LOCATION,"Lifetime min, but not lifetime max, specified for weapon %s. Assuming static lifetime of %.2f seconds.",wip->name, wip->lifetime);
    }

    if (optional_string ("$Lifetime:")) {
        if (wip->life_min >= 0.0f || wip->life_max >= 0.0f) {
            WARNINGF (LOCATION,"Lifetime min or max specified, but $Lifetime was also specified; min or max will be used.");
        }
        stuff_float (&wip->lifetime);
        if (wip->damage_time > wip->lifetime) {
            WARNINGF (LOCATION,"Lifetime is lower than Damage Time, setting Damage Time to be one half the value of Lifetime.");
    ...
    // Goober5000: cargo size is checked for div-0 errors... see below (must
    // parse flags first)
    if (optional_string ("$Cargo Size:")) { stuff_float (&wip->cargo_size); }

    bool is_homing = false;
    if (optional_string ("$Homing:")) { stuff_boolean (&is_homing); }

    if (is_homing || (wip->is_homing ())) {
        char temp_type[NAME_LENGTH];

        // the following five items only need to be recorded if the weapon is a
        // homing weapon
        ...
        if (wip->wi_flags[Weapon::Info_Flags::Homing_heat]) {
            ...
            if (optional_string ("+Seeker Strength:")) {
                ...
                if (wip->seeker_strength <= 0) {
                    WARNINGF (LOCATION,"Seeker Strength for missile \'%s\' must be greater than zero\nReseting value to default.",wip->name);
            ...
            if (optional_string (
                    "+Min Lock Time:")) { // minimum time (in seconds) to
                                          // achieve lock
                stuff_float (&wip->min_lock_time);
            }

            if (optional_string (
                    "+Lock Pixels/Sec:")) { // pixels/sec moved while locking
                stuff_int (&wip->lock_pixels_per_sec);
            }

            if (optional_string (
                    "+Catch-up Pixels/Sec:")) { // pixels/sec moved while
                                                // catching-up for a lock
                stuff_int (&wip->catchup_pixels_per_sec);
            }

            if (optional_string ("+Catch-up Penalty:")) {
                // number of extra pixels to move while locking as a penalty
                // for catching up for a lock
            ...
            if (wip->is_locked_homing ()) {
                // locked homing missiles have a much longer lifespan than the
                // AI think they do
                wip->max_lifetime =
                    wip->lifetime * LOCKED_HOMING_EXTENDED_LIFE_FACTOR;
            }
        }
        else {
    ...
    if ((wip->wi_flags[Weapon::Info_Flags::Swarm]) &&
        optional_string ("+SwarmWait:")) {
        float SwarmWait;
        stuff_float (&SwarmWait);
        if (SwarmWait > 0.0f &&
            SwarmWait * wip->swarm_count < wip->fire_wait) {
    ...
    if (optional_string ("$Free Flight Speed:")) {
        float temp;
        stuff_float (&temp);
        WARNINGF (LOCATION, "Ignoring free flight speed for weapon '%s'",wip->name);
    }
    ...
    if (optional_string ("+Inflight sound type:")) {
        ...
        if (!strcasecmp (type.c_str (), "TARGETED")) {
            ...
            WARNINGF (LOCATION, "Unknown in-flight sound type \"%s\"!",type.c_str ());
            wip->in_flight_play_type = ALWAYS;
        }
    }
    ...
    if (optional_string ("$Rearm Rate:")) {
        if (subtype != WP_MISSILE) { primary_rearm_rate_specified = 1; }

        stuff_float (&rearm_rate);
        if (rearm_rate > 0.0f) { wip->rearm_rate = 1.0f / rearm_rate; }
        else {
            WARNINGF (LOCATION,"Rearm wait of less than 0 on weapon %s; setting to 1",wip->name);
        }
    }
    ...
    if (optional_string ("+Weapon Min Range:")) {
        ...
        if (MinRange > 0.0f &&
            ...
            WARNINGF (LOCATION, "Invalid minimum range on weapon %s; setting to 0",wip->name);
        }
    }
    ...
    if (subtype == WP_LASER) {
        ...
        if (wip->wi_flags[Weapon::Info_Flags::Ballistic]) {
            ...
            if (!primary_rearm_rate_specified && first_time) {
                WARNINGF (LOCATION,"$Rearm Rate for ballistic primary %s not specified.  Defaulting to 100...",wip->name);
        ...
        // not ballistic
            ...
            if (primary_rearm_rate_specified) {
                WARNINGF (LOCATION,"$Rearm Rate specified for non-ballistic primary %s",wip->name);
            }
        }
    }
    // secondary
        ...
        // ballistic
        if (wip->wi_flags[Weapon::Info_Flags::Ballistic]) {
            WARNINGF (LOCATION,"Secondary weapon %s can't be ballistic.  Removing this flag...",wip->name);
    ...
    if (wip->wi_flags[Weapon::Info_Flags::Emp]) {
        if (!wip->shockwave.outer_rad) {
            WARNINGF (LOCATION,"Outer blast radius of weapon %s is zero - EMP will not work.\nAdd $Outer Radius to weapon table entry.",wip->name);
        }
    }

    // also make sure secondaries and ballistic primaries do not have 0 cargo
    // size
    if (subtype == WP_MISSILE ||
        wip->wi_flags[Weapon::Info_Flags::Ballistic]) {
        if (wip->cargo_size == 0.0f) {
            WARNINGF (LOCATION,"Cargo size of weapon %s cannot be 0.  Setting to 1.",wip->name);
            wip->cargo_size = 1.0f;
        }
    }
    ...
    if (optional_string ("$Trail:")) {
        ...
        wip->wi_flags.set (
            Weapon::Info_Flags::Trail); // missile leaves a trail

        if (optional_string ("+Start Width:")) stuff_float (&ti->w_start);

        if (optional_string ("+End Width:")) stuff_float (&ti->w_end);

        if (optional_string ("+Start Alpha:")) stuff_float (&ti->a_start);

        if (optional_string ("+End Alpha:")) stuff_float (&ti->a_end);
    ...
    if (optional_string ("$Impact Effect:")) {
        wip->impact_weapon_expl_effect =
            particle::util::parseEffect (wip->name);
    }
    else {
        ...
        if (optional_string ("$Impact Explosion:")) {
            ...
            if (VALID_FNAME (fname)) {
                ...
                if (bitmapIndex < 0) {
                    WARNINGF (LOCATION, "Couldn't load effect '%s' for weapon '%s'.",fname, wip->name);
                }
            }
        }

        if (optional_string ("$Impact Explosion Radius:")) stuff_float (&size);
        ...
        if (bitmapIndex >= 0 && size > 0.0f) {
            ...
            if (subtype == WP_BEAM) {
                // The original formula is (1.2f + 0.007f * (float)(rand() %
                // 100)) which generates values within [1.2, 1.9)
                auto singleEffect =
                    effects::SingleParticleEffect::createInstance (
                        bitmapIndex, size * 1.2f, size * 1.9f);
                wip->impact_weapon_expl_effect =
                    ParticleManager::get ()->addEffect (singleEffect);
            }
            else {
                auto singleEffect =
                    effects::SingleParticleEffect::createInstance (
                        bitmapIndex, size, size);
                wip->impact_weapon_expl_effect =
                    ParticleManager::get ()->addEffect (singleEffect);
            }
        }
    ...
    if (optional_string ("$Shield Impact Explosion Radius:")) {
        ...
        if (wip->impact_weapon_expl_effect.isValid ()) {
            auto singleEffect =
                dynamic_cast< effects::SingleParticleEffect* > (
                    ParticleManager::get ()->getEffect (
                        wip->impact_weapon_expl_effect));

            if (singleEffect) {
                // Initialize with value of the previously created single
                // particle effect
                wip->shield_impact_explosion_radius =
                    singleEffect->getProperties ().m_radius.next ();
            }
        }
    }

    if (optional_string ("$Dinky Impact Effect:")) {
        wip->dinky_impact_weapon_expl_effect =
            particle::util::parseEffect (wip->name);
    }
    else {
        // This is for compatibility with old tables
        ...
        // Slight variation from original, as the original size is not known
        // anymore the whole effect is copied If the radius was stull specified
        // it needs to be parsed to not mess up something else
        if (optional_string ("$Dinky Impact Explosion Radius:")) {
            float temp;
            stuff_float (&temp);
        }
    ...
    if (optional_string ("$Piercing Impact Effect:")) {
        ...
        if (first_time) {
            ...
            wip->piercing_impact_secondary_effect =
        ...
        if (optional_string ("$Piercing Impact Explosion:")) {
            ...
            if (effectIndex < 0) {
                WARNINGF (LOCATION, "Failed to load effect '%s' for weapon %s!",fname, wip->name);
            }
        }
        ...
        if (optional_string ("$Piercing Impact Life:")) stuff_float (&life);
        ...
        if (effectIndex >= 0 && radius != 0.0f) {
            ...
            wip->piercing_impact_effect =
                ParticleManager::get ()->addEffect (emitterEffect);
            ...
            if (back_velocity != 0.0f) {
                ...
                wip->piercing_impact_secondary_effect =
                    ParticleManager::get ()->addEffect (secondaryEffect);
            }
        }
    }
    ...
    if (optional_string ("$Inflight Effect:")) {
        ...
        wip->state_effects.insert (
            std::make_pair (WeaponState::NORMAL, effetIdx));
    }
    ...
    if (wip->subtype == WP_MISSILE) {
        if (optional_string ("$Freeflight Effect:")) {
            auto effetIdx = particle::util::parseEffect (wip->name);
            wip->state_effects.insert (
                std::make_pair (WeaponState::FREEFLIGHT, effetIdx));
        }
        if (optional_string ("$Ignition Effect:")) {
            auto effetIdx = particle::util::parseEffect (wip->name);
            wip->state_effects.insert (
                std::make_pair (WeaponState::IGNITION, effetIdx));
        }
        if (optional_string ("$Homed Flight Effect:")) {
            auto effetIdx = particle::util::parseEffect (wip->name);
            wip->state_effects.insert (
                std::make_pair (WeaponState::HOMED_FLIGHT, effetIdx));
        }
        if (optional_string ("$Unhomed Flight Effect:")) {
            auto effetIdx = particle::util::parseEffect (wip->name);
            wip->state_effects.insert (
                std::make_pair (WeaponState::UNHOMED_FLIGHT, effetIdx));
        }
    }
    ...
    if (optional_string ("$EMP Time:")) { stuff_float (&wip->emp_time); }

    // This is an optional modifier for a weapon that uses the "apply recoil"
    // flag. recoil_force in ship.cpp line 10445 is multiplied by this if
    // defined.
    if (optional_string ("$Recoil Modifier:")) {
        if (!(wip->wi_flags[Weapon::Info_Flags::Apply_Recoil])) {
            WARNINGF (LOCATION,"$Recoil Modifier specified for weapon %s but this weapon does not have the \"apply recoil\" weapon flag set. Automatically setting the flag",wip->name);
            wip->wi_flags.set (Weapon::Info_Flags::Apply_Recoil);
        }
        stuff_float (&wip->recoil_modifier);
    }

    // Energy suck optional stuff (if WIF_ENERGY_SUCK is not set, none of this
    // matters anyway)
    ...
    if (optional_string ("$Corkscrew:")) {
        ...
        if (optional_string ("+Radius:")) { stuff_float (&wip->cs_radius); }

        if (optional_string ("+Fire Delay:")) { stuff_int (&wip->cs_delay); }

        if (optional_string ("+Counter rotate:")) {
            stuff_boolean (&wip->cs_crotate);
        }

        if (optional_string ("+Twist:")) { stuff_float (&wip->cs_twist); }
    }
    ...
    if (optional_string ("$Electronics:")) {
        if (optional_string ("+New Style:")) { wip->elec_use_new_style = 1; }
        ...
        if (optional_string ("+Intensity:")) {
            float temp;
            stuff_float (&temp);
            WARNINGF (LOCATION, "+Intensity is deprecated");
        }
        ...
        if (optional_string ("+Engine Multiplier:")) {
            ...
            if (!wip->elec_use_new_style)
                WARNINGF (LOCATION,"+Engine multiplier may only be used with new style electronics");
        }
        ...
        if (optional_string ("+Weapon Multiplier:")) {
            ...
            if (!wip->elec_use_new_style)
                WARNINGF (LOCATION,"+Weapon multiplier may only be used with new style electronics");
        }
        ...
        if (optional_string ("+Beam Turret Multiplier:")) {
            ...
            if (!wip->elec_use_new_style)
                WARNINGF (LOCATION,"+Beam turret multiplier may only be used with new style electronics");
        }
        ...
        if (optional_string ("+Sensors Multiplier:")) {
            ...
            if (!wip->elec_use_new_style)
                WARNINGF (LOCATION,"+Sensors multiplier may only be used with new style electronics");
        }
    ...
    if (optional_string ("$Countermeasure:")) {
        if (!(wip->wi_flags[Weapon::Info_Flags::Cmeasure])) {
            WARNINGF (LOCATION,"Weapon \'%s\' has countermeasure information defined, but the \"countermeasure\" flag wasn\'t found in the \'$Flags:\' field.",wip->name);
        }
    ...
    if (optional_string ("$BeamInfo:")) {
        // beam type
        if (optional_string ("+Type:")) { stuff_int (&wip->b_info.beam_type); }
        ...
        if (optional_string ("+Miss Factor:")) {
            for (idx = 0; idx < NUM_SKILL_LEVELS; idx++) {
                float temp;
                if (!stuff_float_optional (&temp)) { break; }
        ...
        for (iff = 0; iff < Num_iffs; iff++) {
            ...
            sprintf (
                miss_factor_string,
                "+%s Miss Factor:", Iff_info[iff].iff_name);
            if (optional_string (miss_factor_string)) {
                // this Miss Factor applies only to the specified IFF
                for (idx = 0; idx < NUM_SKILL_LEVELS; idx++) {
                    if (!stuff_float_optional (
                            &wip->b_info.beam_iff_miss_factor[iff][idx])) {
                        break;
                    }
                }
            }
        ...
        // make sure that we have at least one shot so that TYPE_D beams will
        // work
        if ((wip->b_info.beam_type == BEAM_TYPE_D) &&
            (wip->b_info.beam_shots < 1)) {
            WARNINGF (LOCATION,"Type D beam weapon, '%s', has less than one \"+Shots\" specified!  It must be set to at least 1!!",wip->name);
            wip->b_info.beam_shots = 1;
        }
        ...
        if (optional_string ("+Range:")) { stuff_float (&wip->b_info.range); }
        ...
        if (optional_string ("+Beam Flash Particle Effect:")) {
            wip->flash_impact_weapon_expl_effect =
                particle::util::parseEffect (wip->name);
        }
        else {
            // This is for compatibility with old tables
            ...
            if (optional_string ("+Beam Flash Radius:")) stuff_float (&size);

            if (bitmapIndex >= 0 && size > 0.0f) {
                using namespace particle;

                if (defaultEffect) {
                    // 'size * 1.5f * 0.005f' is another weird thing, the
                    // original code scales the lifetime of the flash particles
                    // based on size so the new effects have to simulate that,
                    // but that onyl applies to the default effect, not a
                    // custom effect seriously, who though that would be a good
                    // idea?
                    auto singleEffect =
                        effects::SingleParticleEffect::createInstance (
                            bitmapIndex, size * 1.2f, size * 1.9f,
        ...
        if (optional_string ("+Beam Piercing Particle Effect:")) {
            ...
            if (bitmapIndex >= 0 && radius > 0.0f) {
                ...
                piercingEffect->setValues (
                    bitmapIndex, radius, velocity, back_velocity, variance);

                wip->piercing_impact_effect =
                    ParticleManager::get ()->addEffect (piercingEffect);
            }
        }
        ...
        while (optional_string ("$Section:")) {
            ...
            if (optional_string ("+Index:")) {
                ...
                if ((bsw_index_override < 0) ||
                    ...
                    WARNINGF (LOCATION,"Invalid +Index value of %d specified for beam section on weapon '%s'; valid values at this point are %d to %d.",bsw_index_override, wip->name, 0,wip->b_info.beam_num_sections - 1);
            }
            ...
            if (bsw_index_override >= 0) {
                if (bsw_index_override < wip->b_info.beam_num_sections) {
                    ...
                    if (!nocreate) {
                        if ((bsw_index_override ==
                             wip->b_info.beam_num_sections) &&
                            (bsw_index_override < MAX_BEAM_SECTIONS)) {
                            bsip = &wip->b_info.sections
                                        [wip->b_info.beam_num_sections++];
                        }
                        else {
                            if (!remove)
                                WARNINGF (LOCATION,"Invalid index for manually-indexed beam section %d (max %d) on weapon %s.",bsw_index_override, MAX_BEAM_SECTIONS,wip->name);

                            bsip = &tbsw;
                            memset (
                                bsip, 0, sizeof (beam_weapon_section_info));
                            generic_anim_init (&bsip->texture, NULL);
                        }
                    }
                    else {
                        if (!remove)
                            WARNINGF (LOCATION,"Invalid index for manually-indexed beam section %d, and +nocreate specified, on weapon %s",bsw_index_override, wip->name);
                ...
                if (wip->b_info.beam_num_sections < MAX_BEAM_SECTIONS) {
                    bsip =
                        &wip->b_info.sections[wip->b_info.beam_num_sections++];
                    generic_anim_init (&bsip->texture, NULL);
                }
                else {
                    WARNINGF (LOCATION,"Too many beam sections for weapon %s - max is %d",wip->name, MAX_BEAM_SECTIONS);
                    bsip = &tbsw;
                    memset (bsip, 0, sizeof (beam_weapon_section_info));
                    generic_anim_init (&bsip->texture, NULL);
                }
            ...
            // section width
            if (optional_string ("+Width:")) stuff_float (&bsip->width);
            ...
            if (optional_string ("+Flicker:")) {
                ...
                if (bsip->flicker < 0.0f || bsip->flicker > 1.0f) {
                    WARNINGF (LOCATION,"Invalid value found for +Flicker on section %d of beam %s. Valid range is 0.0 to 1.0, values will be adjusted.",wip->b_info.beam_num_sections, wip->name);
                    CLAMP (bsip->flicker, 0.0f, 1.0f);
                }
            }

            // zadd
            if (optional_string ("+Zadd:")) stuff_float (&bsip->z_add);
    ...
    while (optional_string ("$Pspew:")) {
        ...
        if (!(wip->wi_flags[Weapon::Info_Flags::Particle_spew])) {
            WARNINGF (LOCATION,"$Pspew specified for weapon %s but this weapon does not have the \"Particle Spew\" weapon flag set. Automatically setting the flag",wip->name);
        ...
        if (optional_string ("+Index:")) {
            ...
            if (spew_index < 0 || spew_index >= MAX_PARTICLE_SPEWERS) {
                WARNINGF (LOCATION,"+Index in particle spewer out of range. It must be between 0 and %i. Tag will be ignored.",MAX_PARTICLE_SPEWERS);
                spew_index = -1;
            }
        }
        ...
        if (optional_string ("+Remove")) {
            if (spew_index < 0) {
                WARNINGF (LOCATION,"+Index not specified or is out of range, can not remove spewer.");
            }
                ...
                wip->particle_spewers[spew_index].particle_spew_lifetime =
                    0.15f;
                wip->particle_spewers[spew_index].particle_spew_scale = 0.8f;
                wip->particle_spewers[spew_index].particle_spew_z_scale = 1.0f;
                wip->particle_spewers[spew_index].particle_spew_rotation_rate =
                    10.0f;
                wip->particle_spewers[spew_index].particle_spew_offset =
                    vmd_zero_vector;
                wip->particle_spewers[spew_index].particle_spew_velocity =
                    vmd_zero_vector;
                generic_anim_init (
                    &wip->particle_spewers[spew_index].particle_spew_anim,
                    NULL);
            }
        }
        else {                    // were not removing the spewer
            if (spew_index < 0) { // index us ether not used or is invalid, so
                                  // figure out where to put things
                // find a free slot in the pspew info array
                for (size_t s = 0; s < MAX_PARTICLE_SPEWERS; s++) {
                    if (wip->particle_spewers[s].particle_spew_type ==
                        PSPEW_NONE) {
                        spew_index = (int)s;
                        break;
                    }
            ...
            if (spew_index < 0) {
                WARNINGF (LOCATION,"Too many particle spewers, max number of spewers is %i.",MAX_PARTICLE_SPEWERS);
            }
            else { // we have a valid index, now parse the spewer already
                if (optional_string (
                        "+Type:")) { // added type field for pspew types, 0 is
                                     // the default for reverse compatability
                                     // -nuke
                    ...
                    // for compatibility with existing tables that don't have a
                    // type tag
                }
                else if (
                    wip->particle_spewers[spew_index].particle_spew_type ==
                    PSPEW_NONE) { // make sure the omission of type wasn't to
                ...
                if (optional_string ("+Count:")) {
                    stuff_int (&wip->particle_spewers[spew_index]
                                    .particle_spew_count);
                }

                if (optional_string ("+Time:")) {
                    stuff_int (
                        &wip->particle_spewers[spew_index].particle_spew_time);
                }

                if (optional_string ("+Vel:")) {
                    stuff_float (
                        &wip->particle_spewers[spew_index].particle_spew_vel);
                }

                if (optional_string ("+Radius:")) {
                    stuff_float (&wip->particle_spewers[spew_index]
                                      .particle_spew_radius);
                }

                if (optional_string ("+Life:")) {
                    stuff_float (&wip->particle_spewers[spew_index]
                                      .particle_spew_lifetime);
                }
                ...
                if (optional_string ("+Z Scale:")) {
                    stuff_float (&wip->particle_spewers[spew_index]
                                      .particle_spew_z_scale);
                }

                if (optional_string ("+Rotation Rate:")) {
                    stuff_float (&wip->particle_spewers[spew_index]
                                      .particle_spew_rotation_rate);
                }

                if (optional_string ("+Offset:")) {
                    stuff_vec3d (&wip->particle_spewers[spew_index]
                                      .particle_spew_offset);
                }

                if (optional_string ("+Initial Velocity:")) {
                    stuff_vec3d (&wip->particle_spewers[spew_index]
                                      .particle_spew_velocity);
                }
    ...
    // check to see if the pspew flag was enabled but no pspew tags were given,
    // for compatability with retail tables
    if (wip->wi_flags[Weapon::Info_Flags::Particle_spew]) {
        bool nospew = true;
        for (size_t s = 0; s < MAX_PARTICLE_SPEWERS; s++)
    ...
    if (optional_string ("$SSM:")) {
        if (stuff_int_optional (&wip->SSM_index) != 2) {
            // We can't make an SSM lookup yet, because weapons are parsed
            // first, but we can save the data to process later. -MageKing17
            stuff_string (fname, F_NAME, NAME_LENGTH);
            ...
            temp_data.filename = filename;
            temp_data.linenum = get_line_num ();
            if (Delayed_SSM_indices_data.find (wip->name) ==
                Delayed_SSM_indices_data.end ())
        ...
        if (optional_string ("+FOF Spread Rate:")) {
            ...
            if (required_string ("+Max FOF:")) {
                ...
                if (wip->max_fof_spread <= 0.0f) {
                    WARNINGF (LOCATION,"WARNING: +Max FOF must be at least as big as $FOF for '%s'! Defaulting to match $FOF, no spread will occur!",wip->name);
    ...
    if (optional_string ("$Shots:")) { stuff_int (&wip->shots); }

    // Left in for compatibility
    if (optional_string ("$decal:")) {
        WARNINGF (LOCATION,"The decal system has been deactivated in FSO builds. Entries for weapon %s will be discarded.",wip->name);
        ...
        if (optional_string ("+burn time:")) { stuff_float (&bogus); }
    }
    ...
    if (optional_string ("$Transparent:")) {
        ...
        if (wip->alpha_max <= 0.0f) {
            WARNINGF (LOCATION,"WARNING:  Alpha is set to 0 or a negative value for '%s'!  Defaulting to 1.0!",wip->name);
        }
        ...
        if (optional_string ("+Alpha Cycle:")) {
            ...
            if (wip->alpha_max == wip->alpha_min)
                WARNINGF (LOCATION,"WARNING:  Alpha is set to cycle for '%s', but max and min values are the same!",wip->name);
        }
    }
    ...
    if (optional_string ("$Weapon Hitpoints:")) {
        ...
        first_time && (wip->wi_flags
                           [Weapon::Info_Flags::Turret_Interceptable,
                            Weapon::Info_Flags::Fighter_Interceptable])) {
    ...
    if (optional_string ("$Armor Type:")) {
        ...
        if (wip->armor_type_idx == -1)
            WARNINGF (LOCATION, "Invalid armor name %s specified for weapon %s", buf,wip->name);
    }

    if (optional_string ("$Burst Shots:")) {
        stuff_int (&wip->burst_shots);
        if (wip->burst_shots > 0) wip->burst_shots--;
    }

    if (optional_string ("$Burst Delay:")) {
        int temp;
        stuff_int (&temp);
        if (temp > 0) { wip->burst_delay = ((float)temp) / 1000.0f; }
    }

    if (optional_string ("$Burst Flags:")) {
        parse_string_flag_list (
            wip->burst_flags, Burst_fire_flags, Num_burst_fire_flags, NULL);
    }
    ...
    if ((wip->wi_flags[Weapon::Info_Flags::Tag]) &&
        ...
        WARNINGF (LOCATION,"%s is a tag missile, but the target must be tagged to shoot it",wip->name);
    }

    // if burst delay is longer than firewait skip the whole burst fire option
    if (wip->burst_delay >= wip->fire_wait) wip->burst_shots = 0;
    ...
    This pattern is very naive such that it calculates the lowest common
    denominator as being all of the periods multiplied together.
    */
    while (optional_string ("$substitute:")) {
        ...
        if (optional_string ("+period:")) {
            stuff_int (&period);
            if (period <= 0) {
                WARNINGF (LOCATION,"Substitution '%s' for weapon '%s' requires a period greater than 0. Setting period to 1.",subname, wip->name);
                period = 1;
            }
            if (optional_string ("+offset:")) {
                stuff_int (&offset);
                if (offset <= 0) {
                    WARNINGF (LOCATION,"Period offset for substitution '%s' of weapon '%s' has to be greater than 0. Setting offset to 1.",subname, wip->name);
                    offset = 1;
                }
            }
        }
            ...
            stuff_int (&index);
            if (index < 0) {
                WARNINGF (LOCATION,"Substitution '%s' for weapon '%s' requires an index greater than 0. Setting index to 0.",subname, wip->name);
                index = 0;
            }
        }

        // we are going to use weapon substition so, make sure that the pattern
        ...
        // if tbler specifies a period then determine if we can fit the
        // resulting pattern neatly into the pattern array.
        if (period > 0) {
            if ((wip->num_substitution_patterns % period) > 0) {
                // not neat, need to expand the pattern so that our frequency
                // pattern fits completly.
                ...
                if (desired_size > MAX_SUBSTITUTION_PATTERNS) {
                    WARNINGF (LOCATION,"The period is too large for the number of substitution patterns!  desired size=%zu, max size=%d",desired_size, MAX_SUBSTITUTION_PATTERNS);
                }
                else {
            ...
            if (index >= MAX_SUBSTITUTION_PATTERNS) {
                WARNINGF (LOCATION,"Substitution pattern index exceeds the maximum size!  Index=%d, max size=%d",index, MAX_SUBSTITUTION_PATTERNS);
            }
            else {
                if ((size_t)index >= wip->num_substitution_patterns) {
                    // need to make the pattern bigger by filling the extra
    ...
    if (optional_string ("$Score:")) { stuff_int (&wip->score); }
...
void translate_spawn_types () {
    ...
    for (i = 0; i < Num_weapon_types; i++) {
        for (j = 0; j < Weapon_info[i].num_spawn_weapons_defined; j++) {
            if ((Weapon_info[i].spawn_info[j].spawn_type > -1) &&
                (Weapon_info[i].spawn_info[j].spawn_type < Num_spawn_types)) {
                ...
                for (k = 0; k < Num_weapon_types; k++) {
                    if (!strcasecmp (
                        ...
                        if (i == k)
                            WARNINGF (LOCATION,"Weapon %s spawns itself.  Infinite recursion?",Weapon_info[i].name);
    ...
    try {
        ...
        if (optional_string ("#Primary Weapons")) {
            while (required_string_either ("#End", "$Name:")) {
                ...
                if (parse_weapon (WP_LASER, Parsing_modular_table, filename) <
                    0) {
                    continue;
                }
            }
        ...
        if (optional_string ("#Secondary Weapons")) {
            while (required_string_either ("#End", "$Name:")) {
                // AL 28-3-98: If parse_weapon() fails, try next .tbl weapon
                if (parse_weapon (
                        WP_MISSILE, Parsing_modular_table, filename) < 0) {
                    continue;
                }
            }
            required_string ("#End");
        ...
        if (optional_string ("#Beam Weapons")) {
            while (required_string_either ("#End", "$Name:")) {
                // AL 28-3-98: If parse_weapon() fails, try next .tbl weapon
                if (parse_weapon (WP_BEAM, Parsing_modular_table, filename) <
                    0) {
                    continue;
                }
            }
        ...
        if (optional_string ("#Countermeasures")) {
            while (required_string_either ("#End", "$Name:")) {
                int idx =
                    parse_weapon (WP_MISSILE, Parsing_modular_table, filename);
        ...
        // Read in a list of weapon_info indicies that are an ordering of the
        // player weapon precedence. This list is used to select an alternate
        // weapon when a particular weapon is not available during weapon
        // selection.
...
void weapon_sort_by_type () {
    weapon_info *lasers = NULL, *big_lasers = NULL, *beams = NULL,
                *missiles = NULL, *big_missiles = NULL,
                *child_primaries = NULL, *child_secondaries = NULL;
    int num_lasers = 0, num_big_lasers = 0, num_beams = 0, num_missiles = 0,
        num_big_missiles = 0, num_child_primaries = 0,
    ...
    for (i = 0; i < MAX_WEAPON_TYPES; i++) {
        ...
        case WP_BEAM: beams[num_beams++] = Weapon_info[i]; break;
    ...
    // designate start of secondary weapons so that we'll have the correct
    // offset later on
...
void weapon_clean_entries () {
    ...
    for (i = 0; i < Num_weapon_types; i++) {
        ...
        if (wip->wi_flags[Weapon::Info_Flags::Beam]) {
            ...
            for (int s_idx = 0; s_idx < wip->b_info.beam_num_sections;
                 s_idx++) {
                beam_weapon_section_info* bsip = &wip->b_info.sections[s_idx];

                // If this is an invisible beam section, we want to keep it.
                // Originally invisible sections were initialized as they were
                ...
                // Now remove empty beam sections as before
                    ...
                    while (new_idx < MAX_BEAM_SECTIONS) {
                        memcpy (
                            &wip->b_info.sections[new_idx - 1],
                            &wip->b_info.sections[new_idx],
            ...
            if (removed) {
                WARNINGF (LOCATION,"NOTE: weapon-cleanup is removing %i stale beam sections, out of %i original, from '%s'.",removed, wip->b_info.beam_num_sections, wip->name);
                wip->b_info.beam_num_sections -= removed;
            }

            if (wip->b_info.beam_num_sections == 0) {
                WARNINGF (LOCATION, "The beam '%s' has 0 usable sections!",wip->name);
            }
        }
    }
}
...
void weapon_release_bitmaps () {
    ...
    for (i = 0; i < Num_weapon_types; i++) {
        ...
        // go ahead and clear out models, the model paging code will actually
        // take care of releasing this stuff if needed, but we have to keep
        // track of current modelnums ourselves
        if (wip->render_type == WRT_POF) wip->model_num = -1;

        // we are only interested in what we don't need for this mission
        if (used_weapons[i]) continue;
        ...
        if (wip->wi_flags
                [Weapon::Info_Flags::Particle_spew]) { // tweaked for multiple
                                                       // particle spews -nuke
            for (size_t s = 0; s < MAX_PARTICLE_SPEWERS;
                 s++) { // just bitmaps that got loaded
                if (wip->particle_spewers[s].particle_spew_type !=
                    PSPEW_NONE) {
                    if (wip->particle_spewers[s]
                            .particle_spew_anim.first_frame >= 0) {
                        bm_release (wip->particle_spewers[s]
                                        .particle_spew_anim.first_frame);
                        wip->particle_spewers[s]
                            .particle_spew_anim.first_frame = -1;
                    }
                }
            }
        }
...
void weapon_load_bitmaps (int weapon_index) {
    ...
    if ((wip->render_type == WRT_LASER) &&
        ...
        // fall back to an animated type
            ...
            WARNINGF (LOCATION, "Could not find a usable bitmap for '%s'!",wip->name);
            WARNINGF (LOCATION,"Could not find a usable bitmap (%s) for weapon '%s'!",wip->laser_bitmap.filename, wip->name);
        }

        // now see if we also have a glow
        if (strlen (wip->laser_glow_bitmap.filename)) {
            wip->laser_glow_bitmap.first_frame =
                bm_load (wip->laser_glow_bitmap.filename);
            ...
            // fall back to an animated type
                ...
                WARNINGF (LOCATION,"Could not find a usable glow bitmap for '%s'!",wip->name);
                WARNINGF (LOCATION,"Could not find a usable glow bitmap (%s) for weapon '%s'!",wip->laser_glow_bitmap.filename, wip->name);
            }
        }
    }
    ...
    if (wip->wi_flags[Weapon::Info_Flags::Beam]) {
        // particle animation
        if ((wip->b_info.beam_particle_ani.first_frame < 0) &&
            strlen (wip->b_info.beam_particle_ani.filename))
            generic_anim_load (&wip->b_info.beam_particle_ani);

        // muzzle glow
        if ((wip->b_info.beam_glow.first_frame < 0) &&
            strlen (wip->b_info.beam_glow.filename)) {
            if (generic_anim_load (&wip->b_info.beam_glow)) {
                // animated version failed to load, try static instead
                wip->b_info.beam_glow.first_frame =
                    bm_load (wip->b_info.beam_glow.filename);
                ...
                if (wip->b_info.beam_glow.first_frame >= 0) {
                    ...
                    WARNINGF (LOCATION,"Could not find a usable muzzle glow bitmap for '%s'!",wip->name);
                    WARNINGF (LOCATION,"Could not find a usable muzzle glow bitmap (%s) for weapon '%s'!",wip->b_info.beam_glow.filename, wip->name);
                }
            }
        }
            ...
            if ((bsi->texture.first_frame < 0) &&
                ...
                if (generic_anim_load (&bsi->texture)) {
                    ...
                    if (bsi->texture.first_frame >= 0) {
                        ...
                        WARNINGF (LOCATION,"Could not find a usable beam section (%i) bitmap for '%s'!",i, wip->name);
                        WARNINGF (LOCATION,"Could not find a usable beam section (%i) bitmap (%s) for weapon '%s'!",i, bsi->texture.filename, wip->name);
                    }
                }
            }
        }
    ...
    if ((wip->wi_flags[Weapon::Info_Flags::Trail]) &&
        (wip->tr_info.texture.bitmap_id < 0))
        generic_bitmap_load (&wip->tr_info.texture);

    // WMC - Don't try to load an anim if no anim is specified, Mmkay?
    if (wip->wi_flags[Weapon::Info_Flags::Particle_spew]) {
        for (size_t s = 0; s < MAX_PARTICLE_SPEWERS;
             s++) { // looperfied for multiple pspewers -nuke
            if (wip->particle_spewers[s].particle_spew_type != PSPEW_NONE) {
                if ((wip->particle_spewers[s].particle_spew_anim.first_frame <
                     0) &&
                    (wip->particle_spewers[s].particle_spew_anim.filename[0] !=
                     '\0')) {
                    wip->particle_spewers[s].particle_spew_anim.first_frame =
                        bm_load (wip->particle_spewers[s]
                                     .particle_spew_anim.filename);

                    if (wip->particle_spewers[s]
                            .particle_spew_anim.first_frame >= 0) {
                        wip->particle_spewers[s]
                            .particle_spew_anim.num_frames = 1;
                        wip->particle_spewers[s]
                            .particle_spew_anim.total_time = 1;
                    }
                    // fall back to an animated type
                    else if (generic_anim_load (&wip->particle_spewers[s]
                                                     .particle_spew_anim)) {
                        WARNINGF (LOCATION,"Could not find a usable particle spew bitmap for '%s'!",wip->name);
                        WARNINGF (LOCATION,"Could not find a usable particle spew bitmap (%s) for weapon '%s'!",wip->particle_spewers[s].particle_spew_anim.filename,wip->name);
                    }
                }
            }
        }
    ...
    if (!used_weapons[weapon_index]) used_weapons[weapon_index]++;
}

/**
 * Checks all of the weapon infos for substitution patterns and caches the
 * weapon_index of any that it finds.
 */
void weapon_generate_indexes_for_substitution () {
    for (int i = 0; i < MAX_WEAPON_TYPES; i++) {
        ...
        if (wip->num_substitution_patterns > 0) {
            for (size_t j = 0; j < wip->num_substitution_patterns; j++) {
                ...
                if (strcasecmp (
                    ...
                    if (weapon_index == -1) { // invalid sub weapon
                        WARNINGF (LOCATION,"Weapon '%s' requests substitution with '%s' which does not seem to exist",wip->name,wip->weapon_substitution_pattern_names[j]);
                        continue;
                    }
                    ...
                    if (Weapon_info[weapon_index].subtype != wip->subtype) {
                        ...
                        WARNINGF (LOCATION,"Weapon '%s' requests substitution with '%s' which is of a different subtype.",wip->name,wip->weapon_substitution_pattern_names[j]);
                        wip->num_substitution_patterns = 0;
                        std::fill (
                            std::begin (wip->weapon_substitution_pattern),
                            std::end (wip->weapon_substitution_pattern), -1);
                        break;
                    }
                }
            ...
            memset (
                wip->weapon_substitution_pattern_names, 0,
                sizeof (char) * MAX_SUBSTITUTION_PATTERNS * NAME_LENGTH);
        }
        ...
        if (wip->failure_rate > 0.0f) {
            if (VALID_FNAME (wip->failure_sub_name)) {
                ...
                if (wip->failure_sub == -1) { // invalid sub weapon
                    WARNINGF (LOCATION,"Weapon '%s' requests substitution with '%s' which does not seem to exist",wip->name, wip->failure_sub_name.c_str ());
                ...
                if (Weapon_info[wip->failure_sub].subtype != wip->subtype) {
                    // Check to make sure secondaries can't be launched by
                    // primaries and vice versa
                    WARNINGF (LOCATION,"Weapon '%s' requests substitution with '%s' which is of a different subtype.",wip->name, wip->failure_sub_name.c_str ());
                    wip->failure_sub = -1;
                    wip->failure_rate = 0.0f;
                }
            }
...
void weapon_do_post_parse () {
    ...
    for (i = 0; i < Num_weapon_types; i++) {
        ...
        if ((first_cmeasure_index < 0) &&
            (wip->wi_flags[Weapon::Info_Flags::Cmeasure]))
    ...
    for (std::vector< species_info >::iterator ii = Species_info.begin ();
         ii != Species_info.end (); ++ii) {
        if (*ii->cmeasure_name) {
            int index = weapon_info_lookup (ii->cmeasure_name);
            if (index < 0)
                WARNINGF (LOCATION,"Could not find weapon type '%s' to use as countermeasure on species '%s'",ii->cmeasure_name, ii->species_name);
            else if (Weapon_info[index].wi_flags[Weapon::Info_Flags::Beam])
                WARNINGF (LOCATION,"Attempt made to set a beam weapon as a countermeasure on species '%s'",ii->species_name);
            else
                ii->cmeasure_index = index;
        }
    }

    // translate all spawn type weapons to referrnce the appropriate spawned
    // weapon entry
...
void weapon_expl_info_init () {
    ...
    for (i = 0; i < (int)LOD_checker.size (); i++) {
        Weapon_explosions.Load (
            LOD_checker[i].filename, LOD_checker[i].num_lods);
    }
...
void weapon_reset_info () {
    for (auto& wi : Weapon_info) { wi.reset (); }
}
...
void weapon_level_init () {
    ...
    for (i = 0; i < MAX_WEAPON_TYPES; i++) {
        ...
        Weapon_info[i].shockwave.damage_type_idx =
            Weapon_info[i].shockwave.damage_type_idx_sav;
    }
    ...
    if (used_weapons == NULL) used_weapons = new int[Num_weapon_types];
...
void weapon_delete (object* obj) {
    ...
    if (wp->hud_in_flight_snd_sig.isValid () &&
        snd_is_playing (wp->hud_in_flight_snd_sig))
...
/**
 * Check if missile is newly locked onto the Player, maybe play a launch
 * warning
 */
void weapon_maybe_play_warning (weapon* wp) {
    if (wp->homing_object == Player_obj) {
        if (!(wp->weapon_flags[Weapon::Weapon_Flags::Lock_warning_played])) {
            ...
            if ((Weapon_info[wp->weapon_info_index]
                     .wi_flags[Weapon::Info_Flags::Homing_heat]) ||
                (Weapon_info[wp->weapon_info_index]
                     .wi_flags[Weapon::Info_Flags::Homing_javelin])) {
                snd_play (gamesnd_get_game_sound (
                    ship_get_sound (Player_obj, GameSounds::HEATLOCK_WARN)));
            }
            else {
                ASSERT (Weapon_info[wp->weapon_info_index]
                            .wi_flags[Weapon::Info_Flags::Homing_aspect]);
                snd_play (gamesnd_get_game_sound (
                    ship_get_sound (Player_obj, GameSounds::ASPECTLOCK_WARN)));
            }
        }
    }
}
...
void detonate_nearby_missiles (object* killer_objp, object* missile_objp) {
    ...
    weapon_info* killer_infop =
        &Weapon_info[Weapons[killer_objp->instance].weapon_info_index];

    if (killer_infop->cm_kill_single) {
        weapon* wp = &Weapons[missile_objp->instance];
        if (wp->lifeleft > 0.2f) {
            WARNINGF (LOCATION,"Countermeasure (%s-%i) detonated missile (%s-%i) Frame: %i",killer_infop->name, killer_objp->signature,Weapon_info[Weapons[missile_objp->instance].weapon_info_index].name,missile_objp->signature, Framecount);
            wp->lifeleft = 0.2f;
        }
        return;
    }
        ...
        if (iff_x_attacks_y (Weapons[killer_objp->instance].team, wp->team)) {
            if (Missiontime - wp->creation_time > F1_0 / 2) {
                if (vm_vec_dist_quick (&killer_objp->pos, &objp->pos) <
                    ...
                    if (wp->lifeleft > 0.2f) {
                        WARNINGF (LOCATION,"Countermeasure (%s-%i) detonated missile (%s-%i) Frame: %i",killer_infop->name, killer_objp->signature,Weapon_info[Weapons[objp->instance].weapon_info_index].name,objp->signature, Framecount);
                        wp->lifeleft = 0.2f;
                    }
                }
            }
    ...
    for (objp = GET_FIRST (&obj_used_list);
        ...
        if ((objp->type == OBJ_SHIP) ||
            ...
            if ((objp->flags[Object::Object_Flags::Protected]) &&
                ((wp->weapon_flags[Weapon::Weapon_Flags::Spawned]) ||
                 (wip->wi_flags[Weapon::Info_Flags::Untargeted_heat_seeker])))
                continue;

            // Spawned weapons should never home in on their parent - even in
            // multiplayer dogfights where they would pass the iff test below
            if ((wp->weapon_flags[Weapon::Weapon_Flags::Spawned]) &&
                (objp == &Objects[weapon_objp->parent]))
                continue;
            ...
            if (iff_x_attacks_y (wp->team, homing_object_team)) {
                if (objp->type == OBJ_SHIP) {
                    ...
                    // if the homing weapon is a huge weapon and the ship that
                    // is being looked at is not huge, then don't home
                    if ((wip->wi_flags[Weapon::Info_Flags::Huge]) &&
                        (sip->is_small_ship () || !sip->is_flyable () ||
                         sip->is_harmless ())) {
                        continue;
                    }

                    // AL 2-17-98: If ship is immune to sensors, can't home on
                    // it (Sandeep says so)!
                    if (sp->flags[Ship::Ship_Flags::Hidden_from_sensors]) {
                        continue;
                    }

                    // Goober5000: if missiles can't home on sensor-ghosted
                    // ships, they definitely shouldn't home on stealth ships
                    ...
                    if (wip->wi_flags[Weapon::Info_Flags::Homing_javelin]) {
                        target_engines = ship_get_closest_subsys_in_sight (
                            sp, SUBSYSTEM_ENGINE, &weapon_objp->pos);

                        if (!target_engines) continue;
                    }
                    ...
                    // don't attempt to home on weapons if the weapon is a huge
                    // weapon or is a javelin homing weapon.
                    if (wip->wi_flags
                            [Weapon::Info_Flags::Huge,
                             Weapon::Info_Flags::Homing_javelin])
                        continue;

                    // don't look for local ssms that are gone for the time
                    // being
                    if (Weapons[objp->instance].lssm_stage == 3) continue;
                }

                dist = vm_vec_normalized_dir (
                    &vec_to_object, &objp->pos, &weapon_objp->pos);
                ...
                dot =
                    vm_vec_dot (&vec_to_object, &weapon_objp->orient.vec.fvec);
...
void find_homing_object_cmeasures (
    const std::vector< object* >& cmeasure_list) {
    for (object* weapon_objp = GET_FIRST (&obj_used_list);
         weapon_objp != END_OF_LIST (&obj_used_list);
        ...
        if (weapon_objp->type == OBJ_WEAPON) {
            ...
            if (wip->is_homing ()) {
                ...
                for (auto cit = cmeasure_list.cbegin ();
                     cit != cmeasure_list.cend (); ++cit) {
                    // don't have a weapon try to home in on itself
                    if (*cit == weapon_objp) continue;

                    weapon* cm_wp = &Weapons[(*cit)->instance];
                    weapon_info* cm_wip =
                        &Weapon_info[cm_wp->weapon_info_index];

                    // don't have a weapon try to home in on missiles fired by
                    // the same team, unless its the traitor team.
                    if ((wp->team == cm_wp->team) && (wp->team != Iff_traitor))
                        continue;

                    vec3d vec_to_object;
                    float dist = vm_vec_normalized_dir (
                        &vec_to_object, &(*cit)->pos, &weapon_objp->pos);
                    ...
                    if (dist < cm_wip->cm_effective_rad) {
                        ...
                        if (wp->cmeasure_ignore_list == nullptr) {
                            ...
                            bool found = false;
                            for (auto ii = wp->cmeasure_ignore_list->cbegin ();
                                 ii != wp->cmeasure_ignore_list->cend ();
                                 ++ii) {
                                if ((*cit)->signature == *ii) {
                                    WARNINGF (LOCATION,"Weapon (%s-%04i) already seen CounterMeasure (%s-%04i) Frame: %i",wip->name, weapon_objp->instance,cm_wip->name, (*cit)->signature,Framecount);
                                    found = true;
                                    break;
                                }
                            }
                        ...
                        if (wip->wi_flags[Weapon::Info_Flags::Homing_aspect]) {
                            // aspect seeker this likely to chase a
                            // countermeasure
                            chance = cm_wip->cm_aspect_effectiveness /
                                     wip->seeker_strength;
                        }
                        else {
                            // heat seeker and javelin HS this likely to chase
                            // a countermeasure
                            chance = cm_wip->cm_heat_effectiveness /
                                     wip->seeker_strength;
                        }

                        // remember this cmeasure so it can be ignored in
                        // future
                        ...
                        if (frand () >= chance) {
                            ...
                            float dot = vm_vec_dot (
                                &vec_to_object, &weapon_objp->orient.vec.fvec);

                            if (dot > best_dot) {
                                best_dot = dot;
                                wp->homing_object = (*cit);
                                cmeasure_maybe_alert_success ((*cit));
                                WARNINGF (LOCATION,"Weapon (%s-%04i) chasing CounterMeasure (%s-%04i) Frame: %i",wip->name, weapon_objp->instance,cm_wip->name, (*cit)->signature,Framecount);
                            }
                        }
                    }
                }
...
bool aspect_should_lose_target (weapon* wp) {
    ...
    if (wp->homing_object->signature != wp->target_sig) {
        if (wp->homing_object->type == OBJ_WEAPON) {
            weapon_info* target_info =
                &Weapon_info[Weapons[wp->homing_object->instance]
                                 .weapon_info_index];

            if (target_info->wi_flags[Weapon::Info_Flags::Cmeasure]) {
                // Check if we can home on this countermeasure
                bool home_on_cmeasure =
                    The_mission.ai_profile->flags
                        [AI::Profile_Flags::Aspect_lock_countermeasure] ||
                    target_info->wi_flags
                        [Weapon::Info_Flags::Cmeasure_aspect_home_on];

                if (!home_on_cmeasure) { return true; }
            }
        }
    }
...
void weapon_home (object* obj, int num, float frame_time) {
    ...
    // Goober5000 - this has been fixed back to more closely follow the
    // original logic.  Remember, the retail code had 0.5 second of free flight
    // time, the first half of which was spent ramping up to full speed.
    if ((hobjp == &obj_used_list) || (f2fl (Missiontime - wp->creation_time) <
                                      (wip->free_flight_time / 2))) {
        if (f2fl (Missiontime - wp->creation_time) > wip->free_flight_time) {
            // If this is a heat seeking homing missile and [free-flight-time]
            // has elapsed since firing and we don't have a target (else we
            // wouldn't be inside the IF), find a new target.
            if (wip->wi_flags[Weapon::Info_Flags::Homing_heat]) {
                find_homing_object (obj, num);
            }
            // modders may want aspect homing missiles to die if they lose
            // their target
            else if (
        ...
        if (wip->acceleration_time > 0.0f) {
            if (Missiontime - wp->creation_time <
                fl2f (wip->acceleration_time)) {
                float t;

                t = f2fl (Missiontime - wp->creation_time) /
                    wip->acceleration_time;
                obj->phys_info.speed =
                    wp->launch_speed +
                    MAX (0.0f, wp->weapon_max_vel - wp->launch_speed) * t;
            }
        }
        // since free_flight_time can now be 0, guard against that
            ...
            if (obj->phys_info.speed > max_speed) {
                obj->phys_info.speed -=
                    frame_time * (2 / wip->free_flight_time);
            }
            else if (
                (obj->phys_info.speed <
                 max_speed / (2 / wip->free_flight_time)) &&
                (wip->wi_flags[Weapon::Info_Flags::Homing_heat])) {
        ...
        vm_vec_copy_scale (
            &obj->phys_info.desired_vel, &obj->orient.vec.fvec,
            obj->phys_info.speed);
    ...
    weapon_info* hobj_infop =
        &Weapon_info[Weapons[hobjp->instance].weapon_info_index];
    ...
    if (wip->acceleration_time > 0.0f) {
        if (Missiontime - wp->creation_time < fl2f (wip->acceleration_time)) {
            ...
            t = f2fl (Missiontime - wp->creation_time) /
                wip->acceleration_time;
            obj->phys_info.speed =
                wp->launch_speed + (wp->weapon_max_vel - wp->launch_speed) * t;
            vm_vec_copy_scale (
                &obj->phys_info.desired_vel, &obj->orient.vec.fvec,
                obj->phys_info.speed);
        }
    ...
    // AL 4-8-98: If original target for aspect lock missile is lost, stop
    // homing WCS - or javelin
    if (wip->is_locked_homing ()) {
        if (wp->target_sig > 0) {
    ...
    // If target subsys is dead make missile pick random spot on target as
    // attack point.
    if (wp->homing_subsys != NULL) {
        if (wp->homing_subsys
                ->flags[Ship::Subsystem_Flags::Missiles_ignore_if_dead]) {
            if ((wp->homing_subsys->max_hits > 0) &&
                (wp->homing_subsys->current_hits <= 0)) {
                wp->homing_object = &obj_used_list;
                return;
            }
        }
    ...
    if ((wip->wi_flags[Weapon::Info_Flags::Homing_javelin]) &&
        (hobjp->type == OBJ_SHIP) && (wp->target_sig > 0) &&
        (wp->homing_subsys != NULL) &&
        (wp->homing_subsys->system_info->type != SUBSYSTEM_ENGINE)) {
        int sindex = ship_get_by_signature (wp->target_sig);
        if (sindex >= 0) {
            ship* enemy = &Ships[sindex];
            wp->homing_subsys = ship_get_closest_subsys_in_sight (
                enemy, SUBSYSTEM_ENGINE, &Objects[wp->objnum].pos);
        }
    }
    ...
    // TODO maybe add flag to allow WF_LOCKED_HOMING to lose target when target
    // is dead
    ...
    case OBJ_WEAPON: {
        bool home_on_cmeasure =
            The_mission.ai_profile
                ->flags[AI::Profile_Flags::Aspect_lock_countermeasure] ||
        ...
        if (((hobj_infop->wi_flags[Weapon::Info_Flags::Cmeasure]) &&
             !home_on_cmeasure)) {
            break;
        }
    ...
    if (hobjp != &obj_used_list) {
        ...
        if (hobjp->type == OBJ_SHIP) {
            ...
            if ((aip->nearest_locked_object == -1) ||
                (dist < aip->nearest_locked_distance)) {
    ...
    if (hobjp != &obj_used_list) {
        ...
        // the homing missile may be seeking a subsystem on a ship.  If so, we
        ...
        if ((wp->homing_subsys != NULL) &&
            ...
            get_subsystem_world_pos (
                hobjp, Weapons[num].homing_subsys, &target_pos);
            wp->homing_pos =
                target_pos; // store the homing position in weapon data
            ...
            if (hobjp->radius < 40.0f) {
                ...
                if (hobjp->type == OBJ_SHIP) {
                    if (!pick_homing_point) {
                        ...
                        // world coords (ie not pick a different vertex)
                        wp->pick_big_attack_point_timestamp = 0;
                    }

                    if (pick_homing_point &&
                        !(wip->wi_flags
                              [Weapon::Info_Flags::Non_subsys_homing])) {
                        // If *any* player is parent of homing missile, then
                        // use position where lock indicator is
                        if (Objects[obj->parent]
                            ...
                            // the timestamp to 0 ensures this.
                            if (wip->is_locked_homing ()) {
                                wp->pick_big_attack_point_timestamp = 0;
                            }
                            else {
                            ...
                            if (pp && pp->locking_subsys) {
                                wp->big_attack_point =
                                    pp->locking_subsys->system_info->pnt;
                            }
                            else {
        ...
        if (IS_VEC_NULL (&target_pos)) return;
        ...
        dist_to_target =
            vm_vec_normalized_dir (&vec_to_goal, &target_pos, &obj->pos);
        ...
        if ((old_dot > 0.1f) && (time_to_target > 0.1f)) {
            if (wip->wi_flags[Weapon::Info_Flags::Variable_lead_homing]) {
                vm_vec_scale_add2 (
                    ...
                    (0.33f * wip->target_lead_scaler *
                     MIN (time_to_target, 6.0f)));
            }
            else if (wip->is_locked_homing ()) {
                vm_vec_scale_add2 (
                    &target_pos, &hobjp->phys_info.vel,
                    MIN (time_to_target, 2.0f));
            }
        }
        ...
        if (wip->wi_flags[Weapon::Info_Flags::Homing_heat]) {
            ...
            WARNINGF (LOCATION,"Tried to make weapon '%s' home, but found it wasn't aspect-seeking or heat-seeking or a Javelin!",wip->name);
        }
        ...
        if (wip->acceleration_time > 0.0f) {
            ...
            !(wip->wi_flags[Weapon::Info_Flags::No_homing_speed_ramp]) &&
            Missiontime - wp->creation_time < i2f (1)) {
            // Default behavior:
            // For first second of weapon's life, it doesn't fly at top speed.
            // It ramps up.
            float t;
        ...
        // turn the missile towards the target only if non-swarm.  Homing swarm
        // missiles choose a different vector to turn towards, this is done in
        // swarm_update_direction().
        if (wp->swarm_index < 0) {
            ai_turn_towards_vector (
                &target_pos, obj, wip->turn_time, nullptr, nullptr, 0.0f, 0,
                nullptr);
            vel = vm_vec_mag (&obj->phys_info.desired_vel);

            vm_vec_copy_scale (
                &obj->phys_info.desired_vel, &obj->orient.vec.fvec, vel);
        }
    }
}

// as Mike K did with ships -- break weapon into process_pre and process_post
// for code to execute before and after physics movement

void weapon_process_pre (object* obj, float /*frame_time*/) {
    if (obj->type != OBJ_WEAPON) return;
    ...
    if ((obj->type == OBJ_WEAPON) &&
        (Weapons[obj->instance].cscrew_index >= 0)) {
    ...
    if (wp->det_range > 0.0f) {
        ...
        if (vm_vec_mag (&temp) >= wp->det_range) { weapon_detonate (obj); }
    }

    // WMC - Maybe detonate weapon anyway!
    if (wip->det_radius > 0.0f) {
        if ((wp->homing_object != &obj_used_list) &&
            (wp->homing_object->type != 0)) {
            if (!IS_VEC_NULL (&wp->homing_pos) &&
                vm_vec_dist (&wp->homing_pos, &obj->pos) <= wip->det_radius) {
                weapon_detonate (obj);
            }
        }
        else if (wp->target_num > -1) {
            if (vm_vec_dist (&obj->pos, &Objects[wp->target_num].pos) <=
                wip->det_radius) {
                weapon_detonate (obj);
            }
        }
...
void weapon_maybe_play_flyby_sound (object* weapon_objp, weapon* wp) {
    ...
    if (!timestamp_elapsed (Weapon_flyby_sound_timer)) { return; }
    ...
    if (!(wp->weapon_flags[Weapon::Weapon_Flags::Played_flyby_sound])) {
        ...
        if (Viewer_obj) { radius = Viewer_obj->radius; }
        else {
            radius = 0.0f;
        }
                ...
                if (Weapon_info[wp->weapon_info_index].flyby_snd.isValid ()) {
                    snd_play_3d (
                        gamesnd_get_game_sound (
                            Weapon_info[wp->weapon_info_index].flyby_snd),
                        &weapon_objp->pos, &Eye_position);
                }
                else {
                    if (Weapon_info[wp->weapon_info_index].subtype ==
                        WP_LASER) {
                        snd_play_3d (
                            gamesnd_get_game_sound (GameSounds::WEAPON_FLYBY),
                            &weapon_objp->pos, &Eye_position);
                    }
                }
                Weapon_flyby_sound_timer = timestamp (200);
                wp->weapon_flags.set (
                    Weapon::Weapon_Flags::Played_flyby_sound);
            }
        }
    }
}
...
static void
weapon_set_state (weapon_info* wip, weapon* wp, WeaponState state) {
...
static void weapon_update_state (weapon* wp) {
    ...
    if (wip->free_flight_time) {
        ...
        if (lifetime < fl2f (wip->free_flight_time)) {
            ...
            lifetime >= fl2f (wip->free_flight_time) &&
            (lifetime - Frametime) <= fl2f (wip->free_flight_time) &&
            wp->homing_object != nullptr) {
...
// process a weapon after physics movement.  MWA reorders some of the code on
// 8/13 for multiplayer.  When adding something to this function, decide
// whether or not a client in a multiplayer game needs to do what is normally
// done in a single player game.  Things like plotting an object on a radar,
// effect for exhaust are things that are done on all machines.  Things which
// calculate weapon targets, new velocities, etc, are server only functions and
// should go after the if ( !MULTIPLAYER_MASTER ) statement See Allender if you
...
void weapon_process_post (object* obj, float frame_time) {
    ...
    // check life left.  Multiplayer client code will go through here as well.
    // We must be careful in weapon_hit when killing a missile that spawn child
    // weapons!!!!
    if (wp->lifeleft < 0.0f) {
        if (wip->subtype & WP_MISSILE) {
            weapon_detonate (obj);
    ...
    if (((wip->wi_flags[Weapon::Info_Flags::Bomb]) ||
         (wip->wi_flags[Weapon::Info_Flags::Shown_on_radar])) &&
        !(wip->wi_flags[Weapon::Info_Flags::Dont_show_on_radar])) {
        if (hud_gauge_active (HUD_RADAR)) { radar_plot_object (obj); }
    }

    // trail missiles
    if ((wip->wi_flags[Weapon::Info_Flags::Trail]) &&
        !(wip->wi_flags[Weapon::Info_Flags::Corkscrew])) {
        if ((wp->trail_ptr != NULL) && (wp->lssm_stage != 3)) {
            ...
            if (wip->render_type == WRT_LASER) {
                ...
                vm_vec_scale_add (
                    &pos, &obj->pos, &obj->orient.vec.fvec,
                    (wip->laser_length / 2));
            }
            else {
                pos = obj->pos;
...
// maybe play a "whizz sound" if close enough to view position
    ...
    if (wp->target_num != -1) {
        if (Objects[wp->target_num].signature == wp->target_sig) {
            ...
            if (cur_dist < wp->nearest_dist) { wp->nearest_dist = cur_dist; }
                ...
                if (obj->parent != Player_obj - Objects) {
                    parent_aip = &Ai_info[Ships[Objects[obj->parent].instance]
                                              .ai_index];
                }

                vm_vec_normalized_dir (
                    &tvec, &v0, &Objects[wp->target_num].pos);
                dot = vm_vec_dot (
                    &tvec, &Objects[wp->target_num].orient.vec.fvec);
                wp->target_num = -1;
    ...
    // a single player or multiplayer server function -- it affects actual
    // weapon movement.
    if (wip->is_homing () &&
        !(wp->weapon_flags[Weapon::Weapon_Flags::No_homing])) {
        weapon_home (obj, num, frame_time);

        // If this is a swarm type missile,
        if (wp->swarm_index >= 0) { swarm_update_direction (obj); }

        if (wp->cscrew_index >= 0) { cscrew_process_post (obj); }
    }
        ...
        if (Missiontime - wp->creation_time < fl2f (wip->acceleration_time)) {
            ...
            obj->phys_info.flags |=
                PF_CONST_VEL; // Max speed reached, can use simpler physics
                              // calculations now
        }
    ...
    if (wip->wi_flags[Weapon::Info_Flags::Local_ssm]) {
        ...
        if ((wp->lssm_stage == 1) &&
            (timestamp_elapsed (wp->lssm_warpout_time))) {
            ...
            wp->lssm_warp_time =
                ((obj->radius * 2) / (obj->phys_info.speed)) + 1.5f;
            wp->lssm_warp_time = MAX (wp->lssm_warp_time, 7.0f);

            // calculate the percerentage of the warpholes life at which the
            // missile is fully in subspace.
            wp->lssm_warp_pct = 1.0f - (3.0f / wp->lssm_warp_time);

            // create the warphole
            vm_vec_add2 (&warpout, &obj->pos);
            wp->lssm_warp_idx = fireball_create (
                &warpout, FIREBALL_WARP, FIREBALL_WARP_EFFECT, -1,
                obj->radius * 1.5f, 1, &vmd_zero_vector, wp->lssm_warp_time, 0,
                &obj->orient);

            if (wp->lssm_warp_idx < 0) {
                WARNINGF (LOCATION,"LSSM: Failed to create warp effect! Please report if this happens frequently.");
                // Abort warping
                wp->lssm_stage = 0;
            }
            else {
        ...
        if ((wp->lssm_stage == 2) &&
            (fireball_lifeleft_percent (&Objects[wp->lssm_warp_idx]) <=
             wp->lssm_warp_pct)) {
            ...
            // get the position of the target, and estimate its position when
            // it warps out so we have an idea of where it will be.
            if (wp->target_num >= 0) {
                vm_vec_scale_add (
                    &wp->lssm_target_pos, &Objects[wp->target_num].pos,
                    &Objects[wp->target_num].phys_info.vel,
                    (float)wip->lssm_warpin_delay / 1000.0f);
            }
            else {
        ...
        // time to warp in.
        if ((wp->lssm_stage == 3) &&
            (timestamp_elapsed (wp->lssm_warpin_time))) {
            vec3d warpin;
            ...
            vm_vec_random_in_circle (
                &warpin, &wp->lssm_target_pos, &target_objp->orient,
                wip->lssm_warpin_radius + target_objp->radius, 1);
            ...
            wp->lssm_warp_idx = fireball_create (
                &warpin, FIREBALL_WARP, FIREBALL_WARP_EFFECT, -1,
                obj->radius * 1.5f, 0, &vmd_zero_vector, wp->lssm_warp_time, 0,
                &orient);
        ...
        // If the previous fireball creation failed just put it into normal
        // space now
        if ((wp->lssm_stage == 4) &&
            (wp->lssm_warp_idx < 0 ||
             fireball_lifeleft_percent (&Objects[wp->lssm_warp_idx]) <=
    ...
    if (wip->hud_in_flight_snd.isValid () &&
        obj->parent_sig == Player_obj->signature) {
        bool play_sound = false;
        switch (wip->in_flight_play_type) {
        case TARGETED: play_sound = wp->homing_object != &obj_used_list; break;
        case UNTARGETED:
        ...
        if (play_sound) {
            if (!wp->hud_in_flight_snd_sig.isValid () ||
                !snd_is_playing (wp->hud_in_flight_snd_sig)) {
                wp->hud_in_flight_snd_sig = snd_play_looping (
                    gamesnd_get_game_sound (wip->hud_in_flight_snd));
            }
        }
    }
 ...
 */
void weapon_set_tracking_info (
    int weapon_objnum, int parent_objnum, int target_objnum,
    int target_is_locked, ship_subsys* target_subsys) {
    object* parent_objp;
    weapon* wp;
    weapon_info* wip;
    int targeting_same = 0;

    if (weapon_objnum < 0) { return; }
    ...
    if (wp->weapon_flags[Weapon::Weapon_Flags::No_homing]) { return; }
    ...
    if (parent_objp != NULL &&
        (Ships[parent_objp->instance]
             .flags[Ship::Ship_Flags::No_secondary_lockon])) {
    ...
    if (parent_objp == NULL || Ships[parent_objp->instance].ai_index >= 0) {
        ...
        if (parent_objp != NULL &&
            Ships[parent_objp->instance].team == target_team) {
            targeting_same = 1;

            // Goober5000 - if we're going bonkers, pretend we're not targeting
            // our own team
            ai_info* parent_aip =
                &Ai_info[Ships[parent_objp->instance].ai_index];
            if (parent_aip->active_goal != AI_GOAL_NONE &&
                parent_aip->active_goal != AI_ACTIVE_GOAL_DYNAMIC) {
                if (parent_aip->goals[parent_aip->active_goal]
        ...
        if (target_objnum != -1 && !targeting_same) {
            ...
            if ((wip->wi_flags[Weapon::Info_Flags::Homing_aspect]) &&
                ...
                if ((Objects[target_objnum].type == OBJ_SHIP) &&
                    ((wp->homing_subsys == NULL) ||
                     (wp->homing_subsys->system_info->type !=
                      SUBSYSTEM_ENGINE))) {
                    ship* target_ship =
                        &Ships[Objects[target_objnum].instance];
                    wp->homing_subsys = ship_get_closest_subsys_in_sight (
                        target_ship, SUBSYSTEM_ENGINE,
                    ...
                    if (wp->homing_subsys == NULL) {
                        ...
                        ASSERT (
                            wp->homing_subsys->parent_objnum == target_objnum);
                        wp->homing_object = &Objects[target_objnum];
                        weapon_maybe_play_warning (wp);
                    }
                }
...
size_t* get_pointer_to_weapon_fire_pattern_index (
    int weapon_type, ship* shipp, ship_subsys* src_turret) {
    ASSERT (shipp != NULL);
    ship_weapon* ship_weapon_p = &(shipp->weapons);
    if (src_turret) { ship_weapon_p = &src_turret->weapons; }
    ASSERT (ship_weapon_p != NULL);

    // search for the corresponding bank pattern index for the weapon_type that
    // is being fired. Note: Because a weapon_type may not be unique to a
    // weapon bank per ship this search may attribute the weapon to the wrong
    // bank.  Hopefully this isn't a problem.
 ...
 *
 * @return Index of weapon in the Objects[] array, -1 if the weapon object was
 * not created
 */
int Weapons_created = 0;
int weapon_create (
    vec3d* pos, matrix* porient, int weapon_type, int parent_objnum,
    int group_id, int is_locked, int is_spawned, float fof_cooldown,
    ...
    if (wip->wi_flags[Weapon::Info_Flags::Beam]) {
        WARNINGF (LOCATION,"An attempt to fire a beam ('%s') through weapon_create() was made.",wip->name);
        return -1;
    }

    parent_objp = NULL;
    if (parent_objnum >= 0) { parent_objp = &Objects[parent_objnum]; }
    ...
    if ((wip->num_substitution_patterns > 0) && (parent_objp != NULL)) {
        ...
        ASSERTX (
            parent_objp->type == OBJ_SHIP, "Expected type OBJ_SHIP, got %d",
            parent_objp->type);
        ASSERTX (
            (parent_objp->instance < MAX_SHIPS) &&
                (parent_objp->instance >= 0),
            "Ship index is %d, which is out of range [%d,%d)",
            parent_objp->instance, 0, MAX_SHIPS);
        ship* parent_shipp = &(Ships[parent_objp->instance]);
        ASSERT (parent_shipp != NULL);

        size_t* position = get_pointer_to_weapon_fire_pattern_index (
            weapon_type, parent_shipp, src_turret);
        ASSERTX (
            position != NULL,
            "'%s' is trying to fire a weapon that is not selected",
        ...
        if (((Weapon_info[weapon_type].subtype == WP_LASER &&
             ...
             (Weapon_info[weapon_type].subtype == WP_MISSILE &&
        ...
        *position = (*position) % wip->num_substitution_patterns;
        ...
        if (wip->weapon_substitution_pattern[curr_pos] == -1) {
            ...
            return weapon_create (
                pos, porient, wip->weapon_substitution_pattern[curr_pos],
                parent_objnum, group_id, is_locked, is_spawned, fof_cooldown);
        }
    }
    ...
    if (wip->failure_rate > 0.0f) {
        ...
        if (test < wip->failure_rate) {
            if (wip->failure_sub != -1) {
                return weapon_create (
                    pos, porient, wip->failure_sub, parent_objnum, group_id,
                    is_locked, is_spawned, fof_cooldown);
            }
            else {
                return -1;
            }
    ...
    if (Num_weapons >= MAX_WEAPONS - 5) {
        // No, do remove for AI ships -- MK, 3/12/98  // don't need to try and
        // delete weapons for ai ships if (
        // !(Objects[parent_objnum].flags[Object::Object_Flags::Player_ship]) )
        // return -1;

        num_deleted = collide_remove_weapons ();

        WARNINGF (LOCATION, "Deleted %d weapons because of lack of slots",num_deleted);
        if (num_deleted == 0) { return -1; }
    }

    for (n = 0; n < MAX_WEAPONS; n++) {
        if (Weapons[n].weapon_info_index < 0) { break; }
    }
    ...
    if (!used_weapons[weapon_type]) weapon_load_bitmaps (weapon_type);
    ...
    if (porient != NULL) { morient = *porient; }
    else {
    ...
    if (fof_cooldown != 0.0f) {
        combined_fof =
            wip->field_of_fire + (fof_cooldown * wip->max_fof_spread);
    }
    ...
    objnum = obj_create (
        OBJ_WEAPON, parent_objnum, n, orient, pos, 2.0f, default_flags);
    ASSERT (objnum >= 0);
    objp = &Objects[objnum];
    ...
    if ((wip->subtype == WP_LASER) ||
        ((wip->subtype == WP_MISSILE) && !(wip->is_homing ()) &&
    ...
    if (parent_objp != NULL) {
        ASSERT (
            parent_objp->type ==
            OBJ_SHIP); // Get Mike, a non-ship has fired a weapon!
        ASSERT (
            (parent_objp->instance >= 0) &&
            (parent_objp->instance < MAX_SHIPS));
        wp->team = Ships[parent_objp->instance].team;
        wp->species =
            Ship_info[Ships[parent_objp->instance].ship_info_index].species;
    }
    else {
    ...
    if (wip->wi_flags[Weapon::Info_Flags::Particle_spew]) {
        for (size_t s = 0; s < MAX_PARTICLE_SPEWERS;
             s++) { // allow for multiple time values
            if (wip->particle_spewers[s].particle_spew_type != PSPEW_NONE) {
    ...
    if (wip->life_min < 0.0f && wip->life_max < 0.0f) {
        ...
        wp->lifeleft =
            ((rand_val) * (wip->life_max - wip->life_min)) + wip->life_min;
        if ((wip->wi_flags[Weapon::Info_Flags::Cmeasure]) &&
            (parent_objp != NULL) &&
            (parent_objp->flags[Object::Object_Flags::Player_ship])) {
            wp->lifeleft *=
                The_mission.ai_profile->cmeasure_life_scale[Game_skill_level];
        }
    }

    if (wip->wi_flags[Weapon::Info_Flags::Cmeasure]) {
        // For the next two frames, any non-timer-based countermeasures will
        // pulse each frame.
        Cmeasures_homing_check = 2;
        if (wip->cmeasure_timer_interval > 0) {
            // Timer-based countermeasures spawn pulsing as well.
            wp->cmeasure_timer =
                timestamp (); // Could also use timestamp(0), but it doesn't
    ...
    if (wip->render_type == WRT_POF) {
        ...
        if (model_get (wip->model_num)->flags & PM_FLAG_HAS_INTRINSIC_ROTATE) {
            wp->model_instance_num =
                model_create_instance (false, wip->model_num);
        }
    }
    ...
    if (wip->acceleration_time > 0.0f) {
        vm_vec_copy_scale (
            ...
            0.01f); // Tiny initial velocity to avoid possible null vec issues
            ...
            if (wip->free_flight_time > 0.0)
                vm_vec_copy_scale (
                    ...
                    vm_vec_dot (
                        ...
                        &parent_objp->orient.vec.fvec) +
                        objp->phys_info.max_vel.xyz.z / 4);
            else
                vm_vec_copy_scale (
                    &objp->phys_info.desired_vel, &objp->orient.vec.fvec,
                    objp->phys_info.max_vel.xyz.z);
    ...
    // Turey - maybe make the initial speed of the weapon take into account the
    // velocity of the parent. Improves aiming during gliding.
    if ((parent_objp != NULL) &&
        (The_mission.ai_profile
             ->flags[AI::Profile_Flags::Use_additive_weapon_velocity])) {
        float pspeed = vm_vec_mag (&parent_objp->phys_info.vel);
        vm_vec_scale_add2 (
            &objp->phys_info.vel, &parent_objp->phys_info.vel,
            wip->vel_inherit_amount);
        wp->weapon_max_vel += pspeed * wip->vel_inherit_amount;
        objp->phys_info.speed = vm_vec_mag (&objp->phys_info.vel);

        if (wip->acceleration_time > 0.0f) wp->launch_speed += pspeed;
    }
    ...
    if (wip->wi_flags[Weapon::Info_Flags::Local_ssm]) {
        ...
        wp->lssm_warpin_time =
            timestamp (wip->lssm_warpout_delay + wip->lssm_warpin_delay);
        wp->lssm_stage = 1;
    }
    else {
        wp->lssm_stage = -1;
    ...
    // NOTE : this function will change some fundamental things about the
    // weapon object
    if ((wip->wi_flags[Weapon::Info_Flags::Flak]) &&
        !(wip->wi_flags[Weapon::Info_Flags::Render_flak])) {
        obj_set_flags (
            &Objects[wp->objnum],
            Objects[wp->objnum].flags - Object::Object_Flags::Renders);
    }
    ...
    if (wip->wi_flags
        ...
        // If a weapon has no trails, make sure we don't try to do anything
        // with them.
        wp->trail_ptr = NULL;
    }
...
#ifndef NDEBUG
        // since debug builds always have cheats enabled, we don't necessarily
        // get the chance to enable thrusters for previously non-loaded weapons
        // (ie, weapons_page_in_cheats()) when using cheat-keys, so we need to
        // make sure and enable thrusters here if needed
        if (pm->n_thrusters > 0) {
    ...
    if (is_spawned) { wp->weapon_flags.set (Weapon::Weapon_Flags::Spawned); }
    ...
    if (Weapons_inherit_parent_collision_group) {
        Objects[objnum].collision_group_id =
            Objects[parent_objnum].collision_group_id;
    }
...
void spawn_child_weapons (object* objp) {
    ...
    ASSERTX (
        objp->type == OBJ_WEAPON || objp->type == OBJ_BEAM,
        "spawn_child_weapons() doesn't make sense for non-weapon non-beam "
        "objects; get a coder!\n");
    ASSERTX (
        objp->instance >= 0,
        "spawn_child_weapons() called with an object with an instance of %d; "
        "get a coder!\n",
        objp->instance);
    ASSERTX (
        !(objp->type == OBJ_WEAPON) || (objp->instance < MAX_WEAPONS),
        "spawn_child_weapons() called with a weapon with an instance of %d "
        "while MAX_WEAPONS is %d; get a coder!\n",
        objp->instance, MAX_WEAPONS);
    ASSERTX (
        !(objp->type == OBJ_BEAM) || (objp->instance < MAX_BEAMS),
        "spawn_child_weapons() called with a beam with an instance of %d "
        "while MAX_BEAMS is %d; get a coder!\n",
        objp->instance, MAX_BEAMS);

    if (objp->type == OBJ_WEAPON) {
        wp = &Weapons[objp->instance];
        ASSERTX (
            (wp->weapon_info_index >= 0) &&
                (wp->weapon_info_index < Num_weapon_types),
        ...
        ASSERTX (
            (bp->weapon_info_index >= 0) &&
                (bp->weapon_info_index < Num_weapon_types),
            "Invalid weapon_info_index of %d; get a coder!\n",
            bp->weapon_info_index);
        wip = &Weapon_info[bp->weapon_info_index];
    }
    else { // Let's make sure we don't do screwball things in a release build
           // if this gets called with a non-weapon non-beam.
        return;
    }
    ...
    if (parent_num >= 0) {
        if ((Objects[parent_num].type != objp->parent_type) ||
            ...
            WARNINGF (LOCATION,"Parent of spawn weapon does not exist.  Not spawning.");
            return;
        }
    }
    ...
    for (i = 0; i < wip->num_spawn_weapons_defined; i++) {
        for (j = 0; j < wip->spawn_info[i].spawn_count; j++) {
            ...
            if (child_wip->wi_flags[Weapon::Info_Flags::Beam]) {
                ...
                fire_info.bfi_flags |=
                    BFIF_FLOATING_BEAM | BFIF_TARGETING_COORDS;
                fire_info.starting_pos = *opos;
                fire_info.beam_info_index = child_id;
                fire_info.team =
                    static_cast< char > (obj_team (&Objects[parent_num]));
                ...
                weapon_objnum = weapon_create (
                    &pos, &orient, child_id, parent_num, -1,
                    wp->weapon_flags[Weapon::Weapon_Flags::Locked_when_fired],
                    1);

                // if the child inherits parent target, do it only if the
                // parent weapon was locked to begin with
                if ((child_wip->wi_flags
                         [Weapon::Info_Flags::Inherit_parent_target]) &&
                    (wp->homing_object != &obj_used_list)) {
                    ...
                    if (wp->swarm_index >= 0) {
                        ...
                        weapon_set_tracking_info (
                            weapon_objnum, parent_num, swarmp->homing_objnum,
                            1, wp->homing_subsys);
                    }
                    else {
                        weapon_set_tracking_info (
                            weapon_objnum, parent_num, wp->target_num, 1,
                            wp->homing_subsys);
                    }
                }
 ...
 */
void weapon_play_impact_sound (
    weapon_info* wip, vec3d* hitpos, bool is_armed) {
    if (is_armed) {
        if (wip->impact_snd.isValid ()) {
            snd_play_3d (
                gamesnd_get_game_sound (wip->impact_snd), hitpos,
                &Eye_position);
        }
    }
    else {
        if (wip->disarmed_impact_snd.isValid ()) {
            snd_play_3d (
                gamesnd_get_game_sound (wip->disarmed_impact_snd), hitpos,
                &Eye_position);
        }
    }
}
 ...
 */
void weapon_hit_do_sound (
    object* hit_obj, weapon_info* wip, vec3d* hitpos, bool is_armed,
    int quadrant) {
    float shield_str;

    // If non-missiles (namely lasers) expire without hitting a ship, don't
    // play impact sound
    if (wip->subtype != WP_MISSILE) {
        if (!hit_obj) {
    ...
    if (timestamp_elapsed (Weapon_impact_timer)) {
        ...
        if (shield_str > 0.1f) {
            ...
            if (hit_obj == Player_obj) {
                snd_play_3d (
                    gamesnd_get_game_sound (GameSounds::SHIELD_HIT_YOU),
                    hitpos, &Eye_position);
                // AL 12-15-97: Add missile impact sound even when shield is
                // hit
                if (wip->subtype == WP_MISSILE) {
                    snd_play_3d (
                        gamesnd_get_game_sound (
                            GameSounds::PLAYER_HIT_MISSILE),
                        hitpos, &Eye_position);
                }
            }
            else {
                snd_play_3d (
                    gamesnd_get_game_sound (GameSounds::SHIELD_HIT), hitpos,
                    &Eye_position);
            }
        }
        else {
            ...
            case WP_LASER:
                if (hit_obj == Player_obj)
                    snd_play_3d (
                        gamesnd_get_game_sound (GameSounds::PLAYER_HIT_LASER),
                        hitpos, &Eye_position);
                else {
                    weapon_play_impact_sound (wip, hitpos, is_armed);
                }
            ...
            case WP_MISSILE:
                ...
                break;
            default:
                WARNINGF (LOCATION,"WARNING ==> Cannot determine sound to play for weapon impact");
                break;
            } // end switch
        }
...
extern bool
turret_weapon_has_flags (ship_weapon* swp, Weapon::Info_Flags flags);

/**
 * Distrupt any subsystems that fall into damage sphere of this Electronics
 * missile
 *
 ...
 */
void weapon_do_electronics_effect (
    object* ship_objp, vec3d* blast_pos, int wi_index) {
    weapon_info* wip;
    ship* shipp;
    ship_subsys* ss;
    ...
    for (ss = GET_FIRST (&shipp->subsys_list);
        ...
        if (dist < wip->shockwave.outer_rad) {
            ...
            if (wip->elec_use_new_style) {
                ...
                // if its a turret or weapon subsytem, take the multiplier into
                // account
                if ((psub->type == SUBSYSTEM_TURRET) ||
                    (psub->type == SUBSYSTEM_WEAPONS)) {
                    // disrupt beams
                    // WMC - do this even if there are other types of weapons
                    // on the turret. I figure, the big fancy electronics on
                    // beams will be used for the other weapons as well. No
                    // reason having two targeting computers on a turret. Plus,
                    // it's easy and fast to code. :)
                    if ((psub->type == SUBSYSTEM_TURRET) &&
                        turret_weapon_has_flags (
                            &ss->weapons, Weapon::Info_Flags::Beam)) {
                ...
                if ((psub->type == SUBSYSTEM_SENSORS) ||
                    (psub->flags[Model::Subsystem_Flags::Awacs])) {
 ...
 */
int weapon_area_calc_damage (
    object* objp, vec3d* pos, float inner_rad, float outer_rad,
    float max_blast, float max_damage, float* blast, float* damage,
    float limit) {
    float dist;
    vec3d box_pt;
    ...
    // could possibly exclude SIF_SMALL_SHIP (& other small objects) from using
    // the bbox
    if (objp->type == OBJ_SHIP) {
        int inside = get_nearest_bbox_point (objp, pos, &box_pt);
        if (inside) { dist = 0.0001f; }
        else {
    ...
    if (dist < inner_rad) {
        ...
        *blast = max_blast;
        ...
        float dist_to_outer_rad_squared =
            (outer_rad - dist) * (outer_rad - dist);
        float total_dist_squared =
            (inner_rad - outer_rad) * (inner_rad - outer_rad);

        // this means the inner and outer radii are basically equal... and
        // since we aren't within the inner radius, we fudge the law of
        // excluded middle to place ourselves outside the outer radius
 ...
 */
void weapon_area_apply_blast (
    vec3d* /*force_apply_pos*/, object* ship_objp, vec3d* blast_pos,
    float blast, int make_shockwave) {
#define SHAKE_CONST 3000
    vec3d force, vec_blast_to_ship, vec_ship_to_impact;
    polymodel* pm;

    // don't waste time here if there is no blast force
    if (blast == 0.0f) return;
    ...
    if (make_shockwave) {
        physics_apply_shock (
            &force, blast, &ship_objp->phys_info, &ship_objp->orient,
            &pm->mins, &pm->maxs, pm->rad);
 ...
 * @param other_obj     Object pointer to ship that weapon impacted on (can be
 * NULL)
 */
void weapon_do_area_effect (
    object* wobjp, shockwave_create_info* sci, vec3d* pos, object* other_obj) {
    weapon_info* wip;
    object* objp;
    float damage, blast;
    ...
    for (objp = GET_FIRST (&obj_used_list);
        ...
        if (objp->type == OBJ_WEAPON) {
            ...
            weapon_info* wip2 =
                &Weapon_info[Weapons[objp->instance].weapon_info_index];
            if (wip2->weapon_hitpoints <= 0) continue;
            if (!((wip2->wi_flags[Weapon::Info_Flags::Takes_blast_damage]) ||
                  (wip->wi_flags[Weapon::Info_Flags::Ciws])))
                continue;
        }
        ...
        if (weapon_area_calc_damage (
                objp, pos, sci->inner_rad, sci->outer_rad, sci->blast,
        ...
        switch (objp->type) {
        case OBJ_SHIP:
            // If we're doing an AoE Electronics blast, do the electronics
            // stuff (unless it also has the regular "electronics" flag and
            // this is the ship the missile directly impacted; then leave it
            // for the regular code below) -MageKing17
            if ((wip->wi_flags[Weapon::Info_Flags::Aoe_Electronics]) &&
                !((objp->flags[Object::Object_Flags::Invulnerable]) ||
                  ((objp == other_obj) &&
                   (wip->wi_flags[Weapon::Info_Flags::Electronics])))) {
                weapon_do_electronics_effect (
                    objp, pos, Weapons[wobjp->instance].weapon_info_index);
            }
        ...
        case OBJ_WEAPON:
            ...
            if (target_wip->armor_type_idx >= 0)
                damage = Armor_types[target_wip->armor_type_idx].GetDamage (
                    damage, wip->damage_type_idx, 1.0f);

            objp->hull_strength -= damage;
            if (objp->hull_strength < 0.0f) {
                Weapons[objp->instance].lifeleft = 0.01f;
                Weapons[objp->instance].weapon_flags.set (
...
bool weapon_armed (weapon* wp, bool hit_target) {
    ...
    if ((wp->weapon_flags[Weapon::Weapon_Flags::Destroyed_by_weapon]) &&
        ...
        if (wobj->parent > -1) { pobj = &Objects[wobj->parent]; }
        else {
            pobj = NULL;
        }

        if (((wip->arm_time) &&
             ((Missiontime - wp->creation_time) < wip->arm_time)) ||
            ((wip->arm_dist) &&
             (pobj != NULL && pobj->type != OBJ_NONE &&
              (vm_vec_dist (&wobj->pos, &pobj->pos) < wip->arm_dist)))) {
            return false;
        }
        if (wip->arm_radius && (!hit_target)) {
            if (wp->homing_object == &obj_used_list) return false;
            if (IS_VEC_NULL (&wp->homing_pos) ||
                vm_vec_dist (&wobj->pos, &wp->homing_pos) > wip->arm_radius)
                return false;
        }
    }
 ...
 */
void weapon_hit (
    object* weapon_obj, object* other_obj, vec3d* hitpos, int quadrant,
    vec3d* hitnormal) {
    ASSERT (weapon_obj != NULL);
    if (weapon_obj == NULL) { return; }
    ASSERT (
        (weapon_obj->type == OBJ_WEAPON) && (weapon_obj->instance >= 0) &&
        (weapon_obj->instance < MAX_WEAPONS));
    ...
    if ((weapon_type < 0) || (weapon_type >= MAX_WEAPON_TYPES)) { return; }
    ...
    if (wp->homing_object != &obj_used_list)
        if (wp->homing_object == other_obj) hit_target = true;
    ...
    if (wip->impact_weapon_expl_effect.isValid () && armed_weapon) {
        ...
        if (hitnormal) { particleSource.setOrientationNormal (hitnormal); }

        particleSource.finish ();
    }
    else if (
        wip->dinky_impact_weapon_expl_effect.isValid () && !armed_weapon) {
        auto particleSource = particle::ParticleManager::get ()->createSource (
            wip->dinky_impact_weapon_expl_effect);
        particleSource.moveTo (hitpos);
        particleSource.setOrientationFromVec (&weapon_obj->phys_info.vel);
    ...
    if ((other_obj != nullptr) && (quadrant == -1) &&
        (wip->piercing_impact_effect.isValid () && armed_weapon)) {
        if ((other_obj->type == OBJ_SHIP) || (other_obj->type == OBJ_DEBRIS)) {
            ...
            if (other_obj->type == OBJ_SHIP) {
                ...
                hull_pct =
                    other_obj->hull_strength / shipp->ship_max_hull_strength;
                dmg_type_idx = wip->damage_type_idx;
                draw_limit = Ship_info[shipp->ship_info_index]
                                 .piercing_damage_draw_limit;

                if (shipp->armor_type_idx != -1) {
                    piercing_type =
                        Armor_types[shipp->armor_type_idx].GetPiercingType (
                            dmg_type_idx);
                    if (piercing_type == SADTF_PIERCING_DEFAULT) {
                        draw_limit = Armor_types[shipp->armor_type_idx]
                                         .GetPiercingLimit (dmg_type_idx);
                    }
                    else if (
                        (piercing_type == SADTF_PIERCING_NONE) ||
                        (piercing_type == SADTF_PIERCING_RETAIL)) {
                        ok_to_draw = 0;
                    }
                }

                if (hull_pct > draw_limit) ok_to_draw = 0;
            }

            if (ok_to_draw) {
                using namespace particle;

                auto primarySource = ParticleManager::get ()->createSource (
                    wip->piercing_impact_effect);
                ...
                if (wip->piercing_impact_secondary_effect.isValid ()) {
                    auto secondarySource =
                        ParticleManager::get ()->createSource (
                            wip->piercing_impact_secondary_effect);
                    secondarySource.moveTo (&weapon_obj->pos);
                    secondarySource.setOrientationMatrix (
                        &weapon_obj->last_orient);
    ...
    // For all objects that had this weapon as a target, wipe it out, forcing
    // find of a new enemy
    ...
    if (((other_obj) && (other_obj->type == OBJ_WEAPON)) ||
        (Weapons[num]
             .weapon_flags[Weapon::Weapon_Flags::Destroyed_by_weapon])) {
        sw_flag |= SW_WEAPON_KILL;
    }

    // Which shockwave?
    shockwave_create_info* sci = &wip->shockwave;
    if (!armed_weapon) { sci = &wip->dinky_shockwave; }

    // check if this is an area effect weapon (i.e. has a blast radius)
    if (sci->inner_rad != 0.0f || sci->outer_rad != 0.0f) {
        if (sci->speed > 0.0f) {
            shockwave_create (
                OBJ_INDEX (weapon_obj), hitpos, sci, sw_flag, -1);
        }
        else {
    ...
    if (wip->wi_flags[Weapon::Info_Flags::Emp]) {
        emp_apply (
            &weapon_obj->pos, wip->shockwave.inner_rad,
            wip->shockwave.outer_rad, wip->emp_intensity, wip->emp_time,
            (wip->wi_flags
                 [Weapon::Info_Flags::Use_emp_time_for_capship_turrets]) != 0);
    }

    // if this weapon has the "Electronics" flag set, then disrupt subsystems
    // in sphere
    if ((other_obj != NULL) &&
        (wip->wi_flags[Weapon::Info_Flags::Electronics])) {
        if (other_obj->type == OBJ_SHIP) {
            weapon_do_electronics_effect (
                other_obj, &weapon_obj->pos,
                Weapons[weapon_obj->instance].weapon_info_index);
        }
    }

    // spawn weapons - note the change from FS 1 multiplayer.
    if (wip->wi_flags[Weapon::Info_Flags::Spawn]) {
        if (!((wip->wi_flags[Weapon::Info_Flags::Dont_spawn_if_shot]) &&
              (Weapons[num].weapon_flags
                   [Weapon::Weapon_Flags::
                        Destroyed_by_weapon]))) { // prevent spawning of
                                                  // children if shot down and
                                                  // the dont spawn if shot
                                                  // flag is set (DahBlount)
...
// Group_id:  If you should quad lasers, they should all have the same group
// id. This will be used to optimize lighting, since each group only needs to
// cast one light. Call this to get a new group id, then pass it to each
// weapon_create call for all the weapons in the group.   Number will be
// between 0 and WEAPON_MAX_GROUP_IDS and will get reused.
int weapon_create_group_id () {
    ...
    current_id++;
    if (current_id >= WEAPON_MAX_GROUP_IDS) { current_id = 0; }
    return n;
}
...
void weapon_mark_as_used (int weapon_type) {
    ...
    if (weapon_type < Num_weapon_types) {
        ...
        if (Weapon_info[weapon_type].num_substitution_patterns > 0) {
            for (size_t i = 0;
                 i < Weapon_info[weapon_type].num_substitution_patterns; i++) {
                used_weapons[Weapon_info[weapon_type]
                                 .weapon_substitution_pattern[i]]++;
            }
        }
    }
}
...
void weapons_page_in () {
    ...
    for (i = 0; i < Num_teams; i++) {
        for (j = 0; j < Team_data[i].num_weapon_choices; j++) {
            used_weapons[Team_data[i].weaponry_pool[j]] +=
    ...
    // release anything loaded that we don't have marked as used for this
    // mission
    if (!Cmdline_load_all_weapons) weapon_release_bitmaps ();

    // Page in bitmaps for all used weapons
    for (i = 0; i < Num_weapon_types; i++) {
        if (!Cmdline_load_all_weapons) {
            if (!used_weapons[i]) {
                WARNINGF (LOCATION, "Not loading weapon id %d (%s)", i,Weapon_info[i].name);
                continue;
            }
        }
        ...
        wip->wi_flags.remove (
            Weapon::Info_Flags::Thruster); // Assume no thrusters
        ...
        case WRT_POF: {
            ...
            for (j = 0; j < pm->n_textures; j++) pm->maps[j].PageIn ();
        ...
        default:
            ASSERTX (
                wip->render_type != WRT_POF && wip->render_type != WRT_LASER,
                "Weapon %s does not have a valid rendering type. Type passed: "
                "%d\n",
        ...
        if (wip->wi_flags[Weapon::Info_Flags::Beam]) {
            ...
            for (idx = 0; idx < wip->b_info.beam_num_sections; idx++)
                bm_page_in_texture (
                    wip->b_info.sections[idx].texture.first_frame);
        ...
        if (wip->wi_flags[Weapon::Info_Flags::Particle_spew]) {
            for (size_t s = 0; s < MAX_PARTICLE_SPEWERS;
                 s++) { // looped, multi particle spew -nuke
                if (wip->particle_spewers[s].particle_spew_type !=
                    PSPEW_NONE) {
                    bm_page_in_texture (wip->particle_spewers[s]
                                            .particle_spew_anim.first_frame);
                }
            }
        }
        ...
        // muzzle flashes
        if (wip->muzzle_flash >= 0) mflash_mark_as_used (wip->muzzle_flash);
...
/**
 * Page_in function for cheaters, grabs all weapons that weren't already in a
 * mission and loads the models for them.
 *
 * Non-model graphics elements will get loaded when they are rendered for the
 * first time. Maybe not the best way to do this but faster and a lot less
 * error prone.
 */
 ...
 * Pages in a single weapon and its substitutes and chilren given the
 * weapon_info index for it
 */
bool weapon_page_in (int weapon_type) {
    ASSERT (used_weapons != NULL);

    if (weapon_type < 0 || weapon_type >= Num_weapon_types) { return false; }
    ...
    if (Weapon_info[weapon_type].num_substitution_patterns > 0) {
        for (size_t i = 0;
             i < Weapon_info[weapon_type].num_substitution_patterns; i++) {
            page_in_weapons.push_back (
                Weapon_info[weapon_type].weapon_substitution_pattern[i]);
        }
    }
    ...
    for (size_t x = 0; x < size; x++) {
        if (Weapon_info[page_in_weapons.at (x)].num_spawn_weapons_defined >
            0) {
            for (int j = 0;
                 j <
                 Weapon_info[page_in_weapons.at (x)].num_spawn_weapons_defined;
                 j++) {
                page_in_weapons.push_back (
                    (int)Weapon_info[page_in_weapons.at (x)]
                        .spawn_info[j]
                        .spawn_type);
            }
        }
    ...
    for (size_t k = 0; k < page_in_weapons.size (); k++) {
        if (used_weapons[page_in_weapons.at (k)]) {
            continue; // If weapon is already paged_in, we don't need to page
                      // it in again
        }
        ...
        case WRT_POF: {
            ...
            for (int j = 0; j < pm->n_textures; j++) pm->maps[j].PageIn ();
        ...
        used_weapons[page_in_weapons.at (
            k)]++; // Ensures weapon can be counted as used
    }

    return true;
}

bool weapon_used (int weapon_type) { return used_weapons[weapon_type] > 0; }

/**
 * Get the "color" of the laser at the given moment (since glowing lasers can
 * cycle colors)
 */
void weapon_get_laser_color (color* c, object* objp) {
    ...
    if ((winfo->laser_color_2.red == winfo->laser_color_1.red) &&
        (winfo->laser_color_2.green == winfo->laser_color_1.green) &&
        (winfo->laser_color_2.blue == winfo->laser_color_1.blue)) {
...
void weapon_maybe_spew_particle (object* obj) {
    ...
    ASSERT (Weapon_info[Weapons[obj->instance].weapon_info_index]
                .wi_flags[Weapon::Info_Flags::Particle_spew]);

    wp = &Weapons[obj->instance];
    wip = &Weapon_info[wp->weapon_info_index];
    vec3d spawn_pos, spawn_vel, output_pos, output_vel, input_pos, input_vel;

    for (int psi = 0; psi < MAX_PARTICLE_SPEWERS;
         psi++) { // iterate through spewers    -nuke
        if (wip->particle_spewers[psi].particle_spew_type != PSPEW_NONE) {
            // if the weapon's particle timestamp has elapsed
            if ((wp->particle_spew_time[psi] == -1) ||
                timestamp_elapsed (wp->particle_spew_time[psi])) {
                // reset the timestamp
                wp->particle_spew_time[psi] =
                    timestamp (wip->particle_spewers[0].particle_spew_time);

                // turn normals and origins to world space if we need to
                if (!vm_vec_same (
                        &wip->particle_spewers[psi].particle_spew_offset,
                        &vmd_zero_vector)) { // don't xform unused vectors
                    vm_vec_unrotate (
                        &spawn_pos,
                        &wip->particle_spewers[psi].particle_spew_offset,
                        &obj->orient);
                ...
                if (!vm_vec_same (
                        &wip->particle_spewers[psi].particle_spew_velocity,
                        &vmd_zero_vector)) {
                    vm_vec_unrotate (
                        &spawn_vel,
                        &wip->particle_spewers[psi].particle_spew_velocity,
                ...
                // spew some particles
                if (wip->particle_spewers[psi].particle_spew_type ==
                    PSPEW_DEFAULT) // default pspew type
                {                  // do the default pspew
                    ...
                    for (idx = 0;
                         idx < wip->particle_spewers[psi].particle_spew_count;
                         idx++) {
                        ...
                        // get a velocity vector of some percentage of the
                        // weapon's velocity
                        vel = obj->phys_info.vel;
                        vm_vec_scale (
                            &vel,
                            wip->particle_spewers[psi].particle_spew_vel);

                        // maybe add in offset and initial velocity
                        if (!vm_vec_same (
                                &spawn_vel,
                                &vmd_zero_vector)) { // add in particle
                                                     // velocity if its
                                                     // available
                        ...
                        if (!vm_vec_same (
                                &spawn_pos,
                                &vmd_zero_vector)) { // add offset if available
                            vm_vec_add2 (&direct, &spawn_pos);
                        }

                        if (wip->wi_flags[Weapon::Info_Flags::Corkscrew]) {
                            vm_vec_add (
                                &particle_pos, &obj->last_pos, &direct);
                        }
                        else {
                        ...
                        // emit the particle
                        if (wip->particle_spewers[psi]
                                .particle_spew_anim.first_frame < 0) {
                            particle::create (
                                &particle_pos, &vel,
                                wip->particle_spewers[psi]
                                    .particle_spew_lifetime,
                                wip->particle_spewers[psi]
                                    .particle_spew_radius,
                    ...
                    float segment_length =
                        wip->max_speed *
                        flFrametime; // determine how long the segment is
                    float segment_angular_length =
                        PI2 *
                        wip->particle_spewers[psi]
                            .particle_spew_rotation_rate *
                        flFrametime; // determine how much the segment rotates
                    float rotation_value =
                        (wp->lifeleft * PI2 *
                         wip->particle_spewers[psi]
                             .particle_spew_rotation_rate) +
                        wp->particle_spew_rand; // calculate a rotational start
                                                // point based on remaining
                                                // life
                    float inc =
                        1.0f /
                        wip->particle_spewers[psi]
                            .particle_spew_count; // determine our incriment
                    float particle_rot;
                    vec3d input_pos_l = ZERO_VECTOR;

                    for (float is = 0; is < 1;
                         is += inc) { // use iterator as a scaler
                        particle_rot =
                            rotation_value +
                            (segment_angular_length *
                             is); // find what point of the rotation were at
                        input_vel.xyz.x =
                            sinf (particle_rot) *
                            wip->particle_spewers[psi]
                                .particle_spew_scale; // determine x/y velocity
                                                      // based on scale and
                                                      // rotation
                        input_vel.xyz.y =
                            cosf (particle_rot) *
                        ...
                        input_vel.xyz.z =
                            wip->max_speed *
                            wip->particle_spewers[psi]
                                .particle_spew_vel; // velocity inheritance
                        vm_vec_unrotate (
                            &output_vel, &input_vel,
                            &obj->orient); // orient velocity to weapon
                        input_pos_l.xyz.x =
                            input_vel.xyz.x * flFrametime *
                            (1.0f - is); // interpolate particle motion
                        input_pos_l.xyz.y =
                            input_vel.xyz.y * flFrametime * (1.0f - is);
                        input_pos_l.xyz.z =
                            segment_length *
                            is; // position particle correctly on the z axis
                        vm_vec_unrotate (
                            &input_pos, &input_pos_l,
                            &obj->orient); // orient to weapon
                        vm_vec_sub (
                            &output_pos, &obj->pos,
                            &input_pos); // translate to world space

                        // maybe add in offset and initial velocity
                        if (!vm_vec_same (
                                &spawn_vel,
                                &vmd_zero_vector)) { // add particle velocity
                                                     // if needed
                            vm_vec_add2 (&output_vel, &spawn_vel);
                        }
                        if (!vm_vec_same (
                                &spawn_pos,
                                &vmd_zero_vector)) { // add offset if needed
                    ...
                    for (idx = 0;
                        ...
                        if (wip->particle_spewers[psi].particle_spew_z_scale !=
                            1.0f) { // don't do the extra math for spherical
                                    // effect
                            temp_vel = input_vel;
                            temp_vel.xyz.z *=
                                wip->particle_spewers[psi]
                                    .particle_spew_z_scale; // for an ovoid
                                                            // particle effect
                                                            // to better
                                                            // combine with
                                                            // laser effects
                            vm_vec_unrotate (
                                &input_vel, &temp_vel,
                                &obj->orient); // so it has to be rotated
                        }

                        vm_vec_add2 (
                            &output_vel, &input_vel); // add to weapon velocity
                        output_pos = obj->pos;
                    ...
                    PSPEW_RING) {
                    float inc =
                        PI2 / wip->particle_spewers[psi].particle_spew_count;

                    for (float ir = 0; ir < PI2;
                         ir += inc) { // use iterator for rotation
                        input_vel.xyz.x =
                            sinf (ir) *
                            wip->particle_spewers[psi]
                                .particle_spew_scale; // generate velocity from
                                                      // rotation data
                        input_vel.xyz.y =
                            cosf (ir) *
                            wip->particle_spewers[psi].particle_spew_scale;
                        input_vel.xyz.z =
                            obj->phys_info.fspeed *
                            wip->particle_spewers[psi].particle_spew_vel;
                        vm_vec_unrotate (
                            &output_vel, &input_vel,
                            &obj->orient); // rotate it to model
                    ...
                    for (int i = 0;
                         i < wip->particle_spewers[psi].particle_spew_count;
                         i++) {
                        ...
                        // compute velocity
                        input_vel.xyz.x =
                            wip->particle_spewers[psi].particle_spew_z_scale *
                            -sin_ang;
                        input_vel.xyz.y =
                            wip->particle_spewers[psi].particle_spew_z_scale *
                            -cos_ang;
                        ...
                        vm_vec_unrotate (
                            &output_vel, &input_vel,
                            &obj->orient); // rotate it to model
                        // place particle on a disk prependicular to the weapon
                        // normal and rotate to model space
                        input_pos_l.xyz.x = sin_ang * len_rand;
                        input_pos_l.xyz.y = cos_ang * len_rand;
                        vm_vec_unrotate (
                            &input_pos, &input_pos_l,
                            &obj->orient); // rotate to world
                        vm_vec_sub (
                            &output_pos, &obj->pos,
                            &input_pos); // translate to world
...
DCF (pspew, "Particle spew help and status provider") {
    ...
    dc_printf (
        "Passing '--status' as an argument to any of the individual spew "
        "commands will show the status of that variable only.\n\n");

    dc_printf (
        "These commands adjust the various properties of the particle spew "
        "system, which is used by weapons when they are fired, are in-flight, "
        "and die (either by impact or by end of life time.\n");
    dc_printf (
        "Generally, a large particle count with small size and scale will "
        "result in a nice dense particle spew.\n");
    dc_printf (
        "Be advised, this effect is applied to _ALL_ weapons, and as such may "
        "drastically reduce framerates on lower powered platforms.\n");
}
...
float weapon_get_damage_scale (weapon_info* wip, object* wep, object* target) {
    ...
    // sanity
    if ((wip == NULL) || (wep == NULL) || (target == NULL)) { return 1.0f; }

    // don't scale any damage if its not a weapon
    if ((wep->type != OBJ_WEAPON) || (wep->instance < 0) ||
        (wep->instance >= MAX_WEAPONS)) {
        return 1.0f;
    }
    ...
    from_player = 0;
    if ((wep->parent >= 0) && (wep->parent < MAX_OBJECTS) &&
        (Objects[wep->parent].flags[Object::Object_Flags::Player_ship])) {
        from_player = 1;
    }

    // if this is a lockarm weapon, and it was fired unlocked
    if ((wip->wi_flags[Weapon::Info_Flags::Lockarm]) &&
        !(wp->weapon_flags[Weapon::Weapon_Flags::Locked_when_fired])) {
        total_scale *= 0.1f;
    }
    ...
    if ((target->type == OBJ_SHIP) &&
        !(The_mission.ai_profile
              ->flags[AI::Profile_Flags::Disable_weapon_damage_scaling]) &&
        !(Ship_info[Ships[target->instance].ship_info_index]
              .flags[Ship::Info_Flags::Disable_weapon_damage_scaling])) {
        ship_info* sip;
        ...
        if ((sip->flags[Ship::Info_Flags::Supercap]) &&
            !(wip->wi_flags[Weapon::Info_Flags::Supercap])) {
            // if the supercap is around 3/4 damage, apply nothing
            if (hull_pct <= 0.75f) { return 0.0f; }
            else {
        ...
        if (is_big_damage_ship && !(wip->hurts_big_ships ())) {
            ...
            if (from_player &&
                !(The_mission.ai_profile
                      ->flags[AI::Profile_Flags::Player_weapon_scale_fix])) {
                // if it's a laser weapon
                if (wip->subtype == WP_LASER) { total_scale *= 0.01f; }
                else {
                    total_scale *= 0.05f;
                }
            }
            ...
            // scale based on hull
            if (hull_pct > 0.1f) { total_scale *= hull_pct; }
            else {
                return 0.0f;
            }
        }
...
void pause_in_flight_sounds () {
    for (int i = 0; i < MAX_WEAPONS; i++) {
        if (Weapons[i].objnum != -1) {
            ...
            if (wp->hud_in_flight_snd_sig.isValid () &&
                snd_is_playing (wp->hud_in_flight_snd_sig)) {
                // Stop sound, it will be restarted in the first frame after
...
void shield_impact_explosion (
    vec3d* hitpos, object* objp, float radius, int idx) {
    int expl_ani_handle = Weapon_explosions.GetAnim (idx, hitpos, radius);
    particle::create (
        hitpos, &vmd_zero_vector, 0.0f, radius,
        particle::PARTICLE_BITMAP_PERSISTENT, expl_ani_handle, objp);
}
...
void weapon_render (object* obj, model_draw_list* scene) {
    ...
    if (wip->wi_flags[Weapon::Info_Flags::Transparent]) {
        if (wp->alpha_current == -1.0f) { wp->alpha_current = wip->alpha_max; }
    ...
    switch (wip->render_type) {
    case WRT_LASER: {
        if (wip->laser_length < 0.0001f) return;
        ...
        if (wip->laser_bitmap.first_frame >= 0) {
            ...
            if (wip->laser_bitmap.num_frames > 1) {
                ...
                framenum = bm_get_anim_frame (
                    wip->laser_bitmap.first_frame, wp->laser_bitmap_frame,
                    wip->laser_bitmap.total_time, true);
            }
            ...
            vm_vec_scale_add (
                &headp, &obj->pos, &obj->orient.vec.fvec, wip->laser_length);

            batching_add_laser (
                wip->laser_bitmap.first_frame + framenum, &headp,
                wip->laser_head_radius, &obj->pos, wip->laser_tail_radius,
                alpha, alpha, alpha);
        }
        ...
        if (wip->laser_glow_bitmap.first_frame >= 0) {
            ...
            // *Tail point "getting bigger" as well as headpoint isn't being
            // taken into consideration, so
            // it caused uneven glow between the head and tail, which really
            // shows in big lasers. So...fixed!    -Et1
            vec3d headp2, tailp;

            vm_vec_scale_add (
                &headp2, &obj->pos, &obj->orient.vec.fvec,
                wip->laser_length * weapon_glow_scale_l);
            vm_vec_scale_add (
                &tailp, &obj->pos, &obj->orient.vec.fvec,
            ...
            if (wip->laser_glow_bitmap.num_frames > 1) {
                ...
                while (wp->laser_glow_bitmap_frame >
            ...
            batching_add_laser (
                wip->laser_glow_bitmap.first_frame + framenum, &headp2,
                wip->laser_head_radius * weapon_glow_scale_f, &tailp,
                wip->laser_tail_radius * weapon_glow_scale_r,
                (c.red * alpha) / 255, (c.green * alpha) / 255,
                (c.blue * alpha) / 255);
        }
    ...
    case WRT_POF: {
        ...
        if ((wip->wi_flags[Weapon::Info_Flags::Thruster]) &&
            ((wp->thruster_bitmap > -1) || (wp->thruster_glow_bitmap > -1))) {
            float ft;
            mst_info mst;
        ...
        model_render_queue (
            &render_info, scene, wip->model_num, &obj->orient, &obj->pos);

        break;
    }

    default:
        WARNINGF (LOCATION, "Unknown weapon rendering type = %i for weapon %s",wip->render_type, wip->name);
    }
}

// Called by hudartillery.cpp after SSMs have been parsed to make sure that
// $SSM: entries defined in weapons are valid.
void validate_SSM_entries () {
    int wi;
    std::vector< std::string >::const_iterator it;
    weapon_info* wip;

    for (it = Delayed_SSM_names.begin (); it != Delayed_SSM_names.end ();
         ++it) {
        ...
        ASSERTX (
            ...
            it->c_str ());
        wip = &Weapon_info[wi];
        WARNINGF (LOCATION,"Starting validation of '%s' [wip->name is '%s'], currently has an SSM_index of %d.",it->c_str (), wip->name, wip->SSM_index);
        wip->SSM_index = ssm_info_lookup (dat->ssm_entry.c_str ());
        if (wip->SSM_index < 0) {
            WARNINGF (LOCATION,"Unknown SSM entry '%s' in specification for %s (%s:line %d).",dat->ssm_entry.c_str (), it->c_str (), dat->filename.c_str (),dat->linenum);
        }
        WARNINGF (LOCATION, "Validation complete, SSM_index is %d.",wip->SSM_index);
    }
    ...
    for (it = Delayed_SSM_indices.begin (); it != Delayed_SSM_indices.end ();
         ++it) {
        ...
        if (wip->SSM_index < -1 ||
            ...
            WARNINGF (LOCATION,"Invalid SSM index '%d' (should be 0-%zu) in specification for %s (%s:line %d).",wip->SSM_index, Ssm_info.size () - 1, it->c_str (),dat->filename.c_str (), dat->linenum);
            wip->SSM_index = -1;
        }
        WARNINGF (LOCATION, "Validation complete, SSM-index is %d.",wip->SSM_index);
    }
}

weapon_info::weapon_info () { this->reset (); }
...
void weapon_info::reset () {
    ...
    this->flak_targeting_accuracy =
        60.0f; // Standard value as defined in flak.cpp
    ...
    this->impact_weapon_expl_effect =
        particle::ParticleEffectHandle::invalid ();

    this->dinky_impact_weapon_expl_effect =
        particle::ParticleEffectHandle::invalid ();

    this->flash_impact_weapon_expl_effect =
        particle::ParticleEffectHandle::invalid ();

    this->piercing_impact_effect = particle::ParticleEffectHandle::invalid ();
    this->piercing_impact_secondary_effect =
        particle::ParticleEffectHandle::invalid ();

    this->muzzle_flash = -1;

    this->emp_intensity = EMP_DEFAULT_INTENSITY;
    this->emp_time = EMP_DEFAULT_TIME; // Goober5000: <-- Look!  I fixed a
                                       // Volition bug!  Gimme $5, Dave!
    ...
    for (size_t s = 0; s < MAX_PARTICLE_SPEWERS;
         s++) { // default values for everything -nuke
        this->particle_spewers[s].particle_spew_type =
            PSPEW_NONE; // added by nuke
        ...
        this->particle_spewers[s].particle_spew_z_scale =
            1.0f; // added by nuke
        ...
        generic_anim_init (
            &this->particle_spewers[s].particle_spew_anim, NULL);
    }

    this->tag_level = -1;
    this->tag_time = -1.0f;

    this->SSM_index = -1; // tag C SSM index, wich entry in the SSM table this
                          // weapon calls -Bobboau
...
const char* weapon_info::get_display_string () {
    if (has_alternate_name ()) { return alt_name; }
    else {
        return name;
    }
}
bool weapon_info::has_alternate_name () { return alt_name[0] != '\0'; }
