...
int weapon_info_lookup(const char *name)
{
        ...
        for (int i = 0; i < Num_weapon_types; i++)
                if (!strcasecmp(name, Weapon_info[i].name))
                        return i;
                ...
                if (!strncasecmp(spawn_str, flag_text.c_str(), 5)) {
                        if (weaponp->num_spawn_weapons_defined < MAX_SPAWN_TYPES_PER_WEAPON) {
                                ...
                                weaponp->spawn_info[weaponp->num_spawn_weapons_defined]
                                        .spawn_type
                                        = (short)Num_spawn_types;
                                skip_length = spawn_str_len + strspn(&temp_string[spawn_str_len], NOX(" \t"));
                                char *num_start = strchr(&temp_string[skip_length], ',');
                                if (num_start == NULL) {
                                        weaponp->spawn_info[weaponp->num_spawn_weapons_defined]
                                                .spawn_count
                                                = DEFAULT_WEAPON_SPAWN_COUNT;
                                        name_length = 999;
                                } else {
                                        weaponp->spawn_info[weaponp->num_spawn_weapons_defined]
                                                .spawn_count
                                                = (short)atoi(num_start + 1);
                                        name_length = num_start - temp_string.get() - skip_length;
                                }

                                weaponp->total_children_spawned += weaponp->spawn_info[weaponp->num_spawn_weapons_defined]
                                                                           .spawn_count;

                                Spawn_names[Num_spawn_types] = strndup(&flag_text[skip_length], name_length);
...
int parse_weapon(int subtype, bool replace, const char *filename)
{
        ...
        if (optional_string("@Laser Color:")) {
                ...
                bool reset = (!first_time && ((wip->laser_color_1.red == wip->laser_color_2.red) && (wip->laser_color_1.green == wip->laser_color_2.green) && (wip->laser_color_1.blue == wip->laser_color_2.blue)));
                stuff_ubyte(&r);
                stuff_ubyte(&g);
                stuff_ubyte(&b);
        ...
        if (optional_string("$Mass:")) {
                ...
                if ((!strcmp(wip->name, "SAAA") && (wip->mass == 4.0f)) || (!strcmp(wip->name, "MjolnirBeam") && (wip->mass == 1000.0f)) || (!strcmp(wip->name, "MjolnirBeam#home") && (wip->mass == 1000.0f))) {
                        wip->mass = 100.0f;
                }
        ...
        if (is_homing || (wip->is_homing())) {
                ...
                if (wip->wi_flags[Weapon::Info_Flags::Homing_heat]) {
                        ...
                        (wip->wi_flags[Weapon::Info_Flags::Homing_aspect]) || (wip->wi_flags[Weapon::Info_Flags::Homing_javelin])) {
                        if (optional_string("+Turn Time:")) {
                                stuff_float(&wip->turn_time);
                        }
        ...
        if (optional_string("$Trail:")) {
                ...
                if (optional_string("+Max Life:")) {
                        ...
                        ti->stamp = int(1000.0f * ti->max_life) / (NUM_TRAIL_SECTIONS + 1);
                }
        ...
        if (optional_string("$Impact Effect:")) {
                ...
                if (bitmapIndex >= 0 && size > 0.0f) {
                        ...
                        if (subtype == WP_BEAM) {
                                ...
                                auto singleEffect = effects::SingleParticleEffect::createInstance(
        ...
        if (optional_string("$Shield Impact Explosion Radius:")) {
                ...
                if (wip->impact_weapon_expl_effect.isValid()) {
                        auto singleEffect = dynamic_cast< effects::SingleParticleEffect * >(
        ...
        if (optional_string("$BeamInfo:")) {
                        ...
                        if (bitmapIndex >= 0 && size > 0.0f) {
                                ...
                                if (defaultEffect) {
                                        ...
                                        auto singleEffect = effects::SingleParticleEffect::createInstance(
                                                bitmapIndex, size * 1.2f, size * 1.9f,
                                                size * 1.5f * 0.005f);
                                        wip->flash_impact_weapon_expl_effect = ParticleManager::get()->addEffect(singleEffect);
                                } else {
                                        auto singleEffect = effects::SingleParticleEffect::createInstance(
                                                bitmapIndex, size * 1.2f, size * 1.9f);
                                        wip->flash_impact_weapon_expl_effect = ParticleManager::get()->addEffect(singleEffect);
                ...
                if (optional_string("+Beam Piercing Particle Effect:")) {
                        ...
                        if (bitmapIndex >= 0 && radius > 0.0f) {
                                ...
                                wip->piercing_impact_effect = ParticleManager::get()->addEffect(piercingEffect);
                        }
                }
                ...
                if (optional_string("+Remove")) {
                        ...
                        if (spew_index < 0) {
                                ...
                                if (optional_string(
                                        ...
                                        wip->particle_spewers[spew_index].particle_spew_type == PSPEW_NONE) { // make sure the omission of type wasn't to
        ...
        if (optional_string("$Weapon Hitpoints:")) {
                ...
                first_time && (wip->wi_flags[Weapon::Info_Flags::Turret_Interceptable, Weapon::Info_Flags::Fighter_Interceptable])) {
        ...
        try {
                ...
                if (optional_string("#Secondary Weapons")) {
                        while (required_string_either("#End", "$Name:")) {
                                // AL 28-3-98: If parse_weapon() fails, try next .tbl weapon
                                if (parse_weapon(
                                            WP_MISSILE, Parsing_modular_table, filename)
                                    < 0) {
                                        continue;
                                }
                        }
        ...
        if (wip->wi_flags[Weapon::Info_Flags::Particle_spew]) {
                for (size_t s = 0; s < MAX_PARTICLE_SPEWERS;
                     s++) { // looperfied for multiple pspewers -nuke
                        if (wip->particle_spewers[s].particle_spew_type != PSPEW_NONE) {
                                if ((wip->particle_spewers[s].particle_spew_anim.first_frame < 0) && (wip->particle_spewers[s].particle_spew_anim.filename[0] != '\0')) {
                                        wip->particle_spewers[s].particle_spew_anim.first_frame = bm_load(wip->particle_spewers[s]
                                        ...
                                        if (wip->particle_spewers[s]
                                            ...
                                            >= 0) {
                                                wip->particle_spewers[s]
                                                        .particle_spew_anim.num_frames
                                                        = 1;
                                                wip->particle_spewers[s]
                                                        .particle_spew_anim.total_time
                                                        = 1;
                                        }
                                        // fall back to an animated type
        ...
        for (objp = GET_FIRST(&obj_used_list);
             objp != END_OF_LIST(&obj_used_list); objp = GET_NEXT(objp)) {
                if ((objp->type == OBJ_SHIP) || ((objp->type == OBJ_WEAPON) && (Weapon_info[Weapons[objp->instance].weapon_info_index].wi_flags[Weapon::Info_Flags::Cmeasure]))) {
                        ...
                        if ((objp->flags[Object::Object_Flags::Protected]) && ((wp->weapon_flags[Weapon::Weapon_Flags::Spawned]) || (wip->wi_flags[Weapon::Info_Flags::Untargeted_heat_seeker])))
                                continue;
                        ...
                        if (iff_x_attacks_y(wp->team, homing_object_team)) {
                                if (objp->type == OBJ_SHIP) {
                                        ...
                                        if ((wip->wi_flags[Weapon::Info_Flags::Huge]) && (sip->is_small_ship() || !sip->is_flyable() || sip->is_harmless())) {
                                                continue;
                                        }
...
bool aspect_should_lose_target(weapon *wp)
{
        ...
        if (wp->homing_object->signature != wp->target_sig) {
                if (wp->homing_object->type == OBJ_WEAPON) {
                        weapon_info *target_info = &Weapon_info[Weapons[wp->homing_object->instance]
                                                                        .weapon_info_index];

                        if (target_info->wi_flags[Weapon::Info_Flags::Cmeasure]) {
                                // Check if we can home on this countermeasure
                                bool home_on_cmeasure = The_mission.ai_profile->flags
...
void weapon_home(object *obj, int num, float frame_time)
{
        ...
        if ((hobjp == &obj_used_list) || (f2fl(Missiontime - wp->creation_time) < (wip->free_flight_time / 2))) {
                if (f2fl(Missiontime - wp->creation_time) > wip->free_flight_time) {
                        ...
                        // their target
                                ...
                                wip->is_locked_homing() && wip->wi_flags[Weapon::Info_Flags::Die_on_lost_lock]) {
                                if (wp->lifeleft > 0.5f) {
                                        wp->lifeleft = fs2::prng::randf(0, 0.1f, 0.5f); // randomise a bit to avoid multiple
                ...
                // since free_flight_time can now be 0, guard against that
                        ...
                        if (obj->phys_info.speed > max_speed) {
                                ...
                                (obj->phys_info.speed < max_speed / (2 / wip->free_flight_time)) && (wip->wi_flags[Weapon::Info_Flags::Homing_heat])) {
        ...
        if ((wip->wi_flags[Weapon::Info_Flags::Homing_javelin]) && (hobjp->type == OBJ_SHIP) && (wp->target_sig > 0) && (wp->homing_subsys != NULL) && (wp->homing_subsys->system_info->type != SUBSYSTEM_ENGINE)) {
        ...
        if ((wip->wi_flags[Weapon::Info_Flags::Homing_javelin]) && (hobjp->type == OBJ_SHIP) && (wp->target_sig > 0) && (wp->homing_subsys == NULL)) {
                wp->homing_object = &obj_used_list;
                return;
        }
        ...
        case OBJ_SHIP:
                ...
                break;
        case OBJ_WEAPON: {
                bool home_on_cmeasure = The_mission.ai_profile
        ...
        if (hobjp != &obj_used_list) {
                ...
                if ((wp->homing_subsys != NULL) && !(wip->wi_flags[Weapon::Info_Flags::Non_subsys_homing]) && hobjp->type == OBJ_SHIP) {
                        ...
                        } else if (pick_homing_point || dist < 500.0f || fs2::prng::randf(0) < flFrametime * 2.) {
                                if (hobjp->type == OBJ_SHIP) {
                ...
                if ((dist_to_target < flFrametime * obj->phys_info.speed * 4.0f + 10.0f) && (old_dot < wip->fov) && (wp->lifeleft > 0.01f) && (wp->homing_object != &obj_used_list) && (wp->homing_object->type == OBJ_SHIP)) {
                        wp->lifeleft = 0.01f;
                }
                ...
                if ((old_dot > 0.1f) && (time_to_target > 0.1f)) {
                        if (wip->wi_flags[Weapon::Info_Flags::Variable_lead_homing]) {
                                vm_vec_scale_add2(
                                        ...
                                        (0.33f * wip->target_lead_scaler * MIN(time_to_target, 6.0f)));
                ...
                if (wip->wi_flags[Weapon::Info_Flags::Homing_heat]) {
                        if ((old_dot < wip->fov) && (dist_to_target > wip->shockwave.inner_rad * 1.1f)) { // Delay finding new target
                ...
                if (wip->acceleration_time > 0.0f) {
                        ...
                        !(wip->wi_flags[Weapon::Info_Flags::No_homing_speed_ramp]) && Missiontime - wp->creation_time < i2f(1)) {
                        // Default behavior:
...
static void weapon_update_state(weapon *wp)
{
        ...
        if (wip->free_flight_time) {
                ...
                if (lifetime < fl2f(wip->free_flight_time)) {
                        ...
                        lifetime >= fl2f(wip->free_flight_time) && (lifetime - Frametime) <= fl2f(wip->free_flight_time) && wp->homing_object != nullptr) {
...
// maybe play a "whizz sound" if close enough to view position
        ...
        if (wip->is_homing() && !(wp->weapon_flags[Weapon::Weapon_Flags::No_homing])) {
                ...
                if (Missiontime - wp->creation_time < fl2f(wip->acceleration_time)) {
                        ...
                        obj->phys_info.flags |= PF_CONST_VEL; // Max speed reached, can use simpler physics
                                                              // calculations now
                }
        ...
        if (parent_objp == NULL || Ships[parent_objp->instance].ai_index >= 0) {
                ...
                if (target_objnum != -1 && !targeting_same) {
                        ...
                        if ((wip->wi_flags[Weapon::Info_Flags::Homing_aspect]) && target_is_locked) {
                                ...
                                (wip->wi_flags[Weapon::Info_Flags::Homing_javelin]) && target_is_locked) {
                                if ((Objects[target_objnum].type == OBJ_SHIP) && ((wp->homing_subsys == NULL) || (wp->homing_subsys->system_info->type != SUBSYSTEM_ENGINE))) {
        ...
        ship_subsys *src_turret)
{
        ...
        if ((wip->num_substitution_patterns > 0) && (parent_objp != NULL)) {
                ...
                ASSERTX(
                        ...
                        parent_objp->type);
                ASSERTX(
                        (parent_objp->instance < MAX_SHIPS) && (parent_objp->instance >= 0),
                ...
                if (((Weapon_info[weapon_type].subtype == WP_LASER && Player_ship->flags[Ship::Ship_Flags::Primary_linked]) || (Weapon_info[weapon_type].subtype == WP_MISSILE && Player_ship->flags[Ship::Ship_Flags::Secondary_dual_fire])) && (curr_pos > 0)) {
                        curr_pos--;
                }
                ++(*position);
        ...
        if (parent_objp != NULL) {
                ASSERT(
                        parent_objp->type == OBJ_SHIP); // Get Mike, a non-ship has fired a weapon!
                ASSERT(
                ...
                for (size_t s = 0; s < MAX_PARTICLE_SPEWERS;
                     s++) { // allow for multiple time values
                        if (wip->particle_spewers[s].particle_spew_type != PSPEW_NONE) {
                                ...
                                wp->particle_spew_rand = fs2::prng::randf(0, 0, PI2); // per weapon randomness
                        }
                }
        }
        ...
        if (wip->wi_flags[Weapon::Info_Flags::Cmeasure]) {
                ...
                if (wip->cmeasure_timer_interval > 0) {
                        ...
                        wp->cmeasure_timer = timestamp(); // Could also use timestamp(0), but it doesn't
...
void spawn_child_weapons(object *objp)
{
        ...
        if (objp->type == OBJ_WEAPON) {
                ...
                ASSERTX(
                        (wp->weapon_info_index >= 0) && (wp->weapon_info_index < Num_weapon_types),
                ...
                ASSERTX(
                        (bp->weapon_info_index >= 0) && (bp->weapon_info_index < Num_weapon_types),
                        "Invalid weapon_info_index of %d; get a coder!\n",
        ...
        for (objp = GET_FIRST(&obj_used_list);
             objp != END_OF_LIST(&obj_used_list); objp = GET_NEXT(objp)) {
                ...
                if (weapon_area_calc_damage(
                            ...
                            sci->damage, &blast, &damage, sci->outer_rad)
                    == -1) {
                        continue;
                }
...
bool weapon_armed(weapon *wp, bool hit_target)
{
        ...
        if ((wp->weapon_flags[Weapon::Weapon_Flags::Destroyed_by_weapon]) && !wip->arm_time && wip->arm_dist == 0.0f && wip->arm_radius == 0.0f) {
                return false;
        } else {
                ...
                if (((wip->arm_time) && ((Missiontime - wp->creation_time) < wip->arm_time)) || ((wip->arm_dist) && (pobj != NULL && pobj->type != OBJ_NONE && (vm_vec_dist(&wobj->pos, &pobj->pos) < wip->arm_dist)))) {
                        return false;
                }
        ...
        vec3d *hitnormal)
{
        ...
        ASSERT(
                (weapon_obj->type == OBJ_WEAPON) && (weapon_obj->instance >= 0) && (weapon_obj->instance < MAX_WEAPONS));
                ...
                if ((other_obj->type == OBJ_SHIP) || (other_obj->type == OBJ_DEBRIS)) {
                        ...
                        if (other_obj->type == OBJ_SHIP) {
                                ...
                                if (shipp->armor_type_idx != -1) {
                                        piercing_type = Armor_types[shipp->armor_type_idx].GetPiercingType(
                                                dmg_type_idx);
                                        if (piercing_type == SADTF_PIERCING_DEFAULT) {
                                                draw_limit = Armor_types[shipp->armor_type_idx]
                                                                     .GetPiercingLimit(dmg_type_idx);
                                        } else if (
                                                (piercing_type == SADTF_PIERCING_NONE) || (piercing_type == SADTF_PIERCING_RETAIL)) {
                                                ok_to_draw = 0;
                                        }
                                }
                        ...
                        if (ok_to_draw) {
                                ...
                                if (wip->piercing_impact_secondary_effect.isValid()) {
                                        auto secondarySource = ParticleManager::get()->createSource(
        ...
        if (wip->wi_flags[Weapon::Info_Flags::Emp]) {
                emp_apply(
                        ...
                        (wip->wi_flags
                                 [Weapon::Info_Flags::Use_emp_time_for_capship_turrets])
                                != 0);
        }
        ...
        if (wip->wi_flags[Weapon::Info_Flags::Spawn]) {
                if (!((wip->wi_flags[Weapon::Info_Flags::Dont_spawn_if_shot]) && (Weapons[num].weapon_flags[Weapon::Weapon_Flags::Destroyed_by_weapon]))) { // prevent spawning of
...
bool weapon_page_in(int weapon_type)
{
        ...
        for (size_t x = 0; x < size; x++) {
                if (Weapon_info[page_in_weapons.at(x)].num_spawn_weapons_defined > 0) {
                        for (int j = 0;
                             j < Weapon_info[page_in_weapons.at(x)].num_spawn_weapons_defined;
                             j++) {
                                page_in_weapons.push_back(
...
void weapon_get_laser_color(color *c, object *objp)
{
        ...
        if (pct > 0.0f) {
                ...
                r += int((winfo->laser_color_2.red - winfo->laser_color_1.red) * pct);
                g += int(
                ...
                if (wip->particle_spewers[psi].particle_spew_type != PSPEW_NONE) {
                        ...
                        if ((wp->particle_spew_time[psi] == -1) || timestamp_elapsed(wp->particle_spew_time[psi])) {
                                ...
                                {                                                                   // do the default pspew
                                        ...
                                        wip->particle_spewers[psi].particle_spew_type == PSPEW_HELIX) {                                                                // helix
                                        ...
                                        float rotation_value = (wp->lifeleft * PI2 * wip->particle_spewers[psi].particle_spew_rotation_rate) + wp->particle_spew_rand; // calculate a rotational start
                                        ...
                                        for (float is = 0; is < 1;
                                             is += inc) {                                                                              // use iterator as a scaler
                                                ...
                                                input_vel.xyz.x = sinf(particle_rot) * wip->particle_spewers[psi].particle_spew_scale; // determine x/y velocity
                                                                                                                                       // based on scale and
                                                                                                                                       // rotation
                                        ...
                                        wip->particle_spewers[psi].particle_spew_type == PSPEW_SPARKLER) { // sparkler
                                        vec3d temp_vel;
                                        ...
                                        for (idx = 0;
                                             ...
                                             idx++) {
                                                ...
                                                if (wip->particle_spewers[psi].particle_spew_z_scale != 1.0f) { // don't do the extra math for spherical
                                                        // effect
                                                        temp_vel = input_vel;
                                                        temp_vel.xyz.z *= wip->particle_spewers[psi]
                                        ...
                                        wip->particle_spewers[psi].particle_spew_type == PSPEW_RING) {
                                        float inc = PI2 / wip->particle_spewers[psi].particle_spew_count;

                                        for (float ir = 0; ir < PI2;
                                             ir += inc) {                                                                    // use iterator for rotation
                                                input_vel.xyz.x = sinf(ir) * wip->particle_spewers[psi].particle_spew_scale; // generate velocity from
                                                                                                                             // rotation data
                                        ...
                                        wip->particle_spewers[psi].particle_spew_type == PSPEW_PLUME) {
                                        ...
                                        for (int i = 0;
                                             ...
                                             i++) {
                                                ...
                                                len_rand = fs2::prng::randf(0) * wip->particle_spewers[psi].particle_spew_scale;
...
float weapon_get_damage_scale(weapon_info *wip, object *wep, object *target)
{
        ...
        if ((target->type == OBJ_SHIP) && !(The_mission.ai_profile->flags[AI::Profile_Flags::Disable_weapon_damage_scaling]) && !(Ship_info[Ships[target->instance].ship_info_index].flags[Ship::Info_Flags::Disable_weapon_damage_scaling])) {
                ship_info *sip;
        ...
        case WRT_LASER: {
                ...
                if (wip->laser_glow_bitmap.first_frame >= 0) {
                        ...
                        if (wip->laser_glow_bitmap.num_frames > 1) {
                                ...
                                framenum = int(
                                        (wp->laser_glow_bitmap_frame * wip->laser_glow_bitmap.num_frames) / wip->laser_glow_bitmap.total_time);
