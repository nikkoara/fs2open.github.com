...
void flak_pick_range (
    ...
    // distance if one was defined.
    if (wip->arm_dist > 0.0f) {
        if (final_range < wip->arm_dist) { final_range = wip->arm_dist; }
    }
    else {
        if (final_range < 10.0f) { final_range = 10.0f; }
    }
...
void flak_jitter_aim (
    ...
    // get the matrix needed to rotate the base direction to the actual
    // direction
    vm_vector_2_matrix (&temp, dir, NULL, NULL);

    // error value
    error_val =
        wip->flak_targeting_accuracy + (wip->flak_targeting_accuracy * 0.65f *
                                        (1.0f - weapon_subsys_strength));
    ...
    // now rotate the twist vector around the x axis (the base aim axis) at a
    // random angle
...
void flak_muzzle_flash (
    ...
    if (Weapon_info[turret_weapon_class].muzzle_flash < 0) { return; }

    mflash_create (
        pos, dir, pip, Weapon_info[turret_weapon_class].muzzle_flash);
}
