...
// beam types
// REMINDER : if you change the behavior of any of these beam types, make sure
// to update their "cones" of possible movement inside of the function
// beam_get_cone_dot(...) in beam.cpp  Otherwise it could cause collisions to
...
#define BEAM_TYPE_D \
    3 // similar to the type A beams, but takes multiple shots and "chases"
      // fighters around
#define BEAM_TYPE_E \
    4 // stupid beam. like type A, only it doesn't aim. it just shoots directly
      // out of the turret
...
struct beam_fire_info  {
    ...
    vec3d targeting_laser_offset; // offset from the center of the object (for
                                  // targeting lasers only)
    ship_subsys* turret;          // where he's shooting from
    float
        accuracy; // 0.0 to 1.0 (only really effects targeting on small ships)
    object* target;             // who's getting shot
    ship_subsys* target_subsys; // (optional), specific subsystem to be
                                // targeted on the target
    vec3d target_pos1;          // if we're shooting off into space
    vec3d target_pos2;  // if we're shooting off into space (optional second
                        // point)
    vec3d starting_pos; // starting positiong for floating beams -MageKing17
    beam_info* beam_info_override; // (optional), pass this in to override all
                                   // beam movement info (for multiplayer)
...
// beam flag defines
#define BF_SAFETY \
    (1 << 0) // if this is set, don't collide or render for this frame.
             // lifetime still increases though
...
struct beam  {
    ...
    object* objp;   // the shooting object (who owns the turret that I am being
                    // fired from)
    ...
    vec3d target_pos2;   // if we're targeting a location in space (optional
                         // second point)
    ...
    // this vector has very special meaning. BEFORE performing collision
    // checks, it basically implies a "direction". meaning the vector between
    // it and last_start is where the beam will be aiming. AFTER performing
    // collision checks, it is the literal world collision point on the object
    // (or meaningless, if we hit nothing). The function beam_move_all_pre() is
    ...
    float beam_section_frame[MAX_BEAM_SECTIONS]; // what frame a beam section
                                                 // animation is on
    ...
    beam_collision
        f_collisions[MAX_FRAME_COLLISIONS]; // collisions for the current frame
    ...
    // exactly how the beam will behave. by passing this is multiplayer from
    // server to client, we can ensure that everything looks the same
    beam_info binfo;
    int bank;
...
// ---------------
// the next functions are probably the only ones anyone should care about
// calling. the rest require somewhat detailed knowledge of beam weapons

// fire a beam, returns objnum on success. the innards of the code handle all
// the rest, foo
...
#endif // FREESPACE2_WEAPON_BEAM_HH
