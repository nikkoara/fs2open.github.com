...
int shockwave_create (
    ...
    for (i = 0; i < MAX_SHOCKWAVES; i++) {
        if (!(Shockwaves[i].flags & SW_USED)) break;
    }

    if (i == MAX_SHOCKWAVES) return -1;

    // try 2D shockwave first, then fall back to 3D, then fall back to default
    // of either this should be pretty fool-proof and allow quick change
    // between 2D and 3D effects
    if (strlen (sci->name)) info_index = shockwave_load (sci->name, false);

    if ((info_index < 0) && strlen (sci->pof_name))
        info_index = shockwave_load (sci->pof_name, true);

    if (info_index < 0) {
        if ((Shockwave_info[0].bitmap_id >= 0) ||
    ...
    sw->pos = *pos;
    sw->num_objs_hit = 0;
    sw->shockwave_info_index = info_index; // only one type for now... type
                                           // could be passed is as a parameter
    sw->current_bitmap = -1;
    ...
    if ((parent_objnum != -1) &&
        Objects[parent_objnum].type == OBJ_WEAPON) { // Goober5000: allow -1
...
void shockwave_set_framenum (int index) {
    ...
    // skip this if it's a 3d shockwave since it won't have the maps managed
    // here
    if (si->bitmap_id < 0) return;

    sw->current_bitmap =
        si->bitmap_id + shockwave_get_framenum (index, si->bitmap_id);
}

/**
 * Given a shockwave index and the animation id/handle return what the current
 * frame num should be
 *
...
void shockwave_move (object* shockwave_objp, float frametime) {
    ...
    if (sw->delay_stamp != -1) {
        if (timestamp_elapsed (sw->delay_stamp)) { sw->delay_stamp = -1; }
        else {
            return;
        }
    }
    ...
    for (objp = GET_FIRST (&obj_used_list);
        ...
        if (objp->type == OBJ_WEAPON) {
            ...
            if (wip->weapon_hitpoints <= 0) continue;

            if (!(wip->wi_flags[Weapon::Info_Flags::Takes_shockwave_damage] ||
                  (sw->weapon_info_index >= 0 &&
                   Weapon_info[sw->weapon_info_index]
                       .wi_flags[Weapon::Info_Flags::Ciws])))
                continue;
        }
        ...
        for (i = 0; i < sw->num_objs_hit; i++) {
            if (objp->signature == sw->obj_sig_hitlist[i]) { break; }
        }

        if (i < sw->num_objs_hit) { continue; }

        if (weapon_area_calc_damage (
                objp, &sw->pos, sw->inner_radius, sw->outer_radius, sw->blast,
        ...
        case OBJ_SHIP:
            ...
            // If we're doing an AoE Electronics shockwave, do the electronics
            // stuff. -MageKing17
            if ((sw->weapon_info_index >= 0) &&
                (Weapon_info[sw->weapon_info_index]
                     .wi_flags[Weapon::Info_Flags::Aoe_Electronics]) &&
                !(objp->flags[Object::Object_Flags::Invulnerable])) {
                weapon_do_electronics_effect (
                    objp, &sw->pos, sw->weapon_info_index);
            }
        ...
        case OBJ_WEAPON:
            ...
            if (wip->armor_type_idx >= 0)
                damage = Armor_types[wip->armor_type_idx].GetDamage (
                    damage,
        ...
        if (objp == Player_obj) {
            ...
            snd_play (
                gamesnd_get_game_sound (GameSounds::SHOCKWAVE_IMPACT), 0.0f,
                vol_scale);
        }
...
void shockwave_render (object* objp, model_draw_list* scene) {
    ...
    if ((sw->current_bitmap < 0) && (sw->model_id < 0)) return;
    ...
    if (sw->model_id > -1) {
        ...
        render_info.set_warp_params (
            -1, 1.0f - (sw->radius / sw->outer_radius), scale);
        ...
        render_info.set_flags (
            MR_NO_LIGHTING | MR_NO_FOGGING | MR_NORMAL | MR_CENTER_ALPHA |
            MR_NO_CULL | MR_NO_BATCH);
        render_info.set_object_number (sw->objnum);

        model_render_queue (
            &render_info, scene, sw->model_id, &Objects[sw->objnum].orient,
            &sw->pos);
        ...
        if (Cmdline_fb_explosions && Default_2D_shockwave_index > -1) {
            ...
            float intensity =
                ((sw->time_elapsed / sw->total_time) > 0.9f)
...
int shockwave_load (const char* s_name, bool shock_3D) {
    ...
    if ((si->bitmap_id >= 0) || (si->model_id >= 0)) return s_index;
    ...
    if (shock_3D) {
        ...
        si->bitmap_id = bm_load_animation (
            si->filename, &si->num_frames, &si->fps, nullptr, nullptr, true);
...
void shockwave_level_init () {
    ...
    if (!Default_shockwave_loaded) {
        ...
        // chief1983 - The first patch broke mods that don't provide a 2d
        // shockwave or define a specific shockwave for each model/weapon
        // (shame on them) The next patch involved a direct copy of the attempt
        // above, with an i < 0 check in place of the command line check.  I've
        // taken that and modified it to spit out a more meaningful message.
        // Might as well not bother trying again if the command line option was
        ...
        if (Shockwave_info[0].model_id >= 0) {
            Assertion (
                ...
                "somehow isn't.\nShockwave_info[0].filename = "
                "\"%s\"\nDefault_shockwave_3D_filename = \"%s\"\nGet a "
                "coder!\n",
                Shockwave_info[0].filename, Default_shockwave_3D_filename);
            Shockwave_info[0].model_id =
...
void shockwave_level_close () {
    ...
    for (++it; it != Shockwave_info.end (); ++it) {
        if (it->bitmap_id >= 0) bm_release (it->bitmap_id);

        if (it->model_id >= 0) model_unload (it->model_id);
    }

    // if there's no extra shockwaves, this is still fine (erasing from end()
    // to end() is both valid and a no-op)
    Shockwave_info.erase (
        Shockwave_info.begin () + (Default_2D_shockwave_index == 1 ? 2 : 1),
        Shockwave_info.end ());
...
void shockwave_page_in () {
    // load in shockwaves
    for (auto it = Shockwave_info.begin (); it != Shockwave_info.end ();
         ++it) {
        if (it->bitmap_id >= 0) {
...
void shockwave_create_info_init (shockwave_create_info* sci) {
    ...
    sci->inner_rad = sci->outer_rad = sci->damage = sci->blast = sci->speed =
        0.0f;
...
void shockwave_create_info_load (shockwave_create_info* sci) {
    ...
    if (strlen (sci->name)) i = shockwave_load (sci->name, false);
