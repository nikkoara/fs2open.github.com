...
void swarm_level_init () {
    ...
    Turret_swarm_validity_next_check_time =
        timestamp (TURRET_SWARM_VALIDITY_CHECKTIME);
}
...
void swarm_maybe_fire_missile (int shipnum) {
    ...
    if (!(Weapon_info[weapon_info_index]
              .wi_flags[Weapon::Info_Flags::Swarm])) {
        sp->num_swarm_missiles_to_fire = 0;
        sp->swarm_missile_bank = -1;
        return;
    }
    ...
    if (timestamp_elapsed (sp->next_swarm_fire)) {
        ...
        sp->next_swarm_fire =
            timestamp (Weapon_info[weapon_info_index].SwarmWait);
...
int swarm_create () {
    ...
    for (i = 0; i < MAX_SWARM_MISSILES; i++) {
        swarmp = &Swarm_missiles[i];
        if (!(swarmp->flags & SWARM_USED)) break;
    }
...
void swarm_update_direction (object* objp) {
    ...
    swarm_info* swarmp;
    vec3d
        obj_to_target; // Vector pointing from the swarm missile to its target
    ...
    // check if homing is lost.. if it is then get a new path to move swarm
    // missile along
    ...
    if (hobjp != &obj_used_list) { swarmp->homing_objnum = OBJ_INDEX (hobjp); }
    ...
    if (timestamp_elapsed (swarmp->change_timestamp)) {
        ...
        if (swarmp->path_num == -1) {
            ...
            vm_vec_scale_add (
                &swarmp->original_target, &objp->pos, &objp->orient.vec.fvec,
                SWARM_CONE_LENGTH);
        ...
        // Calculate a rvec and uvec that will determine the displacement from
        // the intended target.  Use crossprod to generate a right vector, from
        // the missile up vector and the vector connecting missile to the
        // homing object. Faster missiles have a tighter grouping than slower
        // missiles, They should expand at sharp turns
        {
            float missile_dist;  // straight-line distance the missile will
                                 // travel between now and next check
        ...
        // Radius around the target pos. Shortens as the missiles get closer to
        // the target.
        radius = tanf (swarmp->angle_offset) * target_dist;

        // If homing swarm missile is close to target, let missile home in on
        // original target
        if ((target_dist / pi->speed) <=
            ((swarmp->change_time / 1000.0f) * SWARM_FRAME_STOP_SWARMING)) {
        ...
        if (swarmp->change_count == 1) {
            ...
            if (swarmp->path_num > 3) { swarmp->path_num = 0; }
        }
        ...
        case 0: // straight up and down
            if (swarmp->flags & SWARM_POSITIVE_PATH)
                vm_vec_copy_scale (
                    &uvec_component, &swarmp->circle_uvec, radius);
            else
                vm_vec_copy_scale (
                    &uvec_component, &swarmp->circle_uvec, -radius);
            break;
        ...
        case 1: // left/right
            if (swarmp->flags & SWARM_POSITIVE_PATH)
                vm_vec_copy_scale (
                    &rvec_component, &swarmp->circle_rvec, radius);
            else
                vm_vec_copy_scale (
                    &rvec_component, &swarmp->circle_rvec, -radius);
            break;
...
swarm_new_target_calced:
    ...
    ai_turn_towards_vector (
        &swarmp->new_target, objp, wip->turn_time, nullptr, nullptr, 0.0f, 0);
    vel = vm_vec_mag (&objp->phys_info.desired_vel);
    vm_vec_copy_scale (
        &objp->phys_info.desired_vel, &objp->orient.vec.fvec, vel);
}
...
int turret_swarm_create () {
    ...
    for (i = 0; i < MAX_TURRET_SWARM_INFO; i++) {
        tswarmp = &Turret_swarm_info[i];
        if (!(tswarmp->flags & SWARM_USED)) break;
    }
...
void turret_swarm_set_up_info (
    int parent_objnum, ship_subsys* turret, weapon_info* wip, int weapon_num) {
    ...
    /*
    Assert((turret_weapon_class >= 0) && (turret_weapon_class <
    Num_weapon_types)); if((turret_weapon_class < 0) || (turret_weapon_class >=
    Num_weapon_types)){ return;
    }
    */

    // get ship pointer
    Assert ((parent_objnum >= 0) && (parent_objnum < MAX_OBJECTS));
    if ((parent_objnum < 0) || (parent_objnum >= MAX_OBJECTS)) { return; }
    parent_obj = &Objects[parent_objnum];
    Assert (parent_obj->type == OBJ_SHIP);
    shipp = &Ships[parent_obj->instance];
    Assert (
        (turret->turret_enemy_objnum >= 0) &&
        (turret->turret_enemy_objnum < MAX_OBJECTS));
    ...
    if (turret->turret_swarm_num == MAX_TFP) {
        ...
        for (old_s = 0; old_s < (MAX_TFP - 1); old_s++) {
            turret->turret_swarm_info_index[old_s] =
                turret->turret_swarm_info_index[old_s + 1];
        }
    ...
    // make sure time is sufficient to launch all the missiles before next
volley #ifndef NDEBUG Assert(wip->swarm_count * SWARM_MISSILE_DELAY <
wip->fire_wait * 1000.0f); #endif
    ...
    for (subsys = GET_FIRST (&shipp->subsys_list);
        ...
        if (subsys->turret_swarm_num > 0) {
            ...
            for (k = 0; k < swarms_per_turret; k++) {
                ...
                // get turret_swarm_info
                Assert (
                    (turret_tsi >= 0) && (turret_tsi < MAX_TURRET_SWARM_INFO));
                ...
                if (tsi->parent_sig == parent_obj->signature) {
                    ...
                    if (timestamp_elapsed (tsi->time_to_fire)) {
                        ...
                        if (tsi->num_to_launch == 0) {
                            // we are done firing, so see about resetting any
                            // animation timestamps for reversal (closing)...
                            // (I figure that a good estimate is to trigger a
                            // close after three additional swarms had fired -
                            // taylor)
                            if (subsys->turret_animation_position ==
                                MA_POS_READY)
                                subsys
                                    ->turret_animation_done_time = timestamp (
                                    Weapon_info[tsi->weapon_class].SwarmWait *
                                    3);

                            shipp->num_turret_swarm_info--;
                            subsys->turret_swarm_num--;
                            turret_swarm_delete (
                                subsys->turret_swarm_info_index[k]);
            ...
            for (k = 0; k < (MAX_TFP - 1); k++) {
                for (j = (k + 1); j < MAX_TFP; j++) {
                    if ((subsys->turret_swarm_info_index[k] == -1) &&
                        (subsys->turret_swarm_info_index[j] != -1)) {
                        subsys->turret_swarm_info_index[k] =
                            subsys->turret_swarm_info_index[j];
                        subsys->turret_swarm_info_index[j] = -1;
                    }
                }
            }
...
// check Turret_swarm_info for info that are invalid - ie, ships died while
// firing.
