...
typedef struct cscrew_info {
    ...
    matrix real_orient; // the orientation used when calling physics (bashed
                        // before rendering)
...
void cscrew_level_init () {
    memset (
        Corkscrew_missiles, 0, sizeof (cscrew_info) * MAX_CORKSCREW_MISSILES);
}
...
void cscrew_maybe_fire_missile (int shipnum) {
    ...
    if (timestamp_elapsed (sp->next_corkscrew_fire)) {
        sp->next_corkscrew_fire =
            timestamp (Weapon_info[weapon_info_index].cs_delay);
...
int cscrew_create (object* obj) {
    ...
    for (i = 0; i < MAX_CORKSCREW_MISSILES; i++) {
        ...
        if (!(cscrewp->flags & CS_FLAG_USED)) { break; }
    }
    ...
    if (Corkscrew_down_first) { vm_vec_negate (&neg); }
    vm_vec_scale_add2 (&cscrewp->cen_p, &neg, wip->cs_radius);

    // move the missile up so that the corkscrew point is at the muzzle of the
    // gun vm_vec_scale_add2(&obj->pos, &obj->orient.vec.uvec,
    // Corkscrew_radius);
...
void cscrew_process_post (object* objp) {
    ...
    // determine what direction (clockwise or counterclockwise) the missile
    // will spin
    ...
    vm_rot_point_around_line (
        &new_pt, &objp->pos, twist_val, &cen, &objp->orient.vec.fvec);
    objp->pos = new_pt;
...
void cscrew_display_dcf () {
    ...
    if (Corkscrew_shrink) { dc_printf ("Shrink (cscrew_shrink): ON\n"); }
    else {
        dc_printf ("Shrink (cscrew_shrink): OFF\n");
    }
    dc_printf (
        "Corkscrew shrink (cscrew_shrinkval): %f\n", Corkscrew_shrink_val);
    if (Corkscrew_down_first) { dc_printf ("Corkscrew down first : ON\n"); }
    else {
