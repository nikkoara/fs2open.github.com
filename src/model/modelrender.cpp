...
model_render_params::model_render_params ()
    : Model_flags (MR_NORMAL), Debug_flags (0), Objnum (-1),
      Detail_level_locked (-1), Depth_scale (1500.0f), Warp_bitmap (-1),
      Warp_alpha (-1.0f), Xparent_alpha (1.0f), Forced_bitmap (-1),
      Insignia_bitmap (-1), Replacement_textures (NULL),
      Manage_replacement_textures (false), Team_color_set (false),
      Clip_plane_set (false), Animated_effect (-1), Animated_timer (0.0f),
      Thruster_info (), Normal_alpha (false) {
    ...
    if (!Model_texturing) Model_flags |= MR_NO_TEXTURING;

    if (!Model_polys) { Model_flags |= MR_NO_POLYS; }
...
uint model_render_params::get_debug_flags () { return Debug_flags; }

int model_render_params::get_object_number () { return Objnum; }

int model_render_params::get_detail_level_lock () {
    return Detail_level_locked;
}

float model_render_params::get_depth_scale () { return Depth_scale; }

int model_render_params::get_warp_bitmap () { return Warp_bitmap; }

float model_render_params::get_warp_alpha () { return Warp_alpha; }

const vec3d& model_render_params::get_warp_scale () { return Warp_scale; }

const color& model_render_params::get_color () { return Color; }
float model_render_params::get_alpha () { return Xparent_alpha; }

int model_render_params::get_forced_bitmap () { return Forced_bitmap; }

int model_render_params::get_insignia_bitmap () { return Insignia_bitmap; }
...
const vec3d& model_render_params::get_clip_plane_pos () { return Clip_pos; }

const vec3d& model_render_params::get_clip_plane_normal () {
    return Clip_normal;
}

int model_render_params::get_animated_effect_num () { return Animated_effect; }
...
bool model_render_params::is_clip_plane_set () { return Clip_plane_set; }
...
void model_render_params::set_team_color (
    const std::string& team, const std::string& secondaryteam, fix timestamp,
    int fadetime) {
    Team_color_set = model_get_team_color (
        &Current_team_color, team, secondaryteam, timestamp, fadetime);
}

bool model_render_params::is_team_color_set () { return Team_color_set; }
...
void model_render_params::set_replacement_textures (
    ...
    for (auto tr : replacement_textures) {
        for (int i = 0; i < pm->n_textures; ++i) {
            ...
            if (tnum > -1)
                Replacement_textures[i * TM_NUM_TYPES + tnum] =
                    bm_load (tr.new_texture);
        }
    }
}
...
void model_render_params::set_alpha (float alpha) { Xparent_alpha = alpha; }

void model_render_params::set_color (color& clr) { Color = clr; }

void model_render_params::set_color (int r, int g, int b) {
    gr_init_color (&Color, r, g, b);
}

void model_render_params::set_warp_params (
    int bitmap, float alpha, vec3d& scale) {
    Warp_bitmap = bitmap;
    Warp_alpha = alpha;
    Warp_scale = scale;
}
...
void model_render_params::set_debug_flags (uint flags) { Debug_flags = flags; }

void model_render_params::set_object_number (int num) { Objnum = num; }

void model_render_params::set_flags (uint flags) { Model_flags = flags; }
...
bool model_render_params::is_normal_alpha_set () { return Normal_alpha; }

float model_render_params::get_normal_alpha_min () { return Normal_alpha_min; }

float model_render_params::get_normal_alpha_max () { return Normal_alpha_max; }
...
void model_batch_buffer::set_model_transform (
    matrix4& transform, int model_id) {
...
size_t model_batch_buffer::get_buffer_offset () { return Current_offset; }
...
void model_batch_buffer::submit_buffer_data () {
    if (Submodel_matrices.empty ()) { return; }
...
model_draw_list::model_draw_list () : Transformations () { reset (); }
...
void model_draw_list::sort_draws () {
    std::sort (
        Render_keys.begin (), Render_keys.end (),
        [this](const int a, const int b) {
            return model_draw_list::sort_draw_pair (this, a, b);
        });
}
...
void model_draw_list::add_buffer_draw (
    ...
    if (Rendering_to_shadow_map) {
        ...
        // If the zbuffer type is FULL then this buffer may be drawn in the
        // deferred lighting part otherwise we need to make sure that the
        // deferred flag is disabled or else some parts of the rendered colors
        // go missing
        // TODO: This should really be handled somewhere else. This feels like
        // a crude hack...
        auto possibly_deferred =
            draw_data.render_material.get_depth_mode () == ZBUFFER_TYPE_FULL &&
            gr_is_capable (CAPABILITY_DEFERRED_LIGHTING) &&
            !Cmdline_no_deferred_lighting;
        ...
        draw_data.render_material.set_deferred_lighting (
            possibly_deferred ? Deferred_lighting : false);
        draw_data.render_material.set_high_dynamic_range (High_dynamic_range);
        draw_data.render_material.set_shadow_receiving (
            Cmdline_shadow_quality != 0);
    }

    if (tmap_flags & TMAP_FLAG_BATCH_TRANSFORMS &&
        buffer->flags & VB_FLAG_MODEL_ID) {
        ...
        draw_data.transform_buffer_offset =
            TransformBufferHandler.get_buffer_offset ();
...
void model_draw_list::render_buffer (queued_buffer_draw& render_elements) {
    ...
    gr_bind_uniform_buffer (
        uniform_block_type::ModelData, render_elements.uniform_buffer_offset,
        sizeof (graphics::model_uniform_data), _dataBuffer->bufferHandle ());

    gr_render_model (
        &render_elements.render_material, render_elements.vert_src,
        render_elements.buffer, render_elements.texi);
}
...
void model_draw_list::pop_transform () { Transformations.pop (); }
...
void model_draw_list::render_all (gr_zbuffer_type depth_mode) {
    ...
    for (size_t i = 0; i < Render_keys.size (); ++i) {
        ...
        if (depth_mode == ZBUFFER_TYPE_DEFAULT ||
            Render_elements[render_index].render_material.get_depth_mode () ==
                depth_mode) {
...
void model_draw_list::render_arc (arc_effect& arc) {
    ...
    model_render_arc (
        &arc.v1, &arc.v2, &arc.primary, &arc.secondary, arc.width);
...
void model_draw_list::render_arcs () {
    ...
    for (size_t i = 0; i < Arcs.size (); ++i) { render_arc (Arcs[i]); }

    gr_zbuffer_set (mode);
}

void model_draw_list::add_insignia (
...
void model_draw_list::render_insignia (insignia_draw_data& insignia_info) {
    if (insignia_info.clip) {
        ...
        if (vm_vec_dot (&tmp, &insignia_info.clip_normal) < 0.0f) { return; }
    }
...
void model_draw_list::add_outline (
    vertex* vert_array, int n_verts, color* clr) {
...
void model_draw_list::render_outline (outline_draw& outline_info) {
    ...
    g3_render_primitives (
        &material_instance, outline_info.vert_array, outline_info.n_verts,
        PRIM_TYPE_LINES, false);

    g3_done_instance (true);
}

bool model_draw_list::sort_draw_pair (
    model_draw_list* target, const int a, const int b) {
    ...
    if (draw_call_a->vert_src->Vbuffer_handle !=
        draw_call_b->vert_src->Vbuffer_handle) {
        return draw_call_a->vert_src->Vbuffer_handle <
               draw_call_b->vert_src->Vbuffer_handle;
    }

    if (draw_call_a->vert_src->Ibuffer_handle !=
        draw_call_b->vert_src->Ibuffer_handle) {
        return draw_call_a->vert_src->Ibuffer_handle <
               draw_call_b->vert_src->Ibuffer_handle;
    }

    if (draw_call_a->render_material.get_texture_map (TM_BASE_TYPE) !=
        draw_call_b->render_material.get_texture_map (TM_BASE_TYPE)) {
        return draw_call_a->render_material.get_texture_map (TM_BASE_TYPE) <
               draw_call_b->render_material.get_texture_map (TM_BASE_TYPE);
    }

    if (draw_call_a->render_material.get_texture_map (TM_SPECULAR_TYPE) !=
        draw_call_b->render_material.get_texture_map (TM_SPECULAR_TYPE)) {
        return draw_call_a->render_material.get_texture_map (
                   TM_SPECULAR_TYPE) <
               draw_call_b->render_material.get_texture_map (TM_SPECULAR_TYPE);
    }

    if (draw_call_a->render_material.get_texture_map (TM_SPEC_GLOSS_TYPE) !=
        draw_call_b->render_material.get_texture_map (TM_SPEC_GLOSS_TYPE)) {
        return draw_call_a->render_material.get_texture_map (
                   TM_SPEC_GLOSS_TYPE) <
               draw_call_b->render_material.get_texture_map (
                   TM_SPEC_GLOSS_TYPE);
    }

    if (draw_call_a->render_material.get_texture_map (TM_GLOW_TYPE) !=
        draw_call_b->render_material.get_texture_map (TM_GLOW_TYPE)) {
        return draw_call_a->render_material.get_texture_map (TM_GLOW_TYPE) <
               draw_call_b->render_material.get_texture_map (TM_GLOW_TYPE);
    }

    if (draw_call_a->render_material.get_texture_map (TM_NORMAL_TYPE) !=
        draw_call_b->render_material.get_texture_map (TM_NORMAL_TYPE)) {
        return draw_call_a->render_material.get_texture_map (TM_NORMAL_TYPE) <
               draw_call_b->render_material.get_texture_map (TM_NORMAL_TYPE);
    }

    if (draw_call_a->render_material.get_texture_map (TM_HEIGHT_TYPE) !=
        draw_call_b->render_material.get_texture_map (TM_HEIGHT_TYPE)) {
        return draw_call_a->render_material.get_texture_map (TM_HEIGHT_TYPE) <
               draw_call_b->render_material.get_texture_map (TM_HEIGHT_TYPE);
    }

    if (draw_call_a->render_material.get_texture_map (TM_AMBIENT_TYPE) !=
        draw_call_b->render_material.get_texture_map (TM_AMBIENT_TYPE)) {
        return draw_call_a->render_material.get_texture_map (TM_AMBIENT_TYPE) <
               draw_call_b->render_material.get_texture_map (TM_AMBIENT_TYPE);
    }

    if (draw_call_a->render_material.get_texture_map (TM_MISC_TYPE) !=
        draw_call_b->render_material.get_texture_map (TM_MISC_TYPE)) {
        return draw_call_a->render_material.get_texture_map (TM_MISC_TYPE) <
               draw_call_b->render_material.get_texture_map (TM_MISC_TYPE);
    }
...
void model_draw_list::build_uniform_buffer () {
    ...
    for (auto render_index : Render_keys) {
        ...
        // Set lighting here so that it can be captured by the uniform
        // conversion below
        ...
        auto element = _dataBuffer->aligner ()
                           .addTypedElement< graphics::model_uniform_data > ();
        graphics::uniforms::convert_model_material (
            element, queued_draw.render_material, queued_draw.transform,
            queued_draw.scale, queued_draw.transform_buffer_offset);
        queued_draw.uniform_buffer_offset =
            _dataBuffer->aligner ().getCurrentOffset ();
    }
...
model_draw_list::~model_draw_list () { reset (); }

void model_render_add_lightning (
    model_draw_list* scene, model_render_params* interp, polymodel* pm,
    bsp_info* sm) {
    int i;
    float width = 0.9f;
    ...
    // try and scale the size a bit so that it looks equally well on smaller
    // vessels
    if (pm->rad < 500.0f) {
        width *= (pm->rad * 0.01f);
    ...
    for (i = 0; i < sm->num_arcs; i++) {
        ...
        case MARC_TYPE_NORMAL:
            if ((rand () >> 4) & 1) { gr_init_color (&primary, 64, 64, 255); }
            else {
            ...
            // "EMP" style arcs
        case MARC_TYPE_EMP:
            if ((rand () >> 4) & 1) { gr_init_color (&primary, AR, AG, AB); }
            else {
        ...
        scene->add_arc (
            &sm->arc_pts[i][0], &sm->arc_pts[i][1], &primary, &secondary,
            width);
    }
}

float model_render_determine_depth (
    int obj_num, int model_num, matrix* orient, vec3d* pos,
    int detail_level_locked) {
    vec3d closest_pos;
    float depth = model_find_closest_point (
        &closest_pos, model_num, -1, orient, pos, &Eye_position);
...
int model_render_determine_detail (
    float depth, int /*obj_num*/, int model_num, matrix* /*orient*/,
    vec3d* /*pos*/, int /*flags*/, int detail_level_locked) {
...
void model_render_buffers (
    ...
    bool render_as_thruster = (model != NULL) && model->is_thruster &&
                              (model_flags & MR_SHOW_THRUSTERS);
    ...
    if (render_as_thruster) {
        ...
        rendering_material->set_thrust_scale (
            interp->get_thruster_info ().length.xyz.z);
    }
    else {
    ...
    if (tmap_flags & TMAP_FLAG_BATCH_TRANSFORMS && (mn >= 0) &&
        (mn < pm->n_models)) {
    ...
    fix base_frametime =
        model_render_determine_base_frametime (obj_num, model_flags);
    ...
    if (interp->get_forced_bitmap () >= 0) {
        ...
        if ((interp->get_thruster_info ().primary_bitmap >= 0) &&
            (interp->get_thruster_info ().length.xyz.z > 0.0f)) {
    ...
    for (size_t i = 0; i < buffer_size; i++) {
        ...
        if (forced_texture != -2) {
            ...
            if ((replacement_textures != NULL) &&
                (replacement_textures[rt_begin_index + TM_BASE_TYPE] ==
                 REPLACE_WITH_INVISIBLE)) {
                // invisible textures aren't rendered, but we still have to
                // skip assigning the underlying model texture
                texture_maps[TM_BASE_TYPE] = -1;
            }
            else if (
                (replacement_textures != NULL) &&
                (replacement_textures[rt_begin_index + TM_BASE_TYPE] >= 0)) {
                // an underlying texture is replaced with a real new texture
                tex_replace[TM_BASE_TYPE] = texture_info (
                    replacement_textures[rt_begin_index + TM_BASE_TYPE]);
                texture_maps[TM_BASE_TYPE] = model_interp_get_texture (
                    &tex_replace[TM_BASE_TYPE], base_frametime);
            }
            else {
                // we just use the underlying texture
                texture_maps[TM_BASE_TYPE] = model_interp_get_texture (
                    &tmap->textures[TM_BASE_TYPE], base_frametime);
            }

            if (texture_maps[TM_BASE_TYPE] < 0) { continue; }
            ...
            if (!(model_flags & MR_NO_GLOWMAPS)) {
                ...
                if ((replacement_textures != NULL) &&
                    (replacement_textures[rt_begin_index + TM_GLOW_TYPE] >=
                     0)) {
                    tex_replace[TM_GLOW_TYPE] = texture_info (
                        replacement_textures[rt_begin_index + TM_GLOW_TYPE]);
                    texture_maps[TM_GLOW_TYPE] = model_interp_get_texture (
                        &tex_replace[TM_GLOW_TYPE], base_frametime);
                }
                else if (tglow->GetTexture () >= 0) {
                    // shockwaves are special, their current frame has to come
                    // out of the shockwave code to get the timing correct
                    if ((obj_num >= 0) &&
                        (Objects[obj_num].type == OBJ_SHOCKWAVE) &&
                        (tglow->GetNumFrames () > 1)) {
                        texture_maps[TM_GLOW_TYPE] =
                            tglow->GetTexture () +
                            shockwave_get_framenum (
                                Objects[obj_num].instance,
                                tglow->GetTexture ());
                    }
                    else {
                        texture_maps[TM_GLOW_TYPE] =
                            model_interp_get_texture (tglow, base_frametime);
                    }
                }
            }
            ...
            if (!(debug_flags & MR_DEBUG_NO_SPEC)) {
                if (replacement_textures != NULL &&
                    replacement_textures[rt_begin_index + TM_SPECULAR_TYPE] >=
                        0) {
                    tex_replace[TM_SPECULAR_TYPE] =
                        texture_info (replacement_textures
                                          [rt_begin_index + TM_SPECULAR_TYPE]);
                    texture_maps[TM_SPECULAR_TYPE] = model_interp_get_texture (
                        &tex_replace[TM_SPECULAR_TYPE], base_frametime);
                }
                else {
                    texture_maps[TM_SPECULAR_TYPE] = model_interp_get_texture (
                        &tmap->textures[TM_SPECULAR_TYPE], base_frametime);
                }
            }

            if (replacement_textures != NULL &&
                replacement_textures[rt_begin_index + TM_SPEC_GLOSS_TYPE] >=
                    0) {
                tex_replace[TM_SPEC_GLOSS_TYPE] = texture_info (
                    replacement_textures[rt_begin_index + TM_SPEC_GLOSS_TYPE]);
                texture_maps[TM_SPEC_GLOSS_TYPE] = model_interp_get_texture (
                    &tex_replace[TM_SPEC_GLOSS_TYPE], base_frametime);
            }
            else {
                texture_maps[TM_SPEC_GLOSS_TYPE] = model_interp_get_texture (
                    &tmap->textures[TM_SPEC_GLOSS_TYPE], base_frametime);
            }
            ...
            if (detail_level < 2) {
                ...
                if (replacement_textures != NULL) {
                    if (replacement_textures
                            [rt_begin_index + TM_NORMAL_TYPE] >= 0) {
                        tex_replace[TM_NORMAL_TYPE] = texture_info (
                            replacement_textures
                                [rt_begin_index + TM_NORMAL_TYPE]);
                        norm_map = &tex_replace[TM_NORMAL_TYPE];
                    }

                    if (replacement_textures
                            [rt_begin_index + TM_HEIGHT_TYPE] >= 0) {
                        tex_replace[TM_HEIGHT_TYPE] = texture_info (
                            replacement_textures
                                [rt_begin_index + TM_HEIGHT_TYPE]);
                        height_map = &tex_replace[TM_HEIGHT_TYPE];
                    }

                    if (replacement_textures
                            [rt_begin_index + TM_AMBIENT_TYPE] >= 0) {
                        tex_replace[TM_AMBIENT_TYPE] = texture_info (
                            replacement_textures
                                [rt_begin_index + TM_AMBIENT_TYPE]);
                        ambient_map = &tex_replace[TM_AMBIENT_TYPE];
                    }

                    if (replacement_textures[rt_begin_index + TM_MISC_TYPE] >=
                        0) {
                        tex_replace[TM_MISC_TYPE] =
                            texture_info (replacement_textures
                                              [rt_begin_index + TM_MISC_TYPE]);
                        misc_map = &tex_replace[TM_MISC_TYPE];
                    }
                }

                if (debug_flags & MR_DEBUG_NO_DIFFUSE)
                    texture_maps[TM_BASE_TYPE] = -1;
                if (debug_flags & MR_DEBUG_NO_GLOW)
                    texture_maps[TM_GLOW_TYPE] = -1;
                if (debug_flags & MR_DEBUG_NO_SPEC)
                    texture_maps[TM_SPECULAR_TYPE] = -1;
                if (!(debug_flags & MR_DEBUG_NO_MISC))
                    texture_maps[TM_MISC_TYPE] =
                        model_interp_get_texture (misc_map, base_frametime);
                if (!(debug_flags & MR_DEBUG_NO_NORMAL) && Detail.lighting > 0)
                    texture_maps[TM_NORMAL_TYPE] =
                        model_interp_get_texture (norm_map, base_frametime);
                if (!(debug_flags & MR_DEBUG_NO_AMBIENT) &&
                    Detail.lighting > 0)
                    texture_maps[TM_AMBIENT_TYPE] =
                        model_interp_get_texture (ambient_map, base_frametime);
                if (!(debug_flags & MR_DEBUG_NO_HEIGHT) && Detail.lighting > 1)
                    texture_maps[TM_HEIGHT_TYPE] =
                        model_interp_get_texture (height_map, base_frametime);
            }
        }
        else {
            alpha = forced_alpha;

            // Check for invisible or transparent textures so they don't show
            // up in the shadow maps - Valathil
            if (Rendering_to_shadow_map) {
                if ((replacement_textures != NULL) &&
                    (replacement_textures[rt_begin_index + TM_BASE_TYPE] >=
                     0)) {
        ...
        if (tmap->is_transparent) {
            ...
            alpha = (interp->get_warp_alpha () != -1.0f)
                        ? interp->get_warp_alpha ()
                        : 0.8f;
            use_blending = true;
        }
        ...
        if (forced_blend_filter != GR_ALPHABLEND_NONE) { use_blending = true; }

        bool use_depth_test;

        if (use_blending) { use_depth_test = true; }
        else {
            if ((model_flags & MR_NO_ZBUFFER) ||
                (model_flags & MR_ALL_XPARENT)) {
        ...
        gr_alpha_blend blend_mode = model_render_determine_blend_mode (
            texture_maps[TM_BASE_TYPE], use_blending);
        gr_zbuffer_type depth_mode =
            material_determine_depth_mode (use_depth_test, use_blending);

        rendering_material->set_depth_mode (depth_mode);
        rendering_material->set_blend_mode (blend_mode);

        color clr = interp->get_color ();
        model_render_determine_color (
            &clr, alpha, blend_mode, no_texturing ? true : false,
            rendering_material->is_desaturated ());
        rendering_material->set_color (clr);

        if ((tmap_flags & TMAP_FLAG_TEXTURED) &&
            (buffer->flags & VB_FLAG_UV1)) {
            rendering_material->set_texture_map (
                TM_BASE_TYPE, texture_maps[TM_BASE_TYPE]);

            if (texture_maps[TM_BASE_TYPE] >= 0 &&
                bm_has_alpha_channel (texture_maps[TM_BASE_TYPE])) {
                rendering_material->set_texture_type (
                    material::TEX_TYPE_XPARENT);
            }

            rendering_material->set_texture_map (
                TM_GLOW_TYPE, texture_maps[TM_GLOW_TYPE]);
            rendering_material->set_texture_map (
                TM_SPECULAR_TYPE, texture_maps[TM_SPECULAR_TYPE]);
            rendering_material->set_texture_map (
                TM_SPEC_GLOSS_TYPE, texture_maps[TM_SPEC_GLOSS_TYPE]);
            rendering_material->set_texture_map (
                TM_NORMAL_TYPE, texture_maps[TM_NORMAL_TYPE]);
            rendering_material->set_texture_map (
                TM_HEIGHT_TYPE, texture_maps[TM_HEIGHT_TYPE]);
            rendering_material->set_texture_map (
                TM_AMBIENT_TYPE, texture_maps[TM_AMBIENT_TYPE]);
            rendering_material->set_texture_map (
                TM_MISC_TYPE, texture_maps[TM_MISC_TYPE]);
        }

        scene->add_buffer_draw (
            rendering_material, &pm->vert_source, buffer, i, tmap_flags);
    }
}

void model_render_children_buffers (
    model_draw_list* scene, model_material* rendering_material,
    model_render_params* interp, polymodel* pm, polymodel_instance* pmi,
    int mn, int detail_level, uint tmap_flags, bool trans_buffer) {
    int i;
    ...
    if (pmi != NULL) { smi = &pmi->submodel[mn]; }

    if (smi != NULL) {
        if (smi->blown_off) { return; }
    }
    ...
    if (model->is_thruster) {
        if (!(model_flags & MR_SHOW_THRUSTERS)) { return; }
    ...
    if ((model_flags & MR_SHOW_OUTLINE || model_flags & MR_SHOW_OUTLINE_HTL ||
         model_flags & MR_SHOW_OUTLINE_PRESET) &&
        pm->submodel[mn].outline_buffer != NULL) {
        color outline_color = interp->get_color ();
        scene->add_outline (
            pm->submodel[mn].outline_buffer, pm->submodel[mn].n_verts_outline,
            &outline_color);
    }
    else {
        if (trans_buffer &&
            pm->submodel[mn].trans_buffer.flags & VB_FLAG_TRANS) {
            model_render_buffers (
                scene, rendering_material, interp,
                &pm->submodel[mn].trans_buffer, pm, mn, detail_level,
                tmap_flags);
        }
        else {
            model_render_buffers (
                scene, rendering_material, interp, &pm->submodel[mn].buffer,
                pm, mn, detail_level, tmap_flags);
        }
    }
    ...
    while (i >= 0) {
        if (!pm->submodel[i].is_thruster) {
            model_render_children_buffers (
                scene, rendering_material, interp, pm, pmi, i, detail_level,
                tmap_flags, trans_buffer);
        }

        i = pm->submodel[i].next_sibling;
    }

    if (model->is_thruster) { rendering_material->set_lighting (true); }

    scene->pop_transform ();
}

float model_render_determine_light_factor (
    model_render_params* interp, vec3d* pos, uint flags) {
    if (flags & MR_IS_ASTEROID) {
        ...
        if (depth > interp->get_depth_scale ()) {
            ...
            if (temp_light > 1.0f) { return 1.0f; }
...
float model_render_determine_box_scale () {
    ...
    case 2: // 3rd dot is 80%
        box_scale = 0.8f;
        break;
    case 3: // 4th dot is 100% (this is the default setting for "High" and
            // "Very High" settings)
        box_scale = 1.0f;
        break;
    case 4: // 5th dot (max) is 120%
    default: box_scale = 1.2f; break;
    }
...
bool model_render_determine_autocenter (
    vec3d* auto_back, polymodel* pm, int detail_level, uint flags) {
    if (flags & MR_AUTOCENTER) {
        ...
        if (pm->flags & PM_FLAG_AUTOCEN) {
            *auto_back = pm->autocenter;
            ...
            // fake autocenter if we are a missile and don't already have
            // autocen info
            auto_back->xyz.x =
                -((pm->submodel[pm->detail[detail_level]].max.xyz.x +
                   pm->submodel[pm->detail[detail_level]].min.xyz.x) /
                  2.0f);
            auto_back->xyz.y =
                -((pm->submodel[pm->detail[detail_level]].max.xyz.y +
                   pm->submodel[pm->detail[detail_level]].min.xyz.y) /
                  2.0f);
            auto_back->xyz.z =
                -((pm->submodel[pm->detail[detail_level]].max.xyz.z +
                   pm->submodel[pm->detail[detail_level]].min.xyz.z) /
                  2.0f);
            return true;
        }
    }
...
void model_render_determine_color (
    ...
    if (no_texturing || desaturate) {
        // don't override the given color if we're not texturing or we're
        // desaturating
        return;
    }
...
bool model_render_check_detail_box (
    ...
    if (!(flags & MR_FULL_DETAIL) && model->use_render_box) {
        ...
        if ((-model->use_render_box +
             in_box (&box_min, &box_max, &offset, view_pos))) {
            return false;
        }
    }
    ...
    if (!(flags & MR_FULL_DETAIL) && model->use_render_sphere) {
        ...
        if ((-model->use_render_sphere +
             in_sphere (&offset, sphere_radius, view_pos))) {
            return false;
        }
    }
...
void submodel_render_immediate (
    ...
    submodel_render_queue (
        render_info, &model_list, model_num, submodel_num, orient, pos);
...
void submodel_render_queue (
    ...
    if (render_info->is_clip_plane_set ()) {
        rendering_material.set_clip_plane (
            render_info->get_clip_plane_normal (),
            render_info->get_clip_plane_pos ());
    }
    ...
    if (!(flags & MR_NO_TEXTURING)) {
        ...
        if (!(flags & MR_NO_CORRECT)) { tmap_flags |= TMAP_FLAG_CORRECT; }
    }

    if (render_info->get_animated_effect_num () >= 0) {
        tmap_flags |= TMAP_ANIMATED_SHADER;
        rendering_material.set_animated_effect (
            render_info->get_animated_effect_num (),
            render_info->get_animated_effect_timer ());
    }

    bool is_outlines_only_htl =
        (flags & MR_NO_POLYS) && (flags & MR_SHOW_OUTLINE_HTL);

    // set to true since D3d and OGL need the api matrices set
    scene->push_transform (pos, orient);

    vec3d auto_back = ZERO_VECTOR;
    bool set_autocen = model_render_determine_autocenter (
        &auto_back, pm, render_info->get_detail_level_lock (), flags);

    if (set_autocen) { scene->push_transform (&auto_back, NULL); }
    ...
    if (tmap_flags & TMAP_FLAG_PIXEL_FOG) {
        ...
        if (objnum >= 0) obj = &Objects[objnum];
    ...
    if (Rendering_to_shadow_map) { rendering_material.set_depth_bias (-1024); }
    else {
    ...
    if (model_render_check_detail_box (&view_pos, pm, submodel_num, flags)) {
        model_render_buffers (
            scene, &rendering_material, render_info,
            &pm->submodel[submodel_num].buffer, pm, submodel_num, 0,
            tmap_flags);

        if (pm->flags & PM_FLAG_TRANS_BUFFER &&
            pm->submodel[submodel_num].trans_buffer.flags & VB_FLAG_TRANS) {
            model_render_buffers (
                scene, &rendering_material, render_info,
                &pm->submodel[submodel_num].trans_buffer, pm, submodel_num, 0,
                tmap_flags);
        }
    }

    if (pm->submodel[submodel_num].num_arcs) {
        model_render_add_lightning (
            scene, render_info, pm, &pm->submodel[submodel_num]);
    }

    if (set_autocen) { scene->pop_transform (); }

    scene->pop_transform ();
}

void model_render_glowpoint (
    int point_num, vec3d* pos, matrix* orient, glow_point_bank* bank,
    glow_point_bank_override* gpo, polymodel* pm, ship* shipp,
    ...
    vec3d world_norm;
    vec3d tempv;
    vec3d submodel_static_offset; // The associated submodel's static offset in
                                  // the ship's frame of reference
    bool submodel_rotation = false;

    if (bank->submodel_parent > 0 &&
        pm->submodel[bank->submodel_parent].can_move && shipp != NULL) {
        model_find_submodel_offset (
            &submodel_static_offset,
            Ship_info[shipp->ship_info_index].model_num,
            bank->submodel_parent);
    ...
    if (submodel_rotation) {
        ...
        tempv = loc_offset;
        find_submodel_instance_point_normal (
            &loc_offset, &loc_norm, shipp->model_instance_num,
            bank->submodel_parent, &tempv, &loc_norm);
    }
    ...
    if (shipp != NULL) {
        if ((shipp->is_arriving ()) && (shipp->warpin_effect) &&
            Ship_info[shipp->ship_info_index].warpin_type != WT_HYPERSPACE) {
            vec3d warp_pnt, tmp;
            matrix warp_orient;
            ...
            if (vm_vec_dot (&tmp, &warp_orient.vec.fvec) < 0.0f) { return; }
        }
    ...
    case 0: {
        ...
        if (d > 0.0f) {
            ...
            if ((gpo && gpo->glow_bitmap_override)
                    ? (gpo->glow_bitmap > -1)
                    : (bank->glow_bitmap > -1)) {
                int gpflags = TMAP_FLAG_GOURAUD | TMAP_FLAG_RGB |
                              TMAP_FLAG_TEXTURED | TMAP_HTL_3D_UNLIT |
                              TMAP_FLAG_EMISSIVE;

                if (use_depth_buffer) gpflags |= TMAP_FLAG_SOFT_QUAD;

                int bitmap_id = (gpo && gpo->glow_bitmap_override)
                                    ? gpo->glow_bitmap
                                    : bank->glow_bitmap;

                if (use_depth_buffer) {
                    batching_add_volume_bitmap (
                        bitmap_id, &p, 0, (w * 0.5f), d * pulse, w);
                }
                else {
                    batching_add_bitmap (
                        bitmap_id, &p, 0, (w * 0.5f), d * pulse, w);
        ...
        if (gpo && gpo->pulse_type) {
            ...
            if (gpo->pulse_period_override) { period = gpo->pulse_period; }
            else {
                if (gpo->on_time_override) { period = 2 * gpo->on_time; }
                else {
            ...
            if ((gpo && gpo->off_time_override) ? gpo->off_time
                                                : bank->off_time) {
                x = (timestamp () - ((gpo && gpo->disp_time_override)
                                         ? gpo->disp_time
                                         : bank->disp_time)) %
                        (((gpo && gpo->on_time_override) ? gpo->on_time
                                                         : bank->on_time) +
                         ((gpo && gpo->off_time_override) ? gpo->off_time
                                                          : bank->off_time)) -
                    ((gpo && gpo->off_time_override) ? gpo->off_time
                                                     : bank->off_time);
            }
            ...
            case PULSE_SIN:
                pulse = gpo->pulse_bias +
                        gpo->pulse_amplitude *
                            pow (sin (PI2 / period * x), gpo->pulse_exponent);
                break;

            case PULSE_COS:
                pulse = gpo->pulse_bias +
                        gpo->pulse_amplitude *
                            pow (cos (PI2 / period * x), gpo->pulse_exponent);
                break;
            ...
            case PULSE_TRI:
                ...
                if (x > period / 4) {
                    pulse = gpo->pulse_bias +
                            gpo->pulse_amplitude * inv *
                                pow (
                                    1.0f - ((x - period / 4.0f) * 4 / period),
                                    gpo->pulse_exponent);
                }
                else {
                    pulse = gpo->pulse_bias +
                            gpo->pulse_amplitude * inv *
                                pow ((x * 4.0f / period), gpo->pulse_exponent);
                }
                break;
            }
        }

        if (Detail.lighting > 3 && Deferred_lighting && gpo &&
            gpo->is_lightsource) {
            if (gpo->lightcone) {
                vec3d cone_dir_rot;
                ...
                if (gpo->rotating) {
                    vm_rot_point_around_line (
                        ...
                        PI * timestamp () * 0.000033333f * gpo->rotation_speed,
                        &vmd_zero_vector, &gpo->rotation_axis);
                }
                else {
                    cone_dir_rot = gpo->cone_direction;
                }
                ...
                find_submodel_instance_point_normal (
                    &unused, &cone_dir_model, shipp->model_instance_num,
                    bank->submodel_parent, &unused, &cone_dir_rot);
                ...
                light_add_cone (
                    &world_pnt, &cone_dir_screen, gpo->cone_angle,
                    gpo->cone_inner_angle, gpo->dualcone, 1.0f,
                    w * gpo->radius_multi, 1,
                    pulse * gpo->light_color.xyz.x +
                        (1.0f - pulse) * gpo->light_mix_color.xyz.x,
                    pulse * gpo->light_color.xyz.y +
                        (1.0f - pulse) * gpo->light_mix_color.xyz.y,
                    pulse * gpo->light_color.xyz.z +
                        (1.0f - pulse) * gpo->light_mix_color.xyz.z,
                    -1);
            }
            else {
                light_add_point (
                    &world_pnt, 1.0f, w * gpo->radius_multi, 1,
                    pulse * gpo->light_color.xyz.x +
                        (1.0f - pulse) * gpo->light_mix_color.xyz.x,
                    pulse * gpo->light_color.xyz.y +
                        (1.0f - pulse) * gpo->light_mix_color.xyz.y,
    ...
    case 1: {
        ...
        moldel_calc_facing_pts (
            &top1, &bottom1, &fvec, &loc_offset, gpt->radius, 1.0f,
            &View_position);
        moldel_calc_facing_pts (
            &top2, &bottom2, &fvec, &loc_norm, gpt->radius, 1.0f,
            &View_position);
    ...
    if (objnum > -1) {
        ...
        if (objp != NULL && objp->type == OBJ_SHIP) {
            ...
            if (gpoi != sip->glowpoint_bank_override_map.end ()) {
                override_all = true;
                gpo = (glow_point_bank_override*)
                          sip->glowpoint_bank_override_map[-1];
            }
        }
    }

    for (int i = 0; i < pm->n_glow_point_banks;
         i++) { // glow point blink code -Bobboau
        ...
        if (!override_all && sip) {
            ...
            if (gpoi != sip->glowpoint_bank_override_map.end ()) {
                gpo = (glow_point_bank_override*)
                          sip->glowpoint_bank_override_map[i];
            }
            else {
                gpo = NULL;
            }
        ...
        if (bank->glow_timestamp == 0) { bank->glow_timestamp = time; }

        if ((gpo && gpo->off_time_override) ? gpo->off_time : bank->off_time) {
            if (bank->is_on) {
                if (((gpo && gpo->on_time_override) ? gpo->on_time
                                                    : bank->on_time) >
                    ((time - ((gpo && gpo->disp_time_override)
                                  ? gpo->disp_time
                                  : bank->disp_time)) %
                     (((gpo && gpo->on_time_override) ? gpo->on_time
                                                      : bank->on_time) +
...
void model_render_glow_points (
    polymodel* pm, ship* shipp, matrix* orient, vec3d* pos,
    ...
    for (i = 0; i < pm->n_glow_point_banks; i++) {
        ...
        if (bank->glow_bitmap == -1) continue;

        if (pm->submodel[bank->submodel_parent].blown_off) continue;

        if ((gpo && gpo->off_time_override && !gpo->off_time) ? gpo->is_on
                                                              : bank->is_on) {
            ...
            for (j = 0; j < bank->num_points; j++) {
                ...
                if (flick == 1) {
                    model_render_glowpoint (
                        j, pos, orient, bank, gpo, pm, shipp,
                        use_depth_buffer);
                } // flick
            }     // for slot
        }         // bank is on
...
void model_queue_render_thrusters (
    model_render_params* interp, polymodel* pm, int objnum, ship* shipp,
    matrix* orient, vec3d* pos) {
    int i, j;
    int n_q = 0;
    size_t k;
    ...
    if (!(interp->get_model_flags () & MR_SHOW_THRUSTERS)) { return; }

    // get an initial count to figure out how man geo batchers we need
    // allocated
    ...
    if (thruster_info.primary_glow_bitmap >= 0) { do_render = true; }

    // secondary_thruster_batcher
    if (thruster_info.secondary_glow_bitmap >= 0) { do_render = true; }

    // tertiary_thruster_batcher
    if (thruster_info.tertiary_glow_bitmap >= 0) { do_render = true; }

    if (do_render == false) { return; }
    ...
    for (i = 0; i < pm->n_thrusters; i++) {
        vec3d
            submodel_static_offset; // The associated submodel's static offset
        ...
        if (!model_should_render_engine_glow (objnum, bank->obj_num)) continue;
        ...
        // condition is thus a hack to disable the feature while in the lab,
        // and can be removed if the lab is re-structured accordingly.
        // -zookeeper
        if (bank->submodel_num > -1 &&
            pm->submodel[bank->submodel_num].can_move &&
        ...
        for (j = 0; j < bank->num_points; j++) {
            ...
            if (submodel_rotation) {
                ...
                find_submodel_instance_point_normal (
                    &loc_offset, &loc_norm, shipp->model_instance_num,
                    bank->submodel_num, &tempv, &loc_norm);
            }
            ...
            if (shipp) {
                // if ship is warping out, check position of the engine glow to
                // the warp plane
                if ((shipp->is_arriving ()) && (shipp->warpin_effect) &&
                    Ship_info[shipp->ship_info_index].warpin_type !=
                        WT_HYPERSPACE) {
                    vec3d warp_pnt, tmp;
                    matrix warp_orient;
...
#define MAX_SCALE 4.7f
            ...
            float w =
                gpt->radius * (scale + thruster_info.glow_noise * NOISE_SCALE);

            // these lines are used by the tertiary glows, thus we will need to
            // project this all of the time
            g3_transfer_vertex (&p, &world_pnt);
            ...
            if ((thruster_info.primary_glow_bitmap >= 0) && (d > 0.0f)) {
                ...
                batching_add_volume_bitmap (
                    thruster_info.primary_glow_bitmap, &p, 0,
                    (w * 0.5f * thruster_info.glow_rad_factor), d, w * 0.325f);
            }
            ...
            if (thruster_info.tertiary_glow_bitmap >= 0) {
                ...
                batching_add_volume_bitmap_rotated (
                    thruster_info.tertiary_glow_bitmap, &p, magnitude * 4,
                    w * 0.6f * thruster_info.tertiary_glow_rad_factor, fog_int,
                    -(D > 0) ? D : -D);
            }
            ...
            if (thruster_info.secondary_glow_bitmap >= 0) {
                pnt = world_pnt;
                scale = magnitude * (MAX_SCALE - (MIN_SCALE / 2)) +
                        (MIN_SCALE / 2);
                ...
                if (d > 0.0f) {
                    ...
                    vm_vec_scale_add (
                        &norm2, &pnt, &fvec,
                        wVal * 2 * thruster_info.glow_length_factor);
                    ...
                    batching_add_beam (
                        thruster_info.secondary_glow_bitmap, &pnt, &norm2,
                        wVal * thruster_info.secondary_glow_rad_factor * 0.5f,
                        d);

                    if (Scene_framebuffer_in_frame &&
                        thruster_info.draw_distortion &&
                        Cmdline_fb_thrusters) {
                        vm_vec_scale_add (
                            &norm2, &pnt, &fvec,
                            wVal * 2 * thruster_info.distortion_length_factor);
                        int dist_bitmap;
                        ...
                        mag -=
                            (float)((int)mag); // Valathil - Get a fairly
                                               // random but constant number to
                                               // offset the distortion texture

                        batching_add_distortion_beam (
                            dist_bitmap, &pnt, &norm2,
                            wVal * thruster_info.distortion_rad_factor * 0.5f,
                            1.0f, mag);
                    }
                }
            }
            ...
            if (shipp) {
                ...
                if (thruster_info.use_ab)
                    num_particles =
                        sip->afterburner_thruster_particles.size ();
                else
                ...
                for (k = 0; k < num_particles; k++) {
                    ...
                    float v = vm_vec_mag_quick (
                        &Objects[shipp->objnum].phys_info.desired_vel);
                    ...
                    // How close they stick to that normal 0=on normal, 1=180,
                    // 2=360 degree
                    ...
                    particle::emit (
                        &pe, particle::PARTICLE_BITMAP,
                        tp->thruster_bitmap.first_frame);
                }
            }
        }
    }
...
void model_render_insignias (insignia_draw_data* insignia_data) {
    ...
    if ((pm->num_ins <= 0) || (bitmap_num < 0)) return;
    ...
    if (insignia_data->clip) {
        insignia_material.set_clip_plane (
            insignia_data->clip_normal, insignia_data->clip_position);
    }

    // otherwise render them
    for (idx = 0; idx < pm->num_ins; idx++) {
        // skip insignias not on our detail level
        if (pm->ins[idx].detail_level != detail_level) { continue; }
        ...
        for (s_idx = 0; s_idx < pm->ins[idx].num_faces; s_idx++) {
            ...
            light_apply_rgb (
                &vecs[0].r, &vecs[0].g, &vecs[0].b, &pm->ins[idx].vecs[i1],
                &pm->ins[idx].norm[i1], 1.5f);
            light_apply_rgb (
                &vecs[1].r, &vecs[1].g, &vecs[1].b, &pm->ins[idx].vecs[i2],
                &pm->ins[idx].norm[i2], 1.5f);
            light_apply_rgb (
                &vecs[2].r, &vecs[2].g, &vecs[2].b, &pm->ins[idx].vecs[i3],
                &pm->ins[idx].norm[i3], 1.5f);
            vecs[0].a = vecs[1].a = vecs[2].a = 255;

            // draw the polygon
            g3_render_primitives_colored_textured (
                &insignia_material, vecs, 3, PRIM_TYPE_TRIFAN, false);
        }
    }
}
...
void model_render_arc (
    ...
    if (secondary) {
        g3_render_rod (
            secondary, Num_arc_segment_points, Arc_segment_points,
            arc_width * 0.33f);
    }
}

void model_render_debug_children (
    polymodel* pm, int mn, int detail_level, uint debug_flags) {
    int i;
...
void model_render_debug (
    int model_num, matrix* orient, vec3d* pos, uint flags, uint debug_flags,
    int objnum, int detail_level_locked) {
    polymodel* pm = model_get (model_num);
    ...
    float depth = model_render_determine_depth (
        objnum, model_num, orient, pos, detail_level_locked);
    int detail_level = model_render_determine_detail (
        depth, objnum, model_num, orient, pos, flags, detail_level_locked);

    vec3d auto_back = ZERO_VECTOR;
    bool set_autocen = model_render_determine_autocenter (
        &auto_back, pm, detail_level, flags);

    if (set_autocen) { g3_start_instance_matrix (&auto_back, NULL, true); }
    ...
    if (debug_flags & MR_DEBUG_PIVOTS) {
        ...
        model_draw_debug_points (
            pm, &pm->submodel[pm->detail[detail_level]], flags);
    ...
    if (debug_flags & MR_DEBUG_SHIELDS) { model_render_shields (pm, flags); }
    ...
    if ((flags & MR_AUTOCENTER) && (set_autocen)) { g3_done_instance (true); }
...
void model_render_immediate (
    ...
    vec3d* pos, int render, bool sort) {
    model_draw_list model_list;
    ...
    switch (render) {
    case MODEL_RENDER_OPAQUE: model_list.render_all (ZBUFFER_TYPE_FULL); break;
    case MODEL_RENDER_TRANS:
        model_list.render_all (ZBUFFER_TYPE_READ);
        model_list.render_all (ZBUFFER_TYPE_NONE);
        break;
    case MODEL_RENDER_ALL: model_list.render_all (); break;
    }
    ...
    if (render_info->get_debug_flags ()) {
        model_render_debug (
            model_num, orient, pos, render_info->get_model_flags (),
            render_info->get_debug_flags (), render_info->get_object_number (),
            render_info->get_detail_level_lock ());
    }
}

void model_render_queue (
    model_render_params* interp, model_draw_list* scene, int model_num,
    matrix* orient, vec3d* pos) {
    int i;
    ...
    float light_factor =
        model_render_determine_light_factor (interp, pos, model_flags);
    ...
    if (interp->is_clip_plane_set ()) {
        rendering_material.set_clip_plane (
            interp->get_clip_plane_normal (), interp->get_clip_plane_pos ());
    }
        ...
        if (objp->type == OBJ_SHIP) {
            ...
            if (objp->type == OBJ_ASTEROID)
                pmi = model_get_instance (
                    Asteroids[objp->instance].model_instance_num);
    ...
    if (interp->get_animated_effect_num () >= 0) {
        rendering_material.set_animated_effect (
            interp->get_animated_effect_num (),
            interp->get_animated_effect_timer ());
    }

    bool is_outlines_only = (model_flags & MR_NO_POLYS) &&
                            ((model_flags & MR_SHOW_OUTLINE_PRESET) ||
                             (model_flags & MR_SHOW_OUTLINE));
    bool is_outlines_only_htl =
        (model_flags & MR_NO_POLYS) && (model_flags & MR_SHOW_OUTLINE_HTL);

    scene->push_transform (pos, orient);

    float depth = model_render_determine_depth (
        objnum, model_num, orient, pos, interp->get_detail_level_lock ());
    int detail_level = model_render_determine_detail (
        depth, objnum, model_num, orient, pos, model_flags,
        interp->get_detail_level_lock ());

    // If we're rendering attached weapon models, check against the ships'
    // tabled Weapon Model Draw Distance (which defaults to 200)
    if (model_flags & MR_ATTACHED_MODEL && shipp != NULL) {
        if (depth >
            Ship_info[shipp->ship_info_index].weapon_model_draw_distance) {
            scene->pop_transform ();
            return;
        }
    }
    ...
    vec3d auto_back = ZERO_VECTOR;
    bool set_autocen = model_render_determine_autocenter (
        &auto_back, pm, detail_level, model_flags);

    if (set_autocen) { scene->push_transform (&auto_back, NULL); }

    // if we're in nebula mode, fog everything except for the warp holes and
    // other non-fogged models
    if ((The_mission.flags[Mission::Mission_Flags::Fullneb]) &&
        (Neb2_render_mode != NEB2_RENDER_NONE) &&
        !(model_flags & MR_NO_FOGGING)) {
    ...
    if (interp->is_normal_alpha_set ()) {
        rendering_material.set_normal_alpha (
            interp->get_normal_alpha_min (), interp->get_normal_alpha_max ());
    }

    if (interp->uses_thick_outlines ()) {
        rendering_material.set_outline_thickness (
            interp->get_outline_thickness ());
    }

    if ((model_flags & MR_NO_CULL) || (model_flags & MR_ALL_XPARENT) ||
        (interp->get_warp_bitmap () >= 0)) {
    ...
    if (model_flags & MR_STENCIL_READ) {
        ...
        rendering_material.set_stencil_func (
            ComparisionFunction::NotEqual, 1, 0xFFFF);
        rendering_material.set_stencil_op (
        ...
        rendering_material.set_stencil_test (true);
        rendering_material.set_stencil_func (
            ComparisionFunction::Always, 1, 0xFFFF);
        rendering_material.set_stencil_op (
            StencilOperation::Keep, StencilOperation::Keep,
        ...
        rendering_material.set_stencil_func (
            ComparisionFunction::Never, 1, 0xFFFF);
        rendering_material.set_stencil_op (
            StencilOperation::Keep, StencilOperation::Keep,
            StencilOperation::Keep);
    }
    ...
    if ((tmap_flags & TMAP_FLAG_BATCH_TRANSFORMS)) {
        ...
        model_render_buffers (
            scene, &rendering_material, interp,
            &pm->detail_buffers[detail_level], pm, -1, detail_level,
            tmap_flags);
    }
    ...
    if (model_render_check_detail_box (
            &view_pos, pm, pm->detail[detail_level], model_flags)) {
        int detail_model_num = pm->detail[detail_level];

        if ((is_outlines_only || is_outlines_only_htl) &&
            pm->submodel[detail_model_num].outline_buffer != NULL) {
            color outline_color = interp->get_color ();
            scene->add_outline (
                pm->submodel[detail_model_num].outline_buffer,
                pm->submodel[detail_model_num].n_verts_outline,
                &outline_color);
        }
        else {
            model_render_buffers (
                scene, &rendering_material, interp,
                &pm->submodel[detail_model_num].buffer, pm, detail_model_num,
                detail_level, tmap_flags);

            if (pm->submodel[detail_model_num].num_arcs) {
                model_render_add_lightning (
                    scene, interp, pm, &pm->submodel[detail_model_num]);
            }
        }
    }
    ...
    if (pm->flags & PM_FLAG_TRANS_BUFFER &&
        ...
        while (i >= 0) {
            if (!pm->submodel[i].is_thruster) {
                model_render_children_buffers (
                    scene, &rendering_material, interp, pm, pmi, i,
                    detail_level, tmap_flags, trans_buffer);
            }
        ...
        if (model_render_check_detail_box (
            ...
            model_render_buffers (
                scene, &rendering_material, interp,
                &pm->submodel[detail_model_num].trans_buffer, pm,
                detail_model_num, detail_level, tmap_flags);
        }
    }
    ...
    if (!(model_flags & MR_NO_TEXTURING)) {
        scene->add_insignia (
            interp, pm, detail_level, interp->get_insignia_bitmap ());
    }
    ...
    if ((pm->n_glow_point_banks) && !is_outlines_only &&
        !is_outlines_only_htl && !Glowpoint_override) {
        model_render_glow_points (
            pm, shipp, orient, pos, Glowpoint_use_depth_buffer);
    }
    ...
    if (!is_outlines_only && !is_outlines_only_htl) {
        if ((model_flags & MR_AUTOCENTER) && set_autocen) {
            ...
            model_queue_render_thrusters (
                interp, pm, objnum, shipp, orient, &autoback_rotated);
        }
        else {
            model_queue_render_thrusters (
                interp, pm, objnum, shipp, orient, pos);
        }
    }
}

void model_render_set_wireframe_color (color* clr) { Wireframe_color = *clr; }
