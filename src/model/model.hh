...
#define MOVEMENT_TYPE_INTRINSIC_ROTATE \
    4 // intrinsic (non-subsystem-based) rotation
#define MOVEMENT_TYPE_LOOK_AT \
    5 // the subobject is always looking at a 'look at' subobject, as best it
      // can - Bobboau
...
// Hoffoss: Please make sure that subsystem NONE is always index 0, and UNKNOWN
// is always the last subsystem in the list.  Also, make sure that MAX is
// correct. Otherwise, problems will arise in Fred.
...
#define MAX_SPLIT_PLANE \
    5 // number of artist specified split planes (used in big ship explosions)
...
public:
    ...
    char name[MAX_NAME_LEN]; // name of the subsystem.  Probably displayed on
                             // HUD
    char subobj_name[MAX_NAME_LEN]; // Temporary (hopefully) parameter used to
                                    // match stuff in ships.tbl
    char alt_sub_name[NAME_LENGTH]; // Karajorma - Name that overrides name of
                                    // original
    char alt_dmg_sub_name[NAME_LENGTH]; // Name for the damage popup
                                        // subsystems, allows for translation
    int subobj_num; // subobject number (from bspgen) -- used to match
                    // subobjects of subsystems to these entries; index to
                    // polymodel->submodel
    int model_num;  // Which model this is attached to (i.e. the polymodel[]
                    // index); same as polymodel->id
    ...
    int turret_gun_sobj; // Which subobject in this model the firing points are
                         // linked to.
    float
        turret_turning_rate; // How fast the turret turns. Read from ships.tbl
    gamesnd_id turret_base_rotation_snd; // Sound to make when the turret moves
    float turret_base_rotation_snd_mult; // Volume multiplier for the turret
                                         // sounds
    gamesnd_id turret_gun_rotation_snd;  // Sound to make when the turret moves
    float turret_gun_rotation_snd_mult;  // Volume multiplier for the turret
                                         // sounds
    ...
    gamesnd_id rotation_snd; // Sound to make when the subsystem is rotating.
                             // (ie turrets)

    // engine wash info
    struct engine_wash_info*
        engine_wash_pointer; // index into Engine_wash_info

    // rotation specific info
    float turn_rate; // The turning rate of this subobject, if MSS_FLAG_ROTATES
                     // is set.
    ...
    int primary_banks[MAX_SHIP_PRIMARY_BANKS]; // default primary weapons
                                               // -hoffoss
    int primary_bank_capacity[MAX_SHIP_PRIMARY_BANKS]; // capacity of a bank -
                                                       // Goober5000
    int secondary_banks[MAX_SHIP_SECONDARY_BANKS]; // default secondary weapons
                                                   // -hoffoss
    int secondary_bank_capacity[MAX_SHIP_SECONDARY_BANKS]; // capacity of a
                                                           // bank -hoffoss
    int path_num; // path index into polymodel .paths array.  -2 if none
                  // exists, -1 if not defined

    int n_triggers;
    queued_animation*
        triggers; // all the triggered animations associated with this object
...
public:
    bsp_info ()
        : movement_type (-1), movement_axis (0), can_move (false),
          bsp_data_size (0), bsp_data (NULL), collision_tree_index (-1),
          rad (0.0f), blown_off (0), my_replacement (-1), i_replace (-1),
          is_live_debris (0), num_live_debris (0), is_thruster (0),
          is_damaged (0), parent (-1), num_children (0), first_child (-1),
          next_sibling (-1), num_details (0), num_arcs (0),
          outline_buffer (NULL), n_verts_outline (0),
          render_sphere_radius (0.0f), use_render_box (0),
          use_render_box_offset (false), use_render_sphere (0),
          use_render_sphere_offset (false), gun_rotation (false),
          no_collisions (false), nocollide_this_only (false),
          collide_invisible (false), force_turret_normal (false),
          attach_thrusters (false), dumb_turn_rate (0.0f), look_at_num (-1) {
        name[0] = 0;
        lod_name[0] = 0;
        look_at[0] = 0;

        offset = geometric_center = min = max = render_box_min =
            render_box_max = render_box_offset = render_sphere_offset =
                vmd_zero_vector;
    ...
    char name[MAX_NAME_LEN]; // name of the subsystem.  Probably displayed on
                             // HUD
    int movement_type; // -1 if no movement, otherwise rotational or positional
                       // movement -- subobjects only
    int movement_axis; // which axis this subobject moves or rotates on.
    bool can_move; // If true, the position and/or orientation of this submodel
                   // can change due to rotation of itself OR a parent
    ...
    int blown_off;      // If set, this subobject is blown off. Stuffed by
                        // model_set_instance
    int my_replacement; // If not -1 this subobject is what should get rendered
                        // instead of this one
    ...
    int live_debris[MAX_LIVE_DEBRIS]; // array of live debris submodels for a
                                      // submodel
    ...
    int num_details; // How many submodels are lower detail "mirrors" of this
                     // submodel
    int details[MAX_MODEL_DETAIL_LEVELS]; // A list of all the lower detail
                                          // "mirrors" of this submodel
    ...
    int use_render_box; // 0==do nothing, 1==only render this object if you are
                        // inside the box, -1==only if you're outside
    bool use_render_box_offset; // whether an offset has been defined; needed
                                // because one can't tell just by looking at
                                // render_box_offset
    int use_render_sphere; // 0==do nothing, 1==only render this object if you
                           // are inside the sphere, -1==only if you're outside
    bool use_render_sphere_offset; // whether an offset has been defined;
                                   // needed because one can't tell just by
                                   // looking at render_sphere_offset
...
struct model_path  {
    ...
    int type; // What this path takes you to... See MP_TYPE_??? defines above
              // for details
...
struct thruster_bank  {
    ...
    int obj_num;      // what subsystem number this bank is on; index to
...
struct dock_bay  {
    ...
    int type_flags; // indicates what this docking bay can be used for (i.e.
                    // cargo/rearm, etc)
    int num_spline_paths; // number of spline paths which lead to this docking
                          // bay
...
struct ship_bay_t  {
    ...
    int path_indexes[MAX_SHIP_BAY_PATHS]; // index into polymodel->paths[]
                                          // array
    int arrive_flags; // bitfield, set to 1 when that path number is reserved
                      // for an arrival
...
struct shield_info {
    ...
    shield_info ()
        : nverts (0), ntris (0), verts (NULL), tris (NULL), buffer_id (-1),
          buffer_n_verts (0), layout () {}
};
...
struct model_octant  {
    ...
    vec3d** verts;    // The vertices in this octant in the high-res hull.  A
                      // vertex can only be in one octant.
...
struct insignia  {
    ...
    float u[MAX_INS_FACES]
           [MAX_INS_FACE_VECS]; // u tex coords on a per-face-per-vertex basis
    float v[MAX_INS_FACES]
           [MAX_INS_FACE_VECS]; // v tex coords on a per-face-per-vertex bases
...
#define PM_FLAG_HAS_INTRINSIC_ROTATE \
    (1 << 4) // whether this model has an intrinsic rotation submodel somewhere

// Goober5000
class texture_info {
private:
    int original_texture; // what gets read in from file
    int texture; // what texture you draw with; reset to original_textures by
                 // model_set_instance
...
#define TM_AMBIENT_TYPE \
    7 // optional ambient occlusion map with ambient occlusion and cavity
      // occlusion factors for red and green channels.
#define TM_NUM_TYPES \
    8 // WMC - Number of texture_info objects in texture_map
...
public:
    ...
    texture_map () : is_ambient (false), is_transparent (false) {}
};
...
public:
    ...
    polymodel ()
        : id (-1), version (0), flags (0), n_detail_levels (0),
          num_debris_objects (0), n_models (0), num_lights (0), lights (NULL),
          n_view_positions (0), rad (0.0f), core_radius (0.0f), n_textures (0),
          submodel (NULL), n_guns (0), n_missiles (0), n_docks (0),
          n_thrusters (0), gun_banks (NULL), missile_banks (NULL),
          docking_bays (NULL), thrusters (NULL), ship_bay (NULL), shield (),
          shield_collision_tree (NULL), sldc_size (0), n_paths (0),
          paths (NULL), mass (0), num_xc (0), xc (NULL), num_split_plane (0),
          num_ins (0), used_this_mission (0), n_glow_point_banks (0),
          glow_point_banks (NULL), gun_submodel_rotation (0), vert_source () {
        filename[0] = 0;
    ...
    eye view_positions[MAX_EYES]; // viewing positions.  Default to {0,0,0}. in
                                  // location 0

    vec3d autocenter; // valid only if PM_FLAG_AUTOCEN is set

    float rad; // The radius of everything in the model; shields, thrusters.
    float core_radius; // The radius to be used for collision detection in
                       // small ship vs big ships. This is equal to 1/2 of the
                       // smallest dimension of the hull's bounding box.
    ...
    // linked lists for special polygon types on this model.  Most ships I
    // think will have most of these.  (most ships however, probably won't have
    // approach points).
    int n_guns;        // number of primary gun points (not counting turrets)
    int n_missiles;    // number of secondary missile points (not counting
                       // turrets)
    ...
    thruster_bank* thrusters; // array of thruster objects -- likely to change
                              // in the future
    ship_bay_t*
        ship_bay; // contains path indexes for ship bay approach/depart paths
    ...
    cross_section*
        xc; // pointer to array of cross sections (used in big ship explosions)

    int num_split_plane;                // number of split planes
    float split_plane[MAX_SPLIT_PLANE]; // actual split plane z coords (for big
                                        // ship explosions)
    ...
    int used_this_mission; // used for page-in system, how many times this
                           // model has been loaded per mission - taylor
...
// call to unload a model (works like bm_unload()), "force" SHOULD NEVER BE SET
// outside of modelread.cpp!!!!
...
// routine to copy susbsystems.  Must be called when subsystems sets are the
// same -- see ship.cpp
...
#define MR_SHOW_OUTLINE \
    (1 << 0) // Draw the object in outline mode. Color specified by
...
#define MR_STENCIL_WRITE \
    (1 << 3) // Write stencil buffere where the model was rendered
#define MR_STENCIL_READ \
    (1 << 4) // Only draw pixels of the model where the stencil buffer has the
             // right value (see MR_STENCIL_WRITE)
#define MR_SHOW_THRUSTERS \
    (1 << 5) // Show the engine thrusters. See model_set_thrust for how long it
             // draws.
#define MR_NO_COLOR_WRITES \
    (1 << 6) // Don't write anything to the color buffers (used when setting
             // the stencil buffer)
...
#define MR_IS_MISSILE \
    (1 << 13) // When set, treat this as a missilie.  No lighting, small
              // thrusters.
#define MR_SHOW_OUTLINE_PRESET \
    (1 << 14) // Draw the object in outline mode. Color assumed to be set
              // already.
#define MR_SHOW_INVISIBLE_FACES (1 << 15) // Show invisible faces as green...
#define MR_AUTOCENTER \
    (1 << 16) // Always use the center of the hull bounding box as the center,
              // instead of the pivot point
...
#define MR_EDGE_ALPHA \
    (1 << 23) // makes norms that are faceing away from you render more
              // transparent -Bobboau
#define MR_CENTER_ALPHA (1 << 24) // oposite of above -Bobboau
#define MR_NO_FOGGING (1 << 25)   // Don't fog - taylor
#define MR_SHOW_OUTLINE_HTL \
    (1 << 26) // Show outlines (wireframe view) using HTL method
#define MR_NO_GLOWMAPS (1 << 27) // disable rendering of glowmaps - taylor
#define MR_FULL_DETAIL \
    (1 << 28) // render all valid objects, particularly ones that are otherwise
              // in/out of render boxes - taylor
...
// Defines for the render parameter of model_render, model_really_render and
...
// Returns the core radius (smallest dimension of hull's bounding box, used for
// collision detection with big ships only)
...
extern int model_find_2d_bound (
...
// rect.
extern int model_find_2d_bound_min (
    int model_num, matrix* orient, vec3d* pos, int* x1, int* y1, int* x2,
    int* y2);
...
extern void
model_find_submodel_offset (vec3d* outpnt, int model_num, int sub_model_num);
...
extern void find_submodel_instance_world_point (
    vec3d* outpnt, int model_instance_num, int submodel_num,
    const matrix* objorient, const vec3d* objpos);

// Given a polygon model index, find a list of rotating submodels to be used
// for collision
void model_get_rotating_submodel_list (
    std::vector< int >* submodel_vector, object* objp);

// Given a polygon model index, get a list of a model tree starting from that
// index
...
extern void model_set_instance (
    int model_num, int sub_model_num, submodel_instance_info* sii,
    flagset< Ship::Subsystem_Flags >* flags = NULL);
extern void model_set_instance_techroom (
...
extern int submodel_get_points (
    int model_num, int submodel_num, int max_num, vec3d** nts);

// Gets two random points on the surface of a submodel
extern void submodel_get_two_random_points (
    int model_num, int submodel_num, vec3d* v1, vec3d* v2, vec3d* n1 = NULL,
    vec3d* n2 = NULL);
extern void submodel_get_two_random_points_better (
    int model_num, int submodel_num, vec3d* v1, vec3d* v2);
// gets the index into the docking_bays array of the specified type of docking
// point Returns the index.  second functions returns the index of the docking
// bay with the specified name
extern int
model_find_dock_index (int modelnum, int dock_type, int index_to_start_at = 0);
...
struct mc_info  {
    ...
    int flags;    // Flags that the model_collide code looks at.  See MC_???
                  // defines
    float radius; // If MC_CHECK_THICK is set, checks a sphere moving with the
                  // radius.
    ...
    vec3d hit_point;       // Where the collision occurred at in hit_submodel's
                           // coordinate system
    vec3d hit_point_world; // Where the collision occurred at in world
                           // coordinates
...
#define MC_ONLY_BOUND_BOX \
    (1 << 3) // Only check bounding boxes.  Pretty accurate and slower than
             // MC_ONLY_SPHERE. Checks the rotatated bounding box of each
             // submodel. NOTE!  This doesn't set hit_point correctly with
             // MC_CHECK_SPHERELINE
#define MC_CHECK_RAY \
    (1 << 4) // Checks a ray from p0 *through* p1 on to infinity
#define MC_CHECK_SPHERELINE \
    (1 << 5) // Checks a moving sphere rather than just a ray.  Radius
#define MC_SUBMODEL \
    (1 << 6) // If this is set, only check the submodel specified in
             // mc->submodel_num. Use with MC_CHECK_MODEL
...
void model_collide_parse_bsp (
    bsp_collision_tree* tree, void* model_ptr, int version);
...
void model_collide_preprocess (
    matrix* orient, int model_instance_num, int detail = 0);
...
// data.
int model_which_octant_distant (
...
struct mst_info  {
    ...
    mst_info ()
        : primary_bitmap (-1), primary_glow_bitmap (-1),
          secondary_glow_bitmap (-1), tertiary_glow_bitmap (-1),
          distortion_bitmap (-1), use_ab (false), glow_noise (1.0f),
          rotvel (vmd_zero_vector), length (vmd_zero_vector),
          glow_rad_factor (1.0f), secondary_glow_rad_factor (1.0f),
...
// set the insignia bitmap to be used when rendering a ship with an insignia
// (-1 switches it off altogether)
...
// see if the given texture is used by the passed model. 0 if not used, 1 if
// used, -1 on error
int model_find_texture (int model_num, int bitmap);

// find closest point on extended bounding box (the bounding box plus all the
// planes that make it up) returns closest distance to extended box positive
// return value means start_point is outside extended box displaces closest
// point an optional amount delta to the outside of the box closest_box_point
// can be NULL.
float get_world_closest_box_point_with_delta (
    vec3d* closest_box_point, object* box_obj, vec3d* start_point,
...
void model_set_warp_globals (
    float scale_x = 1.0f, float scale_y = 1.0f, float scale_z = 1.0f,
    int bitmap_id = -1, float alpha = -1.0f);
...
#endif // FREESPACE2_MODEL_MODEL_HH
