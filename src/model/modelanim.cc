...
int model_anim_match_type(char *p)
{
        ...
        if (!strncasecmp(p, "docking", 7) || !strncasecmp(p, "\"docking\"", 9)) {
                ...
                !strncasecmp(p, "primary_bank", 12) || !strncasecmp(p, "\"primary_bank\"", 14)) {
                ...
                !strncasecmp(p, "secondary_bank", 14) || !strncasecmp(p, "\"secondary_bank\"", 16)) {
                ...
                !strncasecmp(p, "turret firing", 13) || !strncasecmp(p, "\"turret firing\"", 15)) {
...
void triggered_rotation::add_queue(queued_animation *the_queue, int dir)
{
                ...
                if (i != n_queue) {
                        ...
                        if (i < (MAX_TRIGGERED_ANIMATIONS - 1))
                                memmove(
                                        ...
                                        sizeof(queued_animation) * (MAX_TRIGGERED_ANIMATIONS - (i + 1)));
        ...
        if (new_queue.instance == instance) {
                ...
                if ((direction.xyz.x * rot_vel.xyz.x) == new_queue.vel.xyz.x && (direction.xyz.y * rot_vel.xyz.y) == new_queue.vel.xyz.y && (direction.xyz.z * rot_vel.xyz.z) == new_queue.vel.xyz.z) {
        ...
        if (n_queue > 0) {
                ...
                for (i = 0; (i < n_queue) && (i < MAX_TRIGGERED_ANIMATIONS) && (new_queue.start_time > queue_tmp[i].start_time);
                     i++)
                        ;
...
void queued_animation_correct(queued_animation *qa)
{
        for (int i = 0; i < 3; i++) {
                ...
                if (((qa->vel.a1d[i] * qa->vel.a1d[i]) / qa->accel.a1d[i]) > fabs(qa->angle.a1d[i]))
                        qa->vel.a1d[i] = sqrtf(fabs(qa->accel.a1d[i] * qa->angle.a1d[i]));
        }
}
        ...
        model_subsystem *psub, ship_subsys *ss)
{
        ...
        sii->angs.p = trigger->current_ang.xyz
                              .x; //- (2.0f * PI2 * (trigger->current_ang.xyz.x / (2.0f * PI2)));
        sii->angs.h = trigger->current_ang.xyz
                              .y; //- (2.0f * PI2 * (trigger->current_ang.xyz.y / (2.0f * PI2)));
        sii->angs.b = trigger->current_ang.xyz
        ...
        model_subsystem *psub, queued_animation *anim_q, int subtype)
{
        ...
        if (anim_q->type == TRIGGER_TYPE_DOCK_BAY_DOOR) {
                int anim_subtype = anim_q->subtype - 1; // in the tables, bay door +sub_types are
...
int model_anim_instance_get_actual_time(queued_animation *properties)
{
        ...
        for (int a = 0; a < 3; a++) {
                temp = int(
                               ((3.0f * properties->vel.a1d[a] * properties->vel.a1d[a]) + (2.0f * properties->accel.a1d[a] * fabs(properties->angle.a1d[a]))) / (2.0f * properties->accel.a1d[a] * properties->vel.a1d[a]) * 1000.0f)
                       + properties->start;
                ...
                for (i = 0; i < sip->n_subsystems; i++) {
                        ...
                        for (j = 0; j < psub->n_triggers; j++) {
                                if (psub->triggers[j].type == type) {
                                        ...
                                        psub->triggers[j].real_end_time = model_anim_instance_get_actual_time(
                                                &psub->triggers[j]);
                                }
                        }
                }
                        ...
                        if ((tr->current_vel.a1d[0] != 0.0f) || (tr->current_vel.a1d[1] != 0.0f) || (tr->current_vel.a1d[2] != 0.0f)) {
                                ...
                                for (int a = 0; a < 3; a++) {
                                        ...
                                        if (end_angle > tr->slow_angle.a1d[a]) {
                                                ...
                                                a_time = int(
                                                        ((((2 * tr->rot_vel.a1d[a]) - tr->current_ang.a1d[a]) / tr->rot_accel.a1d[a]) + tr->slow_angle.a1d[a] - tr->current_ang.a1d[a] + (((tr->current_vel.a1d[a] * tr->current_vel.a1d[a]) - (tr->rot_vel.a1d[a] * tr->rot_vel.a1d[a])) / (2 * tr->rot_accel.a1d[a]))) * 1000.0f);
                                                if (ani_time < a_time)
                                                        ani_time = a_time;
                                        } else {
                                                ...
                                                a_time = int(
                                                        (2 * sqrtf(((tr->end_angle.a1d[a] - tr->current_ang.a1d[a]) / tr->rot_accel.a1d[a]) - ((tr->current_vel.a1d[a] * tr->current_vel.a1d[a]) / (tr->rot_accel.a1d[a] * tr->rot_accel.a1d[a])) / 2) - (tr->current_vel.a1d[a] / tr->rot_accel.a1d[a])) * 1000.0f);
                                                if (ani_time < a_time)
                                                        ani_time = a_time;
                                        }
                                }
