...
{
        Replacement_textures = (int *)malloc(MAX_REPLACEMENT_TEXTURES * sizeof(int));
        ...
        vertex_buffer *buffer, size_t texi, uint tmap_flags)
{
        ...
        if (Rendering_to_shadow_map) {
                ...
                // a crude hack...
                auto possibly_deferred = draw_data.render_material.get_depth_mode() == ZBUFFER_TYPE_FULL && gr_is_capable(CAPABILITY_DEFERRED_LIGHTING) && !Cmdline_no_deferred_lighting;
        ...
        for (size_t i = 0; i < buffer_size; i++) {
                ...
                if (forced_texture != -2) {
                        ...
                        if ((replacement_textures != NULL) && (replacement_textures[rt_begin_index + TM_BASE_TYPE] == REPLACE_WITH_INVISIBLE)) {
                                ...
                                (replacement_textures != NULL) && (replacement_textures[rt_begin_index + TM_BASE_TYPE] >= 0)) {
                        ...
                        if (!(debug_flags & MR_DEBUG_NO_SPEC)) {
                                if (replacement_textures != NULL && replacement_textures[rt_begin_index + TM_SPECULAR_TYPE] >= 0) {
                                        tex_replace[TM_SPECULAR_TYPE] = texture_info(replacement_textures
                        ...
                        if (detail_level < 2) {
                                ...
                                if (replacement_textures != NULL) {
                                        ...
                                        if (replacement_textures[rt_begin_index + TM_MISC_TYPE] >= 0) {
                                                tex_replace[TM_MISC_TYPE] = texture_info(replacement_textures
                                ...
                                if (!(debug_flags & MR_DEBUG_NO_AMBIENT) && Detail.lighting > 0)
        ...
        int mn, int detail_level, uint tmap_flags, bool trans_buffer)
{
        ...
        if ((model_flags & MR_SHOW_OUTLINE || model_flags & MR_SHOW_OUTLINE_HTL || model_flags & MR_SHOW_OUTLINE_PRESET) && pm->submodel[mn].outline_buffer != NULL) {
        ...
        case 0: {
                ...
                if (gpo && gpo->pulse_type) {
                        ...
                        case PULSE_SHIFTTRI:
                                ...
                                if ((gpo && gpo->off_time_override) ? gpo->off_time
                                                                    : bank->off_time) {
                                        x %= (((gpo && gpo->on_time_override) ? gpo->on_time
                                                                              : bank->on_time)
                ...
                if (Detail.lighting > 3 && Deferred_lighting && gpo && gpo->is_lightsource) {
                        if (gpo->lightcone) {
                                ...
                                light_add_cone(
                                        ...
                                        pulse * gpo->light_color.xyz.x + (1.0f - pulse) * gpo->light_mix_color.xyz.x,
                                        pulse * gpo->light_color.xyz.y + (1.0f - pulse) * gpo->light_mix_color.xyz.y,
                                        pulse * gpo->light_color.xyz.z + (1.0f - pulse) * gpo->light_mix_color.xyz.z,
                                        -1);
                        } else {
                                light_add_point(
                ...
                if ((gpo && gpo->off_time_override) ? gpo->off_time : bank->off_time) {
                        if (bank->is_on) {
                                if (((gpo && gpo->on_time_override) ? gpo->on_time
                                                                    : bank->on_time)
                                    > ((time - ((gpo && gpo->disp_time_override) ? gpo->disp_time : bank->disp_time)) % (((gpo && gpo->on_time_override) ? gpo->on_time : bank->on_time) + ((gpo && gpo->off_time_override) ? gpo->off_time : bank->off_time)))) {
                                        bank->glow_timestamp = time;
                                        bank->is_on = 0;
                                }
                        } else {
                ...
                if ((gpo && gpo->off_time_override && !gpo->off_time) ? gpo->is_on
                        ...
                        for (j = 0; j < bank->num_points; j++) {
                                ...
                                if (pm->submodel[pm->detail[0]].num_arcs) {
                                        ...
                                        // will fail
                                        flick = fs2::prng::randi(0) % (pm->submodel[pm->detail[0]].num_arcs + j);
                                } else {
                                        flick = 1;
                                }
                        ...
                        if (thruster_info.secondary_glow_bitmap >= 0) {
                                ...
                                if (d > 0.0f) {
                                        ...
                                        if (Scene_framebuffer_in_frame && thruster_info.draw_distortion && Cmdline_fb_thrusters) {
                                                ...
                                                mag -= (float)((int)mag); // Valathil - Get a fairly
        ...
        matrix *orient, vec3d *pos)
{
        ...
        if (!(model_flags & MR_NO_BATCH) && pm->flags & PM_FLAG_BATCHED && !(is_outlines_only || is_outlines_only_htl)) {
