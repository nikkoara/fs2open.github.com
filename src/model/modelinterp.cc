...
static float Interp_box_scale =
    1.0f; // this is used to scale both detail boxes and spheres
...
void model_allocate_interp_data (int n_verts, int n_norms) {
    ...
    if (n_verts > Num_interp_verts_allocated) {
        ...
        // model collide needs a similar size to resize it based on this new
        // value
    ...
    if (n_norms > Num_interp_norms_allocated) {
        ...
        memset (
            Interp_lighting_temp.lights, 0, n_norms * sizeof (model_light));
...
// +offset             vertex data. Each vertex n is a point followed by
// norm_counts[n] normals.
void model_interp_splode_defpoints (
    ubyte* p, polymodel* /*pm*/, bsp_info* /*sm*/, float dist) {
    if (dist == 0.0f) return;
    ...
    for (n = 0; n < nverts; n++) { nnorms += normcount[n]; }
...
void model_interp_defpoints (ubyte* p, polymodel* pm, bsp_info* sm) {
    ...
    for (i = 0; i < nverts; i++) { nnorms += normcount[i]; }

    // allocate new Interp data if size is greater than we already have ready
    // to use
    model_allocate_interp_data (nverts, nnorms);
    ...
    if (Interp_thrust_scale_subobj) {
        ...
        if (Interp_flags & MR_IS_MISSILE) { min_thruster_dist = 0.5f; }
        ...
        (Interp_warp_scale_x != 1.0f) || (Interp_warp_scale_y != 1.0f) ||
        (Interp_warp_scale_z != 1.0f)) {
        for (n = 0; n < nverts; n++) {
            vec3d tmp;
...
void model_interp_edge_alpha (
    ubyte* param_r, ubyte* param_g, ubyte* param_b, vec3d* pnt, vec3d* norm,
    float alpha, bool invert = false) {
    vec3d r;
...
void model_draw_debug_points (polymodel* pm, bsp_info* submodel, uint flags) {
    if (flags & MR_SHOW_OUTLINE_PRESET) { return; }
    ...
    if (submodel) {
        ...
        for (j = 0; j < 8; j++) {
            ...
            model_calc_bound_box (
                bounding_box, &pm->octants[j].min, &pm->octants[j].max);
        ...
        for (j = 0; j < pm->paths[i].nverts; j++) {
            ...
            // point.
            {
                ...
                if (pm->paths[i].verts[j].nturrets > 0) {
                    gr_set_color (
                        0, 0, 255); // draw points covered by turrets in blue
                }
                else {
                ...
                if (j) { g3_draw_line (&prev_pnt, &tmp); }
...
void model_draw_bay_paths_htl (int model_num) {
    ...
    for (idx = 0; idx < pm->n_docks; idx++) {
        for (s_idx = 0; s_idx < pm->docking_bays[idx].num_slots; s_idx++) {
            ...
            vm_vec_scale_add (
                &v2, &v1, &pm->docking_bays[idx].norm[s_idx], 10.0f);
...
int interp_box_offscreen (vec3d* min, vec3d* max) {
    if (keyd_pressed[KEY_LSHIFT]) { return IBOX_ALL_ON; }
    ...
    if (or_codes) { return IBOX_SOME_ON_SOME_OFF; }
 ...
 */
int model_get_rotated_bitmap_points (
    vertex* pnt, float angle, float rad, vertex* v) {
    float sa, ca;
    int i;
    ...
    v[0].world.xyz.x =
        (-width * ca - height * sa) * Matrix_scale.xyz.x + pnt->world.xyz.x;
    v[0].world.xyz.y =
        (-width * sa + height * ca) * Matrix_scale.xyz.y + pnt->world.xyz.y;
    ...
    if (codes_and) return 1; // 1 means off screen
...
DCF (model_darkening, "Makes models darker with distance") {
    if (dc_optional_string_either ("help", "--help")) {
        ...
        dc_printf (
            "Sets the distance at which to start blacking out models (namely "
            "asteroids).\n");
        return;
    }
 ...
 */
float interp_closest_dist_to_box (
    vec3d* hitpt, vec3d* p0, vec3d* min, vec3d* max) {
...
float model_find_closest_point (
    ...
    return interp_closest_dist_to_box (
        &closest_pos, &eye_rel_pos, &pm->submodel[submodel_num].min,
        &pm->submodel[submodel_num].max);
}
...
DCF (tiling, "Toggles rendering of tiled textures (default is on)") {
    ...
    tiling = !tiling;
    if (tiling) { dc_printf ("Tiled textures\n"); }
    else {
...
static int submodel_get_points_internal (int model_num, int submodel_num) {
    ...
    while (chunk_type != OP_EOF) {
        ...
        case OP_DEFPOINTS: {
            ...
            // this must happen only after the interp_data allocation call
            // (since the address changes)
 ...
 */
void submodel_get_two_random_points (
    int model_num, int submodel_num, vec3d* v1, vec3d* v2, vec3d* n1,
    vec3d* n2) {
    int nv = submodel_get_points_internal (model_num, submodel_num);

    // this is not only because of the immediate div-0 error but also because
    // of the less immediate expectation for at least one point (preferably
    // two) to be found
    if (nv <= 0) {
        polymodel* pm = model_get (model_num);
        ...
        vm_vec_zero (v1);
        vm_vec_zero (v2);
        if (n1 != NULL) { vm_vec_zero (n1); }
        if (n2 != NULL) { vm_vec_zero (n2); }
        return;
    }
    ...
    *v2 = *Interp_verts[vn2];
    ...
    if (n1 != NULL) { *n1 = *Interp_norms[vn1]; }
    if (n2 != NULL) { *n2 = *Interp_norms[vn2]; }
}

void submodel_get_two_random_points_better (
    ...
    if (pm != NULL) {
        ...
        bsp_collision_tree* tree = model_get_bsp_collision_tree (
            pm->submodel[submodel_num].collision_tree_index);

        int nv = tree->n_verts;

        // this is not only because of the immediate div-0 error but also
...
void model_set_detail_level (int n) { Interp_detail_level_locked = n; }
 ...
 */
void model_set_insignia_bitmap (int bmap) { Interp_insignia_bitmap = bmap; }
 ...
 */
void model_set_forced_texture (int bmap) { Interp_forced_bitmap = bmap; }

/**
 * Set model transparency for use with MR_ALL_XPARENT
 */
void model_set_alpha (float alpha) { Interp_xparent_alpha = alpha; }
...
int model_find_texture (int model_num, int bitmap) {
    ...
    for (idx = 0; idx < pm->n_textures; idx++) {
        if (pm->maps[idx].FindTexture (bitmap) > -1) { return 1; }
    }
...
// can be NULL.
float get_model_closest_box_point_with_delta (
    vec3d* closest_box_point, vec3d* start_point, int modelnum, int* is_inside,
    float delta) {
    int i, idx;
    ...
    for (i = 0; i < 6; i++) {
        ...
        dist = fvi_ray_plane (
            &box_point, extremes, &ray_direction, start_point, &ray_direction,
            0.0f);
        if (dist > 0) { inside |= masks[i]; }
        if (fabs (dist) < fabs (best_dist)) {
            best_dist = dist;
            if (closest_box_point) {
                vm_vec_scale_add (
                    closest_box_point, &box_point, &ray_direction, delta);
            }
        }
    }

    // is start_point inside the box
    if (is_inside) { *is_inside = (inside == mask_inside); }
...
float get_world_closest_box_point_with_delta (
    ...
    dist = get_model_closest_box_point_with_delta (
        closest_box_point, &box_start, modelnum, is_inside, delta);
...
void model_page_in_textures (int modelnum, int ship_info_index) {
    ...
    if (ship_info_index >= 0) ship_page_in_textures (ship_info_index);
}
...
void model_page_out_textures (int model_num, bool release) {
    ...
    if (model_num < 0) return;

    polymodel* pm = model_get (model_num);

    if (pm == NULL) return;

    if (release && (pm->used_this_mission > 0)) return;

    for (i = 0; i < pm->n_textures; i++) { pm->maps[i].PageOut (release); }

    // NOTE: "release" doesn't work here for some, as of yet unknown, reason -
    // taylor
...
void parse_bsp (int offset, ubyte* bsp_data) {
    ...
    while (id != 0) {
        switch (id) {
        case OP_DEFPOINTS: parse_defpoint (offset, bsp_data); break;

        case OP_SORTNORM: parse_sortnorm (offset, bsp_data); break;

        case OP_FLATPOLY: break;

        case OP_TMAPPOLY: parse_tmap (offset, bsp_data); break;
...
void find_defpoint (int off, ubyte* bsp_data) {
    ...
    for (n = 0; n < nverts; n++) { norm_num += normcount[n]; }
...
void find_tri_counts (int offset, ubyte* bsp_data) {
    ...
    while (id != 0) {
        switch (id) {
        case OP_DEFPOINTS: find_defpoint (offset, bsp_data); break;

        case OP_SORTNORM: find_sortnorm (offset, bsp_data); break;

        case OP_FLATPOLY: break;

        case OP_TMAPPOLY: find_tmap (offset, bsp_data); break;
...
void model_interp_submit_buffers (
    ...
    if (vert_src->Vertex_list != NULL) {
        size_t offset;
        gr_heap_allocate (
            GpuHeap::ModelVertex, vert_src->Vertex_list_size,
            vert_src->Vertex_list, offset, vert_src->Vbuffer_handle);
    ...
    if (vert_src->Index_list != NULL) {
        gr_heap_allocate (
            GpuHeap::ModelIndex, vert_src->Index_list_size,
...
bool model_interp_pack_buffer (
    ...
    auto array = reinterpret_cast< interp_vertex* > (
        static_cast< uint8_t* > (vert_src->Vertex_list) + (vb->vertex_offset));
    ...
    for (i = 0; i < n_verts; i++) {
        ...
        // NOTE: UV->NORM->TSB->MODEL_ID->VERT, This array order *must* be
        // preserved!!

        // tex coords
        if (vb->flags & VB_FLAG_UV1) { outVert->uv = vl->texture_position; }
        else {
            outVert->uv.u = 1.0f;
            outVert->uv.v = 1.0f;
        }
    ...
    for (j = 0; j < vb->tex_buf.size (); j++) {
        ...
        if (vb->tex_buf[j].flags & VB_FLAG_LARGE_INDEX) {
            ...
            for (i = 0; i < n_verts; i++) { mybuf[i] = (ushort)index[i]; }
        }
    }
...
void model_interp_set_buffer_layout (vertex_layout* layout) {
    ...
    // respecification and since the data contains valid data there is no risk
    // of reading garbage data on the GPU

    layout->add_vertex_component (
        vertex_format_data::TEX_COORD2, sizeof (interp_vertex),
        offsetof (interp_vertex, uv));

    layout->add_vertex_component (
        vertex_format_data::NORMAL, sizeof (interp_vertex),
        offsetof (interp_vertex, normal));

    layout->add_vertex_component (
        vertex_format_data::TANGENT, sizeof (interp_vertex),
        offsetof (interp_vertex, tangent));

    layout->add_vertex_component (
        vertex_format_data::MODEL_ID, sizeof (interp_vertex),
        offsetof (interp_vertex, modelId));

    layout->add_vertex_component (
        vertex_format_data::POSITION3, sizeof (interp_vertex),
        offsetof (interp_vertex, pos));
}
...
bool model_interp_config_buffer (
    ...
    // we require consistent stride across vertex buffers so we can use base
    // vertex offsetting for performance reasons
    ...
    if (!update_ibuffer_only) {
        ...
        vert_src->Vertex_list_size +=
            (uint) (vb->stride * vb->model_list->n_verts);
    }
    ...
    for (size_t idx = 0; idx < vb->tex_buf.size (); idx++) {
        ...
        vert_src->Index_list_size += (uint) (
            bd->n_verts * ((bd->flags & VB_FLAG_LARGE_INDEX)
                               ? sizeof (uint)
                               : sizeof (ushort)));

        // even out index buffer so we are always word aligned
        vert_src->Index_list_size +=
            (uint) (vert_src->Index_list_size % sizeof (uint));
    }
...
void interp_configure_vertex_buffers (polymodel* pm, int mn) {
    ...
    for (i = 0; i < MAX_MODEL_TEXTURES; i++) {
        ...
        bsp_polies->generate_triangles (
            i, polygon_list[i].vert, polygon_list[i].norm);
    ...
    for (i = 0; i < MAX_MODEL_TEXTURES; i++) {
        if (!polygon_list[i].n_verts) continue;

        memcpy (
            (model_list->vert) + model_list->n_verts, polygon_list[i].vert,
            sizeof (vertex) * polygon_list[i].n_verts);
        memcpy (
            (model_list->norm) + model_list->n_verts, polygon_list[i].norm,
            sizeof (vec3d) * polygon_list[i].n_verts);

        if (Cmdline_normal) {
            memcpy (
                (model_list->tsb) + model_list->n_verts, polygon_list[i].tsb,
                sizeof (tsb_t) * polygon_list[i].n_verts);
        }

        memcpy (
            (model_list->submodels) + model_list->n_verts,
            polygon_list[i].submodels, sizeof (int) * polygon_list[i].n_verts);
    ...
    if (model_list->submodels != NULL) { vertex_flags |= VB_FLAG_MODEL_ID; }
...
void interp_copy_index_buffer (
    vertex_buffer* src, vertex_buffer* dest, size_t* index_counts) {
    size_t i, j, k;
    size_t src_buff_size;
    buffer_data* src_buffer;
    buffer_data* dest_buffer;
    size_t vert_offset =
        src->vertex_num_offset; // assuming all submodels crunched into this
                                // index buffer have the same stride
    // int vert_offset = 0;
    ...
    for (i = 0; i < dest->tex_buf.size (); ++i) {
        ...
        for (j = 0; j < src->tex_buf.size (); ++j) {
            if (dest_buffer->texture != src->tex_buf[j].texture) { continue; }
...
void interp_fill_detail_index_buffer (
    std::vector< int >& submodel_list, polymodel* pm, vertex_buffer* buffer) {
    size_t index_counts[MAX_MODEL_TEXTURES];
    int i, j;
    int model_num;

    for (i = 0; i < MAX_MODEL_TEXTURES; ++i) { index_counts[i] = 0; }
    ...
    // need to first count how many indexes there are in this entire detail
    // model hierarchy
    for (i = 0; i < (int)submodel_list.size (); ++i) {
        model_num = submodel_list[i];

        if (pm->submodel[model_num].is_thruster) { continue; }
        ...
        for (j = 0; j < num_buffers; ++j) {
            ...
            index_counts[tex_num] +=
                pm->submodel[model_num].buffer.tex_buf[j].n_verts;
        }
    }

    // allocate the respective texture buffers with indexes for our detail
    // buffer
    for (i = 0; i < MAX_MODEL_TEXTURES; ++i) {
        if (index_counts[i] == 0) { continue; }
    ...
    for (i = 0; i < (int)submodel_list.size (); ++i) {
        ...
        interp_copy_index_buffer (
            &pm->submodel[model_num].buffer, buffer, index_counts);
    }
...
void interp_create_detail_index_buffer (polymodel* pm, int detail_num) {
    ...
    if (submodel_list.empty ()) { return; }

    interp_fill_detail_index_buffer (
        submodel_list, pm, &pm->detail_buffers[detail_num]);

    // check if anything was even put into this buffer
    if (pm->detail_buffers[detail_num].tex_buf.empty ()) { return; }

    model_interp_config_buffer (
        &pm->vert_source, &pm->detail_buffers[detail_num], true);
}
...
void interp_create_transparency_index_buffer (polymodel* pm, int mn) {
    ...
    trans_buffer->vertex_num_offset =
        pm->submodel[mn].buffer.vertex_num_offset;
    ...
    if (model_list == NULL || model_list->n_verts < 1) { return; }
    ...
    for (int i = 0; i < (int)tex_buffers.size (); ++i) {
        ...
        if (!texture_lookup.valid ()) { continue; }
        ...
        if (transparent_indices.empty ()) { continue; }

        pm->flags |= PM_FLAG_TRANS_BUFFER;
        trans_buffer->flags |= VB_FLAG_TRANS;

        trans_buffer->tex_buf.push_back (
            buffer_data (transparent_indices.size ()));
...
void model_interp_process_shield_mesh (polymodel* pm) {
    ...
    for (int i = 0; i < pm->shield.ntris; i++) {
        ...
        // recalculate triangle normals to solve some issues regarding triangle
        // collision
        vec3d b_a;
        vec3d c_a;
    ...
    if (!buffer.empty ()) {
        pm->shield.buffer_id =
            gr_create_buffer (BufferType::Vertex, BufferUsageHint::Static);
        pm->shield.buffer_n_verts = n_verts;
        gr_update_buffer_data (
            pm->shield.buffer_id, buffer.size () * sizeof (vec3d), &buffer[0]);

        pm->shield.layout.add_vertex_component (
            vertex_format_data::POSITION3, sizeof (vec3d) * 2, 0);
        pm->shield.layout.add_vertex_component (
            vertex_format_data::NORMAL, sizeof (vec3d) * 2, sizeof (vec3d));
    }
    else {
    ...
    if (obj->type == OBJ_SHIP) {
        ...
        while (ssp != END_OF_LIST (&shipp->subsys_list)) {
            if (!strcmp (subname, ssp->system_info->subobj_name)) {
                ...
                if (ssp->current_hits <= 0) return 0;

                // see if the subsystem is disrupted, in which case it should
                // be inoperable
                if (ship_subsys_disrupted (ssp)) return 0;
...
void model_set_warp_globals (
    float scale_x, float scale_y, float scale_z, int bitmap_id, float alpha) {
...
void model_mix_two_team_colors (
    team_color* dest, team_color* a, team_color* b, float mix_factor) {
    ...
    dest->stripe.r =
        a->stripe.r * (1.0f - mix_factor) + b->stripe.r * mix_factor;
    dest->stripe.g =
        a->stripe.g * (1.0f - mix_factor) + b->stripe.g * mix_factor;
    dest->stripe.b =
        a->stripe.b * (1.0f - mix_factor) + b->stripe.b * mix_factor;
}
    ...
    if (!strcasecmp (secondaryteam.c_str (), "none")) {
        ...
        if (Team_Colors.find (secondaryteam) != Team_Colors.end ()) {
            ...
            if (fadetime != 0) // avoid potential div-by-zero
                time_remaining =
                    (f2fl (Missiontime - timestamp) * 1000) / fadetime;
            CLAMP (time_remaining, 0.0f, 1.0f);
            model_mix_two_team_colors (
                &temp_color, &start, &end, time_remaining);
...
//********************-----CLASS: texture_info-----********************//
texture_info::texture_info () { clear (); }
...
int texture_info::GetNumFrames () { return num_frames; }
int texture_info::GetOriginalTexture () { return original_texture; }
int texture_info::GetTexture () { return texture; }
float texture_info::GetTotalTime () { return total_time; }
...
void texture_info::PageIn () { bm_page_in_texture (texture); }
...
int texture_info::SetTexture (int n_tex) {
    if (n_tex != -1 && !bm_is_valid (n_tex)) return texture;
...
int texture_map::FindTexture (int bm_handle) {
    if (!bm_is_valid (bm_handle)) return -1;

    for (int i = 0; i < TM_NUM_TYPES; i++) {
        if (this->textures[i].GetTexture () == bm_handle) return i;
    }
    return -1;
}
int texture_map::FindTexture (char* fname) {
    if (fname == NULL || !strlen (fname)) return -1;

    char buf[NAME_LENGTH];
    for (int i = 0; i < TM_NUM_TYPES; i++) {
        bm_get_filename (this->textures[i].GetTexture (), buf);
        if (!strextcmp (buf, fname)) { return i; }
    }
    return -1;
}

void texture_map::PageIn () {
    for (int i = 0; i < TM_NUM_TYPES; i++) this->textures[i].PageIn ();
}

void texture_map::PageOut (bool release) {
    for (int i = 0; i < TM_NUM_TYPES; i++) this->textures[i].PageOut (release);
}
...
void texture_map::Clear () {
    ...
    for (int i = 0; i < TM_NUM_TYPES; i++) this->textures[i].clear ();
}

void texture_map::ResetToOriginal () {
    for (int i = 0; i < TM_NUM_TYPES; i++) this->textures[i].ResetTexture ();
}
...
void bsp_polygon_data::process_bsp (int offset, ubyte* bsp_data) {
    ...
    while (id != 0) {
        switch (id) {
        case OP_DEFPOINTS: process_defpoints (offset, bsp_data); break;

        case OP_SORTNORM: process_sortnorm (offset, bsp_data); break;

        case OP_FLATPOLY: process_flat (offset, bsp_data); break;

        case OP_TMAPPOLY: process_tmap (offset, bsp_data); break;
...
void bsp_polygon_data::process_tmap (int offset, ubyte* bsp_data) {
    ...
    // this polygon will be broken up into a triangle fan. first three verts
    // make up the first triangle additional verts are made into new tris
    Num_polies[pof_tex]++;
    ...
    for (int i = 0; i < n_vert; ++i) {
        ...
        if (IS_VEC_NULL (&vert.normal)) vert.normal = *vp (p);
...
int bsp_polygon_data::get_num_triangles (int texture) {
    if (texture < 0) { return MAX (Num_flat_verts - 2 * Num_flat_polies, 0); }

    return MAX (Num_verts[texture] - 2 * Num_polies[texture], 0);
}

int bsp_polygon_data::get_num_lines (int texture) {
    if (texture < 0) { return Num_flat_verts; }

    return Num_verts[texture];
}

void bsp_polygon_data::generate_triangles (
    int texture, vertex* vert_ptr, vec3d* norm_ptr) {
    int num_verts = 0;

    for (uint i = 0; i < Polygons.size (); ++i) {
        if (Polygons[i].texture != texture) { continue; }
        ...
        for (uint j = start_index + 1; j < end_index - 1; ++j) {
            // first vertex of this triangle. Always the first vertex of the
            // polygon
