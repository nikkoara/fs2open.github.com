...
void model_deallocate_interp_data()
{
        ...
        if (Interp_splode_points != NULL)
                free(Interp_splode_points);

        if (Interp_splode_verts != NULL)
                free(Interp_splode_verts);
...
void model_allocate_interp_data(int n_verts, int n_norms)
{
        ...
        ASSERT(
                (n_verts || Num_interp_verts_allocated) && (n_norms || Num_interp_norms_allocated));
        ...
        if (n_verts > Num_interp_verts_allocated) {
                ...
                Interp_points = (vertex *)realloc(Interp_points, n_verts * sizeof(vertex));
                Interp_splode_points = (vertex *)realloc(
                        Interp_splode_points, n_verts * sizeof(vertex));
                Interp_splode_verts = (vec3d *)realloc(Interp_splode_verts, n_verts * sizeof(vec3d));
        ...
        if (n_norms > Num_interp_norms_allocated) {
                ...
                Interp_lighting_temp.lights = (model_light *)malloc(n_norms * sizeof(model_light));
...
void model_interp_defpoints(ubyte *p, polymodel *pm, bsp_info *sm)
{
        ...
        if (Interp_thrust_scale_subobj) {
                ...
                (Interp_warp_scale_x != 1.0f) || (Interp_warp_scale_y != 1.0f) || (Interp_warp_scale_z != 1.0f)) {
                for (n = 0; n < nverts; n++) {
                        vec3d tmp;
                ...
                for (n = 0; n < nverts; n++) {
                        if (GEOMETRY_NOISE != 0.0f) {
                                ...
                                Interp_verts[n] = src;
                                point.xyz.x = src->xyz.x + fs2::prng::randf(0, GEOMETRY_NOISE, -GEOMETRY_NOISE);
                                point.xyz.y = src->xyz.y + fs2::prng::randf(0, GEOMETRY_NOISE, -GEOMETRY_NOISE);
                                point.xyz.z = src->xyz.z + fs2::prng::randf(0, GEOMETRY_NOISE, -GEOMETRY_NOISE);
        ...
        float alpha, bool invert = false)
{
        ...
        if (invert)
                *param_r = *param_g = *param_b = ubyte(int((1.0f - d) * 254.0f * alpha));
        else
        ...
        indexed_vertex_source *vert_src, vertex_buffer *vb)
{
        ...
        for (i = 0; i < n_verts; i++) {
                ...
                ASSERT(
                        ((i * sizeof(interp_vertex)) + sizeof(interp_vertex)) <= (vert_src->Vertex_list_size - vb->vertex_offset));
        ...
        bool update_ibuffer_only)
{
        ...
        for (size_t idx = 0; idx < vb->tex_buf.size(); idx++) {
                ...
                vert_src->Index_list_size += (uint)(
                        bd->n_verts * ((bd->flags & VB_FLAG_LARGE_INDEX) ? sizeof(uint) : sizeof(ushort)));
...
void interp_configure_vertex_buffers(polymodel *pm, int mn)
{
        ...
        if (outline_n_lines > 0) {
                ...
                model->outline_buffer = (vertex *)malloc(sizeof(vertex) * model->n_verts_outline);
        ...
        vertex_buffer *src, vertex_buffer *dest, size_t *index_counts)
{
        ...
        size_t vert_offset = src->vertex_num_offset; // assuming all submodels crunched into this
                ...
                for (j = 0; j < src->tex_buf.size(); ++j) {
                        ...
                        for (k = 0; k < src_buff_size; ++k) {
                                dest_buffer->assign(
                                        dest_buffer->n_verts,
                                        (uint32_t)(
                                                src_buffer->get_index()[k] + vert_offset)); // take into account the vertex offset.
                                dest_buffer->n_verts++;

                                ASSERT(
                                        dest_buffer->n_verts <= index_counts[dest_buffer->texture]);
                        }
                }
        }
}
...
int model_interp_get_texture(texture_info *tinfo, fix base_frametime)
{
        ...
        if (texture >= 0 && num_frames > 1) {
                ...
                cur_time = f2fl(
                        (game_get_overall_frametime() - base_frametime) % fl2f(total_time));
...
void bsp_polygon_data::process_sortnorm(int offset, ubyte *bsp_data)
{
        ...
        if (prelist)
                process_bsp(offset + prelist, bsp_data);
        if (backlist)
                process_bsp(offset + backlist, bsp_data);
        if (onlist)
                process_bsp(offset + onlist, bsp_data);
        if (frontlist)
                process_bsp(offset + frontlist, bsp_data);
        if (postlist)
                process_bsp(offset + postlist, bsp_data);
}
