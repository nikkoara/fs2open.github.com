...
inline int in_box(vec3d *min, vec3d *max, vec3d *pos, vec3d *view_position)
{
        ...
        if ((point.xyz.x >= min->xyz.x) && (point.xyz.x <= max->xyz.x) && (point.xyz.y >= min->xyz.y) && (point.xyz.y <= max->xyz.y) && (point.xyz.z >= min->xyz.z) && (point.xyz.z <= max->xyz.z)) {
                return 1;
        }
...
class model_draw_list {
        ...
        bool Render_initialized = false; //!< A flag for checking if init_render has been called before a
                                         //!< render_all call
