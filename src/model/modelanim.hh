...
#ifndef FREESPACE2_MODEL_MODELANIM_HH
#define FREESPACE2_MODEL_MODELANIM_HH
...
#define TRIGGER_TYPE_INITIAL \
        0                              // This is just the position the subobject should be placed in
...
#define TRIGGER_TYPE_SCRIPTED \
        10 // Triggered exclusively by scripting...maybe SEXPs? -C
#define TRIGGER_TYPE_TURRET_FIRED \
        11 // Triggered after a turret has fired -The E
...
/**
 * This is an object responsable for storing the animation information
 * assosiated with a specific triggered animation, one subobject can have many
...
/**
 * This is the triggered animation object, it is responsable for controlling
 * how the current triggered animation works rot_accel is the acceleration for
 * starting to move and stopping, so figure it in twice.
 */
...
public:
        ...
        void apply_trigger_angles(angles_t *submodel_angles);
        ...
        vec3d rot_vel;    // radians per second, hold this speed when rot_accel has
                          // pushed it to this
...
// starts an animation of a certan type that may be assosiated with a submodel
// of a ship (returns true if an animation was started)
bool model_anim_start_type(
        ship_subsys *pss, int animation_type, int subtype, int direction,
        bool instant = false); // for a specific subsystem
bool model_anim_start_type(
        ship *shipp, int animation_type, int subtype, int direction,
        bool instant = false); // for all valid subsystems

// how long until the animation is done
int model_anim_get_time_type(
        ship_subsys *pss, int animation_type,
        int subtype); // for a specific subsystem
int model_anim_get_time_type(
...
#endif // FREESPACE2_MODEL_MODELANIM_HH
