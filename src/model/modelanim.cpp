...
const char* Animation_type_names[MAX_TRIGGER_ANIMATION_TYPES] = {
    "initial",     "docking-stage-1", "docking-stage-2", "docking-stage-3",
    "docked",      "primary-bank",    "secondary-bank",  "fighterbay",
    "afterburner", "turret-firing",   "scripted",        "turret-fired"
};
...
void triggered_rotation::start (queued_animation* q) {
    ...
    nprintf (
        ("ModelAnim", "Starting animation type %i at %i ...\n", q->type,
         timestamp ()));
    ...
    for (int axis = 0; axis < 3; axis++) {
        direction.a1d[axis] =
            (end_angle.a1d[axis] + q->angle.a1d[axis]) - current_ang.a1d[axis];
        ...
        if (q->accel.a1d[axis] == 0.0f)
            ...
            slow_angle.a1d[axis] =
                end_angle.a1d[axis] + (((q->vel.a1d[axis] * q->vel.a1d[axis]) /
                                        (2.0f * q->accel.a1d[axis])) *
                                       direction.a1d[axis]);
    }
...
triggered_rotation::triggered_rotation () { clear (); }
...
void triggered_rotation::add_queue (queued_animation* the_queue, int dir) {
    ...
    memcpy (
        queue_tmp, queue,
        sizeof (queued_animation) * MAX_TRIGGERED_ANIMATIONS);

    if (n_queue > 0) {
        // remove any items on the queue that are the opposite of what we are
        // thinking about doing
        // if we are reversing an animation see if the forward animation is on
        // the queue already, and remove it
        for (i = 0; i < n_queue && (i < MAX_TRIGGERED_ANIMATIONS); i++) {
            if ((new_queue.type == queue_tmp[i].type) &&
                (new_queue.subtype == queue_tmp[i].subtype)) {
                // same type, if they have the same values (direction reversed)
                // then replace it
                if (new_queue.instance == queue_tmp[i].instance) break;
            }
        }
        ...
        if (i != n_queue) {
            ...
            if (i < (MAX_TRIGGERED_ANIMATIONS - 1))
                memmove (
                    &queue_tmp[i], &queue_tmp[i + 1],
                    sizeof (queued_animation) *
                        (MAX_TRIGGERED_ANIMATIONS - (i + 1)));

            // ok these two animations cancelled each other out, so he doesn't
            // get on the queue
            n_queue--;
    ...
    if (new_queue.start == 0) {
        ...
        // if there is no delay don't bother with the queue, just start the
        // thing
        start (&new_queue);
    ...
    if (new_queue.instance == instance) {
        // same animation is playing that we are about to think about playing
        ...
        if ((direction.xyz.x * rot_vel.xyz.x) == new_queue.vel.xyz.x &&
            ...
            // so this means that there is some sort of delay that's getting
            // fubared becase of other queue items getting removed due to
            // reversal this animation needs to be started now!
            new_queue.start_time = timestamp ();
            new_queue.end_time = timestamp (new_queue.end);
    ...
    if (n_queue > 0) {
        ...
        for (i = 0; (i < n_queue) && (i < MAX_TRIGGERED_ANIMATIONS) &&
                    (new_queue.start_time > queue_tmp[i].start_time);
             i++)
            ;
        ...
        // from the begining of the queue to the item on the queue that is just
        // before the new item
        if (i) memcpy (queue, queue_tmp, sizeof (queued_animation) * i);
        ...
        if ((n_queue >= (i + 1)) && (i < (MAX_TRIGGERED_ANIMATIONS - 1))) {
            if (n_queue >= MAX_TRIGGERED_ANIMATIONS) {
                // if the queue is full, we don't want to copy past the end of
                // it
                memcpy (
                    &queue[i + 1], &queue_tmp[i],
                    sizeof (queued_animation) *
                        (MAX_TRIGGERED_ANIMATIONS - i - 1));
            }
            else {
                memcpy (
                    &queue[i + 1], &queue_tmp[i],
                    sizeof (queued_animation) * (n_queue - i));
            }
        }
...
void triggered_rotation::process_queue () {
    ...
    // all items on the queue are in chronological order (or at least they
    // should be) so execute all items who's starting timestamps are less than
    // the current time
    ...
    // if there are more items on the queue than we just executed reallocate
    // the queue. copy all the items after the last one we executed
...
void queued_animation_correct (queued_animation* qa) {
    for (int i = 0; i < 3; i++) {
        if (qa->accel.a1d[i] == 0.0f) continue;
...
/*
ok a triggered animation works like this, at some point a subobject will be
triggered to rotate when this happens the following phases of rotation will
happen 1) it will accelerate at a constant rate until it reaches a
quasi-arbitrary (there are limitations on what it can be) velocity
2) it will maintain a constant rotational velocity untill it reaches the angle
at which it needs to start slowing down in order to stop at the right end angle
3)it will slow down at the same rate it has sped up earlier, when the
rotational velocity starts going in the wrong direction it'll be locked at 0
and the angle of the submodel will be locked at the angle it's suposed to end
at
*/
//-Bobboau
...
void model_anim_submodel_trigger_rotate (
    ...
    if (ss->triggered_rotation_index < 0) {
        mprintf (
            ("Invalid rotation index for triggered rotation in subsystem %s "
             "in model %s!",
             psub->name,
    ...
    triggered_rotation* trigger =
        &Triggered_rotations[ss->triggered_rotation_index];
    ...
    if (!trigger->has_started) return;

    // save last angles
    sii->prev_angs = sii->angs;

    // process velocity and position
    // first you accelerate, then you maintain a speed, then you slow down,
    // then you stay put
    for (int i = 0; i < 3; i++) {
        // are we moving?
        if ((trigger->current_vel.a1d[i] != 0.0f) ||
            ((trigger->current_ang.a1d[i] * trigger->direction.a1d[i]) <=
             (trigger->slow_angle.a1d[i] * trigger->direction.a1d[i]))) {
            // yep...

            // our velocity is something other than 0 or we are in the
            // acceleration phase (where velocity starts out at 0)

            // while you are not slowing down...
            if ((trigger->current_ang.a1d[i] * trigger->direction.a1d[i]) <=
                (trigger->slow_angle.a1d[i] * trigger->direction.a1d[i])) {
                // speeding up
                if ((trigger->current_vel.a1d[i] * trigger->direction.a1d[i]) <
                        (trigger->rot_vel.a1d[i] *
                         trigger->direction.a1d[i]) &&
                    (trigger->rot_accel.a1d[i] != 0.0f)) {
                    trigger->current_vel.a1d[i] +=
                        (trigger->rot_accel.a1d[i] * flFrametime);
                }
            ...
            // we are slowing down...
                ...
                if (((trigger->current_vel.a1d[i] *
                      trigger->direction.a1d[i]) > 0) &&
                    (trigger->rot_accel.a1d[i] != 0.0f)) {
                    trigger->current_vel.a1d[i] -=
                        (trigger->rot_accel.a1d[i] * flFrametime);
                }
                // velocity does not match desired heading
                    ...
                    // this can happen if we have decelerated too long or if an
                    // animation was reversed quickly the way to tell the
                    // difference between these two cases is the acceleration

                    // if the curent velocity is in the opposite direction as
                    // the acceleration then it was interupted
                    if ((trigger->current_vel.a1d[i] /
                         fabs (trigger->current_vel.a1d[i])) !=
                        (trigger->rot_accel.a1d[i] /
                         fabs (trigger->rot_accel.a1d[i]))) {
                        // this is gona be some messy stuff in here to figure
                        // out when it should start to slow down again it'll
                        // have to make a new slow angle I guess with an
                        // initial v in the opposite direction the time it will
                        // take for it to stop will be v/a, to get back up to
                        // the same speed again we will need twice that it
                        // should be back to where it was in terms of both
                        // speed and position then so...
                        trigger->slow_angle.a1d[i] =
                            trigger->current_ang.a1d[i];
                        trigger->rot_vel.a1d[i] =
                            -(trigger->current_vel.a1d[i]);

                        // I guess that wasn't so messy after all :D

                        // it might hit exactly 0 every now and then, but it
                        // will be before the slow angle so it will be fine
                        // this assumes that the reversed animation is the same
                        // exact animation only played in reverse, if the
                        // speeds or accelerations are diferent then it might
                        // not work
                    }
                    else {
                        // our velocity has gone in the opposite direction
                        // because we decelerated too long
                        trigger->current_vel.a1d[i] = 0.0f;
                    }
                }
            }

            // if we've over shot the angle, this shouldn't happen but it might
            // if odd values are given
            if ((trigger->current_ang.a1d[i] * trigger->direction.a1d[i]) >=
                (trigger->end_angle.a1d[i] * trigger->direction.a1d[i])) {
                ...
                trigger->current_ang.a1d[i] +=
                    (trigger->current_vel.a1d[i] * flFrametime);
            }
        }
        else {
            // not moving
    ...
    // I'm prety sure using the magnitude of the vectors is at least pretty
    // close for any code that might be using it
    ...
    // the extra math here is/was useless, it just returns the exact same value
    // (or really just 0 in the old code)
    sii->angs.p =
        trigger->current_ang.xyz
            .x; //- (2.0f * PI2 * (trigger->current_ang.xyz.x / (2.0f * PI2)));
    sii->angs.h =
        trigger->current_ang.xyz
            .y; //- (2.0f * PI2 * (trigger->current_ang.xyz.y / (2.0f * PI2)));
    sii->angs.b =
        trigger->current_ang.xyz
            .z; //- (2.0f * PI2 * (trigger->current_ang.xyz.z / (2.0f * PI2)));
...
bool subtype_check (
    ...
    if (anim_q->type == TRIGGER_TYPE_DOCK_BAY_DOOR) {
        int anim_subtype =
            anim_q->subtype - 1; // in the tables, bay door +sub_types are
                                 // 1-based so change to 0-based

        if (anim_subtype < 0) {
            if (abs (anim_subtype) != subtype) { return true; }
        }
        else {
            if (anim_subtype == subtype) { return true; }
        }
    }
    else {
        if (anim_q->subtype == subtype) return true;
    }
...
bool model_anim_start_type (
    ...
    if (pss->max_hits > 0 &&
        pss->current_hits <=
            0.0f) // allow subobjects with hitpoints disabled -nuke
        return false;
    ...
    for (int i = 0; i < psub->n_triggers; i++) {
        if ((psub->triggers[i].type == animation_type) &&
            subtype_check (psub, &psub->triggers[i], subtype)) {
...
bool model_anim_start_type (
    ship* shipp, int animation_type, int subtype, int direction,
    bool instant) {
    // this makes the logic for docking triggers a bit cleaner
    if (shipp == NULL) return false;
    ...
    for (pss = GET_FIRST (&shipp->subsys_list);
        ...
        bool rc = model_anim_start_type (
            pss, animation_type, subtype, direction, instant);
...
/**
 * @brief Finds the actual amount of time that motion of an animation type will
 * take to stop, not for gameplay purposes but for stuff that is involved in
 * coordinating the animation itself.
 *
 * @details The time it takes to speed up or slow down is v/a in this time the
 * animation covers an angle = to (v^2)/(2a) (for both directions so v^2/a) so
 * wee need the time it takes for the angle moving at a constant velosity to
 * cover theda - v^2/a v*t = theda - (v^2)/(2*a) => t = -(v^2 -
 * 2*a*theda)/(2*a*v) so finally v/a * 2 - (v^2 - 2*a*theda)/(2*a*v) => (3*v^2
 * + 2*a*theda)/(2*a*v) time = (3*v^2 + 2*a*theda)/(2*a*v)
 */
int model_anim_instance_get_actual_time (queued_animation* properties) {
    ...
    for (int a = 0; a < 3; a++) {
        temp = fl2i (
                   ...
                   (2.0f * properties->accel.a1d[a] * properties->vel.a1d[a]) *
                   1000.0f) +
               properties->start;
...
int model_anim_get_actual_time_type (
    ship* shipp, int animation_type, int subtype) {
    ship_subsys* pss;
    ...
    for (pss = GET_FIRST (&shipp->subsys_list);
        ...
        for (i = 0; i < psub->n_triggers; i++) {
            if ((psub->triggers[i].type == animation_type) &&
                subtype_check (psub, &psub->triggers[i], subtype)) {
                temp_ret =
                    model_anim_instance_get_actual_time (&psub->triggers[i]);
...
int model_anim_get_actual_time_type (
    ship_info* sip, int animation_type, int subtype) {
    model_subsystem* psub;
    int ret = 0;
    int temp_ret = 0;
    int n, i;
...
void model_anim_fix_reverse_times (ship_info* sip) {
    ...
    for (type = 0; type < MAX_TRIGGER_ANIMATION_TYPES; type++) {
        ...
        ani_time =
            model_anim_get_actual_time_type (sip, type, ANIMATION_SUBTYPE_ALL);
            ...
            for (j = 0; j < psub->n_triggers; j++) {
                if (psub->triggers[j].type == type) {
                    ...
                    if (psub->triggers[j].reverse_start == -1)
                        psub->triggers[j].reverse_start =
                            ani_time - model_anim_instance_get_actual_time (
                                           &psub->triggers[j]);

                    if (psub->triggers[j].reverse_start < 0) {
                        mprintf (
                            ("WARNING:  Animation trigger #%i on subsystem "
                             "'%s', for ship '%s', has a negative "
                             "reverse_start value!  Capping it at 0!\n",
                             j, psub->subobj_name, sip->name));
                        psub->triggers[j].reverse_start = 0;
                    }

                    psub->triggers[j].real_end_time =
                        model_anim_instance_get_actual_time (
                            &psub->triggers[j]);
                }
            }
        }
...
int model_anim_get_time_type (
    ...
    for (i = 0; i < psub->n_triggers; i++) {
        if ((psub->triggers[i].type == animation_type) &&
            ((psub->triggers[i].subtype == ANIMATION_SUBTYPE_ALL) ||
             (psub->triggers[i].subtype == subtype))) {
            int ani_time = 0;

            if ((tr->current_vel.a1d[0] != 0.0f) ||
                (tr->current_vel.a1d[1] != 0.0f) ||
                (tr->current_vel.a1d[2] != 0.0f)) {
                // if the subobject is moving then things get really
                // complicated
                int a_time = 0;
                int real_time =
                    model_anim_instance_get_actual_time (&psub->triggers[i]);
                ...
                for (int a = 0; a < 3; a++) {
                    float end_angle =
                        (tr->current_ang.a1d[a] +
                         (((tr->rot_vel.a1d[a] * tr->rot_vel.a1d[a]) -
                           (tr->current_vel.a1d[a] * tr->current_vel.a1d[a])) /
                          (2 * tr->rot_accel.a1d[a])));
                    ...
                    if (end_angle > tr->slow_angle.a1d[a]) {
                        ...
                        a_time = fl2i (
                            ((((2 * tr->rot_vel.a1d[a]) -
                               tr->current_ang.a1d[a]) /
                              tr->rot_accel.a1d[a]) +
                             tr->slow_angle.a1d[a] - tr->current_ang.a1d[a] +
                             (((tr->current_vel.a1d[a] *
                                tr->current_vel.a1d[a]) -
                               (tr->rot_vel.a1d[a] * tr->rot_vel.a1d[a])) /
                              (2 * tr->rot_accel.a1d[a]))) *
                            1000.0f);
                            ...
                            (2 * fl_sqrt (
                                     ...
                                     ((tr->current_vel.a1d[a] *
                                      ...
                                      (tr->rot_accel.a1d[a] *
                                       tr->rot_accel.a1d[a])) /
                                         2) -
                             (tr->current_vel.a1d[a] / tr->rot_accel.a1d[a])) *
                            1000.0f);
                        if (ani_time < a_time) ani_time = a_time;
                    }
                }
 ...
 * @details For things that can't happen until animations are done this is for
 * gameplay purposes, this isn't the actual time
 * @param shipp
        ...
        for (i = 0; i < psub->n_triggers; i++) {
            if (psub->triggers[i].type == TRIGGER_TYPE_INITIAL) {
                if (psub->type == SUBSYSTEM_TURRET) {
                    ...
                    pss->submodel_info_2.angs.p =
                        psub->triggers[i].angle.xyz.x;
                    pss->submodel_info_1.angs.h =
                        psub->triggers[i].angle.xyz.y;
                }
                else {
                    if (pss->triggered_rotation_index < 0) {
                        mprintf (
                            ("Invalid rotation index for triggered rotation "
                             "in subsystem %s in model %s!",
                             psub->name,
                             model_get (
...
bool model_anim_push_and_start_type (
    int stack_unique_id, ship* shipp, int animation_type, int subtype,
    int direction, bool instant) {
    stack_item item;
    item.shipp = shipp;
    ...
    return model_anim_start_type (
        shipp, animation_type, subtype, direction, instant);
}
...
bool model_anim_pop_and_start_type (int stack_unique_id) {
    ...
    return model_anim_start_type (
        item.shipp, item.animation_type, item.subtype, item.direction * -1,
        item.instant);
}
