        ...
        int model_num, model_subsystem *subsystemp, char *props, char *dname)
{
        ...
        // Rotating subsystem
                ...
                (idx = prop_string(
                         props, &p, "$rotate_time", "$rotate_rate", "$rotate"))
                >= 0) {
                subsystemp->flags.set(Model::Subsystem_Flags::Rotates);
                ...
                if ((strstr(props, "$stepped")) != NULL) {
                        ...
                        if ((p = strstr(props, "$fraction_accel")) != NULL) {
                                ...
                                ASSERT(
                                        subsystemp->stepped_rotation->fraction > 0 && subsystemp->stepped_rotation->fraction < 0.5);
                        } else {
        ...
        vec3d *pnt, char *props, char *subobj_name, int model_num)
{
        ...
        for (i = 0; i < n_subsystems; i++) {
                ...
                if (strcasecmp(subobj_name, subsystemp->subobj_name) != 0 && !subsystem_strcasecmp(subobj_name, subsystemp->subobj_name)) {
...
// mprintf(( "Num models = %d\n", pm->n_models ));
                ...
                case ID_SOBJ: { // Subobject header
                        ...
                        if ((p = strstr(props, "$look_at")) != NULL) {
                                ...
                                pm->submodel[n].look_at_num = -2; // Set this to -2 to mark it as something we need to
                        ...
                        if ((pm->submodel[n].movement_type != MOVEMENT_TYPE_NONE) || strstr(props, "$triggered") || strstr(props, "$rotate") || strstr(props, "$gun_rotation")) {
                                pm->submodel[n].can_move = true;
                        } else if (
                                pm->submodel[n].parent >= 0 && pm->submodel[pm->submodel[n].parent].can_move) {
                                pm->submodel[n].can_move = true;
                        }
                ...
                case ID_SHLD: {
                        ...
                        if (pm->shield.ntris > 0) {
                                ...
                                for (i = 0; i < pm->shield.ntris; i++) {
                                        ...
                                        for (j = 0; j < 3; j++) {
                                                pm->shield.tris[i].verts[j] = cfread_int(fp); // read in the indices into the
                                                                                              // shield_vertex list
#ifndef NDEBUG
                                        ...
                                        for (j = 0; j < 3; j++) {
                                                pm->shield.tris[i].neighbors[j] = cfread_int(fp); // read in the neighbor indices --
                ...
                case ID_GLOW: // start glow point reading -Bobboau
                {
                        ...
                        for (int gpb = 0; gpb < gpb_num; gpb++) {
                                ...
                                if (length > 0) {
                                        ...
                                        ASSERT(
                                                strstr((const char *)&props, "$glow_texture=") != NULL);
                ...
                case ID_SPCL: {
                        ...
                        for (i = 0; i < n_specials; i++) {
                                ...
                                if (p != NULL) {
                                        ...
                                        strstr(name, "$enginelarge") || strstr(name, "$enginehuge")) {
                                        do_new_subsystem(
...
void model_load_texture(polymodel *pm, int i, char *file)
{
        ...
        gr_maybe_create_shader(
                SDR_TYPE_MODEL, shader_flags | SDR_FLAG_MODEL_LIGHT | SDR_FLAG_MODEL_ANIMATED | SDR_FLAG_MODEL_FOG);
        ...
        int ferror, int duplicate)
{
        ...
        for (i = 0; i < MAX_POLYGON_MODELS; i++) {
                if (Polygon_models[i]) {
                        if (!strcasecmp(filename, Polygon_models[i]->filename) && !duplicate) {
        ...
        for (i = 0; i < pm->n_models; i++) {
                ...
                for (j = 0; j < pm->n_models; j++) {
                        ...
                        if (strstr(pm->submodel[j].name, live_debris_name)) {
                                ...
                                pm->submodel[i]
                                        .live_debris[pm->submodel[i].num_live_debris++]
                                        = j;
        ...
        float rx, ry, rz;
        rx = fabsf(
                pm->submodel[pm->detail[0]].max.xyz.x - pm->submodel[pm->detail[0]].min.xyz.x);
        ry = fabsf(
                pm->submodel[pm->detail[0]].max.xyz.y - pm->submodel[pm->detail[0]].min.xyz.y);
        rz = fabsf(
                pm->submodel[pm->detail[0]].max.xyz.z - pm->submodel[pm->detail[0]].min.xyz.z);
...
int model_create_instance(bool is_ship, int model_num)
{
        ...
        polymodel_instance *pmi = (polymodel_instance *)malloc(sizeof(polymodel_instance));
        ...
        polymodel *pm, int n_subsystems, model_subsystem *subsystems)
{
        ...
        for (i = 0; i < n_subsystems; i++) {
                for (j = 0; j < pm->n_paths; j++) {
                        if (((subsystems[i].subobj_num != -1) && (subsystems[i].subobj_num == pm->paths[j].parent_submodel)) || (!subsystem_strcasecmp(subsystems[i].subobj_name, pm->paths[j].parent_name))) {
                                if (pm->n_paths > j) {
        ...
        int *y2)
{
        ...
        if (x1)
                *x1 = int(pnt.screen.xyw.x - w);
        if (y1)
                *y1 = int(pnt.screen.xyw.y - h);

        if (x2)
                *x2 = int(pnt.screen.xyw.x + w);
        if (y2)
                *y2 = int(pnt.screen.xyw.y + h);
        ...
        int submodel_num, matrix *objorient)
{
        ...
        ASSERT(
                sm->movement_type == MOVEMENT_TYPE_ROT || sm->movement_type == MOVEMENT_TYPE_INTRINSIC_ROTATE);
...
{
        ASSERT(
                (pm->submodel[submodel_num].parent == pm->detail[0]) || (pm->submodel[submodel_num].parent == -1));
...
{
        ASSERT(
                objp->type == OBJ_SHIP || objp->type == OBJ_WEAPON || objp->type == OBJ_ASTEROID);
        ...
        submodel_instance_info *sii, int model_num, int submodel_num)
{
        ...
        polymodel *pm = model_get(model_num);
        ASSERT(
                pm->submodel[submodel_num].movement_type == MOVEMENT_TYPE_ROT || pm->submodel[submodel_num].movement_type == MOVEMENT_TYPE_INTRINSIC_ROTATE);
        ASSERT(sii);
...
int model_find_dock_name_index(int modelnum, char *name)
{
    ...
    */
        ...
        for (i = 0; i < pm->n_docks; i++) {
                if (!strcasecmp(pm->docking_bays[i].name, name))
                        return i;
        }
                                ...
                                if (optional_string("+lightcone")) {
                                        ...
                                        if (optional_string("$Cone angle:")) {
                                                ...
                                                gpo.cone_inner_angle = cosf(
                                                        (gpo.cone_angle - ((gpo.cone_angle < 20.0f) ? gpo.cone_angle * 0.5f : 20.0f)) / 180.0f * PI);
