...
enum class Subsystem_Flags : size_t {
        ...
        Turret_hull_check, // makes the turret check to see if it's going to shoot
                           // through it's own hull before fireing - Bobboau
        Turret_fixed_fp,   // forces turret (when defined with multiple weapons) to
                           // prevent the firepoints from alternating
        Turret_salvo,      // forces turret to fire salvos (all guns simultaneously) -
                           // independent targeting
        Fire_on_target,    // prevents turret from firing unless it is pointing at the
                           // firingpoints are pointing at the target
        No_ss_targeting,   // toggles the subsystem targeting for the turret
        Turret_reset_idle, // makes turret reset to their initial position if the
                           // target is out of field of view
        Turret_alt_math,   // tells the game to use additional calculations should
                           // turret have a defined y fov
        Dum_rotates,       // Bobboau
        Carry_shockwave,   // subsystem - even with 'carry no damage' flag - will
                           // carry shockwave damage to the hull
        Allow_landing,     // This subsystem can be landed on
        Fov_edge_check,    // Tells the game to use better FOV edge checking with this
                           // turret
        Fov_required,      // Tells game not to allow this turret to attempt targeting
                           // objects out of FOV
        ...
        Ignore_if_dead,    // tells homing missiles to ignore the subsys if its dead
                           // and home on to hull instead of earlier subsys pos
        Allow_vanishing,   // allows subsystem to vanish (prevents explosions &
                           // sounds effects from being played)
        Damage_as_hull,    // applies armor damage to subsystem instead of subsystem
                           // damage - FUBAR
        ...
        Player_turret_sound,
        Turret_only_target_if_can_fire, // Turrets only target things they're
                                        // allowed to shoot at (e.g. if check-hull
                                        // fails, won't keep targeting)
        No_disappear,                   // Submodel won't disappear when subsystem destroyed
        Collide_submodel,               // subsystem takes damage only from hits which impact the
                                        // associated submodel
        Destroyed_rotation,             // allows subobjects to continue to rotate even if they
                                        // have been destroyed
        Turret_use_ammo,                // enables ammo consumption for turrets (DahBlount)
        Autorepair_if_disabled,         // Allows the subsystem to repair itself even if
                                        // disabled (MageKing17)
        No_autorepair_if_disabled,      // Inversion of the previous; disallows this
                                        // particular subsystem if the ship-wide flag is
                                        // set (MageKing17)
...
enum class Render_Flags : size_t {
        ...
        Show_thrusters,       // Show the engine thrusters. See model_set_thrust for how
                              // long it draws.
        Lock_detail,          // Only draw the detail level defined in
        ...
        Is_missile,           // When set, treat this as a missilie.  No lighting, small
                              // thrusters.
        Show_outline_preset,  // Draw the object in outline mode. Color assumed to
                              // be set already.
        Show_invisible_faces, // Show invisible faces as green...
        Autocenter,           // Always use the center of the hull bounding box as the
                              // center, instead of the pivot point
        ...
        Edge_alpha,           // makes norms that are faceing away from you render more
                              // transparent -Bobboau
        ...
        Full_detail,          // render all valid objects, particularly ones that are
                              // otherwise in/out of render boxes - taylor
...
#endif // FREESPACE2_MODEL_MODEL_FLAGS_HH
