...
void model_octant_find_shields (polymodel* pm, model_octant* oct) {
    ...
    for (i = 0; i < pm->shield.ntris; i++) {
        ...
        for (j = 0; j < 3; j++) {
            if (point_in_octant (
                    pm, oct, &pm->shield.verts[tri->verts[j]].pos)) {
                n++;
                break;
            }
        }
...
void moff_tmappoly (
    ubyte* p, polymodel* pm, model_octant* oct, int just_count) {
    int i, nv;
...
int model_octant_find_faces_sub (
    ...
    while (chunk_type != OP_EOF) {
        switch (chunk_type) {
        case OP_DEFPOINTS: moff_defpoints (p, just_count); break;
        ...
        case OP_SORTNORM: {
            ...
            if (backlist)
                model_octant_find_faces_sub (
                    pm, oct, p + backlist, just_count);
            if (onlist)
                model_octant_find_faces_sub (pm, oct, p + onlist, just_count);
            if (frontlist)
                model_octant_find_faces_sub (
                    pm, oct, p + frontlist, just_count);
            if (postlist)
                model_octant_find_faces_sub (
                    pm, oct, p + postlist, just_count);
        } break;
...
// data.
int model_which_octant_distant_many (
