...
int must_clip_line (vertex* p0, vertex* p1, ubyte codes_or, uint flags) {
    ...
    if (!(p0->flags & PF_PROJECTED)) g3_project_vertex (p0);

    if (p0->flags & PF_OVERFLOW) goto free_points;

    if (!(p1->flags & PF_PROJECTED)) g3_project_vertex (p1);
...
free_points:
    ...
    if (p0->flags & PF_TEMP_POINT) free_temp_point (p0);

    if (p1->flags & PF_TEMP_POINT) free_temp_point (p1);
...
int g3_draw_line (vertex* p0, vertex* p1) {
    ...
    if (codes_or & CC_BEHIND) return must_clip_line (p0, p1, codes_or, 0);

    if (!(p0->flags & PF_PROJECTED)) g3_project_vertex (p0);

    if (p0->flags & PF_OVERFLOW) return must_clip_line (p0, p1, codes_or, 0);

    if (!(p1->flags & PF_PROJECTED)) g3_project_vertex (p1);

    if (p1->flags & PF_OVERFLOW) return must_clip_line (p0, p1, codes_or, 0);
...
int g3_draw_sphere_ez (const vec3d* pnt, float rad) {
    ...
    if (flags == 0) {
        ...
        if (!(pt.flags & PF_OVERFLOW)) { g3_draw_sphere (&pt, rad); }
    }
...
int g3_get_bitmap_dims (
    int bitmap, vertex* pnt, float rad, int* x, int* y, int* w, int* h,
    int* size) {
    float t;
    float width, height;
    ...
    if (pnt->flags & PF_OVERFLOW) { return 1; }
...
void g3_draw_horizon_line () {
    ...
    horizon_vec.xyz.x = Unscaled_matrix.vec.rvec.xyz.y * Matrix_scale.xyz.y *
                        Matrix_scale.xyz.z;
    horizon_vec.xyz.y = Unscaled_matrix.vec.uvec.xyz.y * Matrix_scale.xyz.x *
                        Matrix_scale.xyz.z;
    horizon_vec.xyz.z = Unscaled_matrix.vec.fvec.xyz.y * Matrix_scale.xyz.x *
                        Matrix_scale.xyz.y;
...
vec3d g3_square[4] = { { { { -1.0f, -1.0f, 20.0f } } },
                       { { { -1.0f, 1.0f, 20.0f } } },
                       { { { 1.0f, 1.0f, 20.0f } } },
                       { { { 1.0f, -1.0f, 20.0f } } } };

#define MAX_PERSPECTIVE_DIVISIONS \
    5 // should never even come close to this limit

void stars_project_2d_onto_sphere (
    vec3d* pnt, float rho, float phi, float theta) {
    float a = PI * phi;
    float b = PI2 * theta;
    float sin_a = sinf (a);
...
void g3_render_primitives (
    material* mat, vertex* verts, int n_verts, primitive_type prim_type,
    bool orthographic) {
    vertex_layout layout;

    if (orthographic) {
        layout.add_vertex_component (
            vertex_format_data::POSITION2, sizeof (vertex),
            (int)offsetof (vertex, screen));
    }
    else {
        layout.add_vertex_component (
            vertex_format_data::POSITION3, sizeof (vertex),
            (int)offsetof (vertex, world));
    }

    if (orthographic) {
        gr_render_primitives_2d_immediate (
            mat, prim_type, &layout, n_verts, verts,
            n_verts * sizeof (vertex));
    }
    else {
...
void g3_render_primitives_textured (
    material* mat, vertex* verts, int n_verts, primitive_type prim_type,
    bool orthographic) {
    vertex_layout layout;
    ...
    layout.add_vertex_component (
        vertex_format_data::TEX_COORD2, sizeof (vertex),
        (int)offsetof (vertex, texture_position));
...
void g3_render_primitives_colored (
    material* mat, vertex* verts, int n_verts, primitive_type prim_type,
    bool orthographic) {
    vertex_layout layout;
    ...
    layout.add_vertex_component (
        vertex_format_data::COLOR4, sizeof (vertex),
        (int)offsetof (vertex, r));
...
void g3_render_primitives_colored_textured (
    material* mat, vertex* verts, int n_verts, primitive_type prim_type,
    bool orthographic) {
    vertex_layout layout;
...
void g3_render_rect_oriented_internal (
    material* material_params, vec3d* pos, matrix* ori, float width,
    float height) {
    // idiot-proof
    ...
    g3_render_primitives_textured (
        material_params, v, 4, PRIM_TYPE_TRIFAN, false);
}

void g3_render_rect_oriented (
    material* mat_info, vec3d* pos, matrix* ori, float width, float height) {
    g3_render_rect_oriented_internal (mat_info, pos, ori, width, height);
}

void g3_render_rect_oriented (
    material* mat_info, vec3d* pos, vec3d* norm, float width, float height) {
    matrix m;
...
// void render_rotated_bitmap(int texture, float alpha, vertex *pnt, float
// angle, float rad)
void g3_render_rect_screen_aligned_rotated (
    material* mat_params, vertex* pnt, float angle, float rad) {
    rad *= 1.41421356f; // 1/0.707, becase these are the points of a square or
                        // width and hieght rad
    ...
    vm_rot_point_around_line (
        &uvec, &View_matrix.vec.uvec, angle, &vmd_zero_vector,
        &View_matrix.vec.fvec);
...
void g3_render_rect_scaler (material* mat_params, vertex* va, vertex* vb) {
    ...
    u1 = vb->texture_position.u;
    v1 = vb->texture_position.v;
...
// void render_oriented_bitmap_2d(int texture, float alpha, bool blending,
// vertex *pnt, int orient, float rad)
void g3_render_rect_screen_aligned_2d (
    material* mat_params, vertex* pnt, int orient, float rad) {
    vertex va, vb;
    float t, w, h;
    float width, height;
    ...
    if (pnt->flags & PF_OVERFLOW) return;
...
void g3_render_rect_screen_aligned (
    material* mat_params, vertex* pnt, int orient, float rad, float depth) {
    rad *= 1.41421356f; // 1/0.707, becase these are the points of a square or
                        // width and hieght rad
...
void g3_render_laser (
    material* mat_params, vec3d* headp, float head_width, vec3d* tailp,
    float tail_width) {
    head_width *= 0.5f;
    tail_width *= 0.5f;
    ...
    for (i = 0; i < 4; i++) { g3_transfer_vertex (&pts[i], &vecs[i]); }
    ...
    g3_render_primitives_textured (
        mat_params, pts, 4, PRIM_TYPE_TRIFAN, false);
}

// adapted from g3_draw_laser()
// void render_laser_2d(int texture, color* clr, float alpha, vec3d *headp,
// float head_width, vec3d *tailp, float tail_width, float max_len)
void g3_render_laser_2d (
    material* mat_params, vec3d* headp, float head_width, vec3d* tailp,
    float tail_width, float max_len) {
    ...
    if (pt1.flags & PF_OVERFLOW) return;

    g3_rotate_vertex (&pt2, tailp);

    g3_project_vertex (&pt2);
    if (pt2.flags & PF_OVERFLOW) return;
...
void g3_render_rod (color* clr, int num_points, vec3d* pvecs, float width) {
    ...
    g3_render_primitives_colored (
        &material_params, pts, nv, PRIM_TYPE_TRISTRIP, false);
}
...
void g3_render_shield_icon (color* clr, coord2d coords[6], int resize_mode) {
    ...
    if (resize_mode != GR_RESIZE_NONE) {
        gr_resize_screen_pos (
            &coords[0].x, &coords[0].y, NULL, NULL, resize_mode);
        gr_resize_screen_pos (
            &coords[1].x, &coords[1].y, NULL, NULL, resize_mode);
        gr_resize_screen_pos (
            &coords[2].x, &coords[2].y, NULL, NULL, resize_mode);
        gr_resize_screen_pos (
            &coords[3].x, &coords[3].y, NULL, NULL, resize_mode);
        gr_resize_screen_pos (
            &coords[4].x, &coords[4].y, NULL, NULL, resize_mode);
        gr_resize_screen_pos (
            &coords[5].x, &coords[5].y, NULL, NULL, resize_mode);
    }
    ...
    // draw the polys
    g3_render_primitives_colored (
        &material_instance, v, 6, PRIM_TYPE_TRISTRIP, true);
}
...
void g3_render_line_3d (
    color* clr, bool depth_testing, const vec3d* start, const vec3d* end) {
    material mat;

    mat.set_depth_mode (
        (depth_testing) ? ZBUFFER_TYPE_READ : ZBUFFER_TYPE_NONE);
    mat.set_color (*clr);
    ...
    float line[6] = { start->xyz.x, start->xyz.y, start->xyz.z,
                      end->xyz.x,   end->xyz.y,   end->xyz.z };

    vertex_layout vert_def;

    vert_def.add_vertex_component (
        vertex_format_data::POSITION3, sizeof (float) * 3, 0);

    gr_render_primitives_immediate (
        &mat, PRIM_TYPE_LINES, &vert_def, 2, line, sizeof (float) * 6);
}

void g3_render_line_3d (
    bool depth_testing, const vec3d* start, const vec3d* end) {
...
void g3_render_colored_rect (
    ...
    // draw the polys
    g3_render_primitives_colored (
        &material_params, v, 4, PRIM_TYPE_TRIFAN, true);
}

//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//
// flash ball stuff

void flash_ball::initialize (
    int number, float min_ray_width, float max_ray_width, const vec3d* dir,
    const vec3d* pcenter, float outer, float inner, ubyte max_r, ubyte max_g,
    ubyte max_b, ubyte min_r, ubyte min_g, ubyte min_b) {
    if (number < 1) return;
...
void flash_ball::parse_bsp (int offset, ubyte* bsp_data) {
    ...
    while (ID != 0) {
        switch (ID) {
        case OP_EOF: return; break;
        case OP_DEFPOINTS: defpoint (offset, bsp_data); break;
...
void flash_ball::initialize (
    ubyte* bsp_data, float min_ray_width, float max_ray_width,
    const vec3d* dir, const vec3d* pcenter, float outer, float inner,
    ubyte max_r, ubyte max_g, ubyte max_b, ubyte min_r, ubyte min_g,
    ubyte min_b) {
    center = *pcenter;
...
void flash_ball::render (int texture, float rad, float intinsity, float life) {
    for (int i = 0; i < n_rays; i++) {
        ...
        vm_vec_interp_constant (
            &end, &ray[i].start.world, &ray[i].end.world, life);
        vm_vec_scale (&end, rad);
        vm_vec_add2 (&end, &center);

        batching_add_beam (
            texture, &center, &end, ray[i].width * rad, intinsity);
    }
}
