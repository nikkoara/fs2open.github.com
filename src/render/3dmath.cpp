...
ubyte g3_code_vector (const vec3d* p) {
    ...
    if (p->xyz.x > p->xyz.z) cc |= CC_OFF_RIGHT;

    if (p->xyz.y > p->xyz.z) cc |= CC_OFF_TOP;

    if (p->xyz.x < -p->xyz.z) cc |= CC_OFF_LEFT;

    if (p->xyz.y < -p->xyz.z) cc |= CC_OFF_BOT;

    if (p->xyz.z < MIN_Z) cc |= CC_BEHIND;

    if (G3_user_clip) {
        // Check if behind user plane
        if (g3_point_behind_user_plane (p)) { cc |= CC_OFF_USER; }
    }
...
ubyte g3_code_vertex (vertex* p) {
    ...
    if (p->world.xyz.x > p->world.xyz.z) cc |= CC_OFF_RIGHT;

    if (p->world.xyz.y > p->world.xyz.z) cc |= CC_OFF_TOP;

    if (p->world.xyz.x < -p->world.xyz.z) cc |= CC_OFF_LEFT;

    if (p->world.xyz.y < -p->world.xyz.z) cc |= CC_OFF_BOT;

    if (p->world.xyz.z < MIN_Z) cc |= CC_BEHIND;

    if (G3_user_clip) {
        // Check if behind user plane
        if (g3_point_behind_user_plane (&p->world)) { cc |= CC_OFF_USER; }
    }
...
int g3_project_vertex (vertex* p) {
    ...
    if (p->world.xyz.z <= MIN_Z) { p->flags |= PF_OVERFLOW; }
    else {
        w = 1.0f / p->world.xyz.z;
        p->screen.xyw.x =
            (Canvas_width + (p->world.xyz.x * Canvas_width * w)) * 0.5f;
        p->screen.xyw.y =
            (Canvas_height - (p->world.xyz.y * Canvas_height * w)) * 0.5f;
