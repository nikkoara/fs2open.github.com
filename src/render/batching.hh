...
struct batch_info {
    ...
    batch_info ()
        : mat_type (FLAT_EMISSIVE), texture (-1), prim_type (PRIM_TYPE_TRIS),
          thruster (false) {}
    batch_info (material_type mat, int tex, primitive_type prim, bool thrust)
        : mat_type (mat), texture (tex), prim_type (prim), thruster (thrust) {}

    bool operator< (const batch_info& batch) const {
        if (mat_type != batch.mat_type) { return mat_type < batch.mat_type; }

        if (texture != batch.texture) { return texture < batch.texture; }
...
struct batch_buffer_key {
    ...
    batch_buffer_key (uint vertex_mask, primitive_type prim_type)
        : Vertex_mask (vertex_mask), Prim_type (prim_type) {}
...
primitive_batch* batching_find_batch (
    ...
    primitive_type prim_type = PRIM_TYPE_TRIS, bool thruster = false);

void batching_add_bitmap (
    int texture, vertex* pnt, int orient, float rad, float alpha = 1.0f,
    float depth = 0.0f);
void batching_add_bitmap_rotated (
    int texture, vertex* pnt, float angle, float rad, float alpha = 1.0f,
    float depth = 0.0f);
...
void batching_add_laser (
    int texture, vec3d* p0, float width1, vec3d* p1, float width2, int r = 255,
    int g = 255, int b = 255);
...
#endif // FREESPACE2_RENDER_BATCHING_HH
