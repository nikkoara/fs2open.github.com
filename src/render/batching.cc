...
void primitive_batch::add_triangle (
    batch_vertex* v0, batch_vertex* v1, batch_vertex* v2) {
...
void primitive_batch::clear () { Vertices.clear (); }
...
void batching_setup_vertex_layout (vertex_layout* layout, uint vert_mask) {
    ...
    if (vert_mask & vertex_format_data::mask (vertex_format_data::POSITION3)) {
        layout->add_vertex_component (
            vertex_format_data::POSITION3, stride,
            (int)offsetof (batch_vertex, position));
    }

    if (vert_mask &
        vertex_format_data::mask (vertex_format_data::TEX_COORD3)) {
        layout->add_vertex_component (
            vertex_format_data::TEX_COORD3, stride,
            (int)offsetof (batch_vertex, tex_coord));
    }

    if (vert_mask & vertex_format_data::mask (vertex_format_data::COLOR4)) {
        layout->add_vertex_component (
            vertex_format_data::COLOR4, stride,
            (int)offsetof (batch_vertex, r));
    }

    if (vert_mask & vertex_format_data::mask (vertex_format_data::RADIUS)) {
        layout->add_vertex_component (
            vertex_format_data::RADIUS, stride,
            (int)offsetof (batch_vertex, radius));
    }

    if (vert_mask & vertex_format_data::mask (vertex_format_data::UVEC)) {
        layout->add_vertex_component (
            vertex_format_data::UVEC, stride,
            (int)offsetof (batch_vertex, uvec));
    }
}

void batching_init_buffer (
    primitive_batch_buffer* buffer, primitive_type prim_type,
    uint vertex_mask) {
    batching_setup_vertex_layout (&buffer->layout, vertex_mask);

    buffer->buffer_num =
        gr_create_buffer (BufferType::Vertex, BufferUsageHint::Streaming);
...
primitive_batch_buffer*
batching_find_buffer (uint vertex_mask, primitive_type prim_type) {
    batch_buffer_key query (vertex_mask, prim_type);

    std::map< batch_buffer_key, primitive_batch_buffer >::iterator iter =
        Batching_buffers.find (query);
...
primitive_batch* batching_find_batch (
    int texture, batch_info::material_type material_id,
    primitive_type prim_type, bool thruster) {
    // Use the base texture for finding the batch item since all items can
    // reuse the same texture array
    auto base_tex = bm_get_base_frame (texture);

    batch_info query (material_id, base_tex, prim_type, thruster);

    std::map< batch_info, primitive_batch >::iterator iter =
        Batching_primitives.find (query);
...
uint batching_determine_vertex_layout (batch_info* info) {
    ...
    if (info->mat_type == batch_info::VOLUME_EMISSIVE ||
        info->mat_type == batch_info::DISTORTION) {
        return vertex_format_data::mask (vertex_format_data::POSITION3) |
...
void batching_add_bitmap_internal (
    ...
    // make a right vector from the f and up vector, this r vec is exactly what
    // we want, so...
    ...
    if (depth != 0.0f) vm_vec_scale_add (&PNT, &PNT, &fvec, depth);
...
void batching_add_point_bitmap (
    ...
    new_particle.tex_coord.xyz.z =
        (float)(texture - batch->get_render_info ().texture);
...
void batching_add_point_bitmap (
    ...
    vm_rot_point_around_line (
        &up, &vmd_y_vector, angle, &vmd_zero_vector, &vmd_z_vector);
...
void batching_add_beam_internal (
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (offset > 0.0f) { verts[i].radius = offset; }
        else {
...
void batching_add_bitmap (
    ...
    batching_add_bitmap_internal (
        batch, texture, pnt, orient, rad, &clr, depth);
}

void batching_add_bitmap_rotated (
    ...
    batching_add_bitmap_rotated_internal (
        batch, texture, pnt, angle, rad, &clr, depth);
}

void batching_add_volume_bitmap (
    int texture, vertex* pnt, int orient, float rad, float alpha,
    float depth) {
    if (texture < 0) {
        Int3 ();
...
void batching_add_volume_bitmap_rotated (
    int texture, vertex* pnt, float angle, float rad, float alpha,
    float depth) {
    if (texture < 0) {
        Int3 ();
...
void batching_add_distortion_bitmap_rotated (
    int texture, vertex* pnt, float angle, float rad, float alpha,
    float depth) {
    if (texture < 0) {
        Int3 ();
    ...
    if (!gr_is_capable (CAPABILITY_DISTORTION)) { return; }

    primitive_batch* batch =
        batching_find_batch (texture, batch_info::DISTORTION);
...
void batching_add_distortion_beam (
    ...
    primitive_batch* batch = batching_find_batch (
        texture, batch_info::DISTORTION, PRIM_TYPE_TRIS, true);

    color clr;
    batching_determine_blend_color (&clr, texture, intensity);

    batching_add_beam_internal (
        batch, texture, start, end, width, &clr, offset);
}
...
void batching_add_laser (
    ...
    batching_add_laser_internal (
        batch, texture, p0, width1, p1, width2, r, g, b);
}

void batching_add_polygon (
    ...
    batching_add_polygon_internal (
        batch, texture, pos, orient, width, height, &clr);
}
...
void batching_render_batch_item (
    primitive_batch_item* item, vertex_layout* layout,
    primitive_type prim_type, int buffer_num) {
    GR_DEBUG_SCOPE ("Batching render item");
    TRACE_SCOPE (tracing::RenderBatchItem);

    if (item->batch_item_info.mat_type ==
        batch_info::VOLUME_EMISSIVE) { // Cmdline_softparticles
        particle_material material_def;

        material_set_unlit_volume (
            &material_def, item->batch_item_info.texture,
            prim_type == PRIM_TYPE_POINTS);
        gr_render_primitives_particle (
            &material_def, prim_type, layout, (int)item->offset,
            (int)item->n_verts, buffer_num);
    }
    else if (item->batch_item_info.mat_type == batch_info::DISTORTION) {
        distortion_material material_def;

        material_set_distortion (
            &material_def, item->batch_item_info.texture,
            item->batch_item_info.thruster);
        gr_render_primitives_distortion (
            &material_def, PRIM_TYPE_TRIS, layout, (int)item->offset,
            (int)item->n_verts, buffer_num);
    }
    else {
        batched_bitmap_material material_def;

        material_set_batched_bitmap (
            &material_def, item->batch_item_info.texture, 1.0f, 2.0f);
        gr_render_primitives_batched (
            &material_def, PRIM_TYPE_TRIS, layout, (int)item->offset,
            (int)item->n_verts, buffer_num);
    }
}
...
void batching_allocate_and_load_buffer (primitive_batch_buffer* draw_queue) {
    ...
    for (size_t i = 0; i < num_items; ++i) {
        ...
        item->offset = offset;
        item->n_verts = item->batch->load_buffer (
            (batch_vertex*)draw_queue->buffer_ptr, offset);
        item->batch->clear ();
    ...
    if (draw_queue->buffer_num >= 0) {
        gr_update_buffer_data (
            draw_queue->buffer_num, draw_queue->buffer_size,
            draw_queue->buffer_ptr);
    }
}
...
void batching_load_buffers (bool distortion) {
    ...
    for (buffer_iter = Batching_buffers.begin ();
         buffer_iter != Batching_buffers.end (); ++buffer_iter) {
        // zero out the buffers
        buffer_iter->second.desired_buffer_size = 0;
    }

    // assign primitive batch items
    for (bi = Batching_primitives.begin (); bi != Batching_primitives.end ();
         ++bi) {
        ...
        if (num_verts > 0) {
            ...
            primitive_batch_buffer* buffer =
                batching_find_buffer (vertex_mask, render_info.prim_type);
...
void batching_render_buffer (primitive_batch_buffer* buffer) {
    ...
    for (int j = 0; j < batch_info::NUM_RENDER_TYPES; ++j) {
        for (size_t i = 0; i < num_batches; ++i) {
            if (buffer->items[i].batch_item_info.mat_type != j) { continue; }

            batching_render_batch_item (
                &buffer->items[i], &buffer->layout, buffer->prim_type,
                buffer->buffer_num);
        }
    }
...
void batching_shutdown () {
    for (auto buffer_iter = Batching_buffers.begin ();
         buffer_iter != Batching_buffers.end (); ++buffer_iter) {
