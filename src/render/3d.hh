...
// Combo!
#define CC_OFF \
        (CC_OFF_LEFT | CC_OFF_RIGHT | CC_OFF_BOT | CC_OFF_TOP | CC_OFF_USER | CC_BEHIND)
...
/**
 * Set view from x,y,z, viewer matrix, and zoom.  Must call one of
 * g3_set_view_*()
 */
...
extern vec3d
        View_position; // The offset to convert local coordinates to screen

extern matrix
        Light_matrix; // Used to rotate world points into current local coordinates
extern vec3d
        Light_base; // Used to rotate world points into current local coordinates

extern matrix
        Eye_matrix;        // Where the viewer's eye is pointing in World coordinates
...
extern matrix
        Object_matrix; // Where the opject is pointing in World coordinates
...
void g3_start_instance_matrix(
        const vec3d *pos, const matrix *orient, bool set_api = true);
 ...
 *
 * Takes the unrotated surface normal of the plane, and a point on it.  The
...
/*
 * Code a point. Fills in the p3_codes field of the point, and returns the
 * codes
 */
...
int g3_get_bitmap_dims(
        int bitmap, vertex *pos, float radius, int *x, int *y, int *w, int *h,
        int *size);
...
/**
 * Draw a line in HTL mode without having to go through the rotate/project
 * stuff
 */
...
void g3_render_shield_icon(
        color *clr, coord2d coords[6], int resize_mode = GR_RESIZE_FULL);
void g3_render_shield_icon(
        coord2d coords[6], int resize_mode = GR_RESIZE_FULL);
 ...
 * Flash ball
 *
 * A neat looking ball of rays that move around and look all energetic and
 * stuff
 */
struct flash_beam {
        vertex start;
...
public:
        flash_ball(
                int number, float min_ray_width, float max_ray_width = 0,
                const vec3d *dir = &vmd_zero_vector,
                const vec3d *pcenter = &vmd_zero_vector, float outer = PI2,
                float inner = 0.0f, ubyte max_r = 255, ubyte max_g = 255,
                ubyte max_b = 255, ubyte min_r = 255, ubyte min_g = 255,
                ubyte min_b = 255)
                : ray(NULL), n_rays(0)
        {
                initialize(
                        number, min_ray_width, max_ray_width, dir, pcenter, outer, inner,
        ...
        void initialize(
                ...
                ubyte min_b = 255);
        void initialize(
                ubyte *bsp_data, float min_ray_width, float max_ray_width = 0,
                const vec3d *dir = &vmd_zero_vector,
                const vec3d *pcenter = &vmd_zero_vector, float outer = PI2,
                float inner = 0.0f, ubyte max_r = 255, ubyte max_g = 255,
                ubyte max_b = 255, ubyte min_r = 255, ubyte min_g = 255,
