 ...
 * Check if in frame
 */
int g3_in_frame () { return G3_count; }

/**
 * Start the frame
 * Pass true for zbuffer_flag to turn on zbuffering
 */
void g3_start_frame_func (
    int zbuffer_flag, const char* /*filename*/, int /*lineno*/) {
    float s;
    int width, height;
    float aspect;
    ...
    if (zbuffer_flag) { gr_zbuffer_clear (TRUE); }
    else {
...
void g3_start_instance_matrix (
    const vec3d* pos, const matrix* orient, bool set_api) {
    vec3d tempv;
    matrix tempm, tempm2;
    ...
    vm_matrix_x_matrix (
        &Object_matrix, &instance_stack[instance_depth - 1].om, orient);
    ...
    if (set_api) gr_start_instance_matrix (pos, orient);
}
...
void g3_done_instance (bool use_api) {
    ...
    if (use_api) gr_end_instance_matrix ();
}
...
int g3_point_behind_user_plane (const vec3d* pnt) {
    if (G3_user_clip) {
        ...
        if (vm_vec_dot (&tmp, &G3_user_clip_normal) <= 0.0f) { return 1; }
    }
