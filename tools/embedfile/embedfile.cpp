...
// Correct the code so that the program will properly output text to a target
// .h file Correct the code so that the program will properly read bytes in and
// output as hexidecimal string values.
...
bool casecmp (const char* strA, const char* strB) {
    ...
    if (lenA != strlen (strB)) { return false; }

    for (size_t i = 0; i < lenA; i++) {
        if (toupper (strA[i]) != toupper (strB[i])) { return false; }
    }
...
void do_binary_content (
    ...
    file_out << "};" << std::endl;
    file_out << "size_t " << field_name << "_size = sizeof(" << field_name
             << ");" << std::endl;

    if (wxWidgets_image) {
        file_out << std::endl;

        file_out << "wxBitmap& " << field_name << "_to_wx_bitmap()"
                 << std::endl;
        file_out << "{" << std::endl;
        file_out << "\tstatic wxMemoryInputStream memIStream( " << field_name
                 << ", sizeof( " << field_name << " ) );" << std::endl;
...
void do_text_content (
    ...
    while (pos < file_content.length ()) {
        ...
        if (delta != 0) { file_out.write (str + pos, delta); }
...
void write_header (
    std::ostream& out, const std::string& fieldName, bool text_content,
    bool wxWidgets_image) {
    std::string headerDefine (fieldName);

    std::transform (
        fieldName.begin (), fieldName.end (), headerDefine.begin (),
        ::toupper);
    ...
    out << "#endif" << std::endl;
    if (wxWidgets_image) { out << "#include <wx/bitmap.h>" << std::endl; }
...
int main (int argc, char* argv[]) {
    if (argc < 4 || argc > 6) {
        std::cout
            << "Usage: embedfile [-wx] [-text] <inout> <output> <fieldname>"
            << std::endl;
    ...
    if (!text_content) { mode |= std::ios::binary; }

    std::ifstream file_in (input_file.c_str (), mode);
    if (file_in.bad ()) {
        std::cout << "ERROR: Error opening input file: " << input_file
                  << std::endl;
    ...
    if ((int)file_in.tellg () != -1) { input_size = (size_t)file_in.tellg (); }
    else {
        std::cout << "ERROR: Failed to get size of input file: " << input_file
                  << std::endl;
        file_in.close ();
    ...
    if (source_out.bad ()) {
        std::cout << "ERROR: Error opening output file: " << sourceName
                  << std::endl;
    ...
    if (header_out.bad ()) {
        std::cout << "ERROR: Error opening output file: " << headerName
                  << std::endl;
        return error_cantoutputfile;
    }
    ...
    if (text_content) {
        ...
        do_binary_content (
            file_in, source_out, field_name, input_size, wxWidgets_image);
    }
