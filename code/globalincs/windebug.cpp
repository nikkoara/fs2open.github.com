...
#ifdef _MSC_VER
#include <crtdbg.h>
/* Uncomment SHOW_CALL_STACK to show the call stack in Asserts, Warnings, and
 * Errors */
...
public:
    virtual bool ResolveSymbols () { return true; }
    ...
    virtual void
    OnEntry (void* address, const char* module, const char* symbol) {
    ...
    std::string DumpToString () {
        ...
        for (size_t i = 0; i < m_stackFrames.size (); i++) {
            callstack += m_stackFrames[i].module + "! " +
                         m_stackFrames[i].symbol + "\n";
        }
...
public:
    ...
    void ClearReport ();
    int DumpDebugInfo (
        std::ostream& dumpBuffer, const BYTE* caller, HINSTANCE hInstance);
...
void PE_Debug ::DumpLineNumber (
    std::ostream& dumpBuffer, DWORD relativeAddress) {
    PIMAGE_LINENUMBER line = BasedPtr (
        PIMAGE_LINENUMBER, COFFDebugInfo, COFFDebugInfo->LvaToFirstLinenumber);
    ...
    if (lineNum != none) { dumpBuffer << "  line " << lineNum << "\n"; }
}
...
int Sym_compare (const void* arg1, const void* arg2) {
    ...
    if (sym1->section < sym2->section) { return -1; }
    else if (sym1->section > sym2->section) {
        return 1;
    }
    else {
        if (sym1->offset > sym2->offset) { return 1; }
        else {
            return -1;
        }
    }
...
int Sym_compare1 (const void* arg1, const void* arg2) {
    ...
    if (sym1->size < sym2->size) { return 1; }
...
void DumpSymbols () {
    ...
    for (i = 0; i < Num_symbols; i++) {
        ...
        if (sym->size > 0) total_size += sym->size;
...
void PE_Debug::DumpSymbolInfo (
    ...
    for (int i = 0; i < COFFSymbolCount; i++) {
        ...
        if (strnicmp (symName, ".text", 5) == 0 ||
            ...
            if (currentSym->Value <= relativeAddress) {
                PIMAGE_AUX_SYMBOL auxSym =
                    (PIMAGE_AUX_SYMBOL) (currentSym + 1);
                if (currentSym->Value + auxSym->Section.Length >=
                    relativeAddress) {
        ...
        BOOL isFunction =
            ISFCN (currentSym->Type); // Type == 0x20, See WINNT.H
        if (isFunction &&
        ...
        if (!isFunction && (currentSym->SectionNumber >= 0)) {
            if ((symName[0] == '_' && symName[1] != '$') ||
                ...
                Add_Symbol (
                    currentSym->SectionNumber, currentSym->Value, symName,
                    pretty_module);
            }
        }
#endif
...
PIMAGE_SECTION_HEADER PE_Debug ::SectionHeaderFromName (const char* name) {
    ...
    for (unsigned i = 0; i < NT_Header->FileHeader.NumberOfSections; i++) {
        if (strnicmp (
                (const char*)section->Name, name, IMAGE_SIZEOF_SHORT_NAME) ==
            0)
            return (section);
        else
            section++;
...
PIMAGE_COFF_SYMBOLS_HEADER PE_Debug ::GetDebugHeader () {
    ...
    if (NT_Header->FileHeader.PointerToSymbolTable == 0) return (0);
    DWORD debugDirRVA =
        NT_Header->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DEBUG]
            .VirtualAddress;
    if (debugDirRVA == 0) return (0);

    // The following values must be calculated differently for MS/Borland files
    PIMAGE_DEBUG_DIRECTORY debugDir;
    DWORD size;

    // Borland files have the debug directory at the beginning of a .debug
    // section
    ...
    if (debugHeader && debugHeader->VirtualAddress == debugDirRVA) {
        debugDir = (PIMAGE_DEBUG_DIRECTORY) (
            debugHeader->PointerToRawData + (DWORD)fileBase);
    ...
    for (DWORD i = 0; i < debugFormats; i++) {
        if (debugDir->Type == IMAGE_DEBUG_TYPE_COFF)
            return ((PIMAGE_COFF_SYMBOLS_HEADER) (
...
void PE_Debug ::FindDebugInfo () {
    ...
    try {
        ...
        if (fileBase->e_magic == IMAGE_DOS_SIGNATURE) {
            ...
            NT_Header =
                BasedPtr (PIMAGE_NT_HEADERS, fileBase, fileBase->e_lfanew);
            ...
            if (NT_Header->Signature == IMAGE_NT_SIGNATURE) {
                ...
                // Get a pointer to the symbol table and retrieve the number of
                // symbols
                if (NT_Header->FileHeader.PointerToSymbolTable)
                    COFFSymbolTable = BasedPtr (
                        PIMAGE_SYMBOL, fileBase,
...
int PE_Debug::DumpDebugInfo (
    ...
    while ((*p != '\\') && (*p != '/') && (*p != ':')) p--;
    p++;
    if (strlen (p) < 1) { strcpy_s (pretty_module, "<unknown>"); }
    else {
        memmove (pretty_module, p, strlen (p) + 1);
    }
    ...
    if (fileBase) {
        ...
        try {
            ...
            if (COFFSymbolCount != 0 && COFFSymbolTable != NULL) {
                ...
                // dumpBuffer.Printf( "Call stack is unavailable, because there
                // is\r\nno COFF debugging info in this module.\r\n" ) ; JAS
                // dumpBuffer.Printf( "  no debug information\r\n" ) ;
                dumpBuffer << "    " << pretty_module << " "
                           << std::setprecision (8) << std::hex
                           << (void*)caller
...
void DumpCallsStack (std::ostream& dumpBuffer) {
    ...
    __asm MOV parentEBP, EBP
        ...
        if ((parentEBP & 3) ||
            IsBadReadPtr ((DWORD*)parentEBP, sizeof (DWORD))) {
            break;
        }
        ...
        if (IsBadReadPtr (NextCaller, sizeof (BYTE*))) { break; }
