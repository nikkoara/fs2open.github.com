 ...
 * doing anything Unfortunately, GCC (on Linux and Mac OS X) does not yet
 * provide these functions, therefore, we must! (if only to reduce the amount
 * of noise the static analysis tools are spitting out) They are part of
 * ISO/IEC TR 24731 and may find their way into the CRTs at some point, at
 * which point these functions must be removed from the engine. While these
 * functions do not add a huge amount of benefit for heap-allocated strings,
 * they can protect against a class of buffer overruns in stack allocated
 * situations.
 *
 */
