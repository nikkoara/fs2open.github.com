...
#define VM_WARP_CHASE \
    (1 << 6) // View while warping out (form normal view mode)
...
#define VM_FREECAMERA \
    (1 << 13) // Camera is not attached to any particular object, probably
              // under SEXP control
...
extern int
    Game_restoring; // If set, this means we are restoring data from disk
...
#define NUM_DEFAULT_DETAIL_LEVELS \
    4 // How many "predefined" detail levels there are
