 ...
 * Maintained by: portej05 (please run patches past him before committing
 * here!)
 */

/* Based on the ATL headers, however, updated to fix one or two bugs in the ATL
 * headers and moved to the Sym*64 functions
 */
...
BOOL SCP_mspdbcs_ResolveSymbol (
    HANDLE hProcess, UINT_PTR dwAddress,
    SCP_mspdbcs_SDumpStackSymbolInfo& siSymbol);
LPVOID __stdcall SCP_mspdbcs_FunctionTableAccess (
    HANDLE hProcess, DWORD64 dwPCAddress);
DWORD64 __stdcall SCP_mspdbcs_GetModuleBase (
...
BOOL SCP_mspdbcs_ResolveSymbol (
    ...
    __try {
        ...
        if (SymGetSymFromAddr64 (
                hProcess, dwAddress, &(siSymbol.dwOffset), &sym.sym))
#else
        if (SymGetSymFromAddr64 (
                hProcess, (DWORD)dwAddress, &(siSymbol.dwOffset), &sym.sym))
#endif
        {
            pszSymbol = sym.sym.Name;

            if (UnDecorateSymbolName (
                    sym.sym.Name, szUndec,
                    sizeof (szUndec) / sizeof (szUndec[0]),
                    UNDNAME_NO_MS_KEYWORDS | UNDNAME_NO_ACCESS_SPECIFIERS)) {
                pszSymbol = szUndec;
            }
            else if (SymUnDName64 (
                         &sym.sym, szUndec,
                         sizeof (szUndec) / sizeof (szUndec[0]))) {
                pszSymbol = szUndec;
            }
            ...
            if (siSymbol.dwOffset != 0) {
                sprintf_s (
                    szWithOffset, SCP_MSPDBCS_MAX_SYMBOL_LENGTH,
                    "%s + %lld bytes", pszSymbol, siSymbol.dwOffset);
                szWithOffset[SCP_MSPDBCS_MAX_SYMBOL_LENGTH - 1] =
                    '\0'; /* Because sprintf doesn't guarantee NULL terminating
                           */
...
DWORD64 __stdcall SCP_mspdbcs_GetModuleBase (
...
#endif
    {
        ...
        if (VirtualQueryEx (
                hProcess, (LPVOID)returnAddress, &memoryBasicInfo,
                sizeof (MEMORY_BASIC_INFORMATION))) {
            DWORD cch = 0;
            char szFile[_MAX_PATH] = { 0 }; /* Initialise the file path */
            cch = GetModuleFileNameA (
                (HINSTANCE)memoryBasicInfo.AllocationBase, szFile, MAX_PATH);
            SymLoadModule (
                hProcess, NULL, ((cch) ? szFile : NULL),
#if IS_64BIT
...
DWORD WINAPI SCP_mspdbcs_DumpStackThread (LPVOID pv) {
     ...
     * We're not intending to be compatible with IA64 or any other
...
#error UNKNOWN ARCHITECTURE
    ...
    /* Walk the stack */
    for (int currFrame = 0; currFrame < SCP_MSPDBCS_MAX_STACK_FRAMES;
         currFrame++) {
        if (!StackWalk64 (
                dwMachType, pdsti->hProcess, pdsti->hThread, &stackFrame,
                &context, NULL, SCP_mspdbcs_FunctionTableAccess,
                SCP_mspdbcs_GetModuleBase, NULL)) {
    ...
    if (pdsti->pIDS->ResolveSymbols ()) {
        ...
        for (size_t i = 0; i < addresses.size (); i++) {
            ...
            if (SCP_mspdbcs_ResolveSymbol (
                    pdsti->hProcess, (UINT_PTR)addresses[i], info)) {
...
HRESULT SCP_DumpStack (SCP_IDumpHandler* pIDH) {
    ...
    HANDLE hThread = NULL;
    if (!DuplicateHandle (
            hPseudoProcess, hPseudoThread,    /* Source process and thread */
            hPseudoProcess, &hThread,         /* Target process and thread */
            0, FALSE, DUPLICATE_SAME_ACCESS)) /* Non-Inheritable, same access
                                                 as current process/thread */
    ...
    HANDLE workerThread =
        CreateThread (NULL, 0, SCP_mspdbcs_DumpStackThread, &info, 0, &dwID);
