...
// Goober5000
void read_file_text_from_default (
    const default_file& file, char* processed_text, char* raw_text) {
    ...
    if (Parsing_paused && ((processed_text == NULL) || (raw_text == NULL))) {
        Error (
            LOCATION,
            "ERROR: Neither \"processed_text\" nor \"raw_text\" may be NULL "
            "when parsing is paused!!\n");
    }
    ...
    // copy text in the array (but only if the raw text and the array are not
    // the same)
    if (raw_text != file.data) {
        // Copy the file contents into the array and null-terminate it
        // We have to make sure to adjust the size if the size of a char is
        // more than 1
