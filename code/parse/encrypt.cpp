...
void encrypt (
    ...
    *scrambled_len = 0;
    ...
    if (use_8bit) { memcpy (scrambled_text, &Encrypt_signature_8bit, 4); }
    else {
    ...
    for (i = 0; i < text_len; i++) {
        ...
        if ((unsigned char)text[i] == 0x92) { text[i] = 0x27; }

        if (use_8bit) { scrambled_text[byte_offset++] = text[i]; }
        else {
            switch (bit_offset) {
            case 0:
            ...
            case 2:
                ...
                scrambled_text[byte_offset] |=
                    ((text[i] >> 1) & 0x3F); // put in top 6 bits
                byte_offset++;
                scrambled_text[byte_offset] =
                    (char)((text[i] << 7) & 0x80); // put in last bit
                bit_offset = 1;
                break;
            case 3:
                scrambled_text[byte_offset] &= 0xe0; // clear out bottom 5 bits
                scrambled_text[byte_offset] |=
                    ((text[i] >> 2) & 0x1F); // put in top 5 bits
                byte_offset++;
                scrambled_text[byte_offset] =
                    (char)((text[i] << 6) & 0xc0); // put in last two bits
                bit_offset = 2;
                break;
            case 4:
                scrambled_text[byte_offset] &= 0xf0; // clear out bottom 4 bits
                scrambled_text[byte_offset] |=
                    ((text[i] >> 3) & 0x0F); // put in top 4 bits
                byte_offset++;
                scrambled_text[byte_offset] =
                    (char)((text[i] << 5) & 0xe0); // put in last three bits
                bit_offset = 3;
                break;
            case 5:
                scrambled_text[byte_offset] &= 0xf8; // clear out bottom 3 bits
                scrambled_text[byte_offset] |=
                    ((text[i] >> 4) & 0x07); // put in top 3 bits
                byte_offset++;
                scrambled_text[byte_offset] =
                    (char)((text[i] << 4) & 0xf0); // put in last four bits
                bit_offset = 4;
                break;
            case 6:
                scrambled_text[byte_offset] &= 0xfc; // clear out bottom 2 bits
                scrambled_text[byte_offset] |=
                    ((text[i] >> 5) & 0x03); // put in top 2 bits
                byte_offset++;
                scrambled_text[byte_offset] =
                    (char)((text[i] << 3) & 0xf8); // put in last five bits
                bit_offset = 5;
                break;
            case 7:
                scrambled_text[byte_offset] &= 0xfe; // clear out bottom bit
                scrambled_text[byte_offset] |=
                    ((text[i] >> 6) & 0x01); // put in top bit
                byte_offset++;
                scrambled_text[byte_offset] =
                    (char)((text[i] << 2) & 0xfc); // put in last six bits
                bit_offset = 6;
                break;
            default: return;
            }
...
void unencrypt (
    ...
    if ((encrypt_id != Encrypt_signature) &&
        (encrypt_id != Encrypt_signature_8bit)) {
    ...
    for (i = 0; i < scrambled_len; i++) { scrambled_text[i] ^= i; }
    ...
    if (scrambled_len % 7) { num_runs++; }

    for (i = 0; i < num_runs; i++) {
        // a run consists of 8 chars packed into 56 bits (instead of 64)

        text[byte_offset] =
            (char)((scrambled_text[scramble_offset] >> 1) & 0x7f);
        byte_offset++;
        scramble_offset++;

        if (scramble_offset >= scrambled_len) { break; }

        text[byte_offset] =
            (char)((scrambled_text[scramble_offset] >> 2) & 0x3f);
        text[byte_offset] |=
            ((scrambled_text[scramble_offset - 1] << 6) & 0x40);
        byte_offset++;
        scramble_offset++;
        ...
        text[byte_offset] =
            (char)((scrambled_text[scramble_offset] >> 3) & 0x1f);
        text[byte_offset] |=
            ((scrambled_text[scramble_offset - 1] << 5) & 0x60);
        byte_offset++;
        scramble_offset++;
        ...
        text[byte_offset] =
            (char)((scrambled_text[scramble_offset] >> 4) & 0x0f);
        text[byte_offset] |=
            ((scrambled_text[scramble_offset - 1] << 4) & 0x70);
        byte_offset++;
        scramble_offset++;
        ...
        text[byte_offset] =
            (char)((scrambled_text[scramble_offset] >> 5) & 0x07);
        text[byte_offset] |=
            ((scrambled_text[scramble_offset - 1] << 3) & 0x78);
        byte_offset++;
        scramble_offset++;
        ...
        text[byte_offset] =
            (char)((scrambled_text[scramble_offset] >> 6) & 0x03);
        text[byte_offset] |=
            ((scrambled_text[scramble_offset - 1] << 2) & 0x7c);
        byte_offset++;
        scramble_offset++;
        ...
        text[byte_offset] =
            (char)((scrambled_text[scramble_offset] >> 7) & 0x01);
        text[byte_offset] |=
            ((scrambled_text[scramble_offset - 1] << 1) & 0x7e);
        byte_offset++;
...
ushort Lvl1_keys[NUM_LVL1_KEYS] = { 0xa820, 0x71f0, 0x88da, 0x1fff,
                                    0x2718, 0xe6a1, 0x42b8, 0x0ce9,
                                    0x10ec, 0xd77d, 0x3fa9 };
    ...
    *scrambled_len = 0;
    while (*scrambled_len < text_len) {
        ...
        if ((text_len - *scrambled_len) < (NUM_LVL1_KEYS * 2)) {
            memcpy (
                lvl1_block, text + *scrambled_len, text_len - *scrambled_len);
            block_size = text_len - *scrambled_len;
        }
        // if we have at least one full block left
        else {
            memcpy (
                lvl1_block, text + *scrambled_len, NUM_LVL1_KEYS * 2); //-V512
        ...
        block_checksum =
            chksum_add_short (block_checksum, (char*)lvl1_block, block_size);
    ...
    *text_len = 0;
    ...
    while (*text_len < scrambled_len) {
        ...
        if ((scrambled_len - *text_len) < (NUM_LVL1_KEYS * 2)) {
            memcpy (
                lvl1_block, scrambled_text + *text_len,
                scrambled_len - *text_len);
        ...
        // if we have at least one full block left
            ...
            memcpy (
                lvl1_block, scrambled_text + *text_len,
                NUM_LVL1_KEYS * 2); //-V512
            block_size = NUM_LVL1_KEYS * 2;
        }

        // copy the block so that we can properly calculate the next running
        // checksum
        ...
        // the running checksum
        block_checksum = chksum_add_short (
            block_checksum, (char*)lvl1_block_copy, block_size);
...
void encrypt_init () {
    ...
    if (Encrypt_inited) { return; }

    // meddle with the key table so someone reading the disassembly won't be
    // able to get key values unless they're _REALLY_ careful
