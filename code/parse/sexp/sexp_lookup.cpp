...
std::unordered_map< int, std::unique_ptr< sexp::DynamicSEXP > >
    operator_const_mapping;

// This contains which operator is the next free operator in a specific
// category
...
int get_next_free_operator (int category) {
    Assertion (
        ...
        "The primary change category is full so it can't be used for new "
        "operators!");

    auto iter = next_free_operator_mapping.find (category);

    if (iter == next_free_operator_mapping.end ()) {
        // The next operator number isn't initialized yet so we initialize that
        // here
        int max_op_value = 0;
        for (auto& oper : Operators) {
            // We need to do some ugly bit masking here since the SEXP system
            // uses different bytes of the operator number for storing various
            // things
            if ((oper.value & OP_CATEGORY_MASK) == category) {
                // This operator has the right category
                // We only store the number part which is the lowest byte of
                // the operator number
                max_op_value = std::max (max_op_value, oper.value & 0xFF);
            }
        }
        iter = next_free_operator_mapping
                   .insert (std::make_pair (category, max_op_value + 1))
                   .first;
    }
    ...
    try {
        ...
        while (optional_string ("$Operator:")) {
            ...
            if (std::strpbrk (name.c_str (), INVALID_CHARS) != nullptr) {
                error_display (
                    0, "Found invalid SEXP name '%s'!", name.c_str ());
            ...
            if (get_operator_index (name.c_str ()) >= 0) {
                error_display (
                    0, "The SEXP '%s' is already defined!", name.c_str ());
...
void add_dynamic_sexp (std::unique_ptr< DynamicSEXP >&& sexp) {
    ...
    new_op.value = get_next_free_operator (sexp->getCategory ()) |
                   sexp->getCategory () | OP_NONCAMPAIGN_FLAG;
    new_op.type = SEXP_ACTION_OPERATOR;

    operator_const_mapping.insert (
        std::make_pair (new_op.value, std::move (sexp)));
...
DynamicSEXP* get_dynamic_sexp (int operator_const) {
    ...
    if (iter == operator_const_mapping.end ()) { return nullptr; }
...
void dynamic_sexp_shutdown () { operator_const_mapping.clear (); }
int add_subcategory (int parent_category, const std::string& name) {
    // Another hack to make sure change2 is interpreted as the normal change
    // category
    ...
    for (auto& subcat : op_submenu) {
        if ((subcat.id & OP_CATEGORY_MASK) == parent_category) {
            max_subcategory_value =
                std::max (max_subcategory_value, subcat.id & SUBCATEGORY_MASK);
        }
    }
