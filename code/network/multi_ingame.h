...
// ingame join defines - NOTE : it is important to keep these flags so that
// they appear numerically in the order in which the events they represent are
// done
#define INGAME_JOIN_FLAG_SENDING_SETS \
    (1 << 0) // sending player settings to him - it is important that this is
             // done _first_
#define INGAME_JOIN_FLAG_CAMPAIGN_INFO \
    (1 << 1) // sending player settings to him - it is important that this is
             // done _first_
#define INGAME_JOIN_FLAG_LOADING_MISSION \
    (1 << 2) // player has finished loading the mission
#define INGAME_JOIN_FLAG_SENDING_SHIPS \
    (1 << 3) // sending ships to an ingame joiner
#define INGAME_JOIN_FLAG_SENDING_WINGS \
    (1 << 4) // sending wings to an ingame joiner
#define INGAME_JOIN_FLAG_SENDING_RPTS \
    (1 << 5) // sending respawn points to a player
#define INGAME_JOIN_FLAG_SENDING_POST \
    (1 << 6) // sending standard post briefing data block
#define INGAME_JOIN_FLAG_SENDING_WSS (1 << 7) // sending wss slots info
#define INGAME_JOIN_FLAG_PICK_SHIP \
    (1 << 8) // player is in the "pick" ship screen
#define INGAME_JOIN_FLAG_FILE_XFER \
    (1 << 9) // player is in the process of downloading the mission file

#define INGAME_SHIP_UPDATE_TIME \
    1500 // update time information for all ships the ingame joiner can see
...
void send_ingame_ship_request_packet (
    int code, int rdata, net_player* pl = NULL);
