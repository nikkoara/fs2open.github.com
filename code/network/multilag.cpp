...
int Multi_streak_stamp =
    -1; // timestamp telling when the streak of a certain lag is done
...
int multi_lag_select (
    ...
    if (select (nfds, readfds, writefds, except_fds, timeout)) {
        ...
        ret_val = recvfrom (
            readfds->fd_array[0], t_buf, 1024, 0, (SOCKADDR*)&ip_addr,
            &t_from_len);

        // wacky socket error
        if (ret_val == SOCKET_ERROR) { return SOCKET_ERROR; }
    ...
    // NOTE : this _could_ be the packet we just read. In fact, with a 0 lag,
    ...
    while (moveup != END_OF_LIST (&Lag_used_list)) {
        ...
        if ((readfds->fd_array[0] == (SOCKET)moveup->socket) &&
            ((moveup->stamp <= 0) || timestamp_elapsed (moveup->stamp))) {
...
int multi_lag_recvfrom (
    ...
    // if this happens, it means that the multi_lag_select() returned an
    // improper value
    Assert (item);
    // stuff the data
...
int multi_lag_get_random_lag () {
    ...
    if ((Multi_streak_stamp == -1) ||
        (timestamp_elapsed (Multi_streak_stamp))) {
...
int multi_lag_should_be_lost () {
    ...
    if (rand_val <= (Multi_loss_base + mod)) { return 1; }
...
void multi_lagloss_dcf () {
    ...
    if (dc_optional_string_either ("status", "--status") ||
        ...
        dc_printf (
            "%d\t\t%d\t\t%d\t\t%d\n\n", Multi_lag_base, Multi_lag_min,
            Multi_lag_max, Multi_streak_time);

        // display loss settings
        dc_printf ("Loss : \n");
        dc_printf ("Base  \t\tMin   \t\tMax\n");
        dc_printf (
            "%f\t\t%f\t\t%f\n", Multi_loss_base, Multi_loss_min,
            Multi_loss_max);
        return;
    }
    ...
    dc_printf ("lag <ms>\n");
    dc_printf (
        "\tSets the lag base value if <ms> is within the max and min limits "
        "(see lag_min and lag_max)\n");
    dc_printf (
        "\tIf <ms> is outside of the max and min limits, then nothing is "
        "done\n");
    ...
    dc_printf (
        "\tSets the duration of lag streaks where the lag is consistant for "
        "<ms>\n");
    dc_printf (
        "\tEx: A value of 2000 would result in lag streaks that last 2 "
        "seconds each\n\n");

    dc_printf ("lagloss\n");
    dc_printf (
        "\tDisplays this text. Passing --status will display the status of "
        "the entire lag system");
}
...
DCF (lag_min, "Sets the lag min value (Multiplayer)") {
    ...
    if (dc_optional_string_either ("help", "--help")) { multi_lagloss_dcf (); }
...
DCF (loss, "Sets the loss base value (Multiplayer)") {
    ...
    if (val_i > 100) { dc_printf ("Illegal loss base value, ignoring..."); }
    else if (val_i < 0) {
...
DCF (loss_min, "Sets the loss min value (Multiplayer)") {
    ...
    if (val_f > Multi_loss_base) {
        ...
        if (val_f < 0) { Multi_loss_min = -1.0f; }
        else {
...
DCF (loss_max, "Sets the loss max value (Multiplayer)") {
    ...
    if (val_f < Multi_loss_base) {
        ...
        // otherwise set the value
        if (val_f < 0) { Multi_loss_max = -1.0f; }
        else {
...
DCF (lag_streak, "Sets the duration of lag streaks (Multiplayer)") {
    ...
    dc_stuff_int (&val);
    if (val >= 0) { Multi_streak_time = val; }
    else {
