...
void multi_campaign_start (char* filename) {
    ...
    // if we're the server of the game we should also be loading the campaign
    // up. otherwise, we let the standalone do it
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        // start the campaign, passing 0 so we do _not_ load the savefile. this
        // is only for starting new campaigns
        ...
        nprintf (
            ("Network",
             "MULTI CAMPAIGN : overriding respawn setting with mission max "
             "%d\n",
...
void multi_campaign_next_mission () {
    ...
    // this will eventually be replaced with the real filename of the next
    // mission
...
void multi_campaign_do_debrief (int player_status) {
    ...
    if ((Net_player->flags & NETINFO_FLAG_AM_MASTER) &&
        (Campaign.current_mission != -1) && player_status) {
...
void multi_campaign_done_popup () {
    popup (
        ...
        XSTR ("The Campaign Is Complete. Thank You For Playing", 643));
    multi_quit_game (PROMPT_NONE);
}

// evaluate post mission goal stuff for the campaign and send all relevant
// stuff to clients
...
void multi_campaign_client_store_goals (int mission_num) {
    ...
    for (idx = 0; idx < Num_goals; idx++) {
        strcpy_s (
            Campaign.missions[mission_num].goals[idx].name,
            Mission_goals[idx].name);
    }

    // copy mission events into the campaign events
    for (idx = 0; idx < Num_mission_events; idx++) {
        strcpy_s (
            Campaign.missions[mission_num].events[idx].name,
            Mission_events[idx].name);
    }
}
...
void multi_campaign_process_update (ubyte* data, header* hinfo) {
    ...
    case MC_CODE_POOL:
        ...
        // if we're not in campaign mode, bash all weapons and ships to be
        // "allowed"
        if (!val) {
            // all ships
            for (idx = 0; idx < static_cast< int > (Ship_info.size ());
                 idx++) {
    ...
    case MC_CODE_DEBRIEF:
        ...
        Campaign.missions[cur_mission].goals =
            (mgoal*)vm_malloc (sizeof (mgoal) * val);

        GET_DATA (val);
        Campaign.missions[cur_mission].num_events = val;
        Campaign.missions[cur_mission].events =
            (mevent*)vm_malloc (sizeof (mevent) * val);
...
void multi_campaign_send_debrief_info () {
    ...
    // add the goals
    for (idx = 0; idx < Campaign.missions[Campaign.current_mission].num_goals;
         idx++) {
        val = (ubyte)Campaign.missions[Campaign.current_mission]
                  .goals[idx]
                  .status;
        ADD_DATA (val);
    }
    ...
    // add the events
    for (idx = 0; idx < Campaign.missions[Campaign.current_mission].num_events;
         idx++) {
        val = (ubyte)Campaign.missions[Campaign.current_mission]
                  .events[idx]
                  .status;
...
void multi_campaign_send_pool_status () {
    ...
    // if we're not in campaign mode, send a single byte saying "allow all
    // ships and weapons"
    ...
    // otherwise add all relevant ship/weapon pool data
        ...
        for (idx = 0; idx < static_cast< int > (Ship_info.size ()); idx++) {
            if (Campaign.ships_allowed[idx]) { spool_size++; }
        }

        // determine how many weapon types we're going to add
        wpool_size = 0;
        for (idx = 0; idx < Num_weapon_types; idx++) {
            if (Campaign.weapons_allowed[idx]) { wpool_size++; }
        }
...
void multi_campaign_send_start (net_player* pl) {
    ...
    if (pl != NULL) { multi_io_send_reliable (pl, data, packet_size); }
    // send to all players
    else {
        multi_io_send_to_all_reliable (data, packet_size);
    }
...
// campaign start packet for ingame joiners.  Sends filename and goal/event
// name and status
void multi_campaign_send_ingame_start (net_player* pl) {
    ...
    if (Game_mode & GM_CAMPAIGN_MODE) {
        ...
        // send the number and status of all goals event for all previous
        // missions
        for (i = 0; i < Campaign.num_missions; i++) {
            ubyte status;
            ...
            if (!Campaign.missions[i].completed) continue;
...
void multi_campaign_process_ingame_start (ubyte* data, header* hinfo) {
    ...
    case MC_JIP_INITIAL_PACKET:
        ...
        mission_campaign_clear (); // should free all data structures which
                                   // need to be freed
    ...
    case MC_JIP_GE_STATUS:
        ...
        if (num_goals > 0) {
            Campaign.missions[mission_num].goals =
                (mgoal*)vm_malloc (sizeof (mgoal) * num_goals);
        }
        ...
        if (num_events > 0) {
            Campaign.missions[mission_num].events =
                (mevent*)vm_malloc (sizeof (mevent) * num_events);
        }
