...
// this time must be longer than the MULTI_JOIN_REFRESH_TIME but shorter than
// twice the MULTI_JOIN_REFRESH_TIME so that it does not time out between
// refresh times, but cannot last more than 2 complete refreshed without timing
// out. Just trust me - DB
#define MULTI_JOIN_SERVER_TIMEOUT \
    (MULTI_JOIN_REFRESH_TIME + (MULTI_JOIN_REFRESH_TIME / 2))
...
typedef struct multi_create_info {
    ...
    multi_create_info ()
        : flags (0), respawn (0), max_players (0),
...
// initialize/display all bitmaps, etc related to displaying the voice system
// status
...
#define MULTI_SYNC_POST_BRIEFING \
    1                       // moving from the briefing to the gameplay stage
#define MULTI_SYNC_INGAME 2 // ingame joiners data sync
extern int Multi_sync_mode; // should always set this var before calling
                            // GS_EVENT_MULTI_MISSION_SYNC
extern int Multi_sync_countdown; // time in seconds until the mission is going
                                 // to be launched
void multi_sync_init ();
void multi_sync_do ();
void multi_sync_close ();
void multi_sync_start_countdown (); // start the countdown to launch when the
                                    // launch button is pressed

// note : these functions are called from within missiondebrief.cpp - NOT from
// freespace.cpp
...
void multi_debrief_server_left (); // call this when the server has left and we
                                   // would otherwise be saying "contact lost
                                   // with server"
void multi_debrief_stats_accept (); // call this to insure that stats are not
                                    // undone when we leave the debriefing
void multi_debrief_stats_toss ();   // call this to "toss" the stats packet
int multi_debrief_stats_accept_code (); // call this to determine the status of
                                        // multiplayer stats acceptance
void multi_debrief_server_process ();   // process all details regarding moving
                                        // the netgame to its next state
