...
// set this to be true so that game_shutdown() will fire up the launcher, then
// post a quit game event
...
int multi_update_http_init () { return 0; }

// do frame for the popup. returns 0 if not done yet, 1 if succeeded, 2 on
// error
int multi_update_http_do () {
    ...
    if (Multi_update_get == NULL) {
        // error string
        strcpy_s (
            Multi_update_error_string,
            XSTR ("Could not get data from website", 977));
    ...
    if (Multi_update_get->IsConnecting () ||
        Multi_update_get->IsReceiving ()) {
        return 0;
    }
...
int multi_update_error_verifying () {
    ...
    strcat_s (
        out_str, XSTR (
                     "There was an error verifying your version of FreeSpace, "
                     "if you continue, you will not necessarily be up to date",
                     978));

    switch (popup (
        PF_USE_AFFIRMATIVE_ICON | PF_USE_NEGATIVE_ICON, 2,
        XSTR ("&Go back", 1524), XSTR ("&Continue", 1525), out_str)) {
    // continue on in freespace like nothing happened
    case 1:
    case -1: return MULTI_UPDATE_CONTINUE;

    // go back to the main menu
    default: return MULTI_UPDATE_MAIN_MENU;
    }
}

// check to see if the version of FS on this machine is not recent. run in a
// popup if the versions don't check out, bail to the launcher returns 0 if
// freespace should continue, 1 if freespace should go back to the main menu,
...
int multi_update_gobaby () {
    ...
    /*
        ...
        if(strlen(Multi_options_proxy) > 0){
            sprintf(msg, "%s (%s : %d)", XSTR("Verifying FreeSpace
       Version",981), Multi_options_proxy, Multi_options_proxy_port); } else {
            strcpy_s(msg, XSTR("Verifying FreeSpace Version",981));
        }
        ret_code = popup_till_condition(multi_update_http_do,
       XSTR("Cancel",948), msg);
        ...
        case 1 :
            ...
            switch(version_compare(VERSION_LOC_FNAME, NULL, NULL, NULL, NULL,
       NULL, NULL)){
            // error
            case -1:
            ...
            case 0:
                switch(popup(PF_USE_AFFIRMATIVE_ICON | PF_USE_NEGATIVE_ICON, 2,
       "&Update Later", "&Yes", XSTR("A new version of FreeSpace is available.
       You must update to the new version to play on PXO\n\nAuto update now?",
       980))){
                ...
                default:
                    // set things up so that the launcher is launched when
       FreeSpace is done closing Multi_update_fireup_launcher_on_exit = 1;
