...
#define MAX_NET_BUFFER \
    (1024 * 16) // define and variable declaration for our local tcp buffer
...
#define MULTI_SERVER_MAX_TIMEOUT \
    (F1_0 * 4) // after this number of milliseoncds, stop client simulation
#define MULTI_SERVER_MAX_TIMEOUT_LARGE \
    (F1_0 * 40) // done anytime not in mission
#define MULTI_SERVER_WAIT_TIME \
    (F1_0 * 60) // wait 60 seconds to reconnect with the server
#define MULTI_SERVER_GONE 1
#define MULTI_SERVER_ALIVE 2

// define for when to show "slow network" icon
#define MULTI_SERVER_SLOW_PING_TIME \
    700 // when average ping time to server reaches this -- display hud icon
...
int Multi_mission_loaded =
    0; // flag, so that we don't load the mission more than once client side
int Ingame_join_net_signature =
    -1; // signature for the player obj for use when joining ingame
int Multi_button_info_ok = 0; // flag saying it is ok to apply critical button
                              // info on a client machine
int Multi_button_info_id =
    0; // identifier of the stored button info to be applying
...
int Multi_has_cd = 0;       // if this machine has a cd or not (call
...
int Multi_num_players_at_start =
    0; // the # of players present (kept track of only on the server) at the
       // very start of the mission
...
int Netgame_send_time =
    -1; // timestamp used to send netgame info to players before misison starts
int State_send_time = -1; // timestamp used to send state information to the
                          // host before a mission starts
int Gameinfo_send_time =
    -1; // timestamp used by master to send game information to clients
int Next_ping_time = -1; // when we should next ping all
int Multi_server_check_count =
    0; // var to keep track of reentrancy when checking server status
...
LOCAL fix Multi_server_wait_start; // variable to hold start time when waiting
                                   // to reestablish with server
...
void multi_init () {
    ...
    if (!Is_standalone) multi_load_common_icons ();
}

// this is an important function which re-initializes any variables required in
// multiplayer games. Always make sure globals you add are re-initialized here
// !!!!
void multi_vars_init () {
    // initialize this variable right away.  Used in game_level_init for
    // init'ing the player
...
void multi_level_init () {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (Player == &Players[idx]) { continue; }
        Players[idx].reset ();
    }
...
// multi_check_listen() calls low level psnet routine to see if we have a
// connection from a client we should accept.
void multi_check_listen () {
    int i;
    net_addr addr;
    PSNET_SOCKET_RELIABLE sock = INVALID_SOCKET;

    // call psnet routine which calls select to see if we need to check for a
    // connect from a client by passing addr, we are telling check_for_listen
    // to do the accept and return who it was from in addr.  The
    ...
    if (sock != INVALID_SOCKET) {
        ...
        // the connection was accepted in check_for_listen.  Find the netplayer
        // whose address we connected with and assign the socket descriptor.
        // Updated to utilize psnet_same() for address comparison so the port
        // is also taken into account. This allows multiple players using NAT
        // to access a remote server simultneously.
        for (i = 0; i < MAX_PLAYERS; i++) {
            if ((Net_players[i].flags & NETINFO_FLAG_CONNECTED) &&
                (psnet_same (&addr, &(Net_players[i].p_info.addr)))) {
                ...
                send_accept_player_data (
                    &Net_players[i],
                    (Net_players[i].flags & NETINFO_FLAG_INGAME_JOIN) ? 1 : 0);

                // send a netgame update so the new client has all the
                // necessary settings
                ...
                // NETLOG
                ml_printf (
                    NOX ("Accepted TCP connection from %s"),
...
int multi_client_server_dead () {
    ...
    // get the last time we have heard from the server.  If greater than some
    // default, then maybe display some icon on the HUD indicating slow network
    // connection.  if greater than some higher max, stop simulating on the
    // client side until we hear from the server again.
    ...
    // if in mission, use the smaller timeout value.  Outside of mission, use a
    // large one.
    if (MULTI_IN_MISSION) { max = MULTI_SERVER_MAX_TIMEOUT; }
    else {
        max = MULTI_SERVER_MAX_TIMEOUT_LARGE;
    }

    if (this_time > max) { return 1; }
    else {
        return 0;
    }
}
...
int multi_client_wait_on_server () {
    ...
    if (!is_dead) { return MULTI_SERVER_ALIVE; }
    ...
    if (this_time > MULTI_SERVER_WAIT_TIME) { return MULTI_SERVER_GONE; }
#endif

    return FALSE;
}

// function called by multiplayer clients to stop simulating when they have not
// heard from the server in a while.
void multi_client_check_server () {
    ...
    if (Multi_server_check_count) return;

    // make sure we have a valid server
    if (Netgame.server == NULL) { return; }

    Multi_server_check_count++;
    if (multi_client_server_dead ()) { Netgame.flags |= NG_FLAG_SERVER_LOST; }
    else {
    ...
    if (Netgame.flags & NG_FLAG_SERVER_LOST) {
        if (!(Game_mode & GM_IN_MISSION) && !popup_active ()) {
            ...
            rval = popup_till_condition (
                multi_client_wait_on_server, XSTR ("Cancel", 641),
                XSTR (
                    "Contact lost with server.  Stopping simulation until "
                    "contact reestablished.  Press Cancel to exit game.",
                    642));

            if (!rval || (rval == MULTI_SERVER_GONE)) {
                multi_quit_game (
                    PROMPT_NONE, MULTI_END_NOTIFY_NONE,
                    MULTI_END_ERROR_CONTACT_LOST);
            }
            Netgame.flags &= ~(NG_FLAG_SERVER_LOST);
        }
    }
...
void process_packet_normal (ubyte* data, header* header_info) {
    switch (data[0]) {
    case JOIN: process_join_packet (data, header_info); break;

    case GAME_CHAT: process_game_chat_packet (data, header_info); break;

    case NOTIFY_NEW_PLAYER:
        process_new_player_packet (data, header_info);
        break;

    case HUD_MSG: process_hud_message (data, header_info); break;

    case MISSION_MESSAGE:
        process_mission_message_packet (data, header_info);
        break;

    case LEAVE_GAME: process_leave_game_packet (data, header_info); break;

    case GAME_QUERY: process_game_query (data, header_info); break;

    case GAME_ACTIVE: process_game_active_packet (data, header_info); break;

    case GAME_INFO: process_game_info_packet (data, header_info); break;
    ...
    case GAME_UPDATE: process_netgame_update_packet (data, header_info); break;
    ...
    case ACCEPT: process_accept_packet (data, header_info); break;

    case OBJECT_UPDATE: multi_oo_process_update (data, header_info); break;

    case SHIP_KILL: process_ship_kill_packet (data, header_info); break;

    case WING_CREATE: process_wing_create_packet (data, header_info); break;

    case SHIP_CREATE: process_ship_create_packet (data, header_info); break;

    case SHIP_DEPART: process_ship_depart_packet (data, header_info); break;

    case MISSION_LOG_ENTRY:
        process_mission_log_packet (data, header_info);
        break;

    case PING: process_ping_packet (data, header_info); break;
    ...
    case XFER_PACKET:
        ...
        header_info->bytes_processed =
            multi_xfer_process_packet (data + HEADER_LENGTH, sock) +
            HEADER_LENGTH;
        break;
    ...
    case MISSION_ITEM: process_mission_item_packet (data, header_info); break;

    case MULTI_PAUSE_REQUEST:
        process_multi_pause_packet (data, header_info);
        break;

    case INGAME_NAK: process_ingame_nak (data, header_info); break;
    ...
    case MISSION_END: process_endgame_packet (data, header_info); break;
    ...
    case FILE_SIG_INFO: process_file_sig_packet (data, header_info); break;
    ...
    case FILE_SIG_REQUEST: process_file_sig_request (data, header_info); break;
    ...
    case FIRING_INFO: process_firing_info_packet (data, header_info); break;

    case CARGO_REVEALED:
        process_cargo_revealed_packet (data, header_info);
        break;

    case CARGO_HIDDEN: process_cargo_hidden_packet (data, header_info); break;
    ...
    case KICK_PLAYER: process_player_kick_packet (data, header_info); break;

    case PLAYER_SETTINGS:
        process_player_settings_packet (data, header_info);
        break;

    case DENY: process_deny_packet (data, header_info); break;
    ...
    case SLOT_UPDATE: process_pslot_update_packet (data, header_info); break;
    ...
    case VOICE_PACKET: multi_voice_process_packet (data, header_info); break;

    case TEAM_UPDATE: multi_team_process_packet (data, header_info); break;

    case ASTEROID_INFO: process_asteroid_info (data, header_info); break;
    ...
    case COUNTDOWN: process_countdown_packet (data, header_info); break;

    case DEBRIEF_INFO: process_debrief_info (data, header_info); break;
    ...
    case EMP_EFFECT: process_emp_effect (data, header_info); break;
    ...
    case BEAM_FIRED: process_beam_fired_packet (data, header_info); break;

    case SW_STD_QUERY: process_sw_query_packet (data, header_info); break;

    case EVENT_UPDATE: process_event_update_packet (data, header_info); break;

    case VARIABLE_UPDATE:
        process_variable_update_packet (data, header_info);
        break;

    case OBJECT_UPDATE_NEW: multi_oo_process_update (data, header_info); break;

    case WEAPON_DET: process_weapon_detonate_packet (data, header_info); break;

    case FLAK_FIRED: process_flak_fired_packet (data, header_info); break;

    case NETPLAYER_PAIN: process_player_pain_packet (data, header_info); break;

    case LIGHTNING_PACKET: process_lightning_packet (data, header_info); break;

    case BYTES_SENT: process_bytes_recvd_packet (data, header_info); break;
...
void multi_process_bigdata (
    ubyte* data, int len, net_addr* from_addr, int reliable) {
    int type, bytes_processed;
    int player_num;
    header header_info;
    ubyte* buf;

    // the only packets we will process from an unknown player are GAME_QUERY,
    // GAME_INFO, JOIN, PING, PONG, ACCEPT, and GAME_ACTIVE packets
    player_num = find_player (from_addr);

    // find the player who sent the message and mark the last_heard time for
    // this player check to see if netplayer is null (it may be in cases such
    // as getting lists of games from the tracker)
    if (player_num >= 0) {
    ...
    while ((bytes_processed >= 0) && (bytes_processed < len)) {
        ...
        // if its coming from an unknown source, there are only certain packets
        // we will actually process
        if ((player_num == -1) &&
            !multi_is_valid_unknown_packet ((ubyte)type)) {
            return;
        }
        ...
        // MWA -- magic number was removed from header on 8/4/97.  Replaced
        // with bytes_processed variable which gets stuffed whenever a packet
        // is processed.
...
void multi_process_reliable_details () {
    ...
    if (MULTIPLAYER_MASTER) {
        ...
        for (idx = 0; idx < MAX_PLAYERS; idx++) {
            ...
            if ((idx != MY_NET_PLAYER_NUM) &&
                MULTI_CONNECTED (Net_players[idx])) {
                // if this guy's socket is broken or disconnected, kill him
                sock_status =
                    psnet_rel_get_status (Net_players[idx].reliable_socket);
                if ((sock_status == RNF_UNUSED) ||
                    (sock_status == RNF_BROKEN) ||
                    (sock_status == RNF_DISCONNECTED)) {
                    ml_string (
                        "Shutting down rel socket because of disconnect!");
                    delete_player (idx);
                }

                // if we're still waiting for this guy to connect on his
                // reliable socket and he's timed out, boot him
                if (Net_players[idx].s_info.reliable_connect_time != -1) {
                    ...
                    // if he's timed out
                        ...
                        ((time (NULL) -
                          Net_players[idx].s_info.reliable_connect_time) >
                         MULTI_RELIABLE_CONNECT_WAIT) &&
                        (Net_players[idx].reliable_socket == INVALID_SOCKET)) {
                        ml_string (
                            "Player timed out while connecting on reliable "
                            "socket!");
                        delete_player (idx);
                    }
                }
...
// multi_process_incoming reads incoming data off the unreliable and reliable
// ports and sends the data to process_big_data
void multi_process_incoming () {
    ...
    while ((size = psnet_get (data, &from_addr)) > 0) {
        // ingame joiners will ignore UDP packets until they are have picked a
    ...
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        for (idx = 0; idx < MAX_PLAYERS; idx++) {
            if ((Net_players[idx].flags & NETINFO_FLAG_CONNECTED) &&
                ...
                while ((size = psnet_rel_get (
                            Net_players[idx].reliable_socket, data,
                            MAX_NET_BUFFER)) > 0) {
                    multi_process_bigdata (
                        data, size, &Net_players[idx].p_info.addr, 1);
                }
            }
        }
    }
    else {
        // if I'm not the master of the game, read reliable data from my
        // connection with the server
        if ((Net_player->reliable_socket != INVALID_SOCKET) &&
            (Net_player->reliable_socket != 0)) {
            while ((size = psnet_rel_get (
                        Net_player->reliable_socket, data, MAX_NET_BUFFER)) >
                   0) {
...
DCF (
    eye_tog,
    "Toggles setting of the local player eyepoint on every frame "
    "(Multiplayer)") {
...
void multi_do_frame () {
    ...
    // always set the local player eye position/orientation here so we know its
    // valid throughout all multiplayer function calls
    if ((Net_player != NULL) && eye_tog) {
        camid cid = player_get_cam ();
        if (cid.isValid ()) {
            camera* cam = cid.getCamera ();
            cam->get_info (
                &Net_player->s_info.eye_pos, &Net_player->s_info.eye_orient);
        }
    }
    ...
    // clients should check when last time they heard from sever was -- if too
    // long, then pause the simulation so wait for it to possibly come back
    ...
    if ((Next_ping_time < 0) ||
        ((time (NULL) - Next_ping_time) > PING_SEND_TIME)) {
        if ((Net_player->flags & NETINFO_FLAG_AM_MASTER)) {
    ...
    // if I am the master, and we are not yet actually playing the mission,
    // send off netgame status to all other players in the game.  If I am not
    // the master of the game, and we are not in the game, then send out my
    // netplayer status to the host
    if ((Net_player->flags & NETINFO_FLAG_CONNECTED) &&
        !(Game_mode & GM_IN_MISSION)) {
        if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
            if ((Netgame_send_time < 0) ||
                ((time (NULL) - Netgame_send_time) > NETGAME_SEND_TIME)) {
                send_netgame_update_packet ();
            ...
            if ((State_send_time < 0) ||
                ((time (NULL) - State_send_time) > STATE_SEND_TIME)) {
        ...
        (Net_player->flags & NETINFO_FLAG_CONNECTED) &&
        (Game_mode & GM_IN_MISSION)) {
        // if I am connected and am in the mission, do things that need to be
        // done on a regular basis
        if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
            if ((Gameinfo_send_time < 0) ||
                ((time (NULL) - Gameinfo_send_time) > GAMEINFO_SEND_TIME)) {
    ...
    // check to see if we're waiting on confirmation for a restricted ingame
    // join
    ...
    // while in the mission, send my PlayerControls to the host so that he can
    // process my movement
    if (Game_mode & GM_IN_MISSION) {
        ...
        if (!(Net_player->flags & NETINFO_FLAG_AM_MASTER)) {
            ...
            // bytes received info
            if ((Next_bytes_time < 0) ||
                ((time (NULL) - Next_bytes_time) > BYTES_SENT_TIME)) {
                if (Net_player != NULL) {
            ...
            // sending new objects from here is dependent on having objects
            // only created after the game is done moving the objects.  I think
            // that I can enforce this.
            multi_oo_process ();

            // evaluate whether the time limit has been reached or max kills
            // has been reached Commented out by Sandeep 4/12/98, was causing
            // problems with testing.
            if (((f2fl (Netgame.options.mission_time_limit) > 0.0f) &&
                 (Missiontime > Netgame.options.mission_time_limit)) ||
    ...
    if ((Net_player != NULL) && (Net_player->flags & NETINFO_FLAG_AM_MASTER)) {
        ...
        for (idx = 0; idx < MAX_PLAYERS; idx++) {
            if (MULTI_CONNECTED (Net_players[idx]) &&
                ...
                if ((Multi_client_update_times[idx] < 0) ||
                    timestamp_elapsed_safe (
                        Multi_client_update_times[idx], 1000)) {
                    send_client_update_packet (&Net_players[idx]);

                    Multi_client_update_times[idx] =
                        timestamp (MULTI_CLIENT_UPDATE_TIME);
                }
            }
        }
    }
    ...
    if (Game_mode & GM_STANDALONE_SERVER) { std_do_gui_frame (); }

    // dogfight nonstandalone players should recalc the escort list every frame
    if (!(Game_mode & GM_STANDALONE_SERVER) &&
        (Netgame.type_flags & NG_TYPE_DOGFIGHT) && MULTI_IN_MISSION) {
...
void multi_pause_do_frame () {
    ...
    // always set the local player eye position/orientation here so we know its
    // valid throughout all multiplayer function calls if((Net_player != NULL)
    // && eye_tog){ player_get_eye(&Net_player->s_info.eye_pos,
    ...
    if (MULTIPLAYER_MASTER) { multi_oo_process (); }
...
void standalone_main_init () {
    ...
    // NOTE : these values are not incredibly important since they will be
    // overwritten by the host when he joins
    memset (&Netgame, 0, sizeof (Netgame));
    Netgame.game_state =
        NETGAME_STATE_FORMING; // game is currently starting up
    Netgame.security = 0;
    Netgame.server_addr = Psnet_my_addr;
    ...
    Net_player->flags |=
        (NETINFO_FLAG_AM_MASTER | NETINFO_FLAG_CONNECTED |
         NETINFO_FLAG_DO_NETWORKING | NETINFO_FLAG_MISSION_OK);
...
void standalone_main_do () {
    os_sleep (10); // since nothing will really be going on here, we can afford
                   // to give some time back to the operating system.

    // kind of a do-nothing spin state.
    // The standalone will eventually move into the GS_STATE_MULTI_MISSION_SYNC
    // state when a host connects and attempts to start a game
}
...
void multi_standalone_reset_all () {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        // 6/25/98 -- MWA -- call delete_player here to remove the player. This
        // closes down the socket and marks the player as not connected
        // anymore.  It is probably cleaner to do this.
        if (&Net_players[idx] != Net_player) { delete_player (idx); }
    }
...
void multi_standalone_wait_init () {
    ...
    std_multi_add_goals (); // fill in the goals for the mission into the tree
                            // view
    multi_reset_timestamps ();
...
// --------------------------------------------------------------------------------
// multi_server_wait_do_frame() wait for everyone to log in or the host to send
// commands
//
...
// this is an artificial state which will push the standalone into the main
// state without it having to go through the init function (which would
// disconnect everyone and generally just screw things up) it will also
// eventually do tracker stats update
...
void multi_standalone_postgame_do () {
    ...
    if ((Netgame.game_state != NETGAME_STATE_DEBRIEF) &&
        multi_netplayer_state_check (NETPLAYER_STATE_DEBRIEF, 1)) {
...
void multi_reset_timestamps () {
    ...
    for (i = 0; i < MAX_PLAYERS; i++) { Multi_client_update_times[i] = -1; }
    Netgame_send_time = -1;
    ...
    // reset standalone gui timestamps (these are not game critical, so there
    // is not much danger)
...
void multi_display_netinfo () {
    ...
    if (Player->flags & PLAYER_FLAGS_MSG_MODE) { return; }

    gr_set_color_fast (&Color_normal);

    // server or client
    if (MULTIPLAYER_MASTER) {
        gr_string (sx, sy, "SERVER", GR_RESIZE_NONE);
        sy += dy;
            ...
            if (MULTI_CONNECTED (Net_players[idx]) &&
                ...
                if (Net_players[idx].sv_last_pl < 0) {
                    ...
                    gr_printf_no_resize (
                        sx, sy, "%s: %d, %d pl",
                        Net_players[idx].m_player->callsign,
                        Net_players[idx].sv_bytes_sent,
                        Net_players[idx].sv_last_pl);
                    sy += dy;
                }
            }
        }
        ...
        gr_string (sx, sy, "CLIENT", GR_RESIZE_NONE);
        sy += dy;
