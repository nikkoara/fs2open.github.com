...
#define FS2NETD_DEFAULT_BANNER_URL \
    "http://fs2netd.game-warden.com/files/banners"
...
void fs2netd_options_config_init () {
    if (PXO_options_loaded) { return; }

    if (!strlen (Multi_options_g.game_tracker_ip)) {
        ml_printf (
            "NOTICE: Address for game tracker not specified, using default "
            "instead (%s).",
            FS2NETD_DEFAULT_SERVER);
        strncpy (
            Multi_options_g.game_tracker_ip, FS2NETD_DEFAULT_SERVER,
            MULTI_OPTIONS_STRING_LEN);
    }
    else if (!strcmp ("gt.pxo.net", Multi_options_g.game_tracker_ip)) {
        ml_printf (
            "NOTICE: Incompatible game tracker IP detected (gt.pxo.net), "
            "using default instead (%s)!",
            FS2NETD_DEFAULT_SERVER);
        strncpy (
            Multi_options_g.game_tracker_ip, FS2NETD_DEFAULT_SERVER,
            MULTI_OPTIONS_STRING_LEN);
    ...
    if (!strlen (Multi_options_g.user_tracker_ip)) {
        ml_printf (
            "NOTICE: Address for user tracker not specified, using default "
            "instead (%s).",
            FS2NETD_DEFAULT_SERVER);
        strncpy (
            Multi_options_g.user_tracker_ip, FS2NETD_DEFAULT_SERVER,
            MULTI_OPTIONS_STRING_LEN);
    }
    else if (!strcmp ("ut.pxo.net", Multi_options_g.user_tracker_ip)) {
        ml_printf (
            "NOTICE: Incompatible user tracker IP detected (ut.pxo.net), "
            "using default instead (%s)!",
            FS2NETD_DEFAULT_SERVER);
        strncpy (
            Multi_options_g.user_tracker_ip, FS2NETD_DEFAULT_SERVER,
    ...
    if (!strlen (Multi_options_g.tracker_port)) {
        if (FS2NetD_port >= 1024 && FS2NetD_port <= USHRT_MAX) {
            ml_printf (
                "NOTICE: User override for game/user tracker port not "
                "specified, using game_settings.tbl override (%i).",
                FS2NetD_port);
            int result;
            result =
            ...
            if (FS2NetD_port != 0) {
                ml_printf (
                    "ERROR: game_settings.tbl override for game/user tracker "
                    "port '%i' must be between %i and %i.",
                    FS2NetD_port, 1024, USHRT_MAX);
            }
            ml_printf (
                "NOTICE: Port for game/user trackers not specified, using "
                "default instead (%s).",
                FS2NETD_DEFAULT_PORT);
            strncpy (
                Multi_options_g.tracker_port, FS2NETD_DEFAULT_PORT,
                STD_NAME_LEN);
        }
    }
    else {
        long port_tmp =
            strtol (Multi_options_g.tracker_port, (char**)NULL, 10);

        if ((port_tmp < 1024) || (port_tmp > USHRT_MAX)) {
            ml_printf (
                "NOTICE: The port specified for game/user trackers, '%ld', is "
                "outside of the required range, %i through %i!",
                port_tmp, 1024, USHRT_MAX);
            ml_printf (
                "NOTICE: Port for game/user trackers is invalid, using "
                "default instead (%s).",
                FS2NETD_DEFAULT_PORT);
            strncpy (
                Multi_options_g.tracker_port, FS2NETD_DEFAULT_PORT,
    ...
    if (!strlen (Multi_options_g.pxo_ip)) {
        ml_printf (
            "NOTICE: Address for chat server not specified, using default "
            "instead (%s).",
            FS2NETD_DEFAULT_CHAT_SERVER);
        strncpy (
            Multi_options_g.pxo_ip, FS2NETD_DEFAULT_CHAT_SERVER,
            MULTI_OPTIONS_STRING_LEN);
    }
    else if (!strcmp ("chat.pxo.net", Multi_options_g.pxo_ip)) {
        ml_printf (
            "NOTICE: Incompatible chat server IP detected (chat.pxo.net), "
            "using default instead (%s)!",
            FS2NETD_DEFAULT_CHAT_SERVER);
        strncpy (
            Multi_options_g.pxo_ip, FS2NETD_DEFAULT_CHAT_SERVER,
    ...
    if (!strlen (Multi_options_g.pxo_banner_url)) {
        ml_printf (
            "NOTICE: URL for banners not specified, using default instead "
            "(%s).",
            FS2NETD_DEFAULT_BANNER_URL);
        strncpy (
            Multi_options_g.pxo_banner_url, FS2NETD_DEFAULT_BANNER_URL,
            MULTI_OPTIONS_STRING_LEN);
    }
    else if (!strcmp (
                 "http://www.pxo.net/files/banners",
                 Multi_options_g.pxo_banner_url)) {
        ml_printf (
            "NOTICE: Incompatible banner URL detected (chat.pxo.net), using "
            "default instead (%s)!",
            FS2NETD_DEFAULT_BANNER_URL);
        strncpy (
            Multi_options_g.pxo_banner_url, FS2NETD_DEFAULT_BANNER_URL,
...
bool fs2netd_is_online () { return (Is_connected && Logged_in); }
...
static int fs2netd_connect_do () {
    int retval = FS2NetD_ConnectToServer (
        Multi_options_g.game_tracker_ip, Multi_options_g.tracker_port);

    os_sleep (5);

    switch (retval) {
    // connection failed
    case -1: Is_connected = false; return 2;

    // still trying to connect
    case 0: return 0;

    // connected!
    case 1: Is_connected = true; return 1;
    }
...
void fs2netd_connect () {
    ...
    if (Is_standalone) {
        ...
        popup_till_condition (
            fs2netd_connect_do, XSTR ("&Cancel", 779),
            XSTR ("Connecting into FS2NetD", 1575));
    }
...
int fs2netd_login_do () {
    if (Multi_tracker_id < 0) {
        ...
        if (Local_timeout == -1) { Local_timeout = timer_get_seconds () + 15; }
        ...
        // if we have already been through once then only deal with the recieve
        // packet next time
        ...
        if (Multi_tracker_id >= 0) {
            ml_printf (
                "FS2NetD MSG: Login '%s' is valid, session ID is %d!", user,
                Multi_tracker_id);
            do_full_packet = true;
            Local_timeout = -1;
        }
        ...
        if (Local_timeout == -1) { Local_timeout = timer_get_seconds () + 30; }
        ...
        if (do_full_packet) {
            ml_printf (
                "FS2NetD MSG: Requesting login pilot stats for '%s' ...",
                Players[Player_num].callsign);
        }

        int rescode = FS2NetD_GetPlayerData (
            Players[Player_num].callsign, &Players[Player_num], true,
            do_full_packet);
...
bool fs2netd_login () {
    ...
    // don't bother with this if we aren't on FS2NetD
    if (!Om_tracker_flag) { return false; }

    if (!(Game_mode & GM_MULTIPLAYER)) { return false; }

    if (Logged_in && (Multi_tracker_id >= 0)) { return true; }
    ...
    if (!Is_connected) {
        if (!Is_standalone) {
            popup (
                ...
                XSTR ("Failed to connect to FS2NetD server!", 1578));
        }
        else {
    ...
    if (Is_standalone) {
        ...
        rc = popup_till_condition (
            fs2netd_login_do, XSTR ("&Cancel", 779),
            XSTR ("Logging into FS2NetD", 1579));
    }
    ...
    case 1: {
        ...
        if (strlen (user) == 0) {
            strcpy_s (
                error_str,
                "Login failed! No username supplied. Go to options -> multi "
                "options and add one");
            ...
            strcpy_s (
                error_str,
                "Login failed! No password supplied. Go to options -> multi "
                "options and add one");
    ...
    case 2:
        ...
        strcpy_s (
            error_str,
            "An Unknown Error (probably a timeout) occured when trying to "
            "retrieve your pilot data.");
    ...
    case 5:
        ...
        strcpy_s (
            error_str,
            "Invalid pilot name - A serious error has occured, Contact the "
            "FS2NetD Administrator!");
    ...
    case 6:
        ...
        strcpy_s (
            error_str,
            "Invalid SID - A serious error has occured, Contact the FS2NetD "
            "Administrator!");
    ...
    default:
        ...
        strcpy_s (
            error_str,
            "Unknown return case from GetPlayerData(). Contact the FS2NetD "
            "Administrator!");
        retval = false;
        break;
    }
    ...
    if (Is_standalone) { std_destroy_gen_dialog (); }
...
static void fs2netd_handle_ping () {
    ...
    if ((Ping_timestamp != -1) &&
        ((timer_get_seconds () - Ping_timestamp) > 30)) {
        Ping_timestamp = -1;
        ...
        if (!Is_connected) {
            if (!Is_standalone) {
                ...
                popup (
                    PF_USE_AFFIRMATIVE_ICON | PF_TITLE_BIG | PF_TITLE_RED, 1,
                    POPUP_OK,
            ...
            ml_string (
                "FS2NetD MSG: Will attempt an automatic reconnect to server "
                "in 5 minutes...");
            ...
            ml_string (
                "FS2NetD NOTICE: Connection to server has been "
                "reestablished!");
        }
    }

    // send out a ping after 5 minutes of no other activity to make sure the
    // server is still responding (probably not though)
    if ((Ping_timestamp == -1) &&
        ...
        if (Last_activity < 0) { Last_activity = timer_get_seconds (); }
    }
        ...
        if (FS2NetD_CheckValidID () < 0) {
            ...
            if (!Is_connected) {
                if (!Is_standalone) {
                    gamesnd_play_iface (InterfaceSounds::GENERAL_FAIL);
                    popup (
                        PF_USE_AFFIRMATIVE_ICON | PF_TITLE_BIG | PF_TITLE_RED,
                        1, POPUP_OK,
                        "ERROR:\nLost connection to the FS2NetD server!");
                }
                ...
                ml_string (
                    "FS2NetD MSG: Will attempt an automatic reconnect to "
                    "server in 5 minutes...");
                Login_retry_time = timer_get_seconds () + 300;
...
static void fs2netd_handle_messages () {
    ...
    while (FS2NetD_DataReady () && (bytes_read < (int)sizeof (buffer))) {
        int read_size = FS2NetD_GetData (
            buffer + bytes_read, sizeof (buffer) - bytes_read);
    ...
    if ((bytes_read < BASE_PACKET_SIZE)) { return; }
    ...
    while (buffer_offset + BASE_PACKET_SIZE <= bytes_read) {
        ...
        case PCKT_PONG: {
            ...
            ml_printf (
                "FS2NetD PONG: %d ms", timer_get_milliseconds () - itemp);
        ...
        case PCKT_CHAT_CHAN_COUNT_REPLY: {
            ...
            if ((itemp < 0) || (itemp > USHRT_MAX)) { itemp = 0; }
        ...
        case PCKT_SLIST_REPLY: {
            int numServers = 0;
            int svr_flags __UNUSED; // gcc [-Wunused-but-set-variable] doesn't
                                    // like MACROs
            ushort svr_port;
            char svr_ip[16];
            active_game ag;
...
void fs2netd_do_frame () {
    // in a previous processing loop, so don't do a frame until that has
    // completed
    if (In_process) { return; }

    if (!Logged_in) {
        // maybe try and reconnect, if we were bumped due to a comm error ...
        if ((Login_retry_time != -1) &&
            (timer_get_seconds () >= Login_retry_time)) {
            fs2netd_login ();
    ...
    if (!Is_connected) {
        // The ping handling above made us realize that we are disconnected. We
        // will reconnect in some time so we can just return here
        return;
    }
    ...
    if (Is_connected && !Logged_in) { fs2netd_login (); }
}
...
void fs2netd_gameserver_start () {
    ...
    // already been here
    if (Net_player->flags & NETINFO_FLAG_MT_CONNECTED) { return; }
    ...
    if (strlen (Multi_fs_tracker_channel)) {
        strcpy_s (
            Multi_tracker_game_data.chat_channel, Multi_fs_tracker_channel);
    }
...
void fs2netd_gameserver_update (bool force) {
    ...
    if (!force && (timer_get_seconds () < Next_gameserver_update)) { return; }
...
int fs2netd_check_mission_do () {
    ...
    int rescode = FS2NetD_CheckSingleMission (
        Chk_mission_name, Chk_mission_crc, do_full_packet);
...
bool fs2netd_check_mission (char* mission_name) {
    ...
    if (Is_standalone) {
        ...
        rc = popup_till_condition (
            fs2netd_check_mission_do, XSTR ("&Cancel", 779), popup_string);
    }
    ...
    case 3:
        if (!Is_standalone) {
            popup (
                PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                XSTR ("Server request failed!", 1580));
        }
    ...
    case 4:
        if (!Is_standalone) {
            popup (
                PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                XSTR ("Server request timed out!", 1581));
        }
...
static int fs2netd_send_player_do () {
    ...
    int rescode = FS2NetD_SendPlayerData (
        Players[Player_num].callsign, &Players[Player_num], do_full_packet);
...
static int fs2netd_send_player () {
    ...
    if (Is_standalone) {
        ...
        rc = popup_till_condition (
            fs2netd_send_player_do, XSTR ("&Cancel", 779),
...
static void fs2netd_store_stats_results () {
    ...
    multi_display_chat_msg (
        XSTR ("<PXO stats store process complete>", 1001), 0, 0);
    ml_string (XSTR ("<PXO stats store process complete>", 1001));

    if (Multi_debrief_stats_accept_code != 1) {
        sprintf (
            str, XSTR ("<PXO stats store failed for player %s>", 1002),
            Net_player->m_player->callsign);
...
void fs2netd_store_stats () {
    ...
    if (Duplicate_login_detected) {
        Duplicate_login_detected = false;
        multi_display_chat_msg (
            XSTR ("<Duplicate login detected - stats have been tossed>", 1582),
            0, 0);
        ml_string (XSTR (
            "<Duplicate login detected - stats have been tossed>", 1582));
    ...
    if (game_hacked_data ()) {
        multi_display_chat_msg (
            XSTR ("<Hacked tables detected - stats have been tossed>", 1584),
            0, 0);
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            XSTR (
                "You are playing with a hacked tables, your stats will not be "
                "saved",
                1585));
        fs2netd_store_stats_results ();
        return;
    ...
    if ((multi_num_players () <= 1) && (Multi_num_players_at_start <= 1)) {
        multi_display_chat_msg (
            XSTR (
                "<Not enough players were present at game start or end, stats "
                "will not be saved>",
                1048),
            0, 0);
        ml_string (XSTR (
    ...
    /*
       ...
       !MULTI_STANDALONE(Net_players[idx]) && (Net_players[idx].flags &
       NETINFO_FLAG_HAXOR) ) { multi_display_chat_msg( XSTR("<Connected player
       has hacked info - tossing invalid stats>", -1), 0, 0 ); return;
            }
        }
    */
    if (!fs2netd_check_mission (Netgame.mission_name)) {
        multi_display_chat_msg (
            XSTR (
                "<Server detected a non PXO validated mission. Stats will not "
                "be saved>",
                1049),
            0, 0);
        popup (
            PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
            XSTR (
                "This is not a PXO validated mission, your stats will not be "
                "saved",
                1050));
        fs2netd_store_stats_results ();
        return;
    ...
    switch (spd_ret) { // 0 = pilot updated, 1  = invalid pilot, 2 = invalid
                       // (expired?) sid
    case -1: ml_string ("<stats have been tossed - server error>"); break;
    ...
    case 1: ml_string ("<stats have been tossed - pilot error>"); break;
    ...
    default:
        multi_display_chat_msg (
            XSTR ("Unknown Stats Store Request Reply", 1586), 0, 0);
        break;
    }
...
void fs2netd_update_ban_list () {
    ...
    if (Is_standalone) {
        ...
        rc = popup_till_condition (
            fs2netd_update_ban_list_do, XSTR ("&Cancel", 779),
            XSTR ("Requesting IP ban list", 1587));
    }
    ...
    if (!FS2NetD_ban_list.empty ()) {
        CFILE* banlist_cfg =
            cfopen ("banlist.cfg", "wt", CFILE_NORMAL, CF_TYPE_DATA);

        if (banlist_cfg != NULL) {
            for (std::vector< std::string >::iterator bl =
                     FS2NetD_ban_list.begin ();
                 bl != FS2NetD_ban_list.end (); ++bl) {
...
bool fs2netd_player_banned (net_addr* addr) {
    ...
    bool retval = false;
    CFILE* banlist_cfg =
        cfopen ("banlist.cfg", "rt", CFILE_NORMAL, CF_TYPE_DATA);

    if (banlist_cfg == NULL) { return false; }
...
int fs2netd_get_valid_missions_do () {
    ...
    // we should have the CRCs, or there were no missions, so process them
        ...
        if (file_names == NULL) {
            ...
            file_names = (char**)vm_malloc (
                sizeof (char*) * 1024,
                memory::quiet_alloc); // 1024 files should be safe!
            ...
            idx = count = cf_get_file_list (
                1024, file_names, CF_TYPE_MISSIONS, wild_card);
        }
        ...
        // NOTE: that this is done for one file per frame, since this is inside
        // of a popup
        ...
        if (Is_standalone) {
            if (std_gen_is_active ()) { std_gen_set_text (val_text, 1); }
        }
        else {
        ...
        if (file_index >= 0) {
            ...
            if (found) {
                ...
                ml_printf (
                    "FS2NetD Mission Validation: %s  =>  INVALID! -- 0x%08x",
                    full_name, checksum);
            }
        }
    }
...
bool fs2netd_get_valid_missions () {
    ...
    if (Is_standalone) {
        ...
        rc = popup_till_condition (
            fs2netd_get_valid_missions_do, XSTR ("&Cancel", 779),
            XSTR ("Starting mission validation", 1588));
    }
    ...
    case 1:
        if (!Is_standalone) {
            popup (
                PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                XSTR ("Mission validation timed out!", 1589));
        }
    ...
    case 2:
        if (!Is_standalone) {
            popup (
                PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                XSTR (
                    "No missions are available from the server for "
                    "validation!",
                    1590));
        }
    ...
    case 3:
        if (!Is_standalone) {
            popup (
                PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                XSTR ("Memory error during mission validation!", 1591));
        }
...
int fs2netd_update_valid_tables_do () {
    ...
    case -1: Local_timeout = -1; return 2;

    // timeout
    case 1: Local_timeout = -1; return 1;

    // done!
    case 2: Local_timeout = -1; return 3;
    }
...
int fs2netd_update_valid_tables () {
    ...
    if (Table_valid_status.empty ()) { return -1; }
    ...
    if (Is_standalone) {
        ...
        rc = popup_till_condition (
            fs2netd_update_valid_tables_do, XSTR ("&Cancel", 779),
            XSTR ("Starting table validation", 1592));
    }
    ...
    case 1: {
        if (!Is_standalone) {
            popup (
                PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                XSTR ("Table validation timed out!", 1593));
        }
    ...
    case 2: {
        if (!Is_standalone) {
            popup (
                PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                XSTR (
                    "No tables are available from the server for validation!",
                    1594));
        }
    ...
    for (std::vector< crc_valid_status >::iterator tvs =
        ...
        if (tvs->valid) {
            ...
            ml_printf (
                "FS2NetD Table Check: '%s' -- INVALID (0x%x)!", tvs->name,
                tvs->crc32);
            hacked = 1;
        }
    }
...
void fs2netd_add_table_validation (const char* tblname) {
    ...
    if ((tblname == NULL) || !strlen (tblname)) { return; }
...
int fs2netd_get_pilot_info (
    const char* callsign, player* out_plr, bool first_call) {
    if (!Logged_in) { return -2; }
...
void fs2netd_update_game_count (const char* chan_name) {
    ...
    if ((chan_name == NULL) || !strlen (chan_name)) { return; }
...
void fs2netd_spew_table_checksums (const char* outfile) {
    ...
    if (Table_valid_status.empty ()) { return; }
    ...
    for (std::vector< crc_valid_status >::iterator tvs =
        ...
        if (offset < sizeof (filename)) { filename[offset] = '\0'; }
        else {
