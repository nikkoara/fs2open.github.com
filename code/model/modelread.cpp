...
flag_def_list model_render_flags[] = {
    ...
    { "no Zbuffer", MR_NO_ZBUFFER, 0 },   { "no cull", MR_NO_CULL, 0 },
    { "no glowmaps", MR_NO_GLOWMAPS, 0 }, { "force clamp", MR_FORCE_CLAMP, 0 },
};

int model_render_flags_size =
    sizeof (model_render_flags) / sizeof (flag_def_list);

#define MAX_SUBMODEL_COLLISION_ROT_ANGLE \
    (PI / 6.0f) // max 30 degrees per frame
...
// NOTE: Each subsystem must match up with its #define, or there will be
// problems
const char* Subsystem_types[SUBSYSTEM_MAX] = {
    "None",         "Engines",        "Turrets",    "Radar",
    "Navigation",   "Communications", "Weapons",    "Sensors",
    "Solar panels", "Gas collection", "Activation", "Unknown"
};
...
flag_def_list Dock_type_names[] = { { "cargo", DOCK_TYPE_CARGO, 0 },
...
// Goober5000 - reimplementation of Bobboau's $dumb_rotation feature in a way
// that works with the rest of the model instance system note: since these data
// types are only ever used in this file, they don't need to be in model.h
...
// With the basic page in system this can be called from outside of
// modelread.cpp
void model_unload (int modelnum, int force) {
    ...
    if ((num < 0) || (num >= MAX_POLYGON_MODELS)) { return; }
    ...
    // we want to call bm_release() from here rather than just bm_unload() in
    ...
    if (pm->submodel) {
        for (i = 0; i < pm->n_models; i++) {
            ...
            if (pm->submodel[i].collision_tree_index >= 0) {
                model_remove_bsp_collision_tree (
    ...
    if (pm->vert_source.Vbuffer_handle > -1) {
        gr_heap_deallocate (
            GpuHeap::ModelVertex, pm->vert_source.Vertex_offset);
    ...
    // run through Ship_info and if the model has been loaded we'll need to
    // reset the modelnum to -1.
    for (auto it = Ship_info.begin (); it != Ship_info.end (); ++it) {
        if (pm->id == it->model_num) { it->model_num = -1; }

        if (pm->id == it->cockpit_model_num) { it->cockpit_model_num = -1; }

        if (pm->id == it->model_num_hud) { it->model_num_hud = -1; }
    }
...
void model_instance_free_all () {
    ...
    for (i = 0; i < Polygon_model_instances.size (); ++i) {
        if (Polygon_model_instances[i]) { model_delete_instance ((int)i); }
    }

    // clear skybox model instance if we have one; it is not an object and
    // therefore has no <object>_delete function which would remove the
    // instance
...
void model_page_in_stop () {
    ...
    for (i = 0; i < MAX_POLYGON_MODELS; i++) {
        ...
        if (Polygon_models[i]->used_this_mission) continue;
...
void model_init () {
    ...
    for (i = 0; i < MAX_POLYGON_MODELS; i++) { Polygon_models[i] = NULL; }
...
// if p is not null, sets p to the next character AFTER the string and a
// space/equals/colon (not the beginning of the string, as strstr would)
// returns the index of the property found, or -1 if not found
// NB: the first recognized option is returned, so if one option is a substring
// of another, put it later in the list!
...
int prop_string (
    char* props, char** p, const char* option0, const char* option1) {
...
const Model::Subsystem_Flags carry_flags[] = {
    Model::Subsystem_Flags::Crewpoint, Model::Subsystem_Flags::Rotates,
    Model::Subsystem_Flags::Triggered, Model::Subsystem_Flags::Artillery,
    Model::Subsystem_Flags::Stepped_rotate
};
// funciton to copy model data from one subsystem set to another subsystem set.
// This function is called when two ships use the same model data, but since
// the model only gets read in one time, the subsystem data is only present in
// one location.  The ship code will call this routine to fix this situation by
// copying stuff from the source subsystem set to the dest subsystem set.
void model_copy_subsystems (
    int n_subsystems, model_subsystem* d_sp, model_subsystem* s_sp) {
    int i, j;
...
static void set_subsystem_info (
    int model_num, model_subsystem* subsystemp, char* props, char* dname) {
    char* p;
    char buf[64];
    char lcdname[256];
    int idx;
    ...
    // Rotating subsystem
        ...
        (idx = prop_string (
             props, &p, "$rotate_time", "$rotate_rate", "$rotate")) >= 0) {
        subsystemp->flags.set (Model::Subsystem_Flags::Rotates);

        // get value for (a) complete rotation (b) step (c) activation
        get_user_prop_value (
            p, buf); // note: p points to the value since we used prop_string
        ...
        if ((strstr (props, "$stepped")) != NULL) {
            ...
            subsystemp->stepped_rotation->max_turn_rate =
                PI2 / ((1.0f - fraction) * num_steps * t_trans);
        }
...
float get_submodel_delta_angle (submodel_instance_info* sii) {
    ...
    if (delta_angle > PI) { delta_angle = (PI2 - delta_angle); }

    return delta_angle;
}

void do_new_subsystem (
    int n_subsystems, model_subsystem* slist, int subobj_num, float rad,
    vec3d* pnt, char* props, char* subobj_name, int model_num) {
    int i;
    ...
    if (slist == NULL) {
        ...
        if (!ss_warning_shown) {
            mprintf (
                ("No subsystems found for model \"%s\".\n",
    ...
    // try to find the name of the subsystem passed here on the list of
    // subsystems currently on the ship.  Assign the values only when the right
    // subsystem is found
    ...
    for (i = 0; i < n_subsystems; i++) {
        ...
        if (!subsystem_stricmp (subobj_name, subsystemp->subobj_name)) {
            ...
            subsystemp->pnt = *pnt; // use the offset to get the center point
                                    // of the subsystem
    ...
    if (!ss_warning_shown) {
        ...
        Warning (
            ...
            model_get (model_num)->filename);
        mprintf (
            ("Subsystem %s in model %s was not found in ships.tbl!\n",
             subobj_name, model_get (model_num)->filename));
        ss_warning_shown = 1;
    }
    else
    ...
    if (ss_fp) {
        _splitpath (model_filename, NULL, NULL, bname, NULL);
        mprintf (
            ("A subsystem was found in model %s that does not have a record "
             "in ships.tbl.\nA list of subsystems for this ship will be "
...
void print_family_tree (
    polymodel* obj, int modelnum, const char* ident, int islast) {
    char temp[50];
...
void create_vertex_buffer (polymodel* pm) {
    ...
    size_t stride = 0;
    // Determine the global stride of this model (should be the same for every
    // submodel)
    for (i = 0; i < pm->n_models; ++i) {
    ...
    for (i = 0; i < pm->n_detail_levels; ++i) {
        if (pm->detail_buffers[i].model_list == NULL) { continue; }
...
int read_model_file (
    polymodel* pm, const char* filename, int n_subsystems,
    model_subsystem* subsystems, int ferror) {
    CFILE* fp;
    int version;
...
//				mprintf(( "Num models = %d\n", pm->n_models ));
            ...
            if (pm->version >= 1903) {
                if (pm->version >= 2009) {
                    ...
                    // old code where mass wasn't based on area, so do the
                ...
                // a custom MOI is only used for ships, but we should probably
                // log it anyway
        ...
        case ID_SOBJ: { // Subobject header
            ...
            cfread_string_len (
                pm->submodel[n].name, MAX_NAME_LEN, fp); // get the name
            cfread_string_len (
                props, MAX_PROP_LEN, fp); // and the user properties
            ...
            if ((p = strstr (props, "$look_at")) != NULL) {
                ...
                pm->submodel[n].look_at_num =
                    -2; // Set this to -2 to mark it as something we need to
                        // work out the correct subobject number for later,
                        // after all subobjects have been processed
            }
            else {
                pm->submodel[n].look_at_num = -1; // No look_at
            }

            // note, this should come BEFORE do_new_subsystem() for proper
            // error handling (to avoid both rotating and dumb-rotating
            // submodel)
            int idx = prop_string (
                props, &p, "$dumb_rotate_time", "$dumb_rotate_rate",
                "$dumb_rotate");
            if (idx >= 0) {
                ...
                // for past SCP compatibility, $dumb_rotate means
                // $dumb_rotate_rate
                float turn_rate;
                if (idx == 0) {
                    float turn_time = static_cast< float > (atof (buf));
            ...
            // Sets can_move on submodels which are of a rotating type or which
            // have such a parent somewhere down the hierarchy
            if ((pm->submodel[n].movement_type != MOVEMENT_TYPE_NONE) ||
                strstr (props, "$triggered") || strstr (props, "$rotate") ||
                strstr (props, "$gun_rotation")) {
                pm->submodel[n].can_move = true;
            }
            else if (
                pm->submodel[n].parent >= 0 &&
                pm->submodel[pm->submodel[n].parent].can_move) {
                pm->submodel[n].can_move = true;
            }
            ...
            if ((p = strstr (props, "$special")) != NULL) {
                ...
                if (!stricmp (type, "subsystem")) { // if we have a subsystem,
                    ...
                    do_new_subsystem (
                        ...
                        &pm->submodel[n].offset, props, pm->submodel[n].name,
                        pm->id);
            ...
            */
            ...
            if ((p = strstr (props, "$detail_box:")) != NULL) {
                ...
                if ((p = strstr (props, "$box_offset:")) != NULL) {
                    p += 12;
                    while (*p == ' ') p++;
                    pm->submodel[n].render_box_offset.xyz.x =
                        (float)strtod (p, (char**)NULL);
                    while (*p != ',') p++;
                    pm->submodel[n].render_box_offset.xyz.y =
                        (float)strtod (++p, (char**)NULL);
                    while (*p != ',') p++;
                    pm->submodel[n].render_box_offset.xyz.z =
                        (float)strtod (++p, (char**)NULL);
                ...
                if ((p = strstr (props, "$box_min:")) != NULL) {
                    p += 9;
                    while (*p == ' ') p++;
                    pm->submodel[n].render_box_min.xyz.x =
                        (float)strtod (p, (char**)NULL);
                    while (*p != ',') p++;
                    pm->submodel[n].render_box_min.xyz.y =
                        (float)strtod (++p, (char**)NULL);
                    while (*p != ',') p++;
                    pm->submodel[n].render_box_min.xyz.z =
                        (float)strtod (++p, (char**)NULL);
                }
                else {
                ...
                if ((p = strstr (props, "$box_max:")) != NULL) {
                    p += 9;
                    while (*p == ' ') p++;
                    pm->submodel[n].render_box_max.xyz.x =
                        (float)strtod (p, (char**)NULL);
                    while (*p != ',') p++;
                    pm->submodel[n].render_box_max.xyz.y =
                        (float)strtod (++p, (char**)NULL);
                    while (*p != ',') p++;
                    pm->submodel[n].render_box_max.xyz.z =
                        (float)strtod (++p, (char**)NULL);
                }
                else {
            ...
            if ((p = strstr (props, "$detail_sphere:")) != NULL) {
                ...
                if ((p = strstr (props, "$radius:")) != NULL) {
                    p += 8;
                    while (*p == ' ') p++;
                    pm->submodel[n].render_sphere_radius =
                        (float)strtod (p, (char**)NULL);
                }
                else {
                ...
                if ((p = strstr (props, "$offset:")) != NULL) {
                    p += 8;
                    while (*p == ' ') p++;
                    pm->submodel[n].render_sphere_offset.xyz.x =
                        (float)strtod (p, (char**)NULL);
                    while (*p != ',') p++;
                    pm->submodel[n].render_sphere_offset.xyz.y =
                        (float)strtod (++p, (char**)NULL);
                    while (*p != ',') p++;
                    pm->submodel[n].render_sphere_offset.xyz.z =
                        (float)strtod (++p, (char**)NULL);
            ...
            if ((p = strstr (props, "$uvec:")) != NULL) {
                ...
                orient->vec.uvec.xyz.x =
                    (float)(strtod (parsed_string, (char**)NULL));
                ...
                if ((p = strstr (props, "$fvec:")) != NULL) {
                    ...
                    vm_vec_cross (
                        &orient->vec.rvec, &orient->vec.uvec,
                        &orient->vec.fvec);
                    vm_vec_cross (
                        &orient->vec.fvec, &orient->vec.rvec,
                        &orient->vec.uvec);
            ...
            if (pm->submodel[n].is_thruster &&
                !(pm->submodel[n].no_collisions) &&
                !(pm->submodel[n].nocollide_this_only) &&
                !(pm->submodel[n].collide_invisible)) {
                pm->submodel[n].nocollide_this_only = true;
            }
            ...
            // pm->submodel[n].name, pm->submodel[n].parent )); key_getch();

            // mprintf(( "Submodel %d, tree offset %d\n", n,
            // pm->submodel[n].tree_offset )); mprintf(( "Submodel %d, data
            // offset %d\n", n, pm->submodel[n].data_offset )); key_getch();
                ...
                for (i = 0; i < pm->shield.ntris; i++) {
                    ...
                    for (j = 0; j < 3; j++) {
                        pm->shield.tris[i].verts[j] =
                            cfread_int (fp); // read in the indices into the
                                             // shield_vertex list
#ifndef NDEBUG
                    ...
                    for (j = 0; j < 3; j++) {
                        pm->shield.tris[i].neighbors[j] =
                            cfread_int (fp); // read in the neighbor indices --
                                             // indexes into tri list
#ifndef NDEBUG
            ...
            if (pm->n_docks > 0) {
                ...
                for (i = 0; i < pm->n_docks; i++) {
                    ...
                    if (!strnicmp (bay->name, "cargo", 5))
                        ...
                        bay->type_flags =
                            (DOCK_TYPE_REARM | DOCK_TYPE_GENERIC);
        ...
        case ID_GLOW: // start glow point reading -Bobboau
        {
            ...
            for (int gpb = 0; gpb < gpb_num; gpb++) {
                ...
                if (length > 0) {
                    ...
                    // niffiwan: no "props" string found - ensure we don't have
        ...
        case ID_FUEL:
            ...
            if (pm->n_thrusters > 0) {
                ...
                for (i = 0; i < pm->n_thrusters; i++) {
                    ...
                    if (pm->version < 2117) { bank->wash_info_pointer = NULL; }
                    else {
                        cfread_string_len (props, MAX_PROP_LEN, fp);
                        ...
                        if (length > 0) {
                            ...
                            for (int k = 0; k < n_subsystems; k++) {
                                if (!subsystem_stricmp (
                                        ...
                                        engine_subsys_name)) {
                                    bank->submodel_num =
                                        subsystems[k].subobj_num;

                                    bank->wash_info_pointer =
                                        subsystems[k].engine_wash_pointer;
                                    if (bank->wash_info_pointer != NULL) {
                                        table_error = 0;
                                    }
                                    // also set what subsystem this is attached
                                    // to but not if we only have one thruster
                                    // bank do this so that original :V: models
        ...
        case ID_TMIS: {
            ...
            for (i = 0; i < n_banks; i++) {
                ...
                int physical_parent; // The subobj that the firepoints are
                                     // physically attached to (the gun barrel)
                ...
                if (subsystems) {
                    for (snum = 0; snum < n_subsystems; snum++) {
                        ...
                        if (parent == subsystemp->subobj_num) {
                            ...
                            vm_vector_2_matrix (
                                &subsystemp->turret_matrix,
                                &subsystemp->turret_norm, NULL, NULL);
                            ...
                            for (j = 0; j < n_slots; j++) {
                                if (j < MAX_TFP)
                                    cfread_vector (
                                        &subsystemp->turret_firing_point[j],
                                        fp);
                                else {
                                    vec3d bogus;
        ...
        case ID_SPCL: {
            ...
            for (i = 0; i < n_specials; i++) {
                // get the next free object of the subobject list.  Flag error
                // if no more room
                ...
                if (p != NULL) {
                    ...
                    if (!stricmp (
                            type, "subsystem")) { // if we have a subsystem,
                                                  // put it into the list!
                        do_new_subsystem (
                            n_subsystems, subsystems, -1, radius, &pnt,
                            props_spcl, &name[1],
                    ...
                    strstr (name, "$enginelarge") ||
                    strstr (name, "$enginehuge")) {
                    do_new_subsystem (
                        n_subsystems, subsystems, -1, radius, &pnt, props_spcl,
                        &name[1],
        ...
        case ID_PATH:
            ...
            for (i = 0; i < pm->n_paths; i++) {
                ...
                if (pm->version >= 2002) {
                    ...
                    cfread_string_len (
                        pm->paths[i].parent_name, MAX_NAME_LEN - 1, fp);
...
void model_load_texture (polymodel* pm, int i, char* file) {
    ...
    if (strstr (tmp_name, "thruster") || strstr (tmp_name, "invisible") ||
        strstr (tmp_name, "warpmap")) {
        // Don't load textures for thruster animations or invisible textures
        // or warp models!-Bobboau
        tbase->clear ();
    }
    else {
        // check if we should be transparent, include "-trans" but make sure to
        // skip anything that might be "-transport"
        if ((strstr (tmp_name, "-trans") && !strstr (tmp_name, "-transpo")) ||
            strstr (tmp_name, "shockwave") ||
            !strcmp (tmp_name, "nameplate")) {
            tmap->is_transparent = true;
        }

        if (strstr (tmp_name, "-amb")) { tmap->is_ambient = true; }
    ...
    if (tbase->GetTexture () > 0) shader_flags |= SDR_FLAG_MODEL_DIFFUSE_MAP;
    ...
    if ((tspec->GetTexture () > 0 || tspecgloss->GetTexture () > 0) &&
        Cmdline_env && Cmdline_spec) // No env maps without spec map
        shader_flags |= SDR_FLAG_MODEL_ENV_MAP;
    if (tmisc->GetTexture () > 0) shader_flags |= SDR_FLAG_MODEL_MISC_MAP;
    ...
    gr_maybe_create_shader (
        SDR_TYPE_MODEL,
        shader_flags | SDR_FLAG_MODEL_LIGHT | SDR_FLAG_MODEL_ANIMATED);
    gr_maybe_create_shader (
        SDR_TYPE_MODEL, shader_flags | SDR_FLAG_MODEL_LIGHT |
                            SDR_FLAG_MODEL_ANIMATED | SDR_FLAG_MODEL_FOG);
    ...
    gr_maybe_create_shader (
        SDR_TYPE_MODEL,
        shader_flags | SDR_FLAG_MODEL_LIGHT | SDR_FLAG_MODEL_FOG);
...
int model_load (
    const char* filename, int n_subsystems, model_subsystem* subsystems,
    int ferror, int duplicate) {
    int i, num, arc_idx;
    polymodel* pm = NULL;

    if (!model_initted) model_init ();
    ...
    if (org_sig + MAX_POLYGON_MODELS > INT_MAX ||
        org_sig + MAX_POLYGON_MODELS < org_sig) {
    ...
    for (i = 0; i < pm->n_models; i++) {
        ...
        pm->submodel[i].i_replace =
            -1; // assume this doesn't replaces anything
    }
    ...
    for (i = 0; i < pm->n_models; i++) {
        ...
        // This debris comes from a destroyed subsystem when ship is still
        // alive
    ...
    for (i = 0; i < pm->n_models; i++) {
        ...
        if (sm1->lod_name[0] != '\0') { l1 = strlen (sm1->lod_name); }
        ...
        for (j = 0; j < pm->num_debris_objects; j++) {
            if (i == pm->debris_objects[j]) { sm1->is_damaged = 1; }
        }

        for (j = 0; j < MAX_MODEL_DETAIL_LEVELS; j++) { sm1->details[j] = -1; }
        ...
        for (j = 0; j < pm->n_models; j++) {
            ...
            // if sm2 is a detail of sm1 and sm1 is a high detail, then add it
            // to sm1's list
            ...
            if (ndiff == 1) { // They only differ by one character!
                ...
                // Handle LODs named "detail0/1/2/etc" too (as opposed to
                // "detaila/b/c/etc")
                if (sm1->parent == -1 && sm2->parent == -1 &&
                    !sm1->is_damaged && !sm2->is_damaged &&
                    !sm1->is_live_debris && !sm2->is_live_debris) {
                    dl2 = dl2 - dl1;
                    dl1 = 0;
                }

                if ((dl1 < 0) || (dl2 < 0) ||
                    (dl1 >= MAX_MODEL_DETAIL_LEVELS) ||
        ...
        for (j = 0; j < sm1->num_details; j++) {
            if (sm1->details[j] == -1) { sm1->num_details = 0; }
        }
    }
    ...
    if (!Cmdline_old_collision_sys) {
        ...
        for (i = 0; i < pm->n_models; ++i) {
            if (!(pm->submodel[i].nocollide_this_only ||
                  pm->submodel[i].no_collisions)) {
                pm->submodel[i].collision_tree_index =
                    model_create_bsp_collision_tree ();
                bsp_collision_tree* tree = model_get_bsp_collision_tree (
                    pm->submodel[i].collision_tree_index);
                model_collide_parse_bsp (
                    tree, pm->submodel[i].bsp_data, pm->version);
            }
        }
    }
    ...
    for (i = 0; i < pm->n_view_positions; i++) {
        if (pm->view_positions[i].parent == pm->detail[0]) {
            ...
            if (d > pm->core_radius) { pm->core_radius = d; }
        }
    }
...
int model_create_instance (bool is_ship, int model_num) {
    ...
    for (i = 0; i < (int)Polygon_model_instances.size (); i++) {
        if (!Polygon_model_instances[i]) { open_slot = i; }
    }
    ...
    if (pm->flags & PM_FLAG_HAS_INTRINSIC_ROTATE) {
        ...
        for (i = 0; i < pm->n_models; i++) {
            if (pm->submodel[i].movement_type ==
                MOVEMENT_TYPE_INTRINSIC_ROTATE) {
                intrinsic_rotate.list.push_back (submodel_intrinsic_rotation (
                    i, pm->submodel[i].dumb_turn_rate));
            }
        }
...
// fill in the path_num field inside the model_subsystem struct.  This is an
// index into the pm->paths[] array, which is a path that provides a frontal
...
void model_set_subsys_path_nums (
    ...
    for (i = 0; i < n_subsystems; i++) subsystems[i].path_num = -1;
    ...
    for (i = 0; i < n_subsystems; i++) {
        ...
        if (subsystems[i].path_num == -1) subsystems[i].path_num = -2;
    }
}

// Determine the path indices (indicies into pm->paths[]) for the paths used
// for approaching/departing a fighter bay on a capital ship.
void model_set_bay_path_nums (polymodel* pm) {
    int i;
    ...
    */
    ...
    // initializations
    pm->ship_bay->arrive_flags = 0; // bitfield, set to 1 when that path number
                                    // is reserved for an arrival
    pm->ship_bay->depart_flags = 0; // bitfield, set to 1 when that path number
                                    // is reserved for a departure
...
int model_get_parent_submodel_for_live_debris (
    ...
    // Check all its children until we find the submodel to which the live
    // debris belongs
    ...
    while (mn > 0) {
        ...
        if (child->num_live_debris > 0) {
            ...
            // Error( LOCATION, "Could not find parent submodel for live
            // debris.  Possible model error");
        }
...
polymodel* model_get (int model_num) {
    ...
    if (num < 0 || num > MAX_POLYGON_MODELS || !Polygon_models[num] ||
        Polygon_models[num]->id != model_num)
        return NULL;
...
int model_find_2d_bound_min (
    int model_num, matrix* orient, vec3d* pos, int* x1, int* y1, int* x2,
    int* y2) {
    polymodel* po;
    int n_valid_pts;
...
// returns 1 for invalid model, 2 for point offscreen.
// note that x1,y1,x2,y2 aren't clipped to 2d screen coordinates!
int submodel_find_2d_bound_min (
    int model_num, int submodel, matrix* orient, vec3d* pos, int* x1, int* y1,
    int* x2, int* y2) {
    polymodel* po;
    int n_valid_pts;
    int i, x, y, min_x, min_y, max_x, max_y;
...
int model_find_2d_bound (
    ...
    if (pnt.flags & CC_BEHIND) return 2;

    if (!(pnt.flags & PF_PROJECTED)) g3_project_vertex (&pnt);

    if (pnt.flags & PF_OVERFLOW) return 2;
 ...
 * @return 2 for point offscreen
 */
int subobj_find_2d_bound (
    float radius, matrix* /*orient*/, vec3d* pos, int* x1, int* y1, int* x2,
    int* y2) {
    float t, w, h;
    vertex pnt;
...
void model_find_obj_dir (
    vec3d* w_vec, vec3d* m_vec, int model_num, int submodel_num,
    matrix* objorient) {
    vec3d tvec, vec;
    matrix m;
    int mn;
...
void model_instance_find_obj_dir (
    vec3d* w_vec, vec3d* m_vec, int model_instance_num, int submodel_num,
    matrix* objorient) {
    vec3d tvec, vec;
    matrix m;
    int mn;
...
void model_get_rotating_submodel_axis (
    vec3d* model_axis, vec3d* world_axis, int model_instance_num,
    int submodel_num, matrix* objorient) {
    polymodel_instance* pmi = model_get_instance (model_instance_num);
    polymodel* pm = model_get (pmi->model_num);
    ...
    model_instance_find_obj_dir (
        world_axis, model_axis, model_instance_num, submodel_num, objorient);
}
...
void submodel_stepped_rotate (
    ...
    // just to make sure this issue wont pop up again... might cause odd
    // jerking in some extremely odd situations but given that those issues
    // would require the timer to be reseted in any case it probably wont hurt
    float rotation_time;
    ...
    if ((ang_prev == NULL) && (ang_next == NULL)) return;
    ...
    float step_time =
        psub->stepped_rotation->t_transit + psub->stepped_rotation->t_pause;
    ...
    int cur_step =
        (int)std::lround ((rotation_time - step_offset_time) / step_time);
    // mprintf(("cur step %d\n", cur_step));
    ...
    if (cur_step >= psub->stepped_rotation->num_steps) {
        ...
        // sii->step_zero_timestamp += int(1000.0f *
        // (psub->stepped_rotation->num_steps * step_time) + 0.5f);
    ...
    *ang_next = cur_step * step_size;
    ...
    float coast_start_time =
        psub->stepped_rotation->fraction * psub->stepped_rotation->t_transit;
    ...
    if (step_offset_time < coast_start_time) {
        ...
        *ang_next += 0.5f * psub->stepped_rotation->max_turn_accel *
                     accel_time * accel_time;
        sii->cur_turn_rate =
            psub->stepped_rotation->max_turn_accel * accel_time;
    }
        ...
        *ang_next += start_coast_angle +
        ...
        // do decel
        float time_to_pause =
            psub->stepped_rotation->t_transit - step_offset_time;
        *ang_next +=
            (step_size - 0.5f * psub->stepped_rotation->max_turn_accel *
                             time_to_pause * time_to_pause);
        sii->cur_turn_rate =
            psub->stepped_rotation->max_turn_rate * time_to_pause;
    }
    else {
        // do pause
        *ang_next += step_size;
...
void world_find_real_model_point (
    vec3d* out, vec3d* world_pt, polymodel* pm, int submodel_num,
...
void submodel_look_at (polymodel* pm, int mn) {
    ...
    // VA - Run this bit only once for each look_at enabled submodel, to
    // correctly associate the name given in the $look_at: property with the
    // number of that named subobject
    if (sm->look_at_num == -2) {
    ...
    model_find_world_point (
        &mp, &vmd_zero_vector, pmn, sm->look_at_num, &vmd_identity_matrix,
        &vmd_zero_vector);
    world_find_real_model_point (
        &other, &mp, pm, mn, &vmd_identity_matrix, &vmd_zero_vector);
    ...
    if (dot >= 0.0f) { *a = asinf (c.a1d[axis]); }
    else {
...
// Rotates the angle of a submodel, when the submodel has a subsystem (which is
...
// Rotates the angle of a submodel.  If the submodel has a subsystem, the
// execution flow should first go through the other submodel_rotate function
// before this one.  (This function is called directly in the case of
// dumb_rotation.)
void submodel_rotate (bsp_info* sm, submodel_instance_info* sii) {
    ...
    case MOVEMENT_AXIS_X:
        ...
        while (sii->angs.p > PI2) sii->angs.p -= PI2;
        while (sii->angs.p < 0.0f) sii->angs.p += PI2;

        break;
    case MOVEMENT_AXIS_Y:
        sii->angs.h += delta;

        while (sii->angs.h > PI2) sii->angs.h -= PI2;
        while (sii->angs.h < 0.0f) sii->angs.h += PI2;

        break;
    case MOVEMENT_AXIS_Z:
        sii->angs.b += delta;

        while (sii->angs.b > PI2) sii->angs.b -= PI2;
        while (sii->angs.b < 0.0f) sii->angs.b += PI2;
...
psub->ai_rotation.max; return; } else if(sii->angs.p + delta <
psub->ai_rotation.min){ sii->angs.p = psub->ai_rotation.min; return;
        }
        sii->angs.p += delta;
        if (sii->angs.p > PI2 )
            sii->angs.p -= PI2;
...
void model_make_turret_matrix (int model_num, model_subsystem* turret) {
    ...
    model_find_world_dir (
        &fvec, &turret->turret_norm, model_num, turret->turret_gun_sobj,
        &vmd_identity_matrix);
    ...
    gun->angs.h = offset_barrel_h;
    model_find_world_dir (
        &rvec, &turret->turret_norm, model_num, turret->turret_gun_sobj,
        &vmd_identity_matrix);
    ...
    gun->angs.h = offset_barrel_h;
    model_find_world_dir (
        &uvec, &turret->turret_norm, model_num, turret->turret_gun_sobj,
        &vmd_identity_matrix);
...
int model_rotate_gun (
    ...
    vec3d of_dst;
    matrix
        world_to_turret_matrix; // converts world coordinates to turret's FOR
    vec3d world_to_turret_translate; // converts world coordinates to turret's
                                     // FOR
    vec3d tempv;
    ...
    if (turret->flags[Model::Subsystem_Flags::Turret_alt_math])
        ...
        vm_matrix_x_matrix (
            &world_to_turret_matrix, orient, &turret->turret_matrix);
    ...
    base_delta = vm_interp_angle (
        &base_angles->h, desired_angles.h, step_size, limited_base_rotation);
    ...
    if (turret->turret_gun_rotation_snd.isValid ()) {
        if (step_size > 0) {
            ...
            if (gun_delta > 1.0f) gun_delta = 1.0f;
    ...
    if (turret->flags[Model::Subsystem_Flags::Fire_on_target]) {
        base_delta =
            vm_delta_from_interp_angle (base_angles->h, desired_angles.h);
        gun_delta =
            vm_delta_from_interp_angle (gun_angles->p, desired_angles.p);
...
void model_find_submodel_offset (
    vec3d* outpnt, int model_num, int sub_model_num) {
    int mn;
    polymodel* pm = model_get (model_num);
...
void model_find_world_point (
    vec3d* outpnt, vec3d* mpnt, int model_num, int submodel_num,
    const matrix* objorient, const vec3d* objpos) {
    vec3d pnt;
    vec3d tpnt;
    matrix m;
    int mn;
...
void model_instance_find_world_point (
    vec3d* outpnt, vec3d* mpnt, int model_instance_num, int submodel_num,
    const matrix* objorient, const vec3d* objpos) {
    vec3d pnt;
    vec3d tpnt;
    matrix m;
...
// pm - polygon model
// submodel_num - submodel in whose RF we're trying to find the corresponding
// world point orient - orient matrix of ship pos - pos vector of ship
void world_find_model_point (
    ...
    vm_rotate_matrix_by_angles (
        &rotation_matrix, &pm->submodel[submodel_num].angs);

    matrix inv_orientation;
    vm_copy_transpose (
        &inv_orientation, &pm->submodel[submodel_num].orientation);
...
void world_find_model_instance_point (
    vec3d* out, vec3d* world_pt, const polymodel_instance* pmi,
    ...
    matrix rotation_matrix = pm->submodel[submodel_num].orientation;
    vm_rotate_matrix_by_angles (
        &rotation_matrix, &pmi->submodel[submodel_num].angs);
 ...
 */
void find_submodel_instance_point (
    vec3d* outpnt, int model_instance_num, int submodel_num) {
    vm_vec_zero (outpnt);
    ...
    while ((mn >= 0) && (pm->submodel[mn].parent >= 0)) {
        ...
        if (pm->submodel[parent_mn].can_move) {
            ...
            vm_rotate_matrix_by_angles (
                &rotation_matrix, &pmi->submodel[parent_mn].angs);

            vm_copy_transpose (
                &inv_orientation, &pm->submodel[parent_mn].orientation);

            vm_matrix_x_matrix (
                &submodel_instance_matrix, &rotation_matrix, &inv_orientation);
...
/**
 * Finds the current location and rotation (in the ship's frame of reference)
 * of a submodel point, taking into account the rotations of the submodel and
 * any parent submodels it might have.
 *
 * @param *outpnt Output point
 * @param *outnorm Output normal
 * @param model_instance_num Index into Polygon_model_instances
 * @param submodel_num The number of the submodel we're interested in
 * @param *submodel_pnt The point which's current position we want, in the
 * submodel's frame of reference
 * @param *submodel_norm The normal which's current direction we want, in the
 * ship's frame of reference
 */
void find_submodel_instance_point_normal (
    vec3d* outpnt, vec3d* outnorm, int model_instance_num, int submodel_num,
    const vec3d* submodel_pnt, const vec3d* submodel_norm) {
    *outnorm = *submodel_norm;
    ...
    while ((mn >= 0) && (pm->submodel[mn].parent >= 0)) {
        ...
        if (mn == submodel_num) {
            ...
            vm_vec_unrotate (
                &submodel_pnt_offset, &tvec, &submodel_instance_matrix);
        ...
        vm_rotate_matrix_by_angles (
            &rotation_matrix, &pmi->submodel[parent_model_num].angs);

        vm_copy_transpose (
            &inv_orientation, &pm->submodel[parent_model_num].orientation);
 ...
 * @param *submodel_pnt The point which's current position we want, in the
 * submodel's frame of reference
 * @param *submodel_orient The local matrix which's current orientation in the
 * ship's frame of reference we want
 */
void find_submodel_instance_point_orient (
    vec3d* outpnt, matrix* outorient, int model_instance_num, int submodel_num,
    const vec3d* submodel_pnt, const matrix* submodel_orient) {
 ...
 * @param submodel_num The number of the submodel we're interested in
 */
void find_submodel_instance_world_point (
    vec3d* outpnt, int model_instance_num, int submodel_num,
    const matrix* objorient, const vec3d* objpos) {
    vec3d loc_pnt;
...
// Verify rotating submodel has corresponding ship subsystem -- info in which
// to store rotation angle
 ...
 * 2) Are currently rotating (i.e. actually moving and not part of the
 * superstructure due to being destroyed or replaced) 3) Are not rotating too
 * far for collision detection (c.f. MAX_SUBMODEL_COLLISION_ROT_ANGLE)
 */
void model_get_rotating_submodel_list (
    ...
    if (objp->type == OBJ_SHIP) {
        ...
        if (model_instance_num < 0) { return; }
        model_num =
            Weapon_info[Weapons[objp->instance].weapon_info_index].model_num;
    }
    else if (objp->type == OBJ_ASTEROID) {
        model_instance_num = Asteroids[objp->instance].model_instance_num;
        if (model_instance_num < 0) { return; }
        model_num = Asteroid_info[Asteroids[objp->instance].asteroid_type]
                        .model_num[Asteroids[objp->instance].asteroid_subtype];
    }
    else {
        return;
    }
    ...
    if (child_submodel->no_collisions) { // if detail0 has $no_collision set
                                         // dont check childs
        return;
    }
    ...
    while (i >= 0) {
        ...
        // Don't check it or its children if it is destroyed or it is a
        // replacement (non-moving)
        if (!child_submodel->blown_off && (child_submodel->i_replace == -1) &&
            !child_submodel->no_collisions &&
            !child_submodel->nocollide_this_only) {
            // Only look for submodels that rotate or intrinsic-rotate
            if (child_submodel->movement_type == MOVEMENT_TYPE_ROT ||
                child_submodel->movement_type ==
                    MOVEMENT_TYPE_INTRINSIC_ROTATE) {
                ...
                if (sii != nullptr) {
                    // found the correct submodel instance - now check delta
                    // rotation angle not too large
...
void model_get_submodel_tree_list (
    std::vector< int >& submodel_vector, polymodel* pm, int mn) {
...
// reference, and given the object's orient and position,
// return the point in 3-space in outpnt.
void model_find_world_dir (
    vec3d* out_dir, vec3d* in_dir, int model_num, int submodel_num,
...
// model_find_world_dir
void model_instance_find_world_dir (
    vec3d* out_dir, vec3d* in_dir, int model_instance_num, int submodel_num,
    const matrix* objorient) {
    vec3d pnt;
    vec3d tpnt;
    matrix m;
...
void model_clear_instance (int model_num) {
    ...
    for (i = 0; i < pm->n_textures; i++) { pm->maps[i].ResetToOriginal (); }

    for (i = 0; i < pm->n_models; i++) {
        bsp_info* sm = &pm->submodel[i];

        if (pm->submodel[i].is_damaged) { sm->blown_off = 1; }
        else {
            sm->blown_off = 0;
        }
        sm->angs.p = 0.0f;
    ...
    for (i = 0; i < pm->num_lights; i++) { pm->lights[i].value = 0.0f; }
...
void model_clear_submodel_instance (
    submodel_instance* sm_instance, bsp_info* sm) {
...
// initialization during ship set
void model_set_instance_info (
    submodel_instance_info* sii, float turn_rate, float turn_accel) {
    sii->blown_off = 0;
    sii->angs.p = 0.0f;
    sii->angs.b = 0.0f;
    sii->angs.h = 0.0f;
...
void model_set_instance (
    int model_num, int sub_model_num, submodel_instance_info* sii,
    ...
    if (flags != NULL) { instance_flags = *flags; }
    ...
    if ((sm->blown_off) &&
        (!(instance_flags[Ship::Subsystem_Flags::No_replace]))) {
        ...
        // If submodel isn't yet blown off and has a -destroyed replacement
        // model, we prevent the replacement model from being drawn by marking
        // it as having been blown off
...
// instead of using larger but largely unused structures for storing the same
// data
void model_set_instance_techroom (
    int model_num, int sub_model_num, float angle_1, float angle_2) {
    polymodel* pm;
    ...
    // If submodel isn't yet blown off and has a -destroyed replacement model,
    // we prevent the replacement model from being drawn by marking it as
...
void model_update_instance (
    int model_instance_num, int sub_model_num, submodel_instance_info* sii,
    flagset< Ship::Subsystem_Flags >& flags) {
    int i;
    polymodel* pm;
    polymodel_instance* pmi;
    ...
    // Set the "blown out" flags
    if (flags[Ship::Subsystem_Flags::No_disappear]) { smi->blown_off = false; }
    else {
...
void model_do_intrinsic_rotations_sub (intrinsic_rotation* ir) {
    ...
    for (auto submodel_it = ir->list.begin (); submodel_it != ir->list.end ();
        ...
        // (Since this is an intrinsic rotation, we have no associated
        // subsystem, so pass 0 for subsystem flags.)
        flagset< Ship::Subsystem_Flags > empty;
        model_update_instance (
            ir->model_instance_num, submodel_it->submodel_num,
            &submodel_it->submodel_info_1, empty);
    }
}

// Handle the intrinsic rotations for either a) a single ship model; or b) all
// non-ship models.  The reason for the two cases is that
// ship_model_update_instance will be called for each ship via
// obj_move_all_post, but we also need to handle non-ship models once
// obj_move_all_post exits.  Since the two processes are almost identical, they
// are both handled here.
//
// This function is quite a bit different than Bobboau's old
// model_do_dumb_rotation function.  Whereas Bobboau used the brute-force
// technique of navigating through each model hierarchy as it was rendered,
// this function should be seen as a version of obj_move_all_post, but for
// models rather than objects.  In fact, the only reason for the special ship
// case is that the ship intrinsic rotations kind of need to be handled where
// all the other ship rotations are.  (Unless you want inconsistent collisions
        ...
        for (auto intrinsic_it = Intrinsic_rotations.begin ();
            ...
            if (intrinsic_it->model_instance_num == model_instance_num) {
                Assertion (
                    ...
                    "model_do_intrinsic_rotations function!");

                // we're just doing one ship, and in
                // ship_model_update_instance, that ship's angles were already
                // set to zero
    ...
    // we are handling all non-ships
        ...
        for (auto intrinsic_it = Intrinsic_rotations.begin ();
            ...
            if (!intrinsic_it->is_ship) {
                // Just as in ship_model_update_instance: first clear all the
                // angles in the model to zero
                model_clear_submodel_instances (
                    intrinsic_it->model_instance_num);
...
// Finds a point on the rotation axis of a submodel, used in collision,
// generally find rotational velocity
void model_init_submodel_axis_pt (
    submodel_instance_info* sii, int model_num, int submodel_num) {
    vec3d axis;
    ...
    // these will be rotated to about the axis an angle of 0 and PI and we'll
    // find the intersection of the two lines to find a point on the axis
    if (pm->submodel[submodel_num].movement_axis == MOVEMENT_AXIS_X) {
        ...
        axis = vmd_z_vector; // rotation about y is a change in heading
                             // (p,b,h), so we need z
        mpoint2 = &vmd_z_vector;
    }
        ...
        axis = vmd_y_vector; // rotation about z is a change in bank (p,b,h),
                             // so we need y
    }
    else {
    ...
    model_find_world_point (
        &p1, mpoint1, model_num, submodel_num, &vmd_identity_matrix,
        &vmd_zero_vector);
    model_find_world_point (
        &p2, mpoint2, model_num, submodel_num, &vmd_identity_matrix,
        &vmd_zero_vector);

    // find two points rotated into model RF when angs set to PI
    vm_vec_copy_scale ((vec3d*)&pm->submodel[submodel_num].angs, &axis, PI);
    model_find_world_point (
        &v1, mpoint1, model_num, submodel_num, &vmd_identity_matrix,
        &vmd_zero_vector);
    model_find_world_point (
        &v2, mpoint2, model_num, submodel_num, &vmd_identity_matrix,
        &vmd_zero_vector);
...
void model_add_arc (
    int model_num, int sub_model_num, vec3d* v1, vec3d* v2, int arc_type) {
    polymodel* pm;
...
// function to return an index into the docking_bays array which matches the
// criteria passed to this function.  dock_type is one of the DOCK_TYPE_XXX
// defines in model.h Goober5000 - now finds more than one dockpoint of this
// type
int model_find_dock_index (
    int modelnum, int dock_type, int index_to_start_at) {
    int i;
    polymodel* pm;

    // get model and make sure it has dockpoints
    pm = model_get (modelnum);
    if (pm->n_docks <= 0) return -1;

    // look for a dockpoint of this type
    for (i = index_to_start_at; i < pm->n_docks; i++) {
        if (dock_type & pm->docking_bays[i].type_flags) return i;
    }
...
// function to return an index into the docking_bays array which matches the
// string passed Fred uses strings to identify docking positions.  This
// function also accepts generic strings so that a desginer doesn't have to
// know exact names if building a mission from hand.
...
int model_get_dock_types (int modelnum) {
    ...
    pm = model_get (modelnum);
    for (i = 0; i < pm->n_docks; i++) type |= pm->docking_bays[i].type_flags;
...
void swap_bsp_tmappoly (polymodel* pm, ubyte* p) {
    ...
    if (pm->version < 2003) {
        ...
        *vp (p + 20) = center_point;
        ...
        for (i = 0; i < nv; i++) {
            float dist =
                vm_vec_dist (&center_point, Interp_verts[verts[i].vertnum]);
...
void swap_bsp_flatpoly (polymodel* pm, ubyte* p) {
    ...
    verts = (short*)(p + 44);
    for (i = 0; i < nv * 2; i++) { verts[i] = INTEL_SHORT (verts[i]); }
    ...
    if (pm->version < 2003) {
        ...
        *vp (p + 20) = center_point;
        ...
        for (i = 0; i < nv; i++) {
            float dist =
                vm_vec_dist (&center_point, Interp_verts[verts[i * 2]]);
            if (dist > rad) { rad = dist; }
        }
        fl (p + 32) = rad;
    }
...
void swap_bsp_data (polymodel* /*pm*/, void* /*model_ptr*/) {
    ...
    while (chunk_type != OP_EOF) {
        switch (chunk_type) {
        case OP_EOF: return;
        case OP_DEFPOINTS: swap_bsp_defpoints (p); break;
        case OP_FLATPOLY: swap_bsp_flatpoly (pm, p); break;
        case OP_TMAPPOLY: swap_bsp_tmappoly (pm, p); break;
        case OP_SORTNORM: swap_bsp_sortnorms (pm, p); break;
        case OP_BOUNDBOX:
            min = vp (p + 8);
            max = vp (p + 20);
...
void parse_glowpoint_table (const char* filename) {
    try {
        ...
        if (!optional_string ("#Glowpoint overrides")) { return; }
        ...
        while (!required_string_either ("$Name:", "#End")) {
            ...
            if (optional_string ("$On:")) { stuff_boolean (&gpo.is_on); }
            ...
            if (optional_string ("$Texture:")) {
                ...
                if (stricmp (glow_texture_name, "none") != 0) {
                    ...
                    if (gpo.glow_bitmap < 0) {
                        ...
                        nprintf ((
                            "Model", "Glowpoint preset %s texture num is %d\n",
            ...
            if (optional_string ("+light")) {
                ...
                if (optional_string ("+lightcone")) {
                    ...
                    if (optional_string ("$Cone angle:")) {
                        ...
                        gpo.cone_inner_angle = cosf (
                            (gpo.cone_angle - ((gpo.cone_angle < 20.0f)
                                                   ? gpo.cone_angle * 0.5f
                                                   : 20.0f)) /
                            180.0f * PI);
            ...
            if (!skip) {
                std::vector< glow_point_bank_override >::iterator gpoi =
                    get_glowpoint_bank_override_by_name (gpo.name);
                if (gpoi == glowpoint_bank_overrides.end ()) {
                    if (!replace) { glowpoint_bank_overrides.push_back (gpo); }
                }
                else {
                    if (!replace) {
                        Warning (
...
void model_subsystem::reset () {
    ...
    for (auto it = std::begin (turret_matrix.a1d);
         it != std::end (turret_matrix.a1d); ++it)
        *it = 0.0f;
    ...
    for (auto it = std::begin (turret_firing_point);
         it != std::end (turret_firing_point); ++it)
    ...
    for (auto it = std::begin (primary_banks); it != std::end (primary_banks);
         ++it)
        *it = 0;
    for (auto it = std::begin (primary_bank_capacity);
         it != std::end (primary_bank_capacity); ++it)
        *it = 0;
    for (auto it = std::begin (secondary_banks);
         it != std::end (secondary_banks); ++it)
        *it = 0;
    for (auto it = std::begin (secondary_bank_capacity);
         it != std::end (secondary_bank_capacity); ++it)
        *it = 0;
    ...
    for (auto it = std::begin (target_priority);
         it != std::end (target_priority); ++it)
        *it = 0;
