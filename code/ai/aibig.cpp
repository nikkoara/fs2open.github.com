...
#define SCAN_FIGHTERS_INTERVAL \
    2000 // how often an AI fighter/bomber should scan for enemy
         // fighter/bombers if sitting still and pounding on a big ship.  If
         // enemy fighters are close ( < ENTER_STRAFE_THREAT_DIST ), then enter
         // AIM_STRAFE
...
#define STRAFE_RETREAT_COLLIDE_DIST \
    100 // when perpendicular distance to *surface* is less than this, begin
        // retreat
#define STRAFE_RETREAT_BOX_DIST \
    300 // distance beyond the bounding box to retreat
#define STRAFE_MAX_UNHIT_TIME \
    20 // Maximum amount of time to stay in strafe mode if not hit
...
#define GLIDE_STRAFE_DISTANCE \
    50.0f // Distance from the ship to pass when glide strafing
#define GLIDE_STRAFE_MIN_TIME \
    2 // Minimum amount of time to stay on one glide strafe approach vector
#define GLIDE_STRAFE_MAX_TIME \
    15 // Maximum amount of time for each glide strafe pass

// forward declarations
void ai_big_evade_ship ();
void ai_big_chase_attack (
    ai_info* aip, ship_info* sip, vec3d* enemy_pos, float dist_to_enemy,
    int modelnum);
...
extern int model_which_octant_distant_many (
    vec3d* pnt, int model_num, matrix* model_orient, vec3d* model_pos,
    polymodel** pm, int* octs);
extern void
compute_desired_rvec (vec3d* rvec, vec3d* goal_pos, vec3d* cur_pos);
extern void big_ship_collide_recover_start (
    object* objp, object* big_objp, vec3d* collision_normal);

//	Called by ai_big_pick_attack_point.
//	Generates a random attack point.
//	If truly_random flag set (haha), then generate a pretty random number.
//Otherwise, generate a static rand which 	tends to not change from frame to
//frame. 	Try four times and choose nearest point to increase chance of getting
//a good point.
void ai_bpap (
    object* objp, vec3d* attacker_objp_pos, vec3d* attacker_objp_fvec,
    vec3d* attack_point, vec3d* local_attack_point, float fov,
    float weapon_travel_dist, vec3d* surface_normal, ship_subsys* ss) {
    float nearest_dist;
    ...
    if (ss != NULL) tp = ss->system_info;

    best_point = objp->pos;
    nearest_dist = weapon_travel_dist;

    model_which_octant_distant_many (
        attacker_objp_pos, sip->model_num, &objp->orient, &objp->pos, &pm,
        octs);

    num_tries =
        (int)(vm_vec_dist (&objp->pos, attacker_objp_pos) / objp->radius);
    ...
    if (pm->octants[octs[0]].verts) {
        *local_attack_point =
            *pm->octants[octs[0]]
                 .verts[0]; //	Set just in case it doesn't get set below.
    }
    else {
    ...
    for (q = 0; q < 4; q++) {
        ...
        if (octp->nverts > 0) {
            if (num_tries > octp->nverts) num_tries = octp->nverts;
            ...
            for (i = 0; i < num_tries; i++) {
                ...
                dist = vm_vec_normalized_dir (
                    &v2p, &result_point, attacker_objp_pos);
                bool in_fov = false;

                dot = vm_vec_dot (&v2p, attacker_objp_fvec);
                if (tp == NULL) {
                    if (dot > fov) in_fov = true;
                }
                else {
                ...
                if (in_fov) {
                    if (dist < nearest_dist) {
                        ...
                        *local_attack_point = rel_point;
                        ...
                        if (dot > (1.0f + fov) /
                                      2.0f) //	If this point is quite good,
                                            //quit searching for a better one.
                            goto done_1;
                    }
                }
            }
...
done_1:
    ...
    *attack_point = best_point;
    ...
    // Cast from attack_objp_pos to local_attack_pos and check for nearest
    // collision. If no collision, cast to (0,0,0) [center of big ship]**
    // [best_point initialized to 000]

    // do in world coords to get attack point, then translate to local for
    // local_attack_point
    vec3d attack_dir, end_point, temp;
    float dist;
    dist =
        vm_vec_normalized_dir (&attack_dir, attack_point, attacker_objp_pos);
    ...
    if (mc.num_hits > 0) {
        ...
        *attack_point = objp->pos;
        if (surface_normal) { vm_vec_zero (surface_normal); }
    }
}
...
//	If fov != 1.0f, try up to four times to find a point that's in the field of
//view.
void ai_big_pick_attack_point_turret (
    object* objp, ship_subsys* ssp, vec3d* gpos, vec3d* gvec,
    vec3d* attack_point, float fov, float weapon_travel_dist) {
    if (!timestamp_elapsed (ssp->turret_pick_big_attack_point_timestamp)) {
        vec3d result_point;
        vm_vec_unrotate (
            &result_point, &ssp->turret_big_attack_point, &objp->orient);
...
//	Note: Default value for fov is 1.0f  1.0f means don't use fov parameter.
//	If fov != 1.0f, try up to four times to find a point that's in the field of
//view. 	Note, attacker_objp can be a ship or a weapon.
void ai_big_pick_attack_point (
    object* objp, object* attacker_objp, vec3d* attack_point, float fov) {
    ...
    case OBJ_SHIP: {
        ...
        if (!timestamp_elapsed (
            ...
            vm_vec_unrotate (
                &result_point, &attacker_aip->big_attack_point, &objp->orient);
            vm_vec_add (attack_point, &result_point, &objp->pos);
    ...
    case OBJ_WEAPON: {
        ...
        if (!timestamp_elapsed (wp->pick_big_attack_point_timestamp)) {
            ...
            vm_vec_unrotate (
                &result_point, &wp->big_attack_point, &objp->orient);
            vm_vec_add (attack_point, &result_point, &objp->pos);
    ...
    ai_bpap (
        objp, &attacker_objp->pos, &attacker_objp->orient.vec.fvec,
        attack_point, &local_attack_point, fov, 99999.9f, &surface_normal,
        NULL);

    switch (attacker_objp->type) {
    case OBJ_SHIP: {
        ai_info* attacker_aip;
        // if we can't find a new local_attack_point don't change
        // local_attack_point
        if (vm_vec_mag_squared (&local_attack_point) < 1) { return; }
...
void ai_big_evade_ship () {
    ...
    box_dist = get_world_closest_box_point_with_delta (
        &box_vec, En_objp, &player_pos, &is_inside, EVADE_BOX_BASE_DISTANCE);
    if (box_dist > EVADE_BOX_BASE_DISTANCE) {
        aip->submode = SM_ATTACK;
        aip->submode_start_time = Missiontime;
    }
    else if (
        (box_dist > EVADE_BOX_MIN_DISTANCE) &&
...
void ai_big_avoid_ship () { ai_big_evade_ship (); }
...
//										subsystem is within certain field of view.  We don't want
//to check fov when 										strafing, since ship is weaving to avoid turret fire
int ai_big_maybe_follow_subsys_path (int do_dot_check) {
    ...
    if (aip->target_objnum < 0) return 0;

    target_objp = &Objects[aip->target_objnum];

    if ((aip->targeted_subsys != NULL) &&
        (aip->targeted_subsys->system_info->path_num >= 0)) {
        int subsys_path_num, subsys_in_sight, checked_sight;
        float dist;

        // Get models of both source and target
        polymodel* pm = model_get (
            Ship_info[Ships[Pl_objp->instance].ship_info_index].model_num);
        polymodel* pm_t = model_get (
            Ship_info[Ships[target_objp->instance].ship_info_index].model_num);

        // Necessary sanity check
        Assertion (
            aip->targeted_subsys->system_info->path_num < pm_t->n_paths,
            "Invalid Path number %d for subsystem %s on ship %s (Model: %s)\n",
            aip->targeted_subsys->system_info->path_num,
            aip->targeted_subsys->system_info->name,
            Ship_info[Ships[target_objp->instance].ship_info_index].name,
            pm_t->filename);
        if (aip->targeted_subsys->system_info->path_num >= pm_t->n_paths)
            return 0;

        // If attacking a subsystem, ensure that we have an unobstructed line
        // of sight... if not, then move towards path linked to subsystem
        subsys_in_sight =
            0; // assume Pl_objp doesn't have line of sight to subys
        ...
        if (timestamp_elapsed (aip->path_subsystem_next_check)) {
            ...
            ep = &(pm->view_positions[Ships[Pl_objp->instance]
                                          .current_viewpoint]);
            model_find_world_point (
                &geye, &ep->pnt, pm->id, 0, &Pl_objp->orient, &Pl_objp->pos);

            // get world pos of subsystem
            vm_vec_unrotate (
                &gsubpos, &aip->targeted_subsys->system_info->pnt,
                &En_objp->orient);
            vm_vec_add2 (&gsubpos, &En_objp->pos);

            checked_sight = 1;

            // Calc dot between subsys normal (based on using path info), and
            // subsys_to_eye vector.  This is useful later when we want to
            // decide whether we have a realistic line-of-sight to the
            // subsystem.
            vec3d subsys_normal, subsys_to_eye;
            if (do_dot_check) {
                if (!ship_return_subsys_path_normal (
                        &Ships[target_objp->instance], aip->targeted_subsys,
                        &gsubpos, &subsys_normal)) {
            ...
            if (ship_subsystem_in_sight (
                    En_objp, aip->targeted_subsys, &geye, &gsubpos, 1)) {
                subsys_in_sight = 1;
            }
        }
        ...
        min_dot =
            (target_objp->phys_info.fspeed > 5.0f
                 ? MIN_DOT_TO_ATTACK_MOVING_SUBSYS
        ...
        if ((checked_sight && ((!subsys_in_sight) || (dot < min_dot)))) {
            ...
            if (((aip->path_start) == -1 ||
                 (aip->mp_index != subsys_path_num)) &&
                subsys_path_num < pm_t->n_paths) {
                // maybe create a new path
                if (subsys_path_num >= 0) {
                    Assert (aip->target_objnum >= 0);
                    ai_find_path (
                        Pl_objp, aip->target_objnum, subsys_path_num, 0, 1);
                    if (aip->path_start >= 0) {
        ...
        // If there is a path that we are tracking, see if ship has gotten to
        // within aip->path_goal_dist units.  If so, then ship can stop
        // following the path.  This is required since we don't want to follow
        // the path all the way to the subsystem, and we want ships to stop
        // different distances from their path destination
        if (aip->path_length > 0) {
            ...
            // if we've reached the destination, then we can stop following the
            // path
            if (path_done || (dist < aip->path_goal_dist) || in_view) {
                ...
                if (aip->path_start != -1) {
                    ...
                    if (aip->path_cur <
                        (aip->path_start + aip->path_length - 1)) {
...
// This function is only called from ai_big_chase_attack() when a ship is
// flying very slowly and attacking a big ship.  The ship should scan for enemy
// fighter/bombers... if any are close, then return 1, otherwise return 0;
//
// input: aip	=>	ai_info pointer for Pl_objp
//			 sip	=>	ship_info pointer for Pl_objp
//
// exit:		1	=>	ship should enter strafe mode
//				0	=> ship should not change ai mode, no fighter/bomber threats are
//near
//
...
int ai_big_maybe_start_strafe (ai_info* aip, ship_info* sip) {
    // if moving slowly (or stopped), and SIF_SMALL_SHIP, then enter STRAFE
    // mode if enemy fighter/bombers are near
    if (sip->is_small_ship ()) {
        if (timestamp_elapsed (aip->scan_for_enemy_timestamp)) {
            ...
            // iterate through ships, and see if any fighter/bomber from
            // opposite team are near
            ...
            while (so != END_OF_LIST (&Ship_obj_list)) {
                ...
                if (iff_x_attacks_y (
                        Ships[Pl_objp->instance].team, test_sp->team)) {
                    if (Ship_info[test_sp->ship_info_index].is_small_ship ()) {
                        dist_squared = vm_vec_dist_squared (
                            &Pl_objp->pos, &test_objp->pos);
...
void ai_big_chase_attack (
    ...
    if (((time_to_hit = ai_endangered_by_weapon (aip)) < 4.0f) &&
        (time_to_hit >= 0.0f)) {
        ...
        if (Pl_objp->phys_info.speed < 3.0f) {
            ...
            for (objp = GET_FIRST (&obj_used_list);
                ...
                if ((objp->type == OBJ_WEAPON) &&
                    ...
                    if (Weapon_info[Weapons[objp->instance].weapon_info_index]
                        ...
                        if (vm_vec_dot (&in_vec, &objp->orient.vec.fvec) >
                            0.0f) {
                            dist = vm_vec_normalize (&in_vec);
                            if ((dist < 200.0f) &&
                                (vm_vec_dot (&in_vec, &objp->orient.vec.fvec) >
                                 0.95f)) {
                                if ((Objects[objp->parent].signature ==
                                     objp->parent_sig) &&
                                    (vm_vec_dist_quick (
                                         &objp->pos,
                                         &Objects[objp->parent].pos) <
                                     300.0f)) {
            ...
            // Since ship is moving slowly and attacking a large ship, scan if
            // enemy fighters are near, if so then enter strafe mode
            if (ai_big_maybe_start_strafe (aip, sip)) {
                aip->previous_mode = aip->mode;
                aip->mode = AIM_STRAFE;
                aip->submode_parm0 =
                    Missiontime; // use parm0 as time strafe mode entered (i.e.
                                 // MODE start time)
        ...
        // Maybe enter glide strafe (check every 8 seconds, on a different
        ...
        if ((sip->can_glide == true) &&
            ...
            aip->submode_parm0 = Missiontime; // use parm0 as time strafe mode
                                              // entered (i.e. MODE start time)
            aip->submode = AIS_STRAFE_GLIDE_ATTACK;
            aip->submode_parm1 = 0;
            aip->submode_start_time = Missiontime;
        }

        // see if Pl_objp needs to reposition to get a good shot at subsystem
        ...
        dist_to_enemy =
            vm_vec_normalized_dir (&vec_to_enemy, enemy_pos, &Pl_objp->pos);
        ...
        ai_turn_towards_vector (
            enemy_pos, Pl_objp, sip->srotation_time, nullptr, rel_pos, 0.0f, 0,
            rvec);

        // calc range of primary weapon
        weapon_travel_dist =
            ai_get_weapon_dist (&Ships[Pl_objp->instance].weapons);
        ...
        if (aip->targeted_subsys != NULL) {
            if (dist_to_enemy > (weapon_travel_dist - 20)) {
                ...
                if ((aip->ai_flags[AI::AI_Flags::Free_afterburner_use]) &&
                    ...
                    (Pl_objp->phys_info.flags & PF_AFTERBURNER_ON) &&
                    !(aip->ai_flags[AI::AI_Flags::Kamikaze])) {
                    afterburners_stop (Pl_objp);
                }
            }
            else {
                // AL 12-31-97: Move at least as quickly as your target is
                // moving...
                accelerate_ship (
                    aip, MAX (
                             1.0f - dot_to_enemy,
                             Objects[aip->target_objnum].phys_info.fspeed /
                                 sip->max_speed));
            ...
            float accel;
            if (dot_to_enemy < 0.0f) { accel = 0.5f; }
...
void ai_big_chase_ct () { ai_chase_ct (); }

extern void ai_select_secondary_weapon (
    object* objp, ship_weapon* swp, int priority1 = -1, int priority2 = -1);
extern float set_secondary_fire_delay (
    ai_info* aip, ship* shipp, weapon_info* swip, bool burst);
extern void
ai_choose_secondary_weapon (object* objp, ai_info* aip, object* en_objp);
extern int maybe_avoid_big_ship (
    object* objp, object* ignore_objp, ai_info* aip, vec3d* goal_point,
    float delta_time);

extern void maybe_cheat_fire_synaptic (object* objp);

// Determine if Pl_objp should fire weapons at current target, based on input
// parameters
//
// dist_to_enemy	=>		distance (in m) to attack point on current target
// dot_to_enemy	=>		dot product between fvec of Pl_objp and vector from Pl_objp
// to attack point
//
static void
ai_big_maybe_fire_weapons (float dist_to_enemy, float dot_to_enemy) {
    ai_info* aip;
    ship_weapon* swp;
    int has_fired = -1;
    ...
    if (dot_to_enemy >
        ...
        //forward vector with vector to ship 	and also the size of the target
        //relative to distance to target.
        if (dot_to_enemy >
            MAX (
            ...
            if (tswp->num_primary_banks > 0) {
                Assertion (
                    ...
                    "AI tried to select primary bank %d. Might be a model "
                    "error\n",
            ...
            if (tswp->num_secondary_banks > 0) {
                if (!(En_objp->flags[Object::Object_Flags::Protected]) ||
                    ...
                    if (current_bank > -1) {
                        weapon_info* swip =
                            &Weapon_info
                                [tswp->secondary_bank_weapons[current_bank]];
                        ...
                        if (!(En_objp
                            ...
                            ((aip->goals[0].ai_mode &
                             ...
                             swip->wi_flags
                                 [Weapon::Info_Flags::
                                      Puncture])) { // override lockdown on
                                                    // protected ships when
                                                    // using anti subsystem
                                                    // weapons - Valathil
                            //	If ship is protected and very low on hits,
                            //don't fire missiles.
                            if (!(En_objp->flags
                                      [Object::Object_Flags::Protected]) ||
                                (En_objp->hull_strength > 10 * swip->damage)) {
                                if (aip->ai_flags
                                ...
                                if (timestamp_elapsed (
                                    ...
                                    if (swip->wi_flags
                                        ...
                                        firing_range = MIN (
                                            ...
                                            swip->weapon_range);
                                    // reduce firing range of secondaries in
                                    // nebula
                                    ...
                                    float t = 0.25f; //	default delay in
                                                     //seconds until next fire.

                                    if (dist_to_enemy < firing_range * 1.0f) {
                                        // vm_vec_scale_add(&future_enemy_pos,
                                        // enemy_pos, enemy_vel,
                                        // dist_to_enemy/swip->max_speed); if
                                        // (vm_vec_dist_quick(&future_enemy_pos,
                                        // firing_pos) < firing_range * 0.8f) {
...
// Make object Pl_objp chase object En_objp, which is a big ship, not a small
// ship.
void ai_big_chase () {
    ...
    //	Compute the predicted position of the center of the ship, then add the
    //delta to the goal pos.
    if (En_objp->phys_info.speed > 3.0f) {
        set_predicted_enemy_pos (
            &predicted_enemy_pos, Pl_objp, &En_objp->pos,
            &En_objp->phys_info.vel, aip);
    ...
    dist_to_enemy = vm_vec_normalized_dir (
        &vec_to_enemy, &enemy_pos, &player_pos); // - En_objp->radius;
    dot_to_enemy = vm_vec_dot (&vec_to_enemy, &Pl_objp->orient.vec.fvec);
    ...
    //	If just acquired target, or target is not in reasonable cone, don't
    //refine believed enemy position.
    if ((dot_to_enemy < 0.25f) || (aip->target_time < 1.0f) ||
        (aip->ai_flags[AI::AI_Flags::Seek_lock])) {
        update_aspect_lock_information (
            aip, &vec_to_enemy, dist_to_enemy - En_objp->radius,
            En_objp->radius);
    }
    ...
    else if (En_objp->flags[Object::Object_Flags::
                                Protected]) { //	If protected and we're not
                                              //attacking a subsystem, stop
                                              //attacking!
        update_aspect_lock_information (
            aip, &vec_to_enemy, dist_to_enemy - En_objp->radius,
            En_objp->radius);
        aip->target_objnum = -1;
        if (find_enemy (
                OBJ_INDEX (Pl_objp), MAX_ENEMY_DISTANCE,
        ...
        if (aip->target_time < 2.0f)
            if ((dot_to_enemy < 0.9f) || (dist_to_enemy > 300.0f)) {
                ...
                aip->submode_start_time =
                    Missiontime - fl2f (2.75f); //	This backdated start time
                                                //allows immediate switchout.
    ...
    switch (aip->submode) {
    case SM_CONTINUOUS_TURN: ai_big_chase_ct (); break;

    case SM_ATTACK:
    case SM_SUPER_ATTACK:
    case SM_ATTACK_FOREVER:
        ai_big_chase_attack (
            aip, sip, &enemy_pos, dist_to_enemy, sip->model_num);
        break;

    case SM_EVADE: ai_big_evade_ship (); break;

    case SM_AVOID: ai_big_avoid_ship (); break;
    ...
    if (aip->submode != SM_AVOID && aip->submode != SM_EVADE) {
        ...
        //Pl_objp->phys_info.speed * fl_abs(vm_vec_dot(&Pl_objp->phys_info.vel,
        //&aip->big_attack_surface_normal)); 		if (Framecount % 30 == 1) {
        //			mprintf(("normal dist; %.1f, time: %.1f\n",
        //dist_normal_to_enemy, time_to_enemy));
        //		}

        // since we're not in strafe and we may get a bad normal, cap
        // dist_normal_to_enemy as MIN(0.3*dist_to_enemy, self) this will allow
        // us to get closer on a bad normal
        dist_normal_to_enemy =
            MAX (0.3f * dist_to_enemy, dist_normal_to_enemy);
        ...
        if (dist_to_enemy < ATTACK_COLLIDE_BASE_DIST) {
            ...
            if (vm_vec_mag_squared (&aip->big_attack_surface_normal) > 0.9) {
                ...
                if (Pl_objp->phys_info.speed > 0.1) {
                    time_to_enemy =
                        dist_normal_to_enemy / Pl_objp->phys_info.speed;
                }
                else {
                    // a big time
                    time_to_enemy = 100.0f;
            ...
            if ((dist_normal_to_enemy <
                 ATTACK_COLLIDE_AVOID_DIST + speed_dist) ||
                (time_to_enemy < ATTACK_COLLIDE_AVOID_TIME)) {
                ...
                (dist_normal_to_enemy < ATTACK_COLLIDE_SLOW_DIST) ||
        ...
        /*
        if ((dot_to_enemy > 1.0f - 0.1f * En_objp->radius/(dist_to_enemy
        + 1.0f)) && (Pl_objp->phys_info.speed > dist_to_enemy/5.0f)) { if
        (might_collide_with_ship(Pl_objp, En_objp, dot_to_enemy, dist_to_enemy,
        (aip->targeted_subsys == NULL)*2.0f + 1.5f)) { if ((Missiontime -
        aip->last_hit_time > F1_0*4) && (dist_to_enemy < Pl_objp->radius*2 +
        En_objp->radius*2)) { accelerate_ship(aip, -1.0f); } else {
                    aip->submode = SM_AVOID;
                    aip->submode_start_time = Missiontime;
                }
            }
    ...
    case SM_AVOID:
        ...
        {
            int is_inside;
            float box_dist = get_world_closest_box_point_with_delta (
                NULL, En_objp, &Pl_objp->pos, &is_inside, 0.0f);
        ...
        /*		if (dot_to_enemy > -0.2f) {
                    ...
                    if (might_collide_with_ship(Pl_objp, En_objp, dot_to_enemy,
           dist_to_enemy, 3.0f)) { aip->submode_start_time = Missiontime; }
           else { aip->submode = SM_ATTACK; aip->submode_start_time =
           Missiontime; aip->last_attack_time = Missiontime;
                    }
                }
    ...
    //     I noticed AI ships firing inappropriately when following a path near
    //     a big ship.
    //		 TODO: investigate why ships fire (and aren't close to hitting ship)
    //when following a path near 				 a big ship
...
void ai_big_attack_get_data (
    vec3d* enemy_pos, float* dist_to_enemy, float* dot_to_enemy) {
    ...
    vm_vec_scale_add2 (
        &player_pos, &Pl_objp->orient.vec.fvec, Pl_objp->radius);
    ...
    *dot_to_enemy = vm_vec_dot (&vec_to_enemy, &Pl_objp->orient.vec.fvec);
    if ((*dot_to_enemy < 0.25f) || (aip->target_time < 1.0f) ||
        ...
        set_predicted_enemy_pos_turret (
            &predicted_enemy_pos, &gun_pos, Pl_objp, enemy_pos,
            &En_objp->phys_info.vel, weapon_speed, aip->time_enemy_in_range);
    }
...
// check to see if Pl_objp has gotten too close to attacking point.. if so,
// break off by entering AIS_STRAFE_RETREAT
static int ai_big_strafe_maybe_retreat (const vec3d* target_pos) {
    ai_info* aip;
    aip = &Ai_info[Ships[Pl_objp->instance].ai_index];
    ...
    if (vm_vec_mag_quick (&aip->big_attack_surface_normal) > 0.9) {
        dist_normal_to_target =
            -vm_vec_dot (&vec_to_target, &aip->big_attack_surface_normal);
    }
    else {
...
void ai_big_strafe_attack () {
    ...
    if (ai_big_maybe_follow_subsys_path (0)) { return; }

    ai_big_attack_get_data (&target_pos, &target_dist, &target_dot);
    if (ai_big_strafe_maybe_retreat (&target_pos)) return;

    if (aip->ai_flags[AI::AI_Flags::Kamikaze]) {
        if (target_dist < 1200.0f) {
            ai_turn_towards_vector (
                &target_pos, Pl_objp,
                Ship_info[Ships[Pl_objp->instance].ship_info_index]
                    .srotation_time,
    ...
    if (!(aip->ai_flags[AI::AI_Flags::Seek_lock]) ||
        ...
        if (t > 0.0f && t < 1.5f) {
            ...
            case 0:
                vm_vec_scale_add (
                    &rand_vec, &Pl_objp->orient.vec.fvec,
                    &Pl_objp->orient.vec.uvec, 2.0f);
                break;
            case 1:
                vm_vec_scale_add (
                    &rand_vec, &Pl_objp->orient.vec.fvec,
    ...
    if (aip->ai_profile_flags
        ...
        if (target_dist > 1200 || last_hit < F1_0 * 6) { accel = 1.0f; }
        else {
            float attack_time;
            attack_time = f2fl (Missiontime - aip->submode_start_time);
            if (attack_time > 15) { accel = 0.2f; }
    ...
    if ((last_hit > long_enough) &&
        ((Missiontime - aip->submode_parm0) > long_enough)) {
        ai_big_switch_to_chase_mode (aip);
    }
}
...
void ai_big_strafe_glide_attack () {
    ...
    float target_dist, target_dot, target_ship_dist, dot_to_goal,
        flight_dot_to_enemy;
    object* target_objp;
    ...
    dot_to_goal = vm_vec_dot (
        &vec_to_goal,
        &norm_vel_vec); // Angle between flight path and goal point
    flight_dot_to_enemy = vm_vec_dot (
        &target_objp->pos,
        &norm_vel_vec); // Angle between flight path and target ship

    // If we're following a path, then we shouldn't be doing glide strafe
    if (ai_big_maybe_follow_subsys_path (0)) { return; }

    // Gets a point on the target ship to attack, as well as distance and the
    // angle between the nose of the attacker and that point.
    ai_big_attack_get_data (&target_pos, &target_dist, &target_dot);
    ...
    if (aip->submode_parm1 == 0) {
        // Pick goal point. This should be a random point that passes through
        // the circle whose normal is the vector between attacker and target
        ...
        vm_vec_random_in_circle (
            &tangentPoint, &target_objp->pos, &orient,
            target_objp->radius + GLIDE_STRAFE_DISTANCE, 1);
        // Get tangent point in coords relative to ship, scale it up so the
        // actual goal point is far away, then put back in world coords
    ...
    if (aip->submode_parm1 == 1) {
        ...
        if ((Pl_objp->phys_info.fspeed >
             (Pl_objp->phys_info.max_vel.xyz.z * 0.85f)) &&
            (dot_to_goal > 0.99f))
            aip->submode_parm1 = 2;
    }
    if (aip->submode_parm1 == 2) {
        ...
        // If we are still on approach but too far away, this will still
        // trigger. This will allow us to reposition the target point and allow
        // for a "jinking" effect.
        if (target_ship_dist >
                (STRAFE_RETREAT_BOX_DIST + target_objp->radius) &&
            Missiontime - aip->submode_start_time >
                i2f (GLIDE_STRAFE_MIN_TIME)) {
            // This checks whether we are moving toward the target or away from
            // it.  If moving towards, we reset the stage so that we pick a new
            // attack vector (jinking). If moving away, we're at the end of a
            // run so do a full reset (possibly allowing a switch back to
            // conventional strafe).
            if (flight_dot_to_enemy > 0.0f) { aip->submode_parm1 = 0; }
            else {
                aip->submode_parm1 = 3;
    ...
    if ((Missiontime - aip->last_hit_time > long_enough) &&
        ((Missiontime - aip->submode_parm0) > long_enough)) {
        ai_big_switch_to_chase_mode (aip);
    }
}
...
void ai_big_strafe_avoid () {
    ...
    if (mode_time > fl2f (0.5)) { ai_big_strafe_position (); }
...
void ai_big_strafe_retreat1 () {
    ...
    if (Missiontime - aip->submode_start_time > fl2f (1.50f)) {
        ...
        case 0:
            vm_vec_add (
                &rand_vec, &Pl_objp->orient.vec.fvec,
                &Pl_objp->orient.vec.uvec);
            break;
        case 1:
            vm_vec_scale_add (
                &rand_vec, &Pl_objp->orient.vec.fvec,
                &Pl_objp->orient.vec.uvec, -1.0f);
            break;
        case 2:
            vm_vec_add (
                &rand_vec, &Pl_objp->orient.vec.fvec,
                &Pl_objp->orient.vec.rvec);
            break;
        case 3:
            vm_vec_scale_add (
                &rand_vec, &Pl_objp->orient.vec.fvec,
                &Pl_objp->orient.vec.rvec, -1.0f);
            break;
        } // end switch
        ...
        vm_vec_scale_add (
            &aip->prev_goal_point, &aip->goal_point, &rand_vec, 200.0f);
...
// #define	AIS_STRAFE_AVOID		202	// fly evasive vector to avoid incoming
// fire #define	AIS_STRAFE_RETREAT1	203	// fly away from attack point
// (directly) #define	AIS_STRAFE_RETREAT1	204	// fly away from attack point
// (on an avoid vector) #define	AIS_STRAFE_POSITION	205	// re-position to
// resume strafing attack
//
void ai_big_strafe () {
    ai_info* aip;
    ...
    */
    ...
    switch (aip->submode) {
    case AIS_STRAFE_ATTACK: ai_big_strafe_attack (); break;
    case AIS_STRAFE_AVOID: ai_big_strafe_avoid (); break;
    case AIS_STRAFE_RETREAT1: ai_big_strafe_retreat1 (); break;
    case AIS_STRAFE_RETREAT2: ai_big_strafe_retreat2 (); break;
    case AIS_STRAFE_POSITION: ai_big_strafe_position (); break;
...
// See if Pl_objp should enter strafe mode (This is called from
// maybe_evade_dumbfire_weapon(), and the weapon_objnum is for a weapon that is
// about to collide with Pl_objp
//
// Check if weapon_objnum was fired by Pl_objp's target, and whether Pl_objp's
// target is a big ship, if so, enter AIM_STRAFE
int ai_big_maybe_enter_strafe_mode (const object* pl_objp, int weapon_objnum) {
    ...
    if (Objects[aip->target_objnum].type != OBJ_SHIP) { return 0; }

    sip = &Ship_info[Ships[Objects[aip->target_objnum].instance]
                         .ship_info_index];

    // if Pl_objp's target is not a big/capital ship, then cannot enter strafe
    // mode AL 12-31-97: Even though transports are considered big ships, don't
    // enter strafe mode on them
    if (!(sip->is_big_or_huge ()) ||
        (sip->flags[Ship::Info_Flags::Transport])) {
        return 0;
    }

    //	If Pl_objp not a fighter or bomber, don't enter strafe mode. -- MK,
    //11/11/97.
    ...
    if ((parent_objp->signature != weapon_objp->parent_sig) ||
        (parent_objp->type != OBJ_SHIP)) {
        return 0;
    }
    ...
    // if we've got this far, the weapon must have come from the player's
    // target, and it is a big/capital ship... so enter strafe mode
    aip->previous_mode = aip->mode;
    aip->mode = AIM_STRAFE;
...
void ai_big_strafe_maybe_attack_turret (
    ...
    if (Weapons[weapon_objp->instance].turret_subsys == NULL) { return; }

    Assert (weapon_objp->parent >= 0 && weapon_objp->parent < MAX_OBJECTS);
    parent_objp = &Objects[weapon_objp->parent];

    // UnknownPlayer : Decide whether or not this weapon was a beam, in which
    // case it might be a good idea to go after it even if its not on the
    // current target. This is randomized so the ai will not always go after
    // different ships firing beams at them. Approx 1/4 chance we'll go after
    // the other ship's beam.

    bool attack_turret_on_different_ship =
    ...
    if (!attack_turret_on_different_ship) {
        ...
        if (aip->target_objnum != OBJ_INDEX (parent_objp)) { return; }
    }

    // attack the turret
    set_targeted_subsys (
        aip, Weapons[weapon_objp->instance].turret_subsys,
        OBJ_INDEX (parent_objp));
}
