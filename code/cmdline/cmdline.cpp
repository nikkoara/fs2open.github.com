...
Flag exe_params[] = {
    ...
    },
    {
        ...
        "Troubleshoot",
        "http://www.hard-light.net/wiki/index.php/"
        "Command-Line_Reference#-noshadercache",
    },
#ifdef WIN32
    {
        "-fix_registry",
...
// Other
cmdline_parm
    get_flags_arg ("-get_flags", "Output the launcher flags file", AT_STRING);
...
char* drop_extra_chars (char* str) {
    ...
    while (e > s) {
        if (!is_extra_space (str[e])) { break; }

        e--;
    }

    if (e >= s && e != 0) { memmove (str, str + s, e - s + 1); }
...
bool parm_stuff_args (cmdline_parm* parm, int argc, char* argv[], int index) {
    ...
    if (index + 1 < argc) {
        ...
        if (value[0] == '-') {
            ...
            if (parm->args != NULL) {
                if (parm->stacks) { saved_args = parm->args; }
                else {
                    delete[] parm->args;
                }
...
void os_parse_parms (int argc, char* argv[]) {
    ...
    for (int i = 0; i < argc; i++) {
        ...
        if (i == 1 && strncmp (argv[i], "-psn", 4) == 0) { continue; }
...
void os_validate_parms (int argc, char* argv[]) {
    ...
    for (int i = 0; i < argc; i++) {
        ...
        // On OS X this gets passed if the application was launched by
        // double-clicking in the Finder
        if (i == 1 && strncmp (token, "-psn", 4) == 0) { continue; }
        ...
        if (token[0] == '-') {
            ...
            if (parm_found == 0) {
                ...
                if (!stricmp (token, "-help") || !stricmp (token, "--help") ||
                    ...
                    static const int STR_SIZE =
                        25; // max len of exe_params.name + 5 spaces
                    const int AT_SIZE =
                        8; // max len of cmdline_arg_types[] + 2 spaces
                    size_t atp = 0;
                    size_t sp = 0;
                    for (parmp = GET_FIRST (&Parm_list);
...
bool has_cmdline_only_flag (int argc, char* argv[]) {
    for (int i = 0; i < argc; ++i) {
        if (!strcmp (argv[i], PARSE_COMMAND_LINE_STRING)) { return true; }
    }
...
cmdline_parm::cmdline_parm (
    ...
    if (Parm_list_inited == 0) {
        ...
        if (help == NULL) { help = get_param_desc (name); }
...
unix_get_dir_names (const std::string& parent, const std::string& dir) {
    ...
    std::vector< std::string > this_dir_names =
        unix_get_single_dir_names (parent, dir.substr (0, slash));
    ...
    for (ii = this_dir_names.begin (); ii != end; ++ii) {
        ...
        std::vector< std::string > mod_path =
            unix_get_dir_names (this_dir_path, rest);
...
static void handle_unix_modlist (char** modlist, size_t* len) {
    ...
    for (char* cur_mod = strtok (*modlist, ","); cur_mod != NULL;
         cur_mod = strtok (NULL, ",")) {
        std::vector< std::string > this_mod_paths =
...
static json_t* json_get_v1 () {
...

    {
        ...
        json_object_set_new (
            version_obj, "full", json_string (FS_VERSION_FULL));
        json_object_set_new (
            version_obj, "major", json_integer (FS_VERSION_MAJOR));
        json_object_set_new (
            version_obj, "minor", json_integer (FS_VERSION_MINOR));
        json_object_set_new (
            version_obj, "build", json_integer (FS_VERSION_BUILD));

        json_object_set_new (
            version_obj, "has_revision", json_boolean (FS_VERSION_HAS_REVIS));
        json_object_set_new (
            version_obj, "revision", json_integer (FS_VERSION_REVIS));
        json_object_set_new (
            version_obj, "revision_str", json_string (FS_VERSION_REVIS_STR));
    ...
    }
    {
        ...
        for (auto& flag : exe_params) {
            ...
            json_object_set_new (
                flag_obj, "description", json_string (flag.desc));
            json_object_set_new (
                flag_obj, "fso_only", json_boolean (flag.fso_only));
            json_object_set_new (
                flag_obj, "on_flags", json_integer (flag.on_flags));
            json_object_set_new (
                flag_obj, "off_flags", json_integer (flag.off_flags));
            json_object_set_new (flag_obj, "type", json_string (flag.type));
            json_object_set_new (
                flag_obj, "web_url", json_string (flag.web_url));
    ...
    }
    {
        ...
        for (auto& display : displays) {
            ...
            json_object_set_new (
                display_obj, "index", json_integer (display.index));
            json_object_set_new (
                display_obj, "name", json_string (display.name.c_str ()));

            json_object_set_new (display_obj, "x", json_integer (display.x));
            json_object_set_new (display_obj, "y", json_integer (display.y));
            json_object_set_new (
                display_obj, "width", json_integer (display.width));
            json_object_set_new (
                display_obj, "height", json_integer (display.height));
            ...
            for (auto& mode : display.video_modes) {
                ...
                json_object_set_new (
                    mode_obj, "y", json_integer (mode.height));
                json_object_set_new (
                    mode_obj, "bits", json_integer (mode.bit_depth));
    ...
    }
    {
        ...
        json_object_set_new (
            openal_obj, "version_major",
            json_integer (openal_info.version_major));
        json_object_set_new (
            openal_obj, "version_minor",
            json_integer (openal_info.version_minor));

        json_object_set (
            openal_obj, "default_playback",
            json_string (openal_info.default_playback_device.c_str ()));
        json_object_set (
            openal_obj, "default_capture",
            json_string (openal_info.default_capture_device.c_str ()));

        {
            ...
            for (auto& device : openal_info.playback_devices) {
                json_array_append_new (
                    playback_array, json_string (device.c_str ()));
            }

            json_object_set_new (
                openal_obj, "playback_devices", playback_array);
        }
        {
            ...
            for (auto& device : openal_info.capture_devices) {
                json_array_append_new (
                    capture_array, json_string (device.c_str ()));
            }
        ...
        }
        {
            ...
            for (auto& pair : openal_info.efx_support) {
                json_object_set_new (
                    efx_support_obj, pair.first.c_str (),
                    json_boolean (pair.second));
            }
    ...
    }
    {
        ...
        for (auto& info : joysticks) {
            ...
            json_object_set_new (
                joystick_obj, "name", json_string (info.name.c_str ()));
            json_object_set_new (
                joystick_obj, "guid", json_string (info.guid.c_str ()));

            json_object_set_new (
                joystick_obj, "num_axes", json_integer (info.num_axes));
            json_object_set_new (
                joystick_obj, "num_balls", json_integer (info.num_balls));
            json_object_set_new (
                joystick_obj, "num_buttons", json_integer (info.num_buttons));
            json_object_set_new (
                joystick_obj, "num_hats", json_integer (info.num_hats));

            json_object_set_new (
                joystick_obj, "is_haptic", json_boolean (info.is_haptic));
    ...
    {
        json_object_set_new (
            root, "pref_path", json_string (os_get_config_path ().c_str ()));
    }
...
static flag_output_type get_flags_output_type () {
    Assertion (
        get_flags_arg.found (),
        "This function is only valid if -get_flags is present!");
    ...
    if (type == "binary") { return flag_output_type::Binary; }
    else if (type == "json_v1") {
        return flag_output_type::Json_V1;
    }
    else {
        // This is supposed to make it easy for the launcher to recognize an
        // unsupported type
...
static void write_flags () {
    ...
    switch (type) {
    case flag_output_type::Binary: write_flags_file (); break;
...
// Sets externed variables used for communication cmdline information
{
    ...
    if (no_fpscap.found ()) { Cmdline_NoFPSCap = 1; }

    if (loadallweapons_arg.found ()) { Cmdline_load_all_weapons = 1; }

    if (voice_recognition_arg.found ()) { Cmdline_voice_recognition = 1; }

#ifdef Allow_NoWarn
    if (nowarn_arg.found ()) { Cmdline_nowarn = 1; }
#endif
    ...
    if (objupd_arg.found ()) {
        ...
        if (Cmdline_objupd < 0) { Cmdline_objupd = 0; }
        if (Cmdline_objupd > 3) { Cmdline_objupd = 3; }
    }

    if (mpnoreturn_arg.found ()) { Cmdline_mpnoreturn = 1; }
    ...
    // run with no music
    if (nomusic_arg.found ()) { Cmdline_freespace_no_music = 1; }
    ...
    // spew pof info
    if (pof_spew.found ()) { Cmdline_spew_pof_info = 1; }

    // mouse coords
    if (mouse_coords.found ()) { Cmdline_mouse_coords = 1; }

    // net timeout
    if (timeout.found ()) { Cmdline_timeout = timeout.get_int (); }

    // d3d windowed
    if (window_arg.found ()) { Cmdline_window = 1; }
    ...
    if (res_arg.found ()) { Cmdline_res = res_arg.str (); }
    ...
    if (targetinfo_arg.found ()) { Cmdline_targetinfo = 1; }

    if (nomovies_arg.found ()) { Cmdline_nomovies = 1; }

    if (noscalevid_arg.found ()) { Cmdline_noscalevid = 1; }

    if (noparseerrors_arg.found ()) { Cmdline_noparseerrors = 1; }
    ...
    if (fps_arg.found ()) { Show_framerate = 1; }

    if (bmpmanusage_arg.found ()) { Cmdline_bmpman_usage = 1; }

    if (pos_arg.found ()) { Cmdline_show_pos = 1; }

    if (nomotiondebris_arg.found ()) { Cmdline_nomotiondebris = 1; }

    if (mipmap_arg.found ()) { Cmdline_mipmap = 1; }

    if (stats_arg.found ()) { Cmdline_show_stats = 1; }

    if (fov_arg.found ()) {
        Cmdline_fov = fov_arg.get_float ();
        if (Cmdline_fov > 0.1) { VIEWER_ZOOM_DEFAULT = Cmdline_fov; }
        else {
    ...
    if (orb_radar.found ()) { Cmdline_orb_radar = 1; }

    if (use_3dwarp.found ()) { Cmdline_3dwarp = 1; }

    if (use_warp_flash.found ()) { Cmdline_warp_flash = 1; }

    if (allow_autpilot_interrupt.found ()) {
        Cmdline_autopilot_interruptable = 0;
    }

    if (stretch_menu.found ()) { Cmdline_stretch_menu = 1; }
    ...
    if (spec_arg.found ()) { Cmdline_spec = 0; }

    if (no_set_gamma_arg.found ()) { Cmdline_no_set_gamma = 1; }

    if (no_vsync_arg.found ()) { Cmdline_no_vsync = 1; }

    if (normal_arg.found ()) { Cmdline_normal = 0; }

    if (height_arg.found ()) { Cmdline_height = 0; }
    ...
    if (glow_arg.found ()) Cmdline_glow = 0;

    if (query_speech_arg.found ()) Cmdline_query_speech = 1;

    if (ship_choice_3d_arg.found ()) Cmdline_ship_choice_3d = 1;

    if (weapon_choice_3d_arg.found ()) Cmdline_weapon_choice_3d = 1;

    if (ingamejoin_arg.found ()) Cmdline_ingamejoin = 1;
    ...
    if (no_pbo_arg.found ()) { Cmdline_no_pbo = 1; }

    if (no_drawrangeelements.found ()) { Cmdline_drawelements = 1; }

    if (keyboard_layout.found ()) {
        Cmdline_keyboard_layout = keyboard_layout.str ();
    }

    if (gl_finish.found ()) { Cmdline_gl_finish = true; }

    if (no_geo_sdr_effects.found ()) { Cmdline_no_geo_sdr_effects = true; }

    if (set_cpu_affinity.found ()) { Cmdline_set_cpu_affinity = true; }

    if (nograb_arg.found ()) { Cmdline_nograb = true; }

    if (noshadercache_arg.found ()) { Cmdline_noshadercache = true; }
    ...
    if (old_collision_system.found ()) Cmdline_old_collision_sys = 1;

    if (dis_collisions.found ()) Cmdline_dis_collisions = 1;

    if (dis_weapons.found ()) Cmdline_dis_weapons = 1;

    if (no_fbo_arg.found ()) { Cmdline_no_fbo = 1; }

    if (noemissive_arg.found ()) { Cmdline_no_emissive = 1; }
    ...
    if (rearm_timer_arg.found ()) Cmdline_rearm_timer = 1;

    if (save_render_targets_arg.found ()) Cmdline_save_render_targets = 1;

    if (verify_vps_arg.found ()) Cmdline_verify_vps = 1;

    if (no3dsound_arg.found ()) Cmdline_no_3d_sound = 1;

    if (atiswap_arg.found ()) { Cmdline_ati_color_swap = 1; }

    if (enable_3d_shockwave_arg.found ()) { Cmdline_enable_3d_shockwave = 1; }

    if (softparticles_arg.found ()) { Cmdline_softparticles = 1; }

    if (fb_explosions_arg.found ()) { Cmdline_fb_explosions = 1; }

    if (fb_thrusters_arg.found ()) { Cmdline_fb_thrusters = true; }

    if (postprocess_arg.found ()) { Cmdline_postprocess = 1; }

    if (bloom_intensity_arg.found ()) {
        Cmdline_bloom_intensity = bloom_intensity_arg.get_int ();
    }

    if (flightshaftsoff_arg.found ()) { ls_force_off = true; }

    if (reparse_mainhall_arg.found ()) { Cmdline_reparse_mainhall = 1; }

    if (enable_shadows_arg.found ()) { Cmdline_shadow_quality = 2; }
    ...
    // Deprecated flags - CommanderDJ
    if (deprecated_spec_arg.found ()) { Cmdline_deprecated_spec = 1; }

    if (deprecated_glow_arg.found ()) { Cmdline_deprecated_glow = 1; }

    if (deprecated_normal_arg.found ()) { Cmdline_deprecated_normal = 1; }

    if (deprecated_env_arg.found ()) { Cmdline_deprecated_env = 1; }

    if (deprecated_tbp_arg.found ()) { Cmdline_deprecated_tbp = 1; }

    if (deprecated_jpgtga_arg.found ()) { Cmdline_deprecated_jpgtga = 1; }

    if (deprecated_htl_arg.found ()) { Cmdline_deprecated_nohtl = 1; }
