...
Section lookupSectionValue (const char* name) {
    for (auto& pair : SectionMapping) {
        if (!strcmp (pair.second, name)) { return pair.first; }
    }
...
void pilot::JSONFileHandler::writeUByte (
    const char* name, std::uint8_t value) {
    writeInteger (name, value);
}
void pilot::JSONFileHandler::writeShort (
    const char* name, std::int16_t value) {
    writeInteger (name, value);
}
void pilot::JSONFileHandler::writeInt (const char* name, std::int32_t value) {
    writeInteger (name, value);
}
void pilot::JSONFileHandler::writeUInt (
...
void pilot::JSONFileHandler::startSectionWrite (Section id) {
    ...
    if (json_is_array (_currentEl)) {
        ...
        Assertion (
            key_name == nullptr,
            "Inside an array there can be no named section!");
        json_array_append_new (_currentEl, obj);
    }
    else {
        Assertion (
            key_name != nullptr, "Section outside of arrays must be named!");
...
void pilot::JSONFileHandler::endSectionWrite () {
    Assertion (
        json_is_object (_currentEl), "Section ended while not in a section!");
...
void pilot::JSONFileHandler::startArrayWrite (const char* name, size_t, bool) {
    ...
    if (json_is_array (_currentEl)) {
        // We are in an array, section must be unnamed
        Assertion (
            name == nullptr, "Inside an array there can be no named section!");
        json_array_append_new (_currentEl, array);
    }
    else {
        Assertion (
            name != nullptr, "Section outside of arrays must be named!");
...
void pilot::JSONFileHandler::endArrayWrite () {
    Assertion (
        json_is_array (_currentEl), "Array ended while not in an array!");
...
void pilot::JSONFileHandler::flush () {
    Assertion (
        ...
        "Not all sections or arrays have been ended!");
...
void pilot::JSONFileHandler::ensureNotExists (const char* name) {
    Assertion (
        json_is_object (_currentEl),
        "Currently not in an element that supports keys!");
    // Make sure we don't overwrite previous values
    Assertion (
        json_object_get (_currentEl, name) == nullptr,
        "Entry with name %s already exists!", name);
}
