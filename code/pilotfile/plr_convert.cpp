...
plr_data::~plr_data () { controls.clear (); }
...
void pilotfile_convert::plr_import_controls () {
    ...
    if (!num_controls) { return; }

    // it may be less than 118, but it shouldn't be more than 118
    if (num_controls > 118) { throw "Data check failure in controls!"; }
    ...
    for (idx = 0; idx < num_controls; idx++) {
        ...
        if (con.key_id == 255) { con.key_id = -1; }

        con.joy_id = cfread_short (cfp);

        if (con.joy_id == 255) { con.joy_id = -1; }
...
void pilotfile_convert::plr_import_detail () {
    ...
    if ((plr->detail_setting < -1) || (plr->detail_setting > 4)) {
        ...
        (plr->detail_hardware_textures < 0) ||
        (plr->detail_hardware_textures > 4)) {
        data_failure = true;
    }
        ...
        (plr->detail_num_particles < 0) || (plr->detail_num_particles > 4)) {
        data_failure = true;
    }
        ...
        (plr->detail_shield_effects < 0) || (plr->detail_shield_effects > 4)) {
        data_failure = true;
    }
        ...
        (plr->detail_targetview_model < 0) ||
        (plr->detail_targetview_model > 1)) {
        data_failure = true;
    }
    else if (
        (plr->detail_planets_suns < 0) || (plr->detail_planets_suns > 1)) {
        data_failure = true;
    }
    else if (
        (plr->detail_weapon_extras < 0) || (plr->detail_weapon_extras > 1)) {
        data_failure = true;
    }

    if (data_failure) { throw "Data check failure in details!"; }
}
...
void pilotfile_convert::plr_import_stats () {
    ...
    // medals
    for (idx = 0; idx < 18; idx++) { cfread_int (cfp); }
...
void pilotfile_convert::plr_import_red_alert () {
    ...
    for (idx = 0; idx < num_slots; idx++) {
        ...
        // subsystem hits
        for (j = 0; j < 64; j++) { cfread_float (cfp); }

        // aggregate hits
        for (j = 0; j < 12; j++) { cfread_float (cfp); }
...
void pilotfile_convert::plr_import_variables () {
    ...
    for (idx = 0; idx < num_variables; idx++) {
        ...
        cfread_string_len (
            nvar.variable_name, sizeof (nvar.variable_name), cfp);
...
void pilotfile_convert::plr_import () {
    ...
    if (plr_id != 0x46505346) { throw "Invalid file signature!"; }

    fver = cfread_uint (cfp);

    if ((fver != 142) && (fver != 242)) { throw "Unsupported file version!"; }
    ...
    if (fver < 242) { cfread_ubyte (cfp); }

    plr->tips = cfread_int (cfp);

    if ((plr->tips < 0) || (plr->tips > 1)) { throw "Data check failure!"; }
    ...
    cfread_string_len (
        plr->current_campaign, sizeof (plr->current_campaign), cfp);
    cfread_string_len (
        plr->last_ship_flown, sizeof (plr->last_ship_flown), cfp);
    ...
    if (plr->net_protocol == NET_NONE) { plr->net_protocol = NET_TCP; }
...
bool pilotfile_convert::plr_convert (const char* fname, bool inferno) {
    ...
    if (plr == NULL) { plr = new (std::nothrow) plr_data; }
    ...
    cf_create_default_path_string (
        filename, CF_TYPE_SINGLE_PLAYERS, (inferno) ? "inferno" : nullptr,
        false,
        CF_LOCATION_ROOT_USER | CF_LOCATION_ROOT_GAME | CF_LOCATION_TYPE_ROOT);

    if (inferno) { filename.append (DIR_SEPARATOR_STR); }
