...
void pilotfile::update_stats (scoring_struct* stats, bool training) {
    ...
    if (Game_mode & GM_MULTIPLAYER) { p_stats = &multi_stats; }
    else {
    ...
    if (stats->m_medal_earned >= 0) {
        ...
        for (idx = 0; idx < list_size; idx++) {
            if (p_stats->medals_earned[idx].name ==
                Medals[stats->m_medal_earned].name) {
                j = idx;
                break;
            }
        }

        if (j >= 0) { p_stats->medals_earned[j].val++; }
        else {
    ...
    if (stats->m_badge_earned.size ()) {
        list_size = (int)p_stats->medals_earned.size ();
        for (size_t medal = 0; medal < stats->m_badge_earned.size ();
             medal++) {
            j = -1;

            for (idx = 0; idx < list_size; idx++) {
                if (p_stats->medals_earned[idx].name ==
                    Medals[stats->m_badge_earned[medal]].name) {
                    j = idx;
                    break;
                }
            }

            if (j >= 0) { p_stats->medals_earned[j].val = 1; }
            else {
