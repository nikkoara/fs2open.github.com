...
void pilotfile::plr_read_hud () {
    ...
    if (HUD_config.rp_dist < 0 || HUD_config.rp_dist >= RR_MAX_RANGES) {
        Warning (
            LOCATION,
            "Player file has invalid radar range %d, setting to default.\n",
            HUD_config.rp_dist);
...
void pilotfile::plr_write_hud () {
    ...
    // settings
    handler->writeUByte (
        "num_msg_window_lines", HUD_config.num_msg_window_lines);
...
void pilotfile::plr_write_multiplayer () {
    ...
    handler->writeInt (
        "voice_token_wait", p->m_server_options.voice_token_wait);
    handler->writeInt (
        "voice_record_time", p->m_server_options.voice_record_time);
    handler->writeInt (
        "mission_time_limit", (int)p->m_server_options.mission_time_limit);
    handler->writeInt ("kill_limit", p->m_server_options.kill_limit);

    // local options
    handler->writeInt ("local_flags", p->m_local_options.flags);
    handler->writeInt (
        "obj_update_level", p->m_local_options.obj_update_level);
...
void pilotfile::plr_write_controls () {
    ...
    // For some unknown reason, the old code used a short for the array length
    // here...
...
void pilotfile::plr_write_settings () {
    ...
    handler->writeFloat (
        "master_event_music_volume", Master_event_music_volume);
...
bool pilotfile::load_player (const char* callsign, player* _p) {
    ...
    while (!cfeof (cfp)) {
        ...
        if (section_size == 0) {
            mprintf (
                ("PLR => 0 size section with id %d starting at %zu\n",
...
bool pilotfile::save_player (player* _p) {
    ...
    if (Cmdline_json_pilot) { filename += ".json"; }
    else {
        filename += ".plr";
    }
...
bool pilotfile::verify (const char* fname, int* rank, char* valid_language) {
    ...
    if (valid_language) { strcpy (valid_language, p->language); }
