...
// Module data
////////////////////////////////////////////////////////////
static int Player_all_alone_msg_inited =
    0; // flag used for initializing a player-specific voice msg
 ...
 *
 * @param[in,out] ap The current view angles. Is modified towards target
 * angles.
 ...
 * External, External Locked, TrackIR, Freelook (Unlocked), Normal (Locked),
 * and Centering
 *
 * @param[in,out]   ma      The camera veiw angles to modify (magnitude is
 * saturated to be within max_p and max_h).
 * @param[out]  da      The delta angles applied to ma (magnitude is saturated
 * to 1 radian).
...
void view_modify (
    ...
    if (Viewer_mode & VM_CENTERING) {
        ...
        if (Viewer_mode & VM_PADLOCK_ANY) {
            ...
            if (Viewer_mode & VM_PADLOCK_UP) {
                ...
                ma->h = -max_h;
                ma->p = 0.0f;
            } // Else, don't do any ajustments. player_set_padlock_state will
              // reset the states
            ...
            headtracking::HeadTrackingStatus* status =
                headtracking::getStatus ();
            ...
            if (trans.xyz.z < 0) { trans.xyz.z = 0.0f; }

            vm_vec_unrotate (&leaning_position, &trans, &Eye_matrix);
        }
        else {
            // Do slew
            /* These have been commented out until the controls are added into
            Controls_config[] t = (check_control_timef(SLEW_LEFT) -
            check_control_timef(SLEW_RIGHT)); u = (check_control_timef(SLEW_UP)
            - check_control_timef(SLEW_DOWN);
            */
        ...
        u =
            (check_control_timef (PITCH_BACK) -
             check_control_timef (PITCH_FORWARD));
    ...
    if ((Game_time_compression >= F1_0) && !(Viewer_mode & VM_EXTERNAL)) {
        ...
        // If time compression is less than normal, still move camera at same
        // speed This gives a cool matrix effect
...
void do_view_track_target (float /*frame_time*/) {
    ...
    if ((Player_ai->target_objnum == -1) || (Viewer_mode & VM_OTHER_SHIP)) {
        ...
        // perspective, center the view and turn off target padlock because the
        // target won't be at the angle we've calculated from the player's
        // perspective.
    ...
    vm_vec_rotate (
        &forwardvec_rotated, &Player_obj->orient.vec.fvec,
        &Player_obj->orient);

    vm_vec_normalized_dir (
        &view_vector, &targetpos_rotated, &playerpos_rotated);
    ...
    // prevents the player from looking completely behind himself; just over
    // his shoulder
...
/**
 * @brief When VIEW_SLEW is pressed, pitch and heading axes controls viewer
 * direction slewing.
 *
 * @param[in] frame_time The frame time at which this function is called
 *
 * @details Prevents the player's "head" from swiveling unrealistacally far
 * with a max_p of Pi/2 and a max_h of2/3 Pi.
 *
 * @note Some mods may prefer to set their own limits, so, maybe make this as a
 * table option in the future
 */
void do_view_slew (float frame_time) {
    view_modify (
        &chase_slew_angles, &Viewer_slew_angles_delta, PI_2, PI2 / 3,
        frame_time);
    ...
    if (Viewer_mode & VM_CENTERING) {
        // If we're centering the view, check to see if we're actually centered
        // and bypass any view modifications until the view has finally been
        // centered.
...
void do_view_chase (float /*frame_time*/) {
    ...
    t = check_control_timef (VIEW_DIST_INCREASE) -
        check_control_timef (VIEW_DIST_DECREASE);
    Viewer_chase_info.distance += t * 4;
    if (Viewer_chase_info.distance < 0.0f) Viewer_chase_info.distance = 0.0f;
...
void do_thrust_keys (control_info* ci) {
    ci->forward = check_control_timef (FORWARD_THRUST) -
                  check_control_timef (REVERSE_THRUST);
    ci->sideways =
        (check_control_timef (RIGHT_SLIDE_THRUST) -
         check_control_timef (LEFT_SLIDE_THRUST)); // for slideing-Bobboau
    ci->vertical =
        (check_control_timef (UP_SLIDE_THRUST) -
         check_control_timef (DOWN_SLIDE_THRUST)); // for slideing-Bobboau
}

/**
 * Called by single and multiplayer modes to reset information inside of
 ...
 *
 * @param[out] axis       The axis array to store the scaled joystick position.
 * (Must have a size of NUM_JOY_AXIS_ACTIONS)
 * @param[in]  frame_time The frame time at which this function is called.
 *
 * @details This is its own function because we only want to read it at a
 * certain rate, since it takes time.
 */
void playercontrol_read_stick (int* axis, float frame_time) {
    int i;
    ...
    if (Use_mouse_to_fly) {
        ...
        for (i = 0; i < NUM_JOY_AXIS_ACTIONS; i++) {
            ...
            case JOY_X_AXIS: x_axis = i; break;
            case JOY_Y_AXIS: y_axis = i; break;
            case JOY_Z_AXIS: z_axis = i; break;
            }
        }

        if (x_axis >= 0) {
            if (Invert_axis[x_axis]) { dx = -dx; }
            axis[x_axis] += (int)((float)dx * factor);
        }

        if (y_axis >= 0) {
            if (Invert_axis[y_axis]) { dy = -dy; }
            axis[y_axis] += (int)((float)dy * factor);
        }

        if (z_axis >= 0) {
            if (Invert_axis[z_axis]) { dz = -dz; }
            axis[z_axis] += (int)((float)dz * factor);
        }
    }
}

void read_keyboard_controls (
    control_info* ci, float frame_time, physics_info* pi) {
    ...
    int centering_speed =
        7; // the scale speed in which the camera will smoothly center when the
           // player presses Center View
    ...
    chase_angles_to_value (
        &Viewer_slew_angles, &chase_slew_angles, centering_speed);
    ...
    if (Viewer_mode & VM_CAMERA_LOCKED) {
        ...
        if (check_control (BANK_WHEN_PRESSED)) {
            ci->bank = check_control_timef (BANK_LEFT) +
                       check_control_timef (YAW_LEFT) -
                       check_control_timef (YAW_RIGHT) -
                       check_control_timef (BANK_RIGHT);
            ci->heading = 0.0f;
        }
        else {
            kh = (check_control_timef (YAW_RIGHT) -
                  check_control_timef (YAW_LEFT)) /
                 8.0f;
            if (kh == 0.0f) { ci->heading = 0.0f; }
            else if (kh > 0.0f) {
                if (ci->heading < 0.0f) ci->heading = 0.0f;
            }
            else { // kh < 0
                if (ci->heading > 0.0f) ci->heading = 0.0f;
            }
        ...
        kh = (check_control_timef (PITCH_FORWARD) -
              check_control_timef (PITCH_BACK)) /
             8.0f;
        if (kh == 0.0f) { ci->pitch = 0.0f; }
        else if (kh > 0.0f) {
            if (ci->pitch < 0.0f) ci->pitch = 0.0f;
        }
        else { // kh < 0
            if (ci->pitch > 0.0f) ci->pitch = 0.0f;
        }
    ...
    if (!(Game_mode & GM_DEAD)) {
            ...
            if (Player->ci.forward_cruise_percent < 33.3f) {
                snd_play (
                    gamesnd_get_game_sound (
                        ship_get_sound (Player_obj, GameSounds::THROTTLE_UP)),
                    0.0f);
            }
            else if (Player->ci.forward_cruise_percent > 33.3f) {
                snd_play (
                    gamesnd_get_game_sound (ship_get_sound (
                        Player_obj, GameSounds::THROTTLE_DOWN)),
                    0.0f);
            }
        ...
        if (button_info_query (&Player->bi, ZERO_THROTTLE)) {
            ...
            if (ci->forward_cruise_percent > 0.0f &&
                Player_obj->phys_info.fspeed > 0.5) {
                snd_play (
                    gamesnd_get_game_sound (ship_get_sound (
                        Player_obj, GameSounds::ZERO_THROTTLE)),
                    0.0f);
            }
        ...
        if (button_info_query (&Player->bi, MAX_THROTTLE)) {
            ...
            if (ci->forward_cruise_percent < 100.0f) {
                snd_play (
                    gamesnd_get_game_sound (ship_get_sound (
                        Player_obj, GameSounds::FULL_THROTTLE)),
                    0.0f);
            }
        ...
        if (Player_ship != nullptr && Player_ship->ship_info_index >= 0) {
            float z_min =
                Ship_info[Player_ship->ship_info_index].min_vel.xyz.z;
            if (z_min > 0.0f && pi->speed <= z_min) {
                ci->forward = MAX (ci->forward, 0.0f);
                ci->forward_cruise_percent = MAX (
                    ci->forward_cruise_percent,
                    z_min /
                        Ship_info[Player_ship->ship_info_index].max_vel.xyz.z *
                        100.0f);
                override_analog_throttle = 1;
            }
        }

        // AL 12-29-97: If afterburner key is down, player should have full
        // forward thrust (even if afterburners run out)
        if (check_control (AFTERBURNER)) { ci->forward = 1.0f; }
        ...
        if (Player->flags & PLAYER_FLAGS_MATCH_TARGET) {
            if ((Player_ai->last_target == Player_ai->target_objnum) &&
                ...
                tspeed = targeted_objp->phys_info
                             .speed; // changed from fspeed. If target is
                                     // reversing, sliding, or gliding we still
                                     // want to keep up. -- Backslash
                ...
                if (tspeed < MATCH_SPEED_THRESHOLD) {
                    ...
                    if (object_is_docked (targeted_objp)) {
                        tspeed = dock_calc_docked_speed (
                            targeted_objp); // changed from fspeed
                    }
                }
                ...
                if (!((Objects[Player->objnum].phys_info.flags & PF_GLIDING) ||
                      (Objects[Player->objnum].phys_info.flags &
                       PF_FORCE_GLIDE))) {
                    pmax_speed = Ships[Player_obj->instance].current_max_speed;
                    if (pmax_speed > 0.0f) {
                        ci->forward_cruise_percent =
                            (tspeed / pmax_speed) * 100.0f;
                    }
                    else {
                        ci->forward_cruise_percent = 0.0f;
                    }
        ...
        // code to read joystick axis for pitch/heading.  Code to read joystick
        // buttons for bank.
        if (!(Game_mode & GM_DEAD)) {
            playercontrol_read_stick (axis, frame_time);
        }
        ...
        if (Axis_map_to[JOY_ABS_THROTTLE_AXIS] >= 0) {
            scaled = (float)axis[JOY_ABS_THROTTLE_AXIS] * 1.2f / (float)F1_0 -
            ...
            if (!override_analog_throttle || (delta < -1.5f) ||
                (delta > 1.5f)) {
        ...
        if (Axis_map_to[JOY_REL_THROTTLE_AXIS] >= 0)
            ci->forward_cruise_percent +=
                f2fl (axis[JOY_REL_THROTTLE_AXIS]) * 100.0f * frame_time;

        CLAMP (ci->forward_cruise_percent, 0, 100);

        // set up the firing stuff.  Read into control info ala Descent so that
        // weapons will be created during the object simulation phase, and not
        // immediately as was happening before.

        // keyboard: fire the current primary weapon
        if (check_control (FIRE_PRIMARY)) { ci->fire_primary_count++; }

        // for debugging, check to see if the debug key is down -- if so, make
        // fire the debug laser instead
#ifndef NDEBUG
        ...
        if (check_control (AFTERBURNER) && !Player_use_ai) {
            ...
            if (afterburner_last) ci->afterburner_stop = 1;

            afterburner_last = 0;
        }

        // new gliding systems combining code by Backslash, Turey, Kazan, and
        // WMCoolmon
        ...
        // This logic is a bit tricky. It checks to see if the
        ...
        if (check_control (GLIDE_WHEN_PRESSED) != press_glide) {
            if (Player_obj != NULL &&
                Ship_info[Player_ship->ship_info_index].can_glide) {
                // This only works if check_control returns only 1 or 0.
                // Shouldn't be a problem, but this comment's here just in case
                // it is.
        ...
        if (toggle_glide || press_glide) {
            ...
            if (Player_obj != NULL &&
                ...
                if (toggle_glide && press_glide) {
                    // Overkill -- if gliding is toggled on and
                    // glide_when_pressed is pressed, turn glide off
                    if (object_get_gliding (Player_obj) &&
                        !object_glide_forced (Player_obj)) {
                        object_set_gliding (Player_obj, false);
                        ci->forward_cruise_percent = savedspeed;
                        press_glide = !press_glide;
                        snd_play (
                            gamesnd_get_game_sound (ship_get_sound (
                                Player_obj, GameSounds::THROTTLE_UP)),
                            0.0f);
                    }
                }
                    ...
                    if (Ship_info[Player_ship->ship_info_index]
                        ...
                        // If glide_start_snd wasn't set (probably == 0), use
                        // the default throttle down sound
                        snd_play (
                            gamesnd_get_game_sound (ship_get_sound (
                                Player_obj, GameSounds::THROTTLE_DOWN)),
                            0.0f);
            ...
            // Probably don't need to do the second half of this check, but
            // just in case...
            if (Player_obj != NULL &&
                Ship_info[Player_ship->ship_info_index].can_glide) {
                // Only bother doing this if we need to.
                if (object_get_gliding (Player_obj) &&
                    ...
                    if (Ship_info[Player_ship->ship_info_index]
                        ...
                        // If glide_end_snd wasn't set (probably == 0), use the
                        // default throttle up sound
                        snd_play (
                            gamesnd_get_game_sound (ship_get_sound (
                                Player_obj, GameSounds::THROTTLE_UP)),
                            0.0f);
...
void read_player_controls (object* objp, float frametime) {
    ...
    {
        memset (
            &(Player->ci), 0, sizeof (control_info)); // set the controls to 0
        ...
        if ((objp->type == OBJ_SHIP) && (!(Game_mode & GM_DEAD))) {
            ...
            if (!(Warpout_forced) &&
                (Ships[objp->instance].flags[Ship::Ship_Flags::Warp_broken] ||
                 Ships[objp->instance].flags[Ship::Ship_Flags::Warp_never])) {
                HUD_sourced_printf (
                    HUD_SOURCE_HIDDEN, "%s",
                    XSTR ("Cannot warp out at this time.", 81));
                snd_play (
                    gamesnd_get_game_sound (GameSounds::PLAYER_WARP_FAIL));
                gameseq_post_event (GS_EVENT_PLAYER_WARPOUT_STOP);
            }
            else {
                if (Warpout_forced) {
                    Ships[objp->instance].current_max_speed =
                        target_warpout_speed * 2.0f;
                }
                else if (
                    Ships[objp->instance].current_max_speed <
                    target_warpout_speed) {
                    Ships[objp->instance].current_max_speed =
                        target_warpout_speed + 5.0f;
                }
            ...
            if (Player->control_mode == PCM_WARPOUT_STAGE1) {
                ...
                // Wait at least 3 seconds before making sure warp speed is
                // set.
                if (Warpout_time > warpout_delay) {
                    // If we are going around 5% of the target speed, progress
                    // to next stage
    ...
    if (Player_obj->type == OBJ_SHIP && !Player_use_ai) {
        ...
        if (!(Ships[Player_obj->instance].flags[Ship::Ship_Flags::Dying])) {
            ...
            if ((Ships[objp->instance].wash_intensity > 0) &&
                ...
                vm_vec_copy_scale (
                    &wash_rot, &Ships[objp->instance].wash_rot_axis,
                    intensity);
                physics_read_flying_controls (
                    &objp->orient, &objp->phys_info, &(Player->ci),
                    flFrametime, &wash_rot);
            }
            else {
                physics_read_flying_controls (
                    &objp->orient, &objp->phys_info, &(Player->ci),
        ...
        physics_read_flying_controls (
            &objp->orient, &objp->phys_info, &(Player->ci), flFrametime);
    }
}
...
/**
 * Computes the forward_thrust_time needed for the player ship to match
 * velocities with the currently selected target
 *
 * @param no_target_text Default parm (NULL), used to override HUD output when
 * no target exists
...
void player_match_target_speed (
    ...
    if (targeted_objp->type != OBJ_SHIP) { return; }
    ...
    if (Player->flags & PLAYER_FLAGS_MATCH_TARGET) {
        ...
        if (targeted_objp->phys_info.fspeed > MATCH_SPEED_THRESHOLD) {
            ...
            if (object_is_docked (targeted_objp)) {
                if (dock_calc_docked_fspeed (targeted_objp) >
        ...
        if (can_match) {
            ...
            if (match_on_text) {
                if (match_on_text[0]) {
                    HUD_sourced_printf (
                        HUD_SOURCE_HIDDEN, "%s", match_on_text);
                }
            }
            else {
...
// toggle_player_object toggles between the player objects (i.e. the ship they
// are currently flying) and a descent style ship.
...
void toggle_player_object () {
    if (use_descent) {
        ...
        memcpy (
            &Player_obj->phys_info, &Descent_physics, sizeof (physics_info));
    }
    use_descent = !use_descent;

    HUD_sourced_printf (
        HUD_SOURCE_HIDDEN, NOX ("Using %s style physics for player ship."),
        use_descent ? NOX ("DESCENT") : NOX ("FreeSpace"));
}

/**
 * Initialise the data required for determining whether 'all alone' message
 * should play
 */
void player_init_all_alone_msg () {
    ...
    for (so = GET_FIRST (&Ship_obj_list); so != END_OF_LIST (&Ship_obj_list);
        ...
        if (objp == Player_obj) { continue; }

        if (Ships[objp->instance].team == Player_ship->team) {
            int ship_type =
                Ship_info[Ships[objp->instance].ship_info_index].class_type;
            if (ship_type != -1 &&
                (Ship_types[ship_type]
                     .flags[Ship::Type_Info_Flags::Counts_for_alone])) {
                return;
            }
        }
    }

    // There must be no friendly ships present, so prevent the 'all alone'
    // message from ever playing
...
void player_restore_target_and_weapon_link_prefs () {
    ...
    if (Player->flags & PLAYER_FLAGS_LINK_PRIMARY &&
        !(player_sip->flags[Ship::Info_Flags::No_primary_linking])) {
    ...
    if (Player->flags & PLAYER_FLAGS_LINK_SECONDARY &&
        (pm->n_missiles > 0 && pm->missile_banks[0].num_slots > 1)) {
...
void player_level_init () {
    ...
    Player->allow_warn_timestamp =
        1; // init timestamp that is used for managing attack warnings sent to
           // player
    Player->check_warn_timestamp = 1;
    Player->warn_count =
        0; // number of attack warnings player has received this mission

    Player->distance_warning_count =
        0;                             // Number of warning too far from origin
    Player->distance_warning_time = 0; // Time at which last warning was given

    Player->praise_count =
        0; // number of praises player has received this mission
    Player->allow_praise_timestamp =
        1; // timestamp until next praise is allowed
    Player->praise_delay_timestamp =
        0; // timstamp used to delay praises given to the player

    Player->ask_help_count =
        0; // number of times player has been asked for help by wingmen
    Player->allow_ask_help_timestamp =
        1; // timestamp until next ask_help is allowed

    Player->scream_count =
        0; // number of times player has heard wingman screams this mission
    Player->allow_scream_timestamp =
        1; // timestamp until next wingman scream is allowed

    Player->low_ammo_complaint_count =
        0; // number of complaints about low ammo received in this mission
    Player->allow_ammo_timestamp =
        1; // timestamp until next 'Ammo low' message can be played

    Player->praise_self_count =
        0; // number of boasts about kills received in this mission
    Player->praise_self_timestamp =
        1; // timestamp marking time until next boast is allowed

    Player->request_repair_timestamp = 1; // timestamp until next 'requesting
                                          // repair sir' message can be played

    Player->repair_sound_loop = sound_handle::invalid ();
    Player->cargo_scan_loop = sound_handle::invalid ();
    Player->cargo_inspect_time =
        0; // time that current target's cargo has been inspected for

    Player->target_is_dying =
        -1; // The player target is dying, set to -1 if no target
    Player->current_target_sx =
        -1; // Screen x-pos of current target (or subsystem if applicable)
    Player->current_target_sy =
        -1; // Screen y-pos of current target (or subsystem if applicable)
    Player->target_in_lock_cone =
        -1; // Is the current target in secondary weapon lock cone?
    Player->locking_subsys =
        NULL; // Subsystem pointer that missile lock is trying to seek
    Player->locking_on_center =
        0; // boolean, whether missile lock is trying for center of ship or not
    Player->locking_subsys_parent = -1;

    Player->killer_objtype = -1; // type of object that killed player
    Player->killer_weapon_index =
        -1; // weapon used to kill player (if applicable)
...
/**
 * Stop any looping sounds associated with the Player, called from
...
/**
 * Start the repair sound if it hasn't already been started.  Called when a
...
/**
 * @brief See if there is a praise message to deliver to the player.  We want
...
int player_process_pending_praise () {
    ...
    if (timestamp_elapsed (Player->praise_delay_timestamp)) {
        ...
        ship_index =
            ship_get_random_player_wing_ship (SHIP_GET_UNSILENCED, 1000.0f);
        if (ship_index >= 0) {
            // Only praise if above 50% integrity
            if (get_hull_pct (&Objects[Ships[ship_index].objnum]) > 0.5f) {
                if (Player->stats.m_kill_count_ok >
                    10) { // this number should probably be in the AI profile
                          // or mission file rather than hardcoded
                    message_send_builtin_to_player (
                        MESSAGE_HIGH_PRAISE, &Ships[ship_index],
                        MESSAGE_PRIORITY_HIGH, MESSAGE_TIME_SOON, 0, 0, -1,
                        -1);
                }
                else {
                    message_send_builtin_to_player (
                        MESSAGE_PRAISE, &Ships[ship_index],
                        MESSAGE_PRIORITY_HIGH, MESSAGE_TIME_SOON, 0, 0, -1,
                        -1);
                }
                Player->allow_praise_timestamp = timestamp (
                    Builtin_messages[MESSAGE_PRAISE].min_delay *
                    (Game_skill_level + 1));
                Player->allow_scream_timestamp =
                    timestamp (20000); // prevent death scream following praise
                Player->praise_count++;
                return 1;
            }
        }
    ...
    if (Player->praise_delay_timestamp == 0) { return 0; }
...
int player_inspect_cargo (float frametime, char* outstr) {
    ...
    // MWA -- 1/27/98 -- added fighters/bombers to this list.  For multiplayer,
    // we want to show callsign of player G5K -- 10/20/08 -- moved the callsign
    // code into hud_stuff_ship_callsign, where it makes more sense

    // scannable cargo behaves differently.  Scannable cargo is either
    // "scanned" or "not scanned".  This flag can be set on any ship.  Any ship
    // with this set won't have "normal" cargo behavior
    if (!(cargo_sp->flags[Ship::Ship_Flags::Scannable])) {
        if (!(cargo_sip->flags[Ship::Info_Flags::Cargo] ||
    ...
    if (cargo_sp->flags[Ship::Ship_Flags::Cargo_revealed]) {
        ...
        // always bash cargo_inspect_time to 0 since AI ships can reveal cargo
        // that we are in the process of scanning
    ...
    if (Player_ai->current_target_distance <
        MAX (
            player_sip->scan_range_normal,
            (cargo_objp->radius + player_sip->scan_range_normal -
             CARGO_RADIUS_REAL_DELTA))) {
        // check if player is facing cargo, do not proceed with inspection if
        // not
        vm_vec_normalized_dir (
            &vec_to_cargo, &cargo_objp->pos, &Player_obj->pos);
        ...
        // player is facing the cargo, and withing range, so proceed with
        // inspection
...
/**
 * @return 1 if player should display outstr on HUD; 0 if don't display cargo
 * on HUD
 */
int player_inspect_cap_subsys_cargo (float frametime, char* outstr) {
    object* cargo_objp;
    ...
    // don't do any sort of scanning thing unless capship has a non-"nothing"
    // cargo this compensates for changing the "no display" index from -1 to 0
    if (subsys->subsys_cargo_name == 0) { return 0; }
    ...
    get_subsystem_world_pos (
        cargo_objp, Player_ai->targeted_subsys, &subsys_pos);
    ...
    if (cargo_sip->is_huge_ship ()) {
        scan_dist = MAX (
            player_sip->scan_range_capital,
            (subsys_rad + player_sip->scan_range_capital -
             CARGO_RADIUS_REAL_DELTA));
    }
    else {
        scan_dist = MAX (
            player_sip->scan_range_normal,
            (subsys_rad + player_sip->scan_range_normal -
             CARGO_RADIUS_REAL_DELTA));
    }
    ...
    if (Player_ai->current_target_distance < scan_dist) {
        ...
        dot = vm_vec_dot (&vec_to_cargo, &Player_obj->orient.vec.fvec);
        int hud_targetbox_subsystem_in_view (
            object * target_objp, int* sx, int* sy);
        ...
        // player is facing the cargo, and within range, so proceed with
        // inspection
        if (hud_sensors_ok (Player_ship, 0)) {
            Player->cargo_inspect_time += (int)std::lround (frametime * 1000);
        }
 ...
 * function
 */
void player_generate_killer_weapon_name (
    int weapon_info_index, int killer_species, char* weapon_name) {
    if (weapon_info_index < 0) { return; }

#ifndef NDEBUG
    if (Show_killer_weapon ||
        (killer_species == Ship_info[Player_ship->ship_info_index].species)) {
#else
    if (killer_species == Ship_info[Player_ship->ship_info_index].species) {
#endif
        strcpy (
            weapon_name, Weapon_info[weapon_info_index].get_display_string ());
    }
    else {
...
/**
 * Generates the message for death of a player given the information stored in
 * the player object.
 */
void player_generate_death_message (player* player_p) {
    ...
    player_generate_killer_weapon_name (
        player_p->killer_weapon_index, player_p->killer_species, weapon_name);

    switch (player_p->killer_objtype) {
    case OBJ_SHOCKWAVE:
        if (weapon_name[0]) {
            sprintf (
                msg, XSTR ("%s was killed by a missile shockwave", 92),
                player_p->callsign);
        }
        else {
            sprintf (
                msg,
                XSTR ("%s was killed by a shockwave from %s exploding", 93),
                player_p->callsign, player_p->killer_parent_name);
        }
        break;
    ...
    case OBJ_WEAPON:
        ...
        if ((ship_index >= 0) && (Player_ship != NULL) &&
            (Player_ship->team == Ships[ship_index].team)) {
            sprintf (
                msg, XSTR ("%s was killed by friendly fire from %s", 1338),
                player_p->callsign, player_p->killer_parent_name);
        }
        else {
            sprintf (
                msg, XSTR ("%s was killed by %s", 94), player_p->callsign,
    ...
    case OBJ_SHIP:
        if (player_p->flags & PLAYER_FLAGS_KILLED_BY_EXPLOSION) {
            sprintf (
                msg, XSTR ("%s was killed by a blast from %s exploding", 95),
                player_p->callsign, player_p->killer_parent_name);
        }
        else if (player_p->flags & PLAYER_FLAGS_KILLED_BY_ENGINE_WASH) {
            sprintf (
                msg, XSTR ("%s was killed by engine wash from %s", 1494),
                player_p->callsign, player_p->killer_parent_name);
        }
        else {
            sprintf (
                msg, XSTR ("%s was killed by a collision with %s", 96),
                player_p->callsign, player_p->killer_parent_name);
        }
        break;
    ...
    case OBJ_DEBRIS:
        sprintf (
            msg, XSTR ("%s was killed by a collision with debris", 97),
            player_p->callsign);
        break;

    case OBJ_ASTEROID:
        sprintf (
            msg, XSTR ("%s was killed by a collision with an asteroid", 98),
            player_p->callsign);
        break;
    ...
    case OBJ_BEAM:
        if (strlen (player_p->killer_parent_name) <= 0) {
            ...
            if ((ship_index >= 0) && (Player_ship != NULL) &&
                ...
                sprintf (
                    msg, XSTR ("%s was destroyed by a beam from %s", 1082),
                    player_p->callsign, player_p->killer_parent_name);
            }
        }
        break;
    ...
    default:
        sprintf (
            msg, XSTR ("%s was killed by unknown causes", 99),
            player_p->callsign);
        break;
    }
}
    ...
    if (msg.empty ()) {
        ...
        if (Player->flags & PLAYER_KILLED_SELF) {
            ...
            if (Player->flags & PLAYER_FLAGS_KILLED_SELF_SHOCKWAVE) {
                msg = XSTR (
                    "You have killed yourself with a shockwave from your own "
                    "weapon",
                    1421);
            }
...
void player_maybe_play_all_alone_msg () {
    ...
    if (Player->flags & PLAYER_FLAGS_NO_CHECK_ALL_ALONE_MSG) { return; }

    // only check every N seconds
    if (!timestamp_elapsed (Player->check_for_all_alone_msg)) { return; }
    ...
    if (rand () & 1) {
        message_send_builtin_to_player (
            MESSAGE_ALL_ALONE, NULL, MESSAGE_PRIORITY_HIGH,
...
void player_get_padlock_orient (matrix* eye_orient) {
    ...
    if (Viewer_mode & VM_PADLOCK_UP) {
        ...
        vm_vec_copy_scale (
            &eye_orient->vec.uvec, &old_eye_orient.vec.fvec, -1.0f);
    }
    else if (Viewer_mode & VM_PADLOCK_REAR) {
        ...
        vm_vec_copy_scale (
            &eye_orient->vec.fvec, &old_eye_orient.vec.rvec, -1.0f);
...
void player_display_padlock_view () {
    ...
    if (Viewer_mode & VM_PADLOCK_UP) { padlock_view_index = 0; }
    else if (Viewer_mode & VM_PADLOCK_REAR) {
    ...
    if (!(Viewer_mode & (VM_CHASE | VM_EXTERNAL))) {
        ...
        case 0: strcpy_s (str, XSTR ("top view", 101)); break;
        case 1: strcpy_s (str, XSTR ("rear view", 102)); break;
        case 2: strcpy_s (str, XSTR ("left view", 103)); break;
        case 3: strcpy_s (str, XSTR ("right view", 104)); break;
        }
        color col;
...
camid player_get_cam () {
    ...
    if (Player_obj == NULL) { return camid (); }

    // standalone servers can bail here
    if (Game_mode & GM_STANDALONE_SERVER) { return camid (); }

    // if we're not in-mission, don't do this
    if (!(Game_mode & GM_IN_MISSION)) { return camid (); }
    ...
    if (!(Game_mode & (GM_DEAD | GM_DEAD_BLEW_UP))) {
        ...
        if (Viewer_mode & VM_FREECAMERA) {
            ...
            vm_vec_scale_add (
                ...
                -3.0f * viewer_obj->radius - Viewer_chase_info.distance);
            vm_vec_scale_add2 (
                &eye_pos, &viewer_obj->orient.vec.uvec,
                0.75f * viewer_obj->radius);
            vm_vec_sub (&tmp_dir, &viewer_obj->pos, &eye_pos);
            vm_vec_normalize (&tmp_dir);

            // JAS: I added the following code because if you slew up using
            // Descent-style physics, eye_dir and Viewer_obj->orient.vec.uvec
            // are equal, which causes a zero-length vector in the
            // vm_vector_2_matrix call because the up and the forward vector
            // are the same.   I fixed it by adding in a fraction of the right
            // vector all the time to the up vector.
            vec3d tmp_up = viewer_obj->orient.vec.uvec;
            vm_vec_scale_add2 (
                &tmp_up, &viewer_obj->orient.vec.rvec, 0.00001f);
