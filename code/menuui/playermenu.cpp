...
{
	PlayerInfo info;
	if (!player_get_last_player_info(info)) {
		return 0;
	}

	strcpy_s(Player_select_last_pilot, info.callsign.c_str());
	Player_select_last_is_multi = info.is_multi ? 1 : 0;
...
//	sprintf(Copyright_msg1, NOX("FreeSpace 2"));
	...
	gr_get_string_size(&w, NULL, Copyright_msg1.c_str());
...
bool player_get_last_player_info(PlayerInfo& infoOut)
{
	const char* last_player = os_config_read_string(NULL, "LastPlayer", nullptr);
	...
	player plr;
	plr.reset();
	if (!Pilot.load_player(last_player, &plr)) {
		return false;
	}

	infoOut.callsign = last_player;
	infoOut.is_multi = plr.player_was_multi != 0;
