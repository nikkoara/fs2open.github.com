...
void fish_generate () {
    ...
    if (!Fish_inited) { return; }

    // bogus anims
    if ((Fish_left_anim == NULL) || (Fish_right_anim == NULL)) { return; }

    // find a free fish
    f = NULL;
    for (idx = 0; idx < MAX_FISH; idx++) {
        if (!Fish[idx].swimming) { f = &Fish[idx]; }
    }
...
void fishtank_start () {
    ...
    if (Fish_left_anim == NULL) { return; }
    Fish_right_anim = anim_load (FISH_RIGHT_ANIM_NAME);
    if (Fish_right_anim == NULL) { return; }
...
void fishtank_process () {
    ...
    for (idx = 0; idx < MAX_FISH; idx++) {
        ...
        onscreen = 0;
        if ((f->x < (float)gr_screen.max_w_unscaled_zoomed) &&
            ((f->x + FISH_ANIM_WIDTH) >= 0.0f) &&
            (f->y < (float)gr_screen.max_h_unscaled_zoomed) &&
            ((f->y + FISH_ANIM_HEIGHT) >= 0.0f)) {
            onscreen = 1;
        }

        // if he was onscreen before, but is no longer, flush him and make a
        // new fish
