...
void timer_close () {
    if (Timer_inited) { Timer_inited = 0; }
}
...
void timer_init () {
    if (!Timer_inited) {
        ...
        Timer_to_nanoseconds = (long double)NANOSECONDS_PER_SECOND /
                               (long double)Timer_perf_counter_freq;
        Timer_to_microseconds = (long double)MICROSECONDS_PER_SECOND /
                                (long double)Timer_perf_counter_freq;
        Timer_inited = 1;
...
void timestamp_reset () { timestamp_ticker = 2; }
...
void timestamp_inc (fix frametime) {
    // Compute the microseconds, assumes that a fix uses the lower 16 bit for
...
// alternate timestamp functions.  The way these work is you call xtimestamp()
// to get the current counter value, and then call
int timestamp () { return timestamp_ms (); }
...
int timestamp_has_time_elapsed (int stamp, int time) {
    ...
    t = stamp + time;
    if (t <= timestamp_ms ())
        return 1; // if we are unlucky enough to have it wrap on us, this will
                  // assume time has elapsed.
