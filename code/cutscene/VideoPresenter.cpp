...
VideoPresenter::VideoPresenter (const MovieProperties& props)
    ...
    case FramePixelFormat::YUV420:
        ...
        _planeTextureHandles[0] =
            bm_create (8, w, h, _planeTextureBuffers[0].get (), BMP_AABITMAP);

        _planeTextureBuffers[1].reset (new uint8_t[w / 2 * h / 2]);
        _planeTextureHandles[1] = bm_create (
            8, w / 2, h / 2, _planeTextureBuffers[1].get (), BMP_AABITMAP);

        _planeTextureBuffers[2].reset (new uint8_t[w / 2 * h / 2]);
        _planeTextureHandles[2] = bm_create (
            8, w / 2, h / 2, _planeTextureBuffers[2].get (), BMP_AABITMAP);

        material_set_movie (
            &_movie_material, _planeTextureHandles[0], _planeTextureHandles[1],
            _planeTextureHandles[2]);
        break;
    case FramePixelFormat::BGR:
        _planeTextureBuffers[0].reset (new uint8_t[w * h * 3]);
        _planeTextureHandles[0] =
            bm_create (24, w, h, _planeTextureBuffers[0].get (), 0);

        material_set_unlit (
            &_rgb_material, _planeTextureHandles[0], 1.0f, false, false);
        break;
    case FramePixelFormat::BGRA:
        _planeTextureBuffers[0].reset (new uint8_t[w * h * 4]);
        _planeTextureHandles[0] =
            bm_create (32, w, h, _planeTextureBuffers[0].get (), 0);
...
void VideoPresenter::uploadVideoFrame (const VideoFramePtr& frame) {
    ...
    for (size_t i = 0; i < frame->getPlaneNumber (); ++i) {
        ...
        memcpy (
            _planeTextureBuffers[i].get (), data, size.stride * size.height);
        ...
        gr_update_texture (
            _planeTextureHandles[i], bpp, _planeTextureBuffers[i].get (),
...
void VideoPresenter::displayFrame (float x1, float y1, float x2, float y2) {
    ...
    // Use the immediate buffer for storing our data since that is exactly what
    // we need
    auto offset =
        gr_add_to_immediate_buffer (sizeof (vertex_data), vertex_data);

    vertex_layout layout;
    layout.add_vertex_component (
        vertex_format_data::POSITION2, sizeof (vertex_data[0]),
        offsetof (movie_vertex, pos));
    layout.add_vertex_component (
        vertex_format_data::TEX_COORD2, sizeof (vertex_data[0]),
        offsetof (movie_vertex, uv));

    if (_properties.pixelFormat == FramePixelFormat::YUV420) {
        gr_render_movie (
            &_movie_material, PRIM_TYPE_TRISTRIP, &layout, 4,
            gr_immediate_buffer_handle, offset);
    }
    else {
        gr_render_primitives (
            &_rgb_material, PRIM_TYPE_TRISTRIP, &layout, 0, 4,
            gr_immediate_buffer_handle, offset);
    }
}
