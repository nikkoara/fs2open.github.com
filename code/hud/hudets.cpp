...
// -------------------------------------------------------------------------------------------------
// ets_init_ship() is called by a ship when it is created (effectively, for
// every ship at the start of a mission).  This will set the default charge
// rates for the different systems and initialize the weapon energy reserve.
//
...
// -------------------------------------------------------------------------------------------------
// update_ets() is called once per frame for every OBJ_SHIP in the game.  The
// amount of energy to send to the weapons and shields is calculated, and the
// top ship speed is calculated.  The amount of time elapsed from the previous
...
void update_ets (object* objp, float fl_frametime) {
    ...
    max_new_weapon_energy =
        fl_frametime * sinfo_p->max_weapon_regen_per_second * max_g;
    if (objp->flags[Object::Object_Flags::Player_ship]) {
        ship_p->weapon_energy +=
            Energy_levels[ship_p->weapon_recharge_index] *
            max_new_weapon_energy *
            The_mission.ai_profile->weapon_energy_scale[Game_skill_level];
    }
    else {
        ship_p->weapon_energy += Energy_levels[ship_p->weapon_recharge_index] *
                                 max_new_weapon_energy;
    }
    ...
    max_new_shield_energy =
        fl_frametime * sinfo_p->max_shield_regen_per_second *
        shield_get_max_strength (
            objp, true); // recharge rate is unaffected by $Max Shield Recharge
    if (objp->flags[Object::Object_Flags::Player_ship]) {
        shield_delta =
            Energy_levels[ship_p->shield_recharge_index] *
            max_new_shield_energy *
            The_mission.ai_profile->shield_energy_scale[Game_skill_level];
    }
    else {
    ...
    // don't let engine strength affect max speed when playing on lowest skill
    // level
...
float ets_get_max_speed (object* objp, float engine_energy) {
    ...
    if (engine_energy == Energy_levels[INTIAL_ENGINE_RECHARGE_INDEX]) {
        ...
        // do a linear interpolation to find the current max speed, using
        // points (0,1/2 default_max_speed) (.333,default_max_speed) x = x1 +
        // (y-y1) * (x2-x1) / (y2-y1);
        if (engine_energy < Energy_levels[INTIAL_ENGINE_RECHARGE_INDEX]) {
            return 0.5f * sip->max_speed +
                   (engine_energy * (0.5f * sip->max_speed)) /
                       Energy_levels[INTIAL_ENGINE_RECHARGE_INDEX];
        }
        else {
            // do a linear interpolation to find the current max speed, using
            // points (.333,default_max_speed) (1,max_overclock_speed)
            return sip->max_speed +
                   (engine_energy -
                    Energy_levels[INTIAL_ENGINE_RECHARGE_INDEX]) *
                       (sip->max_overclocked_speed - sip->max_speed) /
                       (1.0f - Energy_levels[INTIAL_ENGINE_RECHARGE_INDEX]);
        }
    }
}

// -------------------------------------------------------------------------------------------------
// ai_manage_ets() will determine if a ship should modify it's energy transfer
// percentages, or transfer energy from shields->weapons or from
// weapons->shields
//
...
void ai_manage_ets (object* obj) {
    ...
    if (ship_info_p->power_output == 0) return;

    if (ship_p->flags[Ship::Ship_Flags::Dying]) return;

    // check if any of the three systems are not being used.  If so, don't
    // allow energy management.
    if (!ship_p->ship_max_shield_strength || !ship_info_p->max_speed ||
        !ship_info_p->max_weapon_reserve)
        return;

    float shield_left_percent = get_shield_pct (obj);
    float weapon_left_percent =
    ...
    if (!(obj->flags[Object::Object_Flags::No_shields]) &&
        (shield_left_percent < SHIELDS_MIN_LEVEL_PERCENT)) {
    ...
    if (!(obj->flags[Object::Object_Flags::No_shields])) {
        if (shield_left_percent < SHIELDS_EMERG_LEVEL_PERCENT) {
            ...
            if (shield_left_percent > SHIELDS_MIN_LEVEL_PERCENT ||
                weapon_left_percent <=
                    0.01) // dampen ai enthusiasm for sucking energy to weapons
...
// -------------------------------------------------------------------------------------------------
// set_default_recharge_rates() will set the charge levels for the weapons,
// shields and engines to their default levels
void set_default_recharge_rates (object* obj) {
    ...
    ship_properties = 0;
    if (ship_has_energy_weapons (ship_p)) ship_properties |= HAS_WEAPONS;

    if (!(obj->flags[Object::Object_Flags::No_shields]))
        ship_properties |= HAS_SHIELDS;

    if (ship_has_engine_power (ship_p)) ship_properties |= HAS_ENGINES;
...
// -------------------------------------------------------------------------------------------------
// increase_recharge_rate() will increase the energy flow to the specified
// system (one of WEAPONS, SHIELDS or ENGINES).  The increase in energy will
// result in a decrease to the other two systems.
void increase_recharge_rate (object* obj, SYSTEM_TYPE ship_system) {
    int *gain_index = NULL, *lose_index1 = NULL, *lose_index2 = NULL,
        *tmp = NULL;
    int count = 0;
    ship* ship_p = &Ships[obj->instance];

    if (ship_p->flags[Ship::Ship_Flags::No_ets]) return;

    switch (ship_system) {
    case WEAPONS:
        if (!ship_has_energy_weapons (ship_p)) return;
    ...
    case SHIELDS:
        if (obj->flags[Object::Object_Flags::No_shields]) return;
    ...
    case ENGINES:
        if (!ship_has_engine_power (ship_p)) return;
    ...
    if (!lose_index1 && !lose_index2) return;
    ...
    if (count <= 0) {
        if (obj == Player_obj) {
            snd_play (
                gamesnd_get_game_sound (GameSounds::ENERGY_TRANS_FAIL), 0.0f);
        }
        return;
    }
...
// -------------------------------------------------------------------------------------------------
// decrease_recharge_rate() will decrease the energy flow to the specified
// system (one of WEAPONS, SHIELDS or ENGINES).  The decrease in energy will
// result in an increase to the other two systems.
void decrease_recharge_rate (object* obj, SYSTEM_TYPE ship_system) {
    int *lose_index = NULL, *gain_index1 = NULL, *gain_index2 = NULL,
        *tmp = NULL;
    int count;
    ship* ship_p = &Ships[obj->instance];
    ...
    // return if we can't transfer energy
    if (!gain_index1 && !gain_index2) return;
...
void transfer_energy_weapon_common (
    object* objp, float from_field, float to_field, float* from_delta,
    float* to_delta, float max, float scale) {
    float delta;

    delta = from_field * ENERGY_DIVERT_DELTA * scale;

    if (to_field + *to_delta + delta > max) delta = max - to_field - *to_delta;

    if (delta > 0) {
        if (objp == Player_obj)
            snd_play (gamesnd_get_game_sound (GameSounds::ENERGY_TRANS), 0.0f);

        if (delta > from_field) delta = from_field;
...
// -------------------------------------------------------------------------------------------------
// transfer_energy_to_shields() will transfer ENERGY_DIVERT_DELTA percent of
// weapon energy to shield energy.
void transfer_energy_to_shields (object* obj) {
    ...
    if (!ship_has_energy_weapons (ship_p) ||
        obj->flags[Object::Object_Flags::No_shields]) {
        return;
    }

    transfer_energy_weapon_common (
        obj, ship_p->weapon_energy, shield_get_strength (obj),
        &ship_p->target_weapon_energy_delta, &ship_p->target_shields_delta,
        shield_get_max_strength (obj), 0.5f);
}

// -------------------------------------------------------------------------------------------------
// transfer_energy_to_weapons() will transfer ENERGY_DIVERT_DELTA percent of
// shield energy to weapon energy.
void transfer_energy_to_weapons (object* obj) {
    ...
    transfer_energy_weapon_common (
        obj, shield_get_strength (obj), ship_p->weapon_energy,
        &ship_p->target_shields_delta, &ship_p->target_weapon_energy_delta,
...
void sanity_check_ets_inputs (int (&ets_indexes)[num_retail_ets_gauges]) {
    int i;
    int ets_delta = MAX_ENERGY_INDEX - ets_indexes[ENGINES] -
                    ets_indexes[SHIELDS] - ets_indexes[WEAPONS];
    if (ets_delta != 0) {
 ...
 */
bool validate_ship_ets_indxes (
    const int& ship_idx, int (&ets_indexes)[num_retail_ets_gauges]) {
    if (ship_idx < 0) { return false; }
    if (Ships[ship_idx].objnum < 0) { return false; }
    ship* ship_p = &Ships[ship_idx];

    if (ship_p->flags[Ship::Ship_Flags::No_ets]) return false;
    ...
    case HAS_ENGINES | HAS_SHIELDS:
        zero_one_ets (
            &ets_indexes[WEAPONS], &ets_indexes[ENGINES],
            &ets_indexes[SHIELDS]);
        break;

    case HAS_WEAPONS | HAS_SHIELDS:
        zero_one_ets (
            &ets_indexes[ENGINES], &ets_indexes[SHIELDS],
            &ets_indexes[WEAPONS]);
        break;

    case HAS_ENGINES | HAS_WEAPONS:
        zero_one_ets (
            &ets_indexes[SHIELDS], &ets_indexes[ENGINES],
            &ets_indexes[WEAPONS]);
        break;
...
HudGaugeEts::HudGaugeEts ()
    : HudGauge (
          HUD_OBJECT_ETS_ENGINES, HUD_ETS_GAUGE, false, false,
...
HudGaugeEts::HudGaugeEts (int _gauge_object, int _system_type)
    : HudGauge (
          _gauge_object, HUD_ETS_GAUGE, false, false,
          (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY |
           VM_OTHER_SHIP),
          255, 255, 255),
      System_type (_system_type) {}

void HudGaugeEts::initBarHeight (int _ets_h) { ETS_bar_h = _ets_h; }
...
void HudGaugeEts::initLetter (char _letter) { Letter = _letter; }
...
void HudGaugeEts::blitGauge (int index) {
    ...
    if (index < NUM_ENERGY_LEVELS - 1) {
        ...
        renderBitmapEx (
            Ets_bar.first_frame, x, y_start, w, y_end - y_start, 0,
            ETS_bar_h - clip_h);
    }
    ...
    if (index > 0) {
        ...
        renderBitmapEx (
            Ets_bar.first_frame + 1, x, y_start, w, y_end - y_start, 0,
            clip_h);
        ...
        renderBitmapEx (
            Ets_bar.first_frame + 2, x, y, w, ETS_bar_h - clip_h, 0, 0);
    }
}
...
void HudGaugeEtsRetail::render (float /*frametime*/) {
    ...
    if (Ets_bar.first_frame < 0) { return; }
    ...
    if (ship_has_energy_weapons (ship_p)) {
        ...
        renderPrintf (
            position[0] + Letter_offsets[0], position[1] + Letter_offsets[1],
            NOX ("%c"), Letter);
    }
...
void HudGaugeEtsRetail::initLetters (char* _letters) {
    int i;
    for (i = 0; i < num_retail_ets_gauges; ++i) Letters[i] = _letters[i];
}
...
void HudGaugeEtsShields::render (float /*frametime*/) {
    ...
    if (Player_obj->flags[Object::Object_Flags::No_shields]) { return; }
