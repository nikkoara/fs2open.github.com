...
#define RESET_SHIP_SPEED \
    440 // Speed that a ship is reset to after exceeding MAX_SHIP_SPEED
...
float glide_ramp (
    float v_in, float v_goal, float ramp_time_const, float accel_mult,
    float t);
...
void physics_init (physics_info* pi) {
    ...
    pi->slide_accel_time_const =
        pi->side_slip_time_const; // slide using max_vel.xyz.x & .xyz.y
    ...
    pi->vert_thrust =
        0.0f; // added these two in order to get side and forward thrusters
    pi->side_thrust = 0.0f; // to glow brighter when the ship is moving in the
                            // right direction -Bobboau
...
// You can extend this to 3d by calling it 3 times, once for each x,y,z
// component.

void apply_physics (
    float damping, float desired_vel, float initial_vel, float t,
    float* new_vel, float* delta_pos) {
    if (damping < 0.0001f) {
        if (delta_pos) *delta_pos = desired_vel * t;
        if (new_vel) *new_vel = desired_vel;
    }
    else {
        float dv, e;
        ...
        if (new_vel) *new_vel = dv * e + desired_vel;
    }
}
...
void physics_sim_rot (matrix* orient, physics_info* pi, float sim_time) {
    ...
    if (pi->flags & PF_IN_SHOCKWAVE) {
        if (timestamp_elapsed (pi->shockwave_decay)) {
            ...
            shock_amplitude =
                pi->shockwave_shake_amp * shock_fraction_time_left;
        }
    }
    else {
        rotdamp = pi->rotdamp;
    ...
    apply_physics (
        ...
        &new_vel.xyz.x, NULL);
    apply_physics (
        rotdamp, pi->desired_rotvel.xyz.y, pi->rotvel.xyz.y, sim_time,
        &new_vel.xyz.y, NULL);
    apply_physics (
        rotdamp, pi->desired_rotvel.xyz.z, pi->rotvel.xyz.z, sim_time,
        &new_vel.xyz.z, NULL);
    ...
    /*	//	Make ship shake due to afterburner.
       ...
       !timestamp_elapsed(pi->afterburner_decay) ) { float	max_speed;

            max_speed = vm_vec_mag_quick(&pi->max_vel);
            tangles.p += (float) (rand()-RAND_MAX_2) * RAND_MAX_1f *
       pi->speed/max_speed/64.0f; tangles.h += (float) (rand()-RAND_MAX_2) *
       RAND_MAX_1f * pi->speed/max_speed/64.0f; if ( pi->flags &
       PF_AFTERBURNER_ON ) { pi->afterburner_decay =
       timestamp(ABURN_DECAY_TIME);
            }
        }
    */

    // Make ship shake due to shockwave, decreasing in amplitude at the end of
    // the shockwave
    if (pi->flags & PF_IN_SHOCKWAVE) {
        tangles.p +=
            (float)(myrand () - RAND_MAX_2) * RAND_MAX_1f * shock_amplitude;
        tangles.h +=
            (float)(myrand () - RAND_MAX_2) * RAND_MAX_1f * shock_amplitude;
    }
...
void physics_sim_rot_editor (
    ...
    apply_physics (
        pi->rotdamp, pi->desired_rotvel.xyz.y, pi->rotvel.xyz.y, sim_time,
        &new_vel.xyz.y, NULL);

    apply_physics (
        pi->rotdamp, pi->desired_rotvel.xyz.z, pi->rotvel.xyz.z, sim_time,
        &new_vel.xyz.z, NULL);
...
void physics_sim_vel (
    vec3d* position, physics_info* pi, float sim_time, matrix* orient) {
    ...
    if (pi->flags & PF_DEAD_DAMP) {
        // side_slip_time_const is already quite large and now needs to be
        // applied in all directions
        vm_vec_make (
            &damp, pi->side_slip_time_const, pi->side_slip_time_const,
        ...
        if (timestamp_elapsed (pi->reduced_damp_decay)) {
            ...
            // damp is multiplied by fraction and not fraction^2, gives better
            // collision separation
            float reduced_damp_fraction_time_left =
                timestamp_until (pi->reduced_damp_decay) /
                (float)REDUCED_DAMP_TIME;
            damp.xyz.x = pi->side_slip_time_const *
                         (1 + (REDUCED_DAMP_FACTOR - 1) *
                                  reduced_damp_fraction_time_left);
            damp.xyz.y = pi->side_slip_time_const *
                         (1 + (REDUCED_DAMP_FACTOR - 1) *
                                  reduced_damp_fraction_time_left);
            damp.xyz.z = pi->side_slip_time_const *
                         reduced_damp_fraction_time_left * REDUCED_DAMP_FACTOR;
        }
    }
    else {
        // regular damping
    ...
    // Note: CANNOT maintain a *local velocity* since a rotation can occur in
    // this frame. thus the local velocity of in the next frame can be
    // different (this would require rotate to change local vel and this is not
    // desired
    ...
    apply_physics (
        damp.xyz.x, local_desired_vel.xyz.x, local_v_in.xyz.x, sim_time,
        &local_v_out.xyz.x, &local_disp.xyz.x);

    // find updated LOCAL velocity and position in the local y direction
    apply_physics (
        damp.xyz.y, local_desired_vel.xyz.y, local_v_in.xyz.y, sim_time,
        &local_v_out.xyz.y, &local_disp.xyz.y);

    // find updated LOCAL velocity and position in the local z direction
    // for player ship, damp should normally be zero, but may be altered in a
    // shockwave
    //  in death, shockwave,etc. we want damping time const large for all 3
    //  axes
    ...
    if (excess > 5 && (pi->flags & PF_SPECIAL_WARP_IN)) {
        ...
        local_disp.xyz.z =
            (pi->max_vel.xyz.z * sim_time) +
            excess * (float(SPECIAL_WARP_T_CONST) * (1.0f - exp_factor));
    }
        ...
        local_disp.xyz.z =
            (local_v_in.xyz.z * sim_time) +
            deficeit * (sim_time -
                        (float(SPECIAL_WARP_T_CONST) * (1.0f - exp_factor)));
    }
    else {
        apply_physics (
            damp.xyz.z, local_desired_vel.xyz.z, local_v_in.xyz.z, sim_time,
            &local_v_out.xyz.z, &local_disp.xyz.z);
    }
...
void physics_sim (
    vec3d* position, matrix* orient, physics_info* pi, float sim_time) {
    ...
    if (pi->flags & PF_CONST_VEL) {
        ...
        pi->fspeed = vm_vec_dot (
            ...
            &pi->vel); // instead of vector magnitude -- use only forward
                       // vector since we are only interested in forward
                       // velocity
    }
}

//	-----------------------------------------------------------------------------------------------------------
// Simulate a physics object for this frame.  Used by the editor.  The
// difference between this function and physics_sim() is that this one uses a
// heading change to rotate around the universal Y axis, rather than the local
// orientation's Y axis.  Banking is also ignored.
void physics_sim_editor (
    vec3d* position, matrix* orient, physics_info* pi, float sim_time) {
    physics_sim_vel (position, pi, sim_time, orient);
    ...
    pi->fspeed = vm_vec_dot (
        &orient->vec.fvec,
        &pi->vel); // instead of vector magnitude -- use only forward vector
                   // since we are only interested in forward velocity
}

// function to predict an object's position given the delta time and an objects
// physics info
void physics_predict_pos (
    physics_info* pi, float delta_time, vec3d* predicted_pos) {
    apply_physics (
        pi->side_slip_time_const, pi->desired_vel.xyz.x, pi->vel.xyz.x,
        delta_time, NULL, &predicted_pos->xyz.x);

    apply_physics (
        pi->side_slip_time_const, pi->desired_vel.xyz.y, pi->vel.xyz.y,
        delta_time, NULL, &predicted_pos->xyz.y);

    apply_physics (
        pi->side_slip_time_const, pi->desired_vel.xyz.z, pi->vel.xyz.z,
        delta_time, NULL, &predicted_pos->xyz.z);
}

// function to predict an object's velocity given the parameters
void physics_predict_vel (
    physics_info* pi, float delta_time, vec3d* predicted_vel) {
    if (pi->flags & PF_CONST_VEL) { predicted_vel = &pi->vel; }
    else {
        apply_physics (
            pi->side_slip_time_const, pi->desired_vel.xyz.x, pi->vel.xyz.x,
            delta_time, &predicted_vel->xyz.x, NULL);

        apply_physics (
            pi->side_slip_time_const, pi->desired_vel.xyz.y, pi->vel.xyz.y,
            delta_time, &predicted_vel->xyz.y, NULL);

        apply_physics (
            pi->side_slip_time_const, pi->desired_vel.xyz.z, pi->vel.xyz.z,
            delta_time, &predicted_vel->xyz.z, NULL);
    }
}

// function to predict position and velocity of an object
void physics_predict_pos_and_vel (
    physics_info* pi, float delta_time, vec3d* predicted_vel,
    vec3d* predicted_pos) {
    apply_physics (
        pi->side_slip_time_const, pi->desired_vel.xyz.x, pi->vel.xyz.x,
        delta_time, &predicted_vel->xyz.x, &predicted_pos->xyz.x);

    apply_physics (
        pi->side_slip_time_const, pi->desired_vel.xyz.y, pi->vel.xyz.y,
        delta_time, &predicted_vel->xyz.y, &predicted_pos->xyz.y);

    apply_physics (
        pi->side_slip_time_const, pi->desired_vel.xyz.z, pi->vel.xyz.z,
        delta_time, &predicted_vel->xyz.z, &predicted_pos->xyz.z);
}
...
// function looks at the flying controls and the current velocity to determine
// a goal velocity function determines velocity in object's reference frame and
// goal velocity in object's reference frame
void physics_read_flying_controls (
    matrix* orient, physics_info* pi, control_info* ci, float sim_time,
    vec3d* wash_rot) {
    vec3d goal_vel; // goal velocity in local coords, *not* accounting for
                    // ramping of velcity
    ...
    if (pi->flags & PF_AFTERBURNER_ON) {
        ...
        if (!(pi->afterburner_max_reverse_vel > 0.0f)) { ci->forward = 1.0f; }
    }
    ...
    if (!Flight_controls_follow_eyepoint_orientation || (Player_obj == NULL) ||
    ...
    pi->vert_thrust = ci->vertical; // added these two in order to get side and
                                    // forward thrusters
    pi->side_thrust = ci->sideways; // to glow brighter when the ship is moving
                                    // in the right direction -Bobboau
    ...
    if (pi->flags & PF_ACCELERATES) {
        //
        // Determine *resultant* DESIRED VELOCITY (desired_vel) accounting for
        // RAMPING of velocity Use LOCAL coordinates if slide_enabled, ramp
        // velocity for x and y, otherwise set goal (0)
        //    always ramp velocity for z
        //

        // If reduced damp in effect, then adjust ramp_velocity and
        // desired_velocity can not change as fast. Scale according to
        // reduced_damp_time_expansion.
        ...
        if (pi->flags & PF_SLIDE_ENABLED) {
            ...
            // If reduced damp in effect, then adjust ramp_velocity and
            // desired_velocity can not change as fast
            if (pi->flags & PF_REDUCED_DAMP) {
                ramp_time_const *= reduced_damp_ramp_time_expansion;
            }
            pi->prev_ramp_vel.xyz.x = velocity_ramp (
                pi->prev_ramp_vel.xyz.x, goal_vel.xyz.x, ramp_time_const,
                sim_time);
            ...
            pi->prev_ramp_vel.xyz.y = velocity_ramp (
                pi->prev_ramp_vel.xyz.y, goal_vel.xyz.y, ramp_time_const,
                sim_time);
        }
        else {
            // slide not enabled
        ...
        pi->prev_ramp_vel.xyz.z = velocity_ramp (
            pi->prev_ramp_vel.xyz.z, goal_vel.xyz.z, ramp_time_const,
            sim_time);

        // Deternine the current dynamic glide cap, and ramp to it
        // This is outside the normal "glide" block since we want the cap to
        // adjust whether or not the ship is in glide mode
        float dynamic_glide_cap_goal = 0.0;
        if (pi->flags & PF_AFTERBURNER_ON) {
            dynamic_glide_cap_goal = (goal_vel.xyz.z >= 0.0f)
                                         ? pi->afterburner_max_vel.xyz.z
                                         : pi->afterburner_max_reverse_vel;
        }
        else {
            // Use the maximum value in X, Y, and Z (including overclocking)
            dynamic_glide_cap_goal = MAX (
                MAX (pi->max_vel.xyz.x, pi->max_vel.xyz.y), pi->max_vel.xyz.z);
        }
        pi->cur_glide_cap = velocity_ramp (
            pi->cur_glide_cap, dynamic_glide_cap_goal, ramp_time_const,
            sim_time);
        ...
        if ((pi->flags & PF_GLIDING) || (pi->flags & PF_FORCE_GLIDE)) {
            ...
            // If we're near the (positive) glide cap, decay velocity where we
            // aren't thrusting This is a hack, but makes the flight feel a lot
            // smoother Don't do this if we aren't applying any thrust, we have
            // no glide cap, or the accel multiplier is 0 (no thrust while
            // gliding)
            float cap_decay_threshold = 0.95f;
            float cap_decay_amount = 0.2f;
            if (curGlideCap >= 0.0f &&
                vm_vec_mag (&pi->desired_vel) >=
                    cap_decay_threshold * curGlideCap &&
                vm_vec_mag (&goal_vel) > 0.0f &&
                pi->glide_accel_mult != 0.0f) {
                if (goal_vel.xyz.x == 0.0f)
                    vm_vec_scale_add2 (
                        &pi->desired_vel, &orient->vec.rvec,
                        -cap_decay_amount * local_vel.xyz.x);
                if (goal_vel.xyz.y == 0.0f)
                    vm_vec_scale_add2 (
                        &pi->desired_vel, &orient->vec.uvec,
                        -cap_decay_amount * local_vel.xyz.y);
                if (goal_vel.xyz.z == 0.0f)
                    vm_vec_scale_add2 (
                        &pi->desired_vel, &orient->vec.fvec,
                        -cap_decay_amount * local_vel.xyz.z);
            }

            // The glide_ramp function uses (basically) the same math as the
            // velocity ramp so that thruster power is consistent Only ramp if
            // the glide cap is positive
            float xVal = glide_ramp (
                local_vel.xyz.x, goal_vel.xyz.x, pi->slide_accel_time_const,
                pi->glide_accel_mult, sim_time);
            float yVal = glide_ramp (
                local_vel.xyz.y, goal_vel.xyz.y, pi->slide_accel_time_const,
                pi->glide_accel_mult, sim_time);
            float zVal = 0.0;
            if (pi->flags & PF_AFTERBURNER_ON)
                zVal = glide_ramp (
                    local_vel.xyz.z, goal_vel.xyz.z,
                ...
                if (goal_vel.xyz.z >= 0.0f)
                    zVal = glide_ramp (
                        local_vel.xyz.z, goal_vel.xyz.z,
                        pi->forward_accel_time_const, pi->glide_accel_mult,
                        sim_time);
                else
                    zVal = glide_ramp (
                        local_vel.xyz.z, goal_vel.xyz.z,
                        pi->forward_decel_time_const, pi->glide_accel_mult,
                        sim_time);
            }

            // Compensate for effect of dampening: normal flight cheats here,
            // so /we make up for it this way so glide acts the same way
            ...
            // Only do the glide cap if we have one and are actively thrusting
            // in some direction. Unless AIPF2_GLIDE_DECAY_REQUIRES_THRUST
            // isn't set. -MageKing17
            if (curGlideCap >= 0.0f &&
                (!(The_mission.ai_profile->flags
                       [AI::Profile_Flags::Glide_decay_requires_thrust]) ||
                 ci->forward != 0.0f || ci->sideways != 0.0f ||
                 ci->vertical != 0.0f)) {
            ...
            vm_vec_scale_add2 (
                &pi->desired_vel, &orient->vec.rvec, pi->prev_ramp_vel.xyz.x);
            vm_vec_scale_add2 (
                &pi->desired_vel, &orient->vec.uvec, pi->prev_ramp_vel.xyz.y);
            vm_vec_scale_add2 (
                &pi->desired_vel, &orient->vec.fvec, pi->prev_ramp_vel.xyz.z);
        }
    }
...
void physics_apply_whack (
    vec3d* impulse, vec3d* pos, physics_info* pi, matrix* orient, float mass) {
    ...
    // pi->rotvel.xyz.y, pi->rotvel.xyz.z));
    ...
    // Goober5000 - pi->mass should probably be just mass, as specified in the
    // header
...
// function generates a velocity ramp with a given time constant independent of
// frame rate uses an exponential approach to desired velocity and a cheat when
// close to improve closure speed
float velocity_ramp (
    float v_in, float v_goal, float ramp_time_const, float t) {
    float delta_v;
    float decay_factor;
    float dist;
    ...
    if (ramp_time_const < 0.0001f) { return v_goal; }

    // determine decay factor  (ranges from 0 for short times to 1 for long
    // times) when decay factor is near 0, the velocity in nearly unchanged
...
// Handles thrust values when gliding. Purposefully similar to velocity_ramp in
// order to yeild a similar "feel" in terms of thruster accels (as much as
// possible) This is all in the local frame of reference for a single movement
// axis
float glide_ramp (
    float v_in, float v_goal, float ramp_time_const, float accel_mult,
    float t) {
    if (v_goal == 0.0f) { return 0.0f; }

    // This approach to delta_v allows us to ramp accelerations even in glide
    // mode Get the difference in velocity between current and goal as thrust
    ...
    float decay_factor = (ramp_time_const > 0.0f)
                             ? (1.0f - (float)exp (-t / ramp_time_const))
                             : 1.0f;
    return delta_v * decay_factor;
}

// ----------------------------------------------------------------------------
// physics_apply_shock applies applies a shockwave to an object.  This causes a
// velocity impulse and and a rotational impulse.  This is different than
...
void physics_apply_shock (
    vec3d* direction_vec, float pressure, physics_info* pi, matrix* orient,
    vec3d* min, vec3d* max, float radius) {
    vec3d normal;
    ...
    //  model the effect of the shockwave as if the shockwave were a plane of
    //  projectiles, all moving in the direction direction_vec.  then find the
    //  torque as the cross prod of the force (pressure * area * normal * sin *
    //  scale * mass) normal takes account the fraction of the surface exposed
    //  to the shockwave the sin term is not technically needed but "feels"
    //  better scale factors out the increase in area with larger objects more
    //  massive objects get less rotation

    // find torque due to forces on the right/left face
    if (normal.xyz.x < 0.0f) // normal < 0, hits the right face
        vm_vec_copy_scale (
            &impact_vec, &orient->vec.rvec,
            max->xyz.x * pressure * area.xyz.x * normal.xyz.x * sin.xyz.x /
                pi->mass);
    else // normal > 0, hits the left face
        vm_vec_copy_scale (
            &impact_vec, &orient->vec.rvec,
            min->xyz.x * pressure * area.xyz.x * -normal.xyz.x * sin.xyz.x /
                pi->mass);
    ...
    if (normal.xyz.y < 0.0f)
        vm_vec_copy_scale (
            &impact_vec, &orient->vec.uvec,
            max->xyz.y * pressure * area.xyz.y * normal.xyz.y * sin.xyz.y /
                pi->mass);
    else
        vm_vec_copy_scale (
            &impact_vec, &orient->vec.uvec,
            min->xyz.y * pressure * area.xyz.y * -normal.xyz.y * sin.xyz.y /
                pi->mass);
    ...
    if (normal.xyz.z < 0.0f)
        vm_vec_copy_scale (
            &impact_vec, &orient->vec.fvec,
            max->xyz.z * pressure * area.xyz.z * normal.xyz.z * sin.xyz.z /
                pi->mass);
    else
        vm_vec_copy_scale (
            &impact_vec, &orient->vec.fvec,
            min->xyz.z * pressure * area.xyz.z * -normal.xyz.z * sin.xyz.z /
                pi->mass);
    ...
    if (radius < MIN_RADIUS) { scale = 1.0f; }
    else {
        scale = (MAX_RADIUS - radius) / (MAX_RADIUS - MIN_RADIUS);
    }

    // set shockwave shake amplitude, duration, flag
    pi->shockwave_shake_amp =
        (float)(MAX_SHAKE * (pressure / STD_PRESSURE) * scale);
...
// collision physics. If you need to change the rotation, change
// COLLISION_ROTATION_FACTOR in collide_ship_ship.
#define ROTVEL_COLLIDE_WHACK_CONST 1.0
void physics_collide_whack (
    vec3d* impulse, vec3d* world_delta_rotvel, physics_info* pi,
    matrix* orient, bool is_landing) {
    ...
    vm_vec_rotate (
        ...
        orient); // set so velocity will ramp starting from current speed
                 // ramped velocity is now affected by collision
    // rotate previous ramp velocity (in model coord) to be same as vel (in
    // world coords)
}
...
int check_rotvel_limit (physics_info* pi) {
    ...
    if (!(pi->flags & PF_DEAD_DAMP)) {
        ...
        // > ROTVEL_TOL ); Assert( (pi->max_rotvel.xyz.x <= ROTVEL_CAP) &&
...
void update_reduced_damp_timestamp (physics_info* pi, float impulse) {
    ...
    reduced_damp_decay_time =
        (int)(REDUCED_DAMP_TIME * impulse / (REDUCED_DAMP_VEL * pi->mass));
...
avd_movement::avd_movement ()
    : Pc (0.0f), Vc (0.0f), TSi (0), Pi (0.0f), Vi (0.0f), Pf (0.0f),
      Tf (0.0f), Tai (0.0f), Taf (0.0f), Vf (0.0f), Vm (0.0f), Ai (0.0f),
      Af (0.0f) {}
...
void avd_movement::get (float Time, float* Position, float* Velocity) {
    ...
    if (Position != NULL) *Position = Pc;
    if (Velocity != NULL) *Velocity = Vc;
}
...
void avd_movement::setAVD (
    float final_position, float total_movement_time,
    float starting_accleration_time, float ending_acceleration_time,
    ...
    Vm = (Pf - Pi - 0.5f * (Vi * Tai) - 0.5f * (Vf * Taf)) /
         (Tf - 0.5f * Tai - 0.5f * Taf);
    Ai = (Vm - Vi) / Tai;
    Af = (Vf - Vm) / Taf;
}

void avd_movement::setVD (
    float total_movement_time, float ending_acceleration_time,
    float final_velocity) {
    // Make sure Pc et al are up-to-date
    this->update ();
...
void avd_movement::update (float Time) {
    if (Tf <= 0.0f) {
        ...
        // Movement is in-progress and we must calculate where the thing is
        // now.
        float t = Time;
        float Tc = 0.0f;
