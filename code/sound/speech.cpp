...
bool speech_init () {
    ...
    HRESULT hr = CoCreateInstance (
        CLSID_SpVoice, NULL, CLSCTX_ALL, IID_ISpVoice, (void**)&Voice_device);
...
bool speech_play (const char* text) {
    ...
    if (text == NULL) {
        nprintf (
            ("Speech", "Not playing speech because passed text is null.\n"));
        return false;
    }
    ...
    auto num_chars = MultiByteToWideChar (
        CP_UTF8, 0, work_buffer.c_str (), (int)work_buffer.size (), nullptr,
        0);
    ...
    auto err = MultiByteToWideChar (
        CP_UTF8, 0, work_buffer.c_str (), (int)work_buffer.size (),
        &wide_string[0], num_chars);

    if (err <= 0) { return false; }

    speech_stop ();
    return SUCCEEDED (
        Voice_device->Speak (wide_string.c_str (), SPF_ASYNC, NULL));
#else
    int len = strlen (text);
    char Conversion_buffer[MAX_SPEECH_CHAR_LEN];

    if (len > (MAX_SPEECH_CHAR_LEN - 1)) { len = MAX_SPEECH_CHAR_LEN - 1; }
    ...
    if (write (speech_dev, Conversion_buffer, count) == -1) return false;
...
std::vector< std::string > speech_enumerate_voices () {
    ...
    if (FAILED (hr)) { return std::vector< std::string > (); }
    ...
    hr = ::CoCreateInstance (
        CLSID_SpObjectTokenCategory, NULL, CLSCTX_INPROC_SERVER,
        IID_ISpObjectTokenCategory, (LPVOID*)&comTokenCategory);
    if (FAILED (hr)) { return std::vector< std::string > (); }
    ...
    while (comVoicesCount > 0) {
        ...
        auto buffer_size = WideCharToMultiByte (
            CP_UTF8, 0, id, (int)idlength, nullptr, 0, nullptr, nullptr);

        if (buffer_size > 0) {
            std::string voiceName;
            voiceName.resize (buffer_size);
            buffer_size = WideCharToMultiByte (
                CP_UTF8, 0, id, (int)idlength, &voiceName[0], buffer_size,
                nullptr, nullptr);
