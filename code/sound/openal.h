...
#define OpenAL_ErrorCheck(x, y)                                               \
    do {                                                                      \
        ...
        if (error_text != NULL) {                                             \
            nprintf (                                                         \
                ("Warning", "SOUND: %s:%d - OpenAL error = '%s'\n", __FILE__, \
                 __LINE__, error_text));                                      \
            y;                                                                \
        }                                                                     \
    } while (false);

// like OpenAL_ErrorCheck() except that it gives the error message from x but
...
#define OpenAL_ErrorPrint(x)                                       \
    do {                                                           \
        ...
        if (error_text != NULL) {                                  \
            nprintf (                                              \
                ("Sound", "OpenAL ERROR: \"%s\" in %s, line %i\n", \
                 error_text, __FILE__, __LINE__));                 \
        }                                                          \
    } while (0);

// same as the above two, but looks for ALC errors instead of standard AL
// errors
