...
void init () {
    ...
    if (Anim_bitmap_id_fire == -1) {
        Anim_bitmap_id_fire = bm_load_animation (
            "particleexp01", &Anim_num_frames_fire, nullptr, NULL, 0);
    }

    // Cough, cough
    if (Anim_bitmap_id_smoke == -1) {
        Anim_bitmap_id_smoke = bm_load_animation (
            "particlesmoke01", &Anim_num_frames_smoke, nullptr, NULL, 0);
    }

    // wheeze
    if (Anim_bitmap_id_smoke2 == -1) {
        Anim_bitmap_id_smoke2 = bm_load_animation (
            "particlesmoke02", &Anim_num_frames_smoke2, nullptr, NULL, 0);
    }
}
...
DCF_BOOL2 (
    particles, Particles_enabled, "Turns particles on/off",
    "Usage: particles [bool]\nTurns particle system on/off.  If nothing "
    "passed, then toggles it.\n");

bool init_particle (particle* part, particle_info* info) {
    if (!Particles_enabled) { return false; }
    ...
    case PARTICLE_BITMAP_PERSISTENT: {
        ...
        bm_get_info (
            info->optional_data, NULL, NULL, NULL, &part->nframes, &fps);
    ...
    case PARTICLE_FIRE: {
        if (Anim_bitmap_id_fire < 0) { return false; }
    ...
    case PARTICLE_SMOKE: {
        if (Anim_bitmap_id_smoke < 0) { return false; }
    ...
    case PARTICLE_SMOKE2: {
        if (Anim_bitmap_id_smoke2 < 0) { return false; }
...
void create (particle_info* pinfo) {
    particle part;
    if (!init_particle (&part, pinfo)) { return; }
 ...
 * @return @c true if the particle has expired and should be removed, @c false
 * otherwise
 */
static bool move_particle (float frametime, particle* part) {
    if (part->age == 0.0f) { part->age = 0.00001f; }
    else {
        part->age += frametime;
    }
    ...
    if (part->age > part->max_life) {
        // special case, if max_life is 0 then we want it to render at least
        // once
    ...
    // if the particle is attached to an object which has become invalid, kill
    // it
    ...
    if (remove_particle) { return true; }
...
void move_all (float frametime) {
    ...
    if (Persistent_particles.empty () && Particles.empty ()) return;

    for (auto p = Persistent_particles.begin ();
         p != Persistent_particles.end ();) {
        ParticlePtr part = *p;
        if (move_particle (frametime, part.get ())) {
            // if we're sitting on the very last particle, popping-back will
 ...
 * @return @c true if the particle has been added to the rendering batch, @c
 * false otherwise
 */
static bool render_particle (particle* part) {
    ...
    if (part->attached_objnum >= 0) {
        vm_vec_unrotate (
            &p_pos, &part->pos, &Objects[part->attached_objnum].orient);
    ...
    if (vm_vec_dot_to_point (&Eye_matrix.vec.fvec, &Eye_position, &p_pos) <=
        0.0f) {
        return false;
    }
    ...
    if (part->nframes > 1) {
        framenum =
            bm_get_anim_frame (part->optional_data, part->age, part->max_life);
...
void render_all () {
    ...
    for (auto& part : Persistent_particles) {
        if (render_particle (part.get ())) { render_batch = true; }
    }

    for (auto& part : Particles) {
        if (render_particle (&part)) { render_batch = true; }
    }

    if (render_batch) { batching_render_all (); }
}
...
void emit (
    particle_emitter* pe, ParticleType type, int optional_data, float range) {
    int i, n;
