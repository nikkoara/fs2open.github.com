 ...
 *
 * A particle effect contains all data necessary for spawning the particles for
 * one particle source. A source can't store effect specific data so a particle
 * effect has to be state-less. To add your own particle effects just extend
 * this class and override the functions you need. If the effect type should be
 * available for table creation take a look at constructEffect in
 * ParticleManager.cpp.
 *
 * @ingroup particleSystems
 */
...
public:
     ...
     * this function for parsing the values. The usual parsing functions are
     * available.
     *
     * @param nocreate
     */
     ...
     * implementation should generate new particles according to its
     * configuration. The return value is used to determine if the source
     * should continue to exist. Return @c true if the source should be
     * processed in the next frame, return @c false if this is effect is done.
     *
     * @warning Implementations of this function must be able to handle
     * multiple calls to this function even if a previous call returned @c
     * false.
     *
     ...
     * to a source. This could be used to set the lifetime of a source once if
     * the effect supports it. See #SingleParticleEffect for an example of how
     * this could be used.
     *
