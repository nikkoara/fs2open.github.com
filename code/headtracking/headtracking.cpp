...
bool isEnabled () { return currentProvider != nullptr; }

bool query () {
    if (currentProvider == nullptr) { return false; }

    return currentProvider->query (&status);
}

HeadTrackingStatus* getStatus () { return &status; }

void shutdown () { currentProvider.reset (); }
} // namespace headtracking
