 ...
 *
 * For windows there is a registry entry at
 * HKEY_CURRENT_USER\Software\FreeTrack\FreeTrackClient that specifies where
 * the dll is located.
 */
std::string getLibraryLocation () {
    HKEY hKey = NULL;
    LONG lResult = RegOpenKeyEx (
        HKEY_CURRENT_USER, TEXT ("Software\\FreeTrack\\FreeTrackClient"), 0,
    ...
    lResult = RegQueryValueEx (hKey, "Path", NULL, NULL, NULL, &dwLen);
    ...
    lResult = RegQueryValueEx (
        hKey, "Path", NULL, NULL, reinterpret_cast< BYTE* > (&value[0]),
        &dwLen);
...
namespace freetrack {
FreeTrackLibrary::FreeTrackLibrary ()
    : mFTGetData (nullptr), mFTGetDllVersion (nullptr), mFTReportID (nullptr),
      mFTProvider (nullptr), mEnabled (false) {
    if (!LoadExternal (getLibraryLocation ().c_str ())) return;
...
bool FreeTrackLibrary::GetData (FreeTrackData* data) {
    if (mFTGetData) return mFTGetData (data);

    return false;
}

char* FreeTrackLibrary::GetDllVersion (void) {
    if (mFTGetDllVersion) return mFTGetDllVersion ();

    return nullptr;
}

void FreeTrackLibrary::ReportID (int name) {
    if (mFTReportID) mFTReportID (name);
}

char* FreeTrackLibrary::Provider (void) {
    if (mFTProvider) return mFTProvider ();
...
FreeTrackProvider::FreeTrackProvider () : library (FreeTrackLibrary ()) {
    ...
    if (data.dataID == std::numeric_limits< unsigned int >::max () ||
        ...
        throw internal::HeadTrackingException (
            "The test values reported by FreeTrack were invalid!");
    }

    // I have no idea what a correct value for this function is so I used this
    // random value
    library.ReportID (7919);

    mprintf (
        ("Found FreeTrack provider '%s' with version %s.\n",
...
bool FreeTrackProvider::query (HeadTrackingStatus* statusOut) {
    FreeTrackData data;
    if (!library.GetData (&data)) { return false; }
...
std::string FreeTrackProvider::getName () { return "FreeTrack"; }
} // namespace freetrack
} // namespace headtracking
