...
TrackIRProvider::TrackIRProvider () {
    ...
    if (window == nullptr) {
        throw internal::HeadTrackingException (
            "TrackIR is only available with a valid window!");
    }

    // calling the function that will init all the function pointers for
    // TrackIR stuff (Swifty)
...
std::string TrackIRProvider::getName () { return "TrackIR"; }
} // namespace trackir
} // namespace headtracking
