...
TrackIRDLL::TrackIRDLL () {
    ...
    if (!LoadExternal (TRACKIRBRIDGEDLLNAME)) return;
...
bool TrackIRDLL::Enabled () const { return m_enabled; }
...
int TrackIRDLL::Init (SDL_Window* window) {
    if (m_Init) {
        ...
        if (SDL_GetWindowWMInfo (
                window, &info)) { // the call returns true on success
            // success
...
int TrackIRDLL::Close () {
    if (m_Close) return m_Close ();
    return 0;
}

int TrackIRDLL::Query () {
    if (m_Query) return m_Query ();
    return 0;
}

float TrackIRDLL::GetX () const {
    if (m_GetX) return m_GetX ();
    return 0.0f;
}

float TrackIRDLL::GetY () const {
    if (m_GetY) return m_GetY ();
    return 0.0f;
}

float TrackIRDLL::GetZ () const {
    if (m_GetZ) return m_GetZ ();
    return 0.0f;
}

float TrackIRDLL::GetPitch () const {
    if (m_GetPitch) return m_GetPitch ();
    return 0.0f;
}

float TrackIRDLL::GetRoll () const {
    if (m_GetRoll) return m_GetRoll ();
    return 0.0f;
}

float TrackIRDLL::GetYaw () const {
    if (m_GetYaw) return m_GetYaw ();
    return 0.0f;
}
