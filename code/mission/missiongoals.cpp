...
struct goal_buttons {
    ...
    goal_buttons (const char* name, int x1, int y1, int h)
        : filename (name), x (x1), y (y1), hotspot (h) {}
};
...
static int
    Mission_directive_sound_timestamp; // timestamp to control when directive
                                       // succcess sound gets played
static int
    Mission_directive_special_timestamp; // used to specially mark a directive
                                         // as true even though it's not

const char* Goal_type_text (int n) {
    switch (n) {
    case 0: return XSTR ("Primary", 396);

    case 1: return XSTR ("Secondary", 397);

    case 2: return XSTR ("Bonus", 398);
    }
...
goal_buttons Goal_buttons[NUM_GOAL_SCREEN_BUTTONS] = {
    // XSTR:OFF
    goal_buttons ("MOB_00", 475, 288, 0), goal_buttons ("MOB_01", 475, 336, 1),
...
void goal_list::icons_display (int yoff) {
    ...
    for (i = 0; i < count; i++) {
        ...
        case GOAL_COMPLETE: bmp = Goal_complete_bitmap; break;

        case GOAL_INCOMPLETE: bmp = Goal_incomplete_bitmap; break;

        case GOAL_FAILED: bmp = Goal_failed_bitmap; break;
        }
        ...
        if (bmp >= 0) {
            ...
            if ((y >= Goal_screen_text_y - ys / 2) &&
                (y + ys <= Goal_screen_text_y + Goal_screen_text_h + ys / 2)) {
                gr_set_bitmap (bmp);
...
void goal_text::init () { m_num_lines = 0; }

// Adds lines of goal text.  If passed NULL (or nothing passed) a blank line is
// added.  If the text is too long, it is automatically split into more than
// one line. Returns the number of lines added.
int goal_text::add (const char* text) {
    ...
    count = split_str (
        text,
        Goal_screen_text_w -
            Goal_screen_text_coords[gr_screen.res][GOAL_SCREEN_X_COORD] +
            Goal_screen_icon_xcoord[gr_screen.res],
        m_line_sizes + m_num_lines, m_lines + m_num_lines, max);
    m_num_lines += count;
    return count;
}
...
void goal_text::display (int n, int y) {
    ...
    if (*m_lines[n] == '*') { // header line
        ...
        // custom_size me
        gr_line (
            Goal_screen_icon_x, y1, Goal_screen_text_x - 2, y1,
            GR_RESIZE_MENU);
        gr_line (
            Goal_screen_text_x + w + 1, y1,
            Goal_screen_icon_x + Goal_screen_text_w, y1, GR_RESIZE_MENU);
    }
    else {
...
// mission_init_goals: initializes info for goals.  Called as part of mission
// initialization.
void mission_init_goals () {
    ...
    Mission_directive_special_timestamp =
        timestamp (-1); // need to make invalid right away
}
...
// called once right before entering the show goals screen to do
// initializations.
void mission_show_goals_init () {
    int i, type, team_num = 0; // JAS: I set team_num to 0 because it was used
    ...
    Goal_screen_text_x =
        Goal_screen_text_coords[gr_screen.res][GOAL_SCREEN_X_COORD];
    Goal_screen_text_y =
        Goal_screen_text_coords[gr_screen.res][GOAL_SCREEN_Y_COORD];
    Goal_screen_text_w =
        Goal_screen_text_coords[gr_screen.res][GOAL_SCREEN_W_COORD];
    Goal_screen_text_h =
        Goal_screen_text_coords[gr_screen.res][GOAL_SCREEN_H_COORD];
    Goal_screen_icon_x = Goal_screen_icon_xcoord[gr_screen.res];

    // fill up the lists so we can display the goals appropriately
    for (i = 0; i < Num_goals; i++) {
        if (Mission_goals[i].type &
            INVALID_GOAL) { // don't count invalid goals here
            continue;
        }

        if ((Game_mode & GM_MULTIPLAYER) &&
            (The_mission.game_type & MISSION_TYPE_MULTI_TEAMS) &&
        ...
        switch (type) {
        case PRIMARY_GOAL: Primary_goal_list.add (&Mission_goals[i]); break;
    ...
    common_set_interface_palette (
        "ObjectivesPalette"); // set the interface palette
    Goals_screen_ui_window.create (
    ...
    for (i = 0; i < NUM_GOAL_SCREEN_BUTTONS; i++) {
        ...
        b->button.create (
            &Goals_screen_ui_window, "", b->x, b->y, 60, 30, (i < 2), 1);
...
void mission_show_goals_close () {
    if (Goals_screen_bg_bitmap >= 0) bm_release (Goals_screen_bg_bitmap);

    if (Goal_complete_bitmap) bm_release (Goal_complete_bitmap);

    if (Goal_incomplete_bitmap) bm_release (Goal_incomplete_bitmap);

    if (Goal_failed_bitmap) bm_release (Goal_failed_bitmap);
...
// called once a frame during show goals state to process events and render the
// screen
void mission_show_goals_do_frame (float /*frametime*/) {
    ...
    switch (k) {
    case KEY_ESC: mission_goal_exit (); break;

    case KEY_DOWN: goal_screen_scroll_down (); break;

    case KEY_UP: goal_screen_scroll_up (); break;
...
// Called once right before entering the Mission Log screen to do
// initializations.
int ML_objectives_init (int x, int y, int w, int h) {
    ...
    Goal_screen_text_x =
        x - Goal_screen_icon_xcoord[gr_screen.res] +
        Goal_screen_text_coords[gr_screen.res][GOAL_SCREEN_X_COORD];
    ...
    team_num = 0; // this is the default team -- we will change it if in a
                  // multiplayer team v. team game
...
void ML_objectives_close () {
    if (Goal_complete_bitmap >= 0) { bm_release (Goal_complete_bitmap); }

    if (Goal_incomplete_bitmap >= 0) { bm_release (Goal_incomplete_bitmap); }

    if (Goal_failed_bitmap >= 0) { bm_release (Goal_failed_bitmap); }
}
...
void ML_render_objectives_key () {
    ...
    if (Lcl_gr) {
        ...
        gr_bitmap (
            Objective_key_icon_coords_gr[gr_screen.res][0][0],
            Objective_key_icon_coords_gr[gr_screen.res][0][1], GR_RESIZE_MENU);
        gr_set_bitmap (Goal_incomplete_bitmap);
        gr_bitmap (
            Objective_key_icon_coords_gr[gr_screen.res][1][0],
            Objective_key_icon_coords_gr[gr_screen.res][1][1], GR_RESIZE_MENU);
        gr_set_bitmap (Goal_failed_bitmap);
        gr_bitmap (
            Objective_key_icon_coords_gr[gr_screen.res][2][0],
            Objective_key_icon_coords_gr[gr_screen.res][2][1], GR_RESIZE_MENU);

        gr_string (
            Objective_key_text_coords_gr[gr_screen.res][0][0],
            Objective_key_text_coords_gr[gr_screen.res][0][1],
            XSTR ("Complete", 1437), GR_RESIZE_MENU);
        gr_string (
            Objective_key_text_coords_gr[gr_screen.res][1][0],
            Objective_key_text_coords_gr[gr_screen.res][1][1],
            XSTR ("Incomplete", 1438), GR_RESIZE_MENU);
        gr_string (
            Objective_key_text_coords_gr[gr_screen.res][2][0],
            Objective_key_text_coords_gr[gr_screen.res][2][1],
            XSTR ("Failed", 1439), GR_RESIZE_MENU);
    }
    else {
        gr_set_bitmap (Goal_complete_bitmap);
        gr_bitmap (
            Objective_key_icon_coords[gr_screen.res][0][0],
            Objective_key_icon_coords[gr_screen.res][0][1], GR_RESIZE_MENU);
        gr_set_bitmap (Goal_incomplete_bitmap);
        gr_bitmap (
            Objective_key_icon_coords[gr_screen.res][1][0],
            Objective_key_icon_coords[gr_screen.res][1][1], GR_RESIZE_MENU);
        gr_set_bitmap (Goal_failed_bitmap);
        gr_bitmap (
            Objective_key_icon_coords[gr_screen.res][2][0],
            Objective_key_icon_coords[gr_screen.res][2][1], GR_RESIZE_MENU);

        gr_string (
            Objective_key_text_coords[gr_screen.res][0][0],
            Objective_key_text_coords[gr_screen.res][0][1],
            XSTR ("Complete", 1437), GR_RESIZE_MENU);
        gr_string (
            Objective_key_text_coords[gr_screen.res][1][0],
            Objective_key_text_coords[gr_screen.res][1][1],
            XSTR ("Incomplete", 1438), GR_RESIZE_MENU);
        gr_string (
            Objective_key_text_coords[gr_screen.res][2][0],
            Objective_key_text_coords[gr_screen.res][2][1],
            XSTR ("Failed", 1439), GR_RESIZE_MENU);
    }
}
...
void multi_player_maybe_add_score (int score, int team) {
    ...
    if (!(The_mission.ai_profile
              ->flags[AI::Profile_Flags::Allow_multi_event_scoring])) {
        return;
    }
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            (Net_players[idx].p_info.team == team)) {
            players_in_team++;
        }
    }

    if (!players_in_team) { return; }

    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED (Net_players[idx]) &&
            (Net_players[idx].p_info.team == team)) {
            Net_players[idx].m_player->stats.m_score +=
                (int)(score * scoring_get_scale_factor () / players_in_team);
        }
    }
}
    ...
    if (new_status == GOAL_FAILED) {
        ...
        if (type != BONUS_GOAL) {
            ...
            if ((Game_mode & GM_NORMAL) ||
                ((Net_player != NULL) &&
                 (Net_player->p_info.team == Mission_goals[goal_num].team))) {
        ...
        mission_log_add_entry (
            LOG_GOAL_FAILED, Mission_goals[goal_num].name, NULL, goal_num);
    }
        ...
        if ((Game_mode & GM_NORMAL) ||
            ...
            mission_log_add_entry (
                LOG_GOAL_SATISFIED, Mission_goals[goal_num].name, NULL,
                goal_num);
        }

        if (Game_mode & GM_MULTIPLAYER) {
            multi_team_maybe_add_score (
                Mission_goals[goal_num].score, Mission_goals[goal_num].team);
            multi_player_maybe_add_score (
                Mission_goals[goal_num].score, Mission_goals[goal_num].team);
        }
        else {
            // deal with the score
            Player->stats.m_score +=
                (int)(Mission_goals[goal_num].score * scoring_get_scale_factor ());
        }
    }
}
...
int mission_get_event_status (int event) {
    ...
    if (Mission_events[event].flags & MEF_DIRECTIVE_SPECIAL) {
        ...
        // if the timestamp has elapsed, we can "mark" this directive as true
        ...
        if (Mission_events[event].result) { return EVENT_SATISFIED; }

        if (Mission_events[event].formula < 0) { return EVENT_FAILED; }
...
void mission_event_set_directive_special (int event) {
    // bogus
    if ((event < 0) || (event >= Num_mission_events)) { return; }
...
void mission_event_unset_directive_special (int event) {
    ...
    // this event may be marked temporarily true -- if so, then unmark this
    // value!!!
    if (Mission_events[event].flags & MEF_DIRECTIVE_TEMP_TRUE) {
        Mission_events[event].flags &= ~MEF_DIRECTIVE_TEMP_TRUE;
    }
...
void mission_process_event (int event) {
    ...
    if (Mission_events[event].chain_delay >=
        0) { // this indicates it's chained
        // What everyone expected the chaining behavior to be, as specified in
        // Karajorma's original fix to Mantis #82
        ...
        // Volition's original chaining behavior as used in retail and
        // demonstrated in e.g. btm-01.fsm (or btm-01.fs2 in the Port)
        else {
            if (event > 0) {
                if (!Mission_events[event - 1].result ||
                    ((fix)Mission_events[event - 1].timestamp +
                         i2f (Mission_events[event].chain_delay) >
                     Missiontime)) {
                    sindex = -1; // bypass evaluation
                }
            ...
            if ((event < Num_mission_events - 1) &&
                Mission_events[event + 1].result &&
                (Mission_events[event + 1].chain_delay >= 0)) {
                sindex = -1; // bypass evaluation
            }
        }
    }
    ...
    if (sindex >= 0) {
        ...
        if (Snapshot_all_events ||
            Mission_events[event].mission_log_flags != 0) {
            Log_event = true;

            Current_event_log_buffer = &Mission_events[event].event_log_buffer;
            Current_event_log_variable_buffer =
                &Mission_events[event].event_log_variable_buffer;
            Current_event_log_argument_buffer =
                &Mission_events[event].event_log_argument_buffer;
        }
        result = eval_sexp (sindex);

        // if the directive count is a special value, deal with that first.
        // Mark the event as a special event, and unmark it when the directive
        // is true again.
        if ((Directive_count == DIRECTIVE_WING_ZERO) &&
            !(Mission_events[event].flags & MEF_DIRECTIVE_SPECIAL)) {
            // make it special - which basically just means that its true until
        ...
        if (Sexp_useful_number) { Mission_events[event].flags |= MEF_CURRENT; }
    ...
    if ((sindex >= 0) && (Sexp_nodes[sindex].value == SEXP_KNOWN_FALSE)) {
        ...
        Mission_events[event].satisfied_time = Missiontime;
        // _argv[-1] - repeat_count of -1 would mean repeat indefinitely, so
        // set to 0 instead.
    ...
    if (result && !Mission_events[event].satisfied_time) {
        ...
        if (Mission_events[event].objective_text) {
            Mission_directive_sound_timestamp =
                timestamp (DIRECTIVE_SOUND_DELAY);
        }
    }

    // decrement the trigger count.  When at 0, set the repeat count to 0 so we
    // don't eval this function anymore
    ...
    if (result || timestamp_valid (Mission_events[event].timestamp)) {
        ...
        if (Mission_events[event].repeat_count == 0) {
            ...
            if (Game_mode & GM_MULTIPLAYER) {
                // multiplayer missions (scoring is scaled in the
                // multi_team_maybe_add_score() function)
                multi_team_maybe_add_score (
                    Mission_events[event].score, Mission_events[event].team);
                multi_player_maybe_add_score (
                    Mission_events[event].score, Mission_events[event].team);
            }
            else {
                ...
                Player->stats.m_score +=
                    (int)(Mission_events[event].score * scoring_get_scale_factor ());
            }
        }
        // Set the timestamp for the next check on this event unless we only
        // have a trigger count and no repeat count and this event didn't
        // trigger this frame.
        else if (
            bump_timestamp ||
            ...
            Mission_events[event].timestamp =
                timestamp (Mission_events[event].interval * 1000);
        }
    }

    // see if anything has changed
    if (MULTIPLAYER_MASTER &&
        ((store_flags != Mission_events[event].flags) ||
         (store_formula != Mission_events[event].formula) ||
         (store_result != Mission_events[event].result) ||
         (store_count != Mission_events[event].count))) {
        send_event_update_packet (event);
    }
}

// Maybe play a directive success sound... need to poll since the sound is
// delayed from when the directive is actually satisfied.
...
void mission_eval_goals () {
    ...
    // before checking whether or not we should evaluate goals, we should run
    // through the events and process any whose timestamp is valid and has
    // expired.  This would catch repeating events only
    for (i = 0; i < Num_mission_events; i++) {
        if (Mission_events[i].formula != -1) {
            if (!timestamp_valid (Mission_events[i].timestamp) ||
                !timestamp_elapsed (Mission_events[i].timestamp)) {
                continue;
            }

            // if we get here, then the timestamp on the event has popped -- we
            // should reevaluate
    ...
    for (i = 0; i < Num_mission_events; i++) {
        if (Mission_events[i].formula != -1) {
            // only evaluate this event if the timestamp is not valid.  We do
            // this since we will evaluate repeatable events at the top of the
            // file so we can get the exact interval that the designer asked
            // for.
            if (!timestamp_valid (Mission_events[i].timestamp)) {
    ...
    if (Game_mode & GM_STANDALONE_SERVER) { std_multi_update_goals (); }
...
int mission_goals_met () {
    ...
    for (i = 0; i < Num_goals; i++) {
        ...
        if (((Mission_goals[i].type & GOAL_TYPE_MASK) == PRIMARY_GOAL) ||
            ((Mission_goals[i].type & GOAL_TYPE_MASK) == SECONDARY_GOAL)) {
            if (Mission_goals[i].satisfied == GOAL_INCOMPLETE) {
                all_goals_met = 0;
                break;
            }
...
void mission_goal_validation_change (int goal_num, int valid) {
    ...
    if (valid) { Mission_goals[goal_num].type &= ~INVALID_GOAL; }
    else {
        Mission_goals[goal_num].type |= INVALID_GOAL;
    }
}

// the following function marks a goal invalid.  It can only mark the goal
// invalid if the goal
// is not complete.  The name passed into this funciton should match the name
...
// function to fail all mission goals.  Don't fail events here since this
// funciton is currently called in mission when something bad happens to the
// player (like he makes too many shots on friendlies). Events can still
// happen.  Things will just be really bad for the player.
void mission_goal_fail_all () {
    ...
    for (i = 0; i < Num_goals; i++) {
        ...
        mission_log_add_entry (
            LOG_GOAL_FAILED, Mission_goals[i].name, NULL, i);
    }
}

// function to mark all incomplete goals as failed.  Happens at the end of a
// mission mark the events which are not currently satisfied as failed as well.
void mission_goal_fail_incomplete () {
    ...
    // now for the events.  Must set the formula to -1 and the result to 0 to
    // be a failed event.
    for (i = 0; i < Num_mission_events; i++) {
        if (Mission_events[i].formula != -1) {
...
// small function used to mark all objectives as true.  Used as a debug
// function and as a way to skip past training misisons
...
// small function used to mark all events as completed.  Used in the skipping
// of missions.
...
// some debug console functions to help list and change the status of mission
// goals
DCF (show_mission_goals, "Lists the status of mission goals") {
    ...
    if (dc_optional_string_either ("help", "--help")) {
        dc_printf (
            "Usage: show_mission_goals\n\nList all mission goals and their "
            "current status.\n");
        return;
    }
    ...
    for (i = 0; i < Num_goals; i++) {
        type = Mission_goals[i].type & GOAL_TYPE_MASK;
        dc_printf (
            "%2d. %32s(%10s) -- ", i, Mission_goals[i].name,
            Goal_type_text (type));
        if (Mission_goals[i].satisfied == GOAL_COMPLETE)
            dc_printf ("satisfied.\n");
            ...
            dc_printf (
                "Warning! Mission goal %i is in an invalid state! (value: %i)",
                i, Mission_goals[i].satisfied);
    }
}
...
DCF (change_mission_goal, "Changes the mission goal status") {
    ...
    if (dc_optional_string_either ("help", "--help")) {
        ...
        dc_printf (
            "<goal_num> --  Integer number of goal to change.  See "
            "show_mission_goals\n");
        dc_printf ("[status]   --  Goal status to change to.\n\n");

        dc_printf (
            "The optional [status] field may be either a bool type or a "
            "string.\n");
        dc_printf ("\ttrue  -- Goal status set to 'complete'\n");
        dc_printf ("\tfalse -- Goal status set to 'failed'\n\n");

        dc_printf (
            "A string value of 'satisfied', 'failed', or 'unknown' will set "
            "the goal status to the respective state.\n");
        dc_printf (
            "If [status] is not given, then the goal status will be set to "
            "'unknown'");
        ...
        dc_printf (
            "\t'change_mission_goal 2'       marks goal 2 as "
            "unknown/incomplete\n");
        dc_printf (
            "\t'change_mission_goal 0 satisfied'    marks goal 0 as "
            "satisfied\n");
        return;
    }
    ...
    if (dc_optional_string ("satisfied")) {
        ...
        val_b ? Mission_goals[num].satisfied = GOAL_COMPLETE
              : Mission_goals[num].satisfied = GOAL_FAILED;
    }
    else {
        // No argument given
    ...
    case GOAL_COMPLETE: string = "satisfied"; break;

    case GOAL_FAILED: string = "failed"; break;

    case GOAL_INCOMPLETE: string = "unsatisfied"; break;

    default:
        dc_printf (
            "Warning! Mission goal %i is in an invalid state! (value: %i)",
            num, Mission_goals[num].satisfied);
        return;
    }
...
void mission_goal_mark_all_true (int type) {
    ...
    HUD_sourced_printf (
        HUD_SOURCE_HIDDEN, NOX ("All %s goals marked true"),
        Goal_type_text (type));
}

void goal_screen_button_pressed (int num) {
    switch (num) {
    case GOAL_SCREEN_BUTTON_SCROLL_UP: goal_screen_scroll_up (); break;

    case GOAL_SCREEN_BUTTON_SCROLL_DOWN: goal_screen_scroll_down (); break;

    case GOAL_SCREEN_BUTTON_RETURN: mission_goal_exit (); break;
    }
}
...
void mission_goal_fetch_num_resolved (
    int desired_type, int* num_resolved, int* total, int team) {
    int i, type;
    ...
    *total = 0;
    ...
    for (i = 0; i < Num_goals; i++) {
        ...
        if ((team >= 0) && (Mission_goals[i].team != team)) { continue; }
...
int mission_goals_incomplete (int desired_type, int team) {
    ...
    for (i = 0; i < Num_goals; i++) {
        ...
        if (Mission_goals[i].satisfied == GOAL_INCOMPLETE) { return 1; }
    }
