...
void supernova_start (int seconds) {
    // bogus time
    if ((float)seconds < SUPERNOVA_CUT_TIME) { return; }
...
void supernova_stop () {
    ...
    if (Supernova_status != SUPERNOVA_STARTED) { return; }

    // We're too late.
    if (supernova_time_left () < SUPERNOVA_CUT_TIME) { return; }
...
DCF_INT2 (
    sn_part, sn_particles, 0, INT_MAX,
...
void supernova_do_particles () {
    ...
    // no player ship
    if ((Player_obj == NULL) || (Player_ship == NULL)) { return; }

    // timestamp
    if ((Supernova_particle_stamp == -1) ||
        ...
        for (idx = 0; idx < 10; idx++) {
            if (Cmdline_old_collision_sys) {
                submodel_get_two_random_points (
                    Ship_info[Player_ship->ship_info_index].model_num, 0, &ta,
                    &tb);
            }
            else {
...
DCF_FLOAT2 (
    sn_shud, sn_shudder, 0.0, FLT_MAX,
    "Sets camera shudder rate for being in supernova shockwave (default is "
    "0.45)");
...
void supernova_process () {
    ...
    if (sn_stage) {
        ...
        // sound stuff
        if ((Supernova_time <= SUPERNOVA_SOUND_1_TIME) &&
            !Supernova_sound_1_played) {
            Supernova_sound_1_played = 1;
            snd_play (
                gamesnd_get_game_sound (GameSounds::SUPERNOVA_1), 0.0f, 1.0f,
                SND_PRIORITY_MUST_PLAY);
        }
        if ((Supernova_time <= SUPERNOVA_SOUND_2_TIME) &&
            !Supernova_sound_2_played) {
            Supernova_sound_2_played = 1;
            snd_play (
                gamesnd_get_game_sound (GameSounds::SUPERNOVA_2), 0.0f, 1.0f,
                SND_PRIORITY_MUST_PLAY);
        }

        // if we've crossed from stage 1 to stage 2 kill all particles and
        // stick a bunch on the player ship
        ...
        if ((sn_stage >= 2) && (sn_stage != 5)) { supernova_do_particles (); }
        ...
        if (Supernova_time < 0.0f) {
            ...
            if (Supernova_fade_to_white >= SUPERNOVA_FADE_TO_WHITE_TIME) {
                if (!Supernova_popup) {
                    ...
                    //
                    // don't actually check for a specific campaign here since
                    // others may want to end this way but we should test
...
int supernova_active () {
    ...
    if (Supernova_status == SUPERNOVA_NONE) { return 0; }
...
float supernova_time_left () { return Supernova_time; }

// pct complete the supernova (0.0 to 1.0)
float supernova_pct_complete () {
    // bogus
    if (!supernova_active ()) { return -1.0f; }
...
int supernova_camera_cut () {
    ...
    if (!supernova_active ()) { return 0; }

    // if we're past the critical time
    if (Supernova_time <= SUPERNOVA_CUT_TIME) { return 1; }
...
DCF_FLOAT2 (
    sn_dist, sn_distance, 0.0, FLT_MAX,
    "Sets supernova shockwave distance (default is 300.0f)");

DCF_FLOAT2 (
    sn_cam_dist, sn_cam_distance, 0.0, FLT_MAX,
...
void supernova_get_eye (vec3d* eye_pos, matrix* eye_orient) {
    ...
    vm_vec_scale_add (
        &Supernova_camera_pos, &Player_obj->pos, &whee.vec.rvec,
        sn_cam_distance);
    ...
    *eye_pos = Supernova_camera_pos;
    ...
    // otherwise move it
        ...
        // get a vector somewhere between the supernova shockwave and the
        // player ship
        at = Player_obj->pos;
        ...
        float pct =
            ((SUPERNOVA_CUT_TIME - Supernova_time) /
             SUPERNOVA_CAMERA_MOVE_TIME);
