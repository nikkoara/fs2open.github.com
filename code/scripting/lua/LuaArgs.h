...
public:
    ArgumentException (const std::string& message = "Argument Error!")
        SCP_NOEXCEPT : LuaException (message) {}
...
inline int getArgsInternal (lua_State* /*L*/, bool, int, int) { return 0; }

template< typename T >
inline int popArgumentValue (
    lua_State* L, T& target, bool& /*optionalOut*/, int& stackIndexOut) {
...
template<>
inline int popArgumentValue< opt > (
    lua_State* /*L*/, opt& /*target*/, bool& optionalOut,
    int& /*stackIndexOut*/) {
    optionalOut = true;
...
inline int getArgsInternal (
    lua_State* L, bool optional_arg, int stackIndex, int numArgs, T& target,
    Args&... args) {
