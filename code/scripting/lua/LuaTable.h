 ...
 *
 * @warning Never manipulate the lua stack while using an instance of this
 * class! It's ok to change the stack but it must be in the exact same state
 * when you call the next method of this class.
 */
 ...
 *
 * This class provides a high-level interface to lua tables without the need to
 * directly call lua-API functions.
 *
 * @see LuaConvert
 */
...
public:
     ...
     *
     * This wraps a shared pointer to an iterator so that this class can be
     * copied like the iterator interface expects
     */
    class iterator
        : public std::iterator<
              std::input_iterator_tag, std::pair< LuaValue, LuaValue > > {
    public:
    ...
    void addValue (const IndexType& index, const ValueType& value) {
        ...
        // Push the index and value onto the stac by using the template
        // functions
     ...
     * @return @c true when the value could be successfully converted, @c false
     * otherwise
     */
    template< class IndexType, class ValueType >
     ...
     *
     * @exception LuaException Thrown when an error occurs while converting the
     * value
     */
     ...
     *
     * This can be used with STL algorithms or with the C++11
     * range-base-for-loop
     *
...
bool popValue (
    lua_State* luaState, LuaTable& target, int stackposition = -1,
    bool remove = true);
}
} // namespace luacpp
