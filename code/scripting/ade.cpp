...
// Index 2 - String (ie the key we're trying to access; Object.string,
// Object:string, Object['string'], etc) Index 3 - (Optional) Argument we are
// trying to set Object.String = Argument
int ade_index_handler (lua_State* L) {
    ...
    if (lua_getmetatable (L, obj_ldx)) {
        ...
        if (lua_isuserdata (L, obj_ldx) && ade_id != UINT_MAX &&
            !ADE_SETTING_VAR) {
            ...
            ODATA_SIG_TYPE sig =
                *(ODATA_SIG_TYPE*)(ud + entry->Value.Object.size);
        ...
        if (lua_istable (L, -1)) {
            ...
            if (lua_isfunction (L, -1)) {
                // Set upvalue
                lua_pushvalue (
                    L, lua_upvalueindex (ADE_SETTING_UPVALUE_INDEX));
                if (lua_setupvalue (L, -2, ADE_SETTING_UPVALUE_INDEX) ==
                    NULL) {
        ...
        if (lua_isfunction (L, -1)) {
            ...
            // Set arguments
            for (i = 1; i <= last_arg_ldx; i++) { lua_pushvalue (L, i); }
    ...
    //*****STEP 7: Set sig thingie
        ...
        ADE_SETTING_VAR && ade_id != UINT_MAX && mtb_ldx != INT_MAX &&
        lua_isuserdata (L, obj_ldx)) {
        // WMC - I assume char is one byte
    ...
    if (type_name != NULL) {
        LuaError (
            L, "Could not find index '%s' in type '%s'",
...
int ade_get_operator (const char* tablename) {
    for (auto iter = std::begin (ade_Operators);
         iter != std::end (ade_Operators); ++iter) {
...
const ptrdiff_t Lua_type_names_num =
    std::distance (std::begin (Lua_type_names), std::end (Lua_type_names));
...
ade_table_entry& ade_manager::getEntry (size_t idx) {
    Assertion (
        idx < _table_entries.size (),
        "Invalid index " SIZE_T_ARG " specified!", idx);
...
ade_table_entry::ade_table_entry ()
    : Name (NULL), ShortName (NULL), Idx (UINT_MAX), ParentIdx (UINT_MAX),
      DerivatorIdx (UINT_MAX), Instanced (false), Size (0), Arguments (NULL),
      Description (NULL), ReturnType (NULL), ReturnDescription (NULL),
      Num_subentries (0) {
    Type = '\0';
...
int ade_table_entry::SetTable (lua_State* L, int p_amt_ldx, int p_mtb_ldx) {
    ...
    if (Instanced) {
        ...
        if (nset) { data_ldx = lua_gettop (L); }
        else {
            LuaError (
                L,
                "ade_table_entry::SetTable - Could not set data for '%s' "
                "(" SIZE_T_ARG ")",
                GetName (), Idx);
        }
        ...
        if (data_ldx != INT_MAX) {
            ...
            if (p_amt_ldx != LUA_GLOBALSINDEX) { cleanup_items++; }
            ...
            if (desttable_ldx != INT_MAX) {
                ...
                if (lua_isfunction (L, data_ldx)) {
                    ...
                    if (lua_setupvalue (L, data_ldx, 1) == NULL) {
                        LuaError (
                            L,
                            "ade_table_entry::SetTable - Could not set "
                            "upvalue for '%s' (" SIZE_T_ARG ")",
                            GetName (), Idx);
                    }
                }
                ...
                LuaError (
                    L,
                    "ade_table_entry::SetTable - Could not instance '%s' "
                    "(" SIZE_T_ARG ")",
                    GetName (), Idx);
            }
        }
    ...
    if (Num_subentries ||
        (DerivatorIdx != UINT_MAX && ade_manager::getInstance ()
                                         ->getEntry (DerivatorIdx)
                                         .Num_subentries)) {
        // Create the new metatable
        if (!luaL_newmetatable (L, Name)) {
            LuaError (
                L,
                "ade_table_entry::SetTable - Couldn't create metatable for "
                "table entry '%s' - does a Lua object already exist with this "
                "name?",
                Name);
            return 0;
        }
        ...
        // WMC - Make sure it's instanced, too. This helps keep crashes from
        // happening...
        if (data_ldx != INT_MAX) {
        ...
        lua_pushstring (
            L, "ade_index_handler(get)"); // upvalue(1) = function name
        ...
        lua_pushstring (
            L, "ade_index_handler(set)"); // upvalue(1) = function name
    ...
    if (amt_ldx != INT_MAX) {
        ...
        if (DerivatorIdx != UINT_MAX) {
            for (i = 0; i < ade_manager::getInstance ()
                                ->getEntry (DerivatorIdx)
                                .Num_subentries;
                 i++) {
                ade_manager::getInstance ()
                    ->getEntry (ade_manager::getInstance ()
                                    ->getEntry (DerivatorIdx)
                                    .Subentries[i])
                    .SetTable (L, amt_ldx, mtb_ldx);
            }
        }
        for (i = 0; i < Num_subentries; i++) {
            ade_manager::getInstance ()
                ->getEntry (Subentries[i])
                .SetTable (L, amt_ldx, mtb_ldx);
        }
    }
...
void ade_table_entry::OutputMeta (FILE* fp) {
    ...
    if (!skip_this) {
        ...
        case 'o': {
            ...
            if (Name == NULL) {
                if (DerivatorIdx == UINT_MAX)
                    ...
                    fprintf (
                        fp, "<h2>%s:<a href=\"#%s\">%s</a></h2>\n",
                        getTableEntry (DerivatorIdx).GetName (),
                        getTableEntry (DerivatorIdx).GetName (),
                        getTableEntry (DerivatorIdx).GetName ());
            }
            else {
                ...
                if (ShortName != NULL) fprintf (fp, " (%s)", ShortName);
                if (DerivatorIdx != UINT_MAX)
                    fprintf (
                        fp, ":<a href=\"#%s\">%s</a>",
                        getTableEntry (DerivatorIdx).GetName (),
            ...
            if (ReturnType != NULL) {
                fprintf (
                    fp, "<dd><b>Return Type: </b> %s<br>&nbsp;</dd>\n",
                    ReturnType);
            }

            if (ReturnDescription != NULL) {
                fprintf (
                    fp, "<dd><b>Return Description: </b> %s<br>&nbsp;</dd>\n",
                    ReturnDescription);
            }
        } break;
        case 'u': {
            ...
            if (ReturnType != NULL)
                ...
                fputs ("nil", fp);
            fputs (" </i>", fp);
            if (Name == NULL) { fprintf (fp, "<b>%s", ShortName); }
            else {
        ...
        case 'v': {
            ...
            if (Name == NULL) { fprintf (fp, "<b>%s</b>\n", ShortName); }
            else {
                fprintf (fp, "<b>%s", Name);
                if (ShortName != NULL) fputs (ShortName, fp);
                fputs ("</b>\n", fp);
            }
            if (Arguments != NULL) fprintf (fp, " = <i>%s</i>", Arguments);
            fputs ("</dt>\n", fp);
    ...
    if (!skip_this) fputs ("<br></dl></dd>\n", fp);
}

ade_lib::ade_lib (
    const char* in_name, const ade_lib_handle* parent,
    const char* in_shortname, const char* in_desc) {
    ade_table_entry ate;
    ...
    ate.Value.Object.buf =
        &Num_reinforcements; // WMC - I just chose Num_ship_classes randomly.
                             // MageKing17 - changed to Num_reinforcements,
                             // likewise at random, due to the removal of
                             // Num_ship_classes
...
const char* ade_lib::GetName () const {
    if (GetIdx () == UINT_MAX) return "<Invalid>";

    return getTableEntry (GetIdx ()).GetName ();
}

ade_func::ade_func (
    const char* name, lua_CFunction func, const ade_lib_handle& parent,
    const char* args, const char* desc, const char* ret_type,
    const char* ret_desc) {
    ade_table_entry ate;
    ...
    LibIdx = ade_manager::getInstance ()
                 ->getEntry (parent.GetIdx ())
                 .AddSubentry (ate);
}

ade_virtvar::ade_virtvar (
    const char* name, lua_CFunction func, const ade_lib_handle& parent,
    const char* args, const char* desc, const char* ret_type,
    const char* ret_desc) {
    ade_table_entry ate;
...
ade_indexer::ade_indexer (
    lua_CFunction func, const ade_lib_handle& parent, const char* args,
...
void ade_stackdump (lua_State* L, char* stackdump) {
    ...
    for (int argnum = 1; argnum <= stacksize; argnum++) {
        ...
        switch (type) {
        case LUA_TNIL: strcat (stackdump, "NIL"); break;
        case LUA_TNUMBER:
        ...
        case LUA_TTABLE: {
            if (lua_getmetatable (L, argnum)) {
                ...
                if (lua_isnumber (L, -1)) {
                    sprintf (
                        buf, "Table [%s]",
                        getTableEntry ((size_t)lua_tonumber (L, -1)).Name);
                    strcat (stackdump, buf);
                }
                else
        ...
        case LUA_TFUNCTION:
            ...
            {
                char* upname = (char*)lua_getupvalue (
                    L, argnum, ADE_FUNCNAME_UPVALUE_INDEX);
                if (upname != NULL) {
                    strcat (stackdump, " ");
        ...
        default:
            sprintf (
                buf, "<UNKNOWN>: %s (%f) (%s)", lua_typename (L, type),
                lua_tonumber (L, argnum), lua_tostring (L, argnum));
            strcat (stackdump, buf);
            break;
        }
    }
...
const char* ade_get_type_string (lua_State* L, int argnum) {
    ...
    if (type < 0 || type >= Lua_type_names_num) return "Unknown";
...
int ade_set_object_with_breed (lua_State* L, int obj_idx) {
    ...
    switch (objp->type) {
    case OBJ_SHIP: return ade_set_args (L, "o", l_Ship.Set (object_h (objp)));
    case OBJ_ASTEROID:
    ...
    case OBJ_BEAM: return ade_set_args (L, "o", l_Beam.Set (object_h (objp)));
    default: return ade_set_args (L, "o", l_Object.Set (object_h (objp)));
    }
}
...
void load_default_script (lua_State* L, const char* name) {
    ...
    try {
        ...
        func.setErrorFunction (
            LuaFunction::createFromCFunction (L, ade_friendly_error));
        try {
            func ();
        }
        catch (const LuaException&) {
            // The execution of the function may also throw an exception but
            // that should have been handled by a LuaError before
        }
    }
