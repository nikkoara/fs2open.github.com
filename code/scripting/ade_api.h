...
const size_t INVALID_ID =
    (size_t)-1; // Use -1 to get highest possible unsigned number
...
#define ADE_FUNC(name, parent, args, desc, ret_type, ret_desc)               \
    ...
    ::scripting::ade_func parent##_##name (                                  \
        #name, parent##_##name##_f, parent, args, desc, ret_type, ret_desc); \
    static int parent##_##name##_f (lua_State* L)

/**
 * @brief Declare an API variable
 *
 * Use this to handle forms of type vec.x and vec['x']. Basically an indexer
 * for a specific variable. Format string should be "o*%", where * is indexing
 * value, and % is the value to set to when LUA_SETTTING_VAR is set
 *
...
#define ADE_VIRTVAR(name, parent, args, desc, ret_type, ret_desc)            \
    static int parent##_##name##_f (lua_State* L);                           \
    ::scripting::ade_virtvar parent##_##name (                               \
        #name, parent##_##name##_f, parent, args, desc, ret_type, ret_desc); \
    static int parent##_##name##_f (lua_State* L)
 ...
 * Format string should be "o*%", where * is indexing value, and % is the value
 * to set to when LUA_SETTTING_VAR is set
 *
...
#define ADE_INDEXER(parent, args, desc, ret_type, ret_desc)            \
    ...
    ::scripting::ade_indexer parent##___indexer (                      \
        parent##___indexer_f, parent, args, desc, ret_type, ret_desc); \
    static int parent##___indexer_f (lua_State* L)
...
#define ADE_OBJ_DERIV_IMPL(field, type, name, desc, deriv)                  \
    ...
    const ::scripting::ade_obj< type >& field =                             \
        SCP_TOKEN_CONCAT (get_, field) ()

/**
 * @brief Define an API object
 *
 * An object is similar to a C++ class. Use this if you want to return a
 * special type from a function that should be able to do more on its own.
 *
...
#define ADE_OBJ(field, type, name, desc) \
    ADE_OBJ_DERIV_IMPL (field, type, name, desc, nullptr)

/**
 * @brief Define an API object that derives from another
 *
 * This is the same as ADE_OBJ but this allows to derive this class from
 * another
 *
 * @param field The name of the field by which the class should be accessible
 * @param type The type of the data the class contains
 * @param name The name the class should have in the documentation
 * @param desc Documentation about what this class is
 * @param deriv The class to derive from. This should be the name of the class
 * field (e.g. l_Object)
 *
 * @ingroup ade_api
 */
#define ADE_OBJ_DERIV(field, type, name, desc, deriv) \
    ADE_OBJ_DERIV_IMPL (                              \
        field, type, name, desc, &SCP_TOKEN_CONCAT (get_, deriv) ())

/**
 * @brief Declare an API object but don't define it
 *
 * You should use this in headers if the class should be able to be used by
 * other files
 *
...
#define DECLARE_ADE_OBJ(field, type)                              \
    extern const ::scripting::ade_obj< type >& SCP_TOKEN_CONCAT ( \
        get_, field) ();                                          \
 ...
 *
 * A library is similar to a C++ namespace or a class with static functions. It
 * can be used to group multiple functions together that serve a similar pupose
 *
 ...
 * @param short_name The short name of the library, makes writing scripts
 * easier
 * @param desc Documentation about what this class is
 *
 * @ingroup ade_api
...
#define ADE_LIB(field, name, short_name, desc) \
    ADE_LIB_IMPL (field, name, short_name, desc, nullptr)

/**
 * @brief Define an API library which is the child of another library
 *
 * A library is similar to a C++ namespace or a class with static functions. It
 * can be used to group multiple functions together that serve a similar
 * pupose.
 *
...
#define ADE_LIB_DERIV(field, name, short_name, desc, parent) \
    ADE_LIB_IMPL (                                           \
        field, name, short_name, desc, &SCP_TOKEN_CONCAT (get_, parent) ())

/**
 * @brief Declare an API library but don't define it
 *
 * You should use this in headers if the library should be able to be used by
 * other files
 *
...
#define DECLARE_ADE_LIB(field)                                            \
    ...
    static const ::scripting::ade_lib* SCP_TOKEN_CONCAT (                 \
