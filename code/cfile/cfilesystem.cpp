...
int cf_get_packfile_count (cf_root* root) {
    ...
    for (i = CF_TYPE_ROOT; i < CF_MAX_PATH_TYPES; i++) {
        ...
        if (find_handle != -1) {
            do {
                if (!(find.attrib & _A_SUBDIR)) { packfile_count++; }
...
static char normalize_directory_separator (char in) {
    if (in == '/') { return DIR_SEPARATOR_CHAR; }
                ...
                if (!(find.attrib & _A_SUBDIR)) {
                    ...
                    if (ext) {
                        if (is_ext_in_list (Pathtypes[i].extensions, ext)) {
                            ...
                            sprintf (
                                file_name, "%s%s%s", search_directory.c_str (),
                                DIR_SEPARATOR_STR, find.name);
...
void cf_search_memory_root (int root_index) {
    ...
    for (auto& default_file : default_files) {
        ...
        if (strlen (default_file.path_type) <= 0) {
            // Ignore pure builtin files. They should only be accessible with
            // defaults_get_file
            continue;
        }
        ...
        for (int i = 0; i < CF_MAX_PATH_TYPES; ++i) {
            if (Pathtypes[i].path == nullptr) { continue; }
        ...
        Assertion (
            pathtype != -1,
            "Default file '%s%s%s' does not use a valid path type!",
            default_file.path_type, DIR_SEPARATOR_STR, default_file.filename);
        ...
        file->write_time = time (nullptr); // Just assume that memory files
                                           // were last written to just now
...
static bool verify_file_list_child () {
    ...
    if (Get_file_list_child[0] == DIR_SEPARATOR_CHAR) { return false; }
        ...
        do {
            ...
            if (!(find.attrib & _A_SUBDIR)) {
                if (!Get_file_list_filter ||
                    (*Get_file_list_filter) (find.name)) {
                    if (check_duplicates &&
                        cf_file_already_in_list (list, find.name)) {
                        continue;
                    }
...
int cf_get_file_list (
...
#if defined _WIN32
    cf_create_default_path_string (
        filespec, sizeof (filespec) - 1, pathtype, filter, false,
        location_flags);
    ...
    if (find_handle != -1) {
        do {
            ...
            if (strlen (find.name) >= MAX_FILENAME_LEN) continue;
            ...
            if (!(find.attrib & _A_SUBDIR)) {
                if (!Get_file_list_filter ||
                    ...
                    list[num_files][l] = 0;
                    if (info) info[num_files].write_time = find.time_write;
...
int cf_get_file_list_preallocated (
    ...
    if (find_handle != -1) {
        do {
            ...
            if (!(find.attrib & _A_SUBDIR)) {
                if (strcmp (strstr (filter, "."), strstr (find.name, ".")) !=
                    0)
                    continue;
