...
// this is the constant which defines when a beam is an "area" beam. meaning,
// when we switch on sphereline checking and when a beam gets "stopped" by an
// object. It is a percentage of the object radius which the beam must be wider
// than
#define BEAM_AREA_PERCENT 0.4f

// randomness factor - all beam weapon aiming is adjusted by +/- some factor
// within this range
...
int Beam_good_slash_octants[BEAM_NUM_GOOD_OCTANTS][4] = {
    ...
    { 7, 0, 1, 0 }, { 1, 6, 1, 0 }, { 6, 1, 0, 1 }, { 5, 2, 0, 1 },
    { 0, 7, 0, 1 }, { 7, 1, 1, 0 }, { 6, 0, 1, 0 },
};
int Beam_good_shot_octants[BEAM_NUM_GOOD_OCTANTS][4] = {
    { 5, 0, 1, 0 }, // octant, octant, min/max pt, min/max pt
    { 7, 2, 1, 0 }, { 7, 1, 1, 0 }, { 6, 0, 1, 0 }, { 7, 3, 1, 0 },
    { 6, 2, 1, 0 }, { 5, 1, 1, 0 }, { 4, 0, 1, 0 },
};

// debug stuff - keep track of how many collision tests we perform a second and
// how many we toss a second
...
#define BEAM_WARMUP_PCT(b)                                          \
    (((float)Weapon_info[b->weapon_info_index].b_info.beam_warmup - \
      (float)timestamp_until (b->warmup_stamp)) /                   \
     (float)Weapon_info[b->weapon_info_index].b_info.beam_warmup)

// beam warmdown completion %
#define BEAM_WARMDOWN_PCT(b)                                          \
    (((float)Weapon_info[b->weapon_info_index].b_info.beam_warmdown - \
      (float)timestamp_until (b->warmdown_stamp)) /                   \
     (float)Weapon_info[b->weapon_info_index].b_info.beam_warmdown)
...
typedef struct beam_light_info {
    ...
    ubyte source;  // 0 to light the shooter, 1 for lighting any ship the beam
                   // passes, 2 to light the collision ship
...
DCF (b_whack_damage, "Sets the whack damage threshold (Default is 150f)") {
    if (dc_optional_string_either ("help", "--help")) {
        dc_printf (
            "Sets the threshold to determine whether a big whack or a small "
            "whack should be applied. Values equal or greater than this "
            "threshold will trigger a big whack, while smaller values will "
            "trigger a small whack\n");
        return;
    }
...
// aim the beam (setup last_start and last_shot - the endpoints). also
// recalculates object collision info
...
void beam_get_octant_points (
    int modelnum, object* objp, int oct_index,
    int oct_array[BEAM_NUM_GOOD_OCTANTS][4], vec3d* v1, vec3d* v2);

// given an object, return its model num
int beam_get_model (object* objp);

// for a given object, and a firing beam, determine its critical dot product
// and range
void beam_get_cull_vals (
    object* objp, beam* b, float* cull_dot, float* cull_dist);
...
void beam_calc_facing_pts (
    vec3d* top, vec3d* bot, vec3d* fvec, vec3d* pos, float w, float z_add);
...
// start the firing phase for the beam, return 0 if the beam failed to start,
...
void beam_add_collision (
    beam* b, object* hit_object, mc_info* cinfo, int quad = -1,
    int exit_flag = 0);
...
// return the amount of damage which should be applied to a ship. basically,
// filters friendly fire damage
...
void beam_init () { beam_level_close (); }
...
// fire a beam, returns nonzero on success. the innards of the code handle all
// the rest, foo
int beam_fire (beam_fire_info* fire_info) {
    ...
    if ((fire_info->beam_info_index < 0) ||
        ...
        !(Weapon_info[fire_info->beam_info_index]
              .wi_flags[Weapon::Info_Flags::Beam])) {
        UNREACHABLE (
            "beam_info_index (%d) invalid (either <0, >= %d, or not actually "
            "a beam)!\n",
            fire_info->beam_info_index, Num_weapon_types);
        return -1;
    }
    ...
    if (!(fire_info->bfi_flags & BFIF_FLOATING_BEAM) &&
        ((fire_info->shooter->type != OBJ_SHIP) ||
    ...
    if (new_item == &Beam_free_list) { return -1; }
    ...
    new_item->sig =
        (fire_info->shooter != NULL) ? fire_info->shooter->signature : 0;
    ...
    new_item->target_sig =
        (fire_info->target != NULL) ? fire_info->target->signature : 0;
    ...
    new_item->team = (firing_ship == NULL)
                         ? fire_info->team
                         : static_cast< char > (firing_ship->team);
    ...
    new_item->firingpoint = (fire_info->bfi_flags & BFIF_FLOATING_BEAM)
                                ? -1
    ...
    if (new_item->type == BEAM_TYPE_D) {
        ...
        if ((fire_info->target != NULL) &&
            ...
            if (Ship_info[target_ship->ship_info_index].class_type > -1 &&
                (Ship_types[Ship_info[target_ship->ship_info_index].class_type]
                     .flags[Ship::Type_Info_Flags::Beams_easily_hit])) {
                new_item->type = BEAM_TYPE_A;
            }
        }
    }
    ...
    // beam_get_binfo(...) determines exactly how the beam will behave over the
    // course of its life it fills in binfo, which we can pass to clients in
    // multiplayer
    if (fire_info->beam_info_override != NULL) {
        ...
        beam_get_binfo (
            ...
            wip->b_info.beam_shots); // to fill in b_info	- the set of
                                     // directional aim vectors
    }
    ...
    objnum = obj_create (
        OBJ_BEAM,
        ((fire_info->shooter != NULL) ? OBJ_INDEX (fire_info->shooter) : -1),
        BEAM_INDEX (new_item), &vmd_identity_matrix, &vmd_zero_vector, 1.0f,
        default_flags);
    if (objnum < 0) {
    ...
    if (new_item->objp != nullptr && Weapons_inherit_parent_collision_group) {
        Objects[objnum].collision_group_id =
            new_item->objp->collision_group_id;
    }
...
// fire a targeting beam, returns objnum on success. a much much simplified
// version of a beam weapon targeting lasers last _one_ frame. For a continuous
// stream - they must be created every frame. this allows it to work smoothly
// in multiplayer (detect "trigger down". every frame just create a targeting
// laser firing straight out of the object. this way you get all the advantages
// of nice rendering and collisions). NOTE : only references beam_info_index
// and shooter
int beam_fire_targeting (fighter_beam_fire_info* fire_info) {
    ...
    if (wip->b_info.beam_type != BEAM_TYPE_C) { return -1; }
    ...
    // create the associated object
    objnum = obj_create (
        OBJ_BEAM, OBJ_INDEX (fire_info->shooter), BEAM_INDEX (new_item),
        &vmd_identity_matrix, &vmd_zero_vector, 1.0f,
...
int beam_get_parent (object* bm) {
    ...
    if (bm->type != OBJ_BEAM) { return -1; }
    if (bm->instance < 0) { return -1; }
    b = &Beams[bm->instance];

    if (b->objp == NULL) { return -1; }

    // if the object handle is invalid
    if (b->objp->signature != b->sig) { return -1; }
...
int beam_get_weapon_info_index (object* bm) {
    ...
    Assert (
        (Beams[bm->instance].weapon_info_index > -1) &&
        (Beams[bm->instance].weapon_info_index < Num_weapon_types));

    // return weapon_info_index
    return Beams[bm->instance].weapon_info_index;
}

// given a beam object, get the # of collisions which happened during the last
// collision check (typically, last frame)
...
int beam_get_collision (
    ...
    *collision_objnum =
        Beams[Objects[objnum].instance].r_collisions[num].c_objnum;
    return 1;
}
...
void beam_unpause_sounds () {
    ...
    if (moveup == NULL) { return; }
    while (moveup != END_OF_LIST (&Beam_used_list)) {
        if (Cmdline_no_3d_sound) { beam_recalc_sounds (moveup); }
        else {
            if (moveup->beam_sound_loop.isValid ()) {
...
void beam_get_global_turret_gun_info (
    object* objp, ship_subsys* ssp, vec3d* gpos, vec3d* gvec, int use_angles,
    vec3d* targetp, bool fighter_beam) {
    ship_get_global_turret_gun_info (
        objp, ssp, gpos, gvec, use_angles, targetp);
...
void beam_type_a_move (beam* b) {
    ...
    // LEAVE THIS HERE OTHERWISE MUZZLE GLOWS DRAW INCORRECTLY WHEN WARMING UP
    // OR DOWN get the "originating point" of the beam for this frame.
    // essentially bashes last_start
    if (b->subsys != NULL)
        beam_get_global_turret_gun_info (
            b->objp, b->subsys, &b->last_start, &temp, 1, &temp2,
            (b->flags & BF_IS_FIGHTER_BEAM) > 0);

    // if the "warming up" timestamp has not expired
    if ((b->warmup_stamp != -1) || (b->warmdown_stamp != -1)) { return; }
...
#define BEAM_T(b)                            \
    (((b->binfo.delta_ang / b->life_total) * \
      (b->life_total - b->life_left)) /      \
     b->binfo.delta_ang)
void beam_type_b_move (beam* b) {
    vec3d actual_dir;
    vec3d temp, temp2;
    ...
    // if the two direction vectors are _really_ close together, just use the
    // original direction
    dot_save = vm_vec_dot (&b->binfo.dir_a, &b->binfo.dir_b);
    if ((double)dot_save >= 0.999999999) { actual_dir = b->binfo.dir_a; }
    // otherwise move towards the dir	we calculated when firing this beam
    else {
        vm_vec_interp_constant (
            &actual_dir, &b->binfo.dir_a, &b->binfo.dir_b, BEAM_T (b));
    }
    ...
    if (!is_valid) {
        ...
        vm_vec_scale_add (
            &b->last_shot, &b->last_start, &actual_dir, b->range);
    }
}
...
void beam_type_c_move (beam* b) {
    ...
    vm_vec_scale_add (
        &b->last_shot, &b->last_start, &b->objp->orient.vec.fvec, b->range);
    ...
    shipp->weapon_energy -= num_fire_points *
                            Weapon_info[b->weapon_info_index].energy_consumed *
                            flFrametime;

    if (shipp->weapon_energy < 0.0f) { shipp->weapon_energy = 0.0f; }
}
...
void beam_type_d_move (beam* b) {
    ...
    b->flags &= ~BF_SAFETY;
    if (fire_wait) { b->flags |= BF_SAFETY; }
...
void beam_type_d_get_status (beam* b, int* shot_index, int* fire_wait) {
    ...
    *shot_index = (int)(beam_time / shot_time);
    ...
    // determine if its the firing or waiting section of the shot (fire happens
    // first, THEN wait)
    *fire_wait = 0;
...
void beam_type_e_move (beam* b) {
    ...
    if (b->subsys == NULL) { // If we're a free-floating beam, there's nothing
                             // to calculate here.
        return;
    }
    ...
    // essentially bashes last_start
    beam_get_global_turret_gun_info (
        b->objp, b->subsys, &b->last_start, &turret_norm, 1, &temp,
        (b->flags & BF_IS_FIGHTER_BEAM) > 0);
...
// pre-move (before collision checking - but AFTER ALL OTHER OBJECTS HAVE BEEN
// MOVED)
void beam_move_all_pre () {
    ...
    while (moveup != END_OF_LIST (&Beam_used_list)) {
        ...
        if (!physics_paused) {
            ...
            case BEAM_TYPE_A: beam_type_a_move (b); break;

            // type B beam weapons move across the target somewhat randomly
            case BEAM_TYPE_B: beam_type_b_move (b); break;

            // type C beam weapons are attached to a fighter - pointing forward
            case BEAM_TYPE_C: beam_type_c_move (b); break;

            // type D
            case BEAM_TYPE_D: beam_type_d_move (b); break;
...
void beam_move_all_post () {
    ...
    while (moveup != END_OF_LIST (&Beam_used_list)) {
        ...
        if (moveup->warmup_stamp != -1) {
            ...
            if (bf_status < 0) { beam_delete (moveup); }
            else {
                if (moveup->objp != NULL) {
                ...
                if (timestamp_elapsed (moveup->warmup_stamp)) {
                    // start firing
                    if (!beam_start_firing (moveup)) { beam_delete (moveup); }
                }
            }
        ...
        if (!physics_paused) { moveup->life_left -= flFrametime; }

        // if we're past the shrink point, start shrinking the beam
        if (moveup->life_left <=
            (moveup->life_total * bwi->beam_shrink_factor)) {
        ...
        if (moveup->flags & BF_SHRINK) {
            ...
            if (moveup->shrink < 0.1f) { moveup->shrink = 0.1f; }
        }
        ...
        if (bf_status <= 0) {
            ...
            if (bf_status == -1) { beam_delete (moveup); }
        ...
        // done firing, so go into the warmdown phase
        {
            if ((moveup->life_left <= 0.0f) &&
                (moveup->warmdown_stamp == -1) && (moveup->framecount > 1)) {
                beam_start_warmdown (moveup);
        ...
        // handle any collisions which occured collision (will take care of
        // applying damage to all objects which got hit)
...
void beam_render (beam* b, float u_offset) {
    ...
    if ((b == NULL) || (b->weapon_info_index < 0)) return;

    // if the beam start and endpoints are the same
    if (vm_vec_same (&b->last_start, &b->last_shot)) return;
    ...
    length =
        vm_vec_dist (&b->last_start, &b->last_shot); // beam tileing -Bobboau
    ...
    for (s_idx = 0; s_idx < bwi->beam_num_sections; s_idx++) {
        ...
        beam_calc_facing_pts (
            &top2, &bottom2, &fvec, &b->last_shot,
            bwsi->width * scale * scale * b->shrink, bwsi->z_add);
        ...
        P_VERTICES ();
        STUFF_VERTICES (); // stuff the beam with creamy goodness (texture
                           // coords)

        if (bwsi->tile_type == 1)
            u_scale = length / (bwsi->width * 0.5f) /
                      bwsi->tile_factor; // beam tileing, might make a tileing
                                         // factor in beam index later -Bobboau
        else
            u_scale = bwsi->tile_factor;

        verts[1]->texture_position.u =
            (u_scale +
             (u_offset * bwsi->translation)); // beam tileing -Bobboau
        verts[2]->texture_position.u =
            (u_scale +
             (u_offset * bwsi->translation)); // beam tileing -Bobboau
        ...
        if (bwsi->texture.num_frames > 1) {
            ...
            framenum = bm_get_anim_frame (
                bwsi->texture.first_frame, b->beam_section_frame[s_idx],
                bwsi->texture.total_time, true);
        }

        material material_params;
        material_set_unlit_emissive (
            &material_params, bwsi->texture.first_frame + framenum, 0.9999f,
            2.0f);
        g3_render_primitives_colored_textured (
            &material_params, h1, 4, PRIM_TYPE_TRIFAN, false);
    }
...
void beam_generate_muzzle_particles (beam* b) {
    ...
    if (!((b->Beam_muzzle_stamp == -1) ||
          timestamp_elapsed (b->Beam_muzzle_stamp))) {
        return;
    }

    // never generate anything past about 1/5 of the beam fire time
    if (b->warmup_stamp == -1) { return; }

    // get weapon info
    wip = &Weapon_info[b->weapon_info_index];

    // no specified particle for this beam weapon
    if (wip->b_info.beam_particle_ani.first_frame < 0) return;
    ...
    for (idx = 0; idx < particle_count; idx++) {
        ...
        if (b->objp != NULL) {
            vm_vec_add2 (
                &particle_dir,
                &b->objp->phys_info.vel); // move along with our parent
        }
...
static float get_current_alpha (vec3d* pos) {
    ...
    // if the point is inside the inner radius, alpha is based on distance to
    // the player's eye, becoming more transparent as it gets close
...
void beam_render_muzzle_glow (beam* b) {
    ...
    if (bwi->beam_glow.first_frame < 0) return;
    ...
    if (bwi->directional_glow == true) {
        ...
        There was actually some oversight here when developing the directional
        glow feature and any refactoring of it will require some more complex
        parameters for glow placement.
        */
        ...
        if (bwi->beam_glow.num_frames > 1) {
            ...
            framenum = bm_get_anim_frame (
                bwi->beam_glow.first_frame, b->beam_glow_frame,
                bwi->beam_glow.total_time, true);
        }

        // gr_set_bitmap(bwi->beam_glow.first_frame + framenum,
        // GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL, alpha * pct);
        ...
        material_set_unlit_emissive (
            &material_info, bwi->beam_glow.first_frame + framenum, alpha * pct,
            2.0f);
        g3_render_primitives_textured (
            &material_info, h1, 4, PRIM_TYPE_TRIFAN, false);
    }
    else {
        // draw the bitmap
        ...
        material mat_params;
        material_set_unlit_emissive (
            &mat_params, bwi->beam_glow.first_frame + framenum, alpha * pct,
            2.0f);
        ...
        if (pct > 0.3f) {
            ...
            g3_render_rect_screen_aligned (
                &mat_params, &pt, 0, rad * 0.75f, rad * 0.25f);
        }

        if (pct > 0.5f) {
            // g3_draw_bitmap(&pt, 0, rad * 0.45f, tmap_flags, rad * 0.55f);
            g3_render_rect_screen_aligned (
                &mat_params, &pt, 0, rad * 0.45f, rad * 0.55f);
        }

        if (pct > 0.7f) {
            // g3_draw_bitmap(&pt, 0, rad * 0.25f, tmap_flags, rad * 0.75f);
            g3_render_rect_screen_aligned (
                &mat_params, &pt, 0, rad * 0.25f, rad * 0.75f);
        }
    }
}
...
void beam_render_all () {
    ...
    while (moveup != END_OF_LIST (&Beam_used_list)) {
        ...
        if ((moveup->warmup_stamp == -1) && (moveup->warmdown_stamp == -1) &&
            !(moveup->flags & BF_SAFETY)) {
            // HACK -  if this is the first frame the beam is firing, don't
            // render it
...
// w == width of the diff between top and bottom around pos
void beam_calc_facing_pts (
    vec3d* top, vec3d* bot, vec3d* fvec, vec3d* pos, float w,
    float /*z_add*/) {
    vec3d uvec, rvec;
    vec3d temp;
...
void beam_add_light_small (beam* bm, object* objp, vec3d* pt_override = NULL) {
    ...
    if (pt_override == NULL) {
        float dist;
        vm_vec_dist_to_line (
            &objp->pos, &bm->last_start, &bm->last_shot, &near_pt, &dist);
        if (dist > light_rad) { return; }
    }
    else {
        near_pt = *pt_override;
    }
    ...
    // add a unique light
    light_add_point_unique (
        &near_pt, light_rad * 0.0001f, light_rad, pct, fr, fg, fb,
        OBJ_INDEX (objp));
}
...
void beam_add_light_large (beam* bm, object* objp, vec3d* pt0, vec3d* pt1) {
    ...
    light_add_tube (
        pt0, pt1, 1.0f, light_rad, 1.0f * noise, fr, fg, fb, OBJ_INDEX (objp));
}
...
void beam_add_light (beam* b, int objnum, int source, vec3d* c_point) {
    ...
    if (Beam_light_count >= MAX_BEAM_LIGHT_INFO) { return; }
    ...
    if (c_point != NULL) { l->c_point = *c_point; }
    else {
        Assert (source != 2);
        if (source == 2) { Beam_light_count--; }
    }
}
...
void beam_apply_lighting () {
    ...
    for (idx = 0; idx < Beam_light_count; idx++) {
        ...
        case 0:
            ...
            vm_vec_normalize_quick (&dir);
            vm_vec_scale (&dir, -0.8f); // TODO: This probably needs to *not*
                                        // be stupid. -taylor
            vm_vec_scale_add (
                &pt, &l->bm->last_start, &dir, bwi->beam_muzzle_radius * 5.0f);
        ...
        case 1:
            ...
            beam_add_light_large (
                l->bm, &Objects[l->objnum], &l->bm->last_start,
                &l->bm->last_shot);
            break;

        // from a collision
        case 2:
            // Valathil: Dont render impact lights for shaders, handled by tube
            // lighting
            break;
        }
    }
...
void beam_delete (beam* b) {
    ...
    if (b->objnum >= 0) { obj_delete (b->objnum); }
    b->objnum = -1;
...
int beam_get_model (object* objp) {
    ...
    case OBJ_WEAPON:
        ...
        if (Weapons[objp->instance].weapon_info_index < 0) { return -1; }
        return Weapon_info[Weapons[objp->instance].weapon_info_index]
            .model_num;

    case OBJ_DEBRIS:
        Assert (Debris[objp->instance].is_hull);
        if (!Debris[objp->instance].is_hull) { return -1; }
    ...
    case OBJ_ASTEROID:
        ...
        if (Asteroids[objp->instance].asteroid_type < 0) { return -1; }
        return Asteroid_info[Asteroids[objp->instance].asteroid_type]
            .model_num[pof];
...
void beam_start_warmup (beam* b) {
    ...
    b->warmup_stamp =
        timestamp (Weapon_info[b->weapon_info_index].b_info.beam_warmup);

    // start playing warmup sound
    if (!(Game_mode & GM_STANDALONE_SERVER) &&
        (Weapon_info[b->weapon_info_index]
             .b_info.beam_warmup_sound.isValid ())) {
        snd_play_3d (
            gamesnd_get_game_sound (
                Weapon_info[b->weapon_info_index].b_info.beam_warmup_sound),
            &b->last_start, &View_position);
    }
}
...
int beam_start_firing (beam* b) {
    ...
    case BEAM_TYPE_A:
    case BEAM_TYPE_D: beam_aim (b); break;

    case BEAM_TYPE_B: break;

    case BEAM_TYPE_C: break;
    ...
    case 0: beam_start_warmdown (b); return 1;
    }

    // start the beam firing sound now, if we haven't already
    if ((!b->beam_sound_loop.isValid ()) &&
        (Weapon_info[b->weapon_info_index]
             .b_info.beam_loop_sound.isValid ())) {
        b->beam_sound_loop = snd_play_3d (
            gamesnd_get_game_sound (
                Weapon_info[b->weapon_info_index].b_info.beam_loop_sound),
            &b->last_start, &View_position, 0.0f, NULL, 1, 1.0,
        ...
        if (Weapon_info[b->weapon_info_index].launch_snd.isValid ())
            snd_play_3d (
                gamesnd_get_game_sound (
                    Weapon_info[b->weapon_info_index].launch_snd),
                &b->last_start, &View_position);
        // niffwan - if launch_snd < 0, don't play any sound
    }

    Script_system.SetHookObjects (
        3, "Beam", &Objects[b->objnum], "User", b->objp, "Target", b->target);
    Script_system.RunCondition (
        CHA_BEAMFIRE, 0, NULL, &Objects[b->objnum], b->weapon_info_index);
...
void beam_start_warmdown (beam* b) {
    // timestamp
    b->warmdown_stamp =
        timestamp (Weapon_info[b->weapon_info_index].b_info.beam_warmdown);

    // start the warmdown sound
    if (Weapon_info[b->weapon_info_index]
            .b_info.beam_warmdown_sound.isValid ()) {
        snd_play_3d (
            gamesnd_get_game_sound (
                Weapon_info[b->weapon_info_index].b_info.beam_warmdown_sound),
            &b->last_start, &View_position);
    }
...
void beam_recalc_sounds (beam* b) {
    ...
    if (b->beam_sound_loop.isValid ()) {
        ...
        case 1: pos = b->last_shot; break;
        }

        snd_update_3d_pos (
            b->beam_sound_loop, gamesnd_get_game_sound (bwi->beam_loop_sound),
            &pos);
    }
}
...
void beam_get_binfo (beam* b, float accuracy, int num_shots) {
    ...
    if (b->subsys != NULL) {
        ...
        beam_get_global_turret_gun_info (
            b->objp, b->subsys, &turret_point, &turret_norm, 1, &p2,
            (b->flags & BF_IS_FIGHTER_BEAM) > 0);
        ...
        if (b->flags & BF_TARGETING_COORDS) { p2 = b->target_pos1; }
        else {
            p2 = b->target->pos;
        }
    ...
    if ((model_num < 0) && !(b->flags & BF_TARGETING_COORDS)) { return; }
    ...
    case BEAM_TYPE_A:
        ...
        if (b->flags & BF_TARGETING_COORDS) {
            ...
            // get random model points, this is useful for big ships, because
            // we never miss when shooting at them
            if (Cmdline_old_collision_sys) {
                submodel_get_two_random_points (
                    model_num, 0, &b->binfo.dir_a, &b->binfo.dir_b);
            }
            else {
...
void beam_aim (beam* b) {
    ...
    if (!(b->flags & BF_TARGETING_COORDS)) {
        ...
        if (b->target == NULL) {
            ...
            if (b->type != BEAM_TYPE_C) { return; }
        }
        // get a model # to work with
        else {
            // this can happen if we fire at a target that was just destroyed
            if (beam_get_model (b->target) < 0) { return; }
        }
    }

    if (b->subsys != nullptr &&
        b->type != BEAM_TYPE_C) { // Type C beams don't use this information.
        int temp_int = b->subsys->turret_next_fire_pos;

        if (!(b->flags & BF_IS_FIGHTER_BEAM))
            b->subsys->turret_next_fire_pos = b->firingpoint;

        if (b->subsys->system_info
                ->flags[Model::Subsystem_Flags::Share_fire_direction]) {
            beam_get_global_turret_gun_info (
                b->objp, b->subsys, &b->last_start, &temp, 0, nullptr,
                (b->flags & BF_IS_FIGHTER_BEAM) != 0);
        }
        else {
            // where the shot is originating from (b->last_start gets filled
            // in)
            beam_get_global_turret_gun_info (
                b->objp, b->subsys, &b->last_start, &temp, 1, &p2,
                (b->flags & BF_IS_FIGHTER_BEAM) != 0);
        }
    ...
    case BEAM_TYPE_A:
        ...
        if (b->target_subsys != nullptr) {
            vm_vec_unrotate (
                &b->last_shot, &b->target_subsys->system_info->pnt,
                &b->target->orient);
            vm_vec_add2 (&b->last_shot, &b->target->pos);

            if ((b->subsys != nullptr) &&
                (b->subsys->system_info
                     ->flags[Model::Subsystem_Flags::Share_fire_direction])) {
        ...
        if ((b->target != nullptr) && (b->target->type == OBJ_SHIP) &&
            (Ship_info[Ships[b->target->instance].ship_info_index]
                 .is_big_or_huge ())) {
            if ((b->subsys != nullptr) &&
                (b->subsys->system_info
                     ->flags[Model::Subsystem_Flags::Share_fire_direction])) {
        ...
        if (b->flags & BF_TARGETING_COORDS) {
            if ((b->subsys != nullptr) &&
                (b->subsys->system_info
                     ->flags[Model::Subsystem_Flags::Share_fire_direction])) {
                beam_get_global_turret_gun_info (
                    b->objp, b->subsys, &b->last_start, &temp, 0,
                    &b->target_pos1, (b->flags & BF_IS_FIGHTER_BEAM) != 0);
            ...
            if ((b->subsys != nullptr) &&
                ...
                beam_get_global_turret_gun_info (
                    b->objp, b->subsys, &b->last_start, &temp, 0,
                    &b->target->pos, (b->flags & BF_IS_FIGHTER_BEAM) != 0);
    ...
    case BEAM_TYPE_B:
        if ((b->subsys != nullptr) &&
            ...
            beam_get_global_turret_gun_info (
                b->objp, b->subsys, &b->last_start, &temp, 0, &b->binfo.dir_a,
                (b->flags & BF_IS_FIGHTER_BEAM) != 0);
            ...
            // set the shot point
            vm_vec_scale_add (
                &b->last_shot, &b->last_start, &b->binfo.dir_a, b->range);
        }
        Assert (is_valid_vec (&b->last_shot));
        break;
    ...
    case BEAM_TYPE_C:
        ...
        vm_vec_add2 (&b->last_start, &b->objp->pos);
        vm_vec_scale_add (
            &b->last_shot, &b->last_start, &b->objp->orient.vec.fvec,
            b->range);
        break;
...
void beam_get_octant_points (
    ...
    t1 = oct_array[oct_index][2] ? m->octants[oct_array[oct_index][0]].max
                                 : m->octants[oct_array[oct_index][0]].min;
    t2 = oct_array[oct_index][3] ? m->octants[oct_array[oct_index][1]].max
                                 : m->octants[oct_array[oct_index][1]].min;
...
void beam_jitter_aim (beam* b, float aim) {
    ...
    if ((b->objp != NULL) && (b->objp->signature == b->sig) &&
        (b->objp->type == OBJ_SHIP) && (b->objp->instance >= 0) &&
        (b->objp->instance < MAX_SHIPS)) {
        // get subsytem strength
        subsys_strength = ship_get_subsystem_strength (
            &Ships[b->objp->instance], SUBSYSTEM_WEAPONS);
    ...
    vm_vec_random_in_circle (
        &circle, &b->last_shot, &m, aim * b->target->radius, 0);
...
// collide a beam with a ship, returns 1 if we can ignore all future collisions
...
int beam_collide_ship (obj_pair* pair) {
    ...
    // if the "warming up" timestamp has not expired
    if ((b->warmup_stamp != -1) || (b->warmdown_stamp != -1)) { return 0; }

    // if the beam is on "safety", don't collide with anything
    if (b->flags & BF_SAFETY) { return 0; }

    // if the colliding object is the shooting object, return 1 so this is
    // culled
    if (pair->b == b->objp) { return 1; }

    // try and get a model
    model_num = beam_get_model (pair->b);
    if (model_num < 0) { return 1; }
    ...
    if ((pair->b->type != OBJ_SHIP) || (pair->b->instance < 0)) return 1;
    ship_objp = pair->b;
    shipp = &Ships[ship_objp->instance];

    if (reject_due_collision_groups (weapon_objp, ship_objp)) return 0;
    ...
    // Goober5000 - I tried to make collision code much saner... here begin the
    // (major) changes
    mc_info_init (&mc);
    ...
    // reverse this vector so that we check for exit holes as opposed to
    // entrance holes
    ...
    int shield_collision =
        (pm->shield.ntris > 0) ? model_collide (&mc_shield) : 0;
    int hull_enter_collision = model_collide (&mc_hull_enter);
    int hull_exit_collision = (beam_will_tool_target (b, ship_objp))
                                  ? model_collide (&mc_hull_exit)
                                  : 0;

    // If we have a range less than the "far" range, check if the ray actually
    ...
    if (b->range < BEAM_FAR_LENGTH &&
        ...
        // actually make sure that the collision points are within range of our
        // beam
        if (shield_collision &&
            vm_vec_dist_squared (&b->last_start, &mc_shield.hit_point_world) >
                rangeSq) {
            shield_collision = 0;
        }

        if (hull_enter_collision &&
            vm_vec_dist_squared (
                &b->last_start, &mc_hull_enter.hit_point_world) > rangeSq) {
            hull_enter_collision = 0;
        }

        if (hull_exit_collision &&
            vm_vec_dist_squared (
                &mc_hull_exit.hit_point_world, &b->last_start) > rangeSq) {
            hull_exit_collision = 0;
        }
    }

    // check shields for impact
    // (tooled ships are probably not going to be maintaining a shield over
    // their exit hole, therefore we need only check the entrance, just as with
    ...
    if (!(ship_objp->flags[Object::Object_Flags::No_shields])) {
        ...
        if (shield_collision)
            ...
            hull_enter_collision &&
            (sip->flags[Ship::Info_Flags::Surface_shields]))
        ...
        if ((quadrant_num >= 0) &&
            ((shipp->flags[Ship::Ship_Flags::Dying]) ||
             !ship_is_shield_up (ship_objp, quadrant_num)))
            quadrant_num = -1;
        ...
        if (quadrant_num >= 0) {
            ...
            if (shield_collision) {
                if (mc_shield.shield_hit_tri != -1) {
                    add_shield_point (
                        OBJ_INDEX (ship_objp), mc_shield.shield_hit_tri,
                        &mc_shield.hit_point);
                }
            }
            else {
            ...
            // if this weapon pierces the shield, then do the hit effect, but
            // act like a shield collision never occurred; otherwise, we have a
            // valid hit on this shield
    ...
    if (valid_hit_occurred) {
        ...
        Script_system.SetHookObjects (
            4, "Ship", ship_objp, "Beam", weapon_objp, "Self", ship_objp,
            "Object", weapon_objp);
        bool ship_override =
            Script_system.IsConditionOverride (CHA_COLLIDEBEAM, ship_objp);
        ...
        if (!(weapon_override && !ship_override))
            Script_system.RunCondition (
                CHA_COLLIDEBEAM, '\0', NULL, ship_objp);
...
// collide a beam with an asteroid, returns 1 if we can ignore all future
// collisions between the 2 objects
int beam_collide_asteroid (obj_pair* pair) {
    ...
    if (test_collide.num_hits) {
        ...
        Script_system.SetHookObjects (
            4, "Beam", pair->a, "Asteroid", pair->b, "Self", pair->a, "Object",
            pair->b);
        bool weapon_override =
            Script_system.IsConditionOverride (CHA_COLLIDEASTEROID, pair->a);

        Script_system.SetHookObjects (2, "Self", pair->b, "Object", pair->a);
        bool asteroid_override =
            Script_system.IsConditionOverride (CHA_COLLIDEBEAM, pair->b);
        ...
        if (!(asteroid_override && !weapon_override))
            Script_system.RunCondition (
                CHA_COLLIDEASTEROID, '\0', NULL, pair->a);

        Script_system.SetHookObjects (2, "Self", pair->b, "Object", pair->a);
        if ((asteroid_override && !weapon_override) ||
            (!asteroid_override && !weapon_override))
...
// collide a beam with a missile, returns 1 if we can ignore all future
// collisions between the 2 objects
int beam_collide_missile (obj_pair* pair) {
    ...
    if (test_collide.num_hits) {
        ...
        Script_system.SetHookObjects (
            4, "Beam", pair->a, "Weapon", pair->b, "Self", pair->a, "Object",
            pair->b);
        bool a_override =
            Script_system.IsConditionOverride (CHA_COLLIDEWEAPON, pair->a);
        ...
        if (!(b_override && !a_override)) {
            Script_system.SetHookObjects (
                4, "Beam", pair->a, "Weapon", pair->b, "Self", pair->a,
                "Object", pair->b);
            Script_system.RunCondition (
                CHA_COLLIDEWEAPON, '\0', NULL, pair->a);
        }
        if ((b_override && !a_override) || (!b_override && !a_override)) {
            // Should be reversed
            Script_system.SetHookObjects (
                4, "Weapon", pair->b, "Beam", pair->a, "Self", pair->b,
                "Object", pair->a);
            Script_system.RunCondition (CHA_COLLIDEBEAM, '\0', NULL, pair->b);
        }
...
// collide a beam with debris, returns 1 if we can ignore all future collisions
// between the 2 objects
int beam_collide_debris (obj_pair* pair) {
    ...
    if (test_collide.num_hits) {
        Script_system.SetHookObjects (
            4, "Beam", pair->a, "Debris", pair->b, "Self", pair->a, "Object",
            pair->b);
        bool weapon_override =
            Script_system.IsConditionOverride (CHA_COLLIDEDEBRIS, pair->a);

        Script_system.SetHookObjects (2, "Self", pair->b, "Object", pair->a);
        bool debris_override =
            Script_system.IsConditionOverride (CHA_COLLIDEBEAM, pair->b);
        ...
        if (!(debris_override && !weapon_override))
            Script_system.RunCondition (
                CHA_COLLIDEDEBRIS, '\0', NULL, pair->a);

        Script_system.SetHookObjects (2, "Self", pair->b, "Object", pair->a);
        if ((debris_override && !weapon_override) ||
            (!debris_override && !weapon_override))
            Script_system.RunCondition (CHA_COLLIDEBEAM, '\0', NULL, pair->b);
...
// early-out function for when adding object collision pairs, return 1 if the
// pair should be ignored
int beam_collide_early_out (object* a, object* b) {
    ...
    // get the beam
    Assert (a->instance >= 0);
    if (a->instance < 0) { return 1; }
    Assert (a->type == OBJ_BEAM);
    if (a->type != OBJ_BEAM) { return 1; }
    Assert (Beams[a->instance].objnum == OBJ_INDEX (a));
    if (Beams[a->instance].objnum != OBJ_INDEX (a)) { return 1; }
    bm = &Beams[a->instance];
    Assert (bm->weapon_info_index >= 0);
    if (bm->weapon_info_index < 0) { return 1; }
    bwi = &Weapon_info[bm->weapon_info_index];

    // if the second object has an invalid instance, bail
    if (b->instance < 0) { return 1; }

    if ((vm_vec_dist (&bm->last_start, &b->pos) - b->radius) >
        bwi->b_info.range) {
        return 1;
    } // if the object is too far away, don't bother trying to colide with
      // it-Bobboau
    ...
    case OBJ_DEBRIS:
        ...
        if (!Debris[b->instance].is_hull) { return 1; }
        break;
    case OBJ_WEAPON:
        // targeting lasers only hit ships
        /*		if(bwi->b_info.beam_type == BEAM_TYPE_C){
        ...
        if (The_mission.ai_profile
            ...
            if ((Weapon_info[Weapons[b->instance].weapon_info_index]
                     .weapon_hitpoints <= 0) &&
                (Weapon_info[Weapons[b->instance].weapon_info_index].subtype ==
                 WP_LASER)) {
                return 1;
            }
        }
            ...
            if (Weapon_info[Weapons[b->instance].weapon_info_index].subtype ==
                WP_LASER) {
                return 1;
            }
        }
    ...
    if ((vm_vec_dot (&dot_test, &dot_test2) < cull_dot) &&
        (vm_vec_mag_squared (&dist_test) > cull_dist)) {
        return 1;
    }
...
// add a collision to the beam for this frame (to be evaluated later)
// Goober5000 - erg.  Rearranged for clarity, and also to fix a bug that caused
// is_exit_collision to hardly ever be assigned, resulting in "tooled" ships
// taking twice as much damage (in a later function) as they should.
void beam_add_collision (
    beam* b, object* hit_object, mc_info* cinfo, int quadrant_num,
    int exit_flag) {
    ...
    // I guess we can always just remove the farthest item
        ...
        for (idx = 0; idx < MAX_FRAME_COLLISIONS; idx++) {
            if ((bc == NULL) ||
                (b->f_collisions[idx].cinfo.hit_dist > bc->cinfo.hit_dist))
    ...
    if (quadrant_num >= 0) hud_shield_quadrant_hit (hit_object, quadrant_num);
}

// sort collisions for the frame
bool beam_sort_collisions_func (
    const beam_collision& b1, const beam_collision& b2) {
...
void beam_handle_collisions (beam* b) {
    ...
    if (b->f_collision_count <= 0) { return; }
    ...
    // the first thing we need to do is sort the collisions, from closest to
    // farthest
    std::sort (
    ...
    for (idx = 0; idx < b->f_collision_count; idx++) {
        ...
        if (Viewer_obj == &Objects[target]) draw_effects = 0;
        ...
        for (s_idx = 0; s_idx < b->r_collision_count; s_idx++) {
            if ((r_coll[r_coll_count].c_objnum ==
                 b->r_collisions[s_idx].c_objnum) &&
        ...
        // if the physics timestamp has expired or is not set yet, apply
        // physics
        ...
        if (first_hit && (wi->impact_snd.isValid ())) {
            snd_play_3d (
                gamesnd_get_game_sound (wi->impact_snd),
                &b->f_collisions[idx].cinfo.hit_point_world, &Eye_position);
        }
        ...
        if (draw_effects &&
            ...
            vm_vec_sub (
                &temp_pos, &b->f_collisions[idx].cinfo.hit_point_world,
                &Objects[target].pos);
            vm_vec_rotate (
                &temp_local_pos, &temp_pos, &Objects[target].orient);

            vec3d worldNormal;
            if (Objects[target].type == OBJ_SHIP) {
                auto shipp = &Ships[Objects[target].instance];
                model_instance_find_world_dir (
                    &worldNormal, &b->f_collisions[idx].cinfo.hit_normal,
                    shipp->model_instance_num,
                    b->f_collisions[idx].cinfo.submodel_num,
                    &Objects[target].orient);
            }
            else {
                // Just assume that we don't need to handle model subobjects
                // here
                vm_vec_unrotate (
                    &worldNormal, &b->f_collisions[idx].cinfo.hit_normal,
                    &Objects[target].orient);
            ...
            if (wi->flash_impact_weapon_expl_effect.isValid ()) {
                auto particleSource =
                    particle::ParticleManager::get ()->createSource (
                        wi->flash_impact_weapon_expl_effect);
                particleSource.moveToObject (
                    &Objects[target], &temp_local_pos);
            ...
            if (wi->piercing_impact_effect.isValid ()) {
                ...
                if (!IS_VEC_NULL (&fvec)) {
                    ...
                    if (beam_will_tool_target (b, &Objects[target])) {
                        ...
                        if (Objects[target].type == OBJ_SHIP) {
                            ...
                            if (shipp->armor_type_idx != -1) {
                                if (Armor_types[shipp->armor_type_idx]
                                        .GetPiercingType (
                                            wi->damage_type_idx) ==
                        ...
                        if (Objects[target].type == OBJ_SHIP) {
                            ...
                            hull_pct = Objects[target].hull_strength /
                            ...
                            if (shipp->armor_type_idx != -1) {
                                piercing_type =
                                    Armor_types[shipp->armor_type_idx]
                                        .GetPiercingType (dmg_type_idx);
                    ...
                    if (ok_to_draw) {
                        ...
                        if (widest <=
                            Objects[target].radius * BEAM_AREA_PERCENT) {
                            auto particleSource =
                                particle::ParticleManager::get ()
                                    ->createSource (
                                        wi->piercing_impact_effect);
                            particleSource.moveTo (
                                &b->f_collisions[idx].cinfo.hit_point_world);
                            particleSource.setOrientationFromNormalizedVec (
                                &fvec);
                            particleSource.setOrientationNormal (&worldNormal);
            ...
            if (draw_effects && apply_beam_physics && !physics_paused) {
                ...
                if ((wi->impact_weapon_expl_effect.isValid ()) &&
                    (b->f_collisions[idx].quadrant < 0)) {
                    vec3d worldNormal;
                    if (Objects[target].type == OBJ_SHIP) {
                        auto shipp = &Ships[Objects[target].instance];
                        model_instance_find_world_dir (
                        ...
                        // Just assume that we don't need to handle model
                        // subobjects here
                        vm_vec_unrotate (
                            &worldNormal,
                            &b->f_collisions[idx].cinfo.hit_normal,
        ...
        if (!physics_paused) {
            ...
            case OBJ_WEAPON:
                if (The_mission.ai_profile
                    ...
                    Assert (
                        Weapon_info[Weapons[Objects[target].instance]
                                        .weapon_info_index]
                            .subtype == WP_MISSILE);
            ...
            case OBJ_SHIP:
                ...
                // only apply damage if the collision is not an exit collision.
                // this prevents twice the damage from being done, although it
                // probably be more realistic since two holes are being punched
                // in the ship instead of one.
                if (!b->f_collisions[idx].is_exit_collision) {
                    real_damage = beam_get_ship_damage (b, &Objects[target]) *
                                  damage_time_mod;
                    ship_apply_local_damage (
                        &Objects[target], &Objects[b->objnum],
                        &b->f_collisions[idx].cinfo.hit_point_world,
                        real_damage, b->f_collisions[idx].quadrant);
                }
                // if this is the first hit on the player ship. whack him
                if (apply_beam_physics) {
                    // Goober5000 - AGH!  BAD BAD BAD BAD BAD BAD BAD BAD bug!
                    // The whack's hit point is in *local* coordinates, NOT
                    // world coordinates!
                    beam_apply_whack (
                        b, &Objects[target],
                        &b->f_collisions[idx].cinfo.hit_point);
                }
                break;
            }
        }

        // if the radius of the target is somewhat close to the radius of the
        // beam, "stop" the beam here for now : if its smaller than about 1/3
        // the radius of the ship
        if (widest <= (Objects[target].radius * BEAM_AREA_PERCENT) &&
            !beam_will_tool_target (b, &Objects[target])) {
...
// FIXME - make sure we are truthfull representing the "cone" for all beam
// types get the total possible cone for a given beam in radians
float beam_get_cone_dot (beam* b) {
    switch (b->type) {
    case BEAM_TYPE_A:
    case BEAM_TYPE_C:
...
int beam_ok_to_fire (beam* b) {
    if (b->objp == NULL) { // If we don't have a firing object, none of these
                           // checks make sense.
        return 1;
    }
    ...
    if (b->subsys == NULL) { // IF we don't have a firing turret, none of these
                             // checks make sense.
        return 1;
    }
    ...
    if (!(b->flags & BF_FORCE_FIRING)) {
        ...
        if (ship_subsys_disrupted (b->subsys)) { return -1; }
        ...
        if (The_mission.ai_profile
                ->flags[AI::Profile_Flags::Force_beam_turret_fov]) {
            vec3d turret_normal;

            if (b->flags & BF_IS_FIGHTER_BEAM) {
                turret_normal = b->objp->orient.vec.fvec;
                b->subsys->system_info->flags.remove (
                    Model::Subsystem_Flags::Turret_alt_math);
            }
            else {
                vm_vec_unrotate (
                    &turret_normal, &b->subsys->system_info->turret_norm,
                    &b->objp->orient);
            }
            ...
            beam_get_global_turret_gun_info (
                b->objp, b->subsys, &turret_pos, &turret_dir, 1, &temp,
                (b->flags & BF_IS_FIGHTER_BEAM) > 0);
...
float beam_get_widest (beam* b) {
    ...
    // sanity
    Assert (b->weapon_info_index >= 0);
    if (b->weapon_info_index < 0) { return -1.0f; }

    if (b->beam_width > 0.0f) { widest = b->beam_width; }
    else {
        // lookup
        for (idx = 0;
             idx < Weapon_info[b->weapon_info_index].b_info.beam_num_sections;
             idx++) {
            if (Weapon_info[b->weapon_info_index].b_info.sections[idx].width >
                widest) {
                widest = Weapon_info[b->weapon_info_index]
                             .b_info.sections[idx]
                             .width;
            }
        }
...
void beam_apply_whack (beam* b, object* objp, vec3d* hit_point) {
    ...
    if ((b == NULL) || (objp == NULL) || (hit_point == NULL)) { return; }
    Assert (b->weapon_info_index >= 0);
    wip = &Weapon_info[b->weapon_info_index];
    Assert (
        (objp != NULL) && (objp->type == OBJ_SHIP) && (objp->instance >= 0) &&
    ...
    if ((shipp->ai_index < 0) || (shipp->ai_index >= MAX_AI_INFO)) { return; }
    ...
    if (wip->mass == 0.0f) { return; }

    // determine how big of a whack to apply
    float whack;

    // this if block was added by Bobboau to make beams whack properly while
    // preserving reverse compatibility
    if (wip->mass == 100.0f) {
        if (wip->damage < b_whack_damage) { whack = b_whack_small; }
        else {
            whack = b_whack_big;
        }
    }
    ...
    vec3d whack_dir;
    vm_vec_sub (
        &whack_dir, &b->last_shot,
        &b->last_start); // Valathil - use the beam direction as the force
                         // direction (like a high pressure water jet)
...
float beam_get_ship_damage (beam* b, object* objp) {
    ...
    if (wip->damage <= 0)
        return 0.0f; // Not much point in calculating the attenuation if the
                     // beam doesn't hurt in the first place.
    ...
    if ((b->team == Ships[objp->instance].team) &&
        (wip->damage >
         The_mission.ai_profile->beam_friendly_damage_cap[Game_skill_level])) {
        damage = The_mission.ai_profile
                     ->beam_friendly_damage_cap[Game_skill_level] *
                 attenuation;
    }
    else {
        // normal damage
...
int beam_will_tool_target (beam* b, object* objp) {
    ...
    if ((objp->instance < 0) || (objp->instance >= MAX_SHIPS)) { return 0; }

    ship* shipp = &Ships[objp->instance];
    total_strength = objp->hull_strength;

    if (shipp->armor_type_idx != -1) {
        if (Armor_types[shipp->armor_type_idx].GetPiercingType (
                wip->damage_type_idx) == SADTF_PIERCING_NONE) {
            return 0;
        }
        hp_limit = Armor_types[shipp->armor_type_idx].GetPiercingLimit (
            wip->damage_type_idx);
        if (hp_limit > 0.0f) {
            hp_pct = total_strength / shipp->ship_max_hull_strength;
            if (hp_limit >= hp_pct) return 1;
        }
    }
    ...
    // if the beam is going to apply more damage in about 1 and a half than the
    // ship can take
    damage_in_a_few_seconds =
        (TOOLTIME / (float)BEAM_DAMAGE_TIME) * wip->damage;
