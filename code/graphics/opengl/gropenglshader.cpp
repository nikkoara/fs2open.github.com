...
std::vector< opengl_vert_attrib > GL_vertex_attrib_info = {
    ...
    { opengl_vert_attrib::UVEC,
      "vertUvec",
      { { { 0.0f, 1.0f, 0.0f, 0.0f } } } },
    { opengl_vert_attrib::WORLD_MATRIX,
      "vertWorldMatrix",
      { { { 1.0f, 0.0f, 0.0f, 0.0f } } } },
};
...
static opengl_shader_type_t GL_shader_types[] = {
    { SDR_TYPE_MODEL,
      ...
      "main-f.sdr",
      "main-g.sdr",
      { opengl_vert_attrib::POSITION, opengl_vert_attrib::TEXCOORD,
        opengl_vert_attrib::NORMAL, opengl_vert_attrib::TANGENT,
        opengl_vert_attrib::MODEL_ID },
      "Model Rendering" },

    { SDR_TYPE_EFFECT_PARTICLE,
      "effect-v.sdr",
      "effect-particle-f.sdr",
      "effect-screen-g.sdr",
      { opengl_vert_attrib::POSITION, opengl_vert_attrib::TEXCOORD,
        opengl_vert_attrib::RADIUS, opengl_vert_attrib::COLOR },
      "Particle Effects" },

    { SDR_TYPE_EFFECT_DISTORTION,
      "effect-distort-v.sdr",
      "effect-distort-f.sdr",
      0,
      { opengl_vert_attrib::POSITION, opengl_vert_attrib::TEXCOORD,
        opengl_vert_attrib::RADIUS, opengl_vert_attrib::COLOR },
      "Distortion Effects" },
    ...
    { SDR_TYPE_DEFERRED_CLEAR,
      "deferred-clear-v.sdr",
      "deferred-clear-f.sdr",
      0,
      { opengl_vert_attrib::POSITION },
    ...
    { SDR_TYPE_SHIELD_DECAL,
      ...
      "shield-impact-f.sdr",
      0,
      { opengl_vert_attrib::POSITION, opengl_vert_attrib::NORMAL },
      "Shield Decals" },
    ...
    { SDR_TYPE_DECAL,
      ...
      "decal-f.sdr",
      nullptr,
      { opengl_vert_attrib::POSITION, opengl_vert_attrib::WORLD_MATRIX },
      "Decal rendering" },
 ...
 * When adding a new shader variant for a shader, list all associated uniforms
 * and attributes here
 */
static opengl_shader_variant_t GL_shader_variants[] = {
    { SDR_TYPE_MODEL,
      false,
      SDR_FLAG_MODEL_LIGHT,
      "FLAG_LIGHT",
      {},
      "Lighting" },
    ...
    { SDR_TYPE_DECAL,
      false,
      SDR_FLAG_DECAL_USE_NORMAL_MAP,
      "USE_NORMAL_MAP",
      {},
      "Decal use scene normal map" },
};

static const int GL_num_shader_variants =
    sizeof (GL_shader_variants) / sizeof (opengl_shader_variant_t);

opengl_shader_t* Current_shader = NULL;

opengl_shader_t::opengl_shader_t ()
    : shader (SDR_TYPE_NONE), flags (0), flags2 (0) {}
opengl_shader_t::opengl_shader_t (opengl_shader_t&& other) SCP_NOEXCEPT {
    *this = std::move (other);
}
opengl_shader_t& opengl_shader_t::
operator= (opengl_shader_t&& other) SCP_NOEXCEPT {
    // VS2013 doesn't support implicit move constructors so we need to
...
void opengl_shader_set_current (opengl_shader_t* shader_obj) {
    if (Current_shader != shader_obj) {
        ...
        if (shader_obj) { shader_obj->program->use (); }
        else {
...
size_t opengl_get_shader_idx (shader_type shader_t, unsigned int flags) {
    for (size_t idx = 0; idx < GL_shader.size (); idx++) {
        if (GL_shader[idx].shader == shader_t &&
            GL_shader[idx].flags == flags) {
            return idx;
        }
    }
...
/**
 * Given a set of flags, determine whether a shader with these flags exists
 * within the GL_shader vector. If no shader with the requested flags exists,
 * attempt to compile one.
 *
...
int gr_opengl_maybe_create_shader (shader_type shader_t, unsigned int flags) {
    ...
    if (idx < GL_shader.size ()) return (int)idx;
...
/**
 * Go through GL_shader and call glDeleteObject() for all created shaders, then
 * clear GL_shader
 */
void opengl_shader_shutdown () { GL_shader.clear (); }

static std::string opengl_shader_get_header (
    shader_type type_id, int flags, bool has_geo_shader) {
    ...
    if (has_geo_shader) {
        // If there is a geometry shader then we define a special preprocessor
        // symbol to make writing shaders easier
        sflags << "#define HAS_GEOMETRY_SHADER\n";
    }

    if (type_id == SDR_TYPE_POST_PROCESS_MAIN ||
        type_id == SDR_TYPE_POST_PROCESS_LIGHTSHAFTS ||
        type_id == SDR_TYPE_POST_PROCESS_FXAA) {
        // ignore looking for variants. main post process, lightshafts, and
        // FXAA shaders need special headers to be hacked in
...
/**
 * Load a shader file from disc or from the builtin defaults in def_files.cpp
 * if none can be found. This function will also create a list of preprocessor
...
static std::string opengl_load_shader (const char* filename) {
    ...
    if (Enable_external_shaders) {
        CFILE* cf_shader =
            cfopen (filename, "rt", CFILE_NORMAL, CF_TYPE_EFFECTS);
    ...
    content.assign (
        reinterpret_cast< const char* > (def_shader.data), def_shader.size);

    return content;
}

static void handle_includes_impl (
    std::vector< std::string >& include_stack, std::stringstream& output,
    int& include_counter, const std::string& filename,
    ...
    for (std::string line; std::getline (input, line);) {
        ...
        if (include_start != std::string::npos) {
            auto first_quote =
                line.find ('"', include_start + strlen (INCLUDE_STRING));
            ...
            auto existing_name = std::find_if (
                include_stack.begin (), include_stack.end (),
                [&file_name](const std::string& str) {
            ...
            ++include_counter;
            // The second parameter defines which source string we are
            // currently working with. We keep track of how many excludes have
            // been in the file so far to specify this
            output << "#line 1 " << include_counter + 1 << "\n";

            handle_includes_impl (
                include_stack, output, include_counter, file_name,
                opengl_load_shader (file_name.c_str ()));

            // We are done with the include file so now we can return to the
            // original file
...
static std::string
handle_includes (const char* filename, const std::string& original) {
    std::stringstream output;
    std::vector< std::string > include_stack;
    auto include_counter = 0;

    handle_includes_impl (
        include_stack, output, include_counter, filename, original);

    return output.str ();
}

static std::vector< std::string > opengl_get_shader_content (
    shader_type type_id, const char* filename, int flags,
    bool has_geo_shader) {
    std::vector< std::string > parts;
    parts.push_back (
        opengl_shader_get_header (type_id, flags, has_geo_shader));

    parts.push_back (
        handle_includes (filename, opengl_load_shader (filename)));
...
void opengl_compile_shader_actual (
    ...
    if (sdr_info->geo != NULL) {
        for (int i = 0; i < GL_num_shader_variants; ++i) {
            ...
            if (variant->type_id == sdr && flags & variant->flag &&
                variant->use_geometry_sdr) {
                use_geo_sdr = true;
                break;
            }
        }
    ...
    if (use_geo_sdr) {
        ...
        geom_content = opengl_get_shader_content (
    ...
    for (auto& attr : sdr_info->attributes) {
        new_shader.program->initAttribute (
            GL_vertex_attrib_info[attr].name,
            GL_vertex_attrib_info[attr].attribute_id,
            GL_vertex_attrib_info[attr].default_value);
    }

    for (auto& uniform_block : GL_uniform_blocks) {
        auto blockIndex = glGetUniformBlockIndex (
            new_shader.program->getShaderHandle (), uniform_block.name);

        if (blockIndex != GL_INVALID_INDEX) {
            glUniformBlockBinding (
                new_shader.program->getShaderHandle (), blockIndex,
                static_cast< GLuint > (uniform_block.block_type));
        }
    }
    ...
    for (int i = 0; i < GL_num_shader_variants; ++i) {
        ...
        if (sdr_info->type_id == variant.type_id && variant.flag & flags) {
            for (auto& attr : variant.attributes) {
                ...
                new_shader.program->initAttribute (
                    attr_info.name, attr_info.attribute_id,
...
void gr_opengl_recompile_all_shaders (
    ...
    for (auto sdr = GL_shader.begin (); sdr != GL_shader.end (); ++sdr) {
        if (progress_callback)
            progress_callback (
                std::distance (GL_shader.begin (), sdr), GL_shader.size ());
        sdr->program.reset ();
...
static void opengl_purge_shader_cache_type (const char* ext) {
    ...
    // Previously the cache files were stored in the mod directory. Since we
    // have a better system now, those files should be cleaned out. This is
    // only needed if we have a mod directory since otherwise we would delete
    // the actual cache files
    if (Cmdline_mod != nullptr && strlen (Cmdline_mod) > 0) {
        std::vector< std::string > cache_files;
        cf_get_file_list (
            cache_files, CF_TYPE_CACHE, filter.c_str (), CF_SORT_NONE, nullptr,
            CF_LOCATION_TYPE_PRIMARY_MOD | CF_LOCATION_TYPE_SECONDARY_MODS);

        for (auto& file : cache_files) {
            cf_delete (
                (file + "." + ext).c_str (), CF_TYPE_CACHE,
                CF_LOCATION_TYPE_PRIMARY_MOD |
    ...
    const auto TIMEOUT = 2.0 * 30.0 * 24.0 * 60.0 *
                         60.0; // purge timeout in seconds which is ~2 months
...
/**
 * Initializes the shader system. Creates a 1x1 texture that can be used as a
 * fallback texture when framebuffer support is missing. Also compiles the
 * shaders used for particle rendering.
 */
void opengl_shader_init () {
    ...
    glTexImage2D (
        GL_TEXTURE_2D, 0, GL_RGBA8, 1, 1, 0, GL_BGRA,
    ...
    gr_opengl_maybe_create_shader (
        SDR_TYPE_EFFECT_PARTICLE, SDR_FLAG_PARTICLE_POINT_GEN);
...
void opengl_shader_set_passthrough (bool textured) {
    ...
    Current_shader->program->Uniforms.setUniformf (
        "alphaThreshold", GL_alpha_threshold);
...
void opengl_shader_set_default_material (
    ...
    if (clip_plane.enabled) {
        ...
        clip_equation.xyzw.w =
            -vm_vec_dot (&clip_plane.normal, &clip_plane.position);

        Current_shader->program->Uniforms.setUniform4f (
            "clipEquation", clip_equation);
        Current_shader->program->Uniforms.setUniformMatrix4f (
            "modelMatrix", gr_model_matrix_stack.get_transform ());
    }
    else {
        Current_shader->program->Uniforms.setUniformi ("clipEnabled", 0);
    }
