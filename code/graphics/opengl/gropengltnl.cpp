...
static opengl_vertex_bind GL_array_binding_data[] = {
    { vertex_format_data::POSITION4, 4, GL_FLOAT, GL_FALSE,
      opengl_vert_attrib::POSITION },
    { vertex_format_data::POSITION3, 3, GL_FLOAT, GL_FALSE,
      opengl_vert_attrib::POSITION },
    { vertex_format_data::POSITION2, 2, GL_FLOAT, GL_FALSE,
      opengl_vert_attrib::POSITION },
    { vertex_format_data::SCREEN_POS, 2, GL_INT, GL_FALSE,
      opengl_vert_attrib::POSITION },
    { vertex_format_data::COLOR3, 3, GL_UNSIGNED_BYTE, GL_TRUE,
      opengl_vert_attrib::COLOR },
    { vertex_format_data::COLOR4, 4, GL_UNSIGNED_BYTE, GL_TRUE,
      opengl_vert_attrib::COLOR },
    { vertex_format_data::COLOR4F, 4, GL_FLOAT, GL_FALSE,
      opengl_vert_attrib::COLOR },
    { vertex_format_data::TEX_COORD2, 2, GL_FLOAT, GL_FALSE,
      opengl_vert_attrib::TEXCOORD },
    { vertex_format_data::TEX_COORD3, 3, GL_FLOAT, GL_FALSE,
      opengl_vert_attrib::TEXCOORD },
    { vertex_format_data::NORMAL, 3, GL_FLOAT, GL_FALSE,
      opengl_vert_attrib::NORMAL },
    { vertex_format_data::TANGENT, 4, GL_FLOAT, GL_FALSE,
      opengl_vert_attrib::TANGENT },
    { vertex_format_data::MODEL_ID, 1, GL_FLOAT, GL_FALSE,
      opengl_vert_attrib::MODEL_ID },
    { vertex_format_data::RADIUS, 1, GL_FLOAT, GL_FALSE,
      opengl_vert_attrib::RADIUS },
    { vertex_format_data::UVEC, 3, GL_FLOAT, GL_FALSE,
...
static GLenum convertComparisionFunction (ComparisionFunction func) {
    GLenum mode;
    switch (func) {
    case ComparisionFunction::Always: mode = GL_ALWAYS; break;
    case ComparisionFunction::Equal: mode = GL_EQUAL; break;
    case ComparisionFunction::Greater: mode = GL_GREATER; break;
    case ComparisionFunction::GreaterOrEqual: mode = GL_GEQUAL; break;
    case ComparisionFunction::Less: mode = GL_LESS; break;
    case ComparisionFunction::LessOrEqual: mode = GL_LEQUAL; break;
...
void gr_opengl_update_buffer_data (int handle, size_t size, void* data) {
    ...
    if (size <= buffer_obj.size && buffer_obj.type == GL_UNIFORM_BUFFER) {
        // Uniform buffer can use unsychronized buffer mapping since those are
        // always synchronized by us We also don't care about the previous data
        // and tell OpenGL to map the buffer unsynchronized.
        auto ptr = glMapBufferRange (
            buffer_obj.type, 0, size,
...
void gr_opengl_update_buffer_data_offset (
...
void gr_opengl_delete_buffer (int handle) {
    ...
    switch (buffer_obj.type) {
    case GL_ARRAY_BUFFER: GL_state.Array.BindArrayBuffer (0); break;
    case GL_ELEMENT_ARRAY_BUFFER: GL_state.Array.BindElementBuffer (0); break;
...
int gr_opengl_create_buffer (BufferType type, BufferUsageHint usage) {
    return opengl_create_buffer_object (
        convertBufferType (type), convertUsageHint (usage));
}
...
void gr_opengl_bind_uniform_buffer (
    ...
    glBindBufferRange (
        GL_UNIFORM_BUFFER, static_cast< GLuint > (bind_point), buffer_handle,
...
int opengl_create_texture_buffer_object () {
    ...
    int buffer_object_handle =
        opengl_create_buffer_object (GL_TEXTURE_BUFFER, GL_DYNAMIC_DRAW);
...
void gr_opengl_update_transform_buffer (void* data, size_t size) {
    if (Transform_buffer_handle < 0 || size <= 0) { return; }

    gr_opengl_update_buffer_data (Transform_buffer_handle, size, data);

    opengl_buffer_object& buffer_obj =
        GL_buffer_objects[Transform_buffer_handle];

    // need to rebind the buffer object to the texture buffer after it's been
    // updated. didn't have to do this on AMD and Nvidia drivers but Intel
    // drivers seem to want it.
    glBindTexture (GL_TEXTURE_BUFFER, buffer_obj.texture);
    glTexBuffer (GL_TEXTURE_BUFFER, GL_RGBA32F, buffer_obj.buffer_id);
}

GLuint opengl_get_transform_buffer_texture () {
    if (Transform_buffer_handle < 0) { return 0; }
...
void opengl_tnl_init () {
    ...
    if (Cmdline_shadow_quality) {
        ...
        opengl_set_object_label (
            GL_TEXTURE, Shadow_map_depth_texture, "Scene shadow depth map");
        ...
        // glTexParameteri(GL_TEXTURE_2D_ARRAY_EXT, GL_DEPTH_TEXTURE_MODE_ARB,
        // GL_INTENSITY);
        int size = (Cmdline_shadow_quality == 2 ? 1024 : 512);
        glTexImage3D (
            GL_TEXTURE_2D_ARRAY, 0, GL_DEPTH_COMPONENT32, size, size, 4, 0,
            GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
        // glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32, size, size, 0,
        // GL_DEPTH_COMPONENT, GL_FLOAT, NULL);

        glFramebufferTexture (
            GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, Shadow_map_depth_texture, 0);
        // glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
        // GL_TEXTURE_2D, Shadow_map_depth_texture, 0);
        ...
        opengl_set_object_label (
            GL_TEXTURE, Shadow_map_texture, "Scene shadow map");
        ...
        // glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F_ARB, size, size, 0,
        // GL_RGBA, GL_UNSIGNED_INT_8_8_8_8_REV, NULL);

        glFramebufferTexture (
            GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, Shadow_map_texture, 0);
        // glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
        // GL_TEXTURE_2D, Shadow_map_texture, 0);
...
void opengl_render_model_program (
    model_material* material_info, indexed_vertex_source* vert_source,
    vertex_buffer* bufferp, buffer_data* datap) {
    GL_state.Texture.SetShaderMode (GL_TRUE);

    opengl_tnl_set_model_material (material_info);

    GLubyte* ibuffer =
        reinterpret_cast< GLubyte* > (vert_source->Index_offset);

    GLenum element_type = (datap->flags & VB_FLAG_LARGE_INDEX)
                              ? GL_UNSIGNED_INT
                              : GL_UNSIGNED_SHORT;
    ...
    opengl_bind_vertex_layout (
        bufferp->layout,
        opengl_buffer_get_id (GL_ARRAY_BUFFER, vert_source->Vbuffer_handle),
        opengl_buffer_get_id (
            GL_ELEMENT_ARRAY_BUFFER, vert_source->Ibuffer_handle));

    // If GL_ARB_gpu_shader5 is supprted then the instancing is handled by the
    // geometry shader
    if (!GLAD_GL_ARB_gpu_shader5 && Rendering_to_shadow_map) {
        glDrawElementsInstancedBaseVertex (
            GL_TRIANGLES, (GLsizei)datap->n_verts, element_type,
            ibuffer + datap->index_offset, 4,
            (GLint) (
                vert_source->Base_vertex_offset + bufferp->vertex_num_offset));
    }
    else {
        if (Cmdline_drawelements) {
            glDrawElementsBaseVertex (
                ...
                (GLint) (
                    ...
                    bufferp->vertex_num_offset));
        }
        else {
            glDrawRangeElementsBaseVertex (
                GL_TRIANGLES, datap->i_first, datap->i_last,
                (GLsizei)datap->n_verts, element_type,
                ibuffer + datap->index_offset,
                (GLint) (
                    vert_source->Base_vertex_offset +
                    bufferp->vertex_num_offset));
...
void opengl_create_perspective_projection_matrix (
    matrix4* out, float left, float right, float bottom, float top,
...
void opengl_create_orthographic_projection_matrix (
    matrix4* out, float left, float right, float bottom, float top,
    float near_dist, float far_dist) {
    memset (out, 0, sizeof (matrix4));
...
void gr_opengl_shadow_map_start (
    matrix4* shadow_view_matrix, const matrix* light_orient) {
    if (!Cmdline_shadow_quality) return;
...
void opengl_tnl_set_material (
    material* material_info, bool set_base_map, bool set_clipping) {
    ...
    if (set_clipping) {
        // Only set the clipping state if explicitly requested by the caller to
    ...
    GL_state.StencilFunc (
        convertComparisionFunction (stencilFunc.compare), stencilFunc.ref,
        stencilFunc.mask);

    auto& frontStencilOp = material_info->get_front_stencil_op ();
    GL_state.StencilOpSeparate (
        GL_FRONT, convertStencilOp (frontStencilOp.stencilFailOperation),
    ...
    GL_state.StencilOpSeparate (
        GL_BACK, convertStencilOp (backStencilOp.stencilFailOperation),
        convertStencilOp (backStencilOp.depthFailOperation),
        convertStencilOp (backStencilOp.successOperation));

    GL_state.StencilTest (
        material_info->is_stencil_enabled () ? GL_TRUE : GL_FALSE);

    auto& color_mask = material_info->get_color_mask ();
    GL_state.ColorMask (
        color_mask.x, color_mask.y, color_mask.z, color_mask.w);
    ...
    if (Current_shader->shader == SDR_TYPE_DEFAULT_MATERIAL) {
        opengl_shader_set_default_material (
            base_map >= 0,
            material_info->get_texture_type () == TCACHE_TYPE_AABITMAP, &clr,
            material_info->get_color_scale (), array_index,
...
void opengl_tnl_set_model_material (model_material* material_info) {
    ...
    if (GL_state.CullFace ()) { GL_state.FrontFaceValue (GL_CW); }
    ...
    if (Current_shader->flags & SDR_FLAG_MODEL_DIFFUSE_MAP) {
        Current_shader->program->Uniforms.setUniformi (
            "sBasemap", render_pass);

        gr_opengl_tcache_set (
            material_info->get_texture_map (TM_BASE_TYPE), TCACHE_TYPE_NORMAL,
            &u_scale, &v_scale, &array_index, render_pass);
        ++render_pass;
    }

    if (Current_shader->flags & SDR_FLAG_MODEL_GLOW_MAP) {
        Current_shader->program->Uniforms.setUniformi (
            "sGlowmap", render_pass);

        gr_opengl_tcache_set (
            material_info->get_texture_map (TM_GLOW_TYPE), TCACHE_TYPE_NORMAL,
            &u_scale, &v_scale, &array_index, render_pass);
    ...
    if (Current_shader->flags & SDR_FLAG_MODEL_SPEC_MAP) {
        Current_shader->program->Uniforms.setUniformi (
            "sSpecmap", render_pass);

        if (material_info->get_texture_map (TM_SPEC_GLOSS_TYPE) > 0) {
            gr_opengl_tcache_set (
                material_info->get_texture_map (TM_SPEC_GLOSS_TYPE),
                TCACHE_TYPE_NORMAL, &u_scale, &v_scale, &array_index,
                render_pass);
        }
        else {
            gr_opengl_tcache_set (
                material_info->get_texture_map (TM_SPECULAR_TYPE),
                TCACHE_TYPE_NORMAL, &u_scale, &v_scale, &array_index,
                render_pass);
        }
        ++render_pass;
    ...
    if (Current_shader->flags & SDR_FLAG_MODEL_NORMAL_MAP) {
        Current_shader->program->Uniforms.setUniformi (
            "sNormalmap", render_pass);

        gr_opengl_tcache_set (
            material_info->get_texture_map (TM_NORMAL_TYPE),
            TCACHE_TYPE_NORMAL, &u_scale, &v_scale, &array_index, render_pass);
    ...
    if (Current_shader->flags & SDR_FLAG_MODEL_HEIGHT_MAP) {
        Current_shader->program->Uniforms.setUniformi (
            "sHeightmap", render_pass);

        gr_opengl_tcache_set (
            material_info->get_texture_map (TM_HEIGHT_TYPE),
            TCACHE_TYPE_NORMAL, &u_scale, &v_scale, &array_index, render_pass);
    ...
    if (Current_shader->flags & SDR_FLAG_MODEL_AMBIENT_MAP) {
        Current_shader->program->Uniforms.setUniformi (
            "sAmbientmap", render_pass);

        gr_opengl_tcache_set (
            material_info->get_texture_map (TM_AMBIENT_TYPE),
            TCACHE_TYPE_NORMAL, &u_scale, &v_scale, &array_index, render_pass);
    ...
    if (Current_shader->flags & SDR_FLAG_MODEL_MISC_MAP) {
        Current_shader->program->Uniforms.setUniformi (
            "sMiscmap", render_pass);

        gr_opengl_tcache_set (
            material_info->get_texture_map (TM_MISC_TYPE), TCACHE_TYPE_NORMAL,
            &u_scale, &v_scale, &array_index, render_pass);
    ...
    if (Current_shader->flags & SDR_FLAG_MODEL_SHADOWS) {
        Current_shader->program->Uniforms.setUniformi (
            "shadow_map", render_pass);

        GL_state.Texture.Enable (
            render_pass, GL_TEXTURE_2D_ARRAY, Shadow_map_texture);
    ...
    if (Current_shader->flags & SDR_FLAG_MODEL_ANIMATED) {
        Current_shader->program->Uniforms.setUniformi (
            "sFramebuffer", render_pass);

        if (Scene_framebuffer_in_frame) {
            GL_state.Texture.Enable (
                render_pass, GL_TEXTURE_2D, Scene_effect_texture);
            glDrawBuffer (GL_COLOR_ATTACHMENT0);
        }
        else {
            GL_state.Texture.Enable (
                render_pass, GL_TEXTURE_2D, Framebuffer_fallback_texture_id);
        }
    ...
    if (Current_shader->flags & SDR_FLAG_MODEL_TRANSFORM) {
        Current_shader->program->Uniforms.setUniformi (
            "transform_tex", render_pass);
        GL_state.Texture.Enable (
            render_pass, GL_TEXTURE_BUFFER,
            opengl_get_transform_buffer_texture ());
...
void opengl_tnl_set_material_particle (particle_material* material_info) {
    ...
    Current_shader->program->Uniforms.setUniformMatrix4f (
    ...
    Current_shader->program->Uniforms.setUniformi (
        "blend_alpha",
        material_info->get_blend_mode () != ALPHA_BLEND_ADDITIVE);
...
void opengl_tnl_set_material_batched (batched_bitmap_material* material_info) {
    // This material assumes that the array index is supplied via the vertex
    // attributes
    opengl_tnl_set_material (material_info, true);

    Current_shader->program->Uniforms.setUniformf (
        "intensity", material_info->get_color_scale ());

    Current_shader->program->Uniforms.setUniform4f (
...
void opengl_tnl_set_material_distortion (distortion_material* material_info) {
    ...
    if (material_info->get_thruster_rendering ()) {
        GL_state.Texture.Enable (
            3, GL_TEXTURE_2D, Distortion_texture[!Distortion_switch]);
...
void opengl_tnl_set_material_decal (decal_material* material_info) {
    ...
    if (gr_opengl_tcache_set (
            material_info->get_texture_map (TM_GLOW_TYPE),
            material_info->get_texture_type (), &u_scale, &v_scale,
            &array_index, 1)) {
...
void opengl_bind_vertex_component (
    const vertex_format_data& vert_component, size_t base_offset) {
    opengl_vertex_bind& bind_info =
        GL_array_binding_data[vert_component.format_type];
    ...
    if (Current_shader != NULL) {
        // grabbing a vertex attribute is dependent on what current shader has
        // been set. i hope no one calls opengl_bind_vertex_layout before
        // opengl_set_current_shader
        GLint index = opengl_shader_get_attribute (attrib_info.attribute_id);

        if (index >= 0) {
            GL_state.Array.EnableVertexAttrib (index);
            GL_state.Array.VertexAttribPointer (
                index, bind_info.size, bind_info.data_type,
                bind_info.normalized, (GLsizei)vert_component.stride,
                data_src);
        }
    }
}
...
void opengl_bind_dynamic_layout (vertex_layout& layout, size_t base_offset) {
    ...
    for (size_t i = 0; i < num_vertex_bindings; ++i) {
        opengl_bind_vertex_component (
            *layout.get_vertex_component (i), base_offset);
    }
...
void opengl_bind_vertex_array (const vertex_layout& layout) {
    ...
    for (size_t i = 0; i < layout.get_num_vertex_components (); ++i) {
        ...
        glVertexAttribFormat (
            attribIndex, bind_info.size, bind_info.data_type,
            bind_info.normalized, static_cast< GLuint > (component->offset));
...
void opengl_bind_vertex_layout (
    vertex_layout& layout, GLuint vertexBuffer, GLuint indexBuffer,
    size_t base_offset) {
    GR_DEBUG_SCOPE ("Bind vertex layout");

    if (!GLAD_GL_ARB_vertex_attrib_binding) {
        // We don't have support for the new vertex binding functions so fall
        // back to the single VAO implementation
    ...
    GL_state.Array.BindVertexBuffer (
        0, vertexBuffer, static_cast< GLintptr > (base_offset),
