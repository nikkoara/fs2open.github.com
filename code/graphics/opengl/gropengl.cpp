...
void gr_opengl_set_clip (int x, int y, int w, int h, int resize_mode) {
    ...
    if (to_resize) { gr_resize_screen_pos (&x, &y, &w, &h, resize_mode); }
    else {
    ...
    if (GL_rendering_to_texture) { glScissor (x, y, w, h); }
    else {
...
int gr_opengl_set_color_buffer (int mode) {
    ...
    if (mode) { enabled = GL_state.ColorMask (true, true, true, true); }
    else {
...
int gr_opengl_zbuffer_get () {
    if (!gr_global_zbuffering) { return GR_ZBUFF_NONE; }
...
int gr_opengl_alpha_mask_set (int mode, float alpha) {
    if (mode) { GL_alpha_threshold = alpha; }
    else {
...
int gr_opengl_save_screen () {
    ...
    if (Use_PBOs) {
        ...
        sptr =
            (ubyte*)&opengl_screen_tmp[gr_screen.max_w * gr_screen.max_h * 4];
...
void gr_opengl_set_line_width (float width) {
    if (width <= 1.0f) { GL_state.SetLineWidth (width); }
...
gr_opengl_create_viewport (const os::ViewPortProperties& props) {
    ...
    attrs.pixel_format.alpha_size =
        (gr_screen.bits_per_pixel == 32) ? Gr_alpha.bits : 0;
...
void opengl_setup_function_pointers () {
    ...
    gr_screen.gf_update_buffer_data_offset =
        gr_opengl_update_buffer_data_offset;
    ...
    gr_screen.gf_post_process_restore_zbuffer =
        gr_opengl_post_process_restore_zbuffer;
    ...
    gr_screen.gf_render_primitives_particle =
        gr_opengl_render_primitives_particle;
    gr_screen.gf_render_primitives_batched =
        gr_opengl_render_primitives_batched;
    gr_screen.gf_render_primitives_distortion =
        gr_opengl_render_primitives_distortion;
...
static void APIENTRY debug_callback (
    ...
    case GL_DEBUG_SEVERITY_HIGH_ARB:
        severityStr = "High";
        print_to_general_log = true; // High priority messages are sent to the
                                     // normal log for later troubleshooting
        break;
...
static bool printNextDebugMessage () {
    if (!hasPendingDebugMessage ()) { return false; }
...
bool gr_opengl_init (std::unique_ptr< os::GraphicsOperations >&& graphicsOps) {
    ...
    if (Cmdline_fullscreen_window || Cmdline_window) { opengl_go_windowed (); }
    else {
        opengl_go_fullscreen ();
    }
...
bool gr_opengl_is_capable (gr_capability capability) {
    ...
    case CAPABILITY_TIMESTAMP_QUERY:
        return GLAD_GL_ARB_timer_query !=
               0; // Timestamp queries are available from 3.3 onwards
    case CAPABILITY_SEPARATE_BLEND_FUNCTIONS:
        return GLAD_GL_ARB_draw_buffers_blend !=
               0; // We need an OpenGL extension for this
    }
...
DCF (ogl_minimize, "Minimizes opengl") {
    ...
    if (minimize_ogl) { opengl_minimize (); }
}
