...
gr_zbuffer_type
material_determine_depth_mode (bool depth_testing, bool blending) {
    if (depth_testing) {
        if (blending) { return ZBUFFER_TYPE_READ; }
...
void material_set_unlit (
    material* mat_info, int texture, float alpha, bool blending,
    bool depth_testing) {
    mat_info->set_texture_map (TM_BASE_TYPE, texture);

    gr_alpha_blend blend_mode =
        material_determine_blend_mode (texture, blending);
    gr_zbuffer_type depth_mode =
        material_determine_depth_mode (depth_testing, blending);
...
void material_set_unlit_emissive (
    material* mat_info, int texture, float alpha, float color_scale) {
    material_set_unlit (mat_info, texture, alpha, true, true);

    mat_info->set_color_scale (color_scale);
}

void material_set_unlit_color (
    material* mat_info, int texture, color* clr, bool blending,
    bool depth_testing) {
    mat_info->set_texture_map (TM_BASE_TYPE, texture);
...
void material_set_unlit_color (
    material* mat_info, int texture, color* clr, float alpha, bool blending,
    bool depth_testing) {
...
void material_set_interface (
    material* mat_info, int texture, bool blended, float alpha) {
    mat_info->set_texture_map (TM_BASE_TYPE, texture);
    mat_info->set_texture_type (material::TEX_TYPE_INTERFACE);

    gr_alpha_blend blend_mode =
        material_determine_blend_mode (texture, blended);
...
void material_set_unlit_volume (
    particle_material* mat_info, int texture, bool point_sprites) {
...
void material_set_distortion (
    distortion_material* mat_info, int texture, bool thruster) {
...
void material_set_movie (
    movie_material* mat_info, int y_bm, int u_bm, int v_bm) {
    mat_info->set_depth_mode (ZBUFFER_TYPE_NONE);
    mat_info->set_blend_mode (ALPHA_BLEND_ALPHA_BLEND_ALPHA);
...
void material_set_batched_bitmap (
    batched_bitmap_material* mat_info, int base_tex, float alpha,
    float color_scale) {
...
void material_set_nanovg (nanovg_material* mat_info, int base_tex) {
    ...
    mat_info->set_front_stencil_op (
        StencilOperation::Keep, StencilOperation::Keep,
        StencilOperation::Keep);
    mat_info->set_back_stencil_op (
        StencilOperation::Keep, StencilOperation::Keep,
        StencilOperation::Keep);
}

void material_set_decal (
    material* mat_info, int diffuse_tex, int glow_tex, int normal_tex) {
    mat_info->set_depth_mode (ZBUFFER_TYPE_READ);

    mat_info->set_blend_mode (
        0, material_determine_blend_mode (diffuse_tex, true));
    mat_info->set_blend_mode (
        1, ALPHA_BLEND_ADDITIVE); // Normal blending must always be additive
    mat_info->set_blend_mode (
        2, material_determine_blend_mode (glow_tex, true));
    ...
    // Done't write the alpha channel in any case since that will cause changes
    // to other material properties
    // TODO: If decals should be able to change more than just the material
    // diffuse color then a solution with separate blending equations must be
    // used
    mat_info->set_color_mask (true, true, true, false);
}

material::material ()
    : Sdr_type (SDR_TYPE_DEFAULT_MATERIAL), Tex_type (TEX_TYPE_NORMAL),
      Texture_addressing (TMAP_ADDRESS_WRAP), Depth_mode (ZBUFFER_TYPE_NONE),
      Blend_mode (ALPHA_BLEND_ALPHA_BLEND_ALPHA), Cull_mode (true),
      Fill_mode (GR_FILL_MODE_SOLID), Clr_scale (1.0f), Depth_bias (0) {
    Clr.xyzw.x = 1.0f;
    Clr.xyzw.y = 1.0f;
    Clr.xyzw.z = 1.0f;
    Clr.xyzw.w = 1.0f;
...
uint material::get_shader_flags () const { return 0; }
...
bool material::is_textured () const {
    return Texture_maps[TM_BASE_TYPE] > -1 ||
           Texture_maps[TM_GLOW_TYPE] > -1 ||
           Texture_maps[TM_SPECULAR_TYPE] > -1 ||
           Texture_maps[TM_NORMAL_TYPE] > -1 ||
           Texture_maps[TM_HEIGHT_TYPE] > -1 ||
...
void material::set_texture_type (texture_type t_type) { Tex_type = t_type; }

int material::get_texture_type () const {
    switch (Tex_type) {
    default:
    case TEX_TYPE_NORMAL: return TCACHE_TYPE_NORMAL;
    case TEX_TYPE_XPARENT: return TCACHE_TYPE_XPARENT;
    case TEX_TYPE_INTERFACE: return TCACHE_TYPE_INTERFACE;
    case TEX_TYPE_AABITMAP: return TCACHE_TYPE_AABITMAP;
    }
}

bool material::is_clipped () const { return Clip_params.enabled; }
...
void material::set_clip_plane () { Clip_params.enabled = false; }
...
int material::get_texture_addressing () const { return Texture_addressing; }

void material::set_depth_mode (gr_zbuffer_type mode) { Depth_mode = mode; }

gr_zbuffer_type material::get_depth_mode () const { return Depth_mode; }

void material::set_cull_mode (bool mode) { Cull_mode = mode; }

bool material::get_cull_mode () const { return Cull_mode; }

void material::set_fill_mode (int mode) { Fill_mode = mode; }

int material::get_fill_mode () const { return Fill_mode; }
...
bool material::has_buffer_blend_modes () const { return Has_buffer_blends; }
...
void material::set_depth_bias (int bias) { Depth_bias = bias; }

int material::get_depth_bias () const { return Depth_bias; }
...
const vec4& material::get_color () const { return Clr; }

void material::set_color_scale (float scale) { Clr_scale = scale; }

float material::get_color_scale () const { return Clr_scale; }
void material::set_stencil_test (bool stencil) { Stencil_test = stencil; }
bool material::is_stencil_enabled () const { return Stencil_test; }
void material::set_stencil_mask (uint32_t mask) { Stencil_mask = mask; }
uint32_t material::get_stencil_mask () const { return Stencil_mask; }
void material::set_stencil_func (
    ComparisionFunction compare, int ref, uint32_t mask) {
...
void material::set_stencil_op (
    StencilOperation stencilFailOperation, StencilOperation depthFailOperation,
    StencilOperation successOperation) {
    set_front_stencil_op (
        stencilFailOperation, depthFailOperation, successOperation);
    set_back_stencil_op (
        stencilFailOperation, depthFailOperation, successOperation);
}
void material::set_front_stencil_op (
    StencilOperation stencilFailOperation, StencilOperation depthFailOperation,
    StencilOperation successOperation) {
...
void material::set_back_stencil_op (
    StencilOperation stencilFailOperation, StencilOperation depthFailOperation,
    StencilOperation successOperation) {
...
const bvec4& material::get_color_mask () const { return Color_mask; }

model_material::model_material () : material () {
    set_shader_type (SDR_TYPE_MODEL);
}

void model_material::set_desaturation (bool enabled) { Desaturate = enabled; }

bool model_material::is_desaturated () const { return Desaturate; }

void model_material::set_shadow_casting (bool enabled) {
    Shadow_casting = enabled;
}

bool model_material::is_shadow_casting () const { return Shadow_casting; }

void model_material::set_shadow_receiving (bool enabled) {
    Shadow_receiving = enabled;
}

bool model_material::is_shadow_receiving () const { return Shadow_receiving; }

void model_material::set_light_factor (float factor) { Light_factor = factor; }

float model_material::get_light_factor () const { return Light_factor; }

void model_material::set_lighting (bool mode) { lighting = mode; }

bool model_material::is_lit () const { return lighting; }

void model_material::set_deferred_lighting (bool enabled) {
    Deferred = enabled;
}

void model_material::set_high_dynamic_range (bool enabled) { HDR = enabled; }

void model_material::set_center_alpha (int c_alpha) { Center_alpha = c_alpha; }

int model_material::get_center_alpha () const { return Center_alpha; }

void model_material::set_thrust_scale (float scale) { Thrust_scale = scale; }

float model_material::get_thrust_scale () const { return Thrust_scale; }
...
void model_material::set_team_color () { Team_color_set = false; }

const team_color& model_material::get_team_color () const { return Tm_color; }
...
int model_material::get_animated_effect () const { return Animated_effect; }

float model_material::get_animated_effect_time () const {
    return Animated_timer;
}

void model_material::set_batching (bool enabled) { Batched = enabled; }

bool model_material::is_batched () const { return Batched; }
...
void model_material::set_normal_alpha () { Normal_alpha = false; }

bool model_material::is_normal_alpha_active () const { return Normal_alpha; }
...
void model_material::set_fog () { Fog_params.enabled = false; }

bool model_material::is_fogged () const { return Fog_params.enabled; }
...
uint model_material::get_shader_flags () const {
    ...
    if (is_clipped ()) { Shader_flags |= SDR_FLAG_MODEL_CLIP; }

    if (is_batched ()) { Shader_flags |= SDR_FLAG_MODEL_TRANSFORM; }

    if (Shadow_casting) {
        // if we're building the shadow map, we likely only need the flags here
        // and above so bail
    ...
    if (is_fogged ()) { Shader_flags |= SDR_FLAG_MODEL_FOG; }

    if (Animated_effect >= 0) { Shader_flags |= SDR_FLAG_MODEL_ANIMATED; }
    ...
    if ((get_texture_map (TM_SPECULAR_TYPE) > 0 ||
         get_texture_map (TM_SPEC_GLOSS_TYPE) > 0) &&
        !Specmap_override) {
    ...
    if (get_texture_map (TM_MISC_TYPE) > 0) {
        ...
        if (Team_color_set) { Shader_flags |= SDR_FLAG_MODEL_TEAMCOLOR; }
    }

    if (Deferred) { Shader_flags |= SDR_FLAG_MODEL_DEFERRED; }

    if (HDR) { Shader_flags |= SDR_FLAG_MODEL_HDR; }

    if (Thrust_scale > 0.0f) { Shader_flags |= SDR_FLAG_MODEL_THRUSTER; }

    if (Normal_alpha) { Shader_flags |= SDR_FLAG_MODEL_NORMAL_ALPHA; }
...
particle_material::particle_material () : material () {
...
bool particle_material::get_point_sprite_mode () { return Point_sprite; }

uint particle_material::get_shader_flags () const {
    uint flags = 0;

    if (Point_sprite) { flags |= SDR_FLAG_PARTICLE_POINT_GEN; }

    return flags;
}

distortion_material::distortion_material () : material () {
...
bool distortion_material::get_thruster_rendering () { return Thruster_render; }

shield_material::shield_material () : material () {
...
const matrix& shield_material::get_impact_orient () { return Impact_orient; }

const vec3d& shield_material::get_impact_pos () { return Impact_pos; }

float shield_material::get_impact_radius () { return Impact_radius; }

movie_material::movie_material () : material () {
    set_shader_type (SDR_TYPE_VIDEO_PROCESS);
}
int movie_material::getYtex () const { return Ytex; }
int movie_material::getUtex () const { return Utex; }
int movie_material::getVtex () const { return Vtex; }
void movie_material::setYtex (int _Ytex) { this->Ytex = _Ytex; }
void movie_material::setUtex (int _Utex) { this->Utex = _Utex; }
void movie_material::setVtex (int _Vtex) { movie_material::Vtex = _Vtex; }

batched_bitmap_material::batched_bitmap_material () {
    set_shader_type (SDR_TYPE_BATCHED_BITMAP);
}

nanovg_material::nanovg_material () { set_shader_type (SDR_TYPE_NANOVG); }

decal_material::decal_material () { set_shader_type (SDR_TYPE_DECAL); }
uint decal_material::get_shader_flags () const {
    uint flags = 0;

    if (get_texture_map (TM_NORMAL_TYPE) == -1) {
        // If we don't write to the normal map then we can use the existing
        // normal map for better normal accuracy
