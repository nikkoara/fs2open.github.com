...
class NanoVGRenderer {
    enum class CallType { Fill, ConvexFill, Stroke, Triangles };
    struct Image {
        int bitmap = -1;
        ...
        Image (Image&& other) SCP_NOEXCEPT { *this = std::move (other); }
...
public:
    ...
    void renderFill (
        NVGpaint* paint, NVGscissor* scissor, float fringe,
    ...
    void renderTriangles (
        NVGpaint* paint, NVGscissor* scissor, const NVGvertex* verts,
        int nverts);
    ...
    int updateTexture (
        int image, int x, int y, int w, int h, const unsigned char* data);
