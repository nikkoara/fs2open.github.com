...
enum NVGcreateFlags {
    ...
    // slower, but path overlaps (i.e. self-intersecting or sharp turns) will
    // be drawn just once.
...
int nvglCreateImageFromHandle (
    NVGcontext* ctx, GLuint textureId, int w, int h, int flags);
...
static int glnvg__createShader (
    GLNVGshader* shader, const char* name, const char* header,
...
#if NANOVG_GL_USE_UNIFORMBUFFER
    shader->loc[GLNVG_LOC_FRAG] =
        glGetUniformBlockIndex (shader->prog, "frag");
#else
...
static int glnvg__renderCreate (void* uptr) {
    ...
    if (gl->flags & NVG_ANTIALIAS) {
        if (glnvg__createShader (
                &gl->shader, "shader", shaderHeader, "#define EDGE_AA 1\n",
                fillVertShader, fillFragShader) == 0)
            return 0;
    }
    else {
        if (glnvg__createShader (
                &gl->shader, "shader", shaderHeader, NULL, fillVertShader,
                fillFragShader) == 0)
            return 0;
    }
...
#endif
    gl->fragSize = sizeof (GLNVGfragUniforms) + align -
...
static int glnvg__renderCreateTexture (
    ...
    if (glnvg__nearestPow2 (w) != (unsigned int)w ||
        ...
        if ((imageFlags & NVG_IMAGE_REPEATX) != 0 ||
            ...
            printf (
                "Repeat X/Y is not supported for non power-of-two textures "
                "(%d x %d)\n",
                w, h);
        ...
        if (imageFlags & NVG_IMAGE_GENERATE_MIPMAPS) {
            printf (
                "Mip-maps is not support for non power-of-two textures (%d x "
                "%d)\n",
                w, h);
...
#if defined(NANOVG_GLES2)
        glTexImage2D (
            GL_TEXTURE_2D, 0, GL_LUMINANCE, w, h, 0, GL_LUMINANCE,
...
#ifdef NANOVG_GLES2
        glTexSubImage2D (
            GL_TEXTURE_2D, 0, x, y, w, h, GL_LUMINANCE, GL_UNSIGNED_BYTE,
            data);
#else
        glTexSubImage2D (
...
static int glnvg__convertPaint (
    GLNVGcontext* gl, GLNVGfragUniforms* frag, NVGpaint* paint,
...
#if NANOVG_GL_USE_UNIFORMBUFFER
    glBindBufferRange (
        GL_UNIFORM_BUFFER, GLNVG_FRAG_BINDING, gl->fragBuf, uniformOffset,
        sizeof (GLNVGfragUniforms));
#else
    GLNVGfragUniforms* frag = nvg__fragUniformPtr (gl, uniformOffset);
    glUniform4fv (
        gl->shader.loc[GLNVG_LOC_FRAG], NANOVG_GL_UNIFORMARRAY_SIZE,
...
static void glnvg__fill (GLNVGcontext* gl, GLNVGcall* call) {
    ...
    if (gl->flags & NVG_ANTIALIAS) {
        ...
        for (i = 0; i < npaths; i++)
            glDrawArrays (
                GL_TRIANGLE_STRIP, paths[i].strokeOffset,
                paths[i].strokeCount);
    }
...
static void glnvg__stroke (GLNVGcontext* gl, GLNVGcall* call) {
    ...
    if (gl->flags & NVG_STENCIL_STROKES) {
        ...
        glnvg__setUniforms (
            gl, call->uniformOffset + gl->fragSize, call->image);
...
static void glnvg__renderFlush (void* uptr) {
    ...
    if (gl->ncalls > 0) {
        ...
        glBufferData (
            GL_UNIFORM_BUFFER, gl->nuniforms * gl->fragSize, gl->uniforms,
            GL_STREAM_DRAW);
#endif
        ...
        glBufferData (
            GL_ARRAY_BUFFER, gl->nverts * sizeof (NVGvertex), gl->verts,
            GL_STREAM_DRAW);
        glEnableVertexAttribArray (0);
        glEnableVertexAttribArray (1);
        glVertexAttribPointer (
            0, 2, GL_FLOAT, GL_FALSE, sizeof (NVGvertex),
            (const GLvoid*)(size_t)0);
        glVertexAttribPointer (
            1, 2, GL_FLOAT, GL_FALSE, sizeof (NVGvertex),
...
static void glnvg__renderFill (
    ...
    if (call->type == GLNVG_FILL) {
        ...
        // Fill shader
        glnvg__convertPaint (
            gl, nvg__fragUniformPtr (gl, call->uniformOffset), paint, scissor,
            fringe, fringe, -1.0f);
    }
...
static void glnvg__renderTriangles (
    void* uptr, NVGpaint* paint, NVGscissor* scissor, const NVGvertex* verts,
    int nverts) {
    GLNVGcontext* gl = (GLNVGcontext*)uptr;
    GLNVGcall* call = glnvg__allocCall (gl);
    GLNVGfragUniforms* frag;
    ...
    memcpy (
        &gl->verts[call->triangleOffset], verts, sizeof (NVGvertex) * nverts);
