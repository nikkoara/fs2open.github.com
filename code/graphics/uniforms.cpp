...
void convert_model_material (
    ...
    vm_matrix4_x_matrix4 (
        &data_out->modelViewMatrix, &data_out->viewMatrix,
        &data_out->modelMatrix);
    ...
    if (shader_flags & SDR_FLAG_MODEL_CLIP) {
        if (material.is_clipped ()) {
            ...
            clip_equation.xyzw.w =
                -vm_vec_dot (&clip_info.normal, &clip_info.position);
    ...
    if (shader_flags & SDR_FLAG_MODEL_LIGHT) {
        int num_lights =
            MIN (Num_active_gr_lights, (int)graphics::MAX_UNIFORM_LIGHTS);
        data_out->n_lights = num_lights;

        std::copy (
            std::begin (gr_light_uniforms), std::end (gr_light_uniforms),
            std::begin (data_out->lights));
        ...
        if (Gloss_override_set) { data_out->defaultGloss = Gloss_override; }
        else {
    ...
    if (shader_flags & SDR_FLAG_MODEL_DIFFUSE_MAP) {
        if (material.is_desaturated ()) { data_out->desaturate = 1; }
        else {
        ...
        case ALPHA_BLEND_PREMULTIPLIED: data_out->blend_alpha = 1; break;
        case ALPHA_BLEND_ADDITIVE: data_out->blend_alpha = 2; break;
        default: data_out->blend_alpha = 0; break;
        }

        data_out->sBasemapIndex =
            bm_get_array_index (material.get_texture_map (TM_BASE_TYPE));
    }
    ...
    if (shader_flags & SDR_FLAG_MODEL_GLOW_MAP) {
        ...
        data_out->sGlowmapIndex =
            bm_get_array_index (material.get_texture_map (TM_GLOW_TYPE));
    }
    ...
    if (shader_flags & SDR_FLAG_MODEL_SPEC_MAP) {
        ...
        if (material.get_texture_map (TM_SPEC_GLOSS_TYPE) > 0) {
            data_out->sSpecmapIndex = bm_get_array_index (
                material.get_texture_map (TM_SPEC_GLOSS_TYPE));

            data_out->gammaSpec = 1;

            if (Gloss_override_set) { data_out->alphaGloss = 0; }
            else {
            ...
            data_out->sSpecmapIndex = bm_get_array_index (
                material.get_texture_map (TM_SPECULAR_TYPE));
    ...
    if (shader_flags & SDR_FLAG_MODEL_NORMAL_MAP) {
        data_out->sNormalmapIndex =
            bm_get_array_index (material.get_texture_map (TM_NORMAL_TYPE));
    }

    if (shader_flags & SDR_FLAG_MODEL_AMBIENT_MAP) {
        data_out->sAmbientmapIndex =
            bm_get_array_index (material.get_texture_map (TM_AMBIENT_TYPE));
    }

    if (shader_flags & SDR_FLAG_MODEL_MISC_MAP) {
        data_out->sMiscmapIndex =
            bm_get_array_index (material.get_texture_map (TM_MISC_TYPE));
    }
    ...
    // Team colors are passed to the shader here, but the shader needs to
    // handle their application. By default, this is handled through the r and
    // g channels of the misc map, but this can be changed in the shader; test
    // versions of this used the normal map r and b channels
    if (shader_flags & SDR_FLAG_MODEL_TEAMCOLOR) {
        ...
        data_out->team_glow_enabled =
            bm_has_alpha_channel (material.get_texture_map (TM_MISC_TYPE)) ? 1
                                                                           : 0;
    }
