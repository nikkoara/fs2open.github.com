...
// ---------------------------------------------------------------------------------------
// input:
//			do_repeat		=>		property of button, set to 1 to allow pressed events
//if mouse 										pointer is held over button with left mouse button down, 										otherwise 0
//(useful for buttons that scroll items) 			ignore_focus	=>		whether to
//allow Enter/Spacebar to affect pressed state when 										control has focus
//
void UI_BUTTON::create (
    ...
    if (ignore_focus) { m_flags |= BF_IGNORE_FOCUS; }
...
// sets a hotkey for button that works only when it had focus (or derived
// focus)
void UI_BUTTON::set_hotkey_if_focus (int key) { hotkey_if_focus = key; }
...
// reset anything that needs to be at the start of a new frame before
// processing
...
void UI_BUTTON::draw () {
    ...
    if (uses_bmaps) {
        ...
        if (button_down ()) {
            if (bmap_ids[B_PRESSED] >= 0) { frame_num = B_PRESSED; }
            // otherwise if button is disabled, draw it that way
        }
        else if (disabled_flag) {
            if (bmap_ids[B_DISABLED] >= 0) { frame_num = B_DISABLED; }
            // otherwise, if button is highlighted (mouse is over it, but mouse
            // buttons not down) draw it that way
        }
        else if (m_flags & BF_HIGHLIGHTED) {
            if (bmap_ids[B_HIGHLIGHT] >= 0) { frame_num = B_HIGHLIGHT; }
            // otherwise, just draw it normally
        }
        else {
            if (bmap_ids[B_NORMAL] >= 0) { frame_num = B_NORMAL; }
        }
        ...
        if (disabled_flag) { gr_set_color_fast (&CDARK_GRAY); }
        ...
        if (text) {
            ui_string_centered (
                Middle (w) + offset, Middle (h) + offset, text);
        }
...
void UI_BUTTON::process (int focus) {
    ...
    if (disabled_flag) {
        if (old_flags & BF_DOWN) { m_flags |= BF_JUST_RELEASED; }
        ...
        if (mouse_on_me && B1_JUST_PRESSED) {
            if (m_disabled_function != NULL) { m_disabled_function (); }
        }
    ...
    if (my_wnd->selected_gadget == this) { focus = 1; }

    // show alternate cursor, perhaps?
    maybe_show_custom_cursor ();

    if (!mouse_on_me) { next_repeat = 0; }
    else {
        m_flags |= BF_HIGHLIGHTED;
        if (!(old_flags & BF_HIGHLIGHTED)) {
            ...
            if (m_just_highlighted_function) {
                if (m_flags & BF_SKIP_FIRST_HIGHLIGHT_CALLBACK) {
                    if (first_callback) { do_callback = 0; }
                }

                first_callback = 0;
                if (do_callback) { m_just_highlighted_function (); }
            }
        }
    }
    ...
    // only check for space/enter keystrokes if we are not ignoring the focus
    ...
    if (!(m_flags & BF_IGNORE_FOCUS)) {
        if (focus && (hotkey_if_focus >= 0)) {
            ...
            if ((hotkey_if_focus == KEY_SPACEBAR) &&
                (my_wnd->keypress == KEY_ENTER))
                m_flags |= BF_DOWN | BF_CLICKED;
        }
    }
    ...
    if (!(m_flags & BF_DOWN)) {
        ...
        if ((old_flags & BF_DOWN) &&
            !(old_flags &
              BF_CLICKED)) // check for release of mouse, not hotkey
            m_flags |= BF_JUST_RELEASED;

        // non-repeating buttons behave sort of uniquely..  They activate when
    ...
    if (!(old_flags & BF_DOWN)) {
        ...
        if (user_function) user_function ();
...
int UI_BUTTON::pressed () {
    if (m_flags & BF_CLICKED) return TRUE;
...
// ----------------------------------------------------------------------------------
// Checks if button is down (or up).  This checks the button instead, rather
// than any events that may have caused it to be down.  Buttons also stay down
// for a certain amount of time minimum, to make sure it's long enough for the
// user to see it has went down (since one frame is probably far to quick for
// users to notice it).  Basically, this indicates how the button is being
// drawn, if you want to think of it that way.
...
int UI_BUTTON::button_hilighted () { return m_flags & BF_HIGHLIGHTED; }

// Is the mouse over this button?
void UI_BUTTON::set_button_hilighted () { m_flags |= BF_HIGHLIGHTED; }
...
void UI_BUTTON::maybe_show_custom_cursor () {
    ...
    if (is_mouse_on ()) {
        if ((custom_cursor != NULL) && (previous_cursor == NULL)) {
            previous_cursor =
                io::mouse::CursorManager::get ()->getCurrentCursor ();
